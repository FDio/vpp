/*
 * STRUCTURAL EXAMPLE - IPv6 DAD Client in C
 * 
...
static vapi_error_e
dad_event_handler(vapi_ctx_t ctx, void *callback_ctx,
                  vapi_msg_ip6_dad_event *msg)
{
    ...
    case DAD_STATE_TENTATIVE:
        if (event->dad_count == 0) {
            ...
            printf("  State     : TENTATIVE (retry %u/%u)\n",
                   event->dad_count, event->dad_transmits);
        }
...
main(void)
{
    ...
    rv = vapi_connect(g_app.ctx, "dad_client", NULL, 32, 32,
    ...
    rv = vapi_want_ip6_dad_events(g_app.ctx, req,
                                   register_confirm_handler, NULL);
    if (rv != VAPI_OK) {
    ...
    while (g_app.running) {
        ...
        vapi_set_event_cb(g_app.ctx, vapi_msg_id_ip6_dad_event,
                         (vapi_event_cb)dad_event_handler, NULL);
