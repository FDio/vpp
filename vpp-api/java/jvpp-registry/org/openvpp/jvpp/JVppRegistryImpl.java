...
public final class JVppRegistryImpl implements JVppRegistry, ControlPingCallback {
    ...
    public void register(final JVpp jvpp, final JVppCallback callback) {
        ...
        jvpp.init(connection, callback, connection.getConnectionInfo().queueAddress,
    ...
    public int controlPing(final Class<? extends JVpp> clazz) throws VppInvocationException {
        ...
        // FIXME either add control ping to all callbacks or register ControlPingCallback
        // or add marker interface for callbacks and register obj that implements both marker and CPC
        final ControlPingCallback callback = (ControlPingCallback)map.get(clazz.getName());
