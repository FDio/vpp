...
#include <jvpp-core/org_openvpp_jvpp_JVppCoreImpl.h>
//#include "jvpp-core/jvpp_gen.h"


#include <jvpp/jvpp_core.h>
 ...
 *     'types': []}
 */
JNIEXPORT jint JNICALL Java_org_openvpp_jvpp_JVppCoreImpl_showVersion0
(JNIEnv * env, jclass clazz) {
    vpp_core_main_t * jm = &vpp_core_main;
    //vppjni_main_t *jm = &vppjni_main;
    ...
    // FIXME store context_id in jm
    // my_context_id = vppjni_get_context_id (jm);
    my_context_id = 123;
    printf("my_context_id=%d\n", my_context_id);


    // FIXME do we need result_ready?
    // jm->result_ready = 0;

    mp = vl_msg_api_alloc(sizeof(*mp));
    printf("vl_msg_api_alloc\n");
...
JNIEXPORT void JNICALL Java_org_openvpp_jvpp_JVppCoreImpl_init0
(JNIEnv * env, jclass clazz, jlong queue_address, jint my_client_index) {
    vpp_core_main_t * jm = &vpp_core_main;
    jm->my_client_index = my_client_index;
    jm->vl_input_queue = (unix_shared_memory_queue_t *)queue_address;
