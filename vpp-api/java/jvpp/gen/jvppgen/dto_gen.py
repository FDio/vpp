...
def generate_dtos(func_list, base_package, plugin_package, plugin_name, dto_package, inputfile):
    ...
    for func in func_list:
        ...
        # FIXME HONEYCOMB-185 fix other code in jvpp and in HC + tests here to use constructor + getters
        methods = generate_dto_constructor(camel_case_dto_name, func)
        methods += generate_dto_getters(func)
        ...
        if not util.is_just_notification(func["name"]):
            if util.is_reply(camel_case_dto_name):
                ...
                if util.is_details(camel_case_dto_name):
                    ...
                    # FIXME HONEYCOMB-185 add constructor etc. to dump reply aggregator
...
field_getter_template = Template("""    public $type get$getter_name() {
        return $field_name;
    }\n\n""")


def generate_dto_getters(func):
    getters = ""
    for t in zip(func['types'], func['args']):
        # for retval don't generate dto field in Reply
        field_name = util.underscore_to_camelcase(t[1])
        field_name_capital = util.underscore_to_camelcase_upper(t[1])
        if util.is_retval_field(field_name):
            continue
        getters += field_getter_template.substitute(type=util.jni_2_java_type_mapping[t[0]],
                                                    getter_name=field_name_capital,
...
def generate_dto_tostring(camel_case_dto_name, func):
    ...
    return tostring_template.substitute(cls_name=camel_case_dto_name,
                                        fields_tostring=tostring_fields)
...
constructor_field_template = Template("""        this.$field_name = $field_name;\n""")
constructor_template = Template("""    public $cls_name($params) {
$assignments    }\n\n""")


def generate_dto_constructor(camel_case_dto_name, func):
    params = ""
    assignments = ""
    for t in zip(func['types'], func['args']):
        field_name = util.underscore_to_camelcase(t[1])
        ...
        params += "%s %s, " % (util.jni_2_java_type_mapping[t[0]], field_name)
        assignments += constructor_field_template.substitute(field_name=field_name)

    return constructor_template.substitute(cls_name=camel_case_dto_name,
                                           params=params[:-2],
...
def generate_dump_reply_dto(request_dto_name, base_package, plugin_package, dto_package, camel_case_dto_name,
                            camel_case_method_name, func, methods):
