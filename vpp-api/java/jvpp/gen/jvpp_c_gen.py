...
def generate_jni_impl(func_list):
    jni_impl = []
    for f in func_list:
        f_name = f['name']
        if is_manually_generated(f_name) or util.is_reply(f_name) or util.is_ignored(f_name) or util.is_notification(f_name):
            continue
...
def generate_msg_handlers(func_list):
    handlers = []
    for f in func_list:
        handler_name = f['name']

        if is_manually_generated(handler_name) or not util.is_reply(handler_name) or util.is_ignored(handler_name) or util.is_notification(handler_name):
...
def generate_handler_registration(func_list):
    ...
    for f in func_list:
        ...
        if not util.is_reply(name) or util.is_ignored(name) or util.is_notification(name):
            # TODO handle notifications
            continue
...
def generate_jvpp(func_list):
    """ Generates jvpp C file """
    print "Generating jvpp C"

    jni_impl = generate_jni_impl(func_list)
    msg_handlers = generate_msg_handlers(func_list)
    ...
    jvpp_c_file.write(jvpp_c_template.substitute(
            jni_implementations=jni_impl, msg_handlers=msg_handlers, handler_registration=handler_registration))
    jvpp_c_file.flush()
    jvpp_c_file.close()
