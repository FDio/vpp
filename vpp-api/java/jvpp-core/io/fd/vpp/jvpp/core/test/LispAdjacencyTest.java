...
public class LispAdjacencyTest {
    ...
    private static void addRemoteMapping(final FutureJVppCoreFacade jvpp) throws ExecutionException, InterruptedException {
    ...
    private static void showAdjacencies(final FutureJVppCoreFacade jvpp) throws ExecutionException, InterruptedException {
        final LispAdjacenciesGet request = new LispAdjacenciesGet();
        request.vni = 0;
        final LispAdjacenciesGetReply reply = jvpp.lispAdjacenciesGet(request).toCompletableFuture().get();
        LOG.info("Lisp adjacency created successfully:" + reply.toString());
        for (LispAdjacency adjacency : reply.adjacencies) {
            // TODO add toString to types

            LOG.info("eidType = " + adjacency.eidType);
            LOG.info("reid = " + Arrays.toString(adjacency.reid));
            LOG.info("leid = " + Arrays.toString(adjacency.leid));
            LOG.info("reidPrefixLen = " + adjacency.reidPrefixLen);
            LOG.info("leidPrefixLen = " + adjacency.leidPrefixLen);
            LOG.info("Lisp adjacency created successfully:" + reply.toString());
        }
