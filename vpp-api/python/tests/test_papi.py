...
VL_API_SW_INTERFACE_SET_FLAGS = vpp_papi.plugin_name_to_id('vpe', 'VL_API_SW_INTERFACE_SET_FLAGS')
VL_API_VNET_INTERFACE_COUNTERS = vpp_papi.plugin_name_to_id('vpe', 'VL_API_VNET_INTERFACE_COUNTERS')
VL_API_VNET_IP6_FIB_COUNTERS = vpp_papi.plugin_name_to_id('vpe', 'VL_API_VNET_IP6_FIB_COUNTERS')
def papi_event_handler(result):
    if result.vl_msg_id == VL_API_SW_INTERFACE_SET_FLAGS:
        papi_event.set()
        return
    if result.vl_msg_id == VL_API_VNET_INTERFACE_COUNTERS:
        print('Result', result)
        format = '>' + str(int(len(result.data) / 8)) + 'Q'
        counters = struct.unpack(format, result.data)
        print('Counters:', counters)
        return
    if result.vl_msg_id == VL_API_VNET_IP6_FIB_COUNTERS:
