...
def make_exe():
    ...
    bzl_root = "/" + glob(["pyoxidizer.bzl"], strip_prefix="/").paths()[0].replace("pyoxidizer.bzl", "")
    test_src_dir = bzl_root + "../../../test/"
    ...
    testfiles = glob([test_src_dir + "*.py"], strip_prefix=test_src_dir)
    mypackages = []
    for f in testfiles.paths():
	    print("FIRSTPACKAGE", f)
	    mypackages.append(f.replace(".py", ""))
    ...
    python_config.run_module = "run_tests"
    ...
    # exe.add_python_resources(exe.pip_download(["mypy"]))
	...
	path="build-root/install-vpp-native/vpp/lib/python3.8/site-packages/",
	packages=["vpp_papi"],
    ))

    # testfiles = glob(["test/*.py"], strip_prefix="/home/ubuntu/vpp/test/")
    testfiles = glob([test_src_dir + "*.py"], strip_prefix=test_src_dir)
    mypackages = []
    for f in testfiles.paths():
	    print("FIRSTPACKAGE", f)
    ...
    exe.add_python_resources(exe.read_package_root(
        path="test",
	packages=mypackages #testfiles.paths()
    ))
    # Invoke `pip install` using a requirements file and add the collected resources
    # to our binary.
    # exe.add_python_resources(exe.pip_install(["-r", "../../../test/requirements-3.txt"]))
    exe.add_python_resources(exe.pip_install(["-r", "test/requirements-3.txt"]))
...
def make_install(exe):
    ...
    # files.add_manifest(glob(["../../../test/*/**"], strip_prefix="../../../../test"))
    ...
    files.add_python_resources("vpptestfiles", exe.read_package_root(
        path="test",
        packages=mypackages
    ))
