...
tap_interface::event_handler::handle_populate(const client_db::key_t& key)
{
   ...
   */
  std::shared_ptr<tap_interface_cmds::tap_dump_cmd> cmd =
  ...
  for (auto& record : *cmd) {
    ...
    tap_interface itf(name, interface::type_t::TAP,
                      interface::admin_state_t::UP, route::prefix_t::ZERO);
   ...
   */
  std::shared_ptr<tap_interface_cmds::tapv2_dump_cmd> cmdv2 =
    std::make_shared<tap_interface_cmds::tapv2_dump_cmd>();

  HW::enqueue(cmdv2);
  HW::write();

  for (auto& record : *cmdv2) {
    auto& payload = record.get_payload();

    std::string name = reinterpret_cast<const char*>(payload.dev_name);

    /* FIXME: Add support for retrive IP address */
    tap_interface itf(name, interface::type_t::TAPV2,
                      interface::admin_state_t::UP, route::prefix_t::ZERO);

    VOM_LOG(log_level_t::DEBUG) << "tapv2-dump: " << itf.to_string();
