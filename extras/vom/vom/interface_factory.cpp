...
interface_factory::new_interface(const vapi_payload_sw_interface_details& vd)
{
  ...
  std::string device_type = reinterpret_cast<const char*>(vd.interface_dev_type);
  ...
  const vapi_payload_sw_interface_vhost_user_details& vd)
{
  ...
  sp = interface(name, interface::type_t::VHOST, interface::admin_state_t::DOWN).singular();
  sp->set(hdl);
  return (sp);
}
