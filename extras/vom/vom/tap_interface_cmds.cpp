...
tapv2_create_cmd::issue(connection& con)
{
  ...
  memcpy(payload.host_if_name.buf, m_name.c_str(),
  	 payload.host_if_name.length);
  payload.host_if_name_set = 1;

  if (m_prefix != route::prefix_t::ZERO) {
    if (m_prefix.address().is_v6()) {
      m_prefix.to_vpp((uint8_t *)&payload.host_ip6_prefix_set, payload.host_ip6_prefix.prefix,
                      &payload.host_ip6_prefix.len);
    } else {
      m_prefix.to_vpp((uint8_t *)&payload.host_ip4_prefix_set, payload.host_ip4_prefix.prefix,
