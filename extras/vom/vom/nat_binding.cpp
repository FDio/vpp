...
nat_binding::event_handler::handle_populate(const client_db::key_t& key)
{
  ...
  for (auto& record : *icmd) {
    ...
    if (itf) {
        ...
        VOM_LOG(log_level_t::ERROR) << "nat-binding-input-44 no sw_if_index: " << payload.sw_if_index;
    }
  }
  ...
  for (auto& record : *ocmd) {
    ...
    if (itf) {
        ...
        VOM_LOG(log_level_t::ERROR) << "nat-binding-output-44 no sw_if_index: " << payload.sw_if_index;
    }
  }
  ...
  for (auto& record : *i6cmd) {
    ...
    if (itf) {
        ...
        VOM_LOG(log_level_t::ERROR) << "nat-binding-input-66 no sw_if_index: " << payload.sw_if_index;
    }
  }
}
