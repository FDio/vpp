...
l2_xconnect::sweep()
{
  if (m_xconnect && handle_t::INVALID != m_east_itf->handle() &&
      handle_t::INVALID != m_west_itf->handle()) {
    HW::enqueue(new l2_xconnect_cmds::unbind_cmd(
      m_xconnect, m_east_itf->handle(), m_west_itf->handle()));
  }
...
l2_xconnect::replay()
{
  if (m_xconnect && handle_t::INVALID != m_east_itf->handle() &&
      handle_t::INVALID != m_west_itf->handle()) {
    HW::enqueue(new l2_xconnect_cmds::bind_cmd(m_xconnect, m_east_itf->handle(),
                                               m_west_itf->handle()));
  }
}
...
l2_xconnect::to_string() const
{
  ...
  s << "L2-xconnect:[" << m_east_itf->to_string() << " "
    << m_west_itf->to_string() << " " << m_xconnect.to_string() << "]";
