...
igmp_binding::igmp_binding(const interface& itf,
                           ...
                           const igmp_binding::src_addrs_t& saddrs)
  : m_itf(itf.singular())
  , m_gaddr(gaddr)
  , m_saddrs(saddrs)
...
igmp_binding::sweep()
{
  if (m_binding) {
    HW::enqueue(
      new igmp_binding_cmds::unbind_cmd(m_binding, m_itf->handle(), m_gaddr));
  }
  HW::write();
}
...
{
  /*
   * no updates for the binding. chaning the interface or the group addr is a
