...
gbp_contract::gbp_contract(const gbp_route_domain &grd,
                           sclass_t sclass,
                           sclass_t dclass,
...
{
  if (m_hw) {
    HW::enqueue(new gbp_contract_cmds::create_cmd(m_hw, m_rd->id(), m_sclass,
                                                  m_dclass, m_acl->handle(),
                                                  m_gbp_rules,
...
gbp_contract::update(const gbp_contract& r)
{
   ...
   */
  if (rc_t::OK != m_hw.rc()) {
      HW::enqueue(new gbp_contract_cmds::create_cmd(m_hw, m_rd->id(), m_sclass, m_dclass,
  ...
  for (auto& record : *cmd) {
    ...
    if (acl) {
      ...
      std::shared_ptr<gbp_route_domain> grd =
          gbp_route_domain::find(payload.contract.rd_id);
      ...
      if (grd) {
        gbp_contract gbpc(*grd, payload.contract.sclass, payload.contract.dclass, *acl,
                          rules, allowed_ethertypes);
        OM::commit(key, gbpc);
        VOM_LOG(log_level_t::DEBUG) << "read: " << gbpc.to_string();
      } else {
          VOM_LOG(log_level_t::ERROR) << " no RD:" << payload.contract.rd_id;
      }
    } else {
