...
stat_reader::stat_indexes_t* stat_reader::m_stat_indexes;

stat_reader::stat_reader()
{
  m_client = new stat_client();
  m_stat_indexes = new stat_reader::stat_indexes_t();
...
stat_reader::~stat_reader()
{
  delete m_stat_indexes;
  delete m_client;
}

void
stat_reader::register_stat_indexes(uint32_t index)
{
  m_stat_indexes->insert(index);
}

void
stat_reader::unregister_stat_indexes(uint32_t index)
{
  m_stat_indexes->erase(index);
}

void
stat_reader::get_stats()
{
    ...
    switch (sde.get_stat_segment_type()) {
      ...
      case STAT_DIR_TYPE_COUNTER_VECTOR_COMBINED:
        for (int k = 0; k < m_client->stat_client_vec_len(
             ...
             k++) {
          for (auto& i : *m_stat_indexes) {
            count.packets +=
              sde.get_stat_segment_combined_counter_data()[k][i].packets;
            count.bytes +=
              sde.get_stat_segment_combined_counter_data()[k][i].bytes;

            std::cout << "[" << i << " @  " << k << "] " << count.packets
                      << " packets " << count.bytes << " bytes "
                      << sde.get_stat_segment_name() << std::endl;
          }
        }
        break;
