...
static void
vpp_metric_cleanup (void)
{
  svmdb_unmap (svmdb_client);
}

static g_val_t
vpp_metric_handler (int metric_index)
{
  g_val_t val;
  pid_t *vpp_pidp;
  ...
  switch (metric_index)
	...
	val.d = 0.0;
      break;
    case 1:
      vpp_pidp = svmdb_local_get_vec_variable (svmdb_client, "vpp_pid",
					       sizeof (*vpp_pidp));
...
static Ganglia_25metric vpp_metric_info[] = {
  { 0, "Vector_Rate", 100, GANGLIA_VALUE_DOUBLE, "Packets/Frame", "both",
    "%.1f", UDP_HEADER_SIZE + 8, "VPP Vector Rate" },
  { 0, "VPP_State", 100, GANGLIA_VALUE_DOUBLE, "Run=1", "both", "%.0f",
    UDP_HEADER_SIZE + 8, "VPP State" },
  { 0, "Input_Rate", 100, GANGLIA_VALUE_DOUBLE, "5 sec RX rate", "both",
    "%.1f", UDP_HEADER_SIZE + 8, "VPP Aggregate RX Rate" },
  { 0, "Sig_Error_Rate", 100, GANGLIA_VALUE_DOUBLE,
    "5 sec significant error rate", "both", "%.1f", UDP_HEADER_SIZE + 8,
    "VPP Significant Error Rate" },
  { 0, NULL }
};

mmodule vpp_module = {
  STD_MMODULE_STUFF, vpp_metric_init,	 vpp_metric_cleanup,
  vpp_metric_info,   vpp_metric_handler,
};
