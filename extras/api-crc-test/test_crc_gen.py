...
def run_crc_test(src_dir, dst_dir, output_results=True):
    ...
    src_api_json_path = find(test_api_json_fname, ws_build_root_dir)
    
    if not len(src_api_json_path):
    ...
    copy (src_api_json_path[0], dst_api_json_path)
    ...
    for api_msg in test_api_crc:
        if test_api_crc[api_msg] != orig_api_crc[api_msg]:
            new_crc += 1
            if new_crc == 1:
                print(f'\n\t\t\t{test_name}: API Messages With New CRC\n'
                      '------------------------------------------------'
                      '-----------------------------------')
            print(f'{api_msg:50}'
    ...
    print('------------------------------------------------'
          '-----------------------------------\n'
          f'\t\t\t{test_name}: {new_crc} API Message CRCs changed!\n')
