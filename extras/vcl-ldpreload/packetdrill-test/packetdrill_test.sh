...
packetdrill_test_path=$(dirname $(readlink -f "$0"))
pd_path=$packetdrill_test_path/packetdrill
vpp_path=$(dirname $(dirname $(dirname  $packetdrill_test_path)))/build-root/install-vpp${debug}-native/vpp
...
config_veth(){
	if  $vpp_path/bin/vppctl -s $unix_sock  show  int  |grep host-vppvethout    >/dev/null 2>&1 ; then 
		...
		$vpp_path/bin/vppctl -s $unix_sock set int ip address host-vppvethout 10.10.1.2/24 >/dev/null 2>&1
...
start_vpp(){
	...
	if [  ! -e "$unix_sock" ]; then
		LD_LIBRARY_PATH=$vpp_path/lib  $vpp_path/bin/vpp -c $packetdrill_test_path/pdstartup.conf &
...
# Run packetdrill test case
run_test(){
	if [ ! -f "$pd_path/gtests/net/packetdrill/packetdrill" ]; then
		echo "packetdrill isn't build"
		exit 1
	fi
	...
	for value in ${test_cases[@]}
		do
			echo -e "\n"
		echo starting $value
		LD_PRELOAD=$vpp_path/lib/libvcl_ldpreload.so VCL_CONFIG=$packetdrill_test_path/vcl.conf $pd_path/gtests/net/packetdrill/packetdrill --local_ip=$local_ip --remote_ip=$remote_ip --bind_port=$local_port --connect_port=$remote_port --so_filename=$vpp_path/lib/libpd.so $pd_path/gtests/net/tcp/$value
		echo  ending $value
		echo -e "\n"
