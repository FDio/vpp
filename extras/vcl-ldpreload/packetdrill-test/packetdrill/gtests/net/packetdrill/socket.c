...
void
socket_close (struct state *state, struct fd_state *fd)
{
  ...
  if (socket->protocol == IPPROTO_TCP && socket->live.local.port != 0 &&
      socket->live.remote.port != 0 && !state->config->is_wire_client &&
