...
struct netdev_ops
{
  ...
  int (*send) (struct netdev *netdev, struct packet *packet);
   ...
   * with packet_free().
   */
  int (*receive) (struct netdev *netdev, struct packet **packet, char **error);
};

/* Tear down a netdev and free up the resources it has allocated. */
static inline void
netdev_free (struct netdev *netdev)
{
...
/* Inject a raw TCP/IP packet into the kernel. */
static inline int
netdev_send (struct netdev *netdev, struct packet *packet)
{
 ...
 */
static inline int
netdev_receive (struct netdev *netdev, struct packet **packet, char **error)
{
...
extern int netdev_receive_loop (struct packet_socket *psock,
				...
				struct packet **packet, int *num_packets,
				char **error);
