...
static inline struct local_netdev *
to_local_netdev (struct netdev *netdev)
{
 ...
 */
static void
cleanup_old_device (struct config *config, struct local_netdev *netdev)
{
#if defined(__NetBSD__)
  char *cleanup_command = NULL;
  int result;

  asprintf (&cleanup_command, "/sbin/ifconfig %s down delete > /dev/null 2>&1",
	    TUN_DEV);
  DEBUGP ("running: '%s'\n", cleanup_command);
 ...
 */
static void
check_remote_address (struct config *config, struct local_netdev *netdev)
{
 ...
 */
static void
check_local_anyip (struct config *config)
{
...
static void
create_device (struct config *config, struct local_netdev *netdev)
{
...
loop:
  ...
  if (config->speed != TUN_DRIVER_SPEED_CUR)
    {
      char *command;
      asprintf (&command, "ethtool -s %s speed %u autoneg off", netdev->name,
		config->speed);
      if (system (command) < 0)
	die ("Error executing %s\n", command);
      ...
      asprintf (&command,
		"ifconfig %s down; sleep 1; ifconfig %s up; "
		"sleep 1",
...
static void
set_device_offload_flags (struct local_netdev *netdev)
{
#ifdef linux
  const u32 offload = TUN_F_CSUM | TUN_F_TSO4 | TUN_F_TSO6 | TUN_F_TSO_ECN;
...
/* Bring up the device */
static void
bring_up_device (struct local_netdev *netdev)
{
  struct ifreq ifr;
  memset (&ifr, 0, sizeof (ifr));
 ...
 */
static void
route_traffic_to_device (struct config *config, struct local_netdev *netdev)
{
  ...
  asprintf (&route_command,
	    ...
	    config->is_anyip ? "" : "via ",
	    config->is_anyip ? "" : config->live_gateway_ip_string);
#endif
#if defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__NetBSD__)
...
struct netdev *
local_netdev_new (struct config *config)
{
...
static void
local_netdev_free (struct netdev *a_netdev)
{
 ...
 */
static void
bsd_tun_write (struct local_netdev *netdev, struct packet *packet)
{
  int address_family = htonl (packet_address_family (packet));
  struct iovec vector[2] = { { &address_family, sizeof (address_family) },
...
static void
linux_tun_write (struct local_netdev *netdev, struct packet *packet)
{
  struct virtio_net_hdr gso = { 0 };
  struct iovec vector[2] = { { &gso, sizeof (gso) },
			     { packet_start (packet), packet->ip_bytes } };
...
static int
local_netdev_send (struct netdev *a_netdev, struct packet *packet)
{
  struct local_netdev *netdev = to_local_netdev (a_netdev);
 ...
 */
static void
local_netdev_read_queue (struct local_netdev *netdev, int num_packets)
{
#ifdef linux
...
static int
local_netdev_receive (struct netdev *a_netdev, struct packet **packet,
		      char **error)
{
  struct local_netdev *netdev = to_local_netdev (a_netdev);
  ...
  status =
    netdev_receive_loop (netdev->psock, PACKET_LAYER_3_IP, DIRECTION_OUTBOUND,
...
int
netdev_receive_loop (struct packet_socket *psock, enum packet_layer_t layer,
		     enum direction_t direction, struct packet **packet,
		     int *num_packets, char **error)
{
