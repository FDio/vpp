...
struct packet *
new_udp_packet (int address_family, enum direction_t direction,
		struct ip_info ip_info, u16 udp_payload_bytes, u16 src_port,
		u16 dst_port, char **error)
{
  ...
  const int ip_header_bytes =
    (ip_header_min_len (address_family) + ip_option_bytes);
  const int udp_header_bytes = sizeof (struct udp);
  const int ip_bytes = ip_header_bytes + udp_header_bytes + udp_payload_bytes;
  ...
  set_packet_ip_header (packet, address_family, ip_bytes, ip_info.tos.value,
			ip_info.flow_label, ip_info.ttl, IPPROTO_UDP);

  udp_header = packet_append_header (packet, HEADER_UDP, sizeof (struct udp));
