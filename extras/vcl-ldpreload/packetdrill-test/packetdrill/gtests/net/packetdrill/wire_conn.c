...
void
wire_conn_free (struct wire_conn *conn)
{
  if (conn->fd != -1)
    close (conn->fd);
  free (conn->in.buf);
...
static void
create_tcp_socket (struct wire_conn *conn, enum ip_version_t ip_version)
{
  assert (conn->fd == -1);
...
static void
set_default_tcp_options (struct wire_conn *conn)
{
  int val;
  ...
  val = 128 * 1024;
  if (setsockopt (conn->fd, SOL_SOCKET, SO_RCVBUF, &val, sizeof (val)) < 0)
    {
  ...
  val = 128 * 1024;
  if (setsockopt (conn->fd, SOL_SOCKET, SO_SNDBUF, &val, sizeof (val)) < 0)
    {
...
void
wire_conn_connect (struct wire_conn *conn, const struct ip_address *ip,
		   u16 port, enum ip_version_t ip_version)
{
...
void
wire_conn_bind_listen (struct wire_conn *listen_conn, u16 port,
		       enum ip_version_t ip_version)
{
  ...
  val = 1;
  if (setsockopt (listen_conn->fd, SOL_SOCKET, SO_REUSEADDR, &val,
		  sizeof (val)) < 0)
    {
...
void
wire_conn_accept (struct wire_conn *listen_conn,
 ...
 */
static int
write_bytes (struct wire_conn *conn, const void *buf, int buf_len)
{
  while (buf_len > 0)
    {
...
int
wire_conn_write (struct wire_conn *conn, enum wire_op_t op, const void *buf,
		 int buf_len)
{
...
static int
read_bytes (struct wire_conn *conn, void *buf, int buf_len)
{
  while (buf_len > 0)
    {
...
int
wire_conn_read (struct wire_conn *conn, enum wire_op_t *op, void **buf,
		int *buf_len)
{
