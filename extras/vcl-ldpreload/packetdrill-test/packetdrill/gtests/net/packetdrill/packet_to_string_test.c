...
static void
test_tcp_ipv4_packet_to_string (void)
{
  /* An IPv4/GRE/IPv4/TCP packet. */
  u8 data[] = { /* IPv4: */
		0x45, 0x00, 0x00, 0x54, 0x00, 0x00, 0x00, 0x00, 0xff, 0x2f,
		0xb5, 0x75, 0x02, 0x02, 0x02, 0x02, 0x01, 0x01, 0x01, 0x01,
		/* GRE: */
		0x00, 0x00, 0x08, 0x00,
		/* IPv4, TCP: */
  ...
  assert (error == NULL);
  printf ("dump = '%s'\n", dump);
  expected = "ipv4 2.2.2.2 > 1.1.1.1: gre flags 0x0 proto 0x0800: "
...
static void
test_tcp_ipv6_packet_to_string (void)
{
  ...
  assert (error == NULL);
  printf ("dump = '%s'\n", dump);
  expected = "ipv6 2::2222 > 1::1111: gre flags 0x0 proto 0x86dd: "
...
static void
test_gre_mpls_tcp_ipv4_packet_to_string (void)
{
  ...
  assert (error == NULL);
  printf ("dump = '%s'\n", dump);
  expected = "ipv4 192.168.0.1 > 192.0.2.2: gre flags 0x0 proto 0x8847: "
...
static void
test_tcp_md5_option_to_string (void)
{
  /* An IPv4/TCP packet. */
  u8 data[] = { /* IPv4, TCP: */
		0x45, 0x00, 0x00, 0x3c, 0x00, 0x00, 0x00, 0x00, 0xff, 0x06,
		0x89, 0x56, 0xc0, 0x00, 0x02, 0x01, 0xc0, 0xa8, 0xaf, 0xbb,
		0x8a, 0x6f, 0x1f, 0x90, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
		0x00, 0x01, 0xa0, 0x02, 0x01, 0x00, 0x36, 0x14, 0x00, 0x00,
		0x13, 0x12, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x01, 0x01
  };
