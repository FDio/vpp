...
static __always_inline u32
getblock_32 (const u32 *p, int i)
{
  return p[i];
}

static __always_inline u64
getblock_64 (const u64 *p, int i)
{
  return p[i];
}
...
void
MurmurHash3_x86_32 (const void *key, int len, u32 seed, void *out)
{
...
void
MurmurHash3_x86_128 (const void *key, const int len, u32 seed, void *out)
{
  const u8 *data = (const u8 *) key;
...
void
MurmurHash3_x64_128 (const void *key, const int len, const u32 seed, void *out)
{
  const u8 *data = (const u8 *) key;
  const int nblocks = len / 16;
