...
static inline struct so_netdev *
to_so_netdev (struct netdev *netdev)
{
  return (struct so_netdev *) netdev;
}

static void
so_netdev_free (struct netdev *a_netdev)
{
...
static int
so_netdev_send (struct netdev *a_netdev, struct packet *packet)
{
  struct so_netdev *netdev = to_so_netdev (a_netdev);
...
static int
so_netdev_receive (struct netdev *a_netdev, struct packet **packet,
		   char **error)
{
  struct so_netdev *netdev = to_so_netdev (a_netdev);
...
struct netdev *
so_netdev_new (struct config *config)
{
...
struct so_instance *
so_instance_new (void)
{
  return calloc (1, sizeof (struct so_instance));
}

int
so_instance_init (struct so_instance *instance, const struct config *config,
		  const struct script *script, const struct state *state)
{
  packetdrill_interface_init_t init;
  char *error;

  instance->handle =
    dlopen (config->so_filename,
...
void
so_instance_free (struct so_instance *instance)
{
  if (!instance)
    return;
