...
void
init_script (struct script *script)
{
...
const char *
expression_type_to_string (enum expression_t type)
{
  int i = 0;
...
static bool
lookup_int_symbol (const char *input_symbol, s64 *output_integer,
...
int
symbol_to_int (const char *input_symbol, s64 *output_integer, char **error)
{
  if (lookup_int_symbol (input_symbol, output_integer, cross_platform_symbols))
    return STATUS_OK;

  if (lookup_int_symbol (input_symbol, output_integer, platform_symbols ()))
    return STATUS_OK;
 ...
 */
static const char *
flag_name (struct flag_name *flags_array, u64 flag)
{
...
char *
flags_to_string (struct flag_name *flags_array, u64 flags)
{
  u64 bit_mask = 1;
  int i = 0;
  char *out = strdup ("");
 ...
 */
static int
unescape_cstring_expression (const char *input_string, struct expression *out,
			     char **error)
{
  ...
  while (*c_in != '\0')
      ...
      if (*c_in == '\\')
	{
	  ...
	  switch (*c_in)
		...
		asprintf (error, "invalid hex escape (\\xhh): '\\x%s'", c_in);
		return STATUS_ERR;
	      }
	    default:
	      asprintf (error, "unsupported escape code: '%c'", *c_in);
	      return STATUS_ERR;
	    }
	}
      else
...
void
free_expression_list (struct expression_list *list)
{
  while (list != NULL)
    {
...
static void
concatenate_string_expressions (struct expression *out, struct expression *lhs,
...
static int
evaluate_binary_expression (struct expression *in, struct expression *out,
			    char **error)
{
  ...
  if (strcmp ("|", in->value.binary->op) == 0)
    ...
    {
      asprintf (error, "bad binary operator '%s'", in->value.binary->op);
    }
error_out:
  free_expression (rhs);
  free_expression (lhs);
...
static int
evaluate_list_expression (struct expression *in, struct expression *out,
			  char **error)
{
  ...
  out->value.list = NULL;
  return evaluate_expression_list (in->value.list, &out->value.list, error);
}

static int
evaluate_iovec_expression (struct expression *in, struct expression *out,
			   char **error)
{
...
static int
evaluate_msghdr_expression (struct expression *in, struct expression *out,
			    char **error)
{
...
static int
evaluate_cmsg_expression (struct expression *in, struct expression *out,
			  char **error)
{
...
static int
evaluate_sock_extended_err (struct expression *in, struct expression *out,
			    char **error)
{
...
static int
evaluate_pollfd_expression (struct expression *in, struct expression *out,
			    char **error)
{
...
static int
evaluate_epollev_expression (struct expression *in, struct expression *out,
			     char **error)
{
...
static int
evaluate (struct expression *in, struct expression **out_ptr, char **error)
{
  ...
  *out_ptr = out;
  ...
  switch (in->type)
    ...
    case EXPR_GRE: /* copy as-is */
      memcpy (&out->value.gre, &in->value.gre, gre_len (&in->value.gre));
      break;
    ...
    case EXPR_WORD:
      out->type = EXPR_INTEGER;
      if (symbol_to_int (in->value.buf.ptr, &out->value.num, error))
	return STATUS_ERR;
      break;
    case EXPR_STRING:
    ...
    case EXPR_SOCKET_ADDRESS_IPV4: /* copy as-is */
      out->value.socket_address_ipv4 = malloc (sizeof (struct sockaddr_in));
      memcpy (out->value.socket_address_ipv4, in->value.socket_address_ipv4,
	      sizeof (*(out->value.socket_address_ipv4)));
      break;
    case EXPR_SOCKET_ADDRESS_IPV6: /* copy as-is */
      out->value.socket_address_ipv6 = malloc (sizeof (struct sockaddr_in6));
      memcpy (out->value.socket_address_ipv6, in->value.socket_address_ipv6,
    ...
    case EXPR_CMSG:
      ...
      break;
    case EXPR_SCM_TIMESTAMPING:
      memcpy (&out->value.scm_timestamping, &in->value.scm_timestamping,
    ...
    case EXPR_MPLS_STACK: /* copy as-is */
      ...
      memcpy (out->value.mpls_stack, in->value.mpls_stack,
 ...
 */
int
evaluate_expression_list (struct expression_list *in_list,
			  struct expression_list **out_list, char **error)
{
  ...
  while (in_list != NULL)
	...
	calloc (1, sizeof (struct expression_list));
      *node_ptr = node;
      if (evaluate (in_list->expression, &node->expression, error))
	{
