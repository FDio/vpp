...
static void
checksum_ipv4_packet (struct packet *packet)
{
  ...
  if (packet->tcp != NULL)
    {
      ...
      tcp->check = 0;
      tcp->check = tcp_udp_v4_checksum (ipv4->src_ip, ipv4->dst_ip,
  ...
  else if (packet->udp != NULL)
    {
      ...
      udp->check = 0;
      udp->check = tcp_udp_v4_checksum (ipv4->src_ip, ipv4->dst_ip,
...
static void
checksum_ipv6_packet (struct packet *packet)
{
  ...
  if (packet->tcp != NULL)
  ...
  else if (packet->icmpv6 != NULL)
    {
      ...
      icmpv6->checksum = 0;
      icmpv6->checksum = tcp_udp_v6_checksum (
	&ipv6->src_ip, &ipv6->dst_ip, IPPROTO_ICMPV6, icmpv6, l4_bytes);
    }
  else
    {
      assert (!"not TCP or ICMP");
...
void
checksum_packet (struct packet *packet)
{
