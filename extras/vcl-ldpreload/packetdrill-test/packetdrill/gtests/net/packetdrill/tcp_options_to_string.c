 ...
 */
static int
tcp_md5_option_to_string (FILE *s, struct tcp_option *option)
{
  int digest_bytes, i;

  if (option->length < TCPOLEN_MD5_BASE || option->length > TCPOLEN_MD5SIG)
    return STATUS_ERR;
 ...
 */
static int
tcp_fast_open_option_to_string (FILE *s, struct tcp_option *option, bool exp)
{
  if (exp &&
      ((option->length < TCPOLEN_EXP_FASTOPEN_BASE) ||
       (ntohs (option->data.fast_open_exp.magic) != TCPOPT_FASTOPEN_MAGIC)))
    return STATUS_ERR;

  fprintf (s, exp ? "FOEXP" : "FO");
  int cookie_bytes =
    option->length - (exp ? TCPOLEN_EXP_FASTOPEN_BASE : TCPOLEN_FASTOPEN_BASE);
  ...
  for (i = 0; i < cookie_bytes; ++i)
    fprintf (s, "%02x",
	     exp ? option->data.fast_open_exp.cookie[i] :
		   option->data.fast_open.cookie[i]);
  return STATUS_OK;
}

int
tcp_options_to_string (struct packet *packet, char **ascii_string,
		       char **error)
{
  ...
  for (option = tcp_options_begin (packet, &iter); option != NULL;
	...
	case TCPOPT_WINDOW:
	  fprintf (s, "wscale %u", option->data.window_scale.shift_count);
	  break;
	  ...
	  for (i = 0; i < num_blocks; ++i)
	    {
	      if (i > 0)
		fputc (' ', s);
	      fprintf (s, "%u:%u", ntohl (option->data.sack.block[i].left),
		       ntohl (option->data.sack.block[i].right));
	    }
	  break;

	case TCPOPT_TIMESTAMP:
	  fprintf (s, "TS val %u ecr %u", ntohl (option->data.time_stamp.val),
	...
	default:
	  asprintf (error, "unexpected TCP option kind: %u", option->kind);
	  goto out;
	}
      ++index;
    }
