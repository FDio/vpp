 ...
 *
 * Summary for IPv6:
 * - local address: fd3d:fa7b:d17d::/48 in unique local address space (RFC
 * 4193)
...
static void
set_ipv4_defaults (struct config *config)
{
  if (strlen (config->live_remote_ip_string) == 0)
    strcpy (config->live_remote_ip_string, DEFAULT_V4_LIVE_REMOTE_IP_STRING);
  if (strlen (config->live_netmask_ip_string) == 0)
    strcpy (config->live_netmask_ip_string, DEFAULT_V4_LIVE_NETMASK_IP_STRING);
  ...
  if (strlen (config->live_gateway_ip_string) == 0)
    strcpy (config->live_gateway_ip_string, DEFAULT_V4_LIVE_GATEWAY_IP_STRING);
}

/* Fill in any as-yet-unspecified IP address attributes using IPv6 defaults. */
static void
set_ipv6_defaults (struct config *config)
{
  if (strlen (config->live_remote_ip_string) == 0)
    strcpy (config->live_remote_ip_string, DEFAULT_V6_LIVE_REMOTE_IP_STRING);
  if (strlen (config->live_local_ip_string) == 0)
    generate_random_ipv6_addr (config->live_local_ip_string,
			       DEFAULT_V6_LIVE_LOCAL_IP_STRING,
			       DEFAULT_V6_LIVE_PREFIX_LEN);
  if (strlen (config->live_gateway_ip_string) == 0)
    strcpy (config->live_gateway_ip_string, DEFAULT_V6_LIVE_GATEWAY_IP_STRING);
}
...
static void
set_remote_ip_and_prefix (struct config *config)
{
...
static void
finalize_ipv4_config (struct config *config)
{
  ...
  config->live_prefix_len = netmask_to_prefix (config->live_netmask_ip_string);
...
static void
finalize_ipv4_mapped_ipv6_config (struct config *config)
{
  set_ipv4_defaults (config);
...
static void
finalize_ipv6_config (struct config *config)
{
...
void
finalize_config (struct config *config)
{
...
void
parse_non_fatal_arg (char *arg, struct config *config)
{
...
static void
process_option (int opt, char *optarg, struct config *config, char *where)
{
  int port = 0;
  ...
  switch (opt)
    ...
    case OPT_TCP_TS_TICK_USECS:
      ...
      if (config->tcp_ts_tick_usecs < 0 || config->tcp_ts_tick_usecs > 1000000)
    ...
    case OPT_WIRE_SERVER_IP:
      ...
      config->wire_server_ip = ipv4_parse (config->wire_server_ip_string);
      break;
 ...
 */
char **
parse_command_line_options (int argc, char *argv[], struct config *config)
{
  int c = 0;
  int i = 0;
...
static void
parse_script_options (struct config *config, struct option_list *option_list)
{
  ...
  while (opt != NULL)
	...
	die ("%s: option '%s' requires an argument\n", config->script_path,
      ...
      process_option (options[i].val, opt->value, config, config->script_path);
 ...
 */
void
parse_and_finalize_config (struct invocation *invocation)
{
  DEBUGP ("parse_and_finalize_config\n");

  /* Parse options in script */
  parse_script_options (invocation->config, invocation->script->option_list);
