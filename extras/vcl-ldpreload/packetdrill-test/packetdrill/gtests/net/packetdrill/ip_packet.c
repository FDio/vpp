...
static void
set_ipv4_header (struct ipv4 *ipv4, u16 ip_bytes, u8 tos, u8 ttl, u8 protocol)
{
  ipv4->version = 4;
...
static void
set_ipv6_header (struct ipv6 *ipv6, u16 ip_bytes, u8 tos, u32 flow_label,
...
void
set_ip_header (void *ip_header, int address_family, u16 ip_bytes, u8 tos,
	       u32 flowlabel, u8 ttl, u8 protocol)
{
  if (address_family == AF_INET)
    set_ipv4_header (ip_header, ip_bytes, tos, ttl, protocol);
  else if (address_family == AF_INET6)
    set_ipv6_header (ip_header, ip_bytes, tos, flowlabel, ttl, protocol);
  else
    assert (!"bad ip_version in config");
}

void
set_packet_ip_header (struct packet *packet, int address_family, u16 ip_bytes,
		      u8 tos, u32 flowlabel, u8 ttl, u8 protocol)
{
  ...
  if (address_family == AF_INET)
    {
      ...
      ip_header = packet_append_header (packet, HEADER_IPV4, sizeof (*ipv4));
  ...
  else if (address_family == AF_INET6)
    {
      ...
      ip_header = packet_append_header (packet, HEADER_IPV6, sizeof (*ipv6));
      ip_header->total_bytes = ip_bytes;
...
int
ipv4_header_append (struct packet *packet, const char *ip_src,
		    const char *ip_dst, const u8 tos, const u8 ttl,
		    char **error)
{
...
int
ipv6_header_append (struct packet *packet, const char *ip_src,
		    const char *ip_dst, const u8 tos, const u8 hop_limit,
		    char **error)
{
  struct header *header = NULL;
...
int
ipv4_header_finish (struct packet *packet, struct header *header,
...
int
ipv6_header_finish (struct packet *packet, struct header *header,
		    struct header *next_inner)
{
