...
static int parse_mpls (struct packet *packet, u8 *header_start, u8 *packet_end,
		       char **error);
static int parse_layer3_packet_by_proto (struct packet *packet, u16 proto,
...
static int
parse_layer2_packet (struct packet *packet, u8 *header_start, u8 *packet_end,
		     char **error)
{
  u8 *p = header_start;
  ...
  return parse_layer3_packet_by_proto (packet, ntohs (ether->ether_type), p,
				       packet_end, error);
...
static int
parse_layer3_packet_by_proto (struct packet *packet, u16 proto,
			      u8 *header_start, u8 *packet_end, char **error)
{
  ...
  else if ((proto == ETHERTYPE_MPLS_UC) || (proto == ETHERTYPE_MPLS_MC))
    {
...
static int
parse_layer3_packet (struct packet *packet, u8 *header_start, u8 *packet_end,
		     char **error)
{
  u8 *p = header_start;
...
int
parse_packet (struct packet *packet, int in_bytes, enum packet_layer_t layer,
	      char **error)
{
 ...
 */
static int
parse_ipv4 (struct packet *packet, u8 *header_start, u8 *packet_end,
	    char **error)
{
  ...
  if (ntohs (ipv4->frag_off) & IP_MF)
    { /* more fragments? */
      asprintf (error, "More fragments remaining");
      goto error_out;
    }
  if (ntohs (ipv4->frag_off) & IP_OFFMASK)
    { /* fragment offset */
  ...
  result =
    parse_layer4 (packet, p, layer4_protocol, layer4_bytes, packet_end, error);
 ...
 */
static int
parse_ipv6 (struct packet *packet, u8 *header_start, u8 *packet_end,
	    char **error)
{
  struct header *ip_header = NULL;
  u8 *p = header_start;
...
static int
parse_tcp (struct packet *packet, u8 *layer4_start, int layer4_bytes,
...
static int
parse_udp (struct packet *packet, u8 *layer4_start, int layer4_bytes,
	   u8 *packet_end, char **error)
{
...
static int
parse_icmpv4 (struct packet *packet, u8 *layer4_start, int layer4_bytes,
	      u8 *packet_end, char **error)
{
...
static int
parse_icmpv6 (struct packet *packet, u8 *layer4_start, int layer4_bytes,
	      u8 *packet_end, char **error)
{
  struct header *icmp_header = NULL;
  u8 *p = layer4_start;
...
static int
parse_gre (struct packet *packet, u8 *layer4_start, int layer4_bytes,
	   u8 *packet_end, char **error)
{
...
int
parse_mpls (struct packet *packet, u8 *header_start, u8 *packet_end,
	    char **error)
{
  ...
  mpls_header = packet_append_header (packet, HEADER_MPLS, mpls_header_bytes);
  if (mpls_header == NULL)
    {
...
static int
parse_layer4 (struct packet *packet, u8 *layer4_start, int layer4_protocol,
	      int layer4_bytes, u8 *packet_end, char **error)
{
  if (layer4_protocol == IPPROTO_TCP)
    ...
    {
      return parse_gre (packet, layer4_start, layer4_bytes, packet_end, error);
    }
