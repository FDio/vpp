 ...
 */
static u64
ip_checksum_partial (const void *p, size_t len, u64 sum)
{
...
static u64
tcp_udp_v4_header_checksum_partial (struct in_addr src_ip,
				    struct in_addr dst_ip, u8 protocol,
				    u16 len)
{
...
__be16
tcp_udp_v4_checksum (struct in_addr src_ip, struct in_addr dst_ip, u8 protocol,
		     const void *payload, u16 len)
{
  u64 sum = tcp_udp_v4_header_checksum_partial (src_ip, dst_ip, protocol, len);
...
__be16
ipv4_checksum (void *ip_header, size_t ip_header_bytes)
{
...
static u64
tcp_udp_v6_header_checksum_partial (const struct in6_addr *src_ip,
				    const struct in6_addr *dst_ip, u8 protocol,
				    u32 len)
{
...
__be16
tcp_udp_v6_checksum (const struct in6_addr *src_ip,
		     const struct in6_addr *dst_ip, u8 protocol,
		     const void *payload, u32 len)
{
  u64 sum = tcp_udp_v6_header_checksum_partial (src_ip, dst_ip, protocol, len);
  sum = ip_checksum_partial (payload, len, sum);
  return ip_checksum_fold (sum);
}
...
__be32
sctp_crc32c (const void *packet, u32 len)
{
  u32 i, crc32c;
