...
static void
write_preamble (struct code_state *code)
{
 ...
 */
static void
emit_var (struct code_state *code, const char *name, u64 value)
{
  assert (code->format > FORMAT_NONE);
  assert (code->format < FORMAT_NUM_TYPES);
  switch (code->format)
...
static void
emit_var_end (struct code_state *code)
{
  fprintf (code->file, "\n");
}
...
static void
write_symbols (struct code_state *code)
{
#ifdef linux
...
static void
write_tcp_info (struct code_state *code, const struct _tcp_info *info, int len)
{
...
static void
write_tcp_bbr_cc_info (struct code_state *code, const union _tcp_cc_info *info,
		       int len)
{
...
static void
write_tcp_dctcp_cc_info (struct code_state *code,
			 const union _tcp_cc_info *info, int len)
{
...
static void
write_tcp_vegas_cc_info (struct code_state *code,
			 const union _tcp_cc_info *info, int len)
{
...
static void
write_tcp_cc_info (struct code_state *code, const union _tcp_cc_info *info,
		   int len)
{
...
static void
write_so_meminfo (struct code_state *code, const u32 *mem_info, int len)
{
...
static void
text_free (struct code_text *text)
{
  free (text->text);
  free (text->file_name);
  free (text);
...
static void
data_free (struct code_data *data)
{
  free (data->buffer);
  free (data);
}
...
static struct code_fragment *
fragment_new (void)
{
  struct code_fragment *fragment = calloc (1, sizeof (struct code_fragment));
  return fragment;
}

/* Free the given fragment and all storage to which it points. */
static void
fragment_free (struct code_fragment *fragment)
{
...
static void
write_text (struct code_state *code, struct code_text *text)
{
  assert (code->format > FORMAT_NONE);
  assert (code->format < FORMAT_NUM_TYPES);
  switch (code->format)
...
static void
write_data (struct code_state *code, struct code_data *data)
{
...
static void
write_fragment (struct code_state *code, struct code_fragment *fragment)
{
  assert (fragment->type > FRAGMENT_NONE);
  assert (fragment->type < FRAGMENT_NUM_TYPES);
  switch (fragment->type)
...
static void
write_all_fragments (struct code_state *code)
{
  struct code_fragment *fragment = NULL;
  for (fragment = code->list_head; fragment != NULL; fragment = fragment->next)
    {
...
static void
append_fragment (struct code_state *code, struct code_fragment *fragment)
{
 ...
 */
static void
append_text (struct code_state *code, const char *file_name, int line_number,
	     char *text_buffer)
{
 ...
 */
static void
append_data (struct code_state *code, enum code_data_t data_type,
...
void
code_free (struct code_state *code)
{
 ...
 */
static void
write_code_file (struct code_state *code)
{
 ...
 */
static int
execute_code_command_line (struct code_state *code, char **error)
{
  ...
  if (WIFSIGNALED (status) &&
    ...
    {
      asprintf (error, "'%s' got signal %d (%s)", code->command_line,
		WTERMSIG (status), strsignal (WTERMSIG (status)));
      goto out;
    }
  if (WEXITSTATUS (status) != 0)
    {
      asprintf (error, "'%s' returned non-zero status %d", code->command_line,
		WEXITSTATUS (status));
      goto out;
    }
  result = STATUS_OK;
...
static void
delete_code_file (struct code_state *code)
{
...
int
code_execute (struct code_state *code, char **error)
{
 ...
 */
static void *
get_data (struct state *state, struct event *event, int fd,
	  enum code_data_t data_type, int *len)
{
  int opt_name = 0;
  int data_len = 0;
  int level;
...
void
run_code_event (struct state *state, struct event *event, const char *text)
{
  ...
  if (data == NULL && data_ext == NULL)
    {
      asprintf (&error, "can't find getsockopt to get TCP info");
      goto error_out;
    }
#if HAVE_TCP_CC_INFO
  code->data_type = DATA_TCP_CC_INFO;
  ...
  if (data)
    ...
    {
      asprintf (&error, "can't find getsockopt to get TCP_CC_INFO");
      goto error_out;
    }
#endif /* HAVE_TCP_CC_INFO */
#if HAVE_SO_MEMINFO
  ...
  if (data_meminfo == NULL)
    {
      asprintf (&error, "can't find getsockopt to get sk_meminfo");
      goto error_out;
    }
#endif
