...
packet_encapsulate_and_free (struct packet *outer, struct packet *inner)
{
...
packet_direction (const struct packet *packet)
{
...
static inline int
packet_address_family (const struct packet *packet)
{
  if (packet->ipv4 != NULL)
    return AF_INET;
  if (packet->ipv6 != NULL)
...
static inline u8 *
packet_start (const struct packet *packet)
{
 ...
 */
static inline int
ip_header_min_len (int address_family)
{
...
static inline int
packet_ip_protocol (const struct packet *packet)
{
  if (packet->ipv4 != NULL)
...
static inline int
layer4_header_len (int protocol)
{
...
static inline int
packet_tcp_header_len (const struct packet *packet)
{
  assert (packet->tcp);
...
static inline int
packet_udp_header_len (const struct packet *packet)
{
  assert (packet->udp);
  return sizeof (struct udp);
}
...
static inline int
packet_icmpv4_header_len (const struct packet *packet)
{
...
static inline int
packet_icmpv6_header_len (const struct packet *packet)
{
...
static inline int
packet_tcp_options_len (const struct packet *packet)
{
  assert (packet->tcp);
...
static inline u8 *
packet_tcp_options (struct packet *packet)
{
  assert (packet->tcp);
...
static inline u32
packet_tcp_ts_val (const struct packet *packet)
{
  return get_unaligned_be32 (packet->tcp_ts_val);
}

static inline u32
packet_tcp_ts_ecr (const struct packet *packet)
{
  return get_unaligned_be32 (packet->tcp_ts_ecr);
}

static inline void
packet_set_tcp_ts_val (struct packet *packet, u32 ts_val)
{
  put_unaligned_be32 (ts_val, packet->tcp_ts_val);
}

static inline void
packet_set_tcp_ts_ecr (struct packet *packet, u32 ts_ecr)
{
...
static inline u8 *
packet_payload (const struct packet *packet)
{
  if (packet->tcp)
...
static inline u8 *
packet_end (const struct packet *packet)
{
...
static inline int
packet_payload_len (const struct packet *packet)
{
...
static inline u8 *
packet_echoed_ip_header (struct packet *packet)
{
 ...
 */
static inline struct ipv4 *
packet_echoed_ipv4_header (struct packet *packet)
{
  return (struct ipv4 *) ((packet->icmpv4 != NULL) ? (packet->icmpv4 + 1) :
						     NULL);
}

/* Return the location of the IPv6 header echoed by an ICMP message, or NULL.
 */
static inline struct ipv6 *
packet_echoed_ipv6_header (struct packet *packet)
{
  return (struct ipv6 *) ((packet->icmpv6 != NULL) ? (packet->icmpv6 + 1) :
						     NULL);
}
 ...
 */
static inline int
packet_echoed_ip_header_len (struct packet *packet)
{
  if (packet->icmpv4 != NULL)
    return sizeof (struct ipv4);
  if (packet->icmpv6 != NULL)
...
static inline int
packet_echoed_ip_protocol (struct packet *packet)
{
  if (packet->icmpv4 != NULL)
...
static inline u8 *
packet_echoed_layer4_header (struct packet *packet)
{
...
static inline struct tcp *
packet_echoed_tcp_header (struct packet *packet)
{
...
packet_echoed_udp_header (struct packet *packet)
{
...
static inline u32 *
packet_echoed_tcp_seq (struct packet *packet)
{
