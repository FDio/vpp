...
struct tcp_options *
tcp_options_new (void)
{
  return calloc (1, sizeof (struct tcp_options));
}

struct tcp_option *
tcp_option_new (u8 kind, u8 length)
{
...
int
tcp_options_append (struct tcp_options *options, struct tcp_option *option)
{
...
int
num_sack_blocks (u8 opt_len, int *num_blocks, char **error)
{
  if (opt_len <= 2)
    {
  ...
  if (num_bytes % sizeof (struct sack_block) != 0)
    {
      asprintf (error, "TCP SACK option not a multiple of SACK block size");
      return STATUS_ERR;
    }
