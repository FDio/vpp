...
extern void set_ip_header (void *ip_header, int address_family, u16 ip_bytes,
			   u8 tos, u32 flowlabel, u8 ttl, u8 protocol);

/* Set the packet's IP header pointer and then populate the IP header fields.
 */
extern void set_packet_ip_header (struct packet *packet, int address_family,
				  u16 ip_bytes, u8 tos, u32 flowlabel, u8 ttl,
				  u8 protocol);
 ...
 */
extern int ipv4_header_append (struct packet *packet, const char *ip_src,
 ...
 * and fill in a malloc-allocated error message in *error.
 */
extern int ipv6_header_append (struct packet *packet, const char *ip_src,
			       const char *ip_dst, const u8 tos,
			       const u8 hop_limit, char **error);

/* Finalize the IPv4 header by filling in all necessary fields that
 * were not filled in at parse time.
 */
extern int ipv4_header_finish (struct packet *packet, struct header *header,
			       struct header *next_inner);

/* Finalize the IPv6 header by filling in all necessary fields that
 * were not filled in at parse time.
 */
extern int ipv6_header_finish (struct packet *packet, struct header *header,
			       struct header *next_inner);
