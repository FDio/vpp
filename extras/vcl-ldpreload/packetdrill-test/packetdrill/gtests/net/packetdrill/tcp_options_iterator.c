 ...
 */
static int
get_expected_tcp_option_length (u8 kind, u8 *expected_length, char **error)
{
  switch (kind)
    {
    case TCPOPT_EOL:
 ...
 */
static int
get_tcp_option_length (const u8 *option, const u8 *end, u8 expected_length,
		       u8 *length, char **error)
{
  ...
  *length = *(option + 1);
  ...
  if (expected_length && (*length != expected_length))
    {
      asprintf (error, "bad TCP option length: was %u but expected %u",
...
get_current_option (struct tcp_options_iterator *iter)
{
...
struct tcp_option *
tcp_options_begin (struct packet *packet, struct tcp_options_iterator *iter)
{
...
struct tcp_option *
tcp_options_next (struct tcp_options_iterator *iter, char **error)
{
  ...
  if (get_expected_tcp_option_length (option->kind, &expected_length, error))
    goto out;
  ...
  if (expected_length == 1)
  ...
  else
    {
      ...
      if (get_tcp_option_length (iter->current_option, iter->options_end,
