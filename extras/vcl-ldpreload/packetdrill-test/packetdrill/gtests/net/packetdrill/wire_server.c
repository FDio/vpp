...
static struct wire_server *
wire_server_new (struct wire_conn *accepted_conn,
		 const char *wire_server_device, u16 wire_server_port,
...
static void
wire_server_free (struct wire_server *wire_server)
{
 ...
 */
static void
wire_server_unserialize_argv (struct wire_server *wire_server,
...
static int
wire_server_receive_args (struct wire_server *wire_server)
{
  ...
  if (op != WIRE_COMMAND_LINE_ARGS)
    {
      fprintf (stderr, "bad wire client: expected WIRE_COMMAND_LINE_ARGS\n");
      return STATUS_ERR;
    }

  wire_server_unserialize_argv (wire_server, buf, buf_len);
...
static int
wire_server_receive_script_path (struct wire_server *wire_server)
{
  enum wire_op_t op = WIRE_INVALID;
  void *buf = NULL;
  int buf_len = -1;
  ...
  if (op != WIRE_SCRIPT_PATH)
    {
      fprintf (stderr, "bad wire client: expected WIRE_SCRIPT_PATH\n");
      return STATUS_ERR;
    }
...
static int
wire_server_receive_script (struct wire_server *wire_server)
{
  enum wire_op_t op = WIRE_INVALID;
  void *buf = NULL;
  int buf_len = -1;
...
static int
wire_server_receive_hw_address (struct wire_server *wire_server)
{
  enum wire_op_t op = WIRE_INVALID;
  void *buf = NULL;
  int buf_len = -1;
  ...
  if (op != WIRE_HARDWARE_ADDR)
    {
      fprintf (stderr, "bad wire client: expected WIRE_HARDWARE_ADDR\n");
      return STATUS_ERR;
    }
  if (buf_len != sizeof (wire_server->client_ether_addr))
    {
      fprintf (stderr, "bad wire client: bad hw address length\n");
      return STATUS_ERR;
    }
...
static int
wire_server_send_server_ready (struct wire_server *wire_server)
{
  if (wire_conn_write (wire_server->wire_conn, WIRE_SERVER_READY, NULL, 0))
    {
...
static int
wire_server_receive_client_starting (struct wire_server *wire_server)
{
  enum wire_op_t op = WIRE_INVALID;
  void *buf = NULL;
  int buf_len = -1;
  ...
  if (op != WIRE_CLIENT_STARTING)
    {
      fprintf (stderr, "bad wire client: expected WIRE_CLIENT_STARTING\n");
      return STATUS_ERR;
    }
  if (buf_len != 0)
    {
      fprintf (stderr, "bad wire client: bad WIRE_CLIENT_STARTING length\n");
      return STATUS_ERR;
    }
...
static int
wire_server_receive_packets_start (struct wire_server *wire_server)
{
  enum wire_op_t op = WIRE_INVALID;
  void *buf = NULL;
  int buf_len = -1;
  ...
  if (op != WIRE_PACKETS_START)
    {
      fprintf (stderr, "bad wire client: expected WIRE_PACKETS_START\n");
      return STATUS_ERR;
    }
  if (buf_len != sizeof (start))
    {
      fprintf (stderr, "bad wire client: bad WIRE_PACKETS_START length\n");
      return STATUS_ERR;
    }
  ...
  if (ntohl (start.num_events) != wire_server->num_events)
    {
      fprintf (stderr, "bad client event count; expected %d but got %d",
...
static int
wire_server_send_packet_warning (struct wire_server *wire_server,
				 const char *warning)
{
  if (wire_conn_write (wire_server->wire_conn, WIRE_PACKETS_WARN, warning,
		       strlen (warning)))
    {
...
static int
wire_server_send_packets_done (struct wire_server *wire_server, int result,
			       const char *error)
{
  ...
  if (wire_conn_write (wire_server->wire_conn, WIRE_PACKETS_DONE, buf,
		       buf_len))
    {
...
static int
wire_server_next_event (struct wire_server *wire_server, struct event *event)
{
...
static int
wire_server_run_packet_event (struct wire_server *wire_server,
			      struct event *event, struct packet *packet,
			      char **error)
{
  int result = STATUS_OK;

  result = run_packet_event (wire_server->state, event, packet, error);
  if (result == STATUS_ERR)
    {
      ...
      if (wire_server_send_packets_done (wire_server, STATUS_ERR, *error))
	return STATUS_ERR;
    }
 ...
 */
static int
wire_server_run_script (struct wire_server *wire_server, char **error)
{
  ...
  while (1)
	...
	{
	case PACKET_EVENT:
	  if (wire_server_run_packet_event (
		wire_server, event, event->event.packet, error) == STATUS_ERR)
	    return STATUS_ERR;
	  break;
	case SYSCALL_EVENT:
...
wire_server_thread (void *arg)
{
  ...
  if (parse_script_and_set_config (wire_server->argc, wire_server->argv,
				   &wire_server->config, &wire_server->script,
  ...
  netdev = wire_server_netdev_new (
    &wire_server->config, wire_server->wire_server_device,
    &wire_server->client_ether_addr, &wire_server->server_ether_addr);

  wire_server->state =
    state_new (&wire_server->config, &wire_server->script, netdev);
...
static void
start_wire_server_thread (struct wire_server *wire_server)
{
  DEBUGP ("start_wire_server_thread\n");

  pthread_t thread; /* pthread thread handle */
  if (pthread_create (&thread, NULL, wire_server_thread, wire_server) != 0)
    {
...
void
run_wire_server (const struct config *config)
{
  ...
  while (1)
	...
	wire_server_new (accepted_conn, config->wire_server_device,
