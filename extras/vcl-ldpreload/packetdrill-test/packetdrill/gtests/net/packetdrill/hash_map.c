...
static inline size_t
hash_bucket_num (const struct hash_map *map, u32 key)
{
 ...
 */
static inline size_t
hash_map_pick_bucket_count (size_t num_keys)
{
  size_t buckets = 1;
  ...
  for (bucket_num = 0; bucket_num < map->num_buckets; ++bucket_num)
    {
      ...
      for (node = map->buckets[bucket_num]; node != NULL; node = next)
	{
	  next = node->next;
	  free (node);
	}
...
static void
hash_map_link (struct hash_map *map, struct hash_node *node)
{
...
hash_map_grow (struct hash_map *map)
{
  ...
  for (old_bucket_num = 0; old_bucket_num < old_num_buckets; ++old_bucket_num)
    {
      struct hash_node *node = NULL;
      struct hash_node *next = NULL;
      for (node = old_buckets[old_bucket_num]; node != NULL; node = next)
	{
	  next = node->next;
...
static void
hash_map_insert (struct hash_map *map, u32 key, u32 value)
{
  /* To keep things simple, we target a load factor of 1.0. */
  if ((map->num_keys >= map->num_buckets) && (map->num_buckets < MAX_BUCKETS))
    {
      hash_map_grow (map);
    }
  ++map->num_keys;
...
void
hash_map_set (struct hash_map *map, u32 key, u32 value)
{
...
bool
hash_map_get (const struct hash_map *map, u32 key, u32 *value)
{
  const size_t bucket_num = hash_bucket_num (map, key);
  struct hash_node *node = NULL;
  for (node = map->buckets[bucket_num]; node != NULL; node = node->next)
