...
static bool
is_tcp_flags_spec_valid (const char *flags, char **error)
{
  const char *s;
...
static inline int
is_tcp_flag_set (char flag, const char *flags)
{
...
static inline int
tcp_flag_ace_count (const char *flags)
{
  const char *s;
		...
		const struct tcp_options *tcp_options, char **error)
{
  ...
  const int ip_bytes = ip_header_bytes + tcp_header_bytes + tcp_payload_bytes;
  int ace;
  ...
  set_packet_ip_header (packet, address_family, ip_bytes, ip_info.tos.value,
			ip_info.flow_label, ip_info.ttl, IPPROTO_TCP);
  ...
  if (tcp_options == NULL)
  ...
  else if (tcp_options->length > 0)
    {
      ...
      memcpy (tcp_option_start, tcp_options->data, tcp_options->length);
    }
