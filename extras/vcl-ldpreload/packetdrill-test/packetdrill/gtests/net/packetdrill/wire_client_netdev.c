...
static inline struct wire_client_netdev *
to_client_netdev (struct netdev *netdev)
{
 ...
 */
static void
check_remote_address (struct config *config, struct wire_client_netdev *netdev)
{
...
static void
route_traffic_to_wire_server (struct config *config,
			      struct wire_client_netdev *netdev)
{
  ...
  asprintf (&route_command,
	    ...
	    (config->wire_protocol == AF_INET6) ? "-6" : "",
	    config->live_remote_prefix_string, netdev->name,
...
struct netdev *
wire_client_netdev_new (struct config *config)
{
  ...
  net_setup_dev_address (netdev->name, &config->live_local_ip,
...
static void
wire_client_netdev_free (struct netdev *a_netdev)
{
...
static int
wire_client_netdev_send (struct netdev *a_netdev, struct packet *packet)
{
...
static int
wire_client_netdev_receive (struct netdev *a_netdev, struct packet **packet,
			    char **error)
{
