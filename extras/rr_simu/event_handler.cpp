...
EventHandler::EventHandler(std::unordered_map<std::string, std::string> &configParam) {
    ...
    ini_time = std::chrono::time_point_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now());
    curr_time = ini_time;
    long num_frame = std::ceil((float) input_rate/frame_generator.get_size_esperance()/max_frame_size);

    for (long i = 1; i <= duration*num_frame; i++)
        priority_queue.emplace(FRAME_ARRIVING, curr_time + std::chrono::nanoseconds(1000000000/num_frame*i));
    for (long j = 1; j <= duration*100*1.2; j++)
        priority_queue.emplace(SAVE_DATA, curr_time + std::chrono::nanoseconds(10000000*j));
    ...
    start = std::chrono::high_resolution_clock::now();
        ...
        switch (priority_queue.top().get_type()) {
            ...
            case TRANS_START_TRUE:
                ...
                priority_queue.emplace(TRANS_END_TRUE, curr_time + std::chrono::nanoseconds((long)next_packet->get_pkt_size()*1000000000/output_rate));
                break;

            case TRANS_START_SIMPL:
                next_packet_simpl = sched.select_packet_simpl();
                priority_queue.emplace(TRANS_END_FRAME, curr_time + std::chrono::nanoseconds((long)next_packet_simpl->get_pkt_size()*1000000000/output_rate));
                break;
