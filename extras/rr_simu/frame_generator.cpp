  ...
  std::unordered_map<std::string, std::string> &configParam)
{
  ...
  this->size_repartition_map =
    Parser::parsePairArray (configParam.at ("size_repartition"));

  class_repartition_vec =
    generate_distribution_vector (class_repartition_map, max_num_queue);
  size_repartition_vec =
    generate_distribution_vector (size_repartition_map, 1501);

  for (int i = 0; i < 64; i++)
    size_repartition_vec.at (i) = 0;

  class_d = std::discrete_distribution<> (class_repartition_vec.begin (),
					  class_repartition_vec.end ());
  size_d = std::discrete_distribution<> (size_repartition_vec.begin (),
					 size_repartition_vec.end ());
}

void
FrameGenerator::generate_frame (
...
void
FrameGenerator::generate_packet (
  std::chrono::time_point<std::chrono::steady_clock, std::chrono::nanoseconds>
    &time,
  frame &curr_frame)
{
  ...
  curr_frame.push_back (
    std::make_shared<Packet> (id_count++, pkt_class, size, time));
}

std::vector<float>
FrameGenerator::generate_distribution_vector (
  std::map<int, float> repartition_map, unsigned int max_size)
{
  ...
  if ((repartition_map).size () == max_size)
	...
	{
	  auto it =
	    std::find (element_defined.begin (), element_defined.end (), i);
	  if (it != std::end (element_defined))
		...
		{
		  repartition_vec.push_back (repartition_map.at (*it) * 100.0 /
					     total_sum);
		}
	    }
	  else
	    {
	      if (total_sum < 100)
		{
		  repartition_vec.push_back (
		    (100 - total_sum) / (max_size - element_defined.size ()));
		}
	      else
		{
...
double
FrameGenerator::get_size_esperance ()
{
  double sum = 0;
  int i = 0;
