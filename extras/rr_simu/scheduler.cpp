...
#include <iostream>
 ...
 void Scheduler::init(std::unordered_map<std::string, std::string> &configParam) {
...
void Scheduler::enqueue(frame &curr_frame) {
...
std::shared_ptr<Packet> Scheduler::select_packet() {
...
void Scheduler::forward_packet(std::chrono::time_point<std::chrono::steady_clock,std::chrono::nanoseconds> &time) {
...
void Scheduler::change_active() {
    active = !active;
}

void Scheduler::enqueue_simpl(frame &curr_frame) {
    std::unordered_map<unsigned int, std::queue<std::shared_ptr<Packet>>> pkt_map_simpl;
    std::vector<int> class_vec = {};
    int num_pkt_simpl = 0;
    for (auto pkt_ptr : curr_frame) {
        pkt_map_simpl[pkt_ptr->get_pkt_class()].push(pkt_ptr);
        num_pkt_simpl++;
        if (std::find(class_vec.begin(), class_vec.end(), pkt_ptr->get_pkt_class()) == class_vec.end())
...
std::shared_ptr<Packet> Scheduler::select_packet_simpl() {
    next_pkt_simpl = pkt_queue_simpl.front();
    pkt_queue_simpl.pop();
    return next_pkt_simpl;
}

void Scheduler::forward_packet_simpl(std::chrono::time_point<std::chrono::steady_clock, std::chrono::nanoseconds> &time) {
...
bool Scheduler::is_active_simpl() {
    return active_simpl;
}

void Scheduler::change_active_simpl() {
