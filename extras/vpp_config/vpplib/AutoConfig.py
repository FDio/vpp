...
DEFAULT_SYSTEM_CONFIG_FILENAME = "/usr/local/vpp/vpp-config/configs/system-config.yaml"
...
class AutoConfig(object):
    ...
    def __init__(self, rootdir, filename):
        ...
        :param rootdir: The autoconfiguration file
...
other than VPP? [0-{}][0]? '.format(str(max_other_cores))
    ...
    def modify_huge_pages(self):
        ...
        for i in self._nodes.items():
            ...
            memtotal = node['hugepages']['memtotal']
            memtotal = int(memtotal.split(' ')[0])
            hugesize = int(size.split(' ')[0])
            # The max number of huge pages should be no more than
            # 70% of total memory
            maxpages = (memtotal * 70 / 100) / hugesize
            ...
            print "\nThere currently a total of {} huge pages.". \
                format(total)
            question = \
                "How many huge pages do you want [1024 - {}][1024]? ". \
                format(maxpages)
    ...
    def device_info(node):
        ...
        if len(linkup_devs):
            print ("  Devices with link up (can not be used with VPP):")
            for dev in linkup_devs:
                print ("    " + dev)
        kernel_devs = vpp.get_kernel_devices()
        if len(kernel_devs):
            print ("  Devices bound to kernel drivers:")
            for dev in kernel_devs:
                print ("    " + dev)
        else:
            print ("  No devices bound to kernel drivers")

        dpdk_devs = vpp.get_dpdk_devices()
        if len(dpdk_devs):
            print ("  Devices bound to DPDK drivers:")
            for dev in dpdk_devs:
                print ("    " + dev)
        else:
            print ("  No devices bound to DPDK drivers")
        ...
        for intf in sorted(interfaces.items()):
            ...
            print ("    {:30} {:>6d} {:>4d} {:>7d} {:>4d} {:>7d}".format(name, socket, rx_qs, rx_ds, tx_qs, tx_ds))
