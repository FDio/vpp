...
VPP_DEFAULT_CONFIGURATION_FILE = '{}/vpp/vpp-config/configs/auto-config.yaml'
VPP_SYSTEM_CONFIGURATION_FILE = '{}/vpp/vpp-config/configs/system-config.yaml'
...
def autoconfig_show_system():
    ...
    acfg = AutoConfig(VPP_SYSTEM_CONFIGURATION_FILE.format(rootdir))
...
def autoconfig_hugepage_apply(node):
    ...
    diffs = autoconfig_diff(node, VPP_REAL_HUGE_PAGE_FILE, VPP_HUGE_PAGE_FILE.format(rootdir))
    if diffs != '':
        ...
        # Copy and sysctl
        autoconfig_cp(node, VPP_HUGE_PAGE_FILE.format(rootdir), VPP_REAL_HUGE_PAGE_FILE)
...
def autoconfig_vpp_apply(node):
    ...
    diffs = autoconfig_diff(node, VPP_REAL_STARTUP_FILE, VPP_STARTUP_FILE.format(rootdir))
    if diffs != '':
        print "These are the changes we will apply to"
        ...
        autoconfig_cp(node, VPP_STARTUP_FILE.format(rootdir), VPP_REAL_STARTUP_FILE)
    else:
...
def autoconfig_grub_apply(node):
    ...
    if answer == 'n':
        ...
        # Diff the file
        diffs = autoconfig_diff(node, VPP_REAL_GRUB_FILE, VPP_GRUB_FILE.format(rootdir))
        if diffs != '':
            print "These are the changes we will apply to"
            ...
            autoconfig_cp(node, VPP_GRUB_FILE.format(rootdir), VPP_REAL_GRUB_FILE)
            cmd = "update-grub"
            (ret, stdout, stderr) = VPPUtil.exec_command(cmd)
            if ret != 0:
                raise RuntimeError('{} failed on node {} {} {}'.
...
9 or q) Quit'.format(rootdir, rootdir, rootdir)
...
def autoconfig_setup():
    ...
    if os.path.isfile(VPP_SYSTEM_CONFIGURATION_FILE.format(rootdir)) is True:
        acfg = AutoConfig(VPP_SYSTEM_CONFIGURATION_FILE.format(rootdir))
    else:
        acfg = AutoConfig(VPP_DEFAULT_CONFIGURATION_FILE.format(rootdir))
        acfg.updateconfig()
    ...
    for i in nodes.items():
        ...
        if (os.path.isfile(VPP_STARTUP_FILE.format(rootdir)) is not True) and \
                (os.path.isfile(VPP_REAL_STARTUP_FILE) is True):
            autoconfig_cp(node, VPP_REAL_STARTUP_FILE, '{}'.format(VPP_STARTUP_FILE.format(rootdir)))
        if (os.path.isfile(VPP_HUGE_PAGE_FILE.format(rootdir)) is not True) and \
                (os.path.isfile(VPP_REAL_HUGE_PAGE_FILE) is True):
            autoconfig_cp(node, VPP_REAL_HUGE_PAGE_FILE, '{}'.format(VPP_HUGE_PAGE_FILE.format(rootdir)))
        if (os.path.isfile(VPP_GRUB_FILE.format(rootdir)) is not True) and \
                (os.path.isfile(VPP_REAL_GRUB_FILE) is True):
            autoconfig_cp(node, VPP_REAL_GRUB_FILE, '{}'.format(VPP_GRUB_FILE.format(rootdir)))
