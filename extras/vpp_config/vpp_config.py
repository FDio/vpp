...
def autoconfig_show_system():
    ...
    acfg = AutoConfig(rootdir,  VPP_SYSTEM_CONFIGURATION_FILE)
...
def autoconfig_setup():
    ...
    if os.path.isfile(rootdir + VPP_SYSTEM_CONFIGURATION_FILE) is True:
        acfg = AutoConfig(rootdir, VPP_SYSTEM_CONFIGURATION_FILE)
    else:
        acfg = AutoConfig(rootdir, VPP_DEFAULT_CONFIGURATION_FILE)
