...
ppv2_set_buf_data_len_flags (vlib_buffer_t * b, struct pp2_ppio_desc *d,
			     u32 add_flags)
{
  u16 len;
...
static_always_inline u16
ppv2_next_from_desc (vlib_node_runtime_t * node, struct pp2_ppio_desc * d,
		     vlib_buffer_t * b, u32 * next)
{
  ...
  if (PREDICT_FALSE (DM_RXD_GET_ES (d)))
    {
      *next = VNET_DEVICE_INPUT_NEXT_DROP;
      ...
      if (ec == 0)
	b->error = node->errors[PPV2_INPUT_ERROR_MAC_CE];
      else if (ec == 1)
	b->error = node->errors[PPV2_INPUT_ERROR_MAC_OR];
      else if (ec == 2)
	b->error = node->errors[PPV2_INPUT_ERROR_MAC_RSVD];
      else if (ec == 3)
	b->error = node->errors[PPV2_INPUT_ERROR_MAC_RE];
      return ppv2_set_buf_data_len_flags (b, d, 0);
    }
  ...
  if (PREDICT_TRUE ((l3_info - 1) < 3))
	...
	{
	  *next = VNET_DEVICE_INPUT_NEXT_DROP;
	  b->error = node->errors[PPV2_INPUT_ERROR_IP_HDR];
	  return ppv2_set_buf_data_len_flags (b, d, 0);
	}
      *next = VNET_DEVICE_INPUT_NEXT_IP4_NCS_INPUT;
      return ppv2_set_buf_data_len_flags (b, d,
  ...
  while (n_desc)
    {
      ...
      while (n_desc >= 6 && n_left_to_next >= 2)
	{
	  /* prefetch */
	  CLIB_PREFETCH (d + 4, CLIB_CACHE_LINE_BYTES, LOAD);
	  CLIB_PREFETCH (d + 5, CLIB_CACHE_LINE_BYTES, LOAD);
	  b0 = vlib_get_buffer (vm, pp2_ppio_inq_desc_get_cookie (d + 2));
	  b1 = vlib_get_buffer (vm, pp2_ppio_inq_desc_get_cookie (d + 3));
	  ...
	  bi0 = pp2_ppio_inq_desc_get_cookie (d);
	  bi1 = pp2_ppio_inq_desc_get_cookie (d + 1);
	  to_next[0] = bi0;
	  to_next[1] = bi1;
	  ...
	  vnet_buffer (b1)->sw_if_index[VLIB_RX] = ppif->sw_if_index;

	  if (PREDICT_TRUE (ppif->per_interface_next_index == ~0))
	    {
	      n_rx_bytes += ppv2_next_from_desc (node, d, b0, &next0);
	      n_rx_bytes += ppv2_next_from_desc (node, d + 1, b1, &next1);
	      vnet_feature_start_device_input_x2 (ppif->sw_if_index, &next0,
						  &next1, b0, b1);
	    }
	  else
	    {
	      n_rx_bytes += ppv2_set_buf_data_len_flags (b0, d, 0);
	      n_rx_bytes += ppv2_set_buf_data_len_flags (b1, d + 1, 0);
	  ...
	  if (PREDICT_FALSE (n_trace > 0))
	    {
	      ...
	      if (n_trace > 0)
		ppv2_input_trace (vm, node, next1, b1, &n_trace, ppif, d + 1);
	    }
