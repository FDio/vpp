...
ppv2_main_deinit ()
{
  ...
  {
    ppv2_per_thread_data_t *ptd = vec_elt_at_index (ppm->per_thread_data, i);
    if (ptd->hif)
...
ppv2_main_init ()
{
  ...
  int i, rv;
  u8 *s = 0;
  const int num_hifs_rsrv = 4;
  const int num_bpools_rsrv = 7;

  rv = mv_sys_dma_mem_init (40 << 20);
  if (rv)
    return clib_error_return (0, "mv_sys_dma_mem_init failed, rv = %u", rv);

  init_params.hif_reserved_map = ((1 << num_hifs_rsrv) - 1);
  init_params.bm_pool_reserved_map = ((1 << num_bpools_rsrv) - 1);
  rv = pp2_init (&init_params);
  if (rv)
    {
      err = clib_error_return (0, "ppv2_init failed, rv = %u", rv);
      goto done;
    }
  ...
  vec_foreach_index (i, ppm->per_thread_data)
  {
    ...
    vec_reset_length (s);
    s = format (s, "hif-%d%c", num_hifs_rsrv + i, 0);
...
ppv2_create_if (ppv2_create_if_args_t * args)
{
  ...
  if (vec_len (ppm->per_thread_data) == 0)
    {
      if ((args->error = ppv2_main_init ()) != 0)
	{
	  args->rv = VNET_API_ERROR_INIT_FAILED;
	  return;
	}
