...
uword
ppv2_interface_tx (vlib_main_t * vm,
		   vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ppv2_main_t *ppm = &ppv2_main;
  vnet_interface_output_runtime_t *rund = (void *) node->runtime_data;
  ppv2_if_t *ppif = pool_elt_at_index (ppm->interfaces, rund->dev_instance);
  u32 thread_index = vlib_get_thread_index ();
  ppv2_per_thread_data_t *ptd =
    vec_elt_at_index (ppm->per_thread_data, thread_index);
  u8 qid = thread_index;
  ppv2_outq_t *outq = vec_elt_at_index (ppif->outqs, qid);
  u32 *buffers = vlib_frame_args (frame);
  u16 n_desc = frame->n_vectors, n_left = n_desc, n_sent = n_desc, n_done;
  struct pp2_ppio_desc *d;
  u16 mask = outq->size - 1;

  if (PREDICT_FALSE (pp2_ppio_get_num_outq_done (ppif->ppio, ppif->hif, qid,
						 &n_done)))
    {
      n_done = 0;
      vlib_error_count (vm, node->node_index,
			PPV2_TX_ERROR_PPIO_GET_NUM_OUTQ_DONE, 1);
    }
  ...
  if (pp2_ppio_send (ppif->ppio, ppif->hif, qid, ptd->descs, &n_sent))
    {
      n_sent = 0;
      vlib_error_count (vm, node->node_index, PPV2_TX_ERROR_PPIO_SEND, 1);
    }
  ...
  if (PREDICT_FALSE (n_sent != n_desc))
    {
      ...
      vlib_error_count (vm, node->node_index, PPV2_TX_ERROR_NO_FREE_SLOTS,
			frame->n_vectors - n_sent);
    }
