...
func (e *Endpoint) RequestConnection() (error) {
        // create socket
        fd, err := syscall.Socket(syscall.AF_UNIX, syscall.SOCK_SEQPACKET, 0)
        if err != nil {
                return fmt.Errorf("Failed to create UNIX domain socket: %v", err)
        }
        usa := &syscall.SockaddrUnix{Name: e.socket.filename}

        // Connect to listener socket
        err = syscall.Connect(fd, usa)
        if err != nil {
                return fmt.Errorf("Failed to connect socket %s : %v", e.socket.filename, err)
        }

        // Create control channel
        e.cc, err = e.socket.addControlChannel(fd, e)
        if err != nil {
