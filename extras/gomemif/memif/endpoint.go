...
func (socket *Socket) NewEndpoint (args *Arguments) (*Endpoint, error) {
        ...
        // set default values
        if e.args.NumQueuePairs == 0 {
                e.args.NumQueuePairs = DefaultNumQueuePairs
        }
        if e.args.Log2RingSize == 0 {
                e.args.Log2RingSize = DefaultLog2RingSize
        }
        if e.args.PacketBufferSize == 0 {
                e.args.PacketBufferSize = DefaultPacketBufferSize
        }
...
func (e *Endpoint) initializeRings() (err error) {
	...
	for i := 0; uint16(i) < e.run.NumQueuePairs; i++ {
		...
		//copy(e.regions[0].data[rOffset:], buf.Bytes())
		for j := 0; j < rSize; j++ {
			slot := (uint16(i) + e.run.NumQueuePairs) * uint16(rSize) + uint16(j)
			desc := Desc {
				Flags: 0,
				Region: 0,
				Offset: e.regions[0].PacketBufferOffset + uint32(slot) * e.run.PacketBufferSize,
