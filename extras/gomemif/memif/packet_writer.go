...
func (q *Queue) WritePacket(pkt []byte) int {
	...
	var slot int
	var nFree int
	var packetBufferSize int = int(q.e.run.PacketBufferSize)
	...
	if q.e.args.IsMaster {
		...
		slot = q.readHead()
		nFree = q.ringSize - slot + q.readTail()
	}
	...
	n := copy(q.e.regions[desc.getRegion()].data[offset:offset + packetBufferSize], pkt[:])
	desc.setLength(n)
	for n < len(pkt) {
		nFree--
		if nFree == 0 {
		...
		tmp := copy(q.e.regions[desc.getRegion()].data[offset:offset + packetBufferSize], pkt[:])
		desc.setLength(tmp)
		n += tmp
	}
