...
func (socket *Socket) StartPolling() {
        // FIXME: error reporting from goroutines??
...
func (socket *Socket) handleEvent(event *syscall.EpollEvent) error {
        if socket.listener != nil && socket.listener.fd == int(event.Fd) {
...
func (cc *controlChannel) handleEvent(event *syscall.EpollEvent) error {
        ...
        // asserting event.Fd == cc.fd
        if event.Events == syscall.EPOLLIN {
                size, cc.controlLen, _, _, err = syscall.Recvmsg(cc.fd, cc.data[:] ,cc.control[:], 0)
                if err != nil {
...
func (cc *controlChannel) close(sendMsg bool, str string) (err error) {
        ...
        if cc.e != nil {
                /* clear control channel referance from endpoint so that
