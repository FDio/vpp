...
func (cc *controlChannel) parseHello() (err error) {
	...
	cc.e.run.NumQueuePairs = min16(cc.e.args.NumQueuePairs, hello.MaxRingS2M)
	cc.e.run.NumQueuePairs = min16(cc.e.args.NumQueuePairs, hello.MaxRingM2S)
	cc.e.run.Log2RingSize = min8(cc.e.args.Log2RingSize, hello.MaxLog2RingSize)
...
fmt.Println("NumQueuePairs: ", cc.e.run.NumQueuePairs)
...
func (cc *controlChannel) parseConnect() (err error) {
	...
	cc.e.peerName = string(connect.Name[:])
        cc.e.isConnecting = false
	cc.e.isConnected = true
