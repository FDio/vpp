...
func (e *epoll) close() error {
        // TODO: maybe interrupt epoll_pwait?
        err := syscall.EpollCtl(e.epfd, syscall.EPOLL_CTL_DEL, e.fd, &e.event)
	if err != nil {
		return fmt.Errorf("EpollCtl: %s", err)
	}
        err = syscall.Close(e.epfd)
        if err != nil {
...
func (l* listener) close() error {
        err := l.epoll.close()
        if err != nil {
                return fmt.Errorf("Failed to close epoll: ", err)
        }
...
func (cc *controlChannel) close(sendMsg bool, str string) (err error) {
        ...
        // remove referance from endpoint
        if cc.e != nil {
                cc.e.cc = nil
        }
