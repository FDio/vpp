...
buildstartupfile() {
    ...
    configfile=full_"$config"_"$mode"
...
buildmaketeststartupfiles() {
    ...
    configfile=full_maketest_"$mode"
    # build config file
    rm -f $configfile
...
joinfiles() {
    ...
    cat "$prefixname" "$suffixname" > full_"$config"_"$mode"
    echo "Joined $prefixname and $suffixname into full_${config}_$mode"
}
...
replay() {
    ...
    if [ "$config" = "maketest" ]; then
        if [ ! -f "full_maketest_replay" ]; then
            echo "Run ./run.sh buildmaketeststartupfiles first"
            exit 1
        fi
    else
...
testvectorsize() {
    ...
    for vecsize in $(seq "$start" "$end"); do
        sed -ir "s/    maxframe.*/    maxframe $vecsize/" $fuzzdir/prefix_"$config"
