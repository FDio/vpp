...
CLIENT_ID = "Vppclient"
VPP_JSON_DIR = os.path.abspath('../..') + '/build-root/install-vpp-native/vpp/share/vpp/api/core'
VPP_JSON_DIR_PLUGIN = os.path.abspath('../..') + '/build-root/install-vpp-native/vpp/share/vpp/api/plugins'
API_FILE_SUFFIX = '*.api.json'

def Main(argv):
    file_flag = False
    try:
        opts, args = getopt.getopt(argv,"hf:p:a:", ["help=","file=","pattern=","actions="])
