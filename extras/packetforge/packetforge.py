...
def GetAction(actions, flow):
    ...
    if (type == "mark"):
        flow.update({'actions': VppEnum.vl_api_flow_action_v2_t.FLOW_ACTION_MARK_V2,
                    'mark_flow_id': int(actions.split(' ')[1])})
    elif (type == "next-node"):
        flow.update({'actions': VppEnum.vl_api_flow_action_v2_t.FLOW_ACTION_REDIRECT_TO_NODE_V2,
                    'redirect_node_index': int(actions.split(' ')[1])})
    elif (type == "buffer-advance"):
        flow.update({'actions': VppEnum.vl_api_flow_action_v2_t.FLOW_ACTION_BUFFER_ADVANCE_V2,
                    'buffer_advance': int(actions.split(' ')[1])})
    elif (type == "redirect-to-queue"):
        flow.update({'actions': VppEnum.vl_api_flow_action_v2_t.FLOW_ACTION_REDIRECT_TO_QUEUE_V2,
