...
func EchoBuiltinEchobytesTest(s *KubeSuite) {
	...
	o, err := vppSrv.VppctlBackground(ctx, "test vperf server uri tcp://"+s.Pods.ServerGeneric.IpAddress+"/1234")
	Log(o)
	AssertNil(err)
	o, err = vppCln.Vppctl(ctx, "test vperf client echo-bytes run-time 10 verbose uri tcp://"+s.Pods.ServerGeneric.IpAddress+"/1234")
	Log(o)
