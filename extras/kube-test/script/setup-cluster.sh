...
setup_master() {
  ...
  # Fetch kube-test template from CalicoVPP: bakes in layout-specific values
  # (image name, VPP build path); per-test-run vars (CALICOVPP_VERSION etc.) stay as placeholders
  make -C "$CALICOVPP_DIR" kube-test-template \
...
setup_release() {
  ...
  git -C "$CALICOVPP_DIR" checkout release/v${CALICOVPP_VERSION} 2>/dev/null || \
    git -C "$CALICOVPP_DIR" checkout v${CALICOVPP_VERSION}
