...
func AssertIperfMinTransfer(result IPerfResult, minTransferred int) {
	if *IsCoverage {
		Log("Coverage build; not asserting")
		return
	}
	if result.Start.Details.Protocol == "TCP" {
		AssertGreaterEqual(result.End.TcpReceived.MBytes, minTransferred)
	} else {
		AssertGreaterEqual(result.End.Udp.MBytes, minTransferred)
	}
}
