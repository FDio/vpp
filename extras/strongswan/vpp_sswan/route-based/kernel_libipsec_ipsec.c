...
/**
 * Maximum of inbound SAs attachment to ipip tunnel.
 * Please look: ipsec_tun_protect_update() from VPP.
 */
#define MAX_N_SA_IN 4
...
struct sa_entry_t
{
  ...
  /** Protocol (ESP/AH) */
  uint8_t proto;
  /** Direction of this SA: in, out, forward */
  uint8_t direction;
};

/**
 * Destroy a sa_entry_t object
 */
void
sa_entry_destroy (sa_entry_t *this)
{
  free (this);
}
...
struct policy_entry_t
{
  ...
  /**
   * Installed outbound SA (sa_entry_t)
   */
  sa_entry_t out_sa;

  /**
   * List of installed inbound SAs (sa_entry_t)
   */
  linked_list_t *in_sas;
};
		     ...
		     policy_dir_t dir)
{
  ...
  this->out_sa.spi = ~0;
  this->in_sas = linked_list_create ();
  return this;
}

/**
 * Create a sa_entry_t object
 */
static sa_entry_t *
create_sa_entry (uint32_t spi, uint8_t proto, policy_dir_t dir)
{
  sa_entry_t *this;
  INIT (this, .direction = dir, .spi = spi, .proto = proto);
...
/** \brief Update tunnel protect
   ...
   tunnel protect
    @param sa_in - array of indices sa to attach for inbound
    @param n_sa_in - number of indices sa to attach for inbound
    @param sa_out - index of sa to attach for outbound
*/
void
kernel_libipsec_vpp_tunnel_protect_update (uint32_t sw_if_index,
					   uint32_t *sa_in, uint8_t n_sa_in,
					   uint32_t sa_out)
{
  ...
  char *out = NULL;
  int out_len, i;
  int msg_length = (sizeof (*mp) + (sizeof (*mp->tunnel.sa_in) * n_sa_in));
...
policy_entry_destroy (policy_entry_t *this)
{
  ...
  if (this->in_sas)
    {
      this->in_sas->destroy_function (this->in_sas, (void *) sa_entry_destroy);
    }
...
CALLBACK (sa_entry_equals, bool, sa_entry_t *a, va_list args)
{
  sa_entry_t *b;

  VA_ARGS_VGET (args, b);
  return a->spi == b->spi && a->proto == b->proto &&
	...
	kernel_ipsec_del_sa_t *data)
{
  ...
  DBG4 (DBG_KNL, "removing SA: %x", ntohl (sa->sa_id_p->spi));
 ...
 */
static bool
install_route (private_kernel_libipsec_vpp_ipsec_t *this, host_t *input_src,
	       host_t *input_dst, traffic_selector_t *input_src_ts,
	       traffic_selector_t *input_dst_ts,
	       policy_entry_t *input_policy_1, policy_entry_t *input_policy_2,
	       uint32_t if_id, policy_dir_t dir)
{
  ...
  sa_entry_t *sa = NULL, *sa_found = NULL;
  enumerator_t *enumerator = NULL;

  /* Assigned variables depends on directory */
  if (dir == POLICY_IN)
    {
      policy_in = input_policy_1;
      policy_out = input_policy_2;
      src = input_dst;
      dst = input_src;
      src_ts = input_dst_ts;
      dst_ts = input_src_ts;
    }
  else if (dir == POLICY_OUT)
    {
      policy_in = input_policy_2;
      policy_out = input_policy_1;
      src = input_src;
      dst = input_dst;
      src_ts = input_src_ts;
      dst_ts = input_dst_ts;
  ...
  uint8_t n_sa_in = 0;
  uint32_t n_sa_in_array[MAX_N_SA_IN] = { 0 };
  ...
  o_key.proto = policy_out->out_sa.proto;
  o_key.spi = policy_out->out_sa.spi;
  ...
  if (policy_in != NULL)
    {
      ...
      enumerator = policy_in->in_sas->create_enumerator (policy_in->in_sas);
      while (enumerator->enumerate (enumerator, &sa))
	{
	  i_key.proto = sa->proto;
	  i_key.spi = sa->spi;
	  i_sa = this->sas->get (this->sas, &i_key);
	  if (i_sa != NULL)
	    {
	      n_sa_in_array[n_sa_in] = i_sa->sa_id;
	      n_sa_in++;
	    }
	  else
	    {
	      DBG1 (DBG_KNL, "Do not found SA for inbound!");
	    }
	}
  ...
  if (i_sa != NULL && o_sa != NULL && sw_if_index_ipip_tun != ~0)
    {
      kernel_libipsec_vpp_tunnel_protect_update (
	sw_if_index_ipip_tun, n_sa_in_array, n_sa_in, o_sa->sa_id);
    }
...
{
  policy_entry_t *policy_1, *found_1 = NULL;
  policy_entry_t *policy_2, *found_2 = NULL;
  status_t status;
  host_t *src = NULL, *dst = NULL;
  ...
  policy_1 = create_policy_entry (id->src_ts, id->dst_ts, id->dir);

  this->mutex->lock (this->mutex);
  if (this->policies->find_first (this->policies, policy_entry_equals,
				  (void **) &found_1, policy_1))
    {
      policy_entry_destroy (policy_1);
      policy_1 = found_1;
    }
  else
    { /* use the new one, if we have no such policy */
      this->policies->insert_last (this->policies, policy_1);
    }
  policy_1->refs++;
  ...
  if (id->dir == POLICY_IN)
    {
      sa = create_sa_entry (spi, proto, id->dir);

      if (policy_1->in_sas->find_first (policy_1->in_sas, sa_entry_equals,
					(void **) &sa_found, sa))
	{
	  sa_entry_destroy (sa);
	  sa = sa_found;
	...
	{ /* use the new one, if we have no such sa */
	  policy_1->in_sas->insert_last (policy_1->in_sas, sa);
	}
    }
  else if (id->dir == POLICY_OUT)
    {
      policy_1->out_sa.spi = spi;
      policy_1->out_sa.proto = proto;
    }
  ...
  if (id->dir != POLICY_FWD)
	...
	dir = POLICY_IN;
      ...
      policy_2 = create_policy_entry (id->dst_ts, id->src_ts, dir);

      if (this->policies->find_first (this->policies, policy_entry_equals,
				      (void **) &found_2, policy_2))
	{
	  policy_entry_destroy (policy_2);
	  policy_2 = found_2;
	}
      else
	{ /* do not found the second policy */
      ...
      if (!install_route (this, data->src, data->dst, id->src_ts, id->dst_ts,
			  policy_1, policy_2, if_id, id->dir))
	{
	  return FAILED;
	}
    }
	...
	kernel_ipsec_policy_id_t *id, kernel_ipsec_manage_policy_t *data)
{
  ...
  policy_entry_t *policy_out = NULL, *found_out = NULL;
  ...
  DBG4 (DBG_KNL, "**** del_policy, SPI: %0x, dir: %s",
	ntohl (data->sa->esp.spi),
	id->dir == POLICY_OUT ?
		"POLICY_OUT" :
		(id->dir == POLICY_IN ? "POLICY_IN" : "POLICY_FWD"));
  ...
  /* check if SA is AH */
    ...
    DBG1 (DBG_KNL, "invalid protocol !!!");

  /* only for [in] policy to clean-up and updated ipip tunnel settings */
  if (id->dir == POLICY_IN)
    {
      sa = create_sa_entry (spi, proto, id->dir);

      if (policy->in_sas->find_first (policy->in_sas, sa_entry_equals,
				      (void **) &sa_found, sa))
	{
	  sa_entry_destroy (sa);

	  policy->in_sas->remove (policy->in_sas, sa_found, NULL);
	  sa_entry_destroy (sa_found);
	}

      /* found [out] policy to be able update ipip tunnel protect */
      policy_out = create_policy_entry (id->dst_ts, id->src_ts, POLICY_OUT);

      if (this->policies->find_first (this->policies, policy_entry_equals,
				      (void **) &found_out, policy_out))
	{
	  policy_entry_destroy (policy_out);
	  policy_out = found_out;
	}
      else
	{ /* do not found the outbound policy */
	  policy_entry_destroy (policy_out);
	  policy_out = NULL;
	}

      /* update ipip tunnel protect */
      if (!install_route (this, data->src, data->dst, id->src_ts, id->dst_ts,
			  policy, policy_out, if_id, id->dir))
	{
	  return FAILED;
	}
    }
  ...
  if (policy->route)
	    ...
	    route->src_ip, route->if_name, FALSE) != SUCCESS)
	{
      ...
      if (!kernel_libipsec_vpp_delete_tunnel (if_id))
