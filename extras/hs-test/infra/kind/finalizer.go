...
func (s *KindSuite) RunFinalizerTest(ctx context.Context, testName string, jobFinalizers, podFinalizers []string, jobSuccess bool) {
	...
	// Wait for 30 seconds for job to start and spawn pods/containers
	s.Log("Waiting for 30s for the job to start and spawn the pod ...")
	...
	if !jobSuccess {
		...
		s.Log("Job failed as expected, waiting for TTL cleanup...")
		// wait for another 10 seconds for the failed jobs to clean up
