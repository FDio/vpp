...
func (s *KindSuite) execInPod(namespace, podName, containerName string, command ...string) (string, error) {
	...
	s.Log("Executing kubectl command: kubectl %v", kubectlArgs)
	cmd := exec.Command("kubectl", kubectlArgs...)
	output, err := cmd.CombinedOutput()

	if err != nil {
		s.Log("kubectl exec failed: %v", err)
		s.Log("kubectl output: %s", string(output))
	}
...
func (s *KindSuite) ExecInKindNodeContainer(kindNodeName, containerName string, command ...string) ([]byte, error) {
	...
	if err != nil {
		...
		return []byte(output), fmt.Errorf("failed to execute command on %s in container %s: %v", kindNodeName, containerName, err)
	}
...
// This is a convenience function for the common case of running vppctl commands
