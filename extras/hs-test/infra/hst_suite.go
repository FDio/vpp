...
var NConfiguredVppCpus = flag.Int("vpp_cpus", 3, "number of CPUs assigned to vpp containers")
...
func (s *HstSuite) SkipIfNotEnoughAvailableCpus() {
	...
	maxRequestedCpu = (GinkgoParallelProcess() * s.CpusPerContainer)
	...
	if availableCpus < maxRequestedCpu {
		s.Skip(fmt.Sprintf("Test case cannot allocate requested cpus "+
			...
			s.CpusPerContainer, availableCpus))
	}
}
