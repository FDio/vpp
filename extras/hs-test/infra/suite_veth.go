...
func (s *VethsSuite) SetupTest() {
	...
	if *DryRun {
		...
		for name := range s.Containers {
			s.Log("\033[36mdocker start " + name + " && docker exec -it " + name + " bash")
		}
		s.Log("\nvpp -c /tmp/server-share/etc/vpp/startup.conf &> /proc/1/fd/1")
		s.Log("vppctl -s /tmp/server-share/var/run/vpp/cli.sock\n")

		s.Log("vpp -c /tmp/client-share/etc/vpp/startup.conf &> /proc/1/fd/1")
		s.Log("vppctl -s /tmp/client-share/var/run/vpp/cli.sock\n")
