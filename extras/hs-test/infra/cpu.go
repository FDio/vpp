...
func iterateAndAppend(start int, end int, slice []int) {
	for i := start; i <= end; i++ {
...
func (c *CpuAllocatorT) readCpus() error {
	...
	if c.runningInCi {
		...
		iterateAndAppend(first, second, c.cpus)
		iterateAndAppend(third, fourth, c.cpus)
		...
		for i := first; i <= second; i++ {
			...
			iterateAndAppend(third, fourth, tmpCpus)
			...
			iterateAndAppend(fifth, sixth, tmpCpus)
