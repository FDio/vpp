...
func (s *HstSuite) StartCurl(finished chan error, uri, netNs string, args []string) {
	defer func() {
		finished <- errors.New("curl error")
	}()

	c := []string{"curl", "-v", "-s", "-k", "--max-time", "10", "-o", "/dev/null", "--noproxy", "*"}
	c = append(c, args...)
	c = append(c, uri)
	...
	if err != nil {
		...
		return
	} else if !strings.Contains(string(o), "200") {
		finished <- fmt.Errorf("curl error: response not 200 OK")
		return
	}
	finished <- nil
}
