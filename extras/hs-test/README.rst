...
#. ``make list-tests`` (or ``make help``) shows all test names and updates the `list of tests`_ at the bottom of this document.
...
#. ``funcValue.Name()`` returns the full name of a function (e.g. fd.io/hs-test.MirroringTest), however, we only need the test name (e.g. MirroringTest).
