...
To add a multiworker test, name it ``[name]MTTest``. Doing this, the framework will allocate 3 CPUs to a VPP container, no matter what ``CPUS`` is set to.
Only a single multiworker VPP container is supported for now. Please register multiworker tests as Solo tests to avoid reusing the same cores 
