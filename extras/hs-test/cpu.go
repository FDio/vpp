...
func (c *CpuContext) Release() {
	fmt.Println("before release ctx", c.cpus)
	fmt.Println("before release available", c.cpuAllocator.cpus)
	c.cpuAllocator.cpus = append(c.cpuAllocator.cpus, c.cpus...)
	fmt.Println("after release ctx", c.cpus)
	fmt.Println("after release available", c.cpuAllocator.cpus)
}
...
func (c *CpuAllocatorT) Allocate(nCpus int) (*CpuContext, error) {
	...
	fmt.Println("allocated cpus ", cpuCtx.cpus)
	c.cpus = c.cpus[nCpus:]
	fmt.Println("left cpus ", c.cpus)
	return &cpuCtx, nil
}
