...
func LDPreloadIperfVppTest(s *LdpSuite) {
	...
	go func() {
		...
		s.StartServerApp_v2(serverContainer, "iperf3", cmd, srvCh, stopServerCh)
	}()
	...
	go func() {
		...
		s.StartClientApp_v2(clientContainer, cmd, clnCh, clnRes)
	}()
	s.Log(<-clnRes)
...
func RedisBenchmarkTest(s *LdpSuite) {
	...
	go func() {
		...
		s.StartServerApp_v2(serverContainer, "redis-server", cmd, runningSrv, doneSrv)
	}()
