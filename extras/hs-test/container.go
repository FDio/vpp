 ...
 */
func (c *Container) execServer(command string, envVars map[string]string) error {
	envCliOptions := ""
	if envVars != nil {
		for name, value := range envVars {
			envCliOptions += fmt.Sprintf(" -e %s=%s", name, value)
		}
	}
	return exechelper.Run("docker exec -d" + envCliOptions + " " + c.name + " " + command)
}

func (c *Container) exec(command string, envVars map[string]string) (string, error) {
	envCliOptions := ""
	if envVars != nil {
		for name, value := range envVars {
			envCliOptions += fmt.Sprintf(" -e %s=%s", name, value)
	...
	cliCommand := "docker exec" + envCliOptions + " " + c.name + " " + command
