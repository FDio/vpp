...
func NewContainer(yamlInput map[string]interface{}) (*Container, error) {
	...
	if _, ok := yamlInput["volumes"]; ok {
		...
		for _, volu := range yamlInput["volumes"].([]interface{}) {
			volumeMap := volu.(map[string]interface{})
...
func (c *Container) getVolumeByName(hostDir string) Volume {
...
func (c *Container) getVolumesAsCliOption() string {
	...
	if len(c.volumes) > 0 {
		for _, volume := range c.volumes {
			dockerOption += fmt.Sprintf(" -v %s:%s", volume.hostDir, volume.containerDir)
		}
	}
...
func (c *Container) useVolumeAsWorkDir(hostDir string) *Container {
	var testCaseCopy Container
	testCaseCopy.isOptional = c.isOptional
	testCaseCopy.name = c.name
	testCaseCopy.image = c.image
	testCaseCopy.workDir = c.volumes[hostDir].containerDir
	testCaseCopy.volumes = c.volumes
	testCaseCopy.envVars = c.envVars
	return &testCaseCopy
}
...
func (c *Container) getEnvVarsAsCliOption() string {
	...
	for name, value := range c.envVars {
		dockerOption += fmt.Sprintf(" -e %s=%s", name, value)
	}
	return dockerOption
}
...
func (c *Container) exec(command string) (string, error) {
	dockerCommand := "docker exec -d " + c.name + " " + command
	byteOutput, err := exechelper.CombinedOutput(dockerCommand)
