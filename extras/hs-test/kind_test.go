...
func KindIperfVclTest(s *KindSuite) {
	...
	err := s.Exec(s.ClientName, s.ClientName, s.Namespace, []string{"/bin/bash", "-c", vclConf})
	s.AssertNil(err)
	err = s.Exec(s.ServerName, s.ServerName, s.Namespace, []string{"/bin/bash", "-c", vclConf})
	s.AssertNil(err)

	err = s.Exec(s.ServerName, s.ServerName, s.Namespace, []string{"/bin/bash", "-c",
		"VCL_CONFIG=" + vclPath + " LD_PRELOAD=" + ldpPath + " iperf3 -s -D -4"})
	s.AssertNil(err)
	err = s.Exec(s.ClientName, s.ClientName, s.Namespace, []string{"/bin/bash", "-c",
