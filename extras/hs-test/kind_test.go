...
func init() {
	RegisterKindTests(KindIperfVclTest, NginxRpsTest, NginxProxyMirroringTest,
		FelixCniServerRaceTest, FelixPolicyChurnTest,
		FelixPodChurnTest, FinalizerTests)
}
...
func FelixCniServerRaceTest(s *KindSuite) {
	...
	defer func() {
		...
		if err != nil {
			s.Log("Failed to cleanup jobs with finalizers: %v", err)
		}
	...
	if err != nil {
		s.Log("Failed to create diverse base policies: %v", err)
		return
	}
	...
	go func() {
		...
		for {
			...
			for i := 0; i < 3; i++ {
				...
				if err != nil {
					s.Log("Failed to create dynamic policy %d (port %d): %v", counter, port, err)
				} else {
					s.Log("Created dynamic policy %d with port %d", counter, port)
				}
				counter++
			}
		}
	...
	finalPods, _ := s.ListPodsInNamespace(ctx, s.Namespace)
	s.AssertNil(finalPods)

	// Verify all jobs have been cleaned up (no finalizers!)
	finalJobs, _ := s.ListJobsInNamespace(ctx, s.Namespace)
	s.AssertNil(finalJobs)
...
func FelixPodChurnTest(s *KindSuite) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				s.Log("Failed to create dynamic pod %s: %v", podName, err)
			}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				s.Log("Failed to delete pod %s: %v", podToDelete, err)
			}
	...
	if err != nil {
		s.Log("Failed to get all pods: %v", err)
	} else {
	...
	for _, podName := range remainingPods {
		...
		if err != nil {
			s.Log("Failed to cleanup pod %s: %v", podName, err)
		}
	}
}
...
func FelixPolicyChurnTest(s *KindSuite) {
	...
	err := s.CreateDefaultNetworkPolicies(ctx, s.Namespace)
	if err != nil {
		s.Log("Failed to create network policies: %v", err)
		return
	}
	...
	go func() {
		...
		for {
			...
			for i := 0; i < 3; i++ {
				...
				if err != nil {
					s.Log("Failed to create dynamic diverse policies %d: %v", updateCounter, err)
				}
	...
	if err != nil {
		s.Log("Failed to run 'show capo rules' on kind-worker: %v", err)
	} else {
	...
	if err != nil {
		s.Log("Failed to run 'show capo rules' on kind-worker2: %v", err)
	} else {
		// Verify the expected network policy patterns are present
