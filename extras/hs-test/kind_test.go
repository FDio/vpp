...
func NginxRpsTest(s *KindSuite) {
	...
	s.Exec(s.PodNames.Nginx, s.ContainerNames.Server, []string{"chmod", "+x", "nginx.sh"})

	s.Exec(s.PodNames.Ab, s.ContainerNames.Client, []string{"/bin/bash", "-c", symLink})
	s.Exec(s.PodNames.Nginx, s.ContainerNames.Server, []string{"/bin/bash", "-c", symLink})
	...
	// give nginx a while to start up
