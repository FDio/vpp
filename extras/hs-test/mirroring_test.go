...
func (s *MirroringSuite) TestMirroring() {
	proxyAddress := s.netInterfaces[mirroringClientInterfaceName].IP4AddressString()
...
//        vppErrors := vppProxyContainer.vppInstance.vppctl("show error")
//        s.log(vppErrors)
//
//        vppInterfaces := vppProxyContainer.vppInstance.vppctl("sh int")
//        s.log(vppInterfaces)
}
