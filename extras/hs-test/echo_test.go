...
func TcpWithLossTest(s *VethsSuite) {
	...
	if !s.CoverageRun {
		s.Log("[%s]\n Baseline: %v gbit/s\nWith loss: %v gbit/s", Arch, baseline, withLoss)
		s.AssertGreaterEqual(baseline, withLoss)
		if Arch == "x86_64" {
			s.AssertGreaterEqual(baseline, 0.08)
			s.AssertGreaterEqual(withLoss, 0.03)
		} else {
			s.AssertGreaterEqual(baseline, 0.05)
			s.AssertGreaterEqual(withLoss, 0.02)
		}
	}
}
...
func TcpWithLoss6Test(s *Veths6Suite) {
	...
	if !s.CoverageRun {
		...
		if Arch == "x86_64" {
			...
			s.AssertGreaterEqual(baseline, 0.04)
			s.AssertGreaterEqual(withLoss, 0.015)
		}
	}
}
