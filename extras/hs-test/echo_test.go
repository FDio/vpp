...
func EchoBuiltinPeriodicReportTotalTest(s *VethsSuite) {
	...
	o := clientVpp.Vppctl("test echo client bytes 16m throughput 8m report-interval-total 0.5" +
		" uri tcp://" + s.Interfaces.Server.Ip4AddressString() + "/" + s.Ports.Port1)
	s.Log(o)
	s.AssertContains(o, "Test started")
	s.AssertContains(o, "Test finished")
	if regex.MatchString(o) {
		...
		for i := 0; i < 4; i++ {
			...
			s.AssertEqualWithinThreshold(mbytes, 4.0*(i+1), 0.5)
		}
		// Verify reporting times
		s.AssertEqual(matches[0][1], ".5")
		s.AssertEqual(matches[1][1], "1.0")
		s.AssertEqual(matches[2][1], "1.5")
		s.AssertEqual(matches[3][1], "2.0")
	} else {
		s.AssertEmpty("invalid echo test client output")
	}
}
...
func EchoBuiltinPeriodicReportTest(s *VethsSuite) {
	regex := regexp.MustCompile(`(\d?\.\d)-(\d?.\d)\s+(\d+\.\d+)\s+\d+\.\d+\s+\d+\.\d+`)
	serverVpp := s.Containers.ServerVpp.VppInstance
	...
	o := clientVpp.Vppctl("test echo client bytes 16m throughput 8m report-interval 0.5" +
		" uri tcp://" + s.Interfaces.Server.Ip4AddressString() + "/" + s.Ports.Port1)
	s.Log(o)
	s.AssertContains(o, "Test started")
	s.AssertContains(o, "Test finished")
	if regex.MatchString(o) {
		...
		for i := 0; i < 4; i++ {
			...
			s.AssertEqualWithinThreshold(mbytes, 4.0, 0.5)
		}
		// Verify time interval numbers
		s.AssertEqual(matches[0][1], "0.0")
		s.AssertEqual(matches[0][2], ".5")
		s.AssertEqual(matches[1][1], ".5")
		s.AssertEqual(matches[1][2], "1.0")
		s.AssertEqual(matches[2][1], "1.0")
		s.AssertEqual(matches[2][2], "1.5")
		s.AssertEqual(matches[3][1], "1.5")
		s.AssertEqual(matches[3][2], "2.0")
	} else {
		s.AssertEmpty("invalid echo test client output")
	}
}
