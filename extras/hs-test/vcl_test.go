...
func (s *Veths2Suite) TestTcpWithLoss() {
	...
	s.assertNil(err, "creating container failed", err)
	...
	serverVpp.setServer()
	err = serverVpp.start()
	s.assertNil(err, "starting VPP failed", err)

	_, err = serverVpp.vppctl("test echo server uri tcp://10.10.10.1/20022")
	s.assertNil(err, "starting echo server failed", err)

	clientContainer, err := s.NewContainer("client")
	s.assertNil(clientContainer, "creating container failed", err)
	err = clientContainer.start()
	s.assertNil(err, "starting container failed", err)
