...
func (s *VethsSuite) testVclEcho(proto string) {
	srvVppContainer := s.containers[0]
	srvVol := s.volumes[0]

	_, err := srvVppContainer.useVolumeAsWorkDir(srvVol).execAction("Configure2Veths srv")
	s.assertNil(err)

	clnVppContainer := s.containers[1]
	clnVol := s.volumes[1]

	_, err = clnVppContainer.useVolumeAsWorkDir(clnVol).execAction("Configure2Veths cln")
	s.assertNil(err)

	echoSrvContainer := s.containers[2]

	// run server app
	_, err = echoSrvContainer.useVolumeAsWorkDir(srvVol).execAction("RunEchoServer "+proto)
	s.assertNil(err)

	echoClnContainer := s.containers[3]

	o, err := echoClnContainer.useVolumeAsWorkDir(clnVol).execAction("RunEchoClient "+proto)
	s.assertNil(err)
...
func (s *VethsSuite) testRetryAttach(proto string) {
	...
	_, err := srvVppContainer.useVolumeAsWorkDir(srvVol).execAction("Configure2Veths srv-with-preset-hw-addr")
	s.assertNil(err)
	...
	echoSrvContainer := s.containers[2]
	_, err = echoSrvContainer.useVolumeAsWorkDir(srvVol).execAction("RunVclEchoServer "+proto)
	s.assertNil(err)
	...
	echoClnContainer := s.containers[3]
	_, err = echoClnContainer.useVolumeAsWorkDir(clnVol).execAction("RunVclEchoClient "+proto)
	s.assertNil(err)
...
func (s *VethsSuite) TestTcpWithLoss() {
	...
	serverVpp := NewVppInstance(serverContainer.useVolumeAsWorkDir(serverVolume))
	...
	clientVpp := NewVppInstance(clientContainer.useVolumeAsWorkDir(clientVolume))
