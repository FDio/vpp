 ...
 *
 *       This structure targets a very specific and quite narrow set of
 * use-cases that are not covered by other hash tables.
 *
 ...
 * In particular, it was designed to minimize blocking reads, register usage
 * and cache-lines accesses during a typical lookup. This hash table therefore
 * provides stateful packet processing without performance degradation even
 * when every single lookup has to fetch memory from RAM. This hash table is
 * not thread-safe and requires executing a garbage collection function to
 * clean-up chained buckets.
 *
 *  -- Overview:
 *
 ...
 * single entry. In the absence of hash collision, a single entry which
 * location can deterministically be determined from the key-hash and the hash
 * table header is accessed (One single cache line, without indirection). This
 ...
 *  to this function examines a single fixed entry. It shall therefore be
 * called as many times as there are fixed entries in the hash table in order
 * to ensure a full inspection.
 *
 ...
 *  When seconds are used, the system should run for 136 years without any
 * issue. If milliseconds are used, a shift should be operated on all timeout
 * values on a regular basis (more than every 49 days).
 */
...
#define flowhash_foreach_entry(h, ei)                                         \
  for (ei = 1; ei < (h)->total_entries; ei++)
...
#define flowhash_foreach_valid_entry(h, ei, now)                              \
  flowhash_foreach_entry (h, ei) if (((now) <= (h)->entries[ei].timeout))
...
/**
 * @brief Adjust, if necessary, provided parameters such as being valid
 * flowhash sizes.
 */
static void
flowhash_validate_sizes (u32 *fixed_entries, u32 *collision_buckets)
{
...
static_always_inline FVT (flowhash) *
  FV (flowhash_alloc) (u32 fixed_entries, u32 collision_buckets)
{
  FVT (flowhash) * h;
  uword size;
  void *mem;
  ...
  entries =
    1 + fixed_entries + collision_buckets * FLOWHASH_ENTRIES_PER_BUCKETS;
  ...
  for (i = 1; i <= collision_buckets; i++)
    {
      h->free_buckets_indices[-i] = entries - i * FLOWHASH_ENTRIES_PER_BUCKETS;
    }
...
static void FV (__flowhash_get_chained) (FVT (flowhash) * h,
					 FVT (flowhash_lkey) * k, u32 hash,
					 u32 time_now, u32 *ei);

/**
 * @brief Retrieves an entry index corresponding to a provided key and its
 * hash.
 *
 ...
 * used to access the stored value as well as get or set its associated
 * timeout. The key stored in the returned entry is always set to the provided
 * key.
 *
...
static_always_inline void FV (__flowhash_get_chained) (FVT (flowhash) * h,
						       FVT (flowhash_lkey) * k,
						       u32 hash, u32 time_now,
						       u32 *ei)
{
  ...
  if (h->entries[*ei].chained_entry_index == FLOWHASH_INVALID_ENTRY_INDEX)
      ...
      if (h->free_buckets_position == 0)
	{
	  ...
	  *ei = FLOWHASH_INVALID_ENTRY_INDEX;
	  h->entries[FLOWHASH_INVALID_ENTRY_INDEX].timeout = time_now - 1;
	  FV (flowhash_cpy_key)
  ...
  u32 bi1 = bi0 + 1;
  bi1 = (bi0 & (FLOWHASH_ENTRIES_PER_BUCKETS - 1)) ?
	  bi1 :
...
static_always_inline u32 FV (flowhash_elts) (FVT (flowhash) * h, u32 time_now)
{
  ...
  flowhash_foreach_valid_entry (h, ei, time_now) tot++;
