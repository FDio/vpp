				       ...
				       void *handle)
{
  ...
  s = format (s, "remote_ep_timeout %d",
				   ...
				   void *handle)
{
  ...
  s = format (s, "prefix %U/%d", format_vl_api_address,
				     ...
				     void *handle)
{
  ...
  for (int i = 0; i < a->contract.n_rules; i++)
      ...
      for (int j = 0; j < a->contract.rules[i].nh_set.n_nhs; j++)
	{
	  ...
	  s = format (s, "nhs mac %U ", format_vl_api_mac_address,
		      a->contract.rules[i].nh_set.nhs[j].mac);
	  s = format (s, "nhs bd_id %d ",
		      ntohl (a->contract.rules[i].nh_set.nhs[j].bd_id));
	  s = format (s, "nhs rd_id %d",
