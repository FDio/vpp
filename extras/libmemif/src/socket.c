...
memif_msg_enq_hello (memif_control_channel_t *cc)
{
  ...
  strncpy ((char *) h->name, (char *) cc->sock->args.app_name,
	   sizeof(h->name));
...
memif_msg_enq_init (memif_control_channel_t *cc)
{
  ...
  strncpy ((char *) i->name, (char *) cc->sock->args.app_name,
	   sizeof(i->name));
  if (strlen ((char *) cc->conn->args.secret) > 0)
    strncpy ((char *) i->secret, (char *) cc->conn->args.secret, sizeof (i->secret));
...
static int
memif_msg_enq_add_ring (memif_control_channel_t *cc, uint8_t index, uint8_t dir)
{
  memif_msg_queue_elt_t *e = memif_msg_enq(cc);
...
memif_msg_enq_connect (memif_control_channel_t *cc)
{
  ...
  e->fd = -1;
  strncpy ((char *) cm->if_name, (char *) cc->conn->args.interface_name,
	   sizeof(cm->if_name));
...
int
memif_msg_enq_disconnect (memif_control_channel_t *cc, uint8_t * err_string, uint32_t err_code)
{
  memif_msg_queue_elt_t *e;
			      ...
			      int fd)
{
  ...
  mr =
    (memif_region_t *) cc->sock->args.realloc (c->regions,
...
static int
memif_msg_parse_add_ring (memif_control_channel_t *cc, memif_msg_t * msg, int fd)
{
  ...
  if (ar->flags & MEMIF_MSG_ADD_RING_FLAG_S2M)
	...
	return MEMIF_ERR_MAXRING;
      ...
      mq =
	(memif_queue_t *) cc->sock->args.realloc (c->rx_queues,
	...
	return MEMIF_ERR_MAXRING;
      ...
      mq =
	(memif_queue_t *) cc->sock->args.realloc (c->tx_queues,
				       sizeof (memif_queue_t) *
				       (++c->tx_queues_num));
      memset (mq + ar->index, 0, sizeof (memif_queue_t));
      if (mq == NULL)
...
memif_msg_receive_and_parse (memif_control_channel_t *cc)
{
  ...
  switch (msg.type)
    ...
    case MEMIF_MSG_TYPE_ADD_REGION:
      if ((err =
	   memif_msg_parse_add_region (cc, &msg, fd)) != MEMIF_ERR_SUCCESS)
	return err;
      if ((err = memif_msg_enq_ack (cc)) != MEMIF_ERR_SUCCESS)
	return err;
      break;
    ...
    case MEMIF_MSG_TYPE_ADD_RING:
      if ((err =
	   memif_msg_parse_add_ring (cc, &msg, fd)) != MEMIF_ERR_SUCCESS)
	return err;
      if ((err = memif_msg_enq_ack (cc)) != MEMIF_ERR_SUCCESS)
	return err;
      break;
