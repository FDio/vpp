...
/** \brief Memif get tx ring details
    ...
    @param qid - queue id
    @param ndesc - number of allocated memif_desc_details_t (must be at least
   equal to ring size)

    Note: don't forget to allocate descs field in memif_ring_details_t to hold
   all the descriptors.

    \return memif_err_t
*/
int memif_get_tx_ring_details (memif_conn_handle_t conn,
			       memif_ring_details_t *rd, uint16_t qid,
			       uint16_t ndesc);
...
/** \brief Memif get rx ring details
   ...
   all the descriptors.
    ...
    \return memif_err_t
*/
int memif_get_rx_ring_details (memif_conn_handle_t conn,
			       memif_ring_details_t *rd, uint16_t qid,
			       uint16_t ndesc);
...
/** \brief Memif buffer enq tx at idx
    ...
    Similar to memif_buffer_enq_tx but enqueues the buffer at specified
   descriptor index. Updates desc_index field for the memif buffer.
*/
int memif_buffer_enq_tx_at_idx (memif_conn_handle_t conn, uint16_t qid,
