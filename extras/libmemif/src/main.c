...
memif_strerror (int err_code)
{
  if (err_code >= ERRLIST_LEN)
    ...
    {
      strncpy (memif_buf, memif_errlist[err_code], sizeof (memif_buf));
	      ...
	      memif_on_interrupt_t *on_interrupt, void *private_ctx)
{
  ...
  if ((l = strlen ((char *) args->secret)) > 0)
    strncpy ((char *) conn->args.secret, (char *) args->secret,
...
memif_connect1 (memif_connection_t * c)
{
  ...
  for (i = 0; i < c->rx_queues_num; i++)
      ...
      if (mq != NULL)
	{
	  ...
	  mq->ring->head = mq->ring->tail = mq->last_head = mq->next_buf =
	    0;
	}
    }
...
int
memif_set_next_free_descriptor (memif_conn_handle_t conn, uint16_t qid, uint16_t index)
{
  memif_connection_t *c = (memif_connection_t *) conn;
...
static void
memif_buffer_enq_tx_at_idx_internal (memif_connection_t *c, memif_queue_t *from_q, memif_queue_t *to_q, memif_buffer_t *buf, uint16_t slot)
{
  ...
  *to_d = tmp_d;
  ...
  /* Update source queue counters */
  from_q->next_buf = buf->desc_index + 1;

  /* Update descriptor index and queue for clients buffer */
  buf->desc_index = slot & to_mask;
  buf->queue = to_q;

  /* Update target queue counters */
  to_q->next_buf = buf->desc_index + 1;
}

int
memif_buffer_enq_tx_at_idx (memif_conn_handle_t conn, uint16_t qid, memif_buffer_t *buf, uint16_t index)
{
  memif_connection_t *c = (memif_connection_t *) conn;
  if (EXPECT_FALSE (c == NULL))
    return MEMIF_ERR_NOCONN;
  ...
  memif_buffer_enq_tx_at_idx_internal(c, (memif_queue_t *) buf->queue, &c->tx_queues[qid], buf, index);
		     ...
		     uint16_t * count_out)
{
  ...
  *count_out = 0;
  ...
  ns = ring_size - slot + ring->tail;

  b0 = bufs;

  while (count && ns)
    {
      /* Swaps the descriptors, updates next_buf pointer and updates client memif buffer */
      memif_buffer_enq_tx_at_idx_internal(c, (memif_queue_t *) b0->queue, mq, b0, mq->next_buf);
	  ...
	  if (EXPECT_FALSE (dst_left == 0))
	      ...
	      if (count && ns)
		{
		  *count_out += 1;
		  ...
		  b0->desc_index = mq->next_buf;
		  dst_left =
		    (c->args.is_master) ? ring->desc[mq->next_buf & mask].
		...
		memif_buffer_t * bufs, uint16_t count, uint16_t * tx)
{
  ...
  *tx = 0;
  ...
  while (count)
    {
      b0 = (bufs + *tx);
      /* FIXME: set error to MEMIF_ERR_INVAL_ARG and finish the sending process */
...
int
memif_get_ring_details (memif_connection_t *c, memif_ring_details_t *rd,
			uint16_t qid, uint16_t ndesc, uint8_t direction)
{
  memif_queue_t *mq;
  memif_ring_t *r;
  ...
  if ((c->rx_queues_num <= qid) ||
      (ndesc < (1 << c->rx_queues[qid].log2_ring_size)))
    return MEMIF_ERR_INVAL_ARG;
...
int
memif_get_rx_ring_details (memif_conn_handle_t conn, memif_ring_details_t *rd,
			   uint16_t qid, uint16_t ndesc)
{
  memif_connection_t *c = (memif_connection_t *) conn;
  ...
  return memif_get_ring_details (
    c, rd, qid, ndesc, c->args.is_master ? MEMIF_RING_S2M : MEMIF_RING_M2S);
}

int
memif_get_tx_ring_details (memif_conn_handle_t conn, memif_ring_details_t *rd,
			   uint16_t qid, uint16_t ndesc)
{
  memif_connection_t *c = (memif_connection_t *) conn;
  ...
  return memif_get_ring_details (
    c, rd, qid, ndesc, c->args.is_master ? MEMIF_RING_M2S : MEMIF_RING_S2M);
}
