...
static void
memif_buffer_enq_tx_at_idx_internal (memif_connection_t *c,
				     memif_queue_t *from_q,
				     memif_queue_t *to_q, memif_buffer_t *buf,
				     uint16_t slot)
{
...
int
memif_buffer_enq_tx_at_idx (memif_conn_handle_t conn, uint16_t qid,
			    memif_buffer_t *buf, uint16_t index)
{
  memif_connection_t *c = (memif_connection_t *) conn;
  ...
  memif_buffer_enq_tx_at_idx_internal (c, (memif_queue_t *) buf->queue,
				       &c->tx_queues[qid], buf, index);
  ...
  while (count && ns)
    {
       ...
       * memif buffer */
      memif_buffer_enq_tx_at_idx_internal (c, (memif_queue_t *) b0->queue, mq,
					   b0, mq->next_buf);
