...
void login (log_type type, const char *format, ...);

#ifdef ICMP_DBG


#define DBG(...) do {                                               \
char msg[MAX_LOG_CHAR];                                             \
int size = snprintf (msg,MAX_LOG_CHAR, APP_NAME":%s:%s:%d: ",__FILE__, __func__, __LINE__);    \
if(size > 0)		\
snprintf (msg+size,MAX_LOG_CHAR - size,__VA_ARGS__);                           \
login(DEBUG_TYPE,msg);                              \
} while (0)
...
/* inform, whether mode of connection is set explicitly */
  uint8_t mode_is_expl;
