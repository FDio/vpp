...
void
login (log_type type, const char *format, ...)
{
  va_list arg;
  int done;
  ...
  va_start (arg, format);
  done = vsnprintf (data->msg, MAX_LOG_CHAR, format, arg);
  va_end (arg);
...
/* inform, if thread is running or not*/
...
print_help_args ()
{
  ...
  printf ("\t./ping --aff={cpu0,cpu1,..,cpun}\n");
  printf
  ...
  printf ("\t	main thread of program\n");
  printf ("\tin --vdev:\n");
  printf
    ("\t	id for domain - numeric id. interface with same domain id will be bridged\n");
  printf ("\t	id for i - index of connetion\n");
  printf ("\t	address - address of this example\n");
  printf ("\t	socket_name - name of socket for communication\n");
  printf ("\t	qn_count - number of queues\n");
  printf ("\t	ring_size - size of ring\n");
  printf ("\t	buffer_size - size of buffer\n");
  printf ("\t	q0 - setting of mode for qid 0\n");
  printf
...
print_help_cmds ()
{
  ...
  printf ("\t,where\n");
  printf ("\t	filter - DBG | INFO or his combinations\n");
  printf ("\tsh-count - print counters\n");
  printf ("\tcl-count - clear counters\n");
  printf ("\texit - exit app\n");
  printf ("\tping <ip> [-q idx][-i idx]\n");
  printf ("\t,where\n");
  printf ("\t	-q set index of queue, where packet will be transmited\n");
  printf
...
/*in route mode are receive packets transmit into next interfaces of
connection, with exception his himself. in this mode isn't used zero-copy,
because rx and tx buffers for specific connections are on differnt memory.
one interface can't working with buffers of some another.*/
static int
route_handler (memif_connection_t * c, uint16_t rx,
...
/* check type of imcomming packets. packet, with same source address as address
of specific connection, is skipped. packet with different destination of ip as
specific connection, is skipped. if packet is arp response, it get his mac and
ip into arp table. if packet is ping response, it get status on console from
incomming packets.*/
static int
	      ...
	      memif_thread_data_t * thr_data)
{
  ...
  while (rx--)
	...
	skip_packet = 1;
      else if (arp_ident (rx_bufs->data, &rx_bufs->len))
	skip_packet = 1;

/*after skippic of packet must be freeing shared memory and continue on processing
 of new packet*/
      if (skip_packet)
	{
...
/*in master mode must be create new packet for transmitting. in slave mode is used
zero-copy, where data of packet is only on rx buffer and data his contents is
modificated. program assume, that this incomming packet is type of ping request
and this packet must be modificait on type of ping response*/
      if (c->args.is_master)
	{
...
/*function is called in thread. in the function is cycle of never ending loop.
the condition for exit from function is disconnection of interface, or if
occur any problems. in cycle is reading of packets and further are packets
processing in route or ping mode. If specific connection have valid index
into domain table, is used route mode. */
void *
...
/* receive data from shared memory buffers */
	...
	continue;
      ...
      if (c->idx_domain >= 0)
	{
	  if (route_handler (c, rx, thr_data) < 0)
	    {
	      err_thread = 1;
	      break;
	    }
...
/*count of watched events is only for connected interfaces plus events from console.*/
/*count of watched event is upgrade when interfaces are connecting or disconnecting.*/
  while (cp_cnt_conn)
    {
      if (memif_connections[--cp_cnt_conn].is_connected == 1
	  && !c->set_q0_poll)
	cnt_poll_events++;
    }
  ...
  if (md.rx_queues_num < 1)
    {
      INFO ("number of queues are below than 1");
      return -1;
    }
  ...
  if (c->set_q0_poll)
  ...
  else
    {
      ...
      index_q = 1;
      thread_data[index][0].isRunning = 1;
    }
  ...
  for (; index_q < md.rx_queues_num; index_q++)
      ...
      else
	{
	  ...
	  if (thread_data[index][index_q].isRunning)
	    {
	      INFO ("thread id: %d already running!", ti);
	      continue;
	    }

	  thread_data[index][index_q].index = index;
	  thread_data[index][index_q].qid = index_q;
	  thread_data[index][index_q].id = ti;

	  pthread_create (&thread[index][index_q], NULL,
			  memif_rx_poll,
			  (void *) &thread_data[index][index_q]);

	  if (index_q == 0 && c->set_q0_aff >= 0)
	    {
	      cpu_set_t cpuset;
	      CPU_ZERO (&cpuset);
	      CPU_SET (c->set_q0_aff, &cpuset);

	      int s =
		pthread_setaffinity_np (thread[index][0], sizeof (cpu_set_t),
					&cpuset);
	      if (s != 0)
		INFO ("settin of affinity cpu endin with error code %d", s);
	    }
	}
    }
...
/*count of watched event is upgrade when interfaces are connecting or disconnecting.*/
  ...
  void *ptr;
  int index_q;
  for (index_q = 0; index_q < c->current_cnt_q; index_q++)
...
this interrupt is called, when example is in master mode*/
...
/* filename  of socket is possible specified. if it isn't specified, be used default
socket filname. if this app connect to VPP over memif, must be usted default socket
filename */
...
/* searching of created socket with same name. if the socket exist, shall be used
pointer of this socket */
	  while (cp_idx_conn > 0)
	    {
...
/* if socket with specific name isn't created in previous time, must be created first time */
	  if (c->args.socket == 0)
	    {
      ...
      else
	{
	  ...
	  thread_data[idx_conn][0].rx_bufs =
	    (memif_buffer_t *) malloc (sizeof (memif_buffer_t) *
				       MAX_MEMIF_BUFS);
	  thread_data[idx_conn][0].tx_bufs =
	    (memif_buffer_t *) malloc (sizeof (memif_buffer_t) *
				       MAX_MEMIF_BUFS);
	}
...
main (int argc, char *argv[])
{
  ...
  for (pos_arg = 1; pos_arg < argc; pos_arg++)
    {
      if (parse_arg (argv[pos_arg]) < 0)
	printf ("line \"%s\" will be skipped\n", argv[pos_arg]);
    }
