...
int
parse_corelist (cpu_set_t * corelist, char *saveptr1)
{
...
set_arg_conn (memif_connection_t * c, char *saveptr1)
{
  ...
  while (str_parse = strtok_r (NULL, "=", &saveptr1))
      ...
      if (strcmp (cp_param, "lcores") == 0)
	{
	  ...
	  if (parse_corelist (&c->q0_corelist, value) < 0)
	    return -1;
      ...
      if (strcmp (cp_param, "id") == 0)
	{
	  ...
	  if (*pEnd != '\0')
	    {
	      printf ("%s isn't valid number for parameter 'id'\n", value);
	      return -1;
	    }
	}
      ...
      else if (strcmp (cp_param, "q0-rxmode") == 0)
	{
	  if (strcmp (value, "poll") == 0)
	    ...
	    {
	      printf ("\"%s\" is not supported for parameter q0-rxmode\n",
		      value);
	      return -1;
	    }
	}
