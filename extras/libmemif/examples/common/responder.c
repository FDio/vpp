...
responder (memif_conn_handle_t conn, void *private_ctx, uint16_t qid)
{
    ...
    do {
        /* allocate tx buffers */
        err = memif_buffer_alloc(conn, qid, c->tx_bufs, c->rx_buf_num, &c->tx_buf_num, 2048);
...
responder_zero_copy (memif_conn_handle_t conn, void *private_ctx, uint16_t qid)
{
    ...
    do {
        ...
        /* Swap rx and tx buffers, swapped tx buffers are considered allocated and are ready to be transmitted.
         * Notice that the buffers are swapped only in memif driver and locally remain in rx_bufs queue.
         */
        ...
        /* Notice that we send from rx_bufs as the buffers were only swapped internally in memif driver */
