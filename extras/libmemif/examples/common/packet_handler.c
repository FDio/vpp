...
int basic_packet_handler (memif_connection_t *c)
{
    ...
    /* in case of zero-copy the tx_buf_num will be zero, so the loop body won't execute */
...
int icmp_packet_handler (memif_connection_t *c)
{
    ...
    if (c->tx_buf_num > 0) {
        for (i = 0; i < c->tx_buf_num; i++) {
            resolve_packet(c->rx_bufs[i].data, c->rx_bufs[i].len, c->tx_bufs[i].data, &c->tx_bufs[i].len, c->ip_addr, c->hw_addr);
        }
    } else {
        for (i = 0; i < c->rx_buf_num; i++) {
            resolve_packet_zero_copy(c->rx_bufs[i].data, &c->rx_bufs[i].len, c->ip_addr, c->hw_addr);
        }
    }
