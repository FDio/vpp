...
resolve_packet (void *in_pck, ssize_t in_size,
		void *out_pck, uint32_t * out_size, uint8_t ip_addr[4], uint8_t hw_addr[6])
{
...
int
resolve_packet_zero_copy (void *pck, uint32_t * size, uint8_t ip_addr[4], uint8_t hw_addr[6])
{
  struct ether_header *eh;
  struct ether_arp *eah;
  struct iphdr *ip;
...
int
resolve_packet_zero_copy_add_encap (void **pck_, uint32_t * size, uint8_t ip_addr[4])
{
  struct ether_header *eh;
  struct iphdr *ip;
  struct icmphdr *icmp;
