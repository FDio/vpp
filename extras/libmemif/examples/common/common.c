...
{
  memif_ring_details_t *rd;
  memif_desc_details_t *dd;
  /* Use actual ring size here, but it's also fine if this number is larger */
  uint16_t ndesc = 2048;
  int i;

  /* Allocate details struct */
  rd = malloc(sizeof(memif_ring_details_t) + sizeof(memif_desc_details_t) * ndesc);
  if (rd == NULL) {
    INFO("Out of memory");
    return;
  }
  ...
  if (is_rx)
    memif_get_rx_ring_details(c->conn, rd, qid, ndesc);
  else
    memif_get_tx_ring_details(c->conn, rd, qid, ndesc);

  printf("%s Ring %d details:\n", is_rx ? "RX" : "TX", qid);
  printf("\thead: %d\n", rd->head);
  printf("\ttail: %d\n", rd->tail);

  printf("Descriptors:\n");
  printf("\tID\tOFFSET\tLENGTh\n");
  printf("\t========================================\n");
  for (i = 0; i < rd->ndesc; i++) {
    dd = &rd->descs[i];
    printf("\t%d\t%d\t%d\n", i, dd->offset, dd->length);
  }
}
