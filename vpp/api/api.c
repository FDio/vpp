 ...
 gid_address_t *gid_address)
{
    ...
    if (gid_address_type(gid_address) != GID_ADDR_IP_PREFIX) {
        rmp->is_ipv6 = 0;
        rmp->prefix_len = 0;
        memset(rmp->ip_address, 0, sizeof(rmp->ip_address));
        return;
    }
    ...
    switch (ip_prefix_version(ip_addr)) {
      case IP4:
          rmp->is_ipv6 = 0;
          memcpy(rmp->ip_address, &ip_prefix_v4(ip_addr),
                 sizeof(ip_prefix_v4(ip_addr)));
          break;

      case IP6:
          rmp->is_ipv6 = 1;
          memcpy(rmp->ip_address, &ip_prefix_v6(ip_addr),
                 sizeof(ip_prefix_v6(ip_addr)));
          break;
