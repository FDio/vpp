...
_(ERRORS_DUMP, errors_dump)                                             \
_(FETCH_ERRORS, fetch_errors)                                           \
...
static void vl_api_errors_dump_t_handler (
        vl_api_errors_dump_t *mp, vlib_main_t *vm)
{
    vlib_error_main_t * em = &vm->error_main;
    vl_api_error_details_t *rmp;
    int ni, rv = 0;

    for (ni = 0; ni < vec_len (em->error_strings_heap); ni++)
    {
        REPLY_MACRO2(VL_API_ERROR_DETAILS,
                rmp->error_index = ntohl(ni);
                strncpy((char *)rmp->description, 
                        (char *)em->error_strings_heap[ni], 
                        sizeof(rmp->description));
        );
    }
}

static void vl_api_fetch_errors_t_handler (
        vl_api_fetch_errors_t *mp, vlib_main_t *vm)
{
    vlib_error_main_t * em = &vm->error_main;
    vlib_node_t * n;
    unix_shared_memory_queue_t * q;
    vl_api_error_list_t *rmp;
    u32 code, i, ni;
    u64 c;
    int rv = 0, index = 0;
    ...
    foreach_vlib_main(({
      ...
      for (code = 0; code < n->n_errors; code++)
        ...
        REPLY_MACRO2(VL_API_ERROR_LIST,
                rmp->node_index  = ntohl(n->index);
                rmp->error_index = ntohl(i);
                rmp->error_count = ntohl(c);
        );
        }
        }
      index++;
