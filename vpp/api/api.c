...
(vl_api_trace_profile_add_t *mp)
{
     ...
     * is supported */
    ip6_ioam_trace_profile_set(mp->trace_num_elt, mp->trace_type, 
                               mp->node_id, mp->trace_app_data, 
...
(vl_api_trace_profile_apply_t *mp)
{
    ...
    ip6.as_u32[0] = mp->dest_ipv6[0];
    ip6.as_u32[1] = mp->dest_ipv6[1];
    ip6.as_u32[2] = mp->dest_ipv6[2];
    ip6.as_u32[3] = mp->dest_ipv6[3];
    if (mp->enable != 0) {
         ip6_ioam_set_destination (&ip6, mp->prefix_length,
			      mp->vrf_id, 
