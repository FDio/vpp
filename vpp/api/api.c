        ...
        vl_api_vlib_error_strings_dump_t *mp, vlib_main_t *vm)
{
    ...
    for (ni = 0; ni < vec_len (em->error_strings_heap); ni++)
    {
        ...
        rmp = vl_msg_api_alloc_as_if_client(sizeof (*rmp) + length);
        ...
        vl_api_vlib_errors_dump_t *mp, vlib_main_t *vm)
{
    ...
    foreach_vlib_main(({
        ...
        if (vec_len(error_index) > 0) {
            aditional_length = vec_len(error_index) * sizeof(u64);
            rmp = vl_msg_api_alloc_as_if_client(sizeof (*rmp) + aditional_length);
            rmp->_vl_msg_id  = ntohs (VL_API_VLIB_ERRORS_DETAILS);
            rmp->thread_id   = ntohl(index);
            rmp->num_errors  = ntohl(vec_len(error_index));

            j = 0;
            for (i=0; i<vec_len(error_index); i++) {
                rmp->data[j++] = ntohl(error_index[i]);
                rmp->data[j++] = ntohl(error_count[i]);
            }
