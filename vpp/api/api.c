...
_(LISP_GPE_ENABLE_DISABLE_DUMP, lisp_gpe_enable_disable_dump)           \
...
{
    vl_api_lisp_gpe_enable_disable_details_t *rmp = NULL;
    u8 is_en;

    rmp = vl_msg_api_alloc (sizeof (*rmp));
    memset (rmp, 0, sizeof (*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_LISP_GPE_ENABLE_DISABLE_DETAILS);
...
static void
vl_api_lisp_gpe_enable_disable_dump_t_handler
(vl_api_lisp_gpe_enable_disable_dump_t *mp)
{
    ...
    send_lisp_gpe_enable_disable_details(q);
}
