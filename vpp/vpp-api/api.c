  ...
  (vxlan_gpe_tunnel_t * t, unix_shared_memory_queue_t * q, u32 context)
{
  ...
  ip46_address_t *ip46_local = (ip46_address_t *)(&(rmp->local[0]));
  ip46_address_t *ip46_remote = (ip46_address_t *)(&(rmp->remote[0]));
  if (is_ipv6)
    {
      memcpy (&ip46_local->ip6, &(t->local.ip6), 16);
      memcpy (&ip46_remote->ip6, &(t->remote.ip6), 16);
      rmp->encap_vrf_id = htonl (im6->fibs[t->encap_fib_index].table_id);
      rmp->decap_vrf_id = htonl (im6->fibs[t->decap_fib_index].table_id);
    }
  else
    {
      memcpy (&ip46_local->ip4, &(t->local.ip4), 4);
      memcpy (&ip46_remote->ip4, &(t->remote.ip4), 4);
      rmp->encap_vrf_id = htonl (im4->fibs[t->encap_fib_index].table_id);
