...
{
  int i;
  u32 their_api_version;

  ASSERT (ARRAY_LEN (mp->api_versions) >= ARRAY_LEN (our_api_versions));

  for (i = 0; i < ARRAY_LEN (our_api_versions); i++)
    {
      their_api_version = clib_net_to_host_u32 (mp->api_versions[i]);
      if (their_api_version != *our_api_versions[i])
	{
	  clib_warning ("vpe API mismatch layer %d: 0x%08x instead of 0x%08x",
			i, their_api_version, *our_api_versions[i]);
	  return -1;
	}
    }
  return 0;
