...
define run
	...
	@cd $(STARTUP_DIR) && \
	  $(SUDO) LD_LIBRARY_PATH=$LD_LIBRARY_PATH $(2) $(1)/vpp/bin/vpp $(MINIMAL_STARTUP_CONF)
endef
else
define run
	@cd $(STARTUP_DIR) && \
	  $(SUDO) LD_LIBRARY_PATH=$LD_LIBRARY_PATH $(2) $(1)/vpp/bin/vpp $(shell cat $(STARTUP_CONF) | sed -e 's/#.*//')
endef
endif
