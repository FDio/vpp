...
help:
	...
	@echo "                            Supported platforms: native, aarch64"
...
ifeq ($(PLATFORM),aarch64)
	...
	DEB_DEPENDS += libssl-dev:arm64 libmbedtls-dev:arm64 uuid-dev:arm64
	DEB_DEPENDS += libnuma-dev:arm64 libnl-3-dev:arm64
	DEB_DEPENDS += gcc-8-aarch64-linux-gnu g++-8-aarch64-linux-gnu
    ...
    $(error "Deb package installation: platform $(PLATFORM) unsupported")
endif
endif
else ifneq ("$(wildcard /etc/redhat-release)","")
ifeq ($(OS_ID),rhel)
...
test:
	$(call test,$(PLATFORM),vpp,test)

test-debug:
	$(call test,$(PLATFORM),vpp_debug,test)

test-gcov:
	$(call test,$(PLATFORM),vpp_gcov,test)

test-all:
	$(if $(filter-out $(3),retest),make -C $(BR) PLATFORM=$(PLATFORM) TAG=vpp vom-install,)
	$(eval EXTENDED_TESTS=yes)
	$(call test,$(PLATFORM),vpp,test)

test-all-debug:
	$(if $(filter-out $(3),retest),make -C $(BR) PLATFORM=$(PLATFORM) TAG=vpp_debug vom-install,)
	$(eval EXTENDED_TESTS=yes)
	$(call test,$(PLATFORM),vpp_debug,test)
...
test-shell:
	$(call test,$(PLATFORM),vpp,shell)

test-shell-debug:
	$(call test,$(PLATFORM),vpp_debug,shell)

test-shell-gcov:
	$(call test,$(PLATFORM),vpp_gcov,shell)
...
test-cov:
	@make -C $(BR) PLATFORM=$(PLATFORM) TAG=vpp_gcov vom-install
	$(eval EXTENDED_TESTS=yes)
	$(call test,$(PLATFORM),vpp_gcov,cov)
...
retest:
	$(call test,$(PLATFORM),vpp,retest)

retest-debug:
	$(call test,$(PLATFORM),vpp_debug,retest)
...
install-ext-deps:
	make -C build/external PLATFORM=$(PLATFORM) install-$(PKG)
...
pkg-verify: install-dep $(BR)/.deps.ok install-ext-deps
	...
	@make -C build-root PLATFORM=$(PLATFORM) TAG=vpp wipe-all install-packages
	$(call banner,"Building sample-plugin")
	@make -C build-root PLATFORM=$(PLATFORM) TAG=vpp sample-plugin-install
	$(call banner,"Building libmemif")
	@make -C build-root PLATFORM=$(PLATFORM) TAG=vpp libmemif-install
	$(call banner,"Building VOM")
	@make -C build-root PLATFORM=$(PLATFORM) TAG=vpp vom-install
