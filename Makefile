...
DEB_DEPENDS += libconfuse-dev git-review exuberant-ctags cscope pkg-config doxygen
...
ifeq ($(OS_ID),ubuntu)
	...
	if [ -n "$$MISSING" ] ; then \
	  echo "Please execute \033[0;32m\"make install-dep\"\033[0m to install missing packages:"  ; \
	  echo "$$MISSING" ; \
	  exit 1 ; \
	fi ; \
	exit 0
