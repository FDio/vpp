...
ifeq ($(ARCH),native)
	...
	@sudo ln /usr/bin/$(MACHINE)-linux-gnu-objcopy /usr/bin/objcopy
	@fi
	@find /usr/bin -name strip -type l -exec ls -al {} \; | grep $(MACHINE)
	@retVal=$?
	@if [ $retVal -ne 0 ]; then
	@sudo rm /usr/bin/strip
	@sudo ln /usr/bin/$(MACHINE)-linux-gnu-strip /usr/bin/strip
	@fi
...
ifeq ($(ARCH),aarch64)
	...
	@sudo rm /usr/bin/objcopy
	@sudo ln /usr/bin/aarch64-linux-gnu-objcopy /usr/bin/objcopy
	@sudo rm /usr/bin/strip
	@sudo ln /usr/bin/aarch64-linux-gnu-strip /usr/bin/strip
else
