...
help:
	...
	@echo " test-dpdk           - build and run functional tests (dpdk enabled)"
	@echo " test-dpdk-debug     - build and run functional tests (dpdk enabled, debug build)"
	@echo " retest-dpdk         - run functional tests (dpdk enabled)"
	@echo " retest-dpdk-debug   - run functional tests (dpdk enabled, debug build)"
...
define test
	...
	make -C test \
	  ...
	  WS_ROOT=$(WS_ROOT) V=$(V) TEST=$(TEST) VPP_PYTHON_PREFIX=$(VPP_PYTHON_PREFIX) $(3) $(4)
endef

ifneq ($(JOBS),)
test-dpdk-check:
	@echo "Setting JOBS variable is incompatible with running VPP with DPDK enabled"
	@false
else
test-dpdk-check:
	@true
