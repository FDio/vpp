...
* ``Makefile``: top-level script; used to trigger the artifacts and Docker
  ...
  etc.
* ``Dockerfile.vpp_builder``: used to create a "builder" container that is used
  to compile VPP (e.g. plugins) from source. This allows on to have an Ubuntu
  20.04 build environment locally (to make various labs/examples more
  deterministic/repeatable).
* ``Dockerfile.vpp_builder.dockerignore``: permit/deny list for
  ``Dockerfile.vpp_builder``.
* ``x.sh``: a shell script that launches a Docker container, using the image
  produced by ``Dockerfile.vpp_builder``, with some additional run-time
  arguments to ``docker run`` for the convenience of readers making use of
  these labs/exercises. Mainly used to have an interactive shell into a
  container ready to build VPP, VPP plugins, etc. This script, plus the
  associated Docker image, can be used to build VPP plugins, and then have said
  plugins "baked in" to the run-time images produced by
