...
Reports of security vulnerabilities affecting the FD.io Continuous
Integration (CI) system must be submitted privately to
<security@lists.fd.io>. The FD.io Security Response Team (SRT) and the
appropriate CI maintainers will evaluate and investigate each report, and
mitigate it if necessary. All evaluation, investigation, and mitigation work
must be performed under embargo.

A private GitHub Security Advisory (GHSA) will be created for each report to
capture the discussions, decisions, and resolution of the reported security
vulnerability. Because the CI system is not a released product and runs only
within FD.io-hosted infrastructure, its GHSA shall remain private and will
never be published. The CI vulnerability report therefore does not enter the
public advisory or coordinated-disclosure process.
