...
class Siphon(object):
    ...
    def singleline_codeblock(self, matchobj):
    ...
    def process_special(self, s):
        ...
        s = re.sub(r"@TODO[^\n]*", "", s)
        # ----------- code blocks
        s = re.sub(r"@cliexcmd{(.+?)}", self.singleline_codeblock, s, flags=re.DOTALL)
        s = re.sub(r"@cliexstart{(.+?)}(.+?)@cliexend", self.codeblock, s, flags=re.DOTALL)
        # ----------- lists
