...
The ``vpp-check-libvnet-exports.sh`` command checks an out-of-tree plugin
...
The ``READELF`` environment variable can also select the ELF reader, and
``AWK`` can select an alternative POSIX-compatible awk implementation.
The command requires POSIX ``awk`` and either GNU ``readelf`` or
``llvm-readelf``.
