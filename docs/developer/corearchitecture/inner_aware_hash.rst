...
surface; the IP-layer surface is **opt-in** and the LAG surface is
**always-on with safe fallback**:
...
LAG always-on (with fallback)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The existing ``hash-eth-l34`` registered Ethernet hash function was
updated in place to always attempt an inner peek.  When the inner
header cannot be resolved (non-tunnel traffic, fragmented outer,
unsupported GRE protocol type, etc.) the function transparently falls
back to the outer-only L34 hash, so existing non-tunnel deployments
see no behavior change.  No new hash function is registered and no
bond API change is required; bonds created with the standard
``BOND_API_LB_ALGO_L34`` algorithm pick up inner-aware hashing
automatically for IPinIP / GRE / NVGRE transit traffic.
...
number of branches.  When the opt-in flag is not set, the only cost is
one extra branch on ``flow_hash_config_t`` - well below measurement
noise.
...
Implementation:
  ...
  * ``src/vnet/hash/hash_eth.c``             - LAG caller (always-on)
