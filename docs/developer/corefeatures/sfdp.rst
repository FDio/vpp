...
sfdp-interface-input example
____________________________

The `sfdp-interface-input` node (from `src/plugins/sfdp_services/base/interface_input/`)
illustrates a simple injection pattern using the IP4 feature arc. The node
maps each ingress interface to a tenant, sets the buffer metadata
(`flow_id` to the tenant context id and `tenant_index`), and then forwards
packets to `sfdp-lookup-ip4`.

You can enable it per interface with the CLI:

::

  set sfdp interface-input <interface> tenant <tenant-id>

To disable the feature on the interface:
...
Beyond the built-in IP normalization, SFDP parsers are specialized lookup
nodes used for non-IP or custom key formats. A parser defines how to extract a
lookup key from a packet
...
follow the same threading rules as SFDP services. The use case for blacklisting a
callback is typically to substitute a certain behavior depending on session creation
events with a custom one.
