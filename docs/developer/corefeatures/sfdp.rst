...
After session lookup (and, potentially, session creation), SFDP stores private metadata in the packet buffer:
  ...
  * a tenant index, which identifies the tenant of the session to which the packet belongs. The notion of tenant is clarified in the following
...
Lookup parsing and error handling
_________________________________

The lookup path performs protocol parsing and special-case handling that
influences what services see after `sfdp-lookup`. IP4/IP6 headers are parsed
and normalized, including the pseudo-direction logic described earlier. ICMP
error packets are parsed to recover the original 5-tuple so they can be mapped
back to the correct session. Fragmented traffic is redirected to reassembly
paths, and table overflow or unknown-protocol conditions are handled by
dedicated error-drop nodes.

These behaviors live in the `src/vnet/sfdp/lookup/` sub-tree and define which
fields in `sfdp_buffer_opaque_t` are valid when services run. The nodes used
for slow-path handling and ICMP error processing are tenant-configurable:
...
with `set sfdp services ...` (described above) or the `sfdp_set_services` API.
