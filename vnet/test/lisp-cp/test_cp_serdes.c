...
test_lisp_parse_lcaf ()
{
  ...
  _assert (lcaf_vni (lcaf) == 0x09);

  gid_address_t * nested_gid = lcaf_gid_address (lcaf);
  ...
  _assert (gid_address_lcaf_type (&locs[1].address) == LCAF_INSTANCE_ID);
  lcaf = &gid_address_lcaf (&locs[1].address);
  _assert (lcaf_vni (lcaf) == 0x22446688);

  nested_gid = lcaf_gid_address (lcaf);
  _assert (GID_ADDR_IP_PREFIX == gid_address_type (nested_gid));
