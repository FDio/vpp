...
static clib_error_t * test_gid_parse_lcaf ()
{
  ...
  _assert (lcaf->vni == 9);
  _assert (lcaf->len == 0xa);
  _assert (lcaf->vni_mask_len == 0x18);
done:
...
static clib_error_t * test_gid_parse_lcaf_complex ()
{
  ...
  lcaf_t * lcaf = &gid_address_lcaf (gid_addr_copy);
  _assert (lcaf->type == LCAF_INSTANCE_ID);
  _assert (lcaf->vni == 0x0b);
  _assert (lcaf->len == 0xa);
  _assert (lcaf->vni_mask_len == 0x18);

  gid_address_t * tmp = lcaf_gid_address (lcaf);
  _assert (gid_address_type (tmp) == GID_ADDR_LCAF);
  lcaf = &gid_address_lcaf (tmp);

  _assert (lcaf->type == LCAF_INSTANCE_ID);
  _assert (lcaf->vni == 0x0c);
  _assert (lcaf->len == 0xa);
  _assert (lcaf->vni_mask_len == 0x17);

  tmp = lcaf_gid_address (lcaf);
  _assert (gid_address_type (tmp) == GID_ADDR_LCAF);
  lcaf = &gid_address_lcaf (tmp);

  _assert (lcaf->type == LCAF_INSTANCE_ID);
  _assert (lcaf->vni == 0x0d);
  _assert (lcaf->len == 0x16);
  _assert (lcaf->vni_mask_len == 0x16);
