		 ...
		 vnet_main_t * vnet_main, u32 sw_if_index, u32 vtr_op, u8 add)
{
  ...
  if (vtr_op == L2_VTR_PUSH_2)
    {
      clib_memcpy (out_config->macs_tags.b_dst_address, si->sub.pbb.b_dmac,
		   sizeof (out_config->macs_tags.b_dst_address));
      clib_memcpy (out_config->macs_tags.b_src_address, si->sub.pbb.b_smac,
		   sizeof (out_config->macs_tags.b_src_address));
      out_config->macs_tags.b_type =
	clib_net_to_host_u16 (si->sub.pbb.b_type);
      out_config->macs_tags.priority_dei_id =
	clib_net_to_host_u16 (si->sub.pbb.b_tag.raw_flags);
      out_config->macs_tags.i_type =
	clib_net_to_host_u16 (si->sub.pbb.i_type);
      out_config->macs_tags.priority_dei_uca_res_sid =
