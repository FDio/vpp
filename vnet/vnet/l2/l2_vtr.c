                    ...
                    u8            add)
{
  ...
  if (vtr_op == L2_VTR_PUSH_2)
    {
      clib_memcpy (out_config->macs_tags.b_dst_address, si->sub.pbb.b_dmac, sizeof(out_config->macs_tags.b_dst_address));
      clib_memcpy (out_config->macs_tags.b_src_address, si->sub.pbb.b_smac, sizeof(out_config->macs_tags.b_src_address));
      out_config->macs_tags.b_type = clib_net_to_host_u16(si->sub.pbb.b_type);
      out_config->macs_tags.priority_dei_id = clib_net_to_host_u16(si->sub.pbb.b_tag.raw_flags);
      out_config->macs_tags.i_type = clib_net_to_host_u16(si->sub.pbb.i_type);
      out_config->macs_tags.priority_dei_uca_res_sid = clib_net_to_host_u32(si->sub.pbb.i_tag.raw_flags);
      out_config->push_size = sizeof(ethernet_pbb_header_packed_t);
      out_config->pop_size = 0;

      in_config->raw_data = out_config->raw_data;
      in_config->pop_size = out_config->push_size;
      in_config->push_size = out_config->pop_size;
    }
  else if (vtr_op == L2_VTR_POP_2)
    {
      clib_memcpy (in_config->macs_tags.b_dst_address, si->sub.pbb.b_dmac, sizeof(in_config->macs_tags.b_dst_address));
      clib_memcpy (in_config->macs_tags.b_src_address, si->sub.pbb.b_smac, sizeof(in_config->macs_tags.b_src_address));
      in_config->macs_tags.b_type = clib_net_to_host_u16(si->sub.pbb.b_type);
      in_config->macs_tags.priority_dei_id = clib_net_to_host_u16(si->sub.pbb.b_tag.raw_flags);
      in_config->macs_tags.i_type = clib_net_to_host_u16(si->sub.pbb.i_type);
      in_config->macs_tags.priority_dei_uca_res_sid = clib_net_to_host_u32(si->sub.pbb.i_tag.raw_flags);
      in_config->push_size = 0;
