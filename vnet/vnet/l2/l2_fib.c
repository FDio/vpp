...
static uword
l2fib_mac_age_process (vlib_main_t * vm, vlib_node_runtime_t * rt,
		       vlib_frame_t * f)
{
  ...
  while (1)
	...
	vlib_process_wait_for_event (vm);
      ...
      clib_warning ("event %x", event_type);
      switch (event_type)
	{
	case ~0:
	  break;
      ...
      for (i = 0; i < h->nbuckets; i++)
	{
	  ...
	  for (j = 0; j < (1 << b->log2_pages); j++)
	      ...
	      for (k = 0; k < BIHASH_KVP_PER_PAGE; k++)
		{
		  ...
		  clib_memcpy (&mac, key.fields.mac, 6);
		  if (delta > bd_config->mac_age)
		    l2fib_del_entry (mac, key.fields.bd_index);
		}
	      v++;
	    }
	}
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (l2_mac_age_process_node) = {
    .function = l2fib_mac_age_process,
    .type = VLIB_NODE_TYPE_PROCESS,
    .name = "l2-mac-age-process",
};
/* *INDENT-ON* */
