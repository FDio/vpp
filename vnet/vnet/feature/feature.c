...
#define _(n, l) \
extern char * l##_feature_start_nodes[]; \
int l##_feature_num_start_nodes;
...
#define _(n, l)								\
	case VNET_FEAT_##n:						\
	  feature_start_nodes = l##_feature_start_nodes;		\
	  feature_start_len = l##_feature_num_start_nodes;		\
	  break;
	  foreach_vnet_feature_type
#undef _
	default:
	...
	hash_create_string (0, sizeof (uword));
      ...
      while (reg)
	{
	  hash_set_mem (fm->next_feature_by_name[feature_type], reg->node_name,
...
u32
vnet_feature_index_from_node_name (vnet_feature_type_t type, const char *s)
{
  vnet_feature_main_t *fm = &feature_main;
  vnet_feature_registration_t *reg;
  uword *p;

  p = hash_get_mem (fm->next_feature_by_name[type], s);
  if (p == 0)
    return ~0;
					  ...
					  u32 sw_if_index, int enable_disable)
{
  ...
  feature_index = vnet_feature_index_from_node_name (VNET_FEAT_DEVICE_INPUT, node_name);
...
#define _(f, l)                                           \
  do {                                                    \
    ...
    vlib_cli_output (vm, "%U:", format_c_identifier, #l); \
				...
				ip_config_main_t * cm, u32 sw_if_index)
{
  ...
  for (feature_type = 0;
       feature_type < VNET_N_FEAT_TYPES; feature_type++)
    {
      ...
      vlib_cli_output (vm, "\n%U:", format_c_identifier, vnet_driver_feat_names[feature_type]);
