				  ...
				  u32 sw_if_index, int is_add)
{
  ...
  fm->feature_count_by_sw_if_index[arc] += is_add ? 1 : -1;

  ASSERT (fm->feature_count_by_sw_if_index[arc] >= 0);
...
u32
vnet_feature_index_from_node_name (vnet_feature_arc_t type, const char *s)
{
  vnet_feature_main_t *fm = &feature_main;
  vnet_feature_registration_t *reg;
  uword *p;
					  ...
					  u32 sw_if_index, int enable_disable)
{
  ...
  feature_index =
    vnet_feature_index_from_node_name (VNET_FEAT_DEVICE_INPUT, node_name);
  ...
  vnet_config_update_feature_count (fm, sw_if_index,
				    VNET_FEAT_DEVICE_INPUT, enable_disable);
  ...
  for (feature_arc = 0; feature_arc < VNET_N_FEAT_ARCS; feature_arc++)
    {
      ...
      vlib_cli_output (vm, "\n%U:", format_c_identifier,
		       vnet_feature_arc_names[feature_arc]);
