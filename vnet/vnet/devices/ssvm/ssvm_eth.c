...
int ssvm_eth_create (ssvm_eth_main_t * em, u8 * name, int is_master)
{
  ...
  sh->opaque [CHUNK_POOL_NFREE] = (void *)(intptr_t) em->nbuffers;
                       ...
                       vlib_frame_t * f)
{
  ...
  if ((intptr_t)(sh->opaque [MASTER_ADMIN_STATE_INDEX]) == 0 ||
      (intptr_t)(sh->opaque [SLAVE_ADMIN_STATE_INDEX]) == 0)
    {
      interface_down = 1;
      goto out;
    }
  ...
  if (n_present_in_cache < n_left*2)
      ...
      if (PREDICT_TRUE(n_allocated > 0))
	{
      ...
      sh->opaque [CHUNK_POOL_NFREE] = (void *) (intptr_t) n_available;
