...
dpdk_interface_admin_up_down (vnet_main_t * vnm, u32 hw_if_index, u32 flags)
{
  ...
  if (is_up)
  ...
  else
    {
      ...
      /*
       * Do a barrier sync to make sure all threads at least made
       * it to the top of the main loop.
       */
      vlib_worker_thread_barrier_sync(dm->vlib_main);
      vlib_worker_thread_barrier_release(dm->vlib_main);
