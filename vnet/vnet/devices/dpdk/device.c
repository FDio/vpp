		     ...
		     vlib_buffer_t * b, u32 bi, struct rte_mbuf **mbp)
{
  ...
  struct rte_mbuf *mb = *mbp;

  if (PREDICT_FALSE (b->flags & VLIB_BUFFER_RECYCLE))
    {
      struct rte_mbuf *mb_new = dpdk_replicate_packet_mb (b);
  ...
  if (1 || PREDICT_FALSE (b->flags & VLIB_BUFFER_REPL_FAIL))
    {
      i16 delta;
      u16 new_data_len, new_pkt_len;
      delta = PREDICT_FALSE (b->flags & VLIB_BUFFER_REPL_FAIL) ? 0 :
	vlib_buffer_length_in_chain (vm, b) - (i16) mb->pkt_len;
      new_data_len = (u16) ((i16) mb->data_len + delta);
      new_pkt_len = (u16) ((i16) mb->pkt_len + delta);
      b->current_length = new_data_len;
      mb->data_len = new_data_len;
      mb->pkt_len = new_pkt_len;
      mb->data_off = (PREDICT_FALSE (b->flags & VLIB_BUFFER_REPL_FAIL)) ?
