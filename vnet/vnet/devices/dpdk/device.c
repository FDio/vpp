...
dpdk_tx_burst_internal(uint8_t port_id, uint16_t queue_id,
		struct rte_mbuf **pkts, uint32_t tx_head, uint32_t tx_tail)
{
	uint32_t ret, rv = 0;
      ...
      else if (PREDICT_TRUE (xd->flags & DPDK_DEVICE_FLAG_PMD))
	{
	  if (PREDICT_TRUE (tx_head > tx_tail))
	  ...
	  else
	    {
		  ...
		  if (PREDICT_TRUE(tx_head > tx_tail)) {
			  /* no wrap, transmit in one burst */
			  rv = dpdk_tx_burst_internal(xd->port_id,
					  (uint16_t)queue_id, &tx_vector[tx_tail],
					  tx_head, tx_tail);
		  } else {
			  /*
			   * If we transmitted everything we wanted, then allow 1 retry
  ...
  if (xd->flags & DPDK_DEVICE_FLAG_VHOST_USER) {
	  if (is_up) {
		  ...
		  if (link.link_status == ETH_LINK_UP) {
			  vnet_hw_interface_set_flags(vnm,
					  xd->vlib_hw_if_index,
