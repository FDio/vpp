...
dpdk_lib_init (dpdk_main_t * dm)
{
  ...
  for (i = 0; i < nports; i++)
	...
	devconf = pool_elt_at_index (dm->conf->dev_confs, p[0]);
      ...
      if (devconf && devconf->num_tx_queues > 0 && devconf->num_tx_queues < xd->tx_q_used)
        xd->tx_q_used = clib_min(xd->tx_q_used, devconf->num_tx_queues);

      if (devconf && devconf->num_rx_queues > 1 && dev_info.max_rx_queues >= devconf->num_rx_queues)
        {
...
#undef _
  ...
  pool_foreach (devconf, conf->dev_confs, ({
	...
	conf->default_devconf.num_rx_queues > 0)
      devconf->num_rx_queues = conf->default_devconf.num_rx_queues;

    if (devconf->num_tx_queues == 0 &&
	conf->default_devconf.num_tx_queues > 0)
      devconf->num_tx_queues = conf->default_devconf.num_tx_queues;
