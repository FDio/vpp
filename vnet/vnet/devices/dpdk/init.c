...
dpdk_device_bind_to_uio (void * arg, u8 * dev_dir_name, u8 * file_name)
{
  ...
  /* Read device class */
  s = format (s, "%v/%s%c", dev_dir_name, "class", 0);
  ...
  s = format (s, "%v/%s%c", dev_dir_name, "vendor", 0);
  read_sys_fs ((char *) s, "0x%x", &vid);
  vec_reset_length (s);
  s = format (s, "%v/%s%c", dev_dir_name, "device", 0);
  ...
  while((e = readdir (d)))
    {
      ...
      /* dummy socket, just to give something to ioctl */
  ...
  s = format (s, "/sys/bus/pci/devices/%s/driver/unbind%c", pci_addr, 0);
  write_sys_fs ((char *) s, "%s", pci_addr);
  vec_reset_length (s);

  s = format (s, "/sys/bus/pci/drivers/%v/new_id%c", dm->uio_driver_name, 0);
