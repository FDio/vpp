...
dpdk_port_setup_hqos (dpdk_device_t * xd, dpdk_device_config_hqos_t * hqos)
{
  ...
  vec_validate_aligned (xd->hqos_worker, tm->n_vlib_mains - 1,
  ...
  for (subport_id = 0; subport_id < hqos->port.n_subports_per_port;
       subport_id++)
    {
      ...
      rv =
	rte_sched_subport_config (xd->hqos_iotx->hqos, subport_id,
  ...
  for (i = 0; i < worker_thread_count; i++)
    {
      ...
      xd->hqos_worker[tid].hqos_field0_slabshr =
	__builtin_ctzll (hqos->pktfield0_slabmask);
      ...
      xd->hqos_worker[tid].hqos_field1_slabshr =
	__builtin_ctzll (hqos->pktfield1_slabmask);
      ...
      xd->hqos_worker[tid].hqos_field2_slabshr =
	__builtin_ctzll (hqos->pktfield2_slabmask);
      memcpy (xd->hqos_worker[tid].hqos_tc_table, hqos->tc_table,
...
dpdk_iotx_thread_internal_hqos_dbg_bypass (vlib_main_t * vm)
{
  ...
  while (1)
	...
	vec_elt_at_index (dm->devices_by_iotx_cpu[cpu_index], dev_pos);
...
void
dpdk_iotx_thread (vlib_worker_thread_t * w)
{
  vlib_main_t *vm;
  vlib_thread_main_t *tm = vlib_get_thread_main ();
  dpdk_main_t *dm = &dpdk_main;
...
{
.name = "hqos-threads",.short_name = "hqos-threads",.function =
    dpdk_iotx_thread_fn,};
