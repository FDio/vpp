	...
	if (hw_if_index == dm->devices[dq->device].vlib_hw_if_index)
	  {
	    ...
	    vec_del1 (dm->devices_by_iotx_cpu[i],
		      dq - dm->devices_by_iotx_cpu[i]);
		       ...
		       vlib_cli_command_t * cmd)
{
  ...
  rv =
    rte_sched_pipe_config (xd->hqos_iotx->hqos, subport_id, pipe_id,
			   profile_id);
  if (rv)
			...
			vlib_cli_command_t * cmd)
{
  ...
  for (i = 0; i < worker_thread_count; i++)
    xd->hqos_worker[worker_thread_first + i].hqos_tc_table[entry] = val;
			   ...
			   vlib_cli_command_t * cmd)
{
  ...
  for (i = 0; i < worker_thread_count; i++)
    switch (id)
      {
      case 0:
	xd->hqos_worker[worker_thread_first + i].hqos_field0_slabpos = offset;
	xd->hqos_worker[worker_thread_first + i].hqos_field0_slabmask = mask;
	break;
      case 1:
	xd->hqos_worker[worker_thread_first + i].hqos_field1_slabpos = offset;
	xd->hqos_worker[worker_thread_first + i].hqos_field1_slabmask = mask;
	break;
      case 2:
      default:
	xd->hqos_worker[worker_thread_first + i].hqos_field2_slabpos = offset;
	xd->hqos_worker[worker_thread_first + i].hqos_field2_slabmask = mask;
      }
		   ...
		   vlib_cli_command_t * cmd)
{
  ...
  dpdk_device_hqos_per_iotx_thread_t *iotx;
  ...
  cfg = &devconf->hqos;
  iotx = xd->hqos_iotx;
  wk = &xd->hqos_worker[tr->first_index];
