          ...
          vlib_cli_command_t *cmd)
{
  ...
  if (hqos->iotx_valid)
    vlib_cli_output(vm, " I/O TX thread ID = %u", hqos->iotx);

  vlib_cli_output(vm, " Input SWQ:");
  vlib_cli_output(vm, "   Size = %u packets", hqos->swq_size);

  vlib_cli_output(vm, " Operation:");
  ...
  vlib_cli_output(vm, "   Packet queue size: TC0 = %u packets, TC1 = %u packets, TC2 = %u packets, TC3 = %u packets",
  ...
  for (subport_id = 0; subport_id < vec_len(hqos->subport); subport_id++) {
    vlib_cli_output(vm, " Subport %u:", subport_id);
    vlib_cli_output(vm, "   TB rate = %u bytes/second", hqos->subport[subport_id].tb_rate);
    vlib_cli_output(vm, "   TB size = %u bytes", hqos->subport[subport_id].tb_size);
    vlib_cli_output(vm, "   TC0 rate = %u bytes/second", hqos->subport[subport_id].tc_rate[0]);
    vlib_cli_output(vm, "   TC1 rate = %u bytes/second", hqos->subport[subport_id].tc_rate[1]);
    vlib_cli_output(vm, "   TC2 rate = %u bytes/second", hqos->subport[subport_id].tc_rate[2]);
    vlib_cli_output(vm, "   TC3 rate = %u bytes/second", hqos->subport[subport_id].tc_rate[3]);
    vlib_cli_output(vm, "   TC period = %u milliseconds", hqos->subport[subport_id].tc_period);
  }

  for( profile_id = 0; profile_id < vec_len(hqos->pipe); profile_id++) {
    vlib_cli_output(vm, " Pipe profile %u:", profile_id);
    vlib_cli_output(vm, "   TB rate = %u bytes/second", hqos->pipe[profile_id].tb_rate);
    vlib_cli_output(vm, "   TB size = %u bytes", hqos->pipe[profile_id].tb_size);
    vlib_cli_output(vm, "   TC0 rate = %u bytes/second", hqos->pipe[profile_id].tc_rate[0]);
    vlib_cli_output(vm, "   TC1 rate = %u bytes/second", hqos->pipe[profile_id].tc_rate[1]);
    vlib_cli_output(vm, "   TC2 rate = %u bytes/second", hqos->pipe[profile_id].tc_rate[2]);
    vlib_cli_output(vm, "   TC3 rate = %u bytes/second", hqos->pipe[profile_id].tc_rate[3]);
  ...
  vlib_cli_output(vm, " Packet fields:");
