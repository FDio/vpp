  ...
  if (PREDICT_FALSE(mb_flags & (
      ...
      if (xd->per_interface_next_index != ~0)
	{
	  ...
	  if (PREDICT_TRUE (dpdk_mbuf_is_ip4(mb)))
	    vnet_buffer(b0)->io_handoff.next_index = HANDOFF_DISPATCH_NEXT_IP4_INPUT;
	  else if (PREDICT_TRUE(dpdk_mbuf_is_ip6(mb)))
	    vnet_buffer(b0)->io_handoff.next_index = HANDOFF_DISPATCH_NEXT_IP6_INPUT;
          else if (PREDICT_TRUE(vlib_buffer_is_mpls(b0)))
	    vnet_buffer(b0)->io_handoff.next_index = HANDOFF_DISPATCH_NEXT_MPLS_INPUT;
	  else
