		     ...
		     u16 qid, vlib_node_runtime_t * node)
{
  ...
  if (PREDICT_FALSE (vum->cpus[cpu_index].rx_buffers_len < n_left + 1))
	...
	vlib_buffer_alloc_from_free_list (vm,
      ...
      if (PREDICT_FALSE (vum->cpus[cpu_index].rx_buffers_len < VHOST_USER_RX_BUFFER_STARVATION))
	{
  ...
  while (n_left > 0)
      ...
      while (n_left > 0 && n_left_to_next > 0)
	{
	  ...
	  while (1)
		...
		{
		  if (PREDICT_FALSE(vum->cpus[cpu_index].rx_buffers_len == 0))
		    {
		      /*
		       ...
		       * but valid.
		       */
		      vhost_user_input_rewind_buffers(vm, &vum->cpus[cpu_index],
						      b_head);
		      n_left = 0;
		      goto stop;
		    }
		  ...
		  /* Get next output */
		  vum->cpus[cpu_index].rx_buffers_len--;
		  u32 bi_next = (vum->cpus[cpu_index].rx_buffers)[vum->cpus[cpu_index].rx_buffers_len];
