 ...
 * will exist with the name '<em>VirtualEthernet0/0/x</em>'. If no vHost interfaces
 * have been created, '<em>x</em>' will be the next free index (0,1,2,3, etc.).
 * If vHost interfaces have been created and then deleted, the most recently deleted
 * index will be used. To preserve indices on delete and recreate, delete in reverse
 * order from create.
 *
...
/*?
 * Delete a vHost User interface using the software interface index. Use the
 * '<em>show interfaces</em>' command to determine the software interface index.
 * On deletion, the linux socket will not be deleted.
 *
 * @cliexpar
 * Example of how to delete a vhost interface:
 * @cliexcmd{delete vhost-user sw_if_index 2}
?*/
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (vhost_user_delete_command, static) = {
    .path = "delete vhost-user",
    .short_help = "delete vhost-user sw_if_index <sw_idx>",
    .function = vhost_user_delete_command_fn,
};

/*?
 * Display the attributes of a single vHost User interface or all Vhost
 * User interfaces (omit an interface name to display all vHost interfaces).
 *
 * @cliexpar
 * @parblock
...
VLIB_CLI_COMMAND (show_vhost_user_command, static) = {
    ...
    .short_help = "show vhost-user [<interface>] [descriptors]",
