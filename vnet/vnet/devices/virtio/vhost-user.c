  ...
  if (PREDICT_FALSE(_vec_len(vum->rx_buffers[cpu_index]) < n_left))
    {
	  ...
	  vlib_buffer_alloc_from_free_list(vm, vum->rx_buffers[cpu_index] + curr_len,
  ...
  while (n_left > 0)
      ...
      while (n_left > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_TRUE(n_left > 1))
	    {
	      u32 next_desc = txvq->avail->ring[(txvq->last_avail_idx+1) & qsz_mask];
	      void *buffer_addr = map_guest_mem (vui, txvq->desc[next_desc].addr);
	  ...
	  while(1)
	    {
	      void * buffer_addr = map_guest_mem(vui, desc_table[desc_index].addr);
	      if (PREDICT_FALSE(buffer_addr == 0))
		{
	      ...
	      if (PREDICT_TRUE(desc_table[desc_index].flags & VIRTQ_DESC_F_NEXT))
		{
		  CLIB_PREFETCH(&desc_table[desc_table[desc_index].next], sizeof(vring_desc_t), STORE);
		}
		     ...
		     vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left > 0)
      ...
      if (PREDICT_FALSE (rxvq->last_avail_idx == rxvq->avail->idx))
	{
      ...
      desc_head = desc_index = rxvq->avail->ring[rxvq->last_avail_idx & qsz_mask];
      if (rxvq->desc[desc_head].flags & VIRTQ_DESC_F_INDIRECT)
	{
	  if (PREDICT_FALSE(rxvq->desc[desc_head].len < sizeof(vring_desc_t)))
	    {
	      error = VHOST_USER_TX_FUNC_ERROR_INDIRECT_OVERFLOW;
	      goto done;
	    }
	  if (PREDICT_FALSE(!(desc_table = map_guest_mem(vui, rxvq->desc[desc_index].addr))))
	    {
      ...
      if (PREDICT_FALSE(!(buffer_addr = map_guest_mem(vui, desc_table[desc_index].addr))))
	{
	  error = VHOST_USER_TX_FUNC_ERROR_MMAP_FAIL;
	  goto done;
	}
      ...
      CLIB_PREFETCH(buffer_addr, clib_min(buffer_len, 2*CLIB_CACHE_LINE_BYTES), STORE);
      ...
      vhost_user_log_dirty_pages(vui, desc_table[desc_index].addr, vui->virtio_net_hdr_sz);
      ...
      while (1)
	{
	  ...
	  bytes_to_copy = (bytes_to_copy > buffer_len) ? buffer_len : bytes_to_copy;
	  ...
	  vhost_user_log_dirty_pages(vui,
				     desc_table[desc_index].addr + desc_table[desc_index].len -
				     bytes_left - bytes_to_copy, bytes_to_copy);
      ...
      if (PREDICT_TRUE(n_left >= 2))
	{
      ...
      rxvq->used->ring[rxvq->last_used_idx & qsz_mask].id = desc_head;
      rxvq->used->ring[rxvq->last_used_idx & qsz_mask].len = desc_len;
      vhost_user_log_dirty_ring (vui, rxvq, ring[rxvq->last_used_idx & qsz_mask]);
