		     ...
		     u16 qid, vlib_node_runtime_t * node)
{
  ...
  if (PREDICT_FALSE (vum->cpus[cpu_index].rx_buffers_len <
  ...
  while (n_left > 0)
      ...
      while (n_left > 0 && n_left_to_next > 0)
	{
	  ...
	  while (1)
		...
		{
		  vum->cpus[cpu_index].rx_buffers_len--;
		  u32 bi_next = (vum->cpus[cpu_index].rx_buffers)[vum->cpus[cpu_index].rx_buffers_len];	//Assuming we have some
