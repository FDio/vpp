...
#include <vnet/devices/dpdk/dpdk.h>	// Need this to manipulate rte_mbuf
...
vhost_user_init (vlib_main_t * vm)
{
  ...
  vec_foreach (cpu, vum->cpus)
  {
     ...
     */
    vec_alloc (cpu->rx_buffers, 2 * VLIB_FRAME_SIZE);
    if (!cpu->rx_buffers)
      return clib_error_return (0, "Could not create rx buffer vectors");

    //FIXME: Make sure we don't overflow this
    vec_alloc (cpu->copy, 4 * VLIB_FRAME_SIZE);
    vec_alloc (cpu->tx_headers, VLIB_FRAME_SIZE);
    if (!cpu->copy || !cpu->tx_headers)
      return clib_error_return (0, "Could not create tx copy vectors");
  }
...
vhost_user_rx_discard_packet (vlib_main_t * vm,
			      vhost_user_intf_t * vui,
			      vhost_user_vring_t * txvq,
			      u32 discard_max, u32 * discarded_desc)
{
  u32 discarded_packets = 0;
  u32 avail_idx = txvq->avail->idx;
  u16 qsz_mask = txvq->qsz - 1;
  *discarded_desc = txvq->last_avail_idx;
  while (discarded_packets != discard_max)
    {
      if (avail_idx == txvq->last_avail_idx)
	goto out;

      u16 desc = txvq->avail->ring[txvq->last_avail_idx & qsz_mask];
      txvq->used->ring[txvq->last_used_idx & qsz_mask].id = desc;
      txvq->used->ring[txvq->last_used_idx & qsz_mask].len = 0;
      txvq->last_avail_idx += 1;
      txvq->last_used_idx += 1;
      u16 num_desc = 1;
      while (txvq->desc[desc].flags & VIRTQ_DESC_F_NEXT)
	{
	  desc = txvq->desc[desc].next;
	  if (avail_idx == txvq->last_avail_idx)
	    {
	      //Not enough descriptors for the packet
	      txvq->last_avail_idx -= num_desc;
	      txvq->last_used_idx -= num_desc;
	      goto out;
	    }
	  txvq->used->ring[txvq->last_used_idx & qsz_mask].id = desc;
	  txvq->used->ring[txvq->last_used_idx & qsz_mask].len = 0;
...
out:
  CLIB_MEMORY_BARRIER ();
  *discarded_desc = txvq->last_avail_idx - *discarded_desc;
...
#define VHOST_USER_INPUT_MAX_BUFFERS_PER_PACKET 8
		     ...
		     u16 qid, vlib_node_runtime_t * node)
{
  ...
  u16 qsz_mask;
  u32 map_hint = 0;
  vhost_cpu_t *cpu = &vum->cpus[os_get_cpu_number ()];
  ...
  if (PREDICT_FALSE (!vui->admin_up || !(txvq->enabled)))
    {
       ...
       */
      u32 disc;
      vhost_user_rx_discard_packet (vm, vui, txvq, 256, &disc);
      return 0;
    }
  ...
  if (PREDICT_FALSE (_vec_len (cpu->rx_buffers) <
		     n_left + VHOST_USER_INPUT_MAX_BUFFERS_PER_PACKET + 1))
    {
      u32 curr_len = _vec_len (cpu->rx_buffers);
      _vec_len (cpu->rx_buffers) +=
	vlib_buffer_alloc_from_free_list (vm,
					  cpu->rx_buffers + curr_len,
					  2 * VLIB_FRAME_SIZE - curr_len,
					  VLIB_BUFFER_DEFAULT_FREE_LIST_INDEX);

      if (PREDICT_FALSE (n_left + VHOST_USER_INPUT_MAX_BUFFERS_PER_PACKET
			 > _vec_len (cpu->rx_buffers)))
	{
	  // In case of buffer starvation, discard some packets from the queue
	  // and log the event.
	  // We keep doing best effort for the remaining packets.
	  u32 flush = n_left + VHOST_USER_INPUT_MAX_BUFFERS_PER_PACKET -
	    _vec_len (cpu->rx_buffers);
	  u32 discarded_desc = 0;
	  flush = vhost_user_rx_discard_packet (vm, vui, txvq,
						flush, &discarded_desc);

	  n_left -= discarded_desc;
  ...
  while (n_left > 0)
      ...
      while (n_left > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (_vec_len (cpu->rx_buffers) <
			     VHOST_USER_INPUT_MAX_BUFFERS_PER_PACKET))
	    {
	  ...
	  bi_current = (cpu->rx_buffers)[--_vec_len (cpu->rx_buffers)];
...
#if DPDK > 0
	  rte_mbuf_from_vlib_buffer (b_head)->data_len = 0;
	  rte_mbuf_from_vlib_buffer (b_head)->pkt_len = 0;
#endif
	  ...
	  while (1)
	      ...
	      if (desc_data_offset == desc_table[desc_current].len)
		{
	      ...
	      if (b_current->current_length == VLIB_BUFFER_DATA_SIZE)
		{
		  u32 bi_next = (cpu->rx_buffers)[--_vec_len (cpu->rx_buffers)];	//Assuming we have some
		  b_current->next_buffer = bi_next;
		  b_current->flags |= VLIB_BUFFER_NEXT_PRESENT;
		  bi_current = bi_next;
		  b_current = vlib_get_buffer (vm, bi_current);
#if DPDK > 0
		  rte_mbuf_from_vlib_buffer (b_current)->data_len = 0;
		  rte_mbuf_from_vlib_buffer (b_current)->pkt_len = 0;
#endif
		}

	      // Prepare a copy order executed later for the header
	      vhost_copy_t *cpy = &cpu->copy[_vec_len (cpu->copy)++];
	      u32 desc_data_l =
...
#if DPDK > 0
	      rte_mbuf_from_vlib_buffer (b_current)->data_len += cpy->len;
#endif
...
#if DPDK > 0
	  rte_mbuf_from_vlib_buffer (b_head)->pkt_len =
  ...
  if (PREDICT_FALSE (vhost_user_input_copy (vui, cpu->copy, &map_hint)))
    {
		    ...
		    u32 * map_hint)
{
  ...
  if (PREDICT_TRUE (copy_len >= 4))
      ...
      while (PREDICT_TRUE (copy_len >= 4))
	{
	  ...
	  //FIXME: log takes virtual address
	  vhost_user_log_dirty_pages (vui, cpy[0].dst, cpy[0].len);
	  vhost_user_log_dirty_pages (vui, cpy[1].dst, cpy[1].len);
	  copy_len -= 2;
	  cpy += 2;
	}
    }
  while (copy_len)
	...
	return 1;
      ...
      vhost_user_log_dirty_pages (vui, cpy->dst, cpy->len);
      copy_len -= 1;
      cpy += 1;
    }
  return 0;
	       ...
	       vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left > 0)
	...
	virtio_net_hdr_mrg_rxbuf_t *hdr =
	  &cpu->tx_headers[_vec_len (cpu->tx_headers)];
	_vec_len (cpu->tx_headers)++;
	hdr->hdr.flags = 0;
	hdr->hdr.gso_type = 0;
      ...
      while (1)
	{
	  if (buffer_len == 0)
		...
		{
		  virtio_net_hdr_mrg_rxbuf_t *hdr =
		    &cpu->tx_headers[_vec_len (cpu->tx_headers) - 1];
  ...
  //Do the memory copies
  if (PREDICT_FALSE (vhost_user_tx_copy (vui, cpu->copy, &map_hint)))
    {
      clib_warning ("Memory mapping error on interface hw_if_index=%d "
		    "(Shutting down - Switch interface down and up to restart)",
		    vui->hw_if_index);
