...
#define NO_VISUALIZATION_APP /* Visualization app is a java app */
                  ...
                  if (PREDICT_TRUE (trace0->data_list_elts_left))
                    {
                      ...
                      if (trace0->ioam_trace_type & BIT_ING_INTERFACE) 
                        {
                          if (hm->is_visapp_enabled)
                            {
                              /* For visualization tool to work, ingress_if and 
                               * egress_if should be within (2,3,4,5,6,7). 
                               * Simply choose 3 and 6.
                               */
                             ...
                             { 
                              *elt0 = 
                          (vnet_buffer(b0)->sw_if_index[VLIB_RX]&0xFFFF) << 16 |
                           (adj0->rewrite_header.sw_if_index & 0xFFFF);
                              }
                          elt0++;
                        }
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    ...
    if (hm->is_visapp_enabled)
      if (node_id < 1 || node_id > 4)
        return clib_error_return (0, "node_id can only be 1, 2, 3 or 4\n"); 
...
VLIB_CLI_COMMAND (ip6_set_ioam_destination_cmd, static) = {
  ...
  static clib_error_t *
ip6_set_ioam_visapp_fn (vlib_main_t * vm,
                      unformat_input_t * input,
                      vlib_cli_command_t * cmd)
{
  ip6_hop_by_hop_main_t *hm = &ip6_hop_by_hop_main;
...
VLIB_CLI_COMMAND (ip6_set_ioam_visapp_cmd, static) = {
  .path = "set ioam visapp",
  .short_help = "set ioam visapp <true|false> To modify values required to conform with visualization application",
  .function = ip6_set_ioam_visapp_fn,
};
