...
void
copy_ipfix_info(ip6_header_t * ip, u32 opaque_index1)
{
  ...
  ifix = get_ipfix_flow(opaque_index1);

  /* Copy required fields into ipfix location for ths elts */
  copy_ip6_address(&ifix->src_addr, &ip->src_address);
  copy_ip6_address(&ifix->dst_addr, &ip->dst_address);
  ifix->protocol = hbh->protocol;
  ifix->src_port = 0;
  ifix->dst_port = 0;
  //FIXME look for l4 header not assume l4 header to follow hbyh
  if(ifix->protocol == IP_PROTOCOL_TCP ||
     ifix->protocol == IP_PROTOCOL_UDP)
  {
    u16 hlen = (hbh->length+1)<<3;
    u = (udp_header_t *) (((u8*)hbh)+hlen);
    ifix->src_port = clib_net_to_host_u16(u->src_port);
    ifix->dst_port = clib_net_to_host_u16(u->dst_port);
  }
  ifix->pkt_counter++;
  ifix->bytes_counter += clib_net_to_host_u16 (ip->payload_length)
                                                - ((hbh->length+1)<<3);

  type = to->hdr.type & HBH_OPTION_TYPE_MASK;
  ...
  if (type == HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST)
  {
    ...
    ifix->num_nodes = size_of_all_traceopts / size_of_traceopt_per_node;

    for(i = 0; i < ifix->num_nodes; i++)
    {
      ptr = (u32 *) ((u8*)to->elts + (size_of_traceopt_per_node * i));
      ifix->path[i].node_id = clib_net_to_host_u32 (*ptr) & 0x00ffffff;
      if (to->ioam_trace_type == TRACE_TYPE_IF_TS_APP ||
         to->ioam_trace_type == TRACE_TYPE_IF)
          ifix->path[i].ingress_if = (u16)(((*(ptr+1)) & 0xffff0000) >> 16);
        ifix->path[i].egress_if  = (u16)((*(ptr+1)) & 0x0000ffff);
    }
  }
...
ip6_hbh_ioam_trace_data_list_handler (vlib_buffer_t *b, ip6_header_t *ip, ip6_hop_by_hop_option_t *opt)
{
  ...
  {
    u32 opaque_index1 = vnet_buffer(b)->l2_classify.opaque_index1;
    copy_ipfix_info(ip, opaque_index1);
  }
		  ...
		  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
      ...
      if (hm->ipfix_enabled == 1)
      {
        opaque_index1 = vnet_buffer(b0)->l2_classify.opaque_index1;
        copy_ipfix_info(ip0, opaque_index1);
      }
...
ip6_hop_by_hop_ioam_init (vlib_main_t * vm)
{
  ...
  {
    set_zero_ip6_address(&hm->ipfix_elts[i].dst_addr);
  }
#endif
