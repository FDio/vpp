/*
 * punt.c: punted packets processing
 *
...
clib_error_t *
vnet_punt_add (vlib_main_t * vm, u8 ipv, u8 protocol, u16 port)
{
  /* For now we only support specific UDP port punt */
  {
	      ...
	      unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      unformat (input, "%d", &udp_port);
      /* punt both IPv6 and IPv4 when used in CLI */
      error = vnet_punt_add (vm, ~0, IP_PROTOCOL_UDP, udp_port);
      if (error)
...
VLIB_CLI_COMMAND (punt_udp_command, static) = {
  .path = "punt udp",
  .short_help = "punt udp port-num1 port-num2 ...",
