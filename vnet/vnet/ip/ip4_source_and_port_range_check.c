...
static u8 * format_ip4_source_and_port_range_check_trace (u8 * s, va_list * va)
{
  ...
  if (t->bypass)
    ...
    s = format (s, "fib-index %d src ip %U %s dst port %d: %s",
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
        {
          ...
          for (i = 0; i < IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOLS; i++)
            {
          ...
          if (ip0->protocol == IP_PROTOCOL_UDP)
            fib_index0 = c0->fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_UDP];
          if (ip0->protocol == IP_PROTOCOL_TCP)
            fib_index0 = c0->fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_TCP];
          if (ip0->protocol == IP_PROTOCOL_ICMP)
            fib_index0 = c0->fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_ICMP];

          if (ip1->protocol == IP_PROTOCOL_UDP)
            fib_index1 = c1->fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_UDP];
          if (ip1->protocol == IP_PROTOCOL_TCP)
            fib_index1 = c1->fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_TCP];
          if (ip1->protocol == IP_PROTOCOL_ICMP)
            fib_index1 = c1->fib_index[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_ICMP];
                                       ...
                                       vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "%U", unformat_vnet_sw_interface, vnm,
                    &sw_if_index))
        ;
      else if (unformat (input, "tcp_vrf %d", &vrf_id[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_TCP]))
        vrf_set = 1;
      else if (unformat (input, "udp_vrf %d", &vrf_id[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_UDP]))
        vrf_set = 1;
      else if (unformat (input, "icmp_vrf %d", &vrf_id[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_ICMP]))
        vrf_set = 1;
  ...
  if (!vrf_set)
    return clib_error_return (0, "TCP or UDP or ICMP VRF ID required but not specified");

  for (i = 0; i < IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOLS; i++)
    {

      if (vrf_id[i] == 0)
        return clib_error_return (0, "TCP, UDP or ICMP VRF ID should not be 0 (default). Should be distinct VRF for this purpose. ");
...
VLIB_CLI_COMMAND (set_interface_ip_source_and_port_range_check_command,
  ...
  .short_help = "set int ip source-and-port-range-check <intfc> [tcp-vrf <n>] [udp-vrf <n>] [icmp-vrf <n>] [del]",
};
  ...
  for (i = 0; i < IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOLS; i++)
    {
      ...
      if (!p)
        {
          ip4_fib_t * f;
          f = find_ip4_fib_by_table_index_or_id (im, vrf_id[i], 0 /* flags */);
          fib_index = f->index;
        }
      else
        fib_index = p[0];
      ...
      if (is_add == 0)
        {
          remove_port_range_adjacency(address, length, adj_index, low_ports[i], high_ports[i], fib_index);
        }
      else
        {
          add_port_range_adjacency(address, length, adj_index, low_ports[i], high_ports[i], fib_index);
        }
    }
  return 0;
}
...
ip_source_and_port_range_check_command_fn (vlib_main_t * vm,
                                           ...
                                           vlib_cli_command_t * cmd)
{
  u16 * low_ports[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOLS];
  u16 * high_ports[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOLS];
  u16 this_low;
  u16 this_hi;
  ip4_address_t addr;
  u32 length;
  ...
  u8 prefix_set = 0;
  u32 vrf_id[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOLS];
  int tcp_vrf_set = 0, udp_vrf_set = 0, icmp_vrf_set = 0;
  int tcp_range_set = 0, udp_range_set = 0;
  int is_add = 1;
  int rv;
  int i;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "%U/%d", unformat_ip4_address, &addr, &length))
        prefix_set = 1;
      else if (unformat (input, "tcp-vrf %d", &vrf_id[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_TCP]))
        tcp_vrf_set=1;
      else if (unformat (input, "udp-vrf %d", &vrf_id[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_UDP]))
        udp_vrf_set=1;
      else if (unformat (input, "icmp-vrf %d", &vrf_id[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_ICMP]))
        icmp_vrf_set=1;
      else if (unformat (input, "del"))
        is_add = 0;
      else if (unformat (input, "tcp-port %d", &tmp))
        {
          if (tmp == 0 || tmp > 65535)
            return clib_error_return (0, "tcp-port %d out of range", tmp);
          this_low = tmp;
          this_hi = this_low + 1;
          vec_add1 (low_ports[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_TCP], this_low);
          vec_add1 (high_ports[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_TCP], this_hi);
          tcp_range_set = 1;
        }
      else if (unformat (input, "tcp-range %d - %d", &tmp, &tmp2))
        {
          if (tmp > tmp2)
        ...
        {
          if (tmp == 0 || tmp > 65535)
            return clib_error_return (0, "udp-port %d out of range", tmp);
          this_low = tmp;
          this_hi = this_low + 1;
          vec_add1 (low_ports[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_UDP], this_low);
          vec_add1 (high_ports[IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOL_UDP], this_hi);
          udp_range_set = 1;
        }
      else if (unformat (input, "udp-range %d - %d", &tmp, &tmp2))
        {
          if (tmp > tmp2)
            return clib_error_return (0, "ports %d and %d out of order",
                                      tmp, tmp2);
  ...
  if (tcp_vrf_set == 0 && udp_vrf_set == 0 && icmp_vrf_set == 0)
    return clib_error_return (0, "VRF ID required, not specified");

  if ((tcp_vrf_set && !tcp_range_set)
      || (udp_vrf_set && !udp_range_set))
    return clib_error_return (0, "Both VRF ID and range/port must be set for a protocol.");

  for (i = 0; i < IP_SOURCE_AND_PORT_RANGE_CHECK_PROTOCOLS; i++)
    {
      if (vrf_id[i] == 0)
        return clib_error_return (0, "VRF ID should not be 0 (default). Should be distinct VRF for this purpose. ");
    }
...
VLIB_CLI_COMMAND (ip_source_and_port_range_check_command, static) = {
  ...
  .short_help =
  "set ip source-and-port-range-check <ip-addr>/<mask> [tcp-range <nn>-<nn> tcp-vrf <id>] [udp-range <nn>-<nn> udp-vrf <id>] [icmp-vrf <id>]",
};
