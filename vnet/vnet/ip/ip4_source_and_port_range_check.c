...
_(CHECK_FAIL, "ip4 source and port range check failures")
...
static inline u32 check_adj_port_range_x1 (source_range_check_main_t * srm, 
                                           ip_adjacency_t * adj, 
                                           u16 dst_port,
                                           u32 next)
{
  ...
  if (adj->lookup_next_index != IP_LOOKUP_NEXT_REWRITE || dst_port == 0)
 ...
 vlib_frame_t * frame)
{
  ...
  if (node->flags & VLIB_NODE_FLAG_TRACE)
    vlib_trace_frame_buffers_only 
      (vm, node, from, frame->n_vectors, /* stride */ 1,
       sizeof (ip4_source_and_port_range_check_trace_t));
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  pass0 |= (adj0->lookup_next_index == IP_LOOKUP_NEXT_REWRITE
		    && (source_and_port_range_check_type == IP4_SOURCE_AND_PORT_RANGE_CHECK_REACHABLE_VIA_ANY
	  ...
	  next0 = (pass0 ? next0 : IP4_SOURCE_AND_PORT_RANGE_CHECK_NEXT_DROP);
	  next1 = (pass1 ? next1 : IP4_SOURCE_AND_PORT_RANGE_CHECK_NEXT_DROP);

	  p0->error = error_node->errors[IP4_ERROR_UNICAST_SOURCE_AND_PORT_RANGE_CHECK_FAILS];
	  p1->error = error_node->errors[IP4_ERROR_UNICAST_SOURCE_AND_PORT_RANGE_CHECK_FAILS];
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  ip_adjacency_t * adj0;
	  u32 bi0, next0, adj_index0, pass0;
          udp_header_t * udp0;
	  ...
	  c0 = vnet_get_config_data 
            (&cm->config_main, &vnet_buffer (b0)->ip.current_config_index,
             &next0,
             sizeof (c0[0]));
	  ...
	  /* Pass multicast, IP broadcast, non-TCP/UDP. */
          pass0 = 0;
          ...
          pass0 |= ip0->protocol != IP_PROTOCOL_TCP;
                   
          if (PREDICT_TRUE(pass0 == 0))
            {
              udp0 = ip4_next_header (ip0);
              next0 = check_adj_port_range_x1 
                (srm, adj0, clib_net_to_host_u16(udp0->dst_port), next0);
		     ...
		     vlib_cli_command_t * cmd)
{
  ...
  type = IP4_RX_FEATURE_SOURCE_AND_PORT_RANGE_CHECK;
...
VLIB_CLI_COMMAND (set_interface_ip_source_and_port_range_check_command, 
  ...
  .function = set_ip_source_and_port_range_check,
  .short_help = "set int ip source-and-port-range-check <intfc> [del]",
};
  ...
  if (is_add == 0)
  ...
  else
    {
...

      {
        ...
        template_adj.lookup_next_index = IP_LOOKUP_NEXT_REWRITE;
  ...
  switch(rv)
    ...
    default:
      return clib_error_return 
        (0, "ip4_source_and_port_range_check_add returned %d", rv);
    }
...
VLIB_CLI_COMMAND (ip_source_and_port_range_check_command, static) = {
  ...
  .short_help = 
  "set ip source-and-port-range-check <ip-addr>/<mask> range <nn>-<nn>",
};
                                     ...
                                     vlib_cli_command_t * cmd)
{
  ...
  if (port)
    {
      rv = check_adj_port_range_x1 (srm, adj, (u16) port, 1234);
      if (rv == 1234)
...
VLIB_CLI_COMMAND (show_source_and_port_range_check, static) = {
  ...
  .short_help = 
  "show ip source-and-port-range-check vrf <nn> <ip-addr>",
};
