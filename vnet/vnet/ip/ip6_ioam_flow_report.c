...
#define _(field,mask,item,length)                            \
    ...
    {                                                        \
      ...
      { /* Expect only 2,4 or 8 bytes */                     \
        u64 tmp;                                             \
        tmp = clib_host_to_net_u64(field);                   \
        memcpy (b0->data + next_offset, &tmp, length);       \
      }                                                      \
      next_offset += length;                                 \
    }
                            ...
                            vlib_cli_command_t * cmd)
{
  ...
  for (i = 0; i < vec_len(hm->ioam_flows); i++)
  {
    ...
    s = format(s, "          Pkt_counter : %d  (zero on read)\n", 
                        ipfix->pkt_counter);
    s = format(s, "        Bytes_counter : %d  (zero on read)\n", 
                        ipfix->bytes_counter);
    s = format(s, "               Sfc_id : 0x%x (%d)\n", 
    ...
    s = format(s, "      Start_timestamp : 0x%x (%d)\n", 
                        ipfix->start_timestamp, ipfix->start_timestamp);
    s = format(s, "        End_timestamp : 0x%x (%d)\n", 
                        ipfix->end_timestamp, ipfix->end_timestamp);
...
#if 0 /* Flexible netflow is not enabled */
    s = format(s, "        Fnf Rx packets: %llu\n", rx->fnf_rx_packets);
    s = format(s, "      Fnf Lost packets: %llu\n", rx->fnf_lost_packets);
    s = format(s, " Fnf Reordered packets: %llu\n", rx->fnf_reordered_packets);
    s = format(s, " Fnf Duplicate packets: %llu\n", rx->fnf_dup_packets);
#endif
    s = format(s, "PATH_MAP:\n%U\n", format_path_map,
