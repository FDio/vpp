...
/** \file
    ...
    feature subgraph arc, which needs to run before ip4-lookup.  In
    either base code or a plugin, #include
    <vnet/ip/ip_feature_registration.h> and add the new feature as
    shown:
    ...
    my-ip4-unicast-feature on an interface:
    ...
    tx_cm->config_index_by_sw_if_index = ci. This maintains a
    ...
    processing code, aka in the implementation of my-ip4-unicast-feature:
