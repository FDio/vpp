 ...
 * @param vm vlib_main_t * vlib main structure pointer
 * @param vcm vnet_config_main_t * vnet config main structure pointer
 * @param feature_start_nodes char ** names of start-nodes which use this
 *	  feature graph arc
 * @param num_feature_start_nodes int number of start-nodes
 * @param first_reg vnet_ip_feature_registration_t * first element in
 *        [an __attribute__((constructor)) function built, or
 *        otherwise created] singly-linked list of feature registrations
 * @param (out) in_feature_nodes char *** returned vector of
 *        topologically-sorted feature node names, for use in
 *        show commands
 * @returns clib_error_t 0 on success, otherwise an error message. Errors
...
/*?
 * '<em>show ip features</em>' Display the set of available ip features
 *
 * @cliexpar
 * Example:
...
/*?
 * '<em>show ip interface features</em>' Display the ip features
