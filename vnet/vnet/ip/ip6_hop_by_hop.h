...
#define IOAM_FLOW_TEMPLATE_ID    256
#define MAX_NODES  16
...
#define IOAM_HBYH_POP  2
  u8 ioam_flag;
  /* IPFIX elements. TBD : Make this to an array in future. */
  u32 num_flows;
#define MAX_FLOWS 10
  /* IPFIX flow pool */
  ioam_ipfix_elts_t ipfix_elts[MAX_FLOWS];
...
#define TSP_NANOSECONDS          3
  ...
  ioam_ipfix_elts_t * ioam_sessions;
...
static inline ioam_ipfix_elts_t * get_ipfix_flow(u32 index)
{
  ...
  ifix = pool_elt_at_index (hm->ioam_sessions, index);

  if (is_zero_ip6_address(&ifix->dst_addr))
  { /* Initialize this new flow */
    hm->num_flows++;
    ifix->my_node_id = hm->node_id;
    ifix->sfc_id = 10; /* Arbit number */
  }
...
static inline ioam_ipfix_elts_t * get_ipfix_flow(u32 index)
{
  ...
  if (PREDICT_FALSE((index >= MAX_FLOWS)))
  {
    fformat(stdout, "Number of flows exceeded.  Table index: %d\n", index);
    return (ioam_ipfix_elts_t *) -1;
  }

  ifix = &hm->ipfix_elts[index];
