...
fib_node_index_t
mfib_entry_get_index (const mfib_entry_t * mfib_entry)
{
    return (mfib_entry - mfib_entry_pool);
}
...

{
    ...
    mfib_entry_src_find(mfib_entry, source, &index);
                                ...
                                const fib_route_path_t *rpath)
{
    ...
    {
        msrc->mfes_pl =
            fib_path_list_create(FIB_PATH_LIST_FLAG_NONE,
                                 rpaths);
    }
    else
    {
                        ...
                        mfib_itf_flags_t itf_flags)
{
    ...
    if (NULL == mfib_itf)
    {
         ...
         * A sw if index of ~0 implies a for-us path.
         */
