...
{
  fib_prefix_t prefix = {
    .fp_proto = (is_v4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6),
  };
  u32 fib_index;

  fib_index = fib_table_get_from_table_id (prefix.fp_proto, table_id);
  ...
  if (is_add)
    {
      ...
      dpo_set (&cp_punt,
	       DPO_LISP_CP, 0, (is_v4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6));
...
lisp_gpe_iface_set_table (u32 sw_if_index, u32 table_id, u8 is_ip4)
{
  ...
  fib_index = fib_table_find_or_create_and_lock ((is_ip4 ?
						  FIB_PROTOCOL_IP4 :
						  FIB_PROTOCOL_IP6),
						 table_id);
