...
static_always_inline void
incr_drop_stats (vnet_main_t * vnm, u32 cpu_index, u32 length, u32 sw_if_index,
                 u32 * last_sw_if_index, u32 * n_packets)
{
  vnet_interface_main_t * im;
      ...
      if (PREDICT_TRUE(*last_sw_if_index != ~0))
        {
          ...
          vlib_increment_simple_counter (
              im->sw_if_counters + VNET_INTERFACE_COUNTER_DROP, cpu_index,
                       ...
                       vlib_frame_t * from_frame, u8 is_v4)
{
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
        {
          ...
          if (si0)
            ...
            {
              incr_drop_stats (lgm->vnet_main, cpu_index,
                               vlib_buffer_length_in_chain (vm, b0), si0[0],
                               &last_sw_if_index, &drops);
          ...
          if (si1)
            ...
            {
              incr_drop_stats (lgm->vnet_main, cpu_index,
                               vlib_buffer_length_in_chain (vm, b0), si1[0],
                               &last_sw_if_index, &drops);
  ...
  /* flush iface stats */
  incr_decap_stats (lgm->vnet_main, cpu_index, 0, ~0, &last_sw_if_index, &n_packets,
              &n_bytes);
