...
lisp_gpe_midchain_stack (lisp_gpe_sub_tunnel_t * st)
{
  ...
  adj_nbr_midchain_stack (st->midchain, &tmp);
  ...
  for (i = 0; i < vec_len (t->locator_pairs); i++)
    {
      ...
      st->midchain = adj_nbr_add_or_lock (dst_pfx.fp_proto,
					  FIB_LINK_IP4,
					  &dst_pfx.fp_addr, hi->sw_if_index);
      adj_nbr_midchain_update_rewrite (st->midchain,
...
lisp_gpe_mk_paths_for_sub_tunnels (lisp_gpe_tunnel_t * t)
{
  ...
  vec_foreach_index (ii, t->sub_tunnels)
  {
    ...
    adj = adj_get (st->midchain);

    rpaths[ii].frp_proto =
      (IP4 ==
       ip_addr_version (&t->
			locator_pairs[0].lcl_loc) ? FIB_PROTOCOL_IP4 :
       FIB_PROTOCOL_IP6), rpaths[ii].frp_addr =
      adj->sub_type.midchain.next_hop;
