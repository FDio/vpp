 ...
 *
 *   dpo_nodes[parent_type][parent_proto][node_X] = node_name;
 * i.e.
 ...
 *
 * dpo_edges[parent_type][parent_proto][child_type] = edge_index
 *
...
{
    dpo_set(dpo, DPO_FIRST, FIB_PROTOCOL_NONE, INDEX_INVALID);
}
...
static u32
dpo_get_next_node (dpo_type_t parent_type,
                   dpo_proto_t parent_proto,
                   const dpo_id_t *child_dpo)
{
    dpo_type_t child_type;

    child_type = child_dpo->dpoi_type;
    vec_validate(dpo_edges, parent_type);
    vec_validate(dpo_edges[parent_type], parent_proto);
    vec_validate_init_empty(dpo_edges[parent_type][parent_proto],
                            child_dpo->dpoi_type, ~0);

    /*
     * if the edge index has not yet been created for this node to node transistion
     */
    if (~0 == dpo_edges[parent_type][parent_proto][child_type])
    {
        ...
        ASSERT(NULL != dpo_nodes[child_type][parent_proto]);
        ...
        while (NULL != dpo_nodes[parent_type][parent_proto][pp])
        {
            ...
            while (NULL != dpo_nodes[child_type][parent_proto][cc])
            {
                child_node =
                    vlib_get_node_by_name(vm,
                                          (u8*) dpo_nodes[child_type][parent_proto][cc]);

                /* if (!strncmp(dpo_nodes[child_type][parent_proto][cc], */
                /*              dpo_nodes[parent_type][parent_proto][pp])) */
                /* { */
                /*     /\* */
                /*      * no loops */
                /*      *\/ */
                ...
                if (~0 == dpo_edges[parent_type][parent_proto][child_type])
                {
                    dpo_edges[parent_type][parent_proto][child_type] = edge;
                }
                else
                {
                    ASSERT(dpo_edges[parent_type][parent_proto][child_type] == edge);
                }
                cc++;
            }
            pp++;
    ...
    return (dpo_edges[parent_type][parent_proto][child_type]);
}
 ...
 */
void
dpo_stack (dpo_type_t type,
           dpo_proto_t proto,
           dpo_id_t *parent,
           const dpo_id_t *child)
{
    dpo_stack_i(dpo_get_next_node(type, proto, child), parent, child);
}

/**
 * @brief Stack one DPO object on another, and thus establish a child parent
 * relationship. A new VLIB graph arc is created from the parent node passed
 * to the nodes registered by the child. The VLIB infra will ensure this arc
 * is added only once.
 */
void
dpo_stack_from_node (u32 parent_node,
                     dpo_proto_t proto,
                     dpo_id_t *parent,
                     const dpo_id_t *child)
{
    ...
    child_type = child->dpoi_type;

    vm = vlib_get_main();

    ASSERT(NULL != dpo_nodes[child_type]);
    ASSERT(NULL != dpo_nodes[child_type][proto]);

    child_node =
        vlib_get_node_by_name(vm, (u8*) dpo_nodes[child_type][proto][0]);
