...
replicate_alloc_i (void)
{
    ...
    vlib_validate_combined_counter(&(replicate_main.repm_to_counters),
                                   replicate_get_index(rep));
    vlib_zero_combined_counter(&(replicate_main.repm_to_counters),
                               replicate_get_index(rep));
                  ...
                  u8 *s)
{
    ...
    vlib_get_combined_counter(&(replicate_main.repm_to_counters), repi, &to);
...
format_replicate_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "replicate: index %d", t->rep_index);
  return s;
}
