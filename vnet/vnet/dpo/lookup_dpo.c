                                   ...
                                   dpo_id_t *dpo)
{
    ...
    if (LOOKUP_TABLE_FROM_CONFIG == table_config)
    {
        ...
        case DPO_PROTO_IP6:
            fib_index =
                fib_table_find_or_create_and_lock(table_id,
                                                  dpo_proto_to_fib(proto));
            break;
        case DPO_PROTO_MPLS:
            fib_index = lfib_find_or_create_and_lock(table_id);
            break;
        }
    }
...
format_lookup_dpo (u8 *s, va_list *args)
{
    ...
    else
    {
        ...
        default:
            s = format(s, "%s lookup in %U",
                       lookup_input_names[lkd->lkd_input],
                       format_lfib_table_name, lkd->lkd_fib_index);
            break;
        }
    }
    return (s);
                       ...
                       int table_from_interface)
{
    ...
    while (n_left_from > 0)
    {
        ...
        while (n_left_from > 0 && n_left_to_next > 0)
        {
            ...
            {
                fib_index0 = 
                    lfib_table_get_index_for_sw_if_index(
                        vnet_buffer(b0)->sw_if_index[VLIB_RX]);
            }
            else
            {
            ...
            /* do lookup */
            lbi0 = lfib_table_forwarding_lookup (fib_index0, hdr0);
            lb0  = load_balance_get(lbi0);
            dpo0 = load_balance_get_bucket_i(lb0, 0);
...
format_lookup_mpls_trace (u8 * s, va_list * args)
{
    ...
    /* s = format (s, "%U%U", */
    /*               format_white_space, indent, */
    /*               format_mpls_header, t->packet_data); */
    return s;
}
