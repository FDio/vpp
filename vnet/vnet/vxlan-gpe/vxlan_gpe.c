...
/*   switch (next_index) */
/*     { */
/*     case VXLAN_GPE_INPUT_NEXT_DROP: */
/*       return format (s, "drop"); */
/*     case VXLAN_GPE_INPUT_NEXT_IP4_INPUT: */
/*       return format (s, "ip4"); */
/*     case VXLAN_GPE_INPUT_NEXT_IP6_INPUT: */
/*       return format (s, "ip6"); */
/*     case VXLAN_GPE_INPUT_NEXT_ETHERNET_INPUT: */
/*       return format (s, "ethernet"); */
/*       //    case VXLAN_GPE_INPUT_NEXT_NSH_INPUT: */
/*       //return format (s, "nsh-input"); */
/*     default: */
...
u8 * format_vxlan_gpe_tunnel (u8 * s, va_list * args)
{
  ...
  switch (t->next_protocol)
    {
    case VXLAN_GPE_INPUT_NEXT_DROP:
      s = format (s, "next-protocol drop");
    case VXLAN_GPE_INPUT_NEXT_IP4_INPUT:
    ...
    case VXLAN_GPE_INPUT_NEXT_ETHERNET_INPUT:
      ...
      //return format (s, "nsh-input");
    default:
      s = format (s, "next-protocol unknown %d", t->next_protocol);
    }
...
static int vxlan_gpe_rewrite (vxlan_gpe_tunnel_t * t)
{
  ...
  h0->vxlan.next_protocol = 1; // alagalah TODO VXLAN_NEXT_PROTOCOL_NSH Don't think we should set this? ~0 ?
...
static uword unformat_nsh_map_next (unformat_input_t * input, va_list * args)
{
  ...
  if (unformat (input, "drop"))
    ...
    *result = 1; // alagalah wrong but working this out NSH_INPUT_NEXT_NEXTPROTO_LOOKUP;
  else if (unformat (input, "map"))
    *result = 1; // alagalah wrong but working this out NSH_INPUT_NEXT_REENCAP;
  else
    return 0;
  return 1;
}
