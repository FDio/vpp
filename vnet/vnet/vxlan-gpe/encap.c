  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 4 && n_left_to_next >= 2)
    {
      ...
      if (PREDICT_FALSE(!is_ip4_0))
        next0 = VXLAN_GPE_ENCAP_NEXT_IP6_LOOKUP;
      if (PREDICT_FALSE(!is_ip4_1))
        next1 = VXLAN_GPE_ENCAP_NEXT_IP6_LOOKUP;

      /* IP4 VXLAN header sizeof(ip4_vxlan_gpe_header_t) should be 36 octects */
      /* IP6 VXLAN header sizeof(ip6_vxlan_gpe_header_t) should be 56 octects */
...
#undef _
      ...
      if (PREDICT_FALSE(!is_ip4_0))
      {
        int bogus = 0;
        /* IPv6 UDP checksum is mandatory */
        udp0->checksum = ip6_tcp_udp_icmp_compute_checksum (vm, b0, ip6_0,
                                                            &bogus);
        ASSERT(bogus == 0);
        if (udp0->checksum == 0)
          udp0->checksum = 0xffff;
