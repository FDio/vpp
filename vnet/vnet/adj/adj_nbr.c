			...
			u8 *rewrite)
{
    ...
    if (FIB_LINK_MPLS == adj->ia_link)
    {
        ...
        ai = adj_nbr_find(adj->ia_nh_proto,
...
void
adj_nbr_update_rewrite_header (adj_index_t adj_index,
...
adj_nbr_show_one (BVT(clib_bihash_kv) * kvp,
		  void *arg)
{
    ip_adjacency_t *adj = adj_get(kvp->value);

    vlib_cli_output (arg, "[@%d]  %U: via %U",
                     kvp->value,
                     format_ip_adjacency,
