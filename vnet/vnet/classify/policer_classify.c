                                      ...
                                      int feature_enable)
{
  if (tid == POLICER_CLASSIFY_TABLE_L2)
	...
	cm = vnet_feature_enable_disable ("ip4-unicast", "ip4-policer-classify",
					  sw_if_index, feature_enable, 0, 0);
      else
	cm = vnet_feature_enable_disable ("ip6-unicast", "ip6-policer-classify",
					  sw_if_index, feature_enable, 0, 0);

      pcm->vnet_config_main[tid] = cm;
    }
}
