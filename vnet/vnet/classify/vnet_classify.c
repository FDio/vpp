  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT) 
    {
      if (unformat (input, "del"))
        ...
        ;
                         ...
                         &hit_next_index))
        ;
      else if (unformat (input, "ioam-enc-hit-next %U",unformat_ioam_next_index,
                          &hit_next_index))
       {
        vc_app_add = ioam_flow_add_enc;
        vc_app_del = ioam_flow_del;
      }
      else if (unformat (input, "ioam-dec-hit-next %U",unformat_ioam_next_index,
                         &hit_next_index))
      {
        vc_app_add = ioam_flow_add_dec;
	vc_app_del = ioam_flow_del;
      }
