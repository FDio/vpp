                                   ...
                                   int is_add)
{
  ...
  e->flags = 0;
  e->action = action;
  if (e->action == CLASSIFY_ACTION_REWRITE_IP4_VRF_ID)
    e->metadata = fib_table_find_or_create_and_lock (FIB_PROTOCOL_IP4, metadata);
  else if (e->action == CLASSIFY_ACTION_REWRITE_IP6_VRF_ID)
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT) 
    {
      if (unformat (input, "del"))
        ...
        ;
      else if (unformat (input, "metadata %d", &metadata))
        ;
      else
        {
