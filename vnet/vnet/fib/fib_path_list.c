...
typedef struct fib_path_list_t_ {
    ...
    /**
     * The load-balance this path-list will contribute to its
     * childrens' forwarding.
     */
    dpo_id_t fpl_lb;
} fib_path_list_t;
 ...
 */
static void
fib_path_list_update_lb (fib_path_list_t *path_list)
{
     ...
     */
    load_balance_multipath_update(&path_list->fpl_lb, hash_key,
                                  LOAD_BALANCE_FLAG_NONE);

    FIB_PATH_LIST_DBG(path_list, "update adj: %d",
		      path_list->fpl_lb.dpoi_index);
 ...
 * fib_path_mk_lb
 *
 * Get or make the load-balance this path-list will contribute to its
 * children's forwarding.
 */
static void
fib_path_list_mk_lb (fib_path_list_t *path_list)
{
    dpo_id_t old_dpo = path_list->fpl_lb;
    
    dpo_set(&path_list->fpl_lb,
	    DPO_LOAD_BALANCE,
	    fib_proto_to_dpo(path_list->fpl_nh_proto),
	    load_balance_create(0, fib_proto_to_dpo(path_list->fpl_nh_proto), 0));

    fib_path_list_update_lb(path_list);

    FIB_PATH_LIST_DBG(path_list, "mk adj: %d -> %d",
		      old_dpo.dpoi_index,
		      path_list->fpl_lb.dpoi_index);

    /*
     * let go the old lb. we don't need it anymore
     */
    dpo_reset(&old_dpo);
}
...
{
    int eval, was_looped, is_looped, list_looped;
    fib_node_index_t *path_index;
    fib_path_list_t *path_list;
	...
	was_looped = fib_path_is_looped(*path_index);
	is_looped  = fib_path_recursive_loop_detect(*path_index, copy_ptr);
	list_looped += is_looped;

	if (!!is_looped != !!was_looped || is_looped)
	{
	    /*
	     * the looped status of this path has changed. The path-list's
	     * collapsed multipath adj will need to be re-evaluated.
	     */
	    eval = 1;
	}
    }
