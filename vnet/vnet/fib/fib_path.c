			   ...
			   fib_node_back_walk_ctx_t *ctx)
{
	...
	if (FIB_NODE_BW_REASON_FLAG_EVALUATE & ctx->fnbw_reason)
	{
	     ...
	     */
	    fib_path_recursive_adj_update(path,
			 ...
			 const dpo_id_t *dpo)
{
    ...
    else if (FIB_PATH_CFG_FLAG_EXCLUSIVE & flags)
    {
    ...
    {
	/*
	 * it's gotta be one of the special types...
	 */
	ASSERT(0);
    }
				...
				dpo_id_t *dpo)
{
     ...
     */
    if (FIB_FORW_CHAIN_TYPE_UNICAST_IP == type)
    {
	...
	case FIB_PATH_TYPE_ATTACHED_NEXT_HOP:
	    switch (type)
	    {
	    case FIB_FORW_CHAIN_TYPE_UNICAST_IP:
		dpo_copy(dpo, &path->fp_dpo);
		break;		
		 ...
		 */
		ai = fib_path_attached_next_hop_get_adj(path, FIB_LINK_MPLS);

		dpo_set(dpo, DPO_ADJACENCY, DPO_PROTO_MPLS, ai);
		adj_unlock(ai);
	...
	case FIB_PATH_TYPE_RECURSIVE:
		...
		break;
	    case FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS:
		fib_path_recursive_adj_update(path, type, dpo);
		break;
	    }
	    break;
	case FIB_PATH_TYPE_DEAG:
	    ...
	    {
	    case FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS:
                lookup_dpo_add_or_lock_w_table_id(LFIB_DEFAULT_TABLE_ID,
                                                  DPO_PROTO_MPLS,
                                                  LOOKUP_INPUT_DST_ADDR,
                                                  LOOKUP_TABLE_FROM_CONFIG,
                                                  dpo);
