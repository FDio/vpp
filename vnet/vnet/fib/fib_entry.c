...
format_fib_entry (u8 * s, va_list * args)
{
    ...
    if (!dpo_id_is_valid(&fib_entry->fe_lb[FIB_FORW_CHAIN_TYPE_UNICAST_IP]))
    {
    ...
    else
    {
        ...
        else
        {
            s = format(s, "  %U",
                       format_dpo_id,
                       &fib_entry->fe_lb[FIB_FORW_CHAIN_TYPE_UNICAST_IP],
                       2);
            s = format(s, "\n");
        }
    }
...
fib_entry_contribute_ip_forwarding (fib_node_index_t fib_entry_index)
{
    ...
    return (&fib_entry->fe_lb[FIB_FORW_CHAIN_TYPE_UNICAST_IP]);
}
...
fib_entry_get_adj (fib_node_index_t fib_entry_index)
{
    ...
    return (fib_entry_get_adj_for_source(fib_entry_index,
					 best_source));
}
		    ...
		    const fib_route_path_t *rpath)
{
    ...
    fib_entry = fib_entry_src_action_path_add(fib_entry,
					      source,
					      flags,
					      fib_entry_get_proto(fib_entry),
					      rpath);
		       ...
		       const fib_route_path_t *rpath)
{
    ...
    sflag = fib_entry_src_action_path_remove(fib_entry,
					     source,
					     fib_entry_get_proto(fib_entry),
					     rpath);
...
fib_ip4_address_compare (ip4_address_t * a1,
                         ip4_address_t * a2)
{
    return (clib_net_to_host_u32(a2->data_u32) -
	    clib_net_to_host_u32(a1->data_u32));
}
