...
#define NBR (5+5+6)
    ...
    ai = adj_sr_add_or_lock(FIB_PROTOCOL_IP4, 0, 0);
...
static int
lfib_test_validate_entry (fib_node_index_t fei,
			       u16 n_buckets,
			       ...)
{
    const load_balance_t *lb;
    ...
    dpo = lfib_entry_contribute_forwarding(fei);
...
fib_test_label (void)
{
     ...
     */
    fei = lfib_table_lookup(LFIB_DEFAULT_TABLE_ID,
			    24001,
			    MPLS_EOS);

    FIB_TEST(lfib_test_validate_entry(fei, 
				     2,
				     &l99_eos_o_10_10_10_1,
				     &a_o_10_10_11_1),
    ...
    FIB_TEST(!dpo_cmp(&non_eos_1_1_1_1,
                      lfib_entry_contribute_forwarding(fei)),
	     "PIC-core LB inplace modified %U %U",
             format_dpo_id, &non_eos_1_1_1_1, 0,
             format_dpo_id, lfib_entry_contribute_forwarding(fei), 0);
    ...
    fei = lfib_table_lookup(LFIB_DEFAULT_TABLE_ID,
			    24001,
			    MPLS_EOS);
    FIB_TEST((FIB_NODE_INDEX_INVALID == fei), "24001/eos gone");
    fei = lfib_table_lookup(LFIB_DEFAULT_TABLE_ID,
			    24001,
			    MPLS_NON_EOS);
    FIB_TEST((FIB_NODE_INDEX_INVALID == fei), "24001/neos gone");
...
lfib_test_deagg (void)
{
     ...
     */
    lfe = lfib_table_entry_path_add(lfib_index,
                                    deag_label,
                                    MPLS_EOS,
                                    FIB_PROTOCOL_IP4,
                                    &zero_addr,
    ...
    FIB_TEST((lfe == lfib_table_lookup(lfib_index, deag_label, MPLS_EOS)),
              "%U/%U present",
              format_mpls_unicast_label, deag_label,
              format_mpls_eos_bit, MPLS_EOS);

    dpo = lfib_entry_contribute_forwarding(lfe);
    dpo = load_balance_get_bucket(dpo->dpoi_index, 0);
    lookup_dpo_t *lkd = lookup_dpo_get(dpo->dpoi_index);
    ...
    lfib_table_entry_delete(lfe);

    FIB_TEST((FIB_NODE_INDEX_INVALID == lfib_table_lookup(lfib_index,
                                                          deag_label,
                                                          MPLS_EOS)),
              "%U/%U not present",
              format_mpls_unicast_label, deag_label,
    ...
    FIB_TEST((lfe == lfib_table_lookup(lfib_index, deag_label, MPLS_NON_EOS)),
              "%U/%U present",
              format_mpls_unicast_label, deag_label,
