			      ...
			      fib_source_t source)
{
    ...
    fct = FIB_FORW_CHAIN_TYPE_UNICAST_IP;
    esrc = fib_entry_src_find(fib_entry, source, NULL);
...
fib_entry_src_action_uninstall (fib_entry_t *fib_entry)
{
	...
	dpo_reset(&fib_entry->fe_lb[FIB_FORW_CHAIN_TYPE_UNICAST_IP]);
    }
}
			       ...
			       const fib_route_path_t *rpath)
{
    ...
    fib_entry_src_vft[source].fesv_path_add(esrc,
					    fib_entry,
					    pl_flags,
					    proto,
					    rpath);
				  ...
				  const fib_route_path_t *rpath)
{
    ...
    fib_entry_src_vft[source].fesv_path_remove(esrc,
					       pl_flags,
					       next_hop_proto,
					       rpath);
    /*
			      ...
			      fib_source_t source)
{
	...
	if (FIB_NODE_INDEX_INVALID != esrc->fes_pl)
	{
	    return (fib_path_list_contribute_forwarding(
			esrc->fes_pl,
			FIB_FORW_CHAIN_TYPE_UNICAST_IP));
	}
    }
