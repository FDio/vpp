...
static inline u64 sc_update_cumulative (u64 cumulative, u64 random, u64 secret_share,
...
u8 scv_validate (scv_profile *profile, u64 cumulative, u64 random)
{
 if (profile && profile->validator) {
   return(sc_validate(profile->secret_key, profile->prime, profile->primeinv,
                     cumulative, random));
 }
 return(0);
}
...
clib_error_t *
clear_scv_profile_command_fn (vlib_main_t * vm,
...
static clib_error_t *
set_scv_profile_command_fn (vlib_main_t * vm,
		 unformat_input_t * input,
		 vlib_cli_command_t * cmd)
{
    u64 prime;
    ...
    if (profile) {
      scv_profile_create(profile, prime, poly2, lpc, 
                            secret_share, validity);
      if (validator) scv_set_validator(profile, secret_key);
...
static clib_error_t *
show_scv_profile_command_fn (vlib_main_t * vm,
                                 unformat_input_t * input,
                                 vlib_cli_command_t * cmd)
{
    scv_profile *p = NULL;
...
static clib_error_t *
test_profile_renew_refresh_fn (vlib_main_t * vm,
                    unformat_input_t * input,
                    vlib_cli_command_t * cmd)
{
...
#define TEST_PROFILE_REFRESH 2
  ...
  if (renew_or_refresh == TEST_PROFILE_RENEW)
      rc = scv_profile_renew(
                 path_name, (u8)start_index, (u8)num_profiles);
  else if (renew_or_refresh == TEST_PROFILE_REFRESH)
      rc = scv_profile_refresh(
                 path_name, (u8)start_index, (u8)num_profiles);
  else
    {
      vec_free(path_name);
...
VLIB_CLI_COMMAND (test_ioam_profile_renew_refresh_cmd, static) = {
  ...
  .short_help = "test ioam profile-notification path-name <string> start-index <index> num-profiles <number> <renew|refresh>",
  .function = test_profile_renew_refresh_fn,
};

static clib_error_t *
set_scv_init_fn (vlib_main_t * vm,
                 unformat_input_t * input,
                 vlib_cli_command_t * cmd)
{
    u8 *path_name = 0;
...
VLIB_CLI_COMMAND (set_ioam_sc_init_command, static) = {
    .path = "set scv-init ",
    .short_help = "set scv-init path-name <string> start-index <index> num-profiles <number>",
