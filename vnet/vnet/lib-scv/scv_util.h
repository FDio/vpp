/*
 * scv_util.h -- Service Chain Validation Utility Header
 *
...
static inline
u16 scv_profile_get_id (scv_profile *profile)
{
    if(profile) {
        return(profile->id);
    }
...
static inline
u8 scv_get_profile_in_use(void)
{
...
static inline
u16 scv_get_next_profile_id(vlib_main_t * vm, u16 id)
{
                            ...
                            u16 id, u8 is_encap)
{
    ...
    /* If there are alredy profiles waiting. If so, use existing 
     * start_index. 
     */
    ...
    if (is_encap)
    {
        rc = scv_profile_renew (chain_path_name, 
                             (u8)invalid_profile_start_index, 
                             number_of_invalid_profiles);
        if (rc != 0)
            vlib_cli_output (vm,
               "Renew notification- id start:%d,  num %d failed. rc: %d\n", 
                            invalid_profile_start_index, 
                            number_of_invalid_profiles, 
                            rc);
        else
            vlib_cli_output (vm, 
        ...
        /* Non encap node. Send refresh notification for now. Later
         * set a timer and if there is no profile even after the timeout 
