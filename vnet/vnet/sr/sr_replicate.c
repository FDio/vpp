		  ...
		  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  int i;
	  struct rte_mbuf * mb0, *orig_mb0 = 0; 
	  //	  struct rte_mbuf * mb0prime = 0; 
	  ...
	  /* Advance orig_b0 to the invariant portion */
	  // STUFF TO DO FROM BELOW
	  for (i=0; i < vec_len (pol0->tunnel_indices); i++)
	      ...
	      if (i< vec_len (pol0->tunnel_indices) - 1)
		{
		  ...
		  // Should make vlib_
		  mb0 = dpdk_zerocopy_replicate_packet_mb (orig_b0);
		  ...
		  mb0->ol_flags = orig_mb0->ol_flags;

		  b0 = vlib_buffer_from_rte_mbuf (mb0);
		  vlib_buffer_init_for_free_list (b0, fl);
		  b0->clone_count = rte_mbuf_refcnt_read(mb0);
	      ...
	      else 
		{
		  ...
		  bi0 = orig_bi0;
		  mb0 = rte_mbuf_from_vlib_buffer(b0);
		  b0->clone_count = rte_mbuf_refcnt_read(mb0) -1;
		}
