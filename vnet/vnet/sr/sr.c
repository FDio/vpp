  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if(PREDICT_FALSE(t0->policy_index != ~0))
	    {
	      //next0=SR_REWRITE_NEXT_SR_POLICY;
	      goto trace0;
	    }
                              ...
                              vlib_cli_command_t * cmd)
{
  ...
  if (is_del)
	...
	return clib_error_return (0, "tunnel name to associate to SR policy is required");
      ...
      /* Not sure if this is needed but can add:
	 vec_reset_length (policy->tunnel_indices);
       */
                   ...
                   vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      if (unformat (input, "name %s", &name)) 
	{
	  p=hash_get_mem (sm->policy_index_by_policy_name, name);
	  if(!p)
	    clib_warning("Policy with name  %s not found. Showing all.", name);
	 }
      else 
        break;
    }
  ...
  ({
    ...
    vec_add1 (policies, &sm->policies[p[0]]);
