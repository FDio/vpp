  ...
  while (n_left_from > 0)
      ...
      while (0 && n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_FALSE (ip0->protocol == IPPROTO_IPV6_ROUTE))
	    ...
	    {
	      u32 shift_left_len = sizeof(ip6_header_t);
		  ...
		  shift_left_len += (ext_hdr->n_data_u64s + 1) << 3;
	      ...
	      else
		{
	      ...
	      ip0 = vlib_buffer_get_current (b0);
	      sr0 = (ip6_sr_header_t *) ((u8*)ip0 + shift_left_len);
	      /* $$$ tune */
	  ...
	  if (PREDICT_FALSE (ip1->protocol == IPPROTO_IPV6_ROUTE))
	      ...
	      else
		{
	      ...
	      ip1 = vlib_buffer_get_current (b1);
	      sr1 = (ip6_sr_header_t *) ((u8 *)ip1 + shift_left_len);
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  sr0 = (ip6_sr_header_t *) (ip0 + 1);
	    ...
	    {
	      ip6_hop_by_hop_ext_t *ext_hdr = (ip6_hop_by_hop_ext_t  *) (ip0 + 1);
	      sr0 = (ip6_sr_header_t *) ((u8 *)ext_hdr + ((1 + ext_hdr->n_data_u64s) << 3));
	    }
	...
	do_trace0:
	  ...
	  sr1 = (ip6_sr_header_t *) (ip1 + 1);
          if (PREDICT_FALSE (ip1->protocol == IP_PROTOCOL_IP6_HOP_BY_HOP_OPTIONS))
	    {
	      ...
	      ip6_hop_by_hop_ext_t *ext_hdr = (ip6_hop_by_hop_ext_t  *) (ip1 + 1);
	      sr1 = (ip6_sr_header_t *) ((u8 *)ext_hdr + ((1 + ext_hdr->n_data_u64s) << 3));
	    }
