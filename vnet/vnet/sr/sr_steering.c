  ...
  if (p)
  {
    ...
    if (is_del)
    {
      ...
      else if(steer_pl->classify.traffic_type == SR_STEER_L2)
      {
        ...
        if( vnet_hw_interface_rx_redirect_to_node(sm->vnet_main, 
    ...
    else /* It means user requested to update an existing SR policy */
    {
      /* Retrieve SR policy */
      if(bsid)
      {
...
update_fib:
  ...
  {
    /* Create HW redirection */
    if( vnet_hw_interface_rx_redirect_to_node(sm->vnet_main, 
                                              steer_pl->classify.l2.sw_if_index,
                                              sm->l2_sr_policy_rewrite_index))
      return -3;
  ...
  vlib_cli_command_t * cmd)
{
  ...
  rv = sr_steering_policy (vm, is_del, &bsid, sr_policy_index, fib_table, 
    &prefix, dst_mask_width, sw_if_index, traffic_type);
