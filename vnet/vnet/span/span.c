			   ...
			   u32 dst_sw_if_index, u8 is_add)
{
  ...
  if (src_sw_if_index == ~0)
    return VNET_API_ERROR_SPAN_NO_SUCH_SRC_INTERFACE;
  if (dst_sw_if_index == ~0 && is_add)
    return VNET_API_ERROR_SPAN_NO_SUCH_DST_INTERFACE;
  if (src_sw_if_index == dst_sw_if_index)
    return VNET_API_ERROR_SPAN_SAME_SRC_AND_DST_INTERFACE;
			       ...
			       vlib_cli_command_t * cmd)
{
  ...
  int rv =
    set_span_add_delete_entry (vm, src_sw_if_index, dst_sw_if_index, is_add);
  switch (rv)
    {
    case VNET_API_ERROR_SPAN_NO_SUCH_SRC_INTERFACE:
      return clib_error_return (0, "Source interface must be set");
    case VNET_API_ERROR_SPAN_NO_SUCH_DST_INTERFACE:
      return clib_error_return (0, "Destination interface must be set");
    case VNET_API_ERROR_SPAN_SAME_SRC_AND_DST_INTERFACE:
      return clib_error_return (0,
