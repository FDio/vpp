		  ...
		  vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 4 && n_left_to_c_next >= 2)
	{
	  ...
	  uword *p0 = hash_get (sm->dst_sw_if_index_by_src, dst_if0);
	  uword *p1 = hash_get (sm->dst_sw_if_index_by_src, dst_if1);

	  // first packet
	  if (PREDICT_TRUE (((b0->flags & VLIB_NODE_FLAG_IS_SPAN) == 0) && (p0 != 0)))
	    {
	  ...
	  // second packet
	  if (PREDICT_TRUE (((b1->flags & VLIB_NODE_FLAG_IS_SPAN) == 0) && (p1 != 0)))
	    {
      ...
      while (n_left_from > 0 && n_left_to_next > 0 && n_left_to_c_next > 0)
	{
	  ...
	  if (PREDICT_TRUE (((b0->flags & VLIB_NODE_FLAG_IS_SPAN) == 0) && (p != 0)))
	    {
