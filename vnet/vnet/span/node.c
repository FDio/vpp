	      ...
	      vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  vnet_main_t *vnm = &vnet_main;
  u32 n_left_from, *from, *to_next, *to_mirror_next;
  u32 n_span_packets = 0;
  u32 next_index, mirror_sw_if_index0;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE ((last_mirror_sw_if_index != mirror_sw_if_index0) ||
			     mirror_frame == 0))
	    {
	      if (mirror_frame)
		vnet_put_frame_to_sw_interface (vnm, last_mirror_sw_if_index, mirror_frame);
	      last_mirror_sw_if_index = mirror_sw_if_index0;
	      mirror_frame = vnet_get_frame_to_sw_interface (vnm, mirror_sw_if_index0);
	  ...
	  vnet_feature_redirect (node, sw_if_index0, &next0, b0);
	  ...
	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
		...
		vlib_add_trace (vm, node, b0, sizeof (*t));
	      ...
	      t->mirror_sw_if_index = sm->dst_by_src_sw_if_index[sw_if_index0];
	    }
