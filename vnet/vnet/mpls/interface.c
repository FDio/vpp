  ...
  if (is_enable)
    {
      ...
      lfib_index = lfib_find_or_create_and_lock(LFIB_DEFAULT_TABLE_ID);
  ...
  else
    {
      ...
      lfib_unlock(mm->fib_index_by_sw_if_index[sw_if_index]);
    }
                       ...
                       vlib_cli_command_t * cmd)
{
  ...
  if (is_ip)
  {
      u32 fib_index = fib_table_get_from_table_id(pfx.fp_proto, table_id);
  ...
  else
  {
      ...
      lfe = lfib_table_entry_path_add2(lfib->lf_index,
                                       local_label,
                                       eos,
                                       rpaths);
