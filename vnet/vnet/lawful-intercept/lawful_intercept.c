...
set_li_add_delete_entry (vlib_main_t * vm,
			 ...
			 ip4_address_t collector, u16 udp_port, u8 is_add)
{
		   ...
		   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  return set_li_add_delete_entry (vm, src, collector, udp_port, is_add);
}
		    ...
		    unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output (vm, "Index    Src addr           Collector(ip:port)");
  for (i = 0; i < vec_len (lm->collectors); i++)
    {
