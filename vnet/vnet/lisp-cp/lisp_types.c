...
static u16 gid_simple_addr_size_to_write (gid_simple_address_t * a);
...
u8 *
format_gid_simple_address (u8 * s, va_list * args)
{
  gid_simple_address_t * a = va_arg(*args, gid_simple_address_t *);

  switch (gid_simple_type (a))
    {
    case GID_SIMPLE_ADDR_IP_PREF:
      return format (s, "%U", format_ip_prefix, &gid_simple_ippref (a));

    case GID_SIMPLE_ADDR_MAC:
      return format (s, "%U", format_ip_prefix, &gid_simple_mac (a));

    default:
      clib_warning ("Can't format gid simple type %d!", gid_simple_type (a));
      return 0;
    }
  return 0;
}
  ...
  switch (type)
    ...
    case GID_ADDR_SRC_DST:
      return format (s, "[%d] %U|%U", gid_address_vni(a),
                     format_gid_simple_address, &gid_address_sd_src(a),
                     format_gid_simple_address, &gid_address_sd_dst(a));
    case GID_ADDR_MAC:
...
uword
unformat_gid_simple_address (unformat_input_t * i, va_list * args)
{
  gid_simple_address_t * a = va_arg (*args, gid_simple_address_t *);
  ip_prefix_t ippref;
  u8 mac[6] = {0};
  ...
  if (unformat (i, "%U", unformat_ip_prefix, &ippref))
    {
      gid_simple_type (a) = GID_SIMPLE_ADDR_IP_PREF;
      clib_memcpy (&gid_simple_ippref (a), &ippref, sizeof (ippref));
    }
  else if (unformat (i, "%U", unformat_mac_address, mac))
    {
      gid_simple_type (a) = GID_SIMPLE_ADDR_MAC;
      clib_memcpy (gid_simple_mac (a), mac, sizeof (mac));
    }
  else
    return 0;
...
unformat_gid_address (unformat_input_t * input, va_list * args)
{
  ...
  u8 mac[6] = {0};
  ip_prefix_t ippref;
  gid_simple_address_t sim1, sim2;

  memset (&ippref, 0, sizeof (ippref));
  memset (&sim1, 0, sizeof (sim1));
  memset (&sim2, 0, sizeof (sim2));

  if (unformat (input, "%U|%U", unformat_gid_simple_address, &sim1,
                unformat_gid_simple_address, &sim2))
    {
...
static u8
iana_afi_to_simple_addr_type (u16 type)
{
  switch (type)
    {
    case LISP_AFI_IP:
    case LISP_AFI_IP6:
      return GID_SIMPLE_ADDR_IP_PREF;

    case LISP_AFI_MAC:
      return GID_SIMPLE_ADDR_MAC;
    }
  return ~0;
}

static u16
gid_simple_addr_parse (u8 * p, gid_simple_address_t * a)
{
  u16 afi = clib_net_to_host_u16 ( *(u16 *) p);
  gid_simple_type (a) = iana_afi_to_simple_addr_type (afi);
  ip_address_t * ip_addr = &ip_prefix_addr (&gid_simple_ippref (a));

  switch (gid_simple_type (a))
    {
    case GID_SIMPLE_ADDR_MAC:
      return mac_parse (p, gid_simple_mac (a));
...
{
  lcaf_src_dst_t * sd_hdr;
  gid_address_t * g = a;
  u16 size = 0;
  gid_simple_address_t * src = &gid_address_sd_src (g);
  gid_simple_address_t * dst = &gid_address_sd_dst (g);

  gid_address_type (g) = GID_ADDR_SRC_DST;

  sd_hdr = (lcaf_src_dst_t *)(p + size);
  size += sizeof (sd_hdr[0]);

  size += gid_simple_addr_parse (p + size, src);
  size += gid_simple_addr_parse (p + size, dst);

  if (gid_simple_type (src) == GID_SIMPLE_ADDR_IP_PREF)
    {
      ip_prefix_t * ippref = &gid_simple_ippref (src);
      ip_prefix_len(ippref) = LCAF_SD_SRC_ML(sd_hdr);
    }
  if (gid_simple_type (dst) == GID_SIMPLE_ADDR_IP_PREF)
    {
      ip_prefix_t * ippref = &gid_simple_ippref (dst);
...
static u16
gid_simple_addr_write (u8 * p, gid_simple_address_t * a)
{
  switch (gid_simple_type (a))
    {
    case GID_SIMPLE_ADDR_IP_PREF:
      return ip_prefix_write (p, &gid_simple_ippref (a));

    case GID_SIMPLE_ADDR_MAC:
      return mac_write (p, &gid_simple_mac (a));
...
static u8
gid_simple_address_length (gid_simple_address_t * a)
{
  switch (gid_simple_type (a))
    {
    case GID_SIMPLE_ADDR_IP_PREF:
      return ip_prefix_length (&gid_simple_ippref (a));
    case GID_SIMPLE_ADDR_MAC:
      return 0;
    }
  return 0;
...
sd_write (u8 * p, void * a)
{
  ...
  *(u16 *) p = clib_host_to_net_u16 (LISP_AFI_LCAF);
  ...
  u16 lcaf_len = 4 + sizeof (lcaf_src_dst_t)
    + gid_simple_addr_size_to_write (&sd_src (sd))
    + gid_simple_addr_size_to_write (&sd_dst (sd));
  LCAF_LENGTH (h) = clib_host_to_net_u16 (lcaf_len);
  ...
  LCAF_SD_SRC_ML(&sd_hdr) = gid_simple_address_length (&sd_src (sd));
  LCAF_SD_DST_ML(&sd_hdr) = gid_simple_address_length (&sd_dst (sd));
  clib_memcpy (p + size, &sd_hdr, sizeof (sd_hdr));
  size += sizeof (sd_hdr);

  u16 len = gid_simple_addr_write (p + size, &sd_src (sd));
  if ((u16)~0 == len)
    return ~0;
  size += len;

  len = gid_simple_addr_write (p + size, &sd_dst (sd));
  if ((u16)~0 == len)
    return ~0;
  size += len;
...
static u16
gid_simple_addr_size_to_write (gid_simple_address_t * a)
{
  switch (gid_simple_type (a))
    {
    case GID_SIMPLE_ADDR_IP_PREF:
...
static int
gid_simple_cmp (gid_simple_address_t * a1,
                gid_simple_address_t * a2)
{
  if (gid_simple_type (a1) != gid_simple_type (a2))
    return -1;

  switch (gid_simple_type (a1))
    {
      case GID_SIMPLE_ADDR_IP_PREF:
        return ip_prefix_cmp (&gid_simple_ippref (a1),
                              &gid_simple_ippref (a2));

      case GID_SIMPLE_ADDR_MAC:
        return memcmp (&gid_simple_mac (a1),
                       &gid_simple_mac (a2), 6);
...
sd_cmp (void * a1, void * a2)
{
  ...
  if (gid_simple_cmp (&sd_dst(sd1), &sd_dst(sd2)))
    return -1;
  if (gid_simple_cmp (&sd_src(sd1), &sd_src(sd2)))
    return -1;
  return 0;
}
