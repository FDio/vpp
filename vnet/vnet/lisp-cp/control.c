...
static int
lisp_add_del_negative_static_mapping (gid_address_t * deid, u32 table_id,
 ...
 * @param table_id table ID, used only when adding a negative mapping
...
vnet_lisp_add_del_remote_mapping (gid_address_t * deid, gid_address_t * seid,
                                  ip_address_t * rlocs, u8 action,
                                  u32 table_id, u8 is_add)
{
  vnet_lisp_add_del_mapping_args_t _dm_args, * dm_args = &_dm_args;
  ...
  if ((u32)~0 == mi)
    {
      ...
      if (0 == vec_len (rlocs))
        return lisp_add_del_negative_static_mapping (deid, table_id, ls,
                                                     action, is_add);
...
VLIB_CLI_COMMAND (lisp_add_del_remote_mapping_command) = {
    ...
    .short_help = "lisp remote-mapping add|del vni <vni> table-id <id> "
