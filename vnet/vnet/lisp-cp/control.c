...
static inline
int vnet_lisp_find_duplicate_locator(locator_set_t *ls, locator_t *itloc)
{
  lisp_cp_main_t * lcm = vnet_lisp_cp_get_main();
  locator_t *loc;
  u32 *locit;
  ...
  vec_foreach(locit, ls->locator_indices)
    {
      ...
      if (loc->sw_if_index == itloc->sw_if_index ||
...
static inline
void vnet_lisp_remove_locator_from_locator_set(locator_set_t *ls,
                                               u32 *locit,
                                               u32 ls_index,
                                               u32 loc_id)
{
  ...
  if (a->is_add)
    {
        ...
        vec_foreach (itloc, a->locators)
          {
            ret = vnet_lisp_find_duplicate_locator(ls, itloc);
            if (0 != ret)
              {
                return ret;
              }
        ...
        vec_foreach (locit, ls->locator_indices)
          {
            ...
            if (loc->local && loc->sw_if_index == itloc->sw_if_index)
              {
                vnet_lisp_remove_locator_from_locator_set(ls, locit,
                                                          ls_index, loc_id);
              }
            if (0 == loc->local &&
                !gid_address_cmp(&loc->address, &itloc->address))
