                                ...
                                u64 *nonce_res, u32 * bi_res)
{
  ...
  u32 bi;
  gid_address_t * rlocs = 0;
  gid_address_t gid_seid, gid_deid;
  ...
  /* in case of non-default vni we need to convert seid/deid to LCAF
     so the serializer can build correct message */
  if (gid_address_vni (deid) != 0)
    {
      lcaf_t lcaf = lcaf_iid_init (gid_address_vni (deid));
      gid_deid = gid_build_lcaf (&lcaf, deid);
      gid_seid = gid_build_lcaf (&lcaf, seid);
    }
  else
    {
      gid_deid = deid[0];
  ...
  /* put lisp msg */
  lisp_msg_put_mreq (lcm, b, &gid_seid, &gid_deid, rlocs, is_smr_invoked,
                     nonce_res);

  /* push ecm: udp-ip-lisp */
  lisp_msg_push_ecm (vm, b, LISP_CONTROL_PORT, LISP_CONTROL_PORT, &gid_seid,
                     &gid_deid);
  ...
  if (rlocs)
    vec_free(rlocs);
  gid_address_free (&gid_seid);
  gid_address_free (&gid_deid);
  return b;
}
  ...
  for (i = 0; i < MREP_REC_COUNT(mrep_hdr); i++)
    {
      ...
      /* convert LCAF strucure to simple gid */
      if (gid_address_type (&m_args->deid))
        {
          lcaf_t * lcaf = &gid_address_lcaf (&m_args->deid);
          u32 vni = lcaf_vni (lcaf);
          gid_address_t tmp_gid = lcaf_gid(lcaf)[0];
          gid_address_vni (&tmp_gid) = vni;
          gid_address_free (&m_args->deid);
          m_args->deid = tmp_gid;
        }
