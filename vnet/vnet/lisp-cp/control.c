...
#define PENDING_MR_EXPIRATION_TIME 3.0
                                ...
                                u8 is_smr_invoked, u8 is_resend)
{
  ...
  if (0 == get_mr_and_local_iface_ip (lcm, &mr_ip, &sloc))
    /* map resolver list exhausted */
    {
      if (duplicate_mr)
        duplicate_mr->to_be_removed = 1;
  ...
  b = build_encapsulated_map_request (lcm, seid, deid, loc_set, &mr_ip,
...
remove_dead_pending_map_requests (lisp_cp_main_t * lcm)
{
  pending_map_request_t * pmr;
  uword * pmr_index;
  u64 * to_be_removed = 0, * nonce;

  pool_foreach (pmr, lcm->pending_map_requests_pool,
    ({
      if (pmr->to_be_removed)
        vec_add1 (to_be_removed, pmr->nonce);
    }));

  vec_foreach (nonce, to_be_removed)
    {
