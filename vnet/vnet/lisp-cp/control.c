...
void
get_local_ip_for_dst (lisp_cp_main_t * lcm, ip_address_t * dst,
                      ip_address_t * loc)
{
  u32 adj_index;
  ip_adjacency_t * adj;
  ...
  if (l4)
    {
      ip_addr_v4(loc).as_u32 = l4->as_u32;
      ip_addr_version(loc) = IP4;
    }
  else if (l6)
    {
      clib_memcpy (&ip_addr_v6(loc), l6, sizeof(*l6));
      ip_addr_version(loc) = IP6;
    }
}
                           ...
                           ip_address_t * sloc)
{
  ...
  vec_foreach(mrit, lcm->map_resolvers)
    {
      get_local_ip_for_dst (lcm, mrit, sloc);
      if (0 != sloc) {
...
{
  u32 i, rv, minp = ~0, limitp = 0, li, check_index = 0, done = 0;
