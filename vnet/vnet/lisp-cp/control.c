                                   ...
                                   vlib_cli_command_t * cmd)
{
  ...
  /* if vni is specified we need to have gid address as of LCAF type */
  if (vni != 0)
    {
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "del-all"))
        ...
        {
          // FIXME: this won't work since seid/deid are LCAFs
      ...
      while (n_left_from > 0 && n_left_to_next_drop > 0)
        {
          ...
          if (vni)
            {
              /* vni is non-default - we need to build a LCAF-IID */
              lcaf_t lcaf = lcaf_iid_init (vni);
              dst = gid_build_lcaf (&lcaf, &dst);
              src = gid_build_lcaf (&lcaf, &src);
            }
...
add_fwd_entry (lisp_cp_main_t* lcm, u32 src_map_index, u32 dst_map_index)
{
  ...
  for (i = 0; i < vec_len (dst_ls->locator_indices); i++)
    {
      ...
      if (l->priority < minp
          && (gid_address_type(&l->address) == GID_ADDR_IP_PREFIX
              || gid_address_type(&l->address) == GID_ADDR_LCAF))
        {
          minp = l->priority;
          dl = l;
        }
  ...
  if (dl)
    {
      ...
      if (gid_address_type(gid) == GID_ADDR_LCAF)
        {
          lcaf_t * lcaf = &gid_address_lcaf (&dl->address);
      ...
      for (i = 0; i < vec_len(src_ls->locator_indices); i++)
        {
          ...
          if (ip_addr_version(&gid_address_ip(gid)) == IP4)
            {
              ip4_address_t * l4;
  ...
  /* TODO remove */
  u8 ipver = ip_prefix_version(&gid_address_ippref(
                gid_addr_skip_lcaf(&a->deid)));
