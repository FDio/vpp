                     ...
                     vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 4 && n_left_to_next >= 2)
    {
      ...
      vlib_buffer_advance(b0,
          -(word) (sizeof(udp_header_t) + sizeof(ip4_header_t)));
      ...
      if (PREDICT_FALSE(
          (key0.as_u64[0] != last_key.as_u64[0])
      ...
      {
        nsh_vxlan_gpe_rx_trace_t *tr = vlib_add_trace(vm, node, b0,
            sizeof(*tr));
        tr->next_index = next0;
        tr->error = error0;
      ...
      if (PREDICT_FALSE(
          (key1.as_u64[0] != last_key.as_u64[0])
      ...
      {
        nsh_vxlan_gpe_rx_trace_t *tr = vlib_add_trace(vm, node, b1,
            sizeof(*tr));
        tr->next_index = next1;
        tr->error = error1;
  ...
  vlib_node_increment_counter(vm, nsh_vxlan_gpe_input_node.index,
      NSH_VXLAN_GPE_ERROR_DECAPSULATED, pkts_decapsulated);
