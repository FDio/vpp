               ...
               vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
    ...
    while (n_left_from >= 4 && n_left_to_next >= 2)
    {
      ...
      hi0 = vnet_get_sup_hw_interface(vnm,
      vnet_buffer(b0)->sw_if_index[VLIB_TX]);
      hi1 = vnet_get_sup_hw_interface(vnm,
...
#undef _
      ...
      if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
        nsh_vxlan_gpe_encap_trace_t *tr = vlib_add_trace(vm, node, b0,
            sizeof(*tr));
        tr->tunnel_index = t0 - ngm->tunnels;
      }

      if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
        nsh_vxlan_gpe_encap_trace_t *tr = vlib_add_trace(vm, node, b1,
            sizeof(*tr));
...
#undef _
  ...
  vlib_node_increment_counter(vm, node->node_index,
      NSH_VXLAN_GPE_ENCAP_ERROR_ENCAPSULATED, pkts_encapsulated);
