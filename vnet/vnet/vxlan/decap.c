...
typedef enum {
  ...
  IP_VXLAN_BYPASS_NEXT_IP_LOOKUP,
  IP_VXLAN_BYPASS_N_NEXT,
} ip_vxan_bypass_next_t;

/* Extern references to IP functions */
extern void
  ...
  while (n_left_from > 0)
    {
	  ...
	  /* Default to next IP feature, which should be ip-looup since 
	     vxlan-bypass is supposed to be the last feature. Thus, instead 
	     of calling: vnet_feature_next(
	                   vnet_buffer(b0)->sw_if_index[VLIB_RX], &next0, b0);
	     we hard code next node index to ip-lookup to minimize oevrhead 
	     for non-VXLAN packets */
	  next0 = next1 = IP_VXLAN_BYPASS_NEXT_IP_LOOKUP;
	  ...
	  if (is_ip4) 
	    {
	      if ((addr4.as_u32 == 0) || 
		  (addr4.as_u32 != ip0->dst_address.as_u32))
	        {
	...
	exit0:
	  ...
	  if (is_ip4) 
	    {
	      if ((addr4.as_u32 == 0) ||
		  (addr4.as_u32 != ip1->dst_address.as_u32))
	        {
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* Default to next IP feature, which should be ip-looup since 
	     ...
	     we hard code next node index to ip-lookup to minimize oevrhead 
	     for non-VXLAN packets */
	  next0 = IP_VXLAN_BYPASS_NEXT_IP_LOOKUP;
...
VLIB_REGISTER_NODE (ip4_vxlan_bypass_node) = {
  ...
  .next_nodes = {
    ...
    [IP_VXLAN_BYPASS_NEXT_IP_LOOKUP] = "ip4-lookup",
  },
