 ...
 */
static uword lldp_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node,
                          vlib_frame_t *frame)
{
 ...
 */
static uword lldp_process(vlib_main_t *vm, vlib_node_runtime_t *rt,
                          vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (n->last_sent < 0.01 ||
                now > n->last_sent + lm->msg_tx_interval) {
#if LLDP_DEBUG
                                               ...
                                               u32 sw_if_index, u32 flags)
{
    ...
    if (n) {
        if (!(flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP)) {
            ...
            /* FIXME - the packet sent immediately after schedule isn't send
             * properly - see comment next to lldp_hw_interface_up_down */
...
/* FIXME this callback gets called when interface goes down, but flags are
 * empty, so it's not usable, thus disabled for now */
/*
