...
format_lldp_intfs (u8 * s, va_list * va)
{
  ...
  pool_foreach (n, lm->intfs, (
				...
				sw =
				vnet_get_sw_interface (lm->vnet_main,
						       hw->sw_if_index);
				/* Interface shutdown */
				if (!
				    (sw->flags &
				...
				s =
				format (s,
					"%=25s %=50s %=50s %=10.1f %=10.1f %=25s\n",
					hw->name, "", "", n->last_heard,
					n->last_sent, "our interface down");}
				else
				if (now < n->last_heard + n->ttl)
				{
				s =
				format (s,
					"%=25s %=50U %=50U %=10.1f %=10.1f %=25s\n",
...
{
.path = "show lldp",.short_help = "Show lldp command",.function = show_lldp,};
