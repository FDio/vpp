...
static lldp_cfg_err_t lldp_cfg_intf_set(u32 hw_if_index, int enable)
{
    ...
    if (enable) {
        lldp_intf_t *n = lldp_create_intf(lm, hw_if_index);
...
static clib_error_t *lldp_intf_cmd(vlib_main_t *vm, unformat_input_t *input,
                                   vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "%U %U", unformat_vnet_hw_interface, vnm, &hw_if_index,
        ...
        return lldp_cfg_err_to_clib_err(lldp_cfg_intf_set(hw_if_index, enable));
    } else {
...
static clib_error_t *lldp_cfg_cmd(vlib_main_t *vm, unformat_input_t *input,
                                  vlib_cli_command_t *cmd)
{
    int hold_time = 0;
    int tx_interval = 0;
...
u8 *format_lldp_port_id(u8 *s, va_list *va)
{
    ...
    case LLDP_PORT_ID_SUBTYPE_NAME(intf_name):
        s = format(s, "%s|%U", lldp_port_id_subtype_str(subtype),
                   format_ascii_bytes, id, len);
        break;
...
u8 *format_lldp_chassis_id(u8 *s, va_list *va)
{
    ...
    case LLDP_CHASS_ID_SUBTYPE_NAME(intf_name):
        s = format(s, "%s|%U", lldp_chassis_id_subtype_str(subtype),
                   format_ascii_bytes, id, len);
        break;
    case LLDP_CHASS_ID_SUBTYPE_NAME(mac_addr):
        if (ETHER_ADDR_LEN == len) {
            s = format(s, "%s|%U", lldp_chassis_id_subtype_str(subtype),
                       format_mac_address, id, len);
            break;
        }
    /* fallthrough */
...
static u8 *format_lldp_intfs(u8 *s, va_list *va)
{
    ...
    s = format(s, "LLDP neighbor table:\n%=25s %=50s %=50s %=10s %=10s %=25s\n",
               "Our Port", "Peer Chassis ID", "Peer Port", "Last Heard",
               "Last Sent", "Status");
    ...
    pool_foreach(
        n, lm->intfs, ({
            ...
            /* Interface shutdown */
            if (!(sw->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP)) {
                s = format(s, "%=25s %=50s %=50s %=10.1f %=10.1f %=25s\n",
                           hw->name, "", "", n->last_heard, n->last_sent,
                           "our interface down");
            } else if (now < n->last_heard + n->ttl) {
                s = format(s, "%=25s %=50U %=50U %=10.1f %=10.1f %=25s\n",
                           hw->name, format_lldp_chassis_id,
                           n->chassis_id_subtype, n->chassis_id,
                           vec_len(n->chassis_id), format_lldp_port_id,
                           n->port_id_subtype, n->port_id, vec_len(n->port_id),
                           n->last_heard, n->last_sent, "peer active");
            } else {
                s = format(s, "%=25s %=50s %=50s %=10.1f %=10.1f %=25s\n",
                           hw->name, "", "", n->last_heard, n->last_sent,
                           "peer inactive");
...
u8 *lldp_input_format_trace(u8 *s, va_list *args)
{
    ...
    cur = t->data;
    while (
        ((cur + lldp_tlv_get_length((lldp_tlv_t *)cur)) < t->data + t->len)) {
        tlv = (lldp_tlv_t *)cur;
        if (cur == t->data) {
