	       ...
	       unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  if (unformat (input, "%U %U", unformat_vnet_hw_interface, vnm,
		&hw_if_index, unformat_vlib_enable_disable, &enable))
    {
      return
...
lldp_cfg_set (u8 ** host, int hold_time, int tx_interval)
{
  ...
  if (reschedule)
    {
      vlib_process_signal_event (lm->vlib_main,
				 lm->lldp_process_node_index,
	    ...
	    if (!(sw->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP))
	    ...
	    else if (now < n->last_heard + n->ttl)
	      {
		    s = format(s, "%=25s %=50U %=50U %=10.1f %=10.1f %=25s\n",
		               ...
		               n->port_id_subtype, n->port_id,
		               vec_len(n->port_id), n->last_heard,
		               n->last_sent, "peer active");
	      }
	    else
	      {
		    s = format(s, "%=25s %=50s %=50s %=10.1f %=10.1f %=25s\n",
