...
There are many excellent new features in this release, however a couple of them deserve a special mention.

First, one of the more significant news for this release is a linux-cp plugin: it allows the start of near-seamless integration with the
host control plane, by mirroring the VPP interfaces into a TUN or TAP device created in the linux kernel: all the punted
packets on the receive direction of the VPP interface will be sent to the linux counterpart, and in the transmit direction,
the packets sent by linux kernel stack will be sent out the VPP interface. This plugin lays the foundation for the much easier
integration with external software working in tandem with VPP.

Another very interesting feature is the perfmon plugin. It allows to collect very detailed low-level CPU statistics on a per-node
basis. So, it gives a very useful advanced troubleshooting tool, should you encounter that a specific node's performance is not
on par with what it should be. Note, that the correct functioning of this plugin may require changing the /proc/sys/kernel/perf_event_paranoid
setting allowing the access to the performance counters.

And finally - not a feature, but a clarification. If you look at the API changes table, you will notice a value in the right column
that wasn't there before - "message CRC32 fix". The history of this issue goes back to summer of 2020,
when we discovered that for a considerable amount of time (several months) the calculation of the CRC
on the messages was incorrect. Namely - all but the very first user-defined types in the message did not alter the CRC of it.
In plain words, this means that one might end up with a situation where client and vpp layout of message in memory could be quite different:
the situation that is the reason for the inclusion of a layout-specific value as part of the low-level message registration name.

Luckily, there were no changes that matched this bug. However, simply fixing that bug meant that we would alter the CRC of about half
of the VPP API messages. That would result in a significant pain to the consumers of VPP, for no particular good reason for them.
So we tried to avoid this pain. A message CRC is really an opague number for anyone using it, and the only property of it for the user
is that the messages with the same value of CRC have the same layout on the wire with the sufficiently high degree of certainty.

So, in the fix which went in ([9f84e70c6](https://gerrit.fd.io/r/gitweb?p=vpp.git;a=commit;h=9f84e70c6)) we applied a band-aid to avoid this pain.
Namely, we fix the CRC generation algorithm, but captured the "new" CRC values for those messages that had their CRCs arbitrarily changed by this fix,
and created a substituion table with (message name, new CRC, old CRC) triplets - and for a given message the CRC happened to match the recorded new
value, then we would substitute it with the old value, thus trading in a tiny reduction in collision resistance (since we effectively kick "out of order" two values of CRC out of 2^32) for avoiding the unnecessary pain for the users of adapting to several hundred of messages which changed the CRCs.

This band-aid also had a property that whenever a message does change the definition, then it will automatically get a "correct" calculation of CRC32, will
no longer use the slot in the table, thus the table can be eventually emptied naturally over long period of time - thus allowing a completely painless transition, while also preserving the integrity check for the affected messages - if any fields changed, the CRC would no longer match the "new" value thus no substitution would be made.

Since the only property the message CRC has from the user standpoint is that it should change if the message layout changes, this solution did not appear controversial - 2^32 collision space minus a couple of slots is still pretty close to 2^32. And the fact that it no longer was a perfect CRC32 over an arbitrary representation structure is nothing that a user should worry about.  However, in practice this solution was not accepted very well. So, after a notification, ([da1b76aa8](https://gerrit.fd.io/r/gitweb?p=vpp.git;a=commit;h=da1b76aa8)) has removed the graceful transition table. So, all of the changes that did not happen in 9f84e70c6, happened in da1b76aa8. We hope the longer awareness period about these changes has reduced the inconvenience. We also warn the users to not rely on any other property of the message CRC other than that it changes if the message layout changes.
...
Starting with this release, we add the section about the section with the unresolved Coverity Issues
into the Release Notes. In order to view the issues, visit https://scan.coverity.com/, add
yourself to fd.io VPP project and click on the matching IDs.
...
* _Message CRC32 fix_: please refer to release highlights for description.
