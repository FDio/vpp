...
Another interesting feature is the perfmon plugin. It allows collection of detailed low-level CPU statistics on a per-node
basis. So, it gives a useful advanced troubleshooting tool, should you encounter that a specific node's performance is not
...
And finally - not a feature, but a clarification. Some messages in the API changes table for this release have a
new marking - "message CRC32 fix". The history of this issue goes back to summer of 2020,
when we discovered that for a considerable amount of time (several months) the calculation of the CRC
on the messages was incorrect. Namely - all but the first user-defined types in the message did not alter the CRC of it.
...
There were no changes that got affected by that bug. However, simply fixing that bug meant the CRC of about half
of the VPP API messages would be altered for no reason. That would result in a significant pain to the consumers of VPP.
But, a message CRC is just an opague number for anyone using it, and the only property of it for the user
is that the messages with the same value of CRC have the same layout on the wire with the sufficiently high certainty.

So, the fix which went in ([9f84e70c6](https://gerrit.fd.io/r/gitweb?p=vpp.git;a=commit;h=9f84e70c6)) also contained a "band-aid" to avoid this pain.
It fixed the CRC generation algorithm, but captured the "new" CRC values for those messages that had their CRCs arbitrarily changed by this fix,
and created a substituion table with (message name, new CRC, old CRC) triplets. If for a given message the CRC happened to match the recorded new
value, the code would substitute it with the old value, thus trading in a tiny reduction in collision resistance
 (two values of CRC out of 2^32 space) for not having the users to adapt to several hundred of messages which changed the CRCs.

This band-aid also has a property that whenever a message does change the definition, then it will automatically get a "correct" calculation of CRC32, will
...
Since the choice of CRC32 is just an implementation detail which is supposed to be opaque to the user, it was deemed a reasonable approach.  However, in practice this solution was not accepted well. So, after a notification, ([da1b76aa8](https://gerrit.fd.io/r/gitweb?p=vpp.git;a=commit;h=da1b76aa8)) has removed the graceful transition table. Thus all of the changes that did not happen in 9f84e70c6, happened in da1b76aa8. Hopefully having the notification period about these changes has reduced the inconvenience. We also ask the users to not rely on any other property of the message CRC other than that its value changes if the message layout changes.
