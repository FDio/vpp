...
More than 407 commits since the previous release, including 226 fixes.

## Release Highlights

This release is the first VPP release that introduced
the backwards-compatible API change policy. Its mission
is provide a zero-effort upgrade on a "N+1" version
of VPP for the users, provided no in-progress and
no deprecated APIs are used when running on version N.

If you look at the API changes, you will immediately notice
the effect of this policy being used shortly after 20.05 release.
There are only a couple of messages that had their signature
modified, the rest are still callable even if there are newer
versions.

This process also changes the way the release notes look like, as
now there are two new sections:
- Newly deprecated API messages: if you are using any of these messages,
you must migrate off them to the new version, or, if there is no new
version, collaborate with the feature maintainer on the best approach.
- In-progress API messages: They are work-in-progress, and are *not* subject
to any compatibility process, they can disappear at any time.
If you intend to use these APIs in anything beyond a technology demo,
you will need to collaborate with the feature owner on the possible
path to "productizing" them, thus, making the changes subject to predictable
process. Notice, that unlike the previous list, this is a *total* list of messages,
since the idea is that the messages are not on this list forever - they must
either either eventually move to production or be cleaned up and removed if no longer needed.
...
- Crypto openssl Plugin
  - Add chacha20-poly1305 to crypto-openssl (1b6ed022e)
- Infrastructure Library
  - Multiarch support for OCTEONTX2 SoC (e2f5236dc)
- Plugins
  - AF\_XDP driver
    - AF\_XDP input plugin (4a76d6f6d)
  - CNat
    - Destination based NAT (29f3c7d2e)
  - DPDK
    - Device\_id sorted order for cryptodev (5a849e3b3)
  - Internet Key Exchange (IKEv2) Protocol
    - Add support for NAT traversal (4362baa33)
  ...
  - Wireguard
    - Initial implementation of wireguard protocol (edca1325c)
- VNET
  - Crypto Infra
    - Add chacha20-poly1305 algo (61f49aa38)
    - SW scheduler async crypto engine (2284817ea)
    - Add async crypto APIs (0c936b147)
    - Change cryptodev with new cryptodev API (ef80ad6bf)
  - FLOW
    - Add IPSec ESP/AH flow (d4c3666b9)
    - Add vnet/flow formal API (d0236f725)
  - GENEVE
    - Support geneve interface acting as a bvi (7fc88cf3a)
  - GSO
    - Packet coalesce library (f382b06fe)
  - IPSec
  ...
  - TCP
    - Track reorder with sacks (cc4d6d022)
- VPP Comms Library
