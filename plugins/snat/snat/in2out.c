...
static u8 * format_snat_in2out_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "SNAT_IN2OUT: sw_if_index %d, next index %d, session %d",
...
static uword
snat_in2out_node_fn (vlib_main_t * vm,
		  vlib_node_runtime_t * node,
		  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
          u32 bi0, bi1;
	  vlib_buffer_t * b0, * b1;
          u32 next0 = SNAT_IN2OUT_NEXT_DROP, next1 = SNAT_IN2OUT_NEXT_DROP;
	  ...
	  vnet_get_config_data (&cm->config_main,
          ...
          if (clib_bihash_search_8_8 (&sm->in2out, &kv1, &value1))
            {
              ip4_address_t * first_int_addr;
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
          u32 bi0;
	  vlib_buffer_t * b0;
          u32 next0 = SNAT_IN2OUT_NEXT_DROP;
          u32 sw_if_index0;
          ip4_header_t * ip0;
          ip_csum_t sum0;
          u32 new_addr0, old_addr0;
  ...
  vlib_node_increment_counter (vm, snat_in2out_node.index, 
...
VLIB_NODE_FUNCTION_MULTIARCH (snat_in2out_node, snat_in2out_node_fn);
