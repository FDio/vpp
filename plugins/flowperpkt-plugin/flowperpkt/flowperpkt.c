                                  ...
                                  u16 collector_port)
{
  ...
  f->e_id_length = ipfix_e_id_length (0 /* enterprise */, ipClassOfService, 
                                      1);
  f++;
  f->e_id_length = ipfix_e_id_length (0 /* enterprise */, flowStartNanoseconds, 
                                      8);
  f++;
 ...
 * It must return the incoming frame, or no template packets will be sent
 */
...
(vl_api_flowperpkt_tx_interface_add_del_t * mp)
{
  ...
  rv = flowperpkt_tx_interface_add_del_feature (fm, sw_if_index,
                                                mp->is_add);
