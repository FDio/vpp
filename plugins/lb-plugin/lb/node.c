         ...
         vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  u32 lb_time = (u32) (vlib_time_now(vm) + LB_STICKY_TIMEOUT + 1);
  lb_hash_t *sticky_ht = lbm->per_cpu_sticky_ht[cpu_index];
  if (PREDICT_FALSE(sticky_ht == NULL)) {
    lbm->per_cpu_sticky_ht[cpu_index] = lb_hash_alloc(LB_STICKY_BUCKETS, LB_STICKY_TIMEOUT);
    sticky_ht = lbm->per_cpu_sticky_ht[cpu_index];
  }
  ASSERT(sticky_ht);
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from > 0 && n_left_to_next > 0)
    {
      ...
      if (PREDICT_TRUE(value0 != ~0)) {
        ...
        as0 = &vip0->ass[value0];
      } else if (PREDICT_FALSE(vip0->new_flow_table == NULL)) {
        //No AS in the table at all !
        ...
        clib_warning("AVAILABLE SLOT with AS %d", as0 - vip0->ass);
        error0 = LB_ERROR_NEW_TRACKED;
        //TODO: Decrement counter for previous flow
        ...
        clib_warning("NO AVAILABLE SLOT");
        //Could not store new entry in the table
        as0 = &vip0->ass[vip0->new_flow_table[hash0 & vip0->new_flow_table_mask].as_index];
        error0 = LB_ERROR_NEW_UNTRACKED;
      }
