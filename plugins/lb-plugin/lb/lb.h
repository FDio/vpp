...
typedef struct {
   ...
   * The element at index 0 is reserved for invalid AS.
   * It is used to hold references of free buckets in hash tables
   * and allow same execution path when there is not configured AS.
   */
  lb_as_t *ass;
   ...
   * AS index 0 is initialized to 0 but will be decremented as
   * the sticky flow table is populated.
   */
...
int lb_conf(ip4_address_t *ip4_address, ip6_address_t *ip6_address,
            u32 sticky_buckets, u32 per_cpu_sticky_buckets);
