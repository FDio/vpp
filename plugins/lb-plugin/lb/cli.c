              ...
              unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  u8 plen;
  u32 new_len = 1024;
  u32 sticky_buckets = 1024;
  u32 sticky_size = 1 << 20;
  u8 del = 0;
  int ret;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
  {
    if (unformat(line_input, "new_len %d", &new_len))
      ;
    else if (unformat(line_input, "sticky_buckets %d", &sticky_buckets))
      ;
    else if (unformat(line_input, "sticky_size %d", &sticky_size))
      ;
  ...
  u32 index;
  if (!del) {
    if ((ret = lb_vip_add(&prefix, plen, new_len, sticky_buckets, sticky_size, &index))) {
...
{
  .path = "lb vip",
  .short_help = "lb vip <prefix> [new_len <n>] [sticky_buckets <n>] [sticky_size <n>] [del]",
              ...
              unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  type = ip46_prefix_type(&vip_prefix, vip_plen);
  if ((ret = lb_vip_find_index(&vip_prefix, vip_plen, &vip_index)))
    return clib_error_return (0, "lb_vip_find_index error %d", ret);

  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
  {
    if (unformat(line_input, "%U", unformat_ip46_address, &as_addr, type)) {
              ...
              unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output(vm, "ip4-src-address: %U \n"
                      "ip6-src_address: %U \n",
                  format_ip4_address, &lbm->ip4_src_address,
                  format_ip6_address, &lbm->ip6_src_address);
  vlib_cli_output(vm, "#vips: %u", pool_elts(lbm->vips));
  return NULL;
}
