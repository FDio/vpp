...
static u8 * format_snat_in2out_worker_handoff_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  snat_in2out_worker_handoff_trace_t * t = va_arg (*args, snat_in2out_worker_handoff_trace_t *);
  ...
  if (u->nsessions >= sm->max_translations_per_user)
    {
      ...
      do {
          oldest_per_user_translation_list_index =
            ...
            (sm->per_thread_data[cpu_index].list_pool, u->sessions_per_user_list_head_index);
          ...
          /* Get the session */
          s = pool_elt_at_index (sm->per_thread_data[cpu_index].sessions, session_index);
                               ...
                               vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      if (next_worker_index != current_worker_index)
	{
	  ...
	  hf = vlib_get_worker_handoff_queue_elt (sm->wh_in2out_index,
                                                  next_worker_index,
