...
VNET_IP4_UNICAST_FEATURE_INIT (ip4_snat_in2out, static) = {
  ...
  .runs_before = (char *[]){"snat-out2in-worker-handoff", 0},
...
static clib_error_t * snat_init (vlib_main_t * vm)
{
  ...
  vlib_node_t *hd_node = vlib_get_node_by_name (vm, (u8 *) "handoff-dispatch");
  ...
  /* Hook SNAT in2out and out2in nodes to handoff-dispatch */
  sm->hd_in2out_slot = vlib_node_add_next (vm, hd_node->index, snat_in2out_node.index);
  sm->hd_out2in_slot = vlib_node_add_next (vm, hd_node->index, snat_out2in_node.index);
