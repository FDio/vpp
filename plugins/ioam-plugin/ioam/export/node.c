		  ...
		  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  ip_len1 = clib_net_to_host_u16 (ip61->payload_length) + sizeof(ip6_header_t);
              ...
              if (p0->flags & VLIB_BUFFER_IS_TRACED)
                {
                    ...
                    t->flow_label = clib_net_to_host_u32(ip60->ip_version_traffic_class_and_flow_label);
                    t->next_index = next0;
                  }
                if (p1->flags & VLIB_BUFFER_IS_TRACED)
                  {
                    ...
                    t->flow_label = clib_net_to_host_u32(ip61->ip_version_traffic_class_and_flow_label);
                    t->next_index = next1;
                  }
              }
	NO_BUFFER1:
