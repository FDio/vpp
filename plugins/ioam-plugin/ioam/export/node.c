		  ...
		  vlib_frame_t * frame)
{
  ...
  u32 pkts_recorded = 0;
  ioam_export_buffer_t *my_buf = ioam_export_get_my_buffer(vm);
  vlib_buffer_t *eb0 = 0;
  u32 ebi0 = 0;
  ...
  //TODO what if my_buf is NULL, check here and enqueue frame to next node instead?
  if (PREDICT_FALSE(my_buf == 0))
    {
    }
  else
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  flow_label0 = clib_net_to_host_u32(ip60->ip_version_traffic_class_and_flow_label);
	  flow_label1 = clib_net_to_host_u32(ip61->ip_version_traffic_class_and_flow_label);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  p0 = vlib_get_buffer (vm, bi0);
          ip60 = vlib_buffer_get_current (p0);
	  ip_len0 = clib_net_to_host_u16 (ip60->payload_length) + sizeof(ip6_header_t);
	  ...
	  // fetch length of ip60
          ip_len0 = ip_len0 > DEFAULT_EXPORT_SIZE ? DEFAULT_EXPORT_SIZE : ip_len0;
	  clib_memcpy (eb0->data + eb0->current_length, ip60, ip_len0);
	  eb0->current_length += DEFAULT_EXPORT_SIZE; // To maintain uniform size per export, each 
	                                              // record is default size, ip6 hdr can be
	  my_buf->records_in_this_buffer++;              // used to parse the record correctly
