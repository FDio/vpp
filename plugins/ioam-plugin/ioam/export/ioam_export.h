...
always_inline ioam_export_buffer_t *ioam_export_get_my_buffer(vlib_main_t *vm)
{
  ...
  if (vec_len(em->buffer_per_thread) > vm->cpu_index)
    return(pool_elt_at_index(em->buffer_pool, em->buffer_per_thread[vm->cpu_index]));
  return(0);
}
...
inline static int ioam_export_thread_buffer_init (vlib_main_t *vm)
{
  ...
  vec_validate_aligned (em->buffer_per_thread,
			no_of_threads-1,
			CLIB_CACHE_LINE_BYTES);
  if (!em->buffer_per_thread || !em->buffer_pool)
    {
      return(-1);
    }
