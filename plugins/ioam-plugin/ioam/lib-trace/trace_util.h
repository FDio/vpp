 ...
 */
always_inline
trace_profile *trace_profile_find();
 ...
 */
u64 trace_update_num_elts(trace_profile * profile, u8 num_elts);


extern void clear_trace_profiles();
extern int trace_profile_list_is_enabled(u8 *name);


static inline void trace_profile_reset_usage_stats (trace_profile *tr)
{
  if (tr) {
    tr->total_pkts_using_this_profile = 0;
  }
}

static inline void trace_profile_incr_usage_stats (trace_profile *tr)
{
  if (tr) {
    tr->total_pkts_using_this_profile++;
  }
}
