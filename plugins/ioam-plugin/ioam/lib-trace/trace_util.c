...
static void trace_profile_cleanup(trace_profile * profile)
{
...
static void trace_main_profiles_reset (void)
{
    trace_main_t *sm = &trace_main;
    trace_profile_cleanup(&(sm->profile));
}
    ...
    unformat_input_t * input, vlib_cli_command_t * cmd)
{
    ...
    s = format(s, "         Trace timestamp precision : %d (%s)\n", p->trace_tsp,
         (p->trace_tsp == TSP_SECONDS) ? "Seconds" :
        ((p->trace_tsp == TSP_MILLISECONDS) ? "Milliseconds" :
