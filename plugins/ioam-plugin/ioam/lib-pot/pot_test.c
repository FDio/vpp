...
_(pot_profile_show_config_reply,				\
    errmsg("		ID:%d\n",mp->id);			\
    errmsg("	 Validator:%d\n",mp->validator);		\
...
_(POT_PROFILE_SHOW_CONFIG_REPLY, pot_profile_show_config_reply)
...
static int api_pot_profile_show_config (vat_main_t *vam)
{
    pot_test_main_t * sm = &pot_test_main;
    unformat_input_t *input = vam->input;
    ...
    M(POT_PROFILE_SHOW_CONFIG, pot_profile_show_config);
...
_(pot_profile_show_config, "id [0-1]")
