...
Inband OAM (iOAM) is an implementation study to record operational information in the packet while the packet traverses a path between two points in the network. iOAM is to complement current out-of-band OAM (sometimes also called "active" OAM) mechanisms based on ICMP or other types of probe packets.
...
Overview of iOAM can be found in [iOAM-Devnet] page. The following IETF drafts detail the motivation and mechanism for recording operational information:
 - [ioam-ietf-requirements] - Describes motivation, usecases for iOAM
 - [ioam-ietf-data] - Describes data that can be collected using iOAM
 - [ioam-ietf-transport] - Lists out the transport protocols and mechanism to carry iOAM data
 - [ioam-ietf-proof-of-transit] - Describes the idea of proof of transit and mechanism to operationalize the idea
...
In-band OAM data is added to a packet on entering the in-band OAM-domain and is removed from the packet when exiting the domain. Within the in-band OAM-domain, the in-band OAM data may be updated by network nodes that the packet traverses.
...
In this version of VPP In-band OAM data is transported as options in an IPv6 hop-by-hop extension header. Hence In-band OAM can be enabled for IPv6 traffic.
Following iOAM features are supported:
- **In-band OAM Tracing** : In-band OAM for IPv6 supports multiple data elements to be recorded into the packet as it traverses the network that can be enabled to provide different statistic of the network. Following information can be collected in the tracing data from the nodes a packet traverses:
  - Node ID
...
- **In-band OAM Proof of Transit**: Proof of transit iOAM data is added to every packet for verifying the nodes traversed by the packet.  In-band OAM data is updated at every node that is enabled with iOAM proof of transit and is used to verify whether a packet traversed all the required nodes. When the verifier receives each packet, it can validate whether the packet traversed the required nodes.


## Configuration
The CLI for configuring iOAM are explained here followed by detailed steps and examples to deploy iOAM on VPP as an encapsulating, transit or decapsulating iOAM node in the subsequent sub-sections.

VPP ioam configuration for enabling trace and pot is as follows:

**set ioam rewrite** trace-type <0x1f|0x7|0x9|0x11|0x19> trace-elts <number of trace elements> trace-tsp <0|1|2|3> node-id <node id in hex> app-data <application data in hex> [pot]

Description of each of the options of the CLI follows:
...
#### On in-band OAM encapsulating node
 ...
 ```
 - **Enable tracing** : Specify node id, maximum number of node trace to be recorded, type of data to be enabled for recording, optionally application data to be included
...
```sh
vpp# set ioam rewrite trace-type 0x1f trace-elts 4 trace-tsp 1 node-id 0x1 app-data 0x1234
```

#### On in-band OAM transit node
- Transit node requires trace type, timestamp precision, node id and optionally application data to be configured, to update its node data in the trace option.
Example:
```sh
vpp# set ioam rewrite trace-type 0x1f trace-elts 4 trace-tsp 1 node-id 0x2 app-data 0x1234
```

#### On in-band OAM decapsulating node
- Decapsulating node similar to encapsulating node requires **classification** of the packets to remove iOAM data from.
...
Example:
```sh
vpp# set ioam rewrite trace-type 0x1f trace-elts 4 trace-tsp 1 node-id 0x3 app-data 0x1234
```

### Proof of Transit configuration
To enable Proof of Transit all the nodes that participate and hence are verfied for transit need proof of transit profile. Script to generate proof of transit profile as per the mechanism described in [ioam-ietf-proof-of-transit] will be available at [iOAM-Devnet].

Briefly Proof of transit mechanism implemented here is based on Shamir's Secret Sharing algorithm. The overall algorithm uses two polynomials: POLY-1 and POLY-2, degree of the polynomials depends on number of nodes to be verified for transit.  POLY-1 is secret and constant.  Each node gets a point on POLY-1 at setup-time and keeps it secret.  POLY-2 is public, random and per packet.  Each node is assigned a point on POLY-1 and POLY-2 with the same x index. Each node derives its point on POLY-2 each time a packet arrives at it.  A node then contributes its points on POLY-1 and POLY-2 to construct POLY-3 (POLY-3 = POLY-1 + POLY-2) using lagrange extrapolation and passes it towards the verifier by updating POT data in the packet.  The verifier constructs POLY-3 from accumulated value from all the nodes and its own points on POLY-1 and POLY-2 and verifies whether POLY-3 = POLY-1 + POLY-2.  Only the verifier knows POLY-1.  The solution leverages finite field arithmetic in a field of size "prime number" for reasons explained in description of Shamir's secret sharing algorithm.
...
- **Create POT profile** : set pot profile name <string> id [0-1] [validator-key 0xu64]    prime-number 0xu64 secret_share 0xu64 lpc 0xu64 polynomial2 0xu64 bits-in-random [0-64]
    ...
    - prime-number : for finite field arithmetic as required for proof of transit mechanism
    - secret_share : Unique point for each node on the the secret polynomial
    ...
    - polynomial2 : is the pre-evaluated value of the point on 2nd polynomial unique for each node except for the constant part of the polynomial that changes per packet and arrives as part of the POT data in the packet
    - bits-in-random : to control the size of the random number to be generated and this has to match the other numbers generated and used in the profile as per the algorithm
...
#### On in-band OAM encapsulating node
 ...
 vpp# set int input acl intfc GigabitEthernet0/0/0 ip6-table 0
 ```
 - Configure proof of transit profile list with profiles. Each profile list referred to by a name can contain 2 profiles, only one is in use for updating proof of transit data at any time.
...
#### On in-band OAM transit node
 - Configure proof of transit profile list with profiles for Transit node.
Example:
```sh
