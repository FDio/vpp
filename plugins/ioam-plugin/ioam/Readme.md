...
Overview of iOAM can be found in [iOAM-Devnet] page. The following IETF drafts detail the motivation and mechanism for recording operational information:
 ...
 - [ioam-ietf-proof-of-transit] - Describes the idea of Proof of Transit(POT) and mechanism to operationalize the idea
...
- **In-band OAM Proof of Transit(POT)**: Proof of transit iOAM data is added to every packet for verifying the nodes traversed by the packet.  In-band OAM data is updated at every node that is enabled with iOAM proof of transit and is used to verify whether a packet traversed all the required nodes. When the verifier receives each packet, it can validate whether the packet traversed the required nodes.
...
- Selecting the packets for which iOAM data must be inserted, updated or removed
  ...
  - Selection of packets for update of iOAM data is implicitly done based on the presence of iOAM options in the packet
  - Selection of packets for removing the iOAM data is done based on 5-tuple based classification
- The kind of data to collected
  - Tracing data
  - Proof of transit
