...
recording operational information:
 ...
 and mechanism to carry iOAM data
 - [ioam-ietf-proof-of-transit] - Describes the idea of Proof of Transit(POT)
 and mechanism to operationalize the idea
...
may update the in-band OAM data.
...
- **In-band OAM Tracing** : In-band OAM for IPv6 supports multiple data elements
to be recorded into the packet as it traverses the network that can be enabled
to provide different statistic of the network.
Following information can be collected in the tracing
...
added to every packet for verifying the nodes traversed by the packet.
...
all the required nodes. When the verifier receives each packet,
it can validate whether the packet traversed the required nodes.
...
- Selecting the packets for which iOAM data must be inserted, updated or removed
  ...
  - Selection of packets for update of iOAM data is implicitly done based on the
  presence of iOAM options in the packet
  - Selection of packets for removing the iOAM data is done based on 5-tuple
...
The CLI for configuring iOAM are explained here followed by detailed steps
...
vpp# set int input acl intfc GigabitEthernet0/0/0 ip6-table 0
 ```
 - **Enable tracing** : Specify node id, maximum number of node trace to be
 recorded, type of data to be enabled for recording,
...
- Transit node requires trace type, timestamp precision, node id and
...
- Decapsulating node similar to encapsulating node requires
...
and hence are verified for transit need proof of transit profile.
Script to generate proof of transit profile as per the mechanism
described in [ioam-ietf-proof-of-transit] will be available at [iOAM-Devnet].

Briefly Proof of transit mechanism implemented here is based on
Shamir's Secret Sharing algorithm.
The overall algorithm uses two polynomials:
POLY-1 and POLY-2, degree of the polynomials depends on number of nodes
...
The verifier constructs POLY-3 from accumulated value from all the nodes
...
lpc 0xu64 polynomial2 0xu64 bits-in-random [0-64]
    ...
    - prime-number : for finite field arithmetic as required for
    ...
    generated and this has to match the other numbers generated and used
...
vpp# set int input acl intfc GigabitEthernet0/0/0 ip6-table 0
 ...
 - Configure proof of transit profile list with profiles.
...
tracing using "trace add dpdk-input 20" and
