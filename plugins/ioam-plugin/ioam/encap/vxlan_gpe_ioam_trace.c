...
#define foreach_vxlan_gpe_ioam_trace_stats				\
  _(PROCESSED, "Pkts with vxlan-gpe hop-by-hop trace options")			\
...
vxlan_gpe_ioam_register_option (u8 option,
				int options (vlib_buffer_t * b,
					     ...
					     u8 is_ipv4), u8 * trace (u8 * s,
								      ...
								      * opt))
{
  vxlan_gpe_main_t *im = &vxlan_gpe_main;
  vxlan_gpe_ioam_main_t *hm = &vxlan_gpe_ioam_main;
...
vxlan_gpe_ioam_trace_rewrite_handler (u8 * rewrite_string, u8 * rewrite_size)
{
  ...
  trace_option->hdr.type = VXLAN_GPE_OPTION_TYPE_IOAM_TRACE | HBH_OPTION_TYPE_DATA_CHANGE_ENROUTE;	/* vxlan-gpe_TODO: Fix this */
  ...
  if (PREDICT_TRUE (trace->data_list_elts_left))
      ...
      if (trace->ioam_trace_type & BIT_ING_INTERFACE)
	{
	  if (vxlan_gpe_tunnel == NULL)
	    {
	      u16 tx_if = 0;
	      //if (vnet_buffer (b)->ip.vxlan_gpe_decap == 0)
		{
