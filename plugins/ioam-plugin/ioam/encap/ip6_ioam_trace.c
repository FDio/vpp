...
int
ip6_hbh_ioam_trace_get_sizeof_handler(u32 *result)
{
  u16 size = 0;
  ...
  *result = 0;
  ...
  profile = trace_profile_find(0); /* lib-trace_TODO: Fix this hardcoding */
...
ip6_hbh_ioam_trace_data_list_handler (vlib_buffer_t *b, ip6_header_t *ip, ip6_hop_by_hop_option_t *opt)
{
  ...
  if (PREDICT_TRUE (trace->data_list_elts_left)) {
    ...
    trace_profile_incr_usage_stats(profile);
...
ip6_hop_by_hop_ioam_trace_init (vlib_main_t * vm)
{
  ...
  if (ip6_hbh_get_sizeof_register_option(HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST, ip6_hbh_ioam_trace_get_sizeof_handler) < 0)
    return (clib_error_create("registration of HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST get_sizeof failed"));
