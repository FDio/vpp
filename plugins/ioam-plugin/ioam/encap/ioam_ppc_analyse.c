      ...
      if (ppc_check_wraparound(bitmap->highest, ppc))
        {
          memset( bitmap->array, 0, bitmap->array_size * sizeof(u64));
          BIT_SET(bitmap->array, ppc & bitmap->mask);
          fformat(stderr,"\nWraparund seq number detected, "
                  "resetting current highest %lu to %lu",
                  bitmap->highest, ppc);
          bitmap->highest = ppc;
          return;
        }
        ...
        {
          fformat(stderr,"\nLarge reorder/replay of diff %lu pkts "
                  "detected ,more than %d,current ppc %lu, monitoring "
                  "for peer reset",
          ...
          if (peer_dead_count > 25)
            {
              ...
              memset( bitmap->array, 0, bitmap->array_size * sizeof(u64) );
              BIT_SET(bitmap->array, ppc & bitmap->mask);
              fformat(stderr, "\nDeclaring peer reset: resetting the PPC "
                      "current hightest %lu to %lu",
                      bitmap->highest, ppc);
              bitmap->highest = ppc;
            }
