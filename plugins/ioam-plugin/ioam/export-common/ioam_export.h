...
#define ioam_export_node_common(EM, VM, N, F, HTYPE, L, V, NEXT, OPAQUE)       \
do {                                                                           \
	...
	{                                                                      \
	  ...
	  if (OPAQUE) {                                                        \
            ip6_header_t *ip6_temp =                                           \
                (ip6_header_t *) (eb0->data + eb0->current_length);            \
            u32 flow_label_temp =                                              \
              clib_net_to_host_u32(                                            \
              ip6_temp->ip_version_traffic_class_and_flow_label) & 0xFFF00000; \
            flow_label_temp |=                                                 \
              IOAM_MASK_DECAP_BIT((vnet_buffer(p0)->l2_classify.opaque_index));\
	  ...
	  if (OPAQUE) {                                                        \
            ...
            u32 flow_label_temp =                                              \
              ...
              ip6_temp->ip_version_traffic_class_and_flow_label) & 0xFFF00000; \
            flow_label_temp |=                                                 \
              IOAM_MASK_DECAP_BIT((vnet_buffer(p1)->l2_classify.opaque_index));\
            ip6_temp->ip_version_traffic_class_and_flow_label =                \
              clib_host_to_net_u32(flow_label_temp);                           \
          }                                                                    \
	  eb0->current_length += DEFAULT_EXPORT_SIZE;                          \
