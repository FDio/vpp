...
format_vxlan_gpe_ioam_v4_trace (u8 * s, va_list * args)
{
  ...
  vxlan_gpe_ioam_v4_trace_t *t1
    = va_arg (*args, vxlan_gpe_ioam_v4_trace_t *);
...
vxlan_gpe_encap_decap_ioam_v4_one_inline (vlib_main_t * vm,
				    ...
				    vlib_buffer_t * b0,
				    vxlan_gpe_tunnel_t * t0, u32 *next0, u32 drop_node_val)
{
  ip4_header_t * ip0;
  ...
  while (opt0 < limit0)
	...
	default:
	  if (hm->options[type0])
	    {
	      if ((*hm->options[type0]) (b0, t0, opt0, 1,/* is_ipv4 */ 0 /* use_adj */) < 0)
		{
                  *next0 = drop_node_val;
                  return;
		}
  ...
  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
	...
	vlib_add_trace (vm, node, b0, sizeof (*t));
      ...
      trace_len =
	trace_len <
	ARRAY_LEN (t->fmt_trace.option_data) ? trace_len : ARRAY_LEN (t->fmt_trace.
								option_data);
      t->fmt_trace.trace_len = trace_len;
      clib_memcpy (&(t->fmt_trace.option_data), gpe_ioam0, trace_len);
      t->tunnel_index = t0 - ngm->tunnels;
    }
  return;
}
