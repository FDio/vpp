  ...
  if (!mp->is_ipv6)
    {
      ...
      vni = clib_host_to_net_u32 (mp->vni);
...
vxlan_gpe_init (vlib_main_t * vm)
{
  ...
  clib_error_t *error = 0;
  u8 *name;
  u32 node_index = vxlan_gpe_encap_ioam_v4_node.index;
  vlib_node_t *vxlan_gpe_encap_node = NULL;

  //bzero (sm, sizeof (vxlan_gpe_ioam_main));
  ...
  sm->encap_v4_next_node = vlib_node_add_next (vm, vxlan_gpe_encap_node->index, node_index);
  vec_free (name);
