			    ...
			    u8 ipv6_set)
{
  ...
  t->rewrite_size = size;
  t->flags |= VXLAN_GPE_TUNNEL_IS_IOAM_CAPABLE;
  ...
  if (!ipv6_set)
	...
	(vxlan_gpe_ioam_hdr_t *) (t->rewrite +
      ...
      t->encap_next_node = hm->encap_v4_next_node;
    }
  else
    {
		       ...
		       int has_ppc_option)
{
  ...
  ({
     vxlan_gpe_ioam_set_rewrite (t, hm->has_trace_option,
			    hm->has_pot_option,
			    hm->has_ppc_option,
                            !(ip46_address_is_ip4(&(t->local))));
  }));
