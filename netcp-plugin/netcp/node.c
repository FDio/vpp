...
static u8 * format_netcp_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "NETCP: sw_if_index %d, next index %d",
                        ...
                        vlib_buffer_t * b0)
{
  ...
  ack0 = (netcp_data_header_t *)(nh0+1);
                              ...
                              vlib_buffer_t * b0)
{
  ...
  ip->src_address.as_u32 = s->to.ip4_as_u32;
  ip->dst_address.as_u32 = s->from.ip4_as_u32;
...
u32 ((*netcp_rx_handlers)(netcp_main_t *nm, vlib_node_runtime_t *node, 
                          vlib_buffer_t * b0))[] =
{
#define _(a,b) b##_handler,
  foreach_netcp_type;  
#undef _
};
               ...
               vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  b0 = vlib_get_buffer (vm, bi0);
          ...
          if (PREDICT_FALSE(nh->version != NETCP_VERSION))
            {
              b0->error = node->errors[NETCP_ERROR_BAD_VERSION];
              goto trace0;
            }
          if (PREDICT_FALSE(nh->type >= NETCP_N_TYPES))
            {
              b0->error = node->errors[NETCP_ERROR_BAD_TYPE];
              goto trace0;
            }
          
          fp = netcp_rx_handlers[nh->type];
          next0 = (*fp)(nm, node, b0);
  ...
  vlib_node_increment_counter (vm, netcp_node.index, 
                               NETCP_ERROR_SWAPPED, pkts_swapped);
