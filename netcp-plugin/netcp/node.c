...
typedef struct {
  netcp_header_t header;
  union 
  {
    netcp_send_file_header_t send_file;
    netcp_data_header_t data;
    netcp_ack_header_t ack;
  };
  u32 next_index;
} netcp_trace_t;
...
static u8 * format_netcp_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "NETCP: session-id 0x%x next %s\n", 
              clib_net_to_host_u32(t->header.session_id),
              t->next_index ? "drop" : "ip4-lookup");

  if (t->header.netcp_version != NETCP_VERSION)
    {
      s = format (s, "  illegal version.");
      return s;
    }

  switch (t->header.type)
    {
    case NETCP_TYPE_SEND_FILE:
      s = format 
        (s, "  send: dst file '%s' seg-size %d file-size %lld window-size %d",
         t->send_file.dst_file, clib_net_to_host_u16(t->send_file.segment_size), 
         clib_net_to_host_u64(t->send_file.size_in_bytes), 
         (u32) t->send_file.window_size);
      break;
    case NETCP_TYPE_DATA:
      s = format (s, "  data: offset %lld", 
                  clib_net_to_host_u64(t->data.offset));
      break;
    case NETCP_TYPE_ACK:
      s = format (s, "  ack: offset %lld retval %d", 
                  clib_net_to_host_u64(t->ack.offset),
                  clib_net_to_host_u32(t->ack.retval));
      break;
    default:
      s = format (s, "  illegal type");
      break;
    }
                        ...
                        vlib_buffer_t * b0)
{
  ...
  if (p == 0) 
    {
      ...
      elog (&(nm->vlib_main->elog_main), &e, session_id);
  ...
  CW ("ack: current offset %lld", new_offset);
  ...
  if (s->their_current_offset + nm->segment_size >= s->size_in_bytes)
    {
      f64 now = vlib_time_now(nm->vlib_main);
      f64 delta;
      CW ("ack: done");
      ...
      if (delta == 0.0)
        {
          clib_warning ("delta zero\n");
          delta = 1.0;
        }

      ELOG_TYPE_DECLARE (e) = 
        {
          .format = "send-complete: %.2f bits/sec, session %d",
          .format_args = "f8i4",
        };

      struct { f64 bits_per_sec; u32 session_id;} * ed;
      ed = ELOG_DATA (&(nm->vlib_main->elog_main), e);
      ed->bits_per_sec = (8.0 * ((f64)(s->size_in_bytes))) / delta;
      ed->session_id = session_id;

      clib_warning 
        ("%lld bytes in %.2f seconds, %.2f bytes/sec, %.2f bits/sec",
         s->size_in_bytes, delta,
         (f64)(s->size_in_bytes) / delta,
         (8.0 * ((f64)(s->size_in_bytes))) / delta);
  ...
  if (s->my_current_offset < s->size_in_bytes)
    {
      ...
      vlib_buffer_advance (b0, -(word)(sizeof (*ip0)));
      ...
      ELOG_TYPE_DECLARE (e) = 
        {
          .format = "send-offset: %ld, session-id %d",
          .format_args = "i8i4",
        };
      struct { u64 offset; u32 session_id;} * ed;
      ed = ELOG_DATA (&(nm->vlib_main->elog_main), e);
      ed->offset = s->my_current_offset;
      ed->session_id = s->session_id;
  ...
  else
    {
      ELOG_TYPE_DECLARE (e) = 
        {
          .format = "window-drain: offset %ld session-id %d",
          .format_args = "i8i4",
        };
      struct { u64 offset; u32 session_id;} * ed;
      ed = ELOG_DATA (&(nm->vlib_main->elog_main), e);
      ed->offset = new_offset;
      ed->session_id = s->session_id;
...
#if DELIBERATE_DROP > 0
static u8 drop_one;
static u64 drop_offset = 4398;
#endif
                         ...
                         vlib_buffer_t * b0)
{
...
#if DELIBERATE_DROP > 0
  if (drop_one == 0 && new_offset == drop_offset)
    {
      drop_one = 1;
      CW ("data: deliberate drop at offset %ld", new_offset);
      b0->error = node->errors[NETCP_ERROR_LOST_DATA];
      return NETCP_NEXT_DROP;
    }
#endif

  if (s->my_current_offset + nm->segment_size != new_offset)
    {
      ELOG_TYPE_DECLARE(e) = 
        {
          .format = 
          "lost-data: new offset %ld current offset %ld session %d",
          .format_args = "i8i8i4",
        };
      struct { u64 new_offset; u64 current_offset; u32 session_id;} * ed;
      ed = ELOG_DATA (&(nm->vlib_main->elog_main), e);
      ed->new_offset = new_offset;
      ed->current_offset = s->my_current_offset;
      ed->session_id = session_id;

      CW ("data: lost data new offset %ld my current offset %ld",
          new_offset, s->my_current_offset);
      b0->error = node->errors[NETCP_ERROR_LOST_DATA];
      return NETCP_NEXT_DROP;
    }

  {
    ELOG_TYPE_DECLARE(e) = 
      {
        .format = 
        "new-offset: new offset %ld current offset %ld ok %d",
        .format_args = "i8i8i1",
      };
    struct { u64 new_offset; u64 current_offset; u8 ok;} * ed;
    ed = ELOG_DATA (&(nm->vlib_main->elog_main), e);
    ed->new_offset = new_offset;
    ed->current_offset = s->my_current_offset;
    ed->ok = s->my_current_offset + nm->segment_size < s->size_in_bytes;
  }

  CW ("data: new offset %lld current offset %lld OK %d", 
      new_offset, s->my_current_offset, 
      s->my_current_offset + nm->segment_size < s->size_in_bytes);

  if (s->my_current_offset + nm->segment_size > s->size_in_bytes)
    {
      ELOG_TYPE_DECLARE (e) = 
        {
          .format = "bad_offset: session_id %d",
          .format_args = "i4",
        };
...
send_ack:
  ...
  ack0->offset = clib_host_to_net_u64 (new_offset);
  CW ("send ack: offset %lld", clib_net_to_host_u64(ack0->offset));
  
  {
    ELOG_TYPE_DECLARE(e) = 
      {
        .format = 
        "send-ack: offset %ld session %d",
        .format_args = "i8i4",
      };
    struct { u64 offset; u32 session_id;} * ed;
    ed = ELOG_DATA (&(nm->vlib_main->elog_main), e);
    ed->offset = clib_net_to_host_u64(ack0->offset);
    ed->session_id = session_id;
  }
  ...
  if (s->my_current_offset + nm->segment_size >= s->size_in_bytes 
      && !force_ack)
    {
      ELOG_TYPE_DECLARE (e) = 
        {
          .format = "transfer-complete: session_id %d",
          .format_args = "i4",
        };
      
      elog (&(nm->vlib_main->elog_main), &e, session_id);

      clib_warning ("transfer complete: unmap %s", s->dst_file);
                              ...
                              vlib_buffer_t * b0)
{
  ...
  s->window_size = sf0->window_size;
  s->size_in_bytes = clib_net_to_host_u64(sf0->size_in_bytes);
  s->state = NETCP_STATE_DATA;
  s->my_current_offset = (u64)(- nm->segment_size);
  /* retry case */
  if (s->map_addr == 0)
 ...
 send_ack:
  ...
  /* Point current data at the IP header */
  vlib_buffer_advance (b0, -(word)(sizeof (*ip0)));
  b0->current_length = sizeof (*ip0) + sizeof (*nh0) + sizeof (*ack0);
