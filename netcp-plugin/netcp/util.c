  ...
  if (is_write)
    {
      ...
      rv = mmap (0, map_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
      if (rv == (u8 *)MAP_FAILED)
        rv = 0;
      close(fd);
    }
  else
    {
      ...
      *sizep = statb.st_size;
      ...
      rv = mmap (0, map_size, PROT_READ, MAP_SHARED, fd, 0);
      if (rv == (u8 *)MAP_FAILED)
        rv = 0;
      close(fd);
    }
...
int unmap_file (u8 * filename, u8 * addr, u64 size, int do_truncate)
{
  int rv;
  u64 rnd_size;
