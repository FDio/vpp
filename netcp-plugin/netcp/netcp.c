  ...
  if (total_alloc != send_count)
    {
      ELOG_TYPE_DECLARE (e) = 
        {
          .format = "short alloc: %d",
          .format_args = "i4",
        };
  
      elog (&vm->elog_main, &e, total_alloc);
  ...
  for (i = 0; i < send_count; i++)
    {
      ...
      CW ("send: offset %ld", s->my_current_offset);

      ELOG_TYPE_DECLARE (e) = 
        {
          .format = "send-window-offset: %ld",
          .format_args = "i8",
        };
      struct { u64 offset; } * ed;
        ...
        if (s->retry_count < 5)
          {
            ELOG_TYPE_DECLARE (e) = 
              {
                .format = "retry-session: %d",
                .format_args = "i4",
              };
  
            elog (&(nm->vlib_main->elog_main), &e, s->session_id);
...
static void send_send_file (netcp_main_t * nm, netcp_session_t *s)
{
  ...
  ELOG_TYPE_DECLARE (e) = 
    {
      .format = "send-file: total_size %ld, session id %d",
      .format_args = "i8i4",
    };
  struct { u64 total_size; u32 session_id;} * ed;
  ed = ELOG_DATA (&vm->elog_main, e);
  ed->total_size = s->size_in_bytes;
