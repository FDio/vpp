...
netcp_init (vlib_main_t * vm)
{
  ...
  n = vlib_get_node_by_name (vm, "ip4-lookup");
  ASSERT(n);
  nm->ip4_lookup_index = n->index;

  n = vlib_get_node_by_name (vm, "ip6-lookup");
  ASSERT(n);
...
static void send_window (netcp_main_t * nm, netcp_session_t *)
{
...
static void send_send_file (netcp_main_t * nm, netcp_session_t *s)
{
  ...
  memcpy (sf->dst_filename, s->dst_filename, vec_len(s->dst_filename));
  ...
  if (is_ip4)
    {
      ...
      ipn->ip->ip4.checksum = ip_csum_fold (sum0);
      ipn->ip->ip4.length = new_l0;
    }
  else
    {
