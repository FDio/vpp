...
static void send_window (netcp_main_t * nm, netcp_session_t *s)
{
  ...
  vec_validate (buffers, s->window_size-1);

  nalloc = vlib_buffer_alloc_from_free_list 
    (vm, buffers, s->window_size, VLIB_BUFFER_DEFAULT_FREE_LIST_INDEX);

  /* Timer-based retry if out of buffers... */
  if (nalloc != s->window_size)
    {
  ...
  for (i = 0; i < s->window_size; i++)
    {
      ...
      ip = &ipn->ip;
      nh = &ipn->netcp;
      memcpy (ipn, s->rewrite, vec_len(s->rewrite));
      
      nh->type = NETCP_TYPE_DATA;
      dh = (netcp_data_header_t *)(nh+1);
      b0->current_length += nm->segment_size;
      dh->offset = clib_host_to_net_u64 (s->my_current_offset);
      memcpy (dh->data, s->map_addr + s->my_current_offset, nm->segment_size);
      ip->length = clib_host_to_net_u64(b0->current_length);
...
f64 netcp_send_process_periodic (netcp_main_t * nm)
{
  ...
  ({
    ...
    if (time_remaining_this_session > 0.0)
      {
        if (time_remaining_this_session < poll_time_remaining)
          poll_time_remaining = time_remaining_this_session;
      }
    if (time_remaining_this_session <= 0.0)
      {
        s->my_current_offset = s->their_current_offset;
        s->retry_count++;
...
static void send_send_file (netcp_main_t * nm, netcp_session_t *s)
{
  ...
  memcpy (sf->dst_file, s->dst_file, vec_len(s->dst_file));
