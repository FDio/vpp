  ...
  for (i = 0; i < nmsgs; i++)
    {
      ...
      unserialize_cstring (sm, (char **) &name);
      unserialize_integer (sm, &msg_crc, sizeof (u32));
      name_and_crc = format (0, "%s_%08x%c", name, msg_crc, 0);
      hash_set_mem (am->msg_index_by_name_and_crc, name_and_crc, msg_index);
    }
}

u32
vl_api_get_msg_index (u8 * name, u32 crc)
{
  api_main_t *am = &api_main;
  uword *p;
  ...
  if (am->msg_index_by_name_and_crc)
    {
      ...
      name_and_crc = format (0, "%s_%08x%c", name, crc, 0);
      p = hash_get_mem (am->msg_index_by_name_and_crc, name_and_crc);
      vec_free (name_and_crc);
      if (p)
	return p[0];
    }
  return ~0;
