			 ...
			 vl_api_replay_t which)
{
  ...
  const u32 variable_len_delta =
    offsetof (msgbuf_t, data) - offsetof (msgbuf_t, data_len);
  ...
  for (i = 0; i < first_index; i++)
      ...
      if (is_variable_len)
	{
	  ...
	  msgbuf_t *msgbuf =
	    (msgbuf_t *) (((u8 *) msg) - offsetof (msgbuf_t, data));
...
format_vl_msg_api_trace_status (u8 * s, va_list * args)
{
  ...
  s =
    format (s,
	    "%s: used %d of %d items, %s enabled, %s wrapped, %s allow variable length messages\n",
	    trace_name, vec_len (tp->traces), tp->nitems,
	    tp->enabled ? "is" : "is not",
	    tp->bit_flags & TRACE_FLAG_WRAPPED ? "has" : "has not",
