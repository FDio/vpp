...
openssl_ops_enc_cbc_hmac (vnet_crypto_op_t *ops[], u32 n_ops, clib_thread_index_t thread_index)
{
  ...
  for (i = 0; i < n_ops; i++)
	...
	digest_len = op->digest_len;
      ...
      openssl_key_data_t *kd =
	(openssl_key_data_t *) vnet_crypto_get_simple_key_data_for_thread (op->key, thread_index);
				  ...
				  u32 n_ops, clib_thread_index_t thread_index)
{
  ...
  for (i = 0; i < n_ops; i++)
	...
	digest_len = op->digest_len;
      ...
      openssl_key_data_t *kd =
	(openssl_key_data_t *) vnet_crypto_get_chained_key_data_for_thread (op->key, thread_index);
      enc_ctx = kd->evp_cipher_enc_ctx;
      hmac_ctx = kd->hmac_ctx;
      EVP_EncryptInit_ex (enc_ctx, NULL, NULL, NULL, op->iv);
      HMAC_Init_ex (hmac_ctx, NULL, 0, NULL, NULL);
  ...
  for (i = 0; i < n_ops; i++)
    {
      ...
      openssl_key_data_t *kd =
	chunks ?
	  (openssl_key_data_t *) vnet_crypto_get_chained_key_data_for_thread (op->key,
									      thread_index) :
	  (openssl_key_data_t *) vnet_crypto_get_simple_key_data_for_thread (op->key, thread_index);
      int out_len = 0;
...
openssl_ctx_add_hmac_key_data (vnet_crypto_key_t *key, u8 *key_data, void *ctx __clib_unused)
{
  ...
  openssl_ctx_hmac_add (vnet_crypto_get_cipher_key (key), key->cipher_key_sz + key->integ_key_sz,
			key_data, f ());
}
