			   ...
			   const EVP_CIPHER *cipher, const EVP_MD *md, u32 crypto_key_len)
{
  ...
  cipher_key = key->data + key->cipher_key_offset;
  integ_key = key->data + key->integ_key_offset;
				       ...
				       int is_gcm)
{
  ...
  openssl_ctx_cipher_add (key->data + key->cipher_key_offset, key_data, f (), is_gcm);
}
...
openssl_ctx_add_hmac_group (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  ...
  openssl_ctx_hmac_add (key->data + key->cipher_key_offset, key->cipher_key_sz + key->integ_key_sz,
			key_data, f ());
}
