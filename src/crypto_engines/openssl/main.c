  ...
  for (i = 0; i < n_ops; i++)
    {
      ...
      ctx = ((openssl_key_data_t *) key_data[i])->evp_cipher_enc_ctx;
  ...
  for (i = 0; i < n_ops; i++)
    {
      ...
      ctx = ((openssl_key_data_t *) key_data[i])->evp_cipher_dec_ctx;
...
static_always_inline void openssl_ctx_hmac_add (vnet_crypto_key_t *key,
  ...
  for (i = 0; i < n_ops; i++)
    {
      ...
      ctx = ((openssl_key_data_t *) key_data[i])->hmac_ctx;
      if (ctx == 0)
	{
	  vnet_crypto_key_t *key = vnet_crypto_get_key (op->key_index);
	  openssl_ctx_hmac_add (key, key_data[i], md);
	  ctx = ((openssl_key_data_t *) key_data[i])->hmac_ctx;
	  if (ctx == 0)
	    {
...
openssl_ctx_cipher_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data,
...
static_always_inline void
openssl_ctx_hmac_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data, const EVP_MD *md)
{
  openssl_key_data_t *kd = (openssl_key_data_t *) key_data;
			   ...
			   const EVP_CIPHER *cipher, const EVP_MD *md, u32 crypto_key_len)
{
  ...
  vnet_crypto_key_t h = {
    .data = key->data + crypto_key_len,
  };

  if (key->length < crypto_key_len)
    return;

  k.length = crypto_key_len;
  h.length = key->length - crypto_key_len;

  openssl_ctx_cipher_add (&k, key_data, cipher, 0);
  openssl_ctx_hmac_add (&h, key_data, md);
}
...
#define _(a, b) [VNET_CRYPTO_ALG_HMAC_##a] = b,
  foreach_openssl_hmac_op
#undef _
};

static openssl_cipher_fn_t *openssl_linked_cipher_fn_by_alg[VNET_CRYPTO_N_ALGS] = {
#define _(a, c, h, k, d) [VNET_CRYPTO_ALG_##a] = c,
  foreach_openssl_linked_evp_op
#undef _
};

static openssl_md_fn_t *openssl_linked_md_fn_by_alg[VNET_CRYPTO_N_ALGS] = {
#define _(a, c, h, k, d) [VNET_CRYPTO_ALG_##a] = h,
  foreach_openssl_linked_evp_op
#undef _
};

static u16 openssl_linked_cipher_key_size_by_alg[VNET_CRYPTO_N_ALGS] = {
...
openssl_ctx_add_cipher_group (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  ...
  int is_gcm =
    ((VNET_CRYPTO_ALG_AES_128_GCM <= key->alg) && (VNET_CRYPTO_ALG_AES_256_NULL_GMAC >= key->alg)) ?
      1 :
      0;
  ...
  openssl_ctx_cipher_add (key, key_data, f (), is_gcm);
}
...
static void
openssl_ctx_add_linked_group (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  openssl_cipher_fn_t *cf = openssl_linked_cipher_fn_by_alg[key->alg];
  openssl_md_fn_t *mf = openssl_linked_md_fn_by_alg[key->alg];
  u16 key_size = openssl_linked_cipher_key_size_by_alg[key->alg];
...
static void
openssl_ctx_del_linked_group (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  openssl_ctx_cipher_del (key, key_data);
...
openssl_ctx_add_hmac_group (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  ...
  openssl_ctx_hmac_add (key, key_data, f ());
}
...
#define _(m, a, b, f, l)                                                                           \
  ...
  {                                                                                                \
    return openssl_ops_enc_##m (ops, 0, key_data, n_ops, b (), f, l);                              \
                                                                                                   ...
                                                                                                   \
  u32 openssl_ops_dec_##a (vnet_crypto_op_t *ops[],                                                \
			   vnet_crypto_key_data_t *key_data[] __clib_unused, u32 n_ops)            \
  {                                                                                                \
...
VNET_CRYPTO_REG_OP_GROUP (openssl_linked_group) = {
  .max_key_data_sz = sizeof (openssl_key_data_t),
  .key_data_per_thread = 1,
  .key_add_fn = openssl_ctx_add_linked_group,
  .key_del_fn = openssl_ctx_del_linked_group,
};
