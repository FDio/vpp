...
openssl_ctx_cipher (vnet_crypto_key_op_t kop, void *key_data, const u8 *data,
		    u16 length, const EVP_CIPHER *cipher, int is_gcm)
{
...
static_always_inline void
openssl_ctx_hmac (vnet_crypto_key_op_t kop, void *key_data, const u8 *data,
		  u16 length, const EVP_MD *md)
{
  openssl_per_thread_data_t *ptd = key_data;
  if (VNET_CRYPTO_KEY_OP_ADD == kop)
    {
...
static void
crypto_openssl_key_handler (vnet_crypto_key_op_t kop, void *key_data,
...
#define _(m, a, b, f, l)                                                      \
  ...
  static void openssl_ctx_##a (vnet_crypto_key_op_t kop, void *key_data,      \
			       vnet_crypto_alg_t alg, const u8 *data,         \
			       u16 length)                                    \
  {                                                                           \
    int is_gcm = ((VNET_CRYPTO_ALG_AES_128_GCM <= alg) &&                     \
		  (VNET_CRYPTO_ALG_AES_256_NULL_GMAC >= alg)) ?               \
		   1 :                                                        \
		   0;                                                         \
