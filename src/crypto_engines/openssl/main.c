  ...
  for (i = 0; i < n_ops; i++)
    {
      ...
      openssl_per_thread_data_t *ptd = (u8 *) op->keys + thrd_stride;
...
{
  const u32 thrd_stride = vm->thread_index * stride;
  openssl_per_thread_data_t *ptd = (u8 *) per_thread_data + thrd_stride;
...
openssl_ctx_cipher (vnet_crypto_key_op_t kop, openssl_per_thread_data_t *ptd,
		    const u8 *data, u16 length, const EVP_CIPHER *cipher,
		    int is_gcm)
{
  EVP_CIPHER_CTX *ctx;
  ...
  if (VNET_CRYPTO_KEY_OP_ADD == kop)
    {
      for (u32 i = 0; i < num_threads; i++, ptd = (u8 *) ptd + stride)
	{
...
static_always_inline void
openssl_ctx_hmac (vnet_crypto_key_op_t kop, openssl_per_thread_data_t *ptd,
		  const u8 *data, u16 length, const EVP_MD *md)
{
  if (VNET_CRYPTO_KEY_OP_ADD == kop)
    {
      for (u32 i = 0; i < num_threads; i++, ptd = (u8 *) ptd + stride)
...
#define _(m, a, b, f, l)                                                      \
  ...
  static void *openssl_ctx_##a (                                              \
    vnet_crypto_key_op_t kop, openssl_per_thread_data_t *ptd,                 \
    vnet_crypto_alg_t alg, const u8 *data, u16 length)                        \
  {                                                                           \
    int is_gcm = ((VNET_CRYPTO_ALG_AES_128_GCM <= alg) &&                     \
		  (VNET_CRYPTO_ALG_AES_256_NULL_GMAC >= alg)) ?               \
		   1 :                                                        \
		   0;                                                         \
    return openssl_ctx_cipher (kop, ptd, data, length, b (), is_gcm);         \
  }
...
#define _(a, b)                                                               \
  ...
  static void *openssl_ctx_hmac_##a (                                         \
    vnet_crypto_key_op_t kop, openssl_per_thread_data_t *ptd,                 \
                                                                              \
    vnet_crypto_alg_t alg, const u8 *data, u16 length)                        \
  {                                                                           \
    return openssl_ctx_hmac (kop, ptd, data, length, b ());                   \
  }
...
#undef _
    ...
    {
      openssl_per_thread_data_t *ptd = (u8 *) per_thread_data + i * stride;
