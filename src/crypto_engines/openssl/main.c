...
openssl_ops_enc_cbc_hmac (vnet_crypto_op_t *ops[], u32 n_ops, clib_thread_index_t thread_index)
{
  ...
  for (i = 0; i < n_ops; i++)
	...
	(openssl_combined_key_data_t *) vnet_crypto_get_simple_key_data_for_thread (op->key,
										    thread_index);
...
static_always_inline u32
openssl_ops_hmac_dec_cbc (vnet_crypto_op_t *ops[], u32 n_ops, clib_thread_index_t thread_index)
{
  HMAC_CTX *hmac_ctx;
				  ...
				  u32 n_ops, clib_thread_index_t thread_index)
{
  ...
  for (i = 0; i < n_ops; i++)
	...
	auth_len = op->auth_len;
      ...
      openssl_combined_key_data_t *kd =
	(openssl_combined_key_data_t *) vnet_crypto_get_chained_key_data_for_thread (op->key,
										     thread_index);
      enc_ctx = kd->evp_cipher_enc_ctx;
      hmac_ctx = kd->hmac_ctx;
      EVP_EncryptInit_ex (enc_ctx, NULL, NULL, NULL, op->iv);
		     ...
		     clib_thread_index_t thread_index)
{
  ...
  for (i = 0; i < n_ops; i++)
	...
	chunks ?
	  (openssl_cipher_key_data_t *) vnet_crypto_get_chained_key_data_for_thread (op->key,
										     thread_index) :
	  (openssl_cipher_key_data_t *) vnet_crypto_get_simple_key_data_for_thread (op->key,
										    thread_index);
      int out_len = 0;
		      ...
		      clib_thread_index_t thread_index)
{
  ...
  for (i = 0; i < n_ops; i++)
	...
	EVP_EncryptUpdate (ctx, is_gmac ? 0 : op->dst, &len, op->src, op->len);
      ...
      EVP_CIPHER_CTX_ctrl (ctx, EVP_CTRL_AEAD_GET_TAG, taglen, op->auth);
      op->status = VNET_CRYPTO_OP_STATUS_COMPLETED;
    }
  return n_ops;
}
			   ...
			   clib_thread_index_t thread_index)
{
  return openssl_ops_enc_aead (ops, chunks, n_ops, cipher, /* is_gcm */ 1,
			       /* is_gmac */ 1, fixed, aadlen, thread_index);
}
...
{
  return openssl_ops_enc_aead (ops, chunks, n_ops, cipher, /* is_gcm */ 1,
			       /* is_gmac */ 0, fixed, aadlen, thread_index);
}
		      ...
		      clib_thread_index_t thread_index)
{
  ...
  for (i = 0; i < n_ops; i++)
      ...
      else
	{
      ...
      EVP_CIPHER_CTX_ctrl (ctx, EVP_CTRL_AEAD_SET_TAG, taglen, op->auth);
			   ...
			   clib_thread_index_t thread_index)
{
  return openssl_ops_dec_aead (ops, chunks, n_ops, cipher, /* is_gcm */ 1,
			       /* is_gmac */ 1, fixed, aad_len, thread_index);
}
...
{
  return openssl_ops_dec_aead (ops, chunks, n_ops, cipher, /* is_gcm */ 1,
			       /* is_gmac */ 0, fixed, aad_len, thread_index);
}
		  ...
		  const EVP_MD *md, clib_thread_index_t thread_index)
{
  ...
  for (i = 0; i < n_ops; i++)
	...
	chunks ?
	  (openssl_hmac_key_data_t *) vnet_crypto_get_chained_key_data_for_thread (op->key,
										   thread_index) :
	  (openssl_hmac_key_data_t *) vnet_crypto_get_simple_key_data_for_thread (op->key,
										  thread_index);
...
static_always_inline void
openssl_ctx_cipher_del (vnet_crypto_key_t *key __clib_unused, u8 *key_data)
{
  openssl_cipher_key_data_t *kd = (openssl_cipher_key_data_t *) key_data;
...
static_always_inline void
openssl_ctx_hmac_del (vnet_crypto_key_t *key __clib_unused, u8 *key_data)
{
  openssl_hmac_key_data_t *kd = (openssl_hmac_key_data_t *) key_data;
  HMAC_CTX *ctx = kd->hmac_ctx;
...
static_always_inline void
openssl_ctx_combined_hmac_del (vnet_crypto_key_t *key __clib_unused, u8 *key_data)
{
  openssl_combined_key_data_t *kd = (openssl_combined_key_data_t *) key_data;
  HMAC_CTX *ctx = kd->hmac_ctx;
...
static_always_inline void
openssl_ctx_cipher_md_add (vnet_crypto_key_t *key, u8 *key_data, const EVP_CIPHER *cipher,
			   const EVP_MD *md, u32 crypto_key_len)
{
  ...
  cipher_key = vnet_crypto_get_cipher_key (key);
  auth_key = vnet_crypto_get_auth_key (key);
...
static void
openssl_ctx_add_cipher_group_internal (vnet_crypto_key_t *key, u8 *key_data, int is_gcm)
{
  ...
  openssl_ctx_cipher_add (vnet_crypto_get_cipher_key (key), key_data, f (), is_gcm);
}

static void
openssl_ctx_add_cipher_key_data (vnet_crypto_key_t *key, u8 *key_data, void *ctx)
{
  openssl_ctx_add_cipher_group_internal (key, key_data, pointer_to_uword (ctx));
}

static void
openssl_ctx_del_cipher_key_data (vnet_crypto_key_t *key, u8 *key_data, void *ctx __clib_unused)
{
  openssl_ctx_cipher_del (key, key_data);
}

static void
openssl_ctx_add_combined_key_data (vnet_crypto_key_t *key, u8 *key_data, void *ctx __clib_unused)
{
...
static void
openssl_ctx_del_combined_key_data (vnet_crypto_key_t *key, u8 *key_data, void *ctx __clib_unused)
{
  openssl_ctx_cipher_del (key, key_data);
  openssl_ctx_combined_hmac_del (key, key_data);
}

static void
openssl_ctx_add_hmac_key_data (vnet_crypto_key_t *key, u8 *key_data, void *ctx __clib_unused)
{
  ...
  openssl_ctx_hmac_add (vnet_crypto_get_cipher_key (key), key->cipher_key_sz + key->auth_key_sz,
			key_data, f ());
}

static void
openssl_ctx_del_hmac_key_data (vnet_crypto_key_t *key, u8 *key_data, void *ctx __clib_unused)
{
...
static void
openssl_ctx_change_cipher_group (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  ...
  if (args->action == VNET_CRYPTO_KEY_DATA_ADD || args->action == VNET_CRYPTO_THREAD_KEY_DATA_ADD)
    openssl_ctx_add_cipher_key_data (key, key_data, 0);
  else
    openssl_ctx_del_cipher_key_data (key, key_data, 0);
}

static void
openssl_ctx_change_gcm_group (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  u8 *key_data = openssl_ctx_get_change_data (args);

  if (args->action == VNET_CRYPTO_KEY_DATA_ADD || args->action == VNET_CRYPTO_THREAD_KEY_DATA_ADD)
    openssl_ctx_add_cipher_key_data (key, key_data, uword_to_pointer (1, void *));
  else
    openssl_ctx_del_cipher_key_data (key, key_data, 0);
}

static void
openssl_ctx_change_combined_group (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  u8 *key_data = openssl_ctx_get_change_data (args);

  if (args->action == VNET_CRYPTO_KEY_DATA_ADD || args->action == VNET_CRYPTO_THREAD_KEY_DATA_ADD)
    openssl_ctx_add_combined_key_data (key, key_data, 0);
  else
    openssl_ctx_del_combined_key_data (key, key_data, 0);
}

static void
openssl_ctx_change_hmac_group (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  u8 *key_data = openssl_ctx_get_change_data (args);

  if (args->action == VNET_CRYPTO_KEY_DATA_ADD || args->action == VNET_CRYPTO_THREAD_KEY_DATA_ADD)
    openssl_ctx_add_hmac_key_data (key, key_data, 0);
  else
    openssl_ctx_del_hmac_key_data (key, key_data, 0);
}
...
#define _(m, a, b, f, l)                                                                           \
  static u32 openssl_ops_enc_##a (vnet_crypto_op_t *ops[], u32 n_ops,                              \
				  clib_thread_index_t thread_index)                                \
  {                                                                                                \
    return openssl_ops_enc_##m (ops, 0, n_ops, b (), f, l, thread_index);                          \
  }                                                                                                \
                                                                                                   \
  u32 openssl_ops_dec_##a (vnet_crypto_op_t *ops[], u32 n_ops, clib_thread_index_t thread_index)   \
  {                                                                                                \
    return openssl_ops_dec_##m (ops, 0, n_ops, b (), f, l, thread_index);                          \
  }                                                                                                \
                                                                                                   \
  static u32 openssl_ops_enc_chained_##a (vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, \
					  u32 n_ops, clib_thread_index_t thread_index)             \
  {                                                                                                \
    return openssl_ops_enc_##m (ops, chunks, n_ops, b (), f, l, thread_index);                     \
  }                                                                                                \
                                                                                                   \
  static u32 openssl_ops_dec_chained_##a (vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, \
					  u32 n_ops, clib_thread_index_t thread_index)             \
  {                                                                                                \
    return openssl_ops_dec_##m (ops, chunks, n_ops, b (), f, l, thread_index);                     \
  }
...
#define _(a, c, h, k, d)                                                                           \
  static u32 openssl_ops_enc_##a (vnet_crypto_op_t *ops[], u32 n_ops,                              \
				  clib_thread_index_t thread_index)                                \
  {                                                                                                \
    return openssl_ops_enc_cbc_hmac (ops, n_ops, thread_index);                                    \
  }                                                                                                \
                                                                                                   \
  u32 openssl_ops_dec_##a (vnet_crypto_op_t *ops[], u32 n_ops, clib_thread_index_t thread_index)   \
  {                                                                                                \
    return openssl_ops_hmac_dec_cbc (ops, n_ops, thread_index);                                    \
  }                                                                                                \
                                                                                                   \
  static u32 openssl_ops_enc_chained_##a (vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, \
					  u32 n_ops, clib_thread_index_t thread_index)             \
  {                                                                                                \
    return openssl_ops_enc_cbc_hmac_chained (ops, chunks, n_ops, thread_index);                    \
  }                                                                                                \
                                                                                                   \
  static u32 openssl_ops_dec_chained_##a (vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, \
					  u32 n_ops, clib_thread_index_t thread_index)             \
  {                                                                                                \
    return openssl_ops_hmac_dec_cbc_chained (ops, chunks, n_ops, thread_index);                    \
  }
