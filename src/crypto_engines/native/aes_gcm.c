...
aes_ops_enc_aes_gcm (vnet_crypto_op_t *ops[], u32 n_ops, aes_key_size_t ks, u32 fixed, u32 aad_len,
...
next:
  kd = (aes_gcm_key_data_t *) vnet_crypto_get_simple_key_data (op->key, 0);
  aes_gcm (op->src, op->dst, op->aad, (u8 *) op->iv, op->auth, op->len,
	   fixed ? aad_len : op->aad_len, fixed ? 16 : op->auth_len, kd, AES_KEY_ROUNDS (ks),
	   AES_GCM_OP_ENCRYPT);
...
static_always_inline u32
aes_ops_dec_aes_gcm (vnet_crypto_op_t *ops[], u32 n_ops, aes_key_size_t ks, u32 fixed, u32 aad_len,
		     clib_thread_index_t thread_index)
{
  vnet_crypto_op_t *op = ops[0];
  aes_gcm_key_data_t *kd;
...
aes_gcm_key_exp (vnet_crypto_key_t *key, u8 *key_data, aes_key_size_t ks)
{
  clib_aes_gcm_key_expand ((aes_gcm_key_data_t *) key_data, vnet_crypto_get_cipher_key (key), ks);
}
...
#define _(x)                                                                                       \
  static u32 aes_ops_dec_aes_gcm_##x (vnet_crypto_op_t *ops[], u32 n_ops,                          \
				      clib_thread_index_t thread_index)                            \
  {                                                                                                \
    return aes_ops_dec_aes_gcm (ops, n_ops, AES_KEY_##x, 0, 0, thread_index);                      \
  }                                                                                                \
  static u32 aes_ops_enc_aes_gcm_##x (vnet_crypto_op_t *ops[], u32 n_ops,                          \
				      clib_thread_index_t thread_index)                            \
  {                                                                                                \
    return aes_ops_enc_aes_gcm (ops, n_ops, AES_KEY_##x, 0, 0, thread_index);                      \
  }                                                                                                \
  static u32 aes_ops_dec_aes_gcm_##x##_tag16_aad8 (vnet_crypto_op_t *ops[], u32 n_ops,             \
						   clib_thread_index_t thread_index)               \
  {                                                                                                \
    return aes_ops_dec_aes_gcm (ops, n_ops, AES_KEY_##x, 1, 8, thread_index);                      \
  }                                                                                                \
  static u32 aes_ops_enc_aes_gcm_##x##_tag16_aad8 (vnet_crypto_op_t *ops[], u32 n_ops,             \
						   clib_thread_index_t thread_index)               \
  {                                                                                                \
    return aes_ops_enc_aes_gcm (ops, n_ops, AES_KEY_##x, 1, 8, thread_index);                      \
  }                                                                                                \
  static u32 aes_ops_dec_aes_gcm_##x##_tag16_aad12 (vnet_crypto_op_t *ops[], u32 n_ops,            \
						    clib_thread_index_t thread_index)              \
  {                                                                                                \
    return aes_ops_dec_aes_gcm (ops, n_ops, AES_KEY_##x, 1, 12, thread_index);                     \
  }                                                                                                \
  static u32 aes_ops_enc_aes_gcm_##x##_tag16_aad12 (vnet_crypto_op_t *ops[], u32 n_ops,            \
						    clib_thread_index_t thread_index)              \
  {                                                                                                \
    return aes_ops_enc_aes_gcm (ops, n_ops, AES_KEY_##x, 1, 12, thread_index);                     \
  }
...
static_always_inline void
aes_gcm_key_add (vnet_crypto_key_t *key, u8 *key_data, aes_key_size_t ks)
{
  aes_gcm_key_exp (key, key_data, ks);
}

static void
aes_gcm128_key_change (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  u8 *key_data;
...
static void
aes_gcm192_key_change (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  u8 *key_data;
...
static void
aes_gcm256_key_change (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  u8 *key_data;
