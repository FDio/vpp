...
static_always_inline u32
crypto_native_ops_hmac_sha2_threadsafe (vlib_main_t *vm,
  ...
  for (; n_left; n_left--, op++)
    {
      ...
      clib_sha2_hmac_key_data (type, op->key, op->key_length, kd);
      clib_sha2_hmac_init (&ctx, type, (clib_sha2_hmac_key_data_t *) kd);
      if (op->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS)
	{
	  vnet_crypto_op_chunk_t *chp = chunks + op->chunk_index;
	  for (int j = 0; j < op->n_chunks; j++, chp++)
