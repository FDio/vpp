...
sha2_key_change_handler (vnet_crypto_ctx_t *ctx, vnet_crypto_key_change_args_t *args)
{
  ...
  if (ctx->alg != VNET_CRYPTO_ALG_SHA2_224 && ctx->alg != VNET_CRYPTO_ALG_SHA2_256 &&
      ctx->alg != VNET_CRYPTO_ALG_SHA2_256_ICV12 && ctx->alg != VNET_CRYPTO_ALG_SHA2_256_ICV16)
    return;

  if (ctx->alg == VNET_CRYPTO_ALG_SHA2_224)
    sha2_key_add (ctx, key_data, CLIB_SHA2_224);
  else
    sha2_key_add (ctx, key_data, CLIB_SHA2_256);
}

VNET_CRYPTO_REGISTER_ALG_GROUP (native_sha2_group) = {
  .probe_fn = sha2_probe,
  .max_key_data_sz = sizeof (clib_sha2_hmac_key_data_t),
  .key_change_fn = sha2_key_change_handler,
};

VNET_CRYPTO_REGISTER_HASH_ALG_GROUP (native_hash_group) = {
  .probe_fn = sha2_probe,
};
...
#define _(n, b)                                                                                    \
  ...
  VNET_CRYPTO_REGISTER_HASH_ALG (crypto_native_hash_sha##b) = {                                    \
    .group = &native_hash_group,                                                                   \
    .alg_id = VNET_CRYPTO_HASH_ALG_##n,                                                            \
    .simple_fn = crypto_native_ops_hash_sha##b,                                                    \
    .chained_fn = crypto_native_ops_chained_hash_sha##b,                                           \
  };                                                                                               \
  VNET_CRYPTO_REGISTER_ALG (crypto_native_hmac_sha##b) = {                                              \
