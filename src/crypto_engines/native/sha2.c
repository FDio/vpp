...
{
  clib_sha2_hmac_key_data (type, key->data, key->length, (clib_sha2_hmac_key_data_t *) key_data);
}

static void
sha2_key_add_handler (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  if (key->alg != VNET_CRYPTO_ALG_HMAC_SHA224 && key->alg != VNET_CRYPTO_ALG_HMAC_SHA256)
    return;

  if (key->alg == VNET_CRYPTO_ALG_HMAC_SHA224)
