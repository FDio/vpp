...
static_always_inline u32
crypto_native_ops_hmac_sha2 (vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops,
			     vnet_crypto_op_chunk_t *chunks, clib_sha2_type_t type)
{
  ...
  for (; n_left; n_left--, op++)
	...
	(crypto_native_per_thread_data_t *) ((u8 *) op->keys + thrd_stride);
      ...
      clib_sha2_hmac_init (&ctx, type, (clib_sha2_hmac_key_data_t *) ptd->hmac_key_data);
      if (op->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS)
	{
...
static void
sha2_key_handler (vnet_crypto_key_op_t kop, crypto_native_per_thread_data_t *ptd, const u8 *data,
		  u16 length, clib_sha2_type_t type)
{
  ...
  if (kop == VNET_CRYPTO_KEY_OP_DEL)
	   ...
	   i++, ptd = (crypto_native_per_thread_data_t *) ((u8 *) ptd + cm->stride))
	{
	  ...
	  ptd->hmac_key_data =
	    clib_mem_alloc_aligned (sizeof (clib_sha2_hmac_key_data_t), CLIB_CACHE_LINE_BYTES);
...
#define _(b)                                                                                       \
  ...
  {                                                                                                \
    return crypto_native_ops_hmac_sha2 (vm, ops, n_ops, chunks, CLIB_SHA2_##b);                    \
                                                                                                   ...
                                                                                                   \
  static void sha2_##b##_key_handler (                                                             \
    vnet_crypto_key_op_t kop, crypto_native_per_thread_data_t *ptd, const u8 *data, u16 length)    \
  {                                                                                                \
