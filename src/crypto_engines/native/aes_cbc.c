...
{
  return (aes_cbc_key_data_t *) vnet_crypto_get_simple_key_data (op->key, 0);
}
...
{
  return (aes_cbc_hmac_key_data_t *) vnet_crypto_get_simple_key_data (op->key, 0);
}
			  ...
			  clib_sha2_type_t type, clib_thread_index_t thread_index)
{
  ...
  while (n_left)
	...
	{
	  u32 auth_len = vnet_crypto_get_op_data (h_ops->op)->auth_len;

	  if (auth_len == 0)
	    auth_len = h_ops->auth_len;

	  clib_sha2_hmac_update (&ctx[j], h_ops->auth_src, h_ops->auth_src_len);
	  clib_sha2_hmac_final (&ctx[j], buffer);
	  clib_memcpy_fast (h_ops->auth, buffer, auth_len);
	}
    }
  return n_ops;
}
...
decrypt:
  auth_len = vnet_crypto_get_op_data (op->op)->auth_len;
  if (auth_len == 0)
...
aes_cbc128_key_change (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  ...
  aes_cbc_key_exp (key, key_data, AES_KEY_128);
}
...
aes_cbc192_key_change (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  ...
  if (args->action != VNET_CRYPTO_KEY_DATA_ADD)
    return;
  key_data = args->key_data;
  aes_cbc_key_exp (key, key_data, AES_KEY_192);
}
...
aes_cbc256_key_change (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  ...
  if (args->action != VNET_CRYPTO_KEY_DATA_ADD)
    return;
  key_data = args->key_data;
  aes_cbc_key_exp (key, key_data, AES_KEY_256);
}
			     ...
			     clib_sha2_type_t type)
{
  ...
  clib_sha2_hmac_key_data (type, vnet_crypto_get_auth_key (key), integ_len, &kd->integ_key);
}
