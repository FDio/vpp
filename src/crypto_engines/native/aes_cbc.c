			  ...
			  clib_sha2_type_t type, u32 auth_len, clib_thread_index_t thread_index)
{
  ...
  while (n_left)
	...
	{
	  u32 len = auth_len ? auth_len : h_ops->auth_len;

	  clib_sha2_hmac_update (&ctx[j], h_ops->auth_src, h_ops->auth_src_len);
	  clib_sha2_hmac_final (&ctx[j], buffer);
	  clib_memcpy_fast (h_ops->auth, buffer, len);
	}
    }
  return n_ops;
}
...
#define _(a, b, c)                                                                                   \
  ...
  VNET_CRYPTO_REG_ALG (aes_##a##_cbc_hmac_sha##b) = {                                              \
    ...
    .alg_id = VNET_CRYPTO_ALG_AES_##a##_CBC_SHA##b,                                                \
    .simple = {                                                                                    \
