			  ...
			  aes_key_size_t ks, clib_sha2_type_t type)
{
  ...
  while (n_left)
	...
	{
	  ptd = (crypto_native_per_thread_data_t *) ((u8 *) ops[0]->keys + thrd_stride);
...
static void
aes_cbc_key_exp (vnet_crypto_key_op_t kop, crypto_native_per_thread_data_t *ptd, const u8 *data,
		 aes_key_size_t ks)
{
  crypto_native_main_t *cm = &crypto_native_main;
  ...
  if (kop == VNET_CRYPTO_KEY_OP_DEL)
	   ...
	   i++, ptd = (crypto_native_per_thread_data_t *) ((u8 *) ptd + cm->stride))
	{
	  if (ptd->crypto_key_data)
	    clib_mem_free_s (ptd->crypto_key_data);
	  ptd->crypto_key_data =
	    clib_mem_alloc_aligned (sizeof (aes_cbc_key_data_t), CLIB_CACHE_LINE_BYTES);
...
#define _(x)                                                                                       \
  ...
  CRYPTO_NATIVE_OP_HANDLER (aes_##x##_cbc_dec) = {                                                 \
    ...
    .probe = aes_cbc_cpu_probe,                                                                    \
                                                                                                   ...
                                                                                                   \
  static void aes_cbc_key_exp_##x (vnet_crypto_key_op_t kop, crypto_native_per_thread_data_t *ptd, \
