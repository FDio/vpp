...
aes_ops_enc_aes_cbc_hmac (vnet_crypto_op_t *ops[], vnet_crypto_key_data_t *key_data[], u32 n_ops,
			  aes_key_size_t ks, clib_sha2_type_t type)
{
  ...
  while (n_left)
	...
	{
	  aes_cbc_hmac_key_data_t *kd = (aes_cbc_hmac_key_data_t *) key_data[0];
...
static_always_inline u32
aes_ops_hmac_dec_aes_cbc (vnet_crypto_op_t *ops[], vnet_crypto_key_data_t *key_data[], u32 n_ops,
			  aes_key_size_t ks, clib_sha2_type_t type)
{
  ...
  if (--n_left)
    {
      op += 1;
      key_data += 1;
      kd = (aes_cbc_hmac_key_data_t *) key_data[0];
      goto decrypt;
    }
...
static_always_inline u32
aes_ops_enc_aes_cbc (vnet_crypto_op_t *ops[], vnet_crypto_key_data_t *key_data[], u32 n_ops,
		     aes_key_size_t ks)
{
  u32 i, n_left = n_ops;
  uword key_indices[CRYPTO_NATIVE_AES_CBC_ENC_VEC_SIZE] = {};
  ...
  while (n_left)
      ...
      while (n_left && i < CRYPTO_NATIVE_AES_CBC_ENC_VEC_SIZE)
	{
      ...
      clib_aes_cbc_encrypt_multi ((aes_cbc_key_data_t **) key_data, key_indices, plaintext, oplen,
				  iv, ks, ciphertext, i);
      key_data += CRYPTO_NATIVE_AES_CBC_ENC_VEC_SIZE;
    }
  return n_ops;
}

static_always_inline u32
aes_ops_dec_aes_cbc (vnet_crypto_op_t *ops[], vnet_crypto_key_data_t *key_data[], u32 n_ops,
		     aes_key_size_t ks)
{
  int rounds = AES_KEY_ROUNDS (ks);
  vnet_crypto_op_t *op = ops[0];
  aes_cbc_key_data_t *kd = (aes_cbc_key_data_t *) key_data[0];
  u32 n_left = n_ops;
  ...
  if (--n_left)
    {
      op += 1;
      key_data += 1;
      kd = (aes_cbc_key_data_t *) key_data[0];
      goto decrypt;
    }
...
aes_cbc_key_exp (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data, aes_key_size_t ks)
{
  aes_cbc_key_data_t *kd = (aes_cbc_key_data_t *) key_data;
  if (ks == AES_KEY_128)
    clib_aes128_cbc_key_expand (kd, vnet_crypto_get_cypher_key (key));
  else if (ks == AES_KEY_192)
    clib_aes192_cbc_key_expand (kd, vnet_crypto_get_cypher_key (key));
  else
    clib_aes256_cbc_key_expand (kd, vnet_crypto_get_cypher_key (key));
}

static_always_inline void
aes_cbc_key_add_inline (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data, aes_key_size_t ks)
{
...
static void
aes_cbc128_key_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  aes_cbc_key_add_inline (key, key_data, AES_KEY_128);
}

static void
aes_cbc192_key_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  aes_cbc_key_add_inline (key, key_data, AES_KEY_192);
}

static void
aes_cbc256_key_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  aes_cbc_key_add_inline (key, key_data, AES_KEY_256);
}

static_always_inline void
aes_cbc_sha2_key_add_inline (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data,
			     aes_key_size_t ks, clib_sha2_type_t type)
{
  ...
  aes_cbc_key_exp (key, (vnet_crypto_key_data_t *) &kd->crypto_key, ks);
  clib_sha2_hmac_key_data (type, vnet_crypto_get_integ_key (key), integ_len, &kd->integ_key);
}

static void
aes_cbc128_sha224_key_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  aes_cbc_sha2_key_add_inline (key, key_data, AES_KEY_128, CLIB_SHA2_224);
}

static void
aes_cbc192_sha224_key_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  aes_cbc_sha2_key_add_inline (key, key_data, AES_KEY_192, CLIB_SHA2_224);
}

static void
aes_cbc256_sha224_key_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  aes_cbc_sha2_key_add_inline (key, key_data, AES_KEY_256, CLIB_SHA2_224);
}

static void
aes_cbc128_sha256_key_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  aes_cbc_sha2_key_add_inline (key, key_data, AES_KEY_128, CLIB_SHA2_256);
}

static void
aes_cbc192_sha256_key_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
  aes_cbc_sha2_key_add_inline (key, key_data, AES_KEY_192, CLIB_SHA2_256);
}

static void
aes_cbc256_sha256_key_add (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data)
{
...
VNET_CRYPTO_REG_OP_GROUP (native_cbc128_sha224_group) = {
  ...
  .key_add_fn = aes_cbc128_sha224_key_add,
};

VNET_CRYPTO_REG_OP_GROUP (native_cbc192_sha224_group) = {
  .probe_fn = aes_cbc_sha2_probe,
  .max_key_data_sz = sizeof (aes_cbc_hmac_key_data_t),
  .key_add_fn = aes_cbc192_sha224_key_add,
};

VNET_CRYPTO_REG_OP_GROUP (native_cbc256_sha224_group) = {
  .probe_fn = aes_cbc_sha2_probe,
  .max_key_data_sz = sizeof (aes_cbc_hmac_key_data_t),
  .key_add_fn = aes_cbc256_sha224_key_add,
};

VNET_CRYPTO_REG_OP_GROUP (native_cbc128_sha256_group) = {
  .probe_fn = aes_cbc_sha2_probe,
  .max_key_data_sz = sizeof (aes_cbc_hmac_key_data_t),
  .key_add_fn = aes_cbc128_sha256_key_add,
};

VNET_CRYPTO_REG_OP_GROUP (native_cbc192_sha256_group) = {
  .probe_fn = aes_cbc_sha2_probe,
  .max_key_data_sz = sizeof (aes_cbc_hmac_key_data_t),
  .key_add_fn = aes_cbc192_sha256_key_add,
};

VNET_CRYPTO_REG_OP_GROUP (native_cbc256_sha256_group) = {
  .probe_fn = aes_cbc_sha2_probe,
  .max_key_data_sz = sizeof (aes_cbc_hmac_key_data_t),
  .key_add_fn = aes_cbc256_sha256_key_add,
};

#define foreach_aes_cbc_handler_type _ (128) _ (192) _ (256)

#define _(x)                                                                                       \
  static u32 aes_ops_enc_aes_cbc_##x (vnet_crypto_op_t *ops[], vnet_crypto_key_data_t *key_data[], \
				      u32 n_ops)                                                   \
  {                                                                                                \
    return aes_ops_enc_aes_cbc (ops, key_data, n_ops, AES_KEY_##x);                                \
  }                                                                                                \
                                                                                                   \
  VNET_CRYPTO_REG_OP (aes_##x##_cbc_enc) = {                                                       \
    ...
    .fn = aes_ops_enc_aes_cbc_##x,                                                                 \
                                                                                                   ...
                                                                                                   \
  static u32 aes_ops_dec_aes_cbc_##x (vnet_crypto_op_t *ops[], vnet_crypto_key_data_t *key_data[], \
				      u32 n_ops)                                                   \
  {                                                                                                \
    return aes_ops_dec_aes_cbc (ops, key_data, n_ops, AES_KEY_##x);                                \
  }                                                                                                \
                                                                                                   \
...
#define _(a, b, c)                                                                                 \
  ...
  {                                                                                                \
    return aes_ops_enc_aes_cbc_hmac (ops, key_data, n_ops, AES_KEY_##a, CLIB_SHA2_##b);            \
  }                                                                                                \
                                                                                                   \
  static u32 crypto_native_ops_dec_aes_cbc_##a##_hmac_sha##b (                                     \
    vnet_crypto_op_t *ops[], vnet_crypto_key_data_t *key_data[], u32 n_ops)                        \
  {                                                                                                \
    return aes_ops_hmac_dec_aes_cbc (ops, key_data, n_ops, AES_KEY_##a, CLIB_SHA2_##b);            \
  }                                                                                                \
                                                                                                   \
