...
aes_ctr_key_exp (vnet_crypto_key_op_t kop,
		 ...
		 aes_key_size_t ks)
{
  ...
  if (kop == VNET_CRYPTO_KEY_OP_DEL)
    {
      for (u32 i = 0; i < cm->num_threads; i++, ptd = (u8 *) ptd + cm->stride)
	{
      ...
      for (u32 i = 0; i < cm->num_threads; i++, ptd = (u8 *) ptd + cm->stride)
	{
	  ...
	  ptd->crypto_key_data = clib_mem_alloc_aligned (
	    sizeof (aes_ctr_key_data_t), CLIB_CACHE_LINE_BYTES);
...
#define _(x)                                                                  \
  ...
  static void aes_ctr_key_exp_##x (vnet_crypto_key_op_t kop,                  \
				   crypto_native_per_thread_data_t *ptd,      \
				   const u8 *data, u16 length)                \
  {                                                                           \
    return aes_ctr_key_exp (kop, ptd, data, AES_KEY_##x);                     \
  }
