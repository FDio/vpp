...
#define _(b)                                                                                       \
  ...
  static void aes_ctr_key_exp_##b (vnet_crypto_key_op_t kop, void *key_data, const u8 *data,       \
				   u16 length)                                                     \
  {                                                                                                \
    return aes_ctr_key_exp (kop, key_data, data, AES_KEY_##b);                                     \
  }                                                                                                \
...
#define _(a, b, c, d)                                                                              \
  static u32 crypto_native_ops_enc_aes_ctr_##a##_hmac_sha##b (vlib_main_t *vm,                     \
							      vnet_crypto_op_t *ops[], u32 n_ops)  \
  {                                                                                                \
    return aes_ops_enc_aes_ctr_hmac (vm, ops, n_ops, AES_KEY_##a, CLIB_SHA2_##b);                  \
  }                                                                                                \
                                                                                                   \
  static u32 crypto_native_ops_enc_aes_ctr_##a##_hmac_sha##b##_chained (                           \
    vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops)           \
  {                                                                                                \
    return aes_ops_enc_aes_ctr_hmac_chained (vm, ops, n_ops, chunks, AES_KEY_##a, CLIB_SHA2_##b);  \
  }                                                                                                \
  static u32 crypto_native_ops_dec_aes_ctr_##a##_hmac_sha##b (vlib_main_t *vm,                     \
							      vnet_crypto_op_t *ops[], u32 n_ops)  \
  {                                                                                                \
    return aes_ops_dec_aes_ctr_hmac (vm, ops, n_ops, AES_KEY_##a, CLIB_SHA2_##b);                  \
  }                                                                                                \
  static u32 crypto_native_ops_dec_aes_ctr_##a##_hmac_sha##b##_chained (                           \
    vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops)           \
  {                                                                                                \
    return aes_ops_dec_aes_ctr_hmac_chained (vm, ops, n_ops, chunks, AES_KEY_##a, CLIB_SHA2_##b);  \
  }                                                                                                \
  ...
  CRYPTO_NATIVE_OP_HANDLER (aes_##a##_ctr_hmac_sha##b##_dec) = {                                   \
    ...
    .probe = aes_ctr_sha2_probe,                                                                   \
                                                                                                   ...
                                                                                                   \
  static void aes_ctr_key_exp_##a##_hmac_sha##b (vnet_crypto_key_op_t kop, void *key_data,         \
						 const u8 *data, u16 length)                       \
  {                                                                                                \
    aes_ctr_hmac_key_exp (kop, key_data, data, length, d, AES_KEY_##a, CLIB_SHA2_##b);             \
  }                                                                                                \
                                                                                                   \
