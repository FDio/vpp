...
ipsecmb_retire_hmac_job (IMB_JOB *job, u32 *n_fail, u32 digest_size)
{
  ...
  u32 len = vnet_crypto_get_op_data (op->op)->auth_len;

  if (len == 0)
    len = op->auth_len ? op->auth_len : digest_size;
			 ...
			 IMB_HASH_ALG alg, u32 key_data_offset)
{
  ...
  while (n_ops)
      ...
      for (i = 0; i < n; i++, ops_index++)
	{
	  ...
	  const u8 *kd = vnet_crypto_get_simple_key_data (op->key, 0) + key_data_offset;
			       ...
			       IMB_CIPHER_DIRECTION direction, IMB_CIPHER_MODE cipher_mode)
{
  ...
  while (n_ops)
      ...
      for (i = 0; i < n; i++)
	{
	  ...
	  vnet_crypto_op_t *op = ops[ops_index++];
	  kd = (ipsecmb_aes_key_data_t *) vnet_crypto_get_simple_key_data (op->key, 0);
		 ...
		 ipsecmb_ops_gcm_args_t a)
{
  ...
  for (i = 0; i < n_ops; i++)
      ...
      if (!a.fixed)
	{
      ...
      kd = (struct gcm_key_data *) vnet_crypto_get_chained_key_data (op->key, 0);
      if (!a.chained)
	kd = (struct gcm_key_data *) vnet_crypto_get_simple_key_data (op->key, 0);
      if (a.chained)
	{
			 ...
			 u32 aad_len)
{
  ...
  while (n_ops)
      ...
      for (i = 0; i < n; i++, ops_index++)
	{
	  ...
	  key = vnet_crypto_get_simple_key_data (op->key, 0);
				   ...
				   u32 n_ops, IMB_CIPHER_DIRECTION dir, u32 fixed, u32 aad_len)
{
  ...
  if (dir == IMB_DIR_ENCRYPT)
      ...
      for (i = 0; i < n_ops; i++)
	{
	  ...
	  u8 *key = vnet_crypto_get_chained_key_data (op->key, 0);
	  ...
	  IMB_CHACHA20_POLY1305_ENC_FINALIZE (m, &ctx, op->auth, fixed ? 16 : op->auth_len);
      ...
      for (i = 0; i < n_ops; i++)
	{
	  ...
	  IMB_CHACHA20_POLY1305_DEC_FINALIZE (m, &ctx, scratch, fixed ? 16 : op->auth_len);

	  if (memcmp (op->auth, scratch, fixed ? 16 : op->auth_len))
	    {
	      n_fail = n_fail + 1;
	      op->status = VNET_CRYPTO_OP_STATUS_FAIL_BAD_HMAC;
	    }
				      ...
				      u32 block_size, u32 data_size)
{
...

  {
    ...
    if (key->cipher_key_sz + key->auth_key_sz <= block_size)
      clib_memcpy_fast (key_hash, vnet_crypto_get_cipher_key (key),
			key->cipher_key_sz + key->auth_key_sz);
    else
      hash_fn (vnet_crypto_get_cipher_key (key), key->cipher_key_sz + key->auth_key_sz, key_hash);
...
{
  u8 *key_data;
  u32 sz = key->cipher_key_sz + key->auth_key_sz;
					...
					vnet_crypto_alg_t cipher_alg, vnet_crypto_alg_t integ_alg)
{
  ...
  if (key->auth_key_sz)
    key_data_len = key->auth_key_offset + key->auth_key_sz;
  ...
  k->cipher_key_offset = k->auth_key_offset;
  k->cipher_key_sz = k->auth_key_sz;
  k->auth_key_offset = 0;
  k->auth_key_sz = 0;
