...
ipsecmb_retire_hmac_job (IMB_JOB *job, u32 *n_fail, u32 digest_size)
{
  ...
  u32 len = vnet_crypto_get_op_data (op->op)->digest_len;

  if (len == 0)
    len = op->digest_len ? op->digest_len : digest_size;
...
ipsecmb_ops_hmac_inline (vnet_crypto_op_t *ops[], u32 n_ops, u32 block_size, u32 hash_size,
			 u32 digest_size, IMB_HASH_ALG alg, u32 key_data_offset)
{
  ...
  while (n_ops)
      ...
      for (i = 0; i < n; i++, ops_index++)
	{
	  ...
	  const u8 *kd = vnet_crypto_get_simple_key_data (op->key) + key_data_offset;
...
#define _(a, b, c, d, e, f)                                                                        \
  ...
  {                                                                                                \
    return ipsecmb_ops_hmac_inline (ops, n_ops, d, e, f, IMB_AUTH_HMAC_##b, 0);                    \
  }
			       ...
			       IMB_CIPHER_DIRECTION direction, IMB_CIPHER_MODE cipher_mode)
{
  ...
  while (n_ops)
      ...
      for (i = 0; i < n; i++)
	{
	  ...
	  vnet_crypto_op_t *op = ops[ops_index++];
	  kd = (ipsecmb_aes_key_data_t *) vnet_crypto_get_simple_key_data (op->key);
			  ...
			  u32 digest_size, IMB_HASH_ALG hash_alg)
{
  ...
  n_success = ipsecmb_ops_hmac_inline (hmac_ops, n_hmac_ops, block_size, hash_size, digest_size,
				       hash_alg, sizeof (ipsecmb_aes_key_data_t));
			  ...
			  u32 digest_size, IMB_HASH_ALG hash_alg)
{
  ...
  if (ipsecmb_ops_hmac_inline (hmac_ops, n_hmac_ops, block_size, hash_size, digest_size, hash_alg,
...
#define _(a, b, c, j, d, e, f, g, h, i)                                                            \
  static_always_inline u32 ipsecmb_ops_combined_enc_##a (vnet_crypto_op_t *ops[], u32 n_ops,       \
							 clib_thread_index_t thread_index)         \
  {                                                                                                \
    return ipsecmb_ops_combined_enc (ops, n_ops, b, IMB_CIPHER_##c, f, g, h, IMB_AUTH_HMAC_##d);   \
  }                                                                                                \
                                                                                                   \
  static_always_inline u32 ipsecmb_ops_combined_dec_##a (vnet_crypto_op_t *ops[], u32 n_ops,       \
							 clib_thread_index_t thread_index)         \
  {                                                                                                \
		 ...
		 ipsecmb_ops_gcm_args_t a)
{
  ...
  for (i = 0; i < n_ops; i++)
      ...
      if (!a.fixed)
	{
      ...
      kd = (struct gcm_key_data *) vnet_crypto_get_chained_key_data (op->key);
      if (!a.chained)
	kd = (struct gcm_key_data *) vnet_crypto_get_simple_key_data (op->key);
      if (a.chained)
	{
...
#define _(a, b, f, l)                                                                              \
  static_always_inline u32 ipsecmb_ops_gcm_cipher_enc_##a (                                        \
    vnet_crypto_op_t *ops[], u32 n_ops, clib_thread_index_t thread_index __clib_unused)            \
  {                                                                                                \
			 ...
			 u32 aad_len)
{
  ...
  while (n_ops)
      ...
      for (i = 0; i < n; i++, ops_index++)
	{
	  ...
	  key = vnet_crypto_get_simple_key_data (op->key);
	  ...
	  job->auth_tag_output = dir == IMB_DIR_ENCRYPT ? op->tag : scratch[i];
...
static_always_inline u32
ipsecmb_ops_chacha_poly_enc (vnet_crypto_op_t *ops[], u32 n_ops, clib_thread_index_t thread_index)
{
  return ipsecmb_ops_chacha_poly (ops, n_ops, IMB_DIR_ENCRYPT, 0, 0);
}

static_always_inline u32
ipsecmb_ops_chacha_poly_dec (vnet_crypto_op_t *ops[], u32 n_ops, clib_thread_index_t thread_index)
{
...
#define _(a)                                                                                       \
  static_always_inline u32 ipsecmb_ops_chacha_poly_tag16_aad##a##_enc (                            \
    vnet_crypto_op_t *ops[], u32 n_ops, clib_thread_index_t thread_index)                          \
  {                                                                                                \
				   ...
				   u32 n_ops, IMB_CIPHER_DIRECTION dir, u32 fixed, u32 aad_len)
{
  ...
  if (dir == IMB_DIR_ENCRYPT)
      ...
      for (i = 0; i < n_ops; i++)
	{
	  ...
	  u8 *key = vnet_crypto_get_chained_key_data (op->key);
...
#undef _
  ...
  static void
  crypto_ipsecmb_cipher_key_add_inline (vnet_crypto_key_t *key, u8 *key_data, keyexp_t keyexp)
{
  if (keyexp == 0)
    return;

  keyexp (vnet_crypto_get_cipher_key (key), ((ipsecmb_aes_key_data_t *) key_data)->enc_key_exp,
	  ((ipsecmb_aes_key_data_t *) key_data)->dec_key_exp);
}

#define _(a, b, c)                                                                                 \
  static void crypto_ipsecmb_cipher_key_change_##a (vnet_crypto_key_t *key,                        \
  ...
  {                                                                                                \
    ...
    if (imbm->mgr == 0)                                                                            \
      return;                                                                                      \
                                                                                                   \
    crypto_ipsecmb_cipher_key_add_inline (key, key_data, imbm->mgr->keyexp_##b);                   \
  }                                                                                                \
                                                                                                   \
  VNET_CRYPTO_REG_OP_GROUP (ipsecmb_cipher_##a##_group) = {                                        \
...
#undef _
  ...
  static void
  crypto_ipsecmb_gcm_key_add_inline (vnet_crypto_key_t *key, u8 *key_data, aes_gcm_pre_t pre)
{
  if (pre == 0)
    return;

  pre (vnet_crypto_get_cipher_key (key), (struct gcm_key_data *) key_data);
}

#define _(a, b, f, l)                                                                              \
  static void crypto_ipsecmb_gcm_key_change_##a (vnet_crypto_key_t *key,                           \
						 vnet_crypto_key_change_args_t *args)              \
  {                                                                                                \
    ipsecmb_main_t *imbm = &ipsecmb_main;                                                          \
    u8 *key_data;                                                                                  \
    ...
    if (imbm->mgr == 0)                                                                            \
      return;                                                                                      \
                                                                                                   \
    crypto_ipsecmb_gcm_key_add_inline (key, key_data, imbm->mgr->gcm##b##_pre);                    \
  }                                                                                                \
                                                                                                   \
  VNET_CRYPTO_REG_OP_GROUP (ipsecmb_gcm_##a##_group) = {                                           \
...
#undef _
  ...
  static void
  crypto_ipsecmb_hmac_key_add_inline (vnet_crypto_key_t *key, u8 *key_data,
				      ...
				      u32 block_size, u32 data_size)
{
...

  {
    ...
    if (key->cipher_key_sz + key->integ_key_sz <= block_size)
      clib_memcpy_fast (key_hash, vnet_crypto_get_cipher_key (key),
			key->cipher_key_sz + key->integ_key_sz);
    else
      hash_fn (vnet_crypto_get_cipher_key (key), key->cipher_key_sz + key->integ_key_sz, key_hash);
...
#define _(a, b, c, d, e, f)                                                                        \
  static_always_inline void crypto_ipsecmb_hmac_key_add_internal_##a (vnet_crypto_key_t *key,      \
								      u8 *key_data)                \
  {                                                                                                \
    ipsecmb_main_t *imbm = &ipsecmb_main;                                                          \
                                                                                                   \
    if (imbm->mgr == 0)                                                                            \
      return;                                                                                      \
                                                                                                   \
    crypto_ipsecmb_hmac_key_add_inline (key, key_data, imbm->mgr->c##_one_block, imbm->mgr->c, d,  \
					e * 2);                                                    \
  }                                                                                                \
                                                                                                   \
  static void crypto_ipsecmb_hmac_key_change_##a (vnet_crypto_key_t *key,                          \
						  vnet_crypto_key_change_args_t *args)             \
  {                                                                                                \
    u8 *key_data;                                                                                  \
    if (args->action != VNET_CRYPTO_KEY_DATA_ADD)                                                  \
      return;                                                                                      \
    key_data = args->key_data;                                                                     \
    crypto_ipsecmb_hmac_key_add_internal_##a (key, key_data);                                      \
  }                                                                                                \
                                                                                                   \
  VNET_CRYPTO_REG_OP_GROUP (ipsecmb_auth_##a##_group) = {                                          \
...
#undef _
  ...
  static void
  crypto_ipsecmb_chacha_poly_key_change (vnet_crypto_key_t *key,
					 vnet_crypto_key_change_args_t *args)
{
  u8 *key_data;
  ...
  if (sz > 32)
    sz = 32;
  clib_memcpy_fast (key_data, vnet_crypto_get_cipher_key (key), sz);
}

static void
crypto_ipsecmb_combined_key_add_inline (vnet_crypto_key_t *key, u8 *key_data, keyexp_t keyexp,
					void (*hmac_key_add_fn) (vnet_crypto_key_t *key,
								 u8 *key_data),
					vnet_crypto_alg_t cipher_alg, vnet_crypto_alg_t integ_alg)
{
  uword key_data_len = key->cipher_key_offset + key->cipher_key_sz;

  if (key->integ_key_sz)
    key_data_len = key->integ_key_offset + key->integ_key_sz;

  u8 key_buf[sizeof (vnet_crypto_key_t) + key_data_len];
  vnet_crypto_key_t *k = (vnet_crypto_key_t *) key_buf;

  clib_memcpy_fast (k, key, sizeof (vnet_crypto_key_t) + key_data_len);
  k->alg = cipher_alg;

  crypto_ipsecmb_cipher_key_add_inline (k, key_data, keyexp);
  k->alg = integ_alg;

  k->cipher_key_offset = k->integ_key_offset;
  k->cipher_key_sz = k->integ_key_sz;
  k->integ_key_offset = 0;
  k->integ_key_sz = 0;

  hmac_key_add_fn (k, key_data + sizeof (ipsecmb_aes_key_data_t));
}

#define _(a, b, c, j, d, e, f, g, h, i)                                                            \
  static void crypto_ipsecmb_combined_key_change_##a (vnet_crypto_key_t *key,                      \
						      vnet_crypto_key_change_args_t *args)         \
  {                                                                                                \
    ipsecmb_main_t *imbm = &ipsecmb_main;                                                          \
    u8 *key_data;                                                                                  \
    ...
    if (imbm->mgr == 0)                                                                            \
      return;                                                                                      \
                                                                                                   \
    crypto_ipsecmb_combined_key_add_inline (key, key_data, imbm->mgr->keyexp_##b,                  \
					    crypto_ipsecmb_hmac_key_add_internal_##e,              \
					    VNET_CRYPTO_ALG_AES_##b##_##j, VNET_CRYPTO_ALG_##e);   \
  }                                                                                                \
                                                                                                   \
  VNET_CRYPTO_REG_OP_GROUP (ipsecmb_combined_##a##_group) = {                                      \
