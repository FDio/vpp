  ...
  IMB_HASH_ALG alg, u32 block_size, u32 hash_size, u32 digest_size)
{
  ...
  while (n_ops)
      ...
      for (i = 0; i < n; i++, ops_index++)
	{
	  vnet_crypto_op_t *op = ops[ops_index];
	  ptd = (ipsecmb_per_thread_data_t *) op->keys;
	  ipsecmb_aes_key_data_t *kd = (ipsecmb_aes_key_data_t *) ptd->crypto_key_data;
				   ...
				   u32 aad_len)
{
  ...
  if (dir == IMB_DIR_ENCRYPT)
      ...
      for (i = 0; i < n_ops; i++)
	{
	  ...
	  ASSERT (op->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS);
	  IMB_CHACHA20_POLY1305_INIT (m, ptd->crypto_key_data, &ctx, op->iv, op->aad,
				      fixed ? aad_len : op->aad_len);

	  chp = chunks + op->chunk_index;
	  for (j = 0; j < op->n_chunks; j++)
	    {
	      IMB_CHACHA20_POLY1305_ENC_UPDATE (m, ptd->crypto_key_data, &ctx, chp->dst, chp->src,
						chp->len);
	      chp += 1;
	    }
      ...
      for (i = 0; i < n_ops; i++)
	{
	  ...
	  for (j = 0; j < op->n_chunks; j++)
	    {
	      IMB_CHACHA20_POLY1305_DEC_UPDATE (m, ptd->crypto_key_data, &ctx, chp->dst, chp->src,
						chp->len);
	      chp += 1;
	    }
			      ...
			      const u8 *data, u16 length)
{
  ...
  if (ad->data_size == 0)
	...
	{
	  if (kop == VNET_CRYPTO_KEY_OP_MODIFY || kop == VNET_CRYPTO_KEY_OP_DEL)
	    clib_mem_free_s (ptd->crypto_key_data);

	  if (kop == VNET_CRYPTO_KEY_OP_MODIFY || kop == VNET_CRYPTO_KEY_OP_ADD)
	    {
	      ptd->crypto_key_data = clib_mem_alloc_aligned (length, CLIB_CACHE_LINE_BYTES);
...
#if IMB_VERSION_NUM >= IMB_VERSION(1, 3, 0)
      clib_memset_u8 (pcd->burst_jobs, 0, sizeof (IMB_JOB) * IMB_MAX_BURST_SIZE);
#endif
