...
#define _(a, b, c, j, d, e, f, g, h, i)                                                            \
  static_always_inline u32 ipsecmb_ops_combined_enc_##a (                                          \
    vnet_crypto_op_t *ops[], vnet_crypto_key_data_t *key_data[], u32 n_ops)                        \
  {                                                                                                \
    return ipsecmb_ops_combined_enc (ops, key_data, n_ops, b, IMB_CIPHER_##c, f, g, h,             \
				     IMB_AUTH_HMAC_##d);                                           \
  }                                                                                                \
                                                                                                   \
  static_always_inline u32 ipsecmb_ops_combined_dec_##a (                                          \
    vnet_crypto_op_t *ops[], vnet_crypto_key_data_t *key_data[], u32 n_ops)                        \
  {                                                                                                \
    return ipsecmb_ops_combined_dec (ops, key_data, n_ops, b, IMB_CIPHER_##c, f, g, h,             \
				     IMB_AUTH_HMAC_##d);                                           \
  }
...
static void
crypto_ipsecmb_cipher_key_add_inline (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data,
				      keyexp_t keyexp)
{
  if (keyexp == 0)
    return;
...
#define _(a, b, c)                                                                                 \
  static void crypto_ipsecmb_cipher_key_add_##a (vnet_crypto_key_t *key,                           \
  ...
  VNET_CRYPTO_REG_OP_GROUP (ipsecmb_cipher_##a##_group) = {                                        \
    ...
    .key_add_fn = crypto_ipsecmb_cipher_key_add_##a,                                               \
...
#undef _
  ...
  static void
  crypto_ipsecmb_gcm_key_add_inline (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data,
				     aes_gcm_pre_t pre)
{
  if (pre == 0)
    return;
...
#define _(a, b, f, l)                                                                              \
  static void crypto_ipsecmb_gcm_key_add_##a (vnet_crypto_key_t *key,                              \
					      vnet_crypto_key_data_t *key_data)                    \
  {                                                                                                \
    ipsecmb_main_t *imbm = &ipsecmb_main;                                                          \
                                                                                                   \
  ...
  VNET_CRYPTO_REG_OP_GROUP (ipsecmb_gcm_##a##_group) = {                                           \
    ...
    .key_add_fn = crypto_ipsecmb_gcm_key_add_##a,                                                  \
    .key_del_fn = crypto_ipsecmb_key_del,                                                          \
  };
foreach_ipsecmb_gcm_cipher_op
#undef _
  ...
  static void
  crypto_ipsecmb_hmac_key_add_inline (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data,
...
#define _(a, b, c, d, e, f)                                                                        \
  static void crypto_ipsecmb_hmac_key_add_##a (vnet_crypto_key_t *key,                             \
					       vnet_crypto_key_data_t *key_data)                   \
  {                                                                                                \
    ipsecmb_main_t *imbm = &ipsecmb_main;                                                          \
                                                                                                   \
  ...
  VNET_CRYPTO_REG_OP_GROUP (ipsecmb_auth_##a##_group) = {                                          \
    .max_key_data_sz = 128,                                                                        \
    .key_add_fn = crypto_ipsecmb_hmac_key_add_##a,                                                 \
    .key_del_fn = crypto_ipsecmb_key_del,                                                          \
  };
foreach_ipsecmb_hmac_op
#undef _
...
static void
crypto_ipsecmb_combined_key_add_inline (vnet_crypto_key_t *key, vnet_crypto_key_data_t *key_data,
					keyexp_t keyexp,
...
#define _(a, b, c, j, d, e, f, g, h, i)                                                            \
  static void crypto_ipsecmb_combined_key_add_##a (vnet_crypto_key_t *key,                         \
						   vnet_crypto_key_data_t *key_data)               \
  {                                                                                                \
    ipsecmb_main_t *imbm = &ipsecmb_main;                                                          \
                                                                                                   \
    ...
    crypto_ipsecmb_combined_key_add_inline (key, key_data, imbm->mgr->keyexp_##b,                  \
					    crypto_ipsecmb_hmac_key_add_##e,                       \
  ...
  VNET_CRYPTO_REG_OP_GROUP (ipsecmb_combined_##a##_group) = {                                      \
    ...
    .key_add_fn = crypto_ipsecmb_combined_key_add_##a,                                             \
    .key_del_fn = crypto_ipsecmb_key_del,                                                          \
  };
foreach_ipsecmb_combined_cipher_op
#undef _
