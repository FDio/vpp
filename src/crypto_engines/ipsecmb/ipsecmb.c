  ...
  if (kop == VNET_CRYPTO_KEY_OP_ADD || kop == VNET_CRYPTO_KEY_OP_MODIFY)
      ...
      if (ad->data_size == 0)
	{
	  ...
	  if (alg == VNET_CRYPTO_ALG_CHACHA20_POLY1305)
	    {
	      ipsecmb_aes_chacha_key_ctx_data_t *ctx = key_data;
	      for (u32 i = 0; i < imbm->num_threads; i++, ctx++, ptd++)
		{
		  ctx->chacha_key_data = (void *) data;
		  ctx->mgr = ptd->mgr;
		}
	    }
#endif
      ...
      if (ad->aes_gcm_pre)
	{
	  ipsecmb_aes_gcm_key_ctx_data_t *ctx = key_data;
	  for (u32 i = 0; i < imbm->num_threads; i++, ctx++, ptd++)
	    {
	      ctx->mgr = ptd->mgr;
	      ad->aes_gcm_pre (data, &ctx->gcm_key);
	    }
	  return;
	}
	...
	{
	  ipsecmb_aes_hmac_key_ctx_data_t *ctx = key_data;
	  for (u32 idx = 0; idx < imbm->num_threads; idx++, ctx++, ptd++)
	    {
	      ctx->ctx = ptd;
	      ad->keyexp (data, ctx->aes_key_data.enc_key_exp, ctx->aes_key_data.dec_key_exp);

	      u32 i;
	      const int block_qw = HMAC_MAX_BLOCK_SIZE / sizeof (u64);
	      u64 pad[block_qw], key_hash[block_qw];
	      data += ad->link_data_size;
		...
		pad[i] = key_hash[i] ^ 0x3636363636363636;
	      ad->hash_one_block (pad, ctx->hmac_key_data);

	      for (i = 0; i < block_qw; i++)
		pad[i] = key_hash[i] ^ 0x5c5c5c5c5c5c5c5c;
	      ad->hash_one_block (pad, ((u8 *) ctx->hmac_key_data) + (ad->data_size / 2));
	    }
      ...
      if (ad->keyexp)
	{
	  ipsecmb_aes_key_ctx_data_t *ctx = key_data;
	  for (u32 i = 0; i < imbm->num_threads; i++, ctx++, ptd++)
	    {
	      /* AES CBC */
	      ctx->ctx = ptd;
	...
	{
	  ipsecmb_hmac_key_ctx_data_t *ctx = key_data;
	  for (u32 idx = 0; idx < imbm->num_threads; idx++, ctx++, ptd++)
	    {
	      ctx->ctx = ptd;
