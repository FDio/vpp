...
get_mgr (vlib_main_t *vm)
{
  ...
  u8 *raw = (u8 *) imbm->per_thread_data;
  ipsecmb_per_thread_data_t *ptd =
    (ipsecmb_per_thread_data_t *) (raw + vm->thread_index * imbm->stride);
  return ptd->mgr;
}
...
crypto_ipsecmb_init (vnet_crypto_engine_registration_t *r)
{
  ...
  for (u32 i = 0; i < r->num_threads; i++)
    {
      ptd = (ipsecmb_per_thread_data_t *) (raw + i * imbm->stride);
