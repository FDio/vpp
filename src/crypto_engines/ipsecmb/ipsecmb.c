				      ...
				      u32 block_size, u32 data_size)
{
...

  {
    ...
    if (ctx->cipher_key_sz + ctx->auth_key_sz <= block_size)
      clib_memcpy_fast (key_hash, vnet_crypto_get_cipher_key (ctx),
			ctx->cipher_key_sz + ctx->auth_key_sz);
    else
      hash_fn (vnet_crypto_get_cipher_key (ctx), ctx->cipher_key_sz + ctx->auth_key_sz, key_hash);
					...
					vnet_crypto_alg_t cipher_alg, vnet_crypto_alg_t integ_alg)
{
  ...
  tmp_ctx->cipher_key_offset = tmp_ctx->auth_key_offset;
  tmp_ctx->cipher_key_sz = tmp_ctx->auth_key_sz;
  tmp_ctx->auth_key_offset = 0;
  tmp_ctx->auth_key_sz = 0;
...
#define _(a, b, c, j, d, e, f, g, h, i)                                                              \
  ...
  VNET_CRYPTO_REG_ALG (ipsecmb_##a##_tag##i) = {                                                   \
    .group = &ipsecmb_combined_##a##_group,                                                        \
    .alg_id = VNET_CRYPTO_ALG_##a##_TAG##i,                                                        \
    .simple = {                                                                                    \
      .enc_fn = ipsecmb_ops_combined_enc_##a,                                                      \
      .dec_fn = ipsecmb_ops_combined_dec_##a,                                                      \
    },                                                                                             \
...
#define _(a)                                                                                       \
  VNET_CRYPTO_REG_ALG (ipsecmb_chacha20_poly1305_tag16_aad##a) = {                                 \
    .group = &ipsecmb_chacha_poly_group,                                                           \
    .alg_id = VNET_CRYPTO_ALG_CHACHA20_POLY1305_TAG16_AAD##a,                                      \
    .simple = {                                                                                    \
