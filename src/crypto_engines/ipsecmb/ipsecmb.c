...
ipsecmb_ops_gcm (vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks,
		 u32 n_ops, aes_gcm_enc_dec_t enc_dec_fn,
		 aes_gcm_init_t init_fn, aes_gcm_enc_dec_update_t upd_fn,
		 aes_gcm_enc_dec_finalize_t finalize_fn, u32 is_dec,
		 u32 chained, u32 fixed, u32 aadlen)
{
  ipsecmb_main_t *imbm = &ipsecmb_main;
  ...
  for (i = 0; i < n_ops; i++)
    {
      ...
      vnet_crypto_op_t *op = ops[i];
      u8 scratch[64], *tag = is_dec ? scratch : op->tag;
      u32 taglen = 16;
      ...
      if (chained)
	{
	  ...
	  init_fn (kd, &ctx, op->iv, op->aad, aadlen);
	  chp = chunks + op->chunk_index;
	  for (j = 0; j < op->n_chunks; j++)
	    {
	      upd_fn (kd, &ctx, chp->dst, chp->src, chp->len);
	      chp += 1;
	    }
	  finalize_fn (kd, &ctx, tag, taglen);
	}
      else
	{
	  enc_dec_fn (kd, &ctx, op->dst, op->src, op->len, op->iv, op->aad,
		      aadlen, tag, taglen);
	}

      if (is_dec && (memcmp (op->tag, tag, taglen)))
	{
	  op->status = VNET_CRYPTO_OP_STATUS_FAIL_BAD_HMAC;
	  n_failed++;
	}
...
#define _(a, b, f, l)                                                         \
  static_always_inline u32 ipsecmb_ops_gcm_cipher_enc_##a (                   \
    vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops)                      \
  {                                                                           \
    return ipsecmb_ops_gcm (ops, 0, n_ops, get_mgr (vm)->gcm##b##_enc, 0, 0,  \
			    0, /* is_dec */ 0, /* chained */ 0,               \
			    /* fixed */ f, /* aadlen */ l);                   \
  }                                                                           \
                                                                              \
  ...
  {                                                                           \
    ...
    return ipsecmb_ops_gcm (ops, chunks, n_ops, 0, m->gcm##b##_init,          \
			    m->gcm##b##_enc_update, m->gcm##b##_enc_finalize, \
			    /* is_dec */ 0, /* chained */ 1, /* fixed */ f,   \
			    /* aadlen */ l);                                  \
  }                                                                           \
                                                                              \
  static_always_inline u32 ipsecmb_ops_gcm_cipher_dec_##a (                   \
    vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops)                      \
  {                                                                           \
    return ipsecmb_ops_gcm (ops, 0, n_ops, get_mgr (vm)->gcm##b##_dec, 0, 0,  \
			    0, /* is_dec */ 1, /* chained */ 0,               \
			    /* fixed */ f, /* aadlen */ l);                   \
  }                                                                           \
                                                                              \
  ...
  {                                                                           \
    IMB_MGR *m = get_mgr (vm);                                                \
    return ipsecmb_ops_gcm (ops, chunks, n_ops, 0, m->gcm##b##_init,          \
			    m->gcm##b##_dec_update, m->gcm##b##_dec_finalize, \
			    /* is_dec */ 1, /* chained */ 1, /* fixed */ f,   \
			    /* aadlen */ l);                                  \
  }
