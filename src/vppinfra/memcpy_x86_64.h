...
#endif
  u8 off, vec_bytes = 1 << log2_vec_bytes;
  ...
  if (n <= vec_bytes * 4)
    {
      dv[0] = sv[0];
      dv[1] = sv[1];
      dv = (__typeof__ (dv)) ((u8 *) dv - 2 * vec_bytes + n);
      sv = (__typeof__ (sv)) ((u8 *) sv - 2 * vec_bytes + n);
      dv[0] = sv[0];
      dv[1] = sv[1];
      return dst;
    }
  ...
  if (n <= vec_bytes * 8)
    {
      ...
      dv[2] = sv[2];
      dv[3] = sv[3];
      dv = (__typeof__ (dv)) ((u8 *) dv - 4 * vec_bytes + n);
      sv = (__typeof__ (sv)) ((u8 *) sv - 4 * vec_bytes + n);
      dv[0] = sv[0];
      dv[1] = sv[1];
      dv[2] = sv[2];
      dv[3] = sv[3];
