...
clib_memcpy_x86_64 (void *restrict dst, const void *restrict src, size_t n)
{
  ...
  while (PREDICT_TRUE (n >= vlock_bytes))
    {
...
last:
  ...
  if (PREDICT_TRUE (n))
	...
	"vmovdqu 0x160(%[src]), %[ymm0];\n"
	"vmovdqu %[ymm0], 0x160(%[dst]);\n"

	"vmovdqu 0x140(%[src]), %[ymm0];\n"
	"vmovdqu %[ymm0], 0x140(%[dst]);\n"

	"vmovdqu 0x120(%[src]), %[ymm0];\n"
	"vmovdqu %[ymm0], 0x120(%[dst]);\n"

	"vmovdqu 0x100(%[src]), %[ymm0];\n"
