...
uword vl (void *v) { return vec_len (v); }
...
#define w_thread(x, guts)                                               \
  ...
  {                                                                     \
    ...
    atomic_store (&tm->nadds, nadds);                                   \
    atomic_store (&tm->ndels, ndels);                                   \
    return NULL;                                                        \
  }
...
#define r_thread(x, guts)                                      \
  ...
  {                                                            \
    ...
    while (clib_cpu_time_now () < deadline);                   \
    atomic_store (&data->nlookups, nlookups);                  \
    return NULL;                                               \
  }
    ...
    {                                                                         \
      ...
      */                                                                      \
      ...
      for (i = 0; i < tm->nthreads; i++)                                      \
        {                                                                     \
          atomic_store (&tm->rthread_data[i].nlookups, 0);                    \
      ...
      for (i = 0; i < tm->nthreads; ++i)                                      \
        {                                                                     \
          uword nlookups = atomic_load (&tm->rthread_data[i].nlookups);       \
          fformat (stdout, #x "-> Thread #%d: %wu searches\n", i, nlookups);  \
...
static void cb (CVT (clib_cuckoo) * h, void *ctx)
{
  fformat (stdout, "Garbage callback called...\n");
}

static clib_error_t *test_cuckoo_bihash (test_main_t *tm)
{
  int i;
  uword *p;
  ...
  for (int iter = 0; iter < tm->search_iter; ++iter)
    {
...
clib_error_t *test_cuckoo_bihash_main (test_main_t *tm)
{
