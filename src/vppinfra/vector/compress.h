...
clib_compress_u64_x64 (u64 *dst, u64 *src, u64 mask)
{
  ...
  for (int i = 0; i < 16; i++)
    {
      int cnt = _popcnt32 ((((u8) mask) & 0x0f));
      u64x4_compress_store (sv[i], mask, dst);
      dst += cnt;
      mask >>= 4;
    }
#elif defined(CLIB_HAVE_VEC128_COMPRESS)
  u16x2u *sv = (u16x2u *) src;
  for (int i = 0; i < 32; i++)
    {
      int cnt = _popcnt32 ((((u8) mask) & 0x03));
      u16x2_compress_store (sv[i], mask, dst);
      dst += cnt;
      mask >>= 2;
    }
#else
