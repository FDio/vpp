...
clib_toeplitz_hash_key_expand (u64 *matrixes, u8 *key, int size)
{
  ...
  clib_toeplitz_hash_key_expand_8 (
    (u64x2) u8x16_align_right ((u64x2) kv, (u64x2) kv, 8), m);
}
...
clib_toeplitz_hash_key_init (u8 *key, u32 keylen)
{
  ...
  u64 *mm = (u64 *) ((u8 *) k + k->gfni_offset);
  clib_toeplitz_hash_key_expand (mm, k->data, k->key_length);
