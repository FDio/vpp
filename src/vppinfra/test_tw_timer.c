...
#include <vppinfra/tw_timer_4t_3w_4sl_ov.h>
...
typedef struct
{
  ...
  tw_timer_wheel_4t_3w_4sl_ov_t triple_ov_wheel;
...
static void
run_triple_ov_wheel (tw_timer_wheel_4t_3w_4sl_ov_t * tw, u32 n_ticks)
{
  u32 i;
  f64 now = tw->last_run_time + 1.01;
  ...
  for (i = 0; i < n_ticks; i++)
    {
      tw_timer_expire_timers_4t_3w_4sl_ov (tw, now);
      now += 1.01;
    }
}
...
test2_triple_ov (tw_timer_test_main_t * tm)
{
  ...
  tw_timer_wheel_init_4t_3w_4sl_ov (&tm->triple_ov_wheel,
				    expired_timer_triple_ov_callback,
				    1.0 /* timer interval */ , ~0);


  /* Prime offset */
  initial_wheel_offset = 1;	// 75700;
  ...
  for (i = 0; i < tm->ntimers; i++)
	...
	max_expiration_time = expiration_time;
      ...
      e->stop_timer_handle =
	tw_timer_start_4t_3w_4sl_ov (&tm->triple_ov_wheel, e - tm->test_elts,
				     3 /* timer id */ ,
				     expiration_time);
    }
  ...
  for (i = 0; i < tm->niter; i++)
    {
      ...
      pool_foreach (e, tm->test_elts,
      ({
        tw_timer_stop_4t_3w_4sl_ov (&tm->triple_ov_wheel, e->stop_timer_handle);
        vec_add1 (deleted_indices, e - tm->test_elts);
        if (++j >= tm->ntimers / 4)
          goto del_and_re_add;
      }));
      ...
      for (j = 0; j < tm->ntimers / 4; j++)
	{
	  ...
	  if (e->expected_to_expire == 790)
	    clib_warning ("oingo 790");


	  e->stop_timer_handle = tw_timer_start_4t_3w_4sl_ov
	    (&tm->triple_ov_wheel, e - tm->test_elts, 3 /* timer id */ ,
	     expiration_time);
	}
      adds += j;
    }
  ...
  pool_free (tm->test_elts);
  tw_timer_wheel_free_4t_3w_4sl_ov (&tm->triple_ov_wheel);
  return 0;
}
