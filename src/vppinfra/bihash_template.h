...
typedef CLIB_PACKED (struct {
   ...
   * freelists, we simple dole out memory starting from
   * alloc_arena[alloc_arena_next].
   */
  u64 alloc_arena_next; /* Next offset from alloc_arena to allocate, definitely
			   NOT a constant */
...
typedef BVS (clib_bihash)
{
  ...
  /**
   * A custom format function to print the Key and Value of bihash_key instead
   * of default hexdump
   */
...
static inline void BV (clib_bihash_set_stats_callback) (
  BVT (clib_bihash) * h, void (*cb) (BVT (clib_bihash) *, int, u64), void *ctx)
{
...
static inline int BV (clib_bihash_bucket_is_empty) (BVT (clib_bihash_bucket) *
						    b)
{
...
typedef int (*BV (clib_bihash_foreach_key_value_pair_cb)) (
  BVT (clib_bihash_kv) *, void *);
...
static inline BVT (clib_bihash_bucket) *
...
static inline int
  BV (clib_bihash_search_inline_with_hash) (BVT (clib_bihash) * h, u64 hash,
...
static inline int BV (clib_bihash_search_inline) (BVT (clib_bihash) * h,
						  BVT (clib_bihash_kv) *
						    key_result)
{
  u64 hash;
...
static inline void BV (clib_bihash_prefetch_bucket) (BVT (clib_bihash) * h,
						     u64 hash)
{
...
static inline void BV (clib_bihash_prefetch_data) (BVT (clib_bihash) * h,
						   u64 hash)
{
  BVT (clib_bihash_value) * v;
  BVT (clib_bihash_bucket) * b;
...
static inline int BV (clib_bihash_search_inline_2_with_hash) (
  BVT (clib_bihash) * h, u64 hash, BVT (clib_bihash_kv) * search_key,
...
static inline int
  BV (clib_bihash_search_inline_2) (BVT (clib_bihash) * h,
