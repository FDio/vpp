...
typedef struct
{
  union
  {
    struct
    {
      ...
      u64 generation : 5; // incremented each update, not really reliable under
			  // heavy load, still huge improvement over previous
			  // versions
    };
    u64 as_u64;
  };
...
bucket_changed_retry:
  ...
  for (i = 0; i < limit; i++)
      ...
      if (BV (clib_bihash_key_compare) (v->kvp[i].key, key_result->key))
	{
	  ...
	  *key_result = rv;
	  ...
	  if (BV (clib_bihash_search_need_retry) (b, &localb))
  ...
  // we could have been reading state in the middle of split operation and
  // search key actually could exist
  if (BV (clib_bihash_search_need_retry) (b, &localb))
    goto bucket_changed_retry;
