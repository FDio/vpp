 ...
 *
 * Circular doubly linked list with head sentinel. List elements are
 * allocated out of a "supporting" pool and all pool entries must contain
 * a list element struct, i.e., an anchor, for each list they pertain to.
 */
...
do {									\
  ...
  _lle (llist_prev (LP,name,E),name).next = _lle (E,name).next;		\
  _lle (llist_next (LP,name,E),name).prev = _lle (E,name).prev;		\
 ...
 *
 * List spliced into destination list is left with 0 elements, i.e., head
...
do {									\
  ...
  if (_ll_var (fe) != (H))						\
    {									\
      ...
      _lle (_ll_var (fe),name).prev = llist_entry_index((LP),(P));	\
      _lle (_ll_var (le),name).next = llist_entry_index((LP),_ll_var (ne));\
      _lle ((P),name).next = llist_entry_index ((LP),_ll_var (fe));	\
      _lle (_ll_var (ne),name).prev = llist_entry_index((LP),_ll_var (le));\
      _lle ((H),name).next = llist_entry_index((LP), (H));		\
      _lle ((H),name).prev = _lle ((H),name).next;			\
    }									\
} while (0)
