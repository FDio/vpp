...
void* mspace_get_aligned (mspace msp, 
  ...
  /* 
   * Alignment requests less than CLIB_CACHE_LINE_BYTES are ignored 
   */
  if (align < CLIB_CACHE_LINE_BYTES) {
    char *rv = mspace_malloc (msp, n_user_data_bytes);
    wwp = (unsigned *)rv;
    *wwp = 0;
    rv += sizeof (unsigned);
    return rv;
  ...
  if (align > 4<<10 || align_offset == ~0ULL) {
    ...
    return internal_memalign(ms, (size_t)align, n_user_data_bytes);
  }
...
void mspace_put (mspace msp, void *p)
{
  char *object_header;
  unsigned *wwp;
...
void mspace_put_no_offset (mspace msp, void *p)
{
  mspace_free (msp, p);
}
