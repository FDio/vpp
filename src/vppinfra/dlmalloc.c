...
static FORCEINLINE void
recursive_release_lock (MLOCK_T *lk)
{
  assert (lk->sl != 0);
  if (--lk->c == 0)
    {
...
static FORCEINLINE int
recursive_acquire_lock (MLOCK_T *lk)
{
      ...
      while (s != 0)
	{
	  ...
	  while (segment_holds (s, q) && q != m->top &&
	      ...
	      else
		{
		  ...
		  assert (lastq == 0 ||
			  is_inuse (lastq)); /* Not 2 consecutive free */
	      ...
	      else
		{
		  ...
		  if (is_small (sz))
		    { /* offset by possible bookkeeping */
		      start =
			(void *) ((char *) q + sizeof (struct malloc_chunk));
		    }
		  else
		    {
		      start = (void *) ((char *) q +
...
void *
dlrealloc_in_place (void *oldmem, size_t bytes)
{
  void *mem = 0;
  if (oldmem != 0)
    {
...
int
dlposix_memalign (void **pp, size_t alignment, size_t bytes)
{
  void *mem = 0;
  if (alignment == MALLOC_ALIGNMENT)
    mem = dlmalloc (bytes);
...
size_t
dlbulk_free (void *array[], size_t nelem)
{
...
#if MALLOC_INSPECT_ALL
void
dlmalloc_inspect_all (void (*handler) (void *start, void *end,
				       size_t used_bytes, void *callback_arg),
		      void *arg)
{
  ensure_initialization ();
...
size_t
dlmalloc_set_footprint_limit (size_t bytes)
{
...
int
mspace_track_large_chunks (mspace msp, int enable)
{
  int ret = 0;
...
void
mspace_get_address_and_size (mspace msp, char **addrp, size_t *sizep)
{
  mstate ms;
  msegment *this_seg;
...
CLIB_NOSANITIZE_ADDR __clib_export int
mspace_is_heap_object (mspace msp, void *p)
{
  msegment *this_seg;
  char *pp, *base;
  mstate ms;
...
void
mspace_disable_expand (mspace msp)
{
  mstate ms = (mstate) msp;
...
CLIB_NOSANITIZE_ADDR
int
mspace_enable_disable_trace (mspace msp, int enable)
{
  mstate ms = (mstate) msp;
  int was_enabled = 0;
...
CLIB_NOSANITIZE_ADDR __clib_export void *
mspace_get_aligned (mspace msp, unsigned long n_user_data_bytes,
...
CLIB_NOSANITIZE_ADDR __clib_export void
mspace_put (mspace msp, void *p_arg)
{
  char *object_header;
  unsigned *wwp;
  mstate ms = (mstate) msp;
...
void
mspace_put_no_offset (mspace msp, void *p_arg)
{
  mstate ms = (mstate) msp;
...
CLIB_NOSANITIZE_ADDR __clib_export size_t
mspace_usable_size_with_delta (const void *p)
{
  size_t usable_size;
  char *object_header;
  unsigned *wwp;
...
void *
mspace_realloc_in_place (mspace msp, void *oldmem, size_t bytes)
{
  void *mem = 0;
  if (oldmem != 0)
    {
...
void **
mspace_independent_calloc (mspace msp, size_t n_elements, size_t elem_size,
			   void *chunks[])
{
  size_t sz = elem_size; /* serves as 1-element array */
  mstate ms = (mstate) msp;
...
void **
mspace_independent_comalloc (mspace msp, size_t n_elements, size_t sizes[],
			     void *chunks[])
{
  mstate ms = (mstate) msp;
  if (!ok_magic (ms))
...
size_t
mspace_bulk_free (mspace msp, void *array[], size_t nelem)
{
...
#if MALLOC_INSPECT_ALL
void
mspace_inspect_all (mspace msp,
		    void (*handler) (void *start, void *end, size_t used_bytes,
				     void *callback_arg),
		    void *arg)
{
  mstate ms = (mstate) msp;
...
size_t
mspace_footprint_limit (mspace msp)
{
  size_t result = 0;
  mstate ms = (mstate) msp;
  if (ok_magic (ms))
...
size_t
mspace_set_footprint_limit (mspace msp, size_t bytes)
{
  size_t result = 0;
  mstate ms = (mstate) msp;
  if (ok_magic (ms))
