...
void do_search (test_main_t *tm, CVT (clib_cuckoo) * h)
{
  int i, j;
...
static clib_error_t *test_cuckoo (test_main_t *tm)
{
  int i;
  uword *p;
  ...
  for (i = 0; i < tm->nitems; i++)
    {
      ...
      thread_id = clib_cpu_time_now () % 16;
...
clib_error_t *test_cuckoo_main (test_main_t *tm)
{
