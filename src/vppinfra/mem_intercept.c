				 ...
				 int size_check, int *err)
{
  ...
  if (PREDICT_FALSE (!symbols_resolved))
    {
      __libc_malloc = (typeof (__libc_malloc)) dlsym (RTLD_NEXT, "malloc");
      __libc_realloc = (typeof (__libc_realloc)) dlsym (RTLD_NEXT, "realloc");
      __libc_free = (typeof (__libc_free)) dlsym (RTLD_NEXT, "free");
      __libc_memalign =
	(typeof (__libc_memalign)) dlsym (RTLD_NEXT, "memalign");
      symbols_resolved = 1;
    }
#endif
...
malloc_usable_size (void *p)
{
  ...
  if (clib_mem_main.alloc_free_intercept == 0)
	...
	__malloc_usable_size = (typeof (__malloc_usable_size)) dlsym (
      ...
      return __malloc_usable_size ? __malloc_usable_size (p) : 0;
    }
  if (clib_mem_intercept_get_heap (p))
    return clib_mem_size (p);
  {
    static size_t (*__malloc_usable_size) (void *) = 0;
    if (!__malloc_usable_size)
      __malloc_usable_size =
	(typeof (__malloc_usable_size)) dlsym (RTLD_NEXT, "malloc_usable_size");
    return __malloc_usable_size ? __malloc_usable_size (p) : 0;
  }
}
