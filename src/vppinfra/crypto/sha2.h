...
typedef struct
{
  ...
  u8 block_size;
  u8 digest_size;
  clib_sha2_state_t ipad_st;
  clib_sha2_state_t opad_st;
			 ...
			 clib_sha2_hmac_key_data_t *kd)
{
  ...
  uword data[SHA2_MAX_BLOCK_SIZE / sizeof (uword)] = {};
  uword ikey[SHA2_MAX_BLOCK_SIZE / sizeof (uword)];
  uword okey[SHA2_MAX_BLOCK_SIZE / sizeof (uword)];
  clib_sha2_state_t ipad_st;
  clib_sha2_state_t opad_st;
  ...
  for (int i = 0, w = 0; w < block_size; w += sizeof (uword), i++)
    {
      ikey[i] = data[i] ^ (uword) 0x3636363636363636;
      okey[i] = data[i] ^ (uword) 0x5c5c5c5c5c5c5c5c;
    }

  clib_sha2_state_init (&ipad_st, type);
  clib_sha2_state_init (&opad_st, type);

  if (block_size == CLIB_SHA2_512_BLOCK_SIZE)
    {
      clib_sha512_block (&ipad_st, (u8 *) ikey, 1);
      clib_sha512_block (&opad_st, (u8 *) okey, 1);
    }
  else
    {
      clib_sha256_block (&ipad_st, (u8 *) ikey, 1);
      clib_sha256_block (&opad_st, (u8 *) okey, 1);
    }
  kd->ipad_h = ipad_st.h;
  kd->opad_h = opad_st.h;
}
		     ...
		     clib_sha2_hmac_key_data_t *kd)
{
  ...
  ctx->digest_size = clib_sha2_variants[type].digest_size;
  ctx->type = type;
  clib_sha2_state_init (&ctx->ipad_st, type);
  clib_sha2_state_init (&ctx->opad_st, type);
  ctx->ipad_st.h = kd->ipad_h;
  ctx->opad_st.h = kd->opad_h;
  ctx->ipad_st.total_bytes = ctx->block_size;
  ctx->opad_st.total_bytes = ctx->block_size;
}

static_always_inline void
clib_sha2_hmac_update (clib_sha2_hmac_ctx_t *ctx, const u8 *msg, uword len)
{
  clib_sha2_update_internal (&ctx->ipad_st, ctx->block_size, msg, len);
}
...
clib_sha2_hmac_final (clib_sha2_hmac_ctx_t *ctx, u8 *digest)
{
  ...
  clib_sha2_final_internal (&ctx->ipad_st, ctx->block_size, ctx->digest_size,
			    i_digest);
  clib_sha2_update_internal (&ctx->opad_st, ctx->block_size, i_digest,
			     ctx->digest_size);
  clib_sha2_final_internal (&ctx->opad_st, ctx->block_size, ctx->digest_size,
			    digest);
}
