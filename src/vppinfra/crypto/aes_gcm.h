...
static_always_inline void
aes_gcm_enc_round (aes_data_t *r, aes_gcm_expaned_key_t Ke, uword n_blocks)
{
		   ...
		   u32 n_bytes)
{
  ...
  ctx->Y0 ^= ctx->Ke[0].x1;
  ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[1].x1);

  if (n_blocks > 1)
    aes_gcm_ghash_mul_next (ctx, d[1]);

  ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[2].x1);
  ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[3].x1);

  if (n_blocks > 2)
    aes_gcm_ghash_mul_next (ctx, d[2]);

  ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[4].x1);
  ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[5].x1);

  if (n_blocks > 3)
    aes_gcm_ghash_mul_next (ctx, d[3]);

  ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[6].x1);
  ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[7].x1);

  aes_gcm_ghash_mul_bit_len (ctx);
  aes_gcm_ghash_reduce (ctx);

  ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[8].x1);
  ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[9].x1);

  aes_gcm_ghash_reduce2 (ctx);

  for (i = 10; i < ctx->rounds; i++)
    ctx->Y0 = aes_enc_round (ctx->Y0, ctx->Ke[i].x1);

  aes_gcm_ghash_final (ctx);

  ctx->Y0 = aes_enc_last_round (ctx->Y0, ctx->Ke[i].x1);
}
	 ...
	 const aes_gcm_key_data_t *kd, int aes_rounds, aes_gcm_op_t op)
{
  ...
  if (op != AES_GCM_OP_ENCRYPT)
	...
	ctx->Y0 = aes_enc_round (ctx->Y0, kd->Ke[i].x1);
      ctx->Y0 = aes_enc_last_round (ctx->Y0, kd->Ke[ctx->rounds].x1);
    }
