...
clib_aes_cbc_encrypt (aes_cbc_key_data_t *kd, u8 *dst, const u8 *src,
...
static_always_inline void
clib_aes128_cbc_encrypt (aes_cbc_key_data_t *kd, u8 *ciphertext,
			 const u8 *plaintext, u32 len, const u8 *iv)
{
  clib_aes_cbc_encrypt (kd, ciphertext, plaintext, len, iv, AES_KEY_128);
}

static_always_inline void
clib_aes192_cbc_encrypt (aes_cbc_key_data_t *kd, u8 *ciphertext,
			 const u8 *plaintext, u32 len, const u8 *iv)
{
  clib_aes_cbc_encrypt (kd, ciphertext, plaintext, len, iv, AES_KEY_192);
}

static_always_inline void
clib_aes256_cbc_encrypt (aes_cbc_key_data_t *kd, u8 *ciphertext,
			 const u8 *plaintext, u32 len, const u8 *iv)
{
  clib_aes_cbc_encrypt (kd, ciphertext, plaintext, len, iv, AES_KEY_256);
}
...
static_always_inline void
clib_aes_cbc_decrypt (aes_cbc_key_data_t *kd, u8 *plaintext,
		      const u8 *ciphertext, const u8 *iv, u32 len,
		      aes_key_size_t ks)
{
  int rounds = AES_KEY_ROUNDS (ks);
#if defined(__VAES__) && defined(__AVX512F__)
...
static_always_inline void
clib_aes128_cbc_decrypt (aes_cbc_key_data_t *kd, u8 *plaintext,
			 const u8 *ciphertext, const u8 *iv, u32 len)
{
  clib_aes_cbc_decrypt (kd, plaintext, ciphertext, iv, len, AES_KEY_128);
}

static_always_inline void
clib_aes192_cbc_decrypt (aes_cbc_key_data_t *kd, u8 *plaintext,
			 const u8 *ciphertext, const u8 *iv, u32 len)
{
  clib_aes_cbc_decrypt (kd, plaintext, ciphertext, iv, len, AES_KEY_192);
}

static_always_inline void
clib_aes256_cbc_decrypt (aes_cbc_key_data_t *kd, u8 *plaintext,
			 const u8 *ciphertext, const u8 *iv, u32 len)
{
  clib_aes_cbc_decrypt (kd, plaintext, ciphertext, iv, len, AES_KEY_256);
}
