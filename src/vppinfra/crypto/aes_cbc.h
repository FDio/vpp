...
clib_aes_cbc_encrypt (u8 *dst, u8 *src, aes_cbc_key_data_t *kd, u8 *iv,
		      uword len, aes_key_size_t ks)
{
  int rounds = AES_KEY_ROUNDS (ks);
  u8x16 r, *k = kd->encrypt_key;
...
static_always_inline void
clib_aes128_cbc_encrypt (u8 *dst, u8 *src, aes_cbc_key_data_t *k, u8 *iv,
			 uword len)
{
  clib_aes_cbc_encrypt (dst, src, k, iv, len, AES_KEY_128);
}

static_always_inline void
clib_aes192_cbc_encrypt (u8 *dst, u8 *src, aes_cbc_key_data_t *k, u8 *iv,
			 uword len)
{
  clib_aes_cbc_encrypt (dst, src, k, iv, len, AES_KEY_192);
}

static_always_inline void
clib_aes256_cbc_encrypt (u8 *dst, u8 *src, aes_cbc_key_data_t *k, u8 *iv,
			 uword len)
{
  clib_aes_cbc_encrypt (dst, src, k, iv, len, AES_KEY_256);
}
...
static_always_inline void
aes4_cbc_dec (u8x16 *k, u8x64u *src, u8x64u *dst, u8x16u *iv, int count,
...
static_always_inline void
aes2_cbc_dec (u8x16 *k, u8x32u *src, u8x32u *dst, u8x16u *iv, int count,
	      aes_key_size_t rounds)
{
  u8x32 k2, f = {}, r[4], c[4] = {};
  int i, n_blocks = count >> 4;
...
static_always_inline void
clib_aes_cbc_decrypt (u8 *dst, u8 *src, aes_cbc_key_data_t *k, u8 *iv,
		      uword len, aes_key_size_t ks)
{
  int rounds = AES_KEY_ROUNDS (ks);
#if defined(__VAES__) && defined(__AVX512F__)
  aes4_cbc_dec (k->decrypt_key, (u8x64u *) src, (u8x64u *) dst, (u8x16u *) iv,
		len, rounds);
#elif defined(__VAES__)
  aes2_cbc_dec (k->decrypt_key, (u8x32u *) src, (u8x32u *) dst, (u8x16u *) iv,
		len, rounds);
#else
  aes_cbc_dec (k->decrypt_key, (u8x16u *) src, (u8x16u *) dst, (u8x16u *) iv,
	       len, rounds);
#endif
}

static_always_inline void
clib_aes128_cbc_decrypt (u8 *dst, u8 *src, aes_cbc_key_data_t *k, u8 *iv,
			 uword len)
{
  clib_aes_cbc_decrypt (dst, src, k, iv, len, AES_KEY_128);
}

static_always_inline void
clib_aes192_cbc_decrypt (u8 *dst, u8 *src, aes_cbc_key_data_t *k, u8 *iv,
			 uword len)
{
  clib_aes_cbc_decrypt (dst, src, k, iv, len, AES_KEY_192);
}

static_always_inline void
clib_aes256_cbc_decrypt (u8 *dst, u8 *src, aes_cbc_key_data_t *k, u8 *iv,
			 uword len)
{
  clib_aes_cbc_decrypt (dst, src, k, iv, len, AES_KEY_256);
}
