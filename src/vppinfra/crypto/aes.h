...
#define aes_reflect(r)		   ((u32x16) u8x64_reflect_u8x16 (r))
...
#define aes_reflect(r)		   ((u32x8) u8x32_reflect_u8x16 (r))
...
#define aes_reflect(r)		   ((u32x4) u8x16_reflect (r))
		    ...
		    uword n_blocks)
{
  for (int i = 0; i < n_blocks; i++)
#if N_AES_LANES == 4
    d[i] ^= aes_enc_last_round_x4 (r[i], ek->x4);
#elif N_AES_LANES == 2
    d[i] ^= aes_enc_last_round_x2 (r[i], ek->x2);
#else
    d[i] ^= aes_enc_last_round_x1 (r[i], ek->x1);
#endif
}
