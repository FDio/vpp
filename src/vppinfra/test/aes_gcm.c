...
static const u8 gmac1_key[] = { 0x77, 0xbe, 0x63, 0x70, 0x89, 0x71, 0xc4, 0xe2,
               0x40, 0xd1, 0xcb, 0x79, 0xe8, 0xd7, 0x7f, 0xeb };
static const u8 gmac1_iv[] = { 0xe0, 0xe0, 0x0f, 0x19, 0xfe, 0xd7, 0xba, 0x01,
    0x36, 0xa7, 0x97, 0xf3 };
static const u8 gmac1_aad[] = { 0x7a, 0x43, 0xec, 0x1d, 0x9c, 0x0a, 0x5a, 0x78,
               0xa0, 0xb1, 0x65, 0x33, 0xa6, 0x21, 0x3c, 0xab };
static const u8 gmac1_tag[] = { 0x20, 0x9f, 0xcc, 0x8d, 0x36, 0x75, 0xed, 0x93,
...
clib_aes128_gmac (const aes_gcm_key_data_t *kd,
		     const u8 *aad, u32 aad_bytes,
		     const u8 *iv, u32 tag_bytes, u8 *tag)
{
  aes_gcm (0, 0, (u8x16u *) aad,
	   (u8 *) iv, (u8x16u *) tag, 0, aad_bytes, tag_bytes, kd,
	   AES_KEY_ROUNDS (AES_KEY_128), 1);
}
...
test_clib_aes128_gmac (clib_error_t *err)
{
  ...
  FOREACH_ARRAY_ELT (tc, gmac_test_cases)
    {
      clib_aes_gcm_key_expand (&kd, tc->key128, AES_KEY_128);
      clib_aes128_gmac (&kd, tc->aad, tc->aad_len,
			   tc->iv, tc->tag128_len, tag);
