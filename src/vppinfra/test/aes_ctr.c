...
test_clib_aes128_ctr_enc (clib_error_t *err)
{
  ...
  clib_aes128_ctr_enc (&kd, pt, INC_TEST_BYTES, inc_iv, ct);
  if (memcmp (inc_ct128, ct, INC_TEST_BYTES) != 0)
    {
      fformat(stderr, "a: %U\n", format_hexdump, ct, 32);
      fformat(stderr, "b: %U\n", format_hexdump, inc_ct128, 32);
      return clib_error_return (err, "incremental test failed");
    }
