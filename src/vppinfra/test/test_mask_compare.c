...
test_clib_compare_u16 (clib_error_t *err)
{
  ...
  for (i = 0; i < ARRAY_LEN (array); i++)
	...
	{
	  if (bitmap[j])
	    return clib_error_return (err, "bitmap at position %u not zero",
				      j);
	}
      if (bitmap[j] != 1ULL << (i & 0x3f))
	return clib_error_return (err,
				  "[%u] bitmap at position %u is %lx, "
				  "expected %lx",
				  i, j, bitmap[j], 1ULL << (i % 64));
      if (bitmap[j + 1] != 0xa5a5a5a5a5a5a5a5)
	return clib_error_return (err, "bitmap overrun at position %u", j + 1);
    }
  return err;
}
