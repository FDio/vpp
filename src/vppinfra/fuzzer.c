...
static void (*real_fuzz_driver_func)(const void *data, uword size) = 0;

__clib_export void set_fuzz_driver(void *ptr) {
	real_fuzz_driver_func = ptr;
}
...
__clib_export int LLVMFuzzerTestOneInput(const void *data, uword size) {
    ...
    if (real_fuzz_driver_func) {
	    real_fuzz_driver_func(data, size);
    } else {
	    printf("Fuzz called %ld times with unset driver!\r", counter++);
    }
