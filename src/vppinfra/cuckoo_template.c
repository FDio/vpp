...
/* for single writer, don't bother with atomic loads, we're holding the lock */
#undef atomic_load
#undef atomic_load_explicit
#define atomic_load(x) (*x)
#define atomic_load_explicit(x, mo) (*x)
...
u8 *CV (format_cuckoo_bucket) (u8 *s, va_list *args)
{
  ...
  clib_cuckoo_bucket_aux_t aux = atomic_load (&bucket->aux);
...
static void CV (clib_cuckoo_deep_self_check) (CVT (clib_cuckoo) * h)
{
  ...
  clib_cuckoo_foreach_bucket (bucket, h)
  {
    ...
    clib_cuckoo_bucket_aux_t aux =
        atomic_load_explicit (&bucket->aux, memory_order_relaxed);
    ...
    void (*garbage_callback) (CVT (clib_cuckoo) *, void *), void *garbage_ctx)
{
  ...
  h->buckets = buckets;
  CV (clib_cuckoo_snapshot_refresh) (h);
...
static clib_cuckoo_bucket_aux_t CV (clib_cuckoo_bucket_version_bump_and_lock) (
    CVT (clib_cuckoo_bucket) * b)
{
  clib_cuckoo_bucket_aux_t aux = atomic_load (&b->aux);
 ...
 */
static void clib_cuckoo_path_walk (CVT (clib_cuckoo) * h, uword path_idx,
                                   uword *buckets, uword *offsets)
{
  ...
  for (uword i = 1; i < path->length; ++i)
    {
      CVT (clib_cuckoo_bucket) *b =
          CV (clib_cuckoo_bucket_at_index) (h, buckets[i - 1]);
      buckets[i] = clib_cuckoo_get_other_bucket (
          CV (clib_cuckoo_get_nbuckets) (h), buckets[i - 1],
 ...
 */
static int CV (clib_cuckoo_find_empty_slot_bfs) (
    CVT (clib_cuckoo) * h, clib_cuckoo_lookup_info_t *lookup,
    uword *path_idx_out, uword *found_bucket,
    CVT (clib_cuckoo_kv) * *found_elt)
{
  ...
  for (int i = 0; i < CLIB_CUCKOO_KVP_PER_BUCKET; ++i)
    {
      clib_cuckoo_path_t *path =
  ...
  while (1)
    {
      ...
      uword other_bucket = clib_cuckoo_get_other_bucket (
          CV (clib_cuckoo_get_nbuckets) (h), path->bucket,
      ...
      if (path->bucket != other_bucket)
        {
          if ((*found_elt = CV (clib_cuckoo_bucket_find_empty) (
                   CV (clib_cuckoo_bucket_at_index) (h, other_bucket))))
            {
              /* found empty element */
...
#if CLIB_CUCKOO_DEBUG_PATH
              CLIB_CUCKOO_DBG (
                  "Bucket with empty slot:\n%U", CV (format_cuckoo_bucket),
                  CV (clib_cuckoo_bucket_at_index) (h, other_bucket));
#endif
              goto out;
            }
          ...
          if (path->length < CLIB_CUCKOO_BFS_MAX_PATH_LENGTH &&
              vec_len (h->bfs_search_queue) < CLIB_CUCKOO_BFS_MAX_STEPS)
            {
              ...
              for (int i = 0; i < CLIB_CUCKOO_KVP_PER_BUCKET; ++i)
                {
                  uword new_path_idx = CV (clib_cuckoo_path_extend) (
                      h, path_idx, other_bucket, i);
                  tail[i] = new_path_idx;
                }
            }
        }
                                      ...
                                      u8 reduced_hash)
{
  ...
  int rv = CV (clib_cuckoo_find_empty_slot_bfs) (
      h, lookup, &path_idx, &empty_bucket_idx, &empty_elt);
  if (CLIB_CUCKOO_ERROR_SUCCESS == rv)
    {
      uword buckets[CLIB_CUCKOO_BFS_MAX_PATH_LENGTH];
      uword offsets[CLIB_CUCKOO_BFS_MAX_PATH_LENGTH];
      ...
      for (int i = path->length - 1; i >= 0; --i)
        {
          ...
          if (i == path->length - 1)
            {
              /* we only need to to increase the use count for the bucket with
               * free element - all other buckets' use counts won't change */
              empty_aux = clib_cuckoo_bucket_aux_set_use_count (
                  empty_aux,
                  clib_cuckoo_bucket_aux_get_use_count (empty_aux) + 1);
            }
                                      ...
                                      u8 reduced_hash)
{
  ...
  if ((elt = CV (clib_cuckoo_bucket_find_empty) (bucket1)))
    {
      ...
      aux = clib_cuckoo_bucket_aux_set_use_count (
          aux, clib_cuckoo_bucket_aux_get_use_count (aux) + 1);
  ...
  if ((elt = CV (clib_cuckoo_bucket_find_empty) (
           CV (clib_cuckoo_bucket_at_index) (h, lookup->bucket2))))
    {
      clib_cuckoo_bucket_aux_t aux =
...
{
  CLIB_MEMORY_BARRIER ();
  CVT (clib_cuckoo_bucket) **garbage = NULL;
  for (int i = 0; i < sizeof (h->to_be_freed) / sizeof (*h->to_be_freed); ++i)
    {
      CVT (clib_cuckoo_bucket) **to_be_freed = h->to_be_freed[i];
      CVT (clib_cuckoo_bucket) * *b;
      vec_foreach (b, to_be_freed)
      {
        if (!*b)
          {
            continue;
        ...
        fformat (stdout, "gc: possible garbage: %p in thread %d\n", *b, i);
#endif
        if (~0 == vec_search (garbage, *b))
          {
            vec_add1 (garbage, *b);
          }
      }
      vec_reset_length (to_be_freed);
      h->to_be_freed[i] = to_be_freed;
    }
  CVT (clib_cuckoo_bucket) * *b;
  vec_foreach (b, garbage)
  {
    if (*b == h->buckets)
      {
        continue;
  ...
  for (int i = 0;
       i < sizeof (h->snapshot_buckets) / sizeof (*h->snapshot_buckets); ++i)
    {
#if CLIB_CUCKOO_DEBUG_GC
      if (h->buckets != h->snapshot_buckets[i])
        {
          fformat (stdout,
                   "gc: fix stale snapshot %p in thread %d to current %p\n",
                   h->snapshot_buckets[i], i, h->buckets);
        }
#endif
      h->snapshot_buckets[i] = h->buckets;
    }
  CLIB_MEMORY_BARRIER ();
}
 ...
 */
static int CV (clib_cuckoo_rehash) (CVT (clib_cuckoo) * h)
{
  int rv = CLIB_CUCKOO_ERROR_SUCCESS;
  CVT (clib_cuckoo_bucket) *old = atomic_load (&h->buckets);
  uword old_nbuckets = vec_len (old);
  uword new_nbuckets = 2 * old_nbuckets;
  CVT (clib_cuckoo_bucket) *new = vec_dup_aligned (old, CLIB_CACHE_LINE_BYTES);
  /* allocate space */
  ...
  uword cpu_num = os_get_cpu_number ();
  if (~0 == vec_search (h->to_be_freed[cpu_num], old))
    {
      vec_add1 (h->to_be_freed[cpu_num], old);
    }
   ...
   * are required here
   */
  for (uword old_bucket_idx = 0; old_bucket_idx < old_nbuckets;
       ++old_bucket_idx)
    {
      ...
      int moved = 0;
      clib_cuckoo_bucket_aux_t aux =
          atomic_load_explicit (&old_bucket->aux, memory_order_relaxed);
      ...
      if (moved)
        {
          ...
          aux = clib_cuckoo_bucket_aux_set_use_count (
              aux, clib_cuckoo_bucket_aux_get_use_count (aux) - moved);
          atomic_store_explicit (&old_bucket->aux, aux, memory_order_relaxed);
          aux = atomic_load_explicit (&new_bucket->aux, memory_order_relaxed);
          aux = clib_cuckoo_bucket_aux_set_use_count (
              aux, clib_cuckoo_bucket_aux_get_use_count (aux) + moved);
          atomic_store_explicit (&new_bucket->aux, aux, memory_order_relaxed);
        }
    }
  atomic_store (&h->buckets, new);
  CV (clib_cuckoo_snapshot_refresh) (h);
#if CLIB_CUCKOO_DEBUG_COUNTERS
  ++h->rehashes;
#endif
                              ...
                              CVT (clib_cuckoo_kv) * kvp, int is_add)
{
  ...
  while (atomic_flag_test_and_set_explicit (&h->writer_lock,
                                            memory_order_acquire))
    ;
  u8 reduced_hash = clib_cuckoo_reduce_hash (hash);
restart:
  lookup =
      CV (clib_cuckoo_calc_lookup) (CV (clib_cuckoo_get_snapshot) (h), hash);
  CVT (clib_cuckoo_bucket) *b =
      CV (clib_cuckoo_bucket_at_index) (h, lookup.bucket1);
  ...
  if (!is_add)
    {
      CLIB_CUCKOO_DBG ("%U not present in table", CV (format_cuckoo_kvp), kvp);
  ...
  if (CLIB_CUCKOO_ERROR_SUCCESS != rv)
    {
      CLIB_CUCKOO_DBG (
          ...
          aux.bucket1, aux.bucket2, CV (format_cuckoo_bucket),
          CV (clib_cuckoo_bucket_at_index) (h, aux.bucket1),
      ...
      if (CLIB_CUCKOO_ERROR_SUCCESS != rv)
        {
          ...
          if (CLIB_CUCKOO_ERROR_SUCCESS == rv)
            ...
            {
              CLIB_CUCKOO_ERR ("Rehash failed!");
            }
        }
    }
unlock:
  atomic_flag_clear_explicit (&h->writer_lock, memory_order_release);
  return rv;
}
...
u8 *CV (format_cuckoo) (u8 *s, va_list *args)
{
  ...
  clib_cuckoo_foreach_bucket (b, h)
  {
    ...
    clib_cuckoo_bucket_aux_t aux =
        atomic_load_explicit (&b->aux, memory_order_relaxed);
