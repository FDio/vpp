...
static int CV (clib_cuckoo_add_fast) (CVT (clib_cuckoo) * h,
				      ...
				      CVT (clib_cuckoo_kv) * kvp,
				      u8 reduced_hash,
				      int (*is_stale_cb) (CVT (clib_cuckoo_kv)
							  *, void *),
				      void *arg)
{
  CVT (clib_cuckoo_kv) * elt;
  ...
  if (is_stale_cb)
    {
// FIXME      for (i = 0; i < limit; i++)
// FIXME        {
// FIXME          if (is_stale_cb (&(v->kvp[i]), arg))
// FIXME            {
// FIXME              CLIB_MEMORY_BARRIER ();
// FIXME              clib_memcpy_fast (&(v->kvp[i]), add_v, sizeof (*add_v));
// FIXME              BV (clib_bihash_unlock_bucket) (b);
// FIXME              BV (clib_bihash_increment_stat) (h, BIHASH_STAT_replace, 1);
// FIXME              return (0);
// FIXME            }
// FIXME        }
    }
...
restart:
  ...
  rv =
    CV (clib_cuckoo_add_fast) (h, &lookup, kvp, reduced_hash, is_stale_cb,
			       arg);
  CLIB_CUCKOO_DEEP_SELF_CHECK (h);
  if (CLIB_CUCKOO_ERROR_SUCCESS != rv)
    {
...
int CV (clib_cuckoo_add_or_overwrite_stale) (CVT (clib_cuckoo) * h,
					     CVT (clib_cuckoo_kv) * kvp,
					     int (*is_stale_cb) (CVT
								 (clib_cuckoo_kv)
								 *, void *),
					     void *arg)
{
  return CV (clib_cuckoo_add_del_inline) (h, kvp, 1, is_stale_cb, arg);
}
