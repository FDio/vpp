		     ...
		     char *nfmt, va_list *va)
{
  ...
  if (nfmt)
    {
      /* only object values can have name */
      if (parrent != CLIB_JSON_PARENT_OBJECT)
	return -1;
    }
  ...
  if (p->type == CLIB_JSON_VALUE_OBJECT)
    {
      ...
      nvp->name = nfmt ? va_format (0, nfmt, va) : 0;
...
__clib_export int
clib_json_add_null (clib_json_text_t *j, char *nfmt, ...)
{
  int index;
  va_list va;
  va_start (va, nfmt);
  index = clib_json_new_value (j, CLIB_JSON_VALUE_NULL, nfmt, &va);
  va_end (va);
  return index;
}

__clib_export int
clib_json_add_true (clib_json_text_t *j, char *nfmt, ...)
{
  int index;
  va_list va;
  va_start (va, nfmt);
  index = clib_json_new_value (j, CLIB_JSON_VALUE_TRUE, nfmt, &va);
  va_end (va);
  return index;
}

__clib_export int
clib_json_add_false (clib_json_text_t *j, char *nfmt, ...)
{
  int index;
  va_list va;
  va_start (va, nfmt);
  index = clib_json_new_value (j, CLIB_JSON_VALUE_FALSE, nfmt, &va);
  va_end (va);
  return index;
}

__clib_export int
clib_json_add_string (clib_json_text_t *j, char *nfmt, char *sfmt, ...)
{
  int index;
  clib_json_value_t *v;
  va_list va;
  ...
  index = clib_json_new_value (j, CLIB_JSON_VALUE_STRING, nfmt, &va);
...
__clib_export int
clib_json_add_number (clib_json_text_t *j, char *nfmt, f64 num, ...)
{
  int index;
  clib_json_value_t *v;
  va_list va;
  ...
  index = clib_json_new_value (j, CLIB_JSON_VALUE_NUMBER, nfmt, &va);
...
__clib_export int
clib_json_add_object (clib_json_text_t *j, char *nfmt, ...)
{
  int index;
  va_list va;
  va_start (va, nfmt);
  index = clib_json_new_value (j, CLIB_JSON_VALUE_OBJECT, nfmt, &va);
  va_end (va);
...
__clib_export int
clib_json_add_array (clib_json_text_t *j, char *nfmt, ...)
{
  int index;
  va_list va;
  va_start (va, nfmt);
  index = clib_json_new_value (j, CLIB_JSON_VALUE_ARRAY, nfmt, &va);
  va_end (va);
  j->current_value = index;
  return index;
}
...
__clib_export int
clib_json_get_top_value_index (clib_json_text_t *jt)
{
  return jt->root_value;
}

__clib_export int
clib_json_value_is_type (clib_json_text_t *jt, int value_index,
			 clib_json_value_type_t type)
{
  clib_json_value_t *v;

  if (pool_is_free_index (jt->values, value_index))
    return 0;

  v = pool_elt_at_index (jt->values, value_index);
  if (v->type == type)
    return 1;
  return 0;
}
...
__clib_export int *
clib_json_get_array_value_indices (clib_json_text_t *j, int array_value_index)
{
  clib_json_value_t *v;

  if (!clib_json_value_is_array (j, array_value_index))
    return 0;

  v = pool_elt_at_index (j->values, array_value_index);
  return v->array_val_indices;
}

__clib_export int
clib_json_find_object_value_by_name (clib_json_text_t *j,
				     int object_value_index, char *fmt, ...)
{
  ...
  va_list va;
  u8 *name;
  int rv = CLIB_JSON_ERR_OBJECT_NAME_NOT_FOUND;

  if (!clib_json_value_is_object (j, object_value_index))
    return CLIB_JSON_ERR_INVALID_VALUE_TYPE;

  va_start (va, fmt);
  name = va_format (0, fmt, &va);
  va_end (va);

  v = pool_elt_at_index (j->values, object_value_index);

  pool_foreach (nvp, v->nvpairs)
    if (vec_is_equal (name, nvp->name))
      {
	rv = nvp->value_index;
	break;
      }
...
__clib_export uword
clib_json_unformat_string (clib_json_text_t *j, int value_index, char *fmt,
			   ...)
{
  clib_json_value_t *v;
  ...
  if (!clib_json_value_is_string (j, value_index))
    return 0;

  v = pool_elt_at_index (j->values, value_index);

  unformat_init_string (&input, (char *) v->string, vec_len (v->string));
...
format_clib_json_value (u8 *s, va_list *args)
{
  ...
  switch (v->type)
    ...
    case CLIB_JSON_VALUE_STRING:
      ...
      break;
    case CLIB_JSON_VALUE_NUMBER:
      if (isnan (v->number) || isinf (v->number))
	s = format (s, "null");
      else
	s = format (s, "%.*g", 17, 0.00001);
      break;
    case CLIB_JSON_VALUE_OBJECT:
      vec_add1 (s, '{');
      if (vec_len (v->nvpairs))
	{
	  clib_json_nvpair_t *nvp;
	  ...
	  pool_foreach (nvp, v->nvpairs)
	    {
	      ...
	      s = format (s, "\"%v\": %U", nvp->name, format_clib_json_value,
			  j, nvp->value_index);
			   ...
			   u32 index, u8 *name)
{
  ...
  int to_index = clib_json_new_value (to, fv->type, (char *) name, 0);
  u32 i;
  ...
  switch (tv->type)
	...
	clib_json_append_internal (to, from, fv->array_val_indices[i], 0);
      clib_json_parent (to);
      break;
    case CLIB_JSON_VALUE_OBJECT:
      to->current_value = to_index;
	...
	{
	  name = vec_dup (fv->nvpairs[i].name);
	  vec_add1 (name, 0);
...
__clib_export int
clib_json_append (clib_json_text_t *to, clib_json_text_t *from, char *nfmt,
		  ...)
{
  u8 *name = 0;
  int rv;
  ...
  if (nfmt)
    {
      va_list va;
      va_start (va, nfmt);
      name = va_format (0, nfmt, &va);
      va_end (va);
      vec_add1 (name, 0);
    }

  rv = clib_json_append_internal (to, from, from->root_value, name);
  vec_free (name);
  return rv;
}

__clib_export u8 *
format_clib_json_text (u8 *s, va_list *args)
{
  clib_json_text_t *j = va_arg (*args, clib_json_text_t *);
...
unformat_clib_json_value (unformat_input_t *i, va_list *args)
{
  ...
  while ((c = unformat_get_input (i)) != UNFORMAT_END_OF_INPUT)
	...
	case '[':
	  if ((expect & EXP_VALUE) == 0 || clib_json_add_array (j, name) < 0)
	    goto error;
	  vec_free (name);
	  unformat_get_input (i);
	...
	case '{':
	  if ((expect & EXP_VALUE) == 0 || clib_json_add_object (j, name) < 0)
	    goto error;
	  vec_free (name);
	  unformat_get_input (i);
	  parrent = clib_json_get_parrent_type (j);
	...
	case '"':
	  ...
	  if (expect & EXP_VALUE)
	    {
	      int index =
		clib_json_new_value (j, CLIB_JSON_VALUE_STRING, name, 0);
	      clib_json_value_t *v;
	      vec_free (name);
