...
clib_compare_u16_x64 (u16 v, u16 *a, u32 n_elts)
{
  u64 mask = 0;
...
clib_compare_u16 (u16 v, u16 *a, u64 *bitmap, u32 n_elts)
{
  while (n_elts >= 64)
    {
      bitmap++[0] = clib_compare_u16_x64 (v, a, 64);
      n_elts -= 64;
      a += 64;
    }
  ...
  bitmap[0] = clib_compare_u16_x64 (v, a, n_elts) & pow2_mask (n_elts);
}

static_always_inline u64
clib_compare_u32_x64 (u32 v, u32 *a, u32 n_elts)
{
  u64 mask = 0;
#if defined(CLIB_HAVE_VEC512)
  ...
  c = i16x16_pack (i32x8_pack (v8 == av[0], v8 == av[1]),
		   i32x8_pack (v8 == av[2], v8 == av[3]));
  mask = i8x32_msb_mask ((i8x32) u32x8_permute (c, m));

  c = i16x16_pack (i32x8_pack (v8 == av[4], v8 == av[5]),
		   i32x8_pack (v8 == av[6], v8 == av[7]));
  mask |= (u64) i8x32_msb_mask ((i8x32) u32x8_permute (c, m)) << 32;
  ...
  for (int i = 0; i < 16; i++)
    mask |= (u64) vaddvq_u32 ((av[i] == v4) & m) << (i * 8);
...
static_always_inline void
clib_compare_u32 (u32 v, u32 *a, u64 *bitmap, u32 n_elts)
{
  while (n_elts >= 64)
    {
      bitmap++[0] = clib_compare_u32_x64 (v, a, 64);
      n_elts -= 64;
      a += 64;
    }
  ...
  bitmap[0] = clib_compare_u32_x64 (v, a, n_elts) & pow2_mask (n_elts);
}
...
clib_compress_u32 (u32 *dst, u32 *src, u64 *mask, u32 n_elts)
{
  ...
  return clib_compress_u32_x64 (dst, src, mask[0] & pow2_mask (n_elts));
}

static_always_inline u32
find_first_unused_elt (u64 *used_elt_bitmap)
{
  u64 *ue = used_elt_bitmap;

  while (PREDICT_FALSE (ue[0] == ~0))
    ue++;

  return (ue - used_elt_bitmap) * 64 + count_trailing_zeros (~ue[0]);
}
