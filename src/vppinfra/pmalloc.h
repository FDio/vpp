...
clib_pmalloc_get_arena (clib_pmalloc_main_t * pm, u32 index)
{
  return pool_elt_at_index (pm->arenas, index);
}

always_inline u8 *
clib_pmalloc_last_error (clib_pmalloc_main_t * pm)
{
  return pm->last_err_string;
}

always_inline u32
clib_pmalloc_get_page_index (clib_pmalloc_main_t * pm, void *va)
{
  uword index = (pointer_to_uword (va) - pointer_to_uword (pm->start)) >>
    min_log2 (pm->pagesize);
...
always_inline int
clib_pmalloc_get_arena_fd (clib_pmalloc_main_t * pm, void *va)
{
  u32 index = clib_pmalloc_get_page_index (pm, va);
  return pm->arenas[pm->pages[index].arena_index].fd;
}
