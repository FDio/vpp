...
f64 clib_timebase_offset_from_sunday (u8 *day)
{
  ...
  clib_warning ("'%s' didn't convert", day);
  return 0.0;
}
...
f64 clib_timebase_summer_offset (clib_timebase_t *tb, f64 now)
{
  ...
  if (tb->daylight_time_type == CLIB_TIMEBASE_DAYLIGHT_NONE)
    return 0.0;
  ...
  clib_warning ("year start %U", format_clib_timebase_time, 
                tb->cached_year_start);

  cp->year += 1;

  tb->cached_year_end = clib_timebase_components_to_time (cp);

  clib_warning ("year end %U", format_clib_timebase_time, 
                tb->cached_year_end);
  ...
  clib_warning ("DST starts %U, ends %U",
                format_clib_timebase_time, tb->cached_summer_start,
                format_clib_timebase_time, tb->cached_summer_end);
