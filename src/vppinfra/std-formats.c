 ...
 *    int total_display_length = lenH + lenT
 *    int total_packet_length = original length
 *    int tail_display_length = lenT
 *
 * Example:
 *
 *    u8 pktbuf[2048];
...
{
  u8 *data = va_arg (*args, u8 *);
  uint total_display_length = va_arg (*args, uint);
  uint total_packet_length = va_arg (*args, uint);
  uint tail_display_length = va_arg (*args, uint);
  uint len = total_display_length;
  int i, j, index = 0;
  const int line_len = 16;
  u8 *line_hex = 0;
  u8 *line_str = 0;
