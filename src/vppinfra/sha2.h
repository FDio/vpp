  ...
  if (ctx->n_pending)
    {
      ...
      if (n_bytes < n_left)
	{
	  clib_memcpy_fast (ctx->pending.as_u8 + ctx->n_pending,
			    data, n_bytes);
      ...
      else
	{
	  ...
	  if (ctx->block_size == SHA512_BLOCK_SIZE)
	    clib_sha512_block (ctx, ctx->pending.as_u8);
	  else
	    clib_sha256_block (ctx, ctx->pending.as_u8);
	  ctx->n_pending = 0;
  ...
  if (n_bytes)
    {
      ...
      clib_memcpy_fast (ctx->pending.as_u8, data, n_bytes);
      ctx->n_pending = n_bytes;
    }
  else
    ctx->n_pending = 0;
