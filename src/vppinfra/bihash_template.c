  ...
  if (BIHASH_USE_HEAP)
    {
      ...
      rv = clib_mem_alloc_aligned (nbytes, CLIB_CACHE_LINE_BYTES);
  ...
  if (BIHASH_USE_HEAP)
    {
      ...
      for (int i = 0; i < vec_len (h->freelists); i++)
	{
	  ...
	  while (free_elt_as_u64)
	    {
	      v = BV (clib_bihash_get_value) (h, free_elt_as_u64);
	      free_elt_as_u64 = v->next_free_as_u64;
	      clib_mem_free (v);
	    }
	}
      ...
      for (i = 0; i < h->nbuckets; i++)
	{
	  ...
	  if (BIHASH_KVP_AT_BUCKET_LEVEL && b->log2_pages == 0)
	    continue;

	  clib_mem_free (BV (clib_bihash_get_value) (h, b->offset));
	}

      /* free buckets */
      clib_mem_free (h->buckets);
