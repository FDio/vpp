...
BV (clib_bihash_init2) (BVT (clib_bihash_init2_args) * a)
{
  int i;
  void *oldheap;
...
BIHASH_TEMPLATE_API void
BV (clib_bihash_init) (BVT (clib_bihash) * h, char *name, u32 nbuckets, uword memory_size)
{
...
BIHASH_TEMPLATE_API void
BV (clib_bihash_initiator_init_svm) (BVT (clib_bihash) * h, char *name, u32 nbuckets,
				     u64 memory_size)
{
  uword bucket_size;
  u8 *mmap_addr;
...
BIHASH_TEMPLATE_API void
BV (clib_bihash_responder_init_svm) (BVT (clib_bihash) * h, char *name, int fd)
{
  u8 *mmap_addr;
  u64 memory_size;
...
BIHASH_TEMPLATE_API void
BV (clib_bihash_set_kvp_format_fn) (BVT (clib_bihash) * h, format_function_t *kvp_fmt_fn)
{
  h->kvp_fmt_fn = kvp_fmt_fn;
}

BIHASH_TEMPLATE_API int
BV (clib_bihash_is_initialised) (const BVT (clib_bihash) * h)
{
  return (h->instantiated != 0);
}

BIHASH_TEMPLATE_API void
BV (clib_bihash_free) (BVT (clib_bihash) * h)
{
  int i;
...
BIHASH_TEMPLATE_API int
BV (clib_bihash_add_del_with_hash) (BVT (clib_bihash) * h, BVT (clib_bihash_kv) * add_v, u64 hash,
				    int is_add)
{
...
BIHASH_TEMPLATE_API int
BV (clib_bihash_add_del) (BVT (clib_bihash) * h, BVT (clib_bihash_kv) * add_v, int is_add)
{
...
BIHASH_TEMPLATE_API int
BV (clib_bihash_search) (BVT (clib_bihash) * h, BVT (clib_bihash_kv) * search_key,
			 BVT (clib_bihash_kv) * valuep)
{
  return BV (clib_bihash_search_inline_2) (h, search_key, valuep);
}

BIHASH_TEMPLATE_API u8 *
BV (format_bihash) (u8 *s, va_list *args)
{
