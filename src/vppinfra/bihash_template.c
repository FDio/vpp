...
static void BV (clib_bihash_instantiate) (BVT (clib_bihash) * h)
{
  ...
  __atomic_store_n (&h->instantiated, 1, __ATOMIC_RELEASE);
}
...
BV (make_working_copy) (BVT (clib_bihash) * h, BVT (clib_bihash_bucket) * b)
{
  ...
  __atomic_store_n ((u64 *) b, working_bucket.as_u64, __ATOMIC_RELEASE);
  h->working_copies[thread_index] = working_copy;
}
  ...
  if (BIHASH_KVP_AT_BUCKET_LEVEL == 0 && BV (clib_bihash_bucket_is_empty) (b))
    {
      if (is_add == 0)
	{
	  BV (clib_bihash_unlock_bucket) (b, 0 /* bucket_changed */);
	  return (-1);
	}
      ...
      *v->kvp = *add_v;
      ...
      /* unlocks the bucket */
      __atomic_store_n ((u64 *) b, tmp_b.as_u64, __ATOMIC_RELEASE);
      ...
      for (i = 0; i < limit; i++)
	{
	  if (BV (clib_bihash_key_compare) (v->kvp[i].key, add_v->key))
	      ...
	      if (is_add == 2)
		{
	      ...
	      BV (clib_bihash_unlock_bucket) (b, 1 /* bucket_changed */);
