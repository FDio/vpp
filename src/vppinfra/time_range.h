...
static inline f64 clib_timebase_now (clib_timebase_t *tb)
{
  ...
  clib_warning ("GMT: %U", format_clib_timebase_time, now);

  now += tb->timezone_offset;

  clib_warning ("zone offset: %U", format_clib_timebase_time, now);

  now += clib_timebase_summer_offset (tb, now);

  clib_warning ("summer offset: %U", format_clib_timebase_time, now);
