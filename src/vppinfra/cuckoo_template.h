...
#define clib_cuckoo_foreach_bucket(var, h, body)                       \
  do                                                                   \
    {                                                                  \
      CVT (clib_cuckoo_bucket) *__buckets = atomic_load (&h->buckets); \
      vec_foreach (var, __buckets) { body; }                           \
    }                                                                  \
  while (0)
...
again:
  buckets = atomic_load (&h->buckets);
  lookup = CV (clib_cuckoo_calc_lookup) (buckets, hash);
  do
    {
      rv = CV (clib_cuckoo_bucket_search) (
          vec_elt_at_index (buckets, lookup.bucket1), kvp,
          lookup.reduced_hash);
    }
  ...
  rv = CV (clib_cuckoo_bucket_search) (
      vec_elt_at_index (buckets, lookup.bucket2), kvp, lookup.reduced_hash);
