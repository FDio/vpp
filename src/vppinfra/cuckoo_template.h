...
#define atomic_store(x, y)    \
  ...
  while (0);
#define atomic_store_explicit(x, y, m) \
  do                                   \
    {                                  \
      *x = y;                          \
      CLIB_MEMORY_BARRIER ();          \
...
always_inline clib_cuckoo_bucket_aux_t clib_cuckoo_bucket_aux_set_use_count (
    clib_cuckoo_bucket_aux_t aux, int use_count)
{
...
always_inline clib_cuckoo_bucket_aux_t clib_cuckoo_bucket_aux_set_writer_flag (
    clib_cuckoo_bucket_aux_t aux, int writer_flag)
{
  u64 version = clib_cuckoo_bucket_aux_get_version (aux);
  int use_count = clib_cuckoo_bucket_aux_get_use_count (aux);
  return clib_cuckoo_bucket_aux_pack (version, use_count, writer_flag);
...
typedef struct
{
  ...
  _Atomic(clib_cuckoo_bucket_aux_t) aux;
...
typedef struct CV (clib_cuckoo)
{
  ...
  /** snapshot pointers owned by reader threads (per cpu) */
  CVT (clib_cuckoo_bucket) * snapshot_buckets[CLIB_MAX_MHEAPS];

  /** snapshot pointers no longer in use (per cpu) */
  CVT (clib_cuckoo_bucket) * *to_be_freed[CLIB_MAX_MHEAPS];
   ...
   * readers
   */
  volatile atomic_flag writer_lock;
...
#define clib_cuckoo_foreach_bucket(var, h) \
  vec_foreach (var, CV (clib_cuckoo_get_snapshot) (h))

always_inline void CV (clib_cuckoo_snapshot_refresh) (CVT (clib_cuckoo) * h)
{
  uword cpu_num = os_get_cpu_number ();
  ASSERT (cpu_num < CLIB_MAX_MHEAPS);
  CVT (clib_cuckoo_bucket) *b = atomic_load (&h->buckets);
  if (PREDICT_FALSE (b != h->snapshot_buckets[cpu_num]))
    {
      uword idx =
          vec_search (h->to_be_freed[cpu_num], h->snapshot_buckets[cpu_num]);
      if (~0 == idx)
        {
          vec_add1 (h->to_be_freed[cpu_num], h->snapshot_buckets[cpu_num]);
#if CLIB_CUCKOO_DEBUG_GC
          fformat (stdout,
                   "add prev snapshot %p to to-be-freed list in thread %d\n",
                   h->snapshot_buckets[cpu_num], cpu_num);
#endif
        }
      h->snapshot_buckets[cpu_num] = b;
#if CLIB_CUCKOO_DEBUG_GC
      fformat (stdout, "snapshot: %p in thread %d\n", b, cpu_num);
#endif
    }
}

always_inline CVT (clib_cuckoo_bucket) *
    CV (clib_cuckoo_get_snapshot) (CVT (clib_cuckoo) * h)
{
  ASSERT (h->snapshot_buckets[os_get_cpu_number ()]);
  return h->snapshot_buckets[os_get_cpu_number ()];
}

always_inline u64 CV (clib_cuckoo_get_nbuckets) (CVT (clib_cuckoo) * h)
{
  return vec_len (CV (clib_cuckoo_get_snapshot) (h));
}

always_inline CVT (clib_cuckoo_bucket) *
    CV (clib_cuckoo_bucket_at_index) (CVT (clib_cuckoo) * h, u64 bucket)
{
  return vec_elt_at_index (CV (clib_cuckoo_get_snapshot) (h), bucket);
}

void CV (clib_cuckoo_init) (
    CVT (clib_cuckoo) * h, const char *name, u64 nbuckets,
...
always_inline u8 clib_cuckoo_reduce_hash (u64 hash)
{
...
always_inline u64 clib_cuckoo_get_other_bucket (u64 nbuckets, u64 bucket,
                                                u8 reduced_hash)
{
  u64 mask = (nbuckets - 1);
 ...
 */
always_inline int CV (clib_cuckoo_bucket_search) (CVT (clib_cuckoo) * h,
                                                  u64 bucket,
  ...
  clib_cuckoo_bucket_foreach_idx_unrolled (i, {
    ...
    if (
        ...
        0 == memcmp (&kvp->key, &b->elts[i].key, sizeof (kvp->key)))
      {
        ...
        clib_cuckoo_bucket_aux_t bucket_aux2 = atomic_load (&b->aux);
...
again:
  ...
  do
    {
      rv = CV (clib_cuckoo_bucket_search) (h, lookup.bucket1, kvp,
                                           lookup.reduced_hash);
    }
  ...
  rv = CV (clib_cuckoo_bucket_search) (h, lookup.bucket2, kvp,
                                       lookup.reduced_hash);
