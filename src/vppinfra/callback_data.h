...
/** @brief Decclare and define a callback set type
 ...
 * currently being iterated, then the change will be applied prior to
 ...
 * prior to the next full iteration.
 */
 ...
 * iterated, then the change will be applied prior to the next full
 * iteration.
 */
 ...
 * callback is alreadyu in the set, it is replaced.  If the callback is
 ...
 * @param set_ the callback set
 * @param cbi_ the callback function index
...
do {                                                               \
  ...
  u32 sz_ = vec_len (curr_);                                       \
  u32 i_;                                                          \
  for (i_ = 0; i_ < sz_; i_++)                                     \
    {                                                              \
      CLIB_PREFETCH (&curr_[i_+1], CLIB_CACHE_LINE_BYTES, STORE);  \
      (curr_[i_].fp) (&curr_[i_], __VA_ARGS__);                    \
    }                                                              \
 } while (0)
