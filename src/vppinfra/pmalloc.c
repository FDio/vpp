...
clib_pmalloc_init (clib_pmalloc_main_t * pm, uword size)
{
  ...
  pm->start = mmap (0, size + pagesize, PROT_NONE,
  ...
  off = round_pow2 (pointer_to_uword (pm->start), pagesize) -
    pointer_to_uword (pm->start);
  ...
  munmap (pm->start + (pm->max_pages * pagesize), pagesize - off);
...
next_chunk:
  ...
  va = pm->start + ((pp - pm->pages) << pm->log2_page_sz) +
		   ...
		   u32 numa_node, u32 n_pages)
{
  ...
  va = pm->start + (vec_len (pm->pages) << pm->log2_page_sz);
				  ...
				  uword size, u32 numa_node)
{
  ...
  return pm->start + (pp->index << pm->log2_page_sz);
}
