  ...
  if (pp->chunks == 0)
    {
      ...
      u32 n_subpages = 1U << (pm->def_log2_page_sz - a->log2_page_sz);
      ...
      for (i = 0; i < n_subpages; i++)
	{
	  ...
	  c->start = start;
	  c->prev = prev;
	  c->size = pp->n_free_blocks / n_subpages;
	  start += c->size;
	  if (prev == ~0)
		   ...
		   u32 numa_node, u32 n_pages)
{
  ...
  if (a->log2_page_sz != pm->sys_log2_page_sz)
    {
      pm->error = clib_sysfs_prealloc_hugepages (numa_node, a->log2_page_sz,
						 n_pages);
  ...
  /* if new arena is using smaller page size, we need to rebuild whole
     lookup table */
  if (a->log2_page_sz < pm->lookup_log2_page_sz)
    {
      pm->lookup_log2_page_sz = a->log2_page_sz;
			   ...
			   uword size, uword align, u32 numa_node)
{
  ...
  if (a == 0)
      ...
      if (pm->default_arena_for_numa_node[numa_node] == ~0)
	{
	  ...
	  a->numa_node = numa_node;
	  a->log2_page_sz = pm->def_log2_page_sz;
	}
      else
			  ...
			  u32 ci1, u32 ci2)
{
  ...
  if (c1->start >> (a->log2_page_sz - PMALLOC_LOG2_BLOCK_SZ) !=
      c2->start >> (a->log2_page_sz - PMALLOC_LOG2_BLOCK_SZ))
    return 0;
