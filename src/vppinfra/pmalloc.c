...
format_pmalloc (u8 * s, va_list * va)
{
  ...
  s = format (s, "num-pages %u max-pages %u pagesize %uKB va-start %p",
  ...
  pool_foreach (a, pm->arenas,
    {
      u32 *page_index;
      s = format (s, "\n%Uarena '%s' pages %u numa-node %u fd %d",
		  format_white_space, indent + 2,
		  a->name, vec_len (a->page_indices), a->numa_node, a->fd);
      vec_foreach (page_index, a->page_indices)
      {
	pp = vec_elt_at_index (pm->pages, *page_index);
