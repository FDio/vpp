...
test_valloc (test_main_t * tm)
{
  ...
  ptr = clib_pmalloc_create_shared_arena (pm, "foo arena", 3 << 21, 1);
  if (ptr == 0)
    clib_error ("Failed to alloc shared arena: %s",
		clib_pmalloc_last_error (pm));

  fformat (stdout, "alloc %p\n",
	   clib_pmalloc_alloc_from_arena (pm, ptr, 6144, 4096));
  fformat (stdout, "alloc %p\n",
	   clib_pmalloc_alloc_from_arena (pm, ptr, 6144, 4096));

  clib_pmalloc_arena_t *a;
  a = clib_pmalloc_get_arena (pm, ptr);
  fformat (stdout, "arena fd is %u pa is 0x%llx\n",
	   a->fd, clib_pmalloc_get_pa (pm, ptr));
