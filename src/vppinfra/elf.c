...
format_elf_section (u8 *s, va_list *args)
{
  ...
  if (!h)
    return format (s, "%=40s%=10s%=20s%=8s%=16s%=16s%=16s", "Name", "Index",
		   "Type", "Size", "Align", "Address", "File offset");

  s = format (s, "%-40s%10d%=20U%8Lx%16d%16Lx %Lx-%Lx",
	      elf_section_name (em, es), es->index, format_elf_section_type,
	      h->type, h->file_size, h->align, h->exec_address, h->file_offset,
	      h->file_offset + h->file_size);

  if (h->flags != 0)
    {
#define _(f, i)                                                               \
  if (h->flags & ELF_SECTION_FLAG_##f)                                        \
    s = format (s, " %s", #f);
...
format_elf_segment (u8 *s, va_list *args)
{
  ...
  if (!h)
    return format (s, "%=16s%=16s%=16s%=16s", "Type", "Virt. Address",
		   "Phys. Address", "Size");

  s = format (s, "%=16U%16Lx%16Lx%16Lx%16Lx", format_elf_segment_type, h->type,
	      h->virtual_address, h->physical_address, h->memory_size,
	      h->file_offset);

  if (h->flags != 0)
    {
#define _(f, i)                                                               \
  if (h->flags & ELF_SEGMENT_FLAG_##f)                                        \
    s = format (s, " %s", #f);
...
format_elf_symbol (u8 *s, va_list *args)
{
  ...
  if (!sym)
    return format (s, "%=32s%=16s%=16s%=16s%=16s%=16s", "Symbol", "Size",
		   "Value", "Type", "Visibility", "Section");

  s = format (s, "%-32s%16Ld%16Lx%=16U%=16U%U", elf_symbol_name (t, sym),
	      sym->size, sym->value, format_elf_symbol_binding_and_type,
	      sym->binding_and_type, format_elf_symbol_visibility,
	      sym->visibility, format_elf_symbol_section_name, em,
	      sym->section_index);
...
format_elf_relocation (u8 *s, va_list *args)
{
  ...
  s = format (s, "%16Lx%16U", r->address, format_elf_relocation_type, em,
	      r->symbol_and_type & 0xff);
...
format_elf_main (u8 *s, va_list *args)
{
  ...
  s = format (s,
	      "File header: machine: %U, file type/class %U/%U, "
	      "data-encoding: %U, abi: %U version %d\n",
  ...
  s = format (s, "  entry 0x%Lx, arch-flags 0x%x", em->file_header.entry_point,
	      em->file_header.flags);
...

  {
    ...
    s = format (s, "\nSections %d at file offset 0x%Lx-0x%Lx:\n",
		fh->section_header_count, fh->section_header_file_offset,
...

  {
    ...
    s = format (s, "\nSegments: %d at file offset 0x%Lx-0x%Lx:\n",
		fh->segment_header_count, fh->segment_header_file_offset,
...
#undef _
  ...
  vec_foreach (s, em->sections)
    {
      hash_set_mem (em->section_by_name, elf_section_name (em, s),
		    s - em->sections);
      hash_set (em->section_by_start_address, s->header.exec_address,
		s - em->sections);
    }
}
...
add_symbol_table (elf_main_t *em, elf_section_t *s)
{
  ...
  if (em->first_header.file_class == ELF_64BIT)
    {
      tab->symbols = elf_get_section_contents (em, s - em->sections,
...
#undef _
  ...
  tab->string_table = elf_get_section_contents (em, s->header.link,
  ...
  vec_foreach (sym64, tab->symbols)
    {
      if (sym64->name != 0)
	hash_set_mem (tab->symbol_by_name, tab->string_table + sym64->name,
		      sym64 - tab->symbols);
    }
}
...
elf_parse (elf_main_t *em, void *data, uword data_bytes)
{
  ...
  em->first_header = h[0];
  em->need_byte_swap = CLIB_ARCH_IS_BIG_ENDIAN !=
		       (h->data_encoding == ELF_TWOS_COMPLEMENT_BIG_ENDIAN);
  elf_swap_first_header (em, &em->first_header);

  if (!(h->magic[0] == 0x7f && h->magic[1] == 'E' && h->magic[2] == 'L' &&
	h->magic[3] == 'F'))
...
#undef _
      ...
      elf_set_section_contents (em, tab->section_index, syms,
				vec_bytes (syms));
    }
  else
    {
  ...
  vec_foreach (e, em->dynamic_entries)
	...
	case ELF_DYNAMIC_ENTRY_STRING_TABLE:
	  ...
	  em->dynamic_string_table =
	    elf_get_section_contents_with_starting_address (
	      em, e->data, sizeof (u8),
	...
	case ELF_DYNAMIC_ENTRY_VERSYM:
	  em->versym = elf_get_section_contents_with_starting_address (
	    em, e->data, sizeof (em->versym[0]), &em->versym_section_index);
	  if (em->need_byte_swap)
	    {
	      uword i;
	...
	case ELF_DYNAMIC_ENTRY_VERSION_NEED:
	  em->verneed = elf_get_section_contents_with_starting_address (
	    em, e->data, sizeof (em->verneed[0]), &em->verneed_section_index);
	  if (em->need_byte_swap)
...
layout_sections (elf_main_t *em)
{
  ...
  /* Figure file offsets and exec addresses for sections. */
  {
	...
	if (s->header.flags & ELF_SECTION_FLAG_ALLOC)
	  {
	    ...
	    n_sections_with_changed_exec_address +=
	      s->exec_address_change != 0;
	...
	{
	  s = vec_elt_at_index (em->sections,
...
#undef _
	    ...
	    if (fwrite (&h, sizeof (h), 1, f) != 1)
	      {
		error = clib_error_return_unix (0, "write segment header %U",
...
#undef _
  ...
  /* Write contents for all sections. */
  {
	...
	if (s->header.type == ELF_SECTION_NO_BITS)
	  ...
	  {
	    error = clib_error_return_unix (0, "write %s section contents",
...
#undef _
	    ...
	    if (fwrite (&h, sizeof (h), 1, f) != 1)
	      {
		error = clib_error_return_unix (0, "write %s section header",
						elf_section_name (em, s));
		goto error;
	      }
	  }
...
void
elf_create_section_with_contents (elf_main_t *em, char *section_name,
				  ...
				  void *contents, uword n_content_bytes)
{
  ...
  sts = vec_elt_at_index (em->sections,
			  em->file_header.section_header_string_table_index);
  st = sts->contents;
  ...
  em->file_header.section_header_count +=
    is_new_section && s->header.type != ~0;
}

uword
elf_delete_segment_with_type (elf_main_t *em, elf_segment_type_t segment_type)
{
  uword n_deleted = 0;
  elf_segment_t *s;
