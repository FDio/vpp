...
class Union(Type):
    ...
    def get_c_def(self):
        return "typedef union {\n%s\n} %s;" % (
            "\n".join(
                [
                    "  %s %s;" % (entry[0].get_c_name(), entry[1])
                    for entry in self.type_pairs
                ]
            ),
            self.get_c_name(),
        )
...
def _discriminator_snapshot_lines(fields, struct_prefix, to_net):
    ...
    for f in fields:
        ...
        lines.append("  %s _disc_%s = %s%s;" % (disc_c, disc, struct_prefix, disc))
...
class Message(object):
    ...
    def get_swap_payload_to_be_func_def(self):
        ...
        snap = _discriminator_snapshot_lines(payload_fields, "payload->", to_net=True)
        body = snap + [
    ...
    def get_swap_payload_to_host_func_def(self):
        payload_fields = [p for p in self.fields if p.type != self.header]
        snap = _discriminator_snapshot_lines(payload_fields, "payload->", to_net=False)
        body = snap + [
...
def write_vapi_h(parser, apifilename, name, out):
    ...
    out.write("\n")
    out.write("#define DEFINE_VAPI_MSG_IDS_%s\\\n" % _munged_filename_token(f).upper())
    out.write(
        "\\\n".join(
            [
