...
class Printfun:
    ...
    for (i = 0; i < (u32) ({lfield}); i++) {{
        s = format(s, "\\n%U{n}: %U",
                   format_white_space, indent, format_{t}, (void *) &a->{n}[i], indent);
    }}
"""
...
"""
    ...
    for t in objs:
        if t.__class__.__name__ == "Enum" or t.__class__.__name__ == "EnumFlag":
            sig = (signature_scalar
                   if scalar_needs_alignment(f"vl_api_{t.name}_t") else signature)
