...
'''
    ...
    signature = '''\
static inline uword vl_api_{name}_t_nbo_calc_size (vl_api_{name}_t *a)
{{
'''
        ...
        if tname == 'Using':
            if 'length' in o.alias:
                try:
                    ...
                    # output += f" + vl_api_{o.alias.name}_t_nbo_calc_size({o.name})"
                    print("culprit:")
                    print(o)
                    print(dir(o.alias))
                    print(o.alias)
            ...
            for b in o.block:
                if b.type == 'Option':
                    ...
                    if b.fieldtype.startswith('vl_api_'):
                        ...
                        output += f" + {b.fieldtype}_nbo_calc_size(&a->{b.fieldname})"
                elif b.type == 'Array':
                    if b.lengthfield:
...
'''
    ...
    for s in services:
        ...
        write('   c = (vl_msg_api_msg_config_t) '
              ...
              '   .nbo_calc_size = vl_api_{n}_t_nbo_calc_size,\n'
              '   .is_autoendian = {auto}}};\n'
