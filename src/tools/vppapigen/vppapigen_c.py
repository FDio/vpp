...
class Printfun():
    ...
    def print_string(self, o, stream):
        ...
        if o.modern_vla:
            ...
            write('        s = format(s, "\\n%U{f}: %.*s", format_white_space, indent, '
                  'vl_api_string_len(&a->{f}) - 1, vl_api_from_api_string(&a->{f}));\n'.format(f=o.fieldname))
            write('    } else {\n')
    ...
    def print_array(self, o, stream):
        ...
        forloop = '''\
    for (i = 0; i < {l}; i++) {{
        s = format(s, "\\n%U{n}: %U", format_white_space, indent, format_{t}, &a->{n}[i], indent);
    }}
'''

        forloop_format = '''\
    for (i = 0; i < {l}; i++) {{
        s = format(s, "\\n%U{n}: {t}", format_white_space, indent, &a->{n}[i], indent);
    }}
'''
        ...
        if o.fieldtype == 'u8':
            if o.lengthfield:
                write('    s = format(s, "\\n%U{n}: %U", format_white_space, '
                      'indent, format_hex_bytes, a->{n}, a->{l});\n'.format(n=o.fieldname, l=o.lengthfield))
            else:
                write('    s = format(s, "\\n%U{n}: %U", format_white_space, '
                      'indent, format_hex_bytes, a, {l});\n'.format(n=o.fieldname, l=o.length))
            return

        l = 'a->' + o.lengthfield if o.lengthfield else o.length
        if o.fieldtype in format_strings:
            write(forloop_format.format(l=l, t=format_strings[o.fieldtype], n=o.fieldname))
        else:
            write(forloop.format(l=l, t=o.fieldtype, n=o.fieldname))
    ...
    def print_alias(self, k, v, stream):
        ...
        if 'length' in v.alias and v.alias['length'] and v.alias['type'] == 'u8':
            ...
            write('    return format(s, "{} (print not implemented)"'.format(k))
