...
def _process_service(
    svc: Any, messages_by_name: dict[str, Any]
) -> dict[str, Any]:
    reply = svc.reply
...
def parse_api_file(
    api_file: pathlib.Path, includedirs: list[str]
) -> dict[str, Any]:
    ...
    parser = vppapigen.VPPAPI(
        debug=False, filename=str(api_file), logger=vppapigen.log
    )
...
def build_descriptor(
    api_file: pathlib.Path, s: dict[str, Any]
) -> dict[str, Any]:
...
def main():
    ...
    cliparser.add_argument(
        "--stubs-output",
        action="store",
        help="Directory for final stub files (vapi_types.pyi, provider.pyi)",
    )
    cliparser.add_argument(
        "--quiet",
        action="store_true",
    ...
    for api_file in api_files:
        s = parse_api_file(api_file, [str(src_dir)])
