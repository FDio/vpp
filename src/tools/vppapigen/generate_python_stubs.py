...
def _is_vpp_root(path: str) -> bool:
    """True if path looks like a VPP source checkout root."""
    return bool(path) and os.path.isfile(
        os.path.join(path, "src", "tools", "vppapigen", "vppapigen.py")
    )


def _vpp_root_from_argv() -> str:
    """Read --vpp-root ahead of argparse.

    The root is needed at import time (vppapigen is imported from it), which is
    before main() gets to parse arguments.
    """
    for i, arg in enumerate(sys.argv):
        if arg == "--vpp-root" and i + 1 < len(sys.argv):
            return sys.argv[i + 1]
        if arg.startswith("--vpp-root="):
...
def _find_vpp_root() -> str:
    ...
    Out-of-tree plugin builds run the installed copy of this script, which sits
    outside the checkout (or with no checkout at all), so they pass --vpp-root
    explicitly; the caller is the one that knows where the sources are.

    Otherwise resolve from this script's own location (not the caller's CWD):
    the in-tree copy lives at a fixed depth below the root, so this holds for
    both `make python-api-stubs` and the in-tree build, and needs no git.
    """
    root = _vpp_root_from_argv()
    if not root:
        # <root>/src/tools/vppapigen/<this script>
        root = os.path.dirname(os.path.dirname(os.path.dirname(SCRIPT_DIR)))
    if not _is_vpp_root(root):
        raise SystemExit(f"'{root}' is not a VPP source tree; pass --vpp-root")
    return root
...
def main():
    ...
    cliparser.add_argument(
        ...
        default=BASE_DIR,
        help=(
            "VPP source checkout to generate from. Required when running an "
            "installed copy of this script from outside the checkout (see "
            "_find_vpp_root); it is consumed before argparse runs."
        ),
    )
    cliparser.add_argument(
        "--srcdir",
