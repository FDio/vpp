...
report_actors (void)
{
      ...
      while (ap)
	{
	  ...
	  if (ninstance > 1)
	      ...
	      for (j = 0; j < ninstance; j++)
		{
	      ...
	      linreg (x, y, ninstance, &ap->a, &ap->b, &ap->min, &ap->max,
		      &ap->mean, &ap->r);
	    }
	  else
	    {
  ...
  if (summary_stats)
    printf ("NInst       Offset       Slope    T(Ninst)         Min         "
	    "Max         Avg   %%InstTime           R    Key");
  else
	  ...
	  if (summary_stats)
	    {
	      ...
	      printf ("%6ld %11.2f %11.2f %11.2f %11.2f %11.2f %11.2f %11.2f "
		      "%11.2f 0x%08lx ",
		      ap->ninst, ap->a, ap->b, e10k, ap->min, ap->max,
		      ap->mean, pcttot, ap->r, ap->key);
	    }
	  else
	    printf ("%6ld %11.2f %11.2f %11.2f 0x%08lx ", ap->ninst, ap->a,
		    ap->b, e10k, ap->key);
  ...
  for (i = 0; i < nevents; i++)
	...
	case 0: /* not in state */
	  ...
	  if (event_code == exit_event)
		...
		{
		  fprintf (stderr, "End event before start event, key 0x%x.",
	  ...
	  vec_add1 (tp->start_time,
		    (tp->thread_timestamp + (now - tp->time_thread_on_cpu)));
#ifdef HAVING_TROUBLE
	  printf (
	    "sp %lld key 0x%x start time %llu\n",
	  ...
	  if (!tp->dup_event[sp])
	    {
	      ...
	      record_instance (tp->start_datum[sp],
			       (end_time - tp->start_time[sp]));
