...
u32
find_or_add_event (void *s_arg, char *datum_format)
{
  uword *p;
  u8 *copy_s;
 ...
 */
int
write_cpel_header (FILE *ofp, u32 nsections)
{
...
write_event_defs (FILE *ofp)
{
  ...
  for (i = 0; i < vec_len (the_event_definitions); i++)
	...
	ntohl (this_event_definition->datum_format);
      ...
      if (fwrite (this_event_definition, sizeof (the_event_definitions[0]), 1,
		  ofp) != 1)
	return (0);
    }
  return (1);
 ...
 */
int
write_events (FILE *ofp, u64 clock_ticks_per_second)
{
  cpel_section_header_t sh;
  ...
  sh.data_length =
    ntohl (number_of_events * sizeof (e) + sizeof (event_section_header_t));
...
write_track_defs (FILE *ofp)
{
  ...
  sh.data_length =
    ntohl (vec_len (the_tracks) * sizeof (this_track_definition[0]) +
  ...
  for (i = 0; i < vec_len (the_tracks); i++)
	...
	ntohl (the_tracks[i].strtab_offset);
      ...
      if (fwrite (this_track_definition, sizeof (this_track_definition[0]), 1,
		  ofp) != 1)
	return (0);
    }
  return (1);
 ...
 */
static int
alpha_compare_tracks (const void *a1, const void *a2)
{
  int i;
