...
cpel_dump_file_header (cpel_file_header_t *fh, int verbose, FILE *ofp)
{
  ...
  if (verbose)
    {
      fprintf (
	ofp, "CPEL file: %s-endian, version %d\n",
...
compute_state_statistics (int verbose, FILE *ofp)
{
  ...
  for (i = 0; i < vec_len (bound_tracks); i++)
	...
	{
	  bp->mean_ticks_in_state =
	    bp->total_ticks_in_state / ((f64) vec_len (bp->ticks_in_state));
	}
      /* Accumulate sum: (Xi-Xbar)**2 */
      for (j = 0; j < vec_len (bp->ticks_in_state); j++)
	{
	  fticks = bp->ticks_in_state[j];
	  bp->variance_ticks_in_state += (fticks - bp->mean_ticks_in_state) *
      ...
      if (vec_len (bp->ticks_in_state) > 1)
	{
	  bp->variance_ticks_in_state /=
	    (f64) (vec_len (bp->ticks_in_state) - 1);
	}
    }
}
...
sort_state_statistics (sort_t type, FILE *ofp)
{
  ...
  qsort (bound_tracks, vec_len (bound_tracks), sizeof (bound_track_t),
	 compare);
}
