...
static unsigned char zi_bits[] = {
  ...
  0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x82,
  0x00, 0x00, 0x00, 0x84, 0x00, 0x00, 0x00, 0x88, 0x00, 0x00, 0x00, 0x90, 0x00,
  0x00, 0x00, 0xa0, 0x00, 0x00, 0x00, 0xc0, 0x00, 0x00, 0xfc, 0xff, 0x00, 0x00,
  0x00, 0xc0, 0x00, 0x00, 0x00, 0xa0, 0x00, 0x00, 0x00, 0x90, 0x00, 0x00, 0x00,
  0x88, 0x00, 0x00, 0x00, 0x84, 0x00, 0x00, 0x00, 0x82, 0x00, 0x00, 0x00, 0x80,
...
dtime_per_pixel (v1_geometry_t *vp)
{
...
void
format_popbox_string (char *tmpbuf, int len, event_t *ep, event_def_t *edp)
{
  char *fp;
...
next_snapshot (void)
{
  ...
  if (summary_mode != next->summary_mode)
	...
	NULL,
	(gpointer) (unsigned long long) (summary_mode ? NOSUMMARY_BUTTON :
							SUMMARY_BUTTON));
    }
...
write_snapshot (void)
{
  ...
  for (snap = s_snapshots; !error && snap != NULL; snap = snap->next)
    {
      if (fwrite (&snap->geometry, sizeof (snap->geometry), 1, file) != 1 ||
	  fwrite (&snap->show_event, sizeof (snap->show_event), 1, file) !=
	    1 ||
	  fwrite (snap->pidvec, sizeof (pid_sort_t) * g_npids, 1, file) != 1 ||
	  fwrite (&snap->vscroll_value, sizeof (snap->vscroll_value), 1,
		  file) != 1 ||
	  fwrite (&snap->summary_mode, sizeof (snap->summary_mode), 1, file) !=
	    1 ||
...
read_snapshot (void)
{
  ...
  while (!error && !feof (file))
      ...
      if (fread (snap->pidvec, len, 1, file) != 1)
	{
      ...
      if (fread (&snap->vscroll_value, sizeof (snap->vscroll_value), 1,
		 file) != 1 ||
	  fread (&snap->summary_mode, sizeof (snap->summary_mode), 1, file) !=
	    1 ||
  ...
  if (error)
    ...
    {
      char buf[64];
      snprintf (buf, sizeof (buf), "Read %d snapshots from snapshots.g2",
		records);
      message_line (buf);
    }
}
...
static int
toggle_event_select (GdkEventButton *event, v1_geometry_t *vp)
{
...
static void
toggle_track_select (GdkEventButton *event, v1_geometry_t *vp)
{
  int i;
  int pid_index;
  int y, delta_y;
...
static void
move_current_track (GdkEventButton *event, v1_geometry_t *vp, move_type type)
{
  int i;
  int pid_index;
  int y, delta_y;
...
void
zoom_event (GdkEventButton *e1, GdkEventButton *e2, v1_geometry_t *vp)
{
  double xrange;
  double time_per_pixel;
  ...
  center_on_pixel =
    (double) ((e2->x + e1->x) / 2.00) - (double) vp->pid_ax_width;
...
configure_event (GtkWidget *widget, GdkEventConfigure *event)
{
  ...
  pm = gdk_pixmap_new (widget->window, widget->allocation.width,
		       widget->allocation.height, -1);
  gdk_draw_rectangle (pm, widget->style->white_gc, TRUE, 0, 0,
  ...
  s_v1->npids =
    (s_v1->total_height - s_v1->time_ax_height) / s_v1->strip_height;
...
expose_event (GtkWidget *widget, GdkEventExpose *event)
{
  gdk_draw_pixmap (widget->window,
		   widget->style->fg_gc[GTK_WIDGET_STATE (widget)], pm,
...
anomaly_statistics_init (void)
{
  ...
  for (i = 0; i < g_nevents; i++)
      ...
      else
	{
	  s_v1->mins[pid->pid_index] = (fdata < s_v1->mins[pid->pid_index]) ?
					 fdata :
					 s_v1->mins[pid->pid_index];
	  s_v1->maxes[pid->pid_index] = (fdata > s_v1->maxes[pid->pid_index]) ?
					  fdata :
  ...
  for (i = 0; i < vec_len (s_v1->means); i++)
    s_v1->means[i] =
      s_v1->matches[i] ? (s_v1->means[i] / (f64) s_v1->matches[i]) : 0.0;
  ...
  for (i = 0; i < vec_len (s_v1->variances); i++)
    s_v1->variances[i] =
      s_v1->matches[i] ? (s_v1->variances[i] / (f64) s_v1->matches[i]) : 0.0;
  ...
  for (i = 1; i <= g_nevents; i++)
	  ...
	  s_v1->two_stddevs[pid->pid_index])
	{
	  ...
	  for (i = 0; i < vec_len (s_v1->means); i++)
	    {
	      if (s_v1->matches[i] > 0)
		s = format (s, "{%.0f, %.0f, %.0f, %.0f} ", s_v1->mins[i],
			    s_v1->maxes[i], s_v1->means[i],
...
boolean
anomaly_search_callback (char *s)
{
...
boolean
anomaly_threshold_callback (char *s)
{
  f64 new_threshold;
...
init_track_colors (void)
{
  ...
  if (s_color != NULL)
    {
      gdk_colormap_free_colors (gtk_widget_get_colormap (da), s_color,
				g_npids);
   ...
   * values.
   */
  gdk_colormap_alloc_colors (gtk_widget_get_colormap (da), s_color,
			     g_npids + 1, FALSE, TRUE, dont_care);
}
...
static void
chase_event_etc (enum chase_mode mode)
{
...
static void
print_ps_header (v1_geometry_t *vp, char *filename)
{
  time_t now;
...
static boolean
print_screen_callback (char *filename)
{
...
int
event_time_cmp (const void *a, const void *b)
{
  const event_t *e1 = a;
  const event_t *e2 = b;
 ...
 ****************************************************************************/
static void
slew_tracks (v1_geometry_t *vp, enum view1_button_click which)
{
  event_t *ep;
  pid_sort_t *pp;
  int pid_index;
...
static void
view1_button_click_callback (GtkButton *item, gpointer data)
{
...
void
view1_print_callback (GtkToggleButton *notused, gpointer nu2)
{
  modal_dialog ("Print Screen (PostScript format) to file:",
		"Invalid file: Print Screen to file:", "g2.ps",
		print_screen_callback);
}
...
static void
view1_hscroll (GtkAdjustment *adj, GtkWidget *notused)
{
  ulonglong current_width;
...
static void
view1_vscroll (GtkAdjustment *adj, GtkWidget *notused)
{
...
view1_init (void)
{
  ...
  s_view1_vsadj =
    gtk_adjustment_new (0.0 /* initial value */, 0.0 /* minimum value */,
			2000.0 /* maximum value */, 0.1 /* step increment */,
			10.0 /* page increment */, 10.0 /* page size */);
  ...
  gtk_box_pack_start (GTK_BOX (s_view1_vmenubox), s_view1_pad1, TRUE, FALSE,
		      0);

#endif

  gtk_box_pack_start (GTK_BOX (s_view1_vmenubox), s_view1_topbutton, FALSE,
		      FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_vmenubox), s_view1_vscroll, TRUE, TRUE,
		      0);

  gtk_box_pack_start (GTK_BOX (s_view1_vmenubox), s_view1_bottombutton, FALSE,
		      FALSE, 0);
  ...
  gtk_box_pack_start (GTK_BOX (s_view1_hbox), s_view1_vmenubox, FALSE, FALSE,
		      0);
  ...
  gtk_signal_connect (GTK_OBJECT (s_view1_anomalythresholdbutton), "clicked",
		      GTK_SIGNAL_FUNC (view1_button_click_callback),
  ...
  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox), s_view1_startbutton, FALSE,
		      FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox), s_view1_hscroll, TRUE, TRUE,
		      0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox), s_view1_endbutton, FALSE,
		      FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox), s_view1_zoominbutton, FALSE,
		      FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox), s_view1_anomalybutton, FALSE,
		      FALSE, 0);
  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox), s_view1_anomalynextbutton,
		      FALSE, FALSE, 0);
  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox), s_view1_searchbutton, FALSE,
		      FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox), s_view1_srchagainbutton,
		      FALSE, FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox), s_view1_zoomoutbutton, FALSE,
		      FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_vbox), s_view1_hbox, TRUE, TRUE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_vbox), s_view1_hmenubox, FALSE, FALSE,
		      0);
  ...
  gtk_box_pack_start (GTK_BOX (s_view1_vbox), s_view1_hmenubox2, FALSE, FALSE,
		      0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox2), s_view1_snapbutton, FALSE,
		      FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox2), s_view1_nextbutton, FALSE,
		      FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox2), s_view1_delbutton, FALSE,
		      FALSE, 0);
  ...
  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox2),
		      s_view1_time_slew_left_button, FALSE, FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox2),
		      s_view1_time_slew_right_button, FALSE, FALSE, 0);

  gtk_box_pack_start (GTK_BOX (s_view1_hmenubox2),
		      s_view1_anomalythresholdbutton, FALSE, FALSE, 0);
  ...
  zi_source =
    gdk_bitmap_create_from_data (NULL, (char *) zi_bits, zi_width, zi_height);
  zi_mask =
    gdk_bitmap_create_from_data (NULL, (char *) zi_bkgd, zi_width, zi_height);

  zi_cursor = (GdkCursor *) gdk_cursor_new_from_pixmap (
    zi_source, zi_mask, &fg_black, &bg_white, zi_x_hot, zi_y_hot);
...
void
line_print (int x1, int y1, int x2, int y2)
{
 ...
 ****************************************************************************/
GdkRectangle *
tbox_print (char *s, int x, int y, enum view1_tbox_fn function,
	    GdkRectangle *rp)
{
...
GdkRectangle *
tbox (char *s, int x, int y, enum view1_tbox_fn function)
{
  static GdkRectangle update_rect;
  gint lbearing, rbearing, width, ascent, descent;

  gdk_string_extents (g_font, s, &lbearing, &rbearing, &width, &ascent,
		      &descent);
  ...
  switch (function)
    ...
    case TBOX_DRAW_BOXED:
      gdk_draw_rectangle (pm, da->style->white_gc, TRUE, x,
			  y - (ascent + descent + 3), width + 2,
			  ascent + descent + 3);

      gdk_draw_rectangle (pm, da->style->black_gc, FALSE, x,
			  y - (ascent + descent + 3), width + 2,
			  ascent + descent + 3);

      gdk_draw_string (pm, g_font, da->style->black_gc, x + 1, y - 1,
		       (const gchar *) s);
      /* NOTE FALLTHROUGH */
 ...
 *   BATCH_NEW:      just entered a batch, so initialize the area to update
 *from scratch BATCH_EXISTING: have drawn at least one line in batch mode, so
 *the update area should only be expanded from now on to include the union of
 *the "rectangular hull" of all lines
 ****************************************************************************/
...
void
line (int x1, int y1, int x2, int y2, enum view1_line_fn function)
{
  GdkRectangle update_rect;
  GdkGC *gc = NULL;
...
static void
display_pid_axis (v1_geometry_t *vp)
{
  int y, i, label_tick;
...
void
view1_read_events_callback (void)
{
  int max_vis_index;
...
static void
display_event_data (v1_geometry_t *vp)
{
  int start_index;
  int pid_index;
  int x, y;
...
static void
display_time_axis (v1_geometry_t *vp)
{
  int x, y, i;
  int xoffset, nticks;
  char tmpbuf[128];
  ...
  for (i = 0; i < nticks; i++)
    {
      /* Tick mark */
      line (x + xoffset, y - 3, x + xoffset, y + 3,
	    LINE_DRAW_BLACK + s_print_offset);
...
void
view1_about (char *tmpbuf)
{
  int nsnaps;
  snapshot_t *snaps;

  snprintf (tmpbuf + strlen (tmpbuf), 128,
	    "Minvistime %lld\nMaxvistime %lld\n", s_v1->minvistime,
	    s_v1->maxvistime);
