...
char *general_help =
  ...
  "\n"
  "To view vppinfra-format (.../open-repo/vppinfra/vppinfra/elog.h) event "
  "data:\n"
  "g2 --clib <filename>\n"
...
void
modal_dialog (char *label_text, char *retry_text, char *default_value,
	      boolean (*cb) (char *))
{
...
{
  char *filename =
    (char *) gtk_file_selection_get_filename (GTK_FILE_SELECTION (s_filesel));
...
read_eventdef_callback (GtkToggleButton *item, gpointer data)
{
  ...
  gtk_signal_connect (GTK_OBJECT (GTK_FILE_SELECTION (s_filesel)->ok_button),
		      "clicked", GTK_SIGNAL_FUNC (get_eventdef_name), NULL);

  gtk_signal_connect_object (
    GTK_OBJECT (GTK_FILE_SELECTION (s_filesel)->ok_button), "clicked",
    GTK_SIGNAL_FUNC (gtk_widget_destroy), (gpointer) s_filesel);

  gtk_signal_connect_object (
    GTK_OBJECT (GTK_FILE_SELECTION (s_filesel)->cancel_button), "clicked",
    GTK_SIGNAL_FUNC (gtk_widget_destroy), (gpointer) s_filesel);
...
{
  char *filename =
    (char *) gtk_file_selection_get_filename (GTK_FILE_SELECTION (s_eventsel));
...
read_events_callback (GtkToggleButton *item, gpointer data)
{
  ...
  gtk_signal_connect (GTK_OBJECT (GTK_FILE_SELECTION (s_eventsel)->ok_button),
		      "clicked", GTK_SIGNAL_FUNC (get_events_name), NULL);

  gtk_signal_connect_object (
    GTK_OBJECT (GTK_FILE_SELECTION (s_eventsel)->ok_button), "clicked",
    GTK_SIGNAL_FUNC (gtk_widget_destroy), (gpointer) s_eventsel);

  gtk_signal_connect_object (
    GTK_OBJECT (GTK_FILE_SELECTION (s_eventsel)->cancel_button), "clicked",
    GTK_SIGNAL_FUNC (gtk_widget_destroy), (gpointer) s_eventsel);
  ...
  if (ticks_per_ns_set)
  ...
  else
    {
      ...
      modal_dialog ("Please enter clock ticks per nanosecond",
		    "Invalid: Please enter clock ticks per nanosecond", tmpbuf,
		    get_ticks_per_ns);
    }
}
...
void
infobox_size_request (GtkWidget *widget, GtkRequisition *req,
		      gpointer user_data)
{
...
infobox (char *label_text, char *text)
{
  ...
  gtk_signal_connect (GTK_OBJECT (entry), "size-request",
		      GTK_SIGNAL_FUNC (infobox_size_request), (gpointer) text);
...
read_cpel_callback (GtkToggleButton *item, gpointer data)
{
  ...
  gtk_signal_connect (GTK_OBJECT (GTK_FILE_SELECTION (s_filesel)->ok_button),
		      "clicked", GTK_SIGNAL_FUNC (get_cpel_name), NULL);
...
read_clib_callback (GtkToggleButton *item, gpointer data)
{
  ...
  gtk_signal_connect (GTK_OBJECT (GTK_FILE_SELECTION (s_filesel)->ok_button),
		      "clicked", GTK_SIGNAL_FUNC (get_clib_name), NULL);
...
menu1_init (void)
{
  ...
  s_readeventsclock =
    gtk_menu_item_new_with_label ("Read Event Log with Different Clock Rate");
