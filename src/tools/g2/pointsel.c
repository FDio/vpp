...
static void
point_select_callback (GtkToggleButton *item, gpointer data)
{
  int i = (int) (unsigned long long) data;

  g_eventdefs[i].selected =
    gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (s_event_buttons[i]));
...
static void
button_click_callback (GtkButton *item, gpointer data)
{
  int i;
  ...
  switch (click)
    ...
    case ALL_BUTTON:
      for (i = 0; i < g_neventdefs; i++)
	{
	  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (s_event_buttons[i]),
					TRUE);
...
static void
scroll_callback (GtkAdjustment *adj, GtkWidget *notused)
{
  int i;
...
point_selector_init (void)
{
  ...
  gtk_box_pack_start (GTK_BOX (s_pointselbuttons), s_allbutton, FALSE, FALSE,
		      0);
  gtk_box_pack_start (GTK_BOX (s_pointselbuttons), s_nonebutton, FALSE, FALSE,
		      0);

  gtk_widget_show (s_pointselbuttons);
  gtk_widget_ref (s_pointselbuttons);

  gtk_box_pack_start (GTK_BOX (s_pointselbox), s_pointselbuttons, FALSE, FALSE,
		      0);
...
static void
add_event (ulong event, char *name, char *format)
{
  int i;
  event_def_t *ep;
...
void
add_event_from_cpel_file (ulong event, char *event_format, char *datum_format)
{
  event_def_t *ep;
...
void
add_event_from_clib_file (unsigned int event, char *name,
...
int
event_def_cmp (const void *a1, const void *a2)
{
...
boolean
read_event_definitions (char *filename)
{
  char tmpbuf[128];
...
event_def_t *
find_event_definition (ulong code)
{
  ...
  while (1)
	...
	use_placeholder:
	  ...
	  edp->event = code;
	  edp->format = "0x%x";
	  snprintf (placeholder_string, sizeof (placeholder_string), "E%lu",
		    code);
