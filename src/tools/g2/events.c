...
int
pid_cmp (const void *a1, const void *a2)
{
...
read_events (char *filename)
{
  ...
  s_pidhash =
    (pid_data_t **) g_malloc0 (PIDHASH_NBUCKETS * sizeof (pid_data_t *));
...
void
add_cpel_event (ulonglong delta, ulong track, ulong event, ulong datum)
{
  event_t *ep;
...
void
add_clib_event (double delta, unsigned short track, unsigned short event,
		unsigned int index)
{
  event_t *ep;
...
boolean
unmapfile (char *addr, ulong size)
{
