...
typedef struct
{
  ...
  u32 n_clients;  /* Number of client threads to allocate */
  u32 n_stream_clients; /* Number of QUIC stream clients */
  u64 tx_total;
  u64 rx_total;
...
recv_data_chunk (echo_main_t * em, echo_session_t * s, u8 * rx_buf)
{
  ...
  n_to_read = clib_min (svm_fifo_max_dequeue (s->rx_fifo), s->bytes_to_receive);
  if (!n_to_read)
    return;
...
clients_run (echo_main_t * em)
{
  ...
  start_time = clib_time_now (&em->clib_time);
  while (em->n_clients_connected | (em->state < STATE_READY))
...
void quic_echo_process_opts (int argc, char **argv)
{
  ...
  while (unformat_check_input (a) != UNFORMAT_END_OF_INPUT)
	...
	em->fifo_size = tmp << 10;
      else if (unformat (a, "nclients %d/%d", &em->n_clients, &em->n_stream_clients))
	;
...
main (int argc, char **argv)
{
  ...
  u32 n_clients = clib_max (em->n_clients, 1) * clib_max (em->n_stream_clients, 1);
