...
client_handle_fifo_event_rx (echo_main_t * em, session_fifo_event_t * e,
                             u8 *rx_buf)
{
  int n_read, n_read_now, n_to_read, i;
  svm_fifo_t *rx_fifo;
  session_t *s;
...
{
  f64 start_time, deltat, timeout = 100;
  ...
  while (!em->time_to_stop)
    {
      for (i = 0; i < vec_len (active_connections); i++)
	{
	  delete_session = 1;
	  s = pool_elt_at_index (em->sessions, active_connections[i]);
