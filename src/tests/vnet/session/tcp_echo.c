...
send_data_chunk (echo_main_t * em, echo_session_t * s)
{
  ...
  written = app_send_stream ((app_session_t *) s, test_data + test_buf_offset,
			     bytes_this_chunk, 0);
...
server_handle_rx (echo_main_t * em, session_event_t * e)
{
  ...
  do
    {
      /* The option here is to limit ourselves to max_dequeue or read
       * even the data that was enqueued while we were dequeueing and which
       * now has an rx event in the mq. Either of the two options work. */
