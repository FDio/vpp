...
{
  u32 enq_space, min_chunk = 16 << 10, test_buf_len;
...
clients_run (echo_main_t *em)
{
  ...
  for (i = 0; i < em->n_clients; i++)
    {
      sp = pool_elt_at_index (em->sessions, i);
      sp->bytes_to_send = em->bytes_to_send;
      if (!em->no_return)
	sp->bytes_to_receive = em->bytes_to_send;
    }
  ...
  while (!em->time_to_stop)
    {
      for (i = 0; i < em->n_clients; i++)
	{
	  if (pool_is_free_index (em->sessions, i))
	    continue;
	  ...
	  if (PREDICT_FALSE(delete_session == 1))
	    {
	      em->tx_total += sp->bytes_sent;
	      em->rx_total += sp->bytes_received;
	      client_disconnect (em, sp);
	    }
	}
