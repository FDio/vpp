...
{
  u32 bytes_to_snd, enq_space, min_chunk = 64 << 10;
...
client_send_data (echo_main_t * em)
{
  ...
  end_time = clib_time_now (&em->clib_time);
  delta = end_time - start_time;
  transfer_type = em->no_return ? "full-duplex" : "half-duplex";
  clib_warning("%lld bytes (%lld mbytes, %lld gbytes) in %.2f seconds",
	       em->bytes_to_send, em->bytes_to_send / (1ULL << 20),
	       em->bytes_to_send / (1ULL << 30), delta);
  clib_warning("%.2f bytes/second %s", ((f64 ) em->bytes_to_send) / (delta),
	       transfer_type);
  clib_warning("%.4f gbit/second %s",
	       (((f64 ) em->bytes_to_send * 8.0) / delta / 1e9), transfer_type);
