...
static void
send_test_chunk (udp_echo_main_t * utm, app_session_t *s,
		 u32 bytes)
{
  ...
  while (bytes_to_snd > 0 && !utm->time_to_stop)
    {
      rv = app_send (s, test_data + test_buf_offset, bytes_to_snd, 1);
      if (rv > 0)
	{
	  bytes_to_snd -= rv;
	  test_buf_offset += rv;
...
void
server_handle_fifo_event_rx (udp_echo_main_t * utm,
  ...
  if (utm->do_echo)
    {
      do
	{
	  rv = app_send_stream (s, utm->rx_buf, vec_len(utm->rx_buf), 1);
	}
