...
static void
send_test_chunk (uri_udp_test_main_t * utm, session_t *s,
		 u32 bytes)
{
  u32 bytes_to_snd, max_enqueue, actual_write;
  ...
  while (bytes_to_snd > 0 && !utm->time_to_stop)
      ...
      if (!s->is_vc)
	{
	  ...
	  actual_write = clib_min (bytes_to_snd, max_enqueue);
	  hdr.data_length = actual_write;
	  hdr.data_offset = 0;
	  clib_memcpy ((u8 *) &hdr.rmt_ip, &s->rmt_ip, sizeof(ip46_address_t));
	  hdr.is_ip4 = s->is_ip4;
	  hdr.rmt_port = s->rmt_port;
	  memset(&hdr.lcl_ip, 0, sizeof (ip46_address_t));
	  hdr.lcl_port = 0;
      ...
      else
	{
      ...
      rv = svm_fifo_enqueue_nowait (f, actual_write,
...
static void
recv_test_chunk (uri_udp_test_main_t * utm, session_t * s)
{
  session_dgram_header_t hdr;
  int to_read, rv;
  u32 offset;
  ...
  if (!s->is_vc)
    {
      svm_fifo_peek (s->rx_fifo, 0, sizeof (hdr), (u8 *)&hdr);
      to_read = hdr.data_length - hdr.data_offset;
      offset = hdr.data_offset + SESSION_CONN_HDR_LEN;
      rv = svm_fifo_peek (s->rx_fifo, offset, to_read, utm->rx_buf);
      hdr.data_offset += rv;
      if (hdr.data_offset == hdr.data_length)
	{
	  offset = hdr.data_length + SESSION_CONN_HDR_LEN;
	  svm_fifo_dequeue_drop (s->rx_fifo, offset);
	}
      else
	svm_fifo_overwrite_head (s->rx_fifo, (u8 *)&hdr,
	                         sizeof(session_dgram_pre_hdr_t));
    }
  else
    {
      to_read = clib_max (svm_fifo_max_dequeue (s->rx_fifo),
                          vec_len (utm->rx_buf));
      rv = svm_fifo_dequeue_nowait (s->rx_fifo, to_read, utm->rx_buf);
      ASSERT (rv == to_read);
    }
}
...
vl_api_bind_uri_reply_t_handler (vl_api_bind_uri_reply_t * mp)
{
  ...
  clib_memcpy (&session->lcl_ip, mp->lcl_ip, sizeof(ip46_address_t));
  session->is_ip4 = mp->lcl_is_ip4;
  session->lcl_port = mp->lcl_port;

  utm->state = utm->is_vc ? STATE_BOUND : STATE_READY;
}
...
vl_api_accept_session_t_handler (vl_api_accept_session_t * mp)
{
  ...
  if (mp->server_event_queue_address)
  ...
  else
    {
      ...
      session->rx_fifo = rx_fifo;
      session->tx_fifo = tx_fifo;
      clib_memcpy (&session->rmt_ip, mp->ip, sizeof (ip46_address_t));
...
vl_api_connect_session_reply_t_handler (vl_api_connect_session_reply_t * mp)
{
  ...
  if (mp->client_event_queue_address)
      ...
      if (!unformat (input, "%U", unformat_uri, sep))
	{
      ...
      unformat_free (input);
      clib_memcpy (&session->rmt_ip, &sep->ip, sizeof (ip46_address_t));
      session->rmt_port = sep->port;
    }
  utm->state = STATE_READY;
}
			     ...
			     session_fifo_event_t * e)
{
  ...
  recv_test_chunk (utm, &utm->sessions[rx_fifo->client_session_index]);

  if (utm->do_echo)
    {
      do
	{
	  rv = svm_fifo_enqueue_nowait (tx_fifo, vec_len(utm->rx_buf),
		                        utm->rx_buf);
	}
      while (rv == -2);
...
{
  u8 wait_for_state = utm->is_vc ? STATE_BOUND : STATE_READY;
  application_send_attach (utm);
