...
static int
send_bulk (char *msg, int len, int repeats)
{
  for (int i = 0; i < repeats; i++)
    {
      vac_write (msg, len);
...
vat2_exec_benchmark_ping_async (int repeats)
{
  ...
  while (msgs_to_send > 0)
    {
      result_ready = 0;
      if (msgs_to_send > max_messages)
	n = send_bulk ((char *) &mp, sizeof (mp), max_messages);
      else
	n = send_bulk ((char *) &mp, sizeof (mp), msgs_to_send);
      msgs_to_send -= n;
      while (result_ready < n)
	{
	  ;
	}
    }
