...
int
vl_api_u8_string_fromjson (cJSON *o, u8 *s, int len)
{
...
vl_api_prefix_t_fromjson (void *mp, int *len, cJSON *o, vl_api_prefix_t *a)
{
  ...
  unformat_init_string (&input, p, strlen (p));
  int plen;
  if (unformat (&input, "%U/%d", unformat_ip4_address, &a->address.un.ip4,
		&plen))
    a->address.af = ADDRESS_IP4;
  else if (unformat (&input, "%U/%d", unformat_ip6_address, &a->address.un.ip6,
		     &plen))
...
u8 *
format_vl_api_gbp_scope_t (u8 *s, va_list *args)
{
  return 0;
}
uword
