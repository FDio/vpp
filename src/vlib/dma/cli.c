...
format_config (u8 *s, va_list *args)
{
  ...
  s = format (s, "%-12s%-12s%-24s%-24s\n", "ID", "Backend ID", "Data Addr",
	      "Results Addr");
  s =
    format (s, "%-12d%-12d%-24p%-24p\n", dma_main.configs - config,
	    config->backend_index, config->backend_data, config->result_data);

  s = format (s, "%-12s%-12s%-12s%-12s%-12s\n", "Template", "stride",
	      "src_offset", "dst_offset", "size_offset");
  s = format (s, "%-12s%-12d%-12d%-12d%-12d\n", "", config->template.stride,
	      config->template.src_ptr_offset, config->template.dst_ptr_offset,
