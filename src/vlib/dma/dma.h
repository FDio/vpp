...
vlib_dma_backend_transfer (vlib_dma_config_t *config,
			   vlib_dma_transfer_t *args, u32 n_transfers)
{
  for (int i = 0; i < n_transfers; i++)
    {
      vlib_dma_transfer_t *transfer = args + i;
      *(u8 **) (DMA_DESC_ADDR (DST, i)) = transfer->dst;
      *(u8 **) (DMA_DESC_ADDR (SRC, i)) = transfer->src;
      *(u32 *) (DMA_DESC_ADDR (SIZE, i)) = transfer->size;
      config->n_transfers++;
      config->n_bytes += transfer->size;
      config->template.n_enq++;
    }
}

static_always_inline void
vlib_dma_cpu_transfer (vlib_dma_config_t *config, vlib_dma_transfer_t *args,
		       u32 n_transfers)
{
  for (int i = 0; i < n_transfers; i++)
    {
      ...
      config->cpu_bytes += transfer->size;
      config->cpu_transfers++;
    }
}
		   ...
		   vlib_dma_transfer_t *args, u32 n_transfers, u32 cookie)
{
  ...
  if (PREDICT_FALSE (!config->template.driver_data))
    {
      if (!config->cpu_fallback)
	return ~0;

      if (config->fn->dma_transfer (vm, config_index, n_transfers, cookie))
	return ~0;

      vlib_dma_cpu_transfer (config, args, n_transfers);
      return 0;
    }

  config->template.n_enq = 0;
  vlib_dma_backend_transfer (config, args, n_transfers);
