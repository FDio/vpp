...
typedef struct
{
  ...
  void (*add_dma_transfer) (void *backend_data, u8 *dst, u8 *src, u16 size);
  u16 (*do_dma_transfer) (void *backend_data);
  u16 (*get_completed) (void *backend_data, vlib_dma_completion_cb_fn_t *cb);
  int (*configure) (void *backend, vlib_main_t *vm, u32 flags, u16 size,
...
typedef struct
{
  ...
  u32 backend_index; /* assigned backends */
...
vlib_dma_add_transfer (u32 config_index, u8 *dst, u8 *src, u16 size)
{
  vlib_dma_main_t *dm = &dma_main;
  vlib_dma_config_t *config = dm->configs + config_index;
  config->fn->add_dma_transfer (config->backend_data, dst, src, size);
  config->n_transfers++;
  config->n_bytes += size;
}

static_always_inline u16
vlib_dma_transfer (u32 config_index)
{
  vlib_dma_main_t *dm = &dma_main;
  vlib_dma_config_t *config = dm->configs + config_index;
  return config->fn->do_dma_transfer (config->backend_data);
}
