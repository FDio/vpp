...
typedef void (*vlib_dma_completion_cb_fn_t) (vlib_main_t *vm, uword cookie);

typedef struct
{
  u64 submitted; /* count of dma transfer submitted to dma device */
  u64 completed; /* count of completed dma transfers, exclude cpu
		    completions */
  u64 fallback;	 /* count of dma transfers fallback to cpu */
} vlib_dma_stats_t;
...
typedef struct
{
  ...
  u32 (*dma_transfer) (vlib_main_t *vm, u32 config_index, u32 n_transfers,
		       uword cookie);
  u32 (*get_completed) (vlib_main_t *vm, u32 config_index, uword *cookie);
  ...
  int (*get_stats) (vlib_main_t *vm, u32 config_index,
		    vlib_dma_stats_t *stats);
  void (*reset_stats) (vlib_main_t *vm, u32 config_index);
  u8 *(*dump_info) (vlib_main_t *vm, u32 config_index, u8 *s);
} vlib_dma_config_fn;
...
typedef struct
{
  ...
  vlib_dma_completion_cb_fn_t cb; /* callback when completed */
  u32 backend_index;		  /* backend index */
  u8 cpu_fallback : 1;		  /* do cpu fallback when no resource */
  vlib_dma_transfer_template_t *template;
} vlib_dma_config_t;
...
vlib_dma_get_completed (vlib_main_t *vm, u32 config_index,
			vlib_dma_completion_cb_fn_t *cb, uword *cookie)
{
  vlib_dma_main_t *dm = &dma_main;
  u32 ret = 0;
  vlib_dma_config_t *config = vec_elt_at_index (dm->configs, config_index);
...
/** \brief Retrieve transfer statistic config
 * @param vm            vm vlib_main_t pointer
 * @param config_index  index of config
 * @param stats         pointer of dma statistic
 * @return  0 on success, ~0 on error
 */
static_always_inline u32
vlib_dma_get_stats (vlib_main_t *vm, u32 config_index, vlib_dma_stats_t *stats)
{
  vlib_dma_main_t *dm = &dma_main;
  vlib_dma_config_t *config = vec_elt_at_index (dm->configs, config_index);

  return config->fn->get_stats (vm, config_index, stats);
}
...
vlib_dma_dump_info (vlib_main_t *vm, u32 config_index, u8 *s)
{
  vlib_dma_main_t *dm = &dma_main;
  vlib_dma_config_t *config = dm->configs + config_index;
  return config->fn->dump_info (vm, config_index, s);
}
