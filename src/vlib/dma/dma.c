...
vlib_dma_config (vlib_main_t *vm, vlib_dma_config_args_t *args)
{
  ...
  u32 size;
  size = dma_backend_data_size (args->max_transfers);
  p->backend_data = vlib_physmem_alloc_aligned_on_numa (
    vm, size * args->max_transfer_size, CLIB_CACHE_LINE_BYTES, vm->numa_node);
  p->data_size = size;
  p->data_num = args->max_transfer_size;
  p->data_head = 0;
  p->data_tail = 0;
  p->cb = args->cb;
  ...
  if (thread->backend)
    {
      ...
      if (dma_check_requirements (&assigned->status, args, vm->numa_node))
	return -1;
  ...
  assigned->fn->configure (backend->instance, vm, args, p->backend_data,
			   &p->template);
