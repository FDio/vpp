...
vlib_dma_config (vlib_main_t *vm, vlib_dma_config_args_t *args)
{
  ...
  if (thread->backend)
	...
	return -1;
      assigned->fn->configure (vm, assigned->instance, flags,
			       args->max_transfers, args->cb,
			       &p->backend_data);
  ...
  assigned->fn->configure (vm, backend->instance, flags, args->max_transfers,
			   args->cb, &p->backend_data);
  status = &backend->status;
  status->thread_index = thread_index;
  status->state = DMA_BACKEND_ASSIGNED;

  // thread->backend_index = assigned - dm->backends;
  vec_add1 (thread->reg_configs, config_index);
  p->fn = assigned->fn;
  p->backend_index = assigned - dm->backends;
  return config_index;
