...
vlib_dma_config (vlib_main_t *vm, vlib_dma_config_args_t *args)
{
  ...
  data_size = dma_backend_data_size (args->max_transfer_size);
  p->backend_data = vlib_physmem_alloc_aligned_on_numa (
    vm, data_size * args->max_transfers, CLIB_CACHE_LINE_BYTES, vm->numa_node);
  bzero (p->backend_data, data_size * args->max_transfers);
  ...
  for (i = 0; i < args->max_transfers; i++)
    {
      ...
      record->record_addr =
	(uword) (p->result_data + i * sizeof (dma_result_t));
    }

  /* if thread has backend, attached current config to it */
  if (thread->backend)
    {
