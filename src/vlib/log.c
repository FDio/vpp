		...
		unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  int unthrottle_time;
  int size;
  bool set_unthrottle_time = false;
  bool set_size = false;
  vlib_log_main_t *lm = &log_main;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "unthrottle-time %d", &unthrottle_time))
	{
	  set_unthrottle_time = true;
	}
      else if (unformat (line_input, "size %d", &size))
	{
	  set_size = true;
	}
      else
...
VLIB_CLI_COMMAND (cli_set_log_params, static) = {
  .path = "set log params",
  .short_help = "set log params [unthrottle-time <int>] [size <int>]",
  .function = set_log_params,
};
/* *INDENT-ON* */

static clib_error_t *
test_log_class (vlib_main_t * vm,
		unformat_input_t * input, vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  /* Get a line of input. */
  ...
  vlib_log_level_t level;
  if (unformat
      (line_input, "%U %U", unformat_vlib_log_level, &level,
       unformat_vlib_log_class, &class))
    {
      vlib_log (level, class->index << 16, "%U", format_unformat_input,
		line_input);
    }
  else
    {
...
VLIB_CLI_COMMAND (cli_test_log, static) = {
  .path = "test log",
  .short_help = "test log <class> <level> <message",
  .function = test_log_class,
};
/* *INDENT-ON* */
...
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (cli_test_log2, static) = {
  .path = "test log2",
...
VLIB_EARLY_CONFIG_FUNCTION (log_config, "log");
