  ...
  if ((syslog_enabled || kmsg_enabled) &&
  ...
  else
    {
      ...
      if (kmsg_enabled && lm->kmsg_filp)
        {
          fprintf (lm->kmsg_filp, "%.*s: %.*s", (int) vec_len (l), l,
                   (int) vec_len (s) - is_term, s);
        }
      vec_free (l);
    }
...
vlib_log_init (vlib_main_t *vm)
{
  ...
  while (r)
	...
	vlib_log_get_subclass_data (r->class)->kmsg_level =
	  r->default_kmsg_level;
      r = r->next;
    }
		 ...
		 unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output (vm, "%-20s %U", "  Kmsg Log Level:",
		   format_vlib_log_level, lm->default_kmsg_log_level);
  vlib_cli_output (vm, "%-20s %u msgs/sec", "  Rate Limit:",
		   lm->default_rate_limit);
  vlib_cli_output (vm, "\n");
  vlib_cli_output (vm, "%-22s %-14s %-14s %-14s %s",
		   "Class/Subclass", "Level", "Syslog Level", "Kmsg Log Level",
		   "Rate Limit");
	       ...
	       unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	if (unformat
	    (line_input, "kmsg-level %U", unformat_vlib_log_level,
	     &kmsg_level))
	{
	  set_kmsg_level = true;
	}
...
log_config_class (vlib_main_t * vm, char *name, unformat_input_t * input)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	;
			 ...
			 &tmp.syslog_level))
	;
      else if (unformat (input, "kmsg-level %U", unformat_vlib_log_level,
			 &tmp.kmsg_level))
	;
...
log_config (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (input, "default-kmsg-log-level %U",
			 unformat_vlib_log_level,
