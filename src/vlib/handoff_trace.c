...
format_handoff_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "HANDED-OFF: from thread %d trace index %d", t->prev_thread,
	      t->prev_trace_index);
  return s;
}
...
static uword
handoff_trace_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
		       vlib_frame_t *frame)
{
  vlib_buffer_free (vm, vlib_frame_vector_args (frame), frame->n_vectors);

  vlib_node_increment_counter (vm, node->node_index, HANDOFF_TRACE_ERROR_BUGS,
			       frame->n_vectors);
...
vlib_add_handoff_trace (vlib_main_t *vm, vlib_buffer_t *b)
{
  ...
  if (PREDICT_FALSE (!vlib_trace_buffer (
