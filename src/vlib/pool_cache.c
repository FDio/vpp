...
static vlib_pool_cache_subpool_meta_t *
_pool_cache_get_subpool_meta_chunk (vlib_pool_cache_t *c, u32 pidx)
{
...
static u8 **
_pool_cache_get_pool_chunk (vlib_pool_cache_t *c, u32 pidx)
{
  u32 chunk_index = pidx >> VLIB_POOL_CACHE_LOG2_SUBPOOL_PTR_CHUNK_SIZE;
  u8 **chunk = c->pool_chunks[chunk_index];

  if (chunk == 0)
    {
      vec_validate_aligned (chunk, VLIB_POOL_CACHE_SUBPOOL_PTR_CHUNK_SIZE - 1,
			    CLIB_CACHE_LINE_BYTES);
      c->pool_chunks[chunk_index] = chunk;
    }
  return chunk;
}
...
{
  vlib_pool_cache_subpool_meta_t *meta_chunk, *sp;
  u32 pidx = c->n_subpools;
  u32 old_global_head = c->global_free_list;
  u32 i, n_global;
  uword n_bytes;
  u8 **pool_chunk;
  u8 *pool = 0;
  ...
  pool_chunk = _pool_cache_get_pool_chunk (c, pidx);
  pool_chunk[pidx & VLIB_POOL_CACHE_SUBPOOL_PTR_CHUNK_MASK] = pool;

  meta_chunk = _pool_cache_get_subpool_meta_chunk (c, pidx);
  sp = &meta_chunk[pidx & VLIB_POOL_CACHE_SUBPOOL_PTR_CHUNK_MASK];
  clib_memset (sp, 0, sizeof (*sp));
  vec_validate_aligned (sp->slot_state, c->subpool_size - 1, CLIB_CACHE_LINE_BYTES);
...
_pool_cache_refill (vlib_pool_cache_t *c, vlib_pool_cache_thread_t *pt)
{
  ...
  clib_atomic_fetch_add_rel (&c->global_lock_acquisitions, 1);
  ...
  pt->free_list = head;
  clib_atomic_store_rel_n (&pt->n_cached, batch);
  clib_atomic_fetch_add_rel (&pt->refills, 1);
...
_pool_cache_flush (vlib_pool_cache_t *c, vlib_pool_cache_thread_t *pt, u32 n_cached)
{
  ...
  clib_atomic_store_rel_n (&pt->n_cached, batch);
  clib_atomic_fetch_add_rel (&pt->flushes, 1);
  clib_spinlock_unlock (&c->lock);
}
			     ...
			     uword elt_align)
{
  ...
  vec_validate_aligned (c->pool_chunks, c->max_subpool_chunks - 1, CLIB_CACHE_LINE_BYTES);
  _pool_cache_add_instance (c, name, format_element);
}

void
pool_cache_free_resources (vlib_pool_cache_t *c)
{
  u32 i;
  ...
  for (i = 0; i < c->n_subpools; i++)
    {
      ...
      u8 *pool = _pool_cache_pool_at (c, i);

      clib_mem_unpoison (pool, (uword) c->subpool_size * c->elt_size);
      vec_free (pool);
      /* State and intrusive links are parallel arrays owned by this chunk. */
      vec_free (sp->slot_state);
      vec_free (sp->next_free);
    }
  for (i = 0; i < vec_len (c->pool_chunks); i++)
    vec_free (c->pool_chunks[i]);
  vec_free (c->pool_chunks);
...
int
pool_cache_retire (vlib_pool_cache_t *c, u32 index)
{
  ...
  if (!clib_atomic_cmp_and_swap_acq_relax_n (&sp->slot_state[eidx], &expected,
    ...
    return 0;
  /* RETIRED closes the lookup gate but leaves the payload readable. */
  clib_atomic_fetch_add_relax (&c->n_retired, 1);
  return 1;
}

int
pool_cache_reclaim (vlib_pool_cache_t *c, u32 index)
{
  u32 pidx = _pool_cache_subpool_index (c, index);
  u32 eidx = _pool_cache_elt_index (c, index);
  vlib_pool_cache_subpool_meta_t *sp;
  u64 n_retired;
  u8 expected = VLIB_POOL_CACHE_SLOT_RETIRED;
  ...
  if (!clib_atomic_cmp_and_swap_acq_relax_n (&sp->slot_state[eidx], &expected,
    ...
    return 0;
  n_retired = clib_atomic_fetch_sub_relax (&c->n_retired, 1);
  ASSERT (n_retired > 0);
  /* The caller's grace period has ended, so publish the slot as reusable. */
