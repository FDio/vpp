...
_pool_cache_lock (vlib_pool_cache_t *c)
{
  ...
  /* Slow-path observability. The CLI access it for debug/stats, so we access it with relaxed
   * atomics (explicitly not atomic inc to avoid the RMW lock, and avoid a UB at compile time). */
...
pool_cache_prefill (vlib_pool_cache_t *c, u32 n_elts)
{
  ...
  while (clib_atomic_load_relax_n (&c->n_global_free) < start_free + n_elts)
    _pool_cache_add_subpool_locked (c);
  _pool_cache_unlock (c);
}
