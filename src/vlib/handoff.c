...
VLIB_REGISTER_LOG_CLASS (handoff_log, static) = {
  .class_name = "handoff",
};

#define log_warn(...)  vlib_log_warn (handoff_log.class, __VA_ARGS__)
#define log_debug(...) vlib_log_debug (handoff_log.class, __VA_ARGS__)
  ...
  if (PREDICT_TRUE (n_slots))
    {
      ...
      __atomic_fetch_or (&target_vm->handoff_queue_pending_bmp, hqm->queue_bit, __ATOMIC_SEQ_CST);
      if (do_wakeup)
...
vlib_handoff_queue_alloc (u32 size, u32 dequeue_vector_limit)
{
  ...
  hq->size = size;
  hq->dequeue_vector_limit = dequeue_vector_limit;
...
vlib_handoff_queue_resize (u32 index, u32 queue_size)
{
  ...
  vec_foreach_index (thread_index, hqm->vlib_handoff_queues)
      ...
      if (n_pending > size)
	{
      ...
      new_hq = vlib_handoff_queue_alloc (size, old_hq->dequeue_vector_limit);
  ...
  log_debug ("resized queue index %u from %u to %u buffer indices", index, old_queue_size,
	     queue_size);
...
vlib_handoff_alloc_queues (vlib_handoff_alloc_queues_args_t *a)
{
  ...
  vlib_node_t *node;
  u32 hqm_index;
  u32 dequeue_vector_limit = a->dequeue_vector_limit ?
			       a->dequeue_vector_limit :
			       VLIB_HANDOFF_QUEUE_DEFAULT_DEQUEUE_VECTOR_LIMIT;
  ...
  if (hqm.index == 64)
    log_warn ("too many (>63) active handoff queues, polling will be done as linear search");
  ...
  for (int i = 0; i < tm->n_vlib_mains; i++)
    {
      hq = vlib_handoff_queue_alloc (qsz, dequeue_vector_limit);
  ...
  log_debug ("allocated queue index %u node %U size %u dequeue-vector-limit %u aux %u", hqm_index,
	     format_vlib_node_name, vm, hqm.node_index, queue_size, dequeue_vector_limit,
	     hqm.with_aux);
