  ...
  if (PREDICT_FALSE (queues_bitmap == CLIB_U64_MAX))
      ...
      vec_foreach (hqm, vm->handoff_queue_mains)
	{
	  if (PREDICT_FALSE (hqm->with_aux))
	    vlib_handoff_queue_dequeue_inline (vm, hqm, 1 /* with_aux */);
	  else
	    vlib_handoff_queue_dequeue_inline (vm, hqm, 0 /* with_aux */);
	}
  ...
  while (queues_bitmap)
    {
      ...
      if (PREDICT_FALSE (hqm_index >= vec_len (vm->handoff_queue_mains)))
	continue;
  ...
  while (n_left)
    {
      ...
      u32 n_copy = clib_min (n_left, VLIB_HANDOFF_QUEUE_SLOT_N_ELTS);

      if (n_copy > 1)
	vlib_buffer_copy_indices (slot->buffer_indices + 1, src + 1, n_copy - 1);
      if (n_copy < VLIB_HANDOFF_QUEUE_SLOT_N_ELTS)
	clib_memset_u32 (slot->buffer_indices + n_copy, VLIB_BUFFER_INVALID_INDEX,
			 VLIB_HANDOFF_QUEUE_SLOT_N_ELTS - n_copy);
...
retry:
  ...
  if (PREDICT_TRUE (n_slots))
    {
      __atomic_fetch_or (&vlib_get_main_by_index (thread_index)->handoff_queue_pending_bmp,
			 hqm->queue_bit, __ATOMIC_RELAXED);
      if (do_wakeup)
...
clib_error_t *
vlib_handoff_queue_resize (u32 index, u32 size)
{
  ...
  if (size == 0 || (size & (size - 1)))
    return clib_error_return (0, "size must be a power of 2");
  ...
  vec_foreach_index (thread_index, hqm->vlib_handoff_queues)
      ...
      if (n_pending > size)
	{
	  ...
	  vec_foreach (q, new_queues)
	    clib_mem_free (q[0]);
	  vec_free (new_queues);
	  return clib_error_return (0, "queue has %u pending slots, size %u is too small",
				    n_pending, size);
	}
