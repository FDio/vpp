 ...
 *
 * The ring stores fixed-size slots instead of single buffer indices. VLIB_HANDOFF_QUEUE_SLOT_SIZE
 * is currently sized to 128 bytes, i.e. two standard cachelines worth of buffer indices. Each
 * slot is aligned to that size to avoid producer cacheline bouncing when multiple producers
 * reserve adjacent slots. Optional aux slots are stored in a parallel ring after the buffer-index
 * ring.
 *
...
vlib_handoff_queue_dequeue_inline (vlib_main_t *vm, vlib_handoff_queue_main_t *hqm, u8 with_aux)
{
  ...
  while (n_deq < dequeue_vector_limit)
	...
	break;
      ...
      if (PREDICT_FALSE (f && VLIB_HANDOFF_QUEUE_SLOT_SIZE > n_free))
	{
...
vlib_handoff_queue_update_trace_stop (vlib_handoff_queue_t *hq, u64 stop)
{
  ...
  while (1)
	...
	break;
      ...
      if (__atomic_compare_exchange_n (&hq->trace_stop, &old, stop, 0 /* weak */, __ATOMIC_RELAXED,
				       __ATOMIC_RELAXED))
	break;
