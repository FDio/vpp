 ...
 u16 *thread_indices, u32 n_packets, int drop_on_congestion)
{
  ...
  while (n_packets >= VLIB_FRAME_SIZE)
	...
	vm, fqm, buffer_indices, thread_indices, VLIB_FRAME_SIZE,
	drop_on_congestion);
  ...
  n_enq += vlib_buffer_enqueue_to_thread_inline (
    vm, fqm, buffer_indices, thread_indices, n_packets, drop_on_congestion);
...
(vlib_main_t *vm, vlib_frame_queue_main_t *fqm)
{
  ...
  u32 *from, *to = 0, processed = 0, vectors = 0;
  vlib_frame_t *f = 0;
