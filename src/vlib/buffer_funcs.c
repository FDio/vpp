			 ...
			 u64 *bmp, u32 *dst)
{
  ...
  *bmp |= mask;
  ...
  if (COMPILE_TIME_CONST (n_left) && n_left == 64 && (b0 & b1) == 0xFFFF)
    {
      clib_memcpy_u32 (dst, elts, 64);
      *bmp |= ~0ULL;
      return dst + 64;
...
static_always_inline u32
get_first_unused_elt (extract_data_t *d)
{
...
enqueue_one (vlib_main_t *vm, vlib_node_runtime_t *node, extract_data_t *d,
	     u16 next_index, u32 *buffers, u16 *nexts, u32 n_buffers, u32 *tmp)
{
  vlib_frame_t *f;
  ...
  while (count >= VLIB_FRAME_SIZE)
    {
      ...
      while (n_left)
	{
	  next_index = nexts[get_first_unused_elt (&d)];
	  n_left -= enqueue_one (vm, node, &d, next_index, buffers, nexts,
				 VLIB_FRAME_SIZE, tmp);
	}
  ...
  if (count)
    {
      ...
      n_left -=
	enqueue_one (vm, node, &d, next_index, buffers, nexts, count, tmp);
