...
static void
vlib_worker_thread_call_one_time_release_fns (vlib_main_t *vm, vlib_global_main_t *vgm)
{
...
barrier_trace_sync (f64 t_entry, f64 t_open, f64 t_closed)
{
  ...
  vlib_worker_thread_t *w = vlib_worker_threads + vlib_get_thread_index ();
...
cpu_config (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (input, "barrier-timeout %f",
