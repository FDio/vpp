	...
	;
                         ...
                         &bitmap))
      {
        ...
        if ( (*bitmap & *tr->coremask) !=0 )
          return clib_error_return (0, "crypto-bitmap:%d corelist-bitmap:%d, workers can not be same", *bitmap, *tr->coremask);

        tr = (vlib_thread_registration_t *) p[0];

        tr->crypto_coremask = bitmap;
        tr->crypto_corecount = clib_bitmap_count_set_bits (tr->crypto_coremask);
      }
