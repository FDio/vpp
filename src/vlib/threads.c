...
static clib_error_t *
vlib_launch_thread_internal (void *fp, vlib_worker_thread_t * w, unsigned lcore_id)
{
  vlib_thread_main_t *tm = &vlib_thread_main;
  void *(*fp_arg) (void *) = fp;
  ...
  for (i = 0; i < vec_len (tm->registrations); i++)
	  ...
	  for (j = 0; j < tr->count; j++)
	    {
	      ...
	      err = vlib_launch_thread_internal (vlib_worker_thread_bootstrap_fn, w, 0);
	      if (err)
      ...
      else
	{
	  uword c;
            ...
            clib_bitmap_foreach (c, tr->coremask, ({
              w = vlib_worker_threads + worker_thread_index++;
	      err = vlib_launch_thread_internal (vlib_worker_thread_bootstrap_fn, w, c);
	      if (err)
		clib_error_report (err);
            }));
/* *INDENT-ON* */
