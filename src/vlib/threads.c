...
vlib_worker_thread_node_refork (void)
{
  ...
  _clib_memcpy (&vm_clone->error_main, &vm->error_main,
  ...
  for (j = 0; j < vec_len (nm->nodes); j++)
    {
      ...
      _clib_memcpy (new_n_clone, new_n, sizeof (*new_n));
      /* none of the copied nodes have enqueue rights given out */
      ...
      else
	{
	  ...
	  _clib_memcpy (&new_n_clone->stats_total,
			&old_n_clone->stats_total,
			sizeof (new_n_clone->stats_total));
	  _clib_memcpy (&new_n_clone->stats_last_clear,
  ...
  vec_foreach (rt, nm_clone->nodes_by_type[VLIB_NODE_TYPE_INTERNAL])
  {
    ...
    if (n->runtime_data && n->runtime_data_bytes > 0)
      _clib_memcpy (rt->runtime_data, n->runtime_data,
		    clib_min (VLIB_NODE_RUNTIME_DATA_SIZE,
			      n->runtime_data_bytes));
  }
  ...
  for (j = 0; j < vec_len (old_rt); j++)
    {
      ...
      _clib_memcpy (rt->runtime_data, old_rt[j].runtime_data,
