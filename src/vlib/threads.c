      ...
      for (i = 0; i < vec_len (tm->registrations); i++)
	{
	  ...
	  for (k = 0; k < tr->count; k++)
		...
		if (n->runtime_data && n->runtime_data_bytes > 0)
          ...
          memset(nm_clone->n_vectors_stats, 0, VLIB_WINDOW_SIZE*sizeof(nm_clone->n_vectors_stats[0]));
          memset(nm_clone->cycles_stats, 0, VLIB_WINDOW_SIZE*sizeof(nm_clone->cycles_stats[0]));
