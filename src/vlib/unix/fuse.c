...
static vlib_node_registration_t vlib_fuse_process_node;
...
always_inline vlib_fuse_node_t *
fuse_find_child_by_name (vlib_fuse_handle_t h, vlib_fuse_node_t *p,
...
static void
fuse_reply_open (vlib_fuse_handle_t h, const struct fuse_in_header *in)
{
  struct fuse_open_out o = {};
  vlib_fuse_node_t *n = vlib_fuse_get_node (h, in->nodeid);
  int rv;
...
static void
fuse_reply_read (vlib_fuse_handle_t h, const struct fuse_in_header *in,
		 struct fuse_read_in *ri)
{
  vlib_fuse_node_t *n = vlib_fuse_get_node (h, in->nodeid);
  u64 off = ri->offset;
...
static void
fuse_reply_write (vlib_fuse_handle_t h, const struct fuse_in_header *in,
...
static void
fuse_reply_flush (vlib_fuse_handle_t h, const struct fuse_in_header *in,
		  struct fuse_flush_in *fi)
{
  ...
  rv = n->op_fn (&(vlib_fuse_file_op_data_t){
    .h = h,
...
static void
fuse_reply_release (vlib_fuse_handle_t h, const struct fuse_in_header *in,
...
fuse_fd_read (struct clib_file *f)
{
  ...
  vlib_process_signal_event (vlib_get_main (), vlib_fuse_process_node.index, 0,
			     (uword) h);
  return 0;
}
...
vlib_fuse_create (vlib_fuse_handle_t *hp, vlib_fuse_create_args_t *args)
{
  ...
  *hp = h;
  if (vlib_node_get_state (vm, vlib_fuse_process_node.index) ==
      VLIB_NODE_STATE_DISABLED)
    vlib_node_set_state (vm, vlib_fuse_process_node.index,
...
VLIB_REGISTER_NODE (vlib_fuse_process_node, static) = {
