		...
		u32 * buffer_index, u32 drop_message)
{
  ...
  const uword buffer_size = vlib_buffer_get_default_size (vm);
  word n_bytes_left;
		     ...
		     int n_intfcs_to_probe)
{
  ...
  msm->rx_mtu_n_buffers =
    msm->rx_mtu_n_bytes / vlib_buffer_get_default_size (vm);
  msm->rx_mtu_n_buffers +=
    (msm->rx_mtu_n_bytes % vlib_buffer_get_default_size (vm)) != 0;
