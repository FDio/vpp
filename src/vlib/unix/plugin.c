...
load_one_plugin (plugin_main_t * pm, plugin_info_t * pi, int from_early_init)
{
  ...
  if ((strlen (info->version_required) > 0) &&
      (strncmp
       (VPP_BUILD_VER, info->version_required,
	strlen (info->version_required))))
    {
      clib_warning ("Plugin %s version mismatch: requres version %s",
		    pi->name, info->version_required);
