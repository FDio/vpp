...
vlib_buffer_enqueue_to_thread (vlib_main_t *vm, vlib_node_runtime_t *node, u32 handoff_queue_index,
			       u32 *buffer_indices, u16 *thread_indices, u32 n_packets)
{
  ...
  return (fn) (vm, node, handoff_queue_index, buffer_indices, thread_indices, n_packets);
}
				      ...
				      clib_thread_index_t thread_index, u32 n_packets)
{
  ...
  return (fn) (vm, node, handoff_queue_index, buffer_indices, thread_index, n_packets);
}

static_always_inline u32
vlib_buffer_enqueue_to_thread_with_aux (vlib_main_t *vm, vlib_node_runtime_t *node,
					u32 handoff_queue_index, u32 *buffer_indices, u32 *aux,
					u16 *thread_indices, u32 n_packets)
{
  ...
  return (fn) (vm, node, handoff_queue_index, buffer_indices, aux, thread_indices, n_packets);
}
