...
vlib_file_poll (vlib_main_t *vm)
{
  ...
  struct epoll_event *e;
  int n_fds_ready;
  struct epoll_event epoll_events[256];
   ...
   * do so right away.
   */
  if (PREDICT_FALSE (um->poll_sleep_usec))
    {
      struct timespec tsrem, ts = {
	.tv_nsec = 1000L * um->poll_sleep_usec,
      };
  ...
  if (vlib_last_vectors_per_main_loop (vm) >= 2)
    {
      /* thread is busy, reduce epoll frequency */
...
epoll:
  ...
  sigemptyset (&unblock_all_signals);
  ...
  for (e = epoll_events; e < epoll_events + n_fds_ready; e++)
      ...
      if (!f->active)
	{
	  foreach_int (flag, EPOLLIN, EPOLLOUT, EPOLLERR)
		...
		log_warn ("epoll event %s dropped due to inactive file",
			  str[flag]);
	      }
	  continue;
	}

      if (PREDICT_FALSE (e->events & EPOLLERR))
	{
	  if (f->error_function)
	    {
	      f->error_events++;
	    ...
	    {
	      log_err ("EPOLLERR received for fd %d ('%v')",
		       f->file_descriptor, f->description);
	      if (!f->dont_close)
