 ...
 * elt_at_index remain stable when the cache grows. An element pointer remains
 * valid only until its index is passed to pool_cache_free(); callers must
 * synchronize free against concurrent readers.
 *
...
pool_cache_free (vlib_pool_cache_t *c, u32 index)
{
  ...
  sp = _pool_cache_subpool_meta_at (c, pidx);
  /* Claim the sole release; this rejects double-free. */
