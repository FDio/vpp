...
_pool_cache_next_free (vlib_pool_cache_t *c, u32 index)
{
  ...
  return vec_elt (sp->next_free, eidx);
}
...
_pool_cache_set_next_free (vlib_pool_cache_t *c, u32 index, u32 next)
{
  ...
  ASSERT (index != VLIB_POOL_CACHE_INVALID_INDEX);
  vec_elt (sp->next_free, eidx) = next;
}
...
pool_cache_elt_at_index (vlib_pool_cache_t *c, u32 index)
{
  ...
  u32 eidx = _pool_cache_elt_index (c, index);
  vlib_pool_cache_subpool_t *sp = _pool_cache_subpool_at (c, pidx);
  ASSERT (clib_atomic_load_acq_n (vec_elt_at_index (sp->slot_state, eidx)) ==
...
pool_cache_is_free_index (vlib_pool_cache_t *c, u32 index)
{
  ...
  return clib_atomic_load_acq_n (vec_elt_at_index (sp->slot_state, eidx)) ==
...
pool_cache_alloc (vlib_pool_cache_t *c)
{
  ...
  sp = _pool_cache_subpool_at (c, pidx);
  pt->free_list = vec_elt (sp->next_free, eidx);
  vec_elt (sp->next_free, eidx) = VLIB_POOL_CACHE_INVALID_INDEX;
  clib_atomic_store_rel_n (&pt->n_cached, --n);
  clib_atomic_store_relax_n (vec_elt_at_index (sp->slot_state, eidx),
			     VLIB_POOL_CACHE_SLOT_ALLOCATED);
			  ...
			  vlib_pool_cache_subpool_t *sp)
{
  ...
  vec_elt (sp->next_free, eidx) = pt->free_list;
  pt->free_list = index;
  clib_atomic_store_rel_n (&pt->n_cached, ++n);
...
pool_cache_free (vlib_pool_cache_t *c, u32 index)
{
  ...
  if (!clib_atomic_cmp_and_swap_acq_relax_n (vec_elt_at_index (sp->slot_state, eidx), &expected,
					     VLIB_POOL_CACHE_SLOT_FREE, 0))
    {
      ASSERT (0);
      return;
