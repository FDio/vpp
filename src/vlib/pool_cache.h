 ...
 *   u32 idx = pool_cache_alloc (&flows);
 *   vnet_flow_t *f = pool_cache_elt_at_index (&flows, idx);
 *   pool_cache_free (&flows, idx);
 *
 ...
 *
 * pool_cache_free_resources() requires complete quiescence: no worker may
 * still allocate, free, look up, or format this cache while it is destroyed.
 *
 * Readers which can outlive removal use pool_cache_get_if_live(). The owner
 * first calls pool_cache_retire(), completes its own grace-period mechanism,
 * and then calls pool_cache_reclaim(). Retirement prevents new live lookups
 * but deliberately leaves the element readable for readers which obtained its
 * pointer before retirement. The pool cache does not provide or infer the
 * grace period.
 */
...
void pool_cache_free_resources (vlib_pool_cache_t *c);
...
int pool_cache_retire (vlib_pool_cache_t *c, u32 index);
int pool_cache_reclaim (vlib_pool_cache_t *c, u32 index);
...
_pool_cache_subpool_meta_at (vlib_pool_cache_t *c, u32 pidx)
{
...
_pool_cache_pool_at (vlib_pool_cache_t *c, u32 pidx)
{
  u8 **chunk;

  ASSERT (pidx < clib_atomic_load_acq_n (&c->n_subpools));
  chunk = c->pool_chunks[pidx >> VLIB_POOL_CACHE_LOG2_SUBPOOL_PTR_CHUNK_SIZE];
  ASSERT (chunk != 0);
  ASSERT (chunk[pidx & VLIB_POOL_CACHE_SUBPOOL_PTR_CHUNK_MASK] != 0);
  return chunk[pidx & VLIB_POOL_CACHE_SUBPOOL_PTR_CHUNK_MASK];
}

static_always_inline void *
_pool_cache_elt (vlib_pool_cache_t *c, u32 pidx, u32 eidx)
{
  return _pool_cache_pool_at (c, pidx) + (uword) eidx * c->elt_size;
}
...
pool_cache_elt_at_index (vlib_pool_cache_t *c, u32 index)
{
  ...
  ASSERT (pidx < clib_atomic_load_acq_n (&c->n_subpools));
  sp = _pool_cache_subpool_meta_at (c, pidx);
  ASSERT (clib_atomic_load_acq_n (&sp->slot_state[eidx]) == VLIB_POOL_CACHE_SLOT_ALLOCATED);
  return _pool_cache_elt (c, pidx, eidx);
}

/* A successful lookup is protected only until the caller's grace period ends. */
static_always_inline void *
pool_cache_get_if_live (vlib_pool_cache_t *c, u32 index)
{
  u32 pidx = _pool_cache_subpool_index (c, index);
  u32 eidx = _pool_cache_elt_index (c, index);
  vlib_pool_cache_subpool_meta_t *sp;
  ...
  if (PREDICT_FALSE (index == VLIB_POOL_CACHE_INVALID_INDEX ||
    ...
    return 0;
  sp = _pool_cache_subpool_meta_at (c, pidx);
  if (clib_atomic_load_acq_n (&sp->slot_state[eidx]) != VLIB_POOL_CACHE_SLOT_ALLOCATED)
    return 0;
  return _pool_cache_elt (c, pidx, eidx);
}
...
pool_cache_is_free_index (vlib_pool_cache_t *c, u32 index)
{
  ...
  if (PREDICT_FALSE (index == VLIB_POOL_CACHE_INVALID_INDEX ||
    ...
    return 1;
  sp = _pool_cache_subpool_meta_at (c, pidx);
  return clib_atomic_load_acq_n (&sp->slot_state[eidx]) == VLIB_POOL_CACHE_SLOT_FREE;
}

static_always_inline u32
pool_cache_alloc (vlib_pool_cache_t *c)
{
  ...
  if (PREDICT_FALSE (n == 0))
    {
      ...
      n = clib_atomic_load_relax_n (&pt->n_cached);
    }
  ...
  sp = _pool_cache_subpool_meta_at (c, pidx);
  pt->free_list = sp->next_free[eidx];
  sp->next_free[eidx] = VLIB_POOL_CACHE_INVALID_INDEX;
  clib_atomic_store_rel_n (&pt->n_cached, --n);
  clib_atomic_store_relax_n (&sp->slot_state[eidx], VLIB_POOL_CACHE_SLOT_ALLOCATED);
  clib_mem_unpoison (_pool_cache_elt (c, pidx, eidx), c->elt_size);
  return index;
}
			  ...
			  vlib_pool_cache_subpool_meta_t *sp)
{
  ...
  clib_mem_poison (_pool_cache_elt (c, pidx, eidx), c->elt_size);
  sp->next_free[eidx] = pt->free_list;
  pt->free_list = index;
  clib_atomic_store_rel_n (&pt->n_cached, ++n);
...
static_always_inline void
pool_cache_free (vlib_pool_cache_t *c, u32 index)
{
  u32 pidx = _pool_cache_subpool_index (c, index);
  u32 eidx = _pool_cache_elt_index (c, index);
  vlib_pool_cache_subpool_meta_t *sp;
  u8 expected = VLIB_POOL_CACHE_SLOT_ALLOCATED;
  ...
  sp = _pool_cache_subpool_meta_at (c, pidx);
  /* Claim the sole release; this rejects double-free and RETIRED slots. */
  if (!clib_atomic_cmp_and_swap_acq_relax_n (&sp->slot_state[eidx], &expected,
					     VLIB_POOL_CACHE_SLOT_FREE, 0))
    {
      ASSERT (0);
      return;
