...
typedef struct
{
  ...
  /* refills, flushes, n_cached are only written by their owner thread, and read by main thread when
   * CLI is called */
  u64 refills;
  u64 flushes;
  u32 n_cached;
  /* The indexed VPP thread is the sole concurrent writer of this non-atomic field. */
  u32 free_list;
...
int pool_cache_prefill (vlib_pool_cache_t *c, u32 n_elts);
...
/*
 * pool_cache_put_index() may run on a different VPP thread from the get().
 * The freeing thread caches the index locally, where it may be reused
 * immediately. Callers must synchronize handoff, ensure no concurrent users
 * remain, and release each allocated index exactly once.
 */
static_always_inline void
pool_cache_put_index (vlib_pool_cache_t *c, u32 index)
{
  ...
  if (CLIB_DEBUG)
    {
      int success = clib_atomic_cmp_and_swap_acq_relax_n (
	vec_elt_at_index (sp->next_free, eidx), &expected, VLIB_POOL_CACHE_INVALID_INDEX, 0);
      ASSERT (success);
    }
  else
    {
 ...
 */
#define vlib_foreach_pool_cache(_e, _i, _c, _b)                                                    \
  do                                                                                               \
    {                                                                                              \
      u32 _n_subpools = clib_atomic_load_relax_n (&(_c)->n_subpools);                              \
      u32 _pidx = 0;                                                                               \
                                                                                                   \
      if (_n_subpools == 0)                                                                        \
	break;                                                                                     \
                                                                                                   \
      /* keep only one level of loop nesting, so _b can break from it; */                          \
      for (u32 _eidx = 0;; _eidx++)                                                                \
	{                                                                                          \
	  if (_eidx == (_c)->subpool_size)                                                         \
	    {                                                                                      \
	      _pidx++;                                                                             \
	      _eidx = 0;                                                                           \
	      if (_pidx == _n_subpools)                                                            \
		break;                                                                             \
	    }                                                                                      \
                                                                                                   \
	  u32 _index = _pool_cache_encode_index ((_c), _pidx, _eidx);                              \
	  if (pool_cache_is_free_index ((_c), _index))                                             \
	    continue;                                                                              \
                                                                                                   \
	  (_i) = _index;                                                                           \
	  (_e) = _pool_cache_elt_at_indices ((_c), _pidx, _eidx);                                  \
	  {                                                                                        \
	    _b;                                                                                    \
	  }                                                                                        \
	}                                                                                          \
