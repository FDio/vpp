...
static_always_inline void
_pool_cache_foreach_yield (vlib_main_t *vm, f64 *start)
{
  /* yields for 2 clock ticks every 1 tick to avoid blocking threads for too long
 ...
 *
 * The iterator yields with vlib_process_suspend() and therefore must only be
 * called from VLIB process context.
 */
#define vlib_foreach_pool_cache(_e, _c, _b)                                                        \
  do                                                                                               \
	...
	{                                                                                          \
	  sp = _pool_cache_subpool_at ((_c), pidx);                                                \
	  for (int eidx = 0; eidx < (_c)->subpool_size; eidx++)                                    \
	    {                                                                                      \
	      _pool_cache_foreach_yield (vm, &start);                                              \
	      u32 index = _pool_cache_encode_index (pidx, eidx);                                   \
