			   ...
			   u64 time, u32 n_vectors, u32 is_return)
{
  ...
  if (PREDICT_FALSE (enabled && n_vectors))
	    ...
	    node_index == evm->elog_trace_graph_circuit_node_index)))
	{
	  elog_track (em,
		      /* event type */
		      vec_elt_at_index (is_return ? evm->node_return_elog_event_types :
...
vlib_main_or_worker_loop (vlib_main_t * vm, int is_main)
{
  ...
  u32 handoff_queue_check_counter = 0;
  ...
  while (1)
	...
	vlib_worker_thread_barrier_check ();
      ...
      check_handoff_queues = __atomic_exchange_n (&vm->check_handoff_queues, 0, __ATOMIC_RELAXED);

      if (PREDICT_FALSE (check_handoff_queues + handoff_queue_check_counter))
	{
	  if (check_handoff_queues)
	    handoff_queue_check_counter = 100;

	  /* No handoff queue work found? */
	  if (handoff_queues_dequeue (vm))
	    handoff_queue_check_counter = 100;
	  else
	    handoff_queue_check_counter--;
	}
