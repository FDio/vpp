	       ...
	       u64 last_time_stamp)
{
  ...
  if (VLIB_BUFFER_TRACE_TRAJECTORY && frame)
      ...
      for (i = 0; i < frame->n_vectors; i++)
	{
      ...
      {
        clib_panic ("dispatch free buffer %p bi(0x%x) to node%u", b, from[i], node->node_index);
      }
      while (b->flags & VLIB_BUFFER_NEXT_PRESENT)
      {
		...
		{
		  clib_panic ("dispatch free next buffer %p bi(0x%x) to node%u head bi(0x%x)", b, next_buffer, node->node_index, from[i]);
		}
      }
#endif
	}
