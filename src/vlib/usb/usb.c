...
{
  vlib_usb_desc_t *c, *n, *d, **dv = 0;

  for (c = 0, n = root; (d = _foreach_vlib_usb_desc_helper (root, &c, &n));)
    vec_add1 (dv, d);
...
vlib_usb_create_desc_tree (u8 *data, int n_bytes)
{
  ...
  while (n_bytes > 0)
    {
      struct usb_descriptor_header *h = (typeof (h)) data;
      u8 len = h->bLength;
	...
	root = d;
      ...
      switch (((struct usb_descriptor_header *) data)->bDescriptorType)
	{
	case USB_DT_DEVICE:
	  if (dd)
	    goto error;
