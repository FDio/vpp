...
vlib_usb_device_config_get (vlib_usb_dev_handle_t dh)
{
  u8 buffer[1024];
  ...
  vlib_usb_get_dev_desc (dh, USB_DT_DEVICE, 0, buffer, sizeof (buffer));
  ...
  for (u8 cfg_idx = 0; cfg_idx < dd->device.bNumConfigurations; cfg_idx++)
    {
      ...
      int len = vlib_usb_get_dev_desc (dh, USB_DT_CONFIG, cfg_idx, buffer,
				       sizeof (buffer));
