...
always_inline u32
punt_replicate (vlib_main_t *vm, vlib_node_runtime_t *node, u32 thread_index,
		vlib_buffer_t *b0, u32 bi0, vlib_punt_reason_t pr0,
		u32 *next_index, u32 *n_left_to_next, u32 **to_next,
		u32 *n_dispatched)
{
  ...
  n_cloned0 = vlib_buffer_clone (vm, bi0, punt_clones[thread_index], n_clones0,
...
punt_dispatch_one (vlib_main_t *vm, vlib_node_runtime_t *node,
		   vlib_combined_counter_main_t *cm, u32 thread_index, u32 bi0,
		   u32 *next_index, u32 *n_left_to_next, u32 **to_next,
		   u32 *n_dispatched)
{
  ...
  if (PREDICT_FALSE (pr0 >= vec_len (punt_dp_db)))
    ...
    {
      vlib_increment_combined_counter (cm, thread_index, pr0, 1,
      ...
      else
	{
	   ...
	   */
	  next0 =
	    punt_replicate (vm, node, thread_index, b0, bi0, pr0, next_index,
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 4 && n_left_to_next > 2)
	{
	  ...
	  next0 =
	    punt_dispatch_one (vm, node, cm, thread_index, bi0, &next_index,
			       &n_left_to_next, &to_next, &n_dispatched);
	  next1 =
	    punt_dispatch_one (vm, node, cm, thread_index, bi1, &next_index,
			       &n_left_to_next, &to_next, &n_dispatched);
  ...
  vlib_node_increment_counter (vm, node->node_index, PUNT_ERROR_DISPATCHED,
			       n_dispatched);
