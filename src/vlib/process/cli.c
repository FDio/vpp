			     ...
			     vlib_cli_command_t *cmd)
{
  ...
  rv = vlib_process_set_capabilities (vm, effective);
  switch (rv)
    {
    case VLIB_PROCESS_API_ERROR_GET_CAPABILITIES:
      error = clib_error_return (0, "error getting process capabilities");
      goto err;
    case VLIB_PROCESS_API_ERROR_UNSUPPORTED_CAPABILITIES:
      error = clib_error_return (0, "error non supported capabilities");
      goto err;
    case VLIB_PROCESS_API_ERROR_MODIFY_CAPABILITIES:
			      ...
			      vlib_cli_command_t *cmd)
{
  ...
  rv = vlib_process_get_capabilities (vm, &data);
  switch (rv)
    {
    case VLIB_PROCESS_API_ERROR_GET_CAPABILITIES:
      error = clib_error_return (0, "error getting process capabilities");
