...
format_cdb_object_field (u8 * s, va_list * va)
{
  ...
  s = format (s, "  %U %s", format_cdb_field_action, v->action, ci->name);

  if (v->action == CDB_FIELD_ACTION_SET)
			    ...
			    vlib_cli_command_t * cmd)
{
  ...
  cdb_show_config_datastore (vm, ds);
			  ...
			  unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  if (unformat (line_input, "%s", &field))
    ;
  else
    {
      error = clib_error_return (0, "parse error: '%U'",
  ...
  int rc = vlib_cdb_unset_field (field);
  if (rc < 0)
    error = clib_error_return (0, "cdb: failed to set field '%s'", field);
