...
vlib_stats_register_mem_heap (clib_mem_heap_t *heap)
{
  ...
  mem_vector_index =
    vlib_stats_add_counter_vector ("/mem/%s%c", heap->name, 0);
  ...
  /* Create symlink */
  vlib_stats_add_symlink (mem_vector_index, STAT_MEM_TOTAL, "/mem/%s/used",
			  heap->name);
  vlib_stats_add_symlink (mem_vector_index, STAT_MEM_USED, "/mem/%s/total",
			  heap->name);
  vlib_stats_add_symlink (mem_vector_index, STAT_MEM_FREE, "/mem/%s/free",
			  heap->name);
