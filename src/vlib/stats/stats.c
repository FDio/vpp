			 ...
			 u64 *sequence_out)
{
  ...
  if (sequence_out)
    *sequence_out = metadata->sequence;
...
int
vlib_stats_ring_produce_batch (u32 entry_index, u32 thread_index,
			       void *data_array, u32 count)
{
  ...
  if (thread_index >= ring_buffer->config.n_threads || count == 0)
    return -1;

  u32 entry_size = ring_buffer->config.entry_size;
  u32 ring_size = ring_buffer->config.ring_size;
  u32 head = metadata->head;
  u8 *data_ptr = (u8 *) ring_buffer + ring_buffer->data_offset +
		 thread_index * ring_size * entry_size;

  /* Copy data in batch */
  for (u32 i = 0; i < count; i++)
    {
      u32 offset = head * entry_size;
      clib_memcpy_fast (data_ptr + offset, (u8 *) data_array + i * entry_size,
			entry_size);
...
void **
vlib_stats_ring_reserve_batch (u32 entry_index, u32 thread_index, u32 count)
{
  vlib_stats_segment_t *sm = vlib_stats_get_segment ();
  vlib_stats_entry_t *e = vlib_stats_get_entry (sm, entry_index);
  vlib_stats_ring_buffer_t *ring_buffer = e->data;
  ...
  /* Allocate array of pointers to reserved slots */
  void **slots = clib_mem_alloc (count * sizeof (void *));
  if (!slots)
    return NULL;
...
int
vlib_stats_ring_commit_batch (u32 entry_index, u32 thread_index, u32 count)
{
  vlib_stats_segment_t *sm = vlib_stats_get_segment ();
  vlib_stats_entry_t *e = vlib_stats_get_entry (sm, entry_index);
  vlib_stats_ring_buffer_t *ring_buffer = e->data;
  ...
  /* Single metadata update for entire batch */
  metadata->head = (metadata->head + count) % ring_buffer->config.ring_size;
  metadata->sequence += count;
