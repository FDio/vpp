...
vlib_stats_ring_produce (u32 entry_index, u32 thread_index, void *data)
{
  ...
  /* If ring is full, advance tail to overwrite oldest */
  if (metadata->count >= ring_buffer->config.ring_size)
    {
      metadata->tail = (metadata->tail + 1) % ring_buffer->config.ring_size;
      metadata->overwrite_count++; /* increment overwrite counter */
      /* count stays at max */
    }
  else
    {
  ...
  clib_warning ("Ring produce: entry_index=%u thread_index=%u seq=%lu head=%u "
		"tail=%u count=%u",
		entry_index, thread_index, metadata->sequence, metadata->head,
		metadata->tail, metadata->count);
...
vlib_stats_ring_commit_slot (u32 entry_index, u32 thread_index)
{
  ...
  /* If ring is full, advance tail to overwrite oldest (double-check in case it
   * was filled while we were serializing) */
  if (metadata->count >= ring_buffer->config.ring_size)
    {
      metadata->tail = (metadata->tail + 1) % ring_buffer->config.ring_size;
      metadata->overwrite_count++; /* increment overwrite counter */
