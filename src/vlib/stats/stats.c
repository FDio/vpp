...
vlib_stats_validate (u32 entry_index, ...)
{
  ...
  if (e->n_dimensions == 1)
	...
	vlib_stats_validate_last (data + i, t->size, idx1);
      e->data = data;
    }
	...
	{
	  vec_validate_aligned (data[idx0], idx1, CLIB_CACHE_LINE_BYTES);
	  for (u32 j = 0; j <= vec_len (data[idx0]); j++)
	    vlib_stats_validate_last (data[idx0] + j, t->size, idx2);
	}
      e->data = data;
    }
  else
