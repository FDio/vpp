...
{
  vlib_stats_directory_entry_t *n1 = a1;
  vlib_stats_directory_entry_t *n2 = a2;
...
format_stat_dir_entry (u8 *s, va_list *args)
{
  vlib_stats_directory_entry_t *ep =
    va_arg (*args, vlib_stats_directory_entry_t *);
  char *type_name;
  char *format_string;
			      ...
			      vlib_cli_command_t *cmd)
{
  ...
  vlib_stats_directory_entry_t *show_data;
  int i;
  ...
  for (i = 0; i < vec_len (show_data); i++)
    {
      vlib_stats_directory_entry_t *ep = vec_elt_at_index (show_data, i);
