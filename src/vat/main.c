...
{
  svm_queue_t *q;
  rx_thread_args_t *ta = (rx_thread_args_t *) arg;

  my_memory_client_main = ta->new_memclnt_main;
  my_api_main = ta->new_api_main;
...
connect_to_vpe (char *name)
{
  ...
  rx_thread_args_t _ta, *ta = &_ta;

  if (vl_client_connect_to_vlib ("/vpe-api", name, 32) < 0)
    return -1;

  ta->new_api_main = my_api_main = clib_mem_alloc (sizeof (*my_api_main));
  memset (my_api_main, 0, sizeof (*my_api_main));

  ta->new_memclnt_main = my_memory_client_main =
