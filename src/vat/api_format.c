  ...
  (vl_api_sw_interface_device_info_details_t * mp)
{
  ...
  s = format (s, "%Usw_if_index:      %8X\n"
	      ...
	      format_white_space, indent + 2, ntohl (mp->sup_sw_if_index));
  s = format (s, "%UDevice flags:     %8X\n",
	      format_white_space, indent + 2, mp->dev_flags);
  ...
  s = format (s, "%URSS hash flags: \n"
	      "%UAvailable:      %8X\n"
	      "%UEnabled:        %8X\n",
	      format_white_space, indent + 2,
	      format_white_space, indent + 4,
	      ntohl (mp->rss_hf),
	      format_white_space, indent + 4, ntohl (mp->rss_hf_enabled));
  ...
  s = format (s, "%UMQ mode:        %8d\n",
	      format_white_space, indent + 4, mp->mq_rx_mode);
  ...
  s = format (s, "%UMQ mode:        %8d\n",
	      format_white_space, indent + 4, mp->mq_tx_mode);
  s = format (s, "%UOffload flags:\n"
	      "%UAvailable:  %8X\n"
	      "%UEnabled:    %8X\n",
	      format_white_space, indent + 4,
  ...
  (vl_api_sw_interface_device_info_details_t * mp)
{
  ...
  vat_json_object_add_uint (node, "dev_flags", ntohl (mp->dev_flags));
  ...
  vat_json_object_add_uint (node, "tx_offloads_enabled",
			    ntohl (mp->tx_offloads_enabled));
  vat_json_object_add_uint (node, "rss_hf", ntohl (mp->rss_hf));
  vat_json_object_add_uint (node, "rss_hf_enabled",
			    ntohl (mp->rss_hf_enabled));
  vat_json_object_add_uint (node, "mq_rx_mode", mp->mq_rx_mode);
  vat_json_object_add_uint (node, "mq_tx_mode", mp->mq_tx_mode);
}
