  ...
  (vl_api_sw_interface_lacp_details_t * mp)
{
  ...
  print (vam->ofp,
	 ...
	 mp->actor_system[0], mp->actor_system[1], mp->actor_system[2],
	 mp->actor_system[3], mp->actor_system[4], mp->actor_system[5],
	 ntohs (mp->actor_key), ntohs (mp->actor_port_priority),
	 ntohs (mp->actor_port_number), ntohs (mp->partner_system_priority),
  ...
  print (vam->ofp,
	 ...
	 format_rx_sm_state, ntohl (mp->rx_state),
	 format_tx_sm_state, ntohl (mp->tx_state),
	 format_mux_sm_state, ntohl (mp->mux_state),
  ...
  (vl_api_sw_interface_lacp_details_t * mp)
{
  ...
  vat_json_object_add_uint (node, "rx_state", ntohl (mp->rx_state));
  vat_json_object_add_uint (node, "tx_state", ntohl (mp->tx_state));
  vat_json_object_add_uint (node, "mux_state", ntohl (mp->mux_state));
  vat_json_object_add_uint (node, "ptx_state", ntohl (mp->ptx_state));
}
