  ...
  while (1)
	  ...
	  + sizeof (*mbp))
	{
	  ...
	  if (vec_len (vam->socket_rx_buffer) == 0
	      && vam->control_pings_outstanding == 0)
	    break;
	}
    }
}
...
vat_socket_connect (vat_main_t * vam)
{
  ...
  clib_socket_t *sock = &vam->client_socket;
  msgbuf_t *mbp;
  clib_error_t *error;
  ...
  (vl_api_memfd_segment_create_reply_t * mp)
{
  ...
  if (retval == 0)
    {
      error = receive_fd_msg (vam->socket_fd, &my_fd);
      if (error)
	{
	  retval = -99;
	  goto out;
      ...
      clib_warning ("segment fd is %d", my_fd);
	...
	clib_unix_warning ("close memfd fd pivot");
      if (close (vam->socket_fd) < 0)
	clib_unix_warning ("close socket fd pivot");

      vam->socket_fd = 0;
      vam->vl_input_queue = am->shmem_hdr->vl_input_queue;
      vec_free (vam->socket_tx_buffer);
      /* do not: vec_free (vam->socket_rx_buffer); thats how we got here */
    }
