  ...
  (vl_api_sw_interface_tap_v2_details_t * mp)
{
  ...
  print (vam->ofp, "\n%-16s %-12d %-5d %-12d %-12d %-14U %-30s %-20s %-20s %-30s",
     ...
     mp->host_namespace, mp->host_bridge, ip4, ip6
     );
...
api_sw_interface_tap_v2_dump (vat_main_t * vam)
{
  ...
  print (vam->ofp, "\n%-16s %-12s %-5s %-12s %-12s %-14s %-30s %-20s %-20s %-30s",
         "dev_name", "sw_if_index", "id",
         "rx_ring_sz", "tx_ring_sz", "host_mac_addr", "host_namespace",
         "host_bridge", "host_ip4_addr", "host_ip6_addr");
