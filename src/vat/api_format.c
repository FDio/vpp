...
api_bier_table_add_del (vat_main_t * vam)
{
  ...
  mp->bt_set = set;
  mp->bt_sub_domain = sub_domain;
  mp->bt_bit_header_length = hdr_len;
...
api_bier_route_add_del (vat_main_t * vam)
{
  ...
  /* Construct the API message */
  M (BIER_ROUTE_ADD_DEL, mp);

  mp->br_is_add = is_add;
  mp->br_set = set;
  mp->br_bp = ntohs (bp);
  mp->br_sub_domain = sub_domain;
  mp->br_bit_header_length = hdr_len;
  mp->br_next_hop_out_label = ntohl (next_hop_out_label);
  mp->br_next_hop_proto_is_ip4 = next_hop_proto_is_ip4;

  if (next_hop_proto_is_ip4)
    {
      clib_memcpy (mp->br_next_hop,
