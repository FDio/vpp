...
static void
vl_api_create_virtio_pci_reply_t_handler (vl_api_create_virtio_pci_reply_t *
					  mp)
{
  vat_main_t *vam = &vat_main;
  i32 retval = ntohl (mp->retval);
...
static void vl_api_create_virtio_pci_reply_t_handler_json
  (vl_api_create_virtio_pci_reply_t * mp)
{
  vat_main_t *vam = &vat_main;
  vat_json_node_t node;
...
_(CREATE_VIRTIO_PCI_REPLY, create_virtio_pci_reply)			\
...
static int
api_create_virtio_pci (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  vl_api_create_virtio_pci_t *mp;
  u8 mac_address[6];
  u8 random_mac = 1;
  u32 pci_addr = 0;
