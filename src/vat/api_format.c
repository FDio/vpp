...
api_bier_route_add_del (vat_main_t * vam)
{
  ...
  mp->br_paths[0].dpo_proto = (next_hop_proto_is_ip4 ?
                               DPO_PROTO_IP4 :
                               DPO_PROTO_IP6);
...
vl_api_mpls_fib_path_print (vat_main_t * vam, vl_api_fib_path_t * fp)
{
  if (fp->dpo_proto == DPO_PROTO_IP6)
    print (vam->ofp,
	   ...
	   fp->is_drop, fp->is_unreach, fp->is_prohibit, fp->dpo_proto,
	   format_ip6_address, fp->next_hop);
  else if (fp->dpo_proto == DPO_PROTO_IP4)
    print (vam->ofp,
	   "  weight %d, sw_if_index %d, is_local %d, is_drop %d, "
	   "is_unreach %d, is_prohitbit %d, afi %d, next_hop %U",
	   ntohl (fp->weight), ntohl (fp->sw_if_index), fp->is_local,
				 ...
				 vl_api_fib_path_t * fp)
{
  ...
  vat_json_object_add_uint (node, "next_hop_dpo_proto", fp->dpo_proto);
  if (fp->dpo_proto == DPO_PROTO_IP4)
    {
      clib_memcpy (&ip4, &fp->next_hop, sizeof (ip4));
      vat_json_object_add_ip4 (node, "next_hop", ip4);
    }
  else if (fp->dpo_proto == DPO_PROTO_IP6)
    {
  ...
  (vl_api_ip_route_details_t * mp)
{
  ...
  if (mp->route.prefix.is_ip6) {
    clib_memcpy (&ip6, &mp->route.prefix.address, sizeof (ip6));
    vat_json_object_add_ip6 (node, "prefix", ip6);
  } else {
    clib_memcpy (&ip4, &mp->route.prefix.address, sizeof (ip4));
    vat_json_object_add_ip4 (node, "prefix", ip4);
  }
  vat_json_object_add_uint (node, "mask_length", mp->route.prefix.address_length);
  vat_json_object_add_uint (node, "path_count", count);
  for (i = 0; i < count; i++)
    {
      fp = &mp->route.paths[i];
