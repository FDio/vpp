...
format_vl_api_prefix (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U/%d", format_vl_api_address, &pfx->address, pfx->len);
...
uword
unformat_vl_api_address_family (unformat_input_t *input, va_list *args)
{
...
unformat_vl_api_mprefix (unformat_input_t *input, va_list *args)
{
  ...
  if (unformat (input, "%U/%d", unformat_vl_api_ip4_address,
		&pfx->grp_address.ip4, &pfx->grp_address_length))
    pfx->af = ADDRESS_IP4;
  else if (unformat (input, "%U/%d", unformat_vl_api_ip6_address,
		     &pfx->grp_address.ip6, &pfx->grp_address_length))
    pfx->af = ADDRESS_IP6;
  else if (unformat (input, "%U %U", unformat_vl_api_ip4_address,
		     &pfx->src_address.ip4, unformat_vl_api_ip4_address,
		     &pfx->grp_address.ip4))
    {
      pfx->af = ADDRESS_IP4;
      pfx->grp_address_length = 64;
		     ...
		     &pfx->src_address.ip6, unformat_vl_api_ip6_address,
		     &pfx->grp_address.ip6))
    {
      pfx->af = ADDRESS_IP6;
...
uword
unformat_vl_api_version (unformat_input_t *input, va_list *args)
{
  vl_api_version_t *ver = va_arg (*args, vl_api_version_t *);

  if (unformat (input, "%d.%d.%d-%s+%s", ver->major, ver->minor, ver->patch,
		ver->pre_release, ver->build_metadata))
    return (1);
  else if (unformat (input, "%d.%d.%d-%s", ver->major, ver->minor, ver->patch,
		     ver->pre_release))
    return (1);
