...
sock_test_cfg_sync (sock_test_socket_t *socket)
{
  ...
  tx_bytes = sock_test_write (socket->fd, (uint8_t *) & socket->cfg,
			      sizeof (socket->cfg), NULL,
			      socket->cfg.verbose);
  if (tx_bytes < 0)
    {
  ...
  if ((rx_bytes != sizeof (sock_test_cfg_t))
      || !sock_test_cfg_verify (rl_cfg, &socket->cfg))
    {
      fprintf (stderr,
  ...
  ctrl->cfg.ctrl_handle = rl_cfg->ctrl_handle;
  return 0;
}
...
echo_test_client ()
{
  ...
  for (i = 0; i < ctrl->cfg.num_test_sockets; i++)
      ...
      if (ctrl->cfg.verbose)
	{
	  ...
	  sprintf (buf, "CLIENT (fd %d) RESULTS",
                   tsock->fd);
...
stream_test_client (sock_test_t test)
{
  ...
  printf ("\n" SOCK_TEST_BANNER_STRING
	  ...
	  ctrl->fd, test == SOCK_TEST_TYPE_BI ? "Bi" : "Uni");
  ...
  for (n = 0; n != ctrl->cfg.num_test_sockets; n++)
    {
      ...
      tsock->cfg = ctrl->cfg;
      sock_test_socket_buf_alloc (tsock);
      printf ("\nCLIENT (fd %d): Sending config to server on "
              "test socket %d...\n", tsock->fd, n);
      sock_test_cfg_sync (tsock);
...
main (int argc, char **argv)
{
  ...
  opterr = 0;
  while ((c = getopt (argc, argv, "chn:w:XE:N:R:T:UBV")) != -1)
    switch (c)
      {
      case 'c':
	scm->dump_cfg = 1;
...
#ifdef VCL_TEST
      rv = vppcom_session_connect (ctrl->fd, &server_endpt);
#else
      rv =
	connect (ctrl->fd, (struct sockaddr *) &server_addr,
		 sizeof (server_addr));
#endif
      if (rv < 0)
	{
  ...
  scm->test_socket =
    malloc (sizeof (sock_test_socket_t ) * ctrl->cfg.num_test_sockets);

  for (i = 0; i < ctrl->cfg.num_test_sockets; i++)
    {
      tsock = &scm->test_socket[i];
#ifdef VCL_TEST
      tsock->fd = vppcom_session_create (VPPCOM_VRF_DEFAULT, VPPCOM_PROTO_TCP,
                                        0 /* is_nonblocking */ );
      if (tsock->fd < 0)
        {
          errno = -tsock->fd;
...
#ifdef VCL_TEST
      rv = vppcom_session_connect (tsock->fd, &server_endpt);
#else
      rv =
	connect (tsock->fd, (struct sockaddr *) &server_addr,
		 sizeof (server_addr));
#endif
      if (rv < 0)
	{
  ...
  printf ("CLIENT: All sockets connected!\n");
