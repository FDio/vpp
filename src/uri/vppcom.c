...
#if (CLIB_DEBUG > 0)
/* Set VPPCOM_DEBUG 2 for read/write debug output */
...
vppcom_send_connect_sock (session_t * session, u32 session_index)
{
  ...
  cmp->app_connect = htonl (session_index);
...
#define _(V,B,N)                                \
  if (B)                                        \
    {                                           \
      clib_bitmap_validate (V, N);              \
      clib_memcpy (V, B, vec_len (V));          \
      memset (B, 0, vec_len (V));               \
    }

  _(vcm->rd_bitmap, read_map, n_bits);
  _(vcm->wr_bitmap, write_map, n_bits);
  _(vcm->ex_bitmap, except_map, n_bits);
#undef _
  ...
  do
    {
      ...
      clib_bitmap_foreach (session_index, vcm->rd_bitmap,
        ({
          ...
          clib_spinlock_unlock (&vcm->sessions_lockp);
          if (clib_bitmap_get (vcm->ex_bitmap, session_index) && (rv < 0))
            {
              // TBD: clib_warning
...
select_done:
  vec_reset_length (vcm->rd_bitmap);
  vec_reset_length (vcm->wr_bitmap);
  vec_reset_length (vcm->ex_bitmap);
  return (bits_set);
}
