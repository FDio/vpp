...
get_nfds (void)
{
  ...
  for (nfds = i = 0; i < FD_SETSIZE; i++)
    {
      if (FD_ISSET (i, &ssm->rd_fdset) ||
          FD_ISSET (i, &ssm->wr_fdset))
        nfds = i + 1;
    }
  return nfds;
}
  ...
  if (rx_cfg->ctrl_handle == conn->fd)
    {
      ...
      for (i = 0; i < ssm->conn_pool_size; i++)
        {
          ...
          if (tc->cfg.ctrl_handle == conn->fd)
            {
              tc->stats.start = conn->stats.start;
              ...
              if (conn->cfg.verbose)
                {
                  ...
                  sock_test_stats_dump (buf, &tc->stats,
                                        1 /* show_rx */ ,
      ...
      sock_test_stats_dump ("SERVER RESULTS", &conn->stats,
                            1 /* show_rx */ ,
      ...
      printf ("SERVER (fd %d): %s-directional Stream Test Complete!\n"
...
#else
  rv = bind (ssm->listen_fd, (struct sockaddr *) &servaddr, sizeof (servaddr));
#endif
  if (rv < 0)
    {
      errno_val = errno;
      ...
      for (i = 0; i < ssm->conn_pool_size; i++)
        {
          ...
          if ((client_fd < FD_SETSIZE) && FD_ISSET (client_fd, rfdset))
            {
