...
typedef struct _svm_fifo
{
  ...
  volatile u32 want_ntf;	/**< producer wants nudge */
  volatile u32 has_ntf;
 ...
 */
static inline void
svm_fifo_add_want_ntf (svm_fifo_t * f, u8 ntf_type)
{
  f->want_ntf |= ntf_type;
}
 ...
 */
static inline void
svm_fifo_del_want_ntf (svm_fifo_t * f, u8 ntf_type)
{
  f->want_ntf &= ~ntf_type;
}
 ...
 */
static inline void
svm_fifo_clear_ntf (svm_fifo_t * f)
{
  /* Set the flag if want_notif_if_full was the only ntf requested */
  f->has_ntf = f->want_ntf == SVM_FIFO_WANT_NOTIF_IF_FULL;
  svm_fifo_del_want_ntf (f, SVM_FIFO_WANT_NOTIF);
}
 ...
 */
static inline void
svm_fifo_reset_has_ntf (svm_fifo_t * f)
{
  f->has_ntf = 0;
}
 ...
 * an event needs to be generated.
 *
 * @param f		fifo
 ...
 */
static inline u8
svm_fifo_needs_ntf (svm_fifo_t * f, u32 n_last_deq)
{
  u8 want_ntf = f->want_ntf;
  ...
  if (want_ntf & SVM_FIFO_WANT_NOTIF_IF_FULL)
    {
      ...
      u32 nitems = f->nitems;
      if (!f->has_ntf && max_deq < nitems && max_deq + n_last_deq >= nitems)
	return 1;
    }
  if (want_ntf & SVM_FIFO_WANT_NOTIF_IF_EMPTY)
    {
      if (!f->has_ntf && svm_fifo_is_empty (f))
	return 1;
    }
  return 0;
}
