...
//#define distance_to_tail(f, a) ((f->nitems + a - f->tail) % f->nitems)

always_inline u32
distance_to_tail (svm_fifo_t *f, u32 a)
{
...
//  return s->start;
  return distance_to_tail (f, s->start);
}

always_inline u32
ooo_segment_end_offset (svm_fifo_t *f, ooo_segment_t *s)
{
  return distance_to_tail (f, s->start) + s->length;
}
