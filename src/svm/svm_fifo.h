...
svm_fifo_head (svm_fifo_t * f)
{
  if (!f->shr->head_chunk)
    return 0;
  /* load-relaxed: consumer owned index */
  return (f->shr->head_chunk->data + (f->shr->head - f->shr->head_chunk->start_byte));
}
...
{
  /* load-relaxed: producer owned index */
  return (f->shr->tail_chunk->data + (f->shr->tail - f->shr->tail_chunk->start_byte));
}
...
svm_fifo_clear_deq_ntf (svm_fifo_t * f)
{
  ...
  f->shr->has_deq_ntf = f->shr->want_deq_ntf == SVM_FIFO_WANT_DEQ_NOTIF_IF_FULL;
  ...
  if (want_ntf & SVM_FIFO_WANT_DEQ_NOTIF_IF_FULL)
    {
      ...
      u32 size = f->shr->size;
      if (!f->shr->has_deq_ntf && max_deq < size && max_deq + n_last_deq >= size)
	return 1;
    }
