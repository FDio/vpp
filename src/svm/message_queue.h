 ...
 *
 * Allocates a message queue on the heap. This is heap agnostic, so the
 * function should only be called once the heap has been swapped. Based
 * on the configuration options, apart from the message queue this also
 * allocates (one or multiple) shared-memory rings for the messages.
 *
 * @param cfg 	configuration options (queue len, consumer pid, ring configs)
