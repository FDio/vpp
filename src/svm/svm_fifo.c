...
svm_fifo_max_read_chunk (svm_fifo_t * f)
{
  ...
  ASSERT (!f->shr->head_chunk || f_chunk_includes_pos (f->shr->head_chunk, head));
...
svm_fifo_max_write_chunk (svm_fifo_t * f)
{
  ...
  ASSERT (!f->shr->tail_chunk || f_chunk_includes_pos (f->shr->tail_chunk, tail));
...
svm_fifo_enqueue (svm_fifo_t * f, u32 len, const u8 * src)
{
  ...
  svm_fifo_copy_to_chunk (f, f->shr->tail_chunk, tail, src, len, &f->shr->tail_chunk);
  tail = tail + len;
...
svm_fifo_enqueue_nocopy (svm_fifo_t * f, u32 len)
{
  ...
  if (rb_tree_is_init (&f->ooo_enq_lookup))
    {
      f->shr->tail_chunk = f_lookup_clear_enq_chunks (f, f->shr->tail_chunk, tail);
      f->ooo_enq = 0;
    }
  else
    {
      f->shr->tail_chunk = svm_fifo_find_next_chunk (f, f->shr->tail_chunk, tail);
    }
...
svm_fifo_dequeue (svm_fifo_t * f, u32 len, u8 * dst)
{
  ...
  svm_fifo_copy_from_chunk (f, f->shr->head_chunk, head, dst, len, &f->shr->head_chunk);
  head = head + len;
  ...
  if (f_pos_geq (head, f_chunk_end (f->shr->start_chunk)))
    {
      ...
      f->shr->head_chunk =
	f_chunk_includes_pos (f->shr->start_chunk, head) ? f->shr->start_chunk : 0;
    }
...
svm_fifo_clone (svm_fifo_t * df, svm_fifo_t * sf)
{
  ...
  clib_memcpy_fast (df->shr->head_chunk->data, sf->shr->head_chunk->data, sf->shr->size);
...
svm_fifo_is_sane (svm_fifo_t * f)
{
  ...
  if (f->shr->head_chunk && !f_chunk_includes_pos (f->shr->head_chunk, f->shr->head))
    return 0;
  if (f->shr->tail_chunk && !f_chunk_includes_pos (f->shr->tail_chunk, f->shr->tail))
    return 0;
  if (f->ooo_deq)
    {
      if (rb_tree_is_init (&f->ooo_deq_lookup))
	{
	  if (f_pos_lt (f->ooo_deq->start_byte, f->shr->start_chunk->start_byte)
  ...
  if (f->ooo_enq)
    {
      if (rb_tree_is_init (&f->ooo_enq_lookup))
	{
	  if (f_pos_lt (f->ooo_enq->start_byte, f->shr->start_chunk->start_byte)
