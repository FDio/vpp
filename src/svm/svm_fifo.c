...
{
  return (distance_to_tail (f, a) < distance_to_tail(f, b));
}
...
{
  return distance_to_tail (f, posa) - distance_to_tail (f, posb);
}
...
//  end_offset = offset + length;
  ...
  if (prev && position_leq (f, normalized_position, ooo_segment_end_pos (f, prev)))
    {
      s = prev;
...
u32 last_diff, last_bytes_enqueued, entered_while, last_tail, last_orig_tail, last_norm_start;
/**
...
//  orig_tail = ((nitems + f->tail) - n_bytes_enqueued) % nitems;
//  norm_start = s->start % nitems;
//  diff = (norm_start >= orig_tail) ?
//	  norm_start - orig_tail : (nitems + norm_start) - orig_tail;

  diff = (f->tail >= s->start) ?
      f->tail - s->start : f->nitems + f->tail - s->start;

//  last_diff = diff;
//  last_bytes_enqueued = n_bytes_enqueued;
//  entered_while = 0;
//  last_tail = f->tail;
//  last_orig_tail = orig_tail;
//  last_norm_start = norm_start;
