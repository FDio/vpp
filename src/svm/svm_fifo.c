...
#define MARCH_FN(fn)							\
  ...
  always_inline int CLIB_CPU_OPTIMIZED CLIB_MARCH_SFX (fn ## _ma)	\
...
MARCH_FN (svm_fifo_peek) (svm_fifo_t * f,
			  u32 relative_offset, u32 max_bytes, u8 * copy_here)
{
  u32 total_copy_bytes, first_copy_bytes, second_copy_bytes;
...
#define _(_fn) CLIB_MARCH_SELECT_FN(_fn ## _ma);
foreach_svm_fifo_march_fn
#undef _
void __clib_constructor
svm_fifo_select_march_fns (void)
{
#define _(_fn) _fn ## _selected = _fn ## _ma_multiarch_select ();
  foreach_svm_fifo_march_fn
#undef _
}
