...
{
  return (f_distance_from (f, a, tail) < f_distance_from (f, b, tail));
}
...
{
  return f_distance_from (f, posa, tail) - f_distance_from (f, posb, tail);
}
...
ooo_segment_add (svm_fifo_t * f, u32 offset, u32 head, u32 tail, u32 length)
{
  ...
  ASSERT (offset + length <= f_distance_from (f, head, tail)
	  || head == tail);
...
ooo_segment_try_collect (svm_fifo_t * f, u32 n_bytes_enqueued, u32 * tail)
{
  ...
  s = pool_elt_at_index (f->ooo_segments, f->ooos_list_head);
  diff = f_distance_to (f, s->start, *tail);
