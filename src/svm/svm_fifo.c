...
svm_fifo_update_ooo_deq (svm_fifo_t * f, u32 pos)
{
  ...
  if (svm_fifo_chunk_includes_pos (c, pos))
      ...
      if (c->rb_index != RBTREE_TNIL_INDEX)
	{
      ...
      prev = rb_node (rt, f->ooo_deq->rb_index);
      c->rb_index = rb_tree_add_w_parent (rt, c->start_byte,
                                          pointer_to_uword (c), prev);
      f->ooo_deq = c;
      return;
...
static svm_fifo_chunk_t *
svm_fifo_clear_ooo_chunk (svm_fifo_t * f, svm_fifo_chunk_t *start, u32 pos)
{
  rb_tree_t *rt = &f->ooo_deq_lookup;
  svm_fifo_chunk_t *c;
  rb_node_t *n;
...
svm_fifo_enqueue (svm_fifo_t * f, u32 len, const u8 * src)
{
  ...
  if (PREDICT_FALSE (f->ooos_list_head != OOO_SEGMENT_INVALID_INDEX))
	...
	f->tail_chunk = svm_fifo_clear_ooo_chunk (f, f->tail_chunk, tail);
    }
...
svm_fifo_dequeue_drop (svm_fifo_t * f, u32 len)
{
  ...
  if (!svm_fifo_chunk_includes_pos (f->head_chunk, head))
    f->head_chunk = svm_fifo_clear_ooo_chunk (f, f->head_chunk, head);
