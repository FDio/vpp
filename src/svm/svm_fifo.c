  ...
  if (n_chunk <= len)
    {
      ...
      c = fs_chunk_ptr (f->fs_hdr, c->next);
      while ((to_copy -= n_chunk))
	{
	  n_chunk = clib_min (c->length, to_copy);
	  clib_memcpy_fast (&c->data[0], src + (len - to_copy), n_chunk);
	  c = c->length <= to_copy ? fs_chunk_ptr (f->fs_hdr, c->next) : c;
	}
      if (*last)
	*last = fs_chunk_sptr (f->fs_hdr, c);
    }
  else
    {
...
svm_fifo_init (svm_fifo_t * f, u32 size)
{
  ...
  f->refcnt = 1;
  f->f_shr->head = f->f_shr->tail = f->flags = 0;
  f->f_shr->head_chunk = f->f_shr->tail_chunk = f->f_shr->start_chunk;
  f->ooo_deq = f->ooo_enq = 0;

  min_alloc = size > 32 << 10 ? size >> 3 : 4096;
  min_alloc = clib_min (min_alloc, 64 << 10);
  f->f_shr->min_alloc = min_alloc;

  /*
   * Initialize chunks
   */
  prev = f_start_chunk_ptr (f);
  prev->start_byte = 0;
  prev->enq_rb_index = prev->deq_rb_index = RBTREE_TNIL_INDEX;
  c = fs_chunk_ptr (f->fs_hdr, prev->next);
...
svm_fifo_alloc (u32 data_size_in_bytes)
{
  ...
  f->f_shr->start_chunk = f->f_shr->end_chunk = c;
...
svm_fifo_find_chunk (svm_fifo_t * f, u32 pos)
{
  ...
  c = f_start_chunk_ptr (f);
  while (c && !f_chunk_includes_pos (c, pos))
    c = f_chunk_ptr (f, c->next);
...
svm_fifo_max_read_chunk (svm_fifo_t * f)
{
  ...
  ASSERT (!f->f_shr->head_chunk || f_chunk_includes_pos (f_head_chunk_ptr (f), head));

  if (!f->f_shr->head_chunk)
    {
      f->f_shr->head_chunk = f_chunk_sptr (f, svm_fifo_find_chunk (f, head));
      if (PREDICT_FALSE (!f->f_shr->head_chunk))
	return 0;
    }

  end_chunk = f_chunk_end (f_head_chunk_ptr (f));
...
svm_fifo_max_write_chunk (svm_fifo_t * f)
{
  ...
  ASSERT (!f->f_shr->tail_chunk || f_chunk_includes_pos (f->f_shr->tail_chunk, tail));

  return f->f_shr->tail_chunk ? f_chunk_end (f->f_shr->tail_chunk) - tail : 0;
}
...
f_update_ooo_enq (svm_fifo_t * f, u32 start_pos, u32 end_pos)
{
  ...
  if (PREDICT_FALSE (!rb_tree_is_init (rt)))
    {
      f->ooo_enq = svm_fifo_find_next_chunk (f, f->f_shr->tail_chunk, start_pos);
      return;
    }

  if (rt->root == RBTREE_TNIL_INDEX)
    {
      c = f->f_shr->tail_chunk;
...
svm_fifo_overwrite_head (svm_fifo_t * f, u8 * src, u32 len)
{
  ...
  if (len <= n_chunk)
    ...
    {
      ASSERT (len - n_chunk <= f_chunk_ptr (f, c->next)->length);
      clib_memcpy_fast (&c->data[head_idx], src, n_chunk);
      clib_memcpy_fast (&f_chunk_ptr (f, c->next)->data[0],
                        src + n_chunk, len - n_chunk);
    }
}
...
f_try_chunk_alloc (svm_fifo_t * f, u32 head, u32 tail, u32 len)
{
  ...
  prev = fs_chunk_ptr (f->fs_hdr, f->f_shr->end_chunk);
  free_alloced = f_chunk_end (prev) - tail;

  alloc_size = clib_min (f->f_shr->min_alloc, f->f_shr->size - (tail - head));
  alloc_size = clib_max (alloc_size, len - free_alloced);

  c = fsh_alloc_chunk (f->fs_hdr, f->f_shr->slice_index, alloc_size);
  if (PREDICT_FALSE (!c))
    return -1;
  ...
  while (cur)
    {
      ...
      prev = cur;
      cur = fs_chunk_ptr (f->fs_hdr, cur->next);
    }

  fs_chunk_sptr_link (f->fs_hdr, f->f_shr->end_chunk, c);
  prev->next = 0;
  f->f_shr->end_chunk = fs_chunk_sptr (f->fs_hdr, prev);

  if (!f->f_shr->tail_chunk)
    f->f_shr->tail_chunk = fs_chunk_sptr (f->fs_hdr, c);
...
svm_fifo_enqueue (svm_fifo_t * f, u32 len, const u8 * src)
{
  ...
  if (f_pos_gt (tail + len, f_chunk_end (f->f_shr->end_chunk)))
    {
      if (PREDICT_FALSE (f_try_chunk_alloc (f, head, tail, len)))
	{
	  len = f_chunk_end (f->f_shr->end_chunk) - tail;
	  if (!len)
	    return SVM_FIFO_EGROW;
	}
    }

  old_tail_c = f->f_shr->tail_chunk;

  svm_fifo_copy_to_chunk (f, f->f_shr->tail_chunk, tail, src, len, &f->f_shr->tail_chunk);
  tail = tail + len;
  ...
  if (PREDICT_FALSE (f->ooos_list_head != OOO_SEGMENT_INVALID_INDEX))
    {
      ...
      f->f_shr->tail_chunk = f_lookup_clear_enq_chunks (f, old_tail_c, tail);
      f->ooo_enq = 0;
    }

  /* store-rel: producer owned index (paired with load-acq in consumer) */
  clib_atomic_store_rel_n (&f->f_shr->tail, tail);
...
svm_fifo_enqueue_with_offset (svm_fifo_t * f, u32 offset, u32 len, u8 * src)
{
  ...
  if (f_pos_gt (enq_pos + len, f_chunk_end (f->f_shr->end_chunk)))
    {
...
svm_fifo_enqueue_nocopy (svm_fifo_t * f, u32 len)
{
  ...
  if (rb_tree_is_init (&f->ooo_enq_lookup))
    {
      f->f_shr->tail_chunk = f_lookup_clear_enq_chunks (f, f->f_shr->tail_chunk, tail);
      f->ooo_enq = 0;
    }
  else
    {
      f->f_shr->tail_chunk = svm_fifo_find_next_chunk (f, f->f_shr->tail_chunk, tail);
    }
			   ...
			   u32 n_segs, u8 allow_partial)
{
  ...
  if (!allow_partial)
      ...
      if (f_pos_gt (tail + len, f_chunk_end (f->f_shr->end_chunk)))
	{
      ...
      for (i = 0; i < n_segs; i++)
	{
	  svm_fifo_copy_to_chunk (f, f->f_shr->tail_chunk, tail, segs[i].data,
				  segs[i].len, &f->f_shr->tail_chunk);
	  tail += segs[i].len;
	}
    }
  else
...
f_unlink_chunks (svm_fifo_t * f, u32 end_pos, u8 maybe_ooo)
{
  ...
  prev->next = 0;
  start = f_start_chunk_ptr (f);
  f->f_shr->start_chunk = fs_chunk_sptr (f->fs_hdr, c);
...
svm_fifo_dequeue (svm_fifo_t * f, u32 len, u8 * dst)
{
  ...
  svm_fifo_copy_from_chunk (f, f_head_chunk_ptr (f), head, dst, len, &f->f_shr->head_chunk);
  head = head + len;
  ...
  if (f_pos_geq (head, f_chunk_end (f_start_chunk_ptr (f))))
    fsh_collect_chunks (f->fs_hdr, f->f_shr->slice_index,
			f_unlink_chunks (f, head, 0));

  /* store-rel: consumer owned index (paired with load-acq in producer) */
  clib_atomic_store_rel_n (&f->f_shr->head, head);
  ...
  if (f_pos_geq (head, f_chunk_end (f_start_chunk_ptr (f))))
    {
      ...
      f->f_shr->head_chunk =
	f_chunk_includes_pos (f_start_chunk_ptr (f), head) ? f->f_shr->start_chunk : 0;
    }
...
svm_fifo_dequeue_drop_all (svm_fifo_t * f)
{
  ...
  if (!f->f_shr->head_chunk || !f_chunk_includes_pos (f_head_chunk_ptr (f), head))
    f->f_shr->head_chunk = f_chunk_sptr (f, svm_fifo_find_chunk (f, head));

  f->f_shr->head_chunk = f_chunk_sptr (
      f, f_lookup_clear_deq_chunks (f, f_head_chunk_ptr (f), tail));

  if (f_pos_geq (tail, f_chunk_end (f_start_chunk_ptr (f))))
    fsh_collect_chunks (f->fs_hdr, f->f_shr->slice_index,
			f_unlink_chunks (f, tail, 0));

  /* store-rel: consumer owned index (paired with load-acq in producer) */
  clib_atomic_store_rel_n (&f->f_shr->head, tail);
}
...
svm_fifo_fill_chunk_list (svm_fifo_t * f)
{
  ...
  if (f_chunk_end (f->f_shr->end_chunk) - head >= f->f_shr->size)
    return 0;

  if (f_try_chunk_alloc (f, head, tail, f->f_shr->size - (tail - head)))
    return SVM_FIFO_EGROW;
...
svm_fifo_clone (svm_fifo_t * df, svm_fifo_t * sf)
{
  ...
  clib_memcpy_fast (f_head_chunk_ptr (df)->data, f_head_chunk_ptr (sf)->data,
                    sf->f_shr->size);

  f_load_head_tail_all_acq (sf, &head, &tail);
  clib_atomic_store_rel_n (&df->f_shr->head, head);
  clib_atomic_store_rel_n (&df->f_shr->tail, tail);
}
...
svm_fifo_init_pointers (svm_fifo_t * f, u32 head, u32 tail)
{
  ...
  ASSERT (c != 0);
  f->ooo_deq = c;
  f->f_shr->head_chunk = f_chunk_sptr (f, c);
  c = svm_fifo_find_chunk (f, tail);
  ASSERT (c != 0);
  f->ooo_enq = c;
  f->f_shr->tail_chunk = f_chunk_sptr (f, c);
}

void
svm_fifo_add_subscriber (svm_fifo_t * f, u8 subscriber)
{
  if (f->f_shr->n_subscribers >= SVM_FIFO_MAX_EVT_SUBSCRIBERS)
    return;
  f->f_shr->subscribers[f->f_shr->n_subscribers++] = subscriber;
}
...
svm_fifo_del_subscriber (svm_fifo_t * f, u8 subscriber)
{
  ...
  for (i = 0; i < f->f_shr->n_subscribers; i++)
    {
      if (f->f_shr->subscribers[i] != subscriber)
	continue;
      f->f_shr->subscribers[i] = f->f_shr->subscribers[f->f_shr->n_subscribers - 1];
      f->f_shr->n_subscribers--;
      break;
    }
}
...
svm_fifo_is_sane (svm_fifo_t * f)
{
  ...
  if (f->f_shr->head_chunk && !f_chunk_includes_pos (f_head_chunk_ptr (f), f->f_shr->head))
    return 0;
  if (f->f_shr->tail_chunk && !f_chunk_includes_pos (f_tail_chunk_ptr (f), f->f_shr->tail))
    return 0;
  if (f->ooo_deq)
    {
      if (rb_tree_is_init (&f->ooo_deq_lookup))
	{
	  if (f_pos_lt (f->ooo_deq->start_byte, f_start_chunk_ptr (f)->start_byte)
	      || f_pos_gt (f->ooo_deq->start_byte,
			   f_chunk_end (f_end_chunk_ptr (f))))
	    return 0;
  ...
  if (f->ooo_enq)
    {
      if (rb_tree_is_init (&f->ooo_enq_lookup))
	{
	  if (f_pos_lt (f->ooo_enq->start_byte, f_start_chunk_ptr (f)->start_byte)
	...
	{
	  tmp = svm_fifo_find_next_chunk (f, f->f_shr->tail_chunk,
					  f->ooo_enq->start_byte);
	}
      if (tmp != f->ooo_enq)
	return 0;
  ...
  if (f_start_chunk_ptr (f)->next)
      ...
      do
	{
      ...
      if (chunks_bytes < f->f_shr->tail - f->f_shr->head)
	return 0;
    }
...
svm_fifo_replay (u8 * s, svm_fifo_t * f, u8 no_read, u8 verbose)
{
  ...
  placeholder_fifo = svm_fifo_alloc (f->f_shr->size);
  svm_fifo_init (f, f->f_shr->size);
  clib_memset (f_head_chunk_ptr (f)->data, 0xFF, f->f_shr->size);
  vec_validate (data, f->f_shr->size);
...
format_svm_fifo (u8 * s, va_list * args)
{
  ...
  s = format (s, "cursize %u nitems %u has_event %d min_alloc %u\n",
	      svm_fifo_max_dequeue (f), f->f_shr->size, f->f_shr->has_event, f->f_shr->min_alloc);
  s = format (s, "%Uhead %u tail %u segment manager %u\n", format_white_space,
	      indent, f->f_shr->head, f->f_shr->tail, f->segment_manager);

  if (verbose > 1)
    s = format (s, "%Uvpp session %d thread %d app session %d thread %d\n",
		format_white_space, indent, f->f_shr->master_session_index,
		f->master_thread_index, f->f_shr->client_session_index,
