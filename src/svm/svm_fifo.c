...
svm_fifo_overwrite_head (svm_fifo_t * f, u8 * data, u32 len)
{
  ...
  if (len <= first_chunk)
  ...
  else
    {
      ...
      clib_memcpy_fast (&f->head_chunk->data[0], data + first_chunk,
			len - first_chunk);
    }
}
#endif
  ...
  if (PREDICT_TRUE (copy_here != 0))
    {
      ...
      if (first_copy_bytes < total_copy_bytes)
	{
	  clib_memcpy_fast (copy_here, &f->head_chunk->data[head_idx],
			    first_copy_bytes);
	  /* number of bytes in second copy segment */
	  second_copy_bytes = total_copy_bytes - first_copy_bytes;
	  /* wrap around */
  ...
  if (PREDICT_TRUE (copy_here != 0))
    {
      ...
      if (first_copy_bytes < total_copy_bytes)
	{
	  clib_memcpy_fast (copy_here,
			    &f->head_chunk->data[relative_head_idx],
			    first_copy_bytes);
