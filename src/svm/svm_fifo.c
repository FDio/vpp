...
ooo_segment_add (svm_fifo_t * f, u32 offset, u32 head, u32 tail, u32 length)
{
  ...
  if (prev && f_pos_leq (offset_pos, ooo_segment_end_pos (f, prev)))
    {
      s = prev;
  ...
  if (f_pos_gt (offset_end_pos, s_end_pos))
    {
      ...
      while (it && f_pos_leq (ooo_segment_end_pos (f, it), offset_end_pos))
	{
...
f_update_ooo_enq (svm_fifo_t * f, u32 start_pos, u32 end_pos)
{
  ...
  if (rt->root == RBTREE_TNIL_INDEX)
    {
      c = f->tail_chunk;
      if (f_pos_gt (c->start_byte, start_pos))
	os_panic ();
      if (pool_elts (rt->nodes) > 1)
	os_panic ();

      c->rb_index = rb_tree_add_custom (rt, c->start_byte,
...
static svm_fifo_chunk_t *
f_lookup_clear_chunks (svm_fifo_t * f, rb_tree_t * rt,
		       svm_fifo_chunk_t * start, u32 end_pos)
{
  svm_fifo_chunk_t *c;
  rb_node_t *n;
...
svm_fifo_add_chunk (svm_fifo_t * f, svm_fifo_chunk_t * c)
{
  ...
  if (!f->tail_chunk)
    {
      f->tail_chunk = c;
      if (f_pos_gt (c->start_byte, f->tail))
	os_panic ();
    }
  ...
  if (tail_pos >= head && tail_pos < f->end_chunk->start_byte)
    {
      ...
      prev = svm_fifo_find_chunk (f, clib_max (f->size, last_pos));
      next = prev->next;
...
svm_fifo_reduce_size (svm_fifo_t * f, u32 len, u8 try_shrink)
{
  ...
  cur = svm_fifo_find_chunk (f, f->size - len);
...
static int
f_try_grow (svm_fifo_t * f)
{
  svm_fifo_chunk_t *c;

  c = fsh_alloc_chunk (f->fs_hdr, f->slice_index, f->end_chunk->length);
  if (PREDICT_FALSE (!c))
    return -1;
  ...
  if (PREDICT_FALSE (f->ooos_list_head != OOO_SEGMENT_INVALID_INDEX))
    {
      ...
      f->tail_chunk = f_lookup_clear_chunks (f, &f->ooo_enq_lookup,
					     old_tail_c, tail);
    }
...
svm_fifo_enqueue_with_offset (svm_fifo_t * f, u32 offset, u32 len, u8 * src)
{
  ...
  if (!f->ooo_enq || !f_chunk_includes_pos (f->ooo_enq, tail_idx))
    f_update_ooo_enq (f, tail_idx, tail_idx + len);
...
svm_fifo_chunk_t *
f_unlink_chunks (svm_fifo_t * f, u32 head, u8 clean_lookup)
{
  svm_fifo_chunk_t *start, *prev, *c;
  rb_tree_t *rt;
  rb_node_t *n;

  if (f_chunk_includes_pos (f->start_chunk, head))
    os_panic ();

  ASSERT (!f_chunk_includes_pos (f->start_chunk, head));
  ...
  while (!f_chunk_includes_pos (c, head));
  ...
  if (clean_lookup)
    {
      if (f->ooo_deq && f_pos_lt (f->ooo_deq->start_byte, f_chunk_end (prev)))
	f->ooo_deq = c;
    }
  f->start_chunk = c;
...
//    f->head_chunk = f->start_chunk->next;
  ...
  if (f_pos_gt (head, f_chunk_end (f->start_chunk)))
...
//  fsh_collect_chunks (f->fs_hdr, f, f_unlink_chunks (f, tail, 1));

  f->head_chunk = f_lookup_clear_chunks (f, &f->ooo_deq_lookup,
					 f->head_chunk, tail - 1);
...
{
  if (!(f->flags & SVM_FIFO_F_SHRINK))
    return 0;
  if (!f_chunk_includes_pos (f->head_chunk, f->head))
    return 0;
  if (!f_chunk_includes_pos (f->tail_chunk, f->tail))
    return 0;
...
format_svm_fifo (u8 * s, va_list * args)
{
  ...
  s = format (s, "cursize %u nitems %u has_event %d\n",
	      svm_fifo_max_dequeue (f), f->size, f->has_event);
