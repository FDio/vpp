...
static void
svm_fifo_update_ooo_enq (svm_fifo_t * f, u32 start_pos,
			 u32 end_pos)
{
...
static void
svm_fifo_update_ooo_deq (svm_fifo_t * f, u32 start_pos,
			 u32 end_pos)
{
...
svm_fifo_enqueue_with_offset (svm_fifo_t * f, u32 offset, u32 len, u8 * src)
{
  ...
  if (!f_chunk_includes_pos (f->ooo_enq, tail_idx))
    svm_fifo_update_ooo_enq (f, tail_idx, tail_idx + len);
...
svm_fifo_dequeue (svm_fifo_t * f, u32 len, u8 * dst)
{
  ...
  if (f_pos_gt (head, f_chunk_end (f->start_chunk)))
    fsh_collect_chunks (f->fs_hdr, f, f_unlink_chunks (f, head, 0));
...
svm_fifo_peek (svm_fifo_t * f, u32 offset, u32 len, u8 * dst)
{
  ...
  if (!f_chunk_includes_pos (f->ooo_deq, head_idx))
    svm_fifo_update_ooo_deq (f, head_idx, (head_idx + len));
...
svm_fifo_dequeue_drop (svm_fifo_t * f, u32 len)
{
  ...
  if (f_pos_gt (head, f_chunk_end (f->start_chunk)))
    fsh_collect_chunks (f->fs_hdr, f, f_unlink_chunks (f, head, 1));
...
format_ooo_segment (u8 * s, va_list * args)
{
  ...
  s = format (s, "[%u, %u], len %u, next %d, prev %d", seg->start,
              seg->start + seg->length, seg->length,
	      seg->next, seg->prev);
  return s;
}
