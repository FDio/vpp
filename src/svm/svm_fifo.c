...
int
svm_fifo_commit_async_segments (svm_fifo_t *f)
{
  svm_fifo_async_state_t *state = f->async_state;
  svm_fifo_async_seg_t *segs;
  fs_sptr_t last;
  u32 expected, i, n_bytes = 0, n_segs, tail;
  ...
  segs = state->segs;
  n_segs = vec_len (segs);
  expected = (u32) state->tail - f->shr->tail;
  ...
  tail = f->shr->tail;
  last = f->shr->tail_chunk;
  state->tail = SVM_FIFO_ASYNC_TAIL_INVALID;
  tail = svm_fifo_copy_from_segments (f, f_tail_cptr (f), tail, segs, n_segs, &last);
  ASSERT (tail == f->shr->tail + expected);
