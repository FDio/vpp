 ...
 */
MARCH_FN (svm_fifo_enqueue_with_offset_ma) (svm_fifo_t * f,
					    u32 offset,
					    u32 required_bytes,
					    u8 * copy_from_here)
{
...
#ifndef CLIB_MARCH_VARIANT
CLIB_MARCH_SELECT_FN (svm_fifo_enqueue_with_offset_ma);
			      ...
			      u32 required_bytes, u8 * copy_from_here)
{
  ...
  if (PREDICT_FALSE (fp == 0))
    fp = (void *) svm_fifo_enqueue_with_offset_ma_multiarch_select ();

  return (*fp) (f, offset, required_bytes, copy_from_here);
}
...
MARCH_FN (svm_fifo_dequeue_nowait_ma) (svm_fifo_t * f, u32 max_bytes,
				       u8 * copy_here)
{
  u32 total_copy_bytes, first_copy_bytes, second_copy_bytes;
  u32 cursize, nitems;
