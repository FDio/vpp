  ...
  if (f_pos_geq (head, f_chunk_end (f_start_cptr (f))))
    {
      ...
      f->shr->head_chunk =
	f_chunk_includes_pos (f_start_cptr (f), head) ? f->shr->start_chunk : 0;
    }
...
svm_fifo_is_sane (svm_fifo_t * f)
{
  ...
  if (f->shr->head_chunk && !f_chunk_includes_pos (f_head_cptr (f), f->shr->head))
    return 0;
  if (f->shr->tail_chunk && !f_chunk_includes_pos (f_tail_cptr (f), f->shr->tail))
    return 0;
  if (f->ooo_deq)
    {
