...
svm_fifo_add_chunk (svm_fifo_t * f, svm_fifo_chunk_t * c)
{
   ...
   * called with the heap where the rbtree's pool is. */
  cur = c;
  prev = f->end_chunk;
  while (cur)
    {
      cur->start_byte = prev->start_byte + prev->length;
      rb_tree_add2 (&f->chunk_lookup, cur->start_byte, pointer_to_uword (c));
      prev = cur;
      cur = cur->next;
    }
...
svm_fifo_find_chunk (svm_fifo_t *f, u32 pos)
{
  ...
  while (pos != cur->key)
      ...
      if (rb_node_is_tnil (rt, cur))
	{
	  ...
	  if (pos < prev->key)
	    {
	      cur = rb_tree_find_predecessor (rt, prev);
	      return uword_to_pointer (prev->opaque, svm_fifo_chunk_t *);
	    }
	  /* Hit tnil as a right child. Check if this is the one, otherwise
	   * search for successor */
	  c = uword_to_pointer (prev->opaque, svm_fifo_chunk_t *);
	  if (pos < c->start_byte + c->length)
	    return c;

	  cur = rb_tree_find_successor (rt, prev);
	  return uword_to_pointer (prev->opaque, svm_fifo_chunk_t *);
	}
    }
...
static inline u8
svm_fifo_chunk_has_pos (svm_fifo_chunk_t *c, u32 pos)
{
...
svm_fifo_enqueue_with_offset (svm_fifo_t * f, u32 offset, u32 len, u8 * src)
{
  ...
  if (!svm_fifo_chunk_has_pos (f->ooo_enq, tail_idx))
