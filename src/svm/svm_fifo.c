...
always_inline int
svm_fifo_pos_lt (u32 a, u32 b)
{
  return ((i32)((a)-(b)) < 0);
}

//#define svm_fifo_pos_lt(_a, _b) ((i32)((_a)-(_b)) < 0)
#define svm_fifo_pos_leq(_a, _b) ((i32)((_a)-(_b)) <= 0)
#define svm_fifo_pos_gt(_a, _b) ((i32)((_a)-(_b)) > 0)
#define svm_fifo_pos_geq(_a, _b) ((i32)((_a)-(_b)) >= 0)
	       ...
	       svm_fifo_chunk_t ** last)
{
  ...
  ASSERT (svm_fifo_pos_geq (tail_idx, c->start_byte)
          && svm_fifo_pos_lt (tail_idx, c->start_byte + c->length));
	       ...
	       svm_fifo_chunk_t ** last)
{
  ...
  ASSERT (svm_fifo_pos_geq (head_idx, c->start_byte)
          && svm_fifo_pos_lt (head_idx, c->start_byte + c->length));
...
//  ASSERT (offset + length <= f_distance_to (f, head, tail) || head == tail);
  ASSERT (svm_fifo_pos_leq (offset + length, f_free_count (f, head, tail)));
...
//  offset_end_pos = (tail + offset + length) % f->size;
  ...
  if (prev && svm_fifo_pos_leq (offset_pos, ooo_segment_end_pos (f, prev)))
    {
      ...
      ASSERT (svm_fifo_pos_lt (s->start, offset_pos));
      goto check_tail;
    }
  ...
  if (svm_fifo_pos_lt (offset_end_pos, s->start))
    {
  ...
  else if (svm_fifo_pos_gt (offset_pos, s_end_pos))
    {
      new_s = ooo_segment_alloc (f, offset_pos, length);
      new_index = new_s - f->ooo_segments;
  ...
  /* Merge at head */
  if (svm_fifo_pos_lt (offset_pos, s->start))
    {
      s->start = offset_pos;
...
check_tail:
  ...
  /* Overlapping tail */
  if (svm_fifo_pos_gt (offset_end_pos, s_end_pos))
    {
//      s->length = position_diff (f, offset_end_pos, s->start, tail);
      ...
      while (it && svm_fifo_pos_leq (ooo_segment_end_pos (f, it),
                                     offset_end_pos))
	{
      ...
      if (it && svm_fifo_pos_leq (it->start, offset_end_pos))
	{
...
{
  return (svm_fifo_pos_geq (pos, c->start_byte)
	  && svm_fifo_pos_lt (pos, c->start_byte + c->length));
}
...
svm_fifo_find_node_rbtree (rb_tree_t * rt, u32 pos)
{
  ...
  while (pos != cur->key)
    {
      prev = cur;
      if (svm_fifo_pos_lt (pos, cur->key))
	{
...
//  if (svm_fifo_chunk_includes_pos (f->ooo_enq, end_pos))
//    return;

  if (rt->root == RBTREE_TNIL_INDEX)
    {
      c = f->tail_chunk;
      if (svm_fifo_pos_gt (c->start_byte, start_pos))
	os_panic ();
      if (pool_elts (rt->nodes) > 1)
	os_panic ();

//      c->rb_index = rb_tree_add2 (rt, c->start_byte, pointer_to_uword (c));
      c->rb_index = rb_tree_add_custom (rt, c->start_byte, pointer_to_uword (c),
                                        svm_fifo_pos_lt);
    }
  else
    {
...
//	}
  ...
  if (svm_fifo_chunk_includes_pos (c, end_pos))
    return;

  do
    {
      c = c->next;
      if (!c || c->rb_index != RBTREE_TNIL_INDEX)
	break;

      if (pool_elts (rt->nodes) > 2)
	os_panic ();
...
//  if (svm_fifo_chunk_includes_pos (start, end_pos))
//    return start;
...
svm_fifo_add_chunk (svm_fifo_t * f, svm_fifo_chunk_t * c)
{
  ...
  if (f->start_chunk->next->next && f->start_chunk->next->next->next)
    os_panic ();

  if (!f->tail_chunk)
    {
      f->tail_chunk = c;
      if (svm_fifo_pos_gt (c->start_byte, f->tail))
	os_panic ();
    }
...
always_inline u32
svm_fifo_chunk_end (svm_fifo_chunk_t *c)
{
...
svm_fifo_enqueue (svm_fifo_t * f, u32 len, const u8 * src)
{
  ...
  if (svm_fifo_pos_gt (tail + len, svm_fifo_chunk_end (f->end_chunk)))
    fsh_grow_fifo (f->fs_hdr, f, f->end_chunk->length);

  old_tail = f->tail_chunk;
...
//  tail_idx = (tail + offset) % f->size;
  tail_idx = tail + offset;

//  if (f->flags & SVM_FIFO_F_MULTI_CHUNK)

  if (svm_fifo_pos_gt (tail_idx + len, svm_fifo_chunk_end (f->end_chunk)))
    fsh_grow_fifo (f->fs_hdr, f, f->end_chunk->length);

  if (!svm_fifo_chunk_includes_pos (f->ooo_enq, tail_idx))
    svm_fifo_update_ooo_enq (f, f->tail, tail_idx, tail_idx + len);
...
svm_fifo_chunk_t *
svm_fifo_unlink_chunks (svm_fifo_t *f, u32 head, u8 clean_lookup)
{
  svm_fifo_chunk_t *start, *prev, *c;
  rb_tree_t *rt;
  rb_node_t *n;

  if (svm_fifo_chunk_includes_pos(f->start_chunk, head))
    os_panic ();

  ASSERT (!svm_fifo_chunk_includes_pos(f->start_chunk, head));

  rt = &f->ooo_deq_lookup;
  start = c = f->start_chunk;
  do
    {
      if (clean_lookup && c->rb_index != RBTREE_TNIL_INDEX)
	{
	  n = rb_node (rt, c->rb_index);
//	  ASSERT (n == svm_fifo_find_node_rbtree (rt, c->start_byte));
	  rb_tree_del_node (rt, n);
	  c->rb_index = RBTREE_TNIL_INDEX;
	}
      prev = c;
      c = c->next;
    } while (!svm_fifo_chunk_includes_pos(c, head));
...
svm_fifo_dequeue (svm_fifo_t * f, u32 len, u8 * dst)
{
  ...
  if (!f->head_chunk)
    f->head_chunk = f->start_chunk->next;
...
//    svm_fifo_try_grow (f, head);
  ...
  if (svm_fifo_pos_gt (head, svm_fifo_chunk_end (f->start_chunk)))
    fsh_collect_chunks (f->fs_hdr, f, svm_fifo_unlink_chunks (f, head, 0));
...
//  if (f->flags & SVM_FIFO_F_MULTI_CHUNK)
  if (!svm_fifo_chunk_includes_pos (f->ooo_deq, head_idx))
    svm_fifo_update_ooo_deq (f, head, head_idx, (head_idx + len));
...
//    svm_fifo_try_grow (f, head);
  ...
  if (svm_fifo_pos_gt (head, svm_fifo_chunk_end (f->start_chunk)))
    fsh_collect_chunks (f->fs_hdr, f, svm_fifo_unlink_chunks (f, head, 1));
