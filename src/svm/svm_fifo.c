		       ...
		       svm_fifo_chunk_t * start, u32 end_pos)
{
  ...
  if (f->ooos_list_head == OOO_SEGMENT_INVALID_INDEX
      && c && c->rb_index != RBTREE_TNIL_INDEX)
    {
      n = rb_node (rt, c->rb_index);
      rb_tree_del_node (rt, n);
      c->rb_index = RBTREE_TNIL_INDEX;
...
f_unlink_chunks (svm_fifo_t * f, u32 end_pos, u8 maybe_ooo)
{
  ...
  do
    {
      if (maybe_ooo && c->rb_index != RBTREE_TNIL_INDEX)
	{
	  n = rb_node (rt, c->rb_index);
  ...
  f->start_chunk = c;
  f->head_chunk = f_chunk_includes_pos (c, end_pos) ? c : 0;
  ...
  else if (PREDICT_FALSE (f->ooo_deq != 0))
    f->ooo_deq = 0;
  ...
  if (f->ooo_enq)
    {
	  ...
	  || f_pos_gt (f->ooo_enq->start_byte, f_chunk_end (f->end_chunk)))
	return 0;

      tmp = svm_fifo_find_chunk (f, f->ooo_enq->start_byte);
      if (tmp != f->ooo_enq)
	return 0;
    }
