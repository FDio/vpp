	       ...
	       const u8 * copy_from_here)
{
  ...
  if (PREDICT_TRUE (copy_from_here != 0))
	...
	? (nitems - f->tail) : total_copy_bytes;
      ...
      memcpy_s_inline (&f->data[f->tail], first_copy_bytes, copy_from_here,
		       first_copy_bytes);
	...
	{
	  memcpy_s_inline (&f->data[f->tail], second_copy_bytes,
	       ...
	       u32 offset, u32 required_bytes, u8 * copy_from_here)
{
  ...
  memcpy_s_inline (&f->data[normalized_offset], first_copy_bytes,
		   copy_from_here, first_copy_bytes);
  ...
  if (second_copy_bytes)
    {
      ...
      memcpy_s_inline (&f->data[normalized_offset], second_copy_bytes,
...
svm_fifo_overwrite_head (svm_fifo_t * f, u8 * data, u32 len)
{
  ...
  ASSERT (len <= f->nitems);
  if (len <= first_chunk)
    memcpy_s_inline (&f->data[f->head], len, data, len);
  else
    {
      memcpy_s_inline (&f->data[f->head], first_chunk, data, first_chunk);
      memcpy_s_inline (&f->data[0], len - first_chunk, data + first_chunk,
		       len - first_chunk);
    }
}
#endif
	       ...
	       u8 * copy_here)
{
  ...
  if (PREDICT_TRUE (copy_here != 0))
	...
	? (nitems - f->head) : total_copy_bytes;
      memcpy_s_inline (copy_here, first_copy_bytes, &f->data[f->head],
		       first_copy_bytes);
      ...
      if (second_copy_bytes)
	{
	  memcpy_s_inline (copy_here + first_copy_bytes, second_copy_bytes,
	       ...
	       u32 max_bytes, u8 * copy_here)
{
  ...
  if (PREDICT_TRUE (copy_here != 0))
	...
	(nitems - real_head) : total_copy_bytes;
      memcpy_s_inline (copy_here, first_copy_bytes, &f->data[real_head],
		       first_copy_bytes);
