...
fifo_segment_create (fifo_segment_main_t * sm, fifo_segment_create_args_t * a)
{
  ...
  baseva = a->segment_type == SSVM_SEGMENT_PRIVATE ? ~0 : sm->next_baseva;
...
static svm_fifo_t *
fs_try_alloc_freelist_multi_chunk (ssvm_shared_header_t * sh,
                                   fifo_segment_header_t * fsh,
                                   u32 data_bytes)
{
                   ...
                   u32 data_bytes)
{
  ...
  if (f)
    fsh->n_free_bytes -= svm_fifo_mem_used (f);
			 ...
			 fifo_segment_ftype_t ftype)
{
  ...
  if (!f)
    f = fs_try_alloc_fifo_batch (sh, fsh, data_bytes);
  if (!f)
    {
      u32 fifo_sz = 1 << max_log2 (data_bytes);
      fifo_sz += sizeof (*f) + sizeof (svm_fifo_chunk_t);
      if (fsh->n_free_bytes >= fifo_sz)
	f = fs_try_alloc_fifo (sh, fsh, data_bytes);
      if (!f && fsh->n_fl_chunk_bytes >= fifo_sz)
	f = fs_try_alloc_freelist_multi_chunk (sh, fsh, data_bytes);
      if (!f)
	goto done;
    }
