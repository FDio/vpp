			  ...
			  svm_msg_q_cfg_t *cfg)
{
  ...
  mq = vec_elt_at_index (fs->mq, mq_index);
...
{
  svm_msg_q_t *mq = vec_elt_at_index (fs->mq, mq_index);

  if (mq->q == 0)
    return ~0ULL;

  return (uword) ((u8 *) mq->q - (u8 *) fs->h) - sizeof (u32);
}
