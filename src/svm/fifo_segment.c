...
fsh_alloc_aligned (fifo_segment_header_t *fsh, uword size, uword align)
{
  ...
  return uword_to_pointer (cur_pos_align, void *);
}
...
{
  u32 free = 0;
  while (!clib_atomic_cmp_and_swap_acq_relax_n (&fss->chunk_lock, &free, 1,
                                                0))
    {
...
fss_chunk_free_list_pop (fifo_segment_header_t * fsh,
                         fifo_segment_slice_t * fss, u32 fl_index)
{
  svm_fifo_chunk_t *c;
...
//  int i;
  ...
  fsh->byte_index = seg_start + sizeof (*fsh) + slices_sz;
  fsh->max_byte_index = seg_start + seg_sz;
...
//    {
//      fss = fsh_slice_get (fsh, i);
////      vec_sz = max_chunk_idx * sizeof (*fss->free_chunks);
////      fss->free_chunks = fsh_alloc (fsh, vec_sz);
////      memset (fss->free_chunks, 0, vec_sz);
////      vec_sz = max_chunk_idx * sizeof (*fss->num_chunks);
...
//  fsh->n_free_bytes = fsh_free_space (fsh);
  max_fifo = clib_min ((seg_sz - slices_sz - 4096) / 2,
...
//  fsh->n_reserved_bytes = clib_min (0.01 * fsh->n_free_bytes, 256 << 10);
  fsh->n_reserved_bytes = clib_min (0.01 * seg_sz, 256 << 10);
  sh->ready = 1;
  return (0);
}
...
//  fs->ssvm.requested_va = sm->next_baseva;
 ...
 if ((rv = ssvm_client_init (&fs->ssvm, a->segment_type)))
  ...
  clib_warning ("segment mapped at %x", fs->ssvm.sh);
  fsh = fs->h = (void *) fs->ssvm.sh + (uword) fs->ssvm.sh->opaque[0];
  fs->max_byte_index = fsh->max_byte_index;
...
//  void *oldheap;
  ...
  cmem = fsh_alloc_aligned (fsh, size, CLIB_CACHE_LINE_BYTES);
//  oldheap = ssvm_push_heap (fsh->ssvm_sh);
 ...
 */
static svm_fifo_shared_t *
fs_try_alloc_fifo (fifo_segment_header_t * fsh, u32 slice_index,
		   u32 data_bytes)
{
  fifo_segment_slice_t * fss;
...
fs_fifo_alloc (fifo_segment_t *fs, u32 slice_index)
{
  ...
  pool_get (pfss->fifos, f);
  if (!(*f))
    *f = clib_mem_alloc (sizeof (svm_fifo_t));

  clib_memset ((*f), 0, sizeof (svm_fifo_t));
  (*f)->pool_index = f - pfss->fifos;
  // TODO store slice index
...
{
  u32 slice_index = f->f_shr->slice_index;
  fifo_slice_private_t *pfss;

  pfss = &fs->slices[slice_index];
  pool_put_index (pfss->fifos, f->pool_index);
}
...
fifo_segment_free_fifo (fifo_segment_t * fs, svm_fifo_t * f)
{
  ...
  /* Free fifo chunks */
  fsh_slice_collect_chunks (fsh, fss,
                            fs_chunk_ptr (fsh, f->f_shr->start_chunk));
  ...
  /* Add to free list */
  f->f_shr->next = fss->free_fifos;
//  f->f_shr->prev = 0;
  f->prev = 0;
  fss->free_fifos = f->f_shr;
...
fifo_segment_msg_q_offset (fifo_segment_t *fs)
{
  if (fs->mq.q == 0)
    return ~0ULL;
  return (uword) ((u8 *)fs->mq.q - (u8 *)fs->h);
}
