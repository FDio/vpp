...
static inline void
fs_free_bytes_sub (fifo_segment_header_t * fsh, int size)
{
  clib_atomic_fetch_sub_rel (&fsh->n_free_bytes, size);
}

static inline uword
fs_n_free_bytes (fifo_segment_header_t * fsh)
{
  return clib_atomic_load_relax_n (&fsh->n_free_bytes);
}

static fifo_segment_slice_t *
fs_slice_get (fifo_segment_header_t *fsh, u32 slice_index)
{
  return &fsh->slices[slice_index];
}

static inline void
fs_active_fifos_update (fifo_segment_header_t * fsh, int inc)
{
...
fifo_segment_init (fifo_segment_t * fs)
{
  ...
  for (i = 0; i < fs->n_slices; i++)
    {
      fss = fs_slice_get (fsh, i);
//      fsh->n_free_bytes = &fs->n_free_bytes;
      vec_validate_init_empty (fss->free_chunks, 30, 0);
    }
                                        ...
                                        u32 data_bytes)
{
  ...
  if (!f)
	...
	return 0;
      memset (f, 0, sizeof (*f));
      fs_free_bytes_sub (fsh, sizeof (*f));
//      fsh->n_free_bytes -= sizeof (*f);
  ...
  vec_validate_init_empty (fss->free_chunks, fl_index, 0);
...
//  vec_validate_init_empty (fss->free_chunks, fl_index, 0);
  ...
  n_free_bytes = fs_n_free_bytes (fsh);
                                  ...
                                  u32 data_bytes, fifo_segment_ftype_t ftype)
{
  ...
  fss = fs_slice_get (fsh, slice_index);
...
fifo_segment_free_fifo (fifo_segment_t * fs, svm_fifo_t * f)
{
  ...
  fss = fs_slice_get (fsh, f->slice_index);
...
//  sh = fs->ssvm.sh;
  ssvm_lock_non_recursive (fsh->ssvm_sh, 1);
  oldheap = ssvm_push_heap (fsh->ssvm_sh);
...
//  vec_validate_init_empty (fss->free_chunks, fl_index, 0);
  ...
  if (!c)
	...
	{
	  ssvm_pop_heap (oldheap);
	  ssvm_unlock_non_recursive (fsh->ssvm_sh);
	  return -1;
	}
      fs_free_bytes_sub (fsh, chunk_size + sizeof (*c));
