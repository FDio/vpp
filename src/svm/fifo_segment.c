...
svm_fifo_chunk_t *
fss_fl_try_alloc_multi_chunk (fifo_segment_header_t *fsh, fifo_segment_slice_t *fss, u32 data_bytes)
{
  ...
  if (fl_index > 0 && req_bytes <= fl_size)
    {
      fl_index -= 1;
      fl_size >>= 1;
    }
...
static svm_fifo_chunk_t *
fs_try_alloc_chunk_slow (fifo_segment_header_t *fsh, fifo_segment_slice_t *fss, u32 fl_index,
			 u32 data_bytes)
{
  u32 batch = FIFO_SEGMENT_ALLOC_BATCH_SIZE, chunk_sz;
  ...
  if (data_bytes > chunk_sz && data_bytes <= fss_fl_chunk_bytes (fss))
    {
  ...
  /* If segment still has enough space, try to allocate batch of max
   * usable chunks. They're not necessarily large enough */
  if (chunk_sz <= n_free)
    {
	...
	goto multi_chunk;
      ...
      if (data_bytes > chunk_sz)
	goto multi_chunk;

      /* Request fits in one chunk, use newly allocated chunk */
      c = fss_chunk_free_list_pop (fsh, fss, fl_index);
      if (!c)
	goto multi_chunk;
...
fsh_try_alloc_chunk (fifo_segment_header_t *fsh, fifo_segment_slice_t *fss, u32 data_bytes)
{
  ...
  return fs_try_alloc_chunk_slow (fsh, fss, fl_index, data_bytes);
}
