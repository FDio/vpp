...
static inline fifo_segment_slice_t *
fsh_slice_get2 (fifo_segment_t * fs, u32 slice_index)
{
...
static inline uword
fsh_virtual_mem (fifo_segment_t * fs)
{
...
fifo_segment_init (fifo_segment_t * fs)
{
  ...
  u32 max_chunk_sz, align = 8;
  uword max_fifo, seg_start, seg_size;
  void *seg_data;
//  void *oldheap;
  int i;
  u32 offset = 2 * 4096, vec_sz;
  ...
  fsh->byte_index = seg_start + sizeof (*fsh);
  fsh->max_byte_index = seg_start + seg_size;
...
//  fsh->ssvm_sh = fs->ssvm.sh;
  ...
  fs->slices = fsh_alloc (fsh, sizeof (*fss) * fs->n_slices);
  fsh->slices = fs->slices;
  memset (fs->slices, 0, sizeof (*fss) * fs->n_slices);
...
//  fsh->n_reserved_bytes = clib_min (0.01 * fsh->n_free_bytes, 256 << 10);
  fsh->n_reserved_bytes = clib_min (0.01 * seg_size, 256 << 10);
  sh->ready = 1;
  return (0);
}
				 ...
				 u32 data_bytes, fifo_segment_ftype_t ftype)
{
  ...
  fss = fsh_slice_get2 (fs, slice_index);
...
fifo_segment_free_fifo (fifo_segment_t * fs, svm_fifo_t * f)
{
  ...
  fss = fsh_slice_get2 (fs, f->slice_index);
