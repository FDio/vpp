...
svm_msg_q_alloc (svm_msg_q_cfg_t *cfg)
{
  ...
  mq->q = svm_queue_init (cfg->q_nitems, sizeof(svm_msg_q_msg_t),
                          cfg->consumer_pid, 0);
  vec_validate (mq->rings, cfg->n_rings);
...
#if SVM_MSG_Q_USE_EVENTFD
  mq->q.event_fd = cfg->event_fd;
#endif
...
svm_msg_q_free (svm_msg_q_t * mq)
{
  ...
  close (mq->q.event_fd);
#else
  (void) pthread_mutex_destroy (&mq->mutex);
  (void) pthread_cond_destroy (&mq->condvar);
  clib_mem_free (mq);
#endif
}
...
int
svm_msg_q_sub (svm_queue_t * mq, svm_msg_q_msg_t* msg,
