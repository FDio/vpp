			...
			u32 protect_size)
{
  ...
  pairs_to_allocate = clib_min((s->ssvm.ssvm_size - protect_size) / pair_size,
...
format_svm_fifo_segment_name (u8 * s, va_list * args)
{
  ...
  else if (sp->ssvm.sh->type == SSVM_SEGMENT_MEMFD)
    s = format (s, "[memfd] %v", sp->ssvm.sh->name);
  else if (sp->ssvm.sh->type == SSVM_SEGMENT_SHM)
    s = format (s, "[svm] %s", sp->ssvm.sh->name);
  else
