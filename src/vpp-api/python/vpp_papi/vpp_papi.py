...
class VPPApiClient(object):
    ...
    def _add_stat(self, name, seconds):
        if not name in self.stats:
            self.stats[name] = {'max': seconds, 'count': 1, 'avg': seconds}
        else:
            if seconds > self.stats[name]['max']:
                self.stats[name]['max'] = seconds
            self.stats[name]['count'] += 1
            n = self.stats[name]['count']
            self.stats[name]['avg'] = self.stats[name]['avg'] * (n - 1) / n + seconds / n

    def get_stats(self):
        s = '\n=== API PAPI STATISTICS ===\n'
        for n in sorted(self.stats.items(), key=lambda v: v[1]['avg'], reverse=True):
            s += '{:<30} {:>4} {:>6.2f}\n'.format(n[0], n[1]['count'], n[1]['avg'])
        return s
