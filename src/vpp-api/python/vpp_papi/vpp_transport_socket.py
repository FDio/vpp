...
class VppTransport:
    def __init__(self, parent, read_timeout):
    ...
    def connect(self, name, pfx, msg_handler, rx_qlen):
        ...
        # Download message dictionary
        api_message_table = self.parent.messages['api_message_table']
        api_message_table_reply = \
            self.parent.messages['api_message_table_reply']

        args = {'_vl_msg_id': 23,
                'context': 124}
        b = api_message_table.pack(args)
        self.write(b)
        msg = self.read()
        i, ci = self.parent.header.unpack(msg, 0)
        if i != 24:
        ...
        r = api_message_table_reply.unpack(msg)
