...
class VPP():
    ...
    def __struct(self, t, n=None, e=-1, vl=None):
        ...
        if t in self.messages:
            ...
            #XXX if len(self.messages[t]['sizes'][1]):
            #    print('Gotta a problem', t, self.messages[t]['sizes'][1])
    ...
    def __struct_type_encode(self, msgdef, buf, offset, kwargs):
        ...
        for k in kwargs:
            ...
            if type(kwargs[k]) is str:
                if len(kwargs[k]) > msgdef['sizes'][1][k]:
                    raise ValueError(1, 'Input argument too long: %s (%s > %s)' %
                        (k, len(kwargs[k]), msgdef['sizes'][1][k]))
