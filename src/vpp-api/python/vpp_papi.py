...
class VPP():
    ...
    def find_api_dir(cls):
        ...
        if srcdir:
            # try likely dirs
            x = 'vpp/share/vpp/api/core'
            dirs.append("%s/build-root/install-vpp_debug-native/%s" %
                        (srcdir, x))
            dirs.append("%s/build-root/install-vpp-native/%s" %
                        (srcdir, x))
    ...
    def find_api_files(cls, api_dir=None, patterns='*'):
        """Find API definition files fro mthe given directory with the
        given pattern. If no directory is given then find_api_dir is used
        to located one. If no pattern is given then all definition files in
        that directory are used.

        :param api_dir: A directory in which to locate API definition files.
            If this is None then find_api_dir is called.
