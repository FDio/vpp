...
stat_segment_connect (char *socket_name)
{
  ...
  sm->counter_vector_by_name = (uword *) sm->shared_header->opaque[STAT_SEGMENT_OPAQUE_DIR];
...
maybe_update_cached_pointers (stat_segment_cached_pointer_t *cached_pointers)
{
  ...
  /* Cached pointers OK? */
  if (sm->current_epoch == (u64) sm->shared_header->opaque[STAT_SEGMENT_OPAQUE_EPOCH])
    return;
  ...
  sm->current_epoch = (u64) sm->shared_header->opaque[STAT_SEGMENT_OPAQUE_EPOCH];
}
...
stat_segment_collect (stat_segment_cached_pointer_t *cached_pointers)
{
  ...
  for (i = 0; i < vec_len(cached_pointers); i++) {
    vec_add1(res, copy_data(cached_pointers[i].ep, sm->error_base, cached_pointers[i].name));
  }
