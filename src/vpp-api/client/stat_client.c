...
copy_data (stat_segment_directory_entry_t * ep, stat_client_main_t * sm)
{
  ...
  switch (ep->type)
	...
	stat_segment_adjust (sm, (void *) sm->shared_header->symlink_vector);
      ...
      simple_c =
	stat_segment_adjust (sm, directory_vector[symlink->indexes[0]].data);
      vec_validate (result.symlink_simple_vec, vec_len (simple_c) - 1);
      for (i = 0; i < vec_len (simple_c); i++)
	{
	  counter_t *cb = stat_segment_adjust (sm, (void *) simple_c[i]);
	  result.error_vector[i] = cb[symlink->indexes[1]];
	}
      break;
	...
	stat_segment_adjust (sm, (void *) sm->shared_header->symlink_vector);
      ...
      combined_c =
	stat_segment_adjust (sm, directory_vector[symlink->indexes[0]].data);
      vec_validate (result.symlink_combined_vec, vec_len (combined_c) - 1);
      for (i = 0; i < vec_len (combined_c); i++)
	{
	  counter_t *cb = stat_segment_adjust (sm, (void *) combined_c[i]);
	  result.error_vector[i] = cb[symlink->indexes[1]];
	}
...
stat_segment_data_free (stat_segment_data_t * res)
{
  ...
  for (i = 0; i < vec_len (res); i++)
	...
	case STAT_DIR_TYPE_ERROR_INDEX:
	  ...
	  break;
	case STAT_DIR_TYPE_SYMLINK_SIMPLE:
	  vec_free (res[i].symlink_simple_vec);
	  break;
	case STAT_DIR_TYPE_SYMLINK_COMBINED:
	  vec_free (res[i].symlink_combined_vec);
	  break;
	case STAT_DIR_TYPE_SCALAR_INDEX:
	  break;
	default:
