...
static vlib_stats_directory_entry_t *
...
copy_data (vlib_stats_directory_entry_t *ep, u32 index2, char *name,
	   stat_client_main_t *sm)
{
  ...
  switch (ep->type)
    ...
    case STAT_DIR_TYPE_SYMLINK:
      ...
      {
	vlib_stats_directory_entry_t *ep2;
...
stat_segment_ls_r (uint8_t ** patterns, stat_client_main_t * sm)
{
  ...
  vlib_stats_directory_entry_t *counter_vec = get_stat_vector_r (sm);
...
stat_segment_index_to_name_r (uint32_t index, stat_client_main_t * sm)
{
  vlib_stats_directory_entry_t *ep;
  stat_segment_access_t sa;
  vlib_stats_directory_entry_t *vec;
