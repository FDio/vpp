...
recv_fd (int sock)
{
  ...
  union { char buf[CMSG_SPACE (sizeof (fd))]; struct cmsghdr align; } u;
  msg.msg_iov = &io;
  msg.msg_iovlen = 1;
...
copy_data (stat_segment_directory_entry_t * ep)
{
  ...
  counter_t *error_base, *stats_base;
  ...
  switch (ep->type)
    ...
    case STAT_DIR_TYPE_SCALAR_INDEX:
      stats_base = stat_segment_pointer (sm->shared_header, sm->shared_header->stats_offset);
      result.scalar_value = stats_base[ep->index];
      break;
