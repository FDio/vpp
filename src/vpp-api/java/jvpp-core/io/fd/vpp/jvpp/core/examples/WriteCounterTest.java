...
public class WriteCounterTest {

    private static final Logger LOG = Logger.getLogger(WriteCounterTest.class.getName());
    ...
    private static void testInvokeCounter(boolean setCount, int count) throws Exception {
        ...
        try (final JVppRegistry registry = new JVppRegistryImpl("WriteCounterTest");
            ...
            if (!setCount) {
                ...
                int x = 0;
                do {
                    jvpp.send(createClassifyTable(x++));
    ...
    private static ClassifyAddDelTable createClassifyTable(int tableIndex) {
