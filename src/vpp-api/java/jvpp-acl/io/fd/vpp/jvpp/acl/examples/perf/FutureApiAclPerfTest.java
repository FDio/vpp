...
public class FutureApiAclPerfTest {
        ...
        try (final JVppRegistry registry = new JVppRegistryImpl("FutureApiAclPerfTest");
            ...
            for (int i = 0; i < trials; i++) {
                ...
                for (int x = 0; x < aclCount; x++) {
                    CompletableFuture<AclAddReplaceReply> reply = jvpp.aclAddReplace(request).toCompletableFuture();
    ...
    private static class StopWatch {
         ...
         * Not thread safe but this is not an issue, because reading part process only one message at a time.
         */
        void updateCounter() {
            currentCount++;
