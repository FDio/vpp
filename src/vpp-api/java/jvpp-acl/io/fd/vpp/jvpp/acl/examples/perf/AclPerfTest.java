...
interface AclPerfTest {
    ...
    static AclInterfaceSetAclList createAclList(final int count) {
        AclInterfaceSetAclList request = new AclInterfaceSetAclList();
        request.swIfIndex = 0;
        request.count = (byte) count;
        request.nInput = (byte) count;
        request.acls = new int[count];

        // Assume we know ACL IDs.
        // TODO: HC maps ACL names to IDs, so real test needs to process replies
    ...
    static AclInterfaceSetAclList createSingletonAclList(final int aclId) {
        AclInterfaceSetAclList request = new AclInterfaceSetAclList();
        request.swIfIndex = 0;
        request.count = 1;
        request.nInput = 1;
        request.acls = new int[] {aclId};
        return request;
    }
}
