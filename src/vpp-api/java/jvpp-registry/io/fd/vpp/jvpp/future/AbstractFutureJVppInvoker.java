      ...
      try {
          ...
          synchronized(requests) {
              ...
              if (replyFuture == null) {
                  ...
                  replyCompletableFuture = (CompletableDumpFuture<DUMP>) replyFuture;
              }
          }
          ...
          synchronized(requests) {
              CompletableFuture<? extends JVppReply<?>> replyFuture = requests.get(pingId);
              if (replyFuture == null) {
                  // reply not received yet, put future into map under pingId
                  requests.put(pingId, replyCompletableFuture);
              } else {
                  // reply already received, complete future and remove both futures from map
                  // ping reply couldn't complete the future because it is not in map under
                  // ping id
                  replyCompletableFuture.complete(replyCompletableFuture.getReplyDump());
                  requests.remove(contextId);
                  requests.remove(pingId);
              }
          }
