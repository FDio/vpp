...
def _generate_class_cache(plugin_name, messages):
    return _CLASS_CACHE_TEMPLATE.substitute(
        class_references=_generate_class_references(messages),
        create_references=_generate_create_references(plugin_name, messages),
        delete_references=_generate_delete_references(messages)
    )


def _generate_class_references(messages):
    return "\n".join("jclass %sClass;" % msg.java_name_lower for msg in messages)
...
def _generate_create_references(plugin_name, messages):
    references = []
    # todo
    # class_name = 'io/fd/vpp/jvpp/VppCallbackException'
    for msg in messages:
        references.append(_CREATE_GLOBAL_REF_TEMPLATE.substitute(
            ref_name=msg.java_name_lower,
            fqn_name="io/fd/vpp/jvpp/%s/dto/%s" % (plugin_name, msg.java_name_upper)
        ))
    return "".join(references)


def _generate_delete_references(messages):
    return "// todo _generate_delete_references"
