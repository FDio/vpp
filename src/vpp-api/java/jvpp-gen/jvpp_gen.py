...
class JvppGenerator(object):
    def __init__(self, jsonparser, logger):
        ...
        self.java_ifc_generator = JavaIfcGenerator(jsonparser, logger)
        self.java_impl_generator = JavaImplGenerator(jsonparser, logger)
        self.callback_generator = CallbackGenerator(jsonparser, logger)
...
3) java ifc (done), java impl (done) and callbacks (in progress)
