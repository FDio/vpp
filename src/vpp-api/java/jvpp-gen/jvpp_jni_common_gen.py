...
_REQUEST_FIELD_IDENTIFIER_TEMPLATE = Template("""
    ...
    ${jni_type} ${java_name} = (*env)->Get${jni_accessor}(env, ${object_ref_name}, ${java_name}FieldId);
""")
...
_ARRAY_J2C_SWAP_TEMPLATE = Template("""
    if (${field_reference_name}) {
        ...
        ${field_length_check}
        for (_i = 0; _i < cnt; _i++) {
            ${struct_reference_name}->${c_name}[_i] = ${host_to_net_function}(${field_reference_name}ArrayElements[_i]);
        }
    ...
    for field in element.fields:
        ...
        if is_array(field):
            ...
            initialization.append(template.substitute(
                ...
                c_name=field.name,
                host_to_net_function=field_type.host_to_net_function # fixme: define function that generates string which is type specific
            ))
        else:
