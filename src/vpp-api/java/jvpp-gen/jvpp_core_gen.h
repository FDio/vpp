...
static inline void _net_to_host_custom_type(JNIEnv * env, vl_api_custom_type_t * _net, jobject _host)
{
    ...
    jfieldID array2FieldId = (*env)->GetFieldID(env, customTypeClass, "array2", "[S");
    {
        jshortArray array2 = (*env)->NewShortArray(env, array2_size);
        jshort * array2ArrayElements = (*env)->GetShortArrayElements(env, array2, NULL);
        unsigned int _i;
        for (_i = 0; _i < array2_size; _i++) {
...
static void vl_api_request_with_custom_type_array_reply_t_handler (vl_api_request_with_custom_type_array_reply_t * mp)
{
    ...
    {
        jobjectArray val = (*env)->NewObjectArray(env, 16);
        jobject * valArrayElements = (*env)->GetObjectArrayElements(env, val, NULL);
        unsigned int _i;
        for (_i = 0; _i < 16; _i++) {
            valArrayElements[_i] = _net_to_host_custom_type(mp->val[_i]);
        }

        (*env)->ReleaseObjectArrayElements(env,  val, valArrayElements, 0);
        (*env)->SetObjectField(env, dto, valFieldId, val);
        (*env)->DeleteLocalRef(env, val);
    }
...
static void vl_api_request_with_custom_type_vla_reply_t_handler (vl_api_request_with_custom_type_vla_reply_t * mp)
{
    ...
    {
        jobjectArray val = (*env)->NewObjectArray(env, vla_size);
        jobject * valArrayElements = (*env)->GetObjectArrayElements(env, val, NULL);
        unsigned int _i;
        for (_i = 0; _i < vla_size; _i++) {
            valArrayElements[_i] = _net_to_host_custom_type(mp->val[_i]);
        }
