 ...
 *     {   u'crc': u'0x98cbb7f1'}]
 */
static inline void _net_to_host_custom_type(vl_api_custom_type_t *a)
{
    jclass customTypeClass = (*env)->FindClass(env, "io/fd/vpp/jvpp/core/types/CustomType");
    memset (&(mp->custom_type), 0, sizeof (mp->custom_type)); // will not work if type definition uses VLA syntax
    // (either directly or by nesting)

    jfieldID valFieldId = (*env)->GetFieldID(env, requestClass, "val", "I");
    jint val = (*env)->GetIntField(env, request, valFieldId);

    jfieldID array1FieldId = (*env)->GetFieldID(env, requestClass, "array1", "[B");
    jbyteArray array1 = (*env)->GetObjectField(env, request, array1FieldId);

    jfieldID array2SizeFieldId = (*env)->GetFieldID(env, requestClass, "array2Size", "B");
    jbyte array2Size = (*env)->GetByteField(env, request, array2SizeFieldId);

    jfieldID array2FieldId = (*env)->GetFieldID(env, requestClass, "array2", "[S");
    jshortArray array2 = (*env)->GetObjectField(env, request, array2FieldId);

    mp->val = clib_host_to_net_u32(val);
    ...
    if (array1) {
        ...
        (*env)->GetByteArrayRegion(env, array1, 0, cnt, (jbyte *)mp->array1);
    }

    mp->array2_size = array2Size;
    ...
    if (array2) {
        ...
        if (cnt > max_size) cnt = max_size;
        for (_i = 0; _i < cnt; _i++) {
            mp->array2[_i] = clib_host_to_net_u16(array2ArrayElements[_i]);
        }
...
(JNIEnv * env, jclass clazz, jobject request) {
    ...
    mp->val = _net_to_host_custom_type(val);
...
(JNIEnv * env, jclass clazz, jobject request) {
    ...
    if (val) {
        jCustomType * valArrayElements = (*env)->GetCustomtypeArrayElements(env, val, NULL);
        size_t _i;
        ...
        if (cnt > max_size) cnt = max_size;
        for (_i = 0; _i < cnt; _i++) {
            mp->val[_i] = _net_to_host_custom_type(valArrayElements[_i]);
        }
        (*env)->ReleaseCustomtypeArrayElements (env, val, valArrayElements, 0);
    }
...
static void vl_api_request_with_custom_type_reply_t_handler (vl_api_request_with_custom_type_reply_t * mp)
{
    ...
    (*env)->SetObjectField(env, dto, valFieldId, _host_to_net_custom_type(mp->val));
...
static void vl_api_request_with_custom_type_array_reply_t_handler (vl_api_request_with_custom_type_array_reply_t * mp)
{
    ...
    {
        jobjectArray val = (*env)->NewCustomtypeArray(env, 16);
        jCustomType * valArrayElements = (*env)->GetCustomtypeArrayElements(env, val, NULL);
        unsigned int _i;
        for (_i = 0; _i < 16; _i++) {
            valArrayElements[_i] = _host_to_net_custom_type(mp->val[_i]);
        }
...
static void vl_api_request_with_custom_type_vla_reply_t_handler (vl_api_request_with_custom_type_vla_reply_t * mp)
{
    ...
    {
        jobjectArray val = (*env)->NewCustomtypeArray(env, vla_size);
        jCustomType * valArrayElements = (*env)->GetCustomtypeArrayElements(env, val, NULL);
        unsigned int _i;
