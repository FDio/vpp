...
def is_request(msg):
    return hasattr(msg, '_reply')


def is_reply(msg):
    return hasattr(msg, '_request')


def is_dump(msg):
    return hasattr(msg, '_details')


def is_details(msg):
    return hasattr(msg, '_dump')
...
class Request(Message):
    def __init__(self, name, reply, crc, fields, definition):
        ...
        self._reply = underscore_to_camelcase_upper(reply)

    def __str__(self):
        return "Request{name:%s, reply:%s, crc:%s, fields:%s}" % (self.name, self._reply, self.crc, self.fields)


class Reply(Message):
    def __init__(self, name, request, crc, fields, definition):
        super(Reply, self).__init__(name, crc, fields, definition)
        self._request = underscore_to_camelcase_upper(request)
    ...
    def __str__(self):
        return "Reply{name:%s, request:%s, crc:%s, fields:%s}" % (self.name, self._request, self.crc, self.fields)


class Dump(Message):
    def __init__(self, name, details, crc, fields, definition):
        super(Dump, self).__init__(name, crc, fields, definition)
        self._details = underscore_to_camelcase_upper(details)

    def __str__(self):
        return "Dump{name:%s, details:%s, crc:%s, fields:%s}" % (self.name, self._details, self.crc, self.fields)
...
class Details(Message):
    def __init__(self, name, dump, crc, fields, definition):
        ...
        self._dump = underscore_to_camelcase_upper(dump)
    ...
    def __str__(self):
        return "Details{name:%s, dump:%s, crc:%s, fields:%s}" % (self.name, self._dump, self.crc, self.fields)
...
class JsonParser(object):
    ...
    def _parse_message(self, name, definition):
        self.logger.debug("Parsing message: %s", name)
        crc = None
        fields = []
        for item in definition:
            if _is_crc(item):
                ...
                # todo perhaps return None and skip this message?
        ...
        if self._is_service(name):
            ...
            self.logger.warn("Could not parse message %s (%s)", name, definition)
