...
class Array(Type):
    def __init__(self, name, java_name, jni_signature, base_type, is_swap_needed=True):
        super(Array, self).__init__(name, is_swap_needed)
        self.java_name = java_name
        self.jni_signature = jni_signature
        self.jni_type = "j%sArray" % base_type
        self.jni_accessor = "ObjectField"
        self.jni_base_type = "j%s" % base_type
        self.base_type = base_type.title()
...
class Field(object):
    def __init__(self, name, field_type, array_len=None, array_len_field_name=None):
        self.name = name
        self.java_name = underscore_to_camelcase_lower(name)
        self.type = field_type
        self.array_len = array_len
        self.array_len_field_name = array_len_field_name
...
class JsonParser(object):
    ...
    def _parse_types(self):
        ...
        self._types_by_name = OrderedDict({
            ...
            'u8[]': Array('u8[]', 'byte[]', '[B', 'byte', is_swap_needed=False),
            'u16[]': Array('u16[]', 'short[]', '[S', 'jshortArray', 'Short'),
            'u32[]': Array('u32[]', 'int[]', '[I', 'jintArray', 'Int'),
            'u64[]': Array('u64[]', 'long[]', '[J', 'jlongArray', 'Long')
    ...
    def _parse_messages(self):
        ...
        for json_msg in self._json_messages:
            try:
                ...
                msg = self._parse_message(name, definition)
                self._messages_by_name[msg.name] = msg
    ...
    def _parse_message(self, name, definition):
        ...
        for item in definition:
            if _is_crc(item):
                ...
                array_len_field_name = None
                if len(item) == 4:
                    array_len_field_name = item[3]
                fields.append(Field(item[1], self._types_by_name[item[0]+'[]'], item[2], array_len_field_name)) #todo check array of length 0 special case
            else:
