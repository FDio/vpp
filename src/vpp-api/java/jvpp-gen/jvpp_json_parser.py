...
class Service(Message):
    def __init__(self, name, crc, fields, reply):
        super(Service, self).__init__(name, crc, fields)
        self.reply = reply


class Request(Service):
    def __init__(self, name, crc, fields, reply):
        super(Request, self).__init__(name, crc, fields, reply)

    def __str__(self):
        return "Request{name:%s, crc:%s, fields:%s, reply:%s}" % (self.name, self.crc, self.fields, self.reply)
...
class Reply(Message):
    ...
    def __str__(self):
        return "Reply{name:%s, crc:%s, fields:%s}" % (self.name, self.crc, self.fields)


class Dump(Service):
    def __init__(self, name, crc, fields, reply):
        super(Dump, self).__init__(name, crc, fields, reply)

    def __str__(self):
        return "Dump{name:%s, crc:%s, fields:%s, reply:%s}" % (self.name, self.crc, self.fields, self.reply)
...
class Details(Message):
    ...
    def __str__(self):
        return "Details{name:%s, crc:%s, fields:%s}" % (self.name, self.crc, self.fields)
...
class JsonParser(object):
    ...
    def _parse_message(self, name, definition):
        ...
        if self.is_service(name):
            ...
            if _is_stream(service):
                return Dump(name, crc, [], service['reply'])
            return Request(name, crc, [], service['reply'])
        elif self.is_reply(name):
            return Reply(name, crc, [])
        elif self.is_details(name):
            return Details(name, crc, [])
