...
class JsonParser(object):
    ...
    def _parse_message(self, name, definition):
        ...
        for item in definition:
            if _is_crc(item):
                crc = item['crc']
            elif item[0] in self.types and not _is_array(item):
                fields.append(Field(item[1], self.types[item[0]]))
            elif item[0] in self.types and _is_array(item):
                array_len_field_name = None
                if len(item) == 4:
                    array_len_field_name = item[3]
                fields.append(Field(item[1], self.types[item[0]+'[]'], item[2], array_len_field_name)) #todo check array of length 0 special case
            else:
