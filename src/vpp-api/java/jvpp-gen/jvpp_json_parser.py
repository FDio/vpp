...
# fixme use optional parameters (properties?) instead of subclasses,
...
class JsonParser(object):
    def __init__(self, logger, json_api_files, plugin_name):
        ...
        self.plugin_name = underscore_to_camelcase_upper(plugin_name)
        logger.info("Hello") # todo
    ...
    def _parse_message(self, name, definition):
        ...
        if self.is_service(name):
            ...
            return Request(name, service['reply'], crc, fields)
        elif self.is_reply(name):
            return Reply(name, self._requests_by_reply[name], crc, fields)
