...
class SimpleType(Type):
    def __init__(self, name, java_name, jni_signature, jni_type, jni_accessor, is_swap_needed=True,
        ...
        if is_swap_needed and (not host_to_net_function or not net_to_host_function):
            raise TypeError(
                "Initialization of SimpleType(%s) failed: missing host_to_net_function and net_to_host_function", name)
        self.host_to_net_function = host_to_net_function  # fixme introduce macro and remove that one
...
# todo do we need that type? rething refactor
class Array(Type):
    def __init__(self, base_type):
        ...
        self.java_name_fqn = base_type.java_name_fqn + _ARRAY_SUFFIX
        self.jni_signature = "[%s" % base_type.jni_signature
        self.jni_type = "%sArray" % base_type.jni_type
        self.jni_accessor = "Object"
        self.jni_base_type = base_type.jni_type
        ...
        self.base_type_vpp_name = base_type.vpp_name
        self.jni_base_accessor = base_type.jni_accessor

    def get_host_to_net_function(self, host_ref_name, net_ref_name):
        return self._base_type.get_host_to_net_function(host_ref_name, net_ref_name)
...
class Class(Type):
    def __init__(self, name, crc, fields, definition, plugin_name):
        ...
        self.java_name_fqn = "io.fd.vpp.jvpp.%s.types.%s" % (plugin_name, self.java_name)
        self.crc = crc
        self.fields = fields
        self.doc = _message_to_javadoc(definition)  # todo rename _message_to_javadoc
        self.jni_name = "io/fd/vpp/jvpp/%s/types/%s" % (plugin_name, self.java_name)
        self.jni_signature = "Lio/fd/vpp/jvpp/%s/types/%s;" % (plugin_name, self.java_name)
        self.jni_type = "jobject"
        self.host_to_net_function = "_host_to_net_%s" % name
        self.net_to_host_function = "_net_to_host_%s" % name
        self.jni_accessor = "Object"
...
# todo: reuse K's parser? (what about services?),
# todo: run K parser once?
...
def is_event(msg):
    return isinstance(msg, Event) # todo: can we do better than isinstance
...
class Dump(Message):
    def __init__(self, name, details, crc, fields, definition):
        ...
        self.reply_java = underscore_to_camelcase_upper(details)
...
# todo rename to json API or JsonModel or ApiModel?
class JsonParser(object):
    ...
    def _parse_types(self):
        ...
        self._types_by_name = OrderedDict({
            'u8': SimpleType('u8', 'byte', 'B', 'jbyte', 'Byte', is_swap_needed=False),
            'i8': SimpleType('i8', 'byte', 'B', 'jbyte', 'Byte', is_swap_needed=False),
            ...
            'f64': SimpleType('f64', 'double', 'D', 'jdouble', 'Double', is_swap_needed=False)
        })
    ...
    def _parse_field(self, field, fields):
        ...
        if type_name in self._types_by_name:
            if _is_array(field):
                ...
                # todo check array of length 0 special case
            else:
