...
_SIMPLE_TYPE_NO_SWAP_FIELD_SETTER_TEMPLATE = Template("""
    jfieldID ${java_name}FieldId = (*env)->GetFieldID(env, ${class_ref_name}Class, "${java_name}", "${jni_signature}");
    (*env)->Set${jni_accessor}(env, dto, ${java_name}FieldId, mp->${c_name});
""")



__ARRAY_C2J_NO_SWAP_TEMPLATE = Template("""
    ${jni_type} ${field_reference_name} = (*env)->New${base_type_name}Array(env, ${field_length});
    (*env)->Set${base_type_name}ArrayRegion(env, ${field_reference_name}, 0, ${field_length}, (const jbyte*)mp->${c_name});
...
__ARRAY_C2J_SWAP_TEMPLATE = Template("""
    {
