...
{
    jclass ${class_ref_name} = (*env)->FindClass(env, "${class_FQN}");
$jni_identifiers
$type_initialization
}""")
...
def generate_type_handlers(model):
    ...
    for t in model.types:
        if isinstance(t, Class):
            c_name = t.name
            class_ref_name = "_%s_class" % c_name
            jni_identifiers = generate_j2c_identifiers(t, class_ref_name=class_ref_name, object_ref_name="_host")
