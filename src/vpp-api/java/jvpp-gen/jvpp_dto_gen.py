...
from jvpp_json_parser import is_request, is_reply, is_retval, is_dump, is_details, is_event
...
def generate_dto(work_dir, model, logger):
    ...
    _generate_dto(work_dir, model, logger)
    _generate_dump_reply(work_dir, model, logger)


def _generate_dto(work_dir, model, logger):
...
def _generate_dump_reply(work_dir, model, logger):
    for msg in model.messages:
        if is_details(msg):
            ...
            with open("%s/%sDumpReply.java" % (work_dir, details_class), "w") as f:
                f.write(dto)
