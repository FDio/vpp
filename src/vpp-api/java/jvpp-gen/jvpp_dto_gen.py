...
def _generate_request_dto(msg, model):
    ...
    return _REQUEST_TEMPLATE.substitute(
        ...
        to_string=_generate_to_string(msg_java_name, fields),
...
def _api_message_to_javadoc(api_message):
    ...
    formatted_message = pprint.pformat(api_message, indent=4, width=120, depth=None)
...
def _generate_equals(msg_java_name, fields):
    comparisons = "\n".join(map(lambda f: _EQUALS_FIELD_TEMPLATE.substitute(field_name=f.java_name), fields))
    if comparisons != "":
...
def _generate_to_string(msg_java_name, fields):
    return _TO_STRING_TEMPLATE.substitute(
        ...
        to_string_fields="\n".join(map(lambda f: _TO_STRING_FIELD_TEMPLATE.substitute(field_name=f.java_name), fields))
    )
...
def _generate_reply_dto(msg, model):
    msg_java_name = msg.java_name
    fields = filter(_is_reply_field, msg.fields)
...
class DtoGenerator(object):
    ...
    def generate(self):
        ...
        for msg in model.messages:
            # todo: before push, change to getters or properties
