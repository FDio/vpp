    ...
    for field in msg.fields:
        ...
        if is_array(field):
            ...
            if field_type.is_swap_needed:
                initialization.append('    printf(\"%%p\", %s); // FIXME SWAP' % field.java_name)
            else:
                initialization.append(_ARRAY_J2C_NO_SWAP_TEMPLATE.substitute(
                    field_reference_name=field_type.jni_type,
...
def _generate_field_length_check(field):
    ...
    if field.array_len_field_name:
        field_length = field.array_len_field_name
