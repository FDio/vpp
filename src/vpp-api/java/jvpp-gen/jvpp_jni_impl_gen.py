...
from jvpp_jni_common_gen import generate_j2c_identifiers, generate_j2c_swap
...
def _generate_msg_size(msg):
    ...
    for field in msg.fields:
        # fixme: ignore ZLAs for simplicity (to support them we need to call jni functions to check actual size,
        # which is NOT done in VPP APIGEN 1)
        if field.array_len_field:
            _size_components += " + %s*sizeof(%s)" % (field.array_len_field.java_name, field.type.base_type_vpp_name)
            # fixme: to support nested structures we need to call a function that computes size of type instead of sizeof

    # todo 1) add support for custom types (produce create funciton for each message)
    # todo 2) add support for type nesting (VPP-586)
