...
namespace VOM
{
    ...
    log_t::log_t() : m_level(log_level_t::ERROR), m_o_stream(&std::cout)
    {
    }

    void log_t::set(const log_level_t &level)
    {
        m_level = level;
    }

    void log_t::set(const std::string &ofile)
    {
        m_file_stream.open(ofile);
        m_o_stream = &m_file_stream;
    }

    std::ostream &log_t::stream(const char *file, int line)
    {
        ...
        *m_o_stream << std::endl
                    << std::put_time(tm, "%c") << " [" << m_level.to_string()
                    << "]"
                    << " " << dirs.back() << ":" << line << ": ";
     ...
     */
    const log_level_t &log_t::level() const
    {
        return (m_level);
    }
}
