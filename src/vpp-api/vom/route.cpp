    ...
    namespace route
    {
        ...
        void path::to_vpp(vapi_payload_ip_add_del_route &payload) const
        {
            ...
            if (special_t::STANDARD == m_type)
            {
                ...
                if (m_interface)
                {
                    payload.next_hop_sw_if_index =
                        m_interface->handle().value();
                }
            }
        ...
        void ip_route::replay()
        {
            ...
            {
                HW::enqueue(
                    new update_cmd(m_hw, m_rd->table_id(), m_prefix, m_paths));
            }
        }
        ...
        std::shared_ptr<ip_route> ip_route::find_or_add(const ip_route &temp)
        {
            return (m_db.find_or_add(
                std::make_pair(temp.m_rd->table_id(), temp.m_prefix), temp));
        }
        ...
        ip_route::event_handler::event_handler()
        {
            ...
            inspect::register_handler({"ip-route"}, "ip route configurations",
                                      this);
        }
        ...
        ip_route::event_handler::handle_populate(const client_db::key_t &key)
        {
            ...
            std::shared_ptr<ip_route::dump_v4_cmd> cmd_v4(
                new ip_route::dump_v4_cmd());
            std::shared_ptr<ip_route::dump_v6_cmd> cmd_v6(
                new ip_route::dump_v6_cmd());
            ...
            for (auto &record : *cmd_v4)
            {
                ...
                VOM_LOG(log_level_t::DEBUG) << "ip-route-dump: "
                                            << ip_r.to_string();
        ...
        std::ostream &operator<<(std::ostream &os, const path_list_t &key)
        {
            os << "[";
            for (auto k : key)
            {
