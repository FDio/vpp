...
path::path(const boost::asio::ip::address& nh, const interface& interface,
           uint8_t weight, uint8_t preference)
...
path::path(const route_domain& rd, const boost::asio::ip::address& nh,
           uint8_t weight, uint8_t preference)
  : m_type(special_t::STANDARD)
  , m_nh_proto(nh_proto_t::from_address(nh))
  , m_nh(nh)
...
path::path(const interface& interface, const nh_proto_t& proto, uint8_t weight,
           uint8_t preference)
  : m_type(special_t::STANDARD)
  , m_nh_proto(proto)
  , m_nh()
