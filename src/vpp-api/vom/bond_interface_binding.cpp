 ...
 */
singular_db<bond_interface_binding::key_t, bond_interface_binding>
...
const bond_interface_binding::key_t
bond_interface_binding::key() const
{
...
bond_interface_binding::sweep()
{
  ...
  while (it != m_mem_itfs.cend()) {
    if (m_binding) {
      HW::enqueue(
        new bond_interface_binding_cmds::unbind_cmd(m_binding, it->handle()));
    }
    HW::write();
    ++it;
  }
...
bond_interface_binding::replay()
{
  ...
  while (it != m_mem_itfs.cend()) {
    if (m_binding) {
      HW::enqueue(new bond_interface_binding_cmds::bind_cmd(
        m_binding, m_itf->handle(), *it));
    }
    HW::write();
    ++it;
  }
...
bond_interface_binding::event_handler::order() const
{
   ...
   * but before anything else, so TUNNEL.
   */
  return (dependency_t::BOND_INTERFACE);
}
