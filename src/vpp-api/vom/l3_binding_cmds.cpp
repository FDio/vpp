...
namespace VOM
{
    ...
    rc_t l3_binding::bind_cmd::issue(connection &con)
    {
        ...
        m_pfx.to_vpp(&payload.is_ipv6, payload.address,
    ...
    std::string l3_binding::bind_cmd::to_string() const
    {
        ...
        s << "L3-bind: " << m_hw_item.to_string()
          << " itf:" << m_itf.to_string() << " pfx:" << m_pfx.to_string();
    ...
    l3_binding::dump_v4_cmd::dump_v4_cmd(const handle_t &hdl) : m_itf(hdl)
    {
    }

    l3_binding::dump_v4_cmd::dump_v4_cmd(const dump_v4_cmd &d) : m_itf(d.m_itf)
    {
    }
