...
namespace VOM
{
    ...
    void nat_static::sweep()
    {
        if (m_hw)
        {
            if (m_inside.is_v4())
            {
                HW::enqueue(new delete_44_cmd(m_hw, m_rd->table_id(),
                                              m_inside.to_v4(), m_outside));
            }
        }
        HW::write();
    }
    ...
    void nat_static::replay()
    {
        if (m_hw)
        {
            if (m_inside.is_v4())
            {
                HW::enqueue(new create_44_cmd(m_hw, m_rd->table_id(),
                                              m_inside.to_v4(), m_outside));
            }
        }
    }
    ...
    std::string nat_static::to_string() const
    {
        ...
        s << "nat-static:["
          ...
          << " inside: " << m_inside.to_string() << " outside "
          << m_outside.to_string() << "]";
    ...
    std::shared_ptr<nat_static> nat_static::find_or_add(const nat_static &temp)
    {
        return (m_db.find_or_add(
            std::make_pair(temp.m_rd->key(), temp.m_outside), temp));
    }
