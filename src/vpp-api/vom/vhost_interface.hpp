...
#ifndef __VOM_VHOST_INTERFACE_H__
#define __VOM_VHOST_INTERFACE_H__

#include "vom/interface.hpp"

namespace VOM {
/**
 * A vhost-interface. e.g. a vhost interface
 */
class vhost_interface : public interface
{
public:
  vhost_interface(const std::string& sock,
                  admin_state_t state,
                  const std::string &tag = "");

  ~vhost_interface();
  vhost_interface(const vhost_interface& o);

  /**
   * Return the matching 'singular instance' of the VHOST interface
   */
  std::shared_ptr<vhost_interface> singular() const;
...
private:
  ...
  /**
   * optional tag for the interface in VPP
   */
