    ...
    namespace ACL
    {
        ...
        l2_list::event_handler::handle_populate(const client_db::key_t &key)
        {
            ...
            for (auto &record : *cmd)
            {
                ...
                l2_list acl(hdl, std::string(reinterpret_cast<const char *>(
                                     payload.tag)));
                ...
                for (unsigned int ii = 0; ii < payload.count; ii++)
                {
                    ...
                    l2_rule rule(
                        ii, action_t::from_int(payload.r[ii].is_permit), pfx,
        ...
        l3_list::event_handler::handle_populate(const client_db::key_t &key)
        {
            ...
            for (auto &record : *cmd)
            {
                ...
                const handle_t hdl(payload.acl_index);
                l3_list acl(hdl, std::string(reinterpret_cast<const char *>(
                                     payload.tag)));
