...
* Constructor
*/
    bind_cmd(HW::item<bool>& item, const direction_t& direction,
             const handle_t& itf, const handle_t& acl)
...
* Constructor
*/
    unbind_cmd(HW::item<bool>& item, const direction_t& direction,
               const handle_t& itf, const handle_t& acl)
      : rpc_cmd<HW::item<bool>, rc_t, BIND>(item)
      , m_direction(direction)
      , m_itf(itf)
...
*/
typedef binding<l3_list, vapi::Acl_interface_add_del,
...
*/
typedef binding<l2_list, vapi::Macip_acl_interface_add_del,
...
std::ostream& operator<<(
  std::ostream& os, const std::pair<direction_t, interface::key_type>& key);
};
