    ...
    namespace ACL
    {
        ...
        class binding : public object_base
        {
          public:
            ...
            {
                std::ostringstream s;
                s << "acl-binding:[" << m_direction.to_string() << " "
                  << m_itf->to_string() << " " << m_acl->to_string() << " "
                  << m_binding.to_string() << "]";
          ...
          private:
                                  ...
                                  public inspect::command_handler
            {
              public:
                event_handler()
                {
                    ...
                    inspect::register_handler({"acl-binding"}, "ACL bindings",
                                              this);
                }
            ...
            void update(const binding &obj)
            {
                if (!m_binding)
                {
                    HW::enqueue(new bind_cmd(m_binding, m_direction,
                                             m_itf->handle(), m_acl->handle()));
                }
                HW::write();
            }
            ...
            void sweep(void)
            {
                if (m_binding)
                {
                    HW::enqueue(new unbind_cmd(m_binding, m_direction,
                                               m_itf->handle(),
                                               m_acl->handle()));
                }
                HW::write();
