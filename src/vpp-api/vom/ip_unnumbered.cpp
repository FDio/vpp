...
namespace VOM
{
    ...
    void ip_unnumbered::sweep()
    {
        if (m_config)
        {
            HW::enqueue(new unconfig_cmd(m_config, m_itf->handle(),
                                         m_l3_itf->handle()));
        }
        HW::write();
    }
    ...
    void ip_unnumbered::replay()
    {
        ...
        {
            HW::enqueue(
                new config_cmd(m_config, m_itf->handle(), m_l3_itf->handle()));
        }
    }
    ...
    ip_unnumbered::event_handler::event_handler()
    {
        ...
        inspect::register_handler({"ip-un"}, "IP unnumbered configurations",
                                  this);
    }
