...
ip_unnumbered::sweep()
{
  if (m_config) {
    HW::enqueue(
        new ip_unnumbered_cmds::unconfig_cmd(m_config, m_itf->handle(), m_l3_itf->handle()));
  }
  HW::write();
}
...
{
  if (m_config) {
    HW::enqueue(new ip_unnumbered_cmds::config_cmd(m_config, m_itf->handle(), m_l3_itf->handle()));
  }
}
