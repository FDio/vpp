...
namespace VOM
{
    ...
    neighbour::neighbour(const interface &itf,
      ...
      : m_hw(false), m_itf(itf.singular()), m_mac(mac), m_ip_addr(ip_addr)
    {
    }
    ...
    void neighbour::sweep()
    {
        ...
        {
            HW::enqueue(
                new delete_cmd(m_hw, m_itf->handle(), m_mac, m_ip_addr));
        }
        HW::write();
    }
    ...
    void neighbour::replay()
    {
        ...
        {
            HW::enqueue(
                new create_cmd(m_hw, m_itf->handle(), m_mac, m_ip_addr));
        }
    }
    ...
    std::shared_ptr<neighbour> neighbour::find_or_add(const neighbour &temp)
    {
        return (m_db.find_or_add(
            std::make_tuple(temp.m_itf->key(), temp.m_mac, temp.m_ip_addr),
            temp));
    }
                               ...
                               const l3_proto_t &proto)
    {
        ...
        for (auto &record : *cmd)
        {
            ...
            BOOST_LOG_SEV(logger(), levels::debug)
