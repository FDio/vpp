...
interface::event_handler::handle_populate(const client_db::key_t& key)
{
  ...
  for (auto& bond_itf_record : *bcmd) {
    ...
    std::shared_ptr<bond_interface_binding_cmds::dump_cmd> scmd =
      std::make_shared<bond_interface_binding_cmds::dump_cmd>(
        bond_interface_binding_cmds::dump_cmd(bond_itf->handle()));

    HW::enqueue(scmd);
    HW::write();

    bond_interface_binding::enslaved_itf_t enslaved_itfs;

    for (auto& slave_itf_record : *scmd) {
      std::shared_ptr<bond_member_interface> slave_itf =
    ...
    if (!enslaved_itfs.empty()) {
      bond_interface_binding bid(*bond_itf, enslaved_itfs);
      /*
       * Write each of the discovered interfaces into the OM,
       * but disable the HW Command q whilst we do, so that no
       * commands are sent to VPP
       */
      OM::commit(bid.key(), bid);
    }
  }
