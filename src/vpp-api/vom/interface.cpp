...
namespace VOM
{
    ...
    interface::event_listener::event_listener() : m_status(rc_t::NOOP)
    {
    }
    ...
    interface::stat_listener::stat_listener() : m_status(rc_t::NOOP)
    {
    }
    ...
    void interface::add(const std::string &name, const HW::item<handle_t> &item)
    {
    ...
    void interface::event_handler::handle_populate(const client_db::key_t &key)
    {
        ...
        for (auto &itf_record : *cmd)
        {
            ...
            {
                BOOST_LOG_SEV(logger(), levels::debug) << "dump: "
                                                       << itf->to_string();
                /*
