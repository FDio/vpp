    ...
    namespace ACL
    {
        ...
        class list : public object_base
        {
          public:
             ...
             */
            list(const key_t &key) : m_key(key)
            {
            }

            list(const handle_t &hdl, const key_t &key) : m_hdl(hdl), m_key(key)
            {
            }

            list(const key_t &key, const rules_t &rules)
              : m_key(key), m_rules(rules)
            {
                m_evh.order();
            }
             ...
             */
            list(const list &o)
              : m_hdl(o.m_hdl), m_key(o.m_key), m_rules(o.m_rules)
            {
            }
            ...
            {
                std::ostringstream s;
                s << "acl-list:[" << m_key << " " << m_hdl.to_string()
                  << " rules:[";
                ...
                : public rpc_cmd<HW::item<handle_t>, HW::item<handle_t>, UPDATE>
            {
              public:
                ...
                update_cmd(HW::item<handle_t> &item,
                           const key_t &key,
                           const rules_t &rules)
                  : rpc_cmd<HW::item<handle_t>, HW::item<handle_t>, UPDATE>(
                        item)
                  , m_key(key)
                  , m_rules(rules)
                {
                ...
                {
                    int acl_index =
                        reply.get_response().get_payload().acl_index;
                    int retval = reply.get_response().get_payload().retval;

                    VOM_LOG(log_level_t::DEBUG) << this->to_string() << " "
                                                << retval;

                    HW::item<handle_t> res(acl_index,
                                           rc_t::from_vpp_retval(retval));
        ...
        std::map<const handle_t,
                 std::weak_ptr<ACL::list<RULE, UPDATE, DELETE, DUMP>>>
