...
gbp_subnet::event_handler::handle_populate(const client_db::key_t& key)
{
  ...
  /*     from_bytes(payload.subnet.is_ip6, payload.subnet.address); */
  ...
  /*   mac_address_t mac(payload.subnet.mac); */

  /*   VOM_LOG(log_level_t::DEBUG) << "data: " << payload.subnet.sw_if_index; */

  /*   if (itf && epg) { */
  /*       gbp_subnet gbpe(*itf, address, mac, *epg); */
  /*     OM::commit(key, gbpe); */
