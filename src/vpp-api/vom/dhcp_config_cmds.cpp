...
namespace VOM
{
    dhcp_config::bind_cmd::bind_cmd(HW::item<bool> &item,
      ...
      : rpc_cmd(item), m_itf(itf), m_hostname(hostname), m_client_id(client_id)
    {
    }
    ...
    dhcp_config::unbind_cmd::unbind_cmd(HW::item<bool> &item,
                                        const handle_t &itf,
                                        const std::string &hostname)
      : rpc_cmd(item), m_itf(itf), m_hostname(hostname)
    {
    }
