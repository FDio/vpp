...
vxlan_tunnel::mk_name(const boost::asio::ip::address& src,
                      const boost::asio::ip::address& dst, uint32_t vni)
{
  std::ostringstream s;
...
vxlan_tunnel::vxlan_tunnel(const boost::asio::ip::address& src,
                           const boost::asio::ip::address& dst, uint32_t vni)
  : interface(mk_name(src, dst, vni), interface::type_t::VXLAN,
...
vxlan_tunnel::vxlan_tunnel(const handle_t& hdl,
                           ...
                           const boost::asio::ip::address& dst, uint32_t vni)
  : interface(hdl, l2_address_t::ZERO, mk_name(src, dst, vni),
              interface::type_t::VXLAN, interface::admin_state_t::UP)
  , m_tep(src, dst, vni)
{
}
