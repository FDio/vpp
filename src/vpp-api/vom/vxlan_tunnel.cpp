...
namespace VOM
{
    ...
    vxlan_tunnel::endpoint_t::endpoint_t() : src(), dst(), vni(0)
    {
    }
    ...
    bool vxlan_tunnel::endpoint_t::
    operator<(const vxlan_tunnel::endpoint_t &o) const
    {
        if (src < o.src)
            return true;
        if (dst < o.dst)
    ...
    vxlan_tunnel::event_handler::handle_populate(const client_db::key_t &key)
    {
         ...
         */
        std::shared_ptr<vxlan_tunnel::dump_cmd> cmd(
            new vxlan_tunnel::dump_cmd());
