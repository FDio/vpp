...
namespace VOM
{
    ...
    rc_t::rc_t(int v, const std::string s) : enum_base<rc_t>(v, s)
    {
    }
    rc_t::~rc_t()
    {
    ...
    handle_t::handle_t(int value) : m_value(value)
    {
    }

    handle_t::handle_t() : m_value(~0)
    {
    }
    ...
    const mac_address_t mac_address_t::ONE({0xff, 0xff, 0xff, 0xff, 0xff,
                                            0xff});
    ...
    l2_address_t::l2_address_t(std::initializer_list<uint8_t> i) : bytes(i)
    {
    }

    l2_address_t::l2_address_t(const mac_address_t &mac) : bytes(6)
    {
        std::copy(begin(mac.bytes), std::end(mac.bytes), std::begin(bytes));
    }

    const l2_address_t l2_address_t::ONE({0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                          0xff, 0xff});
    ...
    const direction_t direction_t::OUTPUT(1, "output");

    direction_t::direction_t(int v, const std::string s) : enum_base(v, s)
    {
    }
}
