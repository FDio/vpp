...
singular_db<gbp_endpoint_group::key_t,
            gbp_endpoint_group> gbp_endpoint_group::m_db;
...
gbp_endpoint_group::sweep()
{
  if (m_hw) {
    HW::enqueue(
        new gbp_endpoint_group_cmds::delete_cmd(m_hw, m_epg_id));
  }
  HW::write();
}
...
{
  if (m_hw) {
    HW::enqueue(new gbp_endpoint_group_cmds::create_cmd(m_hw, m_epg_id,
                                                        m_bd->id(),
                                                        m_rd->table_id(),
                                                        m_itf->handle()));
  }
...
gbp_endpoint_group::event_handler::event_handler()
{
  ...
  inspect::register_handler({ "gbp-endpoint-group" }, "GBP Endpoint_Groups", this);
}
...
gbp_endpoint_group::event_handler::handle_populate(const client_db::key_t& key)
{
  ...
  for (auto& record : *cmd) {
    ...
    std::shared_ptr<route_domain> rd = route_domain::find(payload.epg.ip4_table_id);
    std::shared_ptr<bridge_domain> bd = bridge_domain::find(payload.epg.bd_id);

    VOM_LOG(log_level_t::DEBUG) << "data: " << payload.epg.epg_id;
