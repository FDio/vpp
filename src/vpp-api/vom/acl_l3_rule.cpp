...
namespace VOM
{
    namespace ACL
    {
        ...
        void l3_rule::to_vpp(vapi_type_acl_rule &rule) const
        {
            ...
            m_src.to_vpp(&rule.is_ipv6, rule.src_ip_addr,
                         &rule.src_ip_prefix_len);
            m_dst.to_vpp(&rule.is_ipv6, rule.dst_ip_addr,
                         &rule.dst_ip_prefix_len);
        ...
        bool l3_rule::operator==(const l3_rule &rule) const
        {
            return ((m_action == rule.m_action) && (m_src == rule.m_src) &&
                    ...
                    (m_srcport_or_icmptype_first ==
                     rule.m_srcport_or_icmptype_first) &&
                    (m_srcport_or_icmptype_last ==
                     rule.m_srcport_or_icmptype_last) &&
                    (m_dstport_or_icmpcode_first ==
                     rule.m_dstport_or_icmpcode_first) &&
                    (m_dstport_or_icmpcode_last ==
                     rule.m_dstport_or_icmpcode_last) &&
