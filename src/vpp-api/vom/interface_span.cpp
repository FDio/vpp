...
namespace VOM
{
     ...
     */
    singular_db<interface_span::key_type_t, interface_span>
        interface_span::m_db;
    ...
    void interface_span::sweep()
    {
        if (m_config)
        {
            HW::enqueue(new unconfig_cmd(m_config, m_itf_from->handle(),
                                         m_itf_to->handle()));
        }
        HW::write();
    }
    ...
    interface_span::event_handler::handle_populate(const client_db::key_t &key)
    {
        ...
        std::shared_ptr<interface_span::dump_cmd> cmd(
            new interface_span::dump_cmd());
        ...
        for (auto &record : *cmd)
        {
            ...
            std::shared_ptr<interface> itf_to =
    ...
    const interface_span::state_t interface_span::state_t::DISABLED(0,
                                                                    "disable");
    const interface_span::state_t
        interface_span::state_t::RX_ENABLED(1, "rx-enable");
    const interface_span::state_t
        interface_span::state_t::TX_ENABLED(2, "tx-enable");
    const interface_span::state_t
        interface_span::state_t::TX_RX_ENABLED(3, "tx-rx-enable");
