...
gbp_recirc::sweep()
{
  if (m_hw) {
    HW::enqueue(
      new gbp_recirc_cmds::delete_cmd(m_hw, m_itf->handle()));
  }
  HW::write();
}
...
{
  if (m_hw) {
    HW::enqueue(new gbp_recirc_cmds::create_cmd(m_hw, m_itf->handle(),
...
gbp_recirc::event_handler::handle_populate(const client_db::key_t& key)
{
  ...
  for (auto& record : *cmd) {
    ...
    if (itf && epg) {
        gbp_recirc recirc(*itf,
                          (payload.recirc.is_ext?
                           type_t::EXTERNAL :
                           type_t::INTERNAL),
                          *epg);
