...
class VppPGInterface(VppInterface):
    ...
    def __init__(self, vclient, pg_index, gso, gso_size):
        ...
        self._out_path = self._vclient.tempdir + "/" + self._out_file
        self._in_file = "pg%u_in.pcap" % self.pg_index
        self._in_path = self._vclient.tempdir + "/" + self._in_file

    def add_vpp_config(self):
        r = self._vclient.pg_create_interface(
            self._pg_index, self._gso_enabled, self._gso_size)
        ...
        try:
            if os.path.isfile(path):
                name = "%s/history.[timestamp:%f].[%s-counter:%04d].%s" % \
                    (self._vclient.tempdir,
                     time.time(),
                     self.name,
                     counter,
                     file)
                self._vclient.logger.debug("Renaming %s->%s" %
                                          (path, name))
                os.rename(path, name)
        except OSError:
            self._vclient.logger.debug("OSError: Could not rename %s %s" %
                                      (path, file))
    ...
    def enable_capture(self):
        ...
        self._vclient.cli(self.capture_cli)
        self._pcap_reader = None

    def disable_capture(self):
        self._vclient.cli("%s disable" % self.capture_cli)

    def coalesce_enable(self):
        """ Enable packet coalesce on this packet-generator interface"""
        self._coalesce_enabled = 1
        self._vclient.pg_interface_enable_disable_coalesce(self.sw_if_index,
                                                          1)
    ...
    def add_stream(self, pkts, nb_replays=None, worker=None):
        ...
        self._vclient.register_capture(self.cap_name)
        # FIXME this should be an API, but no such exists atm
        self._vclient.cli(self.input_cli)
    ...
    def generate_debug_aid(self, kind):
        ...
        files present. """
        self._vclient.logger.debug("Generating debug aid for %s on %s" %
                                  (kind, self._name))
        link_path, stack_path = ["%s/debug_%s_%s_%s.%s" %
                                 (self._vclient.tempdir, self._name,
    ...
    def _get_capture(self, timeout, filter_out_fn=is_ipv6_misc):
        ...
        try:
            ...
            self._vclient.logger.debug(
                "Capture has %s packets" % len(output.res))
        except BaseException:
            self._vclient.logger.debug("Exception in scapy.rdpcap (%s): %s" %
    ...
    def wait_for_pg_stop(self):
        ...
        while self._vclient.cli('show packet-generator').find("Yes") != -1:
            self._vclient.sleep(0.01)  # yield
            if time.time() > deadline:
                self._vclient.logger.debug("Timeout waiting for pg to stop")
                break
    ...
    def wait_for_capture_file(self, timeout=1):
        ...
        if not os.path.isfile(self.out_path):
            self._vclient.logger.debug(
                ...
                "timeout is %ss" %
                (self.out_path, timeout))
        else:
            self._vclient.logger.debug("Capture file %s already exists" %
                                      self.out_path)
            return True
        ...
        if os.path.isfile(self.out_path):
            self._vclient.logger.debug("Capture file appeared after %fs" %
                                      (time.time() - (deadline - timeout)))
        else:
            self._vclient.logger.debug("Timeout - capture file still nowhere")
            return False
        return True
    ...
    def wait_for_packet(self, timeout, filter_out_fn=is_ipv6_misc):
        ...
        poll = False
        if timeout > 0:
            self._vclient.logger.debug("Waiting for packet")
        else:
            poll = True
            self._vclient.logger.debug("Polling for packet")
        while time.time() < deadline or poll:
            ...
            self._vclient.sleep(0)  # yield
            poll = False
        self._vclient.logger.debug("Timeout - no packets received")
    ...
    def resolve_arp(self, pg_interface=None):
        ...
        if pg_interface is None:
            pg_interface = self
        self._vclient.logger.info("Sending ARP request for %s on port %s" %
                                 (self.local_ip4, pg_interface.name))
        arp_req = self.create_arp_req()
        pg_interface.add_stream(arp_req)
        pg_interface.enable_capture()
        self._vclient.pg_start()
        self._vclient.logger.info(self._vclient.cli("show trace"))
        try:
            captured_packet = pg_interface.wait_for_packet(1)
        except BaseException:
            self._vclient.logger.info("No ARP received on port %s" %
                                     pg_interface.name)
            return
        arp_reply = captured_packet.copy()  # keep original for exception
        try:
            if arp_reply[ARP].op == ARP.is_at:
                self._vclient.logger.info("VPP %s MAC address is %s " %
    ...
    def resolve_ndp(self, pg_interface=None, timeout=1):
        ...
        if pg_interface is None:
            pg_interface = self
        self._vclient.logger.info("Sending NDP request for %s on port %s" %
        ...
        while now < deadline:
            ...
            try:
                ...
                self._vclient.logger.debug(self._vclient.cli("show trace"))
                # we now have the MAC we've been after
                return
            except BaseException:
                self._vclient.logger.info(
