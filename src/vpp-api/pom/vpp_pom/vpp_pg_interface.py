...
class VppPGInterface(VppInterface):
    ...
    def __init__(self, vclient, pg_index, gso, gso_size):
        ...
        r = vclient.vapi.pg_create_interface(pg_index, gso, gso_size)
    ...
    def enable_capture(self):
        ...
        self.vclient.vapi.cli(self.capture_cli)
        self._pcap_reader = None

    def disable_capture(self):
        self.vclient.vapi.cli("%s disable" % self.capture_cli)

    def coalesce_enable(self):
        """ Enable packet coalesce on this packet-generator interface"""
        self._coalesce_enabled = 1
        self.vclient.vapi.pg_interface_enable_disable_coalesce(self.sw_if_index,
                                                            1)
    ...
    def add_stream(self, pkts, nb_replays=None, worker=None):
        ...
        # FIXME this should be an API, but no such exists atm
        self.vclient.vapi.cli(self.input_cli)
    ...
    def get_capture(self, expected_count=None, remark=None, timeout=1,
        ...
        :param expected_count: expected number of packets to capture, if None,
                               then self.vclient.packet_count_for_dst_pg_idx is
        ...
        if expected_count is None:
            expected_count = \
                self.vclient.get_packet_count_for_if_idx(self.sw_if_index)
    ...
    def wait_for_pg_stop(self):
        ...
        while self.vclient.vapi.cli('show packet-generator').find("Yes") != -1:
    ...
    def resolve_arp(self, pg_interface=None):
        ...
        self.vclient.logger.info(self.vclient.vapi.cli("show trace"))
        try:
    ...
    def resolve_ndp(self, pg_interface=None, timeout=1):
        ...
        while now < deadline:
            ...
            try:
                ...
                self.vclient.logger.debug(self.vclient.vapi.cli("show trace"))
