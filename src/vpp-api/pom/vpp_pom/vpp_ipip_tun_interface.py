...
class VppIpIpTunInterface(VppTunnelInterface):
    ...
    def add_vpp_config(self):
        r = self.vclient.vapi.ipip_add_tunnel(
            tunnel={
                'src': self.src,
                'dst': self.dst,
    ...
    def remove_vpp_config(self):
        self.vclient.vapi.ipip_del_tunnel(sw_if_index=self._sw_if_index)

    def query_vpp_config(self):
        ts = self.vclient.vapi.ipip_tunnel_dump(sw_if_index=0xffffffff)
        for t in ts:
