...
def find_bier_table(vclient, bti):
    tables = vclient.vapi.bier_table_dump()
    for t in tables:
...
def find_bier_route(vclient, bti, bp):
    routes = vclient.vapi.bier_route_dump(bti)
    for r in routes:
...
def find_bier_disp_table(vclient, bdti):
    tables = vclient.vapi.bier_disp_table_dump()
    for t in tables:
...
def find_bier_disp_entry(vclient, bdti, bp):
    entries = vclient.vapi.bier_disp_entry_dump(bdti)
    for e in entries:
        if bp == e.bde_bp \
...
def find_bier_imp(vclient, bti, bp):
    imps = vclient.vapi.bier_imp_dump()
    for i in imps:
...
class VppBierTable(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.bier_table_add_del(
            self.id,
            self.mpls_label,
            is_add=1)
...
class VppBierRoute(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.bier_route_add_del(
            self.tbl_id,
            self.bp,
            self.encoded_paths,
            is_add=1)
...
class VppBierImp(VppObject):
    ...
    def add_vpp_config(self):
        res = self._vclient.vapi.bier_imp_add(
            self.tbl_id,
            self.src,
            self.ibytes)
    ...
    def remove_vpp_config(self):
        self._vclient.vapi.bier_imp_del(
            self.bi_index)
...
class VppBierDispTable(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.bier_disp_table_add_del(
            self.id,
            is_add=1)
        self._vclient.registry.register(self, self._vclient.logger)
...
class VppBierDispEntry(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.bier_disp_entry_add_del(
            self.tbl_id,
            self.bp,
            self.payload_proto,
            self.nh_proto,
