...
def query_all_memif_vpp_config(vclient):
    return vclient.memif_dump()


def remove_all_memif_vpp_config(vclient):
    dump = vclient.memif_dump()
    if dump:
        for d in dump:
            vclient.memif_delete(d.sw_if_index)
    dump = vclient.memif_socket_filename_dump()
    if dump:
        for d in dump:
            if d.socket_id != 0:
                vclient.memif_socket_filename_add_del(
