...
class VppBondInterface(VppInterface):
    ...
    def add_vpp_config(self):
        r = self.vclient.vapi.bond_create2(self.mode,
                                        self.lb,
                                        self.numa_only,
                                        self.enable_gso,
                                        self.use_custom_mac,
    ...
    def remove_vpp_config(self):
        self.vclient.vapi.bond_delete(self.sw_if_index)

    def add_member_vpp_bond_interface(self,
                                      sw_if_index,
                                      is_passive=0,
                                      is_long_timeout=0):
        self.vclient.vapi.bond_add_member(sw_if_index,
                                       self.sw_if_index,
                                       is_passive,
                                       is_long_timeout)
    ...
    def detach_vpp_bond_interface(self,
                                  sw_if_index):
        self.vclient.vapi.bond_detach_member(sw_if_index)
