...
def find_bridge_domain(vclient, bd_id):
    bds = vclient.vapi.bridge_domain_dump(bd_id)
    return len(bds) == 1
...
def find_bridge_domain_arp_entry(vclient, bd_id, mac, ip):
    arps = vclient.vapi.bd_ip_mac_dump(bd_id)
    for arp in arps:
...
def find_l2_fib_entry(vclient, bd_id, mac, sw_if_index):
    vmac = MACAddress(mac)
    lfs = vclient.vapi.l2_fib_table_dump(bd_id)
    for lf in lfs:
...
class VppBridgeDomain(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.bridge_domain_add_del(bd_id=self.bd_id,
                                              flood=self.flood,
    ...
    def remove_vpp_config(self):
        self._vclient.vapi.bridge_domain_add_del(bd_id=self.bd_id, is_add=0)
...
class VppBridgeDomainPort(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.sw_interface_set_l2_bridge(
...
class VppBridgeDomainArpEntry(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.bd_ip_mac_add_del(is_add=1,
                                          entry={
                                              'bd_id': self.bd.bd_id,
                                              'ip': self.ip,
                                              'mac': self.mac})
    ...
    def remove_vpp_config(self):
        self._vclient.vapi.bd_ip_mac_add_del(is_add=0,
                                          entry={
                                              'bd_id': self.bd.bd_id,
                                              'ip': self.ip,
                                              'mac': self.mac})
...
class VppL2FibEntry(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.l2fib_add_del(
            self.mac.packed,
            self.bd.bd_id,
            self.itf.sw_if_index,
            is_add=1,
...
class VppL2Vtr(VppObject):
    ...
    def query_vpp_config(self):
        ds = self._vclient.vapi.sw_interface_dump()
