...
class VppSubInterface(VppPGInterface):
    ...
    def remove_vpp_config(self):
        self.vclient.vapi.delete_subif(self.sw_if_index)
    ...
    def _remove_tag(self, packet, vlan=None, tag_type=None):
        if tag_type:
            self.vclient.instance().assertEqual(packet.type, tag_type)

        payload = packet.payload
        if vlan:
            self.vclient.instance().assertEqual(payload.vlan, vlan)
    ...
    def set_vtr(self, vtr, push1q=0, tag=None, inner=None, outer=None):
        ...
        self.vclient.vapi.l2_interface_vlan_tag_rewrite(
...
class VppDot1QSubint(VppSubInterface):
    ...
    def __init__(self, vclient, parent, sub_id, vlan=None):
        ...
        if vlan is None:
            vlan = sub_id
        self._vlan = vlan
        r = vclient.vapi.create_vlan_subif(parent.sw_if_index, vlan)
...
class VppDot1ADSubint(VppSubInterface):
    ...
    def __init__(self, vclient, parent, sub_id, outer_vlan, inner_vlan):
        ...
        r = vclient.vapi.create_subif(sw_if_index=parent.sw_if_index,
...
class VppP2PSubint(VppSubInterface):
    ...
    def __init__(self, vclient, parent, sub_id, remote_mac):
        ...
        r = vclient.vapi.p2p_ethernet_add(parent.sw_if_index,
                                       remote_mac, sub_id)
        self.set_sw_if_index(r.sw_if_index)
