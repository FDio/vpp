...
class VppIpsecSpd(VppObject):
    ...
    def add_vpp_config(self):
        self.vclient.vapi.ipsec_spd_add_del(self.id)
        self.vclient.registry.register(self, self.vclient.logger)

    def remove_vpp_config(self):
        self.vclient.vapi.ipsec_spd_add_del(self.id, is_add=0)

    def object_id(self):
        return "ipsec-spd-%d" % self.id

    def query_vpp_config(self):
        spds = self.vclient.vapi.ipsec_spds_dump()
        for spd in spds:
...
class VppIpsecSpdItfBinding(VppObject):
    ...
    def add_vpp_config(self):
        self.vclient.vapi.ipsec_interface_add_del_spd(self.spd.id,
    ...
    def query_vpp_config(self):
        bs = self.vclient.vapi.ipsec_spd_interface_dump()
        for b in bs:
...
class VppIpsecSpdEntry(VppObject):
    ...
    def add_vpp_config(self):
        rv = self.vclient.vapi.ipsec_spd_entry_add_del(
            self.spd.id,
            self.sa_id,
            self.local_start,
            self.local_stop,
    ...
    def remove_vpp_config(self):
        self.vclient.vapi.ipsec_spd_entry_add_del(
            self.spd.id,
            self.sa_id,
            self.local_start,
            self.local_stop,
    ...
    def query_vpp_config(self):
        ss = self.vclient.vapi.ipsec_spd_dump(self.spd.id)
        for s in ss:
...
class VppIpsecSA(VppObject):
    ...
    def add_vpp_config(self):
        ...
        r = self.vclient.vapi.ipsec_sad_entry_add_del(is_add=1, entry=entry)
        self.stat_index = r.stat_index
        self.vclient.registry.register(self, self.vclient.logger)

    def remove_vpp_config(self):
        r = self.vclient.vapi.ipsec_sad_entry_add_del(
            is_add=0,
            entry={
                'sad_id': self.id,
                'spi': self.spi,
    ...
    def query_vpp_config(self):
        ...
        bs = self.vclient.vapi.ipsec_sa_dump()
        for b in bs:
...
class VppIpsecTunProtect(VppObject):
    ...
    def update_vpp_config(self, sa_out, sas_in):
        ...
        for sa in sas_in:
            self.sas_in.append(sa.id)
        self.sa_out = sa_out.id
        self.vclient.vapi.ipsec_tunnel_protect_update(
            tunnel={
    ...
    def remove_vpp_config(self):
        self.vclient.vapi.ipsec_tunnel_protect_del(
            sw_if_index=self.itf.sw_if_index,
            nh=self.nh)

    def query_vpp_config(self):
        bs = self.vclient.vapi.ipsec_tunnel_protect_dump(
            sw_if_index=self.itf.sw_if_index)
        for b in bs:
...
class VppIpsecInterface(VppInterface):
    ...
    def __init__(self, vclient, mode=None):
    ...
    def add_vpp_config(self):
        r = self.vclient.vapi.ipsec_itf_create(itf={
    ...
    def remove_vpp_config(self):
        self.vclient.vapi.ipsec_itf_delete(sw_if_index=self._sw_if_index)

    def query_vpp_config(self):
        ts = self.vclient.vapi.ipsec_itf_dump(sw_if_index=0xffffffff)
        for t in ts:
