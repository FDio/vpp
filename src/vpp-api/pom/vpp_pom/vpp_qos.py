...
class VppQosRecord(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.qos_record_enable_disable(
            enable=1,
    ...
    def query_vpp_config(self):
        rs = self._vclient.vapi.qos_record_dump()
...
class VppQosStore(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.qos_store_enable_disable(
            enable=1,
    ...
    def query_vpp_config(self):
        rs = self._vclient.vapi.qos_store_dump()
...
class VppQosEgressMap(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.qos_egress_map_update(
            map={'id': self.id,
                 'rows': self.rows})
        self._vclient.registry.register(self, self._vclient.logger)
        return self
    ...
    def remove_vpp_config(self):
        self._vclient.vapi.qos_egress_map_delete(id=self.id)

    def query_vpp_config(self):
        rs = self._vclient.vapi.qos_egress_map_dump()
...
class VppQosMark(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.qos_mark_enable_disable(
            enable=1,
    ...
    def query_vpp_config(self):
        ms = self._vclient.vapi.qos_mark_dump()
