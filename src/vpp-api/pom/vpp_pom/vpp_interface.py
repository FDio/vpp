...
class VppInterface(object):
    ...
    def link_up(self):
        ...
        self.vclient.cli("vclient interface link-state %s up" % self.name)

    def link_down(self):
        """Put interface link-state-down."""
        self.vclient.cli("vclient interface link-state %s down" % self.name)
    ...
    def assert_interface_state(self, admin_up_down, link_up_down,
        ...
        if expect_event:
            ...
            self.vclient.assert_equal(event.sw_if_index, self.sw_if_index,
                                   "sw_if_index")
            self.vclient.assert_equal((event.flags & 1), admin_up_down,
                                   "admin state")
            self.vclient.assert_equal((event.flags & 2), link_up_down,
                                   "link state")
        dump = self.vclient.sw_interface_dump()
        if_state = self.get_interface_config_from_dump(dump)
        self.vclient.assert_equal((if_state.flags & 1), admin_up_down,
                               "admin state")
        self.vclient.assert_equal((if_state.flags & 2), link_up_down,
                               "link state")
