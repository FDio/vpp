    ...
    def assert_interface_state(self, admin_up_down, link_up_down,
        ...
        if expect_event:
            ...
            if event.sw_if_index != self.sw_if_index:
                raise AssertionError("invalid interface %s != %s" % (
                    event.sw_if_index, self.sw_if_index))
            if (event.flags & 1) != admin_up_down or (
                    event.flags & 2) != link_up_down:
                raise AssertionError(
                    "invalid interface state: admin (%s) link (%s)" %
                    ((event.flags & 1), (event.flags & 2)))
        dump = self.vclient.sw_interface_dump()
        if_state = self.get_interface_config_from_dump(dump)
        if (if_state.flags & 1) != admin_up_down or (
                if_state.flags & 2) != link_up_down:
            raise AssertionError(
                "invalid interface state: admin (%s) link (%s)" %
                ((if_state.flags & 1), (if_state.flags & 2)))
