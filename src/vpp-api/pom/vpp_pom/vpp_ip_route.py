...
def find_route(vclient, addr, len, table_id=0, sw_if_index=None):
    ...
    if 4 == prefix.version:
        routes = vclient.vapi.ip_route_dump(table_id, False)
    else:
        routes = vclient.vapi.ip_route_dump(table_id, True)
...
def find_mroute(vclient, grp_addr, src_addr, grp_addr_len,
    ...
    if 4 == ip_mprefix.version:
        routes = vclient.vapi.ip_mroute_dump(table_id, False)
    else:
        routes = vclient.vapi.ip_mroute_dump(table_id, True)
...
def find_mpls_route(vclient, table_id, label, eos_bit, paths=None):
    dump = vclient.vapi.mpls_route_dump(table_id)
    for e in dump:
...
def fib_interface_ip_prefix(vclient, addr, len, sw_if_index):
    ...
    prefix = "%s/%d" % (addr, len)
    addrs = vclient.vapi.ip_address_dump(
        sw_if_index,
...
class VppIpTable(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.ip_table_add_del(is_add=1,
    ...
    def remove_vpp_config(self):
        self._vclient.vapi.ip_table_add_del(is_add=0,
                                         table={'is_ip6': self.is_ip6,
                                                'table_id': self.table_id})

    def replace_begin(self):
        self._vclient.vapi.ip_table_replace_begin(
            table={'is_ip6': self.is_ip6,
                   'table_id': self.table_id})

    def replace_end(self):
        self._vclient.vapi.ip_table_replace_end(
            table={'is_ip6': self.is_ip6,
                   'table_id': self.table_id})

    def flush(self):
        self._vclient.vapi.ip_table_flush(table={'is_ip6': self.is_ip6,
                                              'table_id': self.table_id})

    def dump(self):
        return self._vclient.vapi.ip_route_dump(self.table_id, self.is_ip6)

    def mdump(self):
        return self._vclient.vapi.ip_mroute_dump(self.table_id, self.is_ip6)
...
class VppIpInterfaceAddress(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.sw_interface_add_del_address(
...
class VppIp6LinkLocalAddress(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.sw_interface_ip6_set_link_local_address(
...
class VppIpInterfaceBind(VppObject):
    ...
    def query_vpp_config(self):
        if 0 == self.table.table_id:
            return False
        return self._vclient.vapi.sw_interface_get_table(
            self.intf.sw_if_index,
...
class VppIpRoute(VppObject):
    ...
    def modify(self, paths):
        ...
        self._vclient.vapi.ip_route_add_del(route={'table_id': self.table_id,
                                                'prefix': self.prefix,
                                                'n_paths': len(
                                                    self.encoded_paths),
    ...
    def add_vpp_config(self):
        r = self._vclient.vapi.ip_route_add_del(
    ...
    def remove_vpp_config(self):
        ...
        if self.modified:
            self._vclient.vapi.ip_route_add_del(
                route={'table_id': self.table_id,
                       'prefix': self.prefix,
                       'n_paths': len(
                           self.encoded_paths),
...
class VppIpMRoute(VppObject):
    ...
    def add_vpp_config(self):
        r = self._vclient.vapi.ip_mroute_add_del(route=self.encode(),
                                              is_multipath=1,
                                              is_add=1)
        self.stats_index = r.stats_index
        self._vclient.registry.register(self, self._vclient.logger)
    ...
    def remove_vpp_config(self):
        self._vclient.vapi.ip_mroute_add_del(route=self.encode(),
                                          is_multipath=1,
                                          is_add=0)

    def update_entry_flags(self, flags):
        self.e_flags = flags
        self._vclient.vapi.ip_mroute_add_del(route=self.encode(paths=[]),
                                          is_multipath=1,
                                          is_add=1)
    ...
    def update_path_flags(self, itf, flags):
        ...
        self._vclient.vapi.ip_mroute_add_del(
            route=self.encode(
...
class VppMFibSignal(object):
    ...
    def compare(self, signal):
        self.vclient.assertEqual(self.interface, signal.sw_if_index)
        self.vclient.assertEqual(self.route.table_id, signal.table_id)
        self.vclient.assertEqual(self.route.prefix, signal.prefix)
...
class VppMplsIpBind(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.mpls_ip_bind_unbind(self.local_label,
                                            self.prefix,
                                            table_id=self.table_id,
    ...
    def query_vpp_config(self):
        dump = self._vclient.vapi.mpls_route_dump(self.table_id)
        for e in dump:
...
class VppMplsTable(VppObject):
    ...
    def add_vpp_config(self):
        self._vclient.vapi.mpls_table_add_del(
            self.table_id,
            is_add=1)
        self._vclient.registry.register(self, self._vclient.logger)
    ...
    def query_vpp_config(self):
        dump = self._vclient.vapi.mpls_table_dump()
        for d in dump:
...
class VppMplsRoute(VppObject):
    ...
    def add_vpp_config(self):
        ...
        r = self._vclient.vapi.mpls_route_add_del(self.table_id,
                                               self.local_label,
                                               self.eos_bit,
                                               self.eos_proto,
                                               self.is_multicast,
    ...
    def remove_vpp_config(self):
        ...
        self._vclient.vapi.mpls_route_add_del(self.table_id,
                                           self.local_label,
                                           self.eos_bit,
                                           self.eos_proto,
                                           self.is_multicast,
