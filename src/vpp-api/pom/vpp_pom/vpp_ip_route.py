...
class VppMFibSignal(object):
    ...
    def compare(self, signal):
        if self.interface != signal.sw_if_index:
            raise AssertionError("interface (%s) != signal interface (%s)" % (
                self.interface, signal.sw_if_index))
        if self.route.table_id != signal.table_id:
            raise AssertionError("table id (%s) != signal table id (%s)" % (
                self.route.table_id, signal.table_id))
        if self.route.prefix != signal.prefix:
            raise AssertionError("prefix (%s) != signal prefix (%s)" % (
                self.route.prefix, signal.prefix))
