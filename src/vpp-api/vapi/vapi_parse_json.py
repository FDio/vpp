...
class JsonParser:
    ...
    def gen_vapi_header(self):
        with open('%s%s' % (self.prefix, self.vapi_header), "w") as f:
            self.gen_header(f)
    ...
    def gen_header(self, io):
        ...
        include_guard = "included_%s" % (
            self.vapi_header.replace(".", "_").replace("/", "_"))
        ...
        for header in msg_header_defs:
            ...
            for t, v in header['fields']:
                print("  %s %s;" % (self.types[t].get_c_name(), v))
            print("} %s;" % header['name'])
            print("")
        # print("typedef enum {")
        # for t, v in self.types.items():
        #     try:
        #         print("  %s," % v.get_msg_id_name())
        #     except:
        #         pass
    ...
    def gen_code(self, io):
        orig_stdout = sys.stdout
        sys.stdout = io
        print("#include <%s>" % self.vapi_header)
        ...
        # for n, m in self.messages.items():
        #     print("  [%s] = true," % m.get_msg_id_name())
        # print("};")
        # print("")
        ...
        #           (m.get_msg_id_name(), m.header['name']))
        # print("};")
        # print("")
        # print("static size_t __vapi_msg_id_to_msg_size[] = {")
        # for n, m in self.messages.items():
        #     print("  [%s] = sizeof(%s)," %
        #           (m.get_msg_id_name(), m.get_c_name()))
        # print("};")
        # print("")
        ...
        # if m.is_reply_only() :
        # continue
        # print("  [%s] = %s," % (m.get_msg_id_name(),
        # self.get_reply(n).get_msg_id_name()))
        # print("};")
        # print("")
    ...
    def json_to_header_name(self, json_name):
        ...
        raise Exception("Unexpected json name!")
    ...
    def gen_c_headers_and_code(self):
        ...
        # self.gen_vapi_header()

    def gen_json_header(self, j, io):
        orig_stdout = sys.stdout
        sys.stdout = io
        include_guard = "included_%s" % j.replace(".", "_").replace("/", "_")
        print("#ifndef %s" % include_guard)
        print("#define %s" % include_guard)
        print("")
