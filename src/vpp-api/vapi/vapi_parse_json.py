...
msg_header_defs = [
    MsgHeaderDef(name='vapi_msg_header1_t',
                 fields=[
                     Parameter(param_type='u16', param_name='_vl_msg_id'),
                     Parameter(param_type='u32', param_name='context')]),
    MsgHeaderDef(name='vapi_msg_header2_t',
                 fields=[
                     Parameter(param_type='u16', param_name='_vl_msg_id'),
                     Parameter(param_type='u32', param_name='client_index'),
                     Parameter(param_type='u32', param_name='context')]),
]
...
class JsonParser:
    ...
    def gen_c_headers_and_code(self, prefix):
        if prefix == "" or prefix is None:
            prefix = ""
        else:
            prefix = "%s/" % prefix
        for j in self.json_files:
            with open('%s%s' % (prefix, self.json_to_header_name(j)),
                      "w") as io:
                self.gen_json_header(j, io)
            with open('%s%s' % (prefix, self.json_to_code_name(j)),
                      "w") as io:
...
if __name__ == '__main__':
    ...
    ctx = JsonParser(logger=logger)

    for f in args.files:
        ctx.parse_json_file(f)

    ctx.validate_json_data()
    ctx.gen_c_headers_and_code(prefix=args.prefix)
