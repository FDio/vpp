...
vapi_ctx_alloc (vapi_ctx_t * result)
{
  ...
  pthread_mutex_init (&ctx->recv_mutex, NULL);
...
vapi_send (vapi_ctx_t ctx, void *msg)
{
  ...
  int tmp;
  if (0 != (tmp = pthread_mutex_lock (&ctx->send_mutex)))
    {
      VAPI_DBG ("pthread_mutex_lock() failed, rv=%d:%s", tmp, strerror (tmp));
...
unlock:
  if (0 != (tmp = pthread_mutex_unlock (&ctx->send_mutex)))
    {
      VAPI_DBG ("pthread_mutex_unlock() failed, rv=%d:%s", tmp,
		strerror (tmp));
...
vapi_recv (vapi_ctx_t ctx, void **msg, size_t * msg_size)
{
  ...
  int tmp;
  if (0 != (tmp = pthread_mutex_lock (&ctx->recv_mutex)))
    {
      VAPI_DBG ("pthread_mutex_lock() failed, rv=%d:%s", tmp, strerror (tmp));
      return VAPI_MUTEX_FAILURE;
    }
  ...
  tmp = unix_shared_memory_queue_sub (q, (u8 *) & data, 0);
  if (tmp == 0)
    {
#if VAPI_DEBUG_ALLOC
...
unlock:
  if (0 != (tmp = pthread_mutex_unlock (&ctx->recv_mutex)))
    {
      VAPI_DBG ("pthread_mutex_unlock() failed, rv=%d:%s", tmp,
		strerror (tmp));
      abort ();			/* this really shouldn't happen */
