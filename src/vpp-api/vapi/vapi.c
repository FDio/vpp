...
struct vapi_ctx_s
{
  ...
  bool connected;
  pthread_mutex_t requests_producer_mutex;
  pthread_mutex_t requests_consumer_mutex;
};
		    ...
		    vapi_cb_t callback, void *callback_ctx)
{
  ...
  assert (0 != pthread_mutex_trylock (&ctx->requests_producer_mutex));
  vapi_req_t *slot = &ctx->requests[ctx->requests_end];
  slot->is_dump = is_dump;
  slot->context = context;
  slot->callback = callback;
  slot->callback_ctx = callback_ctx;
  VAPI_DBG ("stored@%d: context:%x", ctx->requests_end, context);
  ctx->requests_end = (ctx->requests_end + 1) % ctx->requests_size;
}

vapi_error_e
vapi_send_with_control_ping (vapi_ctx_t ctx, void *msg,
			     uword size, u32 context)
{
  ...
  return vapi_send2 (ctx, msg, size, ping, sizeof (*ping));
}
...
vapi_ctx_alloc (vapi_ctx_t * result)
{
  ...
  pthread_mutex_init (&ctx->requests_producer_mutex, NULL);
  pthread_mutex_init (&ctx->requests_consumer_mutex, NULL);
  *result = ctx;
  return VAPI_OK;
fail:
  vapi_ctx_free (ctx);
...
vapi_ctx_free (vapi_ctx_t ctx)
{
  ...
  pthread_mutex_destroy (&ctx->requests_producer_mutex);
  pthread_mutex_destroy (&ctx->requests_consumer_mutex);
  free (ctx);
}
...
vapi_error_e
vapi_send (vapi_ctx_t ctx, void *msg, uword size)
{
...
#endif
  tmp =
    unix_shared_memory_queue_add_custom (q, (u8 *) & msg, size,
					 VAPI_MODE_BLOCKING ==
					 ctx->mode ? 0 : 1);
  if (tmp < 0)
    {
...
vapi_error_e
vapi_send2 (vapi_ctx_t ctx, void *msg1, uword size1, void *msg2, uword size2)
{
  vapi_error_e rv = VAPI_OK;
...
#endif
  int tmp =
    unix_shared_memory_queue_add2 (q, (u8 *) & msg1, size1, (u8 *) & msg2,
				   size2,
...
{
  int mrv;
  if (0 != (mrv = pthread_mutex_lock (&ctx->requests_consumer_mutex)))
    {
  ...
  if (ctx->requests[tmp].context == context)
      ...
      while (ctx->requests_start != tmp)
	{
	  ...
	  ctx->requests[ctx->requests_start].callback (ctx,
						       ctx->
						       requests
						       [ctx->requests_start].callback_ctx,
						       VAPI_ENORESP, true,
						       NULL);
      ...
      if (is_last)
	{
      ...
      VAPI_DBG ("after dispatch, req start = %d, end = %d",
		ctx->requests_start, ctx->requests_end);
    }
  if (0 != (mrv = pthread_mutex_unlock (&ctx->requests_consumer_mutex)))
    {
...
{
  int mrv;
  if (0 != (mrv = pthread_mutex_lock (&ctx->requests_producer_mutex)))
    {
      VAPI_DBG ("pthread_mutex_lock() failed, rv=%d:%s", mrv, strerror (mrv));
...
{
  int mrv;
  if (0 != (mrv = pthread_mutex_unlock (&ctx->requests_producer_mutex)))
    {
      VAPI_DBG ("pthread_mutex_unlock() failed, rv=%d:%s", mrv,
		strerror (mrv));
      (void) mrv;		/* avoid warning if the above debug is not enabled */
