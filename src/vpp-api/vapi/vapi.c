...
vapi_error_e
vapi_send_control_ping (vapi_ctx_t ctx, void *msg, u32 context)
{
  vapi_msg_init_control_ping (ctx, msg);
  vapi_msg_control_ping *ping = msg;
  ping->header.context = context;
  vapi_msg_control_ping_swap_to_be (ping);
...
#endif
  int rv = unix_shared_memory_queue_add (q, (u8 *) & msg,
					 VAPI_MODE_BLOCKING ==
					 ctx->mode ? 0 : 1);
  if (rv < 0)
    {
...
void
vapi_set_generic_event_cb (vapi_ctx_t ctx,
			   vapi_error_e (*callback) (vapi_ctx_t ctx,
						     void *callback_ctx,
						     vapi_msg_id_t id,
						     void *msg),
			   void *callback_ctx)
...
void vapi_store_request (vapi_ctx_t ctx, u32 context, bool is_dump,
			 vapi_cb_t callback, void *callback_ctx);
vapi_error_e vapi_send_control_ping (vapi_ctx_t ctx, void *msg, u32 context);
...
void htobevapi_type_msg_header1_t(vapi_type_msg_header1_t *h)
{
  h->_vl_msg_id = htobe16(h->_vl_msg_id);
}

void bevapi_type_msg_header1_ttoh(vapi_type_msg_header1_t *h)
{
  h->_vl_msg_id = be16toh(h->_vl_msg_id);
}

void htobevapi_type_msg_header2_t(vapi_type_msg_header2_t *h)
{
  h->_vl_msg_id = htobe16(h->_vl_msg_id);
}

void bevapi_type_msg_header2_ttoh(vapi_type_msg_header2_t *h)
{
  h->_vl_msg_id = be16toh(h->_vl_msg_id);
}
