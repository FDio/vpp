...
vapi_sock_send (vapi_ctx_t ctx, u8 *msg)
{
  ...
  n = send (ctx->client_socket.fd, &msgbuf, sizeof (msgbuf), 0);
  if (n < sizeof (msgbuf))
    {
      return VAPI_EAGAIN;
    }

  n = send (ctx->client_socket.fd, msg, len, 0);
...
vapi_sock_client_connect (vapi_ctx_t ctx, char *path, const char *name,
			  int ctx_quota, int input_queue_size, bool keepalive)
{
  clib_error_t *error;
  clib_socket_t *sock;
...
vapi_sock_client_disconnect (vapi_ctx_t ctx)
{
  ...
  timeout = clib_time_now (&ctx->time) + 2;

  do
    {
      rv = vapi_sock_client_send_disconnect (ctx);
    }
  while (clib_time_now (&ctx->time) < timeout && rv != VAPI_OK);
  ...
  while (1)
      ...
      if (clib_time_now (&ctx->time) >= timeout)
	{
      ...
      if (vapi_sock_recv_int (ctx, &msg, 0) != VAPI_OK)
	continue;
...
{
  int rv = vapi_sock_send (ctx, msg1);

  if (rv != VAPI_OK)
    return VAPI_EAGAIN;

  rv = vapi_sock_send (ctx, msg2);
...
{
  clib_warning ("vapi_get_swap_To_ho_func id %u", id);
  assert (id < __vapi_metadata.count);
