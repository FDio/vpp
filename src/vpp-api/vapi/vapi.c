		     ...
		     int input_queue_size)
{
  ...
  mp = vl_msg_api_alloc_as_if_client (sizeof (vl_api_memclnt_create_t));
	      ...
	      vapi_mode_e mode, bool handle_keepalives)
{
  ...
  void *tmp = realloc (ctx->requests, size); // TODO: Linux heap
  if (!tmp)
    {
      return VAPI_ENOMEM;
    }
...
vapi_disconnect (vapi_ctx_t ctx)
{
  ...
  vl_input_queue = ctx->vl_input_queue;
#if 0
  vapi_msg_memclnt_delete *md = vapi_alloc_memclnt_delete (ctx);
  rv = vapi_send (ctx, md);
