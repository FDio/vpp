...
int
vapi_sock_send_int (vapi_ctx_t ctx, u8 *msg)
{
  int n;
  int len = vec_len (msg);
  msgbuf_t msgbuf = {
  ...
  n = write (ctx->client_socket.fd, &msgbuf, sizeof (msgbuf));
  if (n < sizeof (msgbuf))
    {
      return -1;
    }

  n = write (ctx->client_socket.fd, msg, len);
...
static int
vapi_sock_recv_int (vapi_ctx_t ctx, u8 **vec_msg, int wait)
{
  ...
  timeout = clib_time_now (&ctx->time) + wait;
  ...
  while (1)
      ...
      while (current_rx_index < sizeof (*mbp))
	{
	  ...
	  if (n < 0)
	    {
	      if (errno == EAGAIN && clib_time_now (&ctx->time) >= timeout)
		return -1;
      ...
      if (vec_len (sock->rx_buffer) >= data_len + sizeof (*mbp))
	{
	  if (data_len)
	    {
	      vec_validate (*vec_msg, data_len - 1);
	      clib_memcpy (*vec_msg, mbp->data, data_len);
	    }
	  else
	    {
	      vec_validate (*vec_msg, 0);
	      vec_set_len (*vec_msg, 0);
	    }
      ...
      if (clib_time_now (&ctx->time) >= timeout)
	return -1;
    }
  return 0;
}
...
static int
vapi_sock_client_connect (vapi_ctx_t ctx, const char *name, int ctx_quota,
			  int input_queue_size, bool keepalive)
{
  clib_error_t *error;
  clib_socket_t *sock;
  ...
  vapi_sock_send_int (ctx, (void *) mp);
  ...
  while (1)
    {
      ...
      for (i = 0; i < 1000; i++)
	{
	  qstatus = vapi_sock_recv_int (ctx, &msg, 0);
...
vapi_sock_client_disconnect (vapi_ctx_t ctx)
{
  ...
  while (1)
      ...
      if (now >= (begin + 2))
	{
      ...
      if (vapi_sock_recv_int (ctx, &msg, 0) < 0)
	continue;
		       ...
		       bool handle_keepalives, bool use_uds)
{
  ...
  if (use_uds)
    {
      if (vapi_sock_client_connect (ctx, name, 0, response_queue_size, true) <
	  0)
	{
	      ...
	      vapi_mode_e mode, bool handle_keepalives)
{
  return vapi_connect_internal (ctx, name, chroot_prefix,
				...
				mode, handle_keepalives, false);
}

vapi_error_e
vapi_connect_ex (vapi_ctx_t ctx, const char *name, const char *chroot_prefix,
		 int max_outstanding_requests, int response_queue_size,
		 int flags)
{
  return vapi_connect_internal (
    ctx, name, chroot_prefix, max_outstanding_requests, response_queue_size,
    flags & VAPI_FLAG_NON_BLOCK, flags & VAPI_FLAG_HANDLE_KEEPALIVES,
...
{
  int rv = vapi_sock_send_int (ctx, msg1);

  if (rv != vec_len (msg1))
    return VAPI_EAGAIN;
  vec_free (msg1);

  rv = vapi_sock_send_int (ctx, msg2);
...
vapi_sock_recv (vapi_ctx_t ctx, void **msg, size_t *msg_size, u32 time)
{
  ...
  rv = vapi_sock_recv_int (ctx, &data, time);
