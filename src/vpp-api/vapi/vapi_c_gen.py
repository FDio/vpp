...
class CStructType (StructType, CStruct):
    ...
    def get_swap_to_be_func_name(self):
        return "htobe%s" % self.get_c_name()

    def get_swap_to_host_func_name(self):
        return "be%stoh" % self.get_c_name()
...
class CMessage (Message):
    ...
    def get_swap_payload_to_host_func_name(self):
        return "%s_payload_swap_to_host" % self.get_c_name()

    def get_swap_payload_to_be_func_name(self):
        return "%s_payload_swap_to_be" % self.get_c_name()
    ...
    def get_swap_to_host_func_name(self):
        return "%s_swap_to_host" % self.get_c_name()

    def get_swap_to_be_func_name(self):
        return "%s_swap_to_be" % self.get_c_name()
    ...
    alloc_ping = [
        ("  if (vapi_get_request_count(ctx) + 1 >= "
    ...
    send_ping = ["    vapi_send_control_ping(ctx, ping, req_context);"]
