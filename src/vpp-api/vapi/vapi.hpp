...
public:
  virtual ~Common_req (){};
  Connection &get_connection () { return con; };
...
public:
  ...
  vapi_error_e dispatch (const Common_req &limit) { return dispatch (&limit); }
...
template <typename M> class Msg : Common_msg
{
  friend class Connection;
...
public:
  ...
  const Msg<Req> &get_request (void) const { return request; }

  const Msg<Resp> &get_response (void) { return response; }
...
public:
  ...
  Msg<Req> &get_request (void) { return request; }

  using resp_type = typename Msg<Resp>::shm_data_type;

  const Result_set<Resp> &get_result_set (void) const { return result_set; }
...
public:
  ...
  Result_set<resp_type> &get_result_set (void) { return result_set; }
