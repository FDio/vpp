...
extern "C"
{
   ...
   * to version issues, the higher-level APIs are not part of the shared
   * library.
   */
  ...
  /**
   * @brief check if message identified by it's message id is known by the vpp
   * to which the connection is open
   */
   ...
   *
   * @param ctx opaque vapi context, must be allocated using vapi_ctx_alloc
   * first
   ...
   *
   * @param ctx opaque vapi context
   * @param mode type of property to wait for - readability, writability or
   * both
   *
   * @return VAPI_OK on success, other error code on error
   */
