...
class CppMessage (CMessage):
    ...
    def get_swap_to_be_template_instantiation(self):
        return "\n".join([
            "template <> void vapi_swap_to_be<%s>(%s *msg)" %
    ...
    def get_swap_to_host_template_instantiation(self):
        return "\n".join([
            "template <> void vapi_swap_to_host<%s>(%s *msg)" %
            (self.get_c_name(), self.get_c_name()),
            "{",
    ...
    def get_alloc_template_instantiation(self):
        return "\n".join([
            "template <> %s* vapi_alloc<%s%s>"
            "(Vapi_connection *con%s)" %
            (self.get_c_name(), self.get_c_name(),
    ...
    def get_reply_template_name(self):
        if self.request is not None and self.request.is_dump():
            return "Vapi_result_set<%s>" % (self.get_c_name())
        else:
            return "Vapi_msg<%s>" % (self.get_c_name())
    ...
    def get_msg_class_instantiation(self):
        return "template class Vapi_msg<%s>;" % self.get_c_name()

    def get_get_msg_id_t_instantiation(self):
        return "\n".join([
            "template <> vapi_msg_id_t vapi_get_msg_id_t<%s>()" %
            self.get_c_name(),
            "{",
            "  return ::%s; " % self.get_msg_id_name(),
            "}",
            ""
            "template <> vapi_msg_id_t vapi_get_msg_id_t<Vapi_msg<%s>>()" %
            self.get_c_name(),
            "{",
            "  return ::%s; " % self.get_msg_id_name(),
            "}",
...
def gen_json_header(parser, logger, j, io):
    ...
    print("")
    print("#include <vapi_cpp.hpp>")
    print("#include <%s>" % json_to_c_header_name(j))
    print("")
...
def gen_json_code(parser, logger, j, io):
    ...
    orig_stdout = sys.stdout
    sys.stdout = io
    print("#include <%s>" % json_to_cpp_header_name(j))
...
def json_to_cpp_header_name(json_name):
    if json_name.endswith(".json"):
        return "%s.vapi_cpp.hpp" % os.path.splitext(json_name)[0]
    raise Exception("Unexpected json name `%s'!" % json_name)


def json_to_cpp_code_name(json_name):
    if json_name.endswith(".json"):
        return "%s.vapi_cpp.cpp" % os.path.splitext(json_name)[0]
    raise Exception("Unexpected json name `%s'!" % json_name)


def gen_cpp_headers_and_code(parser, logger, prefix):
    ...
    for j in parser.json_files:
        ...
        with open('%s%s' % (prefix, json_to_cpp_code_name(j)), "w") as io:
...
if __name__ == '__main__':
    ...
    gen_cpp_headers_and_code(jsonparser, logger, args.prefix)
