...
def gen_json_header(parser, logger, j, io, add_debug_comments):
...
def gen_cpp_headers(parser, logger, prefix, add_debug_comments=False):
    ...
    for j in parser.json_files:
        with open('%s%s' % (prefix, json_to_cpp_header_name(j)), "w") as io:
            gen_json_header(parser, logger, j, io, add_debug_comments)
...
if __name__ == '__main__':
    ...
    gen_cpp_headers(jsonparser, logger, args.prefix)
