...
#ifndef vpp_api_hpp_included
#define vpp_api_hpp_included
...
typedef enum {
  RESPONSE_NOT_READY,   /**< response not ready yet */
  RESPONSE_READY,       /**< response to request is ready */
  RESPONSE_NO_RESPONSE, /**< no response to request (will never come) */
...
public:
  ...
  {
    std::lock_guard<std::mutex> lock (vapi_ctx_mutex);
    vapi_error_e rv = VAPI_OK;
    while (!requests.empty () || event_count)
      {
        void *shm_data;
        size_t shm_data_size;
        vapi_error_e rv = vapi_recv (vapi_ctx, &shm_data, &shm_data_size);
        if (VAPI_OK != rv)
          {
            return rv;
          }
...
private:
  ...
  {
    std::lock_guard<std::mutex> lock (vapi_ctx_mutex);
    u32 req_context = vapi_gen_req_context (vapi_ctx);
...
private:
  ...
  Vapi_msg (Vapi_connection *con, void *shm_data) : con{ con }
  {
...
public:
  Vapi_req (Vapi_connection *con, Args... args,
      ...
      : Vapi_common_req{ con }, callback{ callback },
        request{ con, vapi_alloc<Req> (con, args...) },
        response{ con, nullptr }
  {
  }
...
public:
  ...
  const Vapi_result_set<typename Vapi_msg_resp::resp_type> *
  get_result_set (void) const
  {
    return &result_set;
  }

  using resp_type = typename Vapi_msg_resp::resp_type;
