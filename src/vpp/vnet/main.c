  ...
  if ((main_heap = clib_mem_init_thread_safe (0, main_heap_size)))
    {
      ...
      /* Figure out which numa socket runs the main thread */
      vlib_get_thread_core_socket (&tmp, main_core);
      __os_socket_index = tmp.socket_id;

      /* and use the main heap as that socket's numa heap */
      clib_mem_set_per_socket_heap (main_heap);
