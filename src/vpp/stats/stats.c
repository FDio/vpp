...
do_combined_per_interface_counters (stats_main_t * sm)
{
  ...
  for (i = 0; i < vec_len (sm->regs_tmp); i++)
      ...
      for (j = 0; j < vec_len (sm->clients_tmp); j++)
	{
	  ...
	  /*
	   * 1 message per api call
	   */
	  mp->count = htonl (1);
	  mp->timestamp = vlib_time_now (sm->vlib_main);
	  ...
	  vp->rx_packets = clib_host_to_net_u64 (v.packets);
	  vp->rx_bytes = clib_host_to_net_u64 (v.bytes);
	  cm = im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX;
	  vlib_get_combined_counter (cm, reg->item, &v);
	  vp->tx_packets = clib_host_to_net_u64 (v.packets);
	  vp->tx_bytes = clib_host_to_net_u64 (v.bytes);
