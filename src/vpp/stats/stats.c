  ...
  (vl_api_vnet_interface_simple_counters_t * mp)
{
  ...
  vpe_client_stats_registration_t **regs = 0;
  stats_main_t *sm = &stats_main;
  ...
  if (mp->enable_disable == 0)
    {
      ...
      if (!rp->stats_registrations & INTERFACE_SIMPLE_COUNTERS) // Client but doesn't want this.
        {
          clib_warning ("pid %d: already disabled for interface simple stats...", mp->pid);
          retval = -2;
          goto reply;
        }
      else
  ...
  if (mp->enable_disable == 0)
    {
      ...
      if (!rp->stats_registrations & INTERFACE_COMBINED_COUNTERS) // Client but doesn't want this.
        {
          clib_warning ("pid %d: already disabled for interface COMBINED stats...", mp->pid);
          retval = -2;
          goto reply;
        }
      else
  ...
  if (mp->enable_disable == 0)
    {
      ...
      if (!rp->stats_registrations & IP4_FIB_COUNTERS) // Client but doesn't want this.
        {
          clib_warning ("pid %d: already disabled for interface ip4 fib...", mp->pid);
          retval = -2;
          goto reply;
        }
      else
  ...
  if (mp->enable_disable == 0)
    {
      ...
      if (!rp->stats_registrations & IP6_FIB_COUNTERS) // Client but doesn't want this.
        {
          clib_warning ("pid %d: already disabled for interface ip6 fib...", mp->pid);
          retval = -2;
          goto reply;
        }
      else
