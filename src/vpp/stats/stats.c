...
again:
  ...
  for (j = 0; j < vec_len (fibs); j++)
      ...
      else
	{
      ...
      vec_reset_length (do_fibs->routes);
...
do_ip6_fibs (stats_main_t * sm)
{
  ...
  routes = sm->do_ip46_fib_statics.ip6routes;
  fibs = sm->do_ip46_fib_statics.fibs;
  results = sm->do_ip46_fib_statics.results;

again:
  vec_reset_length (fibs);
  /* *INDENT-OFF* */
