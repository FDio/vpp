...
static int
ip6_ra_events_enable_disable (rd_cp_main_t * rm, int enable)
{
...
vl_api_ip6_ra_event_t_handler (vl_api_ip6_ra_event_t * mp)
{
  ...
  rm->ra_event = malloc (size);
  clib_memcpy (rm->ra_event, mp, size);

  /* tell process there is an event */
  interrupt_process ();
}
...
ip6_ra_event_t_handler (vl_api_ip6_ra_event_t * mp)
{
  ...
  if (!get_interface_mac_address (sw_if_index, mac))
    {
      printf (stderr, "ERROR GETTING MAC ADDRESS\n");
      return;
    }
  ...
  for (i = 0; i < n_prefixes; i++)
	...
	{
	  clib_warning ("preferred_time > valid_time");
	  continue;
	}
	...
	return;
      ...
      /* *INDENT-OFF* */
      pool_foreach (rm->slaac_address_pool, slaac_address,
      ({
        if (slaac_address->sw_if_index != sw_if_index)
          continue;
        if (slaac_address->address_length != prefix_length)
          continue;
        if (!ip6_prefixes_equal (&slaac_address->address, dst_address,
                                 prefix_length))
          continue;
        u8 address_already_present = 1;
        goto pool_foreach_out;
      }));
      /* *INDENT-ON* */
    pool_foreach_out:
      ...
      if (address_already_present)
	{
      ...
      ip6_address_t addr;
      addr.as_u64[0] = prefix->dst_address[0];
      /* Invert the "u" bit */
...
rd_cp_process (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  ip6_ra_events_enable_disable (rm, 1);
  ...
  while (1)
    {
      ...
      if (rm->ra_event)
	{
	  ip6_ra_event_t_handler (rm->ra_event);
	  free (rm->ra_event);
	  rm->ra_event = 0;
	}
