  ...
  (vl_api_ipsec_tunnel_if_add_del_t * mp, void *handle)
{
  ...
  s = format (s, "tx-table-is %U ", ntohl (mp->tx_table_id));
  ...
  s =
    format (s, "local-crypto-key-len %d ", ntohl (mp->local_crypto_key_len));
  s =
    format (s, "local-crypto-key %U ", format_hex_bytes, mp->local_crypto_key,
	    ntohl (mp->local_crypto_key_len));
  s =
    format (s, "remote-crypto-key-len %d ",
	    ntohl (mp->remote_crypto_key_len));
  s =
    format (s, "remote-crypto-key %U ", format_hex_bytes,
	    mp->remote_crypto_key, ntohl (mp->remote_crypto_key_len));
  s = format (s, "local-integ-key-len %d ", ntohl (mp->local_integ_key_len));
  s =
    format (s, "local-integ-key %U ", format_hex_bytes, mp->local_integ_key,
	    ntohl (mp->local_integ_key_len));
  s =
    format (s, "remote-integ-key-len %d ", ntohl (mp->remote_integ_key_len));
  s =
    format (s, "remote-integ-key %U ", format_hex_bytes, mp->remote_integ_key,
	    ntohl (mp->remote_integ_key_len));
