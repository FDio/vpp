  ...
  switch (mp->behavior)
  {
    ...
    case SR_BEHAVIOR_X:
      s = format (s, "Address: %U\nBehavior: X (Endpoint with Layer-3 cross-connect)"
        "\nIface: %U\nNext hop: %U",
        format_ip6_address, (ip6_address_t *)mp->localsid_addr, format_vnet_sw_if_index_name, vnm, 
        ntohl(mp->sw_if_index), format_ip6_address, (ip6_address_t *)mp->nh_addr);
      s = format (s, (mp->end_psp?"End.PSP: True":"End.PSP: False"));
      break;
    case SR_BEHAVIOR_DX4:
      s = format (s, "Address: %U\nBehavior: DX4 (Endpoint with decapsulation with IPv4 cross-connect)"
        "\nIface: %U\nNext hop: %U",
        format_ip6_address, (ip6_address_t *)mp->localsid_addr, format_vnet_sw_if_index_name, vnm, 
        ntohl(mp->sw_if_index), format_ip4_address, (ip4_address_t *)mp->nh_addr);
      break;
    ...
    case SR_BEHAVIOR_DT6:
      s = format (s, "Address: %U\nBehavior: DT6 (Endpoint with decapsulation and specific IPv6 table lookup)"
        "\nTable: %u",
        format_ip6_address, (ip6_address_t *)mp->localsid_addr, ntohl(mp->fib_table));
      break;
    ...
    default:
      ...
      {
        s = format (s, "Address: %U\n Behavior: %u", 
          format_ip6_address, (ip6_address_t *)mp->localsid_addr, mp->behavior);
      }
      else
  ...
  (vl_api_sr_policy_add_t * mp, void *handle)
{
  ...
  s = format (s, (mp->is_encap?"Behavior: Encapsulation":"Behavior: SRH insertion"));
