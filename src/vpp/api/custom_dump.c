  ...
  (vl_api_session_rule_add_del_t * mp, void *handle)
{
  ...
  if (mp->is_ip4)
    s = format (s, "appns %d %s %U/%d %d %U/%d %d action %u", mp->appns_index,
		proto, format_ip4_address, (ip4_address_t *) mp->lcl_ip,
		mp->lcl_plen, format_ip4_address,
		(ip4_address_t *) mp->rmt_ip, mp->rmt_plen, mp->action_index);
  else
    s = format (s, "appns %d %s %U/%d %d %U/%d %d action %u", mp->appns_index,
		proto, format_ip6_address, (ip6_address_t *) mp->lcl_ip,
		mp->lcl_plen, format_ip6_address,
		(ip6_address_t *) mp->rmt_ip, mp->rmt_plen, mp->action_index);
  FINISH;
}
