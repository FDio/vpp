...
dump_metrics (FILE *stream, stat_segment_cached_pointer_t * cp)
{
  ...
  for (i = 0; i < vec_len (res); i++)
	  ...
	  for (k = 0; k < vec_len (res[i].simple_counter_vec); k++)
	    for (j = 0; j < vec_len (res[i].simple_counter_vec[k]); j++)
	      fformat (stream, "%s{thread=\"%d\",interface=\"%d\"} %lld\n",
		       prom_string(res[i].name), k, j, res[i].simple_counter_vec[k][j]);
	  break;
	...
	case STAT_DIR_TYPE_COUNTER_VECTOR_COMBINED:
	  ...
	  for (k = 0; k < vec_len (res[i].simple_counter_vec); k++)
		...
		fformat (stream, "%s_packets{thread=\"%d\",interface=\"%d\"} %lld\n",
			 prom_string(res[i].name), k, j, res[i].combined_counter_vec[k][j].packets);
		fformat (stream, "%s_bytes{thread=\"%d\",interface=\"%d\"} %lld\n",
			 prom_string(res[i].name), k, j, res[i].combined_counter_vec[k][j].bytes);
	      }
	  break;
	...
	case STAT_DIR_TYPE_SCALAR_POINTER:
	  fformat (stream, "# TYPE %s counter\n", prom_string(res[i].name));
	  fformat (stream, "%s %.2f\n", prom_string(res[i].name), res[i].scalar_value);
	  break;

	default:
	  fformat(stderr, "Unknown value\n");
	  ;
	}
    }
