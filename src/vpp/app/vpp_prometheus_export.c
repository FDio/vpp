...
// (per-thread, per-bin) The min_exp (minimum exponent) is now available in
// stat_segment_data_t as res->min_exp.
static void
print_log2_histogram_metric (FILE *stream, stat_segment_data_t *res)
{
  int n_threads = vec_len (res->log2_histogram_bins);
  int min_exp = res->min_exp;
  if (min_exp == 0)
    {
      fprintf (stderr,
	       "WARNING: Could not parse min_exp from histogram name '%s', "
	       "using min_exp=0\n",
	       res->name);
    }
  ...
  for (int thread = 0; thread < n_threads; ++thread)
	...
	{
	  cumulative += bins[i];
	  sum += bins[i] * (1ULL << (min_exp + i)); // midpoint approx
	  fformat (stream, "%s{le=\"%llu\",thread=\"%d\"} %llu\n",
		   sanitized_name, (1ULL << (min_exp + i)), thread,
		   cumulative);
	}
      // +Inf bucket
...
// Remove static int parse_bucket_boundaries, static void
// print_histogram_metric, and all code that checks for _histogram in the name
