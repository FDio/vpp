 ...
 * 
 * Usage: 
 *   vpp_prometheus_push: usage [v2] [username=<name>] [password=<pass>] [interval=<seconds>] [debug=<level>] url=<url> <patterns> 
 * 
 * Sample usage:
 *  vpp_prometheus_push username=admin password=password url=http://127.0.0.1:3000/api/metrics interfaces
 *  vpp_prometheus_push username=admin password=password interval=10 debug=1 url=https://127.0.0.1:9091/metrics/job/cgnat-dut interfaces
 *
...
char* base64_encode(const char* username, const char* password) {
    BIO *bio, *b64;
    FILE *stream;
    ...
    size_t encoded_size = 4 * ((combined_len + 2) / 3); // Base64 output size calculation
  ...
  switch (res->type)
  {
    ...
    case STAT_DIR_TYPE_COUNTER_VECTOR_COMBINED:
      ...
      for (k = 0; k < vec_len (res->simple_counter_vec); k++)
	      ...
	      {
	        fformat (stream, "%s_packets{thread=\"%d\",interface=\"%d\"} %lld\n",
		              prom_string (res->name), k, j,
      ...
      for (k = 0; k < vec_len (res->simple_counter_vec); k++)
      {
	      for (j = 0; j < vec_len (res->simple_counter_vec[k]); j++)
	      {
		        ...
		        !strncmp (tokens[1], "interfaces", lengths[1])))
	        {
		        ...
		        fformat (stream, "%.*s_%.*s{%.*s=\"%.*s\",index=\"%d\",thread=\"%d\"} %lu\n",
	        ...
	        else if (!strncmp (tokens[1], "mem", lengths[1]))
	        {
		        if (num_tokens == 3)
		        {
		          fformat (stream, "%.*s{heap=\"%.*s\",index=\"%d\",thread=\"%d\"} %lu\n",
		                  lengths[1], tokens[1], lengths[2], tokens[2], j, k,
		                  res->simple_counter_vec[k][j]);
		        }
		        else if (num_tokens == 4)
		        {
		          fformat (stream, "%.*s_%.*s{heap=\"%.*s\",index=\"%d\",thread=\"%d\"} %lu\n",
	        ...
	        else if (!strncmp (tokens[1], "err", lengths[1]))
	        {
		        ...
		        fformat (stream, "%.*s{node=\"%.*s\",error=\"%s\",index=\"%d\",thread=\"%d\"} %lu\n",
      ...
      for (k = 0; k < vec_len (res->combined_counter_vec); k++)
      {
	      for (j = 0; j < vec_len (res->combined_counter_vec[k]); j++)
	      {
		          ...
		          !strncmp (tokens[1], "interfaces", lengths[1]))
	        {
		        sanitize (tokens[1], lengths[1]);
		        sanitize (tokens[3], lengths[3]);
		        fformat (stream, "%.*s_%.*s_packets{interface=\"%.*s\",index=\"%d\", thread=\"%d\"} %lu\n",
			              lengths[1], tokens[1], lengths[3], tokens[3],
			              lengths[2], tokens[2], j, k,
			              res->combined_counter_vec[k][j].packets);
		        fformat (stream, "%.*s_%.*s_bytes{interface=\"%.*s\",index=\"%d\",thread=\"%d\"} %lu\n",
			              lengths[1], tokens[1], lengths[3], tokens[3],
			              lengths[2], tokens[2], j, k,
			              res->combined_counter_vec[k][j].bytes);
	        }
...
int partUrl (const char *url, struct Url *config)
{
...
void handleResponse (const char *buffer, ssize_t bytes_read, struct Config *config, int *first)
{
  if (config->debug > 1)
  {
...
int publish_metrics(struct Config *config, u8 **patterns, u8 v2)
{
  ...
  dump_metrics(data_stream, patterns, v2);
  fclose(data_stream);
  ...
  fprintf(header_stream, "Host: %s:%d\r\n", config->url.hostname, config->url.port);
  ...
  if (connect(sock, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
    fprintf(stderr, "Connection to %s:%d failed\n", config->url.ipv4, config->url.port);
    close(sock);
    free(data);
    free(header);
    return -1;
  ...
  if( config->url.https ) 
  {
    ...
    if( config->debug > 0 ) { fprintf (stderr, "Sending header %zu\n", header_size); fprintf (stderr, "%s", header); }
    SSL_write(ssl, header, header_size);
    if( config->debug > 1 ) { fprintf (stderr, "Sending data %zu\n", data_size); fprintf (stderr, "%s", data); }
    ...
    if( config->debug > 1 ) { fprintf (stderr, "Sending header %zu\n", header_size); fprintf (stderr, "%s", header); }
...
int main (int argc, char **argv)
{
  ...
  char *usage =
    "%s: usage [v2] [username=<name>] [password=<pass>] [interval=<seconds>] [debug=<level>] url=<url> <patterns> \n";
  int rv;
  u8 v2 = 0;
  ...
  do {
    ...
    if (rv)
    {
      ...
      publish_metrics(&config, patterns, v2);
