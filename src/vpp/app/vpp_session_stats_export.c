...
#define SESSION_STATS_ENTRY_SIZE 384
 ...
 * from session_stats.h (384 bytes total, schema v6)
 */
typedef struct
{
  ...
  /* TCP ECN/CWR metrics - offset 240 */
  u32 tcp_ecn_ect_packets;
  u32 tcp_ecn_ce_packets;
  u32 tcp_ece_packets;
  u32 tcp_cwr_packets;

  /* TCP events per direction - offset 256 */
  ...
  /* TCP sequence window - offset 296 */
  u32 tcp_last_seq_forward;
  u32 tcp_last_ack_forward;
  u32 tcp_last_seq_reverse;
  u32 tcp_last_ack_reverse;

  /* Custom data metadata - offset 312 */
  u8 custom_data_flags;
  u8 reserved_custom[7];

  /* Custom data - offset 320, size 16 */
  u64 custom_api_data;
  u64 custom_reserved;

  /* Padding to 384 bytes - offset 336, size 48 */
  u8 reserved_pad[48];
...
/* Maximum length for custom API data name (must match session_stats.h) */
#define CUSTOM_API_DATA_NAME_MAX_LEN 20
...
typedef struct
{
  ...
  char
    custom_api_data_name[CUSTOM_API_DATA_NAME_MAX_LEN + 1]; /* Custom API data name from schema */
  u8 schema_parsed;					    /* Whether we've parsed the schema */
} session_exporter_main_t;

static session_exporter_main_t exporter_main;

/*
 * Extract custom_api_data_name from JSON schema string.
 * Simple JSON parser that looks for "custom_api_data_name":"value"
 * Returns extracted name in out_name buffer, or default if not found.
 */
static void
extract_custom_api_data_name_from_schema (const char *schema, size_t schema_len, char *out_name,
					  size_t out_name_len)
{
  const char *key = "\"custom_api_data_name\":\"";
  size_t key_len = strlen (key);

  /* Default name if not found */
  strncpy (out_name, CUSTOM_API_DATA_DEFAULT_NAME, out_name_len - 1);
  ...
  for (size_t i = 0; i + key_len <= schema_len; i++)
    {
      if (strncmp (schema + i, key, key_len) == 0)
	{
	  found = schema + i + key_len;
	  break;
	}
    }
  ...
  /* Extract the value (up to closing quote) */
  size_t value_len = 0;
  const char *end = found;
  while (*end && *end != '"' && value_len < out_name_len - 1)
    {
      out_name[value_len++] = *end++;
    }
...
/*
 * Read schema string from ring buffer and extract custom_api_data_name.
 * Must be called within a stat_segment_access_start/end block.
 */
static void
read_schema_custom_name (stat_client_main_t *shm, vlib_stats_entry_t *ep, ring_metadata_t *metadata)
{
  session_exporter_main_t *em = &exporter_main;

  if (em->schema_parsed || metadata->schema_size == 0)
    return;
  ...
  /* Allocate temporary buffer for schema string */
  char *schema_buf = malloc (schema_size + 1);
  if (!schema_buf)
    return;

  memcpy (schema_buf, schema_ptr, schema_size);
  schema_buf[schema_size] = '\0';

  /* Extract custom_api_data_name from schema */
  extract_custom_api_data_name_from_schema (schema_buf, schema_size, em->custom_api_data_name,
					    sizeof (em->custom_api_data_name));
...
consume_ring_buffer_entries (stat_client_main_t *shm)
{
  ...
  /* Parse schema once to extract custom_api_data_name */
  if (!em->schema_parsed)
    read_schema_custom_name (shm, ep, &metadata);
...
dump_session_metrics (FILE *stream, stat_client_main_t *shm)
{
  ...
  fprintf (stream,
	   "# HELP sfdp_session_tcp_partial_overlap_forward TCP partial overlap events forward\n");
  fprintf (stream, "# TYPE sfdp_session_tcp_partial_overlap_forward counter\n");
  fprintf (stream,
	   "# HELP sfdp_session_tcp_partial_overlap_reverse TCP partial overlap events reverse\n");
  fprintf (stream, "# TYPE sfdp_session_tcp_partial_overlap_reverse counter\n");
  fprintf (stream,
	   "# HELP sfdp_session_tcp_out_of_order_forward TCP out-of-order events forward\n");
  fprintf (stream, "# TYPE sfdp_session_tcp_out_of_order_forward counter\n");
  fprintf (stream,
	   "# HELP sfdp_session_tcp_out_of_order_reverse TCP out-of-order events reverse\n");
  fprintf (stream, "# TYPE sfdp_session_tcp_out_of_order_reverse counter\n");

  /* ECN/CWR metrics */
  fprintf (stream,
	   "# HELP sfdp_session_tcp_ecn_ect_packets Packets with ECN ECT(0) or ECT(1) marks\n");
  fprintf (stream, "# TYPE sfdp_session_tcp_ecn_ect_packets counter\n");
  fprintf (stream,
	   "# HELP sfdp_session_tcp_ecn_ce_packets Packets with ECN CE (Congestion Experienced)\n");
  fprintf (stream, "# TYPE sfdp_session_tcp_ecn_ce_packets counter\n");
  fprintf (stream, "# HELP sfdp_session_tcp_ece_packets TCP packets with ECE flag\n");
  fprintf (stream, "# TYPE sfdp_session_tcp_ece_packets counter\n");
  fprintf (stream, "# HELP sfdp_session_tcp_cwr_packets TCP packets with CWR flag\n");
  fprintf (stream, "# TYPE sfdp_session_tcp_cwr_packets counter\n");
  ...
  for (u32 i = 0; i < vec_len (em->sessions); i++)
    {
      ...
      /* Custom API data as string label */
      const char *custom_name =
	em->custom_api_data_name[0] ? em->custom_api_data_name : CUSTOM_API_DATA_DEFAULT_NAME;
      char custom_value_str[32];
      snprintf (custom_value_str, sizeof (custom_value_str), "0x%lx", s->custom_api_data);

/* Common label set - includes custom data label */
...
#define EMIT_METRIC(metric_name, fmt, value)                                                       \
  ...
  while (0)
      ...
      if (s->proto == 6) /* TCP */
	{
	  ...
	  EMIT_METRIC ("sfdp_session_tcp_partial_overlap_forward", "%u",
		       s->tcp_partial_overlap_events_fwd);
	  EMIT_METRIC ("sfdp_session_tcp_partial_overlap_reverse", "%u",
		       s->tcp_partial_overlap_events_rev);
	  EMIT_METRIC ("sfdp_session_tcp_out_of_order_forward", "%u",
		       s->tcp_out_of_order_events_fwd);
	  EMIT_METRIC ("sfdp_session_tcp_out_of_order_reverse", "%u",
		       s->tcp_out_of_order_events_rev);

	  /* ECN/CWR metrics */
	  EMIT_METRIC ("sfdp_session_tcp_ecn_ect_packets", "%u", s->tcp_ecn_ect_packets);
	  EMIT_METRIC ("sfdp_session_tcp_ecn_ce_packets", "%u", s->tcp_ecn_ce_packets);
	  EMIT_METRIC ("sfdp_session_tcp_ece_packets", "%u", s->tcp_ece_packets);
	  EMIT_METRIC ("sfdp_session_tcp_cwr_packets", "%u", s->tcp_cwr_packets);
	}
	...
	fprintf (stream,
		 ...
		 s->fwd_dst_port, ip_version, custom_name, custom_value_str, export_reason,
		 s->session_index);
      else
	fprintf (stream,
