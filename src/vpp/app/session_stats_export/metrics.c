...
dump_session_metrics (FILE *stream, stat_client_main_t *shm)
{
  ...
  for (u32 i = 0; i < vec_len (em->sessions); i++)
	...
	em->opaque_label ? (const char *) em->opaque_label : OPAQUE_LABEL_DEFAULT_NAME;
      ...
      emit_metric (stream, em, "sfdp_session_packets_forward", base_labels, "%lu",
		   s->packets_forward);
      emit_metric (stream, em, "sfdp_session_packets_reverse", base_labels, "%lu",
		   s->packets_reverse);
      emit_metric (stream, em, "sfdp_session_bytes_forward", base_labels, "%lu", s->bytes_forward);
      emit_metric (stream, em, "sfdp_session_bytes_reverse", base_labels, "%lu", s->bytes_reverse);
      emit_metric (stream, em, "sfdp_session_duration_seconds", base_labels, "%.6f", s->duration);

      emit_metric (stream, em, "sfdp_session_ttl_min_forward", base_labels, "%u",
		   s->ttl_min_forward);
      emit_metric (stream, em, "sfdp_session_ttl_max_forward", base_labels, "%u",
		   s->ttl_max_forward);
      emit_metric (stream, em, "sfdp_session_ttl_min_reverse", base_labels, "%u",
		   s->ttl_min_reverse);
      emit_metric (stream, em, "sfdp_session_ttl_max_reverse", base_labels, "%u",
		   s->ttl_max_reverse);
      emit_metric (stream, em, "sfdp_session_ttl_mean_forward", base_labels, "%.2f",
		   s->ttl_mean_forward);
      emit_metric (stream, em, "sfdp_session_ttl_mean_reverse", base_labels, "%.2f",
		   s->ttl_mean_reverse);
      emit_metric (stream, em, "sfdp_session_ttl_stddev_forward", base_labels, "%.2f",
		   s->ttl_stddev_forward);
      emit_metric (stream, em, "sfdp_session_ttl_stddev_reverse", base_labels, "%.2f",
		   s->ttl_stddev_reverse);

      emit_metric (stream, em, "sfdp_session_rtt_mean_forward_seconds", base_labels, "%.6f",
		   s->rtt_mean_forward);
      emit_metric (stream, em, "sfdp_session_rtt_mean_reverse_seconds", base_labels, "%.6f",
		   s->rtt_mean_reverse);
      emit_metric (stream, em, "sfdp_session_rtt_stddev_forward_seconds", base_labels, "%.6f",
		   s->rtt_stddev_forward);
      emit_metric (stream, em, "sfdp_session_rtt_stddev_reverse_seconds", base_labels, "%.6f",
		   s->rtt_stddev_reverse);

      if (s->proto == 6)
	{
	  emit_metric (stream, em, "sfdp_session_tcp_mss", base_labels, "%u", s->tcp_mss);
	  emit_metric (stream, em, "sfdp_session_tcp_handshake_complete", base_labels, "%u",
		       s->tcp_handshake_complete);
	  emit_metric (stream, em, "sfdp_session_tcp_syn_packets", base_labels, "%u",
		       s->tcp_syn_packets);
	  emit_metric (stream, em, "sfdp_session_tcp_fin_packets", base_labels, "%u",
		       s->tcp_fin_packets);
	  emit_metric (stream, em, "sfdp_session_tcp_rst_packets", base_labels, "%u",
		       s->tcp_rst_packets);
	  emit_metric (stream, em, "sfdp_session_tcp_retransmissions_forward", base_labels, "%u",
		       s->tcp_retransmissions_fwd);
	  emit_metric (stream, em, "sfdp_session_tcp_retransmissions_reverse", base_labels, "%u",
		       s->tcp_retransmissions_rev);
	  emit_metric (stream, em, "sfdp_session_tcp_zero_window_forward", base_labels, "%u",
		       s->tcp_zero_window_events_fwd);
	  emit_metric (stream, em, "sfdp_session_tcp_zero_window_reverse", base_labels, "%u",
		       s->tcp_zero_window_events_rev);
	  emit_metric (stream, em, "sfdp_session_tcp_dupack_forward", base_labels, "%u",
		       s->tcp_dupack_events_fwd);
	  emit_metric (stream, em, "sfdp_session_tcp_dupack_reverse", base_labels, "%u",
		       s->tcp_dupack_events_rev);
	  emit_metric (stream, em, "sfdp_session_tcp_partial_overlap_forward", base_labels, "%u",
		       s->tcp_partial_overlap_events_fwd);
	  emit_metric (stream, em, "sfdp_session_tcp_partial_overlap_reverse", base_labels, "%u",
		       s->tcp_partial_overlap_events_rev);
	  emit_metric (stream, em, "sfdp_session_tcp_out_of_order_forward", base_labels, "%u",
		       s->tcp_out_of_order_events_fwd);
	  emit_metric (stream, em, "sfdp_session_tcp_out_of_order_reverse", base_labels, "%u",
		       s->tcp_out_of_order_events_rev);
	  emit_metric (stream, em, "sfdp_session_tcp_ecn_ect_packets", base_labels, "%u",
		       s->tcp_ecn_ect_packets);
	  emit_metric (stream, em, "sfdp_session_tcp_ecn_ce_packets", base_labels, "%u",
		       s->tcp_ecn_ce_packets);
	  emit_metric (stream, em, "sfdp_session_tcp_ece_packets", base_labels, "%u",
		       s->tcp_ece_packets);
	  emit_metric (stream, em, "sfdp_session_tcp_cwr_packets", base_labels, "%u",
		       s->tcp_cwr_packets);
	  emit_metric (stream, em, "sfdp_session_tcp_last_seq_forward", base_labels, "%u",
		       s->tcp_last_seq_forward);
	  emit_metric (stream, em, "sfdp_session_tcp_last_ack_forward", base_labels, "%u",
		       s->tcp_last_ack_forward);
	  emit_metric (stream, em, "sfdp_session_tcp_last_seq_reverse", base_labels, "%u",
		       s->tcp_last_seq_reverse);
	  emit_metric (stream, em, "sfdp_session_tcp_last_ack_reverse", base_labels, "%u",
		       s->tcp_last_ack_reverse);
	}
