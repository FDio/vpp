...
build_base_labels (char *buf, size_t buflen, const sfdp_session_stats_ring_entry_t *s,
		   ...
		   const char *ip_version, u8 include_custom, const char *custom_name,
  ...
  if (include_custom)
    {
      snprintf (buf, buflen,
		...
		ip_version, custom_name, custom_value);
    }
  else
    {
      snprintf (buf, buflen,
...
dump_session_metrics (FILE *stream, stat_client_main_t *shm)
{
  ...
  for (u32 i = 0; i < vec_len (em->sessions); i++)
	...
	continue;
      ...
      const char *custom_name = em->schema.custom_api_data_name[0] ?
				  em->schema.custom_api_data_name :
				  CUSTOM_API_DATA_DEFAULT_NAME;
      char custom_value_str[32] = { 0 };
      u8 include_custom = 0;
      if (em->schema.has_custom_label)
	{
	  if (!em->schema.has_custom_flags ||
	      (s->custom_data_flags & SFDP_SESSION_STATS_CUSTOM_FLAG_API_DATA))
	    {
	      snprintf (custom_value_str, sizeof (custom_value_str), "%" PRIu64,
			s->custom_data.api_data);
	      include_custom = 1;
	    }
	}

      build_base_labels (base_labels, sizeof (base_labels), s, proto, src_ip, dst_ip, ip_version,
			 include_custom, custom_name, custom_value_str);
