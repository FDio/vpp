...
dump_session_metrics (FILE *stream, stat_client_main_t *shm)
{
  ...
  emit_help_type (stream, "sfdp_session_rtt_stddev_reverse_seconds", "RTT stddev reverse direction",
		  "gauge");
  emit_help_type (stream, "sfdp_session_rtt_min_forward_seconds", "Minimum RTT forward direction",
		  "gauge");
  emit_help_type (stream, "sfdp_session_rtt_min_reverse_seconds", "Minimum RTT reverse direction",
		  "gauge");
  emit_help_type (stream, "sfdp_session_rtt_max_forward_seconds", "Maximum RTT forward direction",
		  "gauge");
  emit_help_type (stream, "sfdp_session_rtt_max_reverse_seconds", "Maximum RTT reverse direction",
		  "gauge");
  emit_help_type (stream, "sfdp_session_syn_rtt_seconds",
		  "SYN->SYN-ACK handshake RTT (0 if not observed)", "gauge");

  emit_help_type (stream, "sfdp_session_tcp_mss_forward",
		  "MSS advertised by the forward (initiator) endpoint", "gauge");
  emit_help_type (stream, "sfdp_session_tcp_mss_reverse",
		  "MSS advertised by the reverse (responder) endpoint", "gauge");
  emit_help_type (stream, "sfdp_session_tcp_handshake_complete", "TCP handshake completed (0 or 1)",
		  "gauge");
  emit_help_type (stream, "sfdp_session_tcp_ts_negotiated",
		  "TCP Timestamps option negotiated in both directions (0 or 1)", "gauge");
  ...
  for (u32 i = 0; i < vec_len (em->sessions); i++)
	...
	em->opaque2_label ? (const char *) em->opaque2_label : OPAQUE2_LABEL_DEFAULT_NAME;
      ...
      emit_metric (stream, em, "sfdp_session_rtt_min_forward_seconds", base_labels, timestamp_ms,
		   "%.6f", s->rtt_min_forward);
      emit_metric (stream, em, "sfdp_session_rtt_min_reverse_seconds", base_labels, timestamp_ms,
		   "%.6f", s->rtt_min_reverse);
      emit_metric (stream, em, "sfdp_session_rtt_max_forward_seconds", base_labels, timestamp_ms,
		   "%.6f", s->rtt_max_forward);
      emit_metric (stream, em, "sfdp_session_rtt_max_reverse_seconds", base_labels, timestamp_ms,
		   "%.6f", s->rtt_max_reverse);
      emit_metric (stream, em, "sfdp_session_syn_rtt_seconds", base_labels, timestamp_ms, "%.6f",
		   s->syn_rtt);

      if (s->proto == 6)
	{
	  emit_metric (stream, em, "sfdp_session_tcp_mss_forward", base_labels, timestamp_ms, "%u",
		       s->tcp_mss_fwd);
	  emit_metric (stream, em, "sfdp_session_tcp_mss_reverse", base_labels, timestamp_ms, "%u",
		       s->tcp_mss_rev);
	  emit_metric (stream, em, "sfdp_session_tcp_handshake_complete", base_labels, timestamp_ms,
		       "%u", s->tcp_handshake_complete);
	  emit_metric (stream, em, "sfdp_session_tcp_ts_negotiated", base_labels, timestamp_ms,
		       "%u", s->tcp_ts_negotiated);
