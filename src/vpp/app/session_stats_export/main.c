...
main (int argc, char **argv)
{
  ...
  u32 max_tracked_sessions = DEFAULT_MAX_TRACKED_SESSIONS;
  ...
  while (unformat_check_input (a) != UNFORMAT_END_OF_INPUT)
	...
	em->session_silence_timeout = session_timeout;
      else if (unformat (a, "max-tracked-sessions %u", &max_tracked_sessions))
	em->max_tracked_sessions = max_tracked_sessions;
      ...
      else if (unformat (a, "help"))
	{
	  ...
	  fformat (stderr, "  session-timeout <s> Stop emitting stale sessions after inactivity "
			   "(default: 300s)\n");
	  fformat (stderr,
		   "  max-tracked-sessions <n> Session cache capacity before oldest "
		   "eviction (default: %u)\n",
  ...
  if (em->max_tracked_sessions == 0)
    {
      fformat (stderr, "max-tracked-sessions must be >= 1\n");
      exit (1);
    }
  ...
  fprintf (stderr, "  Max tracked sessions: %u\n", em->max_tracked_sessions);
  if (em->instance)
