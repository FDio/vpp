...
static void
upsert_session (session_exporter_main_t *em, const sfdp_session_stats_ring_entry_t *decoded,
		f64 now)
{
  /* check if session is already tracked in the exporter */
  /* if true, update existing session - otherwise, create new entry */
  ...
  /* drop new sessions if tracking threshold is reached */
...
consume_ring_buffer_entries (stat_client_main_t *shm)
{
  ...
  /* ensure ring buffer schema at least contains set of core required labels */
  if (!em->schema.has_core_labels)
    {
      stat_segment_access_end (&sa, shm);
      return CONSUME_ERR_SCHEMA;
    }
  ...
  /* in this scenario, we drop all currently trakced sessions to adhere to the new format */
  if (schema_changed)
    {
  ...
  for (u32 thread_index = 0; thread_index < ring.config.n_threads; thread_index++)
	...
	{
	  fprintf (stderr, "Ring buffer overrun detected on thread %u, resetting consumer\n",
		   thread_index);
	  state->local_tail = metadata.head;
	  state->last_sequence = metadata.sequence;
	  continue;
      ...
      while (state->local_tail != metadata.head)
	{
	  ...
	  /* decode entry, and check if it matches an existing entry in the exporter cache */
	  sfdp_session_stats_ring_entry_t decoded;
