...
{
  /* clear schema and custom API data name */
  clib_memset (schema, 0, sizeof (*schema));
  strncpy (schema->custom_api_data_name, CUSTOM_API_DATA_DEFAULT_NAME,
	   sizeof (schema->custom_api_data_name) - 1);
  schema->custom_api_data_name[sizeof (schema->custom_api_data_name) - 1] = '\0';
}
...
schema_finalize (ring_schema_t *schema)
{
  ...
  schema->has_custom_label = schema->fields[FIELD_CUSTOM_API_DATA].valid;
  schema->has_custom_flags = schema->fields[FIELD_CUSTOM_DATA_FLAGS].valid;
		   ...
		   const vlib_stats_ring_metadata_t *metadata)
{
  ...
  /* copy custom API data name if it is set */
  cJSON *custom_name = cJSON_GetObjectItemCaseSensitive (root, "custom_api_data_name");
  if (cJSON_IsString (custom_name) && custom_name->valuestring)
    {
      strncpy (schema->custom_api_data_name, custom_name->valuestring,
	       sizeof (schema->custom_api_data_name) - 1);
      schema->custom_api_data_name[sizeof (schema->custom_api_data_name) - 1] = '\0';
    }
