...
vcl_ct_registration_lock_and_lookup (uword mq_addr)
{
  uword *p;
  clib_spinlock_lock (&vcm->ct_registration_lock);
  clib_warning ("looking up 0x%x hash addr %p", mq_addr, vcm->ct_registration_by_mq);
  p = hash_get (vcm->ct_registration_by_mq, mq_addr);
  if (!p)
    return 0;
  clib_warning ("value %u", p[0]);
...
{
  clib_warning ("adding 0x%x value %u hash addr %p", mq_addr, ctr_index, vcm->ct_registration_by_mq);
...
vcl_mq_epoll_add_evfd (svm_msg_q_t *mq)
{
  ...
  clib_warning ("adding mq fd %u to epoll", mq_fd);
