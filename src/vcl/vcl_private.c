...
void
vcl_worker_detached_init_signal_mq (vcl_worker_t *wrk)
{
  ...
  int sig = 1;
  (void) write (wrk->detached_pipefds[1], &sig, 1);
}
...
{
  int buf;
  (void) read (wrk->detached_pipefds[0], &buf, 1);
  (void) write (wrk->detached_pipefds[1], &buf, 1);
}
...
vcl_worker_detach_sessions (vcl_worker_t *wrk)
{
  ...
  vcl_worker_detached_init_signal_mq (wrk);
}
