...
#define VCL_EP_NEXT_APP_TLV(ep_, tlv_)                                        \
  (VCL_EP_APP_TLV_LEN (tlv_) < VCL_EP_APP_TLV_POS (ep_, tlv_) ?               \
	   ((vppcom_endpt_tlv_t *) ((uint8_t *) tlv_ +                              \
			      VCL_EP_APP_TLV_LEN (tlv_))) :                   \
	   0)
  ...
  extern int vppcom_session_read (uint32_t session_handle, void *buf,
				  size_t n);
  ...
  extern int vppcom_epoll_wait (uint32_t vep_handle,
