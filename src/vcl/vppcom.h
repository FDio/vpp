...
#define VPPCOM_ENV_APP_SCOPE_GLOBAL          "VCL_APP_SCOPE_GLOBAL"
  ...
  static inline char *vppcom_proto_str (vppcom_proto_t proto)
  {
    char *proto_str;
	...
	break;
	case VPPCOM_PROTO_UDP:proto_str = "VPPCOM_PROTO_UDP";
	break;
 ...
 */
  static inline const char *vppcom_retval_str (int retval)
  {
    char *st;
...
/* TBD: make these constructor/destructor function */
  ...
  extern int vppcom_session_bind (uint32_t session_index,
				  vppcom_endpt_t * ep);
  ...
  extern int vppcom_session_read (uint32_t session_index, void *buf,
				  size_t n);
