...
static inline void
sync_config_and_reply (vcl_test_server_conn_t * conn, sock_test_cfg_t * rx_cfg)
{
  conn->cfg = *rx_cfg;
  sock_test_buf_alloc (&conn->cfg, 1 /* is_rxbuf */ ,
		       &conn->buf, &conn->buf_size);
...
void
vcl_test_server_process_opts (vcl_test_server_main_t *ssm, int argc, char **argv)
{
  int v, c;
...
int
vcl_test_server_handle_cfg (vcl_test_server_worker_t *wrk, sock_test_cfg_t *rx_cfg,
...
vcl_test_server_worker_loop (void *arg)
{
  ...
  while (1)
      ...
      else if (num_ev == 0)
	{
      ...
      for (i = 0; i < num_ev; i++)
	{
	  conn = &wrk->conn_pool[ssm->wait_events[i].data.u32];
