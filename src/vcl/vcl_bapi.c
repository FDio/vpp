					   ...
					   mp)
{
  ...
  if (mp->n_fds)
	...
	if (ssvm_segment_attach ((char *) mp->segment_name,
	  ...
	  vcm->mqs_epfd = epoll_create (0);
	  if (vcm->mqs_epfd < 0)
	    clib_warning ("epoll_create() returned");
	  vcl_mq_epoll_add_evfd (fds[n_fds]);
	  n_fds++;
        }
...
static void
vl_app_cut_through_registration_add_t_handler (
    vl_api_app_cut_through_registration_add_t * mp)
{
  ...
  ctr->peer_mq = uword_to_pointer (mp->evt_q_address, svm_msg_q_t *);

  if (mp->fd_flags & SESSION_FD_F_MQ_EVENTFD)
    {
      svm_msg_q_set_consumer_eventfd (ctr->mq, fds[0]);
      svm_msg_q_set_producer_eventfd (ctr->peer_mq, fds[1]);
      mqc_index = vcl_mq_epoll_add_evfd (fds[0]);
...
_(APP_CUT_THROUGH_REGISTRATION_ADD, ct_registration_add)	\
