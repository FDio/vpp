  ...
  if (nsid_len)
    {
      ...
      _clib_memcpy (bmp->namespace_id, vcm->cfg.namespace_id, nsid_len);
...
vppcom_send_connect_sock (vcl_session_t * session)
{
  ...
  _clib_memcpy (cmp->ip, &session->transport.rmt_ip, sizeof (cmp->ip));
  cmp->port = session->transport.rmt_port;
  cmp->proto = session->session_type;
  _clib_memcpy (cmp->options, session->options, sizeof (cmp->options));
...
vppcom_send_bind_sock (vcl_session_t * session)
{
  ...
  _clib_memcpy (bmp->ip, &session->transport.lcl_ip, sizeof (bmp->ip));
  bmp->port = session->transport.lcl_port;
  bmp->proto = session->session_type;
  _clib_memcpy (bmp->options, session->options, sizeof (bmp->options));
  vl_msg_api_send_shmem (vcm->vl_input_queue, (u8 *) & bmp);
}
