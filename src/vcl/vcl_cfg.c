	  ...
	  if (unformat (line_input, "heapsize %s", &chroot_path))
	    {
	      ...
	      VCFG_DBG (0, "VCL<%d>: configured heapsize %s, actual heapsize %ld"
		...
		vec_free (vcl_cfg->vpp_api_filename);
	      ...
	      VCFG_DBG (0, "VCL<%d>: configured api-prefix (%s) and api filename"
		...
		{
		  fprintf (stderr, "VCL<%d>: ERROR: configured vpp-api-q-length "
	    ...
	    {
	      VCFG_DBG (0, "VCL<%d>: configured segment_baseva 0x%lx", getpid (),
		    vcl_cfg->segment_baseva);
	    }
