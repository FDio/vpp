  ...
  if (argv == NULL)
    {
      VCFG_DBG (0, "VCL<%d>: calloc failed, using default heapsize %lu"
		" (0x%lx)", getpid (), (unsigned long)vcl_cfg->heapsize, (unsigned long)vcl_cfg->heapsize);
      goto defaulted;
    }
	  ...
	  if (size == 0)
	    {
	      VCFG_DBG (0, "VCL<%d>: parse error '%s %s', using default "
			...
			argv[i + 1], (unsigned long)vcl_cfg->heapsize, (unsigned long)vcl_cfg->heapsize);
	      goto defaulted;
	    }
...
defaulted:
  ...
  VCFG_DBG (0, "VCL<%d>: allocated VCL heap = %p, size %lu (0x%lx)",
	    getpid (), heap, (unsigned long)vcl_cfg->heapsize, (unsigned long)vcl_cfg->heapsize);
}
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      if (vc_cfg_input)
	{
	  if (unformat (line_input, "heapsize %U", unformat_memory_size,
	    ...
	    {
	      VCFG_DBG (0, "VCL<%d>: configured namespace_secret %llu (0x%llx)",
