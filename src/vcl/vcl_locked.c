...
{
  clib_rwlock_writer_lock (&vlsm->shared_data_lock);
  pool_put_index (vlsm->shared_data_pool, shd_index);
  clib_rwlock_writer_unlock (&vlsm->shared_data_lock);
}
...
static void
vls_worker_free (u32 wrk_index)
{
  pool_put_index (vlsm->workers, wrk_index);
}
...
static void
vls_listener_wrk_set (vcl_locked_session_t * vls, u8 is_active)
{
  vls_shared_data_t *vls_shd;
  ...
  clib_spinlock_lock (&vls_shd->lock);
  clib_bitmap_set (vls_shd->listeners, vls->worker_index, is_active);
  clib_spinlock_unlock (&vls_shd->lock);
...
static u8
vls_listener_wrk_is_active (vcl_locked_session_t * vls)
{
  vls_shared_data_t *vls_shd;
  u8 is_set;
  ...
  clib_spinlock_lock (&vls_shd->lock);
  is_set = clib_bitmap_get (vls_shd->listeners, vls->worker_index);
  clib_spinlock_unlock (&vls_shd->lock);
...
vls_listener_wrk_start_listen (vcl_locked_session_t * vls, u32 wrk_index)
{
  ...
  vls_listener_wrk_set (vls, 1 /* is_active */ );
}
...
vls_listener_wrk_stop_listen (vcl_locked_session_t * vls, u32 wrk_index)
{
  ...
  vls_listener_wrk_set (vls, 0 /* is_active */ );
}
...
vls_unshare_session (vcl_locked_session_t * vls, vcl_worker_t * wrk)
{
  ...
  for (i = 0; i < vec_len (vls_shd->workers_subscribed); i++)
    {
      if (vls_shd->workers_subscribed[i] != wrk->wrk_index)
	continue;
      ...
      if (s->rx_fifo)
	{
      ...
      vec_del1 (vls_shd->workers_subscribed, i);
  ...
  if (vec_len (vls_shd->workers_subscribed))
    {
      vls_shd->owner_wrk_index = vls_shd->workers_subscribed[0];
      vec_del1 (vls_shd->workers_subscribed, 0);
...
//void
//vls_share_vcl_session (vls_worker_t *vls_parent_wrk, vcl_worker_t * wrk,
//                       vcl_session_t * s)
//{
//  vls_shared_data_t *vls_shd;
//  vcl_locked_session_t *vls;
...
//  if (vls->shared_data_index != ~0)
//    {
//      vls_shd = pool_elt_at_index (vlsm->shared_data_pool,
//                                   vls->shared_data_index);
//    }
//  else
//    {
//      pool_get_zero (vlsm->shared_data_pool, vls_shd);
//      vls_shd->owner_wrk = vls_parent_wrk->wrk_index;
//      vls->shared_data_index = vls_shd - vlsm->shared_data_pool;
//    }
//
//  clib_spinlock_lock (vls_shd->lock);
//
//  vec_add1 (vls_shd->workers_subscribed, wrk->wrk_index);
                   ...
                   vls_worker_t *vls_parent_wrk, vcl_worker_t *vcl_wrk)
{
  ...
  if (vls->shared_data_index != ~0)
  ...
  else
    {
      ...
      parent_vls->shared_data_index = vls->shared_data_index;
    }
...
//    vls_share_vcl_session (vls_parent_wrk, wrk, s);
//  }));
//  /* *INDENT-ON* */
...
vls_mp_checks (vcl_locked_session_t * vls, int is_add)
{
  ...
  switch (s->session_state)
	...
	break;
      ...
      if (!vls_listener_wrk_is_active (vls))
	vls_listener_wrk_stop_listen (vls, vls->worker_index);
      break;
    default:
      break;
...
vls_cleanup_forked_child (vcl_worker_t * wrk, vcl_worker_t * child_wrk)
{
  ...
  vls_worker_free (child_wrk->wrk_index);
...
vls_app_exit (void)
{
  ...
  vls_worker_free (vls_get_worker_index ());
