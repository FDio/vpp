	    ...
	    vppcom_endpt_t * ep)
{
  ...
  vls = vls_get_and_lock (vlsh);
  rv = vppcom_session_sendto (vls_to_sh (vls), buf, buflen, flags, ep);
  vls_unlock (vls);
  return rv;
}
	      ...
	      vppcom_endpt_t * ep)
{
  ...
  vls = vls_get_and_lock (vlsh);
  rv = vppcom_session_recvfrom (vls_to_sh (vls), buffer, buflen, flags, ep);
  vls_unlock (vls);
  return rv;
}
               ...
               struct epoll_event *event)
{
  ...
  vls = vls_get_and_lock (vlsh);
  rv = vppcom_epoll_ctl (vls_to_sh (ep_vls), op, vls_to_sh (vls), event);
  vls_unlock (vls);
  vls_unlock (ep_vls);
  return rv;
}
