...
u8
vls_is_shared_by (vcl_locked_session_t * vls, u32 wrk_index)
{
  int i;
...
vls_cleanup_vcl_worker (vcl_worker_t *wrk)
{
  ...
  /* *INDENT-OFF* */
  pool_foreach (s, wrk->sessions, ({
    vls = vls_get_and_lock (vls_si_to_vlsh (s->session_index));
    if (vls && (is_current || vls_is_shared_by (vls, current_wrk)))
...
vls_app_exit (void)
{
//  pthread_mutex_destroy (&vls_mt_mq_mlock);
//  pthread_mutex_destroy (&vls_mt_create_mlock);
