...
{
  clib_spinlock_lock (&vls->migrate_lock);
  uword *p = hash_get (vls->worker_index_to_session_index, wrk_index);
  clib_spinlock_unlock (&vls->migrate_lock);
...
vls_alloc (vcl_session_handle_t sh)
{
  ...
  clib_spinlock_init (&vls->migrate_lock);
...
vls_free (vcl_locked_session_t * vls)
{
  ...
  clib_spinlock_free (&vls->migrate_lock);
