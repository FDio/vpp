...
vls_mt_add (void)
{
  ...
  /* If multi-thread workers are supported, for each new thread a new vcl
   * worker is registered with vpp. Otherwise, all threads use the same
   * vcl worker, so the vcl wrk's thread local variable is updated. */
  if (vls_mt_wrk_supported ())
