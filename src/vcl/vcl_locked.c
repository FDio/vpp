...
{
  clib_rwlock_t lock; /** valid for multithread workers */
...
static inline void
vls_worker_wlock (vls_worker_t *wrk)
{
  if (vls_mt_wrk_supported ())
    clib_rwlock_writer_lock (&wrk->lock);
}

static inline void
vls_worker_wunlock (vls_worker_t *wrk)
{
  if (vls_mt_wrk_supported ())
    clib_rwlock_writer_unlock (&wrk->lock);
}

static inline void
vls_worker_rlock (vls_worker_t *wrk)
{
  if (vls_mt_wrk_supported ())
    clib_rwlock_reader_lock (&wrk->lock);
}

static inline void
vls_worker_runlock (vls_worker_t *wrk)
{
  if (vls_mt_wrk_supported ())
    clib_rwlock_reader_unlock (&wrk->lock);
}
...
static void
vls_sh_to_vlsh_table_set (vls_worker_t *wrk, vcl_session_handle_t sh, u32 vlsh)
{
  vls_worker_wlock (wrk);
  hash_set (wrk->session_handle_to_vlsh_table, sh, vlsh);
  vls_worker_wunlock (wrk);
}

static void
vls_sh_to_vlsh_table_unset (vls_worker_t *wrk, vcl_session_handle_t sh)
{
  vls_worker_wlock (wrk);
...
vls_alloc (vcl_session_handle_t sh)
{
  ...
  vls_sh_to_vlsh_table_set (wrk, sh, vls->vls_index);
  if (vls_mt_wrk_supported ())
    {
...
vls_mt_session_migrate (vcl_locked_session_t *vls)
{
  ...
  vls_sh_to_vlsh_table_set (vls_wrk, vcl_session_handle (session),
			    vls->vls_index);
  return vls;
...
vls_session_cleanup_rpc_handler (void *args)
{
  ...
  vppcom_session_close (sh);
  vls_sh_to_vlsh_table_unset (vls_wrk, sh);
}
