...
{
  return vec_len (vls->shared_workers);
}
...
vls_unshare_session (vcl_locked_session_t *vls)
{
  ...
  if (vls->worker_index == wrk->wrk_index)
    {
      s = vcl_session_get (wrk, vls->session_index);
      if (!s || vec_len (vls->shared_workers) == 0)
	clib_warning ("?!?!HUH?!?!?");
      vls->worker_index = vls->shared_workers[0];
      vec_del1 (vls->shared_workers, 0);
...
vls_share_vcl_session (vcl_worker_t *wrk, vcl_session_t *s)
{
  ...
  clib_warning ("pre lock");
  vls = vls_get_w_dlock (vls_session_index_to_vlsh (s->session_index));
  if (!vls)
    return;
  clib_warning ("post lock");
  vec_add1 (vls->shared_workers, wrk->wrk_index);
  if (s->rx_fifo)
    {
...
void
vls_clone_worker (vcl_worker_t * parent_wrk)
{
  vcl_worker_t *wrk = vcl_worker_get_current ();
  vcl_session_t *s;

  clib_warning ("pre dup");
