...
static vcom_main_t vcom_main = {
  .sid_bit_shift = VCOM_SID_BIT_SHIFT_MIN,
  .sid_bit = (1 << VCOM_SID_BIT_SHIFT_MIN),
  .sid_mask = (1 << VCOM_SID_BIT_SHIFT_MIN) - 1,
...
{
  return snprintf (vcom->app_name, VCOM_APP_NAME_MAX, "vcom-%s-%d",
		   app_name, getpid ()) < 0 ? -1 : 0;
}
...
{
  if (PREDICT_FALSE (sid >= vcom->sid_bit))
    return -EMFILE;
  else
    return (sid | vcom->sid_bit);
}

static inline int
vcom_fd_is_sid (int fd)
{
  return ((u32)fd & vcom->sid_bit) ? 1 : 0;
}

static inline u32
vcom_sid_from_fd (int fd)
{
  return (vcom_fd_is_sid (fd) ? ((u32)fd & vcom->sid_mask) :
          INVALID_SESSION_ID);
}
          ...
          if (env_var_str)
            {
              ...
              if (sscanf (env_var_str, "%u", &tmp) != 1)
                fprintf (stderr, "\nLDP<%d>: WARNING: Invalid debug level "
      ...
      if (epfd > 0)
        {
          ...
          if (VCOM_DEBUG > 1)
            clib_warning ("LDP<%d>: fd %d (0x%x): calling %s(): epfd %u (0x%x)",
          ...
          for (i = 0; i < iovcnt; ++i)
            {
              ...
              rv = vppcom_session_read (sid, iov[i].iov_base,
                                        iov[i].iov_len);
              if (rv < 0)
                  break;
              else
      ...
      if (size < 0)
        {
          int errno_val = errno;
          perror (func_str);
          fprintf (stderr, "%s:%d: LDP<%d>: ERROR: fd %d (0x%x): %s() failed! "
                  "rv %ld, errno = %d\n", __func__, __LINE__, getpid (),
                  fd, fd, func_str, size, errno_val);
          errno = errno_val;
        }
      else
	 ...
	 const __sigset_t * __restrict sigmask)
{
  ...
  if (nfds <= vcom->sid_bit)
    {
      func_str = "libc_pselect";

      if (VCOM_DEBUG > 3)
        clib_warning ("LDP<%d>: calling %s(): nfds %d, readfds %p, writefds %p, "
                      "exceptfds %p, timeout %p, sigmask %p", getpid (),
                      func_str, nfds, readfds, writefds, exceptfds, timeout,
                      sigmask);
  ...
  if (PREDICT_FALSE (vcom->sid_bit > FD_SETSIZE/2))
    {
      clib_warning ("LDP<%d>: ERROR: VCOM sid bit (%d) > FD_SETSIZE/2 !",
                    getpid (), vcom->sid_bit, FD_SETSIZE/2);
      errno = EOVERFLOW;
      return -1;
    }
  ...
  if (VCOM_DEBUG > 3)
    {
      if (rv < 0)
        ...
        clib_warning ("LDP<%d>: returning %d (0x%x)",
                      getpid (), rv, rv);
    }
  return rv;  
}
      ...
      switch (addr->sa_family)
          ...
          if (len != sizeof (struct sockaddr_in))
            {
              clib_warning ("LDP<%d>: ERROR: fd %d (0x%x): sid %u (0x%x): Invalid "
                            "AF_INET addr len %u!", getpid (), fd, fd,
                            sid, sid, len);
              errno = EINVAL;
              rv = -1;
              goto done;
        ...
        case AF_INET6:
          if (len != sizeof (struct sockaddr_in6))
            {
              clib_warning ("LDP<%d>: ERROR: fd %d (0x%x): sid %u (0x%x): Invalid "
                            "AF_INET6 addr len %u!", getpid (), fd, fd,
                            sid, sid, len);
              errno = EINVAL;
              rv = -1;
              goto done;
...
int
getsockname (int fd, __SOCKADDR_ARG addr, socklen_t * restrict len)
{
  int rv;
  char *func_str;
  u32 sid = vcom_sid_from_fd (fd);
...
int
getpeername (int fd, __SOCKADDR_ARG addr, socklen_t * restrict len)
{
  int rv;
  char *func_str;
  u32 sid = vcom_sid_from_fd (fd);
      ...
      do
        {
          ...
          if (size < 0)
            {
              clib_warning ("LDP<%d>: ERROR: fd %d (0x%x): %s(): sid %u (0x%x), "
                            "returned %d (%s)!",
                            getpid (), out_fd, out_fd, func_str, sid, sid,
          ...
          if (VCOM_DEBUG > 2)
            clib_warning ("LDP<%d>: fd %d (0x%x): called %s(): sid %u (0x%x), "
                          ...
                          getpid (), out_fd, out_fd, func_str, sid, sid,
                          results, n_bytes_left, bytes_to_read);
          ...
          if (VCOM_DEBUG > 2)
            clib_warning ("LDP<%d>: fd %d (0x%x): calling %s(): sid %u (0x%x), "
                          ...
                          getpid (), out_fd, out_fd, func_str, sid, sid,
                          vcom->io_buffer, nbytes, results, n_bytes_left);
...
ssize_t
recvfrom (int fd, void *restrict buf, size_t n, int flags,
          __SOCKADDR_ARG addr, socklen_t * restrict addr_len)
{
  ssize_t size;
  char *func_str;
  u32 sid = vcom_sid_from_fd (fd);
...
#ifdef USE_GNU
int
sendmmsg (int fd, struct mmsghdr *vmessages,
      ...
      switch (level)
        ...
        case SOL_TCP:
          switch (optname)
            {
            case TCP_KEEPIDLE:
              func_str = "vppcom_session_attr[SET_TCP_KEEPIDLE]";
              rv =
                vppcom_session_attr (sid, VPPCOM_ATTR_SET_TCP_KEEPIDLE,
                                     0, 0);
              break;
            case TCP_KEEPINTVL:
              func_str = "vppcom_session_attr[TCP_KEEPINTVL]";
              rv =
                vppcom_session_attr (sid, VPPCOM_ATTR_SET_TCP_KEEPINTVL,
                                     0, 0);
              break;
            default:
              break;
        ...
        case SOL_IPV6:
          switch (optname)
            {
            case IPV6_V6ONLY:
              func_str = "vppcom_session_attr[SET_V6ONLY]";
        ...
        case SOL_SOCKET:
          switch (optname)
            {
            case SO_KEEPALIVE:
              func_str = "vppcom_session_attr[SET_KEEPALIVE]";
              rv = vppcom_session_attr (sid, VPPCOM_ATTR_SET_KEEPALIVE,
                                        0, 0);
              break;
            case SO_REUSEADDR:
              func_str = "vppcom_session_attr[SET_REUSEADDR]";
              rv = vppcom_session_attr (sid, VPPCOM_ATTR_SET_REUSEADDR,
                                        0, 0);
              break;
            case SO_BROADCAST:
              func_str = "vppcom_session_attr[SET_BROADCAST]";
  ...
  if (sid != INVALID_SESSION_ID)
    {
      ...
      if (VCOM_DEBUG > 1)
        clib_warning ("LDP<%d>: fd %d (0x%x): calling %s(): sid %u (0x%x), n %d",
...
int
accept4 (int fd, __SOCKADDR_ARG addr, socklen_t * restrict addr_len,
         int flags)
{
...
int
accept (int fd, __SOCKADDR_ARG addr, socklen_t * restrict addr_len)
{
      ...
      if (sid != INVALID_SESSION_ID)
        {
          ...
          if (VCOM_DEBUG > 1)
            clib_warning ("LDP<%d>: epfd %d (0x%x): calling %s(): "
                          "vep_idx %d (0x%x), op %d, sid %d (0x%x), event %p",
                  ...
                  int maxevents, int timeout, const sigset_t * sigmask)
{
  ...
  func_str = "vppcom_session_attr[GET_LIBC_EPFD]";
  libc_epfd = vppcom_session_attr (vep_idx, VPPCOM_ATTR_GET_LIBC_EPFD,
                                   0, 0);
...
ppoll (struct pollfd *fds, nfds_t nfds,
       const struct timespec *timeout, const sigset_t * ss)
{
  if ((errno = -vcom_init ()))
    return -1;
