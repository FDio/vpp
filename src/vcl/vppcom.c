...
vcl_epoll_lt_add (vcl_worker_t *wrk, vcl_session_t *s)
{
  ...
  start = vcl_session_get (wrk, wrk->ep_lt_current);
  prev = vcl_session_get (wrk, start->vep.lt_prev);

  prev->vep.lt_next = s->session_index;
  s->vep.lt_prev = prev->session_index;

  s->vep.lt_next = start->session_index;
  start->vep.lt_prev = s->session_index;
}
...
//      vec_reset_length (wrk->ep_level_evts_fl);
  ...
  cur = start = wrk->ep_lt_current;
  do
    {
      s = vcl_session_get (wrk, cur);
      cur = s->vep.lt_next;
	...
	{
	  /* Will be re-added once we get an event */
	  vcl_epoll_lt_del (wrk, s);
	}
    }
  while (cur != start);
