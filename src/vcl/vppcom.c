...
static void
vcl_send_session_reset (vcl_worker_t *wrk, vcl_session_t *s)
{
  app_session_evt_t _app_evt, *app_evt = &_app_evt;
  ...
  mp = (session_reset_msg_t *) app_evt->evt->data;
  memset (mp, 0, sizeof (*mp));
  mp->client_index = wrk->api_client_handle;
  mp->handle = s->vpp_handle;
  app_send_ctrl_evt_to_vpp (mq, app_evt);
...
int
vppcom_session_reset (u32 session_handle)
{
  vcl_worker_t *wrk = vcl_worker_get_current ();
  vcl_session_t *session;
  vcl_session_state_t state;
  ...
  VDBG (1, "session %u [0x%llx]: reset state (%s)", session->session_index,
	vpp_handle, vcl_session_state_str (state));

  if (PREDICT_FALSE (state == VCL_STATE_LISTEN))
    {
      VDBG (0, "ERROR: Cannot reset a listen socket!");
      return VPPCOM_EBADFD;
    }
  ...
  if (state == VCL_STATE_DISCONNECT)
	...
	return VPPCOM_OK;
      ...
      VDBG (1, "session %u [0x%llx]: sending reset...", session->session_index,
	    vpp_handle);
      vcl_send_session_reset (wrk, session);
    }
