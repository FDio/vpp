...
vcl_epoll_wait_clean_lt (vcl_worker_t *wrk, u32 sid)
{
  ...
  for (i = 0; i < vec_len (wrk->level_evts); i++)
    {
      if (wrk->level_evts[i] == sid)
	vec_add1 (indices, i);
    }

  vec_foreach (pos, indices)
    vec_del1 (wrk->level_evts, pos[0]);
  ...
  if (PREDICT_FALSE (!vcl_session_is_open (s) ||
		     (s->flags & VCL_SESSION_F_SHUTDOWN)))
    {
      VDBG (1, "session %u [0x%llx]: is not open! state 0x%x (%s)",
	    s->session_index, s->vpp_handle, s->session_state,
	    vppcom_session_state_str (s->session_state));
      clib_warning ("session %u [0x%llx]: is not open! state 0x%x (%s)",
		    s->session_index, s->vpp_handle, s->session_state,
		    vppcom_session_state_str (s->session_state));
      return vcl_session_closed_error (s);;
    }
...
{
  u32 *sid, add_event, *le = wrk->level_evts_fl;
  u64 evt_data;
  vcl_session_t *s;
		   ...
		   int maxevents, double wait_for_time)
{
  ...
  if (PREDICT_FALSE (do_lt && n_evts < maxevents))
