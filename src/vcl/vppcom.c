...
vppcom_app_create (char *app_name)
{
  ...
  if (vcm->my_client_index == ~0)
      ...
      if (rv)
	{
      ...
      if (!vcm->session_io_thread.active_session_indexes)
				    ...
				    uint8_t rx, void *ptr)
{
  ...
  ioevent = clib_mem_alloc (sizeof (vppcom_session_ioevent_t));	// DAW_TODO: Use a pool instead of thrashing the memory allocator!
  ioevent->user_cb = cb;
  ioevent->user_cb_data = ptr;

  evk.session_index = session_index;
  evk.eid = rx ? VCL_EVENT_IOEVENT_APP_TX : VCL_EVENT_IOEVENT_APP_RX;
