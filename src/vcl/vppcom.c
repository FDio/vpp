  ...
  while ((sz = svm_msg_q_size (mq)))
    {
      ...
      svm_msg_q_sub_batch (mq, wrk->mq_msg_vector + len, sz);
      n_msgs += sz;
    }
  return n_msgs;
}
			      ...
			      u8 peek)
{
  ...
  if (svm_fifo_is_empty_cons (rx_fifo))
      ...
      while (svm_fifo_is_empty_cons (rx_fifo))
	{
	  ...
	  svm_msg_q_wait2 (mq, 0 /* wait while empty */);
		      ...
		      double time_to_wait, u32 * bits_set)
{
  ...
  if (svm_msg_q_is_empty (mq))
	...
	{
	  if (svm_msg_q_timedwait2 (mq, time_to_wait))
	    return 0;
	}
    }
  vcl_mq_dequeue_batch (wrk, mq, ~0);
			  ...
			  double wait_for_time, u32 * num_ev)
{
  ...
  if (svm_msg_q_is_empty (mq))
	...
	{
	  if (svm_msg_q_timedwait2 (mq, wait_for_time / 1e3))
	    return 0;
	}
    }
