		       ...
		       uint32_t flags)
{
  ...
  is_blocking = !VCL_SESS_ATTR_TEST (listen_session->attr,
	                            VCL_SESS_ATTR_NONBLOCK);
  if (svm_msg_q_is_empty (vcm->app_event_queue) && !is_blocking)
    return VPPCOM_EAGAIN;

  wait_type = is_blocking ? SVM_Q_WAIT : SVM_Q_NOWAIT;

  while (1)
    {
      if (svm_msg_q_sub (vcm->app_event_queue, &msg, wait_type, 0))
	return VPPCOM_EAGAIN;
		   ...
		   int maxevents, double wait_for_time)
{
  ...
  if (svm_msg_q_is_empty (vcm->app_event_queue))
	...
	{
	  svm_msg_q_lock (vcm->app_event_queue);
	  if (svm_msg_q_timedwait (vcm->app_event_queue, wait_for_time * 1e6))
	    return VPPCOM_ETIMEDOUT;
	}
