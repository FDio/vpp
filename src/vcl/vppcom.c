...
vcl_session_transport_attr_handler (vcl_worker_t *wrk, void *data)
{
  ...
  vppcom_endpt_t ep = { .ip = addr_buf };
  u32 size = sizeof (ep);

  vppcom_session_attr (vcl_session_handle (s), VPPCOM_ATTR_GET_EXT_ENDPT, &ep,
		       &size);
  if (ep.is_ip4)
    clib_warning ("adding attr %d %U:%u", mp->attr.type, format_ip4_address,
		  ep.ip, clib_net_to_host_u16 (ep.port));
  else
    clib_warning ("adding attr %d [%U]:%u", mp->attr.type, format_ip6_address,
		  ep.ip, clib_net_to_host_u16 (ep.port));
}
