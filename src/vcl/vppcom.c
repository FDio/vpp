			       ...
			       session_connected_msg_t * mp)
{
  ...
  VDBG (0, "session %u [0x%llx] connected", session->session_index,
        session->vpp_handle, session->rx_fifo);
				  ...
				  session_disconnected_msg_t * msg)
{
  ...
  if ((session->flags & VCL_SESSION_F_PENDING_FREE)
      || (session->flags & VCL_SESSION_F_PENDING_DISCONNECT))
    os_panic ();
...
//  VWRN ("deleting 0x%llx is ct %u", msg->handle, vcl_session_is_ct (session));
