...
vppcom_session_create (u8 proto, u8 is_nonblocking)
{
  ...
  if (VPPCOM_DEBUG > 0)
	...
	u32 data[3];
      ...
      ed->data[2] = VCL_SESS_ATTR_TEST(session->attr, VCL_SESS_ATTR_NONBLOCK);
      /* *INDENT-ON* */
    }
  ...
  if (PREDICT_FALSE (retval))
      ...
      if (VPPCOM_DEBUG > 0)
	{
	  if (session)
	    ...
	    clib_warning ("VCL<%d>: sid %u: connect failed! "
			  "returning %d (%s)", getpid (),
			  session_index, rv, vppcom_retval_str (rv));
	}
    }
			      ...
			      u8 peek)
{
  ...
  do
    {
      if (peek)
	n_read = svm_fifo_peek (session->rx_fifo, 0, n, buf);
      else
	n_read = svm_fifo_dequeue_nowait (session->rx_fifo, n, buf);
    }
  while (!VCL_SESS_ATTR_TEST (session->attr, VCL_SESS_ATTR_NONBLOCK)
	 && (n_read <= 0));
  ...
  if (VPPCOM_DEBUG > 2)
	...
	clib_warning ("VCL<%d>: vpp handle 0x%llx, sid %u: read %d bytes "
		      ...
		      session_index, n_read, session->rx_fifo);
      else
...
vppcom_session_write (uint32_t session_index, void *buf, size_t n)
{
  ...
  if ((n_write > 0) && svm_fifo_set_event (session->tx_fifo))
	...
	clib_warning ("VCL<%d>: vpp handle 0x%llx, sid %u: "
		      ...
		      session->vpp_handle, session_index, q, n_write);
    }
  ...
  if (VPPCOM_DEBUG > 2)
	...
	clib_warning ("VCL<%d>: vpp handle 0x%llx, sid %u: "
		      ...
		      session->vpp_handle, session_index, n_write,
		      session->tx_fifo);
    }
done:
  return rv;
