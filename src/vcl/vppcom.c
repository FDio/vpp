			     ...
			     u8 is_flush)
{
  ...
  if (svm_fifo_is_full_prod (tx_fifo))
	...
	{
	  svm_fifo_add_want_ntf (tx_fifo, SVM_FIFO_WANT_NOTIF);
	  if (vcl_session_is_closing (s))
	    return vcl_session_closing_error (s);
	  svm_msg_q_lock (mq);
	  if (svm_msg_q_is_empty (mq))
	       ...
	       vcl_si_set * except_map, double time_to_wait)
{
  ...
  clib_bitmap_foreach (sid, wrk->wr_bitmap, ({
    ...
    if (!rv)
      ...
      svm_fifo_add_want_ntf (session->tx_fifo, SVM_FIFO_WANT_NOTIF);
  }));
		  ...
		  struct epoll_event *event)
{
  ...
  switch (op)
      ...
      if (vep_session->vep.next_sh != ~0)
	{
      ...
      if (session->tx_fifo)
	svm_fifo_add_want_ntf (session->tx_fifo, SVM_FIFO_WANT_NOTIF_IF_FULL);
      ...
      if (session->vep.next_sh != ~0)
	{
      ...
      if (session->tx_fifo)
	svm_fifo_del_want_ntf (session->tx_fifo, SVM_FIFO_NO_NOTIF);
				...
				struct epoll_event *events, u32 * num_ev)
{
  ...
  switch (e->event_type)
	...
	break;
      ...
      session_evt_data = session->vep.ev.data.u64;
      svm_fifo_reset_has_ntf (session->tx_fifo);
      break;
    case SESSION_CTRL_EVT_ACCEPTED:
      session = vcl_session_accepted (wrk,
				      (session_accepted_msg_t *) e->data);
