...
vce_epoll_wait_connect_request_handler_fn (void *arg)
{
  ...
  vce_event_connect_request_t *ecr = (vce_event_connect_request_t *) reg->ev->data;

  /* Add the accepted_session_index to the FIFO */
  clib_spinlock_lock (&vcm->sessions_lockp);
  clib_fifo_add1 (vcm->client_session_index_fifo, ecr->accepted_session_index);
  clib_spinlock_unlock (&vcm->sessions_lockp);

  /* Recycling the event.
   * This is a handler_fn. It is called from within
   * pthread_mutex_lock (&(evt->generator_lock)) which protects
   * the event_fifo. ...safe to add ev_idx back to FIFO */
  ev_idx = reg->ev - vcm->event_thread.vce_event;
  clib_fifo_add1 (vcm->event_thread.event_index_fifo, ev_idx);

  /* Unregister the handler - which re-instates the old handler */
//  vce_unregister_handler (&vcm->event_thread, reg->ev);
}
...
vl_api_accept_session_t_handler (vl_api_accept_session_t * mp)
{
  ...
  ev_idx = (u32) (ev - vcm->event_thread.vce_event);
