...
vcl_session_accepted_handler (vcl_worker_t * wrk, session_accepted_msg_t * mp)
{
  ...
  _clib_memcpy (&session->transport.rmt_ip, mp->ip, sizeof (ip46_address_t));
			       ...
			       session_connected_msg_t * mp)
{
  ...
  session->vpp_handle = mp->handle;
  session->transport.is_ip4 = mp->is_ip4;
  _clib_memcpy (&session->transport.lcl_ip, mp->lcl_ip,
...
vppcom_session_bind (uint32_t session_handle, vppcom_endpt_t * ep)
{
  ...
  if (ep->is_ip4)
    _clib_memcpy (&session->transport.lcl_ip.ip4, ep->ip,
		  sizeof (ip4_address_t));
  else
    _clib_memcpy (&session->transport.lcl_ip.ip6, ep->ip,
		       ...
		       uint32_t flags)
{
  ...
  while (1)
      ...
      if (e->event_type != SESSION_CTRL_EVT_ACCEPTED)
	{
      ...
      _clib_memcpy (&accepted_msg, e->data, sizeof (accepted_msg));
      svm_msg_q_free_msg (wrk->app_event_queue, &msg);
      break;
    }
...
handle:
  ...
  if (ep)
	...
	_clib_memcpy (ep->ip, &client_session->transport.rmt_ip.ip4,
		      sizeof (ip4_address_t));
      else
	_clib_memcpy (ep->ip, &client_session->transport.rmt_ip.ip6,
		      sizeof (ip6_address_t));
    }
...
vppcom_session_connect (uint32_t session_handle, vppcom_endpt_t * server_ep)
{
  ...
  if (session->transport.is_ip4)
    _clib_memcpy (&session->transport.rmt_ip.ip4, server_ep->ip,
		  sizeof (ip4_address_t));
  else
    _clib_memcpy (&session->transport.rmt_ip.ip6, server_ep->ip,
		  sizeof (ip6_address_t));
...
vppcom_data_segment_copy (void *buf, vppcom_data_segments_t ds, u32 max_bytes)
{
  ...
  _clib_memcpy (buf, ds[0].data, first_copy);
  if (first_copy < max_bytes)
    {
      _clib_memcpy (buf + first_copy, ds[1].data,
  ...
  if (n_bits && read_map)
    {
      ...
      _clib_memcpy (wrk->rd_bitmap, read_map,
  ...
  if (n_bits && write_map)
    {
      ...
      _clib_memcpy (wrk->wr_bitmap, write_map,
  ...
  if (n_bits && except_map)
    {
      ...
      _clib_memcpy (wrk->ex_bitmap, except_map,
		     ...
		     void *buffer, uint32_t * buflen)
{
  ...
  switch (op)
			...
			(*buflen >= sizeof (*ep)) && ep->ip))
	{
	  ...
	  if (session->transport.is_ip4)
	    _clib_memcpy (ep->ip, &session->transport.rmt_ip.ip4,
			  sizeof (ip4_address_t));
	  else
	    _clib_memcpy (ep->ip, &session->transport.rmt_ip.ip6,
			  sizeof (ip6_address_t));
	  *buflen = sizeof (*ep);
			...
			(*buflen >= sizeof (*ep)) && ep->ip))
	{
	  ...
	  if (session->transport.is_ip4)
	    _clib_memcpy (ep->ip, &session->transport.lcl_ip.ip4,
			  sizeof (ip4_address_t));
	  else
	    _clib_memcpy (ep->ip, &session->transport.lcl_ip.ip6,
			  sizeof (ip6_address_t));
	  *buflen = sizeof (*ep);
