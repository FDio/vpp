...
typedef enum event_id_
{
  VCL_EVENT_CONNECT_REQ_ACCEPTED,
  VCL_EVENT_N_EVENTS
} event_id_t;

typedef struct event_connect_request_
{
  event_id_t eid;
  u32 listen_session_index;
  u32 accepted_session_index;
} event_connect_request_t;

/* Union used for sizing */
typedef union event_size_
{
  event_connect_request_t e1;
} event_size_t;
...
typedef struct event_pthread_t_
{
  int has_event;
  pthread_t thread;
...
typedef struct vppcom_main_t_
{
  ...
  /* Event thread */
  event_pthread_t event_thread;
  event_size_t *session_event_fifo;
...
/*
 * TODO Needs re-work along lines of below.
 *
 * Events should only contain the event# and session_index.
 *
 * There will be a pool indexed by <event#+session_index>
 * where each element in the pool is a collection of event_handlers.
 * (the event_handlers will be registered from the interested function.
 *  .. in the case of a event_connect_request, vppcom_session_accept()
 *  would register the handler for event#(VCL_EVENT_CONNECT_REQUEST) and
 *  the listener index that it was called with)
 *
 * The function generating the event will put event details into an
 * event specific typedef ie event_connect_request_t (which will have the
 * session_index of the newly accepted session) that is in a pool indexed
 * by session_index registered (in the above case the listener_session_index).
 *
 *
 *
 * The handler (at registration) will be passed a pointer to the (empty) queue of
 * event results.
 */
int
sizeof_event_t (event_id_t eid)
{
  ASSERT (eid < VCL_EVENT_N_EVENTS);

  switch (eid)
    {
    case VCL_EVENT_CONNECT_REQ_ACCEPTED:
      return sizeof (event_connect_request_t);
...
int
vcl_start_event_thread ()
{
  clib_fifo_validate (vcm->session_event_fifo, VCL_EVENT_N_EVENTS);
  pthread_cond_init (&vcm->event_thread.handler_cond, NULL);
  pthread_cond_init (&vcm->event_thread.generator_cond, NULL);
  pthread_mutex_init (&vcm->event_thread.handler_lock, NULL);
  pthread_mutex_init (&vcm->event_thread.generator_lock, NULL);

  return pthread_create (&vcm->event_thread.thread, NULL /* attr */ ,
			 vcl_event_thread_fn, 0);
}
...
int
generate_event (void *event)
{
  int elts, rv = 0;
  pthread_mutex_lock (&vcm->event_thread.generator_lock);
  elts = clib_fifo_free_elts (vcm->session_event_fifo);
  if (PREDICT_TRUE (elts))
    {
      /* Add event to queue */
      clib_fifo_add1 (vcm->session_event_fifo, *(event_size_t *) event);
      pthread_cond_signal (&vcm->event_thread.generator_cond);
    }
  else
    {
      rv = VNET_API_ERROR_QUEUE_FULL;
    }
  pthread_mutex_unlock (&vcm->event_thread.generator_lock);
  return rv;
}
...
vcl_event_thread_fn (void *arg)
{
  ...
  do
    {
      pthread_mutex_lock (&vcm->event_thread.generator_lock);
      while (vcm->event_thread.has_event == 0)
	{
	  //printf ("\n*****************   in vcl_event_thread: Waiting on cond\n");

	  pthread_cond_wait (&vcm->event_thread.generator_cond,
			     &vcm->event_thread.generator_lock);
//          printf ("\n*****************   in vcl_event_thread: received cond\n");
	}

//      printf ("\n*****************   Got an event\n");

      pthread_mutex_lock (&vcm->event_thread.handler_lock);
      vcm->event_thread.event_ready = 1;
      pthread_cond_signal (&vcm->event_thread.handler_cond);
      pthread_mutex_unlock (&vcm->event_thread.handler_lock);


      vcm->event_thread.has_event = 0;
      pthread_mutex_unlock (&vcm->event_thread.generator_lock);
    }
  while (1);
  return NULL;
...
vl_api_accept_session_t_handler (vl_api_accept_session_t * mp)
{
  ...
  event_connect_request_t *ecr = malloc (sizeof (event_connect_request_t));
  ecr->eid = VCL_EVENT_CONNECT_REQ_ACCEPTED;
  ecr->listen_session_index = listen_session - vcm->sessions;
  ecr->accepted_session_index = session_index;
  int rv = generate_event (ecr);
...
vl_api_connect_sock_t_handler (vl_api_connect_sock_t * mp)
{
  ...
  printf
    ("\n*****************   in vl_api_connect: locking generator_lock\n");

  pthread_mutex_lock (&vcm->event_thread.generator_lock);
  vcm->event_thread.has_event = 1;
  pthread_cond_signal (&vcm->event_thread.generator_cond);
  pthread_mutex_unlock (&vcm->event_thread.generator_lock);

  printf ("\n*****************   in vl_api_connect: unlock\n");
		       ...
		       uint32_t flags)
{
  ...
  event_connect_request_t ecr;
  ecr.listen_session_index = listen_session_index;
  ecr.accepted_session_index = ~0;


  rv = register_event_handler (handler, ecr);
  /* Wait for handler */
  ...
  pthread_mutex_lock (&vcm->event_thread.handler_lock);
  while (vcm->event_thread.event_ready == 0)
    {
      printf
	("\n*****************   vppcom_session_accept: waiting on handler_cond\n");

      pthread_cond_wait (&vcm->event_thread.handler_cond,
			 &vcm->event_thread.handler_lock);
    }
  vcm->event_thread.event_ready = 0;
  pthread_mutex_unlock (&vcm->event_thread.handler_lock);
  printf
