...
static void vcl_wait_for_memory (void *mem)
{
...
void
vcl_cut_through_registration_add (svm_msg_q_t *mq, u32 sid)
{
  ...
  if (mp->server_event_queue_address)
    {
      ...
      vcl_cut_through_registration_add (session->our_evt_q, session_index);
  ...
  if (svm_fifo_is_full (tx_fifo))
      ...
      while (1)
	{
	  ...
	  if (svm_msg_q_is_empty (vcm->app_event_queue)
	      && svm_msg_q_timedwait (vcm->app_event_queue, 10e-6))
	    continue;
  ...
  if (svm_fifo_set_event (tx_fifo))
    {
      ...
      VCL_SESSION_LOCK_AND_GET (session_index, &session);
      app_send_io_evt_to_vpp (session->vpp_evt_q, tx_fifo, FIFO_EVENT_APP_TX,
			      SVM_Q_WAIT);
      VCL_SESSION_UNLOCK ();
