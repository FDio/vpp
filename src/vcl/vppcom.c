...
void vppcom_allocate_heap (void);

/* Forward declaration for internal helper function */
static int vppcom_app_init_common (const char *app_name, int use_pid_suffix);
...
vppcom_app_create (const char *app_name)
{
  ...
  return vppcom_app_init_common (app_name, 0);
}

/* Internal helper function for common VCL app initialization */
static int
vppcom_app_init_common (const char *app_name, int use_pid_suffix)
{
  ...
  /* Set app name based on whether PID suffix is needed */
  if (use_pid_suffix)
    {
  ...
  if (use_pid_suffix)
    {
      VDBG (0, "app_name 'vcl_app_%d', my_client_index %d (0x%x) [config provided programmatically]",
	    vcm->main_pid, vcm->workers[0].api_client_handle, vcm->workers[0].api_client_handle);
    }
  else
    {
...
static void
vppcom_cfg_apply_user_config (vcl_cfg_t *vcl_cfg,
...
vppcom_app_create_with_config (vppcom_cfg_t *user_cfg)
{
 ...
 // vppcom_cfg_heapsize (NULL); /* Pass NULL for default config file behavior */
  ...
  /* Determine app name and whether to use PID suffix */
  if (user_cfg && user_cfg->app_name)
    {
      /* Use provided app name */
      return vppcom_app_init_common (user_cfg->app_name, 0);
    }
  else
    {
      /* Use PID-based app name */
      return vppcom_app_init_common (NULL, 1);
    }
}
