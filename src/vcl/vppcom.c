...
vcl_session_t *
vcl_cut_through_session_get_from_fifo (svm_fifo_t *f, u8 type)
{
  vcl_session_t *s;
...
vcl_epoll_wait_handle_mq (svm_msg_q_t *mq, struct epoll_event *events,
                          u32 maxevents, u32 wait_for_time)
{
  session_disconnected_msg_t *disconnected_msg;
  ...
  for (i = 0; i < n_msgs; i++)
	...
	case SESSION_IO_EVT_CT_TX:
	  session = vcl_cut_through_session_get_from_fifo (e->fifo, 0);
	...
	case SESSION_IO_EVT_CT_RX:
	  session = vcl_cut_through_session_get_from_fifo (e->fifo, 1);
	  sid = vcl_session_index (session);
	  session_events = session->vep.ev.events;
	  if ((EPOLLOUT & session_events)
	      && !svm_fifo_is_full (session->tx_fifo))
  ...
  while (total_wait <= wait_for_time)
    {
      ...
      pool_foreach (cr, vcm->cut_through_registrations, ({
        num_ev += vcl_epoll_wait_handle_mq (cr->mq, events, maxevents - num_ev,
                                            0);
      }));
      /* *INDENT-ON* */

      num_ev += vcl_epoll_wait_handle_mq (vcm->app_event_queue, events,
