...
lt_list_sane (vcl_worker_t *wrk)
{
  ...
  start = wrk->ep_lt_current;
  if (start == ~0)
    return;
  c = vcl_session_get (wrk, start);
  do
    {
      n = vcl_session_get (wrk, c->vep.lt_next);
      if (n->vep.lt_prev != c->session_index)
	os_panic ();
      c = n;
    }
  while (start != c->session_index);
}
...
//      if (s->vep.lt_next && s->vep.lt_next != ~0 && lt_is_member(wrk, s->session_index))
//	os_panic();
				...
				struct epoll_event *events, u32 * num_ev)
{
  ...
  switch (e->event_type)
	...
	{
	  disconnected_msg = (session_disconnected_msg_t*) e->data;
	  s = vcl_session_get_w_vpp_handle (wrk, disconnected_msg->handle);
	  if (!vcl_session_is_closed (s) && !(s->flags & VCL_SESSION_F_IS_VEP_SESSION))
	   {
	     u8 is_ct = s->ct_rx_fifo != 0;
	     svm_fifo_t *f = is_ct ? s->ct_rx_fifo : s->rx_fifo;
	     clib_warning ("close %u %U:%u->%U:%u not epolled? lt %d state %u head %u tail %u", s->session_index,
	                   format_ip46_address, &s->transport.lcl_ip, s->transport.is_ip4, ntohs(s->transport.lcl_port),
	                   format_ip46_address, &s->transport.rmt_ip, s->transport.is_ip4, ntohs(s->transport.rmt_port),
	                   s->vep.lt_next, s->session_state, f->shr->head, f->shr->tail );
	   }
	  s = vcl_session_disconnected_handler (wrk, disconnected_msg);
	}
      else
    ...
    case SESSION_CTRL_EVT_RESET:
      if (!e->postponed)
	{
	  s = vcl_session_get_w_vpp_handle (wrk, ((session_reset_msg_t *)e->data)->handle);
	  if (!vcl_session_is_closed (s) && !(s->flags & VCL_SESSION_F_IS_VEP_SESSION))
	   {
	     clib_warning ("reset %u not epolled? lt %d state %u head %u ", s->session_index, s->vep.lt_next,
	                   s->session_state, s->ct_rx_fifo != 0 ? s->ct_rx_fifo->shr->head : s->rx_fifo->shr->head );
	   }
	  sid = vcl_session_reset_handler (wrk, (session_reset_msg_t *) e->data);
	}
      else
