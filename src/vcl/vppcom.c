			      ...
			      u32 ls_index)
{
  ...
  if (mp->rmt.is_ip4)
    {
      session->transport.original_dst_ip4 = mp->original_dst_ip4;
      session->transport.original_dst_port = mp->original_dst_port;
    }
		     ...
		     void *buffer, uint32_t * buflen)
{
  ...
  switch (op)
	...
	{
	  ep->is_ip4 = session->transport.is_ip4;
	  ep->port = session->transport.original_dst_port;
	  clib_memcpy_fast (ep->ip, &session->transport.original_dst_ip4,
			    sizeof (ip4_address_t));
	  *buflen = sizeof (*ep);
	  VDBG (1,
		...
		(ip4_address_t *) (&session->transport.original_dst_ip4),
		ep->is_ip4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6,
		clib_net_to_host_u16 (ep->port));
	}
      else
