...
vppcom_app_init_common (const char *app_name, int use_pid_suffix)
{
  ...
  if (use_pid_suffix)
    {
      VDBG (0,
	    "app_name 'vcl_app_%d', my_client_index %d (0x%x) [config "
	    "provided programmatically]",
	    vcm->main_pid, vcm->workers[0].api_client_handle,
...
vppcom_app_create_with_config (vppcom_cfg_t *user_cfg)
{
  ...
  // vcl_cfg_parse_heapsize (NULL); /* Pass NULL for default config file
  // behavior */
  vcl_heap_alloc ();
