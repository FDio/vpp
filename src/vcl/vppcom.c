...
always_inline int
vppcom_session_write_inline_ext (vcl_worker_t * wrk, vcl_session_t * s, void *buf,size_t n, 
           void *param_buf, size_t param_len,
           u8 is_flush, u8 is_dgram)
{
  ...
  if(param_buf && ( param_len == sizeof(vppcom_endpt_tlv_t))) {
      ...
      if(p_user_data->data_type == VCL_UDP_SEGMENT){
        gso_size = p_user_data->value;
      }
  }

  if (is_dgram)
    n_write = app_send_dgram_raw_gso (tx_fifo, &s->transport,
				  s->vpp_evt_q, buf, n, 
          gso_size,et,
				  0 /* do_evt */ , SVM_Q_WAIT);
  else
		       ...
		       uint32_t buflen, int flags, vppcom_endpt_t * ep)
{
  ...
  return (vppcom_session_write_inline_ext (wrk, s, buffer, buflen,
               (void*)p_user_data, p_user_data_len, 
               1, s->is_dgram ? 1 : 0));
}
