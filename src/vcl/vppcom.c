...
static void
vcl_send_bind (vcl_worker_t *wrk, vcl_session_t *s)
{
  ...
  app_alloc_ctrl_evt_to_vpp (mq, app_evt, SESSION_CTRL_EVT_BIND);
  ...
  clib_warning ("IS IP4 %u", s->transport.is_ip4);
  clib_memcpy_fast (&rmp->ip, &s->transport.lcl_ip, sizeof (rmp->ip));
  clib_warning ("IS IP4 AGAIN %u", s->transport.is_ip4);
  rmp->port = s->transport.lcl_port;
  rmp->proto = s->session_type;
  clib_warning ("IS IP4 AND AGAIN %u", s->transport.is_ip4);
...
vppcom_session_listen (uint32_t listen_sh, uint32_t q_len)
{
   ...
   */
  vcl_send_bind (wrk, listen_session);
