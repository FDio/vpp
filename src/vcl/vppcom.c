...
static void
vcl_cleanup_forked_child (vcl_worker_t *child_wrk)
{
  ...
  if (child_wrk->forked_child != ~0)
      ...
      if (sub_child)
	{
	  ...
	  if (kill (sub_child->current_pid, 0) < 0)
	    vcl_cleanup_forked_child (sub_child);
	}
    }
  clib_warning ("removing wrk %u", child_wrk->wrk_index);
  vcl_worker_cleanup (child_wrk, 1 /* notify vpp */);
  vec_add1 (vcm->workers_deleted, child_wrk->wrk_index);
}
...
vcl_intercept_sigchld_handler (int signum, siginfo_t * si, void *uc)
{
  ...
  if (wrk->forked_child == ~0)
    {
      clib_warning ("no such forked child");
      return;
    }
  ...
  if (child_wrk)
    {
      clib_warning ("wrk %u intercepted sigchld and removing wrk %u", wrk->wrk_index, wrk->forked_child);
      vcl_cleanup_forked_child (child_wrk);
    }
  wrk->forked_child = ~0;
