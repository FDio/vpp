			   ...
			   session_reset_msg_t * reset_msg)
{
  ...
  if (session->session_state == STATE_LISTEN)
      ...
      for (i = 0; i < vec_len (session->accept_evts_fifo); i++)
	{
	  ...
	  if (accepted_msg->accepted_msg.handle == reset_msg->handle)
	    {
	      clib_warning ("closed before accept");
	      accepted_msg->flags = VCL_ACCEPTED_F_RESET;
	      return VCL_INVALID_SESSION_INDEX;
	    }
	}
      clib_warning ("session was not accepted!");
      return VCL_INVALID_SESSION_INDEX;
    }
...
handle:
  ...
  if (accept_flags)
      ...
      if (accept_flags & VCL_ACCEPTED_F_CLOSED)
	{
      ...
      if (accept_flags & VCL_ACCEPTED_F_RESET)
	{
	  client_session->session_state = STATE_CLOSE_ON_EMPTY;
	  clib_warning ("sending reset reply");
  ...
  switch (e->event_type)
      ...
      if (sid < n_bits && write_map)
	{
	  ...
	  *bits_set += 1;
      ...
      break;
    case SESSION_CTRL_EVT_ACCEPTED:
      session = vcl_session_accepted (wrk, (session_accepted_msg_t *) e->data);
      if (!session)
        {
          VDBG (0, "couldn't find listen session: listener handle %llx",
                ((session_accepted_msg_t *) e->data)->listener_handle);
          break;
        }
      sid = session->session_index;
      if (sid < n_bits && read_map)
				...
				struct epoll_event *events, u32 * num_ev)
{
  ...
  switch (e->event_type)
    ...
    case SESSION_CTRL_EVT_ACCEPTED:
      ...
      if (!session)
	{
	  u64 handle = ((session_accepted_msg_t *) e->data)->listener_handle;
	  clib_warning ("VCL<%d>: ERROR: couldn't find listen session:"
