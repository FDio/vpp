...
static ssize_t
ldp_sendo (vls_handle_t vlsh, const void *buf, size_t n, int flags,
...
static ssize_t
ldp_recvfrom (vls_handle_t vlsh, void *__restrict buf, size_t n, int flags,
	...
	__CONST_SOCKADDR_ARG addr, socklen_t addr_len)
{
  ...
  if (vlsh != INVALID_SESSION_ID)
    {
      size = ldp_sendo (vlsh, buf, n, flags, addr, addr_len);
      if (size < 0)
	{
	  errno = -size;
	  size = -1;
	  ...
	  __SOCKADDR_ARG addr, socklen_t * __restrict addr_len)
{
  ...
  if (vlsh != VLS_INVALID_HANDLE)
    {
      size = ldp_recvfrom (vlsh, buf, n, flags, addr, addr_len);
      if (size < 0)
	{
	  errno = -size;
	  size = -1;
...
sendmsg (int fd, const struct msghdr * msg, int flags)
{
  ...
  if (vlsh != VLS_INVALID_HANDLE)
	...
	{
	  rv = ldp_sendo (vlsh, iov[i].iov_base, iov[i].iov_len, flags,
...
recvmsg (int fd, struct msghdr * msg, int flags)
{
  ...
  if (vlsh != VLS_INVALID_HANDLE)
	...
	{
	  rv = ldp_recvfrom (vlsh, iov[i].iov_base, iov[i].iov_len, flags,
