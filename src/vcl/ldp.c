...
close (int fd)
{
  ...
  if (vlsh != VLS_INVALID_HANDLE)
    {
      epfd = vls_get_libc_fd (vlsh);
      if (PREDICT_FALSE (epfd < 0))
	{
	  errno = -epfd;
	  rv = -1;
	  goto done;
      ...
      if (epfd > 0)
	{
	  ...
	  if (!clib_bitmap_get_no_check (ldpw->vlsh_libc_punt_map, vlsh))
	    ldpw->mq_epfd_added = 0;
	  else
	    clib_bitmap_set_no_check (ldpw->vlsh_libc_punt_map, vlsh, 0);

	  LDBG (0, "fd %d: calling libc_close: epfd %u", fd, epfd);
	  libc_close (epfd);

	  vls_set_libc_fd (vlsh, 0);
	}
