...
fcntl (int fd, int cmd, ...)
{
  ...
  if (sid != VLS_INVALID_HANDLE)
	...
	{
	case F_SETFL:
	  rv = vls_attr (sid, VPPCOM_ATTR_SET_FLAGS, &flags, &size);
	  break;

	case F_GETFL:
	  rv = vls_attr (sid, VPPCOM_ATTR_GET_FLAGS, &flags, &size);
	  if (rv == VPPCOM_OK)
	    rv = flags;
	  break;
	case F_SETFD:
	...
	case FIONBIO:
	  {
	     ...
	     *      with O_NONBLOCK.
	     */
	    rv = vppcom_session_attr (vlsh, VPPCOM_ATTR_SET_FLAGS, &flags,
				      &size);
	  }
	  break;
