	  ...
	  if (!ldpw->select_vcl)
	      ...
	      else if (rv > 0)
		{
		  ...
		  if (writefds)
		    {
                      /* *INDENT-OFF* */
                      clib_bitmap_foreach (vlsh, ldpw->wr_bitmap, ({
                	vlsh = vls_session_index_to_vlsh (si);
                        fd = ldp_vlsh_to_fd (vlsh);
                        if (PREDICT_FALSE (fd < 0))
                          {
		  ...
		  if (exceptfds)
		    {
                      /* *INDENT-OFF* */
                      clib_bitmap_foreach (vlsh, ldpw->ex_bitmap, ({
                	vlsh = vls_session_index_to_vlsh (si);
                        fd = ldp_vlsh_to_fd (vlsh);
                        if (PREDICT_FALSE (fd < 0))
                          {
  ...
  if (vlsh != VLS_INVALID_HANDLE)
	...
	case SOL_IPV6:
	  switch (optname)
	    {
	    case IPV6_V6ONLY:
	      rv = vls_attr (vlsh, VPPCOM_ATTR_GET_V6ONLY,
					optval, optlen);
	      break;
	    default:
	...
	case SOL_SOCKET:
	  switch (optname)
	    {
	    case SO_ACCEPTCONN:
	      rv = vls_attr (vlsh, VPPCOM_ATTR_GET_LISTEN,
					optval, optlen);
	      break;
	    case SO_KEEPALIVE:
	      ...
	      break;
	    case SO_PROTOCOL:
	      rv = vls_attr (vlsh, VPPCOM_ATTR_GET_PROTOCOL,
					optval, optlen);
	    ...
	    case SO_RCVBUF:
	      ...
	      break;
	    case SO_REUSEADDR:
	      rv = vls_attr (vlsh, VPPCOM_ATTR_GET_REUSEADDR,
					optval, optlen);
	      break;
	    case SO_BROADCAST:
	      rv = vls_attr (vlsh, VPPCOM_ATTR_GET_BROADCAST,
					optval, optlen);
	      break;
	    case SO_ERROR:
	      rv = vls_attr (vlsh, VPPCOM_ATTR_GET_ERROR,
					optval, optlen);
	      break;
	    default:
...
epoll_ctl (int epfd, int op, int fd, struct epoll_event *event)
{
  ...
  LDBG (0, "epfd %d, vep_vlsh %d, vlsh %d", epfd, vep_vlsh, vlsh);
  if (vlsh != VLS_INVALID_HANDLE)
    {
