...
ldp_init (void)
{
  ...
  if (ldp->vcl_needs_real_epoll)
    ldp->vcl_needs_real_epoll = vppcom_use_mq_eventfd ();
  ...
  if (sid != INVALID_SESSION_ID)
    {
      ...
      LDBG (0, "LDP<%d>: epfd %d (0x%x): calling %s(): vep_idx %d (0x%x),"
  ...
  else
    {
      ...
      libc_epfd = vppcom_session_attr (vep_idx, VPPCOM_ATTR_GET_LIBC_EPFD, 0,
                                       0);
      LDBG (0, "LDP<%d>: epfd %d (0x%x), vep_idx %d (0x%x): %s() "
      ...
      if (!libc_epfd)
        {
          ...
          LDBG (0, "LDP<%d>: epfd %d (0x%x), vep_idx %d (0x%x): "
      ...
      LDBG (0, "LDP<%d>: epfd %d (0x%x): calling %s(): libc_epfd %d (0x%x), "
                 ...
                 int timeout, const sigset_t * sigmask)
{
  ...
  if (epfd == ldp->vcl_mq_epfd)
    return libc_epoll_pwait (epfd, events, maxevents, 1, sigmask);
