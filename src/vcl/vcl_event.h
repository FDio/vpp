...
{
  struct {
    u32 eid; //TODO reconcile keeping this generic and enum defined in vppcom.c
    u32 session_index; //only supporting session events for now
  };
  u64 as_u64;
} vce_event_key_t;

/* TODO only supporting session events for now */
typedef void (*vce_event_callback_t) (u32 session_index,
				      void *reg /*vce_event_handler_reg_t* */);
...
typedef struct vce_event_thread_
{
  ...
  vce_event_key_t *session_event_fifo;
  vce_event_handler_reg_t *session_event_handlers; //pool
 ...
 */
int vce_generate_event (vce_event_key_t *evk, vce_event_thread_t *evt);
 ...
 * @return success/failure rv
 */
int vce_unregister_handler (vce_event_thread_t *evt, vce_event_key_t evk,
			vce_event_callback_t cb);


/*
 * TODO - have multi-handler registration code ready to go,
 * pushing single handler now as it mirrors functionality we have today
 */
