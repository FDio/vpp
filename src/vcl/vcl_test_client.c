...
vtc_cfg_sync (sock_test_socket_t * sock)
{
  ...
  sock->cfg.seq_num = ++vcm->cfg_seq_num;
  if (sock->cfg.verbose)
    {
...
static int
vtc_connect_test_sockets (vcl_test_client_worker_t *wrk)
{
  vcl_test_client_main_t *vcm = &vcl_client_main;
  ...
  num_test_sockets = wrk->cfg.num_test_sockets;
  ...
  wrk->sessions = calloc (num_test_sockets, sizeof (sock_test_socket_t));
  if (!wrk->sessions)
    {
      vterr ("calloc()", -errno);
      return errno;
  ...
  for (i = 0; i < num_test_sockets; i++)
    {
      tsock = &wrk->sessions[i];
      tsock->fd = vppcom_session_create (vcm->proto,
                                         1 /* is_nonblocking */);
      if (tsock->fd < 0)
        {
      ...
      vtinf ("(fd %d): Test socket %d connected.\n", tsock->fd, i);
    }

  vtinf ("All sockets (%d) connected!\n", vcm->num_test_sockets + 1);
  return 0;
}
...
vtc_worker_init (vcl_test_client_worker_t * wrk)
{
  ...
  vcm->active_workers += 1;
  rv = vtc_connect_test_sockets (wrk);
  if (rv)
    {
  ...
  for (n = 0; n != cfg->num_test_sockets; n++)
	...
	return -1;
      ...
      FD_SET (tsock->fd, &wrk->wr_fdset);
      FD_SET (tsock->fd, &wrk->rd_fdset);
...
vtc_worker_loop (void *arg)
{
  ...
  n_active = wrk->cfg.num_test_sockets;
  while (n_active)
    {
	  ...
	  if (FD_ISSET (vppcom_session_index (tsock->fd), rfdset))
	    {
	      ...
	      if (tsock->stats.rx_bytes >= tsock->cfg.total_bytes)
		vtwrn ("read too many bytes on %u", tsock->fd);
	    }

	  if (FD_ISSET (vppcom_session_index (tsock->fd), wfdset))
	    {
	      if (tsock->stats.tx_bytes >= tsock->cfg.total_bytes)
		continue;
...
static void
vtc_accumulate_stats (vcl_test_client_worker_t *wrk, sock_test_socket_t *ctrl)
{
  sock_test_socket_t *tsock;
  static char buf[64];
  int i, show_rx = 0;
...
vtc_stream_client (vcl_test_client_main_t *vcm)
{
  ...
  vtinf ("(fd %d): Sending config to server on ctrl socket...\n", ctrl->fd);
...
main (int argc, char **argv)
{
  ...
  vtinf ("(fd %d): Control socket connected.\n", ctrl->fd);
