...
vtc_cfg_sync (sock_test_socket_t * sock)
{
  ...
  sock->cfg.seq_num = vt_atomic_add (&vcm->cfg_seq_num, 1);
  if (sock->cfg.verbose)
    {
...
vtc_worker_loop (void *arg)
{
  ...
  check_rx = wrk->test_type == SOCK_TEST_TYPE_BI;
  ...
  while (n_active_sessions)
      ...
      for (i = 0; i < wrk->cfg.num_test_sockets; i++)
	{
	  ...
	  if ((!check_rx && tsock->stats.tx_bytes >= ctrl->cfg.total_bytes)
	      || (check_rx && tsock->stats.rx_bytes >= ctrl->cfg.total_bytes))
	    {
