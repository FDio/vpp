...
vtc_connect_test_sockets (vcl_test_client_worker_t *wrk)
{
  ...
  if (wrk->n_sessions >= num_test_sockets)
    return 0;

  if (wrk->n_sessions)
    wrk->sessions = realloc (wrk->sessions,
	                     num_test_sockets * sizeof(sock_test_socket_t));
  else
  ...
  wrk->n_sessions = num_test_sockets;
...
vtc_worker_test_setup (vcl_test_client_worker_t * wrk)
{
  ...
  switch (wrk->test_type)
  {
    ...
    case SOCK_TEST_TYPE_BI:
      ...
      break;
    default:
      vtwrn ("Unsupported test type: %u", wrk->test_type);
      return -1;
  }
...
vtc_echo_client (vcl_test_client_main_t *vcm)
{
  ...
  wrk->wrk_index = 0;
  wrk->cfg = *cfg;
  wrk->test_type = SOCK_TEST_TYPE_ECHO;
