...
vtc_quic_connect_test_sessions (vcl_test_client_worker_t * wrk)
{
  ...
  if (wrk->n_qsessions)
    ...
    wrk->qsessions = calloc (wrk->cfg.num_test_qsessions, sizeof (vcl_test_session_t));
  ...
  for (i = 0; i < wrk->cfg.num_test_qsessions; i++)
      ...
      if (tq->fd < 0)
	{
      ...
      rv = vppcom_session_quic_connect (tq->fd, &vcm->server_endpt);
      if (rv < 0)
	{
	  vterr ("vppcom_session_quic_connect()", rv);
	  return rv;
	}
  ...
  if (wrk->n_sessions)
    wrk->sessions =
      realloc (wrk->sessions, wrk->cfg.num_test_sessions * sizeof (vcl_test_session_t));
  else
    wrk->sessions = calloc (wrk->cfg.num_test_sessions, sizeof (vcl_test_session_t));
...
main (int argc, char **argv)
{
  ...
  if (vcm->proto == VPPCOM_PROTO_QUIC)
	...
	vtfail ("vppcom_session_create()", quic_session->fd);
      rv = vppcom_session_quic_connect (quic_session->fd, &vcm->server_endpt);
      if (rv)
