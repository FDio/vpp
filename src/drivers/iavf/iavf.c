...
static clib_arg_t iavf_dev_args[] = {
  {
    .name = "num_queues",
    .desc = "Number of queue pairs to request from PF (1-32), or omit to use PF default",
...
iavf_init (vlib_main_t *vm, vnet_dev_t *dev)
{
   ...
   *  num_queues=N : request N queue pairs from the PF (1-32)
   ...
   */
  u32 num_queues = clib_args_get_uint32_val_by_name (dev->args, "num_queues");

  if (num_queues)
    {
      u16 requested_qp;

      requested_qp = (u16) num_queues;
    ...
    log_debug (dev, "num_queues not set, using PF default queue allocation");
