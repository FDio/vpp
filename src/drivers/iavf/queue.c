...
iavf_tx_queue_alloc (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  ...
  clib_ring_new_aligned (atq->rs_slots, txq->size, CLIB_CACHE_LINE_BYTES);
  ...
  atq->buf_free_flags =
    clib_mem_alloc_aligned (sizeof (atq->buf_free_flags[0]) * txq->size, CLIB_CACHE_LINE_BYTES);
  atq->tmp_buf_free_flags =
    clib_mem_alloc_aligned (sizeof (atq->tmp_buf_free_flags[0]) * txq->size, CLIB_CACHE_LINE_BYTES);
...
iavf_tx_queue_free (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  ...
  foreach_pointer (p, aq->tmp_descs, aq->tmp_bufs, aq->buffer_indices, aq->buf_free_flags,
		   aq->tmp_buf_free_flags)
    if (p)
      clib_mem_free (p);
}
...
iavf_tx_queue_stop (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  ...
  atq->n_enqueued = atq->next = atq->n_desc_since_rs = 0;
}
