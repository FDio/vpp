...
iavf_tx_fix_gso_l4_hdr_sz (vlib_buffer_t *b, u32 *l234hdr_sz)
{
  ...
  if (l4_offset <= 0 ||
      l4_offset + sizeof (tcp_header_t) > b->current_length)
    return 0;

  tcp = (tcp_header_t *) (vlib_buffer_get_current (b) + l4_offset);
  l4_hdr_sz = tcp_header_bytes (tcp);

  if (l4_hdr_sz < sizeof (tcp_header_t) ||
      l4_offset + l4_hdr_sz > b->current_length)
    return 0;
...
{
  iavf_tx_fill_data_desc_addr (d, iavf_tx_buffer_dma_addr (vm, b, use_va_dma),
			       b->current_length, cmd);
}
...
{
  if (tso)
    return (b->current_length + IAVF_MAX_DATA_PER_TXD - 1) /
	   IAVF_MAX_DATA_PER_TXD;
  return 1;
}

static_always_inline u32
iavf_tx_tso_chain_len_and_descs (vlib_main_t *vm, vlib_buffer_t *b,
				 u16 *n_descs)
{
  u32 pkt_len = 0;
  u16 n = 0;
...
static_always_inline u16
iavf_tx_fill_buffer_descs (vlib_main_t *vm, iavf_tx_desc_t *d, u32 *tb,
			   u8 *tf, vlib_buffer_t *b, u32 bi, u64 cmd,
			   int use_va_dma, u8 tso)
{
  ...
  while (tso && len > IAVF_MAX_DATA_PER_TXD)
    {
      iavf_tx_fill_data_desc_addr (d, addr, IAVF_MAX_DATA_PER_TXD,
				   cmd & ~IAVF_TXD_CMD_EOP);
      tb[0] = bi;
      tf[0] = n_descs == 0;
      d++;
      tb++;
	...
	else
	{
	  ...
	  if (flags & VNET_BUFFER_F_GSO)
	      ...
	      if (PREDICT_FALSE (gso_error))
		{
	      ...
	      pkt_len =
		iavf_tx_tso_chain_len_and_descs (vm, b[0], &tso_n_descs);

	      if (PREDICT_FALSE (pkt_len <= l234hdr_sz))
		{
		  if (!iavf_tx_fix_gso_l4_hdr_sz (b[0], &l234hdr_sz) ||
		      pkt_len <= l234hdr_sz)
		    {
		      flags &= ~VNET_BUFFER_F_GSO;
		      vlib_error_count (vm, node->node_index,
					IAVF_TX_NODE_CTR_GSO_NOT_NEEDED, 1);
		      goto gso_done;
		    }
		}
	      ...
	      if (PREDICT_FALSE (l234hdr_sz > b[0]->current_length))
		{
		  ...
		  if (PREDICT_FALSE (n_lin == 0 ||
				     l234hdr_sz > b[0]->current_length))
		    {
		  ...
		  pkt_len =
		    iavf_tx_tso_chain_len_and_descs (vm, b[0], &tso_n_descs);
		  vlib_error_count (vm, node->node_index,
				    IAVF_TX_NODE_CTR_GSO_HDR_LINEARIZED, 1);
		}
	      ...
	      if (PREDICT_FALSE (pkt_len <= l234hdr_sz))
		{
	      ...
	      if (PREDICT_FALSE (gso_size < IAVF_MIN_TSO_MSS ||
				 gso_size > IAVF_MAX_TSO_MSS))
		{
	  ...
	  while (b[0]->flags & VLIB_BUFFER_NEXT_PRESENT)
	    {
	      u16 n = iavf_tx_fill_buffer_descs (vm, d, tb, tf, b[0], tb[0],
						 cmd, use_va_dma, is_tso);
	  ...
	  {
	    u16 n = iavf_tx_fill_buffer_descs (
	      vm, d, tb, tf, b[0], tb[0], cmd_eop | cmd, use_va_dma, is_tso);
	    n_desc_left -= n;
	    d += n;
	    tb += n;
	    tf += n;
  ...
  if (n_desc)
      ...
      if (PREDICT_TRUE (next + n_desc <= txq->size))
	{
	  ...
	  vlib_buffer_copy_indices (atq->buffer_indices + next, atq->tmp_bufs,
				    n_desc);
	  clib_memcpy_fast (atq->buf_free_flags + next,
			    atq->tmp_buf_free_flags, n_desc);
	}
      else
	{
	  /* wrap */
	  ...
	  vlib_buffer_copy_indices (atq->buffer_indices,
				    ...
				    n_desc - n_not_wrap);
	  clib_memcpy_fast (atq->buf_free_flags + next,
			    atq->tmp_buf_free_flags, n_not_wrap);
	  clib_memcpy_fast (atq->buf_free_flags,
			    atq->tmp_buf_free_flags + n_not_wrap,
			    n_desc - n_not_wrap);
	}
