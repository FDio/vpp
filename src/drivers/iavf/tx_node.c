...
iavf_tx_alloc_ctx_desc_buffer (vlib_main_t *vm, vnet_dev_tx_queue_t *txq,
			       u32 *bi)
{
  vnet_dev_t *dev = txq->port->dev;
  u8 bpi = vlib_buffer_pool_get_default_for_numa (vm, dev->numa_node);

  return vlib_buffer_alloc_from_pool (vm, bi, 1, bpi);
}
	  ...
	  if (flags & VNET_BUFFER_F_GSO)
	    {
	       ...
	       * Allocate that placeholder only when a TSO descriptor is used. */
	      if (PREDICT_FALSE (
		    iavf_tx_alloc_ctx_desc_buffer (vm, txq, &ctx_bi) != 1))
		{
		  vlib_buffer_free_one (vm, buffers[0]);
		  vlib_error_count (vm, node->node_index,
				    IAVF_TX_NODE_CTR_BUFFER_ALLOC, 1);
		  n_packets_left -= 1;
		  buffers += 1;
		  continue;
		}
