...
iavf_rx_validate_l4_cksum (vlib_buffer_t *b, u64 qw1, u32 flags)
{
  ...
  b->flags &= ~IAVF_RX_L4_CKSUM_FLAGS;
  b->flags |= flags & IAVF_RX_L4_CKSUM_FLAGS;
}

static_always_inline u64
iavf_rx_l4_cksum_qw1 (u64 qw1, iavf_rx_tail_t *t, int maybe_multiseg)
{
  u32 i = 0;
		       ...
		       int maybe_multiseg)
{
  ...
  while (n_left >= 4)
      ...
      if (maybe_multiseg)
	{
      ...
      iavf_rx_validate_l4_cksum (b[0], iavf_rx_l4_cksum_qw1 (qw1[0], tail + 0, maybe_multiseg),
				 flags);
      iavf_rx_validate_l4_cksum (b[1], iavf_rx_l4_cksum_qw1 (qw1[1], tail + 1, maybe_multiseg),
				 flags);
      iavf_rx_validate_l4_cksum (b[2], iavf_rx_l4_cksum_qw1 (qw1[2], tail + 2, maybe_multiseg),
				 flags);
      iavf_rx_validate_l4_cksum (b[3], iavf_rx_l4_cksum_qw1 (qw1[3], tail + 3, maybe_multiseg),
				 flags);
