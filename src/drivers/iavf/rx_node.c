		       ...
		       int maybe_multiseg, u32 l4_cksum_flags)
{
  ...
  while (n_left)
	...
	n_rx_bytes += iavf_rx_attach_tail (vm, bt, b[0], qw1[0], tail + 0);
      ...
      if (l4_cksum_flags && PREDICT_FALSE (iavf_rxd_l4_cksum_needs_fixup (qw1[0])))
	iavf_fixup_l4_cksum_offload (b[0], qw1[0]);
	  ...
	  do
	      ...
	      if (PREDICT_FALSE (tail_desc >= IAVF_RX_MAX_DESC_IN_CHAIN - 1))
		{
		  vlib_error_count (vm, node->node_index,
				    IAVF_RX_NODE_CTR_CHAIN_TOO_LONG, 1);
		  goto no_more_desc;
		}
...
no_more_desc:
  ...
  n_rx_bytes = n_tail_desc ?
		 iavf_process_rx_burst (vm, node, rxq, rtd, &bt, n_rx_packets, 1, l4_cksum_flags) :
		 iavf_process_rx_burst (vm, node, rxq, rtd, &bt, n_rx_packets, 0, l4_cksum_flags);
