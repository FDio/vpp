  ...
  *im = (virtchnl_irq_map_info_t){
  ...
  if (port->attr.caps.interrupt_mode)
	...
	foreach_vnet_dev_port_rx_queue (rxq, port)
	  if (rxq->enabled)
	    {
	      /* rx_thread_index is a VPP thread index (0 = main, 1..n = workers).
	       * Main thread (index 0) does not handle RX in multi-worker mode, so
	       * skip it. Worker indices 1..n map to virtchnl vector slots 0..n-1. */
	      if (rxq->rx_thread_index == 0)
		continue;
