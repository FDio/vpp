  ...
  foreach_vnet_dev_port_rx_queue (q, port)
    {
      ...
      vnet_dev_get_rx_queue_if_buffer_template_ptr (q)->flags |= IAVF_RX_L4_CKSUM_FLAGS;

      vec_foreach_index (i, q->sec_if_rt_data)
	if (q->sec_if_rt_data[i])
	  vnet_dev_get_rx_queue_sec_if_buffer_template_ptr (q, i)->flags |= IAVF_RX_L4_CKSUM_FLAGS;
