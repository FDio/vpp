...
static int
pp2_bm_pool_create (vlib_main_t *vm, struct bm_pool_param *param)
{
...
bad_region:
  vlib_physmem_free (vm, (void *) pool->bm_pool_virt_base);
  clib_mem_free (pool);
  return -EIO;
no_mem:
  clib_mem_free (pool);
...
static int
pp2_bm_pool_destroy (vlib_main_t *vm, uintptr_t cpu_slot, struct pp2_bm_pool *pool)
{
...
pp2_bpool_deinit (struct pp2_bpool *pool)
{
  ...
  if (bm_pool && !pp2_bm_pool_destroy (pool->vm, cpu_slot, bm_pool))
...
pp2_bpool_init (struct pp2_bpool_params *params, struct pp2_bpool **bpool)
{
  ...
  rc = pp2_bm_pool_create (params->vm, &param);
  if (rc)
    return rc;

  pp2_bpools[pp2_id][pool_id].vm = params->vm;
