...
static_always_inline enum pp2_ppio_type
pp2_port_type (struct pp2_port *port)
{
  return port->vpp_port ? port->vpp_port->type : PP2_PPIO_T_NIC;
}
...
pp2_rss_enable (struct pp2_port *port, int en)
{
  ...
  if (pp2_port_type (port) == PP2_PPIO_T_LOG)
    return 0;
...
static int pp2_cls_mng_add_default_flow (struct pp2_port *port);
static int pp2_cls_mng_set_coloring (struct pp2_port *port, int clear);
static int pp2_cls_mng_set_policing (struct pp2_port *port, int clear);
static int pp2_db_cls_lkp_dcod_get (struct pp2_inst *inst, u32 fl_log_id,
				    struct pp2_db_cls_lkp_dcod_t *lkp_dcod);
...
static int
pp2_cls_mng_add_default_flow (struct pp2_port *port)
{
  ...
  tbl_params.default_act.cos->port = port;
...
static int
pp2_cls_mng_set_policing (struct pp2_port *port, int clear)
{
  u32 ref_cnt;
  int rc;
  ...
  if (!clear)
    {
      rc = pp2_cls_plcr_ref_cnt_get (port->parent, port->default_plcr->id, NULL, &ref_cnt);
      if (rc || ref_cnt)
	{
  ...
  rc =
    pp2_cls_plcr_ref_cnt_update (port->parent, port->default_plcr->id,
...
pp2_cls_mng_modify_default_flows (struct pp2_port *port, int clear)
{
  ...
  rc = pp2_cls_mng_set_policing (port, clear);
  if (rc)
    {
      pr_err ("%s(%d) pp2_cls_mng_set_policing fail\n", __func__, __LINE__);
      return -EINVAL;
...
static int
pp2_cls_mng_set_logical_port_params (struct pp2_port *port, struct pp2_ppio_params *params)
{
  int rc;

  rc = pp2_cls_mng_add_default_flow (port);
  if (rc)
    {
...
pp2_port_open_uio (struct pp2_port *port)
{
  ...
  snprintf (tmp_name, max_uio_port_str_size, UIO_PORT_STRING, port->parent->id, pp2_port_id (port));
  uio_info = uio_find_devices_byname (tmp_name);
  if (!uio_info)
    {
...
pp2_port_close (struct pp2_port *port)
{
  ...
  if (NOT_LPBK_PORT (port) && pp2_port_type (port) == PP2_PPIO_T_NIC)
    {
      if (lnx_is_mainline (lnx_id))
...
pp2_ppio_init (vnet_dev_port_t *port, struct pp2_ppio_params *params)
{
  ...
  if (params->type == PP2_PPIO_T_LOG)
    {
      rc = pp2_cls_mng_set_logical_port_params (pp_port, params);
      if (rc)
	{
	       ...
	       u16 *num)
{
  ...
  dm_if = pp2_dm_if_get (pp_port, hif);
  ...
  if (unlikely (desc_sent < desc_req))
    {
      pr_debug ("[%s] pp2_id %u Port %u qid %u, send_request %u sent %u!\n", __func__,
		pp_port->parent->id, pp2_port_id (pp_port), qid, *num, desc_sent);
      *num = desc_sent;
    }
...
{
  if (pp2_port_type (port) == PP2_PPIO_T_NIC)
    {
		      ...
		      struct pp2_cls_tbl_action *action, int lkp_type)
{
  ...
  port = params->default_act.cos->port;
  inst = port->parent;
...
pp2_cls_rss_mode_flows_set (struct pp2_port *port, int rss_mode)
{
  ...
  int lkp_type =
    (pp2_port_type (port) == PP2_PPIO_T_LOG) ? MVPP2_CLS_LKP_MUSDK_LOG_HASH : MVPP2_CLS_LKP_HASH;
...
pp2_prs_create_log_port_entry (struct pp2_port *port, u32 index, enum pp2_ppio_cls_target target)
{
  ...
  /* Update port mask */
  mv_pp2x_prs_tcam_port_set (&pe_log_port, pp2_port_id (port), true);
  ...
  mv_pp2x_prs_tcam_port_set (&pe_orig, pp2_port_id (port), false);
			   ...
			   struct pp2_cls_tbl_action *action, int lkp_type)
{
  ...
  if (lkp_type == MVPP2_CLS_LKP_MUSDK_DSCP_PRI || lkp_type == MVPP2_CLS_LKP_MUSDK_VLAN_PRI)
	...
	tbl_sel = MVPP2_QOS_TBL_SEL_DSCP;
      ...
      qos_info->qos_tbl_index = QOS_LOG_PORT_TABLE_OFF (pp2_port_id (port));
			 ...
			 int lkpid, int lkpid_attr, int set)
{
  ...
  fl_rls->fl->port_bm = (1 << pp2_port_id (port));
  fl_rls->fl->lu_type = lkp_type;
  fl_rls->fl->prio = pp2_cls_mng_lkp_type_to_prio (lkp_type);
  fl_rls->fl->engine = engine;
  fl_rls->fl->udf7 =
    (pp2_port_type (port) == PP2_PPIO_T_LOG) ? MVPP2_CLS_MUSDK_LOG_UDF7 : MVPP2_CLS_MUSDK_NIC_UDF7;
		       ...
		       struct pp2_cls_tbl_rule *rule, struct pp2_port *port)
{
  ...
  rule_port->port_value = (1 << pp2_port_id (port));
...
mv_pp2x_cls_c2_qos_tbl_fill_array (struct pp2_port *port, u8 tbl_sel, uint8_t tc_values[])
{
  ...
  if (pp2_port_type (port) == PP2_PPIO_T_LOG)
    qos_entry.tbl_id = QOS_LOG_PORT_TABLE_OFF (pp2_port_id (port));
  else
...
pp2_cls_mng_tbl_init (struct pp2_cls_tbl_params *params, struct pp2_cls_tbl **tbl, int lkp_type)
{
  ...
  fl_rls->fl[0].port_bm = (1 << pp2_port_id (port));
  ...
  tbl_node->params.default_act.cos->port = params->default_act.cos->port;
			  ...
			  u32 ri_mask)
{
  ...
  if (!pp2_cls_db_prs_match_list_log_port_check (inst))
	...
	default:
      ...
      /* Update port mask */
      mv_pp2x_prs_tcam_port_set (&pe, pp2_port_id (port), true);
