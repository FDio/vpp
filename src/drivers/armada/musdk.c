...
mv_pp2x_prs_flow_id_attr_init (void)
{
  ...
  struct mv_pp2x_prs_flow_id *prs_flow_id_array = mv_pp2x_prs_flow_id_array;
...
u32
musdk_mem_id (u32 pp2_id)
{
  return pp2_get_mem_id (pp2_id);
}
...
pp2_cls_rss_mode_flows_set (struct pp2_port *port, int rss_mode)
{
  ...
  for (lkpid = MVPP2_PRS_FL_START; lkpid < MVPP2_PRS_FL_LAST; lkpid++)
	...
	{
	  if (rss_mode != PP2_PPIO_HASH_T_2_TUPLE && rss_mode != PP2_PPIO_HASH_T_5_TUPLE)
	    {
