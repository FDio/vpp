...
mvpp2_port_init (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  log_debug (dev, "mvpp2_bpool_init(bpool %u) pool-%u:%u ok", index, md->pp_id, mp->bpool.id);

  foreach_vnet_dev_port_rx_queue (q, port)
    {
      mvpp2_rxq_t *prq = vnet_dev_get_rx_queue_data (q);
      struct pp2_rxq *rxq = mp->tc.rx_qs + q->queue_id;
	...
	prq->desc_ptrs[i] = prq->descs + i;
      ...
      rxq->ring_size = q->size;
      rxq->tc_pools_mem_id_index = 0;
    }
  ...
  mp->num_tcs = 1;
  mp->tc.tc_config = (struct pp2_ppio_tc_config) {
  ...
  mp->hash_type = n_rxq > 1 ? hash_type : PP2_PPIO_HASH_T_NONE;
  ...
  if (pp2_cls_mng_eth_start_header_params_set (port, eth_start_hdr))
    {
  ...
  if (pp2_cls_mng_modify_default_flows (port, 0))
    {
...
mvpp2_port_poll (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if (mrv)
    {
      log_debug (dev, "mvpp2_gop_get_link_info: failed, rv %d", mrv);
      return;
    }
  ...
  if (mp->is_enabled)
    {
      ...
      log_debug (port->dev, "pp2_port_stop: %u", port->port_id);
      pp2_port_ingress_disable (port);
