...
mvpp2_port_set_loopback (vnet_dev_port_t *port, int en)
{
  ...
  val = mvpp2_reg_rd (base, PP2_GMAC_PORT_CTRL1_REG);
  if (en)
    val |= PP2_GMAC_PORT_CTRL1_GMII_LOOPBACK_MASK;
  else
    val &= ~PP2_GMAC_PORT_CTRL1_GMII_LOOPBACK_MASK;
  mvpp2_reg_wr (base, PP2_GMAC_PORT_CTRL1_REG, val);
  ...
  if (mac_num == 0 || (mac_num == 2 && pp2_version == MVPP2_VER_PP23))
    {
      ...
      control.as_u32 = mvpp2_reg_rd (base, PP2_XLG_PORT_MAC_CTRL1_REG);
      control.mac_loopback = en;
      control.xgmii_loopback = en;
      mvpp2_reg_wr (base, PP2_XLG_PORT_MAC_CTRL1_REG, control.as_u32);
    }
