...
mvpp2_init (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  mrv = mvpp2_uio_init (dev, &uio_num);
  if (!mrv)
    {
      mrv = mvpp2_uio_map (dev, uio_num, "pp", &md->pp_map_size, (void **) &mem_base);
      if (!mrv)
	{
	  md->pp_base = mem_base;
	  mrv = mvpp2_uio_map (dev, uio_num, "mspg", &md->mspg_map_size, (void **) &mem_base);
	}
      if (!mrv)
	{
	  md->gop_hw_mspg = mem_base;
	  mrv = mvpp2_uio_map (dev, uio_num, "cm3", &md->cm3_map_size, (void **) &mem_base);
	  if (mrv)
	    {
  ...
  if (mrv)
    {
      log_err (dev, "UIO initialization failed, err %d", mrv);
  ...
  for (i = 0; i < n_threads; i++)
      ...
      if (mrv)
	{
      ...
      log_debug (dev, "mvpp2_hif_init(hif %u, thread %u) ok", index, i);
    }
  ...
  log_debug (dev, "mvpp2_bpool_init(bpool %u:%u) ok", md->pp_id, index);
