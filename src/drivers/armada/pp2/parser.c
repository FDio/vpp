...
static int
mvpp2_prs_eth_start_hdr_set (vnet_dev_port_t *port, enum mvpp2_port_eth_start_hdr eth_start_hdr)
{
  ...
  switch (eth_start_hdr)
    ...
    case MVPP2_PORT_HDR_ETH_EXT_DSA:
      ...
      break;
    default:
      log_err (dev, "invalid eth_start_hdr, eth_start_hdr = %d", eth_start_hdr);
      return -EINVAL;
    }

  /* Write to register */
  log_debug (dev, "program port %u eth start-header mode %u: mh %08x", mvpp2_port_id (port),
	     eth_start_hdr, reg_val);
...
static int
mv_pp2x_prs_hw_write (vnet_dev_t *dev, uintptr_t hif_base, struct mv_pp2x_prs_entry *pe)
{
  int i;
...
mvpp2_prs_port_update (vnet_dev_port_t *port, u32 add, u32 tid, u32 ri, u32 ri_mask)
{
  ...
  mv_pp2x_prs_hw_write (dev, hif_base, &pe);

  return 0;
}

static int
mvpp2_prs_eth_start_header_set (vnet_dev_port_t *port, enum mvpp2_port_eth_start_hdr mode)
{
  u32 type;
  int rc;
  u32 nri = 0, ri_mask = 0;
				   ...
				   enum mvpp2_port_eth_start_hdr eth_start_hdr)
{
  ...
  if (rc)
    {
      log_err (dev, "%s(%d) mvpp2_prs_eth_start_header_set fail", __func__, __LINE__);
      return -EINVAL;
    }
...
mv_pp2x_prs_mac_da_accept (vnet_dev_port_t *port, const u8 *da, bool add)
{
  ...
  if (pmap == 0)
	...
	return -EINVAL;
      ...
      mv_pp2x_prs_hw_inv (dev, mp->hif_base, pe.index);
  ...
  mv_pp2x_prs_hw_write (dev, mp->hif_base, &pe);
...
mvpp2_port_add_mac_addr (vnet_dev_port_t *port, const eth_addr_t addr)
{
  ...
  if (mv_check_eaddr_mc (addr))
      ...
      if (vec_len (mp->added_uc_addrs) == MVPP2_PORT_MAX_UC_ADDR)
	{
      ...
      if (fd == -1)
	{
	  log_debug (dev, "unable to open sysfs, updating prs_table internally instead");
      ...
      else
	{
	  ...
	  if (rc < 0)
	    {
	      log_err (dev, "unable to write to sysfs");
	      return VNET_DEV_ERR_INTERNAL;
	    }
	}
