...
static void
mv_pp2x_prs_tcam_port_set (mvpp2_parser_entry_t *pe, unsigned int port, bool add)
{
  int enable_off = HW_BYTE_OFFS (MVPP2_PRS_TCAM_EN_OFFS (MVPP2_PRS_TCAM_PORT_BYTE));
...
static int
mv_pp2x_prs_hw_read (uintptr_t hif_base, mvpp2_parser_entry_t *pe)
{
  int i;
  ...
  mvpp2_reg_wr (hif_base, MVPP2_PRS_TCAM_IDX_REG, pe->index);

  pe->tcam.word[MVPP2_PRS_TCAM_INV_WORD] =
    mvpp2_reg_rd (hif_base, MVPP2_PRS_TCAM_DATA_REG (MVPP2_PRS_TCAM_INV_WORD));
  if (pe->tcam.word[MVPP2_PRS_TCAM_INV_WORD] & MVPP2_PRS_TCAM_INV_MASK)
    return MVPP2_PRS_TCAM_ENTRY_INVALID;

  for (i = 0; i < MVPP2_PRS_TCAM_WORDS; i++)
    pe->tcam.word[i] = mvpp2_reg_rd (hif_base, MVPP2_PRS_TCAM_DATA_REG (i));

  /* Write sram index - indirect access */
  mvpp2_reg_wr (hif_base, MVPP2_PRS_SRAM_IDX_REG, pe->index);
  for (i = 0; i < MVPP2_PRS_SRAM_WORDS; i++)
    pe->sram.word[i] = mvpp2_reg_rd (hif_base, MVPP2_PRS_SRAM_DATA_REG (i));

  return 0;
}

static int
mv_pp2x_prs_hw_write (vnet_dev_t *dev, uintptr_t hif_base, mvpp2_parser_entry_t *pe)
{
  int i;
  ...
  mvpp2_reg_wr (hif_base, MVPP2_PRS_SRAM_IDX_REG, pe->index);
  for (i = 0; i < MVPP2_PRS_SRAM_WORDS; i++)
    mvpp2_reg_wr (hif_base, MVPP2_PRS_SRAM_DATA_REG (i), pe->sram.word[i]);

  /* Write tcam index - indirect access */
  mvpp2_reg_wr (hif_base, MVPP2_PRS_TCAM_IDX_REG, pe->index);
  for (i = 0; i < MVPP2_PRS_TCAM_WORDS; i++)
    mvpp2_reg_wr (hif_base, MVPP2_PRS_TCAM_DATA_REG (i), pe->tcam.word[i]);
...
mvpp2_parser_init (vnet_dev_t *dev)
{
  ...
  val = mvpp2_reg_rd (hif_base, MVPP2_PRS_TCAM_CTRL_REG);
...
static void
mv_pp2x_prs_hw_inv (vnet_dev_t *dev, uintptr_t hif_base, int index)
{
  log_debug (dev, "invalidate parser entry %u", index);

  /* Write index - indirect access */
  mvpp2_reg_wr (hif_base, MVPP2_PRS_TCAM_IDX_REG, index);
  mvpp2_reg_wr (hif_base, MVPP2_PRS_TCAM_DATA_REG (MVPP2_PRS_TCAM_INV_WORD),
...
mv_pp2x_prs_mac_da_accept (vnet_dev_port_t *port, const u8 *da, bool add)
{
  ...
  tid =
    mvpp2x_prs_mac_da_range_find (dev, md, md->pp_base, BIT (mvpp2_port_id (port)), da, mask, 0);
  ...
  if (tid < 0)
    ...
    {
      pe.index = tid;
      mv_pp2x_prs_hw_read (md->pp_base, &pe);
    }
  ...
  if (pmap == 0)
	...
	return -EINVAL;
      ...
      mv_pp2x_prs_hw_inv (dev, md->pp_base, pe.index);
  ...
  mv_pp2x_prs_hw_write (dev, md->pp_base, &pe);
