...
#define MVPP2_MAX_NUM_PUT_BUFFS 8192
...
void
mvpp2_hif_release_descs (vlib_main_t *vm, vnet_dev_t *dev, u16 num_buffs,
			 struct pp2_ppio_desc descs[])
{
  ...
  do
	...
	{
	  u32 occ_desc =
	    mvpp2_hif_reg_read (thread->hif_base, MVPP2_AGGR_TXQ_STATUS_REG (thread->hif_id));
      ...
      if (*desc_rsrvd < num_txds)
	{
	  ...
	  mvpp2_hif_reg_write (thread->hif_base, MVPP2_TXQ_RSVD_REQ_REG, req_val);
	  asm volatile ("dsb sy" : : : "memory");
	  *desc_rsrvd += mvpp2_hif_reg_read (thread->hif_base, MVPP2_TXQ_RSVD_RSLT_REG) &
      ...
      if (block_size < num_txds)
	{
      ...
      asm volatile ("dsb st" : : : "memory");
      mvpp2_hif_reg_write (thread->hif_base, MVPP2_AGGR_TXQ_UPDATE_REG, num_txds);
...
mvpp2_hif_init (vlib_main_t *vm, vnet_dev_t *dev, u32 thread_index, u8 id, u32 out_size)
{
  ...
  descs = clib_mem_alloc (MVPP2_MAX_NUM_PUT_BUFFS * sizeof (*descs));
  clib_memset (descs, 0, MVPP2_MAX_NUM_PUT_BUFFS * sizeof (*descs));
