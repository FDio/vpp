...
mvpp2_hif_aggr_queue_config (mvpp2_hif_t *hif, uintptr_t addr, u32 size)
{
  ...
  mvpp2_hif_reg_wr (hif, MVPP2_AGGR_TXQ_INIT (hif->id), 0x01);
  mvpp2_hif_reg_wr (hif, MVPP2_AGGR_TXQ_DESC_ADDR_REG (hif->id), addr >> MVPP22_DESC_ADDR_SHIFT);
  mvpp2_hif_reg_wr (hif, MVPP2_AGGR_TXQ_DESC_SIZE_REG (hif->id), size);
}
...
mvpp2_hif_alloc (vlib_main_t *vm, vnet_dev_t *dev, mvpp2_hif_t *hif, u32 sz)
{
  ...
  *hif = (mvpp2_hif_t) {
    .id = hif_slot,
    .base = mvpp2_hif_base (md, hif_slot),
    .n_desc = sz,
  };
  hif->descs = vlib_physmem_alloc_aligned (vm, sz * MVPP2_DESC_ALIGNED_SIZE, MVPP2_DESC_Q_ALIGN);
  if (!hif->descs)
    {
  ...
  desc_phys = vlib_physmem_get_pa (vm, hif->descs);
  ...
  if (hif->descs)
    {
      ...
      vlib_physmem_free (vm, hif->descs);
