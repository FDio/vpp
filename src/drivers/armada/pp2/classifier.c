...
mvpp2_cls_c3_cpu_done (uintptr_t hif_base)
{
  ...
  state.as_u32 = mvpp2_reg_rd (hif_base, MVPP2_CLS3_STATE_REG);
...
static void
mv_pp2x_cls_c2_hw_read (uintptr_t hif_base, int index, struct mv_pp2x_cls_c2_entry *c2)
{
  ...
  /* write index reg */
  mvpp2_reg_wr (hif_base, MVPP2_CLS2_TCAM_IDX_REG, index);

  /* read invalid bit */
  reg_val = mvpp2_reg_rd (hif_base, MVPP2_CLS2_TCAM_INV_REG);
  ...
  for (tcm_idx = 0; tcm_idx < MVPP2_CLS_C2_TCAM_WORDS; tcm_idx++)
    c2->tcam.words[tcm_idx] = mvpp2_reg_rd (hif_base, MVPP2_CLS2_TCAM_DATA_REG (tcm_idx));

  c2->sram.regs.action_tbl.as_u32 = mvpp2_reg_rd (hif_base, MVPP2_CLS2_ACT_DATA_REG);
  c2->sram.regs.actions.as_u32 = mvpp2_reg_rd (hif_base, MVPP2_CLS2_ACT_REG);
  c2->sram.regs.qos_attr.as_u32 = mvpp2_reg_rd (hif_base, MVPP2_CLS2_ACT_QOS_ATTR_REG);
  c2->sram.regs.hwf_attr = mvpp2_reg_rd (hif_base, MVPP2_CLS2_ACT_HWF_ATTR_REG);
  c2->sram.regs.rss_attr.as_u32 = mvpp2_reg_rd (hif_base, MVPP2_CLS2_ACT_DUP_ATTR_REG);
  c2->sram.regs.seq_attr = mvpp2_reg_rd (hif_base, MVPP21_CLS2_ACT_SEQ_ATTR_REG);
}

static void
mv_pp2x_cls_c2_hw_write (vnet_dev_t *dev, uintptr_t hif_base, int index,
			 struct mv_pp2x_cls_c2_entry *c2)
{
  ...
  /* write valid bit */
  c2->inv = 0;
  mvpp2_reg_wr (hif_base, MVPP2_CLS2_TCAM_INV_REG, ((c2->inv) << MVPP2_CLS2_TCAM_INV_INVALID_OFF));

  for (tcm_idx = 0; tcm_idx < MVPP2_CLS_C2_TCAM_WORDS; tcm_idx++)
    mvpp2_reg_wr (hif_base, MVPP2_CLS2_TCAM_DATA_REG (tcm_idx), c2->tcam.words[tcm_idx]);

  /* write action_tbl CLSC2_ACT_DATA */
  mvpp2_reg_wr (hif_base, MVPP2_CLS2_ACT_DATA_REG, c2->sram.regs.action_tbl.as_u32);

  /* write actions CLSC2_ACT */
  mvpp2_reg_wr (hif_base, MVPP2_CLS2_ACT_REG, c2->sram.regs.actions.as_u32);

  /* write qos_attr CLSC2_ATTR0 */
  mvpp2_reg_wr (hif_base, MVPP2_CLS2_ACT_QOS_ATTR_REG, c2->sram.regs.qos_attr.as_u32);

  /* write hwf_attr CLSC2_ATTR1 */
  mvpp2_reg_wr (hif_base, MVPP2_CLS2_ACT_HWF_ATTR_REG, c2->sram.regs.hwf_attr);

  /* write rss_attr CLSC2_ATTR2 */
  mvpp2_reg_wr (hif_base, MVPP2_CLS2_ACT_DUP_ATTR_REG, c2->sram.regs.rss_attr.as_u32);
}
...
mvpp2_cls_rss_enable (vnet_dev_port_t *port, int en)
{
  ...
  for (index = 0; index < MVPP2_CLS_C2_TCAM_SIZE; index++)
    {
      mv_pp2x_cls_c2_hw_read (md->pp_base, index, &c2);
      if (c2.inv)
	continue;
      ...
      if (port_id == (1 << mvpp2_port_id (port)))
	{
	  ...
	  mv_pp2x_cls_c2_hw_write (dev, md->pp_base, index, &c2);
	  n_updated++;
	}
    }
...
mv_pp2x_cls_c2_hw_inv (uintptr_t hif_base, int index)
{
  ...
  /* set invalid bit */
  mvpp2_reg_wr (hif_base, MVPP2_CLS2_TCAM_INV_REG, (1 << MVPP2_CLS2_TCAM_INV_INVALID_OFF));

  /* trigger */
  mvpp2_reg_wr (hif_base, MVPP2_CLS2_TCAM_DATA_REG (4), 0);
}
...
mvpp2_cls_c3_hit_cntrs_clear_all (vnet_dev_t *dev, uintptr_t hif_base)
{
  ...
  mvpp2_reg_wr (hif_base, MVPP2_CLS3_CLEAR_COUNTERS_REG, MVPP2_CLS3_CLEAR_ALL);
...
mvpp2_cls_c3_hw_del (vnet_dev_t *dev, uintptr_t hif_base, int index)
{
  ...
  mvpp2_reg_wr (hif_base, MVPP2_CLS3_HASH_OP_REG, operation.as_u32);
...
static void
mvpp2_cls_c2_reset (vnet_dev_t *dev, mvpp2_device_t *md)
{
  int index;
  uintptr_t hif_base = md->pp_base;
