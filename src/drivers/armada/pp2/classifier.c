...
mv_pp2x_range_validate (vnet_dev_t *dev, int value, int min, int max)
{
  if (((value) > (max)) || ((value) < (min)))
    {
      log_err (dev, "%s: value 0x%X (%d) is out of range [0x%X , 0x%X].", __func__, (value),
	       (value), (min), (max));
...
{
  if (!c2 || rss_en >= (1 << MVPP2_CLS2_ACT_DUP_ATTR_RSSEN_BITS))
    return -EINVAL;

  c2->sram.regs.actions &= ~MVPP2_CLS2_ACT_RSS_MASK;
  c2->sram.regs.actions |= 3 /* update and lock */ << MVPP2_CLS2_ACT_RSS_OFF;
...
mv_pp2x_ptr_validate (vnet_dev_t *dev, const void *ptr)
{
  if (!ptr)
    {
      log_err (dev, "%s: null pointer.", __func__);
      return VNET_DEV_ERR_INVALID_ARG;
    }
  return VNET_DEV_OK;
}
			 ...
			 struct mv_pp2x_cls_c2_entry *c2)
{
  ...
  log_debug (dev,
	     ...
	     c2->tcam.words[4], c2->sram.regs.action_tbl, c2->sram.regs.actions,
	     c2->sram.regs.qos_attr, c2->sram.regs.hwf_attr, c2->sram.regs.rss_attr);
...
mvpp2_cls_rss_enable (vnet_dev_port_t *port, int en)
{
  ...
  for (index = 0; index < MVPP2_CLS_C2_TCAM_SIZE; index++)
	...
	{
	  /* Set RSS */
	  rc = mv_pp2x_cls_c2_rss_set (&c2, en);
	  if (rc)
...
static int
mvpp2_c2_set_default_coloring (vnet_dev_port_t *port, int clear __clib_unused)
{
  vnet_dev_t *dev = port->dev;
  mvpp2_device_t *md = vnet_dev_get_data (port->dev);
  int index;
  ...
  for (index = 0; index < MVPP2_CLS_C2_TCAM_SIZE; index++)
      ...
      if (c2.inv == 0 && port_id == (1 << mvpp2_port_id (port)))
	{
	  ...
	  c2.sram.regs.actions |= 2 /* green */ << MVPP2_CLS2_ACT_COLOR_OFF;
...
mvpp2_cls_mng_set_coloring (vnet_dev_port_t *port, int clear)
{
  ...
  if (rc)
    {
      log_err (dev, "%s(%d) mvpp2_c2_set_default_coloring fail", __func__, __LINE__);
      return -EINVAL;
    }
...
mvpp2_cls_c3_hit_cntrs_clear_all (vnet_dev_t *dev, uintptr_t hif_base)
{
  ...
  while (!mvpp2_cls_c3_hit_cntr_clear_done (hif_base))
    if (++iter >= RETRIES_EXCEEDED)
      {
	log_err (dev, "%s:Error - retries exceeded.", __func__);
	return -EBUSY;
      }
...
mvpp2_cls_mng_modify_default_flows (vnet_dev_port_t *port, int clear)
{
  ...
  if (rc)
    {
      log_err (dev, "%s(%d) mvpp2_cls_mng_set_coloring fail", __func__, __LINE__);
      return -EINVAL;
    }
...
static int
mv_pp2x_cls_c2_queue_high_set (struct mv_pp2x_cls_c2_entry *c2, int queue, int from)
{
  if (!c2 || queue >= (1 << MVPP2_CLS2_ACT_QOS_ATTR_QH_BITS))
    return -EINVAL;

  /*set command */
  c2->sram.regs.actions &= ~MVPP2_CLS2_ACT_QH_MASK;
  c2->sram.regs.actions |= 3 /* update and lock */ << MVPP2_CLS2_ACT_QH_OFF;
...
static int
mv_pp2x_cls_c2_queue_low_set (struct mv_pp2x_cls_c2_entry *c2, int queue, int from)
{
  if (!c2 || queue >= (1 << MVPP2_CLS2_ACT_QOS_ATTR_QL_BITS))
    return -EINVAL;

  /*set command */
  c2->sram.regs.actions &= ~MVPP2_CLS2_ACT_QL_MASK;
  c2->sram.regs.actions |= 3 /* update and lock */ << MVPP2_CLS2_ACT_QL_OFF;
...
mvpp2_c2_config_queue (struct mv_pp2x_cls_c2_entry *c2, u16 queue, int from)
{
  ...
  rc = mv_pp2x_cls_c2_queue_low_set (c2, q_low, from);
  if (rc)
    return rc;
  rc = mv_pp2x_cls_c2_queue_high_set (c2, q_high, from);
  if (rc)
    return rc;
...
mvpp2_c2_config_default_queue (vnet_dev_port_t *port, u16 queue)
{
  ...
  for (index = 0; index < MVPP2_CLS_C2_TCAM_SIZE; index++)
      ...
      if (lkp_type == MVPP2_CLS_LKP_DEFAULT)
	{
	  ...
	  log_debug (dev, "Writing index %#x, queue %d, from %d", index, queue,
  ...
  for (pri = 0; pri < line_num; pri++)
    {
       ...
       */
      queue = mvpp2_port_get_rxq_hw_id (port, 0);
      color = MVPP2_PORT_COLOR_GREEN;
      log_debug (dev, "tc_val[%d] %d, queue %d, color %d", pri, tc_values[pri], queue, (int) color);
...
mvpp2_cls_mng_config_default_cos_queue (vnet_dev_port_t *port)
{
  ...
  mvpp2_cls_mng_qos_tbl_dflt_set (port, rxq0_hw_id);
}
...
mvpp2x_cls_oversize_rxq_set (vnet_dev_port_t *port)
{
  ...
  mvpp2_reg_write (mp->hif_base, MVPP2_CLS_OVERSIZE_RXQ_LOW_REG (mp->id),
