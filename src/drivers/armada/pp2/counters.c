...
static int
mvpp2_netdev_ioctl (u32 request, struct ifreq *ifr)
{
...
static int
mvpp2_port_statistics_init (mvpp2_port_t *mp)
{
  ...
  strcpy (ifr.ifr_name, mp->linux_name);
  sset_info->cmd = ETHTOOL_GSSET_INFO;
  sset_info->sset_mask = 1ULL << ETH_SS_STATS;
  ifr.ifr_data = (char *) sset_info;
  if (mvpp2_netdev_ioctl (SIOCETHTOOL, &ifr))
    return -EIO;
  ...
  mp->stats_name = clib_mem_alloc_or_null (sizeof (*mp->stats_name) + len * ETH_GSTRING_LEN);
  if (!mp->stats_name)
    return -ENOMEM;
...
static int
mvpp2_read_port_statistics (vnet_dev_port_t *port, struct pp2_ppio_statistics *stats)
{
  ...
  if (mvpp2_port_statistics_init (mp))
    return -EIO;

  size = sizeof (*values) + mp->stats_name->len * sizeof (u64);
  values = clib_mem_alloc_or_null (size);
  if (!values)
    return -ENOMEM;
...
mvpp2_counter_read (uintptr_t cpu_slot, u32 reg)
{
  u32 value;
...
mvpp2_counter_write (uintptr_t cpu_slot, u32 reg, u32 value)
{
  value = htole32 (value);
			   ...
			   struct pp2_ppio_inq_statistics *stats, int reset)
{
  ...
  ASSERT (tc == 0);
  mvpp2_counter_write (mp->cpu_slot, MVPP2_CNT_IDX_REG, mrq->hw_id);
  mrq->stats.enq_desc += mvpp2_counter_read (mp->cpu_slot, MVPP2_RX_DESC_ENQ_REG);
  value = mvpp2_counter_read (mp->cpu_slot, MVPP2_RX_PKT_FULLQ_DROP_REG);
  mrq->stats.drop_fullq =
    (u64) mrq->stats.drop_fullq + value > UINT_MAX ? UINT_MAX : mrq->stats.drop_fullq + value;
  value = mvpp2_counter_read (mp->cpu_slot, MVPP2_RX_PKT_EARLY_DROP_REG);
  mrq->stats.drop_early =
    (u32) mrq->stats.drop_early + value > USHRT_MAX ? USHRT_MAX : mrq->stats.drop_early + value;
  value = mvpp2_counter_read (mp->cpu_slot, MVPP2_RX_PKT_BM_DROP_REG);
  mrq->stats.drop_bm =
...
static int
mvpp2_read_txq_statistics (vnet_dev_port_t *port, u8 qid, struct pp2_ppio_outq_statistics *stats,
			   int reset)
{
  mvpp2_port_t *mp = vnet_dev_get_port_data (port);
  ...
  mvpp2_counter_write (mp->cpu_slot, MVPP2_CNT_IDX_REG, MVPP2_CNT_IDX_TX (mp->id, mtq->log_id));
  mtq->stats.enq_desc += mvpp2_counter_read (mp->cpu_slot, MVPP2_TX_DESC_ENQ_REG);
  mtq->stats.enq_dec_to_ddr += mvpp2_counter_read (mp->cpu_slot, MVPP2_TX_DESC_ENQ_TO_DRAM_REG);
  mtq->stats.enq_buf_to_ddr += mvpp2_counter_read (mp->cpu_slot, MVPP2_TX_BUF_ENQ_TO_DRAM_REG);
  mtq->stats.deq_desc += mvpp2_counter_read (mp->cpu_slot, MVPP2_TX_PKT_DQ_REG);
  ...
  foreach_vnet_dev_port_rx_queue (q, port)
    {
      ...
      pp2_ppio_inq_get_statistics (port, 0, q->queue_id, &rx_stats, reset);
  ...
  foreach_vnet_dev_port_tx_queue (q, port)
    {
      ...
      pp2_ppio_outq_get_statistics (port, q->queue_id, &tx_stats, reset);
...
{
  pp2_ppio_get_statistics (port, 0, 1);
}
...
mvpp2_txq_clear_counters (vlib_main_t *vm, vnet_dev_tx_queue_t *q)
{
  ...
  pp2_ppio_outq_get_statistics (q->port, q->queue_id, &stats, 1);
}
