...
mvpp2_port_statistics_init (vnet_dev_port_t *port)
{
  ...
  if (mvpp2_netdev_ioctl (dev, SIOCETHTOOL, &ifr))
    return -EIO;
...
static int
mvpp2_read_port_statistics (vnet_dev_port_t *port, mvpp2_port_statistics_t *stats)
{
  vnet_dev_t *dev = port->dev;
  mvpp2_port_t *mp = vnet_dev_get_port_data (port);
...
static int
mvpp2_read_rxq_statistics (vnet_dev_port_t *port, u8 tc, u8 qid, mvpp2_rxq_statistics_t *stats,
			   int reset)
{
  vnet_dev_t *dev = port->dev;
  ...
  mrq->stats.enq_desc += mvpp2_dev_reg_rd_relax (dev, MVPP2_RX_DESC_ENQ_REG);
  value = mvpp2_dev_reg_rd_relax (dev, MVPP2_RX_PKT_FULLQ_DROP_REG);
  mrq->stats.drop_fullq =
    (u64) mrq->stats.drop_fullq + value > UINT_MAX ? UINT_MAX : mrq->stats.drop_fullq + value;
  value = mvpp2_dev_reg_rd_relax (dev, MVPP2_RX_PKT_EARLY_DROP_REG);
  mrq->stats.drop_early =
    (u32) mrq->stats.drop_early + value > USHRT_MAX ? USHRT_MAX : mrq->stats.drop_early + value;
  value = mvpp2_dev_reg_rd_relax (dev, MVPP2_RX_PKT_BM_DROP_REG);
  mrq->stats.drop_bm =
...
static int
mvpp2_read_txq_statistics (vnet_dev_port_t *port, u8 qid, mvpp2_txq_statistics_t *stats, int reset)
{
  vnet_dev_t *dev = port->dev;
  mvpp2_port_t *mp = vnet_dev_get_port_data (port);
  ...
  mtq->stats.enq_desc += mvpp2_dev_reg_rd_relax (dev, MVPP2_TX_DESC_ENQ_REG);
  mtq->stats.enq_dec_to_ddr += mvpp2_dev_reg_rd_relax (dev, MVPP2_TX_DESC_ENQ_TO_DRAM_REG);
  mtq->stats.enq_buf_to_ddr += mvpp2_dev_reg_rd_relax (dev, MVPP2_TX_BUF_ENQ_TO_DRAM_REG);
  mtq->stats.deq_desc += mvpp2_dev_reg_rd_relax (dev, MVPP2_TX_PKT_DQ_REG);
...
static int
mvpp2_rxq_get_statistics (vnet_dev_port_t *port, u8 tc, u8 qid, mvpp2_rxq_statistics_t *stats,
			  int reset)
{
...
static int
mvpp2_txq_get_statistics (vnet_dev_port_t *port, u8 qid, mvpp2_txq_statistics_t *stats, int reset)
{
  return mvpp2_read_txq_statistics (port, qid, stats, reset);
}

static int
mvpp2_port_get_statistics (vnet_dev_port_t *port, mvpp2_port_statistics_t *stats, int reset)
{
  mvpp2_port_t *mp = vnet_dev_get_port_data (port);
