...
#define MVPP2_BPOOL_DUMMY_PKT_EFEC_OFFS (64 + MV_MH_SIZE)
...
static_always_inline void
mvpp2_bpool_desc_set_cookie (struct pp2_ppio_desc *desc, u64 cookie)
{
...
static_always_inline void
mvpp2_bpool_desc_set_pool (struct pp2_ppio_desc *desc, mvpp2_bpool_t *pool)
{
...
vnet_dev_rv_t
mvpp2_bpool_put_buffs (vlib_main_t *vm, vnet_dev_t *dev, mvpp2_buff_release_entry_t buff_entry[],
		       u16 *num)
{
  mvpp2_device_t *md = vnet_dev_get_data (dev);
  ...
  for (u32 i = 0; i < *num; i++)
	...
	desc->cmds[7] = 0;
      ...
      desc->cmds[1] = MVPP2_BPOOL_DUMMY_PKT_EFEC_OFFS;
      mvpp2_bpool_desc_set_cookie (desc, buff_entry[i].buff.cookie);
      mvpp2_bpool_desc_set_pool (desc, buff_entry[i].bpool);
      desc->cmds[3] = TXD_ERR_SUM_MASK;
    }

  mvpp2_hif_release_descs (vm, dev, *num, descs);
  return VNET_DEV_OK;
}
