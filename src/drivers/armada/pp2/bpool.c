...
mvpp2_bm_hw_buf_get (uintptr_t hif_base, u32 pool_id)
{
  ...
  mvpp2_reg_rd (hif_base, MVPP2_BM_PHY_ALLOC_REG (pool_id));
  high.as_u32 = mvpp2_reg_rd (hif_base, MVPP22_BM_PHY_VIRT_HIGH_ALLOC_REG);
  vaddr = high.virt_addr;
  vaddr <<= 32;
  vaddr |= mvpp2_reg_rd (hif_base, MVPP2_BM_VIRT_ALLOC_REG);
  return vaddr;
}
...
mvpp2_bm_pool_bufsize_set (uintptr_t hif_base, u32 pool_id, u32 buf_size)
{
  ...
  mvpp2_reg_wr (hif_base, MVPP2_POOL_BUF_SIZE_REG (pool_id), value);
}
...
mvpp2_bm_pool_flush (vnet_dev_t *dev, uintptr_t hif_base, u32 pool_id)
{
  ...
  resid_bufs =
    (mvpp2_reg_rd (hif_base, MVPP2_BM_POOL_PTRS_NUM_REG (pool_id)) & MVPP22_BM_POOL_PTRS_NUM_MASK) +
    (mvpp2_reg_rd (hif_base, MVPP2_BM_BPPI_PTRS_NUM_REG (pool_id)) & MVPP2_BM_BPPI_PTR_NUM_MASK);
  if (resid_bufs == 0)
    return 0;

  pool_bufs = mvpp2_reg_rd (hif_base, MVPP2_BM_POOL_SIZE_REG (pool_id));
  ...
  return (mvpp2_reg_rd (hif_base, MVPP2_BM_POOL_PTRS_NUM_REG (pool_id)) &
	  MVPP22_BM_POOL_PTRS_NUM_MASK) +
	 (mvpp2_reg_rd (hif_base, MVPP2_BM_BPPI_PTRS_NUM_REG (pool_id)) &
...
mvpp2_bm_hw_pool_destroy (uintptr_t hif_base, u32 pool_id)
{
  mvpp22_bm_pool_ctrl_reg_t control = {
    .as_u32 = mvpp2_reg_rd (hif_base, MVPP2_BM_POOL_CTRL_REG (pool_id)),
  };
  ...
  if (control.state)
    {
      control.stop = 1;
      mvpp2_reg_wr (hif_base, MVPP2_BM_POOL_CTRL_REG (pool_id), control.as_u32);
      do
	control.as_u32 = mvpp2_reg_rd (hif_base, MVPP2_BM_POOL_CTRL_REG (pool_id));
      while (control.state);
    }

  mvpp2_reg_wr (hif_base, MVPP2_BM_INTR_MASK_REG (pool_id), 0);
  mvpp2_reg_wr (hif_base, MVPP2_BM_INTR_CAUSE_REG (pool_id), 0);
  mvpp2_reg_wr (hif_base, MVPP2_BM_POOL_BASE_ADDR_REG (pool_id), 0);
  base_high.as_u32 = mvpp2_reg_rd (hif_base, MVPP22_BM_POOL_BASE_ADDR_HIGH_REG);
  base_high.addr = 0;
  mvpp2_reg_wr (hif_base, MVPP22_BM_POOL_BASE_ADDR_HIGH_REG, base_high.as_u32);
...
{
  mvpp22_bm_pool_base_addr_high_reg_t base_high = {
    .as_u32 = mvpp2_reg_rd (hif_base, MVPP22_BM_POOL_BASE_ADDR_HIGH_REG),
  };
...
mvpp2_bm_hw_pool_create (uintptr_t hif_base, u32 pool_id, u32 bppe_num, uintptr_t pool_phys_addr)
{
  ...
  mvpp2_reg_wr (hif_base, MVPP2_BM_POOL_BASE_ADDR_REG (pool_id), phys_lo);
  base_high.as_u32 = mvpp2_reg_rd (hif_base, MVPP22_BM_POOL_BASE_ADDR_HIGH_REG);
  ...
  pool_bufs = mvpp2_reg_rd (hif_base, MVPP2_BM_POOL_SIZE_REG (pool_id));
  if (!pool_bufs)
    mvpp2_reg_wr (hif_base, MVPP2_BM_POOL_SIZE_REG (pool_id), bppe_num);
...
mvpp2_bm_pool_reset_fc (uintptr_t base, mvpp2_bpool_t *pool)
{
  mvpp22_fc_com_reg_t fc = {
    .as_u32 = mvpp2_reg_rd (base, MSS_CP_FC_COM_REG),
  };
  u8 was_enabled = fc.enable;

  fc.enable = 0;
  mvpp2_reg_wr (base, MSS_CP_FC_COM_REG, fc.as_u32);
  mvpp2_reg_wr (base, MSS_CP_CM3_BUF_POOL_BASE + pool->id * MSS_CP_CM3_BUF_POOL_OFFS, 0);
  fc.as_u32 = mvpp2_reg_rd (base, MSS_CP_FC_COM_REG);
  fc.update = 1;
...
mvpp2_bpool_get_num_buffs (vnet_dev_t *dev, mvpp2_bpool_t *pool, u32 *num_buffs)
{
  ...
  num =
    (mvpp2_reg_rd (hif_base, MVPP2_BM_POOL_PTRS_NUM_REG (pool->id)) &
     MVPP22_BM_POOL_PTRS_NUM_MASK) +
    (mvpp2_reg_rd (hif_base, MVPP2_BM_BPPI_PTRS_NUM_REG (pool->id)) & MVPP2_BM_BPPI_PTR_NUM_MASK);
		      ...
		      mvpp2_buff_info_t *buff)
{
  ...
  paddr = mvpp2_reg_rd (hif_base, MVPP2_BM_PHY_ALLOC_REG (pool->id));
  if (!paddr)
...
mvpp2_bm_flush_pools (vnet_dev_t *dev, uintptr_t hif_base, u16 bm_pool_reserved_map)
{
  ...
  mvpp2_reg_wr (hif_base, MVPP2_BM_PRIO_CTRL_REG, 0);
  mvpp2_reg_wr (hif_base, MVPP22_BM_PHY_VIRT_HIGH_RLS_REG, 0);
}

void
mvpp2_bpool_assign (vnet_dev_port_t *port, u32 pool_id, u32 rxq_id, int is_short)
{
