		      ...
		      mvpp2_tx_desc_cmd_t *cmds, u16 *lengths, u16 n_desc, int is_simple)
{
  ...
  for (u16 i = 0; i < n_desc; i++)
    {
      u32 byte_count = lengths[i];

      lo.as_u16x8[3] = byte_count;
      hi.as_u64x2[0] = addrs[i];

      if (!is_simple)
	lo.as_u32x4[0] = cmds[i].as_u32;

      desc[i].as_u32x4[0] = lo.as_u32x4;
      desc[i].as_u32x4[1] = hi.as_u32x4;
    }
}
