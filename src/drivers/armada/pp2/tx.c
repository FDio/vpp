...
mvpp2_txq_send (vlib_main_t *vm, vnet_dev_tx_queue_t *txq,
		mvpp2_tx_desc_t *descs, u16 *num)
{
  mvpp2_device_t *md = vnet_dev_get_data (txq->port->dev);
  mvpp2_txq_t *mtq = vnet_dev_get_tx_queue_data (txq);
  mvpp2_dev_thread_t *thread = md->threads + vm->thread_index;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (mtq->n_enq)
	...
	vlib_error_count (vm, node->node_index,
			  MVPP2_TX_NODE_CTR_GET_NUM_TX_DONE, 1);
  ...
  for (d = descs, n_left = n_sent; n_left; d++, buffers++, n_left--)
    {
      ...
      mvpp2_tx_desc_set_phys_addr (d, paddr + b0->current_data - len_adj);
      mvpp2_tx_desc_set_pkt_offset (d, 0);
      mvpp2_tx_desc_set_pkt_len (d, b0->current_length + len_adj);
    }
