...
static_always_inline void
pp2_ppio_outq_desc_reset (mvpp2_tx_desc_t *desc)
{
...
static_always_inline void
pp2_ppio_outq_desc_set_phys_addr (mvpp2_tx_desc_t *desc, dma_addr_t addr)
{
  desc->cmds[4] = addr;
...
static_always_inline void
pp2_ppio_outq_desc_set_pkt_offset (mvpp2_tx_desc_t *desc, u8 offset)
{
  desc->cmds[1] = offset;
}

static_always_inline void
pp2_ppio_outq_desc_set_pkt_len (mvpp2_tx_desc_t *desc, u16 len)
{
  desc->cmds[1] = (desc->cmds[1] & ~TXD_BYTE_COUNT_MASK) | (len << 16 & TXD_BYTE_COUNT_MASK);
}

static_always_inline int
pp2_ppio_send (vlib_main_t *vm, vnet_dev_tx_queue_t *txq, mvpp2_tx_desc_t *descs, u16 *num)
{
  mvpp2_device_t *md = vnet_dev_get_data (txq->port->dev);
  mvpp2_txq_t *mtq = vnet_dev_get_tx_queue_data (txq);
  mvpp2_dev_thread_t *thread = md->threads + vm->thread_index;
