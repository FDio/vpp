...
pp2_ppio_get_num_outq_done (vlib_main_t *vm, vnet_dev_tx_queue_t *txq, u16 *num)
{
  ...
  count = pp2_tx_reg_read_relaxed (thread->hif_base, MVPP22_TXQ_SENT_REG (mtq->id));
...
pp2_ppio_send (vlib_main_t *vm, vnet_dev_tx_queue_t *txq, struct pp2_ppio_desc *descs, u16 *num)
{
  mvpp2_device_t *md = vnet_dev_get_data (txq->port->dev);
  mvpp2_txq_t *mtq = vnet_dev_get_tx_queue_data (txq);
  mvpp2_dev_thread_t *thread = md->threads + vm->thread_index;
  ...
  if (PREDICT_FALSE (*desc_rsrvd < num_txds))
    {
      ...
      u32 req_val = mtq->id << MVPP2_TXQ_RSVD_REQ_Q_OFFSET | res_req;
  ...
  for (u16 i = 0; i < block_size; i++)
    {
      descs[i].cmds[1] =
	(descs[i].cmds[1] & ~TXD_DEST_QID_MASK) | (mtq->id << 8 & TXD_DEST_QID_MASK);
  ...
  if (index < num_txds)
      ...
      for (u16 i = 0; i < block_size; i++)
	{
	  descs[index + i].cmds[1] =
	    (descs[index + i].cmds[1] & ~TXD_DEST_QID_MASK) | (mtq->id << 8 & TXD_DEST_QID_MASK);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (mtq->n_enq)
    {
      u16 n_done = 0;
      if (PREDICT_FALSE (pp2_ppio_get_num_outq_done (vm, txq, &n_done)))
  ...
  if (pp2_ppio_send (vm, txq, descs, &n_sent))
    {
      n_sent = 0;
