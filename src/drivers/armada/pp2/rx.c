...
pp2_ppio_recv (vnet_dev_port_t *port, u8 tc, u8 qid, struct pp2_ppio_desc *descs, u16 *num)
{
  ...
  if (recv_req > mrq->desc_received)
    {
      mrq->desc_received =
	mvpp2_rx_reg_read (mp->hif_base, MVPP2_RXQ_STATUS_REG (mrq->id)) & MVPP2_RXQ_OCCUPIED_MASK;
      if (recv_req > mrq->desc_received)
	{
  ...
  mvpp2_rx_reg_write (mp->hif_base, MVPP2_RXQ_STATUS_UPDATE_REG (mrq->id),
...
mrvl_pp2_bpool_put (vlib_main_t *vm, u32 node_index, vnet_dev_rx_queue_t *rxq)
{
  ...
  while (n_bufs >= MRVL_PP2_BUFF_BATCH_SZ)
      ...
      for (i = 0, e = bre; i < MRVL_PP2_BUFF_BATCH_SZ; i++, e++)
	{
      ...
      if (PREDICT_FALSE (pp2_bpool_put_buffs (vm, rxq->port->dev, bre, &n_alloc)))
	{
	  vlib_buffer_free (vm, buffer_indices, n_alloc);
	  if (node_index != CLIB_U32_MAX)
