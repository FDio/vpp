  ...
  for (u32 i = 0; i < n_desc; i++)
    {
      ...
      if (++desc_index == rxq->size)
	desc_index = 0;
    }
  mrq->desc_next_idx = desc_index;

  bt.current_data = 2;
  for (n_left = n_desc, bi = buffer_indices, pl = packet_lengths; n_left >= 4;
       bi += 2, pl += 2, n_left -= 2)
    {
      clib_prefetch_store (vlib_get_buffer (vm, bi[2]));
      clib_prefetch_store (vlib_get_buffer (vm, bi[3]));
