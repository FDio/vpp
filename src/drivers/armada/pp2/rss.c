...
mvpp2_rss_hw_tbl_set (vnet_dev_port_t *port)
{
  ...
  u32 num_queues = vnet_dev_get_port_num_rx_queues (port);
  int entry_idx;
  u16 width;
  int hw_tbl;
  ...
  hw_tbl = mvpp2_rss_hw_table_get (dev, md, num_queues);
  if (hw_tbl < 0)
    {
  ...
  for (entry_idx = 0; entry_idx < MVPP22_RSS_TBL_LINE_NUM; entry_idx++)
    {
      ...
      rss_entry.u.entry.rxq = entry_idx % num_queues;
...
mvpp2_rss_rxq_set (vnet_dev_port_t *port)
{
  ...
  for (j = 0; j < num_queues; j++)
    {
      rss_entry.u.pointer.rxq_idx = mvpp2_port_get_rxq_hw_id (port, j);
...
mvpp2_rss_table_map_alloc (vnet_dev_port_t *port)
{
  ...
  if (num_queues != 1)
      ...
      if (hw_tbl < 0)
	{
	  ...
	  mvpp2_rss_table_map_set (md, idx, idx, num_queues);
...
mvpp2_rss_port_init (vnet_dev_port_t *port, enum mvpp2_port_hash_type hash_type)
{
  ...
  if (mvpp2_rss_enable (port, rss_en))
    {
