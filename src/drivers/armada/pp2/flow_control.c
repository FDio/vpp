  ...
  for (hif_id = 0; hif_id < PP2_MAX_NUM_USED_INTERRUPTS; hif_id++)
	...
	.as_u32 = mp->saved_rx_isr[hif_id],
      ...
      log_debug (port->dev, "port %u HIF %u restored RX interrupt group start %u size %u", mp->id,
		 hif_id, start_queue, num_rx_queues);
    }
}
