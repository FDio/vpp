...
mvpp2_rxq_init (vnet_dev_rx_queue_t *q)
{
  ...
  if (vnet_dev_dma_mem_alloc (vlib_get_main (), dev, q->size * MVPP2_DESC_ALIGNED_SIZE,
  ...
  mvpp2_bpool_assign (port, mp->bpool.id, rxq->hw_id, 1 /* is_short */);
  mvpp2_bpool_assign (port, md->dummy_short_bpool.id, rxq->hw_id, 0 /* is_short */);
