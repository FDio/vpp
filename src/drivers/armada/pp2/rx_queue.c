  ...
  for (tc = 0; tc < mp->num_tcs; tc++)
      ...
      for (qid = 0; qid < mp->tc.tc_config.num_in_qs; qid++)
	{
	  ...
	  rxq->bm_pool_id[MVPP2_BM_POOL_TYPE_SHORT] =
	    tc_cfg->pools[mem_index][MVPP2_BM_POOL_TYPE_SHORT]->id;
	  rxq->bm_pool_id[MVPP2_BM_POOL_TYPE_LONG] =
	    tc_cfg->pools[mem_index][MVPP2_BM_POOL_TYPE_LONG]->id;

	  log_debug (port->dev, "port[%d:%d] tc%d rxq%d mem_index(%d)", md->pp_id, mp->id, tc,
		     rxq->hw_id, mem_index);
	  id++;
	}
    }
