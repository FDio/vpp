...
mvpp2_rxq_offset_set (vnet_dev_rx_queue_t *q)
{
  ...
  int offset = L1_CACHE_LINE_BYTES;
  u32 val;
  ...
  foreach_vnet_dev_port_rx_queue (q, port)
    {
      ...
      log_debug (port->dev, "port[%d:%d] rxq%d", md->pp_id, mp->id, rxq->hw_id);
    }
}
...
mvpp2_rxq_init (vnet_dev_rx_queue_t *q)
{
  ...
  desc_phys = vnet_dev_get_dma_addr (vlib_get_main (), dev, rxq->hw_descs);
  ...
  mvpp2_reg_write (mp->hif_base, MVPP2_RXQ_DESC_ADDR_REG, (desc_phys >> MVPP22_DESC_ADDR_SHIFT));
  ...
  mvpp2_bm_pool_assign (port, mp->bpool.id, rxq->hw_id, MVPP2_BM_POOL_TYPE_SHORT);
  mvpp2_bm_pool_assign (port, md->dummy_short_bpool.id, rxq->hw_id, MVPP2_BM_POOL_TYPE_LONG);
  log_debug (dev, "port[%d:%d] rxq[%d], short_pool(%d), long_pool(%d)\n", md->pp_id, mp->id,
	     rxq->hw_id, mp->bpool.id, md->dummy_short_bpool.id);
