...
mvpp2_loopback_egress_disable (uintptr_t base)
{
  ...
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_PORT_INDEX_REG, port_index.as_u32);
  command.as_u32 = mvpp2_reg_rd (base, MVPP2_TXP_SCHED_Q_CMD_REG);
  ...
  if (q_mask)
	...
	.disable = q_mask,
      ...
      mvpp2_reg_wr (base, MVPP2_TXP_SCHED_Q_CMD_REG, command.as_u32);
    }
...
mvpp2_loopback_init (vnet_dev_t *dev)
{
  ...
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_PORT_INDEX_REG, port_index.as_u32);
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_CMD_1_REG, 0);
  for (u32 qid = 0; qid < MVPP2_MAX_TXQ; qid++)
    {
      ptxq = (MVPP2_MAX_TCONT + MVPP2_LOOPBACK_PORT) * MVPP2_MAX_TXQ + qid;
      mvpp2_reg_wr (base, MVPP2_TXQ_SCHED_TOKEN_CNTR_REG (ptxq), 0);
    }
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_PERIOD_REG, MVPP2_LOOPBACK_TCLK_FREQ / 1000000);
  port_refill.as_u32 = mvpp2_reg_rd (base, MVPP2_TXP_SCHED_REFILL_REG);
  port_refill.period = 1;
  port_refill.tokens = MVPP2_TXP_REFILL_TOKENS_MAX;
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_REFILL_REG, port_refill.as_u32);
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_TOKEN_SIZE_REG, MVPP2_TXP_TOKEN_SIZE_MAX);
  mvpp2_reg_wr (base, MVPP2_RX_CTRL_REG (MVPP2_LOOPBACK_PORT), rx_ctrl.as_u32);
  ...
  mvpp2_reg_wr (base, MVPP2_TXQ_NUM_REG, MVPP2_LOOPBACK_TXQ_ID);
  mvpp2_reg_wr (base, MVPP2_TXQ_DESC_ADDR_LOW_REG,
		((u32) desc_phys) >> MVPP2_TXQ_DESC_ADDR_LOW_SHIFT);
  mvpp2_reg_wr (base, MVPP22_TXQ_DESC_ADDR_HIGH_REG,
		(desc_phys >> 32) & MVPP22_TXQ_DESC_ADDR_HIGH_MASK);
  mvpp2_reg_wr (base, MVPP2_TXQ_DESC_SIZE_REG, MVPP2_LOOPBACK_TXQ_SIZE & MVPP2_TXQ_DESC_SIZE_MASK);
  mvpp2_reg_wr (base, MVPP2_TXQ_INDEX_REG, 0);
  mvpp2_reg_wr (base, MVPP2_TXQ_RSVD_CLR_REG, MVPP2_LOOPBACK_TXQ_ID << MVPP2_TXQ_RSVD_CLR_OFFSET);
  value = mvpp2_reg_rd (base, MVPP2_TXQ_PENDING_REG) & ~MVPP2_TXQ_PENDING_MASK;
  mvpp2_reg_wr (base, MVPP2_TXQ_PENDING_REG, value);

  desc = MVPP2_LOOPBACK_PORT * MVPP2_MAX_TXQ * MVPP2_LOOPBACK_PREFETCH_SIZE;
  prefetch.ptr = desc;
  mvpp2_reg_wr (base, MVPP2_TXQ_PREF_BUF_REG, prefetch.as_u32);
  for (u32 i = 0; i < MVPP2_NUM_HIFS; i++)
    mvpp2_reg_rd (mvpp2_hif_base (md, i), MVPP22_TXQ_SENT_REG (MVPP2_LOOPBACK_TXQ_ID));

  mtu = 3 * (MVPP2_LOOPBACK_MTU + ETH_HLEN) * 8;
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_PORT_INDEX_REG, port_index.as_u32);
  value = mvpp2_reg_rd (base, MVPP2_TXP_SCHED_MTU_REG);
  value = (value & ~MVPP2_TXP_MTU_MAX) | clib_min (mtu, MVPP2_TXP_MTU_MAX);
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_MTU_REG, value);
  port_refill.as_u32 = mvpp2_reg_rd (base, MVPP2_TXP_SCHED_REFILL_REG);
  port_refill.tokens = MVPP2_TXP_REFILL_TOKENS_MAX;
  port_refill.period = MVPP2_TXP_REFILL_PERIOD_MIN;
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_REFILL_REG, port_refill.as_u32);
  mvpp2_reg_wr (base, MVPP2_TXP_SCHED_TOKEN_SIZE_REG, MVPP2_LOOPBACK_TXP_BUCKET_SIZE);
  mvpp2_reg_wr (base, MVPP2_TXQ_SCHED_REFILL_REG (0), queue_refill.as_u32);
  mvpp2_reg_wr (base, MVPP2_TXQ_SCHED_TOKEN_SIZE_REG (0), MVPP2_LOOPBACK_TXQ_BUCKET_SIZE);
...
mvpp2_loopback_deinit (vnet_dev_t *dev)
{
  ...
  mvpp2_loopback_egress_disable (base);
  value =
    mvpp2_reg_rd (base, MVPP2_TX_PORT_FLUSH_REG) | MVPP2_TX_PORT_FLUSH_MASK (MVPP2_LOOPBACK_PORT);
  mvpp2_reg_wr (base, MVPP2_TX_PORT_FLUSH_REG, value);
  mvpp2_reg_wr (base, MVPP2_TXQ_SCHED_TOKEN_CNTR_REG (MVPP2_LOOPBACK_TXQ_ID), 0);
  mvpp2_reg_wr (base, MVPP2_TXQ_NUM_REG, MVPP2_LOOPBACK_TXQ_ID);
  mvpp2_reg_wr (base, MVPP2_TXQ_DESC_ADDR_LOW_REG, 0);
  mvpp2_reg_wr (base, MVPP2_TXQ_DESC_SIZE_REG, 0);
  for (u32 i = 0; i < MVPP2_NUM_HIFS; i++)
    mvpp2_reg_rd (mvpp2_hif_base (md, i), MVPP22_TXQ_SENT_REG (MVPP2_LOOPBACK_TXQ_ID));
