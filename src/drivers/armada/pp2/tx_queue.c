  ...
  if (mtq->buffers == 0)
    {
      ...
      mtq->desc_counts = clib_mem_alloc_aligned (sizeof (u16) * txq->size, CLIB_CACHE_LINE_BYTES);
      clib_memset (mtq->desc_counts, 0, sizeof (u16) * txq->size);
    }
...
mvpp2_txq_init (vnet_dev_tx_queue_t *q)
{
  ...
  mvpp22_txq_pref_buf_reg_t prefetch = {
    .threshold = PP2_TXQ_PREFETCH_16 / 2,
  };
  void *desc_mem;
  u32 j, val, desc_per_txq, desc;
