...
mvpp2_txq_init (vnet_dev_tx_queue_t *q)
{
  ...
  for (j = 0; j < MVPP2_NUM_HIFS; j++)
    {
      uintptr_t hif_base = mvpp2_hif_base (md, j);

      mvpp2_reg_rd (hif_base, MVPP22_TXQ_SENT_REG (txq->hw_id));
    }
}
...
mvpp2_port_txq_deinit (vnet_dev_tx_queue_t *q)
{
  ...
  for (j = 0; j < MVPP2_NUM_HIFS; j++)
    mvpp2_reg_rd (mvpp2_hif_base (md, j), MVPP22_TXQ_SENT_REG (txq->hw_id));
}
