...
#define _(b) ((b) ? '+' : '-')
  ...
  s = format (s, "flags rss_type 0x%x ether 0x%x proto 0x%x vlan1%c vlan2%c l4csum%c", d->rss_type,
	      d->ether_type, d->proto, _ (d->vlan1), _ (d->vlan2), _ (d->l4_csum));

  s = format_newline (s, indent);
  s = format (
    s, "status dd%c eop%c macerr%c v4_sum_ng%c l4_sum_err%c l4_sum_ok%c len %u vlan 0x%x next %u",
    _ (d->dd), _ (d->eop), _ (d->macerr), _ (d->v4_sum_ng), _ (d->l4_sum_err), _ (d->l4_sum_ok),
...
atl_rx_trace (u8 *s, va_list *args)
{
  ...
  s = format_newline (s, indent);
  s = format (s, "desc: %U", format_atl_rx_desc, &t->desc);
  return s;
}
...
format_atl_rpf_info (u8 *s, va_list *args)
{
  ...
  s = format_newline (s, indent);
  s = format (s, "0x%05x %-24s: 0x%08x (hashtype 0x%04x index %u)", ATL_REG_AQ2_RPF_REDIR2,
	      "AQ2_RPF_REDIR2", rpf_redir2.as_u32, rpf_redir2.hashtype, rpf_redir2.index);
