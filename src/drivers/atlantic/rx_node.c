...
atl_rxq_refill_one_batch (vlib_main_t *vm, atl_rx_desc_t *desc_ring, u32 *bi_ring, u16 off, u8 bpi,
			  int va_dma, u32 n_descs)
{
  uword data[4 * ATL_RX_REFILL_BATCH_SZ] __clib_aligned (64);

  if (!vlib_buffer_strict_alloc_from_pool (vm, bi_ring + off, n_descs, bpi))
    return 0;
...
atl_rxq_refill (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq, int va_dma)
{
  ...
  const u32 batch_x4_sz = 4 * ATL_RX_REFILL_BATCH_SZ;
  const u32 batch_x1_sz = ATL_RX_REFILL_BATCH_SZ;

  n_refill = aq->head + size - tail;
  if (n_refill <= batch_x1_sz)
    return;

  n_refill -= batch_x1_sz;
  for (; n_refill >= batch_x1_sz;)
    {
      off = tail & mask;
      if (n_refill >= batch_x4_sz && off + batch_x4_sz <= size)
	{
	  if (!atl_rxq_refill_one_batch (vm, desc_ring, bi_ring, off, bpi, va_dma, batch_x4_sz))
	    break;
	  tail += batch_x4_sz;
	  n_refill -= batch_x4_sz;
	}
      else
	{
	  if (!atl_rxq_refill_one_batch (vm, desc_ring, bi_ring, off, bpi, va_dma, batch_x1_sz))
	    break;
	  tail += batch_x1_sz;
	  n_refill -= batch_x1_sz;
	}
...
atl_rx_one_queue (vlib_main_t *vm, vlib_node_runtime_t *node, vnet_dev_rx_queue_t *rxq)
{
  ...
  u32 n_descs = 0, n_drop = 0, n_pkts = 0, n_rx_bytes = 0, n_trace, n_enq = 0;
  u32 n_pkt_descs = 0;
  ...
  fformat (stderr, "\n");
  for (u32 p = 0, i = 0; p < n_pkts; p++)
    {
      for (u16 s = 0;; s++, i++)
	{
	  u16 slot = (desc_ring_indices[p] + s) & mask;
	  atl_rx_desc_t *d = descs + slot;
	  fformat (stderr, "[%u] %U\n  %U\n", vm->thread_index, format_atl_rx_wb_desc, d,
		   format_hexdump, buffers[i]->data, 34);
	  if (d->qw1.eop)
	    break;
	}
    }
  ...
  if (PREDICT_FALSE (n_pkts < n_descs))
    {
      typeof (buffers[0]->flags) first_flags = bt.flags | VLIB_BUFFER_TOTAL_LENGTH_VALID;
      typeof (buffers[0]->flags) next_flags = bt.flags | VLIB_BUFFER_NEXT_PRESENT;

      for (u32 p = 0, i = 0; p < n_pkts; p++)
	{
	  u16 head_slot = desc_ring_indices[p];
	  u32 tlnif = 0;
	  u16 s;

	  for (s = 0;; s++)
	    {
	      u16 slot = (head_slot + s) & mask;
	      atl_rx_desc_t *d = descs + slot;
	      if (s > 0)
		{
		  buffers[i + s - 1]->next_buffer = buffer_indices[i + s];
		  buffers[i + s - 1]->flags = next_flags;
		  tlnif += buffers[i + s]->current_length;
		}
	      if (d->qw1.eop)
		break;
	    }
	  if (s > 0)
	    {
	      buffers[i]->total_length_not_including_first_buffer = tlnif;
	      buffers[i]->flags = first_flags;
	    }
	...
	to_next[n_enq++] = buffer_indices[i];
    }

  ASSERT (n_left_to_next >= n_enq);
  to_next += n_enq;
