...
atl_init (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  log_info (dev, "HW rev 0x%08x (rev_id 0x%x)", hwrev, hwrev & 0x0f);
  ...
  atl_fw_version_t mac_ver = { .as_u32 = ver_data[0] };
  atl_fw_version_t phy_ver = { .as_u32 = ver_data[1] };
  const char *iface_str = "unknown";
  switch (ver_data[2] & 0xf)
    {
    case 0:
      iface_str = "A0";
      break;
  ...
  log_info (dev, "FW iface versions: MAC %u.%u.%u PHY %u.%u.%u IFACE %s", mac_ver.major,
  ...
  log_info (dev,
	    ...
	    "  resolver base %u count %u (offset %u)",
  ...
  vnet_dev_port_add_args_t port = {
	...
	.stop = atl_port_stop,
	.cable_diag = atl_port_cable_diag,
