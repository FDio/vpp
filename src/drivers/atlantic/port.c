...
atl_port_cable_diag (vlib_main_t *vm __clib_unused, vnet_dev_port_t *port)
{
  vnet_dev_t *dev = port->dev;
  vnet_dev_rv_t rv;
  atl_reg_aq2_fw_interface_in_cable_diag_control_t ctrl;
  ...
  ctrl = atl_reg_rd (dev, ATL_REG_AQ2_FW_INTERFACE_IN_CABLE_DIAG_CONTROL)
	   .aq2_fw_interface_in_cable_diag_control;
  ctrl.toggle = !ctrl.toggle;
  atl_reg_wr (dev, ATL_REG_AQ2_FW_INTERFACE_IN_CABLE_DIAG_CONTROL,
	      (atl_reg_t){ .aq2_fw_interface_in_cable_diag_control = ctrl });
...
atl_rx_path_init (vnet_dev_port_t *port)
{
  ...
  r = atl_reg_rd (dev, ATL_REG_RPB_RPF_RX);
  r.rpb_rpf_rx.tc_mode = 0;
  r.rpb_rpf_rx.fc_mode = 0;
  atl_reg_wr (dev, ATL_REG_RPB_RPF_RX, r);

  atl_reg_wr (dev, ATL_REG_AQ2_RPF_REDIR2, (atl_reg_t){ .aq2_rpf_redir2.hashtype = 0x1FF });

  if (n_rxq > 1)
    {
      r = atl_reg_rd (dev, ATL_REG_RPB_RPF_RX);
      r.rpb_rpf_rx.tc_mode = 1;
      r.rpb_rpf_rx.fc_mode = 1;
      atl_reg_wr (dev, ATL_REG_RPB_RPF_RX, r);
  ...
  /* set tag for primary MAC address */
  r = atl_reg_rd (dev, ATL_REG_RPF_L2UC_MSW (0));
  r.rpf_l2uc_msw.tag = 1;
  atl_reg_wr (dev, ATL_REG_RPF_L2UC_MSW (0), r);
  ...
  /* disable L2 promiscuous and all-multicast filtering in ART */
  tag = (atl_aq2_art_tag_t){};
  mask = (atl_aq2_art_tag_t){ .uc = 0x3f, .allmc = 1 };
...
#if 0
  /* enable L3 IPv6/IPv4 filter */
  reg = atl_reg_rd (dev, ATL_REG_RPF_L3_V6_V4_SELECT);
  reg.rpf_l3_v6_v4_select.v6_v4_select = 1;
  atl_reg_wr (dev, ATL_REG_RPF_L3_V6_V4_SELECT, reg);
#endif
...
atl_set_promisc (vnet_dev_port_t *port, int enabled)
{
  ...
  rp = atl_reg_rd (dev, ATL_REG_AQ2_FW_INTERFACE_IN_REQUEST_POLICY);
  rp.aq2_fw_interface_in_request_policy.promisc_all = enabled;
  rp.aq2_fw_interface_in_request_policy.promisc_mcast = enabled;
  atl_reg_wr (dev, ATL_REG_AQ2_FW_INTERFACE_IN_REQUEST_POLICY, rp);

  lc = atl_reg_rd (dev, ATL_REG_AQ2_FW_INTERFACE_IN_LINK_CONTROL);
  lc.aq2_fw_interface_in_link_control.promiscuous_mode = enabled;
  atl_reg_wr (dev, ATL_REG_AQ2_FW_INTERFACE_IN_LINK_CONTROL, lc);

  l2bc = atl_reg_rd (dev, ATL_REG_RPF_L2BC).rpf_l2bc;
  l2bc.promisc = enabled;
  atl_reg_wr (dev, ATL_REG_RPF_L2BC,
	      (atl_reg_t){
		.rpf_l2bc = l2bc,
	      });

  action = enabled ? (atl_aq2_art_action_t){} : (atl_aq2_art_action_t){ .enable = 1 };

  mask = (atl_aq2_art_tag_t){
    .uc = 0x3f,
    .allmc = 1,
  };
  rv = aq2_filter_art_set (dev, AQ2_RPF_INDEX_L2_PROMISC_OFF, tag, mask, action);
