...
atl_rx_path_init (vnet_dev_port_t *port)
{
  ...
  rv = atl_rss_type_set (port, port->rss_type.ip4, port->rss_type.ip6);
  if (rv != VNET_DEV_OK)
    return rv;
		     ...
		     vnet_dev_port_cfg_change_req_t *req)
{
  ...
  switch (req->type)
    ...
    case VNET_DEV_PORT_CFG_SET_RSS_KEY:
      if (port->started)
	rv = atl_rss_init (port, &req->rss_key, atl_enabled_rxq_count (port));
      break;
    case VNET_DEV_PORT_CFG_SET_RSS_TYPE:
      if (port->started)
	rv = atl_rss_type_set (port, req->rss_type.ip4, req->rss_type.ip6);
      break;
