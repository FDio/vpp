...
atl_aq2_filters_init (vnet_dev_port_t *port)
{
  ...
  atl_reg_aq2_rpf_act_art_req_tag_t mask = {};
  atl_reg_aq2_rpf_act_art_req_action_t action = { .enable = 1 };
  atl_reg_t reg;
  ...
  reg = atl_reg_rd (dev, ATL_REG_AQ2_RPF_REC_TAB_ENABLE);
  reg.aq2_rpf_rec_tab_enable.mask = 0xffff;
  atl_reg_wr (dev, ATL_REG_AQ2_RPF_REC_TAB_ENABLE, reg);

  /* set tag for primary MAC address */
  reg = atl_reg_rd (dev, ATL_REG_RPF_L2UC_MSW (l2_idx));
  reg.rpf_l2uc_msw.tag = 1;
  atl_reg_wr (dev, ATL_REG_RPF_L2UC_MSW (l2_idx), reg);

  /* set tag for L2 broadcast */
  atl_reg_wr (dev, ATL_REG_AQ2_RPF_L2BC_TAG, (atl_reg_t){ .aq2_rpf_l2bc_tag.tag = 1 });

  /* disable L2 promiscuous and all-multicast filtering in ART */
  mask = (atl_reg_aq2_rpf_act_art_req_tag_t){ .uc = 0x3f, .allmc = 1 };
  ...
  mask = (atl_reg_aq2_rpf_act_art_req_tag_t){ .vlan = 0xf, .untag = 1 };
  ...
  action.action = 1;
  mask = (atl_reg_aq2_rpf_act_art_req_tag_t){ .pcp = 0x7 };
  for (u32 i = 0; i < 8; i++)
    {
      action.index = i % 1;
      tag.pcp = i;
  ...
  atl_reg_wr (dev, ATL_REG_AQ2_RPF_NEW_CTRL, (atl_reg_t){ .aq2_rpf_new_ctrl.enable = 1 });
...
atl_mac_addr_init (vnet_dev_port_t *port)
{
  ...
  atl_reg_wr (dev, ATL_REG_RPF_L2UC_MSW (l2_idx), (atl_reg_t){ .rpf_l2uc_msw.en = 0 });
