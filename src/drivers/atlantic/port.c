...
atl_rss_init (vnet_dev_port_t *port, vnet_dev_rss_key_t *rss_key, u32 n_rxq)
{
  ...
  ASSERT (port->rss_config != 0);
  if (rss_key == 0)
    rss_key = &port->rss_config->key;
...
static vnet_dev_rv_t
atl_rss_type_set (vnet_dev_port_t *port, vnet_eth_rss_type_t ip4_type, vnet_eth_rss_type_t ip6_type)
{
  vnet_dev_t *dev = port->dev;
  atl_reg_aq2_rpf_redir2_t rpf_redir2 = {};

  if (ip4_type == VNET_ETH_RSS_TYPE_NOT_SET)
    ip4_type = VNET_ETH_RSS_TYPE_4_TUPLE;
  if (ip6_type == VNET_ETH_RSS_TYPE_NOT_SET)
    ip6_type = VNET_ETH_RSS_TYPE_4_TUPLE;

  if (ip4_type >= VNET_ETH_RSS_N_TYPES || ip6_type >= VNET_ETH_RSS_N_TYPES)
  ...
  switch (ip6_type)
    ...
    case VNET_ETH_RSS_TYPE_DISABLED:
      break;
    case VNET_ETH_RSS_TYPE_4_TUPLE:
      rpf_redir2.hashtype_tcp6 = 1;
      rpf_redir2.hashtype_udp6 = 1;
      rpf_redir2.hashtype_tcp6ex = 1;
      rpf_redir2.hashtype_udp6ex = 1;
      /* FALLTHROUGH */
    case VNET_ETH_RSS_TYPE_2_TUPLE:
    case VNET_ETH_RSS_TYPE_SRC_IP:
    case VNET_ETH_RSS_TYPE_DST_IP:
      rpf_redir2.hashtype_ip6 = 1;
      rpf_redir2.hashtype_ip6ex = 1;
      break;
    default:
      return VNET_DEV_ERR_INVALID_VALUE;
    }
...
atl_rx_path_init (vnet_dev_port_t *port)
{
  ...
  ASSERT (port->rss_config != 0);
  rv = atl_rss_type_set (port, port->rss_config->ip4, port->rss_config->ip6);
  if (rv != VNET_DEV_OK)
    return rv;
  ...
  switch (req->type)
      ...
      if (port->started)
	{
	  if (req->rss_config.key.length)
	    rv = atl_rss_init (port, &req->rss_config.key, atl_enabled_rxq_count (port));
	  if (rv == VNET_DEV_OK)
	    rv = atl_rss_type_set (port, req->rss_config.ip4, req->rss_config.ip6);
	}
      break;
