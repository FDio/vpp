		     ...
		     u32 lvl, vnet_tm_node_params_t *params, const char *flow_name)
{
  ...
  profile = roc_nix_tm_shaper_profile_get (nix, (u32) params->shaper_profile_id);
...
int
oct_tm_sys_shaper_profile_create (u32 hw_if_idx, vnet_tm_shaper_params_t *params)
{
  vnet_main_t *vnm = vnet_get_main ();
  vnet_hw_interface_t *hi = vnet_get_hw_interface (vnm, hw_if_idx);
  vnet_dev_port_t *port = vnet_dev_get_port_from_dev_instance (hi->dev_instance);
...
int
oct_tm_sys_level_get_capabilities (u32 hw_if_idx, vnet_tm_level_capa_params_t *cap, u32 lvl)
{
  vnet_main_t *vnm = vnet_get_main ();
  vnet_hw_interface_t *hi = vnet_get_hw_interface (vnm, hw_if_idx);
  vnet_dev_port_t *port = vnet_dev_get_port_from_dev_instance (hi->dev_instance);
...
int
oct_tm_sys_node_read_stats (u32 hw_if_idx, u32 node_id, vnet_tm_stats_params_t *stats)
{
  vnet_main_t *vnm = vnet_get_main ();
  vnet_hw_interface_t *hi = vnet_get_hw_interface (vnm, hw_if_idx);
  vnet_dev_port_t *port = vnet_dev_get_port_from_dev_instance (hi->dev_instance);
  ...
  if (roc_nix_tm_lvl_is_leaf (nix, node->lvl))
      ...
      if (!rc)
	{
	  ...
	  log_info (dev, "TM node %u stats: pkts %" PRIu64 " bytes %" PRIu64, node_id,
