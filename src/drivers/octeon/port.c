...
oct_init_tm_args (vnet_tm_system_t *tm)
{
  ...
  if (oct_tm_flow_id_to_node_id_hash == 0)
...
oct_port_init (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  roc_nix_rss_key_set (nix, port->rss_config.key);
  ...
  cd->ctqs =
    clib_mem_alloc_aligned (sizeof (oct_txq_t *) * ifs->num_tx_queues, CLIB_CACHE_LINE_BYTES);
  if (!cd->ctqs)
    {
      oct_port_deinit (vm, port);
      return VNET_DEV_ERR_INTERNAL;
  ...
  if (cd->egress_tm)
    {
      ...
      vnet_tm_system_register (&vnet_tm_system_ops, ifs->primary_interface.hw_if_index);
    }
...
oct_op_config_link_speed (vlib_main_t *vm, vnet_dev_port_t *port, u32 link_speed_kbps)
{
  ...
  /* Live port: bounce the CGX link so the SerDes retrains at the new rate. */
  rrv = roc_nix_mac_link_state_set (nix, 0);
  if (rrv)
    return oct_roc_err (dev, rrv, "roc_nix_mac_link_state_set(down) failed");
  ...
  if (rrv)
    return oct_roc_err (dev, rrv, "roc_nix_mac_link_state_set(up) failed");
...
vnet_dev_rv_t
oct_op_config_set_rss_config (vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_rss_config_t *config)
{
  vnet_dev_t *dev = port->dev;
  oct_device_t *cd = vnet_dev_get_data (dev);
  vnet_dev_rv_t rv = VNET_DEV_OK;
  ...
  if (config->set_key)
    roc_nix_rss_key_set (cd->nix, config->key);

  if (config->set_lut)
    {
      rrv = roc_nix_rss_reta_set (cd->nix, 0, config->lut);
      if (rrv)
	{
	  if (config->set_key)
	    roc_nix_rss_key_set (cd->nix, port->rss_config.key);
	  rv = oct_roc_err (dev, rrv, "roc_nix_rss_reta_set() failed");
	}
    }
			      ...
			      vnet_dev_port_cfg_change_req_t *req)
{
  ...
  switch (req->type)
    ...
    case VNET_DEV_PORT_CFG_SET_RSS_CONFIG:
      if ((req->rss_config.set_key && req->rss_config.key_len != ROC_NIX_RSS_KEY_LEN) ||
	  (req->rss_config.set_lut && req->rss_config.lut_len != ROC_NIX_RSS_RETA_MAX))
