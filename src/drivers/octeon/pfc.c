...
static int
oct_nix_pfc_rq_conf (vnet_dev_port_t *port, uint16_t qid, uint8_t tx_pause, uint8_t tc)
{
...
static int
oct_nix_pfc_sq_conf (vnet_dev_port_t *port, uint16_t qid, uint8_t rx_pause, uint8_t tc)
{
  vnet_dev_t *dev = port->dev;
  oct_device_t *cd = vnet_dev_get_data (dev);
  struct roc_nix_pfc_cfg pfc_cfg;
  ...
  if (!cd->rx_pause_en && rx_pause)
    {
      if ((roc_nix_tm_tree_type_get (nix) == ROC_NIX_TM_DEFAULT) && port->attr.max_tx_queues > 1)
	{
	  /*
...
oct_pfc_sys_configure (u32 hw_if_idx, pfc_params_t *params)
{
  ...
  if (roc_nix_is_sdp (nix) || roc_nix_is_lbk (nix))
    return (int) oct_pfc_roc_err (dev, -ENOTSUP, "Prio flow ctrl config is not allowed on SDP/LBK");

  /* Disallow flow control changes when device is in started state */
  if (port->started)
    return (int) oct_pfc_roc_err (dev, -EBUSY, "Stop the port=%d for setting PFC", port->port_id);
...
oct_pfc_sys_disable_pause_frame_flow_ctrl (u32 hw_if_idx, u32 disable)
{
  ...
  foreach_vnet_dev_port_rx_queue (q, port)
	...
	continue;
      ...
      rc = roc_nix_fc_config_set (nix, &fc_cfg);
      if (rc)
	return (int) oct_pfc_roc_err (dev, rc, "Failed to disable flow control on Rx queue %u",
				      rq->qid);
    }
  ...
  foreach_vnet_dev_port_tx_queue (q, port)
	...
	continue;
      ...
      rc = roc_nix_fc_config_set (nix, &fc_cfg);
      if (rc && rc != -EEXIST)
	return (int) oct_pfc_roc_err (dev, rc, "Failed to disable flow control on Tx queue %u",
				      sq->qid);
    }
...
static const pfc_system_t oct_pfc_ops_template = {
  ...
  .pfc_disable_pause_frame_flow_ctrl = oct_pfc_sys_disable_pause_frame_flow_ctrl,
};
