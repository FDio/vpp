...
oct_crypto_session_create (vlib_main_t *vm __clib_unused, oct_crypto_key_t *ckey,
			   vnet_crypto_key_t *key, int op_type)
{
...
void
oct_crypto_key_change_handler (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  ...
  if (args->action == VNET_CRYPTO_KEY_DATA_ADD)
	...
	{
	  if (ckey->sess[type] == NULL &&
	      oct_crypto_session_create (vlib_get_main (), ckey, key, type))
	    {
  ...
  for (type = VNET_CRYPTO_OP_TYPE_ENCRYPT; type <= VNET_CRYPTO_OP_TYPE_DECRYPT; type++)
      ...
      if (ckey->sess[type])
	{
	  if (oct_hw_ctx_cache_enable ())
	    roc_cpt_lf_ctx_flush (&ckey->sess[type]->crypto_dev->lf,
				  &ckey->sess[type]->cpt_ctx.se_ctx, true);
			  ...
			  int decrypt)
{
  ...
  *offset_vaddr = offset_ctrl;
  ...
  index =
    oct_crypto_fill_sg2_comp (gather_comp, index, (u64) offset_vaddr, ROC_SE_OFF_CTRL_LEN + iv_len);
  ...
  index = oct_crypto_fill_sg2_comp (scatter_comp, index, (u64) offset_vaddr + ROC_SE_OFF_CTRL_LEN,
				    iv_len);
...
oct_crypto_async_data_ptr (vlib_buffer_t *buffer, i16 offset)
{
  return buffer->data + offset;
}

static_always_inline u8
oct_crypto_async_aad_inline_len (oct_crypto_sess_t *sess, u8 aad_len, u32 crypto_total_length)
{
  if (sess->aes_gcm || crypto_total_length == 0)
    return aad_len;

  return (aad_len + 15) & ~15;
}

static_always_inline u8 *
oct_crypto_async_iv_buf (oct_crypto_sess_t *sess, vlib_buffer_t *buffer,
			 vnet_crypto_async_frame_elt_t *elts, u8 iv_tmp[16])
{
  u8 *iv;
  u32 *iv_buf;

  iv = oct_crypto_async_data_ptr (buffer, elts->iv_offset);
  if (sess->aes_gcm)
    {
      iv_buf = (u32 *) iv;
  ...
  if (sess->iv_length == 12)
    {
      // iv_buf = (u32 *) iv_tmp;
      clib_memcpy_fast (iv_tmp, iv, sess->iv_length);
      // iv_buf[3] = 0x1;
      return iv_tmp;
    }
...
static_always_inline int
oct_crypto_scatter_gather_mode (oct_crypto_sess_t *sess, struct cpt_inst_s *inst,
				...
				u16 adj_len)
{
  ...
  if (is_aead)
    {
      tag_or_digest = oct_crypto_async_data_ptr (b, elts->tag_offset);
      flags |= ROC_SE_VALID_IV_BUF;
      fc_params.iv_buf = oct_crypto_async_iv_buf (sess, b, elts, iv_tmp);
	...
	{
	  aad = oct_crypto_async_data_ptr (b, elts->aad_offset);
	  fc_params.aad_buf.vaddr = aad;
    ...
    {
      tag_or_digest = oct_crypto_async_data_ptr (b, elts->digest_offset);
      op_minor = ctx->template_w4.s.opcode_minor;

      flags |= ROC_SE_VALID_IV_BUF;
      fc_params.iv_buf = oct_crypto_async_data_ptr (b, elts->iv_offset);
...
oct_crypto_get_async_enqueue_handler (vnet_crypto_op_id_t op_id)
{
  vnet_crypto_op_data_t *od = vnet_crypto_get_op_data (op_id);
  const oct_crypto_alg_data_t *alg = oct_crypto_algs + od->alg;

  return vnet_crypto_get_op_type (op_id) == VNET_CRYPTO_OP_TYPE_ENCRYPT ? alg->enc_fn :
									 alg->dec_fn;
}

static_always_inline i32
oct_crypto_combined_session_update (vlib_main_t *vm, oct_crypto_sess_t *sess,
				    vnet_crypto_key_t *key, u8 type)
{
  vnet_crypto_main_t *cm = &crypto_main;
  vnet_crypto_alg_data_t *ad = cm->algs + key->alg;
  const oct_crypto_alg_data_t *alg = oct_crypto_algs + key->alg;
  const u8 *crypto_key_data, *auth_key_data;
  u16 crypto_key_len = ad->key_len;
  u16 auth_key_len = 0;
  u32 digest_len = ad->digest_len;
  i32 rv = 0;
  ...
  auth_key_len = key->integ_key_sz;
  if (auth_key_len == 0)
    return -1;

  crypto_key_data = vnet_crypto_get_cipher_key (key);
  auth_key_data = vnet_crypto_get_integ_key (key);
...
static_always_inline i32
oct_crypto_aead_session_update (vlib_main_t *vm, oct_crypto_sess_t *sess, vnet_crypto_key_t *key,
				vnet_crypto_op_id_t op_id, u8 type)
{
  vnet_crypto_op_data_t *od = vnet_crypto_get_op_data (op_id);
  const oct_crypto_alg_data_t *alg = oct_crypto_algs + od->alg;
  u32 digest_len = alg->digest_len ? alg->digest_len : od->digest_len;
  i32 rv = 0;
  ...
  rv = roc_se_ciph_key_set (&sess->cpt_ctx, alg->cipher_type, vnet_crypto_get_cipher_key (key),
			    key->cipher_key_sz);
  if (rv)
    {
      ASSERT (0);
...
static_always_inline i32
oct_crypto_session_init (vlib_main_t *vm, oct_crypto_sess_t *session, vnet_crypto_key_t *key,
			 vnet_crypto_op_id_t op_id, int op_type)
{
  ...
  if (cm->algs[key->alg].alg_type == VNET_CRYPTO_ALG_T_COMBINED)
    rv = oct_crypto_combined_session_update (vm, session, key, op_type);
  else
    rv = oct_crypto_aead_session_update (vm, session, key, op_id, op_type);
...
static_always_inline int
oct_crypto_enqueue_enc_dec (vlib_main_t *vm, vnet_crypto_async_frame_t *frame, const u8 is_aead,
			    u8 aad_len, const u8 type)
{
  ...
  for (i = 0; i < frame->n_elts; i++)
    {
      ...
      infl_req->index = i;
      infl_req->aead_algo = false;
      infl_req->esn_enabled = false;
      infl_req->mac_len = 0;

      elts = &frame->elts[i];
      infl_req->fe = elts;
      buffer_index = frame->buffer_indices[i];
      ckey = (oct_crypto_key_t *) vnet_crypto_get_async_key_data (elts->key);
	...
	ret = oct_crypto_session_init (vm, sess, elts->key, frame->op, type);
      if (ret)
	{
	  oct_crypto_update_frame_error_status (frame, i, VNET_CRYPTO_OP_STATUS_FAIL_ENGINE_ERR);
	  return -1;
      ...
      if (is_aead)
	{
	  ...
	  ASSERT (elts->crypto_start_offset + VLIB_BUFFER_PRE_DATA_SIZE >= 0);
	  crypto_start_offset = elts->crypto_start_offset + VLIB_BUFFER_PRE_DATA_SIZE;
	  integ_start_offset = 0;
	  auth_start_offset = 0;
	  auth_len = 0;
	  aad_inline_len = oct_crypto_async_aad_inline_len (sess, aad_len, crypto_total_length);

	  if (aad_len && elts->crypto_start_offset == buffer->current_data)
	    {
		...
		clib_memset (payload + crypto_start_offset - aad_inline_len, 0, aad_inline_len);
	      clib_memcpy_fast (payload + crypto_start_offset - aad_inline_len,
				oct_crypto_async_data_ptr (buffer, elts->aad_offset), aad_len);
	  ...
	  ret = oct_crypto_scatter_gather_mode (
	    sess, inst + i, is_aead, aad_len, payload, elts,
	...
	{
	  ASSERT (elts->crypto_start_offset + VLIB_BUFFER_PRE_DATA_SIZE >= 0);
	  ASSERT (elts->integ_start_offset + VLIB_BUFFER_PRE_DATA_SIZE >= 0);
	  crypto_start_offset = elts->crypto_start_offset + VLIB_BUFFER_PRE_DATA_SIZE;
	  integ_start_offset = elts->integ_start_offset + VLIB_BUFFER_PRE_DATA_SIZE;
...
{
  return oct_crypto_enqueue_enc_dec (vm, frame, 0 /* is_aead */, 0 /* aad_len */,
...
static int
oct_crypto_enqueue_aead_aad_enc (vlib_main_t *vm, vnet_crypto_async_frame_t *frame, u8 aad_len)
{
...
static_always_inline int
oct_crypto_enqueue_aead_aad_dec (vlib_main_t *vm, vnet_crypto_async_frame_t *frame, u8 aad_len)
{
  return oct_crypto_enqueue_enc_dec (vm, frame, 1 /* is_aead */, aad_len,
				     VNET_CRYPTO_OP_TYPE_DECRYPT);
}
...
static int
oct_crypto_enqueue_aead_aad_8_enc (vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
  return oct_crypto_enqueue_aead_aad_enc (vm, frame, 8);
}

static int
oct_crypto_enqueue_aead_aad_12_enc (vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
  return oct_crypto_enqueue_aead_aad_enc (vm, frame, 12);
}

static int
oct_crypto_enqueue_aead_aad_0_enc (vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
  return oct_crypto_enqueue_aead_aad_enc (vm, frame, 0);
}

static int
oct_crypto_enqueue_aead_aad_8_dec (vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
  return oct_crypto_enqueue_aead_aad_dec (vm, frame, 8);
}

static int
oct_crypto_enqueue_aead_aad_12_dec (vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
  return oct_crypto_enqueue_aead_aad_dec (vm, frame, 12);
}

static int
oct_crypto_enqueue_aead_aad_0_dec (vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
			  ...
			  clib_thread_index_t *enqueue_thread_idx)
{
  ...
  for (; last_elts_processed == false;)
      ...
      if (PREDICT_FALSE (res->cn10k.uc_compcode))
	{
      ...
      buffer = vlib_get_buffer (vm, infl_req->frame->buffer_indices[infl_req->index]);
	...
	{
	  clib_memcpy_fast (buffer->data + fe->crypto_start_offset - 8, infl_req->iv, 8);
	  clib_memcpy_fast (buffer->data + fe->crypto_start_offset - 16, infl_req->aad, 8);
	}
      else if (infl_req->esn_enabled)
	{
	  u8 *digest = vnet_crypto_async_get_data_ptr (buffer, fe->digest_offset);
	  clib_memcpy_fast (digest, digest + 4, infl_req->mac_len);
	}
  ...
  frame->state = status == VNET_CRYPTO_OP_STATUS_COMPLETED ? VNET_CRYPTO_FRAME_STATE_SUCCESS :
...
oct_init_crypto_engine_handlers (vlib_main_t *vm, vnet_dev_t *dev __clib_unused)
{
  ...
  for (u32 i = 1; i < VNET_CRYPTO_N_OP_IDS; i++)
    {
      fn = oct_crypto_get_async_enqueue_handler (i);
      if (fn)
	vnet_crypto_register_enqueue_handler (vm, engine_index, i, fn);
    }

  vnet_crypto_register_dequeue_handler (vm, engine_index, oct_crypto_frame_dequeue);
  vnet_crypto_register_async_key_change_handler (vm, engine_index, oct_crypto_key_change_handler,
...
static const oct_crypto_alg_data_t oct_crypto_algs[VNET_CRYPTO_N_ALGS] = {
  [VNET_CRYPTO_ALG_AES_128_CBC_SHA1_TAG12] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_SHA1_TYPE,
    .digest_len = 12,
    .valid = 1,
  },
  [VNET_CRYPTO_ALG_AES_192_CBC_SHA1_TAG12] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_SHA1_TYPE,
  ...
  [VNET_CRYPTO_ALG_AES_256_CBC_SHA1_TAG12] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_SHA1_TYPE,
  ...
  [VNET_CRYPTO_ALG_AES_128_CBC_SHA256_TAG16] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CBC,
  ...
  [VNET_CRYPTO_ALG_AES_192_CBC_SHA256_TAG16] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_SHA2_SHA256,
  ...
  [VNET_CRYPTO_ALG_AES_256_CBC_SHA256_TAG16] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_SHA2_SHA256,
  ...
  [VNET_CRYPTO_ALG_AES_128_CTR_SHA1_TAG12] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
  ...
  [VNET_CRYPTO_ALG_AES_192_CTR_SHA1_TAG12] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA1_TYPE,
  ...
  [VNET_CRYPTO_ALG_AES_256_CTR_SHA1_TAG12] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA1_TYPE,
  ...
  [VNET_CRYPTO_ALG_AES_128_CTR_SHA256_TAG16] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA2_SHA256,
  ...
  [VNET_CRYPTO_ALG_AES_192_CTR_SHA256_TAG16] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA2_SHA256,
  ...
  [VNET_CRYPTO_ALG_AES_256_CTR_SHA256_TAG16] = {
    .enc_fn = oct_crypto_enqueue_combined_alg_enc,
    .dec_fn = oct_crypto_enqueue_combined_alg_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA2_SHA256,
  ...
  [VNET_CRYPTO_ALG_AES_128_GCM_TAG16_AAD8] = {
    .enc_fn = oct_crypto_enqueue_aead_aad_8_enc,
    .dec_fn = oct_crypto_enqueue_aead_aad_8_dec,
  ...
  [VNET_CRYPTO_ALG_AES_128_GCM_TAG16_AAD12] = {
    .enc_fn = oct_crypto_enqueue_aead_aad_12_enc,
    .dec_fn = oct_crypto_enqueue_aead_aad_12_dec,
    .cipher_type = ROC_SE_AES_GCM,
    .aad_len = 12,
    .digest_len = 16,
    .iv_length = 16,
  ...
  [VNET_CRYPTO_ALG_AES_192_GCM_TAG16_AAD8] = {
    .enc_fn = oct_crypto_enqueue_aead_aad_8_enc,
    .dec_fn = oct_crypto_enqueue_aead_aad_8_dec,
    .cipher_type = ROC_SE_AES_GCM,
    .aad_len = 8,
  ...
  [VNET_CRYPTO_ALG_AES_192_GCM_TAG16_AAD12] = {
    .enc_fn = oct_crypto_enqueue_aead_aad_12_enc,
    .dec_fn = oct_crypto_enqueue_aead_aad_12_dec,
    .cipher_type = ROC_SE_AES_GCM,
    .aad_len = 12,
  ...
  [VNET_CRYPTO_ALG_AES_256_GCM_TAG16_AAD8] = {
    .enc_fn = oct_crypto_enqueue_aead_aad_8_enc,
    .dec_fn = oct_crypto_enqueue_aead_aad_8_dec,
    .cipher_type = ROC_SE_AES_GCM,
    .aad_len = 8,
  ...
  [VNET_CRYPTO_ALG_AES_256_GCM_TAG16_AAD12] = {
    .enc_fn = oct_crypto_enqueue_aead_aad_12_enc,
    .dec_fn = oct_crypto_enqueue_aead_aad_12_dec,
    .cipher_type = ROC_SE_AES_GCM,
    .aad_len = 12,
  ...
  [VNET_CRYPTO_ALG_CHACHA20_POLY1305_TAG16_AAD0] = {
    .enc_fn = oct_crypto_enqueue_aead_aad_0_enc,
    .dec_fn = oct_crypto_enqueue_aead_aad_0_dec,
  ...
  [VNET_CRYPTO_ALG_CHACHA20_POLY1305_TAG16_AAD8] = {
    .enc_fn = oct_crypto_enqueue_aead_aad_8_enc,
    .dec_fn = oct_crypto_enqueue_aead_aad_8_dec,
    .cipher_type = ROC_SE_CHACHA20,
    .auth_type = ROC_SE_POLY1305,
  ...
  [VNET_CRYPTO_ALG_CHACHA20_POLY1305_TAG16_AAD12] = {
    .enc_fn = oct_crypto_enqueue_aead_aad_12_enc,
    .dec_fn = oct_crypto_enqueue_aead_aad_12_dec,
    .cipher_type = ROC_SE_CHACHA20,
    .auth_type = ROC_SE_POLY1305,
