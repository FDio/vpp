...
oct_crypto_session_create (oct_crypto_key_t *ckey, vnet_crypto_key_t *key, int op_type)
{
...
oct_crypto_key_change_handler (vnet_crypto_key_t *key, vnet_crypto_key_change_args_t *args)
{
  ...
  if (args->action == VNET_CRYPTO_KEY_DATA_ADD)
	...
	{
	  if (ckey->sess[type] == NULL && oct_crypto_session_create (ckey, key, type))
	    {
				...
				u16 adj_len)
{
  ...
  if (is_aead)
    {
      tag_or_digest = oct_crypto_async_data_ptr (b, elts->icv_offset);
	...
	{
	  fc_params.aad_buf.vaddr = oct_crypto_async_data_ptr (b, elts->aad_offset);
...
oct_crypto_get_async_enqueue_handler (vnet_crypto_op_id_t op_id)
{
  ...
  return vnet_crypto_get_op_type (op_id) == VNET_CRYPTO_OP_TYPE_ENCRYPT ? alg->enc_fn : alg->dec_fn;
}
				    ...
				    vnet_crypto_key_t *key, u8 type)
{
  ...
  if (key->cipher_key_sz + key->auth_key_sz < crypto_key_len)
    return -1;

  auth_key_len = key->auth_key_sz;
  if (auth_key_len == 0)
    return -1;

  crypto_key_data = vnet_crypto_get_cipher_key (key);
  auth_key_data = vnet_crypto_get_auth_key (key);
				...
				vnet_crypto_op_id_t op_id, u8 type)
{
  vnet_crypto_op_data_t *od = vnet_crypto_get_op_data (op_id);
  const oct_crypto_alg_data_t *alg = oct_crypto_algs + od->alg;
  u32 digest_len = alg->digest_len ? alg->digest_len : od->auth_len;
  i32 rv = 0;
		...
		const u8 is_enc)
{
  ...
  for (i = 0; i < frame->n_elts; i++)
      ...
      if (ret)
	{
      ...
      if (is_aead)
	{
	  crypto_total_length = elts->cipher_data_len;
	  ASSERT (elts->cipher_data_start_off + VLIB_BUFFER_PRE_DATA_SIZE >= 0);
	  crypto_start_offset = elts->cipher_data_start_off + VLIB_BUFFER_PRE_DATA_SIZE;
	  integ_start_offset = 0;
	  auth_start_offset = 0;
	  auth_len = 0;
	  aad_inline_len = (sess->aes_gcm || crypto_total_length == 0) ? aad_len : 16;

	  if (aad_len && elts->cipher_data_start_off == buffer->current_data)
	    {
	...
	{
	  ASSERT (elts->cipher_data_start_off + VLIB_BUFFER_PRE_DATA_SIZE >= 0);
	  ASSERT (elts->auth_data_start_off + VLIB_BUFFER_PRE_DATA_SIZE >= 0);
	  crypto_start_offset = elts->cipher_data_start_off + VLIB_BUFFER_PRE_DATA_SIZE;
	  integ_start_offset = elts->auth_data_start_off + VLIB_BUFFER_PRE_DATA_SIZE;
	  enc_auth_len = elts->auth_data_len;
	  crypto_total_length = elts->cipher_data_len;
...
oct_init_crypto_engine_handlers (vlib_main_t *vm, vnet_dev_t *dev __clib_unused)
{
  ...
  if (vnet_crypto_register_async_key_change_handler (
	vm, engine_index, oct_crypto_key_change_handler, sizeof (oct_crypto_key_t)))
    return -1;
...
static const oct_crypto_alg_data_t oct_crypto_algs[VNET_CRYPTO_N_ALGS] = {
  ...
  [VNET_CRYPTO_ALG_AES_128_CBC_MD5_TAG12] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CBC,
  ...
  [VNET_CRYPTO_ALG_AES_192_CBC_MD5_TAG12] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_MD5_TYPE,
  ...
  [VNET_CRYPTO_ALG_AES_256_CBC_MD5_TAG12] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_MD5_TYPE,
  ...
  [VNET_CRYPTO_ALG_AES_128_CBC_SHA384_TAG24] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CBC,
  ...
  [VNET_CRYPTO_ALG_AES_192_CBC_SHA384_TAG24] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_SHA2_SHA384,
  ...
  [VNET_CRYPTO_ALG_AES_256_CBC_SHA384_TAG24] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_SHA2_SHA384,
  ...
  [VNET_CRYPTO_ALG_AES_128_CBC_SHA512_TAG32] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CBC,
  ...
  [VNET_CRYPTO_ALG_AES_192_CBC_SHA512_TAG32] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_SHA2_SHA512,
  ...
  [VNET_CRYPTO_ALG_AES_256_CBC_SHA512_TAG32] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CBC,
    .auth_type = ROC_SE_SHA2_SHA512,
  ...
  [VNET_CRYPTO_ALG_AES_128_CTR_SHA384_TAG24] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA2_SHA384,
  ...
  [VNET_CRYPTO_ALG_AES_192_CTR_SHA384_TAG24] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA2_SHA384,
  ...
  [VNET_CRYPTO_ALG_AES_256_CTR_SHA384_TAG24] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA2_SHA384,
  ...
  [VNET_CRYPTO_ALG_AES_128_CTR_SHA512_TAG32] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA2_SHA512,
  ...
  [VNET_CRYPTO_ALG_AES_192_CTR_SHA512_TAG32] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA2_SHA512,
  ...
  [VNET_CRYPTO_ALG_AES_256_CTR_SHA512_TAG32] = {
    .enc_fn = oct_crypto_enqueue_enc,
    .dec_fn = oct_crypto_enqueue_dec,
    .cipher_type = ROC_SE_AES_CTR,
    .auth_type = ROC_SE_SHA2_SHA512,
