...
/*
 * Sends packets to SQs based on TM queue id encoded in buffer flow_id
 * when it falls within the reserved TM flow_id range.
 */
		  ...
		  oct_tx_ctx_t *ctx, vnet_dev_tx_queue_t *txq, u16 tx_pkts, vlib_buffer_t **bufs)
{
  ...
  while (n_packets > 4)
    {
      if (vnet_tm_flow_id_is_valid (b[0]->flow_id))
	{
	...
	ctq0 = ctq;
      if (vnet_tm_flow_id_is_valid (b[1]->flow_id))
	{
	  tmq = oct_tm_get_node_id_from_flow_id (b[1]->flow_id);
	  if (PREDICT_FALSE (tmq == ~0))
	    {
	      n_unmapped++;
	      ctq1 = ctq;
	...
	ctq1 = ctq;
      if (vnet_tm_flow_id_is_valid (b[2]->flow_id))
	{
	  tmq = oct_tm_get_node_id_from_flow_id (b[2]->flow_id);
	  if (PREDICT_FALSE (tmq == ~0))
	    {
	      n_unmapped++;
	      ctq2 = ctq;
	...
	ctq2 = ctq;
      if (vnet_tm_flow_id_is_valid (b[3]->flow_id))
	{
	  tmq = oct_tm_get_node_id_from_flow_id (b[3]->flow_id);
	  if (PREDICT_FALSE (tmq == ~0))
	    {
	      n_unmapped++;
	      ctq3 = ctq;
  ...
  if (PREDICT_FALSE (n_unmapped))
    vlib_error_count (vm, node->node_index, OCT_TX_NODE_CTR_TM_FLOW_ID_UNMAPPED, n_unmapped);
