...
int
vl_socket_client_write (socket_client_main_t * scm)
{
  int n;
...
void *
vl_socket_msg_alloc (socket_client_main_t *scm, int nbytes)
{
  ...
  (vl_api_sock_init_shm_reply_t * mp)
{
  ...
  vl_socket_client_enable_disable(scm, 0);
  vec_free (new_name);
}
			  ...
			  char *client_name, u32 socket_buffer_size)
{
  ...
  mp = vl_socket_msg_alloc (scm, sizeof (*mp));
  ...
  if (vl_socket_client_write (scm) <= 0)
    {
      clib_unix_warning("socket write (msg)");
      return (-1);
    }
...
int
vl_socket_client_init_shm (socket_client_main_t *scm)
{
  ...
  mp->_vl_msg_id = ntohs (VL_API_SOCK_INIT_SHM);
  mp->client_index = ~0;
  mp->requested_size = 64 << 20;

  rv = vl_socket_client_write (scm);
  if (rv <= 0)
    return rv;
