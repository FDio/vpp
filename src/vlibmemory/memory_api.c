...
#undef _
  ...
  vl_api_msg_set_mp_safe (VL_API_MEMCLNT_KEEPALIVE_REPLY, 1);
  vl_api_msg_set_mp_safe (VL_API_MEMCLNT_KEEPALIVE, 1);
				 ...
				 vlib_node_runtime_t *node, u8 is_private)
{
  ...
  if (PREDICT_FALSE (am->elog_trace_api_messages))
	...
	u32 c;
      ...
      if (m && m->msg_name)
	ed->c = elog_string (am->elog_main, (char *) m->msg_name);
      else
  ...
  if (m && m->msg_handler)
	...
	{
	  fformat (stdout, "[%d]: %s\n", id, m->msg_name);
	  print_fp = (void *) am->messages[id].msg_print_handler;
	  if (print_fp == 0)
	    {
