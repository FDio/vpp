	  ...
	  if (am->vlib_private_rps[i] == svm)
	    {
	      ...
	      ssize_t virtual_base = svm->virtual_base - MMAP_PAGESIZE;
	      ssize_t virtual_size = svm->virtual_size + MMAP_PAGESIZE;
      ...
      for (i = 0; i < vec_len (dead_indices); i++)
	{
	  ...
	  if (regpp)
	      ...
	      if ((*regpp)->vlib_rp != svm)
		{
		  ...
		  /* Note: account for the memfd header page */
		  ssize_t virtual_base = dead_rp->virtual_base - MMAP_PAGESIZE;
		  ssize_t virtual_size = dead_rp->virtual_size + MMAP_PAGESIZE;
