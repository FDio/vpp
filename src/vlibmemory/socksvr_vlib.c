...
vl_api_memfd_segment_create_t_handler (vl_api_memfd_segment_create_t * mp)
{
  ...
  memset (&memfd_private, 0, sizeof (memfd_private));

  /* Embed in api_main_t */
  memfd_private.memfd_size = mp->requested_size;
  memfd_private.requested_va = 0ULL;
  memfd_private.i_am_master = 1;
  memfd_private.name = format (0, "%s%c", regp->name, 0);

  /* Set up a memfd segment of the requested size */
  rv = memfd_master_init (&memfd_private, mp->client_index);

  if (rv)
    goto reply;

  /*$$$ Your message in this space, bootstrap a binary api segment here */

  memfd_private.sh->ready = 1;
...
reply:
  ...
  send_fd_msg (cf->file_descriptor, memfd_private.fd);
}
