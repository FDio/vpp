...
vl_socket_api_send (vl_api_registration_t * rp, u8 * elem)
{
  ...
  if (sock_rp->registration_type == REGISTRATION_TYPE_SOCKET_SERVER)
    unix_save_error (&unix_main, error);

  /* If we didn't finish sending everything, wait until we have space */
  if (vec_len (sock_rp->output_vector) > 0)
    {
      int skip_update = 0 != (cf->flags & UNIX_FILE_DATA_AVAILABLE_TO_WRITE);
...
vl_socket_write_ready (clib_file_t * uf)
{
  ...
  if (n < 0)
	...
	{
	  int skip_update;
	  skip_update = 0 == (uf->flags & UNIX_FILE_DATA_AVAILABLE_TO_WRITE);
