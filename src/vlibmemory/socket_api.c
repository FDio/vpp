...
int
is_zombie_reg_index (u32 reg_index)
{
...
static void
socket_cleanup_zombie_registration_cb (u32 *preg_index)
{
  vl_api_registration_t *rp = vl_socket_get_registration (*preg_index);
  clib_file_main_t *fm = &file_main;
  u32 zombie_file_index = vl_api_registration_file_index (rp);

  clib_file_t *zf = fm->file_pool + zombie_file_index;
...
static void
vl_socket_zombify_reg_index (u32 reg_index)
{
  vl_api_registration_t *rp = vl_socket_get_registration (reg_index);
  ALWAYS_ASSERT (rp != 0);
  if (rp->is_zombie)
  ...
  rp->is_zombie = 1;
  vl_api_force_rpc_call_main_thread (socket_cleanup_zombie_registration_cb,
				     (void *) &reg_index, sizeof (u32));
}
...
vl_socket_read_ready (clib_file_t * uf)
{
  ...
  if (is_zombie_reg_index (reg_index))
    {
      return 0;
    }
  ...
  if (n <= 0)
      ...
      if (errno != EAGAIN)
	{
	  ...
	  vl_socket_zombify_reg_index (reg_index);
	}
