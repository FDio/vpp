...
ip46_set_src_address (u32 sw_if_index, vlib_buffer_t * b0, int is_ip6)
{
  ...
  if (is_ip6)
	...
	{
	  ip6_address_t *s_tmp;
	  s_tmp = ip_interface_get_first_ip (sw_if_index, false);
	  if (s_tmp)
	    {
	      ip6_address_copy (&ip6->src_address, s_tmp);
	      res = true;
	    }
	}
    }
	...
	{
	  ip4_address_t *s_tmp;
	  s_tmp = ip_interface_get_first_ip (sw_if_index, true);
	  if (s_tmp)
	    {
	      ip4->src_address.as_u32 = s_tmp->as_u32;
	      res = true;
	    }
	}
    }
