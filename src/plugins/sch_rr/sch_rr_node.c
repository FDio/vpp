...
static u8 *
format_sch_rr_enqueue_trace (u8 *s, va_list *args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  trace_en_t *tr = va_arg (*args, trace_en_t *);
  s = format (s, "packet enqueued, queue: %d, queue position: %d", tr->queue_i,
	      tr->pos);
  return s;
}

static u8 *
format_sch_rr_dequeue_trace (u8 *s, va_list *args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  ...
  s = format (s,
	      ...
	      "in this queue: %d",
	      tr->queue_i, tr->pos, tr->rem_pkt);
  return s;
}

/*
add the buffer index of each packet contained in the frame at the end of the
queue corresponding to the packet class set as 1 the bit corresponding to the
class in the class bitmap
*/

static void
enqueue_frame (vlib_main_t *vm, vlib_node_runtime_t *node,
	       sch_rr_buffer_t *rr_b, vlib_buffer_t **b, u32 *from, int n_pkt)
{
...
select_next_frame (vlib_main_t *vm, vlib_node_runtime_t *node,
		   sch_rr_buffer_t *rr_b, u32 *i_arr, u32 n_pkt)
{
  int i = 0;
  static int j = -1;
      ...
      if (rr_b->queue_map[j].count)
	{
	  ...
	  if (PREDICT_FALSE (vlib_get_buffer (vm, i_arr[i - 1])->flags &
			     VLIB_BUFFER_IS_TRACED))
	    {
	      trace_de_t *tr;
	      tr = vlib_add_trace (
		vm, node, vlib_get_buffer (vm, i_arr[i - 1]), sizeof (*tr));
	      tr->queue_i = j;
	      tr->pos = i - 1;
...
static uword
sch_rr_enqueue_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			vlib_frame_t *frame)
{
...
static uword
sch_rr_dequeue_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			vlib_frame_t *frame)
{
