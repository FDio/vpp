...
static clib_error_t *
sch_rr_enable_command_fn (vlib_main_t *vm, unformat_input_t *input,
			  vlib_cli_command_t *cmd)
{
  ...
  vnet_feature_enable_disable ("interface-output", "sch_rr_enqueue_node",
			       interface_index, 1, 0, 0);
  vnet_feature_enable_disable ("interface-output", "sch_rr_dequeue_node",
			       interface_index, 1, 0, 0);
  return (NULL);
}
...
static clib_error_t *
sch_rr_init (vlib_main_t *vm)
{

  sch_rr_buffer_t *rr_b;
  vec_validate_aligned (rr_buffer, clib_max (1, vlib_num_workers ()),
