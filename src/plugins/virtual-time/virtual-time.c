...
static f64 virtual_time_stop;

static uword
virtual_time_input (vlib_main_t *vm, vlib_node_runtime_t *node,
		    vlib_frame_t *frame)
{
  ...
  if (vlib_time_now (vm) + next > virtual_time_stop)
...
VLIB_REGISTER_NODE (virtual_time_input_node) = {
  .function = virtual_time_input,
...
static clib_error_t *
virtual_time_adjust_command_fn (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
  ...
  virtual_time_stop = vlib_time_now (vm) + val;

  foreach_vlib_main ()
    vlib_node_set_state (this_vlib_main, virtual_time_input_node.index,
...
VLIB_CLI_COMMAND (virtual_time_command) = {
  .path = "set clock adjust",
  .short_help = "set clock adjust <nn>",
  .function = virtual_time_adjust_command_fn,
};
