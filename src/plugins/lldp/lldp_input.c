		      ...
		      u8 portid_len, u8 portid_subtype, u16 ttl)
{
  ...
  vlib_rpc_call_main_thread (lldp_rpc_update_peer_cb, data, data_size);
}
...
static int
lldp_packet_scan (u32 hw_if_index, const lldp_tlv_t *pkt, u16 pkt_len)
{
...
lldp_input (vlib_main_t * vm, vlib_buffer_t * b0, u32 bi0)
{
  ...
  e =
    lldp_packet_scan (sw_interface->hw_if_index, vlib_buffer_get_current (b0), b0->current_length);
