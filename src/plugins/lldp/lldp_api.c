...
send_lldp (u32 index, vl_api_registration_t *rp, u32 context)
{
  ...
  REPLY_MACRO_DETAILS4_END (
	...
	(vl_api_chassis_id_subtype_t) n->chassis_id_subtype;
      ...
      clib_memcpy (&rmp->chassis_id, n->chassis_id,
		   clib_min (vec_len (n->chassis_id), rmp->chassis_id_len));
      rmp->port_id_len = vec_len (n->port_id);
      clib_memcpy (&rmp->port_id, n->port_id, clib_min (vec_len (n->port_id), rmp->port_id_len));
      ...
      clib_memcpy (rmp->mgmt_ip4, n->mgmt_ip4,
		   clib_min (vec_len ((char *) n->mgmt_ip4), sizeof (rmp->mgmt_ip4)));

      ip6_address_encode ((const ip6_address_t *) n->mgmt_ip6, rmp->mgmt_ip6);

      clib_memcpy (rmp->mgmt_oid, n->mgmt_oid,
		   clib_min (vec_len ((char *) n->mgmt_oid), sizeof (rmp->mgmt_oid)));

      clib_memcpy (rmp->port_desc, n->port_desc,
		   clib_min (vec_len ((char *) n->port_desc), sizeof (rmp->port_desc)));
    }));
}
