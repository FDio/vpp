...
igc_rx_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
		vlib_frame_t *frame)
{
  vnet_dev_rx_node_runtime_t *rt = (void *) node->runtime_data;
  fformat (stderr, "%s: n_rx_queues %u queue_id %u\n", __func__,
	   rt->n_rx_queues, rt->rx_queues[0]->queue_id);
  return 0;
}
...
igc_port_start (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  pool_foreach_index (i, port->rx_queues)
	...
	.enable = 1,
      ...
      u64 dma_addr;
      u16 n_alloc, q = rxq->queue_id;
      igc_rxq_t *iq = vnet_dev_get_rx_queue_data (rxq);
      n_alloc = vlib_buffer_alloc_on_numa (vm, iq->buffer_indices, rxq->n_desc,
					   dev->numa_node);

      if (n_alloc == 0)
	return VNET_DEV_ERR_BUFFER_ALLOC_FAIL;

      for (u16 j = 0; j < n_alloc; j++)
	{
      ...
      igc_reg_wr (id, IGC_REG_RDT (q), n_alloc);
      iq->head = 0;
      iq->tail = n_alloc;
    }
...
VNET_DEV_REGISTER_DRIVER (igc) = {
  ...
  .port_data_sz = sizeof (igc_port_t),
  ...
  .ops = { .device_init = igc_init,
	   ...
	   .port_start = igc_port_start,
	   .port_stop = igc_port_stop,
	   .format_port_status = format_igc_port_status,
