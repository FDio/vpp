...
{
  u32 mask = (0xFFFFFFFF << (32 - plen));

  if (clib_net_to_host_u32(net->as_u32) ==
      (clib_net_to_host_u32(addr->as_u32) & mask))
    return 1;
  return 0;
}

always_inline int
snat_det_forward (snat_main_t * sm, ip4_address_t * in_addr,
                  ip4_address_t * out_addr, u16 * lo_port, u16 * hi_port)
{
  ...
  ({
    if (is_addr_in_net(in_addr, &dm->in_addr, dm->in_plen))
      {
...
always_inline int
snat_det_reverse (snat_main_t * sm, ip4_address_t * out_addr, u16 out_port,
...
{
  u32 mask = 0xFFFFFFFF >> plen;
  return (clib_net_to_host_u32(addr->as_u32) & mask) * SNAT_DET_SES_PER_USER;
}

always_inline u16
snat_det_get_in_port_by_out (snat_main_t * sm, ip4_address_t * in_addr, u64 out_key)
{
  u32 user_offset;
  snat_det_map_t * dm;
  u16 i;
...
always_inline int
snat_det_find_ses_by_in_port_or_create (snat_main_t * sm,
