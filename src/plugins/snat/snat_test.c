  ...
  (vl_api_snat_user_session_details_t *mp)
{
  ...
  fformat(vam->ofp, "session %U to %U\n", 
          format_ip4_address, mp->out2in_ip_address,
          format_ip4_address, mp->in2out_ip_address);
}
  ...
  (vl_api_snat_user_details_t *mp)
{
  ...
  fformat(vam->ofp, "user with ip %U(running on thread %d) "
                    "with %d sessions and %d static sessions\n", 
          format_ip4_address, mp->ip_address, ntohl(mp->cpu_index),
...
_(snat_ipfix_enable_disable, "[domain <id>] [src_port <n>] "     \
  "[disable]")                                                   \
_(snat_user_dump, "")                                            \
_(snat_user_session_dump, "ip_address cpu_index fib index")
