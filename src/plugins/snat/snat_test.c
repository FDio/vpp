  ...
  (vl_api_snat_user_details_t *mp)
{
  ...
  fformat(vam->ofp, "user with ip %U(running on thread %d, with vrf_id %d) "
                    "with %d sessions and %d static sessions\n", 
          format_ip4_address, mp->ip_address, ntohl(mp->cpu_index),
          ntohl(mp->vrf_id), ntohl(mp->nsessions), ntohl(mp->nstaticsessions));
}
...
_(snat_ipfix_enable_disable, "[domain <id>] [src_port <n>] "     \
  "[disable]")                                                   \
_(snat_user_dump, "")                                            \
_(snat_user_session_dump, "ip_address cpu_index vrf_id")
