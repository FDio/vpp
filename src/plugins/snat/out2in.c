  ...
  while (n_left_from > 0)
    {
	  ...
	  b1 = vlib_get_buffer (vm, bi1);
          ...
          key0.ext_host_port = tcp0->ports.src;
          key0.out_port = tcp0->ports.dst;
          ...
          snat_det_reverse(dm0, &ip0->dst_address,
                           clib_net_to_host_u16(tcp0->ports.dst), &new_addr0);

          new_port0 = snat_det_get_in_port_by_out (dm0, &new_addr0, key0.as_u64);
          if (PREDICT_FALSE(!new_port0))
            {
	  ...
	  b1 = vlib_get_buffer (vm, bi1);
          ...
          key1.ext_host_port = tcp1->ports.src;
          key1.out_port = tcp1->ports.dst;
          ...
          snat_det_reverse(dm1, &ip1->dst_address,
                           clib_net_to_host_u16(tcp1->ports.dst), &new_addr1);

          new_port1 = snat_det_get_in_port_by_out (dm1, &new_addr1, key1.as_u64);
          if (PREDICT_FALSE(!new_port1))
            {
              clib_warning("no match src %U:%d dst %d for user %U",
                           format_ip4_address, &ip1->dst_address,
                           clib_net_to_host_u16 (tcp1->ports.src),
                           clib_net_to_host_u16 (tcp1->ports.dst),
