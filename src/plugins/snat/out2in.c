...
typedef struct {
  ...
  u32 next_index;
  u8 cached;
} snat_out2in_reass_trace_t;

static u8 * format_snat_out2in_reass_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  snat_out2in_reass_trace_t * t = va_arg (*args, snat_out2in_reass_trace_t *);

  s = format (s, "SNAT_OUT2IN_REASS: sw_if_index %d, next index %d, status %s",
              t->sw_if_index, t->next_index,
...
vlib_node_registration_t snat_out2in_reass_node;
...
VLIB_REGISTER_NODE (snat_out2in_node) = {
  ...
  .next_nodes = {
    ...
    [SNAT_OUT2IN_NEXT_REASS] = "snat-out2in-reass",
  },
};
VLIB_NODE_FUNCTION_MULTIARCH (snat_out2in_node, snat_out2in_node_fn);

static uword
snat_out2in_reass_node_fn (vlib_main_t * vm,
                           vlib_node_runtime_t * node,
                           vlib_frame_t * frame)
{
  u32 n_left_from, *from, *to_next;
  ...
  while (n_left_from > 0)
    {
	  ...
	  b0 = vlib_get_buffer (vm, bi0);
          ...
          if (PREDICT_FALSE (snat_reass_is_drop_frag(0)))
            {
              next0 = SNAT_OUT2IN_NEXT_DROP;
          ...
          reass0 = snat_ip4_reass_find_or_create (ip0->src_address,
                                                  ip0->dst_address,
                                                  ip0->fragment_id,
                                                  ip0->protocol,
                                                  1,
          ...
          if (PREDICT_FALSE (ip4_is_first_fragment (ip0)))
            {
              ...
              snat_ip4_reass_get_frags (reass0, &fragments_to_loopback);
            }
          else
            {
              if (PREDICT_FALSE (reass0->sess_index == (u32) ~0))
                {
                  if (snat_ip4_reass_add_fragment (reass0, bi0))
                    {
  ...
  vlib_node_increment_counter (vm, snat_out2in_reass_node.index,
                               SNAT_OUT2IN_ERROR_OUT2IN_PACKETS,
                               pkts_processed);

  snat_send_all_to_node (vm, fragments_to_drop, node,
...
VLIB_REGISTER_NODE (snat_out2in_reass_node) = {
  .function = snat_out2in_reass_node_fn,
  .name = "snat-out2in-reass",
  .vector_size = sizeof (u32),
  .format_trace = format_snat_out2in_reass_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
...
VLIB_NODE_FUNCTION_MULTIARCH (snat_out2in_reass_node, snat_out2in_reass_node_fn);
