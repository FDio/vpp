...
snat_det_get_worker_in2out_cb (ip4_header_t * ip0, u32 rx_fib_index0)
{
  ...
  dm = snat_det_map_by_user(sm, &ip0->src_address);
  if (dm && vec_len (sm->workers))
    return sm->first_worker_index +
      (sm->workers[clib_net_to_host_u32(ip0->src_address.as_u32) %
       _vec_len (sm->workers)]);
...
snat_det_get_worker_out2in_cb (ip4_header_t * ip0, u32 rx_fib_index0)
{
  ...
  dm = snat_det_map_by_out(sm, &ip0->src_address);
  if (dm && vec_len (sm->workers))
    {
      snat_det_reverse(dm, &ip0->dst_address,
                       clib_net_to_host_u16(udp0->dst_port), &in_addr);
      return sm->first_worker_index +
        (sm->workers[clib_net_to_host_u32(in_addr.as_u32) %
          _vec_len (sm->workers)]);
    }
                             ...
                             vlib_cli_command_t * cmd)
{
  ...
  if (!dm)
    ...
    {
      snat_det_forward (dm, &in_addr, &out_addr, &lo_port, &hi_port);
