      ...
      if (verbose > 0)
        {
          ...
          ({
            ...
            vlib_cli_output (vm, " sessions number: %d\n", 
                             vlib_get_simple_counter(&dm->ses_num, 0));
            if (verbose > 1)
              {
                      ...
                      ip4_address_t * out_addr, u8 out_plen, int is_add)
{
  ...
  vlib_validate_simple_counter(&det_map->ses_num, 0);
  vlib_zero_simple_counter(&det_map->ses_num, 0);
                             ...
                             vlib_cli_command_t * cmd)
{
  ...
  if (out_port < 1024 || out_port > 65535)
    {
      vlib_cli_output (vm, "wrong port, must be <1024-65535>");
      return 0;
    }
