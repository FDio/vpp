  ...
  while (n_left_from > 0)
    {
	  ...
	  b1 = vlib_get_buffer (vm, bi1);
          ...
          if (PREDICT_FALSE(snat_det_find_ses_by_in(dm0, &ip0->src_address,
              tcp0->ports.src)))
            {
              if (PREDICT_FALSE(snat_det_ses_create(dm0, &ip0->src_address,
                  tcp0->ports.src, &key0)))
                {
                  next0 = SNAT_IN2OUT_NEXT_DROP;
                  goto trace0;
        ...
        trace0:
          ...
          if (PREDICT_FALSE(snat_det_find_ses_by_in(dm1, &ip1->src_address,
              tcp1->ports.src)))
            {
              if (PREDICT_FALSE(snat_det_ses_create(dm1, &ip1->src_address,
                  tcp1->ports.src, &key1)))
                {
