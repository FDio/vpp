 ...
 */
static inline int
snat_not_traslate (snat_main_t * sm, snat_runtime_t * rt, u32 sw_if_index0,
                                         ...
                                         u32 cpu_index)
{
  ...
  if (clib_bihash_search_8_8 (&sm->in2out, &kv0, &value0))
    {
      if (PREDICT_FALSE(snat_not_traslate(sm, rt, sw_if_index0, ip0,
	  ...
	  rx_fib_index1 = vec_elt (sm->ip4_main->fib_index_by_sw_if_index, 
          ...
          kv1.key = key1.as_u64;
            ...
            if (PREDICT_FALSE(clib_bihash_search_8_8 (&sm->in2out, &kv1, &value1) != 0))
            {
              if (is_slow_path)
                {
                  if (PREDICT_FALSE(snat_not_traslate(sm, rt, sw_if_index1, ip1,
