  ...
  while (n_left_from > 0)
    {
	  ...
	  vlib_buffer_t * b0, * b1;
          ...
          u16 old_port0, new_port0, lo_port0;
	  ...
	  b1 = vlib_get_buffer (vm, bi1);
          ...
          new_port0 = clib_host_to_net_u16 (lo_port0 +
            (clib_net_to_host_u16 (tcp0->ports.src) % dm0->ports_per_host));

          key0.ext_host_addr = ip0->dst_address;
          key0.ext_host_port = tcp0->ports.dst;
          key0.out_port = new_port0;

          ses0 = snat_det_find_ses_by_in(dm0, &ip0->src_address, tcp0->ports.src);
          if (PREDICT_FALSE(!ses0))
            {
              ses0 = snat_det_ses_create(dm0, &ip0->src_address, tcp0->ports.src, &key0);
              if (PREDICT_FALSE(!ses0))
                {
                  next0 = SNAT_IN2OUT_NEXT_DROP;
                  goto trace0;
        ...
        trace0:
          ...
          new_port1 = clib_host_to_net_u16 (lo_port1 +
            (clib_net_to_host_u16 (tcp1->ports.src) % dm1->ports_per_host));

          key1.ext_host_addr = ip1->dst_address;
          key1.ext_host_port = tcp1->ports.dst;
          key1.out_port = new_port1;

          ses1 = snat_det_find_ses_by_in(dm1, &ip1->src_address, tcp1->ports.src);
          if (PREDICT_FALSE(!ses1))
            {
              ses1 = snat_det_ses_create(dm1, &ip1->src_address, tcp1->ports.src, &key1);
              if (PREDICT_FALSE(!ses1))
                {
