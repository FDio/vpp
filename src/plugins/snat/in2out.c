  ...
  while (n_left_from > 0)
    {
	  ...
	  vlib_buffer_t * b0, * b1;
          ...
          u16 old_port0, new_port0, lo_port0, hi_port0;
          u16 old_port1, new_port1, lo_port1, hi_port1;
          udp_header_t * udp0, * udp1;
          tcp_header_t * tcp0, * tcp1;
          u32 proto0, proto1;
	  ...
	  b1 = vlib_get_buffer (vm, bi1);
          ...
          new_port0 = clib_host_to_net_u16 (lo_port0 +
            (clib_net_to_host_u16 (tcp0->ports.src) % (hi_port0 - lo_port0 + 1)));

          key0.ext_host_addr = ip0->dst_address;
          key0.ext_host_port = tcp0->ports.dst;
          key0.out_port = new_port0;

          if (PREDICT_FALSE(snat_det_find_ses_by_in_port_or_create (dm0,
              &ip0->src_address, tcp0->ports.src, &key0)))
            {
        ...
        trace0:
          ...
          new_port1 = clib_host_to_net_u16 (lo_port1 +
            (clib_net_to_host_u16 (tcp1->ports.src) % (hi_port1 - lo_port1 + 1)));

          key1.ext_host_addr = ip1->dst_address;
          key1.ext_host_port = tcp1->ports.dst;
          key1.out_port = new_port1;

          if (PREDICT_FALSE(snat_det_find_ses_by_in_port_or_create (dm1,
              &ip1->src_address, tcp1->ports.src, &key1)))
            {
              next0 = SNAT_IN2OUT_NEXT_DROP;
              goto trace1;
            }
