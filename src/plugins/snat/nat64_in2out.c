...
format_nat64_in2out_reass_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "NAT-in2out-reass: sw_if_index %d, next index %d, status %s",
  ...
  if (ctx->first_frag)
    {
      ...
      if (ctx->proto == SNAT_PROTOCOL_TCP)
	{
	  u16 *checksum;
	  ip_csum_t csum;
			       ...
			       nat64_in2out_frag_set_ctx_t * ctx)
{
  ...
  if (ctx->first_frag)
    {
      if (ctx->proto == SNAT_PROTOCOL_UDP)
	checksum = &udp->checksum;
      else
	checksum = &tcp->checksum;
			    ...
			    vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (snat_reass_is_drop_frag (1)))
	    {
	      next0 = NAT64_IN2OUT_NEXT_DROP;
	  ...
	  reass0 = snat_ip6_reass_find_or_create (ip60->src_address,
						  ip60->dst_address,
	  ...
	  if (PREDICT_TRUE (ip6_frag_hdr_offset (frag0)))
		...
		{
		  if (snat_ip6_reass_add_fragment (reass0, bi0))
		    {
		...
		nat64_db_st_entry_get_index (&nm->db, ste0);
	      ...
	      snat_ip6_reass_get_frags (reass0, &fragments_to_loopback);
	    }
