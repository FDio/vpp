...
/**
 * @file
 * @brief S-NAT plugin virtual fragmentation reassembly
 */
#ifndef __included_snat_reass_h__
#define __included_snat_reass_h__
...
#define SNAT_REASS_TIMEOUT_DEFAULT 2
#define SNAT_MAX_REASS_DEAFULT 1024
#define SNAT_MAX_FRAG_DEFAULT 5
#define SNAT_REASS_HT_LOAD_FACTOR (0.75)
...
} snat_reass_ip4_key_t;

/* *INDENT-OFF* */
typedef CLIB_PACKED(struct
{
  snat_reass_ip4_key_t key;
  u32 lru_list_index;
  u32 sess_index;
  f64 last_heard;
...
} snat_reass_ip6_key_t;

/* *INDENT-OFF* */
typedef CLIB_PACKED(struct
{
  snat_reass_ip6_key_t key;
  u32 lru_list_index;
  u32 sess_index;
  f64 last_heard;
  u32 frags_per_reass_list_head_index;
...
typedef struct
{
  ...
  /* IPv4 runtime */
  snat_reass_ip4_t *ip4_reass_pool;
  ...
  /* IPv6 runtime */
  snat_reass_ip6_t *ip6_reass_pool;
...
/**
 * @brief Set S-NAT virtual fragmentation reassembly configuration.
 *
 ...
 */
int snat_reass_set (u32 timeout, u16 max_reass, u8 max_frag, u8 drop_frag,
		    u8 is_ip6);
 ...
 */
u32 snat_reass_get_timeout (u8 is_ip6);
 ...
 */
u16 snat_reass_get_max_reass (u8 is_ip6);
 ...
 */
u8 snat_reass_get_max_frag (u8 is_ip6);
 ...
 */
u8 snat_reass_is_drop_frag (u8 is_ip6);

/**
 * @brief Initialize S-NAT virtual fragmentation reassembly.
 *
 * @param vm vlib main.
 *
 * @return error code.
 */
clib_error_t *snat_reass_init (vlib_main_t * vm);
 ...
 */
snat_reass_ip4_t *snat_ip4_reass_find_or_create (ip4_address_t src,
						 ip4_address_t dst,
 ...
 * @returns 0 on success, non-zero value otherwise.
 */
int snat_ip4_reass_add_fragment (snat_reass_ip4_t * reass, u32 bi);
 ...
 */
void snat_ip4_reass_get_frags (snat_reass_ip4_t * reass, u32 ** bi);

/**
 * @breif Call back function when walking IPv4 reassemblies, non-zero return
 * value stop walk.
 */
typedef int (*snat_ip4_reass_walk_fn_t) (snat_reass_ip4_t * reass, void *ctx);
 ...
 */
void snat_ip4_reass_walk (snat_ip4_reass_walk_fn_t fn, void *ctx);
 ...
 * @returns Reassembly data or 0 on failure.
 */
snat_reass_ip6_t *snat_ip6_reass_find_or_create (ip6_address_t src,
						 ip6_address_t dst,
 ...
 * @returns 0 on success, non-zero value otherwise.
 */
int snat_ip6_reass_add_fragment (snat_reass_ip6_t * reass, u32 bi);
 ...
 * @param bi Vector of buffer indexes.
 */
void snat_ip6_reass_get_frags (snat_reass_ip6_t * reass, u32 ** bi);

/**
 * @breif Call back function when walking IPv6 reassemblies, non-zero return
 * value stop walk.
 */
typedef int (*snat_ip6_reass_walk_fn_t) (snat_reass_ip6_t * reass, void *ctx);
 ...
 * @param ctx A context passed in the visit function.
 */
void snat_ip6_reass_walk (snat_ip6_reass_walk_fn_t fn, void *ctx);

#endif /* __included_snat_reass_h__ */
