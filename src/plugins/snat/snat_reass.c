...
snat_reass_main_t snat_reass_main;

static u32
snat_reass_get_nbuckets (u8 is_ip6)
{
  snat_reass_main_t *srm = &snat_reass_main;
  u32 nbuckets;
  u8 i;

  if (is_ip6)
    nbuckets = (u32) (srm->ip6_max_reass / SNAT_REASS_HT_LOAD_FACTOR);
  else
    nbuckets = (u32) (srm->ip4_max_reass / SNAT_REASS_HT_LOAD_FACTOR);
...
snat_ip4_reass_get_frags_inline (snat_reass_ip4_t * reass, u32 ** bi)
{
  snat_reass_main_t *srm = &snat_reass_main;
  u32 elt_index;
  dlist_elt_t *elt;
...
static_always_inline void
snat_ip6_reass_get_frags_inline (snat_reass_ip6_t * reass, u32 ** bi)
{
  snat_reass_main_t *srm = &snat_reass_main;
  u32 elt_index;
  dlist_elt_t *elt;
...
int
snat_reass_set (u32 timeout, u16 max_reass, u8 max_frag, u8 drop_frag,
		u8 is_ip6)
{
  snat_reass_main_t *srm = &snat_reass_main;
  u32 nbuckets;
  ...
  if (is_ip6)
      ...
      if (srm->ip6_max_reass != max_reass)
	{
	  ...
	  nbuckets = snat_reass_get_nbuckets (0);
	  clib_bihash_free_48_8 (&srm->ip6_reass_hash);
	  clib_bihash_init_48_8 (&srm->ip6_reass_hash, "snat-ip6-reass",
      ...
      if (srm->ip4_max_reass != max_reass)
	{
	  ...
	  clib_bihash_init_16_8 (&srm->ip4_reass_hash, "snat-ip4-reass",
				 nbuckets, nbuckets * 1024);
...
u32
snat_reass_get_timeout (u8 is_ip6)
{
  snat_reass_main_t *srm = &snat_reass_main;
...
u16
snat_reass_get_max_reass (u8 is_ip6)
{
  snat_reass_main_t *srm = &snat_reass_main;
...
u8
snat_reass_get_max_frag (u8 is_ip6)
{
  snat_reass_main_t *srm = &snat_reass_main;
...
u8
snat_reass_is_drop_frag (u8 is_ip6)
{
  snat_reass_main_t *srm = &snat_reass_main;
...
static_always_inline snat_reass_ip4_t *
snat_ip4_reass_lookup (snat_reass_ip4_key_t * k, f64 now)
{
  snat_reass_main_t *srm = &snat_reass_main;
  clib_bihash_kv_16_8_t kv, value;
  snat_reass_ip4_t *reass;
...
snat_reass_ip4_t *
snat_ip4_reass_find_or_create (ip4_address_t src, ip4_address_t dst,
			       u16 frag_id, u8 proto, u8 reset_timeout,
			       u32 ** bi_to_drop)
{
  snat_reass_main_t *srm = &snat_reass_main;
  snat_reass_ip4_t *reass = 0;
  snat_reass_ip4_key_t k;
  ...
  reass = snat_ip4_reass_lookup (&k, now);
  if (reass)
    {
      if (reset_timeout)
	{
  ...
  if (srm->ip4_reass_n >= srm->ip4_max_reass)
      ...
      if (clib_bihash_add_del_16_8 (&srm->ip4_reass_hash, &kv, 0))
	{
      ...
      snat_ip4_reass_get_frags_inline (reass, bi_to_drop);
    }
  else
    {
...
int
snat_ip4_reass_add_fragment (snat_reass_ip4_t * reass, u32 bi)
{
  snat_reass_main_t *srm = &snat_reass_main;
  dlist_elt_t *elt;
  u32 elt_index;
...
void
snat_ip4_reass_get_frags (snat_reass_ip4_t * reass, u32 ** bi)
{
  snat_reass_main_t *srm = &snat_reass_main;

  clib_spinlock_lock_if_init (&srm->ip4_reass_lock);

  snat_ip4_reass_get_frags_inline (reass, bi);

  clib_spinlock_unlock_if_init (&srm->ip4_reass_lock);
}

void
snat_ip4_reass_walk (snat_ip4_reass_walk_fn_t fn, void *ctx)
{
  snat_reass_ip4_t *reass;
  snat_reass_main_t *srm = &snat_reass_main;
  f64 now = vlib_time_now (srm->vlib_main);
...
static_always_inline snat_reass_ip6_t *
snat_ip6_reass_lookup (snat_reass_ip6_key_t * k, f64 now)
{
  snat_reass_main_t *srm = &snat_reass_main;
  clib_bihash_kv_48_8_t kv, value;
  snat_reass_ip6_t *reass;
...
snat_reass_ip6_t *
snat_ip6_reass_find_or_create (ip6_address_t src, ip6_address_t dst,
			       u32 frag_id, u8 proto, u8 reset_timeout,
			       u32 ** bi_to_drop)
{
  snat_reass_main_t *srm = &snat_reass_main;
  snat_reass_ip6_t *reass = 0;
  snat_reass_ip6_key_t k;
  f64 now = vlib_time_now (srm->vlib_main);
  dlist_elt_t *oldest_elt, *elt;
  dlist_elt_t *per_reass_list_head_elt;
  u32 oldest_index, elt_index;
  ...
  reass = snat_ip6_reass_lookup (&k, now);
  if (reass)
    {
      if (reset_timeout)
	{
  ...
  if (srm->ip6_reass_n >= srm->ip6_max_reass)
      ...
      if (clib_bihash_add_del_48_8 (&srm->ip6_reass_hash, &kv, 0))
	{
      ...
      snat_ip6_reass_get_frags_inline (reass, bi_to_drop);
    }
  else
    {
...
int
snat_ip6_reass_add_fragment (snat_reass_ip6_t * reass, u32 bi)
{
  snat_reass_main_t *srm = &snat_reass_main;
  dlist_elt_t *elt;
  u32 elt_index;
...
void
snat_ip6_reass_get_frags (snat_reass_ip6_t * reass, u32 ** bi)
{
  snat_reass_main_t *srm = &snat_reass_main;

  clib_spinlock_lock_if_init (&srm->ip6_reass_lock);

  snat_ip6_reass_get_frags_inline (reass, bi);

  clib_spinlock_unlock_if_init (&srm->ip6_reass_lock);
}

void
snat_ip6_reass_walk (snat_ip6_reass_walk_fn_t fn, void *ctx)
{
  snat_reass_ip6_t *reass;
  snat_reass_main_t *srm = &snat_reass_main;
  f64 now = vlib_time_now (srm->vlib_main);
...
clib_error_t *
snat_reass_init (vlib_main_t * vm)
{
  snat_reass_main_t *srm = &snat_reass_main;
  ...
  /* IPv4 */
  srm->ip4_timeout = SNAT_REASS_TIMEOUT_DEFAULT;
  srm->ip4_max_reass = SNAT_MAX_REASS_DEAFULT;
  srm->ip4_max_frag = SNAT_MAX_FRAG_DEFAULT;
  srm->ip4_drop_frag = 0;
  srm->ip4_reass_n = 0;
  ...
  nbuckets = snat_reass_get_nbuckets (0);
  clib_bihash_init_16_8 (&srm->ip4_reass_hash, "snat-ip4-reass", nbuckets,
			 nbuckets * 1024);
  ...
  /* IPv6 */
  srm->ip6_timeout = SNAT_REASS_TIMEOUT_DEFAULT;
  srm->ip6_max_reass = SNAT_MAX_REASS_DEAFULT;
  srm->ip6_max_frag = SNAT_MAX_FRAG_DEFAULT;
  srm->ip6_drop_frag = 0;
  srm->ip6_reass_n = 0;
  ...
  nbuckets = snat_reass_get_nbuckets (1);
  clib_bihash_init_48_8 (&srm->ip6_reass_hash, "snat-ip6-reass", nbuckets,
			 nbuckets * 1024);
...
static clib_error_t *
snat_reass_command_fn (vlib_main_t * vm, unformat_input_t * input,
		       vlib_cli_command_t * cmd)
{
  ...
  if (!timeout)
    timeout = snat_reass_get_timeout (is_ip6);
  if (!max_reass)
    max_reass = snat_reass_get_max_reass (is_ip6);
  if (!max_frag)
    max_frag = snat_reass_get_max_frag (is_ip6);
  if (drop_frag == (u8) ~ 0)
    drop_frag = snat_reass_is_drop_frag (is_ip6);

  rv =
    snat_reass_set (timeout, (u16) max_reass, (u8) max_frag, drop_frag,
		    is_ip6);
  if (rv)
    {
      error = clib_error_return (0, "snat_set_reass return %d", rv);
      goto done;
    }
...
static int
snat_ip4_reass_walk_cli (snat_reass_ip4_t * reass, void *ctx)
{
  vlib_main_t *vm = ctx;
...
static int
snat_ip6_reass_walk_cli (snat_reass_ip6_t * reass, void *ctx)
{
  vlib_main_t *vm = ctx;
...
static clib_error_t *
show_snat_reass_command_fn (vlib_main_t * vm, unformat_input_t * input,
			    vlib_cli_command_t * cmd)
{
  vlib_cli_output (vm, "S-NAT IPv4 virtual fragmentation reassembly is %s",
		   snat_reass_is_drop_frag (0) ? "DISABLED" : "ENABLED");
  vlib_cli_output (vm, " max-reasssemblies %u", snat_reass_get_max_reass (0));
  vlib_cli_output (vm, " max-fragments %u", snat_reass_get_max_frag (0));
  vlib_cli_output (vm, " timeout %usec", snat_reass_get_timeout (0));
  vlib_cli_output (vm, " reassemblies:");
  snat_ip4_reass_walk (snat_ip4_reass_walk_cli, vm);

  vlib_cli_output (vm, "S-NAT IPv6 virtual fragmentation reassembly is %s",
		   snat_reass_is_drop_frag (1) ? "DISABLED" : "ENABLED");
  vlib_cli_output (vm, " max-reasssemblies %u", snat_reass_get_max_reass (1));
  vlib_cli_output (vm, " max-fragments %u", snat_reass_get_max_frag (1));
  vlib_cli_output (vm, " timeout %usec", snat_reass_get_timeout (1));
  vlib_cli_output (vm, " reassemblies:");
  snat_ip6_reass_walk (snat_ip6_reass_walk_cli, vm);

  return 0;
}

/* *INDENT-OFF* */
VLIB_CLI_COMMAND (snat_reass_command, static) =
{
  .path = "snat virtual-reassembly",
  .short_help = "snat virtual-reassembly ip4|ip6 [max-reassemblies <n>] "
                "[max-fragments <n>] [timeout <sec>] [enable|disable]",
  .function = snat_reass_command_fn,
};

VLIB_CLI_COMMAND (show_snat_reass_command, static) =
{
  .path = "show snat virtual-reassembly",
  .short_help = "show snat virtual-reassembly",
  .function = show_snat_reass_command_fn,
};
/* *INDENT-ON* */
