...
#ifdef POLICER_PLUGIN_INTERNAL

extern policer_main_t policer_main;
extern vlib_combined_counter_main_t policer_counters[];

#else /* POLICER_PLUGIN_INTERNAL */

typedef policer_main_t *(policer_get_main_fn_t) (void);
typedef vlib_combined_counter_main_t *(policer_get_counters_fn_t) (void);

static inline policer_main_t *
policer_get_main (void)
{
  policer_get_main_fn_t *fn =
    (policer_get_main_fn_t *) vlib_get_plugin_symbol ("policer_plugin.so", "policer_get_main");
  if (!fn)
    return 0;
  return fn ();
}

static inline vlib_combined_counter_main_t *
policer_get_counters (void)
{
  policer_get_counters_fn_t *fn = (policer_get_counters_fn_t *) vlib_get_plugin_symbol (
    "policer_plugin.so", "policer_get_counters");
  if (!fn)
    return 0;
  return fn ();
}

#endif /* POLICER_PLUGIN_INTERNAL */
