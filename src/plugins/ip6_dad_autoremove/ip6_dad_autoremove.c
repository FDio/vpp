...
/**
 * Enable the auto-remove functionality
 */
clib_error_t *
...
/**
 * Disable the auto-remove functionality
 */
clib_error_t *
...
ip6_dad_autoremove_init (vlib_main_t *vm)
{
  ...
  error = ip6_dad_autoremove_enable ();
  if (error)
    return error;

  AUTOREMOVE_INFO ("IPv6 DAD auto-remove plugin initialized and enabled");
