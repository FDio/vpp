...
typedef struct http2_conn_ctx_
{
  ...
  u8 *unsend_headers;	/* temporary storing tx fragmented headers */
  u32 unsend_headers_offset;
} http2_conn_ctx_t;
				   ...
				   clib_llist_index_t *next_ri)
{
  ...
  u8 flags = 0;
  u32 n_written, stream_id, max_write, headers_len;
  http2_conn_ctx_t *h2c;
  http2_worker_ctx_t *wrk = http2_get_worker (hc->c_thread_index);
  ...
  *n_emissions += HTTP2_SCHED_WEIGHT_HEADERS_CONTINUATION;
  ...
  ASSERT (vec_len (h2c->unsend_headers) > h2c->unsend_headers_offset);
  headers_len = vec_len (h2c->unsend_headers) - h2c->unsend_headers_offset;

  if (headers_len <= max_write)
    {
      HTTP_DBG (1, "sending last headers fragment");
  ...
  svm_fifo_seg_t segs[2] = {
    ...
    { h2c->unsend_headers + h2c->unsend_headers_offset, headers_len }
  };
  ...
  if (headers_len <= max_write)
  ...
  else
    {
      ...
      *next_ri = clib_llist_entry_index (wrk->req_pool, req);
      ...
      ASSERT (h2c->unsend_headers == 0);
      vec_validate (h2c->unsend_headers, headers_left - 1);
      clib_memcpy_fast (h2c->unsend_headers, response + headers_len,
			headers_left);
      h2c->unsend_headers_offset = 0;
      *n_emissions += HTTP2_SCHED_WEIGHT_HEADERS_CONTINUATION;
    }
