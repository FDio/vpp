...
http2_sched_init_conn (http_conn_t *hc, http2_worker_ctx_t *wrk)
{
...
static_always_inline void
http2_conn_init (http_conn_t *hc)
{
...
static inline void
http2_conn_destroy (http_conn_t *hc)
{
  http2_main_t *h2m = &http2_main;
  http2_worker_ctx_t *wrk = http2_get_worker (hc->c_thread_index);
  u32 cnt;
...
static_always_inline void
http2_req_set_stream_id (http2_req_t *req, http_conn_t *hc, u32 stream_id, u8 unset_old)
{
...
static inline void
http2_conn_free_req (http_conn_t *hc, http2_req_t *req, clib_thread_index_t thread_index)
{
...
static inline void
http2_conn_reset_req (http_conn_t *hc, http2_req_t *req, clib_thread_index_t thread_index)
{
  http2_worker_ctx_t *wrk = http2_get_worker (thread_index);
...
always_inline u32
http2_conn_get_next_stream_id (http_conn_t *hc)
{
...
always_inline void
http2_conn_schedule (http_conn_t *hc, clib_thread_index_t thread_index)
{
  ...
  if (!clib_llist_elt_is_linked (hc, sched_list) && !(hc->flags & HTTP_CONN_F_TS_DESCHED))
    {
      ...
      clib_llist_add_tail (wrk->conn_pool, sched_list, hc, he);
    }
}
			...
			transport_send_params_t *sp)
{
  ...
  if (clib_llist_elt_is_linked (hc, sched_list))
    clib_llist_remove (wrk->conn_pool, sched_list, hc);
...
http2_update_time_callback (f64 now, u8 thread_index)
{
  ...
  if (ci != wrk->sched_head)
    {
      hc = clib_llist_elt (wrk->conn_pool, ci);
...
static_always_inline int
http2_expect_preface (http_conn_t *hc)
{
  u8 *rx_buf;
