				       ...
				       http2_error_t *error)
{
  ...
  *error = hpack_parse_request (req->payload, req->payload_len, buf, 1023,
  ...
  req->base.control_data_len = control_data.path_len +
			       control_data.authority_len +
			       control_data.headers_len;
...
http2_req_state_app_io_more_data (http_conn_t *hc, http2_req_t *req,
				  ...
				  http2_error_t *error)
{
  u32 max_write, n_segs, to_write = 0, n_written = 0;
  svm_fifo_seg_t *app_segs, *seg, *segs = 0;
  ...
  vec_validate (segs, 0);
  segs->len = HTTP2_FRAME_HEADER_SIZE;
  segs->data = fh;
