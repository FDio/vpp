...
typedef struct http2_conn_ctx_
{
  ...
  u8 flags;
<<<<<<< PATCH SET (808e02 http: http/2 starting tcp connection)
  u32 last_stream_id;
=======
  u32 last_opened_stream_id;
  u32 last_processed_stream_id;
>>>>>>> BASE      (20459f http: http/2 stream state machine)
  u64 peer_window;
...
http2_send_server_preface (http_conn_t *hc)
{
  u8 *response;
  http2_main_t *h2m = &http2_main;
  http2_settings_entry_t *settings_list;

  vec_validate (settings_list, 0);
  settings_list[0].identifier = HTTP2_SETTINGS_MAX_CONCURRENT_STREAMS;
  settings_list[0].value = h2m->settings.max_concurrent_streams;
  response = http_get_tx_buf (hc);
  http2_frame_write_settings (settings_list, &response);
  /* TODO send all non-default values */
...
http2_req_state_app_io_more_data (http_conn_t *hc, http2_req_t *req,
				  ...
				  http2_error_t *error)
{
  u32 max_write, max_read, n_segs, to_write = 0, n_written = 0;
  ...
  app_segs =
    http_buffer_get_segs (hb, max_write - HTTP2_FRAME_HEADER_SIZE, &n_segs);
  if (!app_segs)
    {
  ...
  to_write = clib_min (max_write, max_read);

  finished = (max_read - to_write) == 0;
  flags = finished ? HTTP2_FRAME_FLAG_END_STREAM : 0;
  http2_frame_write_data_header (to_write, req->stream_id, flags, fh);
  vec_validate (segs, n_segs);
  segs[0].len = HTTP2_FRAME_HEADER_SIZE;
  segs[0].data = fh;
  for (i = 1; i <= n_segs; i++)
    {
      segs[i].len = app_segs[i - 1].len;
      segs[i].data = app_segs[i - 1].data;
    }

  n_written = http_io_ts_write_segs (hc, segs, n_segs + 1, sp);
  ASSERT (n_written == (HTTP2_FRAME_HEADER_SIZE + to_write));
  vec_free (segs);
  http_buffer_drain (hb, to_write);
...
http2_handle_headers_frame (http_conn_t *hc, http2_frame_header_t *fh)
{
  ...
  if (hc->flags & HTTP_CONN_F_IS_SERVER)
    {
      h2c = http2_conn_ctx_get_w_thread (hc);
      /* streams initiated by client must use odd-numbered stream id and
       * must be greater than all streams that client has opened */
      if (((fh->stream_id % 2) == 0) ||
	  (fh->stream_id < h2c->last_opened_stream_id))
	{
