		  ...
		  if (req->stream_state != HTTP2_STREAM_STATE_CLOSED)
		    {
		      ...
		      if (clib_llist_elt_is_linked (req, sched_list))
			clib_llist_remove (h2m->req_pool[hc->c_thread_index],
					   sched_list, req);
		    }
		}));
...
http2_handle_rst_stream_frame (http_conn_t *hc, http2_frame_header_t *fh)
{
  ...
  session_transport_reset_notify (&req->base.connection);
  if (clib_llist_elt_is_linked (req, sched_list))
    clib_llist_remove (h2m->req_pool[hc->c_thread_index], sched_list, req);
...
void
http2_update_time_callback (f64 now, clib_thread_index_t thread_index)
{
  http2_main_t *h2m = &http2_main;
  http2_conn_ctx_t *h2c;
  http_conn_t *hc;
  ...
  /* run stream tx scheduler for each active connection */
...
const static http_engine_vft_t http2_engine = {
  ...
  .update_time_callback = http2_update_time_callback,
};
