			...
			transport_send_params_t *sp)
{
  ...
  http2_frame_write_goaway (error, h2c->last_stream_id, &response);
				       ...
				       http2_error_t *error)
{
  ...
  *error = hpack_parse_request (req->payload, req->payload_len, buf, 1023,
  ...
  if (req->stream_id > h2c->last_stream_id)
    h2c->last_stream_id = req->stream_id;
				...
				http2_error_t *error)
{
  ...
  if (msg.data.body_len)
  ...
  else
    {
      ...
      /* notify app, because stream is only one request */
      session_transport_closing_notify (&req->base.connection);
    }
  ...
  if (finished)
      ...
      if (hc->flags & HTTP_CONN_F_IS_SERVER)
	{
	  ...
	  if (req->flags & HTTP2_REQ_F_APP_CLOSED)
	    {
	      /* app already closed, confirm */
	      session_transport_closed_notify (&req->base.connection);
	    }
	  else
	    {
	      /* otherwise notify app, because stream is only one request */
	      session_transport_closing_notify (&req->base.connection);
	    }
	}
      else
