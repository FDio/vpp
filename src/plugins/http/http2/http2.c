...
static_always_inline http2_worker_t *
http2_worker_get (clib_thread_index_t thread_index)
{
  return &http2_main.wrk[thread_index];
}
...
http2_conn_ctx_alloc_w_thread (http_conn_t *hc)
{
  ...
  http2_worker_t *wrk = http2_worker_get (hc->c_thread_index);
...
{
  http2_worker_t *wrk = http2_worker_get (thread_index);
...
http2_enable_callback (void)
{
  ...
  vec_validate (h2m->wrk, num_threads - 1);
