...
{
  hpack_error_t rv;
  rv = hpack_decode_request (src, src_len, dst, dst_len, control_data, headers,
			     (void *) dynamic_table, hpack_decode_header);
		      ...
		      hpack_dynamic_table_t *dynamic_table)
{
  hpack_error_t rv;
  rv =
    hpack_decode_response (src, src_len, dst, dst_len, control_data, headers,
			   (void *) dynamic_table, hpack_decode_header);
  HTTP_DBG (3, "%U", format_hpack_dynamic_table, dynamic_table);
  return hpack_error_to_http2_error[rv];
}
