...
always_inline u8
http_memcmp_case (const char *actual, const char *expected, uword len)
{
  uword i, last_a = 0, last_e = 0;
  uword *a, *e;
  ASSERT (actual != 0);
...
http_lookup_header_name (const char *name, uword name_len)
{
  switch (name_len)
	...
	{
	case 'a':
	  if (!http_memcmp_case (name + 1, "ge", 2))
	    return HTTP_HEADER_AGE;
	  break;
	case 'v':
	  if (!http_memcmp_case (name + 1, "ia", 2))
	...
	{
	case 'n':
	  if (!http_memcmp_case (name, "alp", 3))
	    return HTTP_HEADER_ALPN;
	  break;
	case 'e':
	  if (!http_memcmp_case (name, "dat", 3))
	    return HTTP_HEADER_DATE;
	  break;
	case 'p':
	  if (!http_memcmp_case (name, "dpo", 3))
	    return HTTP_HEADER_DPOP;
	  break;
	case 'g':
	  if (!http_memcmp_case (name, "eta", 3))
	    return HTTP_HEADER_ETAG;
	  break;
	case 'm':
	  if (!http_memcmp_case (name, "fro", 3))
	    return HTTP_HEADER_FROM;
	  break;
	case 't':
	  if (!http_memcmp_case (name, "hos", 3))
	    return HTTP_HEADER_HOST;
	  break;
	case 'k':
	  if (!http_memcmp_case (name, "lin", 3))
	    return HTTP_HEADER_LINK;
	  break;
	case 'y':
	  if (!http_memcmp_case (name, "var", 3))
	...
	{
	case 'a':
	  if (!http_memcmp_case (name + 1, "llow", 4))
	    return HTTP_HEADER_ALLOW;
	  break;
	case 'c':
	  if (!http_memcmp_case (name + 1, "lose", 4))
	    return HTTP_HEADER_CLOSE;
	  break;
	case 'r':
	  if (!http_memcmp_case (name + 1, "ange", 4))
	...
	{
	case 'a':
	  if (!http_memcmp_case (name + 1, "ccept", 5))
	    return HTTP_HEADER_ACCEPT;
	  break;
	case 'c':
	  if (!http_memcmp_case (name + 1, "ookie", 5))
	    return HTTP_HEADER_COOKIE;
	  break;
	case 'd':
	  if (!http_memcmp_case (name + 1, "igest", 5))
	    return HTTP_HEADER_DIGEST;
	  break;
	case 'e':
	  if (!http_memcmp_case (name + 1, "xpect", 5))
	    return HTTP_HEADER_EXPECT;
	  break;
	case 'o':
	  if (!http_memcmp_case (name + 1, "rigin", 5))
	    return HTTP_HEADER_ORIGIN;
	  break;
	case 's':
	  if (!http_memcmp_case (name + 1, "erver", 5))
	...
	{
	case 'a':
	  if (!http_memcmp_case (name + 1, "lt-svc", 6))
	    return HTTP_HEADER_ALT_SVC;
	  break;
	case 'e':
	  if (!http_memcmp_case (name + 1, "xpires", 6))
	    return HTTP_HEADER_EXPIRES;
	  break;
	case 'p':
	  if (!http_memcmp_case (name + 1, "urpose", 6))
	    return HTTP_HEADER_PURPOSE;
	  break;
	case 'r':
	  if (!http_memcmp_case (name + 1, "eferer", 6))
	    return HTTP_HEADER_REFERER;
	  if (!http_memcmp_case (name + 1, "efresh", 6))
	    return HTTP_HEADER_REFRESH;
	  break;
	case 't':
	  if (!http_memcmp_case (name + 1, "railer", 6))
	    return HTTP_HEADER_TRAILER;
	  break;
	case 'u':
	  if (!http_memcmp_case (name + 1, "pgrade", 6))
	...
	{
	case 'd':
	  if (!http_memcmp_case (name, "alt-use", 7))
	    return HTTP_HEADER_ALT_USED;
	  break;
	case 'p':
	  if (!http_memcmp_case (name, "cdn-loo", 7))
	    return HTTP_HEADER_CDN_LOOP;
	  break;
	case 'h':
	  if (!http_memcmp_case (name, "if-matc", 7))
	    return HTTP_HEADER_IF_MATCH;
	  break;
	case 'e':
	  if (!http_memcmp_case (name, "if-rang", 7))
	    return HTTP_HEADER_IF_RANGE;
	  break;
	case 'n':
	  if (!http_memcmp_case (name, "locatio", 7))
	    return HTTP_HEADER_LOCATION;
	  break;
	case 'y':
	  if (!http_memcmp_case (name, "priorit", 7))
	...
	{
	case 'e':
	  if (!http_memcmp_case (name + 1, "xpect-ct", 8))
	    return HTTP_HEADER_EXPECT_CT;
	  break;
	case 'f':
	  if (!http_memcmp_case (name + 1, "orwarded", 8))
	    return HTTP_HEADER_FORWARDED;
	  break;
	case 's':
	  if (!http_memcmp_case (name + 1, "ignature", 8))
	...
	{
	case 'a':
	  if (!http_memcmp_case (name + 1, "lternates", 9))
	    return HTTP_HEADER_ALTERNATES;
	  break;
	case 'c':
	  if (!http_memcmp_case (name + 1, "onnection", 9))
	    return HTTP_HEADER_CONNECTION;
	  break;
	case 'd':
	  if (!http_memcmp_case (name + 1, "pop-nonce", 9))
	    return HTTP_HEADER_DPOP_NONCE;
	  break;
	case 'e':
	  if (!http_memcmp_case (name + 1, "arly-data", 9))
	    return HTTP_HEADER_EARLY_DATA;
	  break;
	case 'k':
	  if (!http_memcmp_case (name + 1, "eep-alive", 9))
	    return HTTP_HEADER_KEEP_ALIVE;
	  break;
	case 's':
	  if (!http_memcmp_case (name + 1, "et-cookie", 9))
	    return HTTP_HEADER_SET_COOKIE;
	  break;
	case 'u':
	  if (!http_memcmp_case (name + 1, "ser-agent", 9))
	...
	{
	case 'c':
	  if (!http_memcmp_case (name + 1, "lient-cert", 10))
	    return HTTP_HEADER_CLIENT_CERT;
	  break;
	case 'r':
	  if (!http_memcmp_case (name + 1, "epr-digest", 10))
	    return HTTP_HEADER_REPR_DIGEST;
	  if (!http_memcmp_case (name + 1, "etry-after", 10))
	...
	{
	case 'c':
	  if (!http_memcmp_case (name + 1, "ache-status", 11))
	    return HTTP_HEADER_CACHE_STATUS;
	  if (!http_memcmp_case (name + 1, "ontent-type", 11))
	    return HTTP_HEADER_CONTENT_TYPE;
	  break;
	case 'm':
	  if (!http_memcmp_case (name + 1, "ax-forwards", 11))
	    return HTTP_HEADER_MAX_FORWARDS;
	  break;
	case 'p':
	  if (!http_memcmp_case (name + 1, "roxy-status", 11))
	...
	{
	case 's':
	  if (!http_memcmp_case (name, "accept-range", 12))
	    return HTTP_HEADER_ACCEPT_RANGES;
	  break;
	case 'n':
	  if (!http_memcmp_case (name, "authorizatio", 12))
	    return HTTP_HEADER_AUTHORIZATION;
	  break;
	case 'l':
	  if (!http_memcmp_case (name, "cache-contro", 12))
	    return HTTP_HEADER_CACHE_CONTROL;
	  break;
	case 'e':
	  if (!http_memcmp_case (name, "content-rang", 12))
	    return HTTP_HEADER_CONTENT_RANGE;
	  break;
	case 'h':
	  if (!http_memcmp_case (name, "if-none-matc", 12))
	    return HTTP_HEADER_IF_NONE_MATCH;
	  break;
	case 'd':
	  if (!http_memcmp_case (name, "last-modifie", 12))
	...
	{
	case 'a':
	  if (!http_memcmp_case (name + 1, "ccept-charset", 13))
	    return HTTP_HEADER_ACCEPT_CHARSET;
	  break;
	case 'c':
	  if (!http_memcmp_case (name + 1, "ontent-digest", 13))
	    return HTTP_HEADER_CONTENT_DIGEST;
	  if (!http_memcmp_case (name + 1, "ontent-length", 13))
	...
	{
	case 'g':
	  if (!http_memcmp_case (name, "accept-encodin", 14))
	    return HTTP_HEADER_ACCEPT_ENCODING;
	  break;
	case 'e':
	  if (!http_memcmp_case (name, "accept-languag", 14))
	    return HTTP_HEADER_ACCEPT_LANGUAGE;
	  break;
	case 't':
	  if (!http_memcmp_case (name, "signature-inpu", 14))
	    return HTTP_HEADER_SIGNATURE_INPUT;
	  break;
	case 'r':
	  if (!http_memcmp_case (name, "x-forwarded-fo", 14))
	    return HTTP_HEADER_X_FORWARDED_FOR;
	  break;
	case 's':
	  if (!http_memcmp_case (name, "x-frame-option", 14))
	...
	{
	case 'c':
	  if (!http_memcmp_case (name + 1, "apsule-protocol", 15))
	    return HTTP_HEADER_CAPSULE_PROTOCOL;
	  if (!http_memcmp_case (name + 1, "ontent-encoding", 15))
	    return HTTP_HEADER_CONTENT_ENCODING;
	  if (!http_memcmp_case (name + 1, "ontent-language", 15))
	    return HTTP_HEADER_CONTENT_LANGUAGE;
	  if (!http_memcmp_case (name + 1, "ontent-location", 15))
	    return HTTP_HEADER_CONTENT_LOCATION;
	  break;
	case 'p':
	  if (!http_memcmp_case (name + 1, "roxy-connection", 15))
	    return HTTP_HEADER_PROXY_CONNECTION;
	  break;
	case 'w':
	  if (!http_memcmp_case (name + 1, "ant-repr-digest", 15))
	    return HTTP_HEADER_WANT_REPR_DIGEST;
	  if (!http_memcmp_case (name + 1, "ww-authenticate", 15))
	    return HTTP_HEADER_WWW_AUTHENTICATE;
	  break;
	case 'x':
	  if (!http_memcmp_case (name + 1, "-xss-protection", 15))
	...
	{
	case 'l':
	  if (!http_memcmp_case (name, "cdn-cache-contro", 16))
	    return HTTP_HEADER_CDN_CACHE_CONTROL;
	  break;
	case 'n':
	  if (!http_memcmp_case (name, "client-cert-chai", 16))
	    return HTTP_HEADER_CLIENT_CERT_CHAIN;
	  break;
	case 'e':
	  if (!http_memcmp_case (name, "if-modified-sinc", 16))
	    return HTTP_HEADER_IF_MODIFIED_SINCE;
	  break;
	case 'g':
	  if (!http_memcmp_case (name, "transfer-encodin", 16))
	    return HTTP_HEADER_TRANSFER_ENCODING;
	  break;
	}
      break;
    case 18:
      if (!http_memcmp_case (name, "proxy-authenticate", 18))
	...
	{
	case 'a':
	  if (!http_memcmp_case (name + 1, "uthentication-info", 18))
	    return HTTP_HEADER_AUTHENTICATION_INFO;
	  break;
	case 'c':
	  if (!http_memcmp_case (name + 1, "ontent-disposition", 18))
	    return HTTP_HEADER_CONTENT_DISPOSITION;
	  break;
	case 'i':
	  if (!http_memcmp_case (name + 1, "f-unmodified-since", 18))
	    return HTTP_HEADER_IF_UNMODIFIED_SINCE;
	  break;
	case 'p':
	  if (!http_memcmp_case (name + 1, "roxy-authorization", 18))
	    return HTTP_HEADER_PROXY_AUTHORIZATION;
	  break;
	case 't':
	  if (!http_memcmp_case (name + 1, "iming-allow-origin", 18))
	    return HTTP_HEADER_TIMING_ALLOW_ORIGIN;
	  break;
	case 'w':
	  if (!http_memcmp_case (name + 1, "ant-content-digest", 18))
	...
	{
	case 'e':
	  if (!http_memcmp_case (name, "access-control-max-ag", 21))
	    return HTTP_HEADER_ACCESS_CONTROL_MAX_AGE;
	  break;
	case 'l':
	  if (!http_memcmp_case (name, "authentication-contro", 21))
	    return HTTP_HEADER_AUTHENTICATION_CONTROL;
	  break;
	case 's':
	  if (!http_memcmp_case (name, "x-content-type-option", 21))
	    return HTTP_HEADER_X_CONTENT_TYPE_OPTIONS;
	  break;
	}
      break;
    case 23:
      if (!http_memcmp_case (name, "content-security-policy", 23))
	...
	{
	case 'p':
	  if (!http_memcmp_case (name + 1, "roxy-authentication-info", 24))
	    return HTTP_HEADER_PROXY_AUTHENTICATION_INFO;
	  break;
	case 's':
	  if (!http_memcmp_case (name + 1, "trict-transport-security", 24))
	    return HTTP_HEADER_STRICT_TRANSPORT_SECURITY;
	  break;
	case 'u':
	  if (!http_memcmp_case (name + 1, "pgrade-insecure-requests", 24))
	    return HTTP_HEADER_UPGRADE_INSECURE_REQUESTS;
	  break;
	}
      break;
    case 27:
      if (!http_memcmp_case (name, "access-control-allow-origin", 27))
	return HTTP_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN;
      break;
    case 28:
      if (!http_memcmp_case (name, "access-control-allow-headers", 28))
	return HTTP_HEADER_ACCESS_CONTROL_ALLOW_HEADERS;
      if (!http_memcmp_case (name, "access-control-allow-methods", 28))
	...
	{
	case 's':
	  if (!http_memcmp_case (name, "access-control-expose-header", 28))
	    return HTTP_HEADER_ACCESS_CONTROL_EXPOSE_HEADERS;
	  break;
	case 'd':
	  if (!http_memcmp_case (name, "access-control-request-method", 28))
	    return HTTP_HEADER_ACCESS_CONTROL_REQUEST_METHOD;
	  break;
	}
      break;
    case 30:
      if (!http_memcmp_case (name, "access-control-request-headers", 30))
	return HTTP_HEADER_ACCESS_CONTROL_REQUEST_HEADERS;
      break;
    case 32:
      if (!http_memcmp_case (name, "access-control-allow-credentials", 32))
...
static inline void
http_headers_rx_to_tx (http_msg_t msg, u8 *rx_buf, http_headers_ctx_t *ctx)
{
  http_field_line_t *field_lines, *field_line;
  http_token_t name, value;
  ...
  vec_foreach (field_line, field_lines)
	...
	{
	  http_add_custom_header (ctx, name.base, name.len, value.base, value.len);
	}
    }
}
