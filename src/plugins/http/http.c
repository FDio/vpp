...
http_ctx_alloc_w_thread (u32 thread_index)
{
  ...
  http_tc_t *ctx;
  pool_get_zero (wrk->conn_pool, ctx);
  ctx->c_thread_index = thread_index;
  ctx->h_ctx_handle = ctx - wrk->conn_pool;
...
{
  http_worker_t *wrk = http_worker_get (thread_index);
  return pool_elt_at_index (wrk->conn_pool, ctx_index);
}

void
http_ctx_free (http_tc_t *ctx)
{
  http_worker_t *wrk = http_worker_get (ctx->c_thread_index);
  pool_put (wrk->conn_pool, ctx);
}
...
void
http_disconnect_transport (http_tc_t *ctx)
{
  vnet_disconnect_args_t a = {
    .handle = ctx->h_tc_session_handle,
...
http_connection_timer_start (http_tc_t * hc)
{
  ...
  hc->timer_handle = tw_timer_start_2t_1w_2048sl (&hm->tw, hs_handle, 0, 60);
...
http_session_close_cb (void *hc_handlep)
{
  ...
  hc = http_ctx_get_w_thread (hs_handle & 0x00FFFFFF, hs_handle >> 24);

  if (hsm->debug_level > 1)
    clib_warning ("terminate thread %d index %d hs %llx",
		  hs_handle >> 24, hs_handle & 0x00FFFFFF, hc);
  if (!hc)
    return;
  ...
  for (i = 0; i < vec_len (expired_timers); i++)
    {
      ...
      session_send_rpc_evt_to_thread (hs_handle >> 24,
                                      http_session_close_cb,
				      uword_to_pointer (hs_handle, void *));
    }
}

static uword
http_timer_process (vlib_main_t * vm, vlib_node_runtime_t * rt,
			    vlib_frame_t * f)
{
  http_main_t *hm = &http_main;
  f64 now, timeout = 1.0;
...
void
http_cache_detach_entry (http_tc_t * hc)
{
  // TODO
}
...
u8 *
format_http_sm_caller (u8 * s, va_list * args)
{
  http_sm_caller_t cf = va_arg(*args, http_sm_caller_t);
  char *which = "bogus!";

  switch (cf)
    {
    case HTTP_SM_CALLED_FROM_RX:
      which = "from rx";
      break;
    case HTTP_SM_CALLED_FROM_TX:
      which = "from tx";
      break;
    case HTTP_SM_CALLED_FROM_TIMER:
      which = "from timer";
      break;
...
static u32
static_send_data (http_tc_t * hc, u8 * data, u32 length, u32 offset)
{
  ...
  sent = svm_fifo_enqueue (hc->tx_fifo, to_send, data + offset);

  if (sent <= 0)
    return offset;

  if (svm_fifo_set_event (hc->tx_fifo))
    session_send_io_evt_to_thread (hc->tx_fifo, SESSION_IO_EVT_TX_FLUSH);
...
send_error (http_tc_t * hc, char *str)
{
  ...
  static_send_data (hc, data, vec_len (data), 0);
  vec_free (data);
}
...
read_request (http_tc_t * hc)
{
  ...
  max_dequeue = svm_fifo_max_dequeue (hc->rx_fifo);
  if (PREDICT_FALSE (max_dequeue == 0))
    return -1;

  vec_validate (hc->rx_buf, cursize + max_dequeue - 1);
  n_read = app_recv_stream_raw (hc->rx_fifo, hc->rx_buf + cursize,
				max_dequeue, 0, 0 /* peek */ );
  ASSERT (n_read == max_dequeue);
  if (svm_fifo_is_empty (hc->rx_fifo))
    svm_fifo_unset_event (hc->rx_fifo);
...
/**
 * start state - parse request method and data
 */
static int
state_start (http_tc_t *hc, http_sm_caller_t cf)
{
  http_main_t *hm = &http_main;
  int i, rv;
  ...
  /* send 200 OK first */
  static_send_data (hc, (u8 *) "HTTP/1.1 200 OK\r\n", 17, 0);
  hc->req_state = HTTP_REQ_STATE_OK_SENT;

  return 1;
}

static int
state_send_more_data (http_tc_t *hc, http_sm_caller_t cf)
{

  hc->tx_buf_offset = static_send_data (hc, hc->tx_buf, vec_len (hc->tx_buf),
	                                hc->tx_buf_offset);

  if (hc->tx_buf_offset < vec_len (hc->tx_buf))
    {
      svm_fifo_add_want_deq_ntf (hc->tx_fifo, SVM_FIFO_WANT_DEQ_NOTIF);
      hc->req_state = HTTP_REQ_STATE_SEND_MORE_DATA;
      return 0;
    }

  /* Finished with this transaction, back to HTTP_REQ_STATE_START */
  hc->req_state = HTTP_REQ_STATE_START;

  /* Let go of the file cache entry */
  http_cache_detach_entry (hc);

  return 0;
}

static char *
http_req_content_type (http_tc_t *hc)
{
...
static int
state_sent_ok (http_tc_t *hc, http_sm_caller_t cf)
{
  http_main_t *hm = &http_main;
  u8 *header;
  u32 offset;
  ...
  header = format (0, http_response_template,
                   ...
                   http_req_content_type (hc), vec_len (hc->tx_buf));

  offset = static_send_data (hc, header, vec_len (header), 0);
...
typedef int (*http_sm_handler) (http_tc_t *, http_sm_caller_t);
...
static void
http_req_run_state_machine (http_tc_t *hc, http_sm_caller_t cf)
{
  int rv;

  do
    {
      rv = req_state_funcs[hc->req_state] (hc, cf);
      if (rv < 0)
	return;
    }
  while (rv);
...
http_ts_rx_callback (session_t *ts)
{
  ...
  hc = http_ctx_get_w_thread (ts->opaque, ts->thread_index);
  http_req_run_state_machine (hc, HTTP_SM_CALLED_FROM_RX);
...
http_transport_enable (vlib_main_t *vm, u8 is_en)
{
  ...
  clib_timebase_init (&hm->timebase, 0 /* GMT */ ,
   ...
   */
  tw_timer_wheel_init_2t_1w_2048sl (&hm->tw,
	                            http_timer_process_expired_cb,
...
http_app_tx_callback (void *session, transport_send_params_t * sp)
{
  ...
  ctx = http_ctx_get_w_thread (as->connection_index, as->thread_index);

  // TODO
  if (ctx->state == HTTP_CONN_STATE_CLOSED)
    {
      if (!svm_fifo_max_dequeue_cons (as->rx_fifo))
	http_disconnect_transport (ctx);
    }
  return 0;
}
