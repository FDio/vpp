...
u32
http_ts_write_data (http_conn_t *hc, u8 *data, u32 length)
{
...
int
http_ts_peek_data (http_conn_t *hc, u8 *rx_buf)
{
  u32 max_deq;
  session_t *ts;
  int n_read;
  ...
  n_read = svm_fifo_peek (ts->rx_fifo, 0, max_deq, rx_buf);
...
void
http_ts_drop_peek_data (http_conn_t *hc, u32 len)
{
  session_t *ts;
...
http_ts_rx_callback (session_t *ts)
{
  ...
  /* FIXME */
  http_vfts[HTTP_VERSION_1].transport_rx_callback (hc);
...
http_app_tx_callback (void *session, transport_send_params_t *sp)
{
  ...
  /* FIXME */
  http_vfts[HTTP_VERSION_1].app_tx_callback (hc, sp);
...
http_app_rx_evt_cb (transport_connection_t *tc)
{
  ...
  /* FIXME */
  http_vfts[HTTP_VERSION_1].app_rx_evt_callback (hc);
