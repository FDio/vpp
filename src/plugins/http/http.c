...
http_buffer_get_segs (http_buffer_t *hb, u32 max_len, u32 *n_segs)
{
  ...
  len = svm_fifo_segments2 (hb->src, 0, hb->segs, &_n_segs, max_len);
  if (len < 0)
    return 0;
...
{
  hb->offset += len;
  clib_warning ("drained %u offset %u", len, hb->offset);
  svm_fifo_dequeue_drop (hb->src, len);
//  if (svm_fifo_needs_deq_ntf (hb->src, len))
//    {
//      session_t *as;
//      as = session_get (hb->src->shr->master_session_index,
//                        hb->src->master_thread_index);
 ...
 */
static int
state_wait_method (http_tc_t *hc, transport_send_params_t *sp)
{
  http_main_t *hm = &http_main;
  app_worker_t *app_wrk;
  http_msg_t msg;
 ...
 */
static int
state_wait_app (http_tc_t *hc, transport_send_params_t *sp)
{
  http_main_t *hm = &http_main;
  http_msg_t msg;
  session_t *as;
...
static int
state_send_more_data (http_tc_t *hc, transport_send_params_t *sp)
{
  ...
  seg = http_buffer_get_segs (hb, max_send, &n_segs);
  if (!seg)
    return 0;
  ...
  clib_warning ("sent %u n_segs %u to tcp %u:%u max deq %u", sent,
		n_segs, ts->thread_index, ts->session_index,
		svm_fifo_max_dequeue (ts->tx_fifo));

  http_buffer_drain (hb, sent);

  /* Ask scheduler handler to notify app of deq event if needed */
...
typedef int (*http_sm_handler) (http_tc_t *, transport_send_params_t *sp);
...
static void
http_req_run_state_machine (http_tc_t *hc, transport_send_params_t *sp)
{
  int rv;
