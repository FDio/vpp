...
http_ts_accept_stream (session_t *stream_session)
{
  ...
  hc = http_ctx_get_w_thread (((http_conn_handle_t) conn_session->opaque).conn_index, thread_index);
  ...
  HTTP_DBG (1, "Accepted on connection [%u]%x new stream %x", thread_index,
	    stream->hc_http_conn_index, stream_index);
