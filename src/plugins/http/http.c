...
http_ts_accept_callback (session_t *ts)
{
  ...
  return app_worker_accept_notify (app_wrk, as);
}
...
/**
 * wait for request method from peer - parse request method and data
 */
static int
state_wait_method (http_tc_t *hc, http_sm_caller_t cf)
{
  http_main_t *hm = &http_main;
  int i, rv;
...
static int
state_wait_app (http_tc_t *hc, http_sm_caller_t cf)
{
  // TODO read message from app
...
state_send_more_data (http_tc_t *hc, http_sm_caller_t cf)
{
  ...
  /* Finished with this transaction, back to HTTP_REQ_STATE_WAIT_METHOD */
  hc->req_state = HTTP_REQ_STATE_WAIT_METHOD;
...
http_app_tx_callback (void *session, transport_send_params_t * sp)
{
  ...
  hc = http_ctx_get_w_thread (as->connection_index, as->thread_index);
