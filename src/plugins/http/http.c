...
static int
http_conn_init_tx_buf (http_tc_t *hc, http_msg_t *msg, svm_fifo_t *f)
{
  int i, len, max_len;
  const int n_segs = 5;
  svm_fifo_seg_t fs[n_segs];

  hc->tx_buf.len = sizeof (http_msg_t) + msg->data.len;
  hc->tx_buf.offset = 0;
  hc->tx_buf.cur_seg = 0;

  max_len = hc->tx_buf.len;
  while (max_len)
    {
...
http_conn_free_tx_buf (http_tc_t *hc)
{
  ...
  /* Release the data from the fifo */
  svm_fifo_dequeue_drop (hc->tx_fifo, hb->len + sizeof (http_msg_t));
  vec_free (hc->tx_buf.segs);
}
...
http_connection_timer_start (http_tc_t *hc)
{
  ...
  hc->timer_handle = tw_timer_start_2t_1w_2048sl (&hm->tw, hs_handle, 0,
                                                  timeout);
...
read_request (http_tc_t *hc)
{
  ...
  n_read = app_recv_stream_raw (hc->rx_fifo, hc->rx_buf + cursize, max_dequeue,
				0, 0 /* peek */);
...
state_wait_method (http_tc_t *hc)
{
  ...
  as = session_get_from_handle (hc->h_pa_session_handle);
  if (svm_fifo_enqueue_segments (as->rx_fifo, segs, 2, 0 /* allow partial */))
    {
...
state_wait_app (http_tc_t *hc)
{
  ...
  if (http_conn_init_tx_buf (hc, &msg, as->tx_fifo))
    {
  ...
  /* Add 200 OK first */
  header = format (0, "%s", "HTTP/1.1 200 OK\r\n");
...
{
  http_buffer_t *hb = &hc->tx_buf;
  u32 max_send = 64 << 10, si, n_segs, to_send = 0;
  svm_fifo_seg_t *seg;
  int rv;
  ...
  while (si < vec_len (hb->segs) && hb->segs[si].len + to_send <= max_send)
    {
      si += 1;
      to_send += hb->segs[si].len;
    }

  seg = &hb->segs[hb->cur_seg];
  n_segs = si == hb->cur_seg ? 1 : si - hb->cur_seg;

  rv = svm_fifo_enqueue_segments (hc->tx_fifo, seg, n_segs,
                                  1 /* allow partial */);
...
http_app_tx_callback (void *session, transport_send_params_t *sp)
{
  ...
  if (hc->req_state == HTTP_REQ_STATE_SEND_MORE_DATA)
    {
      if (svm_fifo_max_enqueue (hc->tx_fifo) < 16 << 10)
	{
