...
http_conn_alloc_w_thread (u32 thread_index)
{
  ...
  ctx->h_hc_index = ctx - wrk->conn_pool;
  ctx->h_pa_session_handle = SESSION_INVALID_HANDLE;
  return ctx->h_hc_index;
}

static inline http_tc_t *
http_conn_get_w_thread (u32 ctx_index, u32 thread_index)
{
  http_worker_t *wrk = http_worker_get (thread_index);
