...
static int
http_test_lookup_header_name (vlib_main_t *vm)
{
  http_header_name_t name;

#define _(sym, str_canonical, str_lower, hpack_index)                                              \
  name = http_lookup_header_name (http_token_lit (str_lower));                                     \
  if (name != HTTP_HEADER_##sym)                                                                   \
    {                                                                                              \
      vlib_cli_output (vm, "FAIL: %s != %u", str_lower, name);                                     \
      return 1;                                                                                    \
    }
  foreach_http_header_name
#undef _

    vlib_cli_output (vm, "PASS: http_lookup_header_name");
  return 0;
}
		      ...
		      vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	res = http_test_lookup_header_name (vm);
      ...
      else if (unformat (input, "all"))
	{
	  ...
	  if ((res = http_test_http_header_table (vm)))
	    goto done;
	  if ((res = http_test_lookup_header_name (vm)))
	    goto done;
