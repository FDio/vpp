...
hpack_decode_huffman (u8 **src, u8 *end, u8 **buf, uword *buf_len)
{
  ...
  while (1)
    {
      /* refill */
      while (p < end && accumulator_len <= sizeof (accumulator) * 8 - 8)
	{
	  accumulator <<= 8;
	  accumulator_len += 8;
	  accumulator |= (u64) *p++;
	...
	{
	  /* FIXME slow path (long codes) */
	  ASSERT (0);
	}
      /* all done */
      if (p == end && accumulator_len < 8)
	{
	  /* there might be one more symbol encoded */
...
hpack_decode_string (u8 **src, u8 *end, u8 **buf, uword *buf_len)
{
  ...
  if (is_huffman)
    {
      *src = (p + len);
      len = hpack_decode_huffman (&p, p + len, buf, buf_len);
    }
  else
    {
      if (len > *buf_len)
