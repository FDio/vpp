...
always_inline void
http_as_drop_data_all (http_req_t *req)
{
...
always_inline void
http_as_drop_data (http_req_t *req, u32 len)
{
  session_t *as = session_get_from_handle (req->app_session_handle);
...
always_inline u32
http_as_max_enqueue (http_req_t *req)
{
  session_t *as = session_get_from_handle (req->app_session_handle);
...
always_inline u32
http_as_max_dequeue (http_req_t *req)
{
  session_t *as = session_get_from_handle (req->app_session_handle);
...
always_inline int
http_as_write_segments (http_req_t *req, const svm_fifo_seg_t segs[],
			u32 n_segs)
{
  session_t *as = session_get_from_handle (req->app_session_handle);
  return svm_fifo_enqueue_segments (as->rx_fifo, segs, n_segs,
				    0 /* allow partial */);
}

always_inline void
http_as_get_segments (http_req_t *req, svm_fifo_seg_t *segs, u32 *n_segs,
		      u32 max_bytes)
{
  session_t *as = session_get_from_handle (req->app_session_handle);
  svm_fifo_segments (as->tx_fifo, 0, segs, n_segs, max_bytes);
}

always_inline void
http_as_want_deq_ntf (http_req_t *req)
{
  session_t *as = session_get_from_handle (req->app_session_handle);
...
http_io_ts_read (http_conn_t *hc, u8 *buf, u32 len, u8 peek)
{
  ...
  if (peek)
    return svm_fifo_peek (ts->rx_fifo, 0, len, buf);

  return svm_fifo_dequeue (ts->rx_fifo, len, buf);
}

always_inline void
http_io_ts_drain (http_conn_t *hc, u32 len, u8 clear_evt)
{
  session_t *ts = session_get_from_handle (hc->h_tc_session_handle);
  svm_fifo_dequeue_drop (ts->rx_fifo, len);

  if (clear_evt && svm_fifo_is_empty (ts->rx_fifo))
...
always_inline u32
http_io_ts_write (http_conn_t *hc, u8 *data, u32 len, u8 set_evt)
{
  session_t *ts;
  int rv;
  ...
  rv = svm_fifo_enqueue (ts->tx_fifo, len, data);
  ASSERT (rv == len);

  if (set_evt && svm_fifo_set_event (ts->tx_fifo))
