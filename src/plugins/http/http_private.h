...
u32 http_ts_write_data (http_conn_t *hc, u8 *data, u32 length);
int http_ts_peek_data (http_conn_t *hc, u8 *rx_buf);
void http_ts_drop_peek_data (http_conn_t *hc, u32 len);
...
always_inline void
http_ts_drop_data (http_conn_t *hc, u32 len)
{
...
always_inline void
http_ts_drop_data_all (http_conn_t *hc)
{
  session_t *ts = session_get_from_handle (hc->h_tc_session_handle);
...
always_inline u32
http_ts_max_deq (http_conn_t *hc)
{
  session_t *ts = session_get_from_handle (hc->h_tc_session_handle);
