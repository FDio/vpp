			       ...
			       transport_send_params_t *sp, http3_error_t *error, u32 *n_deq)
{
  ...
  payload = http_encap_udp_payload_datagram (tx_buf, hdr.data_length);
  ASSERT ((payload - tx_buf) <= HTTP_UDP_PROXY_DATAGRAM_CAPSULE_OVERHEAD);
  capsule_size = (payload - tx_buf) + hdr.data_length;
  /* read payload */
  http_io_as_peek (&sctx->base, payload, hdr.data_length, sizeof (hdr));
