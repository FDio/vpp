...
static_always_inline http_req_t *
http3_stream_ctx_alloc (http_ctx_t *stream, u8 is_parent)
{
  ...
  sh.req_index = si;
  sctx->hr_req_handle = sh.as_u32;
  sctx->hr_hc_index = stream->hc_hc_index;
  sctx->c_s_index = SESSION_INVALID_INDEX;
  sctx->c_thread_index = stream->c_thread_index;
...
static_always_inline void
http3_stream_ctx_free (http_ctx_t *stream)
{
  http_req_t *sctx;
  http_ctx_t *hc;
  http3_worker_ctx_t *wrk = http3_worker_get (stream->c_thread_index);
...
static_always_inline void
http3_stream_ctx_free_w_index (u32 stream_index, clib_thread_index_t thread_index,
			       http_ctx_t *stream)
{
  http_ctx_t *hc;
  http3_worker_ctx_t *wrk = http3_worker_get (thread_index);
  http_req_t *sctx = pool_elt_at_index (wrk->stream_pool, stream_index);

  HTTP_DBG (1, "sctx [%u]%x", thread_index, ((http_req_handle_t) sctx->hr_req_handle).req_index);
  if (sctx->flags & HTTP_REQ_F_IS_PARENT)
    {
      hc = http_conn_get_w_thread (stream->hc_http_conn_index, stream->c_thread_index);
      hc->hc_parent_req_index = SESSION_INVALID_INDEX;
...
static_always_inline void
http3_stream_close (http_ctx_t *stream, http_req_t *sctx)
{
  http_ctx_t *hc;
  ...
  if (stream->flags & HTTP_CONN_F_BIDIRECTIONAL_STREAM)
	...
	{
	  HTTP_DBG (1, "stream [%u]%x sctx %x app already closed, confirm", stream->c_thread_index,
		    stream->hc_hc_index, ((http_req_handle_t) sctx->hr_req_handle).req_index);
	  session_transport_closed_notify (&sctx->connection);
	}
      else
	{
	  HTTP_DBG (1, "stream [%u]%x sctx %x all done closing, notify app", stream->c_thread_index,
		    stream->hc_hc_index, ((http_req_handle_t) sctx->hr_req_handle).req_index);
...
static_always_inline void
http3_stream_terminate (http_ctx_t *stream, http_req_t *sctx, http3_error_t err)
{
  /* for client we open quic stream when we get request */
  ASSERT (sctx->state != HTTP_REQ_STATE_WAIT_APP_METHOD);
  ...
  if (!(sctx->flags & HTTP_REQ_F_APP_CLOSED) || !(stream->flags & HTTP_CONN_F_NO_APP_SESSION))
...
static_always_inline void
http3_stream_error_terminate_conn (http_ctx_t *stream, http_req_t *sctx, http3_error_t err)
{
  http_ctx_t *hc;
...
static_always_inline void
http3_stream_ctx_reset (http_ctx_t *old_stream, http_req_t *sctx)
{
...
http3_send_goaway (http_ctx_t *hc)
{
  ...
  ctrl_stream = http_conn_get_w_thread (hc->our_ctrl_stream_hc_index, hc->c_thread_index);
  buf = http_get_tx_buf (ctrl_stream);
...
static void
http3_stream_app_close (http_req_t *sctx, http_ctx_t *stream, u8 is_shutdown)
{
  /* Wait for all data to be written to ts */
  if (http_io_as_max_read (sctx))
    {
...
static void
http3_stream_app_close_parent (http_req_t *sctx, http_ctx_t *stream, u8 is_shutdown)
{
  http_ctx_t *hc;
...
static void
http3_stream_app_close_tunnel (http_req_t *sctx, http_ctx_t *stream, u8 is_shutdown)
{
  http_ctx_t *hc;

  sctx->flags |= is_shutdown ? HTTP_REQ_F_SHUTDOWN_TUNNEL : 0;
  /* Wait for all data to be written to ts */
  if (http_io_as_max_read (sctx))
    {
      HTTP_DBG (1, "wait for all data to be written to ts");
...
{
  ASSERT (http_hc_is_valid (stream->hc_http_conn_index, stream->c_thread_index));
  http_ctx_t *hc = http_conn_get_w_thread (stream->hc_http_conn_index, stream->c_thread_index);
...
http3_req_state_wait_app_reply (http_ctx_t *stream, http_req_t *sctx, transport_send_params_t *sp,
				http3_error_t *error, u32 *n_deq)
{
  ...
  if (sctx->flags & HTTP_REQ_F_IS_TUNNEL)
	...
	default:
	  ...
	  sctx->flags &= ~HTTP_REQ_F_IS_TUNNEL;
	  break;
	}
    }
  ...
  if (msg.data.headers_len)
    app_headers = http_get_app_header_list (sctx, &msg);
  ...
  if (msg.data.body_len)
    {
      ASSERT (!(sctx->flags & HTTP_REQ_F_IS_TUNNEL));
      http_req_tx_buffer_init (sctx, &msg);
      http_req_state_change (sctx, HTTP_REQ_STATE_APP_IO_MORE_DATA);
      rv = HTTP_SM_CONTINUE;
    }
  else
    {
...
static http_sm_result_t
http3_req_state_wait_app_method (http_ctx_t *hc, http_req_t *sctx, transport_send_params_t *sp,
				 http3_error_t *error, u32 *n_deq)
{
  http_msg_t msg;
  ...
  if (msg.method_type == HTTP_REQ_CONNECT)
      ...
      if (msg.data.upgrade_proto != HTTP_UPGRADE_PROTO_NA)
	{
	  ...
	  control_data.path = http_get_app_target (sctx, &msg);
	...
	{
	  control_data.authority = http_get_app_target (sctx, &msg);
  ...
  else if (!(sctx->flags & HTTP_REQ_F_IS_TUNNEL))
    {
...
static http_sm_result_t
http3_req_state_app_io_more_data (http_ctx_t *stream, http_req_t *sctx, transport_send_params_t *sp,
				  http3_error_t *error, u32 *n_deq)
{
  http_buffer_t *hb = &sctx->tx_buf;
  ...
  if (finished)
      ...
      else
	{
	  /* all done, close stream for sending */
	  http_half_close_transport_stream (stream);
	  http_req_state_change (sctx, HTTP_REQ_STATE_WAIT_TRANSPORT_REPLY);
	}
    }
...
static http_sm_result_t
http3_req_state_tunnel_tx (http_ctx_t *stream, http_req_t *sctx, transport_send_params_t *sp,
			   http3_error_t *error, u32 *n_deq)
{
  ...
  max_read = http_io_as_max_read (sctx);
  if (max_read == 0)
    {
  ...
  n_read = http_io_as_read_segs (sctx, segs + 1, &n_segs, max_read);
  ...
  ASSERT (n_written == n_read);
  http_io_as_drain (sctx, n_written);
  http_io_ts_after_write (stream, 0);
...
static http_sm_result_t
http3_req_state_udp_tunnel_tx (http_ctx_t *stream, http_req_t *sctx, transport_send_params_t *sp,
			       http3_error_t *error, u32 *n_deq)
{
  ...
  http_io_as_peek (sctx, (u8 *) &hdr, sizeof (hdr), 0);
  ...
  http_io_as_drain (sctx, sizeof (hdr));
  ...
  /* read payload */
  http_io_as_read_segs (sctx, segs + 2, &n_segs, hdr.data_length);
  ...
  http_io_as_drain (sctx, hdr.data_length);
  http_io_ts_after_write (stream, 0);
...
static_always_inline void
http3_stream_resp_not_implemented (http_ctx_t *stream, http_req_t *sctx)
{
  u8 fh_buf[HTTP3_FRAME_HEADER_MAX_LEN];
  u8 fh_len;
  u32 headers_len, n_written;
...
static http_sm_result_t
http3_req_state_wait_transport_method (http_ctx_t *stream, http_req_t *sctx,
				       ...
				       u32 *n_deq)
{
  ...
  *n_deq = sctx->fh.length;
  ...
  hc = http_conn_get_w_thread (stream->hc_http_conn_index, stream->c_thread_index);
  *error =
    qpack_parse_request (rx_buf, sctx->fh.length, wrk->header_list, vec_len (wrk->header_list),
			 &control_data, &sctx->headers, &hc->qpack_decoder_ctx);
  ...
  sctx->control_data_len = control_data.control_data_len;
  sctx->headers_offset = control_data.headers - wrk->header_list;
  sctx->headers_len = control_data.headers_len;
...
#define _(sym, str)                                                                                \
			       ...
			       http_token_lit (str))) sctx->upgrade_proto =                        \
...
#undef _
      ...
      else
	{
	  ...
	  sctx->upgrade_proto = HTTP_UPGRADE_PROTO_NA;
	}
      sctx->flags |= HTTP_REQ_F_IS_TUNNEL;
    }
  if (control_data.content_len_header_index != ~0)
    {
      sctx->content_len_header_index = control_data.content_len_header_index;
      if (http_parse_content_length (sctx, wrk->header_list))
	{
	  http3_stream_terminate (stream, sctx, HTTP3_ERROR_MESSAGE_ERROR);
	  return HTTP_SM_STOP;
	}
  ...
  sctx->to_recv = sctx->body_len;

  sctx->target_query_offset = 0;
  sctx->target_query_len = 0;
  if (control_data.parsed_bitmap & HPACK_PSEUDO_HEADER_PATH_PARSED)
    {
      sctx->target_path_len = control_data.path_len;
      sctx->target_path_offset = control_data.path - wrk->header_list;
      /* drop leading slash */
      sctx->target_path_offset++;
      sctx->target_path_len--;
      http_identify_optional_query (sctx, wrk->header_list);
    }
  else
    {
      sctx->target_path_len = 0;
      sctx->target_path_offset = 0;
    }

  msg.type = HTTP_MSG_REQUEST;
  msg.method_type = control_data.method;
  msg.data.type = HTTP_MSG_DATA_INLINE;
  msg.data.len = sctx->control_data_len;
  ...
  msg.data.target_path_offset = sctx->target_path_offset;
  msg.data.target_path_len = sctx->target_path_len;
  msg.data.target_query_offset = sctx->target_query_offset;
  msg.data.target_query_len = sctx->target_query_len;
  msg.data.headers_offset = sctx->headers_offset;
  msg.data.headers_len = sctx->headers_len;
  msg.data.headers_ctx = pointer_to_uword (sctx->headers);
  msg.data.upgrade_proto = HTTP_UPGRADE_PROTO_NA;
  msg.data.body_offset = sctx->control_data_len;
  msg.data.body_len = sctx->body_len;
  msg.data.upgrade_proto = sctx->upgrade_proto;

  svm_fifo_seg_t segs[2] = { { (u8 *) &msg, sizeof (msg) },
			     { wrk->header_list, sctx->control_data_len } };
  HTTP_DBG (3, "%U", format_http_bytes, wrk->header_list, sctx->control_data_len);
  http_io_as_write_segs (sctx, segs, 2);
  http_req_state_change (sctx, new_state);
  http_app_worker_rx_notify (sctx);
  sctx->fh.length = 0;
...
static http_sm_result_t
http3_req_state_wait_transport_reply (http_ctx_t *stream, http_req_t *sctx,
				      transport_send_params_t *sp, http3_error_t *error, u32 *n_deq)
{
  ...
  *n_deq = sctx->fh.length;
  ...
  vec_reset_length (sctx->headers);
  *error =
    qpack_parse_response (rx_buf, sctx->fh.length, wrk->header_list, vec_len (wrk->header_list),
			  &control_data, &sctx->headers, &hc->qpack_decoder_ctx);
  if (*error != HTTP3_ERROR_NO_ERROR)
    {
  ...
  sctx->headers_offset = control_data.headers - wrk->header_list;
  sctx->headers_len = control_data.headers_len;
  sctx->status_code = control_data.sc;
...
static http_sm_result_t
http3_req_state_transport_io_more_data (http_ctx_t *stream, http_req_t *sctx,
					transport_send_params_t *sp, http3_error_t *error,
					u32 *n_deq)
{
  ...
  if (sctx->fh.length > sctx->to_recv)
    {
      HTTP_DBG (1, "received more data than expected, fh.len %lu to_recv %lu", sctx->fh.length,
		sctx->to_recv);
  ...
  max_enq = http_io_as_max_write (sctx);
  if (max_enq == 0)
    {
      HTTP_DBG (1, "app's rx fifo full");
      http_io_as_add_want_deq_ntf (sctx);
      *n_deq = 0;
      return HTTP_SM_STOP;
    }

  http_io_ts_read_segs (stream, segs, &n_segs, clib_min (max_deq, max_enq));
  n_written = http_io_as_write_segs (sctx, segs, n_segs);
  ASSERT (sctx->fh.length >= n_written);
  sctx->to_recv -= n_written;
  sctx->fh.length -= n_written;
  http_io_ts_drain (stream, n_written);
  *n_deq = n_written;
  HTTP_DBG (2, "written %lu to recv %lu", n_written, sctx->to_recv);

  if (sctx->to_recv == 0)
    {
      if (stream->flags & HTTP_CONN_F_IS_SERVER)
	http_req_state_change (sctx, HTTP_REQ_STATE_WAIT_APP_REPLY);
      else
	{
	  /* we are done wait for the next app request */
	  http_req_state_change (sctx, HTTP_REQ_STATE_WAIT_APP_METHOD);
	  http3_stream_ctx_reset (stream, sctx);
	}
      res = HTTP_SM_STOP;
    }
...
static http_sm_result_t
http3_req_state_tunnel_rx (http_ctx_t *stream, http_req_t *sctx, transport_send_params_t *sp,
			   http3_error_t *error, u32 *n_deq)
{
  u32 max_enq, max_deq, n_written, n_segs = 2;
  svm_fifo_seg_t segs[n_segs];
  ...
  if ((sctx->flags & HTTP_REQ_F_APP_CLOSED) && !(sctx->flags & HTTP_REQ_F_SHUTDOWN_TUNNEL))
    {
...
static http_sm_result_t
http3_req_state_udp_tunnel_rx (http_ctx_t *stream, http_req_t *sctx, transport_send_params_t *sp,
			       http3_error_t *error, u32 *n_deq)
{
  u8 *rx_buf;
  int rv;
  ...
  if (http_io_as_max_write (sctx) < dgram_size)
    {
      HTTP_DBG (1, "app's rx fifo full");
      http_io_as_add_want_deq_ntf (sctx);
      *n_deq = 0;
...
typedef http_sm_result_t (*http3_sm_handler) (http_ctx_t *hc, http_req_t *sctx,
					      transport_send_params_t *sp, http3_error_t *error,
					      u32 *n_deq);
...
static_always_inline int
http3_req_state_is_tx_valid (http_req_t *sctx)
{
  return tx_state_funcs[sctx->state] ? 1 : 0;
}

static_always_inline http3_error_t
http3_req_run_tx_state_machine (http_ctx_t *stream, http_req_t *sctx, transport_send_params_t *sp)
{
  ...
  do
    {
      res = tx_state_funcs[sctx->state](stream, sctx, sp, &error, 0);
...
static_always_inline int
http3_stream_read_frame_header (http_req_t *sctx, http_ctx_t *stream, u32 *to_deq,
				http3_frame_header_t *fh)
{
  ...
  HTTP_DBG (1, "stream [%u]%x sctx %x, frame type: %x, payload len: %u", stream->c_thread_index,
	    stream->hc_hc_index, ((http_req_handle_t) sctx->hr_req_handle).req_index, fh->type,
	    fh->length);
...
static_always_inline http3_error_t
http3_stream_peek_frame_header (http_req_t *sctx, http_ctx_t *stream)
{
  u8 *rx_buf;
  u32 n_read;
...
static_always_inline void
http3_stream_drop_frame_header (http_req_t *sctx, http_ctx_t *stream)
{
  http_io_ts_drain (stream, sctx->fh.header_len);
}

static_always_inline int
http3_stream_read_settings (http_req_t *sctx, http_ctx_t *stream, u32 *to_deq,
			    http3_frame_header_t *fh)
{
  http3_error_t err;
  http_ctx_t *hc;
...
static u32
http3_stream_transport_rx_drain (CLIB_UNUSED (http_req_t *sctx), http_ctx_t *stream)
{
...
static_always_inline int
http3_stream_read_goaway (http_req_t *sctx, http_ctx_t *stream, u32 *to_deq,
			  http3_frame_header_t *fh)
{
  http3_error_t err;
  http_req_t *parent_sctx;
...
static u32
http3_stream_transport_rx_ctrl (http_req_t *sctx, http_ctx_t *stream)
{
  u32 to_deq, max_deq;
  http_ctx_t *hc;
...
static u32
http3_stream_transport_rx_unknown_type (http_req_t *sctx, http_ctx_t *stream)
{
  ...
  HTTP_DBG (1, "stream type %lx [%u]%x sctx %x", stream_type, stream->hc_hc_index,
	    sctx->c_thread_index, ((http_req_handle_t) sctx->hr_req_handle).req_index);
  ...
  switch (stream_type)
    {
    case HTTP3_STREAM_TYPE_CONTROL:
      if (hc->peer_ctrl_stream_sctx_index != SESSION_INVALID_INDEX)
	{
	  HTTP_DBG (1, "second control stream opened");
	  http3_stream_error_terminate_conn (stream, sctx, HTTP3_ERROR_STREAM_CREATION_ERROR);
	  return n_deq;
	}
      hc->peer_ctrl_stream_sctx_index = ((http_req_handle_t) sctx->hr_req_handle).req_index;
      sctx->transport_rx_cb = http3_stream_transport_rx_ctrl;
      break;
    case HTTP3_STREAM_TYPE_DECODER:
      if (hc->peer_decoder_stream_sctx_index != SESSION_INVALID_INDEX)
	{
	  HTTP_DBG (1, "second decoder stream opened");
	  http3_stream_error_terminate_conn (stream, sctx, HTTP3_ERROR_STREAM_CREATION_ERROR);
	  return n_deq;
	}
      hc->peer_decoder_stream_sctx_index = ((http_req_handle_t) sctx->hr_req_handle).req_index;
      sctx->transport_rx_cb = http3_stream_transport_rx_drain;
      break;
    case HTTP3_STREAM_TYPE_ENCODER:
      if (hc->peer_encoder_stream_sctx_index != SESSION_INVALID_INDEX)
	{
	  HTTP_DBG (1, "second encoder stream opened");
	  http3_stream_error_terminate_conn (stream, sctx, HTTP3_ERROR_STREAM_CREATION_ERROR);
	  return n_deq;
	}
      hc->peer_encoder_stream_sctx_index = ((http_req_handle_t) sctx->hr_req_handle).req_index;
      sctx->transport_rx_cb = http3_stream_transport_rx_drain;
      break;
...
static u32
http3_stream_transport_rx_req (http_req_t *sctx, http_ctx_t *stream, http_req_state_t headers_state)
{
  http3_error_t err;
  http_sm_result_t res = HTTP_SM_CONTINUE;
  ...
  while (res == HTTP_SM_CONTINUE && left_deq)
	...
	case HTTP3_FRAME_TYPE_HEADERS:
	  ...
	  if (sctx->state != headers_state)
	    {
	      HTTP_DBG (1, "unexpected frame, state: %U", format_http_req_state, sctx->state);
	...
	case HTTP3_FRAME_TYPE_DATA:
	  ...
	  if (!(sctx->state == HTTP_REQ_STATE_TRANSPORT_IO_MORE_DATA ||
		sctx->state == HTTP_REQ_STATE_TUNNEL || sctx->state == HTTP_REQ_STATE_UDP_TUNNEL))
	    {
	      HTTP_DBG (1, "unexpected frame, state: %U", format_http_req_state, sctx->state);
	      err = HTTP3_ERROR_FRAME_UNEXPECTED;
	      goto error;
	...
	default:
      ...
      res = rx_state_funcs[sctx->state](stream, sctx, 0, &err, &n_deq);
      left_deq -= n_deq;
...
static u32
http3_stream_transport_rx_req_server (http_req_t *sctx, http_ctx_t *stream)
{
...
static u32
http3_stream_transport_rx_req_client (http_req_t *sctx, http_ctx_t *stream)
{
  return http3_stream_transport_rx_req (sctx, stream,
...
{
  http_req_t *sctx = va_arg (*args, http_req_t *);
  http_ctx_t *stream = va_arg (*args, http_ctx_t *);
  http_ctx_t *hc;
  http_req_t *c_sctx;
  http_ctx_t *c_stream;
  ...
  if (!(stream->flags & HTTP_CONN_F_IS_SERVER && sctx->flags & HTTP_REQ_F_IS_PARENT))
    s = format (s, " %U\n", format_http3_stream_type, sctx->stream_type);
  s = format (s, " req state: %U\n", format_http_req_state, sctx->state);
  s = format (s, " flags: %U\n", format_http_req_flags, sctx);
  if (sctx->flags & HTTP_REQ_F_IS_PARENT)
    {
      hc = http_conn_get_w_thread (stream->hc_http_conn_index, stream->c_thread_index);
      s = format (s, " hc_flags: %U\n", format_http_conn_flags, stream);
      if (hc->our_ctrl_stream_hc_index != SESSION_INVALID_INDEX)
	{
	  c_stream = http_conn_get_w_thread (hc->our_ctrl_stream_hc_index, stream->c_thread_index);
	  ts = session_get_from_handle (c_stream->hc_tc_session_handle);
	  s = format (s, " our_ctrl_stream: hc_index %u ts %u:%u\n", hc->our_ctrl_stream_hc_index,
      ...
      else
	{
      ...
      if (hc->peer_ctrl_stream_sctx_index != SESSION_INVALID_INDEX)
	{
	  c_sctx = http3_stream_ctx_get (hc->peer_ctrl_stream_sctx_index, stream->c_thread_index);
	  c_stream = http_conn_get_w_thread (c_sctx->hr_hc_index, stream->c_thread_index);
	  ts = session_get_from_handle (c_stream->hc_tc_session_handle);
	...
	{
	  c_sctx =
	    http3_stream_ctx_get (hc->peer_encoder_stream_sctx_index, stream->c_thread_index);
	  c_stream = http_conn_get_w_thread (c_sctx->hr_hc_index, stream->c_thread_index);
	  ts = session_get_from_handle (c_stream->hc_tc_session_handle);
	...
	{
	  c_sctx =
	    http3_stream_ctx_get (hc->peer_decoder_stream_sctx_index, stream->c_thread_index);
	  c_stream = http_conn_get_w_thread (c_sctx->hr_hc_index, stream->c_thread_index);
	  ts = session_get_from_handle (c_stream->hc_tc_session_handle);
  ...
  if (!http3_req_state_is_tx_valid (sctx))
      ...
      else
	{
	  clib_warning ("hc [%u]%x invalid tx state: http req state "
			...
			sctx->state, format_http_conn_state, stream);
	  http3_stream_terminate (stream, sctx, HTTP3_ERROR_INTERNAL_ERROR);
	  return;
	}
    }
  ...
  if (sctx->state == HTTP_REQ_STATE_WAIT_APP_METHOD)
  ...
  else
    {
      ...
      http3_stream_terminate (stream, sctx,
			      (sctx->flags & HTTP_REQ_F_IS_TUNNEL) ? HTTP3_ERROR_CONNECT_ERROR :
...
http3_transport_connected_callback (http_ctx_t *hc)
{
  ...
  return http_conn_established (hc, sctx, hc->hc_pa_app_api_ctx);
}
...
http3_transport_stream_accept_callback (http_ctx_t *stream, http_ctx_t *hc)
{
  ...
  if (stream->flags & HTTP_CONN_F_BIDIRECTIONAL_STREAM)
      ...
      if (!(stream->flags & HTTP_CONN_F_IS_SERVER))
	{
      ...
      HTTP_DBG (1, "new req stream accepted [%u]%x", sctx->c_thread_index,
		((http_req_handle_t) sctx->hr_req_handle).req_index);
      if (http_conn_accept_request (stream, sctx, 1))
	{
      ...
      http_req_state_change (sctx, HTTP_REQ_STATE_WAIT_TRANSPORT_METHOD);
      http_stats_app_streams_opened_inc (stream->c_thread_index);
    }
  else
    {
      sctx->transport_rx_cb = http3_stream_transport_rx_unknown_type;
      HTTP_DBG (1, "new unidirectional stream accepted [%u]%x", sctx->c_thread_index,
		((http_req_handle_t) sctx->hr_req_handle).req_index);
...
http3_transport_stream_close_callback (http_ctx_t *stream)
{
  ...
  if (stream->state != HTTP_CONN_STATE_CLOSED)
      ...
      else
	{
	  ...
	  if (stream->flags & HTTP_CONN_F_IS_SERVER)
	    {
	      if (sctx->state < HTTP_REQ_STATE_WAIT_APP_REPLY && !http_io_ts_max_read (stream))
		{
		  HTTP_DBG (1, "request incomplete");
		  http3_stream_terminate (stream, sctx, HTTP3_ERROR_REQUEST_INCOMPLETE);
		  return;
...
http3_conn_connect_stream_callback (http_ctx_t *hc, u32 *req_index)
{
  ...
  sctx->transport_rx_cb = http3_stream_transport_rx_req_client;
  sctx->app_closed_cb = http3_stream_app_close;
  sctx->hr_pa_wrk_index = hc->hc_pa_wrk_index;
  http_req_state_change (sctx, HTTP_REQ_STATE_WAIT_APP_METHOD);
  http_stats_app_streams_opened_inc (thread_index);
  *req_index = sctx->hr_req_handle;
