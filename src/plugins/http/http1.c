...
static http_sm_result_t
http1_req_state_streaming_transport_io_more_data (http_conn_t *hc,
						  http_req_t *req,
						  transport_send_params_t *sp)
{
  u32 max_deq, max_enq, max_read, n_segs = 2;
  ...
  HTTP_DBG (1, "streaming received data from client");
  ...
  if (is_streaming)
    {
       ...
       */
      n_read = http_buffer_get_segs (hb, max_write - 20, &seg, &n_segs);
      if (n_read == 0)
	{
...
static http_sm_handler rx_state_funcs[HTTP_REQ_N_STATES] = {
  ...
  0, /* wait app reply */
  http1_req_state_streaming_transport_io_more_data,
