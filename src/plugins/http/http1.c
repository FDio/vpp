...
http1_send_error (http_conn_t *hc, http_status_code_t ec)
{
  ...
  HTTP_DBG (3, "%v", data);
  http_io_ts_write (hc, data, vec_len (data), 1);
  vec_free (data);
}
...
http1_read_message (http_conn_t *hc, u8 *rx_buf)
{
  ...
  n_read = http_io_ts_read (hc, rx_buf, max_deq, 1);
				      ...
				      transport_send_params_t *sp)
{
  ...
  http_io_ts_drain (hc, len, 1);
  http_app_worker_rx_notify (req);
  return HTTP_SM_STOP;

error:
  http_io_ts_drain (hc, vec_len (rx_buf), 1);
				...
				transport_send_params_t *sp)
{
  ...
  sent = http_io_ts_write (hc, response, vec_len (response), 1);
				 ...
				 transport_send_params_t *sp)
{
  ...
  sent = http_io_ts_write (hc, request, vec_len (request), 1);
			       ...
			       transport_send_params_t *sp)
{
  ...
  while (to_deq > 0)
	  ...
	  (hdr.data_length + HTTP_UDP_PROXY_DATAGRAM_CAPSULE_OVERHEAD))
	{
      ...
      /* send capsule */
      http_io_ts_write (hc, buf, capsule_size, 0);
