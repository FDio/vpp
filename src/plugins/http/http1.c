...
http1_send_error (http_conn_t *hc, http_status_code_t ec)
{
  ...
  HTTP_DBG (3, "%v", data);
  http_write_transport (hc, data, vec_len (data));
  vec_free (data);
}
				      ...
				      transport_send_params_t *sp)
{
  ...
  vec_reset_length (rx_buf);
  rv = http_transport_data_peek (hc, rx_buf);
  ...
  http_transport_data_drop (hc, len);
  vec_reset_length (rx_buf);
...
error:
  http_transport_data_drop_all (hc);
  svm_fifo_unset_event (ts->rx_fifo);
  vec_reset_length (rx_buf);
				...
				transport_send_params_t *sp)
{
  ...
  sent = http_write_transport (hc, response, vec_len (response));
  ...
  if (msg.data.body_len)
    {
      ...
      http_buffer_init (&req->tx_buf, msg_to_buf_type[msg.data.type],
				 ...
				 transport_send_params_t *sp)
{
  ...
  sent = http_write_transport (hc, request, vec_len (request));
