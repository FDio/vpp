...
http1_send_error (http_conn_t *hc, http_status_code_t ec)
{
  ...
  HTTP_DBG (3, "%v", data);
  http_ts_write_data (hc, data, vec_len (data));
  vec_free (data);
}
...
http1_parse_request_line (http_req_t *req, u8 *rx_buf, http_status_code_t *ec)
{
  ...
  /* find version */
  i = v_find_index (rx_buf, next_line_offset - 11, 11, " HTTP/");
  if (i < 0)
    {
				      ...
				      transport_send_params_t *sp)
{
  ...
  rv = http_ts_peek_data (hc, rx_buf);
  ...
  max_enq = http_as_max_enqueue (req);
  max_enq -= sizeof (msg);
  ...
  rv = http_as_write_segments (req, segs, 2);
  ASSERT (rv == (sizeof (msg) + len));

  http_ts_drop_peek_data (hc, len);
...
error:
  http_ts_drop_peek_data (hc, vec_len (rx_buf));
				...
				transport_send_params_t *sp)
{
  ...
  sent = http_ts_write_data (hc, response, vec_len (response));
				 ...
				 transport_send_params_t *sp)
{
  ...
  sent = http_ts_write_data (hc, request, vec_len (request));
					...
					transport_send_params_t *sp)
{
  ...
  rv = http_as_write_segments (req, seg, 1);
  if (rv < 0)
    {
  ...
  req->to_recv -= rv;
  HTTP_DBG (1, "drained %d from ts; remains %lu", rv, req->to_recv);
			   ...
			   transport_send_params_t *sp)
{
  ...
  n_written = http_as_write_segments (req, segs, n_segs);
  ASSERT (n_written > 0);
			   ...
			   transport_send_params_t *sp)
{
  ...
  max_deq = http_as_max_dequeue (req);
  if (PREDICT_FALSE (max_deq == 0))
    {
      HTTP_DBG (1, "max_deq == 0");
      goto check_fifo;
  ...
  while (to_deq > 0)
      ...
      if (PREDICT_FALSE (req->to_skip))
	{
	  if (req->to_skip >= to_deq)
	    ...
	    {
	      svm_fifo_dequeue_drop (ts->rx_fifo, req->to_skip);
	      req->to_skip = 0;
	    }
	}
      n_read =
      ...
      if (PREDICT_FALSE (to_deq < capsule_size))
	{
      ...
      if (http_as_max_enqueue (req) < dgram_size)
	{
	  HTTP_DBG (1, "app's rx fifo full");
	  http_as_want_deq_ntf (req);
	  goto done;
			     ...
			     transport_send_params_t *sp, u8 is_tx)
{
  ...
  do
	...
	res = rx_state_funcs[req->state](hc, req, sp);
