...
#include <plugins/arp/arp_packet.h>
...
static void
arp_nbr_probe (const ip_adjacency_t * adj)
{
...
static void
arp_send_garp (vlib_main_t * vm, u32 sw_if_index, const ip46_address_t * addr)
{
  ...
  if (NULL == addr)
    {
      ip4_main_t *i4m = &ip4_main;
      ip4_addr = ip4_interface_first_address (i4m, sw_if_index, 0);
    }
  else
    ip4_addr = &addr->ip4;
...
const static ip_neighbor_vft_t arp_vft = {
  .inv_probe = arp_nbr_probe,
  .inv_send = arp_send_garp,
