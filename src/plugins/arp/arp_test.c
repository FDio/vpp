...
static int
api_arp_plugin_get_version (vat_main_t * vam)
{
  return -1;
}
...
static void
  vl_api_arp_plugin_get_version_reply_t_handler
  (vl_api_arp_plugin_get_version_reply_t * mp)
{
  vat_main_t *vam = arp_test_main.vat_main;
  clib_warning ("ARP plugin version: %d.%d", ntohl (mp->major),
		ntohl (mp->minor));
  vam->result_ready = 1;
}
