...
static lacp_fsm_state_t lacp_rx_state_initialize[] = {
  ...
  { LACP_ACTION_INITIALIZE,
    LACP_RX_STATE_PORT_DISABLED }, // event 1 PORT_DISABLED
  { LACP_ACTION_INITIALIZE,
    LACP_RX_STATE_PORT_DISABLED },	       // event 2 PORT_MOVED
...
static lacp_fsm_state_t lacp_rx_state_port_disabled[] = {
  ...
  { LACP_ACTION_LACP_DISABLED,
    LACP_RX_STATE_LACP_DISABLED },		  // event 4 LACP_DISABLED
...
static lacp_fsm_state_t lacp_rx_state_current[] = {
  ...
  { LACP_ACTION_LACP_DISABLED,
    LACP_RX_STATE_LACP_DISABLED },		  // event 3 LACP_DISABLED
...
static lacp_fsm_machine_t lacp_rx_fsm_table[] = {
  { lacp_rx_state_initialize }, { lacp_rx_state_port_disabled },
  { lacp_rx_state_expired },	{ lacp_rx_state_lacp_disabled },
  { lacp_rx_state_defaulted },	{ lacp_rx_state_current },
};
...
lacp_set_port_unselected (vlib_main_t *vm, member_if_t *mif)
{
  ...
  lacp_machine_dispatch (&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_UNSELECTED,
			 &mif->mux_state);
}
...
lacp_set_port_moved (vlib_main_t *vm, member_if_t *mif, u8 val)
{
  ...
  if (mif->port_moved)
    lacp_machine_dispatch (&lacp_rx_machine, vm, mif, LACP_RX_EVENT_PORT_MOVED,
			   &mif->rx_state);
...
lacp_port_is_moved (vlib_main_t *vm, member_if_t *mif)
{
  ...
  pool_foreach (mif2, bm->neighbors)
	...
	if ((mif != mif2) && (mif2->rx_state == LACP_RX_STATE_PORT_DISABLED) &&
	    !memcmp (mif2->partner.system, lacpdu->partner.port_info.system,
		     6) &&
	    (mif2->partner.port_number ==
	     lacpdu->partner.port_info.port_number))
	  return 1;
      }
    }
		    ...
		    lacp_fsm_state_t *transition)
{
  ...
  ed->event =
    elog_string (&vlib_global_main.elog_main, "%U-RX: %U, %U->%U%c",
		 format_vnet_sw_if_index_name, vnet_get_main (),
		 mif->sw_if_index, format_rx_event, event, format_rx_sm_state,
		 state, format_rx_sm_state, transition->next_state, 0);
}
...
lacp_init_rx_machine (vlib_main_t *vm, member_if_t *mif)
{
  lacp_machine_dispatch (&lacp_rx_machine, vm, mif, LACP_RX_EVENT_BEGIN,
			 &mif->rx_state);
  lacp_machine_dispatch (&lacp_rx_machine, vm, mif, LACP_RX_EVENT_LACP_ENABLED,
			 &mif->rx_state);
}
