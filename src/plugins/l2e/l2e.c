...
VLIB_CLI_COMMAND (l2_emulation_cli_node, static) = {
  ...
  .short_help =
  "set interface l2 input l2-emulation <interface-name> [disable|enable]\n",
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_TRUE (next0 == ~0))
	    ...
	    vnet_buffer (b0)->l2.feature_bitmap &= ~L2INPUT_FEAT_L2_EMULATION;
	  ...
	  if (PREDICT_TRUE (next1 == ~0))
	    ...
	    vnet_buffer (b1)->l2.feature_bitmap &= ~L2INPUT_FEAT_L2_EMULATION;
