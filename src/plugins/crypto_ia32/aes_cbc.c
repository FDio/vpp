...
#pragma GCC target("aes,sse4.2,rdrnd")
...
aes_cbc_enc (__m128i * k, u8 * src, u8 * dst, u8 * iv, int count,
...
aesni_rand_iv (u8 * p)
{
  _rdrand64_step ((void *) p);
  _rdrand64_step ((void *) (p + 8));
}
...
clib_error_t *
crypto_ia32_aesni_init (vlib_main_t * vm)
{
  extern u32 crypto_ia32_engine_index;
  clib_error_t *error;
  ...
  if (clib_cpu_supports_rdrand () == 0)
    return 0;

#define _(x) \
  vnet_crypto_register_ops_handler (vm, crypto_ia32_engine_index, \
...
VLIB_INIT_FUNCTION (crypto_ia32_aesni_init);
