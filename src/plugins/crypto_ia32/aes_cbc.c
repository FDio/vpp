  ...
  for (i = 0; i < count; i += 16)
    {
      ...
      r4 ^= t4 ^ keys.k4[0];
      for (j = 1; j < rounds; j++)
	r4 = _mm512_aesenc_epi128 (r4, keys.k4[j]);
      r4 = _mm512_aesenclast_epi128 (r4, keys.k4[j]);
...
#else
      r[0] ^= aes_block_load (src[0] + i) ^ keys.k1[0][0];
      r[1] ^= aes_block_load (src[1] + i) ^ keys.k1[0][1];
      r[2] ^= aes_block_load (src[2] + i) ^ keys.k1[0][2];
      r[3] ^= aes_block_load (src[3] + i) ^ keys.k1[0][3];

      for (j = 1; j < rounds; j++)
	{
	  r[0] = _mm_aesenc_si128 (r[0], keys.k1[j][0]);
	  r[1] = _mm_aesenc_si128 (r[1], keys.k1[j][1]);
	  r[2] = _mm_aesenc_si128 (r[2], keys.k1[j][2]);
	  r[3] = _mm_aesenc_si128 (r[3], keys.k1[j][3]);
	}

      r[0] = _mm_aesenclast_si128 (r[0], keys.k1[j][3]);
      r[1] = _mm_aesenclast_si128 (r[1], keys.k1[j][2]);
      r[2] = _mm_aesenclast_si128 (r[2], keys.k1[j][1]);
      r[3] = _mm_aesenclast_si128 (r[3], keys.k1[j][0]);
