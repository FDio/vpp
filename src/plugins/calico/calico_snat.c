...
static void
calico_compute_prefix_lengths_in_search_order (calico_snat_pfx_table_t *
...
int
calico_add_snat_prefix (ip_prefix_t * pfx)
{
  ...
  table->meta[af].non_empty_dst_address_length_bitmap =
    clib_bitmap_set (table->meta[af].non_empty_dst_address_length_bitmap,
		     128 - pfx->len, 1);
  calico_compute_prefix_lengths_in_search_order (table, af);
  return 0;
}

int
calico_del_snat_prefix (ip_prefix_t * pfx)
{
  calico_snat_pfx_table_t *table = &calico_main.snat_pfx_table;
...
u8 *
format_calico_snat_prefix (u8 * s, va_list * args)
{
...
static clib_error_t *
calico_snat_init (vlib_main_t * vm)
{
  calico_snat_pfx_table_t *table = &calico_main.snat_pfx_table;
  ...
  clib_bihash_set_kvp_format_fn_24_8 (&table->ip_hash,
				      format_calico_snat_prefix);

  return (NULL);
}

VLIB_INIT_FUNCTION (calico_snat_init);
