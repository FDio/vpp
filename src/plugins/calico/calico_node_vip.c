...
vlib_node_registration_t calico_vip_ip4_node;
vlib_node_registration_t calico_vip_ip6_node;
...
{
  vlib_combined_counter_main_t *cntm = &calico_translation_counters;
  ...
  if (!rv)
      ...
      if (!session->value.cs_port[VLIB_RX])
	{
	  ...
	  if ((ct->flags & CALICO_TRANSLATION_FLAG_ALLOCATE_PORT)
	       && (rsession_flags & CALICO_SESSION_FLAG_HAS_SNAT)) {
	    sport = 0; /* force allocation */
	    session->value.flags |= CALICO_SESSION_FLAG_ALLOC_PORT;
	    rv = calico_allocate_port (cm, &sport);
	    if (rv)
	      {
		vlib_node_increment_counter (vm, calico_vip_ip4_node.index,
				            CALICO_ERROR_EXHAUSTED_PORTS, 1);
		next0 = CALICO_TRANSLATION_NEXT_DROP;
		goto trace;
	      }
	    }
