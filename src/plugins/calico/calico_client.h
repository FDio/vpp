...
typedef struct calico_client_t_
{
  ...
  /**
   * Parent calico_client index if cloned via interpose
...
extern void calico_client_free_by_ip (ip46_address_t * addr, u8 af);

extern calico_client_t *calico_client_pool;
extern dpo_type_t calico_client_dpo;
...
typedef struct calico_learn_arg_t_
{
  ip_address_t addr;
  u32 ts_index;
} calico_learn_arg_t;
 ...
 */
extern void calico_client_remove_translation (index_t cci,
					      u16 port, ip_protocol_t proto);

/**
 * Add a translation that references this VIP
 */
extern void calico_client_add_translation (index_t cci,
					   u16 port,
 ...
 * new client
 */
extern void calico_client_learn (const calico_learn_arg_t * l);

extern index_t calico_client_add (const ip_address_t * ip, u32 ts_index,
				  u8 flags);
...
/**
 * Calico Client (dpo) flags
 */
typedef enum
{
  /* IP already present in the FIB, need to interpose dpo */
  CALICO_FLAG_EXCLUSIVE = (1 << 1),
  /* Prune this entry */
  CALICO_FLAG_EXPIRES = (1 << 2),
} calico_entry_flag_t;
...
calico_client_ip4_find_index (const ip4_address_t * ip)
{
  uword *p;
 ...
 */
static_always_inline u32
calico_client_cnt_session (calico_client_t * cc)
{
  calico_client_t *ccp = calico_client_get (cc->parent_cci);
 ...
 */
static_always_inline u32
calico_client_uncnt_session (calico_client_t * cc)
{
  calico_client_t *ccp = calico_client_get (cc->parent_cci);
