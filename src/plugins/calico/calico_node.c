...
calico_translation_tx_ip4 (const calico_session_t * session,
			   ip4_header_t * ip4, udp_header_t * udp)
{
  ...
  if (PREDICT_FALSE (ip4->protocol == IP_PROTOCOL_UDP && udp->checksum))
    {
  ...
  else if (PREDICT_FALSE (ip4->protocol == IP_PROTOCOL_TCP && tcp->checksum))
    {
      u16 old_port, new_port;
...
static_always_inline void
calico_translation_rx_ip4 (const calico_session_t * session,
			   ip4_header_t * ip4, udp_header_t * udp)
{
  tcp_header_t *tcp = (tcp_header_t *) udp;
  ip4_address_t old_addr, new_addr;
...
static_always_inline void
calico_translation_tx_ip6 (const calico_session_t * session,
			   ip6_header_t * ip6, udp_header_t * udp)
{
  tcp_header_t *tcp = (tcp_header_t *) udp;
  ip6_address_t old_addr, new_addr;
  ip_csum_t sum;

  ip6_address_copy (&old_addr, &ip6->dst_address);
  ip6_address_copy (&new_addr, &session->value.cs_ip.ip6);
  ip6_address_copy (&ip6->dst_address, &new_addr);

  if (PREDICT_FALSE (ip6->protocol == IP_PROTOCOL_UDP && udp->checksum))
    {
      u16 old_port, new_port;
      ...
      udp->dst_port = new_port;
      sum = udp->checksum;
      sum = ip_csum_add_even (sum, new_addr.as_u64[0]);
      sum = ip_csum_add_even (sum, new_addr.as_u64[1]);
      sum = ip_csum_sub_even (sum, old_addr.as_u64[0]);
      sum = ip_csum_sub_even (sum, old_addr.as_u64[1]);
      sum = ip_csum_update (sum, old_port, new_port,
			    ip4_header_t /* cheat */ ,
			    length /* changed member */ );
      udp->checksum = ip_csum_fold (sum);
    }
  else if (PREDICT_FALSE (ip6->protocol == IP_PROTOCOL_TCP && tcp->checksum))
    {
      u16 old_port, new_port;
...
static_always_inline void
calico_translation_rx_ip6 (const calico_session_t * session,
			   ip6_header_t * ip6, udp_header_t * udp)
{
  tcp_header_t *tcp = (tcp_header_t *) udp;
  ip6_address_t old_addr, new_addr;
  ...
  ip6_address_copy (&old_addr, &ip6->src_address);
  ip6_address_copy (&new_addr, &session->value.cs_ip.ip6);
  ip6_address_copy (&ip6->src_address, &new_addr);
  ...
  while (n_left)
    {
      ...
      calico_timestamp_update (cc->cc_ts_index, now);
      ...
      else
	{
      ...
      if (!rv)
	{
	  /* session hit => translation */
	  session = (calico_session_t *) & bvalue;
	  calico_timestamp_update (session->value.cs_ts_index, now);

	  if (AF_IP4 == af)
	    calico_translation_rx_ip4 (session, ip4, udp0);
	  else
	    calico_translation_rx_ip6 (session, ip6, udp0);
	}
      else
	session = NULL;
		   ...
		   vlib_frame_t * frame, ip_address_family_t af)
{
  ...
  while (n_left)
	...
	{
	  next[0] = CALICO_TRANSLATION_NEXT_DROP;
	  goto trace;
	}

      ct =
	calico_vip_find_translation (vnet_buffer (b[0])->
      ...
      else
	{
	  ...
	  if (NULL == cc)
	    {
	      u64 r0 = 17;
	      if (AF_IP4 == af)
		r0 = (u64) ip4->src_address.as_u32;
	      else
		{
		  r0 = r0 * 31 + ip6->src_address.as_u64[0];
		  r0 = r0 * 31 + ip6->src_address.as_u64[1];
		}
	  ...
	  rsession->value.cs_ts_index = calico_timestamp_new (now);
      ...
      if (AF_IP4 == af)
	calico_translation_tx_ip4 (session, ip4, udp0);
      else
	calico_translation_tx_ip6 (session, ip6, udp0);

      vlib_increment_combined_counter
	(cm, thread_index, cti, 1, vlib_buffer_length_in_chain (vm, b[0]));
