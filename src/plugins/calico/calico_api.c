...
static void
calico_endpoint_tuple_decode (const vl_api_calico_endpoint_tuple_t * in,
			      calico_endpoint_tuple_t * out)
{
  calico_endpoint_decode (&in->src_ep, &out->src_ep);
  calico_endpoint_decode (&in->dst_ep, &out->dst_ep);
}
					    ...
					    * mp)
{
  ...
  calico_endpoint_tuple_t *paths = NULL, *path;
  ...
  for (pi = 0; pi < mp->translation.n_paths; pi++)
    {
      path = &paths[pi];
      calico_endpoint_tuple_decode (&mp->translation.paths[pi], path);
    }
  ...
  id = calico_translation_update (&vip, ip_proto, paths, flags);
...
calico_translation_send_details (u32 cti, void *args)
{
  ...
  calico_ep_trk_t *trk;
  vl_api_calico_endpoint_tuple_t *path;
  size_t msg_size;
  ...
  {
    calico_endpoint_encode (&trk->ct_ep[VLIB_TX], &path->dst_ep);
    calico_endpoint_encode (&trk->ct_ep[VLIB_RX], &path->src_ep);
    path++;
  }
