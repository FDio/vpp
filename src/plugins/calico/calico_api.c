...
static void
  vl_api_calico_register_pod_interface_t_handler
  (vl_api_calico_register_pod_interface_t * mp)
{
  vl_api_calico_register_pod_interface_reply_t *rmp;
  int rv = 0;

  VALIDATE_SW_IF_INDEX (mp);

  rv = calico_register_pod_interface (ntohl (mp->sw_if_index), mp->is_add);

  BAD_SW_IF_INDEX_LABEL;

  REPLY_MACRO (VL_API_CALICO_REGISTER_POD_INTERFACE_REPLY);
}


static void
  vl_api_calico_add_del_pod_cidr_t_handler
  (vl_api_calico_add_del_pod_cidr_t * mp)
{
  vl_api_calico_add_del_pod_cidr_reply_t *rmp;
  ip_prefix_t pfx;
  int rv;

  ip_prefix_decode2 (&mp->prefix, &pfx);
  rv = calico_add_del_pod_cidr (&pfx, mp->is_add);


  REPLY_MACRO (VL_API_CALICO_ADD_DEL_POD_CIDR_REPLY);
}
