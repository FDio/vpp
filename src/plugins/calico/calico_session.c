      ...
      for (j = 0; j < (1 << b->log2_pages); j++)
	{
	  for (k = 0; k < BIHASH_KVP_PER_PAGE; k++)
		...
		continue;
	      ...
	      if ((start_time - CALICO_SESSION_MAX_AGE) >
		  session->value.cs_timestamp)
		{
		  /* age it */
...
{
  calico_main_t *cm = &calico_main;
  cm->session_hash_memory = CALICO_SESSION_DEFAUT_MEMORY;
  cm->session_hash_buckets = CALICO_SESSION_DEFAUT_BUCKETS;
