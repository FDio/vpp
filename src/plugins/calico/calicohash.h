...
#ifndef CALICO_PLUGIN_CALICO_LBHASH_H_
#define CALICO_PLUGIN_CALICO_LBHASH_H_
...
#define CALICO_HASH_DO_NOT_USE_SSE_BUCKETS 0
...
} calico_hash_bucket_t;

typedef struct {
  u32 buckets_mask;
  u32 timeout;
  calico_hash_bucket_t buckets[];
} calico_hash_t;

#define calico_hash_nbuckets(h) (((h)->buckets_mask) + 1)
#define calico_hash_size(h) ((h)->buckets_mask + LBHASH_ENTRY_PER_BUCKET)

#define calico_hash_foreach_bucket(h, bucket) \
  for (bucket = (h)->buckets; \
	bucket < (h)->buckets + calico_hash_nbuckets(h); \
	bucket++)

#define calico_hash_foreach_entry(h, bucket, i) \
    calico_hash_foreach_bucket(h, bucket) \
      for (i = 0; i < LBHASH_ENTRY_PER_BUCKET; i++)

#define calico_hash_foreach_valid_entry(h, bucket, i, now) \
    calico_hash_foreach_entry(h, bucket, i) \
       if (!clib_u32_loop_gt((now), bucket->timeout[i]))

static_always_inline
calico_hash_t *calico_hash_alloc(u32 buckets, u32 timeout)
{
  if (!is_pow2(buckets))
    return NULL;

  // Allocate 1 more bucket for prefetch
  u32 size = ((uword)&((calico_hash_t *)(0))->buckets[0]) +
      sizeof(calico_hash_bucket_t) * (buckets + 1);
  u8 *mem = 0;
  calico_hash_t *h;
...
static_always_inline
void calico_hash_free(calico_hash_t *h)
{
  u8 *mem = (u8 *)h;
  vec_free(mem);
}
...
static_always_inline
void calico_hash_prefetch_bucket(calico_hash_t *ht, u32 hash)
{
  calico_hash_bucket_t *bucket = &ht->buckets[hash & ht->buckets_mask];
  CLIB_PREFETCH(bucket, sizeof(*bucket), READ);
}

static_always_inline
void calico_hash_get(calico_hash_t *ht, u32 hash, u32 vip, u32 time_now,
		 u32 *available_index, u32 *found_value)
{
  calico_hash_bucket_t *bucket = &ht->buckets[hash & ht->buckets_mask];
  *found_value = 0;
  *available_index = ~0;
#if __SSE4_2__ && CALICO_HASH_DO_NOT_USE_SSE_BUCKETS == 0
...
static_always_inline
u32 calico_hash_available_value(calico_hash_t *h, u32 hash, u32 available_index)
{
  return h->buckets[hash & h->buckets_mask].value[available_index];
}

static_always_inline
void calico_hash_put(calico_hash_t *h, u32 hash, u32 value, u32 vip,
		 u32 available_index, u32 time_now)
{
  calico_hash_bucket_t *bucket = &h->buckets[hash & h->buckets_mask];
...
static_always_inline
u32 calico_hash_elts(calico_hash_t *h, u32 time_now)
{
  u32 tot = 0;
  calico_hash_bucket_t *bucket;
  u32 i;
  calico_hash_foreach_valid_entry(h, bucket, i, time_now) {
    tot++;
  }
  return tot;
}

#endif /* CALICO_PLUGIN_CALICO_LBHASH_H_ */
