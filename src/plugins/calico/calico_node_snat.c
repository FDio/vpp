...
/* Calico sub for source NAT as a feature arc on ip[46]-unicast
		    ...
		    calico_session_t * session)
{
  ...
  if (!rv)
	...
	ip46_address_set_ip6 (&ip46_dst_address, &ip6->dst_address);
      rv = calico_search_snat_prefix (&ip46_dst_address, ctx->af);
      if (!rv)
	{
      ...
      if (rv)
	{
      ...
      session->value.flags =
	CALICO_SESSION_FLAG_NO_CLIENT | CALICO_SESSION_FLAG_ALLOC_PORT;

      calico_session_create (session, ctx, CALICO_SESSION_FLAG_HAS_SNAT);
    }
				     ...
				     vlib_frame_t * frame)
{
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
    return calico_node_inline (vm, node, frame, calico_snat_inline, AF_IP4,
			       1 /* do_trace */ );
  return calico_node_inline (vm, node, frame, calico_snat_inline, AF_IP4,
			     0 /* do_trace */ );
}
...
{
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
    return calico_node_inline (vm, node, frame, calico_snat_inline, AF_IP6,
			       1 /* do_trace */ );
  return calico_node_inline (vm, node, frame, calico_snat_inline, AF_IP6,
			     0 /* do_trace */ );
}
...
{
.arc_name = "ip4-unicast",.node_name = "ip4-calico-snat",};

VNET_FEATURE_INIT (calico_snat_ip6_node, static) =
{
.arc_name = "ip6-unicast",.node_name = "ip6-calico-snat",};
