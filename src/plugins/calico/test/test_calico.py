        ...
        for vip in vips:
            for vport in vports:
                t1 = VppCalicoTranslate(
                    ...
                    [VppCalicoEndpoint(self.pg1.remote_hosts[nbr].ip4, 4000 + nbr),
                     VppCalicoEndpoint(self.pg1.remote_hosts[nbr].ip4, 4000 + nbr)])
                t1.add_vpp_config()
                ...
                for src in self.pg0.remote_hosts:
                    for sport in sports:
                        ...
                        for rx in rxs:
                            ...
                            self.assertEqual(rx[IP].dst,
                                             self.pg1.remote_hosts[nbr].ip4)
                            self.assertEqual(rx[UDP].dport, 4000 + nbr)
                            self.assertEqual(rx[IP].src, src.ip4)
                            self.assertEqual(rx[UDP].sport, sport)
                        ...
                        p1 = (Ether(dst=self.pg1.local_mac,
                              ...
                              IP(src=self.pg1.remote_hosts[nbr].ip4,
                                 dst=src.ip4) /
                              UDP(sport=4000 + nbr, dport=sport) /
                              Raw())
                        ...
                        for rx in rxs:
                            ...
                            self.assertEqual(rx[UDP].dport, sport)
                            self.assertEqual(rx[IP].src, vip)
                            self.assertEqual(rx[UDP].sport, vport)

                #
                # packets to the VIP that do not match a translation are dropped
                #
                p1 = (Ether(dst=self.pg0.local_mac,
                            src=src.mac) /
                      IP(src=src.ip4, dst=vip) /
                      UDP(sport=sport, dport=6666) /
                      Raw())
                ...
                #
                # packets from the VIP that do not match a session are forwarded
                #
                p1 = (Ether(dst=self.pg1.local_mac,
                            src=self.pg1.remote_mac) /
                      IP(src=self.pg1.remote_hosts[nbr].ip4,
                         dst=src.ip4) /
                      UDP(sport=6666, dport=sport) /
                      Raw())
                ...
                print(self.vapi.cli("sh ip fib %s" % self.pg0.remote_ip4))
