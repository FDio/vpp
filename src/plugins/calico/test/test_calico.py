...
class TestCalicoTranslation(VppTestCase):
    """ Calico Translation """
    ...
    @classmethod
    def setUpClass(cls):
        super(TestCalicoTranslation, cls).setUpClass()

    @classmethod
    def tearDownClass(cls):
        super(TestCalicoTranslation, cls).tearDownClass()

    def setUp(self):
        super(TestCalicoTranslation, self).setUp()
    ...
    def tearDown(self):
        for i in self.pg_interfaces:
            ...
            i.unconfig_ip6()
            i.admin_down()
        super(TestCalicoTranslation, self).tearDown()

    def test_calico4(self):
        """ Calico Translation 4 """
        ...
        # will time out
        #
        self.vapi.cli("test calico scanner off")
        ...
        for vip in vips:
            for vport in vports:
                for l4p, vl4 in zip(l4_protos, vl4_protos):
                    dep = VppCalicoEndpoint(self.pg1.remote_hosts[nbr].ip4,
                                            4000 + nbr)
                    sep = VppCalicoEndpoint("0.0.0.0", 0)
                    t1 = VppCalicoTranslation(
                        self,
                        VppCalicoEndpoint(vip, vport),
                        vl4,
                        [VppCalicoEndpointTuple(sep, dep),
                         VppCalicoEndpointTuple(sep, dep)])
                    t1.add_vpp_config()

                    self.logger.info(self.vapi.cli("sh ip fib %s" % vip))
                    self.logger.info(self.vapi.cli("sh calico client"))
                    self.logger.info(self.vapi.cli("sh calico translation"))
                    ...
                    #
                    dep = VppCalicoEndpoint(self.pg2.remote_ip4, 5000)
                    sep = VppCalicoEndpoint("0.0.0.0", 0)
                    t1.modify_vpp_config([VppCalicoEndpointTuple(sep, dep)])
            ...
            self.logger.info(self.vapi.cli("sh ip fib %s" %
                                           self.pg0.remote_ip4))
            self.logger.info(self.vapi.cli("sh calico session"))

        #
        # turn the scanner back on and wait untill the sessions
        # all disapper
        #
        self.vapi.cli("test calico scanner on")
        ...
        self.assertTrue(self.vapi.calico_session_dump())
        self.vapi.calico_session_purge()
        self.assertFalse(self.vapi.calico_session_dump())
