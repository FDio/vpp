  ...
  if (NULL == cvip)
    {
      ...
      ip_addr_to_fib_prefix (&vip->ce_ip, &pfx);
      dpo_set (&cvip->cvip_dpo, calico_tx_dpo,
	       fib_proto_to_dpo (pfx.fp_proto), cvipi);

      cvip->cvip_fei = fib_table_entry_special_dpo_add (CALICO_FIB_TABLE,
							&pfx,
							calico_fib_source,
...
format_calico_vip (u8 * s, va_list * args)
{
  ...
  s = format (s, "[%d] calcio-vip:[%U]", cvipi,
...
format_calico_ep_trk (u8 * s, va_list * args)
{
  ...
  s = format (s, "\n    fib-entry:%d", ck->ct_fei);
  s = format (s, "\n    %U", format_dpo_id, &ck->ct_dpo, 6);
...
format_calico_session (u8 * s, va_list * args)
{
  ...
  s = format (s, "session:[%U;%d -> %U;%d, %U] => %U;%d lb:%d age:%f",
...
const static char *const calico_translation_dpo_ip4_nodes[] = {
  "ip4-calico-translate",
  NULL,
};

const static char *const calico_translation_dpo_ip6_nodes[] = {
  "ip6-calico-translate",
  NULL,
};

const static char *const *const calico_translation_dpo_nodes[DPO_PROTO_NUM] = {
  [DPO_PROTO_IP4] = calico_translation_dpo_ip4_nodes,
  [DPO_PROTO_IP6] = calico_translation_dpo_ip6_nodes,
};

static void
calico_translation_dpo_lock (dpo_id_t * dpo)
{
}

static void
calico_translation_dpo_unlock (dpo_id_t * dpo)
{
}

u8 *
format_calico_translation_fib_dpo (u8 * s, va_list * ap)
{
...
const static dpo_vft_t calico_translation_dpo_vft = {
  .dv_lock = calico_translation_dpo_lock,
  .dv_unlock = calico_translation_dpo_unlock,
  .dv_format = format_calico_translation_fib_dpo,
};
...
calico_translation_init (vlib_main_t * vm)
{
  ...
  calico_tx_dpo = dpo_register_new_type (&calico_translation_dpo_vft,
					 calico_translation_dpo_nodes);
