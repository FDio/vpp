...
no_snat:
  return calico_main.default_source_policy (session, b, ip4, ip6, udp0,
					    rsession_flags, ct, ctx, cm, vm);
}
...
calico_init (vlib_main_t * vm)
{
  ...
  calico_main.default_source_policy =
    vlib_get_plugin_symbol ("cnat_plugin.so",
			    "cnat_vip_default_source_policy");

  if (calico_main.cnat_search_snat_prefix == NULL
      || calico_main.cnat_allocate_port == NULL
      || calico_main.default_source_policy == NULL)
