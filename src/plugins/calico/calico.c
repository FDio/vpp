  ...
  if (NULL == ct)
    {
      ...
      ct->ct_proto = proto;
      ct->ct_vipi = cvipi;
      fib_node_init (&ct->ct_node, calico_translation_fib_node_type);
  ...
  if (NULL == cc)
    {
      ...
      ip_addr_to_fib_prefix (&cc->cc_ip, &pfx);
      ...
      cc->cc_fei = fib_table_entry_special_dpo_add (CALICO_FIB_TABLE,
						    &pfx,
						    calico_fib_source,
						    (FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT
						     |
  ...
  if (INDEX_INVALID == cci)
    {
      ...
      /* *INDENT-OFF* */
      hash_foreach(ip, cci, calico_tx_db.ctd_cip,
      ({
