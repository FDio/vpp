...
#define ENA_AENQ_POLL_INTERVAL 3.0
...
ena_aenq_poll (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  while ((ae = ena_get_next_aenq_entry (dev)))
    {
      ena_log_debug (dev, "aenq: group %u syndrome %u phase %u timestamp %lu",
	...
	case ENA_AENQ_GROUP_LINK_CHANGE:
	  ...
	  // ena_device_set_link_state (vm, ed, ae->link_change.link_status !=
	  // 0);
	  break;
	...
	default:
	  ena_log_debug (dev, "aenq %U", format_hexdump, ae, 32);
	};
    }
  return VNET_DEV_OK;
}
