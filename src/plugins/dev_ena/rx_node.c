			 ...
			 int with_flows)
{
  ...
  vlib_buffer_copy_template (&bt, &ep->buffer_template);

  next_index = rt->next_index;

  if (PREDICT_FALSE (vnet_device_input_have_features (rt->sw_if_index)))
    vnet_feature_start_device_input_x1 (rt->sw_if_index, &next_index, &bt);
  ...
  vlib_increment_combined_counter (
    ...
    vm->thread_index, rt->hw_if_index, n_rx_packets, n_rx_bytes);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  foreach_vnet_dev_rx_queue_runtime (rxq, node)
    n_rx += ena_device_input_inline (vm, node, frame, rxq, 0);
  return n_rx;
}
