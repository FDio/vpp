...
ena_rx_queue_alloc (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  ...
  ena_log_debug (dev, "rx_queue_alloc[%u]:", rxq->queue_id);
...
ena_tx_queue_alloc (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  ...
  ena_log_debug (dev, "tx_queue_alloc[%u]:", txq->queue_id);
...
ena_rx_queue_stop (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  ...
  if (eq->sq_created)
	...
	.sq_direction = ENA_ADMIN_SQ_DIRECTION_TX,
      ...
      if ((rv = ena_aq_destroy_sq (vm, dev, &cmd)) != VNET_DEV_OK)
  ...
  if (eq->cq_created)
	...
	.cq_idx = eq->cq_idx,
      ...
      if ((rv = ena_aq_destroy_cq (vm, dev, &cmd)) != VNET_DEV_OK)
