/*
 * mdata.c - skeleton vpp engine plug-in
 *
...
{
  .path = "mdata enable-disable",
  .short_help =
  "mdata enable-disable [disable]",
...
#define _(n) if (b->n) s = format (s, "%s ", #n);
...
#define _(n) if (o->n) s = format (s, "%s ", #n);
...
#define _(n) if (o2->n) s = format (s, "%s ", #n);
