...
#define HLL_ADDFLOW_MAXSIZE 101
...
int hll_enable_disable (hll_main_t * hmp, u32 sw_if_index, u32 multihll_size, u32 size, u8 bits, u8 mode,
		int enable_disable)
{
	...
	if(enable_disable){
		...
		if (index != ~0) {
			clib_warning("BUG: trying to apply twice hll on sw_if_index %d", sw_if_index);
			return VNET_API_ERROR_INVALID_SW_IF_INDEX;
		}

		vec_validate(hmp->input_hll_vec_by_sw_if_index, sw_if_index);
		hll_list_t* hll_init_srcif = clib_mem_alloc(sizeof(hll_list_t));
		hmp->input_hll_vec_by_sw_if_index[sw_if_index] =  (hll_list_t *) hll_init_srcif;
		...
		BV (clib_bihash_init) (&hmp->hll_list_hash[sw_if_index], name,
				hmp->hll_list_hash_buckets, hmp->hll_list_hash_memory);
		...
		hll_init_srcif->hllreg_vec = clib_mem_alloc(sizeof(u8*)*multihll_size);
		for(int i=0; i< multihll_size; i++){
			...
			hll_info_t* hc_nfo = (hll_info_t *) clib_mem_alloc(sizeof(hll_info_t));
		...
		u32 index = vec_search(hmp->associated_sw_if_index, sw_if_index);
		if (index == ~0) {
			clib_warning("BUG: trying to remove hll on a not associated sw_if_index %d", sw_if_index);
			return VNET_API_ERROR_INVALID_SW_IF_INDEX;
		}
...
count_hll_ht (const clib_bihash_kv_16_8_t * kvp, void *args){
	...
	u32 reverse_hll = hlls->reverse_hll_vec[raw_count_index];
	u32 raw_count_value = hlls->hll_raw_count_vec[raw_count_index];

	double estimate = hll_count(hlls, hll_index);
	vlib_cli_output (vm, "%.2f \t %U \t %d \t %d \t %d \t %d", estimate, format_ip46_address, &key, IP46_TYPE_IP4, hll_index, reverse_hll, raw_count_index, raw_count_value);
		...
		vlib_cli_command_t * cmd)
{
	...
	for( i=0; i < vec_len(hmp->associated_sw_if_index); i++) {
		...
		hll_list_t* hlls = (hll_list_t*) hmp->input_hll_vec_by_sw_if_index[hmp->associated_sw_if_index[i]];
		vlib_cli_output (vm, "sw_if_index: %d | bits: %d | size: %d | mode: %d",hmp->associated_sw_if_index[i], hlls->bits, hlls->size, hlls->mode);
		vlib_cli_output (vm, "pkt_count: %d | not_monitored: %d (%.2f)", hlls->pkt_count, hlls->deniedaccess, ( hlls->deniedaccess / (hlls->pkt_count + 1.0)));
		...
		vlib_cli_output (vm, "--------------------------------------");
		vlib_cli_output (vm, "< val > \t < key > \t Hll_index \t reverse_hll \t raw_count_index \t raw_count_value");
		vlib_cli_output (vm, "==========VVV=============");
		...
		vlib_cli_command_t * cmd)
{
	...
	for( i=0; i < vec_len(hmp->associated_sw_if_index); i++) {
		...
		hll_list_t* hlls = (hll_list_t*) hmp->input_hll_vec_by_sw_if_index[hmp->associated_sw_if_index[i]];
		vlib_cli_output (vm, "sw_if_index: %d | bits: %d | size: %d | mode: %d ",hmp->associated_sw_if_index[i], hlls->bits, hlls->size, hlls->mode);
		vlib_cli_output (vm, "pkt_count: %d | not_monitored: %d", hlls->pkt_count, hlls->deniedaccess);
		vlib_cli_output(vm, "\nhash table:\n%U\n", BV (format_bihash), hlls->hll_list_hash, 1);
		...
		BV (clib_bihash_foreach_key_value_pair) (hlls->hll_list_hash, walk_hll_ht, &ctx);
...
int hll_add_flow (hll_main_t * hmp, u32 sw_if_index, hll_key_t * flow)
{

	/* Check if the hll exists*/
	u32 index = vec_search(hmp->associated_sw_if_index, sw_if_index);
	if (index == ~0) {
		clib_warning("BUG: trying to add a flow to a not instantiated hll on sw_if_index %d", sw_if_index);
		return VNET_API_ERROR_INVALID_SW_IF_INDEX;
	}

	hll_list_t* hll = (hll_list_t*) hmp->input_hll_vec_by_sw_if_index[sw_if_index];

	/* according to the mode, load the right flow key */
	u8 mode = hll->mode;
        clib_bihash_kv_16_8_t kv0 = {};
	if (mode == 4 || mode==2) {
		kv0.key[0] = (u64) flow->dst_address;
		kv0.key[1] = (u64) 0;
	} else {
		kv0.key[0] = (u64) flow->src_address;
		kv0.key[1] = (u64) 0;
	}
	...
	if(hll->hll_assigned < hll->multihll_size){
		...
		clib_warning("BUG: no more space for a new hll, max size %d",  hll->multihll_size);
		return HLL_ADDFLOW_MAXSIZE;
	}
...
/* VPP-CLI to associate a flow from the cli to an hll */
	static clib_error_t *
hll_addflow_command_fn (vlib_main_t * vm,
		unformat_input_t * input,
		vlib_cli_command_t * cmd)
{
	hll_main_t * hmp = &hll_main;
	...
	while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	{
		...
		{
			flow_key.src_address = src_v4address.as_u32;
		}
		else if (unformat (input, "dst %U",
					unformat_ip4_address, &dst_v4address))
		{
			flow_key.dst_address = dst_v4address.as_u32;
		}
		else if (unformat (input, "sport %d", &port1))
		{
			flow_key.src_port = htons(port1); 
		}
		else if (unformat (input, "dport %d", &port1))
		{
			flow_key.dst_port = htons(port2); 
		}
		else if (unformat (input, "proto %d", &proto))
		{
			flow_key.protocol = proto; 
		}
	...
	rv = hll_add_flow (hmp, sw_if_index, &flow_key);
	...
	switch(rv)
	{
		...
		case HLL_ADDFLOW_MAXSIZE:
			return clib_error_return (0, "no more space for a new hll");
			break;

		default:
			return clib_error_return (0, "hll_enable_disable returned %d",
					rv);
	}
	return 0;
}
		...
		vlib_cli_command_t * cmd)
{
	...
	rv = hll_enable_disable (hmp, sw_if_index, multihll_size, size, bits, mode, enable_disable);
...
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (hll_addflow_command, static) =
{
	.path = "hll addflow",
	.short_help =
		"hll addflow [src <ip4_address>] [dst <ip4_address>] <interface-name> ",
	.function = hll_addflow_command_fn,
};
...
static clib_error_t * hll_init (vlib_main_t * vm)
{
	hll_main_t * hmp = &hll_main;
