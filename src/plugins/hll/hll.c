...
count_hll_ht (const clib_bihash_kv_16_8_t * kvp, void *args)
{
  ...
  vlib_cli_output (vm, "%.2f \t %U \t %d \t %d \t %d \t %d", estimate,
		   format_ip46_address, &key, IP46_TYPE_IP4, hll_index,
		   reverse_hll, raw_count_index, raw_count_value);
  ...
  for (i = 0; i < vec_len (hmp->associated_sw_if_index); i++)
	...
	input_hll_vec_by_sw_if_index[hmp->associated_sw_if_index[i]];
      vlib_cli_output (vm, "sw_if_index: %d | bits: %d | size: %d | mode: %d",
		       ...
		       hlls->mode);
      vlib_cli_output (vm, "pkt_count: %d | not_monitored: %d (%.2f)",
		       hlls->pkt_count, hlls->deniedaccess,
      ...
      vlib_cli_output (vm, "--------------------------------------");
      vlib_cli_output (vm,
		       "< val > \t < key > \t Hll_index \t reverse_hll \t raw_count_index \t raw_count_value");
  ...
  for (i = 0; i < vec_len (hmp->associated_sw_if_index); i++)
	...
	input_hll_vec_by_sw_if_index[hmp->associated_sw_if_index[i]];
      vlib_cli_output (vm,
		       ...
		       hmp->associated_sw_if_index[i], hlls->bits, hlls->size,
		       hlls->mode);
      vlib_cli_output (vm, "pkt_count: %d | not_monitored: %d",
		       hlls->pkt_count, hlls->deniedaccess);
      vlib_cli_output (vm, "\nhash table:\n%U\n", BV (format_bihash),
...
int
hll_add_flow (hll_main_t * hmp, u32 sw_if_index, hll_key_t * flow)
{
  ...
  if (index == ~0)
    {
      clib_warning
	("BUG: trying to add a flow to a not instantiated hll on sw_if_index %d",
	 sw_if_index);
      return VNET_API_ERROR_INVALID_SW_IF_INDEX;
    }

  hll_list_t *hll =
    (hll_list_t *) hmp->input_hll_vec_by_sw_if_index[sw_if_index];
  ...
  if (hll->hll_assigned < hll->multihll_size)
    ...
    {
      clib_warning ("BUG: no more space for a new hll, max size %d",
		    hll->multihll_size);
			...
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "src %U", unformat_ip4_address, &src_v4address))
	{
