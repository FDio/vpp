...
static u8 * format_hll_trace (u8 * s, va_list * args)
{
...
always_inline void
update_raw_count(hll_list_t * hll, hll_info_t * hll_info0, u32 diff_raw_count0){
	u32 raw_count_index0 = hll_info0->raw_count_index;
	u32 updated_raw_count_value0 = hll->hll_raw_count_vec[raw_count_index0] + diff_raw_count0;

	/* swap index to mantain sort */
	if(raw_count_index0 < hll->multihll_size - 1){
		u32 next_raw_count_index0 = raw_count_index0 + 1;
		if(hll->hll_raw_count_vec[next_raw_count_index0] <= updated_raw_count_value0){
			hll->n_swaps = hll->n_swaps + 1;
			u32 tmp_swap = hll->reverse_hll_vec[next_raw_count_index0];
			hll->reverse_hll_vec[next_raw_count_index0] = hll->reverse_hll_vec[raw_count_index0];
			hll->reverse_hll_vec[raw_count_index0] = tmp_swap;

			hll->hll_raw_count_vec[raw_count_index0] = hll->hll_raw_count_vec[next_raw_count_index0];
			hll->hll_raw_count_vec[next_raw_count_index0] = updated_raw_count_value0;

			((hll_info_t *) hll->hll_info_vec[tmp_swap])->raw_count_index = raw_count_index0;
...
always_inline int 
hll_not_found_x1 (hll_list_t * hll, clib_bihash_kv_16_8_t kv0, u64 hash0, u32 hll_index0) {
		...
		vlib_frame_t * frame)
{
	...
	while (n_left_from > 0)
	{
		...
		while (n_left_from > 0 && n_left_to_next > 0)
		{
			...
			hll_list_t* hll_srcif = (hll_list_t*) hllm->input_hll_vec_by_sw_if_index[sw_if_index0];
