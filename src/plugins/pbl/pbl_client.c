...
format_pbl_client (u8 *s, va_list *args)
{
  ...
  s = format (s, "\n%Uports:%U", format_white_space, indent + 2,
	      format_pbl_ports, pc->pc_port_map);
			  ...
			  dpo_id_t *clone)
{
  ...
  pc_clone->pc_port_map = pc->pc_port_map;
...
pbl_client_stack (pbl_client_t *pc)
{
  ...
  if (INDEX_INVALID != pc->clone_pci)
    {
      pc_clone = pool_elt_at_index (pbl_client_pool, pc->clone_pci);
      dpo_copy (&pc_clone->pc_dpo, &pc->pc_dpo);
    }
...
pbl_client_delete (u32 id)
{
  ...
  clib_bitmap_free (pc->pc_port_map);
...
u32
pbl_client_update (index_t pci, ip_address_t *addr, clib_bitmap_t *port_map,
		   u8 flags, fib_route_path_t *rpaths)
{
  pbl_client_t *pc;
...
pbl_client_last_lock_gone (fib_node_t *node)
{
			 ...
			 vlib_cli_command_t *cmd)
 {
   ...
   while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
     {
       if (unformat (line_input, "addr %U", unformat_ip_address, &addr))
	 ;
       else if (unformat (line_input, "add"))
	 is_add = 1;
       else if (unformat (line_input, "del"))
	 is_add = 0;
       else if (unformat (line_input, "id %d", &pci))
	 ;
       else if (unformat (line_input, "exclusive"))
	 flags = PBL_FLAG_EXCLUSIVE;
       else if (unformat (line_input, "via %U", unformat_fib_route_path,
			  &rpath, &payload_proto))
	 vec_add1 (rpaths, rpath);
       else if (unformat (line_input, "port %u-%u", &port_a, &port_b))
	 clib_bitmap_set_region (port_map, (u16) port_a, 1,
				 (u16) (port_b - port_a + 1));
       else if (unformat (line_input, "port %u", &port_a))
	 clib_bitmap_set (port_map, (u16) port_a, 1);
       else
	 {
   ...
   if (is_add)
     pbl_client_update (pci, &addr, port_map, flags, rpaths);
   else
     pbl_client_delete (pci);
