...
unsigned char function_name_usid[] = "SRv6-uAD-Flow";
unsigned char keyword_usid_str[] = "End.uAD.Flow";
...
static int
srv6_uad_flow_localsid_creation_fn (ip6_sr_localsid_t *localsid)
{
  srv6_ad_flow_localsid_t *ls_mem = localsid->plugin_mem;
...
uword
unformat_srv6_uad_flow_localsid (unformat_input_t *input, va_list *args)
{
  void **plugin_mem_p = va_arg (*args, void **);
  srv6_ad_flow_localsid_t *ls_mem;
...
#define PARAM_AD_IIF (1 << 2)
  ...
  if (!unformat (input, "end.uad.flow %u", &usid_size))
    return 0;
...
const static char *const srv6_uad_flow_ip6_nodes[] = {
  "srv6-uad-flow-localsid",
  NULL,
};

const static char *const *const srv6_uad_flow_nodes[DPO_PROTO_NUM] = {
  [DPO_PROTO_IP6] = srv6_uad_flow_ip6_nodes,
};
...
srv6_ad_flow_init (vlib_main_t *vm)
{
  ...
  sm->srv6_uad_flow_dpo_type =
    dpo_register_new_type (&srv6_ad_flow_vft, srv6_uad_flow_nodes);
  ...
  rv_usid = sr_localsid_register_function (
    ...
    128, &sm->srv6_uad_flow_dpo_type, format_srv6_ad_flow_localsid,
    unformat_srv6_uad_flow_localsid, srv6_uad_flow_localsid_creation_fn,
    srv6_ad_flow_localsid_removal_fn);
  if (rv_usid < 0)
    clib_error_return (
