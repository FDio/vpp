...
always_inline void
ad_flow_entry_update_lru (srv6_ad_flow_localsid_t * ls,
		      ...
		      int lru_delete)
{
  ...
  if (ls->inner_type == AD_TYPE_IP4)
    {
      kv.key[0] =
	((u64) e->key.s_addr.ip4.as_u32 << 32) | (u64) e->key.d_addr.
	ip4.as_u32;
			   ...
			   vlib_combined_counter_main_t ** cnt, f64 now)
{
  ...
  if (!clib_bihash_search_40_8 (h, &kv, &value))
    {
      e =
	pool_elt_at_index (ls_mem->cache,
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (ls0_mem == NULL))
	      ...
	      else
		{
		  /* found */
		  s0 =
		    pool_elt_at_index (ls0_mem->cache,
