...
format_srv6_ad_flow_rewrite_trace (u8 * s, va_list * args)
{
  ...
  return format (s, "SRv6-AD-Flow-rewrite: src %U dst %U",
...
always_inline u32
ad_flow_value_get_thread_index (clib_bihash_kv_40_8_t * value)
{
...
end_ad_flow_walk_expect_first_hdr (vlib_main_t * vm, vlib_buffer_t * b,
			ip6_ext_header_t * first_hdr, u8 first_hdr_type, u8 expected_hdr_type,
...
/**
 * @brief Function doing SRH processing for per-flow AD behavior (IPv6 inner traffic)
 */
static_always_inline void
end_ad_flow_processing_v6 (vlib_main_t * vm,
			 vlib_buffer_t * b,
		   ip6_header_t * ip,
		   srv6_ad_flow_localsid_t * ls_mem, u32 * next,
			 vlib_combined_counter_main_t ** cnt, f64 now)
{
  ...
  /* Compute the total encapsulation size and determine ULH type */
	...
	int ret = end_ad_flow_walk_expect_first_hdr (vm, b,
							ip6_ext_next_header ((ip6_ext_header_t *) srh),
							srh->protocol, IP_PROTOCOL_IPV6, &encap_length,
							(u8 **) &ulh);
...
static_always_inline void
end_ad_flow_processing_v4 (vlib_main_t * vm,
			 vlib_buffer_t * b,
		   ip6_header_t * ip,
		   srv6_ad_flow_localsid_t * ls_mem, u32 * next,
			 vlib_combined_counter_main_t ** cnt, f64 now)
{
  ...
  srv6_ad_is_idle_entry_ctx_t ctx;
	...
	end_ad_flow_walk_expect_first_hdr (vm, b, (void *) (ip + 1),
			ip->protocol, IP_PROTOCOL_IPV6_ROUTE, &encap_length, (u8 **) &srh);
  ...
  /* Add SRH length to the total encapsulation size */
	...
	int ret = end_ad_flow_walk_expect_first_hdr (vm, b,
							ip6_ext_next_header ((ip6_ext_header_t *) srh),
							srh->protocol, IP_PROTOCOL_IP_IN_IP, &encap_length,
							(u8 **) &ulh);
    ...
    srv6_ad_flow_localsid_t *ls_mem0;
	  ...
	  /* SRH processing */
		if (ls_mem0->inner_type == AD_TYPE_IP6)
	  	end_ad_flow_processing_v6 (vm, b0, ip0, ls_mem0, &next0, &cnt0, now);
		else
			end_ad_flow_processing_v4 (vm, b0, ip0, ls_mem0, &next0, &cnt0, now);
	  ...
	  vlib_increment_combined_counter (cnt0,
					   thread_index,
					   ls_mem0->index, 1,
