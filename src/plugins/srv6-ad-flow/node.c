...
static_always_inline void
end_ad_flow_usid_processing_v6 (vlib_main_t *vm, vlib_buffer_t *b,
...
static_always_inline void
end_ad_flow_usid_processing_v4 (vlib_main_t *vm, vlib_buffer_t *b,
				ip6_header_t *ip, u8 usid_len, u8 usid_index,
				u8 usid_next_index, u8 usid_next_len,
				srv6_ad_flow_localsid_t *ls_mem, u32 *next,
				vlib_combined_counter_main_t **cnt,
 ...
 */
static uword
srv6_ad_flow_usid_localsid_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			       vlib_frame_t *frame)
{
  ip6_sr_main_t *srm = &sr_main;
  f64 now = vlib_time_now (vm);
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* SRH processing */
	  if (ls_mem0->inner_type == AD_TYPE_IP6)
	    end_ad_flow_usid_processing_v6 (
	      vm, b0, ip0, ls0->usid_len, ls0->usid_index,
	      ls0->usid_next_index, ls0->usid_next_len, ls_mem0, &next0, &cnt0,
	      &cnt_idx0, now);
	  else
	    end_ad_flow_usid_processing_v4 (
	      vm, b0, ip0, ls0->usid_len, ls0->usid_index,
	      ls0->usid_next_index, ls0->usid_next_len, ls_mem0, &next0, &cnt0,
	      &cnt_idx0, now);
...
VLIB_REGISTER_NODE (srv6_ad_flow_usid_localsid_node) = {
  .function = srv6_ad_flow_usid_localsid_fn,
  .name = "srv6-ad-flow-usid-localsid",
  .vector_size = sizeof (u32),
  .format_trace = format_srv6_ad_flow_localsid_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_next_nodes = SRV6_AD_FLOW_LOCALSID_N_NEXT,
