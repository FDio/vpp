...
ad_flow_per_thread_entry_delete (srv6_ad_flow_localsid_t *ls,
                                 ...
                                 srv6_ad_flow_entry_t *e, int lru_delete)
{
...
static_always_inline int
ad_flow_per_thread_lru_free_one (srv6_ad_flow_localsid_t *ls,
                                 adflow_per_thread_data_t *td, f64 now)
{
...
ad_flow_per_thread_entry_alloc (srv6_ad_flow_localsid_t *ls,
                                adflow_per_thread_data_t *td, f64 now)
{
  srv6_ad_flow_entry_t *e;
...
int ad_flow_per_thread_is_idle_entry_cb (clib_bihash_kv_40_8_t *kv, void *arg)
{
 ...
 */
static_always_inline int end_ad_flow_walk_expect_first_hdr (
    vlib_main_t *vm, vlib_buffer_t *b, ip6_ext_header_t *first_hdr,
    u8 first_hdr_type, u8 expected_hdr_type, u32 *encap_length, u8 **found_hdr)
{
  ...
  if (!e)
    {
      ...
      ctx.now = now;
      ctx.ls = ls_mem;
      clib_bihash_add_or_overwrite_stale_40_8 (
          h, &kv, ad_flow_per_thread_is_idle_entry_cb, &ctx);
    }
  e->last_heard = now;
              ...
              else
                {
                  ...
                  adflow_per_thread_data_t *td =
                      &ls0_mem->per_thread_data
                           [ad_flow_per_thread_value_get_thread_index (
                               &value0)];

                  /* found */
                  s0 = pool_elt_at_index (
                      td->cache,
                      ad_flow_per_thread_value_get_session_index (&value0));
                  ASSERT (s0);
