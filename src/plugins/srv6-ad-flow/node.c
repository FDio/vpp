...
end_ad_flow_processing_v6 (vlib_main_t * vm, vlib_buffer_t * b,
			   ip6_header_t * ip,
  ...
  if (!e)
	...
	(u64) ((u32) (e - per_thread_data->cache)) << 32 | vm->thread_index;
      ...
      clib_bihash_add_or_overwrite_stale_40_8 (h, &kv,
					       ad_flow_per_thread_is_idle_entry_cb,
					       &ctx);
    }
  e->last_heard = now;
...
static_always_inline void
end_ad_flow_processing_v4 (vlib_main_t * vm, vlib_buffer_t * b,
			   ip6_header_t * ip,
			   srv6_ad_flow_localsid_t * ls_mem, u32 * next,
			   vlib_combined_counter_main_t ** cnt, u32 * cnt_idx,
			   f64 now)
			  ...
			  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  vlib_increment_combined_counter (cnt0, thread_index, cnt_idx0, 1,
			  ...
			  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  ls0 = pool_elt_at_index (srm->localsids,
				   sm->
				   sw_iface_localsid4[vnet_buffer
						      (b0)->sw_if_index
						      [VLIB_RX]]);
			  ...
			  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  ls0 = pool_elt_at_index (srm->localsids,
				   sm->
				   sw_iface_localsid6[vnet_buffer
						      (b0)->sw_if_index
						      [VLIB_RX]]);
	  ls0_mem = ls0->plugin_mem;
