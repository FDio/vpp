...
format_vrrp_vr_addrs (u8 *s, va_list *args)
{
  ...
  vec_foreach (addr, addrs)
    {
      s =
	format (s, "%U ", (is_ipv6) ? format_ip6_address : format_ip4_address,
...
format_vrrp_vr_key (u8 *s, va_list *args)
{
  ...
  s =
    format (s, "[%d] sw_if_index %u VR ID %u IPv%d", vr - vmp->vrs,
	    vrc->sw_if_index, vrc->vr_id, (vrc->flags & VRRP_VR_IPV6) ? 6 : 4);
...
format_vrrp_vr_track_ifs (u8 *s, va_list *args)
{
  ...
  vec_foreach (track_if, track_ifs)
    s = format (s, "sw_if_index %u priority %u ", track_if->sw_if_index,
		track_if->priority);
...
format_vrrp_vr (u8 *s, va_list *args)
{
  ...
  s = format (s, "   state %U flags: %U\n", format_vrrp_vr_state,
	      vr->runtime.state, format_vrrp_vr_flags, vr->config.flags);
...
format_vrrp_packet_hdr (u8 *s, va_list *args)
{
  ...
  s = format (s,
	      ...
	      version, pkt->vrrp_version_and_type & 0xf, pkt->vr_id,
	      pkt->priority, pkt->n_addrs,
