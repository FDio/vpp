...
(vl_api_lb_vip_dump_t * mp)
{
  ...
  /* construct as stats under this vip */
  pool_foreach(vip, lbm->vips, {
      memcpy(rmp->vip.pfx.address.un.ip6, vip->prefix.as_u8, sizeof(vip->prefix));
  ...
  (vl_api_registration_t * reg, u32 context, lb_vip_t * vip)
{
  ...
  pool_foreach(as_index, vip->as_indexes, {
      as = &lbm->ass[*as_index];
      memcpy(rmp->as.pfx.address.un.ip6, &(as->address.as_u8), sizeof(as->address));
      rmp->as.pfx.len = 128;
