...
/* Encode internal ip46_address_t + plen (IPv4 uses plen = 96 + ip4_plen)
 * into a vl_api_prefix_t with correct address family and length. */
static void
lb_vip_encode_prefix (lb_vip_t *vip, vl_api_prefix_t *pfx)
{
  if (ip46_address_is_ip4 (&vip->prefix))
    {
      pfx->address.af = ADDRESS_IP4;
      clib_memcpy (&pfx->address.un.ip4, &vip->prefix.ip4, sizeof (ip4_address_t));
      pfx->len = vip->plen - 96;
    }
  else
    {
      pfx->address.af = ADDRESS_IP6;
      clib_memcpy (&pfx->address.un.ip6, &vip->prefix.ip6, sizeof (ip6_address_t));
      pfx->len = vip->plen;
    }
}
...
vl_api_lb_conf_get_t_handler (vl_api_lb_conf_get_t *mp)
{
  ...
  REPLY_MACRO2 (
    VL_API_LB_CONF_GET_REPLY, ({
      clib_memcpy (&rmp->ip4_src_address, &lbm->ip4_src_address, sizeof (lbm->ip4_src_address));
      clib_memcpy (&rmp->ip6_src_address, &lbm->ip6_src_address, sizeof (lbm->ip6_src_address));
...
(vl_api_lb_add_del_vip_t * mp)
{
  ...
  /* Internal plen uses +96 offset for IPv4 (e.g. /8 is stored as 104) */
  u8 plen = (mp->pfx.address.af == ADDRESS_IP4) ? mp->pfx.len + 96 : mp->pfx.len;
  ...
  if (mp->is_del) {
    ...
    vl_api_lb_encap_type_t encap = clib_net_to_host_u32 (mp->encap);

    if (mp->pfx.address.af == ADDRESS_IP4)
      {
...
(vl_api_lb_vip_dump_t * mp)
{
  ...
  pool_foreach (vip, lbm->vips) {
      /* Hide placeholder VIP and soft-deleted VIPs */
      if (vip != lbm->vips && (vip->flags & LB_VIP_FLAGS_USED))
	{
	  ...
	  lb_vip_encode_prefix (vip, &rmp->vip.pfx);
...
(vl_api_lb_as_dump_t * mp)
{
  ...
  /* Convert API prefix to internal ip46 + plen representation */
  if (mp->pfx.address.af == ADDRESS_IP4)
    {
      ip46_address_set_ip4 (&pfx46, (ip4_address_t *) &mp->pfx.address.un.ip4);
      plen46 = mp->pfx.len + 96;
    }
  else
    {
      clib_memcpy (&pfx46.ip6, &mp->pfx.address.un.ip6, sizeof (pfx46.ip6));
      plen46 = mp->pfx.len;
    }

  /* all-zeros prefix with len 0 means dump all */
  dump_all = (mp->pfx.len == 0) && ip46_address_is_zero (&pfx46);

  pool_foreach (vip, lbm->vips)
   {
     if (dump_all || ((pfx46.as_u64[0] == vip->prefix.as_u64[0]) &&
		      (pfx46.as_u64[1] == vip->prefix.as_u64[1]) && (plen46 == vip->plen) &&
		      (mp->protocol == vip->protocol) && (mp->port == vip->port)))
       {
...
(vl_api_lb_flush_vip_t * mp)
{
  ...
  /* Internal plen uses +96 offset for IPv4 (e.g. /8 is stored as 104) */
  vip_plen = (mp->pfx.address.af == ADDRESS_IP4) ? mp->pfx.len + 96 : mp->pfx.len;
