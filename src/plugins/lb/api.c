...
(vl_api_lb_add_del_vip_t * mp)
{
  ...
  ip_address_with_prefix_decode(&mp->pfx, &prefix);
  args.plen = prefix.len
  memcpy (&(args.prefix), &prefix->addr, sizeof(args.prefix));
...
(vl_api_lb_add_del_as_t * mp)
{
  ...
  ip_address_with_prefix_decode (&mp->pfx, &vip_ip_prefix);
  ip_address_decode (&mp->as_address, &as_address);

  if ((rv = lb_vip_find_index(&vip_ip_prefix.addr, vip_ip_prefix.len,
                              mp->protocol, ntohs(mp->port), &vip_index)))
    goto done;
...
(vl_api_lb_as_dump_t * mp)
{
  ...
  dump_all = (prefix.addr.ip6.as_u64[0] == 0) && (prefix.addr.ip6.as_u64[1] == 0);

  /* *INDENT-OFF* */
  pool_foreach(vip, lbm->vips,
  ({
    if ( dump_all
        || ((prefix.addr.as_u64[0] == vip->prefix.addr.as_u64[0])
        && (prefix,addr.as_u64[1] == vip->prefix.addr.as_u64[1])
