...
(vl_api_lb_get_vip_list_t * mp)
{
  ...
  rmp->_vl_msg_id =
    ntohs (VL_API_LB_GET_VIP_LIST_REPLY + lbm->msg_id_base);
...
(vl_api_lb_get_vip_conn_t * mp)
{
  ...
  rv = lb_vip_find_index(&prefix, mp->prefix_length, &vip_index);
  if (rv)
    {
      /* *INDENT-OFF* */
  ...
  rmp->vip_conn.cur_conn =
      vlib_refcount_get(&lbm->vip_refcount, vip_index);
  rmp->vip_conn.max_conn =
      vlib_get_simple_counter(&lbm->vip_counters[LB_VIP_COUNTER_MAX_SESSION],
                              vip_index);
  rmp->vip_conn.total_conn =
      vlib_get_simple_counter(&lbm->vip_counters[LB_VIP_COUNTER_FIRST_PACKET],
                              vip_index);
  ...
  pool_foreach(as_index, vip->as_indexes, {
      as = &lbm->ass[*as_index];
      memcpy(rmp->as_conns[*as_index].ip_prefix, &(as->address), sizeof(as->address.ip6));
      rmp->as_conns[*as_index].cur_conn =
          vlib_refcount_get(&lbm->as_refcount, *as_index);
      rmp->as_conns[*as_index].max_conn =
          vlib_get_simple_counter(&lbm->as_counters[LB_VIP_COUNTER_MAX_SESSION],
                                  *as_index);
      rmp->as_conns[*as_index].total_conn =
          vlib_get_simple_counter(&lbm->as_counters[LB_VIP_COUNTER_FIRST_PACKET],
                                  *as_index);
  });
