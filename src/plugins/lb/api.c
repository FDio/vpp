...
vl_api_lb_conf_t_handler (vl_api_lb_conf_t *mp)
{
  ...
  flow_timeout =
    mp->flow_timeout == ~0 ? lbm->flow_timeout : ntohl (mp->flow_timeout);
...
vl_api_lb_add_del_vip_t_handler (vl_api_lb_add_del_vip_t *mp)
{
  ...
  ip46_type_t type46 = ip_address_decode (&mp->pfx.address, &(args.prefix));
  /* LB plugin uses 96+plen format for IPv4 internally */
  args.plen = (type46 == IP46_TYPE_IP4) ? (96 + mp->pfx.len) : mp->pfx.len;

  if (mp->is_del)
    {
      u32 vip_index;
      if (!(rv = lb_vip_find_index (&(args.prefix), args.plen, mp->protocol,
  ...
  else
    {
      ...
      u32 encap = ntohl (mp->encap);

      if (type46 == IP46_TYPE_IP4)
	{
      ...
      if (encap == LB_API_ENCAP_TYPE_L3DSR)
	{
	  args.encap_args.dscp = (u8) (mp->dscp & 0x3F);
	       ...
	       (encap == LB_API_ENCAP_TYPE_NAT6))
	{
	  args.encap_args.srv_type = ntohl (mp->type);
...
(vl_api_lb_add_del_as_t * mp)
{
  ...
  if ((rv = lb_vip_find_index (&vip_ip_prefix, mp->pfx.len, mp->protocol,
			       ntohs (mp->port), &vip_index)))
    goto done;
...
static void
vl_api_lb_vip_dump_t_handler (vl_api_lb_vip_dump_t *mp)
{
  ...
  pool_foreach (vip, lbm->vips)
      ...
      if (vip != lbm->vips)
	{
	  ...
	  ip_address_encode (&vip->prefix, IP46_TYPE_ANY,
			     &rmp->vip.pfx.address);
...
static void
send_lb_as_details (vl_api_registration_t *reg, u32 context, lb_vip_t *vip)
{
  ...
  pool_foreach (as_index, vip->as_indexes)
      ...
      if (*as_index != 0)
	{
	  ...
	  rmp->context = context;
	  ip_address_encode (&vip->prefix, IP46_TYPE_ANY,
			     (vl_api_address_t *) &rmp->vip.pfx.address);
	  rmp->vip.pfx.len = vip->plen;
	  rmp->vip.protocol = htonl (vip->protocol);
	  rmp->vip.port = htons (vip->port);
...
static void
vl_api_lb_as_dump_t_handler (vl_api_lb_as_dump_t *mp)
{
  lb_main_t *lbm = &lb_main;
  lb_vip_t *vip = 0;
  u8 dump_all = 0;
  ...
  clib_memcpy (&prefix.ip6, mp->pfx.address.un.ip6,
...
static void
vl_api_lb_add_del_intf_nat4_t_handler (vl_api_lb_add_del_intf_nat4_t *mp)
{
  lb_main_t *lbm = &lb_main;
...
static void
vl_api_lb_add_del_intf_nat6_t_handler (vl_api_lb_add_del_intf_nat6_t *mp)
{
  lb_main_t *lbm = &lb_main;
