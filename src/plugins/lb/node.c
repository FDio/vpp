	  ...
	  if ((is_input_v4==1) && (encap_type==LB_ENCAP_TYPE_NAT4))
	    {
	      ...
	      if(PREDICT_TRUE(ip40->protocol == IP_PROTOCOL_UDP))
		{
		     ...
		     u8 is_input_v4)
{
  ...
  while (n_left_from > 0)
  {
      ...
      if (PREDICT_FALSE (p0->flags & VLIB_BUFFER_IS_TRACED))
	{
      ...
      vlib_validate_buffer_enqueue_x1 (vm, node, next_index, to_next,
				       n_left_to_next, pi0,
				       is_input_v4? LB_NODEPORT_NEXT_IP4_NAT4:
			    ...
			    lb_snat4_key_t * mapping)
{
  ...
  m_key4.addr = match->addr;
  m_key4.port = match->port;
  m_key4.protocol = match->protocol;
  m_key4.fib_index = match->fib_index;
			    ...
			    lb_snat6_key_t * mapping)
{
  ...
  if (m->svr_type == LB_SVR_TYPE_VIP_PORT)
    {
      mapping->addr.as_u64[0] = m->vip.as_u64[0];
      mapping->addr.as_u64[1] = m->vip.as_u64[1];
      mapping->port = clib_host_to_net_u16 (m->port);
      mapping->fib_index = m->fib_index;
      mapping->protocol = match->protocol;
    }
    ...
    {
      mapping->addr.as_u64[0] = m->node_ip.as_u64[0];
      mapping->addr.as_u64[1] = m->node_ip.as_u64[1];
      mapping->port = clib_host_to_net_u16 (m->node_port);
      mapping->fib_index = m->fib_index;
      mapping->protocol = match->protocol;
    }
...
static uword
lb_nat_in2out_node_fn (vlib_main_t * vm,
                        vlib_node_runtime_t * node,
                        vlib_frame_t * frame,
			u32 is_nat4)
{
  ...
  stats_node_index = is_nat4?
      lb_nat4_in2out_node.index : lb_nat6_in2out_node.index;
          ...
          if(is_nat4)
            {
              ...
              if (lb_nat44_mapping_match (lbm, &key40, &sm40))
                {
                  next0= LB_NAT4_IN2OUT_NEXT_DROP;
                  goto trace0;
                }

              new_addr0 = sm40.addr.as_u32;
              new_port0 = sm40.port;
	      ...
	      else if (PREDICT_TRUE(proto0 == LB_NAT_PROTOCOL_UDP))
		{
		  ...
		  udp0->checksum = ip_csum_fold(csum);
          ...
          else
            {
              ...
              if (lb_nat66_mapping_match (lbm, &key60, &sm60))
                {
                  next0= LB_NAT6_IN2OUT_NEXT_DROP;
                  goto trace0;
                }

              new_addr0.as_u64[0] = sm60.addr.as_u64[0];
              new_addr0.as_u64[1] = sm60.addr.as_u64[1];
              new_port0 = sm60.port;
...
static uword
lb_nat6_in2out_node_fn (vlib_main_t * vm,
         vlib_node_runtime_t * node, vlib_frame_t * frame)
{
