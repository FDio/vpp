                  ...
                  if (ip40->protocol == IP_PROTOCOL_UDP)
                    {
                      ...
                      csum = uh->checksum;
                      csum = ip_csum_update (csum, old_dst, lbm->ass[asindex0].address.ip4.as_u32,
                                             ip4_header_t, dst_address);
                      csum = ip_csum_update (csum, old_port, vip0->encap_args.target_port,
                                             ip4_header_t, length);
                  ...
                  else if (ip40->protocol == IP_PROTOCOL_TCP)
                    {
                      ...
                      tcp0->checksum = 0;
                      tcp0->checksum = ip4_tcp_udp_compute_checksum (vm, p0, ip40);
                    }
                  else
                    {
                      asindex0 = 0;
                  ...
                  if (ip60->protocol == IP_PROTOCOL_UDP)
                    {
                      ...
                      csum = ip_csum_add_even (csum, vip0->encap_args.target_port);
                      uh->checksum = ip_csum_fold (csum);
                    }
                  else if (ip60->protocol == IP_PROTOCOL_TCP)
                    {
                      ...
                      tcp0->dst_port = vip0->encap_args.target_port;
                      tcp0->checksum = 0;
                      tcp0->checksum = ip6_tcp_udp_icmp_compute_checksum (vm, p0, ip60, &bogus);
                    }
                  else
                    {
                      asindex0 = 0;
