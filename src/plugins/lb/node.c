...
lb_get_sticky_table (u32 thread_index)
{
  ...
  if (PREDICT_FALSE (sticky_ht && (lbm->per_cpu_sticky_buckets !=
				   lb_hash_nbuckets (sticky_ht))))
    {
  ...
  if (PREDICT_FALSE (sticky_ht == NULL))
    {
      lbm->per_cpu[thread_index].sticky_ht =
	lb_hash_alloc (lbm->per_cpu_sticky_buckets, lbm->flow_timeout);
...
lb_node_fn (
  ...
  lb_encap_type_t
    encap_type, // Compile-time parameter is GRE4/GRE6/L3DSR/NAT4/NAT6
  u8
    per_port_vip) // Compile-time parameter stating that is per_port_vip or not
{
  lb_main_t *lbm = &lb_main;
  ...
  if (PREDICT_TRUE (n_left_from > 0))
    {
      ...
      lb_node_get_hash (lbm, p0, is_input_v4, &nexthash0, &next_vip_idx0,
			per_port_vip);
    }
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_TRUE (n_left_from > 1))
	    {
	      ...
	      lb_node_get_hash (lbm, p1, is_input_v4, &nexthash0,
	  ...
	  lb_hash_get (sticky_ht, hash0, vip_index0, lb_time,
	  ...
	  if (PREDICT_TRUE (asindex0 != 0))
	  ...
	  else if (PREDICT_TRUE (available_index0 != ~0))
	    {
	      ...
	      asindex0 =
		vip0->new_flow_table[hash0 & vip0->new_flow_table_mask]
		  .as_index;
	      counter = LB_VIP_COUNTER_FIRST_PACKET;
	      counter = (asindex0 == 0) ? LB_VIP_COUNTER_NO_SERVER : counter;

	      // TODO: There are race conditions with as0 and vip0
	      // manipulation. Configuration may be changed, vectors resized,
	      // etc...
		...
		-1);
	      ...
	      // Add sticky entry
	      // Note that when there is no AS configured, an entry is
	      // configured anyway. But no configured AS is not something that
	      // should happen
	      lb_hash_put (sticky_ht, hash0, asindex0, vip_index0,
	  ...
	  if ((encap_type == LB_ENCAP_TYPE_GRE4) ||
	      ...
	      else /* encap GRE6*/
		{
		  ...
		  ip60->payload_length =
		    clib_host_to_net_u16 (len0 + sizeof (gre_header_t));
		  ip60->protocol = IP_PROTOCOL_GRE;
		}

	      gre0->flags_and_version = 0;
	      gre0->protocol = (is_input_v4) ? clib_host_to_net_u16 (0x0800) :
	  ...
	  else if (encap_type == LB_ENCAP_TYPE_L3DSR) /* encap L3DSR*/
	    {
	      ...
	      csum = ip40->checksum;
	      csum = ip_csum_update (csum, old_tos, new_tos, ip4_header_t,
				     tos /* changed member */);
	      csum = ip_csum_update (csum, old_dst, new_dst, ip4_header_t,
		     ...
		     vlib_frame_t *frame, u8 is_input_v4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  entry0 =
	    hash_get_mem (lbm->vip_index_by_nodeport, &(udp_0->dst_port));

	  // Enqueue to next
	  vnet_buffer (p0)->ip.adj_index[VLIB_TX] =
	    entry0 ? entry0[0] : ADJ_INDEX_INVALID;

	  if (PREDICT_FALSE (p0->flags & VLIB_BUFFER_IS_TRACED))
	    {
	      lb_nodeport_trace_t *tr =
	  ...
	  vlib_validate_buffer_enqueue_x1 (
	    ...
	    is_input_v4 ? LB4_NODEPORT_NEXT_IP4_NAT4 :
 ...
 * @param sm          NAT main.
 * @param match       Address and port to match.
 * @param mapping     External or local address and port of the matched
 * mapping.
 *
 * @returns 0 if match found otherwise 1.
 */
		       ...
		       vlib_frame_t *frame, u32 is_nat4)
{
  ...
  vlib_node_increment_counter (
    vm, stats_node_index, LB_NAT_IN2OUT_ERROR_IN2OUT_PACKETS, pkts_processed);
  return frame->n_vectors;
}
