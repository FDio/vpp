              ...
              unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  u8 del = 0;
  u8 flush = 0;
  u32 weight = ~0; /* sentinel: not provided */
  int ret;
  clib_error_t *error = 0;
  ...
  if (weight != ~0)
    ...
    {
      error = clib_error_return (0, "weight requires exactly one AS address");
      goto done;
    }
      if (del)
    {
  ...
  if (weight != ~0)
  {
    if ((ret = lb_vip_set_as_weight (vip_index, &as_array[0], (u8) weight)))
    {
