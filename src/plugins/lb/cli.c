...
static clib_error_t *
lb_vip_command_fn (vlib_main_t *vm, unformat_input_t *input,
		   vlib_cli_command_t *cmd)
{
  ...
  if ((encap != LB_ENCAP_TYPE_L3DSR) && (dscp != ~0))
    {
      error =
	clib_error_return (0, "lb_vip_add error: "
  ...
  if (!del)
    ...
    {
      if ((ret = lb_vip_find_index (&(args.prefix), args.plen, args.protocol,
				    args.port, &index)))
	{
...
static clib_error_t *
lb_as_command_fn (vlib_main_t *vm, unformat_input_t *input,
		  vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "%U", unformat_ip46_address, &as_addr,
		    IP46_TYPE_ANY))
	{
  ...
  if ((ret = lb_vip_find_index (&vip_prefix, vip_plen, protocol, (u16) port,
				&vip_index)))
    {
      error = clib_error_return (0, "lb_vip_find_index error %d", ret);
      goto done;
  ...
  if (del)
    {
      if ((ret =
	     lb_vip_del_ass (vip_index, as_array, vec_len (as_array), flush)))
	{
...
static clib_error_t *
lb_conf_command_fn (vlib_main_t *vm, unformat_input_t *input,
		    vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "ip4-src-address %U", unformat_ip4_address,
		    &ip4))
	;
      else if (unformat (line_input, "ip6-src-address %U",
			 unformat_ip6_address, &ip6))
	;
      else if (unformat (line_input, "buckets %d", &per_cpu_sticky_buckets))
	;
      else if (unformat (line_input, "buckets-log2 %d",
			 &per_cpu_sticky_buckets_log2))
	{
...
VLIB_CLI_COMMAND (lb_conf_command, static) = {
  .path = "lb conf",
  .short_help = "lb conf [ip4-src-address <addr>] [ip6-src-address <addr>] "
		"[buckets <n>] [timeout <s>]",
  .function = lb_conf_command_fn,
};

static clib_error_t *
lb_show_command_fn (vlib_main_t *vm, unformat_input_t *input,
		    vlib_cli_command_t *cmd)
{
...
static clib_error_t *
lb_show_vips_command_fn (vlib_main_t *vm, unformat_input_t *input,
			 vlib_cli_command_t *cmd)
{
...
static clib_error_t *
lb_set_interface_nat_command_fn (vlib_main_t *vm, unformat_input_t *input,
				 vlib_cli_command_t *cmd, u8 is_nat6)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  ...
  vec_foreach (sw_if_index, inside_sw_if_indices)
      ...
      if (!is_nat6)
	{
	  if (lb_nat4_interface_add_del (*sw_if_index, is_del))
	    {
	      error =
		clib_error_return (0, "%s %U failed", is_del ? "del" : "add",
...
static clib_error_t *
lb_set_interface_nat4_command_fn (vlib_main_t *vm, unformat_input_t *input,
				  vlib_cli_command_t *cmd)
{
...
static clib_error_t *
lb_set_interface_nat6_command_fn (vlib_main_t *vm, unformat_input_t *input,
				  vlib_cli_command_t *cmd)
{
...
static clib_error_t *
lb_flowtable_flush_command_fn (vlib_main_t *vm, unformat_input_t *input,
			       vlib_cli_command_t *cmd)
{
  lb_flush_vip_as (~0, 0);
...
static clib_error_t *
lb_flush_vip_command_fn (vlib_main_t *vm, unformat_input_t *input,
			 vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  int ret;
