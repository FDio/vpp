              ...
              unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  else if (weight != ~0)
  {
    ...
    ret = lb_vip_add_ass (vip_index, as_array, vec_len (as_array), w);
    if (ret == VNET_API_ERROR_VALUE_EXIST)
    {
      u32 i;
      vec_foreach_index (i, as_array)
	{
	  if ((ret = lb_vip_set_as_weight (vip_index, &as_array[i], w)))
	    {
  ...
  {
    if ((ret = lb_vip_add_ass (vip_index, as_array, vec_len (as_array), 100)))
    {
      error = clib_error_return (0, "lb_vip_add_ass error %d", ret);
      goto done;
    }
