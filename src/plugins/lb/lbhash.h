...
#define lb_hash_foreach_bucket(h, bucket)                                     \
  for (bucket = (h)->buckets; bucket < (h)->buckets + lb_hash_nbuckets (h);   \
       bucket++)

#define lb_hash_foreach_entry(h, bucket, i)                                   \
  lb_hash_foreach_bucket (h, bucket) for (i = 0; i < LBHASH_ENTRY_PER_BUCKET; \
					  i++)

#define lb_hash_foreach_valid_entry(h, bucket, i, now)                        \
  lb_hash_foreach_entry (                                                     \
    h, bucket, i) if (!clib_u32_loop_gt ((now), bucket->timeout[i]))

static_always_inline lb_hash_t *
lb_hash_alloc (u32 buckets, u32 timeout)
{
  if (!is_pow2 (buckets))
    return NULL;
...
static_always_inline void
lb_hash_prefetch_bucket (lb_hash_t *ht, u32 hash)
{
...
static_always_inline void
lb_hash_get (lb_hash_t *ht, u32 hash, u32 vip, u32 time_now,
	     u32 *available_index, u32 *found_value)
{
  ...
  *available_index =
    (bitmask) ? __builtin_ctz (bitmask) / 4 : *available_index;

  // mask[*] = (timeout[*] > now) && (hash[*] == hash)
  mask = _mm_and_si128 (
    mask, _mm_cmpeq_epi32 (_mm_loadu_si128 ((__m128i *) bucket->hash),
			   _mm_set1_epi32 (hash)));

  // Load the array of vip values
  // mask[*] = (timeout[*] > now) && (hash[*] == hash) && (vip[*] == vip)
  mask = _mm_and_si128 (
    mask, _mm_cmpeq_epi32 (_mm_loadu_si128 ((__m128i *) bucket->vip),
			   _mm_set1_epi32 (vip)));
  ...
  for (i = 0; i < LBHASH_ENTRY_PER_BUCKET; i++)
    {
      ...
      *found_value = (cmp || timeouted) ? *found_value : bucket->value[i];
      bucket->timeout[i] =
	(cmp || timeouted) ? time_now + ht->timeout : bucket->timeout[i];
      *available_index =
	(timeouted && (*available_index == ~0)) ? i : *available_index;
...
static_always_inline void
lb_hash_put (lb_hash_t *h, u32 hash, u32 value, u32 vip, u32 available_index,
	     u32 time_now)
{
...
static_always_inline u32
lb_hash_elts (lb_hash_t *h, u32 time_now)
{
  u32 tot = 0;
  lb_hash_bucket_t *bucket;
  u32 i;
  lb_hash_foreach_valid_entry (h, bucket, i, time_now) { tot++; }
  return tot;
}
