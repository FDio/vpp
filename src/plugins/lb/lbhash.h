		 ...
		 u32 *available_index, u32 *found_value)
{
  ...
  *found_value = 0;
  *available_index = ~0;
#ifdef CLIB_HAVE_VEC128_COMPARE_MASK
  ...
  u32 bitmask = u32x4_compare_mask (expired);

  if (bitmask)
    *available_index = count_trailing_zeros (bitmask);
  ...
  bitmask = u32x4_compare_mask (match);
  if (bitmask)
    {
      u32 found_index = count_trailing_zeros (bitmask);
