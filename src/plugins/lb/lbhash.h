		 ...
		 u32 *available_index, u32 *found_value)
{
  ...
  *available_index = ~0;
  ...
  u32x4 expired =
    (time_now4 - u32x4_load_unaligned (bucket->timeout)) <
    u32x4_splat (0x7fffffff);
  ...
  u32x4 live = ~expired;
  u32x4 match = live &
		(u32x4_load_unaligned (bucket->hash) == u32x4_splat (hash)) &
		(u32x4_load_unaligned (bucket->vip) == u32x4_splat (vip));
