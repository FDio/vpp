...
// After so many seconds. It is assumed that inter-core race condition will not
// occur.
...
u32
lb_hash_time_now (vlib_main_t *vm)
{
  return (u32) (vlib_time_now (vm) + 10000);
}

u8 *
format_lb_main (u8 *s, va_list *args)
{
  ...
  for (thread_index = 0; thread_index < tm->n_vlib_mains; thread_index++)
      ...
      if (h)
	{
	  ...
	  s = format (s, "  usage: %d / %d\n",
		      lb_hash_elts (h, lb_hash_time_now (vlib_get_main ())),
		      lb_hash_size (h));
	}
    }
...
static char *lb_vip_type_strings[] = {
  [LB_VIP_TYPE_IP6_GRE6] = "ip6-gre6",	 [LB_VIP_TYPE_IP6_GRE4] = "ip6-gre4",
  [LB_VIP_TYPE_IP4_GRE6] = "ip4-gre6",	 [LB_VIP_TYPE_IP4_GRE4] = "ip4-gre4",
  [LB_VIP_TYPE_IP4_L3DSR] = "ip4-l3dsr", [LB_VIP_TYPE_IP4_NAT4] = "ip4-nat4",
  [LB_VIP_TYPE_IP6_NAT6] = "ip6-nat6",
};

u8 *
format_lb_vip_type (u8 *s, va_list *args)
{
...
uword
unformat_lb_vip_type (unformat_input_t *input, va_list *args)
{
  lb_vip_type_t *vipt = va_arg (*args, lb_vip_type_t *);
  u32 i;
  for (i = 0; i < LB_VIP_N_TYPES; i++)
...
u8 *
format_lb_vip (u8 *s, va_list *args)
{
  lb_vip_t *vip = va_arg (*args, lb_vip_t *);
  s = format (s, "%U %U new_size:%u #as:%u%s", format_lb_vip_type, vip->type,
	      format_ip46_prefix, &vip->prefix, vip->plen, IP46_TYPE_ANY,
	      vip->new_flow_table_mask + 1, pool_elts (vip->as_indexes),
...
u8 *
format_lb_as (u8 *s, va_list *args)
{
  lb_as_t *as = va_arg (*args, lb_as_t *);
  return format (s, "%U %s", format_ip46_address, &as->address, IP46_TYPE_ANY,
		 (as->flags & LB_AS_FLAGS_USED) ? "used" : "removed");
}

u8 *
format_lb_vip_detailed (u8 *s, va_list *args)
{
  lb_main_t *lbm = &lb_main;
  lb_vip_t *vip = va_arg (*args, lb_vip_t *);
  ...
  s =
    format (s,
	    "%U %U [%lu] %U%s\n"
	    "%U  new_size:%u\n",
	    format_white_space, indent, format_lb_vip_type, vip->type,
	    vip - lbm->vips, format_ip46_prefix, &vip->prefix, (u32) vip->plen,
	    IP46_TYPE_ANY, (vip->flags & LB_VIP_FLAGS_USED) ? "" : " removed",
	    format_white_space, indent, vip->new_flow_table_mask + 1);
  ...
  u32 *count = 0;
  vec_validate (count,
		pool_len (lbm->ass)); // Possibly big alloc for not much...
  ...
  pool_foreach (as_index, vip->as_indexes)
    {
      ...
      s = format (s, "%U    %U %u buckets   %Lu flows  dpo:%u %s\n",
		  format_white_space, indent, format_ip46_address,
		  &as->address, IP46_TYPE_ANY, count[as - lbm->ass],
...
static int
lb_pseudorand_compare (void *a, void *b)
{
  lb_as_t *asa, *asb;
  lb_main_t *lbm = &lb_main;
...
static void
lb_vip_garbage_collection (lb_vip_t *vip)
{
  lb_main_t *lbm = &lb_main;
	  ...
	  if (lb_vip_is_nat4_port (vip))
	    {
	      ...
	      if (!clib_bihash_search_8_8 (&lbm->mapping_by_as4, &kv4,
					   &value4))
	  ...
	  else if (lb_vip_is_nat6_port (vip))
	    {
	      ...
	      if (!clib_bihash_search_24_8 (&lbm->mapping_by_as6, &kv6,
					    &value6))
...
static void
lb_vip_update_new_flow_table (lb_vip_t *vip)
{
  lb_main_t *lbm = &lb_main;
...
int
lb_conf (ip4_address_t *ip4_address, ip6_address_t *ip6_address,
	 u32 per_cpu_sticky_buckets, u32 flow_timeout)
{
  ...
  lb_get_writer_lock (); // Not exactly necessary but just a reminder that it
			 // exists for my future self
...
static int
lb_vip_port_find_index (ip46_address_t *prefix, u8 plen, u8 protocol, u16 port,
			lb_lkp_type_t lkp_type, u32 *vip_index)
{
  lb_main_t *lbm = &lb_main;
  lb_vip_t *vip;
...
static int
lb_vip_port_find_index_with_lock (ip46_address_t *prefix, u8 plen, u8 protocol,
...
static int
lb_vip_port_find_all_port_vip (ip46_address_t *prefix, u8 plen, u32 *vip_index)
{
  return lb_vip_port_find_index (prefix, plen, ~0, 0, LB_LKP_ALL_PORT_IP,
				 vip_index);
}

/* Find out per-port-vip entry with different protocol and port */
static int
lb_vip_port_find_diff_port (ip46_address_t *prefix, u8 plen, u8 protocol,
			    u16 port, u32 *vip_index)
{
  return lb_vip_port_find_index (prefix, plen, protocol, port,
...
int
lb_vip_find_index (ip46_address_t *prefix, u8 plen, u8 protocol, u16 port,
		   u32 *vip_index)
{
  int ret;
  lb_get_writer_lock ();
  ret =
    lb_vip_port_find_index_with_lock (prefix, plen, protocol, port, vip_index);
  lb_put_writer_lock ();
  return ret;
}

static int
lb_as_find_index_vip (lb_vip_t *vip, ip46_address_t *address, u32 *as_index)
{
  lb_main_t *lbm = &lb_main;
  /* This must be called with the lock owned */
  CLIB_SPINLOCK_ASSERT_LOCKED (&lbm->writer_lock);
...
int
lb_vip_add_ass (u32 vip_index, ip46_address_t *addresses, u32 n)
{
  lb_main_t *lbm = &lb_main;
  lb_get_writer_lock ();
  lb_vip_t *vip;
  ...
  vec_foreach (ip, to_be_added)
      ...
      else
	{
      ...
      as->next_hop_fib_entry_index = fib_table_entry_special_add (
	0, &nh, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
      as->next_hop_child_index = fib_entry_child_add (
	as->next_hop_fib_entry_index, lbm->fib_node_type, as - lbm->ass);
...
int
lb_vip_del_ass_withlock (u32 vip_index, ip46_address_t *addresses, u32 n,
			 u8 flush)
{
  lb_main_t *lbm = &lb_main;
  u32 now = (u32) vlib_time_now (vlib_get_main ());
...
int
lb_vip_del_ass (u32 vip_index, ip46_address_t *addresses, u32 n, u8 flush)
{
  lb_get_writer_lock ();
...
lb_vip_prefix_index_free (lb_main_t *lbm, u32 instance)
{
  ...
  lbm->vip_prefix_indexes =
    clib_bitmap_set (lbm->vip_prefix_indexes, instance, 0);
 ...
 */
static void
lb_vip_add_adjacency (lb_main_t *lbm, lb_vip_t *vip, u32 *vip_prefix_index)
{
  dpo_proto_t proto = 0;
  ...
  fib_table_entry_special_dpo_add (0, &pfx, lb_fib_src,
 ...
 */
static int
lb_vip_add_port_filter (lb_main_t *lbm, lb_vip_t *vip, u32 vip_prefix_index,
			u32 vip_idx)
{
  vip_port_key_t key;
 ...
 */
static int
lb_vip_del_port_filter (lb_main_t *lbm, lb_vip_t *vip)
{
  vip_port_key_t key;
 ...
 */
static void
lb_vip_del_adjacency (lb_main_t *lbm, lb_vip_t *vip)
{
  fib_prefix_t pfx = {};
  u32 vip_idx = 0;
...
int
lb_vip_add (lb_vip_add_args_t args, u32 *vip_index)
{
  lb_main_t *lbm = &lb_main;
  ...
  if ((args.port == 0) &&
      !lb_vip_port_find_diff_port (&(args.prefix), args.plen, args.protocol,
				   args.port, vip_index))
    {
      lb_put_writer_lock ();
      return VNET_API_ERROR_VALUE_EXIST;
  ...
  /* Make sure all VIP for a given prefix (using different ports) have the same
   * type. */
  if ((args.port != 0) &&
      !lb_vip_port_find_diff_port (&(args.prefix), args.plen, args.protocol,
				   args.port, vip_index) &&
...
static void
lb_dpo_lock (dpo_id_t *dpo)
{
}
static void
lb_dpo_unlock (dpo_id_t *dpo)
{
}
...
{
  return (
    (lb_as_t *) (((char *) node) - STRUCT_OFFSET_OF (lb_as_t, fib_node)));
}
...
lb_as_stack (lb_as_t *as)
{
  ...
  dpo_stack (
    dpo_type, lb_vip_is_ip4 (vip->type) ? DPO_PROTO_IP4 : DPO_PROTO_IP6,
    &as->dpo,
    fib_entry_contribute_ip_forwarding (as->next_hop_fib_entry_index));
}

static fib_node_back_walk_rc_t
lb_fib_node_back_walk_notify (fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
...
int
lb_nat4_interface_add_del (u32 sw_if_index, int is_del)
{
  if (is_del)
    {
...
int
lb_nat6_interface_add_del (u32 sw_if_index, int is_del)
{
  if (is_del)
    {
...
lb_init (vlib_main_t *vm)
{
  ...
  lbm->dpo_gre4_port_type =
    dpo_register_new_type (&lb_vft, lb_dpo_gre4_port_nodes);
  lbm->dpo_gre6_port_type =
    dpo_register_new_type (&lb_vft, lb_dpo_gre6_port_nodes);
  lbm->dpo_l3dsr_type = dpo_register_new_type (&lb_vft, lb_dpo_l3dsr_nodes);
  lbm->dpo_l3dsr_port_type =
    dpo_register_new_type (&lb_vft, lb_dpo_l3dsr_port_nodes);
  lbm->dpo_nat4_port_type =
    dpo_register_new_type (&lb_vft, lb_dpo_nat4_port_nodes);
  lbm->dpo_nat6_port_type =
    dpo_register_new_type (&lb_vft, lb_dpo_nat6_port_nodes);
  ...
  clib_bihash_init_8_8 (&lbm->vip_index_per_port, "vip_index_per_port",
			LB_VIP_PER_PORT_BUCKETS, LB_VIP_PER_PORT_MEMORY_SIZE);

  clib_bihash_init_8_8 (&lbm->mapping_by_as4, "mapping_by_as4",
			LB_MAPPING_BUCKETS, LB_MAPPING_MEMORY_SIZE);

  clib_bihash_init_24_8 (&lbm->mapping_by_as6, "mapping_by_as6",
			 LB_MAPPING_BUCKETS, LB_MAPPING_MEMORY_SIZE);
...
#undef _
    ...
    lb_fib_src =
      fib_source_allocate ("lb", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_SIMPLE);
