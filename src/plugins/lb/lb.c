...
static char *lb_vip_type_strings[] = {
  ...
  [LB_VIP_TYPE_IP6_NAT6] = "ip6-nat6",	 [LB_VIP_TYPE_IP6_NAT6_NOPORT] = "ip6-nat6-noport",
};
...
u8 *format_lb_vip_detailed (u8 * s, va_list * args)
{
  ...
  pool_foreach (as_index, vip->as_indexes) {
      as = &lbm->ass[*as_index];
      s = format (s, "%U    %U weight:%u buckets:%u flows:%Lu dpo:%u %s\n", format_white_space,
...
static int lb_pseudorand_compare(void *a, void *b)
{
  ...
  /* Sorting by (replica, address) accomplishes two goals:
   * 1) interleave all ASes at the same replica index so that the remainder buckets
   *    (M mod total_weight) are spread evenly across ASes
   * 2) guarantee a stable mapping across Maglev instances that have added the same
   *    backend+weights, but in a different order. */
...
int
lb_vip_add_ass (u32 vip_index, ip46_address_t *addresses, u32 n, u8 weight, u8 flush)
{
  lb_main_t *lbm = &lb_main;
  lb_get_writer_lock();
  lb_vip_t *vip;
...
next:
  ...
  vec_foreach(ip, to_be_updated) {
    ...
    lbm->ass[*ip].weight = weight;
    /* Evict any sticky entries left over from the previous incarnation
     * of this pool slot, so the re-added AS starts with a clean slate. */
    if (flush)
...
int
lb_vip_set_as_weight (u32 vip_index, ip46_address_t *addr, u8 weight, u8 flush)
{
  lb_main_t *lbm = &lb_main;
  lb_vip_t *vip;
  u32 as_index;
  ...
  /* Hard drain: evict sticky entries pointing at this AS so existing
   * flows rehash via the updated Maglev table onto a different AS. */
  if (flush)
  lb_flush_vip_as (vip_index, as_index);
...
lb_init (vlib_main_t * vm)
{
  ...
  lbm->dpo_nat6_noport_type = dpo_register_new_type (&lb_vft, lb_dpo_nat6_noport_nodes);
