                                 ...
                                 u32 *vip_prefix_index)
{
  ...
  if ((vip->protocol == IP_PROTOCOL_TCP)
      || (vip->protocol == IP_PROTOCOL_UDP))
    {
      ...
      pool_get(lbm->vip_prefix_indexes, vip_prefix_indexes);
      *vip_prefix_index = vip_prefix_indexes - lbm->vip_prefix_indexes;
    }
  else
    {
                                  ...
                                  u32 vip_prefix_index, u32 vip_idx)
{
  ...
  p = hash_get (lbm->vip_index_per_port, key.as_u64);

  if (p)
    return VNET_API_ERROR_TUNNEL_EXIST;

  hash_set (lbm->vip_index_per_port, key.as_u64, vip_idx);
...
static int lb_vip_del_port_filter(lb_main_t *lbm, lb_vip_t *vip)
{
  ...
  hash_unset (lbm->vip_index_per_port, key.as_u64);
...
static void lb_vip_del_adjacency(lb_main_t *lbm, lb_vip_t *vip)
{
  ...
  if ((vip->protocol == IP_PROTOCOL_TCP)
      || (vip->protocol == IP_PROTOCOL_UDP))
    {
      ...
      pool_put(lbm->vip_prefix_indexes, &(lbm->vip_prefix_indexes[vip->vip_prefix_index]));
    }
...
int lb_vip_add(lb_vip_add_args_t args, u32 *vip_index)
{
  ...
  if ((args.type == LB_VIP_TYPE_IP4_L3DSR) ||
      (args.type == LB_VIP_TYPE_IP4_L3DSR_PORT)) {
