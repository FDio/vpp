...
api_lb_conf (vat_main_t *vam)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
			 ...
			 unformat_ip6_address, &ip6_src_address))
	;
  ...
  M (LB_CONF, mp);
  clib_memcpy (&(mp->ip4_src_address), &ip4_src_address,
	       sizeof (ip4_src_address));
  clib_memcpy (&(mp->ip6_src_address), &ip6_src_address,
	       sizeof (ip6_src_address));
...
static int
api_lb_add_del_vip (vat_main_t *vam)
{
  unformat_input_t *line_input = vam->input;
...
static int
api_lb_add_del_as (vat_main_t *vam)
{
...
static int
api_lb_flush_vip (vat_main_t *vam)
{
  ...
  M (LB_FLUSH_VIP, mp);
  clib_memcpy (mp->pfx.address.un.ip6, &vip_prefix.ip6,
...
static int
api_lb_add_del_intf_nat4 (vat_main_t *vam)
{
  // Not yet implemented
  return -99;
}
...
static int
api_lb_add_del_intf_nat6 (vat_main_t *vam)
{
  // Not yet implemented
  return -99;
}
...
static void
vl_api_lb_vip_details_t_handler (vl_api_lb_vip_details_t *mp)
{
  vat_main_t *vam = &vat_main;

  print (vam->ofp, "%24U%14d%14d%18d", format_ip46_address,
	 &mp->vip.pfx.address, IP46_TYPE_ANY, mp->vip.pfx.len,
	 mp->vip.protocol, ntohs (mp->vip.port));
  /*
...
static int
api_lb_vip_dump (vat_main_t *vam)
{
...
static void
vl_api_lb_as_details_t_handler (vl_api_lb_as_details_t *mp)
{
  vat_main_t *vam = &vat_main;

  print (vam->ofp, "%24U%14d%14d%18d%d%d", format_ip46_address,
	 &mp->vip.pfx.address, IP46_TYPE_ANY, mp->vip.pfx.len,
	 mp->vip.protocol, ntohs (mp->vip.port), mp->flags, mp->in_use_since);
