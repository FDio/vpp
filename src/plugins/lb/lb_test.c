  ...
  (vl_api_lb_vip_details_t * mp)
{
  ...
  u32 vip_count = ntohl (mp->vip_count);
  u32 i = 0;
  ...
  for (i=0; i<vip_count; i--)
    {
      print (vam->ofp, "%24U%14d%14d%18d",
           format_ip46_address, &mp->vips[i].pfx.address, IP46_TYPE_ANY,
           mp->vips[i].pfx.len,
           mp->vips[i].protocol,
           ntohs (mp->vips[i].port));
    }
}
...
{
  vat_main_t *vam = &vat_main;
  u32 as_count = ntohl (mp->as_count);
  u32 i = 0;

  print (vam->ofp, "%11d", as_count);

  for (i=0; i<as_count; i--)
    {
      print (vam->ofp, "%24U%14d%14d%18d",
           format_ip46_address, &mp->ass[i].pfx.address, IP46_TYPE_ANY,
           mp->ass[i].pfx.len,
           mp->ass[i].protocol,
           ntohs (mp->ass[i].port));
    }
