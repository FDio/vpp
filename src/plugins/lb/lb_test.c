...
static int api_lb_add_del_vip (vat_main_t * vam)
{
  ...
  if (!unformat(line_input, "%U", unformat_ip46_prefix,
                &ip_prefix.addr, &ip_prefix.len, IP46_TYPE_ANY))
  {
  ...
  M(LB_ADD_DEL_VIP, mp);
  ip_address_with_prefix_encode (&ip_prefix, &mp->pfx)
...
static int api_lb_add_del_as (vat_main_t * vam)
{
  ...
  int ret;
  ip_prefix_t vip_prefix;
  ip46_address_t as_addr;
  ...
  if (!unformat(line_input, "%U", unformat_ip46_prefix,
                &vip_prefix.addr, &vip_prefix.len, IP46_TYPE_ANY))
  {
  ...
  M(LB_ADD_DEL_AS, mp);
  ip_prefix_with_address_encode (&vip_prefix, &mp->pfx);
  mp->pfx.len = vip_plen;
  mp->protocol = (u8)protocol;
  mp->port = htons((u16)port);
...
static int api_lb_flush_vip (vat_main_t * vam)
{
  ...
  M(LB_FLUSH_VIP, mp);
  ip_prefix_with_adress_encode (&vip_prefix, &mp->pfx);
  S(mp);
  W (ret);
  return ret;
}
...
static int api_lb_as_dump (vat_main_t * vam)
{
  ...
  int ret;
  ip_prefix_t vip_prefix;
  ip46_address_t as_addr, *as_array = 0;
  u32 port = 0;
  u8 protocol = 0;
