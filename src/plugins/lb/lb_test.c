...
_(lb_get_vip_list_reply)         \
_(lb_get_as_list_reply)
...
#define foreach_vpe_api_reply_msg                               \
  ...
  _(LB_GET_VIP_LIST_REPLY, lb_get_vip_list_reply)               \
  _(LB_GET_AS_LIST_REPLY, lb_get_as_list_reply)
...
static int api_lb_get_vip_list (vat_main_t * vam)
{
  vl_api_lb_get_vip_list_t *mp;
  int ret;

  M(LB_GET_VIP_LIST, mp);
...
static int api_lb_get_as_list (vat_main_t * vam)
{

  unformat_input_t *line_input = vam->input;
  vl_api_lb_get_as_list_t *mp;
  int ret;
  ip46_address_t vip_prefix, as_addr;
  u8 vip_plen;
  ip46_address_t *as_array = 0;
  ...
  M(LB_GET_AS_LIST, mp);
  clib_memcpy (mp->ip_prefix, &vip_prefix, sizeof (vip_prefix));
  mp->prefix_length = vip_plen;
  mp->protocol = (u8)protocol;
  mp->port = htons((u16)port);
...
_(lb_add_del_as, "<vip-prefix> [protocol (tcp|udp) port <n>] "  \
                 "[<address>] [del] [flush]")              \
_(lb_get_vip_list, "")          \
_(lb_get_as_list, "<vip-prefix> [protocol (tcp|udp) port <n>]")
