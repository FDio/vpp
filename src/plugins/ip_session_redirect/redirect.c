...
ip_session_redirect_find (ip_session_redirect_main_t *im,
			  const u32 session_index, const u32 table_index,
			  const u8 *match, ip_session_redirect_t **ipr)
{
  ...
  if (~0 != session_index)
    {
      if (pool_is_free_index (im->pool, session_index))
	return VNET_API_ERROR_INVALID_VALUE;
      *ipr = pool_elt_at_index (im->pool, session_index);
      return 0;
    }
...
static int
ip_session_redirect_del__ (ip_session_redirect_t *ipr)
{
  vnet_classify_main_t *cm = &vnet_classify_main;
  int rv;

  rv = vnet_classify_add_del_session (
    cm, ipr->table_index, ipr->match, 0 /* hit_next_index */,
...
int
ip_session_redirect_add (vlib_main_t *vm, u32 *session_index,
			 const u32 table_index, const u8 *match,
			 const fib_route_path_t *rpaths, const int is_punt)
{
  ip_session_redirect_main_t *im = &ip_session_redirect_main;
  ...
  rv = ip_session_redirect_find (im, *session_index, table_index, match, &ipr);
  switch (rv)
    {
    case 0:
      ...
      if (~0 != *session_index)
	{
	  /* when updating by session index, match may change */
	  ip_session_redirect_del__ (ipr);
...
int
ip_session_redirect_del (vlib_main_t *vm, const u32 session_index,
			 const u32 table_index, const u8 *match)
{
  ip_session_redirect_main_t *im = &ip_session_redirect_main;
  ip_session_redirect_t *ipr;
  ...
  rv = ip_session_redirect_del__ (ipr);
  if (rv)
    return rv;
			 ...
			 vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	is_punt = 1;
      else if (unformat (line_input, "session-index %u", &session_index))
	;
  ...
  if (is_add)
      ...
      if (0 == rpaths)
	{
      ...
      rv = ip_session_redirect_add (vm, &session_index, table_index, match,
				    rpaths, is_punt);
    }
  else
    {
      rv = ip_session_redirect_del (vm, session_index, table_index, match);
    }

  if (rv)
    error = clib_error_create ("failed with error %d", rv);
  else
    vlib_cli_output (vm, "session_index %u", session_index);
