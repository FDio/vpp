...
int
ip_session_redirect_add (vlib_main_t *vm, const u32 sw_if_index,
			 const u32 table_index, const u8 *match,
			 const fib_route_path_t *rpaths, int is_punt)
{
  ip_session_redirect_main_t *im = &ip_session_redirect_main;
  const vnet_classify_main_t *cm = &vnet_classify_main;
  const vnet_classify_table_t *table;
  ...
  switch (payload_type)
    ...
    case FIB_FORW_CHAIN_TYPE_UNICAST_IP4:
      type = is_punt ? IP_SESSION_REDIRECT_PUNT4 : IP_SESSION_REDIRECT_IP4;
      break;
    case FIB_FORW_CHAIN_TYPE_UNICAST_IP6:
      type = is_punt ? IP_SESSION_REDIRECT_PUNT6 : IP_SESSION_REDIRECT_IP6;
      break;
    default:
  ...
  switch (type)
    {
    case IP_SESSION_REDIRECT_IP4:
      pname = "ip4-inacl";
      ip4_table_index = table_index;
      break;
    case IP_SESSION_REDIRECT_IP6:
      pname = "ip6-inacl";
      ip6_table_index = table_index;
      break;
    case IP_SESSION_REDIRECT_PUNT4:
      pname = "ip4-punt-acl";
      ip4_punt_table_index = table_index;
      break;
    case IP_SESSION_REDIRECT_PUNT6:
      pname = "ip6-punt-acl";
      ip6_punt_table_index = table_index;
      break;
    }

  if (0 == im->refcount[type])
    {
      rv = vnet_set_in_out_acl_intfc (
	vm, sw_if_index, ip4_table_index, ip6_table_index,
	~0 /* l2_table_index */, ip4_punt_table_index, ip6_punt_table_index,
	1 /* is_add */, 0 /* is_output */);
      if (rv)
	return rv;
    }
			 ...
			 const u8 *match)
{
  ...
  if (1 == im->refcount[ipr->type])
	...
	{
	case IP_SESSION_REDIRECT_IP4:
	  ip4_table_index = ipr->table_index;
	  break;
	case IP_SESSION_REDIRECT_IP6:
	  ip6_table_index = ipr->table_index;
	  break;
	case IP_SESSION_REDIRECT_PUNT4:
	  ip4_punt_table_index = ipr->table_index;
	  break;
	case IP_SESSION_REDIRECT_PUNT6:
	  ip6_punt_table_index = ipr->table_index;
	  break;
	}

      rv = vnet_set_in_out_acl_intfc (
	vm, ipr->sw_if_index, ip4_table_index, ip6_table_index,
	~0 /* l2_table_index */, ip4_punt_table_index, ip6_punt_table_index,
	1 /* is_add */, 0 /* is_output */);
      if (rv)
	return rv;
...
static u8 *
format_ip_session_redirect_type (u8 *s, va_list *args)
{
  ...
  switch (ipr->type)
    ...
    case IP_SESSION_REDIRECT_IP6:
      return format (s, "ip6");
    case IP_SESSION_REDIRECT_PUNT4:
      return format (s, "punt4");
    case IP_SESSION_REDIRECT_PUNT6:
      return format (s, "punt6");
    }
...
format_ip_session_redirect (u8 *s, va_list *args)
{
  ...
  index_t ipri = ipr - im->pool;
  s =
    format (s, "[%u] %U from %U table %d key %U\n", ipri,
	    format_ip_session_redirect_type, ipr, format_vnet_sw_if_index_name,
	    vnet_get_main (), ipr->sw_if_index, ipr->table_index,
			 ...
			 vlib_cli_command_t *cmd)
{
  ...
  u8 *match = 0;
  u32 table_index = ~0, sw_if_index = ~0;
  int is_add = 1;
  int is_punt = 0;
  int rv;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (line_input, "punt"))
	{
      ...
      else if (unformat (line_input, "from %U", unformat_vnet_sw_interface,
  ...
  if (~0 == table_index || 0 == match)
    {
      error = clib_error_create ("missing parameters");
      goto out;
    }

  if (is_add)
    {
      if (0 == rpaths || ~0 == sw_if_index)
	{
	  error = clib_error_create ("add: missing bad parameters");
	  goto out;
	}
      rv = ip_session_redirect_add (vm, sw_if_index, table_index, match,
				    rpaths, is_punt);
    }
  else
    {
...
VLIB_CLI_COMMAND (ip_session_redirect_command, static) = {
  ...
  .short_help = "ip session redirect [add] [punt|from <interface>] table "
		"<index> match <match> via <path> | del table <index> "
		"match <match>"
};
