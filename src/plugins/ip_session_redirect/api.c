...
vl_api_ip_session_redirect_add_t_handler (vl_api_ip_session_redirect_add_t *mp)
{
  ...
  session_index = ntohl (mp->session_index);
  vec_add (match, mp->match, mp->match_len);
  rv = ip_session_redirect_add (vm, &session_index, ntohl (mp->table_index),
				match, rpaths, mp->is_punt);
  vec_free (match);

err:
  vec_free (rpaths);
  REPLY_MACRO2 (VL_API_IP_SESSION_REDIRECT_ADD_REPLY,
		({ rmp->session_index = ntohl (session_index); }));
}
...
vl_api_ip_session_redirect_del_t_handler (vl_api_ip_session_redirect_del_t *mp)
{
  ...
  vec_add (match, mp->match, mp->match_len);
  rv = ip_session_redirect_del (vm, ntohl (mp->session_index),
				ntohl (mp->table_index), match);
  vec_free (match);
