...
static void
vl_api_ip_session_redirect_add_reply_t_handler (
  vl_api_ip_session_redirect_add_reply_t *mp)
{
  ...
  if (retval == 0)
    {
      fformat (vam->ofp, "created ip session redirect with index %d\n",
	       ntohl (mp->index));
    }
...
api_ip_session_redirect_del (vat_main_t *vam)
{
  ...
  while (unformat_check_input (vam->input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (vam->input, "index %d", &index))
	;
      else
	{
	  clib_warning ("unknown input '%U'", format_unformat_error,
  ...
  M (IP_SESSION_REDIRECT_DEL, mp);

  mp->index = htonl (index);
...
static int
api_ip_session_redirect_punt_add_del (vat_main_t *vam)
{
  vl_api_ip_session_redirect_punt_add_del_t *mp;
  u32 table_index = ~0;
  u32 sw_if_index = ~0;
  int is_add = 1;
  int is_ip4 = 1;
  ...
  while (unformat_check_input (vam->input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (vam->input, "add"))
	is_add = 1;
      else if (unformat (vam->input, "del"))
	is_add = 0;
      else if (unformat (vam->input, "ip4"))
	is_ip4 = 1;
      else if (unformat (vam->input, "ip6"))
	is_ip4 = 0;
      else if (unformat (vam->input, "table %d", &table_index))
	;
      else if (unformat (vam->input, "%U", unformat_vnet_sw_interface,
  ...
  ip_session_redirect_punt_add_del (vlib_get_main (), sw_if_index, table_index,
				    is_add, is_ip4);

  M (IP_SESSION_REDIRECT_PUNT_ADD_DEL, mp);
