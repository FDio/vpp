...
api_ip_session_redirect_add (vat_main_t *vam)
{
  ...
  while (unformat_check_input (vam->input) != UNFORMAT_END_OF_INPUT)
	...
	is_punt = 1;
      else if (unformat (vam->input, "session-index %u", &session_index))
	;
  ...
  mp->session_index = htonl (session_index);
  ...
  vl_api_ip_session_redirect_add_reply_t *mp)
{
  ...
  if (retval == 0)
    {
      fformat (vam->ofp, "ip session redirect added with session %u\n",
...
api_ip_session_redirect_del (vat_main_t *vam)
{
  ...
  while (unformat_check_input (vam->input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (vam->input, "session-index %u", &session_index))
	;
      else if (unformat (vam->input, "table %u", &table_index))
	;
      else if (unformat (vam->input, "match %U", unformat_classify_match, cm,
