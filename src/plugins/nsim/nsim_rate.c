  ...
  switch (m->type)
    ...
    case NSIM_RATE_MARKOV:
      s = format (s,
		  ...
		  m->markov.good_dwell, format_rate_bps, m->bad_ser,
		  packet_size, format_dwell_secs, m->markov.bad_dwell,
		  m->markov.state ? "bad" : "good");
      break;
    default:
      s = format (s, "none");
      break;
...
void
nsim_rate_model_markov (nsim_rate_model_t *m, f64 good_ser, f64 bad_ser,
