...
typedef struct nsim_loss_model_
{
  ...
  union
  {
     ...
     * packet for dur seconds (wall-clock), then clears. `until` is live state:
     * the time the current burst ends (0 => not bursting). Duration-based (not
     * packet-count) so a retransmit sent ~1 RTT later survives. */
    struct
    {
      f64 prob;
      f64 dur;
     ...
     * everything for `dur` seconds (once), then disable. `start` is stamped on
     * the first packet; `done` latches; `count` tallies dropped packets. */
    struct
    {
      f64 at;
      f64 dur;
...
nsim_loss_target_seq_match (nsim_loss_model_t *m, vlib_buffer_t *b)
{
  ...
  if (!(seq_leq (seq, m->target_seq.target) &&
	seq_lt (m->target_seq.target, end)))
    return 0;
...
nsim_loss_apply (nsim_loss_model_t *m, u32 *seed, f64 now, vlib_buffer_t **b,
		 u8 *action, u32 n)
{
  u32 i;
  ...
  switch (m->type)
      ...
      if (!m->once.done)
	{
	  ...
	  if (elapsed >= m->once.at && elapsed < m->once.at + m->once.dur)
	    for (i = 0; i < n; i++)
	      {
		action[i] |= NSIM_ACTION_DROP;
		m->once.count++;
	      }
	  else if (elapsed >= m->once.at + m->once.dur)
	    m->once.done = 1;
	}
      break;
...
/* Control-plane helpers (nsim_loss.c). */
format_function_t format_nsim_loss_model;
void nsim_loss_model_uniform (nsim_loss_model_t *m, f64 fraction);
void nsim_loss_model_burst (nsim_loss_model_t *m, f64 prob, f64 dur);
void nsim_loss_model_once (nsim_loss_model_t *m, f64 at, f64 dur);
