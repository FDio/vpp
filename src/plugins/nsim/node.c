...
nsim_trace_buffer (vlib_main_t * vm, vlib_node_runtime_t * node,
		   vlib_buffer_t * b, nsim_nodex_ctx_t * ctx, u32 ctx_index,
		   u32 is_drop)
{
...
trace:
  ...
  if (PREDICT_FALSE (is_trace))
    nsim_trace_buffer (vm, node, b, ctx, ctx_index, 0);
}
	     ...
	     int is_cross_connect)
{
  ...
  u32 drops[VLIB_FRAME_SIZE], reorders[VLIB_FRAME_SIZE];
...
slow_path:
  ...
  while (n_left_from > 0)
      ...
      else
	{
	  ...
	  if (PREDICT_FALSE (is_trace))
	    nsim_trace_buffer (vm, node, b[0], &ctx, 0, 1);
	}
  ...
  if (PREDICT_FALSE (ctx.reord_bufs > reorders))
    {
      u32 n_reordered = ctx.reord_bufs - reorders;
