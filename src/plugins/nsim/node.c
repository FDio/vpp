...
always_inline void
nsim_update_drop_and_reorder (nsim_main_t *nsm, vlib_buffer_t **b,
...
always_inline void
nsim_update_drop_and_reorder_x4 (nsim_main_t * nsm,
				 vlib_buffer_t ** b,
				 nsim_nodex_ctx_t *ctx)
{
...
static void
nsim_trace_buffer (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b,
                   nsim_nodex_ctx_t *ctx, u32 ctx_index, u32 is_drop,
                   u32 tx_sw_if_index)
{
  if (b->flags & VLIB_BUFFER_IS_TRACED)
    {
      ...
      t->is_drop = is_drop;
      t->is_lost = ctx->is_drop[ctx_index];
      t->tx_sw_if_index = tx_sw_if_index;
    }
}
		      ...
		      u8 is_cross_connect, u8 is_trace)
{
  ...
  if (PREDICT_FALSE (is_trace))
    {
      u32 sw_if_index = ctx->is_drop[ctx_index] == 0 ? ep->tx_sw_if_index : 0;
      nsim_trace_buffer (vm, node, b, ctx, ctx_index, 0, sw_if_index);
    }
}
	     ...
	     int is_cross_connect)
{
  ...
  while (n_left_from >= 8)
	...
	goto slow_path;
      ...
      nsim_update_drop_and_reorder_x4 (nsm, b, &ctx);
...
slow_path:
  ...
  while (n_left_from > 0)
	...
	{
	  nsim_update_drop_and_reorder (nsm, b, &ctx);
      ...
      else			/* out of wheel space, drop pkt */
	{
	  ...
	  if (PREDICT_FALSE (is_trace))
	    nsim_trace_buffer (vm, node, b[0], &ctx, 0, 1, ~0);
	}
