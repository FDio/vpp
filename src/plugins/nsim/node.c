...
nsim_update_drop_and_reorder (vlib_node_runtime_t * node, nsim_main_t * nsm,
			      ...
			      u8 * is_reord)
{
  if (PREDICT_FALSE (nsm->drop_fraction != 0.0 || nsm->reorder_fraction != 0.0))
    {
  ...
  if (PREDICT_FALSE (nsm->reorder_fraction != 0.0))
    {
      if (!is_drop[0] && rnd[0] <= nsm->reorder_fraction)
	is_reord[0] = 1;
    }
}
				 ...
				 u8 * is_reord)
{
  ...
  if (PREDICT_FALSE (nsm->reorder_fraction != 0.0))
      ...
      if (!is_drop[0] && rnd[0] <= nsm->reorder_fraction)
	{
      ...
      if (!is_drop[1] && rnd[1] <= nsm->reorder_fraction)
	{
	  is_reord[1] = 1;
	}
      if (!is_drop[2] && rnd[2] <= nsm->reorder_fraction)
	{
	  is_reord[2] = 1;
	}
      if (!is_drop[3] && rnd[3] <= nsm->reorder_fraction)
	{
	  is_reord[3] = 1;
	}
    }
