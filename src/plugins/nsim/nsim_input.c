...
/* Drain expired entries (tx_time <= now) from the head of one wheel and send
 * them. Each wheel is kept monotonic in tx_time on insert, so a stop-at-head
 * FIFO drain is correct for it. Returns packets sent. */
always_inline uword
