 ...
 *
 * The queued (bufferbloat) model serializes packets at a single fixed
 * bottleneck rate. A rate model modulates that rate over time so the
 * bottleneck can speed up and slow down while a transfer runs -- the bandwidth
 * analog of the correlated burst-loss model in nsim_loss.h.
 *
 * NSIM_RATE_MARKOV is a two-state (good/bad) continuous-time Markov model, the
 * standard tractable abstraction for a wifi/wireless link: the link mostly runs
 * at the configured rate (good state) but intermittently collapses to a low
 * rate for a correlated interval (bad state) -- rate adaptation dropping to a
 * low MCS, a burst of L2 retransmits, contention, a scan/roam, etc. Dwell times
 * in each state are exponentially distributed, so stalls are memoryless and of
 * variable length. A deep buffer in front of such a link turns a bad-state
 * stall into a transient RTT spike (buffered bytes / low rate), which is how a
 * one-off multi-second latency excursion arises on an otherwise fast path.
 *
 * A rate model only affects the queued model (buffer > 0); with the fixed-delay
 * model there is no serialization step to modulate.
 *
 * The per-packet applier is header-inline so it specializes into each CPU-march
 * variant of the nsim node; control-plane construction/reset/format live in
 * nsim_rate.c.
 */
...
nsim_rate_serialization_time (nsim_rate_model_t *m, u32 *seed, f64 now)
{
  if (PREDICT_FALSE (m->markov.next_transition == 0.0))
    m->markov.next_transition =
      now + nsim_rate_dwell (seed, m->markov.good_dwell);
  ...
  if (now >= m->markov.next_transition)
    {
      m->markov.state ^= 1;
      m->markov.next_transition =
	now + nsim_rate_dwell (seed, m->markov.state ? m->markov.bad_dwell :
						       m->markov.good_dwell);
    }
...
void nsim_rate_model_markov (nsim_rate_model_t *m, f64 good_ser, f64 bad_ser,
			     f64 good_dwell, f64 bad_dwell);
