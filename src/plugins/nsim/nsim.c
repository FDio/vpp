  ...
  for (; i < num_workers + 1; i++)
    {
      ...
      /* Side wheel for late-reordered packets, same geometry. nsim_wheel_alloc
       * sets mmap_size (shared by both since geometry matches). */
      if (reorder_fraction > 0.0)
		     ...
		     unformat_input_t * input, vlib_cli_command_t * cmd)
{
   ...
   * explicit reorder-delay was given (a reordered packet can slip up to ~1 base
   * delay late). */
