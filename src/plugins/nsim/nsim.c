...
format_nsim_config (u8 * s, va_list * args)
{
  ...
  if (nsm->rate.type != NSIM_RATE_NONE)
    s = format (s, " rate model: %U\n", format_nsim_rate_model, &nsm->rate,
		nsm->packet_size);
		     ...
		     unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	drop_seq_set = 1;
      else if (unformat (input, "rate-stall bandwidth %U every %U for %U",
			 unformat_bandwidth, &rate_stall_bw, unformat_delay,
			 &rate_good_dwell, unformat_delay, &rate_stall_dwell))
	;
  ...
  if (wifi_preset || rate_stall_bw > 0.0)
    {
      ...
      if (nsm->buffer_time <= 0.0)
	return clib_error_return (
	  0, "rate model requires the queued model; set 'buffer <time>'");
      if (stall_bw >= bandwidth)
	return clib_error_return (
	  0, "stall bandwidth %.0f must be below link bandwidth %.0f", stall_bw,
	  bandwidth);

      nsim_rate_model_markov (&nsm->rate, nsm->serialization_time,
			      (f64) (packet_size * 8) / stall_bw, good_dwell,
			      stall_dwell);
    }
