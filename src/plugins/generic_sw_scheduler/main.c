  ...
  while (n_left)
    {
      ...
      clib_memcpy_fast (d->nexts + d->offset, nexts, n_to_deq * sizeof (u16));
      n_left -= n_to_deq;
		   ...
		   u32 max_n_buffers)
{
  ...
  /* TODO: fix working and consumer queue index loop problem, currently has
   * bugs */
  for (q = ptd->working_queues /*+ ptd->last_worker_queue_index*/;
       q < vec_end (ptd->working_queues); q++)
    {
      gen_sw_scheduler_data_t *d;
      u32 i;
      u32 tail = q[0]->tail;
	   ...
	   i < head && (pq->head - pq->tail != GEN_SW_SCHED_QUEUE_SIZE); i++)
	{
	  ...
	  if (clib_atomic_bool_cmp_and_swap (&d->flag, GEN_SW_SCHED_FLAG_READY,
					     GEN_SW_SCHED_FLAG_WIP))
	    {
	      ...
	      /*
			    ptd->last_worker_queue_index = q -
		 ptd->working_queues + 1; if (PREDICT_FALSE
		 (ptd->last_worker_queue_index >= vec_len
		 (ptd->working_queues))) ptd->last_worker_queue_index = 0;
	      */
	      pq->head++;
		     ...
		     u32 max_n_buffers)
{
  ...
  for (cq = ptd->consumer_queues /* + ptd->last_consumer_queue_index*/;
      ...
      if (tail != q->tail)
	{
      ...
      /*
	    ptd->last_consumer_queue_index = q - ptd->consumer_queues + 1;
	    if (PREDICT_FALSE (ptd->last_consumer_queue_index >=
			       vec_len (ptd->consumer_queues)))
	      ptd->last_consumer_queue_index = 0;
      */
    }
