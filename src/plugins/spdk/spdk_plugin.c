...
spdk_plugin_configure_poll_lcores (void)
{
  ...
  const struct spdk_cpuset *core_mask = spdk_app_get_core_mask ();
...
spdk_poll_input_fn (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (clib_atomic_load_acq_n (&sm->polling) && ti < vec_len (sm->thread_lcores))
      ...
      vec_foreach (lcore, sm->thread_lcores[ti])
	{
	  ...
	  rc = spdk_app_poll_reactor (*lcore);
	  spdk_vpp_env_set_current_core (SPDK_ENV_LCORE_ID_ANY);

	  if (PREDICT_FALSE (rc && rc != -EAGAIN && rc != -ESHUTDOWN))
	    sm->last_rc = rc;
	}
    }

  if (ti == 0 && sm->running && spdk_app_stop_complete ())
...
spdk_plugin_start_rpc (void *arg)
{
  ...
  opts.json_config_file = sm->json_config;
  opts.reactor_mask = sm->reactor_mask;
  opts.rpc_addr = sm->rpc_addr;
  opts.no_pci = sm->no_pci;
  opts.disable_signal_handlers = true;
  opts.disable_cpumask_locks = true;
  opts.main_core = ctx->spdk_main_lcore;

  /* Initialize SPDK on the VPP dataplane thread that owns its main lcore.
   * The VPP environment does not create or pin any additional reactor
   * pthreads; the input node subsequently polls that reactor in place. */
  spdk_vpp_env_set_current_core (ctx->spdk_main_lcore);
  rc = spdk_app_start_external (&opts, spdk_plugin_started, 0);
  spdk_vpp_env_set_current_core (SPDK_ENV_LCORE_ID_ANY);
  if (rc == 0)
    {
      pthread_mutex_lock (&sm->lock);
