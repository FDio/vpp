...
_(DPI_APP_IP_RULE_ADD_DEL, dpi_app_ip_rule_add_del)         \
_(DPI_APP_L7_RULE_ADD_DEL, dpi_app_l7_rule_add_del)         \
_(DPI_COMPILE, dpi_compile)
...
static void
vl_api_dpi_compile_t_handler (vl_api_dpi_compile_t * mp)
{
  vl_api_dpi_compile_reply_t *rmp;
  int rv = 0;
  dpi_main_t *dm = &dpi_main;
  hs_compile_error_t *compile_err;

  dm->mode = ntohl (mp->mode);
  dm->flags = dpi_parse_flagstr ((char *) mp->flags);
  dm->pattern = (char *) mp->pattern;
  dm->action = ntohl (mp->action);

  rv = hs_compile (dm->pattern, dm->flags, dm->mode,
		   NULL, &dm->db_block, &compile_err);
  if (rv != HS_SUCCESS)
    {
  ...
  rv = hs_alloc_scratch (dm->db_block, &dm->scratch);
  if (rv != HS_SUCCESS)
    {
      hs_free_database (dm->db_block);
    }
...
/* API message handler */
static void vl_api_dpi_app_ip_rule_add_del_t_handler
  (vl_api_dpi_app_ip_rule_add_del_t * mp)
{
  vl_api_dpi_app_ip_rule_add_del_reply_t *rmp = NULL;
  ...
  REPLY_MACRO (VL_API_DPI_APP_IP_RULE_ADD_DEL_REPLY);
}

/* API message handler */
static void vl_api_dpi_app_l7_rule_add_del_t_handler
  (vl_api_dpi_app_l7_rule_add_del_t * mp)
{
  vl_api_dpi_app_l7_rule_add_del_reply_t *rmp = NULL;
  dpi_rule_args_t args = { };
  dpi_main_t *dm = &dpi_main;
  int rv = 0;
  ...
  REPLY_MACRO (VL_API_DPI_APP_L7_RULE_ADD_DEL_REPLY);
}
