...
dpi_ssl_add_protocol (dpi_flow_info_t * flow, u32 protocol)
{
  if ((protocol != DPI_PROTOCOL_SSL)
      && (protocol != DPI_PROTOCOL_SSL_NO_CERT))
    {
      dpi_set_detected_protocol (flow, protocol, DPI_PROTOCOL_UNKNOWN);
    }
  else
    {
...
int
check_punycode_string (char *buffer, int len)
{
  int i = 0;
...
dpi_ssl_get_cert (u8 * payload, u32 payload_len,
		  dpi_flow_info_t * flow, char *buffer, int buffer_len)
{
  u16 total_len;
  u8 server_len;
  u8 handshake_type;
  ...
  if (payload[0] == handshake)
	...
	total_len = payload_len;
      ...
      if (total_len >= sizeof (ssl_header) && (handshake_type == certificate))
	{
	  u32 id_name_num = 0;
	  ...
	  for (i = 9; i < payload_len - 3; i++)
	    {
	      ...
	      if ((payload[i] == 0x55)
		  && (payload[i + 1] == 0x04) && (payload[i + 2] == 0x03))
		{
...
int
dpi_ssl_try_server_cert (u8 * payload, u32 payload_len,
			 dpi_flow_info_t * flow)
{
  ...
  if ((payload_len > 9) && (payload[0] == 0x16))
    {
      ...
      cert[0] = '\0';
      rv = dpi_ssl_get_cert (payload, payload_len, flow, cert, sizeof (cert));
  ...
  if ((payload_len > (sizeof (ssl_header) + sizeof (Handshake_header)))
	  ...
	  || (flow->detected_protocol[0] == DPI_PROTOCOL_SSL))
	{
	  ...
	  if (rv > 0)
		...
		{
		  dpi_set_detected_protocol (flow,
					     host_protocol,
					     dpi_ssl_refine_master_protocol
					     (flow, DPI_PROTOCOL_SSL));
		  return rv;
		}
	    }
