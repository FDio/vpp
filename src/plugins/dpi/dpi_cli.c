...
static clib_error_t *
dpi_compile_command_fn (vlib_main_t * vm,
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	{
	  hsm->flags = dpi_parse_flagstr (flagstr);
	}
      else if (unformat (line_input, "patterns %s", &hsm->pattern))
	;
      else if (unformat (line_input, "action permit"))
	hsm->action = DPI_ACTION_PERMIT;
      else if (unformat (line_input, "action drop"))
	hsm->action = DPI_ACTION_DROP;
      else
	{
 ...
 * @cliexcmd{}
 ?*/
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (dpi_compile_command, static) = {
  .path = "dpi compile",
  .short_help = "dpi compile mode [block|stream] flags [imsHV8W\r]"
                " patterns <patterns> action [permit|drop]",
  .function = dpi_compile_command_fn,
};
/* *INDENT-ON* */
