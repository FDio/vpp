			    ...
			    vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "add name %_%v%_", &name))
	{
	  add_flag = 1;
	  break;
	}
      if (unformat (line_input, "update name %_%v%_", &name))
	{
	  add_flag = 0;
	  break;
	}
  ...
  ASSERT (!pool_is_free_index (dm->dpi_apps, p[0]));

  adr = pool_elt_at_index (dm->dpi_adrs, p[0]);
  if (adr == NULL)
    {
...
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (dpi_adf_app_add_command, static) =
{
  .path = "dpi adr app",
  .short_help = "dpi adr app <add|update> name <app name>",
