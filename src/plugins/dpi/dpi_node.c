...
static int
dpi_match(unsigned int id, unsigned long long from, unsigned long long to,
          unsigned int flags, void *hs_ctx)
{
    u32 * is_match = (u32 *)hs_ctx;
    *is_match = 1;
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
        {
           ...
           */
          if (hs_scan(hsm->db_block, p0, len0, 0, hsm->scratch, dpi_match,
                      &is_match) != HS_SUCCESS) {
          ...
          if (is_match && (hsm->action == DPI_ACTION_DROP))
            {
              next0 = DPI_SCAN_NEXT_DROP;
              error0 = DPI_ERROR_SCAN_FAIL;
              goto exit0;
