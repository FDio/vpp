...
static u32
geneve_packet_matches_global_filter (geneve_pcapng_main_t *gpm,
                       const u8 *outer_hdr, u32 outer_len,
                       const u8 *inner_hdr, u32 inner_len,
                       const geneve_header_t *geneve_hdr,
                       u32 geneve_header_len)
...
/* IPv4 */
	  ...
	  if (destination_capture_index != ~0) {
	    n_matched += 1;
	      ...
	      geneve_output_t *output = &vec_elt(gpm->outputs, destination_capture_index);
	      void *output_ctx = gpm->worker_output_ctx[worker_index][destination_capture_index];
              ...
              output->write_pcapng_epb (output, output_ctx, (sw_if_index0 << 1) | is_output, 
                                         timestamp, orig_len, 
                                         packet_copy, offset);
