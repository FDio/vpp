/*
 * geneve_pcapng_cli.c - VPP plugin for GENEVE PCAPNG CLI commands
 */
...
extern void set_write_pcapng(geneve_output_t *output); // FIXME: to be refactored out to use inlines


void *init_worker_context(geneve_output_t *output, u32 worker_index)
 {
...
pcap_add_capture_destination(char *name, pcapng_destination_type_t cap_type, u8 *path_or_url)
{
    ...
    geneve_output_t *new_destination;
    ...
    case PCAPNG_DEST_HTTP:
        geneve_pcapng_ensure_session_manager();
...
static clib_error_t *
geneve_pcapng_destination_add_command_fn(vlib_main_t * vm,
...
static clib_error_t *
show_geneve_pcapng_destination_command_fn(vlib_main_t * vm,
                                        unformat_input_t * input,
                                        vlib_cli_command_t * cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
geneve_pcapng_destination_del_command_fn(vlib_main_t * vm,
                                       unformat_input_t * input,
                                       vlib_cli_command_t * cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
...
VLIB_CLI_COMMAND(geneve_pcapng_destination_add_command, static) = {
    .path = "geneve pcapng destination add",
    .short_help = "geneve pcapng destination add name <name> {file <path> | gzip <path> | igzip <path> | http <url>}",
    .function = geneve_pcapng_destination_add_command_fn,
};

VLIB_CLI_COMMAND(show_geneve_pcapng_destination_command, static) = {
    .path = "show geneve pcapng destination",
    .short_help = "show geneve pcapng destination [name <name>]",
    .function = show_geneve_pcapng_destination_command_fn,
};

VLIB_CLI_COMMAND(geneve_pcapng_destination_del_command, static) = {
    .path = "geneve pcapng destination del",
    .short_help = "geneve pcapng destination del name <name>",
    .function = geneve_pcapng_destination_del_command_fn,
};
