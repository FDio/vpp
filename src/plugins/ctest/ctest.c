  ...
  if (d)
      ...
      while ((dir = readdir (d)) != NULL)
	{
	  ...
	  if (stat ((void *) filename, &st))
	      ...
	      if (strstr (dir->d_name, "vpp_api_trace."))
		{
	      ...
	      if (strstr (dir->d_name, "_in.pcap"))
		{
		  file_type = TEST_FILE_IN_PCAP;
		}
	      if (strstr (dir->d_name, "_out.pcap"))
		{
	...
	switch (afile->file_type)
	  ...
	  case TEST_FILE_API_DUMP:
	    {
	      /* FIXME: enable the captures on interfaces corresponding to outbound "to compare with" captures, and replay those captures which are inbound. run comparison. bail if it fails. If it succeeds, continue */
	  ...
	  case TEST_FILE_IN_PCAP:
	    /* FIXME: push the file to the vector of pending inbound captures */
	  case TEST_FILE_OUT_PCAP:
	    /* FIXME: push the file to the vector of pending outbound ("to-compare-with") captures */
	  default:
