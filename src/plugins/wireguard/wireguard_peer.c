...
wg_peer_if_adj_change (index_t peeri, void *data)
{
  ...
  if (ip46_address_is_equal (&peer->dst.addr, &adj->sub_type.nbr.next_hop))
    {
      peer->adj_index = *ctx;
	      ...
	      const fib_prefix_t * allowed_ips, u32 wg_sw_if_index)
{
  ...
  adj_nbr_walk_nh (
    wg_sw_if_index,
    fib_proto_from_ip46 (ip46_address_get_type (&peer->dst.addr)),
    &peer->dst.addr, wg_peer_adj_walk, &perri);
