...
wg_peer_enable (vlib_main_t *vm, wg_peer_t *peer)
{
  ...
  noise_remote_init (vm, &peer->remote, peeri, preshared_key, public_key, wg_if->local_idx);
	     ...
	     u16 persistent_keepalive, u32 *peer_index)
{
  ...
  noise_remote_init (vm, &peer->remote, peer - wg_peer_pool, preshared_key, public_key,
		     wg_if->local_idx);
