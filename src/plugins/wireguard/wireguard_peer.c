	      ...
	      const fib_prefix_t * allowed_ips, u32 wg_sw_if_index)
{
  ...
  wg_itf_t *wgi = wg_itf_find_by_sw_if_index (wg_sw_if_index);
  ...
  fib_node_index_t fib_entry_index =
    fib_table_lookup (fib_table_find (FIB_PROTOCOL_IP4, peer->table_id),
		      &pfx);
...
format_wg_peer (u8 * s, va_list * va)
{
  ...
  s = format (s, "[%d] key:%=45s endpoint:[%U, %u] %U keep-alive:%d",
	      peeri,
	      key_64,
