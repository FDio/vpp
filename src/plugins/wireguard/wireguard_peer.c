...
wg_peer_build_rewrite (const wg_peer_t * peer)
{
  ...
  hdr->udp.src_port = peer->src.port;
  hdr->udp.dst_port = peer->dst.port;
  hdr->udp.checksum = 0;
...
wg_peer_adj_stack (wg_peer_t * peer)
{
  ...
  wgi = wg_itf_get (wg_itf_find_by_sw_if_index (sw_if_index));
...
walk_rc_t
wg_peer_itf_admin_state_change (wg_itf_t * wgi, index_t peeri, void *data)
{
...
walk_rc_t
wg_peer_itf_table_change (wg_itf_t * wgi, index_t peeri, void *data)
{
	      ...
	      const fib_prefix_t * allowed_ips, u32 wg_sw_if_index)
{
  ...
  const wg_itf_t *wgi =
    wg_itf_get (wg_itf_find_by_sw_if_index (wg_sw_if_index));
  ...
  adj_nbr_midchain_update_rewrite (peer->adj_index,
				   NULL,
				   NULL,
				   ADJ_FLAG_MIDCHAIN_IP_STACK, peer->rewrite);
	     ...
	     u16 port, u16 persistent_keepalive, u32 * peer_index)
{
  ...
  wg_itf = wg_itf_get (wg_itf_find_by_sw_if_index (tun_sw_if_index));
  if (!wg_itf)
    return (VNET_API_ERROR_INVALID_SW_IF_INDEX);
  ...
  *peer_index = peer - wg_main.peers;
  wg_itf_peer_add (wg_itf, *peer_index);
...
wg_peer_remove (index_t peeri)
{
  ...
  wgi = wg_itf_get (wg_itf_find_by_sw_if_index (peer->wg_sw_if_index));
  wg_itf_peer_remove (wgi, peeri);
