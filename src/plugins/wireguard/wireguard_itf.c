	       ...
	       u16 port, const ip_address_t * src_ip, u32 * sw_if_indexp)
{
  ...
  *sw_if_indexp = (u32) ~ 0;
  ...
  vec_validate_init_empty (wg_itf_index_by_port, port, INDEX_INVALID);
  wg_itf_index_by_port[port] = wg_itf - wg_itf_pool;
...
wg_itf_delete (u32 sw_if_index)
{
  ...
  wg_itf_index_by_port[wg_itf->port] = INDEX_INVALID;
		   ...
		   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  rv = wg_itf_create (instance, private_key, port, &src_ip, &sw_if_index);
