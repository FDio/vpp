...
class TestWg(VppTestCase):
    ...
    def test_wg_set_remove_device(self):
        ...
        # Set device
        ret = self.vapi.wireguard_set_device(private_key=key.private_key, port=port)
    ...
    def test_wg_set_remove_peer(self):
        ...
        self.vapi.wireguard_set_device(private_key=device_key_pr.private_key,
                                port=port)

        # Check peer counter
        peers_count = self.vapi.wireguard_peers_count()
        self.assertEqual(peers_count.count, 0)

        # Create first peer
        self.vapi.cli("create ipip tunnel src " +
                      self.pg0.remote_ip4 +
                      " dst " +
                      self.pg1.remote_ip4)
        ...
        peer_key_pub = self.vapi.wireguard_pubkey(private_key=peer_key_pr.private_key)
