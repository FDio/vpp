			  ...
			  unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "src-port %d", &portSrc))
	;
      else
	{
			...
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  if (AF_IP6 == ip_addr_version (&ip) ||
    ...
    rv = wg_peer_set (wmp, public_key_64,
		      ip_addr_v4 (&ip),
		      ip_addr_v4 (&allowed_ip),
		      portDst, tun_sw_if_index, persistent_keepalive);
			   ...
			   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  rv = wg_peer_remove (wmp, public_key_64);
