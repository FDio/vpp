...
wg_chacha20poly1305_calc (vlib_main_t *vm, u8 *src, u32 src_len, u8 *dst,
			  ...
			  vnet_crypto_op_id_t op_id, uword keys)
{
			      ...
			      u8 key[CHACHA20POLY1305_KEY_SIZE])
{
  ...
  uword raw_key = (uword) vnet_crypto_get_active_engine_key_data (
    keys, VNET_CRYPTO_OP_CHACHA20_POLY1305_ENC,
			      ...
			      u8 key[CHACHA20POLY1305_KEY_SIZE])
{
  ...
  vnet_crypto_key_t *keys =
    vnet_crypto_key_add (vm, VNET_CRYPTO_ALG_CHACHA20_POLY1305,
			 (uint8_t *) derived_key, CHACHA20POLY1305_KEY_SIZE);
  uword raw_key = (uword) vnet_crypto_get_active_engine_key_data (
    keys, VNET_CRYPTO_OP_CHACHA20_POLY1305_DEC,
    VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
