...
format_wg_if (u8 *s, va_list *args)
{
  ...
  s = format (s, "[%d] %U src:%U port:%d", wgii, format_vnet_sw_if_index_name,
	      vnet_get_main (), wgi->sw_if_index, format_ip_address,
	      &wgi->src_ip, wgi->port);

  key_to_base64 (local->l_private, NOISE_PUBLIC_KEY_LEN, key);

  s = format (s, " private-key:%s", key);
  s = format (s, " %U", format_hex_bytes, local->l_private,
  ...
  s =
    format (s, " %U", format_hex_bytes, local->l_public, NOISE_PUBLIC_KEY_LEN);
...
int
wg_if_create (u32 user_instance, const u8 private_key[NOISE_PUBLIC_KEY_LEN],
	      u16 port, const ip_address_t *src_ip, u32 *sw_if_indexp)
{
  ...
  *sw_if_indexp = (u32) ~0;
  ...
  hw_if_index = vnet_register_interface (vnm, wg_if_device_class.index, t_idx,
...
static void
wg_if_table_bind_v4 (ip4_main_t *im, uword opaque, u32 sw_if_index,
...
static void
wg_if_table_bind_v6 (ip6_main_t *im, uword opaque, u32 sw_if_index,
		     u32 new_fib_index, u32 old_fib_index)
{
  wg_if_t *wg_if;
