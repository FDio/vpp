  ...
  if (b != lb)
    {
      ...
      op->key_data = (uword) vnet_crypto_key_data_get (key, VNET_CRYPTO_HANDLER_TYPE_CHAINED);
		       ...
		       u8 *iv, f64 time)
{
  ...
  *r_idx = kp->kp_remote_index;
  ...
  wg_output_tun_add_to_frame (vm, *async_frame, kp->kp_send, payload_len, payload - b->data, bi,
