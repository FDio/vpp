...
static_always_inline void
wg_output_tun_add_to_frame (vlib_main_t *vm, u32 buffer_index, u32 crypto_len,
			    i16 crypto_start_offset, i16 iv_off, u8 flags)
{
  vlib_buffer_t *b = vlib_get_buffer (vm, buffer_index);
  vnet_crypto_buffer_metadata_t md = {
    .cipher_data_len = crypto_len,
    .cipher_data_start_off = crypto_start_offset,
    .auth_data_start_off = 0,
    .iv_off = iv_off,
    .icv_off = crypto_start_offset + crypto_len,
    .aad_off = 0,
    .aad_len = 0,
    .icv_len = NOISE_AUTHTAG_LEN,
    .is_hmac_check = !!(flags & VNET_CRYPTO_OP_FLAG_HMAC_CHECK),
    .is_chained_buffers = !!(flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS),
  };
		       ...
		       uint64_t *nonce, u8 *iv, f64 time)
{
  ...
  *r_idx = kp->kp_remote_index;
  ...
  wg_output_tun_add_to_frame (vm, bi, payload_len, payload - current,
			      vnet_crypto_buffer_metadata_pre_data_off (b), flag);
  async_bi[0] = bi;
