...
wg_prepare_sync_enc_op (vlib_main_t *vm, wg_per_thread_data_t *ptd, vlib_buffer_t *b,
			...
			u8 *dst, u8 *aad, u32 aad_len, u64 nonce, vnet_crypto_key_t *keys, u32 bi,
			u8 *iv)
{
...
static_always_inline void
wg_output_tun_add_to_frame (vlib_main_t *vm, vnet_crypto_async_frame_t *f, uword keys,
		       ...
		       u8 *iv, f64 time)
{
  ...
  *r_idx = kp->kp_remote_index;
  ...
  wg_output_tun_add_to_frame (vm, *async_frame, keys, payload_len, payload - b->data, bi,
