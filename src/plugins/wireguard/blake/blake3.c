...
chunk_state_init (blake3_chunk_state *self, const uint32_t key[8],
		  uint8_t flags)
{
...
static_always_inline size_t
chunk_state_fill_buf (blake3_chunk_state *self, const uint8_t *input,
		      size_t input_len)
{
...
static_always_inline output_t
make_output (const uint32_t input_cv[8], const uint8_t block[BLAKE3_BLOCK_LEN],
...
static_always_inline void
output_root_bytes (const output_t *self, uint64_t seek, uint8_t *out,
		   size_t out_len)
{
...
static_always_inline void
chunk_state_update (blake3_chunk_state *self, const uint8_t *input,
		    size_t input_len)
{
  if (self->buf_len > 0)
    {
...
static_always_inline output_t
parent_output (const uint8_t block[BLAKE3_BLOCK_LEN], const uint32_t key[8],
	       uint8_t flags)
{
...
static_always_inline size_t
compress_chunks_parallel (const uint8_t *input, size_t input_len,
...
static_always_inline size_t
compress_parents_parallel (const uint8_t *child_chaining_values,
			   size_t num_chaining_values, const uint32_t key[8],
			   uint8_t flags, uint8_t *out)
{
#if defined(BLAKE3_TESTING)
...
static size_t
blake3_compress_subtree_wide (const uint8_t *input, size_t input_len,
			      const uint32_t key[8], uint64_t chunk_counter,
			      uint8_t flags, uint8_t *out)
{
  ...
  if (input_len <= blake3_simd_degree () * BLAKE3_CHUNK_LEN)
    {
      return compress_chunks_parallel (input, input_len, key, chunk_counter,
				       flags, out);
    }
...
void
blake3_hasher_init_derive_key_raw (blake3_hasher *self, const void *context,
				   size_t context_len)
{
...
static_always_inline void
hasher_push_cv (blake3_hasher *self, uint8_t new_cv[BLAKE3_OUT_LEN],
		uint64_t chunk_counter)
{
