			 ...
			 uint8_t ets[NOISE_TIMESTAMP_LEN + NOISE_AUTHTAG_LEN])
{
  ...
  vnet_crypto_key_t *key =
    vnet_crypto_key_add (vm, VNET_CRYPTO_ALG_CHACHA20_POLY1305, _key, NOISE_SYMMETRIC_KEY_LEN);
  int ret = false;
  ...
  if (!noise_mix_dh (hs->hs_ck, &_key[0], hs->hs_e, r->r_public))
    goto error;
  vnet_crypto_key_update (key, VNET_CRYPTO_ALG_CHACHA20_POLY1305, _key, NOISE_SYMMETRIC_KEY_LEN);
...
noise_remote_begin_session (vlib_main_t * vm, noise_remote_t * r)
{
  ...
  kp.kp_valid = 1;
  kp.kp_send =
    vnet_crypto_key_add (vm, VNET_CRYPTO_ALG_CHACHA20_POLY1305, key_send, NOISE_SYMMETRIC_KEY_LEN);
  kp.kp_recv =
    vnet_crypto_key_add (vm, VNET_CRYPTO_ALG_CHACHA20_POLY1305, key_recv, NOISE_SYMMETRIC_KEY_LEN);
		      ...
		      uint8_t * dst)
{
  ...
  *r_idx = kp->kp_remote_index;
  uword raw_key = (uword) kp->kp_send;
...
noise_msg_encrypt (vlib_main_t *vm, uint8_t *dst, uint8_t *src, size_t src_len,
		   vnet_crypto_key_t *keys, uint8_t hash[NOISE_HASH_LEN])
{
