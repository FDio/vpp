		   ...
		   const uint8_t public[NOISE_PUBLIC_KEY_LEN], u32 noise_local_idx)
{
  ...
  if (preshared_key_set)
    clib_memcpy (r->r_psk, preshared_key, NOISE_SYMMETRIC_KEY_LEN);
			 ...
			 uint8_t ets[NOISE_TIMESTAMP_LEN + NOISE_AUTHTAG_LEN])
{
  ...
  noise_local_t *l = noise_local_get (r->r_local_idx);
  uint8_t key[NOISE_SYMMETRIC_KEY_LEN] = { 0 };
  ...
  if (!noise_mix_dh (hs->hs_ck, key, hs->hs_e, r->r_public))
    goto error;
  if (!vnet_crypto_ctx_set_cipher_key (ctx, key, NOISE_SYMMETRIC_KEY_LEN))
    goto error;
