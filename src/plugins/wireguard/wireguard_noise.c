...
static void noise_msg_encrypt (vlib_main_t *vm, uint8_t *, uint8_t *, size_t,
			       vnet_crypto_key_t *keys, uint8_t[NOISE_HASH_LEN]);
			 ...
			 uint8_t ets[NOISE_TIMESTAMP_LEN + NOISE_AUTHTAG_LEN])
{
  ...
  vnet_crypto_key_t *key =
    vnet_crypto_key_add_ptr (VNET_CRYPTO_ALG_CHACHA20_POLY1305, _key, NOISE_SYMMETRIC_KEY_LEN);
  int ret = false;
  ...
  /* es */
  if (!noise_mix_dh (hs->hs_ck, &_key[0], hs->hs_e, r->r_public))
    goto error;
  vnet_crypto_key_update (key, _key);

  /* s */
  noise_msg_encrypt (vm, es, l->l_public, NOISE_PUBLIC_KEY_LEN, key, hs->hs_hash);

  /* ss */
  if (!noise_mix_ss (hs->hs_ck, &_key[0], r->r_ss))
    goto error;
  vnet_crypto_key_update (key, _key);

  /* {t} */
  noise_tai64n_now (ets);
  noise_msg_encrypt (vm, ets, ets, NOISE_TIMESTAMP_LEN, key, hs->hs_hash);
  noise_remote_handshake_index_drop (vm, r);
				      ...
				      NOISE_AUTHTAG_LEN])
{
  ...
  /* es */
  if (!noise_mix_dh (hs.hs_ck, &_key[0], l->l_private, ue))
    goto error;
  vnet_crypto_key_update (key, _key);

  /* s */

  if (!noise_msg_decrypt (vm, r_public, es, NOISE_PUBLIC_KEY_LEN + NOISE_AUTHTAG_LEN, key,
			  hs.hs_hash))
    goto error;
  ...
  /* {t} */
  if (!noise_msg_decrypt (vm, timestamp, ets, NOISE_TIMESTAMP_LEN + NOISE_AUTHTAG_LEN, key,
			  hs.hs_hash))
    goto error;
  ;
		       ...
		       uint8_t en[0 + NOISE_AUTHTAG_LEN])
{
  ...
  /* psk */
  noise_mix_psk (hs->hs_ck, hs->hs_hash, &_key[0], r->r_psk);
  vnet_crypto_key_update (key, _key);

  /* {} */
  noise_msg_encrypt (vm, en, NULL, 0, key, hs->hs_hash);
			...
			uint8_t en[0 + NOISE_AUTHTAG_LEN])
{
  ...
  /* psk */
  noise_mix_psk (hs.hs_ck, hs.hs_hash, &_key[0], preshared_key);
  vnet_crypto_key_update (key, _key);

  /* {} */

  if (!noise_msg_decrypt (vm, NULL, en, 0 + NOISE_AUTHTAG_LEN, key, hs.hs_hash))
    goto error;
...
noise_remote_begin_session (vlib_main_t * vm, noise_remote_t * r)
{
  ...
  kp.kp_valid = 1;
  kp.kp_send =
    vnet_crypto_key_add_ptr (VNET_CRYPTO_ALG_CHACHA20_POLY1305, key_send, NOISE_SYMMETRIC_KEY_LEN);
  kp.kp_recv =
    vnet_crypto_key_add_ptr (VNET_CRYPTO_ALG_CHACHA20_POLY1305, key_recv, NOISE_SYMMETRIC_KEY_LEN);
		      ...
		      uint8_t * dst)
{
  ...
  *r_idx = kp->kp_remote_index;
  ...
  wg_chacha20poly1305_calc (vm, src, srclen, dst, NULL, 0, *nonce,
			    VNET_CRYPTO_OP_CHACHA20_POLY1305_ENC, kp->kp_send);
...
noise_msg_encrypt (vlib_main_t *vm, uint8_t *dst, uint8_t *src, size_t src_len,
		   vnet_crypto_key_t *key, uint8_t hash[NOISE_HASH_LEN])
{
  /* Nonce always zero for Noise_IK */
  wg_chacha20poly1305_calc (vm, src, src_len, dst, hash, NOISE_HASH_LEN, 0,
			    VNET_CRYPTO_OP_CHACHA20_POLY1305_ENC, key);
...
{
  /* Nonce always zero for Noise_IK */
  if (!wg_chacha20poly1305_calc (vm, src, src_len, dst, hash, NOISE_HASH_LEN, 0,
				 VNET_CRYPTO_OP_CHACHA20_POLY1305_DEC, key))
    return false;
