...
noise_remote_begin_session (vlib_main_t * vm, noise_remote_t * r)
{
  ...
  /* In either case, this is the time for delayed garbage collection. */
  noise_remote_keypair_free (vm, r, &r->r_garbage_current);
  noise_remote_keypair_free (vm, r, &r->r_garbage_next);
  r->r_garbage_current = NULL;
  r->r_garbage_next = NULL;
