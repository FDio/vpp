...
#define foreach_wg_input_error                                                \
  ...
  _ (NO_PEER_RESP, "No peer from response")                                   \
  _ (NO_PEER_ENC, "No peer before decrypt")                                   \
  _ (NO_PEER_DEC, "No peer after decrypt")                                    \
  _ (NO_PEER_POST, "No peer at post")                                         \
...
wg_handshake_process (vlib_main_t *vm, wg_main_t *wmp, vlib_buffer_t *b,
		      u32 node_idx, u8 is_ip4, u32 *sender_pi, u32 *receiver_pi)
{
  ...
  *receiver_pi = 0;
  ...
  switch (header->type)
	...
	*receiver_pi = resp->reveiver_index;
	...
	if (PREDICT_TRUE (entry != NULL))
	  ...
	  return WG_INPUT_ERROR_NO_PEER_RESP;
		 ...
		 vlib_frame_t *frame, u8 is_ip4, u16 async_next_node)
{
  ...
  while (n_left_from > 0)
	  ...
	  if (PREDICT_FALSE (!peer_idx))
	    {
	      ...
	      n_other += 1;
	      wgi_one_error (vm, node, b, WG_INPUT_ERROR_NO_PEER_ENC);
	      goto out;
	    }
      ...
      else
	{
	  ...
	  wg_input_error_t ret =
	    wg_handshake_process (vm, wmp, b[0], node->node_index, is_ip4, &sender_index, &receiver_index);
  ...
  while (n_left_from > 0)
      ...
      else
	{
	  ...
	  wgi_one_count (vm, node, b, WG_INPUT_ERROR_NO_PEER_DEC);
	  goto trace;
	}
	       ...
	       u8 is_ip4)
{
  ...
  while (n_left > 0)
	...
	{
	  next[0] = WG_INPUT_NEXT_PUNT;
	  wgi_one_count (vm, node, b, WG_INPUT_ERROR_NO_PEER_POST);
	  goto trace;
	}
