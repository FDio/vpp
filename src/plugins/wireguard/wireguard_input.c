		      ...
		      f64 max_handshake_cookie)
{
  ...
  vec_foreach (ii, wg_ifs)
	...
	continue;
      ...
      under_load =
	wg_if_is_under_load (vm, wg_if, inflight, max_handshake_cookie);
		 ...
		 vlib_frame_t *frame, u8 is_ip4, u16 async_next_node)
{
  ...
  static f64 max_handshake_cookie = 1000.0; /* magic number for init value */
  u32 num_threads =
    vec_len (wmp->per_thread_data) - 1; /* how many threads VPP has */
  ...
  while (n_left_from > 0)
      ...
      else
	{
	  ...
	  wg_input_error_t ret = wg_handshake_process (
	    vm, wmp, b[0], node->node_index, is_ip4, inflight_handshake,
	    &is_cookie, max_handshake_cookie);
	  ...
	  if (is_cookie == true)
	    {
	      ...
	      static bool is_max_cookie = false;
	      static u32 count_max_cookie = 0;

	      avg_cookie[idx_cookie] = max;
	      idx_cookie++;
	      idx_cookie = (idx_cookie % MAX_COUNT_COOKIE);

	      if (PREDICT_FALSE (is_max_cookie == false))
		{
		  if (idx_cookie == 0)
		    {
		      count_max_cookie = MAX_COUNT_COOKIE;
		      is_max_cookie = true;
		    }
		  else
		    count_max_cookie = idx_cookie;
		}

	      u32 i;
	      f64 max_avg = 0.0;
	      for (i = 0; i < count_max_cookie; i++)
		{
		  max_avg += avg_cookie[i] / count_max_cookie;
		}
	      if (max_avg > 0.0)
		max_handshake_cookie = max_avg;
	    }
