...
#define foreach_wg_input_error                                                \
  ...
  _ (SUCCESS_HANDSHAKE, "Success (handshake)")                                \
      ...
      else
	{
	  ...
	  if (ret != WG_INPUT_ERROR_NONE)
	  ...
	  else
	    {
	      other_bi[n_other] = from[b - bufs];
	      n_other += 1;
	      wgi_one_count (vm, node, b, WG_INPUT_ERROR_SUCCESS_HANDSHAKE);
	    }
	}
  ...
  while (n_left_from > 0)
      ...
      if (PREDICT_TRUE (peer != NULL))
	{
	  wg_input_error_t ret = wg_input_post_process (
	    vm, node, b[0], data_next, peer, data, &is_keepalive);
	       ...
	       u8 is_ip4)
{
  ...
  while (n_left > 0)
	...
	{
	  wg_input_error_t ret = wg_input_post_process (
	    vm, node, b[0], next, peer, data, &is_keepalive);
	  if (PREDICT_FALSE (WG_INPUT_ERROR_NONE != ret))
	    {
	      wgi_one_count (vm, node, b, ret);
	      goto trace;
