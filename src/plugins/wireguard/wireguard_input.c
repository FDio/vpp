			      ...
			      vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      if (PREDICT_TRUE (header_type == MESSAGE_DATA))
	{
	  ...
	  if (PREDICT_FALSE (peer_idx && (last_peer_idx != peer_idx)))
	    {
	      peer = wg_peer_get (*peer_idx);
	      last_peer_idx = peer_idx;
	    }
	  else if (PREDICT_FALSE (!peer_idx))
	    {
	  ...
	  enum noise_state_crypt state_cr = noise_sync_remote_decrypt (
	    ...
	    data->encrypted_data, encr_len, data->encrypted_data, n_data,
	    iv_data, time);
  ...
  /* decrypt packets */
  wg_process_ops (vm, node, ptd->crypto_ops, data_bufs, data_nexts, drop_next);
  ...
  while (n_left_from > 0)
      ...
      if (allowed)
	{
	  ...
	  data_next[n_data] = WG_INPUT_NEXT_IP4_INPUT;
	}
