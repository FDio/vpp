...
STATIC_ASSERT (0 == (MAX_COUNT_COOKIE & (MAX_COUNT_COOKIE - 1)),
	       "MAX_COUNT_COOKIE must be power of 2");
	  ...
	  if (is_cookie == true)
	    {
	      ...
	      idx_cookie++;
	      /* MAX_COUNT_COOKIE must be pow of 2 */
	      idx_cookie &= (MAX_COUNT_COOKIE - 1);
	      ...
	      while ((i < MAX_COUNT_COOKIE) && (avg_cookie[i] > 0.0))
		{
	      ...
	      wmp->handshake_cookie_rate = avg_cookie_sum / i;
	    }
