...
static_always_inline void
wg_input_add_to_frame (vlib_main_t *vm, vnet_crypto_async_frame_t *f, uword keys, u32 crypto_len,
		       i16 crypto_start_offset, u32 buffer_index, u16 next_node, u8 *iv, u8 *tag,
		       u8 flags)
{
  ...
  if (is_async)
	  ...
	  vnet_crypto_async_frame_is_full (*async_frame))
	{
	  *async_frame = vnet_crypto_async_get_frame (
      ...
      wg_input_add_to_frame (vm, *async_frame, keys, srclen_total, src - b->data, buf_idx,
