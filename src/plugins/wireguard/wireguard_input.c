...
#define foreach_wg_input_error                                                \
  ...
  _ (HANDSHAKE_UNSENT, "Failed sending handshake response")                   \
		      ...
		      u32 node_idx, u8 is_ip4)
{
  ...
  switch (header->type)
	...
	if (PREDICT_FALSE (!wg_send_handshake_response (vm, peer)))
	  {
	    ret = WG_INPUT_ERROR_HANDSHAKE_UNSENT;
	  }
	else
	  {
	...
	ret = WG_INPT_ERROR_REPLY_SEESION;
	break;
      }
    default:
		 ...
		 vlib_frame_t *frame, u8 is_ip4, u16 async_next_node)
{
  ...
  while (n_left_from > 0)
      ...
      else
	{
	  ...
	  if (ret != WG_INPUT_ERROR_HANDSHAKE_UNSENT &&
	      ret != WG_INPUT_ERROR_KEEPALIVE_UNSENT &&
