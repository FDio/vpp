...
#define foreach_wg_input_error                                                \
  ...
  _ (KEEPALIVE_SEND, "Keepalive unsent after handshake")                      \
  _ (HANDSHAKE_SEND, "Failed sending handshake response")                     \
		      ...
		      u32 node_idx, u8 is_ip4)
{
  ...
  switch (header->type)
	...
	if (PREDICT_FALSE (!wg_send_handshake_response (vm, peer)))
	  {
	    ret = WG_INPUT_ERROR_HANDSHAKE_SEND;
	  }
	break;
      }
	...
	if (noise_remote_begin_session (vm, &peer->remote))
	  {
	    ...
	    if (PREDICT_FALSE (!wg_send_keepalive (vm, peer)))
	      {
		ret = WG_INPUT_ERROR_KEEPALIVE_SEND;
	      }
	    else
	      {
		wg_peer_update_flags (peeri, WG_PEER_ESTABLISHED, true);
		ret = WG_INPUT_ERROR_HANDSHAKE_SUCCESS;
	      }
	  }
	break;
      }
		 ...
		 vlib_frame_t *frame, u8 is_ip4, u16 async_next_node)
{
  ...
  while (n_left_from > 0)
      ...
      else
	{
	  ...
	  if (ret != WG_INPUT_ERROR_HANDSHAKE_SENT &&
	      ret != WG_INPUT_ERROR_KEEPALIVE_SENT &&
