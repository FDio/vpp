...
index_t wg_itf_find_by_sw_if_index (u32 sw_if_index);
...
typedef walk_rc_t (*wg_itf_peer_walk_cb_t) (wg_itf_t * wgi, index_t peeri,
					    void *data);
void wg_itf_peer_walk (wg_itf_t * wgi, wg_itf_peer_walk_cb_t fn, void *data);

void wg_itf_peer_add (wg_itf_t * wgi, index_t peeri);
void wg_itf_peer_remove (wg_itf_t * wgi, index_t peeri);

/**
 * Data-plane exposed functions
 */
extern wg_itf_t *wg_itf_pool;

static_always_inline wg_itf_t *
...
extern index_t *wg_itf_index_by_port;

static_always_inline wg_itf_t *
wg_itf_get_by_port (u16 port)
{
  if (vec_len (wg_itf_index_by_port) < port)
    return (NULL);
  if (INDEX_INVALID == wg_itf_index_by_port[port])
    return (NULL);
  return (wg_itf_get (wg_itf_index_by_port[port]));
}
