	       ...
	       u16 port)
{
  ...
  if (!wmp->is_inited)
	...
	{
	  error =
	    clib_error_return (0, "UDP port %d is already in-use",
			       (u16) port);
	  return error;
	}
    ...
    {
      error = clib_error_return (0, "Device already in-use");
      return error;
    }
	     ...
	     u16 port, u16 persistent_keepalive, u32 * peer_index)
{
  ...
  /* *INDENT-OFF* */
  pool_foreach (peer, wg_itf->peers,
  ({
  ...
  if (pool_elts (wg_itf->peers) > MAX_PEERS)
    return (VNET_API_ERROR_LIMIT_EXCEEDED);

  pool_get (wg_itf->peers, peer);
  ...
  if (rv)
    {
      ...
      pool_put (wg_itf->peers, peer);
      return (rv);
    }

  noise_remote_init (&peer->remote, peer - wg_itf->peers, public_key,
		     &wg_itf->local);
...
int
wg_peer_remove (wg_main_t * wmp, index_t peeri)
{
  wg_peer_t *peer_pool = wmp->peers;
  wg_peer_t *peer = NULL;

  if (pool_is_free_index (peer_pool, peeri))
    return VNET_API_ERROR_NO_SUCH_ENTRY;

  peer = pool_elt_at_index (peer_pool, peeri);

  vnet_feature_enable_disable ("ip4-output", "wg-output-tun",
			       peer->tun_sw_if_index, 0, 0, 0);
  wg_peer_clear (wmp->vlib_main, peer);
  pool_put (peer_pool, peer);
