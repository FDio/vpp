...
vl_api_wireguard_itf_create_t_handler (vl_api_wireguard_itf_create_t * mp)
{
  ...
  ip_address_decode2 (&mp->itf.src_ip, &src);
  ...
  if (AF_IP6 == ip_addr_version (&src))
    ...
    rv = wg_itf_create (ntohl (mp->itf.user_instance),
			mp->itf.private_key,
			ntohs (mp->itf.port), &src, &sw_if_index);
...
typedef struct wg_peer_deatils_walk_t_
{
  vl_api_registration_t *reg;
  u32 context;
} wg_peer_deatils_walk_t;

static walk_rc_t
send_wg_peers_details (index_t peeri, void *data)
{
  vl_api_wireguard_peers_details_t *rmp;
  wg_peer_deatils_walk_t *ctx = data;
  ...
  ip_address_encode (&peer->dst_address, IP46_TYPE_ANY, &rmp->peer.endpoint);
