  ...
  (vl_api_wireguard_interface_create_t * mp)
{
  ...
  if (AF_IP6 == ip_addr_version (&src))
    ...
    rv = wg_itf_create (ntohl (mp->interface.user_instance),
			mp->interface.private_key,
...
static walk_rc_t
wireguard_itf_send_details (index_t wgii, void *data)
{
					   ...
					   mp)
{
  ...
  wg_itf_walk (wireguard_itf_send_details, &ctx);
}
...
vl_api_wireguard_genkey_t_handler (vl_api_wireguard_genkey_t * mp)
{
  ...
  ({
    curve25519_gen_secret (rmp->private_key);
  }));
  /* *INDENT-ON* */
}
...
vl_api_wireguard_pubkey_t_handler (vl_api_wireguard_pubkey_t * mp)
{
  ...
  {
    if (!curve25519_gen_public (rmp->public_key, mp->private_key))
