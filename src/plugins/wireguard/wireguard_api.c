...
static void
vl_api_wireguard_interface_create_t_handler (vl_api_wireguard_interface_create_t *mp)
{
  ...
  rv = wg_if_create (ntohl (mp->interface.user_instance), private_key, ntohs (mp->interface.port),
		     &src, &sw_if_index);
...
static void
vl_api_wireguard_interface_delete_t_handler (vl_api_wireguard_interface_delete_t *mp)
{
...
wireguard_if_send_details (index_t wgii, void *data)
{
  ...
  if (ctx->show_private_key)
    clib_memcpy (rmp->interface.private_key, local->l_private, NOISE_PUBLIC_KEY_LEN);
...
static void
vl_api_wireguard_interface_dump_t_handler (vl_api_wireguard_interface_dump_t *mp)
{
  vl_api_registration_t *reg;
  wg_main_t *wmp = &wg_main;
...
vl_api_wireguard_peer_add_t_handler (vl_api_wireguard_peer_add_t *mp)
{
  ...
  rv = wg_peer_add (ntohl (mp->peer.sw_if_index), NULL, false, mp->peer.public_key,
		    ntohl (mp->peer.table_id), &ip_addr_46 (&endpoint), allowed_ips,
		    ntohs (mp->peer.port), ntohs (mp->peer.persistent_keepalive), &peeri);
...
static void
vl_api_wireguard_peer_add_v2_t_handler (vl_api_wireguard_peer_add_v2_t *mp)
{
  vl_api_wireguard_peer_add_v2_reply_t *rmp;
  wg_main_t *wmp = &wg_main;
  index_t peeri = INDEX_INVALID;
  int ii, rv = 0;
  ...
  REPLY_MACRO2 (VL_API_WIREGUARD_PEER_ADD_V2_REPLY, { rmp->peer_index = ntohl (peeri); });
}
...
static walk_rc_t
wg_api_send_peers_v2_details (index_t peeri, void *data)
{
  vl_api_wireguard_peers_v2_details_t *rmp;
  wg_deatils_walk_t *ctx = data;
  const wg_peer_t *peer;
  u8 n_allowed_ips;
  size_t ss;
  ...
  rmp->_vl_msg_id = htons (VL_API_WIREGUARD_PEERS_V2_DETAILS + wg_main.msg_id_base);

  rmp->peer.peer_index = htonl (peeri);
  rmp->peer.flags = peer->flags;
  clib_memcpy (rmp->peer.public_key, peer->remote.r_public, NOISE_PUBLIC_KEY_LEN);
  clib_memcpy (rmp->peer.preshared_key, peer->remote.r_psk, NOISE_SYMMETRIC_KEY_LEN);
  rmp->peer.preshared_key_set = memcmp (peer->remote.r_psk, (uint8_t[NOISE_SYMMETRIC_KEY_LEN]){ 0 },
...
static void
vl_api_wireguard_peers_v2_dump_t_handler (vl_api_wireguard_peers_v2_dump_t *mp)
{
  vl_api_registration_t *reg;
  wg_main_t *wmp = &wg_main;
  ...
  if (mp->peer_index == ~0)
    wg_peer_walk (wg_api_send_peers_v2_details, &ctx);
  else
    wg_api_send_peers_v2_details (ntohl (mp->peer_index), &ctx);
}
...
static void
vl_api_want_wireguard_peer_events_t_handler (vl_api_want_wireguard_peer_events_t *mp)
{
  wg_main_t *wmp = &wg_main;
...
static void
wg_api_send_peer_event (vl_api_registration_t *rp, index_t peer_index, wg_peer_flags flags)
{
...
wg_api_peer_event (index_t peeri, wg_peer_flags flags)
{
  ...
  vlib_rpc_call_main_thread (wg_api_peer_event_cb, (u8 *) &args, sizeof (args));
}
