...
vl_api_wireguard_set_peer_t_handler (vl_api_wireguard_set_peer_t * mp)
{
  ...
  if (0 == mp->n_allowed_ips)
    {
      rv = VNET_API_ERROR_INVALID_VALUE;
      goto done;
    }
  else if (mp->n_allowed_ips > 1)
    {
      /* only one allowed_ip is currently supported */
      rv = VNET_API_ERROR_INVALID_VALUE_2;
      goto done;
    }
  vec_validate (allowed_ips, mp->n_allowed_ips - 1);
  ip_address_decode2 (&mp->endpoint, &endpoint);

  for (ii = 0; ii < mp->n_allowed_ips; ii++)
    ip_address_decode2 (&mp->allowed_ips[ii], &allowed_ips[ii]);
  ...
  if (AF_IP6 == ip_addr_version (&endpoint) ||
    ...
    rv = wg_peer_set (clib_net_to_host_u32 (0), // TODO: Fix
                      mp->public_key,
		      ip_addr_v4 (&endpoint),
...
vl_api_wireguard_remove_peer_t_handler (vl_api_wireguard_remove_peer_t * mp)
{
  ...
  rv = wg_peer_remove (wmp, mp->public_key);
