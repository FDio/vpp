/*
 * iouring_input.c - VPP io_uring input node
 */
...
static uword
iouring_input_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
                       vlib_frame_t *frame)
{
  ...
  io_uring_for_each_cqe (ring, head, cqe)
    {
      ...
      clib_memcpy_fast (vlib_buffer_get_current (b) + offset,
                        cqe, sizeof (iouring_node_cqe_t));
...
iouring_init (vlib_main_t *vm)
{
  ...
  im->workers = clib_mem_alloc_aligned (n_threads * sizeof (iouring_worker_t),
  ...
  for (u32 i = 0; i < im->n_sqpoll_threads; i++)
    {
      ...
      if (rv < 0)
        return clib_error_return (0, "io_uring_queue_init failed for sqpoll %u: %s",
                                  i, strerror (-rv));
  ...
  for (u32 i = im->n_sqpoll_threads; i < n_threads; i++)
    {
      ...
      int rv = io_uring_queue_init_params (im->queue_depth, &wrk->ring, &params);
      if (rv < 0)
        return clib_error_return (0, "io_uring_queue_init failed for worker %u: %s",
                                  i, strerror (-rv));
...
static clib_error_t *
iouring_show_stats_command_fn (vlib_main_t *vm, unformat_input_t *input,
