/*
 * iouring_test.c - io_uring test commands
 */
...
static uword
iouring_test_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
		      vlib_frame_t *frame)
{
...
iouring_test_command_internal (vlib_main_t *vm, struct io_uring_sqe *sqe)
{
  ...
  switch (pending_op.op)
    ...
    case IOURING_TEST_OP_OPENAT:
      if (res < 0)
	vlib_cli_output (vm, "openat '%s' failed: %s", pending_op.path,
			 strerror (-res));
      else
	vlib_cli_output (vm, "openat '%s' succeeded, fd = %d", pending_op.path,
			 res);
    ...
    case IOURING_TEST_OP_CLOSE:
      if (res < 0)
	vlib_cli_output (vm, "close fd %d failed: %s", pending_op.fd,
			 strerror (-res));
      else
...
static clib_error_t *
iouring_test_openat_command_fn (vlib_main_t *vm, unformat_input_t *input,
...
static clib_error_t *
iouring_test_read_command_fn (vlib_main_t *vm, unformat_input_t *input,
			      vlib_cli_command_t *cmd)
{
  struct io_uring *ring = iouring_get_ring (vm->thread_index);
  if (!ring)
...
static clib_error_t *
iouring_test_close_command_fn (vlib_main_t *vm, unformat_input_t *input,
			       vlib_cli_command_t *cmd)
{
  struct io_uring *ring = iouring_get_ring (vm->thread_index);
  if (!ring)
