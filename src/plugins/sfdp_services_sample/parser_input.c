...
sample_parser_interface_enable_disable (u32 sw_if_index, u32 tenant_id, u8 disable)
{
  ...
  if (!disable &&
      vnet_feature_is_enabled ("ip4-unicast", "sfdp-interface-input-ip4", sw_if_index))
  ...
  if (!disable &&
      spm->tenant_idx_by_sw_if_index[sw_if_index] != SAMPLE_PARSER_INVALID_TENANT_INDEX)
  ...
  if (vnet_feature_enable_disable ("ip4-unicast", "sample-parser-input", sw_if_index, !disable,
				   0, 0))
