...
static void
ip6_nbr_probe (const ip_adjacency_t * adj)
{
  icmp6_neighbor_solicitation_header_t *h;
  vnet_main_t *vnm = vnet_get_main ();
  const ip6_address_t *dst, *src;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_TRUE (error0 == ICMP6_ERROR_NONE && o0 != 0 &&
		...
		.mac = *(mac_address_t *) o0->ethernet_address,
	      };
...
static void
ip6_nd_send_na (vlib_main_t * vm,
		u32 sw_if_index, const ip46_address_t * addr)
{
  ...
  if (NULL == addr)
    ip6_addr = ip6_interface_first_address (i6m, sw_if_index);
  else
    ip6_addr = &addr->ip6;
...
const static ip_neighbor_vft_t ip6_nd_impl_vft = {
  .inv_probe = ip6_nbr_probe,
  .inv_send = ip6_nd_send_na,
