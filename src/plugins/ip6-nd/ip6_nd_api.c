...
#define  IP6_ND_PLUGIN_VERSION_MAJOR 1
#define  IP6_ND_PLUGIN_VERSION_MINOR 0

static void
vl_api_ip6_nd_plugin_get_version_t_handler (vl_api_ip6_nd_plugin_get_version_t
					    * mp)
{
  vl_api_ip6_nd_plugin_get_version_reply_t *rmp;
  ...
  rmp->_vl_msg_id =
    ntohs (VL_API_IP6_ND_PLUGIN_GET_VERSION_REPLY + REPLY_MSG_ID_BASE);
  rmp->context = mp->context;
  rmp->major = htonl (IP6_ND_PLUGIN_VERSION_MAJOR);
  rmp->minor = htonl (IP6_ND_PLUGIN_VERSION_MINOR);

  vl_api_send_msg (reg, (u8 *) rmp);
}

static void
  vl_api_ip6_nd_plugin_control_ping_t_handler
  (vl_api_ip6_nd_plugin_control_ping_t * mp)
{
  vl_api_ip6_nd_plugin_control_ping_reply_t *rmp;
  int rv = 0;

  /* *INDENT-OFF* */
  REPLY_MACRO2 (VL_API_IP6_ND_PLUGIN_CONTROL_PING_REPLY,
  ({
