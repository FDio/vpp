...
static int
api_ip6_nd_plugin_control_ping (vat_main_t * vam)
{
  return -1;
}

static int
api_ip6_nd_plugin_get_version (vat_main_t * vam)
{
  return -1;
}
...
api_ip6nd_proxy_dump (vat_main_t * vam)
{
  ...
  MPING (IP6_ND_PLUGIN_CONTROL_PING, mp_ping);
  S (mp_ping);
...
static void
  vl_api_ip6_nd_plugin_get_version_reply_t_handler
  (vl_api_ip6_nd_plugin_get_version_reply_t * mp)
{
  vat_main_t *vam = ip6_nd_test_main.vat_main;
  clib_warning ("IP6_ND plugin version: %d.%d", ntohl (mp->major),
		ntohl (mp->minor));
  vam->result_ready = 1;
}

static void
  vl_api_ip6_nd_plugin_control_ping_reply_t_handler
  (vl_api_ip6_nd_plugin_control_ping_reply_t * mp)
{
  vat_main_t *vam = ip6_nd_test_main.vat_main;
