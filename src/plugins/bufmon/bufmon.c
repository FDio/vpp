			 ...
			 vlib_frame_t *frame)
{
  ...
  for (; pending_frames < vec_len (nm->pending_frames); pending_frames++)
	...
	vec_elt_at_index (nm->pending_frames, pending_frames);
      vlib_frame_t *f = vlib_get_frame (vm, p->frame);
      pnd->out += f->n_vectors;
    }
  ...
  if (enable)
    {
      if (bm->enabled)
	return 0;
      if (vlib_node_set_dispatch_wrapper (vm, bufmon_dispatch_wrapper))
	return clib_error_return (0,
				  "failed to set dispatch wrapper function");
      if (vlib_buffer_set_alloc_callback (vm, bufmon_alloc_callback))
	{
	  vlib_node_set_dispatch_wrapper (vm, 0);
	  return clib_error_return (0,
				    "failed to set alloc callback function");
	}
      bm->enabled = 1;
    }
  else
