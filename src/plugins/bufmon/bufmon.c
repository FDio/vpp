...
static u32
bufmon_count_buffers_range (vlib_main_t *vm, vlib_frame_t *frame, u32 start,
			    u32 end)
{
  vlib_buffer_t *b[VLIB_FRAME_SIZE];
  u32 *from = vlib_frame_vector_args (frame);
  const u32 n = end - start;
  u32 nc = 0;
  u32 i;
...
{
  return bufmon_count_buffers_range (vm, frame, 0, frame->n_vectors);
}
			 ...
			 vlib_frame_t *frame)
{
  ...
  vec_foreach_index (i, pnd->snap)
	...
	pnd->out += bufmon_count_buffers_range (vm, f, pnd->snap[i], f->n_vectors);
    }
  for (u32 i = pre_len; i < vec_len (nm->pending_frames); i++)
    pnd->out +=
      bufmon_count_buffers (vm, vlib_get_frame (vm, nm->pending_frames[i].frame));
