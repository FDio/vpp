		       ...
		       unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output (vm, " desc %x src %x dst %x",
  ...
  /* set CHAINADDR */
  cbdma_set_u64 (cc->bar, 0x90, vlib_physmem_virtual_to_physical (vm, pr, d));
  /* set DMACOUNT */
  cbdma_set_u32(cc->bar, 0x86, 1);

  vlib_cli_output (vm, "desc   %U", format_hexdump, d, 64);

  vlib_cli_output (vm, "src    %U", format_hexdump, src, 256);
  vlib_cli_output (vm, "before %U", format_hexdump, dst, 256);
  vlib_cli_output (vm, "after  %U", format_hexdump, dst, 256);
