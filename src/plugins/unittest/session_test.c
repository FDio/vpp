...
static int
session_test_auto_sdl (vlib_main_t *vm, unformat_input_t *input)
{
  u32 rmt_plen = 0;
  ...
  u32 action = 0;
  int error = 0;
  session_auto_sdl_config_args_t asdl_args = {
    .enable = 1,
    .remove_timeout = 300,
    .threshold = 1,
  };
  ...
  session_auto_sdl_config (&asdl_args);
  ...
  if (unformat_check_input (input) == UNFORMAT_END_OF_INPUT)
	  ...
	  for (int i = 1; i < 255; i++)
	    {
	      ...
	      if (session_auto_sdl_track_prefix (&args) != 0)
		{
      ...
      for (int i = 1; i < 255; i++)
	{
      ...
      u32 expected = 254 * 10 + 254;
      u32 total = session_auto_sdl_pool_size ();
      SESSION_TEST ((total == expected),
		    "total auto sdl entries is %u, expected %u", total,
		    expected);
      session_test_disable_rt_backend_engine (vm);
      total = session_auto_sdl_pool_size ();
      expected = 0;
      SESSION_TEST ((total == expected),
		    "total auto sdl entries is %u, expected %u", total,
		    expected);
	      ...
	      unformat_input_t * input, vlib_cli_command_t * cmd_arg)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	res = session_test_sdl (vm, input);
      else if (unformat (input, "auto-sdl"))
	res = session_test_auto_sdl (vm, input);
      ...
      else if (unformat (input, "all"))
	{
	  ...
	  if ((res = session_test_enable_disable (vm, input)))
	    goto done;
	  if ((res = session_test_auto_sdl (vm, input)))
	    goto done;
	}
      else
	break;
