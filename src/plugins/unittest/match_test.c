...
#define MATCH_TEST_HIT(_comment, _res, _data, _list, _rule, _args...)   \
  {                                                                     \
  MATCH_TEST((_res)->msr_pos.msp_list_index == _list,                   \
             "list:%d != %d; " _comment,                                \
             (_res)->msr_pos.msp_list_index, _list, ##_args);           \
  MATCH_TEST((_res)->msr_pos.msp_rule_index == _rule,                   \
             "rule:%d != %d; " _comment,                                \
             (_res)->msr_pos.msp_rule_index, _rule, ##_args);           \
  MATCH_TEST((_res)->msr_user_ctx == _data,                             \
             "user:0x%x: " _comment,                                    \
             (_res)->msr_user_ctx, ##_args);                            \
}
...
match_test_mask_n_tuple_ip4 (vlib_main_t * vm, const char *engine)
{
  ...
  match_match_one (vm, b, &app, now, &mres);
...
match_test_exact_ip_l4_mk_v6 (match_list_t * list, match_orientation_t mo)
{
  ...
  for (rr = 0; rr < 16; rr++)
      ...
      for (port = 1024; port < 2048; port++)
	{
	  ...
	  rule->mr_exact_ip_l4.meil_l4.ml_port = port;
			 ...
			 match_orientation_t mo, const char *engine)
{
  ...
  vec_foreach (rule, rules)
  {
    ...
    match_match_one (vm, b, &app, now, &mres);
    MATCH_TEST_HIT ("match mask-src-ip-mac hit /32", &mres, data1, 0,
		    rule - rules);
  }
  ...
  if (MATCH_SRC == mo)
    ...
    i->dst_address.as_u32 = clib_host_to_net_u32 (0x0a0a0a01);
  match_match_one (vm, b, &app, now, &mres);
  MATCH_TEST_MISS ("match mask-src-ip-mac miss /32", &mres);
  ...
  match_set_apply (msi, MATCH_SEMANTIC_FIRST,
		   VNET_LINK_IP4, MATCH_SET_TAG_FLAG_0_TAG, &app);
  ...
  handle1 = match_set_list_add (msi, &list, 0, data1);
  match_set_apply (msi, MATCH_SEMANTIC_FIRST,
		   VNET_LINK_ETHERNET, MATCH_SET_TAG_FLAG_0_TAG, &app);
  // MATCH_PRINT(vm, "%U", format_match_set, msi, 1);
  ...
  vec_foreach (rule, rules)
  {
    ...
    match_match_one (vm, b, &app, now, &mres);
    MATCH_TEST_HIT ("match mask-src-ip-mac hit /32 arp", &mres, data1, 0,
		    rule - rules);
  }
			 ...
			 match_orientation_t mo, const char *engine)
{
  ...
  PUSH_HEADER (i, udp);
  b->current_data = vnet_buffer (b)->l2.l2_len = sizeof (eth) + sizeof (evh);
  ...
  vec_foreach (rule, rules)
  {
    ...
    match_match_one (vm, b, &app, now, &mres);
    MATCH_TEST_HIT ("match mask-src-ip-mac hit src /128", &mres, data1, 0,
		    rule - rules);
  }
  match_set_unapply (msi, &app);
  ...
  i = PUSH_HEADER (ev, ip6);
  PUSH_HEADER (i, udp);
  vnet_buffer (b)->l2.l2_len = sizeof (eth) + sizeof (evh);
  ...
  PUSH_HEADER (i, udp);
  vnet_buffer (b)->l2.l2_len = sizeof (eth) + 2 * sizeof (evh);
  ...
  vec_foreach (rule, rules)
  {
    ...
    match_match_one (vm, b, &app, now, &mres);
    MATCH_TEST_HIT ("match mask-src-ip-mac hit src /128 2 tags of 2",
		    &mres, data1, 0, rule - rules);
  }
...
#define N_SETS 4
  ...
  for (ii = 0; ii < N_SETS; ii++)
    {
      ...
      handles[ii] = match_set_list_add (msis[ii], &list, 0, data1);
      match_list_free (&list);
    }
  ...
  msi = match_set_create_and_lock (set_name,
				   MATCH_TYPE_SETS,
				   MATCH_BOTH, ETHERNET_TYPE_IP4, NULL);
  handle = match_set_list_add (msi, &list, 0, data1);
  match_set_apply (msi, MATCH_SEMANTIC_FIRST,
		   VNET_LINK_ETHERNET, MATCH_SET_TAG_FLAG_0_TAG, &app);
  match_list_free (&list);
  ...
  vec_foreach (rs, rules[0])
  {
    vec_foreach (rd, rules[1])
    {
      ...
      match_match_one (vm, b, &app, now, &mres);
      MATCH_TEST_HIT ("match sets hit", &mres, data1, 0, 0);
    }
  }
  vec_foreach (rs, rules[2])
  {
    vec_foreach (rd, rules[3])
    {
      ...
      match_match_one (vm, b, &app, now, &mres);
      MATCH_TEST_HIT ("match sets hit", &mres, data1, 0, 1);
    }
  }
  ...
  vec_foreach (rs, rules[3])
  {
    vec_foreach (rd, rules[2])
    {
      ...
      match_match_one (vm, b, &app, now, &mres);
      MATCH_TEST_MISS ("match sets miss opposite", &mres);
    }
  }
  ...
  vec_foreach (rs, rules[0])
  {
    vec_foreach (rd, rules[3])
    {
      ...
      match_match_one (vm, b, &app, now, &mres);
      MATCH_TEST_MISS ("match sets miss mismatch", &mres);
    }
  }
  ...
  match_set_list_replace (msis[0], handles[0], &list, 0, data1);
  match_list_free (&list);
	    ...
	    unformat_input_t * input, vlib_cli_command_t * cmd_arg)
{
  ...
  for (mo = MATCH_SRC; mo <= MATCH_DST; mo++)
    {
      ...
      // turbo doesn't register for these rule types.
    }
  ...
  // only the linear engine does sets
