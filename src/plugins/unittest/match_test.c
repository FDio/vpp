...
match_test_engine_set (const char *engine, u32 priority)
{
  ...
  for (msem = 0; msem < MATCH_N_SEMANTICS; msem++)
	...
	{
	  match_engine_set_priority (NULL, msem, mtype, len, priority + 1);
...
match_test_mask_n_tuple_ip4 (vlib_main_t * vm, const char *engine)
{
  ...
  match_set_list_replace (msi, handle1, &list);
  ...
  rule8.mr_mask_n_tuple.mnt_src_port.mpr_begin = 1024;
  rule8.mr_mask_n_tuple.mnt_src_port.mpr_end = 2048;
  ...
  match_list_push_back (&list, &rule8);
  match_set_list_replace (msi, handle2, &list);
...
#define N_SETS 4
  ...
  match_set_list_replace (msis[0], handles[0], &list);
  match_list_free (&list);
	    ...
	    unformat_input_t * input, vlib_cli_command_t * cmd_arg)
{
  ...
  for (mo = MATCH_SRC; mo <= MATCH_DST; mo++)
    {
      ...
      res |= match_test_exact_ip6 (vm, mo, "hash");
      res |= match_test_exact_ip4 (vm, mo, "hash");
      res |= match_test_exact_ip6_l4 (vm, mo, "linear");
      res |= match_test_exact_ip4_l4 (vm, mo, "linear");
      res |= match_test_exact_ip6_l4 (vm, mo, "hash");
      res |= match_test_exact_ip4_l4 (vm, mo, "hash");
  ...
  if (0 != pool_elts (match_set_pool) ||
      ...
      0 != pool_elts (match_engine_linear_pool) ||
      0 != pool_elts (match_engine_hash_pool))
