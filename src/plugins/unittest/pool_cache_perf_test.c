...
pool_cache_perf_open_csv (pool_cache_perf_main_t *ptm)
{
  ...
  ptm->csv_fd = open ((char *) ptm->csv_filename, O_WRONLY | O_CREAT | O_APPEND, 0644);
  if (ptm->csv_fd < 0)
			       ...
			       u32 n_participants, u32 mode)
{
  ...
  line =
    format (line, "%llu,%u,%s,%u,%u,%u,%u,%u,%u,%u,%.0f,%llu,%llu,%llu,%.6f,%.6f\n",
	    (unsigned long long) ptm->run_id, (u32) getpid (), pool_cache_perf_mode_name (mode),
	    ptm->n_workers, ptm->rounds, ptm->batch_size, ptm->warmup_rounds, ptm->samples,
...
pool_cache_perf_run_mode (vlib_main_t *vm, pool_cache_perf_main_t *ptm, u32 mode)
{
  ...
  if ((error = pool_cache_perf_prepare_storage (
	 ptm, n_participants, n_participants + (mode == POOL_CACHE_PERF_MODE_OVERLAP ? 1 : 0))))
    return error;
			    ...
			    CLIB_UNUSED (vlib_cli_command_t *cmd))
{
  ...
  u32 modes[] = {
    POOL_CACHE_PERF_MODE_LOCAL,	      POOL_CACHE_PERF_MODE_REFILL,  POOL_CACHE_PERF_MODE_RING,
    POOL_CACHE_PERF_MODE_MANY_TO_ONE, POOL_CACHE_PERF_MODE_OVERLAP,
  };
  u32 i;
