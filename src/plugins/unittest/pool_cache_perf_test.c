			       ...
			       u32 n_participants, u32 mode)
{
  ...
  line = format (
    ...
    (unsigned long long) clib_atomic_load_acq_n (&ptm->pool.state.global_lock_acquisitions),
...
pool_cache_perf_run_mode (vlib_main_t *vm, pool_cache_perf_main_t *ptm, u32 mode)
{
  ...
  for (sample = 0; sample < ptm->samples; sample++)
	...
	{
	  test_perf_pool_cache_free_resources (&ptm->pool);
	  test_perf_pool_cache_init_with_batch (&ptm->pool, "pool-cache-perf",
						ptm->log2_subpool_size, CLIB_CACHE_LINE_BYTES,
						ptm->cache_batch_size);
	}
    }
