...
pool_cache_perf_open_csv (pool_cache_perf_main_t *ptm)
{
  ...
  ptm->csv_fd =
    open ((char *) ptm->csv_filename, O_RDWR | O_CREAT | O_APPEND, 0644);
  if (ptm->csv_fd < 0)
  ...
  if (st.st_size != 0)
    {
      n_read = pread (ptm->csv_fd, existing_header,
		      sizeof (existing_header), 0);
	  ...
	  memcmp (existing_header, csv_header, sizeof (existing_header)) != 0)
	{
	  ...
	  return clib_error_return (
	    0, "refusing to append pool-cache results to `%s': "
...
pool_cache_perf_local_loop (pool_cache_perf_main_t *ptm, u32 *indices, u32 n_rounds)
{
  ...
  for (i = 0; i < n_rounds; i++)
    for (j = 0; j < ptm->batch_size; j++)
      {
	indices[j] = test_perf_pool_cache_alloc (&ptm->pool);
	test_perf_pool_cache_free (&ptm->pool, indices[j]);
      }
...
pool_cache_perf_reset_sample_counters (pool_cache_perf_main_t *ptm)
{
  ...
  vec_foreach (pt, ptm->pool.state.per_thread)
    {
      clib_atomic_store_rel_n (&pt->refills, 0);
      clib_atomic_store_rel_n (&pt->flushes, 0);
    }
  clib_atomic_store_rel_n (&ptm->pool.state.global_lock_acquisitions, 0);
  clib_atomic_store_rel_n (&ptm->pool.state.growths, 0);
}
				       ...
				       u32 n_participants, u32 mode)
{
  ...
  u32 csv_warmup =
    mode == POOL_CACHE_PERF_MODE_GROWTH ? 0 : ptm->warmup_rounds;
  u32 i;
  ...
  line = format (
    line,
    "%llu,%u,%s,%u,%u,%u,%u,%u,%u,%u,%u,%.0f,%llu,%llu,%llu,%.6f,%.6f,%llu,%llu,%llu,%llu\n",
    (unsigned long long) ptm->run_id, (u32) getpid (),
    pool_cache_perf_mode_name (mode), ptm->n_workers, csv_rounds,
    ptm->batch_size, ptm->cache_batch_size, csv_warmup,
    ptm->samples, ptm->log2_subpool_size, sample + 1,
    vm->clib_time.clocks_per_second, (unsigned long long) total_ticks,
    (unsigned long long) max_ticks, (unsigned long long) total_ops,
    cycles_per_op, mops, (unsigned long long) refills,
    (unsigned long long) flushes,
    (unsigned long long) clib_atomic_load_acq_n (
      &ptm->pool.state.global_lock_acquisitions),
    (unsigned long long) clib_atomic_load_acq_n (&ptm->pool.state.growths));
...
pool_cache_perf_run_mode (vlib_main_t *vm, pool_cache_perf_main_t *ptm, u32 mode)
{
  ...
  test_perf_pool_cache_init_with_batch (&ptm->pool, "pool-cache-perf", ptm->log2_subpool_size,
  ...
  vlib_cli_output (vm,
			   ...
			   ptm->cache_batch_size, ptm->warmup_rounds, ptm->samples,
			   ptm->log2_subpool_size);
			    ...
			    CLIB_UNUSED (vlib_cli_command_t *cmd))
{
  ...
  if (ptm->rounds == 0 || ptm->batch_size == 0 || ptm->cache_batch_size == 0 ||
      ptm->samples == 0)
    {
      error = clib_error_return (
	0, "rounds, batch-size, cache-batch-size, and samples must be non-zero");
      goto done;
    }
