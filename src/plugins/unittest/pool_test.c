...
pool_cache_test_multiple_instances (void)
{
  ...
  test_cli_pool_cache_init (&pool_a, "pool-cache-instance-a", POOL_CACHE_TEST_LOG2_SUBPOOL_SIZE,
  ...
  vec_free (formatted);
  test_cli_pool_cache_free (&pool_a, idx);
...
pool_cache_cli_test_init (pool_cache_cli_test_main_t *ptm, u32 log2_subpool_size)
{
  ...
  test_cli_pool_cache_init (&ptm->pool, "pool-cache-cli", log2_subpool_size, CLIB_CACHE_LINE_BYTES);
  ptm->active = 1;
  return 0;
}
...
test_pool_cache_cli_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  switch (action)
    ...
    case POOL_CACHE_CLI_TEST_INIT:
      error = pool_cache_cli_test_init (ptm, log2_subpool_size);
...
VLIB_CLI_COMMAND (test_pool_cache_cli_command, static) = {
  ...
  .short_help = "test pool-cache cli init [log2-subpool-size <n>] [alloc <n>] | alloc [n] | "
