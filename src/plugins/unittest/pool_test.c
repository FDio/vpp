...
pool_cache_test_error_at (pool_cache_test_main_t *ptm, const char *func, u32 line,
			  const char *caller_func, u32 caller_line, u32 has_state,
			  u32 idx, u32 slot, u32 expected_state, u32 actual_state)
{
...
#define pool_cache_test_error_from(ptm, caller_func, caller_line)              \
  pool_cache_test_error_at ((ptm), __func__, __LINE__, (caller_func),          \
			    (caller_line), 0, 0, 0, 0, 0)

#define pool_cache_test_state_error_from(ptm, caller_func, caller_line, idx,   \
					 slot, expected, actual)                 \
  pool_cache_test_error_at ((ptm), __func__, __LINE__, (caller_func),          \
			    (caller_line), 1, (idx), (slot), (expected), (actual))

static_always_inline int
pool_cache_test_wait_for_workers_at (vlib_main_t *vm, pool_cache_test_main_t *ptm,
				     volatile u32 *counter, u32 target,
				     const char *caller_func, u32 caller_line)
{
...
#define pool_cache_test_wait_for_workers(vm, ptm, counter, target)             \
  pool_cache_test_wait_for_workers_at ((vm), (ptm), (counter), (target),       \
				       __func__, __LINE__)
...
static_always_inline void
pool_cache_test_verify_state_at (pool_cache_test_main_t *ptm, u32 *indices, u32 start,
				 u32 end, u8 expected, const char *caller_func,
				 u32 caller_line)
{
  u32 i;
  ...
  for (i = start; i < end; i++)
    {
      u8 actual = pool_cache_test_slot_state (ptm, indices[i]);
...
#define pool_cache_test_verify_state(ptm, indices, start, end, expected)       \
  pool_cache_test_verify_state_at ((ptm), (indices), (start), (end),           \
				   (expected), __func__, __LINE__)
...
pool_cache_test_error_report (pool_cache_test_main_t *ptm, const char *test_name)
{
  ...
  if (ptm->first_error_caller_func)
    error = clib_error_return (
      0,
      "%s pool cache test saw %u errors, first at %s:%u called from %s:%u on thread %u",
  ...
  if (ptm->first_error_has_state)
    {
      ...
      state_detail = format (
	state_detail,
	", index %u slot %u subpool %u elt %u expected state %u actual state %u%c",
