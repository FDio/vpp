...
static int
bier_test_validate_entry (index_t bei, int n_buckets, ...)
{
  ...
  res = BIER_TEST_I ((DPO_LOAD_BALANCE == dpo.dpoi_type), "Entry links to %U",
...
#define N_BIER_ECMP_TABLES 16
  ...
  mpls_fib_index =
    fib_table_find (FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID);
  ...
  BIER_TEST (!fib_test_validate_entry (
	       lfei, FIB_FORW_CHAIN_TYPE_MPLS_EOS, 16, &l_o_bt[0], &l_o_bt[1],
	       &l_o_bt[2], &l_o_bt[3], &l_o_bt[4], &l_o_bt[5], &l_o_bt[6],
	       &l_o_bt[7], &l_o_bt[8], &l_o_bt[9], &l_o_bt[10], &l_o_bt[11],
	       &l_o_bt[12], &l_o_bt[13], &l_o_bt[14], &l_o_bt[15]),
  ...
  BIER_TEST (!dpo_cmp (drop_dpo_get (DPO_PROTO_MPLS), &bfm_1_1_1_1->bfm_dpo),
  ...
  fei = fib_table_entry_update_one_path (
    0, &pfx_1_1_1_1_s_32, FIB_SOURCE_API, FIB_ENTRY_FLAG_NONE, DPO_PROTO_IP4,
    &nh_10_10_10_1, tm->hw[0]->sw_if_index,
    ~0, // invalid fib index
    1, out_lbl_99, FIB_ROUTE_PATH_FLAG_NONE);
  fib_entry_contribute_forwarding (fei, FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS,
				   &neos_dpo_1_1_1_1);
  ...
  ai_mpls_10_10_10_2 = adj_nbr_add_or_lock (
    FIB_PROTOCOL_IP4, VNET_LINK_MPLS, &nh_10_10_10_2, tm->hw[0]->sw_if_index);
  ...
  fei = fib_table_entry_path_add (0, &pfx_1_1_1_1_s_32, FIB_SOURCE_API,
				  FIB_ENTRY_FLAG_NONE, DPO_PROTO_IP4,
				  &nh_10_10_10_2, tm->hw[0]->sw_if_index,
				  ~0, // invalid fib index
				  1, out_lbl_100, FIB_ROUTE_PATH_FLAG_NONE);
  ...
  fei = fib_table_entry_path_add (0, &pfx_1_1_1_2_s_32, FIB_SOURCE_API,
				  FIB_ENTRY_FLAG_NONE, DPO_PROTO_IP4,
				  &nh_10_10_10_2, tm->hw[0]->sw_if_index,
				  ~0, // invalid fib index
				  1, out_lbl_101, FIB_ROUTE_PATH_FLAG_NONE);
  ...
  BIER_TEST (!dpo_cmp (&neos_dpo_1_1_1_2, &bfm_1_1_1_2->bfm_dpo),
  ...
  BIER_TEST (!bier_test_validate_entry (bei_3, 2, &dpo_o_bfm_1_1_1_1,
					&dpo_o_bfm_1_1_1_2),
   ...
   * ECMP tables
   */
  BIER_TEST ((bier_table_fwd_lookup (bier_table_get (l_o_bt[0].bier.table),
				     3) == bfmi_1_1_1_1),
	     "fwd lookup for BP:3 ECMP:0 is 1.1.1.1");
  BIER_TEST ((bier_table_fwd_lookup (bier_table_get (l_o_bt[1].bier.table),
				     3) == bfmi_1_1_1_2),
...
bier_test_mpls_disp (void)
{
  ...
  bier_disp_table_entry_path_add (bier_disp_tbl_id, src, BIER_HDR_PROTO_IPV4,
				  rpaths);
  ...
  BIER_TEST ((bdti1 == lkd->lkd_fib_index), "disp is deag in %d %U",
	     lkd->lkd_fib_index, format_dpo_id, dpo_disp_entry_v4, 0);
  BIER_TEST ((LOOKUP_INPUT_DST_ADDR == lkd->lkd_input),
	     "disp is destination deag in %d %U", lkd->lkd_input,
	     format_dpo_id, dpo_disp_entry_v4, 0);
  BIER_TEST ((LOOKUP_MULTICAST == lkd->lkd_cast),
	     "disp is multicast deag in %d %U", lkd->lkd_input, format_dpo_id,
	     dpo_disp_entry_v4, 0);
...
static clib_error_t *
bier_test (vlib_main_t *vm, unformat_input_t *input,
