 ...
 *
 * The production listeners receive public BFD bridge payloads, so the tests use
 * the same event contract instead of constructing private bfd_session_t objects.
 */
void fib_bfd_notify (bfd_vnet_bridge_listener_t *listener,
                     bfd_vnet_bridge_args_t *args);
void adj_bfd_notify (bfd_vnet_bridge_listener_t *listener,
                     bfd_vnet_bridge_args_t *args);

/**
 * @brief Invoke a BFD bridge listener directly from unit tests.
 */
static void
transient_bfd_notify (void (*fn) (bfd_vnet_bridge_listener_t *,
