...
tcp_tamper_should_drop (u32 thread_index, u32 conn_index, u32 seq, u8 flags)
{
  ...
  /* Queued output can outlive its connection during teardown, so validate. */
  ...
  vec_foreach (r, im->rules)
	...
	continue;
      ...
      if (r->n_drop)
	{
	  /* Sample the sender's recovery state at the first drop so a test can
	   * assert what was in progress when the segment was lost. */
	  if (r->n_dropped == 0 && tc)
	    {
