...
sfifo_test_fifo_grow (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  fifo_segment_t *fs;
  svm_fifo_t *f;
  u32 ooo_chunk, offset, enq_now, deq_now;
  ...
  c = f->end_chunk;
  SFIFO_TEST (c->start_byte == 4096, "end chunk start byte should be %u",
	      4096);
  ...
  ooo_chunk = fifo_size / 10;
  for (i = 10; i > 0; i--)
    {
      offset = i * ooo_chunk;
      enq_now = clib_min (ooo_chunk, vec_len (test_data) - offset);
  ...
  c = f->end_chunk;
  SFIFO_TEST (c->start_byte = 8192, "end chunk should start at %u", 8192);
   ...
   */
  rv = svm_fifo_enqueue (f, ooo_chunk, test_data);
  ...
  for (i = 0; i <= 10; i++)
    {
      offset = i * ooo_chunk;
      deq_now = clib_min (ooo_chunk, vec_len (test_data) - offset);
...
int
sfifo_test_fifo_grow_del (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  svm_fifo_init_pointers (f, f->size - 100, f->size + 100);
  c = clib_mem_alloc (sizeof (svm_fifo_chunk_t) + 100);
  c->length = 100;
  c->start_byte = ~0;
  c->next = 0;
...
sfifo_test_fifo_shrink (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  SFIFO_TEST (f->size == 8 * chunk_size, "nitems expected %u is %u",
	      8 * chunk_size, f->size);
  ...
  SFIFO_TEST (f->size == 11 * chunk_size + 1, "size expected %u is %u",
	      11 * chunk_size + 1, f->size);
  SFIFO_TEST (f->size == 11 * chunk_size - 200, "nitems expected %u is %u",
	      11 * chunk_size - 200, f->size);
  SFIFO_TEST (f->flags & SVM_FIFO_F_SHRINK, "shrink flag should be set");
  SFIFO_TEST (!(f->flags & SVM_FIFO_F_COLLECT_CHUNKS), "collect flag should"
	      " not be set");
  SFIFO_TEST (svm_fifo_is_sane (f), "fifo should be sane");
  ...
  SFIFO_TEST (f->size == 11 * chunk_size + 1, "size expected %u is %u",
	      11 * chunk_size + 1, f->size);
  SFIFO_TEST (f->size == 11 * chunk_size - 300, "nitems expected %u is %u",
	      11 * chunk_size - 300, f->size);
  SFIFO_TEST (f->flags & SVM_FIFO_F_SHRINK, "shrink flag should be set");
  SFIFO_TEST (!(f->flags & SVM_FIFO_F_COLLECT_CHUNKS), "collect flag should"
	      " not be set");
  SFIFO_TEST (svm_fifo_is_sane (f), "fifo should be sane");
  ...
  SFIFO_TEST (f->size == 1 * chunk_size, "nitems expected %u is %u",
	      1 * chunk_size, f->size);
  SFIFO_TEST (f->flags & SVM_FIFO_F_SHRINK, "shrink flag should be set");
  SFIFO_TEST (!(f->flags & SVM_FIFO_F_COLLECT_CHUNKS), "collect flag should"
	      " not be set");
