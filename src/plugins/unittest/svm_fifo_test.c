...
sfifo_test_fifo_reservation (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  n_segs = svm_fifo_provision_chunks_at_offset (
    f, first_len, segs, ARRAY_LEN (segs), second_len);
  SFIFO_TEST (n_segs > 0, "second reservation provisioned in %d segments",
	      n_segs);
  copied = 0;
  for (i = 0; i < n_segs; i++)
    {
      clib_memcpy_fast (segs[i].data, expected + first_len + copied,
			segs[i].len);
      copied += segs[i].len;
    }
  SFIFO_TEST (copied == second_len, "second reservation mapped %u bytes",
	      copied);
  SFIFO_TEST (svm_fifo_max_dequeue_cons (f) == 0,
	      "uncommitted reservation is not visible");

  n_segs = svm_fifo_provision_chunks_at_offset (
    f, 0, segs, ARRAY_LEN (segs), first_len);
  SFIFO_TEST (n_segs > 0, "first reservation provisioned in %d segments",
	      n_segs);
  copied = 0;
  for (i = 0; i < n_segs; i++)
    {
  ...
  SFIFO_TEST (copied == first_len, "first reservation mapped %u bytes",
	      copied);

  /* The producer publishes completed reservations in stream order. */
  svm_fifo_enqueue_nocopy (f, first_len);
  SFIFO_TEST (svm_fifo_max_dequeue_cons (f) == first_len,
	      "only first reservation is visible");
  ...
  rv = svm_fifo_provision_chunks_at_offset (f, 16 << 10, segs,
					     ARRAY_LEN (segs), 1);
