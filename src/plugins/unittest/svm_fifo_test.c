...
sfifo_test_fifo_async (void)
{
  ...
  SFIFO_TEST (svm_fifo_async_refs (f)[0] == 7, "async reference matches");
  ...
  SFIFO_TEST (svm_fifo_enqueue_async (f, &op) == 8, "enqueue first async operation");
  op = (svm_fifo_async_op_t) {
    .data = data + 8,
    .len = 8,
  };
  SFIFO_TEST (svm_fifo_enqueue_async (f, &op) == 8, "enqueue second async operation");
  SFIFO_TEST (svm_fifo_n_async_ops (f) == 2, "two operations staged");

  svm_fifo_commit_async_ops (f);
  SFIFO_TEST (svm_fifo_dequeue (f, ARRAY_LEN (output), output) == ARRAY_LEN (output),
	      "dequeue multiple async operations");
  SFIFO_TEST (!clib_memcmp (data, output, ARRAY_LEN (data)), "multiple async operations match");
