...
sfifo_test_fifo_shrink (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  int i, rv; /*j, test_n_bytes, deq_bytes, enq_bytes, n_deqs, n_enqs; */
  ...
  svm_fifo_add_chunk (f, c);
  SFIFO_TEST (f->size == fifo_size + 11 * 100, "size expected %u is %u",
	      fifo_size + 11 * 100, f->size);
  ...
  /* Try to dequeue and force size reduction */
  ...
  /*
   * No fifo wrap and chunks used (multiple chunks)
   */
  svm_fifo_init_pointers (f, f->size, f->size + f->nitems);
  ...
  /* Try to dequeue and force size reduction. Should not work because fifo
   * is full */
  (void) svm_fifo_max_enqueue (f);
  ...
  /* Last chunk still used */
  svm_fifo_init_pointers (f, f->size + f->nitems - chunk_size,
                          f->size + f->nitems);
  rv = svm_fifo_max_enqueue (f);

  SFIFO_TEST (f->size == 11 * chunk_size + 1, "size expected %u is %u",
              11 * chunk_size + 1, f->size);
  SFIFO_TEST (rv == 7 * chunk_size, "free space expected %u is %u",
              7 * chunk_size, rv);
  SFIFO_TEST (f->flags & SVM_FIFO_F_SHRINK, "shrink flag should be set");
  SFIFO_TEST (!(f->flags & SVM_FIFO_F_COLLECT_CHUNKS), "collect flag should"
              " not be set");

  svm_fifo_init_pointers (f, 2 * f->size, 2 * f->size);
  rv = svm_fifo_max_enqueue (f);
