...
sfifo_test_fifo_async (void)
{
  ...
  seg = (svm_fifo_async_seg_t) { .data = data, .len = 8, .opaque = 7 };
  SFIFO_TEST (svm_fifo_enqueue_async_segment (f, &seg) == 8, "enqueue first async segment");
  seg = (svm_fifo_async_seg_t) { .data = data + 8, .len = 8, .opaque = ~0 };
  ...
  SFIFO_TEST (svm_fifo_async_segments (f)[0].opaque == 7, "async reference matches");
  SFIFO_TEST (svm_fifo_commit_async_segments (f) == ARRAY_LEN (data), "commit async segments");
  ...
  for (i = 0; i < 512; i++)
    {
      ...
      SFIFO_TEST (svm_fifo_commit_async_segments (f) == 2 * half, "commit async data iteration %u",
		  i);
  ...
  for (i = 0; i < 512; i++)
    {
      ...
      svm_fifo_add_async_segment (f, &seg);
      SFIFO_TEST (svm_fifo_commit_async_segments (f) == quarter, "commit async prefix iteration %u",
		  i);
