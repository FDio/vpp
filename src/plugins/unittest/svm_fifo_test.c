...
sfifo_test_fifo_no_wrap (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  SFIFO_TEST ((f->default_chunk_size == default_chunk_size),
	      "f->default_chunk_size %u", f->default_chunk_size);
  ...
  SFIFO_TEST ((f->nitems == default_chunk_size - 1), "f->nitems %u",
	      f->nitems);
  SFIFO_TEST ((f->start_chunk == f->head_chunk), "f->start_chunk %p",
	      f->start_chunk);
  SFIFO_TEST ((f->end_chunk == f->tail_chunk), "f->end_chunk %p",
	      f->end_chunk);
  ...
  removed_chunks = (svm_fifo_chunk_t *) 0x12345678;
  svm_fifo_shrink_non_wrapped (f, &removed_chunks);
  SFIFO_TEST ((removed_chunks == tmp_chunk), "removed_chunks %p",
	      removed_chunks);
  SFIFO_TEST ((removed_chunks->next == NULL), "removed_chunks->next %p",
	      removed_chunks->next);
  SFIFO_TEST ((f->head == 0), "f->head %u", f->head);
  SFIFO_TEST ((f->tail == 0), "f->tail %u", f->tail);
  SFIFO_TEST ((f->size == 0), "f->size %u", f->size);
  ...
  SFIFO_TEST ((f->nitems == default_chunk_size * 2 - 1), "f->nitems %u",
	      f->nitems);
  SFIFO_TEST ((f->start_chunk == f->head_chunk), "f->start_chunk %p",
	      f->start_chunk);
  SFIFO_TEST ((f->tail_chunk == f->head_chunk), "f->tail_chunk %p",
	      f->tail_chunk);
  SFIFO_TEST ((f->end_chunk != f->tail_chunk), "f->end_chunk %p",
	      f->end_chunk);
  SFIFO_TEST ((f->end_chunk != f->start_chunk), "f->end_chunk %p",
	      f->end_chunk);
  SFIFO_TEST ((f->end_chunk == f->start_chunk->next), "f->end_chunk %p",
	      f->end_chunk);
  ...
  SFIFO_TEST ((removed_chunks == tmp_chunk), "removed_chunks %p",
	      removed_chunks);
  SFIFO_TEST ((removed_chunks->next == tmp_chunk2), "removed_chunks->next %p",
	      removed_chunks->next);
  SFIFO_TEST ((f->head == 0), "f->head %u", f->head);
  SFIFO_TEST ((f->tail == 0), "f->tail %u", f->tail);
  SFIFO_TEST ((f->size == 0), "f->size %u", f->size);
  ...
  SFIFO_TEST ((f->start_chunk != f->head_chunk), "f->start_chunk %p",
	      f->start_chunk);
  SFIFO_TEST ((f->head_chunk == f->start_chunk->next), "f->head_chunk %p",
	      f->head_chunk);
  SFIFO_TEST ((f->end_chunk == f->tail_chunk), "f->end_chunk %p",
	      f->end_chunk);
  ...
  SFIFO_TEST ((f->nitems == default_chunk_size - 1), "f->nitems %u",
	      f->nitems);
  SFIFO_TEST ((f->start_chunk == tmp_chunk2), "f->start_chunk %p",
	      f->start_chunk);
  SFIFO_TEST ((f->head_chunk == f->start_chunk), "f->head_chunk %p",
	      f->head_chunk);
  SFIFO_TEST ((f->end_chunk == f->tail_chunk), "f->end_chunk %p",
	      f->end_chunk);
  SFIFO_TEST ((f->end_chunk == f->start_chunk), "f->end_chunk %p",
...
int
sfifo_test_fifo_compare_wrap_no_wrap (vlib_main_t * vm,
				      unformat_input_t * input)
{
  ...
  SFIFO_TEST ((f->start_chunk == f->head_chunk), "f->start_chunk %p",
	      f->start_chunk);
  SFIFO_TEST ((f->tail_chunk == f->head_chunk->next), "f->tail_chunk %p",
	      f->tail_chunk);
  SFIFO_TEST ((f->end_chunk == f->tail_chunk), "f->end_chunk %p",
	      f->end_chunk);
  ...
  for (i = 0; i < (1 << 20); ++i)
    {
      ...
      SFIFO_TEST_SILENT ((f->end_chunk == old_end->next),
			 "[%d] f->end_chunk %p", i, f->end_chunk);
      SFIFO_TEST_SILENT ((f->size == size12KB), "[%d] f->size %d", i,
			 f->size);
      ...
      removed_chunks = (svm_fifo_chunk_t *) 0x12345678;
      svm_fifo_shrink_non_wrapped (f, &removed_chunks);
      SFIFO_TEST_SILENT ((removed_chunks == old_start),
			 "[%d] removed_chunks %p", i, removed_chunks);
      SFIFO_TEST_SILENT ((removed_chunks->next == NULL),
			 "[%d] removed_chunks->next %p", i,
			 removed_chunks->next);
      SFIFO_TEST_SILENT ((f->start_chunk != old_start),
			 "[%d] f->start_chunk %p", i, f->start_chunk);
      ...
      /* rbtree search */
      SFIFO_TEST_SILENT ((rb_tree_n_nodes (&f->chunk_lookup) == 3),
			 "[%d] rb_tree_n_nodes %d", i,
			 rb_tree_n_nodes (&f->chunk_lookup));
      for (j = 0; j < 10; ++j)
	{
	  node = rb_tree_search_subtree (&f->chunk_lookup,
					 rb_node (&f->chunk_lookup,
						  f->chunk_lookup.root),
					 f->head & ~(default_chunk_size - 1));
	  SFIFO_TEST_SILENT (((svm_fifo_chunk_t *) node->opaque ==
			      f->head_chunk), "[%d][%d] node->opaque %p", i,
			     j, node->opaque);
	  node =
	    rb_tree_search_subtree (&f->chunk_lookup,
				    rb_node (&f->chunk_lookup,
					     f->chunk_lookup.root),
				    f->tail & ~(default_chunk_size - 1));
	  SFIFO_TEST_SILENT (((svm_fifo_chunk_t *) node->opaque ==
			      f->tail_chunk), "[%d][%d] node->opaque %p", i,
			     j, node->opaque);
	}
    }
  ...
  old_start = f->start_chunk;
  svm_fifo_shrink_non_wrapped (f, &removed_chunks);
  SFIFO_TEST ((removed_chunks == old_start), "removed_chunks %p",
	      removed_chunks);
  fl_index = fs_freelist_for_size (removed_chunks->length);
  fsh->free_chunks[fl_index] = removed_chunks;
  fsh->n_fl_chunk_bytes += fs_freelist_index_to_size (fl_index);
  ...
  for (i = 0; i < (1 << 20); ++i)
    {
      SFIFO_TEST_SILENT ((f2->head ==
			  (size2KB + size4KB * (i % 4)) % size16KB),
			 "[%d] f2->head %d", i, f2->head);
      SFIFO_TEST_SILENT ((f2->tail ==
			  (size6KB + size4KB * (i % 4)) % size16KB),
			 "[%d] f2->tail %d", i, f2->tail);
      ...
      for (j = 0; j < 10; ++j)
	{
	  node = rb_tree_search_subtree (&f2->chunk_lookup,
					 rb_node (&f2->chunk_lookup,
						  f2->chunk_lookup.root),
					 f2->head_chunk->start_byte);
	  SFIFO_TEST_SILENT (((svm_fifo_chunk_t *) node->opaque ==
			      f2->head_chunk), "[%d][%d] node->opaque %p", i,
			     j, node->opaque);
	  node =
	    rb_tree_search_subtree (&f2->chunk_lookup,
				    rb_node (&f2->chunk_lookup,
					     f2->chunk_lookup.root),
				    f2->tail_chunk->start_byte);
	  SFIFO_TEST_SILENT (((svm_fifo_chunk_t *) node->opaque ==
			      f2->tail_chunk), "[%d][%d] node->opaque %p", i,
			     j, node->opaque);
	}
      SFIFO_TEST_SILENT ((f2->start_chunk == old_start),
			 "[%d] f2->start_chunk %p", i, f2->start_chunk);
      SFIFO_TEST_SILENT ((f2->end_chunk == old_end), "[%d] f2->end_chunk %p",
			 i, f2->end_chunk);
    }
	       ...
	       vlib_cli_command_t * cmd_arg)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (input, "all"))
	{
	  ...
	  if ((res = sfifo_test_fifo_compare_wrap_no_wrap (vm, input)))
	    goto done;
