		 ...
		 vnet_crypto_op_id_t id)
{
  ...
  op->keys = (uword) vnet_crypto_get_active_engine_key_data (
    keys, id, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
  ...
  vec_foreach_index (i, rv)
  {
	...
	switch (t)
	  ...
	  case VNET_CRYPTO_OP_TYPE_ENCRYPT:
	    ...
	    op->iv = tm->inc_data;
	    op->keys = (uword) vnet_crypto_get_active_engine_key_data (
	      key, id, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
	    vec_add1 (keys, key);
      ...
      for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++)
	{
	  ...
	  switch (t)
		...
		if (r->is_chained)
		  {
		    op->keys = (uword) vnet_crypto_get_active_engine_key_data (
		      key, id, VNET_CRYPTO_HANDLER_TYPE_CHAINED);
		    pt = r->pt_chunks;
		    ct = r->ct_chunks;
...
test_crypto_perf (vlib_main_t * vm, crypto_test_main_t * tm)
{
  ...
  for (i = 0; i < n_buffers; i++)
	...
	case VNET_CRYPTO_OP_TYPE_DECRYPT:
	  ...
	  op1->keys = (uword) vnet_crypto_get_active_engine_key_data (
	    op_keys, op1->op, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
	  op2->keys = (uword) vnet_crypto_get_active_engine_key_data (
	    op_keys, op2->op, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
