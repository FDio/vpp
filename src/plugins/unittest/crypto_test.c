...
test_crypto_one (vlib_main_t *vm, crypto_test_main_t *tm,
		 ...
		 vnet_crypto_op_id_t op_id, vnet_crypto_engine_t *e,
		 int chained)
{
  ...
  u8 *text_alloc = 0;
  u8 *hash_alloc = 0;
  u8 *expected_text, *calculated_text, *expected_hash, *calculated_hash;
  u32 text_len = 0;
  u32 hash_len = 0;
  int is_decrypt = od->type == VNET_CRYPTO_OP_TYPE_DECRYPT ||
		   od->type == VNET_CRYPTO_OP_TYPE_AEAD_DECRYPT;

  vnet_crypto_op_t op = {
    .op = op_id,
    .key_index = CLIB_U32_MAX,
    .iv = r->iv.data,
  }, *opp = &op;
  ...
  switch (od->type)
    ...
    case VNET_CRYPTO_OP_TYPE_HASH:
      hash_len = r->hash.length;
      break;
    case VNET_CRYPTO_OP_TYPE_HMAC:
      hash_len = r->hmac.length;
      break;
    default:
  ...
  if (text_len)
    {
      ...
      for (u32 i = 0; i < text_len + 6; i++)
	text_alloc[i] = i;
      op.dst = calculated_text;
    }
  ...
  if (od->type == VNET_CRYPTO_OP_TYPE_AEAD_ENCRYPT ||
      od->type == VNET_CRYPTO_OP_TYPE_AEAD_DECRYPT)
    {
      ...
      op.tag = is_decrypt ? r->tag.data : calculated_hash;
      expected_hash = r->tag.data;
    }
  if (od->type == VNET_CRYPTO_OP_TYPE_HASH)
    {
      op.digest = calculated_hash;
      op.digest_len = hash_len;
      expected_hash = r->hash.data;
    }
  if (od->type == VNET_CRYPTO_OP_TYPE_HMAC)
    {
      op.digest = calculated_hash;
      op.digest_len = hash_len;
      expected_hash = r->hmac.data;
    }
  ...
  if (chained)
      ...
      while (len)
	{
      ...
      e->chained_ops_handlers[op_id](vm, &opp, chunks, 1);
    }
  else
    e->ops_handlers[op_id](vm, &opp, 1);
  ...
  if (hash_len && memcmp (expected_hash, calculated_hash, hash_len) != 0)
    {
...
test_crypto (vlib_main_t * vm, crypto_test_main_t * tm)
{
  ...
  FOREACH_ARRAY_ELT (alg, cm->algs)
	...
	continue;
      ...
      if (tm->alg != VNET_CRYPTO_ALG_NONE && alg->index != tm->alg)
	continue;
      ...
      for (r = tm->test_registrations; r; r = r->next)
	{
	  ...
	  FOREACH_ARRAY_ELT (opp, alg->op_by_type)
		...
		continue;
	      ...
	      if (od->type == VNET_CRYPTO_OP_TYPE_HASH && r->hash.length == 0)
		continue;

	      if (od->type == VNET_CRYPTO_OP_TYPE_HMAC && r->hmac.length == 0)
		continue;

	      if (od->type == VNET_CRYPTO_OP_TYPE_DECRYPT &&
		  r->ciphertext.length == 0)
		continue;

	      if (od->type == VNET_CRYPTO_OP_TYPE_AEAD_DECRYPT &&
		  r->ciphertext.length == 0)
		continue;
...
test_crypto_perf (vlib_main_t * vm, crypto_test_main_t * tm)
{
  ...
  if (tm->alg == VNET_CRYPTO_ALG_NONE)
    return clib_error_return (0, "please specify algorithm");
			...
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (input, "%U", unformat_vnet_crypto_alg, &tm->alg))
	;
      else
