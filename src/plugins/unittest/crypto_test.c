...
sort_registrations (void *a0, void *a1)
{
  ...
  if (r0[0]->alg == r1[0]->alg)
    return (r0[0]->op > r1[0]->op);

  return (r0[0]->alg > r1[0]->alg);
}
...
test_crypto (vlib_main_t * vm, crypto_test_main_t * tm)
{
  ...
  vnet_crypto_job_t *jobs = 0, *j, **batch = 0;
  u8 *computed_data = 0;
  ...
  vec_foreach_index (i, rv)
    {
      ...
      j = jobs + i;
      batch[i] = j;
      vlib_cli_output (vm, "test %s, alg %U op %U", r->name,
		       format_vnet_crypto_alg, r->alg,
  ...
  vnet_crypto_enq_jobs (vm, batch, vec_len (batch));
