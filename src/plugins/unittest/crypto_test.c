...
crypto_test_alg_has_fixed_auth_len (vnet_crypto_alg_t alg)
{
  ...
  switch (alg)
    {
#define _(n, s, cf, inf, d, b) case VNET_CRYPTO_ALG_##n:
      foreach_crypto_hash_fixed_alg
#undef _
#define _(c, h, s, cf, inf, k, d, ak, b) case VNET_CRYPTO_ALG_##c##_##h##_ICV##d:
...
static int
crypto_test_alg_has_fixed_aad_len (vnet_crypto_alg_t alg)
{
  vnet_crypto_alg_data_t *ad = crypto_main.algs + alg;

  return ad->alg_type == VNET_CRYPTO_ALG_T_AEAD && ad->variable_aad_len == 0;
}

static clib_error_t *
crypto_test_auth_len (vnet_crypto_alg_t alg, u32 data_len, u32 *auth_len)
{
  u32 fixed_len;

  fixed_len = crypto_main.algs[alg].auth_len;

  if (fixed_len && crypto_test_alg_has_fixed_auth_len (alg))
    {
      ASSERT (data_len == fixed_len);
      *auth_len = fixed_len;
    }
  else
    *auth_len = data_len;
...
static clib_error_t *
crypto_test_aad_len (vnet_crypto_alg_t alg, u32 data_len, u32 *aad_len)
{
  u32 fixed_len;

  fixed_len = crypto_main.algs[alg].aad_len;

  if (fixed_len && crypto_test_alg_has_fixed_aad_len (alg))
    {
      ASSERT (data_len == fixed_len);
      *aad_len = fixed_len;
    }
		 ...
		 vnet_crypto_op_type_t type, vnet_crypto_engine_id_t engine)
{
  ...
  err = crypto_test_auth_len (r->alg, r->digest.length, &auth_len);
  if (err)
    {
      ...
      for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++)
	{
	  ...
	  switch (t)
		...
		crypto_test_ctx_add_data (vm, engine, r->alg, tm->inc_data, r->key.length, 0);
	      ...
	      if (ad->alg_type == VNET_CRYPTO_ALG_T_AEAD)
		{
		  err = crypto_test_auth_len (r->alg, r->tag.length, &auth_len);
		  if (err)
		    goto done;
		  op->aad = tm->inc_data;
		  op->aad_len = r->aad.length;
		  err = crypto_test_auth_buffer_alloc (
		    vec_elt_at_index (encrypt_auth_buffers, op - encrypt_ops), auth_len);
		  if (err)
		    goto done;
      ...
      for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++)
	{
	  ...
	  switch (t)
		...
		crypto_test_ctx_add_data (vm, engine, r->alg, tm->inc_data, r->key.length, 0);
	      ...
	      op->auth_len = auth_len;
	      err =
		crypto_test_auth_buffer_alloc (vec_elt_at_index (auth_buffers, op - ops), auth_len);
	      if (err)
		goto done;
		  ...
		  if (ad->alg_type != VNET_CRYPTO_ALG_T_AEAD)
		      ...
		      if (is_chained)
			{
			  ...
			  if (ad->alg_type == VNET_CRYPTO_ALG_T_COMBINED)
				...
				{
				  err = crypto_test_auth_buffer_alloc (
				    vec_elt_at_index (auth_buffers, op - ops), auth_len);
				  if (err)
				    goto done;
				  op->auth = vec_elt_at_index (auth_buffers, op - ops)->data;
			...
			crypto_test_result_t *test_result)
{
  ...
  vnet_crypto_main_t *cm = &crypto_main;
  vnet_crypto_alg_data_t *ad = cm->algs + r->alg;
  vlib_node_t *enq_node;
  vlib_node_runtime_t *enq_rt;
  vlib_frame_t *enq_frame = 0;
  u32 post_next_index;
  char *engine_name = vec_elt_at_index (cm->engines, engine)->name;
  ...
  *test_result = CRYPTO_TEST_RESULT_FAIL;
  ...
  enq_node = vlib_get_node_by_name (async_vm, (u8 *) "crypto-enq");
  if (enq_node == 0)
    return clib_error_return (0, "crypto-enq node not found");
  enq_rt = vlib_node_get_runtime (async_vm, enq_node->index);
  post_next_index = vnet_crypto_register_post_node (vm, "error-drop");
  if (post_next_index == ~0)
    return clib_error_return (0, "crypto post node registration failed");
  ...
  if (ad->alg_type == VNET_CRYPTO_ALG_T_AEAD)
    {
      rv = crypto_test_auth_len (r->alg, r->tag.length, &digest_len);
      if (rv)
	goto done;
      rv = crypto_test_aad_len (r->alg, r->aad.length, &aad_len);
    }
  else if (crypto_test_op_has_digest (ad, op_type))
    rv = crypto_test_auth_len (r->alg, crypto_test_digest_data (r, op_type)->length, &digest_len);
  if (rv)
    goto done;
  ...
  enq_frame = vlib_get_frame_to_node (vm, enq_node->index);
  if (enq_frame == 0)
    {
      rv = clib_error_return (0, "crypto-enq frame alloc failed");
      goto done;
    }
  ...
  for (i = 0; i < n_elts; i++)
      ...
      if (ctxs[i] == 0)
	{
      ...
      pre_data_off = vnet_crypto_buffer_metadata_pre_data_off (head);
      iv = head->pre_data;
      aad = iv + r->iv.length;
      ...
      if (op_type == VNET_CRYPTO_OP_TYPE_DECRYPT)
	{
	  if (ad->alg_type == VNET_CRYPTO_ALG_T_AEAD && r->tag.length)
	    clib_memcpy (tag, r->tag.data, digest_len);
	  else if (ad->alg_type == VNET_CRYPTO_ALG_T_COMBINED && r->digest.length)
	    {
	      is_hmac_check = 1;
	      clib_memcpy (tag, r->digest.data, digest_len);
	    }
	}
      md = vnet_crypto_buffer_get_metadata (head);
      *md = (vnet_crypto_buffer_metadata_t){
	.cipher_data_len = input->length,
	.auth_data_len = auth_data_len,
	.cipher_data_start_off = 0,
	.auth_data_start_off = 0,
	.iv_off = pre_data_off,
	.icv_off = pre_data_off + r->iv.length + r->aad.length,
	.aad_off = r->aad.length ? pre_data_off + r->iv.length : 0,
	.aad_len = crypto_test_alg_has_fixed_aad_len (r->alg) ? 0xfe : aad_len,
	.icv_len = crypto_test_alg_has_fixed_auth_len (r->alg) ? 0xfe : digest_len,
	.is_hmac_check = is_hmac_check,
	.is_chained_buffers = is_chained,
      };
    }

  {
    u32 *to = vlib_frame_vector_args (enq_frame);
    vnet_crypto_ctx_t **aux = vlib_frame_aux_args (enq_frame);
    vnet_crypto_deq_scalar_data_t *scalar = vlib_frame_scalar_args (enq_frame);

    enq_frame->n_vectors = n_elts;
    clib_memcpy_fast (to, head_indices, n_elts * sizeof (head_indices[0]));
    clib_memcpy_fast (aux, ctxs, n_elts * sizeof (ctxs[0]));
    *scalar = (vnet_crypto_deq_scalar_data_t){
      .next_node_index = post_next_index,
      .op_type = op_type,
    };
    enq_rt->function (async_vm, enq_rt, enq_frame);
    enqueued = 1;
  }

  rv = crypto_test_wait_for_async_frame (vm, engine, &result);
  if (rv)
    goto done;
  ...
  if (mismatch == 0 && ad->alg_type == VNET_CRYPTO_ALG_T_AEAD &&
	...
	if (memcmp (tag, r->tag.data, digest_len))
	  {
  ...
  *test_result = CRYPTO_TEST_RESULT_OK;
  ...
  if (result.frame)
    {
      vnet_crypto_thread_t *ct = crypto_main.threads + result.frame->enqueue_thread_index;

      pool_put (ct->frame_pool, result.frame);
    }
  if (enq_frame)
    vlib_frame_free (vm, enq_frame);
  if (n_alloc && (!enqueued || result.frame))
    {
      if (is_chained)
  ...
  vec_foreach_index (i, ctxs)
    if (ctxs[i] && (result.frame || !enqueued))
...
leak_test_run_alg (vlib_main_t *vm, vnet_crypto_alg_t alg)
{
  ...
  /* Wait for any async handlers to complete before checking counters. */
  ...
  for (t = 0; t < VNET_CRYPTO_HANDLER_N_TYPES; t++)
	...
	continue;
      if (c->add - c->remove > 2)
	{
	  err = clib_error_return (0, "FAIL %s rekey (t=%u): add=%lld remove=%lld diff=%lld (leak)",
				   ad->name, (u32) t, (long long) c->add, (long long) c->remove,
				   (long long) (c->add - c->remove));
	  break;
	}
    }
  ...
  if (err == 0)
      ...
      for (t = 0; t < VNET_CRYPTO_HANDLER_N_TYPES; t++)
	{
	  ...
	  if (c->add != c->remove)
	    {
	      err = clib_error_return (0, "FAIL %s destroy (t=%u): add=%lld remove=%lld", ad->name,
				       (u32) t, (long long) c->add, (long long) c->remove);
	      break;
	    }
