  ...
  vec_foreach_index (i, rv)
      ...
      for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++)
	{
	  ...
	  switch (t)
	    ...
	    case VNET_CRYPTO_OP_TYPE_HASH:
	      op = ops + n_check_ops++;
	      op->ctx = crypto_test_ctx_add_data (vm, engine, r->alg, 0, 0, 0);
	      vec_add1 (ctxs, op->ctx);
	      vnet_crypto_op_init (op->ctx, op);
  ...
  for (i = 0; i < n_elts; i++)
    {
      ...
      ctxs[i] = crypto_test_ctx_add_data (
	async_vm, engine, r->alg, op_type == VNET_CRYPTO_OP_TYPE_HASH ? 0 : r->key.data,
	op_type == VNET_CRYPTO_OP_TYPE_HASH ? 0 : r->key.length, 1);
      if (ctxs[i] == 0)
	{
