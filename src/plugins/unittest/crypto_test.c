			 ...
			 u32 computed_data_total_len)
{
  ...
  vec_foreach_index (i, rv)
  {
	...
	switch (t)
	  ...
	  case VNET_CRYPTO_OP_TYPE_ENCRYPT:
	    vnet_crypto_op_init (op, id);
	    key = vnet_crypto_key_add (vm, r->alg, tm->inc_data, r->key.length);
	    op->iv = tm->inc_data;
	    op->keys = (uword) key;
	    vec_add1 (keys, key);
  ...
  vec_foreach_index (i, rv)
      ...
      for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++)
	{
	  ...
	  switch (t)
	    ...
	    if (!ad->is_aead)
	      {
		key = vnet_crypto_key_add (vm, r->alg, r->key.data, r->key.length);
		op->iv = r->iv.data;
		vec_add1 (keys, key);
...
test_crypto_perf (vlib_main_t * vm, crypto_test_main_t * tm)
{
  ...
  if (ad->is_link)
    {
      op_keys = vnet_crypto_key_add (vm, ad->link_crypto_alg, key,
