			      ...
			      crypto_test_result_t result)
{
  ...
  row = crypto_test_result_table_find_row (rt, r, alg, type, is_chained);
...
static void
crypto_test_result_table_add_reg_rows (crypto_test_result_table_t *rt, vnet_crypto_main_t *cm,
  ...
  vec_foreach (row, rows)
    {
      ...
      if (e->ops[row->alg][row->type].handlers[ht] != 0)
	return 1;
    }
...
crypto_test_result_table_print (vlib_main_t *vm, crypto_test_result_table_t *rt, int failures_only)
{
  ...
  vec_foreach_index (i, rt->rows)
      ...
      vec_foreach_index (k, rt->engines)
	{
	  ...
	  chained_row = crypto_test_result_table_find_row (rt, row->reg, row->alg, row->type, 1);
	...
	table_format_cell (&t, j, 0, "%s", cm->algs[row->alg].name);
				 ...
				 crypto_test_result_table_t *results)
{
  ...
  crypto_test_result_table_set (results, engine, r, alg, type, is_chained,
				CRYPTO_TEST_RESULT_NOT_SUPPORTED);
}
	       ...
	       crypto_test_result_table_t *results)
{
  ...
  for (op_index = 0; op_index < n_ops; op_index++)
	...
	summary->ok[is_chained]++;
      ...
      crypto_test_result_table_set (results, engine, r, op->ctx->alg, op->type, is_chained,
			  ...
			  const u8 *key_data, u16 key_len)
{
  ...
  if (ad->alg_type == VNET_CRYPTO_ALG_T_COMBINED)
    ctx = vnet_crypto_ctx_create (vm, alg, key_data, ad->key_len, key_data + ad->key_len,
				  key_len - ad->key_len);
  else if (ad->alg_type == VNET_CRYPTO_ALG_T_AUTH && key_len == 0)
    ctx = vnet_crypto_ctx_create (vm, alg, 0, 0, 0, 0);
  else
    ctx = vnet_crypto_ctx_create (vm, alg, key_data, key_len, 0, 0);
		 ...
		 vnet_crypto_op_type_t type, vnet_crypto_engine_id_t engine)
{
  ...
  op->ctx = crypto_test_key_add_data (vm, engine, r->alg, cm->inc_data, r->key.length);
  if (op->ctx == 0)
			 ...
			 crypto_test_engine_summary_t *summary, crypto_test_result_table_t *results)
{
      ...
      for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++)
	{
	  ...
	  switch (t)
	    ...
	    case VNET_CRYPTO_OP_TYPE_ENCRYPT:
	      ...
	      op->ctx = crypto_test_key_add_data (vm, engine, r->alg, tm->inc_data, r->key.length);
  ...
  if (n_encrypt_ops)
      ...
      for (i = 0; i < n_encrypt_ops; i++)
	{
	  ...
	  crypto_test_result_table_set (results, engine, r, op->ctx->alg, op->type, 0, result);
	}
    }
  ...
  vec_foreach_index (i, rv)
      ...
      for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++)
	{
	  ...
	  switch (t)
	    ...
	    case VNET_CRYPTO_OP_TYPE_HASH:
	      op = ops + n_check_ops++;
	      op->ctx = crypto_test_key_add_data (vm, engine, r->alg, 0, 0);
	      vec_add1 (ctxs, op->ctx);
	      vnet_crypto_op_init (op->ctx, op);
	      op->type = VNET_CRYPTO_OP_TYPE_HASH;
	      op->src = tm->inc_data;
	      op->len = r->plaintext_incremental;
	      op->auth_len = r->hash.length;
	      op->auth = encrypted_data + computed_data_total_len;
	      computed_data_total_len += r->hash.length;
	      op->user_data = i;
	      break;
  ...
  *rt = (crypto_test_result_table_t){};
  ...
  vec_foreach_index (i, regs)
	...
	continue;
      ...
      crypto_test_result_table_add_reg_rows (rt, cm, regs[i], 1);
    }
			   ...
			   unittest_crypto_test_registration_t *r, vnet_crypto_op_type_t op_type)
{
  ...
  if (ad->alg_type == VNET_CRYPTO_ALG_T_COMBINED)
    ctx = vnet_crypto_ctx_create (vm, r->alg, r->key.data, ad->key_len, r->key.data + ad->key_len,
				  r->key.length - ad->key_len);
  else if (op_type == VNET_CRYPTO_OP_TYPE_HASH)
    ctx = vnet_crypto_ctx_create (vm, r->alg, 0, 0, 0, 0);
  else
    ctx = vnet_crypto_ctx_create (vm, r->alg, r->key.data, r->key.length, 0, 0);
...
test_crypto_async_case (vlib_main_t *vm, crypto_test_main_t *tm,
			...
			vnet_crypto_alg_t alg, vnet_crypto_op_type_t op_type, u8 is_chained,
			crypto_test_result_t *test_result)
{
  ...
  *test_result = CRYPTO_TEST_RESULT_FAIL;
  ...
  frame = vnet_crypto_async_get_frame (async_vm, alg, op_type);
  if (frame == 0)
    {
  ...
  for (i = 0; i < n_elts; i++)
    {
      ...
      ctxs[i] = crypto_test_async_key_add (async_vm, engine, r, op_type);
      if (ctxs[i] == 0)
	{
  ...
  if (result.frame->n_elts != n_elts)
    {
      log_err (0, "%s %s %U-%U %s: frame elt count mismatch got %u expected %u", engine_name,
	       r->name, format_crypto_op_type_short, op_type, format_vnet_crypto_alg, alg,
  ...
  if (vnet_crypto_async_frame_bitmap_count_set_bits (result.frame->engine_error_bitmap))
    {
      log_err (0, "%s %s %U-%U %s: fail-engine-err %U", engine_name, r->name,
	       format_crypto_op_type_short, op_type, format_vnet_crypto_alg, alg,
  ...
  if (mismatch == 0 &&
      (ad->alg_type == VNET_CRYPTO_ALG_T_COMBINED || op_type == VNET_CRYPTO_OP_TYPE_HMAC ||
       op_type == VNET_CRYPTO_OP_TYPE_HASH) &&
  ...
  if (mismatch)
    {
      log_err (0, "%s %s %U-%U %s: elt %u %s", engine_name, r->name, format_crypto_op_type_short,
	       op_type, format_vnet_crypto_alg, alg, is_chained ? "chained" : "simple", fail_idx,
	       mismatch);
      *test_result = CRYPTO_TEST_RESULT_FAIL;
      goto done;
    }
			  ...
			  crypto_test_result_table_t *results)
{
  ...
  while (r)
      ...
      for (i = 0; i < VNET_CRYPTO_OP_N_TYPES; i++)
	{
	  ...
	  for (is_chained = 0; is_chained <= 1; is_chained++)
	    {
	      err = test_crypto_async_case (vm, tm, r, engine, r->alg, i, is_chained, &result);
	      if (err)
		{
		  summary.error[is_chained]++;
		  crypto_test_result_table_set (results, engine, r, r->alg, i, is_chained,
						CRYPTO_TEST_RESULT_ERROR);
		...
		summary.fail[is_chained]++;
	      ...
	      crypto_test_result_table_set (results, engine, r, r->alg, i, is_chained, result);
	    }
	}
...
test_crypto_perf (vlib_main_t * vm, crypto_test_main_t * tm)
{
  ...
  if (ad->alg_type == VNET_CRYPTO_ALG_T_COMBINED)
    {
	  ...
	  vnet_crypto_alg_has_op_type (tm->alg, VNET_CRYPTO_OP_TYPE_DECRYPT) && ad->key_len)
	{
  ...
  if (tm->engine)
    ctx = crypto_test_key_add_data (vm, engine, tm->alg, key_data, key_sz);
  else if (is_combined_alg)
    ctx =
      vnet_crypto_ctx_create (vm, tm->alg, key_data, combined_crypto_key_len,
			      key_data + combined_crypto_key_len, key_sz - combined_crypto_key_len);
  else
    ctx = vnet_crypto_ctx_create (vm, tm->alg, key_data, key_sz, 0, 0);

  for (i = 0; i < VNET_CRYPTO_OP_N_TYPES; i++)
    {
      if (!vnet_crypto_alg_has_op_type (tm->alg, i))
	continue;
      ot = i;
      break;
    }
