		 ...
		 vnet_crypto_op_id_t id)
{
  ...
  op->key_index = vnet_crypto_key_add (vm, r->alg, cm->inc_data, r->key.length, 0, 0);
...
restore_engines (u32 * engs)
{
  ...
  for (i = 1; i < VNET_CRYPTO_N_OP_IDS; i++)
	...
	vnet_crypto_alg_key_data_set_per_thread (&cm->algs[alg], t,
						 ce->key_data_per_thread[t][alg]);
	}
    }
  ...
  vec_foreach_index (i, rv)
  {
	...
	switch (t)
	  ...
	  case VNET_CRYPTO_OP_TYPE_ENCRYPT:
	    vnet_crypto_op_init (op, id);
	    op->iv = tm->inc_data;
	    op->key_index = vnet_crypto_key_add (vm, r->alg, tm->inc_data, r->key.length, 0, 0);
      ...
      for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++)
	{
	  ...
	  switch (t)
	    ...
	    if (!ad->is_aead)
	      {
		op->iv = r->iv.data;
		op->key_index = vnet_crypto_key_add (vm, r->alg, r->key.data, r->key.length, 0, 0);
		vec_add1 (key_indices, op->key_index);
		...
		if (r->is_chained)
		  {
		    op->iv = r->iv.data;
		    op->key_index =
		      vnet_crypto_key_add (vm, r->alg, r->key.data, r->key.length, 0, 0);
		    vec_add1 (key_indices, op->key_index);
		    op->aad = r->aad.data;
		    op->aad_len = r->aad.length;
...
static int
test_crypto_is_combined_alg (vnet_crypto_alg_t alg, u16 *crypto_key_len, u8 *digest_len)
{
  switch (alg)
    {
#define _(c, h, s, k, d)                                                                           \
...
test_crypto_perf (vlib_main_t * vm, crypto_test_main_t * tm)
{
  ...
  is_combined_alg =
    test_crypto_is_combined_alg (tm->alg, &combined_crypto_key_len, &combined_digest_len);
