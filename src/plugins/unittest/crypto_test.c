...
crypto_test_summary_table_print (vlib_main_t *vm, crypto_test_result_table_t *rt, char *title)
{
  ...
  table_add_hdr_row (&t, 8, "", "", "", mode_labels[0], "", "", mode_labels[1], "");
  table_add_hdr_row (&t, 8, "ID", "Engine", metric_labels[0], metric_labels[1], metric_labels[2],
...
test_crypto_perf (vlib_main_t * vm, crypto_test_main_t * tm)
{
  ...
  if (tm->engine)
    key_index = crypto_test_key_add_data (vm, engine, tm->alg, key, key_sz);
