...
static_always_inline vnet_crypto_op_id_t
crypto_test_reg_op_id (vnet_crypto_main_t *cm, unittest_crypto_test_registration_t *r,
		       vnet_crypto_op_type_t type)
{
  if (!crypto_test_reg_has_op_type (r, type))
    return 0;

  return cm->algs[r->alg].op_by_type[type];
}
				       ...
				       unittest_crypto_test_registration_t *r, u8 add_chained)
{
  ...
  for (i = 0; i < VNET_CRYPTO_OP_N_TYPES; i++)
    {
      vnet_crypto_op_id_t id = crypto_test_reg_op_id (cm, r, i);
	       ...
	       crypto_test_result_table_t *results)
{
  ...
  for (op_index = 0; op_index < n_ops; op_index++)
	...
	fail = 1;
      ...
      if (fail)
	log_err (0, "%s %s %U %s: %v", engine_name, r->name, format_vnet_crypto_op, op->op,
		 is_chained ? "chained" : "simple", err);
			  ...
			  const u8 *key_data, u16 key_len)
{
  ...
  if (ad->alg_type == VNET_CRYPTO_ALG_T_COMBINED)
    key = vnet_crypto_key_add (vm, alg, key_data, ad->key_len, key_data + ad->key_len,
			       key_len - ad->key_len);
  else
    key = vnet_crypto_key_add (vm, alg, key_data, key_len, 0, 0);

  if (key == 0)
    return 0;

  vnet_crypto_key_set_engine (key, VNET_CRYPTO_HANDLER_TYPE_SIMPLE, engine);
  vnet_crypto_key_set_engine (key, VNET_CRYPTO_HANDLER_TYPE_CHAINED, engine);
		     ...
		     unittest_crypto_test_registration_t *r)
{
  ...
  if (cm->algs[r->alg].alg_type == VNET_CRYPTO_ALG_T_AUTH && r->digest.length == 0)
    return 0;
...
test_crypto (vlib_main_t *vm, crypto_test_main_t *tm)
{
  ...
  while (r)
      ...
      if (!crypto_test_reg_matches_alg (tm, r))
	{
      ...
      for (i = 0; i < VNET_CRYPTO_OP_N_TYPES; i++)
	{
	  vnet_crypto_op_id_t id = crypto_test_reg_op_id (&crypto_main, r, i);
			   ...
			   unittest_crypto_test_registration_t *r, vnet_crypto_op_type_t op_type)
{
  ...
  if (ad->alg_type == VNET_CRYPTO_ALG_T_COMBINED)
    key = vnet_crypto_key_add (vm, r->alg, r->key.data, ad->key_len, r->key.data + ad->key_len,
			       r->key.length - ad->key_len);
  else
    key = vnet_crypto_key_add (vm, r->alg, r->key.data, r->key.length, 0, 0);

  if (key == 0)
    return 0;

  vnet_crypto_key_set_engine (key, VNET_CRYPTO_HANDLER_TYPE_ASYNC, engine);
			...
			vnet_crypto_op_id_t op_id, u8 is_chained, crypto_test_result_t *test_result)
{
  ...
  *test_result = CRYPTO_TEST_RESULT_FAIL;
  ...
  for (i = 0; i < n_elts; i++)
    {
      ...
      if (op_type != VNET_CRYPTO_OP_TYPE_HASH)
	{
	  keys[i] = crypto_test_async_key_add (async_vm, engine, r, op_type);
	  if (keys[i] == 0)
	    {
      ...
      if (op_type == VNET_CRYPTO_OP_TYPE_DECRYPT)
	{
      ...
      vnet_crypto_async_add_to_frame (async_vm, frame,
				      op_type == VNET_CRYPTO_OP_TYPE_HASH ? 0 : keys[i],
				      input->length, auth_data_len, 0, 0, head_indices[i], 0,
				      iv - head->data, tag - head->data, aad - head->data, flags);
  ...
  if (result.frame->n_elts != n_elts)
    {
      log_err (0, "%s %s %U %s: frame elt count mismatch got %u expected %u", engine_name, r->name,
	       format_vnet_crypto_op, op_id, is_chained ? "chained" : "simple",
	       result.frame->n_elts, n_elts);
      *test_result = CRYPTO_TEST_RESULT_ERROR;
      goto done;
    }

  if (result.frame->n_engine_errors)
    {
      log_err (0, "%s %s %U %s: fail-engine-err %U", engine_name, r->name, format_vnet_crypto_op,
	       op_id, is_chained ? "chained" : "simple", format_uword_bitmap,
	       result.frame->engine_error_bitmap);
      *test_result = CRYPTO_TEST_RESULT_ERROR;
      goto done;
    }

  if (result.frame->n_hmac_errors)
    {
      log_err (0, "%s %s %U %s: fail-bad-hmac %U", engine_name, r->name, format_vnet_crypto_op,
	       op_id, is_chained ? "chained" : "simple", format_uword_bitmap,
  ...
  if (mismatch)
    {
      log_err (0, "%s %s %U %s: elt %u %s", engine_name, r->name, format_vnet_crypto_op, op_id,
	       is_chained ? "chained" : "simple", fail_idx, mismatch);
      *test_result = CRYPTO_TEST_RESULT_FAIL;
      goto done;
    }
  ...
  *test_result = CRYPTO_TEST_RESULT_OK;
  ...
  vec_foreach_index (i, keys)
    if (keys[i] && (result.frame || !submitted))
      vnet_crypto_key_del (async_vm, keys[i]);
	  ...
	  for (is_chained = 0; is_chained <= 1; is_chained++)
	      ...
	      if (err)
		{
		  ...
		  crypto_test_result_table_set (results, engine, r, id, is_chained,
						CRYPTO_TEST_RESULT_ERROR);
		  log_err (0, "%s %s %U %s: %U", engine_name, r->name, format_vnet_crypto_op, id,
