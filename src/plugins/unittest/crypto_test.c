...
static void
crypto_test_result_table_print (vlib_main_t *vm, crypto_test_result_table_t *rt)
{
  ...
  table_format_cell (&t, -2, -1, "");
  table_format_cell (&t, -2, 0, "");
  table_format_cell (&t, -2, 1, "");
  table_format_cell (&t, -1, -1, "");
  table_format_cell (&t, -1, 0, "alg");
  table_format_cell (&t, -1, 1, "type");
  for (i = -2; i <= -1; i++)
    {
  ...
  vec_foreach_index (i, rt->engines)
      ...
      for (is_chained = 0; is_chained <= 1; is_chained++)
	{
	  ...
	  table_format_cell (&t, -1, col, "%s", is_chained ? "c" : "s");
...
crypto_test_summary_table_print (vlib_main_t *vm, crypto_test_result_table_t *rt, char *title)
{
  ...
  table_add_header_col (&t, 8, "", "", "", mode_labels[0], "", "", mode_labels[1], "");
  table_add_header_col (&t, 8, "ID", "Engine", metric_labels[0], metric_labels[1], metric_labels[2],
			metric_labels[0], metric_labels[1], metric_labels[2]);
...
test_crypto (vlib_main_t *vm, crypto_test_main_t *tm)
{
  ...
  if (tm->quiet)
    ...
    {
      if (tm->verbose)
	crypto_test_result_table_print (vm, &results);
			...
			crypto_test_result_t *test_result)
{
  ...
  *test_result = CRYPTO_TEST_RESULT_FAIL;
  ...
  key_index = crypto_test_async_key_add (async_vm, engine, r, op_type);
  has_key = key_index != ~0;
...
test_crypto_perf (vlib_main_t * vm, crypto_test_main_t * tm)
{
  ...
  if (is_combined_alg)
    ...
    {
      key_sz = cm->algs[tm->alg].key_length;
      if (key_sz == 0)
