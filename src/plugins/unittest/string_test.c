...
test_strcpy (vlib_main_t * vm, unformat_input_t * input)
{
  char src[64], dst[64];
  int i;
  size_t s1len = sizeof (dst);	// including null
  errno_t err;
  ...
  /* Size fail */
  err = strcpy_s (dst + 1, s1len - 1, src);
  if (err == EOK)
    return -1;

  /* overlap fail */
  err = strcpy_s (dst, s1len, dst);
  if (err == EOK)
    return -1;

  /* overlap fail */
  err = strcpy_s (dst, s1len, dst + 1);
  if (err == EOK)
    return -1;
...
test_strncpy (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  err = strncpy_s (dst, s1len, src, s1len - 1);
  if (err != EOK)
    return -1;
  ...
  /* truncation, n >= dmax */
  err = strncpy_s (dst, s1len - 1, src, s1len - 1);
  if (err != EOVERFLOW)
    return -1;
  ...
  /* overlap fail */
  err = strncpy_s (dst, s1len, dst + 1, s1len - 1);
  if (err == EOK)
    return -1;

  /* overlap fail */
  err = strncpy_s (dst, s1len, dst, s1len);
  if (err == EOK)
    return -1;
...
test_strcat (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  err = strcat_s (dst, s1len, src);
  if (err != EOK)
    return -1;
  if (strcmp_s (dst, s1len - 1,
  ...
  /* overlap fail */
  err = strcat_s (dst, s1len, dst + 1);
  if (err != EINVAL)
    return -1;

  /* overlap fail */
  err = strcat_s (dst, s1len, dst);
  if (err != EINVAL)
    return -1;
...
test_strncat (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  err = strncat_s (dst, s1len, src, s1len);
  if (err != EOK)
    return -1;
  if (strcmp_s (dst, s1len - 1,
  ...
  err = strncat_s (dst, strnlen_s (dst, sizeof (dst)), s2, s1len);
  if (err != EINVAL)
    return -1;

  /* overlap fail */
  err = strncat_s (dst, s1len, dst + 1, s1len - 1);
  if (err != EINVAL)
    return -1;

  /* overlap fail */
  err = strncat_s (dst, s1len, dst, s1len);
  if (err != EINVAL)
    return -1;
...
test_strtok (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  p2str = 0;
  tok1 = strtok_s (str1, &len, ",", &p2str);
  if (strcmp_s (tok1, strlen (tok1), str1, &indicator) != EOK)
    return -1;
  if (indicator != 0)
    return -1;
