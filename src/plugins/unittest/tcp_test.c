...
/*
 * Regression test for "reduce loss window once per rto congestion episode".
 *
 * The rto handler runs the loss-window reduction (tcp_cc_init_rxt_timeout,
 * which cuts cwnd / for cubic w_max and bumps tr_occurences) only for the rto
 * that starts an episode. It must NOT re-run for a subsequent rto of the same,
 * still-unrecovered episode -- even though rto_boff can be cleared to 0
 ...
 * clear-and-refire and assert the loss reduction happened exactly once
 * (tr_occurences == 1, cwnd not re-collapsed). Without the fix tr_occurences
 * would be 2.
 */
static int
tcp_test_rto_reduce_once_e2e (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  transport_connection_t *tc;
  u8 *appns_id = 0, *data = 0;
  u32 cwnd_after_first, tr_after_first, boff_after_first;
  int error, rv = 0, routes_added = 0, ns_added = 0;
  ...
  /*
   * First rto: starts the episode, reduces the loss window once.
   */
  ...
  if (!TCP_TEST_I ((client_tc->tr_occurences == 1), "first rto reduces once (tr_occurences %u)",
		   client_tc->tr_occurences))
    {
      rv = 1;
      goto cleanup;
    }
  cwnd_after_first = client_tc->cwnd;
  tr_after_first = client_tc->tr_occurences;
   ...
   * tcp_update_rtt does), but does NOT end the episode (snd_una still below
  ...
  /*
   * The crux: the second rto of the same episode must NOT re-run the loss
   * reduction. tr_occurences stays 1 and cwnd is not cut again.
   */
  if (!TCP_TEST_I ((client_tc->tr_occurences == tr_after_first),
		   "second rto of same episode does not reduce again "
		   "(tr_occurences %u, expected %u)",
		   client_tc->tr_occurences, tr_after_first))
    {
      rv = 1;
      goto cleanup;
    }
  if (!TCP_TEST_I ((client_tc->cwnd >= cwnd_after_first),
		   "second rto does not re-collapse cwnd (%u -> %u)", cwnd_after_first,
		   client_tc->cwnd))
    {
      rv = 1;
      goto cleanup;
   ...
   * recovery) is a legitimate escalation and MUST still reduce. The guard keys
   * on tcp_in_recovery (rto recovery), which is false in fast recovery, so this
   * must not be suppressed. Guards against over-broadening the fix to
   * !tcp_in_cong_recovery, which would wrongly skip this reduction.
   */
  ...
  if (!TCP_TEST_I ((client_tc->tr_occurences == tr_after_first + 1),
		   "rto during fast recovery still reduces "
		   "(tr_occurences %u, expected %u)",
		   client_tc->tr_occurences, tr_after_first + 1))
    {
      rv = 1;
      goto cleanup;
    }
