...
#define DSACK_RX_RESET()                                                                           \
  ...
  TCP_TEST (!tc->bt && tc->dsack_retrans_bytes == 200,
	    "counter fallback tracks retransmissions without byte tracker");
  tc->snd_una = tc->snd_congestion;
  tcp_cong_recovery_off (tc);
  tcp_dsack_recovery_save (tc);
  block.start = 1100;
  ...
  vec_add1 (tc->rcv_opts.sacks, block);
  tc->rcv_opts.n_sack_blocks = 1;
  tcp_rcv_sacks (tc, tc->snd_una + 100, &rs);
  TCP_TEST (!(rs.ack_flags & TCP_ACK_F_DSACK_SPURIOUS),
	    "advancing D-SACK does not discard another retransmission's history");
  TCP_TEST (tc->dsack_retrans_bytes == 100, "first D-SACK leaves one retransmission outstanding");
  vec_reset_length (tc->rcv_opts.sacks);
  rs.ack_flags = 0;
  block.start = 1200;
  block.end = 1300;
  ...
  TCP_TEST (!tc->dsack_retrans_bytes, "all D-SACKed bytes clear the outstanding counter");
  ...
  /* Rescue, repeated-RTO and reneging paths make the counter-only fallback
   * ineligible before an ambiguous retransmission can be used for undo. */
  DSACK_RX_RESET ();
  tc->flags |= TCP_CONN_FAST_RECOVERY | TCP_CONN_RECOVERY;
  tcp_dsack_recovery_start (tc);
  tcp_dsack_track_retransmit (tc, 1100, 1200);
  ...
  tc->rcv_opts.n_sack_blocks = 1;
  tcp_rcv_sacks (tc, tc->snd_una, &rs);
  TCP_TEST (!(rs.ack_flags & TCP_ACK_F_DSACK_SPURIOUS) && !tcp_dsack_undo_disabled (tc),
  ...
  vec_add1 (tc->rcv_opts.sacks, block);
  tc->rcv_opts.n_sack_blocks = 1;
  tcp_rcv_sacks (tc, tc->snd_congestion, &rs);
  TCP_TEST (tcp_dsack_ineligible (tc) && !(rs.ack_flags & TCP_ACK_F_DSACK_SPURIOUS),
...
tcp_test_rto_rpc (void *argp)
{
  ...
  a->dsack_ineligible_after_first = tcp_dsack_ineligible (tc);
  ...
  a->dsack_ineligible_after_second = tcp_dsack_ineligible (tc);
...
tcp_test_rto_reduce_once_e2e (vlib_main_t *vm, unformat_input_t *input)
{
   ...
   * RPC state valid until completion. */
  {
    ...
    if (!TCP_TEST_I (a->dsack_ineligible_after_second,
		     "second rto makes counter-only D-SACK undo ineligible"))
      {
	rv = 1;
	goto cleanup;
      }
