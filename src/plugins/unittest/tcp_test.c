...
tcp_test_delivery (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  TCP_TEST (tcp_bt_is_sane (bt), "tracker should be sane");
  TCP_TEST (pool_elts (bt->samples) == 6,
	    "samples with distinct retransmission state stay split, got %u",
	    pool_elts (bt->samples));
...
tcp_test_bt (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  TCP_TEST (bt->rxt_in_flight == 200, "tracker should own 200 retransmission bytes");
  ...
  TCP_TEST (bt->rxt_in_flight == 225, "middle retransmission should add 25 active bytes");
  ...
  TCP_TEST (bt->rxt_in_flight == 250, "tail retransmission should add 25 active bytes");
  ...
  tc->snd_nxt += 300;
  {
    ...
    tc->snd_rxt_bytes = bt->rxt_in_flight + 100;
    tcp_fastrecovery_on (tc);
    TCP_TEST (tcp_bt_track_rxt (tc, 100, 200) == 100,
	      "retransmission should replace 100 active bytes");
    TCP_TEST (bt->rxt_in_flight == 100 && tc->rxt_delivered == 100,
	      "replacement should keep one copy active");
    tcp_fastrecovery_off (tc);
...
tcp_test_rack (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  bts = pool_elt_at_index (tc->bt->samples, tc->bt->head);
  tcp_bt_sample_mark_lost (tc, bts);
  bts = pool_elt_at_index (tc->bt->samples, bts->next);
  bts->flags |= TCP_BTS_IS_SACKED;
  tcp_cong_recovery_off (tc);
  tcp_rack_recovery_start (tc);
  TCP_TEST (tc->snd_rxt_bytes == 100, "RACK excludes lost but retains SACKed retransmissions");
  tcp_test_rack_cleanup (tc);
  ...
  tc->flags |= TCP_CONN_RXT_PENDING;
  tcp_rack_recovery_start (tc);
  TCP_TEST (tc->bt->rxt_in_flight == 200 && tc->snd_rxt_bytes == 200 && tc->rxt_delivered == 0,
	    "recovery starts from active retransmission copies");

  bts = pool_elt_at_index (tc->bt->samples, tc->bt->head);
  TCP_TEST (tcp_bt_sample_mark_lost (tc, bts) == 100,
	    "losing retransmission retires one active copy");
  TCP_TEST (tc->bt->rxt_in_flight == 100 && tc->rxt_delivered == 100,
	    "lost retransmission is derived as delivered from flight");
  ...
  tcp_bt_sample_delivery_rate (tc, &rs);
  TCP_TEST (tc->bt->rxt_in_flight == 100 && tc->rxt_delivered == 100,
	    "plain SACK keeps the retransmission copy active");
  ...
  clib_memset (&rs, 0, sizeof (rs));
  tcp_bt_sample_delivery_rate (tc, &rs);
  TCP_TEST (tc->bt->rxt_in_flight == 0 && tc->rxt_delivered == tc->snd_rxt_bytes,
	    "cumulative ACK retires the active retransmission copy");
