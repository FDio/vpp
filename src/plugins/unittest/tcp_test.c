...
tcp_test_rack (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  /* A SACK for a newer transmission must not cause immediate loss. The
   * threshold includes RACK.rtt, and only the expired byte-tracker sample is
   * split out of a larger scoreboard hole. */
  ...
  tcp_rack_sample_acked (tc, bts, bts->max_seq);
  ...
  /* A retransmission is a fresh transmission. Once RACK declares that copy
   * lost it must be counted again and high_rxt must be rewound, even though
   * the underlying scoreboard range was already classified lost. */
  tcp_test_rack_init (tc, thread_index);
  ...
  lost = tcp_rack_detect_loss (tc, &next_to);
  TCP_TEST (lost == 100 && tc->rack_rxt_loss_segs == 1,
	    "lost retransmission counted as a new loss");
  TCP_TEST (tc->sack_sb.high_rxt == 0, "lost retransmission rewinds high_rxt");
  tcp_test_rack_cleanup (tc);

  /* With no valid RACK RTT, RTO marks only the first outstanding sample. */
  tcp_test_rack_init (tc, thread_index);
  ...
  /* Timestamp echo older than the retransmission makes its ACK ambiguous. */
  tcp_test_rack_init (tc, thread_index);
  ...
  tc->rcv_opts.tsecr = bts->tx_tsval - 1;
  tcp_test_set_time (thread_index, 4.10);
  tcp_rack_sample_acked (tc, bts, bts->max_seq);
  TCP_TEST (tc->rack_rtt == 0.0, "ambiguous retransmission ACK does not advance RACK");
  tc->rcv_opts.tsecr = bts->tx_tsval;
  tcp_rack_sample_acked (tc, bts, bts->max_seq);
  TCP_TEST (tc->rack_rtt > 0.0, "matching timestamp echo accepts retransmission ACK");
  tcp_test_rack_cleanup (tc);
