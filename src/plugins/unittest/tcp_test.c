...
/*
 * TFO test fixture helpers
 *
 * The TFO unit tests share two patterns of global-state setup. Factor them
 * into file-local helpers so individual tests can stay focused on their
 * specific assertions, and the boilerplate has a single source of truth.
 */

/**
 * Ensure TFO global state (rwlock, hash tables, LRU sentinel heads) is
 * initialised. Idempotent: safe to call from every test, regardless of
 * whether tcp_fastopen_init() has already run in this process.
 */
static void
tcp_tfo_test_init_globals (void)
{
...
/**
 * Install a deterministic 16-byte cookie key and pin the rotation
 * timestamp far in the future so cookie generation/validation tests
 * see a stable key throughout the test. Clears the previous-key slot
 * so dual-key validation does not fire.
 */
static void
tcp_tfo_test_install_key (vlib_main_t *vm, const u8 raw_key[16])
{
  tcp_main_t *tm = &tcp_main;
  tcp_tfo_test_init_globals ();
  clib_memcpy_fast (tm->tfo_key, raw_key, 16);
  tm->tfo_key_prev[0] = 0;
  tm->tfo_key_prev[1] = 0;
 ...
 * Negative cases (bit-flips, wrong lengths, cross-IP spoofing) are
 * exhaustively covered in tcp_test_tfo_invalid_cookie.
 */
static int
tcp_test_tfo_cookie (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  u8 len = 0;
  static const u8 raw_key[16] = { 0xde, 0xad, 0xbe, 0xef, 0xca, 0xfe, 0xba, 0xbe,
				  0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef };

  clib_memset (tc, 0, sizeof (*tc));
  tcp_tfo_test_install_key (vm, raw_key);
...
tcp_test_tfo_options (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  opts_in.tfo_cookie = 0; /* cookie request: no cookie buffer */
  ...
  /* Build a full TCP header + options and parse it back */
  {
    u8 pkt[80];
  ...
  u8 test_cookie[TCP_TFO_COOKIE_LEN_MAX];
  clib_memset (&opts_in, 0, sizeof (opts_in));
  opts_in.flags = TCP_OPTS_FLAG_TFO;
  opts_in.tfo_cookie_len = 8;
  {
    int i;
    for (i = 0; i < 8; i++)
      test_cookie[i] = 0xa0 + i;
  }

  clib_memset (buf, 0, sizeof (buf));
  opts_in.tfo_cookie = test_cookie;
  opts_len = tcp_options_write (buf, &opts_in);

  {
    ...
    TCP_TEST (opts_out.tfo_cookie_len == 8, "cookie len should be 8, got %u",
	      opts_out.tfo_cookie_len);
    u8 *parsed_cookie = tcp_options_get_tfo_cookie (th);
    TCP_TEST (parsed_cookie != 0, "tcp_options_get_tfo_cookie should find cookie");
    TCP_TEST (!clib_memcmp (parsed_cookie, test_cookie, 8), "cookie data should match");
  }
 ...
 * connections. IPv4 vs IPv6 isolation in the cookie cache and blackhole
 * tables is covered by tcp_test_tfo_cache and tcp_test_tfo_blackhole.
 */
static int
tcp_test_tfo_ipv6_e2e (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  u8 cookie_out[TCP_TFO_COOKIE_LEN_MAX];
  u8 len = 0, len_out = 0;
  static const u8 raw_key[16] = { 0xfe, 0xed, 0xfa, 0xce, 0x01, 0x23, 0x45, 0x67,
...
tcp_test_tfo_key_rotation (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  /* Install a known initial key. install_key pins tfo_key_rotate_ts far
   * in the future, so the first cookie is produced with the ORIGINAL key
   * (no rotation yet); we later backdate the timestamp to force rotation
   * and verify that old cookies still validate via the previous-key slot. */
  static const u8 raw_key[16] = { 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa,
  ...
  TCP_TEST (tm->tfo_key_prev[0] == 0 && tm->tfo_key_prev[1] == 0,
	    "prev key must be empty before any rotation");
  ...
  /* Verify rotation: prev slot must now hold the ORIGINAL key */
  TCP_TEST (tm->tfo_key_prev[0] == 0xaaaaaaaaaaaaaaaaULL &&
	      tm->tfo_key_prev[1] == 0xbbbbbbbbbbbbbbbbULL,
	    "after 1st rotation: tfo_key_prev must hold the original key");
