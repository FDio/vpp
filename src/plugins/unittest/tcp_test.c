...
tcp_test_rto_reduce_once_e2e (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  if (!TCP_TEST_I ((client_tc->snd_nxt != client_tc->snd_una),
		   "client has data in flight"))
    {
      rv = 1;
      goto cleanup;
    }
  ...
  if (!TCP_TEST_I (tcp_in_recovery (client_tc),
		   "first rto enters recovery"))
    {
      rv = 1;
      goto cleanup;
    }
  if (!TCP_TEST_I ((client_tc->tr_occurences == 1),
		   "first rto reduces once (tr_occurences %u)",
  ...
  client_tc->rto_boff = 0;
  if (!TCP_TEST_I (tcp_in_recovery (client_tc),
		   "still in recovery before second rto"))
    {
      rv = 1;
      goto cleanup;
    }
  ...
  if (!TCP_TEST_I ((client_tc->cwnd >= cwnd_after_first),
		   "second rto does not re-collapse cwnd (%u -> %u)",
		   cwnd_after_first, client_tc->cwnd))
    {
      rv = 1;
      goto cleanup;
