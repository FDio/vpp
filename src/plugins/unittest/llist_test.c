...
llist_test_basic (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  llist_index_t head, head2, head3;
  u32 old_tail;
  ...
  LLIST_TEST (!llist_is_empty (pelts, ll_test2, he2),
	      "list should not be empty");

  i--;
  /* *INDENT-OFF* */
  llist_foreach_reverse (pelts, ll_test2, he2, e, ({
    if (i != e->data)
	LLIST_TEST (0, "incorrect element i = %u data = %u", i, e->data);
    i--;
  }));
  ...
  head3 = llist_make_head (pelts, ll_test);
  for (i = 0; i < 10; i++)
    {
      pool_get (pelts, e);
      e->data = 300 + i;
      he3 = llist_pool_entry (pelts, head3);
      llist_add (pelts, ll_test, e, he3);
    }

  he = llist_pool_entry (pelts, head);
  he3 = llist_pool_entry (pelts, head3);
  list_test_is_sane (pelts, ll_test, he3);
  e = llist_prev (pelts, ll_test, he);
  old_tail = e->data;

  /*
   * Splice third list into the tail of the first
   */
  llist_splice (pelts, ll_test, e, he3);

  list_test_is_sane (pelts, ll_test, he);
  LLIST_TEST (llist_is_empty (pelts, ll_test, he3), "should be empty");

  e = llist_prev (pelts, ll_test, he);
  LLIST_TEST (e->data == 300, "data for last spliced should be 309 is %u",
	      e->data);
  for (i = 0; i < 10; i++)
    {
      if (e->data != 300 + i)
	LLIST_TEST (0, "incorrect element i = %u data = %u", i, e->data);
      e = llist_prev (pelts, ll_test, e);
    }
