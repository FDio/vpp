...
static clib_error_t *
test_ipsec_spd_outbound_fun_command_fn (vlib_main_t *vm,
					...
					vlib_cli_command_t *cmd)
{
  ...
  for (i = 0; i < 5; i++)
    {
      p[i].type = IPSEC_SPD_POLICY_IP4_OUTBOUND;
      p[i].priority = 10;
      p[i].policy = IPSEC_POLICY_ACTION_PROTECT;
      p[i].id = spd_id;
      p[i].sa_id = sa_id;
      p[i].protocol = 0;
      p[i].lport.start = 1;
      ...
      p[i].rport.stop = 1;
      /* address: 1.0.0.0 */
      p[i].laddr.start.ip4.data_u32 = clib_host_to_net_u32 (16777216 + i * 32);
      p[i].laddr.stop.ip4.data_u32 = clib_host_to_net_u32 (16777216 + i * 32);
      p[i].raddr.start.ip4.data_u32 = clib_host_to_net_u32 (16777216 + i * 32);
      p[i].raddr.stop.ip4.data_u32 = clib_host_to_net_u32 (16777216 + i * 32);
    }

  vlib_cli_output (vm, "Add SPD Policy:");
  for (i = 0; i < 5; i++)
    {
      rv = ipsec_add_del_policy (vm, &p[i], is_add, &stat_index);
      if (rv)
	{
	...
	vlib_cli_output (vm, "\tAdd SPD Policy: %u", stat_index);
    }
  ...
  for (i = 0; i < 5; i++)
    {
      p0 = ipsec_output_policy_match (spd0, 0, 16777216 + i * 32,
				      16777216 + i * 32, 1, 1, 0);
      if (PREDICT_TRUE (p0 != NULL))
	{
	  if (p0->laddr.start.ip4.data_u32 == p[i].laddr.start.ip4.data_u32 &&
	      p0->laddr.stop.ip4.data_u32 == p[i].laddr.stop.ip4.data_u32 &&
	      p0->raddr.start.ip4.data_u32 == p[i].raddr.start.ip4.data_u32 &&
	      p0->raddr.stop.ip4.data_u32 == p[i].raddr.stop.ip4.data_u32)
	    vlib_cli_output (vm, "\tFound SPD Policy: %d", i);
	}
      else
	{
	  clib_warning ("Not found SPD Policy: %d", i);
	  err = clib_error_return (0, "found SPD Policy failure");
	  p0 = 0;
	}
      ASSERT (p0 != 0);
    }
  ...
  for (i = 0; i < 5; i++)
	...
	vlib_cli_output (vm, "\tDelete SPD Policy: %u", i);
    }
  ...
  for (i = 0; i < n_lookup; i++)
      ...
      if (p0 == NULL)
	{
	  if (burst_enabled)
		...
		spd0, 0, (ip4_start + ((flows - 1) - rand_val[j]) * 32),
...
VLIB_CLI_COMMAND (test_ipsec_spd_fun_command, static) = {
  .path = "test ipsec_spd_outbound",
  .short_help = "test ipsec_spd_outbound",
  .function = test_ipsec_spd_outbound_fun_command_fn,
};
