...
static uword
link_speed_test_interface_tx (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static clib_error_t *
link_speed_test_set_link_speed (vnet_main_t *vnm, u32 hw_if_index, vnet_hw_if_speed_t speed)
{
  ...
  vnet_hw_interface_set_link_speed (vnm, hw_if_index, vnet_hw_if_speed_to_kbps (speed));
  return 0;
}
...
link_speed_test_create_interface (vlib_main_t *vm)
{
  ...
  hw->supported_link_speeds = VNET_HW_IF_SPEED_10G | VNET_HW_IF_SPEED_25G | VNET_HW_IF_SPEED_100G;
...
static clib_error_t *
test_link_speed_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  link_speed_test_main_t *tm = &link_speed_test_main;
  vnet_main_t *vnm = vnet_get_main ();
  ...
  LINK_SPEED_TEST_ASSERT (tm->set_speed_call_count == 1, "driver callback invoked (count=%u)",
			  tm->set_speed_call_count);
  LINK_SPEED_TEST_ASSERT (tm->last_requested_speed == VNET_HW_IF_SPEED_10G,
			  "driver received SPEED_10G (got 0x%x)", tm->last_requested_speed);

  /* Verify hw->link_speed was updated by driver callback */
  {
    vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, tm->hw_if_index);
    LINK_SPEED_TEST_ASSERT (hw->link_speed == 10000000, "hw->link_speed == 10000000 (got %u)",
			    hw->link_speed);
  }
  ...
  LINK_SPEED_TEST_ASSERT (tm->last_requested_speed == VNET_HW_IF_SPEED_25G,
			  "driver received SPEED_25G (got 0x%x)", tm->last_requested_speed);
  ...
  LINK_SPEED_TEST_ASSERT (tm->last_requested_speed == VNET_HW_IF_SPEED_100G,
			  "driver received SPEED_100G (got 0x%x)", tm->last_requested_speed);
  ...
  LINK_SPEED_TEST_ASSERT (vnet_hw_if_speed_from_kbps (10000000) == VNET_HW_IF_SPEED_10G,
			  "10000000 Kbps -> SPEED_10G");
  LINK_SPEED_TEST_ASSERT (vnet_hw_if_speed_from_kbps (25000000) == VNET_HW_IF_SPEED_25G,
			  "25000000 Kbps -> SPEED_25G");
  LINK_SPEED_TEST_ASSERT (vnet_hw_if_speed_from_kbps (100000000) == VNET_HW_IF_SPEED_100G,
			  "100000000 Kbps -> SPEED_100G");
  LINK_SPEED_TEST_ASSERT (vnet_hw_if_speed_from_kbps (99999) == VNET_HW_IF_SPEED_UNKNOWN,
			  "99999 Kbps -> SPEED_UNKNOWN");
  LINK_SPEED_TEST_ASSERT (vnet_hw_if_speed_to_kbps (VNET_HW_IF_SPEED_10G) == 10000000,
			  "SPEED_10G -> 10000000 Kbps");
  LINK_SPEED_TEST_ASSERT (vnet_hw_if_speed_to_kbps (VNET_HW_IF_SPEED_UNKNOWN) == 0,
			  "SPEED_UNKNOWN -> 0 Kbps");

  /* Test 7: Verify supported_link_speeds bitmask */
  {
    vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, tm->hw_if_index);
    LINK_SPEED_TEST_ASSERT (hw->supported_link_speeds ==
