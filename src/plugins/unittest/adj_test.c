...
/*
 * These unit tests exercise the same public BFD bridge payload contract used
 * by production code, without constructing private BFD session objects.
 */
void adj_bfd_notify (bfd_vnet_bridge_listener_t *listener, bfd_vnet_bridge_args_t *args);

/*
 * Delivers a synthetic BFD bridge event directly to the ADJ listener.
 */
static void
