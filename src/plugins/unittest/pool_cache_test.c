...
VLIB_POOL_CACHE_DEFINE (test_mw, pool_cache_test_elt_t) = {
  .name = "test_mw",
  .format_element = format_pool_cache_test_element,
};
VLIB_POOL_CACHE_DEFINE (test_cli, pool_cache_test_elt_t) = {
  .name = "pool-cache-cli",
  .format_element = format_pool_cache_test_element,
};
...
{
  test_cli_pool_cache_t pool_a, pool_b;
  vlib_pool_cache_main_t *pcm = &pool_cache_main;
  vlib_pool_cache_t *c;
  clib_error_t *error = 0;
  u8 found_a = 0, found_b = 0;
  u32 idx, other_idx, reused_idx;
  u8 *formatted = 0;
  ...
  test_cli_pool_cache_init_with_batch (&pool_a, "pool-cache-instance-a",
				       POOL_CACHE_TEST_LOG2_SUBPOOL_SIZE, CLIB_CACHE_LINE_BYTES,
				       POOL_CACHE_TEST_CACHE_BATCH_SIZE);
  test_cli_pool_cache_init (&pool_b, "pool-cache-instance-b", POOL_CACHE_TEST_LOG2_SUBPOOL_SIZE,
			    CLIB_CACHE_LINE_BYTES);

  if (pool_a.state.batch_size != POOL_CACHE_TEST_CACHE_BATCH_SIZE ||
      pool_b.state.batch_size != VLIB_POOL_CACHE_DEFAULT_BATCH_SIZE)
    error = clib_error_return (0, "pool-cache batch-size initialization mismatch");

  idx = test_cli_pool_cache_alloc (&pool_a);
  e = test_cli_pool_cache_elt_at_index (&pool_a, idx);
  e->thread_index = 0;
  e->round = 1;
  e->slot = 2;
  e->magic = POOL_CACHE_TEST_MAGIC;
  formatted = test_cli_pool_cache_format_element (formatted, &pool_a, idx);
  if (vec_len (formatted) == 0)
    error = clib_error_return (0, "pool-cache element formatter returned empty output");
  vec_free (formatted);

  if (error == 0 && test_cli_pool_cache_get_if_live (&pool_a, idx) != e)
    error = clib_error_return (0, "pool-cache live lookup failed");
  if (error == 0 && !test_cli_pool_cache_retire (&pool_a, idx))
    error = clib_error_return (0, "pool-cache retirement failed");
  if (error == 0 && test_cli_pool_cache_get_if_live (&pool_a, idx) != 0)
    error = clib_error_return (0, "pool-cache retired index remained live");
  if (error == 0 && test_cli_pool_cache_is_free_index (&pool_a, idx))
    error = clib_error_return (0, "pool-cache retired index reported free");
  if (error == 0 && e->magic != POOL_CACHE_TEST_MAGIC)
    error = clib_error_return (0, "pool-cache retirement poisoned the element");
  if (error == 0 && test_cli_pool_cache_retire (&pool_a, idx))
    error = clib_error_return (0, "pool-cache duplicate retirement succeeded");
  if (error == 0 && test_cli_pool_cache_free_internal_ (&pool_a, idx))
    error = clib_error_return (0, "pool-cache immediate free of retired index succeeded");

  if (error == 0)
    error = pool_cache_test_verify_conservation (&pool_a.state);

  if (error == 0)
    {
      other_idx = test_cli_pool_cache_alloc (&pool_a);
      if (other_idx == idx)
	error = clib_error_return (0, "pool-cache allocated a retired index");
      test_cli_pool_cache_free (&pool_a, other_idx);
    }
  if (error == 0 && !test_cli_pool_cache_reclaim (&pool_a, idx))
    error = clib_error_return (0, "pool-cache reclaim failed");
  if (error == 0 && test_cli_pool_cache_reclaim (&pool_a, idx))
    error = clib_error_return (0, "pool-cache duplicate reclaim succeeded");
  if (error == 0 && !test_cli_pool_cache_is_free_index (&pool_a, idx))
    error = clib_error_return (0, "pool-cache reclaimed index did not report free");
  if (error == 0)
    error = pool_cache_test_verify_conservation (&pool_a.state);
  if (error == 0)
    {
      reused_idx = test_cli_pool_cache_alloc (&pool_a);

      if (test_cli_pool_cache_get_if_live (&pool_a, reused_idx) == 0)
	error = clib_error_return (0, "pool-cache reclaimed index was not live after allocation");
      test_cli_pool_cache_free (&pool_a, reused_idx);
    }

  if (error == 0 &&
      (test_cli_pool_cache_get_if_live (&pool_a, VLIB_POOL_CACHE_INVALID_INDEX) != 0 ||
       test_cli_pool_cache_retire (&pool_a, VLIB_POOL_CACHE_INVALID_INDEX) ||
       test_cli_pool_cache_reclaim (&pool_a, VLIB_POOL_CACHE_INVALID_INDEX)))
    error = clib_error_return (0, "pool-cache invalid retirement operation succeeded");

  idx = test_cli_pool_cache_alloc (&pool_b);
  test_cli_pool_cache_free (&pool_b, idx);
  ...
  if (error == 0 && (!found_a || !found_b))
    error = clib_error_return (0, "pool-cache same-registration instances not found");

  test_cli_pool_cache_free_resources (&pool_a);
  test_cli_pool_cache_free_resources (&pool_b);

  if (error == 0 && (pool_cache_test_cache_is_initialized (&pool_a.state) ||
		     pool_cache_test_cache_is_initialized (&pool_b.state)))
    error = clib_error_return (0, "pool-cache same-registration instance still initialized");
...
pool_cache_cli_test_free_all (pool_cache_cli_test_main_t *ptm)
{
  ...
  for (i = 0; i < vec_len (ptm->indices); i++)
    test_cli_pool_cache_free (&ptm->pool, ptm->indices[i]);
			  ...
			  u32 cache_batch_size)
{
  ...
  test_cli_pool_cache_init_with_batch (&ptm->pool, "pool-cache-cli", log2_subpool_size,
				       CLIB_CACHE_LINE_BYTES, cache_batch_size);
  ptm->active = 1;
  return 0;
}
...
pool_cache_cli_test_alloc (pool_cache_cli_test_main_t *ptm, u32 count)
{
  ...
  for (i = 0; i < count; i++)
    {
      idx = test_cli_pool_cache_alloc (&ptm->pool);
      vec_add1 (ptm->indices, idx);
      if (PREDICT_FALSE (test_cli_pool_cache_is_free_index (&ptm->pool, idx)))
	return clib_error_return (0, "pool-cache-cli allocated index %u reports free", idx);
      e = test_cli_pool_cache_elt_at_index (&ptm->pool, idx);
  ...
  for (i = 0; i < count; i++)
    {
      ...
      test_cli_pool_cache_free (&ptm->pool, idx);
      if (PREDICT_FALSE (!test_cli_pool_cache_is_free_index (&ptm->pool, idx)))
...
pool_cache_cli_test_cleanup (pool_cache_cli_test_main_t *ptm)
{
  ...
  test_cli_pool_cache_free_resources (&ptm->pool);
  ptm->active = 0;
}
...
pool_cache_test_slot_state (vlib_pool_cache_t *c, u32 idx)
{
  u32 pidx = vlib_pool_cache_subpool_index (c, idx);
  u32 elt_index = vlib_pool_cache_elt_index (c, idx);
...
static_always_inline void
pool_cache_test_verify_state_at (pool_cache_test_main_t *ptm, u32 *indices, u32 start, u32 end,
				 u8 expected, const char *caller_func, u32 caller_line)
{
  u32 i;

  for (i = start; i < end; i++)
    {
      u8 actual = pool_cache_test_slot_state (&ptm->pool.state, indices[i]);
      if (PREDICT_FALSE (actual != expected))
	pool_cache_test_state_error_from (ptm, caller_func, caller_line, indices[i], i, expected,
					  actual);
    }
}

#define pool_cache_test_verify_state(ptm, indices, start, end, expected)                           \
  pool_cache_test_verify_state_at ((ptm), (indices), (start), (end), (expected), __func__, __LINE__)
...
pool_cache_test_error_report (pool_cache_test_main_t *ptm, const char *test_name)
{
  ...
  if (ptm->first_error_has_state)
    {
      u32 pidx = vlib_pool_cache_subpool_index (&ptm->pool.state, ptm->first_error_index);
      u32 elt_index = vlib_pool_cache_elt_index (&ptm->pool.state, ptm->first_error_index);
...
pool_cache_test_verify_conservation (vlib_pool_cache_t *c)
{
  ...
  u8 *seen = 0;
  u64 allocated = 0, retired = 0, cached = 0, global, total;
  ...
  for (u64 n = 0; n < global; n++)
      ...
      if (idx >= total || seen[idx])
	{
      ...
      if (pool_cache_test_slot_state (c, idx) != VLIB_POOL_CACHE_SLOT_FREE)
	{
	  error = clib_error_return (0, "non-free global index %u", idx);
	  goto unlock;
	}
      seen[idx] = 1;
      i = vlib_pool_cache_next_free (c, idx);
    }
  ...
  vec_foreach (pt, c->per_thread)
      ...
      for (i = 0; i < n; i++)
	{
	  ...
	  seen[idx] = 1;
	  idx = vlib_pool_cache_next_free (c, idx);
	}
  ...
  for (i = 0; i < n_subpools; i++)
	...
	{
	  u32 idx = vlib_pool_cache_encode_index (c, i, j);
	  u8 state = clib_atomic_load_acq_n (&sp->slot_state[j]);
	  if (state == VLIB_POOL_CACHE_SLOT_ALLOCATED)
	    {
	      allocated++;
	      if (seen[idx])
		{
	  ...
	  else if (state == VLIB_POOL_CACHE_SLOT_RETIRED)
	    {
	      retired++;
	      if (seen[idx])
		{
		  error = clib_error_return (0, "retired index %u appears on a free list", idx);
		  goto unlock;
		}
	    }
	  else if (state == VLIB_POOL_CACHE_SLOT_FREE && !seen[idx])
	    {
	      error = clib_error_return (0, "free index %u appears on no free list", idx);
	      goto unlock;
	    }
	  else if (state != VLIB_POOL_CACHE_SLOT_FREE)
	    {
	      error = clib_error_return (0, "index %u has invalid state %u", idx, state);
	      goto unlock;
	    }
	}
    }

  if (retired != clib_atomic_load_relax_n (&c->n_retired))
    error = clib_error_return (0, "retired counter %llu does not match %llu retired slots",
			       clib_atomic_load_relax_n (&c->n_retired), retired);
  else if (allocated + retired + cached + global != total)
    error = clib_error_return (
      0,
      "pool cache conservation failed: allocated %llu retired %llu cached %llu global %llu "
      "total %llu",
      allocated, retired, cached, global, total);
			    ...
			    u32 *indices, int duplicate_first)
{
  pool_cache_test_free_range (ptm, expected_thread_index, round, indices, 0,
			      POOL_CACHE_TEST_BATCH_SIZE, duplicate_first);
}
			  ...
			  CLIB_UNUSED (vlib_frame_t *frame))
{
   ...
   * first free also verifies duplicate cross-thread frees are rejected. */
  ...
  pool_cache_test_free_batch (ptm, thread_index, POOL_CACHE_TEST_FINAL_ROUND, indices, 0);
...
pool_cache_test_single_thread (vlib_main_t *vm)
{
  ...
  test_mw_pool_cache_init_with_batch (&ptm->pool, "test-single-thread-pool",
				      POOL_CACHE_TEST_LOG2_SUBPOOL_SIZE, CLIB_CACHE_LINE_BYTES,
				      POOL_CACHE_TEST_CACHE_BATCH_SIZE);
  ...
  max_indices =
    round_pow2 (POOL_CACHE_TEST_BATCH_SIZE + 1 +
		  2 * POOL_CACHE_TEST_CACHE_BATCH_SIZE * vec_len (ptm->pool.state.per_thread) +
		  POOL_CACHE_TEST_SUBPOOL_SIZE,
		POOL_CACHE_TEST_SUBPOOL_SIZE);
  vec_validate (ptm->in_use_by_index, max_indices - 1);
  vec_validate (indices, POOL_CACHE_TEST_BATCH_SIZE - 1);

  anchor = test_mw_pool_cache_alloc (&ptm->pool);
  anchor_elt = test_mw_pool_cache_elt_at_index (&ptm->pool, anchor);
  anchor_elt->magic = POOL_CACHE_TEST_MAGIC;
  n_subpools_before = ptm->pool.state.n_subpools;
  pool_cache_test_alloc_batch (ptm, vlib_get_thread_index (), POOL_CACHE_TEST_FINAL_ROUND, indices);
  if (test_mw_pool_cache_elt_at_index (&ptm->pool, anchor) != anchor_elt ||
      anchor_elt->magic != POOL_CACHE_TEST_MAGIC || ptm->pool.state.n_subpools <= n_subpools_before)
    pool_cache_test_error (ptm);
  pool_cache_test_free_batch (ptm, vlib_get_thread_index (), POOL_CACHE_TEST_FINAL_ROUND, indices,
			      0);
  test_mw_pool_cache_free (&ptm->pool, anchor);
  ...
  if (error == 0)
    error = pool_cache_test_verify_conservation (&ptm->pool.state);

  if (error == 0)
    {
      vlib_pool_cache_thread_t *pt =
	vec_elt_at_index (ptm->pool.state.per_thread, vlib_get_thread_index ());
  ...
  if (error == 0 && pool_cache_test_cache_is_initialized (&ptm->pool.state))
				   ...
				   vlib_cli_command_t *cmd)
{
  ...
  clib_memset (ptm, 0, sizeof (*ptm));
  test_mw_pool_cache_init_with_batch (&ptm->pool, "test-multi-worker-pool",
				      POOL_CACHE_TEST_LOG2_SUBPOOL_SIZE, CLIB_CACHE_LINE_BYTES,
				      POOL_CACHE_TEST_CACHE_BATCH_SIZE);
  ...
  max_expected = (u64) n_workers * POOL_CACHE_TEST_BATCH_SIZE +
		 (u64) 2 * POOL_CACHE_TEST_CACHE_BATCH_SIZE * vec_len (ptm->pool.state.per_thread) +
		 POOL_CACHE_TEST_SUBPOOL_SIZE;
  ...
  for (i = 1; i <= n_workers; i++)
    {
      vlib_pool_cache_thread_t *pt = vec_elt_at_index (ptm->pool.state.per_thread, i);
      if (clib_atomic_load_acq_n (&pt->n_cached) > 2 * POOL_CACHE_TEST_CACHE_BATCH_SIZE ||
	  clib_atomic_load_acq_n (&pt->refills) == 0 || clib_atomic_load_acq_n (&pt->flushes) == 0)
	{
	  error =
  ...
  if ((u64) ptm->pool.state.n_subpools * ptm->pool.state.subpool_size > max_expected)
    {
