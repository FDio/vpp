...
pool_cache_test_multiple_instances (void)
{
  ...
  idx = pool_cache_alloc (&pool_a);
  e = pool_cache_elt_at_index (&pool_a, idx);
  e->thread_index = 0;
  e->round = 1;
  e->slot = 2;
  ...
  if (error == 0 && pool_cache_get_if_live (&pool_a, idx) != e)
    error = clib_error_return (0, "pool-cache live lookup failed");
  if (error == 0 && !pool_cache_retire (&pool_a, idx))
    error = clib_error_return (0, "pool-cache retirement failed");
  if (error == 0 && pool_cache_get_if_live (&pool_a, idx) != 0)
  ...
  if (error == 0)
    {
      other_idx = pool_cache_alloc (&pool_a);
      if (other_idx == idx)
	error = clib_error_return (0, "pool-cache allocated a retired index");
      pool_cache_free (&pool_a, other_idx);
    }
  if (error == 0 && !pool_cache_reclaim (&pool_a, idx))
  ...
  if (error == 0)
    {
      reused_idx = pool_cache_alloc (&pool_a);

      if (pool_cache_get_if_live (&pool_a, reused_idx) == 0)
	error = clib_error_return (0, "pool-cache reclaimed index was not live after allocation");
      pool_cache_free (&pool_a, reused_idx);
    }

  if (error == 0 && (pool_cache_get_if_live (&pool_a, VLIB_POOL_CACHE_INVALID_INDEX) != 0 ||
		     pool_cache_retire (&pool_a, VLIB_POOL_CACHE_INVALID_INDEX) ||
		     pool_cache_reclaim (&pool_a, VLIB_POOL_CACHE_INVALID_INDEX)))
    error = clib_error_return (0, "pool-cache invalid retirement operation succeeded");

  idx = pool_cache_alloc (&pool_b);
  pool_cache_free (&pool_b, idx);
  ...
  pool_cache_free_resources (&pool_a);
  pool_cache_free_resources (&pool_b);
...
pool_cache_test_natural_alignment (void)
{
  ...
  for (i = 0; i < POOL_CACHE_TEST_SUBPOOL_SIZE + 1; i++)
    {
      u32 idx = pool_cache_alloc (&pool);
      pool_cache_aligned_elt_t *e = pool_cache_elt_at_index (&pool, idx);
  ...
  vec_foreach_index (i, indices)
    pool_cache_free (&pool, indices[i]);
  vec_free (indices);
  pool_cache_free_resources (&pool);
  return error;
}
...
pool_cache_cli_test_free_all (pool_cache_cli_test_main_t *ptm)
{
  ...
  for (i = 0; i < vec_len (ptm->indices); i++)
    pool_cache_free (&ptm->pool, ptm->indices[i]);
...
pool_cache_cli_test_cleanup (pool_cache_cli_test_main_t *ptm)
{
  ...
  pool_cache_free_resources (&ptm->pool);
  ptm->active = 0;
}
...
{
  u32 pidx = _pool_cache_subpool_index (c, idx);
  u32 elt_index = _pool_cache_elt_index (c, idx);
  vlib_pool_cache_subpool_meta_t *sp = _pool_cache_subpool_meta_at (c, pidx);
				 ...
				 u8 expected, const char *caller_func, u32 caller_line)
{
  ...
  for (i = start; i < end; i++)
    {
      u8 actual = pool_cache_test_slot_state (&ptm->pool, indices[i]);
...
pool_cache_test_verify_conservation (vlib_pool_cache_t *c)
{
  ...
  for (i = 0; i < n_subpools; i++)
    {
      vlib_pool_cache_subpool_meta_t *sp = _pool_cache_subpool_meta_at (c, i);
...
pool_cache_test_single_thread (vlib_main_t *vm)
{
  ...
  anchor = pool_cache_alloc (&ptm->pool);
  anchor_elt = pool_cache_elt_at_index (&ptm->pool, anchor);
  ...
  pool_cache_free (&ptm->pool, anchor);
