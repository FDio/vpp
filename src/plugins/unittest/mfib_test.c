...
static uword
placeholder_interface_tx (vlib_main_t *vm, vlib_node_runtime_t *node,
			  vlib_frame_t *frame)
{
...
static clib_error_t *
test_interface_admin_up_down (vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
...
mfib_test_mk_intf (u32 ninterfaces)
{
  ...
  for (i = 0; i < ninterfaces; i++)
	...
	vnet_get_main (), test_interface_device_class.index, i /* instance */,
	hw_address, &tm->hw_if_indicies[i],
	/* flag change */ 0);
...
static int
mfib_test_validate_rep_v (const replicate_t *rep, u16 n_buckets, va_list *ap)
{
  const dpo_id_t *dpo;
  adj_index_t ai;
  dpo_type_t dt;
  ...
  res = 0;
  MFIB_TEST_REP ((n_buckets == rep->rep_n_buckets), "n_buckets = %d",
		 rep->rep_n_buckets);
  ...
  for (bucket = 0; bucket < n_buckets; bucket++)
    {
      ...
      MFIB_TEST_REP ((dt == dpo->dpoi_type), "bucket %d stacks on %U", bucket,
		     format_dpo_type, dpo->dpoi_type);

      if (DPO_RECEIVE != dt)
	{
	  MFIB_TEST_REP ((ai == dpo->dpoi_index),
			 "bucket %d [exp:%d] stacks on %U", bucket, ai,
			 format_dpo_id, dpo, 0);
	}
    }
  return (res);
...
static int
mfib_test_entry (fib_node_index_t fei, mfib_entry_flags_t eflags,
		 int n_buckets, ...)
{
  ...
  MFIB_TEST_REP ((eflags == mfe->mfe_flags), "%U has %U expect %U",
		 format_mfib_prefix, pfx, format_mfib_entry_flags,
		 mfe->mfe_flags, format_mfib_entry_flags, eflags);

  if (0 == n_buckets)
    {
      MFIB_TEST_REP ((DPO_DROP == mfe->mfe_rep.dpoi_type), "%U links to %U",
		     format_mfib_prefix, pfx, format_dpo_id, &mfe->mfe_rep, 0);
    }
  else
    {
	...
	fei, mfib_forw_chain_type_from_fib_proto (pfx->fp_proto),
	MFIB_ENTRY_FWD_FLAG_NONE, &tmp);
      rep = replicate_get (tmp.dpoi_index);

      MFIB_TEST_REP ((DPO_REPLICATE == tmp.dpoi_type), "%U links to %U",
		     format_mfib_prefix, pfx, format_dpo_type, tmp.dpoi_type);
...
static int
mfib_test_entry_itf (fib_node_index_t fei, u32 sw_if_index,
		     mfib_itf_flags_t flags)
{
  ...
  MFIB_TEST_REP ((NULL != mfi), "%U has interface %d", format_mfib_prefix, pfx,
		 sw_if_index);

  MFIB_TEST_REP ((flags == mfi->mfi_flags),
		 "%U interface %d has flags %U expect %U", format_mfib_prefix,
		 pfx, sw_if_index, format_mfib_itf_flags, flags,
...
static int
mfib_test_entry_no_itf (fib_node_index_t fei, u32 sw_if_index)
{
  const mfib_prefix_t *pfx;
  const mfib_entry_t *mfe;
  const mfib_itf_t *mfi;
  ...
  MFIB_TEST_REP ((NULL == mfi), "%U has no interface %d", format_mfib_prefix,
		 pfx, sw_if_index);
...
static int
mfib_test_i (fib_protocol_t PROTO, vnet_link_t LINKT,
	     const mfib_prefix_t *pfx_no_forward, const mfib_prefix_t *pfx_s_g,
	     ...
	     const fib_prefix_t *pfx_itf, const ip46_address_t *addr_nbr1,
	     const ip46_address_t *addr_nbr2)
{
  fib_node_index_t mfei, mfei_dflt, mfei_no_f, mfei_s_g, mfei_g_1, mfei_g_2,
    mfei_g_3, mfei_g_m;
  ...
  ai_nbr1 =
    adj_nbr_add_or_lock (PROTO, LINKT, addr_nbr1, tm->hw[0]->sw_if_index);
  ai_nbr2 =
    adj_nbr_add_or_lock (PROTO, LINKT, addr_nbr2, tm->hw[0]->sw_if_index);
  ...
  fib_table_entry_update_one_path (
    0, pfx_itf, FIB_SOURCE_INTERFACE,
    (FIB_ENTRY_FLAG_CONNECTED | FIB_ENTRY_FLAG_ATTACHED), DPO_PROTO_IP4, NULL,
    tm->hw[0]->sw_if_index,
    ~0, // invalid fib index
    1,	// weight
  ...
  MFIB_TEST (!mfib_test_entry (mfei_dflt, MFIB_ENTRY_FLAG_DROP, 0),
	     "(*,*) no replcaitions");
  ...
  MFIB_TEST (!mfib_test_entry (mfei_no_f, MFIB_ENTRY_FLAG_NONE, 0),
	     "%U no replcaitions", format_mfib_prefix, pfx_no_forward);
  MFIB_TEST (!mfib_test_entry_itf (mfei_no_f, tm->hw[0]->sw_if_index,
				   MFIB_ITF_FLAG_ACCEPT),
	     "%U interface not accepting", format_mfib_prefix, pfx_no_forward);
  ...
  MFIB_TEST (FIB_NODE_INDEX_INVALID != mfei_s_g, "%U present",
	     format_mfib_prefix, pfx_s_g);
  MFIB_TEST (!mfib_test_entry (mfei_s_g, MFIB_ENTRY_FLAG_NONE, 3,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2, DPO_ADJACENCY_MCAST, ai_3),
	     "%U replicate ok", format_mfib_prefix, pfx_s_g);
  ...
  MFIB_TEST_NS (!mfib_test_entry_itf (mfei_s_g, tm->hw[2]->sw_if_index,
				      MFIB_ITF_FLAG_FORWARD));
  MFIB_TEST_NS (!mfib_test_entry_itf (
    mfei_s_g, tm->hw[3]->sw_if_index,
    (MFIB_ITF_FLAG_FORWARD | MFIB_ITF_FLAG_NEGATE_SIGNAL)));

  /*
   * A (*,G), which the same G as the (S,G).
   * different paths. test our LPM.
   */
  mfei_g_1 = mfib_table_entry_path_update (fib_index, pfx_star_g_1,
					   MFIB_SOURCE_API, &path_via_if0);
  ...
  MFIB_TEST (mfei == mfei_g_1, "%U found via exact match", format_mfib_prefix,
	     pfx_star_g_1);
  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_NONE, 1,
			       DPO_ADJACENCY_MCAST, ai_1),
	     "%U replicate ok", format_mfib_prefix, pfx_star_g_1);

  mfei = mfib_table_lookup (fib_index, pfx_star_g_1);
  MFIB_TEST (mfei == mfei_g_1, "[e:%d a:%d] %U found via LP match", mfei,
	     mfei_g_1, format_mfib_prefix, pfx_star_g_1);
  ...
  MFIB_TEST (mfei == mfei_s_g, "%U found via exact match", format_mfib_prefix,
	     pfx_s_g);

  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_NONE, 3,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2, DPO_ADJACENCY_MCAST, ai_3),
	     "%U replicate OK", format_mfib_prefix, pfx_s_g);
  mfei = mfib_table_lookup (fib_index, pfx_s_g);
  MFIB_TEST (mfei == mfei_s_g, "%U found via LP match", format_mfib_prefix,
	     pfx_s_g);
  ...
  mfei_g_m = mfib_table_entry_path_update (fib_index, pfx_star_g_slash_m,
					   MFIB_SOURCE_API, &path_via_if2);
  mfib_table_entry_path_update (fib_index, pfx_star_g_slash_m, MFIB_SOURCE_API,
				&path_via_if3);
   ...
   */
  mfei = mfib_table_lookup_exact_match (fib_index, pfx_star_g_1);
  MFIB_TEST ((mfei_g_1 == mfei), "%U found via DP LPM: %d", format_mfib_prefix,
	     pfx_star_g_1, mfei);
  ...
  MFIB_TEST (mfei = mfei_g_m, "%U Found via exact match", format_mfib_prefix,
	     pfx_star_g_slash_m);
  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_NONE, 1,
			       DPO_ADJACENCY_MCAST, ai_3),
	     "%U replicate OK", format_mfib_prefix, pfx_star_g_slash_m);
  MFIB_TEST (mfei_g_m == mfib_table_lookup (fib_index, pfx_star_g_slash_m),
	     "%U found via LPM", format_mfib_prefix, pfx_star_g_slash_m);
  ...
  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_NONE, 4,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2, DPO_ADJACENCY_MCAST, ai_3, DPO_RECEIVE,
			       0),
	     "%U replicate OK", format_mfib_prefix, pfx_s_g);
   ...
   */
  mfib_table_entry_path_remove (fib_index, pfx_s_g, MFIB_SOURCE_API,
				&path_for_us);
  ...
  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_NONE, 2,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2),
	     "%U replicate OK", format_mfib_prefix, pfx_s_g);
  MFIB_TEST_NS (
    !mfib_test_entry_itf (mfei, tm->hw[0]->sw_if_index, MFIB_ITF_FLAG_ACCEPT));
  MFIB_TEST_NS (!mfib_test_entry_itf (mfei, tm->hw[1]->sw_if_index,
				      MFIB_ITF_FLAG_FORWARD));
  MFIB_TEST_NS (!mfib_test_entry_itf (mfei, tm->hw[2]->sw_if_index,
				      MFIB_ITF_FLAG_FORWARD));
  MFIB_TEST_NS (
    !mfib_test_entry_itf (mfei, tm->hw[3]->sw_if_index, MFIB_ITF_FLAG_ACCEPT));
  /*
   * Make the path forwarding again
   *  - expect it to be added back to the replication set
   */
  path_via_if3.frp_mitf_flags = (MFIB_ITF_FLAG_FORWARD | MFIB_ITF_FLAG_ACCEPT |
  ...
  MFIB_TEST_NS (!mfib_test_entry_itf (mfei, tm->hw[2]->sw_if_index,
				      MFIB_ITF_FLAG_FORWARD));
  MFIB_TEST_NS (
    !mfib_test_entry_itf (mfei, tm->hw[3]->sw_if_index,
			  (MFIB_ITF_FLAG_FORWARD | MFIB_ITF_FLAG_ACCEPT |
			   MFIB_ITF_FLAG_NEGATE_SIGNAL)));

  /*
   * update flags on the entry
   */
  mfib_table_entry_update (fib_index, pfx_s_g, MFIB_SOURCE_API,
			   MFIB_RPF_ID_NONE, MFIB_ENTRY_FLAG_SIGNAL);
  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_SIGNAL, 3,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2, DPO_ADJACENCY_MCAST, ai_3),
	     "%U replicate OK", format_mfib_prefix, pfx_s_g);
  ...
  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_SIGNAL, 2,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2),
	     "%U replicate OK", format_mfib_prefix, pfx_s_g);
  MFIB_TEST_NS (
  ...
  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_SIGNAL, 1,
  ...
  mfib_table_entry_paths_remove (fib_index, pfx_s_g, MFIB_SOURCE_API,
				 two_paths);

  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_SIGNAL, 0),
	     "%U no replications", format_mfib_prefix, pfx_s_g);

  /*
   * update flags on the entry
   */
  mfib_table_entry_update (
    fib_index, pfx_s_g, MFIB_SOURCE_API, MFIB_RPF_ID_NONE,
    (MFIB_ENTRY_FLAG_SIGNAL | MFIB_ENTRY_FLAG_CONNECTED));
   ...
   */
  path_via_if0.frp_mitf_flags =
    (MFIB_ITF_FLAG_ACCEPT | MFIB_ITF_FLAG_NEGATE_SIGNAL);
  mfei_g_2 = mfib_table_entry_path_update (fib_index, pfx_star_g_2,
					   MFIB_SOURCE_API, &path_via_if0);
  MFIB_TEST (!mfib_test_entry (mfei_g_2, MFIB_ENTRY_FLAG_NONE, 0),
	     "%U No replications", format_mfib_prefix, pfx_star_g_2);
  ...
  path_via_if0.frp_mitf_flags =
    (MFIB_ITF_FLAG_ACCEPT | MFIB_ITF_FLAG_NEGATE_SIGNAL);
  mfei_g_3 = mfib_table_entry_path_update (fib_index, pfx_star_g_3,
					   MFIB_SOURCE_API, &path_via_if0);
  MFIB_TEST (!mfib_test_entry (mfei_g_3, MFIB_ENTRY_FLAG_NONE, 0),
	     "%U No replications", format_mfib_prefix, pfx_star_g_3);
  ...
  if (FIB_PROTOCOL_IP6 == PROTO)
	...
	.as_u64[1] = clib_host_to_net_u64 (0xffffffffffffffff),
      ...
      MFIB_TEST ((mfei_g_m == ip6_mfib_table_fwd_lookup (
				ip6_mfib_get (fib_index), &src,
				&pfx_star_g_slash_m->fp_grp_addr.ip6)),
		 "%U found via DP LPM grp=%U", format_mfib_prefix,
		 pfx_star_g_slash_m, format_ip6_address,
		 &pfx_star_g_slash_m->fp_grp_addr.ip6);
      ...
      MFIB_TEST ((mfei_g_m ==
		  ip6_mfib_table_fwd_lookup (ip6_mfib_get (fib_index),
					     &pfx_s_g->fp_src_addr.ip6, &tmp)),
		 "%U found via DP LPM grp=%U", format_mfib_prefix,
		 pfx_star_g_slash_m, format_ip6_address, &tmp);
      ...
      MFIB_TEST ((mfei_s_g == mfei), "%U found via DP LPM: %d",
		 format_mfib_prefix, pfx_s_g, mfei);

      /*
       * Find the 3 (*,G) s
       */
      mfei = ip6_mfib_table_fwd_lookup (ip6_mfib_get (fib_index), &src,
					&pfx_star_g_1->fp_grp_addr.ip6);
      MFIB_TEST ((mfei_g_1 == mfei), "%U found via DP LPM: %d",
		 format_mfib_prefix, pfx_star_g_1, mfei);
      mfei = ip6_mfib_table_fwd_lookup (ip6_mfib_get (fib_index), &src,
					&pfx_star_g_2->fp_grp_addr.ip6);
      MFIB_TEST ((mfei_g_2 == mfei), "%U found via DP LPM: %d",
		 format_mfib_prefix, pfx_star_g_2, mfei);
      mfei = ip6_mfib_table_fwd_lookup (ip6_mfib_get (fib_index), &src,
					&pfx_star_g_3->fp_grp_addr.ip6);
      MFIB_TEST ((mfei_g_3 == mfei), "%U found via DP LPM: %d",
   ...
   */
  mfib_table_entry_update (fib_index, pfx_s_g, MFIB_SOURCE_API,
			   MFIB_RPF_ID_NONE, MFIB_ENTRY_FLAG_NONE);
  mfei = mfib_table_lookup_exact_match (fib_index, pfx_s_g);
  MFIB_TEST (FIB_NODE_INDEX_INVALID == mfei, "%U gone", format_mfib_prefix,
	     pfx_s_g);

  /*
   * remove the last path on the no forward entry - the last entry
   */
  mfib_table_entry_path_remove (fib_index, pfx_no_forward, MFIB_SOURCE_API,
				&path_via_if0);
  ...
  mfei_g_1 = mfib_table_entry_path_update (fib_index, pfx_star_g_1,
					   MFIB_SOURCE_API, &path_via_nbr1);
  mfei_g_1 = mfib_table_entry_path_update (fib_index, pfx_star_g_1,
					   MFIB_SOURCE_API, &path_via_nbr2);
  MFIB_TEST (!mfib_test_entry (mfei_g_1, MFIB_ENTRY_FLAG_NONE, 2,
  ...
  mfib_table_entry_path_remove (fib_index, pfx_star_g_1, MFIB_SOURCE_API,
				&path_via_nbr1);

  MFIB_TEST (!mfib_test_entry (mfei_g_1, MFIB_ENTRY_FLAG_NONE, 1,
			       DPO_ADJACENCY_INCOMPLETE, ai_nbr2),
	     "%U replicate OK", format_mfib_prefix, pfx_star_g_1);
  MFIB_TEST_NS (!mfib_test_entry_itf (mfei_g_1, tm->hw[0]->sw_if_index,
				      MFIB_ITF_FLAG_FORWARD));
  ...
  mfei =
    mfib_table_entry_special_add (fib_index, pfx_star_g_3, MFIB_SOURCE_SRv6,
				  MFIB_ENTRY_FLAG_ACCEPT_ALL_ITF, repi);
  MFIB_TEST (!mfib_test_entry (
	       mfei,
	       (MFIB_ENTRY_FLAG_ACCEPT_ALL_ITF | MFIB_ENTRY_FLAG_EXCLUSIVE), 1,
	       DPO_ADJACENCY_MCAST, ai_2),
	     "%U exclusive replicate OK", format_mfib_prefix, pfx_star_g_3);
  ...
  mfib_entry_update (
    mfei, MFIB_SOURCE_SRv6,
    (MFIB_ENTRY_FLAG_ACCEPT_ALL_ITF | MFIB_ENTRY_FLAG_EXCLUSIVE),
    MFIB_RPF_ID_NONE, repi2);
  MFIB_TEST (
    !mfib_test_entry (
      mfei, (MFIB_ENTRY_FLAG_ACCEPT_ALL_ITF | MFIB_ENTRY_FLAG_EXCLUSIVE), 1,
      DPO_ADJACENCY_MCAST, ai_1),
    "%U exclusive update replicate OK", format_mfib_prefix, pfx_star_g_3);
  ...
  ai_mpls_10_10_10_1 = adj_nbr_add_or_lock (
    FIB_PROTOCOL_IP4, VNET_LINK_MPLS, &nh_10_10_10_1, tm->hw[0]->sw_if_index);
  ...
  mpls_sw_interface_enable_disable (&mpls_main, tm->hw[0]->sw_if_index, 1, 0);

  lfei = fib_table_entry_update_one_path (
    0, // default MPLS Table
    &pfx_3500, FIB_SOURCE_API, FIB_ENTRY_FLAG_MULTICAST, DPO_PROTO_IP4,
    &nh_10_10_10_1, tm->hw[0]->sw_if_index,
    ~0, // invalid fib index
    1, l3300, FIB_ROUTE_PATH_FLAG_NONE);
  MFIB_TEST (
    !fib_test_validate_entry (lfei, FIB_FORW_CHAIN_TYPE_MPLS_EOS, 1, &mc_0),
  ...
  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_NONE, 1, DPO_REPLICATE,
			       mldp_dpo.dpoi_index),
	     "%U over-mLDP replicate OK", format_mfib_prefix, pfx_s_g);
  ...
  mfei = mfib_table_entry_path_update (fib_index, pfx_s_g, MFIB_SOURCE_API,
				       &path_for_us);
  MFIB_TEST (!mfib_test_entry (mfei, MFIB_ENTRY_FLAG_NONE, 2, DPO_REPLICATE,
			       mldp_dpo.dpoi_index, DPO_RECEIVE, 0),
	     "%U mLDP+for-us replicate OK", format_mfib_prefix, pfx_s_g);
   ...
   */
  mpls_sw_interface_enable_disable (&mpls_main, tm->hw[0]->sw_if_index, 0, 0);
  ...
  MFIB_TEST (n_pls == fib_path_list_pool_size (), "%d=%d path-lists", n_pls,
	     fib_path_list_pool_size ());
  MFIB_TEST (n_reps == pool_elts (replicate_pool), "%d=%d replicates", n_reps,
	     pool_elts (replicate_pool));
  MFIB_TEST (n_entries == pool_elts (mfib_entry_pool),
	     " No more entries %d!=%d", n_entries,
	     pool_elts (mfib_entry_pool));
  MFIB_TEST (n_itfs == pool_elts (mfib_itf_pool), " No more Interfaces %d!=%d",
	     n_itfs, pool_elts (mfib_itf_pool));
  vec_free (two_paths);
...
mfib_test_v4 (void)
{
  const mfib_prefix_t pfx_224_s_8 = { .fp_len = 8,
				      ...
				      .fp_grp_addr = {
					.ip4.as_u32 =
					  clib_host_to_net_u32 (0xe0000000),
				      } };
  ...
  return (mfib_test_i (FIB_PROTOCOL_IP4, VNET_LINK_IP4, &pfx_224_s_8,
		       &pfx_1_1_1_1_c_239_1_1_1, &pfx_239_1_1_1,
		       &pfx_239_1_1_2, &pfx_239_1_1_3, &pfx_239, &pfx_itf,
		       &nbr1, &nbr2));
}
...
mfib_test_v6 (void)
{
  ...
  return (mfib_test_i (FIB_PROTOCOL_IP6, VNET_LINK_IP6, &pfx_ffd_s_12,
		       &pfx_2001_1_c_ff_1, &pfx_ff_1, &pfx_ff_2, &pfx_ff_3,
		       &pfx_ff, &pfx_itf, &nbr1, &nbr2));
}

static int
mfib_test_rr_i (fib_protocol_t FPROTO, dpo_proto_t DPROTO, vnet_link_t LINKT,
		const mfib_prefix_t *pfx_cover, const mfib_prefix_t *pfx_host1,
		const mfib_prefix_t *pfx_host2)
{
   ...
   */
  mfei_host1 = mfib_table_entry_special_add (
    fib_index, pfx_host1, MFIB_SOURCE_RR, MFIB_ENTRY_FLAG_NONE, INDEX_INVALID);
  /*
   * expect its forwarding to match the cover's
   */
  MFIB_TEST (!mfib_test_entry (mfei_host1, MFIB_ENTRY_FLAG_DROP, 0),
	     "%U no replications OK", format_mfib_prefix, pfx_host1);
  ...
  MFIB_TEST (!mfib_test_entry (mfei_cover, MFIB_ENTRY_FLAG_NONE, 1,
			       DPO_ADJACENCY_MCAST, ai_1),
	     "%U replicate OK", format_mfib_prefix, pfx_cover);

  /*
   * expect the /32 forwarding to match the new cover's
   */
  MFIB_TEST (!mfib_test_entry (mfei_host1, MFIB_ENTRY_FLAG_NONE, 1,
			       DPO_ADJACENCY_MCAST, ai_1),
	     "%U replicate OK", format_mfib_prefix, pfx_host1);
   ...
   */
  MFIB_TEST (!mfib_test_entry (mfei_cover, MFIB_ENTRY_FLAG_NONE, 2,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2),
	     "%U replicate OK", format_mfib_prefix, pfx_cover);
  MFIB_TEST (!mfib_test_entry (mfei_host1, MFIB_ENTRY_FLAG_NONE, 2,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2),
	     "%U replicate OK", format_mfib_prefix, pfx_host1);
   ...
   */
  mfei_host2 = mfib_table_entry_special_add (
    fib_index, pfx_host2, MFIB_SOURCE_RR, MFIB_ENTRY_FLAG_NONE, INDEX_INVALID);
  MFIB_TEST (!mfib_test_entry (mfei_host2, MFIB_ENTRY_FLAG_NONE, 2,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2),
	     "%U replicate OK", format_mfib_prefix, pfx_host2);
   ...
   */
  MFIB_TEST (!mfib_test_entry (mfei_cover, MFIB_ENTRY_FLAG_NONE, 3,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2, DPO_RECEIVE, 0),
	     "%U replicate OK", format_mfib_prefix, pfx_cover);
  MFIB_TEST (!mfib_test_entry (mfei_host1, MFIB_ENTRY_FLAG_NONE, 3,
			       DPO_ADJACENCY_MCAST, ai_1, DPO_ADJACENCY_MCAST,
			       ai_2, DPO_RECEIVE, 0),
	     "%U replicate OK", format_mfib_prefix, pfx_cover);
  ...
  mfib_entry_contribute_forwarding (
    mfei_host1, mfib_forw_chain_type_from_dpo_proto (DPROTO),
    MFIB_ENTRY_FWD_FLAG_NONE, &dpo);

  rep = replicate_get (dpo.dpoi_index);
  MFIB_TEST ((3 == rep->rep_n_buckets), "%U replicate 3 buckets",
  ...
  mfib_entry_contribute_forwarding (
    mfei_host1, mfib_forw_chain_type_from_dpo_proto (DPROTO),
    MFIB_ENTRY_FWD_FLAG_NO_LOCAL, &dpo);

  rep = replicate_get (dpo.dpoi_index);
  MFIB_TEST ((2 == rep->rep_n_buckets), "%U no-local replicate 2 buckets",
	     format_mfib_prefix, pfx_host1);
   ...
   */
  mfei_host1 = mfib_table_entry_path_update (fib_index, pfx_host1,
					     MFIB_SOURCE_API, &path_via_if2);
  MFIB_TEST (!mfib_test_entry (mfei_host1, MFIB_ENTRY_FLAG_NONE, 1,
			       DPO_ADJACENCY_MCAST, ai_2),
	     "%U replicate OK", format_mfib_prefix, pfx_host1);
...
mfib_test_rr_v4 (void)
{
  ...
  return (mfib_test_rr_i (FIB_PROTOCOL_IP4, DPO_PROTO_IP4, VNET_LINK_IP4,
			  &pfx_cover, &pfx_host1, &pfx_host2));
}
...
mfib_test_rr_v6 (void)
{
  ...
  return (mfib_test_rr_i (FIB_PROTOCOL_IP6, DPO_PROTO_IP6, VNET_LINK_IP6,
			  &pfx_cover, &pfx_host1, &pfx_host2));
}

static clib_error_t *
mfib_test (vlib_main_t *vm, unformat_input_t *input,
