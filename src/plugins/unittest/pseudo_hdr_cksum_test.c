...
ip4_pseudo_header_cksum (ip4_header_t *ip4)
{
  ...
  psh.l4len =
    clib_host_to_net_u16 (clib_net_to_host_u16 (ip4->length) - sizeof (ip4_header_t));
...
// ipv4
u8 phc_ipv4_tcp_data[50] = { 0x45, 0x00, 0x05, 0xdc, 0xdb, 0x42, 0x40, 0x00, 0x40,
...
test_phc_perf (vlib_main_t *vm, phc_test_main_t *ptm)
{
  ...
  vlib_cli_output (vm,
		   ...
		   "warmup-rounds %u", buffer_size,
		    n_buffers, rounds, warmup_rounds);
