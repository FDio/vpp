                                 ...
                                 unformat_input_t * input)
{
  ...
  /* f->size if the actual fifo size, at this moment */
  ...
  /* verify that fifo is growing larger than original 4KB */
  /* 4KB + 4KB = 8KB */
  ...
  svm_fifo_enqueue (rx_fifo, size_8KB, data);
  max_dequeue = svm_fifo_max_dequeue (rx_fifo);
  SEG_MGR_TEST ((max_dequeue == size_16KB), "max_dequeue %u", max_dequeue);

  /* 16KB + 16KB = 32KB */
  svm_fifo_enqueue (rx_fifo, size_16KB, data);
  max_dequeue = svm_fifo_max_dequeue (rx_fifo);
  SEG_MGR_TEST ((max_dequeue == size_32KB), "max_dequeue %u", max_dequeue);

  /* bulk enqueue */
  for (i = 0; i < 59; ++i)
    svm_fifo_enqueue (rx_fifo, size_32KB, data);
  max_dequeue = svm_fifo_max_dequeue (rx_fifo);
  SEG_MGR_TEST ((max_dequeue == (size_32KB * 60)), "max_dequeue %u", max_dequeue);
  rv = fifo_segment_get_mem_status (fs);
  SEG_MGR_TEST ((rv == MEMORY_PRESSURE_HIGH_PRESSURE),
                "segment_manager_get_mem_status %s", states_str (rv));
  ...
  max_dequeue = svm_fifo_max_dequeue (rx_fifo);
  SEG_MGR_TEST ((max_dequeue == (size_32KB * 40)), "max_dequeue %u", max_dequeue);
  rv = fifo_segment_get_mem_status (fs);
  SEG_MGR_TEST ((rv == MEMORY_PRESSURE_LOW_PRESSURE),
                "segment_manager_get_mem_status %s", states_str (rv));
  ...
  for (i = 0; i < 10; ++i)
    svm_fifo_dequeue_drop (rx_fifo, size_32KB);
  max_dequeue = svm_fifo_max_dequeue (rx_fifo);
  SEG_MGR_TEST ((max_dequeue == (size_32KB * 30)), "max_dequeue %u", max_dequeue);
  rv = fifo_segment_get_mem_status (fs);
  SEG_MGR_TEST ((rv == MEMORY_PRESSURE_NO_PRESSURE),
                "segment_manager_get_mem_status %s", states_str (rv));
  ...
  /* (virtual) fifo size is still large */
  SEG_MGR_TEST ((rx_fifo->size == size_2MB), "rx_fifo->size %u", rx_fifo->size);
