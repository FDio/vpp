...
dummy_virtual_fifo_size_update (segment_manager_t * sm,
                                ...
                                u32 new_value)
{
  segment_manager_update_mem_usage (sm, 0, 0,
...
fifo_tuning_logic_test_basic (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  rv = segment_manager_init (sm, seg_size, prealloc_fifo);
  ...
  SEG_MGR_TEST ((sm->high_watermark == 0),
                "sm->high_watermark %d", sm->high_watermark);
  SEG_MGR_TEST ((sm->low_watermark == 0),
                "sm->low_watermark %d", sm->low_watermark);

  /* since HWM==0, high-pressure is detected on in-use==0 */
...
static int
fifo_tuning_logic_test_app_attach_1 (vlib_main_t * vm, unformat_input_t * input)
{ 
  int rv;
  segment_manager_t *sm;
...
static int
fifo_tuning_logic_test_app_attach_2 (vlib_main_t * vm, unformat_input_t * input)
{
  int rv;
  segment_manager_t *sm;
  uword app_seg_size = 2UL << 30; /* 2GB */
