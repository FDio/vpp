...
static vnet_punt_reason_t punt_reason_v4, punt_reason_v6;
static vnet_punt_hdl_t punt_hdl;
		       ...
		       vlib_frame_t * frame, u8 is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_ip4)
	    vnet_buffer (b0)->punt.reason = punt_reason_v4;
	  else
	    vnet_buffer (b0)->punt.reason = punt_reason_v6;
	  ...
	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
	    {
	      ...
	      t->pt_reason = vnet_buffer (b0)->punt.reason;
	    }
	  vlib_validate_buffer_enqueue_x1 (vm, node, next_index,
...
format_punt_feat_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  punt_feat_trace_t *t = va_arg (*args, punt_trace_t *);

  s = format (s, "reason: %U", format_vnet_punt_reason, t->pt_reason);
	   ...
	   unformat_input_t * input, vlib_cli_command_t * cmd_arg)
{
  ...
  if (unformat (input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index))
      ...
      else
	{
	   ...
	   */
	  punt_hdl = vnet_punt_client_register ("test");

	  rc =
	    vnet_punt_reason_alloc (punt_hdl, "reason-v4", &punt_reason_v4);
	  rc |=
	    vnet_punt_reason_alloc (punt_hdl, "reason-v6", &punt_reason_v6);
	  ASSERT (!rc);
	  ...
	  if (FIB_PROTOCOL_IP4 == fproto)
	    {
	       ...
	       */
	      vnet_punt_register (punt_hdl, punt_reason_v4,
				  "punt-test-pg0-ip4");
	      from = vlib_get_node_by_name (vm, (u8 *) "punt-test-pg0-ip4");
	    }
	  else
	    {
	      vnet_punt_register (punt_hdl, punt_reason_v6,
				  "punt-test-pg0-ip6");
