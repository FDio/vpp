/*
 * flowprobe.c - per-packet data capture flow report plugin
 *
...
static clib_error_t *
flowprobe_create_state_tables (void)
{
  flowprobe_main_t *fm = &flowprobe_main;
...
static bool
time_to_wakeup (u64 last_run, f64 sleep_sec)
{
  vlib_main_t *vm = vlib_get_main ();
  return vlib_time_now (vm) - (last_run + sleep_sec) > 0;
}
...
timer_process (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  f64 sleep_duration;
  u64 *last_run_per_worker = 0;
  vec_validate (last_run_per_worker, vec_len (vlib_mains));

  while (1)
    {
      sleep_duration = fm->active_timer;
      /* Send an interrupt to each timer input node */
      for (i = 0; i < vec_len (worker_vms); i++)
	{
	  worker_vm = worker_vms[i];
	  if (worker_vm && time_to_wakeup (last_run_per_worker[i],
					   fm->sleep_per_worker[i]))
	    {
	      vlib_node_set_interrupt_pending (worker_vm,
					       flowprobe_walker_node.index);
	      last_run_per_worker[i] = vlib_time_now (vm);
	      if (fm->sleep_per_worker[i])
		sleep_duration = fm->sleep_per_worker[i] < sleep_duration ?
		  fm->sleep_per_worker[i] : sleep_duration;
	    }
	}
...
flowprobe_init (vlib_main_t * vm)
{
  ...
  vec_validate (fm->sleep_per_worker, num_threads - 1);
