...
static vlib_node_registration_t flowprobe_input_timer_node;
      ...
      for (i = 0; i < vec_len (worker_vms); i++)
	{
	  ...
	  if (worker_vm)
	    vlib_node_set_interrupt_pending (worker_vm,
					     flowprobe_input_timer_node.index);
	}
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (flowprobe_input_timer_node,static) = {
  .function = input_timer_process,
  .name = "flowprobe-input-timer",
