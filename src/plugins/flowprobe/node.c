...
static inline u32
flowprobe_common_add (vlib_buffer_t * to_b, flowprobe_entry_t * e,
		       u16 offset)
{
  u16 start = offset;
...
static inline u32
flowprobe_l3_ip6_add (vlib_buffer_t * to_b, flowprobe_entry_t * e,
		       u16 offset)
{
  u16 start = offset;
...
static inline u32
flowprobe_l3_ip4_add (vlib_buffer_t * to_b, flowprobe_entry_t * e,
		       u16 offset)
{
  u16 start = offset;
...
static void
flowprobe_delete (u32 my_cpu_number, flowprobe_key_t * k, u32 poolindex)
{
  flowprobe_main_t *fm = &flowprobe_main;
  u32 h;
			  ...
			  flowprobe_variant_t which, flowprobe_trace_t * t)
{
  ...
  if (!e->timer_on)
      ...
      else if (!collision)
	{
	  ...
	  flowprobe_export_entry (vm, e);
	  flowprobe_delete (my_cpu_number, &e->key, poolindex);
	}
    }
}
