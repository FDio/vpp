...
flowprobe_common_add (vlib_buffer_t * to_b, flowprobe_entry_t * e, u16 offset)
{
  ...
  _clib_memcpy (to_b->data + offset, &rx_if, sizeof (rx_if));
  offset += sizeof (rx_if);

  /* Egress interface */
  u32 tx_if = clib_host_to_net_u32 (e->key.tx_sw_if_index);
  _clib_memcpy (to_b->data + offset, &tx_if, sizeof (tx_if));
  offset += sizeof (tx_if);

  /* packet delta count */
  u64 packetdelta = clib_host_to_net_u64 (e->packetcount);
  _clib_memcpy (to_b->data + offset, &packetdelta, sizeof (u64));
  offset += sizeof (u64);

  /* flowStartNanoseconds */
  u32 t = clib_host_to_net_u32 (e->flow_start.sec + NTP_TIMESTAMP);
  _clib_memcpy (to_b->data + offset, &t, sizeof (u32));
  offset += sizeof (u32);
...
flowprobe_l2_add (vlib_buffer_t * to_b, flowprobe_entry_t * e, u16 offset)
{
  ...
  /* src mac address */
  _clib_memcpy (to_b->data + offset, &e->key.src_mac, 6);
  offset += 6;

  /* dst mac address */
  _clib_memcpy (to_b->data + offset, &e->key.dst_mac, 6);
  offset += 6;

  /* ethertype */
  _clib_memcpy (to_b->data + offset, &e->key.ethertype, 2);
  offset += 2;
...
flowprobe_l3_ip6_add (vlib_buffer_t * to_b, flowprobe_entry_t * e, u16 offset)
{
  ...
  /* ip6 src address */
  _clib_memcpy (to_b->data + offset, &e->key.src_address,
		sizeof (ip6_address_t));
  offset += sizeof (ip6_address_t);

  /* ip6 dst address */
  _clib_memcpy (to_b->data + offset, &e->key.dst_address,
		sizeof (ip6_address_t));
  offset += sizeof (ip6_address_t);
  ...
  _clib_memcpy (to_b->data + offset, &octetdelta, sizeof (u64));
  offset += sizeof (u64);
...
flowprobe_l3_ip4_add (vlib_buffer_t * to_b, flowprobe_entry_t * e, u16 offset)
{
  ...
  /* ip4 src address */
  _clib_memcpy (to_b->data + offset, &e->key.src_address.ip4,
		sizeof (ip4_address_t));
  offset += sizeof (ip4_address_t);

  /* ip4 dst address */
  _clib_memcpy (to_b->data + offset, &e->key.dst_address.ip4,
		sizeof (ip4_address_t));
  offset += sizeof (ip4_address_t);
...
flowprobe_l4_add (vlib_buffer_t * to_b, flowprobe_entry_t * e, u16 offset)
{
  ...
  /* src port */
  _clib_memcpy (to_b->data + offset, &e->key.src_port, 2);
  offset += 2;

  /* dst port */
  _clib_memcpy (to_b->data + offset, &e->key.dst_port, 2);
  offset += 2;

  /* tcp control bits */
  u16 control_bits = htons (e->prot.tcp.flags);
  _clib_memcpy (to_b->data + offset, &control_bits, 2);
  offset += 2;
			  ...
			  flowprobe_variant_t which, flowprobe_trace_t * t)
{
  ...
  if (flags & FLOW_RECORD_L2)
    {
      _clib_memcpy (k.src_mac, eth->src_address, 6);
      _clib_memcpy (k.dst_mac, eth->dst_address, 6);
  ...
  if (t)
    {
      ...
      _clib_memcpy (t->src_mac, k.src_mac, 6);
      _clib_memcpy (t->dst_mac, k.dst_mac, 6);
