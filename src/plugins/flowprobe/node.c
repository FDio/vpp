...
flowprobe_delete_by_index (u32 my_cpu_number, u32 poolindex)
{
  ...
  if (fm->active_timer == 0)	/* Not keeping state */
    return;
  ...
  pool_put (fm->pool_per_worker[my_cpu_number], e);
}
  ...
  if (collision)
    {
      ...
      flowprobe_export_entry (vm, e);
      flowprobe_delete_by_index (my_cpu_number, poolindex);
    }
}
			  ...
			  vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  /* *INDENT-OFF* */
  u32 active = 0, passive = 0, poolentries = 0;
  u32 *to_be_removed = 0, *i;

  fm->sleep_per_worker[cpu_index] = FLOWPROBE_ACTIVITY_MEDIUM;

  pool_foreach (e, fm->pool_per_worker[cpu_index], (
    {
      /* Look for active entries and export those */
      if (e->packetcount &&
	  vlib_time_now (vm) > e->last_exported + fm->active_timer)
	{
	  active++;
	  flowprobe_export_entry (vm, e);
	}
	...
	{
	  if (e->packetcount == 0 &&
	      vlib_time_now (vm) > e->last_updated + fm->passive_timer)
	    {
	      vec_add1(to_be_removed, e - fm->pool_per_worker[cpu_index]);
	      passive++;
	    }
	}
      if (active > FLOW_MAXIMUM_EXPORT_ENTRIES - 1)
	{
	  fm->sleep_per_worker[cpu_index] = FLOWPROBE_ACTIVITY_HIGH;
	  goto exit;
	}
      poolentries++;
    }));
...
exit:
  ...
  if (!active && !passive && !poolentries)
    fm->sleep_per_worker[cpu_index] = FLOWPROBE_ACTIVITY_LOW;
