...
static clib_error_t *
lisp_show_adjacencies_command_fn (vlib_main_t *vm, unformat_input_t *input,
...
static clib_error_t *
lisp_add_del_map_server_command_fn (vlib_main_t *vm, unformat_input_t *input,
				    vlib_cli_command_t *cmd)
{
  int rv = 0;
...
static clib_error_t *
lisp_eid_table_map_command_fn (vlib_main_t *vm, unformat_input_t *input,
			       vlib_cli_command_t *cmd)
{
  u8 is_add = 1, is_l2 = 0;
  u32 vni = 0, dp_id = 0;
...
static clib_error_t *
lisp_add_del_ndp_entry_command_fn (vlib_main_t *vm, unformat_input_t *input,
				   vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  clib_error_t *error = NULL;
...
static clib_error_t *
lisp_add_del_l2_arp_entry_command_fn (vlib_main_t *vm, unformat_input_t *input,
				      vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  clib_error_t *error = NULL;
...
static clib_error_t *
lisp_show_l2_arp_entries_command_fn (vlib_main_t *vm, unformat_input_t *input,
				     vlib_cli_command_t *cmd)
{
...
static clib_error_t *
lisp_show_ndp_entries_command_fn (vlib_main_t *vm, unformat_input_t *input,
				  vlib_cli_command_t *cmd)
{
				   ...
				   vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "reid %U", unformat_ip_prefix,
			 reid_ippref))
	{
      ...
      else if (unformat (line_input, "vni %u", &vni))
	{
      ...
      else if (unformat (line_input, "leid %U", unformat_ip_prefix,
			 leid_ippref))
	{
  ...
  if ((gid_address_type (&leid) != gid_address_type (&reid)) ||
      (gid_address_type (&reid) == GID_ADDR_IP_PREFIX &&
       ip_prefix_version (reid_ippref) != ip_prefix_version (leid_ippref)))
    {
...
static clib_error_t *
lisp_map_request_mode_command_fn (vlib_main_t *vm, unformat_input_t *input,
				  vlib_cli_command_t *cmd)
{
...
static clib_error_t *
lisp_show_map_resolvers_command_fn (vlib_main_t *vm, unformat_input_t *input,
				    vlib_cli_command_t *cmd)
{
  lisp_msmr_t *mr;
  lisp_cp_main_t *lcm = vnet_lisp_cp_get_main ();
...
static clib_error_t *
lisp_nsh_set_locator_set_command_fn (vlib_main_t *vm, unformat_input_t *input,
				     vlib_cli_command_t *cmd)
{
...
static clib_error_t *
lisp_pitr_set_locator_set_command_fn (vlib_main_t *vm, unformat_input_t *input,
				      vlib_cli_command_t *cmd)
{
  u8 locator_name_set = 0;
  u8 *locator_set_name = 0;
  ...
  if (0 != rv)
    {
      error =
	clib_error_return (0, "failed to %s pitr!", is_add ? "add" : "delete");
    }
...
static clib_error_t *
lisp_show_pitr_command_fn (vlib_main_t *vm, unformat_input_t *input,
			   vlib_cli_command_t *cmd)
{
  lisp_cp_main_t *lcm = vnet_lisp_cp_get_main ();
  mapping_t *m;
...
format_eid_entry (u8 *s, va_list *args)
{
  ...
  u8 *type =
    ls->local ? format (0, "local(%s)", ls->name) : format (0, "remote");

  if (vec_len (ls->locator_indices) == 0)
    {
      s =
	format (s, "%-35U%-20saction:%-30U%-20u%-u", format_gid_address, gid,
		type, format_negative_mapping_action, mapit->action, ttl, aut);
    }
  else
    {
...
static clib_error_t *
lisp_show_eid_table_command_fn (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
  lisp_cp_main_t *lcm = vnet_lisp_cp_get_main ();
  mapping_t *mapit;
  ...
  vlib_cli_output (vm, "%-35s%-20s%-30s%-20s%-s", "EID", "type", "locators",
		   "ttl", "authoritative");
  ...
  if (print_all)
      ...
      pool_foreach (mapit, lcm->mapping_pool)
	{
	  ...
	  if (filter &&
	      !((1 == filter && ls->local) || (2 == filter && !ls->local)))
	    {
	      continue;
	    }
	  vlib_cli_output (vm, "%U", format_eid_entry, lcm->vnet_main, lcm,
			   mapit, ls);
	}
      /* *INDENT-ON* */
    }
	...
	goto done;
      ...
      locator_set_t *ls =
	pool_elt_at_index (lcm->locator_set_pool, mapit->locator_set_index);
	  ...
	  !((1 == filter && ls->local) || (2 == filter && !ls->local)))
	{
      ...
      vlib_cli_output (vm, "%U,", format_eid_entry, lcm->vnet_main, lcm, mapit,
		       ls);
    }
...
static clib_error_t *
lisp_map_register_set_ttl_command_fn (vlib_main_t *vm, unformat_input_t *input,
				      vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  u32 ttl = 0;
...
static clib_error_t *
lisp_cp_show_locator_sets_command_fn (vlib_main_t *vm, unformat_input_t *input,
				      vlib_cli_command_t *cmd)
{
  locator_set_t *lsit;
  locator_t *loc;
  ...
  pool_foreach (lsit, lcm->locator_set_pool)
	...
	{
	  msg =
	    format (msg, "<%s-%d>", "remote", lsit - lcm->locator_set_pool);
	}
...
static clib_error_t *
lisp_add_del_map_resolver_command_fn (vlib_main_t *vm, unformat_input_t *input,
				      vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
lisp_show_mreq_itr_rlocs_command_fn (vlib_main_t *vm, unformat_input_t *input,
				     vlib_cli_command_t *cmd)
{
  lisp_cp_main_t *lcm = vnet_lisp_cp_get_main ();
					  ...
					  vlib_cli_command_t *cmd)
{
  ...
  if (vnet_lisp_use_petr (&ip, is_add))
    {
      error =
	clib_error_return (0, "failed to %s petr!", is_add ? "add" : "delete");
    }
...
static clib_error_t *
lisp_show_petr_command_fn (vlib_main_t *vm, unformat_input_t *input,
			   vlib_cli_command_t *cmd)
{
  lisp_cp_main_t *lcm = vnet_lisp_cp_get_main ();
  mapping_t *m;
...
static clib_error_t *
lisp_show_map_servers_command_fn (vlib_main_t *vm, unformat_input_t *input,
				  vlib_cli_command_t *cmd)
{
  lisp_msmr_t *ms;
  lisp_cp_main_t *lcm = vnet_lisp_cp_get_main ();
...
static clib_error_t *
lisp_show_stats_command_fn (vlib_main_t *vm, unformat_input_t *input,
			    vlib_cli_command_t *cmd)
{
...
static clib_error_t *
lisp_show_stats_details_command_fn (vlib_main_t *vm, unformat_input_t *input,
				    vlib_cli_command_t *cmd)
{
  ...
  vec_foreach (stat, stats)
    {
      vlib_cli_output (vm, "[%U, %U] [%U, %U] %7u %7u\n", format_fid_address,
		       &stat->seid, format_fid_address, &stat->deid,
		       format_ip_address, &stat->loc_rloc, format_ip_address,
		       &stat->rmt_rloc, stat->counters.packets,
		       stat->counters.bytes);
    }
  vec_free (stats);
  return 0;
...
static clib_error_t *
lisp_stats_enable_disable_command_fn (vlib_main_t *vm, unformat_input_t *input,
				      vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  u8 enable = 0;
