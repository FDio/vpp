...
static clib_error_t *
rdma_nldev_send_device_dump (int fd, u32 seq)
{
  struct
  {
    struct nlmsghdr h;
...
static clib_error_t *
rdma_nldev_send_port_dump (int fd, u32 seq, u32 dev_index)
{
  struct
  {
    struct nlmsghdr h;
    ...
    .h.nlmsg_len = NLMSG_LENGTH (NLA_ALIGN (NLA_HDRLEN + sizeof (u32))),
    .h.nlmsg_type = RDMA_NL_GET_TYPE (RDMA_NL_NLDEV, RDMA_NLDEV_CMD_PORT_GET),
    .h.nlmsg_flags = NLM_F_REQUEST | NLM_F_DUMP,
    .h.nlmsg_seq = seq,
    .a.nla_type = RDMA_NLDEV_ATTR_DEV_INDEX,
    .a.nla_len = NLA_HDRLEN + sizeof (u32),
    .dev_index = dev_index,
  };
...
static clib_error_t *
rdma_nldev_index_from_name (int fd, const char *ibdev_name, u32 seq, u32 *dev_index)
{
  uword found = 0;
  clib_error_t *err;

  if ((err = rdma_nldev_send_device_dump (fd, seq)))
    return err;
  ...
  while (1)
    {
      ...
      if (n < 0)
	return clib_error_return_unix (0, "RDMA netlink device dump recv");
      ...
      for (struct nlmsghdr *h = (struct nlmsghdr *) buf; NLMSG_OK (h, n); h = NLMSG_NEXT (h, n))
	{
	  ...
	  if (h->nlmsg_type == NLMSG_DONE)
	    return found ? 0 : clib_error_return (0, "cannot find RDMA device %s", ibdev_name);
			     ...
			     u8 *port_num)
{
  ...
  if (ibdev_index < 0)
    {
      if ((err = rdma_nldev_index_from_name (fd, ibdev_name, seq++, &dev_index)))
	goto done;
    }
  else
    dev_index = ibdev_index;

  if ((err = rdma_nldev_send_port_dump (fd, seq, dev_index)))
    goto done;
  ...
  while (1)
      ...
      for (struct nlmsghdr *h = (struct nlmsghdr *) buf; NLMSG_OK (h, n); h = NLMSG_NEXT (h, n))
	{
	  ...
	  if (h->nlmsg_type == NLMSG_DONE)
	    {
	      err =
		clib_error_return (0, "cannot map host-if %s to RDMA device %u", ifname, dev_index);
	      goto done;
	    }
	  ...
	  rdma_nldev_parse_attrs (h, tb, RDMA_NL_ARRAY_LEN (tb));
	  if (!rdma_nldev_attr_u32 (tb, RDMA_NLDEV_ATTR_NDEV_INDEX, &ndev_index) ||
	      !rdma_nldev_attr_u32 (tb, RDMA_NLDEV_ATTR_PORT_INDEX, &port_index))
	    continue;

	  if (ndev_index != if_index)
	    continue;
