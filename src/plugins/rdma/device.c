...
static clib_error_t *
rdma_nldev_send_get (int fd, u16 cmd, u16 flags, u32 seq, u16 attr_type, const u32 *attr_value)
{
  struct
  {
    struct nlmsghdr h;
  ...
  if (attr_value)
    {
      req.a.nla_type = attr_type;
      req.a.nla_len = NLA_HDRLEN + sizeof (req.value);
      req.value = *attr_value;
      req.h.nlmsg_len = NLMSG_LENGTH (NLA_ALIGN (req.a.nla_len));
    }
...
static clib_error_t *
rdma_nldev_index_from_name (int fd, const char *ibdev_name, u32 *dev_index)
{
  const u32 seq = 1;
  uword found = 0;
  clib_error_t *err;

  if ((err = rdma_nldev_send_get (fd, RDMA_NLDEV_CMD_GET, NLM_F_DUMP, seq, 0, 0)))
    return err;
...
rdma_nldev_port_from_ifname (const char *ifname, const char *ibdev_name, u8 *port_num)
{
  ...
  if ((err = rdma_nldev_index_from_name (fd, ibdev_name, &dev_index)))
    goto done;

  if ((err = rdma_nldev_send_get (fd, RDMA_NLDEV_CMD_PORT_GET, NLM_F_DUMP, seq,
				  RDMA_NLDEV_ATTR_DEV_INDEX, &dev_index)))
    goto done;
	  ...
	  if (h->nlmsg_type == NLMSG_DONE)
	    {
	      err = clib_error_return (0,
				       "cannot map host-if %s to RDMA device "
				       "%s",
				       ifname, ibdev_name);
	      goto done;
	    }
