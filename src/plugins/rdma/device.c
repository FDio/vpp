...
static void
rdma_rxq_clear_notifications (rdma_rxq_t * rxq)
{
  struct ibv_cq *cq;
  void *cq_ctx;
  while (ibv_get_cq_event (rxq->cq->channel, &cq, &cq_ctx) == 0)
    ibv_ack_cq_events (cq, 1);
}

static clib_error_t *
rdma_rxq_arm_notifications (rdma_rxq_t * rxq)
{
  int ret = ibv_req_notify_cq (rxq->cq, 0);
  if (ret != 0)
    return clib_error_return_unix (0, "ibv_req_notify_cq() failed");
  return 0;
...
rdma_rxq_comp_event_error_ready (clib_file_t * f)
{
  //rdma_main_t *rm = &rdma_main;
  //rdma_device_t *rd = vec_elt_at_index (rm->devices, f->private_data);
  rdma_rxq_t *rxq = RDMA_RXQ_FROM_CLIB_FILE_PRIVATE (f);
  rdma_device_t *rd = RDMA_DEVICE_FROM_RXQ (rxq);
  u16 qid = rxq - rd->rxqs;
...
rdma_rxq_comp_event_read_ready (clib_file_t * f)
{
  ...
  ibv_ack_cq_events (cq, 1);
  rdma_rxq_arm_notifications (rxq);
  return 0;
}
...
rdma_rxq_comp_event_init (rdma_device_t * rd, u16 qid)
{
  ...
  t.private_data = RDMA_RXQ_TO_CLIB_FILE_PRIVATE (rxq);
  t.description =
...
rdma_rxq_init (vlib_main_t * vm, rdma_device_t * rd, u16 qid, u32 n_desc)
{
  ...
  clib_error_t *err = rdma_rxq_comp_event_init (rd, qid);
  if (err)
    return err;
			       ...
			       vnet_hw_interface_rx_mode mode)
{
  ...
  if (mode == VNET_HW_INTERFACE_RX_MODE_POLLING)
	...
	rdma_rxq_clear_notifications (rxq);
    }
  else
    {
      if (old_mode == VNET_HW_INTERFACE_RX_MODE_POLLING)
