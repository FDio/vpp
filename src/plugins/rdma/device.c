...
rdma_dev_set_ucast (rdma_device_t * rd)
{
  ...
  rd->flow_ucast4 =
    rdma_rxq_init_flow (rd, rd->rx_qp4, &rd->hwaddr, &ucast, 0,
			IBV_FLOW_ATTR_FLAGS_DONT_TRAP);
  rd->flow_mcast4 =
	       ...
	       rdma_create_if_args_t * args)
{
...

  {
	...
	clib_memcpy (ifr.ifr_name, rd->linux_ifname,
		     clib_min (vec_len (rd->linux_ifname),
			       sizeof (ifr.ifr_name) - 1));
	if (ioctl (sock, SIOCGIFHWADDR, &ifr) == 0)
	  clib_memcpy (rd->hwaddr.bytes, ifr.ifr_hwaddr.sa_data, 6);
	else
