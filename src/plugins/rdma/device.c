...
#define RDMA_NL_ATTR_OK(a, len)                                              \
  ((len) >= (int) sizeof (struct nlattr) &&                                  \
   (a)->nla_len >= sizeof (struct nlattr) && (a)->nla_len <= (len))
...
rdma_nldev_send_port_get (int fd, u32 dev_index, u32 port_index)
{
  ...
  if ((err = rdma_nldev_add_u32 (&req.h, sizeof (req),
				 RDMA_NLDEV_ATTR_DEV_INDEX, dev_index)))
    return err;
  if ((err = rdma_nldev_add_u32 (&req.h, sizeof (req),
				 RDMA_NLDEV_ATTR_PORT_INDEX, port_index)))
    return err;
...
static clib_error_t *
rdma_nldev_get_device (int fd, const char *ibdev_name, u32 *dev_index,
		       u32 *port_index)
{
  char buf[8192];
  clib_error_t *err;
  ...
  while (1)
	   ...
	   h = NLMSG_NEXT (h, n))
	{
	  ...
	  if (h->nlmsg_type == NLMSG_DONE)
		...
		return 0;
	      return clib_error_return (0,
					"RDMA device %s not found in netlink",
					ibdev_name);
	    }
	  ...
	  if (h->nlmsg_type == NLMSG_ERROR)
	    {
	      ...
	      if (e->error)
		return clib_error_return (0, "RDMA netlink device dump: %s",
					  strerror (-e->error));
	      continue;
	    }
	  ...
	  if (rdma_nldev_attr_str_is (tb[RDMA_NLDEV_ATTR_DEV_NAME],
				      ibdev_name))
	    {
	      *dev_index = index;
	      *port_index = port;
...
static clib_error_t *
rdma_nldev_port_matches_ifname (int fd, u32 dev_index, u32 port_index,
				const char *ifname, uword *match)
{
  char buf[8192];
  clib_error_t *err;
  ...
  while (1)
	   ...
	   h = NLMSG_NEXT (h, n))
	{
	  ...
	  if (h->nlmsg_type == NLMSG_ERROR)
	    {
	      struct nlmsgerr *e = NLMSG_DATA (h);
	      if (e->error)
		return clib_error_return (0, "RDMA netlink port get: %s",
					  strerror (-e->error));
	      got_ack = 1;
	      continue;
	    }
	  ...
	  if (!tb[RDMA_NLDEV_ATTR_NDEV_NAME] ||
	      !rdma_nldev_attr_u32 (tb, RDMA_NLDEV_ATTR_DEV_INDEX,
				    &resp_dev) ||
	      !rdma_nldev_attr_u32 (tb, RDMA_NLDEV_ATTR_PORT_INDEX,
				    &resp_port))
	    continue;
...
static clib_error_t *
rdma_nldev_port_from_ifname (const char *ifname, const char *ibdev_name,
			     u8 *port_num)
{
  int fd;
  ...
  if ((err = rdma_nldev_port_matches_ifname (fd, dev_index, port_index,
					     ifname, &match)))
    goto done;

  if (!match)
    {
      err = clib_error_return (0, "cannot map host-if %s to RDMA device %s "
			       "port %u",
  ...
  if (port_index > 255)
    {
      err = clib_error_return (0, "RDMA port %u exceeds supported port-num "
			       "range",
			       port_index);
      goto done;
    }
  ...
  if (is_mlx5dv)
    {
      ...
      dvcq.cqe_comp_res_format = is_striding ?
	MLX5DV_CQE_RES_FORMAT_CSUM_STRIDX : MLX5DV_CQE_RES_FORMAT_HASH;
      dvcq.cqe_size = 64;
...
rdma_create_if (vlib_main_t * vm, rdma_create_if_args_t * args)
{
  ...
  if (!args->port_num)
    {
      args->error =
	rdma_nldev_port_from_ifname ((char *) args->ifname, ibdev_name,
				     &rd->port_num);
      if (args->error)
	goto err2;
    }
