  ...
  if (rd->flags & RDMA_DEVICE_F_MLX5DV)
    {
      ...
      s = format (s, "\n%Uwq: stride %u wqe-cnt %u comp-mask 0x%lx",
		  format_white_space, indent + 2, rxq->wq_stride,
		  rxq->wqe_cnt, rxq->wq_comp_mask);
      s = format (s, "\n%Ucq: cqn %u cqe-cnt %u comp-mask 0x%lx ci %u",
		  format_white_space, indent + 2, rxq->cqn,
		  1 << rxq->log2_cq_size, rxq->cq_comp_mask, rxq->cq_ci);
