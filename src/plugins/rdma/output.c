...
static_always_inline void
rdma_mlx5_tso_prepare_tcp_checksum (vlib_buffer_t *b)
{
  tcp_header_t *tcp = (tcp_header_t *) (b->data + vnet_buffer (b)->l4_hdr_offset);

  /* mlx5 adds each segment's L4 length, so exclude the super-packet length
   * from the pseudo-header checksum seed. */
  if (b->flags & VNET_BUFFER_F_IS_IP4)
	...
	.proto = ip4->protocol,
      ...
      tcp->checksum = ~clib_ip_csum ((u8 *) &psh, sizeof (psh));
    }
	...
	.src = ip6->src_address,
	.dst = ip6->dst_address,
	.proto = clib_host_to_net_u32 ((u32) ip6->protocol),
      };
			...
			const u32 sq_mask, const u32 lkey)
{
  ...
  rdma_mlx5_tso_prepare_tcp_checksum (b);
