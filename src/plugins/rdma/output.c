  ...
  if (PREDICT_FALSE (hdr_sz > RDMA_MLX5_TSO_HDR_MAX))
    {
      ...
      return RDMA_MLX5_TX_PACKET_DONE;
    }
...
static_always_inline rdma_mlx5_tx_packet_result_t
rdma_mlx5_try_tso_or_regular (vlib_main_t *vm, const vlib_node_runtime_t *node,
			      const rdma_device_t *rd, rdma_txq_t *txq, vlib_buffer_t *b, u32 bi,
			      u16 *tail_p, u32 *wqe_n_p, u32 sq_mask, u32 dseg_mask, u32 mask,
			      u32 lkey_be, rdma_mlx5_wqe_t **last_p, const int is_tso)
{
  ...
  while (n >= 1 && wqe_n >= 1)
    {
      ...
      r = rdma_mlx5_try_tso_or_regular (vm, node, rd, txq, b[0], bi[0], &tail, &wqe_n, sq_mask,
					dseg_mask, mask, lkey_be, &last, is_tso);
	  ...
	  if (b[0]->flags & VLIB_BUFFER_NEXT_PRESENT)
	    {
	      ...
	      STATIC_ASSERT (RDMA_MLX5_WQE_DS * sizeof (struct mlx5_wqe_data_seg) ==
			       MLX5_SEND_WQE_BB,
			     "wrong size");
