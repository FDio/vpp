 ...
 */
static_always_inline void
rdma_mlx5_tso_append_chain (vlib_main_t *vm, const vlib_node_runtime_t *node,
			     rdma_txq_t *txq, vlib_buffer_t *b, u32 bi,
			     u16 tail, u32 sq_mask, u32 dseg_mask, u32 mask,
			     u8 base_ds, u32 lkey_be, u32 *total_wqebb_p,
			     u32 *wqe_n_p, u16 *tail_p,
			     rdma_mlx5_wqe_t **last_p)
{
  ...
  u32 chained_n = 0;
  const u32 dseg_max =
    clib_min (RDMA_MLX5_WQE_DS * (*wqe_n_p) - base_ds,
  ...
  if (chained_b->flags & VLIB_BUFFER_NEXT_PRESENT)
  ...
  else
    {
      ...
      /* Additional WQEBBs must not free chain fragments a second time. */
  ...
  while (n >= 1 && wqe_n >= 1)
      ...
      if (b[0]->flags & VNET_BUFFER_F_GSO)
	{
	  ...
	  if (!(b[0]->flags & VLIB_BUFFER_NEXT_PRESENT))
	    ...
	    {
	      rdma_mlx5_tso_append_chain (vm, node, txq, b[0], bi[0], tail,
					  sq_mask, dseg_mask, mask,
					  base_ds, lkey_be, &total_wqebb,
					  &wqe_n, &tail, &last);
	    }
				   ...
				   vlib_buffer_t **b)
{
  ...
  while (n_posted < n_left_from)
	...
	{
	  /* Chain exceeds the 64-entry SGE limit: drop this packet.
	   * Free the whole chain now; n_posted is not incremented so
	   * this buffer index will never reach rdma_device_output_free. */
	  vlib_buffer_free_one (vm, b[n_posted]->next_buffer);
			      ...
			      vlib_buffer_t **b)
{
  ...
  if (n_posted == 0)
    return rdma_device_output_tx_ibverb_slow (vm, node, rd, txq, n_left_from, first);
