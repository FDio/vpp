			      ...
			      rdma_txq_t * txq)
{
  ...
  u32 mask = pow2_mask (txq->dv_cq_log2sz);
  struct mlx5_cqe64 *cqes = txq->dv_cq_cqes, *cur = cqes + (idx & mask);
  u8 op_own, saved;
  ...
  for (;;)
    {
      ...
      if (((idx >> txq->dv_cq_log2sz) & MLX5_CQE_OWNER_MASK) !=
  ...
  ASSERT (RDMA_TXQ_USED_SZ (txq->head, wqe->ctrl.imm) <= RDMA_TXQ_BUF_SZ (txq)
	  && RDMA_TXQ_USED_SZ (wqe->ctrl.imm,
			       txq->tail) < RDMA_TXQ_BUF_SZ (txq));
				     ...
				     const u16 tail)
{
  ...
  ASSERT (tail != txq->tail
	  && RDMA_TXQ_AVAIL_SZ (txq, txq->head,
				txq->tail) >= RDMA_TXQ_USED_SZ (txq->tail,
								tail));
				    ...
				    rdma_mlx5_wqe_t * wqe, u16 tail)
{
  ...
  vlib_buffer_copy_indices (RDMA_TXQ_BUF (txq, txq->tail), bi,
			    n_left_from - n);
...
#define RDMA_MLX5_WQE_DS_MAX    (1 << 5)
	  ...
	  while (chained_n < dseg_max
		 && chained_b->flags & VLIB_BUFFER_NEXT_PRESENT)
	    {
	      ...
	      dseg = RDMA_TXQ_DV_DSEG (txq, tail + 1, chained_n);
	      if (((clib_address_t) dseg & (MLX5_SEND_WQE_BB - 1)) == 0)
		{
		  ...
		  *RDMA_TXQ_BUF (txq, tail + 1 + RDMA_TXQ_DV_DSEG2WQE (chained_n)) =
