...
#define RDMA_MLX5_WQE_DS_MAX    (1 << 5)
  ...
  /* we fail to enqueue even a single packet */
			   ...
			   u32 n_left_from, u32 * bi, int is_mlx5dv)
{
  ...
  n_left_from = is_mlx5dv ?
    rdma_device_output_tx_mlx5 (vm, node, rd, txq, n_left_from, bi, b,
				&new_tail) : rdma_device_output_tx_ibverb (vm,
									   node,
									   rd,
									   txq,
									   n_left_from,
  ...
  txq->tail = is_mlx5dv ? new_tail : txq->tail + n_left_from;
