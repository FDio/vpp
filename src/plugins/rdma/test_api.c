...
api_rdma_create_v2 (vat_main_t * vam)
{
  ...
  mp->txq_size = clib_host_to_net_u16 (args.txq_size);
  mp->mode = api_rdma_mode (args.mode);
  mp->disable_striding_rq = args.disable_striding_rq;
  mp->disable_legacy_chained_bufs = args.disable_legacy_chained_bufs;
  mp->max_pktlen_override = clib_host_to_net_u16 (args.max_pktlen_override);
