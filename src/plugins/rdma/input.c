...
/* static_always_inline void
rdma_device_legacy_input_refill_stich_chains (vlib_main_t * vm,
					      rdma_rxq_t * rxq,
					      rdma_per_thread_data_t * ptd,
					      u32 slot, u32 n)
{
  vlib_buffer_t **bufs = ptd->tmp_bufs;
  vlib_get_buffers (vm, rxq->bufs + slot, bufs, n);
  u32 *second_bufs = &rxq->second_bufs[slot];
  while (n >= 8)
    {
      bufs[0]->next_buffer = second_bufs[0];
      bufs[1]->next_buffer = second_bufs[1];
      bufs[2]->next_buffer = second_bufs[2];
      bufs[3]->next_buffer = second_bufs[3];
      bufs[4]->next_buffer = second_bufs[4];
      bufs[5]->next_buffer = second_bufs[5];
      bufs[6]->next_buffer = second_bufs[6];
      bufs[7]->next_buffer = second_bufs[7];
      ...
      bufs[4]->flags |= VLIB_BUFFER_NEXT_PRESENT;
      bufs[5]->flags |= VLIB_BUFFER_NEXT_PRESENT;
      bufs[6]->flags |= VLIB_BUFFER_NEXT_PRESENT;
      bufs[7]->flags |= VLIB_BUFFER_NEXT_PRESENT;
      bufs += 8;
      n -= 8;
      second_bufs += 8;
    }
...
rdma_device_input_refill (vlib_main_t * vm, rdma_device_t * rd,
			  rdma_rxq_t * rxq, vlib_buffer_t * bt, int is_mlx5dv,
			  int is_striding)
{
  u32 n_alloc, n;
  u16 ring_space;
  ...
  u32 lkey = rd->lkey;
  int log_stride_per_wqe = is_striding ? rxq->log_stride_per_wqe : 0;
  int log_wqe_sz = rxq->log_wqe_sz;

  /*In legacy mode, maybe some buffers chains are incomplete? */
  if (PREDICT_FALSE (!is_striding && (rxq->incomplete_tail != rxq->tail)))
    {
	...
	rdma_device_legacy_input_refill_additional (vm, rd, rxq, ptd, bt,
						    slot,
						    n_incomplete);
      /*rdma_device_legacy_input_refill_stich_chains (vm, rxq, ptd, slot,
         n_completed); */
  ...
  if (is_mlx5dv)
    {
      ...
      int log_skip_wqe = is_striding ? 0 : log_wqe_sz;
	  ...
	  if (PREDICT_FALSE (rxq->n_total_additional_segs))
	    n_alloc =
	      rdma_device_legacy_input_refill_additional (vm, rd, rxq, ptd,
							  bt, first_slot,
							  n_alloc);
	  /*rdma_device_legacy_input_refill_stich_chains (vm, rxq, ptd,
	     first_slot, n_alloc); */
	}
...
refill:
  rdma_device_input_refill (vm, rd, rxq, &bt, use_mlx5dv,
