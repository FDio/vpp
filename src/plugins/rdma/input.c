...
static_always_inline void
ibv_set_recv_wr_and_sge_chain (vlib_main_t *vm, struct ibv_recv_wr *w,
			       struct ibv_sge *s, u32 head_bi, u32 next_bi,
			       u32 n_sge, u32 data_size, u32 lkey)
{
  u32 bi = head_bi;
					    ...
					    u32 first_slot, u32 n_alloc)
{
  ...
  for (i = 0; i < n_alloc; i++)
	...
	rxq->wqes ? rxq->wqes + ((first_slot + i) << log_wqe_sz) : 0;
      if (chain_sz == 0)
	continue;
			  ...
			  const int is_mlx5dv, const int is_striding)
{
  ...
  if (PREDICT_FALSE
      (is_legacy_multiseg && (rxq->incomplete_tail != rxq->tail)))
    {
  ...
  if (is_legacy_multiseg)
	...
	return;
      ...
      while (n >= 1)
	{
	  ibv_set_recv_wr_and_sge_chain (vm, w++, s, bufs[0],
					 rxq->second_bufs[first_slot],
...
static_always_inline void
rdma_device_legacy_rq_fix_chains (vlib_main_t * vm, rdma_rxq_t * rxq,
				  vlib_buffer_t ** bufs, u32 qs_mask, u32 n)
{
  ...
  if (use_mlx5dv)
      ...
      else
	{
	  ...
	  if (PREDICT_FALSE (slow_path_needed))
	    rdma_device_legacy_rq_fix_chains (vm, rxq, bufs, mask,
					      n_rx_packets);
	}
