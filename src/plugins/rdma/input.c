		   ...
		   u32 mask, u32 * byte_cnt)
{
  ...
  /* The first mini-CQE lives after the title CQE, but subsequent mini-CQE
   * groups advance relative to the title CQE index. */
					    ...
					    int *l4_ok_all)
{
  ...
  /* verify that all ip4 packets have l3_ok flag set and convert packet
     ...
     Also validate the L4_OK bit across all CQEs: if all packets have
  ...
  if (n_left >= 16)
	...
	skip_ip4_cksum = 0;
      if (!u16x16_is_all_zero ((l4_ok_acc16 & l4_ok16) != l4_ok16))
	l4_ok_all_local = 0;
    }
#elif defined CLIB_HAVE_VEC128
  if (n_left >= 8)
	...
	skip_ip4_cksum = 0;
      if (!u16x8_is_all_zero ((l4_ok_acc8 & l4_ok8) != l4_ok8))
	l4_ok_all_local = 0;
    }
#endif
			  ...
			  u16 qid, const int use_mlx5dv)
{
  ...
  if (use_mlx5dv)
	...
	rdma_device_mlx5dv_l3_validate_and_swap_bc (ptd, n_rx_packets, bc, &l4_ok_all);
       ...
       * batch has L4_OK set in the CQEs. Mixed batches fall back to the
       * normal software validation path by leaving the checksum flags
       * unset on the buffer template. */
