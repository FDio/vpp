...
cqe_set_owner (mlx5dv_cqe_t * cqe, u32 n_left, u8 owner)
{
  while (n_left >= 8)
    {
      cqe[0].as_u8[0x3f] = owner;
      cqe[1].as_u8[0x3f] = owner;
      cqe[2].as_u8[0x3f] = owner;
      cqe[3].as_u8[0x3f] = owner;
      cqe[4].as_u8[0x3f] = owner;
      cqe[5].as_u8[0x3f] = owner;
      cqe[6].as_u8[0x3f] = owner;
      cqe[7].as_u8[0x3f] = owner;
      n_left -= 8;
      cqe += 8;
    }
  while (n_left)
...
rdma_device_poll_cq_mlx5dv (rdma_rxq_t * rxq, u32 * byte_cnt)
{
  ...
  while (n_rx_packets < VLIB_FRAME_SIZE)
	...
	{
	  u32 n_mini_cqes = clib_net_to_host_u32 (cqe->as_u32[11]);
	  ...
	  if (n_left >= n_mini_cqes)
	  ...
	  else
	    {
	      if (1)
		{
		  fformat (stderr, "n_mini_cqes = %u, n_rx_packets = %u\n",
			   n_mini_cqes, n_rx_packets);
		  fformat (stderr, "cqe_ci %u\n", cq_ci);
      ...
      if (cqe_last_byte == 0x20)
	{
      ...
      fformat (stderr, "cq_ci 0x%x last byte 0x%x owner 0x%x\n",
	       cq_ci, cqe_last_byte, owner);
