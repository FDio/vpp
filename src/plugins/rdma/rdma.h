...
#define RDMA_MLX5_EMPW_TITLE_DS		  2
#define RDMA_MLX5_EMPW_INLINE_MIN_PACKETS 2
#define RDMA_MLX5_EMPW_INLINE_HDR_SIZE	  sizeof (u32)
#define RDMA_MLX5_EMPW_INLINE_DEFAULT	  60
#define RDMA_MLX5_EMPW_INLINE_MAX                                                                  \
  ((u32) (((RDMA_MLX5_WQE_DS_MAX - RDMA_MLX5_EMPW_TITLE_DS) / RDMA_MLX5_EMPW_INLINE_MIN_PACKETS) * \
...
/* Keep enough room for a multi-packet WQE.  A single 924-byte inline packet
 * would fill the format but is emitted through the regular SEND fallback. */
STATIC_ASSERT (RDMA_MLX5_EMPW_INLINE_MAX == 460, "unexpected eMPW inline limit");
...
typedef struct
{
  ...
  /* eMPW uses independent WQEBB and buffer counters because one WQE can
   * carry several packets.  The map records the buffer tail for every
   * posted WQE, including WQEs which are normally unsignalled. */
  u16 dv_sq_head;
  u16 dv_sq_tail;
  u16 *dv_sq_buf_tail;
