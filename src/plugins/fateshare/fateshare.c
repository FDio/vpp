...
launch_monitor (fateshare_main_t *kmp)
{
  ...
  u8 *logname = format (0, "/tmp/log.monitor.txt\0");
  ...
  int logfd = open ((char *) logname, O_APPEND | O_RDWR | O_CREAT, 0777);
  if (logfd < 0)
    {
  ...
  if (cpid)
	...
	{
	  fprintf (stderr, "parent pid changed while starting\n");
	  exit (1);
	}
      ...
      if (fd < 0)
	{
      ...
      char *ppid_str = (char *) format (0, "%lld\0", ppid_before_fork);

      char **argv = 0;
      vec_validate (argv, vec_len (kmp->commands) + 2 - 1);
      argv[0] = (void *) scmd;
      argv[1] = ppid_str;
      int i;
      vec_foreach_index (i, kmp->commands)
	{
	  argv[2 + i] = (char *) kmp->commands[i];
	}

      int res = execv (argv[0], argv);
      clib_warning ("ERROR execve: %d", res);
      perror ("execve");
...
fateshare_config (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  *new_command = 0;
  ...
  if (fmp->monitor_cmd == 0)
    {
      fmp->monitor_cmd = format (
	0, "%s\0",
	"./build-root/install-vpp_debug-native/vpp/bin/vpp_fateshare_monitor");
    }
  error = launch_monitor (fmp);
  clib_warning ("end of fate share config");
...
// VLIB_CONFIG_FUNCTION (fateshare_config, "fateshare");

VLIB_INIT_FUNCTION (fateshare_init);

VLIB_PLUGIN_REGISTER () = {
  .version = VPP_BUILD_VER,
  .description = "fateshare plugin description goes here",
};
