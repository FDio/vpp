...
#include <lina/lina.h>
#include <lina/private.h>

#define foreach_lina_input_error \
  _(BUFFER_ALLOC_FAIL, "buffer allocation failed")		\
  _(NOT_IP, "not ip packet")

typedef enum
{
#define _(f,s) LINA_INPUT_ERROR_##f,
  foreach_lina_input_error
#undef _
    LINA_INPUT_N_ERROR,
} lina_input_error_t;

static __clib_unused char *lina_input_error_strings[] = {
#define _(n,s) s,
  foreach_lina_input_error
#undef _
};
...
format_lina_input_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  lina_input_trace_t *t = va_arg (*args, lina_input_trace_t *);
  u32 indent = format_get_indent (s);

  s = format (s, "lina: hw_if_index %d next-index %d",
...
lina_next_from_ip_hdr (vlib_node_runtime_t * node, vlib_buffer_t * b)
{
  ...
  b->error = node->errors[LINA_INPUT_ERROR_NOT_IP];
  return VNET_DEVICE_INPUT_NEXT_DROP;
}

static_always_inline void
lina_trace_buffer (vlib_main_t * vm, vlib_node_runtime_t * node,
		   lina_if_t * mif, vlib_buffer_t * b, u32 next, u16 qid,
		   uword * n_tracep)
{
...
static_always_inline void
lina_add_copy_op (lina_per_thread_data_t * ptd, void *data, u32 len,
...
static_always_inline void
lina_add_to_chain (vlib_main_t * vm, vlib_buffer_t * b, u32 * buffers,
		   u32 buffer_size)
{
  vlib_buffer_t *seg = b;
...
static_always_inline u32
lina_desc_is_invalid (lina_if_t * mif, lina_desc_t * d, u32 buffer_length)
{
  u32 rv;
  u16 valid_flags = LINA_DESC_FLAG_NEXT;
...
lina_device_input_inline (vlib_main_t * vm, vlib_node_runtime_t * node,
			  vlib_frame_t * frame, lina_if_t * mif,
			  lina_ring_type_t type, u16 qid,
			  lina_interface_mode_t mode)
{
  ...
  lina_per_thread_data_t *ptd = vec_elt_at_index (mm->per_thread_data,
						  thread_index);
  ...
  lina_copy_op_t *co;
  lina_packet_op_t *po;
  lina_region_index_t last_region = ~0;
  ...
  start_offset = (mode == LINA_INTERFACE_MODE_IP) ? 14 : 0;

  /* for S2M rings, we are consumers of packet buffers, and for M2S rings we
     are producers of empty buffers */
  cur_slot = (type == LINA_RING_S2M) ? mq->last_head : mq->last_tail;
  last_slot = (type == LINA_RING_S2M) ? ring->head : ring->tail;
  if (cur_slot == last_slot)
    goto refill;
  n_slots = last_slot - cur_slot;

  /* construct copy and packet vector out of ring slots */
  while (n_slots && n_rx_packets < LINA_RX_VECTOR_SZ)
    {
      ...
      do
	{
	  ...
	  lina_add_copy_op (ptd, mb0 + src_off, bytes_to_copy, dst_off,
			    n_buffers - 1);
      ...
      cur_slot++;
      n_slots--;
      if ((d0->flags & LINA_DESC_FLAG_NEXT) && n_slots)
	{
	  src_off = 0;
	  goto next_slot;
	}
  ...
  if (PREDICT_FALSE (n_alloc != n_buffers))
	...
	vlib_buffer_free (vm, ptd->buffers, n_alloc);
      vlib_error_count (vm, node->node_index,
			LINA_INPUT_ERROR_BUFFER_ALLOC_FAIL, 1);
      goto refill;
    }
  ...
  u16 nexts[LINA_RX_VECTOR_SZ], *next = nexts;
  u32 to_next_buffers[LINA_RX_VECTOR_SZ], *bi = to_next_buffers;
  ...
  if (mode == LINA_INTERFACE_MODE_ETHERNET)
    {
  ...
  while (n_from >= 8)
    {
      ...
      lina_add_to_chain (vm, b0, ptd->buffers + fbvi[0] + 1, buffer_size);
      lina_add_to_chain (vm, b1, ptd->buffers + fbvi[1] + 1, buffer_size);
      lina_add_to_chain (vm, b2, ptd->buffers + fbvi[2] + 1, buffer_size);
      lina_add_to_chain (vm, b3, ptd->buffers + fbvi[3] + 1, buffer_size);

      if (mode == LINA_INTERFACE_MODE_IP)
	{
	  next[0] = lina_next_from_ip_hdr (node, b0);
	  next[1] = lina_next_from_ip_hdr (node, b1);
	  next[2] = lina_next_from_ip_hdr (node, b2);
	  next[3] = lina_next_from_ip_hdr (node, b3);
	}
...
static_always_inline uword
lina_device_input_zc_inline (vlib_main_t * vm, vlib_node_runtime_t * node,
			     vlib_frame_t * frame, lina_if_t * mif,
			     u16 qid, lina_interface_mode_t mode)
{
  vnet_main_t *vnm = vnet_get_main ();
  lina_main_t *mm = &lina_main;
  lina_ring_t *ring;
  ...
  next_index = (mode == LINA_INTERFACE_MODE_IP) ?
  ...
  /* process ring slots */
  vec_validate_aligned (ptd->buffers, LINA_RX_VECTOR_SZ,
  ...
  while (n_slots && n_rx_packets < LINA_RX_VECTOR_SZ)
    {
      ...
      if (0 && lina_desc_is_invalid (mif, d0, buffer_length))
	return 0;

      cur_slot++;
      n_slots--;
      if (PREDICT_FALSE ((d0->flags & LINA_DESC_FLAG_NEXT) && n_slots))
	{
  ...
  while (n_from)
      ...
      while (n_from >= 8 && n_left_to_next >= 4)
	{
	  ...
	  if (mode == LINA_INTERFACE_MODE_IP)
	    {
	      next0 = lina_next_from_ip_hdr (node, b0);
	      next1 = lina_next_from_ip_hdr (node, b1);
	      next2 = lina_next_from_ip_hdr (node, b2);
	      next3 = lina_next_from_ip_hdr (node, b3);
	    }
	  else if (mode == LINA_INTERFACE_MODE_ETHERNET)
	    {
	  ...
	  if (PREDICT_FALSE (n_trace > 0))
	    {
	      lina_trace_buffer (vm, node, mif, b0, next0, qid, &n_trace);
	      if (PREDICT_FALSE (n_trace > 0))
		lina_trace_buffer (vm, node, mif, b1, next1, qid, &n_trace);
	      if (PREDICT_FALSE (n_trace > 0))
		lina_trace_buffer (vm, node, mif, b2, next2, qid, &n_trace);
	      if (PREDICT_FALSE (n_trace > 0))
		lina_trace_buffer (vm, node, mif, b3, next3, qid, &n_trace);
	    }
...
refill:
  ...
  lina_desc_t *dt = &ptd->desc_template;
  clib_memset (dt, 0, sizeof (lina_desc_t));
  ...
  while (n_alloc >= 32)
    {
      ...
      clib_memcpy (d0, dt, sizeof (lina_desc_t));
      clib_memcpy (d1, dt, sizeof (lina_desc_t));
      clib_memcpy (d2, dt, sizeof (lina_desc_t));
      clib_memcpy (d3, dt, sizeof (lina_desc_t));
...
VLIB_NODE_FN (lina_input_node) (vlib_main_t * vm,
				...
				vlib_frame_t * frame)
{
  ...
  lina_interface_mode_t mode_ip = LINA_INTERFACE_MODE_IP;
  lina_interface_mode_t mode_eth = LINA_INTERFACE_MODE_ETHERNET;

  foreach_device_and_queue (dq, rt->devices_and_queues)
  {
    lina_if_t *mif;
    mif = vec_elt_at_index (mm->interfaces, dq->dev_instance);
    if ((mif->flags & LINA_IF_FLAG_ADMIN_UP) &&
	(mif->flags & LINA_IF_FLAG_CONNECTED))
      {
	if (mif->flags & LINA_IF_FLAG_ZERO_COPY)
	  {
	    if (mif->mode == LINA_INTERFACE_MODE_IP)
	      n_rx += lina_device_input_zc_inline (vm, node, frame, mif,
						   dq->queue_id, mode_ip);
	    else
	      n_rx += lina_device_input_zc_inline (vm, node, frame, mif,
						   dq->queue_id, mode_eth);
	  }
	else if (mif->flags & LINA_IF_FLAG_IS_SLAVE)
	  {
	    if (mif->mode == LINA_INTERFACE_MODE_IP)
	      n_rx += lina_device_input_inline (vm, node, frame, mif,
						LINA_RING_M2S, dq->queue_id,
						mode_ip);
	    else
	      n_rx += lina_device_input_inline (vm, node, frame, mif,
						LINA_RING_M2S, dq->queue_id,
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (lina_input_node) = {
  .name = "lina-input",
  .sibling_of = "device-input",
  .format_trace = format_lina_input_trace,
  .type = VLIB_NODE_TYPE_INPUT,
  .state = VLIB_NODE_STATE_INTERRUPT,
  .n_errors = LINA_INPUT_N_ERROR,
  .error_strings = lina_input_error_strings,
};
