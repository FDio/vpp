...
u32
line_get_n_free_slots (u32 instance_index, u32 ring_index)
{
  ...
  lin = pool_elt_at_index (lm->instances, instance_index);
  ring = vec_elt_at_index (lin->rings, ring_index);
  mask = (1 << lin->shm_hdr->log2_ring_sz) - 1;
  slot = ring->hdr->head;
  return (ring->last_tail - slot- 1) & mask;
}
  ...
  for (i = 0; i < n_rings; i++)
    {
      ...
      vec_validate_aligned (ring->buffer_indices,
			    (1 << lin->log2_ring_sz) - 1,
