...
typedef enum
{
#define _(f,s) LINA_ENQ_ERROR_##f,
				  ...
				  vlib_frame_t * frame)
{
  ...
  /* previous node can request non-default instance by setting
     LINA_ENQUEUE_FRAME_F_INSTANCE_INDEX_VALID frame flag and passing
     instance index as scalar argument */
  if (frame->flags & LINA_ENQUEUE_FRAME_F_INSTANCE_INDEX_VALID)
    {
      lina_enqueue_frame_t *lef = vlib_frame_scalar_args (frame);
      instance_index = lef->instance_index;
    }
  ...
  n_slots = (ring->last_tail - slot - 1) & mask;

  while (n_left && n_slots)
    {
      ...
      ring->buffer_indices[slot] = from[0];
  ...
  if (n_left)
    {
      from = vlib_frame_vector_args (frame) + frame->n_vectors - n_left;
      vlib_buffer_free (vm, from, n_left);
      vlib_error_count (vm, node->node_index, LINA_ENQ_ERROR_NO_FREE_SLOTS,
			n_left);
    }
  ...
  if (n_left != frame->n_vectors)
    {
      u64 b = 1;
      int __clib_unused r;
      r = write (ring->event_fd, &b, sizeof (b));
    }
...
VLIB_REGISTER_NODE (lina_enqueue_node) = {
  ...
  .scalar_size = sizeof (lina_enqueue_frame_t),
