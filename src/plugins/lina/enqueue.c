...
#define foreach_lina_tx_func_error	       \
...
#define _(f,s) LINA_TX_ERROR_##f,
  foreach_lina_tx_func_error
#undef _
    LINA_TX_N_ERROR,
} lina_tx_func_error_t;

static char *lina_tx_func_error_strings[] = {
#define _(n,s) s,
  foreach_lina_tx_func_error
#undef _
};
...
u8 *
format_lina_device_name (u8 * s, va_list * args)
{
  u32 dev_instance = va_arg (*args, u32);
  lina_main_t *mm = &lina_main;
  lina_instance_t *mif = pool_elt_at_index (mm->interfaces, dev_instance);
  lina_socket_file_t *msf;

  msf = pool_elt_at_index (mm->socket_files, mif->socket_file_index);
  s = format (s, "lina%lu/%lu", msf->socket_id, mif->id);
  return s;
}
#endif

static u8 *
format_lina_device (u8 * s, va_list * args)
{
  u32 dev_instance = va_arg (*args, u32);
  int verbose = va_arg (*args, int);
  u32 indent = format_get_indent (s);

  s = format (s, "LINA interface");
  if (verbose)
    {
...
static u8 *
format_lina_tx_trace (u8 * s, va_list * args)
{
...
lina_interface_tx_inline (vlib_main_t * vm, vlib_node_runtime_t * node,
			  vlib_frame_t * frame, lina_instance_t * mif,
			  lina_ring_type_t type, lina_queue_t * mq,
			  lina_per_thread_data_t * ptd)
{
  lina_ring_t *ring;
...
retry:
  ...
  slot = (type == LINA_RING_S2M) ? ring->head : ring->tail;
  ...
  while (n_left && free_slots)
      ...
      if (PREDICT_FALSE (last_region != d0->region))
	{
      ...
      dst_left = (type == LINA_RING_S2M) ? mif->run.buffer_size : d0->length;
      ...
      while (src_left)
	{
	  if (PREDICT_FALSE (dst_left == 0))
	      ...
	      if (free_slots)
		{
		  slot++;
		  free_slots--;
		  d0->flags = LINA_DESC_FLAG_NEXT;
		  d0 = &ring->desc[slot & mask];
		  dst_off = 0;
		  dst_left =
		    (type ==
		     LINA_RING_S2M) ? mif->run.buffer_size : d0->length;
	      ...
	      else
		{
		  ...
		  vlib_error_count (vm, node->node_index,
				    LINA_TX_ERROR_ROLLBACK, 1);
		  slot = saved_slot;
		  goto no_free_slots;
		}
	    }
	  bytes_to_copy = clib_min (src_left, dst_left);
	  lina_add_copy_op (ptd, mb0 + dst_off, bytes_to_copy, src_off,
			    vec_len (ptd->buffers));
...
no_free_slots:
  ...
  if (n_left)
    {
      vlib_error_count (vm, node->node_index, LINA_TX_ERROR_NO_FREE_SLOTS,
			n_left);
    }

  if ((ring->flags & LINA_RING_FLAG_MASK_INT) == 0 && mq->int_fd > -1)
    {
      u64 b = 1;
...
static_always_inline uword
lina_interface_tx_zc_inline (vlib_main_t * vm, vlib_node_runtime_t * node,
			     vlib_frame_t * frame, lina_instance_t * mif,
			     lina_queue_t * mq, lina_per_thread_data_t * ptd)
{
  lina_ring_t *ring = mq->ring;
  u32 *buffers = vlib_frame_args (frame);
  u32 n_left = frame->n_vectors;
...
VNET_DEVICE_CLASS_TX_FN (lina_device_class) (vlib_main_t * vm,
					     vlib_node_runtime_t * node,
					     vlib_frame_t * frame)
{
  lina_main_t *nm = &lina_main;
  vnet_interface_output_runtime_t *rund = (void *) node->runtime_data;
  lina_instance_t *mif =
    pool_elt_at_index (nm->interfaces, rund->dev_instance);
  lina_queue_t *mq;
  u32 thread_index = vm->thread_index;
  lina_per_thread_data_t *ptd = vec_elt_at_index (lina_main.per_thread_data,
						  thread_index);
  ...
  if (mif->flags & LINA_IF_FLAG_ZERO_COPY)
    return lina_interface_tx_zc_inline (vm, node, frame, mif, mq, ptd);
  else if (mif->flags & LINA_IF_FLAG_IS_SLAVE)
    return lina_interface_tx_inline (vm, node, frame, mif, LINA_RING_S2M,
				     mq, ptd);
  else
    return lina_interface_tx_inline (vm, node, frame, mif, LINA_RING_M2S,
				     mq, ptd);
}

static void
lina_set_interface_next_node (vnet_main_t * vnm, u32 hw_if_index,
			      u32 node_index)
{
  lina_main_t *apm = &lina_main;
  ...
  mif->per_interface_next_index =
    vlib_node_add_next (vlib_get_main (), lina_input_node.index, node_index);
}

static void
lina_clear_hw_interface_counters (u32 instance)
{
  /* Nothing for now */
}

static clib_error_t *
lina_interface_rx_mode_change (vnet_main_t * vnm, u32 hw_if_index, u32 qid,
			       vnet_hw_interface_rx_mode mode)
{
  lina_main_t *mm = &lina_main;
  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
  lina_instance_t *mif = pool_elt_at_index (mm->interfaces, hw->dev_instance);
  lina_queue_t *mq = vec_elt_at_index (mif->rx_queues, qid);

  if (mode == VNET_HW_INTERFACE_RX_MODE_POLLING)
    mq->ring->flags |= LINA_RING_FLAG_MASK_INT;
  else
    mq->ring->flags &= ~LINA_RING_FLAG_MASK_INT;
...
static clib_error_t *
lina_interface_admin_up_down (vnet_main_t * vnm, u32 hw_if_index, u32 flags)
{
  lina_main_t *mm = &lina_main;
  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
  lina_instance_t *mif = pool_elt_at_index (mm->interfaces, hw->dev_instance);
  ...
  if (flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP)
    mif->flags |= LINA_IF_FLAG_ADMIN_UP;
  else
    mif->flags &= ~LINA_IF_FLAG_ADMIN_UP;
...
static clib_error_t *
lina_subif_add_del_function (vnet_main_t * vnm,
			     u32 hw_if_index,
...
/* *INDENT-OFF* */
VNET_DEVICE_CLASS (lina_device_class) = {
  .name = "lina",
  .format_device_name = format_lina_device_name,
  .format_device = format_lina_device,
  .format_tx_trace = format_lina_tx_trace,
  .tx_function_n_errors = LINA_TX_N_ERROR,
  .tx_function_error_strings = lina_tx_func_error_strings,
  .rx_redirect_to_node = lina_set_interface_next_node,
  .clear_counters = lina_clear_hw_interface_counters,
  .admin_up_down_function = lina_interface_admin_up_down,
  .subif_add_del_function = lina_subif_add_del_function,
  .rx_mode_change_function = lina_interface_rx_mode_change,
};
