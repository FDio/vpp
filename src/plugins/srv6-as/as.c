...
unsigned char params_str[] = "nh <next-hop> oif <iface-out> iif <iface-in> "
			     "src <src-addr> next <sid> [next <sid> ...]";
...
srv6_as_localsid_creation_fn (ip6_sr_localsid_t *localsid)
{
  ...
  if (ls_mem->inner_type == AS_TYPE_L2)
	...
	"device-input", "srv6-as2-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
      if (ret != 0)
	{
	  free_ls_mem (ls_mem);
  ...
  else if (ls_mem->inner_type == AS_TYPE_IP4)
    {
      ...
      int ret = vnet_feature_enable_disable ("ip4-unicast", "srv6-as4-rewrite",
  ...
  else if (ls_mem->inner_type == AS_TYPE_IP6)
    {
      ...
      int ret = vnet_feature_enable_disable ("ip6-unicast", "srv6-as6-rewrite",
					     ls_mem->sw_if_index_in, 1, 0, 0);
      if (ret != 0)
	{
	  adj_unlock (ls_mem->nh_adj);
      ...
      /* Prepare rewrite string */
      ls_mem->rewrite =
	prepare_rewrite (ls_mem->src_addr, ls_mem->sid_list, IP_PROTOCOL_IPV6);
...
#define PARAM_AS_SRC (1 << 3)
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
			 ...
			 &sw_if_index_in))
	{
      ...
      else if (!(params & PARAM_AS_SRC) &&
	       unformat (input, "src %U", unformat_ip6_address, &src_addr))
	{
	  params |= PARAM_AS_SRC;
	}
...
srv6_as_init (vlib_main_t *vm)
{
  ...
  rv = sr_localsid_register_function (
    ...
    &sm->srv6_as_dpo_type, format_srv6_as_localsid, unformat_srv6_as_localsid,
    srv6_as_localsid_creation_fn, srv6_as_localsid_removal_fn);
  if (rv < 0)
