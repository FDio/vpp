...
{
  u8 *str = vnet_dev_arg_get_uint32 (arg);
  ...
  if (switch_header_type == ~0)
    {
      ...
      switch_header_type = ROC_PRIV_FLAGS_NONE;
    }
}
...
oct_port_init (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  foreach_vnet_dev_port_args (arg, port)
	...
	cp->switch_header_type = oct_switch_port_from_dev_arg_string (dev, arg);
    }
