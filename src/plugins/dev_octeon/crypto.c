...
oct_crypto_key_del_handler (vlib_main_t *vm, vnet_crypto_key_index_t key_index)
{
  ...
  vec_validate (oct_crypto_main.keys[VNET_CRYPTO_OP_TYPE_ENCRYPT], key_index);

  ckey = vec_elt_at_index (oct_crypto_main.keys[VNET_CRYPTO_OP_TYPE_ENCRYPT],
			   key_index);
  if (ckey->sess)
    {
      /*
  ...
  ckey = vec_elt_at_index (oct_crypto_main.keys[VNET_CRYPTO_OP_TYPE_DECRYPT],
			   key_index);
  if (ckey->sess)
    {
      /*
...
oct_crypto_key_add_handler (vlib_main_t *vm, vnet_crypto_key_index_t key_index)
{
  ...
  vec_validate (oct_crypto_main.keys[VNET_CRYPTO_OP_TYPE_DECRYPT], key_index);
  ckey = vec_elt_at_index (oct_crypto_main.keys[VNET_CRYPTO_OP_TYPE_DECRYPT],
			   key_index);
  if (ckey->sess == NULL)
    {
			  ...
			  struct cpt_inst_s *inst, u8 is_decrypt)
{
  ...
  if (PREDICT_FALSE (ret))
    {
      clib_warning ("Cryptodev: sg prep failed");
      return -1;
    }
			    ...
			    const u8 is_aead, u8 aad_len, const u8 type)
{
  ...
  pend_q = &oct_crypto_main.pend_q[vlib_get_thread_index ()];
  ...
  for (i = 0; i < frame->n_elts; i++)
    {
      ...
      key = vec_elt_at_index (oct_crypto_main.keys[type], elts->key_index);
...
oct_conf_sw_queue (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  oct_crypto_main.pend_q = oct_plt_init_param.oct_plt_zmalloc (
    tm->n_vlib_mains * sizeof (oct_crypto_pending_queue_t),
    CLIB_CACHE_LINE_BYTES);
  if (oct_crypto_main.pend_q == NULL)
    {
