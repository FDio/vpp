			   ...
			   vlib_buffer_t *b, u16 adj_len)
{
  ...
  if (is_aead)
  ...
  else
    {
      ...
      if (cpt_op == VNET_CRYPTO_OP_TYPE_ENCRYPT)
	fc_params.iv_buf = payload + elts->integ_length_adj;
      else
	fc_params.iv_buf = elts->iv;
			    ...
			    const u8 is_aead, u8 aad_len, const u8 type)
{
  ...
  /* CBC packets having spi + seq num + IV */
  u8 spi_sq_iv_bytes = 4 + 4 + 16;
  ...
  for (i = 0; i < frame->n_elts; i++)
      ...
      if (is_aead)
	{
	  ...
	  oct_crypto_fill_fc_params (
	    ...
	    elts->crypto_total_length /* enc_len */, aad_iv /* enc_offset */,
	    0 /* auth_len */, 0 /* auth_off */, buffer, adj_len);
	}
      else
	{
	  ...
	  if (type == VNET_CRYPTO_OP_TYPE_ENCRYPT)
	    crypto_total_length -= sess->iv_length;

	  oct_crypto_fill_fc_params (
	    sess, inst + i, is_aead, aad_len, (u8 *) dptr_start_ptr, elts,
	    (oct_crypto_scatter_gather_t *) (infl_req->sg_data) + i,
	    crypto_total_length /* enc_len */,
	    spi_sq_iv_bytes /* enc_offset */, enc_auth_len /* auth_len */,
	    0 /* auth_off */, buffer, adj_len);
	}
