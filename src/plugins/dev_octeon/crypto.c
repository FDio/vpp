...
static void
oct_crypto_key_del_handler (vlib_main_t *vm, vnet_dev_t *dev,
			    vnet_crypto_key_index_t key_index)
{
  ...
  vec_validate (od->keys[VNET_CRYPTO_OP_TYPE_ENCRYPT], key_index);
  ckey = vec_elt_at_index (od->keys[VNET_CRYPTO_OP_TYPE_ENCRYPT], key_index);
  if (ckey->sess)
    {
  ...
  ckey = vec_elt_at_index (od->keys[VNET_CRYPTO_OP_TYPE_DECRYPT], key_index);
  if (ckey->sess)
    {
      oct_plt_init_param.oct_plt_free (ckey->sess);
      ckey->sess = NULL;
...
static void
oct_crypto_key_add_handler (vlib_main_t *vm, vnet_dev_t *dev,
			    vnet_crypto_key_index_t key_index)
{
  oct_crypto_key_t *ckey;
  oct_device_t *od = vnet_dev_get_data (dev);
  ...
  vec_validate (od->keys[VNET_CRYPTO_OP_TYPE_DECRYPT], key_index);
  ckey = vec_elt_at_index (od->keys[VNET_CRYPTO_OP_TYPE_DECRYPT], key_index);
  ckey->sess = NULL;
}

static void
oct_crypto_key_handler (vlib_main_t *vm, vnet_crypto_key_handler_args_t *a)
{
  vnet_dev_t *dev = a->private_data;
  if (a->key_op == VNET_CRYPTO_KEY_OP_DEL)
    {
      oct_crypto_key_del_handler (vm, dev, a->key_index);
      return;
    }
  oct_crypto_key_add_handler (vm, dev, a->key_index);
}

static_always_inline oct_crypto_sess_t *
oct_crypto_session_alloc (vlib_main_t *vm, vnet_dev_t *dev)
{
  extern oct_plt_init_param_t oct_plt_init_param;
  ...
  if (addr == NULL)
    {
      log_err (dev, "Failed to allocate crypto session memory");
      return NULL;
    }
...
void
oct_crypto_burst_submit (vnet_dev_t *dev, struct cpt_inst_s *inst, u32 n_left)
{
  ...
  lmt_base = od->lmtline.lmt_base;
  io_addr = od->lmtline.io_addr;
...
oct_crypto_sg2_inst_prep (vnet_dev_t *dev, struct roc_se_fc_params *params,
  ...
  *offset_vaddr = offset_ctrl;
  ...
  if (decrypt && (req_flags & ROC_SE_VALID_MAC_BUF))
      ...
      if (size)
	{
	  ...
	  if (PREDICT_FALSE (size))
	    {
	      log_err (dev, "Insufficient buffer space, size %d needed", size);
	      return -1;
	    }
	}
    }
  ...
  if (PREDICT_FALSE ((scatter_sz >> 4) || (gather_sz >> 4)))
    {
      log_err (dev, "Exceeds max supported components. Reduce segments");
      ret = -1;
    }
...
static_always_inline int
oct_crypto_cpt_hmac_prep (vnet_dev_t *dev, u32 flags, u64 d_offs, u64 d_lens,
			  ...
			  struct cpt_inst_s *inst, u8 is_decrypt)
{
  ...
  if (PREDICT_FALSE ((encr_offset >> 16) || (iv_offset >> 8) ||
		     (auth_offset >> 8)))
    {
      log_err (dev, "Offset not supported");
      log_err (dev, "enc_offset: %d, iv_offset : %d, auth_offset: %d",
  ...
  ret = oct_crypto_sg2_inst_prep (dev, fc_params, inst, offset_ctrl, src,
				  iv_len, 0, 0, inputlen, outputlen,
				  passthrough_len, flags, 0, is_decrypt);

  if (PREDICT_FALSE (ret))
    {
      log_err (dev, "sg prep failed");
      return -1;
    }
...
static_always_inline void
oct_crypto_fill_fc_params (vnet_dev_t *dev, oct_crypto_sess_t *sess,
			   ...
			   vlib_buffer_t *b, u16 adj_len)
{
  ...
  oct_crypto_cpt_hmac_prep (dev, flags, d_offs, d_lens, &fc_params, inst,
			    cpt_op);
}

static_always_inline u64
oct_cpt_inst_w7_get (vnet_dev_t *dev, oct_crypto_sess_t *sess)
{
  oct_device_t *od = vnet_dev_get_data (dev);
  ...
  inst_w7.s.egrp = od->cpt->eng_grp[CPT_ENG_TYPE_IE];

  return inst_w7.u64;
}

static_always_inline void
oct_map_keyindex_to_session (vnet_dev_t *dev, oct_crypto_sess_t *sess,
			     u32 key_index, u8 type)
{
  oct_device_t *od = vnet_dev_get_data (dev);
  oct_crypto_key_t *ckey;

  ckey = vec_elt_at_index (od->keys[type], key_index);

  ckey->sess = sess;
}

static_always_inline i32
oct_crypto_link_session_update (vlib_main_t *vm, vnet_dev_t *dev,
				oct_crypto_sess_t *sess, u32 key_index,
				u8 type)
{
  ...
  switch (key->async_alg)
    ...
    case VNET_CRYPTO_ALG_AES_256_CBC_SHA512_TAG32:
      ...
      break;
    default:
      log_err (dev, "Crypto: Undefined link algo %u specified. Key index %u",
  ...
  if (rv)
    {
      log_err (dev, "Error in setting cipher key for enc type %u", enc_type);
      return -1;
    }
  ...
  if (rv)
    {
      log_err (dev, "Error in setting auth key for auth type %u", auth_type);
      return -1;
    }

  oct_map_keyindex_to_session (dev, sess, key_index, type);
  /*
   * Map session to crypto key index also. This entry can be referred
   * while deleting key
   */
  oct_map_keyindex_to_session (dev, sess, key->index_crypto, type);

  return 0;
}

static_always_inline i32
oct_crypto_aead_session_update (vlib_main_t *vm, vnet_dev_t *dev,
				oct_crypto_sess_t *sess, u32 key_index,
				u8 type)
{
  ...
  switch (key->async_alg)
    ...
    case VNET_CRYPTO_ALG_AES_256_GCM:
      ...
      break;
    default:
      log_err (dev, "Crypto: Undefined cipher algo %u specified. Key index %u",
	       key->async_alg, key_index);
      return -1;
    }
...
i32
oct_crypto_session_create (vlib_main_t *vm, vnet_dev_t *dev,
			   vnet_crypto_key_index_t key_index, int op_type)
{
  ...
  session = oct_crypto_session_alloc (vm, dev);
  if (session == NULL)
    return -1;

  if (key->type == VNET_CRYPTO_KEY_TYPE_LINK)
    rv = oct_crypto_link_session_update (vm, dev, session, key_index, op_type);
  else
    rv = oct_crypto_aead_session_update (vm, dev, session, key_index, op_type);
  ...
  session->cpt_inst_w7 = oct_cpt_inst_w7_get (dev, session);
...
static_always_inline int
oct_crypto_enq (vlib_main_t *vm, vnet_crypto_async_frame_t *frame,
		const u8 is_aead, u8 aad_len, const u8 type)
{
  vnet_dev_t *dev = frame->user_data;
  oct_device_t *od = vnet_dev_get_data (dev);
  ...
  pend_q = od->pend_q + vlib_get_thread_index ();
  ...
  for (i = 0; i < frame->n_elts; i++)
    {
      ...
      key = vec_elt_at_index (od->keys[type], elts->key_index);

      if (!key->sess)
	{
	  if (oct_crypto_session_create (vm, dev, elts->key_index, type) == -1)
	    {
      ...
      if (is_aead)
	{
	  ...
	  oct_crypto_fill_fc_params (
	    dev, sess, inst + i, is_aead, aad_len, (u8 *) dptr_start_ptr, elts,
  ...
  oct_crypto_burst_submit (dev, inst, frame->n_elts);

  infl_req->elts = frame->n_elts;
  OCT_MOD_INC (pend_q->enq_tail, OCT_CRYPTO_DEFAULT_SW_ASYNC_FRAME_COUNT);
...
static int
oct_crypto_enq_linked_alg_enc (vlib_main_t *vm,
			       vnet_crypto_async_frame_t *frame)
{
  oct_crypto_enq (vm, frame, 0 /* is_aead */, 0 /* aad_len */,
		  VNET_CRYPTO_OP_TYPE_ENCRYPT);
  return 0;
}

static int
oct_crypto_enq_linked_alg_dec (vlib_main_t *vm,
			       vnet_crypto_async_frame_t *frame)
{
  oct_crypto_enq (vm, frame, 0 /* is_aead */, 0 /* aad_len */,
...
static int
oct_crypto_enq_aead_aad_enc (vlib_main_t *vm, vnet_crypto_async_frame_t *frame,
			     u8 aad_len)
{
  oct_crypto_enq (vm, frame, 1 /* is_aead */, aad_len,
		  VNET_CRYPTO_OP_TYPE_ENCRYPT);
...
static int
oct_crypto_enq_aead_aad_dec (vlib_main_t *vm, vnet_crypto_async_frame_t *frame,
			     u8 aad_len)
{
  oct_crypto_enq (vm, frame, 1 /* is_aead */, aad_len,
		  VNET_CRYPTO_OP_TYPE_DECRYPT);
...
static int
oct_crypto_enq_aead_aad_8_enc (vlib_main_t *vm,
			       vnet_crypto_async_frame_t *frame)
{
  return oct_crypto_enq_aead_aad_enc (vm, frame, 8);
}

static int
oct_crypto_enq_aead_aad_12_enc (vlib_main_t *vm,
				vnet_crypto_async_frame_t *frame)
{
  return oct_crypto_enq_aead_aad_enc (vm, frame, 12);
}

static int
oct_crypto_enq_aead_aad_8_dec (vlib_main_t *vm,
			       vnet_crypto_async_frame_t *frame)
{
  return oct_crypto_enq_aead_aad_dec (vm, frame, 8);
}

static int
oct_crypto_enq_aead_aad_12_dec (vlib_main_t *vm,
				vnet_crypto_async_frame_t *frame)
{
  return oct_crypto_enq_aead_aad_dec (vm, frame, 12);
}

static vnet_crypto_async_frame_t *
oct_crypto_frame_dequeue (vlib_main_t *vm, vnet_crypto_frame_deq_fn_args_t *a)
{
  vnet_dev_t *dev = a->private_data;
  oct_device_t *od = vnet_dev_get_data (dev);
  // FIXME struct cpt_inst_s inst[VNET_CRYPTO_FRAME_SIZE];
  ...
  OCT_MOD_INC (pend_q->deq_head, OCT_CRYPTO_DEFAULT_SW_ASYNC_FRAME_COUNT);
  pend_q->n_crypto_inflight--;
  ...
  *a->nb_elts_processed = frame->n_elts;
  *a->enqueue_thread_idx = frame->enqueue_thread_index;
...
#define _(n, k, t, a)                                                         \
  vnet_crypto_register_enqueue_handler (                                      \
    vm, &(vnet_crypto_register_enqueue_handler_args_t){                       \
	  ...
	  .enq_fn = oct_crypto_enq_aead_aad_##a##_enc,                        \
	  .user_data = dev });                                                \
  vnet_crypto_register_enqueue_handler (                                      \
    vm, &(vnet_crypto_register_enqueue_handler_args_t){                       \
	  .engine_index = engine_index,                                       \
	  .op_id = VNET_CRYPTO_OP_##n##_TAG##t##_AAD##a##_DEC,                \
	  .enq_fn = oct_crypto_enq_aead_aad_##a##_dec,                        \
	  .user_data = dev });
...
#define _(c, h, k, d)                                                         \
  vnet_crypto_register_enqueue_handler (                                      \
    vm, &(vnet_crypto_register_enqueue_handler_args_t){                       \
	  ...
	  .enq_fn = oct_crypto_enq_linked_alg_enc,                            \
	  .user_data = dev });                                                \
  vnet_crypto_register_enqueue_handler (                                      \
    vm, &(vnet_crypto_register_enqueue_handler_args_t){                       \
	  .engine_index = engine_index,                                       \
	  .op_id = VNET_CRYPTO_OP_##c##_##h##_TAG##d##_DEC,                   \
	  .enq_fn = oct_crypto_enq_linked_alg_dec,                            \
	  .user_data = dev });
