...
oct_parse_l2 (oct_flow_parse_state *pst)
{
  ...
  while (tpid == ETH_TYPE_DOT1AD || tpid == ETH_TYPE_DOT1Q)
    {
	...
	(ethernet_vlan_header_t *) &pst->generic.mask[pst->generic.off];
      ...
      clib_memcpy (&vlan_mask[vlan_cnt], vlan_hdr,
		   sizeof (ethernet_vlan_header_t));
...
oct_parse_l3 (oct_flow_parse_state *pst)
{
  ...
  if (pst->nxt_proto == ETH_TYPE_MPLS)
    {
  ...
  else if (pst->nxt_proto == ETH_TYPE_IPV4)
    {
  ...
  else if (pst->nxt_proto == ETH_TYPE_IPV6)
    {
	...
	(ip6_header_t *) &pst->generic.spec[pst->generic.off];
      ...
      while (nxt_hdr == IP_PROTO_HOPOPT || nxt_hdr == IP_PROTO_DSTOPT ||
	     nxt_hdr == IP_PROTO_ROUTE)
	{
	  if (pst->generic.off >= pst->generic.len)
	    return 0;
...
#define _(protocol_t, protocol_value, ltype)                                  \
  ...
  _ (esp_header_t, IP_PROTO_ESP, ESP)
  _ (udp_header_t, IP_PROTO_UDP, UDP)
  _ (tcp_header_t, IP_PROTO_TCP, TCP)
  _ (sctp_header_t, IP_PROTO_SCTP, SCTP)
  _ (icmp46_header_t, IP_PROTO_ICMP, ICMP)
  _ (icmp46_header_t, IP_PROTO_ICMPV6, ICMP)
  _ (igmp_header_t, IP_PROTO_IGMP, IGMP)
  _ (gre_header_t, IP_PROTO_GRE, GRE)
