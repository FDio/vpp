...
static int
oct_flow_validate_params (vnet_main_t *vnm, vnet_dev_port_t *port,
			  vnet_flow_t *flow, uword *private_data)
{
  u32 last_queue;
  u32 qid;
  ...
  if (OCT_FLOW_UNSUPPORTED_ACTIONS (flow))
    {
      clib_warning ("Unsupported flow action");
...
static int
oct_flow_add (vnet_main_t *vnm, vnet_dev_port_t *port, vnet_flow_t *flow,
	      uword *private_data)
{
...
static int
oct_flow_del (vnet_main_t *vnm, vnet_dev_port_t *port, vnet_flow_t *flow,
	      uword *private_data)
{
  oct_port_t *oct_port = vnet_dev_get_port_data (port);
  struct roc_npc *npc = &oct_port->npc;
...
int
oct_flow_ops_fn (vnet_main_t *vnm, vnet_flow_dev_op_t op, u32 dev_id,
		 u32 flow_index, uword *priv_data)
{
  vnet_dev_main_t *dm = &vnet_dev_main;
  vnet_dev_port_t **pp = pool_elt_at_index (dm->ports_by_dev_instance, dev_id);
  vnet_dev_port_t *port = pp[0];
  vnet_flow_t *flow = vnet_get_flow (flow_index);
  int rv = 0;

  rv = oct_flow_validate_params (vnm, port, flow, priv_data);
  if (rv < 0)
    {
      log_err (port->dev, "oct_flow_validate_params failed");
      return VNET_FLOW_ERROR_NOT_SUPPORTED;
    }

  if (op == VNET_FLOW_DEV_OP_ADD_FLOW)
    return oct_flow_add (vnm, port, flow, priv_data);

  if (op == VNET_FLOW_DEV_OP_DEL_FLOW)
    return oct_flow_del (vnm, port, flow, priv_data);
