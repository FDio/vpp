...
#define INTEL_CBDMA_LOG2_N_COMPLETIONS 4
  ...
  *ch->completion = 0;
...
#if 0
  log_debug ("chainaddr %p sts 0x%lx err 0x%x", regs->chainaddr, regs->chansts, regs->chanerr);
  log_debug ("chainaddr %p sts 0x%lx err 0x%x", regs->chainaddr, regs->chansts, regs->chanerr);
  log_debug ("completion %p", *ch->completion);
  ch->comp_descs[0].next = ch->comp_descs + 8;
  ch->comp_descs[8].next = ch->comp_descs + 9;
  //ch->comp_descs[8].comp_upd = 0;
  ch->comp_descs[9].next = ch->comp_descs + 10;
  //ch->comp_descs[9].comp_upd = 0;
  ch->comp_descs[10].next = ch->comp_descs + 1;
  //ch->comp_descs[10].comp_upd = 0;
  ch->comp_descs[10].op_type = 1;
  ch->comp_descs[10].nt = 0;
  ch->comp_descs[10].size = 64;
  ch->comp_descs[10].src = (void *) 0xdeadbeef;
  ch->comp_descs[10].dst = ch->comp_descs + 4;

  __atomic_store_n (&regs->dmacount, 1, __ATOMIC_RELEASE);
  for (f64 t = vlib_time_now (vm) + 0.001; vlib_time_now (vm) < t;)
    CLIB_PAUSE ();

  log_debug ("sts %lx comp %p err 0x%x", regs->chansts, *ch->completion, regs->chanerr);

  fformat (stderr, "\n%U\n", format_cbdma_descs, ch->comp_descs, 12);
  ch->comp_descs[0].next = ch->comp_descs + 1;
  ch->comp_descs[1].next = ch->comp_descs + 2;

  __atomic_store_n (&regs->dmacount, 2, __ATOMIC_RELEASE);
  for (f64 t = vlib_time_now (vm) + 0.001; vlib_time_now (vm) < t;)
    CLIB_PAUSE ();
  ...
  return 0;
  ch->comp_descs[1].next = ch->comp_descs + 7;
  __atomic_store_n (&regs->dmacount, 9, __ATOMIC_RELEASE);
  for (f64 t = vlib_time_now (vm) + 0.001; vlib_time_now (vm) < t;)
    CLIB_PAUSE ();

  log_debug ("sts %lx comp %p", regs->chansts, *ch->completion);
#endif
