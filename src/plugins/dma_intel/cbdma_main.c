...
intel_cbdma_assign_channels (vlib_main_t *vm)
{
  ...
  if (vec_len (chv) >= vlib_get_n_threads ())
    {
      n_threads = 1;
      n_dmas = vec_len (chv) / vlib_get_n_threads ();
      for (int i = 0; i < vlib_get_n_threads (); i++)
	{
	  vlib_main_t *tvm = vlib_get_main_by_index (i);
	  for (int j = 0; j < n_dmas; j++)
	    {
	      ch = *vec_elt_at_index (chv, i * n_dmas + j);
	      vec_add1 (dm->cbdma_threads[i].ch, ch);
	      ch->n_threads = n_threads;
	      cbdma_log_debug (
		"Assigning channel %u/%U to thread %u (numa %u)", ch->numa,
		format_vlib_pci_addr, &ch->addr, i, tvm->numa_node);
	    }
	}
    }
  else
