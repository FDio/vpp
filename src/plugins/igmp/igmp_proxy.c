...
igmp_proxy_device_mfib_path_add_del (igmp_group_t *group, u8 add)
{
  ...
  if (add)
    mfib_table_entry_path_update (mfib_index, &mpfx_group_addr, MFIB_SOURCE_IGMP,
				  &via_itf_path, MFIB_ITF_FLAG_FORWARD);
  else
    mfib_table_entry_path_remove (mfib_index, &mpfx_group_addr, MFIB_SOURCE_IGMP,
				  &via_itf_path);
}
...
void
igmp_proxy_device_block_src (igmp_config_t *config, igmp_group_t *group, igmp_src_t *src)
{
  igmp_proxy_device_t *proxy_device;
...
always_inline void
igmp_proxy_device_merge_src (igmp_group_t *proxy_group, igmp_src_t *src, ip46_address_t **srcaddrs,
			 u8 block)
{
  igmp_src_t *proxy_src;
  u32 d_config;
  ...
  if (proxy_src == NULL)
	...
	return;
      ...
      hash_set_mem (proxy_group->igmp_src_by_key[proxy_group->router_filter_mode],
...
igmp_proxy_device_merge_group (igmp_proxy_device_t *proxy_device, igmp_group_t *group,
