...
static clib_error_t *
igmp_configure_command_fn (vlib_main_t * vm, unformat_input_t * input,
			   vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  clib_error_t *error = NULL;
  ...
  if (!unformat_user (input, unformat_line_input, line_input))
    {
      error = clib_error_return (0, "'help igmp configure' or 'igmp configure ?' for help");
      return error;
    }
  ...
  rv = igmp_configure (vm, enable, sw_if_index, saddr, gaddr);
  if (rv == -1)
    {
      if (enable)
	error =
		      ...
		      vlib_cli_command_t * cmd)
{
  ...
  pool_foreach (config, im->configs, (
	...
	pool_foreach (sg, config->sg, (
	  {
	    vlib_cli_output (vm, "\t(S,G): %U:%U", format_ip46_address,
