...
igmp_group_prefix_get_type (const ip46_address_t *gaddr)
{
  ...
  vec_foreach (igp, igmp_group_prefixs)
    {
      if (ip4_destination_matches_route (&ip4_main, &gaddr->ip4,
...
static clib_error_t *
igmp_ssm_range_show (vlib_main_t *vm, unformat_input_t *input,
		     vlib_cli_command_t *cmd)
{
  ...
  vec_foreach (igp, igmp_group_prefixs)
    {
      vlib_cli_output (vm, "%U => %U", format_fib_prefix, &igp->igp_prefix,
