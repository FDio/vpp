...
_(IGMP_CONFIGURE, igmp_configure)                        \
_(IGMP_DUMP, igmp_dump)					 \
_(IGMP_CLEAR_INTERFACE, igmp_clear_interface)		 \
_(WANT_IGMP_EVENTS, want_igmp_events)			 \

static void
vl_api_igmp_configure_t_handler (vl_api_igmp_configure_t * mp)
{
  vlib_main_t *vm = vlib_get_main ();
  vnet_main_t *vnm = vnet_get_main ();
  igmp_main_t *im = &igmp_main;
  vl_api_igmp_configure_reply_t *rmp;
  int rv = 0;
  ...
  rv = igmp_configure (vm, mp->enable, ntohl (mp->sw_if_index), saddr, gaddr);
  ...
  rmp->_vl_msg_id = htons ((VL_API_IGMP_CONFIGURE_REPLY)+im->msg_id_base);
...
static void
send_igmp_details (unix_shared_memory_queue_t * q, igmp_main_t * im, igmp_config_t * config,
		   igmp_sg_t * sg, u32 context)
{
...
vl_api_igmp_clear_interface_t_handler (vl_api_igmp_clear_interface_t * mp)
{
  ...
  rmp = vl_msg_api_alloc (sizeof (*rmp));
  rmp->_vl_msg_id = htons ((VL_API_IGMP_CLEAR_INTERFACE_REPLY)+im->msg_id_base);
  rmp->context = mp->context;
  rmp->retval = htonl (rv);
...
vl_api_want_igmp_events_t_handler (vl_api_want_igmp_events_t * mp)
{
  ...
  if (api_client)
      ...
      if (mp->enable)
	{
      ...
      hash_unset_mem (im->igmp_api_client_by_client_index, &api_client->client_index);
...
void
send_igmp_event (unix_shared_memory_queue_t * q, u32 context, igmp_main_t * im,
		 igmp_config_t * config, igmp_sg_t * sg)
{
  vl_api_igmp_event_details_t *mp = vl_msg_api_alloc (sizeof (*mp));
  memset (mp, 0, sizeof (*mp));

  mp->_vl_msg_id = ntohs ((VL_API_IGMP_EVENT_DETAILS)+im->msg_id_base);
  mp->context = context;
  mp->sw_if_index = htonl (config->sw_if_index);
  clib_memcpy (&mp->saddr, &sg->saddr.ip4, sizeof (ip4_address_t));
  clib_memcpy (&mp->gaddr, &sg->gaddr.ip4, sizeof (ip4_address_t));
  mp->is_join = (sg->group_type == IGMP_MEMBERSHIP_GROUP_mode_is_filter_include) ? 1 : 0;
