...
do {                                                            \
    ...
    rmp->_vl_msg_id = htons ((t)+im->msg_id_base);              \
...
vl_api_igmp_configure_t_handler (vl_api_igmp_configure_t * mp)
{
  ...
  if (!vnet_sw_interface_is_api_valid (vnm, mp->sw_if_index))
    {
      rv = VNET_API_ERROR_INVALID_SW_IF_INDEX;
      goto done;
    }

  if ((vnet_sw_interface_get_flags (vnm, mp->sw_if_index) &&
  ...
  rv = igmp_configure (vm, mp->enable, mp->sw_if_index, saddr, gaddr);

done:
  REPLY_MACRO (VL_API_IGMP_CONFIGURE_REPLY);
}
