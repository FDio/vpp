...
class Igmp:
    ...
    def get_vapi(cls):
        """ Get vapi. """
        if not isinstance(cls.vapi, VppPapiProvider):
            raise ValueError("Igmp.vapi is not valid papi provider")
        return cls.vapi
    ...
    @classmethod
    def enable_on_interface(cls, sw_if_index, mode=IGMP_MODE.HOST):
    ...
    @classmethod
    def disable_on_interface(cls, sw_if_index, mode=IGMP_MODE.HOST):
