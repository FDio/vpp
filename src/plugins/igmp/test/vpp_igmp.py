...
from vpp_event import VppEvent
import socket
import os
...
class VppHostState(VppObject):
    ...
    def encode(self, no_srcs=False):
        ret = {"filter": self.filter, "sw_if_index": self.sw_if_index,
               "n_srcs": 0, "saddrs": [], "gaddr": self.sg.gaddr_p}

        if no_srcs:
            return ret

        ret["n_srcs"] = len(self.sg.saddrs)
        ret["saddrs"] = self.sg.saddrs_encoded

        return ret

    def add_vpp_config(self):
        self._test.vapi.igmp_listen(group=self.encode())

    def remove_vpp_config(self):
        self._test.vapi.igmp_listen(group=self.encode(no_srcs=True))
