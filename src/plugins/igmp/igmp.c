...
/** \brief igmp get IPv4 address
    @param sw_if_index - interface index

    Get IPv4 address of specified interface.
*/
always_inline ip4_address_t *
igmp_get_ip4_address (u32 sw_if_index)
{
  ...
  if (PREDICT_TRUE (if_add_index != ~0))
	...
	pool_elt_at_index (lm->if_address_pool, if_add_index);
      return ip_interface_address_get_address (lm, if_add);
    }
  else
    return NULL;
}
  ...
  if (group)
    {
      ...
      if (config->flags & IGMP_CONFIG_FLAG_PROXY_ENABLED)
	{
	  ip4_addr = igmp_get_ip4_address (config->sw_if_index);
	  if (ip4_addr != NULL)
	    clib_memcpy (&igmp_group->src_addresses[i++], ip4_addr,
	  ...
	  if (config->flags & IGMP_CONFIG_FLAG_PROXY_ENABLED)
	    {
	      ip4_addr = igmp_get_ip4_address (config->sw_if_index);
	      if (ip4_addr != NULL)
		clib_memcpy (&igmp_group->src_addresses[i++], ip4_addr, sizeof (ip4_address_t));
	    }
	  else
	    {
	      pool_foreach (src, group->srcs, (
		 ...
		 igmp_group_t * group, u8 is_report)
{
  ...
  ip4_addr = igmp_get_ip4_address (config->sw_if_index);
  if (ip4_addr != NULL)
    clib_memcpy (&ip4->src_address, ip4_addr, sizeof (ip4_address_t));
...
int
igmp_proxy (vlib_main_t * vm, u8 enable, u32 sw_if_index)
{
  igmp_main_t *im = &igmp_main;
  int rv = 0;
  igmp_config_t *config;
