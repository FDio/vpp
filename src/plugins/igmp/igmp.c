...
{
  igmp_main_t *im = &igmp_main;
  igmp_per_int_timer_t *timer;

  pool_get (im->per_int_timers, timer);
  memset (timer, 0, sizeof (igmp_per_int_timer_t));
  timer->func = func;
  timer->exp_time = time;
...
{
  igmp_main_t *im = &igmp_main;
  igmp_per_sg_timer_t *timer;

  pool_get (im->per_sg_timers, timer);
  memset (timer, 0, sizeof (igmp_per_sg_timer_t));
  timer->func = func;
  timer->exp_time = time;
  timer->sw_if_index = sw_if_index;
  timer->key = key;

  igmp_sort_timers (im->per_sg_timers, vec_len (im->per_sg_timers),
		 ...
		 void *timer_ptr)
{
  ...
  igmp_per_int_timer_t *timer = (igmp_per_int_timer_t *) timer_ptr;
	     ...
	     void *timer_ptr)
{
  igmp_config_t *config;
  igmp_sg_t *sg;
  igmp_per_sg_timer_t *timer = (igmp_per_sg_timer_t *) timer_ptr;

  u32 sw_if_index = timer->sw_if_index;
  f64 exp_time = timer->exp_time;
  igmp_sg_key_t *key = timer->key;
  ...
  /* soirce timer expired, remove (S,G) */
  igmp_configure_internal (vm, 0, sw_if_index, key->saddr, key->gaddr, 0);

done:
  pool_put (im->per_sg_timers, timer);
}
		    ...
		    vlib_frame_t * f)
{
  ...
  while (1)
      ...
      else if (event_type == IGMP_PROCESS_EVENT_REPORT_IMMED)
	{
	  ...
	  igmp_send_msg (vm, rt, im, config, 1);

	  /* send state-change report [Robustness Variable] - 1 times */
	  for (i = 1; i < (config->robustness_var - 1); i++)
	    igmp_send_msg (vm, rt, im, config, /* is_report */ 1);
      ...
      /* timer expired */
      timer->func (vm, rt, im, (void *) timer);
