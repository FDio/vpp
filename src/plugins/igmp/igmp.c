			...
			igmp_group_t * group)
{
  ...
  igmp->groups[0].type = group->type;
  igmp->groups[0].n_aux_u32s = 0;
  clib_memcpy (&igmp->groups[0].dst_address, &group->addr.ip4,
  ...
  pool_foreach (src, group->srcs, (
    {
      ...
      clib_memcpy (&igmp->groups[0].src_addresses[0], &src->addr.ip4,
	       sizeof (ip4_address_t));
    }));
  igmp->groups[0].n_src_addresses =
		 ...
		 igmp_group_t * group, u8 is_report)
{
  ...
  if (PREDICT_FALSE (group != NULL))
    clib_memcpy (&ip4->dst_address, &group->addr.ip4, sizeof (ip4_address_t));
  else
    {
      if (is_report)
	clib_memcpy (&ip4->dst_address,
		     &im->membership_report_address, sizeof (ip4_address_t));
      else
	clib_memcpy (&ip4->dst_address,
		     &im->general_query_address, sizeof (ip4_address_t));
    }
...
#undef igmp_report_type
  ...
  /* 224.0.0.1 */
  im->general_query_address.as_u32 = 0xE0000001;
  /* 224.0.0.22 */
  im->membership_report_address.as_u32 = 0xE0000016;
  ...
  addr0.ip4.as_u32 = im->general_query_address.as_u32;
  ...
  addr1.ip4.as_u32 = im->membership_report_address.as_u32;
