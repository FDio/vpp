  ...
  if (!group)
    {
       ...
       */
      // igmp_pkt_send_state_change_group_report_v3 (config->sw_if_index,
      //                                      group);
    }
  /*   } */
  /* else */
      ...
      if (IGMP_FILTER_MODE_INCLUDE == mode)
        {
          ...
          if (vec_len(added)) {
            igmp_pkt_report_v3_add_report(&report,
                                          group->key,
                                          added,
                                          IGMP_MEMBERSHIP_GROUP_allow_new_sources);
          }
...
static clib_error_t * igmp_init (vlib_main_t * vm)
{
  ...
  error = mfib_register_igmp_node(FIB_PROTOCOL_IP4, "igmp-input");
