...
/* TODO: move to src/vnet/ip/igmp_format.c */
u8 *
...
format_igmp_input_trace (u8 * s, va_list * va)
{
  ...
  s = format (s, "%U", format_igmp_header, t->packet_data, sizeof (t->packet_data));
  return s;
}
...
format_igmp_parse_report_trace (u8 * s, va_list * va)
{
  ...
  s = format (s, "sw_if_index %u next-index %u", t->sw_if_index, t->next_index);
  s = format (s, "%U", format_igmp_report_v3, t->packet_data, sizeof (t->packet_data));
  return s;
}
...
format_igmp_parse_query_trace (u8 * s, va_list * va)
{
  ...
  s = format (s, "%U", format_igmp_query_v3, t->packet_data, sizeof (t->packet_data));
  return s;
}
	    ...
	    vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  sum = ip_incremental_checksum (0, igmp, ip->length - ip4_header_bytes (ip));
	  csum = ~ip_csum_fold (sum);
	  if (igmp->checksum != csum)
	    {
	...
	next_buffer:
	  ...
	  if (PREDICT_FALSE (n_trace > 0))
	      ...
	      if (PREDICT_TRUE (b != 0))
		{
		  ...
		  vlib_trace_buffer (vm, node, next, b, 0);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (igmp->dst.as_u32 == 0)
	      ...
	      else
		{
		  ...
		  igmp_create_timer (IGMP_PER_INT_TIMER_TYPE_REPORT_DELAY,
				     next_resp_time, sw_if_index, NULL);
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (config)
		...
		{
		  DBG ("Interface %u has (S,G)s configured by CLI/API", sw_if_index);
...
/* WIP
 *
 * find existing query response timer and remove it from the queue
 */
	  /* *INDENT-OFF* */
	  pool_foreach (timer, im->per_int_timers, (
	    {
	      if (timer->type == IGMP_PER_INT_TIMER_TYPE_QUERY_RESP &&
			timer->sw_if_index == sw_if_index)
		{
		  pool_put (im->per_int_timers, timer);
		  igmp_sort_timers (im->per_int_timers, vec_len (im->per_int_timers),
...
/**/
	  ...
	  for (i = 0; i < clib_net_to_host_u16 (igmp->n_groups); i++)
	      ...
	      if (group->type == IGMP_MEMBERSHIP_GROUP_mode_is_filter_include)
		{
	      ...
	      else if (group->type == IGMP_MEMBERSHIP_GROUP_mode_is_filter_exclude)
		{
		  for (j = 0;
		       j < clib_net_to_host_u16 (group->n_src_addresses); j++)
		    {
	      ...
	      else if (group->type == IGMP_MEMBERSHIP_GROUP_change_to_filter_include)
		{
		  for (j = 0;
		       j < clib_net_to_host_u16 (group->n_src_addresses); j++)
		    {
		      ...
		      igmp_configure_internal (vm, 1, sw_if_index, saddr,
					       gaddr, 0);
		      src++;
		    }
		}
	      else if (group->type == IGMP_MEMBERSHIP_GROUP_change_to_filter_exclude)
		{
		  for (j = 0;
		       j < clib_net_to_host_u16 (group->n_src_addresses); j++)
		    {
		      ...
		      saddr.ip4 = *src;
		      gaddr.ip4 = group->dst_address;
		      igmp_configure_internal (vm, 0, sw_if_index, saddr,
					       gaddr, 0);
		      src++;
		    }
		}
	      ...
	      else if (group->type == IGMP_MEMBERSHIP_GROUP_block_old_sources)
		{
	      ...
	      len += sizeof (igmp_membership_group_v3_t) + (sizeof (ip4_address_t) * j);
	    }
	next_frame:
	  VLIB_BUFFER_TRACE_TRAJECTORY_INIT (b);
