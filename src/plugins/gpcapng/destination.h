...
typedef struct gpcapng_dest_t {
  char *name;
  void *arg;
  void *(*init) (gpcapng_dest_t *output, u16 worker_index, u16 destination_index);
...
always_inline void worker_context_init_common(void *ctx, pcapng_destination_type_t type) {
   gpcapng_worker_context_common_t *cc = ctx;
   cc->context_signature = CONTEXT_SIGNATURE;
   cc->context_type = type;
}

always_inline void worker_context_verify(void *ctx) {
   gpcapng_worker_context_common_t *cc = ctx;
...
/* Worker Destination Index (WDI): a combination of the worker index and destination index */

always_inline worker_dest_index_t make_wdi(u16 worker_index, u16 destination_index) {
   return (((u32)worker_index << 16) + ((u32) destination_index));
}

always_inline u16 wdi_to_worker_index(worker_dest_index_t wdi) {
   return (wdi >> 16);
}

always_inline u16 wdi_to_destination_index(worker_dest_index_t wdi) {
   return (wdi &0xffff);
}
