...
static int
file_write_pcapng_idb (gpcapng_dest_t *out, void *context, u32 if_index,
		       const char *if_name)
{
  u8 *orig_block;
  u8 *block;
  ...
  result =
    out->chunk_write (context, block, vec_len (block) - (block - orig_block));
...
static int
file_write_pcapng_epb (gpcapng_dest_t *out, void *context, u32 if_index,
		       ...
		       u32 packet_len)
{
  ...
  get_pcapng_epb (&block, if_index, timestamp, orig_len, packet_data,
...
void
set_write_pcapng (gpcapng_dest_t *output)
{
