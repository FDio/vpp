                       ...
                       u32 orig_len, void *packet_data, u32 packet_len)
{
  ...
  sprintf(json_comment, "{\"time: %ld}", (long)time(0));
                       ...
                       u32 orig_len, void *packet_data, u32 packet_len)
{
  ...
  int total_len = get_pcapng_epb(&block, if_index, timestamp, orig_len, packet_data, packet_len);

  int result = out->chunk_write(context, block, total_len);
