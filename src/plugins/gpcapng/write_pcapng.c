...
{
  u8 *vec_shb = get_pcapng_shb_vec();
  int rv = out->chunk_write(context, vec_shb, vec_len(vec_shb));
  vec_free(vec_shb);
  return rv;
}
...
file_write_pcapng_idb (gpcapng_dest_t *out, void *context, u32 if_index, const char *if_name)
{
  ...
  orig_block = vec_new(u8, 0);

  block = get_pcapng_idb(&orig_block, if_index, if_name);

  result = out->chunk_write(context, block, vec_len(block) - (block - orig_block));
...
int
get_pcapng_epb(u8 **buffer, u32 if_index, u64 timestamp,
                       u32 orig_len, void *packet_data, u32 packet_len, void *comment, u32 comment_len)
{
                       ...
                       u32 orig_len, void *packet_data, u32 packet_len)
{
  ...
  char json_comment[100]; // = "{\"app_id\": \"dummy_json\", \"flow_id\": 12345, \"custom_field\": \"value\"}";
  if (common_ctx->packet_counter > common_ctx->last_sent_packet_counter + 1) {
      sprintf(json_comment, "{\"missed packets\": %ld}", common_ctx->packet_counter - common_ctx->last_sent_packet_counter);
  } else {
      sprintf(json_comment, "");
  }

  get_pcapng_epb(&block, if_index, timestamp, orig_len, packet_data, packet_len, json_comment, strlen(json_comment));

  int result = out->chunk_write(context, block, vec_len(block));
                       ...
                       u32 orig_len, void *packet_data, u32 packet_len)
{
  ...
  return get_pcapng_epb(vec_out, if_index, timestamp, orig_len, packet_data, packet_len, json_comment, strlen(json_comment));
}
