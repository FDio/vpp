...
format_pcapng_capture_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "PCAPNG: sw_if_index %d filtering elapsed %ld, dest_index %d",
	      t->sw_if_index, t->filtering_elapsed, t->dest_index);
  return s;
}
...
VNET_FEATURE_INIT (gpcapng_feature_out, static) = {
  ...
  .node_name = "gen-pcapng-capture-out",
  .runs_before = VNET_FEATURES ("interface-output-arc-end"),
};

VNET_FEATURE_INIT (gpcapng_feature_in, static) = {
  .arc_name = "device-input",
  .node_name = "gen-pcapng-capture-in",
  .runs_before = VNET_FEATURES ("interface-output-arc-end"),
};
