...
static void print_http_context(vlib_main_t *vm, void *ctx) {
...
pcapng_http_connect_rpc(void *rpc_args)
{
    ...
    rv = vnet_connect(a);
    if (rv) {
        clib_warning("HTTP PCAPng connect returned: %U", format_session_error, rv);
    }
...
process_http_gpcapng_retries(u16 worker_index)
{
    ...
    while (vec_len(gpm->worker_retry_queue[worker_index]) > 0 && processed < 5) {
        ...
        if (wdi == WDI_POISON_VALUE) {
            clib_warning("Skipping poisoned retry entry (destination was deleted)");
        ...
        for (worker_index = 0; worker_index <= vlib_num_workers(); worker_index++) {
            if (worker_index == 0) {
                ...
                session_send_rpc_evt_to_thread_force(
                    vlib_get_thread_index() + worker_index,
                    http_retry_rpc,
                    0);
            }
        }
...
http_pcapng_init (gpcapng_dest_t *output, u16 worker_index, u16 destination_index)
{
    ...
    u64 options[18];
    int rv;
    u8 *worker_uri = format(0, "%s-%d.pcapng%c", output->arg, worker_index, 0);
    ...
    clib_warning("Worker uri: %s, target_uri: %v", worker_uri, ctx->target_uri);
					...
					session_t *s, session_error_t err)
{
	...
	clib_warning("Send PUT: %s", ctx->target_uri);
