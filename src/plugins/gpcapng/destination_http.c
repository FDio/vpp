					...
					session_t *s, session_error_t err)
{
  ...
  void **per_worker_ctx = gpm->worker_output_ctx[s->thread_index];
  u32 idx = vec_len(per_worker_ctx) - 1; // FIXME: last element - racy!
  s->opaque = idx;

  ctx = per_worker_ctx[idx];
  if (!ctx)
    {
...
http_pcapng_session_disconnect_callback (session_t *s)
{
  geneve_pcapng_main_t *gpm = get_geneve_pcapng_main ();
  void **per_worker_ctx = gpm->worker_output_ctx[s->thread_index];
  http_pcapng_ctx_t *ctx = per_worker_ctx[s->opaque];
...
void geneve_pcapng_ensure_session_manager() {
...
void set_pcapng_output_http(geneve_output_t *output) {
