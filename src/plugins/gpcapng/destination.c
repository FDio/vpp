...
extern void set_write_pcapng (
  gpcapng_dest_t *output); // FIXME: to be refactored out to use inlines
...
u8 *get_pcapng_shb_vec ();

u8 *get_pcapng_idb (u8 **orig_block, u32 if_index, const char *if_name);
...
pcap_del_capture_destination (const char *name)
{
  ...
  for (worker_index = 0; worker_index <= vlib_num_workers (); worker_index++)
	  ...
	  gpm->worker_output_ctx[worker_index][dest_index])
	{
	   ...
	   * Note: This is safe because the bitmap was already allocated when the
	   * destination was created (see wdi_set_ready_flag calls during init).
	   * The bitmap is guaranteed to have space for this dest_index. */
	  gpm->worker_output_ctx_is_ready[worker_index] =
	    clib_bitmap_set (gpm->worker_output_ctx_is_ready[worker_index],
			     dest_index, 0);
	}
      ...
      if (verbose_level > 0)
	{
	  ...
	  for (int worker_index = 0; worker_index <= vlib_num_workers ();
		  ...
		  gpm->worker_output_ctx[worker_index][dest_index])
		{
		  ...
		  vlib_cli_output (vm, "    Ready: %s", is_ready ? "Yes" : "No");
	      ...
	      if (verbose_level > 0)
		{
		  ...
		  for (int worker_index = 0; worker_index <= vlib_num_workers ();
			  ...
			  gpm->worker_output_ctx[worker_index][dest_index])
			{
			  ...
			  if (clib_bitmap_get (gpm->worker_output_ctx_is_ready[worker_index], 
					       dest_index))
			    ready_threads++;
			}
		    }
		  
		  vlib_cli_output (vm, "       Total packets: %lu across %u/%u ready threads",
