...
void wdi_set_ready_flag(worker_dest_index_t wdi, int is_ready) {
   ...
   u16 destination_index = wdi_to_destination_index(wdi);
         gpm->worker_output_ctx_is_ready[worker_index] =
                clib_bitmap_set(gpm->worker_output_ctx_is_ready[worker_index], destination_index, is_ready);
}
...
workers_verify() {
    ...
    for(worker_index = 0; worker_index <= vlib_num_workers(); worker_index++) {
      u32 dest_index;
      pool_foreach_index(dest_index, gpm->outputs) {
          void *ctx = wdi_to_worker_context(make_wdi(worker_index, dest_index));
	  worker_context_verify(ctx);
      }
    }
...
pcap_add_capture_destination(char *name, pcapng_destination_type_t cap_type, u8 *path_or_url)
{
    ...
    for(worker_index = 0; worker_index <= vlib_num_workers(); worker_index++) {
      ...
      void *output_ctx = new_destination->init(new_destination, worker_index, output_index);
      if (output_ctx) {
        /* Store the context */
        gpm->worker_output_ctx[worker_index][output_index] = output_ctx;
        if (cap_type != PCAPNG_DEST_HTTP) {
           /* HTTP destination does not become ready until the connection happens, the rest are ok immediately */
	   u8 *start_data = get_pcapng_preamble_vec();
	   new_destination->chunk_write(output_ctx, start_data, vec_len(start_data));
    ...
    for(worker_index = 0; worker_index <= vlib_num_workers(); worker_index++) {
        ...
        if (vec_len(gpm->worker_retry_queue) > worker_index && 
            ...
            for (i = 0; i < vec_len(gpm->worker_retry_queue[worker_index]); i++) {
                worker_dest_index_t wdi = gpm->worker_retry_queue[worker_index][i].wdi;
                if (wdi_to_destination_index(wdi) == dest_index) {
                    /* Poison this entry */
                    gpm->worker_retry_queue[worker_index][i].wdi = WDI_POISON_VALUE;
                }
            }
        }
    }
                                        ...
                                        vlib_cli_command_t * cmd)
{
    ...
    if (name)
    {
        ...
        vlib_cli_output(vm, "threads:\n");
	for(int worker_index = 0; worker_index <= vlib_num_workers(); worker_index++) {
            vlib_cli_output(vm, "  Thread index: %d\n", worker_index);
	    if (dest->print_worker_context) {
                dest->print_worker_context(vm, gpm->worker_output_ctx[worker_index][dest_index]);
	    }
	   ///XXXXXX 
	}
    }
...
VLIB_CLI_COMMAND(gpcapng_destination_add_command, static) = {
    ...
    .short_help = "gpcapng destination add name <name> {file <path> | http <url>}",
