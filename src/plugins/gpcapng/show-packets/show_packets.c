...
VLIB_PLUGIN_REGISTER () = {
  ...
  .description =
    "GPCAPNG Show Packets - PCAPNG file reader and packet display for GENEVE",
};
...
format_geneve_option (u8 *s, va_list *args)
{
  ...
  switch (opt_class)
    ...
    case 0x0100:
      ...
      s = format (s, "\n      Interpretation: Network Virtualization Base "
		     "Header (RFC 8926)");
      break;
    case 0x0101:
      /* OAM option class */
...
format_inner_ipv4 (u8 *s, va_list *args)
{
  ...
  s =
    format (s, "\n      Inner IPv4: %U -> %U, len=%d, proto=%d",
	    format_ip4_address, &ip->src_address, format_ip4_address,
	    &ip->dst_address, clib_net_to_host_u16 (ip->length), ip->protocol);
  ...
  if (data_len >= hdr_len)
	...
	case IP_PROTOCOL_TCP:
	  {
		...
		s = format (
		  ...
		  format_tcp_port, &tcp->src_port, format_tcp_port,
		  &tcp->dst_port, format_tcp_flags, tcp->flags);
	      }
	    else
	      {
	...
	case IP_PROTOCOL_UDP:
	  {
		...
		s = format (
		  ...
		  format_udp_port, &udp->src_port, format_udp_port,
		  &udp->dst_port, clib_net_to_host_u16 (udp->length));
	      }
	    else
	      {
	...
	case IP_PROTOCOL_ICMP:
	  {
	    icmp46_header_t *icmp =
	      (icmp46_header_t *) (packet_data + hdr_len);
	...
	default:
	  s = format (s, "\n      Inner Protocol %d: %U", ip->protocol,
		      format_hex_bytes, packet_data + hdr_len,
		      data_len - hdr_len);
	  break;
	}
    }
...
format_inner_ipv6 (u8 *s, va_list *args)
{
  ...
  s = format (s, "\n      Inner IPv6: %U -> %U, len=%d, proto=%d",
	      ...
	      &ip->dst_address, payload_length, protocol);
  ...
  if (data_len >= sizeof (ip6_header_t))
	...
	case IP_PROTOCOL_TCP:
	  {
	    tcp_header_t *tcp =
	      (tcp_header_t *) (packet_data + sizeof (ip6_header_t));
	...
	case IP_PROTOCOL_UDP:
	  {
	    udp_header_t *udp =
	      (udp_header_t *) (packet_data + sizeof (ip6_header_t));
	...
	case IP_PROTOCOL_ICMP6:
	  {
	    icmp46_header_t *icmp =
	      (icmp46_header_t *) (packet_data + sizeof (ip6_header_t));
	...
	default:
	  s = format (s, "\n      Inner Protocol %d: %U", protocol,
		      format_hex_bytes, packet_data + sizeof (ip6_header_t),
...
format_inner_packet (u8 *s, va_list *args)
{
  ...
  switch (protocol)
    ...
    case 0x0806: /* ARP */
      s = format (s, "\n      Inner ARP: %U", format_hex_bytes, packet_data,
		  data_len);
      break;

    default:
      s = format (s, "\n      Inner Protocol 0x%04x: %U", protocol,
...
format_geneve_packet (u8 *s, va_list *args)
{
  ...
  switch (ethertype)
	...
	s = format (s, "  IPv4: %U -> %U, len=%d, ttl=%d, protocol=%d\n",
		    format_ip4_address, &ip->src_address, format_ip4_address,
		    &ip->dst_address, clib_net_to_host_u16 (ip->length),
		    ip->ttl, ip->protocol);
	...
	if (ip->protocol == IP_PROTOCOL_UDP)
	  {
	    ...
	    s = format (
	      s, "  UDP: src_port=%U, dst_port=%U, len=%d, checksum=0x%04x\n",
	      format_udp_port, &udp->src_port, format_udp_port, &udp->dst_port,
		...
		if (data_len < ip_hdr_len + sizeof (udp_header_t) +
				 sizeof (geneve_header_t))
		  return format (s, "  Truncated GENEVE header");

		geneve = (geneve_header_t *) (packet_data + ip_hdr_len +
					      sizeof (udp_header_t));
		geneve_hdr_len = sizeof (geneve_header_t) +
				 (((geneve->ver_opt_len & 0x3F) >> 1) * 4);

		s = format (
		  s, "  GENEVE: ver=%U, opt_len=%U, protocol=%U, VNI=%U",
		  format_geneve_version, geneve->ver_opt_len,
		  format_geneve_opt_len, geneve->ver_opt_len,
		  format_geneve_protocol, geneve->protocol, format_geneve_vni,
		  geneve->vni);
		...
		if ((geneve->ver_opt_len & 0x3F) != 0)
		  {
		    s =
		      format (s, "%U", format_geneve_options,
		...
		if (data_len >
		    ip_hdr_len + sizeof (udp_header_t) + geneve_hdr_len)
		  {
		    s = format (s, "%U", format_inner_packet,
				packet_data + ip_hdr_len +
				  sizeof (udp_header_t) + geneve_hdr_len,
				data_len - ip_hdr_len - sizeof (udp_header_t) -
				  geneve_hdr_len,
				geneve->protocol);
		  }
	...
	s =
	  format (s, "  IPv6: %U -> %U, len=%d, hop_limit=%d, protocol=%d\n",
		  format_ip6_address, &ip->src_address, format_ip6_address,
		  &ip->dst_address, clib_net_to_host_u16 (ip->payload_length),
		  ip->hop_limit, ip->protocol);
	...
	if (ip->protocol == IP_PROTOCOL_UDP)
	  {
		...
		geneve =
		  (geneve_header_t *) (packet_data + sizeof (ip6_header_t) +
				       sizeof (udp_header_t));
		geneve_hdr_len = sizeof (geneve_header_t) +
				 (((geneve->ver_opt_len & 0x3F) >> 1) * 4);
...
static clib_error_t *
gpcapng_show_command_fn (vlib_main_t *vm, unformat_input_t *input,
	...
	case PCAPNG_BLOCK_TYPE_SHB:
	  {
	    ...
	    if (verbose)
	      {
		vlib_cli_output (
		  vm, "Section Header Block: magic=0x%08x, version=%d.%d",
		  magic, major, minor);
	      }
	  }
	  break;
	...
	case PCAPNG_BLOCK_TYPE_IDB:
	  {
		...
		interface_names[if_index] =
		  (char *) format (0, "%s%c", ifname, 0);
	      }

	    if (verbose)
	      {
		vlib_cli_output (vm,
				 "Interface Description Block: index=%d, "
				 "name=%s, link_type=%d",
	...
	case PCAPNG_BLOCK_TYPE_EPB:
	  {
		...
		/* Format the packet */
		vlib_cli_output (vm, "%U", format_geneve_packet, packet_data,
				 captured_len);
	...
	case PCAPNG_BLOCK_TYPE_SPB:
	  {
	    ...
	    if (packet_count < max_packets)
	      {
		vlib_cli_output (vm,
				 "\nPacket #%d: len=%d, interface=unknown "
				 "(simple packet block)",
