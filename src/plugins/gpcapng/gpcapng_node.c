...
no_match:
  /*
    ...
    clib_warning("dat: %U", format_hexdump, filter->value,
    vec_len(filter->value)); clib_warning("msk: %U", format_hexdump,
    filter->mask, vec_len(filter->mask));
  */
  return false;
}
...
static u32
geneve_packet_matches_filter (gpcapng_main_t *gpm, const u8 *outer_hdr,
			      ...
			      const geneve_capture_filter_t *filter)
{
  ...
  for (i = 0; i < vec_len (filter->option_filters); i++)
      ...
      if (filter->option_filters[i].option_name)
	{
	  ...
	  p = hash_get_mem (gpm->option_by_name,
      ...
      while (remaining_len >= sizeof (geneve_option_t))
	{
	  ...
	  if (clib_net_to_host_u16 (current_opt->opt_class) == opt_class &&
	      ...
	      if (filter->option_filters[i].data_len > 0)
		{
		  ...
		  /* Make sure we don't try to match more than the actual
		   * option data */
...
static u32
geneve_packet_matches_global_filter (gpcapng_main_t *gpm, const u8 *outer_hdr,
				     u32 outer_len, const u8 *inner_hdr,
				     u32 inner_len,
				     ...
				     u32 geneve_header_len)
{
  ...
  for (i = 0; i < vec_len (gpm->global_filters); i++)
	...
	gpm, outer_hdr, outer_len, inner_hdr, inner_len, geneve_hdr,
	geneve_header_len, &gpm->global_filters[i]);
      if (dest_index != ~0)
	return dest_index;
    }
  ...
  for (u32 pkt_idx = 0; pkt_idx < n_left_from; pkt_idx++)
      ...
      if (vec_len (gpm->global_filters) > 0)
	{
	  destination_capture_index = geneve_packet_matches_global_filter (
	    gpm, outer_header, outer_header_len, inner_header,
	  ...
	  gpm->per_interface[sw_if_index0].capture_enabled)
	{
	  for (i = 0; i < vec_len (gpm->per_interface[sw_if_index0].filters);
	       i++)
	    {
    ...
    packet_classified:
      ...
      pw->buffer_indices[n_classified] = bi0;
      pw->dest_indices[n_classified] = destination_capture_index;
      n_classified++;
			 ...
			 u32 *n_captured, u32 *n_dropped, u32 *n_not_ready)
{
  ...
  for (u32 pkt_idx = 0; pkt_idx < n_classified; pkt_idx++)
			   ...
			   destination_capture_index))
	{
	  ...
	  vec_add_pcapng_epb (&worker_common->buffer_vec, output_ctx,
			      (sw_if_index0 << 1) | is_output, timestamp,
			      orig_len, packet_copy, offset);
