...
gpcapng_enable_capture (u32 sw_if_index, u8 enable)
{
  ...
  gpcapng_per_interface_config_t empty_cfg = { 0 };
  vec_validate_init_empty (gmp->per_interface, sw_if_index, empty_cfg);
  ...
  if (enable)
	...
	"interface-output", "geneve-pcapng-capture-out", sw_if_index, 1, 0, 0);
  ...
  else
    {
      ...
      vnet_feature_enable_disable (
	"interface-output", "geneve-pcapng-capture-out", sw_if_index, 0, 0, 0);
      vnet_feature_enable_disable ("device-input", "geneve-pcapng-capture-in",
				   sw_if_index, 0, 0, 0);
    }
  ...
  for (int i = 0; i <= vlib_num_workers (); i++)
    {
       ...
       * data path */
      vec_validate (pw->buffer_indices, VLIB_FRAME_SIZE - 1);
      vec_validate (pw->dest_indices, VLIB_FRAME_SIZE - 1);
      vec_validate (pw->bufs, VLIB_FRAME_SIZE - 1);
