		...
		u32 comment_len)
{
  ...
  *(u32 *) (block + 24) = orig_len;		  /* Original length */
  ...
  while (buf_iter && rem_len)
    {
      u32 len = buf_iter->current_length > rem_len ? buf_iter->current_length :
						     rem_len;
      clib_memcpy_fast (block + 28 + offset,
			vlib_buffer_get_current (buf_iter), len);
      offset += len;
      rem_len -= len;
      buf_iter = buf_iter->flags & VLIB_BUFFER_NEXT_PRESENT ?
		   vlib_get_buffer (vm, buf_iter->next_buffer) :
		   0;
    }
...
static_always_inline int
vec_add_pcapng_epb (vlib_main_t *vm, u8 **vec_out, void *context, u32 if_index,
		    u64 timestamp, vlib_buffer_t *b0, u32 capture_len)
{
  ...
  return get_pcapng_epb (vm, vec_out, if_index, timestamp, b0, capture_len,
			 json_comment, strlen (json_comment));
}
