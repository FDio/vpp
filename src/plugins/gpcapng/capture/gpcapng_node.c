...
#include <gpcapng/lib/gpcapng_filter_api.h>
			  ...
			  gpcapng_per_worker_t *pw, u32 *n_matched)
{
  ...
  /* Reset the capture indices just as a precaution */
  ...
  filter_impl->selected_fn (
    GPCAPNG_FILTER_API_VERSION, pw->bufs, frame->n_vectors, vm, node, frame,
    is_output, gpm->capture_enabled_bitmap, pw->dest_indices, n_matched,
    &n_captured, pw->filtering_elapsed, pw->filter_names);
  ...
  for (u32 pkt_idx = 0; pkt_idx < n_classified; pkt_idx++)
			   ...
			   destination_capture_index))
	{
	  ...
	  vec_add_pcapng_epb (vm, &worker_common->buffer_vec, output_ctx,
			      (sw_if_index0 << 1) | is_output, timestamp, b0,
			      ~0);
      ...
      else if (destination_capture_index != ~0)
	{
      ...
      if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE) &&
			 (pw->bufs[pkt_idx]->flags & VLIB_BUFFER_IS_TRACED)))
	{
	  pcapng_capture_trace_t *t =
	    vlib_add_trace (vm, node, pw->bufs[pkt_idx], sizeof (*t));
	  t->sw_if_index = vnet_buffer (pw->bufs[pkt_idx])
			     ->sw_if_index[is_output ? VLIB_TX : VLIB_RX];
	  t->filtering_elapsed = pw->filtering_elapsed[pkt_idx];
	  t->dest_index = destination_capture_index;
	  t->filter_name = pw->filter_names[pkt_idx];
	}
    }
}

/* Filter and capture packets - Two-pass implementation */
