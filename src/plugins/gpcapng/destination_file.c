...
static void *
file_output_init (gpcapng_dest_t *output, u16 worker_index,
		  u16 destination_index)
{
  pcapfile_dest_ctx_t *ctx;

  ctx = clib_mem_alloc_aligned (sizeof (pcapfile_dest_ctx_t),
				CLIB_CACHE_LINE_BYTES);
  memset (ctx, 0, sizeof (*ctx));

  worker_context_init_common (ctx, PCAPNG_DEST_FILE);

  /* Create a unique filename per worker */
  ctx->filename =
    (void *) format (0, "%v-%u.pcapng%c", output->arg, worker_index, 0);
  ...
  if (!ctx->file)
    ...
    {
      clib_warning ("File is open: %s. file handle: %p", ctx->filename,
		    ctx->file);
    }
...
void
set_pcapng_output_file (gpcapng_dest_t *output)
{
