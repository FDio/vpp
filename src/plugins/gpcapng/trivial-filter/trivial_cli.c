...
#include <vlib/vlib.h>
#include <vnet/vnet.h>
#include <gpcapng/lib/public_inlines.h>
...
static clib_error_t *
gpcapng_filter_trivial_command_fn (vlib_main_t *vm, unformat_input_t *input,
				   vlib_cli_command_t *cmd)
{
  ...
  if (!unformat_user (input, unformat_line_input, line_input))
    return clib_error_return (
      0, "expected 'capture-all destination {none | name <name>}'");
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (line_input, "destination"))
	{
	  if (unformat (line_input, "none"))
		...
		0, "expected 'none' or 'name <destination-name>'");
	      goto done;
	    }
	}
      else
  ...
  if (capture_none && destination_name)
    {
      error = clib_error_return (
	0, "cannot specify both 'none' and destination name");
      goto done;
    }
  ...
  if (!capture_none && !destination_name)
    {
      error = clib_error_return (
	0, "must specify either 'none' or destination name");
      goto done;
    }
  ...
  if (capture_none)
  ...
  else
    {
      ...
      destination_index =
	gpcapng_plugin.find_destination_by_name ((char *) destination_name);
      if (destination_index == ~0)
	{
	  error = clib_error_return (0, "destination '%s' not found",
				     destination_name);
	  goto done;
	}
      ...
      vlib_cli_output (vm,
		       "Trivial filter set to capture all packets to "
		       "destination '%s' (index %u)",
...
VLIB_CLI_COMMAND (gpcapng_filter_trivial_command, static) = {
  ...
  .short_help =
    "gpcapng filter trivial capture-all destination {none | name <name>}",
