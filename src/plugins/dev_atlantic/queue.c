...
atl_rx_queue_alloc (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  ...
  rv = vnet_dev_dma_mem_alloc (vm, dev, sizeof (atl_rx_desc_t) * rxq->size, 0,
			       (void **) &aq->descs);
  if (rv != VNET_DEV_OK)
    {
...
atl_tx_queue_alloc (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  ...
  aq->buffer_indices = clib_mem_alloc_aligned (
    txq->size * sizeof (aq->buffer_indices[0]), CLIB_CACHE_LINE_BYTES);
  ...
  rv = vnet_dev_dma_mem_alloc (vm, dev, sizeof (atl_tx_desc_t) * txq->size, 0,
			       (void **) &aq->descs);
  ...
  rv = vnet_dev_dma_mem_alloc (vm, dev, CLIB_CACHE_LINE_BYTES,
			       CLIB_CACHE_LINE_BYTES, (void **) &aq->wb);
