...
atl_aq2_read_fw_version (vnet_dev_t *dev)
{
  ...
  rv = atl_aq2_interface_buffer_read (
    dev, ATL_REG_AQ2_FW_INTERFACE_OUT_VERSION_BUNDLE, &bundle_reg.as_u32, 1);
  if (rv != VNET_DEV_OK)
    return rv;
  ...
  rv = atl_aq2_interface_buffer_read (
    dev, ATL_REG_AQ2_FW_INTERFACE_OUT_VERSION_IFACE, &iface_reg.as_u32, 1);
...
atl_init (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  rv = atl_aq2_interface_buffer_read (
    dev, ATL_REG_AQ2_FW_INTERFACE_OUT_VERSION_MAC, ver_data, 3);
  if (rv != VNET_DEV_OK)
    return rv;
  ...
  rv = atl_aq2_interface_buffer_read (
    dev, ATL_REG_AQ2_FW_INTERFACE_OUT_FILTER_CAPS, &filter_caps, 1);
  if (rv != VNET_DEV_OK)
    return rv;
  ...
  ad->resolver_base = ((filter_caps & 0x00ff0000) >> 16) * 8;
