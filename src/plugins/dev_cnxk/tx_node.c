	      ...
	      lmt_line_t *line, u32 flags, int simple, int trace)
{
  ...
  if (trace && flags & VLIB_BUFFER_IS_TRACED)
    {
      cnxk_tx_trace_t *t = vlib_add_trace (vm, ctx->node, b, sizeof (*t));
      t->desc = d;
	       ...
	       vlib_buffer_t **b, u32 n_pkts, int trace)
{
  ...
  for (n_left = n_pkts; n_left >= 8; n_left -= 8, b += 8, l += 8)
	...
	{
	  int simple = 1;
	  cnxk_tx_enq1 (vm, ctx, b[0], l, f0, simple, trace);
	  cnxk_tx_enq1 (vm, ctx, b[1], l + 1, f1, simple, trace);
	  cnxk_tx_enq1 (vm, ctx, b[2], l + 2, f2, simple, trace);
	  cnxk_tx_enq1 (vm, ctx, b[3], l + 3, f3, simple, trace);
	  cnxk_tx_enq1 (vm, ctx, b[4], l + 4, f4, simple, trace);
	  cnxk_tx_enq1 (vm, ctx, b[5], l + 5, f5, simple, trace);
	  cnxk_tx_enq1 (vm, ctx, b[6], l + 6, f6, simple, trace);
	  cnxk_tx_enq1 (vm, ctx, b[7], l + 7, f7, simple, trace);
	  dpl[0] = dpl[1] = dpl[2] = dpl[3] = dpl[4] = dpl[5] = dpl[6] =
	    dpl[7] = 2;
	}
      else
	{
	  int simple = 0;
	  dpl[0] = cnxk_tx_enq1 (vm, ctx, b[0], l, f0, simple, trace);
	  dpl[1] = cnxk_tx_enq1 (vm, ctx, b[1], l + 1, f1, simple, trace);
	  dpl[2] = cnxk_tx_enq1 (vm, ctx, b[2], l + 2, f2, simple, trace);
	  dpl[3] = cnxk_tx_enq1 (vm, ctx, b[3], l + 3, f3, simple, trace);
	  dpl[4] = cnxk_tx_enq1 (vm, ctx, b[4], l + 4, f4, simple, trace);
	  dpl[5] = cnxk_tx_enq1 (vm, ctx, b[5], l + 5, f5, simple, trace);
	  dpl[6] = cnxk_tx_enq1 (vm, ctx, b[6], l + 6, f6, simple, trace);
	  dpl[7] = cnxk_tx_enq1 (vm, ctx, b[7], l + 7, f7, simple, trace);
	}
      dpl += 8;
    }
  ...
  for (; n_left > 0; n_left -= 1, b += 1, l += 1)
    {
      u32 f0 = b[0]->flags;
      dpl++[0] = cnxk_tx_enq1 (vm, ctx, b[0], l, f0, 0, trace);
      or_flags_16 |= f0;
    }
  ...
  if (PREDICT_FALSE (node->flags & VLIB_NODE_FLAG_TRACE))
    {
      for (n_left = clib_min (n_pkts, txq->size - n_enq), n = 0; n_left >= 16;
	   n_left -= 16, b += 16)
	n += cnxk_tx_enq16 (vm, &ctx, txq, b, 16, /* trace */ 1);

      if (n_left)
	n += cnxk_tx_enq16 (vm, &ctx, txq, b, n_left, /* trace */ 1);
    }
  else
    {
      for (n_left = clib_min (n_pkts, txq->size - n_enq), n = 0; n_left >= 16;
	   n_left -= 16, b += 16)
	n += cnxk_tx_enq16 (vm, &ctx, txq, b, 16, /* trace */ 0);

      if (n_left)
	n += cnxk_tx_enq16 (vm, &ctx, txq, b, n_left, /* trace */ 0);
    }
