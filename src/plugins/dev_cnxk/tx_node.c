...
cnxk_tx_enq1 (vlib_main_t *vm, cnxk_tx_ctx_t *ctx, vlib_buffer_t *b,
	      u128 *dword, u32 flags, int simple)
{
  u8 n_dwords = 2;
  u32 total_len = 0;
  cnxk_tx_desc_t d = {
	       ...
	       vlib_buffer_t **b, u32 n_pkts)
{
  ...
  for (n_left = n_pkts; n_left >= 4; n_left -= 4, b += 4)
	...
	{
	  cnxk_tx_enq1 (vm, ctx, b[0], dw, f0, /* simple */ 1);
	  cnxk_tx_enq1 (vm, ctx, b[1], dw + 2, f1, /* simple */ 1);
	  cnxk_tx_enq1 (vm, ctx, b[2], dw + 4, f2, /* simple */ 1);
	  cnxk_tx_enq1 (vm, ctx, b[3], dw + 6, f3, /* simple */ 1);
	  dw += 8;
	...
	{
	  n = cnxk_tx_enq1 (vm, ctx, b[0], dw, f0, /* simple */ 0);
	  dpl++[0] = n;
	  dw += n;
	  n = cnxk_tx_enq1 (vm, ctx, b[1], dw, f1, /* simple */ 0);
	  dpl++[0] = n;
	  dw += n;
	  n = cnxk_tx_enq1 (vm, ctx, b[2], dw, f2, /* simple */ 0);
	  dpl++[0] = n;
	  dw += n;
	  n = cnxk_tx_enq1 (vm, ctx, b[3], dw, f3, /* simple */ 0);
	  dpl++[0] = n;
	  dw += n;
	}
    }
  ...
  if (PREDICT_FALSE (or_flags_16 & not_simple_flags))
    cnxk_lmt_store (ctx->lmtctx, dwords, dwords_per_line, n_pkts);
  else
    cnxk_lmt_store_fixed (ctx->lmtctx, dwords, 2, n_pkts);
