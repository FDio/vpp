...
vnet_dev_rv_t
cnxk_rxq_start (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  ...
  if ((rrv = roc_nix_rq_ena_dis (&crq->rq, 1)))
    return cnxk_roc_err (dev, rrv, "roc_nix_rq_ena_dis() failed");

  return VNET_DEV_OK;
}
void
cnxk_rxq_stop (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  vnet_dev_t *dev = rxq->port->dev;
  cnxk_rxq_t *crq = vnet_dev_get_rx_queue_data (rxq);
  int rrv;
  ...
  if ((rrv = roc_nix_rq_ena_dis (&crq->rq, 0)))
    cnxk_roc_err (dev, rrv, "roc_nix_rq_ena_dis() failed");

  n = cnxk_aura_free_all_buffers (vm, crq->aura_handle, crq->hdr_off);
...
void
cnxk_txq_stop (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  vnet_dev_t *dev = txq->port->dev;
  cnxk_txq_t *ctq = vnet_dev_get_tx_queue_data (txq);
  u32 n;

  n = cnxk_aura_free_all_buffers (vm, ctq->aura_handle, ctq->hdr_off);

  if (ctq->n_enq - n > 0)
    log_err (dev, "%u buffers leaked on tx queue %u stop", ctq->n_enq - n,
	     txq->queue_id);
  else
...
cnxk_port_start (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  foreach_vnet_dev_port_rx_queue (q, port)
    if ((rv = cnxk_rxq_start (vm, q)) != VNET_DEV_OK)
      goto done;
  ...
  if ((rrv = roc_nix_npc_rx_ena_dis (nix, true)))
    {
      rv = cnxk_roc_err (dev, rrv, "roc_nix_npc_rx_ena_dis() failed");
      goto done;
    }
