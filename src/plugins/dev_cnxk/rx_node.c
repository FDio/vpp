...
cnxk_seg_to_bp (void *p)
{
  return (vlib_buffer_t *) p - 1;
}

static_always_inline void
cnxk_rx_attach_tail (vlib_main_t *vm, cnxk_rx_node_ctx_t *ctx,
		     vlib_buffer_t *h, cnxk_nix_rx_cqe_desc_t *d)
{
	       ...
	       vnet_dev_rx_queue_t *rxq, u32 n)
{
  ...
  for (n_left = n; n_left >= 8; d += 4, n_left -= 4, ctx->to_next += 4)
    {
      u32 segs = 0;
      clib_prefetch_store (cnxk_seg_to_bp (d[4].segs0[0]));
      clib_prefetch_store (cnxk_seg_to_bp (d[5].segs0[0]));
      b[0] = cnxk_seg_to_bp (d[0].segs0[0]);
      clib_prefetch_store (cnxk_seg_to_bp (d[6].segs0[0]));
      b[1] = cnxk_seg_to_bp (d[1].segs0[0]);
      clib_prefetch_store (cnxk_seg_to_bp (d[7].segs0[0]));
      b[2] = cnxk_seg_to_bp (d[2].segs0[0]);
      b[3] = cnxk_seg_to_bp (d[3].segs0[0]);
	...
	{
	  cnxk_rx_attach_tail (vm, ctx, b[0], d + 0);
	  cnxk_rx_attach_tail (vm, ctx, b[1], d + 1);
	  cnxk_rx_attach_tail (vm, ctx, b[2], d + 2);
	  cnxk_rx_attach_tail (vm, ctx, b[3], d + 3);
	}
    }
...
static_always_inline void
cnxk_rxq_refill_batch (vlib_main_t *vm, u64 lmt_id, u64 addr,
		       cnxk_npa_lf_aura_batch_free_line_t *lines, u32 *bi,
		       cnxk_npa_lf_aura_batch_free0_t w0, u64 n_lines)
{
  u64 data;
...
static_always_inline u32
cnxk_rxq_refill (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq, u16 n_refill)
{
  ...
  cnxk_rxq_t *crq = vnet_dev_get_rx_queue_data (rxq);
  cnxk_npa_lf_aura_batch_free_line_t *lines;
  ...
  lines = (cnxk_npa_lf_aura_batch_free_line_t *) lmt_addr;
  ...
  while (n_lines >= batch_max_lines)
	...
	goto alloc_fail;
      cnxk_rxq_refill_batch (vm, lmt_id, addr, lines, buffer_indices, w0,
			     batch_max_lines);
  ...
  cnxk_rxq_refill_batch (vm, lmt_id, addr, lines, buffer_indices, w0, n_lines);
  n_enq += batch_bufs;
		     ...
		     vnet_dev_rx_queue_t *rxq, int with_flows)
{
  ...
  n_enq += cnxk_rxq_refill (vm, rxq, rxq->size - n_enq);
  crq->n_enq = n_enq;
