	       ...
	       vnet_dev_rx_queue_t *rxq, u32 n)
{
  ...
  for (n_left = n; n_left >= 8; d += 4, n_left -= 4, ctx->to_next += 4)
    {
      clib_prefetch_store ((vlib_buffer_t *) d[4].segs0[0] - 1);
      clib_prefetch_store ((vlib_buffer_t *) d[5].segs0[0] - 1);
      b[0] = (vlib_buffer_t *) d[0].segs0[0] - 1;
      clib_prefetch_store ((vlib_buffer_t *) d[6].segs0[0] - 1);
      b[1] = (vlib_buffer_t *) d[1].segs0[0] - 1;
      clib_prefetch_store ((vlib_buffer_t *) d[7].segs0[0] - 1);
      b[2] = (vlib_buffer_t *) d[2].segs0[0] - 1;
      b[3] = (vlib_buffer_t *) d[3].segs0[0] - 1;
      ...
      ctx->n_segs += d[0].sg0.segs;
      ctx->n_segs += d[1].sg0.segs;
      ctx->n_segs += d[2].sg0.segs;
      ctx->n_segs += d[3].sg0.segs;
    }
...
cnxk_rxq_refill (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq, u16 n_refill)
{
  ...
  u64 lmt_addr, lmt_id, ah, addr, n_enq = 0;
  u8 bpi = 0;
  cnxk_rxq_t *crq = vnet_dev_get_rx_queue_data (rxq);
  u32 n_lines, n_alloc;
  ...
  ah = crq->aura_handle;
  addr = roc_npa_aura_handle_to_base (ah) + NPA_LF_AURA_BATCH_FREE0;
  addr |= 0x7 << 4;
  ...
  cnxk_npa_lf_aura_batch_free_line_t *lines =
    (cnxk_npa_lf_aura_batch_free_line_t *) lmt_addr;

  cnxk_npa_lf_aura_batch_free0_t w0 = {
    .aura = roc_npa_aura_handle_to_aura (ah),
    .count_eot = 1,
  };
