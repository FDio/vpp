			      ...
			      struct rte_crypto_sym_xform *crypto_xfrm)
{
	...
	if (crypto_xfrm->type == RTE_CRYPTO_SYM_XFORM_AEAD) {
		...
		case RTE_CRYPTO_AUTH_NULL:
			if (w2->s.dir == ROC_IE_SA_DIR_INBOUND &&
			    ipsec_xfrm->replay_win_sz) {
	...
	if (key != NULL && length != 0) {
		...
		for (i = 0; i < (int)(ROC_CTX_MAX_CKEY_LEN / sizeof(uint64_t));
		     i++)
			tmp_key[i] = rte_be_to_cpu_64(tmp_key[i]);
	}
			     ...
			     struct rte_security_ipsec_xform *ipsec_xfrm)
{
	...
	case RTE_SECURITY_IPSEC_TUNNEL_IPV6:
		...
		/* IP Source and Dest are in LE/CPU endian */
		ot_ipsec_update_ipv6_addr_endianness(
			(uint64_t *)&sa->outer_hdr.ipv6.src_addr);
		ot_ipsec_update_ipv6_addr_endianness(
			(uint64_t *)&sa->outer_hdr.ipv6.dst_addr);
			    ...
			    struct roc_ie_on_inb_sa *in_sa)
{
	...
	if (crypto_xform->type != RTE_CRYPTO_SYM_XFORM_AEAD &&
	    crypto_xform->auth.algo == RTE_CRYPTO_AUTH_NULL &&
	    ipsec->replay_win_sz) {
		plt_err("anti-replay can't be supported with integrity service disabled");
		return -EINVAL;
	}
