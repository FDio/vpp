...
vlib_error_desc_t cnxk_tx_node_counters[] = { foreach_cnxk_tx_node_counter };
#undef _

vnet_dev_node_t cnxk_rx_node = {
  .format_trace = format_cnxk_rx_trace,
};

vnet_dev_node_t cnxk_tx_node = {
  .format_trace = format_cnxk_tx_trace,
  .error_counters = cnxk_tx_node_counters,
  .n_error_counters = ARRAY_LEN (cnxk_tx_node_counters),
};
...
cnxk_init_nix (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  *cd->nix = (struct roc_nix){
  ...
  vnet_dev_port_add_args_t port_add_args = {
	...
	.format_info = format_cnxk_rxq_info,
      },
    },
    .tx_queue = {
      .config = {
	...
	.format_info = format_cnxk_txq_info,
      },
    },
  };
