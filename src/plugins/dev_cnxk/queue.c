...
cnxk_txq_init (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  ...
  if ((rrv = roc_npa_pool_create (&ctq->aura_handle, bp->alloc_size, txq->size,
				  &aura, &npapool, 0)))
    {
      cnxk_txq_deinit (vm, txq);
      return cnxk_roc_err (dev, rrv, "roc_npa_pool_create() failed");
    }
...
u8 *
format_cnxk_rxq_info (u8 *s, va_list *args)
{
...
u8 *
format_cnxk_txq_info (u8 *s, va_list *args)
{
  vnet_dev_format_args_t *a = va_arg (*args, vnet_dev_format_args_t *);
