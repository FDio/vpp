...
static uword
pppoe_cp_dispatch_inline (vlib_main_t * vm,
                    ...
                    vlib_frame_t * from_frame)
{
  if(collect_detailed_interface_stats()){
      return pppoe_cp_dispatch_inline(vm, node,from_frame, COLLECT_DETAILED_STATS);
  }else{
      return pppoe_cp_dispatch_inline(vm, node,from_frame, COLLECT_SIMPLE_STATS);
  }
}
