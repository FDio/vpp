  ...
  if (t->sw_if_index != pem->cp_if_index)
    {
      s = format (s,
		  ...
		  t->sw_if_index, t->next_index, t->error, t->pppoe_code,
		  t->ppp_proto);
    }
  else
    {
      s = format (s,
		  ...
		  "  pppoe_code 0x%x  ppp_proto 0x%x",
		  t->cp_if_index, t->next_index, t->error, t->pppoe_code,
		  t->ppp_proto);
    }
  return s;
}
	  ...
	  if (rx_sw_if_index0 == pem->cp_if_index)
	    {
	      pppoe_lookup_1 (&pem->link_table, &cached_key, &cached_result,
			      h0->dst_address, 0, &key0, &bucket0, &result0);
	      ...
	      if (si->type == VNET_SW_INTERFACE_TYPE_SUB)
		{
	      ...
	      clib_memcpy_fast (vlib_buffer_get_current (b0) + 6,
				hi->hw_address, 6);
	    }
	  else
	    {
	      pppoe_lookup_1 (&pem->link_table, &cached_key, &cached_result,
			      h0->src_address, 0, &key0, &bucket0, &result0);
	      tx_sw_if_index0 = result0.fields.sw_if_index;

	      /* learn client session */
	      pppoe_learn_process (&pem->link_table, rx_sw_if_index0, &key0,
				   &cached_key, &bucket0, &result0);
