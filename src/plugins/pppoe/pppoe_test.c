...
api_pppoe_add_del_session (vat_main_t *vam)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "client-ip %U", unformat_ip4_address,
			 &client_ip.ip4))
	{
	  client_ip_set = 1;
	  ipv4_set = 1;
	}
      else if (unformat (line_input, "client-ip %U", unformat_ip6_address,
			 &client_ip.ip6))
	{
	  client_ip_set = 1;
	  ipv6_set = 1;
...
vl_api_pppoe_session_details_t_handler (vl_api_pppoe_session_details_t *mp)
{
  ...
  print (vam->ofp, "%11d%14d%24U%14d%14d%30U%30U", ntohl (mp->sw_if_index),
	 ntohl (mp->session_id), format_ip46_address, &client_ip,
	 IP46_TYPE_ANY, ntohl (mp->encap_if_index), ntohl (mp->decap_vrf_id),
	 format_ethernet_address, mp->local_mac, format_ethernet_address,
	 mp->client_mac);
}
...
api_pppoe_session_dump (vat_main_t *vam)
{
  ...
  if (!vam->json_output)
    {
      print (vam->ofp, "%11s%24s%14s%14s%14s", "sw_if_index", "client_ip",
	     "session_id", "encap_if_index", "decap_fib_index", "local-mac",
	     "client-mac");
    }
...
static void
vl_api_pppoe_add_del_cp_reply_t_handler (
  vl_api_pppoe_add_del_session_reply_t *mp)
{
  vat_main_t *vam = &vat_main;
  i32 retval = ntohl (mp->retval);
