...
format_pppoe_rx_trace (u8 *s, va_list *args)
{
  ...
  if (t->session_index != ~0)
    {
      s = format (
	s, "PPPoE decap from pppoe_session%d session_id %d next %d error %d",
	t->session_index, t->session_id, t->next_index, t->error);
    }
  else
    {
      s = format (
	s, "PPPoE decap error - session for session_id %d does not exist",
	t->session_id);
    }
  return s;
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (type0 == ETHERNET_TYPE_VLAN)
	    {
	      ...
	      if (type0 != ETHERNET_TYPE_PPPOE_DISCOVERY &&
		  type0 != ETHERNET_TYPE_PPPOE_SESSION)
		{
	  ...
	  pppoe_lookup_1 (&pem->session_table, &cached_key, &cached_result,
			  h0->src_address, pppoe0->session_id, &key0, &bucket0,
			  &result0);
	...
	trace0:
	  ...
	  if (type1 == ETHERNET_TYPE_VLAN)
	    {
	      ...
	      if (type1 != ETHERNET_TYPE_PPPOE_DISCOVERY &&
	  ...
	  pppoe_lookup_1 (&pem->session_table, &cached_key, &cached_result,
			  h1->src_address, pppoe1->session_id, &key1, &bucket1,
			  &result1);
  ...
  vlib_node_increment_counter (vm, pppoe_input_node.index,
			       PPPOE_ERROR_DECAPSULATED, pkts_decapsulated);
