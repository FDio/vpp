...
format_pppoe_session (u8 *s, va_list *args)
{
  ...
  s = format (s, "[%d] sw-if-index %d client-ip %U session-id %d ",
	      t - pem->sessions, t->sw_if_index, format_ip46_address,
	      &t->client_ip, IP46_TYPE_ANY, t->session_id);

  s = format (s, "encap-if-index %d decap-fib-index %d\n", t->encap_if_index,
	      t->decap_fib_index);

  s = format (s, "    local-mac %U  client-mac %U", format_ethernet_address,
	      t->local_mac, format_ethernet_address, t->client_mac);
...
static u8 *
pppoe_build_rewrite (vnet_main_t *vnm, u32 sw_if_index, vnet_link_t link_type,
 ...
 */
static void
pppoe_fixup (vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b0,
	     const void *data)
{
...
pppoe_update_adj (vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
  ...
  switch (adj->lookup_next_index)
	...
	ai, pppoe_fixup, (void *) len, ADJ_FLAG_NONE,
...
int
vnet_pppoe_add_del_session (vnet_pppoe_add_del_session_args_t *a,
			    u32 *sw_if_indexp)
{
  pppoe_main_t *pem = &pppoe_main;
  pppoe_session_t *t = 0;
...
#undef _
      ...
      pppoe_update_1 (&pem->session_table, a->client_mac,
		      clib_host_to_net_u16 (a->session_id), &key, &bucket,
		      &result);
      ...
      if (vec_len (pem->free_pppoe_session_hw_if_indices) > 0)
	{
	  ...
	  hw_if_index =
	    pem->free_pppoe_session_hw_if_indices
	...
	return VNET_API_ERROR_NO_SUCH_ENTRY;
      ...
      fib_table_entry_path_remove (a->decap_fib_index, &pfx, pppoe_fib_src,
...
static clib_error_t *
pppoe_add_del_session_command_fn (vlib_main_t *vm, unformat_input_t *input,
...
#undef _
  ...
  switch (rv)
    ...
    default:
      error =
	clib_error_return (0, "vnet_pppoe_add_del_session returned %d", rv);
      goto done;
    }
 ...
 * [0] client-ip 10.0.3.1 session_id 13 encap-if-index 0 decap-vrf-id 13
 sw_if_index 5
...
pppoe_show_walk_cb (BVT (clib_bihash_kv) * kvp, void *arg)
{
  ...
  if (ctx->first_entry)
    {
      ctx->first_entry = 0;
      vlib_cli_output (ctx->vm, "%=19s%=12s%=13s%=14s", "Mac-Address",
		       "session_id", "sw_if_index", "session_index");
    }
  ...
  vlib_cli_output (
    ctx->vm, "%=19U%=12d%=13d%=14d", format_ethernet_address, key.fields.mac,
 ...
 *     Mac Address      session_id      Interface           sw_if_index
session_index
...
pppoe_init (vlib_main_t *vm)
{
  ...
  BV (clib_bihash_init)
  (&pem->link_table, "pppoe link table", PPPOE_NUM_BUCKETS, PPPOE_MEMORY_SIZE);

  BV (clib_bihash_init)
  (&pem->session_table, "pppoe session table", PPPOE_NUM_BUCKETS,
   PPPOE_MEMORY_SIZE);
  ...
  pppoe_fib_src =
    fib_source_allocate ("pppoe", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_API);
