  ...
  (vnet_pppoe_add_del_session_args_t * a, u32 * sw_if_indexp)
{
  ...
  pppoe_lookup_1 (&pem->session_table, &cached_key, &cached_result,
		  a->client_mac, a->session_id, &key, &bucket, &result);
  ...
  /* add/delete reverse route for client ip */
  rpath.frp_sw_if_index = a->encap_if_index;
...
#undef _
	...
	hash_set (pem->pppoe4_session_by_key, key4.client_ip,
		  t - pem->sessions);
      ...
      else
	{
      ...
      fib_table_entry_path_add (a->decap_fib_index, &pfx, FIB_SOURCE_CLI,
				FIB_ENTRY_FLAG_ATTACHED,
				pfx.fp_proto, &pfx.fp_addr,
