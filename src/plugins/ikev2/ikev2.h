...
clib_error_t *ikev2_set_profile_auth (vlib_main_t *vm, u8 *name,
				      ...
				      u8 data_hex_format);
clib_error_t *ikev2_set_profile_id (vlib_main_t *vm, u8 *name, u8 id_type,
				    u8 *data, int is_local);
...
clib_error_t *ikev2_set_profile_ike_transforms (
  vlib_main_t *vm, u8 *name, ikev2_transform_encr_type_t crypto_alg,
  ikev2_transform_integ_type_t integ_alg, ikev2_transform_dh_type_t dh_type,
  u32 crypto_key_size);
clib_error_t *ikev2_set_profile_esp_transforms (
  vlib_main_t *vm, u8 *name, ikev2_transform_encr_type_t crypto_alg,
  ikev2_transform_integ_type_t integ_alg, u32 crypto_key_size);
...
clib_error_t *ikev2_set_profile_tunnel_interface (vlib_main_t *vm, u8 *name,
						  u32 sw_if_index);
vnet_api_error_t ikev2_set_profile_ipsec_udp_port (vlib_main_t *vm, u8 *name,
						   u16 port, u8 is_set);
