...
#define VL_API_CONTROL_PING ikev2_test_main.ping_id
...
format_ikev2_sa_transform (u8 * s, va_list * args)
{
  ...
  switch (tr->type)
    ...
    case IKEV2_TRANSFORM_TYPE_ENCR:
      s =
	format (s, "%U", format_ikev2_transform_encr_type,
		tr->transform_type);
      break;
    case IKEV2_TRANSFORM_TYPE_PRF:
      s =
	format (s, "%U", format_ikev2_transform_prf_type, tr->transform_type);
      break;
    case IKEV2_TRANSFORM_TYPE_INTEG:
      s =
	format (s, "%U", format_ikev2_transform_integ_type,
		tr->transform_type);
      break;
    case IKEV2_TRANSFORM_TYPE_DH:
      s =
	format (s, "%U", format_ikev2_transform_dh_type, tr->transform_type);
      break;
    case IKEV2_TRANSFORM_TYPE_ESN:
      s =
	format (s, "%U", format_ikev2_transform_esn_type, tr->transform_type);
      break;
    default:
      break;
    }

  u32 attrs_len = clib_net_to_host_u32 (tr->attrs_len);
  if (tr->type == IKEV2_TRANSFORM_TYPE_ENCR &&
      tr->transform_type == IKEV2_TRANSFORM_ENCR_TYPE_AES_CBC && tr->key_len)
    s = format (s, "-%u", tr->key_len * 8);
  else if (attrs_len == 4 && tr->attrs[0] == 0x80 && tr->attrs[1] == 0x0e)
    s = format (s, "-%u", tr->attrs[2] * 256 + tr->attrs[3]);
  else if (attrs_len)
    s = format (s, "(unknown attr %U)", format_hex_bytes,
		tr->attrs, tr->attrs_len);
  return s;
}
...
vl_api_ikev2_sa_details_t_handler (vl_api_ikev2_sa_details_t * mp)
{
  ...
  fformat (vam->ofp, "thread index: %d sa index: %d\n",
	   clib_net_to_host_u32 (mp->thread_index),
	   clib_net_to_host_u32 (mp->sa_index));
  fformat (vam->ofp, " iip %U ispi %lx rip %U rspi %lx\n", format_ip4_address,
	   &sa->iaddr, clib_net_to_host_u64 (sa->ispi), format_ip4_address,
	   &sa->raddr, clib_net_to_host_u64 (sa->rspi));
  fformat (vam->ofp, " %U ", format_ikev2_sa_transform, &mp->tr_encr);
  fformat (vam->ofp, "%U ", format_ikev2_sa_transform, &mp->tr_prf);
  fformat (vam->ofp, "%U ", format_ikev2_sa_transform, &mp->tr_integ);
  fformat (vam->ofp, "%U \n", format_ikev2_sa_transform, &mp->tr_dh);

  fformat (vam->ofp, "  SK_d    %U\n",
	   format_hex_bytes, sa->sk_d, sa->sk_d_len);

  fformat (vam->ofp, "  SK_a  i:%U\n        r:%U\n",
	   format_hex_bytes, sa->sk_ai, sa->sk_ai_len, format_hex_bytes,
	   sa->sk_ai, sa->sk_ai_len);

  fformat (vam->ofp, "  SK_e  i:%U\n        r:%U\n", format_hex_bytes,
	   sa->sk_ei, sa->sk_ei_len, format_hex_bytes, sa->sk_er,
	   sa->sk_er_len);

  fformat (vam->ofp, "  SK_p  i:%U\n        r:%U\n", format_hex_bytes,
	   sa->sk_pi, sa->sk_pi_len, format_hex_bytes, sa->sk_pr,
	   sa->sk_pr_len);
...
api_ikev2_child_sa_dump (vat_main_t * vam)
{
  ...
  while (unformat_check_input (i) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (i, "thread_index %d", &thread_index))
	;
  ...
  if (sa_index == ~0 || thread_index == ~0)
    return -99;

  /* Construct the API message */
  M (IKEV2_CHILD_SA_DUMP, mp);

  mp->thread_index = clib_net_to_host_u32 (thread_index);
...
vl_api_ikev2_child_sa_details_t_handler (vl_api_ikev2_child_sa_details_t * mp)
{
  ...
  fformat (vam->ofp, "  child sa %u:\n",
	   clib_net_to_host_u32 (mp->child_sa_index));

  fformat (vam->ofp, "    %U ", format_ikev2_sa_transform, &mp->tr_encr);
  fformat (vam->ofp, "%U ", format_ikev2_sa_transform, &mp->tr_integ);
  fformat (vam->ofp, "%U \n", format_ikev2_sa_transform, &mp->tr_esn);

  fformat (vam->ofp, "    spi(i) %lx spi(r) %lx\n",
	   clib_net_to_host_u32 (child_sa->i_proposals_spi),
	   clib_net_to_host_u32 (child_sa->r_proposals_spi));

  fformat (vam->ofp, "    SK_e  i:%U\n          r:%U\n",
	   format_hex_bytes, child_sa->sk_ei, child_sa->sk_ei_len,
	   format_hex_bytes, child_sa->sk_er, child_sa->sk_er_len);
  if (child_sa->sk_ai_len)
    {
      fformat (vam->ofp, "    SK_a  i:%U\n          r:%U\n",
	       format_hex_bytes, child_sa->sk_ai, child_sa->sk_ai_len,
	       format_hex_bytes, child_sa->sk_ar, child_sa->sk_ar_len);
    }
  vam->result_ready = 1;
}
...
api_ikev2_traffic_selector_dump (vat_main_t * vam)
{
  ...
  while (unformat_check_input (i) != UNFORMAT_END_OF_INPUT)
	...
	is_initiator = 0;
      else if (unformat (i, "thread_index %d", &thread_index))
	;
      else if (unformat (i, "sa_index %d", &sa_index))
	;
  ...
  if (child_sa_index == ~0 || sa_index == ~0 || thread_index == ~0
      || is_initiator == ~0)
    return -99;
  ...
  (vl_api_ikev2_traffic_selector_details_t * mp)
{
  ...
  fformat (vam->ofp, " type %u protocol_id %u addr "
	   "%U - %U port %u - %u\n",
...
api_ikev2_nonce_get (vat_main_t * vam)
{
  ...
  if (sa_index == ~0 || thread_index == ~0 || is_initiator == ~0)
    return -99;
...
vl_api_ikev2_nonce_get_reply_t_handler (vl_api_ikev2_nonce_get_reply_t * mp)
{
  ...
  fformat (vam->ofp, "  nonce:%U\n",
	   format_hex_bytes, mp->nonce, clib_net_to_host_u32 (mp->data_len));
