...
vl_api_ikev2_sa_details_t_handler (vl_api_ikev2_sa_details_t * mp)
{
  ...
  fformat (vam->ofp, " iip %U ispi %lx rip %U rspi %lx\n",
	   format_ip4_address, &sa->iaddr, clib_net_to_host_u64 (sa->ispi),
	   format_ip4_address, &sa->raddr, clib_net_to_host_u64 (sa->rspi));
...
api_ikev2_child_sa_dump (vat_main_t * vam)
{
  ...
  mp->sa_index = sa_index;
  mp->child_sa_index = child_sa_index;
...
{
  vat_main_t *vam = ikev2_test_main.vat_main;
  vl_api_ikev2_child_sa_t *child = &mp->child;
  ...
  fformat (vam->ofp, "    SK_e  i:%U\n          r:%U\n",
	   format_hex_bytes, child->sk_ei, child->sk_ei_len,
	   format_hex_bytes, child->sk_er, child->sk_er_len);
  if (child->sk_ai_len)
    {
      fformat (vam->ofp, "    SK_a  i:%U\n          r:%U\n",
	       format_hex_bytes, child->sk_ai, child->sk_ai_len,
	       format_hex_bytes, child->sk_ar, child->sk_ar_len);
    }
  vam->result_ready = 1;
}
...
api_ikev2_nonce_get (vat_main_t * vam)
{
  ...
  mp->_vl_msg_id = ntohs (VL_API_IKEV2_NONCE_GET + im->msg_id_base);
