...
static int
api_ikev2_profile_v2_dump (vat_main_t *vam)
{
  ikev2_test_main_t *ik = &ikev2_test_main;
  vl_api_ikev2_profile_v2_dump_t *mp;
  vl_api_control_ping_t *mp_ping;
  int ret;

  /* Construct the API message */
  M (IKEV2_PROFILE_V2_DUMP, mp);
...
static void
vl_api_ikev2_profile_v2_details_t_handler (
  vl_api_ikev2_profile_v2_details_t *mp)
{
  vat_main_t *vam = ikev2_test_main.vat_main;
  vl_api_ikev2_profile_v2_t *p = &mp->profile;
...
static int
api_ikev2_profile_set_ts_v2 (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  vl_api_ikev2_profile_set_ts_v2_t *mp;
  u8 *name = 0;
  u8 is_local = 0;
  u32 proto = 0, start_port = 0, end_port = (u32) ~0;
  ip_address_t start_addr, end_addr;
