...
{
  vat_main_t *vam = ikev2_test_main.vat_main;
  vl_api_ikev2_sa_details_t_endian (mp);
  vl_api_ikev2_sa_t *sa = &mp->sa;

  fformat (vam->ofp, "thread index: %d sa index: %d\n",
	   mp->sa.thread_index, mp->sa.sa_index);
  fformat (vam->ofp, " iip %U ispi %lx rip %U rspi %lx\n", format_ip4_address,
	   &sa->iaddr, sa->ispi, format_ip4_address, &sa->raddr, sa->rspi);
  fformat (vam->ofp, " %U ", format_ikev2_sa_transform, &sa->tr_encr);
  fformat (vam->ofp, "%U ", format_ikev2_sa_transform, &sa->tr_prf);
  fformat (vam->ofp, "%U ", format_ikev2_sa_transform, &sa->tr_integ);
  fformat (vam->ofp, "%U \n", format_ikev2_sa_transform, &sa->tr_dh);
...
{
  vat_main_t *vam = ikev2_test_main.vat_main;
  vl_api_ikev2_child_sa_details_t_endian (mp);
  ...
  fformat (vam->ofp, "    %U ", format_ikev2_sa_transform,
	   &child_sa->tr_encr);
  fformat (vam->ofp, "%U ", format_ikev2_sa_transform, &child_sa->tr_integ);
  fformat (vam->ofp, "%U \n", format_ikev2_sa_transform, &child_sa->tr_esn);
...
{
  vat_main_t *vam = ikev2_test_main.vat_main;
  vl_api_ikev2_traffic_selector_details_t_endian (mp);
  ...
  fformat (vam->ofp, " type %u protocol_id %u addr "
	   ...
	   format_ip4_address, &ts->end_addr, ts->start_port, ts->end_port);
  vam->result_ready = 1;
}
...
{
  vat_main_t *vam = ikev2_test_main.vat_main;
  vl_api_ikev2_nonce_get_reply_t_endian (mp);
