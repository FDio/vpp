...
{
  clib_memcpy (nonce, salt, GCM_SALT_SIZE);
  clib_memcpy (nonce + GCM_SALT_SIZE, iv, GCM_IV_SIZE);
}
			 ...
			 int data_len, u8 * aad, u32 aad_len, u8 * tag)
{
  ...
  if (data_len <= GCM_IV_SIZE)
    /* runt data */
    return 0;

  /* extract salt from the end of the key */
  u8 *salt = key + vec_len (key) - GCM_SALT_SIZE;
  ikev2_init_gcm_nonce (nonce, salt, data);

  data += GCM_IV_SIZE;
  data_len -= GCM_IV_SIZE;
  ...
  EVP_CIPHER_CTX_ctrl (ctx, EVP_CTRL_GCM_SET_TAG, GCM_ICV_SIZE, tag);
			 ...
			 v8 * src, u8 * dst, u8 * aad, u32 aad_len, u8 * tag)
{
  ...
  RAND_bytes (dst, GCM_IV_SIZE);
  ikev2_init_gcm_nonce (nonce, key + vec_len (key) - GCM_SALT_SIZE, dst);
  dst += GCM_IV_SIZE;
