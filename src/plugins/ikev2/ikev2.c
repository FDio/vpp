...
ikev2_redirect_flow (ikev2_add_ipsec_tunnel_args_t * a)
{
  ...
  // Fix ikev2 issue initiator where ip (.2 in test setup) is wrong
  if (flow.ip4_n_tuple.src_addr.addr.as_u8[3] == 2)
  {
    flow.ip4_n_tuple.src_addr.addr.as_u8[2] = flow.ip4_n_tuple.dst_addr.addr.as_u8[2];
  } else {
    flow.ip4_n_tuple.dst_addr.addr.as_u8[2] = flow.ip4_n_tuple.src_addr.addr.as_u8[2];
  }
  ...
  if (rv)
     return;
  rv = vnet_flow_enable (vnm, flow_index, 1);  // Assumption, hw_if_idx == 1
