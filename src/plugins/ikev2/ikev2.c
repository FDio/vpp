...
typedef enum
{
  IKEV2_ERROR_HANDOFF_CONGESTION_DROP = IKEV2_N_ERROR,
  IKEV2_NODE_N_ERROR,
} ikev2_node_error_t;

static vlib_error_desc_t ikev2_node_error_counters[] = {
  ...
  [IKEV2_ERROR_HANDOFF_CONGESTION_DROP] = {
    .name = "handoff_congestion_drop",
    .desc = "IKE packets dropped due to handoff congestion",
    .severity = VL_COUNTER_SEVERITY_ERROR,
  },
};
			   ...
			   udp_header_t *udp, u32 len, u32 sw_if_index)
{
  ...
  if (ip_addr_version (&sa->iaddr) == AF_IP4)
    log_debug ("ispi %lx rspi %lx IKE_INIT request received from %U",
	       clib_net_to_host_u64 (ike->ispi), clib_net_to_host_u64 (ike->rspi),
	       format_ip4_address, &ip_addr_v4 (&sa->iaddr));
  else
    log_debug ("ispi %lx rspi %lx IKE_INIT request received from %U",
	       clib_net_to_host_u64 (ike->ispi), clib_net_to_host_u64 (ike->rspi),
	       format_ip6_address, &ip_addr_v6 (&sa->iaddr));
  ...
  if (len < sizeof (*ike))
    {
      log_err ("IKE_INIT request too small");
      return 0;
    }
  ...
  while (p < len && payload != IKEV2_PAYLOAD_NONE)
      ...
      else if (payload == IKEV2_PAYLOAD_NOTIFY)
	{
	  ...
	  if (n->msg_type == IKEV2_NOTIFY_MSG_NAT_DETECTION_SOURCE_IP)
	      ...
	      if (clib_memcmp (src_sha, n->data, vec_len (src_sha)))
		{
		  ...
		  log_debug ("ispi %lx initiator behind NAT", sa->ispi);
		}
	      vec_free (src_sha);
	    }
	  else if (n->msg_type ==
	      ...
	      if (clib_memcmp (dst_sha, n->data, vec_len (dst_sha)))
		{
		  if (sa->natt_state == IKEV2_NATT_ENABLED)
		    sa->natt_state = IKEV2_NATT_ACTIVE;
		  log_debug ("ispi %lx responder (self) behind NAT", sa->ispi);
		}
	      vec_free (dst_sha);
	    }
	  vec_free (n);
	...
	{
	  log_err ("Unknown payload! type=%d", payload);
	  if (ikep->flags & IKEV2_PAYLOAD_FLAG_CRITICAL)
	    {
	      sa->state = IKEV2_STATE_NOTIFY_AND_DELETE;
	      log_debug ("ispi %lx SA state changed to NOTIFY_AND_DELETE", sa->ispi);
  ...
  log_debug ("ispi %lx SA state changed to SA_INIT", sa->ispi);
  return 1;
}
			    ...
			    udp_header_t * udp, u32 len)
{
  ...
  if (ip_addr_version (&sa->raddr) == AF_IP4)
    log_debug ("ispi %lx rspi %lx IKE_INIT response received from %U", sa->ispi, sa->rspi,
	       format_ip4_address, &ip_addr_v4 (&sa->raddr));
  else
    log_debug ("ispi %lx rspi %lx IKE_INIT response received from %U", sa->ispi, sa->rspi,
	       format_ip6_address, &ip_addr_v6 (&sa->raddr));
  ...
  if (sizeof (*ike) > len)
    {
      log_err ("IKE_INIT response too small");
      return;
    }
	  ...
	  if (n->msg_type == IKEV2_NOTIFY_MSG_NAT_DETECTION_SOURCE_IP)
	    {
	      ...
	      if (clib_memcmp (src_sha, n->data, vec_len (src_sha)))
		{
		  log_debug ("ispi %lx responder behind NAT, unsupported", sa->ispi);
		}
	      vec_free (src_sha);
	    }
	  else if (n->msg_type ==
	      ...
	      if (clib_memcmp (dst_sha, n->data, vec_len (dst_sha)))
		{
		  if (sa->natt_state == IKEV2_NATT_ENABLED)
		    sa->natt_state = IKEV2_NATT_ACTIVE;
		  log_debug ("ispi %lx initiator (self) behind NAT", sa->ispi);
		}
	      vec_free (dst_sha);
	    }
	  vec_free (n);
			  ...
			  u8 * payload, u32 rlen, u32 * out_len)
{
  ...
  if (last_payload != IKEV2_PAYLOAD_SK)
    {
      log_err ("Last payload must be SK");
      return 0;
    }
  ...
  if (is_aead)
	...
	return 0;
      ...
      int rc = ikev2_decrypt_aead_data (sa, tr_encr, ikep->payload, plen, aad, aad_len, tag, &dlen);
      if (rc)
	{
	  *out_len = dlen;
	...
	{
	  log_err ("message integrity check failed");
	  vec_free (hmac);
	  return 0;
	}
      vec_free (hmac);

      int rc = ikev2_decrypt_data (sa, tr_encr, ikep->payload, plen, &dlen);
      if (rc)
	{
	  *out_len = dlen;
...
static void
ikev2_initial_contact_cleanup_internal (ikev2_sa_t *sa)
{
  ikev2_main_t *km = &ikev2_main;
  ikev2_sa_t *tmp;
  u32 i, *delete = 0;
  ...
  pool_foreach (tmp, km->sas)
    {
      if (!ikev2_is_id_equal (&tmp->i_id, &sa->i_id) || !ikev2_is_id_equal (&tmp->r_id, &sa->r_id))
	continue;
  ...
  for (i = 0; i < vec_len (delete); i++)
    {
      ...
      {
	  ikev2_delete_tunnel_interface (vnet_get_main (), tmp, c);
      }
      ikev2_delete_sa (tmp);
    }
...
ikev2_initial_contact_cleanup (ikev2_sa_t *sa)
{
  ...
  ikev2_initial_contact_cleanup_internal (sa);
  sa->initial_contact = 0;
}
			...
			ike_header_t * ike, u32 len)
{
  ...
  if (ip_addr_version (&sa->raddr) == AF_IP4)
    log_debug ("ispi %lx rspi %lx EXCHANGE_IKE_AUTH received from %U",
	       clib_host_to_net_u64 (ike->ispi), clib_host_to_net_u64 (ike->rspi),
	       format_ip4_address,
	       sa->is_initiator ? &ip_addr_v4 (&sa->raddr) : &ip_addr_v4 (&sa->iaddr));
  else
    log_debug ("ispi %lx rspi %lx EXCHANGE_IKE_AUTH received from %U",
	       clib_host_to_net_u64 (ike->ispi), clib_host_to_net_u64 (ike->rspi),
	       format_ip6_address,
	       sa->is_initiator ? &ip_addr_v6 (&sa->raddr) : &ip_addr_v6 (&sa->iaddr));
...
malformed:
  ...
  log_debug ("ispi %lx SA state changed to DELETED: malformed IKE_AUTH", sa->ispi);
  return 0;
}
				 ...
				 ikev2_sa_t * sa, ike_header_t * ike, u32 len)
{
  ...
  if (ip_addr_version (&sa->iaddr) == AF_IP4)
    log_debug ("ispi %lx rspi %lx INFORMATIONAL received from %U", clib_host_to_net_u64 (ike->ispi),
	       clib_host_to_net_u64 (ike->rspi), format_ip4_address, &ip_addr_v4 (&sa->iaddr));
  else
    log_debug ("ispi %lx rspi %lx INFORMATIONAL received from %U", clib_host_to_net_u64 (ike->ispi),
	       clib_host_to_net_u64 (ike->rspi), format_ip6_address, &ip_addr_v6 (&sa->iaddr));
  ...
  while (p < dlen && payload != IKEV2_PAYLOAD_NONE)
      ...
      if (payload == IKEV2_PAYLOAD_NOTIFY)	/* 41 */
	{
	  ...
	  if (n->msg_type == IKEV2_NOTIFY_MSG_AUTHENTICATION_FAILED)
	    sa->state = IKEV2_STATE_AUTH_FAILED;
	  log_debug ("ispi %lx SA state changed to AUTH_FAILED", sa->ispi);
	  vec_free (n);
	}
				   ...
				   u32 len)
{
  ...
  if (sa->is_initiator)
    {
      if (ip_addr_version (&sa->raddr) == AF_IP4)
	log_debug ("ispi %lx rspi %lx CREATE_CHILD_SA received from %U",
		   clib_host_to_net_u64 (ike->ispi), clib_host_to_net_u64 (ike->rspi),
		   format_ip4_address, &ip_addr_v4 (&sa->raddr));
      else
	log_debug ("ispi %lx rspi %lx CREATE_CHILD_SA received from %U",
  ...
  else if (ip_addr_version (&sa->iaddr) == AF_IP4)
    log_debug ("ispi %lx rspi %lx CREATE_CHILD_SA received from %U",
	       clib_host_to_net_u64 (ike->ispi), clib_host_to_net_u64 (ike->rspi),
	       format_ip4_address, &ip_addr_v4 (&sa->iaddr));
  else
  ...
  if (sa->is_initiator)
      ...
      if (n)
	{
	  ...
	  if (!child_sa)
	    {
	      log_err ("child SA spi %lx not found", n->spi);
	      goto cleanup_and_exit;
	    }
...
ikev2_sa_match_ts (ikev2_sa_t * sa)
{
  ...
  if (tsi && tsr)
  ...
  else
    {
      vec_free (tsi);
      vec_free (tsr);
      sa->state = IKEV2_STATE_TS_UNACCEPTABLE;
      log_debug ("ispi %lx SA state changed to TS_UNACCEPTABLE", sa->ispi);
    }
}
  ...
  pool_foreach (p, km->profiles)
      ...
      if (sa_auth->method == IKEV2_AUTH_METHOD_SHARED_KEY_MIC)
	{
	  ...
	  if (!clib_memcmp (auth, sa_auth->data, vec_len (sa_auth->data)))
	    {
	    sa->state = IKEV2_STATE_AUTHENTICATED;
	    log_debug ("ispi %lx SA state changed to AUTHENTICATED", sa->ispi);
	    ...
	    {
	    log_err ("shared key mismatch! ispi %lx", sa->ispi);
	    }
	}
      else if (sa_auth->method == IKEV2_AUTH_METHOD_RSA_SIG)
	{
	  ...
	  if (ikev2_verify_sign (p->auth.key, sa_auth->data, authmsg) == 1)
	    ...
	    {
	    log_err ("cert verification failed! ispi %lx", sa->ispi);
	    }
	}
    }
  vec_free (authmsg);
...
ikev2_sa_auth (ikev2_sa_t *sa, vlib_main_t *vm)
{
  ...
  if (!(sa->i_auth.method == IKEV2_AUTH_METHOD_SHARED_KEY_MIC ||
    ...
    {
      log_err ("unsupported authentication method %u", sa->i_auth.method);
      sa->state = IKEV2_STATE_AUTH_FAILED;
      log_debug ("ispi %lx SA state changed to AUTH_FAILED", sa->ispi);
      return;
    }
  ...
  if (sel_p)
    ...
    {
      log_err ("authentication failed, no matching profile found! ispi %lx", sa->ispi);
      sa->state = IKEV2_STATE_AUTH_FAILED;
      log_debug ("ispi %lx SA state changed to AUTH_FAILED", sa->ispi);
    }
  vec_free (key_pad);
...
ikev2_add_tunnel_from_main (ikev2_add_ipsec_tunnel_args_t * a)
{
  ...
  if (~0 == a->sw_if_index)
    {
      ...
      if (!km->ipip_add_tunnel_fn_ptr)
	{
	  log_err ("ipip plugin is not loaded; function is not available");
	  return;
	}
      rv = ((__typeof__ (ipip_add_tunnel_fn_t)) km->ipip_add_tunnel_fn_ptr)
      (IPIP_TRANSPORT_IP4, ~0, &ip_addr_46 (&a->local_ip),
  ...
  if (rv)
    {
      log_err ("installing ipip tunnel failed! local spi: %x", a->local_spi);
      return;
    }
			       ...
			       u32 child_index, u8 is_rekey, u8 kex)
{
  ...
  if (!child->r_proposals)
    {
      ...
      log_debug ("ispi %lx SA state changed to NO_PROPOSAL_CHOSEN", sa->ispi);
      return 1;
    }
  ...
  if (thread_index & 0xffffffc0)
    log_err ("error: thread index exceeds max range 0x3f!");

  if (child_index & 0xfffff000 || sa_index & 0xfffff000)
    log_err ("error: sa/child index exceeds max range 0xfff!");
  ...
  vlib_rpc_call_main_thread (ikev2_add_tunnel_from_main, (u8 *) &a, sizeof (a));
  return 0;
}
...
ikev2_del_tunnel_from_main (ikev2_del_ipsec_tunnel_args_t * a)
{
  ...
  if (~0 == a->sw_if_index)
    ...
    if (!km->ipip_tunnel_db_find_fn_ptr || !km->ipip_del_tunnel_fn_ptr)
    {
      log_err ("ipip plugin is not loaded; function is not available");
      return;
    }
    ipip = ((__typeof__ (ipip_tunnel_db_find_fn_t)) km->ipip_tunnel_db_find_fn_ptr) (&key);
  ...
  if (ipip)
    ((__typeof__ (ipip_del_tunnel_fn_t)) km->ipip_del_tunnel_fn_ptr) (ipip->sw_if_index);
}
			       ...
			       ikev2_child_sa_t * child)
{
  ...
  vlib_rpc_call_main_thread (ikev2_del_tunnel_from_main, (u8 *) &a, sizeof (a));
  return 0;
}
...
static u32
ikev2_generate_message (vlib_main_t *vm, vlib_buffer_t *b, ikev2_sa_t *sa, ike_header_t *ike,
			void *user, udp_header_t *udp, ikev2_stats_t *stats)
{
  ...
  if (ike->exchange == IKEV2_EXCHANGE_SA_INIT)
	...
	{
	  sa->state = IKEV2_STATE_DELETED;
	  log_debug ("ispi %lx SA state changed to DELETED: unexpected IKE_AUTH", sa->ispi);
	  goto done;
	}
    }
	...
	{
	  sa->state = IKEV2_STATE_DELETED;
	  log_debug ("ispi %lx SA state changed to DELETED: auth failed", sa->ispi);
	  goto done;
	}
  ...
  if (ike->exchange == IKEV2_EXCHANGE_SA_INIT)
      ...
      if (tlen + b->current_length + b->current_data > buffer_data_size)
	{
      ...
      if (is_aead)
	{
	  if (!ikev2_encrypt_aead_data (sa, tr_encr, chain->data, ph->payload, (u8 *) ike,
	...
	{
	  if (!ikev2_encrypt_data (sa, tr_encr, chain->data, ph->payload))
	    {
	      tlen = ~0;
	      goto done;
	    }
	  ...
	  if (sa->state == IKEV2_STATE_SA_INIT)
	    {
	      ...
	      clib_memcpy_fast (ike->payload, tmp->payload,
				slen - sizeof (*ike));
	      if (ip_addr_version (&raddr) == AF_IP4)
		log_debug ("ispi %lx IKE_SA_INIT retransmit from %U to %U", ike->ispi,
			   format_ip4_address, &ip_addr_v4 (&raddr), format_ip4_address,
			   &ip_addr_v4 (&iaddr));
	      else
		log_debug ("ispi %lx IKE_SA_INIT retransmit from %U to %U", ike->ispi,
			   format_ip6_address, &ip_addr_v6 (&raddr), format_ip6_address,
			   &ip_addr_v6 (&iaddr));
	      return slen;
	    }
	  /* else ignore req */
	    ...
	    {
	      if (ip_addr_version (&raddr) == AF_IP4)
		log_debug ("ispi %lx IKE_SA_INIT ignore from %U to %U", ike->ispi,
			   format_ip4_address, &ip_addr_v4 (&raddr), format_ip4_address,
			   &ip_addr_v4 (&iaddr));
	      else
		log_debug ("ispi %lx IKE_SA_INIT ignore from %U to %U", ike->ispi,
  ...
  if (msg_id == sa->last_msg_id)
    {
      ...
      if (ip_addr_version (&sa->raddr) == AF_IP4)
      log_debug ("IKE retransmit msgid %d from %U to %U", msg_id, format_ip4_address,
		 &ip_addr_v4 (&sa->raddr), format_ip4_address, &ip_addr_v4 (&sa->iaddr));
      else
      log_debug ("IKE retransmit msgid %d from %U to %U", msg_id, format_ip6_address,
		 &ip_addr_v6 (&sa->raddr), format_ip6_address, &ip_addr_v6 (&sa->iaddr));
      return slen;
    }

  /* old req ignore */
  if (ip_addr_version (&sa->raddr) == AF_IP4)
    log_debug ("IKE req ignore msgid %d from %U to %U", msg_id, format_ip4_address,
	       &ip_addr_v4 (&sa->raddr), format_ip4_address, &ip_addr_v4 (&sa->iaddr));
  else
    log_debug ("IKE req ignore msgid %d from %U to %U", msg_id, format_ip6_address,
	       &ip_addr_v6 (&sa->raddr), format_ip6_address, &ip_addr_v6 (&sa->iaddr));
  return ~0;
}
...
{
  vlib_rpc_call_main_thread (ikev2_del_sa_init_from_main, (u8 *) &ispi, sizeof (ispi));
}
...
ikev2_generate_sa_init_data_and_log (ikev2_sa_t * sa)
{
  ...
  if (rc == IKEV2_GENERATE_SA_INIT_ERR_NO_DH)
    log_err (IKEV2_GENERATE_SA_INIT_OK_ERR_NO_DH_STR);
  else if (rc == IKEV2_GENERATE_SA_INIT_ERR_UNSUPPORTED_DH)
    log_err (IKEV2_GENERATE_SA_INIT_OK_ERR_UNSUPP_STR);
}
...
static uword
ikev2_node_internal (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 fq_index,
		     u8 is_ip4, u8 natt)
{
  ...
  if (PREDICT_FALSE (vm->thread_index != 0))
    {
      n_enq = vlib_buffer_enqueue_to_single_thread (
	vm, node, fq_index, vlib_frame_vector_args (frame), 0, frame->n_vectors);
      vlib_node_increment_counter (vm, node->node_index, IKEV2_ERROR_HANDOFF, n_enq);
      if (PREDICT_FALSE (n_enq < frame->n_vectors))
      vlib_node_increment_counter (vm, node->node_index, IKEV2_ERROR_HANDOFF_CONGESTION_DROP,
  ...
  while (n_left > 0)
	      ...
	      if (ike0->rspi == 0)
		{
		  ...
		  if (sa0->state == IKEV2_STATE_SA_INIT
		      || sa0->state == IKEV2_STATE_NOTIFY_AND_DELETE)
		    {
		      ...
		      slen = ikev2_generate_message (vm, b0, sa0, ike0, 0, udp0, stats);
		      if (~0 == slen)
			vlib_node_increment_counter (vm, node->node_index,
						     IKEV2_ERROR_NO_BUFF_SPACE,
						     1);
	  ...
	  if (p)
	      ...
	      if (sa0->del)
		{
		  if (sa0->del[0].protocol_id != IKEV2_PROTOCOL_IKE)
			...
			if (ch_sa)
			  {
			    ikev2_delete_tunnel_interface (vnet_get_main (), sa0, ch_sa);
			    if (!sa0->is_initiator)
			      {
				vec_add2 (resp, tmp, 1);
	  ...
	  if (p)
	      ...
	      else if (vec_len (sa0->new_child) > 0)
		{
		  ...
		  if (ike_hdr_is_request (ike0))
		    {
		      ike0->flags = IKEV2_HDR_FLAG_RESPONSE;
		      slen = ikev2_generate_message (vm, b0, sa0, ike0, 0, udp0, 0);
		      if (~0 == slen)
		  ...
		  if (!sa0->sa_rekey[0].notify_type)
		    {
		      ...
		      sa0 = tmp;
		      hash_set (km->sa_by_rspi, sa0->rspi, sa0 - km->sas);
		      p = hash_get (km->sa_by_rspi, sa0->rspi);
		      ikev2_complete_sa_rekey (sar, sa0, &sa0->sa_rekey[0]);
		      hash_set (km->sa_by_rspi, sar->rspi, sar - km->sas);
		    }
		  if (ike_hdr_is_response (ike0))
		    {
	...
	{
	  if (is_ip4)
	    log_warn ("IKEv2 exchange %d received from %U to %U", ike0->exchange,
		      format_ip4_address, &ip40->src_address, format_ip4_address,
		      &ip40->dst_address);
	  else
	    log_warn ("IKEv2 exchange %d received from %U to %U", ike0->exchange,
		      format_ip6_address, &ip60->src_address, format_ip6_address,
		      &ip60->dst_address);
	}
		  ...
		  sa0->state == IKEV2_STATE_NOTIFY_AND_DELETE))
	{
	  ...
	  vec_foreach (c, sa0->childs)
	    ikev2_delete_tunnel_interface (vnet_get_main (), sa0, c);
...
{
  ikev2_main_t *km = &ikev2_main;
  return ikev2_node_internal (vm, node, frame, km->handoff_ip4_fq_index, 1 /* is_ip4 */, 0);
}
...
{
  ikev2_main_t *km = &ikev2_main;
  return ikev2_node_internal (vm, node, frame, km->handoff_ip4_natt_fq_index, 1 /* is_ip4 */,
			      1 /* natt */);
}
...
{
  ikev2_main_t *km = &ikev2_main;
  return ikev2_node_internal (vm, node, frame, km->handoff_ip6_fq_index, 0 /* is_ip4 */, 0);
}
...
VLIB_REGISTER_NODE (ikev2_node_ip4,static) = {
  ...
  .n_errors = IKEV2_NODE_N_ERROR,
  .error_counters = ikev2_node_error_counters,
			       ...
			       ikev2_sa_proposal_t ** proposals, int is_ike)
{
  ...
  if (IKEV2_TRANSFORM_INTEG_TYPE_NONE != ts->integ_alg)
	...
	if (td->type == IKEV2_TRANSFORM_TYPE_INTEG
      ...
      if (error)
	{
	log_err ("Didn't find any supported algorithm for IKEV2_TRANSFORM_TYPE_INTEG");
	r = clib_error_return (0, "Unsupported algorithm");
	return r;
	}
    }
...
static void
ikev2_initiate_delete_ike_sa_internal (vlib_main_t *vm, ikev2_sa_t *sa, u8 send_notification)
{
  ikev2_main_t *km = &ikev2_main;
  ...
  if (send_notification)
      ...
      if (!bi0)
	{
      ...
      len = ikev2_generate_message (vm, b0, sa, ike0, 0, 0, 0);
      if (~0 == len)
	return;
...
delete_sa:
  /* delete local SA */
  vec_foreach (c, sa->childs)
    ikev2_delete_tunnel_interface (vnet_get_main (), sa, c);

  u64 rspi = sa->rspi;
  ikev2_sa_free_all_vec (sa);
  uword *p = hash_get (km->sa_by_rspi, rspi);
  if (p)
    {
      hash_unset (km->sa_by_rspi, rspi);
      pool_put (km->sas, sa);
    }
}
...
ikev2_cleanup_profile_sessions (ikev2_main_t * km, ikev2_profile_t * p)
{
  ...
  pool_foreach (sa, km->sas)
  {
    if (sa->profile_index != ~0 && pi == sa->profile_index)
      vec_add1 (del_sai, sa - km->sas);
  }

  vec_foreach (sai, del_sai)
  {
    sa = pool_elt_at_index (km->sas, sai[0]);
    ikev2_initiate_delete_ike_sa_internal (vlib_get_main (), sa, 1);
  }
...
ikev2_initiate_sa_init (vlib_main_t * vm, u8 * name)
{
  ...
  if (valid_ip)
	...
	log_debug ("ispi %lx rspi %lx IKEV2_EXCHANGE_SA_INIT sent to %U",
		   clib_host_to_net_u64 (sa0->ispi), 0, format_ip4_address,
		   &ip_addr_v4 (&p->responder.addr));
      else
	log_debug ("ispi %lx rspi %lx IKEV2_EXCHANGE_SA_INIT sent to %U",
		   clib_host_to_net_u64 (sa0->ispi), 0, format_ip6_address,
		   &ip_addr_v6 (&p->responder.addr));
    }
				...
				ikev2_child_sa_t * csa)
{
  ...
  ikev2_delete_tunnel_interface (vnet_get_main (), sa, csa);
...
ikev2_initiate_delete_ike_sa (vlib_main_t * vm, u64 ispi)
{
  ...
  ikev2_initiate_delete_ike_sa_internal (vm, fsa, 1);
  return 0;
}
			       ...
			       ikev2_child_sa_t * csa)
{
  ...
  len = ikev2_generate_message (vm, b0, sa, ike0, proposals, 0, 0);
  if (~0 == len)
    return;
...
ikev2_sa_del (ikev2_profile_t * p, u32 sw_if_index)
{
  ...
  vec_foreach (sap, sa_vec)
    {
      ikev2_initiate_delete_ike_sa_internal (vlib_get_main (), *sap, 0);
    }
  vec_free (sa_vec);
...
static u8
ikev2_mngr_process_child_sa (vlib_main_t *vm, ikev2_sa_t *sa, ikev2_child_sa_t *csa, u8 del_old_ids)
{
  ikev2_main_t *km = &ikev2_main;
  ikev2_profile_t *p = 0;
  ...
  if (sa->is_initiator && p && csa->time_to_expiration
      ...
      if (!csa->is_expired || csa->rekey_retries > 0)
	{
	  ...
	  if (csa->rekey_retries == 0)
	    ...
	    {
	      csa->rekey_retries--;
	      log_debug ("Rekeying Child SA 0x%x, retries left %d", csa->i_proposals->spi,
			 csa->rekey_retries);
	      if (csa->rekey_retries == 0)
		{
		  csa->rekey_retries = -1;
  ...
  if (del_old_ids)
      ...
      if (~0 == sw_if_index)
	{
	  ...
	  if (sa->is_initiator)
       ...
       if (!km->ipip_tunnel_db_find_fn_ptr)
       {
          log_err ("ipip plugin is not loaded; cannot find ipip tunnel");
          return res;
       }
	  ipip = ((__typeof__ (ipip_tunnel_db_find_fn_t)) km->ipip_tunnel_db_find_fn_ptr) (&key);
...
static void
ikev2_mngr_process_ipsec_sa (vlib_main_t *vm, ipsec_sa_t *ipsec_sa)
{
  ikev2_main_t *km = &ikev2_main;
  ikev2_sa_t *fsa = 0;
  ikev2_profile_t *p = 0;
				   ...
				   ikev2_sa_t *sa)
{
  ...
  ikev2_send_ike (vm, &sa->iaddr, &sa->raddr, bi0, vec_len (sa->last_sa_init_req_packet_data),
...
static void
ikev2_send_informational_request (vlib_main_t *vm, ikev2_sa_t *sa)
{
  ip_address_t *src, *dst;
  ike_header_t *ike0;
  vlib_buffer_t *b0;
  ...
  ikev2_send_ike (vm, src, dst, bi0, len, ikev2_get_port (sa), dp, sa->sw_if_index);
}
...
static int
ikev2_mngr_process_responder_sas (vlib_main_t *vm, ikev2_sa_t *sa)
{
  ikev2_main_t *km = &ikev2_main;
  ...
  if (sa->liveness_period_check < now)
    {
      ...
      ikev2_send_informational_request (vm, sa);
    }
  return 0;
}
  ...
  while (1)
      ...
      pool_foreach (sa, km->sas)
	{
	  ...
	  if (sa->state == IKEV2_STATE_SA_INIT)
		...
		vec_add1 (to_be_deleted, sa - km->sas);
	    }
	  ...
	  vec_foreach (c, sa->childs)
	    ikev2_mngr_process_child_sa (vm, sa, c, del_old_ids);

	  if (!km->dpd_disabled && ikev2_mngr_process_responder_sas (vm, sa))
	    vec_add1 (to_be_deleted, sa - km->sas);
	}
      ...
      vec_foreach (sai, to_be_deleted)
	{
      ...
      /* process ipsec sas */
      ipsec_sa_t *ipsec_sa;
      pool_foreach (ipsec_sa, im->sa_pool)
	{
	  ikev2_mngr_process_ipsec_sa (vm, ipsec_sa);
	}
...
ikev2_lazy_init (ikev2_main_t *km)
{
  ...
  km->evp_ctx = EVP_CIPHER_CTX_new ();
  km->hmac_ctx = HMAC_CTX_new ();
#else
  EVP_CIPHER_CTX_init (&km->_evp_ctx);
  km->evp_ctx = &km->_evp_ctx;
  HMAC_CTX_init (&(km->_hmac_ctx));
  km->hmac_ctx = &km->_hmac_ctx;
#endif
  ...
  km->ipip_add_tunnel_fn_ptr =
    vlib_get_plugin_symbol ("ipip_plugin.so", "ipip_add_tunnel");
  if (!km->ipip_add_tunnel_fn_ptr)
    log_err ("cannot load symbols from ipip plugin");

  km->ipip_del_tunnel_fn_ptr =
    vlib_get_plugin_symbol ("ipip_plugin.so", "ipip_del_tunnel");
  if (!km->ipip_del_tunnel_fn_ptr)
    log_err ("cannot load symbols from ipip plugin");

  km->ipip_tunnel_db_find_fn_ptr =
    vlib_get_plugin_symbol ("ipip_plugin.so", "ipip_tunnel_db_find");
  if (!km->ipip_tunnel_db_find_fn_ptr)
    log_err ("cannot load symbols from ipip plugin");
