...
ikev2_compute_nat_sha1 (u64 ispi, u64 rspi, u8 * ip, u32 ip_size, u16 port)
{
  ...
  clib_memcpy_fast (&buf[8 + 8], ip, ip_size);
  clib_memcpy_fast (&buf[8 + 8 + ip_size], &port, sizeof (port));
			    ...
			    udp_header_t * udp, u32 len)
{
  ...
  while (p < len && payload != IKEV2_PAYLOAD_NONE)
	...
	return;
      ...
      ikep = (ike_payload_header_t *) & ike->payload[p];
      u16 plen = clib_net_to_host_u16 (ikep->length);
      if (plen > current_length || plen < sizeof (*ikep))
	return;
			  ...
			  u8 * payload, u32 rlen, u32 * out_len)
{
  ...
  while (p < len &&
	 *payload != IKEV2_PAYLOAD_NONE && last_payload != IKEV2_PAYLOAD_SK)
    {
      ...
      if (plen < sizeof (*ikep) || plen + p > len)
	return 0;
  ...
  if (is_aead)
	...
	return 0;
      ...
      dlen = ikev2_decrypt_aead_data (ptd, sa, tr_encr, ikep->payload,
      ...
      if (clib_memcmp (hmac, &ikep->payload[plen], tr_integ->key_trunc))
	{
      ...
      dlen = ikev2_decrypt_data (ptd, sa, tr_encr, ikep->payload, plen);
      if (dlen)
	{
	  *out_len = dlen;
      ...
      if (ike0->exchange == IKEV2_EXCHANGE_SA_INIT)
	{
	  ...
	  if (ike0->flags & IKEV2_HDR_FLAG_INITIATOR)
	      ...
	      if (ike0->rspi == 0)
		{
		  if (is_ip4)
		    {
		      ip_address_set (&sa0->raddr, &ip40->dst_address,
				      AF_IP4);
		      ip_address_set (&sa0->iaddr, &ip40->src_address,
				      AF_IP4);
		    }
		  else
		    {
		      ip_address_set (&sa0->raddr, &ip60->dst_address,
				      AF_IP6);
		      ip_address_set (&sa0->iaddr, &ip60->src_address,
				      AF_IP6);
		    }
	  ...
	  else			//received sa_init without initiator flag
	    {
	      if (is_ip4)
		{
		  ip_address_set (&sa0->raddr, &ip40->src_address, AF_IP4);
		  ip_address_set (&sa0->iaddr, &ip40->dst_address, AF_IP4);
		}
	      else
		{
		  ip_address_set (&sa0->raddr, &ip60->src_address, AF_IP6);
		  ip_address_set (&sa0->iaddr, &ip60->dst_address, AF_IP6);
		}
	  ...
	  if (is_ip4)
	    {
	      ...
	      if (sa0->is_initiator)
		{
		  ip_address_copy_addr (&ip40->dst_address, &sa0->raddr);
		  ip_address_copy_addr (&ip40->src_address, &sa0->iaddr);
		}
	      else
		{
		  ip_address_copy_addr (&ip40->dst_address, &sa0->iaddr);
		  ip_address_copy_addr (&ip40->src_address, &sa0->raddr);
		}
	    }
	  else
	    {
	      ...
	      if (sa0->is_initiator)
		{
		  ip_address_copy_addr (&ip60->dst_address, &sa0->raddr);
		  ip_address_copy_addr (&ip60->src_address, &sa0->iaddr);
		}
	      else
		{
		  ip_address_copy_addr (&ip60->dst_address, &sa0->iaddr);
		  ip_address_copy_addr (&ip60->src_address, &sa0->raddr);
		}
	    }
...
ikev2_initiate_sa_init (vlib_main_t * vm, u8 * name)
{
  ...
  /* Create the Initiator Request */
  {
    ...
    nat_detection_sha1 =
      ikev2_compute_nat_sha1 (clib_host_to_net_u64 (sa.ispi),
			      clib_host_to_net_u64 (sa.rspi),
			      ip_addr_bytes (&p->responder.addr),
			      ip_address_size (&p->responder.addr),
