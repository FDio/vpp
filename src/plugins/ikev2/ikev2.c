...
ikev2_add_tunnel_from_main (ikev2_add_ipsec_tunnel_args_t * a)
{
  ...
  if (~0 == a->sw_if_index)
	...
	IPIP_TRANSPORT_IP4, ~0, &ip_addr_46 (&a->local_ip), &ip_addr_46 (&a->remote_ip), 0,
	TUNNEL_ENCAP_DECAP_FLAG_NONE, IP_DSCP_CS0, TUNNEL_MODE_P2P, &sw_if_index);
...
ikev2_lazy_init (ikev2_main_t *km)
{
  ...
  km->ipip_tunnel_db_find_fn_ptr = vlib_get_plugin_symbol ("ipip_plugin.so", "ipip_tunnel_db_find");
  if (!km->ipip_tunnel_db_find_fn_ptr)
    log_err ("cannot load symbols from ipip plugin");
