...
static void
send_sa (ikev2_sa_t * sa, vl_api_registration_t * reg,
	 vl_api_ikev2_sa_dump_t * mp, u32 thread_index, u32 sa_index)
{
  ...
  {
    rmp->thread_index = thread_index;
    rmp->sa_index = sa_index;
...
{
  vl_api_ikev2_sa_dump_t_endian (mp);
  ...
  vec_foreach (tkm, km->per_thread_data)
  {
    ...
    ({
      send_sa (sa, reg, mp, tkm - km->per_thread_data, sa - tkm->sas);
    }));
    /* *INDENT-ON* */
  }
}
...
static void
send_child_sa (ikev2_child_sa_t * child, vl_api_registration_t * reg,
	       vl_api_ikev2_child_sa_dump_t * mp, u32 child_sa_index)
{
  ...
  {
    rmp->child_sa_index = child_sa_index;
    rmp->child_sa.i_proposals_spi =
      child->i_proposals ? child->i_proposals[0].spi : 0;
    rmp->child_sa.r_proposals_spi =
...
{
  vl_api_ikev2_child_sa_dump_t_endian (mp);
  ikev2_main_t *im = &ikev2_main;
  ikev2_main_per_thread_data_t *tkm;
  ikev2_sa_t *sa;
  ...
  vec_foreach (child, sa->childs)
  {
    ...
    send_child_sa (child, reg, mp, child_sa_index);
  }
}

static void
send_ts (ikev2_ts_t * ts, vl_api_registration_t * reg,
...
static void
  ...
  (vl_api_ikev2_traffic_selector_dump_t * mp)
{
  vl_api_ikev2_traffic_selector_dump_t_endian (mp);
  ikev2_main_t *im = &ikev2_main;
  ikev2_main_per_thread_data_t *tkm;
  ikev2_sa_t *sa;
  ikev2_child_sa_t *child;
...
static void
send_nonce (u8 * nonce, vl_api_registration_t * reg,
...
{
  vl_api_ikev2_nonce_get_t_endian (mp);
  ikev2_main_t *im = &ikev2_main;
  ikev2_main_per_thread_data_t *tkm;
  ikev2_sa_t *sa;
  vl_api_registration_t *reg;
  ...
  sa = pool_elt_at_index (tkm->sas, mp->sa_index);
  send_nonce (mp->is_initiator ? sa->i_nonce : sa->r_nonce, reg, mp);
}
