...
cp_ts (vl_api_ikev2_ts_t * vl_api_ts, ikev2_ts_t * ts)
{
  ...
  ip4_address_encode ((ip4_address_t *) & ts->start_addr,
		      vl_api_ts->start_addr);
  ip4_address_encode ((ip4_address_t *) & ts->end_addr, vl_api_ts->end_addr);
}
	 ...
	 vl_api_ikev2_sa_dump_t * mp, u32 thread_index, u32 sa_index)
{
  ...
  REPLY_MACRO2_ZERO (VL_API_IKEV2_SA_DETAILS,
  {
    ...
    ip4_address_encode ((ip4_address_t *) & sa->iaddr, rmp->sa.iaddr);
    ip4_address_encode ((ip4_address_t *) & sa->raddr, rmp->sa.raddr);
    rmp->sa.ispi = sa->ispi;
    rmp->sa.rspi = sa->rspi;
