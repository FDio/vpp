...
abf_itf_attach_stack (abf_itf_attach_t * aia)
{
  ...
  fib_path_list_contribute_forwarding (ap->ap_pl,
				       (AF_IP4 == aia->aia_proto ?
					FIB_FORW_CHAIN_TYPE_UNICAST_IP4 :
					FIB_FORW_CHAIN_TYPE_UNICAST_IP6),
				       FIB_PATH_LIST_FWD_FLAG_COLLAPSE,
				       &via_dpo);

  dpo_stack_from_node ((AF_IP4 == aia->aia_proto ?
			abf_ip4_node.index :
  ...
  while (n_left > 0)
    {
      ...
      match_match_one (vm, b[0], app, now, &res);

      if (match_set_result_is_hit (&res))
	{
	  /*
	   * match:
	   *  follow the DPO chain
	   */
	  dpo_id_t **dpos = res.msr_user_ctx;

	  next[0] = dpos[res.msr_pos.msp_rule_index]->dpoi_next_node;
	  vnet_buffer (b[0])->ip.adj_index[VLIB_TX] =
	    dpos[res.msr_pos.msp_rule_index]->dpoi_index;
	  matches++;
	}
      else
	{
      ...
      if (PREDICT_FALSE (b[0]->flags & VLIB_BUFFER_IS_TRACED))
	{
	  ...
	  tr->next = next[0];
	  tr->result = res.msr_pos.msp_rule_index;
