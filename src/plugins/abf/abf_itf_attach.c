...
abf_setup_acl_lc(fib_protocol_t fproto, u32 sw_if_index)
{
  ...
  acl_plugin_set_acl_vec_for_context (abf_alctx_per_itf[fproto][sw_if_index], acl_vec);
  vec_free(acl_vec);
}
  ...
  if (1 == vec_len (abf_per_itf[fproto][sw_if_index]))
    {
      ...
      abf_alctx_per_itf[fproto][sw_if_index] = acl_plugin_get_lookup_context_index (abf_acl_user_id, sw_if_index, 0);
    }
  else
    {
  ...
  if (0 == vec_len (abf_per_itf[fproto][sw_if_index]))
    {
      ...
      acl_plugin_put_lookup_context_index (abf_alctx_per_itf[fproto][sw_if_index]);
		  ...
		  vlib_frame_t * frame, fib_protocol_t fproto)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  acl_plugin_fill_5tuple (lc_index, b0, (FIB_PROTOCOL_IP6 == fproto), 1, 0, &fa_5tuple0);

	  if (acl_plugin_match_5tuple (lc_index, &fa_5tuple0, (FIB_PROTOCOL_IP6 ==
                                                     fproto), &action, &match_acl_pos, &match_acl_index,
                                                    &match_rule_index,
                                                    &trace_bitmap) )
          {
		/*
...
abf_itf_bond_init (vlib_main_t * vm)
{
  ...
  abf_acl_user_id = acl_plugin_register_user_module("abp plugin", "sw_if_index", NULL);
