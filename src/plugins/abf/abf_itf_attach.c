...
/* *INDENT-ON* */
	...
	while (n_left_from > 0 && n_left_to_next > 0)
	  {
	     ...
	     */
	    acl_plugin_fill_5tuple (am, b0, (FIB_PROTOCOL_IP6 == fproto),
				    1, 0, &fa_5tuple0);

	    vec_foreach (aiai0, attachments0)
	    {
	      aia0 = abf_itf_attach_get (*aiai0);
	      if (acl_plugin_single_acl_match_5tuple (am, aia0->aia_acl,
						      &fa_5tuple0,
						      (FIB_PROTOCOL_IP6 ==
						       fproto), &action,
						      &match_acl_index,
  ...
  {
    .function = abf_input_ip4,.name = "abf-input-ip4",.vector_size =
      sizeof (u32),
  ...
  {
    .function = abf_input_ip6,.name = "abf-input-ip6",.vector_size =
      sizeof (u32),
      // .format_trace = format_acl_fa_trace,
      .type = VLIB_NODE_TYPE_INTERNAL,
      //.n_errors = ARRAY_LEN (acl_fa_error_strings),
  ...
  {
  .arc_name = "ip4-unicast",.node_name = "abf-input-ip4",.runs_after =
      VNET_FEATURES ("acl-plugin-in-ip4-fa"),};

  VNET_FEATURE_INIT (abf_ip6_feat, static) =
  {
  .arc_name = "ip6-unicast",.node_name = "abf-input-ip6",.runs_after =
      VNET_FEATURES ("acl-plugin-in-ip6-fa"),};
