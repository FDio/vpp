...
const static abf_itf_t ABF_ITF_INVALID = {
  .ai_app = MATCH_SET_APP_INVALID,
  .ai_hdl = ~0,
  .ai_set = INDEX_INVALID,
};

static abf_itf_t *abf_itfs[FIB_PROTOCOL_MAX];
...
static void
abf_match_set_update (fib_protocol_t fproto, u32 sw_if_index)
{
  ...
  vec_reset_length (ai->ai_actions);
  ...
  vec_foreach (aiai, ai->ai_attachments)
  {
    ...
    vec_add1 (ai->ai_actions, &aia->aia_dpo);
  }

  if (~0 == ai->ai_hdl)
    {
      ai->ai_hdl =
	match_set_list_add (ai->ai_set, &ai->ai_ml, 0, ai->ai_actions);

      ai->ai_app = match_set_apply (ai->ai_set,
	...
	match_set_list_replace (ai->ai_set, ai->ai_hdl,
				&ai->ai_ml, 0, ai->ai_actions);
      else
	{
	  match_set_list_del (ai->ai_set, &ai->ai_hdl);
	  vec_free (ai->ai_actions);
	}
    }
		...
		u32 policy_id, u32 priority, u32 sw_if_index)
{
   ...
   */
  vec_validate_init_empty (abf_itfs[fproto], sw_if_index, ABF_ITF_INVALID);
  ...
  if (1 == vec_len (ai->ai_attachments))
    {
      ...
      u8 *name = format (NULL, "abf-%U-%U",
			 ...
			 sw_if_index,
			 format_fib_protocol, fproto);
      ai->ai_set = match_set_create_and_lock (name, NULL);
  ...
  abf_match_set_update (fproto, sw_if_index);
...
static u8 *
format_abf_attach_actions (u8 * s, va_list * args)
{
  dpo_id_t **dpos = va_arg (*args, dpo_id_t **);
  u32 index = va_arg (*args, u32);

  s = format (s, "%U", format_dpo_id, dpos[index]);
    ...
    if (sw_if_index < vec_len(abf_itfs[fproto]))
      {
        ...
        vlib_cli_output(vm, " %U", format_match_list_w_action,
                        &ai->ai_ml, 3,
                        format_abf_attach_actions,
                        ai->ai_actions);
		  ...
		  vlib_frame_t * frame, fib_protocol_t fproto)
{
  ...
  while (n_left > 0)
    {
      ...
      app[0] = abf_itfs[fproto][sw_if_index0].ai_app;
      app++;
      n_left--;
      b++;
    }
  ...
  while (n_left > 0)
      ...
      if (MATCH_RESULT_MISS != res[0].msr_pos.msp_rule_index)
	{
	   ...
	   */
	  dpo_id_t **dpos = res[0].msr_user_ctx;

	  next[0] = dpos[res[0].msr_pos.msp_rule_index]->dpoi_next_node;
	  vnet_buffer (b[0])->ip.adj_index[VLIB_TX] =
	    dpos[res[0].msr_pos.msp_rule_index]->dpoi_index;
	  matches++;
	}
      else
	{
      ...
      if (PREDICT_FALSE (b[0]->flags & VLIB_BUFFER_IS_TRACED))
	{
	  ...
	  tr->next = next[0];
	  tr->result = res[0].msr_pos.msp_rule_index;
