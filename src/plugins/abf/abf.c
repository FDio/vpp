                   ...
                   const fib_route_path_t *rpaths)
{
  ...
  if (INDEX_INVALID == abfi)
  ...
  else
    {
      ...
      fib_path_list_child_remove(old_pl,
                                 abf->abf_sibling);
      abf->abf_sibling = ~0;
...
{
    return ((abf_t*)(((char*)node) -
                     STRUCT_OFFSET_OF(abf_t,
                                      abf_node)));
}
...
abf_back_walk_notify (fib_node_t *node,
                      fib_node_back_walk_ctx_t *ctx)
{
    /*
     ...
     */
    fib_walk_sync(abf_fib_node_type,
                  abf_get_index(abf),
                  ctx);
...
{
  abf_fib_node_type =
    fib_node_register_new_type (&abf_vft);
