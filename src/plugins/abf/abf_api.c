  ...
  for (pi = 0; pi < mp->details.n_paths; pi++)
    {
      ...
      if ((MPLS_LABEL_INVALID != ntohl(mp->details.paths[pi].next_hop_via_label)) &&
          (0 != mp->details.paths[pi].next_hop_via_label))
        {
          path.frp_proto = DPO_PROTO_MPLS;
          path.frp_local_label = ntohl(mp->details.paths[pi].next_hop_via_label);
      ...
      if (DPO_PROTO_IP4 == mp->details.paths[pi].next_hop_afi)
        {
            memcpy (&path.frp_addr.ip4,
                    mp->details.paths[pi].next_hop,
                    sizeof (path.frp_addr.ip4));
        }
      else
        {
            memcpy (&path.frp_addr.ip6,
                    mp->details.paths[pi].next_hop,
                    sizeof (path.frp_addr.ip6));
        }
      ...
      if (n_labels == 0)
        ...
        vec_add1 (label_stack, ntohl (mp->details.paths[pi].next_hop_out_label_stack[0]));
      else
        {
          u8 mi;
          vec_validate (label_stack, n_labels - 1);
          for (mi = 0; pi < n_labels; mi++)
            label_stack[mi] = ntohl (mp->details.paths[pi].next_hop_out_label_stack[mi]);
        }
  ...
  if (mp->is_add)
    {
      abf_policy_update(ntohl(mp->details.policy_id),
                        ntohl(mp->details.acl_index),
                        paths);
    }
  else
    {
      abf_policy_delete(ntohl(mp->details.policy_id),
                        paths);
    }
