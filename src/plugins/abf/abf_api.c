...
vl_api_abf_policy_add_del_t_handler (vl_api_abf_policy_add_del_t * mp)
{
  ...
  for (pi = 0; pi < mp->policy.n_paths; pi++)
	...
	.frp_proto = mp->policy.paths[pi].next_hop_afi,
	.frp_sw_if_index = ntohl (mp->policy.paths[pi].next_hop_sw_if_index),
	.frp_weight = mp->policy.paths[pi].next_hop_weight,
	.frp_preference = mp->policy.paths[pi].next_hop_preference,
      };
      mpls_label_t *label_stack = NULL;
      u32 n_labels;

      if ((MPLS_LABEL_INVALID !=
	   ntohl (mp->policy.paths[pi].next_hop_via_label))
	  && (0 != mp->policy.paths[pi].next_hop_via_label))
	{
      ...
      if (mp->policy.paths[pi].is_udp_encap)
	{
	  path_flags |= FIB_ROUTE_PATH_UDP_ENCAP;
	  path.frp_udp_encap_id = ntohl (mp->policy.paths[pi].next_hop_id);
	}
      path.frp_flags = path_flags;

      if (DPO_PROTO_IP4 == mp->policy.paths[pi].next_hop_afi)
	{
	  memcpy (&path.frp_addr.ip4,
		  mp->policy.paths[pi].next_hop, sizeof (path.frp_addr.ip4));
	}
      else
	{
	  memcpy (&path.frp_addr.ip6,
		  mp->policy.paths[pi].next_hop, sizeof (path.frp_addr.ip6));
	}

      n_labels = mp->policy.paths[pi].next_hop_n_out_labels;
      if (n_labels == 0)
	;
      else if (1 == n_labels)
	vec_add1 (label_stack,
		  ntohl (mp->policy.paths[pi].next_hop_out_label_stack[0]));
      else
	{
	  u8 mi;
	  ...
	  for (mi = 0; mi < n_labels; mi++)
	    label_stack[mi] =
	      ntohl (mp->policy.paths[pi].next_hop_out_label_stack[mi]);
	}
...
abf_policy_send_details (u32 api, void *args)
{
  ...
  {
    fp->next_hop_weight = api_rpath->rpath.frp_weight;
    fp->next_hop_preference = api_rpath->rpath.frp_preference;
    fp->next_hop_sw_if_index = htonl (api_rpath->rpath.frp_sw_if_index);
    fp->next_hop_n_out_labels = 0;
    fp->next_hop_afi = copy_fib_next_hop (api_rpath, fp->next_hop);
    fp++;
  }
