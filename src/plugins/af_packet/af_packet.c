...
static void
af_packet_cleanup_q_and_rings (af_packet_if_t *apif)
{
  af_packet_queue_t *rx_queue;
  af_packet_queue_t *tx_queue;
...
af_packet_delete_if (u8 *host_if_name)
{
  ...
  af_packet_cleanup_q_and_rings (apif);
...
af_packet_device_reinit (af_packet_if_t *apif)
{
  ...
  vlib_log_info (apm->log_class, "af_packet_%s reinitialized",
		 apif->host_if_name);
