...
format_soft_rss_if (u8 *s, va_list *args)
{
  ...
  for (soft_rss_rt_match_t *m = rt->match; m < rt->match + rt->n_match; m++)
    s = format (s, "  mask %U match %U key-offset %u key-length %u\n",
		format_hex_bytes, &m->mask, sizeof (m->mask), format_hex_bytes,
		&m->match, sizeof (m->match), m->key_start, m->key_len);
