			      ...
			      vlib_buffer_t *b, u32 esp_seq, u16 block_number)
{
  ...
  if (PREDICT_FALSE (n_trace))
    {
      if (vlib_trace_buffer (vm, node, next_index, b, /* follow_chain */ 0))
	{
...
iptfs_decap_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (b < eb)
	...
	/* --------------------------------- */
	if (sa_index != iptfs_decap_get_ipsec_sa_index (b0))
	  {
	    sa_index = iptfs_decap_get_ipsec_sa_index (b0);
	     ...
	     * only done on SA switch, in the meantime we are using prefetches
	...
	while (b0->current_length)
	  {
		...
		*freebi++ = from[b - bufs];
		...
		if (!iptfs_decap_append_data_inp (vm, thread_index, sa_index, satd, b0, plen, plen,
		...
		else
		  {
		    ...
		    if (!(newb->flags & VLIB_BUFFER_IS_TRACED))
		      iptfs_decap_trace_new_buffer (vm, node, nexti, newb,
						    (iptfs_decap_get_seq64 (b0) & 0xFFFFFFFF),
						    block_number);
		...
		satd->tfs_rx.frag_nseq = iptfs_decap_get_seq64 (b0) + 1;
		...
		*to[nexti]++ = from[b - bufs];
		...
		if (!(b0->flags & VLIB_BUFFER_IS_TRACED))
		  iptfs_decap_trace_new_buffer (
		    vm, node, nexti, b0, (iptfs_decap_get_seq64 (b0) & 0xFFFFFFFF), block_number);
		goto pktdone;
	      }
