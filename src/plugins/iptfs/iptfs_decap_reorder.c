		      ...
		      vlib_buffer_t **tobufs, u64 clock_now)
{
  ...
  while (b < eb)
	...
	iptfs_sa_data_t *satd = iptfs_get_sa_data (iptfs_decap_get_ipsec_sa_index (b0));
	/*
	 * Assemble the sequence number from the 32-bit pieces left by
	 * esp_decrypt.
	 */
	u64 b0_esp_seq = iptfs_decap_get_seq64 (b0) =
	  ((u64) iptfs_decap_get_ipsec_seq_hi (b0) << 32) | iptfs_decap_get_ipsec_seq (b0);
	u8 winlen;
	u8 checkrecurse;
	...
	for (; rent < erent; rent++)
	  {
	    ...
	    *to++ = vlib_get_buffer (vm, last);
	    iptfs_pkt_debug_s (dbg, "%s: 3b) sending %llu from window next was %llu now %llu",
			       __FUNCTION__, iptfs_decap_get_seq64 (vlib_get_buffer (vm, last)),
...
iptfs_decap_reorder_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  for (b = bufs, eb = b + n_vectors; b < eb; b++)
      ...
      if (b + 1 < eb)
	{
      ...
      u32 new_sa_index = iptfs_decap_get_ipsec_sa_index (b0);
      ...
      if (satd->tfs_cc)
	{
	  ...
	  satd->tfs_rx.recv_seq = iptfs_decap_get_seq64 (b0);
	}
