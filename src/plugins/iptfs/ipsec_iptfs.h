 ...
 *
 *	Decap field set from ipsec's esp_post_data so we only have
 *	to assemble the 64-bit value from two 32-bit values once:
 *	(decap path)		u64 iptfs_esp_seq
 *
 * IPTFS->IPSEC:
 *	iptfs sets ipsec.sad_index
 *
 * IPSEC->IPTFS:
 *	OLD: (these fields are in vnet_buffer_opaque_t but no longer valid)
 *	ipsec sets u64 ipsec.esp_seq
 *	ipsec sets u32 ipsec.sad_index
 *
 *	NEW (260916): (these fields are in vnet_buffer_opaque_t)
 *	ipsec sets u32 esp_post_data (b)->decrypt_data.seq_hi
 *	ipsec sets u32 esp_post_data (b)->decrypt_data.seq
 *	ipsec sets u32 esp_post_data (b)->decrypt_data.sa_index
 */
typedef struct
{
  union
  {
    ...
    /* assembled from esp_post_data (b)->decrypt_data */
    struct
    {
      u64 iptfs_esp_seq;
    } decap;
...
STATIC_ASSERT (sizeof (ipsec_private_data_t) <= STRUCT_SIZE_OF (vnet_buffer_opaque2_t, unused),
	       "Custom iptfs private data too large for vnet_buffer_opaque2_t");
...
/*
 * esp_decrypt communicates sa_index and sequence number in its
 * async decrypt data area.
 */
#define iptfs_decap_get_ipsec_sa_index(b) (esp_post_data (b)->decrypt_data.sa_index)
#define iptfs_decap_get_ipsec_seq(b)	  (esp_post_data (b)->decrypt_data.seq)
#define iptfs_decap_get_ipsec_seq_hi(b)	  (esp_post_data (b)->decrypt_data.seq_hi)

/* iptfs decap assembles the 64-bit sequence and puts it here */
#define iptfs_decap_get_seq64(b) (iptfs_private (b)->decap.iptfs_esp_seq)
