...
static int
iptfs_check_support (ipsec_sa_t *sa, void *tfs_config, clib_error_t **pce)
{
  ...
  if (!ipsec_sa_is_set_IS_TUNNEL (sa))
    {
      if (pce)
	*pce = clib_error_return (0, "No IPTFS IPsec transport mode");
  ...
  if (!ipsec_sa_is_set_IS_INBOUND (sa))
      ...
      if (sa->tfs_type == IPSEC_SA_TFS_TYPE_IPTFS_CC)
	{
	  if (sa->id == conf->tfs_inbound_sa_id)
	    {
	      if (pce)
		*pce = clib_error_return (0, "Cannot run iptfs-cc mode w/o valid paired inbound SA "
	...
	{
	  if (pce)
	    *pce = clib_error_return (0, "configuration absent or byterate missing");
...
static int
iptfs_add_del_sa (u32 sa_index, void *tfs_config, u8 is_add, clib_error_t **pce)
{
  if (!iptfs_is_sa_index_possible_IPTFS (sa_index))
    {
      if (pce)
	*pce = clib_error_return (0, "invalid sa_index");
  ...
  if (is_add)
    {
      ...
      int rv = iptfs_check_support (_sa, tfs_config, pce);
      if (rv)
	return rv;
    }
