		    ...
		    int do_flush, flowperpkt_variant_t which)
{
  ...
  if (PREDICT_TRUE (do_flush == 0))
	...
	{
	  ethernet_header_t *eth = vlib_buffer_get_current (data_b);
	  offset += flowperpkt_l2_add (b0, eth, offset);
	}
      if (collect_ip6)
	{
	  ip6_header_t *ip = (ip6_header_t *)
	    ((u8 *) vlib_buffer_get_current (data_b) +
	     vnet_buffer (data_b)->ip.save_rewrite_length);
	  offset += flowperpkt_l3_ip6_add (b0, ip, offset);
	}
      if (collect_ip4)
	{
	  ip4_header_t *ip = (ip4_header_t *)
	    ((u8 *) vlib_buffer_get_current (data_b) +
	     vnet_buffer (data_b)->ip.save_rewrite_length);
	  offset += flowperpkt_l3_ip4_add (b0, ip, offset);
	}
