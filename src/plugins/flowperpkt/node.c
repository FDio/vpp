		       ...
		       u16 offset, u64 now, u16 length)
{
  ...
  /* Timestamp */
  u64 timestamp = clib_host_to_net_u64 (now);
  clib_memcpy (to_b->data + offset, &timestamp, sizeof (u64));
  offset += sizeof (u64);

  /* pkt size */
  u16 pkt_size = clib_host_to_net_u16 (length);
  clib_memcpy (to_b->data + offset, &pkt_size, sizeof (u16));
  offset += sizeof (u16);

  /* packet delta count */
  u64 packetdelta = clib_host_to_net_u64 (1);
...
flowperpkt_l3_ip6_add (vlib_buffer_t * to_b, ip6_header_t * ip, u16 offset)
{
  ...
  /* octetDeltaCount */
  u16 len = clib_net_to_host_u16 (ip->payload_length) + sizeof (ip6_header_t);
  u64 octetdelta = clib_host_to_net_u64 (len);
...
flowperpkt_l3_ip4_add (vlib_buffer_t * to_b, ip4_header_t * ip, u16 offset)
{
  ...
  /* octetDeltaCount */
  u64 octetdelta = clib_host_to_net_u64 (clib_net_to_host_u16 (ip->length));
  clib_memcpy (to_b->data + offset, &octetdelta, sizeof (u64));
  offset += sizeof (u64);
 ...
 * @param timestamp u64 timestamp, milliseconds since 1/1/70
		    ...
		    flowperpkt_variant_t which)
{
  ...
  now = (u64) ((vlib_time_now (vm) - fm->vlib_time_0) * 1e3);
  now += fm->millisecond_time_0;
