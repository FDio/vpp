...
static void flowperpkt_export_entry (vlib_main_t * vm, flowperpkt_entry_t * e);
...
add_to_flow_record_state (vlib_main_t * vm, vlib_node_runtime_t * node,
			  ...
			  flowperpkt_variant_t which, flowperpkt_trace_t * t)
{
  if (fm->disabled)
    return;
  ...
  flowperpkt_entry_t *e = 0;

  if (fm->active_timer > 0)
    e = flowperpkt_lookup (my_cpu_number, &k, &poolindex, &collision);
  if (collision)
...
flowperpkt_get_buffer (vlib_main_t * vm, flowperpkt_variant_t which)
{
  ...
  if (PREDICT_FALSE (b0 == 0))
	...
	{
	  vlib_node_increment_counter (vm, flowperpkt_l2_node.index,
				       FLOWPERPKT_ERROR_BUFFER, 1);
	  return 0;
	}
...
flowperpkt_export_entry (vlib_main_t * vm, flowperpkt_entry_t * e)
{
  ...
  if (offset < flowperpkt_get_headersize ())
    offset = flowperpkt_get_headersize ();
