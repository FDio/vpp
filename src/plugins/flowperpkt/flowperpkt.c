...
static inline u32
flowperpkt_template_ip4_field_count (void)
{
  return 3;
}
...
static inline u32
flowperpkt_template_ip6_field_count (void)
{
  return 3;
}
...
static inline u32
flowperpkt_template_l2_field_count (void)
{
  return 3;
}
...
static inline u32
flowperpkt_template_common_field_count (void)
{
  return 4;
}
...
static int
flowperpkt_template_add (u32 domain_id, u16 src_port,
...
static int
flowperpkt_tx_interface_add_del_feature (flowperpkt_main_t * fm, u32 sw_if_index, int which, bool record_l2, bool record_l3, bool record_l4, int is_add	/* Delete lacks testing */
  )
{
  int rv;
  ...
  if (which == FLOW_VARIANT_L2)
    {
      if (record_l3)
	{
	  rv = flowperpkt_template_add (1, UDP_DST_PORT_ipfix, flags,
  ...
  (vl_api_flowperpkt_tx_interface_add_del_t * mp)
{
  ...
  rv = flowperpkt_tx_interface_add_del_feature (fm, sw_if_index, mp->which,
						mp->record_l2, mp->record_l3,
						mp->record_l4, mp->is_add);
out:
						    ...
						    vlib_cli_command_t * cmd)
{
  ...
  rv = flowperpkt_tx_interface_add_del_feature (fm, sw_if_index, which,
						record_l2, record_l3,
						record_l4, is_add);
  switch (rv)
    {
...
VLIB_CLI_COMMAND (flowperpkt_enable_disable_command, static) = {
    ...
    "record [l2,l3_ip4|l3_ip6,l4] [disable]",
