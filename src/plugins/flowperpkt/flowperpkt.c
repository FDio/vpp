      ...
      if (which == FLOW_VARIANT_L2)
	{
	  ...
	  if (fm->record & FLOW_RECORD_L3 || fm->record & FLOW_RECORD_L4)
		...
		(is_add) ? template_id : 0;
	      rv =
		flowperpkt_template_add_del (1, UDP_DST_PORT_ipfix, flags,
					     flowperpkt_data_callback_l2,
  ...
  (vl_api_flowperpkt_tx_interface_add_del_t * mp)
{
  ...
  if (fm->record == 0)
    {
      clib_warning ("Please specify flowperpkt params record first...");
			      ...
			      vlib_cli_command_t * cmd)
{
  ...
  if (flowperpkt_params (fm, record_l2, record_l3, record_l4,
