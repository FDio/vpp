...
static int
validate_feature_on_interface (flowperpkt_main_t * fm, u32 sw_if_index,
			       u8 which)
{
  vec_validate_init_empty (fm->flow_per_intface, sw_if_index, ~0);

  if (fm->flow_per_intface[sw_if_index] == (u8) ~ 0)
    return -1;
  else if (fm->flow_per_intface[sw_if_index] != which)
    return 0;
  else
    return 1;
}
					 ...
					 u8 which, int is_add)
{
  ...
  fm->flow_per_intface[sw_if_index] = (is_add) ? which : (u8) ~ 0;
		   ...
		   u32 active_timer, u32 passive_timer)
{
   ...
   */
  fm->active_timer =
    (active_timer == (u32) ~ 0 ? FLOWPERPKT_TIMER_ACTIVE : active_timer);
  fm->passive_timer =
    (passive_timer == (u32) ~ 0 ? FLOWPERPKT_TIMER_PASSIVE : passive_timer);
						    ...
						    vlib_cli_command_t * cmd)
{
  ...
  if (fm->record == 0)
    return clib_error_return (0,
			      "Please specify flowperpkt params record first...");

  if (sw_if_index == ~0)
    return clib_error_return (0, "Please specify an interface...");

  if (which == (u8) ~ 0)
    return clib_error_return (0, "Please specify datapath (l2|ip4|ip6)...");
