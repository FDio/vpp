		   ...
		   u32 active_timer, u32 passive_timer)
{
   ...
   * $$$$: Add a way to disable the timer process
   */
  fm->active_timer = active_timer == ~0 ? FLOWPERPKT_TIMER_ACTIVE : 0;
  fm->passive_timer = passive_timer == ~0 ? FLOWPERPKT_TIMER_PASSIVE : 0;
			      ...
			      vlib_cli_command_t * cmd)
{
  ...
  u32 active_timer = FLOWPERPKT_TIMER_ACTIVE;
  u32 passive_timer = FLOWPERPKT_TIMER_PASSIVE;
  ...
  clib_warning ("Active %d, Passive %d\n", active_timer, passive_timer);
