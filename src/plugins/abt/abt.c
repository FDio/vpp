...
abt_attach (u32 sw_if_index, fib_protocol_t fproto, u32 * acls)
{
  ...
  if (INDEX_INVALID == ati)
    {
      /*
       * construct a new attachemnt object
       */
      ...
      at->at_lc_index =
	acl_plugin.get_lookup_context_index (abt_acl_user_id, sw_if_index, 0);
  ...
  rv = acl_plugin.set_acl_vec_for_context (at->at_lc_index, at->at_acls);

  if (rv)
    {
      clib_warning ("Error setting the vector of ACLs: %U",
...
abt_detach (u32 sw_if_index, fib_protocol_t fproto)
{
  ...
  acl_plugin.put_lookup_context_index (at->at_lc_index);
...
abt_cmd (vlib_main_t * vm, unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	fproto = FIB_PROTOCOL_IP6;
      else if (unformat (input, "%d", &acl_index))
...
VLIB_CLI_COMMAND (abt_cmd_node, static) = {
  .path = "abt attach",
  .function = abt_cmd,
  .short_help = "abt attach <ip4|ip6> [del] <value> <interface>",
};
VLIB_CLI_COMMAND (abt_trace_cmd_node, static) = {
  .path = "trace acl",
  .function = abt_cmd,
  .short_help = "trace acl <ip4|ip6> [del] <value> <interface>",
};
/* *INDENT-ON* */
...
abt_init (vlib_main_t * vm)
{
  ...
  abt_acl_user_id =
    acl_plugin.register_user_module ("ABT plugin", "sw_if_index", NULL);
