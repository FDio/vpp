        ...
        tests = [
            {
                ...
                'send': IP(src=self.pg0.remote_ip4, dst='10.10.10.10')/UDP(dport=6871),
                'reply': IP(src=self.pg0.remote_ip4, dst=self.pg1.remote_ip4)/UDP(dport=6871)
            },
            {
                'input': False,
                ...
                'send': IP(src=self.pg0.remote_ip4, dst=self.pg1.remote_ip4)/UDP(dport=6871),
                'reply': IP(src='11.11.11.11', dst=self.pg1.remote_ip4)/UDP(dport=6871)
            },
            {
                'input': True,
                'sw_if_index': self.pg0.sw_if_index,
                ...
                'rewrite': {'mask': 0xa, 'dst': self.pg1.remote_ip4,
                            'dport': 5555},
                'send': IP(src=self.pg0.remote_ip4, dst='10.10.10.10')/UDP(dport=6871),
                'reply': IP(src=self.pg0.remote_ip4, dst=self.pg1.remote_ip4)/UDP(dport=5555)
            },
            {
                'input': True,
                'sw_if_index': self.pg0.sw_if_index,
