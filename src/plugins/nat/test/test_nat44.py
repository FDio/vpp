...
class TestNAT44(MethodHolder):
    ...
    def tearDown(self):
        ...
        if not self.vpp_dead:
            self.vclient.nat_ipfix_enable_disable(
    ...
    def test_dynamic(self):
        ...
        # out2in
        tcpn = self.vclient.statistics.get_counter(
            '/nat44/out2in/slowpath/tcp')[0]
        udpn = self.vclient.statistics.get_counter(
            '/nat44/out2in/slowpath/udp')[0]
        icmpn = self.vclient.statistics.get_counter(
            '/nat44/out2in/slowpath/icmp')[0]
        drops = self.vclient.statistics.get_counter(
            '/nat44/out2in/slowpath/drops')[0]
    ...
    def test_inside_overlapping_interfaces(self):
        ...
        sessions = self.vclient.nat44_user_session_dump(
    ...
    def test_vrf_mode(self):
        ...
        try:
            ...
            self.vclient.ip_table_add_del(
                is_add=0, table={'table_id': vrf_id1})
            self.vclient.ip_table_add_del(
                is_add=0, table={'table_id': vrf_id2})
    ...
    def test_ha_send(self):
        ...
        self.vclient.nat_ha_set_listener(ip_address=self.pg3.local_ip4,
                                         port=12345,
                                         path_mtu=512)
        self.vclient.nat_ha_set_failover(
            ip_address=self.pg3.remote_ip4,
            port=12346,
            session_refresh_interval=10)
