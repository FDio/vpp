...
class TestNAT44EDOutput(VppTestCase):
    ...
    def test_icmp_overrun(self):
        """ more ICMPs than max sessions """

        pkts = [Ether(src=self.pg1.remote_mac, dst=self.pg1.local_mac) /
                IP(src=self.pg1.remote_ip4, dst=self.pg1.local_ip4) /
                ICMP(id=id)
                for id in range(1, self.max_sessions+2)]

        assert len(pkts) > self.max_sessions
        ...
        rx = self.send_and_expect(self.pg1, pkts, self.pg1, self.max_sessions)
        for (recvd, sent) in zip(rx, pkts):
            self.assertEqual(recvd[ICMP].id, sent[ICMP].id)
    ...
    def test_static_dynamic(self):
        ...
        assert p[IP].src == outside_addr
        assert p[TCP].sport == local_sport
