...
class TestNAT44EDOutput(VppTestCase):
    ...
    def test_output_ttl_exceeded(self):
        '''Send TTL exceeded to existing session (traceroute)'''
        ...
        p4 = p_ether / p_ip4 / UDP(sport=1234, dport=4321)
        ...
        p_ether = Ether(src=self.pg1.remote_mac, dst=self.pg1.local_mac)
        p_ip4 = IP(src=self.pg1.remote_ip4, dst=self.pg0.remote_ip4)
        p4 = p_ether / p_ip4 / ICMP(type=11, code=0) / rx[0][IP]
        print('ICMP ERROR TO SEND:')
        p4.show2()
        print('------')

        rx = self.send_and_expect(self.pg1, p4*1, self.pg0)
        self.assertEqual(len(rx), 1)
        print('RX:')
        rx[0].show2()
