...
static void create_bypass_for_fwd (snat_main_t * sm, vlib_buffer_t * b,
				   ...
				   u32 thread_index, nat_6t_flow_t ** o2if);
			  ...
			  u32 thread_index, snat_session_t ** s_p)
{
  ...
  nat_6t_flow_translate_buf (sm, b, ip, o2if, proto);
				      ...
				      nat_6t_flow_t ** o2if_p)
{
  ...
  *o2if_p = o2if = nat_6t_flow_alloc (sm, thread_index, s, s->ext_host_addr,
				      s->ext_host_port, o2i_addr, o2i_port,
				      o2i_fib_index, ip->protocol);
  nat_6t_flow_daddr_rewrite_set (o2if, i2o_addr.as_u32);
  nat_6t_flow_txfib_rewrite_set (o2if, i2o_fib_index);
  if (NAT_PROTOCOL_ICMP != nat_proto)
    {
      nat_6t_flow_dport_rewrite_set (o2if, i2o_port);
    }

  nat_6t_flow_to_ed_kv (tsm, o2if, &kv);

  if (clib_bihash_add_del_16_8 (&sm->flow_hash, &kv, 1))
    {
      // FIXME !!!
  ...
  if (twice_nat == TWICE_NAT || (twice_nat == TWICE_NAT_SELF &&
      ...
      if (rc)
	{
      ...
      nat_6t_flow_saddr_rewrite_set (o2if, s->ext_host_nat_addr.as_u32);
      if (NAT_PROTOCOL_ICMP != nat_proto)
	{
	  nat_6t_flow_sport_rewrite_set (o2if, s->ext_host_nat_port);
	}

      i2of = nat_6t_flow_alloc (sm, thread_index, s, i2o_addr, i2o_port,
				s->ext_host_nat_addr,
				s->ext_host_nat_port, i2o_fib_index,
				ip->protocol);
      nat_6t_flow_daddr_rewrite_set (i2of, s->ext_host_addr.as_u32);
      if (NAT_PROTOCOL_ICMP != nat_proto)
	{
	  nat_6t_flow_dport_rewrite_set (i2of, s->ext_host_port);
	}
      nat_6t_flow_to_ed_kv (tsm, i2of, &kv);
    }
  else
    {
      i2of = nat_6t_flow_alloc (sm, thread_index, s, i2o_addr, i2o_port,
  ...
  nat_6t_flow_saddr_rewrite_set (i2of, o2i_addr.as_u32);
  if (NAT_PROTOCOL_ICMP != nat_proto)
    {
      nat_6t_flow_sport_rewrite_set (i2of, o2i_port);
    }
		       ...
		       nat_6t_flow_t ** o2if)
{
  ...
  if (!clib_bihash_search_16_8 (&sm->flow_hash, &kv, &value))
      ...
      if (proto == NAT_PROTOCOL_OTHER)
	{
      ...
      f = nat_6t_flow_alloc (sm, thread_index, s, ip->dst_address, l_port,
			     ip->src_address, r_port, rx_fib_index,
			     ip->protocol);
      nat_6t_flow_txfib_rewrite_set (f, rx_fib_index);
      nat_6t_flow_to_ed_kv (tsm, f, &kv);
      if (clib_bihash_add_del_16_8 (&sm->flow_hash, &kv, 1))
	{
	  // FIXME !!
					...
					vlib_node_runtime_t * node)
{
  ...
  f = nat_6t_flow_alloc (sm, thread_index, s, ip->dst_address, 0,
			 ip->src_address, 0, m->fib_index, ip->protocol);
  nat_6t_flow_saddr_rewrite_set (f, ip->dst_address.as_u32);
  ...
  f = nat_6t_flow_alloc (sm, thread_index, s, ip->src_address, 0,
			 ip->dst_address, 0, rx_fib_index, ip->protocol);
  nat_6t_flow_daddr_rewrite_set (f, m->local_addr.as_u32);
  nat_6t_flow_txfib_rewrite_set (f, m->fib_index);
  nat_6t_flow_to_ed_kv (tsm, f, &s_kv);
  if (clib_bihash_add_del_16_8 (&sm->flow_hash, &s_kv, 1))
    {
      // FIXME !!
      nat_elog_notice ("out2in flow hash add failed");
  ...
  nat_6t_flow_translate_buf (sm, b, ip, f,
			     ip_proto_to_nat_proto (ip->protocol));
					  ...
					  int is_multi_worker)
{
  ...
  while (n_left_from > 0)
    {
      vlib_buffer_t *b0;
      u32 sw_if_index0, rx_fib_index0, proto0;
      ip4_header_t *ip0;
      udp_header_t *udp0;
      tcp_header_t *tcp0;
      snat_session_t *s0 = 0;
      ...
      if (PREDICT_FALSE (proto0 == NAT_PROTOCOL_ICMP))
	{
	  ...
	  PREDICT_TRUE (!pool_is_free_index
			(tsm->flows,
			 vnet_buffer2 (b0)->nat.ed_out2in_nat_flow_index)))
	{
	  f = pool_elt_at_index (tsm->flows,
				 vnet_buffer2 (b0)->
				 nat.ed_out2in_nat_flow_index);
	  if (PREDICT_TRUE
	      (f->match.daddr.as_u32 == ip0->dst_address.as_u32
	       && f->match.dport == vnet_buffer (b0)->ip.reass.l4_dst_port
	       && f->match.proto == ip0->protocol
	       && f->match.fib_index == rx_fib_index0
	       && f->match.saddr.as_u32 == ip0->src_address.as_u32
	       && f->match.sport == vnet_buffer (b0)->ip.reass.l4_src_port))
	    {
      ...
      if (clib_bihash_search_16_8 (&sm->flow_hash, &kv0, &value0))
	{
      ...
      f = pool_elt_at_index (tsm->flows, ed_value_get_flow_index (&value0));

    skip_lookup:
      s0 = pool_elt_at_index (tsm->sessions, f->session_idx);
					  ...
					  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      if (!clib_bihash_search_16_8 (&sm->flow_hash, &kv0, &value0))
	{
	  ...
	  s0 = pool_elt_at_index (tsm->sessions, o2if->session_idx);
      ...
      if (!s0)
	{
      ...
      nat_6t_flow_translate_buf (sm, b0, ip0, o2if, proto0);
