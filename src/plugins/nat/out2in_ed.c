  ...
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
      ...
      for (i = 0; i < frame->n_vectors; i++)
	{
	  if (b[0]->flags & VLIB_BUFFER_IS_TRACED)
	    {
	      nat_ed_out2in_handoff_trace_t *t =
...
static u8 *
format_nat_ed_out2in_handoff_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  nat_ed_out2in_handoff_trace_t *t =
    va_arg (*args, nat_ed_out2in_handoff_trace_t *);
  return format (s, "out2in ed handoff thread_next index %d", t->thread_next);
}

VLIB_NODE_FN (nat_ed_out2in_handoff_node) (vlib_main_t * vm,
					   vlib_node_runtime_t * node,
					   vlib_frame_t * frame)
{
...
VLIB_REGISTER_NODE (nat_ed_out2in_handoff_node) = {
  .name = "nat-ed-out2in-handoff",
  .vector_size = sizeof (u32),
  .sibling_of = "nat-default",
  .format_trace = format_nat_ed_out2in_handoff_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = 0,
};
/* *INDENT-ON* */
