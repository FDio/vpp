...
format_nat44_ed_out2in_trace (u8 * s, va_list * args)
{
  ...
  s =
    format (s,
	    "%s: sw_if_index %d, next index %d, session %d, translation error %d, flow %U",
	    tag, t->sw_if_index, t->next_index, t->session_index,
	    t->translation_error, format_nat_6t_flow, &t->o2if);
					...
					nat_6t_flow_t ** o2if_p)
{
  ...
  o2if = nat_6t_flow_alloc (sm, thread_index, s, ip->dst_address, 0,
  ...
  o2if = nat_6t_flow_alloc (sm, thread_index, s, ip->src_address, 0,
			    ip->dst_address, 0, rx_fib_index, ip->protocol);
  nat_6t_flow_daddr_rewrite_set (o2if, m->local_addr.as_u32);
  nat_6t_flow_txfib_rewrite_set (o2if, m->fib_index);
					  ...
					  int is_multi_worker)
{
  ...
  while (n_left_from > 0)
	...
	{
	  o2if = pool_elt_at_index (tsm->flows,
				    vnet_buffer2 (b0)->
				    nat.ed_out2in_nat_flow_index);
	  if (PREDICT_TRUE
	      (o2if->match.daddr.as_u32 == ip0->dst_address.as_u32
	       && o2if->match.dport == vnet_buffer (b0)->ip.reass.l4_dst_port
	       && o2if->match.proto == ip0->protocol
	       && o2if->match.fib_index == rx_fib_index0
	       && o2if->match.saddr.as_u32 == ip0->src_address.as_u32
	       && o2if->match.sport ==
      ...
      if (now >= sess_timeout_time)
	{
      ...
      if (!nat_6t_flow_translate_buf (sm, b0, ip0, o2if, proto0))
	{
	  next[0] = NAT_NEXT_DROP;
	  translation_error = 1;
	  goto trace0;
			 ...
			 && (b0->flags & VLIB_BUFFER_IS_TRACED)))
	{
	  ...
	  if (o2if)
	    {
	      clib_memcpy (&t->o2if, o2if, sizeof (t->o2if));
	    }
      ...
      if (PREDICT_FALSE (proto0 == NAT_PROTOCOL_ICMP))
	{
	  next[0] = icmp_out2in_ed_slow_path
	    (sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node,
	     next[0], now, thread_index, &s0, &o2if);
