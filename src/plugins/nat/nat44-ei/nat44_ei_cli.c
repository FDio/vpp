...
{
  nat44_ei_main_per_thread_data_t *tsm =
...
format_nat44_ei_user (u8 *s, va_list *args)
{
  ...
  if (u->nsessions || u->nstaticsessions)
      ...
      while (session_index != ~0)
	{
	  ...
	  s = format (s, "  %U\n", format_nat44_ei_session, tsm, sess);
  ...
  vec_foreach_index (i, nm->per_thread_data)
    {
      ...
      vlib_cli_output (vm, "%U", format_bihash_8_8, &tnm->in2out, verbose);
      vlib_cli_output (vm, "%U", format_bihash_8_8, &tnm->out2in, verbose);
