			...
			u32 rx_fib_index0, u32 thread_index)
{
  ...
  /* has active sessions */
  if (clib_bihash_search_8_8 (&nm->per_thread_data[thread_index].out2in, &kv0,
			      &value0))
    {
				       ...
				       u32 thread_index, u32 sw_if_index)
{
  ...
  init_nat_k (&kv0, ip0->dst_address, dst_port,
	      ip4_fib_table_get_index_for_sw_if_index (sw_if_index), proto0);
  if (!clib_bihash_search_8_8 (&nm->per_thread_data[thread_index].in2out, &kv0,
			       &value0))
    {
      /* hairpinning */
  ...
  if (ctx->now >= sess_timeout_time)
    {
      ...
      if (clib_bihash_add_del_8_8 (&tnm->out2in, &s_kv, 0))
	   ...
	   vlib_node_runtime_t *node, u32 next0, u32 thread_index, f64 now)
{
  ...
  *sessionp = s;
  ...
  init_nat_i2o_kv (&kv0, s, s - nm->per_thread_data[thread_index].sessions);
  if (clib_bihash_add_or_overwrite_stale_8_8 (
	&nm->per_thread_data[thread_index].in2out, &kv0,
	nat44_i2o_is_idle_session_cb, &ctx0))
    nat_elog_notice (nm, "in2out key add failed");

  init_nat_o2i_kv (&kv0, s, s - nm->per_thread_data[thread_index].sessions);
  if (clib_bihash_add_or_overwrite_stale_8_8 (
	&nm->per_thread_data[thread_index].out2in, &kv0,
				 ...
				 void *d, void *e, u8 *dont_translate)
{
  ...
  *fib_index = ip4_fib_table_get_index_for_sw_if_index (sw_if_index0);
  ...
  if (clib_bihash_search_8_8 (&tnm->in2out, &kv0, &value0))
    {
					   ...
					   reass.icmp_type_or_tcp_flags)))
	{
      ...
      s0 = pool_elt_at_index (tnm->sessions, value0.value);
    }
		      ...
		      u32 thread_index, void *d, void *e)
{
  ...
  if (PREDICT_TRUE (nm->pat))
	...
	node, thread_index, b0, ip0, &addr, &port, &fib_index, &proto, d, e,
	&dont_translate);
    }
  else
    {
  ...
  if (vnet_buffer (b0)->sw_if_index[VLIB_TX] == ~0)
    {
      nat44_ei_main_t *nm = &nat44_ei_main;
      if (0 != nat44_ei_icmp_hairpinning (nm, b0, ip0, icmp0))
				...
				u32 thread_index, nat44_ei_session_t **p_s0)
{
  ...
  next0 = nat44_ei_icmp_in2out (b0, ip0, icmp0, sw_if_index0, rx_fib_index0,
				node, next0, thread_index, p_s0, 0);
				...
				int is_output_feature)
{
  ...
  while (n_left_from >= 2)
      ...
      else
	{
      ...
      init_nat_k (&kv0, ip0->src_address,
		  ...
		  proto0);
      if (PREDICT_FALSE (
	    clib_bihash_search_8_8 (&nm->per_thread_data[thread_index].in2out,
				    &kv0, &value0) != 0))
	{
	  if (is_slow_path)
	    {
  ...
  while (n_left_from > 0)
      ...
      else
	{
      ...
      if (clib_bihash_search_8_8 (&nm->per_thread_data[thread_index].in2out,
				  &kv0, &value0))
	{
	  if (is_slow_path)
	    {
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (proto0 == NAT_PROTOCOL_ICMP))
	    {
	      next0 =
		nat44_ei_icmp_in2out (b0, ip0, icmp0, sw_if_index0,
	  ...
	  is_hairpinning = nat44_ei_hairpinning (
	    vm, node, nm, b0, ip0, udp0, tcp0, proto0, 0 /* do_trace */);
