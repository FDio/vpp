...
static int nat44_ei_add_internal_static_mapping (
...
static int nat44_ei_del_internal_static_mapping (
  ip4_address_t l_addr, ip4_address_t e_addr, u16 l_port, u16 e_port,
  nat_protocol_t proto, u32 vrf_id, u32 sw_if_index, u32 flags);
...
nat44_ei_del_static_mappings ()
{
  ...
  pool_foreach (m, pool)
    {
      error = nat44_ei_del_internal_static_mapping (
...

{
  ...
  return nat44_ei_add_internal_static_mapping (l_addr, e_addr, l_port, e_port,
			     ...
			     u32 vrf_id, u32 sw_if_index, u32 flags)
{
  ...
  return nat44_ei_del_internal_static_mapping (
    l_addr, e_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags);
}

static int
nat44_ei_add_internal_static_mapping (ip4_address_t l_addr,
...
static int
nat44_ei_del_internal_static_mapping (ip4_address_t l_addr,
				      ip4_address_t e_addr, u16 l_port,
				      u16 e_port, nat_protocol_t proto,
				      u32 vrf_id, u32 sw_if_index, u32 flags)
{
...
nat44_ei_del_address (ip4_address_t addr, u8 delete_sm)
{
  ...
  if (delete_sm)
	...
	{
	  if (m->external_addr.as_u32 == addr.as_u32)
	    nat44_ei_del_internal_static_mapping (
	      m->local_addr, m->external_addr, m->local_port, m->external_port,
	      m->proto, m->vrf_id, ~0, m->flags);
	}
    }
				      ...
				      u32 is_delete)
{
  ...
  if (is_delete)
    {
      if (!m)
	return;
      rv = nat44_ei_del_internal_static_mapping (
  ...
  if (rv)
    {
      nat_elog_notice_X1 (nm, "nat44_ei_%s_static_mapping returned %d",
