...
nat44_ei_interface_add_del (u32 sw_if_index, u8 is_inside, int is_del)
{
  ...
  if (nm->fq_in2out_index == ~0 && nm->num_workers > 1)
    nm->fq_in2out_index =
      vlib_frame_queue_main_init (nm->in2out_node_index, NAT44_EI_FQ_NELTS);

  if (nm->fq_out2in_index == ~0 && nm->num_workers > 1)
    nm->fq_out2in_index =
      vlib_frame_queue_main_init (nm->out2in_node_index, NAT44_EI_FQ_NELTS);
			       ...
			       u32 thread_index, u8 is_ha)
{
  ...
  init_nat_i2o_k (&kv, s);
  if (clib_bihash_add_del_8_8 (&tnm->in2out, &kv, 0))
    nat_elog_warn (nm, "in2out key del failed");
  init_nat_o2i_k (&kv, s);
  if (clib_bihash_add_del_8_8 (&tnm->out2in, &kv, 0))
...
nat44_ei_worker_db_free (nat44_ei_main_per_thread_data_t *tnm)
{
  ...
  clib_bihash_free_8_8 (&tnm->in2out);
  clib_bihash_free_8_8 (&tnm->out2in);
			 ...
			 u32 user_buckets)
{
  ...
  clib_bihash_init_8_8 (&tnm->in2out, "in2out", translation_buckets, 0);
  clib_bihash_init_8_8 (&tnm->out2in, "out2in", translation_buckets, 0);
  clib_bihash_init_8_8 (&tnm->user_hash, "users", user_buckets, 0);

  clib_bihash_set_kvp_format_fn_8_8 (&tnm->in2out,
				     format_nat44_ei_session_kvp);
  clib_bihash_set_kvp_format_fn_8_8 (&tnm->out2in,
				     format_nat44_ei_session_kvp);
