		      ...
		      tcp_header_t *tcp0, u32 proto0, int do_trace)
{
  ...
  /* or active session */
  else
    {
      if (nm->num_workers > 1)
	ti =
	  (clib_net_to_host_u16 (udp0->dst_port) - 1024) / nm->port_per_thread;
      else
	ti = nm->num_workers;

      init_nat_k (&kv0, ip0->dst_address, udp0->dst_port,
		  nm->outside_fib_index, proto0);
      rv = clib_bihash_search_8_8 (&nm->per_thread_data[ti].out2in, &kv0,
				   &value0);
      si = value0.value;
			   ...
			   ip4_header_t *ip0, icmp46_header_t *icmp0)
{
  ...
  if (icmp_type_is_error_message (
	...
	return 1;
      ...
      if (clib_bihash_search_8_8 (&nm->per_thread_data[ti].out2in, &kv0,
				  &value0))
	return 1;
      si = value0.value;
				  ...
				  &value0))
	{
	  ...
	  if (nm->num_workers > 1)
	    ti =
	      (clib_net_to_host_u16 (icmp_id0) - 1024) / nm->port_per_thread;
	  else
	    ti = nm->num_workers;
	  int rv = clib_bihash_search_8_8 (&nm->per_thread_data[ti].out2in,
					   &kv0, &value0);
	  if (!rv)
	    {
	      si = value0.value;
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (nat44_ei_is_hairpinning (nm, &ip0->dst_address)))
	      ...
	      if (proto0 == NAT_PROTOCOL_TCP || proto0 == NAT_PROTOCOL_UDP)
		{
		  ...
		  nat44_ei_hairpinning (vm, node, nm, b0, ip0, udp0, tcp0,
					proto0, 1 /* do_trace */);
		}
	      else if (proto0 == NAT_PROTOCOL_ICMP)
		{
		  ...
		  nat44_ei_icmp_hairpinning (nm, b0, ip0, icmp0);
		}
	      else
		{
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (nat44_ei_hairpinning (vm, node, nm, b0, ip0, udp0, tcp0, proto0,
				    1 /* do_trace */))
	    next0 = NAT44_EI_HAIRPIN_NEXT_LOOKUP;
