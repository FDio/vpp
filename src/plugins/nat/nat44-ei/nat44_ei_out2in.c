  ...
  if (ctx->now >= sess_timeout_time)
    {
      ...
      if (clib_bihash_add_del_8_8 (&tnm->in2out, &s_kv, 0))
				 ...
				 void *d, void *e, u8 *dont_translate)
{
  ...
  *proto = 0;
  ...
  if (clib_bihash_search_8_8 (&tnm->out2in, &kv0, &value0))
    {
  ...
  while (n_left_from >= 2)
      ...
      if (PREDICT_FALSE (proto0 == NAT_PROTOCOL_ICMP))
	{
      ...
      init_nat_k (&kv0, ip0->dst_address,
		  ...
		  proto0);
      if (clib_bihash_search_8_8 (&nm->per_thread_data[thread_index].out2in,
				  &kv0, &value0))
	{
	  /* Try to match static mapping by external address and port,
	     destination address and port in packet */
