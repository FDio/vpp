			     ...
			     nat44_ed_dir_e dir)
{
  ...
  switch (old_state)
    ...
    case NAT44_ED_TCP_STATE_ESTABLISHED:
      // CLOSING when a FIN is seen from both sides or session has been RST
      if ((ses->tcp_flags[NAT44_ED_DIR_I2O] &
	   ses->tcp_flags[NAT44_ED_DIR_O2I] & TCP_FLAG_FIN) ||
...
nat44_session_update_counters (snat_session_t *s, f64 now, uword bytes,
			       u32 thread_index)
{
  if (NAT44_ED_TCP_STATE_CLOSING != s->tcp_state)
    {
      s->last_heard = now;
    }
  s->total_pkts++;
