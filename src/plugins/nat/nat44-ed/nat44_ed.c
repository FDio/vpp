				 ...
				 ip4_address_t pool_addr, u8 *tag, u8 is_add)
{
  ...
  if (is_sm_switch_address (flags))
    {
      if (is_add)
	{
	  if (!nat44_ed_get_resolve_record (l_addr, l_port, e_port, proto,
	...
	{
	  if (nat44_ed_del_resolve_record (l_addr, l_port, e_port, proto,
  ...
  if (is_add)
    {
      return nat44_ed_add_static_mapping (l_addr, e_addr, l_port, e_port,
					  proto, vrf_id, sw_if_index, flags,
					  pool_addr, tag);
    }
  else
    {
      return nat44_ed_del_static_mapping (l_addr, e_addr, l_port, e_port,
					  proto, vrf_id, sw_if_index, flags);
    }
}
...
nat44_ed_del_static_mappings ()
{
  ...
  pool_foreach (m, pool)
    {
      error = nat44_ed_del_static_mapping (m->local_addr, m->external_addr,
					   m->local_port, m->external_port,
