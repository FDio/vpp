			    ...
			    u32 thread_index, u8 is_ha)
{
  ...
  if (!is_ha)
    nat_syslog_nat44_sdel (0, s->in2out.fib_index, &s->in2out.addr,
			   ...
			   s->proto, is_twice_nat_session (s));
...
nat44_ed_add_static_mapping_when_resolved (
  ...
  u16 e_port, u32 vrf_id, ip_protocol_t proto, int addr_only, u8 *tag,
  int twice_nat, int out2in_only, int identity_nat, ip4_address_t pool_addr,
  int exact)
{
...
nat44_ed_add_static_mapping (ip4_address_t l_addr, ip4_address_t e_addr,
			     ...
			     u32 sw_if_index, ip_protocol_t proto, int is_add,
  ...
  if (is_add)
      ...
      if (m)
	{
	  if (is_identity_static_mapping (m))
		...
		fib_table_find_or_create_and_lock (FIB_PROTOCOL_IP4, vrf_id,
						   sm->fib_src_low);
	      if (nat44_ed_sm_i2o_add (sm, m, m->local_addr, m->local_port,
      ...
      if (!out2in_only)
	{
      ...
      if (addr_only)
	{
	  rc = nat44_ed_sm_o2i_add (sm, m, m->external_addr, 0, 0, 0);
	}
      else
	{
	  rc = nat44_ed_sm_o2i_add (sm, m, m->external_addr, m->external_port,
				    0, proto);
	}
      if (rc)
	{
...
nat44_add_del_lb_static_mapping (ip4_address_t e_addr, u16 e_port,
				 u32 fib_index, ip_protocol_t proto,
				 ...
				 u8 *tag, u32 affinity)
{
  ...
  if (is_add)
      ...
      if (nat44_ed_sm_o2i_add (sm, m, e_addr, e_port, 0, proto))
	{
	  ...
	  // we've done and deal with partial cleanups and since bihash adds
	  // are (extremely improbable) the only points of failure, it's
      ...
      for (i = 0; i < vec_len (locals); i++)
	{
	  ...
	  if (!out2in_only)
				       ...
				       locals[i].fib_index, m->proto))
		{
		  nat_elog_err (sm, "sm i2o key add failed");
		  rc = VNET_API_ERROR_UNSPECIFIED;
		  // here we continue with add operation so that it can be
		  // safely reversed in delete path - otherwise we'd have to
		  // track what we've done and deal with partial cleanups and
		  // since bihash adds are (extremely improbable) the only
		  // points of failure, it's easier to just do it this way
		}
	    }
  ...
  // this is delete path for both delete and failed add
  if (!is_add || rc)
    {
      if (!m)
	return VNET_API_ERROR_NO_SUCH_ENTRY;
	...
	return VNET_API_ERROR_INVALID_VALUE;
      ...
      if (nat44_ed_sm_o2i_del (sm, m->external_addr, m->external_port, 0,
			       m->proto))
	{
	  if (!is_add) // don't warn if recovering from error
	    nat_elog_err (sm, "nat44ed o2i key del failed");
	}
      ...
      pool_foreach (local, m->locals)
      {
		...
		{
		  if (!is_add) // don't warn if recovering from error
		    nat_elog_err (sm, "nat44ed i2o key del failed");
		}
	    }
