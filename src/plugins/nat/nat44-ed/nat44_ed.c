...
/*
static void
nat44_ed_add_fib_entry_reg (ip4_address_t *addr, u8 p_len, u32 sw_if_index,
			      int is_add)
{
  ...
  u32 fib_index = ip4_fib_table_get_index_for_sw_if_index (sw_if_index);
  fib_node_index_t fib_entry_index = fib_table_lookup_exact_match(fib_index,
prefix);
  ...
  else if (FIB_NODE_INDEX_INVALID != fib_entry_index)
    {
      fib_table_entry_delete_i(fib_index, fib_entry_index, &prefix,
sm->fib_src_low);
    }
}
*/
...
static int
nat44_ed_get_fib_entry_reg (ip4_address_t addr, u32 sw_if_index, int *out)
{
  snat_main_t *sm = &snat_main;
...
static_always_inline void
nat44_ed_add_del_addr_to_fib_foreach_sm (u32 sw_if_index, u8 is_add)
{
  snat_main_t *sm = &snat_main;
...
nat44_ed_add_interface (u32 sw_if_index, u8 is_inside)
{
  ...
  if (!is_inside)
      ...
      else
	{
      ...
      nat44_ed_add_del_addr_to_fib_foreach_sm (sw_if_index, 1);
...
nat44_ed_del_interface (u32 sw_if_index, u8 is_inside)
{
  ...
  if (!is_inside)
      ...
      if (outside_fib)
	{
      ...
      nat44_ed_add_del_addr_to_fib_foreach_sm (sw_if_index, 0);
    }
