  ...
  vec_foreach (a, vec)
    {
      ...
      if (error)
	{
	  nat44_ed_log_err ("error occurred while removing adderess");
	}
    }
  vec_free (vec);
...
nat44_ed_del_interfaces ()
{
  ...
  pool_foreach (i, pool)
    {
      if (nat44_ed_interface_is_inside (i))
	{
	  error = nat44_ed_del_interface (i->sw_if_index, 1);
	}
      if (nat44_ed_interface_is_outside (i))
	{
  ...
  pool_foreach (i, pool)
    {
      ...
      if (error)
	{
	  nat44_ed_log_err ("error occurred while removing output interface");
	}
    }
  pool_free (pool);
