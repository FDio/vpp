	      ...
	      u32 thread_index, f64 now)
{
  ...
  if (!is_sm)
    {
      ...
      is_hairpinning = nat44_ed_external_sm_lookup (sm, r_addr, r_port, proto,
						    &daddr, &dport);
  ...
  nat_6t_i2o_flow_init (sm, thread_index, s, l_addr, l_port,
			is_hairpinning ? outside_addr : r_addr, r_port,
			rx_fib_index, proto);
