...
get_outside_fib_index (fib_prefix_t *pfx)
{
  ...
  vec_foreach (f, sm->outside_fibs)
    {
      ...
      if (~0 != fib_entry_get_resolving_interface (fei))
	{
	  fib_index = f->fib_index;
	  break;
	}
    }
...
static_always_inline vrf_table *
get_vrf_table_by_fib (u32 fib_index)
{
  ...
  pool_foreach (t, sm->vrf_tables)
    {
      if (fx_fib_index == t->table_fib_index)
	{
	  return t;
	}
    }
...
is_destination_resolvable (u32 rx_fib_index, ip4_address_t addr)
{
  ...
  if (0 != t)
      ...
      vec_foreach (f, sm->outside_fibs)
	{
	  fei = fib_table_lookup (f->fib_index, pfx);
	  if ((FIB_NODE_INDEX_INVALID != fei) &&
	      (~0 != (i = fib_entry_get_resolving_interface (fei))))
	    {
	      snat_interface_t *i;
