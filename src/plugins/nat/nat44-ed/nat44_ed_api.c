				  ...
				  vrf_table_t *t)
{
  ...
  u32 *vrf_ids = 0;
  vrf_route_t *r;
  uword n_routes = pool_elts (t->routes);

  mp =
    vl_msg_api_alloc_zero (sizeof (*mp) + sizeof (mp->vrf_ids[0]) * n_routes);
  mp->_vl_msg_id =
    ntohs (VL_API_NAT44_ED_VRF_TABLES_DETAILS + sm->msg_id_base);
  mp->context = context;
  mp->table_vrf_id = clib_host_to_net_u32 (t->table_vrf_id);
  mp->n_vrf_ids = clib_host_to_net_u32 (n_routes);

  pool_foreach (r, t->routes)
    {
      vec_add1 (vrf_ids, clib_host_to_net_u32 (r->vrf_id));
    }

  // copy the records
  clib_memcpy (mp->vrf_ids, vrf_ids, sizeof (mp->vrf_ids[0]) * n_routes);
