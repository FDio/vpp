...
{
  snat_main_t *sm = &snat_main;
  vl_api_nat44_ed_add_del_vrf_table_route_t *rmp;
  int rv = 0;
...
vl_api_nat44_ed_add_del_vrf_route_t_handler (
  vl_api_nat44_ed_add_del_vrf_route_t *mp)
{
  vl_api_nat44_ed_add_del_vrf_route_t *rmp;
  int rv =
...
static void nat44_ed_vrf_table_send_details (vl_api_registration_t *rp,
					     u32 context, vrf_table_t *t);
{
  snat_main_t *sm = &snat_main;
  vl_api_nat44_ed_vrf_table_details_t *mp;

  u32 *vrf_ids = 0;
  vrf_route_t *r;

  mp = vl_msg_api_alloc_zero (sizeof (*mp) +
			      sizeof (mp->vrf_ids[0]) * vec_len (t->vrf_ids));
  mp->_vl_msg_id = ntohs (VL_API_NAT44_ED_TABLE_DETAILS + mp->msg_id_base);
  mp->context = context;
  mp->n_vrf_ids = clib_host_to_net_u32 (vec_len (t->vrf_ids));
  ...
  // send the message
  vl_api_snd_msg (rp, (u8 *), mp);
}

static void
vl_api_nat44_ed_vrf_table_dump_t_handler (vl_api_nat44_ed_vrf_table_dump_t *mp)
{
  snat_main_t *sm = &snat_main;
  ...
  pool_foreach (t, sm->vrf_tables)
    {
      nat44_ed_vrf_table_send_details (rp, mp->context, t);
    }
}
