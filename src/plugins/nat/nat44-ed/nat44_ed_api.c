					    ...
					    * mp)
{
  ...
  pool_foreach (m, sm->static_mappings)
   {
     if (!is_identity_nat (m->flags) && !is_lb (m->flags))
  ...
  vl_api_nat44_add_del_lb_static_mapping_t *mp)
{
  ...
  if (mp->is_add)
    {
      rv = nat44_add_lb_static_mapping (
	e_addr, mp->external_port, proto, locals, twice_nat,
	mp->flags & NAT_API_IS_OUT2IN_ONLY, tag,
	clib_net_to_host_u32 (mp->affinity));
    }
  else
    {
      rv = nat44_del_lb_static_mapping (e_addr, mp->external_port, proto,
