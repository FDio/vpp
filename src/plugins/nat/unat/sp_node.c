...
format_unat_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "UNAT SLOWPATH: %U", format_ip4_header, t->ip);
  return s;
}
				...
				unat_key_t *k)
{
  ...
  while (1) {
    if (unat_session_exists(&um->flowhash, k)) {
      *conflicts += 1;
...
static unat_interface_t *
unat_interface_by_sw_if_index (u32 sw_if_index)
{
  unat_main_t *um = &unat_main;
...
{ 
  ...
  while (n_left_from > 0) {
    ...
    if (um->sessions_per_worker[thread_index] == 0) {
      ...
      b[0]->error = node->errors[UNAT_SP_ERROR_THREAD_NOT_ENABLED];
      goto trace0;
    }
     ...
     */
    unat_interface_t *interface = unat_interface_by_sw_if_index(sw_if_index0);
    if (interface == 0 ||
	!interface->in2out ||
	(b[0]->flags & VNET_BUFFER_F_LOCALLY_ORIGINATED && !unat_is_pool_address(&ip0->src_address))) {
...
{ 
  ...
  while (n_left_from > 0) {
    ...
    while (n_left_from > 0 && n_left_to_next > 0) {
       ...
       */
      if (unat_is_pool_address(&ip0->dst_address)) {
	/* Lookup static binding */
	ip4_address_t src = { 0 };
	u16 dport = vnet_buffer (b0)->ip.reass.l4_dst_port;
	clib_bihash_kv_16_8_t k;
	unat_calc_key2(&src, &ip0->dst_address, ip0->protocol, rx_fib_index0, 0, dport, (unat_key_t *)&k);
