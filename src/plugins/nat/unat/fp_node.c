...
static void
unat_tcp_timer_update (u32 thread_index, u32 pool_index, unat_session_t *s)
{
  unat_main_t *um = &unat_main;
  if (tw_timer_handle_is_free_2t_1w_2048sl(um->timers_per_worker[thread_index], s->timer_handle)) {
    s->timer_handle = tw_timer_start_2t_1w_2048sl (um->timers_per_worker[thread_index], pool_index, 0,
						   s->timer);
  } else {
    tw_timer_update_2t_1w_2048sl (um->timers_per_worker[thread_index],
				  s->timer_handle, s->timer);
  }
}
  ...
  *out_fib_index = s->fib_index;
  ...
  if (ip->protocol == IP_PROTOCOL_TCP) {
	...
	if (timer != session->timer) {
	  session->timer = timer;
	  unat_tcp_timer_update(thread_index, pool_index, session);
	}
      }
    }
			     ...
			     vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	   ...
	   */
	  u32 out_fib_index0;
	  u32 pool_index0 = vnet_buffer (b0)->unat.pool_index;
  ...
  //if (vec_len(um->expired_sessions_per_worker[thread_index]))
  unat_sp_spin_timer(thread_index, now, 64);
