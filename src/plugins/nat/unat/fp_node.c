...
unat_fp_inline (vlib_main_t * vm,
		vlib_node_runtime_t * node,
		vlib_frame_t * frame, bool in2out)
{
  unat_main_t *um = &unat_main;
  ...
  while (n_left_from > 0) {
    ...
    errno0 = execute(in2out, pool_index0, thread_index,
...
VLIB_NODE_FN (unat_fp_i2o_node) (vlib_main_t * vm,
				 vlib_node_runtime_t * node,
				 vlib_frame_t * frame)
{
  return unat_fp_inline (vm, node, frame, 1);
}

VLIB_NODE_FN (unat_fp_o2i_node) (vlib_main_t * vm,
				 vlib_node_runtime_t * node,
				 vlib_frame_t * frame)
{
  return unat_fp_inline (vm, node, frame, 0);
}

/* *INDENT-OFF* */
VLIB_REGISTER_NODE(unat_fp_i2o_node) = {
  .name = "unat-fastpath-i2o",
...
VLIB_REGISTER_NODE(unat_fp_o2i_node) = {
  .name = "unat-fastpath-o2i",
  /* Takes a vector of packets. */
  .vector_size = sizeof(u32),
  .sibling_of = "unat-handoff",
  .n_errors = UNAT_FP_N_ERROR,
