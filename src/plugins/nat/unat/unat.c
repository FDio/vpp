...
extern vlib_node_registration_t unat_sp_timer_node;
static vlib_node_registration_t unat_timer_node;

static void
unat_expired_timer_callback (u32 * expired_timers)
{
  ...
  for (i = 0; i < vec_len (expired_timers); i++) {
    ...
    vec_add1 (um->expired_sessions_per_worker[thread_index], poolindex);
  }
}
...
unat_enable_worker (u32 thread_index)
{
  ...
  vec_add1(um->workers_with_timer, thread_index);
  um->timers_per_worker[thread_index] =
    clib_mem_alloc (sizeof (TWT (tw_timer_wheel)));
  tw_timer_wheel_init_2t_1w_2048sl (um->timers_per_worker[thread_index],
				    unat_expired_timer_callback,
				    1.0, 1024);
  pool_init_fixed(um->sessions_per_worker[thread_index], 10000000);
}
...
unat_enable (vlib_main_t *vm)
{
  ...
  vec_validate (um->timers_per_worker, vlib_num_workers() + 1);
  vec_validate (um->expired_sessions_per_worker, vlib_num_workers() + 1);
  vec_validate (um->pool_per_thread, vlib_num_workers() + 1);

  vlib_node_set_state (vm, unat_timer_node.index, VLIB_NODE_STATE_POLLING);
  vlib_node_t *n = vlib_get_node (vm, unat_timer_node.index);
   ...
   */
  um->fast_path_node_index = vlib_frame_queue_main_init (unat_fp_node.index, 256);
  um->slow_path_node_index = vlib_frame_queue_main_init (unat_sp_i2o_node.index, 256);
...
format_unat_fp_session (u8 * s, va_list * args)
{
  ...
  if (ses->instructions & (UNAT_INSTR_DESTINATION_ADDRESS|UNAT_INSTR_DESTINATION_PORT)) {
    s = format (s,
		...
		format_unat_state, ses->state, now - ses->last_heard);
...
timer_process (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  int i;
  vec_add1 (worker_vms, vlib_get_main());
  for (i = 0; i < vlib_num_workers(); i++)
    vec_add1 (worker_vms, vlib_get_worker_vlib_main (i));

  f64 sleep_duration = 1.0;

  while (1) {
    /* Send an interrupt to each timer input node */
    for (i = 0; i < vec_len (um->workers_with_timer); i++) {
      worker_vm = worker_vms[um->workers_with_timer[i]];
      if (worker_vm) {
	vlib_node_set_interrupt_pending (worker_vm,
					 unat_sp_timer_node.index);
      }
    }
    /* Try to achieve a balance between timers and forwarding */
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (unat_timer_node, static) = {
  .function = timer_process,
  .name = "unat-timer-process",
