...
unat_enable_worker (u32 thread_index)
{
  ...
  vec_validate (um->sessions_per_worker, thread_index);
  vec_validate (um->lru_pool, thread_index);
  vec_validate (um->lru_head_index, thread_index);
...
unat_validate_configuration (void)
{
  ...
  if (vec_len(um->sessions_per_worker) == 0) {
    clib_warning("Sessions no initialized");
    return false;
  }
