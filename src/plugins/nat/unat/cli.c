  ...
  if (!unformat_user (input, unformat_line_input, line_input))
    {
      ...
      for (i = 0; i <= vlib_num_workers(); i++) {
	vlib_cli_output(vm, "[%u]: %u", i, pool_elts(um->sessions_per_worker[i]));
      }
      vlib_cli_output(vm, "Timer process running out of steam: %u", um->timer_process_out_of_time);
      vlib_cli_output(vm, "Timer processed: %u", um->timer_processed);
      vlib_cli_output(vm, "Bihash scavenge deleted: %u", um->scavenge_idle_session_deleted);
      vlib_cli_output(vm, "Bihash scavenge not idle: %u", um->scavenge_idle_session_not_idle);
      vlib_cli_output(vm, "Bihash scavenge wrong thread: %u", um->scavenge_idle_session_wrong_thread);
