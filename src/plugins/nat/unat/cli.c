				   ...
				   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  // um->handoff_node = vlib_num_workers() > 1 ? "unat-handoff" : "unat-singleworker";
		      ...
		      vlib_cli_command_t * cmd)
{
  ...
  if (poolindex != ~0) {
    ...
    vec_validate (um->pool_per_thread, thread_index);
...
VLIB_CLI_COMMAND (set_interface_unat_command, static) = {
  ...
  .short_help = "set interface unat <intfc> <in | out>",
};
