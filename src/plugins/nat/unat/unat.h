...
typedef enum
{
 ...
 UNAT_COUNTER_HANDOFF_SAME_WORKER_FP,
...
#define foreach_unat_counter_name					\
  ...
  _(HANDOFF_SAME_WORKER_FP, same_worker_fp, unat/handoff)		\
...
typedef struct {
  ...
  u32 *workers_with_timer;

  u32 timer_process_out_of_time;
  u32 timer_processed;

  u32 scavenge_idle_session_wrong_thread;
  u32 scavenge_idle_session_deleted;
  u32 scavenge_idle_session_not_idle;
...
void unat_enable (vlib_main_t *vm);
void unat_enable_worker (u32 thread_index);
void unat_sp_spin_timer (u32 thread_index, f64 now, u32 max);
