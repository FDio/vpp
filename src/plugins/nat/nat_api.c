  ...
  if (mp->enable)
    {
      ...
      c.user_buckets = ntohl (mp->user_buckets);
      c.user_memory = ntohl (mp->user_memory);

      c.sessions = ntohl (mp->sessions);
      c.session_buckets = ntohl (mp->session_buckets);
				     ...
				     mp, void *handle)
{
  ...
  if (mp->enable)
	...
	s = format (s, "users %u ", val);
      val = ntohl (mp->user_buckets);
      if (val)
	s = format (s, "user_buckets %u ", val);
	...
	s = format (s, "sessions %u ", val);
      val = ntohl (mp->session_buckets);
      if (val)
	s = format (s, "session_buckets %u ", val);
