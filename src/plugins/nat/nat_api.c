...
static void
vl_api_nat44_acl_enable_disable_t_handler (vl_api_nat44_acl_enable_disable_t * mp)
{
  snat_main_t *sm = &snat_main;
  vl_api_nat44_acl_enable_disable_reply_t *rmp;
  int rv = 0;

  sm->acl_enabled = mp->enable != 0;

  REPLY_MACRO (VL_API_NAT44_ACL_ENABLE_DISABLE_REPLY);
}

static void *
vl_api_nat44_acl_enable_disable_t_print (vl_api_nat44_acl_enable_disable_t * mp,
                                         void *handle)
{
  u8 *s;

  s = format (0, "SCRIPT: nat44_acl_enable_disable ");
...
static void
vl_api_nat44_acl_is_enabled_t_handler (vl_api_nat44_acl_is_enabled_t * mp)
{
  unix_shared_memory_queue_t *q;
  snat_main_t *sm = &snat_main;
  vl_api_nat44_acl_is_enabled_reply_t *rmp;
  ...
  rmp = vl_msg_api_alloc (sizeof (*rmp));
  memset (rmp, 0, sizeof (*rmp));
  rmp->_vl_msg_id = ntohs (VL_API_NAT44_ACL_IS_ENABLED_REPLY + sm->msg_id_base);
  rmp->context = mp->context;

  rmp->enabled = sm->acl_enabled;

  vl_msg_api_send_shmem (q, (u8 *) & rmp);
}

static void *
vl_api_nat44_acl_is_enabled_t_print (vl_api_nat44_acl_is_enabled_t * mp,
                                     void *handle)
{
  u8 *s;

  s = format (0, "SCRIPT: nat44_acl_is_enabled ");
...
_(NAT44_ACL_ENABLE_DISABLE, nat44_acl_enable_disable)                   \
_(NAT44_ACL_IS_ENABLED, nat44_acl_is_enabled)                           \
