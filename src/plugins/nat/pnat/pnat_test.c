...
always_inline void *test_vlib_frame_vector_args(vlib_frame_t *f) {
...
static int fill_packets(vlib_main_t *vm, u32 bi, test_5tuple_t *test) {
...
static void test_table(test_t *tests, int no_tests) {
    ...
    for (i = 0; i < no_tests; i++) {
        ...
        fill_packets(vm, bi[i], &tests[i].send);
        fill_packets(vm, expected_bi[i], &tests[i].expect);
    }
