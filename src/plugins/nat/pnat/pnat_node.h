...
static u32 pnat_rewrite_ip4(u32 pool_index, ip4_header_t *ip) {
    ...
    /* Calculate the checksum delta for the header (could be done at
     * configuration time */
    ip_csum_t csumd = 0;
 ...
 */
static inline void pnat_calc_key(u32 sw_if_index, bool input,
...
static_always_inline uword pnat_node_inline(vlib_main_t *vm,
    ...
    while (n_left_from > 0) {
        u32 sw_if_index0 = input ? vnet_buffer(b[0])->sw_if_index[VLIB_RX]
                                 : vnet_buffer(b[0])->sw_if_index[VLIB_TX];
        u16 sport0 = vnet_buffer(b[0])->ip.reass.l4_src_port;
        u16 dport0 = vnet_buffer(b[0])->ip.reass.l4_dst_port;
        u32 iph_offset =
            input ? 0 : vnet_buffer(b[0])->ip.reass.save_rewrite_length;
        ip0 =
        ...
        pnat_mask_t lookup_mask = input ? interface->input_lookup_mask
    ...
    while (n_left_from > 0) {
        ...
        /* 6-tuple lookup */
        if (clib_bihash_search_inline_with_hash_16_8(&pm->flowhash, h[0],
                                                     &kv) == 0) {
            /* Cache hit */
            *pi = kv.value;
            u32 iph_offset =
            ...
            ip0 = (ip4_header_t *)((u8 *)vlib_buffer_get_current(b[0]) +
                                   iph_offset);
            errno0 = pnat_rewrite_ip4(kv.value, ip0);
            if (errno0) {
