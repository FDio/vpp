...
u8 *format_pnat_key(u8 *s, va_list *args) {
    ...
    s = format(s, "[%u] %s %U:%u -> %U:%u, proto: %U", k->sw_if_index,
               k->input ? "input" : "output", format_ip4_address, &k->sa, k->sp,
               format_ip4_address, &k->da, k->dp, format_ip_protocol, k->proto);
    return s;
}
...
u8 *format_pnat_translation(u8 *s, va_list *args) {
    ...
    s = format(s, "[%d, %d, %d] %s match: %U rewrite: %U", index, t->fib_index,
               t->key.sw_if_index, t->key.input ? "input" : "output",
               format_pnat_5tuple, &t->match, format_pnat_5tuple, &t->rewrite);
    return s;
}
