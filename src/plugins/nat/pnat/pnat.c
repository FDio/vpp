 ...
 */
static int
pnat_disable_interface (u32 sw_if_index)
{
  pnat_main_t *pm = &pnat_main;
  pnat_interface_t *interface = pnat_interface_by_sw_if_index(sw_if_index);
  ...
  if (interface->refcount == 0) {
    if (interface->input_enabled) {
      ...
      if (vnet_feature_enable_disable ("ip4-unicast", "pnat-input", sw_if_index, 0, 0, 0) != 0)
        return -1;

    }
    if (interface->output_enabled) {
      ip4_sv_reass_output_enable_disable_with_refcnt(sw_if_index, 0);
      if (vnet_feature_enable_disable ("ip4-output", "pnat-output", sw_if_index, 0, 0, 0) != 0)
        return -1;
    }
...
pnat_disable (void)
{
  pnat_main_t *pm = &pnat_main;
  if (!pm->enabled) return;
  if (pool_elts(pm->translations)) return;
 ...
 */
static int
pnat_interface_check_mask (u32 sw_if_index, bool input, pnat_mask_t mask)
{
  pnat_interface_t *interface = pnat_interface_by_sw_if_index(sw_if_index);
  if (!interface) return 0;
  bool enabled = input ? interface->input_enabled : interface->output_enabled;
  pnat_mask_t interface_mask = input ? interface->input_lookup_mask : interface->output_lookup_mask;
  if (!enabled) return 0;
  if (interface_mask != mask) return -1;
  return 0;
}
                      ...
                      pnat_5tuple_t *rewrite, bool input, u32 *index)
{
  ...
  *index = -1;
  ...
  if (pnat_interface_check_mask(sw_if_index, input, match->mask) != 0) return -2;
 ...
 */
int
pnat_del_translation (u32 index, u32 sw_if_index)
{
  pnat_main_t *pm = &pnat_main;
  ...
  pnat_disable_interface(sw_if_index);
