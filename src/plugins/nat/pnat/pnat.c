...
void pnat_u64x3_mask_from_rewrite(pnat_rewrite_tuple_t *rewrite,
                                  pnat_u64x3_t *pre, pnat_u64x3_t *post) {
    pnat_u64x3_t i = {0};
...
int pnat_binding_add(pnat_match_tuple_t *match, pnat_rewrite_tuple_t *rewrite,
    ...
    *index = -1;
    ...
    t->post.as_u64[0] = clib_host_to_net_u64(
        (u64)clib_net_to_host_u32(t->post_sa.as_u32) << 32 |
        clib_net_to_host_u32(t->post_da.as_u32));
    t->post.as_u64[1] = clib_host_to_net_u64((u64)rewrite->sport << 48 |
                                             (u64)rewrite->dport << 32);
    ...
    if (match->proto == IP_API_PROTO_UDP)
        ...
        t->always_calculate_l4chksum = true;
    }

    if (rewrite->sport || rewrite->dport)
        max_rewrite = 28;
    t->max_rewrite =
        max_rewrite > rewrite->to_offset ? max_rewrite : rewrite->to_offset;
