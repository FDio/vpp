 ...
 */
static clib_error_t *pnat_enable_interface(u32 sw_if_index, bool input,
                                           pnat_mask_t mask) {
    pnat_main_t *pm = &pnat_main;
    ...
    if (!enabled) {
        ...
        pnat_mask_t current_mask = input ? interface->input_lookup_mask
                                         : interface->output_lookup_mask;
 ...
 */
static inline void pnat_calc_key_from_5tuple(u32 sw_if_index, bool input,
                                             pnat_5tuple_t *match,
                                             pnat_key_t *k) {
 ...
 */
static int pnat_interface_check_mask(u32 sw_if_index, bool input,
                                     pnat_mask_t mask) {
    pnat_interface_t *interface = pnat_interface_by_sw_if_index(sw_if_index);
    if (!interface)
        return 0;
    bool enabled = input ? interface->input_enabled : interface->output_enabled;
    pnat_mask_t interface_mask =
        input ? interface->input_lookup_mask : interface->output_lookup_mask;
    if (!enabled)
        return 0;
    if (interface_mask != mask)
        return -1;
    return 0;
}
...
int pnat_add_translation(u32 sw_if_index, pnat_5tuple_t *match,
    ...
    *index = -1;
    ...
    if (pnat_interface_check_mask(sw_if_index, input, match->mask) != 0)
        return -2;
