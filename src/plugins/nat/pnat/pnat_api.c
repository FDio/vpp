...
static void vl_api_pnat_binding_add_t_handler(vl_api_pnat_binding_add_t *mp) {
    ...
    rv = pnat_add_translation(mp->sw_if_index, &mp->match, &mp->rewrite,
                              mp->is_input, &index);

bad_sw_if_index:
    REPLY_MACRO2_END(VL_API_PNAT_BINDING_ADD_REPLY, ({ rmp->index = index; }));
}
...
static void vl_api_pnat_binding_del_t_handler(vl_api_pnat_binding_del_t *mp) {
    ...
    rv = pnat_del_translation(mp->index, mp->sw_if_index);
bad_sw_if_index:
