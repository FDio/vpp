...
static u32
det44_in2out_unknown_protocol (vlib_buffer_t *b0, u32 next0, ip4_header_t *ip0,
			       det44_main_t *dm, vlib_node_runtime_t *node,
			       u32 now, u32 thread_index, void *d, void *e)
{
  snat_det_out_key_t key0;
  snat_det_map_t *mp0 = 0;
  snat_det_session_t *ses0 = 0;
  ...
  if (PREDICT_FALSE (!ses0))
    {
      ...
      if (!ses0)
	ses0 =
	  snat_det_ses_create (thread_index, mp0, &ip0->src_address, 0, &key0);
    }
  if (PREDICT_FALSE (!ses0))
    {
				  ...
				  vlib_frame_t * frame)
{
  ...
  while (n_left_from >= 2)
	...
	{
	  next0 = det44_in2out_unknown_protocol (b0, next0, ip0, dm, node, now,
						 thread_index, &ses0, &mp0);
	  goto trace0;
	}
	...
	{
	  next1 = det44_in2out_unknown_protocol (b1, next1, ip1, dm, node, now,
						 thread_index, &ses1, &mp1);
	  goto trace1;
	}
