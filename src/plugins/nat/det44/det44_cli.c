					...
					vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	{
	  unformat (line_input, "inside vrf %u", &c.outside_vrf_id);
	  unformat (line_input, "outside vrf %u", &c.inside_vrf_id);
	  enable = 1;
	}
...
{
  unformat_input_t _line_input, *line_input = &_line_input;
  sw_if_indices_t *sw_if_indices, *p, e;
...
{
.path = "set det44 timeouts",.function =
    det44_set_timeouts_command_fn,.short_help =
...
{
.path = "show det44 timeouts",.function =
    det44_show_timeouts_command_fn,.short_help = "show det44 timeouts",};
...
{
.path = "det44 plugin",.function =
    det44_plugin_enable_disable_command_fn,.short_help =
...
{
.path = "set interface det44",.function =
    det44_feature_command_fn,.short_help =
