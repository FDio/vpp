...
static u32
det44_out2in_unknown_protocol (vlib_buffer_t * b0, u32 next0,
			       ip4_header_t * ip0, det44_main_t * dm,
			       vlib_node_runtime_t * node, void *d, void *e)
{
  snat_det_out_key_t key0;
  snat_det_map_t *mp0 = 0;
  snat_det_session_t *ses0 = 0;
  ...
  if (PREDICT_FALSE (!ses0))
    {
      det44_log_info ("no match src %U dst %U for user %U",
				  ...
				  vlib_frame_t * frame)
{
  ...
  while (n_left_from >= 2)
	...
	{
	  next0 =
	    det44_out2in_unknown_protocol (b0, next0, ip0, dm, node, &ses0,
					   &mp0);
	  goto trace0;
	}
	...
	{
	  next1 =
	    det44_out2in_unknown_protocol (b1, next1, ip1, dm, node, &ses1,
					   &mp1);
	  goto trace1;
	}
