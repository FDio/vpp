...
#ifndef CLIB_MARCH_VARIANT
u32
snat_icmp_hairpinning (snat_main_t * sm, vlib_buffer_t * b0,
			     ...
			     vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (snat_hairpinning
	      (vm, node, sm, b0, ip0, udp0, tcp0, proto0, 1 /* do_trace */ ))
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (is_hairpinning (sm, &ip0->dst_address)))
	      ...
	      if (proto0 == NAT_PROTOCOL_TCP || proto0 == NAT_PROTOCOL_UDP)
		{
		  ...
		  snat_hairpinning (vm, node, sm, b0, ip0, udp0, tcp0, proto0, 1 /* do_trace */ );
		}
