...
static u8 * format_nat44_in2out_reass_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "NAT44_IN2OUT_REASS: sw_if_index %d, next index %d, status %s, fragment id: 0x%x, fragment offset: %d",
              t->sw_if_index, t->next_index,
              t->cached ? "cached" : "translated",
              t->fragment_id, t->fragment_offset);
        ...
        trace0:
          if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE)
                            && (b0->flags & VLIB_BUFFER_IS_TRACED)))
            {
              ...
              t->sw_if_index = sw_if_index0;
              t->next_index = next0;
              t->fragment_id = (ip0 ? ntohs (ip0->fragment_id) : 0);
              t->fragment_offset = (ip0 ? (ntohs (ip0->flags_and_fragment_offset) & 0x1fff) * 8 : 0);
            }
