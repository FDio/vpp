			...
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "idle %u", &sm->idle_timeout))
	;
      else if (unformat (line_input, "udp %u", &sm->udp_timeout))
	{
			      ...
			      vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output (vm, "idle timeout: %dsec", sm->idle_timeout);
  // fix text
 ...
 *  vpp# set nat timeout idle <n> udp 120 tcp-established 7500 tcp-transitory 250 icmp 90
...
VLIB_CLI_COMMAND (set_timeout_command, static) = {
  ...
  .short_help =
    "set nat timeout [idle <sec> | udp <sec> | tcp-established <sec> "
