			       ...
			       vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output (vm, "total tcp sessions: %u", established);
  ...
  vlib_cli_output (vm, "total udp sessions: %u", established);
  vlib_cli_output (vm, "total icmp sessions: %u", established);
  return 0;
}
				...
				vlib_cli_command_t * cmd)
{
  ...
  if (unformat (input, "detail"))
    verbose = 1;
  else if (unformat (input, "metrics"))
    metrics = 1;
  ...
  vec_foreach_index (i, sm->per_thread_data)
    {
      ...
      if (!metrics)
      ...
      else
        {
          pool_foreach (u, tsm->users,
          ({
            vlib_cli_output (vm, "  %U", format_snat_user_v2, tsm, u,
