			 ...
			 unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (!mode_set && unformat (line_input, "endpoint-dependent"))
	{
      ...
      else if (unformat (input, "user-buckets %u", &c.user_buckets));
      else if (unformat (input, "user-memory %u", &c.user_memory));
      else if (unformat (input, "sessions %u", &c.sessions));
      else if (unformat (input, "session-buckets %u", &c.session_buckets));
  ...
  if (!(c.sessions && c.session_buckets && c.session_memory))
    {
      error =
	clib_error_return (0,
			   "either number of sessions or number of hash buckets and memory is required");
      goto done;
    }
