...
nat_lru_insert (snat_main_per_thread_data_t * tsm,
		       snat_session_t * s, f64 now, ed_protocol_t proto)
{
  ...
  switch (proto)
    ...
    case ED_PROTO_UDP:
      ...
      break;
    case ED_PROTO_TCP:
      s->lru_head_index = tsm->tcp_lru_head_index;
      break;
    case ED_PROTO_ICMP:
  ...
  clib_dlist_addtail (tsm->lru_pool, s->lru_head_index,
			  s->lru_index);
		      ...
		      ed_protocol_t proto)
{
  ...
  nat_lru_insert (tsm, s, now, proto);
