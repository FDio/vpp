...
format_nat_in2out_ed_trace (u8 * s, va_list * args)
{
  ...
  s =
    format (s,
	    ...
	    t->translation_error, format_nat_6t_flow, &t->i2of);
...
icmp_in2out_ed_slow_path (snat_main_t * sm, vlib_buffer_t * b,
			  ...
			  snat_session_t ** s_p, nat_6t_flow_t ** i2of_p)
{
					  ...
					  int is_output_feature)
{
  ...
  while (n_left_from > 0)
      ...
      if (PREDICT_FALSE (proto0 == NAT_PROTOCOL_OTHER))
	{
	  ...
	  if (!nat_6t_flow_translate_buf (sm, b0, ip0, i2of, proto0))
	    {
	      translation_error = 1;
	      goto trace0;
	    }
      ...
      if (!clib_bihash_search_16_8 (&sm->flow_hash, &kv0, &value0))
	{
	  ...
	  s0 = pool_elt_at_index (tsm->sessions, i2of->session_idx);
      ...
      if (!s0)
	{
	  ...
	  next[0] =
	    slow_path_ed (sm, b0, ip0->src_address, ip0->dst_address,
			  ...
			  vnet_buffer (b0)->ip.reass.l4_dst_port,
			  ip0->protocol, rx_fib_index0, &s0, &i2of, node,
			 ...
			 && (b0->flags & VLIB_BUFFER_IS_TRACED)))
	{
	  ...
	  if (i2of)
	    {
	      clib_memcpy (&t->i2of, i2of, sizeof (t->i2of));
	    }
