  ...
  while (n_left_from > 0)
    {
      ...
      u16 lookup_sport = vnet_buffer (b0)->ip.reass.l4_src_port;
      u16 lookup_dport = vnet_buffer (b0)->ip.reass.l4_dst_port;
      ...
      else
	{
	  ...
	  (translation_error =
	   nat_6t_flow_buf_translate (sm, b0, ip0, f, proto0,
				      is_output_feature)))
	{
	  nat_free_session_data (sm, s0, thread_index, 0);
	  nat_ed_session_delete (sm, s0, thread_index, 1);
					  ...
					  int is_output_feature)
{
  ...
  while (n_left_from > 0)
      ...
      if (PREDICT_FALSE (proto0 == NAT_PROTOCOL_OTHER))
	{
	  ...
	  if (NAT_ED_TRNSL_ERR_SUCCESS !=
	      (translation_error =
	       nat_6t_flow_buf_translate (sm, b0, ip0, &s0->i2o, proto0,
					  is_output_feature)))
	    {
	      goto trace0;
	    }
