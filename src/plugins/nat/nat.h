...
typedef struct ed_cuckoo_key_s
{
  struct
  {
    u32 src_address;
...
STATIC_ASSERT (STRUCT_SIZE_OF (ed_cuckoo_kv_t, k) <=
	       STRUCT_SIZE_OF (ed_cuckoo_kv_t, kv.key),
	       "ed key needs to fit in cuckoo key");
...
typedef struct snat_main_s
{
  ...
  clib_cuckoo_16_8_t ed_external_ports;
} snat_main_t;
