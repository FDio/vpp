...
#define init_simple_counter(c, n, sn) \
do                                        \
  {                                       \
    c.name = n;                           \
...
nat44_plugin_enable (nat44_config_t c)
{
  ...
  sm->connection_tracking = c.connection_tracking;
  ...
  sm->max_translations_per_user
    = c.user_sessions ? c.user_sessions : sm->max_translations;

  sm->outside_vrf_id = c.outside_vrf_id;
  sm->outside_fib_index =
    fib_table_find_or_create_and_lock
    (FIB_PROTOCOL_IP4, c.outside_vrf_id, nat_fib_src_hi);

  sm->inside_vrf_id = c.inside_vrf_id;
  sm->inside_fib_index =
    fib_table_find_or_create_and_lock
    (FIB_PROTOCOL_IP4, c.inside_vrf_id, nat_fib_src_hi);
  ...
  if (c.endpoint_dependent)
    {
      ...
      clib_bihash_init_16_8 (&sm->out2in_ed, "out2in-ed", translation_buckets,
  ...
  if (!c.static_mapping_only
      || (c.static_mapping_only && c.connection_tracking))
    {
      ...
      vec_foreach (tsm, sm->per_thread_data)
        {
          nat44_db_init (tsm, c.endpoint_dependent);
        }
      /* *INDENT-ON* */
...
nat44_plugin_disable ()
{
  ...
  if (!sm->static_mapping_only ||
      (sm->static_mapping_only && sm->connection_tracking))
    {
      snat_main_per_thread_data_t *tsm;
     /* *INDENT-OFF* */
      vec_foreach (tsm, sm->per_thread_data)
        {
          nat44_db_free (tsm, sm->endpoint_dependent);
        }
      /* *INDENT-ON* */
    }
...
nat_init (vlib_main_t * vm)
{
  ...
  u32 *p, *bitmap = 0, i, num_threads = 0;
  ...
  // enable/disable should not reset these parameters
  ...
  // should these be reset to default ?
  ...
  nat_init_simple_counter (sm->created_sessions, "created-sessions",
			   "/nat44/created-sessions");
