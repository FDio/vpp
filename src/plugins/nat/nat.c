                             ...
                             vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (!acl_enable_set && unformat (line_input, "enable"))
        {
          acl_enable = 1;
          acl_enable_set = 1;
        }
      else if (!acl_enable_set && unformat (line_input, "disable"))
        {
          acl_enable = 0;
          acl_enable_set = 1;
        }
...
/*?
 * @cliexpar
 * @cliexstart{nat44 acl}
 * Enable or disable ACL
 * To enable ACL, use:
 *  vpp# nat44 acl enable
 * To disable ACL, use:
 *  vpp# nat44 acl disable
 * @cliexend
?*/
VLIB_CLI_COMMAND (snat_acl_set_command, static) = {
  .path = "nat44 acl",
  .short_help = "nat44 acl enable|disable",
