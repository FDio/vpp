...
nat_6t_l3_l4_csum_calc (nat_6t_flow_t * f)
{
  ...
  if (f->ops & NAT_FLOW_OP_SPORT_REWRITE
    ...
    {
      f->l4_csum_delta =
	ip_csum_add_even (f->l4_csum_delta, f->rewrite.sport);
  ...
  if (f->ops & NAT_FLOW_OP_DPORT_REWRITE
    ...
    {
      f->l4_csum_delta =
	ip_csum_add_even (f->l4_csum_delta, f->rewrite.dport);
  ...
  if (f->ops & NAT_FLOW_OP_ICMP_ID_REWRITE
	...
	ip_csum_add_even (f->l4_csum_delta, f->rewrite.icmp_id);
      f->l4_csum_delta =
	ip_csum_sub_even (f->l4_csum_delta, f->match.icmp_id);
    }
  else
    {
      f->rewrite.icmp_id = f->match.icmp_id;
...
nat_6t_flow_ip4_translate (snat_main_t * sm, vlib_buffer_t * b,
			   ip4_header_t * ip, nat_6t_flow_t * f,
			   nat_protocol_t proto, int is_icmp_inner_ip4)
{
      ...
      if (!icmp_type_is_error_message (icmp->type))
	{
	  if ((f->ops & NAT_FLOW_OP_ICMP_ID_REWRITE) &&
	      (f->rewrite.icmp_id != echo->identifier))
	    {
	      ...
	      sum =
		ip_csum_update (sum, echo->identifier, f->rewrite.icmp_id,
				icmp_echo_header_t,
...
nat_6t_flow_buf_translate (snat_main_t * sm, vlib_buffer_t * b,
			   ip4_header_t * ip, nat_6t_flow_t * f,
			   nat_protocol_t proto, int is_output_feature)
{
  ...
  return nat_6t_flow_ip4_translate (sm, b, ip, f, proto,
				    0 /* is_icmp_inner_ip4 */ );
}
...
format_nat_6t_flow (u8 * s, va_list * args)
{
  ...
  if (f->ops & NAT_FLOW_OP_ICMP_ID_REWRITE)
      ...
      if (!r)
	{
      ...
      s =
	format (s, "icmp-id %u ", clib_net_to_host_u16 (f->rewrite.icmp_id));
    }
  if (f->ops & NAT_FLOW_OP_TXFIB_REWRITE)
    {
      if (!r)
