...
static u8 * format_nat44_classify_trace (u8 * s, va_list * args)
{
  ...
  if (t->cached)
    s = format (s, "nat44-classify: cached");
  else
    {
          ...
          if (is_ed && ip0->protocol != IP_PROTOCOL_ICMP)
            {
              if (!ip4_is_fragment (ip0) || ip4_is_first_fragment (ip0))
                {
                  /* process leading fragment/packet (with L4 header) */
		      ...
		      if (PREDICT_FALSE (!reass0))
		        {
		          ...
		          goto enqueue0;
                      ...
                      if (!clib_bihash_search_16_8 (&tsm->in2out_ed, &ed_kv0, &ed_value0))
                        {
                          /* if session exists for this first fragment
                           * then classify it, save classification for future
                           * fragments and set past fragments to be looped over
                           * and reprocessed */
                        ...
                        {
                          /* session doesn't exist so save this information
                           * for future fragments and set past
                           * fragments to be looped over and reprocessed,
                           * continue in code to potentially find
                           * other classification/translation for this packet */
                    ...
                    {
                      /* we got whole packet, classify it */
                      if (!clib_bihash_search_16_8 (&tsm->in2out_ed, &ed_kv0, &ed_value0))
                        {
                          s0 = pool_elt_at_index (tsm->sessions, ed_value0.value);
                          if (s0->in2out.addr.as_u32 != ip0->src_address.as_u32)
                            next0 = NAT44_CLASSIFY_NEXT_OUT2IN;
                          else
                            next0 = NAT44_CLASSIFY_NEXT_IN2OUT;
              ...
              else
                {
                  ...
                  if (reass0->sess_index == ~0 && !reass0->classify_dont_translate)
                    {
		      ...
		      goto enqueue0;
                  ...
                  /* classify based on the classification of first fragment */
                  ...
                  /* first fragment must have been classified as dont_translate
                   * so keep the default next0 and continue in code to
                   * potentially find other classification/translation
                   * for this packet */
                }
            }
