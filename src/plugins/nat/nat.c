...
VNET_FEATURE_INIT (nat_pre_in2out_handoff, static) = {
  .arc_name = "ip4-unicast",
  .node_name = "nat-pre-in2out-handoff",
  .runs_after = VNET_FEATURES ("acl-plugin-in-ip4-fa"),
};
VNET_FEATURE_INIT (nat_pre_out2in_handoff, static) = {
  .arc_name = "ip4-unicast",
  .node_name = "nat-pre-out2in-handoff",
  .runs_after = VNET_FEATURES ("acl-plugin-in-ip4-fa",
                               "ip4-dhcp-client-detect"),
};
...
snat_interface_add_del (u32 sw_if_index, u8 is_inside, int is_del)
{
  ...
  if (sm->static_mapping_only && !(sm->static_mapping_connection_tracking))
	...
	feature_name =
	  is_inside ? "nat-pre-in2out--handoff" : "nat-pre-out2in-handoff";
    ...
    if (i->sw_if_index == sw_if_index)
      {
        if (is_del)
                ...
                if (sm->num_workers > 1 && !sm->deterministic)
                  {
                    ...
                    feature_name = !is_inside ?  "nat-pre-in2out-handoff" :
                                                 "nat-pre-out2in-handoff";
                  }
                else if (sm->deterministic)
                  {
        ...
        else
          {
            ...
            if (sm->num_workers > 1 && !sm->deterministic)
              {
                del_feature_name = !is_inside ?  "nat-pre-in2out-handoff" :
                                                 "nat-pre-out2in-handoff";
...
VLIB_REGISTER_NODE (nat_default_node) = {
  ...
  .next_nodes = {
    ...
    [NAT_NEXT_IN2OUT_HANDOFF] = "nat44-in2out-worker-handoff",
    [NAT_NEXT_IN2OUT_OUTPUT_HANDOFF] = "nat44-in2out-output-worker-handoff",
    ...
    [NAT_NEXT_OUT2IN_HANDOFF] = "nat44-out2in-worker-handoff",
