		      ...
		      f64 now)
{
  ...
  if (now >= sess_timeout_time ||
    ...
    alloc_new:
      do
	{
#define p_nat44_session_alloc_new(x) p_nat44_session_alloc_new##x
	  PROFILE_DECL (p_nat44_session_alloc_new,
			"nat44_session_alloc_new", 5);

	  PROFILE_START (p_nat44_session_alloc_new);
	  s = nat44_session_alloc_new (tsm, u, now);
	  PROFILE_END (p_nat44_session_alloc_new);
	}
      while (0);
      vlib_set_simple_counter (&sm->total_sessions, thread_index, 0,
			       pool_elts (tsm->sessions));
