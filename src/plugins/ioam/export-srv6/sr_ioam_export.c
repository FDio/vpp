				      ...
				      ip4_address_t * src_address)
{
  ...
  if (is_disable == 0)
      ...
      if (em->my_hbh_slot == ~0)
	{
	  ...
	  sr_decap_ioam_node =
	    vlib_get_node_by_name (vm, (u8 *) "sr-decap-ioam-v6");
	  ...
	  em->my_hbh_slot =
	    vlib_node_add_next (vm, sr_decap_ioam_node->index,
				node_index);
	}
    ...
    {
      //sr_set_next_override (SR_DECAP_IOAM_V6_NEXT_POP);
					    ...
					    vlib_cli_command_t * cmd)
{
  ...
  //1, 0);
  if (0 !=
      sr_ioam_export_enable_disable (em, is_disable, &collector, &src))
    {
