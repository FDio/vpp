...
u8 *
sr_ioam_compute_rewrite_encap (u8 * rewrite_buf, u8 tlv_type1)
{
  ip6_header_t *ip6;
  ip6_sr_header_t *srh;
  ...
  ip6 = (ip6_header_t *) rewrite_buf;
  ip6_ext_header_find_t (ip6, prev0, srh, IP_PROTOCOL_IPV6_ROUTE);

  copy_length = header_length = sizeof(ip6_header_t ) + ((srh->length + 1) << 3);
  srh_length = (srh->length + 1) << 3;
...
sr_ioam_compute_rewrite_insert (u8 * rewrite_buf, u8 tlv_type1)
{
  ...
  copy_length += sizeof (ip6_sr_header_t);
  copy_length += (srh->first_segment + 1) * sizeof (ip6_address_t);
