...
int
sr_ioam_trace_rewrite_handler (u8 * rewrite_string,
					   u8 * rewrite_size)
{
				      ...
				      ip6_sr_tlv_header_t * opt)
{
  ...
  if (trace->trace_hdr.ioam_trace_type & BIT_LOOPBACK_REPLY)
    {
      sr_ioam_trace_stats_increment_counter (SR_IOAM_TRACE_LOOPBACK_REPLY,
					      1);
      return rv;
    }
