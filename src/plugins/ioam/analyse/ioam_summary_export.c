...
#define _(field, mask, item, length)                                          \
  {                                                                           \
    field_count++;                                                            \
    fr->fields_to_send =                                                      \
      clib_bitmap_set (fr->fields_to_send, field_index, 1);                   \
  }                                                                           \
  field_index++;
...
#undef _
  ...
  /* Add Src address, dest address, src port, dest port
   * path map,  number of paths manually */
  f->e_id_length = ipfix_e_id_length (0 /* enterprise */, sourceIPv6Address,
				      sizeof (ip6_address_t));
  f++;

  f->e_id_length = ipfix_e_id_length (
    0 /* enterprise */, destinationIPv6Address, sizeof (ip6_address_t));
  f++;
...
#undef _
  ...
  f->e_id_length =
    ipfix_e_id_length (0 /* enterprise */, ioamNumberOfPaths, 2);
  f++;

  /* Add ioamPathMap manually */
  f->e_id_length = ipfix_e_id_length (
    0 /* enterprise */, ioamPathMap,
...
u16
ioam_analyse_add_ipfix_record (flow_report_t *fr, ioam_analyser_data_t *record,
...
#define _(field, mask, item, length)                                          \
  if (clib_bitmap_get (fr->fields_to_send, field_index))                      \
    {                                                                         \
      /* Expect only 4 bytes */                                               \
      u32 tmp;                                                                \
      tmp = clib_host_to_net_u32 ((u32) record->field -                       \
				  (u32) record->chached_data_list->field);    \
	  ...
	  if (0 < (trace->pkt_counter - trace_cached->pkt_counter))
	    {
	      ...
	      u64 old_sum = trace_cached->mean_delay *
...
vlib_frame_t *
ioam_send_flows (flow_report_main_t *frm, flow_report_t *fr, vlib_frame_t *f,
  ...
  vec_foreach_index (i, aggregated_data)
      ...
      if (!record->is_free)
	{
	  ...
	  next_offset = ioam_analyse_add_ipfix_record (
	    fr, record, b0, next_offset, &temp, &temp, 0, 0);
	  records_this_buffer++;
