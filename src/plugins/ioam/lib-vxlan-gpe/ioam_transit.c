...
static uword
vxlan_gpe_transit_ioam (vlib_main_t *vm, vlib_node_runtime_t *node,
			vlib_frame_t *from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE ((ip0->ip_version_and_header_length & 0xF0) ==
			     0x40))
	    {
	      /* ipv4 packets */
				  ...
				  UDP_DST_PORT_VXLAN_GPE)))
		{
		  ...
		  if (PREDICT_FALSE (gpe_hdr0->protocol ==
				     VXLAN_GPE_PROTOCOL_IOAM))
		    {
		      uword *t = NULL;
		      ...
		      if (t)
			{
			  ...
			  vlib_buffer_advance (
			    b0, (word) (sizeof (ethernet_header_t)));
