		       ...
		       vlib_buffer_t *b0)
{
  ...
  vlib_buffer_advance (
    b0, (word) (sizeof (udp_header_t) + sizeof (ip4_header_t) +
		sizeof (vxlan_gpe_header_t) + gpe_ioam0->length));

  return;
}

always_inline void
vxlan_gpe_pop_ioam_v4_one_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
				  ...
				  u32 *next0)
{
  ...
  *next0 = (gpe_ioam0->protocol < VXLAN_GPE_PROTOCOL_MAX) ?
	     ngm->decap_next_node_list[gpe_ioam0->protocol] :
...
always_inline void
vxlan_gpe_pop_ioam_v4_two_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
				  vxlan_gpe_main_t *ngm, vlib_buffer_t *b0,
...
static uword
vxlan_gpe_pop_ioam (vlib_main_t *vm, vlib_node_runtime_t *node,
...
static uword
vxlan_gpe_pop_ioam_v4 (vlib_main_t *vm, vlib_node_runtime_t *node,
