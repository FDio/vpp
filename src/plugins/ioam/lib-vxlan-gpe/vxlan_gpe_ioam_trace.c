...
int
vxlan_gpe_ioam_add_register_option (u8 option, u8 size,
...
vxlan_gpe_ioam_trace_rewrite_handler (u8 *rewrite_string, u8 *rewrite_size)
{
  ...
  *rewrite_size = sizeof (vxlan_gpe_ioam_trace_option_t) +
		  (trace_option_elts * trace_data_size);
	  ...
	  if (trace->ioam_trace_type & BIT_TTL_NODEID)
	    {
	      ...
	      /* The transit case is the only case where the TTL decrement
	       * happens before iOAM processing. For now, use the use_adj flag
	       * as an overload. We can probably use a separate flag instead of
	       * overloading the use_adj flag.
	       */
...
static clib_error_t *
vxlan_gpe_show_ioam_trace_cmd_fn (vlib_main_t *vm, unformat_input_t *input,
...
vxlan_gpe_ioam_trace_get_sizeof_handler (u32 *result)
{
  ...
  *result = 0;
  ...
  size += sizeof (vxlan_gpe_ioam_trace_option_t) +
