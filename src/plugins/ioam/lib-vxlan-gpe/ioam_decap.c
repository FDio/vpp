...
always_inline void
vxlan_gpe_decap_ioam_v4_two_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
				    ...
				    vlib_buffer_t *b1, u32 *next0, u32 *next1)
{
  ...
  *next0 = *next1 = hm->decap_v4_next_override;
  vxlan_gpe_encap_decap_ioam_v4_one_inline (
    vm, node, b0, next0, VXLAN_GPE_DECAP_IOAM_V4_NEXT_DROP, 0 /* use_adj */);
  vxlan_gpe_encap_decap_ioam_v4_one_inline (
    vm, node, b1, next1, VXLAN_GPE_DECAP_IOAM_V4_NEXT_DROP, 0 /* use_adj */);
}

static uword
vxlan_gpe_decap_ioam (vlib_main_t *vm, vlib_node_runtime_t *node,
		      vlib_frame_t *from_frame, u8 is_ipv6)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  vlib_buffer_advance (b0, -(word) (sizeof (udp_header_t) +
					    sizeof (ip4_header_t) +
					    sizeof (vxlan_gpe_header_t)));
	  vlib_buffer_advance (b1, -(word) (sizeof (udp_header_t) +
					    sizeof (ip4_header_t) +
					    sizeof (vxlan_gpe_header_t)));

	  vxlan_gpe_decap_ioam_v4_two_inline (vm, node, ngm, b0, b1, &next0,
					      &next1);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  vxlan_gpe_encap_decap_ioam_v4_one_inline (
	    vm, node, b0, &next0, VXLAN_GPE_DECAP_IOAM_V4_NEXT_DROP,
	    0 /* use_adj */);
...
static uword
vxlan_gpe_decap_ioam_v4 (vlib_main_t *vm, vlib_node_runtime_t *node,
