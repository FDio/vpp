 ...
 * Incremental - Node allocates and pushes telemetry data following option
 * header PoT - Proof of transit E2E - Edge-to-Edge A nodes can use any of
 * these simultaneously, incremental must precede prealloc
 */
 ...
 * Encapsulation node - A node that adds at least one iOAM option and/or iOAM
 * data as packet enters iOAM domain Transit node - Adds iOAM data within an
 * iOAM domain based on the provided iOAM option type. DOES NOT add new or
 * alter existing option types Decapsulation node - In charge of removing iOAM
 * option type and data before leaving iOAM domain
 */
 ...
 * iOAM Prealloc and Incremental Option Header
	 ...
	 +                       Node Data List [n]                      +
	 +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * namespace_id (16-bit) - unique id which is used by the Encap, Tranist, and
 Decap nodes
 *             in order to identify what iOAM options/data to alter/add/remove
 etc.
 *             Allows for a namespace specif interpretation of iOAM data
 fields.
 *             A subset of all the iOAM option types and data fields are
 associated
 ...
 *
 * node_len (5-bit) - specified the length of data added by each no in
 multiples of 4-octets,
 *                    excluding the length of the "Opaque State Snapshot"
 field.
 *                    Set by Encap.
 *
 ...
 *                 Bit 0 "Overflow" (MSB). set if Number of iOAM nodes to
 aggregate their telemetry
 *                 data is greater than the PMTU.
 *
 * remaining_len (7-bit) - specifies the data space in multiples of 4-octets
 remaining for
 *      recording the node data, before the node data list is considered to
 have overflowed.
 *      Can be set to MTU (PMTU) in order to compare with node_len how much
 space there is left.
 *      In prealloc, this can be used as index for data array, = remaining_len
 - node_len
 *
 ...
 *
 * Reserved (8-bit) - Encap must set the value to 0s and ignored by transit
 nodes
 *
 * Node data list (variable length field) - data elements determined by trace
 type
 *      Order of data followed Trace Type bit order (below). If node is not
 able to
...
// Trace overflow flag, but also used to indicate if we need to add transit
// delay
...
#define IOAM_SET_TRACETYPE(tt)                                                \
  ((u32) (clib_host_to_net_u32 ((tt & IOAM_INSTR_BITMAP_MASK) << 8)))
#define IOAM_GET_TRACETYPE(tt)                                                \
  ((u32) ((clib_net_to_host_u32 (tt) >> 8) & IOAM_INSTR_BITMAP_MASK))
...
#define IOAM_MAX_OPAQUE_DATA_BYTE_SIZE                                        \
  ((u16) (IOAM_MAX_OPAQUE_DATA_WORD_SIZE << 2))
...
#define IOAM_SET_OPAQUE_HEADER(len, schemaid)                                 \
  ((u32) ((((len) << 24) & IOAM_OPAQUE_LEN_MASK) |                            \
	  (schemaid & IOAM_OPAQUE_SCHEMEID_MASK)))
...
#define IOAM_MAX_DATA_W_OPAQUE                                                \
  ((u16) (IOAM_MAX_DATA_NO_OPAQUE + 4 + IOAM_MAX_OPAQUE_DATA_BYTE_SIZE))
...
fetch_trace_data_size (trace_profile *profile)
{
  ...
  if (trace_type & IOAM_BIT_VAR_LEN_OP_ST_SNSH)
    {
      trace_data_size +=
	4; // for opaque header, data is variable length but max 255*4 bytes
      trace_data_size += IOAM_GET_OPAQUE_LEN (profile->opaque.len_schemeid)
			 << 2;
    }
...
ioam_print_profile (trace_profile *profile)
{
  if (profile)
	...
	"option-type %d\n - trace-type 0x%x\n - node-type %d\n - "
	"ts-format-sub %d\n - opaque-len %d\n - opaque-id %d\n",
	profile->namespace_id, profile->num_elts, profile->node_id_short,
	profile->app_data_short, profile->node_id_wide, profile->app_data_wide,
	profile->option_type, profile->trace_type, profile->node_type,
	profile->ts_format,
