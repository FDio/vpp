...
trace_profile_cleanup (trace_profile *profile)
{
  ...
  ip6_trace_profile_cleanup (); /* lib-trace_TODO: Remove this once IOAM-IPv6
				   transport is a plugin */
  return 0;
}
			      ...
			      vlib_cli_command_t *cmd)
{
  ...
  if (profile)
    {
      trace_profile_create (profile, trace_type, num_elts, trace_tsp, node_id,
			    app_data);
    }
  return 0;
}
  ...
  s =
    format (s, "         Trace timestamp precision : %d (%s)\n", p->trace_tsp,
	    (p->trace_tsp == TSP_SECONDS) ?
		 ...
		 "Milliseconds" :
		 (((p->trace_tsp == TSP_MICROSECONDS) ? "Microseconds" :
							"Nanoseconds"))));
