...
trace_profile_create (trace_profile *profile, trace_profile *user_defined)
{
  if (profile && !profile->valid)
      ...
      if (user_defined->opaque.data)
	{
	  ...
	  u32 olen = IOAM_GET_OPAQUE_LEN (
	    user_defined->opaque.len_schemeid); // Represented as 4 octets
	  u32 i;
	  for (i = 0; i < olen; i++)
	    {
	      // Freaking endianess
			       ...
			       vlib_cli_command_t *cmd)
{
  ...
  s =
    format (s, "        Timestamp precision : %d (%s)\n", p->ts_format,
	    (p->ts_format == IOAM_TSP_SECONDS) ?
	      "Seconds" :
	      ((p->ts_format == IOAM_TSP_MILLISECONDS) ?
		 "Milliseconds" :
		 (((p->ts_format == IOAM_TSP_MICROSECONDS) ? "Microseconds" :
							     "Nanoseconds"))));
  s = format (s, "        Num of trace nodes  : %d\n", p->num_elts);
  s = format (s, "        Node-ID-Short/Type  : %d / %s\n", p->node_id_short,
	      (p->node_type == IOAM_NODE_ENCAP) ?
		"Encap" :
		((p->node_type == IOAM_NODE_TRANSIT) ? "Transit" : "Decap"));
  s = format (s, "        App Data-Short      : 0x%x\n", p->app_data_short);
  s = format (s, "        Node-ID-Wide/Type   : 0x%Lx / %s\n", p->node_id_wide,
	      (p->node_type == IOAM_NODE_ENCAP) ?
		"Encap" :
		((p->node_type == IOAM_NODE_TRANSIT) ? "Transit" : "Decap"));
  s = format (s, "        App Data-Wide       : 0x%Lx\n", p->app_data_wide);
  s = format (s, "        Queue Depth-Type    : %s\n",
	      (p->queue_depth_type == QUEUE_DEPTH_AF_PACKET) ?
		"AF_PACKET" :
  ...
  if (IOAM_GET_OPAQUE_LEN (p->opaque.len_schemeid))
    {
      ...
      s = format (s, "        Opaque Data         : \n        %U%U",
		  format_white_space, indent, format_hex_bytes, p->opaque.data,
		  IOAM_GET_OPAQUE_LEN (p->opaque.len_schemeid) << 2);
    }
  vlib_cli_output (vm, "%v", s);
  vec_free (stmp);
  ...
  if ((*result & IOAM_OPTION_PREALLOC) && (*result & IOAM_OPTION_INCREMENT))
    {
      vlib_cli_output (vlib_get_main (),
		       "WARNING: option-type (0x%x) contains both prealloc "
		       "and increment, defaulting to prealloc",
		       *result);
      *result &= ~(IOAM_OPTION_INCREMENT);
    }
  if (*result == 0)
...
trace_util_check_input (u8 *inerror, va_list *args)
{
  ...
  u8 all_options = IOAM_OPTION_PREALLOC | IOAM_OPTION_INCREMENT |
		   IOAM_OPTION_POT | IOAM_OPTION_E2E;
  if (input->option_type & ~all_options)
    {
      inerror = format (inerror, " - ERROR: Invalid option-type - [prealloc | "
				 "increment | pot | e2e]\n");
    }
  if (input->option_type == 0)
    {
      vlib_cli_output (
	vm, "WARNING: No option-type chosen, defaulting to prealloc");
  ...
  if ((input->node_type & ~all_nodes) | !input->node_type)
    {
      inerror = format (
	inerror, " - ERROR: Invalid node-type - [encap | transit | decap]\n");
    }
  if (!input->num_elts)
    {
  ...
  if ((input->trace_type & IOAM_BIT_TIMESTAMP_SUB_SEC) &&
    ...
    {
      inerror = format (
	inerror, " - ERROR: ts-format-sub must be - [sec | ms | us | ns]\n");
    }
  ...
  if ((in_olen && in_oid) && (input->trace_type & IOAM_BIT_VAR_LEN_OP_ST_SNSH))
	...
	{
	  vlib_cli_output (vm,
			   "WARNING: opaque-len (%d) != len(opaque-data) (%d) "
			   "octets - setting to (%d)",
	...
	{
	  vlib_cli_output (
	    vm, "WARNING: in_olen (%d) > max (%d) octets - using max length",
      ...
      input->opaque.len_schemeid = IOAM_SET_OPAQUE_HEADER (in_olen, in_oid);
	   ...
	   (!(input->trace_type & IOAM_BIT_VAR_LEN_OP_ST_SNSH) &&
	    (in_olen || in_oid)))
    {
      inerror = format (inerror,
			"ERROR: Opaque, check - opaque bit in trace-type "
			"(0x%x), opaque-len (%d) opaque-id (%d)",
			input->trace_type & IOAM_BIT_VAR_LEN_OP_ST_SNSH,
			in_olen, in_oid);
    }
  return inerror;
			      ...
			      vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (input, "node-id-short %d",
			 &user_defined.node_id_short))
	;
      else if (unformat (input, "node-id-wide 0x%Lx",
			 &user_defined.node_id_wide))
	;
      else if (unformat (input, "app-data-short 0x%x",
			 &user_defined.app_data_short))
	;
      else if (unformat (input, "app-data-wide 0x%Lx",
			 &user_defined.app_data_wide))
	;
      else if (unformat (input, "option-type %U", unformat_option_type,
			 &user_defined.option_type))
	;
      else if (unformat (input, "trace-type 0x%x", &user_defined.trace_type))
	;
      else if (unformat (input, "node-type %U", unformat_node_type,
			 &user_defined.node_type))
	;
      else if (unformat (input, "ts-format-sub %U", unformat_ts_format,
			 &user_defined.ts_format))
	;
      else if (unformat (input, "queue-depth-type %U",
			 unformat_queue_depth_format,
	...
	;
      else if (unformat (input, "opaque-data 0x%U", unformat_hex_string,
  ...
  u8 *errstr = 0;
  errstr = format (errstr, "%U", trace_util_check_input, vm, &user_defined,
		   &olen, &oid);
  if (!errstr)
    {
...
VLIB_CLI_COMMAND (set_trace_profile_command, static) = {
  ...
  .short_help =
    ...
    "namespace-id [n] num-elts [< 255] node-id-short [n > 0 (dec)] "
    "node-id-wide [n > 0x0 (hex)] queue-depth-type (if queue depth in "
    "trace-type) [drv-af-packet | drv-dpdk] node-type [encap | transit | "
    "decap]\n    Optional:\n     option-type [prealloc (default) | increment "
    "| pot | e2e] ts-format-sub [sec | ms | us | ns] app-data-short/-wide [in "
    "hex] opaque-len [n < 1020-bytes] opaque-id [n] opaque-data [hex]",
