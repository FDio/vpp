 ...
 *     look up cache entry.
 *   - locking/synchronization of cache:
 *      - Pool and cache entry access must be synchronized between
 *        threads that process SYN and create cache entry,
 *        threads that receive SYN-ACK, lookup, process and free
 *        cache entries, and cache cleanup on timeout thread.
 *
 *      - 2 levels of lock - per pool/thread lock and per entry
 *        lock is defined. Per pool lock is used when pool_put and
 *        pool_get is performed - i.e when creating and freeing entry.
 *        Cache entry level lock is used to protect cache entry update
 *        and read operations. Cache entry is updates when a response
 *        SYN-ACK is evaluated to be better than the one that is
...
inline static void ioam_cache_sr_rewrite_template_create (void)
{
  ...
  (void)ip6_sr_create_rewrite_string(a, &cm->sr_rewrite_template);
  vec_free (segments);
  ...
  if (entry)
      ...
      if (entry->hbh != 0)
	{
	  ...
	  vlib_put_frame_to_node(vm, node_index, nf);
       ...
       // to ensure, entry is marked as free before releasing the lock
       memset(entry, 0, sizeof(*entry));
    }
}
...
// Should be called with cache ts entry locked
						 ...
						 i32 pool_index)
{
  ...
  if (entry && entry->hbh)
	...
	{
          // release the entry lock before acquiring thread level lock
          entry->lock = 0;
          while (__sync_lock_test_and_set (cm->lockp_ts[thread_id], 1))
            ;
...
ip6_ioam_ts_cache_set_rewrite (void)
{
  ...
  e2e_id->hdr.type = HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE_ID
    | HBH_OPTION_TYPE_SKIP_UNKNOWN;
  e2e_id->hdr.length = sizeof (ioam_e2e_id_option_t) -
    sizeof (ip6_hop_by_hop_option_t);
  e2e_id->e2e_type = 1;
