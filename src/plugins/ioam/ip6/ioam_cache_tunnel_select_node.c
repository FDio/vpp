	          ...
	          (tcp0->flags & TCP_FLAG_ACK) == TCP_FLAG_ACK)
		{
		  ...
		  if (0 == ioam_cache_ts_lookup (ip0,
			...
			if (0 == (result = ioam_cache_ts_update (cache_thread_id,
		      ...
		      ioam_cache_ts_entry_unlock(cache_thread_id,
						 cache_ts_index);
...
#define foreach_ip6_ioam_cache_ts_input_next    \
  _(IP6_LOOKUP_MULTICAST, "ip6-mfib-forward-lookup")	\
			    ...
			    vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  u8 no_of_responses0 = 0, no_of_responses1 = 0 ;
	  ...
	  next0 = next1 = IP6_IOAM_CACHE_TS_INPUT_NEXT_IP6_LOOKUP_MULTICAST;
	  ...
	  if ((tcp0->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN &&
	    ...
	    {
	      no_of_responses0 = ip6_sr_multicastmap_get_count(&ip0->dst_address);
              if (no_of_responses0 > 0)
		{
	...
	NEXT00:
	  ...
	  if ((tcp1->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN &&
	    ...
	    {
	      no_of_responses1 = ip6_sr_multicastmap_get_count(&ip1->dst_address);
              if (no_of_responses1 > 0)
		{
		  /* Create TS select entry */
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  next0 = IP6_IOAM_CACHE_TS_INPUT_NEXT_IP6_LOOKUP_MULTICAST;
	  /* speculatively enqueue b0 to the current next frame */
	  bi0 = from[0];
	  to_next[0] = bi0;
	  from += 1;
	  ...
	  if ((tcp0->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN &&
	      (tcp0->flags & TCP_FLAG_ACK) == 0)
	    {
	      no_of_responses = ip6_sr_multicastmap_get_count(&ip0->dst_address);
