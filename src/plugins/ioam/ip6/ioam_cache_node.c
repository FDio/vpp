				...
				vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  _clib_memcpy (hbh0, rewrite, rewrite_len);
	  _clib_memcpy (srh0, cm->sr_rewrite_template, sr_rewrite_len);
	  /* Copy dst address into the DA slot in the segment list */
	  _clib_memcpy (srh0->segments, ip0->dst_address.as_u64,
			sizeof (ip6_address_t));
	  /* Rewrite the ip6 dst address with the first hop */
	  _clib_memcpy (ip0->dst_address.as_u64, entry->next_hop.as_u64,
			sizeof (ip6_address_t));
	  _clib_memcpy (&srh0->segments[1],
