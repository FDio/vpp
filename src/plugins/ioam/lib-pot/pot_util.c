...
int
pot_profile_create (pot_profile *profile, u64 prime, u64 poly2, u64 lpc,
		    u64 secret_share)
{
...
always_inline u64
pot_update_cumulative_inline (u64 cumulative, u64 random, u64 secret_share,
			      u64 prime, u64 lpc, u64 pre_split,
			      double prime_inv)
{
  u64 share_random = 0;
...
always_inline u8
pot_validate_inline (u64 secret, u64 prime, double prime_inv, u64 cumulative,
		     u64 random)
{
...
clib_error_t *
clear_pot_profile_command_fn (vlib_main_t *vm, unformat_input_t *input,
...
static clib_error_t *
set_pot_profile_command_fn (vlib_main_t *vm, unformat_input_t *input,
			    vlib_cli_command_t *cmd)
{
  u64 prime;
  u64 secret_share;
...
static clib_error_t *
set_pot_profile_activate_command_fn (vlib_main_t *vm, unformat_input_t *input,
				     vlib_cli_command_t *cmd)
{
  pot_main_t *sm = &pot_main;
  u8 *profile_list_name = NULL;
  ...
  if (!pot_profile_list_is_enabled (profile_list_name))
    {
      result =
	clib_error_return (0, "%s list is not enabled, profile in use %s",
			   profile_list_name, sm->profile_list_name);
    }
  else if (0 != pot_profile_set_active ((u8) id))
    {
      result = clib_error_return (0, "Profile %d not defined in %s", id,
...
static clib_error_t *
show_pot_profile_command_fn (vlib_main_t *vm, unformat_input_t *input,
			     vlib_cli_command_t *cmd)
{
  pot_main_t *sm = &pot_main;
  pot_profile *p = NULL;
  ...
  for (i = 0; i < MAX_POT_PROFILES; i++)
	...
	s = format (s, "         Secret key : 0x%Lx (%Ld)\n", p->secret_key,
      ...
      s = format (s, "       Secret share : 0x%Lx (%Ld)\n", p->secret_share,
		  p->secret_share);
      s =
	format (s, "       Prime number : 0x%Lx (%Ld)\n", p->prime, p->prime);
