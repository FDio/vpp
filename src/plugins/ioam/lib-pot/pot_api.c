...
static void
vl_api_pot_profile_add_t_handler (vl_api_pot_profile_add_t *mp)
{
  ...
  if (profile)
    {
      rv = pot_profile_create (profile, clib_net_to_host_u64 (mp->prime),
	...
	goto ERROROUT;
      if (1 == mp->validator)
	(void) pot_set_validator (profile,
...
static void
send_pot_profile_details (vl_api_pot_profile_show_config_dump_t *mp, u8 id)
{
  ...
  if (profile)
    {
      REPLY_MACRO2 (
	VL_API_POT_PROFILE_SHOW_CONFIG_DETAILS, rmp->id = id;
	...
	rmp->polynomial_public =
	  clib_host_to_net_u64 (profile->poly_pre_eval););
    }
  else
    {
      REPLY_MACRO2 (VL_API_POT_PROFILE_SHOW_CONFIG_DETAILS, rmp->id = id;
		    rmp->validator = 0; rmp->secret_key = 0;
		    rmp->secret_share = 0; rmp->prime = 0; rmp->bit_mask = 0;
		    rmp->lpc = 0; rmp->polynomial_public = 0;);
    }
}
...
static void
vl_api_pot_profile_activate_t_handler (vl_api_pot_profile_activate_t *mp)
{
  pot_main_t *sm = &pot_main;
  int rv = 0;
  vl_api_pot_profile_add_reply_t *rmp;
...
static void
vl_api_pot_profile_del_t_handler (vl_api_pot_profile_del_t *mp)
{
  pot_main_t *sm = &pot_main;
  int rv = 0;
