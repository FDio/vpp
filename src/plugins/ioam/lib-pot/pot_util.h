 ...
 * Step 2: Setup a proof of transit  profile that contains all the parameters
 * needed to compute cumulative: Call these functions: pot_profile_find
 *         pot_profile_create
 *         pot_profile_set_bit_mask - To setup how large we want the numbers
 * used in the computation and random number <= 64 bits Step 2a: For validator
 * do this: pot_set_validator Step 2b: On initial node enable the profile to be
 * used: pot_profile_set_active / pot_profile_get_active will return the
 * profile Step 3a: At the initial node to generate Random number that will be
 * read by all other nodes: pot_generate_random Step 3b: At all nodes including
 * initial and verifier call this to compute cumulative: pot_update_cumulative
...
int pot_profile_create (pot_profile *profile, u64 prime, u64 poly2, u64 lpc,
			u64 secret_share);
/*
