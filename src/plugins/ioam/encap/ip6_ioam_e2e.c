  ...
  if (rewrite_string && *rewrite_size == sizeof (ioam_e2e_option_t))
    {
      ...
      e2e_option->hdr.type =
	HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE | HBH_OPTION_TYPE_SKIP_UNKNOWN;
      e2e_option->hdr.length =
	sizeof (ioam_e2e_option_t) - sizeof (ip6_hop_by_hop_option_t);
      return (0);
    }
  return (-1);
}
...
static clib_error_t *
ioam_show_e2e_cmd_fn (vlib_main_t *vm, unformat_input_t *input,
		      vlib_cli_command_t *cmd)
{
  ...
  for (i = 0; i < vec_len (ioam_e2e_main.e2e_data); i++)
	...
	continue;
      ...
      s = show_ioam_seqno_cmd_fn (s, &e2e_data->seqno_data,
