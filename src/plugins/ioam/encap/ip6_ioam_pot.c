...
format_ioam_pot (u8 *s, va_list *args)
{
  ...
  s = format (s, "random = 0x%Lx, Cumulative = 0x%Lx, Index = 0x%x", random,
	      cumulative, pot0 ? pot0->reserved_profile_id : ~0);
  return s;
}

u8 *
ip6_hbh_ioam_proof_of_transit_trace_handler (u8 *s,
...
int
ip6_hbh_ioam_proof_of_transit_handler (vlib_buffer_t *b, ip6_header_t *ip,
				       ip6_hop_by_hop_option_t *opt0)
{
  ...
  if (pot_encap)
    ...
    { /* Non encap node */
      if (PREDICT_FALSE (pot0->reserved_profile_id != pot_profile_index ||
			 pot_profile == 0))
	{
  ...
  pot0->cumulative = clib_host_to_net_u64 (
    pot_update_cumulative (pot_profile, cumulative, random));
  ...
  if (rewrite_string && *rewrite_size == sizeof (ioam_pot_option_t))
    {
      ...
      pot_option->hdr.length =
	sizeof (ioam_pot_option_t) - sizeof (ip6_hop_by_hop_option_t);
      return (0);
    }
  return (-1);
}
...
static clib_error_t *
ip6_show_ioam_pot_cmd_fn (vlib_main_t *vm, unformat_input_t *input,
...
ip6_hop_by_hop_ioam_pot_init (vlib_main_t *vm)
{
  ...
  if (ip6_hbh_add_register_option (
	HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT, sizeof (ioam_pot_option_t),
	ip6_hop_by_hop_ioam_pot_rewrite_handler) < 0)
    return (clib_error_create (
      "registration of HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT for rewrite "
      "failed"));
