  ...
  if (trace_type & IOAM_BIT_TRANSIT_DELAY)
    {
      ...
      s = format (s, ", transit delay (ns): 0x%x",
  ...
  if (trace_type & IOAM_BIT_TTL_NODEID_WIDE)
    {
      ...
      s = format (s, ", ttl: 0x%x, node id wide: 0x%Lx", ttl_node_id >> 56,
					   ...
					   u8 * rewrite_size)
{
  ...
  trace_option->hdr.length =
    sizeof (ioam_trace_hdr_t) + (trace_option_elts * trace_data_size) +
    2; /* ip6_hop_by_hop_option_t: reserved and ioam_type */
  ...
  if (IOAM_GET_OPAQUE_LEN (profile->opaque.len_schemeid))
    {
      node_len += IOAM_GET_OPAQUE_LEN (
	profile->opaque.len_schemeid); // Should already be in 4 octets
    }
			       ...
			       ioam_trace_option_t * trace)
{
  ...
  opt = (ioam_trace_option_t *) ip6_hbh_get_option (
  ...
  if (PREDICT_TRUE (data_list_elts_left))
      ...
      if (trace_type & IOAM_BIT_TIMESTAMP_SUB_SEC)
	{
	  ...
	  *elt = clib_host_to_net_u32 (time_u64.as_u32[0]);
      ...
      // not exactly the 'egress' time, but close to it and provides good
      // insight in nano secs
      if (trace_type & IOAM_BIT_TRANSIT_DELAY)
	{
	  /* Ingress timestamp meta in buffer2->unused data */
	  timestamp_meta_t *time_meta = (void *) &vnet_buffer2 (b)->unused[0];
	  // Copy the ptr to the location it needs to be stored in the packet,
	  // egress timestamp node will handle the rest
	  time_meta->ptr_to_ioam_transit_delay = elt;
	  // could mean CLIB_UNIX not defined, always added in case it's not
	  // possible to add stamp
      ...
      if (trace_type & IOAM_BIT_APPDATA_SHORT_DATA)
	{
	  *elt = clib_host_to_net_u32 (profile->app_data_short);
      ...
      /*
       * Reporting is dependent on what device is selected, see
       * device_driver_info.c
       */
      if (trace_type & IOAM_BIT_QUEUE_DEPTH)
	{
	  ...
	  // Trick here is similar to transit delay except, the queue depth is
	  // saved in unused[7]
	  else if (profile->queue_depth_type & QUEUE_DEPTH_DPDK)
	    {
	      u32 *queueSize_queueDepth = &vnet_buffer2 (b)->unused[7];
	      // Upper bytes is the total queue size, we only intrested in
	      // lower
	      depth =
      ...
      if (trace_type & IOAM_BIT_VAR_LEN_OP_ST_SNSH)
	{
      ...
      if (PREDICT_FALSE ((nlfrl_host & IOAM_FLAGS_MASK) &
			 IOAM_BIT_FLAG_LOOPBACK))
	{
					    ...
					    ip6_hop_by_hop_option_t * opt)
{
  ...
  s = format (
    s, " namespace id %d, trace type 0x%x, %d elts left, %d bytes per node\n",
  ...
  u32 trace_type = IOAM_GET_TRACETYPE (trace->trace_hdr.trace_type);
  while ((u8 *) elt <
	 ((u8 *) (&trace->trace_hdr.data_list[0]) + trace->hdr.length -
	  sizeof (ioam_trace_hdr_t) -
	  2)) /* ip6_hop_by_hop_option_t: reserved and ioam_type */
    {
