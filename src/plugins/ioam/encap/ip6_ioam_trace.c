...
#define foreach_ip6_hop_by_hop_ioam_trace_stats                               \
  ...
  _ (PROFILE_MISS,                                                            \
     "Pkts with ip6 hop-by-hop trace options but no profile set")             \
  ...
  if (PREDICT_TRUE (trace->trace_hdr.data_list_elts_left))
    {
       ...
       */
      elt_index = trace->trace_hdr.data_list_elts_left *
	...
	{
	  *elt = (vnet_buffer (b)->sw_if_index[VLIB_RX] & 0xFFFF) << 16 |
...
ip6_hop_by_hop_ioam_trace_init (vlib_main_t *vm)
{
  ...
  if (ip6_hbh_add_register_option (
	HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST, sizeof (ioam_trace_option_t),
	ip6_hop_by_hop_ioam_trace_rewrite_handler) < 0)
    return (clib_error_create (
      "registration of HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST for rewrite "
      "failed"));
