			 ...
			 int use_iova)
{
  ...
  while (n_left && n_free_desc)
	...
	goto one_by_one;
      ...
      clib_memcpy (txq->bufs + tail, bi, 4 * sizeof (u32));

      avf_wr_txd (vm, d, b[0], /* is_eop */ 1, use_iova);
      avf_wr_txd (vm, d, b[1], /* is_eop */ 1, use_iova);
      avf_wr_txd (vm, d, b[2], /* is_eop */ 1, use_iova);
      avf_wr_txd (vm, d, b[3], /* is_eop */ 1, use_iova);
