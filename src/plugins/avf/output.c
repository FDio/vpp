...
avf_get_buffer_dma_addr (vlib_main_t * vm, vlib_buffer_t * b, int use_iova)
{
			 ...
			 int use_iova)
{
  ...
  if (txq->n_bufs)
    {
      u16 stride = 64, first, slot, n_free = 0;
      first = slot = (txq->next - txq->n_bufs) & mask;
      d = txq->descs + txq->last_descs[slot];
      while (n_free < txq->n_bufs && avf_tx_desc_get_dtyp (d) == 0x0F)
	{
	  d = txq->descs + txq->last_descs[(slot + stride) & mask];
	  if (txq->n_bufs - n_free > stride
	      && avf_tx_desc_get_dtyp (d) == 0x0F)
	    {
	      n_free += stride;
  ...
  n_free_desc = txq->size - txq->n_bufs - 8;
  b = bufs;
  bi = from;
  ...
  while (n_left && n_free_desc)
	...
	goto one_by_one;
      ...
      d[0].qword[0] = avf_get_buffer_dma_addr (vm, b[0], use_iova);
      d[0].qword[1] = ((u64) b[0]->current_length) << 34 | bits;

      d[1].qword[0] = avf_get_buffer_dma_addr (vm, b[1], use_iova);
      d[1].qword[1] = ((u64) b[1]->current_length) << 34 | bits;

      d[2].qword[0] = avf_get_buffer_dma_addr (vm, b[2], use_iova);
      d[2].qword[1] = ((u64) b[2]->current_length) << 34 | bits;

      d[3].qword[0] = avf_get_buffer_dma_addr (vm, b[3], use_iova);
      ...
      if (PREDICT_FALSE (b[0]->flags & VLIB_BUFFER_NEXT_PRESENT))
	{
	  ...
	  u16 count = 1;
	  u16 index = txq->next;
	  d[0].qword[1] |= AVF_TXQ_DESC_CMD_RS;
	  while (nb->flags & VLIB_BUFFER_NEXT_PRESENT)
		...
		break;
	      ...
	      d = txq->descs + index;
	      d[0].qword[0] = avf_get_buffer_dma_addr (vm, nb, use_iova);
	      d[0].qword[1] = ((u64) nb->current_length) << 34;
	      d[0].qword[1] |= AVF_TXQ_DESC_CMD_RS;
	    }

	  /* next */
	  d[0].qword[1] |= AVF_TXQ_DESC_CMD_RS | AVF_TXQ_DESC_CMD_EOP;
	  n_free_desc -= count;
	  txq->last_descs[txq->next] = (txq->next + count - 1) & mask;
	  txq->next = (txq->next + count) & mask;
	}
      else
	{
	  d[0].qword[1] |= AVF_TXQ_DESC_CMD_RS | AVF_TXQ_DESC_CMD_EOP;
	  n_free_desc -= 1;
	  txq->last_descs[txq->next] = txq->next;
  ...
  txq->n_bufs = txq->size - 8 - n_free_desc;
