		...
		u32 *buffers, u32 n_packets, u16 *n_enq_descs, int use_va_dma)
{
  ...
  while (n_packets_left && n_desc_left)
	...
	goto one_by_one;
      ...
      *(u64x8 *) &b =
	u64x8_add (base_ptr, u64x8_from_u32x8 (u32x8_shift_left (
			       buf_indices, CLIB_LOG2_CACHE_LINE_BYTES)));

      or_flags_vec512 =
	_mm512_i64gather_epi64 (u64x8_load_unaligned (b), 0, 1);
#else
	...
	VLIB_BUFFER_NEXT_PRESENT | VNET_BUFFER_F_OFFLOAD | VNET_BUFFER_F_GSO);
      if (PREDICT_FALSE (!u64x8_is_all_zero (
	    u64x8_and (or_flags_vec512, flags_mask_vec512))))
#else
