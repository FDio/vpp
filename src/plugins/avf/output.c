...
retry:
  ...
  while (n_left >= 8)
      ...
      else
	{
	  ...
	  d1->qword[0] =
	    vlib_get_buffer_dma_addr (vm, bi1) + b1->current_data;
	  d2->qword[0] =
	    vlib_get_buffer_dma_addr (vm, bi2) + b2->current_data;
	  d3->qword[0] =
	    vlib_get_buffer_dma_addr (vm, bi3) + b3->current_data;
	}
  ...
  while (n_left)
	...
	d0->qword[0] = vlib_get_buffer_dma_addr (vm, bi0) + b0->current_data;
