...
static_always_inline u64
avf_tx_desc_qw1 (u16 len, u64 cmd)
{
  return (((u64) len) << 34 | cmd | AVF_TXD_CMD_RSV);
}
		...
		u32 *buffers, u32 n_packets, u16 *n_enq_descs, int use_va_dma)
{
  ...
  while (n_packets_left && n_desc_left)
      ...
      else
	{
      ...
      d[0].qword[1] = avf_tx_desc_qw1 (b[0]->current_length, cmd_eop);
      d[1].qword[1] = avf_tx_desc_qw1 (b[1]->current_length, cmd_eop);
      d[2].qword[1] = avf_tx_desc_qw1 (b[2]->current_length, cmd_eop);
      d[3].qword[1] = avf_tx_desc_qw1 (b[3]->current_length, cmd_eop);
	...
	{
	  u64 qw1 = avf_tx_desc_qw1 (b[0]->current_length, cmd_eop);

	  if (PREDICT_FALSE (flags & VNET_BUFFER_F_OFFLOAD))
	    qw1 |= avf_tx_prepare_cksum (b[0], 0 /* is_tso */);
      ...
      while (b[0]->flags & VLIB_BUFFER_NEXT_PRESENT)
	{
	  ...
	  d[0].qword[1] = avf_tx_desc_qw1 (b[0]->current_length, cmd);
      ...
      d[0].qword[1] = avf_tx_desc_qw1 (b[0]->current_length, cmd_eop | cmd);
