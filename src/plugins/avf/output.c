		...
		u32 * buffers, u32 n_packets, int use_va_dma)
{
  ...
  while (n_packets_left && n_desc_left)
    ...
    one_by_one:
      ...
      if (PREDICT_FALSE
	  (is_tso
	   || b[0]->flags & (VNET_BUFFER_F_OFFLOAD_IP_CKSUM |
			     VNET_BUFFER_F_OFFLOAD_TCP_CKSUM |
			     VNET_BUFFER_F_OFFLOAD_UDP_CKSUM)))
      ...
      if (is_tso || b[0]->flags & VLIB_BUFFER_NEXT_PRESENT)
	{
	  ...
	  if (PREDICT_FALSE (is_tso))
		...
		vnet_buffer2 (b[0])->gso_l4_hdr_sz;
	      u16 tlen = vlib_buffer_length_in_chain (vm, b[0]) - l234hdr_sz;
	      d[0].qword[0] = 0;
	      d[0].qword[1] = AVF_TXD_DTYP_CTX | AVF_TXD_CTX_CMD_TSO
		| AVF_TXD_CTX_SEG_MSS (vnet_buffer2 (b[0])->gso_size) |
	  ...
	  if (is_tso || b[0]->flags & VLIB_BUFFER_NEXT_PRESENT)
	      ...
	      if (PREDICT_FALSE (is_tso))
		{
		  u16 l234hdr_sz =
		    ...
		    vnet_buffer2 (b[0])->gso_l4_hdr_sz;
		  u16 tlen =
		    vlib_buffer_length_in_chain (vm, b[0]) - l234hdr_sz;
		  d[0].qword[0] = 0;
		  d[0].qword[1] = AVF_TXD_DTYP_CTX | AVF_TXD_CTX_CMD_TSO
		    | AVF_TXD_CTX_SEG_MSS (vnet_buffer2 (b[0])->gso_size) |
		    AVF_TXD_CTX_SEG_TLEN (tlen);
