...
retry:
  ...
  if (ad->flags & AVF_DEVICE_F_VA_DMA)
    n_enq = avf_tx_enqueue (vm, txq, buffers, n_left_to_send, 1);
  else
    n_enq = avf_tx_enqueue (vm, txq, buffers, n_left_to_send, 0);

  n_left_to_send -= n_enq;
  ...
  return frame->n_vectors - n_left_to_send;
}
