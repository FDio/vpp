...
int avf_fdir_vc_op_callback (void *vc_hdl, enum virthnl_adv_ops vc_op,
                             void *in, u32 in_len, void *out, u32 out_len)
{
  ...
  err = avf_general_virtchnl_event_request (dev_instance, op, in, in_len, out, out_len);
  if (err != 0)
    {
...
static int avf_flow_add (u32 dev_instance, vnet_flow_t *f,
                         avf_flow_entry_t *fe)
{
  avf_device_t *ad = avf_get_device (dev_instance);
  int rv = 0;
...
int avf_flow_ops_fn (vnet_main_t *vm, vnet_flow_dev_op_t op, u32 dev_instance,
                     u32 flow_index, uword *private_data)
{
  vnet_flow_t *flow = vnet_get_flow (flow_index);
  avf_device_t *ad = avf_get_device (dev_instance);
  avf_flow_entry_t *fe;
  avf_flow_lookup_entry_t *fle = 0;
  int rv = 0;
