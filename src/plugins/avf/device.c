...
avf_process_handle_request (vlib_main_t * vm, avf_process_req_t * req)
{
  ...
  if (req->calling_process_index != avf_process_node.index)
    vlib_process_signal_one_time_event (vm, req->calling_process_index,
...
avf_process_request (vlib_main_t * vm, avf_process_req_t * req)
{
  ...
  if (req->calling_process_index != avf_process_node.index)
	...
	vlib_process_create_one_time_event (vm, req->calling_process_index, 0);
      req->one_time_type_index = one_time_type_index;
	...
	clib_panic ("avf_process_request got multiple one-time events");
