...
void
avf_send_to_pf (avf_device_t * ad, virtchnl_ops_t op, void *data, int len)
{
  avf_aq_desc_t *d;
  u64 pa;
...
void
avf_get_vf_resources (vlib_main_t * vm, avf_device_t * ad)
{
  u32 bitmap = 0x30019;
  avf_send_to_pf (ad, VIRTCHNL_OP_GET_VF_RESOURCES, &bitmap, sizeof (bitmap));
}
...
avf_config_vsi_queues (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  avf_send_to_pf (ad, VIRTCHNL_OP_CONFIG_VSI_QUEUES, msg, msg_len);
}

void
avf_config_irq_map (vlib_main_t * vm, avf_device_t * ad)
{
  int count = 1;
  ...
  avf_send_to_pf (ad, VIRTCHNL_OP_CONFIG_IRQ_MAP, msg, msg_len);
}

void
avf_add_eth_addr (vlib_main_t * vm, avf_device_t * ad, u8 count, u8 * macs)
{
  int msg_len =
  ...
  avf_send_to_pf (ad, VIRTCHNL_OP_ADD_ETH_ADDR, msg, msg_len);
}
...
avf_enable_queues (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  avf_send_to_pf (ad, VIRTCHNL_OP_ENABLE_QUEUES, &qs,
		  sizeof (virtchnl_queue_select_t));
}

void
avf_get_stats (vlib_main_t * vm, avf_device_t * ad)
{
  virtchnl_queue_select_t qs = { 0 };
  qs.vsi_id = ad->vsi_id;
  avf_send_to_pf (ad, VIRTCHNL_OP_GET_STATS, &qs,
		  sizeof (virtchnl_queue_select_t));
}
...
avf_device_init (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  avf_send_to_pf (ad, VIRTCHNL_OP_RESET_VF, 0, 0);

  rstat = avf_get_u32 (ad->bar0, AVFGEN_RSTAT);
  while (--n_tries && rstat != 2 && rstat != 3)
    {
      vlib_process_suspend (vm, 10e-3);
      rstat = avf_get_u32 (ad->bar0, AVFGEN_RSTAT);
    }
  if (n_tries == 0)
    return clib_error_return (0, "reset failed");

  avf_adminq_init (vm, ad);
  ver.major = VIRTCHNL_VERSION_MAJOR;
  ver.minor = VIRTCHNL_VERSION_MINOR;
  avf_send_to_pf (ad, VIRTCHNL_OP_VERSION, &ver, sizeof (ver));
  ...
  if (d->v_opcode == VIRTCHNL_OP_VERSION)
	...
	return clib_error_return (0, "unexpected GET_VF_RESOURCES message "
      ...
      avf_log_debug ("  vsi %u num_queue_pairs %u vsi_type %u "
		     ...
		     6);
      ad->vsi_id = v->vsi_id;
      clib_memcpy (ad->hwaddr, v->default_mac_addr, 6);
      err = vnet_hw_interface_change_mac_address (vnm, ad->hw_if_index,
						  ad->hwaddr);
      if (err)
	...
	return clib_error_return (0, "unexpected CONFIG_VSI_QUEUES message "
				  "received");
      ad->init_state = AVF_INIT_STATE_CONFIG_IRQ_MAP_SENT;
      avf_config_irq_map (vm, ad);
    }
  else if (d->v_opcode == VIRTCHNL_OP_CONFIG_IRQ_MAP)
    {
      avf_log_debug ("CONFIG_IRQ_MAP reply received", 0);
      if (ad->init_state != AVF_INIT_STATE_CONFIG_IRQ_MAP_SENT)
	return clib_error_return (0, "unexpected CONFIG_IRQ_MAP message "
				  "received");
      ad->init_state = AVF_INIT_STATE_ADD_ETH_ADDR_SENT;
      avf_add_eth_addr (vm, ad, 1, ad->hwaddr);
    }
  else if (d->v_opcode == VIRTCHNL_OP_ADD_ETH_ADDR)
    {
      avf_log_debug ("ADD_ETH_ADDR reply received", 0);
      if (ad->init_state != AVF_INIT_STATE_ADD_ETH_ADDR_SENT)
	return clib_error_return (0, "unexpected ADD_ETH_ADDR message "
				  "received");
      ad->init_state = AVF_INIT_STATE_ENABLE_QUEUES_SENT;
	...
	return clib_error_return (0, "unexpected ENABLE_QUEUES message "
				  "received");
      ad->init_state = AVF_INIT_STATE_DONE;
      ad->flags |= AVF_DEVICE_F_INITIALIZED;
    }
  else if (d->v_opcode == VIRTCHNL_OP_GET_STATS)
    {
      clib_memcpy (&ad->eth_stats, buf, sizeof (virtchnl_eth_stats_t));
    }
  else
    {
