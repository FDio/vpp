	  ...
	  for (int i = 0; i < vec_len (event_data); i++)
	    {
	      ...
	      avf_op_add_del_eth_addr (vm, ad, 1, m->eth_addr, event_type ==
				       AVF_PROCESS_EVENT_ADD_ETHER_ADDR);
	      vlib_process_signal_event (vm, m->calling_process_index, 0, 0);
	    }
	  break;
			 ...
			 const u8 * address, u8 is_add)
{
  ...
  msg.calling_process_index = vlib_get_current_process_node_index (vm);
  clib_memcpy (msg.eth_addr, address, 6);

  vlib_process_signal_event_pointer (vm, avf_process_node.index, is_add ?
				     AVF_PROCESS_EVENT_ADD_ETHER_ADDR :
