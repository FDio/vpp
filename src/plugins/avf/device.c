...
avf_config_promisc_mode (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  pi.vsi_id = ad->vsi_id;
  pi.flags = 1;
  avf_send_to_pf (ad, VIRTCHNL_OP_CONFIG_PROMISCUOUS_MODE, &pi,
		  sizeof (virtchnl_promisc_info_t));
}
...
clib_error_t *
foo (vlib_main_t * vm, avf_device_t * ad, virtchnl_ops_t op, void *in,
     int in_len, void *out, int out_len)
{
  ...
  avf_send_to_pf (ad, op, in, in_len);
		...
		virtchnl_version_info_t * ver)
{
  ...
  clib_error_t *err = 0;
  myver.major = VIRTCHNL_VERSION_MAJOR;
  myver.minor = VIRTCHNL_VERSION_MINOR;

  err =
    foo (vm, ad, VIRTCHNL_OP_VERSION, &myver,
			 ...
			 virtchnl_vf_resource_t * res)
{
  ...
  err = foo (vm, ad, VIRTCHNL_OP_GET_VF_RESOURCES, &bitmap, sizeof (u32),
	     res, sizeof (virtchnl_vf_resource_t)
...
{
  return foo (vm, ad, VIRTCHNL_OP_DISABLE_VLAN_STRIPPING, 0, 0, 0, 0);
}
  ...
  for (i = 0; i < n_qp; i++)
      ...
      if (i < vec_len (ad->txqs))
	{
	  ...
	  txq->dma_ring_addr =
	    vlib_physmem_virtual_to_physical (vm, am->physmem_region,
					      q->descs);
	  //txq->dma_ring_addr = 0xaaaa2222;
	}
    }

  return foo (vm, ad, VIRTCHNL_OP_CONFIG_VSI_QUEUES, msg, msg_len, 0, 0);
}
...
avf_op_config_irq_map (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  return foo (vm, ad, VIRTCHNL_OP_CONFIG_IRQ_MAP, msg, msg_len, 0, 0);
}
...
avf_op_add_eth_addr (vlib_main_t * vm, avf_device_t * ad, u8 count, u8 * macs)
{
  ...
  return foo (vm, ad, VIRTCHNL_OP_ADD_ETH_ADDR, msg, msg_len, 0, 0);
}

clib_error_t *
avf_op_enable_queues (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  return foo (vm, ad, VIRTCHNL_OP_ENABLE_QUEUES, &qs,
		  ...
		  virtchnl_eth_stats_t * es)
{
  virtchnl_queue_select_t qs = { 0 };
  qs.vsi_id = ad->vsi_id;
  return foo (vm, ad, VIRTCHNL_OP_GET_STATS,
...
avf_device_init (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  if ((error = avf_op_enable_queues (vm, ad)))
    return error;
