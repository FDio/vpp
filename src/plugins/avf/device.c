...
{
  avf_log_debug (ad, "avf_op_general_vc: vsi_id %u general event operation %u",
		 ad->vsi_id, (u8) op);
...
avf_process_handle_request (vlib_main_t * vm, avf_process_req_t * req)
{
  ...
  if (req->type == AVF_PROCESS_REQ_ADD_DEL_ETH_ADDR)
    ...
    req->error = avf_op_general_vc (vm, ad, req->event_op, req->in,
...
clib_error_t *
avf_general_virtchnl_event_request (u32 dev_instance, virtchnl_ops_t op,
				    u8 *in, u32 in_len, u8 *out, u32 out_len)
{
  vlib_main_t *vm = vlib_get_main ();
  avf_process_req_t req;
