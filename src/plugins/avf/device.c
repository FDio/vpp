...
avf_device_init (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  avf_reg_write (ad, VFINT_DYN_CTL0, 0x1fe01);
...
avf_process_one_device (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  vec_foreach (e, ad->events)
      ...
      if (e->event == VIRTCHNL_EVENT_LINK_CHANGE)
	{
	  ...
	  avf_log_debug ("link change event severity %d link_speed %d "
			 "link_status %x", e->severity,
...
avf_create_if (vlib_main_t * vm, avf_create_if_args_t * args)
{
  ...
  if ((args->error = vlib_pci_map_resource (h, 0, &ad->bar0)))
    goto error;
