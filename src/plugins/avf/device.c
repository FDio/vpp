...
avf_device_init (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  if (tm->n_vlib_mains > 4)
    return clib_error_return (0, "no enough tx queues");
...
avf_process_one_device (vlib_main_t * vm, avf_device_t * ad, int is_irq)
{
...
#if 0
  if (is_irq == 0 && ad->flags & AVF_DEVICE_F_INITIALIZED)
    avf_op_get_stats (vm, ad, &ad->eth_stats);
#endif

  if (vec_len (ad->events))
    clib_warning ("%u new events", vec_len (ad->events));
  ...
  vec_foreach (e, ad->events)
      ...
      if (e->event == VIRTCHNL_EVENT_LINK_CHANGE)
	{
	  ...
	  fformat (stderr, "link change event severity %d link_speed %d "
			 "link_status %x\n", e->severity,
			 e->event_data.link_event.link_speed,
			 e->event_data.link_event.link_status);
	}
      else
	fformat (stderr, "event %d severity %d", e->event, e->severity);
    }
  /* *INDENT-ON* */
...
avf_process (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  while (1)
	...
	vlib_process_wait_for_event (vm);
      ...
      clib_warning ("time %.2f", vlib_time_now (vm));
