		 ...
		 void *data, int len)
{
  ...
  _clib_memcpy (d, dt, sizeof (avf_aq_desc_t));
  ...
  if (len)
    {
      ...
      d->addr_hi = (u32) (pa >> 32);
      d->addr_lo = (u32) pa;
      _clib_memcpy (ad->atq_bufs + ad->atq_next_slot * AVF_MBOX_BUF_SZ, data,
		    len);
      d->flags |= AVF_AQ_F_BUF;
    }

  if (ad->flags & AVF_DEVICE_F_ELOG)
    _clib_memcpy (&dc, d, sizeof (avf_aq_desc_t));
...
retry:
  ...
  _clib_memcpy (dt, d, sizeof (avf_aq_desc_t));
...
retry:
  ...
  if (d->v_opcode == VIRTCHNL_OP_EVENT)
	...
	return clib_error_return (0, "event message error");
      ...
      _clib_memcpy (e, buf, sizeof (virtchnl_pf_event_t));
...
avf_op_add_eth_addr (vlib_main_t * vm, avf_device_t * ad, u8 count, u8 * macs)
{
  ...
  for (i = 0; i < count; i++)
    _clib_memcpy (&al->list[i].addr, macs + i * 6, 6);
		 ...
		 avf_create_if_args_t * args)
{
  ...
  _clib_memcpy (ad->hwaddr, res.vsi_res[0].default_mac_addr, 6);
