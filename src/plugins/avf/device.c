...
avf_recv_from_pf (vlib_main_t * vm, avf_device_t * ad, u16 slot)
{
  ...
  if (d->v_opcode == VIRTCHNL_OP_VERSION)
  ...
  else if (d->v_opcode == VIRTCHNL_OP_GET_VF_RESOURCES)
    {
      ...
      virtchnl_txq_info_t txq = { 0 };
      txq.ring_len = AVF_TXQ_SZ;
      txq.dma_ring_addr = 0xdeadbeef;
      avf_send_to_pf (vm, ad, VIRTCHNL_OP_CONFIG_TX_QUEUE, &txq,
		      sizeof (virtchnl_txq_info_t));
    }
