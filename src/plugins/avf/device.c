...
avf_rxq_init (vlib_main_t * vm, avf_device_t * ad, u16 qid)
{
  ...
  rxq->descs = vlib_physmem_alloc_aligned (vm, am->physmem_region, &error,
					   AVF_RXQ_SZ * 32, 64);
  rxq->info.ring_len = AVF_RXQ_SZ;
  rxq->info.vsi_id = ad->vsi_id;
  rxq->info.hdr_size = 32;
  rxq->info.databuffer_size = 2048;
  rxq->info.max_pkt_size = 1514;
  rxq->info.dma_ring_addr =
    vlib_physmem_virtual_to_physical (vm, am->physmem_region, rxq->descs);
...
avf_txq_init (vlib_main_t * vm, avf_device_t * ad, u16 qid)
{
  ...
  txq->descs = vlib_physmem_alloc_aligned (vm, am->physmem_region, &error,
					   AVF_TXQ_SZ * 32, 64);
  txq->info.ring_len = AVF_TXQ_SZ;
  txq->info.vsi_id = ad->vsi_id;
  txq->info.dma_ring_addr =
    vlib_physmem_virtual_to_physical (vm, am->physmem_region, txq->descs);
  return 0;
}
...
avf_device_init (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  ad->flags |= AVF_DEVICE_F_INIT_START;
  return error;
}
...
avf_recv_from_pf (vlib_main_t * vm, avf_device_t * ad, u16 slot)
{
  ...
  if (d->v_opcode == VIRTCHNL_OP_VERSION)
  ...
  else if (d->v_opcode == VIRTCHNL_OP_GET_VF_RESOURCES)
    {
      ...
      clib_memcpy (&ci.qpair[0].txq, &ad->txqs[0].info,
		   sizeof (ci.qpair[0].txq));
      clib_memcpy (&ci.qpair[0].rxq, &ad->rxqs[0].info,
		   sizeof (ci.qpair[0].rxq));
      ci.qpair[0].txq.vsi_id = ad->vsi_id;
      ci.qpair[0].rxq.vsi_id = ad->vsi_id;
      ci.vsi_id = ad->vsi_id;
      ci.num_queue_pairs = 1;
      avf_send_to_pf (vm, ad, VIRTCHNL_OP_CONFIG_VSI_QUEUES, &ci,
		      sizeof (virtchnl_vsi_queue_config_info_t));
    }
...
avf_process_one_device (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  if ((ad->flags & AVF_DEVICE_F_INIT_START) == 0)
