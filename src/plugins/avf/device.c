...
avf_process_request (vlib_main_t * vm, avf_process_req_t * req)
{
  ...
  if (req->calling_process_index != avf_process_node.index)
	...
	case ~0: // Timeout.
	  ...
	  // We should panic but in that case CSIT does not see the log.
	  break;
	default:
	  clib_warning ("unexpected signals type %wu, dropping", event_type);
	  // We should panic or re-schedule the signals.
	}
      vec_free (event_data);
    }
  else
