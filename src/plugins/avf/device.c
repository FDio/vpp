...
static pci_device_id_t avf_pci_device_ids[] = {
  {.vendor_id = PCI_VENDOR_ID_INTEL,.device_id = 0x154c,},
  {0},
};
  ...
  if (d->v_opcode != op)
    {
      return clib_error_return (0,
				"unexpected message receiver "
				"[v_opcode = %u expected %u]",
				d->v_opcode, op);
    }
...
avf_device_init (vlib_main_t * vm, avf_device_t * ad)
{
  ...
  avf_reg_write (ad, AVFINT_DYN_CTL0, 0x1fe01);
...
avf_process_one_device (vlib_main_t * vm, avf_device_t * ad, int is_irq)
{
  ...
  if ((r & 0xf0000000) != (1 << 31))
    {
      error = clib_error_return (0, "arq not enabled, arqlen = 0x%x", r);
      goto error;
    }
  ...
  if ((r & 0xf0000000) != (1 << 31))
    {
      error = clib_error_return (0, "atq not enabled, atqlen = 0x%x", r);
      goto error;
    }
  ...
  vec_foreach (e, ad->events)
      ...
      if (e->event == VIRTCHNL_EVENT_LINK_CHANGE)
	{
	  ...
	  if (ad->flags & AVF_DEVICE_F_ELOG)
		...
		{
		  .format = "avf link change: sw_if_index %d flags 0x%x "
	  ...
	  if (ad->flags & AVF_DEVICE_F_ELOG)
	    {
	      ELOG_TYPE_DECLARE (el) =
		{
		  .format = "avf unknown event: sw_if_index %d event %d "
		    "severity %d",
		  .format_args = "i4i4i1i1",
		};
	      struct
...
static void
avf_msix_handler (vlib_pci_dev_handle_t h, u16 line)
{
  ...
  if (ad->flags & AVF_DEVICE_F_ELOG)
    {
      ...
      ELOG_TYPE_DECLARE (el) =
	{
	  .format = "avf msix irq: sw_if_index %d line %d icr0 0x%x "
	    "dyn_ctl0 0x%x",
	...
	u32 dyn_ctl0;
      ...
      ed->line = line;
      ed->icr0 = icr0;
      ed->dyn_ctl0 = avf_reg_read (ad, AVFINT_DYN_CTL0);
    }

  avf_reg_write (ad, AVFINT_ICR0_ENA1, 0);
  avf_reg_write (ad, AVFINT_DYN_CTL0, (3 << 3));
  ...
  avf_reg_write (ad, AVFINT_ICR0_ENA1, (1 << 30));
  avf_reg_write (ad, AVFINT_DYN_CTL0, (1 << 0) | (3 << 3));
}
...
avf_create_if (vlib_main_t * vm, avf_create_if_args_t * args)
{
  ...
  if ((error = vlib_pci_register_msix_handler (h, 0, 2, &avf_msix_handler)))
    goto error;
