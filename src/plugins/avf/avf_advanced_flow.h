...
struct virtchnl_proto_hdrs
{
  u8 tunnel_level;
  /**
   * specify where protocol header start from. Must be 0 when sending a raw
 ...
 * RAW pattern type
 */
struct avf_raw_pattern
{
  u32 relative : 1;  /**< Look for pattern after the previous item. */
  u32 search : 1;    /**< Search pattern from offset (see also limit). */
  u32 reserved : 30; /**< Reserved, must be set to zero. */
  int offset;	     /**< Absolute or relative offset for pattern. */
  u16 limit;	     /**< Search area limit for start of pattern. */
  u16 length;	     /**< Pattern length. */
  const u8 *pattern; /**< Byte string to look for. */
};
