...
int
avf_fdir_rcfg_create (struct avf_fdir_conf **rcfg, int tunnel_level,
		      u16 vsi, u16 nrxq)
{
  (*rcfg) = avf_malloc (sizeof (**rcfg));
  if ((*rcfg) == NULL)
    {
      return -1;
    }

  avf_memset_genrl (*rcfg, 0, sizeof (**rcfg));
			 ...
			 struct avf_flow_error *error)
{
  ...
  switch (type)
      ...
      if (eth_spec && eth_mask && eth_mask->type)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, eth_spec, sizeof (*eth_spec));
	}
      break;
      ...
      if (ipv4_spec && ipv4_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, ipv4_spec, sizeof (*ipv4_spec));
	}
      break;
      ...
      if (ipv6_spec && ipv6_mask)
	{
	  ...
	  if ((ipv6_mask->vtc_flow & CPU_TO_BE32 (AVF_IPV6_TC_MASK)) ==
	      (CPU_TO_BE32 (AVF_IPV6_TC_MASK)))
	    {
	  ...
	  if (!avf_memcmp (ipv6_mask->src_addr, ipv6_addr_mask,
			   sizeof (ipv6_mask->src_addr)))
	    {
	      rcfg->input_set |= AVF_INSET_IPV6_SRC;
	      VIRTCHNL_ADD_PROTO_HDR_FIELD_BIT (hdr, IPV6, SRC);
	    }
	  if (!avf_memcmp (ipv6_mask->dst_addr, ipv6_addr_mask,
			   sizeof (ipv6_mask->dst_addr)))
	    {
	      rcfg->input_set |= AVF_INSET_IPV6_DST;
	      VIRTCHNL_ADD_PROTO_HDR_FIELD_BIT (hdr, IPV6, DST);

	      avf_memcpy_genrl (hdr->buffer, ipv6_spec, sizeof (*ipv6_spec));
	    }
	}
      ...
      if (udp_spec && udp_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, udp_spec, sizeof (*udp_spec));
	}
      break;
      ...
      if (tcp_spec && tcp_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, tcp_spec, sizeof (*tcp_spec));
	}
      ...
      if (sctp_spec && sctp_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, sctp_spec, sizeof (*sctp_spec));
	}
      break;
      ...
      if (gtp_spec && gtp_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, gtp_spec, sizeof (*gtp_spec));
	}
      ...
      if (gtp_psc_spec && gtp_psc_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, gtp_psc_spec,
			    sizeof (*gtp_psc_spec));
	}
      ...
      if (l2tpv3oip_spec && l2tpv3oip_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, l2tpv3oip_spec,
      ...
      if (esp_spec && esp_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, esp_spec, sizeof (*esp_spec));
	}
      break;
      ...
      if (ah_spec && ah_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, ah_spec, sizeof (*ah_spec));
	}
      break;
      ...
      if (pfcp_spec && pfcp_mask)
	{
	  ...
	  avf_memcpy_genrl (hdr->buffer, pfcp_spec, sizeof (*pfcp_spec));
	}
      break;

    default:
      ret =
	avf_flow_error_set (error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM,
			       ...
			       int act_idx, struct avf_flow_error *error)
{
  ...
  if (rss->queue_num <= 1)
    {
      ret = avf_flow_error_set (error, AVF_FAILURE,
				AVF_FLOW_ERROR_TYPE_ACTION, act,
...
int
avf_fdir_rule_destroy (struct avf_fdir_vc_ctx *ctx,
		       struct avf_fdir_conf *rcfg)
{
  int ret;
