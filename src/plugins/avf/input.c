		      ...
		      vlib_buffer_t ** b, u32 n_left)
{
  ...
  while (n_left >= 4)
      ...
      if (n_left >= 12)
	{
      ...
      n_rx_bytes += b[0]->current_length = qw1[0] >> AVF_RX_DESC_LENGTH_SHIFT;
      n_rx_bytes += b[1]->current_length = qw1[1] >> AVF_RX_DESC_LENGTH_SHIFT;
      n_rx_bytes += b[2]->current_length = qw1[2] >> AVF_RX_DESC_LENGTH_SHIFT;
      n_rx_bytes += b[3]->current_length = qw1[3] >> AVF_RX_DESC_LENGTH_SHIFT;
