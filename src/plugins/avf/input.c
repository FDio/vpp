...
avf_rx_desc_length (avf_rx_desc_t * d)
{
  return avf_get_u64_bits (d, 8, 63, 38);
}
			 ...
			 vlib_frame_t * frame, avf_device_t * ad, u16 qid)
{
  ...
  while (n_rxv < AVF_RX_VECTOR_SZ)
	...
	goto no_dd;
      ...
      rxve->length = avf_rx_desc_length (d);
      or_error |= rxve->error;
      ...
      else
	{
	  ...
	  b->flags = 0;
	  b->current_data = 0;
	  b->current_length = avf_rx_desc_length (d);
