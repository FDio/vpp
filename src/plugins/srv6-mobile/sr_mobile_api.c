				   ...
				   size_t keyword_len)
{
  ...
  vec_foreach (plugin_it, vec_plugins)
    {
      if (!clib_strcmp_with_size (keyword_str, keyword_len,
  ...
  vl_api_sr_mobile_localsid_add_del_t *mp)
{
  ...
  if (mp->behavior[0])
    {
      if (!clib_strcmp_with_size ((char *) mp->behavior, behavior_size,
				  "end.m.gtp4.e"))
	{
	  kind_fn = SRV6_MOBILE_LOCALSID_END_M_GTP4_E;
	}
      else if (!clib_strcmp_with_size ((char *) mp->behavior, behavior_size,
				       "end.m.gtp6.e"))
	{
