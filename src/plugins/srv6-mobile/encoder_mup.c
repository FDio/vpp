...
always_inline void 
alloc_sr_sid_encoder_mup(void **mem_p, 
    const void *v6src_addr, const u32 v6src_len, const void  *v4src_addr,
    const void *v6dst_addr, const u32 v6dst_len, const void *v4dst_addr,
    const u8 qfi, const u8 r, const u8 u, const u32 teid){
...
always_inline void write_v6addr_in_pyload(
    ip6_address_t *v6addr, u8 *payload, u16 payload_len, u16 offset, u16 shift)
{
    if (shift == 0)
    {
...
sr_sid_encoder_format_mup(u8 * s, va_list * args)
{
    ...
    s = format (s, "src v6 prefix: %U/%d, ", format_ip6_address, &ls_mem->v6src_addr, ls_mem->v6src_len);
    s = format (s, "dst v6 prefix: %U/%d", format_ip6_address, &ls_mem->v6dst_addr, ls_mem->v6dst_len);
    s = format (s, "\n\t");
...
sr_sid_encoder_unformat_mup(unformat_input_t * input, va_list * args)
{
    ...
    while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
        if (unformat (input, "mup %U/%d %U %U/%d %U %d %d %d %d", 
            ...
            unformat_ip4_address, &v4src_addr,
            unformat_ip6_address, &v6dst_addr, &v6dst_len,
            unformat_ip4_address, &v4dst_addr,
            &qfi, &r, &u, &teid)){
                break;
            }
        else
    ...
    &v6dst_addr, v6dst_len, &v4dst_addr,
    qfi, r, u, teid);
...
static void 
sr_sid_encoder_build_mup(void *mem_p, ip6_address_t *v6src_addr, ip6_address_t *v6dst_addr)
{
    ...
    *v6dst_addr = ls_mem->v6dst_addr;
    ...
    write_v6addr_in_pyload(v6src_addr, (u8*)&ls_mem->v4src_addr, sizeof(ip4_address_t), s_offset, s_shift);
    write_v6addr_in_pyload(v6dst_addr, (u8*)&ls_mem->v4dst_addr, sizeof(ip4_address_t), d_offset, d_shift);
    d_offset += sizeof(ip4_address_t);
    tmp_args |= ls_mem->qfi << 2;
    tmp_args |= ls_mem->r<<1;
    tmp_args |= ls_mem->u;

    write_v6addr_in_pyload(v6dst_addr, &tmp_args, sizeof(u8), d_offset, d_shift);
    d_offset += sizeof(u8);
    teid = htonl(ls_mem->teid);
    write_v6addr_in_pyload(v6dst_addr, (u8*)&teid, sizeof(u32), d_offset, d_shift);
}
...
sr_sid_encoder_mup_init (vlib_main_t * vm)
{
    ...
    static u8 param_str[] = "<src_v6prefix> <src_v4addr> <dst_v6prefix> <src_v4addr> <qfi> <r> <u> <teid>";

    sr_sid_encoder_flavor_register_function(
        vm, keyword_str, def_str, param_str,
        sr_sid_encoder_format_mup,
        sr_sid_encoder_unformat_mup,
        sr_sid_encoder_removal_mup,
