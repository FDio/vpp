...
int
filter_match_mark_add_and_lock (dpo_proto_t proto, u32 bits, u32 mask,
				dpo_id_t *dpo)
{
  ...
  dpo_set (dpo, filter_match_mark_type, fmm->fmm_base.fm_base.dpoi_proto,
	   fmm - filter_match_mark_pool);
...
{
  filter_match_mark_type =
    dpo_register_new_type (&filter_match_mark_vft, filter_match_mark_nodes);
...
static clib_error_t *
filter_match_mark_cli (vlib_main_t *vm, unformat_input_t *input,
...
static clib_error_t *
filter_match_mark_show (vlib_main_t *vm, unformat_input_t *input,
			vlib_cli_command_t *cmd)
{
  index_t fmmi;
  ...
  pool_foreach_index (fmmi, filter_match_mark_pool)
    {
      vlib_cli_output (vm, " %U handle:[%d, %d]", format_filter_match_mark,
		       fmmi, 0, filter_match_mark_type, fmmi);
    }
