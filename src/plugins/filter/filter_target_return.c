...
filter_target_return_add_and_lock (dpo_proto_t proto, dpo_id_t *dpo)
{
  ...
  dpo_set (dpo, filter_target_return_type, ftr->ftr_proto,
	   ftr - filter_target_return_pool);
...
{
  filter_target_return_type = dpo_register_new_type (
    &filter_target_return_vft, filter_target_return_nodes);
...
static clib_error_t *
filter_target_return_show (vlib_main_t *vm, unformat_input_t *input,
			   vlib_cli_command_t *cmd)
{
  ...
  pool_foreach (ftr, filter_target_return_pool)
    {
      vlib_cli_output (vm, " %U", format_filter_target_return,
