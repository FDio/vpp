...
filter_target_mark_stack (filter_target_mark_t *ftm)
{
  ...
  if (INDEX_INVALID != ftm->ftm_rule)
    dpo_stack (filter_target_mark_type, ftm->ftm_proto, &ftm->ftm_next,
	       filter_rule_dpo_get (ftm->ftm_rule));
  else
    dpo_stack (filter_target_mark_type, ftm->ftm_proto, &ftm->ftm_next,
	       drop_dpo_get (ftm->ftm_proto));
}

int
filter_target_mark_add_and_lock (dpo_proto_t proto, bool xor, u32 mask,
				 u32 bits, dpo_id_t *dpo)
{
  ...
  dpo_set (dpo, filter_target_mark_type, ftm->ftm_proto,
	   ftm - filter_target_mark_pool);
...
format_filter_target_mark (u8 *s, va_list *args)
{
  ...
  s = format (s, "[%d] mark:[%U rule:%d]", ftmi, format_dpo_proto,
	      ftm->ftm_proto, ftm->ftm_rule);
  s = format (s, "\n%U%U", format_white_space, indent + 2, format_dpo_id,
	      &ftm->ftm_next, indent + 4);
...
{
  filter_target_mark_type =
    dpo_register_new_type (&filter_target_mark_vft, filter_target_mark_nodes);
...
static clib_error_t *
filter_target_mark_show (vlib_main_t *vm, unformat_input_t *input,
