...
void
filter_match_stack (dpo_id_t *match, index_t rule, const dpo_id_t *pos,
		    const dpo_id_t *neg)
{
  filter_match_t *fm;
  ...
  dpo_stack (fm->fm_base.dpoi_type, fm->fm_base.dpoi_proto,
	     &fm->fm_results[FILTER_MATCH_YES], pos);
  /* the negative match result */
  dpo_stack (fm->fm_base.dpoi_type, fm->fm_base.dpoi_proto,
	     &fm->fm_results[FILTER_MATCH_NO], neg);
}
