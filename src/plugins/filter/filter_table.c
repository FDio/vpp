 ...
 */
int
filter_table_update (const u8 *name, dpo_proto_t dproto, u32 precedence,
		     index_t *fti_out)
{
...
{
  return (fib_node_child_add (filter_table_node_type, fti,
			      filter_table_node_type, child_index));
}
...
{
  fn->fn_sibling = fib_node_child_add (filter_table_node_type, fti,
...
int
filter_table_chain_add (index_t fti, const u8 *name, filter_hook_type_t fht,
			filter_chain_policy_t policy, u32 precedence,
			index_t *fci_out)
{
  filter_table_t *ft;
  index_t fci;
  ...
  fci = filter_chain_create_and_lock (fti, name, ft->ft_proto, fht, policy,
				      precedence);
...
int
filter_table_rule_append (index_t fti, const u8 *chain, const u8 *rule,
...
format_filter_table (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U[%d] %s: precedence:%d", format_white_space, indent, fti,
	      ft->ft_name, ft->ft_precedence);
...
static clib_error_t *
filter_table_cli (vlib_main_t *vm, unformat_input_t *input,
...
static clib_error_t *
filter_table_show (vlib_main_t *vm, unformat_input_t *input,
		   vlib_cli_command_t *cmd)
{
  index_t fti;
