...
format_filter_target_accept_trace (u8 *s, va_list *args)
{
  ...
  // filter_target_accept_trace_t *t = va_arg (*args,
  // filter_target_accept_trace_t *);
...
always_inline uword
filter_target_accept_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
...
VLIB_NODE_FN (filter_target_accept_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (filter_target_accept_inline (vm, node, frame, DPO_PROTO_IP4));
}

VLIB_NODE_FN (filter_target_accept_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
