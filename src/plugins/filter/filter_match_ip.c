...
int
filter_match_ip_add_and_lock (dpo_proto_t proto, filter_match_dir_t dir,
			      const ip46_address_t *ip, dpo_id_t *dpo)
{
  ...
  dpo_set (dpo, filter_match_ip_type, fmi->fmi_base.fm_base.dpoi_proto,
	   fmi - filter_match_ip_pool);
...
format_filter_match_ip (u8 *s, va_list *args)
{
  ...
  s = format (s, "[%d] match:[%U ip %U]", fmii, format_filter_match_dir,
	      fmi->fmi_dir, format_ip46_address, &fmi->fmi_ip, IP46_TYPE_ANY);
...
unformat_filter_match_ip (unformat_input_t *input, va_list *args)
{
  ...
  if (unformat (input, "%U %U", unformat_filter_match_dir, &fd,
...
{
  filter_match_ip_type =
    dpo_register_new_type (&filter_match_ip_vft, filter_match_ip_nodes);
...
static clib_error_t *
filter_match_ip_show (vlib_main_t *vm, unformat_input_t *input,
