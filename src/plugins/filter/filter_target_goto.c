...
filter_target_goto_stack_i (filter_target_goto_t *ftg)
{
  ...
  dpo_stack (filter_target_goto_dpo_type, ftg->ftg_proto, &ftg->ftg_next,
	     filter_chain_rule_dpo_get (ftg->ftg_chain));
}
...
int
filter_target_goto_add_and_lock (dpo_proto_t proto, index_t fci, dpo_id_t *dpo)
{
  filter_target_goto_t *ftg;
  ...
  dpo_set (dpo, filter_target_goto_dpo_type, ftg->ftg_proto,
	   ftg - filter_target_goto_pool);
...
format_filter_target_goto (u8 *s, va_list *args)
{
  ...
  s = format (s, "[%d] goto:[%U rule:%d to:%s]", ftgi, format_dpo_proto,
	      ftg->ftg_proto, ftg->ftg_rule, fc->fc_name);
  s = format (s, "\n%U%U", format_white_space, indent + 2, format_dpo_id,
	      &ftg->ftg_next, indent + 4);
...
filter_target_goto_back_walk (fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
  filter_target_goto_t *ftg;
...
{
  filter_target_goto_dpo_type = dpo_register_new_type (
    &filter_target_goto_dpo_vft, filter_target_goto_nodes);
...
static clib_error_t *
filter_target_goto_show (vlib_main_t *vm, unformat_input_t *input,
