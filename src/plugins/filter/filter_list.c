...
/**
 * A sorted list of index_t and a set of call backs is invoked when elements
 * are added.
 */
...
filter_list_insert (filter_list_t *fl, index_t elem, void *ctx)
{
  ...
  if (fl->fl_vft.flv_this)
	...
	fl->fl_vft.flv_this (fl->fl_owner, fl->fl_list[pos], INDEX_INVALID,
			     ctx);
    }
  if (fl->fl_vft.flv_prev && pos >= 1)
    fl->fl_vft.flv_prev (fl->fl_owner, fl->fl_list[pos - 1], fl->fl_list[pos],
			 ctx);
