...
format_filter_target_goto_trace (u8 *s, va_list *args)
{
  ...
  filter_target_goto_trace_t *t = va_arg (*args, filter_target_goto_trace_t *);
...
always_inline uword
filter_target_goto_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
  ...
  while (n_left > 0)
    {
      ...
      vlib_increment_combined_counter (cm, thread_index, ftgj0->ftg_chain, 1,
...
VLIB_NODE_FN (filter_target_goto_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (filter_target_goto_inline (vm, node, frame, DPO_PROTO_IP4));
}

VLIB_NODE_FN (filter_target_goto_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
