...
filter_match_all_add_and_lock (dpo_proto_t proto, dpo_id_t *dpo)
{
  ...
  dpo_set (dpo, filter_match_all_type, fma->fma_base.fm_base.dpoi_proto,
	   fma - filter_match_all_pool);
...
{
  filter_match_all_type =
    dpo_register_new_type (&filter_match_all_vft, filter_match_all_nodes);
...
static clib_error_t *
filter_match_all_cli (vlib_main_t *vm, unformat_input_t *input,
...
static clib_error_t *
filter_match_all_show (vlib_main_t *vm, unformat_input_t *input,
		       vlib_cli_command_t *cmd)
{
  index_t fmai;
  ...
  pool_foreach_index (fmai, filter_match_all_pool)
    {
      vlib_cli_output (vm, " %U handle:[%d, %d]", format_filter_match_all,
		       fmai, 0, filter_match_all_type, fmai);
    }
