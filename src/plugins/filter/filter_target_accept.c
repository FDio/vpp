...
filter_target_accept_stack (filter_target_accept_t *fta)
{
  ...
  {
    dpo_stack (filter_target_accept_dpo_type, fta->fta_proto,
	       &fta->fta_next[fht],
...
filter_target_accept_unstack (filter_target_accept_t *fta)
{
  ...
  FOREACH_FILTER_HOOK_BASE_TYPE (fht) { dpo_reset (&fta->fta_next[fht]); }
}

int
filter_target_accept_add_and_lock (index_t fti, dpo_proto_t proto,
				   dpo_id_t *dpo)
{
...
filter_target_accept_from_fib_node (fib_node_t *node)
{
  return ((filter_target_accept_t *) (((char *) node) -
				      STRUCT_OFFSET_OF (filter_target_accept_t,
							fta_node)));
}
...
filter_target_accept_init (vlib_main_t *vm)
{
  ...
  filter_target_accept_dpo_type = dpo_register_new_type (
    &filter_target_accept_vft, filter_target_accept_nodes);
...
static clib_error_t *
filter_target_accept_show (vlib_main_t *vm, unformat_input_t *input,
			   vlib_cli_command_t *cmd)
{
  ...
  pool_foreach (fta, filter_target_accept_pool)
    {
      vlib_cli_output (vm, " %U %U", format_filter_target_accept,
