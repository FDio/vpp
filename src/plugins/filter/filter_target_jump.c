...
filter_target_jump_stack_i (filter_target_jump_t *ftj)
{
  ...
  dpo_stack (filter_target_jump_dpo_type, ftj->ftj_proto, &ftj->ftj_next,
	     filter_chain_rule_dpo_get (ftj->ftj_chain));
  ...
  if (FILTER_HOOK_IS_BASE (ftj->ftj_hook))
    {
      ...
      dpo_stack (filter_target_jump_dpo_type, ftj->ftj_proto, &ftj->ftj_push,
		 filter_chain_push_dpo_get (ftj->ftj_chain));
    }
  else
    {
	...
	dpo_stack (filter_target_jump_dpo_type, ftj->ftj_proto, &ftj->ftj_push,
		   filter_rule_dpo_get (ftj->ftj_rule));
      else
	dpo_stack (filter_target_jump_dpo_type, ftj->ftj_proto, &ftj->ftj_push,
		   drop_dpo_get (ftj->ftj_proto));
    }
}
...
int
filter_target_jump_add_and_lock (dpo_proto_t proto, index_t fci, dpo_id_t *dpo)
{
  filter_target_jump_t *ftj;
  ...
  dpo_set (dpo, filter_target_jump_dpo_type, ftj->ftj_proto,
	   ftj - filter_target_jump_pool);
...
format_filter_target_jump (u8 *s, va_list *args)
{
  ...
  s = format (s, "[%d] jump:[%U rule:%d to:%s]", ftji, format_dpo_proto,
	      ftj->ftj_proto, ftj->ftj_rule, fc->fc_name);
  s = format (s, "\n%U%U", format_white_space, indent + 2, format_dpo_id,
	      &ftj->ftj_next, indent + 4);
  s = format (s, "\n%U%U", format_white_space, indent + 2, format_dpo_id,
	      &ftj->ftj_push, indent + 4);
...
filter_target_jump_back_walk (fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
  filter_target_jump_t *ftj;
...
{
  filter_target_jump_dpo_type = dpo_register_new_type (
    &filter_target_jump_dpo_vft, filter_target_jump_nodes);
...
static clib_error_t *
filter_target_jump_show (vlib_main_t *vm, unformat_input_t *input,
