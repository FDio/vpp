 ...
 * @return error code
 */
extern int filter_table_update (const u8 *name, dpo_proto_t dproto,
				u32 precedence, index_t *fti);
extern index_t filter_table_chain_find (index_t fti, const u8 *chain);

extern int filter_table_chain_add (index_t fti, const u8 *chain,
...
extern int filter_table_rule_append (index_t fti, const u8 *chain,
				     const u8 *rule, const dpo_id_t *match,
				     const dpo_id_t *target, index_t *fri);
extern int filter_table_rule_delete (index_t fti, const u8 *chain,
				     const u8 *rule);
extern int filter_table_rule_delete_index (index_t fti, index_t fci,
					   index_t fri);
extern void filter_table_update_next (index_t fti, index_t next,
...
extern void filter_table_child_add (index_t fti, index_t child_index,
				    filter_node_t *node);
