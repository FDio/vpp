...
void
filter_rule_update (index_t fri, const dpo_id_t *match, const dpo_id_t *target,
		    const dpo_id_t *next)
{
  filter_rule_t *fr;
...
index_t
filter_rule_create_and_lock (const u8 *name, const dpo_id_t *match,
...
format_filter_rule (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U[%d] %s: to:[%Ld:%Ld]", format_white_space, indent, fri,
	      fr->fr_name, count.packets, count.bytes);
  s = format (s, "\n%U%U", format_white_space, indent + 2, format_dpo_id,
	      &fr->fr_match, indent + 4);
  s = format (s, "\n%U%U", format_white_space, indent + 4, format_dpo_id,
	      &fr->fr_target, indent + 6);
  s = format (s, "\n%U%U", format_white_space, indent + 4, format_dpo_id,
	      &fr->fr_next, indent + 6);
...
{
  fn->fn_sibling = fib_node_child_add (filter_rule_node_type, fri,
...
static clib_error_t *
filter_rule_cli (vlib_main_t *vm, unformat_input_t *input,
		 vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "match %U", unformat_filter_match,
			 &match_dpo, dproto))
	;
      else if (unformat (line_input, "target %U", unformat_filter_target,
			 &target_dpo, dproto))
	;
...
static clib_error_t *
filter_rule_show (vlib_main_t *vm, unformat_input_t *input,
		  vlib_cli_command_t *cmd)
{
  index_t fri;
