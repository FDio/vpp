...
filter_chain_update_terminator (filter_chain_t *fc)
{
  ...
  switch (fc->fc_policy)
	...
	filter_target_accept_add_and_lock (fc->fc_table, fc->fc_proto,
					   &fc->fc_terminator);
      else
	filter_target_return_add_and_lock (fc->fc_proto, &fc->fc_terminator);
      break;
...
index_t
filter_chain_create_and_lock (index_t fti, const u8 *name, dpo_proto_t dproto,
			      ...
			      filter_chain_policy_t fcp, u32 precedence)
{
   ...
   */
  filter_target_jump_add_and_lock (fc->fc_proto, fc - filter_chain_pool,
				   &fc->fc_jump);
...
format_filter_chain (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U[%d] %s: hook:%U precedence:%d to:[%Ld:%Ld]",
	      format_white_space, indent, fci, fc->fc_name,
	      format_filter_hook_type, fc->fc_hook, fc->fc_precedence,
	      count.packets, count.bytes);

  s = filter_list_format (s, indent + 2, fc->fc_rules);

  s = format (s, "\n%Uend:%U", format_white_space, indent, format_dpo_id,
	      &fc->fc_terminator, indent + 2);
...
index_t
filter_chain_rule_append (index_t fci, const u8 *name, const dpo_id_t *match,
			  const dpo_id_t *target)
{
  ...
  if (INDEX_INVALID == fri)
    {
      fri =
	filter_rule_create_and_lock (name, match, target, &fc->fc_terminator);
...
{
  fn->fn_sibling = fib_node_child_add (filter_chain_node_type, fci,
				       fn->fn_node.fn_type, child_index);
}
...
static clib_error_t *
filter_chain_cli (vlib_main_t *vm, unformat_input_t *input,
		  vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "%U", unformat_filter_chain_policy, &fcp))
	;
...
static clib_error_t *
filter_chain_show (vlib_main_t *vm, unformat_input_t *input,
		   vlib_cli_command_t *cmd)
{
  index_t fci;
