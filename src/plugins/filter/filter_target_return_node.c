...
format_filter_target_return_trace (u8 *s, va_list *args)
{
  ...
  // filter_target_return_trace_t *t = va_arg (*args,
  // filter_target_return_trace_t *);
...
always_inline uword
filter_target_return_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			     vlib_frame_t *frame, dpo_proto_t dproto)
{
  ...
  while (n_left > 0)
      ...
      if (PREDICT_TRUE (pos0 >= 0))
	{
	  ...
	  dpo0 = &filter_buffer_main.fbm_threads[thread_index]
		    .fptd_stack[bi0][pos0];
...
VLIB_NODE_FN (filter_target_return_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (filter_target_return_inline (vm, node, frame, DPO_PROTO_IP4));
}

VLIB_NODE_FN (filter_target_return_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
VLIB_REGISTER_NODE (filter_target_return_ip4_node) = {
  ...
  // the jump node stacks on the targets that it pushes on the stack
