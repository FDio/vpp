...
int
filter_match_l4_add_and_lock (dpo_proto_t proto, filter_match_dir_t dir,
			      ip_protocol_t ip_proto, u16 port, dpo_id_t *dpo)
{
  ...
  dpo_set (dpo, filter_match_l4_type, fml->fml_base.fm_base.dpoi_proto,
	   fml - filter_match_l4_pool);
...
format_filter_match_l4 (u8 *s, va_list *args)
{
  ...
  s = format (s, "[%d] match:[%U %U %d]", fmli, format_filter_match_dir,
	      fml->fml_dir, format_ip_protocol, fml->fml_iproto,
...
unformat_filter_match_l4 (unformat_input_t *input, va_list *args)
{
  ...
  if (unformat (input, "%U %U %d", unformat_ip_protocol, &ip_proto,
...
const static filter_match_vft_t match_vft = { .fmv_get_base =
						filter_match_l4_get_base,
					      .fmv_unformat =
						unformat_filter_match_l4 };

static clib_error_t *
filter_match_l4_init (vlib_main_t *vm)
{
  filter_match_l4_type =
    dpo_register_new_type (&filter_match_l4_vft, filter_match_l4_nodes);
...
static clib_error_t *
filter_match_l4_show (vlib_main_t *vm, unformat_input_t *input,
		      vlib_cli_command_t *cmd)
{
  ...
  pool_foreach_index (fmli, filter_match_l4_pool)
    {
      vlib_cli_output (vm, " %U handle:[%d, %d]", format_filter_match_l4, fmli,
		       0, filter_match_l4_type, fmli);
    }
