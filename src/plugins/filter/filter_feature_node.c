...
format_filter_feature_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U %U", format_dpo_proto, t->dproto, format_filter_hook_type,
	      t->fht);
...
always_inline uword
filter_feature_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		       vlib_frame_t *frame, filter_hook_type_t fht,
		       dpo_proto_t dproto)
{
   ...
   * filter_starts[dproto][fht].dpoi_index; */
  ...
  while (n_left > 0)
	...
	filter_buffer_main.fbm_threads[thread_index].fptd_stack, bi0,
	CLIB_CACHE_LINE_BYTES);
      vec_reset_length (
	filter_buffer_main.fbm_threads[thread_index].fptd_stack);
...
VLIB_NODE_FN (filter_feature_input_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (
    filter_feature_inline (vm, node, frame, FILTER_HOOK_INPUT, DPO_PROTO_IP4));
}

VLIB_NODE_FN (filter_feature_input_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (
    filter_feature_inline (vm, node, frame, FILTER_HOOK_INPUT, DPO_PROTO_IP6));
}

VLIB_NODE_FN (filter_feature_output_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (filter_feature_inline (vm, node, frame, FILTER_HOOK_OUTPUT,
				 DPO_PROTO_IP4));
}

VLIB_NODE_FN (filter_feature_output_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (filter_feature_inline (vm, node, frame, FILTER_HOOK_OUTPUT,
				 DPO_PROTO_IP6));
