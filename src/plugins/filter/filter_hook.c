...
filter_hook_stack (filter_hook_type_t fht, const dpo_id_t *dpo)
{
  ...
  node = vlib_get_node_by_name (
    vlib_get_main (), (u8 *) filter_feature_nodes[dpo->dpoi_proto][fht]);

  if (node)
    dpo_stack_from_node (node->index, &filter_hook_roots[dpo->dpoi_proto][fht],
			 dpo);
}
...
void
filter_hook_table_add (dpo_proto_t dproto, filter_hook_type_t fht, index_t fti)
{
  filter_hook_t *fh;
  index_t fhi;
...
void
filter_hook_table_remove (dpo_proto_t dproto, filter_hook_type_t fht,
			  index_t fti)
{
  filter_hook_t *fh;
  index_t fhi;
  ...
  filter_list_remove (fh->fh_tables1, fti, &uctx);
}
...
format_filter_hook (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U[%d] hook:[%U %U]:", format_white_space, indent, fhi,
	      format_filter_hook_type, fh->fh_hook, format_dpo_proto,
	      fh->fh_proto);
...
static clib_error_t *
filter_hook_show (vlib_main_t *vm, unformat_input_t *input,
		  vlib_cli_command_t *cmd)
{
...

  {
	...
	if (dpo_id_is_valid (&filter_hook_roots[dproto][fht]))
	  vlib_cli_output (vm, " %U, %U:\n %U", format_dpo_proto, dproto,
			   format_filter_hook_type, fht, format_dpo_id,
			   &filter_hook_roots[dproto][fht], 2);
      }
    }
