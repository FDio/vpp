  ...
  vl_api_memif_socket_filename_add_del_t *mp)
{
  ...
  /* socket_id */
  args->sock_id = clib_net_to_host_u32 (mp->socket_id);
  if (args->sock_id == 0 || args->sock_id == ~0)
    {
  ...
  if (mp->is_add)
    {
      vec_validate (args->sock_filename, len);
      memcpy (args->sock_filename, mp->socket_filename, len);
    }

  if (mp->is_add)
    rv = memif_add_socket_filename (args);
  else
    rv = memif_delete_socket_filename (args->sock_id);
...
reply:
  REPLY_MACRO2 (VL_API_MEMIF_SOCKET_FILENAME_ADD_DEL_V2_REPLY,
		({ rmp->socket_id = htonl (args->sock_id); }));
}
