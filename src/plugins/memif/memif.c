...
void
memif_init_queue_stats(vlib_main_t *vm, memif_if_t * mif, memif_queue_t *mq, u32 i)
{
      u32 j;
      for (j = 0; j < MEMIF_N_Q_WAIT_BIN_COUNTERS; j++) {
	      ...
	      mq->q_wait_jitter_bin_counters[j].name = 0;
              mq->q_wait_jitter_bin_counters[j].stat_segment_name = (void *)
                           format (0, "/memif/%d/queues/%d/q_wait_jitter_bins/%d/hits%c", mif->sw_if_index, i, j, 0);
...
memif_connect (memif_if_t * mif)
{
  ...
  vec_foreach_index (i, mif->tx_queues)
      ...
      if (mq->ring->cookie != MEMIF_COOKIE)
	{
      ...
      memif_init_queue_stats(vm, mif, mq, i);
    }
