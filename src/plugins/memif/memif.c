...
memif_connect (memif_if_t * mif)
{
  ...
  vec_foreach (mr, mif->regions)
	...
	if (vfio_map_extended_mem (vm, uword_to_pointer (mr->shm, u64),
				   (u64) mr->region_size,
				   clib_mem_get_fd_page_size (mr->fd)))
  ...
  vec_foreach_index (i, mif->tx_queues)
      ...
      if (mif->flags & MEMIF_IF_FLAG_USE_DMA)
	{
	  ...
	  mq->dma_data_head = 0;
	  mq->dma_data_tail = 0;
	  mq->dma_data_size = MEMIF_DMA_DATA_SIZE;
	  vec_validate_aligned (mq->dma_data, MEMIF_DMA_DATA_SIZE,
				CLIB_CACHE_LINE_BYTES);

	  vec_foreach (dma_data, mq->dma_data)
	    {
	      vec_validate_aligned (dma_data->data.desc_data,
				    pow2_mask (max_log2_ring_sz),
				    CLIB_CACHE_LINE_BYTES);
	      vec_validate_aligned (dma_data->data.desc_len,
				    pow2_mask (max_log2_ring_sz),
				    CLIB_CACHE_LINE_BYTES);
	      vec_validate_aligned (dma_data->data.desc_status,
				    pow2_mask (max_log2_ring_sz),
				    CLIB_CACHE_LINE_BYTES);
	      vec_validate_aligned (dma_data->data.copy_ops, 0,
				    CLIB_CACHE_LINE_BYTES);
	      vec_reset_length (dma_data->data.copy_ops);
	      vec_validate_aligned (dma_data->data.buffers, 0,
				    CLIB_CACHE_LINE_BYTES);
	      vec_reset_length (dma_data->data.buffers);
	    }
	}
    }
...
memif_init_regions_and_queues (memif_if_t * mif)
{
  ...
  uword n_pages =
    ((r->region_size - 1) >> clib_mem_get_fd_log2_page_size (fd)) + 1;
  r->region_size = n_pages * clib_mem_get_fd_page_size (fd);
