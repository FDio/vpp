...
void
memif_init_queue_stats (vlib_main_t *vm, memif_if_t *mif, memif_queue_t *mq,
			char *kind, u32 i)
{
  u32 j;
  for (j = 0; j < MEMIF_N_Q_WAIT_BIN_COUNTERS; j++)
    {
      ...
      mq->q_wait_bin_counters[j].stat_segment_name =
	(void *) format (0, "/memif/%d/queues/%s/%d/q_wait_bins/%d/hits%c",
			 mif->sw_if_index, kind, i, j, 0);
      vlib_validate_simple_counter (&mq->q_wait_bin_counters[j], 0);
      vlib_zero_simple_counter (&mq->q_wait_bin_counters[j], 0);

      mq->q_wait_fullq_bin_counters[j].name = 0;
      mq->q_wait_fullq_bin_counters[j].stat_segment_name = (void *) format (
	0, "/memif/%d/queues/%s/%d/q_wait_fullq_bins/%d/hits%c",
	mif->sw_if_index, kind, i, j, 0);
      vlib_validate_simple_counter (&mq->q_wait_fullq_bin_counters[j], 0);
      vlib_zero_simple_counter (&mq->q_wait_fullq_bin_counters[j], 0);

      mq->q_wait_jitter_bin_counters[j].name = 0;
      mq->q_wait_jitter_bin_counters[j].stat_segment_name = (void *) format (
	0, "/memif/%d/queues/%s/%d/q_wait_jitter_bins/%d/hits%c",
	mif->sw_if_index, kind, i, j, 0);
