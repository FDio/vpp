...
format_memif_queue (u8 * s, va_list * args)
{
  ...
  for (j=0; j<MEMIF_N_Q_WAIT_BIN_COUNTERS; j++) {
     s = format (s, "%Ubin %d (%ld .. %ld ns): %d [ %d ] / %d\n",
     ...
     vlib_set_simple_counter ( &mq->q_wait_bin_counters[j], vm->thread_index, 0, 0);
     vlib_set_simple_counter ( &mq->q_wait_jitter_bin_counters[j], vm->thread_index, 0, 0);
     vlib_set_simple_counter ( &mq->q_wait_fullq_bin_counters[j], vm->thread_index, 0, 0);
  }
