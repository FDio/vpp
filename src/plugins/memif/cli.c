		       ...
		       vlib_cli_command_t * cmd)
{
  ...
  pool_foreach (mif, mm->interfaces,
    ({
       ...
       vlib_cli_output (vm, "  id %d file %s", mif->id, msf->filename);
       ...
       if (mif->local_disc_string)
         vlib_cli_output (vm, "  local-disc-string \"%s\"", mif->local_disc_string);
       if (mif->remote_disc_string)
         vlib_cli_output (vm, "  remote-disc-string \"%s\"", mif->remote_disc_string);
