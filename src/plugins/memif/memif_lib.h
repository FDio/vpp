...
typedef struct
{
  ...
  u64 next_alloc __attribute__ ((aligned (128)));
...
void memif_loop_run (int timeout);
...
memif_alloc_buffer (memif_if_t * mif, memif_ring_t * ring, u32 buffer_size)
{
    ...
    {
        if (mif->head_offset >= ring->tail - ring->head)
            return NULL;
    }
    else
    {
        if (mif->head_offset >= mask - ring->head + ring->tail)
            return NULL;
    }
 
    void *region = mif->regions[ring->desc[h].region];

    void *next_alloc = region + ring->next_alloc;
    ...
    ring->desc[h].offset = ring->next_alloc;
