...
#define MEMIF_IF_FLAG_INTERRUPT  (1 << 5)
...
typedef struct
{
    ...
    u8 *socket_filename;
    int is_master;
    u8 is_int;/*interrupt mode... move to memif_main?*/
    u8 log2_ring_size;
    u64 buffer_size;
} memif_create_args_t;


void memif_file_update (unix_file_t *f, unix_file_update_type_t update_type);

void memif_set_file_update ();
...
int memif_send (uword if_index, memif_ring_type_t type);

void memif_recv (uword if_index, struct iovec **iov);

always_inline memif_ring_t *
...
memif_get_buffer_len (memif_if_t *mif, memif_ring_t *ring, u16 slot)
{
    return ring->desc[slot].buffer_length;
}
...
memif_alloc_buffer (memif_if_t * mif, memif_ring_t * ring, u32 buffer_size)
{
    ...
    u16 h = ring->head + mif->head_offset & mask;
    if (mif->head_offset >= mask){
        return NULL;
    }
       
    u16 region = ring->desc[h].region;
    
    u64 buffer_offset = sizeof (memif_shm_t) +
        (mif->num_s2m_rings + mif->num_m2s_rings) *
        (sizeof (memif_ring_t) +
        sizeof (memif_desc_t) * (1 << mif->log2_ring_size));
    void *shm_end = mif->regions[region] + buffer_offset + (mif->buffer_size *
         (mif->num_s2m_rings + mif->num_m2s_rings));
    
    void *tail_offset = mif->regions[region] + ring->desc[ring->tail].offset;

    void *next_alloc_end = mif->regions[region] + mif->next_alloc + buffer_size;
    void *next_alloc_end1 = mif->regions[region] + buffer_offset + buffer_size;
  
    if (next_alloc_end > shm_end){
        if (next_alloc_end1 > tail_offset){
            return NULL;
        }
        ring->desc[h].offset = buffer_offset;
    }
    else
    {
        if ((mif->regions[region] + mif->next_alloc) < tail_offset){
            if (next_alloc_end > tail_offset){
                return NULL;
            }
        }
        ring->desc[h].offset = mif->next_alloc;
    }
    ...
    mif->head_offset++;
    return mif->regions[region] + ring->desc[h].offset;
}
