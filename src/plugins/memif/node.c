...
memif_process_buffer_metdata (vlib_main_t *vm, vlib_node_runtime_t *node,
			      memif_per_thread_data_t *ptd, u32 *bi, u16 *next,
			   ...
			   memif_if_t *mif, memif_queue_t *mq)
{
  ...
  memif_ring_t *ring;
  u32 _to_next_bufs[MEMIF_RX_VECTOR_SZ], *to_next_bufs;
  u32 n_rx_packets = 0, n_rx_bytes = 0, n_buffers, n_left_to_next;
  u32 next_index = VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT;
  vlib_buffer_t _bt, *bt = &_bt;
  memif_per_thread_data_t *ptd =
    vec_elt_at_index (mm->per_thread_data, vm->thread_index);
  u16 slot, n_slots, n_alloc;
  i16 start_off;
  ...
  if (is_ip)
    {
      to_next_bufs = _to_next_bufs;
      n_rx_bytes =
	memif_process_buffer_metdata (vm, node, ptd, to_next_bufs, ptd->nexts,
				      bt, n_rx_packets, /* is_ip */ 1);
    }
  else
    {
      ...
      n_rx_bytes =
	memif_process_buffer_metdata (vm, node, ptd, to_next_bufs, ptd->nexts,
				      bt, n_rx_packets, /* is _ip */ 0);
    }

  memif_trace (vm, node, mif, mq->queue_index, to_next_bufs, ptd->nexts,
	       next_index, n_rx_packets);

  if (is_ip)
    {
      vlib_buffer_enqueue_to_next (vm, node, to_next_bufs, ptd->nexts,
				   n_rx_packets);
    }
  else
    {
