...
memif_parse_desc (memif_per_thread_data_t *ptd, memif_if_t *mif,
		  memif_queue_t *mq, u16 next, u16 n_avail, int is_slave)
{
  ...
  memif_desc_t *d = 0;
  u32 buffer_size = mif->run.buffer_size;
  u32 slot = next;
	  ...
	  if (is_slave)
	    {
	      ...
	      u32 n = n_desc - i;
	      while (n > 1)
		descs[(slot - n--) & mask].length = buffer_size;
	    }
	  n_desc = i;
		    ...
		    int maybe_chained)
{
  ...
  while (n_left)
	...
	while (PREDICT_FALSE (n_bytes_left));
      ...
      if (maybe_chained && (desc_status[i].next))
	{
	  src_off = 0;
	  goto next_slot;
	}
			   ...
			   memif_interface_mode_t mode)
{
  ...
  if (mif->flags & MEMIF_IF_FLAG_IS_SLAVE)
    n_desc =
      memif_parse_desc (ptd, mif, mq, cur_slot, n_slots, /* is_slave */ 1);
  else
    n_desc =
      memif_parse_desc (ptd, mif, mq, cur_slot, n_slots, /* is_slave */ 0);
  ...
  if (is_simple)
    ...
    n_buffers = memif_process_desc (vm, node, ptd, mif, /* maybe_chained */ 1);
