...
poll_event (int timeout)
{
    ...
    memset (&evt, 0, sizeof(evt));
    evt.events = EPOLLIN;
    int error = epoll_pwait (epfd, &evt, 1, timeout, NULL);
    if (error < 0)
...
void
memif_recv (uword if_index, struct iovec **iov)
{
    memif_if_t *mif = memif_dump(if_index);
    ...
    while (num_slots)
    {
        (*iov + c)->iov_len = memif_get_buffer_len (mif, ring, rd->last_head);
