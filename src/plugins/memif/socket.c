...
memif_msg_send_connect (memif_if_t * mif)
{
  ...
  strncpy ((char *) c->name, (char *) s, sizeof (c->name));
  vec_free (s);
  return memif_msg_send (mif->conn_fd, &msg, -1);
}

static clib_error_t *
memif_msg_send_connected (memif_if_t * mif)
{
  memif_msg_t msg = { 0 };
...
memif_msg_receive_hello (memif_if_t * mif, memif_msg_t * msg)
{
  ...
  mif->run.num_s2m_rings = clib_min (msg->hello.max_s2m_ring + 1,
				     mif->cfg.num_s2m_rings);
  mif->run.num_m2s_rings = clib_min (msg->hello.max_m2s_ring + 1,
...
memif_msg_receive_connect (memif_if_t * mif, memif_msg_t * msg)
{
  ...
  mif->remote_if_name = memif_str2vec (c->name, sizeof (c->name));
...
memif_msg_receive_disconnect (memif_if_t * mif, memif_msg_t * msg)
{
  ...
  err = clib_error_return (0, "disconnect received");
...
memif_msg_receive (memif_if_t * mif, unix_file_t * uf)
{
  ...
  switch (msg.type)
	...
	return err;
      if ((err = memif_msg_send_connected (mif)))
	return err;
      break;
