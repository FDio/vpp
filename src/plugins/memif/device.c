...
retry:
  ...
  while (n_left && free_slots)
	...
	ring->desc[(saved_slot + i) & mask] = vec_elt (pending_descs, i);
      vec_reset_length (pending_descs);
      /* Apply pending buffer indices. */
      vec_foreach_index (i, pending_bis)
	vec_add1_aligned (ptd->buffers, vec_elt (pending_bis, i),
			  CLIB_CACHE_LINE_BYTES);
      /* TODO: vec_extend_aligned? */
      vec_reset_length (pending_bis);
...
retry:
  ...
  while (n_left && free_slots)
	...
	ring->desc[(slot - slots_in_packet + i) & mask] =
      ...
      /* Apply pending buffer indices. */
      vec_foreach_index (i, pending_bis)
	mq->buffers[(slot - slots_in_packet + i) & mask] =
	  vec_elt (pending_bis, i);
      vec_reset_length (pending_bis);
