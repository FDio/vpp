			   ...
			   memif_ring_type_t type)
{
  ...
  memif_queue_t *mq;
  int i, n_retries = 5;
  int _n_segs[n_left], *n_segs = _n_segs;
  ...
  while (n_left && free_slots)
    {
      ...
      u32 saved_ptd_copy_ops_len = vec_len (ptd->copy_ops);
      u32 saved_ptd_buffers_len = vec_len (ptd->buffers);
      u16 saved_slot = slot;
...
no_free_slots:
  ...
  vec_foreach_index (i, ptd->copy_ops)
  {
    ...
    vlib_buffer_t *b = vlib_get_buffer (vm, bi);
    clib_memcpy (co->data, b->data + co->buffer_offset, co->data_len);
#if 0
    fformat (stderr, "tx index %u src %p dst %p len %u bvi %u offset %u\n", i,
	     vlib_buffer_get_current (b) + co->buffer_offset, co->data,
	     co->data_len, co->buffer_vec_index, co->buffer_offset);
    //fformat (stderr, "%U\n", format_hexdump, co->data, co->data_len);
#endif
  }
					...
					vlib_frame_t * f)
{
  ...
  vnet_interface_output_runtime_t *rund = (void *) n->runtime_data;
  memif_if_t *mif = pool_elt_at_index (nm->interfaces, rund->dev_instance);
  uword r;

  if (mif->flags & MEMIF_IF_FLAG_IS_SLAVE)
    r = memif_interface_tx_inline (vm, n, f, mif, MEMIF_RING_S2M);
  else
    r = memif_interface_tx_inline (vm, n, f, mif, MEMIF_RING_M2S);
  return r;
}
