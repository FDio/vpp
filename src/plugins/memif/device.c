...
retry:
  ...
  if (free_slots > mq->saved_free_slots)
      ...
      if (queue_wait_measurement_uncertainty < 0.1)
	{
	  ...
	  vlib_increment_simple_counter (&mq->q_wait_bin_counters[bin_index],
					 vm->thread_index, 0, 1);
	  vlib_increment_simple_counter (
	    &mq->q_wait_jitter_bin_counters[jitter_bin_index],
	    vm->thread_index, 0, 1);
	}
    ...
    {
      /* We can not put any new packets into the queue, mark this */
      f64 queue_wait_time = now - mq->last_known_drain_time;
      f64 queue_wait_measurement_uncertainty = now - mq->last_tx_start_time;
      if (queue_wait_measurement_uncertainty < 0.1)
	{
	  ...
	  u8 bin_index = count_leading_zeros ((u64) queue_wait_time_ns) - 32;
	  if (mq->saved_qfull_bin != 255)
	    {
	      vlib_increment_simple_counter (
		&mq->q_wait_fullq_bin_counters[mq->saved_qfull_bin],
		vm->thread_index, 0, 1);
	    }

	  vlib_increment_simple_counter (
	    &mq->q_wait_fullq_bin_counters[bin_index], vm->thread_index, 0, 1);
	  mq->saved_qfull_bin = bin_index;
	}
    }
