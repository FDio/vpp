	       ...
	       vlib_dma_batch_t *b)
{
  ...
  dma_info->state = DMA_INFO_STATE_COMPLETED;
  vec_reset_length (ptd->buffers);
  vec_reset_length (ptd->copy_ops);
...
no_free_slots:
  ...
  if (PREDICT_TRUE (!fallback))
    {
      ...
      dma_info->state = DMA_INFO_STATE_SUBMITTED;
  ...
  else if (fallback && dma_info->state == DMA_INFO_STATE_COMPLETED)
    {
					      ...
					      vlib_frame_t * frame)
{
  ...
  if (mif->flags & MEMIF_IF_FLAG_ZERO_COPY)
	...
	n_left = memif_interface_tx_dma_inline (
	  vm, node, from, mif, MEMIF_RING_M2S, mq, n_left);
      else
