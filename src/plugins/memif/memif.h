...
#define MEMIF_RING_FLAG_MASK_INT 1
  uint16_t head __attribute__ ((aligned (MEMIF_CACHELINE_SIZE)));
  uint16_t tail __attribute__ ((aligned (MEMIF_CACHELINE_SIZE)));
  memif_desc_t desc[0] __attribute__ ((aligned (MEMIF_CACHELINE_SIZE)));
} memif_ring_t;
