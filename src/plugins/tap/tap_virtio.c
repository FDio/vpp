...
tap_virtio_main_t tap_virtio_main;
...
clib_error_t *
tap_virtio_vring_init (vlib_main_t *vm, tap_virtio_if_t *vif, u16 idx, u16 sz)
{
  ...
  tap_virtio_log_debug (vif, "vring %u size %u call_fd %d kick_fd %d", idx,
...
inline void
tap_virtio_free_buffers (vlib_main_t *vm, vnet_virtio_vring_t *vring)
{
...
clib_error_t *
tap_virtio_vring_free_rx (vlib_main_t *vm, tap_virtio_if_t *vif, u32 idx)
{
  vnet_virtio_vring_t *vring =
  ...
  if (vring->used)
    {
      tap_virtio_free_buffers (vm, vring);
...
clib_error_t *
tap_virtio_vring_free_tx (vlib_main_t *vm, tap_virtio_if_t *vif, u32 idx)
{
  vnet_virtio_vring_t *vring =
  ...
  tap_virtio_vring_buffering_free (vm, vring->buffering);
  clib_spinlock_free (&vring->lockp);
  return 0;
}

void
tap_virtio_set_packet_coalesce (tap_virtio_if_t *vif)
{
  vnet_main_t *vnm = vnet_get_main ();
...
static void
tap_virtio_vring_fill (vlib_main_t *vm, tap_virtio_if_t *vif,
		       vnet_virtio_vring_t *vring)
{
  tap_virtio_refill_vring_split (vm, vif, vif->type, vring,
				 vif->tap_virtio_net_hdr_sz,
				 tap_virtio_input_node.index);
}

void
tap_virtio_vring_set_rx_queues (vlib_main_t *vm, tap_virtio_if_t *vif)
{
  vnet_main_t *vnm = vnet_get_main ();
  vnet_virtio_vring_t *vring;
  ...
  vec_foreach (vring, vif->rxq_vrings)
	...
	.description = format (0, "%U vring %u", format_tx_node_name,
      ...
      tap_virtio_vring_fill (vm, vif, vring);
    }
  vnet_hw_if_update_runtime_data (vnm, vif->hw_if_index);
}

void
tap_virtio_vring_set_tx_queues (vlib_main_t *vm, tap_virtio_if_t *vif)
{
  vnet_main_t *vnm = vnet_get_main ();
  vnet_virtio_vring_t *vring;
  ...
  if (vif->num_txqs == 0)
    {
      tap_virtio_log_error (vif, "Interface %U has 0 txq",
...
inline void
tap_virtio_set_net_hdr_size (tap_virtio_if_t *vif)
{
  if (vif->features & VIRTIO_FEATURE (VIRTIO_NET_F_MRG_RXBUF) ||
      vif->features & VIRTIO_FEATURE (VIRTIO_F_VERSION_1))
    vif->tap_virtio_net_hdr_sz = sizeof (vnet_virtio_net_hdr_v1_t);
  else
    vif->tap_virtio_net_hdr_sz = sizeof (vnet_virtio_net_hdr_t);
}

inline void
tap_virtio_show (vlib_main_t *vm, u32 *hw_if_indices, u8 show_descr,
  ...
  for (hw_if_index = 0; hw_if_index < vec_len (hw_if_indices); hw_if_index++)
      ...
      if (type & (VIRTIO_IF_TYPE_TAP | VIRTIO_IF_TYPE_TUN))
	{
	  ...
	  if (vif->host_mtu_size)
	    vlib_cli_output (vm, "  host-mtu-size \"%d\"", vif->host_mtu_size);
      ...
      vec_foreach_index (i, vif->txq_vrings)
	{
	  ...
	  if (vif->packet_buffering)
	    {
	      vlib_cli_output (vm, "    %U", tap_virtio_vring_buffering_format,
			       vring->buffering);
	    }
	  if (show_descr)
	    {
...
static clib_error_t *
tap_virtio_init (vlib_main_t *vm)
{
  tap_virtio_main_t *vim = &tap_virtio_main;
  clib_error_t *error = 0;

  vim->log_default = vlib_log_register_class ("tap-virtio", 0);
  vlib_log_debug (vim->log_default, "initialized");

  return error;
}

VLIB_INIT_FUNCTION (tap_virtio_init);
