...
typedef struct
{
  ...
  u16 back;
  u8 is_enable;
} tap_virtio_vring_buffering_t;

static_always_inline clib_error_t *
tap_virtio_vring_buffering_init (tap_virtio_vring_buffering_t **buffering,
				 u32 node_index, u16 size)
{
  ...
  tap_virtio_vring_buffering_t *b_temp = 0;
  b_temp = (tap_virtio_vring_buffering_t *) clib_mem_alloc (
    sizeof (tap_virtio_vring_buffering_t));
  if (!b_temp)
    return clib_error_return (0, "buffering: memory allocation failed");

  clib_memset (b_temp, 0, sizeof (tap_virtio_vring_buffering_t));
...
tap_virtio_vring_buffering_buffers_free (
  vlib_main_t *vm, tap_virtio_vring_buffering_t *buffering)
{
...
static_always_inline void
tap_virtio_vring_buffering_free (vlib_main_t *vm,
				 tap_virtio_vring_buffering_t *buffering)
{
  if (buffering)
    {
      tap_virtio_vring_buffering_buffers_free (vm, buffering);
...
tap_virtio_vring_buffering_is_enable (tap_virtio_vring_buffering_t *buffering)
{
  if (buffering)
...
static_always_inline void
tap_virtio_vring_buffering_set_is_enable (
  tap_virtio_vring_buffering_t *buffering, u8 is_enable)
{
  if (buffering)
...
static_always_inline void
tap_virtio_vring_buffering_set_timeout (
  vlib_main_t *vm, tap_virtio_vring_buffering_t *buffering, f64 timeout_expire)
{
  if (buffering)
...
static_always_inline u8
tap_virtio_vring_buffering_is_timeout (vlib_main_t *vm,
				       tap_virtio_vring_buffering_t *buffering)
{
...
static_always_inline u8
tap_virtio_vring_buffering_is_empty (tap_virtio_vring_buffering_t *buffering)
{
...
static_always_inline u8
tap_virtio_vring_buffering_is_full (tap_virtio_vring_buffering_t *buffering)
{
...
tap_virtio_vring_n_buffers (tap_virtio_vring_buffering_t *buffering)
{
  return (buffering->size - buffering->free_size);
}

static_always_inline u16
tap_virtio_vring_buffering_store_packets (
  tap_virtio_vring_buffering_t *buffering, u32 *bi, u16 n_store)
{
  u16 mask, n_s = 0, i = 0;

  if (!tap_virtio_vring_buffering_is_enable (buffering) ||
      tap_virtio_vring_buffering_is_full (buffering))
    return 0;
...
tap_virtio_vring_buffering_read_from_front (
  tap_virtio_vring_buffering_t *buffering)
{
  u32 bi = ~0;
  u16 mask = buffering->size - 1;
  if (tap_virtio_vring_buffering_is_empty (buffering))
    return bi;
...
static_always_inline u32
tap_virtio_vring_buffering_read_from_back (
  tap_virtio_vring_buffering_t *buffering)
{
  u32 bi = ~0;
  u16 mask = buffering->size - 1;
...
static_always_inline void
tap_virtio_vring_buffering_schedule_node_on_dispatcher (
  vlib_main_t *vm, vnet_hw_if_tx_queue_t *txq,
  tap_virtio_vring_buffering_t *buffering)
{
  if (buffering && tap_virtio_vring_buffering_is_timeout (vm, buffering) &&
      tap_virtio_vring_n_buffers (buffering))
    {
      ...
      f_to[f->n_vectors] =
	tap_virtio_vring_buffering_read_from_back (buffering);
      f->n_vectors++;
      vlib_put_frame_to_node (vm, buffering->node_index, f);
      tap_virtio_vring_buffering_set_timeout (vm, buffering,
					      VIRTIO_BUFFERING_TIMEOUT);
    }
}

static_always_inline u8 *
tap_virtio_vring_buffering_format (u8 *s, va_list *args)
{
  tap_virtio_vring_buffering_t *buffering =
    va_arg (*args, tap_virtio_vring_buffering_t *);
  ...
  s = format (s, "%Usize %u n_buffers %u front %u back %u", format_white_space,
	      indent, buffering->size, tap_virtio_vring_n_buffers (buffering),
