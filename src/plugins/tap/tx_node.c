...
tap_free_used_device_desc (vlib_main_t *vm, vnet_virtio_vring_t *vring)
{
  ...
  vnet_virtio_vring_used_elem_t *ring = vring->used->ring;
  vnet_virtio_vring_desc_t *desc = vring->desc;
  u32 *buffers = vring->buffers;
  u16 desc_freelist = vring->desc_freelist_head;

  n_left = __atomic_load_n (&vring->used->idx, __ATOMIC_ACQUIRE) - last;
  ...
  vring->desc_freelist_head = desc_freelist;
  vring->desc_in_use -= n_free;
...
tap_if_tx_inline (vlib_main_t *vm, vlib_node_runtime_t *node, tap_if_t *tif,
		  vnet_virtio_vring_t *vring, u32 *buffers, u16 n_buffers,
		  int is_tun)
{
  u16 avail;
  u16 sz = vring->queue_size;
  u16 mask = sz - 1;
  u16 *avail_ring = vring->avail->ring;
  u16 desc_freelist_head = vring->desc_freelist_head;
  u32 *vring_buffers = vring->buffers;
  vnet_virtio_vring_desc_t *desc = vring->desc;
  ...
  u16 n_desc = sz - vring->desc_in_use;
  ...
  if (n_enq)
    {
      __atomic_store_n (&vring->avail->idx, avail, __ATOMIC_RELEASE);
      vring->desc_freelist_head = desc_freelist_head;
      vring->desc_in_use += n_enq;
      vring->total_packets += n_enq;
      if ((__atomic_load_n (&vring->used->flags, __ATOMIC_ACQUIRE) &
	   VRING_USED_F_NO_NOTIFY) == 0)
	{
	  ssize_t __clib_unused rv;
	  rv = write (vring->kick_fd, &(u64){ 1 }, sizeof (u64));
	}
    }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (tf->shared_queue)
    CLIB_SPINLOCK_LOCK (vring->lock)

  if (tif->packet_coalesce)
    {
      n_pkts = vnet_gro_inline (vm, vring->flow_table, buffers, n_pkts, to);
      buffers = to;
...
retry:
  ...
  tap_free_used_device_desc (vm, vring);

  if (tif->is_tun)
    n = tap_if_tx_inline (vm, node, tif, vring, buffers, n_left, 1);
  else
    n = tap_if_tx_inline (vm, node, tif, vring, buffers, n_left, 0);
  ...
  if (tf->shared_queue)
    CLIB_SPINLOCK_UNLOCK (vring->lock);
