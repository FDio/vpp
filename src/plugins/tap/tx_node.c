...
static void
tap_virtio_tx_trace (vlib_main_t *vm, vlib_node_runtime_t *node,
		     vlib_buffer_t *b0, u32 bi, int is_tun)
{
  tap_virtio_tx_trace_t *t;
...
static void
tap_virtio_interface_drop_inline (vlib_main_t *vm, tap_virtio_if_t *vif,
...
static void
tap_virtio_memset_ring_u32 (u32 *ring, u32 start, u32 ring_size, u32 n_buffers)
{
...
static void
tap_virtio_free_used_device_desc_split (vlib_main_t *vm,
  ...
  while (n_left)
      ...
      while (e->id == (n_buffers & mask))
	{
      ...
      vlib_buffer_free_from_ring (vm, vring->buffers, slot, sz,
				  (n_buffers - slot));
      tap_virtio_memset_ring_u32 (vring->buffers, slot, sz,
				  (n_buffers - slot));
      used -= (n_buffers - slot);
...
static void
tap_virtio_free_used_device_desc (vlib_main_t *vm, vnet_virtio_vring_t *vring,
				  uword node_index)
{
  tap_virtio_free_used_device_desc_split (vm, vring, node_index);
}
...
add_buffer_to_slot (vlib_main_t *vm, vlib_node_runtime_t *node,
		    tap_virtio_if_t *vif, vnet_virtio_vring_t *vring, u32 bi,
		    u16 free_desc_count, u16 avail, u16 next, u16 mask,
		    int hdr_sz, int do_gso, int csum_offload, int is_tun,
		    int is_indirect, int is_any_layout)
{
  ...
  if (PREDICT_FALSE (b->flags & VLIB_BUFFER_IS_TRACED))
    {
      tap_virtio_tx_trace (vm, node, b, bi, is_tun);
    }
  ...
  if (drop_inline != ~0)
    tap_virtio_interface_drop_inline (vm, vif, node->node_index, &bi, 1,
				      drop_inline);
...
static void
tap_virtio_find_free_desc (vnet_virtio_vring_t *vring, u16 size, u16 mask,
...
static u16
tap_virtio_interface_tx_split_gso_inline (
  vlib_main_t *vm, vlib_node_runtime_t *node, tap_virtio_if_t *vif,
  tap_virtio_if_type_t type, vnet_virtio_vring_t *vring, u32 *buffers,
  u16 n_left, int do_gso, int csum_offload)
{
  ...
  u16 sz = vring->queue_size;
  int hdr_sz = vif->tap_virtio_net_hdr_sz;
  u16 mask = sz - 1;
  ...
  if (PREDICT_FALSE (vring->flags & VRING_TX_OUT_OF_ORDER))
    {
      ...
      tap_virtio_find_free_desc (vring, sz, mask, n_left, next,
  ...
  if (vif->packet_buffering)
    {
      n_buffers = n_buffers_left =
	tap_virtio_vring_n_buffers (vring->buffering);
      ...
      while (n_buffers_left && free_desc_count)
	{
	  ...
	  u32 bi =
	    tap_virtio_vring_buffering_read_from_front (vring->buffering);
	  if (bi == ~0)
	    break;
      ...
      tap_virtio_txq_clear_scheduled (vring);
    }
...
static u16
tap_virtio_interface_tx_gso_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
				    tap_virtio_if_t *vif,
				    tap_virtio_if_type_t type,
				    vnet_virtio_vring_t *vring, u32 *buffers,
				    u16 n_left, int do_gso, int csum_offload)
{
  return tap_virtio_interface_tx_split_gso_inline (
    vm, node, vif, type, vring, buffers, n_left, do_gso, csum_offload);
}

static u16
tap_virtio_interface_tx_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
				tap_virtio_if_t *vif,
				vnet_virtio_vring_t *vring,
				tap_virtio_if_type_t type, u32 *buffers,
				u16 n_left)
{
  ...
  if (hw->caps & VNET_HW_IF_CAP_TCP_GSO)
    return tap_virtio_interface_tx_gso_inline (vm, node, vif, type, vring,
					       buffers, n_left, 1 /* do_gso */,
					       1 /* checksum offload */);
  else if (hw->caps & VNET_HW_IF_CAP_L4_TX_CKSUM)
    return tap_virtio_interface_tx_gso_inline (
      vm, node, vif, type, vring, buffers, n_left, 0 /* no do_gso */,
      1 /* checksum offload */);
  else
    return tap_virtio_interface_tx_gso_inline (
      vm, node, vif, type, vring, buffers, n_left, 0 /* no do_gso */,
...
{
  tap_virtio_main_t *nm = &tap_virtio_main;
  vnet_interface_output_runtime_t *rund = (void *) node->runtime_data;
  tap_virtio_if_t *vif =
    pool_elt_at_index (nm->interfaces, rund->dev_instance);
...
retry:
  ...
  tap_virtio_free_used_device_desc (vm, vring, node->node_index);

  if (vif->type == VIRTIO_IF_TYPE_TAP)
    n_left =
      tap_virtio_interface_tx_inline (vm, node, vif, vring, VIRTIO_IF_TYPE_TAP,
				      &buffers[n_vectors - n_left], n_left);
  else if (vif->type == VIRTIO_IF_TYPE_TUN)
    n_left =
      tap_virtio_interface_tx_inline (vm, node, vif, vring, VIRTIO_IF_TYPE_TUN,
				      &buffers[n_vectors - n_left], n_left);
  else
    ASSERT (0);
  ...
  if (vif->packet_buffering && n_left)
    {
      u16 n_buffered = tap_virtio_vring_buffering_store_packets (
	vring->buffering, &buffers[n_vectors - n_left], n_left);
      n_left -= n_buffered;
    }
  if (n_left)
    tap_virtio_interface_drop_inline (vm, vif, node->node_index,
				      &buffers[n_vectors - n_left], n_left,
...
static void
tap_virtio_set_interface_next_node (vnet_main_t *vnm, u32 hw_if_index,
				    u32 node_index)
{
  tap_virtio_main_t *apm = &tap_virtio_main;
  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
  tap_virtio_if_t *vif = pool_elt_at_index (apm->interfaces, hw->dev_instance);
  ...
  vif->per_interface_next_index = vlib_node_add_next (
    vlib_get_main (), tap_virtio_input_node.index, node_index);
}

static void
tap_virtio_clear_hw_interface_counters (u32 instance)
{
  /* Nothing for now */
}

static void
tap_virtio_set_rx_interrupt (tap_virtio_if_t *vif, vnet_virtio_vring_t *vring)
{
  vring->avail->flags &= ~VRING_AVAIL_F_NO_INTERRUPT;
}

static void
tap_virtio_set_rx_polling (tap_virtio_if_t *vif, vnet_virtio_vring_t *vring)
{
  vring->avail->flags |= VRING_AVAIL_F_NO_INTERRUPT;
}

static clib_error_t *
tap_virtio_interface_rx_mode_change (vnet_main_t *vnm, u32 hw_if_index,
				     u32 qid, vnet_hw_if_rx_mode mode)
{
  tap_virtio_main_t *mm = &tap_virtio_main;
  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
  tap_virtio_if_t *vif = pool_elt_at_index (mm->interfaces, hw->dev_instance);
  vnet_virtio_vring_t *rx_vring = vec_elt_at_index (vif->rxq_vrings, qid);

  if (mode == VNET_HW_IF_RX_MODE_POLLING)
    tap_virtio_set_rx_polling (vif, rx_vring);
  else
    tap_virtio_set_rx_interrupt (vif, rx_vring);
...
static clib_error_t *
tap_virtio_interface_admin_up_down (vnet_main_t *vnm, u32 hw_if_index,
				    u32 flags)
{
  tap_virtio_main_t *mm = &tap_virtio_main;
  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
...
VNET_DEVICE_CLASS (tap_virtio_device_class) = {
  ...
  .format_device = format_tap_virtio_device,
  .format_tx_trace = format_tap_virtio_tx_trace,
  .tx_function_n_errors = VIRTIO_TX_N_ERROR,
  .tx_function_error_strings = tap_virtio_tx_func_error_strings,
  .rx_redirect_to_node = tap_virtio_set_interface_next_node,
  .clear_counters = tap_virtio_clear_hw_interface_counters,
  .admin_up_down_function = tap_virtio_interface_admin_up_down,
  .rx_mode_change_function = tap_virtio_interface_rx_mode_change,
};
