...
tap_interface_drop_inline (vlib_main_t *vm, tap_if_t *tif, uword node_index,
			   u32 *buffers, u16 n, tap_tx_func_error_t error)
{
				   ...
				   int do_gso, int csum_offload)
{
  ...
  int is_indirect = ((tif->features & VIRTIO_RING_F_INDIRECT_DESC_BIT) != 0);
  int is_any_layout = ((tif->features & VIRTIO_F_ANY_LAYOUT_BIT) != 0);
  u16 sz = vring->queue_size;
  ...
  if (PREDICT_FALSE (vring->tx_out_of_order))
    {
