...
#define tap_virtio_log_error(tif, f, ...)                                     \
  vlib_log (VLIB_LOG_LEVEL_ERR, tap_main.log_default, "tap%u: " f,            \
	    (tif)->dev_instance, ##__VA_ARGS__)
#define tap_virtio_log_debug(tif, f, ...)                                     \
  vlib_log (VLIB_LOG_LEVEL_DEBUG, tap_main.log_default, "tap%u: " f,          \
	    (tif)->dev_instance, ##__VA_ARGS__)

clib_error_t *tap_vring_init (vlib_main_t *vm, tap_if_t *tif, u16 idx, u16 sz);
clib_error_t *tap_vring_free_rx (vlib_main_t *vm, tap_if_t *tif, u32 idx);
clib_error_t *tap_vring_free_tx (vlib_main_t *vm, tap_if_t *tif, u32 idx);
void tap_vring_set_rx_queues (vlib_main_t *vm, tap_if_t *tif);
void tap_vring_set_tx_queues (vlib_main_t *vm, tap_if_t *tif);
void tap_set_net_hdr_size (tap_if_t *tif);
void tap_set_packet_coalesce (tap_if_t *tif);
clib_error_t *tap_set_packet_buffering (tap_if_t *tif, u16 size);
