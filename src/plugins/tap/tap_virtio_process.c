...
static uword
tap_virtio_send_interrupt_process (vlib_main_t *vm, vlib_node_runtime_t *rt,
				   vlib_frame_t *f)
{
  tap_virtio_if_t *vif;
...
VLIB_REGISTER_NODE (tap_virtio_send_interrupt_node) = {
  .function = tap_virtio_send_interrupt_process,
  .type = VLIB_NODE_TYPE_PROCESS,
  .name = "tap-virtio-send-interrupt-process",
};
