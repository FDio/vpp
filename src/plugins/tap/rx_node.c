...
tap_virtio_needs_csum (vlib_buffer_t *b0, vnet_virtio_net_hdr_v1_t *hdr,
		       u8 *l4_proto, u8 *l4_hdr_sz, tap_virtio_if_type_t type)
{
...
tap_virtio_n_left_to_process (vnet_virtio_vring_t *vring)
{
  return vring->used->idx - vring->last_used_idx;
}

static_always_inline u16
tap_virtio_get_slot_id (vnet_virtio_vring_t *vring, u16 last, u16 mask)
{
  return vring->used->ring[last & mask].id;
}

static_always_inline u16
tap_virtio_get_len (vnet_virtio_vring_t *vring, const int hdr_sz, u16 last,
		    u16 mask)
{
...
static_always_inline void
tap_virtio_device_input_ethernet (vlib_main_t *vm, vlib_node_runtime_t *node,
...
tap_virtio_device_input_gso_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
				    vlib_frame_t *frame, tap_virtio_if_t *vif,
				    vnet_virtio_vring_t *vring,
				    tap_virtio_if_type_t type, int gso_enabled,
				    int checksum_offload_enabled)
{
  ...
  u32 next_index;
  const int hdr_sz = vif->tap_virtio_net_hdr_sz;
  u32 *to_next = 0;
  u32 n_rx_packets = 0;
  u32 n_rx_bytes = 0;
  u16 mask = vring->queue_size - 1;
  u16 last = vring->last_used_idx;
  u16 n_left = tap_virtio_n_left_to_process (vring);
  vlib_buffer_t bt = {};
  ...
  while (n_left)
      ...
      while (n_left && n_left_to_next)
	{
	  ...
	  u16 slot = tap_virtio_get_slot_id (vring, last, mask);
	  u16 len = tap_virtio_get_len (vring, hdr_sz, last, mask);
	  ...
	  if (checksum_offload_enabled)
	    tap_virtio_needs_csum (b0, hdr, &l4_proto, &l4_hdr_sz, type);
	  ...
	  if (PREDICT_FALSE (num_buffers > 1))
		...
		{
		  last++;
		  u16 cslot = tap_virtio_get_slot_id (vring, last, mask);
		  /* hdr size is 0 after 1st packet in chain buffers */
		  u16 clen = tap_virtio_get_len (vring, 0, last, mask);
      ...
      tap_virtio_device_input_ethernet (vm, node, next_index, vif->sw_if_index,
					vif->hw_if_index);
  ...
  vlib_increment_combined_counter (
    ...
    thread_index, vif->sw_if_index, n_rx_packets, n_rx_bytes);

  return n_rx_packets;
}

static_always_inline uword
tap_virtio_device_input_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
				vlib_frame_t *frame, tap_virtio_if_t *vif,
				u16 qid, tap_virtio_if_type_t type)
{
  ...
  if (vif->gso_enabled)
    rv = tap_virtio_device_input_gso_inline (vm, node, frame, vif, vring, type,
					     1, 1);
  ...
  tap_virtio_refill_vring_split (vm, vif, type, vring, hdr_sz,
				 node->node_index);
  return rv;
}

VLIB_NODE_FN (tap_virtio_input_node)
  ...
  vec_foreach (p, pv)
      ...
      if (vif->flags & VIRTIO_IF_FLAG_ADMIN_UP)
	{
	  if (vif->type == VIRTIO_IF_TYPE_TAP)
	    n_rx += tap_virtio_device_input_inline (
...
VLIB_REGISTER_NODE (tap_virtio_input_node) = {
  .name = "tap-virtio-input",
  .sibling_of = "device-input",
  .format_trace = format_tap_virtio_input_trace,
