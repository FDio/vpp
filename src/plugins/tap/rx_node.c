...
tap_refill_vring (vlib_main_t *vm, tap_if_t *tif, vnet_virtio_vring_t *vring,
		  u32 node_index, int is_tun)
{
...
tap_rx_dequeue (vlib_main_t *vm, vlib_node_runtime_t *node, tap_if_t *tif,
		vnet_virtio_vring_t *vring, u32 *buffers, u16 *nexts,
		u32 *n_rx_bytes, int is_tun)
{
  const int hdr_sz = VIRTIO_NET_HDR_SZ;
  u32 n_rx_packets = 0;
  u16 n_left, mask, last = vring->last_used_idx;
  vlib_buffer_template_t bt;
  u8 off = is_tun ? TUN_DATA_OFFSET : 0;
  vnet_virtio_vring_used_elem_t *ring = vring->used->ring;
  u32 *ring_buffers = vring->buffers;
  ...
  while (n_left)
    {
      ...
      vnet_virtio_net_hdr_v1_t *hdr = vlib_buffer_get_current (b0);
      u16 num_buffers = hdr->num_buffers;
			     ...
			     tap_if_t *tif, u16 qid)
{
  ...
  u32 buffer_indices[VLIB_FRAME_SIZE], *to_next;
  ...
  if (is_tun)
    {
      to_next = buffer_indices;
      n_rx = tap_rx_dequeue (vm, node, tif, vring, to_next, next_indices,
			     &n_rx_bytes, 1);
  ...
  if (n_trace > 0)
	...
	{
	  vlib_buffer_t *b = vlib_get_buffer (vm, to_next[i]);
	  if (vlib_trace_buffer (vm, node, next_indices[i], b,
				 /* follow_chain */ 1))
	    {
	      tap_rx_trace_t *tr;
	      tr = vlib_add_trace (vm, node, b, sizeof (*tr));
	      tr->next_index = next_indices[i];
  ...
  if (is_tun)
    {
      vlib_buffer_enqueue_to_next (vm, node, to_next, next_indices, n_rx);
      tap_refill_vring (vm, tif, vring, node->node_index, 1);
    }
  else
    {
      if (PREDICT_TRUE (next_index == VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT))
	{
      ...
      tap_refill_vring (vm, tif, vring, node->node_index, 0);
    }
  return n_rx;
}
