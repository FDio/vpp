...
tap_refill_vring (vlib_main_t *vm, tap_if_t *tif, tap_rxq_t *rxq,
  ...
  if (is_tun)
    {
      ...
      n_rx = tap_rx_dequeue (vm, node, tif, rxq, to_next, next_indices,
			     &n_rx_bytes, 1);
  ...
  if (is_tun)
    {
      ...
      tap_refill_vring (vm, tif, rxq, node->node_index, 1);
    }
  else
    {
      if (PREDICT_TRUE (next_index == VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT))
	{
      ...
      tap_refill_vring (vm, tif, rxq, node->node_index, 0);
    }
  return n_rx;
}
