...
tap_refill_vring (vlib_main_t *vm, tap_rxq_t *rxq, u32 node_index)
{
...
{
  u8 l4_proto = 0, l4_hdr_sz = 0, type;
			     ...
			     tap_if_t *tif, u16 qid)
{
  ...
  if (is_tun)
    vlib_buffer_enqueue_to_next (vm, node, buffer_indices, next_indices, n_rx);
  else
    {
      if (n_rx)
	{
  ...
  tap_refill_vring (vm, rxq, node->node_index);
  return n_rx;
}
