...
tap_refill_vring_split (vlib_main_t *vm, tap_if_t *tif,
			     ...
			     int checksum_offload_enabled)
{
  ...
  while (n_left)
      ...
      while (n_left && n_left_to_next)
	{
	  ...
	  if (gso_enabled)
	    fill_gso_buffer_flags (b0, hdr, l4_hdr_sz);
...
tap_device_input_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			 tap_if_t *tif, u16 qid, int is_tun)
{
  vnet_virtio_vring_t *vring = vec_elt_at_index (tif->rxq_vrings, qid);
  uword rv;

  if (tif->gso_enabled)
    rv = tap_device_input_gso_inline (vm, node, tif, vring, is_tun, 1, 1);
  else if (tif->csum_offload_enabled)
    rv = tap_device_input_gso_inline (vm, node, tif, vring, is_tun, 0, 1);
  else
    rv = tap_device_input_gso_inline (vm, node, tif, vring, is_tun, 0, 0);

  tap_refill_vring_split (vm, tif, vring, VIRTIO_NET_HDR_SZ, node->node_index);
  return rv;
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  vec_foreach (p, pv)
	...
	{
	  if (tif->is_tun)
	    n_rx += tap_device_input_inline (vm, node, tif, p->queue_id,
					     1 /* is_tun */);
	  else
	    n_rx += tap_device_input_inline (vm, node, tif, p->queue_id,
					     0 /* is_tun */);
