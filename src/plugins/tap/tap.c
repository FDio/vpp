...
tap_vring_fill (vlib_main_t *vm, tap_if_t *tif, tap_rxq_t *rxq)
{
  ...
  vnet_virtio_vring_desc_t dt = {
    .len = vlib_buffer_get_default_data_size (vm) + hdr_sz - off,
  ...
  for (u16 i = 0; i < n_alloc; i++)
    {
      ...
      dt.addr = pointer_to_uword (b->data - hdr_sz + off);
      rxq->desc[i] = dt;
      rxq->avail->ring[i] = i;
    }

  rxq->avail->idx = n_alloc;
  rxq->desc_next = 0;
  rxq->desc_in_use = n_alloc;
  if (rxq->queue_size != n_alloc)
    log_warn (tif,
	      "failed to initially fill vring %u with buffers (requested "
	      "%u, provided %u)",
