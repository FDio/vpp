...
tap_virtio_pre_input_inline (vlib_main_t *vm, vnet_virtio_vring_t *txq_vring,
			     ...
			     u8 packet_buffering)
{
  if (txq->shared_queue)
	...
	{
	  if (tap_virtio_txq_is_scheduled (txq_vring))
	    goto unlock;
	  if (packet_coalesce)
	  ...
	  tap_virtio_txq_set_scheduled (txq_vring);
	unlock:
...
static uword
tap_virtio_pre_input (vlib_main_t *vm, vlib_node_runtime_t *node,
		      vlib_frame_t *frame)
{
  ...
  pool_foreach (vif, vim->interfaces)
      ...
      if (vif->packet_coalesce || vif->packet_buffering)
	{
	  ...
	  vec_foreach (txq_vring, vif->txq_vrings)
		...
		tap_virtio_pre_input_inline (vm, txq_vring, txq,
					     vif->packet_coalesce,
...
void
tap_virtio_pre_input_node_enable (vlib_main_t *vm, tap_virtio_if_t *vif)
{
  tap_virtio_main_t *vim = &tap_virtio_main;
  if (vif->packet_coalesce || vif->packet_buffering)
    {
...
void
tap_virtio_pre_input_node_disable (vlib_main_t *vm, tap_virtio_if_t *vif)
{
  tap_virtio_main_t *vim = &tap_virtio_main;
  if (vif->packet_coalesce || vif->packet_buffering)
    {
