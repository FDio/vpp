...
static u8 *
format_ip6_adl_allowlist_trace (u8 *s, va_list *args)
{
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  lb_index0 =
	    ip6_fib_table_fwding_lookup (c0->fib_index, &ip0->src_address);
	  ...
	  c1 = vnet_get_config_data (
	    &ccm1->config_main, &adl_buffer (b1)->adl.current_config_index,
	    &next1, sizeof (c1[0]));

	  lb_index1 =
	    ip6_fib_table_fwding_lookup (c1->fib_index, &ip1->src_address);
	  ...
	  vlib_increment_combined_counter (
	    ...
	    vlib_buffer_length_in_chain (vm, b0) + sizeof (ethernet_header_t));

	  vlib_increment_combined_counter (
	    vcm, thread_index, lb_index1, 1,
	    vlib_buffer_length_in_chain (vm, b1) + sizeof (ethernet_header_t));
  ...
  vlib_node_increment_counter (
    vm, node->node_index, IP6_ADL_ALLOWLIST_ERROR_ALLOWED, allowed_packets);
