...
static u8 *
format_adl_input_trace (u8 *s, va_list *args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  adl_input_trace_t *t = va_arg (*args, adl_input_trace_t *);

  s = format (s, "ADL_INPUT: sw_if_index %d, next index %d", t->sw_if_index,
	      t->next_index);
  return s;
}
...
#define _(n, f)                                                               \
                                                                              \
  static uword f##_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,       \
			    vlib_frame_t *frame)                              \
  {                                                                           \
