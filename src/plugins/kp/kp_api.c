...
(vl_api_kp_add_del_vip_t * mp)
{
  ...
  if (mp->is_del) {
    u32 vip_index;
    if (!(rv = kp_vip_find_index(&prefix, mp->prefix_length, &vip_index)))
    ...
    rv = kp_vip_add(&prefix, mp->prefix_length, type,
...
static void
vl_api_kp_add_del_as_t_handler
(vl_api_kp_add_del_as_t * mp)
{
  kp_main_t *kpm = &kp_main;
  vl_api_kp_conf_reply_t * rmp;
  int rv = 0;
  u32 vip_index;
  if ((rv = kp_vip_find_index((ip46_address_t *)mp->vip_ip_prefix,
                              mp->vip_prefix_length, &vip_index)))
    goto done;

  if (mp->is_del)
    rv = kp_vip_del_ass(vip_index, (ip46_address_t *)mp->as_address, 1);
  else
    rv = kp_vip_add_ass(vip_index, (ip46_address_t *)mp->as_address, 1);

done:
 REPLY_MACRO (VL_API_KP_CONF_REPLY);
}

static void *vl_api_kp_add_del_as_t_print
(vl_api_kp_add_del_as_t *mp, void * handle)
{
  u8 * s;
  s = format (0, "SCRIPT: kp_add_del_as ");
  s = format (s, "%U ", format_ip46_prefix,
