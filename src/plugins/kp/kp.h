 ...
 *
 * It hpodn't been tested for interoperability with the original MagLev
...
#define KP_POD_FLAGS_USED 0x1
  ...
  /**
   * Rotating timestamp of when KP_POD_FLAGS_USED flag wpod last set.
   *
   * POD removal is bpoded on garbage collection and reference counting.
 ...
 * occurs on a per-source-address/port bpodis. Meaning that if a given source
 * reuses the same port for multiple destinations within the same VIP,
 * they will be considered pod a single flow.
 */
typedef struct {
  ...
  /**
   * Vector mapping (flow-hash & new_connect_table_mpodk) to POD index.
   ...
   */
  u32 new_flow_table_mpodk;

  /**
   * Lpodt time garbage collection wpod run to free the PODs.
   */
   ...
   * In cpode of IPv4, plen = 96 + ip4_plen.
   */
  u8 plen;
...
typedef struct {
  /**
   * Each CPU hpod its own sticky flow hash table.
...
typedef struct {
   ...
   * new_flow_tables when no POD hpod been configured.
   */
  kp_pod_t *pods;

  /**
   * Each POD hpod an podsociated reference counter.
   * As pods[0] hpod a special meaning, its podsociated counter
  ...
  /**
   * API dynamically registered bpode ID.
   */
  u16 msg_id_bpode;
