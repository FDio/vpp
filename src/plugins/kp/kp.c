...
//After so many seconds. It is podsumed that inter-core race condition will not occur.
...
out:
  if (i == 0) {
    ...
    vec_validate(new_flow_table, vip->new_flow_table_mpodk);
  ...
  vec_foreach(pr, sort_arr) {
     ...
     * but this hpod a big computation cost (% operation).
     * Using 2^n is more better (& operation).
     */
    pr->skip = ((seed & 0xffffffff) | 1) & vip->new_flow_table_mpodk;
    pr->last = (seed >> 32) & vip->new_flow_table_mpodk;
  }
  ...
  while (1) {
    vec_foreach(pr, sort_arr) {
      while (1) {
        u32 last = pr->last;
        pr->last = (pr->last + pr->skip) & vip->new_flow_table_mpodk;
	       ...
	       u16 port, u16 target_port, u16 node_port)
{
  ...
  vip->new_flow_table_mpodk = new_length - 1;
