...
static clib_error_t *
cliutils_show_ifpps_command_fn (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
  ...
  while (refresh_count--)
	...
	format_base10,
	(u32) ((vrx[spin].bytes - vrx[spin ^ 1].bytes) * 8 / tsd),
	format_base10,
	(u32) ((vtx[spin].packets - vtx[spin ^ 1].packets) / tsd),
	format_base10,
	(u32) ((vtx[spin].bytes - vtx[spin ^ 1].bytes) * 8 / tsd));
    }
...
VLIB_CLI_COMMAND (show_ifpps_command, static) = {
  .path = "ifpps",
  .short_help = "ifpps <interface> [interval <intv>] [count <count>]",
  .function = cliutils_show_ifpps_command_fn,
  .is_mp_safe = 1,
};

VLIB_PLUGIN_REGISTER () = {
  .version = VPP_BUILD_VER,
  .description = "CLI monitoring utilities",
};
