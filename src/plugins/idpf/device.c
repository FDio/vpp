...
#define IDPF_MBOX_LEN 64
#define IDPF_MBOX_BUF_SZ 4096
#define IDPF_RXQ_SZ 512
#define IDPF_TXQ_SZ 512
#define IDPF_ITR_INT 250
...
typedef enum
{
  IDPF_IRQ_STATE_DISABLED,
  IDPF_IRQ_STATE_ENABLED,
  IDPF_IRQ_STATE_WB_ON_ITR,
} idpf_irq_state_t;

static int
idpf_vc_clean (idpf_device_t *id)
{
...
idpf_read_msg_from_cp (idpf_device_t *id, uint16_t buf_len, uint8_t *buf)
{
		  ...
		  void *in, int in_len)
{
  ...
  idpf_alloc_dma_mem (vm, id, dma_mem, dma_mem->size);
  if (dma_mem->va == NULL)
...
clib_error_t *
idpf_read_one_msg (vlib_main_t *vm, idpf_device_t *id, uint32_t ops,
...
idpf_execute_vc_cmd (vlib_main_t *vm, idpf_device_t *id, idpf_cmd_info_t *args)
{
  ...
  switch (args->ops)
    ...
    case VIRTCHNL2_OP_GET_STATS:
      ...
      error = idpf_read_one_msg (vm, id, args->ops, args->out_size,
				 args->out_buffer);
      if (!error)
			      ...
			      idpf_vport_t *vport, bool map)
{
  ...
  clib_memset(map_info, 0, len);
	...
	if (error != 0)
		return clib_error_return (0, "Failed to execute command of VIRTCHNL2_OP_%s_QUEUE_VECTOR",
			    map ? "MAP" : "UNMAP");
			    ...
			    idpf_vport_t *vport)
{
  ...
  clib_error_t * error = 0;
	...
	qv_map = clib_mem_alloc_aligned(id->n_rx_queues *
			sizeof(virtchnl2_queue_vector_t), CLIB_CACHE_LINE_BYTES);
  clib_memset(qv_map, 0, id->n_rx_queues *
	...
	idpf_log_debug(id, "Value of dynctl_reg_start is 0x%x",
		    dynctl_val);
...
clib_error_t *
idpf_rx_split_bufq_setup(vlib_main_t * vm, idpf_device_t * id, idpf_vport_t * vport, idpf_rxq_t *bufq,
			 u16 qid, u16 rxq_size)
{
  ...
  u32 n_alloc, i;
	...
	bufq->size = rxq_size;
	bufq->next = 0;
	bufq->descs = vlib_physmem_alloc_aligned_on_numa (vm, bufq->size *
							sizeof (virtchnl2_rx_desc_t),
...
clib_error_t *
idpf_split_rxq_init (vlib_main_t * vm, idpf_device_t * id, idpf_vport_t * vport, u16 qid, u16 rxq_size)
{
  clib_error_t *err;
  idpf_rxq_t *rxq;
  u32 n_alloc, i;

  vec_validate_aligned (id->rxqs, qid, CLIB_CACHE_LINE_BYTES);
  rxq = vec_elt_at_index (id->rxqs, qid);
  rxq->size = rxq_size;
  rxq->next = 0;
...
clib_error_t *
idpf_tx_split_complq_setup(vlib_main_t * vm, idpf_device_t * id, idpf_vport_t * vport, idpf_txq_t *complq,
			 u16 qid, u16 txq_size)
{
  ...
  complq->descs = vlib_physmem_alloc_aligned_on_numa (vm, complq->size *
						   sizeof (idpf_tx_desc_t),
						   2 * CLIB_CACHE_LINE_BYTES,
						   id->numa_node);
  if (complq->descs == 0)
    return vlib_physmem_last_error (vm);

  if ((err = vlib_pci_map_dma (vm, id->pci_dev_handle, (void *) complq->descs)))
    return err;

  vec_validate_aligned (complq->bufs, complq->size, CLIB_CACHE_LINE_BYTES);
  complq->qtx_tail = id->bar0 + (vport->chunks_info.tx_compl_qtail_start +
		     ...
		     u16 qid, u16 txq_size)
{
  ...
  vec_validate_aligned (id->txqs, qid, CLIB_CACHE_LINE_BYTES);
  txq = vec_elt_at_index (id->txqs, qid);
  txq->size = txq_size;
  txq->next = 0;
...
clib_error_t *
idpf_vc_config_txq (vlib_main_t *vm, idpf_device_t *id, idpf_vport_t *vport, u16 qid)
{
  idpf_txq_t *txq;
		    ...
		    uint16_t num_vectors)
{
  ...
  clib_memset(alloc_vec, 0, len);
	...
	error = idpf_execute_vc_cmd(vm, id, &args);
	if (error != 0)
		return clib_error_return (0, "Failed to execute command VIRTCHNL2_OP_ALLOC_VECTORS");
  ...
  clib_memcpy_fast (vport->recv_vectors, args.out_buffer, len);
		 ...
		 virtchnl2_version_info_t *ver)
{
  ...
  args.in_args = (uint8_t *) &myver;
  args.in_args_size = sizeof (myver);
  args.out_buffer = id->mbx_resp;
  args.out_size = IDPF_DFLT_MBX_BUF_SIZE;
  error = idpf_execute_vc_cmd (vm, id, &args);
  if (error != 0)
    return clib_error_return (0, "Failed to execute command VIRTCHNL_OP_VERSION");

  ver = (virtchnl2_version_info_t *) args.out_buffer;
  return error;
}
...
{
  virtchnl2_get_capabilities_t caps_msg;
  idpf_cmd_info_t args;
  clib_error_t * error = 0;

  clib_memset(&caps_msg, 0, sizeof(virtchnl2_get_capabilities_t));
	...
	error = idpf_execute_vc_cmd (vm, id, &args);
	if (error != 0)
    return clib_error_return (0, "Failed to execute command VIRTCHNL2_OP_GET_CAPS");

	caps = (virtchnl2_get_capabilities_t *) args.out_buffer;
  return error;
}
...
idpf_vc_query_ptype_info (vlib_main_t *vm, idpf_device_t *id)
{
  ...
  error = idpf_send_vc_msg (vm, id, VIRTCHNL2_OP_GET_PTYPE_INFO, &ptype_info,
			  sizeof (ptype_info));
...
idpf_get_pkt_type(vlib_main_t * vm, idpf_device_t * id)
{
	...
	while (ptype_recvd < IDPF_MAX_PKT_TYPE) {
	    error =
	      idpf_read_one_msg (vm, id, VIRTCHNL2_OP_GET_PTYPE_INFO,
				 IDPF_DFLT_MBX_BUF_SIZE, (u8 *) ptype_info);
	    if (error != 0)
	      {
		idpf_log_err (id, "Fail to get packet type information");
		goto free_ptype_info;
	      }

		ptype_recvd += ptype_info->num_ptypes;
		ptype_offset = sizeof(virtchnl2_get_ptype_info_t) -
		...
		for (i = 0; i < ptype_info->num_ptypes; i++)
		  {
		    ...
		    ptype =
		      (virtchnl2_ptype_t *) ((u8 *) ptype_info + ptype_offset);
			...
			switch (ptype->proto_id[j])
			  ...
			  case VIRTCHNL2_PROTO_HDR_IPV4:
				...
				is_ip = true;
			      ...
			      {
				proto_hdr |=
				  IDPF_PTYPE_INNER_L3_IPV4_EXT_UNKNOWN |
			  ...
			  case VIRTCHNL2_PROTO_HDR_IPV6:
				...
				is_ip = true;
			      ...
			      {
				proto_hdr |=
				  IDPF_PTYPE_INNER_L3_IPV6_EXT_UNKNOWN |
				  IDPF_PTYPE_TUNNEL_IP;
				is_inner = true;
			      }
			    break;
...
static u16
idpf_vport_idx_alloc (idpf_device_t *id)
{
  u16 vport_idx;
  u16 i;

  for (i = 0; i < id->max_vports; i++)
    {
      if (id->vports[i] == NULL)
	break;
    }
...
{
  virtchnl2_create_vport_t vport_msg;
  idpf_cmd_info_t args;
  clib_error_t * error;

  clib_memset (&vport_msg, 0, sizeof (virtchnl2_create_vport_t));
...
clib_error_t *
idpf_ena_dis_vport (vlib_main_t *vm, idpf_device_t *id, idpf_vport_t *vport, bool enable)
{
	virtchnl2_vport_t vc_vport;
	idpf_cmd_info_t args;
  clib_error_t *error;
	...
	error = idpf_execute_vc_cmd(vm, id, &args);
	if (error != 0) {
		return clib_error_return (0, "Failed to execute command of VIRTCHNL2_OP_%s_VPORT",
...
idpf_dealloc_vectors(vlib_main_t *vm, idpf_device_t *id, idpf_vport_t *vport)
{
  ...
  clib_error_t *error;
	...
	error = idpf_execute_vc_cmd(vm, id, &args);
	if (error != 0)
		return clib_error_return (0, "Failed to execute command VIRTCHNL2_OP_DEALLOC_VECTORS");

	return error;
}

clib_error_t *
idpf_dev_vport_init (vlib_main_t *vm, idpf_device_t *id, void *init_params)
{
  idpf_vport_t *vport;
  idpf_vport_param_t *param = init_params;
  virtchnl2_create_vport_t vport_req_info;
  clib_error_t * error = 0;
  ...
  clib_memset (&vport_req_info, 0, sizeof (virtchnl2_create_vport_t));
  ...
  id->cur_vports |= 1ULL << param->devarg_id;
  id->cur_vport_nb++;

  /* Fixme: id->hwaddr for different vports */
  clib_memcpy_fast (id->hwaddr, vport->default_mac_addr, 6);
...
idpf_device_init (vlib_main_t *vm, idpf_main_t *im, idpf_device_t *id,
		  idpf_create_if_args_t *args)
{
  idpf_vport_param_t vport_param;
  idpf_vport_t *vport;
  ...
  rxq_num = args->rxq_single ? args->rxq_single : 1;
  txq_num = args->txq_single ? args->txq_single : vlib_get_n_threads ();

  /*
   * Check API version
   */
  error = idpf_op_version (vm, id, &ver);
  if (error)
    return error;

  if (ver.major != VIRTCHNL2_VERSION_MAJOR_2 ||
      ver.minor != VIRTCHNL2_VERSION_MINOR_0)
    return clib_error_return (0, "incompatible virtchnl version "
  ...
  if (error) {
    idpf_log_err (id, "failed to get packet type");
    return error;
  }
  ...
  error = idpf_op_get_caps(vm, id, &caps);
	if (error) {
    idpf_log_err (id, "failed to get capability");
    return error;
  }

	/* Fixme: eliminate unnecessary items*/
	...
	id->mailbox_dyn_ctl = caps.mailbox_dyn_ctl;
	id->mailbox_vector_id = caps.mailbox_vector_id;
	id->num_allocated_vectors = caps.num_allocated_vectors;
	...
	id->max_tx_hdr_size = caps.max_tx_hdr_size;
	id->max_sg_bufs_per_tx_pkt = caps.max_sg_bufs_per_tx_pkt;
	id->itr_idx_map = caps.itr_idx_map;

	id->vports =
	  clib_mem_alloc (id->max_vports * sizeof (*id->vports));
	id->max_rxq_per_msg =
   ...
   */
	if (args->req_vport_nb == 0)
	  {
	    vport_param.id = id;
	    vport_param.devarg_id = 0;
	    vport_param.idx = idpf_vport_idx_alloc (id);
	    if ((error = idpf_dev_vport_init (vm, id, &vport_param)))
        return error;
	    vport = id->vports[vport_param.idx];
	    if ((error = idpf_ena_dis_vport (vm, id, vport, true)))
	      return error;
	    if ((error = idpf_queue_init (vm, id, vport, args)))
	      return error;
	  }
	else
	  {
	    for (i = 0; i < args->req_vport_nb; i++)
	      {
		vport_param.id = id;
		vport_param.devarg_id = args->req_vports[i];
		vport_param.idx = idpf_vport_idx_alloc (id);
		if ((error = idpf_dev_vport_init (vm, id, &vport_param)))
		  return error;
		vport = id->vports[vport_param.idx];
...
idpf_delete_if (vlib_main_t * vm, idpf_device_t * id, int with_barrier)
{
  ...
  /* *INDENT-OFF* */
  vec_foreach_index (i, id->rxqs)
    {
      idpf_rxq_t *rxq = vec_elt_at_index (id->rxqs, i);
  ...
  /* *INDENT-OFF* */
  vec_foreach_index (i, id->txqs)
    {
      idpf_txq_t *txq = vec_elt_at_index (id->txqs, i);
...
idpf_create_if (vlib_main_t * vm, idpf_create_if_args_t * args)
{
  ...
  idp[0] = id = clib_mem_alloc_aligned (sizeof (idpf_device_t),
  ...
  if ((error = vlib_pci_device_open (vm, &args->addr, idpf_pci_device_ids,
				     &h)))
    {
  ...
  vnet_hw_if_set_input_node (vnm, id->hw_if_index, idpf_input_node.index);
  ...
  for (i = 0; i < id->n_rx_queues; i++)
    {
      ...
      id->rxqs[i].queue_index = qi;
    }

  for (i = 0; i < id->n_tx_queues; i++)
    {
      u32 qi = vnet_hw_if_register_tx_queue (vnm, id->hw_if_index, i);
      id->txqs[i].queue_index = qi;
    }

  for (i = 0; i < vlib_get_n_threads (); i++)
    {
      u32 qi = id->txqs[i % id->n_tx_queues].queue_index;
...
void *
idpf_alloc_dma_mem(vlib_main_t * vm, idpf_device_t * id,
		   idpf_dma_mem_t *mem, u64 size)
{
  ...
  vlib_pci_dev_handle_t h = id->pci_dev_handle;
	...
	/* Fixme */
	mz = vlib_physmem_alloc_aligned_on_numa (vm, sizeof (u8) * IDPF_DFLT_MBX_BUF_SIZE, CLIB_CACHE_LINE_BYTES, id->numa_node);
	if (!mz)
		return NULL;

  error = vlib_pci_map_dma (vm, h, mz);
	mem->size = size;
  if (id->flags & IDPF_DEVICE_F_VA_DMA)
	  mem->va = mz;
	mem->pa = vlib_physmem_get_pa (vm, mz);
...
static void
idpf_set_interface_next_node (vnet_main_t * vnm, u32 hw_if_index,
			     u32 node_index)
{
  ...
  if (node_index == ~0)
    {
      id->per_interface_next_index = node_index;
      return;
    }

  id->per_interface_next_index =
    vlib_node_add_next (vlib_get_main (), idpf_input_node.index, node_index);
}

static char *idpf_tx_func_error_strings[] = {
#define _(n,s) s,
...
static void
idpf_clear_hw_interface_counters (u32 instance)
{
  /* Nothing for now */
}
...
VNET_DEVICE_CLASS (idpf_device_class, ) = {
  ...
  .clear_counters = idpf_clear_hw_interface_counters,
  .format_device = format_idpf_device,
  .format_device_name = format_idpf_device_name,
  .admin_up_down_function = idpf_interface_admin_up_down,
  /* Fixme: rx mode change */
  .rx_redirect_to_node = idpf_set_interface_next_node,
  .tx_function_n_errors = IDPF_TX_N_ERROR,
  .tx_function_error_strings = idpf_tx_func_error_strings,
};
/* *INDENT-ON* */
