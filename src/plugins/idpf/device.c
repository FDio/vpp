...
idpf_op_version (vlib_main_t *vm, idpf_device_t *id)
{
  ...
  clib_memcpy_fast (&ver, args.out_buffer, sizeof (ver));
		  ...
		  virtchnl2_get_capabilities_t *caps)
{
  ...
  clib_memcpy_fast (caps, args.out_buffer, sizeof (*caps));
  return error;
}
		     ...
		     idpf_vport_param_t *param)
{
  ...
  vport_req_info = clib_mem_alloc (IDPF_DFLT_MBX_BUF_SIZE);
  clib_memset (vport_req_info, 0, IDPF_DFLT_MBX_BUF_SIZE);
...
//     vlib_node_add_next (vlib_get_main (), idpf_input_node.index,
//     node_index);
// }
