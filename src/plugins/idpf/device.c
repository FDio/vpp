		  ...
		  virtchnl2_get_capabilities_t *caps)
{
  ...
  caps_msg.rss_caps =
    ...
    VIRTCHNL2_CAP_RSS_IPV4_AH | VIRTCHNL2_CAP_RSS_IPV4_ESP |
    VIRTCHNL2_CAP_RSS_IPV4_AH_ESP | VIRTCHNL2_CAP_RSS_IPV6_AH |
    VIRTCHNL2_CAP_RSS_IPV6_ESP | VIRTCHNL2_CAP_RSS_IPV6_AH_ESP;
...
idpf_mbx_init (vlib_main_t *vm, idpf_device_t *id)
{
  ...
  LIST_FOR_EACH_ENTRY_SAFE (ctlq, NULL, &id->cq_list_head, idpf_ctlq_info_t,
			    cq_list)
  {
		    ...
		    u64 size)
{
  ...
  /* Fixme */
  mz = vlib_physmem_alloc_aligned_on_numa (
    vm, sizeof (u8) * IDPF_DFLT_MBX_BUF_SIZE, CLIB_CACHE_LINE_BYTES,
    id->numa_node);
  if (!mz)
    return NULL;
