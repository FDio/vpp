...
#define _(a, b, c) if (id->flags & (1 << a)) \
t = format (t, "%s%s", t ? " ":"", c);
...
format_idpf_device (u8 * s, va_list * args)
{
  ...
  s = format (s, "\n%Uchecksum capability flags: %U", format_white_space, indent,
	      format_idpf_checksum_cap_flags, id->csum_caps);

  s = format (s, "\n%Unum-queue-pairs %d max-vectors %u max-mtu %u "
	      "rss-key-size %u rss-lut-size %u", format_white_space, indent,
	      id->num_queue_pairs, id->max_vectors, id->max_mtu,
	      id->rss_key_size, id->rss_lut_size);
  if (id->error)
