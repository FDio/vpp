 ...
 */
static int
idpf_ctlq_alloc_desc_ring (vlib_main_t *vm, idpf_device_t *id, struct idpf_ctlq_info *cq)
{
 ...
 */
static int
idpf_ctlq_alloc_bufs (vlib_main_t *vm, idpf_device_t *id, struct idpf_ctlq_info *cq)
{
  int i = 0;
   ...
   */
  cq->bi.rx_buff = (idpf_dma_mem_t **) clib_mem_alloc (
    cq->ring_size * sizeof (idpf_dma_mem_t *));
  if (!cq->bi.rx_buff)
    return IDPF_ERR_NO_MEMORY;
 ...
 */
int
idpf_ctlq_alloc_ring_res (vlib_main_t *vm, idpf_device_t *id, struct idpf_ctlq_info *cq)
{
  int ret_code;
 ...
 */
static void idpf_ctlq_init_regs(vlib_main_t * vm, idpf_device_t * id, struct idpf_ctlq_info * cq,
				bool is_rxq)
{
	       ...
	       idpf_ctlq_create_info_t *qinfo, struct idpf_ctlq_info **cq_out)
{
	...
	*cq_out = vlib_physmem_alloc_aligned_on_numa (
	  vm, sizeof (struct idpf_ctlq_info), 2 * CLIB_CACHE_LINE_BYTES,
	  id->numa_node);
	if (!(*cq_out))
		return IDPF_ERR_NO_MEMORY;
	...
	if (is_rxq) {
		...
		(*cq_out)->bi.tx_msg = clib_mem_alloc_aligned (qinfo->len * sizeof(idpf_ctlq_msg_t *), CLIB_CACHE_LINE_BYTES);
		if (!(*cq_out)->bi.tx_msg) {
...
init_destroy_qs:
  LIST_FOR_EACH_ENTRY_SAFE (cq, NULL, &id->cq_list_head, idpf_ctlq_info,
			    cq_list)
  {
  ...
  if (num_desc_avail == 0 || num_desc_avail < num_q_msg)
	...
	for (i = 0; i < num_q_msg; i++) {
		...
		clib_memcpy (&desc->params, msg->ctx.indirect.context,
	      ...
	      {
		clib_memcpy (&desc->params, msg->ctx.direct,
		...
		idpf_ctlq_msg_t *q_msg)
{
	...
	for (i = 0; i < num_to_clean; i++) {
		...
		if (desc->datalen) {
			...
			clib_memcpy_fast(q_msg[i].ctx.direct,
				    desc->params.raw,
				    IDPF_DIRECT_CTX_SIZE);
		}
