...
#define PF_GLINT_ITR(_i, _INT)                                                \
  (PF_GLINT_BASE + (((_i) + 1) * 4) + ((_INT) *0x1000))
...
#define PF_GLTSYN_CMD_SYNC_EXEC_CMD_M                                         \
  MAKEMASK (0x3, PF_GLTSYN_CMD_SYNC_EXEC_CMD_S)
...
#define IDPF_AQ_ENQ_SUSPEND_TIME  50e-6
#define IDPF_AQ_ENQ_MAX_WAIT_TIME 250e-3
...
#define IDPF_RXD_STATUS(x)	  (1ULL << x)
#define IDPF_RXD_STATUS_DD	  IDPF_RXD_STATUS (0)
#define IDPF_RXD_STATUS_EOP	  IDPF_RXD_STATUS (1)
#define IDPF_RXD_STATUS_FLM	  IDPF_RXD_STATUS (11)
#define IDPF_RXD_ERROR_SHIFT	  19
#define IDPF_RXD_PTYPE_SHIFT	  30
#define IDPF_RXD_LEN_SHIFT	  38
#define IDPF_RX_MAX_DESC_IN_CHAIN 5

#define IDPF_RXD_ERROR_IPE (1ULL << (IDPF_RXD_ERROR_SHIFT + 3))
#define IDPF_RXD_ERROR_L4E (1ULL << (IDPF_RXD_ERROR_SHIFT + 4))

#define IDPF_TXD_CMD(x)		 (1 << (x + 4))
#define IDPF_TXD_CMD_EXT(x, val) ((u64) val << (x + 4))
#define IDPF_TXD_CMD_EOP	 IDPF_TXD_CMD (0)
#define IDPF_TXD_CMD_RS		 IDPF_TXD_CMD (1)
#define IDPF_TXD_CMD_RSV	 IDPF_TXD_CMD (2)

#define IDPF_TXD_CMD_IIPT_NONE	       IDPF_TXD_CMD_EXT (5, 0)
#define IDPF_TXD_CMD_IIPT_IPV6	       IDPF_TXD_CMD_EXT (5, 1)
#define IDPF_TXD_CMD_IIPT_IPV4_NO_CSUM IDPF_TXD_CMD_EXT (5, 2)
#define IDPF_TXD_CMD_IIPT_IPV4	       IDPF_TXD_CMD_EXT (5, 3)

#define IDPF_TXD_CMD_L4T_UNKNOWN IDPF_TXD_CMD_EXT (8, 0)
#define IDPF_TXD_CMD_L4T_TCP	 IDPF_TXD_CMD_EXT (8, 1)
#define IDPF_TXD_CMD_L4T_SCTP	 IDPF_TXD_CMD_EXT (8, 2)
#define IDPF_TXD_CMD_L4T_UDP	 IDPF_TXD_CMD_EXT (8, 3)

#define IDPF_TXD_OFFSET(x, factor, val)                                       \
  (((u64) val / (u64) factor) << (16 + x))
#define IDPF_TXD_OFFSET_MACLEN(val) IDPF_TXD_OFFSET (0, 2, val)
#define IDPF_TXD_OFFSET_IPLEN(val)  IDPF_TXD_OFFSET (7, 4, val)
#define IDPF_TXD_OFFSET_L4LEN(val)  IDPF_TXD_OFFSET (14, 4, val)

#define IDPF_TXD_DTYP_CTX	   0x1ULL
#define IDPF_TXD_CTX_CMD_TSO	   IDPF_TXD_CMD (0)
#define IDPF_TXD_CTX_SEG(val, x)   (((u64) val) << (30 + x))
#define IDPF_TXD_CTX_SEG_TLEN(val) IDPF_TXD_CTX_SEG (val, 0)
#define IDPF_TXD_CTX_SEG_MSS(val)  IDPF_TXD_CTX_SEG (val, 20)

#define IDPF_MEMZONE_NAMESIZE 32
...
STATIC_ASSERT_SIZEOF (idpf_rx_desc_t, 32);
...
typedef struct
{
  ...
  u16 cur_vports; /* bit mask of created vport */
  u16 cur_vport_nb;
  u16 used_vecs_num;
...
{
  idpf_device_t *id;
  u16 devarg_id; /* arg id from user */
  u16 idx;	 /* index in id->vports[]*/
} idpf_vport_param_t;

struct idpf_vport
{
  idpf_device_t *id;
  virtchnl2_create_vport_t *vport_info; /* virtchnl response info handling */
  u16 idx;
  u16 vport_id;
  u32 txq_model;
  u32 rxq_model;
...
extern vlib_node_registration_t idpf_input_node;
extern vlib_node_registration_t idpf_process_node;
extern vnet_device_class_t idpf_device_class;

clib_error_t *idpf_program_flow (u32 dev_instance, int is_add, u8 *rule,
				 u32 rule_len, u8 *program_status,
				 u32 status_len);

/* format.c */
format_function_t format_idpf_device;
format_function_t format_idpf_device_name;
format_function_t format_idpf_input_trace;
format_function_t format_idpf_vf_cap_flags;
format_function_t format_idpf_eth_stats;
...
static inline u32
idpf_get_u32 (void *start, int offset)
{
  return *(u32 *) (((u8 *) start) + offset);
}

static inline u64
idpf_get_u64 (void *start, int offset)
{
  return *(u64 *) (((u8 *) start) + offset);
}

static inline u32
idpf_get_u32_bits (void *start, int offset, int first, int last)
{
  u32 value = idpf_get_u32 (start, offset);
  if ((last == 0) && (first == 31))
    return value;
  value >>= last;
...
static inline u64
idpf_get_u64_bits (void *start, int offset, int first, int last)
{
  u64 value = idpf_get_u64 (start, offset);
  if ((last == 0) && (first == 63))
    return value;
  value >>= last;
  value &= (1 << (first - last + 1)) - 1;
...
static inline void
idpf_set_u32 (void *start, int offset, u32 value)
{
...
static inline u32
idpf_reg_read (idpf_device_t *ad, u32 addr)
{
...
static inline void
idpf_tail_write (volatile u32 *addr, u32 val)
{
#ifdef __MOVDIRI__
  _mm_sfence ();
...
idpf_rxd_is_not_eop (idpf_rx_desc_t *d)
{
  return (d->qword[1] & IDPF_RXD_STATUS_EOP) == 0;
}

static_always_inline int
idpf_rxd_is_not_dd (idpf_rx_desc_t *d)
{
  return (d->qword[1] & IDPF_RXD_STATUS_DD) == 0;
}
...
#define foreach_idpf_tx_func_error                                            \
  _ (SEGMENT_SIZE_EXCEEDED, "segment size exceeded")                          \
  _ (NO_FREE_SLOTS, "no free tx slots")

typedef enum
{
#define _(f, s) IDPF_TX_ERROR_##f,
  foreach_idpf_tx_func_error
#undef _
    IDPF_TX_N_ERROR,
} idpf_tx_func_error_t;
...
clib_error_t *idpf_op_version (vlib_main_t *vm, idpf_device_t *id,
			       virtchnl2_version_info_t *ver);
clib_error_t *idpf_op_get_caps (vlib_main_t *vm, idpf_device_t *id,
				virtchnl2_get_capabilities_t *caps);
clib_error_t *idpf_vc_create_vport (vlib_main_t *vm, idpf_device_t *id,
				    idpf_vport_t *vport,
				    virtchnl2_create_vport_t *vport_req_info);
clib_error_t *idpf_vc_destroy_vport (vlib_main_t *vm, idpf_device_t *id,
				     idpf_vport_t *vport);
clib_error_t *idpf_vc_config_rxq (vlib_main_t *vm, idpf_device_t *id,
				  idpf_vport_t *vport, u16 qid);
clib_error_t *idpf_vc_config_txq (vlib_main_t *vm, idpf_device_t *id,
				  idpf_vport_t *vport, u16 qid);
clib_error_t *idpf_op_enable_queues (vlib_main_t *vm, idpf_device_t *id,
				     idpf_vport_t *vport, u16 qid, bool rx,
				     bool on);
clib_error_t *idpf_ena_dis_vport (vlib_main_t *vm, idpf_device_t *id,
				  idpf_vport_t *vport, bool enable);
