...
static u8 *
format_sch_drr_enqueue_trace (u8 *s, va_list *args)
{
...
static u8 *
format_sch_drr_dequeue_trace (u8 *s, va_list *args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
...
enqueue_frame (vlib_main_t *vm, vlib_node_runtime_t *node,
	       sch_drr_buffer_t *drr_b, vlib_buffer_t **b, u32 *from,
	       int n_pkt)
{
...
select_next_frame (vlib_main_t *vm, vlib_node_runtime_t *node,
		   sch_drr_buffer_t *drr_b, u32 *i_arr,
		   vlib_buffer_t *bufs[VLIB_FRAME_SIZE], u32 n_pkt)
{
  int i = 0;
  static int j = -1;
  ...
  while (i < n_pkt && !clib_bitmap_is_zero (drr_b->class_bitmap))
      ...
      if (drr_b->queue_map[j].count)
	{
	  ...
	  vlib_buffer_t *temp_bufs =
	    vlib_get_buffer (vm, ring_buffer_front (&drr_b->queue_map[j]));
	  if (temp_bufs->current_length <= drr_b->deficit_counter[j])
		...
		{
		  trace_de_t *tr;
		  tr = vlib_add_trace (vm, node,
				       vlib_get_buffer (vm, i_arr[i - 1]),
				       sizeof (*tr));
		  tr->queue_i = j;
		  tr->pos = i - 1;
...
static uword
sch_drr_enqueue_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			 vlib_frame_t *frame)
{
...
static uword
sch_drr_dequeue_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			 vlib_frame_t *frame)
{
  ...
  int n_pkt =
    select_next_frame (vm, node, drr_b, i_arr, bufs, VLIB_FRAME_SIZE);
