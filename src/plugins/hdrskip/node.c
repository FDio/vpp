...
#define foreach_hdrskip_input_error _ (TOO_SHORT, "skip exceeds buffer length")
		      ...
		      vlib_frame_t *frame, int is_trace)
{
  ...
  while (n_left_from > 0)
	...
	(sw_if_index < vec_len (hsm->input_skip_by_sw_if_index)) ?
	  hsm->input_skip_by_sw_if_index[sw_if_index] :
	  0;
      u32 next0;
...
VLIB_NODE_FN (hdrskip_input_node)
		       ...
		       vlib_frame_t *frame, int is_trace)
{
  ...
  while (n_left_from > 0)
	...
	(sw_if_index < vec_len (hsm->output_restore_by_sw_if_index)) ?
	  hsm->output_restore_by_sw_if_index[sw_if_index] :
	  0;
      u32 next0;
...
VLIB_NODE_FN (hdrskip_output_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  if (PREDICT_FALSE (node->flags & VLIB_NODE_FLAG_TRACE))
