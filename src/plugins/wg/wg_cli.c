		      ...
		      unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  if (unformat (line_input, "%s", &secret_64))
    {
      if (!(base64_decode (secret_64, NOISE_KEY_LEN_BASE64, secret)))
	{
			 ...
			 unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output (vm, "Peers count: %d", pool_elts (wmp->peers));
  vlib_cli_output (vm, "Peers:");

  pool_foreach (peer, peer_pool, (
				   {
				   u8 key_64[NOISE_KEY_LEN_BASE64];
				   base64_encode (peer->remote.r_public,
						  NOISE_PUBLIC_KEY_LEN,
						  key_64);
				   vlib_cli_output (vm,
						    "---------------------------------");
				   vlib_cli_output (vm, "Public-key: %s",
						    key_64);
				   vlib_cli_output (vm, "Port: %u",
						    peer->port);
				   vlib_cli_output (vm, "Endpoint: %U",
						    format_ip4_address,
						    &peer->ip4_address);
				   vlib_cli_output (vm, "Allowed ip: %U",
						    format_ip4_address,
						    &peer->allowed_ip);
				   vlib_cli_output (vm, "Tun_sw_if_index: %d",
						    peer->tun_sw_if_index);
				   vlib_cli_output (vm, "Keepalive: %d",
