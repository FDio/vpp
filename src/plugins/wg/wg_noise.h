...
#define NOISE_HASH_LEN		BLAKE2S_HASHSIZE
...
bool noise_local_set_private (noise_local_t *, uint8_t[NOISE_PUBLIC_KEY_LEN]);
...
void noise_remote_init (noise_remote_t *, uint32_t,
			uint8_t[NOISE_PUBLIC_KEY_LEN], noise_local_t *);
...
bool noise_create_initiation (noise_remote_t *,
			      uint32_t * s_idx,
...
bool noise_consume_initiation (noise_local_t *,
			       noise_remote_t **,
			       uint32_t s_idx,
			       uint8_t ue[NOISE_PUBLIC_KEY_LEN],
			       uint8_t es[NOISE_PUBLIC_KEY_LEN +
...
bool noise_create_response (noise_remote_t *,
			    uint32_t * s_idx,
			    uint32_t * r_idx,
			    uint8_t ue[NOISE_PUBLIC_KEY_LEN],
			    uint8_t en[0 + NOISE_AUTHTAG_LEN]);

bool noise_consume_response (noise_remote_t *,
			     uint32_t s_idx,
			     uint32_t r_idx,
			     uint8_t ue[NOISE_PUBLIC_KEY_LEN],
			     uint8_t en[0 + NOISE_AUTHTAG_LEN]);

bool noise_remote_begin_session (noise_remote_t *);
void noise_remote_clear (noise_remote_t *);
void noise_remote_expire_current (noise_remote_t *);

bool noise_remote_ready (noise_remote_t *);

enum noise_state_crypt
noise_remote_encrypt (noise_remote_t *,
		      uint32_t * r_idx,
		      uint64_t * nonce,
		      uint8_t * src, size_t srclen, uint8_t * dst);
enum noise_state_crypt
noise_remote_decrypt (noise_remote_t *,
		      uint32_t r_idx,
		      uint64_t nonce,
		      uint8_t * src, size_t srclen, uint8_t * dst);
