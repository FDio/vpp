...
wg_handshake_process (vlib_main_t * vm, wg_main_t * wmp, vlib_buffer_t * b)
{
  ...
  if (header->type == MESSAGE_HANDSHAKE_COOKIE)
	...
	return WG_INPUT_ERROR_PEER;
      ...
      cookie_maker_consume_payload (vm, &peer->cookie_maker, packet->nonce,
  ...
  mac_state =
    cookie_checker_validate_macs (vm, &wmp->cookie_checker, macs,
  ...
  switch (header->type)
	...
	if (noise_consume_initiation
	    (&wmp->local, &rp, message->sender_index,
	...
	if (!noise_consume_response
	    (&peer->remote, resp->sender_index, resp->receiver_index,
	...
	case MESSAGE_DATA:
	  {
	    ...
	    enum noise_state_crypt state_cr =
	      noise_remote_decrypt (&peer->remote,
				    data->receiver_index,
				    data->counter,
				    data->encrypted_data,
				    encr_len,
