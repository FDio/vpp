...
static void
vl_api_wg_device_dump_t_handler (vl_api_wg_device_dump_t * mp)
{
  vl_api_wg_device_details_t *rmp;
  wg_main_t *wmp = &wg_main;
  int rv = 0;

  /* *INDENT-OFF* */
  REPLY_MACRO2(VL_API_WG_DEVICE_DETAILS,
  ({
       ...
       if (!wmp->is_inited) {
           ...
           base64_encode (wmp->local.l_private, NOISE_PUBLIC_KEY_LEN,key_64);
		       ...
		       u32 context)
{
  ...
  base64_encode (peer->remote.r_public, NOISE_PUBLIC_KEY_LEN, key_64);
...
static void
vl_api_wg_peers_dump_t_handler (vl_api_wg_peers_dump_t * mp)
{
...
vl_api_wg_genkey_t_handler (vl_api_wg_genkey_t * mp)
{
  ...
  base64_encode (secret, NOISE_PUBLIC_KEY_LEN, secret_64);
...
vl_api_wg_pubkey_t_handler (vl_api_wg_pubkey_t * mp)
{
  ...
  if (!(base64_decode (mp->private_key, NOISE_KEY_LEN_BASE64, secret)))
  ...
  base64_encode (public, NOISE_PUBLIC_KEY_LEN, public_64);
