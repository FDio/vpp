  ...
  if (!wmp->is_inited)
    {
      ...
      if (!base64_decode (private_key_64, NOISE_KEY_LEN_BASE64, private_key))
	{
...
wg_device_clear (wg_main_t * wmp)
{
  ...
  ({
    ...
    wg_peer_clear (peer, vlib_time_now (wmp->vlib_main));
	     ...
	     u16 port, u32 tun_sw_if_index, u16 persistent_keepalive)
{
  ...
  if (!base64_decode (public_key_64, NOISE_KEY_LEN_BASE64, public_key))
    {
