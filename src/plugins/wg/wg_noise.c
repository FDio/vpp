...
{
  const void *evp_md_blake2s = EVP_blake2s256();
  uint8_t out[BLAKE2S_HASHSIZE + 1];
  uint8_t sec[BLAKE2S_HASHSIZE];

  /* Extract entropy from "x" into sec */
  u32 l = 0;
  HMAC (evp_md_blake2s, ck, NOISE_HASH_LEN, x, x_len, sec, &l);
  ...
  out[0] = 1;
  HMAC (evp_md_blake2s, sec, BLAKE2S_HASHSIZE, out, 1, out, &l);
  ASSERT (l == BLAKE2S_HASHSIZE);
  ...
  HMAC (evp_md_blake2s, sec, BLAKE2S_HASHSIZE, out,
