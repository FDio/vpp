...
class TestTMC(VppTestCase):
    ...
    def test_tcp_mss_clamping_ip4_tx(self):
        ...
        self.vapi.tmc_enable_disable(self.pg1.sw_if_index, 1424,
                                     is_ipv6=0, direction=3)

        # Verify that the feature is enabled.
        reply = self.vapi.tmc_get_mss(self.pg1.sw_if_index, is_ipv6=0)
        self.assertEqual(reply.mss, 1424)
        self.assertEqual(reply.direction, 3)
        ...
        self.vapi.tmc_enable_disable(self.pg1.sw_if_index, 1420,
                                     is_ipv6=0, direction=2)
    ...
    def test_tcp_mss_clamping_ip6_tx(self):
        ...
        # enable the TCP MSS clamping feature to lower the MSS to 1424.
        self.vapi.tmc_enable_disable(self.pg1.sw_if_index, 1424,
                                     is_ipv6=1, direction=3)

        # Verify that the feature is enabled.
        reply = self.vapi.tmc_get_mss(self.pg1.sw_if_index, is_ipv6=1)
        self.assertEqual(reply.mss, 1424)
        self.assertEqual(reply.direction, 3)
