...
lip_delete (u32 host_sw_if_index)
{
  ...
  if (INDEX_INVALID == lipi)
  ...
  else
    {
      ...
      vnet_feature_enable_disable ("device-input", "lip-xc",
				   lip->lip_host_sw_if_index,
				   0, &lipi, sizeof (lipi));
      vnet_feature_enable_disable ("arp", "lip-arp",
				   lip->lip_phy_sw_if_index,
				   0, &lipi, sizeof (lipi));
...
format_lip (u8 * s, va_list * args)
{
  ...
  s = format (s, "lip:[%d]: %U <-> %U",
	      lip - lip_pool,
	      format_vnet_sw_if_index_name, vnm, lip->lip_host_sw_if_index,
	      format_vnet_sw_if_index_name, vnm, lip->lip_phy_sw_if_index);
