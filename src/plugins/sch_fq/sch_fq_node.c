...
static u8* format_sch_fq_enqueue_trace (u8 *s, va_list *args) {
...
static u8* format_sch_fq_dequeue_trace (u8 *s, va_list *args) {
    CLIB_UNUSED (vlib_main_t *vm) = va_arg (*args, vlib_main_t*);
    CLIB_UNUSED (vlib_node_t *node) = va_arg (*args, vlib_node_t*);
...
static void enqueue_frame(vlib_main_t *vm, vlib_node_runtime_t *node, sch_fq_buffer_t *fq_b, vlib_buffer_t **b, u32 *from, int n_pkt) {
    for (u32 i = 0; i < n_pkt; i++) {
        ...
        if (clib_bitmap_get(fq_b->class_bitmap, class))
            start_time = ring_buffer_back(&fq_b->queue_map[class]).pkt_data.finish_time;
        else {
        ...
        ring_buffer_push(&(fq_b->queue_map[class]), (buffer_t) { .pkt_data.pkt_index = from[i], .pkt_data.finish_time = finish_time});
...
static int select_next_frame(vlib_main_t *vm, vlib_node_runtime_t *node, sch_fq_buffer_t *fq_b, u32* i_arr, u32 n_pkt) {
    ...
    while (!clib_bitmap_is_zero(fq_b->class_bitmap) && i < n_pkt) {
        ...
        do {
            if (ring_buffer_front(&fq_b->queue_map[j]).pkt_data.finish_time < next_pkt_finish_time) {
                next_pkt_class = j;
                next_pkt_finish_time = ring_buffer_front(&fq_b->queue_map[j]).pkt_data.finish_time;
            }
            j = clib_bitmap_next_set(fq_b->class_bitmap, j+1);
        } while (j != ~0);

        i_arr[i++] = ring_buffer_pop(&fq_b->queue_map[next_pkt_class]).pkt_data.pkt_index;

        int pkt_length = vlib_get_buffer(vm, i_arr[i-1])->current_length;
        fq_b->num_round += (pkt_length + fq_b->num_bit_excess)/clib_bitmap_count_set_bits(fq_b->class_bitmap);
        fq_b->num_bit_excess = (pkt_length + fq_b->num_bit_excess)%clib_bitmap_count_set_bits(fq_b->class_bitmap);
...
static uword sch_fq_enqueue_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame) {
...
static uword sch_fq_dequeue_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame) {
