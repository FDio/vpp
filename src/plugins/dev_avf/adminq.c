...
avf_aq_arq_slot_init (vlib_main_t *vm, vnet_dev_t *dev, u16 slot)
{
  ...
  ad->aq_mem->arq[slot] = (avf_aq_desc_t){
    .flags.buf = 1,
    .flags.lb = AVF_AQ_BUF_SIZE > AVF_AQ_LARGE_BUF,
...
static inline void
avf_irq_0_set_state (avf_device_t *ad, int enable)
{
...
static void
avf_adminq_msix_handler (vlib_main_t *vm, vnet_dev_t *dev, u16 line)
{
  log_debug (dev, "MSI-X interrupt 0 received");
  vnet_dev_process_call_op_no_wait (vm, dev, avf_aq_poll);
}

static void
avf_adminq_intx_handler (vlib_main_t *vm, vnet_dev_t *dev)
{
  avf_adminq_msix_handler (vm, dev, 0);
}
...
void
avf_aq_poll_on (vlib_main_t *vm, vnet_dev_t *dev)
{
  avf_device_t *ad = vnet_dev_get_data (dev);
  ...
  if (vnet_dev_get_pci_n_msix_interrupts (dev) > 0)
    {
      vnet_dev_pci_msix_add_handler (vm, dev, avf_adminq_msix_handler, 0, 1);
      vnet_dev_pci_msix_enable (vm, dev, 0, 1);
    }
  else
    vnet_dev_pci_intx_add_handler (vm, dev, avf_adminq_intx_handler);

  avf_irq_0_set_state (ad, 1);
}

void
avf_aq_poll_off (vlib_main_t *vm, vnet_dev_t *dev)
{
  avf_device_t *ad = vnet_dev_get_data (dev);

  avf_irq_0_set_state (ad, 0);
		...
		const u8 *data, u16 len, f64 timeout)
{
  ...
  *d = *desc;
  ...
  log_debug (dev, "slot %u\n  %U", ad->atq_next_slot, format_avf_aq_desc, d);
