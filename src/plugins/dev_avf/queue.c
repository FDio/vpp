...
avf_rx_queue_alloc (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  ...
  if ((rv =
	 vnet_dev_dma_mem_alloc (vm, dev, sizeof (avf_rx_desc_t) * rxq->size,
				 0, (void **) &arq->descs)))
    return rv;

  arq->qrx_tail = ad->bar0 + AVF_QRX_TAIL (rxq->queue_id);
...
avf_tx_queue_alloc (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  ...
  if ((rv =
	 vnet_dev_dma_mem_alloc (vm, dev, sizeof (avf_tx_desc_t) * txq->size,
				 0, (void **) &atq->descs)))
    return rv;
...
vnet_dev_rv_t
avf_rx_queue_start (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  vnet_dev_t *dev = rxq->port->dev;
  avf_rxq_t *arq = vnet_dev_get_rx_queue_data (rxq);
  ...
  for (u32 i = 0; i < n_enq; i++)
    {
      ...
      d[i] = (avf_rx_desc_t){ .addr = dma_addr };
    }
...
void
avf_rx_queue_stop (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  avf_rxq_t *arq = vnet_dev_get_rx_queue_data (rxq);
...
vnet_dev_rv_t
avf_tx_queue_start (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  avf_txq_t *atq = vnet_dev_get_tx_queue_data (txq);
  atq->next = 0;
  atq->n_enqueued = 0;
...
void
avf_tx_queue_stop (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  avf_txq_t *atq = vnet_dev_get_tx_queue_data (txq);
