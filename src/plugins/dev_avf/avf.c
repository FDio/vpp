...
{
  avf_device_t *ad = vnet_dev_get_data (dev);
  virtchnl_version_info_t ver, myver = {
  ...
  rv = avf_vc_op_version (vm, dev, &myver, &ver);
  if (rv != VNET_DEV_OK)
    return rv;
  ...
  avf_vc_op_get_vf_resources (vm, dev, &bitmap, &res);
  if (rv != VNET_DEV_OK)
    return rv;
  ...
  ad->avail_rxq_bmp = pow2_mask (res.num_queue_pairs);
  ad->avail_txq_bmp = pow2_mask (res.num_queue_pairs);

  vnet_dev_set_hw_addr (&port.port.config.hw_addr, 6,
