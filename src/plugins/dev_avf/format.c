...
u8 *
format_avf_rx_desc_qw1 (u8 *s, va_list *args)
{
  avf_rx_desc_qw1_t *qw1 = va_arg (*args, avf_rx_desc_qw1_t *);
...
u8 *
format_avf_rx_trace (u8 *s, va_list *args)
{
  vlib_main_t *vm = va_arg (*args, vlib_main_t *);
  vlib_node_t *node = va_arg (*args, vlib_node_t *);
  avf_rx_trace_t *t = va_arg (*args, avf_rx_trace_t *);
  avf_rx_desc_qw1_t *qw1;
  ...
  qw1 = (avf_rx_desc_qw1_t *) t->qw1s;

  do
    s = format (s, "\n%Udesc %u: %U", format_white_space, indent + 2, i,
		format_avf_rx_desc_qw1, qw1 + i);
  while ((qw1[i++].eop) == 0 && i < AVF_RX_MAX_DESC_IN_CHAIN);

  return s;
}

u8 *
format_avf_port_status (u8 *s, va_list *args)
{
  ...
  s = format (s, "caps: %U", format_avf_vf_cap_flags, ap->vf_cap_flags);
...
u8 *
format_avf_log (u8 *s, va_list *args)
{
