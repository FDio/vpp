...
avf_tx_fill_ctx_desc (vlib_main_t *vm, vnet_dev_tx_queue_t *txq,
		      avf_tx_desc_t *d, vlib_buffer_t *b)
{
...
avf_tx_prepare (vlib_main_t *vm, vlib_node_runtime_t *node,
	  ...
	  if (flags & VNET_BUFFER_F_GSO)
	  ...
	  else if (PREDICT_FALSE (n_desc_needed > 8))
	    {
	      ...
	      vlib_error_count (vm, node->node_index,
				AVF_TX_NODE_CTR_SEG_SZ_EXCEEDED, 1);
	      n_packets_left -= 1;
	      buffers += 1;
	      continue;
	    }
...
retry:
  ...
  if (atq->n_enqueued)
      ...
      while (1)
	{
	  ...
	  if (avf_tx_desc_get_dtyp (atq->descs + slot[0]) != 0x0F)
	    break;
  ...
  if (n_desc)
	...
	{
	  /* no wrap */
	  avf_tx_copy_desc (atq->descs + next, atq->tmp_descs, n_desc);
      ...
      else
	{
	  ...
	  avf_tx_copy_desc (atq->descs + next, atq->tmp_descs, n_not_wrap);
	  avf_tx_copy_desc (atq->descs, atq->tmp_descs + n_not_wrap,
			    n_desc - n_not_wrap);
	  vlib_buffer_copy_indices (atq->buffer_indices + next, atq->tmp_bufs,
				    n_not_wrap);
      ...
      if ((slot = clib_ring_enq (atq->rs_slots)))
	{
	  ...
	  atq->descs[rs_slot].qword[1] |= AVF_TXD_CMD_RS;
	}
  ...
  if (n_left)
	...
	goto retry;
      ...
      vlib_error_count (vm, node->node_index, AVF_TX_NODE_CTR_NO_FREE_SLOTS,
			n_left);
    }
