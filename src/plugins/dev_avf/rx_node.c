...
static_always_inline void
avf_process_flow_offload (vnet_dev_port_t *port, avf_rt_data_t *rtd,
			  uword n_rx_packets)
{
  uword n;
...
avf_process_rx_burst (vlib_main_t *vm, vlib_node_runtime_t *node,
		      vnet_dev_rx_queue_t *rxq, avf_rt_data_t *rtd,
		      vlib_buffer_template_t *bt, u32 n_left,
		      int maybe_multiseg)
{
  vlib_buffer_t **b = rtd->bufs;
  u64 *qw1 = rtd->qw1s;
  avf_rx_tail_t *tail = rtd->tails;
  uword n_rx_bytes = 0;
			 ...
			 vnet_dev_rx_queue_t *rxq, int with_flows)
{
  ...
  avf_rt_data_t *rtd = vnet_dev_get_rt_temp_space (vm);
  avf_rxq_t *arq = vnet_dev_get_rx_queue_data (rxq);
  ...
  while (n_rx_packets < AVF_RX_VECTOR_SZ)
      ...
      if (PREDICT_FALSE (avf_rxd_is_not_eop (d)))
	{
	  u16 tail_desc = 0;
	  u16 tail_next = next;
	  avf_rx_tail_t *tail = rtd->tails + n_rx_packets;
	  avf_rx_desc_t *td;
	  do
	    {
...
no_more_desc:
  ...
  n_rx_bytes =
    n_tail_desc ?
	    avf_process_rx_burst (vm, node, rxq, rtd, &bt, n_rx_packets, 1) :
	    avf_process_rx_burst (vm, node, rxq, rtd, &bt, n_rx_packets, 0);
  ...
  if (PREDICT_FALSE (with_flows && (or_qw1 & mask_flm.as_u64)))
	...
	rtd->next[n] = next_index;
      ...
      avf_process_flow_offload (port, rtd, n_rx_packets);
    }
