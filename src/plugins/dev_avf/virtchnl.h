...
#define foreach_avf_aq_desc_flag                                              \
  _ (1, dd)                                                                   \
  _ (1, cmp)                                                                  \
  _ (1, err)                                                                  \
  _ (1, vfe)                                                                  \
...
STATIC_ASSERT_SIZEOF (avf_aq_desc_flags_t, 2);

typedef enum
{
  AVF_AQ_DESC_OP_QUEUE_SHUTDOWN = 0x0003,
  AVF_AQ_DESC_OP_SEND_TO_PF = 0x0801,
  AVF_AQ_DESC_OP_MESSAGE_FROM_PF = 0x0802,
} __clib_packed avf_aq_desc_op_t;

#define foreach_avf_aq_desc_retval                                            \
  _ (0, OK)                                                                   \
  _ (1, EPERM)                                                                \
  _ (2, ENOENT)                                                               \
  _ (3, ESRCH)                                                                \
...
typedef enum
{
#define _(a, b) AVF_AQ_DESC_RETVAL_##b = a,
  foreach_avf_aq_desc_retval
#undef _
} __clib_packed avf_aq_desc_retval_t;

typedef struct
{
  avf_aq_desc_flags_t flags;
  avf_aq_desc_op_t opcode;
  u16 datalen;
  u16 retval;
  union
  {
