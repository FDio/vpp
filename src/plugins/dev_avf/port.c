...
avf_port_stop (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  avf_vc_op_disable_queues (vm, dev, &qs);
}
		     ...
		     vnet_dev_port_cfg_change_req_t *req)
{
  ...
  if (req->changes.promisc)
    {
      virtchnl_promisc_info_t pi = {
	.vsi_id = ap->vsi_id,
	.unicast_promisc = req->new.promisc,
	.multicast_promisc = req->new.promisc,
      };
  ...
  if (req->changes.primary_hw_addr)
    {
	...
	.list[0].primary = 1,
      ...
      clib_memcpy (al.list[0].addr, port->conf.primary_hw_addr.eth_mac,
		   sizeof (al.list[0].addr));

      rv = avf_vc_op_del_eth_addr (vm, dev, &al);
      if (rv != VNET_DEV_OK)
	return rv;

      clib_memcpy (al.list[0].addr, req->new.primary_hw_addr.eth_mac,
		   sizeof (al.list[0].addr));
