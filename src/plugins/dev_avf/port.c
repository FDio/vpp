...
const static avf_dyn_ctln dyn_ctln_disabled = {};
const static avf_dyn_ctln dyn_ctln_enabled = {
  .clearpba = 1,
  .interval = AVF_ITR_INT / 2,
  .intena = 1,
};
const static avf_dyn_ctln dyn_ctln_wb_on_itr = {
  .clearpba = 1,
  .itr_indx = 1,
  .interval = 32 / 2,
  .wb_on_itr = 1,
...
avf_port_vlan_strip_disable (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if ((rv = avf_vc_op_get_offload_vlan_v2_caps (vm, dev, &vc)))
    return rv;
...
vnet_dev_rv_t
avf_port_rx_irq_enable_disable (vlib_main_t *vm, vnet_dev_port_t *port,
				int enable)
{
  vnet_dev_t *dev = port->dev;
  ...
  if ((rv = avf_vc_op_config_irq_map (vm, dev, im)))
    return rv;
  ...
  for (int i = 0; i < im->num_vectors; i++)
	...
	val = dyn_ctln_enabled.as_u32;
      ...
      avf_reg_write (ad, AVFINT_DYN_CTLN (im->vecmap[i].vector_id), val);
    }
...
avf_port_init (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if ((rv = avf_port_vlan_strip_disable (vm, port)))
    return rv;

  if ((rv = avf_port_init_rss (vm, port)))
    return rv;
...
avf_port_start (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  foreach_vnet_dev_port_rx_queue (q, port)
    if (q->enabled)
      if ((rv = avf_rx_queue_start (vm, q)))
	goto done;

  foreach_vnet_dev_port_tx_queue (q, port)
    if ((rv = avf_tx_queue_start (vm, q)))
      goto done;

  if ((rv = avf_port_update_rss_lut (vm, port)))
    goto done;

  /* configure qpairs */
  if ((rv = avf_port_init_vsi_queues (vm, port)))
    goto done;

  if ((rv = avf_port_rx_irq_enable_disable (vm, port, /* enable */ 1)))
    goto done;

  if ((rv = avf_enable_disable_queues (vm, port, 1)))
    goto done;
  ...
  if (rv)
    {
      foreach_vnet_dev_port_rx_queue (q, port)
	avf_rx_queue_stop (vm, q);
      foreach_vnet_dev_port_tx_queue (q, port)
	avf_tx_queue_stop (vm, q);
    }
  return rv;
}
...
avf_port_stop (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  avf_port_rx_irq_enable_disable (vm, port, /* disable */ 0);

  if (port->dev->poll_stats)
    vnet_dev_poll_port_remove (vm, port, avf_port_poll_stats);

  foreach_vnet_dev_port_rx_queue (rxq, port)
    avf_rx_queue_stop (vm, rxq);

  foreach_vnet_dev_port_tx_queue (txq, port)
    avf_tx_queue_stop (vm, txq);
...
vnet_dev_rv_t
avf_port_cfg_change_validate (vlib_main_t *vm, vnet_dev_port_t *port,
