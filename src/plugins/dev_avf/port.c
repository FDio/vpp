...
avf_port_init_rss (vlib_main_t *vm, vnet_dev_port_t *port, u32 enabled_rxq_bmp)
{
  ...
  if ((ap->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_RSS_PF) == 0)
    return rv;

  if (sizeof (default_rss_key) < ap->rss_key_size)
    {
      log_err (dev, "reported rss key size (%u) too small", ap->rss_key_size);
      return VNET_DEV_ERR_UNSUPPORTED_DEV;
    }

  /* config RSS LUT */
  u.lut.vsi_id = ap->vsi_id;
  u.lut.lut_entries = ap->rss_lut_size;
  ...
  /* config RSS key */
  u.key.vsi_id = ap->vsi_id;
  u.key.key_len = clib_min (sizeof (default_rss_key), ap->rss_key_size);
