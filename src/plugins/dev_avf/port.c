...
static vnet_dev_counter_t avf_port_counters[] = {
  VNET_DEV_CTR_RX_BYTES (0),   VNET_DEV_CTR_TX_BYTES (0),
  VNET_DEV_CTR_RX_PACKETS (0), VNET_DEV_CTR_TX_PACKETS (0),
  VNET_DEV_CTR_RX_DROPS (0),   VNET_DEV_CTR_TX_DROPS (0),
};
...
avf_port_poll_stats (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  avf_device_t *ad = vnet_dev_get_data (dev);
  virtchnl_queue_select_t qs = { .vsi_id = ad->vsi_id };
