...
#include <dev_avf/avf_desc.h>
#include <dev_avf/virtchnl.h>

#define AVF_ITR_INT		 250
#define AVF_RX_MAX_DESC_IN_CHAIN 5
#define AVF_MAX_RSS_KEY_SIZE	 52
#define AVF_MAX_RSS_LUT_SIZE	 64
...
void avf_aq_init (vlib_main_t *, vnet_dev_t *);
void avf_aq_poll_on (vlib_main_t *, vnet_dev_t *);
void avf_aq_poll_off (vlib_main_t *, vnet_dev_t *);
...
format_function_t format_avf_rx_trace;
format_function_t format_avf_port_status;
format_function_t format_avf_log;
...
vnet_dev_rv_t avf_port_cfg_change (vlib_main_t *, vnet_dev_port_t *,
				   vnet_dev_port_cfg_change_req_t *);

/* queue.c */
vnet_dev_rv_t avf_rx_queue_alloc (vlib_main_t *, vnet_dev_rx_queue_t *);
vnet_dev_rv_t avf_tx_queue_alloc (vlib_main_t *, vnet_dev_tx_queue_t *);
vnet_dev_rv_t avf_rx_queue_start (vlib_main_t *, vnet_dev_rx_queue_t *);
vnet_dev_rv_t avf_tx_queue_start (vlib_main_t *, vnet_dev_tx_queue_t *);
void avf_rx_queue_stop (vlib_main_t *, vnet_dev_rx_queue_t *);
void avf_tx_queue_stop (vlib_main_t *, vnet_dev_tx_queue_t *);
void avf_rx_queue_free (vlib_main_t *, vnet_dev_rx_queue_t *);
void avf_tx_queue_free (vlib_main_t *, vnet_dev_tx_queue_t *);

/* counter.c */
void avf_port_poll_stats (vlib_main_t *, vnet_dev_port_t *);
void avf_port_add_counters (vlib_main_t *, vnet_dev_port_t *);
...
{
  avf_reg_read (ad, VFGEN_RSTAT);
  asm volatile("" ::: "memory");
}

#define log_debug(dev, f, ...)                                                \
  vlib_log (VLIB_LOG_LEVEL_DEBUG, avf_log.class, "%U" f, format_avf_log,      \
...
#define foreach_avf_tx_node_counter                                           \
  _ (SEG_SZ_EXCEEDED, seg_sz_exceeded, ERROR, "segment size exceeded")        \
  _ (NO_FREE_SLOTS, no_free_slots, ERROR, "no free tx slots")

typedef enum
{
#define _(f, n, s, d) AVF_TX_NODE_CTR_##f,
  foreach_avf_tx_node_counter
#undef _
} avf_tx_node_counter_t;

#define foreach_avf_rx_node_counter                                           \
  _ (BUFFER_ALLOC, buffer_alloc, ERROR, "buffer alloc error")

typedef enum
{
#define _(f, n, s, d) AVF_RX_NODE_CTR_##f,
  foreach_avf_rx_node_counter
#undef _
} avf_rx_node_counter_t;
