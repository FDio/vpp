			       ...
			       pppoeclient_session_input_result_t *r)
{
  ...
  l2_hdr_len =
    rx_sw_if_index < vec_len (pem->l2_encap_len_by_sw_if_index) ?
  ...
  if (PREDICT_FALSE (l2_hdr_len == 0))
    {
       ...
       * full-parse fallback. */
      vlib_buffer_reset (b);
      if (!pppoeclient_get_l2_info (b, &h, &pppoe, 0, &l2_hdr_len))
	{
  ...
  if (ppp_proto == PPP_PROTOCOL_ip4)
	...
	{
	  clib_warning ("pppox: control handoff queue missing, falling back to worker-side FSM processing");
	   ...
	   * thread and enqueue the other buffer individually. */
	  {
		...
		if (vlib_buffer_enqueue_to_thread (vm, node, pppox_main.input_fq_index, &bi0, &ti,
		  ...
		  vlib_node_increment_counter (vm, node->node_index,
					       PPPOECLIENT_ERROR_HANDOFF_DROP, 1);
	      }
	    else
	      {
		to_next[0] = bi0;
			  ...
			  u16 *ethertype_out)
{
  ...
  (void) pppoeclient_get_l2_info (b, 0, 0, &ethertype, &l2_hdr_len);
  *ethertype_out = ethertype;
  ...
  if (ethertype == ETHERNET_TYPE_PPPOE_DISCOVERY)
	...
	{
	  u16 ti = 0;
	  if (vlib_buffer_enqueue_to_thread (vm, node, pem->discovery_fq_index, &bi, &ti,
					     1) == 0)
	    vlib_node_increment_counter (vm, node->node_index,
					 PPPOECLIENT_ERROR_HANDOFF_DROP, 1);
	  return 1;
      ...
      clib_warning ("pppoeclient: discovery handoff queue missing, falling back to worker-side processing");
      *next = PPPOECLIENT_DISPATCH_NEXT_DISCOVERY_INPUT;
    }
  else if (ethertype == ETHERNET_TYPE_PPPOE_SESSION)
    {
