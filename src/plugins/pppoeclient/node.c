	...
	trace00:
	  ...
	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
		...
		= vlib_add_trace (vm, node, b0, sizeof (*tr));
	      ...
	      /* vlib_add_trace() does not zero the trace storage. */
...
pppoeclient_tcp_mss_fixup (tcp_header_t *tcp, u16 max_mss, u32 tcp_avail)
{
  ...
  for (; opts_len > 0;)
	...
	{
	  /* The MSS option value is not guaranteed to land on a two-byte
	   * boundary inside the TCP options block (a preceding one-byte NOP
	   * is enough), so use the unaligned helper for the load/store. */
	  u16 old_mss = clib_mem_unaligned (data + 2, u16);
	  if (clib_net_to_host_u16 (old_mss) > max_mss)
	    {
	      u16 new_mss = clib_host_to_net_u16 (max_mss);
	      u16 old_csum = old_mss;
	      u16 new_csum = new_mss;

	      clib_mem_unaligned (data + 2, u16) = new_mss;

	      /* ip_csum_update() assumes the changed field sits at an even
	       * offset in the checksummed 16-bit word stream.  When the MSS
	       * value starts at an odd offset it straddles two words and
	       * contributes its byte-swapped value, so swap the operands to
	       * keep the incremental checksum delta correct. */
	      if (PREDICT_FALSE (((data + 2) - (const u8 *) tcp) & 1))
		{
		  old_csum = clib_byte_swap_u16 (old_mss);
		  new_csum = clib_byte_swap_u16 (new_mss);
		}
	      sum = tcp->checksum;
	      sum = ip_csum_update (sum, old_csum, new_csum, tcp_header_t, checksum);
...
/* Locate a TCP header in an IPv6 packet, refusing non-first fragments.
 * ip6_ext_header_walk() records the protocol named by a fragment header's
 * next-header field even when the fragment offset is non-zero, so the bytes
 * it points at after such a fragment are payload rather than a TCP header. */
static_always_inline tcp_header_t *
pppoeclient_ip6_find_tcp (vlib_buffer_t *b, ip6_header_t *ip6)
{
  ip6_ext_hdr_chain_t chain;
  int res;

  res = ip6_ext_header_walk (b, ip6, IP_PROTOCOL_TCP, &chain);
  if (res < 0 || chain.eh[res].protocol != IP_PROTOCOL_TCP)
    return 0;

  for (int i = 0; i < res; i++)
    {
      if (chain.eh[i].protocol == IP_PROTOCOL_IPV6_FRAGMENTATION)
	{
	  ip6_frag_hdr_t *frag = ip6_ext_next_header_offset (ip6, chain.eh[i].offset);
	  if (ip6_frag_hdr_offset (frag) != 0)
	    return 0;
	}
    }

  return ip6_ext_next_header_offset (ip6, chain.eh[res].offset);
}

static_always_inline void
pppoeclient_try_update_tcp_mss (vnet_main_t *vnm, vlib_buffer_t *b, u32 sw_if_index)
{
  ...
  if (ppp_protocol == PPP_PROTOCOL_ip4)
	...
	return;
      ...
      /* Only the first fragment carries the TCP header — rewriting a
       * non-first fragment's payload (and its "checksum") is wrong. */
      if (ip4->protocol == IP_PROTOCOL_TCP && !ip4_is_fragment (ip4))
	{
	  ...
	  /* The overwhelming majority of TCP packets are non-SYN: skip the
	   * cold interface-pool lookup for the mss limit until we know the
	   * packet can actually be clamped. */
	  if (tcp_syn (tcp))
	    {
	...
	return;
      ...
      tcp_header_t *tcp = pppoeclient_ip6_find_tcp (b, ip6);
      if (tcp)
	{
	  ...
	  /* ip6_ext_header_find() only bounds the extension-header chain, not
	   * the TCP header itself - require a full header before the fixup
	   * dereferences tcp_syn()/tcp_doff() (mirrors upstream mss_clamp). */
	  if (tcp_avail >= sizeof (tcp_header_t) && tcp_syn (tcp))
	    {
...
typedef struct
{
  ...
  u32 rx_sw_if_index;
  /* PPP control packet handed off to the main-thread pppox-input queue;
   * the caller must NOT enqueue this buffer to a next node. */
  u8 handoff;
			       ...
			       pppoeclient_session_input_result_t *r)
{
  ...
  /* Callers read this flag unconditionally to choose between the normal
   * enqueue path and the main-thread control handoff.  Only the PPP control
   * branch below sets it, so clear it for every other packet here. */
  r->handoff = 0;

  rx_sw_if_index = vnet_buffer (b)->sw_if_index[VLIB_RX];

  /* Fast path: dispatch hands session frames over with current pointing at
   * the ethernet header; the L2 encap length is cached per access
   * interface, so no L2 re-parse is needed.  The reset stays (cheap) to
   * undo any advance made by upstream device-input features. */
  vlib_buffer_reset (b);

  l2_hdr_len = rx_sw_if_index < vec_len (pem->l2_encap_len_by_sw_if_index) ?
		 (u16) pem->l2_encap_len_by_sw_if_index[rx_sw_if_index] :
		 0;

  if (PREDICT_FALSE (l2_hdr_len == 0))
    {
      /* Cache miss (feature enabled before the cache existed): legacy
       * full-parse fallback. */
      vlib_buffer_reset (b);
      if (!pppoeclient_get_l2_info (b, &h, &pppoe, 0, &l2_hdr_len, 0))
	{
  ...
  else
    {
      ...
      pppoe = (pppoe_header_t *) ((u8 *) h + l2_hdr_len);
    }

  /* Bound the buffer before dereferencing the PPPoE header: a truncated
   * session frame must not make the ver_type read walk past the data. */
  if (PREDICT_FALSE (b->current_length < l2_hdr_len + sizeof (*pppoe) + 2 ||
    ...
    }
  {
    ...
    if (PREDICT_FALSE (decl_len > b->current_length - l2_hdr_len - sizeof (*pppoe)))
      {
	error = PPPOECLIENT_ERROR_BAD_CODE_IN_SESSION;
	goto out;
      }

    /* The declared payload must at least contain the 2-byte PPP protocol
     * field.  Without this check, a frame declaring length 0/1 would make
     * the vlib_buffer_advance (b, sizeof (u16)) below underflow the u16
     * current_length (ASSERT abort in debug builds, 64KB stale-buffer
     * parse in release builds). */
    if (PREDICT_FALSE (decl_len < 2))
      {
	error = PPPOECLIENT_ERROR_BAD_CODE_IN_SESSION;
	goto out;
      }
  ...
  if (ppp_proto == PPP_PROTOCOL_ip4)
    {
      pppoeclient_try_update_tcp_mss (vnm, b, c->pppox_sw_if_index);
...

{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_TRUE (!(r0.handoff || r1.handoff)))
	    {
	      to_next[0] = bi0;
	      to_next[1] = bi1;
	      to_next += 2;
	  ...
	  /* Rare path: at least one PPP control packet.  Hand it to the main
	   * thread and enqueue the other buffer individually. */
	  {
	    u16 ti = 0;
	    if (r0.handoff)
	      {
		if (vlib_buffer_enqueue_to_thread (vm, node, pppox_main.input_fq_index, &bi0, &ti,
						   1) == 0)
		  vlib_node_increment_counter (vm, node->node_index, PPPOECLIENT_ERROR_HANDOFF_DROP,
					       1);
	      }
	    else
	      {
	    ...
	    if (r1.handoff)
	      {
		if (vlib_buffer_enqueue_to_thread (vm, node, pppox_main.input_fq_index, &bi1, &ti,
						   1) == 0)
		  vlib_node_increment_counter (vm, node->node_index, PPPOECLIENT_ERROR_HANDOFF_DROP,
					       1);
	      }
		...
		n_left_to_next -= 1;
		pppoeclient_session_input_trace (vm, node, b1, &r1);
		vlib_validate_buffer_enqueue_x1 (vm, node, next_index, to_next, n_left_to_next, bi1,
						 r1.next);
	      }
	  }
	}
				...
				pppoeclient_session_output_result_t *r)
{
  ...
  /* The client caches hw_if_index/sup_sw_if_index at creation, so use direct
   * pool lookups instead of vnet_get_sup_* walking the interface chain. */
  hw = vnet_get_hw_interface (vnm, c->hw_if_index);
  sup_sw = vnet_get_sw_interface (vnm, c->sup_sw_if_index);
  ...
  ppp_len = vlib_buffer_length_in_chain (vm, b);
  pppoeclient_try_update_tcp_mss (vnm, b, c->pppox_sw_if_index);
  ...
  /* current_length only covers the first buffer of a chain; report the
   * whole PPP payload so a jumbo/chained TX packet is not truncated by
   * the receiver. */
  pppoe->length =
    clib_host_to_net_u16 ((u16) (vlib_buffer_length_in_chain (vm, b) - sizeof (pppoe_header_t)));

  if (PREDICT_TRUE (c->tx_l2_len != 0))
    {
      /* Copy the precomputed L2 template and refresh the source MAC so a
       * runtime MAC change still takes effect without rebuilding it. */
      vlib_buffer_advance (b, -(word) c->tx_l2_len);
      u8 *l2 = vlib_buffer_get_current (b);
      clib_memcpy (l2, c->tx_l2_template, c->tx_l2_len);
      clib_memcpy (l2 + 6, hw->hw_address, 6);
    }
  else
    {
      /* Defensive fallback for a client whose template was never built. */
...
/* Accumulate pppox TX interface counters in per-invocation locals, flushing
 * when the interface changes.  The removed pppox-output node carried the
 * batch interface in node->runtime_data[0], which is cross-thread shared
 * state when several workers transmit at once. */
static_always_inline void
pppoeclient_session_output_account (vnet_interface_main_t *im, u32 thread_index,
				    u32 *stats_sw_if_index, u32 *stats_n_packets,
				    u32 *stats_n_bytes, u32 pppox_sw_if_index, u32 bytes)
{
  if (*stats_n_packets && *stats_sw_if_index != pppox_sw_if_index)
    {
  ...
  *stats_sw_if_index = pppox_sw_if_index;
  (*stats_n_packets)++;
  *stats_n_bytes += bytes;
}
...

{
  ...
  vnet_interface_main_t *im = &pem->vnet_main->interface_main;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (r0.forwarded)
	    pppoeclient_session_output_account (im, thread_index, &stats_pppox_sw_if_index,
						&stats_n_packets, &stats_n_bytes,
						r0.pppox_sw_if_index, r0.bytes);
	  if (r1.forwarded)
	    pppoeclient_session_output_account (im, thread_index, &stats_pppox_sw_if_index,
						&stats_n_packets, &stats_n_bytes,
						r1.pppox_sw_if_index, r1.bytes);
...
/* Map an ingress PPPoE frame on a physical port to the configured access
 * sw_if_index.  The device-input feature runs on the physical port, before
 * ethernet-input demultiplexes tagged frames to sub-interfaces, so the
 * frame's tag stack is matched against each client's stored sub-interface
 * classification.  Returns ~0 when no client owns the frame, in which case
 * the caller must pass it along the feature arc (e.g. to the pppoe plugin's
 * ethertype handler) instead of consuming it. */
static_always_inline u32
pppoeclient_resolve_access_sw_if_index (pppoeclient_main_t *pem, u32 sup_sw_if_index,
					const pppoeclient_l2_tags_t *tags)
{
  u32 *indices;
  u32 best = ~0;
  u32 best_score = 0;
  pppoeclient_t *best_client = 0;
  u32 i;

  if (sup_sw_if_index >= vec_len (pem->client_indices_by_sup_sw_if_index))
    return ~0;

  indices = pem->client_indices_by_sup_sw_if_index[sup_sw_if_index];
  for (i = 0; i < vec_len (indices); i++)
	...
	continue;
      ...
      if (c->vlan_match.n_tags != tags->n_tags)
	continue;
      if (tags->n_tags > 0)
	{
	  if (c->vlan_match.dot1ad != tags->dot1ad)
	    continue;
	  if (!c->vlan_match.outer_any && c->vlan_match.outer_vlan_id != tags->outer_vlan_id)
	    continue;
	}
      if (tags->n_tags > 1 && !c->vlan_match.inner_any &&
	  c->vlan_match.inner_vlan_id != tags->inner_vlan_id)
	continue;

      /* Prefer the most specific match: wildcard tags score lower. */
      score = 1;
      if (tags->n_tags > 0 && !c->vlan_match.outer_any)
	score++;
      if (tags->n_tags > 1 && !c->vlan_match.inner_any)
	score++;
      if (score > best_score)
  ...
  /* Record the tag stack the winning client actually matched.  For exact
   * sub-interfaces this mirrors vlan_match; for wildcard ("default")
   * sub-interfaces it is the only place the concrete VLAN is known, and
   * the TX template, session file and shutdown raw PADT need it.  The tag
   * stack belongs to the access interface, not to one client: several
...
/* Classify one PPPoE frame.  Discovery packets are handed off to the
 * main thread (via pem->discovery_fq_index) so every control-plane state
 * transition runs there; session frames continue on the worker data path.
 * Returns 1 when the buffer was handed off (caller must NOT enqueue it to a
 * next node), 0 otherwise with *next set. */
static_always_inline u8
pppoeclient_dispatch_one (vlib_main_t *vm, vlib_node_runtime_t *node, u32 bi, u32 *next,
			  u16 *ethertype_out)
{
  vlib_buffer_t *b = vlib_get_buffer (vm, bi);
  u16 ethertype = 0;
  u16 l2_hdr_len = sizeof (ethernet_header_t);
  pppoeclient_l2_tags_t tags = { 0 };
  pppoeclient_main_t *pem = &pppoeclient_main;
  u32 sup_sw_if_index;
  u32 access_sw_if_index;

  (void) pppoeclient_get_l2_info (b, 0, 0, &ethertype, &l2_hdr_len, &tags);
  *ethertype_out = ethertype;

  if (ethertype != ETHERNET_TYPE_PPPOE_DISCOVERY && ethertype != ETHERNET_TYPE_PPPOE_SESSION)
    {
      vnet_feature_next (next, b);
      return 0;
    }

  sup_sw_if_index = vnet_buffer (b)->sw_if_index[VLIB_RX];
  access_sw_if_index = pppoeclient_resolve_access_sw_if_index (pem, sup_sw_if_index, &tags);
  if (access_sw_if_index == ~0)
    {
      /* No configured client owns this (port, tag stack): stay out of the
       * way and let the rest of the device-input arc see the frame. */
      vnet_feature_next (next, b);
      return 0;
    }

  /* Downstream nodes key their lookups on the configured access interface,
   * not the physical RX interface. */
  vnet_buffer (b)->sw_if_index[VLIB_RX] = access_sw_if_index;

  if (ethertype == ETHERNET_TYPE_PPPOE_DISCOVERY)
    {
      vlib_buffer_advance (b, l2_hdr_len);
      if (pem->discovery_fq_index != ~0)
	{
	  u16 ti = 0;
	  if (vlib_buffer_enqueue_to_thread (vm, node, pem->discovery_fq_index, &bi, &ti, 1) == 0)
    ...
    {
      /* Leave the buffer at the ethernet header: session-input derives the
       * PPPoE header position from the cached per-interface encap length,
       * avoiding a second L2 parse + reset per packet. */
      *next = PPPOECLIENT_DISPATCH_NEXT_SESSION_INPUT;
    }
...
static_always_inline void
pppoeclient_dispatch_trace_one (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b,
				u32 next, u16 ethertype)
{
  if (PREDICT_FALSE (b->flags & VLIB_BUFFER_IS_TRACED))
    {
      pppoeclient_dispatch_trace_t *tr = vlib_add_trace (vm, node, b, sizeof (*tr));
      tr->next_index = next;
      tr->sw_if_index = vnet_buffer (b)->sw_if_index[VLIB_RX];
      tr->ethertype = ethertype;
    }
}
  ...
  while (n_left_from > 0)
    {
      ...
      /* Process two buffers per iteration with a two-ahead prefetch,
       * mirroring pppoeclient-session-input on the same device-input path.
       * Discovery buffers may be handed off to the main thread inside
       * pppoeclient_dispatch_one(), so the mixed case enqueues only the
       * buffers that were not handed off. */
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  u32 bi0, bi1;
	  vlib_buffer_t *b0, *b1;
	  ...
	  handoff0 = pppoeclient_dispatch_one (vm, node, bi0, &next0, &ethertype0);
	  handoff1 = pppoeclient_dispatch_one (vm, node, bi1, &next1, &ethertype1);

	  if (!handoff0 && !handoff1)
	    {
	      to_next[0] = bi0;
	      to_next[1] = bi1;
	      to_next += 2;
	      ...
	      pppoeclient_dispatch_trace_one (vm, node, b0, next0, ethertype0);
	      pppoeclient_dispatch_trace_one (vm, node, b1, next1, ethertype1);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (pppoeclient_dispatch_one (vm, node, bi0, &next0, &ethertype0))
	    {
	      /* Buffer ownership moved to the main thread. */
	      continue;
	    }
