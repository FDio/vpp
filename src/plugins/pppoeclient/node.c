			       ...
			       pppoeclient_session_input_result_t *r)
{
  ...
  vlib_buffer_reset (b);
  (void) pppoeclient_get_l2_info (b, &h, &pppoe, 0, &l2_hdr_len);
  ...
  ppp_proto = clib_net_to_host_u16 (*(u16 *) (pppoe + 1));
  ...
  if (ppp_proto == PPP_PROTOCOL_ip4)
    {
      ...
      vlib_buffer_advance (b, sizeof (ppp_proto));
  ...
  if (PREDICT_FALSE (b->flags & VLIB_BUFFER_IS_TRACED))
    {
      ...
      tr->session_id = clib_net_to_host_u16 (r->pppoe->session_id);
