...
try_clamp_tcp_mss_in_syn (tcp_header_t *tcp0, u16 max_mss)
{
  ...
  u8 *data = (u8 *) (tcp0 + 1), *opt = data;

  while (opt < data + opts_len)
    {
      u8 kind = opt[0];
      ...
      if (kind == 1)
	{
      ...
      /* All other options must include a valid length byte. */
      if (opt + 1 >= data + opts_len)
	break;

      u8 opt_len = opt[1];
      if (opt_len < 2 || opt + opt_len > data + opts_len)
	break;

      /* MSS option: kind=2, length=4, value(2 bytes) */
      if (kind == TCP_OPTION_MSS && opt_len == 4)
	{
	  u16 mss = clib_net_to_host_u16 (*(u16 *) (opt + 2));
	  if (mss > max_mss)
	    {
	      u16 new_mss_net = clib_host_to_net_u16 (max_mss);
	      ip_csum_t sum0 = tcp0->checksum;
	      sum0 =
		ip_csum_update (sum0, *(u16 *) (opt + 2), new_mss_net, ip4_header_t, /* cheat */
				length /* changed member */);
	      *(u16 *) (opt + 2) = new_mss_net;
