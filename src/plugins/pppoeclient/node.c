				...
				u32 sw_if_index)
{
  ...
  if (ppp_protocol == PPP_PROTOCOL_ip4)
      ...
      if (tcp)
	{
	  ...
	  if (tcp_avail >= sizeof (tcp_header_t))
	    {
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_TRUE (!(r0.handoff || r1.handoff)))
	    {
	      ...
	      vlib_validate_buffer_enqueue_x2 (vm, node, next_index, to_next,
					       n_left_to_next, bi0, bi1, r0.next,
					       r1.next);
	      continue;
	    }
	   ...
	   * thread and enqueue the other buffer individually. */
	  {
	    u16 ti = 0;
	    if (r0.handoff)
	      vlib_buffer_enqueue_to_thread (vm, node, pppox_main.input_fq_index, &bi0, &ti, 1);
	    else
	      {
		to_next[0] = bi0;
		to_next += 1;
		...
		vlib_validate_buffer_enqueue_x1 (vm, node, next_index, to_next,
						 n_left_to_next, bi0, r0.next);
	      }
	    if (r1.handoff)
	      vlib_buffer_enqueue_to_thread (vm, node, pppox_main.input_fq_index, &bi1, &ti, 1);
	    else
	      {
		to_next[0] = bi1;
		to_next += 1;
			  ...
			  u16 *ethertype_out)
{
  ...
  *ethertype_out = ethertype;
  ...
  if (ethertype == ETHERNET_TYPE_PPPOE_DISCOVERY)
	...
	{
	  u16 ti = 0;
	  vlib_buffer_enqueue_to_thread (vm, node, pem->discovery_fq_index, &bi, &ti, 1);
	  return 1;
	}
