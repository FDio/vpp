		...
		int is_broadcast)
{
   ...
   */
  {
    ...
    u32 total_len = pppoeclient_get_l2_encap_len (vnm, c->sw_if_index) + sizeof (pppoe_header_t) +
		    ...
		    sizeof (c->host_uniq) + (cookie_len ? sizeof (pppoe_tag_header_t) + cookie_len : 0);
			    ...
			    pppoeclient_history_cli_filter_t *filter)
{
  ...
  if (hw == 0)
    ...
    vlib_cli_output (
      ...
      format_ethernet_address, hw->hw_address, format_ethernet_address, linux_mac,
      clib_memcmp (linux_mac, hw->hw_address, 6) == 0);
