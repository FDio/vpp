...
pppoeclient_save_session_to_file (pppoeclient_t *c)
{
  ...
  vlib_log (VLIB_LOG_LEVEL_INFO, pppoeclient_log_class,
	    ...
	    src_mac[4], src_mac[5], c->ac_mac_address[0], c->ac_mac_address[1], c->ac_mac_address[2],
					   ...
					   event_summary.service_name_len) != 0))
	{
	  ...
	  (event_summary.cookie_len > 0 &&
	   (event_summary.cookie_value_truncated ||
	    ...
	    clib_memcmp (c->cookie_value, event_summary.cookie_value,
			 event_summary.cookie_len) != 0)))
	{
	  pppoeclient_update_latest_control_disposition (c,
							 PPPOECLIENT_CONTROL_DISPOSITION_IGNORED);
...
pppoeclient_init (vlib_main_t *vm)
{
  ...
  {
    vlib_node_t *dn =
      vlib_get_node_by_name (vm, (u8 *) "pppoeclient-discovery-input");
    if (dn)
      pem->discovery_fq_index =
