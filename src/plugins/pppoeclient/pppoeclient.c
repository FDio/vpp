...
static void pppoeclient_send_padt_raw (const char *linux_ifname, const u8 *src_mac,
				       const u8 *dst_mac, u16 session_id, u8 vlan_valid, u8 n_tags,
				       u8 dot1ad, u16 outer_vlan_id, u16 inner_vlan_id);
...
static void
pppoeclient_dispatch_ref (pppoeclient_main_t *pem, u32 sup_sw_if_index)
{
  /* The device-input arc runs on the physical port, so the feature must be
   * enabled there even when the client is configured on a VLAN
   * sub-interface.  One refcount per physical port. */
  vec_validate_init_empty (pem->dispatch_refcount_by_sw_if_index, sup_sw_if_index, 0);
  if (pem->dispatch_refcount_by_sw_if_index[sup_sw_if_index]++ == 0)
    vnet_feature_enable_disable ("device-input", "pppoeclient-dispatch", sup_sw_if_index, 1, 0, 0);
}

static void
pppoeclient_cache_l2_encap_len (pppoeclient_main_t *pem, u32 sw_if_index)
{
  /* Cache the access interface's L2 encap length for the session fast path
   * (discovery and session frames share the same VLAN configuration). */
...
static void
pppoeclient_sup_client_list_add (pppoeclient_main_t *pem, u32 sup_sw_if_index, u32 client_index)
{
  vec_validate_init_empty (pem->client_indices_by_sup_sw_if_index, sup_sw_if_index, 0);
  vec_add1 (pem->client_indices_by_sup_sw_if_index[sup_sw_if_index], client_index);
}

static void
pppoeclient_sup_client_list_del (pppoeclient_main_t *pem, u32 sup_sw_if_index, u32 client_index)
{
  u32 *indices;
  u32 i;
...
static void
pppoeclient_dispatch_unref (pppoeclient_main_t *pem, u32 sup_sw_if_index)
{
  if (sup_sw_if_index >= vec_len (pem->dispatch_refcount_by_sw_if_index))
    return;
  ...
  if (--pem->dispatch_refcount_by_sw_if_index[sup_sw_if_index] == 0)
    vnet_feature_enable_disable ("device-input", "pppoeclient-dispatch", sup_sw_if_index, 0, 0, 0);
}
...
pppoeclient_clear_runtime_state (pppoeclient_t *c)
{
  ...
  c->ipv6cp_state = 0;
  c->ipv6cp_id = 0;
  clib_memset (&c->session_vlan, 0, sizeof (c->session_vlan));
...
static void
pppoeclient_session_file_path (u32 sw_if_index, u32 host_uniq, char *buf, size_t buf_len)
{
  /* Include host_uniq: multiple clients can share one access interface and
   * must not clobber each other's persistence record (the exit path reads
   * the file to send a raw PADT on shutdown). */
  snprintf (buf, buf_len, PPPOECLIENT_SESSION_DIR "/pppoeclient-%u-%u.session", sw_if_index,
	    host_uniq);
}

/* Resolve the VLAN tag stack the shutdown raw PADT must carry.  Prefer the
 * tag stack observed by the dispatch feature: for wildcard ("default")
 * sub-interfaces it is the only place the concrete VLAN is known.  Fall back
 * to an exact sub-interface configuration when nothing was learned. */
static void
pppoeclient_padt_vlan_from_client (pppoeclient_t *c, u8 *valid, u8 *n_tags, u8 *dot1ad,
				   u16 *outer_vlan_id, u16 *inner_vlan_id)
{
  *valid = 0;
  *n_tags = 0;
  *dot1ad = 0;
  ...
  *inner_vlan_id = 0;
  ...
  if (c->session_vlan.valid)
    {
      *valid = 1;
      *n_tags = c->session_vlan.n_tags;
      *dot1ad = c->session_vlan.dot1ad;
      *outer_vlan_id = c->session_vlan.outer_vlan_id;
      *inner_vlan_id = c->session_vlan.inner_vlan_id;
    }
  else if (c->vlan_match.n_tags == 0)
    *valid = 1;
  else if (!c->vlan_match.outer_any && !c->vlan_match.inner_any)
    {
      *valid = 1;
      *n_tags = c->vlan_match.n_tags;
      *dot1ad = c->vlan_match.dot1ad;
      *outer_vlan_id = c->vlan_match.outer_vlan_id;
      *inner_vlan_id = c->vlan_match.inner_vlan_id;
    }
}

/* Rebuild the cached session-data L2 header.  Called at client creation and
 * whenever the session is established (the AC MAC becomes known), so the TX
 * fast path can copy the encap instead of re-resolving the access interface.
 * The VLAN stack prefers the tag learned by the dispatch feature: wildcard
 * ("default") sub-interfaces have no concrete VLAN in their configuration. */
static void
pppoeclient_update_tx_l2_template (pppoeclient_t *c)
{
  pppoeclient_main_t *pem = &pppoeclient_main;
  vnet_hw_interface_t *hw = vnet_get_hw_interface (pem->vnet_main, c->hw_if_index);
  ethernet_header_t *eth = (ethernet_header_t *) c->tx_l2_template;
  u8 valid, n_tags, dot1ad;
  u16 outer_vlan_id, inner_vlan_id;
  u16 l2_len = sizeof (ethernet_header_t);

  clib_memset (c->tx_l2_template, 0, sizeof (c->tx_l2_template));
  clib_memcpy (eth->dst_address, c->ac_mac_address, sizeof (eth->dst_address));
  if (hw)
    clib_memcpy (eth->src_address, hw->hw_address, sizeof (eth->src_address));

  pppoeclient_padt_vlan_from_client (c, &valid, &n_tags, &dot1ad, &outer_vlan_id, &inner_vlan_id);
  if (!valid)
    n_tags = 0;

  if (n_tags == 2)
    {
      ethernet_vlan_header_t *outer = (ethernet_vlan_header_t *) (eth + 1);
      ethernet_vlan_header_t *inner = outer + 1;

      eth->type = clib_host_to_net_u16 (dot1ad ? ETHERNET_TYPE_DOT1AD : ETHERNET_TYPE_VLAN);
      outer->priority_cfi_and_id = clib_host_to_net_u16 (outer_vlan_id & 0xfff);
      outer->type = clib_host_to_net_u16 (ETHERNET_TYPE_VLAN);
      inner->priority_cfi_and_id = clib_host_to_net_u16 (inner_vlan_id & 0xfff);
      inner->type = clib_host_to_net_u16 (ETHERNET_TYPE_PPPOE_SESSION);
  ...
  else if (n_tags == 1)
    {
      ...
      eth->type = clib_host_to_net_u16 (dot1ad ? ETHERNET_TYPE_DOT1AD : ETHERNET_TYPE_VLAN);
      vlan->priority_cfi_and_id = clib_host_to_net_u16 (outer_vlan_id & 0xfff);
...
pppoeclient_save_session_to_file (pppoeclient_t *c)
{
  ...
  u8 src_mac[6];
  u8 vlan_valid = 0, vlan_n_tags = 0, vlan_dot1ad = 0;
  u16 vlan_outer = 0, vlan_inner = 0;
  u16 vlan_outer_net, vlan_inner_net;
  /* v1 layout: session_id (2B) + ac_mac (6B) + src_mac (6B) + linux_ifname
   * (16B) = 30B.  v2 appends the session's VLAN tag stack (8B) so the
   * shutdown raw PADT can be emitted with the correct tags; readers accept
   * both lengths for backward compatibility. */
  u8 buf[sizeof (u16) + 6 + 6 + IFNAMSIZ + 8];
  size_t off;
  int fd;

  pppoeclient_session_file_path (c->sw_if_index, c->host_uniq, path, sizeof (path));
  ...
  pppoeclient_padt_vlan_from_client (c, &vlan_valid, &vlan_n_tags, &vlan_dot1ad, &vlan_outer,
				     &vlan_inner);
  if (!vlan_valid)
    vlib_log (VLIB_LOG_LEVEL_WARNING, pppoeclient_log_class,
	      "save_session: wildcard sub-interface has no learned VLAN; "
	      "shutdown PADT will be untagged");
  vlan_outer_net = clib_host_to_net_u16 (vlan_outer);
  vlan_inner_net = clib_host_to_net_u16 (vlan_inner);
  buf[off++] = vlan_valid;
  buf[off++] = vlan_n_tags;
  buf[off++] = vlan_dot1ad;
  buf[off++] = 0; /* reserved */
  clib_memcpy (buf + off, &vlan_outer_net, sizeof (vlan_outer_net));
  off += sizeof (vlan_outer_net);
  clib_memcpy (buf + off, &vlan_inner_net, sizeof (vlan_inner_net));
  off += sizeof (vlan_inner_net);
  ...
  vlib_log (VLIB_LOG_LEVEL_INFO, pppoeclient_log_class,
	    ...
	    "vlan-valid=%u n-tags=%u outer=%u inner=%u",
	    path, c->session_id, linux_ifname, src_mac[0], src_mac[1], src_mac[2], src_mac[3],
	    src_mac[4], src_mac[5], c->ac_mac_address[0], c->ac_mac_address[1],
	    c->ac_mac_address[2], c->ac_mac_address[3], c->ac_mac_address[4], c->ac_mac_address[5],
	    vlan_valid, vlan_n_tags, vlan_outer, vlan_inner);
}

static void
pppoeclient_delete_session_file (u32 sw_if_index, u32 host_uniq)
{
  char path[128];
  pppoeclient_session_file_path (sw_if_index, host_uniq, path, sizeof (path));
  unlink (path);
}
...
/* Lower the PPP/LCP state machine for a session that is going away.  Without
 * this the LCP echo timer keeps running after teardown, and a later echo
 * timeout drives cleanup_callback() into an unwanted automatic redial.
 *
 * NB: this takes pppox's ctrl_lock.  Do NOT call it from
 * pppoeclient_teardown_session(): teardown is reachable from pppox
 * callbacks (cleanup_callback -> restart_session_with_reason) that already
 * hold that non-recursive lock, which self-deadlocks.  Only call it from
 * explicit, non-reentrant admin paths such as stop_session(). */
static void
pppoeclient_lower_ppp (pppoeclient_t *c)
{
...
pppoeclient_schedule_discovery (pppoeclient_t *c, f64 next_transmit)
{
  ...
  c->retry_count = 0;
  /* MT variant: this function is also reachable from worker threads via
   * the PADT handling path. */
  vlib_process_signal_event_mt (pem->vlib_main, pppoeclient_process_node.index,
			    ...
			    const ip6_address_t *ip6_peer_addr, u8 prefix_len)
{
  ...
  /* Update the interface link-local address so that DHCPv6 and other IPv6
   * subsystems pick up the address negotiated by IPv6CP instead of the
   * auto-generated one that ip6_link_enable() created. */
  if (ip6_addr && !ip6_address_is_zero (ip6_addr))
    {
      ip6_link_set_local_address (pppox_sw_if_index, ip6_addr);
    }
}
 ...
 * physical interface, bypassing the pppoeclient-session-output path which
 * does not work reliably from the main thread (process node context).
 *
...
pppoeclient_sm (f64 now, f64 timeout, uword pool_index)
{
  ...
  /* Administratively stopped clients stay parked until an explicit
   * open/restart clears the flag; never retransmit PADI/PADR for them. */
  if (c->admin_stopped)
    return timeout;
...
/* Bytes actually available after the PPPoE header.  Callers may hand us a
 * buffer whose current_data was reset back to the L2 header (or advanced to
 * the PPPoE header), so derive the span from the pppoe pointer itself
 * instead of assuming b->current_length starts at the PPPoE header. */
static_always_inline u32
pppoeclient_pppoe_available_len (vlib_buffer_t *b, const pppoe_header_t *pppoe)
{
  const u8 *cur = vlib_buffer_get_current (b);
  const u8 *p = (const u8 *) pppoe;
  uword offset;

  if (p < cur)
    return 0;
  offset = (uword) (p - cur);
  if (offset + sizeof (*pppoe) > b->current_length)
    return 0;
  return b->current_length - (u32) offset - sizeof (*pppoe);
}

int
parse_pppoe_packet (pppoe_header_t *pppoe, u32 available_len, parse_func *func, void *extra)
{
  ...
  /* Never parse past the bytes that are really in the buffer: a peer can
   * declare a length far larger than the frame it sent. */
  if (len > ETH_JUMBO_LEN - sizeof (pppoe_header_t) || (u32) len > available_len)
    {
      return -1;
    }
					      ...
					      vlib_buffer_t *b)
{
  ...
  pppoeclient_control_event_capture_raw_tags (&event_summary, pppoe,
					      pppoeclient_pppoe_available_len (b, pppoe));
						   ...
						   ethernet_header_t **eth_hdr)
{
  ...
  pppoeclient_control_event_capture_raw_tags (event_summary, pppoe,
					      pppoeclient_pppoe_available_len (b, pppoe));
  parse_result = parse_pppoe_packet (pppoe, pppoeclient_pppoe_available_len (b, pppoe),
				     parse_control_event_tags, event_summary);
  vlib_buffer_reset (b);
  *eth_hdr = vlib_buffer_get_current (b);
...
consume_pppoe_discovery_pkt (u32 bi, vlib_buffer_t *b, pppoe_header_t *pppoe)
{
	  ...
	  if (host_uniq == 0)
	    {
	      ...
	      clib_memset (&event_summary, 0, sizeof (event_summary));
	      pppoeclient_control_event_capture_raw_tags (
		&event_summary, pppoe, pppoeclient_pppoe_available_len (b, pppoe));
	      parse_result = parse_pppoe_packet (pppoe, pppoeclient_pppoe_available_len (b, pppoe),
						 parse_control_event_tags, &event_summary);
	      if (parse_result < 0)
		{
  ...
  switch (c->state)
    {
    case PPPOE_CLIENT_DISCOVERY:
      /* A stopped client ignores discovery packets until an explicit
       * open/restart; otherwise a late PADO would clear the park and redial
	...
	break;
      ...
      if (packet_code != PPPOE_PADO)
	{
	  /* Never postpone an already-future deadline: a spoofed
	   * PADS/PADT stream would otherwise keep re-arming the cooldown
	   * and starve the dialer indefinitely. */
	  if (c->next_transmit <= now)
	...
	return PPPOECLIENT_ERROR_MALFORMED_TAGS;
      parse_result =
	parse_pppoe_packet (pppoe, pppoeclient_pppoe_available_len (b, pppoe), parse_pado_tags, c);
      if (parse_result < 0)
	return PPPOECLIENT_ERROR_MALFORMED_TAGS;
      ...
      if (pppox_lower_up_func == 0)
	{
	  ...
	  c->state = PPPOE_CLIENT_DISCOVERY;
	  c->retry_count = 0;
	  /* Apply the rediscovery cooldown instead of re-dialing instantly:
	   * a permanently missing plugin would otherwise hot-loop
	   * PADI->PADO->PADR->PADS->rollback forever. */
	  c->next_transmit = now + PPPOECLIENT_REDISCOVERY_COOLDOWN;
	  client_id = c - pem->clients;
	  pppoeclient_wakeup (client_id);
...
pppoeclient_get_detail_dhcp6_ia_na (u32 sw_if_index, dhcp6_ia_na_client_runtime_t *rt)
{
  ...
  /* The dhcp plugin accessor returns 0 on success and 1 when the client is
   * not enabled.  The caller only needs to know whether a snapshot was
   * obtained; rt->enabled carries the enable state. */
  (void) (*dhcp6_ia_na_client_get_runtime_func) (sw_if_index, rt);
  return 1;
}
				 ...
				 dhcp6_pd_active_prefix_runtime_t *prefix_rt)
{
  ...
  if (dhcp6_pd_client_get_runtime_func)
    {
      /* Accessor returns 0 on success; availability here only means the
       * snapshot call was made.  rt->enabled reports the client state. */
      (void) (*dhcp6_pd_client_get_runtime_func) (sw_if_index, rt);
      have_runtime = 1;
    }
...
pppoeclient_get_detail_dhcp6_pd_consumer (u32 sw_if_index, dhcp6_pd_consumer_runtime_t *rt)
{
  ...
  /* As above: 0 from the accessor means the snapshot is valid; report
   * availability via the symbol lookup, presence via rt->present. */
  (void) (*dhcp6_pd_client_get_consumer_runtime_func) (sw_if_index, rt);
  return 1;
}
			    ...
			    pppoeclient_history_cli_filter_t *filter)
{
  ...
  vlib_cli_output (vm, "    vlan configured n-tags %u dot1ad %u outer %u inner %u any %u/%u",
		   c->vlan_match.n_tags, c->vlan_match.dot1ad, c->vlan_match.outer_vlan_id,
		   c->vlan_match.inner_vlan_id, c->vlan_match.outer_any, c->vlan_match.inner_any);
  vlib_cli_output (vm, "    vlan session %s n-tags %u dot1ad %u outer %u inner %u",
		   c->session_vlan.valid ? "learned" : "unlearned", c->session_vlan.n_tags,
		   c->session_vlan.dot1ad, c->session_vlan.outer_vlan_id,
		   c->session_vlan.inner_vlan_id);
...
pppoeclient_open_session (u32 client_index)
{
  ...
  /* An explicit open always clears a previous administrative stop. */
  c->admin_stopped = 0;
...
pppoeclient_restart_session_with_reason (u32 client_index, u8 disconnect_reason)
{
  ...
  /* Automatic redials (PPP death, auth failure) must not resurrect a client
   * that the operator explicitly stopped.  An explicit restart carries the
   * ADMIN reason and clears the stop before proceeding. */
  if (c->admin_stopped && disconnect_reason != PPPOECLIENT_DISCONNECT_ADMIN)
    return;
  c->admin_stopped = 0;
...
static int
pppoeclient_add_del_internal (vnet_pppoeclient_add_del_args_t *a, u32 *pppox_sw_if_index)
{
  pppoeclient_main_t *pem = &pppoeclient_main;
  pppoeclient_t *c = 0;
  vlib_main_t *vm = pem->vlib_main;
  ...
  if (a->is_add)
	...
	return VNET_API_ERROR_TUNNEL_EXIST;
      ...
      /* host-uniq 0 is indistinguishable from "tag absent" in the matching
       * code, so a client configured with 0 can never be matched when more
       * than one client shares the access interface. */
      if (a->host_uniq == 0)
...
#undef _
      ...
      if (sw == 0)
	{
      ...
      /* The device-input feature is installed on the physical port.  For a
       * sub-interface client, record the parent plus the VLAN
       * classification the dispatch node must match. */
      if (sw->type == VNET_SW_INTERFACE_TYPE_SUB)
	{
	  vnet_sw_interface_t *sup_sw = vnet_get_sw_interface_or_null (vnm, sw->sup_sw_if_index);
	  if (sup_sw == 0 || sup_sw->type != VNET_SW_INTERFACE_TYPE_HARDWARE)
	    {
	      pppoeclient_client_free_resources (c);
	      pool_put (pem->clients, c);
	      return VNET_API_ERROR_INVALID_INTERFACE;
	    }
	  c->sup_sw_if_index = sw->sup_sw_if_index;
	  c->hw_if_index = sup_sw->hw_if_index;
	  c->vlan_match.n_tags =
	    sw->sub.eth.flags.two_tags ? 2 : (sw->sub.eth.flags.one_tag ? 1 : 0);
	  c->vlan_match.dot1ad = sw->sub.eth.flags.dot1ad;
	  c->vlan_match.outer_vlan_id = sw->sub.eth.outer_vlan_id;
	  c->vlan_match.inner_vlan_id = sw->sub.eth.inner_vlan_id;
	  /* A default (non exact-match) sub-interface matches any VLAN not
	   * claimed by a more specific one. */
	  c->vlan_match.outer_any =
	    sw->sub.eth.flags.outer_vlan_id_any || !sw->sub.eth.flags.exact_match;
	  c->vlan_match.inner_any =
	    sw->sub.eth.flags.inner_vlan_id_any || !sw->sub.eth.flags.exact_match;
	}
      else if (sw->type == VNET_SW_INTERFACE_TYPE_HARDWARE)
	{
	  c->sup_sw_if_index = a->sw_if_index;
	  c->hw_if_index = sw->hw_if_index;
	  c->vlan_match.n_tags = 0;
	}
      else
	{
	  pppoeclient_client_free_resources (c);
      ...
      {
	vnet_hw_interface_t *hw = vnet_get_hw_interface_or_null (vnm, c->hw_if_index);
	if (hw == 0)
	  {
	    pppoeclient_client_free_resources (c);
	    pool_put (pem->clients, c);
	...
	pppox_set_interface_name (*pppox_sw_if_index, a->custom_ifname);
      ...
      pppoeclient_dispatch_ref (pem, c->sup_sw_if_index);
      pppoeclient_cache_l2_encap_len (pem, a->sw_if_index);
      pppoeclient_sup_client_list_add (pem, c->sup_sw_if_index, result.fields.client_index);
	...
	if (~0 == edge)
      ...
      /* Wake the process node: a bare 'create pppoe client' should start
       * dialing immediately instead of waiting for the idle-timeout wakeup
       * (up to 100 s). */
      pppoeclient_wakeup (result.fields.client_index);
    }
  else
    {
	...
	return VNET_API_ERROR_NO_SUCH_ENTRY;
      ...
      /* Send an LCP Terminate-Request while the session is still
       * SESSION-state and the pppox mapping is intact, then tear the
       * session down (PADT + park) and free the pppox interface.
      ...
      static void (*pppox_lcp_terminate_func) (u32) = 0;
	...
	return VNET_API_ERROR_UNSUPPORTED;
      PPPOECLIENT_LAZY_PLUGIN_SYMBOL (pppox_lcp_terminate_func, "pppoeclient_plugin.so",
				      "pppox_lcp_terminate");

      if (pppox_lcp_terminate_func)
	(*pppox_lcp_terminate_func) (c->pppox_sw_if_index);

      pppoeclient_stop_session (result.fields.client_index);

      (*pppox_free_interface_func) (c->pppox_hw_if_index);

      pppoeclient_sup_client_list_del (pem, c->sup_sw_if_index, result.fields.client_index);
      pppoeclient_dispatch_unref (pem, c->sup_sw_if_index);
      pppoeclient_delete_session_file (c->sw_if_index, c->host_uniq);

      /* dispatch is refcounted per access interface. */

      pppoeclient_delete_1 (&pem->client_table, a->sw_if_index, a->host_uniq);

      /* Workers dereference pem->clients (session-input/session-output) and
       * client_index_by_pppox_sw_if_index (pppoeclient_get_client_by_pppox_sw_if_index)
       * after bihash lookups.  Barrier-sync so no worker can still hold the
       * freed pool entry or a stale mapping vector pointer. */
...
vnet_pppoeclient_add_del (vnet_pppoeclient_add_del_args_t *a, u32 *pppox_sw_if_index)
{
  ...
  /* The client pool, the bihash tables and the per-super-interface client
   * list are read by worker data paths.  Hold the barrier across the whole
   * add/del so a worker can never observe a half-published client, a freed
   * pool slot or a reallocated vector.  Recursive barriers are tolerated,
   * which matters because the interface add/del callback below re-enters
   * this function. */
  vlib_worker_thread_barrier_sync (vm);
  rv = pppoeclient_add_del_internal (a, pppox_sw_if_index);
  vlib_worker_thread_barrier_release (vm);

  return rv;
}

/* An access interface (physical port or one of its sub-interfaces) is going
 * away: delete every client configured on it before the sw_if_index is
 * recycled.  Without this, discovery retransmits keep dereferencing the
 * freed interface (debug ASSERT, release stale-slot use). */
static clib_error_t *
pppoeclient_sw_interface_add_del (vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
  pppoeclient_main_t *pem = &pppoeclient_main;
  u32 *victims = 0;
  pppoeclient_t *c;
  ...
  if (is_add || pem->vlib_main == 0)
    return 0;

  pool_foreach (c, pem->clients)
    {
      if (c->sw_if_index == sw_if_index || c->sup_sw_if_index == sw_if_index)
	vec_add1 (victims, (u32) (c - pem->clients));
    }

  if (vec_len (victims) == 0)
    return 0;

  /* If the physical port itself is being deleted, drop its dispatch
   * refcount now: the per-client unref below must not try to reprogram a
   * feature arc on an interface that is about to disappear. */
  if (sw_if_index < vec_len (pem->dispatch_refcount_by_sw_if_index))
    pem->dispatch_refcount_by_sw_if_index[sw_if_index] = 0;

  for (i = 0; i < vec_len (victims); i++)
    {
      pppoeclient_t *victim = pool_elt_at_index (pem->clients, victims[i]);
      vnet_pppoeclient_add_del_args_t a;
      u32 pppox_sw_if_index = ~0;

      clib_memset (&a, 0, sizeof (a));
      a.is_add = 0;
      a.sw_if_index = victim->sw_if_index;
      a.host_uniq = victim->host_uniq;
      (void) vnet_pppoeclient_add_del (&a, &pppox_sw_if_index);
    }
...
VNET_SW_INTERFACE_ADD_DEL_FUNCTION (pppoeclient_sw_interface_add_del);
...
set_pppoeclient_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  /* PPP MTU/MRU are 16-bit on the wire (and MRU is truncated by
   * pppox_set_configured_mru), so reject out-of-range values up front. */
  if ((mtu > 0 && (mtu < 576 || mtu > 0xFFFF)) || (mru > 0 && (mru < 576 || mru > 0xFFFF)))
    {
      ...
      vec_free (username);
      vec_free (password);
      return clib_error_return (0, "mtu/mru must be in [576, 65535]");
    }
  ...
  if (timeout > 0 && timeout > 86400)
    {
      ...
      vec_free (username);
      vec_free (password);
      return clib_error_return (0, "timeout must be in [1, 86400] seconds");
    }
...
pppoeclient_init (vlib_main_t *vm)
{
  ...
  pem->discovery_fq_index = ~0;
  {
    vlib_node_t *dn = vlib_get_node_by_name (vm, (u8 *) "pppoeclient-discovery-input");
    if (dn)
      pem->discovery_fq_index = vlib_handoff_alloc_queues (&(vlib_handoff_alloc_queues_args_t) {
	.node_index = dn->index,
	.queue_size = 1024,
      });
    else
      clib_warning ("pppoeclient: discovery-input node not found");
  }
...
pppoeclient_send_padt_raw (const char *linux_ifname, const u8 *src_mac, const u8 *dst_mac,
			   u16 session_id, u8 vlan_valid, u8 n_tags, u8 dot1ad, u16 outer_vlan_id,
			   u16 inner_vlan_id)
{
  int fd;
  ...
  /* Build PADT: Ethernet addresses, optional 802.1Q/802.1ad tag stack, then
   * the 6-byte PPPoE header.  The tag stack is required when the client is on
   * a VLAN sub-interface: an untagged PADT would leave the wrong VLAN and the
   * BAS would keep the session. */
  clib_memcpy (frame + 0, dst_mac, 6);
  clib_memcpy (frame + 6, src_mac, 6);
  off = 12;
  if (vlan_valid && n_tags > 0)
    {
      frame[off++] = dot1ad ? 0x88 : 0x81;
      frame[off++] = dot1ad ? 0xa8 : 0x00;
      tci = clib_host_to_net_u16 (outer_vlan_id & 0xfff);
      clib_memcpy (frame + off, &tci, sizeof (tci));
      off += sizeof (tci);
      if (n_tags > 1)
	{
	  frame[off++] = 0x81;
	  frame[off++] = 0x00; /* inner tag is always 802.1Q */
	  tci = clib_host_to_net_u16 (inner_vlan_id & 0xfff);
	  clib_memcpy (frame + off, &tci, sizeof (tci));
	  off += sizeof (tci);
	}
    }
  frame[off++] = 0x88;
  frame[off++] = 0x63; /* EtherType PPPoE Discovery */
  frame[off++] = 0x11; /* ver=1 type=1 */
  frame[off++] = 0xa7; /* code = PADT */
  frame[off++] = (session_id >> 8) & 0xff;
  frame[off++] = session_id & 0xff;
  frame[off++] = 0x00;
  frame[off++] = 0x00; /* payload length = 0 */
  ...
  if (sendto (fd, frame, off, 0, (struct sockaddr *) &addr, sizeof (addr)) < 0)
    vlib_log (VLIB_LOG_LEVEL_ERR, pppoeclient_log_class, "raw PADT sendto failed: %s",
	      strerror (errno));
  else
    vlib_log (VLIB_LOG_LEVEL_INFO, pppoeclient_log_class,
	      "sent PADT via raw socket on %s session %u vlan-valid %u n-tags %u outer %u "
	      "inner %u",
	      linux_ifname, session_id, vlan_valid, n_tags, outer_vlan_id, inner_vlan_id);
...
pppoeclient_exit (vlib_main_t *vm)
{
  ...
  pool_foreach (c, pem->clients)
    {
      ...
      u16 file_sid = 0;
      u8 file_vlan_valid = 0, file_vlan_n_tags = 0, file_vlan_dot1ad = 0;
      u16 file_vlan_outer = 0, file_vlan_inner = 0;
      int fd, ok = 0;
      ...
      if (fd >= 0)
	{
	  ...
	  /* Accept both the legacy 30-byte record and the current 38-byte
	   * record with the trailing VLAN tag stack. */
	  if (got == sizeof (buf) || got == sizeof (buf) - 8)
	    {
	      size_t off = 0;
		...
		{
		  u16 vlan_tmp;
		  file_vlan_valid = buf[off++];
		  file_vlan_n_tags = buf[off++];
		  file_vlan_dot1ad = buf[off++];
		  off++; /* reserved */
		  clib_memcpy (&vlan_tmp, buf + off, sizeof (vlan_tmp));
		  off += sizeof (vlan_tmp);
		  file_vlan_outer = clib_net_to_host_u16 (vlan_tmp);
		  clib_memcpy (&vlan_tmp, buf + off, sizeof (vlan_tmp));
		  off += sizeof (vlan_tmp);
		  file_vlan_inner = clib_net_to_host_u16 (vlan_tmp);
		}
	      ok = (linux_ifname[0] != '\0');
	    }
	  vlib_log (VLIB_LOG_LEVEL_DEBUG, pppoeclient_log_class,
		    "exit: session file read ok=%d got=%zu ifname='%s' "
		    "file_sid=%u vlan-valid=%u n-tags=%u outer=%u inner=%u",
		    ok, got, ok ? linux_ifname : "(empty)", file_sid, file_vlan_valid,
		    file_vlan_n_tags, file_vlan_outer, file_vlan_inner);
	}
      else
	{
	  if (c->session_id)
      ...
      if (ok)
	{
	  u8 vlan_valid = file_vlan_valid, vlan_n_tags = file_vlan_n_tags;
	  u8 vlan_dot1ad = file_vlan_dot1ad;
	  u16 vlan_outer = file_vlan_outer, vlan_inner = file_vlan_inner;

	  /* Legacy file (or a wildcard sub-interface recorded before the tag
	   * was learned): fall back to the in-memory client state. */
	  if (!vlan_valid)
	    pppoeclient_padt_vlan_from_client (c, &vlan_valid, &vlan_n_tags, &vlan_dot1ad,
					       &vlan_outer, &vlan_inner);
	  /* Use file_sid from session file (not c->session_id which may be 0) */
	  pppoeclient_send_padt_raw (linux_ifname, file_src_mac, file_ac_mac, file_sid, vlan_valid,
				     vlan_n_tags, vlan_dot1ad, vlan_outer, vlan_inner);
	}
      else
	{
	  ...
	  if (sid)
						...
						sizeof (linux_ifname), 1) == 0)
		{
		  ...
		  pppoeclient_send_padt_raw (linux_ifname, src_mac, c->ac_mac_address, sid,
					     vlan_valid, vlan_n_tags, vlan_dot1ad, vlan_outer,
					     vlan_inner);
		}
	      else
		vlib_log (VLIB_LOG_LEVEL_ERR, pppoeclient_log_class,
  ...
  if (pem->client_indices_by_sup_sw_if_index)
    {
      u32 i;
      vec_foreach_index (i, pem->client_indices_by_sup_sw_if_index)
	vec_free (pem->client_indices_by_sup_sw_if_index[i]);
      vec_free (pem->client_indices_by_sup_sw_if_index);
    }
