...
show_pppoeclient_orphan_control_history (vlib_main_t *vm)
{
  ...
  vlib_cli_output (vm, "orphan-control");
  for (i = 0; i < pem->orphan_control_history_count; i++)
    {
      u8 index = (pem->orphan_control_history_next + PPPOECLIENT_CONTROL_HISTORY_LEN -
		  pem->orphan_control_history_count + i) %
		 PPPOECLIENT_CONTROL_HISTORY_LEN;
      pppoeclient_control_event_t *event = &pem->orphan_control_history[index];
      u8 *ac_name = 0;
	...
	ac_name = format (0, "<none>");
      ...
      vlib_cli_output (
	vm,
	"  age %.2fs %U sw-if-index %u peer %U session-id %u host-uniq %u ac-name %v "
	"cookie-len %u error-tag 0x%04x parse-error %u",
	now - event->event_time, format_pppoe_packet_code_name, event->code, event->sw_if_index,
	format_ethernet_address, event->peer_mac, event->session_id, event->host_uniq_present,
	ac_name, event->cookie_len, event->error_tag_type, event->parse_error);
      vec_free (ac_name);
    }
}
      ...
      else
	{
	  ...
	  if (sid)
						...
						sizeof (linux_ifname), 1) == 0)
		{
		  ...
		  if (hw)
		    pppoeclient_send_padt_raw (linux_ifname, hw->hw_address, c->ac_mac_address,
					       sid);
		}
	      else
