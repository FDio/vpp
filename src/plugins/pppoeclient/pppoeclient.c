...
pppoeclient_clear_runtime_state (pppoeclient_t *c)
{
  ...
  clib_memset (c->cached_src_mac, 0, sizeof (c->cached_src_mac));
  c->cached_hw_output_next_index = 0;
  c->cached_link_up = 0;
  c->cached_l2_encap_len = 0;
  c->cached_mtu = 0;
  c->cached_outer_vlan_id = 0;
  c->cached_inner_vlan_id = 0;
  c->cached_one_tag = 0;
  c->cached_two_tags = 0;
  c->discovery_error = 0;
}
...
consume_pppoe_discovery_pkt (u32 bi, vlib_buffer_t *b, pppoe_header_t *pppoe)
{
  ...
  switch (c->state)
      ...
      if (PREDICT_FALSE (result.fields.client_index != ~0))
	{
      ...
      /* Cache data-plane fields to avoid per-packet interface lookups. */
      {
	vnet_main_t *_vnm = pem->vnet_main;
	vnet_hw_interface_t *hw = vnet_get_sup_hw_interface (_vnm, c->sw_if_index);
	vnet_sw_interface_t *sup_sw = vnet_get_sup_sw_interface (_vnm, c->sw_if_index);
	vnet_sw_interface_t *sw = vnet_get_sw_interface (_vnm, c->sw_if_index);
	clib_memcpy (c->cached_src_mac, hw->hw_address, 6);
	c->cached_hw_output_next_index = hw->output_node_index;
	c->cached_link_up = (hw->flags & VNET_HW_INTERFACE_FLAG_LINK_UP) &&
			    (sup_sw->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) &&
			    (sw->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP);
	c->cached_l2_encap_len = pppoeclient_get_l2_encap_len (_vnm, c->sw_if_index);
	c->cached_mtu = sw->mtu[VNET_MTU_L3] > 0 ? sw->mtu[VNET_MTU_L3] : PPPOECLIENT_MTU_DEFAULT;
	c->cached_outer_vlan_id = sw->sub.eth.outer_vlan_id;
	c->cached_inner_vlan_id = sw->sub.eth.inner_vlan_id;
	c->cached_one_tag = sw->type == VNET_SW_INTERFACE_TYPE_SUB && sw->sub.eth.flags.one_tag;
	c->cached_two_tags = sw->type == VNET_SW_INTERFACE_TYPE_SUB && sw->sub.eth.flags.two_tags;
      }
