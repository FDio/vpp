				  ...
				  pppoeclient_control_match_reason_t *reason)
{
  ...
  u8 cookie_matches =
    (summary->cookie_len > 0 && !summary->cookie_value_truncated &&
     c->cookie.type == clib_host_to_net_u16 (PPPOE_TAG_AC_COOKIE) &&
     clib_net_to_host_u16 (c->cookie.length) == summary->cookie_len &&
     clib_memcmp (c->cookie.value, summary->cookie_value, summary->cookie_len) == 0);
...
pppoeclient_save_session_to_file (pppoe_client_t *c)
{
  ...
  if (write (fd, &sid, sizeof (sid)) < 0 || write (fd, c->ac_mac_address, 6) < 0 ||
      write (fd, src_mac, 6) < 0 || write (fd, linux_ifname, IFNAMSIZ) < 0)
...
sync_pppoe_client_live_auth (pppoe_client_t *c)
{
  ...
  if (pppox_set_auth_func == 0)
    {
      pppox_set_auth_func = vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppox_set_auth");
    }
...
sync_pppoe_client_live_default_route4 (pppoe_client_t *c)
{
  ...
  if (pppox_set_add_default_route4_func == 0)
	...
	vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppox_set_add_default_route4");
    }
...
sync_pppoe_client_live_default_route6 (pppoe_client_t *c)
{
  ...
  if (pppox_set_add_default_route6_func == 0)
	...
	vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppox_set_add_default_route6");
    }
...
sync_pppoe_client_live_use_peer_dns (pppoe_client_t *c)
{
  ...
  if (pppox_set_use_peer_dns_func == 0)
	...
	vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppox_set_use_peer_dns");
    }
...
{
  clib_memset (history, 0, PPPOECLIENT_CONTROL_HISTORY_LEN * sizeof (pppoeclient_control_event_t));
  *count = 0;
  *next = 0;
}
					      ...
					      vlib_buffer_t *b)
{
  ...
  if (packet_code == PPPOE_PADS)
    malformed_client = pppoe_client_find_unique_candidate_by_ac_mac (
      pem, sw_if_index, expected_state, eth_hdr->src_address);

  if (malformed_client == 0)
    malformed_client = pppoe_client_find_unique_candidate (pem, sw_if_index, expected_state);
...
consume_pppoe_discovery_pkt (u32 bi, vlib_buffer_t *b, pppoe_header_t *pppoe)
{
  ...
  switch (c->state)
	...
	{
	  pppox_lower_up_func = vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppox_lower_up");
	}
      if (pppox_lower_up_func == 0)
	{
	...
	{
	  pppox_lower_down_func =
	    vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppox_lower_down");
	}
...
static void
show_pppoeclient_debug_filtered_one (vlib_main_t *vm, pppoe_client_t *c,
				     pppoeclient_history_cli_filter_t *filter)
{
  pppoeclient_main_t *pem = &pppoeclient_main;
  u32 client_index = c - pem->clients;
...
pppoeclient_auth_backoff_delay (u32 failures)
{
  ...
  if (delay > PPPOECLIENT_AUTH_BACKOFF_CAP_SEC || failures > 16)
...
#undef _
      ...
      if (pppox_allocate_interface_func == 0)
	{
	  pppox_allocate_interface_func =
	    vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppox_allocate_interface");
	}
      if (pppox_allocate_interface_func == 0)
	{
	  pppoeclient_client_free_resources (c);
      ...
      if (pppox_free_interface_func == 0)
	{
	  pppox_free_interface_func =
	    vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppox_free_interface");
	}
      if (pppox_free_interface_func == 0)
	return VNET_API_ERROR_UNSUPPORTED;
...
VLIB_CLI_COMMAND (clear_pppoeclient_backoff_command, static) = {
  ...
  .short_help = "clear pppoe client backoff {<client-index>|<interface> | all}",
				   ...
				   vlib_cli_command_t *cmd)
{
  ...
  if (client_index != ~0)
	...
	show_pppoeclient_debug_filtered_one (vm, pool_elt_at_index (pem->clients, client_index),
					     &filter);
      else
	show_pppoeclient_debug_one (vm, pool_elt_at_index (pem->clients, client_index));
      return 0;
    }
  ...
  pool_foreach (t, pem->clients)
    {
      if (pppoeclient_history_cli_filter_active (&filter))
	show_pppoeclient_debug_filtered_one (vm, t, &filter);
      else
			   ...
			   u16 session_id)
{
  ...
  snprintf (ifr.ifr_name, IFNAMSIZ, "%s", linux_ifname);
  ...
  int was_down = 0;
  {
	...
	snprintf (up_ifr.ifr_name, IFNAMSIZ, "%s", linux_ifname);
  ...
  if (was_down)
      ...
      if (ctl_fd >= 0)
	{
	  ...
	  snprintf (down_ifr.ifr_name, IFNAMSIZ, "%s", linux_ifname);
			      ...
			      u8 noisy)
{
   ...
   * doesn't modify the MAC. */
  {
	...
	struct ifreq ifr;
	clib_memset (&ifr, 0, sizeof (ifr));
	snprintf (ifr.ifr_name, IFNAMSIZ, "%s", p->if_name);
  ...
  pool_foreach (c, pem->clients)
      ...
      if (fd >= 0)
	{
	  ...
	  ok = (read (fd, &file_sid, sizeof (file_sid)) == sizeof (file_sid) &&
		read (fd, file_ac_mac, 6) == 6 && read (fd, file_src_mac, 6) == 6 &&
		read (fd, linux_ifname, IFNAMSIZ) == IFNAMSIZ && linux_ifname[0] != '\0');
	  close (fd);
	  clib_warning ("pppoeclient_exit: session file read ok=%d ifname='%s' file_sid=%u", ok,
			ok ? linux_ifname : "(empty)", file_sid);
	}
      else
	{
