...
pppoe_client_teardown_session (pppoe_client_t *c, u8 send_padt)
{
  ...
  if (c->session_id)
	...
	send_pppoe_pkt (pem, c, PPPOE_PADT, c->session_id, 0 /* is_broadcast */);
      pppoeclient_delete_session_1 (&pem->session_table, c->sw_if_index, c->ac_mac_address, c->session_id);
      c->session_id = 0;
    }
  ...
  switch (c->state)
      ...
      if (packet_code != PPPOE_PADO)
	{
	  ...
	  ((c->ac_name == 0) || (vec_len (c->ac_name_filter) != vec_len (c->ac_name)) ||
	   memcmp (c->ac_name_filter, c->ac_name, vec_len (c->ac_name_filter)) != 0))
	{
	  break;
	}
...
{
  static u8 (*dhcp6_pd_client_get_consumer_runtime_func) (u32,
							  dhcp6_pd_consumer_runtime_t *) = 0;
  static u8 attempted = 0;
...
format_ppp_phase_name (u8 *s, va_list *args)
{
  static const char *phase_names[] = {
    "DEAD",	   "INITIALIZE", "SERIALCONN", "DORMANT",  "ESTABLISH",
    "AUTHENTICATE", "CALLBACK",   "NETWORK",    "RUNNING",  "TERMINATE",
    "DISCONNECT",   "HOLDOFF",    "MASTER",
  };
...
show_pppoeclient_detail_one (vlib_main_t *vm, pppoe_client_t *c)
{
  ...
  if (t)
	...
	pppoe_client_get_detail_global_ipv6 (c->pppox_sw_if_index, &observed_local_ip6, &observed_prefix_len);
	...
	wan_ipv6_mode = "link-local-only";
      ...
      vlib_cli_output (vm, "    runtime pppox-interface %U sw-if-index %u unit %u session-allocated %u",
	...
	vlib_cli_output (vm, "    runtime peer-dns6 <none>");
      ...
      if (observed_local_present)
	vlib_cli_output (vm, "    runtime wan-ipv6 observed %U/%u", format_ip6_address, &observed_local_ip6,
			 observed_prefix_len);
      else
	vlib_cli_output (vm, "    runtime wan-ipv6 observed <none>");
      vlib_cli_output (vm,
		       "    runtime wan-ipv6-mode %s peer-host-route %u default-route4 %u default-route6 %u",
    ...
    {
      vlib_cli_output (vm, "    runtime pppox-interface sw-if-index %u unit unavailable", c->pppox_sw_if_index);
      vlib_cli_output (vm, "    runtime peer-dns4 primary <none>");
      vlib_cli_output (vm, "    runtime peer-dns4 secondary <none>");
      vlib_cli_output (vm, "    runtime peer-dns6 <none>");
    }
  ...
  if (dhcp6_ia_na_available)
      ...
      if (dhcp6_ia_na_rt.enabled)
	{
	  if (dhcp6_ia_na_rt.T1)
	    vlib_cli_output (
	      vm,
	      "    dhcp6 ia-na enabled addresses %u server-index %u T1 %u (%u remaining) T2 %u (%u remaining)%s",
	      dhcp6_ia_na_rt.address_count, dhcp6_ia_na_rt.server_index, dhcp6_ia_na_rt.T1,
	      dhcp6_ia_na_rt.t1_remaining, dhcp6_ia_na_rt.T2, dhcp6_ia_na_rt.t2_remaining,
	  ...
	  if (dhcp6_ia_na_rt.first_address_present)
	    vlib_cli_output (
	      ...
	      dhcp6_ia_na_rt.first_address_preferred_lt,
	      dhcp6_ia_na_rt.first_address_valid_lt);
	}
      else
  ...
  if (dhcp6_pd_available)
      ...
      if (dhcp6_pd_rt.enabled)
	{
	  if (dhcp6_pd_rt.T1)
	    vlib_cli_output (
	      vm,
	      "    dhcp6 pd enabled prefix-group %s prefixes %u server-index %u T1 %u (%u remaining) T2 %u (%u remaining)%s",
	  ...
	  if (dhcp6_pd_prefix_rt.present)
	    vlib_cli_output (
	      vm, "    dhcp6 delegated-prefix %U/%u preferred-lifetime %u valid-lifetime %u (%u remaining)",
	      format_ip6_address, &dhcp6_pd_prefix_rt.prefix, dhcp6_pd_prefix_rt.prefix_length,
	      dhcp6_pd_prefix_rt.preferred_lt, dhcp6_pd_prefix_rt.valid_lt,
	      dhcp6_pd_prefix_rt.valid_remaining);
	  else
	  ...
	  if (dhcp6_pd_consumer_available)
		...
		vlib_cli_output (
		  vm, "    dhcp6 pd downstream %U address %U/%u consumers %u",
		  format_vnet_sw_if_index_name, pem->vnet_main, dhcp6_pd_consumer_rt.sw_if_index,
		  format_ip6_address, &dhcp6_pd_consumer_rt.address,
		  dhcp6_pd_consumer_rt.prefix_length, dhcp6_pd_consumer_rt.consumer_count);
	      else
  ...
  vlib_cli_output (vm,
	   "    stored-config ac-name %v service-name %v auth-user %v use-peer-dns4 %u add-default-route4 %u add-default-route6 %u",
	   configured_ac_name, configured_service_name, configured_auth_user, c->use_peer_dns,
	   c->use_peer_route4, c->use_peer_route6);
  if (c->mtu || c->mru || c->timeout)
    vlib_cli_output (vm,
		     "    stored-config mtu %u mru %u timeout %u",
...
show_pppoeclient_debug_one (vlib_main_t *vm, pppoe_client_t *c)
{
  ...
  vlib_cli_output (vm, "[%u] access-interface %U host-uniq %u", client_index,
		   format_vnet_sw_if_index_name, pem->vnet_main, c->sw_if_index, c->host_uniq);
  vlib_cli_output (vm, "    pppoe state %U session-id %u pppox-sw-if-index %u", format_pppoe_client_state,
		   c->state, c->session_id, c->pppox_sw_if_index);

  if (t && unit != ~0)
    {
      vlib_cli_output (
	vm, "    pppox unit %u detail-source %s session-allocated %u delete-pending %u",
	unit, unit_from_hw ? "hw-dev-instance" : "sw-if-map", t->pppoe_session_allocated,
	t->delete_pending);
      if (ppp_debug_rt.present)
	{
	  vlib_cli_output (
	    vm, "    ppp phase %U lcp %U ipcp %U ipv6cp %U", format_ppp_phase_name, ppp_debug_rt.phase,
	    format_ppp_fsm_state_name, ppp_debug_rt.lcp_state, format_ppp_fsm_state_name,
	    ppp_debug_rt.ipcp_state, format_ppp_fsm_state_name, ppp_debug_rt.ipv6cp_state);
	  vlib_cli_output (
	    vm, "    ppp timeouts lcp %d ipcp %d ipv6cp %d", ppp_debug_rt.lcp_timeout,
	    ppp_debug_rt.ipcp_timeout, ppp_debug_rt.ipv6cp_timeout);
	  vlib_cli_output (
	    vm,
	    "    ipcp requested default-route4 %u req-dns1 %u req-dns2 %u negotiated-dns1 %U negotiated-dns2 %U",
	    ppp_debug_rt.default_route4, ppp_debug_rt.req_dns1, ppp_debug_rt.req_dns2,
	    format_ip4_address, &ppp_debug_rt.negotiated_dns1, format_ip4_address,
	    &ppp_debug_rt.negotiated_dns2);
	}
      else
...
set_pppoeclient_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  if (route_or_dns_changed)
    vlib_cli_output (vm,
		     "PPPoE client %u updated (route/DNS state synced; active session may need reconnect for full effect)",
		     client_index);
  else
...
VLIB_CLI_COMMAND (set_pppoeclient_command, static) = {
  ...
  .short_help =
    "set pppoe client <index> [ac-name <name>|ac-name any] [service-name <name>|service-name any] [username <user>] [password <pass>] [mtu <n>] [mru <n>] [timeout <n>] "
