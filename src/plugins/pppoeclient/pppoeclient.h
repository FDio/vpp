...
typedef struct
{
  ...
  /* Parent hardware interface that owns the device-input feature.  For a
   * client on a VLAN sub-interface this differs from sw_if_index: VPP runs
   * the device-input arc on the physical port and demultiplexes tagged
   * frames to sub-interfaces in ethernet-input, i.e. after our feature. */
  u32 sup_sw_if_index;
  /* VLAN classification copied from the sub-interface at creation time.
   * The dispatch node uses it to map (physical port, tag stack) to the
   * configured access sw_if_index.  n_tags == 0 means the client is on the
   * physical port and only untagged frames match. */
  struct
  {
    u16 outer_vlan_id;
    u16 inner_vlan_id;
  ...
  /* Actual tag stack observed on the frames of the current session.  For
   * wildcard ("default") sub-interfaces vlan_match has no concrete VLAN ID,
   * so the dispatch feature records what it saw here; pppoeclient_exit()
   * uses it to emit a correctly tagged raw PADT on shutdown. */
  struct
  {
    u16 outer_vlan_id;
    u16 inner_vlan_id;
  ...
  /* Precomputed L2 header for session-data TX: Ethernet addresses, optional
   * VLAN tag stack and the 0x8864 ethertype.  Built at client creation and
   * rebuilt when the session is established (AC MAC known), so the TX fast
   * path only copies bytes instead of re-resolving the access interface. */
  u8 tx_l2_template[sizeof (ethernet_header_t) + 2 * sizeof (ethernet_vlan_header_t)];
  u8 tx_l2_len;
  ...
  u8 use_peer_ipv6;
  /* Operator issued "stop"; blocks discovery processing and automatic
   * PPP-death redials until an explicit open/restart clears it. */
  u8 admin_stopped;
  u8 lcp_state;
  u8 lcp_id;
  u8 lcp_nak;
...
typedef struct
{
  ...
  /* Per-super-interface (physical port) vector of client indices.  The
   * dispatch node walks this to resolve a tagged frame to the configured
   * sub-interface client.  Mutated on the main thread under the worker
   * barrier only. */
  u32 **client_indices_by_sup_sw_if_index;

  /* API message ID base */
  u16 msg_id_base;

  /* Handoff queue delivering discovery control packets from worker
   * threads to the main-thread discovery-input node. ~0 until init. */
  u32 discovery_fq_index;

  /* Per-access-interface L2 encap length (eth + vlan tags), cached when the
		      ...
		      pppoeclient_result_t *result0)
{
  ...
  if (PREDICT_FALSE (clib_bihash_add_del_8_8 (client_table, &kv, 1 /* is_add */) != 0))
    clib_warning ("pppoeclient: client_table insert failed (table full?)");
}
...
pppoeclient_make_session_key (pppoeclient_session_key_t *key0, u32 sw_if_index, const u8 *ac_mac,
			      u16 session_id)
{
  /* The struct is exactly 16 bytes with no implicit padding (verified by
   * the static asserts above), so write every field instead of zeroing the
   * whole key: this is on the per-packet session lookup fast path. */
			      ...
			      u16 session_id, pppoeclient_result_t *result0)
{
  ...
  kv.key[1] = key0.raw[1];
  kv.value = result0->raw;
  if (PREDICT_FALSE (clib_bihash_add_del_16_8 (session_table, &kv, 1 /* is_add */) != 0))
    clib_warning ("pppoeclient: session_table insert failed (table full?)");
}
			    ...
			    const u8 *src_address, const u8 *dst_address)
{
  ...
  {
    /* dot1ad sub-interfaces must transmit the 0x88A8 TPID (previously the
     * outer tag was hardcoded to 0x8100, breaking QinQ/dot1ad configs).
     * Inner tag stays 0x8100 per 802.1ad QinQ. */
    u16 outer_tpid = sw->sub.eth.flags.dot1ad ? ETHERNET_TYPE_DOT1AD : ETHERNET_TYPE_VLAN;
...
/* VLAN tags observed on an ingress frame, used by the dispatch feature to
 * select the configured access interface. */
typedef struct
{
  u16 outer_vlan_id;
  u16 inner_vlan_id;
  u8 n_tags;
  u8 dot1ad;
} pppoeclient_l2_tags_t;

static_always_inline u8
pppoeclient_get_l2_info (vlib_buffer_t *b, ethernet_header_t **eth_hdr, pppoe_header_t **pppoe_hdr,
			 u16 *ethertype, u16 *l2_hdr_len, pppoeclient_l2_tags_t *tags)
{
  ethernet_header_t *eth = vlib_buffer_get_current (b);
  u16 type, hdr_len;

  if (tags)
    clib_memset (tags, 0, sizeof (*tags));
  ...
  if (ethernet_frame_is_tagged (type))
	...
	{
	  tags->n_tags = 1;
	  tags->dot1ad = (type == ETHERNET_TYPE_DOT1AD);
	  tags->outer_vlan_id = clib_net_to_host_u16 (vlan->priority_cfi_and_id) & 0xfff;
	}
      ...
      if (ethernet_frame_is_tagged (type))
	{
	  ...
	  if (tags)
	    {
	      tags->n_tags = 2;
	      tags->inner_vlan_id = clib_net_to_host_u16 (vlan->priority_cfi_and_id) & 0xfff;
	    }
	  type = clib_net_to_host_u16 (vlan->type);
	  hdr_len += sizeof (*vlan);
	}
