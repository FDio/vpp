...
typedef struct
{
  /* ---- Cache line 0: data-plane hot fields (31 bytes, fits in one line) ----
   * Every field below is read per-packet in pppoeclient_session_output_one.
   * Do NOT insert cold fields between sw_if_index and cached_link_up. */
  u32 sw_if_index;
  u32 pppox_sw_if_index;
  u16 session_id;
  u8 ac_mac_address[6];
  u8 cached_src_mac[6];
  u32 cached_hw_output_next_index;
  u8 cached_link_up;
  /* ---- end of hot fields ---- */

  /* Cold fields below: used only at session setup, teardown, or CLI time. */
  u32 hw_if_index;
  u32 hw_output_next_index;
  u32 host_uniq;

  /* Cached data-plane values — set once at session establishment, read
   * per-packet in the TX/RX hot path to avoid vnet_get_sw_interface lookups. */
  u16 cached_l2_encap_len;
  u16 cached_mtu;
  u16 cached_outer_vlan_id;
  u16 cached_inner_vlan_id;
  u8 cached_one_tag;
  u8 cached_two_tags;
...
/* Push L2 header using cached interface info (hot path — no vnet_get_sw_interface). */
static_always_inline pppoe_header_t *
pppoeclient_push_l2_header_cached (vlib_buffer_t *b, u16 ethertype, const u8 *src_address,
				   const u8 *dst_address, u16 encap_len, u8 one_tag, u8 two_tags,
				   u16 outer_vlan_id, u16 inner_vlan_id)
{
  ethernet_header_t *eth;
  ethernet_vlan_header_t *vlan;

  vlib_buffer_advance (b, -((word) encap_len));
  ...
  if (two_tags)
    {
      ...
      outer->priority_cfi_and_id = clib_host_to_net_u16 (outer_vlan_id);
      outer->type = clib_host_to_net_u16 (ETHERNET_TYPE_VLAN);
      inner = outer + 1;
      inner->priority_cfi_and_id = clib_host_to_net_u16 (inner_vlan_id);
  ...
  if (one_tag)
    {
      ...
      eth->type = clib_host_to_net_u16 (ETHERNET_TYPE_VLAN);
      vlan->priority_cfi_and_id = clib_host_to_net_u16 (outer_vlan_id);
...
/* Original version for control-plane callers (low frequency, uses vnet_get_sw_interface). */
static_always_inline pppoe_header_t *
...
{
  vnet_sw_interface_t *sw = vnet_get_sw_interface (vnm, sw_if_index);
  u16 encap_len = pppoeclient_get_l2_encap_len (vnm, sw_if_index);
  u8 one_tag = sw->type == VNET_SW_INTERFACE_TYPE_SUB && sw->sub.eth.flags.one_tag;
  u8 two_tags = sw->type == VNET_SW_INTERFACE_TYPE_SUB && sw->sub.eth.flags.two_tags;
  return pppoeclient_push_l2_header_cached (b, ethertype, src_address, dst_address, encap_len,
					    one_tag, two_tags, sw->sub.eth.outer_vlan_id,
