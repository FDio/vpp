...
/* Resolve pppox-output's next edge to pppoeclient-session-output at plugin
 * init (main thread) so worker TX never sees the uninitialized sentinel. */
void pppox_output_init_next_edge (vlib_main_t *vm);
