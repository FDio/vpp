...
typedef struct
{
  ...
  /* Handoff queue delivering PPP control packets (LCP/PAP/CHAP/IPCP/IPv6CP)
   * from worker threads to the main-thread pppox-input node. ~0 until init. */
  u32 input_fq_index;
...
void pppox_lower_down (u32);
void pppox_lcp_terminate (u32);

/* Resolve pppoeclient-session-output as the pppox TX node at plugin init
 * (main thread).  The old pppox-output pass-through node was merged into
 * session-output to remove a graph hop and its cross-thread stats state. */
void pppox_resolve_tx_node (vlib_main_t *vm);
u32 pppox_get_tx_node_index (void);
