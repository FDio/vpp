...
vl_api_pppox_set_auth_t_handler (vl_api_pppox_set_auth_t *mp)
{
  ...
  if (vl_api_string_len (&mp->username) > 0)
    username = vl_api_from_api_to_new_vec (mp, &mp->username);

  if (vl_api_string_len (&mp->password) > 0)
    password = vl_api_from_api_to_new_vec (mp, &mp->password);
