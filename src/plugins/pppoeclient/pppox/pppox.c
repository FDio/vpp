...
pppox_dup_c_string_vec (u8 *src)
{
  ...
  /* CLI input arrives as a NUL-terminated vec (unformat %s) while API
   * input is a bare byte vector - measure with strnlen so both paths
   * produce identical (NUL-free, strlen-sized) strings.  The previous
   * vec_len() copy counted the trailing NUL, inflating the PAP/CHAP
   * length bytes on the wire by one for CLI-configured credentials. */
  len = clib_strnlen ((const char *) src, vec_len (src));
...
pppox_allocate_interface (u32 pppoeclient_index)
{
  ...
  /* Bypass the per-interface output node and feed pppoeclient-session-output
   * directly; it encapsulates the frame and forwards it to the physical port.
   * Set on both the fresh-registration and hw_if_index-reuse paths. */
  hi->output_node_index = pppox_get_tx_node_index ();
...
pppox_free_interface (u32 hw_if_index)
{
  ...
  /* Steal the custom name before the lock so the hw-interface rename can
   * run outside it (vnet_rename_interface is a main-thread graph op). */
  u8 *custom_name = t->custom_name;
  t->custom_name = 0;

  /* Turn down LCP and cancel every pending FSM timer.  Without lcp_lowerdown
   * a peer-Terminate-Request-armed fsm_timeout survives the free and fires
   * into whatever session later reuses this unit.  All shared-state
   * mutations (mapping, credentials, pool) happen under the lock so worker
   * threads reading them via consume_pppox_ctrl_pkt cannot
   * observe freed or half-torn-down state. */
  clib_spinlock_lock_if_init (&pom->ctrl_lock);
  lcp_lowerdown (unit);
...
/* Send an LCP Terminate-Request on a still-open session.  The delete path
 * calls this BEFORE freeing the interface so the pppox mapping and the
 * client's SESSION state are still intact and the frame actually goes out
 * (pppoeclient_output_ctrl_pkt drops control output for non-SESSION
 * clients). */
__clib_export void
pppox_lcp_terminate (u32 sw_if_index)
{
  pppox_main_t *pom = &pppox_main;
  int unit;
  ...
  clib_spinlock_lock_if_init (&pom->ctrl_lock);
  lcp_close (unit, "pppoeclient delete");
  clib_spinlock_unlock_if_init (&pom->ctrl_lock);
}
...
pppox_set_auth (u32 sw_if_index, u8 *username, u8 *password)
{
  ...
  /* PAP/CHAP carry the credential length in a single byte; oversized
   * values would also overflow the 332-byte CHAP response buffer and the
   * 1504-byte PAP packet buffer.  Reject before copying anything. */
  if (username && clib_strnlen ((const char *) username, vec_len (username)) > 255)
    return VNET_API_ERROR_INVALID_VALUE;
  if (password && clib_strnlen ((const char *) password, vec_len (password)) > 255)
    return VNET_API_ERROR_INVALID_VALUE;
...
pppox_init (vlib_main_t *vm)
{
  ...
  /* Resolve the pppox TX target node before any interface is created. */
  pppox_resolve_tx_node (vm);

  /* Handoff queue: worker threads enqueue PPP control packets here and the
   * main thread dequeues them into pppox-input, so the whole pppd-derived
   * state machine runs on the main thread only. */
  pom->input_fq_index = ~0;
  {
    vlib_node_t *in = vlib_get_node_by_name (vm, (u8 *) "pppox-input");
    if (in)
      pom->input_fq_index = vlib_handoff_alloc_queues (&(vlib_handoff_alloc_queues_args_t) {
	.node_index = in->index,
	.queue_size = 1024,
      });
    else
      clib_warning ("pppox: pppox-input node not found");
  }
 ...
 * pppoeclient-session-output vector-node path.  The old path drops packets
 * when called from the main-thread process-node context.
 */
void
...
/* Execute a main-thread effect callback: direct call on the main thread
 * (the pppd FSM always runs there after the control-packet handoff), RPC
 * for the rare worker caller.  Never RPC from the main thread while
 * ctrl_lock is held - the barrier sync would deadlock against workers
 * spinning on the lock. */
static_always_inline void
pppox_main_thread_effect (void *callback, u8 *args, u32 arg_size)
{
  if (vlib_get_thread_index () == 0)
    {
      ((void (*) (void *)) callback) (args);
      return;
    }
  vlib_rpc_call_main_thread (callback, args, arg_size);
}
int
sifdns (int unit, u32 dns1, u32 dns2)
{
  ...
  /* The pppd FSM runs on the main thread (control packets are handed off
   * from workers).  Calling vlib_rpc_call_main_thread from the main thread
   * executes between the worker barrier sync/release: with ctrl_lock held
   * a worker spinning on the lock never reaches the barrier and VPP hangs
   * until os_panic().  Call the callback directly here instead. */
  if (vlib_get_thread_index () == 0)
    {
      dns_callback (&a);
      return 1;
  ...
  pppox_main_thread_effect ((void *) dns_callback, (u8 *) &a, sizeof (a));
  return 1;
}
...
pppox_set_interface_mtu (int unit, int mtu)
{
  ...
  pppox_main_thread_effect ((void *) mtu_callback, (u8 *) &a, sizeof (a));
}
...
sifdefaultroute (int unit, u_int32_t ouraddr, u_int32_t gateway)
{
  ...
  pppox_main_thread_effect ((void *) default_route_callback, (u8 *) &a, sizeof (a));
  return 1;
}
...
sifaddr (int unit, u32 our_adr, u32 his_adr, u32 net_mask)
{
  ...
  pppox_main_thread_effect ((void *) ifaddr_callback, (u8 *) &a, sizeof (a));
...
sif6addr (int unit, const u8 *ourid, const u8 *hisid)
{
  ...
  pppox_main_thread_effect ((void *) if6addr_callback, (u8 *) &a, sizeof (a));
...
channel_cleanup (int unit)
{
  ...
  pppox_main_thread_effect ((void *) cleanup_callback, (u8 *) &a, sizeof (a));
