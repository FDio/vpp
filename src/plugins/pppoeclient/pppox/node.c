...
/* Resolve the "pppoeclient-session-output" next edge on the main thread at
 * plugin init time.  Before this existed, the edge was created lazily from
 * the data path: workers running before the first main-thread pppox-output
 * frame observed ~0 and silently dropped ALL TX packets, possibly forever
 * (main thread never sees pppox frames under some rx-placement configs). */
void
pppox_output_init_next_edge (vlib_main_t *vm)
{
...
pppox_get_output_next_index (vlib_main_t *vm)
{
  ...
  /* Late fallback: only the main thread may extend the graph.  Normal
   * operation never reaches this once pppox_output_init_next_edge() has
   * run during plugin init. */
  if (PREDICT_FALSE (vlib_get_thread_index () != 0))
    return PPPOX_OUTPUT_NEXT_DROP;

  pppox_output_init_next_edge (vm);
  if (pppoeclient_output_next_index == ~0)
