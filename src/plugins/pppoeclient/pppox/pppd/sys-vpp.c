 ...
 * All protocol state machines (LCP, IPCP, CHAP, PAP) share this single
 * scratch buffer and call output() before returning.  This is safe as
 * long as output() does not re-enter a protocol handler; the VPP
 * integration enqueues the packet and returns without callback. */
...
int do_callback;			    /* set if we want to do callback next */
...
/*
 * get_host_seed - return a host-dependent seed value for the PRNG.
 * Used by magic_init() in magic.c.
 *
 * Seed from /dev/urandom like upstream pppd: getpid() ^ time(NULL) is
 * predictable, so every client's LCP magic number and IPv6CP interface
 * identifier would derive from an easily guessable stream (spoofable
 * loopback detection).  Fall back to the old value if urandom is missing.
 */
int
get_host_seed (void)
{
...
/*
 * check_passwd - Stub for server-side PAP authentication.
 * This PPPoE client plugin never acts as a PAP server, but the imported
 * upap.c references this symbol.  Return UPAP_AUTHNAK so that any
 * misbehaving BAS that sends AUTHREQ gets a clean rejection.
 */
int
check_passwd (int unit, char *user, int userlen, char *passwd, int passwdlen, char **msg)
{
  *msg = "PAP server mode not supported";
  return UPAP_AUTHNAK;
}
