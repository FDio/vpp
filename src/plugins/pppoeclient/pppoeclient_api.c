...
send_pppoeclient_details (pppoe_client_t *t, vl_api_registration_t *reg, u32 context)
{
  ...
  rmp->use_peer_dns4 = t->use_peer_dns;
  ...
  vif = pppoeclient_api_get_detail_virtual_interface (pem, t, t - pem->clients, &unit, &unit_from_hw);
  ...
  observed_local_present =
    pppoeclient_api_get_global_ipv6 (t->pppox_sw_if_index, &observed_local_ip6, &observed_prefix_len);
  ...
  if (pppoeclient_api_get_dhcp6_ia_na (t->pppox_sw_if_index, &dhcp6_ia_na_rt))
      ...
      if (dhcp6_ia_na_rt.first_address_present)
	{
	  ...
	  rmp->dhcp6_ia_na_first_address_valid_lt =
	    htonl (dhcp6_ia_na_rt.first_address_valid_lt);
	}
    }
  ...
  if (pppoeclient_api_get_dhcp6_pd (t->pppox_sw_if_index, &dhcp6_pd_rt, &dhcp6_pd_prefix_rt,
				    &dhcp6_pd_consumer_rt))
    {
      ...
      pppoeclient_copy_api_string (rmp->dhcp6_pd_prefix_group,
				   sizeof (rmp->dhcp6_pd_prefix_group),
      ...
      if (dhcp6_pd_prefix_rt.present)
	{
	  ...
	  rmp->dhcp6_delegated_prefix_preferred_lt =
	    htonl (dhcp6_pd_prefix_rt.preferred_lt);
	  rmp->dhcp6_delegated_prefix_valid_lt = htonl (dhcp6_pd_prefix_rt.valid_lt);
	  rmp->dhcp6_delegated_prefix_valid_remaining =
	    htonl (dhcp6_pd_prefix_rt.valid_remaining);
	}
