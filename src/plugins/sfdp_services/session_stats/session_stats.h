...
typedef struct
{
  u64 api_data; /**< Custom data set via API */
} __clib_packed sfdp_session_stats_custom_data_t;

STATIC_ASSERT_SIZEOF (sfdp_session_stats_custom_data_t, 8);
...
typedef struct
{
  ...
  /* Custom data flags and data */
  u8 custom_data_flags; /**< Flags: bit 0 = API data present */

  /* Custom data area */
  sfdp_session_stats_custom_data_t custom_data; /**< API custom data */
...
STATIC_ASSERT ((sizeof (sfdp_session_stats_ring_entry_t) % CLIB_CACHE_LINE_BYTES) == 0,
	       "sfdp_session_stats_ring_entry_t must be cache-line sized");
