 ...
 * Total size: 384 bytes (6 cache lines)
 *   - Core stats and five-tuple: 0-223 (224 bytes)
 *   - TCP info (MSS, handshake, etc): 224-303 (80 bytes)
 *     - Includes ECN/CWR metrics: ecn_ect, ecn_ce, ece, cwr packets
 *   - Custom data area: 304-327 (24 bytes)
 *     - Flags: 1 byte + 7 reserved
 *     - API data: 64 bits + 64 bits reserved
 *   - Padding: 328-383 (56 bytes)
 */

/* Custom data flags - indicate which custom data sources are present */
#define SFDP_SESSION_STATS_CUSTOM_FLAG_API_DATA (1 << 0) /**< API custom data present */

/* Default invalid value for custom API data when no tenant-specific value is set */
#define SFDP_SESSION_STATS_CUSTOM_API_DATA_INVALID ((u64) ~0ULL)

/* Value to indicate all tenants (max u32) */
#define SFDP_SESSION_STATS_ALL_TENANTS ((u32) ~0U)

/* Maximum length for custom API data name (excluding null terminator) */
#define SFDP_SESSION_STATS_CUSTOM_API_DATA_NAME_MAX_LEN 20
...
/*
 * Custom data structure - 128 bits total
 * First 64 bits: set via API
 * Last 64 bits: reserved for future use
 */
typedef struct
{
  u64 api_data; /**< Custom data set via API (first 64 bits) */
  u64 reserved; /**< Reserved for future use (last 64 bits) */
} __clib_packed sfdp_session_stats_custom_data_t;

STATIC_ASSERT_SIZEOF (sfdp_session_stats_custom_data_t, 16);
...
typedef struct
{
  ...
  /* Custom data flags and data (128 bits total) */
  u8 custom_data_flags;	 /**< Flags: bit 0 = API data present */
  u8 reserved_custom[7]; /**< Reserved for future use */

  /* Custom data area - 128 bits total */
  sfdp_session_stats_custom_data_t custom_data; /**< API (64 bits) + Reserved (64 bits) */

  /* Padding to 384 bytes (6 cache lines) */
  u8 reserved_pad[48];
} __clib_packed sfdp_session_stats_ring_entry_t;

STATIC_ASSERT_SIZEOF (sfdp_session_stats_ring_entry_t, 384);
...
typedef struct
{
  ...
  /* Custom API data name (set before ring buffer init, max 20 chars + null) */
  u8 custom_api_data_name[SFDP_SESSION_STATS_CUSTOM_API_DATA_NAME_MAX_LEN + 1];

  /* Per-tenant custom API data
   * Key: tenant_id (u32, cast to u64 for bihash) -> Entry with value
   * Uses clib_bihash_8_8 where key is the tenant_id
   * and value is a pool index into custom_data_entries */
  clib_bihash_8_8_t custom_data_by_tenant; /**< Hash: tenant_id -> entry pool idx */
  sfdp_session_stats_custom_data_entry_t *custom_data_entries; /**< Pool of entries */
...
int sfdp_session_stats_set_custom_api_data_name (const u8 *name, u32 name_len);
int sfdp_session_stats_get_custom_api_data_name (u8 *name, u32 *name_len);

/* Helper to create custom data key from tenant_id */
static inline u64
sfdp_session_stats_make_custom_data_key (u32 tenant_id)
{
  return (u64) tenant_id;
}
