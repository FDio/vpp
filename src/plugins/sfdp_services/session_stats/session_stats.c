...
sfdp_session_stats_ring_init (vlib_main_t *vm, u32 ring_size)
{
  ...
  if (ring_size == 0)
    {
      clib_warning ("Invalid SFDP session stats ring size %u", ring_size);
      return -1;
    }
				   ...
				   sfdp_session_stats_export_reason_t reason)
{
  ...
  if (session->key_flags & SFDP_SESSION_KEY_FLAG_PRIMARY_VALID_IP4)
    {
      sfdp_session_ip4_key_t *key4 = &skey.key4;
      sfdp_normalise_ip4_key (session, key4, SFDP_SESSION_KEY_PRIMARY);
      entry->is_ip4 = 1;
    ...
    {
      sfdp_session_ip6_key_t *key6 = &skey.key6;
      sfdp_normalise_ip6_key (session, key6, SFDP_SESSION_KEY_PRIMARY);
      entry->is_ip4 = 0;

      /* Endpoint key: src -> dst */
      clib_memcpy (entry->src_ip, &key6->ip6_key.ip6_addr_lo, 16);
      clib_memcpy (entry->dst_ip, &key6->ip6_key.ip6_addr_hi, 16);
      entry->src_port = key6->ip6_key.port_lo;
      entry->dst_port = key6->ip6_key.port_hi;
    }
