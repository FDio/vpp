...
sfdp_session_stats_ring_init (vlib_main_t *vm, u32 ring_size)
{
   ...
   * Version 5: 512-byte format with extended statistics:
   ...
   *   - ECN/CWR metrics (ecn_ect, ecn_ce, ece, cwr packet counts)
   *   - Custom data area (128 bits: 64 bits API + 64 bits reserved)
   *   - Optional custom_api_data_name if set
   */
...
/*
 * Get custom API data for a tenant
 * Returns the custom data value, or SFDP_SESSION_STATS_CUSTOM_API_DATA_INVALID if not set
 * If has_value is non-NULL, sets it to 1 if the value was explicitly set, 0 otherwise
 */
u64
...
/*
 * Set custom API data name (must be called before ring buffer is enabled)
 * Returns:
 *   0 on success
 *  -1 if name is too long
 *  -2 if ring buffer is already enabled
 */
int
...
/*
 * Get custom API data name
 * Returns:
 *   0 on success
 *  -1 if output buffer is NULL
 *  name_len will be set to the actual length of the name
 */
int
