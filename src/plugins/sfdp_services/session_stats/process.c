 ...
 * thread data of the ring buffer.
  ...
  * Worker thread ring slots can still be populated by non-process export paths (e.g. expiry
 * callbacks executing on worker threads).
 */
static uword
...
sfdp_session_stats_periodic_export_enable (vlib_main_t *vm, f64 interval, f64 batch_interval)
{
  ...
  /* use default value if intervals are invalid */
  if (interval > 0)
