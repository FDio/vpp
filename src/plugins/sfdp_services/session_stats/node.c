...
{
  tcp_header_t *tcph;
  u32 l4_off, tcp_hlen, payload_len;
  u8 ack_dir = 1 - direction; /* corresponds to reverse direction */

  /* get tcp header for IPv4/IPv6 */
  if (session->type == SFDP_SESSION_TYPE_IP4)
    {
      ip4_header_t *ip4 = vlib_buffer_get_current (b);

      l4_off = ip4_header_bytes (ip4);
      tcph = ip4_next_header (ip4);
    }
  else
    {
      i16 l4_hdr_offset;
	...
	return;
      ...
      l4_hdr_offset = vnet_buffer (b)->l4_hdr_offset;
      if (l4_hdr_offset < b->current_data)
	return;

      l4_off = l4_hdr_offset - b->current_data;
      tcph = (tcp_header_t *) (b->data + l4_hdr_offset);
    }

  if (PREDICT_FALSE (l4_off + sizeof (*tcph) > l3_len))
    return;

  tcp_hlen = tcp_header_bytes (tcph);

  if (PREDICT_FALSE (tcp_hlen < sizeof (*tcph) || l4_off + tcp_hlen > l3_len))
    return;
  ...
  payload_len = l3_len - l4_off - tcp_hlen;
  ...
  /* Sequence tracking for retransmission and overlap detection */
  if (payload_len > 0)
    {
      ...
      if (stats->last_seq_valid[direction])
	{
	  ...
	  if (seq_leq (end_seq, stats->end_seq_max[direction]))
	    {
	      if (stats->has_pending_gap[direction] &&
		  seq_gt (end_seq, stats->gap_start_seq[direction]) &&
		  stats->tcp.dupack_like[direction] == stats->gap_dupack_snapshot[direction] &&
		  seq_gt (end_seq, stats->tcp.last_ack[direction]))
		{
		  /* out-of-order detection: a gap was previously seen,
		   * this segment fills part of that gap, the
		...
		{
		  /* retransmission detection: either no gap was seen, or dupacks
	  ...
	  /* segment contains max expected end_seq number in range [seq, end_seq (seq +
	   * payload_len)] */
	   ...
	   * number */
	  else if (seq_gt (seq, stats->end_seq_max[direction]))
	    {
	      /* gap detection for when filling segment arrives.
	       * we also snapshot the current dupack
	       * count so we can detect if the receiver sends dupacks
	       * (loss/retranmission event) vs. stays silent (reorder event). */
	      stats->has_pending_gap[direction] = 1;
	      stats->gap_start_seq[direction] = stats->end_seq_max[direction];
	      stats->gap_dupack_snapshot[direction] = stats->tcp.dupack_like[direction];
	    }
	}
