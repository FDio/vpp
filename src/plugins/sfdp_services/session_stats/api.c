  ...
  vl_api_sfdp_session_stats_periodic_export_t *mp)
{
  ...
  if (mp->enable)
    {
      f64 interval = clib_net_to_host_f64 (mp->interval);
      f64 batch_interval = clib_net_to_host_f64 (mp->batch_interval);
      /* return error if batch interval does not respect ratio to main interval */
      rv = sfdp_session_stats_periodic_export_enable (vm, interval, batch_interval);
      if (rv)
...
/* no-op api handler for sfdp_session_stats_ring_entry_abi_id */
static void
vl_api_sfdp_session_stats_ring_entry_abi_id_t_handler (
  vl_api_sfdp_session_stats_ring_entry_abi_id_t *mp)
{
  vl_api_sfdp_session_stats_ring_entry_abi_id_reply_t *rmp;
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  int rv = 0;
				 ...
				 sfdp_session_stats_entry_t *stats)
{
  ...
  mp->rtt_min_fwd = stats->rtt[SFDP_FLOW_FORWARD].min;
  mp->rtt_max_fwd = stats->rtt[SFDP_FLOW_FORWARD].max;
  ...
  mp->rtt_min_rev = stats->rtt[SFDP_FLOW_REVERSE].min;
  mp->rtt_max_rev = stats->rtt[SFDP_FLOW_REVERSE].max;

  /* SYN handshake RTT */
  mp->syn_rtt = stats->syn_rtt;

  /* TCP-specific statistics */
  mp->tcp_mss_fwd = clib_host_to_net_u16 (stats->tcp.mss[SFDP_FLOW_FORWARD]);
  mp->tcp_mss_rev = clib_host_to_net_u16 (stats->tcp.mss[SFDP_FLOW_REVERSE]);
  mp->tcp_handshake_complete = stats->tcp.handshake_complete;
  mp->tcp_ts_negotiated = stats->tcp.timestamp_option_negotiated[SFDP_FLOW_FORWARD] &&
...
vl_api_sfdp_session_stats_get_config_t_handler (vl_api_sfdp_session_stats_get_config_t *mp)
{
  ...
  /* TODO - do we really need to re-compute active sessions here for each config dump ? */
  ...
  for (u32 i = 0; i < vec_len (ssm->stats); i++)
    {
      /* only account for valid entries which have seen traffic */
      sfdp_session_stats_entry_t *s = vec_elt_at_index (ssm->stats, i);
      if (s->version && (s->packets[SFDP_FLOW_FORWARD] > 0 || s->packets[SFDP_FLOW_REVERSE] > 0))
	active_sessions++;
    }
  ...
  REPLY_MACRO2 (VL_API_SFDP_SESSION_STATS_GET_CONFIG_REPLY, ({
		  ...
		  rmp->export_interval = clib_host_to_net_f64 (ssm->export_interval);
		  rmp->export_batch_interval = clib_host_to_net_f64 (ssm->export_batch_interval);
		  rmp->ring_size = clib_host_to_net_u32 (ssm->ring_buffer_size);
		  rmp->total_exports = clib_host_to_net_u64 (sfdp_session_stats_total_exports (
		    ssm)); /* return total export value across all threads */
...
static void
vl_api_sfdp_session_stats_set_tenant_custom_data_t_handler (
  vl_api_sfdp_session_stats_set_tenant_custom_data_t *mp)
{
  vl_api_sfdp_session_stats_set_tenant_custom_data_reply_t *rmp;
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  int rv = 0;
...
static void
vl_api_sfdp_session_stats_clear_tenant_custom_data_t_handler (
  vl_api_sfdp_session_stats_clear_tenant_custom_data_t *mp)
{
  vl_api_sfdp_session_stats_clear_tenant_custom_data_reply_t *rmp;
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  int rv = 0;

  u32 tenant_id = clib_net_to_host_u32 (mp->tenant_id);
  sfdp_session_stats_clear_tenant_custom_data (tenant_id);

  REPLY_MACRO (VL_API_SFDP_SESSION_STATS_CLEAR_TENANT_CUSTOM_DATA_REPLY);
}
