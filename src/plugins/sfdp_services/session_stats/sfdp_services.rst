...
- **Periodic export**: Configurable interval (default 60s). All active
...
Export reasons are encoded in exported entry: ``periodic`` (0), ``expiry`` (1),
``api-request`` (2).
...
- **Ring buffer reader**: No example consumer program is provided yet for
  reading and exporting ring buffer data (e.g., to Prometheus). The current
  example available for a reader is available in the Python test files (see test
  ``test_session_stats_ring_buffer_with_custom_data`` for reference).
