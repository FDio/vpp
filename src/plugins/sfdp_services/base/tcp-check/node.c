		      ...
		      vlib_buffer_t **b, u32 *sf, u32 *nsf)
{
  ...
  if ((nsf[0] & (SFDP_TCP_CHECK_SESSION_FLAG_SEEN_ACK_TO_FIN_INIT)) &&
      (nsf[0] & SFDP_TCP_CHECK_SESSION_FLAG_SEEN_ACK_TO_FIN_RESP))
    {
      /* check if session leverages a secondary key (e.g. NAT'd session) */
      ...
      /* TODO: TIME_WAIT state is not applied currently to sfdp sessions */
      /* with a secondary key */
      if (time_wait_enabled && !has_secondary_key)
