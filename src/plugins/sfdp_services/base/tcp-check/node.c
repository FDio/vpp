...
update_state_one_pkt (sfdp_tw_t *tw, sfdp_tenant_t *tenant, u8 fsol_non_syn_block,
  ...
  if (PREDICT_FALSE (tcp_session->version != session->session_version))
    {
	 ...
	 either block all subsequent traffic or remove */
      if (flags != SFDP_TCP_CHECK_TCP_FLAGS_SYN)
	{
	  if (fsol_non_syn_block)
	    {
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left > 0)
	...
	update_state_one_pkt (tw, tenant, vtcm->fsol_non_syn_block, tcp_session, session,
