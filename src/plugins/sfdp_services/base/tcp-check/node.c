		      ...
		      u16 *to_next, vlib_buffer_t **b, u32 *sf, u32 *nsf)
{
  ...
  if (session->type == SFDP_SESSION_TYPE_IP4)
    {
      u16 total_len = clib_net_to_host_u16 (ip4->length);
      u16 ip_hdr_len = ip4_header_bytes (ip4);

      /* Verify that computed tcp_payload_len does not underflow */
      /* otherwise, clamp to zero */
      tcp_payload_len =
	(total_len > ip_hdr_len + tcp_hdr_len) ? total_len - ip_hdr_len - tcp_hdr_len : 0;
    }
  else
    {
      u16 payload_len = clib_net_to_host_u16 (ip6->payload_length);
      u16 tcp_offset = (u8 *) tcph - (u8 *) (ip6 + 1);
      tcp_payload_len =
	(payload_len > tcp_offset + tcp_hdr_len) ? payload_len - tcp_offset - tcp_hdr_len : 0;
    }
  ...
  if (dir == SFDP_FLOW_FORWARD)
      ...
      if (flags & SFDP_TCP_CHECK_TCP_FLAGS_ACK)
	{
	  ...
	  /* TODO: The two ifs above assume the packet will not get dropped after VPP. */
	  /* Or regular ACK */
	}
      if (flags & SFDP_TCP_CHECK_TCP_FLAGS_FIN)
	{
	  ...
	  nsf[0] &= ~SFDP_TCP_CHECK_SESSION_FLAG_SEEN_ACK_TO_FIN_INIT;
	  /* TODO: This branch assumes the packet will not get dropped after VPP. */
	}
  ...
  if (dir == SFDP_FLOW_REVERSE)
      ...
      if (flags & SFDP_TCP_CHECK_TCP_FLAGS_SYN)
	{
	  if (sf[0] & SFDP_TCP_CHECK_SESSION_FLAG_WAIT_FOR_RESP_SYN)
	    ...
	    {
	      /* Unexpected, must be a SYN flood. Do not drop but do not change state flags either.
	       */
	      goto send;
	      return;
	    }
