		      ...
		      u16 *to_next, vlib_buffer_t **b, u32 *sf, u32 *nsf)
{
  ...
  if (dir == SFDP_FLOW_REVERSE)
      ...
      if (flags & SFDP_TCP_CHECK_TCP_FLAGS_RST)
	{
	  ...
	  remove_session = 1;
	  goto out;
	  /* TODO: Is the current logic ready for in-flight packets from initiator? */
	}
      /* TODO: Are all possibilities covered? */
    }
  ...
  if ((nsf[0] & (SFDP_TCP_CHECK_SESSION_FLAG_SEEN_ACK_TO_FIN_INIT)) &&
      (nsf[0] & SFDP_TCP_CHECK_SESSION_FLAG_SEEN_ACK_TO_FIN_RESP))
    {
      ...
      time_wait = 1;
      /* TODO: Allow early cleanup when session table gets full? */
    }
out:
  ...
  /* TODO: Make sure the logic work when both sides send FIN at the same time. */
