				      ...
				      vlib_cli_command_t *cmd)
{
  ...
  if (unformat (line_input, "block"))
    vtcm->fsol_non_syn_block = 1;
  else if (unformat (line_input, "remove"))
    vtcm->fsol_non_syn_block = 0;
  else
    err = clib_error_return (0, "expected 'block' or 'remove', got '%U'", format_unformat_error,
			     line_input);
...
VLIB_CLI_COMMAND (sfdp_tcp_set_fsol_non_syn_command, static) = {
  ...
  .short_help = "set sfdp tcp-check fsol-non-syn <block|remove>",
