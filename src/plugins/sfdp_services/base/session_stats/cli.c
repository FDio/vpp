				    ...
				    vlib_cli_command_t *cmd)
{
  ...
  vlib_cli_output (vm, "  Ring buffer: %s (size: %u)",
		   ...
		   ssm->ring_buffer_size);
  vlib_cli_output (vm, "  Periodic export: %s (interval: %.1f sec)",
		   ssm->periodic_export_enabled ? "enabled" : "disabled",
		   ssm->export_interval);
  vlib_cli_output (vm, "  Total exports: %lu", ssm->total_exports);
  vlib_cli_output (vm, "");

  vlib_cli_output (vm,
		   "%-10s %-6s %-10s %-12s %-12s %-12s %-12s",
		   "Session", "Proto", "Tenant", "Pkts(fwd)", "Pkts(rev)",
		   "Bytes(fwd)", "Bytes(rev)");
  vlib_cli_output (vm,
		   "---------- ------ ---------- ------------ "
		   "------------ ------------ ------------");
  ...
  sfdp_foreach_session (sfdp, session_index, session)
  {
    ...
    vlib_cli_output (vm, "%-10u %-6u %-10d %-12lu %-12lu %-12lu %-12lu",
		     session->session_id, session->proto, session->tenant_idx,
		     stats->packets[SFDP_FLOW_FORWARD],
		     stats->packets[SFDP_FLOW_REVERSE],
		     stats->bytes[SFDP_FLOW_FORWARD],
		     stats->bytes[SFDP_FLOW_REVERSE]);

    if (verbose && (stats->first_seen > 0 || stats->last_seen > 0))
      {
	vlib_cli_output (vm, "           First seen: %.6f, Last seen: %.6f",
 ...
 */
static clib_error_t *
sfdp_session_stats_periodic_command_fn (vlib_main_t *vm,
  ...
  if (enable)
    {
      ...
      vlib_cli_output (vm, "Periodic export enabled (interval: %.1f sec)",
		       interval);
    }
  else
    {
