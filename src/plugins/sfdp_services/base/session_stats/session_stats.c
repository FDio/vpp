...
sfdp_session_stats_ring_init (vlib_main_t *vm, u32 ring_size)
{
  ...
  schema_string = format (
    schema_string, "\"fields\":["
		   ...
		   "{\"name\":\"tcp_last_seq_forward\",\"type\":\"u32\",\"offset\":288},"
		   "{\"name\":\"tcp_last_ack_forward\",\"type\":\"u32\",\"offset\":292},"
		   "{\"name\":\"tcp_last_seq_reverse\",\"type\":\"u32\",\"offset\":296},"
		   "{\"name\":\"tcp_last_ack_reverse\",\"type\":\"u32\",\"offset\":300},"
		   "{\"name\":\"custom_data_flags\",\"type\":\"u8\",\"offset\":304},"
		   "{\"name\":\"custom_api_data\",\"type\":\"u64\",\"offset\":312}"
		   "]}");
				   ...
				   sfdp_session_stats_export_reason_t reason)
{
  ...
  if (ssm->custom_data_enabled)
    {
      /* Set per-tenant-context API custom data (first 64 bits) */
      u8 has_tenant_data = 0;
      u32 tenant_id = tenant ? tenant->tenant_id : 0;
      u32 context_id = tenant ? tenant->context_id : 0;
      u64 tenant_api_data =
	sfdp_session_stats_get_custom_api_data (tenant_id, context_id, &has_tenant_data);
...
sfdp_session_stats_init (vlib_main_t *vm)
{
  ...
  /* Custom data configuration - disabled by default */
  ssm->custom_data_enabled = 0;

  /* Custom API data name - empty by default */
  clib_memset (ssm->custom_api_data_name, 0, sizeof (ssm->custom_api_data_name));

  /* Per-tenant-context custom data hash table, allocated on demand */
  clib_bihash_init_8_8 (&ssm->custom_data_by_tenant_context, "custom_data_by_tenant_context", 64,
			(uword) 64 * 1024);
...
/*
 * Set custom API data value for a tenant+context (first 64 bits of custom data)
 */
int
sfdp_session_stats_set_custom_api_data (u32 tenant_id, u32 context_id, u64 value)
{
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  ...
  if (context_id == SFDP_SESSION_STATS_ALL_CONTEXTS)
    return -1; /* Cannot set data for "all contexts" - must specify a valid context */

  /* Create key from tenant_id and context_id */
  sfdp_session_stats_custom_data_key_t key_data =
    sfdp_session_stats_make_custom_data_key (tenant_id, context_id);
  clib_bihash_kv_8_8_t kv, result;
  kv.key = key_data.as_u64;

  /* Check if entry already exists */
  if (clib_bihash_search_8_8 (&ssm->custom_data_by_tenant_context, &kv, &result) == 0)
    {
  ...
  else
    {
      ...
      clib_bihash_add_del_8_8 (&ssm->custom_data_by_tenant_context, &kv, 1);
    }
 ...
 * If context_id == SFDP_SESSION_STATS_ALL_CONTEXTS, clear all contexts for the tenant
 */
int
sfdp_session_stats_clear_custom_api_data (u32 tenant_id, u32 context_id)
{
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;

  if (tenant_id == SFDP_SESSION_STATS_ALL_TENANTS || context_id == SFDP_SESSION_STATS_ALL_CONTEXTS)
    {
      ...
      pool_foreach (entry, ssm->custom_data_entries)
	{
      ...
      clib_bihash_free_8_8 (&ssm->custom_data_by_tenant_context);
      clib_bihash_init_8_8 (&ssm->custom_data_by_tenant_context, "custom_data_by_tenant_context",
			    64, (uword) 64 * 1024);
    }
  else
    {
      ...
      if (clib_bihash_search_8_8 (&ssm->custom_data_by_tenant_context, &kv, &result) == 0)
	{
	  ...
	  clib_bihash_add_del_8_8 (&ssm->custom_data_by_tenant_context, &kv, 0);
...
/*
 * Get custom API data for a tenant+context
 ...
 */
u64
sfdp_session_stats_get_custom_api_data (u32 tenant_id, u32 context_id, u8 *has_value)
{
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  ...
  /* No value set for this tenant+context - return invalid marker */
  if (has_value)
    *has_value = 0;
...
/*
 * Enable or disable custom data export
 */
int
sfdp_session_stats_enable_custom_data (u8 enable)
{
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;

  ssm->custom_data_enabled = enable;
