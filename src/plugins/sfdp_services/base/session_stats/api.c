...
/**
 * @brief Map internal decorator type to API decorator type
 * Values are identical, just different type names
 */
#define sfdp_session_stats_decorator_type_to_api(t)                                                \
  ((vl_api_sfdp_session_stats_decorator_type_t) (t))

/**
 * @brief Map API decorator type to internal decorator type
 * Values are identical, just different type names
 */
#define sfdp_session_stats_decorator_type_from_api(t) ((sfdp_session_stats_decorator_type_t) (t))
...
vl_api_sfdp_session_stats_get_config_t_handler (vl_api_sfdp_session_stats_get_config_t *mp)
{
  ...
  REPLY_MACRO2 (VL_API_SFDP_SESSION_STATS_GET_CONFIG_REPLY, ({
		  ...
		  rmp->decorator_type = sfdp_session_stats_decorator_type_to_api (
		    ssm->decorator_type); // FIXME - not the right approach..
		  rmp->decorator_enabled = ssm->decorator_enabled;
		}));
}
...
/**
 * @brief Handler for set decorator request
 */
static void
vl_api_sfdp_session_stats_set_decorator_t_handler (vl_api_sfdp_session_stats_set_decorator_t *mp)
{
  vl_api_sfdp_session_stats_set_decorator_reply_t *rmp;
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  int rv = 0;

  if (mp->enable)
    {
      sfdp_session_stats_decorator_type_t type =
	sfdp_session_stats_decorator_type_from_api (mp->decorator_type);
      ...
      else
	{
	  ...
	  ssm->decorator_enabled = 1;

	  /* For custom types, use the built-in callback that copies
	   * pre-configured custom data */
	  if (type == SFDP_SESSION_STATS_DECORATOR_CUSTOM_U64 ||
	      type == SFDP_SESSION_STATS_DECORATOR_CUSTOM_U32)
	    {
	      ssm->decorator_fn = sfdp_session_stats_custom_decorator_fn;
    ...
    {
      sfdp_session_stats_clear_decorator ();
    }

  REPLY_MACRO (VL_API_SFDP_SESSION_STATS_SET_DECORATOR_REPLY);
}

/**
 * @brief Handler for get decorator request
 */
static void
vl_api_sfdp_session_stats_get_decorator_t_handler (vl_api_sfdp_session_stats_get_decorator_t *mp)
{
  vl_api_sfdp_session_stats_get_decorator_reply_t *rmp;
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  int rv = 0;

  REPLY_MACRO2 (VL_API_SFDP_SESSION_STATS_GET_DECORATOR_REPLY, ({
		  rmp->decorator_type =
		    sfdp_session_stats_decorator_type_to_api (ssm->decorator_type);
		  rmp->decorator_enabled = ssm->decorator_enabled;
		}));
}

/**
 * @brief Handler for set custom u64 decorator values
 */
static void
vl_api_sfdp_session_stats_set_custom_u64_t_handler (vl_api_sfdp_session_stats_set_custom_u64_t *mp)
{
  vl_api_sfdp_session_stats_set_custom_u64_reply_t *rmp;
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  int rv = 0;

  u32 n_values = mp->n_values;
  if (n_values > 8)
    n_values = 8;

  u64 values[8];
  for (u32 i = 0; i < n_values; i++)
    values[i] = clib_net_to_host_u64 (mp->values[i]);

  sfdp_session_stats_set_custom_u64 (values, n_values);

  REPLY_MACRO (VL_API_SFDP_SESSION_STATS_SET_CUSTOM_U64_REPLY);
}

/**
 * @brief Handler for set custom u32 decorator values
 */
static void
vl_api_sfdp_session_stats_set_custom_u32_t_handler (vl_api_sfdp_session_stats_set_custom_u32_t *mp)
{
  vl_api_sfdp_session_stats_set_custom_u32_reply_t *rmp;
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  int rv = 0;
  ...
  u32 values[16];
  for (u32 i = 0; i < n_values; i++)
    values[i] = clib_net_to_host_u32 (mp->values[i]);

  sfdp_session_stats_set_custom_u32 (values, n_values);

  REPLY_MACRO (VL_API_SFDP_SESSION_STATS_SET_CUSTOM_U32_REPLY);
}
