...
vl_api_sfdp_session_stats_get_config_t_handler (vl_api_sfdp_session_stats_get_config_t *mp)
{
  ...
  /* TODO - is the has_any_api_custom_data flag necessary ? */
  ...
  REPLY_MACRO2 (VL_API_SFDP_SESSION_STATS_GET_CONFIG_REPLY, ({
		  ...
		  rmp->custom_data_enabled = ssm->custom_data_enabled;
...
/**
 * @brief Handler for enable/disable custom data
 */
static void
vl_api_sfdp_session_stats_custom_data_enable_t_handler (
  vl_api_sfdp_session_stats_custom_data_enable_t *mp)
{
  vl_api_sfdp_session_stats_custom_data_enable_reply_t *rmp;
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  int rv = 0;

  sfdp_session_stats_enable_custom_data (mp->enable);

  REPLY_MACRO (VL_API_SFDP_SESSION_STATS_CUSTOM_DATA_ENABLE_REPLY);
}

/**
 * @brief Handler for set custom API data
 */
static void
  ...
  vl_api_sfdp_session_stats_set_custom_api_data_t *mp)
{
  ...
  rv = sfdp_session_stats_set_custom_api_data (tenant_id, context_id, data);

  REPLY_MACRO (VL_API_SFDP_SESSION_STATS_SET_CUSTOM_API_DATA_REPLY);
}

/**
 * @brief Handler for clear custom API data
 */
static void
  ...
  vl_api_sfdp_session_stats_clear_custom_api_data_t *mp)
{
  ...
  u32 tenant_id = clib_net_to_host_u32 (mp->tenant_id);
  u32 context_id = clib_net_to_host_u32 (mp->context_id);
  sfdp_session_stats_clear_custom_api_data (tenant_id, context_id);

  REPLY_MACRO (VL_API_SFDP_SESSION_STATS_CLEAR_CUSTOM_API_DATA_REPLY);
}

/**
 * @brief Handler for get global custom data config
 */
static void
vl_api_sfdp_session_stats_get_custom_data_config_t_handler (
  vl_api_sfdp_session_stats_get_custom_data_config_t *mp)
{
  vl_api_sfdp_session_stats_get_custom_data_config_reply_t *rmp;
  sfdp_session_stats_main_t *ssm = &sfdp_session_stats_main;
  int rv = 0;

  /* Check if any tenant has custom API data set */
  u8 has_any_api_data = (pool_elts (ssm->custom_data_entries) > 0);

  REPLY_MACRO2 (VL_API_SFDP_SESSION_STATS_GET_CUSTOM_DATA_CONFIG_REPLY, ({
		  rmp->custom_data_enabled = ssm->custom_data_enabled;
		  rmp->has_any_api_data = has_any_api_data;
		}));
}

/**
 * @brief Handler for get tenant-specific custom data
 */
static void
  ...
  vl_api_sfdp_session_stats_get_tenant_custom_data_t *mp)
{
  ...
  u8 has_api_data = 0;
  u64 api_data_value =
    sfdp_session_stats_get_custom_api_data (tenant_id, context_id, &has_api_data);

  REPLY_MACRO2 (VL_API_SFDP_SESSION_STATS_GET_TENANT_CUSTOM_DATA_REPLY, ({
		  rmp->tenant_id = clib_host_to_net_u32 (tenant_id);
		  rmp->context_id = clib_host_to_net_u32 (context_id);
...
/**
 * @brief Handler for set custom API data name
 */
static void
...
/**
 * @brief Handler for get custom API data name
 */
static void
