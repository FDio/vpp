...
format_sfdp_session_stats_trace (u8 *s, va_list *args)
{
  ...
  sfdp_session_stats_trace_t *t =
    va_arg (*args, sfdp_session_stats_trace_t *);

  s = format (s,
	      "sfdp-session-stats: flow-id %u (session %u, %s)\n"
	      "  packets: fwd=%llu rev=%llu, bytes: fwd=%llu rev=%llu",
	      t->flow_id, t->flow_id >> 1,
	      t->flow_id & 0x1 ? "reverse" : "forward", t->packets_fwd,
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left >= 4)
      ...
      for (int i = 0; i < 4; i++)
	{
	  ...
	  u8 direction = sfdp_direction_from_flow_index (b[i]->flow_id);
	  sfdp_session_stats_entry_t *stats;
	  u32 pkt_len = vlib_buffer_length_in_chain (vm, b[i]);
  ...
  while (n_left)
    {
      ...
      sfdp_session_stats_entry_t *stats;
      u32 pkt_len = vlib_buffer_length_in_chain (vm, b[0]);
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       SFDP_SESSION_STATS_ERROR_COUNTED, n_counted);
