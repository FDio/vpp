...
/* Value to indicate all contexts (max u32) */
#define SFDP_SESSION_STATS_ALL_CONTEXTS ((u32) ~0U)
...
/*
 * Key for per-tenant-context custom API data lookup
 * Combines tenant_id and context_id into a single 64-bit key
 */
typedef union
{
  struct
  {
    u32 tenant_id;  /**< Tenant ID (customer-specified value) */
    u32 context_id; /**< Context ID within the tenant */
  };
  u64 as_u64;
} sfdp_session_stats_custom_data_key_t;

/*
 * Per-tenant-context custom API data entry (stored in hash table)
 */
typedef struct
{
...
typedef struct
{
  ...
  u8 custom_data_enabled; /**< Whether custom data is enabled */
  u8 pad[3];		  /**< Padding */
  ...
  /* Per-tenant-context custom API data
   * Key: (tenant_id << 32) | context_id -> Entry with value
   * Uses clib_bihash_8_8 where key is the combined (tenant_id, context_id)
   * and value is a pool index into custom_data_entries */
  clib_bihash_8_8_t custom_data_by_tenant_context;	       /**< Hash: key -> entry pool idx */
...
int sfdp_session_stats_set_custom_api_data (u32 tenant_id, u32 context_id, u64 value);
int sfdp_session_stats_clear_custom_api_data (u32 tenant_id, u32 context_id);
u64 sfdp_session_stats_get_custom_api_data (u32 tenant_id, u32 context_id, u8 *has_value);
int sfdp_session_stats_enable_custom_data (u8 enable);
...
/* Helper to create custom data key from tenant_id and context_id */
static inline sfdp_session_stats_custom_data_key_t
sfdp_session_stats_make_custom_data_key (u32 tenant_id, u32 context_id)
{
  sfdp_session_stats_custom_data_key_t key;
