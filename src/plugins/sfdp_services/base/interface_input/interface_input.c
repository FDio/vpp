  ...
  if (sw_if_index >= old_size)
    {
      ...
      for (int i = old_size; i <= sw_if_index; i++)
	vec_elt (vim->offload_enabled_by_sw_if_idx[SFDP_INTERFACE_INPUT_PROTO_IP4], i) = (u8) (~0);
    }
  ...
  if (sw_if_index >= old_size)
    {
      ...
      for (int i = old_size; i <= sw_if_index; i++)
	vec_elt (vim->offload_enabled_by_sw_if_idx[SFDP_INTERFACE_INPUT_PROTO_IP6], i) = (u8) (~0);
    }
