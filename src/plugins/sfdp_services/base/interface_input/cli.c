				   ...
				   vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	unset = 1;
      else if (unformat (line_input, "lookup-offload"))
	is_lookup_offload = 1;
...
VLIB_CLI_COMMAND (sfdp_interface_input_set_unset, static) = {
  ...
  .short_help = "set sfdp interface-input <interface> tenant <tenant-id> [ip6] [lookup-offload] [disable]",
