...
static void
vt_session_data_validate (verdict_testbench_main_t *vt)
{
  uword n_sessions = sfdp_num_sessions ();

  if (vec_len (vt->session_data) < n_sessions)
    vec_validate_aligned (vt->session_data, n_sessions - 1, CLIB_CACHE_LINE_BYTES);
}

static u32
vt_deleted_sessions_cb (const u32 *session_indices, u32 n_sessions)
{
  verdict_testbench_main_t *vt = &verdict_testbench_main;
  vnet_main_t *vnm = vnet_get_main ();
  u32 *flow_indices = 0;

  /* Iterate over deleted sessions, and check flow_index_verdict to verify if HW flow
     offload entry must be removed */
  for (u32 i = 0; i < n_sessions; i++)
    {
      sfdp_session_t *s = sfdp_session_at_index (session_indices[i]);
      for (int d = 0; d < SFDP_FLOW_F_B_N; d++)
	{
	  if (s->flow_index_verdict[d] != ~0)
	    {
	      vec_add1 (flow_indices, s->flow_index_verdict[d]);
	      s->flow_index_verdict[d] = ~0;
	      sfdp_flow_aging_context_invalidate (
		&sfdp_main, sfdp_flow_aging_context_index (session_indices[i], d));
	    }
	}
    }
  ...
  if (vec_len (flow_indices) > 0)
    {
      /* TODO - this is done based on the hw_if_index configured with the verdict testbench */
      /* there could be potentially be different hw_if_index to take into consideration for each
	flow similarly to flow lookup offload */
      vnet_flow_async_range_disable (vnm, flow_indices, vt->hw_if_index);
      u32 *fi;
...
SFDP_REGISTER_DELETED_SESSIONS_CALLBACK (verdict_testbench) = {
  .fun = vt_deleted_sessions_cb,
};
...
clib_error_t *
verdict_testbench_enable (verdict_testbench_main_t *vt, u32 tx_sw_if_index, u8 enable_counters,
			  u8 enable_aging, u32 age_timeout, u8 protos)
{
  vnet_main_t *vnm = vnet_get_main ();
  ...
  if (enable_aging)
    {
      /* register verdict testbench flow aging service */
      int rv =
	sfdp_flow_aging_service_register ("sfdp-verdict-testbench", &vt->flow_aging_service_index);
      if (rv)
	{
	  if (protos & VT_PROTO_UDP)
	    {
	  ...
	  return clib_error_return (0, "flow aging service registration failed: %d", rv);
	}
    }
  ...
  vt->enable_aging = enable_aging;
  vt->age_timeout = age_timeout;

  /* partial verdict-offload timeout is twice the configured aging timeout
   * configured with the verdict testbench */
  if (enable_aging)
    sfdp_set_offload_keepalive (2.0 * (f64) age_timeout);
...
verdict_testbench_init (vlib_main_t *vm)
{
  ...
  vt->udp_template_index = ~0;
  vt->tcp_template_index = ~0;
  vt->flow_aging_service_index = VNET_FLOW_AGING_SERVICE_INVALID;
  vt->is_enabled = 0;
  vt->templates_on_hw = 0;
  vt->enabled_protos = 0;
  vt_session_data_validate (vt);
...
VLIB_INIT_FUNCTION (verdict_testbench_init) = {
  .runs_after = VLIB_INITS ("sfdp_init", "vnet_flow_init"),
};
