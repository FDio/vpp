 ...
 *
 * HACK: This entire service is a testbench for benchmarking NIC verdict
 * offload on BlueField DPUs. It is not intended for production use.
 */
...
/*
 * HACK: Abuse session->unused0[0..1] as a u16 packet counter for this
 * testbench. This avoids per-session allocations and stats segment reads.
 * A proper implementation would add a dedicated field to sfdp_session_t.
 */
  ...
  while (n_left)
    {
      ...
      /* HACK: increment counter in unused0 */
