...
set_verdict_testbench_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  u32 age_timeout = VT_DEFAULT_AGE_TIMEOUT;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	enable_counters = 1;
      else if (unformat (input, "enable-aging"))
	enable_aging = 1;
      else if (unformat (input, "age-timeout %u", &age_timeout))
	;
  ...
  return verdict_testbench_enable (vt, tx_sw_if_index, enable_counters, enable_aging, age_timeout,
				   protos);
}
...
VLIB_CLI_COMMAND (set_verdict_testbench_command, static) = {
  ...
  .short_help = "set sfdp verdict-testbench tx-interface <if> "
		"[rx-interface <if>] [enable-counters] [enable-aging] [age-timeout <seconds>] "
		"[udp] [tcp] [disable]",
  ...
  if (vt->is_enabled)
    {
      ...
      vlib_cli_output (vm, "  aging: %s", vt->enable_aging ? "enabled" : "disabled");
      if (vt->enable_aging)
	vlib_cli_output (vm, "  age-timeout: %u seconds", vt->age_timeout);
      vlib_cli_output (vm, "  protocols: %s%s", (vt->enabled_protos & VT_PROTO_UDP) ? "udp " : "",
		       (vt->enabled_protos & VT_PROTO_TCP) ? "tcp" : "");
      if (vt->enabled_protos & VT_PROTO_UDP)
	vlib_cli_output (vm, "  udp_template_index: %u", vt->udp_template_index);
      if (vt->enabled_protos & VT_PROTO_TCP)
	vlib_cli_output (vm, "  tcp_template_index: %u", vt->tcp_template_index);
    }
				    ...
				    vlib_cli_command_t *cmd)
{
  ...
  pool_foreach (f, fm->global_flow_pool)
	...
	continue;
      vnet_flow_counter_t counter = {};
      int rv = vnet_flow_get_counter (vnm, f->index, &counter);
      if (rv)
	continue;
      ...
      if (f->type == VNET_FLOW_TYPE_IP4_N_TUPLE)
	{
	  ...
	  vlib_cli_output (vm, "  flow %u: %U:%u -> %U:%u proto %u  hits %lu bytes %lu", f->index,
			   ...
			   t->protocol.prot, counter.hits, counter.bytes);
	}
      else
	{
	  vlib_cli_output (vm, "  flow %u: hits %lu bytes %lu", f->index, counter.hits,
			   counter.bytes);
	}
      n_flows++;
    }
