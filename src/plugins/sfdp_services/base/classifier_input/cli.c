				    ...
				    vlib_cli_command_t *cmd)
{
  ...
  err = sfdp_classifier_input_set_table (table_index, is_del);
...
static clib_error_t *
sfdp_classifier_input_add_del_session_fn (vlib_main_t *vm,
					  ...
					  vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	is_del = 1;
      else if (unformat (line_input, "match %U", unformat_classify_match, vcm,
			 &match, scim->classify_table_index))
	;
      else
	{
	  err = unformat_parse_error (line_input);
  ...
  err = sfdp_classifier_input_add_del_session (tenant_id, match,
...
VLIB_CLI_COMMAND (sfdp_classifier_input_add_del_session_cmd, static) = {
  ...
  .short_help =
    "sfdp classifier-input session tenant <tenant-id> match <match> [del]",
  .function = sfdp_classifier_input_add_del_session_fn,
};

static clib_error_t *
sfdp_classifier_input_enable_disable_fn (vlib_main_t *vm,
					 unformat_input_t *input,
					 vlib_cli_command_t *cmd)
{
  unformat_input_t line_input_, *line_input = &line_input_;
  ...
  if (rv)
    err = clib_error_return (0, "Failed to %s feature on interface",
...
VLIB_CLI_COMMAND (sfdp_classifier_input_enable_disable_cmd, static) = {
  ...
  .short_help = "set sfdp classifier-input interface <interface> [disable]",
  .function = sfdp_classifier_input_enable_disable_fn,
};

static clib_error_t *
show_sfdp_classifier_input_fn (vlib_main_t *vm, unformat_input_t *input,
			       vlib_cli_command_t *cmd)
{
  ...
  if (scim->classify_table_index == ~0)
	...
	pool_elt_at_index (vcm->tables, scim->classify_table_index);
