...
format_sfdp_classifier_input_trace (u8 *s, va_list *args)
{
  ...
  sfdp_classifier_input_trace_t *t =
    va_arg (*args, sfdp_classifier_input_trace_t *);
...
#define foreach_sfdp_classifier_input_next _ (LOOKUP, "sfdp-lookup-ip4")
...
typedef enum
{
#define _(s, n) SFDP_CLASSIFIER_INPUT_NEXT_##s,
  foreach_sfdp_classifier_input_next
#undef _
    SFDP_CLASSIFIER_INPUT_N_NEXT
} sfdp_classifier_input_next_t;

static_always_inline uword
sfdp_classifier_input_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			      vlib_frame_t *frame)
{
  ...
  if (PREDICT_TRUE (t != NULL))
      ...
      while (n > 0)
	{
	  ...
	  vnet_buffer (bp[0])->l2_classify.hash =
	    vnet_classify_hash_packet_inline (t, h);
	  vnet_classify_prefetch_bucket (t,
					 vnet_buffer (bp[0])->l2_classify.hash);
	  bp += 1;
	  n -= 1;
	}
  ...
  while (n_left)
      ...
      if (sfdp_classifier_input_get_tenant_idx (e->opaque_index, &tenant_idx))
	{
      ...
      current_next[0] = SFDP_CLASSIFIER_INPUT_NEXT_LOOKUP;
	...
	{
	  sfdp_classifier_input_trace_t *tr =
	    vlib_add_trace (vm, node, b[0], sizeof (*tr));
	  tr->sw_if_index = rx_sw_if_index;
	  tr->table_index = table_index;
	  tr->hash = hash;
	  tr->hit = hit;
	  tr->tenant_id = (tenant_idx != (u16) ~0)
			    ? sfdp_tenant_at_index (sfdp, tenant_idx)->tenant_id
			    : ~0;
	}
...
VLIB_NODE_FN (sfdp_classifier_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return sfdp_classifier_input_inline (vm, node, frame);
}

VLIB_REGISTER_NODE (sfdp_classifier_input_node) = {
  .name = "sfdp-classifier-input",
  ...
  .n_next_nodes = SFDP_CLASSIFIER_INPUT_N_NEXT,
  .next_nodes = {
          [SFDP_CLASSIFIER_INPUT_NEXT_LOOKUP] = "sfdp-lookup-ip4",
  },
};

VNET_FEATURE_INIT (sfdp_classifier_input_feat, static) = {
  .arc_name = "ip4-unicast",
  .node_name = "sfdp-classifier-input",
};
