...
format_sfdp_classifier_input_trace (u8 *s, va_list *args)
{
  ...
  s = format (s,
	      ...
	      "hit %d",
	      t->sw_if_index, t->table_index, t->hash, t->hit);
			      ...
			      u8 is_ip6)
{
  ...
  while (n > 0)
    {
      u32 sw_if = vnet_buffer (bp[0])->sw_if_index[VLIB_RX];
      u32 tbl_idx = (sw_if < vec_len (table_index_by_sw_if)) ? table_index_by_sw_if[sw_if] : ~0;
      if (tbl_idx != ~0)
	{
	  vnet_classify_table_t *t = pool_elt_at_index (vcm->tables, tbl_idx);
  ...
  while (n_left)
    {
      ...
      u32 tbl_idx = (rx_sw_if_index < vec_len (table_index_by_sw_if)) ?
		      table_index_by_sw_if[rx_sw_if_index] :
		      ~0;
      u16 tenant_idx = ~0;
      ...
      if (PREDICT_FALSE (e == NULL))
	{
      ...
      /* opaque_index stores tenant_idx */
      u32 oi = e->opaque_index;
      if (PREDICT_FALSE (pool_is_free_index (sfdp->tenants, oi)))
	{
	  vnet_feature_next_u16 (current_next, b[0]);
	  goto trace;
	}
			 ...
			 (b[0]->flags & VLIB_BUFFER_IS_TRACED)))
	{
	  ...
	  tr->table_index = tbl_idx;
	  tr->hash = hash;
	  tr->hit = hit;
	  tr->tenant_id = hit ? sfdp_tenant_at_index (sfdp, tenant_idx)->tenant_id : 0;
	}
