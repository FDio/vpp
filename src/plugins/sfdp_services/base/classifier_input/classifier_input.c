...
sfdp_classifier_input_set_table (u32 table_index, u8 is_del)
{
  ...
  if (pool_is_free_index (vcm->tables, table_index))
    return clib_error_return (0, "Classifier table %d does not exist", table_index);
...
sfdp_classifier_input_add_del_session (u32 tenant_id, const u8 *match, u32 match_len, u8 is_del)
{
  ...
  if (match_len != expected_match_len)
    return clib_error_return (0, "Match length %d does not match expected %d (skip=%d, match=%d)",
  ...
  if (is_del)
	...
	return clib_error_return (0, "Classifier session not found");
      ...
      rv = vnet_classify_add_del_session (cm, scim->classify_table_index, match,
  ...
  *tenant_entry = tenant_idx + 1;
  ...
  rv = vnet_classify_add_del_session (
    cm, scim->classify_table_index, match, ~0, /* hit_next_index - use default */
