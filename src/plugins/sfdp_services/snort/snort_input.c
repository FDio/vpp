...
#define foreach_sfdp_snort_input_error                                        \
  _ (ENQUEUED, "Packets enqueued to snort")
...
format_sfdp_snort_input_trace (u8 *s, va_list *args)
{
  ...
  return format (s, "buffer-index %u instance %u", t->buffer_index,
		 t->instance);
}

static_always_inline uword
sfdp_snort_input_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			 vlib_frame_t *frame)
{
  ...
  daq_vpp_pkt_metadata_t metadata = {
    .flags = DAQ_PKT_FLAG_PRE_ROUTING,
  };
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       SFDP_SNORT_INPUT_ERROR_ENQUEUED, n_enq);
  return n_pkts;
}
...
SFDP_SERVICE_DEFINE (sfdp_snort_input) = { .node_name = "sfdp-snort-input",
