...
static snort_instance_get_index_by_name_fn_t
  *snort_instance_get_index_by_name_ptr = 0;

clib_error_t *
sfdp_snort_set_functions (void)
{
  clib_error_t *err = 0;
  snort_instance_create_ptr =
    vlib_get_plugin_symbol ("snort_plugin.so", "snort_instance_create");
  if (!snort_instance_create_ptr)
    {
      err =
	clib_error_return (0, "Could not find symbol snort_instance_create");
    }

  snort_instance_delete_ptr =
    vlib_get_plugin_symbol ("snort_plugin.so", "snort_instance_delete");
  if (!snort_instance_delete_ptr)
    {
      err =
	clib_error_return (0, "Could not find symbol snort_instance_delete");
    }

  snort_instance_get_index_by_name_ptr = vlib_get_plugin_symbol (
    "snort_plugin.so", "snort_instance_get_index_by_name");
  if (!snort_instance_get_index_by_name_ptr)
    {
      err = clib_error_return (
	0, "Could not find symbol snort_instance_get_index_by_name");
    }
  return err;
}
...
sfdp_snort_add_next_node (vlib_main_t *vm, u8 *name)
{
  ...
  snort_instance_get_index_by_name_ptr (vm, (char *) name,
					&vsm->instance_index);
  ...
  if (!node)
    {
      err = clib_error_return (
	0, "snort dequeue node 'snort-deq-%s' not found", name);
      goto done;
    }
...
static clib_error_t *
sfdp_snort_create_instance_command_fn (vlib_main_t *vm,
				       ...
				       vlib_cli_command_t *cmd)
{
  ...
  rv = snort_instance_create_ptr (
    ...
    &(snort_instance_create_args_t){
      ...
      .drop_bitmap =
	1 << SFDP_DAQ_VERDICT_BLOCK | 1 << SFDP_DAQ_VERDICT_BLACKLIST,
...
static clib_error_t *
sfdp_snort_delete_instance_command_fn (vlib_main_t *vm,
				       unformat_input_t *input,
				       vlib_cli_command_t *cmd)
{
  clib_error_t *err = 0;
  ...
  rv =
    snort_instance_get_index_by_name_ptr (vm, (char *) name, &instance_index);
  if (rv < 0)
    {
      err = clib_error_return (
	0, "snort_instance_get_index_by_name failed: %d, %U", (int) rv,
  ...
  if (rv < 0)
    {
      err = clib_error_return (0, "snort_instance_delete failed: %d,  %U",
