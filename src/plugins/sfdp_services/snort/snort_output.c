...
format_sfdp_snort_verdict (u8 *s, va_list *args)
{
  ...
  static char *strings[SFDP_MAX_DAQ_VERDICT] = {
    [SFDP_DAQ_VERDICT_PASS] = "PASS",		[SFDP_DAQ_VERDICT_BLOCK] = "BLOCK",
    [SFDP_DAQ_VERDICT_REPLACE] = "REPLACE",	[SFDP_DAQ_VERDICT_WHITELIST] = "WHITELIST",
    [SFDP_DAQ_VERDICT_BLACKLIST] = "BLACKLIST", [SFDP_DAQ_VERDICT_IGNORE] = "IGNORE",
  };
...
format_sfdp_snort_output_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "sfdp-snort-output: buffer-index %d next-index %d verdict %U", t->buffer_index,
	      t->next_index, format_sfdp_snort_verdict, t->verdict);
  return s;
}
...
sfdp_snort_output_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  for (; n_left >= 8; n_left -= 4, bi += 4, ni += 4)
    {
      ...
      daq_vpp_pkt_metadata_t metadata0 = {}, metadata1 = {}, metadata2 = {}, metadata3 = {};
	...
	{
	  sfdp_snort_output_trace_t *t = vlib_add_trace (vm, node, b0, sizeof (*t));
	  t->buffer_index = bi[0];
	  t->next_index = ni[0];
	...
	{
	  sfdp_snort_output_trace_t *t = vlib_add_trace (vm, node, b1, sizeof (*t));
	  t->buffer_index = bi[1];
	  t->next_index = ni[1];
	...
	{
	  sfdp_snort_output_trace_t *t = vlib_add_trace (vm, node, b2, sizeof (*t));
	  t->buffer_index = bi[2];
	  t->next_index = ni[2];
	...
	{
	  sfdp_snort_output_trace_t *t = vlib_add_trace (vm, node, b3, sizeof (*t));
	  t->buffer_index = bi[3];
	  t->next_index = ni[3];
  ...
  vlib_node_increment_counter (vm, node->node_index, SFDP_SNORT_OUTPUT_ERROR_INVALID_SESSION,
			       n_invalid_session_pkt_drop);
...
SFDP_SERVICE_DEFINE (sfdp_snort_output) = { .node_name = "sfdp-snort-output",
