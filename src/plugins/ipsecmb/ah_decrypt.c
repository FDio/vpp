...
always_inline void
remove_ah (vlib_main_t * vm, u32 * bi0, u32 * next0,
	   ipsec_sa_t * sa0, u32 ip_hdr_size, u32 icv_size,
  ...
  if (PREDICT_TRUE (sa0->is_tunnel || sa0->is_tunnel_ip6))
	...
	*next0 = AH_DECRYPT_NEXT_IP6_INPUT;
      else
	{
	  ...
	  if (is_ip6)
	    vlib_node_increment_counter (vm, ip6_ah_decrypt_node.index,
					 AH_DECRYPT_ERROR_DECRYPTION_FAILED,
					 1);
	  else
	    vlib_node_increment_counter (vm, ip4_ah_decrypt_node.index,
					 AH_DECRYPT_ERROR_DECRYPTION_FAILED,
					 1);
...
always_inline void
ah_finish_decrypt (vlib_main_t * vm, JOB_AES_HMAC * job,
		   u32 * bi0, u32 * next0, int is_ip6)
{
  ...
  *bi0 = (uintptr_t) job->user_data;
  ...
  remove_ah (vm, bi0, next0, sa0, ip_hdr_size, icv_size, icv_padding_len, ah0,
	     is_ip6);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	      ...
	      if (PREDICT_FALSE (rv))
		{
		  ...
		  if (is_ip6)
		    vlib_node_increment_counter (vm,
						 ip6_ah_decrypt_node.index,
						 AH_DECRYPT_ERROR_REPLAY, 1);
		  else
		    vlib_node_increment_counter (vm,
						 ip4_ah_decrypt_node.index,
						 AH_DECRYPT_ERROR_REPLAY, 1);
		  to_next[0] = bi0;
		  to_next += 1;
		  goto trace;
	  ...
	  if (PREDICT_TRUE (sa0->integ_alg != IPSEC_INTEG_ALG_NONE))
	      ...
	      if (!job)
		{
	      ...
	      ah_finish_decrypt (vm, job, &bi0, &next0, is_ip6);
	    }
	  else
	    {
	      remove_ah (vm, &bi0, &next0, sa0, ip_hdr_size, icv_size, 0, ah0,
			 is_ip6);
	    }
	  to_next[0] = bi0;
	  to_next += 1;
...
static uword
ip4_ah_decrypt_ipsecmb_node_fn (vlib_main_t * vm,
				vlib_node_runtime_t * node,
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_ah_decrypt_ipsecmb_node) = {
    .function = ip4_ah_decrypt_ipsecmb_node_fn,
    .name = "ip4-ah-decrypt-ipsecmb",
...
VLIB_NODE_FUNCTION_MULTIARCH (ip4_ah_decrypt_ipsecmb_node,
			      ip4_ah_decrypt_ipsecmb_node_fn);
static uword
ip6_ah_decrypt_ipsecmb_node_fn (vlib_main_t * vm,
				vlib_node_runtime_t * node,
				vlib_frame_t * from_frame)
{
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip6_ah_decrypt_ipsecmb_node) = {
    .function = ip6_ah_decrypt_ipsecmb_node_fn,
    .name = "ip6-ah-decrypt-ipsecmb",
    .vector_size = sizeof (u32),
    .format_trace = format_ah_decrypt_trace,
    .type = VLIB_NODE_TYPE_INTERNAL,
...
VLIB_NODE_FUNCTION_MULTIARCH (ip6_ah_decrypt_ipsecmb_node,
			      ip6_ah_decrypt_ipsecmb_node_fn);
/*
