  ...
  if (PREDICT_TRUE (sa0->is_tunnel || sa0->is_tunnel_ip6))
	...
	*next0 = AH_DECRYPT_NEXT_IP6_INPUT;
      else
	{
	  ...
	  if (is_ip6)
	    vlib_node_increment_counter (vm, ah6_decrypt_ipsecmb_node.index,
					 AH_DECRYPT_ERROR_DECRYPTION_FAILED,
					 1);
	  else
	    vlib_node_increment_counter (vm, ah4_decrypt_ipsecmb_node.index,
					 AH_DECRYPT_ERROR_DECRYPTION_FAILED,
					 1);
	      ...
	      if (PREDICT_FALSE (rv))
		{
		  ...
		  if (is_ip6)
		    vlib_node_increment_counter (vm,
						 ah6_decrypt_ipsecmb_node.index,
						 AH_DECRYPT_ERROR_REPLAY, 1);
		  else
		    vlib_node_increment_counter (vm,
						 ah4_decrypt_ipsecmb_node.index,
						 AH_DECRYPT_ERROR_REPLAY, 1);
		  to_next[0] = bi0;
		  to_next += 1;
		  goto trace;
...
static uword
ah4_decrypt_ipsecmb_node_fn (vlib_main_t * vm,
			     vlib_node_runtime_t * node,
...
VLIB_REGISTER_NODE (ah4_decrypt_ipsecmb_node) = {
    .function = ah4_decrypt_ipsecmb_node_fn,
...
VLIB_NODE_FUNCTION_MULTIARCH (ah4_decrypt_ipsecmb_node,
			      ah4_decrypt_ipsecmb_node_fn);
static uword
ah6_decrypt_ipsecmb_node_fn (vlib_main_t * vm,
			     vlib_node_runtime_t * node,
			     vlib_frame_t * from_frame)
{
...
VLIB_REGISTER_NODE (ah6_decrypt_ipsecmb_node) = {
    .function = ah6_decrypt_ipsecmb_node_fn,
...
VLIB_NODE_FUNCTION_MULTIARCH (ah6_decrypt_ipsecmb_node,
			      ah6_decrypt_ipsecmb_node_fn);
/*
