			    ...
			    vlib_frame_t * from_frame, int is_ip6)
{
  ...
  esp_decrypt_prepare_jobs (vm, im, em, b, job, n_jobs, is_ip6,
			    ESP_DECRYPT_NEXT_DROP);

  ipsecmb_process_jobs (vm, im, imbm, imbt->mb_mgr, t->jobs, n_jobs,
			0 /* is_encrypt */ , IPSECMB_FUNC (get_next_job),
			IPSECMB_FUNC (submit_job), IPSECMB_FUNC (flush_job),
			is_ip6, ESP_DECRYPT_NEXT_DROP);

  // wrap things up
  u16 nexts[VLIB_FRAME_SIZE], *next = nexts;

  esp_finish_decrypt (vm, next, job, n_jobs, is_ip6, ESP_DECRYPT_NEXT_DROP,
