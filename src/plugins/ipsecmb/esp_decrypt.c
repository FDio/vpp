...
vlib_node_registration_t ip6_esp_decrypt_ipsecmb_node;
vlib_node_registration_t ip4_esp_decrypt_ipsecmb_node;

always_inline void
esp_finish_decrypt (vlib_main_t * vm, JOB_AES_HMAC * job, u32 * next0,
		    ipsec_sa_t * sa0, int is_ip6)
{
		   ...
		   job->auth_tag_output_len_in_bytes)))
	{
	  if (is_ip6)
	    vlib_node_increment_counter (vm,
					 ip6_esp_decrypt_ipsecmb_node.index,
					 ESP_DECRYPT_ERROR_INTEG_ERROR, 1);
	  else
	    vlib_node_increment_counter (vm,
					 ip4_esp_decrypt_ipsecmb_node.index,
					 ESP_DECRYPT_ERROR_INTEG_ERROR, 1);
			    ...
			    vlib_frame_t * from_frame, int is_ip6)
{
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if ((sa0->crypto_alg >= IPSEC_CRYPTO_ALG_AES_CBC_128 &&
	      ...
	      if (!job)
		{
	      ...
	      b0 = vlib_get_buffer (vm, bi0);
	      esp_finish_decrypt (vm, job, &next0, sa0, is_ip6);
  ...
  if (is_ip6)
    vlib_node_increment_counter (vm, ip6_esp_decrypt_ipsecmb_node.index,
				 ESP_DECRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
  else
    vlib_node_increment_counter (vm, ip4_esp_decrypt_ipsecmb_node.index,
				 ESP_DECRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
...
static uword
ip4_esp_decrypt_ipsecmb_node_fn (vlib_main_t * vm,
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_esp_decrypt_ipsec_mb_node) = {
  .function = ip4_esp_decrypt_ipsecmb_node_fn,
  .name = "ip4-esp-decrypt-ipsecmb",
...
VLIB_NODE_FUNCTION_MULTIARCH (ip4_esp_decrypt_ipsecmb_node,
			      ip4_esp_decrypt_ipsecmb_node_fn);

static uword
ip6_esp_decrypt_ipsecmb_node_fn (vlib_main_t * vm,
				 vlib_node_runtime_t * node,
				 vlib_frame_t * from_frame)
{
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip6_esp_decrypt_ipsec_mb_node) = {
  .function = ip6_esp_decrypt_ipsecmb_node_fn,
  .name = "ip6-esp-decrypt-ipsecmb",
  .vector_size = sizeof (u32),
  .format_trace = format_esp_decrypt_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
...
VLIB_NODE_FUNCTION_MULTIARCH (ip6_esp_decrypt_ipsecmb_node,
			      ip6_esp_decrypt_ipsecmb_node_fn);
/*
