		   ...
		   job->auth_tag_output_len_in_bytes)))
	{
	  if (is_ip6)
	    vlib_node_increment_counter (vm,
					 esp6_decrypt_ipsecmb_node.index,
					 ESP_DECRYPT_ERROR_INTEG_ERROR, 1);
	  else
	    vlib_node_increment_counter (vm,
					 esp4_decrypt_ipsecmb_node.index,
					 ESP_DECRYPT_ERROR_INTEG_ERROR, 1);
			    ...
			    vlib_frame_t * from_frame, int is_ip6)
{
  ...
  if (is_ip6)
    vlib_node_increment_counter (vm, esp6_decrypt_ipsecmb_node.index,
				 ESP_DECRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
  else
    vlib_node_increment_counter (vm, esp4_decrypt_ipsecmb_node.index,
				 ESP_DECRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
