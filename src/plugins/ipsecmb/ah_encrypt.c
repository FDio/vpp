		   ...
		   int is_ip6)
{
  if (is_ip6)
	...
	vnet_buffer (b0)->ipsec.ip_version_traffic_class_and_flow_label;
      oh6->hop_limit = vnet_buffer (b0)->ipsec.hop_limit;
    }
  else
    {
      ip4_header_t *oh4 = 0;
      ...
      else
	{
      ...
      oh4->ttl = vnet_buffer (b0)->ipsec.ttl;
			   ...
			   vlib_frame_t * from_frame, int is_ip6)
{
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (esp_seq_advance (sa0)))
		...
		vlib_node_increment_counter (vm,
					     ah6_encrypt_ipsecmb_node.index,
					     AH_ENCRYPT_ERROR_SEQ_CYCLED, 1);
	      else
		vlib_node_increment_counter (vm,
					     ah4_encrypt_ipsecmb_node.index,
					     AH_ENCRYPT_ERROR_SEQ_CYCLED, 1);
	      to_next[0] = bi0;
	      to_next += 1;
	      goto trace;
	  ...
	  if (is_ip6)
	    {			/* is ipv6 */
	      ...
	      vnet_buffer (b0)->ipsec.hop_limit = ih6->hop_limit;
	      vnet_buffer (b0)->
	  ...
	  if (PREDICT_TRUE
	      (!is_ip6 && sa0->is_tunnel && !sa0->is_tunnel_ip6))
	    {
  ...
  if (is_ip6)
    vlib_node_increment_counter (vm, ah6_encrypt_ipsecmb_node.index,
				 AH_ENCRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
  else
    vlib_node_increment_counter (vm, ah4_encrypt_ipsecmb_node.index,
				 AH_ENCRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
