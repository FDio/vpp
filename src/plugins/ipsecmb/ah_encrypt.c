			   ...
			   vlib_frame_t * from_frame, int is_ip6)
{
  ...
  ah_encrypt_prepare_jobs (vm, im, em, b, job, n_jobs, is_ip6,
			   AH_ENCRYPT_NEXT_DROP, AH_ENCRYPT_NEXT_IP4_LOOKUP,
			   AH_ENCRYPT_NEXT_IP6_LOOKUP,
  ...
  ipsecmb_process_jobs (vm, im, &ipsecmb_main,
			...
			t->jobs, n_jobs, 1 /* is_encrypt */ ,
			IPSECMB_FUNC (get_next_job),
			IPSECMB_FUNC (submit_job), IPSECMB_FUNC (flush_job),
			is_ip6, AH_ENCRYPT_NEXT_DROP);

  // wrap things up
  u16 nexts[VLIB_FRAME_SIZE], *next = nexts;
  ah_finish_encrypt (vm, im, next, job, n_jobs, is_ip6);
