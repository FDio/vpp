			       ...
			       u32 thread_index, void *from, u8 size)
{
  ...
  u32 idx;
  random_bytes_t *rb;
  if (PREDICT_TRUE (vec_len (imbm->rb_recycle_lists[thread_index])))
    {
...
random_bytes (ipsecmb_main_t * imbm, u32 thread_index, u8 * where, u8 size)
{
  ...
  if (PREDICT_TRUE (vec_len (imbm->rb_from_traffic[thread_index])))
    {
  ...
  if (PREDICT_FALSE (0 == vec_len (imbm->rb_from_dev_urandom[thread_index])))
    {
      ssize_t bytes_read =
	read (imbm->dev_urandom_fd, imbm->urandom_buffers[thread_index],
	      sizeof (imbm->urandom_buffers[thread_index]));
      if (bytes_read < 0)
	{
      ...
      if (bytes_read < block_size)
	{
      ...
      const ssize_t limit =
	clib_min (bytes_read, sizeof (imbm->urandom_buffers[thread_index]));
      int i;
      for (i = 0;
	   limit - i >= block_size
	   && vec_len (imbm->rb_recycle_lists[thread_index]) > 0;
	   i += block_size)
	{
	  u32 idx = vec_pop (imbm->rb_recycle_lists[thread_index]);
	  random_bytes_t *rb =
	    pool_elt_at_index (imbm->rb_pools[thread_index], idx);
	  clib_memcpy (rb->data, imbm->urandom_buffers[thread_index] + i,
		       block_size);
...
always_inline void
do_tunnel (vlib_buffer_t * b0, ipsec_sa_t * sa0, u32 * next0,
	   u8 * next_hdr_type, ip4_header_t * ih4, ip4_header_t ** oh4,
	   ip6_header_t * ih6, ip6_header_t ** oh6, esp_header_t ** esp,
	   u32 iv_size, int is_ip6)
{
  if (is_ip6)
    {
      *next0 = ESP_ENCRYPT_NEXT_IP6_LOOKUP;
      ASSERT (sa0->is_tunnel_ip6);
...
always_inline void
do_transport (vlib_buffer_t * b0, ipsec_sa_t * sa0, u32 * next0,
	      u8 * next_hdr_type, ip4_header_t * ih4, ip4_header_t ** oh4,
	      ip6_header_t * ih6, ip6_header_t ** oh6, esp_header_t ** esp,
	      u32 iv_size, int is_ip6)
{
			    ...
			    vlib_frame_t * from_frame, int is_ip6)
{
  ...
  while (n_left_from > 0 || packets_in_flight > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  const int iv_size = imbm->crypto_algs[sa0->crypto_alg].iv_size;
	  if (sa0->is_tunnel)
	    do_tunnel (b0, sa0, &next0, &next_hdr_type, ih4, &oh4, ih6, &oh6,
		       &esp, iv_size, is_ip6);
	  else
	    do_transport (b0, sa0, &next0, &next_hdr_type, ih4, &oh4, ih6,
			  &oh6, &esp, iv_size, is_ip6);
