			       ...
			       u32 thread_index, void *from, u8 size)
{
  ...
  u32 idx;
  random_bytes_t *rb;
  if (PREDICT_TRUE (0 < vec_len (imbm->rb_recycle_lists[thread_index])))
    {
      idx = vec_pop (imbm->rb_recycle_lists[thread_index]);
      rb = pool_elt_at_index (imbm->rb_pools[thread_index], idx);
    }
  else
    {
      pool_get (imbm->rb_pools[thread_index], rb);
      idx = rb - imbm->rb_pools[thread_index];
    }
  clib_memcpy (rb->data, from, STRUCT_SIZE_OF (random_bytes_t, data));
  vec_add1 (imbm->rb_from_traffic[thread_index], idx);
}
...
random_bytes (ipsecmb_main_t * imbm, u32 thread_index, u8 * where, u8 size)
{
  ...
  if (PREDICT_TRUE (0 < vec_len (imbm->rb_from_traffic[thread_index])))
    {
      u32 idx = vec_pop (imbm->rb_from_traffic[thread_index]);
      random_bytes_t *rb =
	pool_elt_at_index (imbm->rb_pools[thread_index], idx);
      clib_memcpy (where, rb->data, block_size);
      vec_add1 (imbm->rb_recycle_lists[thread_index], idx);
      return 0;
    }
  if (PREDICT_FALSE (0 == vec_len (imbm->rb_from_dev_urandom[thread_index])))
    {
      u8 buf[256 * block_size];
      ssize_t bytes_read = read (imbm->dev_urandom_fd, buf, sizeof (buf));
      if (bytes_read < 0)
	{
	  clib_unix_warning ("read() from /dev/random failed");
	  return -1;
	}
      const ssize_t limit = clib_min (bytes_read, sizeof (buf));
      int i;
      for (i = 0;
	   i < limit && vec_len (imbm->rb_recycle_lists[thread_index]) > 0;
	   i += block_size)
	{
	  u32 idx = vec_pop (imbm->rb_recycle_lists[thread_index]);
	  random_bytes_t *rb =
	    pool_elt_at_index (imbm->rb_pools[thread_index], idx);
	  clib_memcpy (rb->data, buf + i, block_size);
	  vec_add1 (imbm->rb_from_dev_urandom[thread_index], idx);
	}
      for (; i < limit; i += block_size)
	{
	  random_bytes_t *rb;
	  pool_get (imbm->rb_pools[thread_index], rb);
	  clib_memcpy (rb->data, buf + i, block_size);
	  vec_add1 (imbm->rb_from_dev_urandom[thread_index],
		    rb - imbm->rb_pools[thread_index]);
	}
    }
  u32 idx = vec_pop (imbm->rb_from_dev_urandom[thread_index]);
  random_bytes_t *rb = pool_elt_at_index (imbm->rb_pools[thread_index], idx);
  clib_memcpy (where, rb->data, block_size);
  vec_add1 (imbm->rb_recycle_lists[thread_index], idx);
  return 0;
}
		    ...
		    u32 * bi0, u32 * next0, ipsec_sa_t ** sa0, int is_ip6)
{
  ...
  *sa0 = pool_elt_at_index (im->sad, sa_index0);
  if ((*sa0)->is_tunnel)
    ...
    {
      oh4 = (ip4_header_t *) ((u8 *) vlib_buffer_get_current (b0) +
			      sizeof (ethernet_header_t));
      oh6 = (ip6_header_t *) ((u8 *) vlib_buffer_get_current (b0) +
			      sizeof (ethernet_header_t));
    }
  if (is_ip6)
    {
			    ...
			    vlib_frame_t * from_frame, int is_ip6)
{
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (esp_seq_advance (sa0)))
		...
		vlib_node_increment_counter (vm,
					     esp6_encrypt_ipsecmb_node.index,
					     ESP_ENCRYPT_ERROR_SEQ_CYCLED, 1);
	      else
		vlib_node_increment_counter (vm,
					     esp4_encrypt_ipsecmb_node.index,
					     ESP_ENCRYPT_ERROR_SEQ_CYCLED, 1);
	      // TODO: rekey SA
	      to_next[0] = bi0;
	      to_next += 1;
	  ...
	  if (is_ip6)
		...
		{		/* tunnel mode */
		  ...
		  oh4 =
		    (ip4_header_t *) (((u8 *) ih4) - sizeof (ip4_header_t) -
				      sizeof (esp_header_t) - udp_hdr_size -
				      iv_size);
		...
		{		/* transport mode */
		  ...
		  oh4 = (void *) (((u8 *) ih4) - sizeof (esp_header_t) -
				  udp_hdr_size - iv_size);
		  if (vnet_buffer (b0)->sw_if_index[VLIB_TX] != ~0)
		    {
		      ethernet_header_t *ieh0, *oeh0;
	  ...
	  job->src = (u8 *) vlib_buffer_get_current (b0) + payload_offset -
	    (sizeof (esp_header_t) + iv_size);
  ...
  if (is_ip6)
    vlib_node_increment_counter (vm, esp6_encrypt_ipsecmb_node.index,
				 ESP_ENCRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
  else
    vlib_node_increment_counter (vm, esp4_encrypt_ipsecmb_node.index,
				 ESP_ENCRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
  ...
  if (PREDICT_TRUE (vec_len (imbm->rb_from_traffic[thread_index]) > 0))
    {
      ...
      vec_add (imbm->rb_recycle_lists[thread_index],
	       imbm->rb_from_traffic[thread_index],
	       vec_len (imbm->rb_from_traffic[thread_index]));
      _vec_len (imbm->rb_from_traffic[thread_index]) = 0;
    }
