...
vlib_node_registration_t ip4_esp_encrypt_ipsecmb_node;
vlib_node_registration_t ip6_esp_encrypt_ipsecmb_node;
...
static inline void
esp_finish_encrypt (vlib_main_t * vm, JOB_AES_HMAC * job, u32 * bi0,
		    u32 * next0, ipsec_sa_t ** sa0, int is_ip6)
{
  ip4_header_t *oh4 = 0;
  udp_header_t *udp = 0;
  ip6_header_t *oh6 = 0;
			    ...
			    vlib_frame_t * from_frame, int is_ip6)
{
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (esp_seq_advance (sa0)))
		...
		vlib_node_increment_counter (vm,
					     ip6_esp_encrypt_ipsecmb_node.index,
					     ESP_ENCRYPT_ERROR_SEQ_CYCLED, 1);
	      else
		vlib_node_increment_counter (vm,
					     ip4_esp_encrypt_ipsecmb_node.index,
					     ESP_ENCRYPT_ERROR_SEQ_CYCLED, 1);
	      //TODO: rekey SA
	      to_next[0] = bi0;
	      to_next += 1;
	  ...
	  esp_finish_encrypt (vm, job, &bi0, &next0, &sa0, is_ip6);
  ...
  if (is_ip6)
    vlib_node_increment_counter (vm, ip6_esp_encrypt_ipsecmb_node.index,
				 ESP_ENCRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
  else
    vlib_node_increment_counter (vm, ip4_esp_encrypt_ipsecmb_node.index,
				 ESP_ENCRYPT_ERROR_RX_PKTS,
				 from_frame->n_vectors);
...
static uword
ip4_esp_encrypt_ipsecmb_node_fn (vlib_main_t * vm,
				 vlib_node_runtime_t * node,
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_esp_encrypt_ipsecmb_node) = {
  .name = "ip4-esp-encrypt-ipsecmb",
  .function = ip4_esp_encrypt_ipsecmb_node_fn,
...
VLIB_NODE_FUNCTION_MULTIARCH (ip4_esp_encrypt_ipsecmb_node,
			      ip4_esp_encrypt_ipsecmb_node_fn);

static uword
ip6_esp_encrypt_ipsecmb_node_fn (vlib_main_t * vm,
				 vlib_node_runtime_t * node,
				 vlib_frame_t * from_frame)
{
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip6_esp_encrypt_ipsecmb_node) = {
  .name = "ip6-esp-encrypt-ipsecmb",
  .function = ip6_esp_encrypt_ipsecmb_node_fn,
  .vector_size = sizeof (u32),
  .format_trace = format_esp_encrypt_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
...
VLIB_NODE_FUNCTION_MULTIARCH (ip6_esp_encrypt_ipsecmb_node,
			      ip6_esp_encrypt_ipsecmb_node_fn);
/*
