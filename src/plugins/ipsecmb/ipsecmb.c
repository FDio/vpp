...
sa_expand_keys (u32 sa_index)
{
  ...
  if (sa->crypto_key.len > 0)
    {
      const keyexp_t keyexp_fn = imbm->crypto_algs[sa->crypto_alg].keyexp_fn;
      keyexp_fn (sa->crypto_key.data, samb->aes_enc_key_expanded,
