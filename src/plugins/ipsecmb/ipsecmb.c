...
#define __set_funcs(arch)                                               \
  ...
  if (clib_cpu_supports_avx512f ())
    {
      __set_funcs (avx512);
      vec_foreach (t, imbm->per_thread_data)
      {
	t->mb_mgr = alloc_mb_mgr (0);
	init_mb_mgr_avx512 (t->mb_mgr);
      }
    }
...
u8 *
format_ipsecmb_error (u8 * s, va_list * args)
{
  ipsecmb_error_e e = va_arg (*args, ipsecmb_error_e);
  ...
  switch (e)
    ...
    case IMB_ERR_OK:
      s = format (s, "ok");
      break;
    case IMB_ERR_DECRYPTION_FAILED:
      s = format (s, "decrypt-fail");
      break;
    case IMB_ERR_ENCRYPTION_FAILED:
      s = format (s, "encrypt-fail");
      break;
    case IMB_ERR_INTEG_ERROR:
      s = format (s, "integ-error");
      break;
    case IMB_ERR_SEQ_CYCLED:
...
u8 *
format_ipsecmb_sts (u8 * s, va_list * args)
{
  JOB_STS sts = va_arg (*args, JOB_STS);

  switch (sts)
    {
    case STS_COMPLETED:
      s = format (s, "completed");
      break;
    case STS_INVALID_ARGS:
      s = format (s, "invalid-args");
      break;
    case STS_BEING_PROCESSED:
      s = format (s, "being-processed");
      break;
    case STS_COMPLETED_AES:
      s = format (s, "completed-aes");
      break;
    case STS_COMPLETED_HMAC:
      s = format (s, "completed-hmac");
      break;
...
u8 *
format_esp_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  ipsecmb_esp_trace_t *t = va_arg (*args, ipsecmb_esp_trace_t *);

  s =
    format (s,
	    "spi: %u seq: %u crypto: %U integrity: %U error: %U sts: %U%s\n",
	    t->spi, t->seq, format_ipsec_crypto_alg, t->crypto_alg,
	    format_ipsec_integ_alg, t->integ_alg, format_ipsecmb_error,
	    t->error, format_ipsecmb_sts, t->sts,
