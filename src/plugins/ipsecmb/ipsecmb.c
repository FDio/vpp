...
#undef __set_funcs
  ...
  vec_validate (imbm->rb_pools, tm->n_vlib_mains - 1);
  vec_validate (imbm->rb_from_dev_urandom, tm->n_vlib_mains - 1);
  vec_validate (imbm->rb_recycle_lists, tm->n_vlib_mains - 1);
  vec_validate (imbm->rb_from_traffic, tm->n_vlib_mains - 1);
...
ipsecmb_process (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  next_node = vlib_get_node_by_name (vm, (u8 *) "esp4-decrypt-ipsecmb");
  ASSERT (next_node);
  ...
  next_node = vlib_get_node_by_name (vm, (u8 *) "esp6-encrypt-ipsecmb");
  ASSERT (next_node);
  ...
  next_node = vlib_get_node_by_name (vm, (u8 *) "esp6-decrypt-ipsecmb");
  ASSERT (next_node);
  ...
  next_node = vlib_get_node_by_name (vm, (u8 *) "ah4-encrypt-ipsecmb");
  ASSERT (next_node);
  node = vlib_get_node_by_name (vm, (u8 *) "ipsec4-output");
  ASSERT (node);
  u32 ah4_encrypt_node_index = next_node->index;
  u32 ah4_encrypt_next_index =
    vlib_node_add_next (vm, node->index, next_node->index);

  next_node = vlib_get_node_by_name (vm, (u8 *) "ah4-decrypt-ipsecmb");
  ASSERT (next_node);
  node = vlib_get_node_by_name (vm, (u8 *) "ipsec4-input");
  ASSERT (node);
  u32 ah4_decrypt_node_index = next_node->index;
  u32 ah4_decrypt_next_index =
    vlib_node_add_next (vm, node->index, next_node->index);

  next_node = vlib_get_node_by_name (vm, (u8 *) "ah6-encrypt-ipsecmb");
  ASSERT (next_node);
  node = vlib_get_node_by_name (vm, (u8 *) "ipsec6-output");
  ASSERT (node);
  u32 ah6_encrypt_node_index = next_node->index;
  u32 ah6_encrypt_next_index =
    vlib_node_add_next (vm, node->index, next_node->index);

  next_node = vlib_get_node_by_name (vm, (u8 *) "ah6-decrypt-ipsecmb");
  ASSERT (next_node);
  node = vlib_get_node_by_name (vm, (u8 *) "ipsec6-input");
  ASSERT (node);
  u32 ah6_decrypt_node_index = next_node->index;
  ...
  ipsec_register_ah_backend (im, "ipsecmb backend", ah4_encrypt_node_index,
			     ah4_encrypt_next_index, ah4_decrypt_node_index,
			     ah4_decrypt_next_index, ah6_encrypt_node_index,
			     ah6_decrypt_node_index, ah6_encrypt_next_index,
			     ah6_decrypt_next_index, ipsecmb_check_ah_support,
			     ipsecmb_add_del_sa_session);

  ipsec_register_esp_backend (im, "ipsecmb backend",
