...
clib_error_t *
ipsecmb_check_support (ipsec_sa_t * sa)
{
...
#define __set_funcs(arch)                                               \
  do                                                                    \
    {                                                                   \
      ...
      imbm->funcs.init_mb_mgr = init_mb_mgr_##arch;                     \
      imbm->funcs.submit_job = submit_job_##arch;                       \
      imbm->funcs.submit_job_nocheck = submit_job_nocheck_##arch;       \
      imbm->funcs.flush_job = flush_job_##arch;                         \
      imbm->funcs.queue_size = queue_size_##arch;                       \
      imbm->funcs.get_completed_job = get_completed_job_##arch;         \
      imbm->funcs.get_next_job = get_next_job_##arch;                   \
    }                                                                   \
  while (0);
...
#undef __set_funcs
  ...
  next_node = vlib_get_node_by_name (vm, (u8 *) "ip4-esp-encrypt-ipsecmb");
  ASSERT (next_node);
  ...
  next_node = vlib_get_node_by_name (vm, (u8 *) "ip4-esp-decrypt-ipsecmb");
  ASSERT (next_node);
  ...
  next_node = vlib_get_node_by_name (vm, (u8 *) "ip6-esp-encrypt-ipsecmb");
  ASSERT (next_node);
  ...
  next_node = vlib_get_node_by_name (vm, (u8 *) "ip6-esp-decrypt-ipsecmb");
  ASSERT (next_node);
  ...
  next_node = vlib_get_node_by_name (vm, (u8 *) "ip4-ah-encrypt-ipsecmb");
  ASSERT (next_node);
  node = vlib_get_node_by_name (vm, (u8 *) "ipsec-output-ip4");
  ASSERT (node);
  im->ip4_ah_encrypt_node_index = next_node->index;
  im->ip4_ah_encrypt_next_index =
    vlib_node_add_next (vm, node->index, next_node->index);

  next_node = vlib_get_node_by_name (vm, (u8 *) "ip4-ah-decrypt-ipsecmb");
  ASSERT (next_node);
  node = vlib_get_node_by_name (vm, (u8 *) "ipsec-input-ip4");
  ASSERT (node);
  im->ip4_ah_decrypt_node_index = next_node->index;
  im->ip4_ah_decrypt_next_index =
    vlib_node_add_next (vm, node->index, next_node->index);

  next_node = vlib_get_node_by_name (vm, (u8 *) "ip6-ah-encrypt-ipsecmb");
  ASSERT (next_node);
  node = vlib_get_node_by_name (vm, (u8 *) "ipsec-output-ip6");
  ASSERT (node);
  im->ip6_ah_encrypt_node_index = next_node->index;
  im->ip6_ah_encrypt_next_index =
    vlib_node_add_next (vm, node->index, next_node->index);

  next_node = vlib_get_node_by_name (vm, (u8 *) "ip6-ah-decrypt-ipsecmb");
  ASSERT (next_node);
  node = vlib_get_node_by_name (vm, (u8 *) "ipsec-input-ip6");
  ASSERT (node);
  im->ip6_ah_decrypt_node_index = next_node->index;
  im->ip6_ah_decrypt_next_index =
    vlib_node_add_next (vm, node->index, next_node->index);
  u32 j;
  for (j = skip_master; j < n_mains; j++)
    vlib_node_set_state (vlib_mains[j], node->index, VLIB_NODE_STATE_POLLING);

  im->cb.check_support_cb = ipsecmb_check_support;
  im->cb.add_del_sa_sess_cb = ipsecmb_add_del_sa_session;
