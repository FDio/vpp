...
vhost_user_tx_thread_placement (vhost_user_intf_t *vui, u32 qid)
{
  ...
  // if the full_tx_queue_placement option is enabled, and the number of queues
  // is greater than the number of threads, we want to make sure every queue is
  // assigned to a thread.
  if (rxvq_count > vlib_get_n_threads () && vui->full_tx_queue_placement)
    {
      // using sw_if_index as a seed to reduce the likelihood of the first
      // thread being always referenced first.
  ...
  for (i = 0; i < vec_len (hw_if_indices); i++)
    {
      ...
      vlib_cli_output (vm, "  Full tx queue placement: %s",
		       (vui->full_tx_queue_placement) ? "enabled" :
							"disabled");
      if (vui->enable_gso)
