...
next:
  ...
  CLIB_MULTIARCH_FN (aes_gcm_internal)
  (op->src, op->dst, op->aad, (u8 *) op->iv, op->tag, op->len, op->aad_len,
...
next:
  kd = (aes_gcm_key_data_t *) cm->key_data[op->key_index];
  rv = CLIB_MULTIARCH_FN (aes_gcm_internal)
  (op->src, op->dst, op->aad, (u8 *) op->iv, op->tag, op->len, op->aad_len,
   op->tag_len, kd, AES_KEY_ROUNDS (ks), AES_GCM_OP_DECRYPT);
...
aes_gcm_key_exp (vnet_crypto_key_t *key, aes_key_size_t ks)
{
  ...
  CLIB_MULTIARCH_FN (clib_aes_gcm_key_expand_internal)
  (kd, key->data, ks);
