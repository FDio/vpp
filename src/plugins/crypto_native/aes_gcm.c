...
next:
  ...
  aes_gcm (op->src, (u8x16u *) op->dst, op->aad, (u8 *) op->iv,
	   (u8x16u *) op->tag, op->len, op->aad_len, op->tag_len, kd,
	   AES_KEY_ROUNDS (ks), /* is_encrypt */ 1);
...
next:
  kd = (aes_gcm_key_data_t *) cm->key_data[op->key_index];
  rv = aes_gcm (op->src, (u8x16u *) op->dst, op->aad, (u8 *) op->iv,
		(u8x16u *) op->tag, op->len, op->aad_len, op->tag_len, kd,
		AES_KEY_ROUNDS (ks),
		/* is_encrypt */ 0);
