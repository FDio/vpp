...
static_always_inline void *
crypto_native_key_exp_hmac_sha2_inline (vnet_crypto_key_t * key, clib_sha2_type_t type)
{
  ...
  if (vec_len (key->data) > ctx->block_size)
    {
      ...
      clib_memset_u8 (key_data + ctx->digest_size, 0, ctx->block_size - ctx->digest_size);
...
crypto_native_ops_hmac_sha2_inline (vlib_main_t * vm, vnet_crypto_op_t * ops[], u32 n_ops,
