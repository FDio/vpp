...
next:
  ...
  CLIB_MULTIARCH_FN (clib_aes_ctr_init_internal)
  (&ctx, kd, op->iv, ks);
  if (op->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS)
    {
      vnet_crypto_op_chunk_t *chp = chunks + op->chunk_index;
      for (int j = 0; j < op->n_chunks; j++, chp++)
	CLIB_MULTIARCH_FN (clib_aes_ctr_transform_internal)
      (&ctx, chp->src, chp->dst, chp->len, ks);
    }
  else
    CLIB_MULTIARCH_FN (clib_aes_ctr_transform_internal)
  (&ctx, op->src, op->dst, op->len, ks);
...
aes_ctr_key_exp (vnet_crypto_key_t *key, aes_key_size_t ks)
{
  ...
  CLIB_MULTIARCH_FN (clib_aes_ctr_key_expand_internal)
  (kd, key->data, ks);
