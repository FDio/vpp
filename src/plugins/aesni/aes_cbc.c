  ...
  for (i = 0; i < n_ops; i++)
    {
      ...
      ASSERT (op->alg == VNET_CRYPTO_ALG_AES_128_CBC);
      ASSERT (op->op == VNET_CRYPTO_OP_ENCRYPT);
      aes128_key_expand (k, op->key);
      aes_cbc_enc (k, op->src, op->dst, op->iv, op->len, 10);
    }
  return n_ops;
}
  ...
  for (i = 0; i < n_ops; i++)
    {
      ...
      aes_cbc_dec (k, op->src, op->dst, op->iv, op->len, 10);
    }
  return n_ops;
}
