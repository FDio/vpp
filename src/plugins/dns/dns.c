...
static clib_error_t *
dns_resolve_name_command_fn (vlib_main_t *vm, unformat_input_t *input,
			     vlib_cli_command_t *cmd)
{
  u8 *name = NULL;
  dns_cache_entry_t *ep = 0;
  dns_pending_request_t request = {
    .request_type = DNS_CLI_PENDING_NAME_TO_IP,
  };
  dns_resolve_name_t rn;
  dns_main_t *dm = &dns_main;
  ...
  if (!unformat (input, "%v", &name))
    return clib_error_return (0, "host-name required...");

  if (name == NULL)
    return clib_error_return (0, "host-name required...");

  if (dns_resolve_name (name, &ep, &request, &rn))
    return clib_error_return (0, "cli resolve failure");
  return 0;
}

VLIB_CLI_COMMAND (dns_resolve_name_command) = {
  .path = "dns resolve name",
  .short_help = "dns resolve name <host-name>",
  .function = dns_resolve_name_command_fn,
};
...
void
vnet_send_dns_reply (vlib_main_t *vm, dns_main_t *dm,
		     ...
		     vlib_buffer_t *b)
{
  ...
  /* Afraid is_ipv6 isn't 1 */
  ip_address_family_t is_ipv6 = pr->is_ip6 ? AF_IP6 : AF_IP4;

  vlib_frame_t *f;
  u32 *to_next;
  u32 lookup_node_index =
    is_ipv6 ? ip6_lookup_node.index : ip4_lookup_node.index;
  ...
  if (pr->request_type == DNS_PEER_PENDING_NAME_TO_IP)
      ...
      if (vnet_dns_response_to_reply (ep->dns_response, rn, &ttl))
	{
      ...
      else if (ip_addr_version (&rn->address) != is_ipv6)
	{
	  /* clib_warning ("No A-record...") or ("No AAAA-record..."); */
	  is_fail = 1;
	}
    }
  ...
  if (b == 0)
  ...
  else
    {
      ...
      u8 save_ref_count = b->ref_count;
      vlib_buffer_copy_template (b, &bt);
      b->ref_count = save_ref_count;
  ...
  if (pr->request_type == DNS_PEER_PENDING_NAME_TO_IP)
    qp->type = clib_host_to_net_u16 (is_ipv6 ? DNS_TYPE_AAAA : DNS_TYPE_A);
  else
  ...
  if (is_fail == 0)
      ...
      if (pr->request_type == DNS_PEER_PENDING_NAME_TO_IP)
	{
	  ...
	  rr->type =
	    clib_host_to_net_u16 (is_ipv6 ? DNS_TYPE_AAAA : DNS_TYPE_A);
  ...
  if (is_ipv6)
	...
	return;
      ...
      clib_memset (ip6, 0, sizeof (*ip6) + sizeof (*udp));
	...
	vlib_buffer_length_in_chain (vm, b) - sizeof (ip6_header_t));
      ...
      ip6_address_copy (&ip6->src_address, &src_address);
	...
	clib_host_to_net_u16 (sizeof (udp_header_t) + vec_len (dns_response));
      ...
      /* DNS reply */
      clib_memcpy (&ip6->dst_address, pr->dst_address, sizeof (ip6_address_t));

      /* The actual DNS request */
      udp->checksum = ip6_tcp_udp_icmp_compute_checksum (vm, b, ip6, &junk);
    }
  else
    {
      ip4_header_t *ip4;
	...
	return;
      ...
      dns_response = (u8 *) (udp + 1);
      clib_memset (ip4, 0, sizeof (*ip4) + sizeof (*udp));

      /* Set the packet length */
      b->current_length = sizeof (*ip4) + sizeof (*udp) + vec_len (reply);
      ...
      ip4->src_address.as_u32 = src_address.as_u32;
      clib_memcpy (ip4->dst_address.as_u8, pr->dst_address,
		   sizeof (ip4_address_t));
      ...
      udp->dst_port = pr->dst_port;
      udp->length =
	clib_host_to_net_u16 (sizeof (udp_header_t) + vec_len (reply));
      udp->checksum = 0;
   ...
   * Caller will ship recycled dns reply packets to ip4_lookup or ip6_lookup
   */
  if (is_recycle == 0)
    {
