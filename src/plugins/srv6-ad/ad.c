...
unsigned char keyword_usid_str[] = "End.AD.uSID";
unsigned char def_str[] =
  "Endpoint with dynamic proxy to SR-unaware appliance";
unsigned char def_usid_str[] =
  "Endpoint with uSID and dynamic proxy to SR-unaware appliance";
...
static int
srv6_ad_localsid_usid_creation_fn (ip6_sr_localsid_t *localsid)
{
  srv6_ad_localsid_t *ls_mem = localsid->plugin_mem;
  ...
  return srv6_ad_localsid_creation_fn (localsid);
}
...
uword
unformat_srv6_ad_localsid_usid (unformat_input_t *input, va_list *args)
{
  void **plugin_mem_p = va_arg (*args, void **);
  srv6_ad_localsid_t *ls_mem;
...
#define PARAM_AD_IIF (1 << 2)
  ...
  if (!unformat (input, "end.ad.usid %u", &usid_size))
    return 0;
...
const static char *const srv6_ad_usid_ip6_nodes[] = {
  "srv6-ad-localsid-usid",
  NULL,
};

const static char *const *const srv6_ad_usid_nodes[DPO_PROTO_NUM] = {
  [DPO_PROTO_IP6] = srv6_ad_usid_ip6_nodes,
};
...
srv6_ad_init (vlib_main_t * vm)
{
  ...
  sm->srv6_ad_usid_dpo_type =
    dpo_register_new_type (&srv6_ad_vft, srv6_ad_usid_nodes);

  /* Register SRv6 LocalSID for uSID
   *
   * Register this one first to avoid matching the classic SID CLI.
   */
  rv_usid = sr_localsid_register_function (
    vm, function_name, keyword_usid_str, def_usid_str, params_usid_str, 128,
    &sm->srv6_ad_usid_dpo_type, format_srv6_ad_localsid,
    unformat_srv6_ad_localsid_usid, srv6_ad_localsid_usid_creation_fn,
