...
unsigned char function_name[] = "SRv6-AD";
...
unsigned char function_name_usid[] = "SRv6-AD-uSID";
...
static int
srv6_ad_usid_localsid_creation_fn (ip6_sr_localsid_t *localsid)
{
  srv6_ad_localsid_t *ls_mem = localsid->plugin_mem;
...
uword
unformat_srv6_ad_usid_localsid (unformat_input_t *input, va_list *args)
{
  void **plugin_mem_p = va_arg (*args, void **);
  srv6_ad_localsid_t *ls_mem;
...
srv6_ad_init (vlib_main_t * vm)
{
  ...
  rv_usid = sr_localsid_register_function (
    ...
    128, &sm->srv6_ad_usid_dpo_type, format_srv6_ad_localsid,
    unformat_srv6_ad_usid_localsid, srv6_ad_usid_localsid_creation_fn,
    srv6_ad_localsid_removal_fn);
  if (rv_usid < 0)
    clib_error_return (
