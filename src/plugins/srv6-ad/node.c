 ...
 */
static uword
srv6_ad_usid_localsid_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			  vlib_frame_t *frame)
{
  ip6_sr_main_t *sm = &sr_main;
  u32 n_left_from, next_index, *from, *to_next;
...
VLIB_REGISTER_NODE (srv6_ad_usid_localsid_node) = {
  .function = srv6_ad_usid_localsid_fn,
  .name = "srv6-ad-usid-localsid",
  .vector_size = sizeof (u32),
  .format_trace = format_srv6_ad_localsid_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_next_nodes = SRV6_AD_LOCALSID_N_NEXT,
