 ...
 */
static_always_inline void
end_uad_processing (vlib_buffer_t *b0, ip6_header_t *ip0, ip6_sr_header_t *sr0,
		    ip6_sr_localsid_t *ls0, u32 *next0)
{
  ip6_address_t *new_dst0;
  u16 total_size;
 ...
 */
static uword
srv6_uad_localsid_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
		      vlib_frame_t *frame)
{
  ip6_sr_main_t *sm = &sr_main;
  u32 n_left_from, next_index, *from, *to_next;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* SRH processing */
	  end_uad_processing (b0, ip0, sr0, ls0, &next0);
...
VLIB_REGISTER_NODE (srv6_uad_localsid_node) = {
  .function = srv6_uad_localsid_fn,
  .name = "srv6-uad-localsid",
  .vector_size = sizeof (u32),
  .format_trace = format_srv6_ad_localsid_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_next_nodes = SRV6_AD_LOCALSID_N_NEXT,
