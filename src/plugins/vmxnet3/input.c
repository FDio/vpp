...
inline clib_error_t *
vmxnet3_rxq_refill (vlib_main_t * vm, vmxnet3_device_t * vd, vmxnet3_rxq_t *rxq)
{
  vmxnet3_rx_desc *rxd;
  u32 desc_idx, generation;
  u16 n_refill, n_alloc, mask;

  n_refill = rxq->size - rxq->fill;
  if (PREDICT_TRUE (n_refill <= VMXNET3_INPUT_REFILL_TRESHOLD))
    return 0;

  mask = rxq->size - 1;
  desc_idx = rxq->produce & mask;
  ...
  while (rxq->fill < rxq->size)
    {
      desc_idx = rxq->produce & mask;
      if (rxq->produce & rxq->size)
	generation = 0;
      else
  ...
  while (1)
    {
      ...
      comp_idx = rxq->consume & mask;
      if (rxq->consume & rxq->size)
	generation = 0;
      else
	...
	break;
      ...
      b0->current_length = rx_comp->len;
      b0->flags = VLIB_BUFFER_TOTAL_LENGTH_VALID;

      bi[0] = rxq->bufs[desc_idx];
      bi++;

      // FIXME
      // next maybe VNET_DEVICE_INPUT_NEXT_IP6_INPUT or
      // VNET_DEVICE_INPUT_NEXT_IP4_NCS_INPUT
      next[0] = VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT;
      next++;

      n_rx_packets++;
      n_rx_bytes += rx_comp->len;
    }
