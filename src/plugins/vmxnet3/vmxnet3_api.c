		      ...
		      u8 * interface_name, u32 context)
{
  ...
  mp->rx_next = clib_host_to_net_u64 (rxq->rx_comp_ring.next);
  for (rid = 0; rid < VMXNET3_RX_RING_SIZE; rid++)
    {
      ring = &rxq->rx_ring[rid];
      mp->rx_fill[rid] = htons (ring->fill);
      mp->rx_produce[rid] = clib_host_to_net_u64 (ring->produce);
      mp->rx_consume[rid] = clib_host_to_net_u64 (ring->consume);
    }
  mp->tx_qsize = htons (txq->size);
  mp->tx_next = clib_host_to_net_u64 (txq->tx_comp_ring.next);
  mp->tx_produce = clib_host_to_net_u64 (txq->produce);
  mp->tx_consume = clib_host_to_net_u64 (txq->consume);
