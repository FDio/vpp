...
format_vmxnet3_device_name (u8 *s, va_list *args)
{
  ...
  s = format (s, "vmxnet3-%x/%x/%x/%x", addr->domain, addr->bus, addr->slot,
	      addr->function);
  return s;
}
...
format_vmxnet3_device (u8 *s, va_list *args)
{
  ...
  s = format (s, "\n%Urx queues %u, rx desc %u, tx queues %u, tx desc %u",
	      format_white_space, indent, vd->num_rx_queues, rxq->size,
	      vd->num_tx_queues, txq->size);
  if (vd->error)
...
format_vmxnet3_input_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "vmxnet3: %v (%d) next-node %U", hi->name, t->hw_if_index,
