...
show_vmxnet3 (vlib_main_t * vm, u32 * hw_if_indices, u8 show_descr)
{
  ...
  for (i = 0; i < vec_len (hw_if_indices); i++)
      ...
      vec_foreach_index (qid, vd->rxqs)
	{
	  ...
	  vlib_cli_output (vm, "    size %u fill %llu consume %llu produce %llu",
			   rxq->size, rxq->fill, rxq->consume, rxq->produce);
	  if (show_descr)
	    {
      ...
      vec_foreach_index (qid, vd->rxqs)
	{
	  ...
	  vlib_cli_output (vm, "    size %u consume %llu produce %llu",
			   txq->size, txq->consume, txq->produce);
	  if (show_descr)
	    {
