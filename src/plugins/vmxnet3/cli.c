			   ...
			   vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	args.enable_elog = 1;
      else if (unformat (line_input, "rx-queue-size %u", &tmp)) // FIXME
	args.rxq_size = tmp;
      else if (unformat (line_input, "tx-queue-size %u", &tmp)) // FIXME
	args.txq_size = tmp;
      else
  ...
  for (i = 0; i < vec_len (hw_if_indices); i++)
    {
      ...
      if (show_descr)
	{
	  rxq = vec_elt_at_index (vd->rxqs, 0);
	  vlib_cli_output (vm, "RX descriptors");
	  vlib_cli_output (vm, "  fill %u consume %u produce %u",
			   rxq->fill, rxq->consume, rxq->produce);
	  ...
	  vlib_cli_output (vm, "RX completion descriptors");
	  vlib_cli_output (vm, "  %5s  %10s  %10s  %10s  %10s",
			   "slot", "index", "rss", "len", "flags");
	  for (desc_idx = 0; desc_idx < rxq->size; desc_idx++)
	    {
	      rx_comp = &vd->dma->rx_comp[desc_idx];
	  ...
	  vlib_cli_output (vm, "TX descriptors");
	  vlib_cli_output (vm, "  consume %u produce %u",
			   txq->consume, txq->produce);
	  ...
	  vlib_cli_output (vm, "TX completion descriptors");
	  vlib_cli_output (vm, "  %5s  %10s  %10s",
			   "slot", "index", "flags");
	  for (desc_idx = 0; desc_idx < txq->size; desc_idx++)
	    {
	      tx_comp = &vd->dma->tx_comp[desc_idx];
