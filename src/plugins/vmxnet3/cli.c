  ...
  for (i = 0; i < vec_len (hw_if_indices); i++)
    {
      ...
      vlib_cli_output (vm, "  Number of tx queues: %u", vd->num_tx_queues);
      vlib_cli_output (vm, "  Number of rx queues: %u", vd->num_rx_queues);
      ...
      vlib_cli_output (vm, "  flags: 0x%x", vd->flags);
    }
}
