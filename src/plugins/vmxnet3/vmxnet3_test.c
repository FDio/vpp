...
vl_api_vmxnet3_details_t_handler (vl_api_vmxnet3_details_t * mp)
{
  ...
  fformat (vam->ofp, "%s: sw_if_index %u mac %U\n"
	   "   version: %u\n"
	   "   PCI Address: %U\n"
	   "   RX completion next index %llu"
	   "   RX Queue %u\n"
	   "    ring 0 size %u fill %u consume %llu produce %llu\n"
	   "    ring 1 size %u fill %u consume %llu produce %llu\n"
	   "   TX completion next index %llu"
	   "   TX Queue %u\n"
	   "    size %u consume %llu produce %llu\n"
	   "   state %s\n",
	   ...
	   clib_net_to_host_u64 (mp->rx_consume[0]),
	   clib_net_to_host_u64 (mp->rx_produce[0]),
	   ntohs (mp->rx_qsize), ntohs (mp->rx_fill[1]),
	   clib_net_to_host_u64 (mp->rx_consume[1]),
	   clib_net_to_host_u64 (mp->rx_produce[1]),
	   ntohs (mp->tx_next),
	   ntohs (mp->tx_qid),
	   ntohs (mp->tx_qsize), clib_net_to_host_u64 (mp->tx_consume),
	   clib_net_to_host_u64 (mp->tx_produce),
