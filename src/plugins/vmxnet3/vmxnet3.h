			  ...
			  vmxnet3_rxq_t *rxq)
{
  ...
  mask = rxq->size - 1;
  desc_idx = ring->produce & mask;

  n_alloc = vlib_buffer_alloc_to_ring (vm, ring->bufs, desc_idx, rxq->size,
				       n_refill);
  if (n_alloc != n_refill)
    {
      if (n_alloc)
  ...
  while (n_alloc)
    {
      if (ring->produce & rxq->size)
	generation = 0;
      else
	generation = VMXNET3_RXF_GEN;

      rxd = &rxq->rx_desc[0][desc_idx];
      rxd->address =
	vlib_get_buffer_data_physical_address (vm, ring->bufs[desc_idx]);
      rxd->flags = generation | VLIB_BUFFER_DATA_SIZE;
  ...
  vmxnet3_reg_write (vd, 0, VMXNET3_REG_RXPROD, ring->produce & mask);
			  ...
			  vmxnet3_rxq_t *rxq)
{
  ...
  while (n_alloc)
	...
	generation = VMXNET3_RXF_GEN;
      ...
      rxd = &rxq->rx_desc[1][desc_idx];
      rxd->address =
	vlib_get_buffer_data_physical_address (vm, ring->bufs[desc_idx]);
      rxd->flags = generation | VLIB_BUFFER_DATA_SIZE | VMXNET3_RXF_BTYPE;
  ...
  vmxnet3_reg_write (vd, 0, VMXNET3_REG_RXPROD2, ring->produce & mask);
