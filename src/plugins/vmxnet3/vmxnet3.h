...
#define VMXNET3_RXCOMP_TYPE_LRO                                               \
  (4 << 24) /* RX completion descriptor for LRO */
			  ...
			  vmxnet3_rxq_t *rxq)
{
  ...
  n_alloc = vlib_buffer_alloc_to_ring_from_pool (vm, ring->bufs, ring->produce,
						 rxq->size, n_refill,
  ...
  while (n_alloc)
    {
      ...
      rxd->address = vlib_buffer_get_pa (vm, b);
      rxd->flags =
	ring->gen | vlib_buffer_get_default_data_size (vm) | VMXNET3_RXF_BTYPE;
