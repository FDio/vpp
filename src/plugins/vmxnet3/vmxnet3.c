...
vmxnet3_provision_driver_shared (vlib_main_t * vm, vmxnet3_device_t * vd)
{
  ...
  q->rx.cfg.desc_address[0] = vmxnet3_dma_addr (vm, vd, rxq->rx_desc);
  q->rx.cfg.comp_address = vmxnet3_dma_addr (vm, vd, rxq->rx_comp);
  q->rx.cfg.num_desc[0] = rxq->size;
...
vmxnet3_rxq_init (vlib_main_t * vm, vmxnet3_device_t * vd, u16 qid, u16 qsz)
{
  ...
  rxq->size = qsz;
  rxq->rx_desc = vlib_physmem_alloc_aligned (vm, vmxm->physmem_region, &error,
					     qsz * sizeof (*rxq->rx_desc), 512);
  if (error)
    return error;
  memset (rxq->rx_desc, 0, qsz * sizeof (*rxq->rx_desc));
  rxq->rx_comp = vlib_physmem_alloc_aligned (vm, vmxm->physmem_region, &error,
					     qsz * sizeof (*rxq->rx_comp), 512);
  if (error)
    return error;
		     ...
		     vmxnet3_create_if_args_t * args)
{
  ...
  error = vmxnet3_txq_init (vm, vd, 0, args->txq_size);
  if (error)
    return error;
...
vmxnet3_create_if (vlib_main_t * vm, vmxnet3_create_if_args_t * args)
{
  ...
  pool_foreach (vd, vmxm->devices, ({
    if (!memcmp (&vd->pci_addr, &args->addr, sizeof (vd->pci_addr)))
      {
	args->rv = VNET_API_ERROR_INVALID_VALUE;
	args->error =
  ...
  memcpy (&vd->pci_addr, &args->addr, sizeof (vd->pci_addr));
  ...
  vec_foreach_index (i, vd->rxqs)
    {
      ...
      desc_idx = rxq->consume;
      while (rxq->fill)
	{
	  desc_idx &= mask;
	  bi = rxq->bufs[desc_idx];
	  vlib_buffer_free_one (vm, bi);
	  rxq->fill--;
	  desc_idx++;
	}
      vec_free (rxq->bufs);
      vlib_physmem_free (vm, vmxm->physmem_region, rxq->rx_desc);
  ...
  vec_foreach_index (i, vd->txqs)
    {
      ...
      desc_idx = txq->consume & mask;
      end_idx = txq->produce & mask;
      while (desc_idx != end_idx)
	{
	  bi = txq->bufs[desc_idx];
	  vlib_buffer_free_one (vm, bi);
	  desc_idx++;
	  desc_idx &= mask;
	}
