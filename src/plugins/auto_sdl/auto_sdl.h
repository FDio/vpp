...
uword auto_sdl_pool_size (void);
void auto_sdl_init (void);
void auto_sdl_cleanup_by_fib_index (u32 fib_proto, u32 fib_index);
clib_error_t *auto_sdl_config (auto_sdl_config_args_t *args);
int auto_sdl_track_prefix (auto_sdl_track_prefix_args_t *args);

typedef uword (*auto_sdl_pool_size_fn_t) (void);
typedef int (*auto_sdl_track_prefix_fn_t) (auto_sdl_track_prefix_args_t *args);
typedef void (*auto_sdl_cleanup_by_fib_index_fn_t) (u32 fib_proto,
						    u32 fib_index);
typedef void (*auto_sdl_init_fn_t) (void);
typedef clib_error_t *(*auto_sdl_config_fn_t) (auto_sdl_config_args_t *args);

#define foreach_auto_sdl_plugin_exported_method_name                          \
  _ (cleanup_by_fib_index)                                                    \
  _ (track_prefix)                                                            \
  _ (init)                                                                    \
  _ (pool_size)                                                               \
  _ (config)
