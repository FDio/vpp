...
acl_itf_update (index_t itf_index, index_t acl_index)
{
  ...
  ASSERT(~0 != acl_itf_acl_find (aitf, acl_index));

  // FIXME - we can probably do better
  acl_itf_acl_remove(aitf, acl_index);
  acl_itf_acl_add(aitf, acl_index);
}
          ...
          int *may_clear_sessions)
{
  ...
  if (vec_len (aitf->acls))
	...
	match_set_unapply (aitf->match_set[af], &aitf->match_apps[af]);
      ...
      FOR_EACH_IP_ADDRESS_FAMILY (af)
      {
        if (!match_set_is_empty(aitf->match_set[af]))
          {
  ...
  if (0 == is_add)
    {
      ...
      acl_bind (am, sw_if_index, VLIB_RX,
                NULL, &may_clear_sessions);
      acl_bind (am, sw_if_index, VLIB_TX,
                NULL, &may_clear_sessions);
    }
  return 0;
}
...
static clib_error_t *
acl_show_acl_fn (vlib_main_t * vm,
			   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  acl_main_t *am = &acl_main;
  u32 acl_index = ~0;
...
static clib_error_t *
acl_show_decode_5tuple_fn (vlib_main_t * vm,
				     unformat_input_t *
				     input, vlib_cli_command_t * cmd)
{
  clib_error_t *error = 0;
...
static clib_error_t *
acl_show_interface_fn (vlib_main_t * vm,
...
static clib_error_t *
acl_show_memory_fn (vlib_main_t * vm,
			      unformat_input_t * input,
			      vlib_cli_command_t * cmd)
{
  clib_error_t *error = 0;
...
static clib_error_t *
acl_show_sessions_fn (vlib_main_t * vm,
				unformat_input_t *
				input, vlib_cli_command_t * cmd)
{
  clib_error_t *error = 0;
...
static clib_error_t *
acl_show_tables_fn (vlib_main_t * vm,
			      unformat_input_t * input,
			      vlib_cli_command_t * cmd)
{
  clib_error_t *error = 0;
  ...
  vlib_cli_output (vm, "Stats enabled for ACLs: %d",
		   acl_main.counters_enabled);
  /* if (show_mask_type) */
...
VLIB_CLI_COMMAND (aclplugin_set_command, static) = {
    .path = "set acl2",
    .short_help = "set acl2 session timeout {{udp idle}|tcp {idle|transient}} <seconds>",
    .function = acl_set_fn,
};
...
VLIB_CLI_COMMAND (aclplugin_show_acl_command, static) = {
    ...
    .function = acl_show_acl_fn,
};

VLIB_CLI_COMMAND (aclplugin_show_decode_5tuple_command, static) = {
    .path = "show acl2 decode 5tuple",
    .short_help = "show acl2 decode 5tuple XXXX XXXX XXXX XXXX XXXX XXXX",
    .function = acl_show_decode_5tuple_fn,
};

VLIB_CLI_COMMAND (aclplugin_show_interface_command, static) = {
    .path = "show acl2 interface",
    .short_help = "show acl2 interface [sw_if_index N] [acl]",
    .function = acl_show_interface_fn,
};
VLIB_CLI_COMMAND (aclplugin_show_bind_command, static) = {
    .path = "show acl2 bind",
    .short_help = "show acl2 bind [sw_if_index N] [acl]",
    .function = acl_show_interface_fn,
};

VLIB_CLI_COMMAND (aclplugin_show_memory_command, static) = {
    .path = "show acl2 memory",
    .short_help = "show acl2 memory",
    .function = acl_show_memory_fn,
};

VLIB_CLI_COMMAND (aclplugin_show_sessions_command, static) = {
    .path = "show acl2 sessions",
    .short_help = "show acl2 sessions",
    .function = acl_show_sessions_fn,
};

VLIB_CLI_COMMAND (aclplugin_show_tables_command, static) = {
    .path = "show acl2 tables",
    .short_help = "show acl2 tables [ acl [index N] | applied [ lc_index N ] | mask | hash [verbose N] ]",
    .function = acl_show_tables_fn,
};

VLIB_CLI_COMMAND (aclplugin_show_macip_acl_command, static) = {
    .path = "show acl2 macip acl",
    .short_help = "show acl2 macip acl [index N]",
    .function = acl_show_macip_acl_fn,
};

VLIB_CLI_COMMAND (aclplugin_show_macip_interface_command, static) = {
    .path = "show acl2 macip interface",
    .short_help = "show acl2 macip interface",
    .function = acl_show_macip_interface_fn,
};

VLIB_CLI_COMMAND (aclplugin_clear_command, static) = {
    .path = "clear acl2 sessions",
    .short_help = "clear acl2 sessions",
    .function = acl_clear_fn,
};
...
VLIB_CONFIG_FUNCTION (acl_plugin_config, "acl2");
...
acl_init (vlib_main_t * vm)
{
  ...
  acl_logger = vlib_log_register_class ("acl2", "acl2");
