...
/* logging */
static vlib_log_class_t acl2_logger;
...
#define ACL2_CONN_DB_OWNER_INVALID_MACRO {.conn_db = INDEX_INVALID}
const static acl2_conn_db_owner_t ACL2_CONN_DB_OWNER_INVALID =
  ACL2_CONN_DB_OWNER_INVALID_MACRO;

#define acl2_log_err(...) \
  vlib_log(VLIB_LOG_LEVEL_ERR, acl2_logger, __VA_ARGS__)
#define acl2_log_warn(...) \
  vlib_log(VLIB_LOG_LEVEL_WARNING, acl2_logger, __VA_ARGS__)
#define acl2_log_notice(...) \
  vlib_log(VLIB_LOG_LEVEL_NOTICE, acl2_logger, __VA_ARGS__)
#define acl2_log_info(...) \
  vlib_log(VLIB_LOG_LEVEL_NOTICE, acl2_logger, __VA_ARGS__)
#define acl2_log_debug(...) \
  vlib_log(VLIB_LOG_LEVEL_DEBUG, acl2_logger, __VA_ARGS__)
...
#define FOR_EACH_ACE_AF(_acl, _ace, _af, _body)                 \
{                                                               \
  index_t *_acei;                                               \
  vec_foreach(_acei, _acl->acl_aces)                            \
    {                                                           \
      _ace = pool_elt_at_index(acl2_main.ace_pool, *_acei);     \
      if (match_rule_get_af (&_ace->ace_rule) == af)            \
        {                                                       \
          _body;                                                \
        }                                                       \
    }                                                           \
...
acl2_itf_t *
acl2_itf_create (u32 sw_if_index, vlib_dir_t dir, u32 n_conns)
{
  acl2_main_t *am = &acl2_main;
...
static void
acl2_itf_compile (acl2_itf_t * aitf, ip_address_family_t af, acl2_t * acl)
{
  acl2_main_t *am = &acl2_main;
  ace2_t *ace;

  vec_validate_init_empty (am->conn_dbs[af], aitf->sw_if_index,
			   ACL2_CONN_DB_OWNER_INVALID);

  /* *INDENT-OFF* */
  FOR_EACH_ACE_AF(acl, ace, af,
  ({
    if (ACL2_ACTION_TRACK == ace->ace_action)
      {
        aitf->per_af[af].n_trackers++;

        if (INDEX_INVALID == aitf->per_af[af].conn_db)
          aitf->per_af[af].conn_db =
            conn_track_add_or_lock (am->conn_user,
                                    aitf->sw_if_index,
                                    NULL, af,
                                    ACL2_N_CONNECTIONS_PER_WORKER,
                                    CONN_DB_FLAG_NONE);

        am->conn_dbs[af][aitf->sw_if_index].conn_owners[aitf->dir] = true;

        vec_validate_init_empty (ace->ace_conn_owner, aitf->sw_if_index, ~0);

        if (~0 == ace->ace_conn_owner[aitf->sw_if_index])
          ace->ace_conn_owner[aitf->sw_if_index] =
            conn_track_owner_add (aitf->per_af[af].conn_db);
      }
  }));
  /* *INDENT-ON* */
}
...
static void
acl2_itf_uncompile (acl2_itf_t * aitf, ip_address_family_t af, acl2_t * acl)
{
  ace2_t *ace;
    ...
    if (ACL2_ACTION_TRACK == ace->ace_action)
      {
        ...
        conn_track_owner_flush (aitf->per_af[af].conn_db,
                                ace->ace_conn_owner[aitf->sw_if_index]);
        ace->ace_conn_owner[aitf->sw_if_index] = ~0;
      }
  }));
  /* *INDENT-ON* */
}
...
static void
acl2_itf_update_track_i (acl2_itf_t * aitf, ip_address_family_t af)
{
  acl2_main_t *am = &acl2_main;
  vlib_dir_t reverse;

  reverse = (aitf->dir + 1) % VLIB_N_DIR;

  /* do we need to use connection tracking */
  if (aitf->per_af[af].n_trackers)
    {
      /* there are one or more tracking ACEs bound to this interface */
      if (INDEX_INVALID == am->conn_dbs[af][aitf->sw_if_index].conn_db)
	am->conn_dbs[af][aitf->sw_if_index].conn_db =
	  aitf->per_af[af].conn_db;
      else
	ASSERT (am->conn_dbs[af][aitf->sw_if_index].conn_db ==
		aitf->per_af[af].conn_db);
      am->conn_dbs[af][aitf->sw_if_index].conn_owners[aitf->dir] = true;
    }
  else if (am->conn_dbs[af][aitf->sw_if_index].conn_owners[reverse])
    {
      /* conn tracking (or at least conn lookup) is need because the reverse
       * direction is tracking */
      aitf->per_af[af].conn_db = am->conn_dbs[af][aitf->sw_if_index].conn_db;
    }
  else
    {
      /* there is no reason to conn track */
      am->conn_dbs[af][aitf->sw_if_index].conn_owners[aitf->dir] = false;
      aitf->per_af[af].conn_db = INDEX_INVALID;

      if (!am->conn_dbs[af][aitf->sw_if_index].conn_owners[VLIB_RX] &&
	  !am->conn_dbs[af][aitf->sw_if_index].conn_owners[VLIB_TX] &&
	  INDEX_INVALID != am->conn_dbs[af][aitf->sw_if_index].conn_db)
	conn_db_unlock (&am->conn_dbs[af][aitf->sw_if_index].conn_db);
    }

}

static void
acl2_itf_update_track (acl2_itf_t * aitf, ip_address_family_t af)
{
  acl2_itf_t *aitf_reverse;
  vlib_dir_t reverse;

  acl2_itf_update_track_i (aitf, af);

  /* if the reverse direction exists, it needs to update too */
  reverse = (aitf->dir + 1) % VLIB_N_DIR;
  aitf_reverse = acl2_itf_find (aitf->sw_if_index, reverse);

  if (NULL != aitf_reverse)
    acl2_itf_update_track_i (aitf_reverse, af);
}
...
acl2_itf_acl_update (acl2_itf_t * aitf, u32 acl_index)
{
  ...
  FOR_EACH_IP_ADDRESS_FAMILY (af)
  {
    ...
    acl2_itf_uncompile (aitf, af, acl);
    acl2_itf_compile (aitf, af, acl);
    acl2_itf_update_track (aitf, af);
  }
}
...
acl2_itf_acl_remove (acl2_itf_t * aitf, u32 acl_index)
{
  ...
  FOR_EACH_IP_ADDRESS_FAMILY (af)
  {
    acl2_itf_uncompile (aitf, af, acl2_get (ah->ah_acl));
    acl2_itf_update_track (aitf, af);
...
acl2_match_list_compile (acl2_t * acl, ip_address_family_t af)
{
  ...
  acl_pf = &acl->acl_per_af[af];
  name = format (NULL, "acl-%d-%U", acl - acl2_main.acl_pool,
		 format_ip_address_family, af);
...
ace_destroy (ace2_t * ace)
{
  ...
  af = match_rule_get_af (&ace->ace_rule);

  vec_foreach_index (sw_if_index, ace->ace_conn_owner)
  {
    co = ace->ace_conn_owner[sw_if_index];

    if (~0 != co)
      conn_track_owner_flush (am->conn_dbs[af][sw_if_index].conn_db, co);
  }

  vec_free (ace->ace_conn_owner);
  pool_put (am->ace_pool, ace);
}
      ...
      if (!enable)
	{
	  ...
	  rv |= acl2_feat_func[aitf->layer]
	    (acl2_feat_arc[aitf->layer][af][aitf->dir],
	     acl2_feat[aitf->layer][af][aitf->dir],
	   ...
	   vlib_dir_t dir, u32 * vec_acl_list_index, u32 n_conns)
{
  ...
  aitf = acl2_itf_create (sw_if_index, dir, n_conns);
  ...
  if (0 == is_add)
    {
      ...
      acl2_bind (sw_if_index, VLIB_RX, NULL, 0);
      acl2_bind (sw_if_index, VLIB_TX, NULL, 0);
    }
  return 0;
}
...
format_acl2_itf (u8 * s, va_list * args)
{
  ...
  if (flags & ACL2_FORMAT_DETAIL)
	...
	s = format (s, "\n    %U", format_acl2_hdl, ah);
      ...
      FOR_EACH_IP_ADDRESS_FAMILY (af)
	s =
	format (s, "\n  %U n-trackers:%d match-set:%d conn-db:%d flags:[%U]",
		format_ip_address_family, af, aitf->per_af[af].n_trackers,
		aitf->per_af[af].match_set, aitf->per_af[af].conn_db,
	       ...
	       unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "n_conns %d", &n_conns))
	;
  ...
  acl2_bind (sw_if_index, dir, ais, n_conns);
...
{
  .path = "acl2 bind",
  .short_help = "acl2 bind <INDEX> <INTERFACE> [n_conns X]",
...
acl_init (vlib_main_t * vm)
{
  ...
  acl2_logger = vlib_log_register_class ("acl2", "acl2");
