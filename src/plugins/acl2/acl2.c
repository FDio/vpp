...
int
acl2_bind (acl2_main_t * am,
	   u32 sw_if_index, vlib_dir_t dir, u32 * vec_acl_list_index)
{
  u32 *pacln;
  ...
  if (0 == is_add)
    {
      ...
      acl2_bind (am, sw_if_index, VLIB_RX, NULL);
      acl2_bind (am, sw_if_index, VLIB_TX, NULL);
    }
  return 0;
}
...
static clib_error_t *
acl2_show_acl_fn (vlib_main_t * vm,
...
static clib_error_t *
acl2_show_interface_fn (vlib_main_t * vm,
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  acl2_main_t *am = &acl2_main;
  acl2_format_flag_t flags;
...
static clib_error_t *
acl2_add_del_cmd (vlib_main_t * vm,
		  unformat_input_t * input, vlib_cli_command_t * cmd)
{
...
{
.path = "show acl2 acl",.short_help = "show acl2 acl [index N]",.function =
    acl2_show_acl_fn,};

VLIB_CLI_COMMAND (aclplugin_show_interface_command, static) =
{
.path = "show acl2 interface",.short_help =
    "show acl2 interface [sw_if_index N] [acl]",.function =
    acl2_show_interface_fn,};
VLIB_CLI_COMMAND (aclplugin_show_bind_command, static) =
{
.path = "show acl2 bind",.short_help =
    "show acl2 bind [sw_if_index N] [acl]",.function =
    acl2_show_interface_fn,};

VLIB_CLI_COMMAND (aclplugin_add_del_command, static) =
{
.path = "acl2",.short_help =
    "acl2 [del] action <a> rule <r> ...",.function = acl2_add_del_cmd,};
/* *INDENT-ON* */

static clib_error_t *
acl_init (vlib_main_t * vm)
{
  acl2_main.log_default = vlib_log_register_class ("acl2", "acl2");
