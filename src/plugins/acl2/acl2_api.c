...
vl_api_acl2_update_t_handler (vl_api_acl2_update_t * mp)
{
  ...
  u32 expected_len = sizeof (*mp) + acl_count * sizeof (mp->aces[0]);

  if (verify_message_len (mp, expected_len, "acl2_update"))
    {
      index_t *aces = NULL;

      vec_validate (aces, acl_count - 1);

      for (ii = 0; ii < acl_count; ii++)
	rv |= acl2_ace_decode (&mp->aces[ii], &aces[ii]);

      if (!rv)
	rv = acl2_update (&acl_list_index, aces, mp->tag);
    }
  else
    {
...
vl_api_acl2_bind_t_handler (vl_api_acl2_bind_t * mp)
{
  ...
  if (0 == rv)
	...
	vec_add1 (acl_vec, clib_net_to_host_u32 (mp->acls[i]));
      ...
      rv = acl2_bind (am, sw_if_index, dir, acl_vec);
      vec_free (acl_vec);
    }
