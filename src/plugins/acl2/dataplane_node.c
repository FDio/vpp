	  ...
	  if (acl_check_needed)
	    {
	      ...
	      aitf = acl_itf_get (sw_if_index[0], dir);
               ...
               * fast forward this to point buffer at the l3 */
              if (VNET_LINK_ETHERNET != linkt &&
                  VLIB_TX == dir)
                vlib_buffer_advance(b[0], vnet_buffer(b[0])->ip.save_rewrite_length);

	      match_match_one (vm, b[0], &aitf->match_apps[af], now, &result);
	      ...
	      if (MATCH_RESULT_MISS != result.msr_pos.msp_rule_index)
		{
		  ...
		  if (PREDICT_FALSE (am->counters_enabled))
		    {
...
format_acl_plugin_trace (u8 * s, va_list * args)
{
  ...
  s =
    format (s,
	    "acl2: lc_index: %d, sw_if_index %d, next index %d, action: %d, match: acl %d rule %d trace_bits %08x\n"
