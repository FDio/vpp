...
class MethodHolder(VppTestCase):
    ...
    def macip_acl_dump_debug(self):
        acls = self.vapi.macip2_acl_dump()
        if self.DEBUG:
            for acl in acls:
    ...
    def apply_macip_rules(self, acls):
        for acl in acls:
            reply = self.vapi.macip2_acl_add(acl)
            self.assertEqual(reply.retval, 0)
            self.ACLS.append(reply.acl_index)

    def verify_macip_acls(self, acl_count, rules_count, expected_count=2):
        reply = self.macip2_acl_dump_debug()
        for acl in range(2, (acl_count+1) * 2):
            self.assertEqual(reply[acl - 2].count, rules_count[acl//2-1])

        self.vapi.macip2_acl_interface_get()

        self.vapi.macip2_acl_interface_add_del(sw_if_index=0,
                                               acl_index=0,
                                               is_add=1)
        self.vapi.macip2_acl_interface_add_del(sw_if_index=1,
                                               acl_index=1,
                                               is_add=1)

        reply = self.vapi.macip2_acl_interface_get()
        self.assertEqual(reply.count, expected_count)

    def delete_acls(self):
        for acl in range(len(self.ACLS)-1, -1, -1):
            self.vapi.macip2_acl_del(acl_index=self.ACLS[acl])

        reply = self.vapi.macip2_acl_dump()
    ...
    def create_stream(self, mac_type, ip_type, packet_count,
        ...
        if not (mac_type == self.WILD_MAC and ip_type == self.WILD_IP):
            macip_rule = ({'is_permit': 0,
                           'rule':
                           {
                               'mmim_ip': "::/0" if is_ip6 else "0.0.0.0/0",
                               'mmim_mac':
                               {
                                   'mmm_mac': "00:00:00:00:00:00",
                                   'mmm_mask': "00:00:00:00:00:00",
                               },
                           }})
            macip_rules.append(macip_rule)
    ...
    def run_traffic(self, mac_type, ip_type, traffic, is_ip6, packets,
        ...
        if apply_rules:
            if isMACIP:
                reply = self.vapi.macip2_acl_add(
                    tag=b'',
                    r=test_dict['macip_rules'],
                    count=len(test_dict['macip_rules']))
            else:
            ...
            if isMACIP:
                self.vapi.macip2_acl_interface_add_del(
        ...
        if try_replace:
            if isMACIP:
                reply = self.vapi.macip2_acl_add_replace(
                    tag=b'',
                    r=test_dict['macip_rules'],
                    count=len(test_dict['macip_rules']),
                    acl_index=acl_index)
            else:
                reply = self.vapi.acl2_add_replace(acl_index=acl_index,
                                                   r=test_dict['acl_rules'])
            self.assertEqual(reply.retval, 0)
...
class TestMACIP(MethodHolder):
    ...
    def test_acl_replace(self):
        ...
        reply = self.vapi.macip2_acl_add_replace(r=r2[0], count=len(r2[0]),
                                                 acl_index=2, tag=b'')
        self.assertEqual(reply.retval, 0)
        self.assertEqual(reply.acl_index, 2)
        reply = self.vapi.macip2_acl_add_replace(r=r2[1], count=len(r2[1]),
                                                 acl_index=3, tag=b'')
        self.assertEqual(reply.retval, 0)
        self.assertEqual(reply.acl_index, 3)

        acls_after = self.macip2_acl_dump_debug()
    ...
    def test_delete_intf(self):
        ...
        self.vapi.macip2_acl_interface_add_del(sw_if_index0, 1)
        ...
        self.vapi.macip2_acl_interface_add_del(sw_if_index1, 0)
        ...
        self.vapi.macip2_acl_interface_add_del(sw_if_index2, 1)
        self.vapi.macip2_acl_interface_add_del(sw_if_index3, 1)
...
class TestACLMacip(VppTestCase):
    ...
    def test_macip_pfx(self):
        ...
        acl = self.vapi.macip2_acl_add(r=rules, count=len(rules), tag=b'this')
        ...
        self.vapi.macip2_acl_interface_add_del(
            ...
            acl_index=acl.acl_index,
            is_add=0)
        self.vapi.macip2_acl_del(acl_index=acl.acl_index)
