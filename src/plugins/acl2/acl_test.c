...
#define __plugin_msg_base acl2_test_main.msg_id_base
...
acl2_test_main_t acl2_test_main;

#define foreach_reply_retval_aclindex_handler  \
_(acl2_add_replace_reply) \
_(macip2_acl_add_reply) \
_(macip2_acl_add_replace_reply)

#define _(n)                                            \
    static void vl_api_##n##_t_handler                  \
    (vl_api_##n##_t * mp)                               \
    {                                                   \
        vat_main_t * vam = acl2_test_main.vat_main;   \
...
static void vl_api_acl2_interface_list_details_t_handler
  (vl_api_acl2_interface_list_details_t * mp)
{
  int i;
  vat_main_t *vam = acl2_test_main.vat_main;
  u8 *out = 0;
  vl_api_acl2_interface_list_details_t_endian (mp);
  out =
...
static void
vl_api_macip2_acl_details_t_handler (vl_api_macip2_acl_details_t * mp)
{
}

static void
  vl_api_macip2_acl_interface_list_details_t_handler
  (vl_api_macip2_acl_interface_list_details_t * mp)
{
  // NOT YET IMPLEMENTED
}
...
static void
  vl_api_acl2_interface_etype_whitelist_details_t_handler
  (vl_api_acl2_interface_etype_whitelist_details_t * mp)
{
}

static void
  vl_api_acl2_plugin_get_conn_table_max_entries_reply_t_handler
  (vl_api_acl2_plugin_get_conn_table_max_entries_reply_t * mp)
{
  vat_main_t *vam = acl2_test_main.vat_main;
...
static void
  vl_api_macip2_acl_interface_get_reply_t_handler
  (vl_api_macip2_acl_interface_get_reply_t * mp)
{
}

static void
vl_api_acl2_details_t_handler (vl_api_acl2_details_t * mp)
{
}

static int
api_macip2_acl_interface_get (vat_main_t * vam)
{
  acl2_test_main_t *sm = &acl2_test_main;
  vl_api_macip2_acl_interface_get_t *mp;
  ...
  mp->_vl_msg_id = ntohs (VL_API_MACIP2_ACL_INTERFACE_GET + sm->msg_id_base);
...
/* const static vl_api_acl2_rule_t API_RULE_DEFAULT = { */
...
/* NOT YET IMPLEMENTED */
static int
api_macip2_acl_interface_list_dump (vat_main_t * vam)
{
  return 0;
}

static int
api_acl2_add_replace (vat_main_t * vam)
{
  /* acl2_test_main_t * sm = &acl2_test_main; */
  /* unformat_input_t * i = vam->input; */
  /* vl_api_acl2_add_replace_t * mp; */
  /* u32 acl_index = ~0; */
  /* u32 msg_size = sizeof (*mp); /\* without the rules *\/ */

  /* vl_api_acl2_rule_t *rules = 0; */
  /* int rule_idx = 0; */
  /* int n_rules = 0; */
  ...
  /* mp = vl_msg_api_alloc_as_if_client(msg_size); */
  /* clib_memset (mp, 0, msg_size); */
  /* mp->_vl_msg_id = ntohs (VL_API_ACL2_ADD_REPLACE + sm->msg_id_base); */
  /* mp->client_index = vam->my_client_index; */
  /* if ((n_rules > 0) && rules) */
  /*   clib_memcpy(mp->r, rules, n_rules*sizeof (vl_api_acl2_rule_t)); */
  /* if (tag) */
  /*   { */
...
static int
api_acl2_plugin_get_conn_table_max_entries (vat_main_t * vam)
{
  acl2_test_main_t *sm = &acl2_test_main;
  ...
  mp->_vl_msg_id =
    ntohs (VL_API_ACL2_PLUGIN_GET_CONN_TABLE_MAX_ENTRIES + sm->msg_id_base);
  mp->client_index = vam->my_client_index;
...
static int
api_acl2_stats_intf_counters_enable (vat_main_t * vam)
{
  acl2_test_main_t *sm = &acl2_test_main;
  unformat_input_t *i = vam->input;
  ...
  memset (mp, 0, msg_size);
  mp->_vl_msg_id =
    ntohs (VL_API_ACL2_STATS_INTF_COUNTERS_ENABLE + sm->msg_id_base);
  mp->client_index = vam->my_client_index;
  mp->enable = 1;
...
static int
api_acl2_add_replace_from_file (vat_main_t * vam)
{
/*     int ret = -1; */
...
static int
api_acl2_del (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  vl_api_acl2_del_t *mp;
  u32 acl_index = ~0;
  int ret;
...
static int
api_macip2_acl_del (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  vl_api_acl2_del_t *mp;
  u32 acl_index = ~0;
...
static int
api_macip2_acl_interface_add_del (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  ...
  /* Construct the API message */
  M (MACIP2_ACL_INTERFACE_ADD_DEL, mp);
  mp->acl_index = ntohl (acl_index);
...
static int
api_acl2_interface_set_acl_list (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
...
//  acl_interface_set_acl_list <intfc> | sw_if_index <if-idx> input [acl-idx list] output [acl-idx list]
  ...
  /* Construct the API message */
  M2 (ACL2_INTERFACE_SET_ACL_LIST, mp,
...
static int
api_acl2_interface_set_etype_whitelist (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
...
//  acl_interface_set_etype_whitelist <intfc> | sw_if_index <if-idx> input [ethertype list] output [ethertype list]
  ...
  /* Construct the API message */
  M2 (ACL2_INTERFACE_SET_ETYPE_WHITELIST, mp,
...
api_acl_send_control_ping (vat_main_t * vam)
{
  ...
  if (!acl2_test_main.ping_id)
    acl2_test_main.ping_id =
      vl_msg_api_get_msg_index ((u8 *) (VL_API_CONTROL_PING_CRC));
  mp_ping = vl_msg_api_alloc_as_if_client (sizeof (*mp_ping));
  mp_ping->_vl_msg_id = htons (acl2_test_main.ping_id);
...
static int
api_acl2_interface_list_dump (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  u32 sw_if_index = ~0;
...
static int
api_acl2_dump (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  u32 acl_index = ~0;
  vl_api_acl2_dump_t *mp;
  int ret;
...
static int
api_macip2_acl_dump (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  u32 acl_index = ~0;
  vl_api_acl2_dump_t *mp;
...
static int
api_acl2_interface_etype_whitelist_dump (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  u32 sw_if_index = ~0;
  vl_api_acl2_interface_etype_whitelist_dump_t *mp;
  int ret;
  ...
  /* Construct the API message */
  M (ACL2_INTERFACE_ETYPE_WHITELIST_DUMP, mp);
  mp->sw_if_index = ntohl (sw_if_index);
...
static int
api_macip2_acl_add (vat_main_t * vam)
{
  /* acl2_test_main_t * sm = &acl2_test_main; */
  /* unformat_input_t * i = vam->input; */
  /* vl_api_macip2_acl_add_t * mp; */
  /* u32 msg_size = sizeof (*mp); /\* without the rules *\/ */

  /* vl_api_macip2_acl_rule_t *rules = 0; */
  /* int rule_idx = 0; */
  /* int n_rules = 0; */
  /* int n_rules_override = -1; */
  ...
  /* mp = vl_msg_api_alloc_as_if_client(msg_size); */
  /* clib_memset (mp, 0, msg_size); */
  /* mp->_vl_msg_id = ntohs (VL_API_MACIP2_ACL_ADD + sm->msg_id_base); */
  /* mp->client_index = vam->my_client_index; */
  /* if ((n_rules > 0) && rules) */
...
static int
api_macip2_acl_add_replace (vat_main_t * vam)
{
  /* acl2_test_main_t * sm = &acl2_test_main; */
  /* unformat_input_t * i = vam->input; */
  /* vl_api_macip2_acl_add_replace_t * mp; */
  /* u32 acl_index = ~0; */
  /* u32 msg_size = sizeof (*mp); /\* without the rules *\/ */
  ...
  /* mp = vl_msg_api_alloc_as_if_client(msg_size); */
  /* clib_memset (mp, 0, msg_size); */
  /* mp->_vl_msg_id = ntohs (VL_API_MACIP2_ACL_ADD_REPLACE + sm->msg_id_base); */
  /* mp->client_index = vam->my_client_index; */
  /* if ((n_rules > 0) && rules) */
  /*   clib_memcpy(mp->r, rules, n_rules*sizeof (mp->r[0])); */
  /* if (tag) */
