...
acl_switch_inline (vlib_main_t * vm,
		   ...
		   vlib_dir_t dir,
		   vnet_link_t linkt,
		   ip_address_family_t af, bool do_track, bool do_counters)
{
  vlib_buffer_t *bufs[VLIB_FRAME_SIZE], **b = bufs;
  u32 n_left, *from, pkts_acl_permit, thread_index;
...
acl_switch (vlib_main_t * vm,
	    ...
	    vlib_frame_t * frame,
	    vlib_dir_t dir,
	    vnet_link_t linkt, ip_address_family_t af, bool do_track)
{
  if (acl2_main.counters_enabled)
    return acl_switch_inline (vm, node, frame, dir, linkt, af, do_track,
			      true);
  else
    return acl_switch_inline (vm, node, frame, dir, linkt, af, do_track,
			      false);
...
VLIB_NODE_FN (acl_track_in_l2_ip6_node) (vlib_main_t * vm,
					 vlib_node_runtime_t * node,
					 vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_RX, VNET_LINK_ETHERNET, AF_IP6,
...
VLIB_NODE_FN (acl_track_in_l2_ip4_node) (vlib_main_t * vm,
					 vlib_node_runtime_t * node,
					 vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_RX, VNET_LINK_ETHERNET, AF_IP4,
...
VLIB_NODE_FN (acl_track_out_l2_ip6_node) (vlib_main_t * vm,
					  vlib_node_runtime_t * node,
					  vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_TX, VNET_LINK_ETHERNET, AF_IP6,
...
VLIB_NODE_FN (acl_track_out_l2_ip4_node) (vlib_main_t * vm,
					  vlib_node_runtime_t * node,
					  vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_TX, VNET_LINK_ETHERNET, AF_IP4,
...
VLIB_NODE_FN (acl_in_ip6_node) (vlib_main_t * vm,
				...
				vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_RX, VNET_LINK_IP6, AF_IP6, false);
}
...
VLIB_NODE_FN (acl_in_ip4_node) (vlib_main_t * vm,
				...
				vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_RX, VNET_LINK_IP4, AF_IP4, false);
}
...
VLIB_NODE_FN (acl_out_ip6_node) (vlib_main_t * vm,
				 ...
				 vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_TX, VNET_LINK_IP6, AF_IP6, false);
}
...
VLIB_NODE_FN (acl_out_ip4_node) (vlib_main_t * vm,
				 ...
				 vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_TX, VNET_LINK_IP4, AF_IP4, false);
}

VLIB_NODE_FN (acl_track_in_ip6_node) (vlib_main_t * vm,
				      vlib_node_runtime_t * node,
				      vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_RX, VNET_LINK_IP6, AF_IP6, true);
}

VLIB_NODE_FN (acl_track_in_ip4_node) (vlib_main_t * vm,
				      vlib_node_runtime_t * node,
				      vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_RX, VNET_LINK_IP4, AF_IP4, true);
}

VLIB_NODE_FN (acl_track_out_ip6_node) (vlib_main_t * vm,
				       vlib_node_runtime_t * node,
				       vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_TX, VNET_LINK_IP6, AF_IP6, true);
}

VLIB_NODE_FN (acl_track_out_ip4_node) (vlib_main_t * vm,
				       vlib_node_runtime_t * node,
				       vlib_frame_t * frame)
{
  return acl_switch (vm, node, frame, VLIB_TX, VNET_LINK_IP4, AF_IP4, true);
}
...
VLIB_REGISTER_NODE (acl_track_in_l2_ip6_node) =
{
  .name = "acl2-track-in-ip6-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_acl2_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (acl2_error_strings),
...
VNET_FEATURE_INIT (acl_track_in_l2_ip6_feature, static) =
{
  .arc_name = "l2-input-ip6",
  .node_name = "acl2-track-in-ip6-l2",
  .runs_before = VNET_FEATURES ("l2-input-feat-arc-end"),
};

VLIB_REGISTER_NODE (acl_track_in_l2_ip4_node) =
{
  .name = "acl2-track-in-ip4-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_acl2_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (acl2_error_strings),
...
VNET_FEATURE_INIT (acl_track_in_l2_ip4_feature, static) =
{
  .arc_name = "l2-input-ip4",
  .node_name = "acl2-track-in-ip4-l2",
  .runs_before = VNET_FEATURES ("l2-input-feat-arc-end"),
};

VLIB_REGISTER_NODE (acl_track_out_l2_ip6_node) =
{
  .name = "acl2-track-out-ip6-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_acl2_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (acl2_error_strings),
...
VNET_FEATURE_INIT (acl_track_out_l2_ip6_feature, static) =
{
  .arc_name = "l2-output-ip6",
  .node_name = "acl2-track-out-ip6-l2",
  .runs_before = VNET_FEATURES ("l2-output-feat-arc-end"),
};

VLIB_REGISTER_NODE (acl_track_out_l2_ip4_node) =
{
  .name = "acl2-track-out-ip4-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_acl2_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (acl2_error_strings),
...
VNET_FEATURE_INIT (acl_track_out_l2_ip4_feature, static) =
{
  .arc_name = "l2-output-ip4",
  .node_name = "acl2-track-out-ip4-l2",
  .runs_before = VNET_FEATURES ("l2-output-feat-arc-end"),
};

VLIB_REGISTER_NODE (acl_track_in_ip6_node) =
{
  .name = "acl2-track-in-ip6",
  .vector_size = sizeof (u32),
  .format_trace = format_acl2_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (acl2_error_strings),
...
VNET_FEATURE_INIT (acl_track_in_ip6_feature, static) =
{
  .arc_name = "ip6-unicast",
  .node_name = "acl2-track-in-ip6",
  .runs_before = VNET_FEATURES ("ip6-flow-classify"),
};

VLIB_REGISTER_NODE (acl_track_in_ip4_node) =
{
  .name = "acl2-track-in-ip4",
  .vector_size = sizeof (u32),
  .format_trace = format_acl2_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (acl2_error_strings),
...
VNET_FEATURE_INIT (acl_track_in_ip4_feature, static) =
{
  .arc_name = "ip4-unicast",
  .node_name = "acl2-track-in-ip4",
  .runs_before = VNET_FEATURES ("ip4-flow-classify"),
};

VLIB_REGISTER_NODE (acl_track_out_ip6_node) =
{
  .name = "acl2-track-out-ip6",
  .vector_size = sizeof (u32),
  .format_trace = format_acl2_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (acl2_error_strings),
...
VNET_FEATURE_INIT (acl_track_out_ip6_feature, static) =
{
  .arc_name = "ip6-output",
  .node_name = "acl2-track-out-ip6",
  .runs_before = VNET_FEATURES ("interface-output"),
};

VLIB_REGISTER_NODE (acl_track_out_ip4_node) =
{
  .name = "acl2-track-out-ip4",
  .vector_size = sizeof (u32),
  .format_trace = format_acl2_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (acl2_error_strings),
...
VNET_FEATURE_INIT (acl_track_out_ip4_feature, static) =
{
  .arc_name = "ip4-output",
  .node_name = "acl2-track-out-ip4",
  .runs_before = VNET_FEATURES ("interface-output"),
};
