...
static void
macip_rule_to_match_rule (const vl_api_macip2_acl_rule_t * rule,
  ...
  if (macip_acl_is_link_l2 (sw_if_index))
  ...
  else
    {
      ...
      ({
        ...
        am->macip_match_apps_by_sw_if_index[linkt][sw_if_index] =
          mam->apps[sw_if_index] =
          match_set_apply (mam->msi, MATCH_SEMANTIC_FIRST, linkt, tag_flags);
...
int
macip_acl_add_list (u32 count, vl_api_macip2_acl_rule_t rules[],
  ...
  for (i = 0; i < count; i++)
    {
      ...
      match_rule_t mr1 = {
	.mr_type = MATCH_TYPE_MASK_SRC_IP_MAC,
      };

      macip_rule_to_match_rule (&rules[i], &mr1, &action);

      if (mr1.mr_mask_src_ip_mac.mmim_ip.mip_ip.addr.version == AF_IP4)
	{
	  linkt = VNET_LINK_IP4;
  ...
  if (~0 == mai)
    {
      ...
      ({
        ...
        mam->msi = match_set_create_and_lock (name, acl_mk_heap());
        vec_free(name);
 ...
 */
static int
macip_acl_interface_del_acl (macip_acl_main_t * mm, u32 sw_if_index)
{
  u32 macip_acl_index;
  macip_acl_t *a;
...
static int
macip_acl_interface_add_acl (macip_acl_main_t * mm, u32 sw_if_index,
			     u32 macip_acl_index)
{
  macip_acl_t *a;
  ...
  if (~0 != mm->macip_acl_by_sw_if_index[sw_if_index])
    macip_acl_interface_del_acl (mm, sw_if_index);
				 ...
				 u32 macip_acl_index)
{
  ...
  if (is_add)
    rv = macip_acl_interface_add_acl (mm, sw_if_index, macip_acl_index);
  else
    rv = macip_acl_interface_del_acl (mm, sw_if_index);
...
format_macip_acl (u8 * s, va_list * args)
{
  ...
  ({
    s = format (s, "\n %U match-list:  %U",
                ...
                format_match_list_w_action, &mam->ml, 3,
...
{
  if (0 == is_add)
    macip_acl_interface_del_acl (&macip_acl_main, sw_if_index);
