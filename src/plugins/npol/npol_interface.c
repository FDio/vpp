				 ...
				 vlib_cli_command_t *cmd)
{
  ...
  u32 rx_policy_def = 1;
  u32 rx_profile_def = 1;
  u32 tx_policy_def = 1;
  u32 tx_profile_def = 1;
  u32 policy_id;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "rx-policy-def %d", &rx_policy_def))
	;
      else if (unformat (line_input, "rx-profile-def %d", &rx_profile_def))
	;
      else if (unformat (line_input, "tx-policy-def %d", &tx_policy_def))
	;
      else if (unformat (line_input, "tx-profile-def %d", &tx_profile_def))
	;
  ...
  if (vec_len (policy_list) < num_rx_policies + num_tx_policies)
    {
      error = clib_error_return (
	0, "number of rx and tx policies is greater than applied policies");
      goto done;
    }
  ...
  rv = npol_configure_policies (sw_if_index, num_rx_policies, num_tx_policies,
				vec_len (policy_list) - num_rx_policies -
				  num_tx_policies,
				policy_list, invert_rx_tx, rx_policy_def,
				tx_policy_def, rx_profile_def, tx_profile_def);
