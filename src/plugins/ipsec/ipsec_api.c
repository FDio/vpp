...
vl_api_ipsec_spd_add_del_t_handler (vl_api_ipsec_spd_add_del_t * mp)
{
  ...
  REPLY_MACRO (VL_API_IPSEC_SPD_ADD_DEL_REPLY + im->msg_id_base);
#endif
}
  ...
  (vl_api_ipsec_interface_add_del_spd_t * mp)
{
  ...
  REPLY_MACRO (VL_API_IPSEC_INTERFACE_ADD_DEL_SPD_REPLY + im->msg_id_base);
}
  ...
  (vl_api_ipsec_tunnel_protect_update_t * mp)
{
  ...
  REPLY_MACRO (VL_API_IPSEC_TUNNEL_PROTECT_UPDATE_REPLY + im->msg_id_base);
}
  ...
  (vl_api_ipsec_tunnel_protect_del_t * mp)
{
  ...
  REPLY_MACRO (VL_API_IPSEC_TUNNEL_PROTECT_DEL_REPLY + im->msg_id_base);
}
...
send_ipsec_tunnel_protect_details (index_t itpi, void *arg)
{
  ...
  mp->_vl_msg_id =
    ntohs (VL_API_IPSEC_TUNNEL_PROTECT_DETAILS + im->msg_id_base);
...
out:
  /* *INDENT-OFF* */
  REPLY_MACRO2 (VL_API_IPSEC_SPD_ENTRY_ADD_DEL_REPLY + im->msg_id_base,
...
out:
  /* *INDENT-OFF* */
  REPLY_MACRO2 (VL_API_IPSEC_SAD_ENTRY_ADD_DEL_REPLY + im->msg_id_base,
...
out:
  /* *INDENT-OFF* */
  REPLY_MACRO2 (VL_API_IPSEC_SAD_ENTRY_ADD_DEL_V2_REPLY + im->msg_id_base,
		{ rmp->stat_index = htonl (sa_index); });
  /* *INDENT-ON* */
}
...
out:
  REPLY_MACRO2 (VL_API_IPSEC_SAD_ENTRY_ADD_DEL_V3_REPLY + im->msg_id_base,
		{ rmp->stat_index = htonl (sa_index); });
}
			 ...
			 u32 context)
{
  ...
  clib_memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id = ntohs (VL_API_IPSEC_SPDS_DETAILS + im->msg_id_base);
  mp->context = context;
			...
			u32 context)
{
  ...
  mp = vl_msg_api_alloc (sizeof (*mp));
  clib_memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id = ntohs (VL_API_IPSEC_SPD_DETAILS + im->msg_id_base);
  mp->context = context;
				  ...
				  u32 sw_if_index, u32 context)
{
  ...
  clib_memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id =
    ntohs (VL_API_IPSEC_SPD_INTERFACE_DETAILS + im->msg_id_base);
  mp->context = context;
...
vl_api_ipsec_itf_create_t_handler (vl_api_ipsec_itf_create_t * mp)
{
  ...
  /* *INDENT-OFF* */
  REPLY_MACRO2 (VL_API_IPSEC_ITF_CREATE_REPLY + im->msg_id_base,
...
vl_api_ipsec_itf_delete_t_handler (vl_api_ipsec_itf_delete_t * mp)
{
  ...
  REPLY_MACRO (VL_API_IPSEC_ITF_DELETE_REPLY + im->msg_id_base);
}
...
send_ipsec_sa_details (ipsec_sa_t * sa, void *arg)
{
  ...
  mp = vl_msg_api_alloc (sizeof (*mp));
  clib_memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id = ntohs (VL_API_IPSEC_SA_DETAILS + im->msg_id_base);
  mp->context = ctx->context;
...
send_ipsec_sa_v2_details (ipsec_sa_t * sa, void *arg)
{
  ...
  mp = vl_msg_api_alloc (sizeof (*mp));
  clib_memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id = ntohs (VL_API_IPSEC_SA_V2_DETAILS + im->msg_id_base);
  mp->context = ctx->context;
...
send_ipsec_sa_v3_details (ipsec_sa_t *sa, void *arg)
{
  ...
  mp = vl_msg_api_alloc (sizeof (*mp));
  clib_memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id = ntohs (VL_API_IPSEC_SA_V3_DETAILS + im->msg_id_base);
  mp->context = ctx->context;
...
vl_api_ipsec_backend_dump_t_handler (vl_api_ipsec_backend_dump_t * mp)
{
  ...
  pool_foreach (ab, im->ah_backends) {
    ...
    clib_memset (mp, 0, sizeof (*mp));
    mp->_vl_msg_id = ntohs (VL_API_IPSEC_BACKEND_DETAILS + im->msg_id_base);
    mp->context = context;
...
vl_api_ipsec_select_backend_t_handler (vl_api_ipsec_select_backend_t * mp)
{
  ...
  REPLY_MACRO (VL_API_IPSEC_SELECT_BACKEND_REPLY + im->msg_id_base);
}
...
vl_api_ipsec_set_async_mode_t_handler (vl_api_ipsec_set_async_mode_t * mp)
{
  ...
  REPLY_MACRO (VL_API_IPSEC_SET_ASYNC_MODE_REPLY + im->msg_id_base);
}

#include <ipsec/ipsec.api.c>
static clib_error_t *
