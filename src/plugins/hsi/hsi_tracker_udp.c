  ...
  if (!hsi_udp_track_peer_update (a->session_handle, a->peer_session_handle))
    {
      ...
      hsi_session_send_cleanup_pair (a->target_session_handle);
				     ...
				     session_handle_t peer_session_handle)
{
  ...
  a->target_session_handle = session_handle;
  a->session_handle =
    hsi_test_peer_rpc_fail_consume (TRANSPORT_PROTO_UDP, HSI_TEST_PEER_RPC_FAIL_PEER_UPDATE) ?
      SESSION_INVALID_HANDLE :
      a->target_session_handle;
...
hsi_udp_track_send_drain_start (session_t *s, session_t *peer_s)
{
  ...
  a->owner_thread = thread_index;
  a->target_session_handle = session_handle (peer_s);
  a->session_handle =
    hsi_test_peer_rpc_fail_consume (TRANSPORT_PROTO_UDP, HSI_TEST_PEER_RPC_FAIL_DRAIN_START) ?
      SESSION_INVALID_HANDLE :
      a->target_session_handle;
...
hsi_udp_track_send_commit (session_t *peer_s, hsi_udp_track_snapshot_t *peer)
{
  ...
  a->target_session_handle = session_handle (peer_s);
  a->session_handle =
    hsi_test_peer_rpc_fail_consume (TRANSPORT_PROTO_UDP, HSI_TEST_PEER_RPC_FAIL_COMMIT) ?
      SESSION_INVALID_HANDLE :
      a->target_session_handle;
  a->peer = *peer;
