...
static clib_error_t *
hsi_test_peer_rpc_fail_command_fn (vlib_main_t *vm, unformat_input_t *input,
				   vlib_cli_command_t *cmd)
{
  hsi_main_t *hm = &hsi_main;
  transport_proto_t proto;
  hsi_test_peer_rpc_fail_op_t op;
  ...
  if (unformat (input, "tcp"))
    ...
    return clib_error_return (0, "expected tcp or udp");

  if (unformat (input, "commit"))
    op = HSI_TEST_PEER_RPC_FAIL_COMMIT;
  else if (unformat (input, "drain-start"))
    op = HSI_TEST_PEER_RPC_FAIL_DRAIN_START;
  else if (unformat (input, "peer-update"))
    op = HSI_TEST_PEER_RPC_FAIL_PEER_UPDATE;
  else
    return clib_error_return (0, "expected commit, drain-start or peer-update");

  if (!unformat (input, "once"))
    return clib_error_return (0, "expected once");

  hm->test_peer_rpc_fail_proto = proto;
  hm->test_peer_rpc_fail_op = op;
  hm->test_peer_rpc_fail_once = 1;
  return 0;
}

VLIB_CLI_COMMAND (hsi_test_peer_rpc_fail_command, static) = {
  .path = "test hsi peer-rpc-fail",
  .short_help = "test hsi peer-rpc-fail [tcp|udp] "
		"[commit|drain-start|peer-update] once",
  .function = hsi_test_peer_rpc_fail_command_fn,
};
