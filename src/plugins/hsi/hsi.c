...
hsi_tracked_action_to_lookup_result (hsi_tracked_action_t action, u32 *next)
{
  switch (action)
    ...
    case HSI_TRACKED_ACTION_CONSUME:
...
always_inline void
hsi_enqueue_owned_buffer (u32 **to_next, u16 **next, u32 *n_to_next, u32 bi, u32 next_index,
			  u8 owned)
{
  (*to_next)[0] = bi;
  (*next)[0] = next_index;
  ...
  while (n_left_from >= 4)
    {
      ...
      owned0 = hsi_lookup_and_update (vm, b[0], &next0, is_ip4, is_input);
      hsi_enqueue_owned_buffer (&to_next, &next, &n_to_next, from[0], next0, owned0);

      owned1 = hsi_lookup_and_update (vm, b[1], &next1, is_ip4, is_input);
      hsi_enqueue_owned_buffer (&to_next, &next, &n_to_next, from[1], next1, owned1);
