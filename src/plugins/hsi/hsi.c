...
hsi_lookup_and_update (vlib_buffer_t *b, u32 *next, u8 is_ip4, u8 is_input)
{
  ...
  switch (proto)
	...
	{
	  if (!hsi_have_intercept_proto (proto, is_ip4) || !tcp_syn (tcp_hdr))
	    {
	      vnet_feature_next (next, b);
	      break;
	    }
	  vnet_buffer (b)->tcp.connection_index =
	    hsi_main.intercept_listeners[!is_ip4][proto];
	  *next = HSI_INPUT_NEXT_TCP_LISTEN;
	}
      break;
    case IP_PROTOCOL_UDP:
      have_udp = hsi_udp_lookup (b, ip_hdr, is_ip4);
      if (!have_udp && !hsi_have_intercept_proto (proto, is_ip4))
	{
	  vnet_feature_next (next, b);
	  break;
	}
      /* NEED udp_no_lookup node */
