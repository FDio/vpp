...
VLIB_CLI_COMMAND (hsi_command, static) = {
  .path = "hsi",
  .short_help = "hsi [intercept [tcp | udp | all]] [tcp drain-cache max-packets <n>]",
  ...
  vec_foreach_index (i, hm->wrk)
    {
      ...
      vlib_cli_output (vm,
		       "thread %u tcp-drain-cached %lu tcp-drain-cache-flushed %lu "
		       "tcp-drain-cache-dropped %lu tcp-drain-stalled %lu",
		       i, wrk->tcp_drain_cached, wrk->tcp_drain_cache_flushed,
		       wrk->tcp_drain_cache_dropped, wrk->tcp_drain_stalled);
    }
