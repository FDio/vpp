  ...
  if (!s || !peer_s)
    {
      ...
      hsi_tcp_track_send_abort (a->target_session_handle);
...
hsi_tcp_track_send_drain_start (session_t *s, session_t *peer_s)
{
  ...
  a->session_handle =
    hsi_test_peer_rpc_fail_consume (TRANSPORT_PROTO_TCP, HSI_TEST_PEER_RPC_FAIL_DRAIN_START) ?
...
hsi_tcp_track_send_commit (session_t *peer_s, hsi_tcp_track_snapshot_t *peer)
{
  ...
  a->target_session_handle = session_handle (peer_s);
  a->session_handle =
    hsi_test_peer_rpc_fail_consume (TRANSPORT_PROTO_TCP, HSI_TEST_PEER_RPC_FAIL_COMMIT) ?
      SESSION_INVALID_HANDLE :
      a->target_session_handle;
  a->peer = *peer;
