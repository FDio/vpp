...
hsi_test_peer_rpc_fail_consume (transport_proto_t proto, hsi_test_peer_rpc_fail_op_t op)
{
  hsi_main_t *hm = &hsi_main;

  if (!hm->test_peer_rpc_fail_once || hm->test_peer_rpc_fail_proto != proto ||
      hm->test_peer_rpc_fail_op != op)
    return 0;

  hm->test_peer_rpc_fail_once = 0;
  hm->test_peer_rpc_fail_op = HSI_TEST_PEER_RPC_FAIL_NONE;
  return 1;
}
...
struct hsi_udp_track_commit_req_
{
  ...
  session_handle_t session_handle;
  session_handle_t target_session_handle;
