...
static int hsi_tcp_drain_try_complete (session_t *s, tcp_connection_t *tc, hsi_tcp_drain_t *drain,
				       f64 now);
static int hsi_udp_drain_try_complete (session_t *s, udp_connection_t *uc, hsi_udp_drain_t *drain,
				       f64 now);
static void hsi_tcp_drain_enqueue_cached_buffers (vlib_main_t *vm, u32 node_index, u32 *buffers);
...
static void hsi_tcp_drain_unregister_time_update_rpc (void *arg);
static void hsi_udp_drain_unregister_time_update_rpc (void *arg);
static void hsi_tcp_fin_wait_unregister_time_update_rpc (void *arg);
static u8 hsi_session_cleanup_fifos (session_t *s);
static void hsi_session_send_cleanup_pair (session_handle_t first);
...
hsi_session_cleanup_peer_handle (session_t *s)
{
  ...
  switch (proto)
	...
	return p ? p[0] : SESSION_INVALID_HANDLE;
      }
    default:
      return SESSION_INVALID_HANDLE;
    }
			    ...
			    udp_header_t *udp_hdr, u8 is_ip4)
{
  ...
  if (PREDICT_FALSE (vec_len (drain->cached_buffers) >= hsi_main.udp_drain_cache_max_packets ||
		     drain->cached_bytes + len > HSI_UDP_DRAIN_CACHE_MAX_BYTES))
    {
...
hsi_udp_drain_stop (udp_connection_t *uc)
{
  ...
  if (vec_len (drain->cached_buffers))
    {
      vlib_buffer_free (vlib_get_main_by_index (uc->c_thread_index), drain->cached_buffers,
			vec_len (drain->cached_buffers));
      hsi_worker_counter_add (wrk, udp_drain_cache_dropped, vec_len (drain->cached_buffers));
      vec_free (drain->cached_buffers);
    }
...
hsi_udp_drain_try_complete (session_t *s, udp_connection_t *uc, hsi_udp_drain_t *drain, f64 now)
{
  ...
  else if (hsi_udp_track_needs_drain (peer_s))
    return 0;
...
hsi_tcp_fin_wait_update_time (f64 time_now, u8 thread_index)
{
  ...
  hash_foreach (key, value, wrk->tcp_fin_wait_by_session_conn, ({
		  (void) value;
		  vec_add1 (keys, key);
		}));
...
hsi_tracker_show (vlib_main_t *vm)
{
  ...
  vec_foreach_index (i, hm->wrk)
	...
	vlib_cli_output (vm, "thread %u tcp-drain-active %u tcp-fin-wait-active %u", i,
      ...
      if (pool_elts (wrk->udp_drains))
	vlib_cli_output (vm, "thread %u udp-drain-active %u", i, (u32) pool_elts (wrk->udp_drains));
