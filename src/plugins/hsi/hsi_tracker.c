...
typedef struct hsi_session_cleanup_pair_req_
{
  session_handle_t first;
  session_handle_t second;
} hsi_session_cleanup_pair_req_t;

typedef struct hsi_session_fifo_dealloc_req_
{
  svm_fifo_t *rx_fifo;
  svm_fifo_t *tx_fifo;
} hsi_session_fifo_dealloc_req_t;
...
static_always_inline u8 hsi_tcp_segment_data_len (void *ip_hdr, tcp_header_t *tcp_hdr, u8 is_ip4,
						  u32 *data_len);
static_always_inline u8 hsi_tcp_segment_in_rcv_wnd (tcp_connection_t *tc, u32 seq, u32 seq_end);
static_always_inline void hsi_tcp_update_snd_wnd (tcp_connection_t *tc, u32 seq, u32 ack,
						  tcp_header_t *tcp_hdr);
static int hsi_tcp_drain_try_complete (session_t *s, tcp_connection_t *tc, hsi_tcp_drain_t *drain);
static void hsi_tcp_track_abort_session (session_t *s);
static void hsi_tcp_track_send_abort (session_handle_t session_handle);
static void hsi_tcp_fin_wait_update_time (f64 time_now, u8 thread_index);
			    ...
			    tcp_header_t *tcp_hdr, u8 is_ip4)
{
  ...
  if (PREDICT_FALSE (vec_len (drain->cached_buffers) >= hsi_main.tcp_drain_cache_max_packets ||
		     drain->cached_bytes + len > HSI_TCP_DRAIN_CACHE_MAX_BYTES))
    {
  ...
  vec_add1 (drain->cached_buffers, vlib_get_buffer_index (vm, b));
  drain->cached_bytes += len;
  ...
  if (s)
	...
	session_program_tx_io_evt (session_handle (s), SESSION_IO_EVT_TX);
      hsi_tcp_drain_try_complete (s, tc, drain);
    }
...
hsi_tcp_drain_stop (tcp_connection_t *tc)
{
  ...
  if (vec_len (drain->cached_buffers))
    {
      vlib_buffer_free (vlib_get_main_by_index (tc->c_thread_index), drain->cached_buffers,
			vec_len (drain->cached_buffers));
      hsi_worker_counter_add (wrk, tcp_drain_cache_dropped, vec_len (drain->cached_buffers));
      vec_free (drain->cached_buffers);
    }
...
hsi_tcp_drain_try_complete (session_t *s, tcp_connection_t *tc, hsi_tcp_drain_t *drain)
{
  ...
  rv = hsi_tcp_drain_update_and_needs_drain (s, tc, drain);
  if (rv < 0)
    {
      hsi_tcp_drain_fail_pair (drain);
      return -1;
  ...
  if (s->thread_index == peer_s->thread_index)
      ...
      if (peer_drain)
	{
	  if (peer_drain->state == HSI_TCP_DRAIN_STATE_FAILED)
	    return -1;
	  rv = hsi_tcp_drain_update_and_needs_drain (peer_s, peer_tc, peer_drain);
	  if (rv < 0)
	    {
...
hsi_tcp_track_session_pair (session_t *s, session_t *peer_s)
{
  ...
  ASSERT (hsi_tcp_track_is_possible (tc0, tc1));
...
hsi_udp_track_session_pair (session_t *s, session_t *peer_s)
{
  ...
  ASSERT (hsi_udp_track_is_possible (uc0, uc1));
  ASSERT (!hsi_udp_track_needs_drain (s));
  ASSERT (!hsi_udp_track_needs_drain (peer_s));
...
static void
hsi_session_fifo_dealloc_rpc (void *arg)
{
  hsi_session_fifo_dealloc_req_t *a = arg;
...
static_always_inline void
hsi_session_dealloc_fifos (svm_fifo_t *rx_fifo, svm_fifo_t *tx_fifo)
{
  hsi_session_fifo_dealloc_req_t *a;
  clib_thread_index_t master_thread;

  if (!rx_fifo || !tx_fifo)
    return;

  master_thread = rx_fifo->master_thread_index;
  if (master_thread == vlib_get_thread_index () || rx_fifo->refcnt > 1 ||
  ...
  a->rx_fifo = rx_fifo;
  a->tx_fifo = tx_fifo;
  session_send_rpc_evt_to_thread_force (master_thread, hsi_session_fifo_dealloc_rpc, a);
}
...
hsi_session_transport_cleanup (session_t *s)
{
  ...
  hsi_session_dealloc_fifos (rx_fifo, tx_fifo);
  s->rx_fifo = 0;
  s->tx_fifo = 0;
  session_free (s);
}
...
hsi_session_cleanup (session_t *s)
{
  ...
  hsi_worker_proto_counter_inc (hsi_worker_get (vlib_get_thread_index ()), proto,
				cleanup_completed);
}
...
static void hsi_session_send_cleanup (session_handle_t session_handle);

static void
hsi_session_cleanup_pair_rpc (void *arg)
{
  hsi_session_cleanup_pair_req_t *a = arg;
  session_handle_tu_t sh = { .handle = a->first };
  session_t *s;
  ...
  if (a->second != SESSION_INVALID_HANDLE)
    hsi_session_send_cleanup (a->second);
...
static void
hsi_session_send_cleanup_pair (session_handle_t first, session_handle_t second)
{
  session_handle_tu_t sh = { .handle = first };
  hsi_session_cleanup_pair_req_t *a;

  a = clib_mem_alloc (sizeof (*a));
  a->first = first;
  a->second = second;
  session_send_rpc_evt_to_thread_force (sh.thread_index, hsi_session_cleanup_pair_rpc, a);
}

static_always_inline u8
hsi_session_uses_borrowed_fifos (session_t *s)
{
				     ...
				     hsi_tcp_cleanup_reason_t reason)
{
  ...
  session_t *peer_s;
  hsi_worker_t *wrk;
  session_handle_t local_handle, first, second;
  u8 local_borrowed, peer_borrowed;
  ...
  local_borrowed = hsi_session_uses_borrowed_fifos (
    session_get_from_handle_safe ((session_handle_tu_t){ .handle = local_handle }));
  peer_borrowed = peer_s && hsi_session_uses_borrowed_fifos (peer_s);

  if (local_borrowed && !peer_borrowed)
    {
      first = local_handle;
      second = trk->peer_session_handle;
    }
  else
    {
  ...
  hsi_session_send_cleanup_pair (first, second);
}
...
hsi_tcp_fin_wait_update_time (f64 time_now, u8 thread_index)
{
  ...
  pool_foreach (tc, tcp_main.wrk[thread_index].connections)
    {
      if (!(tc->cfg_flags & TCP_CFG_F_TRACKED) || tc->state != TCP_STATE_CLOSED)
	continue;
...
hsi_tracker_show (vlib_main_t *vm)
{
  ...
  vec_foreach_index (i, hm->wrk)
	...
	{
	  vlib_cli_output (vm,
			   "thread %u tcp-drain session 0x%lx peer 0x%lx state %s cache %u/%u "
			   "bytes %u age %.3f idle %.3f",
			   i, drain->session_handle, drain->peer_session_handle,
			   hsi_tcp_drain_state_name (drain->state), vec_len (drain->cached_buffers),
			   hsi_main.tcp_drain_cache_max_packets, drain->cached_bytes,
			   now - drain->start_time, now - drain->last_progress_time);
	}
      ...
      pool_foreach (tc, tcp_main.wrk[i].connections)
	{
	  ...
	  vlib_cli_output (vm,
			   "thread %u tcp-tracked session 0x%lx peer 0x%lx "
			   "flags 0x%x peer-fin-ack %u seq-delta %d ack-delta %d "
			   "packets %lu bytes %lu",
			   i, session_make_handle (tc->c_s_index, tc->c_thread_index),
			   trk->peer_session_handle, trk->flags, trk->peer_fin_ack, trk->seq_delta,
