...
{
  return hsi_tcp_drain_get (s->thread_index, hsi_session_conn_key_from_session (s));
}
				   ...
				   tcp_header_t *tcp_hdr, u32 data_len, u32 *seq, u32 *ack)
{
  ...
  *ack = clib_net_to_host_u32 (tcp_hdr->ack_number);
  seq_end = *seq + data_len + tcp_fin (tcp_hdr);
				   ...
				   void *ip_hdr, tcp_header_t *tcp_hdr, u8 is_ip4)
{
  ...
  if (tcp_ack (tcp_hdr) && !rst && (tcp_fin (tcp_hdr) || (trk->flags & HSI_TRACKER_F_FIN_RCVD)))
