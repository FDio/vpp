...
hsi_session_get_from_handle_safe (session_handle_t peer_session_handle)
{
  session_handle_tu_t handle = { .handle = peer_session_handle };

  if (peer_session_handle == SESSION_INVALID_HANDLE ||
      handle.thread_index >= vec_len (session_main.wrk))
    return 0;

  return session_get_from_handle_safe (handle);
}
...
static int
hsi_tcp_track_session_pair (session_t *s, session_handle_t peer_session_handle)
{
  ...
  peer_s = hsi_session_get_from_handle_safe (peer_session_handle);
...
static int
hsi_udp_track_session_pair (session_t *s, session_handle_t peer_session_handle)
{
...
hsi_track_session_pair (session_t *s, session_handle_t peer_session_handle)
{
  ...
  switch (proto)
    ...
    case TRANSPORT_PROTO_TCP:
      return hsi_tcp_track_session_pair (s, peer_session_handle);
    case TRANSPORT_PROTO_UDP:
      return hsi_udp_track_session_pair (s, peer_session_handle);
    default:
      return -1;
    }
}
