...
drop:
  hsi_worker_get (tc->c_thread_index)->tcp_drain_cache_dropped++;
  return HSI_TCP_TRACKED_ACTION_DROP;
}
  ...
  if (vec_len (drain->cached_buffers))
    {
      ...
      wrk->tcp_drain_cache_dropped += vec_len (drain->cached_buffers);
...
hsi_tcp_drain_flush_cached_buffers (tcp_connection_t *tc)
{
  ...
  if (vec_len (forward))
	...
	vm, tc->c_is_ip4 ? ip4_lookup_node.index : ip6_lookup_node.index, forward);
      wrk->tcp_drain_cache_flushed += vec_len (forward);
    }

  if (vec_len (drops))
    {
      vlib_buffer_free (vm, drops, vec_len (drops));
      wrk->tcp_drain_cache_dropped += vec_len (drops);
    }
