...
struct hsi_tcp_track_commit_req_
{
...
static_always_inline void
hsi_udp_tracker_pending_clear (udp_connection_t *uc)
{
  hsi_udp_tracker_pending_t *trk = hsi_udp_tracker_pending_from_connection (uc);

  if (trk->magic == HSI_UDP_TRACKER_MAGIC)
    {
      trk->flags &= ~HSI_UDP_TRACKER_F_PENDING;
      if (!trk->flags)
	trk->magic = 0;
...
static void
hsi_tcp_track_prepare (session_t *s, tcp_connection_t *tc)
{
  hsi_tcp_track_snapshot_t snap;

  hsi_tcp_track_snapshot (s, tc, &snap);
  hsi_tcp_tracker_pending_set (tc, &snap);
}
...
static int
hsi_tcp_track_send_commit (session_t *s, hsi_tcp_track_snapshot_t *peer)
{
  ...
  a->conn_index = s->connection_index;
  a->peer = *peer;
  session_send_rpc_evt_to_thread (s->thread_index, hsi_tcp_track_commit_rpc, a);

  return 0;
}

static int
hsi_tcp_track_session_pair (session_t *s0, session_t *s1)
{
  tcp_connection_t *tc0, *tc1;
  hsi_tcp_track_snapshot_t snap0, snap1;
  u8 is_same_thread;

  tc0 = hsi_tcp_connection_at_session (s0);
  tc1 = hsi_tcp_connection_at_session (s1);
  is_same_thread = s0->thread_index == s1->thread_index;
  ...
  if (is_same_thread)
    {
      hsi_tcp_track_snapshot (s0, tc0, &snap0);
      hsi_tcp_track_snapshot (s1, tc1, &snap1);

      hsi_tcp_track_commit (s0, &snap1);
      hsi_tcp_track_commit (s1, &snap0);
  ...
  if (hsi_tcp_track_send_commit (s1, &snap0))
    return -1;
...
static int
hsi_udp_track_snapshots_compatible (hsi_udp_track_snapshot_t *s0, hsi_udp_track_snapshot_t *s1)
{
  if (s0->session_handle == s1->session_handle)
    return 0;
  if (s0->is_ip4 != s1->is_ip4)
    return 0;
...
static int
hsi_udp_track_prepare (session_t *s, hsi_udp_track_snapshot_t *snap)
{
  udp_connection_t *uc;

  uc = hsi_udp_connection_at_session (s);

  if ((uc->cfg_flags & UDP_CFG_F_TRACKED) || hsi_udp_tracker_is_pending (uc))
    return -1;

  if (hsi_udp_track_snapshot (s, uc, snap))
    return -1;
...
static void
hsi_udp_track_abort (session_t *s)
{
  udp_connection_t *uc;

  uc = hsi_udp_connection_at_session (s);

  if (hsi_udp_tracker_is_pending (uc))
    hsi_udp_tracker_pending_clear (uc);
}
...
static int
hsi_udp_track_session_pair (session_t *s0, session_t *s1)
{
  hsi_udp_track_snapshot_t snap0, snap1;
  udp_connection_t *uc0, *uc1;

  if (s0->thread_index == s1->thread_index)
    {
      if (hsi_udp_track_prepare (s0, &snap0))
	return -1;

      if (hsi_udp_track_prepare (s1, &snap1))
	{
	  hsi_udp_track_abort (s0);
	  return -1;
	}

      if (!hsi_udp_track_snapshots_compatible (&snap0, &snap1))
	{
	  hsi_udp_track_abort (s0);
	  hsi_udp_track_abort (s1);
	  return -1;
	}

      hsi_udp_track_commit (s0, &snap1);
      hsi_udp_track_commit (s1, &snap0);

      return 0;
    }

  uc0 = hsi_udp_connection_at_session (s0);
  uc1 = hsi_udp_connection_at_session (s1);
  if (uc1->cfg_flags & UDP_CFG_F_TRACKED)
    return -1;

  if (hsi_udp_track_snapshot (s1, uc1, &snap1))
    return -1;
...
__clib_export int
hsi_track_session_pair (session_t *s0, session_t *s1)
{
  ...
  if (s0->session_type != s1->session_type)
    return -1;

  if (s0->session_state >= SESSION_STATE_TRANSPORT_CLOSING ||
      s1->session_state >= SESSION_STATE_TRANSPORT_CLOSING)
    return -1;

  if (thread_index != s0->thread_index)
    return -1;

  proto = session_get_transport_proto (s0);
  switch (proto)
    {
    case TRANSPORT_PROTO_TCP:
      return hsi_tcp_track_session_pair (s0, s1);
    case TRANSPORT_PROTO_UDP:
      return hsi_udp_track_session_pair (s0, s1);
    default:
      return -1;
    }
}
