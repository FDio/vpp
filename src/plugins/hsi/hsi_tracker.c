...
hsi_tcp_session_conn_key (u32 session_index, u32 conn_index)
{
  return ((uword) session_index << 32) | conn_index;
}

static_always_inline uword
hsi_tcp_session_conn_key_from_session (session_t *s)
{
  return hsi_tcp_session_conn_key (s->session_index, s->connection_index);
}

static_always_inline uword
hsi_tcp_session_conn_key_from_connection (tcp_connection_t *tc)
{
  return hsi_tcp_session_conn_key (tc->c_s_index, tc->c_c_index);
}
...
static_always_inline hsi_tcp_drain_t *
hsi_tcp_drain_get_from_session (session_t *s)
{
  return hsi_tcp_drain_get (s->thread_index, hsi_tcp_session_conn_key_from_session (s));
}
		     ...
		     tcp_connection_t *peer_tc)
{
  ...
  key = hsi_tcp_session_conn_key_from_session (s);
...
hsi_tcp_drain_try_complete (session_t *s, tcp_connection_t *tc, hsi_tcp_drain_t *drain)
{
  ...
  if (hsi_tcp_track_needs_drain (s, tc) || hsi_tcp_track_needs_drain (peer_s, peer_tc))
    return 0;
...
__clib_export int
hsi_track_session_pair_try_complete (session_t *s)
{
  clib_thread_index_t thread_index = vlib_get_thread_index ();
  transport_proto_t proto;

  if (!s || thread_index != s->thread_index)
    return -1;
  ...
  switch (proto)
	...
	return hsi_tcp_drain_try_complete (s, tc, hsi_tcp_drain_get_from_session (s));
      }
    case TRANSPORT_PROTO_UDP:
      {
	udp_connection_t *uc = hsi_udp_connection_at_session (s);

	return (uc->cfg_flags & UDP_CFG_F_TRACKED) ? 1 : -1;
      }
    default:
      return -1;
    }
