...
{
  ASSERT (s->thread_index == vlib_get_thread_index ());
  ASSERT (!s->rx_fifo || s->rx_fifo->master_thread_index == s->thread_index ||
	  s->rx_fifo->refcnt > 1 || vlib_thread_is_main_w_barrier ());
  ASSERT (!s->tx_fifo || s->tx_fifo->master_thread_index == s->thread_index ||
	  s->tx_fifo->refcnt > 1 || vlib_thread_is_main_w_barrier ());
