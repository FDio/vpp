...
{
.name = "GTPU",.format_device_name = format_gtpu_name,.format_tx_trace =
    format_gtpu_encap_trace,.tx_function =
...
{
.name = "GTPU",.format_header =
    format_gtpu_header_with_length,.build_rewrite = default_build_rewrite,};
...
{
#if (CLIB_DEBUG > 0)
  ASSERT (FIB_NODE_TYPE_GTPU_TUNNEL == node->fn_type);
#endif
...
static int
gtpu_rewrite (gtpu_tunnel_t * t, bool is_ip6)
{
  union
  {
  ...
  /* GTPU header */
  vnet_set_teid_and_flags (gtpu, t->teid);
  ...
  if (!is_ip6)
    {
      ...
      key4.teid = clib_host_to_net_u32 (a->teid << 8);
      p = hash_get (gtm->gtpu4_tunnel_by_key, key4.as_u64);
    }
  else
    {
      key6.src = a->dst.ip6;
      key6.teid = clib_host_to_net_u32 (a->teid << 8);
...
#undef _
      ...
      else
	{
      ...
      fib_node_init (&t->node, FIB_NODE_TYPE_GTPU_TUNNEL);
      ...
      if (!ip46_address_is_multicast (&t->dst))
	{
	  ...
	  t->sibling_index = fib_entry_child_add
	    (t->fib_entry_index, FIB_NODE_TYPE_GTPU_TUNNEL, t - gtm->tunnels);
				...
				vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (line_input, "teid %d", &teid))
	{
	  if (teid >> 24)
	    {
	      error = clib_error_return (0, "teid %d out of range", teid);
	      goto done;
	    }
	}
      else
  ...
  if (teid == 0)
    {
      error = clib_error_return (0, "teid not specified");
      goto done;
    }
...
gtpu_init (vlib_main_t * vm)
{
  ...
  fib_node_register_type (FIB_NODE_TYPE_VXLAN_TUNNEL, &gtpu_vft);
