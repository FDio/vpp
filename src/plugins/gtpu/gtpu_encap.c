		    ...
		    u32 is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (is_ip4)
	    {
	      /* IP4 GTPU header should be 36 octects */
              ASSERT(sizeof(ip4_gtpu_header_t) == 36);
              ASSERT(vec_len(t0->rewrite) == sizeof(ip4_gtpu_header_t));
	      ASSERT(vec_len(t1->rewrite) == sizeof(ip4_gtpu_header_t));
...
#undef _
	  ...
	  else /* ipv6 */
	    {
	      ...
	      /* IP6 GTPU header should be 56 octects */
              ASSERT(sizeof(ip6_gtpu_header_t) == 56);
              ASSERT(vec_len(t0->rewrite) == sizeof(ip6_gtpu_header_t));
	      ASSERT(vec_len(t1->rewrite) == sizeof(ip6_gtpu_header_t));
