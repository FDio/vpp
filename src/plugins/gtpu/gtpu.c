...
vnet_gtpu_add_del_rx_flow (u32 hw_if_index, u32 t_index, int is_add)
{
  ...
  if (is_add)
      ...
      if (t->flow_index == ~0)
	{
	  ...
	  vnet_flow_range_add (vnm, gtm->flow_range, &flow, &t->flow_index);
	}

      return vnet_flow_range_enable (vnm, gtm->flow_range, t->flow_index, hw_if_index);
    }

  /* flow index is removed when the tunnel is deleted */
  return vnet_flow_range_disable (vnm, gtm->flow_range, t->flow_index, hw_if_index);
}
...
gtpu_init (vlib_main_t * vm)
{
  ...
  gtm->flow_range = vnet_flow_alloc_range (gtm->vnet_main, "gtpu", 1024 * 1024);
