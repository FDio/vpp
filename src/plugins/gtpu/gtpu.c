...
u8 * format_gtpu_encap_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "GTPU encap to gtpu_tunnel%d tteid %u ",
...
format_gtpu_tunnel (u8 * s, va_list * args)
{
  ...
  s = format (s, "[%d] src %U dst %U teid %u tteid %u "
 ...
 * [0] src 10.0.3.1 dst 10.0.3.3 teid 13 tx-teid 55 encap_fib_index 0 sw_if_index 5 decap_next l2 pdu-disabled
 * @cliexend
 ?*/
/* *INDENT-OFF* */
...
static clib_error_t *
gtpu_forward_command_fn (vlib_main_t * vm,
			 unformat_input_t * input, vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  ...
  switch (rv)
    ...
    case VNET_API_ERROR_INVALID_ARGUMENT:
      error = clib_error_return (0,
				 "one and only one of unknown-teid, unknown-type or bad-header must be specified");
      goto done;
