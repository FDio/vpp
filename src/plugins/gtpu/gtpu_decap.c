      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  b1 = vlib_get_buffer (vm, bi1);
	    ...
	    vlib_buffer_advance
          ...
          if (is_ip4) {
            vlib_buffer_advance
              (b0, sizeof(*ip4_0)+sizeof(udp_header_t)+(gtpu0->length+8));
	    vlib_buffer_advance
              (b1, sizeof(*ip4_1)+sizeof(udp_header_t)+(gtpu0->length+8));
          } else {
	    vlib_buffer_advance
              (b0, sizeof(*ip6_0)+sizeof(udp_header_t)+(gtpu1->length+8));
            vlib_buffer_advance
              (b1, sizeof(*ip6_1)+sizeof(udp_header_t)+(gtpu1->length+8));
          }
	  ...
	  if (PREDICT_FALSE ((gtpu0->ver_flags & GTPU_VER) != GTPU_V1_VER))
	    {
	  ...
	  /* Manipulate packet 0 */
 	    ...
 	    /* Make sure GTPU tunnel exist according to packet SIP and teid */
           if (PREDICT_FALSE (key4_0.as_u64 != last_key4.as_u64))
              {
                p0 = hash_get (vxm->gtpu4_tunnel_by_key, key4_0.as_u64);
	    ...
	    goto trace0;
 	    ...
 	    /* Make sure GTPU tunnel exist according to packet SIP and teid */
           if (PREDICT_FALSE (memcmp(&key6_0, &last_key6, sizeof(last_key6)) != 0))
              {
                p0 = hash_get_mem (vxm->gtpu6_tunnel_by_key, &key6_0);
                if (PREDICT_FALSE (p0 == NULL))
                  {
                    error0 = GTPU_ERROR_NO_SUCH_TUNNEL;
                    next0 = GTPU_INPUT_NEXT_DROP;
	...
	next0:
	  ...
	  if (PREDICT_FALSE (sw_if_index0 != stats_sw_if_index)) 
        ...
        trace0:
          ...
          if (PREDICT_FALSE ((gtpu1->ver_flags & GTPU_VER) != GTPU_V1_VER))
	    {
	    ...
	    /* Make sure unicast GTPU tunnel exist by packet SIP and teid */
	    if (PREDICT_FALSE (key4_1.as_u64 != last_key4.as_u64))
              {
                p1 = hash_get (vxm->gtpu4_tunnel_by_key, key4_1.as_u64);
	    ...
	    /* Make sure GTPU tunnel exist according to packet SIP and teid */
            if (PREDICT_FALSE (memcmp(&key6_1, &last_key6, sizeof(last_key6)) != 0))
              {
                p1 = hash_get_mem (vxm->gtpu6_tunnel_by_key, &key6_1);
