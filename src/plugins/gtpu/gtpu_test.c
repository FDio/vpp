  ...
  (vl_api_gtpu_tunnel_details_t * mp)
{
  ...
  //print (vam->ofp, "%11d%24U%24U%14d%18d%13d%13d%19d" %12d%5d%15d%17d
...
static void vl_api_gtpu_add_del_forward_reply_t_handler
  ...
  if (!vam->json_output)
    {
      print (vam->ofp, "%11s%24s%24s%14s%18s%13s%13s%19s%12s%5s%15s%17s",
	     ...
	     "mcast_sw_if_index", "pdu_extension", "qfi", "is_forwarding",
	     "forwarding_type");
    }
