...
static void
ptls_vpp_crypto_aead_encrypt_init (ptls_aead_context_t * _ctx, const void * iv, const void * aad, size_t aadlen)
{
  struct vpp_aead_context_t * ctx = (struct vpp_aead_context_t *)_ctx;
  ctx->op.iv = (void *)iv;
  ctx->op.aad = (void *)aad;
...
static size_t
ptls_vpp_crypto_aead_encrypt_update (ptls_aead_context_t * _ctx, void * output, const void * input, size_t inlen)
{
  struct vpp_aead_context_t * ctx = (struct vpp_aead_context_t *)_ctx;
			       ...
			       const void *key, vnet_crypto_alg_t alg)
{
  ...
  ctx->op.key_index = vnet_crypto_key_add (vm, ctx->alg, (void *)key, key_len);
  ctx->chunk_index = 0;
...
{
  return ptls_vpp_crypto_aead_setup_crypto (ctx, is_enc, key, VNET_CRYPTO_ALG_AES_128_GCM);
}
...
ptls_vpp_crypto_aead_aes256gcm_setup_crypto (ptls_aead_context_t * ctx,
					 int is_enc, const void *key)
{
  return ptls_vpp_crypto_aead_setup_crypto (ctx, is_enc, key, VNET_CRYPTO_ALG_AES_256_GCM);
}
