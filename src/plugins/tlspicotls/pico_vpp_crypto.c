				     ...
				     ptls_vpp_do_transform_fn do_transform)
{
  ...
  ctx->keys = vnet_crypto_key_add (vm, algo, (u8 *) key, _ctx->algo->key_size);
			      ...
			      const void *aad, size_t aadlen)
{
  ...
  ctx->op.src = (u8 *) input;
  ctx->op.dst = _output;
  ctx->op.keys = (uword) vnet_crypto_get_active_engine_key_data (
    ctx->keys, ctx->id, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
				   ...
				   const void *aad, size_t aadlen)
{
  ...
  ptls_aead__build_iv (ctx->super.algo, ctx->op.iv, ctx->static_iv, seq);
  ctx->op.keys = (uword) vnet_crypto_get_active_engine_key_data (
    ctx->keys, ctx->id, VNET_CRYPTO_HANDLER_TYPE_CHAINED);
  ctx->op.n_chunks = 2;
...
ptls_vpp_crypto_aead_dispose_crypto (ptls_aead_context_t * _ctx)
{
  ...
  clib_rwlock_writer_lock (&picotls_main.crypto_keys_rw_lock);
  vnet_crypto_key_del (vm, ctx->keys);
  clib_rwlock_writer_unlock (&picotls_main.crypto_keys_rw_lock);
}
				   ...
				   vnet_crypto_alg_t alg)
{
  ...
  clib_rwlock_writer_lock (&picotls_main.crypto_keys_rw_lock);
  ctx->keys = vnet_crypto_key_add (vm, alg, (void *) key, key_len);
  clib_rwlock_writer_unlock (&picotls_main.crypto_keys_rw_lock);
