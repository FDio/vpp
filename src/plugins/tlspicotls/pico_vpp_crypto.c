				     ...
				     ptls_vpp_do_transform_fn do_transform)
{
  ...
  ctx->keys.crypto =
    vnet_crypto_key_add_obj (vm, algo, (u8 *) key, _ctx->algo->key_size);
  ctx->keys.integ = NULL;
...
ptls_vpp_crypto_aead_dispose_crypto (ptls_aead_context_t * _ctx)
{
  ...
  clib_rwlock_writer_lock (&picotls_main.crypto_keys_rw_lock);
  vnet_crypto_key_del_obj (vm, ctx->keys.crypto);
  clib_rwlock_writer_unlock (&picotls_main.crypto_keys_rw_lock);
}
				   ...
				   vnet_crypto_alg_t alg)
{
  ...
  clib_rwlock_writer_lock (&picotls_main.crypto_keys_rw_lock);
  ctx->keys.crypto = vnet_crypto_key_add_obj (vm, alg, (void *) key, key_len);
  ctx->keys.integ = NULL;
  clib_rwlock_writer_unlock (&picotls_main.crypto_keys_rw_lock);
