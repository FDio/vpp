...
//    goto do_enq;
  ...
  len = svm_fifo_segments (tcp_rx_fifo, 0, tcp_fs, n_segs, max_len);
  if (len <= 0)
    return 0;

  n_fs_app =
    svm_fifo_provision_chunks (app_rx_fifo, app_fs, n_segs, max_len);
  if (n_fs_app <= 0)
    return 0;

  while (read < len && ai < n_fs_app)
    {
      is_nocopy = app_fs[ai].len < 16 << 10 ? 0 : 1;
      deq_now = clib_min (tcp_fs[i].len, len - read);
      if (is_nocopy)
	{
	  ptls_buffer_init (buf, app_fs[ai].data, app_fs[ai].len);
	  deq_now = clib_min (deq_now, 16 << 10);
	...
	{
	  u32 buf_len = deq_now + (16 << 10);
	  vec_validate (pm->rx_bufs[thread_index], buf_len);
	  ptls_buffer_init (buf, pm->rx_bufs[thread_index], buf_len);
//	  deq_now = clib_min (tcp_fs[i].len, 16 << 10);
//	  deq_now = tcp_fs[i].len;
	  ret = ptls_receive (ptls_ctx->tls, buf, tcp_fs[i].data, &deq_now);
	  ASSERT (ret == 0 || ret == PTLS_ERROR_IN_PROGRESS);
	  ...
	  while (to_copy)
	    {
	      if (app_fs[ai].len <= to_copy)
		{
		  clib_memcpy_fast (app_fs[ai].data,
				    buf->base + (buf->off - to_copy),
				    app_fs[ai].len);
		  to_copy -= app_fs[ai].len;
		  ai += 1;
		}
	      else
		{
		  ...
		  app_fs[ai].len -= to_copy;
		  app_fs[ai].data += to_copy;
		  to_copy = 0;
		}
	    }
	}
...
static int
picotls_app_to_tcp_write (picotls_ctx_t *ptls_ctx, svm_fifo_t *app_tx_fifo,
			  svm_fifo_t *tcp_tx_fifo, u32 max_len)
{
  int read = 0, rv, i = 0, ti = 0, len, n_fs_tcp, deq_len;
  u32 wrote = 0, n_enq, max_enq, to_copy, thread_index;
  const int n_segs = 2;
  svm_fifo_seg_t app_fs[n_segs], tcp_fs[n_segs];
  picotls_main_t *pm = &picotls_main;
  ...
  thread_index = ptls_ctx->ctx.c_thread_index;
  len = svm_fifo_segments (app_tx_fifo, 0, app_fs, n_segs, max_len);
  if (len <= 0)
    return 0;

  n_fs_tcp =
    svm_fifo_provision_chunks (tcp_tx_fifo, tcp_fs, n_segs, 1000 + max_len);
  if (n_fs_tcp <= 0)
    return 0;

  ptls_buffer_init (buf, tcp_fs[0].data, tcp_fs[0].len);

  while (read < len)
    {
      is_nocopy = 0;
      max_enq = tcp_fs[ti].len;
      max_enq += ti < (n_fs_tcp - 1) ? tcp_fs[ti + 1].len : 0;
      deq_len = ptls_compute_deq_len (ptls_ctx, tcp_fs[ti].len, app_fs[i].len,
				      max_enq, &is_nocopy);
      if (is_nocopy)
	{
	  rv = ptls_send (ptls_ctx->tls, buf, app_fs[i].data, deq_len);

	  ASSERT (rv == 0 && buf->off >= buf_off);
	  n_enq = buf->off - buf_off;
	  buf_off = buf->off;
	  wrote += n_enq;

	  ASSERT (tcp_fs[ti].len >= n_enq);
	  tcp_fs[ti].len -= n_enq;
	  tcp_fs[ti].data += n_enq;
	  if (!tcp_fs[ti].len)
	    {
	      ti += 1;
	      buf_off = 0;
      ...
      else
	{
      ...
      read += deq_len;
      ASSERT (deq_len <= app_fs[i].len);
      if (deq_len == app_fs[i].len)
	{
	  i += 1;
	}
      else
	{
	  app_fs[i].len -= deq_len;
	  app_fs[i].data += deq_len;
	}
    }
		   ...
		   transport_send_params_t *sp)
{
  ...
  to_tcp_len =
    picotls_app_to_tcp_write (ptls_ctx, app_tx_fifo, tcp_tx_fifo, deq_now);
