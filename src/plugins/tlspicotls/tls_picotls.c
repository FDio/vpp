  ...
  if (!TLS_RX_IS_LEFT (ptls_ctx))
      ...
      if (PREDICT_FALSE(deq_now < deq_max))
	{
	  ...
	  ptls_ctx->rx_len += off;
        ...
        ret = ptls_receive (ptls_ctx->tls, buf, svm_fifo_head (tls_rx_fifo), (size_t *)&deq_now);
...
enq_buf:
  ...
  off = svm_fifo_enqueue (app_rx_fifo, TLS_READ_LEFT_LEN (ptls_ctx), TLS_READ_OFFSET (ptls_ctx));
  if (off < 0)
  {
...
final:
  ...
  if (TLS_RX_IS_LEFT (ptls_ctx) || TLS_READ_IS_LEFT (ptls_ctx) || svm_fifo_max_dequeue (tls_rx_fifo))
    tls_add_vpp_q_builtin_rx_evt (tls_session);
