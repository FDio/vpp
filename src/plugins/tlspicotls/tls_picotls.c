...
static int
ptls_do_copy (ptls_buffer_t *buf, svm_fifo_seg_t *app_fs, u32 to_copy,
              u32 *app_fs_idx, u32 n_fs_app)
{
  u32 ai = *app_fs_idx;
  ...
  while (to_copy)
      ...
      if (app_fs[ai].len <= to_copy)
	{
	  ...
	  if (ai == n_fs_app)
	    {
	      clib_warning("postponed left %u", to_copy);
	      break;
	    }
	}
      else
  ...
  if (ptls_ctx->read_buffer_offset)
    {
      ...
      left = ptls_do_copy (buf, app_fs, to_copy, &ai, n_fs_app);
      wrote += to_copy - left;
      if (left)
	{
...
//	  deq_now = tcp_fs[i].len;
	  ...
	  if (ret == PTLS_ERROR_IN_PROGRESS)
	    clib_warning ("deq_now %u len %u", deq_now, tcp_fs[i].len);

	  to_copy = buf->off;
//	  if (to_copy > space)
//	    os_panic ();

//	  clib_warning ("to copy %u", to_copy);
	  ...
	  while (to_copy)
	      ...
	      else
		{
	      ...
	      if (ai == n_fs_app)
		{
//		  clib_warning ("postponed left %u offset %u", to_copy,
//		                ptls_ctx->read_buffer_offset);
		  break;
		}
	    }
	  ...
	  if (!to_copy)
	    ...
	    {
	      ptls_ctx->read_buffer_offset = buf->off - to_copy;
	      wrote += ptls_ctx->read_buffer_offset;
	    }
	}

//      clib_warning ("read %u tcp len %u", deq_now, tcp_fs[i].len);
...
//  if (svm_fifo_max_dequeue (tcp_rx_fifo))
