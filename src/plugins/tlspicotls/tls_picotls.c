                          ...
                          svm_fifo_t *tcp_tx_fifo, u32 max_len, u32 *readp)
{
  ...
  n_fs_tcp = svm_fifo_provision_chunks (tcp_tx_fifo, tcp_fs, n_segs, 1000 + max_len);
  if (n_fs_tcp <= 0)
    return 0;
  ...
  while (read < len && i < n_segs)
	...
	{
	  vec_validate (ptls_ctx->write_content, max_enq);
	  ptls_buffer_init (buf, ptls_ctx->write_content, max_enq);
...
{
  picotls_ctx_t *ptls_ctx = (picotls_ctx_t *) ctx;
  u32 deq_max, from_app_len = 0, deq_now;
  u32 enq_max;
  int to_tls_len = 0;
  ...
  to_tls_len = picotls_app_to_tcp_write (ptls_ctx, app_tx_fifo, tls_tx_fifo,
	                                 deq_now, &from_app_len);
  if (!to_tls_len)
    goto check_tls_fifo;
