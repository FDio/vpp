...
static int
picotls_try_handshake_write (picotls_ctx_t * ptls_ctx, session_t * tls_session, ptls_buffer_t * buf)
{
    u32 enq_max, enq_now;
    svm_fifo_t * f;
    int write, buf_left;
...
picotls_start_listen (tls_ctx_t * lctx)
{
    ...
    static ptls_key_exchange_algorithm_t * key_exchange[] = {&ptls_openssl_x25519, 
...
static inline int
picotls_do_handshake (picotls_ctx_t * ptls_ctx, session_t * tls_session, u8 * input, int input_len)
{
    ...
    do {
        ...
        do {
            ...
            {
                write = picotls_try_handshake_write (ptls_ctx, tls_session, &buf);
            }
...
picotls_ctx_read (tls_ctx_t * ctx, session_t * tls_session)
{
    ...
    if (!picotls_handshake_is_over (ctx))
    {
        ...
        {
            deq_now = clib_min (svm_fifo_max_read_chunk (tls_rx_fifo), deq_max - from_tls_len);
            from_tls_len += svm_fifo_dequeue (tls_rx_fifo, deq_now, input + from_tls_len);
        }
    ...
    from_tls_len = svm_fifo_dequeue (tls_rx_fifo, deq_now, ptls_ctx->rx_content + ptls_ctx->rx_len);
    if (from_tls_len <= 0)
    {
        tls_add_vpp_q_builtin_rx_evt (tls_session);
        goto app_fifo;
    ...
    {
        deq_now = clib_min (svm_fifo_max_read_chunk (tls_rx_fifo), deq_max - from_tls_len);
        from_tls_len += svm_fifo_dequeue (tls_rx_fifo, deq_now, ptls_ctx->rx_content + ptls_ctx->rx_len + from_tls_len);
    }
...
app_fifo:
    ...
    do {
        ...
        ret = ptls_receive (ptls_ctx->tls, buf, ptls_ctx->rx_content + ptls_ctx->rx_offset + off, &consumed);
        off += consumed;
    ...
    if (ret == 0)
    {
        ...
        {
            enq_now = clib_min (svm_fifo_max_write_chunk (app_rx_fifo), buf->off - to_app_len);
            to_app_len += svm_fifo_enqueue (app_rx_fifo, enq_now, buf->base + to_app_len);
        }
    }
...
static inline int
picotls_content_process (ptls_t * tls, svm_fifo_t * src_fifo, svm_fifo_t * dst_fifo, int content_len, int total_record_overhead, int is_no_copy)
{
    ptls_buffer_t _buf, * buf = &_buf;
    ...
    if (is_no_copy)
    {
        ...
        if (to_dst_len < buf->off)
            to_dst_len += svm_fifo_enqueue (dst_fifo, buf->off - to_dst_len, buf->base + to_dst_len);
        ptls_buffer_dispose (buf);
    }
    return to_dst_len;
}
...
picotls_ctx_write (tls_ctx_t * ctx, session_t * app_session)
{
    ...
    if (enq_now > record_overhead)
    {
        ...
        num_records = ceil ((f64)from_app_len / PTLS_MAX_PLAINTEXT_RECORD_SIZE);
    ...
    to_tls_len = picotls_content_process (ptls_ctx->tls, app_tx_fifo, tls_tx_fifo, from_app_len, total_overhead, is_nocopy);
