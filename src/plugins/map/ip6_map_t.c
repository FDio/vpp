			...
			bool udp_checksum)
{
  ...
  if (vnet_buffer (p)->map_t.map_domain_index != ~0)
    {
      map_domain_t *sd =
	pool_elt_at_index (mm->domains,
			   vnet_buffer (p)->map_t.map_domain_index);
      anti_spoof_fib_index = sd->ip4_fib_index;
    }

  ip4_map_get_domain (anti_spoof_fib_index, &ip4->src_address,
		      &map_domain_index, &error);
  if (error)
    return error;
	  ...
	  if (!d0)
	    {
	      error0 = MAP_ERROR_NONE;
	      d0 =
		ip6_map_get_domain_dst (&ip60->dst_address,
					&vnet_buffer (p0)->map_t.
					map_domain_index, &error0);
