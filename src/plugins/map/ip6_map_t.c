...
ip6_map_t (vlib_main_t * vm, vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /*
	   * Resolve the MAP domain. Legacy single-tenant domains key on
	   * the v6 source against ip6_prefix_tbl (which holds DMR
	   * prefixes); per-VRF domains key on the v6 destination against
	   * ip6_src_prefix_tbl (which holds each domain's per-tenant
	   * ip6_src prefix). We don't know which kind of domain the
	   * packet belongs to yet, so try the legacy (src-in-DMR) lookup
	   * first and fall back to the per-VRF (dst-in-ip6_src) table if
	   * it misses.
	   *
