  ...
  if (mm->frag_inner)
      ...
      if (df && !mm->frag_ignore_df)
	{
	  ...
	  *error = MAP_ERROR_DF_SET;
      ...
      ip6_frag_do_fragment (vm, bi, mtu, 0, buffers, &fragerror);
    }
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_TRUE (error0 == MAP_ERROR_NONE))
		...
		{
		  next0 =
		    ip4_map_fragment (p0, d0->mtu, df0, buffers0, &error0);
		}
	      else
		{
		  next0 =
	  ...
	  if (PREDICT_TRUE (error0 == MAP_ERROR_NONE))
	    {
		  ...
		  (d0->mtu
		   && (clib_net_to_host_u16 (ip6h0->payload_length) +
		       sizeof (*ip6h0) > d0->mtu)))
		{
		  clib_warning ("OLE OLE OLE fragment: %u %u", next0,
				d0->mtu);
		  next0 =
