  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  u32 next0 = IP4_MAP_NEXT_IP6_LOOKUP, next1 =
	    IP4_MAP_NEXT_IP6_LOOKUP;
	  u32 *buffer0 = 0, *buffer1 = 0;
	  bool free_original_buffer0 = false, free_original_buffer1 = false;
	  u32 *frag_from0, frag_left0, *frag_from1, frag_left1;
	  ...
	  /* If not for us fall through to single loop */
	  if (!d0 || !d1)
	    goto single_loop;
	  ...
	  if (PREDICT_TRUE (error1 == MAP_ERROR_NONE))
		...
		{
		  next1 =
		    ip4_map_fragment (vm, pi1, d1->mtu, df1, &buffer1,
				      &error1);

		  if (error1 == MAP_ERROR_NONE)
		    {
		      free_original_buffer1 = true;
		    }
		}
	      else
		{
	  ...
	  if (free_original_buffer1)
	    {
	      vlib_buffer_free_one (vm, pi1);	/* Free original packet */
	    }
	  else
	    {
	      vec_add1 (buffer1, pi1);
	    }

	  frag_from0 = buffer0;
	  frag_left0 = vec_len (buffer0);
	  frag_from1 = buffer1;
	  frag_left1 = vec_len (buffer1);

	  if (frag_left0 == 1 && frag_left1 == 1)
	    {
	      to_next += 2;
	      n_left_to_next -= 2;
	      ...
	      while (frag_left0 > 0)
		{
	      ...
	      while (frag_left1 > 0)
		{
		  while (frag_left1 > 0 && n_left_to_next > 0)
		    {
		      u32 i1;
		      i1 = to_next[0] = frag_from1[0];
		      frag_from1 += 1;
		      frag_left1 -= 1;
		      to_next += 1;
		      n_left_to_next -= 1;

		      vlib_get_buffer (vm, i1)->error =
