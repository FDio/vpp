		   ...
		   u8 v6_lookup_side, u32 *map_domain_index, u16 mtu, u8 flags, u8 *tag)
{
   ...
   * to the default FIB and matches the single-tenant semantics. */
  ip4_fib_index =
  ...
  *map_domain_index = d - mm->domains;
  ...
  /* ip6_prefix_tbl holds each domain's DMR. Single-tenant MAP-T matches
   * against this table on the reverse path's v6 source.
   *
   * Per-VRF MAP-T (v6_lookup_side=DST) deliberately shares a single DMR
   * across multiple tenants — e.g. all tenants pointing at the same
   * shared headend address get the same /128 ip6_prefix, with tenant
   * identity encoded in ip6_src. If we inserted their DMR here the
   * second tenant would overwrite the first in the global LPM;
   * identification must flow through ip6_src_prefix_tbl instead. Skip
   * the insert for those domains; map_delete_domain mirrors the skip. */
			   ...
			   unformat_input_t * input, vlib_cli_command_t * cmd)
{
   ...
   * post-create (see the contract on MAP_V6_LOOKUP_SIDE_DEFAULT in
   * map.h). */
