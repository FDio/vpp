		   ...
		   u32 * map_domain_index, u16 mtu, u8 flags, u8 * tag)
{
  ...
  ip4_fib_index = fib_table_find_or_create_and_lock (FIB_PROTOCOL_IP4,
						     ip4_table_id,
						     FIB_SOURCE_MAP);
  ip6_fib_index = fib_table_find_or_create_and_lock (FIB_PROTOCOL_IP6,
						     ip6_table_id,
						     FIB_SOURCE_MAP);
  ...
  *map_domain_index = d - mm->domains;
   ...
   * with the flag (codex review, cndp-vpp#23 P1). */
  if (v6_lookup_side == MAP_V6_LOOKUP_SIDE_DEFAULT)
    d->v6_lookup_side =
      (ip4_table_id != 0) ? MAP_V6_LOOKUP_DST : MAP_V6_LOOKUP_SRC;
  else
  ...
  mm->ip4_prefix_tbl->add_vrf (mm->ip4_prefix_tbl, d->ip4_fib_index,
			       &d->ip4_prefix, ip4_prefix_len,
			       *map_domain_index);
  ...
  /* ip6_prefix_tbl holds each domain's DMR. Upstream / cndp-vpp match
   * against this table on the reverse path's v6 source [dgeist].
   *
...
map_delete_domain (u32 map_domain_index)
{
  ...
  if (map_domain_index < vec_len (mm->domain_extras))
    ip4_prefix_len =
      vec_elt_at_index (mm->domain_extras, map_domain_index)->ip4_prefix_len;

  mm->ip4_prefix_tbl->delete_vrf (mm->ip4_prefix_tbl, d->ip4_fib_index,
				  &d->ip4_prefix, ip4_prefix_len);
			   ...
			   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  map_create_domain (&ip4_prefix, ip4_prefix_len,
		     ...
		     ea_bits_len, psid_offset, psid_length,
		     ip4_table_id, ip6_table_id,
		     v6_lookup_side,
		     &map_domain_index, mtu, flags, tag);
...
format_map_domain (u8 * s, va_list * args)
{
  ...
  s = format (s,
	      ...
	      format_ip4_address, &d->ip4_prefix,
	      de ? de->ip4_prefix_len : 0,
