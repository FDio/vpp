...
vl_api_map_add_domain_v2_t_handler (vl_api_map_add_domain_v2_t * mp)
{
  ...
  REPLY_MACRO2_END(VL_API_MAP_ADD_DOMAIN_V2_REPLY,
  ({
    rmp->index = index;
  }));
}
...
static void
send_domain_v2_details (u32 map_domain_index, vl_api_registration_t * rp,
			u32 context)
{
  map_main_t *mm = &map_main;
  ...
  if (mm->domain_extras)
    de = vec_elt_at_index (mm->domain_extras, map_domain_index);
  REPLY_MACRO_DETAILS4(VL_API_MAP_DOMAIN_V2_DETAILS, rp, context,
  ({
    rmp->domain_index = htonl (map_domain_index);
    clib_memcpy (&rmp->ip6_prefix.address, &d->ip6_prefix,
		 sizeof (rmp->ip6_prefix.address));
    ...
    rmp->mtu = htons (d->mtu);
    rmp->ip4_table_id =
      htonl (fib_table_get_table_id (d->ip4_fib_index, FIB_PROTOCOL_IP4));
    rmp->ip6_table_id =
      htonl (fib_table_get_table_id (d->ip6_fib_index, FIB_PROTOCOL_IP6));
