...
int map_add_del_psid (u32 map_domain_index, u16 psid, ip6_address_t * tep,
		      u8 is_add);
int map_if_enable_disable (bool is_enable, u32 sw_if_index, u8 is_translation);
...
// OLE: This doesn't work. Must use IP6 source address as key????
// But how the hell does that work? Hard-coded to be 64 for all domains.
...
/*
 * OLE: Also do a lookup in the IPv4 MAP table?
 *      Require source /64 to be unique per MAP domain?
                    ...
                    u8 *error)
{
  ...
  clib_warning("OLE: Found in table: %d", mdi);
  if (mdi == ~0) {
    *error = MAP_ERROR_NO_DOMAIN;
    mdi = MAP_DOMAIN_NO_DOMAIN;
  }
