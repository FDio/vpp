...
/*
 * v6_lookup_side: pass MAP_V6_LOOKUP_SRC for single-tenant semantics,
 * MAP_V6_LOOKUP_DST for per-VRF MAP-T. The create-time LPM insert
 * decision (ip6_prefix_tbl) depends on this value — do not mutate
...
int map_create_domain (ip4_address_t *ip4_prefix, u8 ip4_prefix_len, ip6_address_t *ip6_prefix,
 ...
 *
 * Single-tenant MAP-T keys the IPv6 (reverse, decap) lookup on the IPv6
 * source address. Per-VRF MAP-T keys the reverse-path domain lookup on
 * the IPv6 destination instead, because the per-tenant v6 prefix stamped
 * into the forward-path v6 source lands in the return packet's v6
 * destination. Both modes are selectable per domain so single-tenant and
 * per-VRF callers coexist in the same plugin.
 */
typedef enum
{
  MAP_V6_LOOKUP_SRC = 0, /* single-tenant default */
...
/*
 * Per-VRF MAP-T adds two FIB indices plus a lookup-side u8, so
 * map_domain_t now spans two 64-byte cache lines. Hot-path fields live
 * at the head of the struct so the first access is a single cache line
 * fetch; the per-VRF fields only matter once we know the domain matched.
 */
typedef struct
{
...
ip4_map_get_domain (u32 fib_index, ip4_address_t *addr, u32 *map_domain_index, u8 *error)
{
  map_main_t *mm = &map_main;

  u32 mdi = mm->ip4_prefix_tbl->lookup_vrf (mm->ip4_prefix_tbl, fib_index, addr, 32);
  if (mdi == ~0)
    {
 ...
 *                              Used by single-tenant MAP-T, where
 ...
 *                              per-tenant prefix + embedded v4 source.
 *
...
static_always_inline map_domain_t *
ip6_map_get_domain_dst (ip6_address_t *addr, u32 *map_domain_index, u8 *error)
{
  map_main_t *mm = &map_main;
...
/*
 * Pre-resolved adjacency bypass. Single-tenant MAP-T stuffs the globally
 * pre-resolved next-hop into adj_index[VLIB_TX] to skip the ip6-lookup
 * node on the forward path. That bypass is keyed on the default FIB, so
 * it is only safe when the caller-supplied domain rides the default
 * underlay VRF. Per-VRF domains must fall through to ip6-lookup in their
 * own ip6_fib_index; vnet_buffer(b)->sw_if_index[VLIB_TX] is set to the
 * tenant fib_index before ip6-lookup runs (see the MAP-T fast path).
 */
static_always_inline bool
ip4_map_ip6_lookup_bypass (vlib_buffer_t *p0, ip4_header_t *ip, map_domain_t *d)
{
#ifdef MAP_SKIP_IP6_LOOKUP
...
static_always_inline bool
ip6_map_ip4_lookup_bypass (vlib_buffer_t *p0, ip4_header_t *ip, map_domain_t *d)
{
#ifdef MAP_SKIP_IP6_LOOKUP
