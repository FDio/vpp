  ...
  vec_foreach (p, merged_paths)
    {
      ...
      size_t msg_size = sizeof (vl_api_tracepath_details_t) +
			 n_nodes * sizeof (vl_api_trace_path_node_t);
      ...
      rmp->_vl_msg_id =
	ntohs (VL_API_TRACEPATH_DETAILS + tracepath_base_msg_id);
      rmp->context = mp->context;
      rmp->path_id = clib_host_to_net_u64 (p->path_id);
      rmp->n_pkts = htonl (p->n_pkts);
      rmp->thread_bitmap = clib_host_to_net_u64 (
	(p->thread_bitmap ? p->thread_bitmap[0] : 0));
      ...
      for (u32 i = 0; i < n_nodes; i++)
	{
	  ...
	  u32 name_len =
	    clib_min (vec_len (node->name), sizeof (rmp->nodes[i].name) - 1);
