...
static inline u64
trace_path_id (u32 path_len, u32 *path_indices)
{
  /* For each path-id: upper 32 bits = path length, lower 32 bits = crc32c of node indices */
  u32 crc = clib_crc32c ((u8 *) path_indices, sizeof (u32) * path_len);
  return ((u64) path_len << 32) | crc;
}
...
trace_path_from_header (vlib_trace_header_t *h)
{
  ...
  if (path.path_length > 0)
    {
      path.n_pkts = 1;
      path.path_id = trace_path_id (path.path_length, path.path_indices);
    }
...
trace_paths_collect_all (void)
{
  ...
  /* TODO: thread_bitmap is u64 and therefore supports at most 64 threads.
   * we should consider switching to clib_bitmap_t to support VPP configurations with more threads
   */
  ...
  if (vec_len (merged_paths) == 0)
      ...
      vec_foreach (p, merged_paths)
	{
	  ...
	  s = format (s, "  [%d] Count: %d  ID: 0x%016lx  Length: %2d  Threads: %U\n", i, p->n_pkts,
		      p->path_id, p->path_length, format_thread_list, p->thread_bitmap);
...
show_trace_path_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  for (u32 idx = 0; idx < n_paths && traces_shown < max_traces; idx++)
      ...
      foreach_vlib_main ()
	{
	  ...
	  if (traces_shown >= max_traces && i < vec_len (traces))
	    s = format (s, "Limiting display to %d packets. To display more specify max.\n",
			max_traces);
  ...
  if (traces_shown == 0)
    s = format (s, "No traces found for requested path indices\n");
