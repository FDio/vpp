...
api_nsh_add_del_entry (vat_main_t *vam)
{
  ...
  if (nsp_set == 0)
    return -1; // TODO Error type for this cond: clib_error_return (0, "nsp not
	       // specified");

  if (nsi_set == 0)
    return -2; // TODO Error type for this cond:clib_error_return (0, "nsi not
	       // specified");
...
vl_api_nsh_entry_details_t_handler (vl_api_nsh_entry_details_t *mp)
{
  ...
  fformat (vam->ofp, "%11d%11d%11d%11d%14d%14d%14d%14d%14d\n", mp->ver_o_c,
	   mp->length, mp->md_type, mp->next_protocol, ntohl (mp->nsp_nsi),
	   ntohl (mp->c1), ntohl (mp->c2), ntohl (mp->c3), ntohl (mp->c4));
}
  ...
  if (!vam->json_output)
    {
      fformat (vam->ofp, "%11s%11s%15s%14s%14s%13s%13s%13s%13s\n", "ver_o_c",
	       "length", "md_type", "next_protocol", "nsp_nsi", "c1", "c2",
	       "c3", "c4");
    }
...
api_nsh_add_del_map (vat_main_t *vam)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	next_node = NSH_NODE_NEXT_DROP; // Once moved to NSHSFC see
					// nsh.h:foreach_nsh_input_next to
					// handle this case
      else
  ...
  if (nsp_set == 0 || nsi_set == 0)
    return -1; // TODO create return value: clib_error_return (0, "nsp nsi pair
	       // required. Key: for NSH entry");

  if (mapped_nsp_set == 0 || mapped_nsi_set == 0)
    return -2; // TODO create return valuee clib_error_return (0, "mapped-nsp
	       // mapped-nsi pair required. Key: for NSH entry");

  if (next_node == ~0)
    return -3; // TODO clib_error_return (0, "must specific action:
	       // [encap-gre-intf <nn> | encap-vxlan-gpe-intf <nn> |
	       // encap-none]");
...
vl_api_nsh_map_details_t_handler (vl_api_nsh_map_details_t *mp)
{
  ...
  fformat (vam->ofp, "%14d%14d%14d%14d\n", ntohl (mp->nsp_nsi),
	   ntohl (mp->mapped_nsp_nsi), ntohl (mp->sw_if_index),
	   ntohl (mp->next_node));
}
...
api_nsh_map_dump (vat_main_t *vam)
{
  ...
  if (!vam->json_output)
    {
      fformat (vam->ofp, "%16s%16s%13s%13s\n", "nsp_nsi", "mapped_nsp_nsi",
	       "sw_if_index", "next_node");
    }
...
#define foreach_vpe_api_msg                                                   \
  _ (nsh_add_del_entry, "{nsp <nn> nsi <nn>} c1 <nn> c2 <nn> c3 <nn> c4 "     \
			"<nn> [md-type <nn>] [tlv <xx>] [del]")               \
  _ (nsh_entry_dump, "")                                                      \
  _ (nsh_add_del_map,                                                         \
     "nsp <nn> nsi <nn> [del] mapped-nsp <nn> mapped-nsi <nn> "               \
