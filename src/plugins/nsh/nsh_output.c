...
format_nsh_output_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "\n%U%U", format_white_space, indent,
	      format_ip_adjacency_packet_data, t->adj_index, t->packet_data,
	      sizeof (t->packet_data));
  return s;
}

static inline uword
nsh_output_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		   vlib_frame_t *from_frame, int is_midchain)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  eth_hdr0 =
	    (ethernet_header_t *) ((u8 *) hdr0 - sizeof (ethernet_header_t));
	  eth_hdr0->type = clib_host_to_net_u16 (ETHERNET_TYPE_NSH);
	  eth_hdr1 =
	    (ethernet_header_t *) ((u8 *) hdr1 - sizeof (ethernet_header_t));
	  ...
	  if (adj_index0 == adj_index1)
		...
		&adjacency_counters, thread_index, adj_index0, 2,
