...
typedef struct
{
  ...
  /*
   * For API or CLI configuration and construct the rewrite buffer, invokes
   * add_options() function. In the encap node, i.e. when performing PUSH nsh
   * header, invokes options() function. In the swap node, i.e. when performing
   * SWAP nsh header, invokes swap_options() function. In the decap node, i.e.
   * when performing POP nsh header, invokes pop_options() function.
   */
