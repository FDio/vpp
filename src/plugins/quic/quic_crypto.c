...
static size_t
quic_crypto_aead_decrypt_push (quic_ctx_t * qctx, ptls_aead_context_t * _ctx,
...
quic_crypto_batch_rx_packets (quic_crypto_batch_ctx_t * batch_ctx)
{
  ...
  vnet_crypto_process_ops (vm,
			   batch_ctx->aead_crypto_rx_packets_ops,
		   ...
		   ptls_aead_context_t ** aead_ctx)
{
  ...
  quic_crypto_conn_ciphers_t *conn_cipher = NULL, *cipher = NULL;

  /* *INDENT-OFF* */
  pool_foreach (cipher, qm->crypto_conn_ciphers,
  ({
    if (cipher->conn == conn)
      {
        conn_cipher = cipher;
  ...
  if (!conn_cipher)
    {
      pool_get (qm->crypto_conn_ciphers, conn_cipher);
      clib_memset (conn_cipher, 0, sizeof (*conn_cipher));

      conn_cipher->conn = conn;
      conn_cipher->key_phase = 0;
    }
  else
    {
      conn_cipher->key_phase++;
    }

  conn_cipher->traffic_keys[conn_cipher->key_phase].hp_ctx = hp_ctx;
  conn_cipher->traffic_keys[conn_cipher->key_phase].aead_ctx = aead_ctx;
}
  ...
  if (QUICLY_DEBUG)
	...
	*hpkey_hex =
	quicly_hexdump (hpkey, aead->ctr_cipher->key_size, SIZE_MAX);
      fprintf (stderr,
	       "%s:\n  aead-secret: %s\n  hp-key: %s  is_enc %d aead %p header_p %p \n",
	       __FUNCTION__, secret_hex, hpkey_hex, is_enc, aead_ctx, hp_ctx);
      free (secret_hex);
      free (hpkey_hex);
    }

  if (epoch == QUICLY_EPOCH_1RTT && !is_enc)
    save_conn_ciphers (conn, (hp_ctx != NULL) ? *hp_ctx : NULL, aead_ctx);
...
quic_crypto_decrypt_packet (quic_ctx_t * qctx, quic_rx_packet_ctx_t * pctx)
{
  ...
  quic_crypto_conn_ciphers_t *ciphers = get_conn_ciphers (qctx->conn);
  ...
  int key_phase_bit =
    (pctx->packet.octets.base[0] & QUICLY_KEY_PHASE_BIT) >
    (QUICLY_KEY_PHASE_BIT - 1);

  if (key_phase_bit != (ciphers->key_phase & 1))
    {
      fprintf (stderr, "%s key_phase_bit != (ciphers->key_phase & 1) \n\r",
	       __FUNCTION__);
      goto Error;
    }

  if ((ptlen =
       quic_crypto_aead_decrypt_push (qctx, aead[aead_index],
				      pctx->packet.octets.base + aead_off,
				      pctx->packet.octets.base + aead_off,
				      pctx->packet.octets.len - aead_off, pn,
  ...
  pctx->packet.decrypted.key_phase = ciphers->key_phase;
			       ...
			       const void *key, const EVP_CIPHER * cipher)
{
  ...
  clib_rwlock_writer_lock (&quic_main.quic_rw_lock);
  ctx->key_index = vnet_crypto_key_add (vm, algo,
					(u8 *) key, _ctx->algo->key_size);
  clib_rwlock_writer_unlock (&quic_main.quic_rw_lock);
