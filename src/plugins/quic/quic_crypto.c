...
vnet_crypto_op_t aead_crypto_ops[MAX_PACKET];
vnet_crypto_op_t aead_decrypt_ops[MAX_PACKET];

size_t crypto_op_count = 0;
size_t decrypt_op_count = 0;

quic_crypto_op_t crypto_ops[MAX_PACKET];
quic_crypto_op_t decrypt_ops[MAX_PACKET];
  ...
  for (int i = 0; i < crypto_op_count; i++)
    {
      ...
      cur_op->crypto_codec->encrypt_packet_done (crypto_ops[i].
						 encrypt_cb_ctx.header_protection,
						 crypto_ops[i].
						 encrypt_cb_ctx.first_byte_at,
						 crypto_ops[i].
						 encrypt_cb_ctx.dst_payload_from);
    }
  ...
  for (int i = 0; i < decrypt_op_count; i++)
    {
      ...
      cur_op->crypto_codec->
	decrypt_packet_done (decrypt_ops[i].decrypt_cb_ctx.conn,
			     decrypt_ops[i].decrypt_cb_ctx.packet,
			     decrypt_ops[i].decrypt_cb_ctx.space,
			     decrypt_ops[i].decrypt_cb_ctx.epoch,
			     decrypt_ops[i].decrypt_cb_ctx.pn,
			     decrypt_ops[i].decrypt_cb_ctx.aead_off,
			     decrypt_ops[i].decrypt_cb_ctx.ptlen);
    }
