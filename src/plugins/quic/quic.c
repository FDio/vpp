...
//       fprintf (stderr, "%s num_packets %ld nb_tx_packets %ld \n\r",
//             __FUNCTION__, num_packets,
//             qm->wrk_ctx[thread_index].crypto_context_batch.nb_tx_packets);
	  ...
	  if (quic_main.vnet_crypto_enabled)
	    {
	      quic_crypto_finalize_send_packet (&packets[i],
						&qm->wrk_ctx
						[thread_index].crypto_context_batch.crypto_tx_packet_ctx
						[i]);
	    }
...
//   quicly_datagram_t *dgram;
  ...
  size_t payload_len =
    quicly_send_stateless_reset (quicly_ctx, qctx->conn, &dest, &src,
				 &pctx->packet.cid.dest.plaintext, payload);
  if (payload_len == 0)
    return 1;
