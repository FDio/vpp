...
static void
quic_session_migrate_callback (session_t * s, session_handle_t new_handle)
{
  /*
  ...
  u32 new_thread = session_thread_from_handle (new_handle);
  clib_bihash_kv_16_8_t kv;
  clib_bihash_16_8_t *h;
  quic_ctx_t *ctx;

  QUIC_DBG (1, "Session %x migrated to %lx", s->session_index, new_handle);
  /* *INDENT-OFF* */
      ...
      if (ctx->udp_session_handle == old_session_handle)
        {
          ...
          ctx->udp_session_handle = new_handle;
            /* Mark connection as moving in the conn map */
            h = &quic_main.connection_hash;
