...
{
  vec_validate (quic_lib_vfts, type);
  quic_lib_vfts[type] = *vft;
}
...
{
  int i;
  for (i = 0; i < vec_len (quic_lib_vfts); i++)
    {
      if (quic_lib_vfts[i].get_conn_ctx)
	return i;
    }
...
quic_get_lib_type (quic_lib_type_t requested, quic_lib_type_t preferred)
{
  if (requested != QUIC_LIB_NONE)
    {
      if (quic_lib_vfts[requested].get_conn_ctx)
	return requested;
      return QUIC_LIB_NONE;
    }
  if (!quic_lib_vfts[preferred].get_conn_ctx)
...
quic_transfer_connection (u32 ctx_index, u32 dest_thread)
{
  ...
  if (PREDICT_FALSE (!quic_lib_vfts[qm->lib_type].receive_connection))
    {
      QUIC_DBG (1, "receive_connection() not available for %s library\n",
		quic_lib_type_str (qm->lib_type));
      return;
    }
  session_send_rpc_evt_to_thread (dest_thread, quic_lib_vfts[qm->lib_type].receive_connection ,
				  (void *) temp_ctx);
}
...
quic_show_aggregated_stats (vlib_main_t * vm)
{
  ...
  for (i = 0; i < num_workers + 1; i++)
	...
	if (quic_ctx_is_conn (ctx) && ctx->conn)
	  {
	    quic_lib_connection_get_stats (ctx->conn, &st);
