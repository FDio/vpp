...
quic_enable (vlib_main_t *vm, u8 is_en)
{
  ...
  qm->engine_type =
    quic_get_engine_type (qm->engine_type, QUIC_ENGINE_QUICLY);
  if (qm->engine_type == QUIC_ENGINE_NONE)
    {
...
quic_config_fn (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  /*
./build-root/build-vpp_debug-native/vpp/bin/vpp unix interactive plugins { plugin quic_plugin.so { enable } plugin quic_quicly_plugin.so { enable } plugin quic_openssl_plugin.so { enable } plugin dpdk_plugin.so { disable } } quic { quic-engine openssl }
   */
    ...
    if (engine_str && !clib_strcmp ((char *) engine_str, "openssl"))
      ...
      {
        clib_warning ("unknown quic-engine '%s' - defaulting to quicly engine", engine_str);
        qm->engine_type = QUIC_ENGINE_QUICLY;
      }
    vec_free (engine_str);
  }
