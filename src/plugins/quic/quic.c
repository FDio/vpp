...
quic_notify_app_connected (quic_ctx_t * ctx)
{
  ...
  quic_session->al_handle = quic_session_alloc_app_listener (app_wrk->app_index, quic_session->thread_index);
				      ...
				      transport_endpoint_t * tep, u8 is_lcl)
{
  ...
  if (ctx->is_listener)
    {
      app_listener =
	app_listener_get (ctx->c_quic_ctx_id.udp_session_handle);
      udp_listen_session = app_listener_get_session (app_listener);
