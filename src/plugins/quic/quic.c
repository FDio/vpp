...
{
  quic_worker_ctx_t *wrk_ctx = quic_main.wrk_ctx;
...
quic_connect_stream (session_t * quic_session, session_endpoint_cfg_t * sep)
{
  ...
  quic_increment_counter (&quic_main, QUIC_ERROR_OPENED_STREAM, 1);
  ...
  if (app_worker_connect_notify (app_wrk, stream_session, SESSION_E_NONE,
				 sep->opaque))
    {
      ...
      quic_increment_counter (&quic_main, QUIC_ERROR_CLOSED_STREAM, 1);
      quic_eng_reset_stream_connect_error (stream);
      return -1;
    }
...
quic_engine_init ()
{
  ...
  vec_validate (quic_main.ctx_pool, qm->num_threads - 1); /* quicly ctx_pool */

  QUIC_DBG (1, "Initializing quic engine to %s",
	    quic_engine_type_str (qm->engine_type));

  wrk_ctx = quic_main.wrk_ctx;
  for (i = 0; i < qm->num_threads; i++)
    {
      pool_get_aligned_safe (wrk_ctx[i].crypto_ctx_pool, crctx,
			     CLIB_CACHE_LINE_BYTES);
      pool_program_safe_realloc ((void **) &wrk_ctx[i].crypto_ctx_pool,
