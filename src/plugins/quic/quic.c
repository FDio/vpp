...
quic_get_quicly_ctx_from_ctx (quic_ctx_t * ctx)
{
  quic_main_t *qm = &quic_main;
  uword *p;
  p = hash_get (qm->quicly_ctx_by_cr_index, ctx->crypto_ctx_index);
  if (p == NULL)
    return 0;
  return (quicly_context_t *) p[0];
}
...
quic_store_quicly_ctx (u32 crypto_ctx_index, u8 is_client)
{
  ...
  p = hash_get (qm->quicly_ctx_by_cr_index, crypto_ctx_index);
  /* We already habe a quicly context for that crypto context */
  if (p != NULL)
    return 0;

  crypto_ctx_subscribe_transport (crctx, TRANSPORT_PROTO_QUIC);	/* ask for ping on delete */
  ...
  hash_set (qm->quicly_ctx_by_cr_index, crypto_ctx_index, quicly_ctx);
...
quic_crypto_ctx_cleanup (u32 cr_index)
{
  ...
  quic_main_t *qm = &quic_main;
  quicly_ctx_data_t *quicly_ctx_data;
  p = hash_get (qm->quicly_ctx_by_cr_index, cr_index);
  if (p == NULL)
    {
      QUIC_DBG (1, "No quicly context found for crypto context 0x%x",
		cr_index);
      return;
    }
  quicly_ctx_data = (quicly_ctx_data_t *) p[0];
  clib_mem_free (quicly_ctx_data);
  hash_unset (qm->quicly_ctx_by_cr_index, cr_index);
}
...
quic_init (vlib_main_t * vm)
{
  ...
  qm->quicly_ctx_by_cr_index = hash_create (0, sizeof (uword));
