...
quic_sendable_packet_count (session_t * udp_session)
{
  ...
  return clib_min (max_enqueue / packet_size,
...
stop_sending:
  if (svm_fifo_set_event (udp_session->tx_fifo))
    if ((err = session_send_io_evt_to_thread (udp_session->tx_fifo, SESSION_IO_EVT_TX)))
      {
      	clib_warning ("Event enqueue errored %d", err);
      	ASSERT (0);
      }

  QUIC_DBG (3, "UDP TX bytes %u", svm_fifo_max_dequeue (udp_session->tx_fifo));
  QUIC_DBG (3, "UDP RX bytes %u", svm_fifo_max_dequeue (udp_session->rx_fifo));
  QUIC_DBG (3, "TX Has evt ? %u", svm_fifo_has_event(udp_session->tx_fifo));
...
static void
quic_check_app_rx_data (session_t *stream_session)
{
  u32 max_deq;
  quic_ctx_t *sctx;
  svm_fifo_t *f;
		 ...
		 size_t len)
{
  ...
  if (off == stream_data->app_rx_data_len)
	...
	app_worker_lock_and_send_event (app_wrk, stream_session,
					SESSION_IO_EVT_RX);
      quic_check_app_rx_data (stream_session);
    }
  else
    {
...
quic_app_rx_callback (session_t * udp_session)
{
  ...
  struct sockaddr *sa = (struct sockaddr *) &sa6;
  socklen_t salen;
  u32 max_deq, full_len, ctx_index = UINT32_MAX, ctx_thread =
    UINT32_MAX, ret;
  u8 *data;
  int err;
