...
void
quic_register_engine (const quic_engine_vft_t *vft, quic_engine_type_t engine_type)
{
...
quic_get_first_avail_engine_type (void)
{
  int i;
  for (i = 0; i < vec_len (quic_engine_vfts); i++)
    {
      if (quic_engine_vfts[i].get_conn_ctx)
	return i;
    }
  return QUIC_ENGINE_NONE;
}

static quic_engine_type_t
quic_get_engine_type (quic_engine_type_t requested, quic_engine_type_t preferred)
{
  if (requested != QUIC_ENGINE_NONE)
    {
      if (quic_engine_vfts[requested].get_conn_ctx)
	return requested;
      return QUIC_ENGINE_NONE;
    }
  if (!quic_engine_vfts[preferred].get_conn_ctx)
    return quic_get_first_avail_engine_type ();
  return preferred;
}
  ...
  for (i = 0; i < qm->num_threads; i++)
    {
	  ...
	  if (crctx->ckpair_index == ckpair->cert_key_index)
	    {
	      quic_eng_crypto_context_make_key_from_crctx (&kv, crctx);
...
quic_connect_stream (session_t * quic_session, session_endpoint_cfg_t * sep)
{
  ...
  if (quic_eng_connect_stream (conn, &stream, &stream_data, is_unidir))
    {
  ...
  if ((rv = app_worker_init_connected (app_wrk, stream_session)))
    {
      ...
      quic_eng_reset_stream_connect_error (stream);
...
quic_connect_connection (session_endpoint_cfg_t * sep)
{
  ...
  // TODO: move to quic_quicly_crypto.c
  // ctx->crypto_engine = ccfg->crypto_engine;
  // ctx->ckpair_index = ccfg->ckpair_index;
  if ((error = quic_eng_crypto_context_acquire (ccfg->crypto_engine, ccfg->ckpair_index)))
    return error;
...
quic_check_quic_session_connected (quic_ctx_t *ctx)
{
  ...
  session_connected = quic_eng_is_session_connected (ctx);
...
quic_transfer_connection (u32 ctx_index, u32 dest_thread)
{
  ...
  if (PREDICT_FALSE (!quic_engine_vfts[qm->engine_type].receive_connection))
    {
      QUIC_DBG (1, "receive_connection() not available for %s library\n",
		quic_engine_type_str (qm->engine_type));
      return;
    }
  session_send_rpc_evt_to_thread (dest_thread, quic_engine_vfts[qm->engine_type].receive_connection ,
				  (void *) temp_ctx);
}
...
quic_custom_tx_callback (void *s, transport_send_params_t * sp)
{
  ...
  {
    QUIC_DBG("quic_eng_stream_tx(ctx=0x%lx) failed!", ctx);
    return 0;
  }
...
rx_start:
  ...
  for (i = 0; i < max_packets; i++)
	...
	break;
      ...
      rv = quic_eng_process_one_rx_packet (udp_session_handle, f, fifo_offset,
				       &packets_ctx[i]);
  ...
  for (i = 0; i < max_packets; i++)
	...
	quic_eng_receive_a_packet (ctx, &packets_ctx[i]);
	break;
      case QUIC_PACKET_TYPE_ACCEPT:
	// FIXME: Process return value and handle errors.
	quic_eng_accept_connection (&packets_ctx[i]);
	break;
      case QUIC_PACKET_TYPE_RESET:
	// FIXME: Process return value and handle errors.
	quic_eng_reset_connection (udp_session_handle, &packets_ctx[i]);
	break;
      }
    }
  ctx = prev_ctx = NULL;
...
quic_init (vlib_main_t * vm)
{
  ...
  qm->session_cache.super.cb = quic_eng_encrypt_ticket_cb;
...
quic_show_aggregated_stats (vlib_main_t * vm)
{
	...
	if (quic_ctx_is_conn (ctx) && ctx->conn)
	  {
	    ...
	    agg_stats.num_packets_ack_received += st.num_packets_ack_received;
  ...
  vlib_cli_output (vm, "Packets Received %lu",
		   agg_stats.num_packets_received);
...
quic_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  qm->engine_type = quic_get_engine_type (QUIC_ENGINE_QUICLY, QUIC_ENGINE_OPENSSL);
  fprintf (stderr, "DAW: %s", __FUNCTION__);
  return 0;
}
