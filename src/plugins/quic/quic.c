...
/*    QUIC_DBG (2, "received data: %lu bytes", len); */
  ...
  ASSERT (svm_fifo_enqueue (rx_fifo, len, input.base) == len);
...
quic_custom_tx_callback (void *s)
{
  ...
  data = malloc (deq_max);
  ASSERT (svm_fifo_peek (f, 0, deq_max, data) == deq_max);
  stream = ctx->c_quic_ctx_id.stream;

  ASSERT (!quicly_streambuf_egress_write (stream, data, deq_max));
  free (data);
  ASSERT (svm_fifo_dequeue_drop (f, deq_max) == deq_max);
...
quic_app_rx_callback (session_t * udp_session)
{
  ...
  struct sockaddr *sa = (struct sockaddr *) &sa6;
  socklen_t salen;
  u32 max_deq, len, full_len, ctx_index, ctx_thread = UINT32_MAX;
  u8 *data;
  int err;
  ...
  do
	...
	return 0;
      ...
      ASSERT (svm_fifo_peek (f, 0, SESSION_CONN_HDR_LEN, (u8 *) & ph) ==
	      SESSION_CONN_HDR_LEN);
      ASSERT ((len = ph.data_length - ph.data_offset) >= 0);
      ASSERT ((full_len =
	       ph.data_length + ph.data_offset + SESSION_CONN_HDR_LEN) <=
	      max_deq);

      /* Quicly can read len bytes from the fifo at offset:
       * ph.data_offset + SESSION_CONN_HDR_LEN */
      data = malloc (ph.data_length);
      ASSERT (svm_fifo_peek
	      (f, ph.data_offset + SESSION_CONN_HDR_LEN, ph.data_length,
	       data) == ph.data_length);
      plen =
