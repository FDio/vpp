...
static int
quic_session_accept_callback (session_t * s)
{
  clib_warning ("QUIC session Accept???");
  return -1;
}
...
static void
fill_quic_connection_info (transport_connection_t * tc,
			   u8 ip[16], u8 * is_ip4, u16 * port, u8 is_lcl)
{
  ...
  QUIC_DBG (2, "Called fill_quic_connection_info");
  ...
  if (q_ctx->is_stream)
  ...
  else
    {
      ...
      tp_vft->fill_connection_info (udp_tc, ip, is_ip4, port, is_lcl);
    }
}
...
const static transport_proto_vft_t quic_proto = {
  ...
  .fill_connection_info = fill_quic_connection_info,
};
/* *INDENT-ON* */
