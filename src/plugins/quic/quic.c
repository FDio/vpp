...
{
  if (PREDICT_FALSE(!quic_main.wrk_ctx)) {
    return 0;
  }
...
{
  if (quic_set_time_now (thread_index))
  {
    tw_timer_wheel_1t_3w_1024sl_ov_t *tw = &quic_main.wrk_ctx[thread_index].timer_wheel;
  ...
  if (rv)
    {
      QUIC_DBG (2,
                ...
                "failed (rv=%d)",conn, &stream, &stream_data, is_unidir, rv);
      return -1;
    }
...
quic_transfer_connection (u32 ctx_index, u32 dest_thread)
{
  ...
  session_send_rpc_evt_to_thread (
    dest_thread, quic_engine_vfts[qm->engine_type].receive_connection,
    (void *) temp_ctx);
}
				   ...
				   session_cleanup_ntf_t ntf)
{
  ...
  quic_stop_ctx_timer (&quic_main, ctx);
...
quic_init (vlib_main_t * vm)
{
  ...
  clib_warning ("DAW: %s", __FUNCTION__);
...
quic_main_loop_init (vlib_main_t *vm)
{
  ...
  assert (qm->engine_type != QUIC_ENGINE_NONE);
  assert (quic_engine_vfts);
  if (quic_engine_vfts && qm->engine_type)
    {
      qm->wrk_ctx = quic_engine_vfts[qm->engine_type].engine_init (qm);
    }
  return 0;
}
