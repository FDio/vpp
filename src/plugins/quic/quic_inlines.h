...
static_always_inline void
quic_lib_show_aggregated_stats (vlib_main_t * vm)
{
  quic_main_t *qm = get_quic_main ();
  ...
  if (PREDICT_FALSE (!quic_vfts[qm->lib_type].show_aggregated_stats))
    {
      QUIC_DBG (1, "show_aggregated_stats() not available for %s library\n",
		quic_lib_type_str (qm->lib_type));
      return;
    }
  quic_vfts[qm->lib_type].show_aggregated_stats (vm);
}
...
quic_lib_reset_stream_connect_error (void * quic_stream)
{
  ...
  if (PREDICT_FALSE (!quic_vfts[qm->lib_type].reset_stream_connect_error))
    {
      QUIC_DBG (1, "reset_stream_connect_error() not available for %s library\n",
		quic_lib_type_str (qm->lib_type));
      return;
    }
  quic_vfts[qm->lib_type].reset_stream_connect_error (quic_stream);
}
