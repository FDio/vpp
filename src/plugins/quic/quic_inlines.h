...
quic_eng_udp_session_rx_packets (session_t * udp_session)
{
  ...
  if (PREDICT_FALSE (!quic_engine_vfts[qm->engine_type].udp_session_rx_packets))
    {
      QUIC_DBG (1, "udp_session_rx_packets() not available for %s library\n",
		quic_engine_type_str (qm->engine_type));
      return -1;
    }
  return (quic_engine_vfts[qm->engine_type].udp_session_rx_packets (udp_session));
}
...
quic_eng_app_cert_key_pair_delete (app_cert_key_pair_t * ckpair)
{
  ...
  if (PREDICT_FALSE (!quic_engine_vfts[qm->engine_type].app_cert_key_pair_delete))
    {
      QUIC_DBG (1, "app_cert_key_pair_delete() not available for %s library\n",
		quic_engine_type_str (qm->engine_type));
      return -1;
    }
...
quic_eng_crypto_context_acquire (quic_ctx_t * ctx)
{
  ...
  if (PREDICT_FALSE (!quic_engine_vfts[qm->engine_type].crypto_context_acquire))
    {
      QUIC_DBG (1, "crypto_context_acquire() not available for %s library\n",
		quic_engine_type_str (qm->engine_type));
      return -1;
    }
...
quic_eng_crypto_context_release (u32 crypto_context_index, u8 thread_index)
{
  ...
  if (PREDICT_FALSE (!quic_engine_vfts[qm->engine_type].crypto_context_release))
    {
      QUIC_DBG (1, "crypto_context_release() not available for %s library\n",
		quic_engine_type_str (qm->engine_type));
      return;
    }
  quic_engine_vfts[qm->engine_type].crypto_context_release (crypto_context_index,
                                                            thread_index);
}
...
quic_eng_connect (quic_ctx_t * ctx, u32 ctx_index, u32 thread_index, struct sockaddr *sa)
{
  ...
  return (quic_engine_vfts[qm->engine_type].connect (ctx, ctx_index, thread_index, sa));
}
