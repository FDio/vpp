...
quic_lib_format_connection_stats (u8 * s, quic_ctx_t * ctx)
{
  quic_main_t *qm = get_quic_main ();
  ...
  return (quic_vfts[qm->lib_type].format_connection_stats (s, ctx));
}

static_always_inline u8 *
quic_lib_format_stream_id (u8 * s, va_list * args)
{
  quic_main_t *qm = get_quic_main ();
  ...
  if (PREDICT_FALSE (!quic_vfts[qm->lib_type].format_stream_id))
    {
      QUIC_DBG (1, "format_stream_id() not available for %s library\n",
		quic_lib_type_str (qm->lib_type));
      return s;
    }
  return (quic_vfts[qm->lib_type].format_stream_id (s, args));
}
