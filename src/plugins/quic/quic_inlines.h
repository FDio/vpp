...
quic_eng_connection_delete (quic_ctx_t *ctx)
{
  ...
  if (PREDICT_FALSE (!quic_engine_vfts[qm->engine_type].connection_delete))
    {
      QUIC_DBG (1, "connection_delete() not available for %s engine\n",
		quic_engine_type_str (qm->engine_type));
      return;
    }
...
quic_eng_store_conn_ctx (void *conn, quic_ctx_t *ctx)
{
  ...
  if (PREDICT_FALSE (!quic_engine_vfts[qm->engine_type].store_conn_ctx))
    {
      QUIC_DBG (1, "store_conn_ctx() not available for %s engine\n",
		quic_engine_type_str (qm->engine_type));
      return;
    }
...
quic_eng_reset_stream_connect_error (void *quic_stream)
{
  ...
  if (PREDICT_FALSE (
    ...
    {
      QUIC_DBG (1,
		"reset_stream_connect_error() not available for %s engine\n",
		quic_engine_type_str (qm->engine_type));
      return;
    }
