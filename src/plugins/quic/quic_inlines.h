...
quic_eng_engine_init (quic_main_t *qm)
{
  ...
  if (PREDICT_FALSE (!quic_engine_vfts[qm->engine_type].engine_init))
    {
      QUIC_DBG (1, "engine_init() not available for %s library\n",
		quic_engine_type_str (qm->engine_type));
      return 0;
    }
  return (quic_engine_vfts[qm->engine_type].engine_init (qm));
}
...
quic_eng_format_stream_ctx_stream_id (u8 *s, va_list *args)
{
  ...
  if (PREDICT_FALSE (
    ...
    {
      QUIC_DBG (1,
		"format_stream_ctx_stream_id() not available for %s library\n",
		quic_engine_type_str (qm->engine_type));
      return s;
    }
  return (
...
quic_eng_reset_stream_connect_error (void *quic_stream)
{
  ...
  if (PREDICT_FALSE (
	!quic_engine_vfts[qm->engine_type].reset_stream_connect_error))
    {
      QUIC_DBG (1,
		"reset_stream_connect_error() not available for %s library\n",
		quic_engine_type_str (qm->engine_type));
      return;
    }
