...
static int vpp_crypto_cipher_setup_crypto(ptls_cipher_context_t *_ctx, int is_enc, const void *key, const EVP_CIPHER *cipher,
...
{
    return vpp_crypto_cipher_setup_crypto(ctx, 1, key, EVP_aes_128_ctr(), vpp_crypto_cipher_encrypt);
}

static int aes256ctr_setup_crypto(ptls_cipher_context_t *ctx, int is_enc, const void *key)
{
    return vpp_crypto_cipher_setup_crypto(ctx, 1, key, EVP_aes_256_ctr(), vpp_crypto_cipher_encrypt);
}

size_t vpp_crypto_aead_encrypt(ptls_aead_context_t *_ctx, void *output, const void *input, size_t inlen, uint64_t seq, 
...
size_t vpp_crypto_aead_decrypt(ptls_aead_context_t *_ctx, void *_output, const void *input, size_t inlen,
                                const void *iv, const void *aad, size_t aadlen)
{
    QUIC_DBG (1, "[quic] %s", __FUNCTION__);
...
ptls_cipher_suite_t vpp_crypto_aes128gcmsha256 = {PTLS_CIPHER_SUITE_AES_128_GCM_SHA256, &vpp_crypto_aes128gcm,
                                                     &ptls_openssl_sha256};
ptls_cipher_suite_t vpp_crypto_aes256gcmsha384 = {PTLS_CIPHER_SUITE_AES_256_GCM_SHA384, &vpp_crypto_aes256gcm,
                                                     &ptls_openssl_sha384};


ptls_cipher_suite_t *vpp_crypto_cipher_suites[] = {&vpp_crypto_aes256gcmsha384, &vpp_crypto_aes128gcmsha256, NULL};
