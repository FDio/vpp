...
__clib_export void
quic_conn_accept_timer_stop (quic_worker_ctx_t *wc, quic_ctx_t *ctx)
{
  quic_timer_wheel_t *tw = &wc->timer_wheel;
  quic_conn_timer_stop (tw, ctx, QUIC_TIMER_ACCEPT);
}

__clib_export void
quic_conn_tx_timer_stop (quic_worker_ctx_t *wc, quic_ctx_t *ctx)
{
  quic_timer_wheel_t *tw = &wc->timer_wheel;
  quic_conn_timer_stop (tw, ctx, QUIC_TIMER_TX);
}

__clib_export void
quic_conn_tx_timer_update (quic_worker_ctx_t *wc, quic_ctx_t *ctx, int64_t next_timeout)
{
  quic_timer_wheel_t *tw = &wc->timer_wheel;
  ...
  if (ctx->timers[QUIC_TIMER_TX] == QUIC_TIMER_HANDLE_INVALID)
      ...
      if (next_timeout == INT64_MAX)
	{
      ...
      quic_conn_timer_start (tw, ctx, QUIC_TIMER_TX, next_interval);
    }
  else
    {
      if (next_timeout == INT64_MAX)
