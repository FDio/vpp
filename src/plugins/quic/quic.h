...
#define foreach_quic_wrk_ctx_field                                                       \
  _ (int64_t, time_now)		 /**< worker time */                                           \
  _ (tw_timer_wheel_1t_3w_1024sl_ov_t, timer_wheel)  /**< worker timer wheel */          \
  _ (crypto_context_t, *crypto_ctx_pool)		 /**< per thread pool of crypto contexes */  \
  _ (clib_bihash_24_8_t, crypto_context_hash)  /**< per thread [params:crypto_ctx_index] hash */
...
quic_get_time (void)
{
  ...
  return qm->wrk_ctx[thread_index].time_now;
}
...
typedef struct quic_engine_vft_
{
  ...
  int (*connect) (quic_main_t *qm, quic_ctx_t * ctx, u32 ctx_index, u32 thread_index, struct sockaddr *sa);
