...
#define foreach_quic_wrk_ctx_field                                            \
  ...
  _ (tw_timer_wheel_1t_3w_1024sl_ov_t,                                        \
     timer_wheel) /**< worker timer wheel */                                  \
  _ (crypto_context_t,                                                        \
     *crypto_ctx_pool) /**< per thread pool of crypto contexes */
...
quic_ctx_get_if_valid (u32 ctx_index, u32 thread_index)
{
  if (pool_is_free_index (get_quic_main ()->ctx_pool[thread_index], ctx_index))
    return 0;
  return pool_elt_at_index (get_quic_main ()->ctx_pool[thread_index],
			    ctx_index);
}
