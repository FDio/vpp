...
typedef struct quic_lib_api_vft_
{
  int (*init_crypto_context) (crypto_context_t *crctx, quic_ctx_t *ctx);
  quic_ctx_t *(*conn_ctx_get) (void * conn);
} quic_lib_api_vft_t;
...
extern void quic_on_receive (quicly_stream_t * stream, size_t off, const void *src, size_t len);
extern void quic_fifo_egress_shift (quicly_stream_t * stream, size_t delta);
extern void quic_fifo_egress_emit (quicly_stream_t * stream, size_t off, void *dst, size_t * len, int *wrote_all);
