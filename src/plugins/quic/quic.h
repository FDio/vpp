...
/* Core QUIC plugin configuration */
#define QUIC_DEBUG		  0
...
/* QUIC engine types - each engine is implemented as a separate plugin */
typedef enum quic_engine_type_
{
  QUIC_ENGINE_NONE,
  QUIC_ENGINE_QUICLY,  /* Implemented in quic_quicly plugin */
  QUIC_ENGINE_OPENSSL, /* Implemented in quic_openssl plugin */
...
/* QUIC engine virtual function table - defines the interface that each
 * QUIC engine implementation must provide. Engine implementations are
 * provided by separate plugins (e.g. quic_quicly, quic_openssl) */
typedef struct quic_engine_vft_
{
  ...
  void (*connection_receive) (quic_ctx_t *ctx);
...
/* Engine registration functions - called by engine implementation plugins */
