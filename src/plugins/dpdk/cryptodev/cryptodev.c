				      ...
				      u32 n_jobs, cryptodev_op_type_t op_type)
{
  ...
  n = rte_crypto_op_bulk_alloc (numa->cop_pool,
				RTE_CRYPTO_OP_TYPE_SYMMETRIC,
				(struct rte_crypto_op **) cet->cops, n_jobs);
  cop = cet->cops;
  while (n)
    {
      vlib_buffer_t *b_src, *b_dst = 0;
      ...
      if (last_key_index != job[0]->key_index)
	{
      ...
      m_src = rte_mbuf_from_vlib_buffer (b_src);
      /* oop operation */
      if (job[0]->asrc != job[0]->adst)
	{
	  b_dst = vlib_get_buffer (vm, job[0]->async_dst.bi);
  ...
  if (n_enqueue < n_jobs)
    {
      for (i = n_enqueue; i < n_jobs; i++)
	{
	  /* failed enqueue ops need to retrieve array idx */
	  if (i < n)
	    jobs[cop[i]->user_data]->status =
	      VNET_CRYPTO_OP_STATUS_FAIL_ENGINE_ERR;
	  else
	    jobs[i]->status = VNET_CRYPTO_OP_STATUS_FAIL_ENGINE_ERR;
	}
      if (n_enqueue < n)
	rte_mempool_put_bulk (numa->cop_pool,
			      (void **) (cet->cops + n_enqueue),
			      n - n_enqueue);
    }
			   ...
			   u32 n_post_jobs)
{
  ...
  if (n_dequeue >= 4)
    {
      u8 *p[4];
      p[0] = (u8 *) cop[0] + offsetof (cryptodev_op_t, buffer_index);
      p[1] = (u8 *) cop[1] + offsetof (cryptodev_op_t, buffer_index);
      p[2] = (u8 *) cop[2] + offsetof (cryptodev_op_t, buffer_index);
      p[3] = (u8 *) cop[3] + offsetof (cryptodev_op_t, buffer_index);

      CLIB_PREFETCH (cop[0], CLIB_CACHE_LINE_BYTES, LOAD);
      CLIB_PREFETCH (p[0], CLIB_CACHE_LINE_BYTES, LOAD);
      CLIB_PREFETCH (cop[1], CLIB_CACHE_LINE_BYTES, LOAD);
      CLIB_PREFETCH (p[1], CLIB_CACHE_LINE_BYTES, LOAD);
      CLIB_PREFETCH (cop[2], CLIB_CACHE_LINE_BYTES, LOAD);
      CLIB_PREFETCH (p[2], CLIB_CACHE_LINE_BYTES, LOAD);
      CLIB_PREFETCH (cop[3], CLIB_CACHE_LINE_BYTES, LOAD);
  ...
  while (n_dequeue >= 4)
	...
	{
	  u8 *p[4];
	  p[0] = (u8 *) cop[4] + offsetof (cryptodev_op_t, buffer_index);
	  p[1] = (u8 *) cop[5] + offsetof (cryptodev_op_t, buffer_index);
	  p[2] = (u8 *) cop[6] + offsetof (cryptodev_op_t, buffer_index);
	  p[3] = (u8 *) cop[7] + offsetof (cryptodev_op_t, buffer_index);

	  CLIB_PREFETCH (cop[4], CLIB_CACHE_LINE_BYTES, LOAD);
	  CLIB_PREFETCH (p[0], CLIB_CACHE_LINE_BYTES, LOAD);

	  CLIB_PREFETCH (cop[5], CLIB_CACHE_LINE_BYTES, LOAD);
	  CLIB_PREFETCH (p[1], CLIB_CACHE_LINE_BYTES, LOAD);

	  CLIB_PREFETCH (cop[6], CLIB_CACHE_LINE_BYTES, LOAD);
	  CLIB_PREFETCH (p[2], CLIB_CACHE_LINE_BYTES, LOAD);

	  CLIB_PREFETCH (cop[7], CLIB_CACHE_LINE_BYTES, LOAD);
	  CLIB_PREFETCH (p[3], CLIB_CACHE_LINE_BYTES, LOAD);
	}

      post_job[0].status = decode_crypto_op_status (cop[0]->op.status);
      post_job[0].bi = cop[0]->buffer_index;
      post_job[0].next = cop[0]->next_index;

      post_job[1].status = decode_crypto_op_status (cop[1]->op.status);
      post_job[1].bi = cop[1]->buffer_index;
      post_job[1].next = cop[1]->next_index;

      post_job[2].status = decode_crypto_op_status (cop[2]->op.status);
      post_job[2].bi = cop[2]->buffer_index;
      post_job[2].next = cop[2]->next_index;

      post_job[3].status = decode_crypto_op_status (cop[3]->op.status);
...
dpdk_cryptodev_init (vlib_main_t * vm)
{
  ...
  clib_bitmap_foreach (numa, tm->cpu_socket_bitmap, (
    {
      ...
      mp = rte_crypto_op_pool_create( (const char *) name,
      				      RTE_CRYPTO_OP_TYPE_SYMMETRIC,
				      n_cop_elts,
				      VLIB_FRAME_SIZE,
				      cop_priv_sz,
  ...
  vec_validate_aligned(cmt->per_thread_data, n_workers - 1,
