...
{
  return ad->alg_type == VNET_CRYPTO_ALG_T_INTEG ? digest_len : aad_len;
}
				  ...
				  vnet_crypto_frame_enq_fn_t *dec_fn)
{
  ...
  dpdk_log_debug ("cryptodev async install backend=%s alg=%s enc-op=%u dec-op=%u", backend_name,
		  ad->name, enc_op, dec_op);
}
