...
cryptodev_frame_build_sgl (vlib_main_t * vm, struct rte_crypto_vec *data_vec,
			   u16 * n_seg, vlib_buffer_t * b)
{
				     ...
				     cryptodev_op_type_t op_type)
{
  ...
  cryptodev_key_t *key;
  u32 last_key_index;
  union rte_crypto_sym_ofs cofs, last_cofs;
  i16 min_ofs;
  u32 max_end;
  ...
  last_cofs.raw = cofs.raw = compute_ofs_linked_alg (fe, &min_ofs, &max_end);
  ...
  while (n_elts)
      ...
      if (n_elts > 2)
	{
      ...
      if (PREDICT_FALSE (last_key_index != fe->key_index ||
			 (cofs.raw != last_cofs.raw &&
			  n_elts != frame->n_elts)))
	{
	  last_cofs.raw = cofs.raw;
      ...
      if (PREDICT_FALSE (fe->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS))
	{
	  ...
	  if (cryptodev_frame_build_sgl (vm, vec, &n_seg, b[0]) < 0)
	    {
	      cryptodev_mark_frame_err_status (frame,
					       VNET_CRYPTO_OP_STATUS_FAIL_ENGINE_ERR);
	      return -1;
			     ...
			     cryptodev_op_type_t op_type, u8 aad_len)
{
  ...
  while (n_elts)
	...
	{
	  vec[0].iova =
	    pointer_to_uword (b[0]) + 256 + fe->crypto_start_offset;
	  vec[0].base =
	    (void *) ((u8 *) b[0] + 256 + fe->crypto_start_offset);
...
/* create a dumb session to initialize dp service, and use the data in
 * data-path
 */
static void
initialize_dp_service (cryptodev_numa_data_t *numa_data, u8 dev_id, u16 q_id,
		       u8 *data)
{
  struct rte_crypto_sym_xform xform;
  struct rte_cryptodev_sym_session *session =
      rte_cryptodev_sym_session_create (numa_data->sess_pool);
  u8 *key_data = 0;
  ...
  vec_validate (key_data, 15);

  key.alg = VNET_CRYPTO_ALG_AES_128_GCM;
  key.data = key_data;
  key.type = VNET_CRYPTO_KEY_TYPE_DATA;

  prepare_aead_xform (&xform, CRYPTODEV_OP_TYPE_ENCRYPT, &key, 8);

  ret = rte_cryptodev_sym_session_init (dev_id, session, &xform,
					numa_data->sess_priv_pool);
  ASSERT (ret == 0);

  ret = rte_cryptodev_dp_configure_service(
      dev_id, q_id, RTE_CRYPTO_DP_SYM_AEAD, RTE_CRYPTO_OP_WITH_SESSION,
      (union rte_cryptodev_session_ctx)session,
      (struct rte_crypto_dp_service_ctx *)data, 0);
  ASSERT (ret == 0);

  cryptodev_session_del (session);
  ...
  for (i = 0; i < info.max_nb_queue_pairs; i++)
    {
      ...
      initialize_dp_service(numa_data, cryptodev_id, i,
			    cdev_inst->dp_service_buffer);
