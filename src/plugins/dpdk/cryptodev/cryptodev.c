...
cryptodev_adj_mbuf_chain (vlib_main_t * vm, struct rte_mbuf *m,
			  vlib_buffer_t * b, u8 * digest)
{
  rte_iova_t digest_iova = 0;
  while (b->next_buffer != 0)
    {
	...
	return 0;
      if (b->current_data != 0)
	m->data_off += b->current_data;
      if (b->current_length > m->data_len)
	m->data_len = b->current_length;
      if (b->data <= digest &&
	  b->data + b->current_data + b->current_length > digest)
	digest_iova = rte_pktmbuf_iova (m) + digest -
	  rte_pktmbuf_mtod (m, u8 *);
    }
				     ...
				     u32 digest_len)
{
  ...
  while (n_elts)
	...
	sop->auth.digest.phys_addr = cryptodev_adj_mbuf_chain (vm, sop->m_src,
							       b, fe->digest);
			     ...
			     cryptodev_op_type_t op_type, u8 aad_len)
{
  ...
  while (n_elts)
	...
	sop->aead.digest.phys_addr = cryptodev_adj_mbuf_chain (vm, sop->m_src,
							       b, fe->tag);
