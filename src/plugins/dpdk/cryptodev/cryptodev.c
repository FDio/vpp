...
cryptodev_is_linked_alg (vnet_crypto_alg_t alg)
{
  switch (alg)
    {
#define _(a, b, c, d, e)                                                                           \
		      ...
		      const vnet_crypto_key_t *key)
{
  ...
  u32 digest_len = ~0, cipher_key_len = 0;
  const u8 *auth_key_data = 0;
  u32 auth_key_len = 0;
...
#undef _
  ...
  auth_key_data = key->data + cipher_key_len;
  auth_key_len = key->length - cipher_key_len;

  xform_cipher->cipher.algo = cipher_algo;
  xform_cipher->cipher.key.data = key->data;
  xform_cipher->cipher.key.length = cipher_key_len;
  ...
  xform_auth->auth.key.data = auth_key_data;
  xform_auth->auth.key.length = auth_key_len;
...
cryptodev_check_supported_vnet_alg (vnet_crypto_key_t *key)
{
  ...
  if (cryptodev_is_linked_alg (key->alg))
    {
      switch (key->alg)
	{
#define _(a, b, c, d, e)                                                      \
