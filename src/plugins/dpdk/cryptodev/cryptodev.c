...
#define CRYPTODEV_TAG_OFFSET (offsetof (cryptodev_op_t, tag))
...
cryptodev_frame_linked_algs_enqueue (vlib_main_t * vm,
				     vnet_crypto_async_frame_t * frame,
				     cryptodev_op_type_t op_type, u32 is_enc,
				     u32 digest_len)
{
  cryptodev_main_t *cmt = &cryptodev_main;
  ...
  vec_validate_aligned (cet->cops, n_elts, CLIB_CACHE_LINE_BYTES);
  ...
  while (n_elts)
    {
      ...
      u16 integ_offset = fe->integ_start_offset;
      ...
      if (last_key_index != fe->key_index)
	{
      ...
      sop->m_src = sop->m_dst = rte_mbuf_from_vlib_buffer (b);
      ...
      if (PREDICT_TRUE (fe->integ_start_offset < 0))
	{
      ...
      sop->cipher.data.length = fe->total_length - fe->crypto_start_offset;
      sop->auth.data.offset = integ_offset;
      sop->auth.data.length = fe->total_length - fe->integ_start_offset;
      sop->auth.digest.data = cop[0]->digest;
      sop->auth.digest.phys_addr =
	cop[0]->op.phys_addr + CRYPTODEV_DIGEST_OFFSET;
      clib_memcpy_fast (cop[0]->iv, fe->iv, 16);
      if (!is_enc)
	clib_memcpy_fast (cop[0]->digest, fe->digest, digest_len);
      cop++;
      bi++;
      fe++;
      n_elts--;
  ...
  rte_ring_sp_enqueue_burst (cet->ring, (void *) cet->cops, frame->n_elts,
			     NULL);
			     ...
			     u32 is_enc)
{
  ...
  while (n_elts)
      ...
      if (PREDICT_FALSE (fe->crypto_start_offset < 0))
	{
      ...
      sop->aead.data.length = fe->total_length - crypto_offset;
      sop->aead.data.offset = crypto_offset;
      sop->aead.digest.data = cop[0]->tag;
      sop->aead.digest.phys_addr =
	cop[0]->op.phys_addr + CRYPTODEV_TAG_OFFSET;
      clib_memcpy_fast (cop[0]->iv, fe->iv, 12);
      clib_memcpy_fast (cop[0]->aad, fe->aad, aad_len);
      if (!is_enc)
	clib_memcpy_fast (cop[0]->tag, fe->tag, 16);
      cop++;
      bi++;
      fe++;
      n_elts--;
  ...
  cet->cops[0]->n_elts = cop - cet->cops;
...
#define CRYPTODEV_GET_RING_OBJ(ring, pos, cop) \
do { \
  cryptodev_op_t **r = (void *)&ring[1]; \
  cop = r[(ring->cons.head + pos) & ring->mask]; \
} while (0)

static_always_inline vnet_crypto_async_frame_t *
cryptodev_frame_enc_dequeue (vlib_main_t * vm, u32 tag_len)
{
  cryptodev_main_t *cmt = &cryptodev_main;
  cryptodev_numa_data_t *numa = cmt->per_numa_data + vm->numa_node;
  cryptodev_engine_thread_t *cet = cmt->per_thread_data + vm->thread_index;
  cryptodev_op_t *cop0, **cop;
  vnet_crypto_async_frame_elt_t *fe;
  vnet_crypto_async_frame_t *frame;
  u32 n_elts, n_dequeue;
  u32 ss0 = 0, ss1 = 0, ss2 = 0, ss3 = 0;	/* sum of status */

  if (PREDICT_FALSE (rte_ring_empty (cet->ring)))
    return NULL;

  if (cet->inflight)
    cet->inflight -= rte_cryptodev_dequeue_burst
      (cet->cryptodev_id, cet->cryptodev_q,
       (struct rte_crypto_op **) cet->cops, VNECT_CRYPTO_FRAME_SIZE);

  /* peek first obj in the ring, get frame and n_elts data */
  CRYPTODEV_GET_RING_OBJ (cet->ring, 0, cop0);
  frame = cop0->frame;
  n_elts = cop0->n_elts;

  /* peek [n_elt - 1] in the ring, if processed, dequeue them. */
  CRYPTODEV_GET_RING_OBJ (cet->ring, (n_elts - 1), cop0);
  if (PREDICT_FALSE (cop0->op.status == RTE_CRYPTO_OP_STATUS_NOT_PROCESSED))
    return NULL;

  /* an duplicated dequeue from ring to update head/tail only */
  cet->cops[0] = 0;
  n_dequeue = rte_ring_sc_dequeue_bulk (cet->ring, (void **) cet->cops,
					n_elts, NULL);
  ASSERT (n_dequeue == n_elts);
  ...
  frame->state =
    (VNET_CRYPTO_OP_STATUS_COMPLETED * frame->n_elts ==
     (ss0 + ss1 + ss2 + ss3)) ?
...
static_always_inline vnet_crypto_async_frame_t *
cryptodev_frame_dec_dequeue (vlib_main_t * vm)
{
  cryptodev_main_t *cmt = &cryptodev_main;
  cryptodev_numa_data_t *numa = cmt->per_numa_data + vm->numa_node;
  cryptodev_engine_thread_t *cet = cmt->per_thread_data + vm->thread_index;
...
{ \
  return cryptodev_frame_gcm_enqueue (vm, frame, \
				      CRYPTODEV_OP_TYPE_DECRYPT, f, 0); \
} \
static_always_inline vnet_crypto_async_frame_t * \
cryptodev_dequeue_##a##_AAD##f##_enc (vlib_main_t * vm) \
{ \
  return cryptodev_frame_enc_dequeue (vm, e); \
} \
static_always_inline vnet_crypto_async_frame_t * \
cryptodev_dequeue_##a##_AAD##f##_dec (vlib_main_t * vm) \
{ \
  return cryptodev_frame_dec_dequeue (vm); \
}
...
{ \
  return cryptodev_frame_linked_algs_enqueue (vm, frame, \
					      CRYPTODEV_OP_TYPE_DECRYPT, 0, \
					      d); \
} \
static_always_inline vnet_crypto_async_frame_t * \
cryptodev_dequeue_##a##_##c##_TAG##d##_enc (vlib_main_t * vm) \
{ \
  return cryptodev_frame_enc_dequeue (vm, d); \
} \
static_always_inline vnet_crypto_async_frame_t * \
cryptodev_dequeue_##a##_##c##_TAG##d##_dec (vlib_main_t * vm) \
{ \
  return cryptodev_frame_dec_dequeue (vm); \
}
...
cryptodev_configure (vlib_main_t *vm, uint32_t cryptodev_id)
{
   ...
   **/
  if (rte_cryptodev_socket_id (cryptodev_id) != vm->numa_node)
    return -1;

  int ret = check_cryptodev_alg_support (cryptodev_id);
  if (ret != 0)
    return ret;
...
dpdk_disable_cryptodev_engine (vlib_main_t * vm)
{
  ...
  u32 numa = vec_elt_at_index (cmt->per_numa_data, vm->numa_node);
  cryptodev_numa_data_t *numa_data;
...
dpdk_cryptodev_init (vlib_main_t * vm)
{
  ...
  n_cop_elts = max_pow2 (n_workers * VLIB_FRAME_SIZE * 4);
  ...
  for (i = skip_master; i < tm->n_vlib_mains; i++)
    {
      ...
      ptd->ring = rte_ring_create((char *) name, VLIB_FRAME_SIZE * 4,
