				     ...
				     cryptodev_op_type_t op_type)
{
  ...
  cryptodev_op_t **cop;
  u32 *bi;
  u32 n_enqueue, n_elts = frame->n_elts;
  ...
  while (n_elts)
      ...
      if (PREDICT_TRUE (fe->integ_start_offset < 0))
	{
      ...
      sop->auth.digest.data = fe->tag;
      sop->auth.digest.phys_addr = cryptodev_get_data_iova (pm, fe->tag);
      if (PREDICT_FALSE (sop->auth.digest.phys_addr == 0))
	{
	  cryptodev_mark_frame_err_status
	    (frame, VNET_CRYPTO_OP_STATUS_FAIL_ENGINE_ERR);
	  rte_mempool_put_bulk (numa->cop_pool, (void **) cet->cops,
				frame->n_elts);
	  return -1;
	}
  ...
  rte_ring_enqueue_burst (cet->ring, (void *) cet->cops, frame->n_elts, NULL);
			     ...
			     cryptodev_op_type_t op_type, u8 aad_len)
{
  ...
  cryptodev_op_t **cop;
  u32 *bi;
  u32 n_enqueue = 0, n_elts = frame->n_elts;
  cryptodev_key_t *key = 0;
  u32 last_key_index = ~0;
  ...
  while (n_elts)
      ...
      if (PREDICT_FALSE (fe->crypto_start_offset < 0))
	{
      ...
      sop->aead.digest.phys_addr = cryptodev_get_data_iova (pm, fe->tag);
      if (PREDICT_FALSE (sop->aead.digest.phys_addr == 0))
	{
	  cryptodev_mark_frame_err_status
	    (frame, VNET_CRYPTO_OP_STATUS_FAIL_ENGINE_ERR);
	  rte_mempool_put_bulk (numa->cop_pool, (void **) cet->cops,
...
cryptodev_frame_dequeue (vlib_main_t * vm)
{
  ...
  cryptodev_numa_data_t *numa = cmt->per_numa_data + vm->numa_node;
  cryptodev_engine_thread_t *cet = cmt->per_thread_data + vm->thread_index;
  cryptodev_op_t *first_cop, *last_cop, **cop;
  vnet_crypto_async_frame_elt_t *fe;
  vnet_crypto_async_frame_t *frame;
  u32 n_elts;
  u32 ss0 = 0, ss1 = 0, ss2 = 0, ss3 = 0;	/* sum of status */

  if (rte_ring_empty (cet->ring))
    return NULL;

  /* peek first obj in the ring, get frame and n_elts data */
  CRYPTODEV_GET_RING_OBJ (cet->ring, 0, first_cop);
  n_elts = first_cop->n_elts;

  /* an duplicated dequeue from cryptodev queue to update head/tail only */
  if (cet->inflight)
    cet->inflight -=
      rte_cryptodev_dequeue_burst (cet->cryptodev_id, cet->cryptodev_q,
				   (struct rte_crypto_op **) cet->cops,
				   VNECT_CRYPTO_FRAME_SIZE);

  /* peek [n_elt - 1] in the ring, if processed, dequeue them. */
  CRYPTODEV_GET_RING_OBJ (cet->ring, (n_elts - 1), last_cop);
  if (PREDICT_FALSE (last_cop->op.status ==
		     RTE_CRYPTO_OP_STATUS_NOT_PROCESSED))
    return NULL;

  rte_ring_sc_dequeue_bulk (cet->ring, (void **) cet->cops, n_elts, NULL);

  frame = first_cop->frame;
  fe = frame->elts;
  cop = cet->cops;
  ...
  while (n_elts > 4)
    {
      ...
      ss1 += fe[1].status = cryptodev_status_conversion[cop[0]->op.status];
      ss2 += fe[2].status = cryptodev_status_conversion[cop[0]->op.status];
      ss3 += fe[3].status = cryptodev_status_conversion[cop[0]->op.status];
      fe += 4;
      cop += 4;
      n_elts -= 4;
    }
...
cryptodev_probe (vlib_main_t * vm)
{
  ...
  for (i = 0; i < n_cryptodevs; i++)
	...
	continue;
      ...
      int ret = check_cryptodev_alg_support (i);
      if (ret != 0)
	continue;
...
#define _(a, b, c, d, e, f) \
  vnet_crypto_register_async_handler \
	...
	cryptodev_enqueue_##a##_AAD##f##_enc, cryptodev_frame_dequeue); \
  vnet_crypto_register_async_handler \
    (vm, eidx, VNET_CRYPTO_OP_##a##_TAG##e##_AAD##f##_DEC, \
	cryptodev_enqueue_##a##_AAD##f##_dec, cryptodev_frame_dequeue); \
