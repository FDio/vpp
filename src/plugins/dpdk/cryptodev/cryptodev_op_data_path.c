				     ...
				     cryptodev_op_type_t op_type)
{
  ...
  while (n_elts)
    {
      if (last_index != frame->elts[index].key_index)
	{
	  last_key_index = frame->elts[index].key_index;
	  vnet_key = pool_elt_at_index (crypto_main.keys, last_key_index);
	}
	  ...
	  if (ret_frame)
	      ...
	      while (n_ret_frame_elts)
		{
		  if (last_key_index !=
			...
			ret_frame->elts[ret_frame_index].key_index;
		      vnet_key =
			pool_elt_at_index (crypto_main.keys, last_key_index);
		    }
		  if (vnet_crypto_key_unref (vnet_key))
			...
			vnet_crypto_key_del_main, (u8 *) &vnet_key,
			sizeof (vnet_crypto_key_t *));
		    }
		  --n_ret_frame_elts;
		  ++ret_frame_index;
		}
