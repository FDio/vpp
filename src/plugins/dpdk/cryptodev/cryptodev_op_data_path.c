			      ...
			      cryptodev_op_type_t op_type, u8 aad_len)
{
  ...
  ring->head &= (VNET_CRYPTO_FRAME_POOL_SIZE - 1);
  frame->state = aad_len;
  /* reuse state field for storing op_type */
  frame->state |= (op_type << CRYPTODEV_OP_TYPE_BIT);
  return 0;
}
				  ...
				  u32 *enqueue_thread_idx)
{
  ...
  ss0 = ring->frames[ring->deq].ss0;
  ss1 = ring->frames[ring->deq].ss1;
  ss2 = ring->frames[ring->deq].ss2;
  ss3 = ring->frames[ring->deq].ss3;
  ...
  ring->frames[ring->deq].ss0 = (u8) ss0;
  ring->frames[ring->deq].ss1 = (u8) ss1;
  ring->frames[ring->deq].ss2 = (u8) ss2;
  ring->frames[ring->deq].ss3 = (u8) ss3;
...
cryptodev_enqueue_frame (vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
  ...
  u8 linked_or_aad_len = frame->state & 0x7;
  if (linked_or_aad_len == 1)
  ...
  else
    {
      ...
      cryptodev_aead_enqueue_internal (vm, frame, op_type, linked_or_aad_len);
    }
}
			 ...
			 u32 *enqueue_thread_idx)
{
  ...
  if (PREDICT_TRUE (ring->frames[ring->enq].f != 0))
    cryptodev_enqueue_frame (vm, ring->frames[ring->enq].f);
