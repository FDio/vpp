...
compute_ofs_linked_alg (vnet_crypto_buffer_metadata_t *md, i16 *min_ofs, u32 *max_end)
{
  union rte_crypto_sym_ofs ofs;
  u32 crypto_end = md->cipher_data_start_off + md->cipher_data_len;
  u32 integ_end = md->auth_data_start_off + md->auth_data_len;

  *min_ofs = clib_min (md->cipher_data_start_off, md->auth_data_start_off);
  *max_end = clib_max (crypto_end, integ_end);

  ofs.ofs.cipher.head = md->cipher_data_start_off - *min_ofs;
  ofs.ofs.cipher.tail = *max_end - crypto_end;
  ofs.ofs.auth.head = md->auth_data_start_off - *min_ofs;
					      ...
					      cryptodev_op_type_t op_type)
{
  ...
  ctxs = frame->ctxs + ring->frames[*enq].enq_elts_head;
  ...
  while (n_elts)
    {
      ...
      vnet_crypto_buffer_metadata_t *md = vnet_crypto_buffer_get_metadata (b[0]);
	...
	sess_op_type =
	  md->is_hmac_check ? VNET_CRYPTO_OP_TYPE_DECRYPT : VNET_CRYPTO_OP_TYPE_ENCRYPT;

      if (n_elts > 2)
	{
	  clib_prefetch_load (&ctxs[1]);
	  clib_prefetch_load (&ctxs[2]);
	  vlib_prefetch_buffer_header (b[1], LOAD);
	  vlib_prefetch_buffer_header (b[2], LOAD);
	}

      if (PREDICT_FALSE (last_key_index != key_index))
	{
	  cryptodev_key_data_t *key = cryptodev_get_key_data (ctx);
	  union rte_cryptodev_session_ctx sess_ctx;
	  ...
	  cet->reset_sess = key_sess;
	  sess_ctx.crypto_sess = key_sess;
      ...
      cofs.raw = compute_ofs_linked_alg (md, &min_ofs, &max_end);
      ...
      if (cmt->iova_mode == RTE_IOVA_VA)
	{
	  ...
	  iv_vec.va = (void *) iv;
	  iv_vec.iova = pointer_to_uword (iv);
	  digest_vec.va = (void *) tag;
	  digest_vec.iova = pointer_to_uword (tag);
	}
      else
	{
	  vec[0].base = (void *) (b[0]->data + min_ofs);
	  ...
	  iv_vec.va = (void *) iv;
	  iv_vec.iova = vlib_physmem_get_pa (vm, iv);
	  digest_vec.va = (void *) tag;
	  digest_vec.iova = vlib_physmem_get_pa (vm, tag);
	}

      if (PREDICT_FALSE (md->is_chained_buffers))
	{
	  vec[0].len = clib_min ((u32) (b[0]->current_data + b[0]->current_length - min_ofs),
				 max_end - min_ofs);
...
error_exit:
  ring->frames[*enq].frame_elts_errs_mask = cryptodev_mark_frame_fill_err (
    frame, ring->frames[*enq].frame_elts_errs_mask, ring->frames[*enq].enq_elts_head, max_to_enq,
  ...
  while (n_elts)
    {
      ctx = ctxs[0];
      vnet_crypto_alg_data_t *ad = crypto_main.algs + ctx->alg;
      u8 is_gmac = ad->cipher_family == VNET_CRYPTO_ALG_FAMILY_AES_NULL_GMAC;
      ...
      u8 *iv = vnet_crypto_buffer_metadata_get_ptr (vm, b[0], md->iv_off);
      u8 *tag = vnet_crypto_buffer_metadata_get_ptr (vm, b[0], md->icv_off);
      u8 *aad = vnet_crypto_buffer_metadata_get_ptr (vm, b[0], md->aad_off);
      ...
      if (PREDICT_FALSE (last_key_index != key_index))
	{
	  ...
	  if (PREDICT_FALSE (cryptodev_session_get (key, vm->numa_node, op_type) == 0))
	    {
	  ...
	  if (!is_gmac &&
	      PREDICT_FALSE (rte_cryptodev_sym_session_opaque_data_get (cryptodev_session_get (
	  ...
	  /* Keep a valid session for ctx reset on the local error path. */
	  cet->reset_sess = cryptodev_session_get (key, vm->numa_node, op_type);
	  sess_ctx.crypto_sess = cryptodev_session_get (key, vm->numa_node, op_type);
      ...
      if (cmt->iova_mode == RTE_IOVA_VA)
	{
	  ...
	  aad_vec.iova = pointer_to_uword (cet->aad_buf + aad_offset);
	}
      else
	{
	  iv_vec.va = (void *) iv;
      ...
      if (aad_len != 0)
	clib_memset (cet->aad_buf + aad_offset, 0, 16);

      if (aad_len == 8)
	*(u64 *) (cet->aad_buf + aad_offset) = *(u64 *) aad;
      else if (aad_len != 0)
	{
	  /* aad_len == 12 */
	  *(u64 *) (cet->aad_buf + aad_offset) = *(u64 *) aad;
	  *(u32 *) (cet->aad_buf + aad_offset + 8) = *(u32 *) (aad + 8);
	}

      if (is_gmac)
	{
	  u32 first_len, size;
	  struct rte_crypto_vec *sgv = vec + 2;
	  vlib_buffer_t *lb = b[0];

	  vec[0].base = (void *) (cet->aad_buf + aad_offset);
	  vec[0].iova = aad_vec.iova;
	  vec[0].len = aad_len;

	  first_len = clib_min ((u32) (b[0]->current_length - md->cipher_data_start_off),
				md->cipher_data_len);
	  vec[1].base = (void *) (vlib_buffer_get_current (b[0]) + md->cipher_data_start_off);
	  if (cmt->iova_mode == RTE_IOVA_VA)
	    vec[1].iova = pointer_to_uword (vec[1].base);
	  else
	    vec[1].iova = vlib_buffer_get_current_pa (vm, b[0]) + md->cipher_data_start_off;
	  vec[1].len = first_len;
	  n_seg = 2;
	  size = md->cipher_data_len - first_len;

	  while ((lb->flags & VLIB_BUFFER_NEXT_PRESENT) && size)
	    {
	      u32 len;
		...
		goto error_exit;
	      ...
	      sgv->base = (void *) vlib_buffer_get_current (lb);
	      if (cmt->iova_mode == RTE_IOVA_VA)
		sgv->iova = pointer_to_uword (sgv->base);
	      else
		sgv->iova = vlib_buffer_get_current_pa (vm, lb);
	      sgv->len = len;
	      size -= len;
	      sgv++;
	      n_seg += 1;
	...
	{
	  vec[0].base = (void *) (vlib_buffer_get_current (b[0]) + md->cipher_data_start_off);
	  if (cmt->iova_mode == RTE_IOVA_VA)
	    vec[0].iova = pointer_to_uword (vec[0].base);
	  else
	    vec[0].iova = vlib_buffer_get_current_pa (vm, b[0]) + md->cipher_data_start_off;
	  vec[0].len = 1;
	  cofs.ofs.auth.tail = 1;
	  cofs.ofs.cipher.tail = 1;
	}
      else if (PREDICT_FALSE (md->is_chained_buffers))
	{
	  vec[0].base = (void *) (vlib_buffer_get_current (b[0]) + md->cipher_data_start_off);
	  vec[0].len = md->cipher_data_len;
	  if (cmt->iova_mode == RTE_IOVA_VA)
	    vec[0].iova = pointer_to_uword (vec[0].base);
	  else
	    vec[0].iova = vlib_buffer_get_current_pa (vm, b[0]) + md->cipher_data_start_off;

	  vec[0].len = clib_min ((u32) (b[0]->current_length - md->cipher_data_start_off),
				 md->cipher_data_len);
...
cryptodev_post_dequeue (void *frame, u32 index, u8 is_op_success)
{
  ...
  if (!is_op_success)
    vlib_crypto_async_frame_set_hmac_fail (f, index);
}
  ...
  if (PREDICT_FALSE (n_success < n_deq))
    {
      ...
      for (i = 0; i < n_deq; i++)
	{
	  if (uword_bitmap_is_bit_set (frame->bad_hmac_bitmap, idx + i))
...
cryptodev_raw_enq_aead_aad_0_enc (vlib_main_t *vm,
				  vnet_crypto_async_frame_t *frame)
{
  return cryptodev_raw_aead_enqueue (vm, frame, VNET_CRYPTO_OP_TYPE_ENCRYPT, 0);
}
...
cryptodev_raw_enq_aead_aad_8_enc (vlib_main_t *vm,
				  vnet_crypto_async_frame_t *frame)
{
  return cryptodev_raw_aead_enqueue (vm, frame, VNET_CRYPTO_OP_TYPE_ENCRYPT, 8);
}
static_always_inline int
cryptodev_raw_enq_aead_aad_12_enc (vlib_main_t *vm,
				   vnet_crypto_async_frame_t *frame)
{
  return cryptodev_raw_aead_enqueue (vm, frame, VNET_CRYPTO_OP_TYPE_ENCRYPT, 12);
}
...
cryptodev_raw_enq_aead_aad_0_dec (vlib_main_t *vm,
				  vnet_crypto_async_frame_t *frame)
{
  return cryptodev_raw_aead_enqueue (vm, frame, VNET_CRYPTO_OP_TYPE_DECRYPT, 0);
}
...
cryptodev_raw_enq_aead_aad_8_dec (vlib_main_t *vm,
				  vnet_crypto_async_frame_t *frame)
{
  return cryptodev_raw_aead_enqueue (vm, frame, VNET_CRYPTO_OP_TYPE_DECRYPT, 8);
}
static_always_inline int
cryptodev_raw_enq_aead_aad_12_dec (vlib_main_t *vm,
				   vnet_crypto_async_frame_t *frame)
{
  return cryptodev_raw_aead_enqueue (vm, frame, VNET_CRYPTO_OP_TYPE_DECRYPT, 12);
}
...
static_always_inline int
cryptodev_raw_enq_hmac (vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
  return cryptodev_frame_linked_algs_enqueue (vm, frame, VNET_CRYPTO_OP_TYPE_HMAC);
}

static vnet_crypto_frame_enq_fn_t *
cryptodev_raw_get_enq_fn (const vnet_crypto_alg_data_t *ad, vnet_crypto_op_type_t op_type)
{
  ...
  if (ad->alg_type == VNET_CRYPTO_ALG_T_AEAD)
	...
	{
	case 0:
	  return is_enc ? cryptodev_raw_enq_aead_aad_0_enc : cryptodev_raw_enq_aead_aad_0_dec;
	case 8:
	  return is_enc ? cryptodev_raw_enq_aead_aad_8_enc : cryptodev_raw_enq_aead_aad_8_dec;
	case 12:
	  return is_enc ? cryptodev_raw_enq_aead_aad_12_enc : cryptodev_raw_enq_aead_aad_12_dec;
	default:
	  return 0;
	}
    }
...
cryptodev_register_raw_hdl (vlib_main_t *vm, u32 eidx)
{
  ...
  ref_cnt = cryptodev_register_async_algs (vm, eidx, cryptodev_raw_get_enq_fn, "raw");
