			...
			u32 *max_end)
{
  ...
  u32 crypto_end = fe->cipher_data_start_off + fe->cipher_data_len;
  u32 integ_end = fe->auth_data_start_off + fe->auth_data_len;

  *min_ofs = clib_min (fe->cipher_data_start_off, fe->auth_data_start_off);
  *max_end = clib_max (crypto_end, integ_end);

  ofs.ofs.cipher.head = fe->cipher_data_start_off - *min_ofs;
  ofs.ofs.cipher.tail = *max_end - crypto_end;
  ofs.ofs.auth.head = fe->auth_data_start_off - *min_ofs;
  ...
  while (n_elts)
    {
      ...
      u8 *iv = vnet_crypto_async_get_data_ptr (vm, b[0], fe->iv_offset);
      u8 *tag = vnet_crypto_async_get_data_ptr (vm, b[0], fe->icv_offset);
      ...
      if (PREDICT_FALSE (last_key_index != key_index))
	{
	  ...
	  if (PREDICT_FALSE (key_sess == 0))
	    {
	      status = cryptodev_session_create (vm, key_index, 0, fe->icv_len);
  ...
  while (n_elts)
    {
      ...
      u8 *iv = vnet_crypto_async_get_data_ptr (vm, b[0], fe->iv_offset);
      u8 *tag = vnet_crypto_async_get_data_ptr (vm, b[0], fe->icv_offset);
      u8 *aad = vnet_crypto_async_get_data_ptr (vm, b[0], fe->aad_offset);
      ...
      if (PREDICT_FALSE (last_key_index != key_index))
	{
	  ...
	  if (PREDICT_FALSE (cryptodev_session_get (key, vm->numa_node, op_type) == 0))
	    {
	      status = cryptodev_session_create (vm, key_index, aad_len, fe->icv_len);
	      if (PREDICT_FALSE (status < 0))
		goto error_exit;
	    }
	...
	*(u64 *) (cet->aad_buf + aad_offset) = *(u64 *) aad;
      ...
      if (is_gmac)
	{
	  ...
	  first_len =
	    clib_min ((u32) (b[0]->current_data + b[0]->current_length - fe->cipher_data_start_off),
		      fe->cipher_data_len);
	  vec[1].base = (void *) (b[0]->data + fe->cipher_data_start_off);
	  if (cmt->iova_mode == RTE_IOVA_VA)
	    vec[1].iova = pointer_to_uword (vec[1].base);
	  else
	    vec[1].iova = vlib_buffer_get_pa (vm, b[0]) + fe->cipher_data_start_off;
	  vec[1].len = first_len;
	  n_seg = 2;
	  size = fe->cipher_data_len - first_len;
      ...
      else if (PREDICT_FALSE (fe->cipher_data_len == 0))
	{
	  vec[0].base = (void *) (b[0]->data + fe->cipher_data_start_off);
	  if (cmt->iova_mode == RTE_IOVA_VA)
	    vec[0].iova = pointer_to_uword (vec[0].base);
	  else
	    vec[0].iova = vlib_buffer_get_pa (vm, b[0]) + fe->cipher_data_start_off;
	  vec[0].len = 1;
	  cofs.ofs.auth.tail = 1;
	  cofs.ofs.cipher.tail = 1;
	}
      else if (PREDICT_FALSE (fe->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS))
	{
	  vec[0].base = (void *) (b[0]->data + fe->cipher_data_start_off);
	  vec[0].len = fe->cipher_data_len;
	  if (cmt->iova_mode == RTE_IOVA_VA)
	    vec[0].iova = pointer_to_uword (vec[0].base);
	  else
	    vec[0].iova = vlib_buffer_get_pa (vm, b[0]) + fe->cipher_data_start_off;
