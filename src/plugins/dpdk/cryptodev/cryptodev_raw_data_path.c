...
cryptodev_raw_dequeue (vlib_main_t *vm, vnet_crypto_frame_deq_fn_args_t *a)
{
  cryptodev_main_t *cmt = &cryptodev_main;
  ...
  while (cet->inflight > 0 && dequeue_more)
    dequeue_more = cryptodev_raw_dequeue_internal (vm, a->enqueue_thread_idx);
  ...
  if (PREDICT_TRUE (ring_elt->f != 0) &&
    ...
    {
      *a->nb_elts_processed = ring_elt->n_elts;
...
#define _(a, b, c, d, e, f, g)                                                \
  ...
  if (cryptodev_check_cap_support (&cap_aead_idx, g, e, f))                   \
	...
	vm, &(vnet_crypto_register_enqueue_handler_args_t){                   \
	      .engine_index = eidx,                                           \
	      .op_id = VNET_CRYPTO_OP_##a##_TAG##e##_AAD##f##_ENC,            \
	      .enq_fn = cryptodev_raw_enq_aead_aad_##f##_enc });              \
      vnet_crypto_register_enqueue_handler (                                  \
	vm, &(vnet_crypto_register_enqueue_handler_args_t){                   \
	      .engine_index = eidx,                                           \
	      .op_id = VNET_CRYPTO_OP_##a##_TAG##e##_AAD##f##_DEC,            \
	      .enq_fn = cryptodev_raw_enq_aead_aad_##f##_dec });              \
      ref_cnt++;                                                              \
    }
...
#define _(a, b, c, d, e)                                                      \
  ...
  if (cryptodev_check_cap_support (&cap_cipher_idx, c, -1, -1) &&             \
	...
	vm, &(vnet_crypto_register_enqueue_handler_args_t){                   \
	      .engine_index = eidx,                                           \
	      .op_id = VNET_CRYPTO_OP_##a##_##d##_TAG##e##_ENC,               \
	      .enq_fn = cryptodev_raw_enq_linked_alg_enc });                  \
      vnet_crypto_register_enqueue_handler (                                  \
	vm, &(vnet_crypto_register_enqueue_handler_args_t){                   \
	      .engine_index = eidx,                                           \
	      .op_id = VNET_CRYPTO_OP_##a##_##d##_TAG##e##_DEC,               \
	      .enq_fn = cryptodev_raw_enq_linked_alg_dec });                  \
      ref_cnt++;                                                              \
    }
    foreach_cryptodev_link_async_alg
#undef _

    if (ref_cnt) vnet_crypto_register_dequeue_handler (
      vm, &(vnet_crypto_register_dequeue_handler_args_t){
	    .engine_index = eidx, .deq_fn = cryptodev_raw_dequeue });
