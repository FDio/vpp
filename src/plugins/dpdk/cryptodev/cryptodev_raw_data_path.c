  ...
  while (n_elts)
    {
      ...
      u16 n_seg = 1;
      u32 key_index = fe->ctx->index;
      vnet_crypto_alg_data_t *ad = crypto_main.algs + fe->ctx->alg;
	...
	{
	  cryptodev_key_data_t *key = cryptodev_get_key_data (fe->ctx);
	  union rte_cryptodev_session_ctx sess_ctx;
	  ...
	  if (PREDICT_FALSE (key_sess == 0))
	    {
	      status = cryptodev_session_create (vm, fe->ctx, 0, fe->icv_len);
		...
		goto error_exit;
	      key_sess = cryptodev_session_get (key, vm->numa_node, sess_op_type);
		   ...
		   PREDICT_FALSE (rte_cryptodev_sym_session_opaque_data_get (key_sess) !=
				  (u64) fe->icv_len))
	    {
	      cryptodev_sess_handler (vm, fe->ctx);
	      status = cryptodev_session_create (vm, fe->ctx, 0, fe->icv_len);
	      if (PREDICT_FALSE (status < 0))
		goto error_exit;
	      key_sess = cryptodev_session_get (key, vm->numa_node, sess_op_type);
  ...
  while (n_elts)
      ...
      if (PREDICT_FALSE (last_key_index != key_index))
	{
	  ...
	  if (PREDICT_FALSE (cryptodev_session_get (key, vm->numa_node, op_type) == 0))
	    {
	      status = cryptodev_session_create (vm, fe->ctx, aad_len, fe->icv_len);
	      if (PREDICT_FALSE (status < 0))
		goto error_exit;
	    }
