...
dpdk_port_setup_hqos (dpdk_device_t * xd, dpdk_device_config_hqos_t * hqos)
{
  ...
  for (i = 0; i < worker_thread_count + 1; i++)
	...
	tid = i;
      ...
      xd->hqos_wt[tid].n_subports_per_port = hqos->port.n_subports_per_port;
      xd->hqos_wt[tid].n_pipes_per_subport = hqos->port.n_pipes_per_subport;
      xd->hqos_wt[tid].n_pipes_per_subport_log2 =
	 __builtin_ctz(hqos->port.n_pipes_per_subport);
...
dpdk_hqos_qindex(dpdk_device_hqos_per_worker_thread_t * hqos,
			u32 subport, u32 pipe,
			u32 traffic_class, u32 queue)
{
  return ((subport & (hqos->n_subports_per_port - 1)) <<
			 (hqos->n_pipes_per_subport_log2 + 4)) |
			 ((pipe & (hqos->n_pipes_per_subport - 1)) << 4) |
			 ((traffic_class &
...
static_always_inline void
dpdk_hqos_pkt_write(dpdk_device_hqos_per_worker_thread_t * hqos,
			struct rte_mbuf *pkt, u32 subport, u32 pipe,
			u32 traffic_class, u32 queue, u8 color)
{
  u32 queue_id = dpdk_hqos_qindex(hqos, subport, pipe,
			 traffic_class, queue);

  rte_mbuf_sched_set(pkt, queue_id, traffic_class, color);
}
  ...
  for (i = 0; i < (n_pkts & (~0x3)); i += 4)
    {
      ...
      dpdk_hqos_pkt_write(hqos, pkt0, pkt0_subport, pkt0_pipe,
			 pkt0_tc, pkt0_tc_q, 0);
      dpdk_hqos_pkt_write(hqos, pkt1, pkt1_subport, pkt1_pipe,
			 pkt1_tc, pkt1_tc_q, 0);
      dpdk_hqos_pkt_write(hqos, pkt2, pkt2_subport, pkt2_pipe,
			 pkt2_tc, pkt2_tc_q, 0);
      dpdk_hqos_pkt_write(hqos, pkt3, pkt3_subport, pkt3_pipe,
			 pkt3_tc, pkt3_tc_q, 0);
    }
  ...
  for (; i < n_pkts; i++)
    {
      ...
      dpdk_hqos_pkt_write(hqos, pkt, pkt_subport, pkt_pipe,
			 pkt_tc, pkt_tc_q, 0);
    }
}
