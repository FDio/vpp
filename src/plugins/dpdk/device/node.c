...
{
  b0->flags |= VLIB_BUFFER_FLOW_MARK;
  u32 mark0 = mb0->hash.fdir.hi;
  vnet_buffer2 (b0)->flow_id = hw_flows->flows[mark0].flow_id;
  return hw_flows->flows[mark0].next_index;
}
		   ...
		   u16 flags, u32 n_trace)
{
  ...
  while (n_buffers > 0)
      ...
      while (n_buffers >= 12 && n_left_to_next >= 4)
	{
	  ...
	  else if (flags & DPDK_DEVICE_FLAG_FLOW && or_ol_flags & PKT_RX_FDIR)
	    {
	      if (mb0->ol_flags & PKT_RX_FDIR)
		next0 = dpdk_rx_next_from_flow (mb0, b0, hw_flows);
	      else
		next0 = dpdk_rx_next_from_etype (mb0);
	      if (mb1->ol_flags & PKT_RX_FDIR)
		next1 = dpdk_rx_next_from_flow (mb1, b1, hw_flows);
	      else
		next1 = dpdk_rx_next_from_etype (mb1);
	      if (mb2->ol_flags & PKT_RX_FDIR)
		next2 = dpdk_rx_next_from_flow (mb2, b2, hw_flows);
	      else
		next2 = dpdk_rx_next_from_etype (mb2);
	      if (mb3->ol_flags & PKT_RX_FDIR)
		next3 = dpdk_rx_next_from_flow (mb3, b3, hw_flows);
	      else
