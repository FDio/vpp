...
always_inline u32
dpdk_rx_next (vlib_node_runtime_t * node, vlib_buffer_t * b, u8 flags,
	      u32 next)
{
  ethernet_header_t *h = vlib_buffer_get_current (b);
			      ...
			      u32 * buffers)
{
  ...
  v = (v - u64x4_splat (bm->buffer_mem_start)) >> 6;

  /* permute 256-bit register so lower u32s of each buffer indice are
		       ...
		       uword n_rx_packets, int maybe_multiseg)
{
  ...
  ptd->n_bytes = n_bytes;
  ptd->or_flags = or_flags;
}

static_always_inline void
dpdk_find_nexts (vlib_main_t * vm, vlib_node_runtime_t * node,
		 ...
		 u32 next_index)
{
  ...
  while (n_left >= 12)
    {
      ...
      b4[0] = vlib_buffer_from_rte_mbuf (mb[0]);
      b4[1] = vlib_buffer_from_rte_mbuf (mb[1]);
      b4[2] = vlib_buffer_from_rte_mbuf (mb[2]);
      b4[3] = vlib_buffer_from_rte_mbuf (mb[3]);
      next[0] = dpdk_rx_next (node, b4[0], flags[0], next_index);
      next[1] = dpdk_rx_next (node, b4[1], flags[1], next_index);
      next[2] = dpdk_rx_next (node, b4[2], flags[2], next_index);
      next[3] = dpdk_rx_next (node, b4[3], flags[3], next_index);
      adv4[0] = device_input_next_node_advance[next[0]];
      adv4[1] = device_input_next_node_advance[next[1]];
      adv4[2] = device_input_next_node_advance[next[2]];
      adv4[3] = device_input_next_node_advance[next[3]];
      b4[0]->current_data += adv4[0];
      b4[1]->current_data += adv4[1];
      b4[2]->current_data += adv4[2];
      b4[3]->current_data += adv4[3];
      b4[0]->current_length -= adv4[0];
      b4[1]->current_length -= adv4[1];
      b4[2]->current_length -= adv4[2];
      b4[3]->current_length -= adv4[3];
  ...
  while (n_left)
    {
      i = 0;
      b = vlib_buffer_from_rte_mbuf (mb[i]);
      next[i] = dpdk_rx_next (node, b, flags[i], next_index);
      adv = device_input_next_node_advance[next[i]];
      b->current_data += adv;
      b->current_length -= adv;
		   ...
		   vlib_node_runtime_t * node, u32 thread_index, u16 queue_id)
{
  ...
  if (xd->flags & DPDK_DEVICE_FLAG_MAYBE_MULTISEG)
    dpdk_process_rx_burst (vm, ptd, n_rx_packets, 1);
  else
    dpdk_process_rx_burst (vm, ptd, n_rx_packets, 0);


  dpdk_find_nexts (vm, node, ptd, n_rx_packets, next_index);
...
uword __attribute__ ((optimize ("O3")))
