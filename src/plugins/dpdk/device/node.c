...
dpdk_device_input (dpdk_main_t * dm, dpdk_device_t * xd,
		   ...
		   int maybe_multiseg, int collect_detailed_stats)
{
  u32 n_buffers;
...
#define inc_counter(ctype, rx_tx)\
  ...
  vlib_increment_combined_counter\
    (vnet_get_main ()->interface_main.combined_sw_if_counters\
     + ctype,\
     thread_index, xd->vlib_sw_if_index, stats_n_packets[ctype], stats_n_bytes[ctype]);\
  }
  if (collect_detailed_stats)
    {
      ...
      foreach_device_and_queue (dq, rt->devices_and_queues)
      {
        ...
        if (xd->flags & DPDK_DEVICE_FLAG_MAYBE_MULTISEG)
          n_rx_packets += dpdk_device_input (
              dm, xd, node, thread_index, dq->queue_id, /* maybe_multiseg */ 1,
              COLLECT_DETAILED_STATS);
        else
          n_rx_packets += dpdk_device_input (
              dm, xd, node, thread_index, dq->queue_id, /* maybe_multiseg */ 0,
              COLLECT_DETAILED_STATS);
      }
    }
  else
