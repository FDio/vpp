  ...
  for (i = 0; i < count; i++)
    {
      ...
      /* the magic num 27 is the bit number difference between PKT_RX_L4_CKSUM_BAD
         and VNET_BUFFER_F_L4_CHECKSUM_CORRECT */
      b[i]->flags ^= (mb[i]->ol_flags & PKT_RX_L4_CKSUM_BAD) << 27;
    }
  return rv;
}
