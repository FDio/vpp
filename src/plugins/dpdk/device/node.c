	  ...
	  if (txd)
	    {
	      ...
	      do
		{
		  if (rte_eth_tx_descriptor_status (tx_port_id, tx_queue_id,
						    num_desc) ==
	      ...
	      // Now put into buffer opaque2, 32-bit = [ring_size][descriptors
	      // available]
	      u32 *opaque =
		&vnet_buffer2 (b)
		   ->unused[7]; // 8, 32-bit words in opaque2, use last
	      *opaque = clib_host_to_net_u32 (((u32) ring_size << 16) |
					      tx_ring_desc_avai);
	    }
	}
    }
