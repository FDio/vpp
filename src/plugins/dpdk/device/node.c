...
static_always_inline void
dpdk_cksum (vlib_node_runtime_t * node, struct rte_mbuf *mb,
	    vlib_buffer_t * b, u16 * next)
{
  if ((mb->ol_flags & PKT_RX_IP_CKSUM_BAD) == 0)
    return;
  ...
  while (i + 8 < n_rx_packets)
    {
      ...
      next[0] = use_etype ? dpdk_rx_next_from_etype (mb[0]) : next_index;
      next[1] = use_etype ? dpdk_rx_next_from_etype (mb[1]) : next_index;
      next[2] = use_etype ? dpdk_rx_next_from_etype (mb[2]) : next_index;
      next[3] = use_etype ? dpdk_rx_next_from_etype (mb[3]) : next_index;

      adv0 = use_etype ? device_input_next_node_advance[next[0]] : 0;
      adv1 = use_etype ? device_input_next_node_advance[next[1]] : 0;
      adv2 = use_etype ? device_input_next_node_advance[next[2]] : 0;
      adv3 = use_etype ? device_input_next_node_advance[next[3]] : 0;

      off0 = mb[0]->data_off - RTE_PKTMBUF_HEADROOM;
      off1 = mb[1]->data_off - RTE_PKTMBUF_HEADROOM;
      off2 = mb[2]->data_off - RTE_PKTMBUF_HEADROOM;
      off3 = mb[3]->data_off - RTE_PKTMBUF_HEADROOM;

      dpdk_prefetch_buffer (mb[6]);
      dpdk_prefetch_buffer (mb[7]);

      vnet_buffer (b0)->l2_hdr_offset = off0;
      vnet_buffer (b1)->l2_hdr_offset = off1;
      vnet_buffer (b2)->l2_hdr_offset = off2;
      vnet_buffer (b3)->l2_hdr_offset = off3;

      b0->current_data = off0 + adv0;
      b1->current_data = off1 + adv1;
      b2->current_data = off2 + adv2;
      b3->current_data = off3 + adv3;
	...
	{
	  n_bytes += dpdk_process_subseq_segs (vm, b0, mb[0], fl);
	  n_bytes += dpdk_process_subseq_segs (vm, b1, mb[1], fl);
	  n_bytes += dpdk_process_subseq_segs (vm, b2, mb[2], fl);
	  n_bytes += dpdk_process_subseq_segs (vm, b3, mb[3], fl);
	}
	...
	{
	  dpdk_cksum (node, mb[0], b0, next);
	  dpdk_cksum (node, mb[1], b1, next + 1);
	  dpdk_cksum (node, mb[2], b2, next + 2);
	  dpdk_cksum (node, mb[3], b3, next + 3);
	}
