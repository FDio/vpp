...
enum
{
  DPDK_RX_F_CKSUM_GOOD = 0,
  DPDK_RX_F_CKSUM_BAD = 1,
};

always_inline u32
dpdk_rx_next (vlib_node_runtime_t * node, vlib_buffer_t * b, u8 flags,
	      int use_etype, int maybe_bad, u32 next)
{
  ethernet_header_t *h = vlib_buffer_get_current (b);

  if (maybe_bad && (flags & (1 << DPDK_RX_F_CKSUM_BAD)))
    {
			      ...
			      u32 * buffers)
{
  ...
  *(u32x4 *) buffers = u32x8_extract_lo (v2);
#else
...
dpdk_ol_flags_extract (struct rte_mbuf **mb, u8 * flags, int count)
{
  ...
  for (i = 0; i < count; i++)
    {
      u8 bad = (mb[i]->ol_flags & PKT_RX_IP_CKSUM_BAD) != 0;
      u8 good = (mb[i]->ol_flags & PKT_RX_IP_CKSUM_GOOD) != 0;
      flags[i] = (bad << DPDK_RX_F_CKSUM_BAD | good << DPDK_RX_F_CKSUM_GOOD);
      rv |= flags[i];
    }
  return rv;
}
...
static_always_inline void
dpdk_dispatch_rx_burst (vlib_main_t * vm, vlib_node_runtime_t * node,
			dpdk_per_thread_data_t * ptd, uword n_rx_packets,
			u32 next_index, int maybe_cksum_bad)
{
  u32 n_left = n_rx_packets;
  u32 *buffers;
  struct rte_mbuf **mb;
  ...
  while (n_left)
    {
      ...
      while (n_left >= 12 && n_left_to_next >= 4)
	{
	  ...
	  for (i = 0; i < 4; i++)
	    {
	      ...
	      next[i] = dpdk_rx_next (node, b, flags[i], use_etype,
				      maybe_cksum_bad, next_index);
	      adv = use_etype ? device_input_next_node_advance[next[i]] : 0;
	      b->current_data += adv;
		   ...
		   vlib_node_runtime_t * node, u32 thread_index, u16 queue_id)
{
  ...
  if (ptd->or_flags & (1 << DPDK_RX_F_CKSUM_BAD))
    dpdk_dispatch_rx_burst (vm, node, ptd, n_rx_packets, next_index, 0);
  else
    dpdk_dispatch_rx_burst (vm, node, ptd, n_rx_packets, next_index, 1);
