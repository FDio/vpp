...
dpdk_insert_tx_ring_stats(vlib_buffer_t *b, dpdk_main_t * dm)
{
  ...
  if(adj)
  {
    ...
    if(hw)
    {
      dpdk_device_t *txd = vec_elt_at_index (dm->devices, hw->dev_instance);
      if(txd)
      {
        ...
        do
        {
          if (rte_eth_tx_descriptor_status(tx_port_id, tx_queue_id, num_desc) == RTE_ETH_TX_DESC_DONE)
          {
            tx_ring_desc_avai++;
          }
        ...
        *opaque = clib_host_to_net_u32(((u32)ring_size << 16) | tx_ring_desc_avai);
      }
    }
  } 
}
