...
always_inline u32
dpdk_rx_next (vlib_node_runtime_t * node, u16 etype, u8 flags, u32 next)
{
  ...
  while (n_left >= 12)
    {
      ...
      next[1] = *(u16 *) ((u8 *) mb[1] + next[1] + sizeof (vlib_buffer_t));
      next[2] = *(u16 *) ((u8 *) mb[2] + next[2] + sizeof (vlib_buffer_t));
      next[3] = *(u16 *) ((u8 *) mb[3] + next[3] + sizeof (vlib_buffer_t));
      next[0] = dpdk_rx_next (node, next[0], flags[0], next_index);
      next[1] = dpdk_rx_next (node, next[1], flags[1], next_index);
      next[2] = dpdk_rx_next (node, next[2], flags[2], next_index);
      next[3] = dpdk_rx_next (node, next[3], flags[3], next_index);
  ...
  while (n_left)
    {
      ...
      next[0] = *(u16 *) ((u8 *) mb[0] + next[0] + sizeof (vlib_buffer_t));
      next[0] = dpdk_rx_next (node, next[0], flags[0], next_index);
      adv = device_input_next_node_advance[next[0]];
      b->current_data += adv;
