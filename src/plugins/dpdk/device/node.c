		   ...
		   u16 queue_id)
{
  ...
  while (n_rx_packets < DPDK_RX_BURST_SZ)
    {
      u32 vacant = DPDK_RX_BURST_SZ - n_rx_packets;
      u32 vacant_up_to_32 = clib_min (vacant, 32);

      newly_read = rte_eth_rx_burst (
	xd->port_id, queue_id, ptd->mbufs + n_rx_packets, vacant_up_to_32);
      n_rx_packets += newly_read;

      if (newly_read < vacant_up_to_32)
	break;
    }
