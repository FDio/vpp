...
dpdk_get_xstats (dpdk_device_t * xd)
{
  if (!(xd->flags & DPDK_DEVICE_FLAG_ADMIN_UP))
    return;
  int len;
  if ((len = rte_eth_xstats_get (xd->device_dpdk_port_index, NULL, 0)) > 0)
    {
      vec_validate (xd->xstats, len - 1);
      vec_validate (xd->last_cleared_xstats, len - 1);

      len =
	rte_eth_xstats_get (xd->device_dpdk_port_index, xd->xstats,
			    vec_len (xd->xstats));
...
dpdk_update_counters (dpdk_device_t * xd, f64 now)
{
  ...
  rte_eth_stats_get (xd->device_dpdk_port_index, &xd->stats);
