...
u8 *
format_sfp_eeprom_connector_type (u8 *s, va_list *args)
{
...
u8 *
format_sfp_eeprom_encoding_type (u8 *s, va_list *args)
{
  ...
  if (!is_terse)
    {
      ...
      vlib_cli_output (vm, "    Connector: 0x%02x (%U)", se->connector_type,
		       format_sfp_eeprom_connector_type, se->connector_type);
      vlib_cli_output (vm, "    Encoding: 0x%02x (%U)", se->encoding,
		       format_sfp_eeprom_encoding_type, se->encoding);
