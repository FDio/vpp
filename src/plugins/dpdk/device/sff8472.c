...
static const char *
sff8472_transceiver_type (u8 id)
{
  switch (id)
    {
    case 0x01:
      return "GBIC";
    case 0x02:
      return "Module/connector soldered to motherboard";
    case 0x03:
...
sff8472_decode_sfp_eeprom (vlib_main_t *vm, sfp_eeprom_t *se, u8 is_terse)
{
  ...
  if (!is_terse)
    {
      vlib_cli_output (vm, "    Identifier: 0x%02x (%s)", se->id,
		       sff8472_transceiver_type (se->id));
...
void
sff8472_decode_module (vlib_main_t *vm, u8 *eeprom_data, u32 length,
		       u8 is_terse)
{
  sff8472_base_id_t *base_id;
  u8 vendor_name[17] = { 0 };
  u8 vendor_pn[17] = { 0 };
  u8 vendor_rev[5] = { 0 };
  u8 vendor_sn[17] = { 0 };
  ...
  if (length < sizeof (sff8472_base_id_t))
    {
      vlib_cli_output (
	vm,
	"  Warning: EEPROM data too short for complete SFF-8472 A0 parsing");
      return;
    }

  base_id = (sff8472_base_id_t *) eeprom_data;

  vlib_cli_output (vm, "  SFF-8472 Module Information:");
  if (!is_terse)
    {
      vlib_cli_output (vm, "    Identifier: 0x%02x (%s)", base_id->identifier,
		       sff8472_transceiver_type (base_id->identifier));
      vlib_cli_output (vm, "    Extended Identifier: 0x%02x",
		       base_id->ext_identifier);
      vlib_cli_output (vm, "    Connector: 0x%02x (%s)", base_id->connector,
		       sff8472_connector_type (base_id->connector));
      vlib_cli_output (vm, "    Encoding: 0x%02x (%s)", base_id->encoding,
		       sff8472_encoding_type (base_id->encoding));
      vlib_cli_output (vm, "    Nominal Bit Rate: %u00 Mbps",
		       base_id->br_nominal);

      /* Length information */
      if (base_id->length_smf_km)
	vlib_cli_output (vm, "    Length (SMF): %u km",
			 base_id->length_smf_km);
      if (base_id->length_smf)
	vlib_cli_output (vm, "    Length (SMF): %u00 m", base_id->length_smf);
      if (base_id->length_50um)
	vlib_cli_output (vm, "    Length (OM2 50um): %u0 m",
			 base_id->length_50um);
      if (base_id->length_625um)
	vlib_cli_output (vm, "    Length (OM1 62.5um): %u0 m",
			 base_id->length_625um);
      if (base_id->length_copper)
	vlib_cli_output (vm, "    Length (Copper): %u m",
			 base_id->length_copper);
      if (base_id->length_om3)
	vlib_cli_output (vm, "    Length (OM3 50um): %u0 m",
			 base_id->length_om3);
    }

  /* Vendor information */
  clib_memcpy (vendor_name, base_id->vendor_name, 16);
  /* Trim trailing spaces */
  for (int i = 15; i >= 0 && vendor_name[i] == ' '; i--)
    vendor_name[i] = '\0';
  vlib_cli_output (vm, "    Vendor Name: %s", vendor_name);

  vlib_cli_output (vm, "    Vendor OUI: %02x:%02x:%02x",
		   base_id->vendor_oui[0], base_id->vendor_oui[1],
		   base_id->vendor_oui[2]);

  clib_memcpy (vendor_pn, base_id->vendor_pn, 16);
  /* Trim trailing spaces */
  for (int i = 15; i >= 0 && vendor_pn[i] == ' '; i--)
    vendor_pn[i] = '\0';
  vlib_cli_output (vm, "    Vendor Part Number: %s", vendor_pn);

  clib_memcpy (vendor_sn, base_id->vendor_sn, 16);
  /* Trim trailing spaces */
  for (int i = 15; i >= 0 && vendor_sn[i] == ' '; i--)
    vendor_sn[i] = '\0';
  vlib_cli_output (vm, "    Vendor Serial Number: %s", vendor_sn);
  ...
  if (!is_terse)
    {
      clib_memcpy (vendor_rev, base_id->vendor_rev, 4);
      /* Trim trailing spaces */
	...
	vendor_rev[i] = '\0';
      ...
      /* Wavelength */
      wavelength = sff8472_read_u16 (base_id->wavelength);
      if (wavelength)
	vlib_cli_output (vm, "    Wavelength: %u nm", wavelength);

      clib_memcpy (date_code, base_id->date_code, 8);
      vlib_cli_output (vm, "    Date Code: %.8s", date_code);

      /* Options and compliance */
      vlib_cli_output (vm, "    Diagnostic Monitoring: %s",
		       (base_id->diagnostic_monitoring_type & 0x40) ?
			 "implemented" :
			 "not implemented");
      vlib_cli_output (vm, "    Enhanced Options: 0x%02x",
		       base_id->enhanced_options);
      vlib_cli_output (vm, "    SFF-8472 Compliance: 0x%02x",
		       base_id->sff8472_compliance);
    }
}
