			    ...
			    u8 is_terse)
{
  ...
  temp = sff8472_convert_temperature (&eeprom_data_a2[96]);
  vcc = sff8472_convert_voltage (&eeprom_data_a2[98]);
  tx_bias = sff8472_convert_current (&eeprom_data_a2[100]);
  tx_power = sff8472_convert_power (&eeprom_data_a2[102]);
  rx_power = sff8472_convert_power (&eeprom_data_a2[104]);
  ...
  if (length >= 256 + sizeof (sff8472_diag_t))
    {
