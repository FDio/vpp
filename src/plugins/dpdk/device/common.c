...
dpdk_device_setup (dpdk_device_t * xd)
{
  ...
  /* If explicit l4 checksum rx offload is enabled flags should be read
   * from the buffers. Currently not supported */
  if (xd->port_conf.rxmode.offloads &
      (DEV_RX_OFFLOAD_TCP_CKSUM | DEV_RX_OFFLOAD_UDP_CKSUM))
    xd->buffer_flags &= ~(VNET_BUFFER_F_L4_CHECKSUM_COMPUTED |
