...
dpdk_device_configure_async_flow_offload (dpdk_device_t *xd)
{
  ...
  if (rv == -ENOTSUP)
    ...
    {
      dpdk_device_flow_warning (xd, "rte_flow_info_get", rv);
  ...
  if (rv == -ENOTSUP)
    ...
    {
      dpdk_device_flow_warning (xd, "rte_flow_configure", rv);
  ...
  /* Async batch sizing. The sync path already has its own flow_cache_size
     initialized in dpdk_device_setup; bump it if async offload requires
     a larger batch. */
  if (xd->flow_cache_size < 2 * xd->async_flow_offload_queue_size)
