...
dpdk_lib_init (dpdk_main_t * dm)
{
  ...
  for (i = 0; i < nports; i++)
	...
	{
	  xd->rx_q_used =
	    clib_min (devconf->num_rx_queues, dev_info.max_rx_queues);
...
#undef _
      ...
      if (devconf->vxlan_offload_enabled)
	{
	  ...
	  xd->offload_next_index =
	    vlib_node_add_next (vm, dpdk_input_node.index,
				dpdk_vxlan_passthru_input_node.index);
	  ...
	  clib_bitmap_foreach (i, devconf->vxlan_offload.workers,
            ({
              vnet_queue_assign_rx_thread (dm->vnet_main, node_idx, xd->hw_if_index,
	  ...
	  for (; q < n_rx_queues; q++)
	    {
	      vnet_queue_assign_rx_thread (dm->vnet_main, node_idx,
					   xd->hw_if_index, q,
					   vnet_dev_next_worker_thread_index
					   (&vnet_device_main));
	    }
	}

      int n_rx_queues = xd->rx_q_used + xd->vxlan_rx.queues.count;
      vec_validate_aligned (xd->rx_vectors, n_rx_queues,
			    CLIB_CACHE_LINE_BYTES);
		    ...
		    unformat_input_t * input, u8 is_default)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	if (unformat
	    (input, "vxlan-offload %U", unformat_vlib_cli_sub_input,
	     &sub_input))
	{
