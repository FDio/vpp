...
dpdk_lib_init (dpdk_main_t * dm)
{
  ...
  if (vmbus_dev)
  {
    ...
    if (unformat
      ...
      {
        p = hash_get (dm->conf->device_config_index_by_vmbus_addr, vmbus_addr.as_u32[0]);
      }
  }
  ...
  vec_foreach (addr, addrs)
    {
      ...
      if (num_whitelisted)
      {
      uword * p = hash_get (conf->device_config_index_by_vmbus_addr, addr->as_u32[0]);
      if (!p) {
            /* No devices blacklisted, but have whitelisted. blacklist all non-whitelisted */
            pool_get (conf->dev_confs, devconf);
            hash_set (conf->device_config_index_by_vmbus_addr, addr->as_u32[0],
                      devconf - conf->dev_confs);
