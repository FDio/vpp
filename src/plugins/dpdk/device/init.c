...
dpdk_counters_xstats_init (dpdk_device_t *xd)
{
  ...
  xd->xstats_counters.stat_segment_name = (char *) format (
    0, "/if/xstats/%U/%s%c", format_vnet_sw_if_index_name, xd->sw_if_index, 0);
  ...
  if (ret >= 0 && ret <= len)
      ...
      vec_foreach_index (i, xstats_names)
	{
	  ...
	  xd->xstats_symlinks[i] = vlib_stats_add_symlink (
	    ...
	    format_vnet_sw_if_index_name, xd->sw_if_index,
	  ...
	  if (xd->xstats_symlinks[i] == STAT_SEGMENT_INDEX_INVALID)
		...
		xd->sw_if_index, xstats_names[i].name, i, 0);
	    }
	}
    }
