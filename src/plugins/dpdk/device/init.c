...
/* The function check_l3cache helps check if Level 3 cache exists or not on
  current CPUs return value 1: exist. return value 0: not exist.
*/
static int
check_l3cache ()
{
...
dpdk_enable_l4_csum_offload (dpdk_device_t *xd)
{
  ...
  xd->flags |= DPDK_DEVICE_FLAG_TX_OFFLOAD | DPDK_DEVICE_FLAG_INTEL_PHDR_CKSUM;
}
  ...
  RTE_ETH_FOREACH_DEV (i)
  {
	...
	if (dev_info.switch_info.port_id != (uint16_t) -1)
	  xd->interface_name_suffix =
	    format (0, "%d", dev_info.switch_info.port_id);
      }
    /* Handle interface naming for devices with multiple ports sharing same PCI
     * ID */
    else if (pci_dev &&
...
#define _(s, f)                                                               \
  else if (dev_info.driver_name && !strcmp (dev_info.driver_name, s))         \
...
#undef _
	...
	if (devconf->num_rx_desc)
	...
	else
	  {
	    ...
	    /* If num_rx_desc is not specified by VPP user, the current CPU is
	    working with 2M page and has no L3 cache, default num_rx_desc is
	    changed to 512 from original 1024 to help reduce TLB misses.
	    */
	...
	if (devconf->num_tx_desc)
	...
	else
	  {
	    ...
	    /* If num_tx_desc is not specified by VPP user, the current CPU is
	    working with 2M page and has no L3 cache, default num_tx_desc is
	    changed to 512 from original 1024 to help reduce TLB misses.
	    */
	    if ((clib_mem_get_default_hugepage_size () == 2 << 20) &&
		check_l3cache () == 0)
	...
	if (dev_info.max_rx_pktlen >=
	    (ETHERNET_MAX_PACKET_BYTES + sizeof (ethernet_header_t)))
	  {
		...
		(dev_info.max_rx_pktlen >=
		 (ETHERNET_MAX_PACKET_BYTES + sizeof (ethernet_header_t) + 4)))
	      {
		max_rx_frame += 4;
	      }
	  }
    ...
    if (vec_len (xd->errors))
      dpdk_log_err ("setup failed for device %U. Errors:\n  %U",
		    format_dpdk_device_name, i, format_dpdk_device_errors, xd);
...
dpdk_bind_devices_to_uio (dpdk_config_main_t *conf)
{
  ...
  vec_foreach (addr, addrs)
      ...
      if (d)
	{
      ...
      if (error)
	{
	  vlib_log_warn (dpdk_main.log_default, "%U", format_clib_error,
			 error);
	...
	{
	  uword *p =
	    hash_get (conf->device_config_index_by_pci_addr, addr->as_u32);
      ...
      for (i = 0; i < vec_len (conf->blacklist_by_pci_vendor_and_device); i++)
	{
	  ...
	  device =
	    (u16) (conf->blacklist_by_pci_vendor_and_device[i] & 0xFFFF);
	  if (d->vendor_id == vendor && d->device_id == device)
	      ...
	      if (devconf == 0)
		{
		  ...
		  hash_set (conf->device_config_index_by_pci_addr,
			    addr->as_u32, devconf - conf->dev_confs);
	...
	{
	  /*
	   * For vmxnet3 PCI, unless it is explicitly specified in the
	   * whitelist, the default is to put it in the blacklist.
	   */
	  if (devconf == 0)
	    {
	      pool_get (conf->dev_confs, devconf);
	...
	;
      ...
      else if (d->vendor_id == 0x1d0f && d->device_id >= 0xec20 &&
	       d->device_id <= 0xec21)
	;
	...
	;
      ...
      else if (d->vendor_id == 0x1077 && d->device_id >= 0x8070 &&
	       d->device_id <= 0x8090)
	;
      /* Mellanox CX3, CX3VF */
      else if (d->vendor_id == 0x15b3 && d->device_id >= 0x1003 &&
	       d->device_id <= 0x1004)
	{
	  continue;
	}
      /* Mellanox CX4, CX4VF, CX4LX, CX4LXVF, CX5, CX5VF, CX5EX, CX5EXVF */
      else if (d->vendor_id == 0x15b3 && d->device_id >= 0x1013 &&
	       d->device_id <= 0x101a)
	{
	  continue;
	}
      /* Mellanox CX6, CX6VF, CX6DX, CX6DXVF */
      else if (d->vendor_id == 0x15b3 && d->device_id >= 0x101b &&
	       d->device_id <= 0x101e)
	{
	  continue;
	}
      /* Broadcom NetXtreme S, and E series only */
	       ...
	       ((d->device_id >= 0x16c0 && d->device_id != 0x16c6 &&
		 d->device_id != 0x16c7 && d->device_id != 0x16dd &&
		 d->device_id != 0x16f7 && d->device_id != 0x16fd &&
		 d->device_id != 0x16fe && d->device_id != 0x170d &&
		 d->device_id != 0x170c && d->device_id != 0x170e &&
		 d->device_id != 0x1712 && d->device_id != 0x1713) ||
      ...
      else
	{
	  dpdk_log_warn ("Unsupported PCI device 0x%04x:0x%04x found "
			 "at PCI address %s\n",
			 (u16) d->vendor_id, (u16) d->device_id, pci_addr);
	  continue;
	}
		    ...
		    u8 is_default)
{
  ...
  else if (devconf->workers && clib_bitmap_count_set_bits (devconf->workers) !=
...
dpdk_config (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
			 ...
			 &vmbus_addr))
	{
      ...
      else if (unformat (input, "blacklist %x:%x:%x.%x", &domain, &bus,
			 &device, &func))
	{
	  tmp = format (0, "-b%c", 0);
	  vec_add1 (conf->eal_init_args, tmp);
...
#define _(a)                                                                  \
  ...
  {                                                                           \
	...
	clib_warning (                                                        \
	  "af_packet obsoleted. Use CLI 'create host-interface'.");           \
...
#define _(x)                                                                  \
  ...
  pool_foreach (devconf, conf->dev_confs)
	...
	if (devconf->devargs)
	  ...
	  {
	    tmp =
	      format (0, "%U%c", format_vlib_pci_addr, &devconf->pci_addr, 0);
	  }
	vec_add1 (conf->eal_init_args, tmp);
      }
...
#undef _
  ...
  for (i = 1; i < vec_len (conf->eal_init_args); i++)
    conf->eal_init_args_str =
      format (conf->eal_init_args_str, "%s ", conf->eal_init_args[i]);
  ...
  if (LINK_STATE_ELOGS)
    {
      vlib_main_t *vm = vlib_get_main ();
      ELOG_TYPE_DECLARE (e) = {
	.format = "update-link-state: sw_if_index %d, admin_up %d,"
	...
	u8 new_link_state;
      ...
      ed->old_link_state =
	(u8) vnet_hw_interface_is_link_up (vnm, xd->hw_if_index);
  ...
  while (1)
    {
      ...
      f64 min_wait = dm->link_state_poll_interval < dm->stat_poll_interval ?
		       dm->link_state_poll_interval :
		       dm->stat_poll_interval;
      ...
      vec_foreach (xd, dm->devices)
	{
	  ...
	  if ((now - xd->time_last_link_update) >=
	      dm->link_state_poll_interval)
	    dpdk_update_link_state (xd, now);
	}
    }
...
dpdk_init (vlib_main_t *vm)
{
  ...
  STATIC_ASSERT (offsetof (dpdk_device_t, cacheline1) == CLIB_CACHE_LINE_BYTES,
		 "Data in cache line 0 is bigger than cache line size");
  STATIC_ASSERT (
    offsetof (frame_queue_trace_t, cacheline0) == 0,
  ...
  STATIC_ASSERT (RTE_CACHE_LINE_SIZE == 1 << CLIB_LOG2_CACHE_LINE_BYTES,
		 "DPDK RTE CACHE LINE SIZE does not match with "
  ...
  dm->log_cryptodev = vlib_log_register_class ("dpdk",