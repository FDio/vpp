			  ...
			  const u8 * address, u8 is_add)
{
   ...
   * Note: the ethdev wrapper reports "mac address not configured" as
   * -EADDRINUSE on the remove path (it never surfaces -ENOENT), so both
   * sides of the tolerance test use EADDRINUSE/EEXIST.
   */
  if (is_add ? (error == -EADDRINUSE || error == -EEXIST) :
	       (error == -EADDRINUSE || error == -ENOENT))
    {
