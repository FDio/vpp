...
static clib_error_t *
dpdk_interface_get_rss_queues (struct vnet_main_t *vnm,
			       ...
			       uword ** bitmap)
{
  ...
  ret =
    rte_eth_dev_rss_reta_query (xd->port_id, reta_conf, dev_info.reta_size);
  if (ret)
    {
  ...
  for (i = 0; i < dev_info.reta_size; i++)
	...
	continue;
      ...
      *bitmap = clib_bitmap_set (*bitmap, reta_conf[idx].reta[shift], 1);
    }
...
dpdk_interface_set_rss_queues (struct vnet_main_t *vnm,
			       struct vnet_hw_interface_t *hi, uword * bitmap)
{
  dpdk_main_t *xm = &dpdk_main;
  u32 hw_if_index = hi->hw_if_index;
  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
...
VNET_DEVICE_CLASS (dpdk_device_class) = {
  ...
  .get_rss_queues_function = dpdk_interface_get_rss_queues,
