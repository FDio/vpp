...
dpdk_report_tx_drop (vlib_main_t *vm, u32 thread_index, u32 if_index,
		     u32 node_index, struct rte_mbuf **mb, u32 n_ok,
		     u32 n_packets)
{
  ...
  rte_pktmbuf_free_bulk (&mb[n_ok], n_dropped);
}
...
/*
 * This function calls the dpdk's tx_prepare function to prepare the packets.
 * It manages a lock per-device if the device does not
 * support multiple queues. It returns the number of packets prepared
 * and validated to be transmitted
 */
static_always_inline u16
tx_prepare_vector_internal (vlib_main_t *vm, dpdk_device_t *xd,
			    ...
			    u8 is_shared)
{
  ...
  n_prep = rte_eth_tx_prepare (xd->port_id, queue_id, mb, n_left);
					     ...
					     vlib_frame_t * f)
{
  ...
  n_prep = tx_prepare_vector_internal (vm, xd, ptd->mbufs, n_packets, queue_id,
				       is_shared);

  {
	...
	dpdk_report_tx_drop (vm, thread_index, xd->sw_if_index,
			     node->node_index, ptd->mbufs, n_prep, n_packets);
      }
  }
...

  {
	...
	tx_pkts -= n_left;
	dpdk_report_tx_drop (vm, thread_index, xd->sw_if_index,
			     node->node_index, ptd->mbufs, n_prep - n_left,
			     n_prep);
      }
  }
