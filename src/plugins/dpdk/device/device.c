...
dpdk_buffer_tx_offload (dpdk_device_t * xd, vlib_buffer_t * b,
			struct rte_mbuf *mb)
{
  u32 ip_cksum = b->flags & VNET_BUFFER_F_MISSING_IP_CKSUM;
  u32 tcp_cksum = b->flags & VNET_BUFFER_F_MISSING_TCP_CKSUM;
  u32 udp_cksum = b->flags & VNET_BUFFER_F_MISSING_UDP_CKSUM;
  ...
  while (n_left >= 8)
      ...
      else
	{
      ...
      if (PREDICT_FALSE ((xd->flags & DPDK_DEVICE_FLAG_TX_OFFLOAD) &&
			 (or_flags & VNET_BUFFER_F_MISSING_TCP_CKSUM)))
	{
