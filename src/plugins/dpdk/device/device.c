				 ...
				 struct vnet_hw_interface_t *hi, u8 *hash_key)
{
  ...
  if (ret != 0)
    return vnet_error (VNET_ERR_UNSUPPORTED,
		       "rte_eth_dev_rss_hash_conf_get err %d", ret);

  old_key_len = rss_conf.rss_key_len;
  rss_conf.rss_key = hash_key;
  rss_conf.rss_key_len = vec_len (hash_key);
  ret = rte_eth_dev_rss_hash_update (xd->port_id, &rss_conf);
  if (ret == -EINVAL)
    {
      return vnet_error (VNET_ERR_KEY_LENGTH,
			 "unsupported RSS key length (expected %d, got %d)",
			 old_key_len, rss_conf.rss_key_len);
    }
  else if (ret == -ENOTSUP)
    {
