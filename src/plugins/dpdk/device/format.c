...
#define foreach_dpdk_pkt_offload_flag                                         \
  foreach_dpdk_pkt_rx_offload_flag foreach_dpdk_pkt_tx_offload_flag
  ...
  if (l->link_status)
    {
      ...
      s = format (s, "%s duplex ",
		  (l->link_duplex == ETH_LINK_FULL_DUPLEX) ? "full" : "half");
      s = format (s, "mtu %d %s\n", hi->max_packet_bytes,
		  promisc ? " promisc" : "");
    }
  else
...
format_dpdk_device_module_info (u8 *s, va_list *args)
{
  ...
  if (rte_eth_dev_get_module_eeprom (xd->port_id, &ei) == 0)
    {
      s = format (s, "%U", format_sfp_eeprom,
		  ei.data + (mi.type == RTE_ETH_MODULE_SFF_8436 ? 0x80 : 0));
    }
  else
...
format_dpdk_device (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U\n%Ucarrier %U", format_dpdk_device_type, dev_instance,
	      format_white_space, indent + 2, format_dpdk_link_status, xd);
  s = format (s, "%Uflags: %U\n", format_white_space, indent + 2,
	      format_dpdk_device_flags, xd);
  ...
  if (xd->flags & DPDK_DEVICE_FLAG_PMD)
      ...
      if (1 < verbose)
	{
      ...
      s = format (s, "%Urx offload avail:  %U\n", format_white_space,
		  indent + 2, format_dpdk_rx_offload_caps, di.rx_offload_capa);
      s =
	format (s, "%Urx offload active: %U\n", format_white_space, indent + 2,
		format_dpdk_rx_offload_caps, xd->port_conf.rxmode.offloads);
      s = format (s, "%Utx offload avail:  %U\n", format_white_space,
		  indent + 2, format_dpdk_tx_offload_caps, di.tx_offload_capa);
      s =
	format (s, "%Utx offload active: %U\n", format_white_space, indent + 2,
...
#define _(N, V)                                                               \
  if (xd->stats.V != 0)                                                       \
    {                                                                         \
      s = format (s, "\n%U%-40U%16Lu", format_white_space, indent + 2,        \
...
#undef _
  ...
  if (ret >= 0 && ret <= len)
      ...
      vec_foreach_index (i, xd->xstats)
	{
	  ...
	  if (verbose == 2 || (verbose && xstat->value))
	    {
	      xs = format (xs, "\n%U%-38s%16Lu", format_white_space,
			   indent + 4, xstat_names[i].name, xstat->value);
	    }
	}
  ...
  if (xs)
    {
      s = format (s, "\n%Uextended stats:%v", format_white_space, indent + 2,
		  xs);
      vec_free (xs);
    }
...
format_dpdk_tx_trace (u8 *s, va_list *va)
{
  ...
  s = format (s, "\n%U%U", format_white_space, indent, format_dpdk_rte_mbuf,
	      &t->mb, &t->data);
...
format_dpdk_rx_trace (u8 *s, va_list *va)
{
  ...
  if (!f)
    f = format_hex_bytes;
  s = format (s, "\n%U%U", format_white_space, indent, f, t->buffer.pre_data,
	      sizeof (t->buffer.pre_data));
...
#define _(F, S)                                                               \
  if (*ol_flags & F)                                                          \
    {                                                                         \
      s = format (s, "\n%U%s (0x%04x) %s", format_white_space, indent, #F, F, \
		  S);                                                         \
    }
...
#define _(F, P, S)                                                            \
  {                                                                           \
    rx_dynflag_offset =                                                       \
      rte_mbuf_dynflag_lookup (RTE_MBUF_DYNFLAG_##F##_NAME, P);               \
...
format_dpdk_rte_mbuf_vlan (u8 *s, va_list *va)
{
  ...
  if (clib_net_to_host_u16 (vlan_hdr->type) == ETHERNET_TYPE_DOT1AD)
    {
      s = format (s, "%U 802.1q vlan ", format_ethernet_vlan_tci,
		  clib_net_to_host_u16 (vlan_hdr->priority_cfi_and_id));
      vlan_hdr++;
    }

  s = format (s, "%U", format_ethernet_vlan_tci,
	      clib_net_to_host_u16 (vlan_hdr->priority_cfi_and_id));
...
format_dpdk_rte_mbuf (u8 *s, va_list *va)
{
  ...
  s = format (
    ...
    mb->outer_l2_len, mb->outer_l3_len, format_white_space, indent,
    mb->hash.rss, mb->hash.fdir.hi, mb->hash.fdir.lo);
  ...
  if (mb->packet_type)
    s = format (s, "\n%U%U", format_white_space, indent, format_dpdk_pkt_types,
		&mb->packet_type);
