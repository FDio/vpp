...
dpdk_get_flow_entry_by_flow_index (dpdk_device_t * xd, u32 flow_index)
{
  uword *p = hash_get (xd->flow_mark_by_flow_index, flow_index);
  if (p)
...
dpdk_flow_ops_fn (vnet_flow_dev_op_t op, u32 dev_instance, void *data)
{
  ...
  if (op == VNET_FLOW_DEV_OP_DEL_FLOW)
    {
      fe = dpdk_get_flow_entry_by_flow_index (xd, flow->index);
	...
	return VNET_FLOW_ERROR_INTERNAL;
      ...
      hash_unset (xd->flow_mark_by_flow_index, flow->index);
  ...
  hash_set (xd->flow_mark_by_flow_index, flow->index, fe->mark);
...
format_dpdk_flow (u8 * s, va_list * args)
{
  ...
  dpdk_main_t *dm = &dpdk_main;
  dpdk_device_t *xd = vec_elt_at_index (dm->devices, dev_instance);
  dpdk_flow_entry_t *fe = dpdk_get_flow_entry_by_flow_index (xd, flow_index);
