...
dpdk_flow_async_op_add (dpdk_device_t *xd, dpdk_flow_template_entry_t *fte, u32 queue_id,
			vnet_flow_range_t *range, uword *private_data)
{
  ...
  for (fi = 0; fi < range->count;)
      ...
      for (bi = 0; bi < q->batch_size && fi < range->count; bi++, fi++)
	{
	  ...
	  flow = vnet_flow_range_get_flow (range, fi);
  ...
  if ((rv = dpdk_flow_async_push_pull (xd, q, true)))
    {
      dpdk_device_flow_error (xd, "rte_flow_push/pull");
      return rv;
    }
			...
			vnet_flow_range_t *range, uword *private_data)
{
  ...
  for (fi = 0; fi < range->count;)
      ...
      for (bi = 0; bi < q->batch_size && fi < range->count; bi++, fi++)
	{
	  ...
	  fe = vec_elt_at_index (xd->flow_entries, *flow_private_data);
			...
			vnet_flow_range_t *range, uword *private_template_data, uword *private_data)
{
  ...
  if (op == VNET_FLOW_DEV_OP_ADD_FLOW)
    rv = dpdk_flow_async_op_add (xd, fte, DPDK_DEFAULT_ASYNC_FLOW_QUEUE_INDEX, range, private_data);
  else if (op == VNET_FLOW_DEV_OP_DEL_FLOW)
    rv = dpdk_flow_async_op_del (xd, fte, DPDK_DEFAULT_ASYNC_FLOW_QUEUE_INDEX, range, private_data);
  else
    return VNET_FLOW_ERROR_NOT_SUPPORTED;
