...
dpdk_flow_ops_fn (vnet_flow_dev_op_t op, u32 dev_instance, void *data)
{
  ...
  vnet_hw_interface_t *hw = vnet_get_hw_interface (dm->vnet_main, xd->hw_if_index);
  fle->next_index = vlib_node_add_next (dm->vlib_main, hw->input_node_index, flow->redirect_node_index);
