		      ...
		      dpdk_flow_items_args_t *args)
{
  ...
  if (f->type == VNET_FLOW_TYPE_GENERIC)
    {
      args->generic[0].pattern = f->generic.pattern.spec;
      args->generic[1].pattern = f->generic.pattern.mask;
  ...
  switch (protocol)
    ...
    case IP_PROTOCOL_L2TP:
      args->l2tp[0].session_id = clib_host_to_net_u32 (f->ip4_l2tpv3oip.session_id);
    ...
    case IP_PROTOCOL_IPSEC_ESP:
      args->esp[0].hdr.spi = clib_host_to_net_u32 (f->ip4_ipsec_esp.spi);
    ...
    case IP_PROTOCOL_IPSEC_AH:
      args->ah[0].spi = clib_host_to_net_u32 (f->ip4_ipsec_ah.spi);
      args->ah[1].spi = ~0;
	...
	{
	  args->gtp[0].teid = clib_host_to_net_u32 (f->ip4_gtpc.teid);
	  args->gtp[1].teid = ~0;
	...
	{
	  args->gtp[0].teid = clib_host_to_net_u32 (f->ip4_gtpu.teid);
	  args->gtp[1].teid = ~0;
			       ...
			       dpdk_flow_items_args_t *args)
{
  ...
  FOREACH_FLOW_ITEM (args->items, item) { item->spec = NULL; }
...
dpdk_flow_async_op_add (dpdk_device_t *xd, dpdk_flow_template_entry_t *fte, u32 queue_id,
			u32 *flow_indices, uword *private_data)
{
...
static_always_inline int
dpdk_flow_async_op_del (dpdk_device_t *xd, u32 queue_id, u32 count, uword *private_data)
{
  vlib_main_t *vm = vlib_get_main ();
  ...
  for (fi = 0; fi < count;)
      ...
      for (bi = 0; bi < shadow_q.batch_size && fi < count; bi++, fi++)
	{
	  ...
	  *flow_private_data = ~0;
      ...
      if ((rv = dpdk_flow_async_push_pull (xd, &shadow_q, false)))
	{
	  dpdk_device_flow_warning (xd, "rte_flow_push/pull");
	  goto error;
	}
...
error:
  {
    int rv2;
    if ((rv2 = dpdk_flow_async_push_pull (xd, &shadow_q, true)))
      {
	dpdk_device_flow_warning (xd, "rte_flow_push/pull");
	rv = rv ? rv : rv2;
      }
...
dpdk_flow_async_ops_fn (vnet_main_t *vnm, vnet_flow_dev_op_t op, u32 dev_instance,
			u32 *flow_indices, uword *private_template_data, uword *private_data)
{
  vlib_main_t *vm = vlib_get_main ();
  dpdk_main_t *dm = &dpdk_main;
  dpdk_device_t *xd = vec_elt_at_index (dm->devices, dev_instance);
  ...
  if (op == VNET_FLOW_DEV_OP_ADD_FLOW)
      ...
      if (private_template_data == 0)
	{
      ...
      rv = dpdk_flow_async_op_add (xd, fte, DPDK_DEFAULT_ASYNC_FLOW_QUEUE_INDEX, flow_indices,
				   private_data);
    }
  else if (op == VNET_FLOW_DEV_OP_DEL_FLOW)
    {
      rv = dpdk_flow_async_op_del (xd, DPDK_DEFAULT_ASYNC_FLOW_QUEUE_INDEX, vec_len (flow_indices),
				   private_data);
    }
  else
    {
			   ...
			   u32 flow_template_index, uword *private_data)
{
  ...
  switch (template->type)
    ...
    case VNET_FLOW_TYPE_IP6_IP6_N_TUPLE:
    case VNET_FLOW_TYPE_GENERIC:
      if ((rv = dpdk_flow_template_add (xd, template, fte, n_flows)))
	goto done;
      break;
    default:
      rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
