...
dpdk_flow_add (dpdk_device_t * xd, vnet_flow_t * f, dpdk_flow_entry_t * fe)
{
  ...
  if (f->type == VNET_FLOW_TYPE_IP6_N_TUPLE)
    {
      ...
      _clib_memcpy (ip6[0].hdr.src_addr, &t6->src_addr.addr, 16);
      _clib_memcpy (ip6[1].hdr.src_addr, &t6->src_addr.mask, 16);
      _clib_memcpy (ip6[0].hdr.dst_addr, &t6->dst_addr.addr, 16);
      _clib_memcpy (ip6[1].hdr.dst_addr, &t6->dst_addr.mask, 16);
  ...
  if (f->type == VNET_FLOW_TYPE_IP4_VXLAN)
	...
	.vni_reserved = clib_host_to_net_u32 (((u32) - 1) << 8)
      ...
      _clib_memcpy (raw[0].val + raw_sz, &spec_hdr, vxlan_hdr_sz);
      raw[0].item.pattern = raw[0].val + raw_sz;
      _clib_memcpy (raw[1].val + raw_sz, &mask_hdr, vxlan_hdr_sz);
