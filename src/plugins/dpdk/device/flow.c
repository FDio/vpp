  ...
  if (f->type == VNET_FLOW_TYPE_ETHERNET)
    {
      ...
      memset (&eth[0], 0, sizeof (eth[0]));
      memset (&eth[1], 0, sizeof (eth[1]));

      /* check if SMAC/DMAC/Ether_type assigned */
      if (!mac_address_is_all_zero (te->eth_hdr.dst_address))
	{
	  memcpy (&eth[0].dst, &te->eth_hdr.dst_address, sizeof (eth[0].dst));
	  memset (&eth[1].dst, 0xFF, sizeof (eth[1].dst));
	}

      if (!mac_address_is_all_zero (te->eth_hdr.src_address))
	{
	  memcpy (&eth[0].src, &te->eth_hdr.src_address, sizeof (eth[0].src));
	  memset (&eth[1].src, 0xFF, sizeof (eth[1].src));
	}
