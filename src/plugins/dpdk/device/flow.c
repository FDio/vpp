...
struct rte_flow_pattern_template_attr pattern_attr = { .ingress = 1, .relaxed_matching = 1 };
  ...
  if (f->actions & VNET_FLOW_ACTION_AGE)
    {
      ...
      /* Rather than pass a pointer in dpdk's xd->flow_entries pool
       * we pass a pool index in the context pointer value */
      args->age.context = (void *) (uword) (fe - xd->flow_entries);
	...
	case RTE_FLOW_ACTION_TYPE_AGE:
	  {
	    ...
	    conf->context = (void *) (uword) (fe - xd->flow_entries);
	  }
	  break;
...
{
  dpdk_flow_async_template_table_destroy (xd, &fte->table);
...
dpdk_flow_add (dpdk_device_t *xd, vnet_flow_t *f, dpdk_flow_entry_t *fe)
{
  ...
  if (rv)
    {
      dpdk_device_flow_warning (xd, "rte_flow_validate", rv);
  ...
  if (!fe->handle)
    {
      dpdk_device_flow_warning (xd, "rte_flow_create", rte_errno);
      return VNET_FLOW_ERROR_NOT_SUPPORTED;
    }
...
int
dpdk_flow_async_template_table_create (dpdk_device_t *xd,
				       struct rte_flow_template_table_attr *template_attr,
				       struct rte_flow_item *pattern,
				       struct rte_flow_action *actions_conf,
				       struct rte_flow_action *actions_mask,
				       dpdk_flow_template_table_t *table)
{
  struct rte_flow_pattern_template_attr pattern_attr = {
    .relaxed_matching = 1,
    .ingress = template_attr->flow_attr.ingress,
    .egress = template_attr->flow_attr.egress,
    .transfer = template_attr->flow_attr.transfer,
  };
  ...
  table->pattern_handle =
    rte_flow_pattern_template_create (xd->port_id, &pattern_attr, pattern, &xd->last_flow_error);
  if (!table->pattern_handle)
    {
      rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
      goto done;
    }

  table->actions_handle = rte_flow_actions_template_create (xd->port_id, &action_attr, actions_conf,
							    actions_mask, &xd->last_flow_error);
  if (!table->actions_handle)
    {
      rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
      goto done_pattern_handle;
    }
  table->table_handle =
    rte_flow_template_table_create (xd->port_id, template_attr, &table->pattern_handle, 1,
				    &table->actions_handle, 1, &xd->last_flow_error);
  if (!table->table_handle)
    {
      rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
...
done_actions_handle:
  rte_flow_actions_template_destroy (xd->port_id, table->actions_handle, &xd->last_flow_error);
  table->actions_handle = 0;

done_pattern_handle:
  rte_flow_pattern_template_destroy (xd->port_id, table->pattern_handle, &xd->last_flow_error);
  table->pattern_handle = 0;
...
int
dpdk_flow_async_template_table_destroy (dpdk_device_t *xd, dpdk_flow_template_table_t *table)
{
  if (table->table_handle)
    {
      rte_flow_template_table_destroy (xd->port_id, table->table_handle, &xd->last_flow_error);
      table->table_handle = NULL;
    }

  if (table->actions_handle)
    {
      rte_flow_actions_template_destroy (xd->port_id, table->actions_handle, &xd->last_flow_error);
      table->actions_handle = NULL;
    }

  if (table->pattern_handle)
    {
      rte_flow_pattern_template_destroy (xd->port_id, table->pattern_handle, &xd->last_flow_error);
      table->pattern_handle = NULL;
    }
...
dpdk_flow_template_add (dpdk_device_t *xd, vnet_flow_t *t, dpdk_flow_template_entry_t *fte)
{
  ...
  if ((rv = dpdk_flow_async_template_table_create (xd, &template_attr, item_args.items, actions,
						   actions_mask, &fte->table)))
    goto done;
...
dpdk_flow_ops_fn (vnet_main_t *vnm, vnet_flow_dev_op_t op, u32 dev_instance, u32 flow_index)
{
  ...
  if (op == VNET_FLOW_DEV_OP_DEL_FLOW)
	...
	{
	  dpdk_device_flow_warning (xd, "rte_flow_destroy", rv);
	  return VNET_FLOW_ERROR_INTERNAL;
	}
...
dpdk_flow_async_op_add (dpdk_device_t *xd, dpdk_flow_template_entry_t *fte, u32 *flow_indices)
{
  ...
  /* ensure per-worker caches have enough entries */
...
error:
  ...
  /* drain excess back to shared pool if caches grew too large */
  if (vec_len (ptd->fe_cache) > 2 * xd->flow_cache_size)
    dpdk_flow_fe_cache_drain (xd, ptd, xd->flow_cache_size);
  if (vec_len (ptd->fle_cache) > 2 * xd->flow_cache_size)
    dpdk_flow_fle_cache_drain (xd, ptd, xd->flow_cache_size);
...
dpdk_poll_aged_flows (dpdk_device_t *xd, f64 now)
{
  ...
  /* Resolve each DPDK age context (fe pool index) to its vnet_flow_t and
   * batch entries for the aged-flow callback */
  u32 n_valid = 0;
  u32 n_unknown = 0;
  u32 *aged_flow_indices = 0;
  u64 *age_opaques = 0;
  vec_alloc (aged_flow_indices, n_aged);
  vec_alloc (age_opaques, n_aged);

  for (int i = 0; i < n_aged; i++)
    {
      u32 fe_index = (u32) (uword) dm->aged_flow_opaque[i];
      dpdk_flow_entry_t *fe;
      vnet_flow_t *f;

      if (pool_is_free_index (xd->flow_entries, fe_index))
	{
	  n_unknown++;
	  continue;
	}

      fe = pool_elt_at_index (xd->flow_entries, fe_index);
      f = vnet_get_flow (fe->flow_index);
      if (!f)
	{
	  n_unknown++;
	  continue;
      ...
      vec_add1 (age_opaques, f->age_opaque);
      vec_add1 (aged_flow_indices, fe->flow_index);
      n_valid++;
    }

  if (PREDICT_FALSE (n_unknown))
    dpdk_log_debug ("[%u] %u aged flow contexts did not match a live flow entry", xd->port_id,
		    n_unknown);

  /* Hand off the batch to the consumer, which is responsible for
   * vnet_flow_async_range_disable + vnet_flow_del. */
  if (n_valid > 0 && fm->aged_flow_cb)
    fm->aged_flow_cb (age_opaques, aged_flow_indices, xd->hw_if_index, n_valid);

  vec_free (age_opaques);
  vec_free (aged_flow_indices);
  vec_free (dm->aged_flow_opaque);

  dpdk_log_debug ("[%u] %d aged flows reaped", xd->port_id, n_valid);
}
