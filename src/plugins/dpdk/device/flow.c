  ...
  if (!fe->handle)
    {
      ...
      clib_warning ("%-25s: %s\n", "last DPDK error message",
		    xd->last_flow_error.message ? xd->
		    last_flow_error.message : "n/a");
      rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
      goto done;
    }
done:
  ...
  if (op == VNET_FLOW_DEV_OP_DEL_FLOW)
    {
      ...
      rv = rte_flow_destroy (xd->device_index, fe->handle,
			     &xd->last_flow_error);
      if (rv != 0)
	return VNET_FLOW_ERROR_INTERNAL;
      ...
      goto disable_hw_offload_support;
    }
