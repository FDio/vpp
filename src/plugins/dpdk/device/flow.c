		  ...
		  void *opaque)
{
  ...
  if (op == VNET_FLOW_DEV_OP_GET_COUNTER)
	...
	return VNET_FLOW_ERROR_INTERNAL;
      /* opaque value is expected to be a pointer to a counter for this operation */
      vnet_flow_counter_t *counter = (vnet_flow_counter_t *) opaque;
      counter->hits = count.hits;
      counter->bytes = count.bytes;
      return 0;
    }
