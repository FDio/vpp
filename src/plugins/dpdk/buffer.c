...
clib_error_t *CLIB_MULTIARCH_FN (_dpdk_buffer_pools_create) (vlib_main_t *
							     ...
							     u32 nmbufs)
{
#define DEFAULT_CACHE_MEMPOOL_OPS_NAME    "vpp\0"
  ...
  strncpy (ops.name, DEFAULT_CACHE_MEMPOOL_OPS_NAME,
	   strlen (DEFAULT_CACHE_MEMPOOL_OPS_NAME));
  ...
  strncpy (ops.name, no_cache_ops_name, strlen (no_cache_ops_name));
  ...
  strncpy (cache_ops_name, DEFAULT_CACHE_MEMPOOL_OPS_NAME,
	   strlen (DEFAULT_CACHE_MEMPOOL_OPS_NAME));
  ...
  if (ops_name)
    strncpy (ops_name, cache_ops_name, strlen (cache_ops_name));
