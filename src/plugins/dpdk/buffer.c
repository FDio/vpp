...
dpdk_buffer_pool_init (vlib_main_t * vm, vlib_buffer_pool_t * bp)
{
  ...
  /* non-cached mempool */
  nmp = rte_mempool_create_empty ((char *) bp->name, vec_len (bp->buffers),
...
static int
dpdk_ops_vpp_enqueue_no_cache (struct rte_mempool *mp, void *const *obj_table,
			       unsigned n)
{
  ...
  /* revert pool on each buffer to cached mempool */
  for (i = 0; i < n; i++)
    {
  ...
  return dpdk_ops_vpp_enqueue (cmp, obj_table, n);
}
...
static const struct rte_mempool_ops ops_vpp_no_cache = {
  .name = "vpp-no-cache",
  .alloc = dpdk_ops_vpp_alloc,
  .free = dpdk_ops_vpp_free,
  .enqueue = dpdk_ops_vpp_enqueue_no_cache,
  .dequeue = dpdk_ops_vpp_dequeue_no_cache,
  .get_count = dpdk_ops_vpp_get_count_no_cache,
};

MEMPOOL_REGISTER_OPS (ops_vpp_no_cache);
