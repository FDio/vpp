			       ...
			       crypto_data_t * data, u32 drv_id, u32 sa_idx)
{
  ...
  if (vec_len (data->session_by_drv_id_and_sa_index) <= sa_idx)
    {
      u32 clen = vec_len (data->session_by_drv_id_and_sa_index);
      u32 add_cnt = (sa_idx - clen > 32) ? (sa_idx - clen + 1) : 32;
      vec_resize (data->session_by_drv_id_and_sa_index, add_cnt);
    }
  sess_by_sa =
    vec_elt_at_index (data->session_by_drv_id_and_sa_index, sa_idx);
  sess_by_sa->dev_mask |= 1L << drv_id;
		    ...
		    crypto_worker_main_t * cwm, u8 is_outbound)
{
  ...
  val = (uword *) get_session_by_drv_and_sa_idx (data, res->drv_id, sa_idx);
