 ...
 *
 * Copyright (c) 2019 Rubicon Communications, LLC and/or its affiliates.
...
VLIB_NODE_FN (dpdk_esp4_encrypt_handoff) (vlib_main_t * vm,
					  vlib_node_runtime_t * node,
					  vlib_frame_t * from_frame)
{
  dpdk_crypto_main_t *dm = &dpdk_crypto_main;

  return ipsec_handoff (vm, node, from_frame, dm->esp4_enc_fq_index, true);
}

VLIB_NODE_FN (dpdk_esp4_encrypt_tun_handoff) (vlib_main_t * vm,
					      vlib_node_runtime_t * node,
					      vlib_frame_t * from_frame)
{
  dpdk_crypto_main_t *dm = &dpdk_crypto_main;

  return ipsec_handoff (vm, node, from_frame, dm->esp4_enc_tun_fq_index,
			true);
}

VLIB_NODE_FN (dpdk_esp4_decrypt_handoff) (vlib_main_t * vm,
					  vlib_node_runtime_t * node,
					  vlib_frame_t * from_frame)
{
  dpdk_crypto_main_t *dm = &dpdk_crypto_main;

  return ipsec_handoff (vm, node, from_frame, dm->esp4_dec_fq_index, true);
}

VLIB_NODE_FN (dpdk_esp6_encrypt_handoff) (vlib_main_t * vm,
					  vlib_node_runtime_t * node,
					  vlib_frame_t * from_frame)
{
  dpdk_crypto_main_t *dm = &dpdk_crypto_main;

  return ipsec_handoff (vm, node, from_frame, dm->esp6_enc_fq_index, true);
}

VLIB_NODE_FN (dpdk_esp6_encrypt_tun_handoff) (vlib_main_t * vm,
					      vlib_node_runtime_t * node,
					      vlib_frame_t * from_frame)
{
  dpdk_crypto_main_t *dm = &dpdk_crypto_main;

  return ipsec_handoff (vm, node, from_frame, dm->esp6_enc_tun_fq_index,
			true);
}

VLIB_NODE_FN (dpdk_esp6_decrypt_handoff) (vlib_main_t * vm,
					  vlib_node_runtime_t * node,
					  vlib_frame_t * from_frame)
{
  dpdk_crypto_main_t *dm = &dpdk_crypto_main;

  return ipsec_handoff (vm, node, from_frame, dm->esp6_dec_fq_index, true);
}

/* *INDENT-OFF* */
VLIB_REGISTER_NODE (dpdk_esp4_encrypt_handoff) = {
  .name = "dpdk-esp4-encrypt-handoff",
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (dpdk_esp4_encrypt_tun_handoff) = {
  .name = "dpdk-esp4-encrypt-tun-handoff",
  .vector_size = sizeof (u32),
  .format_trace = format_ipsec_handoff_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN(ipsec_handoff_error_strings),
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (dpdk_esp4_decrypt_handoff) = {
  .name = "dpdk-esp4-decrypt-handoff",
  .vector_size = sizeof (u32),
  .format_trace = format_ipsec_handoff_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN(ipsec_handoff_error_strings),
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (dpdk_esp6_encrypt_handoff) = {
  .name = "dpdk-esp6-encrypt-handoff",
  .vector_size = sizeof (u32),
  .format_trace = format_ipsec_handoff_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN(ipsec_handoff_error_strings),
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (dpdk_esp6_encrypt_tun_handoff) = {
  .name = "dpdk-esp6-encrypt-tun-handoff",
  .vector_size = sizeof (u32),
  .format_trace = format_ipsec_handoff_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN(ipsec_handoff_error_strings),
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (dpdk_esp6_decrypt_handoff) = {
  .name = "dpdk-esp6-decrypt-handoff",
  .vector_size = sizeof (u32),
  .format_trace = format_ipsec_handoff_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN(ipsec_handoff_error_strings),
