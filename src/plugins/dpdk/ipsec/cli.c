...
format_crypto_resource (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U thr_id %3d qp %2u dec_inflight %u, enc_inflights %u\n",
	      format_white_space, indent, (i16) res->thread_idx, res->qp_id,
	      res->inflights[0], res->inflights[1]);
...
format_crypto (u8 *s, va_list *args)
{
  ...
  if (dev->features)
      ...
      for (mask = 1; mask != 0; mask <<= 1)
	{
	  ...
	  if (feat)
	    {
	      s =
		format (s, "%s%s", pre, rte_cryptodev_get_feature_name (feat));
	      pre = ", ";
	    }
	}
      s = format (s, "\n");
  ...
  s = format (
    ...
    stats.enqueued_count, stats.dequeued_count, stats.enqueue_err_count,
  ...
  vec_foreach (res_idx, cwm->resource_idx)
    {
      ...
      s = format (s, "%s%-20s dev-id %2u queue-pair %2u\n", ind,
		  vec_elt_at_index (dcm->dev, res->dev_id)->name, res->dev_id,
		  res->qp_id);
...
static clib_error_t *
set_dpdk_crypto_placement_fn (vlib_main_t *vm, unformat_input_t *input,
			      vlib_cli_command_t *cmd)
{
...
static clib_error_t *
clear_dpdk_crypto_placement_fn (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
...
format_dpdk_mempool (u8 *s, va_list *args)
{
  ...
  s =
    format (s, "%s\n%Uavailable %7d, allocated %7d total %7d\n", mp->name,
	    format_white_space, indent + 2, count, mp->size - count, mp->size);
  s = format (s, "%Uphys_addr %p, flags %08x, nb_mem_chunks %u\n",
	      format_white_space, indent + 2, mp->mz->iova, mp->flags,
	      mp->nb_mem_chunks);
  s = format (s, "%Uelt_size %4u, header_size %3u, trailer_size %u\n",
	      format_white_space, indent + 2, mp->elt_size, mp->header_size,
	      mp->trailer_size);
...
static clib_error_t *
show_dpdk_crypto_pools_fn (vlib_main_t *vm, unformat_input_t *input,
			   vlib_cli_command_t *cmd)
{
  dpdk_crypto_main_t *dcm = &dpdk_crypto_main;
  crypto_data_t *data;
