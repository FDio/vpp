			  ...
			  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  esp_header_t *esp0;
	  esp_footer_t *f0;
	  u8 is_ipv6, next_hdr_type, protocol = IP_PROTOCOL_IPSEC_ESP;
	  u32 iv_size;
	  u16 orig_sz;
	  u8 trunc_size;
	  u16 rewrite_len;
	  ...
	  if (sa0->is_tunnel)
	      ...
	      if (!is_ipv6 && !sa0->is_tunnel_ip6)	/* ip4inip4 */
		{
		  ...
		  esp0 = (sa0->udp_encap) ? &ouh0->esp : &oh0->esp;
	  ...
	  if (is_ipv6)
		...
		{
		  ouh0->udp.length =
		    clib_host_to_net_u16 (ouh0->ip4.length -
