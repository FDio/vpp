...
vlib_node_registration_t ip4_dpdk_esp_encrypt_node;
vlib_node_registration_t ip6_dpdk_esp_encrypt_node;
			 ...
			 vlib_frame_t * from_frame, int is_ip6)
{
  ...
  if (ret)
    {
      if (is_ip6)
	vlib_node_increment_counter (vm, ip6_dpdk_esp_encrypt_node.index,
				     ESP_ENCRYPT_ERROR_DISCARD, 1);
      else
	vlib_node_increment_counter (vm, ip4_dpdk_esp_encrypt_node.index,
				     ESP_ENCRYPT_ERROR_DISCARD, 1);
      /* Discard whole frame */
      return n_left_from;
    }
	      ...
	      if (PREDICT_FALSE (res_idx == (u16) ~ 0))
		{
		  ...
		  if (is_ip6)
		    vlib_node_increment_counter (vm,
						 ip6_dpdk_esp_encrypt_node.index,
						 ESP_ENCRYPT_ERROR_NOSUP, 1);
		  else
		    vlib_node_increment_counter (vm,
						 ip4_dpdk_esp_encrypt_node.index,
						 ESP_ENCRYPT_ERROR_NOSUP, 1);
		  to_next[0] = bi0;
		  to_next += 1;
		  n_left_to_next -= 1;
  ...
  if (is_ip6)
    {
      ...
      crypto_enqueue_ops (vm, cwm, 1, ip6_dpdk_esp_encrypt_node.index,
  ...
  else
    {
      ...
      crypto_enqueue_ops (vm, cwm, 1, ip4_dpdk_esp_encrypt_node.index,
			  ESP_ENCRYPT_ERROR_ENQ_FAIL, numa);
    }
...
static uword
ip4_dpdk_esp_encrypt_node_fn (vlib_main_t * vm,
			      vlib_node_runtime_t * node,
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_dpdk_esp_encrypt_node) = {
  .function = ip4_dpdk_esp_encrypt_node_fn,
  .name = "ip4-dpdk-esp-encrypt",
...
VLIB_NODE_FUNCTION_MULTIARCH (ip4_dpdk_esp_encrypt_node,
			      ip4_dpdk_esp_encrypt_node_fn);

static uword
ip6_dpdk_esp_encrypt_node_fn (vlib_main_t * vm,
			      vlib_node_runtime_t * node,
			      vlib_frame_t * from_frame)
{
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip6_dpdk_esp_encrypt_node) = {
  .function = ip6_dpdk_esp_encrypt_node_fn,
  .name = "ip6-dpdk-esp-encrypt",
  .flags = VLIB_NODE_FLAG_IS_OUTPUT,
  .vector_size = sizeof (u32),
  .format_trace = format_esp_encrypt_trace,
  .n_errors = ARRAY_LEN (esp_encrypt_error_strings),
...
VLIB_NODE_FUNCTION_MULTIARCH (ip6_dpdk_esp_encrypt_node,
			      ip6_dpdk_esp_encrypt_node_fn);
/*
