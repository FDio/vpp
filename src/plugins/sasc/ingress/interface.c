...
static inline uword
sasc_input_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_dpo,
                       bool is_output) {
...
static inline uword
sasc_feature_arc_return_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node,
                                    vlib_frame_t *frame) {
    u32 *from = vlib_frame_vector_args(frame);
    u32 n_left = frame->n_vectors;
    vlib_buffer_t *bufs[VLIB_FRAME_SIZE], **b;
