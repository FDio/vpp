...
enum sasc_input_next_e {
    SASC_INTERFACE_NEXT_LOOKUP,
    SASC_INTERFACE_NEXT_DROP,
    SASC_INTERFACE_N_NEXT
};
...
static inline u8 *format_sasc_input_trace(u8 *s, va_list *args) {
...
static inline u8 *format_sasc_feature_arc_return_trace(u8 *s, va_list *args) {
    CLIB_UNUSED(vlib_main_t * vm) = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
...
static inline uword sasc_input_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node,
                                           vlib_frame_t *frame, bool is_dpo, bool is_output) {
...
static inline uword sasc_feature_arc_return_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node,
                                                        vlib_frame_t *frame) {
    u32 *from = vlib_frame_vector_args(frame);
    u32 n_left = frame->n_vectors;
    vlib_buffer_t *bufs[VLIB_FRAME_SIZE], **b;
    ...
    while (n_left > 0) {
        ...
        if (next[0] > node->n_next_nodes) {
            sasc_log_err("next index %d invalid %U", next[0], format_ip4_header, ip,
                         sizeof(ip4_header_t));
