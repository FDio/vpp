...
sasc_interface_input_enable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd) {
    ...
    if (sw_if_index == ~0) {
        err = clib_error_return(0, "no valid interface provided");
        goto done;
    }

    if (tenant_id == ~0) {
        err = clib_error_return(0, "cannot enable without valid tenant_id");
        goto done;
    }
    int rv = sasc_interface_input_enable_disable(sw_if_index, tenant_id, output_arc, 1 /* is_enable */);
    if (rv != 0) {
        err = clib_error_return(0, "error when enabling sasc on interface");
    }
...
/*
 *  disable sasc ingress interface <interface>
 */
static clib_error_t *
sasc_interface_input_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd) {
    unformat_input_t line_input_, *line_input = &line_input_;
    clib_error_t *err = 0;
    u32 sw_if_index = ~0, tenant_id = ~0;
    bool output_arc = false;
    ...
    int rv = sasc_interface_input_enable_disable(sw_if_index, tenant_id, output_arc, 0 /*is_enable=*/);
    if (rv != 0) {
        err = clib_error_return(0, "error when disabling sasc on interface");
    }
...
VLIB_CLI_COMMAND(sasc_interface_input_disable_command, static) = {
    .path = "delete sasc ingress interface",
    .short_help = "delete sasc ingress interface <interface> [output]",
    .function = sasc_interface_input_disable_command_fn,
};
