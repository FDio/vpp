...
typedef struct {
    ...
    struct {
        ...
        u32 thread_index;                   // 4 bytes - matches table value type
        u16 tenant_idx;                     // 2 bytes
        session_version_t session_version;  // 2 bytes
        sasc_session_state_t state;         // 1 byte
        // sasc_session_type_t type;          // 1 byte
    } __attribute__((packed)); // 48 bytes total for fast path fields
...
_Static_assert(sizeof(sasc_session_t) == 128, "Size of sasc_session_t should be 128");
...
static_always_inline sasc_proto_group_t sasc_proto_to_group(u8 proto) {
    switch (proto) {
...
static_always_inline u32 sasc_service_chain_next_index(u32 ingress_node_index,
                                                       sasc_proto_group_t proto_group,
                                                       u32 chain_id) {
    return (ingress_node_index * SASC_MAX_CHAINS * SASC_PROTO_GROUP_N) +
           (chain_id * SASC_PROTO_GROUP_N) + proto_group;
}

static_always_inline void sasc_buffer_init_chain(u32 ingress_node_index, u8 proto, vlib_buffer_t *b,
                                                 u32 effective_index) {
...
static_always_inline void sasc_next(vlib_buffer_t *b, u16 *next_index) {
    sasc_buffer_opaque_t *sbo = sasc_buffer(b);
