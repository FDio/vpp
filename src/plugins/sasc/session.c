 ...
 */
void
sasc_set_service_chain(sasc_tenant_t *tenant, u8 proto, u32 *bitmaps);
void
sasc_set_session_service_chain(sasc_tenant_t *tenant, sasc_session_t *session, u8 proto);

static inline sasc_session_t *
sasc_session_from_lookup_value(sasc_main_t *sasc, u64 value, u32 *thread_index,
                               u32 *session_index) {
...
sasc_session_t *
sasc_create_session(u16 tenant_idx, sasc_session_key_t *primary, sasc_session_key_t *secondary,
    ...
    *flow_index = pseudo_flow_idx; // Keep original flow index for direction
    ...
    sasc_log_debug("Created session [%u] %U", session_idx, format_sasc_session_key,
                   &session->forward_key);
    return session;
}

sasc_session_t *
sasc_lookup_session(u32 context_id, ip_address_t *src, u16 sport, u8 protocol, ip_address_t *dst,
                    u16 dport) {
    sasc_main_t *sasc = &sasc_main;
    u64 value;
    if (!src || !dst)
...
void
sasc_session_remove_core(sasc_main_t *sasc, sasc_session_t *session, u32 thread_index,
    ...
    vlib_increment_simple_counter(&sasc->counters[SASC_COUNTER_REMOVED], thread_index,
...
void
sasc_session_remove(sasc_main_t *sasc, sasc_session_t *session, u32 thread_index,
                    u32 session_index) {
...
int
sasc_session_generate_reverse_key(sasc_session_key_t *forward_key,
