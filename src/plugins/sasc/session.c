...
static inline sasc_session_t *sasc_session_from_lookup_value(sasc_main_t *sasc, u64 value,
                                                             u32 *thread_index,
                                                             u32 *session_index) {
...
static int sasc_session_add_del_key(sasc_session_key_t *k, int is_add, u64 value, u64 *h) {
...
sasc_session_t *sasc_create_session(u16 tenant_idx, sasc_session_key_t *primary,
                                    sasc_session_key_t *secondary, bool is_static,
                                    u32 *flow_index) {
    sasc_main_t *sasc = &sasc_main;
    ...
    *flow_index = pseudo_flow_idx; // Keep original flow index for direction
    ...
    // Initialize session
    clib_memcpy_fast(&session->forward_key, primary, sizeof(session->forward_key));
    if (secondary) {
        clib_memcpy_fast(&session->reverse_key, secondary, sizeof(session->reverse_key));
    }
    ...
    session->created = (u32)vlib_time_now(vlib_get_main());
    ...
    clib_warning("Created session [%u] %U", session_idx, format_sasc_session_key,
                 &session->forward_key);
    return session;
}

sasc_session_t *sasc_lookup_session(u32 context_id, ip_address_t *src, u16 sport, u8 protocol,
                                    ip_address_t *dst, u16 dport) {
    sasc_main_t *sasc = &sasc_main;
    u64 value;
    if (!src || !dst)
        return 0;
    ...
    // clib_warning("Searching for session %U", format_sasc_session_key, &k);
    if (clib_bihash_search_inline_40_8(&sasc->session_hash, &kv))
        return 0;
    value = kv.value;
...
void sasc_session_remove_core(sasc_main_t *sasc, sasc_session_t *session, u32 thread_index,
                              u32 session_index, bool stop_timer) {
    clib_warning("***Removing session %U", format_sasc_session_key, &session->forward_key);
    u64 h;
    // clib_bihash_kv_40_8_t kv2 = {0};
    // clib_memcpy(&kv2.key, &session->forward_key, sizeof(session->forward_key));
    ...
    sasc_log_debug("Removing session %u %U", session_index, format_sasc_session_key,
                   &session->forward_key);
    // if (stop_timer) {
    //   sasc_log_debug("Stopping timer for session %u", session_index);
    //   sasc_session_timer_stop(sasc, session, thread_index);
    // }
    if (sasc_session_add_del_key(&session->forward_key, 0, 0, &h)) {
        sasc_log_err("Failed to remove session key from table");
    }
    if (sasc_session_add_del_key(&session->reverse_key, 0, 0, &h)) {
    ...
    // }
    // vlib_increment_simple_counter(&sasc->tenant_simple_ctr[SASC_TENANT_COUNTER_REMOVED],
    // thread_index,
    //                               session->tenant_idx, 1);

    // vlib_increment_combined_counter(&sasc->tenant_combined_ctr[SASC_TENANT_COUNTER_TX],
    // thread_index, session->tenant_idx,
    //                                 session->pkts[SASC_FLOW_FORWARD],
    //                                 session->bytes[SASC_FLOW_FORWARD]);
    // vlib_increment_combined_counter(&sasc->tenant_combined_ctr[SASC_TENANT_COUNTER_RX],
    // thread_index, session->tenant_idx,
...
void sasc_session_remove(sasc_main_t *sasc, sasc_session_t *session, u32 thread_index,
                         u32 session_index) {
 ...
 * Reset counters.
 */
void sasc_session_reopen(sasc_main_t *sasc, u32 thread_index, sasc_session_t *session) {
#if 0
...
bool sasc_session_is_expired_session_idx(sasc_main_t *sasc, u32 session_index) {
...
int sasc_session_try_add_secondary_key(sasc_main_t *sasc, u32 thread_index, u32 pseudo_flow_index,
    ...
    clib_memcpy(&session->keys[SASC_SESSION_KEY_SECONDARY], key, sizeof(*key));
...
int sasc_session_generate_reverse_key(sasc_session_key_t *forward_key,
...
static void sasc_session_expiry_notify(u32 *session_indices) {
...
bool sasc_session_walk_and_expire(u32 max_walk_entries, u32 max_expire_entries, u32 *cursor) {
    ...
    vec_foreach (session_index, to_delete) {
        ...
        sasc_log_debug("Removing session %u %U", *session_index, format_sasc_session_key,
                       &session->forward_key);
        clib_warning("Removing session %u %U", *session_index, format_sasc_session_key,
                     &session->forward_key);
        sasc_session_remove(sasc, session, session->thread_index, *session_index);
    }
 ...
 */
static uword sasc_session_expiry_process(vlib_main_t *vm, vlib_node_runtime_t *rt,
                                         vlib_frame_t *f) {
...
static void sasc_session_expiry_callback(u32 *session_indices) {
    sasc_main_t *sasc = &sasc_main;
    u32 *session_index;
    sasc_session_t *session;
    ...
    vec_foreach (session_index, session_indices) {
        ...
        clib_warning("Session expired: %U", format_sasc_session_key, &session->forward_key);

        cbor_item_t *obj = sasc_session_to_cbor(session);

        if (session->service_chain[SASC_FLOW_FORWARD] != 0xFFFF &&
            session->service_chain[SASC_FLOW_FORWARD] < vec_len(sasc->chains)) {
            u32 *forward_chain =
                *vec_elt_at_index(sasc->chains, session->service_chain[SASC_FLOW_FORWARD]);
            vec_foreach (service_index, forward_chain) {
                if (*service_index < vec_len(sm->services) &&
                    sm->services[*service_index]->format_service_cbor)
                    sm->services[*service_index]->format_service_cbor(obj, thread_index, session_idx);
            }
        }
    }
}

static clib_error_t *sasc_session_expiry_init(vlib_main_t *vm) {
