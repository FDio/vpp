...
typedef struct {
    ...
    u32 seq_num;
    u32 ack_num;
    u16 flags;               /* Changed from u8 to u16 to support ICMP type/code encoding */
    const char *options_str; /* Optional Scapy TCP options string */
...
vlib_buffer_t *build_packet(char *packetdef, u32 *bi);
vlib_buffer_t *build_tcp_packet_with_flags(u32 *bi, tcp_packet_info_t *info);
...
#define SASC_TEST_REGISTER_ICMP(name, desc, packets, post_fn)                                      \
    static int sasc_test_##name##_fn(void) {                                                       \
        return run_icmp_test(#name, packets, ARRAY_LEN(packets), post_fn);                         \
    }                                                                                              \
    SASC_TEST_REGISTER(name, desc, SASC_TEST_CATEGORY_ICMP, sasc_test_##name##_fn, post_fn,        \
                       "ICMP functionality")

int run_tcp_test(const char *name, const test_packet_template_t *pkts, int len,
                 int (*post_fn)(void));
int run_retransmit_test(const char *name, const test_packet_template_t *pkts, int len,
                        bool expect_retransmit, int (*post_fn)(void));
int run_reorder_test(const char *name, const test_packet_template_t *pkts, int len,
                     bool expect_reorder, int (*post_fn)(void));
int run_udp_test(const char *name, const test_packet_template_t *pkts, int len,
                 int (*post_fn)(void));
int run_icmp_test(const char *name, const test_packet_template_t *pkts, int len,
                  int (*post_fn)(void));
