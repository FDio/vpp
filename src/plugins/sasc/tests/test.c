...
void tcp_packet_info_init(tcp_packet_info_t *pkt, u32 src_ip, u32 dst_ip, u16 sport, u16 dport,
...
int verify_session_state(u32 session_id, session_verification_t *expected) {
...
static int verify_retransmit_detection(u32 session_id, bool expect_retransmit) {
    sasc_main_t *sasc = &sasc_main;
...
static int verify_reorder_detection(u32 session_id, bool expect_reorder) {
    sasc_main_t *sasc = &sasc_main;
    sasc_tcp_check_main_t *tcm = &sasc_tcp_check_main;
...
int sasc_test_tcp_sequence(tcp_test_sequence_t *sequence) {
    vlib_main_t *vm = vlib_get_main();
    u32 *bi_array = 0;
...
int run_udp_test(const char *name, const test_packet_template_t *pkts, int len,
    ...
    for (int i = 0; i < len; i++) {
        ...
        u8 *packet_def = 0;
        packet_def = format(
            0, "Ether()/IP(src='%U', dst='%U')/UDP(sport=%d, dport=%d)",
...
static int sasc_run_test_by_name(const char *test_name) {
...
static int sasc_run_all_tests(void) {
    int test_count = 0;
    int passed_count = 0;
    int failed_count = 0;
...
static void sasc_list_tests(void) {
    clib_warning("=== Available SASC Tests ===");

    const char *category_names[] = {"Basic", "TCP Lifecycle", "Retransmit", "Reorder", "Advanced", "UDP"};
...
static clib_error_t *sasc_test_list_command_fn(vlib_main_t *vm, unformat_input_t *input,
...
static clib_error_t *sasc_test_run_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                              vlib_cli_command_t *cmd) {
...
static int sasc_run_category(sasc_test_category_t category) {
    int passed_count = 0;
    int failed_count = 0;

    const char *category_names[] = {"Basic", "TCP Lifecycle", "Retransmit", "Reorder", "Advanced"};
...
static clib_error_t *sasc_test_category_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                   vlib_cli_command_t *cmd) {
    unformat_input_t line_input_, *line_input = &line_input_;
    clib_error_t *err = 0;
    u8 *category_name = 0;
    ...
    if (!category_name) {
        err = clib_error_return(
            0, "missing category name (basic, tcp-lifecycle, retransmit, reorder, advanced)");
        goto done;
    }
    ...
    if (!category_found) {
        err = clib_error_return(
            0, "unknown category '%s'. Use: basic, tcp-lifecycle, retransmit, reorder, advanced",
            category_name);
        goto done;
    }
...
VLIB_CLI_COMMAND(sasc_test_category_command, static) = {
    ...
    .short_help = "test sasc category <category> - Run all tests in a category (basic, "
                  "tcp-lifecycle, retransmit, reorder, advanced)",
    .function = sasc_test_category_command_fn,
};

/* Comprehensive test runner that executes all SASC tests */
static clib_error_t *sasc_test_all_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                              vlib_cli_command_t *cmd) {
...
static clib_error_t *sasc_test_pcap_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                               vlib_cli_command_t *cmd) {
    unformat_input_t line_input_, *line_input = &line_input_;
    pcap_main_t pcap_main = {0};
    clib_error_t *err = 0;
    char *pcap_filename = 0;
    u32 max_packets_per_frame = 256; /* VPP frame size limit */
    ...
    /* Process packets in batches of max_packets_per_frame */
    for (u32 batch_start = 0; batch_start < n_packets; batch_start += max_packets_per_frame) {
        u32 batch_size = clib_min(max_packets_per_frame, n_packets - batch_start);
    ...
    clib_warning("PCAP processing completed: %u packets processed in batches", n_packets);
...
VLIB_CLI_COMMAND(sasc_test_pcap_command, static) = {
    .path = "test sasc pcap",
    .short_help = "test sasc pcap <filename> - Run SASC tests with pcap capture",
