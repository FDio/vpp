...
static clib_error_t *sasc_test_pcap_command_fn(vlib_main_t *vm, unformat_input_t *input,
    ...
    u32 bi[n_packets];
    u32 n_buffers = vlib_buffer_alloc(vm, bi, n_packets);
    ASSERT(n_buffers == n_packets);

    vlib_node_t *lookup_node = vlib_get_node_by_name(vm, (u8 *)"sasc-lookup-ip4");
    vlib_frame_t *f = vlib_get_frame_to_node(vm, lookup_node->index);
    u32 *to_next = vlib_frame_vector_args(f);
    f->n_vectors = n_packets;

    for (u32 i = 0; i < n_packets; i++) {
        vlib_buffer_t *b = vlib_get_buffer(vm, bi[i]);
        u8 *pkt = pcap_main.packets_read[i];
        clib_warning("Packet length: %d", vec_len(pkt));
