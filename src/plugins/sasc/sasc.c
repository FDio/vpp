...
#define MAX_SASC_SESSIONS 1024 * 128
...
static_always_inline bool
sasc_add_node_to_chain(vlib_main_t *vm, vlib_node_t *node, u32 service_index, u32 **next_indices,
                       u32 *next_index) {
...
static bool
sasc_build_next_indices_from_effective_chain(vlib_main_t *vm, u32 *effective_services,
                                             vlib_node_t *start_node, u32 **next_indices) {
...
sasc_build_next_indices_from_effective_chains(sasc_main_t *sasc) {
    ...
    for (u32 ingress_idx = 0; ingress_idx < SASC_INGRESS_NODE_N_LOOKUPS; ingress_idx++) {
        ...
        for (u32 chain_id = 0; chain_id < max_chain_id; chain_id++) {
            for (sasc_proto_group_t proto_group = 0; proto_group < SASC_PROTO_GROUP_N;
                 proto_group++) {
                // Get the effective service chain
                u32 *effective_services =
                    sasc_get_effective_service_chain(sasc, chain_id, proto_group);
                ...
                u32 *next_indices = 0;
                bool success = sasc_build_next_indices_from_effective_chain(
                    vm, effective_services, ingress_node, &next_indices);
                ...
                if (success && vec_len(next_indices) > 0) {
                    ...
                    if (error_drop_node) {
                        vec_add1(next_indices, vlib_node_add_next(vm, ingress_node->index,
...
clib_error_t *
sasc_tenant_add_del(sasc_main_t *sasc, u32 tenant_idx, u32 context_id, u32 forward_chain_id,
                    u32 reverse_chain_id, u32 miss_chain_id, bool is_add) {
    clib_error_t *err = 0;
...
sasc_build_effective_service_chains(sasc_main_t *sasc) {
    ...
    for (u32 chain_id = 0; chain_id < max_chain_id; chain_id++) {
        ...
        for (sasc_proto_group_t proto_group = 0; proto_group < SASC_PROTO_GROUP_N; proto_group++) {
            ...
            vec_foreach (service_index, chain) {
                if (*service_index < vec_len(sm->services)) {
                    sasc_service_registration_t *service =
                        vec_elt_at_index(sm->services, *service_index)[0];
            ...
            sasc_log_debug("Built effective chain %d for proto_group %d with %d services", chain_id,
                           proto_group, vec_len(effective_services));
        }
    }
