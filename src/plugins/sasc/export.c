...
static const sasc_field_desc_t session_desc[] = {
    {"tenant_id", SASC_T_U32, offsetof(sasc_session_t, tenant_idx), NULL, NULL, 0, SASC_T_U32, 0},
...
static void
serialize_sasc_session_services_cbor(cbor_item_t *services_map, u32 thread_index, sasc_session_t *session) {
        ...
        if (*service_index < vec_len(sm->services) && sm->services[*service_index]->format_service_cbor) {
            ...
            if (service_obj) {
                bool success = cbor_map_add(
                    services_map,
                    (struct cbor_pair){.key = cbor_move(cbor_build_string(sm->services[*service_index]->node_name)),
                                       .value = cbor_move(service_obj)});
                if (!success) {
                    sasc_log_err("Failed to format service %s for session %U", sm->services[*service_index]->node_name,
                                 format_sasc_session_key, &sp->forward_key);
                }
            }
        }
    }
...
#endif
        case SASC_T_ARRAY: {
            // For array type, we need a getter function that returns the array data
            if (d->get_value) {
                d->get_value(st, session_index, &v);
    ...
    for (size_t i = 0; i < nfields; i++) {
        ...
        uint8_t type_id =
            ...
            (desc[i].type == SASC_T_ARRAY)       ? 24 :
                                                   255; // CBOR type 5 is text string, 23 for histogram, 24 for array
