    ...
    for (size_t i = 0; i < nfields; i++) {
        ...
        case SASC_T_SESSION_KEY: {
            if (d->get_value) {
                ...
                sasc_session_key_t key_copy = temp_v.session_key_val;
                if (!cbor_array_push(arr, cbor_move(cbor_build_session_key(&key_copy))))
                    return 0;
            } else {
...
format_field_value(u8 *s, const sasc_field_desc_t *d, const sasc_value_t *v, const void *st, u32 session_index,
    ...
    case SASC_T_SESSION_KEY: {
        if (d->get_value) {
            ...
            d->get_value(st, session_index, &temp_v);
            sasc_session_key_t key_copy = temp_v.session_key_val;
            return format(s, "%U%s: %U\n", format_white_space, indent, d->name, format_sasc_session_key, &key_copy);
        } else {
            sasc_session_key_t *k = (sasc_session_key_t *)((const u8 *)st + d->offset);
