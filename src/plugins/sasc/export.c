...
#define SASC_RING_ENTRY_SIZE 512 // 512 bytes fixed size per entry
        ...
        if (session->service_chain[SASC_FLOW_FORWARD] != 0xFFFF &&
            ...
            vec_foreach (service_index, forward_chain) {
                if (*service_index < vec_len(sm->services) &&
                    ...
                    if (!sm->services[*service_index]->format_service_cbor(
                            session_obj, thread_index, session_idx)) {
...
static clib_error_t *sasc_dump_sessions_toring_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                   vlib_cli_command_t *cmd) {
    sasc_ring_init();
...
VLIB_CLI_COMMAND(dump_sasc_session_toring_command, static) = {
    .path = "dump sasc session toring",
    .short_help = "dump sasc session toring",
    .function = sasc_dump_sessions_toring_command_fn,
};
