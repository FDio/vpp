...
static clib_error_t *
sasc_dump_sessions_toring_command_fn(vlib_main_t *vm, unformat_input_t *input,
...
VLIB_CLI_COMMAND(dump_sasc_session_toring_command, static) = {
    ...
    .short_help = "dump sasc session toring <ring_size>",
...
sasc_test_batch_export_command_fn(vlib_main_t *vm, unformat_input_t *input,
    ...
    if (sasc_stats_ring_index == CLIB_U32_MAX) {
        return clib_error_return(
            0, "Ring buffer not initialized. Run 'dump sasc session toring' first.");
    }
