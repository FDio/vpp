 ...
 */
static clib_error_t *sasc_show_services_command_fn(vlib_main_t *vm, unformat_input_t *input,
...
static clib_error_t *sasc_set_services_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                  vlib_cli_command_t *cmd) {
...
static clib_error_t *show_sasc_service_chains_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                         vlib_cli_command_t *cmd) {
    ...
    vec_foreach_index (chain_index, sasc->chains) {
        ...
        clib_warning("vec_len(chain): %d", vec_len(chain));
...
static clib_error_t *sasc_tenant_add_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                vlib_cli_command_t *cmd) {
    unformat_input_t line_input_, *line_input = &line_input_;
    clib_error_t *err = 0;
    sasc_main_t *sasc = &sasc_main;
...
#undef _
    ...
    err = sasc_tenant_add_del(sasc, tenant_idx, context_id, chain_ids[SASC_SERVICE_CHAIN_FORWARD],
                              chain_ids[SASC_SERVICE_CHAIN_REVERSE],
                              chain_ids[SASC_SERVICE_CHAIN_MISS], true);
done:
    unformat_free(line_input);
    return err;
}
...
VLIB_CLI_COMMAND(sasc_tenant_add_del_command, static) = {
    ...
    .short_help =
        "set sasc tenant <tenant-index> context <context-id> forward <service-chain-id> reverse "
        "<service-chain-id> miss <service-chain-id>",
    .function = sasc_tenant_add_command_fn,
};

static clib_error_t *sasc_show_tenant_detail_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                        vlib_cli_command_t *cmd) {
    unformat_input_t line_input_, *line_input = &line_input_;
    clib_error_t *err = 0;
    sasc_main_t *sasc = &sasc_main;
...
static clib_error_t *sasc_show_sessions_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                   vlib_cli_command_t *cmd) {
    unformat_input_t line_input_, *line_input = &line_input_;
    clib_error_t *err = 0;
    sasc_main_t *sasc = &sasc_main;
    ...
    pool_foreach (session, sasc->sessions) {
        ...
        vlib_cli_output(vm, "%u %d %U %U %U %d", session - sasc->sessions, session->tenant_idx,
                        format_sasc_session_key, &session->forward_key, format_ip_protocol,
                        session->keys[0].proto, format_sasc_session_state, session->state,
                        (int)remaining_time);
        vlib_cli_output(vm, "  F-chain: %U", format_sasc_service_chain,
                        session->service_chain[SASC_FLOW_FORWARD]);
        vlib_cli_output(vm, "  R-chain: %U", format_sasc_service_chain,
                        session->service_chain[SASC_FLOW_REVERSE]);

        if (detail)
            vlib_cli_output(vm, "  %U", format_sasc_session_detail, session);
    }
    return 0;
}
...
VLIB_CLI_COMMAND(show_sasc_sessions_command, static) = {
    ...
    .short_help = "show sasc session [session index] [thread <n>] [tenant <tenant-id>] "
                  "[0x<session-id>] [detail]",
    .function = sasc_show_sessions_command_fn,
};

static clib_error_t *sasc_show_next_indices_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                       vlib_cli_command_t *cmd) {
    sasc_main_t *sasc = &sasc_main;
    vlib_node_t *node;
    u32 *next_indices;
    ...
    vlib_cli_output(vm, "Service Chain Next Indices:");
    vlib_cli_output(vm, "==========================");

    // Iterate through configured service chains
    vec_foreach_index (chain_id, sasc->chains) {
        u32 *chain = *vec_elt_at_index(sasc->chains, chain_id);
        if (!chain)
            continue;

        vlib_cli_output(vm, "\nService Chain %d:", chain_id);
        vlib_cli_output(vm, "-------------------");
        vlib_cli_output(vm, "  Services: %U", format_sasc_service_chain, chain_id);

        // Show next indices for each ingress node
        for (int ingress = 0; ingress < SASC_INGRESS_NODE_N_LOOKUPS; ingress++) {
            const char *ingress_names[] = {[SASC_INGRESS_NODE_LOOKUP_IP4] = "sasc-lookup-ip4"};
            vlib_cli_output(vm, "\n  Ingress Node: %s", ingress_names[ingress]);

            // Show next indices for each protocol group
            for (int proto = 0; proto < SASC_PROTO_GROUP_N; proto++) {
                const char *proto_names[] = {
                    [SASC_PROTO_GROUP_ALL] = "All", [SASC_PROTO_GROUP_TCP] = "TCP"};

                // Calculate next indices index using the same formula as sasc_buffer_init_chain
                next_indices_index = (ingress * SASC_MAX_CHAINS * SASC_PROTO_GROUP_N) +
                                     (chain_id * SASC_PROTO_GROUP_N) + proto;

                if (next_indices_index >= vec_len(sasc->next_indices) ||
                    !sasc->next_indices[next_indices_index])
                    continue;
                ...
                vlib_cli_output(vm, "    Protocol Group: %s", proto_names[proto]);
                vlib_cli_output(vm, "      Next Nodes:");

                // Get the ingress node
                u32 ingress_node_index = sasc_ingress_node_index(ingress);
                node = vlib_get_node(vm, ingress_node_index);
                if (!node)
                    continue;

                // Show the chain of next nodes
                for (int i = 0; i < vec_len(next_indices); i++) {
                    u32 next_node_index = node->next_nodes[next_indices[i]];
                    vlib_node_t *next_node = vlib_get_node(vm, next_node_index);
                    if (next_node) {
                        vlib_cli_output(vm, "        %s (local index: %d, global index: %d)",
                                        next_node->name, next_indices[i], next_node_index);
...
static clib_error_t *sasc_set_timeout_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                 vlib_cli_command_t *cmd) {
    unformat_input_t line_input_, *line_input = &line_input_;
    clib_error_t *err = 0;
    sasc_main_t *sasc = &sasc_main;
...
VLIB_CLI_COMMAND(sasc_set_timeout_command,
                 static) = {.path = "set sasc timeout",
                            .short_help = "set sasc timeout"
                                          " <timeout-name> <timeout-value>",
                            .function = sasc_set_timeout_command_fn};

static clib_error_t *sasc_show_summary_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                  vlib_cli_command_t *cmd) {
    sasc_main_t *sasc = &sasc_main;

    vlib_cli_output(vm, "Configuration:");
    //   vlib_cli_output(vm, "Max Tenants: %d", sasc_cfg_main.no_tenants);
    //   vlib_cli_output(vm, "Max Sessions: %d", sasc_cfg_main.no_sessions);
...
static clib_error_t *sasc_clear_sessions_command_fn(vlib_main_t *vm, unformat_input_t *input,
                                                    vlib_cli_command_t *cmd) {
    sasc_session_clear();
    return 0;
}
