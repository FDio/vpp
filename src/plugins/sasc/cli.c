...
static clib_error_t *sasc_show_sessions_command_fn(vlib_main_t *vm, unformat_input_t *input,
    ...
    pool_foreach (session, sasc->sessions) {
        ...
        vlib_cli_output(vm, "%u %d %U %U %U %d", session - sasc->sessions, session->tenant_idx,
                        ...
                        (int)remaining_time);
        vlib_cli_output(vm, "  F-chain: %U", format_sasc_effective_service_chain,
                        session->service_chain[SASC_FLOW_FORWARD]);
        vlib_cli_output(vm, "  R-chain: %U", format_sasc_effective_service_chain,
    ...
    for (u32 chain_id = 0; chain_id < max_chain_id; chain_id++) {
        ...
        for (sasc_proto_group_t proto_group = 0; proto_group < SASC_PROTO_GROUP_N; proto_group++) {
            ...
            vlib_cli_output(vm, "    Effective Services: %U\n",
                            format_sasc_service_chain_from_vector, effective_services);
                ...
                vec_foreach (next_index, next_indices) {
                    ...
                    if (next_node) {
                        vlib_cli_output(vm, "        %s (local index: %d, global index: %d)",
                                        next_node->name, *next_index, next_node_index);
