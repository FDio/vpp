...
#undef _
    ...
    err = sasc_tenant_add_del(sasc, tenant_idx, context_id, chain_ids[SASC_SERVICE_CHAIN_FORWARD],
                              ...
                              chain_ids[SASC_SERVICE_CHAIN_MISS],
...
sasc_show_sessions_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd) {
    ...
    if (session_index != ~0) {
        ...
        vlib_cli_output(
            vm, "%u %d %U %U %U %d %d/%d %s", session - sasc->sessions, session->tenant_idx,
            format_sasc_session_key, &session->forward_key, format_ip_protocol,
            session->keys[0].proto, format_sasc_session_state, session->state, (int)remaining_time,
        ...
        pool_foreach (session, sasc->sessions) {
            ...
            vlib_cli_output(vm, "%u %d %U %U %U %d %d/%d %s", session - sasc->sessions,
                            session->tenant_idx, format_sasc_session_key, &session->forward_key,
                            format_ip_protocol, session->keys[0].proto, format_sasc_session_state,
                            session->state, (int)remaining_time,
