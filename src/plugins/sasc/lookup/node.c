...
sasc_lookup_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_ip6,
    ...
    again:
        ...
        if (sasc_lookup_with_hash(h[0], k, &value)) {
            ...
            sasc_service_chain_type_t chain_type = k->proto == SASC_IP_PROTOCOL_ICMP_ERROR ?
                                                       SASC_SERVICE_CHAIN_ICMP_ERROR :
                                                       SASC_SERVICE_CHAIN_MISS;
            ...
            if (lookup_mode == SASC_LOOKUP_MODE_DEFAULT) {
                u32 effective_index = sasc_get_effective_service_chain_index(
                    sasc, tenant->service_chains[chain_type], sasc_proto_to_group(k->proto));
        ...
        if (flow_thread_index == thread_index) {
            ...
            /* Propagate PCAP sampling from session to buffer */
