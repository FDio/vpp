...
int sasc_lookup_with_hash(u64 hash, sasc_session_key_t *k, u64 *v) {
...
int sasc_lookup(sasc_session_key_t *k, u64 *v) {
    u64 h = clib_bihash_hash_40_8((clib_bihash_kv_40_8_t *)(k));
    return sasc_lookup_with_hash(h, k, v);
}

static_always_inline uword sasc_lookup_inline(vlib_main_t *vm, vlib_node_runtime_t *node,
    ...
    while (n_left) {
        sasc_calc_key(b[0], sasc_buffer(b[0])->context_id, k, h, is_ip6, lookup_mode);
        h += 1;
        k += 1;
        b += 1;
        n_left -= 1;
    ...
    again:
        ...
        if (sasc_lookup_with_hash(h[0], k, &value)) {
            ...
            //   sasc_buffer(b[0])->service_bitmap =
            //     is_ip6 ? SASC_SERVICE_MASK_ICMP6_ERROR_FWD : SASC_SERVICE_MASK_ICMP_ERROR_FWD;
            // } else {
            ...
            if (lookup_mode == SASC_LOOKUP_MODE_DEFAULT) {
                ...
                u32 effective_index = sasc_get_effective_service_chain_index(
                    sasc, tenant->service_chains[SASC_SERVICE_CHAIN_MISS], sasc_proto_to_group(k->proto));
        ...
        if (flow_thread_index == thread_index) {
            ...
            if (sasc_session_is_expired(session, now)) {
                clib_warning("***Expired session: %u %U (%.02f)", session_index,
                             format_sasc_session_key, k, sasc_session_remaining_time(session, now));
                sasc_log_warn("Expired session: %u %U (%.02f)", session_index,
            ...
            sasc_buffer_init_chain(
                SASC_INGRESS_NODE_LOOKUP_IP4, k->proto, b[0],
                session->service_chain[sasc_direction_from_flow_index(flow_index)]);
            ...
            session->bytes[sasc_direction_from_flow_index(flow_index)] +=
                sasc_get_l3_length(vm, b[0]);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left) {
    ...
    sasc_session_t *session = sasc_session_at_index_check(vcdp, session_index);
    if (!session) {
    ...
    if (sasc_session_is_expired(session, now)) {
      ...
      sasc_session_remove(vcdp, session, thread_index, session_index);
 ...
 */
static u8 *format_sasc_lookup_trace(u8 *s, va_list *args) {
    ...
    s = format(s, "\n%Urx ifindex %d, hash 0x%x flow-id %u", format_white_space, indent,
               t->sw_if_index, t->hash, t->flow_id);
