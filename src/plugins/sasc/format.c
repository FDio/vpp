...
u8 *format_sasc_session_key(u8 *s, va_list *args) {
    sasc_session_key_t *key = va_arg(*args, sasc_session_key_t *);
    u32 context_id = key->context_id;
    s = format(s, "%d: %U: %U:%d %U:%d", context_id, format_ip_protocol, key->proto,
               format_ip46_address, &key->src, IP46_TYPE_ANY, clib_net_to_host_u16(key->sport),
               format_ip46_address, &key->dst, IP46_TYPE_ANY, clib_net_to_host_u16(key->dport));
    return s;
}
 ...
 */
u8 *format_sasc_service_chain_helper(u8 *s, u32 *service_indices) {
    if (!service_indices) {
        return s;
    }
...
u8 *format_sasc_service_chain(u8 *s, va_list *args) {
...
u8 *format_sasc_service_chain_from_vector(u8 *s, va_list *args) {
 ...
 */
u8 *format_sasc_effective_service_chain(u8 *s, va_list *args) {
...
u8 *format_sasc_tenant(u8 *s, va_list *args) {
...
#undef _
    ...
    for (int i = 0; i < SASC_SERVICE_CHAIN_N; i++) {
        if (tenant->service_chains[i] != ~0U) {
            s = format(s, "%U%s: %d: %U\n", format_white_space, indent, chain_names[i],
                       tenant->service_chains[i], format_sasc_service_chain,
                       tenant->service_chains[i]);
        }
    }
    return s;
}

u8 *format_sasc_tenant_extra(u8 *s, va_list *args) {
    ...
    // ctr = vlib_get_simple_counter(&vcdp->tenant_simple_ctr[VCDP_TENANT_COUNTER_CREATED],
    // tenant_idx); s = format(s, "%Ucreated: %llu\n", format_white_space, indent + 2, ctr); ctr =
    // vlib_get_simple_counter(&vcdp->tenant_simple_ctr[VCDP_TENANT_COUNTER_REMOVED], tenant_idx);
    // s = format(s, "%Uexpired: %llu\n", format_white_space, indent + 2, ctr);

    // vlib_get_combined_counter(&vcdp->tenant_combined_ctr[VCDP_TENANT_COUNTER_RX], tenant_idx,
    ...
    // vlib_get_combined_counter(&vcdp->tenant_combined_ctr[VCDP_TENANT_COUNTER_TX], tenant_idx,
...
u8 *format_sasc_session_state(u8 *s, va_list *args) {
...
u8 *format_sasc_session_detail(u8 *s, va_list *args) {
    ...
    s = format(s, "\n%Upackets: %llu / %llu", format_white_space, 2, session->pkts[SASC_FLOW_FORWARD], session->pkts[SASC_FLOW_REVERSE]);
    s = format(s, "\n%Ubytes: %llu / %llu", format_white_space, 2, session->bytes[SASC_FLOW_FORWARD], session->bytes[SASC_FLOW_REVERSE]);
    s = format(s, "\n%Uduration: %u\n", format_white_space, 2, session->last_heard - session->created);
    ...
    if (session->service_chain[SASC_FLOW_FORWARD] != 0xFFFF) {
        ...
        vec_foreach (service_index, forward_chain) {
            if (*service_index < vec_len(sm->services) && 
                sm->services[*service_index]->format_service)
                s = sm->services[*service_index]->format_service(s, thread_index, session_idx);
        }
    }
    return s;
}
