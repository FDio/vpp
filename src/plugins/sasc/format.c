...
format_sasc_session_detail(u8 *s, va_list *args) {
    ...
    s = format(s, "\n%Upackets: %llu / %llu", format_white_space, 2,
               session->pkts[SASC_FLOW_FORWARD], session->pkts[SASC_FLOW_REVERSE]);
    s = format(s, "\n%Ubytes: %llu / %llu", format_white_space, 2,
               session->bytes[SASC_FLOW_FORWARD], session->bytes[SASC_FLOW_REVERSE]);
    s = format(s, "\n%Uduration: %u\n", format_white_space, 2,
               session->last_heard - session->created);

    if (session->service_chain[SASC_FLOW_FORWARD] != 0xFFFF) {
        u32 *forward_chain =
            sasc->effective_service_chains[session->service_chain[SASC_FLOW_FORWARD]];
