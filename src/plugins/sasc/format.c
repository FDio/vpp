...
u8 *format_sasc_service_chain(u8 *s, va_list *args) {
    ...
    // remove the last space if we added any services
    if (vec_len(chain) > 0 && vec_len(s) > 0) {
        s[vec_len(s) - 1] = '\0';
    }
    return s;
}
...
u8 *format_sasc_session_detail(u8 *s, va_list *args) {
    ...
    if (session->service_chain[SASC_FLOW_FORWARD] != 0xFFFF && 
        ...
        u32 *forward_chain = *vec_elt_at_index(sasc->chains, session->service_chain[SASC_FLOW_FORWARD]);
