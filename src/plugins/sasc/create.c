...
sasc_create_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_ip6) {
    ...
    while (n_left) {
        ...
        if (session) {
            ...
            // Sample at most one new TCP session per minute (per-thread)
            // If sampled, mark both session and buffer so the entire session is traced.
            if (k->proto == IP_PROTOCOL_TCP) {
                static __thread u32 last_sampled_sec;
                if (last_sampled_sec == 0 || (now_sec - last_sampled_sec) >= 60) {
                    last_sampled_sec = now_sec;
                    session->flags |= SASC_SESSION_F_PCAP_SAMPLE;
                    b[0]->flags |= SASC_BUFFER_F_PCAP_TRACE;
                }
            }
