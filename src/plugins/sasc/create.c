...
static u8 *format_sasc_create_trace(u8 *s, va_list *args) {
...
void sasc_set_session_service_chain(sasc_tenant_t *tenant, sasc_session_t *session, u8 proto) {
    ...
    //         tenant->service_chains[SASC_SERVICE_CHAIN_TCP_FORWARD];
    //     session->service_chain[SASC_FLOW_REVERSE] =
    //         tenant->service_chains[SASC_SERVICE_CHAIN_TCP_REVERSE];
    // } else {
        session->service_chain[SASC_FLOW_FORWARD] =
            tenant->service_chains[SASC_SERVICE_CHAIN_FORWARD];
        session->service_chain[SASC_FLOW_REVERSE] =
            tenant->service_chains[SASC_SERVICE_CHAIN_REVERSE];
    // }
}
...
//
static_always_inline uword sasc_create_inline(vlib_main_t *vm, vlib_node_runtime_t *node,
    ...
    while (n_left) {
        ...
        sasc_session_t *session = sasc_create_session(tenant_idx, k, &reverse_key, false, &flow_index);
        if (session) {
            // Initialize buffer with forward service chain for this protocol group
            sasc_buffer_init_chain(SASC_INGRESS_NODE_LOOKUP_IP4, k->proto, b[0], session->service_chain[SASC_FLOW_FORWARD]);
        } else {
            error = SASC_CREATE_ERROR_NO_KEY;
