...
static u8 *format_sasc_l4_lifecycle_trace(u8 *s, va_list *args) {
    vlib_main_t __clib_unused *vm = va_arg(*args, vlib_main_t *);
    vlib_node_t __clib_unused *node = va_arg(*args, vlib_node_t *);
    sasc_l4_lifecycle_trace_t *t = va_arg(*args, sasc_l4_lifecycle_trace_t *);

    s = format(s, "sasc-l4-lifecycle: flow-id %u (session %u, %s) new_state: %U", t->flow_id,
               t->flow_id >> 1, t->flow_id & 0x1 ? "reverse" : "forward", format_sasc_session_state,
               t->new_state);
    return s;
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame) {
    ...
    while (n_left) {
        ...
        if (session->state == SASC_SESSION_STATE_FSOL && direction == SASC_FLOW_REVERSE) {
            ...
            sasc_log_debug("Establishing session [%u] %U", session_idx, format_sasc_session_key,
                           &session->forward_key);
