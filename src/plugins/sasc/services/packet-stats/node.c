...
static f64
compute_q_delivery(u32 icmp_unreach, u32 icmp_frag_needed, u32 icmp_ttl_expired,
...
static f64
compute_q_packetization(const sasc_session_t *session,
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame) {
    ...
    while (n_left) {
        ...
        welford_update(&session_data->ttl_mean, &session_data->ttl_m2, &session_data->ttl_count,
                       ttl);
        ...
        welford_update(&session_data->iat_mean, &session_data->iat_m2, &session_data->iat_count,
                       iat);
...
u8
sasc_quality_compute(const sasc_session_t *session,
                     sasc_packet_stats_session_data_t *session_data) {
    ...
    session_data->iat_cv =
        (session_data->iat_mean > 0) ? session_data->iat_stddev / session_data->iat_mean : 0.0;
    ...
    session_data->q_continuity = compute_q_continuity(
        session_data->burst_count, session_data->idle_periods, session_data->frames_touched);
    session_data->q_delivery =
        compute_q_delivery(session_data->icmp_unreach, session_data->icmp_frag_needed,
                           session_data->icmp_ttl_expired, session_data->icmp_packet_too_big);
    ...
    session_data->quality_score =
        (session_data->q_stability * 0.40) + (session_data->q_congestion * 0.25) +
...
/* ICMP event hooks */
void
sasc_quality_note_icmp_unreach(u32 session_index) {
    sasc_packet_stats_main_t *psm = &sasc_packet_stats_main;
    sasc_packet_stats_session_data_t *session_data = &psm->session_data[session_index];
    session_data->icmp_unreach++;
}

void
sasc_quality_note_icmp_frag_needed(u32 session_index) {
    sasc_packet_stats_main_t *psm = &sasc_packet_stats_main;
    sasc_packet_stats_session_data_t *session_data = &psm->session_data[session_index];
    session_data->icmp_frag_needed++;
}

/* ICMP error callback function */
void
sasc_packet_stats_icmp_error_callback(const sasc_icmp_error_info_t *error_info) {
    sasc_packet_stats_main_t *psm = &sasc_packet_stats_main;
    sasc_packet_stats_session_data_t *session_data = &psm->session_data[error_info->session_index];

    // Update the appropriate counter based on error type
    ...
    case SASC_ICMP_ERROR_TTL_EXPIRED:
        session_data->icmp_ttl_expired++;
        break;
    case SASC_ICMP_ERROR_PACKET_TOO_BIG:
        session_data->icmp_packet_too_big++;
        break;
    default:
        // Unknown error type, count as unreachable
        session_data->icmp_unreach++;
        break;
    }
}

// Field descriptions for schema generation
static const sasc_field_desc_t packet_stats_desc[] = {
    {"quality_score", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, quality_score), -1,
     NULL, NULL, NULL, NULL, false},
    {"q_stability", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, q_stability), -1, NULL,
     NULL, NULL, NULL, false},
    {"q_congestion", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, q_congestion), -1, NULL,
     NULL, NULL, NULL, false},
    {"q_continuity", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, q_continuity), -1, NULL,
     NULL, NULL, NULL, false},
    {"q_delivery", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, q_delivery), -1, NULL,
     NULL, NULL, NULL, false},
    {"q_packetization", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, q_packetization), -1,
     NULL, NULL, NULL, NULL, false},
    {"tiny_packets", SASC_T_U32, offsetof(sasc_packet_stats_session_data_t, tiny_packets), -1, NULL,
     NULL, NULL, NULL, false},
    {"iat_mean", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, iat_mean), -1, NULL, NULL,
     NULL, NULL, false},
    {"iat_stddev", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, iat_stddev), -1, NULL,
     NULL, NULL, NULL, false},
    {"iat_cv", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, iat_cv), -1, NULL, NULL, NULL,
     NULL, false},
    {"ecn_ect", SASC_T_U32, offsetof(sasc_packet_stats_session_data_t, ecn_ect), -1, NULL, NULL,
     NULL, NULL, false},
    {"ecn_ce", SASC_T_U32, offsetof(sasc_packet_stats_session_data_t, ecn_ce), -1, NULL, NULL, NULL,
     NULL, false},
    {"ttl_min", SASC_T_U8, offsetof(sasc_packet_stats_session_data_t, ttl_min), -1, NULL, NULL,
     NULL, NULL, false},
    {"ttl_max", SASC_T_U8, offsetof(sasc_packet_stats_session_data_t, ttl_max), -1, NULL, NULL,
     NULL, NULL, false},
    {"ttl_mean", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, ttl_mean), -1, NULL, NULL,
     NULL, NULL, false},
    {"ttl_stddev", SASC_T_F64, offsetof(sasc_packet_stats_session_data_t, ttl_stddev), -1, NULL,
     NULL, NULL, NULL, false},
    {"dscp_changes", SASC_T_U32, offsetof(sasc_packet_stats_session_data_t, dscp_changes), -1, NULL,
     NULL, NULL, NULL, false},
    {"icmp_unreach", SASC_T_U32, offsetof(sasc_packet_stats_session_data_t, icmp_unreach), -1, NULL,
     NULL, NULL, NULL, false},
    {"icmp_frag_needed", SASC_T_U32, offsetof(sasc_packet_stats_session_data_t, icmp_frag_needed),
     -1, NULL, NULL, NULL, NULL, false},
    {"icmp_ttl_expired", SASC_T_U32, offsetof(sasc_packet_stats_session_data_t, icmp_ttl_expired),
     -1, NULL, NULL, NULL, NULL, false},
    {"icmp_packet_too_big", SASC_T_U32,
     offsetof(sasc_packet_stats_session_data_t, icmp_packet_too_big), -1, NULL, NULL, NULL, NULL,
     false},
};

static const size_t packet_stats_field_count =
    sizeof(packet_stats_desc) / sizeof(packet_stats_desc[0]);
...
format_sasc_packet_stats_service(u8 *s, u32 thread_index, u32 session_index, bool detail) {
    ...
    if (!detail) {
        return format(s,
                      ...
                      format_white_space, 2, session_data->quality_score,
                      session_data->iat_mean * 1000, session_data->iat_stddev * 1000,
                      session_data->ttl_min, session_data->ttl_max, session_data->ecn_ect,
                      session_data->ecn_ce,
                      session_data->icmp_unreach + session_data->icmp_frag_needed +
                          session_data->icmp_ttl_expired + session_data->icmp_packet_too_big,
                      session_data->tiny_packets);
    }
...
export_packet_stats_schema(void) {
    return sasc_export_schema_generic("packet_stats", packet_stats_desc, packet_stats_field_count,
                                      1);
}
