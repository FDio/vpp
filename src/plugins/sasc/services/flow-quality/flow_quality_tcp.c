...
/* Welford update for RTT */
static inline void
rtt_update(sasc_tcp_quality_session_data_t *st, f64 sample) {
    st->rtt_count++;
    f64 delta = sample - st->rtt_mean;
    st->rtt_mean += delta / st->rtt_count;
    st->rtt_m2 += delta * (sample - st->rtt_mean);
}

static inline f64
rtt_stddev(const sasc_tcp_quality_session_data_t *st) {
    if (st->rtt_count < 2)
        return 0.0;
    f64 var = st->rtt_m2 / (st->rtt_count - 1);
...
fq_tcp_on_packet(vlib_main_t *vm, vlib_buffer_t *b, u8 dir, tcp_header_t *th, u32 session_index, f64 now) {
    ...
    const u32 payload_len = l3_len - vnet_buffer(b)->l4_hdr_offset - tcp_hlen;
...
fq_tcp_scorer(u8 protocol, u32 session_index) {
    ...
    /* RTT variability penalty based on relative jitter (not absolute SD)
    ...
    f64 rtt_relative_variability = 0.0;
    if (st->rtt_mean > 0.001) { /* Only calculate if we have meaningful RTT data (>1ms) */
        rtt_relative_variability = rtt_sd / st->rtt_mean;
    }

    /* Congestion: loss + TCP ECN + relative RTT jitter */
    f64 cong_delta = loss_penalty + ecn_penalty +
                     clib_min(10.0, rtt_relative_variability * 100.0); /* 0.1 -> 10, 0.2 -> 10 (capped) */
    core->q_congestion = clib_max(0.0, core->q_congestion - cong_delta);

    /* Stability: relative RTT jitter influence (steeper than congestion term) */
    f64 stab_delta = clib_min(15.0, rtt_relative_variability * 150.0); /* 0.1 -> 15, 0.2 -> 15 (capped) */
