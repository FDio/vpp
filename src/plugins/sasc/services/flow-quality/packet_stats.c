...
const sasc_field_desc_t flow_quality_tcp_desc[] = {
    ...
    {"last_data_seq", SASC_T_ARRAY, offsetof(sasc_tcp_quality_session_data_t, last_data_seq), NULL, NULL, 0, SASC_T_U32,
     2},
...
static bool
is_tcp(const void *data, u32 session_index) {
...
format_sasc_packet_stats_service(u8 *s, u32 thread_index, u32 session_index, bool detail) {
    ...
    if (!detail) {
        return format(s,
                      ...
                      session_data->iat_stddev * 1000, session_data->ttl_min[0],
                      session_data->ttl_max[0],                           /* Forward direction */
