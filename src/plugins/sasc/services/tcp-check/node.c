...
static bool
format_sasc_tcp_check_service_cbor(cbor_item_t *s, u32 thread_index, u32 session_index) {
    sasc_tcp_check_main_t *tcm = &sasc_tcp_check_main;
    sasc_tcp_check_session_state_t *tcp_session = &tcm->state[session_index];
    ...
    success &= cbor_array_push(s, obj);
    if (!success) {
        sasc_log_err("Failed to add TCP check service to CBOR array");
        cbor_decref(&obj);
        return false;
    }
    return true;
    ...
    if (flags & SASC_TCP_CHECK_TCP_FLAGS_ACK && data_len == 0) {
        if (acknum == tcp_session->last_dup_ack_seq[dir]) {
            ...
            if (tcp_session->dup_ack_count[dir] >= 3) {
                ...
                vlib_increment_simple_counter(&stcm->counters[SASC_TCP_CHECK_COUNTER_FAST_RETRANSMIT],
                                              thread_index, tenant_idx, 1);
                sasc_log_warn(
