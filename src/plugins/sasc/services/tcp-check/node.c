...
check_tcp_anomalies(sasc_tcp_check_main_t *stcm, u32 thread_index, u16 tenant_idx,
    ...
    if (data_len > 0) {
        ...
        if (tcp_session->last_data_len[dir] > 0 && seqnum == tcp_session->last_seq[dir]) {
            is_retransmit = true;
            sasc_log_debug("Retransmit detected: dir=%u, seq=%u, last_seq=%u, last_data_len=%u",
                           dir, seqnum, tcp_session->last_seq[dir],
            ...
            is_retransmit = true;
            sasc_log_debug(
                "Payload retransmit detected: dir=%u, seq=%u, expected_seq=%u, last_data_len=%u",
                dir, seqnum, tcp_session->expected_seq[dir], tcp_session->last_data_len[dir]);
        }
    }

    /* Debug logging for retransmit detection */
    if (data_len > 0) {
        sasc_log_debug("Retransmit check: dir=%u, seq=%u, ack=%u, data_len=%u, last_seq=%u, "
                       "last_data_len=%u, expected_seq=%u",
                       dir, seqnum, acknum, data_len, tcp_session->last_seq[dir],
                       tcp_session->last_data_len[dir], tcp_session->expected_seq[dir]);
    }
    ...
    if (is_retransmit) {
        ...
        sasc_log_debug("TCP retransmit: session %u, dir %u, count %u, delay %.3fms, "
                       ...
                       retransmit_delay * 1000.0, tcp_session->retransmit_burst_count[dir], seqnum,
                       acknum);
        ...
        if (retransmit_rate > 0.05) {
            sasc_log_debug("High retransmission rate: %.2f%% in direction %u for session %u",
                           retransmit_rate * 100.0, dir, session - sasc_main.sessions);
        }
        if (tcp_session->retransmit_burst_count[dir] > 3) {
            sasc_log_debug("Retransmission burst detected: %u consecutive retransmits in "
        ...
        if (tcp_session->expected_seq[dir] != 0 && data_len > 0 &&
            ...
            if (seq_diff < tolerance && (peer_win == 0 || seq_diff < peer_win)) {
                bool peer_implies_reorder = false;
                ...
                vlib_increment_simple_counter(&stcm->counters[SASC_TCP_CHECK_COUNTER_REORDER],
                                              thread_index, tenant_idx, 1);
                if (peer_implies_reorder) {
                    sasc_log_debug(
                        "TCP reorder (SACK corroborated): session %u, direction %u, count %u, "
                        "expected_seq %u, actual_seq %u, diff %u, peer_window=%u",
                        session - sasc_main.sessions, dir, tcp_session->reorder_count[dir],
                        expected, seqnum, seq_diff, peer_win);
                } else {
                    sasc_log_warn("TCP reorder detected: session %u, direction %u, count %u, "
                                  "expected_seq %u, actual_seq %u, diff %u, peer_window=%u",
                                  session - sasc_main.sessions, dir,
                                  tcp_session->reorder_count[dir], expected, seqnum, seq_diff,
                                  peer_win);
                }
            }
        }
