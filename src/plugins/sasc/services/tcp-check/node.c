...
static u8 *format_sasc_tcp_check_trace(u8 *s, va_list *args) {
...
static_always_inline f64 calculate_retransmission_rate(sasc_tcp_check_session_state_t *tcp_session, u8 dir) {
...
static_always_inline f64 calculate_avg_retransmit_delay(sasc_tcp_check_session_state_t *tcp_session, u8 dir) {
    if (tcp_session->retransmit_delay_count[dir] == 0) return 0.0;
...
static u8 *format_sasc_tcp_check_service(u8 *s, u32 thread_index, u32 session_index) {
    ...
    if (tcp_session->retransmit_count[SASC_FLOW_FORWARD] > 0 ||
        ...
        s = format(s, ", retransmits fwd:%u(%.2f%%,%.1fms) rev:%u(%.2f%%,%.1fms)",
                   tcp_session->retransmit_count[SASC_FLOW_FORWARD], fwd_rate * 100.0, fwd_delay * 1000.0,
                   tcp_session->retransmit_count[SASC_FLOW_REVERSE], rev_rate * 100.0, rev_delay * 1000.0);
    }
...
static void format_sasc_tcp_check_service_cbor(cbor_item_t *s, u32 thread_index, u32 session_index) {
    sasc_tcp_check_main_t *tcm = &sasc_tcp_check_main;
    sasc_tcp_check_session_state_t *tcp_session = &tcm->state[session_index];
    ...
    /* Basic info */
    cbor_map_add(obj, (struct cbor_pair){
        .key = cbor_move(cbor_build_string("session")),
        .value = cbor_move(cbor_build_uint32(session_index))
    });
    
    cbor_map_add(obj, (struct cbor_pair){
        .key = cbor_move(cbor_build_string("flags")),
        .value = cbor_move(cbor_build_uint32(tcp_session->flags))
    });

    /* Retransmit info */
    if (tcp_session->retransmit_count[SASC_FLOW_FORWARD] > 0 || tcp_session->retransmit_count[SASC_FLOW_REVERSE] > 0) {
        cbor_item_t *retransmit = cbor_new_definite_map(2);
        cbor_map_add(retransmit, (struct cbor_pair){
            .key = cbor_move(cbor_build_string("forward")),
            .value = cbor_move(cbor_build_uint32(tcp_session->retransmit_count[SASC_FLOW_FORWARD]))
        });
        cbor_map_add(retransmit, (struct cbor_pair){
            .key = cbor_move(cbor_build_string("reverse")),
            .value = cbor_move(cbor_build_uint32(tcp_session->retransmit_count[SASC_FLOW_REVERSE]))
        });
        cbor_map_add(obj, (struct cbor_pair){
            .key = cbor_move(cbor_build_string("retransmits")),
            .value = cbor_move(retransmit)
        });
    }

    /* Reorder info */
    if (tcp_session->reorder_count[SASC_FLOW_FORWARD] > 0 || tcp_session->reorder_count[SASC_FLOW_REVERSE] > 0) {
        cbor_item_t *reorder = cbor_new_definite_map(2);
        cbor_map_add(reorder, (struct cbor_pair){
            .key = cbor_move(cbor_build_string("forward")),
            .value = cbor_move(cbor_build_uint32(tcp_session->reorder_count[SASC_FLOW_FORWARD]))
        });
        cbor_map_add(reorder, (struct cbor_pair){
            .key = cbor_move(cbor_build_string("reverse")),
            .value = cbor_move(cbor_build_uint32(tcp_session->reorder_count[SASC_FLOW_REVERSE]))
        });
        cbor_map_add(obj, (struct cbor_pair){
            .key = cbor_move(cbor_build_string("reorders")),
            .value = cbor_move(reorder)
        });
    }

    /* Anomalies */
    u32 total_anomalies = tcp_session->invalid_tcp_header_count + tcp_session->malformed_flags_count + 
                          tcp_session->unexpected_syn_count + tcp_session->protocol_violation_count +
                          tcp_session->invalid_fin_ack_count + tcp_session->fast_retransmit_count +
                          tcp_session->window_probe_count + tcp_session->handshake_timeout_count;
    
    if (total_anomalies > 0) {
        cbor_map_add(obj, (struct cbor_pair){
            .key = cbor_move(cbor_build_string("anomalies")),
            .value = cbor_move(cbor_build_uint32(total_anomalies))
        });
    }

    /* RTT info */
    if (tcp_session->rtt_count[SASC_FLOW_FORWARD] > 0 || tcp_session->rtt_count[SASC_FLOW_REVERSE] > 0) {
        cbor_item_t *rtt = cbor_new_definite_map(2);
        
        if (tcp_session->rtt_count[SASC_FLOW_FORWARD] > 0) {
            f64 avg_rtt = tcp_session->rtt_sum[SASC_FLOW_FORWARD] / tcp_session->rtt_count[SASC_FLOW_FORWARD];
            cbor_map_add(rtt, (struct cbor_pair){
                .key = cbor_move(cbor_build_string("forward_avg_ms")),
                .value = cbor_move(cbor_build_float4(avg_rtt * 1000.0))
            });
        }
        
        if (tcp_session->rtt_count[SASC_FLOW_REVERSE] > 0) {
            f64 avg_rtt = tcp_session->rtt_sum[SASC_FLOW_REVERSE] / tcp_session->rtt_count[SASC_FLOW_REVERSE];
            cbor_map_add(rtt, (struct cbor_pair){
                .key = cbor_move(cbor_build_string("reverse_avg_ms")),
                .value = cbor_move(cbor_build_float4(avg_rtt * 1000.0))
            });
        }
        
        cbor_map_add(obj, (struct cbor_pair){
            .key = cbor_move(cbor_build_string("rtt")),
            .value = cbor_move(rtt)
        });
    }

    cbor_array_push(s, obj);
}
 ...
 */
static_always_inline bool parse_tcp_timestamps(tcp_header_t *tcp, u8 tcp_hdr_len, u32 *ts_val,
                                              u32 *ts_ecr) {
    if (tcp_hdr_len <= 20)
        return false;
 ...
 */
static_always_inline void do_rtt_stat_update(sasc_tcp_check_session_state_t *tcp_session, u8 dir,
                                             f64 rtt) {
    if (rtt < 0)
    ...
    /* Update histogram (in milliseconds) */
    u32 rtt_ms = rtt * 1000;
    if (rtt_ms < 10)
        tcp_session->rtt_histogram[dir][0]++;
    else if (rtt_ms < 50)
        tcp_session->rtt_histogram[dir][1]++;
    else if (rtt_ms < 100)
        tcp_session->rtt_histogram[dir][2]++;
    else if (rtt_ms < 200)
        tcp_session->rtt_histogram[dir][3]++;
    else if (rtt_ms < 500)
        tcp_session->rtt_histogram[dir][4]++;
    else if (rtt_ms < 1000)
        tcp_session->rtt_histogram[dir][5]++;
    else if (rtt_ms < 2000)
        tcp_session->rtt_histogram[dir][6]++;
    else if (rtt_ms < 5000)
        tcp_session->rtt_histogram[dir][7]++;
    else if (rtt_ms < 10000)
        tcp_session->rtt_histogram[dir][8]++;
    else
        tcp_session->rtt_histogram[dir][9]++;
}
...
static_always_inline void check_tcp_anomalies(sasc_tcp_check_session_state_t *tcp_session,
                                              sasc_session_t *session, f64 current_time, u8 dir,
                                              tcp_header_t *tcp, u32 packet_length) {
    u32 seqnum = clib_net_to_host_u32(tcp->seq_number);
    u32 acknum = clib_net_to_host_u32(tcp->ack_number);
    u16 window_size = clib_net_to_host_u16(tcp->window);
    u8 flags = tcp->flags & SASC_TCP_CHECK_TCP_FLAGS_MASK;

    clib_warning("Processing: %U\n%U", format_sasc_session_key, &session->forward_key,
    ...
    if (tcp_hdr_len < 20) {
        ...
        clib_warning("Invalid TCP header length: %u", tcp_hdr_len);
        return;
    }
    ...
    if ((flags & SASC_TCP_CHECK_TCP_FLAGS_SYN) && (flags & SASC_TCP_CHECK_TCP_FLAGS_FIN)) {
        ...
        clib_warning("Malformed TCP flags: SYN and FIN set together");
        return;
    }
    ...
    if ((flags & SASC_TCP_CHECK_TCP_FLAGS_SYN) && (flags & SASC_TCP_CHECK_TCP_FLAGS_RST)) {
        ...
        tcp_session->malformed_flags_count++;
        clib_warning("Malformed TCP flags: SYN and RST set together");
        return;
    }
    ...
    if (tcp_session->last_seq[dir] == seqnum && tcp_session->last_ack[dir] == acknum) {
        ...
        if (current_is_data == last_was_data) {
            ...
            if (tcp_session->retransmit_delay_count[dir] < 16) {
                tcp_session->retransmit_delays[dir][tcp_session->retransmit_delay_count[dir]] = retransmit_delay;
            ...
            clib_warning("TCP retransmit: session %u, dir %u, count %u, delay %.3fms, burst_count %u, seq %u, ack %u",
                         session - sasc_main.sessions, dir, tcp_session->retransmit_count[dir],
                         retransmit_delay * 1000.0, tcp_session->retransmit_burst_count[dir], seqnum, acknum);
            ...
            if (retransmit_rate > 0.05) {
                clib_warning("High retransmission rate: %.2f%% in direction %u for session %u",
            ...
            if (tcp_session->retransmit_burst_count[dir] > 3) {
                clib_warning("Retransmission burst detected: %u consecutive retransmits in direction %u for session %u",
                             tcp_session->retransmit_burst_count[dir], dir, session - sasc_main.sessions);
            }

            // Alert on excessive retransmission delays (>1 second)
            if (avg_delay > 1.0) {
                clib_warning("Excessive retransmission delay: %.3fms average in direction %u for session %u",
        ...
        if (tcp_session->expected_seq[dir] != 0 && data_len > 0 &&
            ...
            if (seq_diff < tolerance && seq_diff < window_size) {
                ...
                clib_warning("TCP reorder detected: session %u, direction %u, count %u, "
    ...
    if (flags & SASC_TCP_CHECK_TCP_FLAGS_ACK) {
        ...
        if (acked_data == 0 && tcp_session->last_data_len[other_dir] > 0) {
            tcp_session->expected_seq[other_dir] = acknum;
            clib_warning("Pure ACK processing: dir=%u, acknum=%u, updated expected_seq[%u]=%u", dir,
                         acknum, other_dir, acknum);
        }
    }
    ...
    if (data_len > 0) {
        tcp_session->expected_seq[dir] = seqnum + data_len;
        clib_warning("Data packet: dir=%u, seq=%u, data_len=%u, updated expected_seq[%u]=%u", dir,
 ...
 */
static_always_inline int init_tcp_session(sasc_tcp_check_session_state_t *tcp_session,
                                          sasc_session_t *session, u8 flags) {
    /* Reset session state if version mismatch */
    clib_warning("Session version mismatch, resetting session");
    ...
    if (flags != SASC_TCP_CHECK_TCP_FLAGS_SYN) {
        ...
        clib_warning("Session starts with non-SYN packet");
 ...
 */
static_always_inline int process_tcp_flags(sasc_tcp_check_session_state_t *tcp_session,
                                           sasc_session_t *session, u8 dir, u8 flags, u32 seqnum,
                                           u32 acknum, u32 sf, u32 *nsf, u8 *remove_session) {
    *nsf = sf;
    *remove_session = 0;
 ...
 */
static_always_inline void finalize_session_state(sasc_tcp_check_session_state_t *tcp_session,
                                                 u32 nsf, u8 remove_session, int result) {
    /* Update session state */
    tcp_session->flags = nsf;

    if (remove_session || result == -1) {
        clib_warning("Session is being removed");
        // session->state = SASC_SESSION_STATE_TCP_TRANSITORY;  // TODO: Define this constant
    }
}

static_always_inline void update_state_one_pkt(sasc_tenant_t *tenant,
                                               sasc_tcp_check_session_state_t *tcp_session,
                                               sasc_session_t *session, f64 current_time, u8 dir,
                                               tcp_header_t *tcp, u32 *sf, u32 *nsf,
                                               u32 packet_length) {
    u8 flags = tcp->flags & SASC_TCP_CHECK_TCP_FLAGS_MASK;
    u32 acknum = clib_net_to_host_u32(tcp->ack_number);
    u32 seqnum = clib_net_to_host_u32(tcp->seq_number);
    ...
    *nsf = *sf;
    ...
    /* Check for anomalies */
    check_tcp_anomalies(tcp_session, session, current_time, dir, tcp, packet_length);
    ...
    /* Handle final state transitions and session cleanup */
    finalize_session_state(tcp_session, *nsf, remove_session, result);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame) {
    ...
    while (n_left > 0) {
        ...
        tenant = sasc_tenant_at_index(sasc, sasc_buffer(b[0])->tenant_index);
        // clib_warning("Processing packet for session %u", session_idx);
        tcp_header_t *tcp = 0;
        ...
        if (b[0]->flags & VNET_BUFFER_F_L4_HDR_OFFSET_VALID) {
            ...
            // clib_warning("TCP packet received %U", format_tcp_header, tcp, 40);
        } else {
            clib_warning("TCP packet received but l4_hdr_offset flag is not valid");
        }
        update_state_one_pkt(tenant, tcp_session, session, current_time, dir, tcp, sf, nsf,
                             b[0]->current_length);
