...
void
CLIB_MULTIARCH_FN (gpcapng_trivial_filter_classify) (u32 api_version,
                                                     vlib_buffer_t **bufs,
                                                     u32 n_buffers,
                                                     vlib_main_t *vm,
...
trivial_filter_init (vlib_main_t *vm)
{
  ...
  trivial_filter.selected_fn = CLIB_MARCH_FN_POINTER (gpcapng_trivial_filter_classify);
  
  int rv = gpcapng_plugin.register_filter_impl (&trivial_filter);
  if (rv != 0)
    {
      return clib_error_return (0, "Failed to register trivial filter implementation (error %d)", rv);
    }
