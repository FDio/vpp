			     ...
			     vnet_crypto_op_id_t op_id, u8 is_aead)
{
  ...
  if (is_aead && (fe->flags & VNET_CRYPTO_OP_FLAG_HMAC_CHECK))
    cipher_len += fe->icv_len;
  ...
  op->op = op_id;
  op->iv = vnet_crypto_async_get_data_ptr (vm, b, fe->iv_offset);
  op->key = fe->key;
  op->flags = fe->flags;
  op->user_data = index;
  ...
  if (!is_aead)
    {
      op->auth = vnet_crypto_async_get_data_ptr (vm, b, fe->icv_offset);
      op->auth_len = fe->icv_len;
    }
  else
    {
      op->auth = vnet_crypto_async_get_data_ptr (vm, b, fe->icv_offset);
      op->aad = vnet_crypto_async_get_data_ptr (vm, b, fe->aad_offset);
	     ...
	     vnet_crypto_op_chunk_t *chunks, u8 is_chained)
{
   ...
   * and set the error bit in the corresponding frame.
   */
  if (n_fail)
    {
