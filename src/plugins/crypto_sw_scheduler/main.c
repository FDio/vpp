...
crypto_sw_scheduler_get_auth_ptr (vlib_main_t *vm, vlib_buffer_t *b,
				  const vnet_crypto_async_frame_elt_t *fe)
{
  u8 *auth;
...

  {
    ...
    pd2 = esp_post_data2 (b);
    if (pd2->icv_removed && pd2->free_buffer_index)
      {
	vlib_buffer_t *tmp;

	tmp = vlib_get_buffer (vm, pd2->free_buffer_index);
	return vlib_buffer_get_current (tmp);
      }
  }
			     ...
			     vnet_crypto_op_id_t op_id, u8 is_aead)
{
  ...
  if (!is_aead)
    {
      op->auth = crypto_sw_scheduler_get_auth_ptr (vm, b, fe);
	     ...
	     vnet_crypto_op_chunk_t *chunks, u8 is_chained)
{
  ...
  if (is_chained)
    vnet_crypto_process_chained_ops (vm, op, chunks, n_ops);
  else
    vnet_crypto_process_ops (vm, op, n_ops);
