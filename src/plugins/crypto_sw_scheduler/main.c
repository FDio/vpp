...
crypto_sw_scheduler_config (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
		    ...
		    &crypto_sw_scheduler_queue_size))
	{
	  if ((crypto_sw_scheduler_queue_size &
	       (crypto_sw_scheduler_queue_size - 1)) != 0)
	    {
	      return clib_error_return (0, "input %d is not pow2",
					format_unformat_error,
					crypto_sw_scheduler_queue_size);
	    }
	  cm->crypto_sw_scheduler_queue_size = crypto_sw_scheduler_queue_size;
  ...
  if (cm->crypto_sw_scheduler_queue_size < CRYPTO_SW_SCHEDULER_QUEUE_SIZE)
    {
      cm->crypto_sw_scheduler_queue_size = CRYPTO_SW_SCHEDULER_QUEUE_SIZE;
      cm->crypto_sw_scheduler_queue_mask = CRYPTO_SW_SCHEDULER_QUEUE_MASK;
    }
