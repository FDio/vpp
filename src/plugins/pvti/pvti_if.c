...
format_pvti_if (u8 *s, va_list *args)
{
  ...
  s = format (s, "[%d] %U local:%U:%d remote:%U:%d underlay_mtu:%d", pvtii,
		...
		u32 *sw_if_indexp)
{
  ...
  *sw_if_indexp = (u32) ~0;
  ...
  pool_elt_at_index (pvti_main.if_pool, t_idx)->bi0 = INDEX_INVALID;
  pool_elt_at_index (pvti_main.if_pool, t_idx)->rx_bi0 = INDEX_INVALID;
  pool_elt_at_index (pvti_main.if_pool, t_idx)->rx_bi0_first = INDEX_INVALID;
  pool_elt_at_index (pvti_main.if_pool, t_idx)->current_tx_seq = 42;
