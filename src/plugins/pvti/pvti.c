				  ...
				  vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (input, "underlay-fib %d", &underlay_fib_index))
	{
      ...
      else if (unformat (input, "underlay-table %d", &underlay_table_id))
	{
	  ...
	  if (~0 == fib_index)
	    {
	      return clib_error_return (0, "Nonexistent table id %d",
					underlay_table_id);
	    }
...
VLIB_CLI_COMMAND (pvti_interface_create_command, static) = {
  ...
  .short_help =
    ...
    "local-ip <ip-addr> ][ underlay-mtu <MTU>][underlay-fib <fib-index>]",
