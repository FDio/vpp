				  ...
				  vlib_cli_command_t *cmd)
{
  ...
  u16 underlay_mtu = 1500;
  u32 underlay_fib_index = ~0;
  bool peer_set = 0;

  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "peer %U %d %d", unformat_ip_address, &peer_ip,
		    &peer_port, &local_port))
	{
	  peer_set = 1;
	}
      else if (unformat (input, "underlay-mtu %d", &underlay_mtu))
	{
	  // MTU set
	}
      else if (unformat (input, "local-ip %U", unformat_ip_address, &local_ip))
	{
	  // local IP set
	}
      else if (unformat (input, "underlay-fib %d", &underlay_fib_index))
	{
	  // underlay fib set
	}
      else
...
VNET_FEATURE_INIT (pvti, static) = {
  .arc_name = "device-input", .node_name = "pvti", .runs_before = 0,
...
pvti_early_config (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  u8 *runs_before = 0;
  int rbi = 0;
  if (vec_len (vnet_feat_pvti.runs_before) == 0)
    {
      rbi = 0;
    }
  else
    {
      rbi = vec_len (vnet_feat_pvti.runs_before) - 1;
    }
  vec_validate (vnet_feat_pvti.runs_before, rbi);
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      if (unformat (input, "runs-before %v", &runs_before))
	{
	  ...
	  vnet_feat_pvti.runs_before[rbi] = (char *) runs_before;
	  vec_add1 (vnet_feat_pvti.runs_before, 0);
	}
      else
