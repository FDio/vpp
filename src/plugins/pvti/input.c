...
#define PVTI_RX_MAX_LENGTH 2048
		  if (pvti_if0->rx_streams[stream_index].rx_bi0 ==
		      INDEX_INVALID)
		    {
		      clib_warning (
		     ...
		     true_chunk_count < pvti0->reass_chunk_count)
		{
	      ...
	      if (pvti_if0->rx_streams[stream_index].rx_bi0_first !=
		  INDEX_INVALID)
		{
	  ...
	  while ((b0->current_length > 0) &&
		...
		{
		  clib_error ("RX ERR: length too short for a chunk");
		  break;
		}
	      pvti_chunk_header_t *pvc0 = vlib_buffer_get_current (b0);
	      chunks[true_chunk_count] = pvc0;
