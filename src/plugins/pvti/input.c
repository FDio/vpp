...
always_inline void
pvti_enqueue_rx_bi_to_next (pvti_per_thread_data_t *ptd, u32 bi0, u16 next0)
{
...
#define PVTI_RX_MAX_LENGTH 2048
	      if (pvti_if0->rx_streams[stream_index].rx_bi0 == INDEX_INVALID)
		{
		  clib_warning (
	      ...
	      else
		{
		  ...
		  if (pvti_if0->rx_streams[stream_index]
			...
			.rx_expected_inner_length)
		    {
		      ...
		      pvti_enqueue_rx_bi_to_next (
			ptd, pvti_if0->rx_streams[stream_index].rx_bi0_first,
			next0);
      ...
      else
	{
	  ...
	  if (pvti_if0->rx_streams[stream_index].rx_bi0_first != INDEX_INVALID)
	    {
	      clib_warning ("RX PVTI: discard chunk being reassembled");
	      vlib_buffer_free_one (
		vm, pvti_if0->rx_streams[stream_index].rx_bi0_first);
	      pvti_if0->rx_streams[stream_index].rx_bi0 = INDEX_INVALID;
	      pvti_if0->rx_streams[stream_index].rx_bi0_first = INDEX_INVALID;
	      pvti_if0->rx_streams[stream_index].rx_received_inner_length = 0;
	      pvti_if0->rx_streams[stream_index].rx_expected_inner_length = 0;
	      pvti_if0->rx_streams[stream_index].rx_next0 = 0;
	    }
	}
      ...
      while ((b0->current_length > 0) && true_chunk_count < max_chunk_count)
	{
	  ...
	  if (PREDICT_TRUE (vlib_trace_buffer (vm, node, next0, nb0,
		...
		vlib_add_trace (vm, node, nb0, sizeof (*t));
	    }
	  ...
	  pvti_enqueue_rx_bi_to_next (ptd, nbi0, next0);
	}
