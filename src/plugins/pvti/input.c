...
typedef enum
{
  PVTI_INPUT_TRACE_DROP = 0,
  PVTI_INPUT_TRACE_DECAP,
  PVTI_INPUT_TRACE_FREE
...
format_pvti_input_trace (u8 *s, va_list *args)
{
  ...
  s = format (
    s,
    "PVTI-IN: sw_if_index %d, next index %d, trace_type: %d, chunkcnt: %d\n",
    t->sw_if_index, t->next_index, t->trace_type, t->chunk_count);
