...
#define _(f,s) s,
static char * pvti_input_trace_type_names[] = {
  foreach_pvti_input_trace_type
};
#undef _
...
format_pvti_input_trace (u8 *s, va_list *args)
{
  ...
  s = format (
    s,
    "PVTI-IN: sw_if_index %d, next index %d, trace_type: %s(%d), chunkcnt: %d\n",
    t->sw_if_index, t->next_index, get_pvti_trace_type_name(t->trace_type), t->trace_type, t->chunk_count);
