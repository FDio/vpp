...
#define PVTI_RX_MAX_LENGTH 2048
		  vlib_buffer_t *rb0 = vlib_get_buffer (vm, pvti_if0->rx_bi0);
		  u16 allowed_length =
		  ...
		  if (allowed_length > chunk_payload_length)
			...
			chunk_payload_length - allowed_length;
		      u32 nrbi0 = pvti_get_new_buffer (vm, pvti_if0);
		  ...
		  pvti_if0->rx_received_inner_length += chunk_payload_length;
		  if (pvti_if0->rx_received_inner_length ==
		      pvti_if0->rx_expected_inner_length)
		    {
		      // FIXME: send the buffer
		      to_next[0] = pvti_if0->rx_bi0_first;
		      to_next += 1;
		      n_left_to_next -= 1;
		      next0 = pvti_if0->rx_next0;
		      vlib_validate_buffer_enqueue_x1 (
			vm, node, next_index, to_next, n_left_to_next,
			pvti_if0->rx_bi0_first, next0);
		      // clean out the current reassemly state
		      pvti_if0->rx_bi0 = INDEX_INVALID;
		      pvti_if0->rx_bi0_first = INDEX_INVALID;
		      pvti_if0->rx_received_inner_length = 0;
		      pvti_if0->rx_expected_inner_length = 0;
		      pvti_if0->rx_next0 = 0;
		    }
	  ...
	  while ((b0->current_length > 0) &&
		  ...
		  total_chunk_length)
		{
		  ...
		  ASSERT (pvti_if0->rx_bi0_first == INDEX_INVALID);
		  pvti_if0->rx_next0 = next0;
		  pvti_if0->rx_bi0 = bi0;
		  pvti_if0->rx_bi0_first = bi0;
		  pvti_if0->rx_expected_inner_length = inner_length;
		  pvti_if0->rx_received_inner_length =
