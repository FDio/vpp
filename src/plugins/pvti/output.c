...
{
  if (vec_len (pvti_if0->tx_streams[stream_index].bi_store) > 0)
    {
      u32 bi0 = vec_pop (pvti_if0->tx_streams[stream_index].bi_store);
      vlib_buffer_t *b0 = vlib_get_buffer (vm, bi0);
      b0->current_data = 0;
      b0->current_length = 0;
      return bi0;
...
#define TRIGGER_PVTI_PACKET_REDO(_i0, _frag)                                  \
  ...
  is_first_frag = _frag;
	      ...
	      else
		{
		  ...
		  // buffer shortly.
		  vec_add1 (pvti_if0->tx_streams[stream_index].bi_store, bi0);
	  ...
	  if (pvti_if0->tx_streams[stream_index].bi0 != INDEX_INVALID)
				 ...
				 (b0->flags & VLIB_BUFFER_IS_TRACED)))
		{
	      ...
	      vlib_validate_buffer_enqueue_x1 (vm, node, next_index, to_next,
					       n_left_to_next, bi0, next0);
	      pvti_if0->tx_streams[stream_index].bi0 = INDEX_INVALID;
	      u32 *bi_store = pvti_if0->tx_streams[stream_index].bi_store;
	      if (bi_store)
		{
		  vlib_buffer_free (vm, bi_store, vec_len (bi_store));
		  vec_set_len (pvti_if0->tx_streams[stream_index].bi_store, 0);
		}
	    }
