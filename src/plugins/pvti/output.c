			      ...
			      pvti_if_t *pvti_if0, u8 stream_index, int is_ip6)
{
  ...
  if (is_ip6_encap)
	...
	b0->current_data + sizeof (ip6_header_t);
  ...
  // We are calculating the checksums ourselves
  vnet_buffer (b0)->oflags &=
...
#define TRIGGER_PVTI_PACKET_REDO(_i0, _frag)                                  \
  ...
  is_first_frag = _frag;
	      ...
	      else
		{
		  ...
		  // buffer shortly.
		  pvti_put_buffer (vm, pvti_if0, stream_index, bi0);
	...
	trace_out:
	  ...
	  ASSERT (b0->data[b0->current_data] == 0x45);
	  to_next += 1;
	  n_left_to_next -= 1;
