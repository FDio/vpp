...
encapsulate_pvti_buffer_ip4 (vlib_main_t *vm, pvti_if_t *pvti_if0)
{
  ...
  ve->ip4.fragment_id =
    clib_host_to_net_u16 ((pvti_if0->current_tx_seq & 0xffff) - 1);
...
// 1500 - 20 - 8;
#define PVTI_PACKET_OVERHEAD                                                  \
  (20 + 8 + sizeof (pvti_packet_header_t) + PVTI_ALIGN_BYTES)
	...
	redo_packet:
	  ...
	  if (pvti_if0->bi0 == INDEX_INVALID)
					  ...
					  is_first_frag || is_chained_buf))
		{
	      ...
	      if (wip0_chunk_length > pvti_if0->bi0_max_current_length)
		{
		  /* we already are over the limit on the first packet. Chop it
		   * up. */
		  u16 excess_length =
		    wip0_chunk_length - pvti_if0->bi0_max_current_length;
  ...
  is_first_frag = _frag;
	      ...
	      else
		{
		  ...
		  clib_memcpy (tail, vlib_buffer_get_current (b0),
			       b0->current_length);
		  pvti_if0->ip4_chunk_count += 1;
		  pvti_if0->ip4_reass_chunk_count += is_chained_buf;
		  is_chained_buf = 0;
