...
format_pvti_output_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "PVTI-OUT(%d): sw_if_index %d, next index %d underlay_mtu %d",
...
encapsulate_pvti_buffer_ip4 (vlib_main_t *vm, pvti_if_t *pvti_if0)
{
  ...
  ASSERT ((thread_index & 0xffffff00) == 0);
  ...
  pvti0->thread_id = thread_index & 0xff;
  pvti0->reass_chunk_count = pvti_if0->ip4_reass_chunk_count;
		       ...
		       int first_frag)
{
  ...
  pvti_if0->ip4_reass_chunk_count = first_frag ? 1 : 0;

  // 1500 - 20 - 8;
  ASSERT (pvti_if0->underlay_mtu > 28);
  pvti_if0->bi0_max_current_length = pvti_if0->underlay_mtu - 20 - 8;
  return 1;
}
  ...
  while (n_left_from > 0)
	...
	redo_packet:
	  if (pvti_if0->bi0 == INDEX_INVALID)
	    {
	      if (!start_new_pvti_buffer (vm, pvti_if0, bi0,
					  0 /* first_frag */))
		{
		  ...
		  pvti_if0->bi0_max_current_length)
		{
		  ...
		  if (INDEX_INVALID == nbi0)
		  ...
		  else
		    {
		      ...
		      to_next[0] = xbi0;
		      start_new_pvti_buffer (vm, pvti_if0, nbi0,
					     1 /* first_frag */);
		      bi0 = xbi0;
		      b0 = vlib_get_buffer (vm, bi0);
		      goto trace_out;
            ...
            // Attempt to append the WIP chunk to the existing buffer
	    {
	      ...
	      u16 potential_new_length = wip0_chunk_length + vb0->current_length;
	      ...
	      if (potential_new_length > pvti_if0->bi0_max_current_length)
		{
		  ...
		  if (wip0_chunk_length >
		      pvti_if0->bi0_max_current_length)
		    {
		      // This new chunk is too large even for a single packet, and
                      // we are trying to coalesce with an already existing chunk.
                      // See if we cam fit the excess part of it here.
                      // Splitting into two chunks requires an additional chunk header.
                      // So we add it to excess length

		      u16 excess_length = wip0_chunk_length
                             - pvti_if0->bi0_max_current_length
                             + sizeof (pvti_chunk_header_t);

                      u16 available_space = pvti_if0->bi0_max_current_length - vb0->current_length;
                      //u16 excess_length = wip0_chunk_length - available_space;
			  ...
			  pvti_if0->bi0_max_current_length)
			{
			  ...
			  clib_memcpy (tail, vlib_buffer_get_current (b0),
				       excess_length);
                          ...
                          pvc0 = tail;
                          pvc0->total_chunk_length = clib_host_to_net_u16(excess_length);
		      ...
		      else
			{
			  ...
			  send the currently built packet, and redo the first part.
			  */
			  ...
			  vlib_validate_buffer_enqueue_x1 (
			    ...
			    pvti_if0->bi0, next0);
			  /* FIXME: tracing ^^^^^ ! This duplicates the code. */
		      ...
		      else
			{
			   ...
			   * length, queue the remainder */
                          u16 append_length = pvti_if0->bi0_max_current_length - vb0->current_length;

			  void *tail =
			    vlib_buffer_put_uninit (vb0, append_length);
			  clib_memcpy (tail, vlib_buffer_get_current (b0),
				       append_length);
                          // store the new chunk length
                          pvc0 = tail;
	      ...
	      else
		{
		  ...
		  clib_memcpy (tail, vlib_buffer_get_current (b0),
			       b0->current_length);
		  pvti_if0->ip4_chunk_count += 1;
		  // store the index in the temp space in case we need a
		  // buffer shortly.
