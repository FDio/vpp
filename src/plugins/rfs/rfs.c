...
rfs_compute_thread (vlib_buffer_t *b, u8 is_ip4)
{
  ...
  if (is_ip4)
	...
	return ~0;
      ...
      return hash % rfs_main.n_workers;
    }
  else
    {
      /* TBD */
                      ...
                      vlib_frame_t *frame, u32 fq_index)
{
  ...
  while (n_left_from > 0)
    {
      ti[0] = vnet_buffer (b[0])->sw_if_index[VLIB_TX];
      n_left_from -= 1;
      ti += 1;
      b += 1;
    }
...
VLIB_REGISTER_NODE (rfs4_handoff) =
{
  .name = "rfs4-handoff",
  .vector_size = sizeof (u32),
...
VLIB_REGISTER_NODE (rfs6_handoff) =
{
  .name = "rfs6-handoff",
  .vector_size = sizeof (u32),
  .format_trace = format_rfs_handoff_trace,
...
rfs_init (vlib_main_t *vm)
{
  ...
  rm->out4_fq_index =
    vlib_frame_queue_main_init (rfs4_input_node.index, 0);
  rm->out6_fq_index =
    vlib_frame_queue_main_init (rfs6_input_node.index, 0);
