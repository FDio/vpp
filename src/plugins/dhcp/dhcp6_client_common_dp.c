	  ...
	  switch (dhcpv60->msg_type)
	      ...
	      if (!discard)
		{
		  if (!is_pd_packet && ia_na_client_state)
		    {
		      /* Fill data first, then publish count.  Concurrent readers
		       * (e.g. dhcp6_ia_na_client_get_runtime) may observe the
		       * count after our write but the array contents only after
		       * the memcpy completes; ordering count last keeps them
		       * from seeing dns_server_count > 0 with stale entries. */
