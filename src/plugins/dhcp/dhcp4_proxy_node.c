...
#define VPP_DHCP_OPTION82_SIZE                                                \
  (VPP_DHCP_OPTION82_SUB1_SIZE + VPP_DHCP_OPTION82_SUB5_SIZE +                \
...
format_dhcp_proxy_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "  %U", format_dhcp_header, t->packet_data,
...
static uword
dhcp_proxy_to_server_input (vlib_main_t *vm, vlib_node_runtime_t *node,
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (o->option == DHCP_PACKET_OPTION_END && o <= end)
	      ...
	      if (vss)
		{
		  ...
		  o->data[17 + id_len] =
		    DHCP_PACKET_OPTION_END; /* "end-of-options" (0xFF) */
	  ...
	  else
	    {
	      vlib_node_increment_counter (vm, dhcp_proxy_to_server_node.index,
					   DHCP_PROXY_ERROR_OPTION_82_ERROR,
					   1);
	    }
...
static uword
dhcp_proxy_to_client_input (vlib_main_t *vm, vlib_node_runtime_t *node,
			    vlib_frame_t *from_frame)
{
	  ...
	  while (o->option != DHCP_PACKET_OPTION_END && o < end)
	      ...
	      if (o->option == 82)
		{
		  ...
		  while (sub->option != DHCP_PACKET_OPTION_END && sub < subend)
			...
			{
			  sw_if_index =
			    ((sub->data[0] << 24) | (sub->data[1] << 16) |
			     (sub->data[2] << 8) | (sub->data[3]));
			}
		      else if (sub->option == 5 && sub->length == 4)
			{
		      ...
		      else if (sub->option == 151 && sub->length == 7 &&
			       sub->data[0] == 1)
			vss_exist = 1;
...
int
dhcp4_proxy_set_server (ip46_address_t *addr, ip46_address_t *src_addr,
			u32 rx_table_id, u32 server_table_id, int is_del)
{
  ...
  rx_fib_index = fib_table_find_or_create_and_lock (
    FIB_PROTOCOL_IP4, rx_table_id, FIB_SOURCE_DHCP);

  if (is_del)
    {
      if (dhcp_proxy_server_del (FIB_PROTOCOL_IP4, rx_fib_index, addr,
				 server_table_id))
	{
	  fib_table_entry_special_remove (rx_fib_index, &all_1s,
					  FIB_SOURCE_DHCP);
    ...
    {
      if (dhcp_proxy_server_add (FIB_PROTOCOL_IP4, addr, src_addr,
				 rx_fib_index, server_table_id))
	{
	  fib_table_entry_special_add (rx_fib_index, &all_1s, FIB_SOURCE_DHCP,
...
static clib_error_t *
dhcp4_proxy_set_command_fn (vlib_main_t *vm, unformat_input_t *input,
			    vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "server %U", unformat_ip4_address,
		    &server_addr.ip4))
	set_server = 1;
...
VLIB_CLI_COMMAND (dhcp_proxy_set_command, static) = {
  .path = "set dhcp proxy",
  .short_help = "set dhcp proxy [del] server <ip-addr> src-address <ip-addr> "
		"[server-fib-id <n>] [rx-fib-id <n>]",
  ...
  vec_foreach (server, proxy->dhcp_servers)
    {
      ...
      s = format (s, "%u,%U  ", server_fib->table_id, format_ip46_address,
...
static clib_error_t *
dhcp4_proxy_show_command_fn (vlib_main_t *vm, unformat_input_t *input,
			     vlib_cli_command_t *cmd)
{
...
static clib_error_t *
dhcp_option_82_vss_fn (vlib_main_t *vm, unformat_input_t *input,
		       vlib_cli_command_t *cmd)
{
  ...
  switch (rv)
    ...
    case VNET_API_ERROR_NO_SUCH_ENTRY:
      return clib_error_return (
	0, "option 82 vss for table %d not found in in pool.", tbl_id);
    default:
      return clib_error_return (0, "BUG: rv %d", rv);
    }
}
...
VLIB_CLI_COMMAND (dhcp_proxy_vss_command, static) = {
  ...
  .short_help = "set dhcp option-82 vss [del] table <table id> [oui <n> "
		"vpn-id <n> | vpn-ascii-id <text>]",
  .function = dhcp_option_82_vss_fn,
};

static clib_error_t *
dhcp_vss_show_command_fn (vlib_main_t *vm, unformat_input_t *input,
			  vlib_cli_command_t *cmd)
{
	  ...
	  if (ia0)
	    {
	      ...
	      vlib_cli_output (vm, "%=20U%=20U", format_vnet_sw_if_index_name,
