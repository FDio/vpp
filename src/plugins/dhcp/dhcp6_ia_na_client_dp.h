 ...
 *
 * Returns 0 on success with rt->enabled == 1; the caller may rely on every
 * other field of *rt in that case.  Returns non-zero when the client is
 * disabled, sw_if_index is unknown, or rt is NULL.
 * On non-zero, *rt is zeroed when non-NULL and no field other than
 * rt->enabled (== 0) is meaningful. */
