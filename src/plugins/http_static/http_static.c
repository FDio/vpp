...
static void
vl_api_http_static_disable_v5_t_handler (vl_api_http_static_disable_v5_t *mp)
{
  hss_main_t *hsm = &hss_main;
  vl_api_http_static_disable_v5_reply_t *rmp;
  int rv;

  rv = hss_destroy (hsm->vlib_main);

  REPLY_MACRO (VL_API_HTTP_STATIC_DISABLE_V5_REPLY);
}

static void
vl_api_http_static_get_v5_t_handler (vl_api_http_static_get_v5_t *mp)
{
  hss_main_t *hsm = &hss_main;
  vl_api_http_static_get_v5_reply_t *rmp;
  u8 *addr = NULL;
  int rv = 0;
  ...
  REPLY_MACRO2 (VL_API_HTTP_STATIC_GET_V5_REPLY, {
