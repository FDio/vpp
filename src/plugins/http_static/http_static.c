...
_(HTTP_STATIC_ENABLE_DISABLE, http_static_enable_disable)

/* Action function shared between message handler and debug CLI */

int
http_static_enable_disable (http_static_main_t * hmp, u32 sw_if_index,
			    int enable_disable)
{
  ...
  vnet_feature_enable_disable ("device-input", "http_static",
...
static clib_error_t *
http_static_enable_disable_command_fn (vlib_main_t * vm,
				       ...
				       vlib_cli_command_t * cmd)
{
  ...
  rv = http_static_enable_disable (hmp, sw_if_index, enable_disable);
  ...
  switch (rv)
    ...
    default:
      return clib_error_return (0, "http_static_enable_disable returned %d",
				rv);
    }
  return 0;
}
...
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (http_static_enable_disable_command, static) =
{
  .path = "http_static enable-disable",
  .short_help =
  "http_static enable-disable <interface-name> [disable]",
  .function = http_static_enable_disable_command_fn,
};
/* *INDENT-ON* */

/* API message handler */
static void vl_api_http_static_enable_disable_t_handler
  (vl_api_http_static_enable_disable_t * mp)
{
  vl_api_http_static_enable_disable_reply_t *rmp;
  http_static_main_t *hmp = &http_static_main;
  int rv;

  rv = http_static_enable_disable (hmp, ntohl (mp->sw_if_index),
				   (int) (mp->enable_disable));

  REPLY_MACRO (VL_API_HTTP_STATIC_ENABLE_DISABLE_REPLY);
}
