	  ...
	  if (stat ((char *) path, sb) < 0	/* cant even stat the file */
	  ...
	  else
	    {
	      ...
	      redirect = format (0, "HTTP/1.1 301 Moved Permanently\r\n"
				 "Location: http://192.168.10.1/%s%c", path);
	      static_send_data (hs, redirect, vec_len (redirect) - 1, 0);
	      vec_free (path);
	      vec_free (redirect);
	      goto close_session;
	    }
...
static_send_response:
  ...
  /* read the file if we haven't done so yet. */
  if (hs->data == 0)
    {
  ...
  /* Send an http response, which (irritatingly) needs the data length */
  http_response = format (0, http_response_template, http_type,
			  vec_len (hs->data));
...
static_send_data:
  ...
  /*
   * Try to send data. Ideally, we hope that the fifos are large
