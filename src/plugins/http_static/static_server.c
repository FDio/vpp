...
(void *fp, char *url, int request_type)
{
  ...
  builtin_table = (request_type == 0) ? hsm->get_url_handlers :
		   ...
		   state_machine_called_from_t cf)
{
  ...
  if ((i = v_find_index (request, "GET ")) >= 0)
    /* request_type = 0, GET */
    goto find_end;
  else if ((i = v_find_index (request, "POST ")))
    {
      request_type = 1; /* POST */
      goto find_end;
    }
...
find_end:
  ...
  /* drop in a NULL byte for lookup purposes. POST handlers beware */
  ...
  if (hsm->debug_level > 0)
    clib_warning ("%s '%s'", (request_type) == 0 ? "GET" : "POST", path);
