...
static void
hss_cache_attach_entry (u32 ce_index, u8 **data, u64 *data_len)
{
  hss_main_t *hsm = &hss_main;
 ...
 */
static void
hss_cache_detach_entry (u32 ce_index)
{
  hss_main_t *hsm = &hss_main;
  hss_cache_entry_t *ce;
...
hss_cache_lookup_and_attach (u8 *path, u8 **data, u64 *data_len)
{
  ...
  if (ce_index != ~0)
    hss_cache_attach_entry (ce_index, data, data_len);
...
hss_cache_add_and_attach (u8 *path, u8 **data, u64 *data_len)
{
  ...
  *data_len = vec_len (file_data);
  ...
  if (BV (clib_bihash_add_del) (&hsm->name_to_data, &kv, 1 /* is_add */) < 0)
    {
...
hss_ts_cleanup (session_t *s, session_cleanup_ntf_t ntf)
{
  ...
  if (hs->cache_pool_index != ~0)
    {
      hss_cache_detach_entry (hs->cache_pool_index);
      hs->cache_pool_index = ~0;
    }
