...
/* _(sym, stats-name, severity, description) */
#define foreach_vxlan_p2mp_encap_error                                                             \
  _ (UNICAST, "unicast", INFO, "unicast packets sent")                                             \
  _ (BUM, "bum", INFO, "BUM packets flooded")                                                      \
  _ (STALE, "stale", ERROR, "packets dropped: stale MAC entry")                                    \
  _ (NO_ENDPOINTS, "no-endpoints", ERROR, "packets dropped: no endpoints")

static vlib_error_desc_t vxlan_p2mp_encap_error_counters[] = {
#define _(sym, name, sev, desc) { name, desc, VL_COUNTER_SEVERITY_##sev },
...
typedef enum
{
#define _(sym, name, sev, desc) VXLAN_P2MP_ENCAP_ERROR_##sym,
  foreach_vxlan_p2mp_encap_error
#undef _
...
VLIB_REGISTER_NODE (vxlan4_p2mp_encap_node) = {
  ...
  .format_trace = format_vxlan_encap_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (vxlan_p2mp_encap_error_counters),
  .error_counters = vxlan_p2mp_encap_error_counters,
  .n_next_nodes = VXLAN_ENCAP_N_NEXT,
  .next_nodes = {
    [VXLAN_ENCAP_NEXT_DROP] = "error-drop",
  },
