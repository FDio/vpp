...
static void
vl_api_vxlan_p2mp_add_del_mac_endpoint_t_handler (vl_api_vxlan_p2mp_add_del_mac_endpoint_t *mp)
{
  vl_api_vxlan_p2mp_add_del_mac_endpoint_reply_t *rmp;
  int rv = 0;
  u32 sw_if_index = ntohl (mp->sw_if_index);

  VALIDATE_SW_IF_INDEX (mp);

  /* Resolve ep_dst to an ep_pool_index */
  vxlan_main_t *vxm = &vxlan_main;
  u32 t_index = vnet_vxlan_get_tunnel_index (sw_if_index);
  if (t_index == ~0)
    {
  ...
  ip46_address_t ep_dst;
  ip_address_decode (&mp->ep_dst, &ep_dst);
  ...
  vec_foreach (ep_idxp, t->endpoint_indices)
    {
      ...
      if (ip46_address_cmp (&ep->dst, &ep_dst) == 0)
	{
	  ep_pool_index = *ep_idxp;
	  break;
	}
    }
  if (ep_pool_index == ~0)
  ...
  rv = vnet_vxlan_p2mp_add_del_mac_ep (sw_if_index, mp->mac, ep_pool_index, mp->is_add);

done:
  BAD_SW_IF_INDEX_LABEL;
  REPLY_MACRO (VL_API_VXLAN_P2MP_ADD_DEL_MAC_ENDPOINT_REPLY);
}
...
typedef struct
{
  ...
  u32 context;
  u32 sw_if_index;
} vxlan_p2mp_mac_dump_ctx_t;

static int
vxlan_p2mp_mac_dump_cb (clib_bihash_kv_8_8_t *kv, void *arg)
{
  vxlan_p2mp_mac_dump_ctx_t *ctx = arg;
  vxlan_main_t *vxm = &vxlan_main;
  vl_api_vxlan_p2mp_mac_endpoint_details_t *rmp;

  u32 ep_pool_index = (u32) kv->value;
  if (pool_is_free_index (vxm->endpoint_pool, ep_pool_index))
    return BIHASH_WALK_CONTINUE; /* stale entry, skip */

  vxlan_endpoint_t *ep = pool_elt_at_index (vxm->endpoint_pool, ep_pool_index);

  rmp = vl_msg_api_alloc (sizeof (*rmp));
  clib_memset (rmp, 0, sizeof (*rmp));
  rmp->_vl_msg_id = ntohs (REPLY_MSG_ID_BASE + VL_API_VXLAN_P2MP_MAC_ENDPOINT_DETAILS);
  rmp->context = ctx->context;
  rmp->sw_if_index = htonl (ctx->sw_if_index);
  /* MAC is stored as 6 bytes in the low bits of the u64 key */
  clib_memcpy (rmp->mac, &kv->key, 6);
  ip_address_encode (&ep->dst, IP46_TYPE_ANY, &rmp->ep_dst);
...
static void
vxlan_p2mp_mac_dump_one_tunnel (vxlan_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
  if (!t->mac_to_ep_initialized)
    return;
  vxlan_p2mp_mac_dump_ctx_t ctx = {
    .reg = reg,
    .context = context,
    .sw_if_index = t->sw_if_index,
  };
  clib_bihash_foreach_key_value_pair_8_8 (&t->mac_to_ep, vxlan_p2mp_mac_dump_cb, &ctx);
}

static void
vl_api_vxlan_p2mp_mac_endpoint_dump_t_handler (vl_api_vxlan_p2mp_mac_endpoint_dump_t *mp)
{
  ...
  if (~0 == sw_if_index)
	...
	{
	  if (t->is_p2mp)
	    vxlan_p2mp_mac_dump_one_tunnel (t, reg, mp->context);
	}
    }
  else
    {
	...
	return;
      vxlan_tunnel_t *t =
	pool_elt_at_index (vxm->tunnels, vxm->tunnel_index_by_sw_if_index[sw_if_index]);
      if (t->is_p2mp)
	vxlan_p2mp_mac_dump_one_tunnel (t, reg, mp->context);
    }
}
