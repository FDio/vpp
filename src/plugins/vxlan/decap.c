				       ...
				       vlib_frame_t * f)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 3 && n_left_to_next > 3)
	{
	  ...
	  u32 t_index0 = VNET_FLOW_INDEX_FROM_MARK (b0->flow_id);
	  u32 t_index1 = VNET_FLOW_INDEX_FROM_MARK (b1->flow_id);
	  u32 t_index2 = VNET_FLOW_INDEX_FROM_MARK (b2->flow_id);
	  u32 t_index3 = VNET_FLOW_INDEX_FROM_MARK (b3->flow_id);
