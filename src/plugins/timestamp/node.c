		       ...
		       vlib_frame_t *frame, u8 egress)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* Pass the timestamp, thanks to the vnet_buffer->opaque2 unused
	   * metadata field */
	  ...
	  if (egress)
	    {
		  ...
		  time_meta->timestamp_ingress && time_meta->timestamp_egress)
		{
		  time_u64_t transit_delay;
		  transit_delay.as_u64 =
		    time_meta->timestamp_egress - time_meta->timestamp_ingress;
		  // overflow
		  if (transit_delay.as_u32[1])
		    {
		      transit_delay.as_u32[0] =
			0x80000000; // overflow as per IETF
		    }
  ...
  if (egress)
	...
	vm, node->node_index, TIMESTAMP_ERROR_EGRESS_STAMPED, pkts_stamped);
    }
  else
    {
      vlib_node_increment_counter (
	vm, node->node_index, TIMESTAMP_ERROR_INGRESS_STAMPED, pkts_stamped);
    }
...
static uword
timestamp_egress_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			  vlib_frame_t *frame)
{
