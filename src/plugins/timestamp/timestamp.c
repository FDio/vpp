...
int
timestamp_enable_disable (timestamp_main_t *sm, u32 sw_if_index,
			  int enable_disable)
{
  ...
  vnet_feature_enable_disable ("device-input", "timestamp-ingress",
			       sw_if_index, enable_disable, 0, 0);
  vnet_feature_enable_disable ("interface-output", "timestamp-egress",
			       sw_if_index, enable_disable, 0, 0);
...
static clib_error_t *
timestamp_enable_disable_command_fn (vlib_main_t *vm, unformat_input_t *input,
...
VLIB_CLI_COMMAND (enable_disable_timestamp, static) = {
  .path = "timestamp",
  .short_help = "timestamp <interface-name> [disable]",
 ...
 */
static void
vl_api_timestamp_enable_disable_t_handler (
  vl_api_timestamp_enable_disable_t *mp)
{
  ...
  rv = timestamp_enable_disable (sm, ntohl (mp->sw_if_index),
