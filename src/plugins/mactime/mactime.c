      ...
      if (found == 0)
	{
	  ...
	  memset (dp, 0, sizeof (*dp));
          vlib_validate_simple_counter (&mm->allow_counters,
                                        dp - mm->devices);
          vlib_zero_simple_counter (&mm->allow_counters,
                                    dp - mm->devices);
          vlib_validate_simple_counter (&mm->drop_counters,
                                        dp - mm->devices);
          vlib_zero_simple_counter (&mm->drop_counters,
                                    dp - mm->devices);
			 ...
			 unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  for (i = 0; i < vec_len (pool_indices); i++)
	...
	default:
      ...
      allow = vlib_get_simple_counter (&mm->allow_counters,
                                       dp - mm->devices);
      drop = vlib_get_simple_counter (&mm->drop_counters,
                                      dp - mm->devices);
