	  ...
	  for (i = 0; i < clib_net_to_host_u32 (mp->count); i++)
	    {
	      ...
	      vec_add1 (dp->allow_ranges, r[0]);
	    }
	  if (mp->drop)
	    dp->flags = MACTIME_DEVICE_FLAG_DROP;
	  if (mp->allow)
	    dp->flags = MACTIME_DEVICE_FLAG_ALLOW;
...
mactime_init (vlib_main_t * vm)
{
  ...
  clib_bihash_init_8_8 (&mm->lookup_table, "mactime lookup table",
			MACTIME_NUM_BUCKETS, MACTIME_MEMORY_SIZE);

  /* $$$ timezone config param */
  clib_timebase_init (&mm->timebase,
		      -5 /* EST */ , CLIB_TIMEBASE_DAYLIGHT_USA);
  return error;
}
			 ...
			 unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output (vm, "%-20s %20s %16s",
		   "Device Name", "MAC address", "Current Status");
  ...
  for (i = 0; i < vec_len (pool_indices); i++)
    {
      ...
      for (i = 0; i < vec_len (dp->allow_ranges); i++)
	{
	  clib_timebase_range_t *r = dp->allow_ranges + i;
	  f64 start0, end0;
      ...
      if (dp->flags & MACTIME_DEVICE_FLAG_DROP)
	current_status = 0;
      if (dp->flags & MACTIME_DEVICE_FLAG_ALLOW)
	current_status = 1;
