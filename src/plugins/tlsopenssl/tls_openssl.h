...
#define ERRORPR(fmt_str, ...)                                                 \
  do                                                                          \
    {                                                                         \
      FILE *f = fopen ("/tmp/vpp-err.log", "a");                              \
      if (f != NULL)                                                          \
	{                                                                     \
	  struct timespec ts;                                                 \
	  clock_gettime (CLOCK_REALTIME, &ts);                                \
	  fprintf (f, "[%ld.%06ld]vpp[%d:%lx:%02ld][%d:%s] " fmt_str,         \
...
int openssl_read_from_ssl_into_fifo (svm_fifo_t *f, SSL *ssl);
