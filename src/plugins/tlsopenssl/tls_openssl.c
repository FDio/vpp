		       ...
		       transport_send_params_t *sp)
{
  ...
  if (wrote < 0)
    {
      int err = SSL_get_error (oc->ssl, wrote);
      if (err == SSL_ERROR_WANT_WRITE)
	{
	  TLS_DBG (2, "CTX: %p WANT_WRITE_ERROR", ctx);
	  return 0;
	}
    }
  ...
  if (!wrote)
	...
	{
	  TLS_DBG (2, "WANT_ASYNC_FOR_WR: CTX: %p WR_SIZE: %d", ctx, deq_max);
	  vpp_tls_async_init_event (ctx, tls_async_write_event_handler, sp, ts,
				    SSL_ASYNC_EVT_WR, deq_max);
	  return 0;
	}
      goto check_tls_fifo;
    }
