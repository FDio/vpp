...
openssl_server_async_cert_cb (app_crypto_async_reply_t *reply)
{
  ...
  thread_index = reply->handle >> 32;
  ASSERT (thread_index == vlib_get_thread_index ());
  ctx = openssl_ctx_get_w_thread (handle, ((u32) handle) & TLS_IDX_MASK);
...
openssl_server_cert_callback (SSL *ssl, void *arg)
{
  ...
  app_crypto_async_req_handle_t handle;
  ...
  //   req->req_type = APP_CRYPTO_ASYNC_REQ_TYPE_CERT;
  //   req->async_cert.servername = (const u8 *) servername;
  //   req->handle = handle;
  //   req->cb = cb;
  //   req->app_wrk_index = app_wrk_index;
