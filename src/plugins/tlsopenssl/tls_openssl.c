  ...
  while (to_deq > 0)
    {
      ...
      rv = svm_fifo_peek (app_session->tx_fifo, 0, sizeof (hdr),
			  (u8 *) & hdr);
...
openssl_ctx_read_dtls (tls_ctx_t * ctx, session_t * us)
{
  ...
  if (PREDICT_FALSE (SSL_in_init (oc->ssl)))
    {
      clib_warning ("about to try handshake thread vlib %u us %u ctx %u",
                    vlib_get_thread_index (), us->thread_index, ctx->c_thread_index);
...
tls_openssl_init (vlib_main_t * vm)
{
  ...
  for (i = 0; i < num_threads; i++)
    {
      vec_validate (om->rx_bufs[i], 2000);
      vec_validate (om->tx_bufs[i], 2000);
    }
