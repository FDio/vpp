...
static inline int
openssl_ctx_try_tx (tls_ctx_t *ctx)
{
  u8 thread_index = vlib_get_thread_index ();
  openssl_ctx_t *oc = (openssl_ctx_t *) ctx;
                   ...
                   svm_fifo_t *tls_tx_fifo)
{
  ...
  svm_fifo_peek (app_tx_fifo, 0, deq_now, om->tx_bufs[thread_index]);
...
check_app_fifo:
  ...
  if ((pending = BIO_ctrl_pending (oc->wbio) > 0))
	...
	tls_add_vpp_q_evt (tls_rx_fifo, FIFO_EVENT_BUILTIN_RX);
      ...
      /* SSL_read may handle re-negotiations */
      openssl_ctx_try_tx (ctx);
    }
...
{
  char* PREFERRED_CIPHERS = "ALL:!ADH:!LOW:!EXP:!MD5:!RC4-SHA:!DES-CBC3-SHA:@STRENGTH";
  ...
  rv = SSL_CTX_set_cipher_list (oc->ssl_ctx, (const char *) PREFERRED_CIPHERS);
  if (rv != 1)
    {
