...
openssl_init_client_ctx (transport_proto_t proto)
{
  ...
  if (proto == TRANSPORT_PROTO_TLS)
    om->client_ssl_ctx = client_ssl_ctx;
  else
    om->dtls_client_ssl_ctx = client_ssl_ctx;
...
{
  openssl_main_t *om = &openssl_main;
  if (PREDICT_FALSE (!om->client_ssl_ctx))
    {
      if (openssl_init_client_ctx (TRANSPORT_PROTO_TLS) &&
	  proto == TRANSPORT_PROTO_TLS)
	return 0;
      if (openssl_init_client_ctx (TRANSPORT_PROTO_DTLS) &&
	  proto == TRANSPORT_PROTO_DTLS)
	return 0;
      ;
    }

  return proto == TRANSPORT_PROTO_TLS ? om->client_ssl_ctx :
					om->dtls_client_ssl_ctx;
}
