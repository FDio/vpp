...
{
  u8 thread_index = oc->ctx.c_thread_index;
  ...
  vec_validate (om->rx_bufs[thread_index], deq_max);
  read = tls_fifo_recv (tls_session->server_rx_fifo,
			om->rx_bufs[thread_index], deq_max);
  if (read > 0)
    {
			     ...
			     stream_session_t * tls_session)
{
  ...
  if ((pending = BIO_ctrl_pending (oc->rbio)) > 0)
    {
      vec_validate (om->tx_bufs[thread_index], enq_max);
      read = BIO_read (oc->rbio, om->tx_bufs[thread_index], enq_max);
      sent = tls_fifo_send (tls_session->server_tx_fifo,
			    om->tx_bufs[thread_index], read);
      ASSERT (sent == read);
      return sent;
    }
  return 0;
...
openssl_ctx_handshake_rx (tls_ctx_t * ctx, stream_session_t * tls_session)
{
  ...
  TLS_DBG (2, "tls state for %u is %u", oc->openssl_ctx_index, oc->ssl.state);
  ...
  TLS_DBG (1, "Handshake for %u complete. TLS cipher is %x",
	   oc->openssl_ctx_index, oc->ssl_ctx.session->ciphersuite);
  return rv;
}
...
openssl_ctx_write (tls_ctx_t * ctx, stream_session_t * app_session)
{
  ...
  u32 max_buf = 100 * TLS_CHUNK_SIZE;
  stream_session_t *tls_session;
  ...
  pending = BIO_ctrl_pending (oc->rbio);
  deq_now = clib_min (deq_max, clib_max (0, max_buf - pending));
  ...
  if ((pending = BIO_ctrl_pending (oc->rbio)) > 0)
    {
      ...
      read = BIO_read (oc->rbio, om->tx_bufs[thread_index], enq_max);
      sent =
	tls_fifo_send (tls_session->server_tx_fifo, om->tx_bufs[thread_index],
		       read);
      ASSERT (sent == read);
...
openssl_ctx_read (tls_ctx_t * ctx, stream_session_t * tls_session)
{
  ...
  int read = 0, enq = 0, wrote, pending;
  u32 max_buf = 100 * TLS_CHUNK_SIZE;
  u32 deq_max, enq_max, deq_now;
  ...
  read =
    tls_fifo_recv (tls_session->server_rx_fifo, om->rx_bufs[thread_index],
		   deq_now);
  if (read > 0)
    {
      wrote = BIO_write (oc->wbio, om->rx_bufs[thread_index], read);
      ASSERT (wrote == read);
      if (read < deq_max)
	tls_add_vpp_q_evt (tls_session->server_rx_fifo,
			   FIFO_EVENT_BUILTIN_RX);
    }

check_app_fifo:

  if (ctx->app_session_handle == SESSION_INVALID_HANDLE)
    return read;
...
openssl_ctx_init_server (tls_ctx_t * ctx)
{
  ...
  rv =
    SSL_CTX_set_cipher_list (oc->ssl_ctx, (const char *) PREFERRED_CIPHERS);
  if (rv != 1)
    {
      TLS_DBG ("Couldn't set cipher");
      return -1;
