...
openssl_read_from_bio_into_fifo (svm_fifo_t *f, BIO * bio)
{
  ...
  read = BIO_read (bio, svm_fifo_tail (f), deq_now);
  if (read <= 0)
    return 0;
  ...
  while (read < enq_max)
    {
      ...
      if (!c)
	break;
      deq_now = clib_min (c->length, enq_max - read);
      rv = BIO_read (bio, c->data, deq_now);
      if (rv <= 0)
	break;
      read += rv;
    }
...
openssl_read_from_ssl_into_fifo (svm_fifo_t *f, SSL * ssl)
{
  ...
  read = SSL_read (ssl, svm_fifo_tail (f), deq_now);
  if (read <= 0)
    return 0;
  ...
  while (read < enq_max)
	...
	break;
      deq_now = clib_min (c->length, enq_max - read);
      rv = SSL_read (ssl, c->data, deq_now);
      if (rv <= 0)
	break;
      read += rv;
    }
...
openssl_ctx_write (tls_ctx_t * ctx, session_t * app_session)
{
  openssl_ctx_t *oc = (openssl_ctx_t *) ctx;
  int wrote = 0, read;
  u32 deq_max, to_write, max_buf = 100 * TLS_CHUNK_SIZE, max_space;
