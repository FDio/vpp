...
static void
openssl_server_async_cert_cb (app_crypto_async_cert_handle_t handle,
			      u32 ckpair_index)
{
  clib_thread_index_t thread_index;
  app_cert_key_pair_t *ckpair;
  ...
  thread_index = handle >> 32;
  ASSERT (thread_index == vlib_get_thread_index ());
  oc = openssl_ctx_get_w_thread (handle, ((u32) handle) & TLS_IDX_MASK);
...
openssl_server_cert_callback (SSL *ssl, void *arg)
{
  ...
  app_crypto_async_cert_handle_t handle;
  ...
  handle = (u64) ctx->c_thread_index << 32 | ctx->tls_ctx_handle;
  oc->async_cert_ticket = app_crypto_get_ck_for_sni (
    servername, openssl_server_async_cert_cb, handle);
