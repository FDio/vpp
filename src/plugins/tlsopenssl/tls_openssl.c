...
{
  openssl_ctx_t *oc = (openssl_ctx_t *) ctx;
  /* If the shutdown process fails, clear the passive close flag to ensure
     SSL_shutdown occurs during context free.
  */
  if (SSL_shutdown (oc->ssl) < 0)
    {
      if (ctx->flags & TLS_CONN_F_PASSIVE_CLOSE)
	{
	  ctx->flags &= ~TLS_CONN_F_PASSIVE_CLOSE;
	  TLS_DBG (1, "Unset passive close");
	}
    }
