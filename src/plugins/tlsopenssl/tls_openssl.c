  ...
  if (!(ctx->flags & TLS_CONN_F_MIGRATED))
      ...
      if (SSL_is_init_finished (oc->ssl))
	{
	  if (openssl_main.async && ctx->flags & TLS_CONN_F_PASSIVE_CLOSE)
	    {
	      TLS_DBG (3, "SSL_shutdown finish async mode: flags: %ld",
		       ctx->flags);
	      SSL_shutdown (oc->ssl);
	    }
	    ...
	    TLS_DBG (3, "SSL_shutdown did not finish: flags: %ld", ctx->flags);
	}
      else
	TLS_DBG (3, "SSL_init did not finish: flags: %ld", ctx->flags);
