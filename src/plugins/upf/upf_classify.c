	      ...
	      vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  direction =
	    vnet_buffer (b)->gtpu.src_intf == INTF_ACCESS ? UL_SDF : DL_SDF;

	  pl =
	    vlib_buffer_get_current (b) + vnet_buffer (b)->gtpu.data_offset;

	  acl =
	    is_ip4 ? active->sdf[direction].ip4 : active->sdf[direction].ip6;
	  if (acl == NULL)
	    {
