			...
			int field, const upf_pdr_t * pdr)
{
  if (ipfilter_address_cmp_const (&pdr->pdi.acl.src_address, ACL_FROM_ANY) ==
	...
	{
	  ip6->field[field + i].value.u32 =
	    clib_net_to_host_u32 (pdr->pdi.acl.src_address.address.
				  ip6.as_u32[i]);
	...
	if (pdr->pdi.ue_addr.flags & IE_UE_IP_ADDRESS_V4)
	  {
	    ...
	    vrf_ip->fib_index =
	      ip4_fib_table_get_index_for_sw_if_index (pdr->
						       pdi.src_sw_if_index);
	...
	if (pdr->pdi.ue_addr.flags & IE_UE_IP_ADDRESS_V6)
	  {
	    ...
	    vrf_ip->fib_index =
	      ip6_fib_table_get_index_for_sw_if_index (pdr->
						       pdi.src_sw_if_index);
...
format_sx_session (u8 * s, va_list * args)
{
  ...
  vec_foreach (pdr, rules->pdr)
  {
    ...
    s = format (s, "  Outer Header Removal: %u\n"
		"  FAR Id: %u\n"
		"  URR Ids: [", pdr->outer_header_removal, pdr->far_id);
