...
static int
http_redirect_server_session_connected_callback (u32 app_index,
						 u32 api_context,
						 stream_session_t * s,
						 u8 is_fail)
{
...
static int
http_redirect_server_add_segment_callback (u32 client_index,
...
static session_cb_vft_t http_redirect_server_session_cb_vft = {
  ...
  .session_disconnect_callback =
    http_redirect_server_session_disconnect_callback,
  .session_connected_callback =
    http_redirect_server_session_connected_callback,
...
create_api_loopback (vlib_main_t * vm)
{
  ...
  hsm->my_client_index =
    vl_api_memclnt_create_internal ("http_redirect_server",
				    hsm->vl_input_queue);
  return 0;
}
...
server_attach ()
{
...

  {
    ...
    server = application_get (hsm->app_index);
    clib_warning ("wrk_index: %u\n",
		  application_get_default_worker (server)->wrk_index);
  }

  return 0;
}

/* create per fib listeing socket for the HTTP redirect server */
static int
http_redirect_server_listen (u32 fib_index, int is_ip4)
{
  ...
  *cfg = (session_endpoint_cfg_t) SESSION_ENDPOINT_CFG_NULL;
  ...
  if (is_ip4)
    {
      vec_validate_init_empty (hsm->ip4_listen_session_by_fib_index,
			       fib_index, 0);
      hsm->ip4_listen_session_by_fib_index[fib_index] = tl->session_index;
    }
  else
    {
      vec_validate_init_empty (hsm->ip6_listen_session_by_fib_index,
			       fib_index, 0);
      hsm->ip6_listen_session_by_fib_index[fib_index] = tl->session_index;
    }
...
http_redirect_server_create (vlib_main_t * vm, u32 fib_index, int is_ip4)
{
  ...
  if ((rv = http_redirect_server_listen (fib_index, is_ip4)))
    {
      assert (rv == 0);
...
u32
upf_http_redirect_server_create (u32 fib_index, int is_ip4)
{
  ...
  return is_ip4 ?
    http_redirect_server_main.ip4_listen_session_by_fib_index[fib_index] :
