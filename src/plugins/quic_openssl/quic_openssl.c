...
quic_openssl_engine_init (quic_main_t *qm)
{
  ...
  // Set the QUIC transport method
  SSL_CTX_set_quic_method(qom->ssl_ctx, &quic_transport_method);
...
quic_openssl_crypto_context_acquire (quic_ctx_t *ctx)
{
  ...
  if (ctx->connection_id.len > 0) {
    SSL_set_quic_transport_params(qom->ssl_conn,
                                ctx->connection_id.data,
                                ctx->connection_id.len);
  }
              ...
              clib_thread_index_t thread_index, struct sockaddr *sa)
{
  ...
  if (sa->sa_family == AF_INET) {
    ...
    if (!SSL_set1_initial_peer_addr(qom->ssl_conn, sa)) {
