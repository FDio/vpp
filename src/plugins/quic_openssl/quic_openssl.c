                           ...
                           unsigned int inlen, void *arg)
{
  ...
  if (SSL_select_next_proto((unsigned char **)out, outlen,
                           supported_alpn, sizeof(supported_alpn),
...
quic_openssl_engine_init (quic_main_t *qm)
{
  ...
  // OpenSSL 3.5.0 API: Set QUIC transport callbacks using SSL_CTX_set_quic_transport_params
  // Note: OpenSSL 3.5.0 handles QUIC transport internally, no need for custom method
  // The OSSL_QUIC_client_method() already includes all necessary QUIC handling
  ...
  if (SSL_CTX_set_alpn_protos(qom->ssl_ctx, alpn_protocols, sizeof(alpn_protocols)) != 0) {
...
quic_openssl_debug_connection(quic_ctx_t *ctx)
{
  ...
  if (alpn_selected && alpn_len > 0) {
    char alpn_str[32];
    clib_memcpy(alpn_str, alpn_selected, clib_min(alpn_len, sizeof(alpn_str) - 1));
...
quic_openssl_crypto_context_acquire (quic_ctx_t *ctx)
{
  ...
  QUIC_DBG (3, "Crypto context acquired successfully for ctx %u", ctx->c_c_index);
  return 0;
}
             ...
             u8 is_unidir)
{
  ...
  *quic_stream = stream;
  QUIC_DBG(3, "Created new QUIC stream: %p (unidirectional: %u)", stream, is_unidir);
  return 0;
}
...
quic_openssl_connect_stream_error_reset (void *quic_stream)
{
  ...
  // Use OpenSSL 3.5.0 API for stream reset
  SSL_STREAM_RESET_ARGS reset_args = {
    .quic_error_code = 0x01, // QUIC_ERR_INTERNAL_ERROR
  };

  if (SSL_stream_reset(stream, &reset_args, sizeof(reset_args)) != 1) {
    QUIC_ERR("Failed to reset QUIC stream");
  }

  QUIC_DBG(3, "Reset QUIC stream: %p", stream);
}
...
quic_openssl_connection_get_stats (void *conn, quic_stats_t *stats)
{
  ...
  // Use OpenSSL 3.5.0 API to get connection statistics
  OSSL_PARAM params[3];
  uint64_t rtt = 0;
  size_t rtt_len = sizeof(rtt);

  params[0] = OSSL_PARAM_construct_uint64("rtt", &rtt, &rtt_len);
  params[1] = OSSL_PARAM_construct_end();

  if (SSL_get_params(ssl_conn, params) == 1) {
...
static int quic_set_encryption_secrets(SSL *ssl, OSSL_ENCRYPTION_LEVEL level,
                                     ...
                                     size_t secret_len)
{
  // OpenSSL 3.5.0 API for setting encryption secrets
  if (!SSL_provide_quic_data(ssl, level, read_secret, secret_len) ||
      !SSL_provide_quic_data(ssl, level, write_secret, secret_len)) {
    QUIC_ERR("Failed to provide QUIC encryption secrets");
    return 0;
  }
  return 1;
}
...
{
  // OpenSSL 3.5.0 API for adding handshake data
  if (!SSL_provide_quic_data(ssl, level, data, len)) {
    QUIC_ERR("Failed to provide QUIC handshake data");
    return 0;
  }
  return 1;
}
...
{
  // OpenSSL 3.5.0 handles flight flushing internally
  // This is typically a no-op in OpenSSL QUIC implementation
  return 1;
}

static int quic_send_alert(SSL *ssl, OSSL_ENCRYPTION_LEVEL level, uint8_t alert)
{
  // OpenSSL 3.5.0 API for sending alerts
  SSL_QUIC_ALERT_DATA alert_data = {
    .alert_code = alert,
    .error_code = alert,
    .reason = NULL,
    .reason_len = 0
  ...
  if (!SSL_QUIC_send_alert(ssl, level, &alert_data)) {
    QUIC_ERR("Failed to send QUIC alert");
    return 0;
  }
  return 1;
}

// Define the QUIC method structure using OpenSSL 3.5.0 API
static const SSL_QUIC_METHOD quic_transport_method = {
...
// Add certificate loading if needed
static int
quic_openssl_load_certificates(SSL_CTX *ctx, const char *cert_file, const char *key_file)
{
  if (SSL_CTX_use_certificate_file(ctx, cert_file, SSL_FILETYPE_PEM) <= 0) {
    QUIC_ERR("Failed to load certificate file");
    return -1;
  }

  if (SSL_CTX_use_PrivateKey_file(ctx, key_file, SSL_FILETYPE_PEM) <= 0) {
    QUIC_ERR("Failed to load private key file");
    return -1;
  }

  if (!SSL_CTX_check_private_key(ctx)) {
    QUIC_ERR("Certificate and private key do not match");
    return -1;
  }
...
quic_openssl_validate_alpn(SSL *ssl)
{
  ...
  if (alpn_selected && alpn_len > 0) {
    ...
    QUIC_DBG(2, "ALPN negotiated protocol: %s (length: %u)", alpn_str, alpn_len);
    ...
    else if (alpn_len == 8 && clib_memcmp(alpn_selected, "hq-inter", 8) == 0) {
