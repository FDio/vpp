...
static int quic_set_encryption_secrets(SSL *ssl, enum ssl_encryption_level_t level,
                                     const uint8_t *read_secret,
                                     const uint8_t *write_secret,
                                     size_t secret_len)
{
  if (SSL_set_quic_transport_params(ssl, level, read_secret, write_secret, secret_len) != 1) {
    QUIC_ERR("Failed to set QUIC transport parameters");
    return -1;
  }
  return 0;
}

static int quic_add_handshake_data(SSL *ssl, enum ssl_encryption_level_t level,
                                 const uint8_t *data, size_t len)
{
  if (SSL_add_handshake_data(ssl, level, data, len) != 1) {
    QUIC_ERR("Failed to add handshake data");
    return -1;
  }
  return 0;
}
...
{
  if (SSL_flush_flight(ssl) != 1) {
    QUIC_ERR("Failed to flush flight data");
    return -1;
  }
  return 0;
}
...
static int quic_send_alert(SSL *ssl, enum ssl_encryption_level_t level,
                         uint8_t alert)
{
  if (SSL_send_alert(ssl, level, alert) != 1) {
    QUIC_ERR("Failed to send alert");
    return -1;
  }
  return 0;
}

// Define the QUIC method structure
