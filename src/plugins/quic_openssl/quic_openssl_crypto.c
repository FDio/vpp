...
quic_openssl_create_ssl_ctx (u8 is_server)
{
  ...
  // Load OpenSSL 3.x providers if needed
  quic_openssl_load_openssl3_legacy_provider ();
  ...
  if (SSL_CTX_set_alpn_protos (ssl_ctx, alpn_protocols,
...
quic_openssl_crypto_init_per_thread (quic_main_t *qm, u8 thread_index)
{
  ...
  pool_init_fixed (cwctx->crypto_ctx_pool, QUIC_CRYPTO_CTX_POOL_PER_THREAD_SIZE);

  QUIC_DBG (2, "Initialized OpenSSL crypto context pool for thread %u", thread_index);
}
...
int
quic_openssl_crypto_context_init (quic_openssl_crypto_context_t *crctx, u8 is_server)
{
  ...
  if (!data->rbio || !data->wbio)
    {
      QUIC_ERR ("Failed to create BIOs for crypto context %u", crctx->ctx_index);
      if (data->rbio) BIO_free (data->rbio);
      if (data->wbio) BIO_free (data->wbio);
  ...
  if (!is_server && SSL_set_alpn_protos (data->ssl_conn, alpn_protocols,
                           alpn_protocols_len) != 0)
    {
      QUIC_ERR ("Failed to set ALPN protocols on SSL connection for crypto context %u",
                crctx->ctx_index);
      SSL_free (data->ssl_conn);
      data->ssl_conn = NULL;
      return -1;
  ...
  if (RAND_bytes ((unsigned char *)data->cid_key, QUIC_OPENSSL_IV_LEN) != 1)
    {
      QUIC_ERR ("Failed to generate CID key for crypto context %u", crctx->ctx_index);
      SSL_free (data->ssl_conn);
      data->ssl_conn = NULL;
      return -1;
    }
