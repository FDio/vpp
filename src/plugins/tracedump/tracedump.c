...
vl_api_trace_v3_dump_t_handler (vl_api_trace_v3_dump_t *mp)
{
  ...
  int i, j, p;
  u32 client_index;
  u32 first_position, max, first_thread_id, last_thread_id, cache_size;
  u32 n_threads = vlib_get_n_threads ();
  u8 *s = 0;
  ...
  first_position = clib_net_to_host_u32 (mp->position);
  max = clib_net_to_host_u32 (mp->max);
  cache_size = clib_net_to_host_u32 (mp->cache_size);
  cache_size = cache_size < max ? max : cache_size;
	  ...
	  pool_foreach (th, tm->trace_buffer_pool)
	    {
	      ...
	      // to avoid blocking threads for to long
	      // if there is a lot of trace
	      if (++p == cache_size)
		break;
	    }
