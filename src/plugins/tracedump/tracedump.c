...
/* API message handler */
static void
vl_api_tracedump_t_handler (vl_api_tracedump_t * mp)
{
  ...
  for (i = iterator_thread_id; i < vec_len (client_trace_cache); i++)
      ...
      for (j = iterator_position; j < vec_len (client_trace_cache[i]); j++)
	{
	  ...
	  rmp->_vl_msg_id =
	    htons (VL_API_TRACEDUMP_REPLY + (tdmp->msg_id_base));
  ...
  if (sent_some == 0)
    {
      /* Don't complain if the client told us to flush the cache */
      if (iterator_thread_id != ~0 && iterator_position != ~0)
	{
	  rmp = vl_msg_api_alloc (sizeof (*rmp));
	  rmp->_vl_msg_id = htons (VL_API_TRACEDUMP_REPLY
				   + (tdmp->msg_id_base));
	  rmp->context = mp->context;
	  rmp->retval = clib_host_to_net_u32 (VNET_API_ERROR_NO_SUCH_ENTRY);
	  vl_api_send_msg (rp, (u8 *) rmp);
	}
    }
  vec_free (s);
