...
static void
graph_node_print (vlib_main_t *vm, vlib_node_t *n, bool want_arcs)
{
...
static clib_error_t *
graph_node_show_cmd (vlib_main_t *vm, unformat_input_t *input,
...
VLIB_CLI_COMMAND (graph_node_show_command, static) = {
  .path = "show graph",
  .short_help =
    "show graph [node <index>|<name>] [want_arcs] [input|trace_supported] "
    "[drop] [output] [punt] [handoff] [no_free] [polling] [interrupt]",
