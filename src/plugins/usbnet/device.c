...
usbnet_ctrl_in (vlib_main_t *vm, usbnet_dev_t *ud, u8 req, void *data, u16 len)
{
  ...
  rv = libusb_control_transfer (ud->dh,
				...
				/* timeout */ USB_TIMEOUT);
  if (rv < 0)
    return vnet_error (VNET_ERR_SYSCALL_ERROR_5, "usbnet_ctrl_in: %s",
		       libusb_strerror (rv));
  return 0;
}
...
usbnet_delete_if_internal (vlib_main_t *vm, usbnet_dev_t *ud)
{
  ...
  if (ud->data_if_claimed)
    libusb_release_interface (ud->dh, ud->data_if);

  if (ud->ctrl_if_claimed)
    libusb_release_interface (ud->dh, ud->ctrl_if);
...
usbnet_create_if (vlib_main_t *vm, usbnet_create_if_args_t *args)
{
  ...
  const struct libusb_interface_descriptor *ctrl_if_desc;
  clib_error_t *err = 0;
  int rv;
  ...
  for (u8 cfg_index = 0; cfg_index < dd.bNumConfigurations; cfg_index++)
      ...
      for (u32 k = 0; k < cd->bNumInterfaces; k++)
	{
	  ...
	  else if (id->bInterfaceSubClass == USB_CDC_SUBCLASS_MBIM &&
  ...
  if (dd.bNumConfigurations > 1)
      ...
      if (acd->bConfigurationValue != cd->bConfigurationValue)
	{
	  log_debug (ud, "configuration change needed (%u -> %u)",
		     acd->bConfigurationValue, cd->bConfigurationValue);
	  ...
	  if ((rv = libusb_set_configuration (ud->dh,
  ...
  if ((err = usbnet_ctrl_in (vm, ud, CDC_NCM_GET_NTB_PARAMETERS, &ntb_param,
			     sizeof (ntb_param))))
    goto done;
