...
usbnet_create_if (vlib_main_t *vm, usbnet_create_if_args_t *args)
{
  ...
  vlib_usb_get_dev_desc (ud->dh, USB_DT_DEVICE, 0, &dd, sizeof (dd));

  log_debug (ud, "%U", format_usb_desc, &dd);
  log_debug (ud, "iManufacturer '%U' iProduct '%U' iSerialNumber '%U'",
	     format_vlib_usb_string_desc, ud->dh, dd.iManufacturer,
	     format_vlib_usb_string_desc, ud->dh, dd.iProduct,
	     format_vlib_usb_string_desc, ud->dh, dd.iSerialNumber);

  dt = vlib_usb_device_config_get (ud->dh);
  ...
  if (dd.bNumConfigurations > 1)
...
#if 0
      int rv =
	ioctl (ud->usbdev->fd, USBDEVFS_CLAIMINTERFACE, &(unsigned int){ 0 });
      log_notice (ud, "rv %d", rv);
      ...
      if (active_config != required_config)
	{
	  ...
	  int rv = ioctl (ud->usbdev->fd, USBDEVFS_DISCONNECT_CLAIM, &dc);
	  log_notice (ud, "rv %d", rv);
	  rv = ioctl (ud->usbdev->fd, USBDEVFS_CLAIMINTERFACE, &(int){ 0 });
	  log_notice (ud, "0rv %d", rv);
	  rv = ioctl (ud->usbdev->fd, USBDEVFS_CLAIMINTERFACE, &(int){ 1 });
	  log_notice (ud, "1rv %d", rv);
	  rv = ioctl (ud->usbdev->fd, USBDEVFS_CLAIMINTERFACE, &(int){ 2 });
	  log_notice (ud, "2rv %d", rv);
#endif
  ...
  log_notice (ud, "claim %u rv %d", ctrl_desc->interface.bInterfaceNumber,
	      format_clib_error, ue);
