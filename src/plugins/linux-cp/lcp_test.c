...
api_lcp_itf_pair_add_del (vat_main_t * vam)
{
  ...
  if (ns)
    clib_memcpy (mp->namespace, ns, clib_strnlen((char *) ns, IFNAMSIZ - 1));
...
vl_api_lcp_itf_pair_details_t_handler (vl_api_lcp_itf_pair_details_t *mp)
{
  ...
  fformat (vam->ofp,
	   "phy sw_if_index %u host_sw_if_index %u tap %s vif_index %u netns %s\n",
...
api_lcp_default_ns_set (vat_main_t *vam)
{
  ...
  if (ns)
    {
      int len = clib_strnlen((char *) ns, IFNAMSIZ);
      if (len >= IFNAMSIZ)
	{
