...
#define foreach_lip_xc_l3				  \
  _(XC_IP4, "L3 x-connnect IPv4")			  \
  _(XC_IP6, "L3 x-connnect IPv6")

typedef enum
{
#define _(sym,str) LIP_XC_L3_NEXT_##sym,
  foreach_lip_xc_l3
#undef _
    LIP_XC_L3_N_NEXT,
} lip_xc_l3_next_t;
 ...
 * X-connect all packets from the HOST to the PHY.
 */
VLIB_NODE_FN (lip_xc_l3_node) (vlib_main_t * vm,
			       vlib_node_runtime_t * node,
			       vlib_frame_t * frame)
{
  u32 n_left_from, *from, *to_next, n_left_to_next;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  vnet_buffer (b0)->sw_if_index[VLIB_TX] = lip->lip_phy_sw_if_index;
	  if (PREDICT_TRUE ((b0->data[0] & 0xf0) == 0x40))
	    {
	      next0 = LIP_XC_L3_NEXT_XC_IP4;
	      vnet_buffer (b0)->ip.adj_index[VLIB_TX] =
		lip->lip_phy_adjs.ip4_adj_index;
	    }
	  else
	    {
	      next0 = LIP_XC_L3_NEXT_XC_IP6;
	      vnet_buffer (b0)->ip.adj_index[VLIB_TX] =
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (lip_xc_l3_node) = {
  .name = "linux-cp-xc-l3",
  .vector_size = sizeof (u32),
  .format_trace = format_lip_xc_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,

  .n_next_nodes = LIP_XC_L3_N_NEXT,
  .next_nodes = {
    [LIP_XC_L3_NEXT_XC_IP4] = "ip4-midchain",
    [LIP_XC_L3_NEXT_XC_IP6] = "ip6-midchain",
  },
};

VNET_FEATURE_INIT(lip_xc_node_l3, static) =
{
  .arc_name = "device-input",
  .node_name = "linux-cp-xc-l3",
  .runs_before = VNET_FEATURES ("ethernet-input"),
};
/* *INDENT-ON* */
...
#define foreach_lcp_ethernet_error			    \
_(PACKETS, "ARP packets processed")			    \
_(COPIES, "ARP replies copied to host")

typedef enum
{
#define _(sym,str) LCP_ETHERNET_ERROR_##sym,
  foreach_lcp_ethernet_error
#undef _
  LCP_ETHERNET_N_ERROR,
} lcp_ethernet_error_t;

char *lcp_ethernet_error_strings[] = {
				  ...
				  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 2  && n_left_to_next >= 2)
	{
	  ...
	  next0 = next1 = LIP_ETHERNET_NEXT_DROP;
  ...
  if (n_copies)
    {
      vlib_buffer_enqueue_to_single_next (vm, node, reply_copies,
					  LIP_ETHERNET_NEXT_IO, n_copies);
      vlib_error_count (vm, node->node_index, LCP_ETHERNET_ERROR_COPIES,
			n_copies);
    }

  vlib_error_count (vm, node->node_index, LCP_ETHERNET_ERROR_PACKETS,
		    frame->n_vectors);
...
VLIB_REGISTER_NODE (lcp_ethernet_node) = {
  ...
  .n_errors = LCP_ETHERNET_N_ERROR,
  .error_strings = lcp_ethernet_error_strings,
  .n_next_nodes = LIP_ETHERNET_N_NEXT,
  .next_nodes = {
    [LIP_ETHERNET_NEXT_DROP] = "error-drop",
    [LIP_ETHERNET_NEXT_IO] = "interface-output",
  },
};

VNET_FEATURE_INIT (lcp_ethernet_arp_feat, static) =
{
  .arc_name = "arp",
  .node_name = "linux-cp-ethernet",
