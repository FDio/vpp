...
#define foreach_lcp_xc_mpls_next                                              \
  _ (DROP, "mpls-drop")                                                       \
  _ (IO, "mpls-output")

typedef enum
{
#define _(sym, str) LCP_XC_MPLS_NEXT_##sym,
  foreach_lcp_xc_mpls_next
#undef _
    LCP_XC_MPLS_N_NEXT,
} lcp_xc_mpls_next_t;

// ### NOTE(pim) BEGIN of incorrect code
...
// ### NOTE(pim) END of incorrect code
...
VLIB_REGISTER_NODE (lcp_xc_mpls) = { .name = "linux-cp-xc-mpls",
				     ...
				     .next_nodes = {
				       [LCP_XC_MPLS_NEXT_DROP] = "mpls-drop",
				       [LCP_XC_MPLS_NEXT_IO] = "mpls-output",
				     } };
