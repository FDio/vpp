...
lcp_osi_init (vlib_main_t *vm)
{
  ...
  fn = (osi_register_input_protocol_fn) vlib_get_plugin_symbol (
    "osi_plugin.so", "osi_register_input_protocol");

  if (fn == NULL)
    {
      error = clib_error_return (
	0, "osi_register_input_protocol not found in osi_plugin.so");
      return error;
    }
