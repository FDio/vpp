		  ...
		  u32 phy_sw_if_index, u8 * host_name, u32 host_index)
{
  ...
  if (INDEX_INVALID == lipi)
	...
	{
	  vec_validate_init_empty (lip_db_by_host, lip->lip_host_sw_if_index,
				   0);
...
int
lcp_itf_pair_create(u32 phy_sw_if_index, u8 *host_tap_name)
{
  vlib_main_t *vm;
  vnet_main_t *vnm;
		      ...
		      vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  while (1)
	...
	lcp_itf_pair_create(lipn->lip_phy_sw_if_index, lipn->lip_host_name);
      }
