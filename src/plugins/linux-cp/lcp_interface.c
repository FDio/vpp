...
lcp_itf_set_adjs (lcp_itf_pair_t * lip)
{
  if (lip->lip_host_type == LCP_ITF_HOST_TUN)
	...
	adj_mcast_add_or_lock (FIB_PROTOCOL_IP4,
			       VNET_LINK_IP4, lip->lip_phy_sw_if_index);
      lip->lip_phy_adjs.adj_index[AF_IP6] =
	adj_mcast_add_or_lock (FIB_PROTOCOL_IP6,
			       VNET_LINK_IP6, lip->lip_phy_sw_if_index);
    }
}
		  ...
		  u32 host_index, lip_host_type_t host_type, u8 * ns)
{
  ...
  FOR_EACH_IP_ADDRESS_FAMILY (af)
    ip_feature_enable_disable (af, N_SAFI, IP_FEATURE_INPUT,
			       lcp_itf_l3_feat_names[lip->lip_host_type]
			       [af], lip->lip_host_sw_if_index, 1, NULL, 0);
...
lcp_itf_pair_delete (u32 phy_sw_if_index)
{
  ...
  LCP_ITF_PAIR_INFO ("pair delete: {%U, %U, %s}",
		     format_vnet_sw_if_index_name, vnet_get_main (),
		     lip->lip_phy_sw_if_index,
		     format_vnet_sw_if_index_name, vnet_get_main (),
  ...
  FOR_EACH_IP_ADDRESS_FAMILY (af)
    ip_feature_enable_disable (af, N_SAFI, IP_FEATURE_INPUT,
			       lcp_itf_l3_feat_names[lip->lip_host_type]
			       [af], lip->lip_host_sw_if_index, 0, NULL, 0);
  ...
  if (vnet_sw_interface_is_sub (vnet_get_main (), lip->lip_host_sw_if_index))
    {
      lcp_netlink_del_link ((const char *) lip->lip_host_name);
		     ...
		     lip_host_type_t host_if_type, u8 * ns)
{
  vlib_main_t *vm;
  vnet_main_t *vnm;
  u32 vif_index, host_sw_if_index;
  ...
  if (vnet_sw_interface_is_sub (vnm, phy_sw_if_index))
      ...
      if (!vif_index)
	{
       ...
       */
      if (!err && vnet_create_sub_interface (lip->lip_host_sw_if_index, vlan,
					     18, 0, vlan, &host_sw_if_index))
	LCP_ITF_PAIR_INFO ("failed create vlan: %d on %U",
			   vlan,
			   format_vnet_sw_if_index_name, vnet_get_main (),
  ...
  lcp_itf_pair_add (host_sw_if_index,
		    phy_sw_if_index,
		    host_if_name, vif_index, host_if_type, ns);
