...
lcp_netlink_add_link_vrf (u32 table_id, const char *name)
{
  ...
  sk = nl_socket_alloc ();
  if ((rc = nl_connect (sk, NETLINK_ROUTE)) < 0)
    {
      nl_close (sk);
      LCP_ITF_PAIR_ERR ("netlink_add_link_vrf: Netlink connect error: %s",
			nl_geterror (rc));
      return clib_error_return (NULL, "Unable to connect socket: %d", rc);
    }
  ...
  err = NULL;
  if ((rc = rtnl_link_add (sk, link, NLM_F_CREATE)) < 0)
    {
      LCP_ITF_PAIR_ERR ("netlink_add_link_vrf: Link \"%s\" add error: %s",
			name, nl_geterror (rc));
      err = clib_error_return (NULL, "Unable to add link %s: %d", name, rc);
    }
