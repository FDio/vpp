...
lcp_netlink_add_link_vlan (int parent, u32 vlan, u16 proto, const char *name)
{
  ...
  if ((err = rtnl_link_vlan_set_id (link, vlan)) < 0)
    {
      LCP_ITF_PAIR_ERR ("netlink_add_link_vlan: vlan set error: %s",
			nl_geterror (err));
      return clib_error_return (NULL, "Unable to set VLAN %d on link %s: %d",
				vlan, name, err);
    }

  if ((err = rtnl_link_vlan_set_protocol (link, htons (proto))) < 0)
    {
      LCP_ITF_PAIR_ERR ("netlink_add_link_vlan: proto set error: %s",
			nl_geterror (err));
      return clib_error_return (NULL, "Unable to set proto %d on link %s: %d",
				proto, name, err);
    }
...
lcp_itf_pair_find_walk (vnet_main_t *vnm, u32 sw_if_index, void *arg)
{
  ...
  if (sw && (sw->sub.eth.inner_vlan_id == 0) &&
    ...
    {
      LCP_ITF_PAIR_ERR (
	"pair_create: found match %U outer %d dot1ad %d inner-dot1q %d",
...
/* Return the index of the sub-int on thie phy that has the given vlan and
 * proto */
static index_t
lcp_itf_pair_find_by_outer_vlan (u32 hw_if_index, u16 vlan, bool dot1ad)
{
  ...
  vnet_hw_interface_walk_sw (vnet_get_main (), hw_if_index,
		     ...
		     u32 *host_sw_if_indexp)
{
  ...
  if (vnet_sw_interface_is_sub (vnm, phy_sw_if_index))
      ...
      if (!llip)
	{
      ...
      LCP_ITF_PAIR_ERR ("pair_create: linux parent %U", format_lcp_itf_pair,
			llip);
      ...
      if (!vif_index)
	{
	  ...
	  if (!err && -1 != ns_fd)
		...
		{
		  LCP_ITF_PAIR_ERR (
		    "pair_create: cannot set link name:'%s' in namespace:'%s'",
		    host_if_name, ns);
		}
	    }
	  if (!err)
