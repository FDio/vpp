				...
				vlib_cli_command_t * cmd)
{
  ...
  r = lcp_itf_pair_create(sw_if_index, tap_name);

  vec_free (tap_name);

  if (r)
      return clib_error_return (0,
				"linux-cp pair creation failed (%d)", r);
  return 0;
}
...
VLIB_CLI_COMMAND (lcp_itf_pair_create_command, static) = {
	.path = "lcp create",
	.short_help = "liux-cp create  <sw_if_index>|<if-name> tap <tap-name>",
