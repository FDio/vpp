...
#define LCP_NETLINK_DBG(...) vlib_log_debug (lcp_netlink_logger, __VA_ARGS__);

#define LCP_NETLINK_INFO(...) vlib_log_notice (lcp_netlink_logger, __VA_ARGS__);

#define LCP_NETLINK_ERROR(...) vlib_log_err (lcp_netlink_logger, __VA_ARGS__);
...
lcp_netlink_lip_ts_check (nl_msg_info_t *msg_info, lcp_itf_pair_t *lip)
{
  ...
  LCP_NETLINK_INFO ("Early message received for %U",
...
lcp_netlink_link_del (struct rtnl_link *rl, void *ctx)
{
  ...
  if (INDEX_INVALID != lipi)
	...
	return;
      ...
      LCP_NETLINK_INFO ("delete link: %s - %U", rtnl_link_get_type (rl),
		       format_vnet_sw_if_index_name, vnet_get_main (),
		       lip->lip_phy_sw_if_index);
	...
	{
	  LCP_NETLINK_INFO ("delete vlan: %s -> %U", rtnl_link_get_name (rl),
			   format_vnet_sw_if_index_name, vnet_get_main (),
			   lip->lip_phy_sw_if_index);
    ...
    LCP_NETLINK_INFO ("ignore link del: %s - %s", rtnl_link_get_type (rl),
...
lcp_netlink_link_add (struct rtnl_link *rl, void *ctx)
{
  ...
  if (INDEX_INVALID != lipi)
      ...
      else
	{
      ...
      LCP_NETLINK_DBG ("link: %s (%d) -> %U/%U %s", rtnl_link_get_name (rl),
      ...
      if (INDEX_INVALID != lipi)
	{
	  ...
	  if (vnet_create_sub_interface (lip->lip_phy_sw_if_index, vlan, 18, 0,
	    ...
	    {
	      LCP_NETLINK_INFO ("failed create phy vlan: %s on %U",
			       rtnl_link_get_name (rl),
			       format_vnet_sw_if_index_name, vnet_get_main (),
			       lip->lip_phy_sw_if_index);
	      return;
	  ...
	  if (vnet_create_sub_interface (lip->lip_host_sw_if_index, vlan, 18,
	    ...
	    {
	      LCP_NETLINK_INFO ("failed create vlan: %s on %U",
			       rtnl_link_get_name (rl),
			       format_vnet_sw_if_index_name, vnet_get_main (),
	...
	{
	  LCP_NETLINK_INFO ("ignore parent-link add: %s - %s",
			   rtnl_link_get_type (rl), rtnl_link_get_name (rl));
	}
    }
  else
    LCP_NETLINK_INFO ("ignore link add: %s - %s", rtnl_link_get_type (rl),
		     rtnl_link_get_name (rl));
}
...
static void
lcp_netlink_mk_addr (const struct nl_addr *rna, ip_address_t *ia)
{
  ...
  if (~0 != sw_if_index)
    {
      ...
      lcp_netlink_mk_addr (rtnl_addr_get_local (rla), &nh);
      ...
      else if (AF_IP6 == ip_addr_version (&nh))
	{
      ...
      LCP_NETLINK_DBG ("link-addr: %U %U/%d", format_vnet_sw_if_index_name,
...
lcp_netlink_neigh_del (struct rtnl_neigh *rn)
{
  ...
  sw_if_index = lcp_netlink_intf_h2p (rtnl_neigh_get_ifindex (rn));
  ...
  if (~0 != sw_if_index)
    {
      ...
      lcp_netlink_mk_addr (rtnl_neigh_get_dst (rn), &nh);
	...
	{
	  LCP_NETLINK_DBG ("neighbor del: %U %U", format_ip_address, &nh,
			  format_vnet_sw_if_index_name, vnet_get_main (),
			  sw_if_index);
	}
    }
  else
    LCP_NETLINK_INFO ("ignore neighbour del on: %d",
  ...
  if (~0 != sw_if_index)
      ...
      if (ll && (state & NUD_VALID))
	{
	  ...
	  if (rv)
	    ...
	    {
	      LCP_NETLINK_DBG ("neighbor add: %U %U", format_ip_address, &nh,
			      format_vnet_sw_if_index_name, vnet_get_main (),
			      sw_if_index);
	    }
	}
	...
	/* It's a delete */
	lcp_netlink_neigh_del (rn);
    }
  else
    LCP_NETLINK_INFO ("ignore neighbour add on: %d",
		     rtnl_neigh_get_ifindex (rn));
}
...
lcp_netlink_route_path_parse (struct rtnl_nexthop *rnh, void *arg)
{
  ...
  if (~0 != sw_if_index)
	...
	path->frp_mitf_flags = MFIB_ITF_FLAG_FORWARD;
      ...
      LCP_NETLINK_DBG (" path:[%U]", format_fib_route_path, path);
    }
}
...
lcp_netlink_route_del (struct rtnl_route *rr)
{
  ...
  nlt = lcp_netlink_table_find (lcp_netlink_table_k2f (table_id), pfx.fp_proto);

  LCP_NETLINK_DBG ("route del: %d:%U %U", rtnl_route_get_table (rr),
...
lcp_netlink_route_add (struct rtnl_route *rr)
{
  ...
  if (FIB_PROTOCOL_IP6 == pfx.fp_proto &&
    ...
    {
      LCP_NETLINK_DBG ("route skip: %d:%U %U", rtnl_route_get_table (rr),
		      format_fib_prefix, &pfx, format_fib_entry_flags,
		      entry_flags);
    }
  else
    {
      LCP_NETLINK_DBG ("route add: %d:%U %U", rtnl_route_get_table (rr),
		      format_fib_prefix, &pfx, format_fib_entry_flags,
		      entry_flags);
	...
	LCP_NETLINK_DBG ("no paths for route add: %d:%U %U",
