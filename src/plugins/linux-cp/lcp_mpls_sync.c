...
lcp_mpls_sync_pair_add_cb (lcp_itf_pair_t *lip)
{
  ...
  if (phy_is_enabled)
    mpls_sw_interface_enable_disable (&mpls_main, lip->lip_host_sw_if_index, 1);
}

void
lcp_mpls_sync_state_cb (struct mpls_main_t *mm, uword opaque, u32 sw_if_index, u32 is_enable)
{
  lcp_itf_pair_t *lip;
  index_t lipi;
  int curr_ns_fd = -1;
