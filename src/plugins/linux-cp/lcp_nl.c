...
#define _(stype, mtype, tname, fn)                                            \
	  ...
	  do                                                                  \
	    {                                                                 \
	      n_msgs = lcp_nl_recv_dump_replies (stype, nm->sync_batch_limit, \
						 &is_done);                   \
	      if (n_msgs < 0)                                                 \
		{                                                             \
		  NL_ERROR ("Error receiving dump replies of type " tname     \
			    ": %s (%d)", nl_geterror (n_msgs), n_msgs);       \
		  break;                                                      \
		}                                                             \
	      else if (n_msgs == 0)                                           \
		{                                                             \
		  NL_ERROR ("EOF while receiving dump replies of type "       \
			    tname);                                           \
		  break;                                                      \
		}                                                             \
	      else                                                            \
		...
		vm, nm->sync_batch_delay_ms * 1e-3);                          \
	      event_type = vlib_process_get_events (vm, &event_data);         \
	      vec_reset_length (event_data);                                  \
                                                                              \
