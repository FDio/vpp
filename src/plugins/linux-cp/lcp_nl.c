...
nl_route_process_msgs (void)
{
  ...
  /* TODO: Add batching. Process a certain number of messages and then quit.
   * The goal is to make sure that the read buffer on the netlink socket
   * does not get filled up while we're processing the incoming messages. So
   * we ought to process a certain number of messages and then pause so there
   * is an opportunity for more incoming messages to be read and queued.
   * The maximum batch size can be on the order of 1k - 10k and could be
   * configurable via startup.conf settings or API messages.
   */
  vec_foreach (msg, nm->nl_msgs)
  {
		  ...
		  vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (1)
    {
      /* TODO: Add a timeout in order to support batching. Currently we only
       * wait for events. When an event occurs, we could process a subset of
       * the message queue and then yield so that more messages could be read
       * from the socket and queued. If we did not process the entire queue,
       * we would want to come back and continue processing after a short
       * delay rather than waiting for another event.
       * The period of time to delay between batches could be configurable
       * via startup.conf setting.
       */
...
nl_route_cb (struct nl_msg *msg, void *arg)
{
  ...
  /* increment ref count on msg and queue it for later processing */
  nlmsg_get (msg);
  vec_add1 (nm->nl_msgs, msg);

  /* notify process node of new message */
