...
lcp_itf_pair_sync_state (lcp_itf_pair_t *lip)
{
   ...
   */
  if ((sw->flags & VNET_SW_INTERFACE_FLAG_UNNUMBERED) && !lcp_sync_unnumbered ())
    {
      LCP_ITF_PAIR_NOTICE ("sync_state: not syncing addresses on unnumbered %U",
...
VNET_SW_INTERFACE_ADD_DEL_FUNCTION_PRIO (lcp_itf_interface_add_del, VNET_ITF_FUNC_PRIORITY_LOW + 1);
