...
#define LCP_ROUTER_DBG(...)                             \
  vlib_log_debug (lcp_router_logger, __VA_ARGS__);
  ...
  if (INDEX_INVALID != lipi)
	...
	if (lcp_router_lip_ts_check ((nl_msg_info_t *)ctx, lip))
	  return;
        ...
        LCP_ROUTER_INFO ("delete link: %s - %U",
			 rtnl_link_get_type (rl),
			 format_vnet_sw_if_index_name, vnet_get_main (),
			 lip->lip_phy_sw_if_index);
...
lcp_router_ip6_mroutes_add_del (u32 sw_if_index, u8 is_add)
{
  ...
  mfib_index = mfib_table_get_index_for_sw_if_index (FIB_PROTOCOL_IP6,
						     sw_if_index);
...
lcp_router_link_add (struct rtnl_link *rl, void *ctx)
{
  ...
  if (INDEX_INVALID != lipi)
      ...
      if (INDEX_INVALID != lipi)
	{
	  ...
	  if (vnet_create_sub_interface (lip->lip_phy_sw_if_index, vlan,
					 18, 0, vlan, &sub_phy_sw_if_index))
	    {
	  ...
	  if (vnet_create_sub_interface (lip->lip_host_sw_if_index, vlan,
					 18, 0, vlan, &sub_host_sw_if_index))
	    {
	  ...
	  lcp_itf_pair_add (sub_host_sw_if_index,
			    sub_phy_sw_if_index,
			    if_namev,
			    rtnl_link_get_ifindex (rl),
  ...
  if (~0 != sw_if_index)
      ...
      else if (AF_IP6 == ip_addr_version (&nh))
	{
      ...
      LCP_ROUTER_DBG ("link-addr: %U %U/%d", format_vnet_sw_if_index_name,
		      ...
		      format_ip_address, &nh, rtnl_addr_get_prefixlen (rla));
    }
}
  ...
  if (NULL == nlt)
    {
      ...
      nlt->nlt_fib_index =
	fib_table_find_or_create_and_lock (nlt->nlt_proto, nlt->nlt_id,
					   lcp_rt_fib_src);
      nlt->nlt_mfib_index =
...
{
  return (rt_proto <=
	  RTPROT_STATIC) ? lcp_rt_fib_src : lcp_rt_fib_src_dynamic;
}
...
lcp_router_route_add (struct rtnl_route *rr)
{
  ...
  if (FIB_PROTOCOL_IP6 == pfx.fp_proto &&
    ...
    {
      LCP_ROUTER_DBG ("route skip: %d:%U %U",
		      rtnl_route_get_table (rr), format_fib_prefix, &pfx,
		      format_fib_entry_flags, entry_flags);
    }
  else
    {
      LCP_ROUTER_DBG ("route add: %d:%U %U",
		      rtnl_route_get_table (rr), format_fib_prefix, &pfx,
		      format_fib_entry_flags, entry_flags);
...
lcp_router_init (vlib_main_t * vm)
{
   ...
   */
  lcp_rt_fib_src = fib_source_allocate ("lcp-rt",
					FIB_SOURCE_PRIORITY_HI,
					FIB_SOURCE_BH_API);

  lcp_rt_fib_src_dynamic = fib_source_allocate ("lcp-rt-dynamic",
						FIB_SOURCE_PRIORITY_HI + 1,
						FIB_SOURCE_BH_API);
