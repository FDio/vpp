...
ige_rx_queue_alloc (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  ...
  /* FIXME: check allocation result and unwind on failure. */
  clib_memset_u32 (iq->buffer_indices, 0, rxq->size);

  rv = vnet_dev_dma_mem_alloc (vm, dev, sizeof (ige_rx_desc_t) * rxq->size, 0,
			       (void **) &iq->descs);
  if (rv != VNET_DEV_OK)
    return rv; /* FIXME: free buffer_indices and release queue bitmap before
		  returning. */

  log_debug (dev, "queue %u allocated", rxq->queue_id);
  return rv;
}
...
ige_tx_queue_alloc (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  ...
  log_debug (dev, "queue %u allocated", txq->queue_id);
  return VNET_DEV_OK;
}
...
ige_rx_queue_free (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  ...
  /* FIXME: free iq->buffer_indices when tearing down to avoid leaking the CPU
   * ring. */
