	      ...
	      f64 intial_delay, f64 timeout)
{
  ...
  for (f64 delay = intial_delay, total_time = delay; total_time < timeout;
       delay *= 2, total_time += delay)
    {
      ...
      if ((val & mask) == match)
	{
	  log_debug (dev, "reg_poll: reg %05x (suspend %.6f)", reg,
		     vlib_time_now (vm) - t0);
	  return 1;
	}
      vlib_process_suspend (vm, delay);
    }
  log_debug (dev, "reg_poll: reg %05x timeout", reg);
  return 0;
}
...
{
  ige_reg_swsm_t swsm;
  log_debug (dev, "reg_sw_fw_sync_release:");
...
ige_reg_sw_fw_sync_acquire (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  log_debug (dev, "reg_sw_fw_sync_acquire:");
  ...
  if (i == timeout)
    {
      log_debug (dev, "reg_sw_fw_sync_acquire: timeout acquiring SWSM");
      return 0;
    }
  ...
  if (i == timeout)
    {
      ...
      swsm.swesmbi = 0;
      ige_reg_wr (dev, IGE_REG_SWSM, swsm.as_u32);
      log_debug (dev, "reg_sw_fw_sync_acquire: timeout acquring SWSMBI");
      return 0;
    }

  log_debug (dev, "reg_sw_fw_sync_acquire: acquired");
  return 1;
}
