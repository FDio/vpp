...
ige_init (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  clib_memcpy (&port.port.attr.hw_addr.addr[0], &tmp, 4);
  ige_reg_rd (id, IGE_REG_RAH0, &tmp);
  clib_memcpy (&port.port.attr.hw_addr.addr[4], &tmp, 2);
  port.port.attr.hw_addr.type = VNET_DEV_HW_ADDR_TYPE_ETHERNET;
