...
ige_init (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  ige_phy_rw_t rw[2] = { { .addr = 2 }, { .addr = 3 } };
  rv = ige_phy_rw (vm, dev, rw, ARRAY_LEN (rw));

  if (rv != VNET_DEV_OK)
    {
      log_err (dev, "failed to read PHY_ID");
      return rv;
    }

  u32 phyid = (u32) rw[0].data << 16 | rw[1].data;
  fformat (stderr, "PHY_ID 0x%08x\n", phyid);
