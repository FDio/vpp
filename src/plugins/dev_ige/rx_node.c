...
ige_rxq_refill (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq, int use_va_dma)
{
  ...
  buffer_pool_index = vnet_dev_get_rx_queue_buffer_pool_index (rxq);
  ...
  foreach_vnet_dev_rx_queue_runtime (rxq, node)
    {
      ...
      u16 mask = rxq->size - 1;
      ige_rx_desc_t *d = iq->descs + (iq->head & mask);

      if (d->hdr_addr)
	{
	  /* FIXME: packets are dropped instead of being enqueued to a next
	   * node. */
	  vlib_buffer_t *b =
	    vlib_get_buffer (vm, iq->buffer_indices[iq->head]);
	  /* FIXME: head should be masked when indexing buffer_indices to avoid
	   * overruns. */
	  if (0)
	    {
	      fformat (stderr,
