  ...
  if (ip->last_status.as_u32 != status.as_u32)
    {
      ...
      log_debug (dev, "port_status_poll: %U", format_ige_reg_diff,
      ...
      if (ip->last_status.full_duplex != status.full_duplex)
	{
	  ...
	  log_debug (dev, "port_poll: duplex changed to %s",
...
ige_port_start (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if (rctl.rx_enable)
    {
      log_warn (dev, "port_start: port %u rx is unexpectedlly enabled",
		port->port_id);
      rctl.rx_enable = 0;
...
{
  vnet_dev_t *dev = port->dev;
  log_debug (dev, "port stop: port %u", port->port_id);
  ige_reg_rctl_t rctl;
