...
ige_enq_tx_desc (vlib_main_t *vm, vlib_buffer_t *b, u32 bi, int first,
		 int last, ige_tx_desc_t *descs, u32 *buffer_indices,
		 u16 *tail, u16 mask, int use_va)
{
...
ige_txq_enq (vlib_main_t *vm, vlib_node_runtime_t *node,
	     vnet_dev_tx_queue_t *txq, u32 *from, u32 max_pkts, int use_va)
{
  ...
  while (n_pkts < max_pkts && tail < max_tail)
      ...
      if (b->flags & VLIB_BUFFER_NEXT_PRESENT)
	{
	  ...
	  ige_enq_tx_desc (vm, b, hbi, 1, 0, d, bi, &tail, mask, use_va);
	  for (u32 i = 0; i + 1 < n; i++)
	    ige_enq_tx_desc (vm, tb[i], tbi[i], 0, 0, d, bi, &tail, mask,
			     use_va);
	  ige_enq_tx_desc (vm, tb[n - 1], tbi[n - 1], 0, 1, d, bi, &tail, mask,
			   use_va);
	}
      else
	ige_enq_tx_desc (vm, b, hbi, 1, 1, d, bi, &tail, mask, use_va);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_reties--)
    {
      if (dev->va_dma)
	n = ige_txq_enq (vm, node, txq, from, n_left, 1);
      else
	n = ige_txq_enq (vm, node, txq, from, n_left, 0);
