...
pppox_output (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
      ...
      // A future rewrite could move this into a more generic adj/rewrite path.
	  ...
	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
	    {
	      /* No extra trace payload for the fast-path pair case today. */
	    }
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
	    {
	      /* No additional trace payload for the single-buffer path today. */
	    }
