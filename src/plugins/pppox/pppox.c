...
pppox_restart_dead_client (void)
{
  ...
  if (pppoe_client_restart_session_with_reason_func == 0 && pppoe_client_restart_session_func == 0)
    {
      pppoe_client_restart_session_with_reason_func =
	vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppoe_client_restart_session_with_reason");
  ...
  pool_foreach (vif, pom->virtual_interfaces)
      ...
      if (phase[unit] == PHASE_DEAD && vif->pppoe_session_allocated && !vif->delete_pending)
	{
	  if (pppoe_client_restart_session_with_reason_func)
	    (*pppoe_client_restart_session_with_reason_func) (
	      vif->pppoe_client_index, PPPOX_PPPOECLIENT_DISCONNECT_PPP_DEAD);
	  else
...
output (int unit, u8 *p, int len)
{
  ...
  if (PREDICT_FALSE (output_ctrl_pkt_fn == 0))
    {
      output_ctrl_pkt_fn =
	vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppoeclient_output_ctrl_pkt");
    }
	  ...
	  pppoe_client_restart_session_func == 0)
	{
	  pppoe_client_restart_session_with_reason_func =
	    vlib_get_plugin_symbol ("pppoeclient_plugin.so",
				    "pppoe_client_restart_session_with_reason");
	  if (pppoe_client_restart_session_with_reason_func == 0)
	    pppoe_client_restart_session_func =
	      vlib_get_plugin_symbol ("pppoeclient_plugin.so", "pppoe_client_restart_session");
	}
      if (pppoe_client_restart_session_with_reason_func)
	(*pppoe_client_restart_session_with_reason_func) (
	  client_index, PPPOX_PPPOECLIENT_DISCONNECT_PPP_DEAD);
