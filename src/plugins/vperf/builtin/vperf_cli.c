...
vp_server_create_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  u8 server_uri_set = 0, rx_buffer = 0, *appns_id = 0;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	vpsm->cfg.report_interval = 1;
      else if (unformat (input, "rx-buffer"))
	rx_buffer = 1;
 ...
 * @cliexcmd{vperf server uri tcp://6.0.1.2:1234}
 ?*/
VLIB_CLI_COMMAND (vp_server_create_command, static) = {
  ...
  .short_help = "vperf server [uri <proto://ip:port>] [fifo-size <bytes>[k|m|g]]\n"
		...
		"[prealloc-fifos <n>] [appns <id>] [report-interval [<seconds>]] [rx-buffer]",
 ...
 * second (zero copy):
 * @cliexcmd{vperf client uri tcp://6.0.1.2:1234}
 ?*/
