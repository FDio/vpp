...
vp_proto_server_stream_rx_no_echo (vp_test_session_t *es, session_t *s)
{
  ...
  int rv = svm_fifo_dequeue_drop (rx_fifo, n_read);
...
vp_proto_server_dgram_rx (vp_test_session_t *es, session_t *s, u8 *rx_buf, u8 test_bytes)
{
  ...
  if (PREDICT_FALSE (svm_fifo_peek (rx_fifo, 0, sizeof (ph), (u8 *) &ph) != sizeof (ph)))
    goto rx_stale;

  if (PREDICT_FALSE (ph.data_length < ph.data_offset ||
		     ph.data_length > fifo_bytes - SESSION_CONN_HDR_LEN))
    goto rx_stale;
  ...
  max_enqueue = svm_fifo_max_enqueue_prod (tx_fifo);
  if (PREDICT_FALSE (max_enqueue <= sizeof (session_dgram_hdr_t)))
    goto rx_event;
  max_enqueue -= sizeof (session_dgram_hdr_t);
  if (PREDICT_FALSE (max_enqueue < dgram_bytes))
    goto rx_event;
...
vp_proto_udp_server_rx_no_echo (vp_test_session_t *es, session_t *s)
{
  ...
  if (svm_fifo_needs_deq_ntf (rx_fifo, bytes_received))
    {
      svm_fifo_clear_deq_ntf (rx_fifo);
      session_program_transport_io_evt (s->handle, SESSION_IO_EVT_RX);
    }
...
vp_proto_client_dgram_rx_inline (vp_test_session_t *es, session_t *s, u8 *rx_buf, u8 test_bytes)
{
  ...
  if (svm_fifo_needs_deq_ntf (rx_fifo, n_read + sizeof (u32)))
    {
      svm_fifo_clear_deq_ntf (rx_fifo);
      session_program_transport_io_evt (s->handle, SESSION_IO_EVT_RX);
    }
...
vp_proto_http_proxy_send_resp (session_t *s, u8 is_udp)
{
  ...
  msg.code = is_udp ? HTTP_STATUS_SWITCHING_PROTOCOLS : HTTP_STATUS_OK;
...
vp_proto_http_read_connect_tcp_req (session_t *s)
{
  ...
  ASSERT (msg.method_type == HTTP_REQ_CONNECT);
  ASSERT (msg.data.upgrade_proto == HTTP_UPGRADE_PROTO_NA);
...
vp_proto_http_connect_read_resp (vp_test_session_t *es, session_t *s, u8 *rx_buf)
{
  ...
  ASSERT (msg.code == HTTP_STATUS_OK);
...
vp_proto_http_read_connect_udp_req (session_t *s)
{
  ...
  ASSERT (msg.type == HTTP_MSG_REQUEST);
  ASSERT (msg.method_type == HTTP_REQ_CONNECT);
  ASSERT (msg.data.upgrade_proto == HTTP_UPGRADE_PROTO_CONNECT_UDP);
  svm_fifo_dequeue_drop (s->rx_fifo, msg.data.len);
}
...
vp_proto_http_server_dgram_rx (vp_test_session_t *es, session_t *s, u8 *rx_buf, u8 test_bytes)
{
  ...
  while (max_dequeue > sizeof (ph))
      ...
      if (PREDICT_FALSE (max_enqueue < dgram_size))
	{
      ...
      ASSERT (vec_len (rx_buf) >= dgram_size);
      bytes_received += ph.data_length;
      es->dgrams_received++;
      es->dgrams_sent++;
	...
	vp_proto_test_bytes ((rx_buf + sizeof (session_dgram_hdr_t) + sizeof (u32)),
      ...
      ASSERT (rv == dgram_size);
      max_dequeue -= dgram_size;
      max_enqueue -= dgram_size;
    }
  es->bytes_received += bytes_received;
  es->bytes_sent += bytes_received;
  ...
  if (bytes_received && svm_fifo_set_event (tx_fifo))
    session_program_tx_io_evt (s->handle, SESSION_IO_EVT_TX);
				      ...
				      u8 test_bytes)
{
  ...
  es->bytes_received += bytes_received;
  es->bytes_to_receive -= bytes_received;
