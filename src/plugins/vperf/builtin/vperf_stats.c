...
static inline void
vp_update_rtt_stats (f64 session_rtt, vp_rtt_stat_t *rtt_stats)
{
...
void
vp_update_rtt_stats_tcp (vp_test_session_t *es, vp_rtt_stat_t *rtt_stats)
{
  ...
  if (s)
      ...
      if (PREDICT_TRUE (tc != NULL))
	{
	  ...
	  vp_update_rtt_stats (tcpc->srtt * TCP_TICK, rtt_stats);
	}
    }
}

void
vp_update_rtt_stats_udp (vp_test_session_t *es, vp_rtt_stat_t *rtt_stats)
{
  vp_update_rtt_stats (es->rtt, rtt_stats);
}
...
void
vp_print_footer (vlib_main_t *vm, vp_test_proto_t proto)
{
  if (proto == VP_PROTO_UDP)
    vp_cli (ECHO_SEPARATOR_UDP);
  else
    vp_cli (ECHO_SEPARATOR);
}

void
vp_print_periodic_stats (vlib_main_t *vm, u8 print_header, vp_test_cfg_t *cfg, vp_stats_t *stats,
      ...
      pool_foreach (sess, wrk->sessions)
	{
	  ...
	  if (cfg->proto == VP_PROTO_UDP)
	  ...
	  else if (cfg->proto == VP_PROTO_TCP)
	    {
	      ...
	      if (s)
		{
		  vp_update_rtt_stats_tcp (sess, &stats->rtt_stats);
  ...
  if (cfg->proto == VP_PROTO_UDP)
      ...
      if (print_header)
	{
	  ...
	  if (cfg->report_interval_total)
	    vp_cli ("Run time (s)  Transmitted   Received   Throughput   "
		    "%sSent/received dgrams",
		    (cfg->report_interval_jitter ? "Jitter      " : "Roundtrip   "));
	  else
	    vp_cli ("Interval (s)  Transmitted   Received   Throughput   "
		    "%sSent/received dgrams",
		    (cfg->report_interval_jitter ? "Jitter      " : "Roundtrip   "));
	}
      if (cfg->report_interval_total)
	{
	  vp_cli ("%-13.1f %-13U %-10U %+9Ub/s %+9.3fms %llu/%llu", interval_end, format_base10,
      ...
      else
	{
	  ...
	  vp_cli ("%.1f-%-9.1f %-13U %-10U %+9Ub/s %+9.3fms %llu/%llu", interval_start,
      ...
      if (print_header)
	{
	  vp_cli (ECHO_SEPARATOR);
	  if (cfg->report_interval_total)
	    vp_cli ("Run time (s)  Transmitted   Received   Throughput   Roundtrip");
	  else
	...
	vp_cli ("%.1f-%-9.1f %-13U %-10U %+9Ub/s %+7.3fms", interval_start, interval_end,
...
void
vp_print_final_stats (vlib_main_t *vm, f64 total_delta, vp_test_cfg_t *cfg, vp_stats_t *stats,
		      vp_test_worker_t *wrks)
{
  u64 total_bytes;
  f64 dgram_loss;
  ...
  if ((cfg->proto == VP_PROTO_TCP || (cfg->proto == VP_PROTO_UDP && cfg->echo_bytes)) &&
      !cfg->is_server)
    {
      /* display rtt stats in milliseconds */
      if (stats->rtt_stats.n_sum == 1)
	vp_cli ("%.05fms roundtrip", stats->rtt_stats.min_rtt * 1000);
      else if (stats->rtt_stats.n_sum > 1)
	vp_cli ("%.05fms/%.05fms/%.05fms min/avg/max roundtrip", stats->rtt_stats.min_rtt * 1000,
  ...
  if (cfg->proto == VP_PROTO_UDP || cfg->proto == VP_PROTO_HTTP_CONNECT_UDP)
      ...
      if (cfg->echo_bytes)
	{
	  if (cfg->is_server)
	    {
	      vp_cli ("received total %llu datagrams, sent total %llu datagrams",
      ...
      else
	{
	  if (cfg->is_server)
	    {
	      vp_cli ("received total %llu datagrams", stats->rx_total_dgrams);
	    }
	  else
	    {
	      dgram_loss = (stats->tx_total_dgrams ?
			      ((f64) (stats->tx_total_dgrams - stats->peer_dgrams_received) /
			       (f64) stats->tx_total_dgrams * 100.0) :
			      0.0);
	      vp_cli ("sent total %llu datagrams, lost %llu datagrams (%.2f%%)",
  ...
  vp_cli ("%lld bytes (%lld mbytes, %lld gbytes) in %.2f seconds", total_bytes,
	  total_bytes / (1ULL << 20), total_bytes / (1ULL << 30), total_delta);
  vp_cli ("%u bytes/second %s", flt_round_nearest (((f64) total_bytes) / (total_delta)),
	  transfer_type);
  vp_cli ("%UB/s %s", format_base10, flt_round_nearest (((f64) total_bytes) / (total_delta)),
	  transfer_type);
}
