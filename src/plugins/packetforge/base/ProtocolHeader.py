...
class ProtocolHeader:
    ...
    def Adjust(self):
        ...
        for f1 in autoIncreases:
            for f2 in increaseHeaders:
                f1.UpdateValue(ExpressionConverter.IncreaseValue(f1.Value, f2.Size >> 3), True)


    def resolveAllSize(self):
        for phf in self.fields:
            if (phf.Field.Optional != None and not self.resolveOptional(phf.Field.Optional)):
                size = 0
            else:
    ...
    def append(self, big, phf):
        ...
        if (phf.Field.Format == InputFormat.u8 or
            phf.Field.Format == InputFormat.u16 or
            phf.Field.Format == InputFormat.u32):
