...
enable_current_event (perfmon_main_t * pm)
{
  ...
  if (p->cap_user_rdpmc == 0)
    {
      clib_warning ("cap_user_rdpmc == 0");
      munmap (p, pm->page_size);
      close (fd);
      return;
    }

  if (p->index == 0)
    clib_warning ("p->index is ZERO");

  pm->rdpmc_index = p->index - 1;
  pm->perf_event_page = (void *) p;
  ...
  pm->vlib_main->perf_counter_id = pm->rdpmc_index;
}
...
disable_event (perfmon_main_t * pm)
{
  ...
  pm->vlib_main->perf_counter_id = ~0;

  if (ioctl (pm->pm_fd, PERF_EVENT_IOC_DISABLE, 0) < 0)
    clib_unix_warning ("disable ioctl");

  if (munmap (pm->perf_event_page, pm->page_size) < 0)
...
scrape_and_clear_counters (perfmon_main_t * pm)
{
  ...
  for (j = 0; j < vec_len (vlib_mains); j++)
      ...
      for (i = 0; i < vec_len (nm->nodes); i++)
	{
      ...
      /* Clear the per-node perfmon counters */
      for (i = 0; i < vec_len (nm->nodes); i++)
	{
	  n = nm->nodes[i];
  ...
  for (j = 0; j < vec_len (vlib_mains); j++)
      ...
      for (i = 0; i < vec_len (nodes); i++)
	{
	  ...
	  vectors_this_counter = n->stats_total.vectors -
