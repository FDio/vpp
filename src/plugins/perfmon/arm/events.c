...
arm_bundle_supported (perfmon_bundle_t *b)
{
  ...
  {
    u32 k;
    clib_bitmap_foreach(k, b->event_disabled) // iterate through set bits
    {
      for(u32 j = 0; j < b->n_columns; j++)
      {
        if (clib_bitmap_get(b->column_disabled, j))
          continue;
        if(b->column_events[j] & (1 << k))
        {
          clib_bitmap_set (b->column_disabled, j, 1);
          log_debug ("bundle \'%s\': disabling column %d as event not supported", b->name, j);
        }
      }
    }
  }
