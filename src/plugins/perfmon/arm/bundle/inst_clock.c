...
bundle_init_fn (vlib_main_t *vm, perfmon_bundle_t *b)
{
   ...
   *                  col  events                                 */
  set_column_events (b, 2, CPU_CYCLES, VA_END);
  set_column_events (b, 3, INST_RETIRED, VA_END);
  set_column_events (b, 5, CPU_CYCLES, VA_END);
  set_column_events (b, 6, INST_RETIRED, VA_END);
  set_column_events (b, 7, CPU_CYCLES, INST_RETIRED, VA_END);
  return 0;
}
