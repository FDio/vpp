...
bundle_init_fn (vlib_main_t *vm, perfmon_bundle_t *b)
{
   ...
   *                  col  events                                 */
  set_column_events (b, 0, L1I_TLB, VA_END);
  set_column_events (b, 1, L1I_TLB_REFILL, VA_END);
  set_column_events (b, 2, L1I_TLB, L1I_TLB_REFILL, VA_END);
  set_column_events (b, 3, L2I_TLB, VA_END);
  set_column_events (b, 4, L2I_TLB_REFILL, VA_END);
  set_column_events (b, 5, L2I_TLB, L2I_TLB_REFILL, VA_END);
  return 0;
}
