...
bundle_init_fn (vlib_main_t *vm, perfmon_bundle_t *b)
{
   ...
   *                  col  events                                 */
  set_column_events (b, 0, STALL_BACKEND, VA_END);
  set_column_events (b, 1, STALL_FRONTEND, VA_END);
  set_column_events (b, 2, STALL_BACKEND, VA_END);
  set_column_events (b, 3, STALL_FRONTEND, VA_END);
  return 0;
}
