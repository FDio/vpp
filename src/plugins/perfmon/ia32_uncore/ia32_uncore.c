...
#define foreach_ia32_uncore_unit_type \
  _(IMC, "imc", "integrated Memory Controller (iMC)") \
  _(UPI, "upi", "Intel Ultra Path Interconnect (UPI)") \

typedef enum
{
#define _(t,n,fmt) PERFMON_IA32_UNCORE_UNIT_##t,
...
intel_ia32_uncore_add_unit (perfmon_source_t * src, ia32_uncore_unit_type_t u,
			    char *name, char *type_str, int *socket_by_cpu_id)
{
  ...
  while (1)
	...
	break;
	  ...
	  in->type = perf_type;
	  in->cpu = j;
	  in->name = (char *) format (0, "%u/%u", socket_by_cpu_id[j], i);
...
#define _(t,n,fmt) \
  intel_ia32_uncore_add_unit (src, PERFMON_IA32_UNCORE_UNIT_##t, n, fmt,\
			      numa_by_cpu_id);
  foreach_ia32_uncore_unit_type;
#undef _
...
PERFMON_REGISTER_BUNDLE (intel_ia32_uncore_imc_bw) = {
  ...
  .instance_type = PERFMON_IA32_UNCORE_UNIT_IMC,
