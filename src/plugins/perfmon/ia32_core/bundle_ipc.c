...
format_ipc_node (u8 * s, va_list * args)
{
  ...
  s = format (s, "%10.2f", (f64) nc->value[0] / nc->value[1]);
  s = format (s, "%10.2f", (f64) nc->value[1] / nc->n_packets);
  s = format (s, "%10.2f", (f64) nc->value[0] / nc->n_packets);
  return s;
}

PERFMON_REGISTER_BUNDLE (ipc) = {
  .name = "ipc",
  ...
  .n_events = 3,
  .format_fn = format_ipc_node,
  .column_headers = PERFMON_STRINGS ("pkt/call", "IPC", "clk/pkt", "inst/pkt"),
};
