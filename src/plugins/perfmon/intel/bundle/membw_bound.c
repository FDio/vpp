...
static f64
calculate_membw_bound (perfmon_stats_t *ts, int idx)
{
  if (!ts->n_packets)
    return 0;

  return (u64) ts->value[idx] / ts->n_packets;
}

static u64
update_membw_bound (perfmon_stats_t *ts, int idx)
{
  return (f64) calculate_membw_bound (ts, idx);
}
...
format_intel_membw_bound (u8 *s, va_list *args)
{
  ...
  f64 sv = calculate_membw_bound (ss, row);
