...
static const char *procfs_pci_dev_path = "/proc/bus/pci";
...
get_sad_ctrl_cfg (vlib_pci_addr_t *addr)
{
  int fd = 0;
  u32 value;
  char *base_path = procfs_pci_dev_path;
  u8 *dev_node_name = format (0, "%s/%02x/%02x.%x", base_path,
  ...
  pread (fd, &value, sizeof (u32), SNR_ICX_SAD_CONTROL_CFG_OFFSET);
  close(fd);
...
static u64
get_bus_to_sad_mappings (vlib_main_t *vm, index_t **ph,
			 iio_uncore_sad_t **pp)
{
  index_t *h = 0;
...
format_stack_socket (u8 *s, va_list *va)
{
  ...
  realpath ((char *) link_path, (char *) abs_path);

  unformat_init_string (&input, (char *) abs_path,
			clib_strnlen ((char *) abs_path, PATH_MAX));

  if (!unformat (&input, SYSFS_DEVICES_PCI "%x:%x/%s",
                 &domain, &bus, link_path))
    return s;
  ...
  uword *pu = hash_get (h, bus);
  e = pool_elt_at_index(p, (index_t) pu[0]);
  if(!e) return s;

  u8 *s1 = format (0, "IIO%u/%u", e->socket_id, e->iio_unit_id);
...
init_intel_uncore_iio_bw (vlib_main_t *vm, struct perfmon_bundle *b)
{
  ...
  s = format (0, "%-10s%-5s%-13s%-12s%-14s%-16s%-32s\n", "Stack", "Port",
  ...
  vec_foreach (addr, addrs)
	...
	continue;
      ...
      /*
       *TODO: investigate product_name is a vector or a string. 
       */

      u8 *s1 = format (
	0, "%-10U%-5U%-13U%04x:%04x   %-14U%-16s%-32s", format_stack_socket, p,
...
/*
 * This bundle is currently only tested on Intel Icelake. 
 */
static int
is_icelake ()
{
