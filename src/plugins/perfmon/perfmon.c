    ...
    u8 model, u8 stepping)
{
  ...
  vec_foreach (rt, pm->perfmon_tables)
    {
      if (perfmon_cpu_model_matches (rt->models, rt->n_models,
            model, stepping))
          return rt->events;
    }
  return 0;
...
static uword  __attribute__ ((unused))
atox (u8 * s)
{
  uword rv = 0;
...
unformat_processor_event (unformat_input_t * input, va_list * args)
{
  ...
  idx = (u32) (hp->value[0]);
	...
	pe_config |= pm->perfmon_table[idx].event_code[0]; // TODO what to do with multi code events?
