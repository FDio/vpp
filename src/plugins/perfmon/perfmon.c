...
perfmon_init (vlib_main_t * vm)
{
  ...
  vec_add2 (pm->perfmon_tables, pmt, 1);
  pmt->cpuid = 0x0306C3;
  pmt->table = perfmon_haswell_table;
  vec_add2 (pm->perfmon_tables, pmt, 1);
  pmt->cpuid = 0x0406E3;
  pmt->table = perfmon_skylake_table;
  vec_add2 (pm->perfmon_tables, pmt, 1);
  pmt->cpuid = 0x0506E3;
  pmt->table = perfmon_skylake_table;

  for (i = 0; i < vec_len (pm->perfmon_tables); i++)
    {
      if (pm->perfmon_tables[i].cpuid == cpuid)
	{
	  pm->perfmon_table = perfmon_parse_table
	    (pm->perfmon_tables[i].table);
	  if (pm->perfmon_table == NULL)
	    clib_warning ("cpu-specific table failed to parse...");
	  break;
	}
    }

  if (pm->perfmon_table == 0)
    clib_warning ("no perfmon table found for cpuid 0x%x", cpuid);
