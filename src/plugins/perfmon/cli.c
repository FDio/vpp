...
#undef _
  ...
  /* clear the active type as the user has something in mind */
  b->active_type = 0;
			  ...
			  vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
      ...
      if (unformat (line_input, "bundle %U", unformat_perfmon_bundle_name, &b))
	{
	  ...
	  b->active_type = sizeof (u64) * 8;
	  b->active_type -= count_leading_zeros (b->type_flags) + 1;
	}
