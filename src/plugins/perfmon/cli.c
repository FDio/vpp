...
{
  perfmon_bundle_t *b = perfmon_main.active_bundle;
...
static u8 *
format_perfmon_reading_generic (u8 *s, va_list * args)
{
  perfmon_main_t *pm = &perfmon_main;
  perfmon_reading_t *r = va_arg (*args, perfmon_reading_t *);
  s = format (s, "%12.3f", (f64) r->time_running * 1e-9);
  for (int j = 0; j < pm->active_bundle->n_events; j++)
    s = format (s, "%12lu", r->value[j]);
  return s;
}
...
static clib_error_t *
show_perfmon_stats_command_fn (vlib_main_t * vm,
				 unformat_input_t * input,
				 vlib_cli_command_t * cmd)
{
  perfmon_main_t *pm = &perfmon_main;
  ...
  for (int i = 0; i < n_instances; i++)
	...
	{
	  r = vec_elt_at_index (readings, i);
	  s = format (s, "%U", b->format_reading ? b->format_reading :
			format_perfmon_reading_generic, r);
	}

      if (vec_len (s))
        vlib_cli_output (vm, "%-28s: %v\n", in->name, s);
