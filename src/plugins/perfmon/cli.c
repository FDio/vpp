				...
				vlib_cli_command_t * cmd)
{
  ...
  if (vb == 0)
    {
      char *key;
#define _ vec_add (vb, &b, 1);
      hash_foreach_mem (key, b, pm->bundle_by_name, _);
#undef _
    }
  else
    verbose = 1;
...
format_perfmon_source (u8 * s, va_list * args)
{
  ...
  if (src == 0)
    return format (s, "%-20s%-9s %s",
		   "Name", "NumEvents", "Description");
				 ...
				 vlib_cli_command_t * cmd)
{
  ...
  if (vs == 0)
    {
      char *key;
#define _ vec_add (vs, &s, 1);
      hash_foreach_mem (key, s, pm->source_by_name, _);
#undef _
    }
  else
    verbose = 1;
  ...
  for (int i = 0; i < n_instances; i++)
      ...
      if (b->type == PERFMON_BUNDLE_TYPE_NODE)
	{
	  ...
	  for (int j = 0; j < tr->n_nodes; j++)
		...
		table_format_cell (t, ++col, -1, "%U",
				   format_vlib_node_name, vm, j);
