  ...
  if (b->column_headers)
    {
      ...
      while (hdr[0])
	table_format_cell (t, -1, n_col++, "%s", hdr++[0]);
    }
  else
    {
      n_col = b->n_events + 1;
      table_format_cell (t, -1, 0, "RunTime");
      for (int i = 1; i < n_col; i++)
        table_format_cell (t, -1, i, "[%u]", i);
    }
  ...
  for (int i = 0; i < n_instances; i++)
    {
      in = vec_elt_at_index (it->instances, i);
      r = vec_elt_at_index (readings, i);
      table_format_cell (t, i, -1, "%s", in->name);
      if (b->format_fn == 0)
	{
           table_format_cell (t, i, 0, "%.3f", (f64) r->time_running * 1e-9);
	  for (int j = 1; j < n_col; j++)
       table_format_cell(t, i, j, "%lu", r, j);
	}
    else
