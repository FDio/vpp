...
static arping_main_t arping_main;
	  ...
	  if (arp0->opcode == clib_host_to_net_u16 (ETHERNET_ARP_OPCODE_reply))
	    {
	      ...
	      if (aif0->ip_address.ip.ip4.as_u32 ==
		  arp0->ip4_over_ethernet[0].ip4.as_u32)
		{
		  aif0->recv_from4.ip4.as_u32 =
		    arp0->ip4_over_ethernet[0].ip4.as_u32;
		  clib_memcpy (&aif0->recv_from4.mac,
	  ...
	  if (arp1->opcode == clib_host_to_net_u16 (ETHERNET_ARP_OPCODE_reply))
	    {
	      ...
	      if (aif1->ip_address.ip.ip4.as_u32 ==
		  arp1->ip4_over_ethernet[0].ip4.as_u32)
		{
		  aif1->recv_from4.ip4.as_u32 =
		    arp1->ip4_over_ethernet[0].ip4.as_u32;
		  clib_memcpy (&aif1->recv_from4.mac,
			       &arp0->ip4_over_ethernet[0].mac, 6);
		  aif1->reply_count++;
		}
	    }
	  ...
	  if (PREDICT_FALSE ((b0->flags & VLIB_BUFFER_IS_TRACED)))
	    {
	      ...
	      clib_memcpy (&t->reply.mac, &arp0->ip4_over_ethernet[0].mac, 6);
	    }
	  if (PREDICT_FALSE ((b1->flags & VLIB_BUFFER_IS_TRACED)))
	    {
	      ...
	      clib_memcpy (&t->reply.mac, &arp1->ip4_over_ethernet[0].mac, 6);
	    }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 2 && n_left_to_next >= 2)
	{
	  ...
	  if (sol_adv0->icmp.type == ICMP6_neighbor_advertisement)
	    {
	      aif0 = am->interfaces[sw_if_index0];
	      if (clib_memcmp (&aif0->ip_address.ip.ip6,
			       &sol_adv0->target_address,
			       sizeof (aif0->ip_address.ip.ip6)) == 0)
		{
		  clib_memcpy (&aif0->recv_from6.ip6,
			       &sol_adv0->target_address,
			       sizeof (aif0->recv_from6.ip6));
		  clib_memcpy (&aif0->recv_from6.mac,
	  ...
	  if (sol_adv1->icmp.type == ICMP6_neighbor_advertisement)
	    {
	      aif1 = am->interfaces[sw_if_index1];
	      if (clib_memcmp (&aif1->ip_address.ip.ip6,
			       &sol_adv1->target_address,
			       sizeof (aif1->ip_address.ip.ip6)) == 0)
		{
		  clib_memcpy (&aif1->recv_from6.ip6,
			       &sol_adv1->target_address,
			       sizeof (aif1->recv_from6.ip6));
		  clib_memcpy (&aif1->recv_from6.mac,
	  ...
	  if (PREDICT_FALSE ((b0->flags & VLIB_BUFFER_IS_TRACED)))
	    {
	      ...
	      clib_memcpy (&t->reply.ip6, &sol_adv0->target_address,
			   sizeof (t->reply.ip6));
	      clib_memcpy (&t->reply.mac, lladdr0->ethernet_address, 6);
	    }
	  if (PREDICT_FALSE ((b1->flags & VLIB_BUFFER_IS_TRACED)))
	    {
	      ...
	      clib_memcpy (&t->reply.ip6, &sol_adv1->target_address,
			   sizeof (t->reply.ip6));
	      clib_memcpy (&t->reply.mac, lladdr1->ethernet_address, 6);
	    }
...
static clib_error_t *
arping_neighbor_advertisement (vlib_main_t *vm, u32 sw_if_index, u8 is_ip4,
			       ip4_address_t *a4, ip6_address_t *a6,
			       u32 interval, u32 repeat)
{
  ...
  while (repeat > 0)
    {
      if (is_ip4)
	ip4_neighbor_advertise (vm, vnm, sw_if_index, a4);
      else
	ip6_neighbor_advertise (vm, vnm, sw_if_index, a6);
      repeat--;
      if ((interval > 0.0) && (repeat > 0))
...
static clib_error_t *
arping_neighbor_probe_dst (vlib_main_t *vm, u32 sw_if_index, u8 is_ip4,
			   ip4_address_t *a4, ip6_address_t *a6, u32 interval,
			   u32 repeat)
{
  arping_main_t *am = &arping_main;
  arping_intf_t aif_storage;
  arping_intf_t *aif = &aif_storage;
  u32 send_count = 0;
  clib_error_t *error;

  /* Disallow multiple sends on the same interface for now. Who needs it? */
  if (am->interfaces && (am->interfaces[sw_if_index] != 0))
    {
  ...
  vec_validate (am->interfaces, sw_if_index - 1);
  clib_memset (aif, 0, sizeof (*aif));
  aif->interval = interval;
  aif->repeat = repeat;
  aif->reply_count = 0;
  am->interfaces[sw_if_index] = aif;

  if (is_ip4)
    {
      aif->ip_address.ip.ip4 = *a4;
      aif->ip_address.version = AF_IP4;
      vnet_feature_enable_disable ("arp", "arping-input", sw_if_index, 1, 0,
				   0);
    }
  else
    {
      aif->ip_address.ip.ip6 = *a6;
      aif->ip_address.version = AF_IP6;
      vnet_feature_enable_disable ("ip6-local", "arping6-input", sw_if_index,
				   1, 0, 0);
    }
  ...
  while (repeat > 0)
      ...
      if (is_ip4)
	{
	  vlib_cli_output (vm, "Sending %u ARP Request to %U", send_count,
			   format_ip4_address, a4);
	  ip4_neighbor_probe_dst (sw_if_index, a4);
	}
      else
	{
	  vlib_cli_output (vm, "Sending %u ARP Request to %U", send_count,
			   format_ip6_address, a6);
	  ip6_neighbor_probe_dst (sw_if_index, a6);
	}
      repeat--;
      if ((interval > 0.0) && (repeat > 0))
	vlib_process_suspend (vm, interval);
  ...
  /* wait for a second on the ARP reply */
  if (aif->reply_count <= send_count)
    vlib_process_suspend (vm, 1.0);

  if (is_ip4)
    vnet_feature_enable_disable ("arp", "arping-input", sw_if_index, 0, 0, 0);
  else
    vnet_feature_enable_disable ("ip6-local", "arping6-input", sw_if_index, 0,
				 0, 0);
  ...
  if (aif->reply_count)
	...
	vlib_cli_output (vm, "Received %u ARP Replies from %U (%U)",
			 aif->reply_count, format_mac_address,
			 &aif->recv_from4.mac, format_ip4_address,
			 &aif->recv_from4.ip4);
      else
	vlib_cli_output (
	  vm, "Received %u ICMP6 neighbor advertisements from %U (%U)",
	  aif->reply_count, format_mac_address, &aif->recv_from6.mac,
	  format_ip6_address, &aif->recv_from6.ip6);
    }
  else
    vlib_cli_output (vm, "Received 0 Reply");

  am->interfaces[sw_if_index] = 0;
		   ...
		   vlib_cli_command_t *cmd)
{
  ...
  u32 repeat = ARPING_DEFAULT_REPEAT;
  u8 is_ip4 = 0, garp = 0;
  vnet_main_t *vnm = vnet_get_main ();
  ...
  if (garp)
    {
      error = arping_neighbor_advertisement (vm, sw_if_index, is_ip4, &a4, &a6,
					     interval, repeat);
      goto done;
    }

  error = arping_neighbor_probe_dst (vm, sw_if_index, is_ip4, &a4, &a6,
				     interval, repeat);
