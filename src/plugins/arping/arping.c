      ...
      while (n_left_from >= 2 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_TRUE (arp0->opcode ==
			    clib_host_to_net_u16 (ETHERNET_ARP_OPCODE_reply)))
	    {
	      ...
	      if (PREDICT_TRUE (aif0->ip_address.ip.ip4.as_u32 ==
				arp0->ip4_over_ethernet[0].ip4.as_u32))
		{
		  aif0->recv_from4.ip4.as_u32 =
		    arp0->ip4_over_ethernet[0].ip4.as_u32;
		  clib_memcpy_fast (&aif0->recv_from4.mac,
	  ...
	  if (PREDICT_TRUE (arp1->opcode ==
			    clib_host_to_net_u16 (ETHERNET_ARP_OPCODE_reply)))
	    {
	      ...
	      if (PREDICT_TRUE (aif1->ip_address.ip.ip4.as_u32 ==
				arp1->ip4_over_ethernet[0].ip4.as_u32))
		{
		  aif1->recv_from4.ip4.as_u32 =
		    arp1->ip4_over_ethernet[0].ip4.as_u32;
		  clib_memcpy_fast (&aif1->recv_from4.mac,
				    &arp0->ip4_over_ethernet[0].mac, 6);
		  aif1->reply_count++;
		}
	    }
      ...
      while (n_left_from >= 2 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_TRUE (sol_adv0->icmp.type ==
			    ICMP6_neighbor_advertisement))
	    {
	      aif0 = am->interfaces[sw_if_index0];
	      if (PREDICT_TRUE (
		    clib_memcmp (&aif0->ip_address.ip.ip6,
				 &sol_adv0->target_address,
				 sizeof (aif0->ip_address.ip.ip6)) == 0))
		{
		  clib_memcpy_fast (&aif0->recv_from6.ip6,
				    &sol_adv0->target_address,
				    sizeof (aif0->recv_from6.ip6));
		  clib_memcpy_fast (&aif0->recv_from6.mac,
	  ...
	  if (PREDICT_TRUE (sol_adv1->icmp.type ==
			    ICMP6_neighbor_advertisement))
	    {
	      aif1 = am->interfaces[sw_if_index1];
	      if (PREDICT_TRUE (
		    clib_memcmp (&aif1->ip_address.ip.ip6,
				 &sol_adv1->target_address,
				 sizeof (aif1->ip_address.ip.ip6)) == 0))
		{
		  clib_memcpy_fast (&aif1->recv_from6.ip6,
				    &sol_adv1->target_address,
				    sizeof (aif1->recv_from6.ip6));
		  clib_memcpy_fast (&aif1->recv_from6.mac,
			   ...
			   u32 repeat)
{
  ...
  if (is_ip4)
    arping_vnet_feature_enable_disable (vm, "arp", "arping-input", sw_if_index,
					0, 0, 0);
  else
    arping_vnet_feature_enable_disable (vm, "ip6-local", "arping6-input",
					sw_if_index, 0, 0, 0);
