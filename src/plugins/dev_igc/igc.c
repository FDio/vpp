...
#define vnet_dev_foreach(e, p)                                                \
  ...
  vnet_dev_foreach (rxq, port->rx_queues)
      ...
      for (u16 j = 0; j < iq->tail; j++)
	{
      ...
      igc_reg_wr (id, IGC_REG_RDLEN (q), rxq->n_desc * sizeof (igc_rx_desc_t));
...
igc_rx_queue_alloc (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  ...
  iq->buffer_indices = clib_mem_alloc_aligned (
    rxq->n_desc * sizeof (iq->buffer_indices[0]), CLIB_CACHE_LINE_BYTES);
  clib_memset_u32 (iq->buffer_indices, 0, rxq->n_desc);

  rv = vnet_dev_dma_mem_alloc (vm, dev, sizeof (igc_rx_desc_t) * rxq->n_desc,
			       0, (void **) &iq->descs);
  return rv;
}
...
igc_init (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  vnet_dev_port_add_args_t port = {
    ...
    .rx_queue_data_sz = sizeof (igc_rxq_t),
    .tx_queue_data_sz = sizeof (igc_txq_t),
    .max_frame_size = 9728,
    .ops = {
      .rx_node_fn = &igc_rx_node_fn,
      .tx_node_fn = &igc_tx_node_fn,
      .init = igc_port_init,
