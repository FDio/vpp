...
static clib_error_t *
sch_qfq_enable_command_fn (vlib_main_t *vm, unformat_input_t *input,
			   vlib_cli_command_t *cmd)
{
  ...
  vnet_feature_enable_disable ("interface-output", "sch_qfq_enqueue_node",
			       interface_index, 1, 0, 0);
  vnet_feature_enable_disable ("interface-output", "sch_qfq_dequeue_node",
			       interface_index, 1, 0, 0);
  return (NULL);
}
...
static clib_error_t *
sch_qfq_init (vlib_main_t *vm)
{

  sch_qfq_buffer_t *qfq_b;
  vec_validate_aligned (qfq_buffer, clib_max (1, vlib_num_workers ()),
  ...
  vec_foreach (qfq_b, qfq_buffer)
      ...
      for (int i = 0; i < MAX_GROUP; i++)
	{
	  ...
	  clib_bitmap_vec_validate (qfq_b->group_map[i].bucket_bitmap,
				    MAX_BUCKET);
	}
