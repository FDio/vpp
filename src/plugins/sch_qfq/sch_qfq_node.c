...
static u8 *
format_sch_qfq_enqueue_trace (u8 *s, va_list *args)
{
...
static u8 *
format_sch_qfq_dequeue_trace (u8 *s, va_list *args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
...
int
compute_group_state (vlib_main_t *vm, sch_qfq_buffer_t *qfq_b, int g)
{
  int s =
    (qfq_b->group_map[g].approx_start_time <= qfq_b->virtual_time) ? ER : IR;
  int n = clib_bitmap_next_set (qfq_b->state_bitmaps[ER], g + 1);
  s += (n != ~0 && qfq_b->group_map[n].approx_finish_time <
		     qfq_b->group_map[g].approx_finish_time) ?
	       1 :
	       0;
  return s;
}
...
void
insert_bucket (vlib_main_t *vm, sch_qfq_buffer_t *qfq_b, int g, int class)
{

  if (qfq_b->flow_map[class].next_flow == NULL ||
      qfq_b->flow_map[class].next_flow == &qfq_b->flow_map[class])
    clib_bitmap_set (qfq_b->group_map[g].bucket_bitmap,
		     qfq_b->flow_map[class].bucket, 0);

  int s = ((qfq_b->flow_map[class].start_time &
	    ~(qfq_b->group_map[g].slot_size - 1)) -
	   qfq_b->group_map[g].approx_start_time) >>
	  g;
  s = s % MAX_BUCKET;
...
void
move_groups (clib_bitmap_t *mask, clib_bitmap_t *src, clib_bitmap_t *dest)
{
...
void
make_eligible (vlib_main_t *vm, sch_qfq_buffer_t *qfq_b,
	       unsigned long long old_time)
{
  ...
  *qfq_b->tool_bitmap -= 1;
  move_groups (qfq_b->tool_bitmap, qfq_b->state_bitmaps[IR],
	       qfq_b->state_bitmaps[ER]);
  move_groups (qfq_b->tool_bitmap, qfq_b->state_bitmaps[IB],
	       qfq_b->state_bitmaps[EB]);
}

void
unblock_groups (vlib_main_t *vm, sch_qfq_buffer_t *qfq_b, int group_i,
		unsigned long long old_finish_time)
{
  ...
  if (x == ~0 || qfq_b->group_map[x].approx_finish_time > old_finish_time)
    {
      ...
      *qfq_b->tool_bitmap -= 1;
      move_groups (qfq_b->tool_bitmap, qfq_b->state_bitmaps[EB],
		   qfq_b->state_bitmaps[ER]);
      move_groups (qfq_b->tool_bitmap, qfq_b->state_bitmaps[IB],
		   qfq_b->state_bitmaps[IR]);
    }
}

static void
enqueue_pkt (vlib_main_t *vm, sch_qfq_buffer_t *qfq_b, vlib_buffer_t *b,
	     u32 *from, u8 class, u32 pkt_index, bool force_insert)
{
  // append the pkt to its flow buffer, if the flow is already active, the work
  // is done
  ...
  qfq_b->flow_map[class].start_time =
    max (qfq_b->flow_map[class].finish_time, qfq_b->virtual_time);
  qfq_b->flow_map[class].finish_time =
    qfq_b->flow_map[class].start_time + b->current_length;

  if (clib_bitmap_first_set (qfq_b->group_map[g].bucket_bitmap) == ~0 ||
      qfq_b->flow_map[class].start_time <
	qfq_b->group_map[g].approx_start_time)
    {
      qfq_b->state_bitmaps[IR] =
	clib_bitmap_set (qfq_b->state_bitmaps[IR], g, 0);
      qfq_b->state_bitmaps[IB] =
	clib_bitmap_set (qfq_b->state_bitmaps[IB], g, 0);
      qfq_b->group_map[g].approx_start_time =
	qfq_b->flow_map[class].start_time &
	~(qfq_b->group_map[g].slot_size - 1);
      qfq_b->group_map[g].approx_finish_time =
	qfq_b->group_map[g].approx_start_time +
	2 * qfq_b->group_map[g].slot_size;
    }
  ...
  qfq_b->state_bitmaps[state] =
    clib_bitmap_set (qfq_b->state_bitmaps[state], g, 1);
}

static void
update_groups_flows (vlib_main_t *vm, sch_qfq_buffer_t *qfq_b, int group_i,
		     group_t *group, flow_t *flow, int pkt_i)
{
  ...
  if (clib_bitmap_is_zero (group->bucket_bitmap))
    ...
    {
      flow_t *next_flow =
	group->bucket_list[clib_bitmap_first_set (group->bucket_bitmap)];
      group->approx_start_time =
	next_flow->start_time & ~(group->slot_size - 1);
      group->approx_finish_time =
	group->approx_start_time + 2 * group->slot_size;
  ...
  if (state == IDLE || group->approx_finish_time > old_finish_time)
	...
	clib_bitmap_set (qfq_b->state_bitmaps[ER], group_i, 0);
      qfq_b->state_bitmaps[state] =
	clib_bitmap_set (qfq_b->state_bitmaps[state], group_i, 1);
  ...
  if (!clib_bitmap_is_zero (qfq_b->tool_bitmap))
    {
      if (clib_bitmap_is_zero (qfq_b->state_bitmaps[ER]))
	qfq_b->virtual_time =
	  max (qfq_b->virtual_time,
	       qfq_b->group_map[clib_bitmap_first_set (qfq_b->tool_bitmap)]
		 .approx_start_time);
...
enqueue_frame (vlib_main_t *vm, vlib_node_runtime_t *node,
	       sch_qfq_buffer_t *qfq_b, vlib_buffer_t **b, u32 *from,
	       int n_pkt)
{
	  ...
	  if (flow->pkt_queue.count > 0 && group_i != flow->group_index)
	      ...
	      else
		{
		  ...
		  if (group->bucket_list[flow->bucket] == NULL)
		    {
		      group->bucket_bitmap = clib_bitmap_set (
			group->bucket_bitmap, flow->bucket, 0);
		      unsigned long long old_finish_time =
			group->approx_finish_time;
		      int state;
		      if (clib_bitmap_is_zero (group->bucket_bitmap))
			...
			{
			  flow_t *next_flow =
			    group->bucket_list[clib_bitmap_first_set (
			      group->bucket_bitmap)];
			  group->approx_start_time =
			    next_flow->start_time & ~(group->slot_size - 1);
			  group->approx_finish_time =
		      ...
		      if (state == IDLE ||
			  group->approx_finish_time > old_finish_time)
			{
			  qfq_b->state_bitmaps[ER] = clib_bitmap_set (
			    qfq_b->state_bitmaps[ER], group_i, 0);
			  qfq_b->state_bitmaps[state] = clib_bitmap_set (
			    qfq_b->state_bitmaps[state], group_i, 1);
			  unblock_groups (vm, qfq_b, group_i, old_finish_time);
			}
		    }
		  else
...
static int
qfq_select_next_frame (vlib_main_t *vm, vlib_node_runtime_t *node,
		       sch_qfq_buffer_t *qfq_b, u32 *i_arr, u32 n_pkt)
{
  int i = 0;
  while (!clib_bitmap_is_zero (qfq_b->state_bitmaps[ER]) && i < n_pkt)
      ...
      if (flow->next_flow == NULL || flow->next_flow == flow)
	{
	  if (flow->next_flow == NULL || flow->pkt_queue.count <= 1)
	    {
	      group->bucket_bitmap =
		clib_bitmap_set (group->bucket_bitmap, flow->bucket, 0);
...
static uword
sch_qfq_enqueue_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			 vlib_frame_t *frame)
{
...
static uword
sch_qfq_dequeue_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			 vlib_frame_t *frame)
{
