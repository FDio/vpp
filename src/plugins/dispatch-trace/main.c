	  ...
	  if (PREDICT_FALSE (dtm->filter_enable))
	    {
		  ...
		  pp->current_filter_function (b,
					       ~0, /* classify_table_index */
					       0 /* full classify */) != 1)
		continue;
	    }
  ...
  if (a->status)
      ...
      if (dtm->enable)
	{
	  ...
	  if (dtm->filter_enable)
	    vlib_cli_output (vm, "BPF filter enabled");
 ...
 *
 * - <b>filter</b> - Enables BPF packet filtering. The filter must be
 *   configured first using '<em>set bpf trace filter</em>' and
 *   '<em>set pcap filter function bpf_trace_filter</em>' commands.
 *   If not specified, all packets will be captured.
 *
 * @cliexpar
