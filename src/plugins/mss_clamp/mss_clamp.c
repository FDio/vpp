			...
			vlib_cli_command_t *cmd)
{
  ...
  if (is_enable)
	...
	return clib_error_return (
	  0, "disable and tx/rx cannot be specified together");
    }
...
VLIB_CLI_COMMAND (mssc_ip4_enable_disable_command, static) = {
  ...
  .short_help = "set interface ip tcp-mss-clamp [enable|disable|rx|tx] "
		"<interface-name> mss <size>",
...
VLIB_CLI_COMMAND (mssc_ip6_enable_disable_command, static) = {
  ...
  .short_help = "set interface ip6 tcp-mss-clamp [enable|disable|rx|tx] "
		"<interface-name> mss <size>",
  .function = mssc_enable_command_fn,
  .function_arg = 1, // IPv6
};
  ...
  if (sw_if_index == ~0)
      ...
      vec_foreach_index (ii, dir_enabled)
	{
	  if (MSS_CLAMP_DIR_NONE != dir_enabled[ii])
	    {
	      u32 mss = max_mss[ii];
	      vlib_cli_output (vm, "%U: %d%s", format_vnet_sw_if_index_name,
	...
	{
	  vlib_cli_output (vm, "%U: disabled", format_vnet_sw_if_index_name,
			   vnet_get_main (), sw_if_index);
	}
    }
