...
carries the session index — the slot is multiplexed in time and has no
other meaning to CNAT before lookup.
...
* ``cnat-writeback-ip4`` (``cnat_node_lookup.c``) — inserts the reverse
  bihash entry. Honours the staged ``ts->ts_scope_id[VLIB_TX]``; for plain
  VIP/DNAT translations (no SNAT policy) the staged value is zero, and the
  writeback mirrors ``ts->ts_scope_id[VLIB_RX]`` so the return packet
  re-enters under the same scope as the forward path.
