		      ...
		      vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  f64 start_time;
  int enabled = 0, i = 0;
  int to_delete_now[100];  // indices to delete this iteration
  int to_delete_next[100]; // indices to delete in the next iteration
  int del_now_count = 0;
  int del_next_count = 0;
  ...
  while (1)
	...
	default:
      ...
      i = cnat_session_scan (vm, start_time, i, to_delete_now, to_delete_next,
  ...
  for (i = 0; i < n_buffers; i++)
    {
      ...
      // vnet_buffer2 (b)->session.state = 2;
