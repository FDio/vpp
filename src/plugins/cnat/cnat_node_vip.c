  ...
  if (!(ct->flags & CNAT_TR_FLAG_NO_RETURN_SESSION))
    {
      ...
      cnat_rsession_create (rw, vnet_buffer2 (b)->session.flow_id, CNAT_FIB_TABLE,
			    1 /* add client */, 0, 0, 0);
    }
		  ...
		  ip_address_family_t af, f64 now, u8 do_trace)
{
  ...
  *next0 = cc->cc_parent.dpoi_next_node;
  ...
  ts = cnat_timestamp_update (vnet_buffer2 (b)->session.flow_id, now);
		     ...
		     ip_address_family_t af, f64 now, u8 do_trace)
{
  ...
  if (CNAT_LOOKUP_IS_RETURN != vnet_buffer2 (b)->session.state)
    {
      ASSERT (0 == vnet_buffer2 (b)->session.flow_id);
