				  ...
				  cnat_client_t *cc)
{
  ...
  if (af == AF_IP4)
    ...
    ip6_address_copy (&rw->tuple.ip6[VLIB_TX],
		      &trk0->ct_ep[VLIB_TX].ce_ip.ip.ip6);
  ...
  if (!ip_address_is_zero (&trk0->ct_ep[VLIB_RX].ce_ip))
    {
      ...
      do_snat = 1;
      if (af == AF_IP4)
	rw->tuple.ip4[VLIB_RX].as_u32 =
	  trk0->ct_ep[VLIB_RX].ce_ip.ip.ip4.as_u32;
      else
	ip6_address_copy (&rw->tuple.ip6[VLIB_RX],
			  &trk0->ct_ep[VLIB_RX].ce_ip.ip.ip6);
  ...
  if (CNAT_LOOKUP_IS_RETURN != vnet_buffer2 (b)->session.state)
    {
      ...
      cnat_timestamp_exp (vnet_buffer2 (b)->session.generic_flow_id);
