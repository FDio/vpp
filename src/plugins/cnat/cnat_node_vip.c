  ...
  if (!(ct->flags & CNAT_TR_FLAG_NO_RETURN_SESSION))
    {
      ...
      cnat_rsession_create_client (rw, af);
      ...
      cnat_rsession_create (rw, af, CNAT_SESSION_FLAG_HAS_CLIENT, iproto,
		  ...
		  u16 *next0, ip_address_family_t af, f64 now, u8 do_trace)
{
  ...
  *next0 = cc->cc_parent.dpoi_next_node;
  ...
  ts = cnat_timestamp_update (vnet_buffer (b)->session.generic_flow_id, now);
  ...
  else if (vnet_buffer (b)->session.state == CNAT_LOOKUP_IS_RETURN)
    {
  ...
  if (PREDICT_FALSE (do_trace))
    cnat_add_trace (vm, node, b, rw, NULL /* ct: fixme */);
}
