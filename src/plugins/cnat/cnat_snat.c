...
cnat_set_snat (ip4_address_t * ip4, ip6_address_t * ip6, u32 sw_if_index)
{
  ...
  cnat_addr_resolution_cleanup (INDEX_INVALID, CNAT_RESOLV_ADDR_SNAT);
  ...
  if (cnat_main.snat_ip4.as_u32 == 0 && sw_if_index != INDEX_INVALID)
	...
	cnat_add_addr_resolution (sw_if_index, AF_IP4, CNAT_RESOLV_ADDR_SNAT);
    }
  if (cnat_main.snat_ip6.as_u64[0] == 0 && cnat_main.snat_ip6.as_u64[1] == 0
	...
	cnat_add_addr_resolution (sw_if_index, AF_IP6, CNAT_RESOLV_ADDR_SNAT);
    }
}
