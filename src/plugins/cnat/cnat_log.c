...
#define CNAT_LOG_SESSION_FMT                                                                       \
  "index %d proto %d return %d " CNAT_LOG_IP4_PORT_FMT "->" CNAT_LOG_IP4_PORT_FMT
#define CNAT_LOG_SESSION_FMT_ARGS "i4i1i1" CNAT_LOG_IP4_PORT_FMT_ARGS CNAT_LOG_IP4_PORT_FMT_ARGS
...
static clib_error_t *
cnat_log_enable_disable_cli (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	enable = 0;
      else if (unformat (input, "ip %U", unformat_ip46_address, &ip, IP46_TYPE_ANY))
	;
      else
