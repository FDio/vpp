	       ...
	       u32 sw_if_index)
{
  ...
  ip6_pfx_len -= 64;
  if ((ip4_set && ip4_pfx_len > 32) || (ip6_set && ip6_pfx_len > 64))
    return VNET_API_ERROR_INVALID_VALUE;
  ...
  cpe4 = ip4_set ? cnat_snat_policy_entry_get (AF_IP4, fwd_fib_index) : 0;
  cpe6 = ip6_set ? cnat_snat_policy_entry_get (AF_IP6, fwd_fib_index) : 0;
  ...
  if (sw_if_set || ip6_set)
      ...
      if (ip6_set)
	{
	  ...
	  cpe->snat_ip6_mask =
	    clib_host_to_net_u64 (((u64) 1 << (64 - ip6_pfx_len)) - 1);
		   ...
		   vlib_cli_command_t *cmd)
{
  ...
  cnat_set_snat (CNAT_FIB_TABLE, CNAT_FIB_TABLE, &ip4, 32, &ip6, 128,
		 sw_if_index);
