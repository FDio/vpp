...
{
  u8 *a = va_arg (*args, u8 *);
  if (unformat (input, "exclude-v4"))
    *a = CNAT_SNAT_IF_MAP_EXCLUDE_V4;
  else if (unformat (input, "exclude-v6"))
    *a = CNAT_SNAT_IF_MAP_EXCLUDE_V6;
...
format_cnat_snat_interface_map_type (u8 *s, va_list *args)
{
  ...
  switch (mtype)
    {
    case CNAT_SNAT_IF_MAP_EXCLUDE_V4:
      s = format (s, "Excluded v4");
      break;
    case CNAT_SNAT_IF_MAP_EXCLUDE_V6:
      s = format (s, "Excluded v6");
      break;
...
VLIB_CLI_COMMAND (cnat_snat_policy_add_del_if_command, static) = {
  ...
  .short_help = "set cnat snat-policy if [del]"
		"[table [exclude-v4 exclude-v6 k8s]] [interface]",
...
int
cnat_snat_policy_default (vlib_buffer_t *b, cnat_session_t *session)
{
...
cnat_set_snat_policy (cnat_snat_policy_type_t policy)
{
  ...
  switch (policy)
    {
    case CNAT_SNAT_POLICY_DEFAULT:
      cpm->snat_policy = cnat_snat_policy_default;
      break;
...
cnat_snat_policy_set_cmd_fn (vlib_main_t *vm, unformat_input_t *input,
			     vlib_cli_command_t *cmd)
{
  cnat_snat_policy_type_t policy = CNAT_SNAT_POLICY_DEFAULT;
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
...
VLIB_CLI_COMMAND (cnat_snat_policy_set_cmd, static) = {
  ...
  .short_help = "set cnat snat-policy [default|k8s]",
...
cnat_snat_init (vlib_main_t *vm)
{
  ...
  for (int i = 0; i < CNAT_N_SNAT_IF_MAP; i++)
    clib_bitmap_validate (cpm->interface_maps[i], 4096); /* TODO */
