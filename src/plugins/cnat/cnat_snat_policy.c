...
cnat_snat_policy_entry_get_default (void)
{
  cnat_snat_policy_entry_t *cpe =
    cnat_snat_policy_entry_get (AF_IP4, CNAT_FIB_TABLE);
  if (!cpe)
			 ...
			 const cnat_5tuple_t *tuple)
{
  ...
  cnat_search_snat_prefix_mkkey__ (&kv, af,
				   af == AF_IP4 ?
				     (void *) &tuple->ip[VLIB_TX].ip4 :
				     (void *) &tuple->ip[VLIB_TX].ip6);
...
cnat_snat_policy_none (cnat_5tuple_t *tuple, cnat_snat_policy_entry_t *cpe,
...
cnat_snat_policy_action_t
cnat_snat_policy_k8s (cnat_5tuple_t *tuple, cnat_snat_policy_entry_t *cpe,
		      const vlib_buffer_t *b, ip_address_family_t af)
{
  ASSERT (cpe);
  ...
  if (!clib_bitmap_get (cpe->interface_maps[CNAT_SNAT_IF_MAP_INCLUDE_POD],
			in_if) &&
      !clib_bitmap_get (cpe->interface_maps[CNAT_SNAT_IF_MAP_INCLUDE_POD],
			out_if))
    {
      if (AF_IP6 == af &&
	  ip6_address_is_equal (&tuple->ip[VLIB_RX].ip6,
				&ip_addr_v6 (&cpe->snat_ip6.ce_ip)))
	return CNAT_SNAT_POLICY_ACTION_NOOP;
      if (AF_IP4 == af &&
	  ip4_address_is_equal (&tuple->ip[VLIB_RX].ip4,
  ...
  if ((AF_IP6 == af && ip6_address_is_equal (&tuple->ip[VLIB_RX].ip6,
					     &tuple->ip[VLIB_TX].ip6)) ||
			 ...
			 cnat_snat_policy_action_t default_action)
{
  ...
  if (cnat_snat_policy_dnat_rewrite (cpe, af,
				     af == AF_IP4 ?
				       (void *) &tuple->ip[VLIB_RX].ip4 :
				       (void *) &tuple->ip[VLIB_RX].ip6,
				     1 /* is_src */))
...
cnat_snat_policy_action_t
cnat_snat_policy_dnat (cnat_5tuple_t *tuple, cnat_snat_policy_entry_t *cpe,
		       const vlib_buffer_t *b, ip_address_family_t af)
{
  return cnat_snat_policy_dnat__ (tuple, cpe, b, af,
				  CNAT_SNAT_POLICY_ACTION_SNAT_ALLOC);
}

cnat_snat_policy_action_t
cnat_snat_policy_dnat_only (cnat_5tuple_t *tuple,
			    cnat_snat_policy_entry_t *cpe,
			    const vlib_buffer_t *b, ip_address_family_t af)
{
  return cnat_snat_policy_dnat__ (tuple, cpe, b, af,
				  CNAT_SNAT_POLICY_ACTION_SNAT_KEEP);
}
  ...
  if (is_del)
    {
      ...
      cnat_client_free_by_ip (&ip_addr_46 (&ep->ce_ip), ret_fib_index,
			      0 /* is_session */);
...
static void
cnat_snat_cleanup (cnat_snat_policy_main_t *cpm, cnat_snat_policy_entry_t *cpe,
		   u32 fwd_fib_index)
{
  u32 index = cpe - cpm->snat_policies_pool;

  if (cpe->snat_ip4.ce_flags & CNAT_EP_FLAG_RESOLVED)
    cnat_client_free_by_ip (&ip_addr_46 (&cpe->snat_ip4.ce_ip),
			    cpe->ret_fib_index4, 0 /* is_session */);
  if (cpe->snat_ip6.ce_flags & CNAT_EP_FLAG_RESOLVED)
    cnat_client_free_by_ip (&ip_addr_46 (&cpe->snat_ip6.ce_ip),
	       ...
	       u8 cnat_addresses_flags)
{
  ...
  if ((ip4_set && ip4_pfx_len > 32) ||
      (ip6_set && (ip6_pfx_len < 64 || ip6_pfx_len > 128)))
    return VNET_API_ERROR_INVALID_VALUE;
  ...
  if (sw_if_set || ip4_set)
      ...
      if (ip4_set)
	{
	  ...
	  cpe->snat_ip4_mask =
	    clib_host_to_net_u32 (((u32) 1 << (32 - ip4_pfx_len)) - 1);
	...
	{
	  cnat_translation_watch_addr (index, 0, &cpe->snat_ip4,
				       CNAT_RESOLV_ADDR_SNAT);
	}
      else
	{
	  cnat_client_add_pfx (&cpe->snat_ip4.ce_ip, ip4_pfx_len,
			       ret_fib_index, ~0 /* fwd_fib_index */,
			       CNAT_TR_FLAG_EXCLUSIVE |
				 CNAT_TR_FLAG_RETURN_ONLY |
  ...
  if (sw_if_set || ip6_set)
      ...
      if (ip6_set)
	{
	  ...
	  cpe->snat_ip6_mask =
	    clib_host_to_net_u64 (((u64) 1 << (128 - ip6_pfx_len)) - 1);
	...
	{
	  cnat_translation_watch_addr (index, 0, &cpe->snat_ip6,
				       CNAT_RESOLV_ADDR_SNAT);
	}
      else
	{
	  cnat_client_add_pfx (&cpe->snat_ip6.ce_ip, ip6_pfx_len,
			       ret_fib_index, ~0 /* fwd_fib_index */,
			       CNAT_TR_FLAG_EXCLUSIVE |
				 CNAT_TR_FLAG_RETURN_ONLY |
			     cnat_addresses_flags);
		...
		vlib_cli_command_t *cmd)
{
  ...
  vlib_cli_output (vm, "Source NAT\n  ip4: %U\n  ip6: %U\n\n",
		   format_cnat_endpoint, &cpe->snat_ip4, format_cnat_endpoint,
		   &cpe->snat_ip6);
