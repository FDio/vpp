...
cnat_input_feature_get_rw (vlib_main_t *vm, vlib_buffer_t *b, ip_address_family_t af,
			   cnat_timestamp_t *ts)
{
		       ...
		       ip_address_family_t af, u8 do_trace)
{
  ...
  if (n_left >= 4)
    {
      ...
      ts[0] = cnat_timestamp_get (vnet_buffer2 (b[0])->session.scope_or_flow_id);
      ts[1] = cnat_timestamp_get (vnet_buffer2 (b[1])->session.scope_or_flow_id);
      ts[2] = cnat_timestamp_get (vnet_buffer2 (b[2])->session.scope_or_flow_id);
      ts[3] = cnat_timestamp_get (vnet_buffer2 (b[3])->session.scope_or_flow_id);
    }
  ...
  while (n_left >= 4)
      ...
      if (n_left >= 8)
	{
	  ...
	  ts[0] = cnat_timestamp_get (vnet_buffer2 (b[4])->session.scope_or_flow_id);
	  ts[1] = cnat_timestamp_get (vnet_buffer2 (b[5])->session.scope_or_flow_id);
	  ts[2] = cnat_timestamp_get (vnet_buffer2 (b[6])->session.scope_or_flow_id);
	  ts[3] = cnat_timestamp_get (vnet_buffer2 (b[7])->session.scope_or_flow_id);
...
always_inline cnat_timestamp_rewrite_t *
cnat_output_feature_get_rw (vlib_main_t *vm, vlib_buffer_t *b, ip_address_family_t af,
			    cnat_timestamp_t *ts)
{
  if (vnet_buffer2 (b)->session.state == CNAT_LOOKUP_IS_OK)
