...
format_cnat_timestamp (u8 *s, va_list *args)
{
  ...
  s = format (s, "%Ulast_seen:%u lifetime:%u ref:%u fib:%u",
	      format_white_space, indent, ts->last_seen, ts->lifetime,
	      ts->ts_session_refcnt, ts->fib_index);
...
format_cnat_session (u8 * s, va_list * args)
{
  ...
  s = format (s, "%U => [%U]\n%Uindex:%d\n%U%U", format_cnat_5tuple,
	      &sess->key.cs_5tuple, format_cnat_session_flags,
	      sess->value.cs_flags, format_white_space, indent + 2,
	      sess->value.cs_session_index, format_white_space, indent + 2,
...
cnat_session_show_cbak (BVT (clib_bihash_kv) * kvp, void *arg)
{
  ...
  if (a->refcount)
    {
      const cnat_timestamp_t *ts =
	cnat_timestamp_get (s->value.cs_session_index);
		   ...
		   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (input, "return"))
	{
      ...
      else if (unformat (input, "ip %U", unformat_ip46_address, &arg.ip,
			 IP46_TYPE_ANY))
	{
	  arg.verbose = 1;
	}
  ...
  vlib_cli_output (vm, "CNat Sessions: now:%f\n%U\n", vlib_time_now (vm),
		   BV (format_bihash), &cnat_session_db, 0 /* verbose */);
  if (arg.verbose)
...
cnat_reverse_session_key (cnat_session_t *rsession, const cnat_timestamp_t *ts,
			  const cnat_session_location_t loc,
			  const cnat_timestamp_direction_t dir,
...
cnat_reverse_session_free (cnat_session_t *session)
{
  ...
  if (cnat_reverse_session_key (rsession, ts, CNAT_LOCATION_INPUT, dir, rdir))
    ;
  else if (cnat_reverse_session_key (rsession, ts, CNAT_LOCATION_FIB, dir,
				     rdir))
    ;
  else if (cnat_reverse_session_key (rsession, ts, CNAT_LOCATION_OUTPUT, dir,
				     rdir))
    ;
  else
    {
      /* nothing found, try to just swap the 5tuple... */
      cnat_5tuple_copy (&rsession->key.cs_5tuple, &session->key.cs_5tuple,
			1 /* swap */);
  ...
  if (!i)
    {
       ...
       */
      index_t *slot;
      pool_foreach (slot, cnat_ep_trk_deleted)
	{
	  if (!pool_is_free_index (cnat_ep_trk_pool, *slot))
	    {
	      cnat_ep_trk_t *ep_trk =
		pool_elt_at_index (cnat_ep_trk_pool, *slot);
	      pool_put (cnat_ep_trk_pool, ep_trk);
	    }
	}
      pool_free (cnat_ep_trk_deleted);

      /* Promote "pending deletions" to "confirmed".
       * Using two pools (pending + confirmed) ensures we scan all sessions
       * once more before actually freeing a backend. Prevents deleting while
       * iterating.
       */
      pool_foreach (slot, cnat_ep_trk_pending_delete)
	{
	  index_t *new_slot;
	  pool_get (cnat_ep_trk_deleted, new_slot);
	  *new_slot = *slot;
	}
      pool_free (cnat_ep_trk_pending_delete);
    }
	  ...
	  for (k = 0; k < BIHASH_KVP_PER_PAGE; k++)
		...
		continue;
		  ...
		  session->key.cs_5tuple.iproto !=
		    0) // TODO: this is a work around a bug where sessions with
		       // 0 values are found
		{
		  /* age it */
...
cnat_session_init (vlib_main_t * vm)
{
  ...
  BV (clib_bihash_init)
  (&cnat_session_db, "CNat Session DB",
   session_max / BIHASH_KVP_PER_PAGE /* buckets */,
  ...
  vec_foreach_index (i, ctm->ts_pools)
    {
      ...
      vlib_cli_output (vm, "-- Pool %d [%d/%d]", i, cnt,
		       pool_max_len (ts_pool));
      if (!verbose)
	continue;
      pool_foreach (ts, ts_pool)
	{
	  vlib_cli_output (vm, "[%d] %U", ts - ts_pool, format_cnat_timestamp,
			   ts, 0);
