  ...
  if (!cnat_ts_is_free_index (sess->value.cs_session_index))
    {
      ...
      s = format (s, "%U => [%U] %U", format_cnat_5tuple, &sess->key.cs_5tuple,
		  format_cnat_session_flags, sess->value.cs_flags,
		  format_cnat_timestamp, ts);
    }
  else
    s =
      format (s, "%U => [%U] (gone)", format_cnat_5tuple, &sess->key.cs_5tuple,
	      format_cnat_session_flags, sess->value.cs_flags);
...
cnat_session_free (cnat_session_t * session)
{
  ...
  hash = cnat_5tuple_hash (&session->key.cs_5tuple);

  cnat_bihash_add_del_hash (&cnat_session_db, bkey, hash, 0 /* is_add */);
}
