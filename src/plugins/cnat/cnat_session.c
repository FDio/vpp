...
cnat_session_show_cbak (BVT (clib_bihash_kv) * kvp, void *arg)
{
  ...
  if (!ip46_address_is_zero (&a->ip) &&
      !ip46_address_cmp (&a->ip, &s->key.cs_5tuple.ip[VLIB_RX]) &&
      !ip46_address_cmp (&a->ip, &s->key.cs_5tuple.ip[VLIB_TX]))
    return BIHASH_WALK_CONTINUE;
...
{
  cnat_session_show_cbak_arg_t arg = { .vm = vm, .max = 50 };
  int v;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	arg.flags |= CNAT_SESSION_IS_RETURN;
      else if (unformat (input, "ip %U", unformat_ip46_address, IP46_TYPE_ANY,
			 &arg.ip))
	;
...
cnat_reverse_session_free (cnat_session_t *session)
{
  ...
  int is_return =
    session->value.cs_flags & CNAT_SESSION_IS_RETURN ? CNAT_IS_RETURN : 0;
