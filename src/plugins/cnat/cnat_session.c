...
static int
cnat_reverse_session_tuple (cnat_session_t *rsession,
			    const cnat_timestamp_t *ts,
...
cnat_reverse_session_free (cnat_session_t *session)
{
  ...
  /* 1st determine directions to use: we want to check the opposite direction
   * 1st, and failing that using the same direction and inverse the tuple */
  ...
  /* try to find the right rewrite: INPUT > FIB > OUTPUT and opposite
   * direction > same direction */
  if (cnat_reverse_session_tuple (rsession, ts, CNAT_LOCATION_INPUT, dir,
				  rdir))
    ;
  else if (cnat_reverse_session_tuple (rsession, ts, CNAT_LOCATION_FIB, dir,
				       rdir))
    ;
  else if (cnat_reverse_session_tuple (rsession, ts, CNAT_LOCATION_OUTPUT, dir,
				       rdir))
    ;
  else
    {
