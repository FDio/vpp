...
cnat_writeback_new_flow (vlib_buffer_t *b, ip_address_family_t af, u16 *next)
{
  ...
  cnat_make_buffer_5tuple (b, af, &session->key.cs_5tuple, iph_offset, 1 /* swap */, rewrite_from);
  ...
  u8 protocol = (af == AF_IP4) ?
		  ((ip4_header_t *) ((u8 *) vlib_buffer_get_current (b) + iph_offset))->protocol :
		  ((ip6_header_t *) ((u8 *) vlib_buffer_get_current (b) + iph_offset))->protocol;
  u16 udp_dst_port = 0;
  if (protocol == IP_PROTOCOL_UDP)
    {
      udp_dst_port =
	(af == AF_IP4) ?
	  ((udp_header_t *) (((ip4_header_t *) ((u8 *) vlib_buffer_get_current (b) + iph_offset)) +
			     1))
	    ->dst_port :
	  ((udp_header_t *) (((ip6_header_t *) ((u8 *) vlib_buffer_get_current (b) + iph_offset)) +
			     1))
	    ->dst_port;
    }

  /* Increment refcnt only if not IP-in-IP or VXLAN */
  if ((protocol != IP_PROTOCOL_IP_IN_IP) &&
      !(protocol == IP_PROTOCOL_UDP && clib_net_to_host_u16 (udp_dst_port) == CNAT_VXLAN_UDP_PORT))
   ...
   * twice */
  vnet_buffer2 (b)->session.state = CNAT_LOOKUP_IS_NONE;
