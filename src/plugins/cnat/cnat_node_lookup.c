...
always_inline void
cnat_writeback_new_flow (vlib_buffer_t *b, ip_address_family_t af)
{
  ...
  ts = cnat_timestamp_get (vnet_buffer2 (b)->session.generic_flow_id);
  clib_atomic_add_fetch (&ts->ts_session_refcnt, 1);

  cnat_bihash_add_or_overwrite_with_cb (&cnat_session_db, &bkey,
