			 ...
			 cnat_lb_type_t lb_type, flow_hash_config_t fhc)
{
  ...
  /* Mark deleted paths and collect unchanged ones */
  ...
  vec_foreach (trk_index, ct->ct_paths_indexes)
	...
	{
	  /* unchanged backend: keep it */
	  vec_add1 (new_paths_indexes, *trk_index);
	}
    }

  vec_free (ct->ct_paths_indexes);
  ct->ct_paths_indexes = new_paths_indexes;
  ct->flags &= ~CNAT_TR_FLAG_STACKED;

  /* Add only genuinely new paths */
  u64 path_idx = 0;
  vec_foreach (path, paths)
    {
      cnat_resolve_ep_tuple (path);
      cnat_translation_watch_addr (ct->ct_index, path_idx << 32 | VLIB_RX, &path->src_ep,
				   CNAT_RESOLV_ADDR_BACKEND);
      cnat_translation_watch_addr (ct->ct_index, path_idx << 32 | VLIB_TX, &path->dst_ep,
				   CNAT_RESOLV_ADDR_BACKEND);
      path_idx++;
