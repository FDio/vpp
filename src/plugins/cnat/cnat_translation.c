...
cnat_translation_delete (u32 id)
{
  ...
  cnat_addr_resolution_cleanup (id, CNAT_RESOLV_ADDR_ANY);
			 ...
			 cnat_endpoint_tuple_t * paths, u8 flags)
{
  ...
  if (INDEX_INVALID == cci)
    {
      cnat_addr_resolution_cleanup (ct->index, CNAT_RESOLV_ADDR_TRANSLATION);
      addr_resolution_t *ar;
      ar = cnat_add_addr_resolution (vip->ce_sw_if_index,
				     vip->ce_af,
				     ...
				     ip_address_t * address)
{
  ...
  ASSERT (ct->ct_vip.ce_flags && CNAT_EP_FLAG_RESOLVING);
  cnat_remove_translation_from_db (ct->ct_cci, &ct->ct_vip, ct->ct_proto);
  ct->ct_cci = cnat_client_add (address, ct->flags);

  ip_address_copy (&ct->ct_vip.ce_ip, address);
  ct->ct_vip.ce_flags &= ~CNAT_EP_FLAG_RESOLVING;
