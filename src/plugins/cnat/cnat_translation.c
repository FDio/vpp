...
cnat_ep_trk_t *cnat_ep_trk_pool = NULL;
clib_bihash_8_8_t cnat_ep_trk_db;
...
cnat_translation_stack (cnat_translation_t * ct)
{
  ...
  if (ct->fhc != 0)
    fhc = ct->fhc;
  lbi = load_balance_create (vec_len (ct->ct_active_paths_indexes),
...
void
cnat_delete_sessions_for_translation (cnat_translation_t *ct, index_t trk,
				      u8 all)
{
  ...
  vec_foreach_index (pidx, ctm->ts_pools)
      ...
      vec_foreach_index (idx, ts_pool)
	{
	  ...
	  if ((ts.cti == ct->index) && (all || ts.trk == trk))
	    {
	      session_idx = pidx << ctm->log2_pool_sz | idx;
	      cnat_session_delete (session_idx);
	      fprintf (stderr,
		       ">>>>> NOOP deleted session %u for translation %u\n",
		       session_idx, ct->index);
	    }
	}
    }
}
...
cnat_translation_delete (u32 id, u32 fib_index)
{
  ...
  fprintf (stderr, "       deleting all sessions for translation %u\n",
	   ct->index);
  cnat_delete_sessions_for_translation (ct, 0, 1 /* delete all sessions */);
			 ...
			 cnat_lb_type_t lb_type, flow_hash_config_t fhc)
{
  ...
  vec_foreach (trk_index, ct->ct_active_paths_indexes)
      ...
      if (deleted)
	{
	  fprintf (
	    stderr,
	    "       deleting trk sessions for translation %u trkindex %d\n",
	    ct->index, *trk_index);
	  cnat_delete_sessions_for_translation (ct, *trk_index, 0);
	}
    }
  ...
  vec_reset_length (ct->ct_paths_indexes);
  ct->flags &= ~CNAT_TR_FLAG_STACKED;
  ...
  vec_foreach (path, paths)
  {
    ...
    trk->ct_dpo = tmp;
    trk->index = trk - cnat_ep_trk_pool;
    fprintf (stderr, "added trk index %d for translation %d\n", trk->index,
	     ct->index);

    vec_add1 (ct->ct_paths_indexes, trk->index);
...
format_cnat_translation (u8 * s, va_list * args)
{
  ...
  vec_foreach (ck_index, ct->ct_paths_indexes)
    {
    ck = pool_elt_at_index (cnat_ep_trk_pool, *ck_index);
			      ...
			      vlib_cli_command_t * cmd)
{
  ...
  cnat_endpoint_t vip;
  u8 flags = CNAT_TR_FLAG_EXCLUSIVE;
