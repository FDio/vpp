...
void
cnat_add_translation (index_t cci, u16 port, ip_protocol_t proto, index_t cti)
{
...
void
cnat_remove_translation (index_t cci, u16 port, ip_protocol_t proto)
{
  clib_bihash_kv_8_8_t bkey;
  u64 key;
...
cnat_translation_delete (u32 id)
{
  ...
  cnat_client_remove_translation (ct->ct_cci, ct->ct_vip.ce_port,
				  ct->ct_proto);
			 ...
			 const cnat_endpoint_tuple_t * paths, u8 flags)
{
  ...
  if (NULL == ct)
    {
      ...
      cnat_client_add_translation (cci, ct->ct_vip.ce_port, ct->ct_proto,
				   cti);

      vlib_validate_combined_counter (&cnat_translation_counters, cti);
      vlib_zero_combined_counter (&cnat_translation_counters, cti);
    }
  ct->flags = flags;
  cti = ct - cnat_translation_pool;
  ...
  vec_foreach (path, paths)
  {
    ...
    cnat_tracker_track (cti, path, trk);
  }
...
format_cnat_translation (u8 * s, va_list * args)
{
  ...
  vec_foreach (ck, ct->ct_paths)
    s = format (s, "\n%U%U",
		format_white_space, indent, format_cnat_ep_trk, ck,
		indent + 2);
  ...
  if (INDEX_INVALID == cti)
    {
      ...
      ({
        vlib_cli_output(vm, "%U", format_cnat_translation, cti, 1);
      }));
      /* *INDENT-ON* */
    }
  else
