...
class VppCNatEndpointTuple(object):
...
class TestCNatTranslation(VppTestCase):
    ...
    def cnat_translation(self, ip_v, vips):
        """ CNat Translation """

        ip_class = IPv6 if ip_v == 'ip6' else IP
        ...
        for vip in vips:
            for vport in vports:
                for l4p, vl4 in zip(l4_protos, vl4_protos):
                    dep = VppCNatEndpoint(
                        ...
                        4000 + nbr)
                    sep = VppCNatEndpoint("0.0.0.0", 0)
                    t1 = VppCNatTranslation(
                        self,
                        VppCNatEndpoint(vip, vport),
                        vl4,
                        [VppCNatEndpointTuple(sep, dep),
                         VppCNatEndpointTuple(sep, dep)])
                    t1.add_vpp_config()

                    self.logger.info(self.vapi.cli(
                        "sh %s fib %s" %
                        ('ip6' if ip_v == 'ip6' else 'ip', vip)))
                    ...
                    for src in self.pg0.remote_hosts:
                        for sport in sports:
                            ...
                            p1 = (Ether(dst=self.pg0.local_mac,
                                        src=src.mac) /
                                  ip_class(src=getattr(src, ip_v), dst=vip) /
                            ...
                            for rx in rxs:
                                ...
                                self.assertEqual(rx[ip_class].src, vip)
                    ...
                    #
                    dep = VppCNatEndpoint(
                        getattr(self.pg2, 'remote_' + ip_v),
                        5000)
                    sep = VppCNatEndpoint("0.0.0.0", 0)
                    t1.modify_vpp_config([VppCNatEndpointTuple(sep, dep)])
            ...
            self.logger.info(self.vapi.cli("sh %s fib %s" % (
                'ip6' if ip_v == 'ip6' else 'ip',
                getattr(self.pg0, 'remote_' + ip_v))))
            self.logger.info(self.vapi.cli("sh cnat session"))
        ...
        n_tries = 0
        sessions = self.vapi.cnat_session_dump()
        while (len(sessions) and n_tries < 10):
            n_tries += 1
            sessions = self.vapi.cnat_session_dump()
            self.sleep(2)

        self.assertTrue(n_tries < 10)
    ...
    def test_cnat4(self):
        ...
        self.cnat_translation('ip4', vips)
