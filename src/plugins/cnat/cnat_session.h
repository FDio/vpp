...
typedef struct cnat_session_t_
{
  ...
  union
  {
    struct
    {
      ...
      /**
       * Caller-defined context identifier. Defaults to fib_index.
       * Callers may supply a custom value to disambiguate sessions
       * from different scopes (e.g. tunnels) with identical 5-tuples.
       */
      u32 context_id;
    };
    u64 as_u64[6];
...
{
  /* generic_flow_id contains a scope_id (tunnel context) rather than a session
   * index. Set by external plugins before CNAT lookup; cleared by
   * cnat_lookup_create_or_return() when it overwrites generic_flow_id. */
...
/**
 * Read the scope_id from session.generic_flow_id when HAS_SCOPE flag is set.
 * External plugins (e.g. CNHE) write scope_id into generic_flow_id and set
 * the flag before CNAT lookup. After lookup, generic_flow_id holds the
 * session index and the
 * flag is cleared by cnat_lookup_create_or_return().
 */
...
/**
 * Store a scope_id in the buffer for CNAT to use as context_id.
 * Writes generic_flow_id (24-bit) and sets HAS_SCOPE flag. Pass 0 to clear.
 * Callers must ensure scope_id fits in 24 bits.
 */
