...
typedef struct cnat_session_t_
{
  ...
  union
  {
    struct
    {
       ...
       * (matching scope_or_flow_id in the buffer opaque).
...
{
  /* scope_or_flow_id contains a scope_id rather than a session index.
...
#define cnat_scope_id(b)                                                                           \
  ((vnet_buffer2 (b)->session.flags & CNAT_BUFFER_SESSION_FLAG_HAS_SCOPE) ?                        \
     (u32) vnet_buffer2 (b)->session.scope_or_flow_id :                                            \
     CNAT_SCOPE_ID_NONE)

static_always_inline void
cnat_set_scope_id (vlib_buffer_t *b, u32 scope_id)
{
  if (scope_id != CNAT_SCOPE_ID_NONE)
    {
      vnet_buffer2 (b)->session.scope_or_flow_id = scope_id;
      vnet_buffer2 (b)->session.flags |= CNAT_BUFFER_SESSION_FLAG_HAS_SCOPE;
    }
  else
    {
