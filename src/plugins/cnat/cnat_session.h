...
typedef struct cnat_session_t_
{
  ...
  union
  {
    struct
    {
       ...
       * multi-tenant contexts). 0 means no scope; effective range
       * for an active scope is the value an external feature stores
       * in the vlib buffer's generic flow_id field before CNAT
       * lookup.
       */
      u32 cs_scope_id;
    };
 ...
 * populate cs_scope_id in the session key. A value of 0 means no scope.
 *
