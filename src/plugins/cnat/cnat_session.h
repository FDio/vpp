...
{
  /* flow_id contains a scope_id (tunnel context) rather than a session index.
   * Set by external plugins before CNAT lookup; cleared by
   * cnat_lookup_create_or_return() when it overwrites flow_id. */
...
/**
 * Read the scope_id from session.flow_id when HAS_SCOPE flag is set.
 * External plugins (e.g. CNHE) write scope_id into flow_id and set the flag
 * before CNAT lookup. After lookup, flow_id holds the session index and the
...
#define cnat_scope_id(b)                                                                           \
  ((vnet_buffer2 (b)->session.flags & CNAT_BUFFER_SESSION_FLAG_HAS_SCOPE) ?                        \
     (u32) vnet_buffer2 (b)->session.flow_id :                                                     \
     0)

/**
 * Store a scope_id in the buffer for CNAT to use as context_id.
 * Writes flow_id and sets HAS_SCOPE flag. Pass 0 to clear.
 */
...
cnat_set_scope_id (vlib_buffer_t *b, u32 scope_id)
{
  if (scope_id)
    {
      vnet_buffer2 (b)->session.flow_id = scope_id;
      vnet_buffer2 (b)->session.flags |= CNAT_BUFFER_SESSION_FLAG_HAS_SCOPE;
    }
  else
    {
