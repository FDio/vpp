...
void
cnat_client_free_by_ip (ip4_address_t *ip4, ip6_address_t *ip6, u8 af,
			u32 fib_index, int is_session)
{
  cnat_client_t *cc;
  cc = (AF_IP4 == af ? cnat_client_ip4_find (ip4, fib_index) :
			     cnat_client_ip6_find (ip6, fib_index));
  ASSERT (NULL != cc);
...
cnat_client_init (vlib_main_t * vm)
{
  ...
  cnat_client_node_ip4_cnat_tx =
    vlib_get_node_by_name (vm, (u8 *) "ip4-cnat-tx")->index;
  cnat_client_node_ip6_cnat_tx =
    vlib_get_node_by_name (vm, (u8 *) "ip6-cnat-tx")->index;
  cnat_client_node_ip4_cnat_return =
    vlib_get_node_by_name (vm, (u8 *) "ip4-cnat-return")->index;
  cnat_client_node_ip6_cnat_return =
    vlib_get_node_by_name (vm, (u8 *) "ip6-cnat-return")->index;
