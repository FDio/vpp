 ...
 */
static void
cnat_client_dpo_interpose (const dpo_id_t *original, const dpo_id_t *parent, dpo_id_t *clone)
{
...
format_cnat_client (u8 *s, va_list *args)
{
  ...
  s = format (s, "[%d] cnat-client:[%U] tr:%d sess:%d locks:%u", cci, format_ip_address, &cc->cc_ip,
	      cc->tr_refcnt, cc->session_refcnt, cc->cc_locks);
  ...
  if (cnat_client_is_clone (cc))
    s = format (s, "\n%Uclone of [%d]\n%U%U", format_white_space, indent + 2, cc->parent_cci,
		format_white_space, indent + 2, format_dpo_id, &cc->cc_parent, indent + 4);

  return (s);
}

static clib_error_t *
cnat_client_show (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  index_t cci;
...
cnat_client_init (vlib_main_t *vm)
{
  ...
  clib_bihash_init_16_8 (&cnat_client_db.cc_ip_id_hash, "CNat client DB", cm->client_hash_buckets,
