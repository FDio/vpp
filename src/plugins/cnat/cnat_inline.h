...
cnat_timestamp_alloc (u32 scope_id, bool is_v6)
{
  ...
  if (PREDICT_FALSE (scope_id < vec_len (sessions_per_vrf) &&
		     vec_elt (sessions_per_vrf, scope_id) <= 0))
    goto err;
  ...
  if (scope_id < vec_len (sessions_per_vrf))
    vec_elt (sessions_per_vrf, scope_id)--;
