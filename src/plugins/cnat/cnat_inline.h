...
always_inline void
cnat_timestamp_destroy (u32 index, u32 fib_index, bool is_v6)
{
  cnat_timestamp_mpool_t *ctm = &cnat_timestamps;
  int *sessions_per_vrf =
    is_v6 ? ctm->sessions_per_vrf_ip6 : ctm->sessions_per_vrf_ip4;
...
always_inline void
cnat_timestamp_free (u32 index, u32 fib_index, bool is_v6)
{
  cnat_timestamp_t *ts = cnat_timestamp_get (index);
  ASSERT (ts);
  if (0 == clib_atomic_sub_fetch (&ts->ts_session_refcnt, 1))
	...
	if (ts->ts_rw_bm & (1 << i))
      ...
      cnat_timestamp_destroy (index, fib_index, is_v6);
    }
}
