			      ...
			      bool is_v6, bool alloc_if_not_found)
{
  ...
  if (rv)
	...
	ksession->value.cs_session_index;
      ...
      // we put the original 5tuple in the rewrite of the sessionto use it
      ...
      // need to get rid of ts->fw_session_5tuple, but this is buggy for the
      // moment
      ts->cts_rewrites[0].tuple = ksession->key.cs_5tuple;
    }
