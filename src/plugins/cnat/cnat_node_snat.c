...
cnat_snat_feature_new_flow_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
				   ...
				   cnat_timestamp_t *ts, u32 fwd_fib_index)
{
...
cnat_snat_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
		   ...
		   f64 now, u8 do_trace, u8 is_client, u8 is_return)
{
  cnat_timestamp_rewrite_t *rw = NULL;
  ...
  if (is_client)
    {
      cnat_client_t *cc =
	cnat_client_get (vnet_buffer (b)->ip.adj_index[VLIB_TX]);
  ...
  if (vnet_buffer2 (b)->session.state == CNAT_LOOKUP_IS_OK)
    ...
    {
      rw =
	cnat_snat_feature_new_flow_inline (vm, node, b, af, ts, fwd_fib_index);
    }
  else if (is_return &&
	   vnet_buffer2 (b)->session.state == CNAT_LOOKUP_IS_RETURN)
    {
		      ...
		      f64 now, u8 do_trace)
{
  cnat_snat_node_fn (vm, node, b, next0, af, now, do_trace,
		     false /* is_client */, false /* is_return */);
}

static_always_inline void
cnat_snat_node_input_return (vlib_main_t *vm, vlib_node_runtime_t *node,
			     vlib_buffer_t *b, u16 *next0,
			     ip_address_family_t af, f64 now, u8 do_trace)
{
  cnat_snat_node_fn (vm, node, b, next0, af, now, do_trace,
		     false /* is_client */, true /* is_return */);
}

static_always_inline void
cnat_snat_node_client (vlib_main_t *vm, vlib_node_runtime_t *node,
		       vlib_buffer_t *b, u16 *next0, ip_address_family_t af,
		       f64 now, u8 do_trace)
{
  cnat_snat_node_fn (vm, node, b, next0, af, now, do_trace,
		     true /* is_client */, true /* is_return */);
}

static_always_inline uword
cnat_snat_node (vlib_main_t *vm, vlib_node_runtime_t *node,
		vlib_frame_t *frame, ip_address_family_t af,
		cnat_node_sub_t sub_fn, u8 is_feature)
{
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
    return cnat_lookup_inline (vm, node, frame, af, 1 /* do_trace */, sub_fn,
			       is_feature, true /* alloc_if_not_found */);
  return cnat_lookup_inline (vm, node, frame, af, 0 /* do_trace */, sub_fn,
			     is_feature, true /* alloc_if_not_found */);
}
...
{
  return cnat_snat_node (vm, node, frame, AF_IP4, cnat_snat_node_input,
			 1 /* is_feature */);
}
...
VLIB_NODE_FN (cnat_snat_ip6_node) (vlib_main_t * vm,
				   ...
				   vlib_frame_t * frame)
{
  return cnat_snat_node (vm, node, frame, AF_IP6, cnat_snat_node_input,
			 1 /* is_feature */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return cnat_snat_node (vm, node, frame, AF_IP4, cnat_snat_node_input_return,
			 1 /* is_feature */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return cnat_snat_node (vm, node, frame, AF_IP6, cnat_snat_node_input_return,
			 1 /* is_feature */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return cnat_snat_node (vm, node, frame, AF_IP4, cnat_snat_node_client,
			 0 /* is_feature */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return cnat_snat_node (vm, node, frame, AF_IP6, cnat_snat_node_client,
			 0 /* is_feature */);
}
