...
cnat_snat_node_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
		   ...
		   f64 now, u8 do_trace, u8 is_client)
{
  cnat_timestamp_rewrite_t *rw = NULL;
...
static_always_inline void
cnat_snat_node_dnat (vlib_main_t *vm, vlib_node_runtime_t *node,
		     vlib_buffer_t *b, u16 *next0, ip_address_family_t af,
		     f64 now, u8 do_trace)
{
  cnat_snat_node_fn (vm, node, b, next0, af, now, do_trace,
...
cnat_snat_node (vlib_main_t *vm, vlib_node_runtime_t *node,
		vlib_frame_t *frame, ip_address_family_t af, u8 is_client)
{
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
    return cnat_lookup_inline (
      vm, node, frame, af, 1 /* do_trace */,
      (is_client ? cnat_snat_node_dnat : cnat_snat_node_input),
...
{
  return cnat_snat_node (vm, node, frame, AF_IP4, false /* is_client */);
}
...
VLIB_NODE_FN (cnat_snat_ip6_node) (vlib_main_t * vm,
				   ...
				   vlib_frame_t * frame)
{
  return cnat_snat_node (vm, node, frame, AF_IP6, false /* is_client */);
}
...
VLIB_NODE_FN (cnat_snat_ip4_snat_dnat)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return cnat_snat_node (vm, node, frame, AF_IP4, true /* is_client */);
}

VLIB_NODE_FN (cnat_snat_ip6_snat_dnat)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return cnat_snat_node (vm, node, frame, AF_IP6, true /* is_client */);
}

VLIB_REGISTER_NODE (cnat_snat_ip4_snat_dnat) = {
  .name = "ip4-cnat-snat-dnat",
  .vector_size = sizeof (u32),
  .format_trace = format_cnat_trace,
  .n_errors = CNAT_N_ERROR,
  .error_strings = cnat_error_strings,
...
VLIB_REGISTER_NODE (cnat_snat_ip6_snat_dnat) = {
  .name = "ip6-cnat-snat-dnat",
  .vector_size = sizeof (u32),
  .format_trace = format_cnat_trace,
  .n_errors = CNAT_N_ERROR,
  .error_strings = cnat_error_strings,
