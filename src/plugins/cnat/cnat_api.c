...
vl_api_feature_cnat_enable_disable_t_handler (vl_api_feature_cnat_enable_disable_t *mp)
{
  ...
  rv = vnet_sw_interface_cnat_enable_disable (ntohl (mp->sw_if_index), mp->enable_disable);
...
apply_main_vrf_cnat_snat_policy_on_vrf (u32 sw_if_index)
{
  ...
  if (vec_len (cpm->snat_policy_per_fwd_fib_index4) > CNAT_FIB_TABLE)
    {
      u32 cpe_index_v4 = vec_elt (cpm->snat_policy_per_fwd_fib_index4, CNAT_FIB_TABLE);
      u32 fib_index_ip4 = vec_elt (ip4_main.fib_index_by_sw_if_index, sw_if_index);
      cnat_init_port_allocator (fib_index_ip4);
      vec_validate_init_empty_aligned (cpm->snat_policy_per_fwd_fib_index4, fib_index_ip4, 0,
				       CLIB_CACHE_LINE_BYTES);
      cpm->snat_policy_per_fwd_fib_index4[fib_index_ip4] = cpe_index_v4;
    }

  if (vec_len (cpm->snat_policy_per_fwd_fib_index6) > CNAT_FIB_TABLE)
    {
      u32 cpe_index_v6 = vec_elt (cpm->snat_policy_per_fwd_fib_index6, CNAT_FIB_TABLE);
      u32 fib_index_ip6 = vec_elt (ip6_main.fib_index_by_sw_if_index, sw_if_index);
      cnat_init_port_allocator (fib_index_ip6);

      vec_validate_init_empty_aligned (cpm->snat_policy_per_fwd_fib_index6, fib_index_ip6, 0,
				       CLIB_CACHE_LINE_BYTES);
...
static void
vl_api_apply_default_cnat_snat_t_handler (vl_api_apply_default_cnat_snat_t *mp)
{
  vl_api_apply_default_cnat_snat_reply_t *rmp;
  int rv = 0;
  ...
  BAD_SW_IF_INDEX_LABEL;
  REPLY_MACRO (VL_API_APPLY_DEFAULT_CNAT_SNAT_REPLY);
}
...
vl_api_cnat_set_snat_addresses_t_handler (vl_api_cnat_set_snat_addresses_t *mp)
{
  ...
  rv = cnat_set_snat (CNAT_FIB_TABLE, CNAT_FIB_TABLE, &ip4, 32, &ip6, 128, sw_if_index,
		      CNAT_SNAT_POLICY_FLAG_NONE, mp->flags);
