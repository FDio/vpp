...
STATIC_ASSERT ((int) CNAT_TRANSLATION_NO_RETURN_SESSION == (int) CNAT_TR_FLAG_NO_RETURN_SESSION,
	       "cnat api enum mismatch");
STATIC_ASSERT ((int) CNAT_TRANSLATION_NO_CLIENT == (int) CNAT_TR_FLAG_NO_CLIENT,
	       "cnat api enum mismatch");
...
STATIC_ASSERT ((int) CNAT_SNAT_POLICY_F_NONE == (int) CNAT_SNAT_POLICY_FLAG_NONE,
	       "cnat api enum mismatch");
STATIC_ASSERT ((int) CNAT_SNAT_POLICY_F_BUFFER_NEXT == (int) CNAT_SNAT_POLICY_FLAG_BUFFER_NEXT,
	       "cnat api enum mismatch");
STATIC_ASSERT ((int) CNAT_SNAT_POLICY_F_NO_CLIENT == (int) CNAT_SNAT_POLICY_FLAG_NO_CLIENT,
	       "cnat api enum mismatch");
STATIC_ASSERT ((int) CNAT_SNAT_POLICY_F_USE_AS_DEFAULT ==
		 (int) CNAT_SNAT_POLICY_FLAG_USE_AS_DEFAULT,
	       "cnat api enum mismatch");
...
static void
vl_api_feature_cnat_enable_disable_t_handler (vl_api_feature_cnat_enable_disable_t *mp)
{
  ...
  rv = cnat_sw_interface_enable_disable (ntohl (mp->sw_if_index), mp->enable_disable);
...
cnat_translation_send_details (u32 cti, void *args)
{
  ...
  n_paths = vec_len (ct->ct_paths_indexes);
...
vl_api_cnat_snat_addresses_dump_t_handler (vl_api_cnat_snat_addresses_dump_t *mp)
{
  ...
  pool_foreach (cpe, cpm->snat_policies_pool)
    ...
    /* The dump message exposes ret_table_id*, which only makes sense
     * when ret_scope_id* is interpreted as a fib index. For non-fib
     * scopes (eg. tenant ids) fib_table_get_table_id will return ~0. */
    rmp->ret_table_id4 =
      clib_host_to_net_u32 (fib_table_get_table_id (cpe->ret_scope_id4, FIB_PROTOCOL_IP4));
    rmp->ret_table_id6 =
      clib_host_to_net_u32 (fib_table_get_table_id (cpe->ret_scope_id6, FIB_PROTOCOL_IP6));
    ip6_address_encode (&ip_addr_v6 (&cpe->snat_ip6.ce_ip), rmp->snat_ip6);
    ip4_address_encode (&ip_addr_v4 (&cpe->snat_ip4.ce_ip), rmp->snat_ip4);
    rmp->sw_if_index = clib_host_to_net_u32 (cpe->snat_ip6.ce_sw_if_index);
...
static void
vl_api_cnat_set_snat_addresses_v2_t_handler (vl_api_cnat_set_snat_addresses_v2_t *mp)
{
  vl_api_cnat_set_snat_addresses_v2_reply_t *rmp;
  u32 sw_if_index = clib_net_to_host_u32 (mp->sw_if_index);
  u32 fwd_fib_index = clib_net_to_host_u32 (mp->fwd_fib_index);
  /* mp->ret_fib_index is the wire field name (kept for ABI), but the
   * dataplane interprets it as a scope_id. */
  u32 ret_scope_id = clib_net_to_host_u32 (mp->ret_fib_index);
  ip4_address_t ip4;
  ip6_address_t ip6;
  int rv;
  ...
  rv = cnat_set_snat (fwd_fib_index, ret_scope_id, &ip4, 32, &ip6, 128, sw_if_index,
