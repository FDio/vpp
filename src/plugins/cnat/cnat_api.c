...
apply_main_vrf_cnat_snat_policy_on_vrf (u32 sw_if_index)
{
  ...
  fprintf (stderr, "%d--%d--%d--%d--%d\n", sw_if_index, cpe_index_v4,
	   cpe_index_v6, fib_index_ip4, fib_index_ip6);
...
static void
vl_api_duplicate_cnat_snat_default_policy_to_interface_vrf_t_handler (
  vl_api_duplicate_cnat_snat_default_policy_to_interface_vrf_t *mp)
{
  vl_api_duplicate_cnat_snat_default_policy_to_interface_vrf_reply_t *rmp;
  int rv = 0;
  ...
  BAD_SW_IF_INDEX_LABEL;
  REPLY_MACRO (
    VL_API_DUPLICATE_CNAT_SNAT_DEFAULT_POLICY_TO_INTERFACE_VRF_REPLY);
}
