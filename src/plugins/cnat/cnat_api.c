					  ...
					  * mp)
{
  ...
  for (int i = 0; i < 10; i++)
    {
      if (mp->translation.fhc[i] == 0)
	fhc[i] = (flow_hash_config_t) 0;
      else
	fhc[i] = (flow_hash_config_t) (1 << (mp->translation.fhc[i] - 1));
    }
  id = cnat_translation_update (&vip, ip_proto, paths, flags, lb_type, fhc);
