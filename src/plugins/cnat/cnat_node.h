		       ...
		       u8 *cts_flags)
{
  ...
  if (cts_flags && *cts_flags & CNAT_TS_RW_FLAG_CACHE_TS_L4)
	...
	ip_csum_update (csum, ip4->src_address.as_u32,
      ...
      csum =
	ip_csum_update (csum, old_port[VLIB_TX], new_port[VLIB_TX],
			ip4_header_t /* cheat */, length /* changed member */);
      csum =
	ip_csum_update (csum, old_port[VLIB_RX], new_port[VLIB_RX],
			ip4_header_t /* cheat */, length /* changed member */);
			 ...
			 cnat_5tuple_t *tup, u32 iph_offset, u8 swap)
{
  ...
  flow_hash_config_t hc = IP_FLOW_HASH_DEFAULT;
  hc |= swap ? IP_FLOW_HASH_REVERSE_SRC_DST : 0;
  if (AF_IP4 == af)
    {
      ip4_header_t *ip4;
...
cnat_rsession_create (cnat_timestamp_rewrite_t *rw, u32 flow_id)
{
  ...
  cnat_bihash_add_or_overwrite_with_cb (&cnat_session_db, &rkey,
		    ...
		    cnat_node_sub_t cnat_sub, u8 is_feature)
{
  ...
  /* this never changes */
  ((cnat_session_t *) &bkey[0])->key.__cs_pad = 0;
  ((cnat_session_t *) &bkey[1])->key.__cs_pad = 0;
  ((cnat_session_t *) &bkey[2])->key.__cs_pad = 0;
  ((cnat_session_t *) &bkey[3])->key.__cs_pad = 0;
