      ...
      if (PREDICT_FALSE (ip4->protocol == IP_PROTOCOL_ICMP))
	{
	  ...
	  if (icmp_type_is_error_message (icmp->type))
	  ...
	  else if (icmp_type_is_echo (icmp->type))
	    {
	      ...
	      tup->iproto = ip4->protocol;
	      // we assume icmp.identifier is request.RX and reply.TX
	      tup->port[VLIB_RX ^ swap] = icmp->type == ICMP4_echo_request ? echo->identifier : 0;
	      tup->port[VLIB_TX ^ swap] = icmp->type == ICMP4_echo_request ? 0 : echo->identifier;
	    }
	}
      else if ((ip4->protocol == IP_PROTOCOL_UDP ||
      ...
      if (PREDICT_FALSE (ip6->protocol == IP_PROTOCOL_ICMP6))
	{
	  ...
	  if (icmp6_type_is_error_message (icmp->type))
	  ...
	  else if (icmp6_type_is_echo (icmp->type))
	    {
	      ...
	      tup->iproto = ip6->protocol;
	      // we assume icmp.identifier is request.RX and reply.TX
	      tup->port[VLIB_RX ^ swap] = icmp->type == ICMP6_echo_request ? echo->identifier : 0;
	      tup->port[VLIB_TX ^ swap] = icmp->type == ICMP6_echo_request ? 0 : echo->identifier;
	    }
	}
      else if (ip6->protocol == IP_PROTOCOL_UDP ||
...
cnat_rsession_create_client (cnat_timestamp_rewrite_t *rw)
{
  ...
  cc =
    (AF_IP4 == rw->tuple.af ? cnat_client_ip4_find (&rw->tuple.ip4[VLIB_RX]) :
...
cnat_rsession_create (cnat_timestamp_rewrite_t *rw, u32 flow_id)
{
  ...
  rsession->value.cs_flags =
    CNAT_SESSION_FLAG_HAS_CLIENT | CNAT_SESSION_IS_RETURN;
