			 ...
			 cnat_5tuple_t *tup, u32 iph_offset, u8 swap)
{
  ...
  if (AF_IP4 == af)
	      ...
	      if (PREDICT_FALSE (ip4->protocol == IP_PROTOCOL_ICMP))
		{
		  ...
		  if (icmp_type_is_echo (icmp->type))
			...
			(cnat_echo_header_t *) (icmp + 1);
		      tup->ip4[VLIB_RX ^ swap].as_u32 =
			ip4->dst_address.as_u32;
		      tup->ip4[VLIB_TX ^ swap].as_u32 =
	       ...
	       ip4->protocol == IP_PROTOCOL_SCTP)
	{
	  ...
	  tup->iproto = ip4->protocol;
	  clib_memcpy_fast ((void *) &tup->ip4, (void *) &ip4->src_address,
			    12);
	}
    }
  else
		  ...
		  if (icmp6_type_is_echo (icmp->type))
		    {
		      cnat_echo_header_t *echo =
			(cnat_echo_header_t *) (icmp + 1);
		      ip6_address_copy (&tup->ip6[VLIB_RX ^ swap],
					&ip6->dst_address);
		      ip6_address_copy (&tup->ip6[VLIB_TX ^ swap],
					&ip6->src_address);
...
cnat_rsession_create_client (cnat_timestamp_rewrite_t *rw, u32 ret_fib_index)
{
  ...
  cc = (AF_IP4 == rw->tuple.af ?
		cnat_client_ip4_find (&rw->tuple.ip4[VLIB_RX], ret_fib_index) :
