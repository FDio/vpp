		...
		const cnat_timestamp_t *ts, const cnat_timestamp_rewrite_t *rw)
{
  ...
  t->flow_id = vnet_buffer2 (b)->session.generic_flow_id;
  t->flow_state = vnet_buffer2 (b)->session.state;

  if (ts)
    clib_memcpy (&t->ts, ts, sizeof (cnat_timestamp_t));

  t->lookup_context_id = cnat_scope_id (b) ? cnat_scope_id (b) : vnet_buffer (b)->ip.fib_index;
		      ...
		      u16 *sport, int *sport_retries, int *sport_failures)
{
  ...
  rsession->key.context_id = ret_fib_index;
...
/*
 * When a scoped lookup (context_id derived from session.generic_flow_id with
 * HAS_SCOPE flag) misses, retry with context_id = fib_index because
 * return sessions are always stored with ret_fib_index.
 * Restore the original context_id when both lookups miss so
 * new-session creation keeps the caller's scope.
 */
#define CNAT_SCOPE_ID_FALLBACK(n)                                                                  \
  if (rv[n] && session[n]->key.context_id != vnet_buffer (b[n])->ip.fib_index)                     \
    {                                                                                              \
      u32 _saved_ctx_##n = session[n]->key.context_id;                                             \
      session[n]->key.context_id = vnet_buffer (b[n])->ip.fib_index;                               \
      hash[n] = cnat_bihash_hash (&bkey[n]);                                                       \
      rv[n] = cnat_bihash_search_i2_hash (&cnat_session_db, hash[n], &bkey[n], &bvalue[n]);        \
      if (rv[n])                                                                                   \
	{                                                                                          \
	  session[n]->key.context_id = _saved_ctx_##n;                                             \
	  hash[n] = cnat_bihash_hash (&bkey[n]);                                                   \
	}                                                                                          \
    }
		    ...
		    bool alloc_if_not_found)
{
  ...
  if (n_left >= 4)
    {
      ...
      session[0]->key.context_id =
	cnat_scope_id (b[0]) ? cnat_scope_id (b[0]) : vnet_buffer (b[0])->ip.fib_index;
      session[1]->key.context_id =
	cnat_scope_id (b[1]) ? cnat_scope_id (b[1]) : vnet_buffer (b[1])->ip.fib_index;
      session[2]->key.context_id =
	cnat_scope_id (b[2]) ? cnat_scope_id (b[2]) : vnet_buffer (b[2])->ip.fib_index;
      session[3]->key.context_id =
	cnat_scope_id (b[3]) ? cnat_scope_id (b[3]) : vnet_buffer (b[3])->ip.fib_index;
  ...
  while (n_left >= 4)
      ...
      if (n_left >= 8)
	{
	  ...
	  session[0]->key.context_id =
	    cnat_scope_id (b[4]) ? cnat_scope_id (b[4]) : vnet_buffer (b[4])->ip.fib_index;
	  session[1]->key.context_id =
	    cnat_scope_id (b[5]) ? cnat_scope_id (b[5]) : vnet_buffer (b[5])->ip.fib_index;
	  session[2]->key.context_id =
	    cnat_scope_id (b[6]) ? cnat_scope_id (b[6]) : vnet_buffer (b[6])->ip.fib_index;
	  session[3]->key.context_id =
	    cnat_scope_id (b[7]) ? cnat_scope_id (b[7]) : vnet_buffer (b[7])->ip.fib_index;
