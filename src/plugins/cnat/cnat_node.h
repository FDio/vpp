...
cnat_ip4_translate_l4 (ip4_header_t *ip4, udp_header_t *udp, u16 *pkt_csum,
		       ...
		       u16 new_port[VLIB_N_DIR], u32 oflags, u16 *csum_diff,
		       u8 *cts_flags)
{
  ...
  if (cts_flags && *cts_flags & CNAT_TS_RW_FLAG_CACHE_TS_L4)
  ...
  else
    {
      ...
      csum =
	ip_csum_update (csum, ip4->dst_address.as_u32,
			new_addr[VLIB_TX].as_u32, ip4_header_t, dst_address);
      csum =
	ip_csum_update (csum, ip4->src_address.as_u32,
			new_addr[VLIB_RX].as_u32, ip4_header_t, src_address);

      csum = ip_csum_update (csum, old_port[VLIB_TX], new_port[VLIB_TX],
			     udp_header_t, dst_port);
      csum = ip_csum_update (csum, old_port[VLIB_RX], new_port[VLIB_RX],
...
cnat_ip4_translate_l3 (ip4_header_t *ip4, ip4_address_t new_addr[VLIB_N_DIR],
		       u32 oflags, u16 *csum_diff, u8 *cts_flags)
{
  ...
  if (cts_flags && *cts_flags & CNAT_TS_RW_FLAG_CACHE_TS_L3)
    ...
    {
      csum =
	ip_csum_update (csum, old_addr[VLIB_TX].as_u32,
			new_addr[VLIB_TX].as_u32, ip4_header_t, dst_address);
      csum =
	ip_csum_update (csum, old_addr[VLIB_RX].as_u32,
			new_addr[VLIB_RX].as_u32, ip4_header_t, src_address);
      if (csum_diff)
	{
	  ...
	  if (icmp_type_is_error_message (icmp->type))
	  ...
	  else if (icmp_type_is_echo (icmp->type))
	    {
	      ...
	      tup->port[VLIB_RX ^ swap] = echo->identifier;
	       ...
	       (ip4->protocol == IP_PROTOCOL_IP_IN_IP && !rewrite_from))
	{
	  udp = (udp_header_t *) (ip4 + 1);
      ...
      // in this case the buffer does not contain the session content itself
      // because the packet has been encapsulated, we use the rewrite.
      else if (ip4->protocol == IP_PROTOCOL_IP_IN_IP)
	{
	  ip46_address_set_ip4 (&tup->ip[VLIB_TX ^ swap],
	  ...
	  if (icmp6_type_is_error_message (icmp->type))
	  ...
	  else if (icmp6_type_is_echo (icmp->type))
	    {
	      ...
	      tup->port[VLIB_RX ^ swap] = echo->identifier;
	       ...
	       (ip6->protocol == IP_PROTOCOL_IP_IN_IP && !rewrite_from))
	{
	  udp = (udp_header_t *) (ip6 + 1);
      ...
      // in this case the buffer does not contain the session content itself
      // because the packet has been encapsulated, we use the rewrite.
      else if (ip6->protocol == IP_PROTOCOL_IP_IN_IP)
	{
	  ip46_address_set_ip6 (&tup->ip[VLIB_TX ^ swap],
...
cnat_rsession_create_client (cnat_timestamp_rewrite_t *rw, u32 ret_fib_index)
{
  ...
  const ip_address_family_t af =
    ip46_address_is_ip4 (&rw->tuple.ip[VLIB_RX]) ? AF_IP4 : AF_IP6;
...
cnat_rsession_create (cnat_timestamp_rewrite_t *rw, u32 flow_id,
		      u32 ret_fib_index, int add_client, u16 *sport,
		      int *sport_retries, int *sport_failures)
{
  ...
  if (sport)
    {
      ...
      *sport_retries = *sport_failures = 0;
      ...
      int rv =
	cnat_bihash_add_del (&cnat_session_db, &rkey, 2 /* no overwrite */);
      if (!rv)
	goto out; /* success! */
  ...
  cnat_bihash_add_with_overwrite_cb (&cnat_session_db, &rkey,
				     cnat_session_free_stale_cb, NULL);
