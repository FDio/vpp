		...
		const cnat_timestamp_t *ts, const cnat_timestamp_rewrite_t *rw)
{
  ...
  /* Computed the same way as the session key at lookup time so the
   * trace reflects the effective disambiguator. */
  t->lookup_scope_id = b->flow_id ? b->flow_id : vnet_buffer (b)->ip.fib_index;
		      ...
		      u16 *sport, int *sport_retries, int *sport_failures)
{
  ...
  /* create the reverse flow key — return traffic arrives via the DPO
   * path where no CNAT scope feature runs, so the reverse session is
   * keyed by the return-direction fib_index (= the return packet's
   * RX fib_index via the flow_id-or-fib fallback at lookup). */
  cnat_5tuple_copy (&rsession->key.cs_5tuple, &rw->tuple, 1 /* swap */);
  rsession->key.cs_scope_id = ret_fib_index;
