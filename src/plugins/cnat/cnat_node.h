		...
		const cnat_timestamp_t *ts, const cnat_timestamp_rewrite_t *rw)
{
  ...
  t->scope_or_flow_id = vnet_buffer2 (b)->session.scope_or_flow_id;
		      ...
		      u16 *sport, int *sport_retries, int *sport_failures)
{
  ...
  /* create the reverse flow key — return traffic arrives via the DPO path
   * where no scope is set, so reverse sessions are always unscoped. */
