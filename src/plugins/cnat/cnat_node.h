  ...
  while (n_left >= 4)
      ...
      if (n_left >= 8)
	{
	  if (is_output)
		...
		vnet_buffer (b[4])->ip.save_rewrite_length, 0, 0);
	      cnat_make_buffer_5tuple (
		b[5], af, (cnat_5tuple_t *) &bkey[1],
		vnet_buffer (b[5])->ip.save_rewrite_length, 0, 0);
	      cnat_make_buffer_5tuple (
		b[6], af, (cnat_5tuple_t *) &bkey[2],
		vnet_buffer (b[6])->ip.save_rewrite_length, 0, 0);
	      cnat_make_buffer_5tuple (
		b[7], af, (cnat_5tuple_t *) &bkey[3],
		vnet_buffer (b[7])->ip.save_rewrite_length, 0, 0);
	    }
	  else
	    {
