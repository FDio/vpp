		    ...
		    cnat_node_sub_t cnat_sub, u8 is_feature)
{
  ...
  u64 hash[4];
  int rv[4];
  u32 *fib_index_by_sw_if_index = AF_IP4 == af ?
  ...
  if (n_left >= 4)
    {
      ...
      session[1]->key.fib_index = vnet_buffer (b[0])->ip.fib_index;
      session[2]->key.fib_index = vnet_buffer (b[0])->ip.fib_index;
      session[3]->key.fib_index = vnet_buffer (b[0])->ip.fib_index;
...
cnat_node_select_ip6 (ip6_address_t *ip, const ip6_address_t *pfx, u64 mask)
{
  if ((u64) ~0 == mask)
  ...
  else
    {
      ...
      addr = clib_crc32c ((void *) ip, sizeof (*ip));
#else
      addr = clib_xxhash (ip6_address_hash_to_u64 (ip));
#endif
      ip->as_u64[0] = pfx->as_u64[0];
      ip->as_u64[1] = pfx->as_u64[1] | (addr & mask);
    }
}
