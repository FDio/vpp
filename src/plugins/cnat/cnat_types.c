...
cnat_lazy_init (void)
{
  ...
  /* Mark all slots as available. cnat_timestamp_alloc uses this bitmap to
   * find the next slot with free space; NULL means not yet initialised. */
  for (u32 i = 0; i < ctm->pool_max; i++)
    ctm->ts_free = clib_bitmap_set (ctm->ts_free, i, 1);
  cnat_ep_trk_t *eptrk;
