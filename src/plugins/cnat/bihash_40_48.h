...
format_bihash_kvp_40_48 (u8 *s, va_list *args)
{
  ...
  s = format (
    s, "key %llu %llu %llu %llu %llu value %llu %llu %llu %llu %llu %u",
    v->key[0], v->key[1], v->key[2], v->key[3], v->key[4], v->value[0],
    v->value[1], v->value[2], v->value[3], v->value[4], v->value[5]);
  return s;
}
