...
format_cnat_feature_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "input-if:%d output-if:%d snat-pol:%d ", t->input_if,
	      t->output_if, t->snat_policy_result);
...
cnat_input_feature_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
		       vlib_buffer_t *b, cnat_node_ctx_t *ctx, int rv,
		       cnat_session_t *session)
{
...
VLIB_REGISTER_NODE (
  cnat_input_feature_ip4_node) = { .name = "cnat-input-ip4",
...
VLIB_REGISTER_NODE (
  cnat_output_feature_ip4_node) = { .name = "cnat-output-ip4",
				    .vector_size = sizeof (u32),
				    .format_trace = format_cnat_feature_trace,
				    .type = VLIB_NODE_TYPE_INTERNAL,
				    .n_errors = CNAT_N_ERROR,
...
VLIB_REGISTER_NODE (
  cnat_output_feature_ip6_node) = { .name = "cnat-output-ip6",
				    .vector_size = sizeof (u32),
				    .format_trace = format_cnat_feature_trace,
				    .type = VLIB_NODE_TYPE_INTERNAL,
				    .n_errors = CNAT_N_ERROR,
