		       ...
		       snort_qpair_t *qp)
{
  ...
  daq_vpp_head_tail_t tail, last_tail = qp->ebuf_tail;
  u32 from[VLIB_FRAME_SIZE];
  u16 nexts[VLIB_FRAME_SIZE];
  u32 n_recv;

  // recv injected buffers from ebuf ring
  tail = __atomic_load_n (&qp->hdr->deq.ebuf_tail, __ATOMIC_ACQUIRE);

  n_recv = tail - last_tail;

  for (u32 i = 0; i < n_recv; i++)
    {
      daq_vpp_ebuf_desc_t *desc = &qp->ebuf_ring[last_tail & mask];
      ...
      bi = qp->ebuffers[last_tail & mask];
      qp->ebuffers[last_tail & mask] = VLIB_BUFFER_INVALID_INDEX;
  ...
  if (n_recv)
    {
      ...
      qp->ebuf_tail = last_tail;
    }

  // allocate more ebufs if needed
