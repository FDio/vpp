		 ...
		 int fds[])
{
  ...
  DEBUG ("socket reply type %u received (rv: %s)", reply->type,
...
daq_vpp_rv_t
daq_vpp_connect (uint16_t num_instances)
{
  daq_vpp_main_t *vdm = &daq_vpp_main;
  daq_vpp_msg_reply_t reply = {};
  daq_vpp_rv_t rv = DAQ_VPP_OK;
  int fd;

  rv = daq_vpp_socket_connect ();
  if (rv != DAQ_VPP_OK)
    goto err;
...
daq_vpp_rv_t
daq_vpp_find_or_add_input (char *name, daq_vpp_input_t **inp)
{
  daq_vpp_main_t *vdm = &daq_vpp_main;
  daq_vpp_input_t *in = vdm->inputs;
  ...
  rv = daq_vpp_request (&req, &reply, 1, &fd);
  ...
  DEBUG ("%lu bytes of descriptor memory mapped at %p", in->shm_size,
	 in->shm_base);
  ...
  for (daq_vpp_qpair_index_t qi = 0; qi < in->num_qpairs; qi++)
	...
	.get_input_qpair = {
      ...
      rv = daq_vpp_request (&req, &reply, 2, fds);
      if (rv != DAQ_VPP_OK)
	goto err;
