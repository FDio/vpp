		     ...
		     void **ctxt_ptr)
{
  ...
  ctx->msg_pool_info.available = sz;
  ctx->msg_pool_info.size = sz;
  ctx->msg_pool_info.mem_size = sz * sizeof (daq_vpp_msg_pool_entry_t);
		  ...
		  struct timeval tv)
{
  ...
  pe->pkthdr.address_space_id = d->address_space_id;
  pe->pkthdr.ingress_index = qp->input_index;
  pe->msg.data = data;
			 ...
			 const DAQ_Msg_t *msgs[], unsigned max_recv)
{
  ...
  uint32_t desc_index, next_desc_index = qp->enq_ring[tail & mask];

  gettimeofday (&tv, NULL);
  for (n_left = n_recv; n_left > 1; n_left--, tail++, msgs++)
    {
      desc_index = next_desc_index;
      next_desc_index = qp->enq_ring[(tail + 1) & mask];
...
daq_vpp_msg_finalize (void *handle, const DAQ_Msg_t *msg, DAQ_Verdict verdict)
{
  ...
  head = __atomic_load_n (&h->deq_head, __ATOMIC_RELAXED);
  d = h->descs + pe->index;

  d->action = translation_table[verdict];
  qp->deq_ring[head & mask] = pe->index;
  head = head + 1;
  __atomic_store_n (&h->deq_head, head, __ATOMIC_RELEASE);
...
daq_vpp_ioctl (void *handle, DAQ_IoctlCmd cmd, void *arg, size_t arglen)
{
  ...
  DEBUG ("instance_id %u cmd %u arglen %lu", ctx->instance_id, cmd, arglen);
