			 ...
			 const DAQ_Msg_t *msgs[], unsigned max_recv)
{
  ...
  uint32_t desc_index, next_desc_index = qp->enq_ring[next & mask];

  gettimeofday (&tv, NULL);
  for (n_left = n_recv; n_left > 1; n_left--, next++, msgs++)
    {
      desc_index = next_desc_index;
      next_desc_index = qp->enq_ring[(next + 1) & mask];
