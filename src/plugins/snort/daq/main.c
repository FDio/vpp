		     ...
		     void **ctxt_ptr)
{
  ...
  rrv = daq_vpp_parse_qpair_ids (ctx, end_of_name, &qpair_ids, &n_qpair_ids);
  if (rrv != DAQ_SUCCESS)
    goto err;

  if (!vdm->config_parsed)
    {
      rrv = daq_vpp_parse_config (ctx, modcfg);
      if (rrv != DAQ_SUCCESS)
	goto err;
      vdm->config_parsed = 1;
    }
  ...
  if (!vdm->connected)
    {
      rrv = daq_vpp_connect (ctx, n_instances);
  ...
  rrv = daq_vpp_find_or_add_input (ctx, name, &in);
  if (rrv != DAQ_SUCCESS)
    goto err;
  ...
  if (ctx->epoll_fd == -1)
    {
      rrv =
	daq_vpp_err (ctx, "failed to create epoll instance for instance %u",
		     ctx->instance_id);
      goto err;
    }
  ...
  if (n_qpair_ids)
      ...
      for (uint32_t i = 0; i < n_qpair_ids; i++)
	{
	  ...
	  if (!qp)
	    {
	      rrv = daq_vpp_err (ctx, "cannot find qpair %u.%u",
				 qpair_ids[i].thread_id, qpair_ids[i].queue_id,
				 ctx->instance_id);
	      goto err;
	    }
	  rrv = daq_vpp_add_qpair_to_instance (ctx, qp);
	  if (rrv != DAQ_SUCCESS)
	    goto err;
	}
      free (qpair_ids);
    ...
    for (daq_vpp_qpair_index_t i = 0; i < in->num_qpairs; i++)
      {
	rrv = daq_vpp_add_qpair_to_instance (ctx, in->qpairs + i);
	if (rrv != DAQ_SUCCESS)
	  goto err;
      }
