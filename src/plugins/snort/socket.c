  ...
  vec_foreach (cqp, c->qpairs)
    {
      ...
      // FIXME snort_per_thread_data_t *ptd = vec_elt_at_index
	...
	__atomic_store_n (&qp->ready, 1, __ATOMIC_RELEASE);
      ...
      // FIXME clib_interrupt_set (ptd->interrupts, (int) uf->private_data);
...
static void
snort_msg_get_input_qpair (snort_client_t *c, daq_vpp_msg_req_t *req,
			   snort_client_msg_queue_elt *e)
{
  daq_vpp_msg_reply_get_input_qpair_t *r = &e->msg.get_input_qpair;
  u32 instance_index = req->get_input_qpair.input_index;
  u32 qpair_index = req->get_input_qpair.qpair_index;
  snort_instance_t *si;
  snort_qpair_t *qp;
  u8 *base;
  ...
  *e = (snort_client_msg_queue_elt){
  ...
  switch (req.type)
    ...
    case DAQ_VPP_MSG_TYPE_GET_INPUT_QPAIR:
      snort_msg_get_input_qpair (c, &req, e);
      break;
