				 ...
				 vlib_cli_command_t *cmd)
{
  ...
  pool_foreach (si, sm->instances)
      ...
      vec_foreach_pointer (qp, si->qpairs)
	{
	  ...
	  vlib_cli_output (
	    vm, "    ebqueue: ring_offset %u ring_size %u  head %lu tail %lu",
	    (u8 *) qp->ebuf_ring - (u8 *) si->shm_base,
	    1 << qp->log2_ebuf_queue_size, qp->hdr->ebuf.head, qp->ebuf_tail);
	  ...
	  if (verbose)
	      ...
	      for (u32 i = 0; i < total_desc; i++)
		{
		  ...
		  vlib_cli_output (vm, "  %-6d  %-12u   %-12u  %-14u %U", i,
				   ...
				   qpe->freelist_next, format_snort_desc_flags,
				   d->flags);
		}
	    }
	}
