...
{
  unsigned n_recv, n_left;
  uint32_t head, mask = qp->queue_size - 1;
  ...
  n_recv = n_left = (head - qp->next_desc) & mask;

  while (n_left--)
    {
      uint8_t *data;
      uint32_t desc_index = qp->enq_ring[qp->next_desc];
      daq_vpp_desc_t *d = qp->descs + desc_index;
      VPPDescData *dd = qp->desc_data + desc_index;
      dd->pkthdr.pktlen = d->length;
      data = dd->msg.data = vc->bpools[d->buffer_pool].base + d->offset;

      qp->next_desc = (qp->next_desc + 1) & mask;

#if 0
      printf ("qpair %u head %u index %u\n", vc->next_qpair, qp->next_desc,
	      desc_index);
      printf ("desc buffer_pool %u offset %u len %u", d->buffer_pool,
	      d->offset, d->length);
		     ...
		     const DAQ_Msg_t *msgs[], DAQ_RecvStatus *rstat)
{
  ...
  for (int i = 0; i < n_events; i++)
      ...
      if ((n = vpp_daq_msg_receive_one (vc, qp, msgs, max_recv - n_recv)))
	{
      ...
      read (qp->enq_fd, &b, sizeof (b));
      printf ("enqueue event received on qpair %u\n",
	      vc->epoll_events[i].data.u32);
    }
