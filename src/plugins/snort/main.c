...
snort_deq_ready (clib_file_t *uf)
{
  ...
  vlib_node_set_interrupt_pending (vm, uf->private_data);
  return 0;
}
...

{
  ...
  size = round_pow2 ((uword) nqp * qpair_mem_sz, clib_mem_get_page_size ());
  ...
  vec_validate_aligned (si->qpairs, nqp - 1, CLIB_CACHE_LINE_BYTES);
  ...
  for (int i = 0; i < nqp; i++)
	...
	&(clib_file_t){
	  ...
	  .description = format (0, "snort dequeue for instance '%s' qpair %u",
				 si->name, i),
	  .polling_thread_index = i,
	  .private_data = si->dequeue_node_index });
    }
