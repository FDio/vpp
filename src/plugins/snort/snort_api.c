			     ...
			     vl_api_registration_t *rp, u32 context)
{
  ...
  REPLY_MACRO_DETAILS5 (
    VL_API_SNORT_INSTANCE_DETAILS, name_len, rp, context, ({
      ...
      rmp->qpairs_per_thread = clib_host_to_net_u32 (instance->qpairs_per_thread);
    }));
}
...
static void
send_snort_interface_details (u32 sw_if_index, u16 in_instance_index, u16 out_instance_index,
			      vl_api_registration_t *rp, u32 context)
{
  ...
  if (sw_if_index == INDEX_INVALID)
    {
      ...
      REPLY_AND_DETAILS_VEC_MACRO(VL_API_SNORT_INTERFACE_GET_REPLY,
	  ...
	  send_snort_interface_details (cursor, *index, *outdex, rp, mp->context);
      }))
      /* clang-format on */
    }
  else
      ...
      if (snort_get_instance_by_index (*index) || snort_get_instance_by_index (*outdex))
	{
	  ...
	  send_snort_interface_details (sw_if_index, *index, *outdex, rp, mp->context);
	}
      else
	{
	  rv = VNET_API_ERROR_NO_SUCH_ENTRY;
...
vl_api_snort_interface_detach_t_handler (vl_api_snort_interface_detach_t *mp)
{
  ...
  u32 instance_index = clib_net_to_host_u32 (mp->instance_index);
  u32 sw_if_index = clib_net_to_host_u32 (mp->sw_if_index);
  u8 in = 0, out = 0, snort_dir = mp->snort_dir;
