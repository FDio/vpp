...
static void
vl_api_snort_instance_by_name_t_handler (vl_api_snort_instance_by_name_t *mp)
{
  vl_api_snort_instance_by_name_reply_t *rmp;
  snort_instance_t *instance;
  char *name;
  int rv = 0;

  name = vl_api_from_api_to_new_c_string (&mp->name);
  instance = snort_get_instance_by_name (name);
  if (!instance)
    {
      clib_error_return (0, "unknown instance '%s' requested via api", name);
      rv = VNET_API_ERROR_NO_SUCH_ENTRY;
    }

  REPLY_MACRO2 (VL_API_SNORT_INSTANCE_BY_NAME_REPLY, ({
		  if (!rv)
		    {
		      rmp->instance_index =
			clib_host_to_net_u32 (instance->index);
		    }
		}));
}

static void
vl_api_snort_instance_by_index_t_handler (vl_api_snort_instance_by_index_t *mp)
{
  vl_api_snort_instance_by_index_reply_t *rmp;
  int rv = 0;
  u32 index = clib_net_to_host_u32 (mp->instance_index);
  snort_instance_t *si;
  u32 name_len = 0;

  si = snort_get_instance_by_index (index);
  if (!si)
    {
      clib_error_return (0, "unknown instance with index %d requested via api",
			 index);
      rv = VNET_API_ERROR_NO_SUCH_ENTRY;
    }
  else
    {
      name_len = vec_len (si->name);
    }
  REPLY_MACRO3 (VL_API_SNORT_INSTANCE_BY_INDEX_REPLY, name_len, {
    if (!rv)
      vl_api_vec_to_api_string (si->name, &rmp->name);
  });
}
...
vl_api_snort_instance_create_t_handler (vl_api_snort_instance_create_t *mp)
{
  ...
  else if ((si = snort_get_instance_by_name (name)))
    {
...
vl_api_snort_instance_delete_t_handler (vl_api_snort_instance_delete_t *mp)
{
  ...
  err = snort_instance_delete (vm, instance_index);
...
vl_api_snort_interface_attach_t_handler (vl_api_snort_interface_attach_t *mp)
{
  ...
  if (!instance)
    rv = VNET_API_ERROR_NO_SUCH_ENTRY;
  else if (snort_interface_enable_disable (vm, (char *) instance->name,
...
vl_api_snort_client_disconnect_t_handler (vl_api_snort_client_disconnect_t *mp)
{
  ...
  if ((client_index >= pool_elts (sm->clients)) ||
      (client = pool_elt_at_index (sm->clients, client_index)) == NULL)
    {
      rv = VNET_API_ERROR_NO_SUCH_ENTRY;
    }
  else
  ...
  vl_api_snort_instance_disconnect_t *mp)
{
  ...
  if (!si)
    rv = VNET_API_ERROR_NO_SUCH_ENTRY;
  else if (si->client_index != INDEX_INVALID)
    {
...
vl_api_snort_interface_detach_t_handler (vl_api_snort_interface_detach_t *mp)
{
  ...
  err = snort_interface_enable_disable (vm, NULL, sw_if_index,
					0 /* is_enable */, 0);
