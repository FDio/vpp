...
snort_interface_flow_add_del (snort_interface_flow_add_del_args_t *a)
{
  ...
  if (fi->is_ip4)
      ...
      if (clib_bihash_search_inline_16_8 (&sm->v4_flow_hash, &kv) == 0)
	{
      ...
      return clib_bihash_add_del_16_8 (&sm->v4_flow_hash, &kv, a->is_add);
    }
  else
    {
      clib_bihash_kv_48_8_t kv;
      ...
      if (clib_bihash_search_inline_48_8 (&sm->v6_flow_hash, &kv) == 0)
	{
      ...
      kv.value = snort_interface_flow_index (sif, sf);
      return clib_bihash_add_del_48_8 (&sm->v6_flow_hash, &kv, a->is_add);
    }
}
  ...
  while (n_left_from)
    {
      ...
      flow = snort_interface_flow_lookup_w_ih (sm, sif,
...
vlib_node_registration_t snort_node;
...
int
snort_create_memif_interface (void)
{
  snort_main_t *sm = snort_get_main ();
  vlib_main_t *vm = vlib_get_main ();
  int (*fp) (vlib_main_t *, memif_create_if_args_t *);
  ...
  fp = vlib_get_plugin_symbol ("memif_plugin.so", "memif_create_if");
  if ((*fp) (vm, &args))
    return -1;

  sm->sw_if_index = args.sw_if_index;
  return 0;
}

int
snort_enable_disable (u8 is_en)
{
  if (is_en)
    {
      if (snort_create_memif_interface ())
	{
