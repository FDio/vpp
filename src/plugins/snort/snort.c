...
snort_interface_add_del (snort_interface_add_del_args_t * a)
{
  ...
  if (a->is_add)
      ...
      if (sif)
	{
      ...
      sm->snort_iface_by_sw_if_index[a->sw_if_index] =
	snort_interface_index (sif);
      vnet_feature_enable_disable ("ip4-unicast", "snort-input",
				   a->sw_if_index, 1, 0, 0);
    }
  else
    {
...
{
  kv->key[0] = (u64) dst->as_u32 << 32 | (u64) src->as_u32;
  kv->key[1] = (u64) proto << 32 | (u64) dst_port << 16 | (u64) src_port;
  kv->value = ~0ULL;
}
...
{
  kv->key[0] = src->as_u64[0];
  kv->key[1] = src->as_u64[1];
  kv->key[2] = dst->as_u64[0];
  kv->key[3] = dst->as_u64[1];
  kv->key[4] = (u64) proto << 32 | (u64) dst_port << 16 | (u64) src_port;
  kv->key[5] = 0;
  kv->value = ~0ULL;
}

static inline snort_flow_t *
snort_interface_flow_lookup_ih (snort_main_t * sm, snort_interface_t * sif,
				void *ih, u8 is_ip4)
{
...
static uword
snort_input_fn (vlib_main_t * vm, vlib_node_runtime_t * node,
		vlib_frame_t * frame)
{
  ...
  while (n_left_from > 4)
	...
	CLIB_PREFETCH (b[3]->data, CLIB_CACHE_LINE_BYTES, LOAD);
      ...
      flow0 = snort_interface_flow_lookup_ih (sm, sif0,
					      vlib_buffer_get_current (b[0]),
					      1);
      flow1 = snort_interface_flow_lookup_ih (sm, sif1,
