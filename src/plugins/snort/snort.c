...
snort_interface_lookup (u32 sw_if_index)
{
  ...
  return sm->interfaces[sif_index];
}
...
int
snort_interface_add_del (u32 sw_if_index, u8 is_add)
{
  snort_interface_t *sif;
  ...
  if (is_add)
      ...
      if (sif)
	{
      ...
      sif->sw_if_index = sw_if_index;
      vnet_feature_enable_disable ("ip4-unicast", "snort", sw_if_index, 1, 0,
	                           0);
    }
  else
    {
      if (!sif)
	{
	  clib_warning ("Snort not enabled on interface: %u", sw_if_index);
	  return -1;
	}
      vnet_feature_enable_disable ("ip4-unicast", "snort", sw_if_index, 0, 0,
	                           0);
  ...
  if (is_ip4)
    {
      ...
      snort_make_kv4 (&kv, &ih4->src_address, &ih4->dst_address,
	              uh->src_port, uh->dst_port, ih4->protocol);
  ...
  if (fi->is_ip4)
    {
      ...
      if (clib_bihash_search_inline_16_8 (&sm->v4_flow_hash, &kv) == 0)
	{
	  clib_warning("flow already exists");
	  return -1;
	}
      kv.value = snort_interface_flow_index (sif, sf);
      return clib_bihash_add_del_16_8 (&sm->v4_flow_hash, &kv);
    }
  else
    {
      clib_bihash_kv_48_8_t kv;
      snort_make_kv4 (&kv, &fi->v6.src, &fi->v6.dst,
      ...
      if (clib_bihash_search_inline_48_8 (&sm->v6_flow_hash, &kv) == 0)
	{
      ...
      kv.value = snort_interface_flow_index (sif, sf);
      return clib_bihash_add_del_48_8 (&sm->v6_flow_hash, &kv);
    }
}

static uword
snort_fn (vlib_main_t * vm, vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from)
    {
      ...
      sif = snort_interface_lookup (sw_if_index0);
      flow = snort_interface_flow_lookup_w_ih (sm, sif, vlib_get_current (b), 1);
      if (flow && flow->action == SNORT_ACTION_FWD)
	{
	  vnet_feature_next (sw_if_index0, &next0, b);
	}
      else
	{
	  next0 = SNORT_NEXT_INSPECT;
	  vnet_buffer (b)->sw_if_index[VLIB_TX] = sm->sw_if_index;
	}
...
VLIB_NODE_FUNCTION_MULTIARCH (snort_node, snort_fn);

#define MEMIF_IFACE_RX_QUEUES 1
#define MEMIF_IFACE_TX_QUEUES 1
#define MEMIF_IFACE_BUFFER_SIZE 2048
#define MEMIF_IFACE_RING_SIZE 102

static u32
snort_create_iface (void)
{
  ...
  memif_create_if_args_t args = {
      ...
      .secret = "secret",
      .rx_queues = MEMIF_IFACE_RX_QUEUES,
      .tx_queues = MEMIF_IFACE_TX_QUEUES,
      .log2_ring_size = min_log2 (MEMIF_IFACE_RING_SIZE),
      .buffer_size = MEMIF_IFACE_BUFFER_SIZE,
  };

  if (memif_create_if (vm, &args))
    return ~0;

  return args.sw_if_index;
}
...
{
  snort_main_t *sm = &snort_main;
  if ((error = vlib_call_init_function (vm, memif_init)))
    return error;

  sm->sw_if_index = snort_create_iface ();
  if (sm->sw_if_index == ~0)
    {
      clib_warning ("failed to create memif interface");
      return 0;
    }
