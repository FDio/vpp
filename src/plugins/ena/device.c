		   ...
		   void **pp, char *fmt, ...)
{
  ...
  *pp = p;
  ...
  ena_log_info (ed,
		"%s: allocated %u bytes at %p (PA: 0x%lx) on numa %u for '%v'",
...
ena_create_if (vlib_main_t *vm, ena_create_if_args_t *args)
{
  ...
  if (0)
    {
      if ((err = ena_physmem_alloc (vm, ed, 4096, 4096,
				    (void **) &ed->debug_area, "debug area")))
	goto done;
  ...
  if (ena_admin_feature_is_supported (ed, ENA_ADMIN_FEAT_ID_MAX_QUEUES_EXT))
	...
	txq_sz = clib_min (max, ENA_DEFAULT_TXQ_SIZE);
  ...
  for (u16 i = 0; i < n_rxq; i++)
    if ((err = ena_create_rx_queue (vm, ed, rxq_sz)))
      goto done;

  for (u16 i = 0; i < n_txq; i++)
    if ((err = ena_create_tx_queue (vm, ed, txq_sz)))
      goto done;
