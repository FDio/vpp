...
clib_error_t *
ena_aq_req (vlib_main_t *vm, ena_device_t *ed, ena_aq_opcode_t opcode,
		       ...
		       ena_aq_feature_id_t feat_id, void *data)
{
  ...
  if ((err = ena_aq_req (vm, ed, ENA_AQ_OPCODE_SET_FEATURE, &fd,
		       ...
		       ena_aq_feature_id_t feat_id, void *data)
{
  ...
  if ((err = ena_aq_req (vm, ed, ENA_AQ_OPCODE_GET_FEATURE, &fd,
...
{
  ena_aq_create_cq_cmd_t *c = va_arg (*args, ena_aq_create_cq_cmd_t *);
  s = format (s, "%s %u", "interrupt_mode_enabled", c->interrupt_mode_enabled);
  s = format (s, ", %s %u", "cq_entry_size_words", c->cq_entry_size_words);
  s = format (s, ", %s %u", "cq_depth", c->cq_depth);
  s = format (s, ", %s %x", "msix_vector", c->msix_vector);
  s = format (s, ", %s %U", "cq_ba", format_ena_mem_addr, c->cq_ba);
  return s;
}
...
{
  ena_aq_create_cq_resp_t *r = va_arg (*args, ena_aq_create_cq_resp_t *);
#define _(n) s = format (s, "%s %u", #n, r->n);
  _ (cq_idx);
  _ (cq_actual_depth);
  _ (numa_node_register_offset);
  _ (cq_head_db_register_offset);
  _ (cq_interrupt_unmask_register_offset);
#undef _
  return s;
}
  ...
  if (rxq->sq_idx != 0xffff)
    {
      ena_aq_destroy_sq_cmd_t cmd = { .sq_idx = rxq->sq_idx,
				      .sq_direction = 2 };
      err = ena_aq_req (vm, ed, ENA_AQ_OPCODE_DESTROY_SQ, &cmd,
			sizeof (ena_aq_create_sq_cmd_t), 0, 0);
      if (err)
	{
  ...
  if (rxq->cq_idx != 0xffff)
    {
      ...
      err = ena_aq_req (vm, ed, ENA_AQ_OPCODE_DESTROY_CQ, &cmd,
			sizeof (ena_aq_create_cq_cmd_t), 0, 0);
      if (err)
	{
	  ena_log_err (ed, "destroy_rx_queue: %U", format_clib_error, err);
	  clib_error_free (err);
...
ena_create_rx_queue (vlib_main_t *vm, ena_device_t *ed, u16 n_desc)
{
  ...
  ena_log_debug (ed, "create_cq_cmd %U", format_ena_aq_create_cq_cmd, &cqcmd);
  ena_set_mem_addr (vm, ed, &cqcmd.cq_ba, rxq->cqes);

  if ((err = ena_aq_req (vm, ed, ENA_AQ_OPCODE_CREATE_CQ, &cqcmd,
			 sizeof (ena_aq_create_cq_cmd_t), &cqresp,
			 sizeof (ena_aq_create_cq_resp_t))))
    goto done;
  ena_log_debug (ed, "create_cq_resp %U", format_ena_aq_create_cq_resp,
		 &cqresp);
