...
ena_create_tx_queue (vlib_main_t *vm, ena_device_t *ed, u16 log2_n_desc)
{
  ...
  for (u32 i = 0; i < n_desc; i++)
    {
      txq->sqes[i].req_id_lo = i;
      txq->sqes[i].req_id_hi = i >> 10;
    }
...
ena_create_if (vlib_main_t *vm, ena_create_if_args_t *args)
{
  ...
  hi->driver_supported_features.rx_offset = 0;
  hi->driver_supported_features.interrupt_moderation = 0;
  hi->driver_supported_features.rx_buf_mirroring = 0;
  hi->driver_supported_features.rss_configurable_function_key = 0;
