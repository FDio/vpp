...
static ena_admin_host_info_t host_info = {
  ...
  .driver_version = { .major = 1, .minor = 0, .sub_minor = 0 },
  .ena_spec_version = { .major = 2, .minor = 0 },
  .driver_supported_features = { .rss_configurable_function_key = 1,
				 .interrupt_moderation = 1 }
};
...
ena_device_init (vlib_main_t *vm, ena_device_t *ed)
{
  ...
  /* reset */
  if ((err = ena_reg_reset (vm, ed, ENA_REG_RESET_REASON_NORMAL)))
    return err;
  ;
