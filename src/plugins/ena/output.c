...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  /* if infra tells us that this queue is shgared between threads we need to
   * acquire spinlock */
  if (tf->shared_queue)
    {
      typeof (txq->lock) tmp = 0;
      while (!__atomic_compare_exchange_n (&txq->lock, &tmp, 1, 0,
