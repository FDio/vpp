...
static_always_inline void
ena_set_tx_offload (ena_tx_ctx_t *ctx, vlib_buffer_t *b, ena_tx_desc_t *d, ena_tx_meta_desc_t *md)
{
  /* Is there any offload? */
  if (PREDICT_TRUE (!((b->flags & VNET_BUFFER_F_OFFLOAD) | (b->flags & VNET_BUFFER_F_GSO))))
    return;
  vnet_buffer_oflags_t oflags = vnet_buffer (b)->oflags;
  if (oflags & VNET_BUFFER_OFFLOAD_F_IP_CKSUM) {
    d->l3_csum_en = 1;
    d->l3_proto_idx = (oflags & VNET_BUFFER_F_IS_IP4) ? ETHERNET_TYPE_IP4 : ETHERNET_TYPE_IP6;
  }
  ...
  md->l3_hdr_len = vnet_buffer (b)->l4_hdr_offset - vnet_buffer (b)->l3_hdr_offset;
...
ena_txq_enq_one (vlib_main_t *vm, ena_tx_ctx_t *ctx, vlib_buffer_t *b0,
		 ena_tx_desc_t *d, u16 n_free_desc, u32 *f, int use_iova, int tx_offloads)
{
  ena_tx_desc_t first_desc;
  ena_tx_meta_desc_t meta_desc = { .ext_valid = 1, .eth_meta_type = 1, .meta_store = 1, .meta_desc = 1, .first = 1 };
  vlib_buffer_t *b;
  u32 i, n;
  u8 n_meta = 0;
  ...
  ctx->n_packets_left -= 1;
  ctx->n_bytes +=
    ena_txq_wr_sqe (vm, b0, use_iova, d++, n, first_desc);
...
ena_txq_enq (vlib_main_t *vm, ena_tx_ctx_t *ctx, ena_txq_t *txq, int use_iova, int tx_offloads)
{
  ...
  while (n_desc_left >= 4 && ctx->n_packets_left >= 8)
    {
      ...
      if (PREDICT_FALSE (!tx_offloads && ((b0->flags | b1->flags | b2->flags | b3->flags) &
	...
	{
	  n = ena_txq_enq_one (vm, ctx, b0, d, n_desc_left, f, use_iova, tx_offloads);
	  if (n == 0)
	    break;
	  n_desc_left -= n;
	  f += n;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  ena_tx_meta_desc_t cached_meta = { .ext_valid = 1, .eth_meta_type = 1, .meta_store = 1, .meta_desc = 1, .first = 1 };
