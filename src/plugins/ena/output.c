			      ...
			      ena_txq_t *txq)
{
  ...
  u16 mask = pow2_mask (txq->log2_n_desc);
  u32 n_free = 0;
  u32 phase;
  ...
  while (cd->phase == phase)
    {
      u16 req_id = cd->req_id & pow2_mask (ENA_MAX_LOG2_TXQ_SIZE);
      u16 n_in_chain = cd->req_id >> ENA_MAX_LOG2_TXQ_SIZE;

      n_free += n_in_chain;
      uword_bitmap_set_bits_at_index (txq->free_bitmap, req_id, 1);
      while (n_in_chain-- > 1)
	uword_bitmap_set_bits_at_index (txq->free_bitmap, ++req_id & mask, 1);
	     ...
	     int use_iova)
{
  ...
  if (n)
      ...
      if (n_before_wrap >= n)
	{
	  ...
	  sqe = txq->sqes + offset;
	  ena_txq_copy_sqes (sqe, d, n_copy);
	}
      else
	{
