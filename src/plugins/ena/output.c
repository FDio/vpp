...
#define ENA_TX_REQ_ID_MASK  ((1U << ENA_MAX_LOG2_TXQ_SIZE) - 1)
...
const static ena_tx_cdesc_t mask_phase = { .phase = 1 };

const static ena_tx_cdesc_t mask_ph_nic = {
  .req_id = (typeof (mask_ph_nic.req_id)) ~ENA_TX_REQ_ID_MASK, .phase = 1
};

const static ena_tx_cdesc_t mask_reqid = { .req_id = ENA_TX_REQ_ID_MASK };

static const ena_tx_cdesc_t match_ph0_nic1 = {
  .req_id = (typeof (match_ph0_nic1.req_id)) (1U << ENA_MAX_LOG2_TXQ_SIZE),
  .phase = 0
};
static const ena_tx_cdesc_t match_ph1_nic1 = {
  .req_id = (typeof (match_ph1_nic1.req_id)) (1U << ENA_MAX_LOG2_TXQ_SIZE),
  .phase = 1
};
...
ena_txq_deq (vlib_main_t *vm, ena_txq_enq_ctx *ctx, ena_txq_t *txq)
{
  ...
  u32 next = txq->cq_next;
  u32 offset, n = 0;
  u32 *buffers = txq->buffers;
  u32 n_to_check;
  u32 n_enqueued, *bi;
  u32 *buffers_to_free = em->per_thread_data[vm->thread_index].buffer_indices;

  /* process completion queue entries and remove buffers for completed requests
   * from the queue */
  offset = next & ctx->mask;
  cd = cqes + offset;
  n_to_check = ctx->n_desc - offset;
  match_phase.phase = 1 & ~(next >> ctx->log2_n_desc);
...
more:
  while (n < n_to_check)
...
#ifdef CLIB_HAVE_VEC256
      while (n + 3 < n_to_check)
	{
	  u32x8 r, v;

	  /* load lower 32-bits of 8 CQEs in 256-bit register */
	  r = u32x8_shuffle2 (*(u32x8u *) cd, *(u32x8u *) cd + 1, 0, 2, 4, 6,
			      8, 10, 12, 14);
	...
	goto done;
      ...
      req_id &= ENA_TX_REQ_ID_MASK;

      buffers_to_free[n++] = buffers[req_id];
  ...
  vlib_buffer_free_no_next (vm, buffers_to_free, n);
  txq->cq_next = next + n;

  /* CQEs can arrive out of order, so we cannot blindly advance SQ tail for
   * number of free slots, instead we check that each slot is completed */
  offset = txq->sq_tail & ctx->mask;
  bi = buffers + offset;
  n_enqueued = txq->sq_head - txq->sq_tail;
  n_to_check = clib_min (n_enqueued, ctx->n_desc - offset);
		...
		ena_tx_desc_t *dp, u32 n_in_chain, ena_tx_desc_t desc)
{
  ...
  /* write descriptor as single 16-bit score */
  dp[0].as_u64x2 = desc.as_u64x2;
  return len;
}

static_always_inline void
ena_txq_copy_sqes (ena_tx_desc_t *d, ena_tx_desc_t *s, u32 count)
{
  u32 req_id_mask = pow2_mask (ENA_MAX_LOG2_TXQ_SIZE);
  ena_tx_desc_t mask = { .phase = 1,
			 .req_id_lo = req_id_mask,
			 .req_id_hi = req_id_mask >> 10 };

  while (count--)
    {
      ena_tx_desc_t t;

      /* preserve phase and lower 13 bits of req_id */
      t.as_u64x2 = d[0].as_u64x2 & mask.as_u64x2;
      /* XOR so it flips phase, other fields are already zero */
      t.as_u64x2 ^= s++[0].as_u64x2;
      /* write descriptor as single 16-bit score */
      d++[0] = t;
    }
}
	     ...
	     int use_iova)
{
  ...
  if (n)
      ...
      else
	{
	  ...
	  ena_txq_copy_sqes (txq->sqes, d, n_copy);
	}
