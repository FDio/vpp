	       ...
	       void *sqe_data, u8 sqe_data_sz, void *cqe_data, u8 cqe_data_sz)
{
  ...
  if (ed->initialized && my_process_index != ena_process_node.index)
    {
      /* to avoid rtace only ENA process node is allowed to operate on AQ
       * after device is initialized, others need to submit request by
       * sending ENA_PROCESS_EVENT_ADMIN_REQ signal */
      uword *event_data = 0;
      ena_admin_req_t req = { .calling_process_index = my_process_index,
			      .ed = ed,
			      .opcode = opcode,
			      .sqe_data = sqe_data,
      ...
      vlib_process_signal_event_pointer (vm, ena_process_node.index,
					 ENA_PROCESS_EVENT_ADMIN_REQ, &req);
      vlib_process_wait_for_event_or_clock (vm, 5.0);
      if (vlib_process_get_events (vm, &event_data) != 0)
	req.err = vnet_error (
	  VNET_ERR_BUG,
	  "ena_admin_req failed, ena-process node failed to respond");
      vec_free (event_data);
      return req.err;
    }
...
void
ena_admin_req_process_callback (vlib_main_t *vm, void *p)
{
  ena_admin_req_t *req = p;

  req->err = ena_admin_req (vm, req->ed, req->opcode, req->sqe_data,
			    req->sqe_data_sz, req->cqe_data, req->cqe_data_sz);
		     ...
		     ena_admin_create_sq_resp_t *resp)
{
  ...
  ena_log_debug (ed, "create_sq_cmd_req:%v", format_ena_admin_create_sq_cmd,
		 cmd);
