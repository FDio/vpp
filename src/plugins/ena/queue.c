...
ena_rx_queue_enable (vlib_main_t *vm, ena_device_t *ed, u16 queue_index)
{
  ...
  for (int i = 0; i < ARRAY_LEN (rxq->refill_bitmap); i++)
    rxq->refill_bitmap[i] = ~0ULL;
...
ena_rx_queue_disable (vlib_main_t *vm, ena_device_t *ed, u16 queue_index)
{
  ...
  if (rxq->n_refill < n_desc)
    {
      typeof (rxq->refill_bitmap[0]) *bmp = rxq->refill_bitmap, bit;
      u32 n_free, i = 0, n_bits = sizeof (*bmp) * 8;
      u32 buffers_to_free[n_bits];

      while (i < n_desc)
	{
	  if (bmp++[0])
	    {
	      for (bit = 1, n_free = 0; bit; bit <<= 1, i++)
		if ((bmp[-1] & bit) == 0)
		  buffers_to_free[n_free++] = rxq->buffers[i];

	      vlib_buffer_free (vm, buffers_to_free, n_free);
	    }
	  else
	    i += n_bits;
	}
