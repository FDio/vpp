...
format_ena_device (u8 *s, va_list *args)
{
  ...
  s = format (s, "\n%Utx: queues %u, desc %u (min %u max %u)",
	      format_white_space, indent, ed->n_tx_queues, txq->size, 0, 0);
...
format_ena_admin_feat_desc (u8 *s, va_list *args)
{
  ...
  switch (feat_id)
	...
	s = format (s, " supported_groups %u", d->supported_groups);
	s = format (s, " enabled_groups %u", d->enabled_groups);
      }
      break;
