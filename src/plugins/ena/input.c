			 ...
			 int with_flows)
{
  ...
  if (rxq->cqes[0].phase)
    {
      vlib_buffer_t *b = vlib_get_buffer (vm, rxq->buffers[0]);
      fformat (stderr, "\n%U\n", format_hexdump, rxq->cqes, 16);
      fformat (stderr, "l3 proto %u\n", rxq->cqes[0].l3_proto_idx);
      fformat (stderr, "%U\n", format_hexdump, b->data, rxq->cqes[0].length);
      exit (1);
    }
