  ...
  for (int i = 0; i < vec_len (pv); i++)
    {
      ...
      if ((ad->flags & ENA_DEVICE_F_ADMIN_UP) == 0)
	continue;
      if (PREDICT_FALSE (ad->flags & ENA_DEVICE_F_RX_FLOW_OFFLOAD))
	n_rx +=
	  ena_device_input_inline (vm, node, frame, ad, pv[i].queue_id, 1);
      else
	n_rx +=
	  ena_device_input_inline (vm, node, frame, ad, pv[i].queue_id, 0);
    }
