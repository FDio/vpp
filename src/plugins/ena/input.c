...
ena_device_input_cq_dequeue_no_wrap (ena_rx_cdesc_t *cd, u32 next,
				     ...
				     ena_rx_cdesc_len_and_req_id_t *lri,
				     u16 n_left,
...
static_always_inline void
ena_rx_desc_write (vlib_main_t *vm, ena_rx_desc_t *d, vlib_buffer_t *b,
		   int use_va)
{
  ena_rx_desc_t flip_phase = { .lo.phase = 1 };
  u64x2 r = d->as_u64x2;
  r ^= flip_phase.as_u64x2;
  r[1] = use_va ? vlib_buffer_get_va (b) : vlib_buffer_get_pa (vm, b);
  d->as_u64x2 = r;
}
...
{
  u32 n_desc = 1U << rxq->log2_n_desc;
  u32 n_refill = 0;
  u32 batch_size = sizeof (rxq->refill_bitmap[0]) * 8;

  for (u32 i = 0, j = 0; i < n_desc; i += batch_size, j += 1)
    {
      u32 n, *bi = rxq->buffers + i;
      ena_rx_desc_t *d = rxq->sqes + i;
      typeof (rxq->refill_bitmap[0]) all_one = ~0;

      if (rxq->n_refill < batch_size)
	goto done;

      if (rxq->refill_bitmap[j] != all_one)
	continue;

      n = vlib_buffer_alloc_from_pool (vm, bi, batch_size,
				       rxq->buffer_pool_index);
      if (PREDICT_FALSE (n != batch_size))
	{
	  vlib_buffer_free (vm, bi, n);
	  goto done;
	}

      for (n = batch_size; n > 0; n -= 8, bi += 8, d += 8)
	{
	  vlib_buffer_t *b[8];
	  vlib_get_buffers (vm, bi, b, 8);
	  ena_rx_desc_write (vm, d + 0, b[0], use_va);
	  ena_rx_desc_write (vm, d + 1, b[1], use_va);
	  ena_rx_desc_write (vm, d + 2, b[2], use_va);
	  ena_rx_desc_write (vm, d + 3, b[3], use_va);
	  ena_rx_desc_write (vm, d + 4, b[4], use_va);
	  ena_rx_desc_write (vm, d + 5, b[5], use_va);
	  ena_rx_desc_write (vm, d + 6, b[6], use_va);
	  ena_rx_desc_write (vm, d + 7, b[7], use_va);
	}

      n_refill += batch_size;
      rxq->refill_bitmap[j] = 0;
    }
done:
  if (n_refill)
    {
      rxq->n_refill -= n_refill;
      rxq->sq_next += n_refill;
			 ...
			 int with_flows)
{
  ...
  ena_rx_cdesc_len_and_req_id_t lri[VLIB_FRAME_SIZE], *l;
  ...
  n_deq = ena_device_input_cq_dequeue_no_wrap (rxq->cqes, cq_next, log2_n_desc,
					       statuses, lri, VLIB_FRAME_SIZE,
					       &st_or, &st_and);
  ...
  if (PREDICT_FALSE (((cq_next + n_deq) & mask) == 0))
	...
	lri + n_deq, VLIB_FRAME_SIZE - n_deq, &st_or, &st_and);
      n_deq += n_deq2;
    }
  ...
  for (u32 i = 0; i < n_deq; i++)
    {
      int sz = sizeof (rxq->refill_bitmap[0]) * 8;
      u16 req_id = lri[i].req_id;
      rxq->refill_bitmap[req_id / sz] |= 1ULL << (req_id % sz);
      bi[i] = rxq->buffers[req_id];
    }
  ...
  for (b = buffers, l = lri, f = flags, n_left = n_deq; n_left >= 8;
       b += 4, f += 4, l += 4, n_left -= 4)
    {
      ...
      n_rx_bytes += b[0]->current_length = l[0].length;
      b[0]->flags = f[0];
      vlib_buffer_copy_template (b[1], &bt);
      n_rx_bytes += b[1]->current_length = l[1].length;
      b[1]->flags = f[1];
      vlib_buffer_copy_template (b[2], &bt);
      n_rx_bytes += b[2]->current_length = l[2].length;
      b[2]->flags = f[2];
      vlib_buffer_copy_template (b[3], &bt);
      n_rx_bytes += b[3]->current_length = l[3].length;
      b[3]->flags = f[3];
    }
      ...
      foreach_vlib_frame_bitmap_set_bit_index (i, head_bmp)
	{
	  ...
	  if (vlib_trace_buffer (vm, node, next_index, b, 0))
		...
		vlib_add_trace (vm, node, b, sizeof (*tr));
	      ...
	      tr->n_desc = 1;
	      tr->length = lri[i].length;
	      tr->req_id = lri[i].req_id;
	      tr->status = statuses[i];
	      while (statuses[j].last == 0)
		{
		  j++;
		  tr->n_desc++;
		  tr->length += lri[j].length;
		}
