...
ena_device_input_refill (vlib_main_t *vm, ena_rxq_t *rxq, int use_va)
{
  ...
  u16 *req_ids = ena_rx_queue_get_consumed_req_id_array (rxq);
  ...
  if (PREDICT_FALSE (n_refill))
    clib_memmove (ena_rx_queue_get_consumed_req_id_array (rxq), req_ids,
		  n_refill * sizeof (req_ids[0]));
			 ...
			 int with_flows)
{
  ...
  u16 *req_ids = ena_rx_queue_get_consumed_req_id_array (rxq) + rxq->n_refill;
  ...
  for (u32 i = 0; i < n_deq; i++)
    {
      ...
      enq_buffers[slot] = VLIB_BUFFER_INVALID_INDEX;
    }
