...
ena_device_input_refill (vlib_main_t *vm, ena_rxq_t *rxq, int use_va)
{
  ...
  if (n_refill)
      ...
      while (n_refill >= 8)
	{
      ...
      __atomic_store_n (rxq->sq_db, rxq->sq_next + rxq->n_enq,
			__ATOMIC_RELEASE);
    }
}
			 ...
			 int with_flows)
{
  ...
  u32 n_trace, n_deq, n_deq2 = 0, first, n_left;
  u32 next = rxq->sq_next;
  u8 log2_n_desc = rxq->log2_n_desc;
  ...
  if (PREDICT_FALSE (n_deq2))
    {
      u32 n_deq1 = n_deq - n_deq2;
      vlib_buffer_copy_indices (bi, rxq->buffers + first, n_deq1);
      vlib_buffer_copy_indices (bi + n_deq1, rxq->buffers, n_deq2);
    }
  else
    vlib_buffer_copy_indices (bi, rxq->buffers + first, n_deq);
