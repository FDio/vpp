			 ...
			 int with_flows)
{
  ...
  u32 flags[VLIB_FRAME_SIZE + 8];
  uword n_rx_packets = 0, n_rx_bytes = 0;
  ena_rxq_t *rxq = pool_elt_at_index (ed->rxqs, qid);
  vlib_frame_bitmap_t head_buf_bmp = {};
  ena_rx_cdesc_status_t st_or = {}, st_and = {};
  u32 n_deq = 0;
  u32 next = rxq->next;
  u8 log2_n_desc = rxq->log2_n_desc;
  ...
  n_deq = ena_device_input_cq_dequeue_no_wrap (
    rxq->cqes, next, log2_n_desc, statuses, lengths, VLIB_FRAME_SIZE, &st_or,
    &st_and);
  ...
  if (PREDICT_FALSE (((next + n_deq) & mask) == 0))
	...
	lengths + n_deq, VLIB_FRAME_SIZE - n_deq, &st_or, &st_and);
      if (n)
	{
  ...
  vlib_get_buffers (vm, buffer_indices, buffers, n_deq);
  ena_device_input_status_to_flags (statuses, flags, n_deq, head_buf_bmp);
  n_rx_packets = vlib_frame_bitmap_count_set_bits (head_buf_bmp);

  for (u32 i = 0; i < n_deq; i++)
    {
      vlib_buffer_copy_template (buffers[i], &bt);
      buffers[i]->current_length = lengths[i];
      buffers[i]->flags = flags[i];
    }

  if (n_rx_packets < n_deq)
    {
      vlib_buffer_t *hb = 0;
      vlib_frame_bitmap_t tail_buf_bmp = {};
      u32 i, total_len = 0, head_flags = 0, tail_flags = 0;
      /* tail descriptor contains protocol info so we need to combine
       * head and tail buiffer flags */
	...
	{
	  vlib_buffer_t *pb = vlib_get_buffer (vm, buffer_indices[i - 1]);
	  pb->next_buffer = buffer_indices[i];
  ...
  for (u32 i = 0; i < n_deq; i++)
    fformat (stderr, "%u: %U\n", i, format_vnet_buffer, buffers[i]);
