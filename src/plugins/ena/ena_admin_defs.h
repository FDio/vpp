...
#undef _
} __clib_packed ena_admin_opcode_t;
...
#undef _
} __clib_packed ena_admin_compl_status_t;

/* id, versiom, get, set, name, struct */
#define foreach_ena_admin_feature_id                                          \
  _ (1, 0, 1, 0, DEVICE_ATTRIBUTES, ena_admin_feat_device_attr_t)             \
  _ (2, 0, 1, 0, MAX_QUEUES_NUM, ena_admin_feat_max_queue_num_t)              \
  _ (3, 0, 1, 0, HW_HINTS, ena_admin_feat_hw_hints_t)                         \
  _ (4, 0, 1, 1, LLQ, ena_admin_feat_llq_t)                                   \
  _ (5, 0, 1, 0, EXTRA_PROPERTIES_STRINGS,                                    \
     ena_admin_feat_extra_properties_strings_t)                               \
  _ (6, 0, 1, 0, EXTRA_PROPERTIES_FLAGS,                                      \
     ena_admin_feat_extra_properties_flags_t)                                 \
  _ (7, 1, 1, 0, MAX_QUEUES_EXT, ena_admin_feat_max_queue_ext_t)              \
  _ (10, 0, 1, 1, RSS_HASH_FUNCTION, ena_admin_feat_rss_hash_function_t)      \
  _ (11, 0, 1, 0, STATELESS_OFFLOAD_CONFIG,                                   \
     ena_admin_feat_stateless_offload_config_t)                               \
  _ (12, 0, 1, 1, RSS_INDIRECTION_TABLE_CONFIG,                               \
     ena_admin_feat_rss_ind_table_config_t)                                   \
  _ (14, 0, 0, 1, MTU, ena_admin_feat_mtu_t)                                  \
  _ (18, 0, 1, 1, RSS_HASH_INPUT, ena_admin_feat_rss_hash_input_t)            \
  _ (20, 0, 1, 0, INTERRUPT_MODERATION, ena_admin_feat_intr_moder_t)          \
  _ (26, 0, 1, 1, AENQ_CONFIG, ena_admin_feat_aenq_config_t)                  \
  _ (27, 0, 1, 0, LINK_CONFIG, ena_admin_feat_link_config_t)                  \
  _ (28, 0, 0, 1, HOST_ATTR_CONFIG, ena_admin_feat_host_attr_config_t)        \
  _ (29, 0, 1, 1, PHC_CONFIG, ena_admin_feat_phc_config_t)
...
#undef _
} __clib_packed ena_admin_feature_id_t;
...
#undef _
} __clib_packed ena_admin_stats_type_t;
...
#undef _
} __clib_packed ena_admin_stats_scope_t;
...
} ena_admin_feat_rss_hash_input_t;

STATIC_ASSERT_SIZEOF (ena_admin_feat_rss_hash_input_t, 4);

typedef struct
{
  u16 intr_delay_resolution;
  u16 reserved;
} ena_admin_feat_intr_moder_t;

typedef struct
{
  ena_admin_aenq_groups_t supported_groups;
  ena_admin_aenq_groups_t enabled_groups;
} ena_admin_feat_aenq_config_t;

#define foreach_ena_admin_link_types                                          \
  _ (0, 1000, 1G)                                                             \
  _ (1, 2500, 2_5G)                                                           \
  _ (2, 5000, 5G)                                                             \
  _ (3, 10000, 10G)                                                           \
...
#define _(b, v, n) ENA_ADMIN_LINK_TYPE_##n = (1U << b),
  foreach_ena_admin_link_types
#undef _
} ena_admin_link_types_t;

typedef struct
{
  u32 speed;
  ena_admin_link_types_t supported;
  u32 autoneg : 1;
  u32 duplex : 1;
} ena_admin_feat_link_config_t;

STATIC_ASSERT_SIZEOF (ena_admin_feat_link_config_t, 12);
...
typedef struct
{
  u16 cq_idx;
  u16 reserved;
} ena_admin_feat_rss_ind_table_entry_t;
...
typedef struct
{
  ...
  u8 reserved;
  u32 inline_index;
  ena_admin_feat_rss_ind_table_entry_t inline_entry;
} ena_admin_feat_rss_ind_table_config_t;

typedef struct
{
  u32 mtu;
} ena_admin_feat_mtu_t;
...
} ena_admin_feat_max_queue_num_t;
...
} ena_admin_feat_max_queue_ext_t;
...
typedef struct
{
  ...
  u32 selected_func;
  u32 init_val;
} ena_admin_feat_rss_hash_function_t;
...
typedef struct
{
  ...
  u32 debug_area_size;
} ena_admin_feat_host_attr_config_t;
...
} ena_admin_feat_llq_t;

typedef struct
{
  /* feat common */
  u8 flags;
  ena_admin_feature_id_t feature_id;
  u8 feature_version;
  u8 _reserved;
} ena_admin_get_set_feature_common_desc_t;

STATIC_ASSERT_SIZEOF (ena_admin_get_set_feature_common_desc_t, 4);

typedef struct
{
  ena_admin_aq_ctrl_buff_info_t control_buffer;
  ena_admin_stats_type_t type;
  ena_admin_stats_scope_t scope;
  u16 _reserved3;
  u16 queue_idx;
  u16 device_id;
...
#define foreach_ena_admin_basic_counter                                       \
...
#define foreach_ena_admin_eni_counter                                         \
...
typedef struct
{
  /* common desc */
  u16 command;
  ena_admin_compl_status_t status;
  union
  {
    struct
    {
