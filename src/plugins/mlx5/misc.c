...
clib_error_t *
mlx5_mempage_alloc (mlx5_device_t * md, void **va)
{
  ...
  mem = vlib_physmem_alloc_aligned (vm, mm->physmem_region, &error, 4096 * 64,
				    4096);
...
found:
  ...
  mb->bitmap |= 1ULL << slot;
  *va = mb->mem + slot * 4096;
  hash_set (mm->mempage_by_pa, mlx5_physmem_v2p (md, *va), index << 8 | slot);
  return 0;
}
