...
typedef struct
{
  ...
  /* pool of mempage blocks and hash by physical address  */
  mlx5_mempage_block_t *mempage_blocks;
  uword *mempage_by_pa;
...
#define mlx5_log_err(dev, f, ...) \
  vlib_log(VLIB_LOG_LEVEL_ERR, mlx5_main.log_default, "%U: " f, \
	   format_vlib_pci_addr, vlib_pci_get_addr(dev->pci_dev_handle), ## __VA_ARGS__)
...
static inline u64
mlx5_physmem_v2p (uword * p)
{
  vlib_main_t *vm = vlib_get_main ();
  mlx5_main_t *mm = &mlx5_main;
  return vlib_physmem_virtual_to_physical (vm, mm->physmem_region, p);
}

clib_error_t *mlx5_mempage_alloc (void **va);
void mlx5_mempage_free_pa (uword pa);
