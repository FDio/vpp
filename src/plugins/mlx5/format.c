...
format_mlx5_device_name (u8 * s, va_list * args)
{
  ...
  s = format (s, "mlx5Ethernet%x/%x/%x/%x",
	      md->pci_device.bus_address.domain,
	      md->pci_device.bus_address.bus,
	      md->pci_device.bus_address.slot,
	      md->pci_device.bus_address.function);
  return s;
}
...
format_mlx5_device (u8 * s, va_list * args)
{
  ...
  vlib_pci_device_t *pd = &md->pci_device;
  ...
  if ((error = mlx5_cmd_query_nic_vport_state (cmdq, &state)))
    goto error;
  ...
  if ((error = mlx5_cmd_access_register (cmdq, MLX5_REG_RW_READ,
  ...
  error = mlx5_cmd_access_register (cmdq, MLX5_REG_RW_READ, MLX5_REG_PDDR, 0,
				    pddr);


  s = format (s, "Mellanox %s", pd->product_name);
  ...
  s = format (s, "\n%U%-35s%U", format_white_space, indent + 2,
	      "pci address", format_vlib_pci_addr, &pd->bus_address);
  s = format (s, "\n%U%-35s%U", format_white_space, indent + 2,
	      "pci speed", format_vlib_pci_link_speed, pd);
  s = format (s, "\n%U%-35s%U", format_white_space, indent + 2,
	      "part number", format_vlib_pci_vpd, pd->vpd_r, "PN");
  s = format (s, "\n%U%-35s%U", format_white_space, indent + 2,
	      "serial number", format_vlib_pci_vpd, pd->vpd_r, "SN");
  s = format (s, "\n%Umodule info:\n%U%U",
	      format_white_space, indent,
	      format_white_space, indent + 2,
...
format_mlx5_counters (u8 * s, va_list * args)
{
  ...
  if ((error =
       mlx5_cmd_access_register (cmdq, MLX5_REG_RW_READ, MLX5_REG_PPCNT, 0,
				 buff)))
    goto error;
...
format_mlx5_hca_cap_cur_max (u8 * s, va_list * args)
{
  ...
  if ((error = mlx5_cmd_query_hca_cap (cmdq, 0, type, max_hca_cap)))
    goto error;

  if ((error = mlx5_cmd_query_hca_cap (cmdq, 1, type, cur_hca_cap)))
    goto error;
