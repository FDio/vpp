		   ...
		   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  vec_foreach (md, mm->devices)
  {
    ...
    error =
      mlx5_cmd_query_flow_counter (cmdq, md->flow_counter_id, 4, 0, data);
    if (error)
      {
		   ...
		   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  vec_foreach (md, mm->devices)
  {
    ...
    error =
      mlx5_cmd_access_register (cmdq, MLX5_REG_RW_READ, MLX5_REG_PDDR, 0,
				pddr);
    mlx5_put_cmdq (cmdq);
			...
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	{
	  vlib_cli_output (vm, "%U\n", format_mlx5_hca_cap_cur_max, cmdq,
      ...
      else if (unformat (line_input, "nic-vport"))
	{
	  ...
	  if ((error = mlx5_cmd_query_nic_vport_context (cmdq, ctx)))
	    {
      ...
      else if (unformat (line_input, "eq"))
	{
	  ...
	  if ((error = mlx5_cmd_query_eq (cmdq, md->eqn, eq_ctx, &bitmask)))
	    {
	      s = format (s, "  Error: %U", format_clib_error, error);
	      clib_error_free (error);
	    }
      ...
      else if (unformat (line_input, "txq %u", &qid))
	{
	  ...
	  if ((error = mlx5_cmd_query_cq (cmdq, txq->cqn, cq_ctx)))
	    {
	      s = format (s, "  Error: %U", format_clib_error, error);
	      clib_error_free (error);
	    }
	  ...
	  if ((error = mlx5_cmd_query_sq (cmdq, txq->sqn, sq_ctx, wq_ctx)))
	    {
	      s = format (s, "  Error: %U", format_clib_error, error);
	      clib_error_free (error);
	    }
      ...
      else if (unformat (line_input, "rxq %u", &qid))
	{
	  ...
	  s = format (s, "Completion Queue (cqn %u):\n", rxq->cqn);
	  if ((error = mlx5_cmd_query_cq (cmdq, rxq->cqn, cq_ctx)))
	    {
	      s = format (s, "  Error: %U", format_clib_error, error);
	      clib_error_free (error);
	    }
	  ...
	  if ((error = mlx5_cmd_query_rq (cmdq, rxq->rqn, rq_ctx, wq_ctx)))
	    {
	      s = format (s, "  Error: %U", format_clib_error, error);
	      clib_error_free (error);
	    }
	  ...
	  s =
	    format (s, "Queue Counter (counter_set_id %u):\n",
		    rxq->counter_set_id);
	  if ((error =
	       mlx5_cmd_query_q_counter (cmdq, rxq->counter_set_id, 0,
					 &qcnt)))
	    {
	      s = format (s, "  Error: %U", format_clib_error, error);
	      clib_error_free (error);
