...
mlx5_init (vlib_main_t * vm)
{
  ...
  error = vlib_physmem_region_alloc (vm, "mlx5_pool", 256  << 20, 0,
...
static clib_error_t *
mlx5_pci_init (vlib_main_t * vm, vlib_pci_dev_handle_t h)
{
  clib_error_t *error = 0;
  mlx5_main_t *mm = &mlx5_main;
  mlx5_device_t *md;
  ...
  if ((error = vlib_pci_map_resource (h, 0, (void *) &md->hca)))
    goto error;
