...
mlx5_txq_init (vlib_main_t * vm, mlx5_device_t * md, int queue_id)
{
  ...
  if ((error = mlx5_cmd_create_cq (md, cmdq, &txq->cqn, cq_ctx, txq->cq_mem, 1)))
    goto error;
  ...
  if ((error = mlx5_cmd_create_sq (md, cmdq, &txq->sqn, sq_ctx, wq_ctx,
				   txq->sq_mem, num_pages)))
    goto error;

  if ((error = mlx5_cmd_modify_sq (md, cmdq, txq->sqn, 1)))
    goto error;
...
mlx5_rxq_init (vlib_main_t * vm, mlx5_device_t * md, int queue_id)
{
  ...
  for (i = 0; i < n_slots; i++)
    {
      ...
      pa = vlib_get_buffer_data_physical_address (vm, rxq->enq[i]);
  ...
  *rxq->rq_db = clib_host_to_net_u32 (n_slots);
  ...
  if ((error = mlx5_cmd_create_cq (md, cmdq, &rxq->cqn, cq_ctx, rxq->cq_mem, 1)))
    goto error;

  if ((error = mlx5_cmd_alloc_q_counter (md, cmdq, &rxq->counter_set_id)))
    goto error;
  ...
  if ((error = mlx5_cmd_create_rq (md, cmdq, &rxq->rqn, rq_ctx, wq_ctx,
				   rxq->rq_mem, 1)))
    goto error;

  if ((error = mlx5_cmd_modify_rq (md, cmdq, rxq->rqn, 1)))
    goto error;
...
mlx5_device_init (vlib_main_t * vm, mlx5_device_t * md)
{
  ...
  if ((error = mlx5_cmd_alloc_transport_domain (md, cmdq, &md->transport_domain)))
    goto error;

  if ((error = mlx5_cmd_query_special_contexts (md, cmdq, &md->reserved_lkey, 0)))
    goto error;
  ...
  if ((error = mlx5_cmd_create_flow_group (md, cmdq, MLX5_FLOW_TABLE_TYPE_NIC_RX,
...
mlx5_delete_if (vlib_main_t * vm, mlx5_device_t * md)
{
  ...
  vlib_pci_device_close (md->pci_dev_handle);
...
mlx5_create_if (vlib_main_t * vm, mlx5_create_if_args_t * args)
{
  ...
  if ((error = vlib_pci_device_open (&args->addr, mlx5_pci_device_ids, &h)))
    {
      pool_put (mm->devices, md);
  ...
  if (mm->physmem_region_alloc == 0)
    {
      u32 flags = VLIB_PHYSMEM_F_INIT_MHEAP | VLIB_PHYSMEM_F_HUGETLB;
      error = vlib_physmem_region_alloc (vm, "mlx5", 4 << 20, 0,
					 flags, &mm->physmem_region);
      if (error)
	goto error;
      mm->physmem_region_alloc = 1;
    }

  vlib_pci_set_private_data (h, md->dev_instance);

  if ((error = vlib_pci_bus_master_enable (h)))
    goto error;

  if ((error = vlib_pci_map_region (h, 0, (void *) &md->hca)))
    goto error;
  ...
  if (r & (1 << 31))
    {
      error = clib_error_return (0, "device %U not ready",
				 format_vlib_pci_addr, vlib_pci_get_addr (h));
      goto error;
    }
  ...
  /* alloc cmdq */
  md->cmdq_mem = vlib_physmem_alloc_aligned (vm, mm->physmem_region, &error,
					     64 * (1 << log_cmdq_size),
  ...
  mlx5_set_u64 ((void *) md->hca, 0x10, mlx5_physmem_v2p (md, (void *) md->cmdq_mem));
