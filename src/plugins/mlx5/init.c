...
mlx5_device_init (vlib_main_t * vm, mlx5_device_t * md)
{
  ...
  if ((error = vlib_pci_enable_msix_irq (vm, md->pci_dev_handle, 0, 1)))
    goto error;
...
static void
mlx5_irq_0_handler (vlib_main_t * vm, vlib_pci_dev_handle_t h, u16 line)
{
  mlx5_main_t *mm = &mlx5_main;
  uword pd = vlib_pci_get_private_data (vm, h);
  mlx5_device_t *md = pool_elt_at_index (mm->devices, pd);

  clib_warning ("int %u", md->dev_instance);
}
