...
static void *
mlx5_cmdq_mbox_init (vlib_main_t * vm, int len, u8 * data)
{
  ...
  physmem =
    vlib_physmem_alloc_aligned (vm, mm->physmem_region, &error,
  ...
  while (len > 0)
    {
      ...
      u64 pa = vlib_physmem_virtual_to_physical (vm, mm->physmem_region,
						 (void *) mbox);
...
static clib_error_t *
mlx5_cmdq_sendmsg (mlx5_cmdq_t * cmdq)
{
  mlx5_main_t *mm = &mlx5_main;
  ...
  if (vec_len (cmdq->in) > 16)
    {
      mbox_in = mlx5_cmdq_mbox_init (vm, vec_len (cmdq->in), cmdq->in);
      ...
      if (mbox_in == 0)
	{
      ...
      pa = vlib_physmem_virtual_to_physical (vm, mm->physmem_region, mbox_in);
      /* set input mailbox pointer[63:9] */
      mlx5_set_u64 (p, 0x08, pa);
    }

  if (vec_len (cmdq->out) > 16)
    {
      mbox_out = mlx5_cmdq_mbox_init (vm, vec_len (cmdq->out), 0);
      ...
      if (mbox_out == 0)
	{
      ...
      pa =
	vlib_physmem_virtual_to_physical (vm, mm->physmem_region, mbox_out);
      /* set output mailbox pointer[63:9] */
      mlx5_set_u64 (p, 0x30, pa);
    }
...
static void
mlx5_cmdq_prepare (mlx5_cmdq_t * cmdq, int in_len, int out_len, u16 opcode,
		   u16 op_mod)
{
...
clib_error_t *
mlx5_cmd_query_hca_cap (mlx5_cmdq_t * cmdq, int is_current, u8 type,
			u8 * data)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 8, MLX5_HCA_CAP_SZ + 0x10, MLX5_CMD_QUERY_HCA_CAP,
		     (type << 1) | (is_current != 0));

  if ((error = mlx5_cmdq_sendmsg (cmdq)))
    return error;
...
clib_error_t *
mlx5_cmd_init_hca (mlx5_cmdq_t * cmdq)
{
  mlx5_cmdq_prepare (cmdq, 8, 12, MLX5_CMD_INIT_HCA, 0);
  return mlx5_cmdq_sendmsg (cmdq);
}

clib_error_t *
mlx5_cmd_enable_hca (mlx5_cmdq_t * cmdq)
{
  mlx5_cmdq_prepare (cmdq, 8, 12, MLX5_CMD_ENABLE_HCA, 0);
  return mlx5_cmdq_sendmsg (cmdq);
}

clib_error_t *
mlx5_cmd_query_pages (mlx5_cmdq_t * cmdq, u16 type, u32 * num_pages)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 16, MLX5_CMD_QUERY_PAGES, type);
  ...
  DBG ("type = %u num_pages = %d", type, mlx5_get_u32 (cmdq->out, 0x0c));

  return 0;
}

clib_error_t *
mlx5_cmd_manage_pages (mlx5_cmdq_t * cmdq, i32 num_pages)
{
  mlx5_main_t *mm = &mlx5_main;
  vlib_main_t *vm = vlib_get_main ();
  clib_error_t *error = 0;
  ...
  mlx5_cmdq_prepare (cmdq, 16 + num_alloc * 8, 16 + num_free * 8,
  ...
  for (i = 0; i < num_alloc; i++)
    {
      ...
      error = mlx5_mempage_alloc (&page);

      if (error)
	return error;

      u64 pa =
	vlib_physmem_virtual_to_physical (vm, mm->physmem_region, page);
      mlx5_set_u64 (cmdq->in, 0x10 + i * 8, pa);
    }
  ...
  DBG ("num_pages = %u output_num_entries = %u totol_pages = %u", num_pages,
       output_num_entries, hash_elts (*pages));

  return error;
}

clib_error_t *
mlx5_cmd_set_hca_cap (mlx5_cmdq_t * cmdq, u8 type, u8 * data)
{
  mlx5_cmdq_prepare (cmdq, 0x10 + MLX5_HCA_CAP_SZ, 0x10, MLX5_CMD_SET_HCA_CAP,
		     (type << 1));
...
clib_error_t *
mlx5_cmd_query_issi (mlx5_cmdq_t * cmdq, u16 * current_issi,
		     u32 * supported_issi)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 8, 112, MLX5_CMD_QUERY_ISSI, 0);
...
clib_error_t *
mlx5_cmd_set_issi (mlx5_cmdq_t * cmdq, u16 current_issi)
{
  mlx5_cmdq_prepare (cmdq, 16, 16, MLX5_CMD_SET_ISSI, 0);
...
clib_error_t *
mlx5_cmd_query_special_contexts (mlx5_cmdq_t * cmdq, u32 * resd_lkey,
				 u32 * null_mkey)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 32, MLX5_CMD_QUERY_SPECIAL_CONTEXTS, 0);
...
clib_error_t *
mlx5_cmd_create_eq (mlx5_cmdq_t * cmdq, u8 * eqn, void *ctx, u64 bitmask,
		    void *physmem, int num_pages)
{
  ...
  mlx5_cmdq_prepare (cmdq, 0x110 + num_pages * 8, 16, MLX5_CMD_CREATE_EQ, 0);
  if (ctx)
    memcpy (cmdq->out + 0x10, ctx, MLX5_EQ_CTX_SZ);
  mlx5_set_u64 (cmdq->in, 0x58, bitmask);
  pa = vlib_physmem_virtual_to_physical (vm, mm->physmem_region, physmem);
  mlx5_set_u64 (cmdq->in, 0x110, pa);	/* eq page address */
...
clib_error_t *
mlx5_cmd_query_eq (mlx5_cmdq_t * cmdq, u8 eq_number, u8 * ctx, u64 * bitmask)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 0x118, MLX5_CMD_QUERY_EQ, 0);
...
clib_error_t *
mlx5_cmd_gen_eqe (mlx5_cmdq_t * cmdq, u8 eqn, u8 type, u8 sub_type)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 0x50, 0x10, MLX5_CMD_GEN_EQE, 0);
...
clib_error_t *
mlx5_cmd_create_cq (mlx5_cmdq_t * cmdq, u32 * cqn, void *ctx, void *physmem,
		    int num_pages)
{
  mlx5_main_t *mm = &mlx5_main;
  vlib_main_t *vm = vlib_get_main ();
  ...
  mlx5_cmdq_prepare (cmdq, 0x110 + num_pages * 8, 16, MLX5_CMD_CREATE_CQ, 0);
  memcpy (cmdq->in + 0x10, ctx, MLX5_CQ_CTX_SZ);
  pa = vlib_physmem_virtual_to_physical (vm, mm->physmem_region, physmem);
  mlx5_set_u64 (cmdq->in, 0x110, pa);
...
clib_error_t *
mlx5_cmd_destroy_cq (mlx5_cmdq_t * cmdq, u32 cqn)
{
  mlx5_cmdq_prepare (cmdq, 0x10, 0x10, MLX5_CMD_DESTROY_CQ, 0);
...
clib_error_t *
mlx5_cmd_query_cq (mlx5_cmdq_t * cmdq, u32 cqn, u8 * ctx)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 0x118, MLX5_CMD_QUERY_CQ, 0);
  mlx5_set_bits (cmdq->in, 0x08, 23, 0, cqn);
...
clib_error_t *
mlx5_cmd_query_nic_vport_state (mlx5_cmdq_t * cmdq,
				mlx5_nic_vport_state_t * state)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 16, MLX5_CMD_QUERY_NIC_VPORT_STATE, 0);
...
clib_error_t *
mlx5_cmd_modify_nic_vport_state (mlx5_cmdq_t * cmdq, u8 state)
{
  mlx5_cmdq_prepare (cmdq, 16, 16, MLX5_CMD_MODIFY_NIC_VPORT_STATE, 0);
  mlx5_set_bits (cmdq->in, 0x0c, 7, 4, state);
  return mlx5_cmdq_sendmsg (cmdq);
}

clib_error_t *
mlx5_cmd_query_nic_vport_context (mlx5_cmdq_t * cmdq, u8 * ctx)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, MLX5_NIC_VPORT_CTX_SZ + 16,
...
clib_error_t *
mlx5_cmd_alloc_q_counter (mlx5_cmdq_t * cmdq, u8 * counter_set_id)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 16, MLX5_CMD_ALLOC_Q_COUNTER, 0);
...
clib_error_t *
mlx5_cmd_dealloc_q_counter (mlx5_cmdq_t * cmdq, u8 counter_set_id)
{
  mlx5_cmdq_prepare (cmdq, 16, 16, MLX5_CMD_DEALLOC_Q_COUNTER, 0);
...
clib_error_t *
mlx5_cmd_query_q_counter (mlx5_cmdq_t * cmdq, u8 counter_set_id, int clear,
			  mlx5_q_counter_t * cnt)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 0x20, 0x100, MLX5_CMD_QUERY_Q_COUNTER, 0);
  if (clear)
...
clib_error_t *
mlx5_cmd_alloc_pd (mlx5_cmdq_t * cmdq, u32 * pd)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 16, MLX5_CMD_ALLOC_PD, 0);
...
clib_error_t *
mlx5_cmd_alloc_uar (mlx5_cmdq_t * cmdq, u32 * uar)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 16, MLX5_CMD_ALLOC_UAR, 0);
...
clib_error_t *
mlx5_cmd_access_register (mlx5_cmdq_t * cmdq, mlx5_reg_rw_t rw,
			  u16 register_id, u32 argument, u8 * data)
{
  clib_error_t *error;
  ...
  if (rw == MLX5_REG_RW_READ)
    {
      mlx5_cmdq_prepare (cmdq, data_size + 16, 16 + data_size,
			 MLX5_CMD_ACCESS_REGISTER, 1);
    }
  else
    {
      mlx5_cmdq_prepare (cmdq, 16 + data_size, 16,
...
clib_error_t *
mlx5_cmd_alloc_transport_domain (mlx5_cmdq_t * cmdq, u32 * td)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 16, MLX5_CMD_ALLOC_TRANSPORT_DOMAIN, 0);
...
clib_error_t *
mlx5_cmd_create_tir (mlx5_cmdq_t * cmdq, u32 * tirn, void *ctx)
{
  clib_error_t *error = 0;

  mlx5_cmdq_prepare (cmdq, 0x20 + MLX5_TIR_CTX_SZ, 16, MLX5_CMD_CREATE_TIR,
		     0);
...
clib_error_t *
mlx5_cmd_create_sq (mlx5_cmdq_t * cmdq, u32 * sqn, void *sq_ctx, void *wq_ctx,
		    void *physmem, int num_pages)
{
  mlx5_main_t *mm = &mlx5_main;
  vlib_main_t *vm = vlib_get_main ();
  ...
  mlx5_cmdq_prepare (cmdq, 0x118, 16, MLX5_CMD_CREATE_SQ, 0);
  memcpy (cmdq->in + 0x20, sq_ctx, MLX5_SQ_CTX_SZ);
  memcpy (cmdq->in + 0x50, wq_ctx, MLX5_WQ_CTX_SZ);
  mlx5_set_u64 (cmdq->in, 0x50 + 0xc0,
		vlib_physmem_virtual_to_physical (vm, mm->physmem_region,
						  physmem));
...
clib_error_t *
mlx5_cmd_modify_sq (mlx5_cmdq_t * cmdq, u32 sqn, u8 state)
{
  mlx5_cmdq_prepare (cmdq, 0x118, 0x10, MLX5_CMD_MODIFY_SQ, 0);
...
clib_error_t *
mlx5_cmd_destroy_sq (mlx5_cmdq_t * cmdq, u32 sqn)
{
  mlx5_cmdq_prepare (cmdq, 0x10, 0x10, MLX5_CMD_DESTROY_SQ, 0);
  mlx5_set_bits (cmdq->in, 0x08, 23, 0, sqn);
...
clib_error_t *
mlx5_cmd_query_sq (mlx5_cmdq_t * cmdq, u32 sqn, u8 * sq_ctx, u8 * wq_ctx)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 0x118, MLX5_CMD_QUERY_SQ, 0);
...
clib_error_t *
mlx5_cmd_create_rq (mlx5_cmdq_t * cmdq, u32 * rqn, void *rq_ctx, void *wq_ctx,
		    void *physmem, int num_pages)
{
  mlx5_main_t *mm = &mlx5_main;
  vlib_main_t *vm = vlib_get_main ();
  ...
  mlx5_cmdq_prepare (cmdq, 0x118, 16, MLX5_CMD_CREATE_RQ, 0);
  memcpy (cmdq->in + 0x20, rq_ctx, MLX5_RQ_CTX_SZ);
  memcpy (cmdq->in + 0x50, wq_ctx, MLX5_WQ_CTX_SZ);
  pa = vlib_physmem_virtual_to_physical (vm, mm->physmem_region, physmem);
  mlx5_set_u64 (cmdq->in, 0x50 + 0xc0, pa);
...
clib_error_t *
mlx5_cmd_modify_rq (mlx5_cmdq_t * cmdq, u32 rqn, u8 state)
{
  mlx5_cmdq_prepare (cmdq, 0x118, 0x10, MLX5_CMD_MODIFY_RQ, 0);
...
clib_error_t *
mlx5_cmd_destroy_rq (mlx5_cmdq_t * cmdq, u32 rqn)
{
  mlx5_cmdq_prepare (cmdq, 0x10, 0x10, MLX5_CMD_DESTROY_RQ, 0);
  mlx5_set_bits (cmdq->in, 0x08, 23, 0, rqn);
...
clib_error_t *
mlx5_cmd_query_rq (mlx5_cmdq_t * cmdq, u32 rqn, u8 * rq_ctx, u8 * wq_ctx)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 16, 0x118, MLX5_CMD_QUERY_RQ, 0);
...
clib_error_t *
mlx5_cmd_create_tis (mlx5_cmdq_t * cmdq, u8 prio, u32 td, u32 * tisn)
{
  clib_error_t *error;
  mlx5_cmdq_prepare (cmdq, 0x20 + 0x100, 0x10, MLX5_CMD_CREATE_TIS, 0);
...
clib_error_t *
mlx5_cmd_create_rqt (mlx5_cmdq_t * cmdq, u32 * rqtn, void *ctx)
{
  clib_error_t *error = 0;

  mlx5_cmdq_prepare (cmdq, 0x20 + MLX5_RQT_CTX_SZ, 16, MLX5_CMD_CREATE_RQT,
		     0);
...
clib_error_t *
mlx5_cmd_create_flow_table (mlx5_cmdq_t * cmdq, u8 table_type, void *ctx,
			    u32 * table_id)
{
  clib_error_t *error = 0;

  mlx5_cmdq_prepare (cmdq, 0x10 + MLX5_FLOW_TABLE_CTX_SZ, 16,
...
clib_error_t *
mlx5_cmd_destroy_flow_table (mlx5_cmdq_t * cmdq, u8 table_type, u32 table_id)
{
  mlx5_cmdq_prepare (cmdq, 0x40, 0x10, MLX5_CMD_DESTROY_FLOW_TABLE, 0);
  mlx5_set_bits (cmdq->in, 0x10, 31, 24, table_type);
...
clib_error_t *
mlx5_cmd_set_flow_table_root (mlx5_cmdq_t * cmdq, u8 table_type, u32 table_id)
{
  mlx5_cmdq_prepare (cmdq, 0x40, 0x10, MLX5_CMD_SET_FLOW_TABLE_ROOT, 0);
  mlx5_set_bits (cmdq->in, 0x10, 31, 24, table_type);
  mlx5_set_bits (cmdq->in, 0x14, 23, 0, table_id);
...
clib_error_t *
mlx5_cmd_create_flow_group (mlx5_cmdq_t * cmdq, u8 table_type, u32 table_id,
			    u32 start_flow_index, u32 end_flow_index,
			    u8 match_criteria_enable, u8 * match_criteria,
			    u32 * group_id)
{
  clib_error_t *error = 0;

  mlx5_cmdq_prepare (cmdq, 0x400, 16, MLX5_CMD_CREATE_FLOW_GROUP, 0);
  mlx5_set_bits (cmdq->in, 0x10, 31, 24, table_type);
  mlx5_set_bits (cmdq->in, 0x14, 23, 0, table_id);
...
clib_error_t *
mlx5_cmd_set_flow_table_entry (mlx5_cmdq_t * cmdq, u8 table_type,
			       u32 table_id, u8 modify_enable_mask,
			       u32 flow_index, u8 * ctx)
{
  mlx5_cmdq_prepare (cmdq, 0x40 + MLX5_FLOW_CTX_SZ, 16,
...
clib_error_t *
mlx5_cmd_alloc_flow_counter (mlx5_cmdq_t * cmdq, u16 * flow_counter_id)
{
  clib_error_t *error = 0;

  mlx5_cmdq_prepare (cmdq, 0x10, 0x10, MLX5_CMD_ALLOC_FLOW_COUNTER, 0);
...
clib_error_t *
mlx5_cmd_dealloc_flow_counter (mlx5_cmdq_t * cmdq, u16 flow_counter_id)
{
  mlx5_cmdq_prepare (cmdq, 0x10, 0x10, MLX5_CMD_DEALLOC_FLOW_COUNTER, 0);
...
clib_error_t *
mlx5_cmd_query_flow_counter (mlx5_cmdq_t * cmdq, u16 flow_counter_id,
			     u16 num_of_counters, int clear, u8 * counters)
{
  clib_error_t *error = 0;

  mlx5_cmdq_prepare (cmdq, 0x20, 0x10 + 0x10 * num_of_counters,
