...
connect_to_vpp (void)
{
  ...
  if (vapi_connect_from_vpp (ctx, "apifromplugin", 64, 32,
			     VAPI_MODE_BLOCKING, true) != VAPI_OK)
    {
...
static void *
api_show_version_blocking2_fn (void *args)
{
  if (!connected)
    {
      if (connect_to_vpp () != 0)
  ...
  printf ("--- Basic show version2 message - reply test ---\n");
...
// Wait for one keepalive message
//  vapi_dispatch_one (ctx);
 
  vapi_disconnect_from_vpp (ctx);
  connected = false;
  clib_warning ("Existing thread");
  vapi_ctx_free (ctx);
		     ...
		     vlib_cli_command_t *cmd)
{
  ...
  pthread_t thread;
  int rv = pthread_create (&thread, NULL, api_show_version_blocking2_fn, 0);
  if (rv)
    {
