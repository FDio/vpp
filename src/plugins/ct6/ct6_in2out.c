...
static inline ct6_session_t *
create_or_recycle_session (ct6_main_t * cmp,
			   clib_bihash_kv_48_8_t * kvpp, f64 now,
			   u32 my_thread_index, u32 * recyclep, u32 * createp)
{
		   ...
		   int is_trace)
{
  ...
  while (n_left_from > 0)
	...
	{
	  s0 = create_or_recycle_session (cmp, &kvp0, now, my_thread_index,
					  &recycled, &created);
