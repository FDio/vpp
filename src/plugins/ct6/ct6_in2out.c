...
ct6_update_session_hit (ct6_main_t * cmp, ct6_session_t * s0, f64 now)
{
  ...
  /* Remove from LRU listheads, if applicable */
  ...
  /*
   * And re-add at the head of the forward LRU list,
   * tail of the reverse LRU list
   */
  if (cmp->first_index[thread_index] != -1)
    {
      next_sess = pool_elt_at_index (cmp->sessions[thread_index],
  ...
  if (cmp->last_index[thread_index] == -1)
			       ...
			       u32 * createp)
{
  ...
  if (s0->expires < now || vec_len (cmp->sessions[my_thread_index])
  ...
  cmp->first_index[my_thread_index] = s0 - cmp->sessions[my_thread_index];
  if (s0->next_index == ~0)
    cmp->last_index[my_thread_index] = cmp->first_index[my_thread_index];
  else
    {
