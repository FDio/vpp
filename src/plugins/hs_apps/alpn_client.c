  ...
  if (err)
    {
      ...
      vlib_process_signal_event_mt (cm->vlib_main, cm->cli_node_index,
				    AC_CLI_CONNECT_FAILED, 0);
      return -1;
    }

  cm->alpn_proto = tls_get_alpn_selected (s->connection_index);
...
ac_connect ()
{
  ...
  ext_cfg->crypto.alpn_protos[0] = TLS_ALPN_PROTO_HTTP2;
  ext_cfg->crypto.alpn_protos[1] = TLS_ALPN_PROTO_HTTP1;
...
ac_run (vlib_main_t *vm)
{
  ...
  switch (event_type)
    ...
    case AC_CLI_TEST_DONE:
      switch (cm->alpn_proto)
	{
	case TLS_ALPN_PROTO_HTTP2:
	  vlib_cli_output (vm, "ALPN selected h2");
	  break;
	case TLS_ALPN_PROTO_HTTP1:
	  vlib_cli_output (vm, "ALPN selected http/1.1");
	  break;
	case TLS_ALPN_PROTO_NONE:
	  vlib_cli_output (vm, "no ALPN");
	  break;
	default:
	  ASSERT (0);
	  break;
    ...
    case AC_CLI_CONNECT_FAILED:
      error = clib_error_return (0, "connect failed");
      break;
    default:
