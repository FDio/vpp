			       ...
			       session_t *s, session_error_t err)
{
  ...
  vec_foreach (header, hcm->custom_header)
    {
      http_add_header (&hcm->req_headers, (const char *) header->name,
...
hc_tx_callback (session_t *s)
{
  ...
  n_enq = clib_min (svm_fifo_size (s->tx_fifo), to_send);
  rv = svm_fifo_enqueue (s->tx_fifo, n_enq, hcm->data + hcm->data_offset);
	       ...
	       vlib_cli_command_t *cmd)
{
  ...
  u8 *path = 0;
  u8 *file_data;
  http_header_ht_t *new_header = vec_new (http_header_ht_t, 0);
  u8 *name;
  u8 *value;
  int rv;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (line_input, "header %v:%v", &name, &value))
	{
	  ...
	  vec_add (hcm->custom_header, new_header, 1);
	}
