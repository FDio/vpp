			       ...
			       session_t *s, session_error_t err)
{
  ...
  if (hcm->is_file && hcm->req_method == HTTP_REQ_POST)
    {
      http_add_header (
	&hcm->headers, http_header_name_token (HTTP_HEADER_CONTENT_TYPE),
  ...
  hcm->headers_buf = http_serialize_headers (hcm->headers);
  vec_free (hcm->headers);
	       ...
	       vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
			 ...
			 &hcm->custom_value))
	{
	  http_add_header (&hcm->headers, (const char *) hcm->custom_key,
