...
static inline void
hc_msg_set_fifo_offsets (http_msg_t *msg)
{
			       ...
			       session_error_t err)
{
  ...
  if (!wrk->has_common_headers)
	...
	hc_msg_set_fifo_offsets (&wrk->msg);
    }
	     ...
	     http_status_code_t sc)
{
  ...
  if ((rv = http_parse_from_location_header (location_header, &http_location)))
    {
      clib_warning ("location header parse error: %U", format_session_error,
		    rv);
      vlib_process_signal_event_mt (wrk->vlib_main, hcm->cli_node_index,
  ...
  vec_reset_length (hcm->target);
  if (http_location.path_offset == 0 &&
    ...
    {
      vec_add (hcm->target, location_header->base + http_location.path_offset,
  ...
  if ((http_location.uri_type == HTTP_URI_TYPE_RELATIVE) ||
	...
	{
	  hcm->connect_sep.transport_proto = TRANSPORT_PROTO_TLS;
...
hc_get_event (vlib_main_t *vm)
{
  ...
  switch (event_type)
    ...
    case HC_NO_LOCATION_HEADER:
      ...
      break;
    case HC_LOC_PARSE_ERROR:
      err = clib_error_return (0, "error: failed parsing location header");
      break;
	       ...
	       vlib_cli_command_t *cmd)
{
  ...
  if (hcm->use_ptr && hcm->req_method == HTTP_REQ_PUT)
    {
