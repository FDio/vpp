			       ...
			       session_t *s, session_error_t err)
{
  ...
  // if (stats->is_repeat && clock_gettime (CLOCK_REALTIME, &stats->start))
  //   return -1;
  stats->start = clib_time_now (&stats->clib_time);
...
hc_session_disconnect_callback (session_t *s)
{
  ...
  if (hcm->repeat_count && stats->request_count != ~0)
    {
      HTTP_DBG (1, "repeat > 0, skipping disconnect");
      return;
    }
...
hc_rx_callback (session_t *s)
{
  ...
  if (hc_session->to_recv == 0)
	  ...
	  strstr ((char *) hcm->resp_headers, "Connection: close"))
	{
	  hcm->repeat_count = 0;
	  clib_warning (
	    "received 'Connection: close' header, setting repeat_count to 0");
	}
...
repeat:
  ...
  if ((stats->request_count <= hcm->repeat_count) &&
      stats->request_count != ~0)
    {
      send_err =
  ...
  if (stats->is_repeat)
    {
      stats->end = clib_time_now (&stats->clib_time);
      elapsed_time = stats->end - stats->start;
      vlib_cli_output (vm, "< %d request(s) in %d ms", stats->request_count,
		       elapsed_time);
    }
	       ...
	       vlib_cli_command_t *cmd)
{
  ...
  clib_time_init (&stats->clib_time);
