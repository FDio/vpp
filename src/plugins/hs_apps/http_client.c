...
static int
hc_connect_streams (u64 parent_handle, hc_session_t *parent_session)
{
  hc_main_t *hcm = &hc_main;
  ...
  wrk = hc_worker_get (parent_session->thread_index);

  for (i = 0; i < (hcm->max_streams - 1); i++)
    {
      hs = hc_session_alloc (wrk);
      hs->parent_index = parent_session->session_index;
  ...
  if (http_version == HTTP_VERSION_2 && hcm->max_streams > 1)
    {
      ...
      HTTP_DBG (1, "parent connected, going to open %u streams",
		hcm->max_streams - 1);
      if (hc_connect_streams (session_handle (s), hc_session))
	return -1;
    }
