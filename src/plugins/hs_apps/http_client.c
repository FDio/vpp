...
hc_rx_callback (session_t *s)
{
  ...
  if (hc_session->to_recv == 0)
    ...
    http_init_header_table_buf (&ht, msg);
      ...
      clib_memcpy(ht.buf,hc_session->resp_headers,msg.data.headers_len - 1);
      ...
      if (content_type)
        {
          clib_warning(content_type->base);
          if(!clib_strcmp(content_type->base,"text/"))
...
hc_get_event (vlib_main_t *vm)
{
  ...
  switch (event_type)
      ...
      if (hcm->filename)
	{
      ...
      if (hcm->verbose && !hc_session->no_print)
	{
	  wrk = hc_worker_get (hcm->worker_index);
	  hc_session = hc_session_get (wrk->session_index, wrk->thread_index);
