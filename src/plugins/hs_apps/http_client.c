			       ...
			       session_t *s, session_error_t err)
{
  ...
  if (hcm->is_file && hcm->req_method == HTTP_REQ_POST)
    {
      http_add_header (
	&hcm->req_headers, http_header_name_token (HTTP_HEADER_CONTENT_TYPE),
  ...
  hcm->headers_buf = http_serialize_headers (hcm->req_headers);
  vec_free (hcm->req_headers);
  ...
  rv = svm_fifo_enqueue (s->tx_fifo, vec_len (hcm->data), hcm->data);
  if (rv != vec_len (hcm->data))
    {
...
hc_rx_callback (session_t *s)
{
  ...
  if (msg.data.headers_len)
	...
	{
	  clib_warning ("no headers received");
	  return -1;
	}
...
hc_run (vlib_main_t *vm)
{
  ...
  switch (event_type)
    ...
    case HC_REPLY_RECEIVED:
      if (hcm->filename)
	{
	  file_ptr = fopen ((char *) hcm->filename, "w");
	  if (file_ptr == NULL)
	    {
	      vlib_cli_output (vm, "couldn't open file %s", hcm->filename);
	    }
...
hc_cleanup ()
{
  ...
  vec_free (hcm->custom_key);
  vec_free (hcm->custom_value);
}
	       ...
	       vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "header %v:%v", &hcm->custom_key,
			 &hcm->custom_value))
	{
	  http_add_header (&hcm->req_headers, (const char *) hcm->custom_key,
			   vec_len (hcm->custom_key),
			   (const char *) hcm->custom_value,
			   vec_len (hcm->custom_value));
	}
      else
	{
