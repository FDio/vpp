...
int
echo_segment_attach_mq (uword segment_handle, uword mq_offset, svm_msg_q_t **mq,
                        u8 is_vpp)
{
  echo_main_t *em = &echo_main;
  fifo_segment_t *fs;
  ...
  if (is_vpp)
    {
      *mq = clib_mem_alloc (sizeof(svm_msg_q_t));
      memset(*mq, 0, sizeof(svm_msg_q_t));
    }
  ...
  fs = fifo_segment_get_segment (&em->segment_main, fs_index);
  if (is_vpp)
    ssvm_msg_q_attach (&fs->ssvm, mq_offset, *mq);
  else
    *mq = fifo_segment_msg_q_attach (fs, mq_offset);
...
vl_api_app_attach_reply_t_handler (vl_api_app_attach_reply_t * mp)
{
  ...
  if (mp->n_fds)
	...
	if (echo_segment_attach (ECHO_MQ_SEG_HANDLE, 0, SSVM_SEGMENT_MEMFD,
      ...
      echo_segment_attach_mq (ECHO_MQ_SEG_HANDLE, mp->vpp_ctrl_mq, &em->ctrl_mq, 1);
      ...
      if (mp->fd_flags & SESSION_FD_F_MEMFD_SEGMENT)
	{
      ...
      echo_segment_attach_mq (segment_handle, mp->app_mq, &em->app_mq, 0);
