...
#ifndef __included_vpp_echo_bapi_c__
#define __included_vpp_echo_bapi_c__
...
void
echo_disconnect_session (u64 handle, u32 opaque)
{
  echo_main_t *em = &echo_main;
...
vl_api_unbind_uri_reply_t_handler (vl_api_unbind_uri_reply_t * mp)
{
  ...
  em->proto_cb_vft->cleanup_cb (listen_session);
}
...
{
  echo_main_t *em = &echo_main;
  if (mp->retval && (em->proto_cb_vft->connect_failed_cb))
    em->proto_cb_vft->connect_failed_cb (mp->context);
}
