...
static void
quic_echo_cleanup_cb (echo_session_t * s)
{
  echo_main_t *em = &echo_main;
  quic_echo_proto_main_t *eqm = &quic_echo_proto_main;
  echo_session_t *ls;
...
static void
quic_echo_connected_cb (session_connected_msg_t * mp,
			echo_session_t * session)
{
  echo_main_t *em = &echo_main;
  quic_echo_proto_main_t *eqm = &quic_echo_proto_main;
...
quic_echo_disconnected_reply_cb (echo_session_t * s)
{
  if (s->session_type == ECHO_SESSION_TYPE_STREAM)
    ...
    quic_echo_cleanup_cb (s);	/* We can clean Q/Lsessions right away */
}
...
static void
quic_echo_parent_died_cb (echo_session_t * s)
{
  echo_main_t *em = &echo_main;
  quic_echo_proto_main_t *eqm = &quic_echo_proto_main;
  clib_atomic_sub_fetch (&em->n_clients_connected, 1);
...
static void
quic_echo_connect_failed_cb (u32 context)
{
  /* retry connect */
  echo_session_t *session;
  echo_main_t *em = &echo_main;
...
echo_proto_cb_vft_t quic_echo_proto_cb_vft = {
  ...
  .connect_failed_cb = quic_echo_connect_failed_cb,
  ...
  .cleanup_parent_died_cb = quic_echo_parent_died_cb,
