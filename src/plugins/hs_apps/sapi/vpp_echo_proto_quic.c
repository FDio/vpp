...
} echo_proto_quic_main_t;

echo_proto_quic_main_t echo_proto_quic_main;
...
echo_on_connected_connect (session_connected_msg_t * mp, u32 session_index)
{
  ...
  echo_proto_quic_main_t *eqm = &echo_proto_quic_main;
...
static void
echo_cleanup_cb (echo_session_t * s)
{
  echo_main_t *em = &echo_main;
  echo_proto_quic_main_t *eqm = &echo_proto_quic_main;
  echo_session_t *ls;
    ...
    if (s->session_type == ECHO_SESSION_TYPE_QUIC)
      {
        ...
        if (eqm->send_quic_disconnects == ECHO_CLOSE_F_ACTIVE)
          {
            echo_send_rpc (em, echo_disconnect_session, (void *) s->vpp_session_handle, 0);
            clib_atomic_fetch_add (&em->stats.active_count.q, 1);
          }
        else if (eqm->send_quic_disconnects == ECHO_CLOSE_F_NONE)
          {
...
static void
echo_connected_cb (session_connected_msg_t * mp, echo_session_t * session)
{
  echo_main_t *em = &echo_main;
  echo_proto_quic_main_t *eqm = &echo_proto_quic_main;
...
static void
echo_accepted_cb (session_accepted_msg_t * mp, echo_session_t * session)
{
  echo_main_t *em = &echo_main;
  echo_proto_quic_main_t *eqm = &echo_proto_quic_main;
  echo_session_t *ls;
...
static void
echo_disconnected_reply_cb (echo_session_t * s)
{
  if (s->session_type == ECHO_SESSION_TYPE_STREAM)
    s->session_state = ECHO_SESSION_STATE_CLOSING;
  else
    echo_cleanup_cb (s);	/* We can clean Q/Lsessions right away */
}

static void
echo_disconnected_cb (session_disconnected_msg_t * mp, echo_session_t * s)
{
  echo_main_t *em = &echo_main;
  if (s->session_type == ECHO_SESSION_TYPE_STREAM)
    {
...
static void
echo_reset_cb (session_reset_msg_t * mp, echo_session_t * s)
{
  echo_main_t *em = &echo_main;
  if (s->session_type == ECHO_SESSION_TYPE_STREAM)
    {
...
static int
echo_process_opts_cb (unformat_input_t * a)
{
  echo_main_t *em = &echo_main;
  echo_proto_quic_main_t *eqm = &echo_proto_quic_main;
...
static void
echo_set_defaults_before_opts_cb ()
{
  echo_proto_quic_main_t *eqm = &echo_proto_quic_main;
...
static void
echo_set_defaults_after_opts_cb ()
{
  echo_proto_quic_main_t *eqm = &echo_proto_quic_main;
  echo_main_t *em = &echo_main;
  u8 default_f_active;
  ...
  em->n_clients = clib_max (1, eqm->n_stream_clients) * em->n_clients;
  em->n_sessions = em->n_clients + eqm->n_stream_clients + 1;
...
static void
echo_parent_died_cb (echo_session_t * s)
{
  echo_main_t *em = &echo_main;
  echo_proto_quic_main_t *eqm = &echo_proto_quic_main;
  clib_atomic_sub_fetch (&em->n_clients_connected, 1);
...
static void
echo_connect_failed_cb (u32 context)
{
  /* retry connect */
  echo_session_t *session;
  echo_main_t *em = &echo_main;
  u8 *uri;
  if (context == SESSION_INVALID_INDEX)
    {
    ...
    {
      session = pool_elt_at_index (em->sessions, context);
      uri = format (0, "QUIC://session/%lu", session->vpp_session_handle);
      ECHO_LOG (1, "Retrying connect %s", uri);
      echo_send_rpc (em, echo_send_connect, (void *) uri, context);
    }
}

echo_proto_cb_vft_t echo_quic_proto_cb_vft = {
  .disconnected_cb = echo_disconnected_cb,
  .connected_cb = echo_connected_cb,
  .connect_failed_cb = echo_connect_failed_cb,
  .accepted_cb = echo_accepted_cb,
  .reset_cb = echo_reset_cb,
  .disconnected_reply_cb = echo_disconnected_reply_cb,
  .cleanup_cb = echo_cleanup_cb,
  .cleanup_parent_died_cb = echo_parent_died_cb,
  .process_opts_cb = echo_process_opts_cb,
  .print_usage_cb = echo_print_usage_cb,
  .set_defaults_before_opts_cb = echo_set_defaults_before_opts_cb,
  .set_defaults_after_opts_cb = echo_set_defaults_after_opts_cb,
};

ECHO_REGISTER_PROTO (TRANSPORT_PROTO_QUIC, echo_quic_proto_cb_vft);
