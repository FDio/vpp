...
#undef _
  ...
  volatile u64
    accepted_session_count; /* sessions we accepted (as a listener) */
} echo_session_t;
...
enum quic_session_state_t
{
  ...
  ECHO_SESSION_STATE_AWAIT_CLOSING, /* Data transfer is done, wait for close
				       evt */
...
{
  void (*connected_cb) (session_connected_bundled_msg_t *mp, u32 session_index,
			u8 is_failed); /* Session is connected */
  void (*accepted_cb) (session_accepted_msg_t *mp,
		       echo_session_t *session); /* Session got accepted */
  void (*bound_uri_cb) (session_bound_msg_t *mp,
			echo_session_t *session); /* Session got bound */
  void (*reset_cb) (session_reset_msg_t *mp,
		    echo_session_t *s); /* Received RESET on session */
  void (*disconnected_cb) (
    session_disconnected_msg_t *mp,
    echo_session_t *s); /* Received DISCONNECT on session */
  void (*sent_disconnect_cb) (
    echo_session_t *s); /* ACK disconnect we sent to vpp */
  void (*cleanup_cb) (echo_session_t *s,
		      u8 parent_died); /* Session should be cleaned up (parent
					  listener may be dead) */
  /* Add CLI options */
...
typedef struct
{
  ...
  uword *session_index_by_vpp_handles;	/* Hash table : quic_echo s_id -> vpp
					   s_handle */
