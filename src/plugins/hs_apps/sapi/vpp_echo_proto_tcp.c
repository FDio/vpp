...
static void
tcp_echo_cleanup_cb (echo_session_t * s)
{
...
static void
tcp_echo_connected_cb (session_connected_msg_t * mp, echo_session_t * session)
{
...
static void
tcp_echo_parent_died_cb (echo_session_t * s)
{
  echo_main_t *em = &echo_main;
  clib_atomic_sub_fetch (&em->n_clients_connected, 1);
...
echo_proto_cb_vft_t echo_tcp_proto_cb_vft = {
  ...
  .cleanup_parent_died_cb = tcp_echo_parent_died_cb,
};
