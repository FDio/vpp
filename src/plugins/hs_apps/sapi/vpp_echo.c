...
#include <hs_apps/sapi/vpp_echo_proto_udp.c>
...
echo_check_closed_listener (echo_main_t * em, echo_session_t * s)
{
  ...
  em->proto_cb_vft->cleanup_parent_died_cb (s);
}
  ...
  if (!s->bytes_to_send && !s->bytes_to_receive)
      ...
      else
	{
	  ...
	  if (em->send_stream_disconnects == ECHO_CLOSE_F_ACTIVE)
	    {
	      echo_send_rpc (em, echo_disconnect_session,
...
session_connected_handler (session_connected_msg_t * mp)
{
  ...
  echo_session_handle_add_del (em, mp->handle, session->session_index);
  em->proto_cb_vft->connected_cb (mp, session);
}
...
print_usage_and_exit (void)
{
  ...
  for (i = 0; i < TRANSPORT_N_PROTO; i++)
    {
      echo_proto_cb_vft_t *vft = ecm->available_proto_cb_vft[i];
...
static void
echo_set_each_proto_defaults_before_opts (echo_common_main_t * ecm)
{
  int i;
  for (i = 0; i < TRANSPORT_N_PROTO; i++)
    {
...
main (int argc, char **argv)
{
  ...
  echo_set_each_proto_defaults_before_opts (ecm);
  echo_process_opts (argc, argv);
  echo_process_uri (em);
  em->proto_cb_vft =
    ecm->available_proto_cb_vft[em->uri_elts.transport_proto];
  if (!em->proto_cb_vft)
    {
