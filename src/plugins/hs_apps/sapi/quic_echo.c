...
#include <hs_apps/sapi/common_echo.c>
...
session_reset_handler (session_reset_msg_t * mp)
{
  ...
  if (session->session_type == QUIC_SESSION_TYPE_STREAM)
    {
      em->stats.reset_count.s++;
      session->session_state = QUIC_SESSION_STATE_CLOSING;
    }
  else
    {
      em->stats.reset_count.q++;
      echo_cleanup_session (em, session);	/* We can clean Q/Lsessions right away */
    }
