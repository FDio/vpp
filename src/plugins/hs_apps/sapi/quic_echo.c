...
#define ECHO_RX_BUF_SIZE (1 << 20)
#define ECHO_TX_BUF_SIZE (1024 * 1024 - 1)
...
echo_assert_test_suceeded (echo_main_t * em)
{
  ...
  CHECK (em->tx_total, em->n_stream_clients * em->n_clients * em->bytes_to_send, "Not enough data sent");
  CHECK (0, hash_elts (em->session_index_by_vpp_handles), "Some sessions are still open");
}
...
echo_thread_fn (void *arg)
{
  ...
  vec_validate (rx_buf, ECHO_RX_BUF_SIZE);
  ...
  while (!em->time_to_stop)
	...
	n_sent = send_data_chunk (s, em->connect_test_data, s->bytes_sent % ECHO_TX_BUF_SIZE, ECHO_TX_BUF_SIZE);
	...
	idle_loop = 0;
      if (idle_loop == 1e7)
	{
	  DBG ("Idle client TX:%dB RX:%dB", s->bytes_to_send, s->bytes_to_receive);
	  DBG ("Idle FIFOs  TX:%dB RX:%dB", svm_fifo_max_dequeue (s->tx_fifo), svm_fifo_max_dequeue (s->rx_fifo));
	}
    }
  DBG ("[%lu/%lu] -> S(%x) -> [%lu/%lu]",
					   ...
					   mp)
{
  ...
  if (mp->n_fds)
	...
	if (ssvm_segment_attach ((char *) mp->segment_name,
	    ...
	    {
	      echo_fail ("svm_fifo_segment_attach ('%s') failed", mp->segment_name);
	      return;
	    }
...
quic_echo_process_opts (int argc, char **argv)
{
  ...
  clib_warning ("RX %lu", em->bytes_to_receive);
  clib_warning ("TX %lu", em->bytes_to_send);
  clib_warning ("data_source %lu", em->data_source);
...
main (int argc, char **argv)
{
  ...
  vec_validate (em->connect_test_data, ECHO_TX_BUF_SIZE);
