...
static void
hcpc_delete_session (session_t *s, u8 is_tun)
{
  hcpc_session_t *ps;

  HCPC_DBG ("session %u (is tun %u)", s->opaque, is_tun);
...
hcpc_start_listen ()
{
  ...
  a->app_index = hcpcm->listener_app_index;
  clib_memcpy (&a->sep_ext, &hcpcm->listener_sep,
	       sizeof (hcpcm->listener_sep));
...
http_rx_callback (session_t *s)
{
  ...
  ps = hcpc_session_get (s->opaque);
  ASSERT (ps);
  if (ps->state == HCPC_SESSION_CONNECTING)
    return hcpc_read_http_connect_resp (s, ps);

  http_tx_fifo = s->rx_fifo;

  if (svm_fifo_set_event (http_tx_fifo))
    session_program_tx_io_evt (http_tx_fifo->vpp_sh, SESSION_IO_EVT_TX);
...
http_alloc_session_fifos (session_t *s)
{
  ...
  if (!ps)
    ...
    {
      HCPC_DBG ("tunnel stream session");
...
static clib_error_t *
hcpc_command_fn (vlib_main_t *vm, unformat_input_t *input,
		 vlib_cli_command_t *cmd)
{
  ...
  if ((rv = parse_uri ((char *) listener_uri, &hcpcm->listener_sep)))
    {
...
VLIB_CLI_COMMAND (hcpc_command, static) = {
  .path = "http connect proxy client",
  .short_help = "server-uri <http[s]://ip:port> listener <tcp|udp://ip:port>",
  .function = hcpc_command_fn,
};
