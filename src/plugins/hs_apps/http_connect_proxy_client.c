...
static void
hcpc_session_accepted (u32 ps_index)
{
  hcpc_main_t *hcpcm = &hcpc_main;
  hcpc_session_t *ps;
  ...
  s = session_get_from_handle_if_valid (ps->listener.session_handle);
  clib_spinlock_unlock_if_init (&hcpc_main.sessions_lock);
  if (!s)
    return;
  ...
  if (old_state >= SESSION_STATE_CLOSING)
    {
...
static void
hcpc_session_accepted_rpc (void *arg)
{
  uword ps_index = pointer_to_uword (arg);
  hcpc_session_accepted (ps_index);
}
...
http_rx_callback (session_t *s)
{
  ...
  if (ps->http_establishing)
      ...
      if (ps->listener_diconnected || hcpc_read_http_connect_resp (s))
	{
      ...
      u32 listener_thread_index =
	session_thread_from_handle (ps->listener.session_handle);
      u32 ps_index = ps->session_index;
      clib_spinlock_unlock_if_init (&hcpcm->sessions_lock);
      if (s->thread_index != listener_thread_index)
	session_send_rpc_evt_to_thread (listener_thread_index,
					hcpc_session_accepted_rpc,
					uword_to_pointer (ps_index, void *));
      else
	hcpc_session_accepted (ps->session_index);
      return 0;
    }
  if (ps->listener_diconnected)
    {
...
listener_accept_callback (session_t *s)
{
  ...
  s->opaque = ps->session_index;
  /* set to ready when we get response to http connect method */
