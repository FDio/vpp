  ...
  if (PREDICT_FALSE (max_enqueue > sizeof (session_dgram_hdr_t) && max_send))
    {
      ...
      svm_fifo_enqueue (f, sizeof (hdr), (u8 *) &hdr);
      svm_fifo_enqueue_nocopy (f, max_enqueue);
      es->dgrams_sent++;
...
{
  u32 test_buf_len, test_buf_offset, bytes_sent;
  svm_fifo_t *f = es->tx_fifo;
  ...
  if (max_enqueue <= (sizeof (session_dgram_hdr_t) + sizeof (u32)))
    return 0;
  max_enqueue -= sizeof (session_dgram_hdr_t);
  max_enqueue = clib_min (max_enqueue, max_send);
  // FIXME:
  max_enqueue = clib_min (max_enqueue, TRANSPORT_PACER_MIN_MSS);
  ...
  rv = app_send_dgram_segs ((app_session_t *) es, data_segs, 2, max_enqueue + sizeof (u32), 0);
  es->dgrams_sent++;
  bytes_sent = clib_max (rv - sizeof (u32), 0);
  es->bytes_sent += bytes_sent;
  return bytes_sent;
}
