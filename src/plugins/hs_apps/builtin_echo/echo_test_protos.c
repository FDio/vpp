...
always_inline int
et_server_rx_inline (echo_test_session_t *es, session_t *s, u8 *rx_buf, u8 test_bytes)
{
  ...
  /* No space in tx fifo */
  if (PREDICT_FALSE (max_enqueue == 0))
  ...
  actual_transfer = svm_fifo_dequeue (rx_fifo, max_transfer, rx_buf);
  ...
  /* Echo back */
  actual_transfer = svm_fifo_enqueue (tx_fifo, max_transfer, rx_buf);
  if (actual_transfer != max_transfer)
...
static int
et_server_rx (echo_test_session_t *es, session_t *s, u8 *rx_buf)
{
  return et_server_rx_inline (es, s, rx_buf, 0);
}

static int
et_tcp_server_rx_test_bytes (echo_test_session_t *es, session_t *s, u8 *rx_buf)
{
  return et_server_rx_inline (es, s, rx_buf, 1);
}

static echo_test_proto_vft_t echo_test_tcp = {
  .listen = et_tcp_listen,
  .server_rx = et_server_rx,
  .server_rx_test_bytes = et_tcp_server_rx_test_bytes,
};
...
always_inline int
et_udp_server_rx_inline (echo_test_session_t *es, session_t *s, u8 *rx_buf, u8 test_bytes)
{
  u32 max_dequeue, max_enqueue, max_transfer;
  int actual_transfer;
  clib_thread_index_t thread_index = s->thread_index;
...
{
  return et_udp_server_rx_inline (es, s, rx_buf, 0);
}

static int
et_udp_server_rx_test_bytes (echo_test_session_t *es, session_t *s, u8 *rx_buf)
{
  return et_udp_server_rx_inline (es, s, rx_buf, 1);
}
