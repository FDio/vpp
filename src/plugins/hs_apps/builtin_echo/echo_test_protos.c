...
et_udp_server_rx_no_echo (echo_test_session_t *es, session_t *s)
{
  svm_fifo_t *rx_fifo = s->rx_fifo;
  session_dgram_pre_hdr_t ph;
  u32 max_deq, left_deq;

  left_deq = max_deq = svm_fifo_max_dequeue_cons (rx_fifo);

  while (left_deq)
    {
      svm_fifo_peek (rx_fifo, 0, sizeof (ph), (u8 *) &ph);
      es->dgrams_received++;
      es->bytes_received += ph.data_length;
      left_deq -= svm_fifo_dequeue_drop (rx_fifo, sizeof (session_dgram_hdr_t) + ph.data_length);
    }

  if (svm_fifo_needs_deq_ntf (rx_fifo, max_deq))
    {
      svm_fifo_clear_deq_ntf (rx_fifo);
      session_program_transport_io_evt (s->handle, SESSION_IO_EVT_RX);
    }
...
always_inline u8
et_http_connect_tcp_read_resp (echo_test_session_t *es, session_t *s, u8 *rx_buf)
{
  http_msg_t msg;
  int rv;
...
echo_test_proto_vft_t et_http_connect_tcp_vft[2] = {
  {
    ...
    .client_rx = et_http_connect_tcp_read_resp,
    .client_rx_test_bytes = et_http_connect_tcp_read_resp,
  },
  {
    .server_rx = et_http_server_rx,
    .server_rx_test_bytes = et_http_server_rx_test_bytes,
