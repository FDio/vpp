...
ec_ctrl_session_rx_callback (session_t *s)
{
  ...
  switch (cfg.cmd)
	...
	case EC_EXITING:
	  /* post test sync */
	  ecm->server_bytes_received = cfg.total_bytes;
	  ecm->server_dgrams_received = cfg.num_reads;
	  signal_evt_to_cli (EC_CLI_CFG_SYNC);
	  break;
...
ec_print_final_stats (vlib_main_t *vm, f64 total_delta)
{
  ...
  if (ecm->cfg.proto == TRANSPORT_PROTO_UDP)
	...
	{
	  dgram_loss =
	    (ecm->tx_total_dgrams ? ((f64) (ecm->tx_total_dgrams - ecm->rx_total_dgrams) /
				     (f64) ecm->tx_total_dgrams * 100.0) :
				    0.0);
	  ec_cli ("sent total %llu datagrams, received total %llu datagrams, lost %llu datagrams "
		  "(%.2f%%)",
	...
	{
	  dgram_loss =
	    (ecm->tx_total_dgrams ? ((f64) (ecm->tx_total_dgrams - ecm->server_dgrams_received) /
				     (f64) ecm->tx_total_dgrams * 100.0) :
				    0.0);
	  ec_cli ("sent total %llu datagrams, lost %llu datagrams (%.2f%%)", ecm->tx_total_dgrams,
		  ecm->tx_total_dgrams - ecm->server_dgrams_received, dgram_loss);
	}
    }
...
ec_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  if (use_default_mode)
	...
	return clib_error_return (0, "failed: can't set fixed tx chunk for a throughput run!");
      ...
      if (ecm->run_time && !ecm->report_interval)
	ecm->report_interval = 1;
    }
