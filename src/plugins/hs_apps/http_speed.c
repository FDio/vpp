...
static hs_session_t *
hs_session_alloc (u32 thread_index)
{
...
static hs_session_t *
hs_session_get (u32 thread_index, u32 hs_index)
{
  hs_main_t *hsm = &hs_main;
...
static void
hs_session_free (hs_session_t *hs)
{
  hs_main_t *hsm = &hs_main;
...
static void
hs_session_tx_zc (hs_session_t *hs, session_t *ts)
{
  u32 to_send, space;
  u64 max_send;
  int rv;
  ...
  max_send = hs->data_len - hs->tx_offset;
...
static void
hs_start_send_data (hs_session_t *hs, http_status_code_t status)
{
  http_msg_t msg;
  session_t *ts;
  int rv;
  ...
  hs_session_tx_zc (hs, ts);
}

static int
try_test_file (hs_session_t *hs, u8 *request)
{
  ...
  clib_warning ("data len %lu", file_size);
  hs->data_len = file_size;

  hs_start_send_data (hs, HTTP_STATUS_OK);
...
static int
hs_ts_rx_callback (session_t *ts)
{
  hs_session_t *hs;
  u8 *request = 0;
  http_msg_t msg;
  ...
  hs = hs_session_get (ts->thread_index, ts->opaque);
  ...
  if (msg.type != HTTP_MSG_REQUEST || msg.method_type != HTTP_REQ_GET)
    {
      hs_start_send_data (hs, HTTP_STATUS_METHOD_NOT_ALLOWED);
      goto done;
    }

  if (!msg.data.len)
    {
      hs_start_send_data (hs, HTTP_STATUS_BAD_REQUEST);
      goto done;
    }
  ...
  if (try_test_file (hs, request))
    hs_start_send_data (hs, HTTP_STATUS_NOT_FOUND);
...
static int
hs_ts_accept_callback (session_t *ts)
{
  hs_session_t *hs;

  hs = hs_session_alloc (ts->thread_index);
...
static int
hs_ts_connected_callback (u32 app_index, u32 api_context, session_t *s,
			   session_error_t err)
{
  clib_warning ("called...");
  return -1;
...
static void
hs_ts_disconnect_callback (session_t *s)
{
  hs_main_t *hsm = &hs_main;
...
static void
hs_ts_reset_callback (session_t *s)
{
  hs_main_t *hsm = &hs_main;
  vnet_disconnect_args_t _a = { 0 }, *a = &_a;
...
static void
hs_ts_cleanup_callback (session_t *s, session_cleanup_ntf_t ntf)
{
  hs_session_t *hs;
  ...
  hs = hs_session_get (s->thread_index, s->opaque);
  if (!hs)
    return;
...
static int
hc_del_segment_callback (u32 client_index, u64 segment_handle)
{
  return 0;
}

static session_cb_vft_t hs_session_cb_vft = {
  .session_accept_callback = hs_ts_accept_callback,
  .session_disconnect_callback = hs_ts_disconnect_callback,
  .session_connected_callback = hs_ts_connected_callback,
  .add_segment_callback = hc_add_segment_callback,
  .del_segment_callback = hc_del_segment_callback,
  .builtin_app_rx_callback = hs_ts_rx_callback,
  .builtin_app_tx_callback = hs_ts_tx_callback,
  .session_reset_callback = hs_ts_reset_callback,
  .session_cleanup_callback = hs_ts_cleanup_callback,
};
