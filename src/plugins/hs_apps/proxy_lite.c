...
proxy_lite_session_alloc (void)
{
  ...
  ASSERT (!pm->sessions_lock || CLIB_SPINLOCK_IS_LOCKED (&pm->sessions_lock));
...
{
  ASSERT (!proxy_lite_main.sessions_lock ||
	  CLIB_SPINLOCK_IS_LOCKED (&proxy_lite_main.sessions_lock));
