...
typedef int (*proxy_lite_hsi_track_session_pair_try_complete_fn) (session_t *s);

typedef enum proxy_lite_session_state_
{
  PROXY_LITE_S_CONNECTING,
  PROXY_LITE_S_PROXYING,
  PROXY_LITE_S_HSI_DRAINING,
...
typedef struct proxy_lite_main_
{
  ...
  proxy_lite_hsi_track_session_pair_try_complete_fn hsi_track_session_pair_try_complete;
...
proxy_lite_hsi_symbols_init (void)
{
  ...
  if (pm->hsi_track_session_pair && pm->hsi_track_session_pair_try_complete)
    return 1;

  pm->hsi_track_session_pair = vlib_get_plugin_symbol ("hsi_plugin.so", "hsi_track_session_pair");
  pm->hsi_track_session_pair_try_complete =
    vlib_get_plugin_symbol ("hsi_plugin.so", "hsi_track_session_pair_try_complete");

  return pm->hsi_track_session_pair && pm->hsi_track_session_pair_try_complete;
}
...
proxy_lite_session_has_pending (session_t *s)
{
  ...
  if (svm_fifo_max_dequeue_cons (s->tx_fifo))
    return 1;
  if (svm_fifo_has_ooo_data (s->rx_fifo))
    return 1;
  return 0;
}
...
static void
proxy_lite_try_complete_hsi (session_t *s)
{
  proxy_lite_main_t *pm = &proxy_lite_main;
  proxy_lite_side_ctx_t *ctx;
  proxy_lite_session_t *ps;
  ...
  if (ps->state == PROXY_LITE_S_HSI_DRAINING && pm->hsi_track_session_pair_try_complete (s) == 1)
    {
      pm->hsi_tracked++;
...
proxy_lite_forward_rx (session_t *s)
{
  ...
  if (state == PROXY_LITE_S_HSI_DRAINING)
    proxy_lite_try_complete_hsi (s);
...
static void
proxy_lite_hsi_offload_rpc (void *arg)
{
  u32 ps_index = pointer_to_uword (arg);
  proxy_lite_main_t *pm = &proxy_lite_main;
  proxy_lite_session_t *ps;
  session_handle_t po_handle, ao_handle;
  ...
  ao_handle = ps->ao_handle;
  clib_spinlock_unlock_if_init (&pm->sessions_lock);

  sh.handle = po_handle;
  po_s = session_get_from_handle_if_valid (sh);
  sh.handle = ao_handle;
  ao_s = session_get_from_handle_safe (sh);
  if (!po_s || !ao_s)
    goto fail;

  if (proxy_lite_session_has_pending (po_s) || proxy_lite_session_has_pending (ao_s))
    {
      clib_spinlock_lock_if_init (&pm->sessions_lock);
      ps = proxy_lite_session_get (ps_index);
      ps->state = PROXY_LITE_S_PROXYING;
      pm->hsi_skipped_pending++;
      pm->proxied++;
      clib_spinlock_unlock_if_init (&pm->sessions_lock);
      proxy_lite_forward_rx (po_s);
      return;
    }

  rv = pm->hsi_track_session_pair (po_s, ao_handle);
  if (rv)
    goto fail;

  clib_spinlock_lock_if_init (&pm->sessions_lock);
  ps = proxy_lite_session_get (ps_index);
  if (pm->hsi_track_session_pair_try_complete (po_s) == 1)
    {
      pm->hsi_tracked++;
      proxy_lite_session_finish_hsi (ps);
    }
    ...
    {
      ps->state = PROXY_LITE_S_HSI_DRAINING;
      pm->hsi_draining++;
    }
  clib_spinlock_unlock_if_init (&pm->sessions_lock);
  return;
...
static int
proxy_lite_start_hsi_offload (proxy_lite_session_t *ps)
{
  session_send_rpc_evt_to_thread_force (ps->po_thread_index, proxy_lite_hsi_offload_rpc,
					uword_to_pointer (ps - proxy_lite_main.sessions, void *));
					   ...
					   session_error_t err)
{
  ...
  if (hsi_offload)
    {
      proxy_lite_start_hsi_offload (ps);
      return 0;
    }
...
proxy_lite_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  if (pm->server_sep.transport_proto != TRANSPORT_PROTO_TCP ||
      pm->client_sep.transport_proto != TRANSPORT_PROTO_TCP)
    {
      error = clib_error_return (0, "proxy-lite currently supports tcp only");
      goto done;
    }
...
VLIB_CLI_COMMAND (proxy_lite_command, static) = {
  .path = "proxy-lite",
  .short_help = "proxy-lite server-uri <tcp://ip/port> client-uri <tcp://ip/port> "
		"[fifo-size <n>[k|m]] [segment-size <n>[k|m]] [hsi-offload]",
...
proxy_lite_show_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  vlib_cli_output (vm, "proxy-lite %s%s", pm->started ? "started" : "stopped",
		   pm->hsi_offload ? " hsi-offload" : "");
  vlib_cli_output (vm, "accepted %lu connected %lu proxied %lu", pm->accepted, pm->connected,
		   pm->proxied);
  vlib_cli_output (vm, "hsi tracked %lu draining %lu failed %lu skipped-pending %lu",
		   pm->hsi_tracked, pm->hsi_draining, pm->hsi_failed, pm->hsi_skipped_pending);
