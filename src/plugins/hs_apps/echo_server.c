...
typedef struct
{
   ...
   * Config params
   */
  u8 no_echo; /**< Don't echo traffic */ // TODO DELETE
  hs_test_cfg_t cfg;
...
static void
es_test_cmd_stop (echo_server_main_t *esm, session_t *s, hs_test_cfg_t *c)
{
  clib_warning ("TODO cleanup all sessions");
}
...
echo_server_rx_ctrl_callback (session_t *s)
{
  ...
  switch (cfg.cmd)
    ...
    case HS_TEST_CMD_SYNC:
      clib_warning ("cmd sync");
      return es_test_cmd_sync (esm, s, &cfg);
      break;
    case HS_TEST_CMD_START:
      clib_warning ("cmd start");
      break;
    case HS_TEST_CMD_STOP:
      clib_warning ("cmd stop");
      es_test_cmd_stop (esm, s, &cfg);
      break;
    default:
