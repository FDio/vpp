...
{
  ec_main_t *ecm = &ec_main;
  f64 time_now, print_delta;
  u64 total_bytes, received_bytes = 0, sent_bytes = 0, dgrams_sent = 0,
		   dgrams_received = 0;
  ec_worker_t *wrk;
  ec_session_t *sess;
  vec_foreach (wrk, ecm->wrk)
  ...
  if (ecm->transport_proto == TRANSPORT_PROTO_UDP)
	...
	ec_cli ("TX Kbytes   RX Kbytes   Throughput (Bps)   "
      ...
      ec_cli ("%-*llu %-*llu %-*lld %llu/%llu", 11, sent_bytes / (1ULL << 10),
  ...
  else
    {
      if (print_header)
	ec_cli ("TX Kbytes   RX Kbytes   Throughput (Bps)");
      ec_cli ("%-*llu %-*llu %-*lld", 11, sent_bytes / (1ULL << 10), 11,
	      received_bytes / (1ULL << 10), 18,
	      flt_round_nearest (
		((f64) (total_bytes - ecm->last_total_bytes)) / print_delta));
      ecm->last_print_time = time_now;
      ecm->last_total_bytes = total_bytes;
