...
ec_connect_rpc (void *args)
{
  ...
  while (ci < n_clients)
      ...
      if (ci - ecm->ready_connections > 128)
	{
      ...
      /* the increment avoids creating api context -1 in quic connected callback
       * which is used for marking echo control session */
      a->api_context = ci + 1;
      if (needs_crypto)
	{
	       ...
	       vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
	  ...
	  ecm->no_return = 1;
      else if (unformat (line_input, "fifo-size %U",
                  unformat_memory_size, &ecm->fifo_size))
	;
