...
#define EC_UDP_RTT_TX_FLAG 1
#define EC_UDP_RTT_RX_FLAG 2
...
receive_data_chunk (ec_worker_t *wrk, ec_session_t *es)
{
  ...
  // temporary, until 43285 won't get merged
  if (ecm->cfg.test_bytes || ecm->echo_bytes)
    {
      n_read =
  ...
  if (n_read > 0)
	...
	{
	  // possibly unneeded, just delta send_rtt with vlib_time_now
	  es->recv_rtt = vlib_time_now (ecm->vlib_main);
	  es->rtt_stat |= EC_UDP_RTT_RX_FLAG;
	  update_rtt_stats (es->recv_rtt - es->send_rtt);
	}
      if (ecm->cfg.verbose)
	{
          ELOG_TYPE_DECLARE (e) =
...
ec_session_disconnect_callback (session_t *s)
{
  ...
  tc = transport_get_connection (TRANSPORT_PROTO_TCP, s->connection_index,
				 s->thread_index);
  tcpc = tcp_get_connection_from_transport (tc);
  clib_warning ("tcp srtt %.08u", tcpc->srtt);
...
parse_config:
  ...
  if (ecm->transport_proto == TRANSPORT_PROTO_TCP ||
	...
	ec_cli ("%.08fs roundtrip", ecm->rtt_stats.min_rtt);
      else
	ec_cli ("%.08fs/%.08fs/%.08fs min/avg/max roundtrip",
		ecm->rtt_stats.min_rtt,
		ecm->rtt_stats.sum_rtt / ecm->rtt_stats.n_sum,
		ecm->rtt_stats.max_rtt);
    }
