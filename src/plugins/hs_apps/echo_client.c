  ...
  if (ecm->transport_proto == TRANSPORT_PROTO_UDP)
      ...
      if (print_header)
	{
	  ...
	  if (ecm->report_interval_total)
	    ec_cli ("Run time (s)  TX Mbytes   RX Mbytes   Throughput (MB/s)  "
		    "Sent/received dgrams");
	  else
	    ec_cli ("Interval (s)  TX Mbytes   RX Mbytes   Throughput (MB/s)  "
		    "Sent/received dgrams");
	}
      if (ecm->report_interval_total)
	ec_cli ("%-*.1f %-*.2f %-*.2f %-*.2f %llu/%llu", 13, interval_end, 11,
		(f64) sent_bytes / (1ULL << 20), 11,
		(f64) received_bytes / (1ULL << 20), 18,
		(f64) total_bytes / (time_now - ecm->test_start_time) /
		  (1ULL << 20),
		dgrams_sent, dgrams_received);
      else
	ec_cli (
	  "%.1f-%-*.1f %-*.2f %-*.2f %-*.2f %llu/%llu", interval_start, 9,
	  interval_end, 11,
	  (f64) (sent_bytes - ecm->last_total_tx_bytes) / (1ULL << 20), 11,
	  (f64) (received_bytes - ecm->last_total_rx_bytes) / (1ULL << 20), 18,
	  ((f64) (total_bytes - last_total_bytes)) / print_delta /
	    (1ULL << 20),
      ...
      if (print_header)
	{
      ...
      if (ecm->report_interval_total)
	ec_cli ("%-*.1f %-*.2f %-*.2f %-*.2f", 13, interval_end, 11,
		(f64) sent_bytes / (1ULL << 20), 11,
		(f64) received_bytes / (1ULL << 20), 18,
		(f64) total_bytes / (time_now - ecm->test_start_time) /
		  (1ULL << 20));
      else
	ec_cli ("%.1f-%-*.1f %-*.2f %-*.2f %-*.2f", interval_start, 9,
		interval_end, 11,
		(f64) (sent_bytes - ecm->last_total_tx_bytes) / (1ULL << 20),
		11, (f64) (received_bytes - ecm->last_total_rx_bytes), 18,
		((f64) (total_bytes - last_total_bytes)) / print_delta /
		  (1ULL << 20));
      if (print_footer)
	ec_cli ("-------------------------------------------------------");
	       ...
	       vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
			 ...
			 &ecm->report_interval))
	;
      else if (unformat (line_input, "report-interval-total %f",
			 &ecm->report_interval))
