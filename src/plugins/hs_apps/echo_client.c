...
ec_print_periodic_stats (vlib_main_t *vm, bool print_header, bool print_footer)
{
  ...
  if (ecm->transport_proto == TRANSPORT_PROTO_UDP)
	  ...
	  tput =
	    format (tput, "%Ub/s", format_base10,
		    flt_round_nearest ((f64) total_bytes /
		      ...
		      8);
	  ec_cli (
	    "%-13.1f %-13U %-10U %-12s %-11.3fms %llu/%llu", interval_end,
	    format_base10, sent_bytes, format_base10, received_bytes, tput,
	    (ecm->report_interval_jitter ? jitter :
					   ecm->rtt_stats.last_rtt * 1000),
	  ...
	  tput =
	    format (tput, "%Ub/s", format_base10,
		    flt_round_nearest ((f64) (total_bytes - last_total_bytes) /
				       print_delta) *
		      8);
	  ec_cli ("%.1f-%-9.1f %-13U %-10U %-12s %-11.3fms %llu/%llu",
      ...
      if (print_header)
	{
      ...
      if (ecm->report_interval_total)
	ec_cli ("%-13.1f %-13U %-10U %-11.3fms %Ub/s", interval_end,
		format_base10, sent_bytes, format_base10, received_bytes,
		rtt * 1000, format_base10,
		flt_round_nearest ((f64) total_bytes /
				   (time_now - ecm->test_start_time)) *
		  8);
      else
	ec_cli ("%.1f-%-9.1f %-13U %-10U %-11.3fms %Ub/s", interval_start,
		interval_end, format_base10,
		sent_bytes - ecm->last_total_tx_bytes, format_base10,
		received_bytes - ecm->last_total_rx_bytes, rtt * 1000,
		format_base10,
		flt_round_nearest (((f64) (total_bytes - last_total_bytes)) /
				   print_delta) *
		  8);
