...
ec_ctrl_send ()
{
  ...
  if (EC_DBG)
    {
      clib_warning ("sending test paramters to the server..");
  ...
  switch (cfg.cmd)
	...
	case EC_STARTING:
	  if ((rx_bytes != sizeof (cfg)) ||
	      !hs_test_cfg_verify (&cfg, &ecm->cfg))
	    {
	...
	default:
	  clib_warning ("unexpected cmd! %d", ecm->run_test);
	}
      break;
