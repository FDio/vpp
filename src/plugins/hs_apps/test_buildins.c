...
static uword
test_buildins_timer_process (vlib_main_t *vm, vlib_node_runtime_t *rt,
			     vlib_frame_t *f)
{
...
VLIB_REGISTER_NODE (test_buildins_timer_process_node) = {
  .function = test_buildins_timer_process,
  .type = VLIB_NODE_TYPE_PROCESS,
  .name = "test-buildins-timer-process",
...
static void
test_buildins_init (vlib_main_t *vm)
{
  tb_main_t *tbm = &tb_main;
  ...
  vlib_node_set_state (vm, test_buildins_timer_process_node.index,
		       VLIB_NODE_STATE_POLLING);
  n = vlib_get_node (vm, test_buildins_timer_process_node.index);
  vlib_start_process (vm, n->runtime_index);
}

static clib_error_t *
test_buildins_enable_command_fn (vlib_main_t *vm, unformat_input_t *input,
				 vlib_cli_command_t *cmd)
{
  test_buildins_init (vm);
  return 0;
}

VLIB_CLI_COMMAND (test_buildins_enable_command, static) = {
  .path = "test-buildins enable",
  .short_help = "test-buildins enable",
  .function = test_buildins_enable_command_fn,
};
