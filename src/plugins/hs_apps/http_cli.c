...
hcs_unlisten ()
{
  ...
  if (value)
    {
      uri_map_t *map = pool_elt_at_index (hcm->uri_map_pool, *value);
  ...
  if (hcm->app_index != (u32) ~0)
      ...
      else if (listener_add == 0)
	{
	  if (hcs_unlisten () != 0)
	    {
	      vec_free (hcm->uri);
	      return clib_error_return (0, "failed to stop listening");
	    }
	  else
	    goto done;
	}
...
hcs_main_init (vlib_main_t *vm)
{
  ...
  default_uri = format (0, "tcp://0.0.0.0/80");
  return 0;
}
