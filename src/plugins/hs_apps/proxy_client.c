...
static int
pc_send_http_connect_udp_req (session_t *s)
{
  ...
  target = format (0, "/.well-known/masque/udp/%U/%u/", format_ip46_address,
		   &pcm->target_sep.ip, pcm->target_sep.is_ip4,
			       ...
			       session_error_t err)
{
  ...
  else if (pcm->target_sep.transport_proto == TRANSPORT_PROTO_UDP)
    return pc_send_http_connect_udp_req (s);
  else
    return -1;
}
