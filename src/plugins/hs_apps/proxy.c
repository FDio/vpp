...
static u32
common_fifo_tuning_callback (session_t * s, svm_fifo_t * f, u32 bytes)
{
  proxy_main_t *pm = &proxy_main;
  ...
  ASSERT (act < SESSION_FIFO_LAST_ACTION_N_ACTIONS);
  ...
  if (act == SESSION_FIFO_LAST_ACTION_ENQUEUED)
	...
	return fifo_in_use;
      ...
      if (seg_usage < pm->high_watermark && fifo_usage > 80)
	return fifo_in_use;
...
proxy_server_attach ()
{
  ...
  a->options[APP_OPTIONS_FIFO_INC_THRESH] = (u64) pm->fifo_inc_thresh;
  a->options[APP_OPTIONS_FIFO_DEC_THRESH] = (u64) pm->fifo_dec_thresh;
...
active_open_attach (void)
{
  ...
  options[APP_OPTIONS_FIFO_INC_THRESH] = (u64) pm->fifo_inc_thresh;
  options[APP_OPTIONS_FIFO_DEC_THRESH] = (u64) pm->fifo_dec_thresh;
				...
				vlib_cli_command_t * cmd)
{
  ...
  pm->low_watermark = 50;
  pm->fifo_inc_thresh = 50;
  pm->fifo_dec_thresh = 95;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	pm->low_watermark = (u8) tmp32;
      else if (unformat (input, "fifo-inc-thresh %d", &tmp32))
	pm->fifo_inc_thresh = (u8) tmp32;
      else if (unformat (input, "fifo-dec-thresh %d", &tmp32))
	pm->fifo_dec_thresh = (u8) tmp32;
...
VLIB_CLI_COMMAND (proxy_create_command, static) =
{
  ...
  .short_help = "test proxy server [server-uri <tcp://ip/port>]"
      ...
      "[low-watermark <nn>][fifo-inc-thresh <nn>][fifo-inc-thresh <nn>]"
      "[rcv-buf-size <nn>][prealloc-fifos <nn>]"
