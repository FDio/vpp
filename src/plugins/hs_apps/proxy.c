...
proxy_session_can_shutdown (session_t *s, transport_proto_t ao_tp)
{
  if (ao_tp != TRANSPORT_PROTO_TCP)
    return 0;
...
proxy_try_close_session (session_t *s, int is_active_open)
{
  ...
  if (is_active_open)
	  ...
	  !ps->ao_disconnected && !ps->active_open_establishing)
	{
	  ...
	  transport_proto_t ao_tp = session_get_transport_proto (ao_s);
			     ...
			     session_ft_action_t act, u32 bytes)
{
  ...
  /* New segments may be created in other threads during this callback, have to
   * get read lock on segment manager. */
  fifo_segment_t *fs = segment_manager_get_segment_w_lock (sm, f->segment_index);
