...
proxy_try_delete_session (session_t * s, u8 is_active_open)
{
  ...
  if (is_active_open)
      ...
      if (ps->po.session_handle == SESSION_INVALID_HANDLE)
	{
	  ...
	  if (session_thread_from_handle (ps->po.session_handle) !=
	      s->thread_index)
	    {
...
proxy_tx_callback (session_t * proxy_s)
{
  ...
  //   /* Force ack on active open side to update rcv wnd. Make sure it's done
  //   on
  //    * the right thread */
...
active_open_tx_callback (session_t * ao_s)
{
  ...
  //   session_send_rpc_evt_to_thread (
  //     session_thread_from_handle (ps->vpp_server_handle), proxy_force_ack,
  //     arg);
