...
typedef struct
{
  ...
  const char *port;
} tcp_test_peer_client_t;

static void
tcp_test_peer_usage (void)
{
  fprintf (stderr, "Usage:\n"
		   "  tcp_test_peer server --listen IP --port PORT --control PATH "
		   "[--rcvbuf BYTES] [--window-clamp BYTES] [--pause-read]\n"
		   "  tcp_test_peer client --control PATH [--connect IP --port PORT]\n"
		   "  tcp_test_peer ctl --control PATH COMMAND\n"
		   "Commands:\n"
...
static int
tcp_test_peer_set_nonblock (int fd)
{
...
static int
tcp_test_peer_make_listener (const char *listen_ip, const char *port)
{
  ...
  if (fd < 0)
    return -1;
  if (tcp_test_peer_set_nonblock (fd))
    {
      close (fd);
      return -1;
    }
...
static int
tcp_test_peer_make_ctl_listener (const char *path)
{
...
static int
tcp_test_peer_ctl_reply (int fd, const char *reply)
{
...
static int
tcp_test_peer_handle_command (tcp_test_peer_server_t *srv, int fd, const char *cmd)
{
  char reply[256];
  ...
  if (!strcmp (cmd, "pause-read"))
    {
      srv->paused = 1;
      return tcp_test_peer_ctl_reply (fd, "ok\n");
    }
  ...
  if (!strcmp (cmd, "stats"))
    {
      ...
      return tcp_test_peer_ctl_reply (fd, reply);
    }

  return tcp_test_peer_ctl_reply (fd, "error=unknown-command\n");
}

static void
tcp_test_peer_handle_ctl (tcp_test_peer_server_t *srv)
{
  char buf[256];
  ssize_t n;
  int fd;
  ...
  if (n > 0)
	...
	n--;
      buf[n] = 0;
      (void) tcp_test_peer_handle_command (srv, fd, buf);
    }
...
static int
tcp_test_peer_client_connect (tcp_test_peer_client_t *cl, const char *connect_ip, const char *port)
{
  struct in_addr addr;
...
static void
tcp_test_peer_client_disconnect (tcp_test_peer_client_t *cl)
{
  if (cl->conn_fd < 0)
    return;
...
static int
tcp_test_peer_client_send (tcp_test_peer_client_t *cl, uint64_t total_bytes)
{
...
static int
tcp_test_peer_client_handle_command (tcp_test_peer_client_t *cl, int fd, char *cmd)
{
  char reply[256];
  ...
  if (!verb)
    return tcp_test_peer_ctl_reply (fd, "error=empty-command\n");
  ...
  if (!strcmp (verb, "connect"))
    {
      ...
      if (arg1 && !arg2)
	return tcp_test_peer_ctl_reply (fd, "error=missing-port\n");

      rv = tcp_test_peer_client_connect (cl, arg1 ? arg1 : cl->connect_ip, arg2 ? arg2 : cl->port);
      if (rv < 0)
	{
  ...
  if (!strcmp (verb, "send"))
    {
      ...
      arg1 = strtok_r (0, " ", &saveptr);
      if (!arg1)
	return tcp_test_peer_ctl_reply (fd, "error=missing-bytes\n");

      bytes = strtoull (arg1, 0, 10);
      rv = tcp_test_peer_client_send (cl, bytes);
      if (rv < 0)
	{
  ...
  if (!strcmp (verb, "close"))
    {
      tcp_test_peer_client_disconnect (cl);
      return tcp_test_peer_ctl_reply (fd, "ok\n");
    }
...
static void
tcp_test_peer_client_handle_ctl (tcp_test_peer_client_t *cl)
{
  char buf[256];
  ssize_t n;
  int fd;
  ...
  if (n > 0)
	...
	n--;
      buf[n] = 0;
      (void) tcp_test_peer_client_handle_command (cl, fd, buf);
    }
...
static void
tcp_test_peer_accept_conn (tcp_test_peer_server_t *srv)
{
  int fd, one = 1;
...
static void
tcp_test_peer_drain_conn (tcp_test_peer_server_t *srv)
{
...
static int
tcp_test_peer_server_run (const char *listen_ip, const char *port, const char *control_path,
			  uint32_t rcvbuf, uint32_t window_clamp, int paused)
{
  struct pollfd pfds[3];
  tcp_test_peer_server_t srv = {
    .listen_fd = -1,
    .conn_fd = -1,
    .ctl_fd = -1,
    .paused = paused,
  ...
  srv.listen_fd = tcp_test_peer_make_listener (listen_ip, port);
  srv.ctl_fd = tcp_test_peer_make_ctl_listener (control_path);
  ...
  fprintf (stderr, "tcp_test_peer: listening on %s:%s control=%s paused=%d\n", listen_ip, port,
	   control_path, paused);
...
static int
tcp_test_peer_client_run (const char *connect_ip, const char *port, const char *control_path)
{
  struct pollfd pfd = {};
  tcp_test_peer_client_t cl = {
    .conn_fd = -1,
    .ctl_fd = -1,
    .running = 1,
    .control_path = control_path,
  ...
  cl.ctl_fd = tcp_test_peer_make_ctl_listener (control_path);
  if (cl.ctl_fd < 0)
    goto fail;

  rv = vppcom_app_create ("tcp_test_peer");
  if (rv)
    goto fail;
  cl.app_created = 1;
  ...
  if (connect_ip || port)
    {
      rv = tcp_test_peer_client_connect (&cl, connect_ip, port);
      if (rv < 0)
	goto fail;
    }

  fprintf (stderr, "tcp_test_peer: client control=%s target=%s:%s connected=%d\n", control_path,
  ...
  while (cl.running)
	...
	tcp_test_peer_client_handle_ctl (&cl);
    }
...
fail:
  if (rv)
    fprintf (stderr, "tcp_test_peer client error: %d\n", rv);
  tcp_test_peer_client_disconnect (&cl);
  if (cl.app_created)
    vppcom_app_destroy ();
  if (cl.ctl_fd >= 0)
...
static int
tcp_test_peer_ctl_run (const char *control_path, const char *cmd)
{
  struct sockaddr_un sun = {};
  char buf[512];
  ssize_t n;
...
main (int argc, char **argv)
{
  ...
  if (!strcmp (argv[1], "server"))
      ...
      if (!port || !control_path)
	{
      ...
      return tcp_test_peer_server_run (listen_ip, port, control_path, rcvbuf, window_clamp, paused);
    }
  ...
  if (!strcmp (argv[1], "client"))
      ...
      if (!control_path || ((connect_ip && !port) || (!connect_ip && port)))
	{
      ...
      return tcp_test_peer_client_run (connect_ip, port, control_path);
    }
  ...
  if (!strcmp (argv[1], "ctl"))
      ...
      if (!control_path || !cmd[0])
	{
      ...
      return tcp_test_peer_ctl_run (control_path, cmd);
    }
