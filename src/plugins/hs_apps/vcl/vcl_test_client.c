...
vtc_connect_test_sessions (vcl_test_client_worker_t * wrk)
{
  ...
  for (i = 0; i < n_test_sessions; i++)
	...
	return rv;
      ...
      //	}
      //
      //      if (vcm->proto == VPPCOM_PROTO_TLS || vcm->proto ==
      //      VPPCOM_PROTO_DTLS)
      //	{
      ...
      //      flags = O_NONBLOCK;
      //      flen = sizeof (flags);
      //      vppcom_session_attr (ts->fd, VPPCOM_ATTR_SET_FLAGS, &flags,
      //      &flen); vtinf ("Test session %d (fd %d) connected.", i, ts->fd);
    }
