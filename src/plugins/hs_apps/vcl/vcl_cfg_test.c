 ...
 *
 * Usage:
 *   Usage: vcl_cfg_test -s <server_ip> [-d <debug_level>] [-n <namespace>] [-t
 * <timeout>]
 *
 * Options:
...
vt_clu_parse_args (vt_clu_main_t *vclum, int argc, char **argv)
{
  ...
  opterr = 0;
  while ((c = getopt (argc, argv, "s:d:n:a:t:")) != -1)
    switch (c)
      {
      case 's':
...
vt_clu_server_test (vt_clu_main_t *vclum)
{
  ...
  vtinf ("Server test starting (timeout: %d seconds)", vclum->timeout_seconds);
  ...
  /* Bind to the endpoint with timeout */
  do
    {
	...
	break;
      ...
      if (elapsed_time >= vclum->timeout_seconds)
	{
	  fprintf (
	    stderr,
	    "\nERROR: vppcom_session_bind() timed out after %d seconds\n",
	    vclum->timeout_seconds);
