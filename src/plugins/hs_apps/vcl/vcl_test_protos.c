...
static void
vt_session_add_srtp_policy (vcl_test_session_t *ts)
{
  transport_endpt_cfg_srtp_t *test_policy;
  uint32_t cfg_size;

  cfg_size = sizeof (*test_policy) + sizeof (test_key);
  test_policy = malloc (cfg_size);
  memset (test_policy, 0, sizeof (*test_policy));

  test_policy->ssrc_type = 1 /* ssrc_specific */;
  test_policy->ssrc_value = 0xcafebabe;
  ...
  vppcom_session_attr (ts->fd, VPPCOM_ATTR_SET_ENDPT_EXT_CFG, test_policy,
		       &cfg_size);
  free (test_policy);
}

static void
vt_srtp_session_init (vcl_test_session_t *ts)
{
  srtp_hdr_t *hdr;

  hdr = malloc (sizeof (srtp_hdr_t));
  ts->opaque = hdr;
...
vt_srtp_read (vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
  ...
  hdr = ts->opaque;
  if (((srtp_hdr_t *) buf)->ssrc != hdr->ssrc)
    {
      vtinf ("changed!");
      hdr->ssrc = ((srtp_hdr_t *) buf)->ssrc;
    }
  return (rx_bytes);
}
