 ...
 *
 * Usage:
 *   Server: vcl_test_cl_udp_cfg -s <server_ip> [-w <num_workers>] [-d <debug_level>] [-n <namespace>]
 *   Client: vcl_test_cl_udp_cfg -c <server_ip> [-w <num_workers>] [-d <debug_level>] [-n <namespace>]
 *
 * Options:
...
typedef struct vtclu_main_
{
  ...
  /* Temporary storage for config values during argument parsing */
  char *temp_namespace_id;
  char *temp_api_socket;
} vt_clu_main_t;
...
//       cfg->vpp_app_socket_api = format (0, "%s%c", socket_path, 0);
      cfg->vpp_app_socket_api = (u8 *)socket_path;
    }
}
...
vt_clu_parse_args (vt_clu_main_t *vclum, int argc, char **argv)
{
  ...
  opterr = 0;
  while ((c = getopt (argc, argv, "s:c:w:d:n:a:")) != -1)
    switch (c)
      {
      case 's':
	...
	vclum->debug_level = atoi (optarg);
	break;
      case 'n':
	/* Store namespace ID for later application */
	vclum->temp_namespace_id = strdup (optarg);
	break;
      case 'a':
	/* Store VPP API socket path for later application */
	vclum->temp_api_socket = strdup (optarg);
	break;
      }
...
main (int argc, char **argv)
{
  ...
  /* Apply stored configuration values after VCL initialization */
  if (vclum->temp_namespace_id)
    {
      vcl_test_set_namespace_id (&vclum->vcl_cfg, vclum->temp_namespace_id);
      free (vclum->temp_namespace_id);
      vclum->temp_namespace_id = NULL;
    }
  
  if (vclum->temp_api_socket)
    {
      vcl_test_set_api_socket (&vclum->vcl_cfg, vclum->temp_api_socket);
//       free (vclum->temp_api_socket);
//       vclum->temp_api_socket = NULL;
    }
  ...
  rv = vppcom_app_create_with_config ("vcl_test_cl_udp_cfg", &vclum->vcl_cfg);
  if (rv)
    {
  ...
  vtinf ("Starting %s with %d worker(s) [using programmatic VCL config]",
