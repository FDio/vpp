...
void
otx2_device_error (otx2_device_t * xd, char *str, int rv)
{
  otx2_log_err ("Interface %U error %d: %s",
		format_otx2_device_name, xd->port_id, rv, rte_strerror (rv));
  xd->errors = clib_error_return (xd->errors, "%s[port:%d, errno:%d]: %s",
				  str, xd->port_id, rv, rte_strerror (rv));
}

void
otx2_device_setup (otx2_device_t * xd)
{
  ...
  if (bitmap)
    {
      otx2_log_warn ("unsupported tx offloads requested on port %u: %U",
		     xd->port_id, format_otx2_tx_offload_caps, bitmap);
  ...
  if (bitmap)
    {
      otx2_log_warn ("unsupported rx offloads requested on port %u: %U",
		     xd->port_id, format_otx2_rx_offload_caps, bitmap);
  ...
  if (rv < 0)
    {
      otx2_device_error (xd, "rte_eth_dev_configure", rv);
      goto error;
    }
  ...
  for (j = 0; j < xd->tx_q_used; j++)
	...
	rv =
	  rte_eth_tx_queue_setup (xd->port_id, j,
				  ...
				  &xd->tx_conf);
      if (rv < 0)
	otx2_device_error (xd, "rte_eth_tx_queue_setup", rv);
    }
  ...
  for (j = 0; j < xd->rx_q_used; j++)
    {
      ...
      struct rte_mempool *mp = otx2_mempool_by_buffer_pool_index[bpidx];
	...
	rv = rte_eth_rx_queue_setup (xd->port_id, j, xd->nb_rx_desc,
      ...
      if (rv < 0)
	otx2_device_error (xd, "rte_eth_rx_queue_setup", rv);
    }
...
error:
  xd->flags |= OTX2_DEVICE_FLAG_PMD_INIT_FAIL;
  sw->flags |= VNET_SW_INTERFACE_FLAG_ERROR;
}

void
otx2_device_start (otx2_device_t * xd)
{
  int rv;
  ...
  if (rv)
    {
      otx2_device_error (xd, "rte_eth_dev_start", rv);
      return;
    }
  ...
  if (rv)
    otx2_device_error (xd, "rte_eth_dev_default_mac_addr_set", rv);
  ...
  otx2_log_info ("Interface %U started",
		 format_otx2_device_name, xd->port_id);
}

void
otx2_device_stop (otx2_device_t * xd)
{
  if (xd->flags & OTX2_DEVICE_FLAG_PMD_INIT_FAIL)
    return;
  ...
  otx2_log_info ("Interface %U stopped",
		 format_otx2_device_name, xd->port_id);
}

always_inline int
otx2_port_state_callback_inline (otx2_portid_t port_id,
				 enum rte_eth_event_type type, void *param)
{
  ...
  if (type != RTE_ETH_EVENT_INTR_LSC)
    {
      otx2_log_info ("Unknown event %d received for port %d", type, port_id);
      return -1;
    }
  ...
  if (link_up)
    otx2_log_info ("Port %d Link Up - speed %u Mbps - %s",
		   port_id, (unsigned) link.link_speed,
		   (link.link_duplex == ETH_LINK_FULL_DUPLEX) ?
		   "full-duplex" : "half-duplex");
  else
    otx2_log_info ("Port %d Link Down\n\n", port_id);

  return 0;
}

int
otx2_port_state_callback (otx2_portid_t port_id,
			  enum rte_eth_event_type type,
			  void *param,
			  void *ret_param __attribute__ ((unused)))
{
  return otx2_port_state_callback_inline (port_id, type, param);
}

/* If this device is PCI return pointer to info, otherwise NULL */
struct rte_pci_device *
otx2_get_pci_device (const struct rte_eth_dev_info *info)
{
