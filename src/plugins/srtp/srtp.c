...
srtp_ctx_half_open_alloc (void)
{
  ...
  pool_get_aligned_will_expand (sm->half_open_ctx_pool, will_expand, 0);
  if (PREDICT_FALSE (will_expand && vlib_num_workers ()))
    {
      clib_rwlock_writer_lock (&sm->half_open_rwlock);
      pool_get (sm->half_open_ctx_pool, ctx);
      ctx_index = ctx - sm->half_open_ctx_pool;
      clib_rwlock_writer_unlock (&sm->half_open_rwlock);
    }
  else
    {
      /* reader lock assumption: only main thread will call pool_get */
      clib_rwlock_reader_lock (&sm->half_open_rwlock);
      pool_get (sm->half_open_ctx_pool, ctx);
      ctx_index = ctx - sm->half_open_ctx_pool;
      clib_rwlock_reader_unlock (&sm->half_open_rwlock);
    }
  clib_memset (ctx, 0, sizeof (*ctx));
  return ctx_index;
}
...
void
srtp_ctx_half_open_free (u32 ho_index)
{
  srtp_main_t *sm = &srtp_main;
  clib_rwlock_writer_lock (&sm->half_open_rwlock);
  pool_put_index (srtp_main.half_open_ctx_pool, ho_index);
  clib_rwlock_writer_unlock (&sm->half_open_rwlock);
}

srtp_ctx_t *
srtp_ctx_half_open_get (u32 ctx_index)
{
  srtp_main_t *sm = &srtp_main;
  clib_rwlock_reader_lock (&sm->half_open_rwlock);
  return pool_elt_at_index (sm->half_open_ctx_pool, ctx_index);
}

void
srtp_ctx_half_open_reader_unlock ()
{
  clib_rwlock_reader_unlock (&srtp_main.half_open_rwlock);
}

u32
srtp_ctx_half_open_index (srtp_ctx_t * ctx)
{
  return (ctx - srtp_main.half_open_ctx_pool);
}
...
void
srtp_notify_app_enqueue (srtp_ctx_t * ctx, session_t * app_session)
{
  app_worker_t *app_wrk;
  app_wrk = app_worker_get_if_valid (app_session->app_wrk_index);
  if (PREDICT_TRUE (app_wrk != 0))
    srtp_add_app_q_evt (app_wrk, app_session);
}
...
srtp_custom_tx_callback (void *session, transport_send_params_t * sp)
{
  ...
  sp->flags = 0;
  ctx = srtp_ctx_get (app_session->connection_index);
...
{
  u32 udp_si, udp_ti, ctx_index, ctx_engine;
  srtp_ctx_t *ctx = va_arg (*args, srtp_ctx_t *);

  session_parse_handle (ctx->srtp_session_handle, &udp_si, &udp_ti);
  srtp_ctx_parse_handle (ctx->srtp_ctx_handle, &ctx_index, &ctx_engine);
...
format_srtp_listener_ctx (u8 * s, va_list * args)
{
  ...
  s = format (s, "[%d:%d][TLS] app_wrk %u udp %d:%d",
	      ctx->c_thread_index, ctx->c_s_index,
...
format_srtp_ctx_state (u8 * s, va_list * args)
{
  ...
  if (us->session_state == SESSION_STATE_LISTENING)
	...
	s = format (s, "%s", "CLOSING");
      else if (srtp_ctx_handshake_is_over (ctx))
...
format_srtp_connection (u8 * s, va_list * args)
{
  ...
  if (verbose)
    {
      s = format (s, "%-" SESSION_CLI_STATE_LEN "U", format_srtp_ctx_state,
		  ctx);
      if (verbose > 1)
	s = format (s, "\n");
    }
...
format_srtp_half_open (u8 * s, va_list * args)
{
  u32 tc_index = va_arg (*args, u32);
  u32 __clib_unused thread_index = va_arg (*args, u32);
  srtp_ctx_t *ctx = srtp_ctx_half_open_get (tc_index);
  s = format (s, "[SRTP] half-open app %u", ctx->parent_app_wrk_index);
  srtp_ctx_half_open_reader_unlock ();
  return s;
}
