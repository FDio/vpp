...
srtp_ctx_init_client (srtp_tc_t *ctx)
{
  ...
  srtp_crypto_policy_set_rtp_default (&srtp_policy.rtp);
  srtp_crypto_policy_set_rtcp_default (&srtp_policy.rtcp);
  srtp_policy.ssrc.type = ctx->policy_cfg.ssrc_type;
  srtp_policy.ssrc.value = ctx->policy_cfg.ssrc_value;
  srtp_policy.key = ctx->key;
  srtp_policy.ekt = NULL;
  srtp_policy.next = NULL;
  srtp_policy.window_size = ctx->policy_cfg.window_size;
  srtp_policy.allow_repeat_tx = ctx->policy_cfg.allow_repeat_tx;

  if (srtp_create (&ctx->srtp_ctx, &srtp_policy) != srtp_err_status_ok)
    {
		...
		transport_send_params_t *sp)
{
  ...
  while (to_deq > 0)
	...
	{
	  clib_warning ("failed to protect %u", rv);
	  ASSERT (0);
	}
...
srtp_session_accept_callback (session_t *us)
{
  ...
  ctx->key = clib_mem_alloc (ctx->policy_cfg.key_len);
  clib_memcpy (ctx->key, lctx->key, ctx->policy_cfg.key_len);
...
srtp_migrate_ctx (void *arg)
{
  ...
  u32 ctx_handle, thread_index, old_thread_index;
...
srtp_session_migrate_callback (session_t *us, session_handle_t new_sh)
{
  ...
  clib_warning ("session about to migrate srtp ctx");
  ctx = srtp_ctx_get_w_thread (us->opaque, us->thread_index);
  ctx->srtp_session_handle = new_sh;
  ctx = srtp_ctx_detach (ctx);

  session_send_rpc_evt_to_thread (new_thread, srtp_migrate_ctx, (void *) ctx);
}
...
srtp_connect (transport_endpoint_cfg_t *tep)
{
  ...
  policy_cfg = (transport_endpt_cfg_srtp_t *) sep->ext_cfg->data;
  ctx->policy_cfg = *policy_cfg;
  ctx->key = clib_mem_alloc (policy_cfg->key_len);
  clib_memcpy (ctx->key, policy_cfg->key, policy_cfg->key_len);
...
srtp_start_listen (u32 app_listener_index, transport_endpoint_t *tep)
{
  ...
  policy_cfg = (transport_endpt_cfg_srtp_t *) sep->ext_cfg->data;
  lctx->policy_cfg = *policy_cfg;
  lctx->key = clib_mem_alloc (policy_cfg->key_len);
  clib_memcpy (lctx->key, policy_cfg->key, policy_cfg->key_len);
