	     ...
	     ip_adjacency_t * adj, vlib_buffer_t * b0, const void *data)
{
  ...
  ip4_header_t *ip4 = vlib_buffer_get_current (b0);
  ip4->length = clib_host_to_net_u16 (vlib_buffer_length_in_chain (vm, b0));
  ip4->dst_address.as_u32 = sixrd_get_addr(t, adj->sub_type.nbr.next_hop.as_u64[0]);
  ip4->checksum = ip4_header_checksum (ip4);
}
...
ip6ip_tunnel_stack (adj_index_t ai, u32 fib_entry_index)
{
  ...
  t = pool_elt_at_index (sm->tunnels, sm->tunnel_index_by_sw_if_index[sw_if_index]);
...
sixrd_tunnel_stack (adj_index_t ai)
{
  ...
  u32 fib_index = 0; // XXX: Put in the right fix_index!
  sixrd_main_t *sm = &sixrd_main;
  ip_adjacency_t *adj = adj_get(ai);
  sixrd_tunnel_t *t;
  u32 sw_if_index = adj->rewrite_header.sw_if_index;
...
sixrd_update_adj (vnet_main_t * vnm, u32 sw_if_index, adj_index_t ai)
{
  ...
  default:
	...
	.fp_addr = { .ip4 = da4, },
      ...
      u32 fib_entry_index = fib_table_entry_special_add (t->fib_index, &pfx,  FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
      /*u32 sibling_index = */fib_entry_child_add (fib_entry_index, FIB_NODE_TYPE_IP6IP_TUNNEL, t->tunnel_index);

      adj_nbr_midchain_update_rewrite (ai, ip6ip_fixup, NULL, ADJ_FLAG_NONE,
				     sixrd_build_rewrite(vnm, sw_if_index, adj_get_link_type(ai), NULL));
      ip6ip_tunnel_stack(ai, fib_entry_index);
      clib_warning("Adding to FIB %U\n", format_ip4_address, &da4);
      break;
    }
  }
}
...
/*
 * Virtual function table registered by IP6IP tunnels
 * for participation in the FIB object graph.
 */
static fib_node_back_walk_rc_t
ip6ip_tunnel_back_walk (fib_node_t * node, fib_node_back_walk_ctx_t * ctx)
{
#if 0
  ip6ip_tunnel_restack (gre_tunnel_from_fib_node (node));
#endif
  clib_warning("Called\n");
...
static fib_node_t *
ip6ip_tunnel_fib_node_get (fib_node_index_t index)
{
#if 0
  gre_tunnel_t *gt;
  gre_main_t *gm;
...
static void ip6ip_tunnel_last_lock_gone (fib_node_t * node) {}

const static fib_node_vft_t ip6ip_vft = {
  .fnv_get = ip6ip_tunnel_fib_node_get,
  .fnv_last_lock = ip6ip_tunnel_last_lock_gone,
  .fnv_back_walk = ip6ip_tunnel_back_walk,
};
...
static clib_error_t *sixrd_init(vlib_main_t *vm) {
  ...
  fib_node_register_type (FIB_NODE_TYPE_IP6IP_TUNNEL, &ip6ip_vft);
