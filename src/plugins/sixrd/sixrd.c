...
sixrd_update_adj (vnet_main_t * vnm, u32 sw_if_index, adj_index_t ai)
{
  ...
  default:
	...
	.fp_addr = { .ip4 = da4, },
      ...
      adj_delegate_t *aed = adj_delegate_find_or_add(adj, ADJ_DELEGATE_6RD);
      ASSERT(aed);
      aed->sixrd_fib_entry_index = fib_entry_index;
...
sixrd_create_tunnel(ip6_address_t *ip6_prefix, u8 ip6_prefix_len,
  ...
  /* Tunnel already configured */
  if (find_tunnel_by_ip4_address(ip4_src)) return VNET_API_ERROR_INVALID_VALUE;
 ...
 */
static void
adj_delegate_last_lock_gone(ip_adjacency_t *adj, adj_delegate_t *aed)
{
  fib_table_entry_delete_index (aed->sixrd_fib_entry_index, FIB_SOURCE_RR);
  adj_delegate_remove(adj, ADJ_DELEGATE_6RD);
}

const static adj_delegate_vft_t adj_delegate_vft = {
  .fnv_last_lock = adj_delegate_last_lock_gone,
};
...
static clib_error_t *sixrd_init(vlib_main_t *vm) {
  ...
  adj_delegate_register_type(ADJ_DELEGATE_6RD, &adj_delegate_vft);
