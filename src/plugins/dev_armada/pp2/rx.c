...
mrvl_pp2_rx_one_if (vlib_main_t *vm, vlib_node_runtime_t *node,
		    ...
		    u32 next_index, struct pp2_ppio_desc **desc_ptrs,
		    u32 n_desc, i32 current_data, i32 len_adj)
{
  ...
  u32 sw_if_index = vnet_dev_port_get_intf_sw_if_index (rxq->port);
		    ...
		    vlib_frame_t *frame, vnet_dev_rx_queue_t *rxq)
{
  ...
  if (mp->is_dsa)
      ...
      for (i = 0; i < n_desc; i++)
	{
	  ...
	  u8 *start = b->data;
	  dsa_src[i] = mv_dsa_tag_read (start + 14).src_port_or_lag;

	  t = mv_dsa_tag_read (start + 14);
	  fformat (stderr, "\n%U\n", format_mv_dsa_tag, &t);
      ...
      while (n_avail)
	{
	  ...
	  u16 first_dsa_src =
	    dsa_src[vlib_frame_bitmap_find_first_set (avail_bmp)];

	  clib_mask_compare_u16 (first_dsa_src, dsa_src, selected_bmp, n_desc);
	  n_sel = vlib_frame_bitmap_count_set_bits (selected_bmp);
	  n_avail -= n_sel;
	  clib_compress_u64 ((uword *) sel_descs, (uword *) mp->desc_ptrs,
			     selected_bmp, n_desc);
	  mrvl_pp2_rx_one_if (
	    vm, node, rxq, mp->dsa_to_sec_if[first_dsa_src]->buffer_template,
	    rxq->next_index, sel_descs, n_sel, 6, -4);
	}
    }
  else
    {
      mrvl_pp2_rx_one_if (vm, node, rxq, rxq->buffer_template, rxq->next_index,
			  mp->desc_ptrs, n_desc, 2, 0);
    }
