		    ...
		    vlib_frame_t *frame, vnet_dev_rx_queue_t *rxq)
{
  ...
  if (mp->is_dsa)
      ...
      for (i = 0; i < n_desc; i++)
	{
	  ...
	  u8 *start = b->data;
	  dsa_srcs[i] = mv_dsa_tag_read (start + 14).src_port_or_lag;
      ...
      while (n_avail)
	{
	  ...
	  u16 dsa_src = dsa_srcs[vlib_frame_bitmap_find_first_set (avail_bmp)];

	  clib_mask_compare_u16 (dsa_src, dsa_srcs, selected_bmp, n_desc);
	  n_sel = vlib_frame_bitmap_count_set_bits (selected_bmp);
	  n_avail -= n_sel;

	  if ((1 << dsa_src) & mp->valid_dsa_src_bitmap)
	    {
	      clib_compress_u64 ((uword *) sel_descs, (uword *) mp->desc_ptrs,
				 selected_bmp, n_desc);
	      mrvl_pp2_rx_one_if (vm, node, rxq,
				  vnet_dev_get_rx_queue_sec_if_rt_data (
				    rxq, mp->dsa_to_sec_if[dsa_src]),
				  sel_descs, n_sel, 6, -4);
	    }
	  else
	    {
