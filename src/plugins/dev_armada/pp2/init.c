...
mvpp2_probe (vlib_main_t *vm, vnet_dev_bus_index_t bus_index, void *dev_info)
{
  ...
  if (vnet_dev_dt_node_is_compatible (di->node, "marvell,armada-7k-pp22"))
...
mvpp2_init (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  vnet_dev_dt_node_t *sc;
  int pp_id = -1;

  if (!vnet_dev_dt_node_is_compatible (dd->node, "marvell,armada-7k-pp22"))
    return VNET_DEV_ERR_NOT_SUPPORTED;

  sc = vnet_dev_dt_deref_node (dd->node, "marvell,system-controller");

  if (sc && vec_len (sc->path) > strlen ("/cpX/"))
    {
      log_notice (dev, "sys cont is '%v'", sc->path);
  ...
  vec_foreach_pointer (cn, dd->node->child_nodes)
    {
      ...
      log_debug (dev, "child node '%v'", cn->path);
      p = vnet_dev_dt_get_node_property_by_name (cn, "port-id");

      if (!vnet_dev_dt_proprerty_is_u32 (p))
	continue;

      ppio_id = vnet_dev_dt_proprerty_get_u32 (p);
