...
static void
getFamily_DROPMON (SFLOWDM *dmt)
{
#define SFLOWDM_FAM_LEN	      sizeof (NET_DM_GENL_NAME)
#define SFLOWDM_FAM_FOOTPRINT NLMSG_ALIGN (SFLOWDM_FAM_LEN)
  char fam_name[SFLOWDM_FAM_FOOTPRINT] = {};
  memcpy (fam_name, NET_DM_GENL_NAME, SFLOWDM_FAM_LEN);
  generic_send (dmt->nl_sock, dmt->id, GENL_ID_CTRL, CTRL_CMD_GETFAMILY,
		CTRL_ATTR_FAMILY_NAME, fam_name, SFLOWDM_FAM_LEN,
		SFLOWDM_FAM_FOOTPRINT, ++dmt->nl_seq);
  dmt->state = SFLOWDM_STATE_WAIT_FAMILY;
}
...
static void
processNetlink_GENERIC (SFLOWDM *dmt, struct nlmsghdr *nlh)
{
  ...
  for (int offset = GENL_HDRLEN; offset < msglen;)
	...
	case CTRL_ATTR_VERSION:
	  dmt->genetlink_version = *(u32 *) attr_datap;
	  break;
	case CTRL_ATTR_FAMILY_ID:
	  dmt->family_id = *(u16 *) attr_datap;
  ...
  if (dmt->family_id && dmt->group_id)
    {
      SFLOW_DBG ("psample state->READY\n");
      dmt->state = SFLOWDM_STATE_READY;
    }
}

// TODO: we can take out the fns for reading DROPMON here
...
static void
processNetlink (SFLOWDM *dmt, struct nlmsghdr *nlh)
{
...
static void
readNetlink_DROPMON (SFLOWDM *dmt, int fd)
{
  uint8_t recv_buf[SFLOWDM_DROPMON_READNL_RCV_BUF];
  int numbytes = recv (fd, recv_buf, sizeof (recv_buf), 0);
  if (numbytes <= 0)
    {
      SFLOW_ERR ("readNetlink_DROPMON returned %d : %s\n", numbytes,
		 strerror (errno));
      return;
    }
...
SFLOWDM_open (SFLOWDM *dmt)
{
  if (dmt->nl_sock == 0)
    {
      dmt->nl_sock = generic_open (dmt->id);
      if (dmt->nl_sock > 0)
	{
	  dmt->state = SFLOWDM_STATE_OPEN;
	  setSendBuffer (dmt->nl_sock, SFLOWDM_DROPMON_READNL_SND_BUF);
	  getFamily_DROPMON (dmt);
	}
    }
  return (dmt->nl_sock > 0);
}
...
SFLOWDM_close (SFLOWDM *dmt)
{
  if (dmt->nl_sock > 0)
	...
	{
	  SFLOW_ERR ("SFLOWDM_close: returned %d : %s\n", err,
		     strerror (errno));
	}
    }
  return false;
...
SFLOWDM_open_step (SFLOWDM *dmt)
{
  switch (dmt->state)
    {
    case SFLOWDM_STATE_INIT:
      SFLOWDM_open (dmt);
      break;
    case SFLOWDM_STATE_OPEN:
      getFamily_DROPMON (dmt);
      break;
    case SFLOWDM_STATE_WAIT_FAMILY:
      readNetlink_DROPMON (dmt, dmt->nl_sock);
      break;
    case SFLOWDM_STATE_READY:
      break;
    }
  return dmt->state;
}
...
bool
SFLOWDMSpec_setAttr (SFLOWDMSpec *spec, int field,
		     void *val, int len)
{
  SFLOWDMAttr *dma = &spec->attr[field];
  if (dma->included)
    return false;
  dma->included = true;
  dma->attr.nla_type = field;
  dma->attr.nla_len = sizeof (dma->attr) + len;
  int len_w_pad = NLMSG_ALIGN (len);
  dma->val.iov_len = len_w_pad;
  dma->val.iov_base = val;
  spec->n_attrs++;
...
int
SFLOWDMSpec_send (SFLOWDM *dmt, SFLOWDMSpec *spec)
{
  ...
  spec->nlh.nlmsg_pid = generic_pid (dmt->id);

  spec->ge.cmd = NET_DM_CMD_PACKET_ALERT;
  spec->ge.version = 0; // NET_DM_CFG_VERSION==0 but no NET_DM_CMD_VERSION
...
#define MAX_IOV_FRAGMENTS (2 * NET_DM_ATTR_MAX) + 2
  ...
  frag++;
  int nn = 0;
  for (u32 ii = 0; ii < NET_DM_ATTR_MAX; ii++)
    {
      SFLOWDMAttr *dma = &spec->attr[ii];
      if (dma->included)
	{
	  nn++;
