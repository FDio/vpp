...
static void
vl_api_sflow_drop_monitoring_t_handler (vl_api_sflow_drop_monitoring_t *mp)
{
  vl_api_sflow_drop_monitoring_reply_t *rmp;
  sflow_main_t *smp = &sflow_main;
  int rv;

  rv = sflow_drop_monitoring (smp, mp->drop_M);

  REPLY_MACRO (VL_API_SFLOW_DROP_MONITORING_REPLY);
}
