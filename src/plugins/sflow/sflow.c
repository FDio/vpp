...
read_worker_fifos (vlib_main_t *vm, sflow_main_t *smp)
{
  ...
  for (; batch < SFLOW_READ_BATCH; batch++)
	   ...
	   thread_index++)
	{
      ...
      if (psample_send == 0 && dropmon_send == 0)
	{
      ...
      else
	{
	  ...
	  vlib_node_increment_counter (smp->vlib_main, sflow_node.index,
				       SFLOW_ERROR_DROPMON_SEND, dropmon_send);
...
sflow_sampling_start (sflow_main_t *smp)
{
  ...
  smp->sflow_usersock.nl.group_id = SFLOW_NETLINK_USERSOCK_MULTICAST;
  /* set up (or reset) sampling context for each thread */
...
sflow_drop_monitoring (sflow_main_t *smp, u32 dropM)
{
  if (dropM != smp->dropM)
    {
      // drop-monitoring changed - tell all active interfaces.
      smp->dropM = dropM;
      sflow_enable_disable_all (smp);
    }
  return 0;
			    ...
			    vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, SFLOW_DIRN_INGRESS_STR))
	sampling_D = SFLOW_DIRN_INGRESS;
      else if (unformat (input, SFLOW_DIRN_EGRESS_STR))
	sampling_D = SFLOW_DIRN_EGRESS;
      else if (unformat (input, SFLOW_DIRN_BOTH_STR))
  ...
  if (sampling_D == ~0)
    return clib_error_return (
      0, "Please specify a sampling direction (%s|%s|%s)...",
      SFLOW_DIRN_INGRESS_STR, SFLOW_DIRN_EGRESS_STR, SFLOW_DIRN_BOTH_STR);
...
sflow_direction_str (u32 direction)
{
  switch (direction)
    ...
    case SFLOW_DIRN_INGRESS:
      return SFLOW_DIRN_INGRESS_STR;
    case SFLOW_DIRN_EGRESS:
      return SFLOW_DIRN_EGRESS_STR;
    case SFLOW_DIRN_BOTH:
      return SFLOW_DIRN_BOTH_STR;
    }
  return SFLOW_DIRN_NONE_STR;
}
...
VLIB_CLI_COMMAND (sflow_direction_command, static) = {
  ...
  .short_help = "sflow direction <" SFLOW_DIRN_INGRESS_STR
		"|" SFLOW_DIRN_EGRESS_STR "|" SFLOW_DIRN_BOTH_STR ">",
