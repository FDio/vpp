...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  for(u32 pkt = n_left_from; pkt > 0; --pkt) {
    clib_warning("sflow_drop_node (pkt=%d)\n", pkt);
    ...
    // buffer rewind - lifted from interface_output.c
    ethernet_header_t *en = vlib_buffer_get_current (bN);
    /*
    ...
    if (bN->flags & VNET_BUFFER_F_L2_HDR_OFFSET_VALID)
      {
	// TODO: should this decide the discard.header_protocol field?
	// We could pull out the 16-bit ethernet protocol type because
	// PSAMPLE actually wants that number, but if the sample is
	// actually an IP header then we'll need to indicate that too.
	// TODO: make sure that routed packets come through here too
	// and don't go straight from ip-output to NIC.
    ...
    sflow_sample_t discard = {
      .sample_type = SFLOW_SAMPLETYPE_DISCARD,
      .input_if_index = if_index,
      .sampled_packet_size =
      bN->current_length + bN->total_length_not_including_first_buffer,
      .header_bytes = hdr,
    ...
    if (PREDICT_FALSE (!sflow_drop_fifo_enqueue (&sfwk->drop_fifo, &discard)))
      sfwk->ddrp++; // drop-sample drops
  }
  
  /* the rest of this is boilerplate code to pass packets on - typically to "drop" */
