...
my_pair_details_cb(struct vapi_ctx_s *ctx,
  ...
  sflow_per_interface_data_t *sfif = (sflow_per_interface_data_t *)callback_ctx;
...
get_lcp_itf_pairs(void *magic) {
  ...
  if((rv = sflow_vapi_connect(vac)) != VAPI_OK) {
    ...
    for(int ii = 1; ii < vec_len(intfs); ii++) {
      ...
      if(sfif
	 && sfif->sflow_enabled) {
	// TODO: if we try non-blocking we might not be able to just pour all the requests in here.
	// Might be better to do them one at a time - e.g. when we poll for counters.
  ...
  // TODO: how to tell if heap-allocated data is stored separately per thread? And if so,
  // how to tell the allocator to GC all data for the thread when it exits?
  return(void *)rv;
}

int
sflow_vapi_read_linux_if_index_numbers(sflow_vapi_client_t *vac, sflow_per_interface_data_t *itfs) {
...
int
sflow_vapi_check_for_linux_if_index_results(sflow_vapi_client_t *vac, sflow_per_interface_data_t *itfs) {
  // request completed?
  // TODO: if we use non-blocking mode do we have to call something here to receive results?
  int req_active = clib_atomic_load_acq_n(&vac->vapi_request_active);
  if(req_active == false
     && vac->vapi_itfs != NULL) {
    ...
    for(int ii = 1; ii < vec_len(vac->vapi_itfs); ii++) {
      sflow_per_interface_data_t *sfif1 = vec_elt_at_index(vac->vapi_itfs, ii);
