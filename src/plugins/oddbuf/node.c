	       ...
	       int is_ip4, int is_trace)
{
  ...
  while (n_left_from > 0)
      ...
      if (1)
	{
	  ...
	  src = b0->data + b0->current_data + b0->current_length - n_to_copy;
	  b0next->current_data = second_chunk_offset;
	  b0next->current_length = n_to_copy;
	  dst = b0next->data + b0next->current_data;

	  for (i = 0; i < n_to_copy; i++)
	    dst[i] = src[i];

	  b0->current_length -= n_to_copy;
	  b0next->current_length = n_to_copy;
