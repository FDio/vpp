		       ...
		       int enable_disable)
{
  ...
  oddbuf_create_periodic_process (omp);
  ...
  vlib_process_signal_event (omp->vlib_main,
			     omp->periodic_node_index,
			     ODDBUF_EVENT_PERIODIC_ENABLE_DISABLE,
...
oddbuf_init (vlib_main_t * vm)
{
  ...
  omp->msg_id_base = vl_msg_api_get_msg_ids
    ((char *) name, VL_MSG_FIRST_AVAILABLE);

  error = oddbuf_plugin_api_hookup (vm);

  /* Add our API messages to the global name_crc hash table */
  setup_message_id_table (omp, &api_main);
...
VLIB_PLUGIN_REGISTER () =
{
  ...
  .description = "oddbuf plugin description goes here",
};
/* *INDENT-ON* */
