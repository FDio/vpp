...
quic_quicly_crypto_context_alloc (u8 thread_index)
{
  ...
  pool_get (wc->crypto_ctx_pool, crctx);
...
quic_quicly_init_crypto_context (crypto_context_t *crctx, quic_ctx_t *ctx)
{
  ...
  if (!qm->vnet_crypto_init)
      ...
      else
	{
	  ...
	  qm->default_crypto_engine = ctx->crypto_engine = CRYPTO_ENGINE_VPP;
  ...
  if (qqm->vnet_crypto_enabled && ctx->crypto_engine == CRYPTO_ENGINE_VPP)
    {
  QUIC_DBG (
    2, "Init crctx: quic_quicly_crypto_engine, crctx_ndx 0x%08lx, thread %d",
  ...
  QUIC_DBG (
    2,
    "Init crctx (quicly_default_crypto_engine): crctx_ndx 0x%08lx, thread %d",
    crctx->ctx_index, ctx->c_thread_index);
...
quic_quicly_crypto_context_acquire (quic_ctx_t *ctx)
{
  ...
  if (!clib_bitmap_get (qqm->available_crypto_engines, ctx->crypto_engine))
    {
      QUIC_DBG (1, "Quic does not support crypto engine %U (%u)",
		format_crypto_engine, ctx->crypto_engine, ctx->crypto_engine);
