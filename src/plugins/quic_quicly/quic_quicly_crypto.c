...
quic_quicly_init_crypto_context (crypto_context_t *crctx, quic_ctx_t *ctx)
{
  ...
  quicly_ctx->max_packets_per_key = qqm->max_packets_per_key;
  ...
  quicly_ctx->transport_params.max_idle_timeout = qqm->connection_timeout;

  if (qqm->default_quic_cc == QUIC_CC_CUBIC)
    quicly_ctx->init_cc = &quicly_cc_cubic_init;
  else if (qqm->default_quic_cc == QUIC_CC_RENO)
  ...
  if (ctx->crypto_engine == CRYPTO_ENGINE_NONE)
    {
      QUIC_DBG (2, "No crypto engine specified, using %d",
		qqm->default_crypto_engine);
      ctx->crypto_engine = qqm->default_crypto_engine;
    }
