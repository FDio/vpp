  ...
  if (PREDICT_FALSE (!qm->vnet_crypto_init))
      ...
      else
	{
	  ...
	  if (quic_quicly_register_cipher_suite (
		...
		CRYPTO_ENGINE_VPP;
	      vec_validate (qqm->per_thread_crypto_keys, qm->num_threads);
	      for (i = 0; i < qm->num_threads; i++)
		{
		  qqm->per_thread_crypto_keys[i] = vnet_crypto_key_add (
...
static uword
quic_quicly_crypto_set_key (crypto_key_t *key, vnet_crypto_op_id_t op_id)
{
  u8 thread_index = vlib_get_thread_index ();
  quic_quicly_main_t *qqm = &quic_quicly_main;
  vnet_crypto_key_t *vnet_key = qqm->per_thread_crypto_keys[thread_index];

  ASSERT (key->algo);
  ASSERT (key->key_len);

  vnet_crypto_key_update (vnet_key, key->algo, key->key, key->key_len);

  return (uword) vnet_crypto_get_active_engine_key_data (
    vnet_key, op_id, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
}
				   ...
				   int coalesced)
{
  ...
  aead_crctx->op.keys =
    quic_quicly_crypto_set_key (&aead_crctx->key, aead_crctx->id);
  ...
  hp_ctx->op.keys = quic_quicly_crypto_set_key (&hp_ctx->key, hp_ctx->id);
