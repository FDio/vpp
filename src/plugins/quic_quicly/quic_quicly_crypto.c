...
{
  quic_quicly_worker_ctx_t *wrk_ctx = (quic_quicly_worker_ctx_t *) quic_quicly_main.qm->wrk_ctx;
  crypto_context_t *crctx;
  clib_bihash_kv_24_8_t kv;
  int i;

  for (i = 0; i < quic_quicly_main.qm->num_threads; i++)
    {
          ...
          if (crctx->ckpair_index == ckpair->cert_key_index)
            {
              ...
              clib_bihash_add_del_24_8 (&wrk_ctx[i].crypto_context_hash, &kv, 0 /* is_add */);
            }
        }
    }
  return 0;
...
quic_quicly_crypto_context_alloc (u8 thread_index)
{
  ...
  pool_get (qqm->wrk_ctx[thread_index].crypto_ctx_pool, crctx);
  clib_memset (crctx, 0, sizeof (*crctx));
  idx = (crctx - qqm->wrk_ctx[thread_index].crypto_ctx_pool);
...
static void
quic_quicly_crypto_context_free_if_needed (crypto_context_t * crctx, u8 thread_index)
{
  quic_quicly_main_t *qqm = &quic_quicly_main;
  clib_bihash_kv_24_8_t kv;
  if (crctx->n_subscribers)
    return;
  quic_quicly_crypto_context_make_key_from_crctx (&kv, crctx);
  clib_bihash_add_del_24_8 (&qqm->wrk_ctx[thread_index].crypto_context_hash,
			    &kv, 0 /* is_add */);
  clib_mem_free (crctx->data);
  pool_put (qqm->wrk_ctx[thread_index].crypto_ctx_pool, crctx);
}
...
quic_quicly_init_crypto_context (crypto_context_t *crctx, quic_ctx_t *ctx)
{
  ...
  if (vec_len (cm->engines) == 0)
  ...
  else
    {
      ...
      vec_validate (qqm->per_thread_crypto_key_indices, qqm->qm->num_threads);
      for (i = 0; i < qqm->qm->num_threads; i++)
      {
         qqm->per_thread_crypto_key_indices[i] =
            vnet_crypto_key_add (vlib_get_main (), VNET_CRYPTO_ALG_AES_256_CTR, empty_key, 32);
      }
    }
  ...
  ptls_ctx->encrypt_ticket = &qqm->qm->session_cache.super;
  clib_memcpy (quicly_ctx, &quicly_spec_context, sizeof (quicly_context_t));

  quicly_ctx->max_packets_per_key = qqm->qm->max_packets_per_key;
  ...
  quicly_ctx->transport_params.max_idle_timeout = qqm->qm->connection_timeout;

  if (qqm->qm->default_quic_cc == QUIC_CC_CUBIC)
    quicly_ctx->init_cc = &quicly_cc_cubic_init;
  else if (qqm->qm->default_quic_cc == QUIC_CC_RENO)
...
;
  ...
  if (ctx->crypto_engine == CRYPTO_ENGINE_NONE)
    {
      QUIC_DBG (2, "No crypto engine specified, using %d",
		qqm->qm->default_crypto_engine);
      ctx->crypto_engine = qqm->qm->default_crypto_engine;
    }
  ...
  if (clib_bihash_search_24_8
      (&qqm->wrk_ctx[ctx->c_thread_index].crypto_context_hash, &kv, &kv) == 0)
    {
  ...
  if (quic_quicly_init_crypto_context (crctx, ctx))
    goto error;
  if (vnet_app_add_cert_key_interest (ctx->ckpair_index, qqm->qm->app_index))
    goto error;
  crctx->n_subscribers++;
  clib_bihash_add_del_24_8 (
    &qqm->wrk_ctx[ctx->c_thread_index].crypto_context_hash, &kv,
    1 /* is_add */);
  return 0;
...
static int
quic_quicly_crypto_setup_cipher (quicly_crypto_engine_t *engine, quicly_conn_t *conn,
...
quic_quicly_get_time (quicly_now_t * self)
{
  ...
  return (int64_t) qqm->wrk_ctx[thread_index].time_now;
}
...
void
quic_quicly_crypto_decrypt_packet (quic_ctx_t *qctx, quic_quicly_rx_packet_ctx_t *pctx)
{
...
static int
quic_quicly_crypto_cipher_setup_crypto (ptls_cipher_context_t *_ctx, int is_enc,
...
static int
quic_quicly_crypto_aes128ctr_setup_crypto (ptls_cipher_context_t *ctx, int is_enc,
				    const void *key)
{
  return quic_quicly_crypto_cipher_setup_crypto (ctx, 1, key, EVP_aes_128_ctr ());
}

static int
quic_quicly_crypto_aes256ctr_setup_crypto (ptls_cipher_context_t *ctx, int is_enc,
				    const void *key)
{
  return quic_quicly_crypto_cipher_setup_crypto (ctx, 1, key, EVP_aes_256_ctr ());
}
...
static int
quic_quicly_crypto_aead_aes128gcm_setup_crypto (ptls_aead_context_t *ctx, int is_enc,
					 const void *key, const void *iv)
{
...
static int
quic_quicly_crypto_aead_aes256gcm_setup_crypto (ptls_aead_context_t *ctx, int is_enc,
					 const void *key, const void *iv)
{
  return quic_quicly_crypto_aead_setup_crypto (ctx, is_enc, key, iv,
					EVP_aes_256_gcm ());
...
ptls_cipher_suite_t *quic_quicly_crypto_cipher_suites[] = {
  &quic_quicly_crypto_aes256gcmsha384, &quic_quicly_crypto_aes128gcmsha256, NULL
};

quicly_crypto_engine_t quic_quicly_crypto_engine = { quic_quicly_crypto_setup_cipher,
					      quic_quicly_crypto_encrypt_packet };
