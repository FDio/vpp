...
{
  quic_quicly_worker_ctx_t *wrk_ctx = quic_quicly_main.wrk_ctx;
  crypto_context_t *crctx;
	  ...
	  if (crctx->ckpair_index == ckpair->cert_key_index)
	    {
	      ...
	      clib_bihash_add_del_24_8 (&wrk_ctx[i].crypto_context_hash, &kv,
					0 /* is_add */);
	    }
	}
    }
...
quic_quicly_crypto_context_alloc (u8 thread_index)
{
  ...
  pool_get_aligned_safe (qqm->wrk_ctx[thread_index].crypto_ctx_pool, crctx,
  ...
  if (ctx->crypto_engine == CRYPTO_ENGINE_NONE)
    {
      QUIC_DBG (2, "No crypto engine specified, using %U",
		format_crypto_engine, qqm->qm->default_crypto_engine);
