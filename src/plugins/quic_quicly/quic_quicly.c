...
u32 quic_quicly_error[QUIC_APP_ERROR_SIZE] = {
  QUICLY_ERROR_FROM_APPLICATION_ERROR_CODE(QUIC_APP_ERROR_CLOSE_NOTIFY),
  QUICLY_ERROR_FROM_APPLICATION_ERROR_CODE(QUIC_APP_ALLOCATION_ERROR),
  QUICLY_ERROR_FROM_APPLICATION_ERROR_CODE(QUIC_APP_ACCEPT_NOTIFY_ERROR),
  QUICLY_ERROR_FROM_APPLICATION_ERROR_CODE(QUIC_APP_CONNECT_NOTIFY_ERROR)
};

#if QUIC_DEBUG >= 2
static_always_inline u16 quic_quicly_quic_error_from_quicly_error (int quicly_error)
{
  u16 quic_error = (u16) ~0;

  if (quicly_error == quic_quicly_error[QUIC_APP_ERROR_CLOSE_NOTIFY])
      quic_error = QUIC_APP_ERROR_CLOSE_NOTIFY;
  else if (quicly_error == quic_quicly_error[QUIC_APP_ALLOCATION_ERROR])
      quic_error = QUIC_APP_ALLOCATION_ERROR;
  else if (quicly_error == quic_quicly_error[QUIC_APP_ACCEPT_NOTIFY_ERROR])
      quic_error = QUIC_APP_ACCEPT_NOTIFY_ERROR;
  else if (quicly_error == quic_quicly_error[QUIC_APP_CONNECT_NOTIFY_ERROR])
      quic_error = QUIC_APP_CONNECT_NOTIFY_ERROR;

  QUIC_ASSERT(quic_error != (u16) ~0);
  return quic_error;
}
#endif
...
quic_quicly_on_receive_reset (quicly_stream_t *stream, int quicly_error)
{
  ...
  clib_warning ("RESET_STREAM: session 0x%lx (%U)",
		session_handle (stream_session), quic_format_err,
		quic_quicly_quic_error_from_quicly_error(quicly_error));
#endif
  session_transport_closing_notify (&sctx->connection);
}
  ...
  if ((rv = app_worker_init_connected (app_wrk, stream_session)))
    {
      ...
      quicly_reset_stream (stream, quic_quicly_error[QUIC_APP_ALLOCATION_ERROR]);
  ...
  if ((rv = app_worker_accept_notify (app_wrk, stream_session)))
    {
      ...
      quicly_reset_stream (stream, quic_quicly_error[QUIC_APP_ACCEPT_NOTIFY_ERROR]);
      return 0; /* Frame is still valid */
    }
...
quic_quicly_proto_on_close (u32 ctx_index, u32 thread_index)
{
  ...
  if (quic_ctx_is_stream (ctx))
	...
	quicly_reset_stream (stream, quic_quicly_error[QUIC_APP_ERROR_CLOSE_NOTIFY]);
      }
  ...
  switch (ctx->conn_state)
    ...
    case QUIC_CONN_STATE_READY:
      ...
      quicly_close (conn, quic_quicly_error[QUIC_APP_ERROR_CLOSE_NOTIFY], "Closed by peer");
...
{
  quicly_reset_stream ((quicly_stream_t *) quic_stream, QUIC_APP_CONNECT_NOTIFY_ERROR);
}
