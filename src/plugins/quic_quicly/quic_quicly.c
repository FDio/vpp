			...
			size_t len)
{
  ...
  if (off == stream_data->app_rx_data_len)
	...
	{
	  QUIC_DBG (2, "stream rcv side closed, ctx_index %u, thread_index %u",
		    sctx->c_c_index, sctx->c_thread_index);
	  sctx->flags |= QUIC_F_STREAM_RX_CLOSED;
	  session_transport_closing_notify (&sctx->connection);
	  session_set_state (stream_session, SESSION_STATE_READY);
	}
    }
  else
    {
