...
quic_quicly_crypto_context_get (u32 cr_index, u32 thread_index)
{
  ...
  return pool_elt_at_index (qqm->wrk_ctx[thread_index].crypto_ctx_pool,
			    cr_index & 0x00ffffff);
}
...
quic_quicly_send_packets (quic_ctx_t * ctx)
{
  ...
  do
	...
	if ((err = quic_quicly_send_datagram (udp_session, &packets[i], &ctx->rmt_ip, ctx->rmt_port)))
	  goto quicly_error;
      }
      n_sent += num_packets;
    }
  ...
  if (rv == QUIC_PACKET_TYPE_RECEIVE)
    {
	  ...
	  qqm->default_crypto_engine == CRYPTO_ENGINE_VPP)
      {
...
static void
quic_quicly_engine_init (quic_main_t *qm, u32 num_threads)
{
  quic_quicly_main_t *qqm = &quic_quicly_main;
  tw_timer_wheel_1t_3w_1024sl_ov_t *tw;
  ...
  qqm->default_crypto_engine = CRYPTO_ENGINE_PICOTLS;
  qqm->max_packets_per_key = DEFAULT_MAX_PACKETS_PER_KEY;
  qqm->default_quic_cc = QUIC_CC_RENO;
  
  vec_validate (qqm->ctx_pool, num_threads - 1); /* quicly ctx_pool */
   ...
   */
  vec_validate (qqm->wrk_ctx, num_threads - 1);
  qqm->qm->wrk_ctx = qqm->wrk_ctx;
...
const static quic_engine_vft_t quic_quicly_engine_vft = {
  .quic_engine_init = quic_quicly_engine_init,
