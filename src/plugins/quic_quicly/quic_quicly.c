...
quic_quicly_on_stream_open (quicly_stream_open_t *self, quicly_stream_t *stream)
{
  ...
  stream_session->app_wrk_connect_index =
    session_resolve_child_stream_app_wrk_connect_index (stream_session->app_wrk_index, parent);
	...
	case QUIC_PACKET_TYPE_RECEIVE:
	  ...
	  if (ctx->conn_state <= QUIC_CONN_STATE_HANDSHAKE)
		...
		ctx->app_wrk_connect_index =
		  session_connect_originated_get_valid_app_wrk_connect_index (us);
