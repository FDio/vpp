...
quic_quicly_reset_connection (u64 udp_session_handle, quic_rx_packet_ctx_t * pctx)
{
  ...
  rv = quic_quicly_send_datagram (udp_session, &packet, &qctx->rmt_ip, &qctx->lcl_ip);
  quic_quicly_set_udp_tx_evt (udp_session);
  return rv;
}
...
static u8 *
quic_quicly_format_stream_id (u8 * s, va_list * args)
{
...
static u8 *
quic_quicly_format_connection_stats (u8 * s, quic_ctx_t * ctx)
{
...
const static quic_lib_vft_t quic_quicly_vft = {
  ...
  .format_stream_id = quic_quicly_format_stream_id,
