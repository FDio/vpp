...
quic_quicly_send_packets (quic_ctx_t *ctx)
{
  ...
  struct iovec packets[QUIC_SEND_PACKET_VEC_SIZE]; // TODO: GET THIS OFF OF THE STACK

  uint8_t
    buf[QUIC_SEND_PACKET_VEC_SIZE * quic_quicly_get_quicly_ctx_from_ctx (ctx)
				      ->transport_params.max_udp_payload_size]; // TODO: GET THIS OFF OF THE STACK
  ...
  quic_worker_ctx_t *wc = quic_quicly_main.qm->wrk_ctx;
  ...
  quic_update_timer (wc, ctx, next_timeout);
  return n_sent;
...
quic_quicly_timer_expired (u32 conn_index)
{
  ...
  QUIC_DBG (4, "Timer expired for conn %u at %ld", conn_index,
	    quic_get_time (quic_quicly_main.qm->wrk_ctx));
...
static void
quic_quicly_connection_receive (quic_ctx_t *ctx)
{
  ...
  QUIC_DBG (2, "Received conn %u (now %u)", ctx->c_thread_index,
	    new_ctx_id);
  ...
  quic_update_timer (wc, new_ctx, next_timeout);
