...
static int
quic_quicly_send_datagram (session_t *udp_session, struct iovec *packet, ip46_address_t *rmt_ip, u16 rmt_port)
{
  u32 max_enqueue, len;
...
static_always_inline void
quic_quicly_addr_to_ip46_addr (quicly_address_t * quicly_addr, ip46_address_t * ip46_addr, u16 *ip46_port)
{
...
// static_always_inline void
// quic_quicly_ip46_addr_to_quicly_addr (ip46_address_t * ip46_addr, u16 *ip46_port, quicly_address_t * quicly_addr)
// {
//   if (ip46_address_is_ip4 (ip46_addr))
//     {
//       struct sockaddr_in *sa4 = (struct sockaddr_in *) &quicly_addr->sa;
//       sa4->sin_port = *ip46_port;
//       sa4->sin_addr.s_addr = ip46_addr->ip4.as_u32 ;
//     }
//   else
//     {
//       struct sockaddr_in6 *sa6 = (struct sockaddr_in6 *) &quicly_addr->sa;
//        sa6->sin6_port = *ip46_port;
//       clib_memcpy_fast (&sa6->sin6_addr, &ip46_addr->ip6, 16);
//     }
// }
  ...
  do
    {
      ...
      quic_quicly_addr_to_ip46_addr (&quicly_rmt_ip, &ctx->rmt_ip, &ctx->rmt_port);
...
static int
quic_quicly_reset_connection (u64 udp_session_handle, quic_quicly_rx_packet_ctx_t * pctx)
{
  ...
  quic_ctx_t *ctx = quic_ctx_get (pctx->ctx_index, pctx->thread_index);
  ...
  rv = quic_quicly_send_datagram (udp_session, &packet, &ctx->rmt_ip, ctx->rmt_port);
  quic_quicly_set_udp_tx_evt (udp_session);
  return rv;
}
...
quic_quicly_proto_on_close (u32 ctx_index, u32 thread_index)
{
  ...
  switch (ctx->conn_state)
    ...
    case QUIC_CONN_STATE_READY:
      ...
      quicly_close (conn, QUIC_QUICLY_APP_ERROR_CLOSE_NOTIFY, "Closed by peer");
...
quic_quicly_accept_connection (quic_quicly_rx_packet_ctx_t * pctx)
{
  ...
  if ((rv = quicly_accept (
	 ...
	 &qqm->wrk_ctx[pctx->thread_index].next_cid, NULL, NULL)))
    {
      /* Invalid packet, pass */
      assert (conn == NULL);
  ...
  ++qqm->wrk_ctx[pctx->thread_index].next_cid.master_id;
...
quic_quicly_process_one_rx_packet (u64 udp_session_handle, svm_fifo_t *f,
			    u32 fifo_offset, quic_quicly_rx_packet_ctx_t *pctx)
{
  size_t plen;
  u32 full_len, ret;
  ...
  if (rv == QUIC_PACKET_TYPE_RECEIVE)
    {
	  ...
	  qqm->qm->default_crypto_engine == CRYPTO_ENGINE_VPP)
      {
...
quic_quicly_connect (quic_ctx_t * ctx, u32 ctx_index, u32 thread_index, struct sockaddr *sa)
{
  ...
  ret = quicly_connect (&conn, quicly_ctx, (char *) ctx->srv_hostname, sa,
			NULL, &qqm->wrk_ctx[thread_index].next_cid,
			ptls_iovec_init (NULL, 0), &qqm->hs_properties,
			NULL, NULL);
  ++qqm->wrk_ctx[thread_index].next_cid.master_id;  
...
static_always_inline void
quic_quicly_reset_stream_connect_error (void * quic_stream)
{
  quicly_reset_stream ((quicly_stream_t *) quic_stream, QUIC_QUICLY_APP_CONNECT_NOTIFY_ERROR);
}
...
quic_quicly_engine_init (quic_main_t *qm)
{
  ...
  qqm->qm = qm;
  clib_bihash_init_16_8 (&qqm->connection_hash, "quic (quicly engine) connections", 1024,
			 4 << 20);
  clib_bitmap_alloc (qqm->available_crypto_engines,
		     app_crypto_engine_n_types ());
  qqm->qm->default_crypto_engine = CRYPTO_ENGINE_PICOTLS;
  qqm->qm->default_quic_cc = QUIC_CC_RENO;
  
  vec_validate (qm->ctx_pool, qm->num_threads - 1); /* quicly ctx_pool */

  /* The wrk_ctx vector is never allocated/resized anywhere else!
   * Do NOT copy this design pattern unless you KNOW the vector is
   * allocated ONLY ONCE (you have been warned)!
   */
  vec_validate (qqm->wrk_ctx, qm->num_threads - 1);
  qm->wrk_ctx = (quic_worker_ctx_t *) qqm->wrk_ctx;
  
  for (i = 0; i < qm->num_threads; i++)
    {
      qqm->wrk_ctx[i].next_cid.thread_id = i;
      tw = &qqm->wrk_ctx[i].timer_wheel;
      tw_timer_wheel_init_1t_3w_1024sl_ov (tw, quic_quicly_expired_timers_dispatch,
					   1e-3 /* timer period 1ms */, ~0);
      tw->last_run_time = vlib_time_now (vlib_get_main ());
      clib_bihash_init_24_8 (&qqm->wrk_ctx[i].crypto_context_hash,
...
rx_start:
  ...
  for (i = 0; i < max_packets; i++)
	...
	break;
      ...
      rv = quic_quicly_process_one_rx_packet (udp_session_handle, f, fifo_offset,
				       &packets_ctx[i]);
...
const static quic_engine_vft_t quic_quicly_engine_vft = {
  ...
  .encrypt_ticket_cb = quic_quicly_encrypt_ticket_cb,
  ...
  .reset_stream_connect_error = quic_quicly_reset_stream_connect_error,
...
{
  vlib_get_plugin_symbol ("quic_plugin.so", "quic_input_node");
  vlib_get_plugin_symbol ("quic_plugin.so", "quic_register_lib");

  quic_register_engine (&quic_quicly_engine_vft, QUIC_ENGINE_QUICLY);
  printf ("DAW: %s", __FUNCTION__);
  return 0;
}
