...
quic_quicly_connection_delete (quic_ctx_t *ctx)
{
  ...
  quic_stop_ctx_timer (qqm->qm, ctx);
...
quic_quicly_update_timer (quic_ctx_t *ctx)
{
  ...
  next_interval = next_timeout - quic_quicly_get_time ();
...
// static_always_inline void
// quic_quicly_ip46_addr_to_quicly_addr (ip46_address_t * ip46_addr, u16
// *ip46_port, quicly_address_t * quicly_addr)
// {
...
quic_quicly_get_quic_ctx_if_valid (u32 ctx_index, u32 thread_index)
{
  quic_main_t *qm = quic_quicly_main.qm;
...
static int
quic_quicly_connect (quic_ctx_t *ctx, u32 ctx_index, u32 thread_index,
		     struct sockaddr *sa)
{
  clib_bihash_kv_16_8_t kv;
  quicly_context_t *quicly_ctx;
  ...
  if (ctx->conn == NULL)
    {
      QUIC_DBG (2, "ERROR ctx->conn == NULL");
      return -1;
    }
  quicly_ctx = quic_quicly_get_quicly_ctx_from_ctx (ctx);
  conn = ctx->conn;
  ret = quicly_connect (&conn, quicly_ctx, (char *) ctx->srv_hostname, sa,
			...
			ptls_iovec_init (NULL, 0), &qqm->hs_properties, NULL,
			NULL);
...
quic_quicly_engine_init (quic_main_t *qm)
{
  ...
  qqm->qm->max_packets_per_key = DEFAULT_MAX_PACKETS_PER_KEY;
  ...
  for (i = 0; i < qm->num_threads; i++)
    {
      ...
      QUIC_DBG (
	2, "Initialized wrk_ctx[%u].crypto_context_hash (buckets = 0x%lx)", i,
	qqm->wrk_ctx[i].crypto_context_hash.buckets);
      pool_get_aligned_safe (qqm->wrk_ctx[i].crypto_ctx_pool, crctx,
			     CLIB_CACHE_LINE_BYTES);
      pool_program_safe_realloc ((void **) &qqm->wrk_ctx[i].crypto_ctx_pool,
				 QUIC_CRYPTO_CTX_POOL_PER_THREAD_SIZE,
				 CLIB_CACHE_LINE_BYTES);
    }
  return ((quic_worker_ctx_t *) qqm->wrk_ctx);
}
