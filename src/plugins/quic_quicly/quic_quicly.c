...
{
  quic_quicly_main_t *qqm = &quic_quicly_main;
  quicly_cid_plaintext_t *next_cid = qqm->next_cid;
  clib_bihash_24_8_t *crctx_hash = qqm->crypto_ctx_hash;
  quic_worker_ctx_t *wrk_ctx = qqm->qm->wrk_ctx;
  tw_timer_wheel_1t_3w_1024sl_ov_t *tw;
  u32 i;
  ...
  vec_validate (next_cid, qm->num_threads - 1);
  vec_validate (crctx_hash, qm->num_threads - 1);
  for (i = 0; i < qm->num_threads; i++)
    {
      tw = &wrk_ctx[i].timer_wheel;
