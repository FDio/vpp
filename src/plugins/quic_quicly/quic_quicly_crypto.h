...
quic_quicly_crypto_context_get (u32 cr_index, u32 thread_index)
{
  ...
  return pool_elt_at_index (qqm->qm->wrk_ctx[thread_index].crypto_ctx_pool,
			    cr_index & 0x00ffffff);
}
...
extern void quic_quicly_crypto_context_release (u32 crypto_context_index, u8 thread_index);
extern int quic_quicly_app_cert_key_pair_delete (app_cert_key_pair_t * ckpair);
extern int quic_quicly_encrypt_ticket_cb (ptls_encrypt_ticket_t *_self, ptls_t *tls,
			int is_encrypt, ptls_buffer_t *dst, ptls_iovec_t src);
extern void quic_quicly_crypto_decrypt_packet (quic_ctx_t *qctx, quic_quicly_rx_packet_ctx_t *pctx);
