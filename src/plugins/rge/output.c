...
rge_tx_desc_get_dtyp (rge_tx_desc_t * d)
{
  return d->qword[1] & 0x0f;
}

static_always_inline u16
rge_tx_enqueue (vlib_main_t * vm, rge_txq_t * txq, u32 * buffers,
		u32 n_packets, int use_va_dma)
{
  u16 next = txq->next;
  u64 bits = RGE_TXD_CMD_EOP | RGE_TXD_CMD_RSV;
  u16 n_desc = 0;
  u16 *slot, n_desc_left, n_packets_left = n_packets;
  u16 mask = txq->size - 1;
  vlib_buffer_t *b[4];
  rge_tx_desc_t *d = txq->descs + next;
...
VNET_DEVICE_CLASS_TX_FN (rge_device_class) (vlib_main_t * vm,
					    vlib_node_runtime_t * node,
					    vlib_frame_t * frame)
{
  rge_main_t *am = &rge_main;
  vnet_interface_output_runtime_t *rd = (void *) node->runtime_data;
  rge_device_t *ad = pool_elt_at_index (am->devices, rd->dev_instance);
  u32 thread_index = vm->thread_index;
  u8 qid = thread_index;
  rge_txq_t *txq = vec_elt_at_index (ad->txqs, qid % ad->num_queue_pairs);
...
retry:
  ...
  if (txq->n_enqueued)
      ...
      while (1)
	{
	  ...
	  if (rge_tx_desc_get_dtyp (txq->descs + complete_slot) != 0x0F)
	    break;
  ...
  if (ad->flags & RGE_DEVICE_F_VA_DMA)
    n_enq = rge_tx_enqueue (vm, txq, buffers, n_left, 1);
  else
    n_enq = rge_tx_enqueue (vm, txq, buffers, n_left, 0);
  ...
  if (n_left)
	...
	goto retry;
      ...
      vlib_error_count (vm, node->node_index,
			RGE_TX_ERROR_NO_FREE_SLOTS, n_left);
    }
