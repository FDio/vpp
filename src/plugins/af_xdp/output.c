...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  for (i = 0, n = 0, n_desc = 0; i < AF_XDP_TX_RETRIES && n < n_tx; i++)
	...
	af_xdp_device_output_free (vm, node, txq, /* is_mb */ 1);
      else
	af_xdp_device_output_free (vm, node, txq, /* is_mb */ 0);

      if (ad->flags & AF_XDP_DEVICE_F_MULTI_BUFFER)
	n_enq = af_xdp_device_output_tx_try (vm, node, ad, txq, n_tx - n, from + n, &n_desc_enq,
					     /* is_mb */ 1);
      else
	n_enq = af_xdp_device_output_tx_try (vm, node, ad, txq, n_tx - n, from + n, &n_desc_enq,
					     /* is_mb */ 0);
