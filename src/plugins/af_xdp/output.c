  ...
  while (n >= 8)
    {
      ...
      u64x8 b0 =
	u64x8_load_unaligned (compl + 0) >> CLIB_LOG2_CACHE_LINE_BYTES;
      u32x8_store_unaligned (u64x8_reduce_to_u32x8 (b0), bi + 0);
#elif defined(CLIB_HAVE_VEC256)
      u64x4 b0 =
	u64x4_load_unaligned (compl + 0) >> CLIB_LOG2_CACHE_LINE_BYTES;
      u64x4 b1 =
	u64x4_load_unaligned (compl + 4) >> CLIB_LOG2_CACHE_LINE_BYTES;
      ...
      u32x4_store_unaligned (u32x8_extract_lo (b2), bi + 0);
      u32x4_store_unaligned (u32x8_extract_lo (b3), bi + 4);
#else
			    ...
			    af_xdp_txq_t * txq, const u32 n_tx)
{
  ...
  af_xdp_log (VLIB_LOG_LEVEL_ERR, ad, "sendto() failed: %s (%i)",
					       ...
					       vlib_frame_t * frame)
{
  ...
  af_xdp_txq_t *txq =
    vec_elt_at_index (ad->txqs, thread_index % vec_len (ad->txqs));
  u32 *from;
  u32 n, n_tx;
  int i;
