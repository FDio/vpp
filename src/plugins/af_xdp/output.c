...
af_xdp_device_output_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame,
			     af_xdp_device_t *ad, af_xdp_txq_t *txq, int with_multi_buffer)
{
  u32 *from;
  u32 n, n_tx, n_desc;
  int i;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (ad->flags & AF_XDP_DEVICE_F_MULTI_BUFFER)
    n = af_xdp_device_output_inline (vm, node, frame, ad, txq, 1);
  else
    n = af_xdp_device_output_inline (vm, node, frame, ad, txq, 0);
