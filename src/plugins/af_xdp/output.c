...
/*
 * Reclaim completed TX buffers from the XSK completion queue.
 *
 * is_mb = 0 (single-buffer mode):
 *   Each completion maps to a single-buffer packet. Uses a vectorized
 *   loop (AVX2 when available) to extract buffer indices in bulk.
 *   Calls vlib_buffer_free() which follows buffer chains — safe here
 *   because single-buffer packets may have been linearized from chains.
 *
 * is_mb = 1 (multi-buffer mode):
 *   Each completion maps to exactly one buffer segment, since segments
 *   are submitted to the TX ring individually. Calls
 *   vlib_buffer_free_no_next() to free each segment in isolation without
 *   following next_buffer pointers (the next segment is its own separate
 *   completion entry).
 *
...
/*
 * Enqueue TX descriptors to the XSK TX ring.
 *
 * is_mb = 0 (single-buffer mode):
 *   Reserves n_tx slots upfront. Uses a vectorized loop for packets without
 *   chained buffers, falling back to scalar with linearization for chained
 *   packets. Handles ring wrap-around. Returns n_tx (packets == descriptors).
 *
 * is_mb = 1 (multi-buffer mode):
 *   Pre-counts the total descriptors needed across pending packets, then
 *   reserves that many slots in one call. Submits each segment as a separate
 *   descriptor, setting XDP_PKT_CONTD on all but the last. Returns the number
 *   of packets submitted; *n_tx_desc receives the total descriptor count.
 *
 * is_mb must be a compile-time constant so the compiler eliminates the
 * unused branch and generates optimal code for each mode.
 */
...
af_xdp_device_output_tx_try (vlib_main_t *vm, const vlib_node_runtime_t *node, af_xdp_device_t *ad,
			     af_xdp_txq_t *txq, u32 n_tx, u32 *bi, u32 *n_tx_desc, const int is_mb)
{
	  ...
	  while (1)
	    {
	      ...
	      u64 offset = (sizeof (vlib_buffer_t) + b->current_data)
  ...
  for (i = 0, n = 0, n_desc = 0; i < AF_XDP_TX_RETRIES && n < n_tx; i++)
    {
      ...
      af_xdp_device_output_free (vm, node, txq, is_mb);
      n_enq =
	af_xdp_device_output_tx_try (vm, node, ad, txq, n_tx - n, from + n, &n_desc_enq, is_mb);
      n += n_enq;
      n_desc += n_desc_enq;
    }
