...
af_xdp_device_output_tx_try_mb (vlib_main_t *vm, af_xdp_device_t *ad, af_xdp_txq_t *txq, u32 n_tx,
				u32 *bi, u32 *n_tx_desc, u32 *n_tx_dropped)
{
  ...
  for (pidx = 0; pidx < n_tx; pidx++)
    {
      ...
      if (PREDICT_FALSE (n_segs > max_segs))
	{
	  if (n_desc)
	    break;
			     ...
			     af_xdp_device_t *ad, af_xdp_txq_t *txq, const int is_mb)
{
  ...
  for (i = 0, n = 0, n_desc = 0; i < AF_XDP_TX_RETRIES && n < n_tx; i++)
    {
      u32 n_enq, n_desc_enq, n_dropped = 0;
      ...
      if (is_mb)
	{
	  ...
	  n_enq = af_xdp_device_output_tx_try_mb (vm, ad, txq, n_tx - n, from + n, &n_desc_enq,
						  &n_dropped);
	  if (PREDICT_FALSE (n_dropped))
	    vlib_error_count (vm, node->node_index, AF_XDP_TX_ERROR_TOO_MANY_SEGS, n_dropped);
	}
      else
	{
