...
af_xdp_device_output_free (vlib_main_t *vm, af_xdp_txq_t *txq, const int is_mb)
{
  const __u64 * compl;
  ...
  /* we rely on casting addr (u64) -> bi (u32) to discard XSK offset below */
...
af_xdp_device_buffer_n_segs (vlib_main_t *vm, vlib_buffer_t *b, u32 *has_zero_len_seg)
{
  u32 n = 1;

  *has_zero_len_seg = b->current_length == 0;
...
static_always_inline u32
af_xdp_device_output_tx_try_mb (vlib_main_t *vm, af_xdp_device_t *ad, af_xdp_txq_t *txq, u32 n_tx,
				u32 *bi, u32 *n_tx_desc, u32 *n_tx_too_many_segs,
				u32 *n_tx_zero_len_segs)
{
  const uword start = vm->buffer_main->buffer_mem_start;
  const u32 max_segs = ad->tx_max_segs;
  u32 n_desc_budget, n_desc = 0, n_enq = 0, n_drop_prefix = 0, idx, pidx;

  n_desc_budget = xsk_prod_nb_free (&txq->tx, max_segs);
  ...
  for (pidx = 0; pidx < n_tx; pidx++)
    {
      ...
      u32 has_zero_len_seg;
      u32 n_segs = af_xdp_device_buffer_n_segs (vm, b, &has_zero_len_seg);

      if (PREDICT_FALSE (has_zero_len_seg || n_segs > max_segs))
	{
	  if (n_desc)
	    break;

	  vlib_buffer_free_one (vm, bi[pidx]);
	  if (has_zero_len_seg)
	    *n_tx_zero_len_segs += 1;
	  else
	    *n_tx_too_many_segs += 1;
	  n_drop_prefix++;
	  n_enq++;
	  continue;
	}
  ...
  for (pidx = n_drop_prefix; pidx < n_enq; pidx++)
    {
      vlib_buffer_t *b = vlib_get_buffer (vm, bi[pidx]);
...
static_always_inline u32
af_xdp_device_output_tx_try (vlib_main_t *vm, af_xdp_device_t *ad, af_xdp_txq_t *txq, u32 n_tx,
			     u32 *bi, u32 *n_tx_desc)
{
...
wrap_around:
  ...
  while (n >= 8)
    {
      if (PREDICT_FALSE (
	    b[0]->flags & VLIB_BUFFER_NEXT_PRESENT || b[1]->flags & VLIB_BUFFER_NEXT_PRESENT ||
	    b[2]->flags & VLIB_BUFFER_NEXT_PRESENT || b[3]->flags & VLIB_BUFFER_NEXT_PRESENT))
	{
	  break;
	}
...
static_always_inline u32
af_xdp_device_output_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame,
			     af_xdp_device_t *ad, af_xdp_txq_t *txq, const int is_mb)
{
  ...
  for (i = 0, n = 0, n_desc = 0; i < AF_XDP_TX_RETRIES && n < n_tx; i++)
    {
      u32 n_enq, n_desc_enq, n_too_many_segs = 0, n_zero_len_segs = 0;

      if (is_mb)
	{
	  af_xdp_device_output_free (vm, txq, /* is_mb */ 1);
	  n_enq = af_xdp_device_output_tx_try_mb (vm, ad, txq, n_tx - n, from + n, &n_desc_enq,
						  &n_too_many_segs, &n_zero_len_segs);
	  if (PREDICT_FALSE (n_too_many_segs))
	    vlib_error_count (vm, node->node_index, AF_XDP_TX_ERROR_TOO_MANY_SEGS, n_too_many_segs);
	  if (PREDICT_FALSE (n_zero_len_segs))
	    vlib_error_count (vm, node->node_index, AF_XDP_TX_ERROR_ZERO_LENGTH_SEG,
			      n_zero_len_segs);
	}
      else
	{
	  af_xdp_device_output_free (vm, txq, /* is_mb */ 0);
	  n_enq = af_xdp_device_output_tx_try (vm, ad, txq, n_tx - n, from + n, &n_desc_enq);
	}
      n += n_enq;
      n_desc += n_desc_enq;
    }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (ad->flags & AF_XDP_DEVICE_F_MULTI_BUFFER)
    n = af_xdp_device_output_inline (vm, node, frame, ad, txq, /* is_mb */ 1);
  else
    n = af_xdp_device_output_inline (vm, node, frame, ad, txq, /* is_mb */ 0);
