...
#define AF_XDP_TX_MAX_SEGS 18 /* CONFIG_MAX_SKB_FRAGS + 1 portable limit. */
  ...
  for (pidx = 0; pidx < n_tx; pidx++)
    {
      ...
      if (PREDICT_FALSE (n_segs > AF_XDP_TX_MAX_SEGS))
	{
	  if (n_desc)
	    break;
			     ...
			     af_xdp_device_t *ad, af_xdp_txq_t *txq, const int is_mb)
{
  ...
  for (i = 0, n = 0, n_desc = 0; i < AF_XDP_TX_RETRIES && n < n_tx; i++)
      ...
      if (is_mb)
	{
	  ...
	  n_enq =
	    af_xdp_device_output_tx_try_mb (vm, txq, n_tx - n, from + n, &n_desc_enq, &n_dropped);
