...
The ``prog`` argument accepts two forms:

- A bare filename (e.g. ``xsk_def_xdp_prog.o``): the program is looked
  up in ``LIBXDP_OBJECT_PATH`` (typically ``/usr/lib/bpf``).
- A path containing ``/`` (e.g. ``/path/to/my_prog.o`` or
  ``extras/bpf/af_xdp.bpf.o``): the program is opened directly from the
  filesystem.
