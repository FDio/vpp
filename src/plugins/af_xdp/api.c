...
static af_xdp_create_flag_t
af_xdp_api_flags_v2 (vl_api_af_xdp_flag_v2_t flags)
{
  af_xdp_create_flag_t cflags = 0;

  if (flags & AF_XDP_API_FLAGS_V2_NO_SYSCALL_LOCK)
    cflags |= AF_XDP_CREATE_FLAGS_NO_SYSCALL_LOCK;
  if (flags & AF_XDP_API_FLAGS_V2_MULTI_BUFFER)
...
static void
vl_api_af_xdp_create_v4_t_handler (vl_api_af_xdp_create_v4_t *mp)
{
  vlib_main_t *vm = vlib_get_main ();
  af_xdp_main_t *rm = &af_xdp_main;
  vl_api_af_xdp_create_v4_reply_t *rmp;
  af_xdp_create_if_args_t args;
  int rv;
  ...
  args.netns = mp->netns[0] ? (char *) mp->netns : 0;
  args.mode = af_xdp_api_mode (mp->mode);
  args.flags = af_xdp_api_flags_v2 (mp->flags);
  args.rxq_size = mp->rxq_size;
  args.txq_size = mp->txq_size;
  args.rxq_num = mp->rxq_num;
  ...
  /* clang-format off */
  REPLY_MACRO2_END (VL_API_AF_XDP_CREATE_V4_REPLY,
    ({
      rmp->sw_if_index = args.sw_if_index;
    }));
  /* clang-format on */
