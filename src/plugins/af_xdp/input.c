...
af_xdp_device_input_bufs (vlib_main_t *vm, const af_xdp_device_t *ad, af_xdp_rxq_t *rxq, u32 *bis,
			  const u32 n_rx, const vlib_buffer_template_t *bt, u32 idx)
{
...
af_xdp_device_input_bufs_mb (vlib_main_t *vm, af_xdp_rxq_t *rxq, u32 *bis, const u32 n_rx,
			     const vlib_buffer_template_t *bt, u32 idx, u32 *n_rx_bytes)
{
  const u32 mask = rxq->rx.mask;
			    ...
			    af_xdp_device_t *ad, u16 qid, const int is_mb)
{
  ...
  if (is_mb)
    n_rx_packets =
      af_xdp_device_input_bufs_mb (vm, rxq, to_next, n_rx_desc, &bt.template, idx, &n_rx_bytes);
  else
    {
      n_rx_packets = n_rx_desc;
      n_rx_bytes = af_xdp_device_input_bufs (vm, ad, rxq, to_next, n_rx_packets, &bt.template, idx);
    }
