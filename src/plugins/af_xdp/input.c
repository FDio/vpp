...
af_xdp_device_input_refill_db (vlib_main_t * vm,
			       ...
			       const af_xdp_device_t * ad, af_xdp_rxq_t * rxq,
			       const u32 n_alloc)
{
  int ret;
  ...
  vlib_error_count (vm, node->node_index, AF_XDP_INPUT_ERROR_POLL_FAILURES,
		    1);
  af_xdp_log (VLIB_LOG_LEVEL_ERR, ad, "poll() failed: %s (%i)",
  ...
  while (n >= 8)
    {
      ...
      u64x8 b0 = u32x8_extend_to_u64x8 (u32x8_load_unaligned (bi + 0));
      u64x8_store_unaligned (bi2addr (b0), fill + 0);
#elif defined(CLIB_HAVE_VEC256)
      u64x4 b0 = u32x4_extend_to_u64x4 (u32x4_load_unaligned (bi + 0));
      u64x4 b1 = u32x4_extend_to_u64x4 (u32x4_load_unaligned (bi + 4));
      u64x4_store_unaligned (bi2addr (b0), fill + 0);
      u64x4_store_unaligned (bi2addr (b1), fill + 4);
#else
      fill[0] = bi2addr (bi[0]);
      fill[1] = bi2addr (bi[1]);
      fill[2] = bi2addr (bi[2]);
