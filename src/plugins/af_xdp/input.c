...
af_xdp_device_input_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame,
			    af_xdp_device_t *ad, u16 qid, int with_multi_buffer)
{
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  vec_foreach (p, pv)
	...
	n_rx += af_xdp_device_input_inline (vm, node, frame, ad, p->queue_id, 1);
      else
	n_rx += af_xdp_device_input_inline (vm, node, frame, ad, p->queue_id, 0);
    }
