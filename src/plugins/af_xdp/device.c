...
af_xdp_create_if (vlib_main_t *vm, af_xdp_create_if_args_t *args)
{
  ...
  if (args->busy_poll_usecs || args->busy_poll_budget)
    {
      if (args->busy_poll_usecs == 0)
	args->busy_poll_usecs = AF_XDP_BUSY_POLL_USECS_DEFAULT;
      if (args->busy_poll_budget == 0)
  ...
  if (args->busy_poll_usecs > INT_MAX || args->busy_poll_budget > UINT16_MAX)
    {
      args->rv = VNET_API_ERROR_INVALID_VALUE;
      args->error = clib_error_return (0, "busy-poll-usecs must fit an int and budget a u16");
      goto err0;
    }
