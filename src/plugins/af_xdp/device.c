...
xsk_socket_init (u64 umem_size, const u8 * ifname, int ifindex, u32 queue_id)
{
  u32 opt_xdp_flags = XDP_FLAGS_UPDATE_IF_NOEXIST;
  u32 opt_xdp_bind_flags = 0;
  __u32 prog_id;
  ///
  ...
  if (!(umem = xsk_umem_init (umem_size)))
    goto error_calloc;
  ...
  rv = xsk_socket__create (&xsk->xsk, (const char *) ifname, queue_id,
  ...
  rv = bpf_get_link_xdp_id (ifindex, &prog_id, opt_xdp_flags);
  if (rv)
    {
  ...
  rv = xsk_ring_prod__reserve (&xsk->umem->fq,
			       XSK_RING_PROD__DEFAULT_NUM_DESCS, &idx);
  if (rv != XSK_RING_PROD__DEFAULT_NUM_DESCS)
    {
...
//int
//xsk_if_init (u8 * ifname, int ifindex, u32 key, u32 queue_id,
...
//  int rv = 0;
  ...
  if (!(xsk = xsk_socket_init (NUM_FRAMES * XSK_UMEM__DEFAULT_FRAME_SIZE,
			       args->ifname, ifindex, args->queue_id)))
    return;

  pool_get (am->devices, ad);

//  if ((rv = xsk_if_init (args->ifname, ifindex, args->key, args->queue_id, &ad->xsk)))
//    {
//      args->rv = rv;
...
//                    frame->n_vectors);
//
//  if (PREDICT_FALSE (err = drain_cq (xsk, frame->n_vectors)))
//     vlib_error_count (vm, node->node_index, sendto_tx_error(err), 1);
...
//              ? xsk->outstanding_tx / KERNEL_TX_BATCH_SIZE
//              : 0;
...
//              return -ENOSPC;
  ...
  while (n_vectors-- && batch_size--)
      ...
      do
	{
      ...
      xsk_ring_prod__tx_desc (&xsk->tx, tx_idx + n_sent)->addr = addr;
      xsk_ring_prod__tx_desc (&xsk->tx, tx_idx + n_sent)->len = len;
					       ...
					       vlib_frame_t * frame)
{
  ...
  free_frames = xsk_umem_free_frames (xsk);
  ...
  if (PREDICT_FALSE (xsk_ring_prod__reserve (&xsk->tx, batch_size,
					     &tx_idx) != batch_size))
    {
      vlib_error_count (vm, node->node_index, AF_XDP_TX_ERROR__TXRING_OVERRUN,
			batch_size);
  ...
  n_sent = txq_enq (vm, xsk, frame, tx_idx, batch_size);
