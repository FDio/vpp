...
af_xdp_load_program (af_xdp_create_if_args_t * args, af_xdp_device_t * ad)
{
  ...
  ad->linux_ifindex = if_nametoindex (args->linux_ifname);
  if (!ad->linux_ifindex)
    {
		     ...
		     af_xdp_device_t * ad, int qid, int rxq_num, int txq_num)
{
  ...
  vec_validate (args->linux_ifname, IFNAMSIZ - 1);	/* libbpf expects ifname to be at least IFNAMSIZ */
  if (xsk_socket__create
      (xsk, args->linux_ifname, qid, *umem, rx, tx, &sock_config))
    {
...
af_xdp_create_if (vlib_main_t * vm, af_xdp_create_if_args_t * args)
{
  ...
  ad->pool =
    vlib_buffer_pool_get_default_for_numa (vm,
					   af_xdp_get_numa
					   (args->linux_ifname));
  ad->linux_ifname = format (0, "%s", args->linux_ifname);
  if (!args->name || 0 == args->name[0])
    ad->name = format (0, "%s/%d", args->linux_ifname, ad->dev_instance);
  else
