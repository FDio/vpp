...
static void
af_xdp_get_q_max (const char *ifname, int *rxq_num, int *txq_num)
{
  ...
  *rxq_num = clib_max (ec.max_combined, ec.max_rx);
  *txq_num = clib_max (ec.max_combined, ec.max_tx);
}
...
af_xdp_create_if (vlib_main_t * vm, af_xdp_create_if_args_t * args)
{
  ...
  af_xdp_get_q_max (args->linux_ifname, &rxq_num, &txq_num);
