...
format_gbp_hash_mode (u8 * s, va_list * args)
{
  ...
  switch (hash_mode)
    {
#define _(v,a,b) case GBP_HASH_MODE_##v: return (format (s, "%s", a));
...
static int
gbp_contract_mk_lb_hp_valid (gbp_hash_mode_t gu_hash_mode)
{
  if (gu_hash_mode < 1 || gu_hash_mode > 3)
    return 0;
  return 1;
}
...
gbp_contract_mk_lb (index_t gui, fib_protocol_t fproto)
{
  ...
  if (!gbp_contract_mk_lb_hp_valid (gu->gu_hash_mode))
    return;
