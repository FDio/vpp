...
gbp_learn_l2_cp (const gbp_learn_l2_t * gl2)
{
  ...
  /*
   * if the packet arrived on the base VNI interface then, pretend that
   * the endpoint was learned from the MAC spine-proxy so that packet
   * sent back to the endpoint will be sent via that proxy
   */
  /* gbi = gbp_bridge_domain_find_by_bd_index (gl2->bd_index); */

  /* GBP_LEARN_DBG ("L2 EP: %U, epg:%d bd-index:%d GBP-BD:%d", */
  /*             format_mac_address_t, &gl2->mac, gl2->epg, */
  /*             gl2->bd_index, gbi); */
  ...
  /* gbp_endpoint_update ((gl2->sw_if_index == gb->gb_vni_sw_if_index ? */
  /*                    gb->gb_uu_flood_sw_if_index : */
  /*                    gl2->sw_if_index), */
  /*                   NULL, */
  /*                   &gl2->mac, gl2->epg, GBP_ENDPOINT_FLAG_DYNAMIC, NULL); */
  gbp_endpoint_update (gl2->sw_if_index, NULL,
		       &gl2->mac, gl2->epg, GBP_ENDPOINT_FLAG_DYNAMIC, NULL);
}
