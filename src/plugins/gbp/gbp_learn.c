...
#define GBP_LEARN_DBG(...)                                      \
    vlib_log_notice (gbp_learn_main.gl_logger, __VA_ARGS__);
...
{
  GBP_LEARN_DBG ("L2 EP: %U, %d", format_mac_address_t, &gl2->mac, gl2->epg);
  gbp_endpoint_update (gl2->sw_if_index, NULL, &gl2->mac, gl2->epg,
		       GBP_ENDPOINT_FLAG_DYNAMIC, NULL);
}

static void
gbp_learn_l2_dp (const u8 * mac, u32 bd_index, u32 sw_if_index, epg_id_t epg)
{
  gbp_learn_l2_t gl2 = {
  ...
  clib_memcpy (gl2.mac.bytes, mac, 6);
...
format_gbp_learn_l2_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "new:%d throttled:%d mac:%U itf:%d epg:%d",
	      t->new, t->throttled,
...
{
  GBP_LEARN_DBG ("L2 EP: %U, %d", format_ip46_address, &gl3->ip,
	  ...
	  if (FIB_PROTOCOL_IP6 == fproto)
	    {
	      ...
	      ge0 = gbp_endpoint_find_ip6 (&ip0->src_address, fib_index0);

	      if (NULL == ge0)
		{
		  t0 = throttle_check (&glm->gl_l3_throttle, thread_index,
				       ip6_address_hash_to_u32
				       (&ip0->src_address), seed);
	      ...
	      if (PREDICT_FALSE ((b0->flags & VLIB_BUFFER_IS_TRACED)))
		{
		  ...
		  ip46_address_set_ip6 (&t->ip, &ip0->src_address);
		  t->new = (NULL == ge0);
		  t->throttled = t0;
		  t->sw_if_index = sw_if_index0;
		  t->epg = epg0;
	  ...
	  else
	    {
	      ...
	      ip0 = vlib_buffer_get_current (b0);
	      ge0 = gbp_endpoint_find_ip4 (&ip0->src_address, fib_index0);

	      if (NULL == ge0)
		{
		  t0 = throttle_check (&glm->gl_l3_throttle, thread_index,
				       ip0->src_address.as_u32, seed);

		  if (!t0)
		    {
		      gbp_learn_ip4_dp (&ip0->src_address,
					fib_index0, sw_if_index0, epg0);
		    }
		}
	      else
	      ...
	      if (PREDICT_FALSE ((b0->flags & VLIB_BUFFER_IS_TRACED)))
		{
		  ...
		  t = vlib_add_trace (vm, node, b0, sizeof (*t));
		  ip46_address_set_ip4 (&t->ip, &ip0->src_address);
		  t->new = (NULL == ge0);
		  t->throttled = t0;
		  t->sw_if_index = sw_if_index0;
		  t->epg = epg0;
...
void
gbp_learn_enable (u32 sw_if_index, u8 is_l2)
{
  if (is_l2)
...
void
gbp_learn_disable (u32 sw_if_index, u8 is_l2)
{
  if (is_l2)
