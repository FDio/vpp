...
from vpp_pom.vpp_vxlan_gbp_tunnel import find_vxlan_gbp_tunnel, \
    INDEX_INVALID, VppVxlanGbpTunnel
from vpp_pom.vpp_neighbor import VppNeighbor
from vpp_pom.plugins.vpp_acl import AclRule, VppAcl
from vpp_pom.plugins.vpp_gbp import find_gbp_endpoint, \
    VppGbpEndpointRetention, VppGbpContractNextHop, VppGbpContractRule, \
    VppGbpEndpoint, VppGbpRecirc, VppGbpExtItf, VppGbpSubnet, \
    VppGbpEndpointGroup, VppGbpBridgeDomain, VppGbpRouteDomain, \
    VppGbpContract, VppGbpVxlanTunnel
try:
    text_type = unicode
except NameError:
    text_type = str
...
class TestGBP(VppTestCase):
    ...
    def test_gbp(self):
        ...
        recircs = [VppGbpRecirc(self.vclient, epgs[0], self.loop3),
                   ...
                   VppGbpRecirc(
                       self.vclient, epgs[3], self.loop6, is_ext=True),
                   VppGbpRecirc(self.vclient, epgs[4], self.loop7,
                                is_ext=True)]
        ...
        for epg in epgs:
            ...
            VppIpInterfaceBind(self.vclient, epg.uplink,
                               epg.rd.t4).add_vpp_config()
            VppIpInterfaceBind(self.vclient, epg.uplink,
                               epg.rd.t6).add_vpp_config()
        ...
        for ep in eps:
            ...
            for fip in ep.fips:
                ba = VppBridgeDomainArpEntry(
                    self.vclient, epg_nat.bd.bd, ep.mac, fip)
                ba.add_vpp_config()
        ...
        self.logger.info(self.vclient.cli(
            "sh vlib graph ip4-gbp-src-classify"))
    ...
    def test_gbp_learn_l2(self):
        ...
        ip_addr = VppIpInterfaceAddress(
            self.vclient, gbd1.bvi, "10.0.0.128", 32)
    ...
    def test_gbp_learn_l3(self):
        ...
        rd1 = VppGbpRouteDomain(self.vclient, 2, 401,
                                t4, t6, tun_ip4_uu, tun_ip6_uu)
        rd1.add_vpp_config()
    ...
    def test_gbp_redirect(self):
        ...
        # Bind the BVI to the RD
        #
        b_ip4 = VppIpInterfaceBind(
            self.vclient, self.loop0, t4).add_vpp_config()
        b_ip6 = VppIpInterfaceBind(
            self.vclient, self.loop0, t6).add_vpp_config()
        ...
        b_lo4_ip4 = VppIpInterfaceBind(
            self.vclient, self.loop4, t4).add_vpp_config()
        b_lo4_ip6 = VppIpInterfaceBind(
            self.vclient, self.loop4, t6).add_vpp_config()
        ebd = VppBridgeDomain(self.vclient, 100)
        ebd.add_vpp_config()
        gebd = VppGbpBridgeDomain(
            self.vclient, ebd, rd1, self.loop4, None, None)
        gebd.add_vpp_config()
        # the external epg
        ...
        VppGbpSubnet(self.vclient, rd1, "2001:10:1::128", 64,
                     ...
                     sclass=765).add_vpp_config()
        # external endpoints
        VppL2Vtr(self.vclient, self.vlan_100,
                 L2_VTR_OP.L2_POP_1).add_vpp_config()
        eep1 = VppGbpEndpoint(
            self.vclient,
            self.vlan_100,
            eepg,
        ...
        eep1.add_vpp_config()
        VppL2Vtr(self.vclient, self.vlan_101,
                 L2_VTR_OP.L2_POP_1).add_vpp_config()
        eep2 = VppGbpEndpoint(
            self.vclient,
            self.vlan_101,
    ...
    def test_gbp_redirect_extended(self):
        ...
        # Bind the BVI to the RD
        #
        b_lo0_ip4 = VppIpInterfaceBind(
            self.vclient, self.loop0, t4).add_vpp_config()
        b_lo0_ip6 = VppIpInterfaceBind(
            self.vclient, self.loop0, t6).add_vpp_config()
        b_lo1_ip4 = VppIpInterfaceBind(
            self.vclient, self.loop1, t4).add_vpp_config()
        b_lo1_ip6 = VppIpInterfaceBind(
            self.vclient, self.loop1, t6).add_vpp_config()
        b_lo2_ip4 = VppIpInterfaceBind(
            self.vclient, self.loop2, t4).add_vpp_config()
        b_lo2_ip6 = VppIpInterfaceBind(
            self.vclient, self.loop2, t6).add_vpp_config()
    ...
    def test_gbp_l3_out(self):
        ...
        bd1.add_vpp_config()
        gbd1 = VppGbpBridgeDomain(
            self.vclient, bd1, rd1, self.loop0, None, tun_bm)
        gbd1.add_vpp_config()
    ...
    def test_gbp_anon_l3_out(self):
        ...
        # Bind the BVI to the RD
        #
        bind_l0_ip4 = VppIpInterfaceBind(
            self.vclient, self.loop0, t4).add_vpp_config()
        bind_l0_ip6 = VppIpInterfaceBind(
            self.vclient, self.loop0, t6).add_vpp_config()
        ...
        bd1.add_vpp_config()
        gbd1 = VppGbpBridgeDomain(
            self.vclient, bd1, rd1, self.loop0, None, None)
        gbd1.add_vpp_config()
        ...
        #
        ext_itf = VppGbpExtItf(
            self.vclient, self.vlan_100, bd1, rd1, anon=True)
        ext_itf.add_vpp_config()
        ext_itf = VppGbpExtItf(
            self.vclient, self.vlan_101, bd1, rd1, anon=True)
        ext_itf.add_vpp_config()
