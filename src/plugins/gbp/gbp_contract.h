		    ...
		    gbp_contract_error_t * err)
{
  ...
  *err = GBP_CONTRACT_ERROR_DROP_CONTRACT;
  ...
  /* check ACL */
  me = match_engine_get (MATCH_SEMANTIC_FIRST, MATCH_TYPE_MASK_N_TUPLE);
  me->mev_match_one (vm, b, contract->gc_app[pnode], &result);

  if (MATCH_RESULT_MISS != result.msr_rule_index)
    goto contract_deny;

  if (PREDICT_FALSE (result.msr_rule_index >= vec_len (contract->gc_rules)))
    {
      *err = GBP_CONTRACT_ERROR_DROP_NO_RULE;
      goto contract_deny;
    }

  *rule = gbp_rule_get (contract->gc_rules[result.msr_rule_index]);
