...
vl_api_gbp_endpoint_add_t_print (vl_api_gbp_endpoint_add_t * a, void *handle)
{
  ...
  s = format (s, "mac %d %U", format_vl_api_mac_address, a->endpoint.mac);
  s = format (s, "tun src %U", format_vl_api_address, a->endpoint.tun.src);
  s = format (s, "tun dst %U", format_vl_api_address, a->endpoint.tun.dst);

  for (int i = 0; i < a->endpoint.n_ips; i++)
    s = format (s, "ips %U", format_vl_api_address, a->endpoint.ips[i]);
				   ...
				   void *handle)
{
  ...
  if (a->is_add)
    s = format (s, "recirc add ");
  else
    s = format (s, "recirc del ");
				   ...
				   void *handle)
{
  ...
  if (a->is_add)
    s = format (s, "subnet add ");
  else
    s = format (s, "subnet del ");
  ...
  s =
    format (s, "prefix %U/%d", format_vl_api_address,
	    a->subnet.prefix.address,
	    ntohl (a->subnet.prefix.address_length));
				     ...
				     void *handle)
{
  ...
  if (a->is_add)
    s = format (s, "contract add ");
  else
    s = format (s, "contract del ");
  s = format (s, "sclass %d", ntohs (a->contract.sclass));
  s = format (s, "dclass %d", ntohs (a->contract.dclass));
  s = format (s, "acl_index %d", ntohl (a->contract.acl_index));
  for (int i = 0; i < a->contract.n_rules; i++)
    {
      s = format (s, "action %d", ntohl (a->contract.rules[i].action));
      s =
	...
	{
	  s =
	    format (s, "nhs ip %U", format_vl_api_address,
	  ...
	  s =
	    format (s, "nhs bd_id %d",
		    a->contract.rules[i].nh_set.nhs[j].bd_id);
	  s =
	    format (s, "nhs rd_id %d",
		    a->contract.rules[i].nh_set.nhs[j].rd_id);
	}
    }
  ...
  for (int i = 0; i < a->contract.n_ether_types; i++)
    {
      s =
	format (s, "ethertypes %d %d", i,
		ntohs (a->contract.allowed_ethertypes[i]));
    }
				    ...
				    void *handle)
{
  ...
  if (a->is_add)
    s = format (s, "ext itf add ");
  else
    s = format (s, "ext itf del ");
