...
static u32
gdb_vxlan_dep_add (const gbp_vxlan_learn_t * gl)
{
...
gdb_vxlan_dep_del (u32 sw_if_index)
{
  ...
  vnet_vxlan_gbp_tunnel_del (sw_if_index);
}
...
gbp_vxlan_learn (const gbp_vxlan_learn_t * gl)
{
  ...
  gt = gbp_vxlan_tunnel_get (gl->gti);
  sw_if_index = gdb_vxlan_dep_add (gl);
  ...
  if (~0 != sw_if_index)
	...
	{
	  gd->gd_itf = gbp_itf_add_and_lock (sw_if_index, gt->gt_bd_index);
...
always_inline void
gbp_vxlan_learn_ip6 (const ip6_address_t * src,
		     const ip6_address_t * dst, u32 vni, index_t gti)
{
...
gbp_vxlan_learn_ip4 (const ip4_address_t * src,
		     const ip4_address_t * dst, u32 vni, index_t gti)
{
  gbp_vxlan_learn_t gl = {
    .src.ip4 = *src,
    .dst.ip4 = *dst,
...
static uword
gbp_vxlan6_decap (vlib_main_t * vm,
		  vlib_node_runtime_t * node, vlib_frame_t * from_frame)
{
  return gbp_vxlan_decap (vm, node, from_frame, 0);
}
...
VLIB_REGISTER_NODE (gbp_vxlan6_input_node) =
{
  .function = gbp_vxlan6_decap,
  .name = "gbp-vxlan6",
  .vector_size = sizeof (u32),
  .n_errors = GBP_VXLAN_N_ERROR,
  .error_strings = gbp_vxlan_error_strings,
...
VLIB_NODE_FUNCTION_MULTIARCH (gbp_vxlan6_input_node, gbp_vxlan6_decap)
/* *INDENT-ON* */
...
format_gbp_vxlan_tunnel (u8 * s, va_list * args)
{
  ...
  s = format (s, "GBP VXLAN tunnel: hw:%d sw:%d vni:%d\n %U",
	      gt->gt_hw_if_index, gt->gt_sw_if_index, gt->gt_vni,
	      format_gbp_vxlan_tunnel_layer, gt->gt_layer);
  if (GBP_VXLAN_TUN_L2 == gt->gt_layer)
    s = format (s, " bd:%d", gt->gt_bd_index);
  else
    s = format (s, " rd:%d", gt->gt_rd_index);
...
int
gbp_vxlan_tunnel_add (u32 vni, u8 is_l2, u32 bd_id,
		      u32 ip4_table_id, u32 ip6_table_id, u32 * sw_if_indexp)
{
  gbp_vxlan_tunnel_t *gt;
  index_t gti;
  uword *p;
  ...
  if (NULL == p)
    {
      ...
      if (is_l2)
	{
	  gt->gt_layer = GBP_VXLAN_TUN_L2;
	  gt->gt_bd_index = bd_find_index (&bd_main, bd_id);
	  /* set it up as a GBP interface */
	  gt->gt_itf =
	    gbp_itf_add_and_lock (gt->gt_sw_if_index, gt->gt_bd_index);
	  gbp_learn_enable (gt->gt_sw_if_index, 1);
	}
      else
	{
	  gt->gt_layer = GBP_VXLAN_TUN_L3;
	  gt->gt_rd_index[FIB_PROTOCOL_IP4] =
	    fib_table_find (FIB_PROTOCOL_IP4, ip4_table_id);
	  gt->gt_rd_index[FIB_PROTOCOL_IP6] =
	    fib_table_find (FIB_PROTOCOL_IP6, ip6_table_id);
	}
...
gbp_vxlan_tunnel_del (u32 vni)
{
  ...
  if (NULL != p)
	...
	if (~0 != gd->gd_sw_if_index)
      ...
      if (GBP_VXLAN_TUN_L2 == gt->gt_layer)
	{
	  gbp_learn_enable (gt->gt_sw_if_index, 0);
