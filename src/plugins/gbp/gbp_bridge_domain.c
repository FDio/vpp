...
#define GBP_BD_DBG(...)                           \
    vlib_log_notice (gb_logger, __VA_ARGS__);
...
index_t
gbp_bridge_domain_find_by_bd_index (u32 bd_index)
{
  if (vec_len (gbp_bridge_domain_db.gbd_by_bd_index) <= bd_index)
    return (INDEX_INVALID);

  return (gbp_bridge_domain_db.gbd_by_bd_index[bd_index]);
}
...
gbp_bridge_domain_add_and_lock (u32 bd_id,
				u32 bvi_sw_if_index, u32 uu_flood_sw_if_index)
{
  gbp_bridge_domain_t *gb;
  index_t gbi;
  ...
  if (INDEX_INVALID == gbi)
	...
	return (VNET_API_ERROR_BD_NOT_MODIFIABLE);
      ...
      gb->gb_uu_flood_sw_if_index = uu_flood_sw_if_index;
      ...
      if (~0 != gb->gb_uu_flood_sw_if_index)
	set_int_l2_mode (vlib_get_main (), vnet_get_main (),
			 MODE_L2_BRIDGE, gb->gb_uu_flood_sw_if_index,
  ...
  if (0 == gb->gb_locks)
    {
      ...
      if (~0 != gb->gb_uu_flood_sw_if_index)
	set_int_l2_mode (vlib_get_main (), vnet_get_main (),
			 MODE_L3, gb->gb_uu_flood_sw_if_index,
  ...
  if (add)
	...
	return clib_error_return (0, "interface must be specified");
      ...
      gbp_bridge_domain_add_and_lock (bd_id,
				      bvi_sw_if_index, uu_flood_sw_if_index);
    }
  else
...
format_gbp_bridge_domain (u8 * s, va_list * args)
{
  ...
  if (NULL != gb)
    s = format (s, "[%d] bd:[%d,%d], bvi:%U uu-flood:%U locks:%d",
                ...
                format_vnet_sw_if_index_name, vnm, gb->gb_uu_flood_sw_if_index,
                gb->gb_locks);
  else
    s = format (s, "NULL");
