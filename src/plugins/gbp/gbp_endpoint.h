...
typedef enum gbp_endpoint_flags_t_
{
  ...
  GBP_ENDPOINT_FLAG_DYNAMIC = (1 << GBP_ENDPOINT_ATTR_DYNAMIC),
} gbp_endpoint_flags_t;

#define GBP_ENDPOINT_ATTR_NAMES { \
    [GBP_ENDPOINT_ATTR_BOUNCE]  = "bounce",      \
    [GBP_ENDPOINT_ATTR_DYNAMIC] = "dynamic",      \
}
...
typedef struct gbp_ep_by_ip_itf_db_t_
{
  ...
  clib_bihash_24_8_t ged_by_ip_itf;
  clib_bihash_16_8_t ged_by_mac_itf;
} gbp_ep_db_t;
...
gbp_endpoint_find_mac (const u8 * mac, u32 bd_index)
{
  ...
  rv = clib_bihash_search_16_8 (&gbp_ep_db.ged_by_mac_itf, &key, &value);
