...
gbp_classify_inline (vlib_main_t * vm,
		     ...
		     vlib_frame_t * frame, u8 is_null, u8 is_l3)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_null)
	    {
	      src_epg = ~0;
	      next0 = vnet_l2_feature_next (b0, gscm->l2_input_feat_next[1],
		...
		{
		  next0 =
		    vnet_l2_feature_next (b0, gscm->l2_input_feat_next[0],
...
{
  return (gbp_classify_inline (vm, node, frame, 0, 0));
}
...
gbp_null_classify (vlib_main_t * vm,
		   vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  return (gbp_classify_inline (vm, node, frame, 1, 0));
}

static uword
gbp_l3_src_classify (vlib_main_t * vm,
		     vlib_node_runtime_t * node, vlib_frame_t * frame)
{
...
gbp_src_classify_init (vlib_main_t * vm)
{
  ...
  feat_bitmap_init_next_nodes (vm,
			       ...
			       em->l2_input_feat_next[0]);
  feat_bitmap_init_next_nodes (vm,
			       gbp_null_classify_node.index,
			       L2INPUT_N_FEAT,
			       l2input_get_feat_names (),
			       em->l2_input_feat_next[1]);
