		     ...
		     gbp_endpoint_flags_t flags, u32 * handle)
{
  ...
  if (NULL != ips && !ip46_address_is_zero(ips))
	...
	if (gbp_endpoint_is_dynamic (ge))
          {
             ...
             * tunnel since the BD may not contain the EP's MAC since it was
             * L3 learned. The dst MAC address used is the 'BD's MAC'.
             */
            ip_neighbor_add (ip, !ip46_address_is_ip4 (ip),
                             gbp_bridge_domain_get_remote_mac(gg->gg_gbd)->bytes,
                             ge->ge_sw_if_index,
	...
	fib_table_entry_path_add (gbp_endpoint_group_get_fib_index
				  (gg, fproto), &pfx, FIB_SOURCE_PLUGIN_HI,
				  FIB_ENTRY_FLAG_NONE,
				  fib_proto_to_dpo (fproto), ip,
				  gbp_endpoint_group_get_bvi (gg), ~0, 1,
...
gbp_endpoint_delete (index_t gei)
{
  ...
  if (NULL != ge->ge_ips)
	...
	ip_neighbor_del (ip, !ip46_address_is_ip4 (ip),
			 gbp_endpoint_group_get_bvi (gg));
	...
	fib_table_entry_path_remove (gbp_endpoint_group_get_fib_index
				     (gg, fproto), &pfx, FIB_SOURCE_PLUGIN_HI,
				     fib_proto_to_dpo (fproto), ip,
				     gbp_endpoint_group_get_bvi (gg), ~0, 1,
