			 ...
			 gbp_rule_t * out)
{
  ...
  rv = gbp_hash_mode_decode (in->hash_mode, &out->gu_hash_mode);

  if (0 != rv)
    return rv;

  vec_validate (out->gu_nhs, in->n_nhs - 1);

  for (ii = 0; ii < in->n_nhs; ii++)
    {
      rv = gbp_next_hop_decode (&in->nhs[ii], &out->gu_nhs[ii]);
...
gbp_contract_rule_decode (const vl_api_gbp_rule_t * in, gbp_rule_t * out)
{
  ...
  rv = gbp_contract_rule_action_deocde (in->action, &out->gu_action);

  if (0 != rv)
    return rv;

  if (GBP_RULE_REDIRECT == out->gu_action)
    {
      rv = gbp_next_hop_set_decode (&in->nh_set, out);
    }
