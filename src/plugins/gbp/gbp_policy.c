...
format_gbp_policy_trace (u8 *s, va_list *args)
{
  ...
  s = format (
    s, "scope:%d sclass:%d, dclass:%d, action:%U flags:%U acl: %d rule: %d",
    t->scope, t->sclass, t->dclass, format_gbp_rule_action, t->action,
    format_vxlan_gbp_header_gpflags, t->flags, t->acl_match, t->rule_match);
...
gbp_policy_init (vlib_main_t *vm)
{
  ...
  feat_bitmap_init_next_nodes (vm, node->index, L2OUTPUT_N_FEAT,
