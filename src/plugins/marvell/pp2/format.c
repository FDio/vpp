...
#define _(c)                                                                  \
  if (stat.c)                                                                 \
    s2 = format (s2, "\n%U%-25U%16Ld", format_white_space, indent + 2,        \
  ...
  vec_foreach_index (i, ppif->inqs)
    {
      ...
      if (vec_len (s2))
	s = format (s, "\n%UInput queue %u statistics:%v", format_white_space,
		    indent, i, s2);
      vec_reset_length (s2);
    }
  vec_foreach_index (i, ppif->outqs)
    {
      ...
      if (vec_len (s2))
	s = format (s, "\n%UOutput queue %u statistics:%v", format_white_space,
		    indent, i, s2);
      vec_reset_length (s2);
    }
#undef _
...
format_mrvl_pp2_input_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "pp2: %v (%d) next-node %U", hi->name, t->hw_if_index,
