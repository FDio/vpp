		 ...
		 int is_ip6, u8 * r_action, u32 * trace_bitmap)
{
  ...
  *r_action = 0;    /* default deny */
  ...
  vec_foreach_index (i, if_config->policies) {
    // If policies are configured, stop evaluation before profiles
    if (if_config->pass_id > 0 && i >= if_config->pass_id && !passed)
        return 1;
...
capo_match_policy (capo_policy_t *policy, u32 is_inbound, u32 is_ip6, fa_5tuple_t *pkt_5tuple)
{
  ...
  u32 *rules = is_inbound ? policy->rule_ids[VLIB_TX] : policy->rule_ids[VLIB_RX];
  u32 *rule_id;
  capo_rule_t *rule;
  int r;
	...
	if (l4proto == IP_PROTOCOL_ICMP || l4proto == IP_PROTOCOL_ICMP6)
	  {
	    ...
	    if (!filter->should_match && filter->value == type)
	      return -1;
           ...
           // A rule with an ICMP type / code specified doesn't match a non-icmp packet
           return -1;
	break;
