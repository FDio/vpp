...
int capo_match_policy (capo_policy_t *policy, u32 is_inbound, u32 is_ip6, fa_5tuple_t *pkt_5tuple);
int capo_match_rule (capo_rule_t *rule, u32 is_ip6, fa_5tuple_t *pkt_5tuple);

u8 ip_ipset_contains_ip4 (capo_ipset_t *ipset, ip4_address_t *addr);
u8 ip_ipset_contains_ip6 (capo_ipset_t *ipset, ip6_address_t *addr);
u8 ipport_ipset_contains_ip4 (capo_ipset_t *ipset, ip4_address_t *addr, u8 l4proto, u16 port);
u8 ipport_ipset_contains_ip6 (capo_ipset_t *ipset, ip6_address_t *addr, u8 l4proto, u16 port);
		 ...
		 int is_ip6, u8 * r_action, u32 * trace_bitmap)
{
  ...
  clib_bihash_kv_4_12_t conf_kv;
  capo_interface_config_t *if_config;
  capo_policy_t *policy;
  int r;
  u32 i;

  if (clib_bihash_search_4_12 (capo_main.if_config, &conf_kv, &conf_kv) != 0)
    {
      // no config for this interface found
      return 0;
    }
  if_config = (capo_interface_config_t *) conf_kv.value;

  *r_action = 0;    /* default deny */

  /* *INDENT-OFF* */
  pool_foreach_index (i, if_config->policies, ({
    policy = &capo_policies[if_config->policies[i]];
    ...
    switch (r) {
        ...
        case CAPO_DENY:
            return 1;
        case CAPO_PASS:
            if (i >= if_config->pass_id) {
                clib_warning ("BUG: PASS action in profiles");
                return 1;
            }
            i = if_config->pass_id;
            break;
...
int
capo_match_policy (capo_policy_t *policy, u32 is_inbound, u32 is_ip6, fa_5tuple_t *pkt_5tuple)
{
  u32 *rules = is_inbound ? policy->rule_ids[VLIB_RX] : policy->rule_ids[VLIB_TX];
  u32 *rule_id;
  capo_rule_t *rule;
  int r;

  /* *INDENT-OFF* */
  pool_foreach (rule_id, rules, ({
...
capo_match_rule (capo_rule_t *rule, u32 is_ip6, fa_5tuple_t *pkt_5tuple)
{
  ...
  vec_foreach (filter, rule->filters) {
    ...
    default:
        ...
        break;
    /* TODO: icmp     Q: is there always a match on L4 proto == icmp with icmp matches? */
    }
  }
  ...
  if (rule->prefixes[CAPO_SRC])
  {
      ip_prefix_t *prefix;
      u8 found = 0;
      pool_foreach (prefix, rule->prefixes[CAPO_SRC], ({
          if (is_ip6) {
            if (ip6_destination_matches_route (&ip6_main, src_ip6,
                &ip_addr_v6(&prefix->addr), prefix->len)) {
                found = 1;
                break;
            }
          } else {
            if (ip4_destination_matches_route (&ip4_main, src_ip4,
                &ip_addr_v4(&prefix->addr), prefix->len)) {
                found = 1;
                break;
            }
          }
  ...
  {
      ip_prefix_t *prefix;
      pool_foreach (prefix, rule->prefixes[CAPO_NOT_SRC], ({
          if (is_ip6) {
            if (ip6_destination_matches_route (&ip6_main, src_ip6,
                &ip_addr_v6(&prefix->addr), prefix->len)) {
                return -1;
  ...
  if (rule->prefixes[CAPO_DST])
  {
      ip_prefix_t *prefix;
      u8 found = 0;
      pool_foreach (prefix, rule->prefixes[CAPO_DST], ({
          if (is_ip6) {
  ...
  {
      ip_prefix_t *prefix;
      pool_foreach (prefix, rule->prefixes[CAPO_NOT_DST], ({
          if (is_ip6) {
            if (ip6_destination_matches_route (&ip6_main, dst_ip6,
                &ip_addr_v6(&prefix->addr), prefix->len)) {
                return -1;
  ...
  if (rule->ip_ipsets[CAPO_SRC])
  {
      u32 *ipset;
      u8 found = 0;
      pool_foreach (ipset, rule->ip_ipsets[CAPO_SRC], ({
          if (is_ip6) {
            if (ip_ipset_contains_ip6(&capo_ipsets[*ipset], src_ip6)) {
                found = 1;
                break;
            }
          } else {
            if (ip_ipset_contains_ip4(&capo_ipsets[*ipset], src_ip4)) {
                found = 1;
                break;
            }
          }
  ...
  {
      u32 *ipset;
      pool_foreach (ipset, rule->ip_ipsets[CAPO_NOT_SRC], ({
          if (is_ip6) {
            if (ip_ipset_contains_ip6(&capo_ipsets[*ipset], src_ip6)) {
                return -1;
            }
  ...
  if (rule->ip_ipsets[CAPO_DST])
  {
      u32 *ipset;
      u8 found = 0;
      pool_foreach (ipset, rule->ip_ipsets[CAPO_DST], ({
          if (is_ip6) {
            if (ip_ipset_contains_ip6(&capo_ipsets[*ipset], dst_ip6)) {
                found = 1;
                break;
            }
          } else {
            if (ip_ipset_contains_ip4(&capo_ipsets[*ipset], dst_ip4)) {
                found = 1;
                break;
            }
          }
  ...
  {
      u32 *ipset;
      pool_foreach (ipset, rule->ip_ipsets[CAPO_NOT_DST], ({
          if (is_ip6) {
            if (ip_ipset_contains_ip6(&capo_ipsets[*ipset], dst_ip6)) {
                return -1;
            }
  ...
  if (rule->port_ranges[CAPO_SRC])
  {
      ...
      pool_foreach (range, rule->port_ranges[CAPO_SRC], ({
  ...
  {
      capo_port_range_t *range;
      pool_foreach (range, rule->port_ranges[CAPO_NOT_SRC], ({
          if (range->start <= src_port && src_port <= range->end) {
              return -1;
          }
      }));
  ...
  {
      capo_port_range_t *range;
      pool_foreach (range, rule->port_ranges[CAPO_DST], ({
  ...
  {
      capo_port_range_t *range;
      pool_foreach (range, rule->port_ranges[CAPO_NOT_DST], ({
          if (range->start <= dst_port && dst_port <= range->end) {
              return -1;
          }
      }));
  ...
  {
      u32 *ipset;
      pool_foreach (ipset, rule->ipport_ipsets[CAPO_SRC], ({
          if (is_ip6) {
            if (ipport_ipset_contains_ip6(&capo_ipsets[*ipset], src_ip6, l4proto, src_port)) {
                src_port_found = 1;
                break;
            }
          } else {
            if (ipport_ipset_contains_ip4(&capo_ipsets[*ipset], src_ip4, l4proto, src_port)) {
                src_port_found = 1;
                break;
            }
          }
  ...
  {
      u32 *ipset;
      pool_foreach (ipset, rule->ipport_ipsets[CAPO_NOT_SRC], ({
          if (is_ip6) {
            if (ipport_ipset_contains_ip6(&capo_ipsets[*ipset], src_ip6, l4proto, src_port)) {
                return -1;
            }
  ...
  {
      u32 *ipset;
      pool_foreach (ipset, rule->ipport_ipsets[CAPO_DST], ({
          if (is_ip6) {
            if (ipport_ipset_contains_ip6(&capo_ipsets[*ipset], dst_ip6, l4proto, dst_port)) {
                dst_port_found = 1;
                break;
            }
          } else {
            if (ipport_ipset_contains_ip4(&capo_ipsets[*ipset], dst_ip4, l4proto, dst_port)) {
                dst_port_found = 1;
                break;
            }
          }
  ...
  {
      u32 *ipset;
      pool_foreach (ipset, rule->ipport_ipsets[CAPO_NOT_DST], ({
          if (is_ip6) {
            if (ipport_ipset_contains_ip6(&capo_ipsets[*ipset], dst_ip6, l4proto, dst_port)) {
                return -1;
            }
  ...
  if ((rule->port_ranges[CAPO_SRC] || rule->ipport_ipsets[CAPO_SRC]) && (!src_port_found)) {
      return -1;
  }
  if ((rule->port_ranges[CAPO_DST] || rule->ipport_ipsets[CAPO_DST]) && (!dst_port_found)) {
      return -1;
  }
...
u8 ipport_ipset_contains_ip4 (capo_ipset_t *ipset, ip4_address_t *addr, u8 l4proto, u16 port)
{
  ...
  pool_foreach (member, ipset->members, ({
      ...
      if (l4proto == member->ipport.l4proto &&
          port == member->ipport.port &&
