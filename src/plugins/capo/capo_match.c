...
//   }
  ...
  if (rule->ipport_ipsets[CAPO_SRC])
	...
	if (is_ip6)
	  {
	    if (ipport_ipset_contains_ip6
		(&capo_ipsets[*ipset], src_ip6, l4proto, src_port))
	      {
		src_port_found = 1;
		break;
	      }
	  ...
	  {
	    if (ipport_ipset_contains_ip4
		(&capo_ipsets[*ipset], src_ip4, l4proto, src_port))
	      {
		src_port_found = 1;
		break;
	      }
  ...
  if (rule->ipport_ipsets[CAPO_DST])
	...
	if (is_ip6)
	  {
	    if (ipport_ipset_contains_ip6
		(&capo_ipsets[*ipset], dst_ip6, l4proto, dst_port))
	      {
		dst_port_found = 1;
		break;
	      }
	  ...
	  {
	    if (ipport_ipset_contains_ip4
		(&capo_ipsets[*ipset], dst_ip4, l4proto, dst_port))
	      {
		dst_port_found = 1;
		break;
	      }
...
ip_ipset_contains_ip4 (capo_ipset_t * ipset, ip4_address_t * addr)
{
  ...
  pool_foreach (member, ipset->members, (
					  {
					  if (member->address.version !=
					      AF_IP4)
					  {
					  continue;}
					  if (!ip4_address_compare
					      (addr,
					       &ip_addr_v4
					       (&member->address)))
					  {
			   ...
			   u8 l4proto, u16 port)
{
  ...
  pool_foreach (member, ipset->members, (
					  {
					  if (member->ipport.addr.version !=
					      AF_IP4)
					  {
					  continue;}
					  if (l4proto ==
					      member->ipport.l4proto
					      && port == member->ipport.port
					      && !ip4_address_compare (addr,
								       &ip_addr_v4
								       (&member->ipport.addr)))
					  {
					  return 1;}
