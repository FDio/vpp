			 ...
			 u32 num_profiles, u32 * policy_ids)
{
  ...
  if (!found)
    {
      capo_main.
	acl_plugin.wip_add_del_custom_access_io_policy (1 /* is_add */ ,
							sw_if_index,
							0 /* is_input */ ,
							capo_match_func);
      capo_main.
...
capo_sw_interface_add_del (vnet_main_t * vnm, u32 sw_if_index, u32 is_add)
{
  ...
  rv =
    capo_main.acl_plugin.wip_add_del_custom_access_io_policy (0 /* is_add */ ,
							      sw_if_index, 0	/* is_input */
							      ,
							      capo_match_func);
  if (rv)
    clib_warning ("error deleting caiop: %d", rv);
  rv =
    capo_main.acl_plugin.wip_add_del_custom_access_io_policy (0 /* is_add */ ,
							      sw_if_index, 1	/* is_input */
							      ,
							      capo_match_func);
  if (rv)
    clib_warning ("error deleting caiop: %d", rv);
			     ...
			     vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat
	  (line_input, "%U", unformat_sw_if_index, NULL, &sw_if_index))
	;
				 ...
				 vlib_cli_command_t * cmd)
{
  ...
  rv = capo_configure_policies (sw_if_index, num_ingress, num_egress,
				vec_len (policy_list) - num_ingress -
				num_egress, policy_list);
...
VLIB_CLI_COMMAND (capo_interface_configure_cmd, static) = {
  ...
  .short_help = "capo interface configure [interface | sw_if_index N] in <num_ingress> out <num_egress> <policy_id> ...",
};
/* *INDENT-ON* */
