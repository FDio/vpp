...
format_capo_policy (u8 *s, va_list *args)
{
  ...
  /**
   * VLIB_RX from the point of view of VPP, is actually
   * egress from the container, and VLIB_TX from the point
   * of view of VPP is actually ingress in a container
   */
  if (verbose)
    {
