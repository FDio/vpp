...
class TestCapo(VppTestCase):
    ...
    def capo_test_icmp(self, is_v6):
        ...
        # Create policy with inbound rule
        ...
        # Configure policy on pg1, pass=1 to have default deny behavior
        self.configure_policies( [ policy0 ], interface=self.pg1, pass_policy_id=1)
        ...
        self.send_and_assert_no_replies(self.pg0, icmp_echo_request * 1, self.pg1)
        
        # TODO changing rules should probably wipe sessions in VPP?
        self.vapi.cli("clear acl-plugin sessions")

        # Define filter on ICMP type  / should match = 0
        filters = [
            VppCapoFilter(VppEnum.vl_api_capo_rule_filter_type_t.CAPO_RULE_FILTER_ICMP_TYPE,
                          value=11, should_match=0),
            VppCapoFilter(), 
            VppCapoFilter()]
        
        rule_icmp_type.capo_rule_update(action=VppEnum.vl_api_capo_rule_action_t.CAPO_ALLOW, 
                        filters=filters)

        # send icmp type 11 and expect the packet is filtered
        icmp_echo_request = (Ether(src=self.pg0.remote_mac,
                                dst=self.pg0.local_mac) /
                          IP46(src=client_addr, dst=remote_addr) /
        ...
        policy0.capo_policy_delete()
        rule_icmp_type.capo_rule_delete()
    ...
    def capo_test_l4proto(self, is_v6, l4proto):
        ...
        pkt = (Ether(src=self.pg0.remote_mac,
                          ...
                          ICMP(type=8, code=3) /
                          Raw(load=b'\x0a' * 18))
        self.send_and_assert_no_replies(self.pg0, pkt * 1, self.pg1)

        self.vapi.cli("clear acl-plugin sessions")

        # Define filter on l4proto+1 / should match = 0
        filters = [
            VppCapoFilter(VppEnum.vl_api_capo_rule_filter_type_t.CAPO_RULE_FILTER_L4_PROTO,
            ...
            VppCapoFilter()]
        # update rule
        rule_l4_proto.capo_rule_update(action=VppEnum.vl_api_capo_rule_action_t.CAPO_ALLOW, 
                        filters=filters)
        ...
        self.configure_policies([], interface=self.pg1, pass_policy_id=0)
        policy0.capo_policy_delete()
        rule_l4_proto.capo_rule_delete()
