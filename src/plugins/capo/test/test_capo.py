...
def send_test_packets(test, from_if, to_if, passing_packets, dropped_packets):
    if len(passing_packets) > 0:
        rxl = test.send_and_expect(from_if, passing_packets, to_if)
        test.assertEqual(len(rxl), len(passing_packets))
        for i in range(len(passing_packets)):
            ...
            if IP in tx:
                ...
                del tx.hlim, rx.hlim
            test.assertEqual(rx, tx)
    if len(dropped_packets) > 0:
        test.send_and_assert_no_replies(from_if, dropped_packets, to_if, timeout=0.1)
    test.vapi.cli("clear acl-plugin sessions")
...
class TestCapoMatches(VppTestCase):
    ...
    def setUp(self):
        ...
        self.configure_policies([self.policy], interface=self.pg1, pass_policy_id=1)
    ...
    def tearDown(self):
        ...
        self.configure_policies([], interface=self.pg1, pass_policy_id=0)
    ...
    def do_test_one_rule(self, filters, matches, matching_packets, not_matching_packets):
        ...
        send_test_packets(self, self.pg0, self.pg1, matching_packets, not_matching_packets)
...
class TestCapoPolicies(VppTestCase):
    ...
    def test_default_verdict(self):
        # If profiles only are configured (pass_id = 0), default is allow
    ...
    def test_realistic_policy(self):
        ...
        r = self.vapi.capo_configure_policies(self.pg1.sw_if_index, 1, 1, [policy._policy_id])
        self.assertEqual(0, r.retval)
        base = Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac) / IP(src=self.pg0.remote_ip4,
                dst=self.pg1.remote_ip4)
        base_other_src = Ether(src=self.pg0.remote_hosts[1].mac, dst=self.pg0.local_mac) / IP(
                src=self.pg0.remote_hosts[1].ip4, dst=self.pg1.remote_ip4)
        passing = [
            base / ICMP(type=8),
            base_other_src / ICMP(type=8),
            base / TCP(sport=1, dport=8080) / random_payload(),
        ]
        dropped = [
            base / ICMP(type=3),
            base_other_src / TCP(sport=1, dport=8080) / random_payload(),
            base / UDP(sport=1, dport=8080) / random_payload(),
            base / TCP(sport=1, dport=8081) / random_payload(),
        ]
        send_test_packets(self, self.pg0, self.pg1, passing, dropped)
        # Cleanup
        r = self.vapi.capo_configure_policies(self.pg1.sw_if_index, 1, 1, [policy._policy_id])
        self.assertEqual(0, r.retval)
