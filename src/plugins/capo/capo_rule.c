...
format_capo_rule_entry (u8 * s, va_list * args)
{
  ...
  switch (entry->type)
    ...
    case CAPO_PORT_RANGE:
      s = format (s, "[%u-%u]",
		  entry->data.port_range.start, entry->data.port_range.end);
      break;
    case CAPO_IP_SET:
...
unformat_capo_rule_entry (unformat_input_t * input, va_list * args)
{
  ...
  if (unformat (input, "%U %U",
		unformat_rule_key_flag, &entry->flags,
...
unformat_capo_rule_filter (unformat_input_t * input, va_list * args)
{
  ...
  else if (unformat (input, "icmp-code%U%d",
		     unformat_capo_should_match, &filter->should_match,
		     &filter->value))
...
format_capo_rule (u8 * s, va_list * args)
{
  ...
  s = format (s, "[%d] %U %U\n",
	      rule - capo_rules,
	      format_ip_address_family, rule->af,
...
static clib_error_t *
capo_rules_show_cmd_fn (vlib_main_t * vm,
...
static clib_error_t *
capo_rules_add_cmd_fn (vlib_main_t * vm,
		       unformat_input_t * input, vlib_cli_command_t * cmd)
{
...
static clib_error_t *
capo_rules_del_cmd_fn (vlib_main_t * vm,
		       unformat_input_t * input, vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  clib_error_t *error = 0;
