#ifndef __included_gpcapng_filter_h__
#define __included_gpcapng_filter_h__
...
struct geneve_option_def_t
{
  ...
  geneve_opt_data_type_t
    preferred_type; /* Preferred input type for this option */
  format_function_t
    *format_fn; /* Optional format function for displaying option */
};

/* API declarations */
// static clib_error_t *gpcapng_init (vlib_main_t * vm);
void gpcapng_register_option_def (const char *name, u16 class, u8 type,
				  u8 length,
				  geneve_opt_data_type_t preferred_type);
int gpcapng_add_filter (u32 sw_if_index, const geneve_capture_filter_t *filter,
			u8 is_global);
...
struct geneve_option_filter_t
{
  ...
  u8
    data_len; /* Length of data to match (can be shorter than actual option) */
...
struct geneve_capture_filter_t
{
  ...
  u32
    destination_index; /* output destination index for the filter, 0 = first */
...
#endif /* __included_gpcapng_filter_h__ */
