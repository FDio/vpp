...
parse_ipv4_prefix (unformat_input_t *input, va_list *args)
{
  ...
  clib_warning ("Parsed address: %U prefix len %d", format_ip4_address, &ip4,
		prefix_len);
...
static clib_error_t *
parse_hex_byte (unformat_input_t *input, u8 **value, u8 **mask, u8 offset,
		u8 len)
{
  u8 i, byte_val;
...
static clib_error_t *
create_ipv4_5tuple_filter (unformat_input_t *input,
			   geneve_tuple_filter_t *filter)
{
  ...
  value[0] = 0x40;
  mask[0] = 0xf0;
  clib_warning ("Attempt to parse filter");

  /* Parse fields in any order */
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "src-ip %U", parse_ipv4_prefix, &value, &mask,
		    IP4_SRC_IP_OFFSET))
	{
	  /* Source IP already parsed */
	}
      else if (unformat (input, "dst-ip %U", parse_ipv4_prefix, &value, &mask,
			 IP4_DST_IP_OFFSET))
	{
	  /* Destination IP already parsed */
	}
      else if (unformat (input, "src-port %U", parse_port, &value, &mask,
			 IP4_SRC_PORT_OFFSET))
	{
	  /* Source port already parsed */
	}
      else if (unformat (input, "dst-port %U", parse_port, &value, &mask,
			 IP4_DST_PORT_OFFSET))
	{
	  /* Destination port already parsed */
	}
      else if (unformat (input, "proto %U", parse_protocol, &value, &mask,
			 IP4_PROTO_OFFSET))
	{
	  /* Protocol already parsed */
	}
      else if (unformat (input, "raw %U", parse_hex_byte, &value, &mask, 0,
			 vec_len (value)))
	{
...
static clib_error_t *
create_ipv6_5tuple_filter (unformat_input_t *input,
			   geneve_tuple_filter_t *filter)
{
  clib_error_t *error = NULL;
  u8 *value = filter->value;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "src-ip %U", parse_ipv6_prefix, &value, &mask,
		    IP6_SRC_IP_OFFSET))
	{
	  /* Source IP already parsed */
	}
      else if (unformat (input, "dst-ip %U", parse_ipv6_prefix, &value, &mask,
			 IP6_DST_IP_OFFSET))
	{
	  /* Destination IP already parsed */
	}
...
int
gpcapng_add_filter (u32 sw_if_index, const geneve_capture_filter_t *filter,
		    u8 is_global)
{
  ...
  if (!is_global)
	...
	return VNET_API_ERROR_INVALID_SW_IF_INDEX;
      ...
      /* Ensure we have space for this interface in simple filter data */
      vec_validate (sfm->per_interface, sw_if_index);
    }
  ...
  if (is_global)
    {
      vec_add2 (sfm->global_filters, new_filter, 1);
    }
  else
    {
      vec_add2 (sfm->per_interface[sw_if_index].filters, new_filter, 1);
    }
  ...
  if (filter->option_filters)
      ...
      for (i = 0; i < vec_len (filter->option_filters); i++)
	{
	  ...
	  if (filter->option_filters[i].data)
	    {
	      new_filter->option_filters[i].data = vec_dup_aligned (
		filter->option_filters[i].data, CLIB_CACHE_LINE_BYTES);
	    }

	  if (filter->option_filters[i].mask)
	    {
	      new_filter->option_filters[i].mask = vec_dup_aligned (
		filter->option_filters[i].mask, CLIB_CACHE_LINE_BYTES);
	    }
	}
    }
...
gpcapng_del_filter (u32 sw_if_index, u32 filter_id, u8 is_global)
{
  ...
  if (is_global)
    {
      filters = sfm->global_filters;
    }
  else
    {
      /* Check interface exists */
      if (sw_if_index >= vec_len (sfm->per_interface))
	return VNET_API_ERROR_INVALID_SW_IF_INDEX;

      filters = sfm->per_interface[sw_if_index].filters;
    }
  ...
  for (i = 0; i < vec_len (filters); i++)
      ...
      if (filters[i].filter_id == filter_id)
	{
	  ...
	  if (is_global)
	    {
	      vec_delete (sfm->global_filters, 1, i);
	    }
	  else
	    {
	      vec_delete (sfm->per_interface[sw_if_index].filters, 1, i);
	    }
...
int
gpcapng_del_filter_by_name (u32 sw_if_index, char *filter_name, u8 is_global)
{
  simple_filter_main_t *sfm = get_simple_filter_main ();
  geneve_capture_filter_t *filters;
  int i;
  ...
  for (i = 0; i < vec_len (filters); i++)
    {
      if (clib_strcmp (filters[i].name, filter_name) == 0)
	{
	  /* Cleanup option filters */
	  if (filters[i].option_filters)
	    {
...
format_tuple_filter (u8 *s, va_list *args)
{
  ...
  if (filter->length > proto_offset && proto_offset &&
      filter->mask[proto_offset])
    {
	...
	s =
	  format (s, "0x%x (mask 0x%x)\n", proto, filter->mask[proto_offset]);

      /* Ports */
      if (filter->length > src_port_offset + 1 &&
	  (filter->mask[src_port_offset] || filter->mask[src_port_offset + 1]))
	{
	  u16 src_port = 0;
      ...
      if (filter->length > dst_port_offset + 1 &&
	  (filter->mask[dst_port_offset] || filter->mask[dst_port_offset + 1]))
	{
	  u16 dst_port = 0;
			     ...
			     geneve_opt_data_type_t preferred_type)
{
  ...
  /* Add to vector */
  index = vec_len (sfm->option_defs);
  vec_add1 (sfm->option_defs, opt_def);

  /* Add to hash tables */
  hash_set_mem (sfm->option_by_name, opt_def.option_name, index);

  key = ((u64) class << 8) | type;
  hash_set (sfm->option_by_class_type, key, index);
}
  ...
  switch (type)
      ...
      for (i = 0; i < data_len; i++)
	{
	  if (!unformat (input, "%x", &byte_val))
		...
		0, "invalid raw format, expected %d hex bytes", data_len);
	    }
	  raw_data[i] = byte_val;
	}
      *data = raw_data;
...
static clib_error_t *
gpcapng_filter_command_fn (vlib_main_t *vm, unformat_input_t *input,
			   vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (line_input, "option %s", &option_name))
	{
	  ...
	  p = hash_get_mem (sfm->option_by_name, option_name);
	  if (!p)
	    {
	      error =
		clib_error_return (0, "unknown option name: %s", option_name);
	      goto done;
	    }
	  ...
	  if (unformat (line_input, "any"))
		...
		data_type = GENEVE_OPT_TYPE_STRING;
	      ...
	      error = parse_option_data (
		line_input, data_type, &opt_filter.data, opt_filter.data_len);
	      if (error)
		goto done;
		...
		{
		  error =
		    parse_option_data (line_input, GENEVE_OPT_TYPE_RAW,
			 ...
			 &filter.option_filters->type))
	{
	  ...
	  p = hash_get (sfm->option_by_class_type, key);
	  ...
	  if (p)
	      ...
	      else if (unformat (line_input, "value"))
		{
		  ...
		  opt_filter.data_len = opt_def->length;
		  error =
		    parse_option_data (line_input, data_type, &opt_filter.data,
				       opt_filter.data_len);
		  if (error)
		    goto done;
		  ...
		  if (unformat (line_input, "mask"))
		    {
		      error = parse_option_data (
			line_input, GENEVE_OPT_TYPE_RAW, &opt_filter.mask,
			opt_filter.data_len);
		      if (error)
			goto done;
		    }
	      ...
	      if (unformat (line_input, "any"))
		{
	      ...
	      else if (unformat (line_input, "length %d",
				 &opt_filter.data_len))
		{
		  if (unformat (line_input, "value"))
		    {
		      error = parse_option_data (
			line_input, GENEVE_OPT_TYPE_RAW, &opt_filter.data,
			opt_filter.data_len);
		      if (error)
			goto done;
  ...
  if (is_add)
      ...
      if (filter_id != ~0)
	{
	  ...
	  if (rv < 0)
	    {
	      error = clib_error_return (0, "failed to delete filter (id: %d)",
					 filter_id);
	      goto done;
	    }
	...
	{
	  int rv =
	    gpcapng_del_filter_by_name (sw_if_index, filter_name, is_global);
	  if (rv < 0)
	    {
	      error = clib_error_return (
		0, "failed to delete filter (name: %v)", filter_name);
	      goto done;
	    }

	  vlib_cli_output (vm, "Deleted GENEVE %s filter with name: %v",
			   is_global ? "global" : "interface", filter_name);
	}
    }
...
VLIB_CLI_COMMAND (gpcapng_filter_command, static) = {
  .path = "gpcapng filter",
  .short_help =
    "gpcapng filter name <filtername> [interface <interface> | global] "
    "[ver <ver>] [opt-len <len>] [protocol <proto>] [vni <vni>] "
    "[outer-ipv4 | outer-ipv6 | inner-ipv4 | inner-ipv6] "
    "[option <name> [any|value [raw|ipv4|ipv6|uint8|uint16|uint32|string] "
    "<data> [mask <mask>]]] "
    "[option-direct class <class> type <type> [any|value "
    "[raw|ipv4|ipv6|uint8|uint16|uint32|string] <data> [mask <mask>]]] "
    "[del id <id>]",
...
register_default_options (void)
{
  ...
  gpcapng_register_option_def ("vpp-metadata", 0x0123, 0x01, 8,
			       GENEVE_OPT_TYPE_UINT32);
  gpcapng_register_option_def ("legacy-oam", 0x0F0F, 0x01, 4,
			       GENEVE_OPT_TYPE_UINT32);
  gpcapng_register_option_def ("tenant-ip", 0x0124, 0x02, 4,
			       GENEVE_OPT_TYPE_IPV4);
  gpcapng_register_option_def ("tenant-ipv6", 0x0124, 0x03, 16,
			       GENEVE_OPT_TYPE_IPV6);
  gpcapng_register_option_def ("flow-id", 0x0125, 0x01, 4,
			       GENEVE_OPT_TYPE_UINT32);
  gpcapng_register_option_def ("app-id", 0x0125, 0x02, 2,
			       GENEVE_OPT_TYPE_UINT16);
  gpcapng_register_option_def ("service-tag", 0x0126, 0x01, 8,
			       GENEVE_OPT_TYPE_STRING);
}

static clib_error_t *
gpcapng_register_option_command_fn (vlib_main_t *vm, unformat_input_t *input,
				    vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  clib_error_t *error = NULL;
  ...
  switch (data_type)
	...
	{
	  error = clib_error_return (
	    0, "length must be at least 4 bytes for IPv4 data type");
	  goto done;
	}
      break;
	...
	{
	  error = clib_error_return (
	    0, "length must be at least 16 bytes for IPv6 data type");
	  goto done;
	}
      break;
	...
	{
	  error = clib_error_return (
	    0, "length must be at least 1 byte for uint8 data type");
	  goto done;
	}
      break;
	...
	{
	  error = clib_error_return (
	    0, "length must be at least 2 bytes for uint16 data type");
	  goto done;
	}
      break;
	...
	{
	  error = clib_error_return (
	    0, "length must be at least 4 bytes for uint32 data type");
	  goto done;
	}
      break;
	...
	{
	  error = clib_error_return (
	    0, "length must be at least 1 byte for string data type");
	  goto done;
	}
      break;
  ...
  /* Register the option */
  gpcapng_register_option_def ((char *) name, opt_class, type, length,
			       data_type);
  vlib_cli_output (vm,
		   "Registered GENEVE option: name=%s, class=0x%x, type=0x%x, "
		   "length=%d, data-type=%s",
		   ...
		   data_type == GENEVE_OPT_TYPE_STRING ? "string" :
							 "unknown");
...
{
  geneve_opt_data_type_t type =
    va_arg (*args, int); /* enum is promoted to int */
...
static clib_error_t *
gpcapng_show_options_command_fn (vlib_main_t *vm, unformat_input_t *input,
				 vlib_cli_command_t *cmd)
{
  simple_filter_main_t *sfm = get_simple_filter_main ();
  u32 i;
  ...
  vlib_cli_output (vm, "%-20s %-10s %-10s %-10s %s", "Name", "Class", "Type",
		   "Length", "Data Type");
  vlib_cli_output (vm, "%-20s %-10s %-10s %-10s %s", "--------------------",
		   "----------", "----------", "----------", "----------");

  /* Display all registered options */
  for (i = 0; i < vec_len (sfm->option_defs); i++)
    {
      geneve_option_def_t *opt = &sfm->option_defs[i];
      vlib_cli_output (vm, "%-20s 0x%-8x %-10u %-10u %U", opt->option_name,
		       opt->opt_class, opt->type, opt->length,
...
static clib_error_t *
gpcapng_show_filters_command_fn (vlib_main_t *vm, unformat_input_t *input,
				 vlib_cli_command_t *cmd)
{
  simple_filter_main_t *sfm = get_simple_filter_main ();
  u32 sw_if_index;
  ...
  if (vec_len (sfm->global_filters) > 0)
	...
	{
	  geneve_capture_filter_t *filter = &sfm->global_filters[i];

	  vlib_cli_output (vm, "  Filter Name: %s", filter->name);
	  vlib_cli_output (vm, "  Filter ID: %u", filter->filter_id);
	  vlib_cli_output (vm, "  Destination Output Index: %u",
	  ...
	  if (filter->option_filters)
	      ...
	      for (j = 0; j < vec_len (filter->option_filters); j++)
		{
		  ...
		  if (filter->option_filters[j].option_name)
		    {
		      ...
		      uword *p =
			hash_get_mem (sfm->option_by_name,
				      filter->option_filters[j].option_name);
		      if (p)
			{
			  geneve_option_def_t *opt_def =
			    &sfm->option_defs[p[0]];
			...
			{
			  /* This shouldn't happen if validation was done at
			   * filter creation */
			  opt_class = 0;
			  opt_type = 0;
			  name =
			    (char *) filter->option_filters[j].option_name;
			}
		    }
		  else
		    {
		      ...
		      uword *p = hash_get (sfm->option_by_class_type, key);
		      if (p)
			{
			  geneve_option_def_t *opt_def =
			    &sfm->option_defs[p[0]];
		  ...
		  if (name)
		    vlib_cli_output (
		      vm, "      Option: %s (class=0x%x, type=0x%x)", name,
		      opt_class, opt_type);
		  else
  ...
  if (init_result)
    {
      vlib_cli_output (vm, "Error accessing gpcapng plugin: %s",
		       init_result->what);
      return 0;
    }

  uword *capture_bitmap = gpcapng_plugin.get_capture_enabled_bitmap ();
  for (sw_if_index = 0; sw_if_index < vec_len (sfm->per_interface);
       sw_if_index++)
    {
      if (sfm->per_interface[sw_if_index].filters == 0)
	continue;

      if (vec_len (sfm->per_interface[sw_if_index].filters) == 0)
	continue;
	...
	format_vnet_sw_interface_name, vnet_get_main (), sw, sw_if_index,
	clib_bitmap_get (capture_bitmap, sw_if_index) ? "enabled" :
							"disabled");

      /* Display each filter on this interface */
      for (i = 0; i < vec_len (sfm->per_interface[sw_if_index].filters); i++)
	{
	  geneve_capture_filter_t *filter =
	    &sfm->per_interface[sw_if_index].filters[i];
...
VLIB_CLI_COMMAND (gpcapng_register_option_command, static) = {
  ...
  .short_help = "gpcapng register-option name <name> class <class> type "
		"<type> length <length>"
