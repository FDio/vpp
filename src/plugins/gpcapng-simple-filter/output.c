...
find_global_filter_by_name (const char *name)
{
  ...
  if (!name || !sfm->global_filters)
    return NULL;

  vec_foreach (filter, sfm->global_filters)
    {
...
find_interface_filter_by_name (const char *name, u32 *found_sw_if_index)
{
  ...
  if (!name || !sfm->per_interface)
    return NULL;

  /* Search through all interfaces */
  for (sw_if_index = 0; sw_if_index < vec_len (sfm->per_interface);
       sw_if_index++)
    {
      if (!sfm->per_interface[sw_if_index].filters)
	continue;

      vec_foreach (filter, sfm->per_interface[sw_if_index].filters)
	{
	  if (filter->name && strcmp (filter->name, name) == 0)
	    {
					      ...
					      vlib_cli_command_t *cmd)
{
  ...
  if (!filter_name || !dest_name)
    {
      error =
	clib_error_return (0, "must specify filter name and destination name");
      goto done;
    }

  /* Find the destination by name via vtable */
  gpcapng_plugin_methods_t gpcapng_plugin;
  clib_error_t *init_result = gpcapng_plugin_exports_init (&gpcapng_plugin);
  if (init_result)
    return init_result;

  dest_index = gpcapng_plugin.find_destination_by_name ((char *) dest_name);
  if (dest_index == ~0)
    {
  ...
  if (!filter)
    {
      ...
      filter =
	find_interface_filter_by_name ((char *) filter_name, &sw_if_index);
    }
  ...
  if (sw_if_index == ~0)
    {
      vlib_cli_output (
	vm, "Set global filter '%s' output to destination '%s' (index %u)",
  ...
  else
    {
      vlib_cli_output (
	vm,
	"Set interface %u filter '%s' output to destination '%s' (index %u)",
				       ...
				       vlib_cli_command_t *cmd)
{
  ...
  if (sw_if_index == ~0)
    {
      vlib_cli_output (vm, "Stopped output for global filter '%s'",
		       filter_name);
    }
  else
    {
