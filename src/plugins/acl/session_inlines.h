...
static const u8 icmp6_invmap[] = { [ICMP6_echo_request - 128] =
				     ICMP6_echo_reply + 1,
				   [ICMP6_node_information_request - 128] =
				     ICMP6_node_information_response + 1 };

/* Supported ICMPv6 messages for session creation */
static const u8 icmp6_valid_new[] = { [ICMP6_echo_request - 128] = 1,
				      [ICMP6_node_information_request - 128] =
					1 };
...
fa_session_get_timeout_type (acl_main_t *am, fa_session_t *sess)
{
  ...
  u16 masked_flags = sess->tcp_flags_seen.as_u16 &
		     ((TCP_FLAGS_RSTFINACKSYN << 8) + TCP_FLAGS_RSTFINACKSYN);
...
get_session_ptr_no_check (acl_main_t *am, u16 thread_index, u32 session_index)
{
...
is_valid_session_ptr (acl_main_t *am, u16 thread_index, fa_session_t *sess)
{
  acl_fa_per_worker_data_t *pw = &am->per_worker_data[thread_index];
  return ((sess != 0) &&
	  ((sess - pw->fa_sessions_pool) < pool_len (pw->fa_sessions_pool)));
}
			      ...
			      u64 now)
{
  ...
  u8 list_id = sess->deleted ? ACL_TIMEOUT_PURGATORY :
			       fa_session_get_timeout_type (am, sess);
  uword thread_index = os_get_thread_index ();
  acl_fa_per_worker_data_t *pw = &am->per_worker_data[thread_index];
  /* the retrieved session thread index must be necessarily the same as the one
   * in the key */
  ...
  if (l4i.proto == icmp_protos[is_ip6])
    {
       ...
       * The other messages will be forwarded without creating a reverse
       * session.
       */

      int valid_reverse_sess =
	(type >= 0 && (type <= icmp_valid_new_size[is_ip6]) &&
...
always_inline void
reverse_session_add_del_ip6 (acl_main_t *am, clib_bihash_kv_40_8_t *pkv,
			     int is_add)
{
...
always_inline void
reverse_session_add_del_ip4 (acl_main_t *am, clib_bihash_kv_16_8_t *pkv,
			     int is_add)
{
			   ...
			   fa_full_session_id_t sess_id)
{
  ...
  if (sess->is_ip6)
    {
      clib_bihash_add_del_40_8 (&am->fa_ip6_sessions_hash, &sess->info.kv_40_8,
				0);
      reverse_session_add_del_ip6 (am, &sess->info.kv_40_8, 0);
    }
  else
    {
      clib_bihash_add_del_16_8 (&am->fa_ip4_sessions_hash, &sess->info.kv_16_8,
				0);
			    ...
			    u32 sw_if_index, u64 now)
{
  ...
  while ((FA_SESSION_BOGUS_INDEX != sess_id.session_index) &&
      ...
      if (sess->link_enqueue_time + fa_session_get_timeout (am, sess) < now)
	{
	  ...
	  /* interface that needs the sessions may not be the interface of the
	   * session. */
