...
static void
macip_rule_to_match_rule (const vl_api_macip_acl_rule_t * rule,
			  match_rule_t * mr)
{
  mr->mr_type = MATCH_TYPE_MASK_SRC_IP_MAC;
  mac_address_from_bytes (&mr->mr_mask_src_ip_mac.mm_mac.mmm_mac,
			  rule->src_mac);
  mac_address_from_bytes (&mr->mr_mask_src_ip_mac.mm_mac.mmm_mask,
			  rule->src_mac_mask);
  mr->mr_mask_src_ip_mac.mm_ip.mip_len = rule->src_ip_prefix_len;

  if (rule->is_ipv6)
    {
      mr->mr_mask_src_ip_mac.mm_ip.mip_ip.version = AF_IP6;
      memcpy (&ip_addr_v6 (&mr->mr_mask_src_ip_mac.mm_ip.mip_ip),
	      rule->src_ip_addr, 16);
    }
  else
    {
      mr->mr_mask_src_ip_mac.mm_ip.mip_ip.version = AF_IP4;
      memcpy (&ip_addr_v4 (&mr->mr_mask_src_ip_mac.mm_ip.mip_ip),
	      rule->src_ip_addr, 4);
    }
}
  ...
  if (macip_acl_is_link_l2 (sw_if_index))
    {
      ...
      ({
        ...
        am->macip_match_apps_by_sw_if_index[linkt][sw_if_index] =
          ...
          match_set_apply (mam->msi, MATCH_SEMANTIC_ANY,
                          VNET_LINK_ETHERNET,
                          tag_flags);
  ...
  else
    {
      ...
      ({
        ...
        am->macip_match_apps_by_sw_if_index[linkt][sw_if_index] =
          mam->apps[sw_if_index] =
          match_set_apply (mam->msi, MATCH_SEMANTIC_ANY, linkt, tag_flags);
		    ...
		    u32 * acl_list_index, u8 * tag)
{
  ...
  for (i = 0; i < count; i++)
    {
      macip_action_t action = (rules[i].is_permit ?
			       MACIP_ACTION_PERMIT : MACIP_ACTION_DENY);

      /* build separate lists for IP4, IP6 and ARP rules */
      linkt = (rules[i].is_ipv6 ? VNET_LINK_IP6 : VNET_LINK_IP4);
      match_rule_t mr1 = {
	.mr_proto = (rules[i].is_ipv6 ?
		     ETHERNET_TYPE_IP6 : ETHERNET_TYPE_IP4),
      };
      macip_rule_to_match_rule (&rules[i], &mr1);
      ...
      if (VNET_LINK_IP4 == linkt)
	{
	  ...
	  macip_rule_to_match_rule (&rules[i], &mr2);
