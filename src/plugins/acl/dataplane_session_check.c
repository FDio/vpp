...
always_inline uword
acl_fa_inner_check_sessions_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
				...
				const int reclassify_sessions)
{
  ...
  if (!with_stateful_datapath)
    {
      /* this function is only useful in a stateful datapath */
      return 0;
    }
  ...
  while (n_left > 0)
	...
	{
	default:
	  acl_fa_prefetch_session_bucket_for_hash (am, is_ip6, hash[9]);
	  /* fallthrough */
	case 9:
	case 8:
	case 7:
	case 6:
	  acl_fa_prefetch_session_data_for_hash (am, is_ip6, hash[5]);
	  /* fallthrough */
	case 5:
	case 4:
	case 3:
	case 2:
	  acl_fa_find_session_with_hash (am, is_ip6, sw_if_index[1], hash[1],
					 &fa_5tuple[1],
...
always_inline void
acl_fa_outer_check_sessions_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
				vlib_frame_t *frame, const u8 variant)
{
  acl_fa_inner_check_sessions_fn (
    vm, node, frame, (variant & (1 << ACLP_DP_IS_IP6)) != 0,
...
#define DO_WORK(x)                                                            \
  case x:                                                                     \
    {                                                                         \
      acl_fa_outer_check_sessions_fn (vm, node, frame, x);                    \
      break;                                                                  \
    }

void CLIB_MULTIARCH_FN (acl_fa_check_sessions_fn) (vlib_main_t *vm,
