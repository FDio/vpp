...
always_inline void
acl_fa_outer_check_sessions_fn (vlib_main_t * vm,
				vlib_node_runtime_t * node,
				vlib_frame_t * frame, const u8 variant)
{
  acl_fa_inner_check_sessions_fn (vm, node, frame,
				  ...
				  (variant & (1 << ACLP_DP_WITH_STATEFUL_DP))
				  != 0,
				  (variant & (1 << ACLP_DP_NODE_TRACE_ON)) !=
				  0,
				  (variant &
				   (1 << ACLP_DP_RECLASSIFY_SESSIONS)) != 0);

}

#define DO_WORK(x) case x: { acl_fa_outer_check_sessions_fn(vm, node, frame, x); break; }
