...
format_session_bihash_5tuple (u8 * s, va_list * args)
{
  ...
  return format (s, "l3 %U -> %U"
                 ...
                 p5t->l4.proto, p5t->l4.is_input, p5t->l4.is_slowpath, p5t->l4.reserved0, p5t->l4.port[0],
                 p5t->l4.port[1],
                 sess->session_index, sess->thread_index, sess->intf_policy_epoch);
}
...
acl_fa_verify_init_sessions (acl_main_t * am)
{
  if (!am->fa_sessions_hash_is_initialized)
      ...
      for (wk = 0; wk < vec_len (am->per_worker_data); wk++)
	{
      ...
      clib_bihash_set_kvp_format_fn_40_8(&am->fa_sessions_hash, format_session_bihash_5tuple);
