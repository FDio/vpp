...
format_ip46_session_bihash_kv (u8 *s, va_list *args, int is_ip6)
{
  ...
  return (format (s, "l3 %U -> %U %U | sess id %d thread id %d epoch %04x",
		  format_addr_func, paddr0, format_addr_func, paddr1,
		  format_fa_session_l4_key, &a5t.l4, sess->session_index,
		  sess->thread_index, sess->intf_policy_epoch));
}
...
acl_fa_verify_init_sessions (acl_main_t *am)
{
  if (!am->fa_sessions_hash_is_initialized)
      ...
      for (wk = 0; wk < vec_len (am->per_worker_data); wk++)
	{
	  ...
	  /*
	   * // In lieu of trying to preallocate the pool and its free bitmap,
	   * rather use pool_init_fixed
...
acl_fa_get_list_head_expiry_time (acl_main_t *am, acl_fa_per_worker_data_t *pw,
				  u64 now, u16 thread_index, int timeout_type)
{
...
acl_fa_check_idle_sessions (acl_main_t *am, u16 thread_index, u64 now)
{
	...
	while (n_expired < am->fa_max_deleted_sessions_per_interval)
	  {
	    ...
	    if (!acl_fa_conn_time_to_check (am, pw, now, thread_index,
		...
		elog_acl_maybe_trace_X3 (am,
					 "acl_fa_check_idle_sessions: expire "
					 "session %d in list %d on thread %d",
  ...
  vec_foreach (psid, pw->expired)
      ...
      if (!pool_is_free_index (pw->fa_sessions_pool, fsid.session_index))
	{
	  ...
	  int clearing_interface = clib_bitmap_get (
	    pw->pending_clear_sw_if_index_bitmap, sw_if_index);
	  if (am->trace_sessions > 3)
	    {
	      elog_acl_maybe_trace_X2 (
		am,
		"acl_fa_check_idle_sessions: now %lu sess_timeout_time %lu",
		"i8i8", now, sess_timeout_time);
	      elog_acl_maybe_trace_X4 (
		am,
		"acl_fa_check_idle_sessions: session %d sw_if_index %d "
		"timeout_passed %d clearing_interface %d",
		"i4i4i4i4", (u32) fsid.session_index, (u32) sess->sw_if_index,
		(u32) timeout_passed, (u32) clearing_interface);
	    }
	  if (timeout_passed || clearing_interface)
		  ...
		  if (am->trace_sessions > 3)
		    {
		      elog_acl_maybe_trace_X2 (
			am,
			"acl_fa_check_idle_sessions: deleted session %d "
			"sw_if_index %d",
		  ...
		  if (am->trace_sessions > 3)
		    {
		      elog_acl_maybe_trace_X2 (
			am,
			"acl_fa_check_idle_sessions: session %d sw_if_index "
			"%d marked as deleted, put to purgatory",
			"i4i4", (u32) fsid.session_index,
			(u32) sess->sw_if_index);
		    }
		}
	      ...
	      if (am->trace_sessions > 3)
		{
		  elog_acl_maybe_trace_X2 (
		    am,
		    "acl_fa_check_idle_sessions: restart timer for session %d "
		    "sw_if_index %d",
		    "i4i4", (u32) fsid.session_index, (u32) sess->sw_if_index);
		}
  ...
  elog_acl_maybe_trace_X1 (
    am, "acl_fa_check_idle_sessions: done, total sessions expired: %d", "i4",
    (u32) total_expired);
...
send_one_worker_interrupt (vlib_main_t *vm, acl_main_t *am, int thread_index)
{
  ...
  if (!pw->interrupt_is_pending)
	...
	vlib_mains[thread_index],
	acl_fa_worker_session_cleaner_process_node.index);
      elog_acl_maybe_trace_X1 (
	am, "send_one_worker_interrupt: send interrupt to worker %u", "i4",
	((u32) thread_index));
...
static int
purgatory_has_connections (vlib_main_t *vm, acl_main_t *am, int thread_index)
{
  acl_fa_per_worker_data_t *pw = &am->per_worker_data[thread_index];
 ...
 */
static uword
acl_fa_worker_conn_cleaner_process (vlib_main_t *vm, vlib_node_runtime_t *rt,
				    vlib_frame_t *f)
{
  ...
  acl_fa_per_worker_data_t *pw = &am->per_worker_data[thread_index];
  int num_expired;
  elog_acl_maybe_trace_X1 (am, "acl_fa_worker_conn_cleaner interrupt: now %lu",
			   "i8", now);
      ...
      if (0 == pw->swipe_end_time)
	{
	  ...
	  if ((pw->pending_clear_sw_if_index_bitmap == 0) ||
	      (pw->serviced_sw_if_index_bitmap == 0))
	    {
	      elog_acl_maybe_trace_X1 (
		am,
		"acl_fa_worker_conn_cleaner: now %lu, someone tried to call "
		"clear but one of the bitmaps are empty",
		"i8", now);
	  ...
	  else
	    {
	      ...
	      clib_warning (
		"WORKER-CLEAR: (before and) swiping sw-if-index bitmap: %U, "
		"my serviced bitmap %U",
	  ...
	  if (clib_bitmap_is_zero (pw->pending_clear_sw_if_index_bitmap))
	    {
	      ...
	      elog_acl_maybe_trace_X1 (am,
				       "acl_fa_worker_conn_cleaner: now %lu, "
				       "clearing done, nothing to do",
				       "i8", now);
...
#ifdef FA_NODE_VERBOSE_DEBUG
	      clib_warning (
		"WORKER-CLEAR: swiping sw-if-index bitmap: %U, my serviced "
		"bitmap %U",
		format_bitmap_hex, pw->pending_clear_sw_if_index_bitmap,
		format_bitmap_hex, pw->serviced_sw_if_index_bitmap);
#endif
	      elog_acl_maybe_trace_X1 (
		am, "acl_fa_worker_conn_cleaner: swiping until %lu", "i8",
		now);
	      /* swipe through the connection lists until enqueue timestamps
	       * become above "now" */
  ...
  // clib_warning("WORKER-CLEAR: checked %d sessions (clear_in_progress: %d)",
  // num_expired, pw->clear_in_process);
  elog_acl_maybe_trace_X2 (
    am,
    "acl_fa_worker_conn_cleaner: checked %d sessions (clear_in_process: %d)",
    "i4i4", (u32) num_expired, (u32) pw->clear_in_process);
  if (pw->clear_in_process)
    {
      if (pw->swipe_end_time == 0)
	{
	  /* we were clearing but we could not process any more connections.
	   * time to stop. */
	  clib_bitmap_zero (pw->pending_clear_sw_if_index_bitmap);
	  pw->clear_in_process = 0;
	  elog_acl_maybe_trace_X1 (
	...
	{
	  elog_acl_maybe_trace_X1 (am,
				   "acl_fa_worker_conn_cleaner: now %lu, more "
				   "work to do - requesting interrupt",
				   "i8", now);
	  /* should continue clearing.. So could they please sent an interrupt
	   * again? */
      ...
      else
	{
      ...
      elog_acl_maybe_trace_X3 (am,
			       "acl_fa_worker_conn_cleaner: now %lu, "
			       "interrupt needed: %u, interrupt unwanted: %u",
...
{
  int i;
  /* Can't use vec_len(am->per_worker_data) since the threads might not have
   * come up yet; */
				...
				vlib_frame_t *f)
{
  ...
  u64 next_expire;
  /* We should check if there are connections to clean up - at least twice a
   * second */
  ...
  while (1)
      ...
      for (ti = 0; ti < vec_len (vlib_mains); ti++)
	{
	  ...
	  for (tt = 0; tt < vec_len (pw->fa_conn_list_head); tt++)
	      ...
	      if ((head_expiry < next_expire) && !pw->interrupt_is_pending)
		{
		  elog_acl_maybe_trace_X3 (am,
					   ...
					   "now %lu, worker: %u tt: %u",
					   "i8i2i2", now, ti, tt);
		  elog_acl_maybe_trace_X2 (
		    am,
		    "acl_fa_session_cleaner_process: head expiry: %lu, is "
		    "earlier than curr next expire: %lu",
      ...
      /* If no pending connections and no ACL applied then no point in timing
       * out */
      if (!has_pending_conns && (0 == am->fa_total_enabled_count))
	{
	  am->fa_cleaner_cnt_wait_without_timeout++;
	  elog_acl_maybe_trace_X1 (
	    am, "acl_conn_cleaner: now %lu entering wait without timeout",
	    "i8", now);
	  ...
	  if (timeout <= 0)
	  ...
	  else
	    {
	      ...
	      elog_acl_maybe_trace_X2 (am,
				       "acl_conn_cleaner: now %lu entering "
				       "wait with timeout %.6f sec",
				       "i8f8", now, timeout);
	...
	case ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX:
	  {
	    ...
	    elog_acl_maybe_trace_X1 (
	      am,
	      "acl_fa_session_cleaner_process: now %lu, received "
		...
		elog_acl_maybe_trace_X1 (
		  am,
		  "acl_fa_session_cleaner_process: "
		  "ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX %u",
		  "i4", *sw_if_index0);
		if (*sw_if_index0 == ~0)
		  {
		    clear_all = 1;
		...
		else
		  {
		    if (!pool_is_free_index (
			  ...
			  *sw_if_index0))
		      {
			clear_sw_if_index_bitmap = clib_bitmap_set (
			  clear_sw_if_index_bitmap, *sw_if_index0, 1);
		      }
		  }
	      }
	    acl_log_info (
		...
		while (pw0->clear_in_process)
		  {
		    ...
		    elog_acl_maybe_trace_X1 (
		      am,
		      "ACL_FA_NODE_CLEAN: waiting previous cleaning cycle to "
		      "finish on %u",
		      "i4", (u32) (pw0 - am->per_worker_data));
		...
		if (pw0->clear_in_process)
		  {
		    acl_log_err ("ERROR-BUG! Could not initiate cleaning on "
				 "worker because another cleanup in progress");
		  }
		else
		  {
		    if (clear_all)
		      {
			/* if we need to clear all, then just clear the
			 * interfaces that we are servicing */
		...
		while (pw0->clear_in_process)
		  {
		    ...
		    elog_acl_maybe_trace_X1 (
		      am,
		      "ACL_FA_NODE_CLEAN: waiting for my cleaning cycle to "
		      "finish on %u",
		      "i4", (u32) (pw0 - am->per_worker_data));
		    vlib_process_suspend (vm, 0.0001);
		    if (pw0->interrupt_is_needed)
	...
	default:
	  ...
	  vlib_node_increment_counter (
	    vm, acl_fa_session_cleaner_process_node.index,
      ...
      if (interrupts_needed)
	{
	  ...
	  /* never go into zero-wait either though - we need to give the space
	   * to others */
  ...
  if (is_input)
    {
      ...
      vnet_feature_enable_disable ("ip6-unicast", "acl-plugin-in-ip6-fa",
				   sw_if_index, enable_disable, 0, 0);
      clib_mem_set_heap (oldheap);
      am->fa_in_acl_on_sw_if_index = clib_bitmap_set (
	am->fa_in_acl_on_sw_if_index, sw_if_index, enable_disable);
    }
  else
    {
      ...
      vnet_feature_enable_disable ("ip6-output", "acl-plugin-out-ip6-fa",
				   sw_if_index, enable_disable, 0, 0);
      clib_mem_set_heap (oldheap);
      am->fa_out_acl_on_sw_if_index = clib_bitmap_set (
	am->fa_out_acl_on_sw_if_index, sw_if_index, enable_disable);
    }
