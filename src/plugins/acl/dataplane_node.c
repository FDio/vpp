  ...
  while (n_left > 0)
      ...
      if (acl_check_needed)
	{
	  ...
	  int is_match = acl_plugin_match_5tuple_inline (
	    ...
	    &action, &match_acl_pos, &match_acl_in_index, &match_rule_index,
	    &trace_bitmap);
	  ...
	  if (2 == action)
	      ...
	      if (acl_fa_can_add_session (am, is_input, sw_if_index[0]))
		{
		  ...
		  fa_full_session_id_t f_sess_id = acl_fa_add_session (
		    am, is_input, is_ip6, sw_if_index[0], now, &fa_5tuple[0],
		    current_policy_epoch);
		...
		int is_input, int is_l2_path)
{
  ...
  if (am->fa_sessions_hash_is_initialized && (!am->bypass_session_check))
