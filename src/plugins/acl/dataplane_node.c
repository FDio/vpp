  ...
  if (PREDICT_FALSE (b->flags & VLIB_BUFFER_IS_TRACED))
    {
      ...
      memcpy (&t->packet_info[0], &fa_5tuple->kv_40_8,
	      sizeof (t->packet_info));
/*
  ...
  while (n_left > 0)
    {
      ...
      CLIB_PREFETCH (b[0], CLIB_CACHE_LINE_BYTES, STORE);
      ...
      if (action > 0)
	{
      ...
      if (PREDICT_FALSE (node->flags & VLIB_NODE_FLAG_TRACE)) {
        trace_bitmap |= 0x80000000 * flag_session_found;
	maybe_trace_buffer (vm, node, b[0], sw_if_index[0], lc_index0, next0,
