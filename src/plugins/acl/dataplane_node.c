...
#define ACL_PLUGIN_PREFETCH_GAP 12
		      ...
		      vlib_frame_t *frame, const u8 variant)
{
  ...
  int is_input = variant & (1 << ACLP_DP_IS_INPUT) ? 1 : 0;
  // int is_l2_path = variant & (1 << ACLP_DP_IS_L2_PATH) ? 1 : 0;
  // int with_stateful_datapath = variant & (1 << ACLP_DP_WITH_STATEFUL_DP) ? 1
  // : 0;
  int node_trace_on = variant & (1 << ACLP_DP_NODE_TRACE_ON) ? 1 : 0;
  // int reclassify_session = variant & (1 << ACLP_DP_RECLASSIFY_SESSIONS) ? 1
  // : 0;
  ...
  while (n_left > 0)
    {
      ...
      if (1)
	{
	  pkts_exist_session += (PP_EXISTING_SESSION == match_policy_prio[0]);

	  if (match_policy_prio[0] < PP_EXISTING_SESSION)
	    {
	      b[0]->error = error_node->errors[action[0]];

	      pkts_acl_permit += (1 == action[0]);
...
#define DO_WORK(x)                                                            \
  case x:                                                                     \
    {                                                                         \
      acl_fa_node_common_prepare_fn (vm, node, frame, x);                     \
...
format_acl_plugin_trace (u8 * s, va_list * args)
{
  ...
  s = format (
    s,
    "acl-plugin: lc_index: %d, sw_if_index %d, next index %d, action: %d, "
    "policy: %d, match: acl %d rule %d trace_bits %08x\n"
    ...
    t->packet_info[0], t->packet_info[1], t->packet_info[2], t->packet_info[3],
    t->packet_info[4], t->packet_info[5], t->hash);
