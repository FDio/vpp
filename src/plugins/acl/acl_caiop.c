	   ...
	   acl_plugin_private_caiop_match_5tuple_func_t func)
{
  ...
  int *pcount =
    is_input ? &am->custom_access_input_policies_count :
  ...
  if (sw_if_index >= vec_len (*pvecvec))
      ...
      else
	{
	  ...
	  *pbitmap = clib_bitmap_set (*pbitmap, sw_if_index, 0);
	  (*pcount)--;
    int enable = (*pcount > 0) || is_acl_enabled_on_sw_if_index (sw_if_index, is_input);
    return acl_interface_inout_enable_disable (am, sw_if_index, is_input, enable);
	}
    }
}
		   ...
		   acl_plugin_private_caiop_match_5tuple_func_t func)
{
  ...
  return is_add ?
    caiop_add (sw_if_index, is_input, func) :
				  ...
				  vlib_cli_command_t * cmd)
{
  ...
  u32 ret = acl_caiop_add_del (!is_del, sw_if_index, is_input,
			       dummy_match_5tuple_fun);
  if (ret != 0)
    {
...
VLIB_CLI_COMMAND (aclplugin_test_custom_policy_command, static) = {
    ...
    .short_help = "test acl-plugin custom-access-policy [del] [input] [sw_if_index <N>]",
