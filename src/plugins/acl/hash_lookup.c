...
void
hash_acl_apply(acl_main_t *am, u32 lc_index, int acl_index)
{
  int i;
...
{
  u32 **applied_acls = 0; // FIXME-FIXME: vec_elt_at_index(am->acl_vec_by_lc_index, lc_index)
  int i;
  ...
  for(i = start_index; i < vec_len(*applied_acls); i++) {
    hash_acl_apply(am, lc_index, *vec_elt_at_index(*applied_acls, i));
  }
}
