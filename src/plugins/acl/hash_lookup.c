...
get_applied_hash_aces (acl_main_t *am, u32 lc_index)
{
  ...
  /*is_input ? vec_elt_at_index(am->input_hash_entry_vec_by_sw_if_index,
     sw_if_index) : vec_elt_at_index(am->output_hash_entry_vec_by_sw_if_index,
     sw_if_index);
  */
...
{
  DBG ("HASH ADD/DEL: %016llx %016llx %016llx %016llx %016llx %016llx %016llx "
       "add %d",
...
static u8
first_mask_contains_second_mask (int is_ip6, fa_5tuple_t *mask1,
				 fa_5tuple_t *mask2)
{
  int i;
  if (is_ip6)
...
{
  if (mask > thresh)
    return clib_host_to_net_u32 ((clib_net_to_host_u32 (mask) << numshifts) &
				 0xFFFFFFFF);
  else
    return else_val;
}
...
relax_ip4_addr (ip4_address_t *ip4_mask, int relax2)
{
  ...
  if (ip4_mask->as_u32 == 0xffffffff)
    ip4_mask->as_u32 = clib_host_to_net_u32 (
      (clib_net_to_host_u32 (ip4_mask->as_u32) << shifts[0]) & 0xFFFFFFFF);
  else
    ip4_mask->as_u32 =
      shift_ip4_if (ip4_mask->as_u32, 0xffffff00, shifts[1],
		    shift_ip4_if (ip4_mask->as_u32, 0xffff0000, shifts[2],
				  shift_ip4_if (ip4_mask->as_u32, 0xff000000,
						shifts[3], ip4_mask->as_u32)));
}
...
relax_tuple (fa_5tuple_t *mask, int is_ip6, int relax2)
{
   ...
   * are close together (within 4 bits for our experiments), we use both of
   * them. Otherwise, we drop the smaller (less specific) address and its
   * associated port field from consideration
   */
...
lock_mask_type_index (acl_main_t *am, u32 mask_type_index)
{
  ...
  ace_mask_type_entry_t *mte =
    pool_elt_at_index (am->ace_mask_type_pool, mask_type_index);
  mte->refcount++;
...
static u32
tm_assign_mask_type_index (acl_main_t *am, fa_5tuple_t *mask, int is_ip6,
			   u32 lc_index)
{
  ...
  if (vec_len (*hash_applied_mask_info_vec) > 0)
    {
      for (order_index = vec_len ((*hash_applied_mask_info_vec)) - 1;
	   order_index >= 0; order_index--)
	{
	  minfo =
	    vec_elt_at_index ((*hash_applied_mask_info_vec), order_index);
  ...
  if (~0 == mask_type_index)
    {
      /* if no mask is found, then let's use a relaxed version of the original
       * one, in order to be used by new ace_entries */
      ...
      hash_applied_mask_info_t **hash_applied_mask_info_vec =
	vec_elt_at_index (am->hash_applied_mask_info_vec_by_lc_index,
			  lc_index);
  ...
  mte = am->ace_mask_type_pool + mask_type_index;
  DBG0 ("TM-ASSIGN MTE index %d new refcount %d", mask_type_index,
	mte->refcount);
  return mask_type_index;
}
			  ...
			  clib_bihash_kv_48_8_t *kv)
{
  ...
  applied_hash_ace_entry_t *pae =
    vec_elt_at_index ((*applied_hash_aces), new_index);
  hash_acl_info_t *ha = vec_elt_at_index (am->hash_acl_infos, pae->acl_index);

  /* apply the mask to ace key */
  hash_ace_info_t *ace_info =
    vec_elt_at_index (ha->rules, pae->hash_ace_info_index);
  ace_mask_type_entry_t *mte =
    vec_elt_at_index (am->ace_mask_type_pool, pae->mask_type_index);
...
static void
add_del_hashtable_entry (acl_main_t *am, u32 lc_index,
			 applied_hash_ace_entry_t **applied_hash_aces,
...
remake_hash_applied_mask_info_vec (
  acl_main_t *am, applied_hash_ace_entry_t **applied_hash_aces, u32 lc_index)
{
...
vec_del_collision_rule (collision_match_rule_t **pvec, u32 applied_entry_index)
{
  ...
  while (i < _vec_len ((*pvec)))
	...
	{
	  /* vec_del1 ((*pvec), i) would be more efficient but would reorder
	   * the elements. */
	  vec_delete ((*pvec), 1, i);
	  deleted++;
...
static void acl_plugin_print_pae (vlib_main_t *vm, int j,
				  applied_hash_ace_entry_t *pae);
...
{
  DBG0 ("DEL COLLIDING RULE: head_index %d applied index %d", head_index,
	applied_entry_index);
		    ...
		    u32 head_index, u32 applied_entry_index)
{
  ...
  DBG0 ("ADD COLLIDING RULE: head_index %d applied index %d", head_index,
	applied_entry_index);
  if (ACL_HASH_LOOKUP_DEBUG > 0)
    acl_plugin_print_pae (acl_main.vlib_main, head_index, head_pae);
...
static u32
activate_applied_ace_hash_entry (acl_main_t *am, u32 lc_index,
				 applied_hash_ace_entry_t **applied_hash_aces,
				 u32 new_index)
{
  clib_bihash_kv_48_8_t kv;
  ASSERT (new_index != ~0);
  DBG ("activate_applied_ace_hash_entry lc_index %d new_index %d", lc_index,
       new_index);
...
static void
assign_mask_type_index_to_pae (acl_main_t *am, u32 lc_index, int is_ip6,
			       applied_hash_ace_entry_t *pae)
{
  hash_acl_info_t *ha = vec_elt_at_index (am->hash_acl_infos, pae->acl_index);
  hash_ace_info_t *ace_info =
   ...
   */
  mte =
    vec_elt_at_index (am->ace_mask_type_pool, ace_info->base_mask_type_index);
  mask = mte->mask;
  if (am->use_tuple_merge)
    pae->mask_type_index =
      tm_assign_mask_type_index (am, &mask, is_ip6, lc_index);
  else
    pae->mask_type_index = assign_mask_type_index (am, &mask);
}

static void split_partition (acl_main_t *am, u32 first_index, u32 lc_index,
			     int is_ip6);

static void
check_collision_count_and_maybe_split (acl_main_t *am, u32 lc_index,
				       int is_ip6, u32 first_index)
{
  applied_hash_ace_entry_t **applied_hash_aces =
    get_applied_hash_aces (am, lc_index);
  applied_hash_ace_entry_t *first_pae =
    vec_elt_at_index ((*applied_hash_aces), first_index);
...
hash_acl_apply (acl_main_t *am, u32 lc_index, int acl_index, u32 acl_position)
{
  ...
  if (!am->acl_lookup_hash_initialized)
    {
      BV (clib_bihash_init)
      (&am->acl_lookup_hash, "ACL plugin rule lookup bihash",
  ...
  applied_hash_acl_info_t *pal =
    vec_elt_at_index ((*applied_hash_acls), lc_index);

  /* ensure the list of applied hash acls is initialized and add this acl# to
   * it */
  u32 index = vec_search (pal->applied_acls, acl_index);
  if (index != ~0)
    {
      clib_warning ("BUG: trying to apply twice acl_index %d on lc_index %d, "
		    "according to lc",
		    acl_index, lc_index);
      ASSERT (0);
      return;
  ...
  /* since we know (in case of no split) how much we expand, preallocate that
   * space */
  if (vec_len (ha->rules) > 0)
    {
      int old_vec_len = vec_len (*applied_hash_aces);
      vec_validate ((*applied_hash_aces),
		    old_vec_len + vec_len (ha->rules) - 1);
      _vec_len ((*applied_hash_aces)) = old_vec_len;
    }

  /* add the rules from the ACL to the hash table for lookup and append to the
   * vector*/
  for (i = 0; i < vec_len (ha->rules); i++)
	...
	vec_elt_at_index ((*applied_hash_aces), new_index);
      ...
      u32 first_index = activate_applied_ace_hash_entry (
	am, lc_index, applied_hash_aces, new_index);
      if (am->use_tuple_merge)
	check_collision_count_and_maybe_split (am, lc_index, is_ip6,
					       first_index);
    }
...
static u32
find_head_applied_ace_index (applied_hash_ace_entry_t **applied_hash_aces,
			     u32 curr_index)
{
  ASSERT (curr_index != ~0);
  applied_hash_ace_entry_t *pae =
...
static void
set_collision_head_ae_index (applied_hash_ace_entry_t **applied_hash_aces,
			     ...
			     u32 new_index)
{
  ...
  vec_foreach (cr, colliding_rules)
    {
      applied_hash_ace_entry_t *pae =
	vec_elt_at_index ((*applied_hash_aces), cr->applied_entry_index);
      pae->collision_head_ae_index = new_index;
    }
}

static void
move_applied_ace_hash_entry (acl_main_t *am, u32 lc_index,
			     applied_hash_ace_entry_t **applied_hash_aces,
			     u32 old_index, u32 new_index)
{
  ...
  /* move the entry */
  *vec_elt_at_index ((*applied_hash_aces), new_index) =
    *vec_elt_at_index ((*applied_hash_aces), old_index);

  /* update the linkage and hash table if necessary */
  applied_hash_ace_entry_t *pae =
    vec_elt_at_index ((*applied_hash_aces), old_index);
  applied_hash_ace_entry_t *new_pae =
    vec_elt_at_index ((*applied_hash_aces), new_index);
  ...
  if (new_pae->colliding_rules)
    {
      ...
      collision_match_rule_t *cr =
	vec_elt_at_index (new_pae->colliding_rules, 0);
      ASSERT (cr->applied_entry_index == old_index);
      cr->applied_entry_index = new_index;
      set_collision_head_ae_index (applied_hash_aces, new_pae->colliding_rules,
				   new_index);
    }
  else
    {
      ...
      u32 head_index =
	find_head_applied_ace_index (applied_hash_aces, new_index);
      ASSERT (head_index != ~0);
      applied_hash_ace_entry_t *head_pae =
	vec_elt_at_index ((*applied_hash_aces), head_index);
	...
	{
	  collision_match_rule_t *cr =
	    vec_elt_at_index (head_pae->colliding_rules, i);
...
deactivate_applied_ace_hash_entry (
  acl_main_t *am, u32 lc_index, applied_hash_ace_entry_t **applied_hash_aces,
  u32 old_index)
{
  applied_hash_ace_entry_t *pae =
    vec_elt_at_index ((*applied_hash_aces), old_index);
  DBG ("UNAPPLY DEACTIVATE: lc_index %d applied index %d", lc_index,
       old_index);
  if (ACL_HASH_LOOKUP_DEBUG > 0)
    {
  ...
  if (pae->collision_head_ae_index != old_index)
    {
      DBG ("UNAPPLY = index %d has collision head %d", old_index,
	   pae->collision_head_ae_index);

      u32 head_index =
	find_head_applied_ace_index (applied_hash_aces, old_index);
      ASSERT (head_index != ~0);
      del_colliding_rule (applied_hash_aces, head_index, old_index);
    }
  else
    {
      /* It was the first entry. We need either to reset the hash entry or
       * delete it */
      ...
      if (vec_len (pae->colliding_rules) > 0)
	{
	  ...
	  applied_hash_ace_entry_t *next_pae =
	    vec_elt_at_index ((*applied_hash_aces), next_pae_index);
	  /* Remove ourselves and transfer the ownership of the colliding rules
	   * vector */
	  next_pae->colliding_rules = pae->colliding_rules;
	  set_collision_head_ae_index (
	    applied_hash_aces, next_pae->colliding_rules, next_pae_index);
	  add_del_hashtable_entry (am, lc_index, applied_hash_aces,
				   next_pae_index, 1);
	}
      else
	{
	  /* no next entry, so just delete the entry in the hash table */
	  add_del_hashtable_entry (am, lc_index, applied_hash_aces, old_index,
				   0);
	}
    }
  DBG0 ("Releasing mask type index %d for pae index %d on lc_index %d",
	pae->mask_type_index, old_index, lc_index);
...
hash_acl_unapply (acl_main_t *am, u32 lc_index, int acl_index)
{
  ...
  if (index == ~0)
    {
      clib_warning ("BUG: trying to unapply unapplied acl_index %d on "
		    "lc_index %d, according to lc",
		    acl_index, lc_index);
      return;
    }
  ...
  if (index2 == ~0)
    {
      clib_warning ("BUG: trying to unapply twice acl_index %d on lc_index "
		    "%d, according to h-acl info",
		    acl_index, lc_index);
      return;
    }
  ...
  if (vec_len ((*applied_hash_aces)) <= i)
    {
      ...
      /* we went all the way without finding any entries. Probably a list was
       * empty. */
      return;
    }
  ...
  DBG ("base_offset: %d, tail_offset: %d, tail_len: %d", base_offset,
       tail_offset, tail_len);

  for (i = 0; i < vec_len (ha->rules); i++)
    {
      deactivate_applied_ace_hash_entry (am, lc_index, applied_hash_aces,
					 base_offset + i);
    }
  for (i = 0; i < tail_len; i++)
    {
      ...
      DBG0 ("UNAPPLY MOVE: lc_index %d, applied index %d -> %d", lc_index,
	    tail_offset + i, base_offset + i);
      move_applied_ace_hash_entry (am, lc_index, applied_hash_aces,
				   tail_offset + i, base_offset + i);
    }
...
hash_acl_reapply (acl_main_t *am, u32 lc_index, int acl_index)
{
  ...
  DBG0 ("Start index for acl %d in lc_index %d is %d", acl_index, lc_index,
	start_index);
  /*
   * This function is called after we find out the lc_index where ACL is
   * applied. If the by-lc_index vector does not have the ACL#, then it's a
   * bug.
   */
...
static void
make_mask_and_match_from_rule (fa_5tuple_t *mask, acl_rule_t *r,
			       hash_ace_info_t *hi)
{
  ...
  /* we will need to be matching based on lc_index and mask_type_index when
   * applied */
  mask->pkt.lc_index = ~0;
  ...
  if (r->proto != 0)
    {
      ...
      /* Calculate the src/dst port masks and make the src/dst port matches
       * accordingly */
      make_port_mask (&mask->l4.port[0], r->src_port_or_type_first,
		      r->src_port_or_type_last);
      hi->match.l4.port[0] = r->src_port_or_type_first & mask->l4.port[0];

      make_port_mask (&mask->l4.port[1], r->dst_port_or_code_first,
...
hash_acl_add (acl_main_t *am, int acl_index)
{
  ...
  /* avoid small requests by preallocating the entire vector before running the
   * additions */
  ...
  /*
   * if an ACL is applied somewhere, fill the corresponding lookup data
   * structures. We need to take care if the ACL is not the last one in the
   * vector of ACLs applied to the interface.
   */
...
hash_acl_delete (acl_main_t *am, int acl_index)
{
  ...
  /*
   * If the ACL is applied somewhere, remove the references of it (call
   * hash_acl_unapply) this is a different behavior from the linear lookup
   * where an empty ACL is "deny all",
   *
   ...
   *
   * The part to remember is that this routine is called in process of
   * reapplication during the acl_add_replace() API call - the old acl ruleset
   * is deleted, then the new one is added, without the change in the applied
   * ACLs - so this case has to be handled.
   */
  hash_acl_info_t *ha = vec_elt_at_index (am->hash_acl_infos, acl_index);
  u32 *lc_list_copy = 0;
  {
...
acl_plugin_show_tables_mask_type (void)
{
  ...
  pool_foreach (mte, am->ace_mask_type_pool)
    {
      vlib_cli_output (vm,
		       "     %3d: %016llx %016llx %016llx %016llx %016llx "
		       "%016llx  refcount %d",
		       mte - am->ace_mask_type_pool, mte->mask.kv_40_8.key[0],
		       mte->mask.kv_40_8.key[1], mte->mask.kv_40_8.key[2],
		       mte->mask.kv_40_8.key[3], mte->mask.kv_40_8.key[4],
...
acl_plugin_show_tables_acl_hash_info (u32 acl_index)
{
  ...
  for (i = 0; i < vec_len (am->hash_acl_infos); i++)
      ...
      for (j = 0; j < vec_len (ha->rules); j++)
	{
	  ...
	  vlib_cli_output (
	    vm,
	    "    %4d: %016llx %016llx %016llx %016llx %016llx %016llx base "
	    "mask index %d acl %d rule %d action %d\n",
...
static void
acl_plugin_print_colliding_rule (vlib_main_t *vm, int j,
				 collision_match_rule_t *cr)
{
  vlib_cli_output (vm, "        %4d: acl %d ace %d acl pos %d pae index: %d",
		   j, cr->acl_index, cr->ace_index, cr->acl_position,
...
acl_plugin_print_pae (vlib_main_t *vm, int j, applied_hash_ace_entry_t *pae)
{
  vlib_cli_output (
    vm,
    "    %4d: acl %d rule %d action %d bitmask-ready rule %d mask type index: "
    "%d colliding_rules: %d collision_head_ae_idx %d hitcount %lld acl_pos: "
    "%d",
    j, pae->acl_index, pae->ace_index, pae->action, pae->hash_ace_info_index,
    pae->mask_type_index, vec_len (pae->colliding_rules),
    pae->collision_head_ae_index, pae->hitcount, pae->acl_position);
  int jj;
  for (jj = 0; jj < vec_len (pae->colliding_rules); jj++)
    acl_plugin_print_colliding_rule (
      vm, jj, vec_elt_at_index (pae->colliding_rules, jj));
}

static void
acl_plugin_print_applied_mask_info (vlib_main_t *vm, int j,
				    hash_applied_mask_info_t *mi)
{
  vlib_cli_output (vm,
		   "    %4d: mask type index %d first rule index %d "
		   "num_entries %d max_collisions %d",
		   j, mi->mask_type_index, mi->first_rule_index,
		   mi->num_entries, mi->max_collisions);
}
  ...
  for (lci = 0; (lci < vec_len (am->applied_hash_acl_info_by_lc_index)); lci++)
      ...
      if (lci < vec_len (am->hash_applied_mask_info_vec_by_lc_index))
	{
	  ...
	  for (j = 0;
		...
		vm, j, &am->hash_applied_mask_info_vec_by_lc_index[lci][j]);
	    }
	}
	  ...
	  for (j = 0; j < vec_len (am->hash_entry_vec_by_lc_index[lci]); j++)
	    {
	      acl_plugin_print_pae (vm, j,
				    &am->hash_entry_vec_by_lc_index[lci][j]);
	    }
	}
    }
}
...
{
  int update = (clib_net_to_host_u64 (mask_addr->as_u64[0]) <
...
{
  int update = (clib_net_to_host_u32 (mask_addr->as_u32) <
...
split_partition (acl_main_t *am, u32 first_index, u32 lc_index, int is_ip6)
{
  ...
  for (i = 0; i < collisions; i++)
    {
      ...
      pae = vec_elt_at_index ((*applied_hash_aces),
			      colliding_rules[i].applied_entry_index);
      ...
      mte = vec_elt_at_index (am->ace_mask_type_pool,
      ...
      else
	{
	  ...
	  for (j = 0; j < 2; j++)
	    {
	      if (is_ip6)
		ensure_ip6_min_addr (&min_tuple->ip6_addr[j],
				     &mask->ip6_addr[j]);
	      else
		ensure_ip4_min_addr (&min_tuple->ip4_addr[j],
				     &mask->ip4_addr[j]);
	  ...
	  for (j = 0; j < 2; j++)
	    {
	      if (is_ip6)
		ensure_ip6_max_addr (&max_tuple->ip6_addr[j],
				     &mask->ip6_addr[j]);
	      else
		ensure_ip4_max_addr (&max_tuple->ip4_addr[j],
				     &mask->ip4_addr[j]);
  ...
  if (is_ip6)
    {
      ...
      for (i = 0; i < 2; i++)
	{
	  delta += count_bits (max_tuple->ip6_addr[0].as_u64[i]) -
		   count_bits (min_tuple->ip6_addr[0].as_u64[i]);
	}
    }
  else
    {
      delta += count_bits (max_tuple->ip4_addr[0].as_u32) -
	       count_bits (min_tuple->ip4_addr[0].as_u32);
    }
  if (delta > best_delta)
    {
      best_delta = delta;
  ...
  if (is_ip6)
    {
      ...
      for (i = 0; i < 2; i++)
	{
	  delta += count_bits (max_tuple->ip6_addr[1].as_u64[i]) -
		   count_bits (min_tuple->ip6_addr[1].as_u64[i]);
	}
    }
  else
    {
      delta += count_bits (max_tuple->ip4_addr[1].as_u32) -
	       count_bits (min_tuple->ip4_addr[1].as_u32);
    }
  if (delta > best_delta)
    {
      best_delta = delta;
  ...
  /* SRC_port dimension */
  delta =
    count_bits (max_tuple->l4.port[0]) - count_bits (min_tuple->l4.port[0]);
  if (delta > best_delta)
    {
      best_delta = delta;
  ...
  /* DST_port dimension */
  delta =
    count_bits (max_tuple->l4.port[1]) - count_bits (min_tuple->l4.port[1]);
  if (delta > best_delta)
    {
      best_delta = delta;
  ...
  switch (best_dim)
    ...
    case DIM_SRC_ADDR:
      ...
      min_tuple->ip4_addr[0].as_u32 = clib_host_to_net_u32 (
	(clib_net_to_host_u32 (max_tuple->ip4_addr[0].as_u32) << (shifting)) &
	0xFFFFFFFF);
    ...
    case DIM_DST_ADDR:
      ...
      /*
			      ipv4_block =
	 count_bits(max_tuple->addr[1].as_u64[1]); if(ipv4_block > shifting)
				      min_tuple->addr[1].as_u64[1] =
					      clib_host_to_net_u64((clib_net_to_host_u64(max_tuple->addr[1].as_u64[1])
	 << (shifting))&0xFFFFFFFF); else{ shifting = shifting - ipv4_block;
      ...
      */
      min_tuple->ip4_addr[1].as_u32 = clib_host_to_net_u32 (
	(clib_net_to_host_u32 (max_tuple->ip4_addr[1].as_u32) << (shifting)) &
	0xFFFFFFFF);
  ...
  DBG ("TM-split_partition - mask type index-assigned!! -> %d",
       new_mask_type_index);

  if (coll_mask_type_index == new_mask_type_index)
    {
      // vlib_cli_output(vm, "TM-There are collisions over threshold, but i'm
      // not able to split! %d %d", coll_mask_type_index, new_mask_type_index);
      return;
    }
  ...
  for (i = 0; i < collisions; i++)
    {
      ...
      applied_hash_ace_entry_t *pop_pae =
	vec_elt_at_index ((*applied_hash_aces), r_ace_index);
      ha = vec_elt_at_index (am->hash_acl_infos, pop_pae->acl_index);
      DBG ("TM-Population-collision: base_ace:%d (ace_mask:%d, "
	   "first_collision_mask:%d)",
	...
	continue;
      ...
      /* The very first repopulation gets the lock by virtue of a new mask
       * being created above */
      if (++repopulate_count > 1)
	lock_mask_type_index (am, new_mask_type_index);

      activate_applied_ace_hash_entry (am, lc_index, applied_hash_aces,
				       r_ace_index);
    }
