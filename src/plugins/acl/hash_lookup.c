...
hash_acl_apply(acl_main_t *am, u32 sw_if_index, u8 is_input, int acl_index)
{
  ...
  for(i=0; i < vec_len(ha->rules); i++) {
    ...
    /* the match for IPv4 might be inconsistent with the requested mask, make sure it is sane */
    ace_mask_type_entry_t *mte = &am->ace_mask_type_pool[ha->rules[i].mask_type_index];
    u64 *pmask = (u64 *)&mte->mask;
    int j;
    for(j=0; j<6; j++) {
      kv.key[j] = kv.key[j] & pmask[j];
    }
