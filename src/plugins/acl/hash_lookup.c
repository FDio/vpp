			    ...
			    u32 index, int is_add)
{
  ...
  kv_val->applied_entry_index = index;
  /* by default assume all values are shadowed -> check all mask types */
  kv_val->shadowed = 1;
...
make_port_mask(u16 *portmask, u16 port_first, u16 port_last)
{
  ...
  *portmask = 0;
  ...
  clib_warning("FIXME:BUG: The port range is not power of two - things will break");
  return 1;
}
...
void hash_acl_delete(acl_main_t *am, int acl_index)
{
  ...
  /* FIXME: if the ACL is applied somewhere, remove the references of it (call hash_acl_unapply) */
