...
u8
compare_tuples (int is_ip6, fa_5tuple_t * mask1, fa_5tuple_t * mask2)
{
  int i;
  if (is_ip6)
    {
...
void relax_ip4_addr(ip4_address_t *ip4_mask, int relax2) {
...
*/
	...
	mask->pkt.l4_valid = 0;
	if(compare_tuples(is_ip6, mask, &save_mask) != 0){
...
//	for(int order_index = 0; order_index < vec_len((*hash_applied_mask_info_vec)); order_index++) {
	for(int order_index = vec_len((*hash_applied_mask_info_vec)) -1; order_index >= 0; order_index--) {
		...
		mte = vec_elt_at_index(am->ace_mask_type_pool, for_mask_type_index);
                // FIXME: we will need to ensure here that masks are for the same AF
		if(compare_tuples(is_ip6, &mte->mask, mask) == 0){
  ...
  for (int i = 0; i < vec_len ((*applied_hash_aces)); i++)
    {
      ...
      if (minfo->first_rule_index > new_index)
        minfo->first_rule_index = new_index;
    }
...
check_collision_count_and_maybe_split(acl_main_t *am, u32 lc_index, int is_ip6, u32 first_index)
{
  ...
  applied_hash_ace_entry_t *first_pae = vec_elt_at_index((*applied_hash_aces), first_index);
  if (vec_len(first_pae->colliding_rules) > TM_SPLIT_THRESHOLD) {
...
//	for(i=0; i<collisions; i++){
	for(r_ace_index=0; r_ace_index < vec_len((*applied_hash_aces)); r_ace_index++) {
		...
		if(compare_tuples(is_ip6, min_tuple, pop_mask) != 0) continue;
