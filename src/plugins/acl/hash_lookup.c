...
{
  applied_ace_hash_entry_t **applied_hash_aces = match->pkt.is_input ? &am->input_hash_entry_vec_by_sw_if_index[match->pkt.sw_if_index] :
                                                    &am->output_hash_entry_vec_by_sw_if_index[match->pkt.sw_if_index];
  applied_ace_hash_entry_t *pae = &((*applied_hash_aces)[index]);
...
match_an_acl(acl_main_t *am, fa_5tuple_t *match)
{
  ...
  for(mask_type_index=0; mask_type_index < pool_len(am->ace_mask_type_pool); mask_type_index++) {
    ...
    kv_key->pkt.mask_type_index = mask_type_index;
	...
	if (result_val->need_portrange_check) {
          ...
          while ((curr_index != ~0) && !match_portranges(am, match, curr_index)) {
            ...
            applied_ace_hash_entry_t *pae = &((*applied_hash_aces)[curr_index]);
            clib_warning("entry %d did not portmatch, advancing to %d", curr_index, pae->next_applied_entry_index);
            curr_index = pae->next_applied_entry_index;
          }
          if (curr_index < curr_match_index) {
            clib_warning("The index %d is the new candidate in portrange matches.", curr_index);
	    ...
	    if (!result_val->shadowed) {
            ...
            clib_warning("Curr portmatch index %d is too big vs. current matched one %d", curr_index, curr_match_index);
          }
        } else {
          /* The usual path is here. Found an entry in front of the current candiate - so it's a new one */
          clib_warning("This match is the new candidate");
          curr_match_index = result_val->applied_entry_index;
	  if (!result_val->shadowed) {
          /* new result is known to not be shadowed, so no point to look up further */
            break;
...
{
    clib_warning("HASH ADD/DEL: %016llx %016llx %016llx %016llx %016llx %016llx add %d",
...
fill_applied_hash_ace_kv(acl_main_t *am,
                            applied_ace_hash_entry_t **applied_hash_aces,
                            ...
                            u32 new_index, clib_bihash_kv_48_8_t *kv)
{
  ...
  applied_ace_hash_entry_t *pae = &((*applied_hash_aces)[new_index]);
  hash_acl_info_t *ha = &am->hash_acl_infos[pae->acl_index];
                            ...
                            u32 new_index)
{
  ...
  clib_warning("APPLY ADD KY: %016llx %016llx %016llx %016llx %016llx %016llx",
			kv.key[0], kv.key[1], kv.key[2],
  ...
  if (res == 0) {
    ...
    clib_warning("A key already exists, with applied entry index: %d", existing_index);
    ASSERT(existing_index != ~0);
    applied_ace_hash_entry_t *existing_pae = &((*applied_hash_aces)[existing_index]);
    ...
    while (existing_pae->next_applied_entry_index != ~0) {
      if (existing_index == existing_pae->next_applied_entry_index) {
        clib_warning("existing and next index of entry %d are the same, abort", existing_index);
      ...
      clib_warning("...advance to chained entry index: %d", existing_index);
    }
    /* link ourseves in */
...
static void
applied_hash_entries_analyze(acl_main_t *am, applied_ace_hash_entry_t **applied_hash_aces)
{
  /*
...
hash_acl_apply(acl_main_t *am, u32 sw_if_index, u8 is_input, int acl_index)
{
  ...
  /*
   * FIXME: if the applied ACL is empty, the current code will cause a
   ...
   * To preserve the current behavior, we would need to expand the ha->rules
   * here to have just a single "deny any ipv4 and ipv6", and also add
   * the cleanup into the hash_acl_add() to remove that entry...
   */
                            ...
                            u32 old_index, u32 new_index)
{
  ...
  applied_ace_hash_entry_t *pae = &((*applied_hash_aces)[old_index]);

  if (pae->prev_applied_entry_index != ~0) {
    applied_ace_hash_entry_t *prev_pae = &((*applied_hash_aces)[pae->prev_applied_entry_index]);
  ...
  if (pae->next_applied_entry_index != ~0) {
    applied_ace_hash_entry_t *next_pae = &((*applied_hash_aces)[pae->next_applied_entry_index]);
...
hash_acl_unapply(acl_main_t *am, u32 sw_if_index, u8 is_input, int acl_index)
{
  ...
  for(i=0; i < vec_len((*applied_hash_aces)); i++) {
    if ((*applied_hash_aces)[i].acl_index == acl_index) {
      clib_warning("Found applied ACL#%d at applied index %d", acl_index, i);
      break;
    }
  }
  if (vec_len((*applied_hash_aces)) <= i) {
    clib_warning("Did not find applied ACL#%d at sw_if_index %d", acl_index, sw_if_index);
  ...
  clib_warning("base_offset: %d, tail_offset: %d, tail_len: %d", base_offset, tail_offset, tail_len);

  for(i=0; i < vec_len(ha->rules); i ++) {
    clib_warning("UNAPPLY DEACTIVATE: sw_if_index %d is_input %d, applied index %d", sw_if_index, is_input, base_offset + i);
  ...
  for(i=0; i < tail_len; i ++) {
    ...
    clib_warning("UNAPPLY MOVE: sw_if_index %d is_input %d, applied index %d ->", sw_if_index, is_input, tail_offset+i, base_offset + i);
...
make_port_mask(u16 *portmask, u16 port_first, u16 port_last)
{
   ...
   * here could be a better optimization which would
...
void hash_acl_delete(acl_main_t *am, int acl_index)
{
  ...
  /* FIXME: this is a different behavior from the linear lookup where an empty ACL is "deny all" */
  if (acl_index < vec_len(am->input_sw_if_index_vec_by_acl)) {
    u32 *sw_if_index;
    vec_foreach(sw_if_index, am->input_sw_if_index_vec_by_acl[acl_index]) {
