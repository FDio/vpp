	      ...
	      u32 * acl_list_index, u8 * tag)
{
  ...
  if (vec_len (am->acl_add_list_cbs) > 0)
    clib_call_callbacks (am->acl_add_list_cbs, am, *acl_list_index, 0 /* before */);
  ...
  if (vec_len (am->acl_add_list_cbs) > 0)
    clib_call_callbacks (am->acl_add_list_cbs, am, *acl_list_index, 1 /* after */);
  return 0;
}
...
acl_del_list (u32 acl_list_index)
{
  ...
  if (vec_len (am->acl_del_list_cbs) > 0)
    clib_call_callbacks (am->acl_del_list_cbs, am, acl_list_index, 0 /* before */);
  ...
  acl_plugin_lookup_context_notify_acl_change (acl_list_index);
  if (vec_len (am->acl_del_list_cbs) > 0)
    clib_call_callbacks (am->acl_del_list_cbs, am, acl_list_index, 1 /* after */);
  return 0;
}
