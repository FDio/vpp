				 ...
				 int enable_disable)
{
  ...
  void *oldheap = clib_mem_set_heap (am->vlib_main->heap_base);
  rv = vnet_l2_feature_enable_disable ("l2-in-ip4", "acl-plugin-in-ip4-l2",
				       sw_if_index, enable_disable, 0, 0);
  if (rv)
    clib_error ("Could not enable on input");
  rv = vnet_l2_feature_enable_disable ("l2-in-ip6", "acl-plugin-in-ip6-l2",
				       sw_if_index, enable_disable, 0, 0);
  if (rv)
    clib_error ("Could not enable on input");

  if (intf_has_etype_whitelist (am, sw_if_index, 1))
    vnet_l2_feature_enable_disable ("l2-in-nonip", "acl-plugin-in-nonip-l2",
				    sw_if_index, enable_disable, 0, 0);
				  ...
				  int enable_disable)
{
  ...
  void *oldheap = clib_mem_set_heap (am->vlib_main->heap_base);
  rv = vnet_l2_feature_enable_disable ("l2-out-ip4", "acl-plugin-out-ip4-l2",
				       sw_if_index, enable_disable, 0, 0);
  if (rv)
    clib_error ("Could not enable on output");
  rv = vnet_l2_feature_enable_disable ("l2-out-ip6", "acl-plugin-out-ip6-l2",
				       sw_if_index, enable_disable, 0, 0);
  if (rv)
    clib_error ("Could not enable on output");
  if (intf_has_etype_whitelist (am, sw_if_index, 0))
    vnet_l2_feature_enable_disable ("l2-out-nonip", "acl-plugin-out-nonip-l2",
				    sw_if_index, enable_disable, 0, 0);
