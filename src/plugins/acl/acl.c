...
{
  *p_epoch = ((1 + *p_epoch) & FA_POLICY_EPOCH_MASK) + FA_POLICY_EPOCH_IS_INPUT;
}
...
acl_interface_add_inout_acl (u32 sw_if_index, u8 is_input, u32 acl_list_index)
{
  ...
  if (is_input)
      ...
      if (vec_len (am->input_acl_vec_by_sw_if_index[sw_if_index]) == 0)
	{
      ...
      increment_policy_epoch_input (vec_elt_at_index (am->input_policy_epoch_by_sw_if_index, sw_if_index));
    }
  else
    {
      ...
      if (vec_len (am->output_acl_vec_by_sw_if_index[sw_if_index]) == 0)
	{
      ...
      increment_policy_epoch_output (vec_elt_at_index (am->output_policy_epoch_by_sw_if_index, sw_if_index));
    }
  clib_mem_set_heap (oldheap);
  return 0;
}
