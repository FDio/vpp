  ...
  if (unformat (input, "sessions"))
	...
	out0 = format(out0, "  connection add/del stats:\n", wk);
        ...
        ({
          ...
          u64 n_existing_out = sw_if_index < vec_len(pw->fa_existing_session_packets_by_sw_if_index[0]) ? pw->fa_existing_session_packets_by_sw_if_index[0][sw_if_index] : 0;
          u64 n_existing_in = sw_if_index < vec_len(pw->fa_existing_session_packets_by_sw_if_index[1]) ? pw->fa_existing_session_packets_by_sw_if_index[1][sw_if_index] : 0;
          out0 = format(out0, "    sw_if_index %d: add %lu - del %lu = %lu\n", sw_if_index, n_adds, n_dels, n_adds - n_dels);
          out0 = format(out0, "                    existing session packets: in %lu out %lu", n_existing_in, n_existing_out);
        }));
