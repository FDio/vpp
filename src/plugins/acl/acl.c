...
static int
acl_interface_set_inout_acl_list (acl_main_t *am, u32 sw_if_index, u8 is_input, u32 *vec_acl_list_index)
{
  u32 *pacln;
  ...
  /*
  * prepare or delete the lookup context if necessary.
  */
  ...
  if (vec_len(vec_acl_list_index) > 0) {
    if (~0 == (*pinout_lc_index_by_sw_if_index)[sw_if_index]) {
      int lc_index = acl_plugin_get_lookup_context_index(am->interface_acl_user_id, sw_if_index, is_input);
      ASSERT(lc_index >= 0);
				 ...
				 u32 acl_list_index)
{
  ...
  acl_interface_set_inout_acl_list(am, sw_if_index, is_input, acl_vec);
  vec_free(acl_vec);
  return rv;
}
  ...
  (vl_api_acl_interface_set_acl_list_t * mp)
{
  ...
  if (pool_is_free_index (im->sw_interfaces, sw_if_index))
      ...
      if (0 == rv)
	{
          ...
          rv = acl_interface_set_inout_acl_list(am, sw_if_index, 0, out_acl_vec);
          rv = rv || acl_interface_set_inout_acl_list(am, sw_if_index, 1, in_acl_vec);
          vec_free(in_acl_vec);
