...
{
  acl_main_t *am = &acl_main;
  if (acl_list_index >= vec_len(am->acls) {
  ...
  if (is_input)
    {
      ...
      for(i = vec_len(am->input_acl_vec_by_sw_if_index[sw_if_index])-1; i>=0; i--)
        hash_acl_unapply(am, sw_if_index, 1, am->input_acl_vec_by_sw_if_index[sw_if_index][i]);
  ...
  else
    {
      ...
      for(i = vec_len(am->output_acl_vec_by_sw_if_index[sw_if_index])-1; i>=0; i--)
        hash_acl_unapply(am, sw_if_index, 0, am->output_acl_vec_by_sw_if_index[sw_if_index][i]);
  ...
  if (pool_is_free_index(im->sw_interfaces, sw_if_index))
  ...
  else
    {
      ...
      for (i = 0; i < mp->count; i++)
        {
          if(ntohl (mp->acls[i]) >= vec_len(am->acls)) {
  ...
  else if (unformat (input, "acl"))
    {
      ...
      for(i=0; i<vec_len(am->acls); i++) {
        ...
        for(j=0; j<am->acls[i].count; j++) {
          r = &am->acls[i].rules[j];
          out0 = format(out0, "  %04d: %s ", j, r->is_ipv6 ? "ipv6" : "ipv4");
        ...
        for(swi = 0; (swi < vec_len(am->input_applied_hash_acl_info_by_sw_if_index)) ||
          ...
          if (swi < vec_len(am->input_hash_entry_vec_by_sw_if_index)) {
            ...
            for(j=0; j<vec_len(am->input_hash_entry_vec_by_sw_if_index[swi]); j++) {
              ...
              out0 = format(out0, "    %4d: acl %d rule %d hash-ready rule %d next %d prev %d\n",
                                       j, pae->acl_index, pae->ace_index, pae->hash_ace_info_index,
