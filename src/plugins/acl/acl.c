...
#define PRINT_AND_RESET(vm, out0) do { clib_warning("%v", out0); vec_reset_length(out0); } while(0)
...
static void
acl_print_acl_x (acl_vector_print_func_t vpr, vlib_main_t * vm, acl_main_t * am, int acl_index)
{
  acl_rule_t *r;
	      ...
	      u32 * acl_list_index, u8 * tag)
{
  ...
  clib_warning("API dbg: acl_add_list index %d tag %s", *acl_list_index, tag);
  ...
  clib_warning("API dbg: acl_add_list true index %d", *acl_list_index, tag);
...
static int acl_is_used_by(u32 acl_index, u32 **foo_index_vec_by_acl) {
  if (acl_index < vec_len (foo_index_vec_by_acl))
    {
      if (vec_len (vec_elt (foo_index_vec_by_acl, acl_index))
	  > 0)
	{
				 ...
				 int enable_disable)
{
  ...
  if (clib_bitmap_get(am->in_acl_on_sw_if_index, sw_if_index) == enable_disable)
    return 0;
				  ...
				  int enable_disable)
{
  ...
  if (clib_bitmap_get(am->out_acl_on_sw_if_index, sw_if_index) == enable_disable)
    return 0;
...
static int
acl_interface_inout_enable_disable (acl_main_t * am, u32 sw_if_index, int is_input,
				  int enable_disable)
{
  if (is_input)
...
static int
acl_interface_set_inout_acl_list (acl_main_t *am, u32 sw_if_index, u8 is_input, u32 *vec_acl_list_index, int *may_clear_sessions)
{
  u32 *pacln;
  ...
  clib_warning("API dbg: acl_interface_set_inout_acl_list: sw_if_index %d is_input %d acl_vec: [%U]", sw_if_index, is_input, format_vec32, vec_acl_list_index, "%d");
  ...
  u32 **pinout_lc_index_by_sw_if_index = is_input ? &am->input_lc_index_by_sw_if_index
                                                  : &am->output_lc_index_by_sw_if_index;

  u32 ***pinout_acl_vec_by_sw_if_index = is_input ? &am->input_acl_vec_by_sw_if_index
                                                : &am->output_acl_vec_by_sw_if_index;

  u32 ***pinout_sw_if_index_vec_by_acl = is_input ? &am->input_sw_if_index_vec_by_acl
  ...
  clib_warning("btimaps: old seen %U new seen %U changed %U", format_bitmap_hex, old_seen_acl_bitmap, format_bitmap_hex, seen_acl_bitmap, format_bitmap_hex, change_acl_bitmap);
  ...
  */
  ...
  if (vec_len(vec_acl_list_index) > 0) {
    ...
    if (~0 == lc_index) {
      lc_index = acl_plugin_get_lookup_context_index(am->interface_acl_user_id, sw_if_index, is_input);
      ASSERT(lc_index >= 0);
...
static void
acl_interface_reset_inout_acls (u32 sw_if_index, u8 is_input, int *may_clear_sessions)
{
  acl_main_t *am = &acl_main;
  void *oldheap = acl_set_heap (am);
				 ...
				 u32 acl_list_index)
{
  ...
  acl_interface_set_inout_acl_list(am, sw_if_index, is_input, acl_vec, &may_clear_sessions);
  vec_free(acl_vec);
  return rv;
}
...
acl_plugin_show_interface (acl_main_t * am, u32 sw_if_index, int show_acl)
{
  ...
  for (swi = 0; (swi < vec_len (am->input_acl_vec_by_sw_if_index)) ||
	  ...
	  (vec_len (am->output_acl_vec_by_sw_if_index[swi]) > 0))
	{
      ...
      if (swi < vec_len (am->input_lc_index_by_sw_if_index))
        {
          vlib_cli_output(vm, "   input lookup context index: %d",
                          am->input_lc_index_by_sw_if_index[swi]);
        }
      if (swi < vec_len (am->output_lc_index_by_sw_if_index))
        {
...
#undef _
  ...
  am->interface_acl_user_id = acl_plugin_register_user_module("interface ACL", "sw_if_index", "is_input");
