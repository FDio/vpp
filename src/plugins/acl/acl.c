...
static int etype_whitelist_add_sessions(acl_main_t * am, u32 sw_if_index, int is_input, u32 etype_table_index)
{
  ...
  for(i=0; i<vec_len(whitelist); i++) {
    match[12] = whitelist[i] & 0xff;
    match[13] = (whitelist[i] >> 8) & 0xff;
    rv = rv || vnet_classify_add_del_session (cm, etype_table_index, match, next,
				 whitelist[i], 0, 0, 0, 1);
  }
...
acl_hook_l2_input_classify (acl_main_t * am, u32 sw_if_index)
{
  ...
  rv =
    acl_classify_add_del_table_tiny (cm, dot1ad_5tuple_mask,
				     sizeof (dot1ad_5tuple_mask) - 1, etype_table_index,
...
acl_plugin_show_interface (acl_main_t * am, u32 sw_if_index, int show_acl)
{
  ...
  for (swi = 0; (swi < vec_len (am->input_acl_vec_by_sw_if_index)) ||
	...
	continue;
      ...
      if ((swi < vec_len (am->input_etype_whitelist_by_sw_if_index)) &&
	  (vec_len (am->input_etype_whitelist_by_sw_if_index[swi]) > 0))
	{
	  vlib_cli_output (vm, "  input etype whitelist: %U", format_vec16,
			   am->input_etype_whitelist_by_sw_if_index[swi], "%04x");
        }
      if ((swi < vec_len (am->output_etype_whitelist_by_sw_if_index)) &&
	  (vec_len (am->output_etype_whitelist_by_sw_if_index[swi]) > 0))
	{
	  vlib_cli_output (vm, " output etype whitelist: %U", format_vec16,
			   am->output_etype_whitelist_by_sw_if_index[swi], "%04x");
        }
