...
static acl_action_t *
acl_compile (index_t acl_index, match_list_t * ml)
{
  acl_action_t *actions = NULL;
  acl_list_t *acl;
  acl_rule_t *ar;
  u8 *name;

  acl = acl_list_get (acl_index);
  name = format (NULL, "acl-%d", acl_index);

  match_list_init (ml, name, vec_len (acl->rules));

  vec_foreach (ar, acl->rules)
  {
    vec_add1 (actions, ar->action);
    match_list_push_back (ml, &ar->rule);
  }
...
acl_itf_acl_add (acl_itf_t * aitf, u32 acl_index)
{
  ...
  ah->acl_actions = acl_compile (acl_index, &ah->acl_match);
  ah->acl_hdl = match_set_list_add (aitf->match_set,
				    &ah->acl_match,
				    vec_len (aitf->acls), ah->acl_actions);
...
acl_itf_acl_remove (acl_itf_t * aitf, u32 acl_index)
{
  ...
  match_set_list_del (aitf->match_set, &ah->acl_hdl);

  vec_free (ah->acl_actions);
  match_list_free (&ah->acl_match);
...
acl_list_update (index_t * aip, acl_rule_t * rules, u8 * tag)
{
  ...
  acl_log_debug ("update: %U", format_acl, ai, ACL_FORMAT_BRIEF);
...
const static char * acl_feat[ACL_ITF_N_LAYERS][N_AF][VLIB_N_RX_TX] = {
  [ACL_ITF_LAYER_L2] = {
    [AF_IP4] = {
      [VLIB_RX] = "acl-plugin-in-ip4-l2",
      [VLIB_TX] = "acl-plugin-out-ip4-l2",
    },
    [AF_IP6] = {
      [VLIB_RX] = "acl-plugin-in-ip6-l2",
      [VLIB_TX] = "acl-plugin-out-ip6-l2",
    },
  },
  [ACL_ITF_LAYER_L3] = {
    [AF_IP4] = {
      [VLIB_RX] = "acl-plugin-in-ip4",
      [VLIB_TX] = "acl-plugin-out-ip4",
    },
    [AF_IP6] = {
      [VLIB_RX] = "acl-plugin-in-ip6",
      [VLIB_TX] = "acl-plugin-out-ip6",
    },
  },
};
/* *INDENT-ON* */
				  ...
				  int *may_clear_sessions)
{
  ...
  if (INDEX_INVALID == aitf->match_set)
    {
      // FIXME - need set for v4 and v6
      match_name = format (NULL, "acl-%U-%U",
			   format_vlib_rx_tx, dir,
			   format_vnet_sw_if_index_name, vnet_get_main (),
			   sw_if_index);

      aitf->match_set =
	match_set_create_and_lock (match_name, acl_mk_heap ());
      vec_free (match_name);
    }
  ...
  if (vec_len (aitf->acls))
    {
      ...
      FOR_EACH_IP_ADDRESS_FAMILY (af)
	match_set_unapply (aitf->match_set, &aitf->match_apps[af]);

      FOR_EACH_IP_ADDRESS_FAMILY (af)
	aitf->match_apps[af] = match_set_apply (aitf->match_set,
  ...
  else
    {
      ...
      FOR_EACH_IP_ADDRESS_FAMILY (af)
	match_set_unapply (aitf->match_set, &aitf->match_apps[af]);
      match_set_unlock (&aitf->match_set);
...
format_acl_list_hdl (u8 * s, va_list * args)
{
  ...
  s = format (s, "[acl:%d match-list:%d]", ah->acl_index, ah->acl_hdl);
...
format_acl_itf (u8 * s, va_list * args)
{
  ...
  if (flags & ACL_FORMAT_DETAIL)
	...
	s = format (s, "\n    %U", format_acl_list_hdl, ah);
      ...
      s = format (s, "\n  match-set:%d", aitf->match_set);
    }
  if (flags & ACL_FORMAT_VERBOSE)
    s = format (s, "\n  %U", format_match_set, aitf->match_set);
