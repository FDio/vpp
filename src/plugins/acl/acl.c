			   ...
			   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  u32 acl_index = ~0;
  if (!unformat (input, "index %u", &acl_index))
    {
	/*
         * I don't care about the return value since
         * on failure acl_index just stays ~0. But coverity does.
         * So make it happy.
         */
    }
  ...
  if(!unformat (input, "sw_if_index %u", &sw_if_index))
    {
       ...
       * since on error the sw_if_index just stays as it is.
       * But coverity does. So let it be happy.
       */
    }
				...
				vlib_cli_command_t * cmd)
{
  ...
  if(!unformat (input, "thread %u index %u", &show_session_thread_id,
	    &show_session_session_index))
    {
      /* the thread and session vars just remain ~0. */
    }
  if(!unformat (input, "verbose %u", &show_bihash_verbose))
    {
      /* same thing here. But let coverity be happy! */
    }
