...
static clib_error_t *
acl_set_macip_aclplugin_acl_fn (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
...
static clib_error_t *
macip_acl_delete_aclplugin_acl_fn (vlib_main_t *vm, unformat_input_t *input,
				   vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  int rv;
...
static clib_error_t *
macip_add_aclplugin_interface_fn (vlib_main_t *vm, unformat_input_t *input,
				  vlib_cli_command_t *cmd)
{
  int rv = 0;
  u32 sw_if_index = ~0;
 ...
 *
 *  @cliexcmd{set macip acl-plugin acl <permit|deny|permit+reflect>
 *  ip <PREFIX> mac <MAC> mask <int> [tag FOO] {use comma
 *  separated list for multiple rules}}
 ?*/
VLIB_CLI_COMMAND (aclplugin_set_acl_command_macip, static) = {
  .path = "set macip acl-plugin acl ",
  .short_help = "set macip acl-plugin acl <permit|deny|permit+reflect> "
		"ip <PREFIX> mac <MAC> mask <int> [tag FOO] {use comma "
		"separated list for multiple rules}",
  .function = acl_set_macip_aclplugin_acl_fn,
};
 ...
 *
 * @cliexpar
 * <b><em> set macip acl-plugin interface <interface> <acl INDEX> [del]
 </b></em>
 * @cliexend
 ?*/
VLIB_CLI_COMMAND (macip_aclplugin_set_interface_command, static) = {
  .path = "set macip acl-plugin interface",
  .short_help = "set macip acl-plugin interface <interface> <acl INDEX> [del]",
  .function = macip_add_aclplugin_interface_fn,
};
 ...
 *  any interface.
 *
 * @cliexcmd{delete macip acl-plugin acl index <idx>}
 ?*/
VLIB_CLI_COMMAND (macip_aclplugin_delete_acl_command, static) = {
  .path = "delete macip acl-plugin acl",
  .short_help = "delete macip acl-plugin acl index <idx>",
  .function = macip_acl_delete_aclplugin_acl_fn,
};
