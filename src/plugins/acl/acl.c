...
static void
acl_print_acl_x (acl_vector_print_func_t vpr, vlib_main_t *vm, acl_main_t *am,
		 int acl_index)
{
  acl_rule_t *r;
  ...
  for (j = 0; j < vec_len (acl_rules); j++)
    {
      ...
      out0 =
	format (out0, " src %U/%d", format_ip46_address, &r->src,
	...
	{
	  out0 = format (out0, " tcpflags %d mask %d", r->tcp_flags_value,
			 r->tcp_flags_mask);
	}
      out0 = format (out0, "\n");
      vpr (vm, out0);
  ...
  vl_api_acl_plugin_get_conn_table_max_entries_t *mp)
{
  ...
  rmp->_vl_msg_id = ntohs (VL_API_ACL_PLUGIN_GET_CONN_TABLE_MAX_ENTRIES_REPLY +
			   am->msg_id_base);
  rmp->context = mp->context;
...
increment_policy_epoch (acl_main_t *am, u32 sw_if_index, int is_input)
{
  ...
  u32 **ppolicy_epoch_by_swi = is_input ?
...
validate_and_reset_acl_counters (acl_main_t *am, u32 acl_index)
{
  ...
  for (i = old_len; i < vec_len (am->combined_acl_counters); i++)
	...
	(void *) format (0, "/acl/%d/matches%c", i, 0);
...
static int
acl_add_list (u32 count, vl_api_acl_rule_t rules[], u32 *acl_list_index,
	      u8 *tag)
{
  acl_main_t *am = &acl_main;
  acl_list_t *a;
...
acl_del_list (u32 acl_list_index)
{
  ...
  /* lookup contexts cover other cases, not just inbound/outbound, so check
   * that */
...
{
  u16 **v = is_input ? am->input_etype_whitelist_by_sw_if_index :
				 ...
				 int enable_disable)
{
  ...
  if (intf_has_etype_whitelist (am, sw_if_index, 1))
    vnet_l2_feature_enable_disable ("l2-input-nonip", "acl-plugin-in-nonip-l2",
				    sw_if_index, enable_disable, 0, 0);
...
static int
acl_interface_set_inout_acl_list (acl_main_t *am, u32 sw_if_index, u8 is_input,
				  ...
				  int *may_clear_sessions)
{
  ...
  if (am->trace_acl > 255)
    clib_warning ("API dbg: acl_interface_set_inout_acl_list: sw_if_index %d "
		  "is_input %d acl_vec: [%U]",
  ...
  clib_bitmap_foreach (acln, change_acl_bitmap)
      ...
      if (clib_bitmap_get (old_seen_acl_bitmap, acln))
	{
	  ...
	  if (acln < vec_len ((*pinout_sw_if_index_vec_by_acl)))
	    {
	      int index = vec_search ((*pinout_sw_if_index_vec_by_acl)[acln],
				      sw_if_index);
  ...
  if (am->reclassify_sessions)
    ...
    {
      /* if no commonalities between the ACL# - then we should definitely clear
       * the sessions */
  ...
  /*
   * prepare or delete the lookup context if necessary, and if context exists,
   * set ACL list
   */
  ...
  if (vec_len (vec_acl_list_index) > 0)
    {
      ...
      if (~0 == lc_index)
	{
	  lc_index = acl_plugin.get_lookup_context_index (
	    am->interface_acl_user_id, sw_if_index, is_input);
	...
	{
	  acl_plugin.put_lookup_context_index (
	    (*pinout_lc_index_by_sw_if_index)[sw_if_index]);
...
macip_create_classify_tables (acl_main_t *am, u32 macip_acl_index)
{
  ...
  for (i = 0; i < a->count; i++)
    {
      if (~0 == (match_type_index = macip_find_match_type (
		   mvec, a->rules[i].src_mac_mask, a->rules[i].src_prefixlen,
		   a->rules[i].is_ipv6)))
	{
	  match_type_index = vec_len (mvec);
	  vec_validate (mvec, match_type_index);
	  memcpy (mvec[match_type_index].mac_mask, a->rules[i].src_mac_mask,
		  6);
  ...
  /* Put the most frequently used tables last in the list so we can create
   * classifier tables in reverse order */
      ...
      if (!is6)
	{
	  ...
	  for (tags = 2; tags >= 0; tags--)
	      ...
	      if (mt->has_egress)
		{
		  ...
		  /* AYXX: FIXME here - can we tighten the ARP-related table
		   * more ? */
		  /* mask captures just the destination and the ethertype */
		  mask_len =
		    ((l3_offset + (sizeof (u32x4) - 1)) / sizeof (u32x4)) *
		    sizeof (u32x4);
		  acl_classify_add_del_table_small (
		    cm, mask, mask_len, out_last_table,
		    (~0 == out_last_table) ? 0 : ~0, out_last_tag_table, 1);
  ...
  for (i = 0; i < a->count; i++)
	...
	macip_find_match_type (mvec, a->rules[i].src_mac_mask,
			       a->rules[i].src_prefixlen, a->rules[i].is_ipv6);
	  ...
	  if (!is6 && (mvec[match_type_index].out_arp_table_index != ~0))
	      ...
	      for (tags = 2; tags >= 0; tags--)
		{
		  ...
		  vnet_classify_add_del_session (
		    cm, tag_table, mask, a->rules[i].is_permit ? ~0 : 0, i, 0,
		    action, metadata, 1);
		}
	    }
	}
...
macip_destroy_classify_tables (acl_main_t *am, u32 macip_acl_index)
{
  ...
  if (a->ip4_table_index != ~0)
    {
      acl_classify_add_del_table_small (cm, 0, ~0, ~0, ~0, &a->ip4_table_index,
					0);
      a->ip4_table_index = ~0;
    }
  if (a->ip6_table_index != ~0)
    {
      acl_classify_add_del_table_small (cm, 0, ~0, ~0, ~0, &a->ip6_table_index,
					0);
      a->ip6_table_index = ~0;
    }
					     ...
					     int is_apply)
{
  ...
  for (i = 0; i < vec_len (am->macip_acl_by_sw_if_index); i++)
	...
	rv = rv || rv0;
	rv0 = vnet_set_output_acl_intfc (
	  am->vlib_main, i, a->out_ip4_table_index, a->out_ip6_table_index,
		    ...
		    u32 *acl_list_index, u8 *tag)
{
  ...
  if (*acl_list_index != ~0)
	...
	{
	  /* tried to replace a non-existent ACL, no point doing anything */
	  clib_warning ("acl-plugin-error: Trying to replace nonexistent "
			"MACIP ACL %d (tag %s)",
			*acl_list_index, tag);
	  return VNET_API_ERROR_NO_SUCH_ENTRY;
	}
  ...
  /* if replacing the ACL, unapply the classifier tables first - they will be
   * gone.. */
  if (~0 != *acl_list_index)
   ...
   */
  rv =
    rv || macip_maybe_apply_unapply_classifier_tables (am, *acl_list_index, 1);
  return rv;
}
...
macip_acl_interface_del_acl (acl_main_t *am, u32 sw_if_index)
{
  ...
  rv |= vnet_set_output_acl_intfc (
    am->vlib_main, sw_if_index, a->out_ip4_table_index, a->out_ip6_table_index,
    a->out_l2_table_index, 0);
  /* Unset the MACIP ACL index */
  am->macip_acl_by_sw_if_index[sw_if_index] = ~0;
  /* macip_acl_interface_add_acl did a vec_add1() to this previously, so
   * [sw_if_index] should be valid */
...
vl_api_acl_interface_add_del_t_handler (vl_api_acl_interface_add_del_t *mp)
{
  ...
  if (pool_is_free_index (im->sw_interfaces, sw_if_index))
    ...
    rv = acl_interface_add_del_inout_acl (sw_if_index, mp->is_add,
  ...
  vl_api_acl_interface_set_acl_list_t *mp)
{
  ...
  if (pool_is_free_index (im->sw_interfaces, sw_if_index))
      ...
      if (0 == rv)
	{
	  ...
	  rv = acl_interface_set_inout_acl_list (
	    am, sw_if_index, 0, out_acl_vec, &may_clear_sessions);
	  rv = rv || acl_interface_set_inout_acl_list (
		       am, sw_if_index, 1, in_acl_vec, &may_clear_sessions);
	  vec_free (in_acl_vec);
...
static void
send_acl_details (acl_main_t *am, vl_api_registration_t *reg, acl_list_t *acl,
		  u32 context)
{
...
static void
send_acl_interface_list_details (acl_main_t *am, vl_api_registration_t *reg,
...
vl_api_acl_interface_list_dump_t_handler (vl_api_acl_interface_list_dump_t *mp)
{
  ...
  if (mp->sw_if_index == ~0)
	...
	{
	  send_acl_interface_list_details (am, reg, swif->sw_if_index,
					   mp->context);
	}
      /* *INDENT-ON* */
    }
  ...
  vl_api_macip_acl_interface_add_del_t *mp)
{
  ...
  if (pool_is_free_index (im->sw_interfaces, sw_if_index))
    ...
    rv = macip_acl_interface_add_del_acl (ntohl (mp->sw_if_index), mp->is_add,
					  ntohl (mp->acl_index));
  ...
  if (sw_if_index == ~0)
      ...
      vec_foreach_index (sw_if_index, am->macip_acl_by_sw_if_index)
	{
	  if (~0 != am->macip_acl_by_sw_if_index[sw_if_index])
		...
		am, reg, sw_if_index,
		am->macip_acl_by_sw_if_index[sw_if_index], mp->context);
	    }
	}
    }
  else
	...
	{
	  send_macip_acl_interface_list_details (
	    am, reg, sw_if_index, am->macip_acl_by_sw_if_index[sw_if_index],
	    mp->context);
	}
    }
}
  ...
  if (mp->sw_if_index == ~0)
      ...
      pool_foreach (swif, im->sw_interfaces)
	{
	  send_acl_interface_etype_whitelist_details (
	    am, reg, swif->sw_if_index, mp->context);
	}
      /* *INDENT-ON* */
    }
  else
...
static clib_error_t *
acl_set_aclplugin_fn (vlib_main_t *vm, unformat_input_t *input,
		      vlib_cli_command_t *cmd)
{
  ...
  if (unformat (input, "event-trace"))
	...
	{
	  error = clib_error_return (0, "expecting trace level, got `%U`",
  ...
  if (unformat (input, "session"))
	...
	{
	  /* The commands here are for tuning/testing. No user-serviceable
	   * parts inside */
	  if (unformat (input, "max-entries"))
	      ...
	      if (!unformat (input, "%u", &val))
		{
		  error = clib_error_return (
		    0, "expecting maximum number of entries, got `%U`",
		    format_unformat_error, input);
		  goto done;
		}
	      else
	  ...
	  if (unformat (input, "event-trace"))
		...
		{
		  error =
		    clib_error_return (0, "expecting trace level, got `%U`",
				       format_unformat_error, input);
		  goto done;
		}
	      else
	  ...
	  if (unformat (input, "udp"))
	      ...
	      if (unformat (input, "idle"))
		{
		  if (!unformat (input, "%u", &timeout))
		    {
		      error = clib_error_return (
			0, "expecting timeout value in seconds, got `%U`",
			format_unformat_error, input);
		      goto done;
		    }
		  else
...
my_macip_acl_rule_t_pretty_format (u8 *out, va_list *args)
{
  ...
  out = format (
    out, "%s action %d ip %U/%d mac %U mask %U", a->is_ipv6 ? "ipv6" : "ipv4",
    a->is_permit, format_ip46_address, &a->src_ip_addr,
    a->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, a->src_prefixlen,
    my_format_mac_address, a->src_mac, my_format_mac_address, a->src_mac_mask);
  return (out);
}
...
macip_acl_print (acl_main_t *am, u32 macip_acl_index)
{
  ...
  vlib_cli_output (vm,
		   "MACIP acl_index: %d, count: %d (true len %d) tag {%s} is "
		   "free pool slot: %d\n",
		   macip_acl_index, a->count, vec_len (a->rules), a->tag,
		   free_pool_slot);
  vlib_cli_output (
    vm, "  ip4_table_index %d, ip6_table_index %d, l2_table_index %d\n",
    a->ip4_table_index, a->ip6_table_index, a->l2_table_index);
  vlib_cli_output (vm,
		   "  out_ip4_table_index %d, out_ip6_table_index %d, "
		   "out_l2_table_index %d\n",
...
static clib_error_t *
acl_set_aclplugin_interface_fn (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
...
static clib_error_t *
acl_set_aclplugin_acl_fn (vlib_main_t *vm, unformat_input_t *input,
			  vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (line_input, "action %d", &action))
	{
      ...
      else if (unformat (line_input, "src %U/%d", unformat_ip46_address, &src,
			 IP46_TYPE_ANY, &src_prefix_length))
	{
	  vec_validate_acl_rules (rules, rule_idx);
      ...
      else if (unformat (line_input, "dst %U/%d", unformat_ip46_address, &dst,
			 IP46_TYPE_ANY, &dst_prefix_length))
	{
	  vec_validate_acl_rules (rules, rule_idx);
      ...
      else if (unformat (line_input, "tcpflags %d %d", &tcpflags, &tcpmask))
	{
      ...
      else if (unformat (line_input, "tcpflags %d mask %d", &tcpflags,
			 &tcpmask))
	{
	  vec_validate_acl_rules (rules, rule_idx);
	  rules[rule_idx].tcp_flags_value = tcpflags;
...
static clib_error_t *
acl_show_aclplugin_macip_acl_fn (vlib_main_t *vm, unformat_input_t *input,
				 vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
  acl_main_t *am = &acl_main;
  ...
  for (i = 0; i < vec_len (am->macip_acls); i++)
	...
	{
	  vlib_cli_output (
	    vm, "  applied on sw_if_index(s): %U\n", format_vec32,
	    vec_elt (am->sw_if_index_vec_by_macip_acl, i), "%d");
	}
    }
...
static clib_error_t *
acl_show_aclplugin_acl_fn (vlib_main_t *vm, unformat_input_t *input,
			   vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
  acl_main_t *am = &acl_main;
...
static clib_error_t *
acl_show_aclplugin_lookup_context_fn (vlib_main_t *vm, unformat_input_t *input,
				      vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
...
static clib_error_t *
acl_show_aclplugin_lookup_user_fn (vlib_main_t *vm, unformat_input_t *input,
				   vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
			   ...
			   int detail)
{
  ...
  for (swi = 0; (swi < vec_len (am->input_acl_vec_by_sw_if_index)) ||
		(swi < vec_len (am->output_acl_vec_by_sw_if_index));
       swi++)
    {
...
static clib_error_t *
acl_show_aclplugin_decode_5tuple_fn (vlib_main_t *vm, unformat_input_t *input,
				     vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
  u64 five_tuple[6] = { 0, 0, 0, 0, 0, 0 };

  if (unformat (input, "%llx %llx %llx %llx %llx %llx", &five_tuple[0],
...
static clib_error_t *
acl_show_aclplugin_interface_fn (vlib_main_t *vm, unformat_input_t *input,
				 vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
  acl_main_t *am = &acl_main;
...
static clib_error_t *
acl_show_aclplugin_memory_fn (vlib_main_t *vm, unformat_input_t *input,
			      vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
...
static void
acl_plugin_show_sessions (acl_main_t *am, u32 show_session_thread_id,
			  u32 show_session_session_index)
{
  ...
  for (wk = 0; wk < vec_len (am->per_worker_data); wk++)
	...
	{
	  vlib_cli_output (vm,
			   "  session index %u:", show_session_session_index);
	  fa_session_t *sess =
	    pw->fa_sessions_pool + show_session_session_index;
	  u64 *m = (u64 *) &sess->info;
	  vlib_cli_output (
	    vm, "    info: %016llx %016llx %016llx %016llx %016llx %016llx",
      ...
      pool_foreach (swif, im->sw_interfaces)
	{
	  ...
	  u64 n_epoch_changes =
	    (sw_if_index <
		 vec_len (pw->fa_session_epoch_change_by_sw_if_index) ?
	       pw->fa_session_epoch_change_by_sw_if_index[sw_if_index] :
	       0);
	  vlib_cli_output (
	    vm, "    sw_if_index %d: add %lu - del %lu = %lu; epoch chg: %lu",
	    sw_if_index, n_adds, n_dels, n_adds - n_dels, n_epoch_changes);
	}
      /* *INDENT-ON* */
...
#define _(cnt, desc)                                                          \
  vlib_cli_output (vm, "             %20lu: %s", am->cnt, desc);
...
#undef _
  ...
  vlib_cli_output (
    ...
    ((f64) am->fa_current_cleaner_timer_wait_interval) * 1000.0 /
      (f64) vm->clib_time.clocks_per_second);
  vlib_cli_output (vm, "Reclassify sessions: %d", am->reclassify_sessions);
}

static clib_error_t *
acl_show_aclplugin_sessions_fn (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
  acl_main_t *am = &acl_main;
...
static clib_error_t *
acl_show_aclplugin_tables_fn (vlib_main_t *vm, unformat_input_t *input,
			      vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
  ...
  if (!(show_mask_type || show_acl_hash_info || show_applied_info ||
	show_bihash))
    {
...
static clib_error_t *
acl_clear_aclplugin_fn (vlib_main_t *vm, unformat_input_t *input,
			vlib_cli_command_t *cmd)
{
  clib_error_t *error = 0;
  acl_main_t *am = &acl_main;
...
VLIB_CLI_COMMAND (aclplugin_set_command, static) = {
  .path = "set acl-plugin",
  .short_help = "set acl-plugin session timeout {{udp idle}|tcp "
		"{idle|transient}} <seconds>",
...
VLIB_CLI_COMMAND (aclplugin_show_tables_command, static) = {
  ...
  .short_help = "show acl-plugin tables [ acl [index N] | applied [ lc_index "
		"N ] | mask | hash [verbose N] ]",
 ...
 *
 * @cliexpar
 * <b><em> set acl-plugin acl <permit|deny> src <PREFIX> dst <PREFIX> proto
 <TCP|UDP> sport <X-Y> dport <X-Y> [tag FOO] </b></em>
 * @cliexend
 ?*/
VLIB_CLI_COMMAND (aclplugin_set_acl_command, static) = {
  .path = "set acl-plugin acl",
  .short_help =
    "set acl-plugin acl <permit|deny> src <PREFIX> dst <PREFIX> proto X sport "
    "X-Y dport X-Y [tag FOO] {use comma separated list for multiple rules}",
...
acl_plugin_config (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "connection hash buckets %d",
		    &conn_table_hash_buckets))
	am->fa_conn_table_hash_num_buckets = conn_table_hash_buckets;
      else if (unformat (input, "connection hash memory %U",
			 unformat_memory_size, &conn_table_hash_memory_size))
	...
	am->fa_conn_table_max_entries = conn_table_max_entries;
      else if (unformat (input, "main heap size %U", unformat_memory_size,
			 &main_heap_size))
	clib_warning ("WARNING: ACL heap is now part of the main heap. 'main "
		      "heap size' is ineffective.");
      else if (unformat (input, "hash lookup heap size %U",
			 unformat_memory_size, &hash_heap_size))
	clib_warning ("WARNING: ACL heap is now part of the main heap. 'hash "
		      "lookup heap size' is ineffective.");
	...
	am->hash_lookup_hash_buckets = hash_lookup_hash_buckets;
      else if (unformat (input, "hash lookup hash memory %U",
			 unformat_memory_size, &hash_lookup_hash_memory))
	...
	am->use_tuple_merge = use_tuple_merge;
      else if (unformat (input, "tuple merge split threshold %d",
...
acl_init (vlib_main_t *vm)
{
  ...
  vec_validate (am->per_worker_data, tm->n_vlib_mains - 1);
  {
	...
	vec_validate (pw->expired, ACL_N_TIMEOUTS *
				     am->fa_max_deleted_sessions_per_interval);
...
#define _(N, v, s)                                                            \
  am->fa_ipv6_known_eh_bitmap =                                               \
    clib_bitmap_set (am->fa_ipv6_known_eh_bitmap, v, 1);
  foreach_acl_eh
#undef _
