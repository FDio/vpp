...
class AclRule():
    ...
    def __init__(self, is_permit, src_prefix=IPv4Network('0.0.0.0/0'),
                 dst_prefix=IPv4Network('0.0.0.0/0'), proto=0, ports=PORTS_ALL):
    ...
    def update_ports(self):
        if self._ports == self.PORTS_ALL:
            self.sport_from = 0
            self.dport_from = 0
            self.sport_to = 65535 if self._proto != 1 and self._proto != 58 else 255
...
class VppMacipAclInterface(VppObject):
    ...
    def add_vpp_config(self):
        for acl in self.acls:
            self._test.vapi.macip_acl_interface_add_del(is_add=True,
                                                        sw_if_index=self._sw_if_index, acl_index=acl.acl_index)
        self._test.registry.register(self, self._test.logger)

    def remove_vpp_config(self):
        for acl in self.acls:
            self._test.vapi.macip_acl_interface_add_del(is_add=False,
                                                        sw_if_index=self._sw_if_index, acl_index=acl.acl_index)
