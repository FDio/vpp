...
from vpp_sub_interface import L2_VTR_OP, VppSubInterface, VppDot1QSubint, \
    VppDot1ADSubint
from vpp_acl import AclRule, VppAcl, VppAclInterface, VppEtypeWhitelist, VppMacipAclInterface, VppMacipAcl, MacipRule
    ...
    def create_rules(self, mac_type=EXACT_MAC, ip_type=EXACT_IP,
        ...
        for acl in range(2, (acl_count+1) * 2):
            ...
            for i in range(0, (rules_count[int((acl / 2) - 1)])):
                ...
                rule = MacipRule(is_permit=self.PERMIT, src_prefix=ip_network((ip_pack, ip_len)),
    ...
    def create_stream(self, mac_type, ip_type, packet_count,
        ...
        for p in range(0, packet_count):
            ...
            if is_permit:
                ...
                src_network = IPv6Network((packet[rule_l3_layer].src, rule_prefix_len)) if is_ip6 else IPv4Network(
                    (packet[rule_l3_layer].src, rule_prefix_len))
                dst_network = IPv6Network((packet[rule_l3_layer].dst, rule_prefix_len)) if is_ip6 else IPv4Network(
                ...
                acl_rule.sport_fo = rule_l4_sport
...
class TestMACIP(MethodHolder):
    ...
    def test_delete_intf(self):
        ...
        intf = []
        macip_alcs = self.apply_macip_rules(self.create_rules(
                                            acl_count=3, rules_count=[3, 5, 4]))
