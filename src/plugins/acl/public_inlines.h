...
acl_plugin_match_5tuple_inline_and_count (void *p_acl_main, u32 lc_index,
					   ...
					   u32 packet_size)
{
  static volatile u32 test_acl_match = 0;
  static volatile u32 test_ace_match = 0;
  ...
  if (PREDICT_TRUE(ret)) {
	  ...
	  vlib_increment_combined_counter(am->combined_acl_counters + test_acl_match, thread_index, test_ace_match, 1, packet_size);
  }
  return ret;
}
