...
always_inline void
tm_multi_acl_match_get_applied_ace_index_x2(acl_main_t *am, u32 lc_index, fa_5tuple_t *match0, fa_5tuple_t *match1, u32 *r_match_index0, u32 *r_match_index1)
{
  clib_bihash_kv_48_8_t kv0;
  clib_bihash_kv_48_8_t kv1;
  clib_bihash_kv_48_8_t result0;
  clib_bihash_kv_48_8_t result1;
  fa_5tuple_t *kv_key0 = (fa_5tuple_t *)kv0.key;
  fa_5tuple_t *kv_key1 = (fa_5tuple_t *)kv1.key;
  hash_acl_lookup_value_t *result_val0 = (hash_acl_lookup_value_t *)&result0.value;
  hash_acl_lookup_value_t *result_val1 = (hash_acl_lookup_value_t *)&result1.value;

  u64 *pmatch0 = (u64 *)match0;
  u64 *pmask0;
  u64 *pkey0;
  u64 *pmatch1 = (u64 *)match1;
  u64 *pmask1;
  u64 *pkey1;
  int mask_type_index, order_index;
  u32 curr_match_index0 = (~0 -1);
  u32 curr_match_index1 = (~0 -1);
...
//Added by Valerio
  ...
  u32 cand_ord_index0=0; 
  u32 cand_ord_index1=0; 
  u32 count_htaccess=0; 
  u32 count_col=0;
  ...
  for(order_index = 0; order_index < vec_len((*hash_applied_mask_pool)); order_index++) {
    ...
    if  (minfo->max_priority > (curr_match_index0+1)) {
      in_progress0 = 0;
    }
    if  (minfo->max_priority > (curr_match_index1+1)) {
      in_progress1 = 0;
    }
    if ((in_progress0 | in_progress1) == 0) {
      break;
    }
    ...
    if (in_progress0) { 
      *pkey0++ = *pmatch0++ & *pmask0++;
      *pkey0++ = *pmatch0++ & *pmask0++;
      *pkey0++ = *pmatch0++ & *pmask0++;
      *pkey0++ = *pmatch0++ & *pmask0++;
      *pkey0++ = *pmatch0++ & *pmask0++;
      *pkey0++ = *pmatch0++ & *pmask0++;
      kv_key0->pkt.mask_type_index_lsb = mask_type_index;
#ifdef VALE_ELOG_ACL
  count_htaccess++; 
#endif
      res0 = clib_bihash_search_inline_2_48_8 (&am->acl_lookup_hash, &kv0, &result0);
    }

    if (in_progress1) { 
      *pkey1++ = *pmatch1++ & *pmask1++;
      *pkey1++ = *pmatch1++ & *pmask1++;
      *pkey1++ = *pmatch1++ & *pmask1++;
      *pkey1++ = *pmatch1++ & *pmask1++;
      *pkey1++ = *pmatch1++ & *pmask1++;
      *pkey1++ = *pmatch1++ & *pmask1++;
      kv_key1->pkt.mask_type_index_lsb = mask_type_index;
#ifdef VALE_ELOG_ACL
  count_htaccess++; 
#endif
      res1 = clib_bihash_search_inline_2_48_8 (&am->acl_lookup_hash, &kv1, &result1);
    }



    if (res0 == 0 && in_progress0) {
	    //check collisions
	    u32 curr_index = result_val0->applied_entry_index;
	    applied_hash_ace_entry_t *pae = vec_elt_at_index((*applied_hash_aces),curr_index);
	    u64 collisions = pae->collision + 1;
	    int i=0;
	    for(i=0; i < collisions; i++){
		    pae = vec_elt_at_index((*applied_hash_aces),curr_index);
		    if(curr_index < curr_match_index0){
			    //Added by Valerio
#ifdef VALE_ELOG_ACL
			    if(collisions > 1) count_col= (count_col + 1);
#endif
			    if(tm_single_ace_match_5tuple(am, pae->acl_index, pae->ace_index, match0)){
				    curr_match_index0 = curr_index;
#ifdef VALE_ELOG_ACL
				    cand_ord_index0 = order_index;
#endif
			    }
		    }
		    curr_index = pae->next_applied_entry_index;
    ...
    if (res1 == 0 && in_progress1) {
	    //check collisions
	    u32 curr_index = result_val1->applied_entry_index;
	    applied_hash_ace_entry_t *pae = vec_elt_at_index((*applied_hash_aces),curr_index);
	    u64 collisions = pae->collision + 1;
	    int i=0;
	    for(i=0; i < collisions; i++){
		    pae = vec_elt_at_index((*applied_hash_aces),curr_index);
		    if(curr_index < curr_match_index1){
			    //Added by Valerio
#ifdef VALE_ELOG_ACL
			    if(collisions > 1) count_col= (count_col + 1);
#endif
			    if(tm_single_ace_match_5tuple(am, pae->acl_index, pae->ace_index, match1)){
				    curr_match_index1 = curr_index;
#ifdef VALE_ELOG_ACL
				    cand_ord_index1 = order_index;
#endif
			    }
		    }
		    curr_index = pae->next_applied_entry_index;
  ...
  
  {
    ...
    //number of collisions
    ed->col = count_col;
    ed->cand_ord_index = cand_ord_index0;
    if (curr_match_index < vec_len((*applied_hash_aces))) {
      applied_hash_ace_entry_t *pae = vec_elt_at_index((*applied_hash_aces), curr_match_index0);
      ed->ace_i = pae->ace_index;
    }
    /*End of Log event*/
  }
  {
    ...
    //number of collisions
    ed->col = count_col;
    ed->cand_ord_index = cand_ord_index1;
    if (curr_match_index < vec_len((*applied_hash_aces))) {
      applied_hash_ace_entry_t *pae = vec_elt_at_index((*applied_hash_aces), curr_match_index1);
      ed->ace_i = pae->ace_index;
    }
    /*End of Log event*/
  }
...
#endif
  *r_match_index0 = curr_match_index0;
  *r_match_index1 = curr_match_index1;
...
always_inline void
acl_plugin_match_5tuple_inline_x2 (u32 lc_index,
                                           fa_5tuple_t * pkt_5tuple0,
                                           fa_5tuple_t * pkt_5tuple1,
                                           int is_ip6,
                                           u8 * r_action0,
                                           u8 * r_action1,
                                           u32 * r_acl_pos_p0,
...
{
  acl_main_t *am = p_acl_main;
  fa_5tuple_t * pkt_5tuple_internal0 = (fa_5tuple_t *)pkt_5tuple0;
  fa_5tuple_t * pkt_5tuple_internal1 = (fa_5tuple_t *)pkt_5tuple1;

  pkt_5tuple_internal0->pkt.lc_index = lc_index;
  pkt_5tuple_internal1->pkt.lc_index = lc_index;

  int is_frag0 = pkt_5tuple_internal0->pkt.is_nonfirst_fragment;
  int is_frag1 = pkt_5tuple_internal1->pkt.is_nonfirst_fragment;

  if (PREDICT_TRUE(am->use_hash_acl_matching)) {
    if (PREDICT_FALSE(is_frag0 || is_frag1)) {
      /*
       * For the non-initial fragments we can not build the key with ports,
       * thus the tuplemerge algorithm as is will give incorrect result.
       * Fallback to linear search in that case.
      ...
      linear_multi_acl_match_5tuple(lc_index, pkt_5tuple_internal0, is_ip6, r_action0,
                                 r_acl_pos_p0, r_acl_match_p0, r_rule_match_p0, trace_bitmap0);
      linear_multi_acl_match_5tuple(lc_index, pkt_5tuple_internal1, is_ip6, r_action1,
                                 r_acl_pos_p1, r_acl_match_p1, r_rule_match_p1, trace_bitmap1);
    } else {
      applied_hash_ace_entry_t **applied_hash_aces = vec_elt_at_index(am->hash_entry_vec_by_lc_index, lc_index);
      ...
      tm_multi_acl_match_get_applied_ace_index_x2(am, lc_index, pkt_5tuple_internal0, pkt_5tuple_internal1, &match_index0, &match_index1);

      if (match_index0 < vec_len((*applied_hash_aces))) {
	applied_hash_ace_entry_t *pae = vec_elt_at_index((*applied_hash_aces), match_index0);
	pae->hitcount++;
	*r_acl_pos_p0 = pae->acl_position;
	*r_acl_match_p0 = pae->acl_index;
	*r_rule_match_p0 = pae->ace_index;
	*r_action0 = pae->action;
      }
      if (match_index1 < vec_len((*applied_hash_aces))) {
	applied_hash_ace_entry_t *pae = vec_elt_at_index((*applied_hash_aces), match_index1);
	pae->hitcount++;
	*r_acl_pos_p1 = pae->acl_position;
	*r_acl_match_p1 = pae->acl_index;
	*r_rule_match_p1 = pae->ace_index;
	*r_action1 = pae->action;
      }
    }
  } else {
    linear_multi_acl_match_5tuple(lc_index, pkt_5tuple_internal0, is_ip6, r_action0,
