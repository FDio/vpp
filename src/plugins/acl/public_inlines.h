...
static void __acl_plugin_init_p_acl_main (void)
{
  ...
  /*
   * The below will fail for two reasons:
   * 1) vlib_get_plugin_symbol() uses hash by name which at this point
   *    in time is not valid, after the vector of plugin infos
   *    was sorted alphabetically.
   * 2) even if the hash is valid, we can not guarantee that the 
   *    requested plugin has already been loaded, so the dlsym will fail.
   *
   * One option is to have this code push something on to a name-indexed
   * list of initializers in the plugin infra, so the below code could
   * be executed just after the acl plugin initialization ?... 
   */

  clib_error_t * err = acl_plugin_exports_init(&m);
  if (err) {
    clib_error("Could not init the acl plugin exports: %U", format_clib_error, err);
  }
  p_acl_main = m.get_p_acl_main();
  clib_warning("ACL-plugin init: setting p_acl_main at %p to %p\n", &p_acl_main, p_acl_main);
}
