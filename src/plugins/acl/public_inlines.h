...
always_inline int
single_rule_match_5tuple (acl_rule_t * r, fa_5tuple_t * pkt_5tuple)
{
  int is_ip6 = pkt_5tuple->pkt.is_ip6;
...
always_inline int
single_ace_match_5tuple (acl_main_t * am, u32 acl_index, u32 ace_index,
			 fa_5tuple_t * pkt_5tuple)
{
  acl_list_t *a = &am->acls[acl_index];
  acl_rule_t *r = &a->rules[ace_index];
  return single_rule_match_5tuple (r, pkt_5tuple);
}
      ...
      if (res == 0)
	{
	  ...
	  for (i = 0; i < vec_len (crs); i++)
	      ...
	      if (crs[i].applied_entry_index >= curr_match_index)
		{
	      ...
	      if (single_rule_match_5tuple (&crs[i].rule, match))
		{
