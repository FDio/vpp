...
always_inline void
acl_fill_tuple_l4(acl_main_t * am, vlib_buffer_t * b0, int l4_offset, int proto, int icmp_proto_val, fa_5tuple_t * p5tuple_pkt)
{
  u16 ports[2];
...
always_inline u16
acl_fill_tuple_ip6_get_proto (acl_main_t * am, vlib_buffer_t * b0, int l3_offset, fa_5tuple_t * p5tuple_pkt, const int fill_l4)
{
  ...
  if (fill_l4) {
    ...
    acl_fill_tuple_l4(am, b0, l4_offset, proto, IP_PROTOCOL_ICMP6, p5tuple_pkt);
  } else {
...
always_inline u16
acl_fill_tuple_ip4_get_proto (acl_main_t * am, vlib_buffer_t * b0, int l3_offset, fa_5tuple_t * p5tuple_pkt, const int fill_l4)
{
  u8 proto = 0;
 // memset(p5tuple_pkt->kv.key, 0, 4*sizeof(p5tuple_pkt->kv.key[0]));
  ...
  if (fill_l4) {
    p5tuple_pkt->l4.proto = proto;
    acl_fill_tuple_l4(am, b0, l4_offset, proto, IP_PROTOCOL_ICMP, p5tuple_pkt);
  } else {
    vnet_buffer (b0)->l4_hdr_offset = l4_offset;
  }
  return proto;
...
always_inline void
acl_fill_5tuple (acl_main_t * am, vlib_buffer_t * b0, const int is_ip6,
		 const int is_input, const int is_l2_path, fa_5tuple_t * p5tuple_pkt)
{
  ...
  if (is_ip6)
    {
      acl_fill_tuple_ip6_get_proto(am, b0, l3_offset, p5tuple_pkt, 1);
    }
  else
    {
      acl_fill_tuple_ip4_get_proto(am, b0, l3_offset, p5tuple_pkt, 1);
    }
}
...
acl_fill_5tuple_l3_get_proto (acl_main_t * am, vlib_buffer_t * b0, const int is_ip6, const int l3_offset, fa_5tuple_t * p5tuple_pkt)
{
  ...
  if (is_ip6)
    {
      proto = acl_fill_tuple_ip6_get_proto(am, b0, l3_offset, p5tuple_pkt, 0);
    }
  else
    {
      proto = acl_fill_tuple_ip4_get_proto(am, b0, l3_offset, p5tuple_pkt, 0);
    }
  return proto;
}
