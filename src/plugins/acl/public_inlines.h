...
acl_plugin_exports_init (acl_plugin_methods_t *m)
{
  acl_plugin_methods_vtable_init_fn_t mvi;

  LOAD_SYMBOL_FROM_PLUGIN_TO ("acl_plugin.so", acl_plugin_methods_vtable_init,
			      mvi);
  return (mvi (m));
}
...
{
  /* For the purposes of this code, "within" means we have at least 8 bytes
   * after it */
...
always_inline void
acl_fill_5tuple_l4_and_pkt_data (acl_main_t *am, u32 sw_if_index0,
				 vlib_buffer_t *b0, int is_ip6, int is_input,
				 int l3_offset,
  ...
  if (is_ip6)
    {
      ...
      /* IP6 EH handling is here, increment l4_offset if needs to, update the
       * proto */
      ...
      if (PREDICT_FALSE (need_skip_eh))
	{
	  while (need_skip_eh && offset_within_packet (b0, l4_offset))
	      ...
	      if (PREDICT_FALSE (ACL_EH_FRAGMENT == proto))
		{
		  ...
		  u16 frag_offset =
		    *(u16 *) get_ptr_to_offset (b0, 2 + l4_offset);
...
always_inline void
acl_fill_5tuple (acl_main_t *am, u32 sw_if_index0, vlib_buffer_t *b0,
		 ...
		 fa_5tuple_t *p5tuple_pkt)
{
  ...
  acl_fill_5tuple_l4_and_pkt_data (am, sw_if_index0, b0, is_ip6, is_input,
				   l3_offset, &p5tuple_pkt->l4,
				   &p5tuple_pkt->pkt);
}

always_inline void
acl_plugin_fill_5tuple_inline (void *p_acl_main, u32 lc_index,
			       vlib_buffer_t *b0, int is_ip6, int is_input,
			       int is_l2_path, fa_5tuple_opaque_t *p5tuple_pkt)
{
  acl_main_t *am = p_acl_main;
  acl_fill_5tuple (am, 0, b0, is_ip6, is_input, is_l2_path,
		   (fa_5tuple_t *) p5tuple_pkt);
}
		       ...
		       int prefixlen)
{
  ...
  if (memcmp (addr1, addr2, prefixlen / 8))
    {
      /* If the starting full bytes do not match, no point in bittwidling the
       * thumbs further */
      return 0;
    }
  if (prefixlen % 8)
...
always_inline int
single_acl_match_5tuple (acl_main_t *am, u32 acl_index,
			 ...
			 u32 *trace_bitmap)
{
  ...
  for (i = 0; i < vec_len (acl_rules); i++)
      ...
      if (is_ip6 != r->is_ipv6)
	{
      ...
      if (is_ip6)
	{
	  if (!fa_acl_match_ip6_addr (&pkt_5tuple->ip6_addr[1], &r->dst.ip6,
				      r->dst_prefixlen))
	    continue;
	  if (!fa_acl_match_ip6_addr (&pkt_5tuple->ip6_addr[0], &r->src.ip6,
				      r->src_prefixlen))
	    continue;
	}
      else
	{
	  if (!fa_acl_match_ip4_addr (&pkt_5tuple->ip4_addr[1], &r->dst.ip4,
				      r->dst_prefixlen))
	    continue;
	  if (!fa_acl_match_ip4_addr (&pkt_5tuple->ip4_addr[0], &r->src.ip4,
				      r->src_prefixlen))
	    continue;
	}
      ...
      if (r->proto)
	{
	  ...
	  if (PREDICT_FALSE (pkt_5tuple->pkt.is_nonfirst_fragment &&
	    ...
	    {
	      /* non-initial fragment with frag match configured - match this
	       * rule */
	  ...
	  /* A sanity check just to ensure we are about to match the ports
	  ...
	  if (!fa_acl_match_port (pkt_5tuple->l4.port[0],
	  ...
	  if (!fa_acl_match_port (pkt_5tuple->l4.port[1],
...
always_inline int
acl_plugin_single_acl_match_5tuple (void *p_acl_main, u32 acl_index,
				    fa_5tuple_t *pkt_5tuple, int is_ip6,
				    u8 *r_action, u32 *r_acl_match_p,
...
always_inline int
linear_multi_acl_match_5tuple (void *p_acl_main, u32 lc_index,
			       fa_5tuple_t *pkt_5tuple, int is_ip6,
			       u8 *r_action, u32 *acl_pos_p, u32 *acl_match_p,
			       u32 *rule_match_p, u32 *trace_bitmap)
{
  ...
  acl_lookup_context_t *acontext =
    pool_elt_at_index (am->acl_lookup_contexts, lc_index);
...
#endif
      if (single_acl_match_5tuple (am, acl_vector[i], pkt_5tuple, is_ip6,
				   &action, acl_match_p, rule_match_p,
				   trace_bitmap))
	{
	  *r_action = action;
	  *acl_pos_p = i;
...
match_portranges (acl_main_t *am, fa_5tuple_t *match, u32 index)
{
  ...
  applied_hash_ace_entry_t **applied_hash_aces =
    vec_elt_at_index (am->hash_entry_vec_by_lc_index, match->pkt.lc_index);
  applied_hash_ace_entry_t *pae =
    vec_elt_at_index ((*applied_hash_aces), index);

  acl_rule_t *r = &(am->acls[pae->acl_index].rules[pae->ace_index]);

#ifdef FA_NODE_VERBOSE_DEBUG
  clib_warning ("PORTMATCH: %d <= %d <= %d && %d <= %d <= %d ?",
		r->src_port_or_type_first, match->l4.port[0],
		r->src_port_or_type_last, r->dst_port_or_code_first,
		match->l4.port[1], r->dst_port_or_code_last);
#endif

  return (((r->src_port_or_type_first <= match->l4.port[0]) &&
	   r->src_port_or_type_last >= match->l4.port[0]) &&
	  ((r->dst_port_or_code_first <= match->l4.port[1]) &&
	   r->dst_port_or_code_last >= match->l4.port[1]));
}
...
always_inline u32
multi_acl_match_get_applied_ace_index (acl_main_t *am, int is_ip6,
				       fa_5tuple_t *match)
{
  ...
  for (order_index = 0; order_index < vec_len ((*hash_applied_mask_info_vec));
	...
	{
	  /* Index in this and following (by construction) partitions are
	   * greater than our candidate, Avoid trying to match! */
	  break;
	}
			     ...
			     u32 *rule_match_p, u32 *trace_bitmap)
{
  ...
  applied_hash_ace_entry_t **applied_hash_aces =
    vec_elt_at_index (am->hash_entry_vec_by_lc_index, lc_index);
  u32 match_index =
    multi_acl_match_get_applied_ace_index (am, is_ip6, pkt_5tuple);
  if (match_index < vec_len ((*applied_hash_aces)))
    {
      applied_hash_ace_entry_t *pae =
	vec_elt_at_index ((*applied_hash_aces), match_index);
      pae->hitcount++;
				...
				u32 *trace_bitmap)
{
  ...
  if (PREDICT_TRUE (am->use_hash_acl_matching))
      ...
      if (PREDICT_FALSE (pkt_5tuple_internal->pkt.is_nonfirst_fragment))
	{
	   ...
	   */
	  return linear_multi_acl_match_5tuple (
...
acl_plugin_match_5tuple_inline_and_count (void *p_acl_main, u32 lc_index,
					  ...
					  u32 *r_acl_pos_p, u32 *r_acl_match_p,
					  u32 *r_rule_match_p,
					  u32 *trace_bitmap, u32 packet_size)
{
  acl_main_t *am = p_acl_main;
  int ret = 0;
  fa_5tuple_t *pkt_5tuple_internal = (fa_5tuple_t *) pkt_5tuple;
