...
tm_multi_acl_match_get_applied_ace_index(acl_main_t *am, fa_5tuple_t *match)
{
  ...
  clib_bihash_kv_48_8_t kv_prefetch;
...
//Added by Valerio
  ...
  for(order_index = 0; order_index < vec_len((*hash_applied_mask_pool)); order_index++) {
    ...
    if (order_index < vec_len((*hash_applied_mask_pool)) - 1) {
      u64 *pmatch_prefetch = (u64 *)match;
      u64 *pmask_prefetch;
      u64 *pkey_prefetch;
      hash_applied_mask_info_t *minfo_prefetch = vec_elt_at_index((*hash_applied_mask_pool), order_index+1); 

      ace_mask_type_entry_t *mte = vec_elt_at_index(am->ace_mask_type_pool, minfo_prefetch->mask_type_index);
      pmask_prefetch = (u64 *)&mte->mask;
      pkey_prefetch = (u64 *)kv_prefetch.key;
      *pkey_prefetch++ = *pmatch_prefetch++ & *pmask_prefetch++;
      *pkey_prefetch++ = *pmatch_prefetch++ & *pmask_prefetch++;
      *pkey_prefetch++ = *pmatch_prefetch++ & *pmask_prefetch++;
      *pkey_prefetch++ = *pmatch_prefetch++ & *pmask_prefetch++;
      *pkey_prefetch++ = *pmatch_prefetch++ & *pmask_prefetch++;
      *pkey_prefetch++ = *pmatch_prefetch++ & *pmask_prefetch++;
      ...
      u64 prefetch_hash = clib_bihash_hash_48_8(&kv_prefetch);
      clib_bihash_prefetch_bucket_48_8(&am->acl_lookup_hash, prefetch_hash);
    }
