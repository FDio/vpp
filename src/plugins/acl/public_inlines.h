...
always_inline void
acl_fill_5tuple_orig (acl_main_t * am, u32 lc_index0, vlib_buffer_t * b0, int is_ip6,
		 int is_input, int is_l2_path, fa_5tuple_t * p5tuple_pkt0)
{
  ...
  fa_packet_info_t pkt0 = { .is_ip6 = is_ip6, .mask_type_index_lsb = ~0 };
      ...
      else if ((IP_PROTOCOL_TCP == proto0) || (IP_PROTOCOL_UDP == proto0))
	{
	  ...
	  pkt0.tcp_flags =
	    *(u8 *) get_ptr_to_offset (b0,
				       l4_offset0 + offsetof (tcp_header_t,
							     flags));
	  pkt0.tcp_flags_valid = (proto0 == IP_PROTOCOL_TCP);
	}
    }
...
acl_fill_5tuple (acl_main_t * am, u32 lc_index0, vlib_buffer_t * b0, int is_ip6,
		 int is_input, int is_l2_path, fa_5tuple_t * p5tuple_pkt0,
                 int fill_session, fa_5tuple_t * p5tuple_sess0)
{
  /* IP4 and IP6 protocol numbers of ICMP */
  static u8 icmp_protos_v4v6[] = { IP_PROTOCOL_ICMP, IP_PROTOCOL_ICMP6 };
  ...
  if (is_ip6)
	...
	*(u8 *) get_ptr_to_offset (b0,
  ...
  else
    {
      ...
      vnet_buffer (b0)->l3_hdr_offset = l3_offset0;

      ip4_header_t *ip4_ptr0 = get_ptr_to_offset(b0, l3_offset0);
      
      tcp_header_t *l4_ptr0 = ip4_next_header(ip4_ptr0);
      icmp46_header_t *l4_icmp_ptr0 = (void *)l4_ptr0;

      /* 0x45 = ihl; XX = tos; XX XX = total len; XX XX = id ; X000 = first frag */
      
        /* clib_memcpy(pak, get_ptr_to_offset (b0,
                                      offsetof (ip4_header_t,
                                                ttl) + l3_offset0), 16); */
      ip46_address_mask_ip4(&p5tuple_pkt0->addr[0]);
      ip46_address_mask_ip4(&p5tuple_pkt0->addr[1]);

      p5tuple_pkt0->addr[0].ip4 = ip4_ptr0->src_address;
      p5tuple_pkt0->addr[1].ip4 = ip4_ptr0->dst_address;

      l4_0.proto = proto0 = ip4_ptr0->protocol;

      vnet_buffer (b0)->l4_hdr_offset = l3_offset0 + ip4_header_bytes(ip4_ptr0);

      int is_tcp_or_udp_bool =  (proto0 == IP_PROTOCOL_TCP) | (proto0 == IP_PROTOCOL_UDP);
      u16 is_tcp_or_udp = 1 + ~((u16)is_tcp_or_udp_bool);

      int is_icmp_bool = (proto0 == icmp_protos_v4v6[is_ip6]);
      u16 is_icmp = 1 + ~((u16)is_icmp_bool);

      l4_0.port[0] = (clib_net_to_host_u16 (l4_ptr0->src_port) & is_tcp_or_udp) |
                     (l4_icmp_ptr0->type & is_icmp) ;
      l4_0.port[1] = (clib_net_to_host_u16 (l4_ptr0->dst_port) & is_tcp_or_udp) |
                     (l4_icmp_ptr0->code & is_icmp);

      pkt0.l4_valid = is_tcp_or_udp_bool | is_icmp_bool;
      pkt0.tcp_flags = l4_ptr0->flags;
      pkt0.tcp_flags_valid = (proto0 == IP_PROTOCOL_TCP);
     
      p5tuple_pkt0->l4.as_u64 = l4_0.as_u64; 

      if (fill_session) {
        fa_session_l4_key_t sess_l4_0 = { 0 }; 

        ip46_address_mask_ip4(&p5tuple_sess0->addr[src_index]);
        ip46_address_mask_ip4(&p5tuple_sess0->addr[dst_index]);
        p5tuple_sess0->addr[src_index].ip4 = ip4_ptr0->src_address;
        p5tuple_sess0->addr[dst_index].ip4 = ip4_ptr0->dst_address;
        sess_l4_0.proto = proto0;
        sess_l4_0.port[src_index] = l4_0.port[0];
        sess_l4_0.port[dst_index] = l4_0.port[1];
        p5tuple_sess0->l4.as_u64 = sess_l4_0.as_u64;
      }
      rv = is_tcp_or_udp_bool;
    }
  if (is_ip6 && (PREDICT_TRUE (offset_within_packet (b0, l4_offset0))))
    {
      vnet_buffer (b0)->l4_hdr_offset = l4_offset0;
      pkt0.l4_valid = 1;

      /* just assume TCP/UDP like header. ICMP will take care and incur overhead */
      clib_memcpy (&ports0,
		       get_ptr_to_offset (b0,
					  l4_offset0 + offsetof (tcp_header_t,
								src_port)),
      ...
      pkt0.tcp_flags = *(u8 *) get_ptr_to_offset (b0, l4_offset0 + offsetof (tcp_header_t, flags));
      pkt0.tcp_flags_valid = (proto0 == IP_PROTOCOL_TCP);
      if (PREDICT_FALSE(icmp_protos_v4v6[is_ip6] == proto0))
	{
	  /* type */
		 ...
		 int is_input, int is_l2_path, fa_5tuple_opaque_t * p5tuple_pkt)
{
  ...
  acl_fill_5tuple(am, lc_index0, b0, is_ip6, is_input, is_l2_path, (fa_5tuple_t *)p5tuple_pkt, 0, 0);
