			 ...
			 int l3_offset, fa_5tuple_t * p5tuple_pkt)
{
  if (is_ip6)
    ...
    {
      int ii;
      for (ii = 0; ii < ARRAY_LEN (p5tuple_pkt->l3_zero_pad); ii++)
	{
				 ...
				 fa_packet_info_t * p5tuple_pkt)
{
  ...
  tmp_l4_flags |= (VLIB_RX == dir) ? FA_SK_L4_FLAG_IS_INPUT : 0;
...
always_inline void
acl_fill_5tuple (acl_main_t * am,
		 u32 sw_if_index0, vlib_buffer_t * b0, int is_ip6,
		 vlib_dir_t dir, int is_l2_path, fa_5tuple_t * p5tuple_pkt)
{
  int l3_offset;
  ...
  acl_fill_5tuple_l4_and_pkt_data (am, sw_if_index0, b0, is_ip6, dir,
...
always_inline int
single_rule_match_5tuple (acl_rule_t * r, int is_ip6,
