...
#define TCP_FLAGS_RSTFINACKSYN                                                \
  (TCP_FLAG_RST + TCP_FLAG_FIN + TCP_FLAG_SYN + TCP_FLAG_ACK)
...
is_session_l4_key_u64_slowpath (u64 l4key)
{
...
typedef union
{
  struct
  {
    ...
    /* This field should align with u64 value in bihash_40_8 and bihash_16_8
     * keyvalue struct */
...
format_fa_session_l4_key (u8 *s, va_list *args)
{
  ...
  return (format (s,
		  "l4 lsb_of_sw_if_index %d proto %d l4_is_input %d "
		  "l4_slow_path %d l4_flags 0x%02x port %d -> %d",
		  l4->lsb_of_sw_if_index, l4->proto, is_input, is_slowpath,
...
/*
 * A few compile-time constraints on the size and the layout of the union, to
 * ensure it makes sense both for bihash and for us.
 */

#define CT_ASSERT_EQUAL(name, x, y)                                           \
  typedef int assert_##name##_compile_time_assertion_failed[((x) == (y)) - 1]
CT_ASSERT_EQUAL (fa_l3_key_size_is_40, offsetof (fa_5tuple_t, pkt),
		 offsetof (clib_bihash_kv_40_8_t, value));
CT_ASSERT_EQUAL (fa_ip6_kv_val_at_pkt, offsetof (fa_5tuple_t, pkt),
		 offsetof (fa_5tuple_t, kv_40_8.value));
CT_ASSERT_EQUAL (fa_ip4_kv_val_at_pkt, offsetof (fa_5tuple_t, pkt),
		 offsetof (fa_5tuple_t, kv_16_8.value));
CT_ASSERT_EQUAL (fa_l4_key_t_is_8, sizeof (fa_session_l4_key_t), sizeof (u64));
CT_ASSERT_EQUAL (fa_packet_info_t_is_8, sizeof (fa_packet_info_t),
		 sizeof (u64));
CT_ASSERT_EQUAL (fa_l3_kv_size_is_48, sizeof (fa_5tuple_t),
		 sizeof (clib_bihash_kv_40_8_t));
CT_ASSERT_EQUAL (fa_ip4_starts_at_kv16_key, offsetof (fa_5tuple_t, ip4_addr),
...
CT_ASSERT_EQUAL (fa_full_session_id_size_is_64, sizeof (fa_full_session_id_t),
		 sizeof (u64));
...
typedef struct
{
  ...
  /* if not zero, look at all the elements until their enqueue timestamp is
   * after below one */
  ...
  /* Counter of already deleted sessions being deleted - should not increment
   * unless a bug */
  ...
  /*
   * Set to copy of a "generation" counter in main thread so we can sync the
   * interrupts.
   */
...
/* use like: elog_acl_maybe_trace_X2(am, "foobar: %d some u64: %lu", "i4i8",
...
/* use like: elog_acl_maybe_trace_X3(am, "foobar: %d some u64 %lu baz: %d",
 * "i4i8i4", int32_value, u64_value, int_value); */
...
/* use like: elog_acl_maybe_trace_X4(am, "foobar: %d some int %d baz: %d bar:
 * %d", "i4i4i4i4", int32_value, int32_value2, int_value, int_value); */
