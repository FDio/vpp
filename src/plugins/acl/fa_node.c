		  ...
		  vnet_feature_next (sw_if_index0, &next0, b0);
                ...
                switch (action_steering) {
                  case ACL_ACTION_STEERING_SET_NEXT:
                    /* depending on policy action, set the next either to MSB (action 5) or LSB (action 4) from opaque index */
                    next0 = (vnet_buffer(b0)->l2_classify.opaque_index >> (ACL_ACTION_POLICY(action) == 1 ? 1 : 0)) & 0xffff;
