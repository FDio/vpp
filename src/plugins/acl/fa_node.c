...
acl_fa_conn_list_add_session (acl_main_t * am, fa_full_session_id_t sess_id, u64 now)
{
  ...
  u16 thread_index = sess->thread_index;
  acl_fa_per_worker_data_t *pw = &am->per_worker_data[thread_index];
  // FIXME: ASSERT (sess->thread_index == sess_id.thread_index)
  // FIXME: ASSERT (sess->thread_index == thread_index)
  ...
  if (~0 != pw->fa_conn_list_tail[list_id]) {
    ...
    // FIXME: ASSERT(prev_sess->thread_index == sess->thread_index)
  }
...
acl_fa_conn_list_delete_session (acl_main_t *am, fa_full_session_id_t sess_id)
{
  ...
  if (thread_index != sess_id.thread_index) {
    clib_warning("list ID %d != curr cpu index, not deleting");
    return 0;
  }
  fa_session_t *sess = get_session_ptr(am, sess_id.thread_index, sess_id.session_index);
  // FIXME: ASSERT(sess->thread_index == thread_index)
...
acl_fa_restart_timer_for_session (acl_main_t * am, u64 now, fa_full_session_id_t sess_id)
{
  ...
  if (acl_fa_conn_list_delete_session(am, sess_id)) {
    ...
    /* FIXME-MULTICORE: not our thread - need to post an event to its owner */
    return 0;
  }
}
...
{
  fa_session_t *sess = get_session_ptr(am, sess_id.thread_index, sess_id.session_index);
  // FIXME ASSERT(sess.thread_index == os_get_thread_index ())
...
acl_fa_check_idle_sessions(acl_main_t *am, u16 thread_index, u64 now)
{
  ...
  /* if we were advancing and reached the end (no more sessions to recycle), reset the fast-forward timestamp */
		    ...
		    fa_5tuple_t * p5tuple)
{
  ...
  uword thread_index = os_get_thread_index();
  acl_fa_per_worker_data_t *pw = &am->per_worker_data[thread_index];
  //FIXME: ASSERT(thread_index < 65535);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (acl_fa_ifc_has_sessions (am, sw_if_index0))
		  ...
		  (am, sw_if_index0, &kv_sess, &value_sess))
		{
          ...
          // FIXME: ASSERT(f_sess_id.thread_index < vec_len(vlib_mains))
              ...
              vlib_node_runtime_t * rt, vlib_frame_t * f)
{
   ...
   if (pw->clear_in_process) {
	 ...
	 clib_bitmap_zero(pw->pending_clear_sw_if_index_bitmap);
         ...
         clib_warning("WORKER-CLEAR: (before and) swiping sw-if-index bitmap: %U, my serviced bitmap %U", format_bitmap_hex, pw->pending_clear_sw_if_index_bitmap, format_bitmap_hex, pw->serviced_sw_if_index_bitmap);
       ...
       if (clib_bitmap_is_zero(pw->pending_clear_sw_if_index_bitmap)) {
         ...
         clib_warning("WORKER-CLEAR: swiping sw-if-index bitmap: %U, my serviced bitmap %U", format_bitmap_hex, pw->pending_clear_sw_if_index_bitmap, format_bitmap_hex, pw->serviced_sw_if_index_bitmap);
   ...
   if (pw->clear_in_process) {
     if (0 == num_expired) {
       ...
       /* should continue clearing.. So we might as well raise ourselves an interrupt again. */
				...
				vlib_frame_t * f)
{
  ...
  while (1)
    {
      // DELETE u32 count_deleted_sessions = 0;
      // DELETE u32 count_already_deleted = 0;
      ...
      has_pending_conns = 1; // FIXME: Temp to keep it rolling
	...
	case ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX:
	  {
		 ...
		 *sw_if_index0);
	      ...
	      // FIXME DELETE count_deleted_sessions += count;
	    ...
	    clib_warning("ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX bitmap: %U", format_bitmap_hex, clear_sw_if_index_bitmap);
            /// P0: FIXME: put a wait to avoid race conditions!
            acl_fa_per_worker_data_t *pw0;
	    vec_foreach(pw0, am->per_worker_data) {
              // FIXME: this check is wrong, but this needs to be done properly with waiting anyway
	      while (pw0->clear_in_process) {
            ...
            /* send some interrupts */
	    {
	      int i;
	      for (i = vec_len (vlib_mains) > 1 ? 1 : 0; i < vec_len (vlib_mains); i++)
		if (vlib_mains[i]) {
	...
	default:
      ...
      // FIXME: send an interrupt to a [0] if and only if the vector length is zero?
      // Else the worker threads start at [1] onwards ?
      {
        int i;
        for (i = vec_len (vlib_mains) > 1 ? 1 : 0; i < vec_len (vlib_mains); i++)
          if (vlib_mains[i]) {
	...
	_vec_len (event_data) = 0;
      ...
      // fixme this logic
      int count_deleted_sessions = 0;
      int count_already_deleted = 0;
