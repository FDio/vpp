...
{
  const int iaca_analyze = is_input && !is_l2_path && !is_ip6;
  ...
  while (n_left_from > 0)
    {
	  ...
	  int valid_new_sess = acl_make_5tuple_session_key (am, is_input, is_ip6, pkt_5tuple0, sess_5tuple0);
          bm0->sess_5tuple.l4.lsb_of_sw_if_index = bm0->save_sw_if_index & 0xffff; // sw_if_index0 & 0xffff;
          if (PREDICT_FALSE(!valid_new_sess)) {
            u32 sw_if_index0 = bm0->save_sw_if_index;
/*
	    if (is_input)
	      sw_if_index0 = vnet_buffer (b0)->sw_if_index[VLIB_RX];
	    else
	      sw_if_index0 = vnet_buffer (b0)->sw_if_index[VLIB_TX];
*/
	    valid_new_sess = acl_make_5tuple_icmp_session_key (am, is_input, is_ip6, sw_if_index0,  pkt_5tuple0, sess_5tuple0);
          }
          bm0->valid_new_sess = valid_new_sess;
          ...
          if (n_left_from > 2)
            {
              ...
              CLIB_PREFETCH (bm1, 2*CLIB_CACHE_LINE_BYTES, STORE);
            }
          if (n_left_from > 1)
            {
              block_meta_t *bm1 = &block_meta[1];
