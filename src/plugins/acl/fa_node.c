...
#define STEP1_ONE_SHOT
#ifdef STEP1_ONE_SHOT
  /*
  ...
  while (n_left_from > 0)
    {
	  ...
	  bi0 = from[0];
          ...
          // bm0->action = 1; /* reset action to default deny */
	  ...
	  { 
            ...
            CLIB_PREFETCH (p1->data, CLIB_CACHE_LINE_BYTES, LOAD);
	  ...
	  acl_fill_5tuple (am, lc_index0, b0, is_ip6, is_input, is_l2_path, (void *)&bm0->pkt_5tuple);
  ...
  /*
   * Step 1.1: extract the L3 portions of the tuples
   */
  ...
  while (n_left_from >= 4)
    {
	  ...
	  { 
            ...
            vlib_buffer_t * p6;
            vlib_buffer_t * p7;
            p4 = vlib_get_buffer (vm, from[8]);
            p5 = vlib_get_buffer (vm, from[9]);
            p6 = vlib_get_buffer (vm, from[10]);
            p7 = vlib_get_buffer (vm, from[11]);
            ...
            CLIB_PREFETCH (&block_meta[4], 2*CLIB_CACHE_LINE_BYTES, STORE);
            CLIB_PREFETCH (&block_meta[5], 2*CLIB_CACHE_LINE_BYTES, STORE);
            CLIB_PREFETCH (&block_meta[6], 2*CLIB_CACHE_LINE_BYTES, STORE);
            CLIB_PREFETCH (&block_meta[7], 2*CLIB_CACHE_LINE_BYTES, STORE);
          }

	  u32 bi0; vlib_buffer_t *b0;
	  u32 bi1; vlib_buffer_t *b1;
	  u32 bi2; vlib_buffer_t *b2;
	  u32 bi3; vlib_buffer_t *b3;
          block_meta_t *bm0 = &block_meta[0];
          ...
          fa_5tuple_t *pkt_5tuple2 = &bm2->pkt_5tuple;
          fa_5tuple_t *pkt_5tuple3 = &bm3->pkt_5tuple;
	  bi0 = from[0];
	  bi1 = from[1];
	  bi2 = from[2];
	  bi3 = from[3];
          ...
          bm0->action = 0; /* reset action to default deny */
          bm1->action = 0; /* reset action to default deny */
          bm2->action = 0; /* reset action to default deny */
          bm3->action = 0; /* reset action to default deny */
          // bm0->action = 1; /* reset action to default deny */
          bm0->error = ACL_FA_ERROR_ACL_CHECK;
          bm1->error = ACL_FA_ERROR_ACL_CHECK;
          bm2->error = ACL_FA_ERROR_ACL_CHECK;
          bm3->error = ACL_FA_ERROR_ACL_CHECK;
          // bm0->session_found = 0;
          bm0->all_flags = 0;
          bm1->all_flags = 0;
          bm2->all_flags = 0;
          bm3->all_flags = 0;
          bm0->match_rule_as_u64 = ~0ULL;
          bm1->match_rule_as_u64 = ~0ULL;
          bm2->match_rule_as_u64 = ~0ULL;
          bm3->match_rule_as_u64 = ~0ULL;
	  ...
	  if (is_input) {
	    ...
	    sw_if_index3 = vnet_buffer (b3)->sw_if_index[VLIB_TX];
          ...
          bm0->save_sw_if_index = sw_if_index0;
          bm1->save_sw_if_index = sw_if_index1;
          bm2->save_sw_if_index = sw_if_index2;
          bm3->save_sw_if_index = sw_if_index3;
	  /*
	   * Extract the L3 matching info into a 5-tuple structure,
	   */
          vnet_buffer (b0)->l3_hdr_offset = acl_get_l3_offset(b0, is_ip6, is_input, is_l2_path);
          vnet_buffer (b1)->l3_hdr_offset = acl_get_l3_offset(b1, is_ip6, is_input, is_l2_path);
          vnet_buffer (b2)->l3_hdr_offset = acl_get_l3_offset(b2, is_ip6, is_input, is_l2_path);
          vnet_buffer (b3)->l3_hdr_offset = acl_get_l3_offset(b3, is_ip6, is_input, is_l2_path);
          set_block_save_proto(b0, is_l2_path, acl_fill_5tuple_l3_get_proto(am, b0, is_ip6, vnet_buffer (b0)->l3_hdr_offset, pkt_5tuple0));
          set_block_save_proto(b1, is_l2_path, acl_fill_5tuple_l3_get_proto(am, b1, is_ip6, vnet_buffer (b1)->l3_hdr_offset, pkt_5tuple1));
          set_block_save_proto(b2, is_l2_path, acl_fill_5tuple_l3_get_proto(am, b2, is_ip6, vnet_buffer (b2)->l3_hdr_offset, pkt_5tuple2));
          set_block_save_proto(b3, is_l2_path, acl_fill_5tuple_l3_get_proto(am, b3, is_ip6, vnet_buffer (b3)->l3_hdr_offset, pkt_5tuple3));
  ...
  /*
   * Step 1.2: extract the L4 portions of the tuples
   */
  ...
  while (n_left_from > 0)
    {
	  ...
	  if (is_input)
	    lc_index0 = am->input_lc_index_by_sw_if_index[bm0->save_sw_if_index];
	  else
	    lc_index0 = am->output_lc_index_by_sw_if_index[bm0->save_sw_if_index];

	  /*
	   * Extract the L4 matching info into a 5-tuple structure,
	   */

          acl_fill_5tuple_l4_and_finish(am, lc_index0, b0, is_ip6, vnet_buffer (b0)->l4_hdr_offset, get_block_save_proto(b0, is_l2_path), pkt_5tuple0);
...
#define TWO_STRIDE_ACL_CHECK
...

  {
    ...
    if (PREDICT_FALSE(!bm0->acl_check_needed)) while (!bm0->acl_check_needed) {
      bm0 = bm_next;
      if (n_left_from == 0)
        break;
      bm_next++;
    ...
    if (PREDICT_FALSE(!bm1->acl_check_needed)) while (!bm1->acl_check_needed) {
      bm1 = bm_next;
      if (n_left_from == 0)
        break;
      bm_next++;
    ...
    if (PREDICT_FALSE(!bm2->acl_check_needed)) while (!bm2->acl_check_needed) {
      bm2 = bm_next;
      if (n_left_from == 0)
        break;
      bm_next++;
    ...
    if (PREDICT_FALSE(!bm3->acl_check_needed)) while (!bm3->acl_check_needed) {
      bm3 = bm_next;
      if (n_left_from == 0)
        break;
      bm_next++;
	  ...
	  //if (bm0->action > 0)
	    {
	      if (is_l2_path)
                {
		  u32 new_next0 = (bm0->action > 0) ? vnet_l2_feature_next (b0, l2_feat_next_node_index, 0) : 0;
                  next0 = (bm0->action == 0) ? 0 : new_next0;
                }
	      else
	        if (bm0->action > 0)
