...
#define STEP1_FULL
   ...
   */
  block_meta = pw->block_meta[is_ip6];
		  ...
		  if (acl_fa_can_add_session (am, is_input, sw_if_index0))
		    {
                      if (PREDICT_TRUE (bm0->valid_new_sess)) {
                        fa_session_t *sess = acl_fa_add_session (am, is_input,
                                                                 sw_if_index0,
                                                                 now, &bm0->pkt_5tuple, current_policy_epoch0); // FIXME: wrong session key here.
                        acl_fa_track_session (am, is_input, sw_if_index0, now,
                                              sess, &bm0->pkt_5tuple);
                        pkts_new_session += 1;
                      } else {
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)
			     && (b0->flags & VLIB_BUFFER_IS_TRACED)))
	    {
	      ...
	      t->packet_info[5] = bm0->pkt_5tuple.kv.key[5];
	      t->action = bm0->action;
