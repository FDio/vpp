...
//===Added by Valerio
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 8 && n_left_to_next >= 4)
	{
          ...
          fa_5tuple_t fa_5tuple1;
          fa_5tuple_t fa_5tuple2;
          fa_5tuple_t fa_5tuple3;
          fa_5tuple_t kv_sess0;
          fa_5tuple_t kv_sess1;
          fa_5tuple_t kv_sess2;
          fa_5tuple_t kv_sess3;
	  ...
	  if (is_input)
          ...
          u16 current_policy_epoch1 = sw_if_index1 < vec_len(*p_epoch_vec) ? vec_elt(*p_epoch_vec, sw_if_index1) : (is_input * FA_POLICY_EPOCH_IS_INPUT);
          u16 current_policy_epoch2 = sw_if_index2 < vec_len(*p_epoch_vec) ? vec_elt(*p_epoch_vec, sw_if_index2) : (is_input * FA_POLICY_EPOCH_IS_INPUT);
          u16 current_policy_epoch3 = sw_if_index3 < vec_len(*p_epoch_vec) ? vec_elt(*p_epoch_vec, sw_if_index3) : (is_input * FA_POLICY_EPOCH_IS_INPUT);
	  /*
	  ...
	  if (acl_fa_ifc_has_sessions (am, sw_if_index0))
	    {
              int found_session0 = acl_fa_find_session (am, sw_if_index0, &kv_sess0, &value_sess0);
              int found_session1 = acl_fa_find_session (am, sw_if_index1, &kv_sess1, &value_sess1);
              int found_session2 = acl_fa_find_session (am, sw_if_index2, &kv_sess2, &value_sess2);
              int found_session3 = acl_fa_find_session (am, sw_if_index3, &kv_sess3, &value_sess3);

// FIXME HACK
	      if (found_session0 || found_session1 || found_session2 || found_session3)
		{
		  ...
		  if (acl_fa_can_add_session (am, is_input, sw_if_index0))
		    {
                      if (PREDICT_TRUE (valid_new_sess0)) {
                        fa_session_t *sess = acl_fa_add_session (am, is_input,
                                                                 sw_if_index0,
                                                                 now, &kv_sess0, current_policy_epoch0);
                        acl_fa_track_session (am, is_input, sw_if_index0, now,
                                              sess, &fa_5tuple0);
                        pkts_new_session += 1;
                      } else {
		  ...
		  if (acl_fa_can_add_session (am, is_input, sw_if_index1))
		    {
                      if (PREDICT_TRUE (valid_new_sess1)) {
                        fa_session_t *sess = acl_fa_add_session (am, is_input,
                                                                 sw_if_index1,
                                                                 now, &kv_sess1, current_policy_epoch1);
		  ...
		  if (acl_fa_can_add_session (am, is_input, sw_if_index2))
		    {
                      if (PREDICT_TRUE (valid_new_sess2)) {
                        fa_session_t *sess = acl_fa_add_session (am, is_input,
                                                                 sw_if_index2,
                                                                 now, &kv_sess2, current_policy_epoch2);
		  ...
		  if (acl_fa_can_add_session (am, is_input, sw_if_index3))
		    {
                      if (PREDICT_TRUE (valid_new_sess3)) {
                        fa_session_t *sess = acl_fa_add_session (am, is_input,
                                                                 sw_if_index3,
                                                                 now, &kv_sess3, current_policy_epoch3);
...
/** FIXME THE TRACE **/
	  ...
	  vlib_validate_buffer_enqueue_x4 (vm, node, next_index,
					   to_next, n_left_to_next, bi0, bi1, bi2, bi3, next0, next1, next2, next3);
	}
