  ...
  while (n_left_from > 1)
    {
      ...
      fpp1 = vnet_buffer_acl_info (b1);

      sw_if_index0 = vnet_buffer (b0)->sw_if_index[vlib_tx_rx];
      sw_if_index1 = vnet_buffer (b1)->sw_if_index[vlib_tx_rx];

      if (PREDICT_TRUE (n_left_from > 3))
        {
          acl_fa_prefetch_buffer_and_data(vm, from[2]);
...

      {
	      ...
	      acl_fill_5tuple (am, b0, is_ip6, is_input, is_l2_path, &fa_5tuple0);
	      acl_fill_5tuple (am, b1, is_ip6, is_input, is_l2_path, &fa_5tuple1);

	      fa_5tuple0.l4.lsb_of_sw_if_index = sw_if_index0 & 0xffff;
	      fa_5tuple1.l4.lsb_of_sw_if_index = sw_if_index1 & 0xffff;

	      fa_5tuple0.pkt.sw_if_index = sw_if_index0;
	      fa_5tuple1.pkt.sw_if_index = sw_if_index1;

	      fa_5tuple0.pkt.is_ip6 = is_ip6;
	      fa_5tuple1.pkt.is_ip6 = is_ip6;

	      fa_5tuple0.pkt.is_input = is_input;
	      fa_5tuple1.pkt.is_input = is_input;

	      fa_5tuple0.pkt.mask_type_index_lsb = ~0;
	      fa_5tuple1.pkt.mask_type_index_lsb = ~0;

	      acl_make_5tuple_session_key (is_input, &fa_5tuple0, &fpp0->five_tuple);
	      acl_make_5tuple_session_key (is_input, &fa_5tuple1, &fpp1->five_tuple);
      }

      vnet_buffer (b0)->l2_classify.hash = BV (clib_bihash_hash) (&fpp0->five_tuple.kv);
      vnet_buffer (b1)->l2_classify.hash = BV (clib_bihash_hash) (&fpp1->five_tuple.kv);
  ...
  while (n_left_from > 1)
    {
      ...
      if (PREDICT_TRUE (n_left_from > 5))
        {
          acl_fa_prefetch_buffer_and_data(vm, from[4]);
          acl_fa_prefetch_buffer_and_data(vm, from[5]);
        }

      if (PREDICT_TRUE (n_left_from > 3))
        {
          vlib_buffer_t *bX0 = vlib_get_buffer (vm, from[2]);
          vlib_buffer_t *bX1 = vlib_get_buffer (vm, from[3]);
          BV (clib_bihash_prefetch_bucket) (&am->fa_sessions_hash, vnet_buffer (bX0)->l2_classify.hash);
          BV (clib_bihash_prefetch_bucket) (&am->fa_sessions_hash, vnet_buffer (bX1)->l2_classify.hash);
        }

      if (PREDICT_FALSE(BV (clib_bihash_search) (&am->fa_sessions_hash, &fpp0->five_tuple.kv, &value_sess0)))
        vnet_buffer (b0)->l2_classify.hash = ACL_FA_NO_SESSION;
      else
        vnet_buffer (b0)->l2_classify.hash = value_sess0.value;

      if (PREDICT_FALSE(BV (clib_bihash_search) (&am->fa_sessions_hash, &fpp1->five_tuple.kv, &value_sess1)))
        vnet_buffer (b1)->l2_classify.hash = ACL_FA_NO_SESSION;
      else
        vnet_buffer (b1)->l2_classify.hash = value_sess1.value;
