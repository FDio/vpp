  ...
  while (n_left_from > 0)
    {
	  ...
	  acl_plugin_fill_5tuple_inline (lc_index0, b0, is_ip6, is_input, is_l2_path, (void *)&bm0->pkt_5tuple);
...
/*
	  ...
	  pkt0.lc_index = lc_index0;
          ...
          bm0->pkt_5tuple.kv.value = 0ULL;
          bm0->pkt_5tuple.pkt.as_u64 = pkt0.as_u64;
*/
...
#else
  /*
   * Step 1.0: extract the L3 offsets
   */
...
*/
  ...
  block_meta[stride_count+0].buffer_zone_meta_record = 1;
  block_meta[stride_count+1].buffer_zone_meta_record = 1;
  block_meta[stride_count+2].buffer_zone_meta_record = 1;
  block_meta[stride_count+3].buffer_zone_meta_record = 1;
  block_meta[stride_count+4].buffer_zone_meta_record = 1;
  block_meta[stride_count+5].buffer_zone_meta_record = 1;
  block_meta[stride_count+6].buffer_zone_meta_record = 1;
  block_meta[stride_count+7].buffer_zone_meta_record = 1;

  block_meta[stride_count+0].acl_check_needed = 0;
  block_meta[stride_count+1].acl_check_needed = 0;
  block_meta[stride_count+2].acl_check_needed = 0;
  block_meta[stride_count+3].acl_check_needed = 0;
  block_meta[stride_count+4].acl_check_needed = 0;
  block_meta[stride_count+5].acl_check_needed = 0;
  block_meta[stride_count+6].acl_check_needed = 0;
  block_meta[stride_count+7].acl_check_needed = 0;

  block_meta[stride_count+0].action = 0;
  block_meta[stride_count+1].action = 0;
  block_meta[stride_count+2].action = 0;
  block_meta[stride_count+3].action = 0;
  block_meta[stride_count+4].action = 0;
  block_meta[stride_count+5].action = 0;
  block_meta[stride_count+6].action = 0;
  block_meta[stride_count+7].action = 0;
	   ...
	   */
          ...
          if (valid_new_sess) {
            ...
            if (acl_fa_ifc_has_sessions (am, sw_if_index0)) {
              if (acl_fa_find_session_with_hash(am, sw_if_index0, sess_5tuple0, &value_sess, bm0->hash)) {
		  fa_full_session_id_t f_sess_id;
...
#define TWO_STRIDE_ACL_CHECK
  ...
  // if (iaca_analyze) { IACA_START; }
  {
    ...
    acl_tmhash_match_begin(am, &bm0->pkt_5tuple, &wip0, bm0->buffer_zone_meta_record);
    acl_tmhash_match_begin(am, &bm1->pkt_5tuple, &wip1, bm1->buffer_zone_meta_record);
    acl_tmhash_match_begin(am, &bm2->pkt_5tuple, &wip2, bm2->buffer_zone_meta_record);
    acl_tmhash_match_begin(am, &bm3->pkt_5tuple, &wip3, bm3->buffer_zone_meta_record);
    ...
    while (n_active > 0) {
      ...
      if (PREDICT_FALSE(!res0)) while (!res0) {
        u32 match_acl_pos0;
        u8 action0;
        acl_tmhash_check_finalize(am, &bm0->pkt_5tuple, &wip0, &action0,
	...
	bm0->action = action0;
      ...
      if (PREDICT_FALSE(!res1)) while (!res1) {
        u32 match_acl_pos1;
        u8 action1;
        acl_tmhash_check_finalize(am, &bm1->pkt_5tuple, &wip1, &action1,
	...
	bm1->action = action1;
      ...
      if (PREDICT_FALSE(!res2)) while (!res2) {
        u32 match_acl_pos2;
        u8 action2;
        acl_tmhash_check_finalize(am, &bm2->pkt_5tuple, &wip2, &action2,
	...
	bm2->action = action2;
      ...
      if (PREDICT_FALSE(!res3)) while (!res3) {
        u32 match_acl_pos3;
        u8 action3;
        acl_tmhash_check_finalize(am, &bm3->pkt_5tuple, &wip3, &action3,
