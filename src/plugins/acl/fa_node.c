...
static int
acl_fa_conn_time_to_check (acl_main_t *am, u64 now, u16 thread_index, u32 session_index)
{
   ...
   
  {
    ...
    for(tt = 0; tt < ACL_N_TIMEOUTS; tt++) {
      while((vec_len(pw->expired) < am->fa_max_deleted_sessions_per_interval)
	    ...
	    && (acl_fa_conn_time_to_check(am, now, thread_index,
...
{
  clib_warning("FIXME: unimplemented!");
  return 0;
}
/* *INDENT-ON* */
...
#endif
   acl_fa_check_idle_sessions(am, thread_index, now);
   return 0;
}
