...
*/
...
#ifdef DONT_DO_EXTRA_HASHES

  /*
   * Step 2: calculate the hashes
   */
  ...
  while (n_left_from > 0)
    {
          ...
          bm0->pkt_hash = clib_bihash_hash_40_8(&pkt_5tuple0->kv);
          block_meta++;
          from++;
          n_left_from--;
    }
	  ...
	  int valid_new_sess = acl_make_5tuple_session_key (am, is_input, is_ip6, sw_if_index0,  pkt_5tuple0, sess_5tuple0);
          if (valid_new_sess) {
            ...
            if (acl_fa_ifc_has_sessions (am, sw_if_index0)) {
              if (acl_fa_find_session(am, sw_if_index0, sess_5tuple0, &value_sess)) {
		  fa_full_session_id_t f_sess_id;
		  ...
		  /* FIXME -- match_rule_index = f_sess_id.session_index; */
  ...
  /*
   * Step 4: check ACLs where needed 
   */
	  ...
	  if (bm0->acl_check_needed)
	    {
	      ...
	      acl_plugin_match_5tuple_inline (lc_index0, (fa_5tuple_opaque_t *)&bm0->pkt_5tuple,
				       ...
				       &match_rule_index0, &trace_bitmap);
              bm0->match_acl_in_index = match_acl_in_index0 & 0xffff;
              bm0->match_rule_index = match_rule_index0 & 0xffff;
	      bm0->error = action;
              bm0->action = action;
	      if (1 == action)
              {
  ...
  /*
   * Step 6: propagate actions to packets
   */
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)
			     && (b0->flags & VLIB_BUFFER_IS_TRACED)))
	    {
	      ...
	      t->action = bm0->action;
	      t->trace_bitmap = 0xdeadbeef; // trace_bitmap;
	    }
