...
acl_fa_conn_list_delete_session (acl_main_t *am, fa_full_session_id_t sess_id)
{
  ...
  if (thread_index != sess_id.thread_index) {
    // FIXME: Should it rather be an assert here ? (See restart_timer_for_session)
...
{
  int i;
  int n_threads = vec_len(vlib_mains); // Can't use vec_len(am->per_worker_data) since the threads might not have come up yet;
  for (i = n_threads > 1 ? 1 : 0; i < n_threads; i++) {
    if (vlib_mains[i] && !am->per_worker_data[i].interrupt_is_pending) {
      vlib_node_set_interrupt_pending (vlib_mains[i],
		  acl_fa_worker_session_cleaner_process_node.index);
      am->per_worker_data[i].interrupt_is_pending = 1;
    }
  }
}
  ...
  while (1)
    {
      ...
      // has_pending_conns = 1; // FIXME: Temp to keep it rolling all the time
