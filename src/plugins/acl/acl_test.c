...
static void
vl_api_acl_plugin_get_version_reply_t_handler (
  vl_api_acl_plugin_get_version_reply_t *mp)
{
  vat_main_t *vam = acl_test_main.vat_main;
  clib_warning ("ACL plugin version: %d.%d", ntohl (mp->major),
		ntohl (mp->minor));
  vam->result_ready = 1;
}

static void
vl_api_acl_interface_list_details_t_handler (
...
static void
vl_api_macip_acl_interface_list_details_t_handler (
...
static void
vl_api_acl_interface_etype_whitelist_details_t_handler (
...
static void
vl_api_acl_plugin_get_conn_table_max_entries_reply_t_handler (
...
static void
vl_api_acl_details_t_handler (vl_api_acl_details_t *mp)
{
  int i;
  vat_main_t *vam = acl_test_main.vat_main;
...
vl_api_macip_acl_rule_t_pretty_format (u8 *out, vl_api_macip_acl_rule_t *a)
{
  ...
  out = format (out, "%s action %d ip %s/%d mac %U mask %U",
		a->src_prefix.address.af ? "ipv6" : "ipv4", a->is_permit, src,
		a->src_prefix.len, my_format_mac_address, a->src_mac,
		my_format_mac_address, a->src_mac_mask);
  return (out);
}

static void
vl_api_macip_acl_details_t_handler (vl_api_macip_acl_details_t *mp)
{
  int i;
  vat_main_t *vam = acl_test_main.vat_main;
...
static void
vl_api_macip_acl_interface_get_reply_t_handler (
  vl_api_macip_acl_interface_get_reply_t *mp)
{
  int i;
  vat_main_t *vam = acl_test_main.vat_main;
  u8 *out =
    format (0, "sw_if_index with MACIP ACL count: %d\n", ntohl (mp->count));
  for (i = 0; i < ntohl (mp->count); i++)
    {
      out = format (
	out, "  macip_acl_interface_add_del sw_if_index %d add acl %d\n", i,
	ntohl (mp->acls[i]));
    }
  out = format (out, "\n");
  clib_warning ("%s", out);
...
static int
api_acl_plugin_get_version (vat_main_t *vam)
{
...
static int
api_macip_acl_interface_get (vat_main_t *vam)
{
  acl_test_main_t *sm = &acl_test_main;
  vl_api_acl_plugin_get_version_t *mp;
  u32 msg_size = sizeof (*mp);
...
/* NOT YET IMPLEMENTED */
static int
api_acl_plugin_control_ping (vat_main_t *vam)
{
  return 0;
}
static int
api_macip_acl_interface_list_dump (vat_main_t *vam)
{
  return 0;
}

static int
api_acl_add_replace (vat_main_t *vam)
{
  acl_test_main_t *sm = &acl_test_main;
  ...
  while (unformat_check_input (i) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (i, "action %d", &action))
	{
      ...
      else if (unformat (i, "src %U/%d", unformat_ip4_address, &src_v4address,
			 &src_prefix_length))
	{
	  vec_validate_acl_rules (rules, rule_idx);
	  memcpy (rules[rule_idx].src_prefix.address.un.ip4, &src_v4address,
		  4);
	  rules[rule_idx].src_prefix.address.af = ADDRESS_IP4;
	  rules[rule_idx].src_prefix.len = src_prefix_length;
	}
      else if (unformat (i, "src %U/%d", unformat_ip6_address, &src_v6address,
			 &src_prefix_length))
	{
	  vec_validate_acl_rules (rules, rule_idx);
	  memcpy (rules[rule_idx].src_prefix.address.un.ip6, &src_v6address,
		  16);
	  rules[rule_idx].src_prefix.address.af = ADDRESS_IP6;
	  rules[rule_idx].src_prefix.len = src_prefix_length;
	}
      else if (unformat (i, "dst %U/%d", unformat_ip4_address, &dst_v4address,
			 &dst_prefix_length))
	{
	  vec_validate_acl_rules (rules, rule_idx);
	  memcpy (rules[rule_idx].dst_prefix.address.un.ip4, &dst_v4address,
		  4);
	  rules[rule_idx].dst_prefix.address.af = ADDRESS_IP4;
	  rules[rule_idx].dst_prefix.len = dst_prefix_length;
	}
      else if (unformat (i, "dst %U/%d", unformat_ip6_address, &dst_v6address,
			 &dst_prefix_length))
	{
	  vec_validate_acl_rules (rules, rule_idx);
	  memcpy (rules[rule_idx].dst_prefix.address.un.ip6, &dst_v6address,
		  16);
...
static int
api_acl_plugin_get_conn_table_max_entries (vat_main_t *vam)
{
  acl_test_main_t *sm = &acl_test_main;
  ...
  mp->_vl_msg_id =
    ntohs (VL_API_ACL_PLUGIN_GET_CONN_TABLE_MAX_ENTRIES + sm->msg_id_base);
  mp->client_index = vam->my_client_index;
...
static int
api_acl_stats_intf_counters_enable (vat_main_t *vam)
{
  acl_test_main_t *sm = &acl_test_main;
  unformat_input_t *i = vam->input;
  ...
  memset (mp, 0, msg_size);
  mp->_vl_msg_id =
    ntohs (VL_API_ACL_STATS_INTF_COUNTERS_ENABLE + sm->msg_id_base);
  mp->client_index = vam->my_client_index;
  mp->enable = 1;
...
@0.0.0.0/1      131.179.121.0/24        0 : 65535       0 : 65535 0x00/0x00
0x0000/0x0000
@128.0.0.0/1    85.54.226.0/23  0 : 65535       0 : 65535       0x00/0x00
0x0000/0x0000
@128.0.0.0/1    85.54.48.0/23   0 : 65535       0 : 65535       0x00/0x00
0x0000/0x0000
@128.0.0.0/1    31.237.44.0/23  0 : 65535       0 : 65535       0x00/0x00
0x0000/0x0000
@0.0.0.0/1      255.84.184.0/23 0 : 65535       0 : 65535       0x00/0x00
0x0000/0x0000
@132.92.0.0/16  0.0.0.0/0       0 : 65535       0 : 65535       0x01/0xFF
0x0000/0x0000
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (!unformat (input,
		     ...
		     &sport_low, &sport_high, &dport_low, &dport_high, &proto,
		     &protomask, &unused1, &unused2))
	{
...
static int
api_macip_acl_del (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  vl_api_acl_del_t *mp;
  u32 acl_index = ~0;
...
static int
api_acl_interface_add_del (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  ...
  //    acl_interface_add_del <intfc> | sw_if_index <if-idx> acl_index
  //    <acl-idx> [out] [del]
...
static int
api_macip_acl_interface_add_del (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
...
static int
api_acl_interface_set_acl_list (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  ...
  //  acl_interface_set_acl_list <intfc> | sw_if_index <if-idx> input [acl-idx
  //  list] output [acl-idx list]
  ...
  /* Construct the API message */
  M2 (ACL_INTERFACE_SET_ACL_LIST, mp,
...
static int
api_acl_interface_set_etype_whitelist (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  ...
  //  acl_interface_set_etype_whitelist <intfc> | sw_if_index <if-idx> input
  //  [ethertype list] output [ethertype list]
  ...
  /* Construct the API message */
  M2 (ACL_INTERFACE_SET_ETYPE_WHITELIST, mp,
      sizeof (u32) * (vec_len (etypes_in) + vec_len (etypes_out)));
  mp->sw_if_index = ntohl (sw_if_index);
  ...
  if (vec_len (etypes_in) > 0)
    clib_memcpy (mp->whitelist, etypes_in,
		 vec_len (etypes_in) * sizeof (etypes_in[0]));
...
static int
api_acl_interface_list_dump (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  u32 sw_if_index = ~0;
...
static int
api_macip_acl_dump (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  u32 acl_index = ~0;
  vl_api_acl_dump_t *mp;
...
static int
api_acl_interface_etype_whitelist_dump (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  u32 sw_if_index = ~0;
...
static int
api_macip_acl_add (vat_main_t *vam)
{
  acl_test_main_t *sm = &acl_test_main;
  unformat_input_t *i = vam->input;
  ...
  while (unformat_check_input (i) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (i, "action %d", &action))
	{
      ...
      else if (unformat (i, "ip %U/%d", unformat_ip4_address, &src_v4address,
			 &src_prefix_length) ||
      ...
      else if (unformat (i, "src"))
	{
      ...
      else if (unformat (i, "ip %U/%d", unformat_ip6_address, &src_v6address,
			 &src_prefix_length) ||
	       unformat (i, "ip %U", unformat_ip6_address, &src_v6address))
	{
	  ...
	  vec_validate_macip_acl_rules (rules, rule_idx);
	  memcpy (rules[rule_idx].src_prefix.address.un.ip4, &src_v6address,
		  4);
	  rules[rule_idx].src_prefix.address.af = ADDRESS_IP6;
	  rules[rule_idx].src_prefix.len = src_prefix_length;
	}
...
static int
api_macip_acl_add_replace (vat_main_t *vam)
{
  acl_test_main_t *sm = &acl_test_main;
  unformat_input_t *i = vam->input;
