...
/* const static vl_api_acl_rule_t API_RULE_DEFAULT = { */
/*     .action = ACL_API_ACTION_PERMIT, */
/*     .rule = { */
/*         .mnt_proto = 0, */
/*     }, */
/* }; */

/* #define vec_validate_acl_rules(v, idx) vec_validate_init_empty(v, idx, API_RULE_DEFAULT) */
...
api_acl_send_control_ping (vat_main_t * vam)
{
  ...
  if (!acl_test_main.ping_id)
    acl_test_main.ping_id =
      vl_msg_api_get_msg_index ((u8 *) (VL_API_CONTROL_PING_CRC));
  mp_ping = vl_msg_api_alloc_as_if_client (sizeof (*mp_ping));
  mp_ping->_vl_msg_id = htons (acl_test_main.ping_id);
