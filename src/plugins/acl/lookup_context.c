...
static u32
get_acl_user_id (acl_main_t *am, char *user_module_name, char *val1_label,
		 char *val2_label)
{
...
static int
acl_user_id_valid (acl_main_t *am, u32 acl_user_id)
{
...
static int
acl_lc_index_valid (acl_main_t *am, u32 lc_index)
{
...
static u32
acl_plugin_register_user_module (char *user_module_name, char *val1_label,
				 char *val2_label)
{
  acl_main_t *am = &acl_main;
  /*
...
static int
acl_plugin_get_lookup_context_index (u32 acl_user_id, u32 val1, u32 val2)
{
  acl_main_t *am = &acl_main;
...
lock_acl (acl_main_t *am, u32 acl, u32 lc_index)
{
  ...
  elog_acl_cond_trace_X2 (am, (am->trace_acl), "lock acl %d in lc_index %d",
			  "i4i4", acl, lc_index);
...
{
  vec_validate (am->lc_index_vec_by_acl, acl);
  elog_acl_cond_trace_X2 (am, (am->trace_acl), "unlock acl %d in lc_index %d",
			  "i4i4", acl, lc_index);
 ...
 */
static void
acl_plugin_put_lookup_context_index (u32 lc_index)
{
  acl_main_t *am = &acl_main;

  elog_acl_cond_trace_X1 (am, (am->trace_acl),
			  "LOOKUP-CONTEXT: put-context lc_index %d", "i4",
			  lc_index);
  ...
  u32 index = vec_search (
    am->acl_users[acontext->context_user_id].lookup_contexts, lc_index);
  ASSERT (index != ~0);
 ...
 */
static int
acl_plugin_set_acl_vec_for_context (u32 lc_index, u32 *acl_list)
{
  int rv = 0;
  ...
  if (am->trace_acl)
	...
	am, (1), "LOOKUP-CONTEXT: set-acl-list lc_index %d", "i4", lc_index);
      for (i = 0; i < vec_len (acl_list); i++)
	{
	  elog_acl_cond_trace_X2 (am, (1), "   acl-list[%d]: %d", "i4i4", i,
				  acl_list[i]);
	}
    }
  if (!acl_lc_index_valid (am, lc_index))
...
void
acl_plugin_lookup_context_notify_acl_change (u32 acl_num)
{
  acl_main_t *am = &acl_main;
...
static void
acl_plugin_fill_5tuple (u32 lc_index, vlib_buffer_t *b0, int is_ip6,
			int is_input, int is_l2_path,
			fa_5tuple_opaque_t *p5tuple_pkt)
{
  acl_plugin_fill_5tuple_inline (&acl_main, lc_index, b0, is_ip6, is_input,
				 is_l2_path, p5tuple_pkt);
}

static int
acl_plugin_match_5tuple (u32 lc_index, fa_5tuple_opaque_t *pkt_5tuple,
			 int is_ip6, u8 *r_action, u32 *r_acl_pos_p,
...
acl_plugin_show_lookup_user (u32 user_index)
{
  ...
  pool_foreach (auser, am->acl_users)
	...
	{
	  vlib_cli_output (vm, "index %d:%s:%s:%s", curr_user_index,
			   auser->user_module_name, auser->val1_label,
			   auser->val2_label);
	}
    }
}
...
acl_plugin_show_lookup_context (u32 lc_index)
{
  ...
  vlib_main_t *vm = am->vlib_main;
  acl_lookup_context_t *acontext;
  // clib_warning("LOOKUP-CONTEXT: lc_index %d acl_list [ %U ]", lc_index,
  // format_vec32, acl_list, "%d");
  ...
  pool_foreach (acontext, am->acl_lookup_contexts)
      ...
      if ((lc_index == ~0) || (curr_lc_index == lc_index))
	{
	  if (acl_user_id_valid (am, acontext->context_user_id))
	    {
	      acl_lookup_context_user_t *auser =
		pool_elt_at_index (am->acl_users, acontext->context_user_id);
	      vlib_cli_output (
		vm, "index %d:%s %s: %d %s: %d, acl_indices: %U",
	    ...
	    {
	      vlib_cli_output (vm,
			       "index %d: user_id: %d user_val1: %d "
			       "user_val2: %d, acl_indices: %U",
