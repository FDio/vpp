...
int acl_plugin_set_acl_vec_for_context (u32 lc_index, u32 *acl_list)
{
  ...
  for(i=0; i<vec_len(old_acl_vector); i++)
    hash_acl_unapply(am, lc_index, old_acl_vector[i]);
  for(i=0; i<vec_len(acl_list); i++)
    hash_acl_apply(am, lc_index, acl_list[i]);
  // unapply_old_acls()
  // apply_new_acls()
...
{
  clib_warning("LOOKUP-CONTEXT: notify change acl %d - FIXME Code here", acl_num);
