...
Else we kick off the quantum of cleanup, and raise another interrupt to ourselves if that cleanup operation returns non-zero,
...
One potential inefficiency is the bitmap values set by the traffic - there is nothing to clear them.
...
would trigger the cleanup of the bits in the 
