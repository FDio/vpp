...
static int
acl_action_decode (vl_api_acl_action_t action, acl_action_t * out)
{
  switch (action)
    {
#define _(a,b)                                  \
...
static int
acl_rule_decode (const vl_api_acl_rule_t * in, acl_rule_t * out)
{
  int rv;
...
{
  return ((vl_api_acl_action_t) in);
}

static void
acl_rule_encode (const acl_rule_t * in, vl_api_acl_rule_t * out)
{
