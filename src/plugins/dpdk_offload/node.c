...
static u8 * format_dpdk_vxlan_rx_trace (u8 * s, va_list * args)
{
  ...
  rx_dma_trace = vlib_get_plugin_symbol ("dpdk_plugin.so", "forat_dpdk_rx_dma_trace");
...
static_always_inline u8
dpdk_check_ip (vlib_buffer_t *b, u16 len)
{
  ip4_vxlan_header_t * hdr = vlib_buffer_get_current(b) - sizeof *hdr;
  u16 ip_len = clib_net_to_host_u16 (hdr->ip4.length);
  return ip_len > len || hdr->ip4.ttl == 0 || hdr->ip4.ip_version_and_header_length != 0x45;
}
		  ...
		  vlib_node_runtime_t * node, u32 thread_index, u16 queue_id, u32 n_trace)
{
  ...
  while (n_buffers > 0)
  {
    ...
    while (n_buffers >= 4 && n_left_to_next >= 4)
    {
      ...
      u8 ip_err0 = dpdk_check_ip (b0, mb0->pkt_len);
      u8 ip_err1 = dpdk_check_ip (b1, mb1->pkt_len);
      u8 ip_err2 = dpdk_check_ip (b2, mb2->pkt_len);
      u8 ip_err3 = dpdk_check_ip (b3, mb3->pkt_len);
    ...
    while (n_buffers > 0 && n_left_to_next > 0)
    {
      ...
      if (dpdk_check_ip (b0, mb0->pkt_len) || dpdk_check_ip_udp_len (b0) || dpdk_check_udp_csum (vm, b0))
        next0 = DPDK_VXLAN_OFFLOAD_NEXT_DROP, error0 = DPDK_OFFLOAD_IP_CHECKSUM_ERROR;
      dpdk_rx_error_from_mb (mb0, &next0, &error0);
      b0->error = node->errors[error0];
      u32 sw_if_index0 = vnet_buffer (b0)->sw_if_index[VLIB_RX];
      vlib_increment_combined_counter (rx_counter[next0], thread_index, sw_if_index0, 1, len0);
