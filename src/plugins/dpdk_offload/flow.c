...
dpdk_add_remove_vxlan_flow (u32 hw_if_index, u32 sw_if_index, int is_add)
{
  ...
  if (conf->vxlan_rx.q_range.count == 0)
    return clib_error_return (0, "no vxlan queues configured");
...
dpdk_enable_disable_vxlan_flow (u32 hw_if_index, u32 sw_if_index, int is_add)
{
  ...
  conf->vxlan_rx.enabled =
    clib_bitmap_set (conf->vxlan_rx.enabled, sw_if_index, is_add);

  u32 last = conf->vxlan_rx.n_enabled;
  conf->vxlan_rx.n_enabled = clib_bitmap_count_set_bits (conf->vxlan_rx.enabled);
  // enable or disable offload input node
  int enable = last == 0 && conf->vxlan_rx.n_enabled > 0;
  int disable = last > 0 && conf->vxlan_rx.n_enabled == 0;
  ...
  if (enable || disable)
  {
    ...
    int q;
    for (q = 0; q < conf->vxlan_rx.q_range.count; q++)
    {
      int queue_id = conf->vxlan_rx.q_range.first + q;
  ...
  vec_foreach (xd, dm->devices)
    {
      ...
      if (!conf)
        continue;
      int enabled = clib_bitmap_get(conf->vxlan_rx.enabled, if_index);
      if (enabled == 0)
        continue;
