...
dpdk_offload_setup_workers (dpdk_device_t * xd, dpdk_offload_state_t * state, u32 node_idx)
{
  ...
  clib_bitmap_foreach (i, conf->workers,
    ({
      vnet_hw_interface_assign_rx_thread (vnm, xd->hw_if_index, q++,
                                          node_idx, vdm->first_worker_thread_index + i);
     }));
  /* *INDENT-ON* */
  ASSERT (!conf->workers || q == rx_q_total);
  for (; q < rx_q_total; q++)
    vnet_hw_interface_assign_any_rx_thread (vnm, xd->hw_if_index, q, node_idx);
