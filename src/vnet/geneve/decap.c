	      ...
	      vlib_frame_t * from_frame, u32 is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  vnet_geneve_ntoh (geneve0);
	  vnet_geneve_ntoh (geneve1);
	  ...
	  if (PREDICT_FALSE
	    ...
	    {
		  clib_warning("MARCO_PRINT: vnet_get_geneve_version (geneve0) != GENEVE_VERSION");
	  ...
	  if (is_ip4)
	    {
	      ...
	      key4_0.vni = vnet_get_geneve_vni (geneve0);
	      ...
	      if (PREDICT_FALSE (key4_0.as_u64 != last_key4.as_u64))
		{
		  ...
		  if (PREDICT_FALSE (p0 == NULL))
		    {
			  clib_warning("MARCO_PRINT: p0 == NULL");
		...
		{
	    	  clib_warning("MARCO_PRINT: validate_geneve_fib (b0, t0, is_ip4) == 0)");
		  error0 = GENEVE_ERROR_NO_SUCH_TUNNEL;
		  next0 = GENEVE_INPUT_NEXT_DROP;
		  goto trace0;
		}
	  ...
	  else			/* !is_ip4 */
	    {
	      ...
	      key6_0.vni = vnet_get_geneve_vni (geneve0);
	...
	next0:
	  ...
	  clib_warning("MARCO_PRINT: next0 = %u", next0);
	...
	trace0:
	  ...
	  clib_warning("MARCO_PRINT: IN TRACE0 = %u", b0->error);
	  ...
	  if (is_ip4)
	    {
	      ...
	      key4_1.vni = vnet_get_geneve_vni (geneve1);;
	  ...
	  else			/* !is_ip4 */
	    {
	      ...
	      key6_1.vni = vnet_get_geneve_vni (geneve1);;
