...
static u8 * format_geneve_rx_trace (u8 * s, va_list * args)
{
  ...
  if (t->tunnel_index != ~0)
    {
      s = format (s, "GENEVE decap from geneve_tunnel%d vni %d next %d error %d",
             ...
             u32 is_ip4)
{
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  b1 = vlib_get_buffer (vm, bi1);
	    ...
	    vlib_buffer_advance
          ...
          if (is_ip4) {
            vlib_buffer_advance
              (b0, sizeof(*ip4_0)+sizeof(udp_header_t)+GENEVE_BASE_HEADER_LENGTH+vnet_get_geneve_options_len(geneve0));
	    vlib_buffer_advance
              (b1, sizeof(*ip4_1)+sizeof(udp_header_t)+GENEVE_BASE_HEADER_LENGTH+vnet_get_geneve_options_len(geneve1));
          } else {
	    vlib_buffer_advance
              (b0, sizeof(*ip6_0)+sizeof(udp_header_t)+GENEVE_BASE_HEADER_LENGTH+vnet_get_geneve_options_len(geneve0));
            vlib_buffer_advance
              (b0, sizeof(*ip6_0)+sizeof(udp_header_t)+GENEVE_BASE_HEADER_LENGTH+vnet_get_geneve_options_len(geneve1));
          }
	  ...
	  if (PREDICT_FALSE (geneve0->ver != GENEVE_VERSION))
	    {
          ...
          if (is_ip4) {
            key4_0.local = ip4_0->src_address.as_u32;
	    ...
	    if (PREDICT_FALSE (ip4_address_is_multicast (&ip4_0->dst_address)))
	      {
		key4_0.local = ip4_0->dst_address.as_u32;
		key4_0.vni = geneve0->vni;
	    ...
	    goto trace0;
            ...
            key6_0.local.as_u64[0] = ip6_0->src_address.as_u64[0];
            key6_0.local.as_u64[1] = ip6_0->src_address.as_u64[1];
	    ...
	    if (PREDICT_FALSE (ip6_address_is_multicast (&ip6_0->dst_address)))
	      {
		key6_0.local.as_u64[0] = ip6_0->dst_address.as_u64[0];
		key6_0.local.as_u64[1] = ip6_0->dst_address.as_u64[1];
		key6_0.vni = geneve0->vni;
		p0 = hash_get_mem (vxm->geneve6_tunnel_by_key, &key6_0);
		if (PREDICT_TRUE (p0 != NULL)) 
		  {
	  ...
	  if (PREDICT_FALSE (geneve1->ver != GENEVE_VERSION))
	    {
          ...
          if (is_ip4) {
            key4_1.local = ip4_1->src_address.as_u32;
	    ...
	    if (PREDICT_FALSE (ip4_address_is_multicast (&ip4_1->dst_address)))
	      {
		key4_1.local = ip4_1->dst_address.as_u32;
		key4_1.vni = geneve1->vni;
		/* Make sure mcast GENEVE tunnel exist by packet DIP and VNI */
		p1 = hash_get (vxm->geneve4_tunnel_by_key, key4_1.as_u64);
	    ...
	    goto trace1;
            ...
            key6_1.local.as_u64[0] = ip6_1->src_address.as_u64[0];
            key6_1.local.as_u64[1] = ip6_1->src_address.as_u64[1];
	    ...
	    if (PREDICT_FALSE (ip6_address_is_multicast (&ip6_1->dst_address)))
	      {
		key6_1.local.as_u64[0] = ip6_1->dst_address.as_u64[0];
		key6_1.local.as_u64[1] = ip6_1->dst_address.as_u64[1];
		key6_1.vni = geneve1->vni;
		p1 = hash_get_mem (vxm->geneve6_tunnel_by_key, &key6_1);
		if (PREDICT_TRUE (p1 != NULL)) 
		  {
  ...
  vlib_node_increment_counter (vm, is_ip4? 
			       geneve4_input_node.index:geneve6_input_node.index,
                               GENEVE_ERROR_DECAPSULATED, 
                               pkts_decapsulated);
