...
#define GENEVE_VNI_MASK			0xC3E07F00

static inline u32
vnet_get_geneve_vni (geneve_header_t * h)
{
  return ((h->vni_rsvd & GENEVE_VNI_MASK) >> GENEVE_VNI_SHIFT);
}
...
vnet_set_geneve_vni (geneve_header_t * h, u32 vni)
{
  ...
  h->vni_rsvd |= (vni << GENEVE_VNI_SHIFT);
}
...
vnet_geneve_ntoh (geneve_header_t * h)
{
  ...
  h->vni_rsvd = clib_net_to_host_u32 (h->vni_rsvd);
}
...
vnet_geneve_hton (geneve_header_t * h)
{
  ...
  h->vni_rsvd = clib_host_to_net_u32 (h->vni_rsvd);
}
