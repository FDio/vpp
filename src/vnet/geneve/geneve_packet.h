...
#define GENEVE_OPTLEN_MASK		0xFF000000
#define GENEVE_O_BIT_MASK		0xFF800000
#define GENEVE_C_BIT_MASK		0xFFC00000
#define GENEVE_6_RESERVED_MASK	0x7FFF0000
#define GENEVE_PROTOCOL_MASK	0xFFFFFFFF

static inline u32
vnet_get_geneve_vni (geneve_header_t * h)
{
	u32 vni = clib_net_to_host_u32 (h->vni_rsvd) >> GENEVE_VNI_SHIFT;
	clib_warning("MARCO_PRINT: %u", vni);
	return (clib_net_to_host_u32 (h->vni_rsvd) >> GENEVE_VNI_SHIFT);
}

static inline void
vnet_set_geneve_vni (geneve_header_t * h, u32 vni)
{
	h->vni_rsvd = clib_host_to_net_u32 (vni << GENEVE_VNI_SHIFT);
}

static inline u8
vnet_get_geneve_version (geneve_header_t * h)
{
	u32 ver = (h->first_word & GENEVE_VERSION_MASK) >> GENEVE_VERSION_SHIFT;
	clib_warning("MARCO_PRINT: %u", ver);
...
vnet_set_geneve_version (geneve_header_t * h, u8 version)
{
  ...
  h->first_word |= (version << GENEVE_VERSION_SHIFT);
}

static inline u8
vnet_get_geneve_options_len (geneve_header_t * h)
{
	u8 opt_len = (h->first_word & GENEVE_OPTLEN_MASK) >> GENEVE_OPTLEN_SHIFT;
	clib_warning("MARCO_PRINT: %u", opt_len);
...
vnet_set_geneve_options_len (geneve_header_t * h, u8 len)
{
  ...
  h->first_word |= (len << GENEVE_OPTLEN_SHIFT);
}
...
vnet_set_geneve_oamframe_bit (geneve_header_t * h, u8 oam)
{
  ...
  h->first_word |= (oam << GENEVE_O_BIT_SHIFT);
}
...
vnet_set_geneve_critical_bit (geneve_header_t * h, u8 critical_opts)
{
  ...
  h->first_word |= (critical_opts << GENEVE_C_BIT_SHIFT);
}
...
vnet_set_geneve_protocol (geneve_header_t * h, u16 protocol)
{
  ...
  h->first_word |= (protocol);
}

static inline void
vnet_geneve_ntoh (geneve_header_t * h)
{
	clib_warning("MARCO_PRINT: (NTOH) HEADER BEFORE: %u", h->first_word);
  h->first_word = clib_net_to_host_u32 (h->first_word);
  clib_warning("MARCO_PRINT: (NTOH) HEADER AFTER: %u", h->first_word);
}

static inline void
vnet_geneve_hton (geneve_header_t * h)
{
	clib_warning("MARCO_PRINT: (HTON) HEADER BEFORE: %u", h->first_word);
  h->first_word = clib_host_to_net_u32 (h->first_word);
  clib_warning("MARCO_PRINT: (HTON) HEADER AFTER: %u", h->first_word);
}
