...
{
  geneve_tunnel_t *t;
  geneve_main_t *gem = &geneve_main;

  t = pool_elt_at_index (gem->tunnels, index);
...
geneve_rewrite (geneve_tunnel_t * t, bool is_ip6)
{
  ...
  /* geneve header */
  vnet_set_vni_and_flags (geneve, t->vni);
...
static bool
geneve_decap_next_is_valid (geneve_main_t * gem, u32 is_ip6, u32 decap_next_index)
{
  vlib_main_t *vm = gem->vlib_main;
...
mcast_shared_add (ip46_address_t * dst, fib_node_index_t mfei, adj_index_t ai)
{
  ...
  hash_set_key_copy (&geneve_main.mcast_shared, dst, new_ep.as_u64);
}
...
mcast_shared_remove (ip46_address_t * dst)
{
  ...
  hash_unset_key_free (&geneve_main.mcast_shared, dst);
}
  ...
  (vnet_geneve_add_del_tunnel_args_t * a, u32 * sw_if_indexp)
{
  ...
  if (!is_ip6)
    {
      ...
      p = hash_get (gem->geneve4_tunnel_by_key, key4.as_u64);
    }
  else
    {
      key6.src = a->dst.ip6;
      key6.vni = clib_host_to_net_u32 (a->vni << 8);
      p = hash_get_mem (gem->geneve6_tunnel_by_key, &key6);
    }
  ...
  if (a->is_add)
	...
	a->decap_next_index = geneve_INPUT_NEXT_L2_INPUT;
      if (!geneve_decap_next_is_valid (gem, is_ip6, a->decap_next_index))
	return VNET_API_ERROR_INVALID_DECAP_NEXT;

      pool_get_aligned (gem->tunnels, t, CLIB_CACHE_LINE_BYTES);
      memset (t, 0, sizeof (*t));
...
#undef _
      ...
      geneve_rewrite (t, is_ip6);

      /* copy the key */
      if (is_ip6)
	hash_set_key_copy (&gem->geneve6_tunnel_by_key, &key6,
			   t - gem->tunnels);
      else
	hash_set (gem->geneve4_tunnel_by_key, key4.as_u64, t - gem->tunnels);

      vnet_hw_interface_t *hi;
      if (vec_len (gem->free_geneve_tunnel_hw_if_indices) > 0)
	{
	  vnet_interface_main_t *im = &vnm->interface_main;
	  hw_if_index = gem->free_geneve_tunnel_hw_if_indices
	    [vec_len (gem->free_geneve_tunnel_hw_if_indices) - 1];
	  _vec_len (gem->free_geneve_tunnel_hw_if_indices) -= 1;

	  hi = vnet_get_hw_interface (vnm, hw_if_index);
	  hi->dev_instance = t - gem->tunnels;
      ...
      else
	{
	  hw_if_index = vnet_register_interface
	    (vnm, geneve_device_class.index, t - gem->tunnels,
	     geneve_hw_class.index, t - gem->tunnels);
	  hi = vnet_get_hw_interface (vnm, hw_if_index);
	}
      ...
      vec_validate_init_empty (gem->tunnel_index_by_sw_if_index, sw_if_index,
			       ~0);
      gem->tunnel_index_by_sw_if_index[sw_if_index] = t - gem->tunnels;
      ...
      if (!ip46_address_is_multicast (&t->dst))
	{
	  ...
	  t->sibling_index = fib_entry_child_add
	    (t->fib_entry_index, FIB_NODE_TYPE_GENEVE_TUNNEL, t - gem->tunnels);
	...
	return VNET_API_ERROR_NO_SUCH_ENTRY;
      ...
      t = pool_elt_at_index (gem->tunnels, p[0]);
      ...
      set_int_l2_mode (gem->vlib_main, vnm, MODE_L3, t->sw_if_index, 0, 0, 0,
		       0);
      vec_add1 (gem->free_geneve_tunnel_hw_if_indices, t->hw_if_index);

      gem->tunnel_index_by_sw_if_index[t->sw_if_index] = ~0;

      if (!is_ip6)
	hash_unset (gem->geneve4_tunnel_by_key, key4.as_u64);
      else
	hash_unset_key_free (&gem->geneve6_tunnel_by_key, &key6);
				...
				vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (line_input, "vni %d", &vni))
	{
      ...
      else if (unformat (line_input, "protocol %d", &protocol))
	;
      else if (unformat (line_input, "opt_len %d", &opt_len))
	;
      else
	{
 ...
 * @cliexcmd{create geneve tunnel src 10.0.3.1 dst 10.0.3.3 vni 13 encap-vrf-id 7}
 * Example of how to delete a geneve Tunnel:
 * @cliexcmd{create geneve tunnel src 10.0.3.1 dst 10.0.3.3 vni 13 del}
 ?*/
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (create_geneve_tunnel_command, static) = {
  .path = "create geneve tunnel",
  .short_help =
  "create geneve tunnel src <local-vtep-addr>"
  " {dst <remote-vtep-addr>|group <mcast-vtep-addr> <intf-name>} "
  " vni <nn> [protocol <nn>] [opt_len <nn>]"
			     ...
			     vlib_cli_command_t * cmd)
{
  ...
  if (pool_elts (gem->tunnels) == 0)
    vlib_cli_output (vm, "No geneve tunnels configured...");

  pool_foreach (t, gem->tunnels, (
				   {
				   vlib_cli_output (vm, "%U",
...
geneve_init (vlib_main_t * vm)
{
  ...
  gem->vnet_main = vnet_get_main ();
  gem->vlib_main = vm;

  /* initialize the ip6 hash */
  gem->geneve6_tunnel_by_key = hash_create_mem (0,
					      sizeof (geneve6_tunnel_key_t),
					      sizeof (uword));
  gem->vtep6 = hash_create_mem (0, sizeof (ip6_address_t), sizeof (uword));
  gem->mcast_shared = hash_create_mem (0,
