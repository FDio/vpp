			    ...
			    void *cb)
{
  ...
  log_info (port->dev, "feature_update_cb: starting");

  if (arc_index != vdm->eth_port_rx_feature_arc_index)
    return;

  log_info (port->dev, "feature_update_cb: new arc index");
  ...
  log_info (port->dev, "feature_update_cb: instance exists");
  ...
  log_info (port->dev, "feature_update_cb: port exists");

  if (vnet_have_features (arc_index, sw_if_index))
    {
      log_info (port->dev, "feature_update_cb: old feature present");
	...
	{
	  log_info (port->dev, "feature_update_cb: difference detected");
	  intf->current_config_index = current_config_index;
    ...
    {
      log_info (port->dev, "feature_update_cb: no old feature");

      if (intf->feature_arc)
	{
	  log_info (port->dev, "feature_update_cb: arc detected");
  ...
  if (update_runtime)
    {
      log_info (port->dev, "feature_update_cb: runtime update needed");
	...
	vnet_dev_rx_queue_rt_request (
      ...
      log_info (port->dev, "runtime update requested due to chgange in "
