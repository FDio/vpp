...
extern vlib_node_registration_t eth_port_rx_node;
  ...
  if (node_index == ~0)
  ...
  else
    {
      u16 next_index = vlib_node_add_next (vlib_get_main (),
					   eth_port_rx_node.index, node_index);
...
vnet_dev_admin_up_down_fn (vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
  ...
  vnet_dev_port_config_change_req_t ccr = {
    .changes.admin_state = 1,
    .new.admin_up = is_up,
  };

  rv = vnet_dev_process_port_config_change_req (vm, p, &ccr);
