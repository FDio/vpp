...
#define _(b, n, d) STATIC_ASSERT (VL_API_DEV_FLAG_##n == VNET_DEV_F_##n, "");
foreach_vnet_dev_flag;
#undef _

#define _(b, n, d)                                                            \
  STATIC_ASSERT (VL_API_DEV_PORT_FLAG_##n == VNET_DEV_PORT_F_##n, "");
...
vl_api_dev_attach_t_handler (vl_api_dev_attach_t *mp)
{
  ...
  a.flags.n = mp->flags;
  strncpy (a.device_id, (char *) mp->device_id, sizeof (a.device_id) - 1);
  strncpy (a.driver_name, (char *) mp->driver_name,
	   sizeof (a.driver_name) - 1);
...
#undef _
  ...
  strncpy (a.intf_name, (char *) mp->intf_name, sizeof (a.intf_name) - 1);
  vec_add (a.args, mp->args.buf, mp->args.length);
