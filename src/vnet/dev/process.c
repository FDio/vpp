			    ...
			    vnet_dev_event_data_t *edp)
{
  ...
  if (edp - rv != VNET_DEV_PENDING)
    {
      vec_free (edp);
      log_debug (dev, "return after discarding copy");
    }
  else
    {
      edp->rv = rv;
      log_debug (dev, "return with rv");
    }
  return rv;
}
				      ...
				      vnet_dev_event_data_t *edp)
{
  ...
  vnet_dev_process_event_send (vm, dev, edp, 0);
  ...
  if (pool_elts (dev->periodic_ops) == 1)
    {
      ...
      vnet_dev_process_event_send (vm, dev, &ed, 1);
				       ...
				       vnet_dev_port_op_no_rv_t *op)
{
  ...
  vnet_dev_process_event_send (vm, port->dev, &ed, 1);
}
