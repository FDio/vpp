...
typedef struct
{
  ...
  union
  {
    struct
    {
      ...
      vnet_dev_port_config_change_req_t *change_req;
    } port_config_change;
    struct
    {
      vnet_dev_op_t *op;
			    ...
			    vnet_dev_event_data_t *ed)
{
  ...
  switch (ed->event)
    ...
    case VNET_DEV_EVENT_PORT_CONFIG_CHANGE_REQ:
      ...
      rv =
	vnet_dev_port_config_change (vm, p, ed->port_config_change.change_req);
      break;
...
vnet_dev_rv_t
vnet_dev_process_port_config_change_req (
  vlib_main_t *vm, vnet_dev_port_t *port,
  vnet_dev_port_config_change_req_t *pccr)
{
