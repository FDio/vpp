...
typedef enum
{
  ...
  VNET_DEV_MGMT_OP_ACTION_TX_QUEUE_ASSIGN,
  VNET_DEV_MGMT_OP_ACTION_TX_QUEUE_UNASSIGN,
...
_vnet_dev_exec_op (vlib_main_t *vm, vnet_dev_mgmt_op_t *op)
{
  ...
  if (op->action == VNET_DEV_MGMT_OP_ACTION_RX_QUEUE_ASSIGN ||
      op->action == VNET_DEV_MGMT_OP_ACTION_RX_QUEUE_UNASSIGN)
    {
      ...
      u32 node_index = op->rx_queue->port->rx_node_index;
      ...
      if (op->action == VNET_DEV_MGMT_OP_ACTION_RX_QUEUE_ASSIGN)
	{
      ...
      else if (op->action == VNET_DEV_MGMT_OP_ACTION_RX_QUEUE_ASSIGN)
	{
	  u32 i;
...
dev_mgmt_fn (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  for (u16 i = tail; i < head; i++)
	...
	break;
      ...
      __atomic_store_n (&mm->tail, i, __ATOMIC_RELEASE);
...
static void
_vnet_dev_mgmt_add_op (vlib_main_t *vm, vnet_dev_mgmt_op_t *ops, int n)
{
  vnet_dev_mgmt_main_t *mm = &mgmt_main;
  ...
  if (n_ops - (head - tail) < n)
    clib_panic ("dev-mgmt queue lock");
...
void
vnet_dev_mgmt_rx_queue_unassign_thread (vlib_main_t *vm,
					vnet_dev_rx_queue_t *rxq)
{
  vnet_dev_mgmt_op_t op = {
    .action = VNET_DEV_MGMT_OP_ACTION_RX_QUEUE_UNASSIGN,
    .rx_queue = rxq,
    .thread_index = rxq->polling_thread_index,
  };

  ASSERT (rxq->polling_thread_index != VNET_DEV_THREAD_NOT_ASSIGNED);

  _vnet_dev_mgmt_add_op (vm, &op, 1);
}

void
vnet_dev_mgmt_rx_queue_assign_thread (vlib_main_t *vm,
				      vnet_dev_rx_queue_t *rxq,
				      u32 thread_index)
{
  vnet_dev_mgmt_op_t op = {
