...
VLIB_REGISTER_NODE (eth_port_rx_node) = {
...
VNET_FEATURE_ARC_INIT (eth_port_rx, static) = {
  .arc_name = "eth-port-rx",
  .start_nodes = VNET_FEATURES ("eth-port-rx"),
...
vnet_dev_port_stop (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  foreach_vnet_dev_port_rx_queue (q, port)
    {
      q->started = 0;
      log_debug (dev, "port_stop: port %u rx queue %u stopped", port->port_id,
		 q->queue_id);
    }
  ...
  foreach_vnet_dev_port_tx_queue (q, port)
    {
      q->started = 0;
      log_debug (dev, "port_stop: port %u tx queue %u stopped", port->port_id,
		 q->queue_id);
    }
...
vnet_dev_port_start (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  foreach_vnet_dev_port_rx_queue (q, port)
    if (q->enabled)
      {
	log_debug (dev, "port_start: port %u rx queue %u started",
		   port->port_id, q->queue_id);
	q->started = 1;
      }
  ...
  foreach_vnet_dev_port_tx_queue (q, port)
    if (q->enabled)
      {
	log_debug (dev, "port_start: port %u tx queue %u started",
		   port->port_id, q->queue_id);
	q->started = 1;
      }
		   ...
		   vnet_dev_port_add_args_t *args)
{
  ...
  port->conf.max_frame_size = args->port.attr.max_supported_frame_size;
  port->conf.primary_hw_addr = args->port.attr.hw_addr;
...
{
  typeof (req->changes.as_number) c;
  if (req->validated)
    return VNET_DEV_OK;

  c = port->started ? port->attr.supported_cfg_changes.started.as_number :
			    port->attr.supported_cfg_changes.stopped.as_number;

  c = ~c & req->changes.as_number;

  if (c)
    {
      log_debug (port->dev, "unsupored change(s) requested: %U",
		 format_vnet_dev_port_cfg_changes, c);
      return VNET_DEV_ERR_NOT_SUPPORTED;
    }

  if (req->changes.max_frame_size)
    {
      if (req->new.max_frame_size > port->attr.max_supported_frame_size)
	return VNET_DEV_ERR_INVALID_VALUE;
      if (req->new.max_frame_size == port->conf.max_frame_size)
	return VNET_DEV_ERR_NO_CHANGE;
    }

  if (req->changes.primary_hw_addr)
    {
      if (clib_memcmp (&req->new.primary_hw_addr, &port->conf.primary_hw_addr,
			  ...
			  vnet_dev_port_cfg_change_req_t *req)
{
  ...
  if (req->changes.max_frame_size)
    port->conf.max_frame_size = req->new.max_frame_size;

  if (req->changes.primary_hw_addr)
    clib_memcpy (&req->new.primary_hw_addr, &port->conf.primary_hw_addr,
		 sizeof (vnet_dev_hw_addr_t));

  if (req->changes.promisc)
    port->conf.promisc = req->new.promisc;
...
vnet_dev_port_if_create (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if (port->attr.type == VNET_DEV_PORT_TYPE_ETHERNET)
	...
	vnm, &(vnet_eth_interface_registration_t){
	       .address = port->conf.primary_hw_addr.eth_mac,
	       .max_frame_size = port->conf.max_frame_size,
