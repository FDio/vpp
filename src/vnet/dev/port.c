		   ...
		   vnet_dev_port_add_args_t *args)
{
  ...
  port->conf.hw_addr = args->port.attr.hw_addr;
...
vnet_dev_port_start_stop (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if (port->started == 0 && port->conf.admin_up == 1)
    rv = vnet_dev_port_start (vm, port);
  else if (port->started == 1 && port->conf.admin_up == 0)
    vnet_dev_port_stop (vm, port);
  else
    {
...
vnet_dev_port_config_change (vlib_main_t *vm, vnet_dev_port_t *port,
			     vnet_dev_port_config_change_req_t *req)
{
  vnet_dev_t *dev = port->dev;
  vnet_dev_rv_t rv = VNET_DEV_OK;
  int start_stop = 0;
  ...
  if (req->changes.admin_state)
    {
      ASSERT (req->new.admin_up != port->conf.admin_up);
      port->conf.admin_up = req->new.admin_up;
      log_debug (dev, "port %u admin state changed to %s", port->port_id,
		 port->conf.admin_up ? "up" : "down");
      start_stop = 1;
    }
				  ...
				  u32 frame_size)
{
  ...
  if (p->conf.max_frame_size == frame_size)
    return 0;

  if (p->attr.max_supported_frame_size < frame_size)
    return vnet_error (
      VNET_ERR_UNSUPPORTED,
      "requested max frame size too big, maximum supported value is %u",
      p->attr.max_supported_frame_size);

  vnet_dev_port_config_change_req_t req = {
    .changes.max_frame_size = 1,
    .new.max_frame_size = frame_size,
  };

  rv = vnet_dev_process_port_config_change_req (vm, p, &req);

  if (rv != VNET_DEV_OK)
    return vnet_error (VNET_ERR_UNSPECIFIED, "%U", format_vnet_dev_rv);
			       ...
			       u32 flags)
{
  ...
  switch (flags)
    ...
    case ETHERNET_INTERFACE_FLAG_ACCEPT_ALL:
      ...
      ccr.new.promisc_bcast = 1;
      ccr.new.promisc_mcast = 1;
      break;
    default:
      return ~0;
    }
...
vnet_dev_port_if_create (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if (port->attr.type == VNET_DEV_PORT_TYPE_ETHERNET)
	...
	vnm, &(vnet_eth_interface_registration_t){
	       .address = port->conf.hw_addr.addr,
