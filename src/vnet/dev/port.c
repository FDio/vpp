  ...
  *(ifs) = (vnet_dev_port_interfaces_t){
  ...
  if (a->name[0] == 0)
      ...
      if (n >= sizeof (a->name))
	{
      ...
      clib_memcpy (ifs->name, s, n);
      ifs->name[n] = 0;
      vec_free (s);
    }
  else
    clib_memcpy (ifs->name, a->name, sizeof (ifs->name));
  ...
  if (port->attr.type == VNET_DEV_PORT_TYPE_ETHERNET)
	...
	vnet_hw_interface_set_link_speed (
      ...
      ifs->tx_node_index = hw->tx_node_index;
      ...
      else
	{
	  ...
	  rx_node_index =
	    vlib_register_node (vm, &rx_node_reg, "%s-rx", ifs->name);
	}
	...
	vnet_dev_default_next_index_by_port_type[port->attr.type];
      ...
      log_debug (dev,
		 ...
		 hw_if_index, sw_if_index, rx_node_index, ifs->tx_node_index);
    }
...
vnet_dev_port_add_sec_if (vlib_main_t *vm, vnet_dev_port_t *port, void *ptr)
{
  ...
  if (port->attr.type == VNET_DEV_PORT_TYPE_ETHERNET)
	...
	.index = sip - port->interfaces->secondary_interfaces, .id = a->id
      };
