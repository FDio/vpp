  ...
  foreach_vnet_dev_port_tx_queue (q, port)
      ...
      clib_bitmap_foreach (ti, q->assigned_threads)
	{
	  ...
	  vlib_node_runtime_t *nr =
	    vlib_node_get_runtime (tvm, intf->tx_node_index);
...
vnet_dev_rv_t
vnet_dev_port_sec_if_create (vlib_main_t *vm, vnet_dev_port_t *port, void *ptr)
{
  vnet_dev_main_t *dm = &vnet_dev_main;
  ...
  log_err (port->dev, "TODO %u", a->sw_if_index);
  ...
  if (port->attr.type == VNET_DEV_PORT_TYPE_ETHERNET)
    {
      ...
      vnet_dev_port_sec_if_t *sif, **sip;
...
vnet_dev_rv_t
vnet_dev_port_sec_if_remove (vlib_main_t *vm, vnet_dev_port_t *port, void *ptr)
{
  vnet_dev_port_sec_if_remove_args_t *a = ptr;
