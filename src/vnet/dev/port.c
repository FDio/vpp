...
vnet_dev_port_free (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  log_debug (dev, "freeing port %u (%U) hw_if %u sw_if %u rxq %u txq %u",
	     ...
	     pool_elts (port->rx_queues), pool_elts (port->tx_queues));
...
vnet_dev_port_stop (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  log_debug (dev, "stopping port %u (hw_if %u sw_if %u)", port->port_id,
	     vnet_dev_port_primary_hw_if_index (port),
	     vnet_dev_port_primary_sw_if_index (port));
  ...
  log_debug (dev, "port %u stopped (hw_if %u sw_if %u)", port->port_id,
	     vnet_dev_port_primary_hw_if_index (port),
	     vnet_dev_port_primary_sw_if_index (port));
  port->started = 0;
}
...
vnet_dev_port_start (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  log_debug (dev, "starting port %u (hw_if %u sw_if %u)", port->port_id,
	     vnet_dev_port_primary_hw_if_index (port),
	     vnet_dev_port_primary_sw_if_index (port));
  ...
  port->started = 1;
  log_debug (dev, "port %u started (hw_if %u sw_if %u)", port->port_id,
	     vnet_dev_port_primary_hw_if_index (port),
	     vnet_dev_port_primary_sw_if_index (port));
  ...
  *(ifs) = (vnet_dev_port_interfaces_t){
  ...
  if (port->attr.type == VNET_DEV_PORT_TYPE_ETHERNET)
	...
	vnet_dev_default_next_index_by_port_type[port->attr.type];
      ...
      log_debug (dev,
		 "port %u primary interface %s created hw_if %u sw_if %u "
		 "rx_node %u tx_node %u",
		 port->port_id, ifs->primary_interface.name, hw_if_index,
		 sw_if_index, rx_node_index,
...
vnet_dev_port_if_remove (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  log_debug (port->dev,
	     "removing port %u primary interface %U (hw_if %u sw_if %u)",
	     port->port_id, format_vnet_dev_port_primary_intf_name, port,
	     vnet_dev_port_primary_hw_if_index (port),
	     vnet_dev_port_primary_sw_if_index (port));
...
vnet_dev_port_del_sec_if (vlib_main_t *vm, vnet_dev_port_t *port, void *ptr)
{
  ...
  log_debug (port->dev, "removing secondary interface sw_if %u",
	     a->sw_if_index);
