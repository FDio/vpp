			 ...
			 u16 queue_size)
{
  ...
  rxq->next_index = vnet_dev_default_next_index_by_port_type[port->type];
		   ...
		   vnet_dev_port_add_args_t *args)
{
  ...
  ASSERT (args->port.config.max_supported_frame_size);
  ...
  port->max_frame_size = args->port.config.max_supported_frame_size;
  port->hw_addr = args->port.config.hw_addr;
...
vnet_dev_port_start_stop (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if (port->started == 0 && port->admin_up == 1)
    rv = vnet_dev_port_start (vm, port);
  else if (port->started == 1 && port->admin_up == 0)
    vnet_dev_port_stop (vm, port);
  else
    {
