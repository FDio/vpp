...
vnet_dev_rv_t
vnet_dev_api_create_port_sec_if (vlib_main_t *vm,
				 vnet_dev_api_create_port_sec_if_args_t *args)
{
  vnet_dev_port_t *port = 0;
  vnet_dev_t *dev = 0;
  ...
  rv = vnet_dev_process_call_port_op_with_ptr (
    vm, port, vnet_dev_port_sec_if_create, &a);
  args->sw_if_index = (rv == VNET_DEV_OK) ? a.sw_if_index : ~0;
...
vnet_dev_rv_t
vnet_dev_api_remove_port_sec_if (vlib_main_t *vm,
				 vnet_dev_api_remove_port_sec_if_args_t *args)
{
  vnet_dev_main_t *dm = &vnet_dev_main;
  ...
  return vnet_dev_process_call_port_op_with_ptr (
    vm, port, vnet_dev_port_sec_if_remove,
    &(vnet_dev_port_sec_if_remove_args_t){ .sw_if_index = args->sw_if_index });
}
