...
vnet_dev_rv_t
vnet_dev_rx_queue_init (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  return VNET_DEV_OK;
}

vnet_dev_rv_t
vnet_dev_tx_queue_init (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  return VNET_DEV_OK;
}
...
vnet_dev_rx_queue_remove (vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
  ...
  if (dev->ops.rx_queue_free)
    dev->ops.rx_queue_free (vm, rxq);
...
vnet_dev_rx_queue_add (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  rxq = _vnet_dev_alloc_with_data (sizeof (vnet_dev_port_t),
				   driver->registration->rx_queue_data_sz);
  ...
  if (dev->ops.rx_queue_alloc)
    rv = dev->ops.rx_queue_alloc (vm, rxq);
...
vnet_dev_tx_queue_remove (vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
  ...
  if (dev->ops.tx_queue_free)
    dev->ops.tx_queue_free (vm, txq);
...
vnet_dev_tx_queue_add (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  txq = _vnet_dev_alloc_with_data (sizeof (vnet_dev_port_t),
				   driver->registration->tx_queue_data_sz);
  ...
  if (dev->ops.tx_queue_alloc)
    rv = dev->ops.tx_queue_alloc (vm, txq);
...
vnet_dev_port_remove (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if (dev->ops.port_free)
    dev->ops.port_free (vm, port);
		   ...
		   vnet_dev_port_add_args_t args)
{
  ...
  port = _vnet_dev_alloc_with_data (sizeof (vnet_dev_port_t),
				    driver->registration->port_data_sz);
