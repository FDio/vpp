...
#define foreach_vnet_dev_port_cfg_change                                      \
  _ (promisc)                                                                 \
  _ (max_frame_size)                                                          \
  _ (primary_hw_addr)                                                         \
  _ (secondary_hw_addr)
...
typedef union
{
  ...
  {
#define _(x) u8 x : 1;
    foreach_vnet_dev_port_cfg_change
#undef _
  };
  u8 as_number;
} vnet_dev_port_cfg_changes_t;
...
{
  vnet_dev_port_cfg_changes_t changes;
  struct
  {
    u8 promisc : 1;
    u16 max_frame_size;
    vnet_dev_hw_addr_t primary_hw_addr;
    vnet_dev_hw_addr_t *secondary_hw_addr_to_add;
    vnet_dev_hw_addr_t *secondary_hw_addr_to_remove;
  } new;
  u8 validated;
...
typedef struct
{
  ...
  struct
  {
    vnet_dev_port_cfg_changes_t started;
    vnet_dev_port_cfg_changes_t stopped;
  } supported_cfg_changes;
...
typedef struct
{
  ...
  vnet_dev_op_no_rv_t *device_deinit;
  vnet_dev_op_t *device_reset;
...
format_function_t format_vnet_dev_port_cfg_changes;
...
#define foreach_vnet_dev_rx_port_next                                         \
...
#define _(n, s) VNET_DEV_ETH_RX_PORT_NEXT_##n,
  foreach_vnet_dev_rx_port_next
#undef _
    VNET_DEV_ETH_RX_PORT_N_NEXTS
} vnet_dev_eth_rx_port_next_t;
