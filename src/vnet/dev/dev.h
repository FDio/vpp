...
typedef struct
{
  ...
  /* rx & tx queue */
  vnet_dev_node_fn_t *rx_node_fn;
  vnet_dev_node_fn_t *tx_node_fn;
...
{
  vnet_dev_port_type_t type;
  vnet_dev_queue_config_t rx_queue;
  vnet_dev_queue_config_t tx_queue;
  vnet_dev_port_config_t port;
  vnet_dev_port_ops_t ops;
...
#define foreach_vnet_dev_rx_queue_runtime(q, node)                            \
  for (vnet_dev_rx_queue_t *                                                  \
	...
	**__last =                                                            \
	  __qp + ((vnet_hw_if_rx_node_runtime_t *) (node)->runtime_data)      \
		   ->n_rx_queues,                                             \
	*(q) = *__qp;                                                         \
