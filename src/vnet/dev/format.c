...
format_vnet_dev_interface_name (u8 *s, va_list *args)
{
  ...
  if (di->is_primary_if)
    return format (s, "%s", p->interfaces->name);

  si = vnet_dev_port_get_sec_if_by_index (p, di->sec_if_index);
  return format (s, "%s/%u", di->port->interfaces->name, si->id);
}
...
format_vnet_dev_port_info (u8 *s, va_list *args)
{
  ...
  if (port->interfaces)
    {
      s =
	format (s, "assigned, primary interface name is '%U', RX node is '%U'",
		format_vnet_sw_if_index_name, vnm,
		port->interfaces->primary_interface.sw_if_index,
		format_vlib_node_name, vm, port->interfaces->rx_node_index);
    }
  else
