...
format_vnet_dev_port_info (u8 *s, va_list *args)
{
  ...
  s = format (s, "\n%UMax frame size is %u (max supported %u)",
	      format_white_space, indent, port->conf.max_frame_size,
...
u8 *
format_vnet_dev_port_cfg_changes (u8 *s, va_list *args)
{
  vnet_dev_port_cfg_changes_t chg = { .as_number = va_arg (*args, u32) };
  int line = 0;

#define _(x)                                                                  \
  if (chg.x)                                                                  \
    s = format (s, "%s%s", line++ ? "" : ", ", #x);
