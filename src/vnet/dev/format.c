...
format_vnet_dev_interface_info (u8 *s, va_list *args)
{
  ...
  if (dev->bus_type == VNET_DEV_BUS_TYPE_PCIE)
    {
      ...
      err = vlib_pci_read_write_config (vm, dev->pci.handle, VLIB_READ, 0,
					&cfg, sizeof (cfg));
      if (!err)
	{
  ...
  if (dev->ops.format_port_status)
    s = format (s, "\n%U%U", format_white_space, indent + 2,
		dev->ops.format_port_status, port);
  ...
  s = format (s, ", %u RX queues (max %u), %u TX queues (max %u)",
	      pool_elts (port->rx_queues), port->max_rx_queues,
	      pool_elts (port->tx_queues), port->max_tx_queues);
  ...
  pool_foreach_index (i, port->rx_queues)
    {
      ...
      s = format (s, "\n%UNum descriptors is %u", format_white_space,
		  indent + 4, rxq->n_desc);
      s = format (s, "\n%UPolling thread is %u", format_white_space,
		  indent + 4, rxq->polling_thread_index);
    }
