...
format_vnet_dev_rx_queue_info (u8 *s, va_list *args)
{
  ...
  s = format (s, "RX queue %u:", rxq->queue_id);
  s = format (s, "\n%USize is %u, buffer pool index is %u", format_white_space,
	      indent, rxq->size, rxq->buffer_pool_index);
...
format_vnet_dev_tx_queue_info (u8 *s, va_list *args)
{
  ...
  s = format (s, "TX queue %u:", txq->queue_id);
  s = format (s, "\n%USize is %u", format_white_space, indent + 4, txq->size);
...
format_vnet_dev_interface_info (u8 *s, va_list *args)
{
  ...
  foreach_vnet_dev_pool (q, port->rx_queues)
    {
      s = format (s, "'n%UQueue %u:", format_white_space, indent + 2,
		  q->queue_id);
  ...
  foreach_vnet_dev_pool (q, port->tx_queues)
    {
      s = format (s, "\n%UQueue %u:", format_white_space, indent + 2,
		  q->queue_id);
      s = format (s, "\n%U%U", format_white_space, indent + 4,
