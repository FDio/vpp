				  ...
				  u32 node_index)
{
  ...
  log_debug (di->port->dev, "starting");
  if (di->is_primary_if)
  ...
  if (node_index == ~0)
    {
      log_debug (di->port->dev, "no node index");
      intf->redirect_to_node_next_index = 0;
      if (intf->feature_arc == 0)
	{
	  log_debug (di->port->dev, "feature arc zero");
	  intf->rx_next_index =
  ...
  else
    {
      ...
      log_debug (di->port->dev, "node index %u next index %u", node_index,
		 next_index);
  ...
  intf->rx_next_index =
    node_index == ~0 ?
      vnet_dev_default_next_index_by_port_type[di->port->attr.type] :
      node_index;
  log_debug (di->port->dev, "rx_next_index %u", intf->rx_next_index);
