			  ...
			  const u8 *new)
{
  ...
  vnet_dev_set_hw_addr_eth_mac (&req.new.primary_hw_addr, new);

  log_debug (p->dev, "port_mac_change: %U", format_vnet_dev_hw_addr,
	     &req.new.primary_hw_addr);
			      ...
			      u8 is_add)
{
  ...
  vnet_dev_port_cfg_change_req_t req = {
    .changes.secondary_hw_addr = 1,
  };

  vec_add2 (req.new.secondary_hw_addr_to_add, addr, 1);
  vnet_dev_set_hw_addr_eth_mac (addr, address);

  log_debug (p->dev, "add_del_mac_address received (addr %U is_add %u",
	     format_vnet_dev_hw_addr, addr, is_add);

  rv = vnet_dev_port_cfg_change_req_validate (vm, p, &req);
  if (rv != VNET_DEV_OK)
    {
      err = vnet_dev_port_err (vm, p, rv,
  ...
  if ((rv = vnet_dev_process_port_cfg_change_req (vm, p, &req)) != VNET_DEV_OK)
    {
      err = vnet_dev_port_err (
	vm, p, rv, "device failed to add/remove secondary hw address");
      goto done;
    }

done:
  vec_free (req.new.secondary_hw_addr_to_add);
  return err;
}
