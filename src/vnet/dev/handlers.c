			  ...
			  const u8 *new)
{
  ...
  log_warn (p->dev, "unsupported request for port_mac_change received");
  vnet_dev_rv_t rv;
  ...
  clib_memcpy (req.new.hw_addr.addr, new, 6);

  log_debug (p->dev, "port_mac_change: %U", format_vnet_dev_hw_addr,
	     &req.new.hw_addr.addr);
...
{
  vnet_dev_port_t *p = vnet_dev_get_port_from_dev_instance (hi->dev_instance);
  log_warn (p->dev, "unsupported request for add_del_mac_address received");
