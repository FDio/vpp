  ...
  while ((uio_entry = readdir (uio_dir)))
      ...
      while ((map_entry = readdir (map_dir)))
	{
	  ...
	  if (sscanf (map_entry->d_name, "map%d%c", &index, &extra) != 1)
	    continue;
	  snprintf (path, sizeof (path), "%s/%s/name", maps_path, map_entry->d_name);
	  if (vnet_dev_bus_platform_read_string (path, map_name, sizeof (map_name)) ||
	      strcmp (name, map_name))
	    continue;
	  snprintf (path, sizeof (path), "%s/%s/offset", maps_path, map_entry->d_name);
	  if (vnet_dev_bus_platform_read_u64 (path, &uio_offset))
	    continue;
	  snprintf (path, sizeof (path), "%s/%s/size", maps_path, map_entry->d_name);
	  if (vnet_dev_bus_platform_read_u64 (path, &uio_size))
	    continue;
