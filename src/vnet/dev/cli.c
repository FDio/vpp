...
static clib_error_t *
device_create_subif_cmd_fn (vlib_main_t *vm, unformat_input_t *input,
			    vlib_cli_command_t *cmd)
{
  vnet_dev_api_create_port_subif_args_t a = {};
  vnet_main_t *vnm = vnet_get_main ();
  vnet_dev_rv_t rv;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "%U", unformat_vnet_sw_interface, vnm,
		    &a.sup_sw_if_index))
	;
      else if (unformat (input, "sw-if-index %u", &a.sup_sw_if_index))
	;
      else if (!a.args && unformat (input, "args %v", &a.args))
	;
      else
  ...
  rv = vnet_dev_api_create_port_subif (vm, &a);

  vec_free (a.args);

  if (rv != VNET_DEV_OK)
    return clib_error_return (0, "unable to create_subif: %U",
			      format_vnet_dev_rv, rv);

  return 0;
}

VLIB_CLI_COMMAND (device_create_subif_cmd, static) = {
  .path = "device create-sub-interface",
  .short_help =
    "device create-sub-interface [<interface-name> | sw-if-index <n>] id <n> "
    "[args <sub-if-args>]",
  .function = device_create_subif_cmd_fn,
  .is_mp_safe = 1,
};

static clib_error_t *
device_remove_subif_cmd_fn (vlib_main_t *vm, unformat_input_t *input,
			    vlib_cli_command_t *cmd)
{
  vnet_dev_api_remove_port_subif_args_t a = { .sw_if_index = ~0 };
  vnet_main_t *vnm = vnet_get_main ();
  vnet_dev_rv_t rv;
  ...
  if (a.sw_if_index == ~0)
    return clib_error_return (0, "please specify existing sub-interface name");

  rv = vnet_dev_api_remove_port_subif (vm, &a);

  if (rv != VNET_DEV_OK)
    return clib_error_return (0, "unable to remove sub-interface: %U",
			      format_vnet_dev_rv, rv);

  return 0;
}

VLIB_CLI_COMMAND (device_remove_subif_cmd, static) = {
  .path = "device remove-sub-interface",
  .short_help =
    "device remove-sub-interface [<interface-name> | sw-if-index <n>]",
  .function = device_remove_subif_cmd_fn,
  .is_mp_safe = 1,
};
