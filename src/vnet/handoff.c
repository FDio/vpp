...
more:
  ...
  if (PREDICT_FALSE (node->flags & VLIB_NODE_FLAG_TRACE))
    worker_handoff_trace_frame (vm, node, bufs, thread_indices, n_comp);

  n_enq = vlib_buffer_enqueue_to_thread (vm, node, hm->frame_queue_index,
					 out_bufs, thread_indices, n_comp, 1);
  if (n_enq < n_comp)
  ...
  if (n_dropped)
    vlib_node_increment_counter (
      vm, node->node_index, WORKER_HANDOFF_ERROR_CONGESTION_DROP, n_dropped);
