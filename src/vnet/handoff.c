				  ...
				  uword * bitmap, int enable_disable)
{
  ...
  if (hm->frame_queue_index == ~0)
    {
      // If multiple worker threads will handoff to each other,the minimum value is must greatered than hm->num_workers * hm->num_workers,
      // otherwise, it will dead lock
      ...
      else if ((frame_queue_nelts >= FRAME_QUEUE_MAX_NELTS) && (frame_queue_nelts < FRAME_QUEUE_MAX_NELTS * 2))
      {
        frame_queue_nelts = FRAME_QUEUE_MAX_NELTS * 2;
      }
      else
      {
        frame_queue_nelts = FRAME_QUEUE_MAX_NELTS * 4;  // Set 256 as the maximum value for now
      }
