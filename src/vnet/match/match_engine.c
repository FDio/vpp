		  ...
		  unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	mtype_end = mtype_begin + 1;
      else if (unformat (line_input, "%U", unformat_match_semantic, &msem_begin))
	msem_end = msem_begin + 1;
      else if (unformat (line_input, "%d", &len_begin))
	len_end = len_begin + 1;
      else if (unformat (line_input, "%U", unformat_token, valid_chars, &engine))
	;
      else
...
static void
match_noop_update (match_set_t * mt, const match_set_app_t * msa)
{
}
