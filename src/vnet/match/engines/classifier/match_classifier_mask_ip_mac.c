...
static match_classifier_mask_class_t *match_classifier_mask_class_data_pool;
...
#define MATCH_CLASSIFIER_MK_IP4_TBL(_h, _s, _template, _i, _f)          \
{                                                                       \
  clib_memcpy (&(_h), (_template), sizeof ((_h)));                      \
                                                                        \
  match_classifier_mk_ip4_hdr_mask (mcmck, &_h._s.ip4, &_h._s.eh);       \
                                                                        \
  mcmc->mcmc_table_indices[_i] =                                        \
    ...
    match_classifier_mk_table ((u8*) &h,                                \
                               ...
                               table_index,                             \
                               CLASSIFY_FLAG_USE_CURR_DATA,             \
                               match_set_get_l2_offset (linkt, _f), 0); \
}

#define MATCH_CLASSIFIER_MK_IP6_TBL(_h, _s, _template, _i, _f)          \
{                                                                       \
  clib_memcpy (&(_h), (_template), sizeof ((_h)));                      \
                                                                        \
  match_classifier_mk_ip6_hdr_mask (mcmck, &_h._s.ip6, &_h._s.eh);       \
                                                                        \
  mcmc->mcmc_table_indices[_i] =                                        \
    table_index =                                                       \
			    ...
			    match_classifier_mask_class_t * mcmc)
{
  switch (mcmck->mcmck_proto)
    ...
    case ETHERNET_TYPE_ARP:
      return (match_classifier_mk_arp_tables (mcmck, table_index, flags,
					      linkt, mcmc));
    case ETHERNET_TYPE_IP4:
      return (match_classifier_mk_ip4_tables (mcmck, table_index, flags,
					      linkt, mcmc));
    case ETHERNET_TYPE_IP6:
      return (match_classifier_mk_ip6_tables (mcmck, table_index, flags,
					      linkt, mcmc));
    default:
      ASSERT (0);
      break;
...
match_classifier_rule_mk_mask (const match_rule_t * mr)
{
  ...
  switch (mr->mr_type)
    ...
    case MATCH_TYPE_MASK_SRC_IP_MAC:
      ...
      mcmck.mcmck_ip = mr->mr_mask_src_ip_mac.mmim_ip.mip_ip.len;
      break;
...
match_classifier_ctx_teardown (match_engine_classifier_t * ectx)
{
   ...
   */
  /* *INDENT-OFF* */
  hash_foreach (mcmck, mcmci, ectx->mec_hash,
  ({
    mcmc = pool_elt_at_index(match_classifier_mask_class_data_pool, mcmci);
...
const static match_engine_vft_t mc_vft_first = {
  ...
  .mev_match = match_engine_classifier_match_mask_src_ip_mac_first,
};
...
const static match_engine_vft_t mc_vft_any = {
  ...
  .mev_unapply = match_classifier_unapply_mask_src_ip_mac,
  .mev_format = format_match_classifier,
  .mev_match = match_engine_classifier_match_mask_src_ip_mac_any,
};
