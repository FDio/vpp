...
static u32 MATCH_CLASSIFIER_MASK_N_TUPLE_RANGE_MAX = 0xff;
					     ...
					     * mcmck, u8 itype, u8 icode)
{
  ...
  mcmck->mcmck_icmp_type = (ICMP_INVALID == itype ? 0 : 0xff);
  mcmck->mcmck_icmp_code = (ICMP_INVALID == icode ? 0 : 0xff);
}
				  ...
				  match_engine_classifier_app_t * app)
{
  ...
  match_classifier_match_class_from_icmp_rule (mnt, &mcmck, itype, icode);
				   ...
				   match_engine_classifier_app_t * app)
{
  ...
  else if (match_icmp_type_range_is_any (mitr))
    {
				...
				match_engine_classifier_app_t * app)
{
  ...
  match_classifier_match_class_from_l4_rule (mnt, s_port, d_port, exact,
					     &mcmck);
