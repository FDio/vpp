			       ...
			       ip_address_family_t af, match_result_t * res)
{
   ...
   */
  hash1 = hash0 = vnet_classify_hash_packet_inline (vct0, h0);
  ...
  while (vct0
      ...
      if (PREDICT_TRUE (vct0->next_table_index != ~0))
	{
      ...
      if (PREDICT_TRUE(NULL != vct1))
        {
          hash1 = vnet_classify_hash_packet_inline (vct1, h0);
          vnet_classify_prefetch_bucket (vct1, hash1);
        }
	  ...
	  if (PREDICT_TRUE (e0->next_index == ~0))
	  ...
	  else
	    {
	      ...
	      mcch0 = pool_elt_at_index (match_classifier_clash_head_pool,
					 e0->next_index);
              n_clashes = vec_len (mcch0->mcch_clashes);

	      for (cc = 0; cc < n_clashes; cc++)
	      {
		bool match;

                mcci0 = mcch0->mcch_clashes[cc];

                /* if (PREDICT_TRUE(cc < n_clashes - 2)) */
                /*   CLIB_PREFETCH(pool_elt_at_index (match_classifier_clash_pool, */
                /*                                    mcch0->mcch_clashes[cc+1]), */
                /*                 sizeof(*mcc0), LOAD); */

                mcc0 = pool_elt_at_index (match_classifier_clash_pool, mcci0);
