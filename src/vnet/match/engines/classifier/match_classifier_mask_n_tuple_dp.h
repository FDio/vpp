...
match_set_result_update_if_better (match_set_result_t * msr1,
				   const match_set_result_t * msr2)
{
  if (match_set_pos_is_better (&msr2->msr_pos, &msr1->msr_pos))
    *msr1 = *msr2;
}
					    ...
					    match_set_result_t * res)
{
  ...
  u8 ip_hdr0[VNET_CLASSIFY_VECTOR_SIZE * 5];
  ...
  res->msr_pos = MATCH_SET_POS_MISS;

  meca0 =
    pool_elt_at_index (match_engine_classifier_app_pool, app->msa_index);
  ...
  if (AF_IP6 == af)
      ...
      if (PREDICT_FALSE (ip6->protocol == IP_PROTOCOL_IPV6_FRAGMENTATION))
	{
	  ...
	  clib_memcpy_fast (ip_hdr0, h0, sizeof (*ip6));

	  /* copy next-hop proto from the frag extension header */
	  ip6 = (ip6_header_t *) ip_hdr0;
	  ...
	  clib_memcpy_fast (ip_hdr0 + sizeof (*ip6),
			    ip6_ext_next_header (ext0),
			    sizeof (ip_hdr0) - sizeof (*ip6));
	  ...
	  if (PREDICT_TRUE (e0->next_index == ~0))
	    {
	      mcs0 = pool_elt_at_index (match_classifier_session_pool,
					e0->opaque_index);
	      match_set_result_update_if_better (res, &mcs0->mcs_result);
	    }
	  else
	    {
		...
		if (match)
		  {
		     ...
		     * we're done */
		    match_set_result_update_if_better (res,
						       &mcc0->mcc_result);
		    break;
		  }
	      }
 ...
 */
static_always_inline void
match_engine_classifier_match_mask_n_tuple_ip4 (vlib_main_t * vm,
						vlib_buffer_t * b,
						const match_set_app_t * app,
						f64 now,
						match_set_result_t * res)
{
  match_engine_classifier_match_mask_n_tuple (vm, b, app, now, AF_IP4, res);
}

static_always_inline void
match_engine_classifier_match_mask_n_tuple_ip6 (vlib_main_t * vm,
						vlib_buffer_t * b,
						const match_set_app_t * app,
						f64 now,
						match_set_result_t * res)
{
  match_engine_classifier_match_mask_n_tuple (vm, b, app, now, AF_IP6, res);
}
