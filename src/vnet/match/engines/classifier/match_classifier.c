				...
				match_classifier_mask_class_data_t * mcmcd)
{
  ...
  if (flags & MATCH_SET_TAG_FLAG_2_TAG)
	...
	match_classifier_mk_table (mask,
				   ...
				   sizeof (ah), table_index,
				   match_set_get_l2_offset
				   (linkt, MATCH_SET_TAG_FLAG_2_TAG));
    }
  ...
  switch (mr->mr_type)
    ...
    case MATCH_TYPE_MASK_SRC_IP_MAC:
      mac_address_copy (&mcmc.mcmc_mac,
			&mr->mr_mask_src_ip_mac.mmim_mac.mmm_mask);
      mcmc.mcmc_ip = mr->mr_mask_src_ip_mac.mmim_ip.mip_ip.len;
      break;
  ...
  switch (mr->mr_type)
    ...
    case MATCH_TYPE_MASK_SRC_IP_MAC:
      mac_address_copy (&ah.ip4_over_ethernet[ARP_SENDER].mac,
			&mr->mr_mask_src_ip_mac.mmim_mac.mmm_mac);
      ah.ip4_over_ethernet[ARP_SENDER].ip4 =
	ip_addr_v4 (&mr->mr_mask_src_ip_mac.mmim_ip.mip_ip.addr);
      break;
    default:
      return;
    }
			     ...
			     ip4_header_t * iph, ethernet_header_t * eh)
{
  switch (mr->mr_type)
    {
    case MATCH_TYPE_MASK_SRC_IP_MAC:
      mac_address_to_bytes (&mr->mr_mask_src_ip_mac.mmim_mac.mmm_mac,
			    eh->src_address);
      iph->src_address =
	ip_addr_v4 (&mr->mr_mask_src_ip_mac.mmim_ip.mip_ip.addr);
      break;
  ...
  switch (mr->mr_type)
    ...
    case MATCH_TYPE_MASK_SRC_IP_MAC:
      mac_address_to_bytes (&mr->mr_mask_src_ip_mac.mmim_mac.mmm_mac,
			    eh->src_address);
      iph->src_address =
	ip_addr_v6 (&mr->mr_mask_src_ip_mac.mmim_ip.mip_ip.addr);
      break;
    default:
      return;
    }
...
const static match_engine_vft_t mc_vft_first = {
  ...
  .mev_match = match_engine_classifier_match_first,
};
...
const static match_engine_vft_t mc_vft_any = {
  ...
  .mev_unapply = match_classifier_unapply_mask_src_ip_mac,
  .mev_format = format_match_classifier,
  .mev_match = match_engine_classifier_match_any,
};

static clib_error_t *
match_classifier_init (vlib_main_t * vm)
{
  match_engine_register ("classifier", MATCH_TYPE_MASK_SRC_IP_MAC,
			 MATCH_SEMANTIC_ANY, 50, &mc_vft_any);
  match_engine_register ("classifier", MATCH_TYPE_MASK_SRC_IP_MAC,
