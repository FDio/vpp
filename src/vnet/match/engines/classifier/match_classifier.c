...
static u8 *
format_match_classifier_clash (u8 * s, va_list * args)
{
  match_classifier_clash_t *mcc;
  index_t mcci = va_arg (*args, index_t);
  u32 indent = va_arg (*args, u32);

  mcc = pool_elt_at_index (match_classifier_clash_pool, mcci);

  s = format (s, "%U[%d] clash: pos:[%U] res:%U",
	      format_white_space, indent, mcci,
	      format_match_set_pos, &mcc->mcc_pos,
	      format_match_result, mcc->mcc_result);
...
format_match_classifier_session (u8 * s, va_list * args)
{
  ...
  if (INDEX_INVALID != mcs->mcs_clash)
    s =
      format (s, "\n%U", format_match_classifier_clash, mcs->mcs_clash,
	      indent + 4);
...
match_classifier_mask_class_unlock (index_t * mcmci)
{
  ...
  if (0 == mcmc->mcmc_locks)
	...
	pool_elt_at_index (match_classifier_engine_pool, mcmc->mcmc_engine);
      ...
      hash_unset_mem_free (&mce->mce_masks, &mcmc->mcmc_key);
					 ...
					 mcmck, const match_set_pos_t * msp)
{
  ...
  p = hash_get_mem (mce->mce_masks, mcmck);
  ...
  if (p)
  ...
  else
    {
      ...
      hash_set_mem_alloc (&mce->mce_masks, &mcmc->mcmc_key, mcmci);
    }
...
static void
match_classifier_list_apply (match_classifier_list_t * mcl,
			     const match_set_entry_t * mse,
			     u32 index, match_classifier_engine_t * mce)
{
  /* *INDENT-OFF* */
  match_classifier_ctx_t ctx = {
    .mcc_engine = mce,
...
static index_t
match_classifier_list_insert (const match_set_entry_t * mse,
			      u32 index, match_classifier_engine_t * mce)
{
  match_classifier_list_t *mcl;
  ...
  vec_insert (mce->mce_lists, 1, index);
  mce->mce_lists[index] = mcl - match_classifier_list_pool;

  match_classifier_list_apply (mcl, mse, index, mce);

  return (mce->mce_lists[index]);
}
...
match_classifier_mk_walk_entries (const match_set_entry_t * mse,
				  u32 index, void *data)
{
  match_classifier_list_insert (mse, index, data);
...
match_classifier_clash_sort (void *v1, void *v2)
{
  ...
  if (match_set_pos_is_better (&mcc1->mcc_pos, &mcc2->mcc_pos))
    return -1;
  return 1;
}
...
static void
match_classifier_list_unapply (match_classifier_list_t * mcl)
{
  index_t *mcri;
...
match_classifier_list_destroy (index_t mcli)
{
  ...
  match_classifier_list_unapply (mcl);

  pool_put (match_classifier_list_pool, mcl);
}

static void
match_classifier_update_tables (match_classifier_engine_t * mce)
{
  /*
  ...
  if (vec_len (masks))
	...
	if (INDEX_INVALID == mcmc->mcmc_table)
	  mcmc->mcmc_table =
	...
	else
	  {
	    ...
	    vct = pool_elt_at_index (vcm->tables, mcmc->mcmc_table);

	    vct->next_table_index = next_table_index;
	  }
	next_table_index = mcmc->mcmc_table;
    ...
    {
      mce->mce_table_index = INDEX_INVALID;
    }
...
match_classifier_apply_i (match_set_t * ms, match_classifier_engine_t * mce)
{
   ...
   */
  mce->mce_masks =
    hash_create_mem (0, sizeof (match_classifier_mask_class_key_t),
		     sizeof (u32));
  ...
  match_classifier_update_tables (mce);
...
void
match_classifier_list_delete (match_set_t * ms,
			      index_t msei, const match_set_app_t * msa)
{
  ...
  pos = match_set_list_position (ms, msei);

#ifdef CLIB_DEBUG
  match_classifier_list_t *mcl;

  mcl = pool_elt_at_index (match_classifier_list_pool, mce->mce_lists[pos]);
  ASSERT (mcl->mcl_set_entry == msei);
#endif

  /* remove all its state */
  match_classifier_list_destroy (mce->mce_lists[pos]);

  /* remove from the copied set, preserving order */
  vec_delete (mce->mce_lists, 1, pos);

  /*
   * we don't need to update all the result locations on a delete,
   * they won't be absoulately correct (i.e. list 6 will think it is
   * list 7) but they will be relatively correct, which is all that
   * matters.
   */
...
void
match_classifier_list_add (match_set_t * ms,
			   index_t msei, const match_set_app_t * msa)
{
  /* nothing clever here. destroy all state and start again */
  match_classifier_engine_t *mce;
  ...
  /*
   * determine where, i.e. at what priority it has added this list
   */
  pos = match_set_list_position (ms, msei);
  ...
  if (~0 != pos)
    {
      ...
      mcli = match_classifier_list_insert (match_set_entry_get (msei),
					   pos, mce);
      match_classifier_update_tables (mce);
      match_classifier_list_add_sessions (mcli);

      /*
       * need to poke all the lists (and their sessions) after the
       * insertion point to update their list location
       */
      for (; pos < vec_len (mce->mce_lists); pos++)
	{
	  match_classifier_list_t *mcl;
	  index_t *mcri;

	  mcl = pool_elt_at_index (match_classifier_list_pool,
				   mce->mce_lists[pos]);

          /* *INDENT-OFF* */
          match_set_pos_t mpos = {
            .msp_list_index = pos,
            .msp_rule_index = ~0,
          };
          /* *INDENT-ON* */
	  ...
	  vec_foreach (mcri, mcl->mcl_rules)
	  {
	    ...
	    mpos.msp_rule_index = mcr->mcr_rule.mr_index;

	    vec_foreach (mcsi, mcr->mcr_sessions)
	    {
	      mcs = match_classifier_session_get (*mcsi);
	      mcs->mcs_pos = mpos;
	    }
	  }
	}
...
void
match_classifier_list_replace (match_set_t * ms,
			       index_t msei, const match_set_app_t * msa)
{
  /* nothing clever here. destroy all state and start again */
  match_classifier_engine_t *mce;
  ...
  if (~0 != pos)
    {
      ...
      match_classifier_list_unapply (mcl);
      match_classifier_list_apply (mcl, match_set_entry_get (msei), pos, mce);
      match_classifier_update_tables (mce);
      match_classifier_list_add_sessions (mcl - match_classifier_list_pool);
    }
}
