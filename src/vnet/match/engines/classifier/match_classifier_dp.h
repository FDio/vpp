	  ...
	  if (NULL != e0)
	    {
	      ...
	      res[0].msr_rule_index = e0->opaque_index;
	      res[0].msr_user_ctx = p[0].usr_ctx;
	    }
	  else
	    // miss
	    res[0].msr_rule_index = ~0;
	}
      else
	{
	  ...
	  res[0].msr_rule_index = mi;
	  res[0].msr_user_ctx = p[0].usr_ctx;
	}
      res[0].msr_list_index = 0;
...
static_always_inline void
match_engine_classifier_match_any (vlib_main_t * vm,
				   vlib_buffer_t ** bufs,
				   u32 n_bufs,
				   match_set_app_t * apps,
				   match_set_result_t * results)
...
static_always_inline void
match_engine_classifier_match_first (vlib_main_t * vm,
				     vlib_buffer_t ** bufs,
				     u32 n_bufs,
				     match_set_app_t * apps,
				     match_set_result_t * results)
