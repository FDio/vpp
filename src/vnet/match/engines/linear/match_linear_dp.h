			   ...
			   match_orientation_t mo, ethernet_type_t etype)
{
  ...
  if (VNET_LINK_IP4 == mel0->mel_linkt || VNET_LINK_IP6 == mel0->mel_linkt)
    {
      eh0 = vlib_buffer_get_current (b) - vnet_buffer (b)->l2.l2_len;
      ip40 = vlib_buffer_get_current (b);
      ip60 = vlib_buffer_get_current (b);
      arp0 = vlib_buffer_get_current (b);
    }
  else
    {
      eh0 = vlib_buffer_get_current (b);
      ip40 = vlib_buffer_get_current (b) + vnet_buffer (b)->l2.l2_len;
...
match_engine_linear_match_sets_rule (vlib_main_t * vm,
				     ...
				     u32 count, vlib_buffer_t * b, f64 now)
{
  ...
  FOR_EACH_MATCH_ORIENTATION (mo)
  {
	...
	if (!clib_bitmap_get (mel0->mel_bitmap[mo], mss->mss_set[mo]))
	  {
	    match_match_one (vm, b, &mel0->mel_app[mo][count], now,
			     &mel0->mel_res[mo][count]);
	    clib_bitmap_set (mel0->mel_bitmap[mo], mss->mss_set[mo], 1);
	  }
      }
    else
      mel0->mel_res[mo][count].msr_pos.msp_rule_index =
	mel0->mel_res[mo][count].msr_pos.msp_list_index = 0;
  }

  return (match_set_result_is_hit (&mel0->mel_res[MATCH_SRC][count]) &&
	  match_set_result_is_hit (&mel0->mel_res[MATCH_DST][count]));
}
...
match_engine_linear_match_mask_ip_mac_src_ip4 (vlib_main_t * vm,
					       ...
					       match_set_result_t * res)
{
  match_engine_linear_match (vm, buf, app, now, res,
