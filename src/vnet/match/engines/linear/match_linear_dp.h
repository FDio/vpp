  ...
  while (n_left > 0)
    {
      ...
      mel0 = pool_elt_at_index (match_engine_linear_pool, app[0]);
      ms0 = match_set_get (mel0->mel_set);

      eh0 = vlib_buffer_get_current (b[0]) + mel0->mel_offset;

      if (VNET_LINK_IP4 == mel0->mel_linkt ||
	  VNET_LINK_IP6 == mel0->mel_linkt)
	{
	  ip40 = vlib_buffer_get_current (b[0]);
	  ip60 = vlib_buffer_get_current (b[0]);
	}
      else
	{
	  ip40 =
	    (ip4_header_t *) ((u8 *) eh0 + vnet_buffer (b[0])->l2.l2_len);
	...
	vec_foreach (mr0, mse0->mse_list.ml_rules)
	{
	  ...
	  if (match0)
	    {
	      res[0].msr_index = mr0->mr_index;
	      res[0].msr_user_ctx = mse0->mse_usr_ctxt;
	      goto next;
	    }
	}
      }
      res[0].msr_index = MATCH_RESULT_MISS;
