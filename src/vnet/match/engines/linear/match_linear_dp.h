			   ...
			   match_orientation_t mo, ethernet_type_t etype)
{
  ...
  match0 = false;
  n_rules = vec_len (mel0->mel_list.ml_rules);

  /* match against each rule in turn */
  for (mri0 = 0; mri0 < n_rules; mri0++)
    {
      mr0 = &mel0->mel_list.ml_rules[mri0];

      if (mri0 < (n_rules - 2))
	CLIB_PREFETCH (&mel0->mel_list.ml_rules[mri0 + 1],
		       sizeof (match_rule_t), LOAD);
...
match_engine_linear_match_sets_rule (vlib_main_t * vm,
				     match_engine_linear_set_t * mels,
				     ...
				     i16 l2_offset, i16 l3_offset, f64 now)
{
  ...
  {
    if (match_set_app_is_valid (&mels->mels_app[mo]))
      {
	if (!clib_bitmap_get (ptd->melptd_bitmap[mo], mels->mels_set[mo]))
	  {
	    ptd->melptd_match[mo][count] =
	      match_match_one (vm, b, l2_offset, l3_offset,
			       &mels->mels_app[mo], now,
			       &ptd->melptd_res[mo][count]);
	    clib_bitmap_set (ptd->melptd_bitmap[mo], mels->mels_set[mo], 1);
	  }
      }
    else
      ptd->melptd_match[mo][count] = true;
  }

  return (ptd->melptd_match[MATCH_SRC][count] &&
	  ptd->melptd_match[MATCH_DST][count]);
}
				...
				f64 now, match_result_t * res)
{
  ...
  /* get the table to search in */
  mel0 = pool_elt_at_index (match_engine_linear_pool, app->msa_index);
  ptd = &match_engine_linear_per_thread[vm->thread_index];

  clib_bitmap_zero (ptd->melptd_bitmap[MATCH_SRC]);
  clib_bitmap_zero (ptd->melptd_bitmap[MATCH_DST]);
  n_rules = vec_len (mel0->mel_sets);

  /* walk each entry */
  for (ii = 0; ii < n_rules; ii++)
    {
      match0 = match_engine_linear_match_sets_rule (vm, &mel0->mel_sets[ii],
						    ptd, ii, b, l2_offset,
						    l3_offset, now);

      if (match0)
	{
	  *res = mel0->mel_sets[ii].mels_res;
	  return (match0);
	}
    }
