...
{
  if (AF_IP6 == ip_addr_version (&mr->mr_mask_src_ip_mac.mmim_ip.mip_ip.addr))
    return (false);

  if (!match_mac_mask (&mr->mr_mask_src_ip_mac.mmim_mac, eh->src_address))
    return (false);

  return (match_ip4_prefix
	  (&mr->mr_mask_src_ip_mac.mmim_ip, &ip->src_address));
}
...
match_engine_linear_mask_src_ip6_mac (const ethernet_header_t * eh,
				      ...
				      const match_rule_t * mr)
{
  if (AF_IP4 == ip_addr_version (&mr->mr_mask_src_ip_mac.mmim_ip.mip_ip.addr))
    return (false);

  if (!match_mac_mask (&mr->mr_mask_src_ip_mac.mmim_mac, eh->src_address))
    return (false);

  return (match_ip6_prefix
	  (&mr->mr_mask_src_ip_mac.mmim_ip, &ip->src_address));
}

static_always_inline bool
match_engine_linear_mask_n_tuple_match_l4 (const match_mask_n_tuple_t * mnt,
					   const void *l4)
{
...
static_always_inline bool
match_engine_linear_ip4_mask_n_tuple (const ip4_header_t * ip,
				      const match_rule_t * mr)
{
  ...
  return (match_engine_linear_mask_n_tuple_match_l4 (mnt, ip + 1));
}

static_always_inline bool
match_engine_linear_ip6_mask_n_tuple (const ip6_header_t * ip,
				      const match_rule_t * mr)
{
  const match_mask_n_tuple_t *mnt;
...
match_engine_linear_match_one (vlib_main_t * vm,
			       vlib_buffer_t * b,
			       match_set_app_t app,
			       match_set_result_t * res, match_type_t mtype)
{
  ...
  mel0 = pool_elt_at_index (match_engine_linear_pool, app);
  ms0 = match_set_get (mel0->mel_set);

  eh0 = vlib_buffer_get_current (b) + mel0->mel_offset;
  ...
  vec_foreach (msei0, ms0->ms_entries)
  {
    ...
    vec_foreach (mr0, mse0->mse_list.ml_rules)
    {
      switch (mtype)
	{
	...
	case MATCH_TYPE_MASK_N_TUPLE:
	  match0 = match_engine_linear_ip4_mask_n_tuple (ip40, mr0);
	  match0 |= match_engine_linear_ip6_mask_n_tuple (ip60, mr0);
	}

      if (match0)
	{
	  res->msr_list_index = msei0 - ms0->ms_entries;
	  res->msr_rule_index = mr0->mr_index;
	  res->msr_user_ctx = mse0->mse_usr_ctxt;
	  return;
	}
    }
  }
  res->msr_rule_index = MATCH_RESULT_MISS;
}
...
match_engine_linear_match (vlib_main_t * vm,
			   ...
			   match_set_result_t * results, match_type_t mtype)
{
  ...
  while (n_left > 0)
    {
      match_engine_linear_match_one (vm, b[0], app[0], &res[0], mtype);
...
{                                                                       \
  match_engine_linear_match(vm, bufs, n_bufs, apps, results,            \
                            MATCH_TYPE_##a);                            \
}                                                                       \
static_always_inline void                                               \
match_engine_linear_match_one_##a (vlib_main_t * vm,                    \
                                  vlib_buffer_t *buf,                   \
                                  match_set_app_t app,                  \
                                  match_set_result_t * result)          \
{                                                                       \
  match_engine_linear_match_one(vm, buf, app, result, MATCH_TYPE_##a);  \
}
foreach_match_type
#undef _
#endif
