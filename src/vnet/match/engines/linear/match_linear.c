 ...
 * list sizes with this list engines, so it's not an imperative.
 */
static void
...
format_match_linear (u8 * s, va_list * args)
{
  ...
  s = format (s, "%U[%d] linear: ", format_white_space, indent, meli);

  s = format (s, "%U", format_match_list, &mel->mel_list, indent + 2);
  return (s);
}
