...
unformat_match_rule (unformat_input_t * input, va_list * args)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	found = 1;
      else if (unformat (input, "src-ip %U", unformat_ip_prefix, &src_ip))
	{
	  switch (mr->mr_type)
	    {
	    case MATCH_TYPE_MASK_N_TUPLE:
	      mr->mr_mask_n_tuple.mnt_src_ip.mip_ip = src_ip;
	      ip4_preflen_to_mask (mr->mr_mask_n_tuple.mnt_src_ip.mip_ip.len,
				   &ip_addr_v4 (&mr->
						mr_mask_n_tuple.mnt_src_ip.
						mip_mask));
	      break;
	    case MATCH_TYPE_MASK_SRC_IP_MAC:
	      mr->mr_mask_ip_mac.mmim_ip.mip_ip = src_ip;
	      ip4_preflen_to_mask (mr->mr_mask_ip_mac.mmim_ip.mip_ip.len,
				   &ip_addr_v4 (&mr->mr_mask_ip_mac.
						mmim_ip.mip_mask));
	    }
	}
      else if (unformat (input, "dst-ip %U", unformat_ip_prefix, &dst_ip))
	{
	  mr->mr_mask_n_tuple.mnt_dst_ip.mip_ip = dst_ip;
	  ip4_preflen_to_mask (mr->mr_mask_n_tuple.mnt_dst_ip.mip_ip.len,
			       &ip_addr_v4 (&mr->mr_mask_n_tuple.
					    mnt_dst_ip.mip_mask));
	}
