...
format_match_ip_prefix (u8 * s, va_list * args)
{
  ...
  s = format (s, "%U/%d", format_ip_prefix, &mip->mip_ip);
...
format_match_port_range (u8 * s, va_list * args)
{
  ...
  s = format (s, "%d -> %d",
	      clib_net_to_host_u16 (mpr->mpr_begin),
	      clib_net_to_host_u16 (mpr->mpr_end));

  return (s);
}

u8 *
format_match_icmp_range (u8 * s, va_list * args)
{
  const match_icmp_range_t *mir = va_arg (*args, match_icmp_range_t *);

  s = format (s, "%d -> %d", mir->mir_begin, mir->mir_end);
...
format_match_rule (u8 * s, va_list * args)
{
  ...
  switch (mr->mr_type)
    ...
    case MATCH_TYPE_MASK_N_TUPLE:
      s =
	format (s,
		"\n%U[src:%U dst:%U, %U, sport:[%U] dport:[%U] tcp-flags:[%U]",
		format_white_space, indent + 2, format_match_ip_prefix,
		&mr->mr_mask_n_tuple.mnt_src_ip, format_match_ip_prefix,
		&mr->mr_mask_n_tuple.mnt_dst_ip, format_ip_protocol,
		mr->mr_mask_n_tuple.mnt_ip_proto, format_match_port_range,
		&mr->mr_mask_n_tuple.mnt_src_port, format_match_port_range,
		&mr->mr_mask_n_tuple.mnt_dst_port, format_match_tcp_flags,
		&mr->mr_mask_n_tuple.mnt_tcp);
      break;
    }
  return (s);
}
