  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (input, "src-ip %U", unformat_ip_prefix, &src_ip))
	{
	  switch (mr->mr_type)
	    {
	    case MATCH_TYPE_MASK_N_TUPLE:
	      match_ip_prefix_set (&mr->mr_mask_n_tuple.mnt_src_ip, &src_ip);
	      break;
	    case MATCH_TYPE_MASK_SRC_IP_MAC:
	      match_ip_prefix_set (&mr->mr_mask_ip_mac.mmim_ip, &src_ip);
	      break;
	    }
	}
      else if (unformat (input, "dst-ip %U", unformat_ip_prefix, &dst_ip))
	match_ip_prefix_set (&mr->mr_mask_n_tuple.mnt_dst_ip, &dst_ip);
