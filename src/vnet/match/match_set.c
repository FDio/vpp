...
match_set_engine_list_action (match_set_t * ms,
			      index_t msei, match_engine_list_action_t action)
{
  match_semantic_t msem;
  ...
  FOR_EACH_MATCH_SEMANTIC (msem)
  {
	...
	if (me->mev_apply == msar->msar_engine->mev_apply)
	  msar->msar_engine->mev_list_actions[action] (ms, msei, msa);
	else
	  {
			...
			const match_list_t * ml, u16 priority)
{
  ...
  match_set_engine_list_action (ms, mh, MATCH_ENGINE_LIST_REPLACE);
...
match_set_list_add (index_t msi, const match_list_t * ml, u16 priority)
{
  ...
  match_set_engine_list_action (ms, msei, MATCH_ENGINE_LIST_ADD);
...
match_set_list_del (index_t msi, match_handle_t * mh)
{
  ...
  if (~0 != pos)
    {
      match_set_engine_list_action (ms, *mh, MATCH_ENGINE_LIST_DELETE);
