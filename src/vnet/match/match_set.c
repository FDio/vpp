...
static match_set_t *match_set_pool;
static match_set_entry_t *match_set_entry_pool;

static match_engine_vft_t match_engines[MATCH_N_SEMANTICS][MATCH_N_TYPES];
...
format_match_set (u8 * s, va_list * args)
{
  ...
  s = format (s, "%v: locks:%d", ms->ms_tag, ms->ms_locks);
  ...
  FOR_EACH_VNET_LINK (linkt)
  {
	...
	if (0 != ms->ms_apps[msem][linkt].msar_locks)
	  {
	    s = format (s, "\n  %U, %U locks:",
			format_vnet_link, linkt, format_match_semantic, msem,
			ms->ms_apps[msem][linkt].msar_locks);
	    s =
	      format (s, "\n%U",
		      match_engines[msem][ms->ms_type].mev_format,
...
match_set_update (match_set_t * ms)
{
  ...
  FOR_EACH_VNET_LINK (linkt)
  {
	...
	if (ms->ms_apps[msem][linkt].msar_locks)
	  {
	    match_engines[msem][ms->ms_type].mev_update
...
static walk_rc_t
match_set_hardest_match_entry (const match_set_entry_t * mse, void *data)
{
...
void
match_engine_register (match_type_t type,
		       match_semantic_t sem, const match_engine_vft_t * vft)
{
  match_engines[sem][type] = *vft;
}
		 ...
		 vnet_link_t linkt, match_set_tag_flags_t flags)
{
  ...
  if (0 == ms->ms_apps[sem][linkt].msar_locks)
	...
	match_engines[sem][ms->ms_type].mev_apply (ms, linkt, flags);
  ...
  FOR_EACH_VNET_LINK (linkt)
  {
	...
	if (msb == ms->ms_apps[msem][linkt].msar_app &&
	    ms->ms_apps[msem][linkt].msar_locks)
	  {
	    ...
	    if (0 == ms->ms_apps[msem][linkt].msar_locks)
	      {
		match_engines[msem][ms->ms_type].mev_unapply
		  (ms, ms->ms_apps[msem][linkt].msar_app);
