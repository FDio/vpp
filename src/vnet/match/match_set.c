...
{
  return (msa->msa_index != INDEX_INVALID);
}
...
format_match_set (u8 * s, va_list * args)
{
  ...
  FOR_EACH_VNET_LINK (linkt)
  {
    ...
    {
      match_set_app_ref_t *msar = &ms->ms_apps[msem][linkt];

      if (0 != msar->msar_locks)
	{
	  s = format (s, "\n   %U, %U locks:%d",
		      format_vnet_link, linkt,
		      format_match_semantic, msem, msar->msar_locks);
	  s =
	    format (s, "\n%U",
...
index_t
match_set_create_and_lock (const u8 * name, match_type_t rtype, void *heap)
{
  match_set_t *ms;
...
match_set_update (match_set_t * ms)
{
  ...
  FOR_EACH_VNET_LINK (linkt)
  {
      ...
      if (ms->ms_apps[msem][linkt].msar_locks)
	{
	  ...
	  msar = &ms->ms_apps[msem][linkt];
	  msa = &ms->ms_apps[msem][linkt].msar_app;
	  ...
	  if (me->mev_apply == msar->msar_engine->mev_apply)
	  ...
	  else
	    {
	      ...
	      msar->msar_engine = me;
	      msar->msar_engine->mev_apply (ms, msem, linkt, msar->msar_flags,
					    msa);
	    }
	}
    }
  ...
  if (0 == msar->msar_locks)
    {
      ...
      msar->msar_engine->mev_apply (ms, msem, linkt, flags, msa);
