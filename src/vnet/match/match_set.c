...
static u32
match_set_list_position (const match_set_t * ms, index_t msei)
{
  u32 pos;

  vec_foreach_index (pos, ms->ms_entries)
  {
    if (msei == ms->ms_entries[pos])
      return (pos);
  }
...
static void
match_set_engine_list_action (match_set_t * ms,
			      index_t msei,
			      u32 pos, match_engine_list_action_t action)
{
  match_semantic_t msem;
  ...
  FOR_EACH_MATCH_SEMANTIC (msem)
  {
	...
	/*
	 * if the engine has changed as a result of the update
	 * unapply with the old and apply with the new, else update
	 */
	if (me->mev_apply == msar->msar_engine->mev_apply)
	  msar->msar_engine->mev_list_actions[action] (ms, msei, pos, msa);
	else
	  {
...
static void
match_set_entry_update (match_set_t * ms,
			match_set_entry_t * mse, const match_list_t * ml)
{
  u32 i;
		      ...
		      const match_list_t * ml, u16 priority)
{
  ...
  match_set_entry_update (ms, mse, ml);
}

void
match_set_list_replace (index_t msi,
			match_handle_t mh, const match_list_t * ml)
{
  match_set_entry_t *mse;
  match_set_t *ms;
  void *heap;
  ...
  match_set_engine_list_action (ms, mh,
				match_set_list_position (ms, mh),
...
match_set_list_add (index_t msi, const match_list_t * ml, u16 priority)
{
  ...
  msei = match_set_entry_get_index (mse);
  vec_add1 (ms->ms_entries, msei);
  vec_sort_with_function (ms->ms_entries, match_set_entry_cmp);

  match_set_engine_list_action (ms, msei,
				match_set_list_position (ms, msei),
...
match_set_list_del (index_t msi, match_handle_t * mh)
{
  ...
  pos = vec_search (ms->ms_entries, *mh);

  if (~0 != pos)
    {
      vec_delete (ms->ms_entries, 1, pos);

      match_set_engine_list_action (ms, *mh, pos, MATCH_ENGINE_LIST_DELETE);
