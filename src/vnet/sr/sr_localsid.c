  ...
  ip46_address_t *nh_addr, void *ls_plugin_mem)
{
  ...
  if (p)
  {
    if (is_del)
    {
      ...
      if(ls->behavior == SR_BEHAVIOR_X || ls->behavior == SR_BEHAVIOR_DX6 || ls->behavior == SR_BEHAVIOR_DX4)
  ...
  fib_table_entry_special_dpo_add (
    fib_index, &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
  dpo_reset (&dpo);
    ...
    vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
  {
    if (unformat (input, "del"))
      is_del = 1;
        ...
        &resulting_address))
      address_set = 1;
    else if (!address_set && unformat (input, "addr %U", unformat_ip6_address, 
        &resulting_address))
      address_set = 1;
    ...
    else if (!end_psp && unformat (input, "end.psp"))
      end_psp = 1;
    else 
      break;
  }
  ...
  if (vlan_index != (u32)~0)
    return clib_error_return (0, "Error: SRv6 End.DX2 with rewrite VLAN tag not supported by now.");
  if (end_psp && !(behavior == SR_BEHAVIOR_END || behavior == SR_BEHAVIOR_X ))
    return clib_error_return (0, "Error: SRv6 End.PSP only compatible with End and End.X");
  ...
  vlib_cli_command_t * cmd)
{
  ...
  for (i = 0; i < vec_len (localsid_list); i++)
  {
    ...
    switch (ls->behavior)
    {
      ...
      case SR_BEHAVIOR_X:
        vlib_cli_output (vm,"\tAddress: \t%U\n\tBehavior: \tX (Endpoint with Layer-3 cross-connect)"
          "\n\tIface:  \t%U\n\tNext hop: \t%U",
          format_ip6_address, &ls->localsid, format_vnet_sw_if_index_name, vnm, 
          ls->sw_if_index, format_ip6_address, &ls->next_hop.ip6);
        break;
      case SR_BEHAVIOR_DX4:
        vlib_cli_output (vm,"\tAddress: \t%U\n\tBehavior: \tDX4 (Endpoint with decapsulation and IPv4 cross-connect)"
          "\n\tIface:  \t%U\n\tNext hop: \t%U",
          format_ip6_address, &ls->localsid, format_vnet_sw_if_index_name, vnm, 
          ls->sw_if_index, format_ip4_address, &ls->next_hop.ip4);
        break;
      case SR_BEHAVIOR_DX6:
      ...
      case SR_BEHAVIOR_DT6:
        vlib_cli_output (vm,"\tAddress: \t%U\n\tBehavior: \tDT6 (Endpoint with decapsulation and specific IPv6 table lookup)"
          "\n\tTable: %u",
          format_ip6_address, &ls->localsid, ls->fib_table);
        break;
    ...
    if(ls->end_psp)
      vlib_cli_output(vm, "\tEnd.PSP: \tTrue\n");
 ...
 */
static clib_error_t * 
clear_sr_localsid_counters_command_fn (vlib_main_t * vm, unformat_input_t * input, 
  vlib_cli_command_t * cmd)
{
  ip6_sr_main_t * sm = &sr_main;
                        ...
                        u32 * next0)
{
  ...
  if(PREDICT_TRUE(sr0->type == ROUTING_HEADER_TYPE_SR))
  {
    if(PREDICT_TRUE(sr0->segments_left == 1))
    {
      ...
      clib_memcpy((void *)ip0 + sr_len, ip0, (void *)sr0-(void *)ip0);
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 8 && n_left_to_next >= 4)
    {
      ...
      vlib_increment_combined_counter
        (((next0 == SR_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) : &(sm->sr_ls_valid_counters)),
        cpu_index,
        ls0 - sm->localsids,
        1, vlib_buffer_length_in_chain (vm, b0));

      vlib_increment_combined_counter
        (((next1 == SR_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) : &(sm->sr_ls_valid_counters)),
        cpu_index,
        ls1 - sm->localsids,
        1, vlib_buffer_length_in_chain (vm, b1));

      vlib_increment_combined_counter
        (((next2 == SR_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) : &(sm->sr_ls_valid_counters)),
        cpu_index,
        ls2 - sm->localsids,
        1, vlib_buffer_length_in_chain (vm, b2));

      vlib_increment_combined_counter
        (((next3 == SR_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) : &(sm->sr_ls_valid_counters)),
        cpu_index,
        ls3 - sm->localsids,
...
sr_localsid_register_function (vlib_main_t * vm, u8 *fn_name, 
  u8 *keyword_str, u8 *def_str, u8 *params_str, dpo_type_t *dpo,
 ...
 */
static clib_error_t * 
show_sr_localsid_behaviors_command_fn (vlib_main_t * vm, unformat_input_t * input, 
  vlib_cli_command_t * cmd)
{
  ip6_sr_main_t * sm = &sr_main;
  sr_localsid_fn_registration_t *plugin;
