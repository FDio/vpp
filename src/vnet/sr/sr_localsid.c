  ...
  ip46_address_t *nh_addr, void *ls_plugin_mem)
{
  ...
  if (p)
  {
    if (is_del)
    {
      ...
      if(ls->behavior == SR_BEHAVIOR_DX6 || ls->behavior == SR_BEHAVIOR_DX4)
    ...
    vlib_cli_command_t * cmd)
{
  ...
  if (behavior == SR_BEHAVIOR_DX2 && is_decap)
  ...
  rv = sr_cli_localsid (is_del, &resulting_address, is_decap, behavior, 
...
_(NO_USP, "(SR-Error) USP Not available (segments left > 0)")       \
 ...
 */
/* Fixme: to be fully compliant we need to check SL value */
static_always_inline void
...
/**
 * @brief Function doing End processing with USP 
 */
//Fixme: support OAM (hop-by-hop header) here!
static_always_inline void
end_usp_srh_processing (vlib_node_runtime_t * node,
                        vlib_buffer_t * b0,
                        ip6_header_t * ip0,
                        ip6_sr_header_t * sr0,
                        u32 * next0)
{
  ...
  if(PREDICT_TRUE(ip0->protocol == IP_PROTOCOL_IPV6_ROUTE))
  {
    if(PREDICT_TRUE(sr0->type == ROUTING_HEADER_TYPE_SR))
    {
      if(PREDICT_TRUE(sr0->segments_left == 1))
      {
        ...
        /* Assuming there is no OAM header.... :( */
        copy_src0 = (u64 *)ip0;
  ...
  /* Error. Routing header of type != SR */
  *next0 = SR_LOCALSID_NEXT_ERROR;
  b0->error = node->errors[SR_LOCALSID_ERROR_NO_USP];
}
  ...
  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 8 && n_left_to_next >= 4)
    {
      ...
      end_srh_processing ( node, b1, ip1, sr1, &next1);
      end_srh_processing ( node, b2, ip2, sr2, &next2);
      end_srh_processing ( node, b3, ip3, sr3, &next3);
 ...
 */
int
sr_localsid_register_function (vlib_main_t * vm, u8 *fn_name, dpo_type_t *dpo,
