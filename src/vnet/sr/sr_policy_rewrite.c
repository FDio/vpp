  ...
  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 8 && n_left_to_next >= 4)
    {
      ...
      new_l1 = ip1->payload_length + clib_net_to_host_u16(ip1_encap->length);
      new_l2 = ip2->payload_length + clib_net_to_host_u16(ip2_encap->length);
      new_l3 = ip3->payload_length + clib_net_to_host_u16(ip3_encap->length);
...
VLIB_REGISTER_NODE (sr_policy_rewrite_encaps_v4_node) = {
  ...
  .name = "sr-policy-rewrite-encaps-v4",
  .vector_size = sizeof (u32),
  .format_trace = format_sr_policy_rewrite_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = SR_POLICY_REWRITE_N_ERROR,
  ...
  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 8 && n_left_to_next >= 4)
    {
      ...
      new_l0 = ip0->payload_length + b0->current_length;
      new_l1 = ip1->payload_length + b1->current_length;
      new_l2 = ip2->payload_length + b2->current_length;
      new_l3 = ip3->payload_length + b3->current_length;
      ...
      /* TODO: Update next header value to 59 */
...
VLIB_REGISTER_NODE (sr_policy_rewrite_encaps_l2_node) = {
  ...
  .name = "sr-policy-rewrite-encaps-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_sr_policy_rewrite_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = SR_POLICY_REWRITE_N_ERROR,
