...
create_sl (ip6_sr_policy_t *sr_policy, ip6_address_t *sl, u32 weight, u8 is_encap)
{
  ...
  if (is_encap)
  {
    ...
    dpo_set ( &segment_list->ip4_dpo, sr_pr_encaps_dpo_type, DPO_PROTO_IP4, 
            segment_list - sm->sid_lists );
    dpo_set ( &segment_list->bsid_dpo, sr_pr_bsid_encaps_dpo_type, DPO_PROTO_IP6,
          segment_list - sm->sid_lists );
  }
  else
  {
    dpo_set ( &segment_list->ip6_dpo, sr_pr_insert_dpo_type, DPO_PROTO_IP6, 
            segment_list - sm->sid_lists );
    dpo_set ( &segment_list->bsid_dpo, sr_pr_bsid_insert_dpo_type, DPO_PROTO_IP6,
          segment_list - sm->sid_lists );
  }
              ...
              u8 operation, ip6_address_t *segments, u32 sl_index, u32 weight)
{
  ...
  {
    /* Create the new SL */
    segment_list = create_sl (sr_policy, segments, weight, sr_policy->is_encap);
  ...
  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 8 && n_left_to_next >= 4)
    {
      ...
      clib_memcpy (((u8 *)ip0_encap) - vec_len(sl0->rewrite), sl0->rewrite, vec_len(sl0->rewrite));
      clib_memcpy (((u8 *)ip1_encap) - vec_len(sl1->rewrite), sl1->rewrite, vec_len(sl1->rewrite));
      clib_memcpy (((u8 *)ip2_encap) - vec_len(sl2->rewrite), sl2->rewrite, vec_len(sl2->rewrite));
      clib_memcpy (((u8 *)ip3_encap) - vec_len(sl3->rewrite), sl3->rewrite, vec_len(sl3->rewrite));
                      ...
                      ip4_header_t * ip0_encap)
{
  ...
  ip0->ip_version_traffic_class_and_flow_label = 
    clib_host_to_net_u32(0|((6&0xF)<<28)| ((ip0_encap->tos&0xFF)<<20));
  sr0 = (void *)(ip0+1);
    ...
    while (n_left_from >= 8 && n_left_to_next >= 4)
    {
      ...
      else
      {
        ...
        vnet_buffer(b0)->ip.adj_index[VLIB_TX] = 
          sp0->segments_lists[(vnet_buffer(b0)->ip.flow_hash & (vec_len(sp0->segments_lists)-1))];
      }
      ...
      else
      {
        ...
        vnet_buffer(b1)->ip.adj_index[VLIB_TX] = 
          sp1->segments_lists[(vnet_buffer(b1)->ip.flow_hash & (vec_len(sp1->segments_lists)-1))];
      }
      ...
      else
      {
        ...
        vnet_buffer(b2)->ip.adj_index[VLIB_TX] = 
          sp2->segments_lists[(vnet_buffer(b2)->ip.flow_hash & (vec_len(sp2->segments_lists)-1))];
      }
      ...
      else
      {
        ...
        vnet_buffer(b3)->ip.adj_index[VLIB_TX] = 
          sp3->segments_lists[(vnet_buffer(b3)->ip.flow_hash & (vec_len(sp3->segments_lists)-1))];
      }
      ...
      clib_memcpy (((u8 *)en0) - vec_len(sl0->rewrite), sl0->rewrite, vec_len(sl0->rewrite));
      clib_memcpy (((u8 *)en1) - vec_len(sl1->rewrite), sl1->rewrite, vec_len(sl1->rewrite));
      clib_memcpy (((u8 *)en2) - vec_len(sl2->rewrite), sl2->rewrite, vec_len(sl2->rewrite));
      clib_memcpy (((u8 *)en3) - vec_len(sl3->rewrite), sl3->rewrite, vec_len(sl3->rewrite));
      ...
      sr0->protocol = sr1->protocol = sr2->protocol = sr3->protocol = IP_PROTOCOL_IP6_NONXT;
  ...
  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 8 && n_left_to_next >= 4)
    {
      ...
      clib_memcpy ((u8 *)ip0-vec_len(sl0->rewrite), (u8 *)ip0, (void *)sr0-(void *)ip0);
      clib_memcpy ((u8 *)ip1-vec_len(sl1->rewrite), (u8 *)ip1, (void *)sr1-(void *)ip1);
      clib_memcpy ((u8 *)ip2-vec_len(sl2->rewrite), (u8 *)ip2, (void *)sr2-(void *)ip2);
      clib_memcpy ((u8 *)ip3-vec_len(sl3->rewrite), (u8 *)ip3, (void *)sr3-(void *)ip3);

      clib_memcpy (((u8 *)sr0-vec_len(sl0->rewrite)), sl0->rewrite, vec_len(sl0->rewrite));
      clib_memcpy (((u8 *)sr1-vec_len(sl1->rewrite)), sl1->rewrite, vec_len(sl1->rewrite));
      clib_memcpy (((u8 *)sr2-vec_len(sl2->rewrite)), sl2->rewrite, vec_len(sl2->rewrite));
      clib_memcpy (((u8 *)sr3-vec_len(sl3->rewrite)), sl3->rewrite, vec_len(sl3->rewrite));
  ...
  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 8 && n_left_to_next >= 4)
    {
      ...
      ASSERT (VLIB_BUFFER_PRE_DATA_SIZE >= (vec_len(sl0->rewrite_bsid) + b0->current_data));
      ASSERT (VLIB_BUFFER_PRE_DATA_SIZE >= (vec_len(sl1->rewrite_bsid) + b1->current_data));
      ASSERT (VLIB_BUFFER_PRE_DATA_SIZE >= (vec_len(sl2->rewrite_bsid) + b2->current_data));
      ASSERT (VLIB_BUFFER_PRE_DATA_SIZE >= (vec_len(sl3->rewrite_bsid) + b3->current_data));
      ...
      clib_memcpy ((u8 *)ip0-vec_len(sl0->rewrite_bsid), (u8 *)ip0, (void *)sr0-(void *)ip0);
      clib_memcpy ((u8 *)ip1-vec_len(sl1->rewrite_bsid), (u8 *)ip1, (void *)sr1-(void *)ip1);
      clib_memcpy ((u8 *)ip2-vec_len(sl2->rewrite_bsid), (u8 *)ip2, (void *)sr2-(void *)ip2);
      clib_memcpy ((u8 *)ip3-vec_len(sl3->rewrite_bsid), (u8 *)ip3, (void *)sr3-(void *)ip3);

      clib_memcpy (((u8 *)sr0-vec_len(sl0->rewrite_bsid)), sl0->rewrite_bsid, vec_len(sl0->rewrite_bsid));
      clib_memcpy (((u8 *)sr1-vec_len(sl1->rewrite_bsid)), sl1->rewrite_bsid, vec_len(sl1->rewrite_bsid));
      clib_memcpy (((u8 *)sr2-vec_len(sl2->rewrite_bsid)), sl2->rewrite_bsid, vec_len(sl2->rewrite_bsid));
      clib_memcpy (((u8 *)sr3-vec_len(sl3->rewrite_bsid)), sl3->rewrite_bsid, vec_len(sl3->rewrite_bsid));
  ...
  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from >= 8 && n_left_to_next >= 4)
    {
      ...
      ip6_ext_header_find_t (ip0_encap, prev0, sr0, IP_PROTOCOL_IPV6_ROUTE);
      ip6_ext_header_find_t (ip1_encap, prev1, sr1, IP_PROTOCOL_IPV6_ROUTE);
      ip6_ext_header_find_t (ip2_encap, prev2, sr2, IP_PROTOCOL_IPV6_ROUTE);
      ip6_ext_header_find_t (ip3_encap, prev3, sr3, IP_PROTOCOL_IPV6_ROUTE);
