...
/** \brief Create multiple clones of buffer and store them in the supplied array
    ...
    @param head_size - (u16) size of packet (not-cloned area)
    @param buffers - (u32 * ) buffer index array
    @param n_buffers - (u32) number of buffers requested
    @return - (u32) number of buffers actually cloned, may be
    less than the number requested or zero
*/
always_inline u32
vlib_buffer_clone (vlib_main_t * vm, u32 src_buffer, u16 head_size,
		      ...
		      vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  u32 num_cloned =
	    vlib_buffer_clone (vm, bi0, 128, clones, num_replicas - 1);
	  clones[num_replicas - 1] = bi0;

	  if (PREDICT_FALSE (num_replicas != num_cloned + 1))
	    {
