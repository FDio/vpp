/*
 * sr_vpn.c: ipv6 segment routing vpn services
 *
 ...
 */
int 
sr_steering_policy(vlib_main_t * vm, int is_del, ip6_address_t *bsid, 
  u32 sr_policy_index, u32 table_id, ip46_address_t *prefix, u32 mask_width,
  u32 sw_if_index, u8 traffic_type)
{
    ...
    if (is_del)
    {
      if(steer_pl->classify.traffic_type == SR_STEER_IPV6)
      {
        ...
        fib_table_entry_delete (
          fib_table_id_find_fib_index ( FIB_PROTOCOL_IP6, 
      ...
      else if(steer_pl->classify.traffic_type == SR_STEER_IPV4)
      {
        ...
        fib_table_entry_delete (
          fib_table_id_find_fib_index ( FIB_PROTOCOL_IP4, 
                                        steer_pl->classify.l3.fib_table),
          &pfx, 
          FIB_SOURCE_SR);
      }
      else if(steer_pl->classify.traffic_type == SR_STEER_L2)
      {
        ...
        vnet_feature_enable_disable ("device-input", "sr-policy-rewrite-encaps-l2",
                               sw_if_index, 0, 0, 0);
  ...
  {
    clib_memcpy (&steer_pl->classify.l3.prefix, prefix, sizeof(ip46_address_t));
  ...
  vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
  {
    ...
    else if(!sr_policy_set && unformat(input, "via sr policy index %d", &sr_policy_index))
      sr_policy_set = 1;
    else if(!sr_policy_set && unformat(input, "via sr policy bsid %U", unformat_ip6_address, &bsid))
      sr_policy_set = 1;
  ...
  rv = sr_steering_policy (vm, is_del, (sr_policy_index == ~(u32)0 ? &bsid : NULL), 
    sr_policy_index, fib_table, &prefix, dst_mask_width, sw_if_index, traffic_type);
  ...
  for (i = 0; i < vec_len (steer_policies); i++)
  {
    ...
    if(steer_pl->classify.traffic_type == SR_STEER_L2)
    {
      vlib_cli_output (vm,"L2 %U\t%U",
        format_vnet_sw_if_index_name, vnm, steer_pl->classify.l2.sw_if_index,
        format_ip6_address, &pl->bsid);
    }
    else if (steer_pl->classify.traffic_type == SR_STEER_IPV4)
    {
      vlib_cli_output (vm,"L3 %U/%d\t%U",
        format_ip4_address, &steer_pl->classify.l3.prefix.ip4,
    ...
    {
      vlib_cli_output (vm,"L3 %U/%d\t%U",
        format_ip6_address, &steer_pl->classify.l3.prefix.ip6,
        steer_pl->classify.l3.mask_width,
        format_ip6_address, &pl->bsid);
    }
  }
...
clib_error_t *sr_steering_init (vlib_main_t *vm)
{
  ...
  sm->steer_policies_index_by_key = hash_create_mem(0, sizeof(sr_steering_key_t), 
    sizeof(uword));
...
VNET_FEATURE_INIT (sr_policy_rewrite_encaps_l2, static) = 
{
  ...
  .node_name = "sr-policy-rewrite-encaps-l2",
