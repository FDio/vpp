...
static prefix_main_t prefix_main;
...
dhcp6_pd_reply_event_handler (vl_api_dhcp6_pd_reply_event_t * mp)
{
  ...
  if (mp->msg_type == DHCPV6_MSG_ADVERTISE)
    {
      ...
      if (mp->inner_status_code == DHCPV6_STATUS_NOPREFIX_AVAIL)
	{
	  clib_warning
  ...
  if (mp->status_code == DHCPV6_STATUS_UNSPEC_FAIL)
    {
...
cp_ip6_address_config_init (vlib_main_t * vm)
{
  ...
  memset (apm->active_prefix_index_by_prefix_group, ~0,
	  sizeof (apm->active_prefix_index_by_prefix_group));
...
cp_ip6_address_add_del_now (ip6_address_info_t * address_info, u8 is_add)
{
  ...
  if (address_info->prefix_group != PREFIX_GROUP_NONE)
    prefix_index =
      apm->active_prefix_index_by_prefix_group[address_info->prefix_group];
  else
    prefix_index = ~0;
  ...
  if (is_add && !address_info->configured_in_data_plane)
	...
	{
	  if (address_info->prefix_group == ~0)
	    {
	      error =
		ip6_add_del_interface_address (vm, address_info->sw_if_index,
...
cp_ip6_address_prefix_add_del_handler (u32 prefix_index, u8 is_add)
{
  ...
  prefix = &pm->prefix_pool[prefix_index];
  prefix_group = prefix->prefix_group;

  if (is_add)
    {
      if (apm->active_prefix_index_by_prefix_group[prefix->prefix_group] ==
	  ~0)
	{
	  apm->active_prefix_index_by_prefix_group[prefix->prefix_group] =
	    prefix_index;
	  for (i = 0; i < vec_len (apm->addresses); i++)
	    {
	      address_info = &apm->addresses[i];
	      if (address_info->prefix_group == prefix_group)
	  ...
	  prefix_index)
	{
	  ...
	  apm->active_prefix_index_by_prefix_group[prefix->prefix_group] = ~0;
	  new_prefix_index =
	    cp_ip6_address_find_new_active_prefix (prefix_group);
...
static int
cp_ip6_address_add_del (u32 sw_if_index, u32 prefix_group,
			ip6_address_t address, u8 prefix_length, u8 is_add)
{
  ...
  if (!(prefix_group < PREFIX_GROUP_MAX || prefix_group == PREFIX_GROUP_NONE))
  ...
  vec_foreach (address_info, apm->addresses)
  {
	...
	address_info->prefix_group == prefix_group &&
  ...
  (vl_api_ip6_add_del_address_using_prefix_t * mp)
{
  ...
  rv =
    cp_ip6_address_add_del (sw_if_index, prefix_group, address,
 ...
 * @cliexcmd{set ip6 address GigabitEthernet2/0/0 prefix group 0 ::7/64}
 * Example of how to delete IPv6 address:
 * @cliexcmd{set ip6 address GigabitEthernet2/0/0 prefix group 0 ::7/64 del}
 * @endparblock
?*/
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (ip6_address_add_del_command, static) = {
  .path = "set ip6 address",
  .short_help = "set ip6 address <interface> [prefix group <n>] "
                "<address> [del]",
  ...
  for (i = 0; i < vec_len (apm->addresses); i++)
    {
      address_info = &apm->addresses[i];
      vlib_cli_output (vm,
		       "sw_if_index: %u, prefix_group: %d, address: %U/%d",
		       address_info->sw_if_index, address_info->prefix_group,
  ...
  for (i = 0; i < vec_len (rm->client_state_by_sw_if_index); i++)
      ...
      if (cs->enabled)
	{
	  ...
	  if (cs->rebinding)
	    ...
	    rebinding = "";
	  if (cs->T1)
	    vlib_cli_output (vm, "sw_if_index: %u, T1: %u (%s), "
			     "T2: %u (%s), server index: %u%s",
