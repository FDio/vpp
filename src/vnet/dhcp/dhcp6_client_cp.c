...
send_client_message_start_stop (u32 sw_if_index, u32 server_index,
				u8 msg_type, address_info_t * address,
				u8 start)
{
  ...
  if (address)
    {
      params.include_address = 1;
      params.preferred_lt = address->preferred_lt;
      params.valid_lt = address->valid_lt;
      memcpy (&params.address, &address->address, 16);
    }
...
dhcp6_reply_event_handler (vl_api_dhcp6_reply_event_t * mp)
{
  ...
  client_config = &rm->client_state_by_sw_if_index[sw_if_index];
  ...
  if (mp->msg_type == DHCPV6_MSG_ADVERTISE)
    {
      ...
      send_client_message_start_stop (sw_if_index, server_index,
				      DHCPV6_MSG_REQUEST, &address_info, 1);
    }
  ...
  for (i = 0; i < n_addresses; i++)
	...
	{
	  f64 remaining_life_time = address_info->due_time - current_time;
	  if (valid_time > 2 * 60 * 60 || valid_time > remaining_life_time)
	    ...
	    address_info->due_time = current_time + 2 * 60 * 60;
	}
      else if (valid_time != 0)
	{
	  ...
	  address_info->due_time = due_time;

	  if (due_time > rm->max_valid_due_time)
	    rm->max_valid_due_time = due_time;
	}
	...
	&rm->client_state_by_sw_if_index[sw_if_index];
      ...
      client_state->address_info = address_info;

      clib_warning ("Interface %u obtained lease %U lifetime %u:%u",
		    address_info->sw_if_index, format_ip6_address,
		    &address_info->address, address_info->preferred_lt,
...
dhcp6_client_enable_disable (u32 sw_if_index, u8 enable)
{
  ...
  if (!old_enabled && enable)
    ...
    {
      send_client_message_start_stop (sw_if_index, ~0, ~0, NULL, 0);
      /* *INDENT-OFF* */
      pool_foreach (address_info, rm->address_pool,
      ({
        if (address_info->sw_if_index == sw_if_index)
...
#include <vnet/dhcp/dhcp6_client_cp.api.h>
...
#define _(id,n,crc) vl_msg_api_add_msg_name_crc (am, #n "_" #crc, id);
  foreach_vl_msg_name_crc_dhcp6_client_cp;
#undef _
}
