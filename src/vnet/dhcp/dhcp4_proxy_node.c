...
/* linearizes the data up to a single buffer
 * returns <0 on failure or free linear space left in buffer */
static inline int
		       ...
		       u32 ** discard_vector)
{
  ...
  return max_len - len - b->current_data;
}
			    ...
			    vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
	...
	{
	  /* linearize needed to "unclone" and scan options */
	  int space_left = dhcp_buffer_linearize (vm, b0, &to_discard);
	  if (space_left < 0)
	    {
	      error0 = DHCP_PROXY_ERROR_PKT_TOO_BIG;
	      goto drop_packet;
