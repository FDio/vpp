...
get_server_index (u8 * data, u16 len)
{
  ...
  for (i = 0; i < vec_len (server_entries); i++)
    {
      se = &server_entries[i];
      if (se->len == len && 0 == memcpy (se->data, data, len))
	return i;
    }
	  ...
	  switch (dhcpv60->msg_type)
	      ...
	      while (options_length > 0)
		{
		  ...
		  if (oo == DHCPV6_OPTION_IA_NA)
		    ...
		    {
		      report.status_code = ntohs (*(u16 *) option->data);
		    }
				  ...
				  u32 type)
{
  ...
  if (type == DHCPV6_MSG_SOLICIT)
	...
	clib_host_to_net_u16 (sizeof (*elapsed) - sizeof (elapsed->opt));
      elapsed->elapsed_10ms =
	clib_host_to_net_u16 ((u16) ((now - transaction_start) * 100));
      d += sizeof (*elapsed);

      ia_hdr = (dhcpv6_ia_header_t *) d;
      ia_hdr->opt.option = clib_host_to_net_u16 (DHCPV6_OPTION_IA_NA);
      ia_hdr->opt.length =
	clib_host_to_net_u16 (sizeof (*ia_hdr) - sizeof (ia_hdr->opt));
      ia_hdr->iaid = 1;
      ia_hdr->t1 = 0;
      ia_hdr->t2 = 0;
      d += sizeof (*ia_hdr);
	...
	clib_host_to_net_u16 (sizeof (*ia_hdr) - sizeof (ia_hdr->opt));
      ...
      if (client_state->params.include_address)
	{
	  opt_addr = (dhcpv6_ia_opt_addr_t *) d;
	  ia_hdr->opt.length =
	    clib_host_to_net_u16 (sizeof (*ia_hdr) + sizeof (*opt_addr) -
				  sizeof (ia_hdr->opt));
	  opt_addr->opt.option =
	    clib_host_to_net_u16 (DHCPV6_OPTION_IAPREFIX);
	  opt_addr->opt.length =
	    clib_host_to_net_u16 (sizeof (*opt_addr) -
				  sizeof (opt_addr->opt));
	  memcpy (&opt_addr->addr.as_u8, client_state->params.address, 16);
	  opt_addr->valid = client_state->params.valid_lt;
	  opt_addr->preferred = client_state->params.preferred_lt;
	  d += sizeof (*opt_addr);
	}
    }
  else
  ...
  (vl_api_dhcp6_send_client_message_t * mp)
{
  ...
  params.include_address = mp->include_address;
  params.preferred_lt = ntohl (mp->preferred_lt);
  params.valid_lt = ntohl (mp->valid_lt);
  memcpy (params.address, mp->address, 16);
  ...
  if (p)
      ...
      else
	{
	  ...
	  if (pool_elts (am->dhcp6_reply_events_registrations) == 0)
	    set_publisher_node (~0, REPORT_MAX);
	  goto reply;
	}
    }
