	      ...
	      while (options_length > 0)
		{
		  ...
		  if (oo == DHCPV6_OPTION_IA_PD)
		      ...
		      while (inner_options_length > 0)
			{
			  ...
			  if (inner_oo == DHCPV6_OPTION_IAPREFIX)
			    {
				  ...
				  u32 type)
{
  ...
  if (type == DHCPV6_MSG_SOLICIT || type == DHCPV6_MSG_REQUEST ||
      ...
      for (i = 0; i < n_prefixes; i++)
	{
	  ...
	  opt_pd->valid = pref->valid_lt;
	  opt_pd->preferred = pref->preferred_lt;
	  d += sizeof (*opt_pd);
	}
    }
  else
			      ...
			      dhcp6_pd_send_client_message_params_t * params)
{
  ...
  dhcp6_pd_client_state_t empty_state = { 0, };
	      ...
	      vpe_client_registration_t *reg;
              ...
              ({
                ...
                if (vl_reg && vl_api_can_send_msg (vl_reg))
                  {
                    event->_vl_msg_id = htons (VL_API_DHCP6_PD_REPLY_EVENT);
  ...
  (vl_api_want_dhcp6_pd_reply_events_t * mp)
{
  ...
  set_publisher_node (dhcp6_pd_reply_process_node.index,
