      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  d0 =
	    pool_elt_at_index (map_main.domains,
          ...
          if (icmp6_to_icmp(p0, ip6_to_ip4_set_icmp_cb, d0, ip6_to_ip4_set_inner_icmp_cb, d0))
            {
              error0 = MAP_ERROR_ICMP;
              goto err0;
            }
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  p1 = vlib_get_buffer (vm, pi1);
          ...
          if (ip6_to_ip4_tcp_udp(p0, ip6_to_ip4_set_cb, p0))
            {
          ...
          if (ip6_to_ip4_tcp_udp(p1, ip6_to_ip4_set_cb, p1))
            {
