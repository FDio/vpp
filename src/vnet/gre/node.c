  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  u32 i0, i1, next0, next1, protocol0, protocol1;
          ...
          u32 ip6_tun_src1[4], ip6_tun_dst1[4];
	  ...
	  b1 = vlib_get_buffer (vm, bi1);
          ...
          if (!is_ipv6)
          ...
          else
            {
              ...
              ip6_tun_src0[0] = ip6_0->src_address.as_u32[0];
              ip6_tun_src0[1] = ip6_0->src_address.as_u32[1];
              ip6_tun_src0[2] = ip6_0->src_address.as_u32[2];
              ip6_tun_src0[3] = ip6_0->src_address.as_u32[3];
              ip6_tun_dst0[0] = ip6_0->dst_address.as_u32[0];
              ip6_tun_dst0[1] = ip6_0->dst_address.as_u32[1];
              ip6_tun_dst0[2] = ip6_0->dst_address.as_u32[2];
              ip6_tun_dst0[3] = ip6_0->dst_address.as_u32[3];
              ip6_tun_src1[0] = ip6_1->src_address.as_u32[0];
              ip6_tun_src1[1] = ip6_1->src_address.as_u32[1];
              ip6_tun_src1[2] = ip6_1->src_address.as_u32[2];
              ip6_tun_src1[3] = ip6_1->src_address.as_u32[3];
              ip6_tun_dst1[0] = ip6_1->dst_address.as_u32[0];
              ip6_tun_dst1[1] = ip6_1->dst_address.as_u32[1];
              ip6_tun_dst1[2] = ip6_1->dst_address.as_u32[2];
              ip6_tun_dst1[3] = ip6_1->dst_address.as_u32[3];
          ...
          if (PREDICT_TRUE(next0 == GRE_INPUT_NEXT_IP4_INPUT
			   ...
			   || next0 == GRE_INPUT_NEXT_MPLS_INPUT))
            {
              ...
              if (!is_ipv6)
                ...
                key = ((u64)(ip6_tun_dst0[0]) << 32) | (u64)(ip6_tun_src0[0]);
          ...
          if (PREDICT_TRUE(next1 == GRE_INPUT_NEXT_IP4_INPUT
			   ...
			   || next1 == GRE_INPUT_NEXT_MPLS_INPUT))
            {
              ...
              if (!is_ipv6)
                ...
                key = ((u64)(ip6_tun_dst1[0]) << 32) | (u64)(ip6_tun_src1[0]);
			     ...
			     u32 node_index)
{
  ...
  u16 * n;
  u8 is_ipv6 = protocol == GRE_PROTOCOL_ip6 ? 1 : 0;
  ...
  pi->next_index = vlib_node_add_next (vm,
				       !is_ipv6 ? gre4_input_node.index : gre6_input_node.index,
				       node_index);

  /* Setup gre protocol -> next index sparse vector mapping. */
  rt = vlib_node_get_runtime_data (vm, !is_ipv6 ? gre4_input_node.index : gre6_input_node.index);
