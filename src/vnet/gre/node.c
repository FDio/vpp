...
always_inline uword
gre_tun_decap (vlib_main_t * vm,
	       vlib_node_runtime_t * node,
	       vlib_frame_t * frame, const int is_ipv6)
{
  gre_main_t *gm = &gre_main;
  u32 *from, n_left_from;
  vlib_buffer_t *bufs[VLIB_FRAME_SIZE], **b = bufs;
...
VLIB_NODE_FN (gre4_tun_decap_node) (vlib_main_t * vm,
				    ...
				    vlib_frame_t * from_frame)
{
  return gre_tun_decap (vm, node, from_frame, /* is_ip6 */ 0);
}
...
VLIB_NODE_FN (gre6_tun_decap_node) (vlib_main_t * vm,
				    ...
				    vlib_frame_t * from_frame)
{
  return gre_tun_decap (vm, node, from_frame, /* is_ip6 */ 1);
}
