			       ...
			       vlib_frame_t * frame)
{
  ...
  while (n_left_from >= 2)
      ...
      if (PREDICT_FALSE (gt[0]->type == GRE_TUNNEL_TYPE_ERSPAN))
	{
	  ...
	  erspan_t2_t *h0 = vlib_buffer_get_current (b[0]);
	  u32 seq_num = clib_atomic_fetch_add (&gt[0]->gre_sn->seq_num, 1);
      ...
      if (PREDICT_FALSE (gt[1]->type == GRE_TUNNEL_TYPE_ERSPAN))
	{
	  ...
	  erspan_t2_t *h0 = vlib_buffer_get_current (b[1]);
	  u32 seq_num = clib_atomic_fetch_add (&gt[1]->gre_sn->seq_num, 1);
	  u64 hdr = clib_host_to_net_u64 (ERSPAN_HDR2);
	  h0->seq_num = clib_host_to_net_u32 (seq_num);
	  h0->t2_u64 = hdr;
