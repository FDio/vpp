			     ...
			     vlib_cli_command_t * cmd)
{
  ...
  if (is_add)
      ...
      if (!(m_args & 2))
	{
      ...
      rv = ipsec_sa_add_and_lock (id, spi, proto, crypto_alg, &ck, integ_alg, &ik, flags,
				  originator, tfs_type, tfs_config, clib_host_to_net_u32 (salt),
				  udp_src, udp_dst, anti_replay_window_size, &tun, &sai);
    }
  else
    {
