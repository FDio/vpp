		...
		vnet_crypto_op_t * ops, vlib_buffer_t * b[], u16 * nexts)
{
  ...
  while (n_fail)
	...
	{
	  u32 bi = op->user_data;
	  ah_decrypt_set_next_index (b[bi], node, vm->thread_index,
				     AH_DECRYPT_ERROR_INTEG_ERROR, bi, nexts,
  ...
  while (n_left > 0)
	  ...
	  if (ip4_is_fragment (ih4))
	    {
	      ah_decrypt_set_next_index (b[0], node, vm->thread_index,
					 AH_DECRYPT_ERROR_DROP_FRAGMENTS, 0,
      ...
      if (PREDICT_TRUE (sa0->integ_alg != IPSEC_INTEG_ALG_NONE))
	{
	  if (PREDICT_FALSE (ipsec_sa_is_set_USE_ESN (sa0) &&
	    ...
	    {
	      ah_decrypt_set_next_index (b[0], node, vm->thread_index,
					 AH_DECRYPT_ERROR_NO_TAIL_SPACE, 0,
					 next, AH_DECRYPT_NEXT_DROP);
	      goto next;
	    }
