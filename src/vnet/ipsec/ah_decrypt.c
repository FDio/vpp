		   ...
		   int is_ip6)
{
  ...
  while (n_left > 0)
	...
	{
	  u16 extra_tail_space = pd->icv_size;
	  if (ipsec_sa_is_set_USE_ESN (sa0))
	    extra_tail_space += sizeof (u32);

	  if (PREDICT_FALSE (pd->current_data + b[0]->current_length
			     + extra_tail_space > buffer_data_size))
	    {
	  ...
	  pd->auth_data = pd->current_data + b[0]->current_length;
	  pd->digest = icv - b[0]->data;
	  ...
	  if (ipsec_sa_is_set_USE_ESN (sa0))
	    {
	      ...
	      pd->auth_data += sizeof (seq_hi);
	      clib_memcpy (op->src + b[0]->current_length, &seq_hi,
			   sizeof (seq_hi));
	    }

	  clib_memcpy (b[0]->data + pd->auth_data, icv, pd->icv_size);
  ...
  while (n_left > 0)
	...
	{
	  if (PREDICT_FALSE
	      (clib_memcmp (b[0]->data + pd->digest,
			    b[0]->data + pd->auth_data, pd->icv_size)))
	    {
