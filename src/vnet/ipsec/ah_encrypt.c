		   ...
		   int is_ip6)
{
  ...
  while (n_left > 0)
      ...
      if (vnet_buffer (b[0])->ipsec.sad_index != current_sa_index)
	{
      ...
      if (PREDICT_FALSE ((u16) ~0 == sert->thread_index))
	{
	  /* this is the first packet to use this SA, claim the SA
	   * for this thread. this could happen simultaneously on
	   * another thread */
	  clib_atomic_cmp_and_swap (&sert->thread_index, ~0,
				    ipsec_sa_assign_thread (thread_index));
	}

      if (PREDICT_TRUE (thread_index != sert->thread_index))
	{
	  vnet_buffer (b[0])->ipsec.thread_index = sert->thread_index;
	  next[0] = AH_ENCRYPT_NEXT_HANDOFF;
	  goto next;
	}

      if (PREDICT_FALSE (esp_seq_advance (sert)))
	{
      ...
      if (PREDICT_TRUE (sert->is_tunnel))
	{
	  if (is_ip6)
      ...
      if (is_ip6)
	{
	  ...
	  if (PREDICT_FALSE (sert->is_tunnel))
	    {
	      ip6_set_dscp_network_order (&oh6_0->ip6, sert->dscp);
	      tunnel_encap_fixup_6o6 (sert->tunnel_flags, &ih6_0->ip6,
				      &oh6_0->ip6);
	    }
	  ...
	  oh6_0->ah.spi = sert->spi_le;
	  oh6_0->ah.seq_no = clib_net_to_host_u32 (sert->seq);
      ...
      else
	{
	  ...
	  oh0->ip4.length =
	    clib_host_to_net_u16 (vlib_buffer_length_in_chain (vm, b[0]));
	  oh0->ah.spi = sert->spi_le;
	  oh0->ah.seq_no = clib_net_to_host_u32 (sert->seq);
	  oh0->ah.nexthdr = next_hdr_type;
	  oh0->ah.hdrlen =
	    (sizeof (ah_header_t) + icv_size + padding_len) / 4 - 2;
	}

      if (PREDICT_TRUE (!is_ip6 && sert->is_tunnel && sert->is_tunnel_v6))
	{
	  clib_memcpy_fast (&oh0->ip4.address_pair,
			    &sert->ip4_hdr.address_pair,
			    sizeof (ip4_address_pair_t));

	  next[0] = sert->dpo.dpoi_next_node;
	  vnet_buffer (b[0])->ip.adj_index[VLIB_TX] = sert->dpo.dpoi_index;
	}
      else if (is_ip6 && sert->is_tunnel && sert->is_tunnel_v6)
	{
	  clib_memcpy_fast (&oh6_0->ip6.src_address,
			    &sert->ip6_hdr.src_address,
			    sizeof (ip6_address_t) * 2);
	  next[0] = sert->dpo.dpoi_next_node;
	  vnet_buffer (b[0])->ip.adj_index[VLIB_TX] = sert->dpo.dpoi_index;
	}

      if (PREDICT_TRUE (sert->integ_op_id))
	{
	  vnet_crypto_op_t *op;
	  vec_add2_aligned (ptd->integ_ops, op, 1, CLIB_CACHE_LINE_BYTES);
	  vnet_crypto_op_init (op, sert->integ_op_id);
	  ...
	  op->key_index = sert->integ_key_index;
	  op->user_data = b - bufs;
	  if (sert->use_esn)
	    {
	      u32 seq_hi = clib_host_to_net_u32 (sert->seq_hi);
