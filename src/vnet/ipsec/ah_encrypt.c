		   ...
		   int is_ip6)
{
  ...
  while (n_left > 0)
      ...
      if (PREDICT_TRUE (thread_index != ort->thread_index))
	{
	  ...
	  /*This is to fix the handoff problem which will come if the packets received
           * on different thread and processed on the different threads most likely happen
           * controll packets generated on the same machine where ipsec is configured and has
           * to go through IPsec AH*/
          pd->skip = 1;
	  goto next;
	}
