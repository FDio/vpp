			     ...
			     u16 rp, u32 pol_id)
{
  ...
  ip4_5tuple.kv_16_8_1.value =
    (((u64) pol_id) << 32) | ((u64) im->epoch_count);
			      ...
			      u16 rp)
{
  ...
  if ((kv_result.value != ~0ULL) &&
      (im->epoch_count == ((u32) (kv_result.value & 0xFFFFFFFF))))
    {
			   ...
			   u16 rp)
{
  ...
  add_bihash:
     ...
     * table.
     */
    if (im->flow_cache_flag && !im->spd_update_flag)
      {
	/* Add an Entry in Flow cache */
	ipsec4_spd_add_bihash_entry (
