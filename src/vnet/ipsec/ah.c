...
always_inline void
ah_finish_encrypt_one (vlib_main_t * vm, vlib_buffer_t * b0, int is_ip6)
{
  if (is_ip6)
    {
      ip6_header_t *oh6 = 0;
...
always_inline void
ah_finish_encrypt_inline (vlib_main_t * vm, ipsec_main_t * im, u16 * next,
			  ipsec_job_desc_t * job, int n_jobs, int is_ip6)
{
  while (n_jobs)
      ...
      if (IPSEC_ERR_OK == job->error)
	{
	  ...
	  ah_finish_encrypt_one (vm, b0, is_ip6);
...
{
  if (is_ip6)
    ah_finish_encrypt_inline (vm, im, next, job, n_jobs, 1 /*is_ip6 */ );
  else
    ah_finish_encrypt_inline (vm, im, next, job, n_jobs, 0 /*is_ip6 */ );
}

always_inline void
ah_encrypt_prepare_jobs_inline (vlib_main_t * vm, ipsec_main_t * im,
...
{
  if (is_ip6)
    return ah_encrypt_prepare_jobs_inline (vm, im, em, b, job, n_jobs,
...
always_inline void
ah_decrypt_prepare_jobs_inline (vlib_main_t * vm, ipsec_main_t * im,
				ipsec_proto_main_t * em, vlib_buffer_t ** b,
				ipsec_job_desc_t * job, int n_jobs,
				int is_ip6, u32 next_index_drop)
{
...
{
  if (is_ip6)
    return ah_decrypt_prepare_jobs_inline (vm, im, em, b, job, n_jobs,
					   1 /*is_ip6 */ , next_index_drop);
  else
    return ah_decrypt_prepare_jobs_inline (vm, im, em, b, job, n_jobs,
					   0 /*is_ip6 */ , next_index_drop);
...
always_inline void
ah_finish_decrypt_one (vlib_main_t * vm, ipsec_job_desc_t * job, int is_ip6,
...
always_inline void
ah_finish_decrypt_inline (vlib_main_t * vm, u16 * next,
			  ipsec_job_desc_t * job, int n_jobs, int is_ip6,
			  u32 next_index_drop, u32 next_index_ip4_input,
			  u32 next_index_ip6_input, u32 next_index_gre_input)
{
  while (n_jobs)
    {
      if (IPSEC_ERR_OK == job->error)
	{
	  ah_finish_decrypt_one (vm, job, is_ip6, next_index_drop,
...
{
  if (is_ip6)
    return ah_finish_decrypt_inline (vm, next, job, n_jobs, 1 /*is_ip6 */ ,
				     next_index_drop, next_index_ip4_input,
				     next_index_ip6_input,
				     next_index_gre_input);
  else
    return ah_finish_decrypt_inline (vm, next, job, n_jobs, 0 /*is_ip6 */ ,
				     next_index_drop, next_index_ip4_input,
				     next_index_ip6_input,
				     next_index_gre_input);
}
