			...
			vlib_frame_t * from_frame)
{
  ...
  u8 ip_hdr_len, esp_hdr_len, esp_ftr_len, iv_len, icv_len;
  ...
  ip_hdr_len = sizeof (ip4_header_t);
  esp_hdr_len = sizeof (esp_header_t);
  esp_ftr_len = sizeof (esp_footer_t);
  iv_len = 16;
	      ...
	      if (t->hw_if_index != ~0)
		{
		  ...
		  if (PREDICT_TRUE (sw_if_index0 == last_sw_if_index))
		  ...
		  else
		    {
		      ...
		      n_bytes -= n_packets * (ip_hdr_len + esp_hdr_len +
					      iv_len + esp_ftr_len + icv_len);
  ...
  if (last_t)
    {
      ...
      n_bytes -= n_packets * (ip_hdr_len + esp_hdr_len + iv_len +
			      esp_ftr_len + icv_len);
