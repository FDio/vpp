...
#define _(c, h, s, k, d)                                                      \
  ...
  if (vnet_calg->alg == VNET_CRYPTO_ALG_##c &&                                \
    ...
    {                                                                         \
      ...
      if (irt)                                                                \
	{                                                                     \
	  irt->op_id = VNET_CRYPTO_OP_##c##_##h##_TAG##d##_DEC;               \
  ...
  if (irt)
      ...
      if (is_async)
	{
	  ...
	  if (ipsec_sa_is_set_USE_ESN (sa))
	    {
#define _(n, s, ...)                                                          \
  if (irt->op_id == VNET_CRYPTO_OP_##n##_DEC)                                 \
    irt->op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD12_DEC;                        \
...
#define _(n, s, ...)                                                          \
  if (irt->op_id == VNET_CRYPTO_OP_##n##_DEC)                                 \
    irt->op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD8_DEC;                         \
