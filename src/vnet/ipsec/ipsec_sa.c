...
ipsec_sa_inb_refresh_op_tmpl (ipsec_sa_inb_rt_t *irt)
{
  ...
  if (irt->is_async || (!irt->keys.crypto && !irt->keys.integ) || !irt->op_id)
    {
  ...
  irt->op_tmpl_single.keys = (uword) &irt->keys;
  irt->op_tmpl_chained.keys = (uword) &irt->keys;
...
ipsec_sa_set_async_mode (ipsec_sa_t *sa, int is_enabled)
{
  ...
  u32 is_async;
    ...
    {
      if (!sa->keys.crypto && !sa->keys.integ)
	{
      ...
      else
	{
	  vnet_crypto_alg_t alg =
	    sa->keys.crypto ? sa->keys.crypto->alg : sa->keys.integ->alg;
	  if (sa->keys.crypto && sa->keys.integ)
	    alg = vnet_crypto_link_algs (sa->keys.crypto->alg,
					 sa->keys.integ->alg);

	  vnet_crypto_op_id_t *op_ids = vnet_crypto_ops_from_alg (alg);
...
ipsec_sa_init_runtime (ipsec_sa_t *sa)
{
  ...
  if (ipsec_sa_get_outb_rt (sa))
	...
	clib_max (4, im->crypto_algs[sa->crypto_alg].block_align);
      ...
      ort->op_tmpl_single.keys = (uword) &ort->keys;
      ort->op_tmpl_chained.keys = (uword) &ort->keys;
	...
	im->integ_algs[sa->integ_alg].bld_integ_op_tmpl;
      if ((!ort->keys.crypto && !ort->keys.integ) || !ort->op_id ||
	  ort->is_async)
		       ...
		       const tunnel_t *tun, u32 *sa_out_index)
{
  ...
  *ort = (ipsec_sa_outb_rt_t){
  ...
  if (crypto_alg != IPSEC_CRYPTO_ALG_NONE)
    {
      sa->keys.crypto = vnet_crypto_key_add_obj (
  ...
  if (integ_alg != IPSEC_INTEG_ALG_NONE)
    {
      sa->keys.integ = vnet_crypto_key_add_obj (
...
ipsec_sa_del (ipsec_sa_t * sa)
{
  ...
  if (sa->crypto_alg != IPSEC_CRYPTO_ALG_NONE)
    vnet_crypto_key_del_obj (vm, sa->keys.crypto);
  if (sa->integ_alg != IPSEC_INTEG_ALG_NONE)
    vnet_crypto_key_del_obj (vm, sa->keys.integ);
