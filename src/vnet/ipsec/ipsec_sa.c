...
ipsec_sa_inb_refresh_op_tmpl (ipsec_sa_inb_rt_t *irt)
{
  ...
  irt->op_tmpl_single.key_data =
    (uword) vnet_crypto_key_data_get (irt->key, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
  irt->op_tmpl_chained.key_data =
    (uword) vnet_crypto_key_data_get (irt->key, VNET_CRYPTO_HANDLER_TYPE_CHAINED);
...
ipsec_sa_init_runtime (ipsec_sa_t *sa)
{
  ...
  if (ipsec_sa_get_outb_rt (sa))
	...
	(uword) vnet_crypto_key_data_get (ort->key, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
      ort->op_tmpl_chained.key_data =
	(uword) vnet_crypto_key_data_get (ort->key, VNET_CRYPTO_HANDLER_TYPE_CHAINED);
		       ...
		       const tunnel_t *tun, u32 *sa_out_index)
{
  ...
  *ort = (ipsec_sa_outb_rt_t){
  ...
  if (crypto_alg != IPSEC_CRYPTO_ALG_NONE)
	...
	{
	  sa->key =
	    vnet_crypto_key_add_ptr (im->crypto_algs[crypto_alg].alg, (u8 *) ck->data, ck->len);
	}
  ...
  if (integ_alg != IPSEC_INTEG_ALG_NONE && sa->key == NULL)
    {
      sa->key = vnet_crypto_key_add_ptr (im->integ_algs[integ_alg].alg, (u8 *) ik->data, ik->len);
