...
ipsec_sa_set_async_mode (ipsec_sa_t *sa, int is_enabled)
{
  ...
  if (is_enabled)
	...
	cipher_key_index = sa->crypto_sync_key_index;
      ...
      inb_cipher_op_id = sa->crypto_async_enc_op_id;
		 ...
		 bool is_tun)
{
  ...
  if (ipsec_sa_is_set_UDP_ENCAP (sa))
      ...
      if (dst_port != IPSEC_UDP_PORT_NONE && dst_port != sa->udp_dst_port)
	{
	  ...
	  if (sa->outb_rt)
	    sa->outb_rt->udp_hdr.dst_port = clib_host_to_net_u16 (dst_port);
	}
      if (src_port != IPSEC_UDP_PORT_NONE && src_port != (sa->udp_src_port))
	{
	  sa->udp_src_port = src_port;
	  if (sa->outb_rt)
	    sa->outb_rt->udp_hdr.src_port = clib_host_to_net_u16 (src_port);
	}
    }
  return (0);
}
...
ipsec_sa_del (ipsec_sa_t * sa)
{
  ...
  if (ipsec_sa_is_set_IS_TUNNEL (sa) && !ipsec_sa_is_set_IS_INBOUND (sa))
    dpo_reset (&sa->outb_rt->dpo);
  if (sa->crypto_alg != IPSEC_CRYPTO_ALG_NONE)
    vnet_crypto_key_del (vm, sa->crypto_sync_key_index);
  if (sa->integ_alg != IPSEC_INTEG_ALG_NONE)
    vnet_crypto_key_del (vm, sa->integ_sync_key_index);
  if (ipsec_sa_is_set_ANTI_REPLAY_HUGE (sa))
    if (sa->inb_rt && sa->inb_rt->replay_window_huge)
      clib_bitmap_free (sa->inb_rt->replay_window_huge);
...
ipsec_sa_bind (u32 id, u32 worker, bool bind)
{
  ...
  if (sa->inb_rt)
    sa->inb_rt->thread_index = thread_index;
  if (sa->outb_rt)
    sa->outb_rt->thread_index = thread_index;
  return 0;
}
