...
ipsec_sa_set_crypto_alg (ipsec_sa_t * sa, ipsec_crypto_alg_t crypto_alg)
{
  ...
  sa->crypto_key_type = im->crypto_algs[crypto_alg].key_type;
...
ipsec_sa_set_integ_alg (ipsec_sa_t * sa, ipsec_integ_alg_t integ_alg)
{
  ...
  sa->integ_key_type = im->integ_algs[integ_alg].key_type;
	      ...
	      const ip46_address_t * tun_dst, u32 * sa_out_index)
{
  ...
  sa->crypto_key_index = vnet_crypto_key_add (vm, sa->crypto_key_type,
					      (u8 *) ck->data, ck->len);
  sa->integ_key_index = vnet_crypto_key_add (vm, sa->integ_key_type,
					     (u8 *) ik->data, ik->len);
...
ipsec_set_sa_key (u32 id, const ipsec_key_t * ck, const ipsec_key_t * ik)
{
  ...
  if (ck)
    {
      clib_memcpy (&sa->crypto_key, ck, sizeof (sa->crypto_key));
      vnet_crypto_key_modify (vm, sa->crypto_key_index, (u8 *) ck->data,
			      ck->len);
    }
  ...
  if (ik)
    {
      ...
      vnet_crypto_key_modify (vm, sa->integ_key_index, (u8 *) ik->data,
			      ik->len);
    }
