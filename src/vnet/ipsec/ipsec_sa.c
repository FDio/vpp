...
static void
ipsec_sa_get_async_op_ids (const ipsec_sa_t *sa, const ipsec_sa_crypto_integ_map_t *m,
			   vnet_crypto_op_id_t *enc_op_id, vnet_crypto_op_id_t *dec_op_id)
{
  *enc_op_id = 0;
  *dec_op_id = 0;
...
#define _(n, s, ...)                                                                               \
  if (sa->crypto_sync_enc_op_id == VNET_CRYPTO_OP_##n##_ENC)                                       \
    *enc_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD12_ENC;                                             \
  if (sa->crypto_sync_dec_op_id == VNET_CRYPTO_OP_##n##_DEC)                                       \
    *dec_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD12_DEC;
...
#define _(n, s, ...)                                                                               \
  if (sa->crypto_sync_enc_op_id == VNET_CRYPTO_OP_##n##_ENC)                                       \
    *enc_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD8_ENC;                                              \
  if (sa->crypto_sync_dec_op_id == VNET_CRYPTO_OP_##n##_DEC)                                       \
    *dec_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD8_DEC;
      foreach_crypto_aead_alg
#undef _
    }
  if (*enc_op_id == 0)
    {
      *enc_op_id = m->enc_esn_op_id ? m->enc_esn_op_id : m->enc_op_id;
      *dec_op_id = m->dec_esn_op_id ? m->dec_esn_op_id : m->dec_op_id;
    }
}

static_always_inline void
ipsec_sa_set_sync_op_ids (ipsec_sa_t *sa, const ipsec_sa_crypto_integ_map_t *m)
{
  vnet_crypto_op_id_t *op_ids;

  sa->crypto_sync_enc_op_id = m->enc_op_id;
  sa->crypto_sync_dec_op_id = m->dec_op_id;

  if (m->alg == VNET_CRYPTO_ALG_NONE)
    return;

  op_ids = vnet_crypto_ops_from_alg (m->alg);

  if (op_ids[VNET_CRYPTO_OP_TYPE_ENCRYPT] && op_ids[VNET_CRYPTO_OP_TYPE_DECRYPT])
    {
      sa->crypto_sync_enc_op_id = op_ids[VNET_CRYPTO_OP_TYPE_ENCRYPT];
      sa->crypto_sync_dec_op_id = op_ids[VNET_CRYPTO_OP_TYPE_DECRYPT];
      return;
    }

  if (op_ids[VNET_CRYPTO_OP_TYPE_HMAC])
    {
      sa->crypto_sync_enc_op_id = op_ids[VNET_CRYPTO_OP_TYPE_HMAC];
      sa->crypto_sync_dec_op_id = op_ids[VNET_CRYPTO_OP_TYPE_HMAC];
    }
}
		       ...
		       const tunnel_t *tun, u32 *sa_out_index)
{
  ...
  *ort = (ipsec_sa_outb_rt_t){
  ...
  ipsec_sa_get_async_op_ids (sa, m, &async_enc_op_id, &async_dec_op_id);
