		       ...
		       const tunnel_t *tun, u32 *sa_out_index)
{
  ...
  *ort = (ipsec_sa_outb_rt_t){
  ...
  if (crypto_alg != IPSEC_CRYPTO_ALG_NONE)
    {
      sa->key = vnet_crypto_key_add (vm, im->crypto_algs[crypto_alg].alg, (u8 *) ck->data, ck->len);
      if (!sa->key)
	{
	  pool_put (im->sa_pool, sa);
  ...
  if (integ_alg != IPSEC_INTEG_ALG_NONE)
    {
      if (sa->key)
	{
	  vnet_crypto_integ_key_add (sa->key, im->integ_algs[integ_alg].alg, (u8 *) ik->data,
				     ik->len);
	}
      else
	{
	  sa->key =
	    vnet_crypto_key_add (vm, im->integ_algs[integ_alg].alg, (u8 *) ik->data, ik->len);
	  if (!sa->key)
	    {
	      pool_put (im->sa_pool, sa);
	      return VNET_API_ERROR_KEY_LENGTH;
