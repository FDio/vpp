		       ...
		       const tunnel_t *tun, u32 *sa_out_index)
{
  ...
  *ort = (ipsec_sa_outb_rt_t){
  ...
  sa->key_index = vnet_crypto_key_add (vm, m->alg, crypto_key_data, crypto_key_len, integ_key_data,
				       integ_key_len);
  ...
  if (ipsec_sa_is_set_IS_TUNNEL (sa) && !ipsec_sa_is_set_IS_INBOUND (sa))
	...
	{
	  tunnel_build_v6_hdr (
	    &sa->tunnel, (ipsec_sa_is_set_UDP_ENCAP (sa) ? IP_PROTOCOL_UDP : IP_PROTOCOL_IPSEC_ESP),
	    &ort->ip6_hdr);
	}
      else
	{
