...
ipsec_sa_inb_refresh_op_tmpl (ipsec_sa_inb_rt_t *irt)
{
  ...
  irt->op_tmpl_single.keys = (uword) vnet_crypto_get_active_engine_key_data (
    irt->key, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
  irt->op_tmpl_chained.keys = (uword) vnet_crypto_get_active_engine_key_data (
    irt->key, VNET_CRYPTO_HANDLER_TYPE_CHAINED);
  ...
  u32 is_async;
      ...
      else
	{
	  vnet_crypto_op_id_t *op_ids =
	    vnet_crypto_ops_from_alg (sa->key->alg);
...
ipsec_sa_init_runtime (ipsec_sa_t *sa)
{
  ...
  if (ipsec_sa_get_outb_rt (sa))
	...
	clib_max (4, im->crypto_algs[sa->crypto_alg].block_align);
      ...
      ort->op_tmpl_single.keys =
	(uword) vnet_crypto_get_active_engine_key_data (
	  ort->key, VNET_CRYPTO_HANDLER_TYPE_SIMPLE);
      ort->op_tmpl_chained.keys =
	(uword) vnet_crypto_get_active_engine_key_data (
	  ort->key, VNET_CRYPTO_HANDLER_TYPE_CHAINED);
		       ...
		       const tunnel_t *tun, u32 *sa_out_index)
{
  ...
  *ort = (ipsec_sa_outb_rt_t){
  ...
  if (crypto_alg != IPSEC_CRYPTO_ALG_NONE)
    {
      sa->key = vnet_crypto_key_add (vm, im->crypto_algs[crypto_alg].alg,
				     (u8 *) ck->data, ck->len);
      if (!sa->key)
	{
	  pool_put (im->sa_pool, sa);
  ...
  if (integ_alg != IPSEC_INTEG_ALG_NONE)
    {
      if (sa->key)
	{
	  vnet_crypto_integ_key_add (sa->key, im->integ_algs[integ_alg].alg,
				     (u8 *) ik->data, ik->len);
	}
      else
	{
	  sa->key = vnet_crypto_key_add (vm, im->integ_algs[integ_alg].alg,
					 (u8 *) ik->data, ik->len);
	  if (!sa->key)
	    {
	      pool_put (im->sa_pool, sa);
