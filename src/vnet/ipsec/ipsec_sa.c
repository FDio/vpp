...
#define _(c, h, s, k, d)                                                      \
  ...
  if (vnet_calg->alg == VNET_CRYPTO_ALG_##c &&                                \
	...
	{                                                                     \
      ...
      if (ort)                                                                \
	{                                                                     \
	  ort->cipher_op_id = VNET_CRYPTO_OP_##c##_##h##_TAG##d##_ENC;        \
  ...
  if (ort)
    {
      ...
      ort->cipher_op_id = vnet_calg->enc_op_id;
      ...
      if (is_async)
	{
	  ...
	  if (ipsec_sa_is_set_USE_ESN (sa))
	    {
#define _(n, s, ...)                                                          \
  if (ort->cipher_op_id == VNET_CRYPTO_OP_##n##_ENC)                          \
    ort->cipher_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD12_ENC;                 \
...
#define _(n, s, ...)                                                          \
  if (ort->cipher_op_id == VNET_CRYPTO_OP_##n##_ENC)                          \
    ort->cipher_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD8_ENC;                  \
