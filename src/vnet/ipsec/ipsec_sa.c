...
ipsec_sa_set_async_mode (ipsec_sa_t *sa, int is_enabled)
{
  ...
  if (ipsec_sa_get_outb_rt (sa))
	...
	ort->cached.prepare_sync_op = 1;
      ort->cached.is_async = ort->cached.is_async;
      ort->cached.prepare_sync_op = ort->cached.prepare_sync_op;
    }
}
...
ipsec_sa_init_runtime (ipsec_sa_t *sa, const ipsec_sa_crypto_integ_map_t *m)
{
  ...
  if (ipsec_sa_get_outb_rt (sa))
    {
      ipsec_sa_outb_rt_t *ort = ipsec_sa_get_outb_rt (sa);
      ort->cached.use_anti_replay = ipsec_sa_is_set_USE_ANTI_REPLAY (sa);
      ...
      ort->cached.has_cipher = cipher_iv_size != 0;
	...
	ort->cached.prepare_sync_op = 1;
      ort->cached.spi_be = ort->cached.spi_be;
      ort->cached.integ_icv_size = ort->cached.integ_icv_size;
      ort->cached.esp_block_align = ort->cached.esp_block_align;
      ort->cached.cipher_iv_size = ort->cached.cipher_iv_size;
      ort->cached.is_async = ort->cached.is_async;
      ort->cached.prepare_sync_op = ort->cached.prepare_sync_op;
      ort->cached.is_null_gmac = ort->cached.is_null_gmac;
      ort->cached.is_aead = ort->cached.is_aead;
      ort->cached.is_ctr = ort->cached.is_ctr;
      ort->cached.has_cipher = ort->cached.has_cipher;
      ort->cached.needs_integ = ort->cached.needs_integ;
      ort->cached.needs_sync_enc = ort->cached.needs_sync_enc;
      ort->cached.is_tunnel_v6 = ort->cached.is_tunnel_v6;
      ort->cached.udp_encap = ort->cached.udp_encap;
      ort->cached.need_udp_cksum = ort->cached.need_udp_cksum;
      ort->cached.need_tunnel_fixup = ort->cached.need_tunnel_fixup;
      ort->cached.next_hdr_protocol = ort->cached.next_hdr_protocol;
      ort->cached.esp_iv_bytes = ort->cached.esp_iv_bytes;
      ort->cached.tunnel_fixed_hdr_bytes = ort->cached.tunnel_fixed_hdr_bytes;
		       ...
		       const tunnel_t *tun, u32 *sa_out_index)
{
  ...
  *ort = (ipsec_sa_outb_rt_t){
  ...
  if (key_alg != VNET_CRYPTO_ALG_NONE)
    {
      sa->key_index = vnet_crypto_key_add (vm, key_alg, crypto_key_data, crypto_key_len,
					   integ_key_data, integ_key_len);
