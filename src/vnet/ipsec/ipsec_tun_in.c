				...
				vlib_frame_t * from_frame, int is_ip6)
{
  ...
  while (n_left_from > 0)
	...
	{
	  key40->key.remote_ip = ip40->src_address;
	  key40->key.spi = esp0->spi;

	  if (key40->key.as_u64 == last_key4.key.as_u64)
	    {
	      itr0 = last_result;
	    }
	  else
	      ...
	      if (!rv)
		{
		  ...
		  last_result = itr0;
		  last_key4.key.as_u64 = key40->key.as_u64;
		}
	      else
		{
