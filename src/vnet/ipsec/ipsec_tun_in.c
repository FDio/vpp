				...
				vlib_frame_t * from_frame, int is_ip6)
{
  ...
  while (n_left_from > 0)
    {
      u32 sw_if_index0, len0, hdr_sz0, itpi0;
      ...
      else
	{
      ...
      vnet_buffer (b[0])->ipsec.sad_index = itp0->itp_sa[INBOUND];
      vnet_buffer (b[0])->ipsec.protect_index = itpi0;
      ...
      else
	{
	   ...
	   */
	  /* if (itp0->itp_flags & IPSEC_PROTECT_TUN) */
	  /*   next[0] = itp0->itp_edge; */
	  /* else */
