...
ipsec_sa_anti_replay_window_shift (ipsec_sa_t *sa, u32 inc, bool ar_huge)
{
  ...
  if (inc < window_size)
      ...
      if (ar_huge)
	{
	  ...
	  clib_prefetch_store (&window[i_block]);
	  ...
	  if (i_word_start + inc <= uword_bits)
	      ...
	      while (full_words >= 4)
		{
		  if (full_words >= 8)
		    {
		      clib_prefetch_store (
			&window[(i_block + 4) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 5) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 6) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 7) & (n_blocks - 1)]);
		    }
