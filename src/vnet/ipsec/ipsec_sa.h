...
extern vlib_log_class_t ipsec_sa_logger;
...
#define foreach_ipsec_sa_flags                                                \
  ...
  _ (2048, ANTI_REPLAY_HUGE, "anti-replay-huge")
...
extern int ipsec_sa_add_and_lock (
  ...
  const tunnel_t *tun, u32 *sa_out_index, u32 anti_replay_window_size);
...
ipsec_sa_anti_replay_get_64b_window (const ipsec_sa_t *sa)
{
  ...
  u32 tl = sa->seq % window_size;

  if (PREDICT_TRUE (tl >= 63))
    return clib_bitmap_get_multiple (sa->replay_window_huge, tl - 63, 64);

  w = clib_bitmap_get_multiple_no_check (sa->replay_window_huge, 0, tl + 1)
      << (63 - tl);
  w |= clib_bitmap_get_multiple_no_check (sa->replay_window_huge,
					  window_size - 63 + tl, 63 - tl);
...
ipsec_sa_anti_replay_check (const ipsec_sa_t *sa, u32 seq)
{
  ...
  if (PREDICT_FALSE (ipsec_sa_is_set_ANTI_REPLAY_HUGE (sa)))
    return clib_bitmap_get (sa->replay_window_huge, seq % window_size);
  else
				     ...
				     u32 *hi_seq_req)
{
  ...
  if (!ipsec_sa_is_set_USE_ESN (sa))
	...
	*hi_seq_req = 0;
	...
	return 0;
      ...
      /* does the packet falls out on the left of the window */
      if (sa->seq >= seq + IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE (sa))
	return 1;
  ...
  if (PREDICT_TRUE (sa->seq >= IPSEC_SA_ANTI_REPLAY_WINDOW_MAX_INDEX (sa)))
    {
      /*
       ...
       */
      if (seq < IPSEC_SA_ANTI_REPLAY_WINDOW_LOWER_BOUND (sa))
	{
	  /*
	  ...
	  if (post_decrypt)
	    {
	      /* ?: in pre-crypto, the packet seqh was set to sa->seq_hi+1
	       * and it is not modified during the integrity check, so we
	       * never end there */
...
ipsec_sa_anti_replay_window_shift (ipsec_sa_t *sa, u32 inc)
{
  ...
  if (inc < window_size)
      ...
      if (PREDICT_FALSE (ipsec_sa_is_set_ANTI_REPLAY_HUGE (sa)))
	{
	  ...
	  /* mitigate the case where sa->seq <= window_size and wrongly
	     computing the lower bound index in the window */
	  u32 window_lower_bound = (sa->seq + 1) % window_size;
	  u32 window_next_lower_bound =
	    (window_lower_bound + inc) % window_size;
	  clib_bitmap_t *mask = 0;
	  clib_bitmap_validate (mask, window_size);

	  if (window_lower_bound < window_next_lower_bound)
	    {
	      clib_bitmap_set_region (mask, window_lower_bound, 1, inc);
	    }
	  else
	    {
	      clib_bitmap_set_region (mask, window_lower_bound, 1,
				      window_size - window_lower_bound);
	      clib_bitmap_set_region (mask, 0, 1, window_next_lower_bound);
	    }

	  clib_bitmap_and (mask, sa->replay_window_huge);

	  seen = clib_bitmap_count_set_bits (mask);

	  /* clib_bitmap_and trims trailling zeroes */
	  clib_bitmap_xor (sa->replay_window_huge, mask);
	  clib_bitmap_validate (sa->replay_window_huge, window_size);
	  clib_bitmap_free (mask);

	  clib_bitmap_set_no_check (sa->replay_window_huge,
				    (sa->seq + inc) % window_size, 1);
	}
      else
	{
	  /*
