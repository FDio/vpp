...
typedef struct
{
  ...
  vnet_crypto_key_index_t async_key_index;
  u32 salt;
  u16 thread_index;
...
STATIC_ASSERT_SIZEOF (ipsec_sa_dec_rt_t, CLIB_CACHE_LINE_BYTES);
...
STATIC_ASSERT_SIZEOF (ipsec_sa_enc_rt_t, 2 * CLIB_CACHE_LINE_BYTES);

typedef struct
{
  CLIB_CACHE_LINE_ALIGN_MARK (cacheline0);
  u16 udp_port_registered : 1;
  u16 dpo_created : 1;
  u16 hash_set : 1;
  u32 spi;
  ...
  ipsec_sa_enc_rt_t *enc_rt;
  ipsec_sa_dec_rt_t *dec_rt;
  u16 udp_src_port;
  u16 udp_dst_port;
} ipsec_sa_t;
...
#define IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE(_sa)                                 \
  (u32) (PREDICT_FALSE (_sa->anti_reply_huge) ?                               \
...
#define IPSEC_SA_ANTI_REPLAY_WINDOW_N_SEEN(_sa)                               \
  (u64) (PREDICT_FALSE (_sa->anti_reply_huge) ?                               \
...
always_inline u64
ipsec_sa_anti_replay_get_64b_window (const ipsec_sa_dec_rt_t *sdrt)
{
  if (!sdrt->anti_reply_huge)
    return sdrt->replay_window;

  u64 w;
  u32 window_size = IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE (sdrt);
  u32 tl_win_index = sdrt->seq & (window_size - 1);

  if (PREDICT_TRUE (tl_win_index >= 63))
    return clib_bitmap_get_multiple (sdrt->replay_window_huge,
				     tl_win_index - 63, 64);

  w = clib_bitmap_get_multiple_no_check (sdrt->replay_window_huge, 0,
					 tl_win_index + 1)
      << (63 - tl_win_index);
  w |= clib_bitmap_get_multiple_no_check (sdrt->replay_window_huge,
...
always_inline int
ipsec_sa_anti_replay_check (const ipsec_sa_dec_rt_t *sdrt, u32 seq,
			    bool ar_huge)
{
  u32 window_size = IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE_KNOWN_WIN (sdrt, ar_huge);
  ...
  if (ar_huge)
    return clib_bitmap_get (sdrt->replay_window_huge, seq & (window_size - 1));
  else
    return (sdrt->replay_window >> (window_size + seq - sdrt->seq - 1)) & 1;
 ...
 */
always_inline int
ipsec_sa_anti_replay_and_sn_advance (const ipsec_sa_dec_rt_t *sdrt, u32 seq,
				     ...
				     u32 *hi_seq_req, bool ar_huge)
{
  ...
  u32 window_lower_bound =
    IPSEC_SA_ANTI_REPLAY_WINDOW_LOWER_BOUND_KNOWN_WIN (sdrt, ar_huge);
  ...
  if (!sdrt->use_esn)
	...
	*hi_seq_req = 0;
      ...
      if (!sdrt->use_anti_replay)
	return 0;

      if (PREDICT_TRUE (seq > sdrt->seq))
	return 0;

      /* does the packet fall out on the left of the window */
      if (sdrt->seq >= seq + window_size)
	return 1;

      return ipsec_sa_anti_replay_check (sdrt, seq, ar_huge);
    }
  ...
  if (PREDICT_TRUE (window_size > 0 && sdrt->seq >= window_size - 1))
    {
      /*
...
always_inline u32
ipsec_sa_anti_replay_window_shift (ipsec_sa_dec_rt_t *sdrt, u32 inc,
				   bool ar_huge)
{
  u32 n_lost = 0;
  u32 seen = 0;
  ...
  if (inc < window_size)
      ...
      if (ar_huge)
	{
	  ...
	  clib_bitmap_t *window = sdrt->replay_window_huge;
	  u32 window_lower_bound = (sdrt->seq + 1) & (window_size - 1);
      ...
      else
	{
	   ...
	   */
	  u64 old = sdrt->replay_window & pow2_mask (inc);
	  /* the number of packets we saw in this section of the window */
	  seen = count_set_bits (old);
	  sdrt->replay_window =
	    ((sdrt->replay_window) >> inc) | (1ULL << (window_size - 1));
	}
  ...
  else
    {
      ...
      n_lost = window_size -
	       IPSEC_SA_ANTI_REPLAY_WINDOW_N_SEEN_KNOWN_WIN (sdrt, ar_huge);
	...
	{
	  clib_bitmap_zero (sdrt->replay_window_huge);
	  clib_bitmap_set_no_check (sdrt->replay_window_huge,
				    (sdrt->seq + inc) & (window_size - 1), 1);
	}
      else
	{
	  sdrt->replay_window = 1ULL << (window_size - 1);
	}
    }
 ...
 */
always_inline u64
ipsec_sa_anti_replay_advance (ipsec_sa_dec_rt_t *sdrt, u32 thread_index,
			      u32 seq, u32 hi_seq, bool ar_huge)
{
  ...
  if (sdrt->use_esn)
    {
      int wrap = hi_seq - sdrt->seq_hi;

      if (wrap == 0 && seq > sdrt->seq)
	{
	  pos = seq - sdrt->seq;
	  n_lost = ipsec_sa_anti_replay_window_shift (sdrt, pos, ar_huge);
	  sdrt->seq = seq;
	}
      else if (wrap > 0)
	{
      ...
      else if (wrap < 0)
	{
	  ...
	  if (ar_huge)
	    ...
	    sdrt->replay_window |= (1ULL << (window_size - 1 - pos));
	}
      else
	{
	  pos = sdrt->seq - seq;
