				     ...
				     u32 *hi_seq_req)
{
  ...
  u32 window_size = irt->anti_replay_window_size;
  u32 window_lower_bound = irt->seq - irt->anti_replay_window_size + 1;
				   ...
				   u32 inc)
{
  ...
  if (inc < window_size)
      ...
      else
	{
      ...
      uword_bitmap_set_bits_at_index (window,
  ...
  else
    {
      ...
      n_lost = window_size - uword_bitmap_count_set_bits (
			       irt->replay_window, window_size / uword_bits);

      /* any sequence numbers that now fall outside the window
       * are forever lost */
      n_lost += inc - window_size;

      for (u32 i = 0; i < window_size / uword_bits; i++)
	irt->replay_window[i] = 0;
      uword_bitmap_clear (irt->replay_window, window_size / uword_bits);
