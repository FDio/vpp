...
typedef void ipsec_build_single_op_tmpl_sig (ipsec_sa_outb_rt_t *ort,
					     vnet_crypto_op_t *op, u8 *payload,
					     u16 payload_len, u32 hdr_len,
					     void *esp);
typedef ipsec_build_single_op_tmpl_sig *ipsec_build_single_op_tmpl_fn_t;

typedef void
ipsec_build_chained_op_tmpl_sig (ipsec_sa_outb_rt_t *ort, vnet_crypto_op_t *op,
				 vlib_main_t *vm, void *ptd, vlib_buffer_t **b,
				 vlib_buffer_t *lb, u8 *payload,
				 u16 payload_len, u32 hdr_len, void *esp);
typedef ipsec_build_chained_op_tmpl_sig *ipsec_build_chained_op_tmpl_fn_t;

typedef void ipsec_build_single_op_sig (ipsec_sa_outb_rt_t *ort,
					vnet_crypto_op_t *op, vlib_main_t *vm,
					void *ptd, vlib_buffer_t **b,
					u8 *payload, u16 payload_len,
					u32 hdr_len, void *esp, u32 user_data);
typedef ipsec_build_single_op_sig *ipsec_build_single_op_fn_t;

typedef void ipsec_build_chained_op_sig (ipsec_sa_outb_rt_t *ort,
					 vlib_main_t *vm, void *ptd,
					 vlib_buffer_t **b, vlib_buffer_t *lb,
					 u8 *payload, u16 payload_len,
					 u32 hdr_len, void *esp,
					 u32 user_data);
typedef ipsec_build_chained_op_sig *ipsec_build_chained_op_fn_t;
...
typedef struct ipsec_sa_outb_rt_t_
{
  ...
  esp_ctr_nonce_t ctr_nonce_tmpl;
  esp_aead_t aad_tmpl;
  ipsec_build_single_op_tmpl_fn_t bld_single_enc_op_tmpl;
  ipsec_build_single_op_tmpl_fn_t bld_single_integ_op_tmpl;
  ipsec_build_chained_op_tmpl_fn_t bld_chained_enc_op_tmpl;
  ipsec_build_chained_op_tmpl_fn_t bld_chained_integ_op_tmpl;
  ip_dscp_t t_dscp;
