	      ...
	      while (full_words >= 8)
		{
		  if (full_words >= 16)
			...
			&window[(i_block + 8) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 9) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 10) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 11) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 12) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 13) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 14) & (n_blocks - 1)]);
		      clib_prefetch_store (
			&window[(i_block + 15) & (n_blocks - 1)]);
		    }
