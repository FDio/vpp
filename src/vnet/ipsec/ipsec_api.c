...
vl_api_ipsec_sad_entry_add_del_t_handler (vl_api_ipsec_sad_entry_add_del_t *mp)
{
  ...
  rv = ipsec_sa_add_and_lock (
    ...
    htons (mp->entry.udp_dst_port), &tun, &sa_index, 0);
			...
			u32 *sa_index)
{
  ...
  return ipsec_sa_add_and_lock (
    id, spi, proto, crypto_alg, &crypto_key, integ_alg, &integ_key, flags,
    entry->salt, htons (entry->udp_src_port), htons (entry->udp_dst_port),
    &tun, sa_index, ntohl (entry->anti_replay_window_size));
}

static void
vl_api_ipsec_sad_entry_add_del_v4_t_handler (
  vl_api_ipsec_sad_entry_add_del_v4_t *mp)
{
  vl_api_ipsec_sad_entry_add_del_v4_reply_t *rmp;
  u32 id, sa_index = ~0;
  int rv;
  ...
  REPLY_MACRO2 (VL_API_IPSEC_SAD_ENTRY_ADD_DEL_V4_REPLY,
		{ rmp->stat_index = htonl (sa_index); });
}
  ...
  pool_foreach (ab, im->ah_backends)
    {
      ...
      clib_memset (mp, 0, sizeof (*mp));
      mp->_vl_msg_id =
	ntohs (REPLY_MSG_ID_BASE + VL_API_IPSEC_BACKEND_DETAILS);
      mp->context = context;
