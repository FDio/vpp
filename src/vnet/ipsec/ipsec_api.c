...
send_ipsec_tunnel_protect_details (index_t itpi, void *arg)
{
  ...
  mp->tun.sw_if_index = htonl (itp->it_sw_if_index);
  mp->tun.sa_in = htonl (itp->it_sa[IPSEC_PROTECT_DIR_INBOUND]);
  mp->tun.sa_out = htonl (itp->it_sa[IPSEC_PROTECT_DIR_OUTBOUND]);
