  ...
  (vl_api_ipsec_sad_entry_add_del_t * mp)
{
  ...
  rv = ipsec_sa_add_and_lock (id, spi, proto, crypto_alg, &crypto_key, integ_alg, &integ_key, flags,
			      IPSEC_SA_TFS_TYPE_NO_TFS, NULL, mp->entry.salt,
			...
			u32 *sa_index)
{
  ...
  return ipsec_sa_add_and_lock (
    id, spi, proto, crypto_alg, &crypto_key, integ_alg, &integ_key, flags, IPSEC_SA_TFS_TYPE_NO_TFS,
    NULL, entry->salt, htons (entry->udp_src_port), htons (entry->udp_dst_port), 0, &tun, sa_index);
}
			...
			u32 *sa_index)
{
  ...
  return ipsec_sa_add_and_lock (id, spi, proto, crypto_alg, &crypto_key, integ_alg, &integ_key,
				flags, IPSEC_SA_TFS_TYPE_NO_TFS, NULL, entry->salt,
...
ipsec_sad_entry_add_v5 (const vl_api_ipsec_sad_entry_add_v3_t *mp, u32 *sa_index)
{
  ...
  if (config_len > sizeof (mp->entry.tfs_config))
    return VNET_API_ERROR_INVALID_ARGUMENT;
  void *tfs_config = ipsec_tfs_config_decode (mp->entry.tfs_config, config_len);

  rv = ipsec_sa_add_and_lock (id, spi, proto, crypto_alg, &crypto_key, integ_alg, &integ_key, flags,
			      tfs_type, tfs_config, entry->salt, htons (entry->udp_src_port),
			      htons (entry->udp_dst_port), ntohl (entry->anti_replay_window_size),
			      &tun, sa_index);
  ...
  if (ipsec_sa_is_set_USE_ANTI_REPLAY (sa) && irt)
    {
      ...
      mp->entry.anti_replay_window_size = clib_host_to_net_u32 (irt->anti_replay_window_size);
    }
  ...
  if (im->tfs_format_config_cb && ipsec_sa_is_IPTFS (sa))
    {
      ...
      u16 config_len = clib_min (sizeof (mp->entry.tfs_config), vec_len (s));
      mp->entry.tfs_config_len = htons (config_len);
      clib_memcpy_fast (mp->entry.tfs_config, s, config_len);
      vec_free (s);
    }
