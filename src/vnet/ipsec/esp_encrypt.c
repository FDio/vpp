      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_TRUE (sa0->crypto_alg != IPSEC_CRYPTO_ALG_NONE))
		...
		vlib_buffer_length_in_chain (vm, i_b0);
	      const u32 space_left = space_left_in_buffer(last_i_b0);
	      if (pad_bytes + sizeof (esp_footer_t) > space_left)
		{
		  last_empty_buffer = vec_len (empty_buffers) - 1;
	  ...
	  const u32 space_left = space_left_in_buffer(last_ob0);
	  if (sig_size > space_left)
	    {
