			 ...
			 vlib_frame_t * frame)
{
  ...
  for (i = 0; i < n_left - 8; i += 4)
    {
      vlib_prefetch_buffer_header (b[i + 4], LOAD);
      vlib_prefetch_buffer_header (b[i + 5], LOAD);
      vlib_prefetch_buffer_header (b[i + 6], LOAD);
      vlib_prefetch_buffer_header (b[i + 7], LOAD);

      next[i] = (esp_post_data (b[i]))->next_index;
      next[i + 1] = (esp_post_data (b[i + 1]))->next_index;
      next[i + 2] = (esp_post_data (b[i + 2]))->next_index;
      next[i + 3] = (esp_post_data (b[i + 3]))->next_index;
    }

  for (; i < n_left; i++)
    next[i] = (esp_post_data (b[i]))->next_index;

  vlib_buffer_enqueue_to_next (vm, node, from, next, frame->n_vectors);
  return frame->n_vectors;
}
