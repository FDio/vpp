...
vlib_node_registration_t ip4_esp_encrypt_node;
vlib_node_registration_t ip6_esp_encrypt_node;
		    ...
		    int is_ip6)
{
  ...
  if (PREDICT_FALSE (vec_len (empty_buffers) < n_left_from))
    {
      if (is_ip6)
	vlib_node_increment_counter (vm, ip6_esp_encrypt_node.index,
				     ESP_ENCRYPT_ERROR_NO_BUFFER,
				     n_left_from);
      else
	vlib_node_increment_counter (vm, ip4_esp_encrypt_node.index,
				     ESP_ENCRYPT_ERROR_NO_BUFFER,
				     n_left_from);
...
static uword
ip4_esp_encrypt_node_fn (vlib_main_t * vm,
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_esp_encrypt_node) = {
  .function = ip4_esp_encrypt_node_fn,
  .name = "ip4-esp-encrypt",
...
VLIB_NODE_FUNCTION_MULTIARCH (ip4_esp_encrypt_node, ip4_esp_encrypt_node_fn);

static uword
ip6_esp_encrypt_node_fn (vlib_main_t * vm,
			 vlib_node_runtime_t * node,
			 vlib_frame_t * from_frame)
{
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip6_esp_encrypt_node) = {
  .function = ip6_esp_encrypt_node_fn,
  .name = "ip6-esp-encrypt",
  .vector_size = sizeof (u32),
  .format_trace = format_esp_encrypt_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
...
VLIB_NODE_FUNCTION_MULTIARCH (ip6_esp_encrypt_node, ip6_esp_encrypt_node_fn);
/*
