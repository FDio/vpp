...
{
  if (vnet_crypto_get_crypto_op_id (ort->op_id) != VNET_CRYPTO_OP_NONE)
    {
      vnet_crypto_op_t *op;
      vec_add2_aligned (crypto_ops[0], op, 1, CLIB_CACHE_LINE_BYTES);
      vnet_crypto_op_init (op, vnet_crypto_get_crypto_op_id (ort->op_id));
		    ...
		    u16 async_next_node)
{
  ...
  while (n_left > 0)
      ...
      else
	{
      ...
      if (is_async)
	{
	  async_op = vnet_crypto_get_crypto_op_id (ort->op_id);
