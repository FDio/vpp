...
esp_encrypt_chain_integ (vlib_main_t * vm, ipsec_per_thread_data_t * ptd,
			 ipsec_sa_t * sa0, vlib_buffer_t * b,
			 vlib_buffer_t * lb, u8 icv_sz, u8 * start,
			 u32 start_len, u8 * digest, u16 * n_ch, u32 seq_hi)
{
  vnet_crypto_op_chunk_t *ch;
  vlib_buffer_t *cb = b;
  u32 n_chunks = 1;
...
esp_prepare_sync_op (vlib_main_t * vm, ipsec_per_thread_data_t * ptd,
		     ...
		     esp_encrypt_runtime_sa_data_t * st,
		     vlib_buffer_t * b, u16 index)
{
			 ...
			 esp_encrypt_runtime_sa_data_t *st, u16 async_next)
{
  ...
  post->encrypt_data.next_index = next;
		    ...
		    u16 async_next_node)
{
  ...
  while (n_left > 0)
      ...
      if (is_sched)
	{
	  ...
	  esp_post->encrypt_data.sad_index = sa_index0;
	  esp_post->encrypt_data.seq = rt.seq;
	  esp_post->encrypt_data.seq_hi = rt.seq_hi;
	  if (ipsec_sa_is_set_IS_AEAD (sa0))
	    esp_post->encrypt_data.gcm_iv_counter = rt.ctr_iv_counter;
	  goto trace;
	}
  ...
  while (n_left > 8)
    {
      ...
      next[0] = (esp_post_data (b[0]))->encrypt_data.next_index;
      next[1] = (esp_post_data (b[1]))->encrypt_data.next_index;
      next[2] = (esp_post_data (b[2]))->encrypt_data.next_index;
      next[3] = (esp_post_data (b[3]))->encrypt_data.next_index;
			  ...
			  u16 sched_next)
{
  ...
  while (n_left > 0)
      ...
      if (n_left > 2)
	{
      ...
      sa_index0 = post->encrypt_data.sad_index;
      ...
      if (sa_index0 != current_sa_index)
	{
      ...
      rt.seq = post->encrypt_data.seq;
      rt.seq_hi = post->encrypt_data.seq_hi;
      if (ipsec_sa_is_set_IS_AEAD (sa0))
	rt.ctr_iv_counter = post->encrypt_data.gcm_iv_counter;

      err = esp_encrypt_process (vm, node, sa0, b[0],
				 &post->encrypt_data.next_index, &rt, &st, lt,
				 buffer_data_size, is_tun);
  ...
  esp_process_chained_ops (vm, node, ptd->chained_crypto_ops, bufs,
			   nexts, ptd->chunks, drop_next);

  esp_process_ops (vm, node, ptd->integ_ops, bufs, nexts, drop_next);
  esp_process_chained_ops (vm, node, ptd->chained_integ_ops, bufs,
			   nexts, ptd->chunks, drop_next);

  /* every packet has to be enqueued into scheduler to maintain ordering */
  while (n < frame->n_vectors)
    {
      u16 n1 = vnet_scheduler_enqueue_buffers (vm,
