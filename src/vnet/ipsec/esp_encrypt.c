	  ...
	  if (USE_ASYNC_CRYPTO)
            {
              ...
              vec_validate_aligned (j, 0, CLIB_CACHE_LINE_BYTES);
              j->data = op[0];
              j->next_node = next_async_node_index;
              j->next_index = next[0];
              async_op = j;
		    ...
		    vlib_frame_t * frame, int is_ip6)
{
  ...
  while ((j = vnet_crypto_dequeue_one_job (q)) != NULL)
    {
      ASSERT(j);
