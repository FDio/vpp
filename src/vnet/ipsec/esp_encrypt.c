...
esp_prepare_async_frame (vlib_main_t * vm,
			 ...
			 u16 async_next, vlib_buffer_t * lb)
{
  ...
  u32 crypto_len = payload_len - icv_sz;
  u8 *tag = payload + crypto_len, *iv, *aad = 0;
  u8 flag = 0;
  u32 key_index;
  i16 crypto_start_offset = payload - b->data, integ_start_offset = 0;
  u16 integ_len_adj = 0;
  ...
  if (ipsec_sa_is_set_IS_AEAD (sa))
  ...
  else
    {
      /* cipher then hash */
      iv = payload - iv_sz;
      integ_len_adj = iv_sz + sizeof (esp_header_t);
      integ_start_offset = crypto_start_offset - integ_len_adj;
		    ...
		    u16 async_next)
{
  ...
  while (n_left > 0)
      ...
      if (is_async)
	{
	  ...
	  if (esp_prepare_async_frame (vm, &async_frame, sa0, b[0], esp,
