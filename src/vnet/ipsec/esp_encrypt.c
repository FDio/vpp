...
static_always_inline void
esp_fill_udp_hdr (ipsec_sa_enc_rt_t *sert, udp_header_t *udp, u16 len)
{
  clib_memcpy_fast (udp, &sert->udp_hdr, sizeof (udp_header_t));
 ...
 * details. */
static_always_inline void *
esp_generate_iv (ipsec_sa_enc_rt_t *sert, void *payload, int iv_sz)
{
  ASSERT (iv_sz >= sizeof (u64));
  u64 *iv = (u64 *) (payload - iv_sz);
  clib_memset_u8 (iv, 0, iv_sz);
  *iv = clib_pcg64i_random_r (&sert->iv_prng);
  return iv;
}
...
esp_encrypt_chain_integ (vlib_main_t *vm, ipsec_per_thread_data_t *ptd,
			 ipsec_sa_enc_rt_t *sert, vlib_buffer_t *b,
			 ...
			 u32 start_len, u8 *digest, u16 *n_ch)
{
  ...
  while (1)
      ...
      if (lb == cb)
	{
	  ...
	  if (sert->use_esn)
	    {
	      u32 seq_hi = clib_net_to_host_u32 (sert->seq_hi);
...
esp_prepare_sync_op (vlib_main_t *vm, ipsec_per_thread_data_t *ptd,
		     ...
		     vnet_crypto_op_t **integ_ops, ipsec_sa_enc_rt_t *sert,
...
{
  if (sert->crypto_op_id)
    {
      vnet_crypto_op_t *op;
      vec_add2_aligned (crypto_ops[0], op, 1, CLIB_CACHE_LINE_BYTES);
      vnet_crypto_op_init (op, sert->crypto_op_id);
      ...
      void *pkt_iv = esp_generate_iv (sert, payload, iv_sz);

      op->key_index = sert->cipher_key_index;
      op->user_data = bi;
	  ...
	  if (sert->is_aead)
	    {
	      ...
	      op->aad_len = esp_aad_enc_fill (op->aad, esp, sert, seq_hi);
	      op->tag = payload + crypto_len;
	      op->tag_len = 16;
	      if (PREDICT_FALSE (sert->is_null_gmac))
		{
			 ...
			 u16 async_next, vlib_buffer_t *lb)
{
  ...
  u8 *tag, *iv, *aad = 0;
  u8 flag = 0;
  const u32 key_index = sert->cipher_key_index;
  ...
  if (sert->is_ctr)
      ...
      if (sert->is_aead)
	{
	  ...
	  esp_aad_enc_fill (aad, esp, sert, sert->seq_hi);
	  if (PREDICT_FALSE (sert->is_null_gmac))
	    {
	      /* RFC-4543 ENCR_NULL_AUTH_AES_GMAC: IV is part of AAD */
		    ...
		    u16 async_next_node)
{
  ...
  while (n_left > 0)
      ...
      if (sa_index0 != current_sa_index)
	{
	  ...
	  esp_align = sert->esp_block_align;
	  icv_sz = sert->integ_icv_size;
	  iv_sz = sert->crypto_iv_size;
	  is_async = im->async_mode | sert->is_async;
	}
      ...
      if (PREDICT_FALSE ((u16) ~0 == sert->thread_index))
	{
      ...
      if (PREDICT_FALSE (thread_index != sert->thread_index))
	{
      ...
      if (sert->is_tunnel)
	{
	  ...
	  if (sert->udp_encap)
	    {
	      ...
	      esp_fill_udp_hdr (sert, (udp_header_t *) (payload - hdr_len),
	  ...
	  if (sert->is_tunnel_v6)
	    {
	      ...
	      clib_memcpy_fast (ip6, &sert->ip6_hdr, sizeof (ip6_header_t));

	      if (VNET_LINK_IP6 == lt)
		{
		  *next_hdr_ptr = IP_PROTOCOL_IPV6;
		  tunnel_encap_fixup_6o6 (sert->tunnel_flags,
					  (const ip6_header_t *) payload, ip6);
		}
	      else if (VNET_LINK_IP4 == lt)
		{
		  *next_hdr_ptr = IP_PROTOCOL_IP_IN_IP;
		  tunnel_encap_fixup_4o6 (sert->tunnel_flags, b[0],
	  ...
	  else
	    {
	      ...
	      clib_memcpy_fast (ip4, &sert->ip4_hdr, sizeof (ip4_header_t));

	      if (VNET_LINK_IP6 == lt)
		{
		  *next_hdr_ptr = IP_PROTOCOL_IPV6;
		  tunnel_encap_fixup_6o4_w_chksum (
		    sert->tunnel_flags, (const ip6_header_t *) payload, ip4);
		}
	      else if (VNET_LINK_IP4 == lt)
		{
		  *next_hdr_ptr = IP_PROTOCOL_IP_IN_IP;
		  tunnel_encap_fixup_4o4_w_chksum (
		    sert->tunnel_flags, (const ip4_header_t *) payload, ip4);
		}
	      else if (VNET_LINK_MPLS == lt)
		{
		  *next_hdr_ptr = IP_PROTOCOL_MPLS_IN_IP;
	  ...
	  if (sert->udp_encap && sert->is_tunnel_v6)
	    {
      ...
      else			/* transport mode */
	{
	  ...
	  if (udp)
	    {
	      udp_len = len - ip_len;
	      esp_fill_udp_hdr (sert, udp, udp_len);
	    }
      ...
      else
	{
      ...
      esp->spi = spi;
      esp->seq = clib_net_to_host_u32 (sert->seq);

      if (is_async)
	{
	  async_op = sert->async_op_id;
	  ...
	  esp_prepare_async_frame (vm, ptd, async_frames[async_op], sert, b[0],
				   esp, payload, payload_len, iv_sz, icv_sz,
				   from[b - bufs], sync_next[0], hdr_len,
				   async_next_node, lb);
	}
      else
	esp_prepare_sync_op (vm, ptd, crypto_ops, integ_ops, sert,
			     sert->seq_hi, payload, payload_len, iv_sz, icv_sz,
			     n_sync, b, lb, hdr_len, esp);
      ...
      if (PREDICT_FALSE (b[0]->flags & VLIB_BUFFER_IS_TRACED))
	{
	  ...
	  if (INDEX_INVALID == sa_index0)
	    ...
	    {
	      ipsec_sa_t *sa0 = ipsec_sa_get (sa_index0);
	      tr->sa_index = sa_index0;
	      tr->spi = sa0->spi;
	      tr->seq = sert->seq;
	      tr->sa_seq_hi = sert->seq_hi;
	      tr->udp_encap = sert->udp_encap;
