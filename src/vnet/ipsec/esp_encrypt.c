		    ...
		    u16 async_next_node)
{
  ...
  while (n_left > 0)
      ...
      if (PREDICT_FALSE (b[0]->flags & VLIB_BUFFER_IS_TRACED))
	{
	  ...
	  tr->spi = (sa0 ? sa0->spi : ~0);
	  tr->seq = (sa0 ? sa0->seq : ~0);
	  tr->sa_seq_hi = (sa0 ? sa0->seq_hi : ~0);
	  tr->udp_encap = (sa0 ? ipsec_sa_is_set_UDP_ENCAP (sa0) : ~0);
	  tr->crypto_alg = (sa0 ? sa0->crypto_alg : ~0);
	  tr->integ_alg = (sa0 ? sa0->integ_alg : ~0);
	}
