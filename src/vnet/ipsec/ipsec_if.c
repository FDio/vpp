		     ...
		     vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  next0 = IPSEC_OUTPUT_NEXT_IP4_ESP_ENCRYPT;
				  ...
				  u32 * sw_if_index)
{
  ...
  if (args->is_add)
	...
	(vnm->vlib_main, hi->tx_node_index, im->ip4_esp_encrypt_node_index,
	 IPSEC_OUTPUT_NEXT_IP4_ESP_ENCRYPT);

      ASSERT (slot == IPSEC_OUTPUT_NEXT_IP4_ESP_ENCRYPT);
