...
ipsec_tun_get_by_sw_if_index (u32 sw_if_index)
{
  ...
  p = hash_get (im->ipsec_if_by_sw_if_index, sw_if_index);

  if (p)
    {
      return (pool_elt_at_index (im->tunnel_interfaces, p[0]));
    }
				  ...
				  u32 * sw_if_index)
{
  ...
  if (args->is_add)
	...
	(vnm->vlib_main, hi->tx_node_index, im->esp4_encrypt_node_index, 0);
      ...
      hash_set (im->ipsec_if_by_sw_if_index, t->sw_if_index,
		t - im->tunnel_interfaces);
	...
	return VNET_API_ERROR_INVALID_VALUE;
      ...
      hash_unset (im->ipsec_if_by_sw_if_index, t->sw_if_index);
...
ipsec_tunnel_if_init (vlib_main_t * vm)
{
  ...
  im->ipsec_if_by_sw_if_index = hash_create (0, sizeof (uword));
