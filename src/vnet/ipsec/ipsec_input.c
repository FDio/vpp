				       ...
				       ipsec_spd_policy_type_t policy_type, u32 policy_index)
{
  ...
  if (old_seq == 0)
    __atomic_fetch_add (&im->ipsec4_in_spd_flow_cache_entries, 1, __ATOMIC_RELAXED);
...
ipsec_input_policy_match (ipsec_spd_t *spd, u32 sa, u32 da, u32 spi, ipsec_spd_policy_type_t pt)
{
  ...
  sa = clib_net_to_host_u32 (sa);
  da = clib_net_to_host_u32 (da);

  if (pt == IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT)
    {
      vec_foreach_index (policy_index, policy_indices)
	{
...
ipsec_ah_packet_process (vlib_main_t *vm, ipsec_main_t *im, ip4_header_t *ip0, ah_header_t *ah0,
			 ...
			 bool flow_cache_enabled, vlib_buffer_t **b, vlib_node_runtime_t *node,
...

{
  ...
  u8 has_space0;
  bool search_flow_cache = flow_cache_enabled;
  ...
  while (1)
	...
	if (search_flow_cache)
	  {
	    p0 = ipsec4_input_spd_find_flow_cache_entry (im, sa, da, policy_type);
	  }
	else
	  {
	    p0 = ipsec_input_policy_match (spd0, sa, da, spi, policy_type);

	    if (flow_cache_enabled && p0)
	      ipsec4_input_spd_add_flow_cache_entry (im, sa, da, policy_type, p0 - im->policies);
	  }
...
ipsec_esp_packet_process (vlib_main_t *vm, ipsec_main_t *im, ip4_header_t *ip0, udp_header_t *udp0,
			  ...
			  bool flow_cache_enabled, bool is_udp, vlib_buffer_t **b,
			  vlib_node_runtime_t *node, ipsec_input_counters_t *counters, u16 *next)
				  ...
				  vlib_frame_t * frame)
{
  ...
  bool flow_cache_enabled = im->input_flow_cache_flag;
  ...
  while (n_left_from > 0)
      ...
      if (ip0->protocol == IP_PROTOCOL_UDP)
	{
	      ...
	      (clib_host_to_net_u16 (4500) == udp0->dst_port))
	  {
	    ...
	    ipsec_esp_packet_process (vm, im, ip0, udp0, esp0, thread_index, spd0,
				      flow_cache_enabled, 1, b, node, &counters, next);
	  }
	}
      else if (PREDICT_TRUE (ip0->protocol == IP_PROTOCOL_IPSEC_ESP))
	{
	  esp0 = (esp_header_t *) ((u8 *) ip0 + ip4_header_bytes (ip0));
	  ipsec_esp_packet_process (vm, im, ip0, NULL, esp0, thread_index, spd0, flow_cache_enabled,
				    0, b, node, &counters, next);
	}
      else if (ip0->protocol == IP_PROTOCOL_IPSEC_AH)
	{
	  ah0 = (ah_header_t *) ((u8 *) ip0 + ip4_header_bytes (ip0));

	  ipsec_ah_packet_process (vm, im, ip0, ah0, thread_index, spd0, flow_cache_enabled, b,
				   node, &counters, next);
	}
      else
	{
