...
ipsec_input_policy_match (ipsec_spd_t *spd, u32 sa, u32 da,
			  ipsec_spd_policy_type_t policy_type, u8 protocol)
{
  ...
  vec_foreach (i, spd->policies[policy_type])
  {
	...
	ipsec4_input_spd_add_flow_cache_entry (im, sa, da, policy_type,
					       protocol, *i);
      }
    return p;
  }
				  ...
				  u8 protocol)
{
  ...
  return_policy:
	...
	ipsec4_input_spd_add_flow_cache_entry (
	  im, sa, da, IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT, protocol, *i);
      }
  ...
  while (n_left_from > 0)
	...
	esp_or_udp:
	  if (search_flow_cache) // attempt to match policy in flow cache
		...
		IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT, ip0->protocol);
	    }
		...
		clib_net_to_host_u32 (esp0->spi), ip0->protocol);
	    }
	  ...
	  if (search_flow_cache)
	    {
	      p0 = ipsec4_input_spd_find_flow_cache_entry (
		im, ip0->src_address.as_u32, ip0->dst_address.as_u32,
		IPSEC_SPD_POLICY_IP4_INBOUND_BYPASS, ip0->protocol);
	    }
	  ...
	  if (search_flow_cache)
	    {
	      p0 = ipsec4_input_spd_find_flow_cache_entry (
		im, ip0->src_address.as_u32, ip0->dst_address.as_u32,
		IPSEC_SPD_POLICY_IP4_INBOUND_DISCARD, ip0->protocol);
	    }
	...
	ah:
	  if (search_flow_cache)
		...
		spd0, clib_net_to_host_u32 (ip0->src_address.as_u32),
		clib_net_to_host_u32 (ip0->dst_address.as_u32),
		clib_net_to_host_u32 (ah0->spi), ip0->protocol);
	    }
