	  ...
	  if (PREDICT_FALSE (ip0->protocol == IP_PROTOCOL_UDP))
				...
				spd0->fp_spd.ip4_in_lookup_hash_idx))
		{
		  ipsec_fp_in_5tuple_from_ip4_range (
		    &tuples[0], ip0->src_address.as_u32,
		    ip0->dst_address.as_u32, clib_net_to_host_u32 (esp0->spi),
		  ...
		  if (search_flow_cache && p0 == NULL)
		    {
		      ...
		      // goto udp_encp_esp;
		       ...
		       ((clib_net_to_host_u16 (udp0->dst_port) !=
			 clib_host_to_net_u16 (sa0->udp_hdr.dst_port)) &&
			(clib_net_to_host_u16 (udp0->checksum) == 0)))
		{
		  goto ipsec_unprocessed;
		}

	      else
		{
		  // if UDP non ESP encap packet, just forward it.
		  goto ipsec_bypassed; // ipsec_bypassed udp_unprotected;
		}
	    }
