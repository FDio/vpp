...
always_inline void
ipsec_goto_ah_trace (vlib_buffer_t **b, vlib_node_runtime_t *node,
  ...
  while (policy_found == 0)
      ...
      if (policy_found == 0)
	{
	  ...
	  has_space0 = vlib_buffer_has_space (
	    b[0], (clib_address_t) (ah0 + 1) - (clib_address_t) ip0);
	  ...
	  if (PREDICT_TRUE ((p0 != NULL) & (has_space0)))
		...
		clib_net_to_host_u16 (ip0->length));
	      ...
	      ipsec_goto_ah_trace (b, node, vm, ip0, ah0, has_space0, spd0, p0,
				   pi0);
	    }
	  else
	    {
      ...
      if (search_flow_cache && p0 == NULL)
	{
      ...
      else if (search_flow_cache == false && p0 == NULL && policy_found == 0)
	{
	  policy_found = 1;
