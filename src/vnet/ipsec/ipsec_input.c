				  ...
				  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      if (ip0->protocol == IP_PROTOCOL_UDP)
	{
	  ...
	  /*
	   * As per rfc3948 in UDP Encapsulated Header, UDP dst port is 4500,
	   * UDP checksum must be Zero, and receivers must not depend upon UPD
	   * checksum inside ESP header, SPI ID value MUST NOT be a zero value.
	   */
	  if (udp0->dst_port == 4500)
	    {
