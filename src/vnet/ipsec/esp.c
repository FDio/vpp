...
always_inline void
esp_finish_encrypt_one (vlib_main_t * vm, ipsec_job_desc_t * job,
...
always_inline void
esp_finish_encrypt_inline (vlib_main_t * vm, ipsec_main_t * im,
			   ...
			   u32 next_index_interface_output)
{
  while (n_jobs)
	...
	{
	  esp_finish_encrypt_one (vm, job, thread_index, is_ip6,
				  next_index_drop,
...
void
esp_finish_encrypt (vlib_main_t * vm, ipsec_main_t * im, u16 * next,
		    ipsec_job_desc_t * job, u32 n_jobs,
		    int thread_index, int is_ip6, u32 next_index_drop,
		    u32 next_index_interface_output)
{
  if (is_ip6)
    return esp_finish_encrypt_inline (vm, im, next, job, n_jobs,
...
always_inline void
esp_encrypt_prepare_jobs_inline (vlib_main_t * vm, ipsec_main_t * im,
				 ipsec_proto_main_t * em, vlib_buffer_t ** b,
				 ipsec_job_desc_t * job, u32 n_jobs,
				 int is_ip6, int (*random_bytes) (u8 * dest,
								  int len),
				 u32 next_index_drop,
...
void
esp_encrypt_prepare_jobs (vlib_main_t * vm, ipsec_main_t * im,
			  ipsec_proto_main_t * em, vlib_buffer_t ** b,
			  ipsec_job_desc_t * job, u32 n_jobs, int is_ip6,
			  int (*random_bytes) (u8 * dest, int len),
			  u32 next_index_drop, u32 next_index_ip4_lookup,
			  u32 next_index_ip6_lookup)
{
  if (is_ip6)
    return esp_encrypt_prepare_jobs_inline (vm, im, em, b, job, n_jobs,
					    1 /*is_ip6 */ , random_bytes,
					    next_index_drop,
					    next_index_ip4_lookup,
					    next_index_ip6_lookup);
  else
    return esp_encrypt_prepare_jobs_inline (vm, im, em, b, job, n_jobs,
					    0 /*is_ip6 */ , random_bytes,
					    next_index_drop,
					    next_index_ip4_lookup,
					    next_index_ip6_lookup);
}
	    ...
	    u8 * dst, int len, u8 * key, u8 * iv, int is_encrypt)
{
  ...
  if (!EVP_CipherUpdate (ctx, dst, &out_len, src, len))
    {
      return -1;
    }
...
always_inline void
esp_decrypt_prepare_jobs_inline (vlib_main_t * vm, ipsec_main_t * im,
				 ipsec_proto_main_t * em, vlib_buffer_t ** b,
				 ipsec_job_desc_t * job, u32 n_jobs,
...
void
esp_decrypt_prepare_jobs (vlib_main_t * vm, ipsec_main_t * im,
			  ipsec_proto_main_t * em, vlib_buffer_t ** b,
			  ipsec_job_desc_t * job, u32 n_jobs, int is_ip6,
			  u32 next_index_drop)
{
  if (is_ip6)
    esp_decrypt_prepare_jobs_inline (vm, im, em, b, job, n_jobs,
...
always_inline void
esp_finish_decrypt_one (vlib_main_t * vm, ipsec_job_desc_t * job, int is_ip6,
...
void
esp_finish_decrypt (vlib_main_t * vm, u16 * next, ipsec_job_desc_t * job,
  ...
  while (n_jobs)
    {
      if (IPSEC_ERR_OK == job->error)
	{
	  esp_finish_decrypt_one (vm, job, is_ip6, next_index_drop,
