...
ipsec_tun_protect_update (u32 sw_if_index, u32 sa_in_id, u32 sa_out_id)
{
  ...
  if (INDEX_INVALID == iti)
	...
	return (-1);
      ...
      rv = hw_class->tun_desc (sw_if_index, &it->it_tun_src,
			       &it->it_tun_dst, &it->it_decap_node_index);
	...
	return (-2);
      ...
      vnet_feature_enable_disable ("ip4-output",
				   "esp4-encrypt-tun",
				   sw_if_index, 1,
				   &it->it_sa[IPSEC_PROTECT_DIR_OUTBOUND],
				   sizeof (it->it_sa
					   [IPSEC_PROTECT_DIR_OUTBOUND]));
      vnet_feature_enable_disable ("ip6-output", "esp6-encrypt-tun",
				   sw_if_index, 1,
				   &it->it_sa[IPSEC_PROTECT_DIR_OUTBOUND],
				   sizeof (it->it_sa
					   [IPSEC_PROTECT_DIR_OUTBOUND]));
	...
	{
	  it->it_crypto_src = sa->tunnel_dst_addr;
	  it->it_crypto_dst = sa->tunnel_src_addr;
	}
      else
	{
	  it->it_crypto_src = it->it_tun_src;
	  it->it_crypto_dst = it->it_tun_dst;
	}

      if (ip46_address_is_ip4 (&it->it_crypto_dst))
	{
	  ipsec4_tunnel_key_t key = {
	    .remote_ip = it->it_crypto_dst.ip4.as_u32,
	    .spi = sa->spi,
	  };
	  hash_set (im->tun4_protect_by_key, &key, it - ipsec_protect_pool);
	  esp_decrypt_node =
      ...
      else
	{
	  ipsec6_tunnel_key_t key = {
	    .remote_ip = it->it_crypto_dst.ip6,
	    .spi = sa->spi,
	  };
...
{
  .path = "ipsec tunnel protect",.function =
    ipsec_tun_protect_cmd,.short_help =
