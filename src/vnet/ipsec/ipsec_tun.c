...
ipsec_tun_protect_feature_set (ipsec_tun_protect_t * itp, u8 enable)
{
  ...
  ITP_DBG2 ("%s %s on %U", (enable ? "enable" : "disable"),
	    enc_node,
  ...
  if (itp->itp_flags & IPSEC_PROTECT_L2)
    {
      rv = vnet_feature_enable_disable ("ethernet-output", enc_node,
					itp->itp_sw_if_index, enable,
					NULL, 0);
    }
  else
    {
      rv = vnet_feature_enable_disable ("ip4-output", enc_node,
					itp->itp_sw_if_index, enable,
					NULL, 0);
      rv = vnet_feature_enable_disable ("ip6-output", enc_node,
					itp->itp_sw_if_index, enable,
					NULL, 0);
    }
  ASSERT (!rv);
...
static void
ipsec_tun_protect_update_from_nhrp (ipsec_tun_protect_t * itp,
			  ...
			  const ip_address_t * nh, u32 sa_out, u32 * sas_in)
{
  ...
  if (INDEX_INVALID == itpi)
      ...
      if (ip46_address_is_zero (&itp->itp_tun.dst))
	{
	  ...
	  ipsec_tun_protect_update_from_nhrp
	    (itp, nhrp_entry_find (sw_if_index, &peer));
	}
...
static void
ipsec_tun_nhrp_entry_added (const nhrp_entry_t * ne)
{
  ...
  peer46 = nhrp_entry_get_peer (ne);
  ip_address_from_46 (peer46,
		      (ip46_address_is_ip4 (peer46) ?
		       FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6), &peer);

  itpi = ipsec_tun_protect_find (nhrp_entry_get_sw_if_index (ne), &peer);
  ...
  ipsec_tun_protect_update_from_nhrp (itp, ne);
  ipsec_tun_protect_set_crypto_addr (itp);
  ipsec_tun_protect_rx_db_add (&ipsec_main, itp);

  ITP_DBG (itp, "nhrp-added");
}

static void
ipsec_tun_nhrp_entry_deleted (const nhrp_entry_t * ne)
{
  const ip46_address_t *peer46;
  ipsec_tun_protect_t *itp;
  ip_address_t peer;
  ...
  itp = ipsec_tun_protect_get (itpi);
  ipsec_tun_protect_rx_db_remove (&ipsec_main, itp);
  ipsec_tun_protect_update_from_nhrp (itp, NULL);
  ipsec_tun_protect_set_crypto_addr (itp);

  ITP_DBG (itp, "nhrp-removed");
}
...
const static nhrp_vft_t ipsec_tun_nhrp_vft = {
  .nv_added = ipsec_tun_nhrp_entry_added,
  .nv_deleted = ipsec_tun_nhrp_entry_deleted,
};
...
ipsec_tunnel_protect_init (vlib_main_t * vm)
{
  ...
  nhrp_register (&ipsec_tun_nhrp_vft);
