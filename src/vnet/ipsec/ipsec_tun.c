...
{
  u32 sai = itp->itp_sa[IPSEC_PROTECT_DIR_OUTBOUND];
...
ipsec_tun_protect_db_add (ipsec_main_t * im, const ipsec_protect_t * itp)
{
  ...
  sa = ipsec_sa_get (itp->itp_sa[IPSEC_PROTECT_DIR_INBOUND]);
...
ipsec_tun_protect_update (u32 sw_if_index, u32 sa_in_id, u32 sa_out_id)
{
  ...
  if (INDEX_INVALID == itpi)
	...
	return (-1);
      ...
      itp->itp_sa[IPSEC_PROTECT_DIR_INBOUND] = sa_in;
      itp->itp_sa[IPSEC_PROTECT_DIR_OUTBOUND] = sa_out;

      rv = hw_class->tun_desc (sw_if_index, &itp->itp_tun.src,
			       &itp->itp_tun.dst, &itp->itp_decap_node_index);
      ...
      if (ipsec_sa_is_set_IS_TUNNEL (sa))
	{
	  ...
	  itp->itp_flags = IPSEC_PROTECT_TUN;
	}
      else
	{
