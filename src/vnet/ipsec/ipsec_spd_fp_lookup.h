  ...
  if (policy->policy == IPSEC_POLICY_ACTION_PROTECT)
      ...
      if (ipsec_sa_is_set_IS_TUNNEL (s))
	{
          if (!policy->is_ipv6)
            ...
            {
               if (ip6_address_compare (&match->ip6_laddr, &s->tunnel.t_dst.ip.ip6) != 0)
                 return (0);

               if (ip6_address_compare (&match->ip6_raddr, &s->tunnel.t_src.ip.ip6) != 0)
	         return (0);
           }
	}
      else
        {
          if (!policy->is_ipv6)
            ...
            {
               if (ip6_address_compare (&match->ip6_laddr, &policy->laddr.start.ip6) < 0)
                 return (0);

               if (ip6_address_compare (&policy->laddr.stop.ip6, &match->ip6_laddr) < 0)
                 return (0);

               if (ip6_address_compare (&match->ip6_raddr, &policy->raddr.start.ip6) < 0)
                 return (0);
