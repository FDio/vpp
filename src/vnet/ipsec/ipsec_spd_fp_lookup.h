...
single_rule_match_5tuple (ipsec_policy_t *policy, ipsec_fp_5tuple_t *match)
{
  ...
  if (PREDICT_FALSE (policy->protocol &&
		     (policy->protocol != match->protocol)))
    return (0);

  if (!policy->is_ipv6)
    {
      if (PREDICT_FALSE (match->laddr.as_u32 < policy->laddr.start.ip4.as_u32))
	return (0);

      if (PREDICT_FALSE (match->laddr.as_u32 > policy->laddr.stop.ip4.as_u32))
	return (0);

      if (PREDICT_FALSE (match->raddr.as_u32 < policy->raddr.start.ip4.as_u32))
	return (0);

      if (PREDICT_FALSE (match->raddr.as_u32 > policy->raddr.stop.ip4.as_u32))
	return (0);
    }
  else
    {
