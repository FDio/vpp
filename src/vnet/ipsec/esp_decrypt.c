		    ...
		    int is_ip6)
{
  ...
  if (PREDICT_FALSE (vec_len (empty_buffers) < n_left_from))
    {
      if (is_ip6)
	vlib_node_increment_counter (vm, ip6_esp_decrypt_node.index,
				     ESP_DECRYPT_ERROR_NO_BUFFER,
				     n_left_from);
      else
	vlib_node_increment_counter (vm, ip4_esp_decrypt_node.index,
				     ESP_DECRYPT_ERROR_NO_BUFFER,
				     n_left_from);
	      ...
	      if (PREDICT_FALSE (rv))
		{
		  ...
		  if (is_ip6)
		    vlib_node_increment_counter (vm,
						 ip6_esp_decrypt_node.index,
						 ESP_DECRYPT_ERROR_REPLAY, 1);
		  else
		    vlib_node_increment_counter (vm,
						 ip4_esp_decrypt_node.index,
						 ESP_DECRYPT_ERROR_REPLAY, 1);
		  o_bi0 = i_bi0;
		  to_next[0] = o_bi0;
		  to_next += 1;
...
static uword
ip4_esp_decrypt_node_fn (vlib_main_t * vm,
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_esp_decrypt_node) = {
  .function = ip4_esp_decrypt_node_fn,
  .name = "ip4-esp-decrypt",
...
VLIB_NODE_FUNCTION_MULTIARCH (ip4_esp_decrypt_node, ip4_esp_decrypt_node_fn);

static uword
ip6_esp_decrypt_node_fn (vlib_main_t * vm,
			 vlib_node_runtime_t * node,
			 vlib_frame_t * from_frame)
{
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip6_esp_decrypt_node) = {
  .function = ip6_esp_decrypt_node_fn,
  .name = "ip6-esp-decrypt",
  .vector_size = sizeof (u32),
  .format_trace = format_esp_decrypt_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
...
VLIB_NODE_FUNCTION_MULTIARCH (ip6_esp_decrypt_node, ip6_esp_decrypt_node_fn);
/*
