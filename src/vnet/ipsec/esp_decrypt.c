		...
		vnet_crypto_op_t * ops, vlib_buffer_t * b[], u16 * nexts)
{
  ...
  n_fail = n_ops - vnet_crypto_submit_ops (vm, op, n_ops);
  ...
  while (n_fail)
	...
	{
	  u32 bi = op->user_data;
	  b[bi]->error = node->errors[ESP_DECRYPT_ERROR_CRYPTO_ENGINE_ERROR];
	  nexts[bi] = ESP_DECRYPT_NEXT_DROP;
	  n_fail--;
	}
      op++;
	      ...
	      esp_decrypt_packet_data2_t * pd2)
{
  ...
  u16 first_sz = pd->icv_sz - last_sz;
...
static_always_inline void
esp_crypto_chain_sync (vlib_main_t * vm, ipsec_per_thread_data_t * ptd,
		       esp_decrypt_packet_data_t * pd,
		       esp_decrypt_packet_data2_t * pd2,
		       vnet_crypto_op_t * op, vlib_buffer_t * b, u8 * payload,
		       u16 len, int is_aead)
{
  ...
  while (1)
      ...
      if (pd2->lb == cb)
	{
	  if (is_aead)
	    {
	      if (pd2->lb->current_length < pd->icv_sz)
		{
		  op->tag = esp_move_icv (vm, b, pd, pd2);
		...
		op->tag = vlib_buffer_get_tail (pd2->lb) - pd->icv_sz;
	    }

	  if (pd2->icv_removed)
	    ch->len = cb->current_length;
	  else
	    ch->len = cb->current_length - pd->icv_sz;
	}
      else
	ch->len = cb->current_length;
...
static_always_inline void
esp_crypto_chain_async (vlib_main_t * vm, ipsec_per_thread_data_t * ptd,
			esp_decrypt_packet_data_t * pd,
			esp_decrypt_packet_data2_t * pd2,
			vnet_crypto_op_t * op, vlib_buffer_t * b,
			u8 * payload, u16 curr_len, int is_aead)
{
  vlib_buffer_t *cb = b;
  u32 len = curr_len - pd->iv_sz + pd->icv_sz;
  op->flags |= VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS;
  cb = vlib_get_buffer (vm, cb->next_buffer);
  ...
  while (1)
      ...
      if (pd2->lb == cb)
	{
	  if (is_aead)
		...
		{
		  op->tag = esp_move_icv (vm, b, pd, pd2);
		  len -= pd->icv_sz - pd2->lb->current_length;
		  break;
		}
	      else
		op->tag = vlib_buffer_get_tail (pd2->lb) - pd->icv_sz;
	  ...
	  if (pd2->icv_removed)
	    len += cb->current_length;
	  else
	    len += cb->current_length - pd->icv_sz;
	}
      else
	len += cb->current_length;
			 ...
			 u16 * next, int is_ip6, int is_tun, int is_async)
{
  ...
  if ((pd->flags & tun_flags) == 0 && !is_tun)	/* transport mode */
	...
	clib_memcpy_le64 (ip, old_ip, ip_hdr_sz);
      ...
      b->current_length = pd->current_length + ip_hdr_sz - adv;
		    ...
		    int is_ip6, int is_tun, u16 async_next)
{
  ...
  while (n_left > 0)
      ...
      if (vnet_buffer (b[0])->ipsec.sad_index != current_sa_index)
	{
	  ...
	  is_async = vnet_crypto_is_op_async (sa0->crypto_dec_op_id);
	}
      ...
      if (PREDICT_TRUE (thread_index != sa0->decrypt_thread_index))
	{
      ...
      current_protect_index = vnet_buffer (b[0])->ipsec.protect_index;
      ...
      else
	{
      ...
      if (is_async)
	crypto_ops = &ptd->async_ops;

      pd->current_length = b[0]->current_length;

      /* we need 4 extra bytes for HMAC calculation when ESN are used */
      /* Chained buffers can process ESN as a separate chunk */
      if (pd2->lb == b[0] && ipsec_sa_is_set_USE_ESN (sa0) && cpd.icv_sz &&
	  (pd2->lb->current_data + pd2->lb->current_length + 4
	   > buffer_data_size))
	{
      ...
      if (PREDICT_TRUE (sa0->integ_op_id != VNET_CRYPTO_OP_NONE))
	{
	  ...
	  /* for same reason as esp_encrypt, there is no async crypto handling
	     for integ ops */
	  ...
	  if (pd->is_chain)
	    {
	      ...
	      if (pd2->lb->current_length < cpd.icv_sz)
		op->digest = esp_move_icv (vm, b[0], pd, pd2);
	      else
		op->digest = vlib_buffer_get_tail (pd2->lb) - cpd.icv_sz;
      ...
      if (sa0->crypto_dec_op_id != VNET_CRYPTO_OP_NONE)
	{
	  ...
	  if (is_async)
		...
		&(esp_post_data (b[0]))->decrypt_data;
	      esp_decrypt_packet_data2_t *async_pd2 = esp_post_data2 (b[0]);
	      op->flags |= VNET_CRYPTO_OP_FLAG_ASYNC;
	      op->async_src.bi = from[b - bufs];
	      op->async_src.next = async_next;
	      op->async_src.crypto_start_offset =
		payload + cpd.iv_sz - b[0]->data;
	      op->adst = op->asrc;
	      ...
	      *async_pd2 = *pd2;
	      ...
	      if (pd->is_chain)
		{
		  esp_crypto_chain_async (vm, ptd, async_pd, async_pd2, op,
					  b[0], payload, len,
					  ipsec_sa_is_set_IS_AEAD (sa0));
		}
	      async_pd->protect_index = current_protect_index;
	    }
	  else
	    {
	      op->src = op->dst = payload += cpd.iv_sz;
	      if (pd->is_chain)
		esp_crypto_chain_sync (vm, ptd, pd, pd2, op, b[0], payload,
				       len, ipsec_sa_is_set_IS_AEAD (sa0));
	    }
	}
  ...
  esp_submit_ops (vm, node, ptd->async_ops, bufs, nexts);
  ...
  while (n_left)
      ...
      if (PREDICT_FALSE (b[0]->flags & VLIB_BUFFER_IS_TRACED))
	{
	  ...
	  if (next[0] != ESP_DECRYPT_NEXT_PENDING)
			 ...
			 vlib_frame_t * from_frame, int is_ip6, int is_tun)
{
  ...
  while (n_left > 0)
      ...
      if (n_left > 2)
	{
      ...
      if (pd->is_chain)
	esp_decrypt_post_crypto (vm, node, pd, 0, b[0], next, is_ip6, is_ip6,
				 1);
      else
	{
