			  ...
			  if (pd2->icv_removed)
			    {
			      ...
			      tmp_b =
				vlib_get_buffer (vm, pd2->free_buffer_index);
			      esn = tmp_b->data - sz;
			    }
			  else
			    {
	  ...
	  while (1)
	      ...
	      if (pd2->lb == cb)
		{
		  if (ipsec_sa_is_set_IS_AEAD (sa0))
			...
			{
			  u16 dif = 0;
			  op->tag = esp_move_icv (vm, b, pd2, icv_sz, &dif);
			...
			op->tag = vlib_buffer_get_tail (pd2->lb) - icv_sz;
		    }
				 ...
				 u16 async_next)
{
  ...
  u32 key_index;
  u32 crypto_len = len - esp_sz - iv_sz;
  i16 crypto_start_offset = payload - b->data + esp_sz + iv_sz;
      ...
      if (pd->is_chain)
	{
	  ...
	  while (1)
	      ...
	      if (pd2->lb == cb)
		{
		  ...
		  if (pd2->icv_removed)
		    crypto_len += cb->current_length;
		  else
		    crypto_len += cb->current_length - icv_sz;
		}
	      else
		crypto_len += cb->current_length;
  ...
  integ_len_adj = esp_sz + iv_sz;

  if (pd->is_chain)
    {
      vlib_buffer_t *cb = b;
      crypto_len = pd->current_length - crypto_start_offset +
	integ_start_offset;
      flags |= VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS;
      /* special case when ICV is splitted and needs to be reassembled
       * first -> move it to the last buffer. Also take into account
      ...
      if (pd2->lb->current_length < icv_sz)
	{
	  ...
	  if (ipsec_sa_is_set_USE_ESN (sa0))
	      ...
	      else
		{
		   ...
		   * (with ICV data) */
		  ASSERT (pd2->icv_removed);
		  vlib_buffer_t *tmp = vlib_get_buffer (vm,
							pd2->free_buffer_index);
		  clib_memcpy_fast (vlib_buffer_get_current (tmp) - sz,
				    &seq_hi, sz);
		  crypto_len =
		    b->current_length - crypto_start_offset +
		    integ_start_offset;
		  extra_esn = 1;
		}
	    }

	  if (!extra_esn && pd2->lb == b)
	    {
	      /* we now have a single buffer of crypto data, adjust
	       * the length (second buffer contains only ICV) */
	      crypto_len = b->current_length;
	      goto out;
	    }
	}
      else
	  ...
	  if (pd2->lb == cb)
	      ...
	      if (ipsec_sa_is_set_USE_ESN (sa0))
		{
		  ...
		  if (space_left < sz)
			...
			{
			  clib_memcpy_fast (tmp, tag, ESP_MAX_ICV_SIZE);
			  clib_memcpy_fast (tag, &seq_hi, sz);
			  clib_memcpy_fast (tag + sz, tmp, ESP_MAX_ICV_SIZE);
			  tag += sz;
			}
		      integ_len_adj += sz;
		    }
    ...
    esp_insert_esn (vm, sa0, pd2, &integ_len_adj, &tag, &len, b, payload);
  ...
  while (n_left > 0)
      ...
      if (vnet_buffer (b[0])->ipsec.sad_index != current_sa_index)
	{
	  ...
	  if (is_async)
		...
		{
		  vnet_crypto_async_submit_open_frame (vm, async_frame);
		}
	      async_frame =
      ...
      if (pd->current_length < cpd.icv_sz + esp_sz + cpd.iv_sz)
	{
      ...
      if (is_async)
	{
	  int ret = esp_decrypt_prepare_async_frame (vm, ptd, &async_frame,
						     sa0, payload, len,
						     cpd.icv_sz,
						     cpd.iv_sz,
						     pd, pd2,
	...
	esp_decrypt_prepare_sync_op (vm, ptd, &crypto_ops, &integ_ops, op,
				     sa0, payload, len, cpd.icv_sz,
				     cpd.iv_sz, pd, pd2, b[0], b - bufs);
      /* next */
    next:
      n_left -= 1;
      next += 1;
