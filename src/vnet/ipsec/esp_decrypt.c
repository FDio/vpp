      ...
      while (n_left_from > 0)
	{
	  ...
	  if (PREDICT_TRUE (sa0->integ_alg != IPSEC_INTEG_ALG_NONE))
		...
		em->ipsec_proto_main_integ_algs[sa0->integ_alg].trunc_size;
	      ...
	      //u8 *icv = vlib_buffer_get_current (ib[0]) + ib[0]->current_length - icv_size;
	      ib[0]->current_length -= icv_size;

	      hmac_calc (sa0->integ_alg, sa0->integ_key.data,
			 sa0->integ_key.len, (u8 *) esp0,
			 ib[0]->current_length, sig, sa0->use_esn,
			 sa0->seq_hi);
	  ...
	  if ((sa0->crypto_alg >= IPSEC_CRYPTO_ALG_AES_CBC_128 &&
	      ...
	      if (PREDICT_FALSE (!sa0->is_tunnel && !sa0->is_tunnel_ip6))
		{
	      ...
	      f0 =
		(esp_footer_t *) ((u8 *) vlib_buffer_get_current (ob[0]) +
				  ob[0]->current_length);
