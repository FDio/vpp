	  ...
	  if (IP_PROTOCOL_AGGFRAG == next_header)
	    {
	       ...
	       *
	       * New plan (260916):
	       * Decrypt path IPTFS always gets these fields from
	       * esp_post_data (b)->decrypt_data.* (which is in the
	       * vnet buffer's opaque area).
	       *
	       * These fields are already set in the async case.
	       * For sync case, we copy from the (stack-based) pd
	       * into esp_post_data (b)->decrypt_data.*
	       */
	      if (!is_async)
		{
		  esp_post_data (b)->decrypt_data.seq_hi = pd->seq_hi;
		  esp_post_data (b)->decrypt_data.seq = pd->seq;
		  esp_post_data (b)->decrypt_data.sa_index = pd->sa_index;
		}
	    }
	}
      else
