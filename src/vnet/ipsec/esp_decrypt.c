...
esp_buffer_normalize (vlib_main_t *vm, vlib_buffer_t *first, u16 n_bytes)
{
  ...
  if (lb == first)
    ASSERT (lb->current_length >= ESP_DECRYPT_NORMALIZE_SIZE);
  else if (lb->current_length < ESP_DECRYPT_NORMALIZE_SIZE)
    {
      n_from_prev = ESP_DECRYPT_NORMALIZE_SIZE - lb->current_length;
	...
	first->total_length_not_including_first_buffer += n_from_prev;
      clib_memset (vlib_buffer_get_tail (before_last), 0, n_from_prev);
    }
  ...
  clib_memcpy_fast (tmp_b->data, vlib_buffer_get_tail (lb) - ESP_DECRYPT_NORMALIZE_SIZE,
		    ESP_DECRYPT_NORMALIZE_SIZE);
  tmp_b->current_length = ESP_DECRYPT_NORMALIZE_SIZE;
  lb->current_length -= ESP_DECRYPT_NORMALIZE_SIZE;
  if (lb == first)
    first->total_length_not_including_first_buffer += ESP_DECRYPT_NORMALIZE_SIZE;
...
esp_decrypt_prepare_async_frame (vlib_main_t *vm, ipsec_per_thread_data_t *ptd,
				 vnet_crypto_async_frame_t *f, ipsec_sa_inb_rt_t *irt, u8 *payload,
				 u16 len, u8 icv_sz, u8 iv_sz, esp_decrypt_packet_data_t *pd,
				 u32 bi, vlib_buffer_t *b, u16 async_next)
{
  const u8 esp_sz = sizeof (esp_header_t);
  ...
  *async_pd = *pd;
  ...
  vnet_crypto_async_add_to_frame (vm, f, ctx, crypto_len, integ_len, crypto_start_offset,
		    ...
		    u16 async_next_node)
{
  ...
  int is_async = 0;
  vnet_crypto_alg_t async_alg = ~0;
  vnet_crypto_engine_id_t async_engine = VNET_CRYPTO_ENGINE_ID_NONE;
  vnet_crypto_async_frame_t *async_f = 0;
  async_frame_t async_frames[VLIB_FRAME_SIZE];
  u32 n_async_frames = 0;
  ...
  while (n_left > 0)
      ...
      if (is_async)
	{
	  ...
	  async_alg = irt->ctx->alg;
	  async_engine = vnet_crypto_ctx_get_engine (irt->ctx, VNET_CRYPTO_HANDLER_TYPE_ASYNC);
	  async_f = 0;

	  for (i = 0; i < n_async_frames; i++)
	    if (async_frames[i].alg == async_alg && async_frames[i].engine == async_engine &&
		!vnet_crypto_async_frame_is_full (async_frames[i].frame))
	      {
		async_f = async_frames[i].frame;
		break;
	      }
	  ...
	  if (async_f == 0)
	    {
	      async_f = vnet_crypto_async_get_frame (vm, async_alg, VNET_CRYPTO_OP_TYPE_DECRYPT);
	      if (PREDICT_FALSE (!async_f))
		{
	      ...
	      vec_add1 (ptd->async_frames, async_f);
	      async_frames[n_async_frames].alg = async_alg;
	      async_frames[n_async_frames].engine = async_engine;
	      async_frames[n_async_frames].frame = async_f;
	      n_async_frames++;
	    }

	  err = esp_decrypt_prepare_async_frame (vm, ptd, async_f, irt, payload, len,
						 irt->integ_icv_size, irt->cipher_iv_size, pd,
						 from[b - bufs], b[0], async_next_node);
