			 ...
			 int is_ip6, int is_tun, int is_async)
{
  ...
  if (irt->is_transport && !is_tun) /* transport mode */
	       ...
	       (u16) ~0)
	{
	  ...
	  if (is_async && (IP_PROTOCOL_AGGFRAG == next_header))
	    {
	      /*
		    ...
		    u16 async_next_node)
{
  ...
  while (n_left > 0)
	...
	irt, pd->seq, ~0, false, &pd->seq_hi);
      ...
      /* For some reason GCM is zeroing this value out */
      {
	if (ipsec_sa_is_IPTFS (ipsec_sa_get (current_sa_index)))
	  vnet_buffer (b[0])->ipsec.iptfs_esp_seq = ((u64) pd->seq_hi << 32) | pd->seq;
      }
