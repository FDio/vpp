				 ...
				 u16 async_next)
{
  ...
  u16 offset = payload - b->data;
  u32 key_index;
  u32 total_len = offset + len;
  i16 crypto_start_offset = offset + esp_sz + iv_sz, integ_start_offset = 0;
  u8 flags = 0;
  ...
  if (ipsec_sa_is_set_IS_AEAD (sa0))
  ...
  else
    {
      ...
      integ_start_offset = offset;
    }

  if (pd->is_chain)
    {
      vlib_buffer_t *cb = b;
      total_len = pd->current_length;
      /* special case when ICV is splitted and needs to be reassembled
       * first -> move it to the last buffer. Also take into account
       * that ESN needs to be added after encrypted data and may or
       * may not fit in the tail.*/
      ...
      while (1)
	{
	  if (pd2->lb == cb)
	    {
	      if (pd2->icv_removed)
		total_len += cb->current_length;
	      else
		total_len += cb->current_length - icv_sz;
	      if (ipsec_sa_is_set_USE_ESN (sa0))
		{
		  u32 seq_hi = clib_host_to_net_u32 (sa0->seq_hi);
		  u8 tmp[ESP_MAX_ICV_SIZE], sz = sizeof (sa0->seq_hi);
    ...
    esp_insert_esn (vm, sa0, pd2, &total_len, &tag, &len, b, payload);
...
out:
  ...
  *async_pd2 = *pd2;
  ...
  return vnet_crypto_async_add_to_frame (vm, f, key_index, total_len,
					 crypto_start_offset,
					 integ_start_offset,
