  ...
  vl_api_sw_interface_ip6nd_ra_config_v2_t *mp)
{
  ...
  vlib_main_t *vm = vlib_get_main ();
  int rv = 0;
  u8 defaults, send_radv, managed, other, ll_option, send_unicast,
    use_lifetime;

  defaults = mp->defaults == 1;
  send_radv = mp->send_radv == 1;
  managed = mp->managed == 1;
  other = mp->other == 1;
  ll_option = mp->ll_option == 1;
  send_unicast = mp->send_unicast == 1;
  use_lifetime = mp->use_lifetime == 1;

  VALIDATE_SW_IF_INDEX (mp);

  rv = ip6_ra_config_v2 (
    vm, ntohl (mp->sw_if_index), send_radv, managed, other, ll_option,
    send_unicast, use_lifetime, ntohl (mp->lifetime),
    ntohl (mp->initial_count), ntohl (mp->initial_interval),
    ntohl (mp->max_interval), ntohl (mp->min_interval), defaults);
