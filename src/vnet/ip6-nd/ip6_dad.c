...
handle_dad_conflict (vlib_main_t *vm, u32 sw_if_index, const ip6_address_t *address)
{
  ...
  pool_foreach (entry, dm->dad_entries)
	  ...
	  ip6_address_is_equal (&entry->address, address))
	{
	  ...
	  DAD_ERR ("DAD FAILED: Duplicate address %U on sw_if_index %u - ADDRESS REMOVED",
...
ip6_dad_delegate_disable (index_t iddi)
{
  ...
  /* Collect indices of DAD entries to stop (cannot modify pool during iteration) */
  pool_foreach_index (entry_index, dm->dad_entries)
    {
      entry = pool_elt_at_index (dm->dad_entries, entry_index);
      if (entry->sw_if_index == sw_if_index)
	{
	  vec_add1 (indices_to_stop, entry_index);
	}
    }

  /* Now stop DAD for collected entries */
  for (u32 i = 0; i < vec_len (indices_to_stop); i++)
    {
      entry = pool_elt_at_index (dm->dad_entries, indices_to_stop[i]);
      DAD_INFO ("Stopping DAD for %U on sw_if_index %u (interface disabled)", format_ip6_address,
		&entry->address, sw_if_index);
      ip6_dad_stop (sw_if_index, &entry->address);
    }

  vec_free (indices_to_stop);
...
/**
 * @brief Format delegate for display (called by show ip6 interface)
 *
 * @param s String buffer
 * @param args Arguments (iddi, indent)
