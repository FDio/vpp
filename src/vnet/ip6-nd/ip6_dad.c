...
{
  vl_api_ip6_dad_state_t state = va_arg (*args, int);
...
ip6_dad_enable_command_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  /* Save initial DAD state for rollback */
  bool was_enabled = ip6_dad_main.dad_enabled;
  ...
  if (has_transmits || has_delay)
	...
	delay = cur_delay;
      ...
      error = ip6_dad_config (transmits, delay);
      if (error)
	{
	  /* Rollback: restore initial DAD state */
	  ip6_dad_enable_disable (was_enabled);
	}
      return error;
    }
