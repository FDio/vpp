 ...
 */
void
ip6_dad_send_event (u32 sw_if_index, const ip6_address_t *address, vl_api_ip6_dad_state_t state,
...
/**
 * Cleanup function called when a client disconnects
 */
static clib_error_t *
vl_api_ip6_dad_event_reaper (u32 client_index)
{
  ip6_dad_main_t *dm = &ip6_dad_main;
  ip6_dad_event_registration_t *reg;
  ...
  pool_foreach (reg, dm->dad_event_registrations)
    {
      if (reg->client_index == client_index)
	{
	  pool_put (dm->dad_event_registrations, reg);
	  vlib_log_notice (dm->log_class, "Client %u unregistered (disconnected)", client_index);
	  break;
	}
    }
...
VL_MSG_API_REAPER_FUNCTION (vl_api_ip6_dad_event_reaper);
