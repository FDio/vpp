...
static int
ip6_ra_unnumbered (u32 input_sw_if_index, u32 conn_sw_if_index)
{
...
static u32
ip6_ra_src_is_on_link (u32 sw_if_index, const ip6_address_t *src_addr)
{
  ...
  if (~0 == fib_index)
    return ICMP6_ERROR_ROUTER_SOLICITATION_SOURCE_NOT_ON_LINK;
  ...
  do
	...
	src_first_fei = src_fei;
      ...
      FOR_EACH_SRC_ADDED (src_fib_entry, src, source, ({
			    ...
			    /* reject packets claiming one of our own addresses as the RS source */
  ...
  if (!attached)
      ...
      if (~0 == conn_sw_if_index)
	{
      ...
      else if (!ip6_ra_unnumbered (sw_if_index, conn_sw_if_index))
	return ICMP6_ERROR_ROUTER_SOLICITATION_SOURCE_NOT_ON_LINK;
    }
  ...
  if (attached && sw_if_index != conn_sw_if_index &&
      !ip6_ra_unnumbered (sw_if_index, conn_sw_if_index))
    return ICMP6_ERROR_ROUTER_SOLICITATION_SOURCE_NOT_ON_LINK;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (!is_unspecified && !is_link_local)
	    error0 = ip6_ra_src_is_on_link (sw_if_index0, &ip0->src_address);
