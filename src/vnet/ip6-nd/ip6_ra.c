  ...
  for (i = 0; i < vec_len (radv_indices); i++)
	...
	continue;
	  ...
	  if (this_prefix->prefix_len >= prefix_len &&
	      ip6_address_is_equal_masked (&this_prefix->prefix, address, &mask))
	{
	    int rv = ip6_ra_prefix (vm, radv_info->sw_if_index, &this_prefix->prefix,
				    this_prefix->prefix_len, 0 /* use_default */, valid_time,
				    preferred_time, 0 /* no_advertise */, 0 /* off_link */,
				    0 /* no_autoconfig */, 0 /* no_onlink */, 0 /* is_no */);
	    if (rv != 0)
