		     ...
		     u32 min_interval)
{
  ...
  radv_info->adv_other_flag = !!(flags & IP6_RA_FLAG_OTHER);
  radv_info->send_unicast = !!(flags & IP6_RA_FLAG_SEND_UNICAST);
  radv_info->adv_link_layer_address = !!(flags & IP6_RA_FLAG_LL_OPTION);

  /* default intervals if zeroes */
  if ((max_interval == 0) && (min_interval == 0))
    {
      max_interval = DEF_MAX_RADV_INTERVAL;
      min_interval = DEF_MIN_RADV_INTERVAL;
    }
  /* make default min 75% of max if not set */
  if ((max_interval != 0) && (min_interval == 0))
    min_interval = .75 * max_interval;

  /* if manually set wrong min interval*/
  if ((min_interval > .75 * max_interval) || (min_interval < 3))
    return VNET_API_ERROR_INVALID_VALUE;
