...
ip6_ra_prefix (vlib_main_t *vm, u32 sw_if_index, ip6_address_t *prefix_addr,
	       ...
	       u8 no_autoconfig, u8 fixed, u8 no_onlink, u8 is_no)
{
  ip6_ra_t *radv_info;
  ...
  if (!prefix)
    {
      ...
      prefix->enabled = 1;
      prefix->decrement_lifetime_flag = !(fixed != 0);
  ...
  if (add_radv_info)
	     ...
	     unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	{
	  if (unformat (line_input, "no-advertise"))
	    ...
	    no_autoconfig = 1;
	  else if (unformat (line_input, "fixed"))
	    fixed = 1;
      ...
      ip6_ra_prefix (vm, sw_if_index, &ip6_addr, addr_len,
		     ...
		     no_autoconfig, fixed, no_onlink, is_no);
    }
