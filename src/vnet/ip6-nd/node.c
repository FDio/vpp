...
ip6_nd_proxy_enable_disable (u32 sw_if_index, u8 enable)
{
  ...
  if (enable)
    vnet_feature_enable_disable ("ip6-local", "ip6-nd-proxy", sw_if_index, 1,
				 NULL, 0);
  else
    vnet_feature_enable_disable ("ip6-local", "ip6-nd-proxy", sw_if_index, 0,
				 NULL, 0);
  return 0;
}
			  ...
			  vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (ip6_is_solicited_node_multicast_address (&ip6->dst_address))
	    {
	      ...
	      if (type0 == ICMP6_neighbor_solicitation)
		{
		  if (
		    ...
		    {
		      ip_neighbor_learn_t learn = { .sw_if_index =
						      sw_if_index0,
						    .ip = {
						      .version = AF_IP6,
						      .ip.ip6 =
		      ...
		      memcpy (&learn.mac, icmp6_nd_ell_addr->ethernet_address,
			      sizeof (learn.mac));
		  ...
		  if (FIB_NODE_INDEX_INVALID == fei)
		    {
		      ...
		      ip6_neighbor_probe_dst (sw_if_index0, &ip6->dst_address);
		    }
		  else
		    {
		      next0 = ICMP6_NEIGHBOR_SOLICITATION_NEXT_REPLY;
...
VNET_FEATURE_INIT (ip6_nd_proxy_node, static) = {
  .arc_name = "ip6-local",
  .node_name = "ip6-nd-proxy",
  .runs_before = VNET_FEATURES ("ip6-icmp-input"),
};
