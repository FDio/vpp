...
#define IOAM_HBYH_POP 2
  ...
  int (*add_options[MAX_IP6_HBH_OPTION]) (u8 *rewrite_string,
					  u8 *rewrite_size);
  int (*pop_options[MAX_IP6_HBH_OPTION]) (vlib_buffer_t *b, ip6_header_t *ip,
  ...
  /* Array of function pointers to handle hbh options being used with
   * classifier */
...
extern clib_error_t *ip6_ioam_enable (int has_trace_option, int has_pot_option,
...
int ip6_hbh_add_register_option (u8 option, u8 size,
...
int ip6_hbh_get_sizeof_register_option (
  u8 option, int get_sizeof_hdr_options (u32 *rewrite_size));

int ip6_ioam_set_rewrite (u8 **rwp, int has_trace_option, int has_pot_option,
			  int has_seq_no);
