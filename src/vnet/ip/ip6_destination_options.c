 ...
 *
 * This file implements the processing of IPv6 destination options extension header.
 * It identifies the next protocol based on the contents of the destination options
 * and routes the packet accordingly or drops it if the protocol is unsupported.
 */
#include <vnet/vnet.h>
...
typedef struct
{
  u8 ip6_destionation_options_enabled;
...
{
  ip6_destination_options_main_t *idom = &ip6_destination_options_main;
  return idom->ip6_destionation_options_enabled;
}
...
ip6_destination_options_enable_disable (u8 enable)
{
  ...
  if (enable && !idom->ip6_destionation_options_enabled)
    {
      idom->ip6_destionation_options_enabled = 1;
      ip6_register_protocol (IP_PROTOCOL_IP6_DESTINATION_OPTIONS,
			     ip6_destination_options_node.index);
    }
  else if (!enable && idom->ip6_destionation_options_enabled)
    {
      idom->ip6_destionation_options_enabled = 0;
...
static clib_error_t *
ip6_destination_options_enable_disable (vlib_main_t *vm,
    unformat_input_t *input, vlib_cli_command_t *cmd)
{
  ...
  return ip6_destination_options_enable(enable);
}
...
VLIB_CLI_COMMAND (ip6_destination_options_enable_disable_command, static) = {
    ...
    .function = ip6_destination_options_enable_disable,
};
...
ip6_destination_options_init (vlib_main_t *vm)
{
 ...
 idom->next_by_ip_protocol[IP_PROTOCOL_IP_IN_IP] =
