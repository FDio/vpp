...
typedef enum
{
  ...
  IP6_DEST_OPT_NEXT_IPIP6, // Forward to IPIP6 input node
  IP6_DEST_OPT_N_NEXT,
  ...
  while (n_left_from)
      ...
      if (next_hdr == IP_PROTOCOL_IP6_DESTINATION_OPTIONS)
	{
	  ...
	  if (final_next_hdr == IP_PROTOCOL_IPV6)
	    {
	      next[0] = IP6_DEST_OPT_NEXT_IPIP6;
	    }
	  else
	    {
...
VLIB_REGISTER_NODE(ip6_destination_options_node) = {
	...
	[IP6_DEST_OPT_NEXT_IPIP6] = "ipip6-input", // Forward to IPIP6 input
    },
};
...
ip6_destination_options_init (vlib_main_t *vm)
{
  ...
  clib_memset (idom, 0, sizeof (*idom));

  // enable IPv6 destination options processing by default
  ip6_destination_options_enable_disable (1 /* enable */);
