...
format_ip_path_mtu_adj_delegate (const adj_delegate_t *aed, u8 *s)
{
  ...
  ip_adj = pool_elt_at_index (ip_path_mtu_adj_delegate_pool,
                              aed->ad_index);
...
ip_pmtu_adj_delegate_adj_created (adj_index_t ai)
{
  ...
  ip_pmtu_key_t key = {
    ...
    .table_id = fib_table_get_table_id_for_sw_if_index(adj->ia_nh_proto,
...
ip_ptmu_adj_walk_remove (adj_index_t ai,
                         void *ctx)
{
  adj_delegate_t *ad;
...
static adj_walk_rc_t
ip_ptmu_adj_walk_update (adj_index_t ai,
                         void *ctx)
{
  ip_path_mtu_adj_delegate_t *ipp_ad;
  adj_delegate_t *ad;
...
void
ip_pmtu_dpo_add_or_lock (fib_protocol_t fproto,
                         u16 pmtu,
                         dpo_id_t * dpo)
{
  ip_pmtu_dpo_t *ipm;
...
format_ip_pmtu_dpo (u8 * s, va_list * ap)
{
  ...
  s = format (s, "ip-pmtu-dpo: %U, mtu:%d",
	      format_dpo_proto, ipm->ipm_proto, ipm->ipm_pmtu);
 ...
 */
static void
ip_pmtu_dpo_interpose (const dpo_id_t * original,
                       ...
                       dpo_id_t * clone)
{
  ...
  dpo_stack (ip_pmtu_dpo_type,
	     ipm_clone->ipm_proto,
             &ipm_clone->ipm_dpo, parent);
  dpo_set (clone,
	   ip_pmtu_dpo_type,
	   ipm_clone->ipm_proto,
...
int
ip_path_mtu_update (const ip_address_t *nh,
                    u32 table_id,
                    u16 pmtu)
{
  fib_prefix_t pfx;
...
ip_path_module_init (vlib_main_t *vm)
{
  ...
  ip_pmtu_dpo_type = dpo_register_new_type (&ip_pmtu_dpo_vft,
                                            ip_pmtu_dpo_nodes);
...
static inline uword
ip_pmtu_dpo_inline (vlib_main_t * vm,
...
VLIB_NODE_FN (ip4_ip_pmtu_dpo_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
  return (ip_pmtu_dpo_inline (vm, node, from_frame, 0));
}

VLIB_NODE_FN (ip6_ip_pmtu_dpo_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
