			  ...
			  ip6_reass_kv_t * kv, u32 * icmp_bi, u8 * do_handoff)
{
  ...
  if (!clib_bihash_search_48_8
    ...
    {
      if (thread_index != kv->v.thread_index)
	{
	  *do_handoff = 1;
	  return NULL;
	}
  ...
  if (rt->reass_n >= rm->max_reass_n)
  ...
  else
    {
      ...
      reass->id = ((u64) thread_index * 1000000000) + rt->id_counter;
      ++rt->id_counter;
      reass->first_bi = ~0;
  ...
  kv->v.thread_index = thread_index;
  reass->last_heard = now;
    ...
    .next_nodes =
        {
                ...
                [IP6_REASSEMBLY_NEXT_HANDOFF] = "ip6-reassembly-feature-handoff",
        },
};
/* *INDENT-ON* */
...
always_inline uword
ip6_reassembly_handoff_node_fn (vlib_main_t * vm, vlib_node_runtime_t * node,
				vlib_frame_t * frame, bool is_feature)
{
  ip6_reass_main_t *rm = &ip6_reass_main;
...
static uword
ip6_reassembly_handoff (vlib_main_t * vm,
			vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  return ip6_reassembly_handoff_node_fn (vm, node, frame,
					 false /* is_feature */ );
}

/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip6_reassembly_handoff_node) = {
  .function = ip6_reassembly_handoff,
...
VLIB_NODE_FUNCTION_MULTIARCH (ip6_reassembly_handoff_node, ip6_reassembly_handoff);


static uword
ip6_reassembly_feature_handoff(vlib_main_t * vm,
                               vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  return ip6_reassembly_handoff_node_fn (vm, node, frame, true /* is_feature */ );
}


/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip6_reassembly_feature_handoff_node) = {
  .function = ip6_reassembly_feature_handoff,
  .name = "ip6-reassembly-feature-handoff",
  .vector_size = sizeof (u32),
  .format_trace = format_ip6_reassembly_handoff_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN(ip6_reassembly_handoff_error_strings),
...
VLIB_NODE_FUNCTION_MULTIARCH (ip6_reassembly_feature_handoff_node,
			      ip6_reassembly_feature_handoff);
