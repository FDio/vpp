...
ip6_reass_add_trace (vlib_main_t * vm, vlib_node_runtime_t * node,
		     ip6_reass_main_t * rm, ip6_reass_t * reass, u32 bi,
...
void
ip6_reass_discard_all (vlib_main_t * vm, ip6_reass_main_t * rm,
		       ip6_reass_t * reass, u32 ** vec_discard_bi)
{
  u32 bi = reass->first_bi;
  while (~0 != bi)
    {
      vec_add1 (*vec_discard_bi, bi);
      vlib_buffer_t *b = vlib_get_buffer (vm, bi);
...
static void
ip6_reass_on_timeout (vlib_main_t * vm, ip6_reass_main_t * rm,
		      ip6_reass_t * reass, u32 * icmp_bi,
		      u32 ** vec_timeout_bi)
{
  if (~0 == reass->first_bi)
    {
  ...
  ip6_reass_discard_all (vm, rm, reass, vec_timeout_bi);
}
			  ...
			  u32 ** vec_timeout_bi)
{
  ...
  if (!clib_bihash_search_48_8 (&rm->hash, &kv, &value))
	...
	{
	  ip6_reass_on_timeout (vm, rm, reass, icmp_bi, vec_timeout_bi);
		    ...
		    u32 * next0, vlib_error_t * error0)
{
  ...
  *bi0 = reass->first_bi;
  *next0 = IP6_REASSEMBLY_NEXT_LOCAL;
  *error0 = IP6_ERROR_NONE;
  ...
  while (tmp)
    {
      ...
      --rm->buffers_n;
      vnet_buffer_opaque_t *tmpvnb = vnet_buffer (tmp);
      u32 data_len = ip6_reass_buffer_get_data_len (tmp);
      i32 data_offset = ip6_reass_buffer_get_data_offset (tmp);
      vlib_buffer_advance (tmp, tmpvnb->ip.reass.ip6_frag_hdr_offset +
			   sizeof (ip6_frag_hdr_t) + data_offset);
  ...
  ip->payload_length =
    clib_host_to_net_u16 (clib_net_to_host_u16 (ip->payload_length) -
			  sizeof (ip6_frag_hdr_t) +
			  first_b->total_length_not_including_first_buffer);
  first_b->current_length -= sizeof (*frag_hdr);
  ip6_reass_free (rm, reass);
  ip6_tcp_udp_icmp_validate_checksum (vm, first_b);
...
static u32
ip6_reass_get_next_bi (vlib_main_t * vm, u32 bi)
{
  vlib_buffer_t *b = vlib_get_buffer (vm, bi);
  if (b->flags & VLIB_BUFFER_NEXT_PRESENT)
    {
...
static void
ip6_reass_insert_buffer_in_chain (vlib_main_t * vm,
				  ip6_reass_t * reass,
				  u32 prev_range_bi, u32 new_next_bi)
{
  ...
  reass->data_len += ip6_reass_buffer_get_data_len (b);
}
...
ip6_reass_update (vlib_main_t * vm, vlib_node_runtime_t * node,
		  ip6_reass_main_t * rm, ip6_reass_t * reass, u32 * bi0,
		  u32 * next0, vlib_error_t * error0,
		  ip6_frag_hdr_t * frag_hdr, u32 ** vec_discard_overlap_bi)
{
  int consumed = 0;
  ...
  if (~0 == reass->first_bi)
    {
      ...
      ip6_reass_insert_buffer_in_chain (vm, reass, prev_range_bi, *bi0);
  ...
  while (~0 != candidate_range_bi)
      ...
      if (fragment_first > candidate_vnb->ip.reass.range_last)
	{
	  ...
	  candidate_range_bi = ip6_reass_get_next_bi (vm, candidate_range_bi);
	  if (candidate_vnb->ip.reass.range_last < fragment_last &&
	      ~0 == candidate_range_bi)
	    {
	      ...
	      ip6_reass_insert_buffer_in_chain (vm, reass, prev_range_bi,
						*bi0);
	      consumed = 1;
	      break;
	    }
      ...
      else
	{
	  ...
	  ip6_reass_discard_all (vm, rm, reass, vec_discard_overlap_bi);
	  ip6_reass_free (rm, reass);
	  if (fb->flags & VLIB_BUFFER_IS_TRACED)
	    {
  ...
  if (~0 != reass->last_packet_octet &&
    ...
    {
      ip6_reass_finalize (vm, node, rm, reass, bi0, next0, error0);
    }
  else
    {
      if (consumed)
		...
		vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0 || vec_len (vec_timeout_bi) > 0 ||
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  u32 icmp_bi = ~0;
	  ip6_reass_t *reass =
	    ip6_reass_find_or_create (vm, rm, &k, &icmp_bi, &vec_timeout_bi);

	  u32 error0 = IP6_ERROR_NONE;
	  if (reass)
	    {
	      ip6_reass_update (vm, node, rm, reass, &bi0, &next0, &error0,
				frag_hdr, &vec_discard_overlap_bi);
	    }
	  else
	    {
	      next0 = IP6_REASSEMBLY_NEXT_DROP;
...
{
  .function = ip6_reassembly,.name = "ip6-reassembly",.vector_size =
    sizeof (u32),.format_trace = format_ip6_reass_trace,.n_errors =
    ARRAY_LEN (ip6_reassembly_error_strings),.error_strings =
    ip6_reassembly_error_strings,.n_next_nodes =
    IP6_REASSEMBLY_N_NEXT,.next_nodes =
  {
  [IP6_REASSEMBLY_NEXT_LOCAL] = "ip6-local",
