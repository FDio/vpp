...
static uword
ip_interface_address_event_process (vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
  ...
  while (1)
	...
	case SW_INTERFACE_IP_ADDR_EVENT:
	  vec_foreach (event, events)
	    ip_interface_address_handle_event (event);
	  break;
  ...
  if (p)
    {
      ...
      if (~0 != vec_search_with_function (ipws, watch, ip_interface_address_watch_cmp))
	/* duplicate */
	return;
    }
...
static void
common_interface_address_signal (ip46_type_t type, u32 sw_if_index, void *address, u32 prefix_len,
				 u32 is_delete)
{
  ...
  for (i = 0; i < 2; i++)
      ...
      vec_foreach (watcher, watchers)
	{
	  ...
	  event = vlib_process_signal_event_data (vlib_get_main (),
						  ...
						  SW_INTERFACE_IP_ADDR_EVENT, 1, sizeof (*event));
...
static void
ip4_interface_address_signal (ip4_main_t *im, uword opaque, u32 sw_if_index, ip4_address_t *address,
			      u32 prefix_len, u32 if_address_index, u32 is_delete)
{
  common_interface_address_signal (IP46_TYPE_IP4, sw_if_index, address, prefix_len, is_delete);
}

static void
ip6_interface_address_signal (ip6_main_t *im, uword opaque, u32 sw_if_index, ip6_address_t *address,
			      u32 prefix_len, u32 if_address_index, u32 is_delete)
{
  common_interface_address_signal (IP46_TYPE_IP6, sw_if_index, address, prefix_len, is_delete);
}
...
static clib_error_t *
ip_interface_address_watchers_show (vlib_main_t *vm, unformat_input_t *input,
...
{
  mhash_init (&ipiw_db.ipiwdb_hash, sizeof (ip_interface_address_watcher_t *),
