  ...
  while (rem)
      ...
      if ((to_b = frag_buffer_alloc (org_from_b, &to_bi)) == 0)
	{
	  *error = IP_FRAG_ERROR_MEMORY;
      ...
      to_ip4 = vlib_buffer_get_current (to_b) + l2unfragmentablesize;
		  ...
		  vlib_frame_t * frame, u32 node_index, bool is_ip6)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_ip6)
	    ip6_frag_do_fragment (vm, pi0, mtu, 0, &buffer, &error0);
	  else
	    ip4_frag_do_fragment (vm, pi0, mtu, 0, &buffer, &error0);
