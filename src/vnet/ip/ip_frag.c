 ...
 */
#define COPY_MPLS_HDR(from_b, to_b)                                                           \
{ 											      \
  if ((vnet_buffer (from_b)->ip_frag.flags) & IP_FRAG_FLAG_MPLS_HEADER)                       \
  {                                                                                           \
    u8 mpls_hdr_length = vnet_buffer(from_b)->mpls.mpls_hdr_length;                           \
    u8 *org_from_mpls_packet = from_b->data + (from_b->current_data - mpls_hdr_length);       \
    clib_memcpy_fast ((to_b->data - mpls_hdr_length), org_from_mpls_packet, mpls_hdr_length); \
  }                                                                                           \
}     											      \
  ...
  if ((vnet_buffer (from)->ip_frag.flags) & IP_FRAG_FLAG_MPLS_HEADER)
    {
       ...
       vnet_buffer (to)->mpls.mpls_hdr_length = vnet_buffer (from)->mpls.mpls_hdr_length;
    }
}
