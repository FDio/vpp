		      ...
		      u16 l2unfragmentablesize, u32 ** buffer)
{
  ...
  if (mtu < sizeof (ip4_header_t) || headroom + head_bytes > vlib_buffer_get_default_data_size (vm))
    {
      return IP_FRAG_ERROR_CANT_FRAGMENT_HEADER;
    }

  max = clib_min ((i32) mtu - sizeof (ip4_header_t),
		  (i32) vlib_buffer_get_default_data_size (vm) - headroom - head_bytes) &
	~0x7;
		      ...
		      u16 l2unfragmentablesize, u32 ** buffer)
{
  ...
  if (mtu < sizeof (ip6_header_t) + sizeof (ip6_frag_hdr_t) ||
      headroom + head_bytes > vlib_buffer_get_default_data_size (vm))
    return IP_FRAG_ERROR_CANT_FRAGMENT_HEADER;

  max = clib_min ((i32) mtu - sizeof (ip6_header_t) - sizeof (ip6_frag_hdr_t),
		  (i32) vlib_buffer_get_default_data_size (vm) - headroom - head_bytes) &
	~0x7;
  if (PREDICT_FALSE (max == 0))
    return IP_FRAG_ERROR_CANT_FRAGMENT_HEADER;
