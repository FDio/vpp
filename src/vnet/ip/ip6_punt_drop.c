...
ip6_punt_policer_add_del (u8 is_add, u32 policer_index)
{
  ...
  vnet_feature_enable_disable ("ip6-punt", "ip6-punt-policer", 0, is_add, 0,
			       0);
}
#endif /* CLIB_MARCH_VARIANT */

static clib_error_t *
ip6_punt_police_cmd (vlib_main_t *vm, unformat_input_t *main_input,
...
void
ip6_punt_redirect_add (u32 rx_sw_if_index, u32 tx_sw_if_index,
		       ip46_address_t *nh)
{
...
{
  ip_punt_redirect_add (FIB_PROTOCOL_IP6, rx_sw_if_index,
...
static clib_error_t *
ip6_punt_redirect_cmd (vlib_main_t *vm, unformat_input_t *main_input,
		       vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  fib_route_path_t *rpaths = NULL, rpath;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	rx_sw_if_index = ~0;
      else if (unformat (line_input, "rx %U", unformat_vnet_sw_interface, vnm,
			 &rx_sw_if_index))
	;
...
VLIB_CLI_COMMAND (ip6_punt_redirect_command, static) = {
  ...
  .short_help = "ip6 punt redirect [add|del] rx [<interface>|all] via [<nh>] "
		"<tx_interface>",
};
...
static clib_error_t *
ip6_punt_redirect_show_cmd (vlib_main_t *vm, unformat_input_t *main_input,
			    vlib_cli_command_t *cmd)
{
