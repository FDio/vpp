			      ...
			      vlib_frame_t * frame, int is_glean)
{
  ...
  vec_validate (im->nd_throttle_bitmaps, thread_index);
  vec_validate (im->nd_throttle_seeds, thread_index);
  vec_validate (im->nd_throttle_last_seed_change_time, thread_index);
  ...
  if (time_now - im->nd_throttle_last_seed_change_time[thread_index] > 1e-3)
    {
      ...
      memset (im->nd_throttle_bitmaps[thread_index], 0, 512 / BITS (u8));
