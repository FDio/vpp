...
ip4_reass_buffer_get_data_offset (vlib_buffer_t * b)
{
  ...
  return (i32) vnb->ip.reass.range_first - (i32) vnb->ip.reass.fragment_first;
}
...
ip4_reass_on_timeout (vlib_main_t * vm, ip4_reass_main_t * rm,
		      ip4_reass_t * reass, u32 ** vec_timeout_bi)
{
  u32 bi = reass->first_bi;
  while (~0 != bi)
    {
      vec_add1 (*vec_timeout_bi, bi);
      vlib_buffer_t *b = vlib_get_buffer (vm, bi);
...
ip4_reass_find_or_create (vlib_main_t * vm, ip4_reass_main_t * rm,
			  ip4_reass_key_t * k, u32 ** vec_timeout_bi)
{
  ...
  if (!clib_bihash_search_24_8 (&rm->hash, &kv, &value))
	...
	{
	  ip4_reass_on_timeout (vm, rm, reass, vec_timeout_bi);
		    ...
		    u32 * next0, vlib_error_t * error0)
{
  ...
  *bi0 = reass->first_bi;
  *next0 = IP4_REASSEMBLY_NEXT_LOCAL;
  *error0 = IP4_ERROR_NONE;
  vlib_buffer_t *first_b = vlib_get_buffer (vm, reass->first_bi);
  vlib_buffer_t *tmp = NULL;
  if (first_b->flags & VLIB_BUFFER_NEXT_PRESENT)
    {
      tmp = vlib_get_buffer (vm, first_b->next_buffer);
    }
  u32 total_length = 0;
  while (tmp)
    {
      ...
      i32 data_offset = ip4_reass_buffer_get_data_offset (tmp);
      vlib_buffer_advance (tmp, ip4_header_bytes (ip) + data_offset);
      tmp->current_length = data_len;
  ...
  ip4_reass_free (rm, reass);
  if (first_b->flags & VLIB_BUFFER_IS_TRACED)
    {
...
static u32
ip4_reass_get_next_bi (vlib_main_t * vm, u32 bi)
{
  vlib_buffer_t *b = vlib_get_buffer (vm, bi);
  if (b->flags & VLIB_BUFFER_NEXT_PRESENT)
    {
      return b->next_buffer;
    }
  return ~0;
}

static void
ip4_reass_insert_buffer_in_chain (vlib_main_t * vm,
				  ip4_reass_t * reass,
				  u32 prev_range_bi, u32 new_next_bi)
{
  vlib_buffer_t *b = vlib_get_buffer (vm, new_next_bi);
  if (~0 != prev_range_bi)
    {
      vlib_buffer_t *prev_b = vlib_get_buffer (vm, prev_range_bi);
      u32 old_prev_next = prev_b->next_buffer;
      prev_b->next_buffer = new_next_bi;
      if (prev_b->flags & VLIB_BUFFER_NEXT_PRESENT)
	{
	  b->flags |= VLIB_BUFFER_NEXT_PRESENT;
	  b->next_buffer = old_prev_next;
	}
      else
	{
	...
	{
	  b->flags |= VLIB_BUFFER_NEXT_PRESENT;
	  b->next_buffer = reass->first_bi;
	}
      reass->first_bi = new_next_bi;
    }
  reass->data_len += ip4_reass_buffer_get_data_len (b);
}

static void
ip4_reass_discard_buffer_from_chain (vlib_main_t * vm,
				     vlib_node_runtime_t * node,
				     ip4_reass_main_t * rm,
				     u32 ** vec_discard_overlap_bi,
				     ...
				     u32 discard_bi)
{
  ...
  if (~0 != prev_range_bi)
	...
	{
	  prev_b->flags |= VLIB_BUFFER_NEXT_PRESENT;
	  prev_b->next_buffer = discard_b->next_buffer;
	}
      else
	{
	  prev_b->flags &= ~VLIB_BUFFER_NEXT_PRESENT;
  ...
  else if (discard_b->flags & VLIB_BUFFER_NEXT_PRESENT)
    {
      reass->first_bi = discard_b->next_buffer;
    }
  else
    {
      reass->first_bi = ~0;
    }
  vec_add1 (*vec_discard_overlap_bi, discard_bi);
  discard_b->flags &= ~VLIB_BUFFER_NEXT_PRESENT;
  reass->data_len -= ip4_reass_buffer_get_data_len (discard_b);
  if (discard_b->flags & VLIB_BUFFER_IS_TRACED)
    {
		  ...
		  u32 ** vec_discard_overlap_bi)
{
  ...
  ASSERT (!(fb->flags & VLIB_BUFFER_NEXT_PRESENT));
  ...
  u32 fragment_length = fvnb->ip.reass.fragment_length =
  ...
  if (~0 == reass->first_bi)
    {
      ...
      ip4_reass_insert_buffer_in_chain (vm, reass, prev_range_bi, *bi0);
      if (fb->flags & VLIB_BUFFER_IS_TRACED)
	{
  ...
  while (~0 != candidate_range_bi)
      ...
      if (fragment_first > candidate_vnb->ip.reass.range_last)
	{
	  ...
	  candidate_range_bi = ip4_reass_get_next_bi (vm, candidate_range_bi);
	  if (candidate_vnb->ip.reass.range_last < fragment_last &&
	      ~0 == candidate_range_bi)
	    {
	      ...
	      ip4_reass_insert_buffer_in_chain (vm, reass, prev_range_bi,
						*bi0);
	      consumed = 1;
	      break;
	    }
      ...
      else
	{
	  ...
	  if (fragment_first < candidate_vnb->ip.reass.range_first)
	      ...
	      if (overlap < ip4_reass_buffer_get_data_len (candidate_b))
		{
		  ...
		  if (candidate_b->flags & VLIB_BUFFER_NEXT_PRESENT)
		    {
		      prev_range_bi = candidate_range_bi;
		      candidate_range_bi = candidate_b->next_buffer;
		      continue;
		    }
		  else
		    {
		      // special case - last range discarded
		      ip4_reass_insert_buffer_in_chain (vm, reass,
							candidate_range_bi,
							*bi0);
		      consumed = 1;
		    }
	  ...
	  if (discard_candidate)
	    {
	      int next_present =
		candidate_b->flags & VLIB_BUFFER_NEXT_PRESENT;
	      u32 next_bi = candidate_b->next_buffer;
	      // discard candidate range, probe next range
	      ip4_reass_discard_buffer_from_chain (vm, node, rm,
						   vec_discard_overlap_bi,
						   reass, prev_range_bi,
						   candidate_range_bi);
	      if (next_present)
		{
		  candidate_range_bi = next_bi;
		  continue;
		}
	      else
		{
  ...
  if (~0 != reass->last_packet_octet &&
    ...
    {
      ip4_reass_finalize (vm, node, rm, reass, bi0, next0, error0);
    }
  else
    {
      if (consumed)
		...
		vlib_frame_t * frame)
{
  ...
  static u32 *vec_timeout_bi = NULL;	// indexes of buffers which timed out
  static u32 *vec_discard_overlap_bi = NULL;	// indexes of buffers which were discarded due to overlap
  while (n_left_from > 0 || vec_len (vec_timeout_bi) > 0 ||
	 vec_len (vec_discard_overlap_bi) > 0)
    {
      vlib_get_next_frame (vm, node, next_index, to_next, n_left_to_next);

      while (vec_len (vec_timeout_bi) > 0 && n_left_to_next > 0)
	{
	  u32 bi = vec_pop (vec_timeout_bi);
	  vlib_buffer_t *b = vlib_get_buffer (vm, bi);
      ...
      while (vec_len (vec_discard_overlap_bi) > 0 && n_left_to_next > 0)
	{
	  u32 bi = vec_pop (vec_discard_overlap_bi);
	  vlib_buffer_t *b = vlib_get_buffer (vm, bi);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  k.unused = 0;
	  ip4_reass_t *reass =
	    ip4_reass_find_or_create (vm, rm, &k, &vec_timeout_bi);
...
{
  .function = ip4_reassembly,.name = "ip4-reassembly",.vector_size =
    sizeof (u32),.format_trace = format_ip4_reass_trace,.n_errors =
    ARRAY_LEN (ip4_reassembly_error_strings),.error_strings =
    ip4_reassembly_error_strings,.n_next_nodes =
    IP4_REASSEMBLY_N_NEXT,.next_nodes =
  {
  [IP4_REASSEMBLY_NEXT_LOCAL] = "ip4-local",
			...
			vlib_node_runtime_t * node, vlib_frame_t * f)
{
  ...
  while (true)
	...
	default:
      ...
      while (vec_len (vec_timeout_bi) > 0)
	{
