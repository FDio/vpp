  ...
  if (~0 != t->reass_id)
    {
      ...
      s =
	format (s,
		"first bi: %u, data len: %u, ip/fragment[%u, %u], thread: %u",
		t->trace_range.first_bi, t->total_data_len, t->fragment_first,
		t->fragment_last, t->thread_id);
    }
  switch (t->action)
    {
    case RANGE_SHRINK:
...
always_inline void
ip4_reass_free (vlib_main_t *vm, ip4_reass_main_t * rm, ip4_reass_per_thread_t * rt,
		ip4_reass_t * reass)
{
  clib_bihash_kv_16_8_t kv;
  kv.key[0] = reass->key.as_u64[0];
  kv.key[1] = reass->key.as_u64[1];
  int rv = clib_bihash_add_del_16_8 (&rm->hash, &kv, 0);
  u8 *v = format (NULL, "thread %d, clib_bihash_add_del_16_8 (%U, 0) == %d",
		  vm->thread_index, 
		  format_hexdump, &kv.key, sizeof (kv.key), rv);
  printf ("%.*s\n", vec_len (v), v);
  fflush (stdout);
...
again:
  ...
  int rv;
  rv = clib_bihash_search_16_8
    (&rm->hash, (clib_bihash_kv_16_8_t *) kv, (clib_bihash_kv_16_8_t *) kv);
  u8 *v = format (NULL, "thread %d, clib_bihash_search_16_8 (%U) == %d",
		  vm->thread_index, 
		  format_hexdump, &kv->kv.key, sizeof (kv->kv.key), rv);
  printf ("%.*s\n", vec_len (v), v);
  fflush (stdout);
  if (!rv)
    {
  ...
  rv = clib_bihash_add_del_16_8 (&rm->hash, (clib_bihash_kv_16_8_t *) kv, 2);
  v = format (NULL, "thread %d, clib_bihash_add_del_16_8 (%U, 2) == %d",
		  vm->thread_index, 
		  format_hexdump, &kv->kv.key, sizeof (kv->kv.key), rv);
  printf ("%.*s\n", vec_len (v), v);
  fflush (stdout);
  if (rv)
    {
      ...
      reass = NULL;
      // if other worker created the context already, free it and work with
      // the other copy
      if (-2 == rv)
	goto again;
    }
...
#if 0
  rm->sendout_fq_index =
    vlib_frame_queue_main_init (ip4_reass_sendout_node.index, 0);
  rm->sendout_fq_feature_index =
    vlib_frame_queue_main_init (ip4_reass_sendout_feature_node.index, 0);
#endif
