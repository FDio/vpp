...
format_igmp_header (u8 * s, va_list * args)
{
  ...
  s =
    format (s, "\n%Utype 0x%02x, code %u, checksum 0x%04x",
	    format_white_space, indent, hdr->type, hdr->code,
...
format_igmp_report_v3 (u8 * s, va_list * args)
{
  ...
  s =
    format (s, "\n%Ugroups %u", format_white_space, indent,
  ...
  for (i = 0; i < clib_net_to_host_u16 (igmp->n_groups); i++)
    {
      ...
      s =
	format (s, "\n%Utype 0x%02x, sources %u", format_white_space, indent,
		group->type, clib_net_to_host_u16 (group->n_src_addresses));
      len +=
