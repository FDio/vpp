...
typedef struct
{
  ...
  u32 max_update;		/* Max probe/delete actions allowed per run */
  ...
  while (curr_idx != ~0)
    {
      ...
      if ((update_count >= cfg->max_update) ||
	  (((loop_count % 100) == 0) &&
		       ...
		       vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  f64 start, next_scan = TIME_MAX;
  u32 ip4_nidx = 0;		/* ip4 neighbor pool index */
  u32 ip6_nidx = 0;		/* ip6 neighbor pool index */
  uword *event_data = 0;
  u8 purge = 0;			/* flag to purge stale entry during scan */
  ...
  while (1)
	...
	{
	  timeout = IP_NEIGHBOR_DEF_SCAN_INTERVAL;
	  ip4_nidx = ip6_nidx = 0;
	  purge = 0;
	  continue;
	}
	...
	ip4_nidx = ip_neighbor_scan (vm, start, ip4_nidx, /* ip4 */ 0, purge);

      if (ip6_nidx != ~0)	/* scan ip6 neighbors */
	ip6_nidx = ip_neighbor_scan (vm, start, ip6_nidx, /* ip6 */ 1, purge);
...
VLIB_CLI_COMMAND (ip_scan_neighbor_command, static) = {
  ...
  .short_help = "ip scan-neighbor [enable | disable] [interval <n-min>] [max-time <n-usec>] [max-update <n>] [delay <n-msec>] [stale <n-min>]",
  .is_mp_safe = 1,
};
/* *INDENT-ON* */
		       ...
		       vlib_cli_command_t * cmd)
{
  ...
  if (cfg->scan_interval == 0.0)
    ...
    vlib_cli_output (vm, "IP neighbor scan - current time is %.4f sec\n   "
