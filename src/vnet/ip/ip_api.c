		       ...
		       u32 context, fib_node_index_t fib_entry_index)
{
  ...
  fib_api_prefix_encode (pfx, &mp->route.prefix);
  mp->route.table_id =
...
{
  mfib_route_path_t *rpaths, *rpath;
  ...
  mfib_entry_encode (mfib_entry_index, &rpaths);
...
static int
ip_route_add_del_t_handler (vl_api_ip_route_add_del_t * mp)
{
  ...
  fib_api_prefix_decode (&mp->route.prefix, &pfx);
  ...
  vec_validate (rpaths, mp->route.n_paths);
  ...
  fib_api_route_add_del (mp->is_add,
			 mp->is_multipath,
			 fib_index, &pfx, FIB_ENTRY_FLAG_NONE, rpaths);
  stats_dsunlock ();
...
vl_api_ip_route_add_del_t_handler (vl_api_ip_route_add_del_t * mp)
{
  ...
  rv = ip_route_add_del_t_handler (mp);

  rv = (rv == 0) ? vnm->api_errno : rv;

  REPLY_MACRO (VL_API_IP_ROUTE_ADD_DEL_REPLY);
}
...
mroute_add_del_handler (u8 is_add,
			...
			u32 entry_flags,
			u32 rpf_id, mfib_route_path_t * rpath)
{
  stats_dslock_with_hint (1 /* release hint */ , 2 /* tag */ );

  if (0)			//!is_local && ~0 == next_hop_sw_if_index)
    {
...
{
  mfib_route_path_t rpath = { 0 };
  mfib_prefix_t pfx;
  u32 fib_index;
  int rv;

  mfib_api_prefix_decode (&mp->prefix, &pfx);
  ...
  rv = mfib_api_path_decode (&mp->path, &rpath);

  if (0 != rv)
    goto out;

  mroute_add_del_handler (mp->is_add, fib_index, &pfx,
			  ntohl (mp->entry_flags),
			  ntohl (mp->rpf_id), &rpath);
out:
  return (rv);
}
