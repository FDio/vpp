...
vl_api_ip_route_lookup_t_handler (vl_api_ip_route_lookup_t * mp)
{
  ...
  rmp = vl_msg_api_alloc_zero (sizeof (*rmp) + npaths * sizeof (*fp));
  ...
  rmp->_vl_msg_id = htons (VL_API_IP_ROUTE_LOOKUP_REPLY + REPLY_MSG_ID_BASE);
  rmp->context = mp->context;
  rmp->retval = ntohl (rv);
  vl_api_send_msg (reg, (u8 *) rmp);	/* *INDENT-OFF* */
  vec_free (rpaths);
}
