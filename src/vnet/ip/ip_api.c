...
static void
vl_api_want_ip_interface_address_events_t_handler (vl_api_want_ip_interface_address_events_t *mp)
{
  ...
  if (vl_api_can_send_msg (reg))
    {
      mp = vl_msg_api_alloc (sizeof (*mp));
      clib_memset (mp, 0, sizeof (*mp));
      mp->_vl_msg_id = ntohs (VL_API_IP_INTERFACE_ADDRESS_EVENT + REPLY_MSG_ID_BASE);
	...
	{
	  clib_warning ("ip interface address event on sw interface index %u: "
			"queue stuffed!",
			event->sw_if_index);
