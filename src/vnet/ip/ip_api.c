...
void
vl_api_ip_forus_reass_enable_disable_t_handler (
  vl_api_ip_forus_reass_enable_disable_t *mp)
{
  vl_api_ip_forus_reass_enable_disable_reply_t *rmp;
  int rv = 0;

  ip4_full_forus_reass_enable_disable (mp->enable_ip4);
  ip6_full_forus_reass_enable_disable (mp->enable_ip6);

  REPLY_MACRO (VL_API_IP_FORUS_REASS_ENABLE_DISABLE_REPLY);
}

void
vl_api_ip_forus_reass_get_t_handler (vl_api_ip_forus_reass_get_t *mp)
{
  vl_api_ip_forus_reass_get_reply_t *rmp;
  int rv = 0;
  REPLY_MACRO2 (VL_API_IP_FORUS_REASS_GET, {
    rmp->ip4_is_enabled = ip4_full_forus_reass_enabled ();
    rmp->ip6_is_enabled = ip6_full_forus_reass_enabled ();
  });
}
