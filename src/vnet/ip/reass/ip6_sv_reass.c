...
always_inline u32
ip6_sv_reass_buffer_get_data_offset (vlib_buffer_t * b)
{
...
always_inline int
/* pick len bytes from buffer b starting at src (which is within buffer b),
 * possibly picking data from next buffer if required */
pick_data (vlib_main_t * vm, vlib_buffer_t * b, u8 * src, u8 * dst,
	   size_t len)
{
  u8 *tail;
again:
  tail = vlib_buffer_get_tail (b);
  if (src > tail)
    return 0;
  if (src < tail)
    {
      const size_t copy_bytes = clib_min (tail - src, len);
      clib_memcpy_fast (dst, src, copy_bytes);
      len -= copy_bytes;
      dst += copy_bytes;
    }
  if (len && (b->flags & VLIB_BUFFER_NEXT_PRESENT))
    {
		     ...
		     ip6_frag_hdr_t * frag_hdr)
{
  ...
  if (0 == fragment_first)
						...
						ext_hdr->n_data_u64s * 8))
	{
      ...
      reass->payload_length = fip->payload_length;
	  ...
	  do
	    {
	      udp_header_t u;
	      udp_header_t *udp =
		(udp_header_t *) (ip6_ext_next_header (ext_hdr));
	      if (!vlib_object_within_buffer_data
		  (vm, fb, udp, sizeof (*udp)))
		{
		  if (!pick_data (vm, fb, (u8 *) udp, (u8 *) & u, sizeof (u)))
		    {
		      return IP6_SV_REASS_RC_INTERNAL_ERROR;
		    }
		  udp = &u;
	  ...
	  do
	    {
	      ...
	      if (!vlib_object_within_buffer_data
		  (vm, fb, tcp, sizeof (*tcp)))
		{
		  if (!pick_data (vm, fb, (u8 *) tcp, (u8 *) & t, sizeof (t)))
		    {
		      return IP6_SV_REASS_RC_INTERNAL_ERROR;
		    }
		  tcp = &t;
  ...
  if (PREDICT_FALSE (fb->flags & VLIB_BUFFER_IS_TRACED))
    {
      ip6_sv_reass_add_trace (vm, node, rm, reass, bi0, REASS_FRAGMENT_CACHE);
    }
			  ...
			  vlib_frame_t * frame, bool is_feature)
{
  ...
  ip6_sv_reass_t **finished_reassemblies = NULL;
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (do_handoff))
	      ...
	      if (reass->is_complete)
		{
		  ...
		  vnet_buffer (b0)->ip.reass.payload_length =
		    reass->payload_length;
  ...
  ip6_sv_reass_t **reass_p;
  vec_foreach (reass_p, finished_reassemblies)
  {
    ip6_sv_reass_t *reass = *reass_p;
    vlib_buffer_t *b0;
    u32 next0 = IP6_SV_REASSEMBLY_NEXT_INPUT;
    u32 idx;
      ...
      if (is_feature)
	{
      ...
      vnet_buffer (b0)->ip.reass.ip_version = 6;
      vnet_buffer (b0)->ip.reass.payload_length = reass->payload_length;
      vnet_buffer (b0)->ip.reass.ip_proto = reass->ip_proto;
      vnet_buffer (b0)->ip.reass.l4_src_port = reass->l4_src_port;
      vnet_buffer (b0)->ip.reass.l4_dst_port = reass->l4_dst_port;
...
/* *INDENT-OFF* */
VNET_FEATURE_INIT (ip6_sv_reassembly_feature, static) = {
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip6_sv_reass_expire_node, static) = {
  ...
  vec_foreach (bip, reass->cached_buffers)
  {
	...
	s =
	  format (s,
		  ...
		  ip6_sv_reass_buffer_get_data_offset (b),
		  ip6_sv_reass_buffer_get_data_len (b),
