...
typedef union
{
  ...
  u64 as_u64;
} ip4_deep_reass_val_t;

typedef union
{
  struct
  {
    ip4_deep_reass_key_t k;
    ip4_deep_reass_val_t v;
  };
  clib_bihash_kv_16_8_t kv;
} ip4_deep_reass_kv_t;

always_inline u32
ip4_deep_reass_buffer_get_data_offset (vlib_buffer_t * b)
{
...
always_inline u16
ip4_deep_reass_buffer_get_data_len (vlib_buffer_t * b)
{
  vnet_buffer_opaque_t *vnb = vnet_buffer (b);
  return clib_min (vnb->ip.reass.range_last, vnb->ip.reass.fragment_last) -
    (vnb->ip.reass.fragment_first + ip4_deep_reass_buffer_get_data_offset (b)) + 1;
}

typedef struct
{
  // hash table key
  ip4_deep_reass_key_t key;
...
typedef struct
{
  ip4_deep_reass_t *pool;
  u32 reass_n;
  u32 id_counter;
  clib_spinlock_t lock;
} ip4_deep_reass_per_thread_t;
...
typedef struct
{
  ...
  // per-thread data
  ip4_deep_reass_per_thread_t *per_thread_data;
  ...
  u32 ip4_drop_idx;
  u32 ip4_deep_reass_expire_node_idx;

  /** Worker handoff */
  u32 fq_index;
  u32 fq_feature_index;

} ip4_deep_reass_main_t;

extern ip4_deep_reass_main_t ip4_deep_reass_main;

#ifndef CLIB_MARCH_VARIANT
ip4_deep_reass_main_t ip4_deep_reass_main;
...
typedef enum
{
  ...
  RANGE_OVERLAP,
  FINALIZE,
} ip4_deep_reass_trace_operation_e;
...
typedef struct
{
  ...
  u32 data_len;
  u32 first_bi;
} ip4_deep_reass_range_trace_t;

typedef struct
{
  ip4_deep_reass_trace_operation_e action;
  u32 reass_id;
  ip4_deep_reass_range_trace_t trace_range;
  u32 size_diff;
  u32 op_id;
  u32 fragment_first;
  u32 fragment_last;
  u32 total_data_len;
} ip4_deep_reass_trace_t;

extern vlib_node_registration_t ip4_deep_reass_node;
extern vlib_node_registration_t ip4_deep_reass_node_feature;

static void
ip4_deep_reass_trace_details (vlib_main_t * vm, u32 bi,
			 ip4_deep_reass_range_trace_t * trace)
{
  vlib_buffer_t *b = vlib_get_buffer (vm, bi);
  vnet_buffer_opaque_t *vnb = vnet_buffer (b);
  trace->range_first = vnb->ip.reass.range_first;
  trace->range_last = vnb->ip.reass.range_last;
  trace->data_offset = ip4_deep_reass_buffer_get_data_offset (b);
  trace->data_len = ip4_deep_reass_buffer_get_data_len (b);
  trace->range_bi = bi;
}

static u8 *
format_ip4_deep_reass_range_trace (u8 * s, va_list * args)
{
  ip4_deep_reass_range_trace_t *trace = va_arg (*args, ip4_deep_reass_range_trace_t *);
...
static u8 *
format_ip4_deep_reass_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  ip4_deep_reass_trace_t *t = va_arg (*args, ip4_deep_reass_trace_t *);
  ...
  switch (t->action)
    ...
    case RANGE_SHRINK:
      s = format (s, "\n%Ushrink %U by %u", format_white_space, indent,
		  format_ip4_deep_reass_range_trace, &t->trace_range,
		  t->size_diff);
      break;
    case RANGE_DISCARD:
...
static void
ip4_deep_reass_add_trace (vlib_main_t * vm, vlib_node_runtime_t * node,
		     ip4_deep_reass_main_t * rm, ip4_deep_reass_t * reass, u32 bi,
		     ip4_deep_reass_trace_operation_e action, u32 size_diff)
{
  vlib_buffer_t *b = vlib_get_buffer (vm, bi);
  vnet_buffer_opaque_t *vnb = vnet_buffer (b);
  ...
  ip4_deep_reass_trace_t *t = vlib_add_trace (vm, node, b, sizeof (t[0]));
  t->reass_id = reass->id;
  t->action = action;
  ip4_deep_reass_trace_details (vm, bi, &t->trace_range);
...
#if 0
  static u8 *s = NULL;
  s = format (s, "%U", format_ip4_deep_reass_trace, NULL, NULL, t);
  printf ("%.*s\n", vec_len (s), s);
  fflush (stdout);
  vec_reset_length (s);
#endif
...
always_inline void
ip4_deep_reass_free (ip4_deep_reass_main_t * rm, ip4_deep_reass_per_thread_t * rt,
		ip4_deep_reass_t * reass)
{
  clib_bihash_kv_16_8_t kv;
...
always_inline void
ip4_deep_reass_on_timeout (vlib_main_t * vm, ip4_deep_reass_main_t * rm,
		      ip4_deep_reass_t * reass)
{
...
always_inline void
ip4_deep_reass_init (ip4_deep_reass_t * reass)
{
  reass->first_bi = ~0;
...
always_inline ip4_deep_reass_t *
ip4_deep_reass_find_or_create (vlib_main_t * vm, ip4_deep_reass_main_t * rm,
			  ip4_deep_reass_per_thread_t * rt, ip4_deep_reass_kv_t * kv,
			  u8 * do_handoff)
{
  ip4_deep_reass_t *reass = NULL;
  ...
  if (!clib_bihash_search_16_8
	...
	{
	  ip4_deep_reass_on_timeout (vm, rm, reass);
	  ip4_deep_reass_free (rm, rt, reass);
	  reass = NULL;
	}
    }
...
always_inline ip4_deep_reass_rc_t
ip4_deep_reass_finalize_deep (vlib_main_t * vm, vlib_node_runtime_t * node,
			 ip4_deep_reass_main_t * rm, ip4_deep_reass_per_thread_t * rt,
			 ip4_deep_reass_t * reass, u32 * bi0, u32 * next0,
			 u32 * error0, bool is_feature)
{
  ...
  do
	  ...
	  !(vnb->ip.reass.range_last > vnb->ip.reass.fragment_first))
	{
      ...
      u32 data_len = ip4_deep_reass_buffer_get_data_len (tmp);
      u32 trim_front =
	ip4_header_bytes (ip) + ip4_deep_reass_buffer_get_data_offset (tmp);
      u32 trim_end =
	vlib_buffer_length_in_chain (vm, tmp) - trim_front - data_len;
      if (tmp_bi == reass->first_bi)
	{
	  /* first buffer - keep ip4 header */
	  if (0 != ip4_deep_reass_buffer_get_data_offset (tmp))
	    {
	      return IP4_REASS_RC_INTERNAL_ERROR;
	    }
	  trim_front = 0;
  ...
  if (PREDICT_FALSE (first_b->flags & VLIB_BUFFER_IS_TRACED))
    {
      ip4_deep_reass_add_trace (vm, node, rm, reass, reass->first_bi, FINALIZE, 0);
#if 0
...
always_inline ip4_deep_reass_rc_t
ip4_deep_reass_insert_range_in_chain (vlib_main_t * vm,
				 ip4_deep_reass_main_t * rm,
				 ip4_deep_reass_per_thread_t * rt,
				 ip4_deep_reass_t * reass,
				 u32 prev_range_bi, u32 new_next_bi)
{
  ...
  reass->data_len += ip4_deep_reass_buffer_get_data_len (new_next_b);
  return IP4_REASS_RC_OK;
}

always_inline ip4_deep_reass_rc_t
ip4_deep_reass_remove_range_from_chain (vlib_main_t * vm,
				   vlib_node_runtime_t * node,
				   ip4_deep_reass_main_t * rm,
				   ip4_deep_reass_t * reass, u32 prev_range_bi,
				   u32 discard_bi)
{
  ...
  reass->data_len -= ip4_deep_reass_buffer_get_data_len (discard_b);
  while (1)
    {
      vlib_buffer_free_one (vm, discard_bi);
      if (PREDICT_FALSE (discard_b->flags & VLIB_BUFFER_IS_TRACED))
	{
	  ip4_deep_reass_add_trace (vm, node, rm, reass, discard_bi, RANGE_DISCARD,
			       0);
	}
...
always_inline ip4_deep_reass_rc_t
ip4_deep_reass_update (vlib_main_t * vm, vlib_node_runtime_t * node,
		  ip4_deep_reass_main_t * rm, ip4_deep_reass_per_thread_t * rt,
		  ip4_deep_reass_t * reass, u32 * bi0, u32 * next0,
		  u32 * error0, bool is_feature)
{
  ...
  ip4_deep_reass_rc_t rc = IP4_REASS_RC_OK;
  int consumed = 0;
  ...
  if (~0 == reass->first_bi)
    {
      ...
      rc =
	ip4_deep_reass_insert_range_in_chain (vm, rm, rt, reass, prev_range_bi,
					 *bi0);
	...
	{
	  ip4_deep_reass_add_trace (vm, node, rm, reass, *bi0, RANGE_NEW, 0);
	}
      *bi0 = ~0;
      ...
      if (fragment_first > candidate_vnb->ip.reass.range_last)
	{
	  ...
	  if (candidate_vnb->ip.reass.range_last < fragment_last &&
	      ~0 == candidate_range_bi)
	    {
	      ...
	      rc =
		ip4_deep_reass_insert_range_in_chain (vm, rm, rt, reass,
						 prev_range_bi, *bi0);
	      if (IP4_REASS_RC_OK != rc)
		{
		  return rc;
      ...
      else
	{
	  if (fragment_first >= candidate_vnb->ip.reass.range_first &&
	      fragment_last <= candidate_vnb->ip.reass.range_last)
	    {
	      ...
	      if (PREDICT_FALSE (fb->flags & VLIB_BUFFER_IS_TRACED))
		{
		  ip4_deep_reass_add_trace (vm, node, rm, reass, *bi0,
				       RANGE_OVERLAP, 0);
		}
	      break;
	    }
	  ...
	  if (fragment_first < candidate_vnb->ip.reass.range_first)
		...
		fragment_last - candidate_vnb->ip.reass.range_first + 1;
	      if (overlap < ip4_deep_reass_buffer_get_data_len (candidate_b))
		{
		  ...
		  if (PREDICT_FALSE (fb->flags & VLIB_BUFFER_IS_TRACED))
		    {
		      ip4_deep_reass_add_trace (vm, node, rm, reass,
	  ...
	  if (discard_candidate)
	    {
	      ...
	      rc =
		ip4_deep_reass_remove_range_from_chain (vm, node, rm, reass,
						   prev_range_bi,
						   candidate_range_bi);
	      if (IP4_REASS_RC_OK != rc)
		{
  ...
  if (~0 != reass->last_packet_octet &&
    ...
    {
      return ip4_deep_reass_finalize_deep (vm, node, rm, rt, reass, bi0, next0,
				      error0, is_feature);
    }
  else
    {
...
always_inline uword
ip4_deep_reass_inline (vlib_main_t * vm, vlib_node_runtime_t * node,
		       vlib_frame_t * frame, bool is_feature)
{
  u32 *from = vlib_frame_vector_args (frame);
  u32 n_left_from, n_left_to_next, *to_next, next_index;
  ip4_deep_reass_main_t *rm = &ip4_deep_reass_main;
  ip4_deep_reass_per_thread_t *rt = &rm->per_thread_data[vm->thread_index];
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  ip4_deep_reass_t *reass =
	    ip4_deep_reass_find_or_create (vm, rm, rt, &kv, &do_handoff);
	  ...
	  if (PREDICT_FALSE (do_handoff))
	    ...
	    {
	      ip4_deep_reass_rc_t rc =
		ip4_deep_reass_update (vm, node, rm, rt, reass, &bi0, &next0,
				  &error0, is_feature);
	      switch (rc)
		{
...
static char *ip4_deep_reass_error_strings[] = {
#define _(sym, string) string,
  foreach_ip4_error
#undef _
};

VLIB_NODE_FN (ip4_deep_reass_node) (vlib_main_t * vm, vlib_node_runtime_t * node,
			       vlib_frame_t * frame)
{
  return ip4_deep_reass_inline (vm, node, frame, false /* is_feature */ );
}

/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_deep_reass_node) = {
    .name = "ip4-drembly",
    .vector_size = sizeof (u32),
    .format_trace = format_ip4_deep_reass_trace,
    .n_errors = ARRAY_LEN (ip4_deep_reass_error_strings),
    .error_strings = ip4_deep_reass_error_strings,
    .n_next_nodes = IP4_REASSEMBLY_N_NEXT,
    .next_nodes =
        {
                [IP4_REASSEMBLY_NEXT_INPUT] = "ip4-input",
                [IP4_REASSEMBLY_NEXT_DROP] = "ip4-drop",
                [IP4_REASSEMBLY_NEXT_HANDOFF] = "ip4-drembly-handoff",

        },
};
/* *INDENT-ON* */

VLIB_NODE_FN (ip4_deep_reass_node_feature) (vlib_main_t * vm,
				       vlib_node_runtime_t * node,
				       vlib_frame_t * frame)
{
  return ip4_deep_reass_inline (vm, node, frame, true /* is_feature */ );
}

/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_deep_reass_node_feature) = {
    .name = "ip4-drembly-feature",
    .vector_size = sizeof (u32),
    .format_trace = format_ip4_deep_reass_trace,
    .n_errors = ARRAY_LEN (ip4_deep_reass_error_strings),
    .error_strings = ip4_deep_reass_error_strings,
    ...
    .next_nodes =
        {
                [IP4_REASSEMBLY_NEXT_INPUT] = "ip4-input",
                [IP4_REASSEMBLY_NEXT_DROP] = "ip4-drop",
                [IP4_REASSEMBLY_NEXT_HANDOFF] = "ip4-dr-feature-hoff",
        },
};
/* *INDENT-ON* */

/* *INDENT-OFF* */
VNET_FEATURE_INIT (ip4_deep_reass_feature, static) = {
    .arc_name = "ip4-unicast",
    .node_name = "ip4-drembly-feature",
...
static void
ip4_deep_reass_set_params (u32 timeout_ms, u32 max_reassemblies,
		      u32 expire_walk_interval_ms)
{
  ip4_deep_reass_main.timeout_ms = timeout_ms;
  ip4_deep_reass_main.timeout = (f64) timeout_ms / (f64) MSEC_PER_SEC;
  ip4_deep_reass_main.max_reass_n = max_reassemblies;
  ip4_deep_reass_main.expire_walk_interval_ms = expire_walk_interval_ms;
}

vnet_api_error_t
ip4_deep_reass_set (u32 timeout_ms, u32 max_reassemblies,
	       u32 expire_walk_interval_ms)
{
  u32 old_nbuckets = ip4_deep_reass_get_nbuckets ();
  ip4_deep_reass_set_params (timeout_ms, max_reassemblies,
			expire_walk_interval_ms);
  vlib_process_signal_event (ip4_deep_reass_main.vlib_main,
			     ip4_deep_reass_main.ip4_deep_reass_expire_node_idx,
			     IP4_EVENT_CONFIG_CHANGED, 0);
  u32 new_nbuckets = ip4_deep_reass_get_nbuckets ();
  if (ip4_deep_reass_main.max_reass_n > 0 && new_nbuckets > old_nbuckets)
    {
      clib_bihash_16_8_t new_hash;
      ...
      clib_bihash_init_16_8 (&new_hash, "ip4-dr", new_nbuckets,
			     new_nbuckets * 1024);
      clib_bihash_foreach_key_value_pair_16_8 (&ip4_deep_reass_main.hash,
					       ip4_rehash_cb, &ctx);
      if (ctx.failure)
	{
	...
	{
	  clib_bihash_free_16_8 (&ip4_deep_reass_main.hash);
	  clib_memcpy_fast (&ip4_deep_reass_main.hash, &new_hash,
			    sizeof (ip4_deep_reass_main.hash));
	}
    }
  return 0;
}
...
vnet_api_error_t
ip4_deep_reass_get (u32 * timeout_ms, u32 * max_reassemblies,
	       u32 * expire_walk_interval_ms)
{
  *timeout_ms = ip4_deep_reass_main.timeout_ms;
  *max_reassemblies = ip4_deep_reass_main.max_reass_n;
  *expire_walk_interval_ms = ip4_deep_reass_main.expire_walk_interval_ms;
  return 0;
}

static clib_error_t *
ip4_deep_reass_init_function (vlib_main_t * vm)
{
  ip4_deep_reass_main_t *rm = &ip4_deep_reass_main;
  ...
  node = vlib_get_node_by_name (vm, (u8 *) "ip4-drembly-expire-walk");
  ASSERT (node);
  rm->ip4_deep_reass_expire_node_idx = node->index;

  ip4_deep_reass_set_params (IP4_REASS_TIMEOUT_DEFAULT_MS,
			IP4_REASS_MAX_REASSEMBLIES_DEFAULT,
			IP4_REASS_EXPIRE_WALK_INTERVAL_DEFAULT_MS);

  nbuckets = ip4_deep_reass_get_nbuckets ();
  ...
  rm->fq_index = vlib_frame_queue_main_init (ip4_deep_reass_node.index, 0);
  rm->fq_feature_index =
    vlib_frame_queue_main_init (ip4_deep_reass_node_feature.index, 0);


  return error;
}

VLIB_INIT_FUNCTION (ip4_deep_reass_init_function);
#endif /* CLIB_MARCH_VARIANT */

static uword
ip4_deep_reass_walk_expired (vlib_main_t * vm,
			vlib_node_runtime_t * node, vlib_frame_t * f)
{
  ...
  while (true)
	...
	{
	  ip4_deep_reass_per_thread_t *rt = &rm->per_thread_data[thread_index];
	  clib_spinlock_lock (&rt->lock);
	  ...
	  int *i;
          ...
          {
            ip4_deep_reass_t *reass = pool_elt_at_index (rt->pool, i[0]);
            ip4_deep_reass_on_timeout (vm, rm, reass);
            ip4_deep_reass_free (rm, rt, reass);
          }
          /* *INDENT-ON* */
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_deep_reass_expire_node, static) = {
    .function = ip4_deep_reass_walk_expired,
    .type = VLIB_NODE_TYPE_PROCESS,
    .name = "ip4-drembly-expire-walk",
    .format_trace = format_ip4_deep_reass_trace,
    .n_errors = ARRAY_LEN (ip4_deep_reass_error_strings),
    .error_strings = ip4_deep_reass_error_strings,
...
static u8 *
format_ip4_deep_reass_key (u8 * s, va_list * args)
{
  ip4_deep_reass_key_t *key = va_arg (*args, ip4_deep_reass_key_t *);
  s =
    format (s,
...
format_ip4_reass (u8 * s, va_list * args)
{
  ...
  ip4_deep_reass_t *reass = va_arg (*args, ip4_deep_reass_t *);

  s = format (s, "ID: %lu, key: %U\n  first_bi: %u, data_len: %u, "
	      "last_packet_octet: %u, trace_op_counter: %u\n",
	      reass->id, format_ip4_deep_reass_key, &reass->key,
  ...
  while (~0 != bi)
	...
	format (s,
		...
		ip4_deep_reass_buffer_get_data_offset (b),
		ip4_deep_reass_buffer_get_data_len (b),
...
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (show_ip4_deep_reass_cmd, static) = {
    .path = "show ip4-drembly",
    .short_help = "show ip4-drembly [details]",
...
#ifndef CLIB_MARCH_VARIANT
vnet_api_error_t
ip4_deep_reass_enable_disable (u32 sw_if_index, u8 enable_disable)
{
  return vnet_feature_enable_disable ("ip4-unicast",
				      "ip4-drembly-feature", sw_if_index,
				      enable_disable, 0, 0);
}
#endif /* CLIB_MARCH_VARIANT */


#define foreach_ip4_deep_reass_handoff_error                       \
...
#define _(sym,str) IP4_REASSEMBLY_HANDOFF_ERROR_##sym,
  foreach_ip4_deep_reass_handoff_error
#undef _
    IP4_REASSEMBLY_HANDOFF_N_ERROR,
} ip4_deep_reass_handoff_error_t;

static char *ip4_deep_reass_handoff_error_strings[] = {
#define _(sym,string) string,
  foreach_ip4_deep_reass_handoff_error
#undef _
};
...
} ip4_deep_reass_handoff_trace_t;

static u8 *
format_ip4_deep_reass_handoff_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  ip4_deep_reass_handoff_trace_t *t =
    va_arg (*args, ip4_deep_reass_handoff_trace_t *);

  s =
    format (s, "ip4-drembly-handoff: next-worker %d",
	    t->next_worker_index);
...
always_inline uword
ip4_deep_reass_handoff_node_inline (vlib_main_t * vm,
				    vlib_node_runtime_t * node,
				    vlib_frame_t * frame, bool is_feature)
{
  ip4_deep_reass_main_t *rm = &ip4_deep_reass_main;
...
VLIB_NODE_FN (ip4_deep_reass_handoff_node) (vlib_main_t * vm,
					    vlib_node_runtime_t * node,
					    vlib_frame_t * frame)
{
  return ip4_deep_reass_handoff_node_inline (vm, node, frame,
					     false /* is_feature */ );
}


/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_deep_reass_handoff_node) = {
  .name = "ip4-drembly-handoff",
  .vector_size = sizeof (u32),
  .n_errors = ARRAY_LEN(ip4_deep_reass_handoff_error_strings),
  .error_strings = ip4_deep_reass_handoff_error_strings,
  .format_trace = format_ip4_deep_reass_handoff_trace,
...
/* *INDENT-OFF* */
VLIB_NODE_FN (ip4_deep_reass_feature_handoff_node) (vlib_main_t * vm,
						    vlib_node_runtime_t *
						    node,
						    vlib_frame_t * frame)
{
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_deep_reass_feature_handoff_node) = {
