...
VLIB_NODE_FN (ip4_full_reass_node) (vlib_main_t * vm,
				    ...
				    vlib_frame_t * frame)
{
  return ip4_full_reass_inline (vm, node, frame, NORMAL, false /* is_forus */);
}
...
VLIB_NODE_FN (ip4_full_forus_reass_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return ip4_full_reass_inline (vm, node, frame, NORMAL, true /* is_forus */);
}

VLIB_REGISTER_NODE (ip4_full_forus_reass_node) = {
    .name = "ip4-full-forus-reassembly",
    .vector_size = sizeof (u32),
    .format_trace = format_ip4_full_reass_trace,
    .n_errors = ARRAY_LEN (ip4_full_reass_error_strings),
    .error_strings = ip4_full_reass_error_strings,
    ...
    .next_nodes =
        {
                [IP4_FULL_REASS_NEXT_INPUT] = "ip4-input",
                [IP4_FULL_REASS_NEXT_DROP] = "ip4-drop",
                [IP4_FULL_REASS_NEXT_HANDOFF] = "ip4-full-forus-reassembly-handoff",
...
VLIB_NODE_FN (ip4_full_reass_node_custom) (vlib_main_t * vm,
					   ...
					   vlib_frame_t * frame)
{
  return ip4_full_reass_inline (vm, node, frame, CUSTOM, false /* is_forus */);
}
...
VLIB_NODE_FN (ip4_full_forus_reass_handoff_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return ip4_full_reass_handoff_node_inline (vm, node, frame, NORMAL,
					     true /* is_forus */);
...
VLIB_REGISTER_NODE (ip4_full_forus_reass_handoff_node) = {
  .name = "ip4-full-forus-reassembly-handoff",
  .vector_size = sizeof (u32),
  .n_errors = ARRAY_LEN(ip4_full_reass_handoff_error_strings),
  .error_strings = ip4_full_reass_handoff_error_strings,
  .format_trace = format_ip4_full_reass_handoff_trace,
...
void
ip4_full_forus_reass_enable_disable (int enable)
{
  if (enable)
    {
