...
always_inline void
ip4_full_reass_drop_all (vlib_main_t * vm, vlib_node_runtime_t * node, ip4_full_reass_main_t * rm, ip4_full_reass_t * reass, bool is_feature)
{
...
always_inline void
ip4_full_reass_init (ip4_full_reass_t * reass, bool is_feature)
{
  reass->first_bi = ~0;
...
ip4_full_reass_find_or_create (vlib_main_t * vm, vlib_node_runtime_t *node, ip4_full_reass_main_t * rm, ip4_full_reass_per_thread_t * rt, ip4_full_reass_kv_t * kv, u8 * do_handoff, bool is_feature)
{
  ...
  if (!clib_bihash_search_16_8
	...
	{
	  ip4_full_reass_drop_all (vm, node, rm, reass, is_feature);
  ...
  if (rt->reass_n >= rm->max_reass_n)
  ...
  else
    {
      ...
      ++rt->id_counter;
      ip4_full_reass_init (reass, is_feature);
      ++rt->reass_n;
    }
...
ip4_full_reass_finalize_deep (vlib_main_t * vm, vlib_node_runtime_t * node,
			      ...
			      u32 * next0, u32 * error0, bool is_feature)
{
		       ...
		       u32 * error0, bool is_feature)
{
  ...
  if (~0 != reass->last_packet_octet &&
    ...
    {
      return ip4_full_reass_finalize_deep (vm, node, rm, rt, reass, bi0,
					   next0, error0, is_feature);
    }
  else
    {
      if (consumed)
		       ...
		       vlib_frame_t * frame, bool is_feature)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  ip4_full_reass_t *reass =
	    ip4_full_reass_find_or_create (vm, node, rm, rt, &kv, &do_handoff, is_feature);
      ...
      for (thread_index = 0; thread_index < nthreads; ++thread_index)
	{
	  ...
	  int *i;
          ...
          vec_foreach (i, pool_indexes_to_free)
          {
            ...
            ip4_full_reass_drop_all (vm, node, rm, reass, reass->is_feature);
            ip4_full_reass_free (rm, rt, reass);
          }
          /* *INDENT-ON* */
