...
always_inline u32
ip4_sv_reass_buffer_get_data_offset (vlib_buffer_t * b)
{
...
always_inline u16
ip4_sv_reass_buffer_get_data_len (vlib_buffer_t * b)
{
  vnet_buffer_opaque_t *vnb = vnet_buffer (b);
  return clib_min (vnb->ip.reass.range_last, vnb->ip.reass.fragment_last) -
    (vnb->ip.reass.fragment_first + ip4_sv_reass_buffer_get_data_offset (b)) +
    1;
}
		     ...
		     ip4_sv_reass_t * reass, u32 bi0)
{
  ...
  if (0 == fragment_first)
    {
      ...
      switch (reass->ip_proto)
	{
	case IP_PROTOCOL_UDP:
	  do
	    {
	      udp_header_t *udp = (udp_header_t *) (fip + 1);
	...
	case IP_PROTOCOL_TCP:
	  do
	    {
	      tcp_header_t *tcp = (tcp_header_t *) (fip + 1);
  ...
  if (PREDICT_FALSE (fb->flags & VLIB_BUFFER_IS_TRACED))
    {
      ip4_sv_reass_add_trace (vm, node, rm, reass, bi0, REASS_FRAGMENT_CACHE);
    }
		     ...
		     vlib_frame_t * frame, bool is_feature)
{
  ...
  ip4_sv_reass_t **finished_reassemblies = NULL;
  ...
  ip4_sv_reass_t **reass_p;
  vec_foreach (reass_p, finished_reassemblies)
  {
    ip4_sv_reass_t *reass = *reass_p;
    vlib_buffer_t *b0;
  ...
  vec_foreach (bip, reass->cached_buffers)
  {
	...
	s =
	  format (s,
		  ...
		  ip4_sv_reass_buffer_get_data_offset (b),
		  ip4_sv_reass_buffer_get_data_len (b),
