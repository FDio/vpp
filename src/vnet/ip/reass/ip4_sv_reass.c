...
typedef struct
{
  ...
  u8 next_index;
  // fifo of cached buffer indexes
  u32 *cached_buffers;
  // true if this reassembly is completed
  bool is_complete;
  // l4 ip protocol
  u8 ip_proto;
  // l4 source port
...
format_ip4_sv_reass_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "reass id: %u", t->reass_id);
  return s;
}
...
ip4_sv_reass_update (vlib_main_t * vm, vlib_node_runtime_t * node,
		     ip4_sv_reass_t * reass, u32 bi0, bool is_feature)
{
  ...
  if (0 == fragment_first)
	...
	case IP_PROTOCOL_TCP:
	  t = (tcp_header_t *) (fip + 1);
	  reass->src_port = t->src_port;
	  reass->dst_port = t->dst_port;
	  break;
	case IP_PROTOCOL_UDP:
	  u = (udp_header_t *) (fip + 1);
	  reass->src_port = u->src_port;
	  reass->dst_port = u->dst_port;
	  break;
	}
      u32 *bi;
      vec_foreach (bi, reass->cached_buffers)
      {
	vlib_buffer_t *b = vlib_get_buffer (vm, *bi);
	vnet_buffer (b)->ip.reass.ip_proto = reass->ip_proto;
	vnet_buffer (b)->ip.reass.l4_src_port = reass->src_port;
	vnet_buffer (b)->ip.reass.l4_dst_port = reass->dst_port;
      }
    }
  ip4_sv_reass_add_trace (vm, node, reass, bi0);
  return;
}
		     ...
		     vlib_frame_t * frame, bool is_feature)
{
  ...
  ip4_sv_reass_t *completed_reass = NULL;
  u32 completed_reass_cached_buffers_offset = 0;
  ...
  while (n_left_from > 0)
    {
      ...
      if (completed_reass)
	{
	  while (completed_reass_cached_buffers_offset <
		 vec_len (completed_reass->cached_buffers)
		 && n_left_to_next > 0)
	    {
	      u32 bi0 = vec_elt (completed_reass->cached_buffers,
				 completed_reass_cached_buffers_offset);
	      to_next[0] = bi0;
	      to_next += 1;
	      n_left_to_next -= 1;
	  ...
	  if (completed_reass_cached_buffers_offset >=
	      vec_len (completed_reass->cached_buffers))
	    {
	      completed_reass = NULL;
	      completed_reass_cached_buffers_offset = 0;
	    }
	}
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (do_handoff))
		...
		{
		  ip4_sv_reass_update (vm, node, reass, bi0, is_feature);
		  if (reass->is_complete)
		    {
		      completed_reass = reass;
		      completed_reass_cached_buffers_offset = 0;
		    }
		  goto next_packet;
		}
    ...
    .next_nodes =
        {
                [IP4_SV_REASSEMBLY_NEXT_INPUT] = "ip4-input",
                [IP4_SV_REASSEMBLY_NEXT_DROP] = "ip4-drop",
                [IP4_SV_REASSEMBLY_NEXT_HANDOFF] = "ip4-svr-feature-hoff",
        },
};
/* *INDENT-ON* */

/* *INDENT-OFF* */
VNET_FEATURE_INIT (ip4_sv_reass_feature, static) = {
  ...
  if (ip4_sv_reass_main.max_reass_n > 0 && new_nbuckets > old_nbuckets)
    {
      ...
      clib_bihash_init_16_8 (&new_hash, "ip4-svr", new_nbuckets,
			     new_nbuckets * 1024);
...
ip4_sv_reass_init_function (vlib_main_t * vm)
{
  ...
  ip4_sv_reass_set_params (IP4_REASS_TIMEOUT_DEFAULT_MS,
			   IP4_REASS_MAX_REASSEMBLIES_DEFAULT,
			   IP4_REASS_EXPIRE_WALK_INTERVAL_DEFAULT_MS);

  nbuckets = ip4_sv_reass_get_nbuckets ();
  clib_bihash_init_16_8 (&rm->hash, "ip4-svr", nbuckets, nbuckets * 1024);
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip4_sv_reass_expire_node, static) = {
...
format_ip4_sv_reass (u8 * s, va_list * args)
{
  ...
  s =
    format (s,
	    "ID: %lu, key: %U\n  is_completed: %s, cached_buffers: %u\n",
	    reass->id, format_ip4_sv_reass_key, &reass->key,
	    reass->is_complete ? "yes" : "no",
	    vec_len (reass->cached_buffers));
...
static clib_error_t *
show_ip4_sv_reass (vlib_main_t * vm,
  ...
  for (thread_index = 0; thread_index < nthreads; ++thread_index)
      ...
      if (details)
	{
          ...
          pool_foreach (reass, rt->pool, {
            vlib_cli_output (vm, "%U", format_ip4_sv_reass, reass);
          });
          /* *INDENT-ON* */
	}
