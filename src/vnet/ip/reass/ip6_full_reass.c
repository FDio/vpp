...
ip6_full_reassembly_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			    ...
			    bool is_custom_app, bool is_forus)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_forus && !rm->is_forus_reass_enabled)
	    {
	      next0 = IP6_FULL_REASSEMBLY_NEXT_DROP;
	      goto skip_reass;
	    }
...
VLIB_NODE_FN (ip6_full_forus_reass_node)
...
VLIB_REGISTER_NODE (ip6_full_forus_reass_node) = {
    .name = "ip6-full-forus-reassembly",
    .vector_size = sizeof (u32),
    .format_trace = format_ip6_full_reass_trace,
    .n_errors = ARRAY_LEN (ip6_full_reassembly_error_strings),
    .error_strings = ip6_full_reassembly_error_strings,
    ...
    .next_nodes =
        {
                ...
                [IP6_FULL_REASSEMBLY_NEXT_DROP] = "ip6-drop",
                [IP6_FULL_REASSEMBLY_NEXT_ICMP_ERROR] = "ip6-icmp-error",
                [IP6_FULL_REASSEMBLY_NEXT_HANDOFF] = "ip6-full-forus-reassembly-handoff",
        },
};
						 ...
						 vlib_frame_t * frame)
{
  return ip6_full_reassembly_handoff_inline (
    vm, node, frame, false /* is_feature */, false /* is_forus */);
}
...
VLIB_NODE_FN (ip6_full_forus_reassembly_handoff_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return ip6_full_reassembly_handoff_inline (
...
VLIB_REGISTER_NODE (ip6_full_forus_reassembly_handoff_node) = {
  .name = "ip6-full-forus-reassembly-handoff",
  .vector_size = sizeof (u32),
  .n_errors = ARRAY_LEN(ip6_full_reassembly_handoff_error_strings),
  .error_strings = ip6_full_reassembly_handoff_error_strings,
  .format_trace = format_ip6_full_reassembly_handoff_trace,
...
{
  return ip6_full_reassembly_handoff_inline (
    vm, node, frame, true /* is_feature */, false /* is_forus */);
}
...
void
ip6_full_forus_reass_enable_disable (int enable)
{
  if (enable)
    {
