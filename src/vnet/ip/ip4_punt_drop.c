...
ip4_punt_policer_add_del (u8 is_add, u32 policer_index)
{
  ...
  vnet_feature_enable_disable ("ip4-punt", "ip4-punt-policer", 0, is_add, 0,
			       0);
}
#endif /* CLIB_MARCH_VARIANT */

static clib_error_t *
ip4_punt_police_cmd (vlib_main_t *vm, unformat_input_t *main_input,
...
void
ip4_punt_redirect_add (u32 rx_sw_if_index, u32 tx_sw_if_index,
		       ip46_address_t *nh)
{
...
{
  ip_punt_redirect_add (FIB_PROTOCOL_IP4, rx_sw_if_index,
...
static clib_error_t *
ip4_punt_redirect_cmd (vlib_main_t *vm, unformat_input_t *main_input,
		       vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  ip46_address_t nh = { 0 };
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
			 ...
			 &nh.ip4, unformat_vnet_sw_interface, vnm,
			 &tx_sw_if_index))
	;
      else if (unformat (line_input, "via %U", unformat_vnet_sw_interface, vnm,
			 &tx_sw_if_index))
	;
      else
	{
...
VLIB_CLI_COMMAND (ip4_punt_redirect_command, static) = {
  ...
  .short_help = "ip punt redirect [add|del] rx [<interface>|all] via [<nh>] "
		"<tx_interface>",
};

static clib_error_t *
ip4_punt_redirect_show_cmd (vlib_main_t *vm, unformat_input_t *main_input,
			    vlib_cli_command_t *cmd)
{
