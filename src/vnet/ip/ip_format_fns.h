...
format_vl_api_ip6_prefix_t (u8 * s, va_list * args)
{
  ...
  return format (s, "%U/%d", format_ip6_address, &a->address, a->len);
}
...
format_vl_api_ip4_prefix_t (u8 * s, va_list * args)
{
  ...
  return format (s, "%U/%d", format_ip4_address, &a->address, a->len);
}
...
format_vl_api_prefix_t (u8 * s, va_list * args)
{
  ...
  return format (s, "%U/%d", format_vl_api_address_t, &a->address, a->len);
}
