...
ip_in_out_acl_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		      vlib_buffer_t **b, u16 *next, u32 n_left, int is_ip4,
		      int is_output, int do_trace)
{
  ...
  if (n_left >= 2)
	...
	vnet_buffer (b[1])->sw_if_index[is_output ? VLIB_TX : VLIB_RX];
      ...
      table_index[2] = am->classify_table_index_by_sw_if_index[is_output][tid]
							      [sw_if_index[2]];
      table_index[3] = am->classify_table_index_by_sw_if_index[is_output][tid]
							      [sw_if_index[3]];
  ...
  while (n_left >= 2)
      ...
      if (n_left >= 4)
	{
	  ...
	  if (t[2]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
	    h[2] = (void *) vlib_buffer_get_current (b[2]) +
		   t[2]->current_data_offset;
	  else
	    h[2] = b[2]->data;

	  if (t[3]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
	    h[3] = (void *) vlib_buffer_get_current (b[3]) +
	  ...
	  if (is_output)
	    {
	      /* Save the rewrite length, since we are using the l2_classify
	       * struct */
	  ...
	  vnet_classify_prefetch_bucket (t[2],
					 vnet_buffer (b[2])->l2_classify.hash);
	  vnet_classify_prefetch_bucket (t[3],
					 vnet_buffer (b[3])->l2_classify.hash);
	}
      ...
      if (PREDICT_TRUE (table_index[0] != ~0))
	{
	  ...
	  if (e[0])
		...
		e[0]->opaque_index;
	      ...
	      _next[0] = (e[0]->next_index < n_next_nodes) ? e[0]->next_index :
							     _next[0];
	      ...
	      while (1)
		{
		  if (PREDICT_TRUE (t[0]->next_table_index != ~0))
		    t[0] =
		      pool_elt_at_index (vcm->tables, t[0]->next_table_index);
		  else
		    {
		  ...
		  if (t[0]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
		    h[0] = (void *) vlib_buffer_get_current (b[0]) +
			   t[0]->current_data_offset;
		  else
		    h[0] = b[0]->data;

		  /* advance the match pointer so the matching happens on IP
		   * header */
		  if (is_output)
		  ...
		  e[0] = vnet_classify_find_entry_inline (t[0], (u8 *) h[0],
							  hash[0], now);
		  if (e[0])
		    {
		      vnet_buffer (b[0])->l2_classify.opaque_index =
      ...
      if (PREDICT_TRUE (table_index[1] != ~0))
	{
	  ...
	  if (e[1])
		...
		e[1]->opaque_index;
	      ...
	      _next[1] = (e[1]->next_index < n_next_nodes) ? e[1]->next_index :
							     _next[1];
	      ...
	      while (1)
		{
		  if (PREDICT_TRUE (t[1]->next_table_index != ~0))
		    t[1] =
		      pool_elt_at_index (vcm->tables, t[1]->next_table_index);
		  else
		    {
		  ...
		  if (t[1]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
		    h[1] = (void *) vlib_buffer_get_current (b[1]) +
		  ...
		  e[1] = vnet_classify_find_entry_inline (t[1], (u8 *) h[1],
							  hash[1], now);
		  if (e[1])
		    {
		      vnet_buffer (b[1])->l2_classify.opaque_index =
      ...
      if (do_trace && b[0]->flags & VLIB_BUFFER_IS_TRACED)
	{
	  ...
	  _t->offset =
	    (e[0] && t[0]) ? vnet_classify_get_offset (t[0], e[0]) : ~0;
	}
      ...
      if (do_trace && b[1]->flags & VLIB_BUFFER_IS_TRACED)
	{
	  ...
	  _t->offset =
	    (e[1] && t[1]) ? vnet_classify_get_offset (t[1], e[1]) : ~0;
	}

      if ((_next[0] == ACL_NEXT_INDEX_DENY) && is_output)
	{
	  /* on output, for the drop node to work properly, go back to ip
	   * header */
  ...
  while (n_left > 0)
      ...
      if (PREDICT_TRUE (table_index0 != ~0))
	{
	  ...
	  if (t0->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
	    h0 = (void *) vlib_buffer_get_current (b[0]) +
	  ...
	  if (e0)
	      ...
	      while (1)
		{
		  ...
		  e0 = vnet_classify_find_entry_inline (t0, (u8 *) h0, hash0,
							now);
		  if (e0)
		    {
		      vnet_buffer (b[0])->l2_classify.opaque_index =
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
    ip_in_out_acl_inline (vm, node, bufs, nexts, frame->n_vectors,
			  1 /* is_ip4 */, 0 /* is_output */, 1 /* is_trace */);
  else
    ip_in_out_acl_inline (vm, node, bufs, nexts, frame->n_vectors,
			  1 /* is_ip4 */, 0 /* is_output */, 0 /* is_trace */);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
    ip_in_out_acl_inline (vm, node, bufs, nexts, frame->n_vectors,
			  1 /* is_ip4 */, 1 /* is_output */, 1 /* is_trace */);
  else
    ip_in_out_acl_inline (vm, node, bufs, nexts, frame->n_vectors,
			  1 /* is_ip4 */, 1 /* is_output */, 0 /* is_trace */);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
    ip_in_out_acl_inline (vm, node, bufs, nexts, frame->n_vectors,
			  0 /* is_ip4 */, 0 /* is_output */, 1 /* is_trace */);
  else
    ip_in_out_acl_inline (vm, node, bufs, nexts, frame->n_vectors,
			  0 /* is_ip4 */, 0 /* is_output */, 0 /* is_trace */);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
    ip_in_out_acl_inline (vm, node, bufs, nexts, frame->n_vectors,
			  0 /* is_ip4 */, 1 /* is_output */, 1 /* is_trace */);
  else
    ip_in_out_acl_inline (vm, node, bufs, nexts, frame->n_vectors,
			  0 /* is_ip4 */, 1 /* is_output */, 0 /* is_trace */);
