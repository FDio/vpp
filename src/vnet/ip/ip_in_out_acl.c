...
ip_in_out_acl_inline (vlib_main_t * vm,
		 vlib_node_runtime_t * node, vlib_frame_t * frame, int is_ip4, int is_output)
{
  ...
  if (is_ip4)
    {
      ...
      if (is_output)
	error_node = vlib_node_get_runtime (vm, ip4_input_node.index); // FIXME!!!
      else
  ...
  else
    {
      ...
      if (is_output)
        error_node = vlib_node_get_runtime (vm, ip6_input_node.index); // FIXME !!!
      else
	      ...
	      if (e0)
		{
		  ...
		  if (is_ip4)
		    error0 = (next0 == ACL_NEXT_INDEX_DENY) ?
		      (is_output ? IP4_ERROR_OUTACL_SESSION_DENY : IP4_ERROR_INACL_SESSION_DENY) : IP4_ERROR_NONE;
		  else
		    error0 = (next0 == ACL_NEXT_INDEX_DENY) ?
		      (is_output ? IP6_ERROR_OUTACL_SESSION_DENY : IP6_ERROR_INACL_SESSION_DENY) : IP6_ERROR_NONE;
	      ...
	      else
		{
		  while (1)
		      ...
		      else
			{
			  ...
			  if (is_ip4)
			    error0 = (next0 == ACL_NEXT_INDEX_DENY) ?
			      (is_output ? IP4_ERROR_OUTACL_TABLE_MISS : IP4_ERROR_INACL_TABLE_MISS) : IP4_ERROR_NONE;
			  else
			    error0 = (next0 == ACL_NEXT_INDEX_DENY) ?
			      (is_output ? IP6_ERROR_OUTACL_TABLE_MISS : IP6_ERROR_INACL_TABLE_MISS) : IP6_ERROR_NONE;
			  b0->error = error_node->errors[error0];
			  break;
			}
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       is_output ? IP_OUTACL_ERROR_CHAIN_HIT : IP_INACL_ERROR_CHAIN_HIT, chain_hits);
