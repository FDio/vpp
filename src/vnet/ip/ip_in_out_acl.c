		      ...
		      int do_trace)
{
  ...
  while (n_left >= 2)
	...
	[sw_if_index[1]];
      ...
      t[0] = pool_elt_at_index (vcm->tables, table_index[0]);
      t[1] = pool_elt_at_index (vcm->tables, table_index[1]);

      if (t[0]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
	h[0] =
	  (void *) vlib_buffer_get_current (b[0]) + t[0]->current_data_offset;
      else
	h[0] = b[0]->data;

      if (t[1]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
	h[1] =
	  (void *) vlib_buffer_get_current (b[1]) + t[1]->current_data_offset;
      else
	h[1] = b[1]->data;
      ...
      if (is_output)
	{
      ...
      vnet_buffer (b[0])->l2_classify.hash = hash[0];
      vnet_buffer (b[1])->l2_classify.hash = hash[1];

      vnet_buffer (b[0])->l2_classify.table_index = table_index[0];
      vnet_buffer (b[1])->l2_classify.table_index = table_index[1];
