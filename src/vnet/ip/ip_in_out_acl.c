...
format_ip_in_out_acl_trace (u8 * s, u32 is_output, va_list * args)
{
  ...
  s = format (
    ...
    "action %s, fib_index %d",
    is_output ? "OUTACL" : "INACL", t->sw_if_index, t->next_index, t->table_index, t->offset,
    t->action < ARRAY_LEN (action_strings) ? action_strings[t->action] : "unknown", t->fib_index);
