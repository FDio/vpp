...
format_ip_in_out_acl_trace (u8 *s, u32 is_output, va_list *args)
{
  ...
  switch (t->action)
    ...
    case CLASSIFY_ACTION_NONE:
      s = format (s, "none, metadata %d", t->metadata);
      break;
    case CLASSIFY_ACTION_SET_IP4_FIB_INDEX:
      s = format (s, "set-ip4-fib-index");
      break;
    case CLASSIFY_ACTION_SET_IP6_FIB_INDEX:
      s = format (s, "set-ip6-fib-index");
      break;
    case CLASSIFY_ACTION_SET_METADATA:
      s = format (s, "set-metadata, metadata %d, fib_index %d", t->metadata, t->fib_index);
      break;
    default:
      s = format (s, "unknown");
    }
...
ip_in_out_acl_inline_trace (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame,
	  ...
	  if (e[0])
	      ...
	      while (1)
		{
		  ...
		  if (PREDICT_TRUE (table_index[0] != ~0))
		    ...
		    {
		      _next[0] =
			(t[0]->miss_next_index < n_next_nodes) ? t[0]->miss_next_index : _next[0];
	  ...
	  if (e[1])
	      ...
	      while (1)
		{
		  ...
		  if (PREDICT_TRUE (table_index[1] != ~0))
		    ...
		    {
		      _next[1] =
			(t[1]->miss_next_index < n_next_nodes) ? t[1]->miss_next_index : _next[1];
...
ip_in_out_acl_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame,
		      const in_out_acl_table_id_t tid, u32 *fib_index_by_sw_if_index,
		      const vlib_node_registration_t *parent_error_node, const u32 error_none_index,
		      const u32 error_deny_index, const u32 error_miss_index,
		      const vlib_rx_or_tx_t way, const int is_output)
{
...
#define ip_in_out_acl_inline_trace__(do_trace)                                                     \
  ...
  vlib_node_increment_counter (vm, node->node_index,
			       is_output ? IP_OUTACL_ERROR_HIT : IP_INACL_ERROR_HIT, hits);
  vlib_node_increment_counter (vm, node->node_index,
			       is_output ? IP_OUTACL_ERROR_CHAIN_HIT : IP_INACL_ERROR_CHAIN_HIT,
			       chain_hits);
...
{
  return ip_in_out_acl_inline (vm, node, frame, IN_OUT_ACL_TABLE_IP4,
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return ip_in_out_acl_inline (vm, node, frame, IN_OUT_ACL_TABLE_IP4_PUNT,
			       ip4_main.fib_index_by_sw_if_index, &ip4_input_node, IP4_ERROR_NONE,
			       IP4_ERROR_INACL_SESSION_DENY, IP4_ERROR_INACL_TABLE_MISS,
			       ~0 /* way */, 0 /* is_output */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return ip_in_out_acl_inline (vm, node, frame, IN_OUT_ACL_TABLE_IP4, NULL, &ip4_input_node,
			       IP4_ERROR_NONE, IP4_ERROR_INACL_SESSION_DENY,
			       IP4_ERROR_INACL_TABLE_MISS, VLIB_TX, 1 /* is_output */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return ip_in_out_acl_inline (vm, node, frame, IN_OUT_ACL_TABLE_IP6,
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return ip_in_out_acl_inline (vm, node, frame, IN_OUT_ACL_TABLE_IP6, NULL, &ip6_input_node,
			       IP6_ERROR_NONE, IP6_ERROR_INACL_SESSION_DENY,
			       IP6_ERROR_INACL_TABLE_MISS, VLIB_TX, 1 /* is_output */);
}
