...
punt_socket_register (bool is_ip4, u8 protocol, u16 port, char *client_pathname)
{
  ...
  memset(&c, 0, sizeof(c));
  memcpy (c.caddr.sun_path, client_pathname,
	  sizeof (c.caddr.sun_path));
...
static uword
punt_socket_rx_fd (vlib_main_t * vm,
		   vlib_node_runtime_t * node,
		   u32 fd)
{
  ...
  clib_warning("Scheduled to read on %d\n", fd);
...
clib_error_t *
vnet_punt_socket_del (vlib_main_t * vm, bool is_ip4,
