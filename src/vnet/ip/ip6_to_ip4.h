...
static_always_inline int
icmp6_to_icmp_header (icmp46_header_t * icmp, u32 icmp_len,
	       ...
	       ip6_to_ip4_set_fn_t inner_fn, void *inner_ctx)
{
  ...
  if (icmp6_to_icmp_header (icmp, ip6_pay_len, &inner_ip6))
    return -1;
...
ip6_to_ip4_tcp_udp (vlib_buffer_t * p, ip6_to_ip4_set_fn_t fn, void *ctx)
{
  ...
  if (l4_protocol == IP_PROTOCOL_TCP)
    {
      checksum = (u16 *) u8_ptr_add (ip6, l4_offset + 16);
    ...
    {
      checksum = (u16 *) u8_ptr_add (ip6, l4_offset + 6);
      *checksum = 0;
    }
