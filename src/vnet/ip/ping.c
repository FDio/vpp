...
static_always_inline uword
ip46_get_cli_process_id_for_buf (vlib_main_t * vm, u32 bi0, int is_ip6)
{
  ...
  cli_process_id =
    get_cli_process_id_by_icmp_id_mt (vm, clib_net_to_host_u16 (net_icmp_id));
			    ...
			    int is_ip6)
{
  vlib_buffer_t *b0 = vlib_get_buffer (vm, bi0);
  /*
   * Fun fact: vlib_time_now() tells slightly different
   * time depending on the vlib_main it works
   * with. As a result, on multicore this
   * means the packet might have negative
   * RTT if we just ask for vlib_time_now(vm)
   * here. So, take the time from the
   * main thread.
   *
   * FIXME: is this thread-safe ?
   */
  f64 nowts = vlib_time_now (vlib_mains[0]);

  /* Pass the timestamp to the cli_process thanks to the vnet_buffer unused metadata field */

  /* FIXME-MAYBE: Camping on unused data... just ensure statically that there is enough space */
  ...
  clib_memcpy (vnet_buffer (b0)->unused, &nowts, sizeof (nowts));
...
ip46_echo_reply_maybe_trace_buffer (vlib_main_t * vm,
				    ...
				    uword cli_process_id, vlib_buffer_t * b0,
				    int is_ip6)
{
				    ...
				    int is_ip6)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  uword cli_process_id =
	    ip46_get_cli_process_id_for_buf (vm, bi0, is_ip6);

	  if (do_trace)
	    ip46_echo_reply_maybe_trace_buffer (vm, node, cli_process_id, b0,
						is_ip6);
 ...
 *
 * Deliberately "static" rather than "static inline"
...
// FIXME max length upper bound calculation should be more precise here
		       ...
		       vlib_buffer_t * b0, int is_ip6)
{
  ...
  if (is_ip6)
	...
	~ip_csum_fold (ip_incremental_checksum
		       (0, icmp46, total_length - l4_offset));
    }
  b0->current_length = total_length;
}
...
// FIXME: refactor these two below into one AF-agnostic one
static void
		       ...
		       u32 ping_burst, u32 verbose)
{
  ...
  for (i = 1; i <= ping_repeat; i++)
      ...
      if (pa4)
	{
      ...
      // FIXME both of the send_ip*ping return number of packets now. Fix that.
      if (SEND_PING_OK == res)
	{
		...
		for (i = 0; i < vec_len (event_data); i++)
		  {
		    ...
		    n_replies++;
		    // FIXME: the buffer index might have arrived from a different thread... is it ok ?
		    // FIXME: what happens if there is a keypress (so we exit this loop) while there are buffers in-flight to here ? smells like a possible leak...
		    if (0 != bi0)
		      vlib_buffer_free (vm, &bi0, 1);
		  }
	      }
