			      ...
			      u8 event_type, vlib_buffer_t * b0)
{
  ...
  if (vlib_buffer_alloc (vlib_mains[pr->cli_thread_index], &bi0_copy, 1) == 1)
    {
      void *dst = vlib_buffer_get_current (vlib_get_buffer (vlib_mains[pr->cli_thread_index], bi0_copy));
  ...
  clib_memcpy(vnet_buffer(vlib_get_buffer(vlib_mains[pr->cli_thread_index], bi0_copy))->unused,
      &nowts,
      sizeof(nowts));
  vlib_process_signal_event (vlib_mains[pr->cli_thread_index], pr->cli_process_id, event_type, bi0_copy);
  return 1;
}
