...
ip6_sas_commonlen (ip6_address_t *a1, ip6_address_t *a2)
{
    ...
    if (fa == 0) {
        ...
        if (la == 0)
            return 128;
        return 64 + count_leading_zeros(la);
    } else {
        return count_leading_zeros(fa);
    }
}
...
{
    u32 a = clib_net_to_host_u32(a1->as_u32) ^ clib_net_to_host_u32(a2->as_u32);
    if (a == 0)
        return 32;
 ...
 */
bool
ip6_sas_by_sw_if_index (u32 sw_if_index, ip6_address_t *dst,
			ip6_address_t *src)
{
...
bool
ip4_sas_by_sw_if_index (u32 sw_if_index, ip4_address_t *dst,
			ip4_address_t *src)
{
  ip_interface_address_t *ia = 0;
  ...
  if (bestsrc)
    {
      ...
      clib_warning ("Chose source %U for destination %U at %u bestlen: %u",
		    format_ip4_address, src, format_ip4_address, dst,
		    sw_if_index, bestlen);
      return true;
    }
  return false;
 ...
 * outbound interface NOTE: What to do if multiple output interfaces? ECMP
 * destination.
 *
 */
bool
