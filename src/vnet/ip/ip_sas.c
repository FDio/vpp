 ...
 * That means that an interface with just an IPv6 link-local address must also be
 * configured with an unnumbered configuration pointing to a numbered interface.
 */
...
/*
 * walk all addresses on an interface and pick the source address with the longest common prefix with destination.
 */
bool
