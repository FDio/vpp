...
ip_is_local (u32 fib_index, ip46_address_t * ip46_address, u8 is_ip4)
{
  ...
  _clib_memcpy (&prefix.fp_addr, ip46_address, sizeof (ip46_address_t));
...
ip_copy (ip46_address_t * dst, ip46_address_t * src, u8 is_ip4)
{
  if (is_ip4)
    ...
    _clib_memcpy (&dst->ip6, &src->ip6, sizeof (ip6_address_t));
}
...
ip_set (ip46_address_t * dst, void *src, u8 is_ip4)
{
  if (is_ip4)
    ...
    _clib_memcpy (&dst->ip6, (ip6_address_t *) src, sizeof (ip6_address_t));
}
