...
static lacp_fsm_state_t lacp_mux_state_detached[] = {
  {LACP_NOACTION, LACP_MUX_STATE_DETACHED},	// event 0 BEGIN
...
static lacp_fsm_state_t lacp_mux_state_waiting[] = {
  {LACP_ACTION_DETACHED, LACP_MUX_STATE_DETACHED},	// event 0 BEGIN
  {LACP_NOACTION, LACP_MUX_STATE_WAITING},	// event 1 SELECTED
  ...
  if (p)
    {
      ...
      {
	p1 = *vec_elt_at_index (bif->active_slaves, i);
	if (p1 == sif->sw_if_index)
	  {
