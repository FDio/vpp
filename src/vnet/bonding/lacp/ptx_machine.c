...
static lacp_fsm_state_t lacp_ptx_state_fast_periodic[] = {
  ...
  {LACP_ACTION_NO_PERIODIC, LACP_PTX_STATE_NO_PERIODIC},	// event 1 NO_PERIODIC
  {LACP_ACTION_FAST_PERIODIC, LACP_PTX_STATE_FAST_PERIODIC},	// event 2 START_PERIODIC
  {LACP_ACTION_SLOW_PERIODIC, LACP_PTX_STATE_SLOW_PERIODIC},	// event 3 LONG_TIMEOUT
  {LACP_ACTION_TIMER_EXPIRED, LACP_PTX_STATE_PERIODIC_TX},	// event 4 TIMER_EXPIRED
...
static lacp_fsm_state_t lacp_ptx_state_periodic_tx[] = {
  ...
  {LACP_ACTION_NO_PERIODIC, LACP_PTX_STATE_NO_PERIODIC},	// event 1 NO_PERIODIC
  {LACP_ACTION_TIMER_EXPIRED, LACP_PTX_STATE_PERIODIC_TX},	// event 2 START_PERIODIC
  {LACP_ACTION_FAST_PERIODIC, LACP_PTX_STATE_SLOW_PERIODIC},	// event 3 LONG_TIMEOUT
  {LACP_ACTION_TIMER_EXPIRED, LACP_PTX_STATE_PERIODIC_TX},	// event 4 TIMER_EXPIRED
  {LACP_ACTION_SLOW_PERIODIC, LACP_PTX_STATE_FAST_PERIODIC},	// event 5 SHORT_TIMEOUT
};
...
lacp_ptx_action_no_periodic (void *p1, void *p2)
{
  ...
  if (n->lacp_enabled || n->port_enabled ||
    ...
    lacp_machine_dispatch (&lacp_ptx_machine, vm, n,
			   LACP_PTX_EVENT_START_PERIODIC, &n->ptx_state);
...
lacp_ptx_action_slow_periodic (void *p1, void *p2)
{
  ...
  n->periodic_timer = vlib_time_now (vm) + LACP_SLOW_PERIODIC_TIMER;
...
lacp_ptx_action_fast_periodic (void *p1, void *p2)
{
  ...
  n->periodic_timer = vlib_time_now (vm) + LACP_FAST_PERIODIC_TIMER;
...
lacp_ptx_action_timer_expired (void *p1, void *p2)
{
  ...
  n->ntt = 1;
  if (n->partner.state & LACP_STATE_LACP_TIMEOUT)
    lacp_machine_dispatch (&lacp_ptx_machine, vm, n,
			   LACP_PTX_EVENT_SHORT_TIMEOUT, &n->ptx_state);
  else
    lacp_machine_dispatch (&lacp_ptx_machine, vm, n,
			   LACP_PTX_EVENT_LONG_TIMEOUT, &n->ptx_state);
...
void
lacp_init_ptx_machine (vlib_main_t * vm, lacp_neighbor_t * n)
{
  lacp_machine_dispatch (&lacp_ptx_machine, vm, n, LACP_PTX_EVENT_BEGIN,
			 &n->ptx_state);
}
