...
#define foreach_lacp_rx_event          \
  _(0, BEGIN, "begin")                 \
  _(1, PORT_MOVED, "port moved")       \
  _(2, LACP_ENABLED, "lacp enabled")   \
  _(3, LACP_DISABLED, "lacp disabled") \
  _(4, PDU_RECEIVED, "pdu received")   \
  _(5, TIMER_EXPIRED, "timer expired")
...
#define foreach_lacp_rx_sm_state       \
  _(0, UNKNOWN, "unknown")             \
  _(1, INITIALIZE, "initialize")       \
  _(2, PORT_DISABLED, "port disabled") \
  _(3, EXPIRED, "expired")             \
...
void lacp_rx_debug_func (int event, int state, lacp_fsm_state_t * transition);
