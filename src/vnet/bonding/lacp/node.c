	      ...
	      vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      if (PREDICT_FALSE (n_trace > 0))
	{
	  ...
	  len = (b0->current_length < sizeof (t0->lacpdu))
	    ? b0->current_length : sizeof (t0->lacpdu);
	  t0->len = len;
	  clib_memcpy (&t0->lacpdu, vlib_buffer_get_current (b0), len);
	}
...
lacp_process (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  bm->lacp_process_node_index = lacp_process_node.index;

  ethernet_register_input_type (vm, ETHERNET_TYPE_SLOW_PROTOCOLS /* LACP */ ,
				lacp_input_node.index);

  poll_time_remaining = LACP_FAST_PERIODIC_TIMER;
  while (1)
    {
