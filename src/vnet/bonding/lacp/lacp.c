...
lacp_periodic (vlib_main_t * vm)
{
  ...
  ({
    ...
    if ((n->periodic_timer != 0.0) &&
        (now >= n->periodic_timer + LACP_FAST_PERIODIC_TIMER))
      {
	n->periodic_timer = 0.0;
        lacp_machine_dispatch (&lacp_ptx_machine, vm, n,
			       LACP_PTX_EVENT_TIMER_EXPIRED, &n->ptx_state);
      }
    if ((n->current_while_timer != 0.0) &&
	(now >= n->current_while_timer + (f64) n->ttl_in_seconds))
      {
...
void
lacp_init_neighbor (lacp_neighbor_t * n, u8 * hw_address, u16 port_number,
		    u32 group)
{
  n->wait_while_timer = 0.0;
  n->current_while_timer = 0.0;
  ...
  clib_memcpy (n->actor.system, hw_address, 6);
