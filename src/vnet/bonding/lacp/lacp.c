 ...
 */
static void
lacp_send_ethernet_lacp_pdu (bond_main_t * bm, slave_if_t * sif, int count)
{
  u32 *to_next;
  ...
  for (i = 0; i < count; i++)
	...
	(vm, &bm->packet_templates[sif->packet_template_index], &bi0);
	...
	break;
      ...
      sif->periodic_timer = vlib_time_now (vm);
    }
}
 ...
 */
static int
lacp_pick_packet_template (bond_main_t * bm, slave_if_t * sif)
{
...
void
lacp_send_lacp_pdu (bond_main_t * bm, slave_if_t * sif, int count)
{
  if (sif->packet_template_index == (u8) ~ 0)
    {
      /* If we don't know how to talk to this peer, don't try again */
      if (lacp_pick_packet_template (bm, sif))
	{
	  sif->periodic_timer = 1e70;
	  return;
	}
    }
  ...
  switch (sif->packet_template_index)
    ...
    case LACP_PACKET_TEMPLATE_ETHERNET:
      lacp_send_ethernet_lacp_pdu (bm, sif, count);
      break;

    default:
      ASSERT (0);
    }
  sif->periodic_timer = vlib_time_now (bm->vlib_main);
}
...
lacp_periodic (vlib_main_t * vm)
{
  ...
  ({
    ...
    if ((sif->current_while_timer != 0.0) && (now >= sif->current_while_timer))
      {
        lacp_machine_dispatch (&lacp_rx_machine, vm, sif,
			       LACP_RX_EVENT_TIMER_EXPIRED, &sif->rx_state);
      }

    if ((sif->periodic_timer != 0.0) && (now >= sif->periodic_timer))
      {
        lacp_machine_dispatch (&lacp_ptx_machine, vm, sif,
			       LACP_PTX_EVENT_TIMER_EXPIRED, &sif->ptx_state);
      }
    if ((sif->wait_while_timer != 0.0) && (now >= sif->wait_while_timer))
      {
	sif->ready_n = 1;
        sif->wait_while_timer = 0.0;
...
lacp_periodic_init (vlib_main_t * vm)
{
  ...
  vlib_packet_template_init
    (vm, &bm->marker_packet_templates[MARKER_PACKET_TEMPLATE_ETHERNET],
     /* data */ &m,
     sizeof (m),
     /* alloc chunk size */ 8,
     "marker-ethernet");
...
{
  sif->wait_while_timer = 0.0;
  sif->current_while_timer = 0.0;
  sif->actor_churn_timer = 0.0;
  sif->partner_churn_timer = 0.0;
  ...
  if (p)
    {
      ...
      if ((sif->port_enabled == 0) && sif->lacp_enabled)
	{
...
lacp_hw_interface_up_down (vnet_main_t * vnm, u32 hw_if_index, u32 flags)
{
  ...
  if (p)
    {
      sif = pool_elt_at_index (bm->neighbors, p[0]);
      if (!(flags & VNET_HW_INTERFACE_FLAG_LINK_UP) && sif->lacp_enabled)
	{
	  lacp_init_state_machines (vm, sif);
	  lacp_init_neighbor (sif, sif->actor_admin.system,
			      ntohs (sif->actor_admin.port_number),
