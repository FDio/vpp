...
lacp_send_lacp_pdu (vlib_main_t * vm, slave_if_t * sif)
{
  ...
  lacp_start_periodic_timer (bm->vlib_main, sif, sif->is_long_timeout ?
...
lacp_periodic (vlib_main_t * vm)
{
  ...
  ({
	...
	lacp_timer_is_expired (bm->vlib_main, sif->current_while_timer))
      {
	...
	lacp_timer_is_expired (bm->vlib_main, sif->periodic_timer))
      {
	...
	lacp_timer_is_expired (bm->vlib_main, sif->wait_while_timer))
      {
	sif->ready_n = 1;
