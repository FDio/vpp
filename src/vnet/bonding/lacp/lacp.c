 ...
 */
static void
lacp_send_ethernet_lacp_pdu (bond_main_t * bm, slave_if_t * sif)
{
  u32 *to_next;
...
void
lacp_send_lacp_pdu (slave_if_t * sif)
{
  ...
  switch (sif->packet_template_index)
    ...
    case LACP_PACKET_TEMPLATE_ETHERNET:
      lacp_send_ethernet_lacp_pdu (bm, sif);
      break;
  ...
  lacp_start_periodic_timer (bm->vlib_main, sif, LACP_FAST_PERIODIC_TIMER);
}
