...
lacp_fill_pdu (bond_main_t * bm, vnet_hw_interface_t * hw,
	       lacp_pdu_t * lacpdu, slave_if_t * sif)
{
  /* Actor TLV */
...
lacp_send_ethernet_lacp_pdu (bond_main_t * bm, slave_if_t * sif, int count)
{
  ...
  for (i = 0; i < count; i++)
	...
	break;
      ...
      lacp_fill_pdu (bm, hw, &h0->lacp, sif);
...
lacp_periodic (vlib_main_t * vm)
{
  ...
  ({
    ...
    sw = vnet_get_sw_interface (bm->vnet_main, sif->sw_if_index);
	...
	(now >= sif->current_while_timer + (f64) sif->ttl_in_seconds))
      {
	...
	(now >= sif->wait_while_timer + LACP_AGGREGATE_WAIT_TIME))
      {
        sif->ready = 1;
