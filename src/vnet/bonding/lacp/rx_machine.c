...
static lacp_fsm_state_t lacp_rx_state_initialize[] = {
  ...
  {LACP_ACTION_INITIALIZE, LACP_RX_STATE_PORT_DISABLED},	// event 1 PORT_MOVED
  ...
  {LACP_NOACTION, LACP_RX_STATE_INITIALIZE},	// event 5 TIMER_EXPIRED
};
...
static lacp_fsm_state_t lacp_rx_state_port_disabled[] = {
  ...
  {LACP_ACTION_EXPIRED, LACP_RX_STATE_EXPIRED},	// event 2 LACP_ENABLED
...
static lacp_fsm_state_t lacp_rx_state_expired[] = {
  ...
  {LACP_NOACTION, LACP_RX_STATE_EXPIRED},	// event 1 PORT_MOVED
  {LACP_NOACTION, LACP_RX_STATE_EXPIRED},	// event 2 LACP_ENABLED
  {LACP_NOACTION, LACP_RX_STATE_EXPIRED},	// event 3 LACP_DISABLED
...
static void
lacp_set_port_moved (vlib_main_t * vm, slave_if_t * sif)
{
  sif->port_moved = 0;
...
lacp_rx_action_expired (void *p1, void *p2)
{
  ...
  if ((sif->current_while_timer != 0.0) &&
    ...
    timer_expired = 0;
  sif->current_while_timer = now + LACP_SHORT_TIMOUT_TIME;
  sif->actor.state |= LACP_STATE_EXPIRED;
  if (timer_expired)
    lacp_machine_dispatch (&lacp_rx_machine, vm, sif,
