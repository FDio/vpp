 ...
 *  LACP State = UNKNOWN
 */
static lacp_fsm_state_t lacp_tx_state_begin[] = {
  {LACP_ACTION_TRANSMIT, LACP_TX_STATE_TRANSMIT},	// event 0 BEGIN
  {LACP_NOACTION, LACP_TX_STATE_UNKNOWN},	// event 1 TRANSMIT
};
...
static lacp_fsm_state_t lacp_tx_state_transmit[] = {
  {LACP_NOACTION, LACP_TX_STATE_TRANSMIT},	// event 0 BEGIN
  {LACP_ACTION_TRANSMIT, LACP_TX_STATE_TRANSMIT},	// event 1 TRANSMIT
};
...
{
  static lacp_state_struct lacp_tx_sm_state_array[] = {
#define _(b, s, n) {.bit = b, .str = #s, },
...
void
lacp_tx_debug_func (int event, int state, lacp_fsm_state_t * transition)
{
  clib_warning ("TX: event %U, old state %U, new state %U", format_tx_event,
		event, format_tx_sm_state, state, format_tx_sm_state,
		transition->next_state);
}

void
lacp_init_tx_machine (vlib_main_t * vm, lacp_neighbor_t * n)
{
  lacp_machine_dispatch (&lacp_tx_machine, vm, n, LACP_TX_EVENT_BEGIN,
			 &n->tx_state);
