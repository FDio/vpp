...
#define foreach_lacp_ptx_event          \
  _(0, BEGIN, "begin")                  \
  _(1, NO_PERIODIC, "no periodic")      \
  _(2, START_PERIODIC, "start periodic") \
  _(3, LONG_TIMEOUT, "long tiemout")    \
  _(4, TIMER_EXPIRED, "timer expired")  \
  _(5, SHORT_TIMEOUT, "short timeout")
...
#define foreach_lacp_ptx_sm_state       \
  ...
  _(3, FAST_PERIODIC, "fast periodic")  \
  _(4, PERIODIC_TX, "periodic transmission")
...
void lacp_ptx_debug_func (int event, int state,
			  lacp_fsm_state_t * transition);
u8 *format_ptx_sm_state (u8 * s, va_list * args);
