	       ...
	       vlib_frame_t * frame)
{
  ...
  if (PREDICT_FALSE (vec_len (bif->slaves) < 1))
    {
      vlib_error_count (vm, node->node_index, BOND_INPUT_ERROR_NO_SLAVE,
			frame->n_vectors);
  ...
  if (PREDICT_FALSE (bif->admin_up == 0))
    {
      vlib_error_count (vm, node->node_index, BOND_INPUT_ERROR_IF_DOWN,
			frame->n_vectors);
      return frame->n_vectors;
    }
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  // Let some layer2 packets pass through. TODO what else beside
	  // LACP and CDP?
	  if (eth->type != htons (ETHERNET_TYPE_SLOW_PROTOCOLS) &&
	      eth->type != htons (ETHERNET_TYPE_CDP))
	    {
