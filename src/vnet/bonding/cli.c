...
bond_disable_collecting_distributing (vlib_main_t * vm, slave_if_t * sif)
{
  ...
  if (bif->mode == BOND_MODE_LACP)
    /* XXX: Remove state counter?? */
    stat_segment_set_state_counter(bm->stats[bif->sw_if_index][sif->sw_if_index], sif->actor.state);
}
...
bond_enslave (vlib_main_t * vm, bond_enslave_args_t * args)
{
  ...
  if (bif->mode == BOND_MODE_LACP)
    {
      u8 *name = format(0, "/if/lacp/%u/%u/state", bif->sw_if_index, sif->sw_if_index);
      vec_validate(bm->stats, bif->sw_if_index);
      vec_validate(bm->stats[bif->sw_if_index], sif->sw_if_index);

      if (stat_segment_register_state_counter(name, &bm->stats[bif->sw_if_index][sif->sw_if_index]) != 0) {
	args->rv = VNET_API_ERROR_INVALID_INTERFACE;
	args->error =
	  clib_error_return (0, "bond interface statistics failure");
	vec_free(name);
	return;
      }
      vec_free(name);
