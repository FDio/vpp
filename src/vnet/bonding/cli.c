			...
			vlib_cli_command_t * cmd)
{
  ...
  if (group_is_set == 0)
    return clib_error_return (0, "Missing bonding-group number");
...
bond_create_slave (vlib_main_t * vm, bond_create_slave_args_t * args)
{
  ...
  if (!bif)
    {
      args->error =
	clib_error_return (0, "bonding-group interface not found");
      return;
    }
  ...
  vec_add1 (bif->slaves, n);
  lacp_init_neighbor (n, bif->sw_if_index, vec_len (bif->slaves), args->group);	//TODO for del
			      ...
			      vlib_cli_command_t * cmd)
{
  ...
  if (args.error)
    return args.error;
  if (group_is_set == 0)
    return clib_error_return (0, "Missing bundle number");
  if (args.slave == ~0)
...
VLIB_CLI_COMMAND (enslave_interface_command, static) = {
  .path = "enslave",
  .short_help = "enslave interface <interface> bundle <id> [passive] [slow-rate]",
