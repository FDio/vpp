...
VLIB_CLI_COMMAND (bond_create_command, static) = {
  .path = "create bundle",
  .short_help = "create bundle <id> [hw-addr <mac-address>] mode {round-robin | active-backup | broadcast | {lacp | xor} [load-balance { src-dst-mac | src-mac | dst-mac | src-dst-ip | src-ip | dst-ip }]}",
