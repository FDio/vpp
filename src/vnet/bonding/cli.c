...
static void
bond_delete_neighbor (bond_if_t * bif, slave_if_t * sif)
{
  ...
  {
    uword p = *vec_elt_at_index (bif->active_slaves, i);
    if (p == sif->sw_if_index)
      {
			...
			vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	args.mode = BOND_MODE_LACP;
      else if (unformat (line_input, "load-balance %U",
...
VLIB_CLI_COMMAND (bond_create_command, static) = {
  .path = "create bundle",
  .short_help = "create bundle <id> mode lacp [hw-addr <mac-address>] [load-balance { src-dst-mac | src-mac | dst-mac | src-dst-ip | src-ip | dst-ip }]",
