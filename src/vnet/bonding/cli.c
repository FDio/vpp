...
static void
lacp_delete_neighbor (bond_if_t * bif, lacp_neighbor_t * n)
{
  bond_main_t *bm = &bond_main;
  int i;
  lacp_neighbor_t *target;
  ...
  {
    target = *vec_elt_at_index (bif->slaves, i);
    if (target->sw_if_index == n->sw_if_index)
      {
...
bond_create_slave (vlib_main_t * vm, bond_create_slave_args_t * args)
{
  ...
  bif->port_number_bitmap = clib_bitmap_set (bif->port_number_bitmap,
					     port_number, 1);
  lacp_init_neighbor (n, bif->sw_if_index, port_number, args->group);
