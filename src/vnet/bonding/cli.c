...
bond_create_if (vlib_main_t * vm, bond_create_if_args_t * args)
{
  ...
  if (hash_get (bm->dev_instance_by_group_id, args->group))
    {
      args->error = clib_error_return (0, "bond id already existed");
			...
			vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "bond %d", &args.group))
	group_is_set = 1;
      else
  ...
  if (group_is_set == 0)
    return clib_error_return (0, "Missing bond id");
...
VLIB_CLI_COMMAND (bond_create_command, static) = {
  .path = "create",
  .short_help = "create bond <id> [hw-addr <mac-address>] mode {round-robin | active-backup | broadcast | {lacp | xor} [load-balance { l2 | l23 | l34 }]}",
...
bond_enslave (vlib_main_t * vm, bond_enslave_args_t * args)
{
  ...
  hw = vnet_get_sup_hw_interface (vnm, sw->sw_if_index);
...
VLIB_CLI_COMMAND (enslave_interface_command, static) = {
  .path = "enslave",
  .short_help = "enslave interface <interface> bond <id> [passive] [long-timeout]",
...
show_bond (vlib_main_t * vm)
{
  ...
  vlib_cli_output (vm, "%-16s %-12s %-5s %-12s %-13s %-14s %-14s",
		   "interface name", "sw_if_index", "id", "mode",
		   "load balance", "active slaves", "slaves");

  /* *INDENT-OFF* */
  pool_foreach (bif, bm->interfaces,
  ({
    vlib_cli_output (vm, "%-16U %-12d %-5d %-12U %-13U %-14u %-14u",
		     format_bond_interface_name, bif->dev_instance,
		     bif->sw_if_index, bif->group, format_bond_mode, bif->mode,
