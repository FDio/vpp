...
static void
lacp_send_sw_interface_details (vpe_api_main_t * am,
				vl_api_registration_t * reg,
				...
				u32 context)
{
  ...
  mp = vl_msg_api_alloc (sizeof (*mp));
  memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id = htons (VL_API_SW_INTERFACE_LACP_DETAILS);
  mp->sw_if_index = htonl (lacp_if->sw_if_index);
  mp->actor_system_priority = htons (lacp_if->actor_system_priority);
  mp->actor_key = htons (lacp_if->actor_key);
  mp->actor_port_priority = htons (lacp_if->actor_port_priority);
  mp->actor_port_number = htons (lacp_if->actor_port_number);
  mp->actor_state = lacp_if->actor_state;
  clib_memcpy (mp->actor_system, lacp_if->actor_system, 6);
  mp->partner_system_priority = htons (lacp_if->partner_system_priority);
  mp->partner_key = htons (lacp_if->partner_key);
  mp->partner_port_priority = htons (lacp_if->partner_port_priority);
  mp->partner_port_number = htons (lacp_if->partner_port_number);
...
vl_api_sw_interface_lacp_dump_t_handler (vl_api_sw_interface_lacp_dump_t * mp)
{
  ...
  {
    lacp_send_sw_interface_details (am, reg, lacp_if, mp->context);
  }
