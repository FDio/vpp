...
bond_get_frame_to_sw_interface (vnet_main_t * vnm, vlib_main_t * vm,
				u32 sw_if_index)
{
  vnet_hw_interface_t *hw = vnet_get_sup_hw_interface (vnm, sw_if_index);
  return vlib_get_frame_to_node (vm, hw->output_node_index);
}

// TODO fix the bug in vnet_put_frame_to_sw_interface instead. It always
// put the frame to main vm and crash under heavy traffic with worker threads
always_inline void
bond_put_frame_to_sw_interface (vnet_main_t * vnm, vlib_main_t * vm,
				u32 sw_if_index, vlib_frame_t * f)
{
  vnet_hw_interface_t *hw = vnet_get_sup_hw_interface (vnm, sw_if_index);
  return vlib_put_frame_to_node (vm, hw->output_node_index, f);
}
  ...
  for (i = 1; i < vec_len (bif->active_slaves); i++)
    {
      ...
      f = bond_get_frame_to_sw_interface (vnm, vm, sw_if_index);
      ...
      if (PREDICT_TRUE (c0 != 0))
	{
	  ...
	  f->n_vectors++;
	  bond_put_frame_to_sw_interface (vnm, vm, sw_if_index, f);
	}
    }
	    ...
	    vlib_frame_t * frame)
{
  ...
  if (PREDICT_TRUE (vec_len (bif->active_slaves) == 0))
    {
      bi0 = from[0];
  ...
  while (n_left_from > 0)
    {
      ...
      bi0 = from[0];
      b0 = vlib_get_buffer (vm, bi0);
      n_rx_bytes += b0->total_length_not_including_first_buffer;
	...
	(bond_load_balance_table[bif->lb]).load_balance (vm, node, bif, b0);
      ...
      f = bond_get_frame_to_sw_interface (vnm, vm, sif_if_index);
      to_next = vlib_frame_vector_args (f);
      to_next += f->n_vectors;

      to_next[0] = vlib_get_buffer_index (vm, b0);
      f->n_vectors++;
      bond_put_frame_to_sw_interface (vnm, vm, sif_if_index, f);
  ...
  vlib_increment_combined_counter
    (vnet_main.interface_main.combined_sw_if_counters
     ...
     bif->sw_if_index, frame->n_vectors, n_rx_bytes);
