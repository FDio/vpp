					  ...
					  vlib_frame_t * frame)
{
  ...
  if (bif->lb == BOND_LB_AB)
    {
      u32 sw_if_index = *vec_elt_at_index (bif->active_slaves, 0);
      bond_update_sw_if_index (ptd, bif, from, bufs, &sw_if_index, n_left,
			       /* single_sw_if_index */ 1);
      bond_tx_trace (vm, node, bufs, frame->n_vectors);
      goto done;
    }
  ...
  while (n_left > 4)
    {
      h[0] = *vec_elt_at_index (bif->active_slaves, h[0]);
      h[1] = *vec_elt_at_index (bif->active_slaves, h[1]);
      h[2] = *vec_elt_at_index (bif->active_slaves, h[2]);
      h[3] = *vec_elt_at_index (bif->active_slaves, h[3]);
      h += 4;
      n_left -= 4;
    }
  while (n_left)
  ...
  bond_update_sw_if_index (ptd, bif, from, bufs, hashes, n_left,
