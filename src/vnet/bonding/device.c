			     ...
			     uword slave_cnt)
{
  ...
  for (port = 1; port < slave_cnt; port++)
    {
...
bond_lb_l2 (vlib_main_t * vm, vlib_node_runtime_t * node,
		      bond_if_t * bif, vlib_buffer_t * b0, uword slave_cnt)
{
		       ...
		       bond_if_t * bif, vlib_buffer_t * b0, uword slave_cnt)
{
  ...
  if ((ethertype != htons (ETHERNET_TYPE_IP4)) &&
    ...
    return (bond_lb_l2 (vm, node, bif, b0, slave_cnt));
  ...
  if (ip_version == 0x4)
	...
	lb_hash_hash_2_tuples (clib_mem_unaligned (&ip4->address_pair, u64),
			       a);
      if (BOND_MODULO_SHORTCUT (slave_cnt))
	return (c & (slave_cnt - 1));
      else
	return c % slave_cnt;
    }
		       ...
		       bond_if_t * bif, vlib_buffer_t * b0, uword slave_cnt)
{
  ...
  if (ip_version == 0x4)
	...
	(ip4->protocol == IP_PROTOCOL_UDP);
      ...
      a = t1 ^ t2;
      return lb_hash_hash_2_tuples (clib_mem_unaligned (&ip4->address_pair, u64), a);
    }
  else if (ip_version == 0x6)
    {
      u64 a;
			       ...
			       uword slave_cnt)
{
  ...
  if (BOND_MODULO_SHORTCUT (slave_cnt))
    bif->lb_rr_last_index &= slave_cnt - 1;
  else
    bif->lb_rr_last_index %= slave_cnt;

  return bif->lb_rr_last_index;
}

static_always_inline u32
bond_lb_active_backup (vlib_main_t * vm,
				 vlib_node_runtime_t * node,
				 bond_if_t * bif, vlib_buffer_t * b0,
				 uword slave_cnt)
{
...
bond_tx_inline (vlib_main_t * vm, vlib_node_runtime_t * node,
		...
		u32 *h, u32 n_left, uword slave_cnt, u32 lb_alg)
{
  while (n_left >= 4)
    {
      ...
      if (n_left >= 8)
	{
      ...
      if (PREDICT_TRUE (slave_cnt > 1))
	{
	  if (lb_alg == BOND_LB_L2)
	    {
	      h[0] = bond_lb_l2 (vm, node, bif, b[0], slave_cnt);
	      h[1] = bond_lb_l2 (vm, node, bif, b[1], slave_cnt);
	      h[2] = bond_lb_l2 (vm, node, bif, b[2], slave_cnt);
	      h[3] = bond_lb_l2 (vm, node, bif, b[3], slave_cnt);
	    }
	  else if (lb_alg == BOND_LB_L34)
	    {
	      h[0] = bond_lb_l34 (vm, node, bif, b[0], slave_cnt);
	      h[1] = bond_lb_l34 (vm, node, bif, b[1], slave_cnt);
	      h[2] = bond_lb_l34 (vm, node, bif, b[2], slave_cnt);
	      h[3] = bond_lb_l34 (vm, node, bif, b[3], slave_cnt);
	    }
	  else if (lb_alg == BOND_LB_L23)
	    {
	      h[0] = bond_lb_l23 (vm, node, bif, b[0], slave_cnt);
	      h[1] = bond_lb_l23 (vm, node, bif, b[1], slave_cnt);
	      h[2] = bond_lb_l23 (vm, node, bif, b[2], slave_cnt);
	      h[3] = bond_lb_l23 (vm, node, bif, b[3], slave_cnt);
	    }
	  else if (lb_alg == BOND_LB_RR)
	    {
	      h[0] = bond_lb_round_robin (vm, node, bif, b[0], slave_cnt);
	      h[1] = bond_lb_round_robin (vm, node, bif, b[1], slave_cnt);
	      h[2] = bond_lb_round_robin (vm, node, bif, b[2], slave_cnt);
	      h[3] = bond_lb_round_robin (vm, node, bif, b[3], slave_cnt);
	    }
	  else if (lb_alg == BOND_LB_BC)
	    {
	      h[0] = bond_lb_broadcast (vm, node, bif, b[0], slave_cnt);
	      h[1] = bond_lb_broadcast (vm, node, bif, b[1], slave_cnt);
	      h[2] = bond_lb_broadcast (vm, node, bif, b[2], slave_cnt);
	      h[3] = bond_lb_broadcast (vm, node, bif, b[3], slave_cnt);
	    }
	  else if (lb_alg == BOND_LB_AB)
	    {
	      h[0] = bond_lb_active_backup (vm, node, bif, b[0], slave_cnt);
	      h[1] = bond_lb_active_backup (vm, node, bif, b[1], slave_cnt);
	      h[2] = bond_lb_active_backup (vm, node, bif, b[2], slave_cnt);
	      h[3] = bond_lb_active_backup (vm, node, bif, b[3], slave_cnt);
	    }
	  else
	    {
	      ASSERT (0);
					  ...
					  vlib_frame_t * frame)
{
  ...
  slave_cnt = vec_len (bif->active_slaves);
  if (PREDICT_FALSE (slave_cnt == 0))
    {
      vlib_buffer_free (vm, vlib_frame_args (frame), frame->n_vectors);
      vlib_increment_simple_counter (vnet_main.interface_main.sw_if_counters +
				     VNET_INTERFACE_COUNTER_DROP,
  ...
  if (bif->lb == BOND_LB_L2)
    bond_tx_inline (vm, node, bif, bufs, hashes, n_left, slave_cnt, BOND_LB_L2);
  else if (bif->lb == BOND_LB_L34)
    bond_tx_inline (vm, node, bif, bufs, hashes, n_left, slave_cnt, BOND_LB_L34);
  else if (bif->lb == BOND_LB_L23)
    bond_tx_inline (vm, node, bif, bufs, hashes, n_left, slave_cnt, BOND_LB_L23);
  else if (bif->lb == BOND_LB_RR)
    bond_tx_inline (vm, node, bif, bufs, hashes, n_left, slave_cnt, BOND_LB_RR);
  else if (bif->lb == BOND_LB_BC)
    bond_tx_inline (vm, node, bif, bufs, hashes, n_left, slave_cnt, BOND_LB_BC);
  else if (bif->lb == BOND_LB_AB)
    bond_tx_inline (vm, node, bif, bufs, hashes, n_left, slave_cnt, BOND_LB_AB);
  else
    ASSERT (0);
  ...
  if (BOND_MODULO_SHORTCUT (slave_cnt))
    {
      u32 mask = slave_cnt - 1;
#ifdef CLIB_HAVE_VEC256
      u32x8 mask8 = u32x8_splat (mask);
      while (n_left > 16)
	{
	  u32x8_store_unaligned (u32x8_load_unaligned (h) & mask8, h);
	  u32x8_store_unaligned (u32x8_load_unaligned (h + 8) & mask8, h + 8);
	  n_left -= 8;
	  h += 8;
	}
#endif
      ...
      while (n_left > 4)
	{
      ...
      ptd->per_port_queue[sif_if_index0].buffers[ptd->per_port_queue [sif_if_index0].n_buffers] = vlib_get_buffer_index (vm, b[0]);
      ptd->per_port_queue[sif_if_index0].n_buffers++;

      ptd->per_port_queue[sif_if_index1].buffers[ptd->per_port_queue [sif_if_index1].n_buffers] = vlib_get_buffer_index (vm, b[1]);
      ptd->per_port_queue[sif_if_index1].n_buffers++;

      ptd->per_port_queue[sif_if_index2].buffers[ptd->per_port_queue [sif_if_index2].n_buffers] = vlib_get_buffer_index (vm, b[2]);
      ptd->per_port_queue[sif_if_index2].n_buffers++;

      ptd->per_port_queue[sif_if_index3].buffers[ptd->per_port_queue [sif_if_index3].n_buffers] = vlib_get_buffer_index (vm, b[3]);
  ...
  for (p = 0; p < slave_cnt; p++)
    {
      vlib_frame_t *f;
      u32 *to_next;
      u32 sw_if_index = *vec_elt_at_index (bif->active_slaves, p);
