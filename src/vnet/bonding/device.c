  ...
  while (n_left >= 4)
      ...
      if (PREDICT_TRUE (n_slaves > 1))
	{
	  if (lb_alg == BOND_LB_L2)
	    ...
	    {
	      h[0] = bond_lb_active_backup (vm, node, bif, b[0], n_slaves);
	      h[1] = bond_lb_active_backup (vm, node, bif, b[1], n_slaves);
	      h[2] = bond_lb_active_backup (vm, node, bif, b[2], n_slaves);
	      h[3] = bond_lb_active_backup (vm, node, bif, b[3], n_slaves);
	    }
	  else
	    {
	      ASSERT (0);
					  ...
					  vlib_frame_t * frame)
{
  ...
  ptd->per_port_queue[sif_if_index0].
    buffers[ptd->per_port_queue[sif_if_index0].n_buffers] =
    vlib_get_buffer_index (vm, b[0]);
  ptd->per_port_queue[sif_if_index0].n_buffers++;

  ptd->per_port_queue[sif_if_index1].
    buffers[ptd->per_port_queue[sif_if_index1].n_buffers] =
    vlib_get_buffer_index (vm, b[1]);
  ptd->per_port_queue[sif_if_index1].n_buffers++;

  ptd->per_port_queue[sif_if_index2].
    buffers[ptd->per_port_queue[sif_if_index2].n_buffers] =
    vlib_get_buffer_index (vm, b[2]);
  ptd->per_port_queue[sif_if_index2].n_buffers++;

  ptd->per_port_queue[sif_if_index3].
    buffers[ptd->per_port_queue[sif_if_index3].n_buffers] =
