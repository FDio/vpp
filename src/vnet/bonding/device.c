  ...
  while (n_left_from > 0)
    {
      ...
      // sw_if_index points to the bond interface
      p = hash_get (bm->bond_by_sw_if_index, sw_if_index);
      if (PREDICT_FALSE (p == 0))
	{
	  sw = vnet_get_sw_interface_safe (vnm, sw_if_index);
	  if (PREDICT_TRUE (sw != 0))
	    {
	      // look deeper for the bond subinterface
	  ...
	  if (PREDICT_TRUE (vec_len (bif->active_slaves) >= 1))
	      ...
	      if (PREDICT_TRUE (bif->admin_up == 1))
		{
		  port = (bond_load_balance_table[bif->lb]).load_balance
		    (vm, node, bif, b0);
		  bif_if_index = *vec_elt_at_index (bif->active_slaves, port);
		  vnet_buffer (b0)->sw_if_index[VLIB_TX] = bif_if_index;

		  f = vnet_get_frame_to_sw_interface (vnm, bif_if_index);
		  to_next = vlib_frame_vector_args (f);
		  to_next += f->n_vectors;
		  to_next[0] = vlib_get_buffer_index (vm, b0);
		  f->n_vectors++;
		  vnet_put_frame_to_sw_interface (vnm, bif_if_index, f);
		...
		{
		  vlib_buffer_free (vm, vlib_frame_args (frame), 1);
      ...
      else
	{
	  ...
	  vlib_error_count (vm, node->node_index, BOND_TX_ERROR_NO_SLAVE, 1);
	}
