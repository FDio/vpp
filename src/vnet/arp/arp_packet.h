...
{
  const u8 ms_nlb_unicast_mac_prefix[] = { 0x02, 0xbf };

  return memcmp (mac->bytes, ms_nlb_unicast_mac_prefix,
		 sizeof (ms_nlb_unicast_mac_prefix)) == 0 &&
	 memcmp (mac->bytes + sizeof (ms_nlb_unicast_mac_prefix), addr->as_u8,
		 sizeof (addr->as_u8)) == 0;
}
