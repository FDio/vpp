					      ...
					      va_list *args)
{
  ...
  /* Named type. */
  if (unformat_user (input, unformat_vlib_number_by_name, am->opcode_by_name,
		     &i))
    {
      *result = i;
      return 1;
...
{
  int *result = va_arg (*args, int *);
  if (!unformat_user (input, unformat_ethernet_arp_opcode_host_byte_order,
		      result))
    return 0;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  error0 = (0 == arp0->ip4_over_ethernet[0].ip4.as_u32 ?
		      ETHERNET_ARP_ERROR_l3_dst_address_unset :
		      error0);
...
static uword
arp_disabled (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  u32 n_left_from, next_index, *from, *to_next, n_left_to_next;
...
arp_reply (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
...

	  {
		...
		src_fei =
		  ip4_fib_table_lookup (ip4_fib_get (fib_index0),
					&arp0->ip4_over_ethernet[0].ip4, mask);
		...
		FOR_EACH_SRC_ADDED (
		  src_fib_entry, src, source, ({
		    src_flags =
		      fib_entry_get_flags_for_source (src_fei, source);
			 ...
			 * The curious aspect of this setup is that ARP
			 * requests from the host will come from the VPP's own
			 * address. So don't drop immediately here, instead go
			 * see if this is a proxy ARP case.
			 */
			goto next_feature;
		      }
	  ...
	  is_vrrp_reply0 = ((arp0->opcode == clib_host_to_net_u16 (
					       ETHERNET_ARP_OPCODE_reply)) &&
			    (!memcmp (arp0->ip4_over_ethernet[0].mac.bytes,
...
unformat_pg_arp_header (unformat_input_t *input, va_list *args)
{
  ...
  if (!unformat (
	input, "%U: %U/%U -> %U/%U", unformat_pg_edit,
	unformat_ethernet_arp_opcode_net_byte_order, &p->opcode,
	unformat_pg_edit, unformat_mac_address_t, &p->ip4_over_ethernet[0].mac,
	unformat_pg_edit, unformat_ip4_address, &p->ip4_over_ethernet[0].ip4,
	unformat_pg_edit, unformat_mac_address_t, &p->ip4_over_ethernet[1].mac,
	unformat_pg_edit, unformat_ip4_address, &p->ip4_over_ethernet[1].ip4))
    {
 ...
 */
static void
arp_enable_disable_interface (ip4_main_t *im, uword opaque, u32 sw_if_index,
			      u32 is_enable)
{
  ethernet_arp_main_t *am = &ethernet_arp_main;
...
#define _(a, b)                                                               \
  vnet_pcap_drop_trace_filter_add_del (rt->errors[ETHERNET_ARP_ERROR_##a],    \
				       1 /* is_add */);
    foreach_ethernet_arp_error
#undef _
  }
