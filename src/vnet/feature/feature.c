			     ...
			     void *feature_config, u32 n_feature_config_bytes)
{
  ...
  int rv =  vnet_feature_enable_disable_with_index (arc_index, feature_index,
						 sw_if_index, enable_disable,
						 feature_config,
						 n_feature_config_bytes);
  if(VNET_API_ERROR_NO_SUCH_ENTRY == rv){
      clib_warning("no such entry: %s, %s", arc_name, node_name);
  }
  return rv;
}
