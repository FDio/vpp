...
#define VNET_FEATURE_ARC_INIT(x, ...)                                         \
  ...
  static vnet_feature_arc_registration_t __clib_unused                        \
    __clib_unused_vnet_feat_arc_##x
#define VNET_FEATURE_INIT(x, ...)                                             \
...
#define VNET_FEATURE_ARC_ORDER(x, ...)                                        \
  extern vnet_feature_constraint_registration_t __clib_unused                 \
    vnet_feature_constraint_##x;                                              \
...
void vnet_config_update_feature_count (vnet_feature_main_t *fm, u8 arc,
...
int vnet_feature_enable_disable_with_index (u8 arc_index, u32 feature_index,
					    u32 sw_if_index,
					    int enable_disable,
					    void *feature_config,
...
u32 vnet_feature_modify_end_node (u8 arc_index, u32 sw_if_index,
				  u32 node_index);
...
static_always_inline void *
vnet_feature_arc_start_w_cfg_index (u8 arc, u32 sw_if_index, u32 *next,
...
vnet_feature_next_with_data (u32 *next0, vlib_buffer_t *b0, u32 n_data_bytes)
{
  ...
  return vnet_get_config_data (&cm->config_main, &b0->current_config_index,
			       next0, n_data_bytes);
}
				    ...
				    vlib_buffer_t *b0)
{
  ...
  if (PREDICT_FALSE (clib_bitmap_get (
	...
	vec_elt (cm->config_index_by_sw_if_index, sw_if_index);
      vnet_get_config_data (&cm->config_main, &b0->current_config_index, next0,
			    /* # bytes of config data */ 0);
    }
}

static_always_inline void
vnet_feature_start_device_input_x2 (u32 sw_if_index, u32 *next0, u32 *next1,
				    vlib_buffer_t *b0, vlib_buffer_t *b1)
{
  ...
  if (PREDICT_FALSE (clib_bitmap_get (
	...
	vec_elt (cm->config_index_by_sw_if_index, sw_if_index);
      ...
      vnet_get_config_data (&cm->config_main, &b0->current_config_index, next0,
			    /* # bytes of config data */ 0);
      vnet_get_config_data (&cm->config_main, &b1->current_config_index, next1,
			    /* # bytes of config data */ 0);
    }
}

static_always_inline void
vnet_feature_start_device_input_x4 (u32 sw_if_index, u32 *next0, u32 *next1,
				    u32 *next2, u32 *next3, vlib_buffer_t *b0,
				    ...
				    vlib_buffer_t *b3)
{
  ...
  if (PREDICT_FALSE (clib_bitmap_get (
	...
	vec_elt (cm->config_index_by_sw_if_index, sw_if_index);
      ...
      vnet_get_config_data (&cm->config_main, &b1->current_config_index, next1,
			    /* # bytes of config data */ 0);
      vnet_get_config_data (&cm->config_main, &b2->current_config_index, next2,
			    /* # bytes of config data */ 0);
      vnet_get_config_data (&cm->config_main, &b3->current_config_index, next3,
			    /* # bytes of config data */ 0);
    }
}
...
typedef void (*vnet_feature_update_cb_t) (u32 sw_if_index, u8 arc_index,
...
int vnet_feature_is_enabled (const char *arc_name,
			     const char *feature_node_name, u32 sw_if_index);
