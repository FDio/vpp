...
typedef struct
{
  union
  {
    ...
    /* cnat session
     *
     * generic_flow_id serves dual purpose:
     *   BEFORE cnat lookup: scope_id (tunnel context) when HAS_SCOPE flag set
     *   AFTER  cnat lookup: session index (timestamp pool index)
     * cnat_lookup_create_or_return() clears flags and overwrites generic_flow_id
     * with the session index, completing the transition.
     */
    struct
    {
      u64 generic_flow_id : 24;	/* scope_id before lookup, session index after */
