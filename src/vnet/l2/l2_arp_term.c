...
format_arp_term_input_trace (u8 *s, va_list *va)
{
  ...
  s = format (s, "%U",
	      t->packet_data[0] == 0 ? format_ethernet_arp_header :
				       format_ip6_header,
...
l2_arp_term_publish (l2_arp_term_publish_event_t *ctx)
{
  ...
  vlib_process_signal_event (vlib_get_main (), l2_arp_term_process_node.index,
...
static inline void
l2_arp_term_publish_v6_dp (u32 sw_if_index, const ip6_address_t *addr,
...
static inline int
vnet_ip6_nd_term (vlib_main_t *vm, vlib_node_runtime_t *node,
		  vlib_buffer_t *p0, ethernet_header_t *eth, ip6_header_t *ip,
...
arp_term_l2bd (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (!ethernet_mac_address_equal (
		eth0->src_address, arp0->ip4_over_ethernet[0].mac.bytes)) ||
...
arp_term_init (vlib_main_t *vm)
{
  ...
  feat_bitmap_init_next_nodes (vm, arp_term_l2bd_node.index, L2INPUT_N_FEAT,
