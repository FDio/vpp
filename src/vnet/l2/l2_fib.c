...
/**
 * Set the mac table size and number of buckets.
 * The CLI format is:
 *    l2fib set table-size <count> num-buckets <count>
 */
static clib_error_t *
l2fib_set (vlib_main_t * vm,
	   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "table-size %U", unformat_memory_size,
		    &table_size))
	;
      else if (unformat (line_input, "num-buckets %u", &n_buckets))
	;
      else
	{
...
/*?
 * This command sets mac table size and number of buckets.
 * To take effect this command must be used before table is created or
 * table needs to be cleared with "clear l2fib" command.
 */
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (l2fib_set_cli, static) = {
  .path = "l2fib set",
  .short_help = "l2fib set table-size <count> num-buckets <count>",
  .function = l2fib_set,
};
/* *INDENT-ON* */
