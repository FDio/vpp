		 ...
		 u16 b_vlanid, u32 i_sid, u16 vlan_outer_tag)
{
  ...
  if (vtr_op == L2_VTR_POP_2)
    ...
    {
      _clib_memcpy (in_config->macs_tags.b_dst_address, b_dmac,
		    sizeof (in_config->macs_tags.b_dst_address));
      _clib_memcpy (in_config->macs_tags.b_src_address, b_smac,
  ...
  *i_sid = 0;
  ...
  if (in_config->push_and_pop_bytes == 0)
	...
	*vtr_op = L2_VTR_PUSH_2;
      ...
      _clib_memcpy (&eth_hdr->dst_address, in_config->macs_tags.b_dst_address,
		    sizeof (eth_hdr->dst_address));
      _clib_memcpy (&eth_hdr->src_address, in_config->macs_tags.b_src_address,
