...
vl_api_l2_flags_get_t_handler (vl_api_l2_flags_get_t *mp)
{
  ...
  if (sw_if_index < vec_len (l2input_main.configs))
    feature_bitmap = clib_atomic_load_relax_n (&l2input_main.configs[sw_if_index].feature_bitmap);
  if (sw_if_index < vec_len (l2output_main.configs))
    out_feature_bitmap =
      clib_atomic_load_relax_n (&l2output_main.configs[sw_if_index].feature_bitmap);
...
vl_api_l2_flags_set_t_handler (vl_api_l2_flags_set_t *mp)
{
  ...
  if (in_bitmap && sw_if_index < vec_len (l2input_main.configs))
    {
      if (mp->is_set)
	clib_atomic_fetch_or (&l2input_main.configs[sw_if_index].feature_bitmap, in_bitmap);
      else
	clib_atomic_fetch_and (&l2input_main.configs[sw_if_index].feature_bitmap, ~in_bitmap);
    }
  if (out_bitmap && sw_if_index < vec_len (l2output_main.configs))
    {
      if (mp->is_set)
	clib_atomic_fetch_or (&l2output_main.configs[sw_if_index].feature_bitmap, out_bitmap);
      else
	clib_atomic_fetch_and (&l2output_main.configs[sw_if_index].feature_bitmap, ~out_bitmap);
    }
...
vl_api_l2_interface_feat_flags_get_t_handler (vl_api_l2_interface_feat_flags_get_t *mp)
{
  ...
  u32 sw_if_index = ntohl (mp->sw_if_index);
  if (sw_if_index < vec_len (l2input_main.configs))
    flags = l2_bitmap_to_intf_feat_flags (
      clib_atomic_load_relax_n (&l2input_main.configs[sw_if_index].feature_bitmap));
...
vl_api_l2_interface_feat_flags_set_t_handler (vl_api_l2_interface_feat_flags_set_t *mp)
{
  ...
  if (bitmap && sw_if_index < vec_len (l2input_main.configs))
    {
      if (mp->is_set)
	clib_atomic_fetch_or (&l2input_main.configs[sw_if_index].feature_bitmap, bitmap);
      else
	clib_atomic_fetch_and (&l2input_main.configs[sw_if_index].feature_bitmap, ~bitmap);
    }
...
l2_api_hookup (vlib_main_t * vm)
{
  ...
  /* Mark read-only dumps and the new atomic-safe flag handlers as mp safe */
  vl_api_set_msg_thread_safe (
    am, REPLY_MSG_ID_BASE + VL_API_BRIDGE_DOMAIN_DUMP, 1);
  vl_api_set_msg_thread_safe (am, REPLY_MSG_ID_BASE + VL_API_L2_FLAGS_GET, 1);
  vl_api_set_msg_thread_safe (am, REPLY_MSG_ID_BASE + VL_API_L2_FLAGS_SET, 1);
  vl_api_set_msg_thread_safe (am, REPLY_MSG_ID_BASE + VL_API_L2_INTERFACE_FEAT_FLAGS_GET, 1);
  vl_api_set_msg_thread_safe (am, REPLY_MSG_ID_BASE + VL_API_L2_INTERFACE_FEAT_FLAGS_SET, 1);
