...
vl_api_l2_flags_get_t_handler (vl_api_l2_flags_get_t *mp)
{
  ...
  l2_input_config_t *in_cfg = &l2input_main.configs[sw_if_index];
  if (!l2_input_is_bridge (in_cfg) && !l2_input_is_xconnect (in_cfg))
    {
      rv = VNET_API_ERROR_INVALID_SW_IF_INDEX;
      goto bad_sw_if_index;
    }
  feature_bitmap = clib_atomic_load_relax_n (&in_cfg->feature_bitmap);
...
vl_api_l2_flags_set_t_handler (vl_api_l2_flags_set_t *mp)
{
  ...
  if (in_bitmap)
    {
      if (mp->is_set)
	clib_atomic_fetch_or (&in_cfg->feature_bitmap, in_bitmap);
      else
	clib_atomic_fetch_and (&in_cfg->feature_bitmap, ~in_bitmap);
    }
...
vl_api_l2_interface_feat_flags_get_t_handler (vl_api_l2_interface_feat_flags_get_t *mp)
{
  ...
  l2_input_config_t *cfg = &l2input_main.configs[sw_if_index];
  if (!l2_input_is_bridge (cfg))
    {
      rv = VNET_API_ERROR_INVALID_SW_IF_INDEX;
      goto bad_sw_if_index;
    }
  flags = l2_bitmap_to_intf_feat_flags (clib_atomic_load_relax_n (&cfg->feature_bitmap));
...
vl_api_l2_interface_feat_flags_set_t_handler (vl_api_l2_interface_feat_flags_set_t *mp)
{
  ...
  if (bitmap)
    {
      if (mp->is_set)
	clib_atomic_fetch_or (&cfg->feature_bitmap, bitmap);
      else
	clib_atomic_fetch_and (&cfg->feature_bitmap, ~bitmap);
    }
