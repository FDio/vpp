			       ...
			       int shg_set, int update_feature_bitmap)
{
  while (n_left >= 8)
	...
	{
	  clib_prefetch_load (vlib_buffer_get_current (b[4]));
	  clib_prefetch_load (vlib_buffer_get_current (b[5]));
	  clib_prefetch_load (vlib_buffer_get_current (b[6]));
	  clib_prefetch_load (vlib_buffer_get_current (b[7]));
	}
