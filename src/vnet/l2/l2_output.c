...
l2output_vtr (vlib_node_runtime_t * node, l2_output_config_t * config,
	      u32 feature_bitmap, vlib_buffer_t * b, u16 * next)
{
...
l2output_process_batch_inline (vlib_main_t * vm, vlib_node_runtime_t * node,
			       ...
			       vlib_buffer_t ** b, u16 * next, u32 n_left,
			       int l2_efp, int l2_vtr, int l2_pbb,
...
l2output_process_batch (vlib_main_t * vm, vlib_node_runtime_t * node,
			...
			u16 * next, u32 n_left, int l2_efp, int l2_vtr,
			int l2_pbb)
{
  u32 feature_bitmap = config->feature_bitmap & ~L2OUTPUT_FEAT_OUTPUT;
  if (config->shg == 0 && feature_bitmap == 0)
    l2output_process_batch_inline (vm, node, config, b, next, n_left, l2_efp,
				   l2_vtr, l2_pbb, 0, 0);
  ...
  while (n_left)
    {
      u16 off, count;
      u16 new_next;
      u32 *current_sw_if_index = sw_if_index;
      n_left -= count = clib_count_equal_u32 (sw_if_index, n_left);
      off = current_sw_if_index - sw_if_indices;

      config = vec_elt_at_index (msm->configs, current_sw_if_index[0]);
      ...
      if (config->out_vtr_flag && config->output_vtr.push_and_pop_bytes)
	{
	  if (feature_bitmap & L2OUTPUT_FEAT_EFP_FILTER)
	    l2output_process_batch (vm, node, config, bufs + off,
				    nexts + off, count,
				    /* l2_efp */ 1,
				    /* l2_vtr */ 1,
				    /* l2_pbb */ 0);
