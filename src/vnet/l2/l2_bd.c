		   ...
		   const u8 * mac_addr, u8 is_ip6, u8 is_add)
{
  ...
  if (is_ip6)
      ...
      if (is_add)
	{
	  if (old_mac == 0)
	    ...
	    {			/* update mac for ip6 address */
	      hp = hash_get_pair (bd_cfg->mac_by_ip6, &ip_addr->ip6);
	  ...
	  if (old_mac && (*old_mac == new_mac))
	    {
	      hp = hash_get_pair (bd_cfg->mac_by_ip6, &ip_addr->ip6);
	      ip6_addr_key = (ip6_address_t *) hp->key;
	      hash_unset_mem (bd_cfg->mac_by_ip6, &ip_addr->ip6);
    ...
    {
      old_mac = (u64 *) hash_get (bd_cfg->mac_by_ip4, ip_addr->ip4.as_u32);
      if (is_add)
	{
	  if (old_mac && (*old_mac == new_mac))
	    return 0;		/* mac entry already exist */
	  hash_set (bd_cfg->mac_by_ip4, ip_addr->ip4.as_u32, new_mac);
	}
      else
	{
	  if (old_mac && (*old_mac == new_mac))
	    hash_unset (bd_cfg->mac_by_ip4, ip_addr->ip4.as_u32);
	  else
	    return 1;
	}
    }
