		     ...
		     u64 feature_mask, u32 * sw_if_index, u8 enable_gso)
{
   ...
   * is a clash.
   *   feature mask disables gso, enable_gso is configured,
   *     then gso is enable
   *   feature mask enables gso, enable_gso is not configured,
   *     then gso is enable
   */
  if (vui->enable_gso == 0)
    if ((feature_mask & FEATURE_VIRTIO_NET_F_HOST_GUEST_TSO_FEATURE_BITS) ==
