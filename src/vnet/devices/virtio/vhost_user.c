...
vhost_user_socket_read (clib_file_t * uf)
{
  ...
  switch (msg.request)
      ...
      for (i = 0; i < msg.memory.nregions; i++)
	{
	  ...
	  vu_log_debug
	    (vui, "map memory region %d addr 0 len 0x%lx fd %d mapped 0x%lx "
	...
	(1 << VHOST_USER_PROTOCOL_F_MQ);
      msg.size = sizeof (msg.u64);
      vu_log_debug
	(vui, "if %d msg VHOST_USER_GET_PROTOCOL_FEATURES - reply 0x%016llx",
	 vui->hw_if_index, msg.u64);
      break;

    case VHOST_USER_SET_PROTOCOL_FEATURES:
      vu_log_debug
	(vui, "if %d msg VHOST_USER_SET_PROTOCOL_FEATURES features 0x%016llx",
	 vui->hw_if_index, msg.u64);
