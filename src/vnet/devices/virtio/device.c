...
STATIC_ASSERT (sizeof (virtio_ip4_psh_t) == 12, "ipv4 pseudo header is 12B");
STATIC_ASSERT (sizeof (virtio_ip6_psh_t) == 40, "ipv6 pseudo header is 40B");

static_always_inline u16
virtio_ip4_pseudo_header_cksum (ip4_header_t *ip4, u16 l3_hdr_len)
{
  virtio_ip4_psh_t psh = { 0 };
  ...
  return ~clib_net_to_host_u16 (ip_csum (&psh, sizeof (virtio_ip4_psh_t)));
}

static_always_inline u16
virtio_ip6_pseudo_header_cksum (ip6_header_t *ip6)
{
  virtio_ip6_psh_t psh = { 0 };
  ...
  return ~clib_net_to_host_u16 (ip_csum (&psh, sizeof (virtio_ip6_psh_t)));
}
		      ...
		      const int is_l2)
{
  ...
  if (b->flags & VNET_BUFFER_F_IS_IP4)
      ...
      if (oflags & VNET_BUFFER_OFFLOAD_F_TCP_CKSUM)
	{
	  ...
	  tcp->checksum = virtio_ip4_pseudo_header_cksum (
      ...
      else if (oflags & VNET_BUFFER_OFFLOAD_F_UDP_CKSUM)
	{
	  ...
	  udp->checksum = virtio_ip4_pseudo_header_cksum (
	    ip4, gho.l4_hdr_offset - gho.l3_hdr_offset);
      ...
      if (oflags & VNET_BUFFER_OFFLOAD_F_TCP_CKSUM)
	{
	  tcp_header_t *tcp =
	    (tcp_header_t *) (vlib_buffer_get_current (b) + gho.l4_hdr_offset);
	  tcp->checksum = virtio_ip6_pseudo_header_cksum (ip6);
	  hdr->csum_offset = STRUCT_OFFSET_OF (tcp_header_t, checksum);
	}
      else if (oflags & VNET_BUFFER_OFFLOAD_F_UDP_CKSUM)
	{
	  udp_header_t *udp =
	    (udp_header_t *) (vlib_buffer_get_current (b) + gho.l4_hdr_offset);
	  udp->checksum = virtio_ip6_pseudo_header_cksum (ip6);
	  hdr->csum_offset = STRUCT_OFFSET_OF (udp_header_t, checksum);
	}
    }
}
