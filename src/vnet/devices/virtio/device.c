...
virtio_ip4_pseudo_header_cksum (ip4_header_t *ip4)
{
  virtio_ip4_psh psh;
  ...
  psh.l4len = ip4->length;
  return ip_csum (&psh, sizeof (psh));
}
		      ...
		      const int is_l2)
{
  ...
  if (b->flags & VNET_BUFFER_F_IS_IP4)
      ...
      if (oflags & VNET_BUFFER_OFFLOAD_F_TCP_CKSUM)
	{
	  ...
	  tcp->checksum = virtio_ip4_pseudo_header_cksum (ip4);
      ...
      else if (oflags & VNET_BUFFER_OFFLOAD_F_UDP_CKSUM)
	{
	  ...
	  udp->checksum = virtio_ip4_pseudo_header_cksum (ip4);
