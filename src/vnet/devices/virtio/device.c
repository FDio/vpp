...
add_buffer_to_slot (vlib_main_t * vm, virtio_if_t * vif,
		    ...
		    u16 avail, u16 next, u16 mask, const int hdr_sz,
		    const int is_l2, const int do_gso, const int csum_offload,
		    uword node_index)
{
  u16 n_added = 0;
  vring_desc_t *d;
...
static_always_inline u16
add_buffer_to_slot_packed (vlib_main_t * vm, virtio_if_t * vif,
			   virtio_if_type_t type, virtio_vring_t * vring,
			   u32 bi, u16 next, const int hdr_sz,
			   const int is_l2, const int do_gso,
			   const int csum_offload, uword node_index)
{
...
virtio_interface_tx_packed_gso_inline (vlib_main_t * vm,
				       ...
				       vlib_frame_t * frame,
				       virtio_if_t * vif,
				       virtio_if_type_t type, const int is_l2,
				       const int do_gso,
...
retry:
  ...
  virtio_free_used_device_desc_packed (vm, vring, node->node_index);
  ...
  if (vif->packet_buffering)
      ...
      while (n_buffers_left && used < sz)
	{
	  ...
	  n_added =
	    add_buffer_to_slot_packed (vm, vif, type, vring, bi,
				       next, hdr_sz, is_l2, do_gso,
  ...
  while (n_left && used < sz)
      ...
      if (b0->flags & VLIB_BUFFER_IS_TRACED)
	{
      ...
      n_added =
	add_buffer_to_slot_packed (vm, vif, type, vring, buffers[0],
				   next, hdr_sz, is_l2, do_gso, csum_offload,
				   node->node_index);
      buffers++;
      n_left--;
      if (PREDICT_FALSE (n_added == 0))
      ...
      while (n_buffers_left && free_desc_count)
	{
	  ...
	  n_added =
	    add_buffer_to_slot (vm, vif, type, vring, bi, free_desc_count,
				avail, next, mask, hdr_sz, is_l2, do_gso,
				csum_offload, node->node_index);
	  if (PREDICT_FALSE (n_added == 0))
	    {
	      n_buffers_left--;
...
static_always_inline uword
virtio_interface_tx_packed_inline (vlib_main_t * vm,
				   vlib_node_runtime_t * node,
				   vlib_frame_t * frame, virtio_if_t * vif,
				   virtio_if_type_t type, const int is_l2)
{
  ...
  if (hw->flags & VNET_HW_INTERFACE_FLAG_SUPPORTS_GSO)
    return virtio_interface_tx_packed_gso_inline (vm, node, frame, vif, type,
						  is_l2, 1 /* do_gso */ ,
						  1 /* checksum offload */ ,
						  vif->packet_coalesce);
...
static_always_inline uword
virtio_interface_tx_split_inline (vlib_main_t * vm,
				  vlib_node_runtime_t * node,
				  vlib_frame_t * frame, virtio_if_t * vif,
				  virtio_if_type_t type, const int is_l2)
{
  ...
  if (hw->flags & VNET_HW_INTERFACE_FLAG_SUPPORTS_GSO)
    return virtio_interface_tx_split_gso_inline (vm, node, frame, vif, type,
						 is_l2, 1 /* do_gso */ ,
						 1 /* checksum offload */ ,
						 vif->packet_coalesce);
  else if (hw->flags & VNET_HW_INTERFACE_FLAG_SUPPORTS_TX_L4_CKSUM_OFFLOAD)
...
{
  if (vif->is_packed)
    return virtio_interface_tx_packed_inline (vm, node, frame, vif, type,
					      is_l2);
  else
    return virtio_interface_tx_split_inline (vm, node, frame, vif, type,
					     is_l2);
}
					       ...
					       vlib_frame_t * frame)
{
  ...
  if (vif->type == VIRTIO_IF_TYPE_TAP)
    return virtio_interface_tx_inline (vm, node, frame, vif,
				       VIRTIO_IF_TYPE_TAP, 1 /* is_l2 */ );
  else if (vif->type == VIRTIO_IF_TYPE_PCI)
    return virtio_interface_tx_inline (vm, node, frame, vif,
				       VIRTIO_IF_TYPE_PCI, 1 /* is_l2 */ );
  else if (vif->type == VIRTIO_IF_TYPE_TUN)
    return virtio_interface_tx_inline (vm, node, frame, vif,
				       VIRTIO_IF_TYPE_TUN, 0 /* is_l2 */ );
  else
    ASSERT (0);
