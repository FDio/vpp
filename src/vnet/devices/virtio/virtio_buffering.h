			     ...
			     u32 node_index, u16 size)
{
  ...
  vritio_vring_buffering_t *b_temp = 0;
  b_temp =
    (vritio_vring_buffering_t *)
    clib_mem_alloc (sizeof (vritio_vring_buffering_t));
  clib_memset (b_temp, 0, sizeof (vritio_vring_buffering_t));
...
static_always_inline u16
virtio_vring_buffering_store_packets (vritio_vring_buffering_t * buffering,
				      u32 * bi, u16 n_store)
{
  u16 mask, n_s = 0, i = 0;
...
static_always_inline u32
virtio_vring_buffering_read_from_front (vritio_vring_buffering_t * buffering)
{
  u32 bi = ~0;
...
static_always_inline u32
virtio_vring_buffering_read_from_back (vritio_vring_buffering_t * buffering)
{
  u32 bi = ~0;
  u16 mask = buffering->size - 1;
  if (virtio_vring_buffering_is_empty (buffering))
