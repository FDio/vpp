...
vl_api_virtio_pci_create_t_handler (vl_api_virtio_pci_create_t * mp)
{
  ...
  ap->rxq_sz = ntohs (mp->rx_ring_sz);
  ap->txq_sz = ntohs (mp->tx_ring_sz);
  ...
  rmp->_vl_msg_id = ntohs (VL_API_VIRTIO_PCI_CREATE_REPLY);
...
vl_api_virtio_pci_delete_t_handler (vl_api_virtio_pci_delete_t * mp)
{
  ...
  rmp = vl_msg_api_alloc (sizeof (*rmp));
  rmp->_vl_msg_id = ntohs (VL_API_VIRTIO_PCI_DELETE_REPLY);
  rmp->context = mp->context;
  rmp->retval = ntohl (rv);
			       ...
			       virtio_if_t * vif, u32 context)
{
  ...
  mp->sw_if_index = htonl (vif_if->sw_if_index);
  mp->rx_ring_sz = htons (vif->rxq_sz);
  mp->tx_ring_sz = htons (vif->txq_sz);
...
static void
vl_api_sw_interface_virtio_pci_dump_t_handler (vl_api_sw_interface_virtio_pci_dump_t *
					   mp)
{
  int rv;
  vl_api_registration_t *reg;
...
static void
virtio_pci_setup_message_id_table (api_main_t * am)
{
#define _(id,n,crc) vl_msg_api_add_msg_name_crc (am, #n "_" #crc, id);
  foreach_vl_msg_name_crc_virtio_pci;
#undef _
}
...
#undef _
   ...
   */
  virtio_pci_setup_message_id_table (am);
