			    ...
			    vlib_frame_t * frame, virtio_if_t * vif, u16 qid)
{
  ...
  p = vring->last_used_idx;

  if (vring->used->idx == p)
    return 0;

  num_slots = vring->used->idx - p;
  ...
  while (num_slots)
	...
	{
	  u16 slot = vring->used->ring[p & mask].id;
	  u16 len = vring->used->ring[p & mask].len - 10;
	  u32 bi0 = vring->buffers[slot];
	  vlib_buffer_t *b0 = vlib_get_buffer (vm, bi0);
#if 0
	  clib_warning ("lu %u u %u a %u slot %u num_slots %u len %u",
			p, vring->used->idx, vring->avail->idx, slot,
			num_slots, len);
#endif
  ...
  vring->last_used_idx = p;

  while ((vring->avail->idx & mask) != (vring->last_used_idx & mask))
    {
      u32 slot = vring->avail->idx & mask;
      u32 bi;
      vlib_buffer_alloc (vm, &bi, 1);
      vlib_buffer_t *b = vlib_get_buffer (vm, bi);
      vring->desc[slot].addr =
	pointer_to_uword (vlib_buffer_get_current (b)) - 10;
      vring->desc[slot].len = VLIB_BUFFER_DATA_SIZE + 10;
      vring->desc[slot].flags = VRING_DESC_F_WRITE;
      vring->avail->ring[slot] = slot;
      vring->buffers[slot] = bi;
      vring->avail->idx++;
    }
