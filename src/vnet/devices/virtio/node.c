...
virtio_refill_vring (vlib_main_t * vm, virtio_vring_t * vring)
{
  ...
  if (PREDICT_FALSE (n_alloc < n_slots))
    {
      clib_error ("alloc fail %u %u", n_alloc, n_slots);
      n_slots = n_alloc;
    }
     ...
     gso_type_t type)
{
  ...
  u16 src_left, dst_left, n_from, l2hdr_sz, l3hdr_sz, l4hdr_sz,
    ip_len, temp_sz, fid, tcp_flags, new_payload_sz;
  int is_ip4;
  ...
  if (is_tcp)
    {
      ...
      /* store riginal flags for last packet and reset FIN and RST */
      tcp_flags = tcp->flags;
...
#if 0
      _DBG ("\nhdr: flags 0x%02x gso_type 0x%02x hdr_len %u "
	    "gso_size %u csum_start %u csum_offset %u num_buffers %u",
	    hdr->flags, hdr->gso_type, hdr->hdr_len, hdr->gso_size,
	    hdr->csum_start, hdr->csum_offset, hdr->num_buffers);
      _DBG ("\nip: length %u checksum %x", clib_net_to_host_u16 (ip4->length),
	    ip4->checksum);
      _DBG ("\nudp: length %u checksum %x",
	    clib_net_to_host_u16 (udp->length), udp->checksum);
#endif
    }
  ...
  /* payload size without headers */
  u16 payload = clib_net_to_host_u16 (ip4->length) - l3hdr_sz - l4hdr_sz;

  if (is_ip4)
    ip4->length = clib_host_to_net_u16 (ip_len);
  else
    ip6->payload_length = clib_host_to_net_u16 (ip_len);
  ...
  if (is_ip4)
    ...
    ip6->payload_length =
      clib_host_to_net_u16 (db->current_length - l2hdr_sz);
