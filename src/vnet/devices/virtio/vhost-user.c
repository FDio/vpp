...
vhost_user_rx_thread_placement ()
{
  ...
  pool_foreach (vui, vum->vhost_user_interfaces, {
      vec_foreach (queue, vui->rx_queues)
	{
	  ...
	  thread_index = vnet_get_device_input_thread_index (vnm,
							     vui->hw_if_index,
							     *queue);
	  rv = vnet_device_input_unassign_thread (vnm, vui->hw_if_index,
						  *queue, thread_index);
	  if (rv)
  ...
  pool_foreach (vui, vum->vhost_user_interfaces, {
	...
	mode = VNET_DEVICE_INPUT_MODE_POLLING;
      else
	mode = VNET_DEVICE_INPUT_MODE_INTERRUPT;

      vec_foreach (queue, vui->rx_queues)
	{
	  vnet_set_device_input_node (vnm, vui->hw_if_index,
				      vhost_user_input_node.index);
	  vnet_device_input_assign_thread (vnm, vui->hw_if_index, *queue, ~0);
	  vnet_device_input_set_mode (vnm, vui->hw_if_index, *queue, mode);
	}
  });
  /* *INDENT-ON* */
}
