...
virtio_vring_init (vlib_main_t * vm, virtio_if_t * vif, u16 idx, u16 sz)
{
  ...
  if (idx % 2)
    {
      vec_validate_aligned (vif->txq_vrings, idx - 1, CLIB_CACHE_LINE_BYTES);
...
{
  virtio_vring_t *vring = vec_elt_at_index (vif->rxq_vrings, idx);
...
{
  virtio_vring_t *vring = vec_elt_at_index (vif->txq_vrings, idx);
...
virtio_vring_set_numa_node (vlib_main_t * vm, virtio_if_t * vif, u32 idx)
{
  ...
  virtio_vring_t *vring = vec_elt_at_index (vif->rxq_vrings, idx);
  thread_index =
    vnet_get_device_input_thread_index (vnm, vif->hw_if_index, idx);
