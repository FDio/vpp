...
clib_error_t *
clib_netlink_set_if_attr (int ifindex, unsigned short rta_type, void * data, int data_len)
{
  clib_error_t *err = 0;
  int sock;
...
clib_netlink_set_if_namespace (int ifindex, char * net_ns)
{
  ...
  err = clib_netlink_set_if_attr (ifindex, IFLA_NET_NS_FD, &ns_fd, sizeof(int));
  close (ns_fd);
  return err;
}
...
tap_create_if (vlib_main_t * vm, tap_create_if_args_t * args)
{
  ...
  if ((vif->remote_features & (1ULL << VIRTIO_NET_F_MRG_RXBUF)) == 0)
    {
      err = clib_error_return (0, "vhost-net backend doesn't support "
  ...
  if ((vif->remote_features & (1ULL << VIRTIO_F_VERSION_1)) == 0)
    {
      err = clib_error_return (0, "vhost-net backend is not virtio 1.0 "
			       "compliant");
      goto error;
    }
