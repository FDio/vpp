...
_(TAPV2_CREATE, tapv2_create)                       \
_(TAPV2_DELETE, tapv2_delete)                       \
_(SW_INTERFACE_TAPV2_DUMP, sw_interface_tapv2_dump)

static void
vl_api_tapv2_create_t_handler (vl_api_tapv2_create_t * mp)
{
  vlib_main_t *vm = vlib_get_main ();
  int rv;
  vl_api_tapv2_create_reply_t *rmp;
  ...
  rmp->_vl_msg_id = ntohs (VL_API_TAPV2_CREATE_REPLY);
...
static void
vl_api_tapv2_delete_t_handler (vl_api_tapv2_delete_t * mp)
{
  vlib_main_t *vm = vlib_get_main ();
  int rv;
  vpe_api_main_t *vam = &vpe_api_main;
  vl_api_tapv2_delete_reply_t *rmp;
  unix_shared_memory_queue_t *q;
  ...
  rmp = vl_msg_api_alloc (sizeof (*rmp));
  rmp->_vl_msg_id = ntohs (VL_API_TAPV2_DELETE_REPLY);
  rmp->context = mp->context;
  rmp->retval = ntohl (rv);
			       ...
			       tap_interface_details_t * tap_if, u32 context)
{
  ...
  mp = vl_msg_api_alloc (sizeof (*mp));
  memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id = ntohs (VL_API_SW_INTERFACE_TAPV2_DETAILS);
...
static void
vl_api_sw_interface_tapv2_dump_t_handler (vl_api_sw_interface_tapv2_dump_t *
					  mp)
{
  int rv;
