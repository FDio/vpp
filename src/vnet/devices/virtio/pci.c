...
#define PCI_DEVICE_ID_VIRTIO_NIC			0x1041
...
//    vd->flags |= VIRTIO_DEVICE_F_ELOG;
  ...
  if ((error =
       vlib_pci_device_open (&args->addr, virtio_pci_device_ids, &h)))
    {
...
*/
  ...
  error = ethernet_register_interface (vnm, virtio_device_class.index,
				       ...
				       &vd->hw_if_index, virtio_pci_flag_change);
...
void
virtio_pci_delete_if (vlib_main_t * vm, virtio_pci_device_t * vd)
{
  vnet_main_t *vnm = vnet_get_main ();
  virtio_pci_main_t *vmxm = &virtio_pci_main;
//  u32 i, bi;
