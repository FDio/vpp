...
virtio_pci_enable_gso (vlib_main_t * vm, virtio_if_t * vif)
{
  ...
  mq_hdr.ctrl.class = VIRTIO_NET_CTRL_GUEST_OFFLOADS;
  mq_hdr.ctrl.cmd = VIRTIO_NET_CTRL_GUEST_OFFLOADS_SET;
  mq_hdr.status = VIRTIO_NET_ERR;
  u64 offloads =
    VIRTIO_NET_F_GUEST_CSUM | VIRTIO_NET_F_GUEST_TSO4 |
    VIRTIO_NET_F_GUEST_TSO6;
  *((u64 *) mq_hdr.data) = offloads;

  status = virtio_pci_send_ctrl_msg (vm, vif, &mq_hdr, sizeof (offloads));
			   ...
			   u64 req_features)
{
   ...
   */
  u64 supported_features = VIRTIO_FEATURE (VIRTIO_NET_F_CTRL_GUEST_OFFLOADS)
