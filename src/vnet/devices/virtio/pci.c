...
static void
legacy_setup_queue (vlib_main_t * vm, virtio_if_t * vd, u16 queue_id,
		    virtio_vring_t * vring)
{
  ...
  vlib_pci_write_io_u16 (vm, vd->pci_dev_handle, VIRTIO_PCI_QUEUE_SEL,
			 &queue_id);
  vlib_pci_write_io_u32 (vm, vd->pci_dev_handle, VIRTIO_PCI_QUEUE_PFN, &addr);
}
...
virtio_pci_vring_init (vlib_main_t * vm, virtio_if_t * vd, u16 idx)
{
  ...
  legacy_setup_queue (vm, vd, idx, vring);
