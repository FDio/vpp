...
static void
legacy_read_config (virtio_if_t * vd, void *dst, int len, u32 addr)
{
//  u32 size = 0;
  vlib_pci_read_write_io (vd->pci_dev_handle, VLIB_READ,
			  PCI_CONFIG_SIZE + addr, dst, len);
}
...
/*
  while (len > 0)
      ...
      if (len >= 4)
	{
	  size = 4;
	  *(u32 *) dst =
	    virtio_pci_reg_read_u32 (vd, 0, PCI_CONFIG_SIZE + addr);
          vlib_pci_read_io_u32 (h, PCI_CONFIG_SIZE + addr, dst);
	}
      else if (len >= 2)
	{
	  size = 2;
	  *(u16 *) dst =
	    virtio_pci_reg_read_u16 (vd, 0, PCI_CONFIG_SIZE + addr);
          vlib_pci_read_io_u16 (h, PCI_CONFIG_SIZE + addr, dst);
	}
      else
	{
	  size = 1;
	  *(u8 *) dst =
	    virtio_pci_reg_read_u8 (vd, 0, PCI_CONFIG_SIZE + addr);
          vlib_pci_read_io_u8 (h, PCI_CONFIG_SIZE + addr, dst);
	}
      dst = (u8 *) dst + size;
      addr += size;
      len -= size;
...
static void
legacy_write_config (virtio_if_t * vd, void *src, int len, u32 addr)
{
  // u32 size = 0;

  vlib_pci_read_write_io (vd->pci_dev_handle, VLIB_WRITE,
			  PCI_CONFIG_SIZE + addr, src, len);
}
...
/*
  while (len > 0)
	...
	{
	  size = 4;
	  virtio_pci_reg_write_u32 (vd, 0, PCI_CONFIG_SIZE + addr, src);
	}
      else if (len >= 2)
	{
	  size = 2;
	  virtio_pci_reg_write_u16 (vd, 0, PCI_CONFIG_SIZE + addr, src);
	}
      else
	{
	  size = 1;
	  virtio_pci_reg_write_u8 (vd, 0, PCI_CONFIG_SIZE + addr, src);
	}
      src = (u8 *) src + size;
      addr += size;
      len -= size;
...
{
  u32 features;
  vlib_pci_read_io_u32 (vd->pci_dev_handle, VIRTIO_PCI_HOST_FEATURES,
			&features);
  return features;
}
...
legacy_set_features (virtio_if_t * vd, u64 features)
{
  ...
  vlib_pci_write_io_u32 (vd->pci_dev_handle, VIRTIO_PCI_GUEST_FEATURES,
			 (u32 *) & features);
  //virtio_pci_reg_write_u32 (vd, 0, VIRTIO_PCI_GUEST_FEATURES, &features);
}
...
{
  u8 status = 0;
  //return virtio_pci_reg_read_u8 (vd, 0, VIRTIO_PCI_STATUS);
  vlib_pci_read_io_u8 (vd->pci_dev_handle, VIRTIO_PCI_STATUS, &status);
  return status;
}
...
legacy_set_status (virtio_if_t * vd, u8 status)
{
  if (status != VIRTIO_CONFIG_STATUS_RESET)
    status |= legacy_get_status (vd);
  vlib_pci_write_io_u8 (vd->pci_dev_handle, VIRTIO_PCI_STATUS, &status);
  //virtio_pci_reg_write_u8 (vd, 0, VIRTIO_PCI_STATUS, &status);
}
...
{
  u8 isr = 0;
  vlib_pci_read_io_u8 (vd->pci_dev_handle, VIRTIO_PCI_ISR, &isr);
  return isr;
//  return virtio_pci_reg_read_u8 (vd, 0, VIRTIO_PCI_ISR);
}
...
{
  u16 queue_num = 0;
  vlib_pci_write_io_u16 (vd->pci_dev_handle, VIRTIO_PCI_QUEUE_SEL, &queue_id);
  vlib_pci_read_io_u16 (vd->pci_dev_handle, VIRTIO_PCI_QUEUE_NUM, &queue_num);
  return queue_num;
  // virtio_pci_reg_write_u16 (vd, 0, VIRTIO_PCI_QUEUE_SEL, &queue_id);
  // return virtio_pci_reg_read_u16 (vd, 0, VIRTIO_PCI_QUEUE_NUM);
}


static void
legacy_setup_queue (virtio_if_t * vd, u16 queue_id, virtio_vring_t * vring)
{
  u32 addr =
    pointer_to_uword ((void *) vring->desc) >> VIRTIO_PCI_QUEUE_ADDR_SHIFT;
  vlib_pci_write_io_u16 (vd->pci_dev_handle, VIRTIO_PCI_QUEUE_SEL, &queue_id);
  vlib_pci_write_io_u32 (vd->pci_dev_handle, VIRTIO_PCI_QUEUE_PFN, &addr);
}
...
static clib_error_t *
virtio_pci_device_init (vlib_main_t * vm)
{
  virtio_pci_main_t *vpm = &virtio_pci_main;
  clib_error_t *error = 0;
  if (vpm->physmem_region_alloc == 0)
    {
      u32 flags = VLIB_PHYSMEM_F_INIT_MHEAP | VLIB_PHYSMEM_F_HUGETLB;
      error =
	vlib_physmem_region_alloc (vm, "virtio pci descriptors", 4 << 20, 0,
				   flags, &vpm->physmem_region);
      if (error)
	return error;
      vpm->physmem_region_alloc = 1;
    }
  return error;
}

static void
virtio_pci_set_mac (virtio_if_t * vd)
{
  legacy_write_config (vd, vd->mac_addr, sizeof (vd->mac_addr), 0);
}

static u32
virtio_pci_get_mac (virtio_if_t * vd)
{
  if (vd->remote_features & VIRTIO_FEATURE (VIRTIO_NET_F_MAC))
    {
      legacy_read_config (vd, vd->mac_addr, sizeof (vd->mac_addr), 0);
      return 0;
    }
  return 1;
}
...
static u16
virtio_pci_is_link_up (virtio_if_t * vd)
{
  /*
  ...
  if (vd->remote_features & VIRTIO_FEATURE (VIRTIO_NET_F_STATUS))
    legacy_read_config (vd, &status, sizeof (status), /* mac */ 6);
  //status = virtio_pci_reg_read_u16 (vd, 0, PCI_CONFIG_SIZE + /* mac */ 6);
  return status;
}
...
virtio_pci_irq_1_handler (vlib_pci_dev_handle_t h, u16 line)
{
  ...
  if (virtio_pci_is_link_up (vd) & VIRTIO_NET_S_LINK_UP)
    {
...
static void
virtio_pci_irq_handler (vlib_pci_dev_handle_t h)
{
  virtio_main_t *vmxm = &virtio_main;
  uword pd = vlib_pci_get_private_data (h);
  virtio_if_t *vd = pool_elt_at_index (vmxm->interfaces, pd);
  ...
  isr = legacy_get_isr (vd);

  if (isr & VIRTIO_PCI_ISR_CONFIG)
    virtio_pci_irq_1_handler (h, line);
/*
If the lower bit is set: look through the used rings of all virtqueues for the device, to see if any
progress has been made by the device which requires servicing */
  if (isr & VIRTIO_PCI_ISR_INTR)
    virtio_pci_irq_0_handler (h, line);
}
...
virtio_pci_read_caps (vlib_pci_dev_handle_t h, virtio_if_t * vd)
{
  ...
  if ((error = vlib_pci_read_config_u8 (h, PCI_CAPABILITY_LIST, &pos)))
...
print_device (virtio_if_t * vd)
{
  for (int i = 0; i < 64; i += 4)
    {
      u32 data = 0;
      vlib_pci_read_io_u32 (vd->pci_dev_handle, i, &data);
      //virtio_pci_reg_read_u32 (vd, 0, i);
      clib_warning ("0x%lx", data);
    }
}

clib_error_t *
virtio_pci_read (vlib_pci_dev_handle_t h, virtio_if_t * vd)
{
  clib_error_t *error = 0;
  u8 status;
  ...
  status = legacy_reset (vd);
  print_device_status (status);
  legacy_set_status (vd, VIRTIO_CONFIG_STATUS_ACK);
  legacy_set_status (vd, VIRTIO_CONFIG_STATUS_DRIVER);
...
static void
virtio_negotiate_features (virtio_if_t * vd, u64 req_features)
{
  /*
  ...
  if (vd->remote_features & req_features & VIRTIO_FEATURE (VIRTIO_NET_F_MTU))
    {
      ...
      legacy_read_config (vd, &config.mtu, sizeof (config.mtu), 10);
      if (config.mtu < 64)
	vd->features &= ~VIRTIO_FEATURE (VIRTIO_NET_F_MTU);
    }

  vd->features = req_features & vd->remote_features;
  legacy_set_features (vd, vd->features);
}
...
#define ALIGN(x) (((x) + VIRTIO_PCI_VRING_ALIGN) & ~VIRTIO_PCI_VRING_ALIGN)

/*always_inline unsigned vring_size(u32 qsz)
{
...
clib_error_t *
virtio_pci_vring_init (vlib_main_t * vm, virtio_if_t * vd, u16 idx, u16 sz)
{
  virtio_pci_main_t *vpm = &virtio_pci_main;
  clib_error_t *error = 0;
  virtio_net_config_t config;
  u16 max_queue_pairs = 1;
  u16 ii = 0, queue_size = 0;

  if (vd->features & VIRTIO_FEATURE (VIRTIO_NET_F_MQ))
    {
      legacy_read_config (vd, &config.max_virtqueue_pairs,
			  sizeof (config.max_virtqueue_pairs), 8);
      max_queue_pairs = config.max_virtqueue_pairs;
    }

  clib_warning ("max virtqueue pairs %u", max_queue_pairs);

  if (max_queue_pairs < 1 && max_queue_pairs > 0x8000)
    clib_error_return (error, "max queue pair is %x", max_queue_pairs);

  for (ii = 0; ii < max_queue_pairs * 2; ii++)
    {
      queue_size = legacy_get_queue_num (vd, ii);
...
//  struct vhost_vring_file file = { 0 };
  ...
  i = vring_size (sz, VIRTIO_PCI_VRING_ALIGN);

  ptr =
    vlib_physmem_alloc_aligned (vm, vpm->physmem_region, &error, i,
				VIRTIO_PCI_VRING_ALIGN);
  //ptr = clib_mem_alloc_aligned (i, VIRTIO_PCI_VRING_ALIGN);
  memset (ptr, 0, i);
  struct vring vr;
  vr.desc = vring->desc;
  vr.avail = vring->avail;
  vr.used = vring->used;
  vring_init (&vr, sz, ptr, VIRTIO_PCI_VRING_ALIGN);
//  vring->desc = (struct vring_desc *) ptr;
//  vring->avail = (struct vring_avail *) (ptr + sz * sizeof(struct vring_desc));
//  vring->used = (struct vring_used *) (ptr + ALIGN(sizeof(struct vring_desc)*sz + sizeof(u16)*(3 + sz)));
  ...
  ((vring_avail_t *) (vring->avail))->flags = VIRTIO_RING_FLAG_MASK_INT;

  // i = (i, VIRTIO_PCI_VRING_ALIGN);
  ...
  legacy_setup_queue (vd, idx, vring);
...
virtio_pci_create_if (vlib_main_t * vm, virtio_pci_create_if_args_t * args)
{
  ...
  if ((error = vlib_pci_map_region (h, 0, (void **) &bar[0])))
    if ((error = vlib_pci_io_region (h, 0)))
      goto error;

  if ((error = vlib_pci_map_region (h, 1, (void **) &bar[1])))
    goto error;

  vd->bar[1] = ((u64) bar[1]);

  if ((error = virtio_pci_device_init (vm)))
    goto error;

  if ((error = virtio_pci_read_caps (h, vd)) == 0)
    {
      virtio_pci_read (h, vd);
    }

  virtio_negotiate_features (vd, args->features);
  ...
  if (msix_enabled == VIRTIO_MSIX_ENABLED)
    ...
    {
      vlib_pci_register_intx_handler (h, &virtio_pci_irq_handler);
    }

  if ((error = vlib_pci_intr_enable (h)))
    goto error;

//  if ((error = virtio_pci_vring_init (vd, args->rxq_size, args->txq_size)))
//    goto error;

  if ((error = virtio_pci_vring_init (vm, vd, 0, args->rxq_size)))
    goto error;

  if ((error = virtio_pci_vring_init (vm, vd, 1, args->txq_size)))
    goto error;

  if (virtio_pci_get_mac (vd))
    {
...
*/
  ...
  vlib_pci_intr_disable (vd->pci_dev_handle);
  legacy_reset (vd);
...
/*  vec_free (vd->txqs);
*/
  vlib_physmem_region_free (vm, vpm->physmem_region);
