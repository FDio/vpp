...
static clib_error_t *
virtio_pci_get_max_virtqueue_pairs (virtio_if_t * vd)
{
...
virtio_pci_vring_init (vlib_main_t * vm, virtio_if_t * vd, u16 idx)
{
  ...
  queue_size = legacy_get_queue_num (vd, idx);
...
void
virtio_pci_read_device_feature (virtio_if_t * vd)
{
...
int
virtio_pci_reset_device (virtio_if_t * vd)
{
  u8 status = 0;
...
clib_error_t *
virtio_pci_read_caps (virtio_if_t * vd)
{
  clib_error_t *error = 0;
			...
			virtio_pci_create_if_args_t * args)
{
  ...
  if ((error = virtio_pci_get_max_virtqueue_pairs (vd)))
    goto error;
  ...
  legacy_set_status (vd, VIRTIO_CONFIG_STATUS_DRIVER_OK);
