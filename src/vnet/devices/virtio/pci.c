...
static void
virtio_pci_irq_n_handler (vlib_main_t * vm, vlib_pci_dev_handle_t h, u16 line)
{
...
virtio_pci_irq_handler (vlib_main_t * vm, vlib_pci_dev_handle_t h)
{
  ...
  if (isr & VIRTIO_PCI_ISR_INTR)
	...
	virtio_pci_irq_n_handler (vm, h, line);
    }
			...
			virtio_pci_create_if_args_t * args, void **bar)
{
  ...
  if (vif->msix_enabled == VIRTIO_MSIX_ENABLED)
	...
	{
	  virtio_log_warning (vif, "config msix vector 0 is not set");
	}
      else
	{
      ...
      for (int i = 0, j = 1; i < vif->max_queue_pairs; i++)
	{

	  if (vif->virtio_pci_func->set_queue_irq (vm, vif, RX_QUEUE (i),
						   j) == VIRTIO_MSI_NO_VECTOR)
	    {
	      virtio_log_warning (vif, "%s (%u) %s %u %s", "queue",
				  RX_QUEUE (i), "msix vector", j,
				  "is not set");
	    }
	  else
	    {
...
virtio_pci_create_if (vlib_main_t * vm, virtio_pci_create_if_args_t * args)
{
  ...
  u32 interrupt_count = vlib_pci_get_num_msix_interrupts (vm, h);
  if (interrupt_count > 1)
						   ...
						   &virtio_pci_irq_0_handler)))
	{
      ...
      if ((error =
	   vlib_pci_register_msix_handler (vm, h, 1, interrupt_count,
					   &virtio_pci_irq_n_handler)))
	{
	  args->rv = VNET_API_ERROR_INVALID_REGISTRATION;
	  virtio_log_error (vif,
