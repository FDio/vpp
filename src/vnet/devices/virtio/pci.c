...
{
//  u32 size = 0;
  vlib_pci_read_write_io (vd->pci_dev_handle, VLIB_READ, PCI_CONFIG_SIZE + addr, dst, len);
}
/*
  while (len > 0)
    {
...
legacy_write_config (virtio_if_t * vd, void *src, int len, u32 addr)
{
 // u32 size = 0;
  ...
  vlib_pci_read_write_io (vd->pci_dev_handle, VLIB_WRITE, PCI_CONFIG_SIZE + addr, src, len);
}
/*
  while (len > 0)
    {
...
{
  u32 features;
  vlib_pci_read_io_u32 (vd->pci_dev_handle, VIRTIO_PCI_HOST_FEATURES, &features);
  return features;
}
...
legacy_set_features (virtio_if_t * vd, u64 features)
{
  ...
  vlib_pci_write_io_u32 (vd->pci_dev_handle, VIRTIO_PCI_GUEST_FEATURES, (u32 *) &features);
...
{
  if (vd->remote_features & VIRTIO_NET_F_MAC)
    {
...
virtio_pci_is_link_up(virtio_if_t * vd)
{
   ...
   */
  u16 status = 1;
  if (vd->remote_features & VIRTIO_NET_F_STATUS)
...
virtio_negotiate_features (virtio_if_t * vd, u64 req_features)
{
  ...
  if (vd->remote_features & req_features & VIRTIO_NET_F_MTU)
    {
      virtio_net_config_t config;
      legacy_read_config (vd, &config.mtu, sizeof (config.mtu), 10);
      if (config.mtu < 64)
	vd->features &= ~VIRTIO_NET_F_MTU;
    }
...
clib_error_t *
virtio_pci_vring_init (virtio_if_t * vd, u16 idx, u16 sz)
{
  clib_error_t *error = 0;
  virtio_net_config_t config;
  u16 max_queue_pairs = 1;
  u16 i = 0, queue_size = 0;

  if (vd->features & VIRTIO_NET_F_MQ)
    {
  ...
  for (i = 0; i < max_queue_pairs * 2; i++)
    {
      queue_size = legacy_get_queue_num (vd, i);
