...
legacy_write_config (virtio_if_t * vd, void *src, int len, u32 addr)
{
  ...
  while (len > 0)
	...
	{
	  size = 4;
	  vlib_pci_write_io_u32 (h, PCI_CONFIG_SIZE + addr, src);
	}
      else if (len >= 2)
	{
	  size = 2;
	  vlib_pci_write_io_u16 (h, PCI_CONFIG_SIZE + addr, src);
	}
      else
	{
	  size = 1;
	  vlib_pci_write_io_u8 (h, PCI_CONFIG_SIZE + addr, src);
	}
      src = (u8 *) src + size;
      addr += size;
      len -= size;
...
print_device (virtio_if_t * vd)
{
  ...
  vlib_pci_read_io_u32 (vd->pci_dev_handle, VIRTIO_PCI_GUEST_FEATURES,
			&data_u32);
  clib_warning ("guest features 0x%lx", data_u32);
  vlib_pci_read_io_u32 (vd->pci_dev_handle, VIRTIO_PCI_QUEUE_PFN, &data_u32);
  clib_warning ("queue address 0x%lx", data_u32);
  vlib_pci_read_io_u16 (vd->pci_dev_handle, VIRTIO_PCI_QUEUE_NUM, &data_u16);
  clib_warning ("queue size 0x%x", data_u16);
  vlib_pci_read_io_u16 (vd->pci_dev_handle, VIRTIO_PCI_QUEUE_SEL, &data_u16);
  clib_warning ("queue select 0x%x", data_u16);
  vlib_pci_read_io_u16 (vd->pci_dev_handle, VIRTIO_PCI_QUEUE_NOTIFY,
			&data_u16);
  clib_warning ("queue notify 0x%x", data_u16);
  vlib_pci_read_io_u8 (vd->pci_dev_handle, VIRTIO_PCI_STATUS, &data_u8);
  clib_warning ("status 0x%x", data_u8);
  vlib_pci_read_io_u8 (vd->pci_dev_handle, VIRTIO_PCI_ISR, &data_u8);
  clib_warning ("isr 0x%x", data_u8);

  u8 mac[6];
  legacy_read_config (vd, mac, sizeof (mac), 0);
  clib_warning ("mac %U", format_ethernet_address, mac);
  legacy_read_config (vd, &data_u16, sizeof (u16), /* offset to status */ 6);
  clib_warning ("link up/down status 0x%x", data_u16);
  legacy_read_config (vd, &data_u16, sizeof (u16),
		      /* offset to max_virtqueue */ 8);
  clib_warning ("num of virtqueue 0x%x", data_u16);
  legacy_read_config (vd, &data_u16, sizeof (u16), /* offset to mtu */ 10);
