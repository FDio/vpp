...
clib_error_t *
virtio_pci_read_caps (vlib_pci_dev_handle_t h)
{
  clib_error_t *error = 0;
  ...
  while (pos)
	...
	case VIRTIO_PCI_CAP_COMMON_CFG:
	  printf ("VIRTIO_PCI_CAP_COMMON_CFG");
	...
	case VIRTIO_PCI_CAP_NOTIFY_CFG:
	  ...
	  printf ("VIRTIO_PCI_CAP_NOTIFY_CFG");
	  break;
	case VIRTIO_PCI_CAP_DEVICE_CFG:
	  //hw->dev_cfg = get_cfg_addr(dev, &cap);
	  printf ("VIRTIO_PCI_CAP_DEVICE_CFG");
	  break;
	case VIRTIO_PCI_CAP_ISR_CFG:
	  printf ("VIRTIO_PCI_CAP_ISR_CFG");
...
//    vd->flags |= VIRTIO_DEVICE_F_ELOG;
  ...
  if ((error = virtio_pci_read_caps (h)))
    goto error;
