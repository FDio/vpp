...
inline void
device_status (vlib_main_t * vm, virtio_if_t * vd)
{
...
#undef _
  ...
  vlib_cli_output (vm, "  status 0x%x", vd->status);

  status_entry = (struct status_struct *) &status_array;
  while (status_entry->str)
    {
      if (vd->status & status_entry->bit)
...
inline void
debug_device_config_space (vlib_main_t * vm, virtio_if_t * vd)
{
  u32 data_u32;
  u16 data_u16;
  u8 data_u8;
...
virtio_pci_create_if (vlib_main_t * vm, virtio_pci_create_if_args_t * args)
{
  ...
  pool_foreach (vd, vmxm->interfaces, ({
    if (vd->pci_addr.as_u32 == args->addr)
      {
	args->rv = VNET_API_ERROR_INVALID_VALUE;
	args->error =
  ...
  if ((error =
       vlib_pci_device_open (vm, (vlib_pci_addr_t *) & vd->pci_addr,
