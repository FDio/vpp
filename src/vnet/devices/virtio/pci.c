			      ...
			      u16 num_queues)
{
  ...
  while (n_left)
    {
      ...
      clib_warning ("writing slot %u len %u", slot, len);
  ...
  clib_memcpy (&result, vlib_buffer_get_current (b),
	       sizeof (struct virtio_ctrl_mq_status_hdr));
  if (result.ctrl.class == VIRTIO_NET_CTRL_MQ)
    clib_warning ("VIRTIO_NET_CTRL_MQ");
  if (result.ctrl.cmd == VIRTIO_NET_CTRL_MQ_VQ_PAIRS_SET)
    clib_warning ("VIRTIO_NET_CTRL_MQ_VQ_PAIRS_SET");

  clib_warning ("multi-queues : %u", result.num_mqs.virtqueue_pairs);
  virtio_log_debug (vim, vif, "control-queue: status from backend : %u",
		    result.status);
...
virtio_pci_vring_init (vlib_main_t * vm, virtio_if_t * vif, u16 queue_num)
{
  ...
  if (queue_num % 2)
    {
      if (idx > vtm->n_vlib_mains)
	return error;
      vec_validate_aligned (vif->txq_vrings, idx, CLIB_CACHE_LINE_BYTES);
      vring = vec_elt_at_index (vif->txq_vrings, idx);
      clib_spinlock_init (&vring->lockp);
    }
  else
    {
      vec_validate_aligned (vif->rxq_vrings, idx, CLIB_CACHE_LINE_BYTES);
      vring = vec_elt_at_index (vif->rxq_vrings, idx);
    }
  i = vring_size (queue_size, VIRTIO_PCI_VRING_ALIGN);
  i = round_pow2 (i, VIRTIO_PCI_VRING_ALIGN);
  ptr =
			...
			virtio_pci_create_if_args_t * args)
{
  ...
  for (int i = 0; i < vif->max_queue_pairs; i++)
    {
      if ((error = virtio_pci_vring_init (vm, vif, i * 2)))
	virtio_log_warning (vim, vif, "%s (%u) %s", "error in rxq-queue",
			    i * 2, "initialization");

      if ((error = virtio_pci_vring_init (vm, vif, (i * 2) + 1)))
...
virtio_pci_create_if (vlib_main_t * vm, virtio_pci_create_if_args_t * args)
{
  ...
  vec_foreach_index (i, vif->rxq_vrings)
  {
    ...
    virtio_vring_set_numa_node (vm, vif, i);
...
virtio_pci_delete_if (vlib_main_t * vm, virtio_if_t * vif)
{
  ...
  for (i = 0; i < vif->max_queue_pairs; i++)
    {
      virtio_pci_legacy_del_queue (vm, vif, i * 2);
      virtio_pci_legacy_del_queue (vm, vif, (i * 2) + 1);
    }
