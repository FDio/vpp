  ...
  pool_foreach_index (index, dev->ports)
      ...
      if (port->type == VNET_DEV_PORT_TYPE_ETHERNET)
	{
	  ...
	  vnet_eth_interface_registration_t eir = {
	    ...
	    .max_frame_size = 1514,
	    .dev_instance = port->dev_instrance,
	  ...
	  port->hw_if_index = vnet_eth_register_interface (vnm, &eir);
	  sw = vnet_get_hw_sw_interface (vnm, port->hw_if_index);
	  port->sw_if_index = sw->sw_if_index;

	  vnet_hw_if_set_input_node (vnm, port->hw_if_index,
				     port->rx_node_index);
	}
    }
		   ...
		   vnet_dev_port_add_args_t args)
{
  ...
  port->dev_instrance = intf - dm->interfaces;
