				  ...
				  u16 queue_id, u32 node_index)
{
  ...
  vec_validate_init_empty (hw->input_node_index_by_queue, queue_id, 0);
					 ...
					 u16 queue_id)
{
  ...
  vm = vlib_mains[idx];
  rt =
    vlib_node_get_runtime_data (vm, hw->input_node_index_by_queue[queue_id]);
  ...
  vlib_node_set_interrupt_pending (vm,
				   hw->input_node_index_by_queue[queue_id]);
}
