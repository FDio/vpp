...
#if 0
static inline void
dpdk_ipsec_update_counters (dpdk_device_t * xd, f64 now)
{
}
#endif
...
dpdk_ipsec_process (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  for (dev_id = rte_cryptodev_count () - 1; dev_id >= 0; dev_id--)
      ...
      for (qp = 0; qp < dev_conf.nb_queue_pairs; qp++)
	{
	  ...
	  if (ret < 0)
	    {
	      clib_warning ("cryptodev %u qp %u setup error",
			    dev_id, qp);
	      goto error;
	    }
	}
      ...
      if (!vec_elt (dcm->cop_pools, dev_conf.socket_id))
	{
	  ...
	  /* TODO total crypto ops estimate */
...
error:
  /* TODO free allocated vectors and hash tables? */
