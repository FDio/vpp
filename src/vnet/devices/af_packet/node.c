			   ...
			   vlib_frame_t * frame, af_packet_if_t * apif)
{
  ...
  while ((tph->tp_status & TP_STATUS_USER) && (n_free_bufs > min_bufs))
	     ...
	     n_left_to_next)
	{
          ...
          struct virtio_net_hdr *vh = (struct virtio_net_hdr *) (((u8*)tph)
                  + tph->tp_mac - sizeof(struct virtio_net_hdr));
		  ...
		  if (PREDICT_TRUE (offset == 0))
			...
			clib_host_to_net_u16 (tph->tp_vlan_tci);
		      ...
		      bytes_copied = sizeof (ethernet_header_t);
                 ...
                 * equivalent to the vnet csum flag and we should have it
                 * hot at this point
                 **/
                if (PREDICT_TRUE(
                    ((apm->af_packet_global_flags & AF_PACKET_USES_VNET_HEADERS) != 0) &&
                    ((tph->tp_status & TP_STATUS_CSUMNOTREADY) != 0)
                   )) {
                    wsum_addr = (u16 *)(((u8 *) vlib_buffer_get_current (b0)) +
                        vlan_len + vh->csum_start + vh->csum_offset);
                    if (bytes_copied <= vh->csum_start) {
                        ...
                        wsum  = ip_csum_and_memcpy(wsum,
                                ((u8 *) vlib_buffer_get_current (b0)) +
                                vh->csum_start + vlan_len,
                                (u8 *) tph + tph->tp_mac + offset + vh->csum_start,
