...
create_packet_v3_sock (int host_if_index, tpacket_req3_t *rx_req,
		       ...
		       u32 *hdrlen_ptr, u8 *is_cksum_gso_enabled, u8 is_fanout)
{
  af_packet_main_t *apm = &af_packet_main;
  ...
  if (is_fanout)
    {
      int fanout =
	(((PACKET_FANOUT_FLAG_UNIQUEID | PACKET_FANOUT_HASH) << 16));
		      ...
		      u8 queue_id, u8 is_fanout)
{
  ...
  if (rx_queue)
	...
	AF_PACKET_RX_BLOCK_NR * rx_frames_per_block;
      rx_queue->rx_req->tp_retire_blk_tov = 0;
  ...
  ret = create_packet_v3_sock (apif->host_if_index, rx_req, tx_req, &fd, &ring,
			       &hdrlen, &is_cksum_gso_enabled, is_fanout);
  ...
  if (rx_queue)
	...
	{
	  rx_queue->rx_ring[i] = ring + i * rx_queue->rx_req->tp_block_size;
	}
      ...
      ring =
	ring + rx_queue->rx_req->tp_block_size * rx_queue->rx_req->tp_block_nr;
    }
  ...
  if (tx_queue)
	...
	{
	  tx_queue->tx_ring[i] = ring + i * tx_queue->tx_req->tp_block_size;
	}
...
af_packet_create_if (af_packet_create_if_arg_t *arg)
{
  ...
  apif->dev_instance = apif - apm->interfaces;
  ...
  mhash_set_mem (&apm->if_index_by_host_if_name, host_if_name_dup,
		 (uword *) &apif->dev_instance, 0);
  arg->sw_if_index = apif->sw_if_index;
...
af_packet_rx_queue_free (af_packet_if_t *apif, af_packet_queue_t *rx_queue)
{
  ...
  if (rx_queue->rx_ring)
    {
      u32 ring_sz =
	rx_queue->rx_req->tp_block_size * rx_queue->rx_req->tp_block_nr;
      // FIXME: unmap the memory
      if (munmap (rx_queue->rx_ring, ring_sz))
	vlib_log_warn (
	  apm->log_class,
	  "Host interface %s could not free ring from rx queue %u",
	  apif->host_if_name, rx_queue->queue_id);
...
af_packet_tx_queue_free (af_packet_if_t *apif, af_packet_queue_t *tx_queue)
{
  ...
  if (tx_queue->tx_ring)
    {
      u32 ring_sz =
	tx_queue->tx_req->tp_block_size * tx_queue->tx_req->tp_block_nr;
      // FIXME: unmap the memory
      if (munmap (tx_queue->tx_ring, ring_sz))
	vlib_log_warn (
	  apm->log_class,
	  "Host interface %s could not free ring from tx queue %u",
	  apif->host_if_name, tx_queue->queue_id);
...
af_packet_delete_if (u8 *host_if_name)
{
  ...
  mhash_unset (&apm->if_index_by_host_if_name, host_if_name,
	       (uword *) &apif->dev_instance);
