 ...
 */
static void vnet_hw_interface_update_node_state (vlib_main_t *vm,
						 vnet_hw_interface_t *hw,
						 vnet_hw_interface_rx_runtime_t *rt)
{
  rt->enabled_node_state =
      rt->queue_mode_counters[VNET_HW_INTERFACE_RX_MODE_INTERRUPT] ?
	  VLIB_NODE_STATE_INTERRUPT : VLIB_NODE_STATE_DISABLED;
  rt->enabled_node_state =
      (rt->queue_mode_counters[VNET_HW_INTERFACE_RX_MODE_POLLING] ||
	  rt->queue_mode_counters[VNET_HW_INTERFACE_RX_MODE_ADAPTIVE]) ?
	      VLIB_NODE_STATE_POLLING : rt->enabled_node_state;
 ...
 * assigned to it.
 */
static void vnet_hw_interface_rx_unschedule (vnet_main_t * vnm,
					     vnet_hw_interface_t *hw,
					     u16 queue_id)
{
  ...
  DBG_VNET("rx remove queue %d from %U ", queue_id,
 ...
 * if any, is used.
 */
static void vnet_hw_interface_rx_schedule (vnet_main_t * vnm,
					   vnet_hw_interface_t *hw,
					   u16 queue_id)
{
  ...
  DBG_VNET("rx set queue %d from %U", queue_id,
	   format_vnet_hw_interface_name, vnm, hw);
  ...
  DBG_VNET("rx set queue %d from %U on thread %d and rss %d", queue_id,
  	   format_vnet_hw_interface_name, vnm, hw,
  ...
  vnet_hw_interface_rx_runtime_t *rt =
      vlib_node_get_runtime_data (vm, hw->input_node_index);
  vnet_hw_interface_rx_queue_runtime_t q = {
  ...
  rxq->current_rx_queue_runtime =
      &rt->queues_per_rss[rxq->rss_slot][vec_len(rt->queues_per_rss[rxq->rss_slot])];
  ...
  if (rxq->current_rx_queue_runtime != NULL)
    {
      ...
      vnet_hw_interface_rx_unschedule(vnm, hw, queue_id);
      rxq->rss_slot = rss_slot;
      rxq->thread_index = thread_index;
      vnet_hw_interface_rx_schedule(vnm, hw, queue_id);
				   ...
				   u16 queue_id, u8 disable)
{
  ...
  DBG_VNET("rx %s queue %d from %U", disable?"disable":"enable", queue_id,
	format_vnet_hw_interface_name, vnm, hw);

  if ((!disable && (rxq->current_rx_queue_runtime != NULL)) ||
			       ...
			       u16 queue_id, vnet_hw_interface_rx_mode mode)
{
  ...
  if (dev_class->rx_mode_change_function)
    if ((err = dev_class->rx_mode_change_function(vnm, hw_if_index, queue_id,
					       mode)) != NULL)
			       ...
			       u16 queue_id, vnet_hw_interface_rx_mode * mode)
{
  ...
  if (!rxq)
    {
      mode = VNET_HW_INTERFACE_RX_MODE_UNKNOWN;
      return VNET_API_ERROR_INVALID_QUEUE_INDEX;
    }
	...
	if (rxq->rss_slot > rss_mask && rxq->current_rx_queue_runtime != NULL)
	  {
	    vnet_hw_interface_set_rx_thread(vnm, hw_if_index,
					    rxq - hw->rx_queues, rxq->thread_index,
...
static u16
vnet_hw_interface_get_tx_queue_index(vnet_hw_interface_t *hw,
				     u32 queue_id)
{
...
int vnet_hw_interface_update_tx_queues(vnet_main_t * vnm,
				       vnet_hw_interface_t *hw)
{
  u32 thread_index = 0;
  ...
  for (thread_index = 0;
      ...
      thread_index++)
    {
      ...
      DBG_VNET("  thread=%d rt=%p rt->tx_queue_per_rss=%p", thread_index,
	       rt, rt->tx_queue_per_rss);

      vec_validate(rt->tx_queue_per_rss, rt->rss_mask);
      vec_foreach(queue, rt->tx_queue_per_rss) {
	if (queue - rt->tx_queue_per_rss <= rt->rss_mask) {
	    /* Active RSS, need to find a tx queue */
	    if ((queue->tx_queue_index =
		vnet_hw_interface_get_tx_queue_index(hw,
			queue->configured_queue)) == 0xffff)
	      {
	    ...
	    queue->queue_id = hw->tx_queues[queue->tx_queue_index].queue_id;

	    /* This pretty complicated logic is just here to detect when
	     * a queue is used by more than one thread */
	    hw->tx_queues[queue->tx_queue_index].thread_index =
		(hw->tx_queues[queue->tx_queue_index].thread_index == 0 ||
		    hw->tx_queues[queue->tx_queue_index].thread_index == (thread_index + 1) ) ?
		    (thread_index + 1) : ~0;
	    DBG_VNET("Counting queue index %d -> %d", queue->tx_queue_index,
		     hw->tx_queues[queue->tx_queue_index].thread_index);
	} else {
	    queue->tx_queue_index = ~0;
	    queue->queue_id = ~0;
	}
      }
    }
        ...
        vec_foreach(queue, rt->tx_queue_per_rss) {
          ...
          if (queue->tx_queue_index != ~0 &&
              hw->tx_queues[queue->tx_queue_index].thread_index == ~0)
            {
              queue->lockp =
        	  &vnm->tx_locks[
			hw->tx_queues[queue->tx_queue_index].tx_lock_index].lock[0];
            }
				   ...
				   u8 disable)
{
  ...
  q.tx_lock_index = lock - vnm->tx_locks;
				   ...
				   u32 thread_index, u16 rss_mask)
{
  ...
  DBG_VNET("Set TX rss for iface %U and thread %d to %d",
	   format_vnet_hw_interface_name, vnm, hw, thread_index, rss_mask + 1);
  ...
  rt->rss_mask = rss_mask;
  vec_validate(rt->tx_queue_per_rss, rss_mask);
  vnet_hw_interface_update_tx_queues(vnm, hw);
  return 0;
}

int
vnet_hw_interface_set_tx_queue_thread (vnet_main_t * vnm, u32 hw_if_index,
				       u32 thread_index, u16 rss_slot,
				       u16 queue_id)
{
  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
  ...
  vec_validate(rt->tx_queue_per_rss, rss_slot);
