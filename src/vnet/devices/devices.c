...
void
vnet_device_input_assign_worker (u32 node_index, u32 hw_if_index,
				 u16 queue_id, uword cpu_index)
{
  ...
  if (hw->input_node_index == 0)
    {
      hw->input_node_index = node_index;
    }
  else
    ASSERT (hw->input_node_index == node_index);
			  ...
			  vlib_cli_command_t * cmd)
{
  ...
  foreach_vlib_main (({
    clib_bitmap_foreach (si, pn->sibling_bitmap,
	  ...
	  {
	    s = format (s, "    interface %U queue %u\n",
...
static int
vnet_device_move (u32 hw_if_index, u16 queue_id, uword cpu_index)
{
  vnet_main_t *vnm = vnet_get_main ();
  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
  vnet_device_input_runtime_t *rt;
...
deleted:
  ...
  rt =
    vlib_node_get_runtime_data (vlib_mains[cpu_index], hw->input_node_index);
		      ...
		      vlib_cli_command_t * cmd)
{
  ...
  rv = vnet_device_move (hw_if_index, queue, cpu_index);
