...
void
vnet_queue_assign_rx_thread (vnet_main_t * vnm, u32 node_index, u32 hw_if_index, u16 queue_id, uword thread_index)
{
  ...
  vnet_device_input_runtime_t *rt = vlib_node_get_runtime_data (vm, node_index);
				    ...
				    u16 queue_id, uword thread_index)
{
  ...
  vnet_queue_assign_rx_thread (vnm, hw->input_node_index, hw_if_index, queue_id, thread_index);
  vlib_main_t *vm = vlib_mains[thread_index];
  vlib_node_set_state (vm, hw->input_node_index, VLIB_NODE_STATE_POLLING);
}
