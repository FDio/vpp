		       ...
		       vlib_cli_command_t * cmd)
{
  ...
  pool_foreach (mif, mm->interfaces,
    ({
       ...
       vlib_cli_output (vm, "  ring-size %u num-c2s-rings %u num-s2c-rings %u",
			(1 << mif->log2_ring_size), mif->num_s2m_rings, mif->num_m2s_rings);
       for (i=0; i< mif->num_s2m_rings; i++)
         {
	   memif_ring_t * ring = memif_get_s2m_ring (mif, i);
