...
memif_get_buffer (memif_if_t * mif, memif_ring_type_t type, u16 ring_num,
		  u16 slot)
{
  void *p = mif->shm;
  int ring_size =
  ...
  p += sizeof (memif_shm_t);
  p += (mif->num_s2m_rings + mif->num_m2s_rings) * ring_size;

  slot += (ring_num + type * mif->num_s2m_rings) * (1 << mif->log2_ring_size);

  return p + (slot * mif->buffer_size);
}
