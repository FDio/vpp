...
memif_connect (vlib_main_t * vm, memif_if_t * mif)
{
  ...
  vec_validate_aligned (mif->s2m_ring_data, mif->num_s2m_rings - 1,
			CLIB_CACHE_LINE_BYTES);
  vec_validate_aligned (mif->m2s_ring_data, mif->num_m2s_rings - 1,
			CLIB_CACHE_LINE_BYTES);

  for (i = 0; i < mif->num_s2m_rings; i++)
    {
      rd = vec_elt_at_index (mif->s2m_ring_data, i);
      vec_validate_aligned (rd->buffers, 1 << mif->log2_ring_size,
			    CLIB_CACHE_LINE_BYTES);
    }
  for (i = 0; i < mif->num_m2s_rings; i++)
    {
      rd = vec_elt_at_index (mif->m2s_ring_data, i);
      vec_validate_aligned (rd->buffers, 1 << mif->log2_ring_size,
			    CLIB_CACHE_LINE_BYTES);

      /* slave needs to provide free buffers to master */
      if (mif->flags & MEMIF_IF_FLAG_IS_SLAVE)
	{
	  memif_ring_t *ring = memif_get_m2s_ring (mif, i);
	  vlib_buffer_alloc (vm, rd->buffers, (1 << mif->log2_ring_size));
	  for (j = 0; j < (1 << mif->log2_ring_size); j++)
	    {
	      vlib_buffer_t *b = vlib_get_buffer (vm, rd->buffers[j]);
	      ring->desc[j].addr =
		pointer_to_uword (vlib_buffer_get_current (b));
	      ring->desc[j].length = n_buffer_bytes;
	      ring->desc[j].flags = 0;
	      ring->tail = 0;
	      ring->head = (1 << mif->log2_ring_size) - 1;
	    }
	}
    }
...
memif_connect_master (vlib_main_t * vm, memif_if_t * mif)
{
  ...
  msg.shared_mem_size = sizeof (memif_shm_t) +
  ...
  ((memif_shm_t *) mif->shm)->cookie = 0xdeadbeef;

  for (i = 0; i < mif->num_s2m_rings; i++)
    {
      ring = memif_get_s2m_ring (mif, i);
      ring->head = ring->tail = 0;
    }
  for (i = 0; i < mif->num_m2s_rings; i++)
    {
      ring = memif_get_m2s_ring (mif, i);
      ring->head = ring->tail = 0;
    }
