			   ...
			   memif_ring_type_t type)
{
  ...
  while (n_left > 3 && free_slots > 1)
    {
      if (PREDICT_TRUE (head + 3 < ring_size))
	{
	  CLIB_PREFETCH (first_b + (head + 2) * mif->buffer_size,
			 CLIB_CACHE_LINE_BYTES, STORE);
	  CLIB_PREFETCH (first_b + (head + 3) * mif->buffer_size,
			 CLIB_CACHE_LINE_BYTES, STORE);
	...
	{
	  CLIB_PREFETCH (first_b + ((head + 2) % mask) * mif->buffer_size,
			 CLIB_CACHE_LINE_BYTES, STORE);
	  CLIB_PREFETCH (first_b + ((head + 3) % mask) * mif->buffer_size,
			 CLIB_CACHE_LINE_BYTES, STORE);
