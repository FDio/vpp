                           ...
                           memif_ring_type_t type)
{
  ...
  // TODO: move lock to mif->ring_data once we really support multiple rings
  ...
  while (n_left > 3 && free_slots > 3)
    {
      ...
      ASSERT(ring->desc[head].length <= mif->buffer_size); // TODO: drop the packet
      head = (head + 1) & mask;
