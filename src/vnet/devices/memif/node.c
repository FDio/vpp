			   ...
			   memif_ring_type_t type)
{
  ...
  u16 num_slots;
  void *first_b = memif_get_buffer (mif, type, rid, 0);
  ...
  while (num_slots)
    {
      ...
      while (num_slots > 3 && n_left_to_next > 2)
	{
	  if (PREDICT_TRUE (rd->last_head + 3 < ring_size))
	    {
	      CLIB_PREFETCH (first_b + (rd->last_head + 2) * mif->buffer_size,
			     CLIB_CACHE_LINE_BYTES, LOAD);
	      CLIB_PREFETCH (first_b + (rd->last_head + 3) * mif->buffer_size,
			     CLIB_CACHE_LINE_BYTES, LOAD);
	    ...
	    {
	      CLIB_PREFETCH (first_b +
			     ((rd->last_head + 2) % mask) * mif->buffer_size,
			     CLIB_CACHE_LINE_BYTES, LOAD);
	      CLIB_PREFETCH (first_b +
			     ((rd->last_head + 3) % mask) * mif->buffer_size,
			     CLIB_CACHE_LINE_BYTES, LOAD);
	  ...
	  bi1 = nm->rx_buffers[cpu_index][last_empty_buffer - 1];
	  _vec_len (nm->rx_buffers[cpu_index]) -= 2;

	  if (last_empty_buffer > 4)
	    {
	      memif_prefetch_buffer_and_data (vm,
					      nm->rx_buffers[cpu_index]
					      [last_empty_buffer - 2]);
	      memif_prefetch_buffer_and_data (vm,
					      nm->rx_buffers[cpu_index]
					      [last_empty_buffer - 3]);
	  ...
	  /* copy buffer */
	  void *mb0 = first_b + rd->last_head * mif->buffer_size;
	  ...
	  void *mb1 = first_b + rd->last_head * mif->buffer_size;
