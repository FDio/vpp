...
typedef enum
{
  ...
  VNET_DEV_EVENT_PORT_ADMIN_STATE_CHANGE,
      ...
      if (new_event_data)
	{
	  ...
	  vec_foreach (ed, event_data)
		...
		case VNET_DEV_EVENT_PERIODIC_STOP:
		  ...
		  periodic = 0;
		  break;
		case VNET_DEV_EVENT_PORT_ADMIN_STATE_CHANGE:
		  p = ed->port_admin_state_change.port;
		  p->admin_up = ed->port_admin_state_change.state != 0;
		  if (p->dev->ops.port_admin_state_change)
		    p->dev->ops.port_admin_state_change (vm, p);
		  log_debug (dev, "port %u admin state changed to %s",
			     p->port_id, p->admin_up ? "up" : "down");
		  break;
		default:
		  ASSERT (0);
		};
  ...
  if (l > 0)
    {
      ...
      vlib_node_rename (vm, n->index, "%v-process", dev->name);
	...
	.runtime_data_bytes = sizeof (void *),
      ...
      vlib_register_node (vm, &r, "%v-process", dev->name);
...
vnet_dev_admin_up_down_fn (vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
  ...
  const vnet_dev_event_data_t
    ed = { .event = VNET_DEV_EVENT_PORT_ADMIN_STATE_CHANGE,
	   .port_admin_state_change = {
	     .port = p,
	     .state = (flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) != 0,
	   } };

  vnet_dev_process_event_send (vm, p->dev, ed);
  return 0;
}
