...
typedef enum
{
  VNET_DEV_EVENT_PROCESS_QUIT = 1,
  VNET_DEV_EVENT_PERIODIC_START = 2,
  VNET_DEV_EVENT_PERIODIC_STOP = 3,
...
vnet_dev_process_quit (vlib_main_t *vm, vnet_dev_t *dev)
{
  ...
  vnet_dev_event_data_t ed = {
    .calling_process_index = vlib_get_current_process_node_index (vm),
  };

  vlib_process_signal_event_pointer (vm, dev->process_node_index,
				     VNET_DEV_EVENT_PROCESS_QUIT, &ed);
		    ...
		    vnet_dev_periodic_op_t pop)
{
  ...
  *p = pop;
  if (pool_elts (dev->periodic_ops) == 1)
    vlib_process_signal_event_pointer (vm, dev->process_node_index,
				       VNET_DEV_EVENT_PERIODIC_START, 0);
  return 1;
}
...
_vnet_dev_poll_remove (vlib_main_t *vm, vnet_dev_t *dev, void *op, void *arg)
{
  ...
  pool_foreach (pop, dev->periodic_ops)
	...
	if (pool_elts (dev->periodic_ops) == 0)
	  vlib_process_signal_event_pointer (vm, dev->process_node_index,
					     VNET_DEV_EVENT_PERIODIC_STOP, 0);
	return 1;
      }
  return 0;
}
