...
/*?
 * There is a VPP development package for NETMAP/VALE. '<em>netmap</em>'
 * is a framework for very fast packet I/O from userspace. '<em>VALE</em>'
 * is an equally fast in-kernel software switch using the netmap API.
 * '<em>netmap</em>' includes '<em>netmap pipes</em>', a shared memory
 * packet transport channel. Together, they provide a high speed user-space
 * interface that allows VPP to patch into a linux namespace or linux container.
 * The NETMAP/VALE package generates the '<em>netmap.ko</em>' kernel module
 * that needs to be loaded before netmap interfaces can be created,
 *
 * Create a netmap interface that will attach to a linux namespace
 * or linux container. The interface (linux namespace or container) must
 * exist. Once created, a new netmap interface will exist in VPP with the
 * name '<em>netmap-<ifname></em>', where '<em><ifname></em>' takes
 * one of two forms:
