		 ...
		 vlib_cli_command_t * cmd)
{
  ...
  if (classify_table_index != (u32) ~ 0 &&
      pool_is_free_index (vcm->tables, classify_table_index))
    return clib_error_return (0, "classify table %d doesn't exist",
			      classify_table_index);
  ...
  hw->n_trace = count;
  hw->trace_classify_table_index = classify_table_index;
