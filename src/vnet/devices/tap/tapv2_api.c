...
static void
vl_api_tap_create_v4_t_handler (vl_api_tap_create_v4_t *mp)
{
  ...
  ap->busypoll_timeout_us = mp->busypoll_timeout_us;
...
static void
tap_send_sw_interface_details_v3 (vpe_api_main_t *am,
				  vl_api_registration_t *reg,
				  tap_interface_details_t *tap_if, u32 context)
{
  vl_api_sw_interface_tap_v3_details_t *mp;
  mp = vl_msg_api_alloc (sizeof (*mp));
  clib_memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id = REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_TAP_V2_DETAILS;
  mp->id = tap_if->id;
  mp->sw_if_index = tap_if->sw_if_index;
  mp->tap_flags = tap_if->tap_flags;
  clib_memcpy (mp->dev_name, tap_if->dev_name,
	       MIN (ARRAY_LEN (mp->dev_name) - 1,
		    strlen ((const char *) tap_if->dev_name)));
  mp->rx_ring_sz = tap_if->rx_ring_sz;
  mp->tx_ring_sz = tap_if->tx_ring_sz;
  ...
  mp->host_mtu_size = tap_if->host_mtu_size;
  mp->busypoll_timeout_us = tap_if->busypoll_timeout_us;
  mac_address_encode (&tap_if->host_mac_addr, mp->host_mac_addr);
...
static void
vl_api_sw_interface_tap_v3_dump_t_handler (
  vl_api_sw_interface_tap_v3_dump_t *mp)
{
  int rv;
  ...
  filter_sw_if_index = mp->sw_if_index;
  if (mp->sw_if_index != ~0)
  ...
  vec_foreach (tap_if, tapifs)
	...
	tap_send_sw_interface_details_v3 (am, reg, tap_if, mp->context);
    }
