				...
				vnet_crypto_handler_type_t t)
{
  ...
  vnet_crypto_op_id_t id = 0;

  if (ad->alg_type == VNET_CRYPTO_ALG_T_AUTH)
    {
      id = ad->op_by_type[VNET_CRYPTO_OP_TYPE_HMAC];
      if (id == 0)
	id = ad->op_by_type[VNET_CRYPTO_OP_TYPE_HASH];
    }
  else
    {
      id = ad->op_by_type[VNET_CRYPTO_OP_TYPE_ENCRYPT];
      if (id == 0)
	id = ad->op_by_type[VNET_CRYPTO_OP_TYPE_DECRYPT];
    }

  if (id == 0)
    return VNET_CRYPTO_ENGINE_ID_NONE;

  return cm->opt_data[id].active_engine_index[t];
}
...
vnet_crypto_key_set_default_engine (vnet_crypto_key_t *key, vnet_crypto_handler_type_t t)
{
  ...
  vnet_crypto_key_set_engine (key, t, vnet_crypto_key_default_engine (cm, key->alg, t));
}
