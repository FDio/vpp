				      ...
				      u32 n_ops)
{
  ...
  if (chunks)
	...
	rv = (cm->ops_handlers[opt]) (vm, ops, 0, n_ops);
    }
  return rv;
}
...
vnet_crypto_register_ops_handler (vlib_main_t * vm, u32 engine_index,
				  ...
				  vnet_crypto_ops_handler_t * fn)
{
  vnet_crypto_register_ops_handler_inline (vm, engine_index, opt, fn,
					   CRYPTO_OP_SIMPLE);
}

void
vnet_crypto_register_ops_handler_chained (vlib_main_t * vm, u32 engine_index,
					  vnet_crypto_op_id_t opt,
					  vnet_crypto_ops_handler_t * fn)
{
  vnet_crypto_register_ops_handler_inline (vm, engine_index, opt, fn,
