...
vnet_crypto_key_alloc_per_thread (vlib_main_t *vm, u16 length)
{
  ...
  key = &cm->keys_arena_base[key_index];
...
vnet_crypto_key_del_obj (vlib_main_t *vm, vnet_crypto_key_t *key)
{
  vnet_crypto_main_t *cm = &crypto_main;
  vnet_crypto_engine_t *engine;
  u32 key_owner_thread = key->index / cm->keys_per_thread;

  vec_foreach (engine, cm->engines)
    if (engine->key_op_handler)
      engine->key_op_handler (VNET_CRYPTO_KEY_OP_DEL, key->index);
  ...
  if (key_owner_thread == vm->thread_index)
    {
      ...
      cm->keys_free_bitmap =
	clib_bitmap_set (cm->keys_free_bitmap, key->index, 1);

      log_debug ("key %u deleted by owner thread %u", key->index,
		 vm->thread_index);
    }
  else
    {
      /* Key is owned by another thread — NOT updating bitmap */
      /* Slot remains "leaked", but key->is_deleted=1 prevents reuse */
      log_debug (
	"key %u deleted by non-owner thread %u (owner=%u) - slot not freed",
	key->index, vm->thread_index, key_owner_thread);
    }
}
				...
				vnet_crypto_key_t *key_integ)
{
  ...
  if (PREDICT_FALSE (!key_crypto || !key_integ))
    return 0;

  if (PREDICT_FALSE (key_crypto->is_deleted || key_integ->is_deleted))
    return 0;

  alg = vnet_crypto_link_algs (key_crypto->alg, key_integ->alg);
  if (PREDICT_FALSE (alg == VNET_CRYPTO_ALG_NONE))
    return 0;

  key = vnet_crypto_key_alloc_per_thread (vm, /* length */ 0);
  ...
  key->is_link = 1;
  key->length = 0;
  key->index_crypto = key_crypto->index;
  key->index_integ = key_integ->index;
...
{
  vnet_crypto_key_t *key_crypto = vnet_crypto_get_key (index_crypto);
  vnet_crypto_key_t *key_integ = vnet_crypto_get_key (index_integ);
  vnet_crypto_key_t *key_linked =
    vnet_crypto_key_add_linked_obj (vm, key_crypto, key_integ);

  return key_linked ? key_linked->index : ~0;
}
...
vnet_crypto_init (vlib_main_t * vm)
{
  ...
  u32 arena_size = cm->keys_arena_max_keys * key_size;
