				...
				vnet_crypto_op_chunk_t * chunks, u32 n_ops)
{
  ...
  vnet_crypto_handler_type_t ht = chunks ? VNET_CRYPTO_HANDLER_TYPE_CHAINED :
...
u32
vnet_crypto_key_add_idx (vlib_main_t *vm, vnet_crypto_alg_t alg, u8 *data,
			 u16 length)
{
  vnet_crypto_main_t *cm = &crypto_main;
  ...
  vec_foreach (engine, cm->engines)
    if (engine->key_op_handler)
      engine->key_op_handler (VNET_CRYPTO_KEY_OP_ADD,
			      (void *) (uword) key->index,
...
vnet_crypto_key_del_idx (vlib_main_t *vm, vnet_crypto_key_index_t index)
{
  ...
  vec_foreach (engine, cm->engines)
    if (engine->key_op_handler)
      engine->key_op_handler (VNET_CRYPTO_KEY_OP_DEL, (void *) (uword) index,
			      VNET_CRYPTO_ALG_NONE, 0, 0);
...
vnet_crypto_key_update_idx (vlib_main_t *vm, vnet_crypto_key_index_t index)
{
  ...
  vec_foreach (engine, cm->engines)
    if (engine->key_op_handler)
      engine->key_op_handler (VNET_CRYPTO_KEY_OP_MODIFY,
			      (void *) (uword) index, VNET_CRYPTO_ALG_NONE, 0,
			      0);
}

u32
vnet_crypto_key_add_linked_idx (vlib_main_t *vm,
...
vnet_crypto_key_t *
vnet_crypto_key_add (vlib_main_t *vm, vnet_crypto_alg_t alg, const u8 *data,
		     u16 length)
{
  vnet_crypto_main_t *cm = &crypto_main;
  vnet_crypto_engine_t *engine;
  ...
  key = (vnet_crypto_key_t *) ((u8 *) cm->keys_data +
			       key_index * cm->data_per_key);
  ...
  vec_foreach (engine, cm->engines)
	...
	{
	  key->ctx_data[eidx] = (u8 *) cm->engine_keys_data[eidx] +
				key_index * cm->engine_data_per_key[eidx];
	  if (engine->key_op_handler)
	    engine->key_op_handler (VNET_CRYPTO_KEY_OP_ADD,
				    key->ctx_data[eidx], alg, data, length);
	}
      else
	key->ctx_data[eidx] = 0;
      eidx++;
  ...
  vec_foreach (engine, cm->engines)
      ...
      if (engine->key_op_handler)
	{
	  if (cm->algs[key->alg].is_link)
	    {
	      engine->key_op_handler (
		VNET_CRYPTO_KEY_OP_DEL, key->ctx_data[eidx],
	    ...
	    {
	      engine->key_op_handler (VNET_CRYPTO_KEY_OP_DEL,
				      key->ctx_data[eidx], key->alg, NULL, 0);
	    }
	}
      eidx++;
    }
...
void
vnet_crypto_key_update (vnet_crypto_key_t *key, vnet_crypto_alg_t alg,
			const u8 *data, u16 length)
{
  ...
  vec_foreach (engine, cm->engines)
    {
      if (engine->key_op_handler)
	engine->key_op_handler (VNET_CRYPTO_KEY_OP_MODIFY, key->ctx_data[eidx],
				alg, data, length);
      eidx++;
    }
}
			   ...
			   const u8 *integ_data, u16 length)
{
  ...
  vec_foreach (engine, cm->engines)
    {
      if (engine->key_op_handler)
	engine->key_op_handler (VNET_CRYPTO_KEY_OP_ADD, key->ctx_data[eidx],
				integ_alg, integ_data, length);
...
void *
vnet_crypto_get_active_engine_key_data (vnet_crypto_key_t *key,
					vnet_crypto_op_id_t opt,
      ...
      while ((entry = readdir (dp)))
	{
	  ...
	  if (r->per_thread_data_sz)
		...
		round_pow2 (r->per_thread_data_sz, CLIB_CACHE_LINE_BYTES);
	      u64 alloc = engine_key_alloc_sz * tm->n_vlib_mains *
		...
		clib_mem_alloc_aligned (alloc, CLIB_CACHE_LINE_BYTES);
	      ...
	      clib_warning ("%s: allocated %u bytes for all keys", r->name,
			    alloc);
	    }
	  if (r->extra_data_sz)
	    {
		...
		clib_mem_alloc_aligned (alloc, CLIB_CACHE_LINE_BYTES);
	      ...
	      clib_warning ("%s: allocated %u bytes for extra engine data",
			    r->name, alloc);
	    }
	  ...
	  cm->engine_data_per_key[eidx] =
	    engine_key_alloc_sz * tm->n_vlib_mains;
	}
...
vnet_crypto_init (vlib_main_t * vm)
{
  ...
  cm->data_per_key =
    round_pow2 (sizeof (vnet_crypto_key_t), CLIB_CACHE_LINE_BYTES);
  u32 keys_data_size = cm->keys_arena_max_keys * cm->data_per_key;

  cm->keys_data =
    clib_mem_alloc_aligned (keys_data_size, CLIB_CACHE_LINE_BYTES);
  if (!cm->keys_data)
  ...
  clib_warning ("crypto keys arena: %u bytes for %u keys (all free)",
		keys_data_size, cm->keys_arena_max_keys);
