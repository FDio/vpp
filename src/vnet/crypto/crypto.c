			      ...
			      vnet_crypto_handler_type_t t, uword per_thread_offset)
{
  ...
  return (vnet_crypto_key_data_t *) (key->data + key_data_offset + per_thread_offset);
}
			...
			u8 chained_per_thread)
{
  ...
  vnet_crypto_key_layout_compute ((uword) k->data, crypto_length, integ_length, simple_size,
			  ...
			  vnet_crypto_handler_type_t t)
{
  ...
  log_debug ("key-add type %u alg %s key-index %u fn %p per-thread %u key-data-sz %u", t,
	     cm->algs[key->alg].name, key->index, fn,
	     vnet_crypto_alg_key_data_is_per_thread (ad, t), ad->key_data_sz[t]);
...
u32
vnet_crypto_key_add (vlib_main_t *vm, vnet_crypto_alg_t alg, u8 *crypto_data, u16 crypto_length,
		     u8 *integ_data, u16 integ_length)
{
  ...
  u16 chained_stride = 0, chained_size = 0;
  u8 simple_per_thread = 0, chained_per_thread = 0;
  u16 length = crypto_length + integ_length;

  ASSERT (alg != 0);

  if (length == 0 || crypto_data == 0 || (integ_length && integ_data == 0))
    return ~0;
  ...
  clib_memcpy (key->data + key->cipher_key_offset, crypto_data, crypto_length);
  if (integ_length)
    clib_memcpy (key->data + key->integ_key_offset, integ_data, integ_length);
...
vnet_crypto_key_del (vlib_main_t * vm, vnet_crypto_key_index_t index)
{
  ...
  clib_memset_u8 (key->data, 0, key_data_size_total);
...
vnet_crypto_alg_t
vnet_crypto_combined_alg (vnet_crypto_alg_t crypto_alg, vnet_crypto_alg_t integ_alg)
{
#define _(c, h, s, k ,d) \
