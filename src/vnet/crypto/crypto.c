...
vnet_crypto_run (vlib_main_t * vm, vnet_crypto_op_t * ops[], u32 n_ops)
{
  ...
  for (i = 0; i < n_ops; i++)
	...
	rv += (cm->ops_handlers[qidx]) (vm, ops + i, 1);
      else
...
u32
vnet_crypto_register_provider (vlib_main_t * vm, char *name, int prio,
			       char *desc)
{
  vnet_crypto_main_t *cm = &crypto_main;
  vnet_crypto_provider_t *p;

  vec_add2 (cm->providers, p, 1);
  p->name = name;
  p->desc = desc;
  p->priority = prio;

  return p - cm->providers;
}
				  ...
				  vnet_crypto_ops_handler_t * fn)
{
  vnet_crypto_main_t *cm = &crypto_main;
  u32 qidx = vnet_crypto_queue_index (alg, op);
  vec_validate_aligned (cm->ops_handlers, VNET_CRYPTO_N_QUEUES - 1,
			CLIB_CACHE_LINE_BYTES);
