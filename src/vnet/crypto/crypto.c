				   ...
				   vnet_crypto_key_data_fn_t *key_del_fn)
{
  ...
  for (alg = 0; alg < VNET_CRYPTO_N_ALGS; alg++)
    {
      vnet_crypto_register_key_handler_for_alg (engine_index, alg, VNET_CRYPTO_HANDLER_TYPE_SIMPLE,
						key_add_fn, key_del_fn, 0, 0);
      vnet_crypto_register_key_handler_for_alg (engine_index, alg, VNET_CRYPTO_HANDLER_TYPE_CHAINED,
						key_add_fn, key_del_fn, 0, 0);
    }
}
			    ...
			    vnet_crypto_key_index_t index_integ)
{
  ...
  linked_len = key_crypto->length + key_integ->length;
  key = vnet_crypoto_key_alloc (linked_len);
  key->alg = linked_alg;
  clib_memcpy_fast (key->data, key_crypto->data, key_crypto->length);
  clib_memcpy_fast (key->data + key_crypto->length, key_integ->data, key_integ->length);
