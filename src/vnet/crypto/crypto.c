			...
			u32 n_jobs)
{
  ...
  while (n_jobs--)
	...
	{
	  n_enq =
	    vnet_crypto_enqueue_ops_call_handler (vm, cm, current_op_type,
						  first_job, n);
	  rv += n_enq;
...
int
vnet_crypto_set_op_mode (vlib_main_t * vm, char *alg_name,
			 vnet_crypto_mode_t mode)
{
  ...
  vlib_node_state_t old = VLIB_NODE_STATE_DISABLED;
  vlib_node_state_t new = VLIB_NODE_STATE_DISABLED;

  for (i = 0; i < VNET_CRYPTO_N_OP_IDS; i++)
    if (cm->ops_mode[i] == VNET_CRYPTO_MODE_ASYNC)
      {
	old = VLIB_NODE_STATE_POLLING;
	break;
      }
  ...
  for (i = skip_master; i < tm->n_vlib_mains; i++)
    vlib_node_set_state (vlib_mains[i], node->index, new);
