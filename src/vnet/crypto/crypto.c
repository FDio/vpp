...
u32
vnet_crypto_enq_jobs (vlib_main_t * vm, vnet_crypto_job_t ** jobs, u32 n_jobs)
{
  ...
  while (n_enq < n_jobs)
    {
      vnet_crypto_job_t *j = jobs[n_enq];
...
vlib_error_t *
vnet_crypto_register_job_handler (vlib_main_t * vm, vnet_crypto_alg_t alg,
				  vnet_crypto_op_t op,
				  vnet_crypto_job_handler_t * fn)
{
  vnet_crypto_main_t *cm = &crypto_main;
  u32 qidx = vnet_crypto_queue_index (alg, op);
  vec_validate_init_empty_aligned (cm->handlers, VNET_CRYPTO_N_QUEUES - 1,
