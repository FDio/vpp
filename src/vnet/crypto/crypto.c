...
static_always_inline void
crypto_set_active_async_engine (vnet_crypto_op_data_t *od,
				vnet_crypto_op_id_t id, u32 ei)
{
  vnet_crypto_main_t *cm = &crypto_main;
  vnet_crypto_engine_t *ce = vec_elt_at_index (cm->engines, ei);
  ...
  if (ce->ops[id].handlers[t] && ce->dequeue_handler)
    {
      od->active_engine_index[t] = ei;
      od->handlers[t] = ce->ops[id].handlers[t];
    }
}
...
int
vnet_crypto_is_set_async_handler (vnet_crypto_op_id_t op)
{
  vnet_crypto_main_t *cm = &crypto_main;

  return cm->opt_data[op].handlers[VNET_CRYPTO_HANDLER_TYPE_ASYNC] != 0;
}
