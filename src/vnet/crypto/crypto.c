...
vnet_crypto_key_alloc (u32 length, vnet_crypto_alg_t alg)
{
  ...
  u32 alloc_sz = sizeof (vnet_crypto_key_t);
...
vnet_crypto_key_add_inline (vnet_crypto_alg_t alg, const u8 *data, u16 length)
{
  ...
  u8 *ped = key->data + length;

  vnet_crypto_engine_t *e =
    vec_elt_at_index (cm->engines, ad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_SIMPLE]);
  if (e->key_op_handler)
    e->key_op_handler (VNET_CRYPTO_KEY_OP_ADD, ped, alg, key->data, length);
...
vnet_crypto_key_del_inline (vnet_crypto_key_t *key)
{
  ...
  vnet_crypto_engine_t *e =
    vec_elt_at_index (cm->engines, ad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_SIMPLE]);
  if (e->key_op_handler)
    e->key_op_handler (VNET_CRYPTO_KEY_OP_DEL, ped, key->alg, key->data, key->length);
...
vnet_crypto_key_update (vnet_crypto_key_t *key, const u8 *data)
{
  ...
  vnet_crypto_alg_data_t *ad = cm->algs + key->alg;
  u8 *ped = key->data + key->length;

  vnet_crypto_engine_t *e =
    vec_elt_at_index (cm->engines, ad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_SIMPLE]);
  if (e->key_op_handler)
    e->key_op_handler (VNET_CRYPTO_KEY_OP_MODIFY, ped, key->alg, key->data, key->length);
  if (ad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_CHAINED] !=
      ad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_SIMPLE])
    {
      ped += ad->per_engine_key_size;
			   ...
			   vnet_crypto_alg_t integ_alg, const u8 *integ_data, u16 integ_length)
{
  ...
  if (e->key_op_handler)
    e->key_op_handler (VNET_CRYPTO_KEY_OP_ADD, ped, linked_alg, key->data, integ_length);
  ...
  if (dp)
      ...
      while ((entry = readdir (dp)))
	{
	  ...
	  while (alg != VNET_CRYPTO_N_ALGS)
	    {
	      u16 key_data_size = r->key_data_sz[alg];
	      e->key_data_sz[alg] = r->key_data_sz[alg];

	      ad = cm->algs + alg;
	      if (key_data_size)
		{
		  u16 peks = round_pow2 (key_data_size * tm->n_vlib_mains, CLIB_CACHE_LINE_BYTES);
		  ad->per_engine_key_size = clib_max (ad->per_engine_key_size, peks);
		}
	      alg++;
	    }
