				   ...
				   vnet_crypto_handler_type_t t)
{
  ...
  for (i = 0; i < vec_len (cm->ctxs); i++)
	...
	{
	  if (vnet_crypto_ctx_get_engine (cm->ctxs[i], t) != old_ei)
	    continue;
	}
      else if (vnet_crypto_ctx_get_engine (cm->ctxs[i], t) != ei)
	continue;
...
static void
vnet_crypto_ctx_layout_init (vnet_crypto_main_t *cm)
{
  ...
  FOREACH_ARRAY_ELT (kl, cm->ctx_layout)
    {
      alg = kl - cm->ctx_layout;
      kl->cipher_key_len = cm->algs[alg].cipher_key_len;
      kl->auth_key_len = cm->algs[alg].auth_key_len;
...
vnet_crypto_register_post_node (vlib_main_t * vm, char *post_node_name)
{
  ...
  cc = vlib_get_node_by_name (vm, (u8 *) "crypto-deq");
  ...
  *p = 0;
  ...
  for (r = cm->engine_registrations; r; r = r->next)
      ...
      if (r->reg_op_groups)
	{
      ...
      if (r->reg_hash_groups)
	{
	  vnet_crypto_reg_hash_alg_group_t *rog = r->reg_hash_groups;
	  vnet_crypto_reg_hash_alg_group_t **best_groups = 0;

	  while (rog)
	    {
	      int p = rog->probe_fn ? rog->probe_fn () : 1;
	      vnet_crypto_reg_hash_alg_group_t **bg;
	      int found = 0;

	      log_debug ("engine %s hash-group %s probe=%d", r->name, rog->name, p);
	  ...
	  vec_foreach (bg, best_groups)
	    {
	      vnet_crypto_reg_hash_alg_t *ra = bg[0]->algs;

	      log_debug ("engine %s selected hash-group %s priority=%d", r->name, bg[0]->name,
			 bg[0]->priority);

	      while (ra)
		{
		  if (ra->simple_fn || ra->chained_fn)
		    {
		      log_debug ("register hash engine=%s group=%s alg=%U fn=%p cfn=%p", r->name,
				 bg[0]->name, format_vnet_crypto_hash_alg, ra->alg_id,
				 ra->simple_fn, ra->chained_fn);
		      vnet_crypto_register_hash_handler_inline (vm, engine, ra->alg_id,
								ra->simple_fn, ra->chained_fn);
		    }
...
vnet_crypto_init (vlib_main_t * vm)
{
  ...
  vec_foreach (ct, cm->threads)
    {
      ...
      clib_spinlock_init (&ct->completed_frames_lock);
  ...
  cm->crypto_node_index = vlib_get_node_by_name (vm, (u8 *) "crypto-deq")->index;
...
vnet_crypto_main_loop_enter (vlib_main_t *vm)
{
  ...
  vnet_crypto_ctx_layout_init (cm);
