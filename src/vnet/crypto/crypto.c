...
vnet_crypto_key_alloc (u32 length, vnet_crypto_alg_t alg)
{
  ...
  alloc_sz += active_engine_count * ad->per_engine_key_size;
...
vnet_crypto_key_add_inline (vnet_crypto_alg_t alg, const u8 *data, u16 length)
{
  ...
  vnet_crypto_engine_t *e =
    vec_elt_at_index (cm->engines, ad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_SIMPLE]);
  if (e->key_op_handler)
    {
      u8 *ptkd = ped;
      u8 *ptd_ctx = e->per_thread_data;
      for (u32 idx = 0; idx < tm->n_vlib_mains; idx++, ptkd += e->key_data_sz[key->alg])
	{
	  e->key_op_handler (VNET_CRYPTO_KEY_OP_ADD,
			     (vnet_crypto_key_handler_args_t){ .alg = key->alg,
							       .per_thread_ctx = ptd_ctx,
							       .per_thread_key_data = ptkd,
							       .key = key->data,
  ...
  if (ad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_CHAINED] !=
      ad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_SIMPLE])
    {
      ped += ad->per_engine_key_size;
      vnet_crypto_engine_t *e =
	vec_elt_at_index (cm->engines, ad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_CHAINED]);
      if (e->key_op_handler)
	{
	  u8 *ptkd = ped;
	  u8 *ptd_ctx = e->per_thread_data;
  ...
  if (e->key_op_handler)
    {
      ...
      for (u32 idx = 0; idx < tm->n_vlib_mains; idx++, ptkd += e->key_data_sz[key->alg])
	{
	  e->key_op_handler (VNET_CRYPTO_KEY_OP_DEL,
			     (vnet_crypto_key_handler_args_t){ .alg = key->alg,
							       .per_thread_ctx = ptd_ctx,
							       .per_thread_key_data = ptkd,
							       .key = key->data,
      ...
      clib_memset (ped, 0xfe, ad->per_engine_key_size);
    }
  ...
  if (e->key_op_handler)
    {
      ...
      for (u32 idx = 0; idx < tm->n_vlib_mains; idx++, ptkd += e->key_data_sz[key->alg])
	{
	  e->key_op_handler (VNET_CRYPTO_KEY_OP_MODIFY,
			     (vnet_crypto_key_handler_args_t){ .alg = key->alg,
							       .per_thread_ctx = ptd_ctx,
							       .per_thread_key_data = ptkd,
							       .key = key->data,
			   ...
			   vnet_crypto_alg_t integ_alg, const u8 *integ_data, u16 integ_length)
{
  ...
  vnet_crypto_engine_t *e =
    vec_elt_at_index (cm->engines, lad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_SIMPLE]);
  if (e->key_op_handler)
    {
      u8 *ptkd = ped;
      u8 *ptd_ctx = e->per_thread_data;
      for (u32 idx = 0; idx < tm->n_vlib_mains; idx++, ptkd += e->key_data_sz[linked_alg])
	{
	  e->key_op_handler (VNET_CRYPTO_KEY_OP_ADD,
			     (vnet_crypto_key_handler_args_t){ .alg = linked_alg,
							       .per_thread_ctx = ptd_ctx,
							       .per_thread_key_data = ptkd,
							       .key = key->data,
							       .key_length = integ_length });
	  if (ptd_ctx)
	    ptd_ctx += e->per_thread_data_sz;
	}
    }
  ...
  if (lad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_CHAINED] !=
    ...
    {
      ped += lad->per_engine_key_size;
      vnet_crypto_engine_t *e =
	vec_elt_at_index (cm->engines, lad->active_eidx[VNET_CRYPTO_HANDLER_TYPE_CHAINED]);
      if (e->key_op_handler)
	{
	  u8 *ptkd = ped;
	  u8 *ptd_ctx = e->per_thread_data;
      ...
      while ((entry = readdir (dp)))
	{
	  ...
	  while (alg != VNET_CRYPTO_N_ALGS)
	      ...
	      if (r->key_data_sz[alg])
		{
		  ...
		  ad->per_engine_key_size = clib_max (ad->per_engine_key_size, pek_sz);
		}
	      alg++;
	    }
