...
vnet_crypto_ctx_set_default_engine (vnet_crypto_ctx_t *ctx, vnet_crypto_handler_type_t t)
{
  ...
  vnet_crypto_ctx_set_engine (ctx, t, cm->active_engine_index[ctx->alg][t]);
}
...
vnet_crypto_key_alloc (u16 cipher_key_len, u16 auth_key_len, const vnet_crypto_key_layout_t *kl)
{
  ...
  tmpl.auth_key_offset = cipher_key_len;
...
vnet_crypto_ctx_create (vnet_crypto_alg_t alg)
{
  ...
  ctx = vnet_crypto_key_alloc (ad->cipher_key_len, ad->auth_key_len, cm->key_layout + alg);
  ctx->alg = alg;
