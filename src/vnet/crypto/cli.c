...
static u8 *
format_vnet_crypto_modes (u8 * s, va_list * args)
{
  vnet_crypto_alg_t alg = va_arg (*args, vnet_crypto_alg_t);
  vnet_crypto_main_t *cm = &crypto_main;
  vnet_crypto_alg_data_t *d = vec_elt_at_index (cm->algs, alg);
  ...
  for (i = 0; i < VNET_CRYPTO_OP_N_TYPES; i++)
	...
	continue;
      ...
      if (first == 0)
        s = format (s, "\n%U", format_white_space, indent);
      s = format (s, "%-20U%-20U", format_vnet_crypto_op_type, od->type,
		  format_vnet_crypto_async_status,
		  vnet_crypto_get_mode (id), s);
      first = 0;
    }
  return s;
}
  ...
  for (i = 0; i < VNET_CRYPTO_OP_N_TYPES; i++)
	...
	continue;
      ...
      if (first == 0)
        s = format (s, "\n%U", format_white_space, indent);
      s = format (s, "%-20U%-20U", format_vnet_crypto_op_type, od->type,
		  format_vnet_crypto_engine, od->active_async_engine_index, s);

      vec_foreach (e, cm->engines)
	{
	  if (e->enqueue_handler[id] != 0)
	    s = format (s, "%U ", format_vnet_crypto_engine, e - cm->engines);
	}
      first = 0;
    }
  ...
  if (all)
    {
      ...
      ({
        (void) value;
        rc += vnet_crypto_set_async_handler (key, engine);
      }));
      /* *INDENT-ON* */
  ...
  else
    {
      vec_foreach (arg, args)
      {
	rc = vnet_crypto_set_async_handler (arg[0], engine);
	if (rc)
	  {
	    vlib_cli_output (vm, "failed to set engine %s for %s!",
			     engine, arg[0]);
...
VLIB_CLI_COMMAND (set_crypto_async_handler_command, static) =
{
  ...
  .short_help = "set crypto async handler cipher [cipher2 cipher3 ...] engine",
				  ...
				  vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	on = VNET_CRYPTO_MODE_ASYNC;
      else if (unformat (line_input, "off"))
	on = VNET_CRYPTO_MODE_SYNC;
      else if (unformat (line_input, "%s", &s))
	vec_add1 (args, s);
      else
	{
  ...
  if (all)
    {
      ...
      ({
        (void) value;
        rc += vnet_crypto_set_op_mode (vm, key, (vnet_crypto_mode_t) on);
      }));
      /* *INDENT-ON* */
  ...
  else
    {
      vec_foreach (arg, args)
      {
	rc = vnet_crypto_set_op_mode (vm, arg[0], (vnet_crypto_mode_t) on);
	if (rc)
	  {
	    vlib_cli_output (vm, "failed to set async mode %s for %s!",
			     on ? "on" : "off", arg[0]);
	  }
      }
    }
...
static clib_error_t *
show_crypto_mode_fn (vlib_main_t * vm, unformat_input_t * input,
		     vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  int i;
  ...
  vlib_cli_output (vm, "%-20s%-20s%s", "Algo", "Type", "Mode");

  for (i = 0; i < VNET_CRYPTO_N_ALGS; i++)
    vlib_cli_output (vm, "%-20U%U", format_vnet_crypto_alg, i,
		     format_vnet_crypto_modes, i);

  return 0;
}

/* *INDENT-OFF* */
VLIB_CLI_COMMAND (show_crypto_mode_command, static) =
{
  .path = "show crypto modes",
  .short_help = "show crypto modes",
  .function = show_crypto_mode_fn,
};
/* *INDENT-ON* */
