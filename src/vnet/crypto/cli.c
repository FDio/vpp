...
static int
crypto_alg_name_cmp (const vnet_crypto_alg_t *a0, const vnet_crypto_alg_t *a1)
{
  vnet_crypto_main_t *cm = &crypto_main;

  return strcmp (cm->algs[*a0].name, cm->algs[*a1].name);
}
...
show_crypto_algorithms_table (vlib_main_t *vm)
{
  ...
  for (vnet_crypto_alg_t alg = 1; alg < VNET_CRYPTO_N_ALGS; alg++)
    vec_add1 (algs, alg);
  vec_sort_with_function (algs, crypto_alg_name_cmp);
		...
		vec_foreach (e, cm->engines)
		  {
			...
			if (vec_len (cell))
			  cell = format (cell, " ");
			if (e->ops[a - cm->algs][i].handlers[ht] ==
			    cm->op_type_handlers[a - cm->algs][i][ht])
