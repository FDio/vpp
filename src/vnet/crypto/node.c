  ...
  while (n_job >= 8)
    {
      ...
      if (PREDICT_FALSE (job[0]->status != VNET_CRYPTO_OP_STATUS_COMPLETED))
	  {
	    ...
	    vlib_node_increment_counter (vm, node->node_index, job[0]->status,
					 1);
	  }
	else
	  next[0] = job[0]->next;
      ...
      if (PREDICT_FALSE (job[1]->status != VNET_CRYPTO_OP_STATUS_COMPLETED))
	  {
	    ...
	    vlib_node_increment_counter (vm, node->node_index, job[1]->status,
					 1);
	  }
	else
	  next[1] = job[1]->next;
      ...
      if (PREDICT_FALSE (job[2]->status != VNET_CRYPTO_OP_STATUS_COMPLETED))
	  {
	    ...
	    vlib_node_increment_counter (vm, node->node_index, job[2]->status,
					 1);
	  }
	else
	  next[2] = job[2]->next;
      ...
      if (PREDICT_FALSE (job[3]->status != VNET_CRYPTO_OP_STATUS_COMPLETED))
	  {
	    ...
	    vlib_node_increment_counter (vm, node->node_index, job[3]->status,
					 1);
	  }
	else
	  next[3] = job[3]->next;
                                     ...
                                     vlib_frame_t * frame)
{
  ...
  vec_foreach_index (index, cm->async_ops)
  {
    ...
    vnet_crypto_post_process_ops (vm, node, odt, job, bi, next, n_deq);
