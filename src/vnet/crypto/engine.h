...
typedef union
{
  struct
  {
    vnet_crypto_sync_op_fn_t *enc_fn;
    vnet_crypto_sync_op_fn_t *dec_fn;
    vnet_crypto_sync_op_fn_t *hmac_fn;
  };
  vnet_crypto_sync_op_fn_t *fn[VNET_CRYPTO_OP_N_TYPES];
...
typedef struct vnet_crypto_reg_hash_alg
{
  struct vnet_crypto_reg_hash_alg *next;
  struct vnet_crypto_reg_hash_alg_group *group;
  vnet_crypto_hash_alg_t alg_id;
  vnet_crypto_hash_fn_t *simple_fn;
  vnet_crypto_hash_fn_t *chained_fn;
} vnet_crypto_reg_hash_alg_t;

typedef struct vnet_crypto_reg_hash_alg_group
{
  struct vnet_crypto_reg_hash_alg_group *next;
  const char *name;
  vnet_crypto_is_supported_fn_t *probe_fn;
  int priority;
  vnet_crypto_reg_hash_alg_t *algs;
} vnet_crypto_reg_hash_alg_group_t;
...
#define VNET_CRYPTO_REGISTER_ENGINE()                                                              \
...
#define VNET_CRYPTO_REGISTER_ALG_GROUP(x)                                                          \
...
#define VNET_CRYPTO_REGISTER_ALG(x)                                                                \
...
#define VNET_CRYPTO_REGISTER_HASH_ALG_GROUP(x)                                                     \
  static vnet_crypto_reg_hash_alg_group_t x;                                                       \
  static void __clib_constructor __vnet_crypto_reg_hash_group_ctor_##x (void)                      \
  {                                                                                                \
    if (x.name == 0)                                                                               \
      x.name = #x;                                                                                 \
    x.next = __vnet_crypto_engine.reg_hash_groups;                                                 \
    __vnet_crypto_engine.reg_hash_groups = &x;                                                     \
  }                                                                                                \
  static vnet_crypto_reg_hash_alg_group_t x

#define VNET_CRYPTO_REGISTER_HASH_ALG(x)                                                           \
  static vnet_crypto_reg_hash_alg_t x;                                                             \
  static void __clib_constructor __vnet_crypto_reg_hash_ctor_##x (void)                            \
  {                                                                                                \
    if (x.group == 0)                                                                              \
      return;                                                                                      \
    x.next = x.group->algs;                                                                        \
