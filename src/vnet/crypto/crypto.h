...
#define VNET_CRYPTO_OP_FLAG_ASYNC (1 << 2)
  u32 key_index;
  u32 len;
  u16 aad_len;
  union {
...
typedef void (vnet_crypto_op_free_t) (vlib_main_t *vm, vnet_crypto_op_t *op);
...
typedef struct
{
  ...
  u32 is_async[VNET_CRYPTO_N_OP_IDS];
...
int vnet_crypto_set_async_mode (char *ops_handler_name, u32 is_async);

int vnet_crypto_set_async_handler (char *ops_handler_name, char *engine);

u32 vnet_crypto_is_async_mode (vnet_crypto_op_id_t opt);
