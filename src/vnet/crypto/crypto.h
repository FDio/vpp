...
typedef struct
{
  vnet_crypto_key_t *crypto;
  vnet_crypto_key_t *integ;
} vnet_crypto_op_keys_t;
...
typedef struct
{
  ...
  void *keys_arena_base; /* fixed-size array */
  u32 keys_arena_max_keys;	      /* max key count */
  u32 data_per_key;          /* size of key + data */
...
void vnet_crypto_key_update (vnet_crypto_key_index_t index);
...
{
  vnet_crypto_main_t *cm = &crypto_main;
  return (vnet_crypto_key_t *)((u8 *)cm->keys_arena_base + index * cm->data_per_key);
}
