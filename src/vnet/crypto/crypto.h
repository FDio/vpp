...
#define VNECT_CRYPTO_FRAME_SIZE 32
...
#define VNET_CRYPTO_FRAME_STATE_ELT_ERROR 3
  ...
  u16 n_elts;
  vnet_crypto_async_frame_elt_t elts[VNECT_CRYPTO_FRAME_SIZE];
  u32 buffer_indices[VNECT_CRYPTO_FRAME_SIZE];
  u16 next_node_index[VNECT_CRYPTO_FRAME_SIZE];
...
vnet_crypto_async_add_to_frame (vlib_main_t * vm,
				vnet_crypto_async_frame_t ** frame,
				u32 key_index,
				u32 total_len, i16 crypto_start_offset,
				...
				u8 flags)
{
  ...
  if (PREDICT_FALSE (f->n_elts == VNECT_CRYPTO_FRAME_SIZE))
    {
