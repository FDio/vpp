...
{
  return ctx->_key + ctx->cipher_key_offset;
}

static_always_inline const u8 *
vnet_crypto_get_auth_key (const vnet_crypto_ctx_t *ctx)
{
  return ctx->_key + ctx->auth_key_offset;
}
...
vnet_crypto_key_get_engine (vnet_crypto_ctx_t *ctx, vnet_crypto_handler_type_t t)
{
...
typedef struct
{
  ...
  vnet_crypto_engine_id_t active_op_engine_index[VNET_CRYPTO_N_ALGS][VNET_CRYPTO_OP_N_TYPES]
						[VNET_CRYPTO_HANDLER_N_TYPES];
  void *op_type_handlers[VNET_CRYPTO_N_ALGS][VNET_CRYPTO_OP_N_TYPES][VNET_CRYPTO_HANDLER_N_TYPES];
...
vnet_crypto_ctx_t *vnet_crypto_ctx_create (vlib_main_t *vm, vnet_crypto_alg_t alg,
...
void vnet_crypto_key_update (vlib_main_t *vm, vnet_crypto_ctx_t *ctx);
...
vnet_crypto_alg_has_op_type (vnet_crypto_alg_t alg, vnet_crypto_op_type_t type)
{
  ...
  switch (alg_type)
    ...
    case VNET_CRYPTO_ALG_T_AUTH:
      return type == VNET_CRYPTO_OP_TYPE_HASH || type == VNET_CRYPTO_OP_TYPE_HMAC;
			  ...
			  uword per_thread_offset)
{
  ...
  key_data = ctx->_key + key_data_offset + per_thread_offset;
						 ...
						 vnet_crypto_async_frame_t *frame)
{
  ...
  if (engine == VNET_CRYPTO_ENGINE_ID_NONE)
    fn = cm->op_type_handlers[frame->alg][frame->type][VNET_CRYPTO_HANDLER_TYPE_ASYNC];
  else
    {
...
vnet_crypto_async_add_to_frame (vlib_main_t *vm, vnet_crypto_async_frame_t *f,
				vnet_crypto_ctx_t *ctx, u32 cipher_data_len, u32 auth_data_len,
				...
				u32 buffer_index, u16 next_node, u8 *iv, u8 *icv, u8 *aad, u8 flags)
{
  ...
  fe->icv = icv;
  fe->aad = aad;
  fe->aad_len = cm->algs[f->alg].aad_len;
  fe->icv_len = cm->algs[f->alg].auth_len;
  fe->flags = flags;
