...
{
  vnet_hw_if_rxq_poll_vector_t *dq1 = a1;
  vnet_hw_if_rxq_poll_vector_t *dq2 = a2;
...
vnet_hw_if_update_runtime_data (vnet_main_t * vnm, u32 hw_if_index)
{
  ...
  vnet_hw_if_rxq_poll_vector_t *pd, **d = 0;
  ...
  pool_foreach (rxq, im->hw_if_rx_queues, ({
    ...
    vec_add2_aligned (d[rxq->thread_index], pd, 1, CLIB_CACHE_LINE_BYTES);
    pd->dev_instance = hi->dev_instance;
    pd->queue_id = rxq->queue_id;
