...
vnet_hw_if_update_runtime_data (vnet_main_t * vnm, u32 hw_if_index)
{
  ...
  pool_foreach (rxq, im->hw_if_rx_queues, ({
    ...
    if (per_thread_node_state[rxq->thread_index] == VLIB_NODE_STATE_INTERRUPT)
      n_interrupts = rxq - im->hw_if_rx_queues;

    if (per_thread_node_state[rxq->thread_index] != VLIB_NODE_STATE_POLLING)
      continue;
  ...
  if (something_changed)
      ...
      for (int i = 0; i < n_threads; i++)
	{
	  ...
	  if (per_thread_node_state[i] == VLIB_NODE_STATE_INTERRUPT)
	    clib_interrupt_resize (&rt->rxq_interrupts, n_interrupts);
	  else
