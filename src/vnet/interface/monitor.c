			      ...
			      vlib_cli_command_t *cmd)
{
  ...
  while (refresh_count--)
		 ...
		 ts[spin] + refresh_interval - vlib_time_now (vm)) > 0.0))
	{
      ...
      vlib_cli_output (
	vm, "rx: %Upps %Ubps tx: %Upps %Ubps", 
	format_base10, (u32) ((vrx[spin].packets - vrx[spin ^ 1].packets) / tsd),
	format_base10, (u32) (8 * (vrx[spin].bytes - vrx[spin ^ 1].bytes) / tsd),
	format_base10, (u32) ((vtx[spin].packets - vtx[spin ^ 1].packets) / tsd),
	format_base10, (u32) (8 * (vtx[spin].bytes - vtx[spin ^ 1].bytes) / tsd));
    }
