...
void vnet_hw_if_txq_update_runtime_data (vnet_main_t *vnm, u32 hw_if_index);
...
static_always_inline u32
vnet_hw_if_get_tx_queue_id (vlib_main_t *vm, vlib_node_runtime_t *node,
			    vlib_frame_t *frame)
{
  u32 qid;
  if (frame->flags & VNET_HW_TXQ_INDEX_SET)
  ...
  else
    {
      ...
      ASSERT (rt->thread_index == vm->thread_index);
      qid = rt->txq_index;
    }
