...
static clib_error_t *
set_interface_link_speed (vlib_main_t *vm, unformat_input_t *input,
			  vlib_cli_command_t *cmd)
{
  vnet_main_t *vnm = vnet_get_main ();
  u32 hw_if_index = ~0;
...
static clib_error_t *
show_interface_link_speed_capa (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
  vnet_main_t *vnm = vnet_get_main ();
  u32 hw_if_index = ~0;
  ...
  if (hw->supported_link_speeds == 0)
    {
      vlib_cli_output (vm, "no speed capabilities reported for %U",
		       format_vnet_hw_if_index_name, vnm, hw_if_index);
      return 0;
    }

  vlib_cli_output (vm, "Supported speeds for %U:",
		   format_vnet_hw_if_index_name, vnm, hw_if_index);
