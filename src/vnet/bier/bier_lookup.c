             ...
             vlib_frame_t * from_frame)
{
    ...
    while (n_left_from > 0)
    {
        ...
        while (n_left_from > 0 && n_left_to_next > 0)
        {
            ...
            {
                /*
                 * we can only 'clone' a buffer 255 times, so do so in chunks
                 * of 256 where we copy the original and then clone it.
                 */
                while (n_clones > 256)
                {
                    vlib_buffer_t *copy;
                    copy = vlib_buffer_copy (vm, b0);

                    num_cloned +=
                        vlib_buffer_clone(vm,
                ...
                num_cloned +=
                    vlib_buffer_clone(vm, bi0,
                                      blm->blm_clones[thread_index] + num_cloned,
                                      n_clones - 1,
                                      n_bytes + 8);
