...
static uword
bier_lookup (vlib_main_t *vm, vlib_node_runtime_t *node,
	     vlib_frame_t *from_frame)
{
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  for (index = 0; index < num_buckets; index++)
	      ...
	      while (bucket)
		{
		  ...
		  fbs +=
		    (((num_buckets - 1) - index) * BIER_BIT_MASK_BITS_PER_INT);
		  ...
		  if (PREDICT_TRUE (INDEX_INVALID != bfmi0))
			...
			vm, node->node_index, BIER_LOOKUP_ERROR_FMASK_UNRES,
			1);
		      bucket = ((int *) bbs.bbs_buckets)[index];
		      continue;
		    }
	  ...
	  if (PREDICT_TRUE (0 != n_clones))
	    {
	      ...
	      num_cloned =
		vlib_buffer_clone (vm, bi0, blm->blm_clones[thread_index],
  ...
  vlib_node_increment_counter (vm, bier_lookup_node.index,
			       BIER_LOOKUP_ERROR_NONE, from_frame->n_vectors);
...
format_bier_lookup_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "BIER: next [%d], tbl:%d BFM:%d", t->next_index, t->bt_index,
	      t->bfm_index);
  return s;
}
