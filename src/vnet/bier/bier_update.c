...
clib_error_t *
vnet_bier_table_cmd (vlib_main_t *vm, unformat_input_t *input,
...
VLIB_CLI_COMMAND (bier_table_command) = {
  .path = "bier table",
  .short_help = "bier table [add|del] sd <sub-domain> set <SET> bsl "
		"<bit-string-length> [mpls <label>]",
  .function = vnet_bier_table_cmd,
};

clib_error_t *
vnet_bier_route_cmd (vlib_main_t *vm, unformat_input_t *input,
		     vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      else if (unformat (input, "bp %d", &bp))
	{
      ...
      else if (unformat (input, "via %U", unformat_fib_route_path, &brp,
			 &payload_proto))
	{
	}
      else
...
static clib_error_t *
show_bier_fib_command_fn (vlib_main_t *vm, unformat_input_t *input,
			  vlib_cli_command_t *cmd)
{
