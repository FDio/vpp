...
bier_entry_link_walk (fib_node_index_t pl_index, fib_node_index_t path_index,
		      void *arg)
{
  bier_entry_t *be = arg;
  index_t bfmi;
...
static fib_path_list_walk_rc_t
bier_entry_unlink_walk (fib_node_index_t pl_index, fib_node_index_t path_index,
			void *arg)
{
  bier_entry_t *be = arg;
  index_t bfmi;
...
index_t
bier_entry_create (index_t bti, bier_bp_t bp)
{
  bier_entry_t *be;
...
static void
bier_entry_table_ecmp_walk_add_fmask (index_t btei, void *arg)
{
  bier_entry_t *be = arg;
  ;
  ...
  if (FIB_NODE_INDEX_INVALID != be->be_path_list)
	...
	be->be_path_list, FIB_FORW_CHAIN_TYPE_BIER,
      ...
      if (dpo.dpoi_type == DPO_LOAD_BALANCE)
	{
	  ...
	  choice = load_balance_get_bucket_i (
	    lb, btid->bti_ecmp & (lb->lb_n_buckets_minus_1));
	}
      else
	{
	  choice = &dpo;
  ...
  if (FIB_NODE_INDEX_INVALID != be->be_path_list)
    {
      ...
      be->be_path_list = FIB_NODE_INDEX_INVALID;
      bier_table_ecmp_walk (be->be_bti, bier_entry_table_ecmp_walk_add_fmask,
			    be);
    }
...
void
bier_entry_path_add (index_t bei, const fib_route_path_t *rpaths)
{
  ...
  if (FIB_NODE_INDEX_INVALID == be->be_path_list)
	...
	(FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
      be->be_sibling_index = fib_path_list_child_add (
	be->be_path_list, FIB_NODE_TYPE_BIER_ENTRY, bier_entry_get_index (be));
    }
  else
    {
      ...
      be->be_path_list = fib_path_list_copy_and_path_add (
	old_pl_index, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF),
	rpaths);
...
void
bier_entry_path_update (index_t bei, const fib_route_path_t *rpaths)
{
  fib_node_index_t old_pl_index;
  bier_entry_t *be;
...
int
bier_entry_path_remove (index_t bei, const fib_route_path_t *rpaths)
{
  fib_node_index_t old_pl_index;
  bier_entry_t *be;
  ...
  be->be_path_list = fib_path_list_copy_and_path_remove (
    old_pl_index, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF),
    rpaths);
  ...
  if (be->be_path_list != old_pl_index)
      ...
      if (FIB_NODE_INDEX_INVALID != be->be_path_list)
	{
	  ...
	  fib_path_list_walk (be->be_path_list, bier_entry_link_walk, be);
	  be->be_sibling_index = fib_path_list_child_add (
	    be->be_path_list, FIB_NODE_TYPE_BIER_ENTRY,
...
void
bier_entry_contribute_forwarding (index_t bei, dpo_id_t *dpo)
{
...
bier_entry_get_from_node (fib_node_t *node)
{
  return ((bier_entry_t *) (((char *) node) -
			    STRUCT_OFFSET_OF (bier_entry_t, be_node)));
}
...
bier_entry_back_walk_notify (fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
  /*
