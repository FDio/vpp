...
bier_disp_entry_restack (bier_disp_entry_t *bde, bier_hdr_proto_id_t pproto)
{
  ...
  dpo_stack (DPO_BIER_DISP_ENTRY, DPO_PROTO_BIER,
	     &bde->bde_fwd[pproto].bde_dpo, &via_dpo);
}

void
bier_disp_entry_path_add (index_t bdei, bier_hdr_proto_id_t pproto,
			  const fib_route_path_t *rpaths)
{
  ...
  if (FIB_NODE_INDEX_INVALID == *pli)
    {
      *pli = fib_path_list_create (
	(FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
    }
  else
    {
      *pli = fib_path_list_copy_and_path_add (
	old_pli, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF),
	rpaths);
    }
...
int
bier_disp_entry_path_remove (index_t bdei, bier_hdr_proto_id_t pproto,
			     const fib_route_path_t *rpaths)
{
  fib_node_index_t *pli, old_pli;
  bier_disp_entry_t *bde;
  ...
  if (FIB_NODE_INDEX_INVALID != *pli)
    {
      *pli = fib_path_list_copy_and_path_remove (
	old_pli, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF),
	rpaths);
...
format_bier_disp_entry (u8 *s, va_list *args)
{
  ...
  FOR_EACH_BIER_HDR_PROTO (pproto)
  {
	...
	s = format (s, "%U", format_fib_path_list, bde->bde_pl[pproto],
		    indent + 4);

	if (flags & BIER_SHOW_DETAIL)
	  {
	    s = format (s, "\n%UForwarding:", format_white_space, indent + 4);
	    s = format (s, "\n%Urpf-id:%d", format_white_space, indent + 6,
			bde->bde_fwd[pproto].bde_rpf_id);
	    s = format (s, "\n%U%U", format_white_space, indent + 6,
			format_dpo_id, &bde->bde_fwd[pproto].bde_dpo,
			indent + 2);
	  }
      }
  }
...
{
  fib_show_memory_usage ("BIER disposition", pool_elts (bier_disp_entry_pool),
...
format_bier_disp_entry_dpo (u8 *s, va_list *ap)
{
  ...
  s =
    format (s, "%U", format_bier_disp_entry, index, indent, BIER_SHOW_DETAIL);
...
{
  dpo_register (DPO_BIER_DISP_ENTRY, &bier_disp_entry_vft,
...
static clib_error_t *
show_bier_disp_entry (vlib_main_t *vm, unformat_input_t *input,
