...
bier_disp_entry_add_or_lock (void)
{
    ...
    /*
     * memset to 0 makes then all invalid
     */
    bde->bde_locks = 0;
    ...
    FOR_EACH_BIER_HDR_PROTO(pproto)
    {
        if (INDEX_INVALID != bde->bde_pl[pproto])
        {
            s = format(s, "\n%U%U",
                       ...
                       format_dpo_id, &bde->bde_dpo[pproto], indent+2);
...
/*     { */
/*         pool_foreach(bi, bier_disp_entry_pool, */
/*         ({ */
/*             vlib_cli_output(vm, "%U", format_bier_disp_entry, */
/*                             bier_disp_entry_get_index(bi), */
/*                             1, */
/*                             BIER_SHOW_BRIEF); */
/*         })); */
/*     } */
...
/*     .path = "show bier disp", */
/*     .short_help = "show bier disp [index]", */
