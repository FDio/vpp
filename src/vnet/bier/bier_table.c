...
bier_table_mk_key (const bier_table_id_t *id)
{
   ...
   * for later
   */
  u32 key = ((id->bti_sub_domain << 24) | (id->bti_set << 16) |
	     (id->bti_ecmp << 8) | (id->bti_hdr_len << 4) | (id->bti_type));
...
bier_table_init (bier_table_t *bt, const bier_table_id_t *id, mpls_label_t ll)
{
  ...
  if (bier_table_is_main (bt))
    {
      vec_validate_init_empty_aligned (bt->bt_entries, num_entries,
				       INDEX_INVALID, CLIB_CACHE_LINE_BYTES);
    }
  else
    {
      vec_validate_init_empty_aligned (bt->bt_fmasks, num_entries,
				       INDEX_INVALID, CLIB_CACHE_LINE_BYTES);
    }
}
...
bier_table_rm_bift (bier_table_t *bt)
{
  ...
  bier_bift_table_entry_remove (bier_bift_id_encode (
    bt->bt_id.bti_set, bt->bt_id.bti_sub_domain, bt->bt_id.bti_hdr_len));
}
...
bier_table_mk_lfib (bier_table_t *bt)
{
  ...
  if (MPLS_LABEL_INVALID != bt->bt_ll)
	...
	FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID, FIB_SOURCE_BIER);

      /*
       * stack the entry on the forwarding chain produced by the
       * path-list via the ECMP tables.
       */
      fib_path_list_contribute_forwarding (bt->bt_pl, FIB_FORW_CHAIN_TYPE_BIER,
					   FIB_PATH_LIST_FWD_FLAG_COLLAPSE,
					   &dpo);

      mpls_fib_index =
	fib_table_find (FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID);
      bt->bt_lfei = fib_table_entry_special_dpo_add (
	mpls_fib_index, &pfx, FIB_SOURCE_BIER, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
      dpo_reset (&dpo);
    }
}
...
{
  fib_show_memory_usage ("BIER-table", pool_elts (bier_table_pool),
			 pool_len (bier_table_pool), sizeof (bier_table_t));
}
static u8 *
...
void
bier_table_route_path_update_i (const bier_table_id_t *btid, bier_bp_t bp,
				fib_route_path_t *brps, u8 is_replace)
{
...
void
bier_table_route_path_update (const bier_table_id_t *btid, bier_bp_t bp,
			      fib_route_path_t *brps)
{
  bier_table_route_path_update_i (btid, bp, brps, 1);
}
void
bier_table_route_path_add (const bier_table_id_t *btid, bier_bp_t bp,
			   fib_route_path_t *brps)
{
...
void
bier_table_route_path_remove (const bier_table_id_t *btid, bier_bp_t bp,
			      fib_route_path_t *brps)
{
...
format_bier_table (u8 *s, va_list *ap)
{
  ...
  s = format (s, "[@%d] bier-table:[%U local-label:%U", bti,
	      format_bier_table_id, &bt->bt_id, format_mpls_unicast_label,
	      bt->bt_ll);
...
void
bier_table_walk (const bier_table_id_t *bti, bier_table_walk_fn_t fn,
		 void *ctx)
{
  bier_table_t *bt;
  bier_entry_t *be;
