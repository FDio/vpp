...
void
bier_table_delete (const bier_table_id_t *bti)
{
    uword *p;
    u32 key;
...
{
    return (bier_table_create(btid, MPLS_LABEL_INVALID));
}
                      ...
                      const fib_route_path_t *brp)
{
    ...
    bfmi = bier_fmask_db_find_or_create_and_lock(bier_table_get_index(bt),
                                                 &fmid,
                                                 brp->frp_label_stack[0]);

    /* convert to a vector */
    vec_add1(rpaths, *brp);
    rpaths[0].frp_bier_fib_index = bti;
    ...
    bier_entry_path_add(bei, rpaths);
                         ...
                         const fib_route_path_t *brp)
{
    ...
    vec_add1(rpaths, *brp);
    rpaths[0].frp_bier_fib_index = bier_table_get_index(bt);

    if (0 == bier_entry_path_remove(bei, rpaths))
    {
        /* 0 remaining paths */
