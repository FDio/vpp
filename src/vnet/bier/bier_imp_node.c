...
always_inline uword
bier_imp_dpo_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
...
{
  return (bier_imp_dpo_inline (vm, node, frame, FIB_PROTOCOL_IP4,
			       BIER_HDR_PROTO_IPV4));
}

VLIB_REGISTER_NODE (
  bier_imp_ip4_node) = { .name = "bier-imp-ip4",
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (bier_imp_dpo_inline (vm, node, frame, FIB_PROTOCOL_IP6,
			       BIER_HDR_PROTO_IPV6));
}

VLIB_REGISTER_NODE (
  bier_imp_ip6_node) = { .name = "bier-imp-ip6",
			 .vector_size = sizeof (u32),
