...
const static u16 bier_hdr_len_num_buckets[] = {
  [BIER_HDR_LEN_INVALID] = 0, [BIER_HDR_LEN_64] = 8,
  [BIER_HDR_LEN_128] = 16,    [BIER_HDR_LEN_256] = 32,
  [BIER_HDR_LEN_512] = 64,    [BIER_HDR_LEN_1024] = 128,
  [BIER_HDR_LEN_2048] = 256,  [BIER_HDR_LEN_4096] = 512,
};

const static u16 bier_hdr_len_num_bits[] = {
  [BIER_HDR_LEN_INVALID] = 0, [BIER_HDR_LEN_64] = 64,
  [BIER_HDR_LEN_128] = 128,   [BIER_HDR_LEN_256] = 256,
  [BIER_HDR_LEN_512] = 512,   [BIER_HDR_LEN_1024] = 1024,
  [BIER_HDR_LEN_2048] = 2048, [BIER_HDR_LEN_4096] = 4096,
};

const static u16 bier_hdr_len_prefix_len[] = {
  [BIER_HDR_LEN_INVALID] = 0, [BIER_HDR_LEN_64] = 7,
  [BIER_HDR_LEN_128] = 8,     [BIER_HDR_LEN_256] = 9,
  [BIER_HDR_LEN_512] = 10,    [BIER_HDR_LEN_1024] = 11,
  [BIER_HDR_LEN_2048] = 12,   [BIER_HDR_LEN_4096] = 13,
};
...
{
  bier_hdr_len_id_t hli =
    va_arg (*ap, int); // int promotion of bier_hdr_len_id_t
...
bier_bift_id_t
bier_bift_id_encode (bier_table_set_id_t set, bier_table_sub_domain_id_t sd,
...
void
bier_bift_id_decode (bier_bift_id_t id, bier_table_set_id_t *set,
		     bier_table_sub_domain_id_t *sd, bier_hdr_len_id_t *bsl)
{
  *set = id & 0xff;
  id = id >> 8;
  *sd = id & 0xff;
...
format_bier_table_id (u8 *s, va_list *ap)
{
  ...
  return (format (s, "sub-domain:%d set:%d ecmp:%d bsl:%U",
		  btid->bti_sub_domain, btid->bti_set, btid->bti_ecmp,
...
format_bier_hdr (u8 *s, va_list *ap)
{
  ...
  return (
    format (s, "nibble:%d version:%d hdr-len:%U entropy:%d proto:%U src:%d",
	    bier_hdr_get_1st_nibble (&copy), bier_hdr_get_version (&copy),
	    format_bier_hdr_len_id, bier_hdr_get_len_id (&copy),
	    bier_hdr_get_entropy (&copy), format_bier_hdr_proto,
	    bier_hdr_get_proto_id (&copy), bier_hdr_get_src_id (&copy)));
}
...
format_bier_bift_id (u8 *s, va_list *ap)
{
  ...
  return (format (s, "0x%x -> set:%d sd:%d hdr-len:%U", id, set, sd,
