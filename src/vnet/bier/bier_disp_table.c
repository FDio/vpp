...
void
bier_disp_table_contribute_forwarding (index_t bdti, dpo_id_t *dpo)
{
...
format_bier_disp_table (u8 *s, va_list *ap)
{
  ...
  s = format (s, "bier-disp-table:[%d]; table-id:%d locks:%d", bdti,
	      bdt->bdt_table_id, bdt->bdt_locks);
	  ...
	  if (INDEX_INVALID != bdt->bdt_db[ii])
	    {
	      ...
	      s = format (s, "\n%U", format_bier_disp_entry, bdt->bdt_db[ii],
...
format_bier_disp_table_dpo (u8 *s, va_list *ap)
{
  ...
  return (
    format (s, "%U", format_bier_disp_table, bdti, indent, BIER_SHOW_BRIEF));
}

static void
bier_disp_table_entry_insert (index_t bdti, bier_bp_t src, index_t bdei)
{
  bier_disp_table_t *bdt;
...
static void
bier_disp_table_entry_remove (index_t bdti, bier_bp_t src)
{
  bier_disp_table_t *bdt;
...
static index_t
bier_disp_table_lookup_hton (index_t bdti, bier_bp_t bp)
{
...
void
bier_disp_table_entry_path_add (u32 table_id, bier_bp_t src,
...
void
bier_disp_table_walk (u32 table_id, bier_disp_table_walk_fn_t fn, void *ctx)
{
...
bier_disp_table_dpo_mem_show (void)
{
  fib_show_memory_usage (
    "BIER disposition table", pool_elts (bier_disp_table_pool),
    pool_len (bier_disp_table_pool), sizeof (bier_disp_table_t));
}
...
const static char *const bier_disp_table_bier_nodes[] = { "bier-disp-lookup",
							  NULL };
...
{
  dpo_register (DPO_BIER_DISP_TABLE, &bier_disp_table_dpo_vft,
...
static clib_error_t *
show_bier_disp_table (vlib_main_t *vm, unformat_input_t *input,
  ...
  if (INDEX_INVALID == bdti)
      ...
      pool_foreach (bdt, bier_disp_table_pool)
	{
	  vlib_cli_output (vm, "%U", format_bier_disp_table,
			   bier_disp_table_get_index (bdt), 0,
			   BIER_SHOW_BRIEF);
	}
    }
  else
