 ...
 */
#define BIER_BIT_MASK_MAX_POS (256)
#define BIER_BIT_MASK_MIN_POS (1)

/**
 * How may buckets of 4 bytes are used to show a BitString.
 * Today the maximum is 4 bytes per 8 buckets = 256 bits.
 */
#define BIER_FORMAT_BUCKET_MAX  8
#define BIER_FORMAT_BSL_MAX     256
...
/**
 * The parts of the BIER header that follow the BitString field
 */
#define BIER_HEADER_END                         \
    /**                                         \
     * OAM, reserved and Proto                  \
     */                                         \
    u16 bh_oam_res_proto;                       \
                                                \
    /**                                         \
     * The BIER SRC ID (the BFIR)               \
     */                                         \
    bier_hdr_src_id_t bh_src_id;                \


/**
 * A BIER header of variable length
 * The encoding follows:
 *   https://tools.ietf.org/html/draft-ietf-bier-mpls-encapsulation-06
 */
typedef struct bier_hdr_t_ {
    /**
     * The first nibble is always set to all 0101
