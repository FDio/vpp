...
#define BIER_HDR_LEN_IDS                                                      \
  {                                                                           \
    [BIER_HDR_LEN_INVALID] = "invalid", [BIER_HDR_LEN_64] = "64",             \
...
#define FOR_EACH_BIER_HDR_LEN(_len)                                           \
  for (_item = BIER_HDR_LEN_64; _item <= BIER_HDR_LEN_4096; _item++)
...
#define BIER_HDR_PROTO_ID_NAMES                                               \
  {                                                                           \
    ...
    [BIER_HDR_PROTO_ETHERNET] = "ethernet", [BIER_HDR_PROTO_IPV4] = "ipv4",   \
    [BIER_HDR_PROTO_IPV6] = "ipv6", [BIER_HDR_PROTO_VXLAN] = "vxlan",         \
...
#define FOR_EACH_BIER_HDR_PROTO(_proto)                                       \
  for (_proto = BIER_HDR_PROTO_MPLS_DOWN_STREAM;                              \
       _proto <= BIER_HDR_PROTO_OAM; _proto++)
...
extern bier_hdr_proto_id_t
bier_bift_id_get_bit_string_length (bier_bift_id_t bift_id);
...
extern void bier_bift_id_decode (bier_bift_id_t id, bier_table_set_id_t *set,
				 bier_table_sub_domain_id_t *sd,
				 bier_hdr_len_id_t *bsl);
