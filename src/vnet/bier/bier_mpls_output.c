...
static char * bier_mpls_output_error_strings[] = {
 ...
 */
#define foreach_bier_mpls_output_next                \
_(DROP, "bier-drop")

typedef enum {
#define _(s,n) BIER_MPLS_OUTPUT_NEXT_##s,
  foreach_bier_mpls_output_next
#undef _
  BIER_MPLS_OUTPUT_N_NEXT,
} bier_mpls_output_next_t;

typedef enum {
#define bier_error(n,s) BIER_MPLS_OUTPUT_ERROR_##n,
#include <vnet/bier/bier_output_error.def>
#undef bier_error
  BIER_MPLS_OUTPUT_N_ERROR,
} bier_mpls_output_error_t;

/**
 * Forward declaration
 */
vlib_node_registration_t bier_mpls_output_node;

/**
 * @brief Packet trace recoed for a BIER output
 */
typedef struct bier_mpls_output_trace_t_
{
    u32 next_index;
    index_t bfm_index;
} bier_mpls_output_trace_t;

static uword
bier_mpls_output (vlib_main_t * vm,
                  ...
                  vlib_frame_t * from_frame)
{
    ...
    vlib_node_increment_counter (vm, bier_mpls_output_node.index,
                                 BIER_MPLS_OUTPUT_ERROR_NONE,
                                 from_frame->n_vectors);
    return (from_frame->n_vectors);
}

static u8 *
format_bier_mpls_output_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  bier_mpls_output_trace_t * t = va_arg (*args, bier_mpls_output_trace_t *);
...
VLIB_REGISTER_NODE (bier_mpls_output_node) = {
    .function = bier_mpls_output,
    .name = "bier-mpls-output",
    /* Takes a vector of packets. */
    .vector_size = sizeof (u32),

    .n_errors = BIER_MPLS_OUTPUT_N_ERROR,
    .error_strings = bier_mpls_output_error_strings,

    .n_next_nodes = BIER_MPLS_OUTPUT_N_NEXT,
    .next_nodes = {
        [BIER_MPLS_OUTPUT_NEXT_DROP] = "bier-drop",
    },

    .format_trace = format_bier_mpls_output_trace,
};
