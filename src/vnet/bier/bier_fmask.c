...
static void
bier_fmask_bits_init (bier_fmask_bits_t *bits, bier_hdr_len_id_t hlid)
{
  bits->bfmb_refs = clib_mem_alloc (sizeof (bits->bfmb_refs[0]) *
				    bier_hdr_len_id_to_num_bits (hlid));
  clib_memset (
    bits->bfmb_refs, 0,
    (sizeof (bits->bfmb_refs[0]) * bier_hdr_len_id_to_num_bits (hlid)));
   ...
   */
  bits->bfmb_input_reset_string.bbs_buckets = clib_mem_alloc_aligned (
    sizeof (bits->bfmb_input_reset_string.bbs_buckets[0]) *
      bier_hdr_len_id_to_num_buckets (hlid),
    CLIB_CACHE_LINE_BYTES);
  clib_memset (bits->bfmb_input_reset_string.bbs_buckets, 0,
	       sizeof (bits->bfmb_input_reset_string.bbs_buckets[0]) *
		 bier_hdr_len_id_to_num_buckets (hlid));
}
...
bier_fmask_stack (bier_fmask_t *bfm)
{
  ...
  fib_path_list_contribute_forwarding (
    bfm->bfm_pl, fct, FIB_PATH_LIST_FWD_FLAG_COLLAPSE, &via_dpo);
...
void
bier_fmask_contribute_forwarding (index_t bfmi, dpo_id_t *dpo)
{
  bier_fmask_t *bfm;
...
u32
bier_fmask_child_add (fib_node_index_t bfmi, fib_node_type_t child_type,
		      fib_node_index_t child_index)
{
  return (fib_node_child_add (FIB_NODE_TYPE_BIER_FMASK, bfmi, child_type,
			      child_index));
};

void
bier_fmask_child_remove (fib_node_index_t bfmi, u32 sibling_index)
{
...
static void
bier_fmask_init (bier_fmask_t *bfm, const bier_fmask_id_t *fmid,
  ...
  *bfm->bfm_id = *fmid;
  ...
  if (!(bfm->bfm_flags & BIER_FMASK_FLAG_DISP))
      ...
      else
	{
	   ...
	   */
	  id = bier_bift_id_encode (btid->bti_set, btid->bti_sub_domain,
				    btid->bti_hdr_len);
  ...
  bfm->bfm_pl = fib_path_list_create (
    (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
  bfm->bfm_sibling = fib_path_list_child_add (
    bfm->bfm_pl, FIB_NODE_TYPE_BIER_FMASK, bier_fmask_get_index (bfm));
  vec_free (rpaths);
...
void
bier_fmask_link (index_t bfmi, bier_bp_t bp)
{
  bier_fmask_t *bfm;
...
void
bier_fmask_unlink (index_t bfmi, bier_bp_t bp)
{
  bier_fmask_t *bfm;
...
format_bier_fmask (u8 *s, va_list *ap)
{
  ...
  s = format (s, "fmask: nh:%U bs:%U locks:%d ", format_ip46_address,
	      &bfm->bfm_id->bfmi_nh, IP46_TYPE_ANY, format_bier_bit_string,
	      &bfm->bfm_bits.bfmb_input_reset_string, bfm->bfm_node.fn_locks);
  s = format (s, "flags:");
...
void
bier_fmask_encode (index_t bfmi, bier_table_id_t *btid,
...
bier_fmask_get_from_node (fib_node_t *node)
{
  return ((bier_fmask_t *) (((char *) node) -
			    STRUCT_OFFSET_OF (bier_fmask_t, bfm_node)));
}
...
bier_fmask_back_walk_notify (fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
  /*
...
{
  fib_show_memory_usage ("BIER-fmask", pool_elts (bier_fmask_pool),
			 pool_len (bier_fmask_pool), sizeof (bier_fmask_t));
}
...
static clib_error_t *
bier_fmask_show (vlib_main_t *vm, unformat_input_t *input,
		 vlib_cli_command_t *cmd)
{
  ...
  if (INDEX_INVALID == bfmi)
	...
	{
	  vlib_cli_output (vm, "[@%d] %U", bier_fmask_get_index (bfm),
