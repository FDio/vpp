...
bier_fmask_resolve (bier_fmask_t *bfm)
{
    if (bfm->bfm_flags & BIER_FMASK_FLAG_DISP)
    {
        bfm->bfm_disp = bier_disp_table_add_or_lock(bfm->bfm_disp_tbl_id);
    }
    else
    {
        /*
...
bier_fmask_unresolve (bier_fmask_t *bfm)
{
    ...
    {
        /*
         * un source a recursive route through which we resolve.
         */
        fib_prefix_t pfx = {
            .fp_addr = bfm->bfm_id.bfmi_nh,
            .fp_proto = (ip46_address_is_ip4(&(bfm->bfm_id.bfmi_nh)) ?
                 ...
                 const fib_route_path_t *rpath)
{
    ...
    {
        bfm->bfm_disp_tbl_id = rpath->frp_bier_fib_index;
    }
...
format_bier_fmask (u8 *s, va_list *ap)
{
    ...
    s = format(s, "\n%U%U",
               ...
               format_dpo_id, &bfm->bfm_dpo, indent+4);
