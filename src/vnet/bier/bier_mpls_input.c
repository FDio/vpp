...
typedef enum {
#define bier_error(n,s) BIER_MPLS_INPUT_ERROR_##n,
#include <vnet/bier/bier_mpls_input_error.def>
#undef bier_error
  BIER_MPLS_INPUT_N_ERROR,
} bier_mpls_input_error_t;
...
vlib_node_registration_t bier_mpls_input_node;

/**
 * @brief Packet trace recoed for a BIER output
 */
typedef struct bier_mpls_input_trace_t_
{
    u32 next_index;
    u32 bt_index;
} bier_mpls_input_trace_t;
...
static uword
bier_mpls_input (vlib_main_t * vm,
                 ...
                 vlib_frame_t * from_frame)
{
    ...
    while (n_left_from > 0)
    {
        ...
        while (n_left_from > 0 && n_left_to_next > 0)
        {
            ...
            if (PREDICT_TRUE(bier_hdr_validate(bh0, bt0->bt_id.bti_hdr_len)))
            {
                ...
                b0->error = node->errors[BIER_MPLS_INPUT_ERROR_INVALID_HEADER];
            }

            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED))
            {
                bier_mpls_input_trace_t *tr;
    ...
    vlib_node_increment_counter (vm, bier_mpls_input_node.index,
                                 BIER_MPLS_INPUT_ERROR_PKTS_VALID,
                                 from_frame->n_vectors);
    return (from_frame->n_vectors);
}

static u8 *
format_bier_mpls_input_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  bier_mpls_input_trace_t * t = va_arg (*args, bier_mpls_input_trace_t *);
...
VLIB_REGISTER_NODE (bier_mpls_input_node) = {
  .function = bier_mpls_input,
  .name = "bier-mpls-input",
  /* Takes a vector of packets. */
  .vector_size = sizeof (u32),

  .n_errors = BIER_MPLS_INPUT_N_ERROR,
  .error_strings = bier_error_strings,

  .n_next_nodes = BIER_INPUT_N_NEXT,
  .next_nodes = {
        [BIER_INPUT_NEXT_BIER_LOOKUP] = "bier-mpls-lookup",
        [BIER_INPUT_NEXT_DROP] = "bier-drop",
  },

  .format_trace = format_bier_mpls_input_trace,
};
