...
index_t
bier_imp_add_or_lock (const bier_table_id_t *bti, bier_bp_t sender,
		      const bier_bit_string_t *bs)
{
  ...
  FOR_EACH_FIB_IP_PROTOCOL (fproto)
  {
    ...
    dpo_stack (DPO_BIER_IMP, fib_proto_to_dpo (fproto), &bi->bi_dpo[fproto],
	       &dpo);
    dpo_reset (&dpo);
  }
  ...
  if (0 == bi->bi_locks)
    {
      ...
      FOR_EACH_FIB_IP_PROTOCOL (fproto) { dpo_reset (&bi->bi_dpo[fproto]); }
...
format_bier_imp (u8 *s, va_list *args)
{
  ...
  s = format (s, "bier-imp:[%d]: tbl:[%U] hdr:[%U]", bier_imp_get_index (bi),
	      format_bier_table_id, &bi->bi_tbl, format_bier_hdr, &bi->bi_hdr);
...
{
  fib_show_memory_usage ("BIER imposition", pool_elts (bier_imp_pool),
			 pool_len (bier_imp_pool), sizeof (bier_imp_t));
}
...
static clib_error_t *
show_bier_imp (vlib_main_t *vm, unformat_input_t *input,
	       vlib_cli_command_t *cmd)
{
  ...
  if (INDEX_INVALID == bii)
	...
	{
	  vlib_cli_output (vm, "%U", format_bier_imp, bier_imp_get_index (bi),
			   1, BIER_SHOW_BRIEF);
	}
    }
  else
