...
static uword
bier_output (vlib_main_t *vm, vlib_node_runtime_t *node,
	     vlib_frame_t *from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  vlib_increment_combined_counter (
	    cm, thread_index, bfmi0, 1, vlib_buffer_length_in_chain (vm, b0));
	  ...
	  bier_bit_string_logical_and_string (
	    &bfm0->bfm_bits.bfmb_input_reset_string, &bbs);
  ...
  vlib_node_increment_counter (vm, bier_output_node.index,
			       BIER_OUTPUT_ERROR_NONE, from_frame->n_vectors);
...
format_bier_output_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, " next [%d], BFM index %d label:%x", t->next_index,
	      t->bfm_index, t->bfm_label);
  return s;
}
