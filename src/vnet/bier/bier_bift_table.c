...
void
bier_bift_table_entry_add (bier_bift_id_t id, const dpo_id_t *dpo)
{
  if (NULL == bier_bift_table)
      ...
      for (ii = 0; ii < BIER_BIFT_N_ENTRIES; ii++)
	{
       ...
       * UDP port
       */
      udp_register_dst_port (vlib_get_main (), UDP_DST_PORT_BIER,
			     bier_bift_input_node.index, 0);
      udp_register_dst_port (vlib_get_main (), UDP_DST_PORT_BIER,
			     bier_bift_input_node.index, 1);
    }

  dpo_stack_from_node (bier_bift_input_node.index,
		       &bier_bift_table->bblt_dpos[id], dpo);
...
static uword
bier_bift_input (vlib_main_t *vm, vlib_node_runtime_t *node,
...
clib_error_t *
show_bier_bift_cmd (vlib_main_t *vm, unformat_input_t *input,
		    vlib_cli_command_t *cmd)
{
  ...
  if (~0 == set)
	  ...
	  if (!dpo_is_drop (&bier_bift_table->bblt_dpos[ii]))
	    {
	      ...
	      vlib_cli_output (vm, "set: %d, sub-domain:%d, BSL:%U", set,
			       sub_domain, format_bier_hdr_len_id, bsl);
	      vlib_cli_output (vm, "  %U", format_dpo_id,
	...
	{
	  vlib_cli_output (vm, "set: %d, sub-domain:%d, BSL:%U", set,
			   sub_domain, format_bier_hdr_len_id, hdr_len);
	  vlib_cli_output (vm, "  %U", format_dpo_id,
