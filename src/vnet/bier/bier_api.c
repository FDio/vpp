...
static void
send_bier_table_details (vl_api_registration_t *reg, u32 context,
...
static void
send_bier_route_details (const bier_table_t *bt, const bier_entry_t *be,
			 void *args)
{
  ...
  fib_path_list_walk_w_ext (be->be_path_list, NULL, fib_path_encode,
			    &path_ctx);
...
static void
send_bier_imp_details (vl_api_registration_t *reg, u32 context,
		       const bier_imp_t *bi)
{
...
static void
send_bier_disp_table_details (vl_api_registration_t *reg, u32 context,
...
vl_api_bier_disp_entry_add_del_t_handler (vl_api_bier_disp_entry_add_del_t *mp)
{
  ...
  vec_foreach_index (ii, brps)
	...
	{
	  clib_memcpy (&brp->frp_addr.ip4, &mp->bde_paths[ii].nh.address.ip4,
		       sizeof (brp->frp_addr.ip4));
	}
      else if (FIB_API_PATH_NH_PROTO_IP6 == mp->bde_paths[ii].proto)
	{
	  clib_memcpy (&brp->frp_addr.ip6, &mp->bde_paths[ii].nh.address.ip6,
  ...
  if (mp->bde_is_add)
    {
      bier_disp_table_entry_path_add (table_id, bp, mp->bde_payload_proto,
				      brps);
    }
  else
    {
      bier_disp_table_entry_path_remove (table_id, bp, mp->bde_payload_proto,
					 brps);
    }
...
send_bier_disp_entry_details (const bier_disp_table_t *bdt,
			      const bier_disp_entry_t *bde, u16 bp, void *args)
{
...
vl_api_bier_disp_entry_dump_t_handler (vl_api_bier_disp_entry_dump_t *mp)
{
  ...
  bier_disp_table_walk (ntohl (mp->bde_tbl_id), send_bier_disp_entry_details,
			&ctx);
}
