...
vnet_policer_get_l2_overhead (vnet_main_t *vnm, u32 sw_if_index, vlib_dir_t dir, u8 is_l2)
{
  /* L2 input/output policers don't need adjustment, they have an ethernet header
   * L3 output runs after ip-rewrite, which has prepended the ethernet header.
   * L3 input runs after ip-unicast, which has stripped the ethernet header.
   */
  if (is_l2 || dir == VLIB_TX)
    return 0;
...
static inline uword
vnet_policer_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame,
		     vlib_dir_t dir, u8 is_l2)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  act0 = vnet_policer_police (vm, b0, pi0, time_in_policer_periods, POLICE_CONFORM, true,
				      vnet_policer_get_l2_overhead (vnm, sw_if_index0, dir, is_l2));

	  act1 = vnet_policer_police (vm, b1, pi1, time_in_policer_periods, POLICE_CONFORM, true,
				      vnet_policer_get_l2_overhead (vnm, sw_if_index1, dir, is_l2));
...
{
  return vnet_policer_inline (vm, node, frame, VLIB_RX, 0);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return vnet_policer_inline (vm, node, frame, VLIB_RX, 1);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return vnet_policer_inline (vm, node, frame, VLIB_TX, 0);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return vnet_policer_inline (vm, node, frame, VLIB_TX, 1);
}
	  ...
	  if (PREDICT_TRUE (table_index0 != ~0))
	    {
	      ...
	      if (e0)
		{
		  act0 = vnet_policer_police (vm, b0, e0->next_index, time_in_policer_periods,
	      ...
	      else
		{
		  while (1)
			...
			{
			  act0 =
			    vnet_policer_police (vm, b0, e0->next_index, time_in_policer_periods,
						 e0->opaque_index, false, 0);
			  if (PREDICT_FALSE (act0 == QOS_ACTION_DROP))
			    {
			      next0 = POLICER_CLASSIFY_NEXT_INDEX_DROP;
