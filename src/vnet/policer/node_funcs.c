...
vnet_policer_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		     vlib_frame_t *frame, u8 police_input)
{
	  ...
	  if (police_input)
	    {
	      ...
	      pi0 = pm->policer_index_by_sw_if_index_rx[sw_if_index0];
	      pi1 = pm->policer_index_by_sw_if_index_rx[sw_if_index1];
	    }
	  else
	    {
	      ...
	      pi0 = pm->policer_index_by_sw_if_index_tx[sw_if_index0];
	      pi1 = pm->policer_index_by_sw_if_index_tx[sw_if_index1];
	    }
...
{
  return vnet_policer_inline (vm, node, frame, 1);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return vnet_policer_inline (vm, node, frame, 0);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return policer_handoff (vm, node, frame, vnet_policer_main.fq_index_rx, ~0);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return policer_handoff (vm, node, frame, vnet_policer_main.fq_index_tx, ~0);
}
