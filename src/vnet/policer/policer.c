			 ...
			 unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "%U", unformat_vnet_sw_interface, vnm, &interface_index))
	;
      else
	{
	  error = clib_error_return (0, "unknown input `%U'",
  ...
  if (~0 != interface_index)
	...
	{
	  vlib_cli_output (vm, "Interface %u has no %s policer applied", interface_index,
			   (dir == VLIB_RX) ? "input" : "output");
	  goto done;
	}
      policer_index = pm->policer_index_by_sw_if_index[dir][interface_index];
      vlib_cli_output (vm, "Interface %u, %s policer index %u", interface_index,
		       (dir == VLIB_RX) ? "input" : "output", policer_index);
    }
...
VLIB_CLI_COMMAND (show_policer_command, static) = {
  .path = "show policer",
  .short_help =
    "show policer [name <name> | index <index> | [input|output] <interface>|sw_if_index <sw_idx>]",
