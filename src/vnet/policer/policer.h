...
int policer_add (vlib_main_t *vm, u8 *name, qos_pol_cfg_params_st *cfg,
		 u32 *policer_index);

int policer_update (vlib_main_t *vm, u8 *name, qos_pol_cfg_params_st *cfg);
int policer_del (vlib_main_t *vm, u8 *name);
int policer_reset (vlib_main_t *vm, u8 *name);
int policer_bind_worker (u8 *name, u32 worker, bool bind);
int policer_input (u8 *name, u32 sw_if_index, vlib_dir_t dir, bool apply);
int policer_name_from_index (u32 policer_index, u8 **name);
