			...
			int is_echo)
{
  ...
  if (is_echo)
      ...
      if (!(rv = bfd_udp_get_echo_src_ip6 (&headers->ip6.src_address)))
	{
      ...
      _clib_memcpy (&headers->ip6.dst_address, &key->local_addr.ip6,
    ...
    {
      _clib_memcpy (&headers->ip6.src_address, &key->local_addr.ip6,
		    sizeof (headers->ip6.src_address));
      _clib_memcpy (&headers->ip6.dst_address, &key->peer_addr.ip6,
		    sizeof (headers->ip6.dst_address));
  ...
  while (n_left_from > 0)
      ...
      if (b0->flags & VLIB_BUFFER_IS_TRACED)
	{
	  ...
	  len = (b0->current_length < sizeof (t0->data)) ? b0->current_length
	    : sizeof (t0->data);
	  t0->len = len;
	  _clib_memcpy (t0->data, vlib_buffer_get_current (b0), len);
	}
