...
format_bfd_auth_key (u8 *s, va_list *args)
{
  ...
  if (key)
    {
      s = format (s, "{auth-type=%u:%s, conf-key-id=%u, use-count=%u}, ", key->auth_type,
		  bfd_auth_type_str (key->auth_type), key->conf_key_id, key->use_count);
    }
  else
    {
      s = format (s, "{none}");
...
static void
bfd_set_state (vlib_main_t *vm, bfd_main_t *bm, bfd_session_t *bs, bfd_state_e new_state,
	       int handling_wakeup)
{
  if (bs->local_state != new_state)
    {
      BFD_DBG ("Change state, bs_idx=%d: %s->%s", bs->bs_idx, bfd_state_string (bs->local_state),
...
bfd_calc_next_tx (bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
  if (bs->local_detect_mult > 1)
    {
      ...
      bs->tx_timeout_nsec =
	bs->last_tx_nsec + (1 - .25 * (random_f64 (&bm->random_seed))) * bs->transmit_interval_nsec;
      if (bs->tx_timeout_nsec < now)
	{
	  ...
	  BFD_DBG ("Missed %lu transmit events (now is %lu, calc "
		   "tx_timeout is %lu)",
		   (now - bs->tx_timeout_nsec) / bs->transmit_interval_nsec, now,
		   bs->tx_timeout_nsec);
  ...
  else
    {
      ...
      bs->tx_timeout_nsec = bs->last_tx_nsec + (.9 - .15 * (random_f64 (&bm->random_seed))) *
  ...
  if (bs->tx_timeout_nsec)
    {
      BFD_DBG ("Next transmit in %lu nsec/%.02fs@%lu", bs->tx_timeout_nsec - now,
	       (bs->tx_timeout_nsec - now) * SEC_PER_NSEC, bs->tx_timeout_nsec);
    }
}
  ...
  if (bs->echo_tx_timeout_nsec < now)
    {
      ...
      BFD_DBG ("Missed %lu echo transmit events (now is %lu, calc tx_timeout "
	       "is %lu)",
	       (now - bs->echo_tx_timeout_nsec) / bs->echo_transmit_interval_nsec, now,
	       bs->echo_tx_timeout_nsec);
      bs->echo_tx_timeout_nsec = now;
    }
  BFD_DBG ("Next echo transmit in %lu nsec/%.02fs@%lu", bs->echo_tx_timeout_nsec - now,
	   (bs->echo_tx_timeout_nsec - now) * SEC_PER_NSEC, bs->echo_tx_timeout_nsec);
}
...
bfd_recalc_detection_time (bfd_session_t *bs)
{
  if (bs->local_state == BFD_STATE_init || bs->local_state == BFD_STATE_up)
	...
	clib_max (bs->effective_required_min_rx_nsec, bs->remote_desired_min_tx_nsec);
      BFD_DBG ("Recalculated detection time %lu nsec/%.3fs", bs->detection_time_nsec,
	       bs->detection_time_nsec * SEC_PER_NSEC);
    }
}

static void
bfd_set_timer (bfd_main_t *bm, bfd_session_t *bs, u64 now, int handling_wakeup)
{
  u64 next = 0;
  u64 rx_timeout = 0;
  u64 tx_timeout = 0;
  ...
  if (BFD_STATE_up != bs->local_state || (!bs->remote_demand && bs->remote_min_rx_usec) ||
  ...
  if (next)
    {
      ...
      u32 wheel_time_ticks = (bs->event_time_nsec - now) / bm->nsec_per_tw_tick +
			     ((bs->event_time_nsec - now) % bm->nsec_per_tw_tick != 0);
      BFD_DBG ("event_time_nsec %lu (%lu nsec/%.3fs in future) -> "
	       "wheel_time_ticks %u",
	       bs->event_time_nsec, bs->event_time_nsec - now,
      ...
      else
	{
	  ...
	  BFD_DBG ("tw_timer_start(%p, %u, 0, %lu) == %u;", &bm->wheel, bs->bs_idx,
		   wheel_time_ticks);
	}
      ...
      if (!handling_wakeup)
	{
	  ...
	  send_signal =
	     ...
	     * avoid the expense of sending the event. The 2x multiplier is to workaround the race
	     * whereby simultaneous event + expired timer create one recurring bogus wakeup/suspend
	     * instance, due to double scheduling of the node on the pending list.
	     */
      ...
      if (send_signal)
	{
	  vlib_process_signal_event_mt (bm->vlib_main, bm->bfd_process_node_index,
...
static void
bfd_set_effective_desired_min_tx (bfd_main_t *bm, bfd_session_t *bs, u64 now,
...
static void
bfd_set_remote_required_min_rx (bfd_session_t *bs, u32 remote_required_min_rx_usec)
{
...
static void
bfd_set_remote_required_min_echo_rx (bfd_session_t *bs, u32 remote_required_min_echo_rx_usec)
{
 ...
 *
 * The bridge payload deliberately contains only the fields used by vnet
...
bfd_publish_vnet_event (bfd_listen_event_e event, const bfd_session_t *bs)
{
  ...
  bfd_vnet_bridge_publish (event, &payload);
}
...
bfd_session_start (bfd_main_t *bm, bfd_session_t *bs)
{
  ...
  bfd_recalc_tx_interval (bs);
  vlib_process_signal_event (bm->vlib_main, bm->bfd_process_node_index, BFD_EVENT_NEW_SESSION,
			     bs->bs_idx);
  ...
  if (len > STRUCT_SIZE_OF (bfd_pkt_t, head))
    {
      s = format (s,
		  ...
		  bfd_diag_code_string (bfd_pkt_get_diag_code (pkt)), bfd_pkt_get_state (pkt),
		  bfd_state_string (bfd_pkt_get_state (pkt)), bfd_pkt_get_poll (pkt),
		  bfd_pkt_get_final (pkt), bfd_pkt_get_control_plane_independent (pkt),
      ...
      if (len >= sizeof (bfd_pkt_t) && pkt->head.length >= sizeof (bfd_pkt_t))
	{
	  ...
	  pkt->head.length >= sizeof (bfd_pkt_with_common_auth_t) && bfd_pkt_get_auth_present (pkt))
	{
...
/**
 * RPC payload used to re-read a BFD session on the main thread before notifying
 * vnet listeners.
 */
typedef struct
{
 ...
 * stale bfd_session_t pointer to the bridge.
 */
static void
 ...
 *
 * Existing BFD API events and the vnet bridge notifications are both preserved.
 * Worker-originated changes use RPC so consumers continue to observe updates on
 * the main thread.
 */
static void
bfd_on_state_change (bfd_main_t *bm, bfd_session_t *bs, u64 now, int handling_wakeup)
{
  ...
  switch (bs->local_state)
	...
	bm, bs, now, clib_max (bs->config_desired_min_tx_nsec, bm->default_desired_min_tx_nsec));
      bfd_set_effective_required_min_rx (bs, bs->config_required_min_rx_nsec);
...
static int
bfd_transport_control_frame (vlib_main_t *vm, vlib_node_runtime_t *rt, u32 bi, bfd_session_t *bs)
{
  switch (bs->transport)
    {
    case BFD_TRANSPORT_UDP4:
...
static int
bfd_transport_echo (vlib_main_t *vm, vlib_node_runtime_t *rt, u32 bi, bfd_session_t *bs)
{
  switch (bs->transport)
    {
    case BFD_TRANSPORT_UDP4:
...
static void
bfd_add_sha1_auth_section (vlib_main_t *vm, vlib_buffer_t *b, bfd_session_t *bs)
{
...
bfd_init_control_frame (bfd_session_t *bs, vlib_buffer_t *b)
{
  ...
  if (bs->echo)
    {
      pkt->req_min_rx =
	clib_host_to_net_u32 (bfd_nsec_to_usec (bs->effective_required_min_rx_nsec));
    }
  else
    {
...
static void
bfd_process_trace_buf (vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_buffer_t *b,
		       bfd_session_t *bs)
{
...
static void
bfd_send_echo (vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
  ...
  if (now >= bs->echo_tx_timeout_nsec)
      ...
      if (vlib_buffer_alloc (vm, &bi, 1) != 1)
	{
      ...
      pkt->checksum =
	bfd_calc_echo_checksum (bs->local_discr, pkt->expire_time_nsec, bs->echo_secret);
    ...
    {
      BFD_DBG ("No need to send echo packet now, now is %lu, tx_timeout is %lu", now,
	       bs->echo_tx_timeout_nsec);
    }
}

static void
bfd_send_periodic (vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs,
		   u64 now)
{
  ...
  if (now >= bs->tx_timeout_nsec)
    ...
    {
      BFD_DBG ("No need to send control frame now, now is %lu, tx_timeout is %lu", now,
	       bs->tx_timeout_nsec);
    }
}

void
bfd_init_final_control_frame (vlib_main_t *vm, vlib_buffer_t *b, bfd_session_t *bs)
{
...
static void
bfd_check_rx_timeout (vlib_main_t *vm, bfd_main_t *bm, bfd_session_t *bs, u64 now,
		      int handling_wakeup)
{
  if (bs->last_rx_nsec + bs->detection_time_nsec <= now)
    {
      ...
      bfd_set_remote_required_min_rx (bs, 1);
	   ...
	   bs->echo_last_rx_nsec + bs->echo_transmit_interval_nsec * bs->local_detect_mult <= now)
    {
...
void
bfd_on_timeout (vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs,
		u64 now)
{
  ...
  switch (bs->local_state)
      ...
      if (BFD_POLL_NOT_NEEDED == bs->poll_state && !bs->echo && bfd_is_echo_possible (bs))
	{
	  ...
	  bs->echo_tx_timeout_nsec = now;
	  bfd_set_effective_required_min_rx (bs, clib_max (bm->min_required_min_rx_while_echo_nsec,
 ...
 */
static uword
bfd_process (vlib_main_t *vm, vlib_node_runtime_t *rt, CLIB_UNUSED (vlib_frame_t *f))
{
  bfd_main_t *bm = &bfd_main;
  u32 *expired = 0;
  ...
  while (1)
      ...
      if (pool_elts (bm->sessions))
	{
	  ...
	  if (!first_expires_in_ticks)
	    ...
	    {
	      BFD_DBG ("tw_timer_first_expires_in_ticks(%p) returns %luticks", &bm->wheel,
...
static clib_error_t *
bfd_sw_interface_up_down (CLIB_UNUSED (vnet_main_t *vnm), CLIB_UNUSED (u32 sw_if_index), u32 flags)
{
  // bfd_main_t *bm = &bfd_main;
...
static clib_error_t *
bfd_hw_interface_up_down (CLIB_UNUSED (vnet_main_t *vnm), CLIB_UNUSED (u32 hw_if_index), u32 flags)
{
  // bfd_main_t *bm = &bfd_main;
...
bfd_verify_pkt_common (const bfd_pkt_t *pkt)
{
  ...
  if (pkt->head.length < sizeof (bfd_pkt_t) ||
      (bfd_pkt_get_auth_present (pkt) && pkt->head.length < sizeof (bfd_pkt_with_common_auth_t)))
    {
...
{
  BFD_DBG ("Switching authentication key from %U to %U for bs_idx=%u", format_bfd_auth_key,
	   bs->auth.curr_key, format_bfd_auth_key, bs->auth.next_key, bs->bs_idx);
...
static int
bfd_verify_pkt_auth_seq_num (vlib_main_t *vm, bfd_session_t *bs, u32 received_seq_num,
			     int is_meticulous)
{
  /*
   * RFC 5880 6.8.1:
  ...
  if (now - bs->last_rx_nsec > bs->detection_time_nsec * 2)
    {
      BFD_DBG ("BFD peer unresponsive for %lu nsec, which is > 2 * "
	       ...
	       "flag",
	       now - bs->last_rx_nsec, bs->detection_time_nsec * 2);
      bs->auth.remote_seq_number_known = 0;
    }
  if (bs->auth.remote_seq_number_known)
      ...
      if (bs->auth.remote_seq_number > max_u32 - 3 * bs->local_detect_mult)
	{
	  ...
	  if (received_seq_num > x && received_seq_num < bs->auth.remote_seq_number + is_meticulous)
	    {
	      BFD_ERR ("Recvd sequence number=%u out of ranges <0, %u>, <%u, %u>", received_seq_num,
		       x, bs->auth.remote_seq_number + is_meticulous, max_u32);
	      return 0;
	    }
	}
      else
	{
	  ...
	  if (received_seq_num < min || received_seq_num > max)
	    {
	      BFD_ERR ("Recvd sequence number=%u out of range <%u, %u>", received_seq_num, min,
		       max);
	      return 0;
	    }
	}
...
static int
bfd_verify_pkt_auth_key_sha1 (vlib_main_t *vm, const bfd_pkt_t *pkt, u32 pkt_size,
			      CLIB_UNUSED (bfd_session_t *bs), u8 bfd_key_id,
			      bfd_auth_key_t *auth_key)
{
  ...
  if (with_common->common_auth.type != auth_key->auth_type)
    {
      BFD_ERR ("BFD auth type mismatch, packet auth=%d:%s doesn't match "
	       "in-use auth=%d:%s",
	       with_common->common_auth.type, bfd_auth_type_str (with_common->common_auth.type),
  ...
  if (pkt_size < sizeof (*with_sha1) ||
    ...
    {
      BFD_ERR ("BFD size mismatch, payload size=%u, expected=%u, auth_len=%u, "
	       "expected=%u",
	       pkt_size, sizeof (*with_sha1), with_sha1->sha1_auth.type_len.len,
  ...
  BFD_ERR ("SHA1 hash: %U doesn't match the expected value: %U", format_hex_bytes, hash_from_packet,
	   sizeof (hash_from_packet), format_hex_bytes, calculated_hash, sizeof (calculated_hash));
  return 0;
}

static int
bfd_verify_pkt_auth_key (vlib_main_t *vm, const bfd_pkt_t *pkt, u32 pkt_size, bfd_session_t *bs,
			 u8 bfd_key_id, bfd_auth_key_t *auth_key)
{
  ...
  switch (auth_key->auth_type)
    {
    case BFD_AUTH_TYPE_reserved:
      vlib_log_err (bm->log_class, "internal error, unexpected auth_type=%d:%s",
		    auth_key->auth_type, bfd_auth_type_str (auth_key->auth_type));
      return 0;
    case BFD_AUTH_TYPE_simple_password:
      /* fallthrough */
    case BFD_AUTH_TYPE_keyed_md5:
      /* fallthrough */
    case BFD_AUTH_TYPE_meticulous_keyed_md5:
      vlib_log_err (bm->log_class, "internal error, not implemented, unexpected auth_type=%d:%s",
		    auth_key->auth_type, bfd_auth_type_str (auth_key->auth_type));
      return 0;
    case BFD_AUTH_TYPE_keyed_sha1:
      /* fallthrough */
	...
	{
	  const u32 seq_num =
	    clib_net_to_host_u32 (((bfd_pkt_with_sha1_auth_t *) pkt)->sha1_auth.seq_num);
 ...
 */
int
bfd_verify_pkt_auth (vlib_main_t *vm, const bfd_pkt_t *pkt, u16 pkt_size, bfd_session_t *bs)
{
  if (bfd_pkt_get_auth_present (pkt))
	  ...
	  if (bs->auth.is_delayed && bs->auth.next_key)
	    {
	      ...
	      if (bfd_verify_pkt_auth_key (vm, pkt, pkt_size, bs, bs->auth.next_bfd_key_id,
					   bs->auth.next_key))
		{
      ...
      else
	{
	  ...
	  if (bfd_verify_pkt_auth_key (vm, pkt, pkt_size, bs, bs->auth.curr_bfd_key_id,
				       bs->auth.curr_key))
	    {
...
bfd_error_t
bfd_consume_pkt (vlib_main_t *vm, bfd_main_t *bm, const bfd_pkt_t *pkt, u32 bs_idx)
{
  bfd_lock_check (bm);
  ...
  if (bfd_pkt_get_auth_present (pkt))
	...
	case BFD_AUTH_TYPE_keyed_md5:
	  /* fallthrough */
	case BFD_AUTH_TYPE_meticulous_keyed_md5:
	  vlib_log_crit (bm->log_class, "internal error, unexpected auth_type=%d:%s", auth_type,
  ...
  if (bfd_pkt_get_final (pkt))
      ...
      if (BFD_POLL_IN_PROGRESS == bs->poll_state)
	{
	  ...
	  if (BFD_STATE_up == bs->local_state)
		...
		bs, clib_max (bs->echo * bm->min_required_min_rx_while_echo_nsec,
			      bs->config_required_min_rx_nsec));
	    }
	}
...
format_bfd_session (u8 *s, va_list *args)
{
  ...
  s = format (s,
	      ...
	      "      curr-key=%U\n"
	      "      next-key=%U",
	      bs->bs_idx, bfd_hop_type_string (bs->hop_type), bfd_state_string (bs->local_state),
	      bfd_state_string (bs->remote_state), bs->local_discr, bs->remote_discr,
	      bfd_diag_code_string (bs->local_diag), (bs->echo ? "yes" : "no"),
	      bs->config_desired_min_tx_usec, bs->config_required_min_rx_usec, 1,
	      bs->local_detect_mult, bs->remote_min_rx_usec, bs->remote_min_echo_rx_usec,
	      (bs->remote_demand ? "yes" : "no"), bfd_poll_state_string (bs->poll_state),
	      bs->auth.local_seq_number, bs->auth.remote_seq_number,
	      (bs->auth.is_delayed ? "yes" : "no"), format_bfd_auth_key, bs->auth.curr_key,
	      format_bfd_auth_key, bs->auth.next_key);
  return s;
}
...
{
  const bfd_session_t *bs = va_arg (*args, bfd_session_t *);
  s = format (s, "bs_idx=%u local-state=%s remote-state=%s", bs->bs_idx,
...
vnet_api_error_t
bfd_auth_activate (bfd_session_t *bs, u32 conf_key_id, u8 bfd_key_id, u8 is_delayed)
{
  bfd_main_t *bm = &bfd_main;
  const uword *key_idx_p = hash_get (bm->auth_key_by_conf_key_id, conf_key_id);
  if (!key_idx_p)
    {
      vlib_log_err (bm->log_class, "authentication key with config ID %u doesn't exist)",
		    conf_key_id);
...
vnet_api_error_t
bfd_auth_set_key (u32 conf_key_id, u8 auth_type, u8 key_len, const u8 *key_data)
{
  bfd_main_t *bm = &bfd_main;
...
bfd_auth_del_key (u32 conf_key_id)
{
  ...
  if (key_idx_p)
    ...
    {
      /* no such key */
      vlib_log_err (bm->log_class, "authentication key with conf ID %u does not exist",
		    conf_key_id);
      return VNET_API_ERROR_BFD_ENOENT;
    }
  return 0;
