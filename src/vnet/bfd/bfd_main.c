...
#ifdef BE_INCONSISTENT_IN_SINGLE_CORE_AND_MAKE_UNITTEST_WORK
/*
 ...
 *
 * Just get rid of the attempts to send the event and expose the same
...
bfd_rpc_notify_listeners_cb (const bfd_rpc_notify_listeners_t* a)
{
  ...
  if (!pool_is_free_index (bm->sessions, bs_idx))
    {
      bfd_session_t *bs =
	pool_elt_at_index (bm->sessions, bs_idx);
