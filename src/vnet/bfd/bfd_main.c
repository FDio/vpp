	       ...
	       int handling_wakeup)
{
  ...
  if (next)
	...
	((bs->event_time_nsec - now) % bm->nsec_per_tw_tick != 0);
      ...
      if (bs->tw_id)
	{
	  tw_timer_update (&bm->wheel, bs->tw_id, wheel_time_ticks);
	...
	{
	  bs->tw_id =
	    tw_timer_start (&bm->wheel, bs->bs_idx, 0, wheel_time_ticks);
...
bfd_process (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  while (1)
	...
	tw_timer_first_expires_in_ticks (&bm->wheel);
//      if (!first_expires_in_ticks)
//      {
//        BFD_DBG
//          ("tw_timer_first_expires_in_ticks(%p) returns 0ticks, using 1 instead",
//           &bm->wheel);
//        first_expires_in_ticks = 1;
//      }
//      else
//      {
...
//      }
	...
	now + first_expires_in_ticks * bm->nsec_per_tw_tick;
      ...
      bfd_unlock (bm);
      f64 timeout = (next_expire_nsec - now) * SEC_PER_NSEC;;
      BFD_DBG ("vlib_process_wait_for_event_or_clock(vm, %.04f)", timeout);
	...
	default:
      ...
      bfd_lock (bm);
      expired = tw_timer_expire_timers_vec (&bm->wheel, vm_time, expired);
...
bfd_main_init (vlib_main_t * vm)
{
  ...
  BFD_DBG ("tw_timer_wheel_init(%p, %p, %.04f, %u)", &bm->wheel, NULL,
	   1.00 / BFD_TW_TPS, ~0);
  tw_timer_wheel_init (&bm->wheel, NULL, 1.00 / BFD_TW_TPS, ~0);
