 ...
 *
 * This is the stable public snapshot exchanged between the BFD implementation
 * and VNET consumers through the notifier. It is not an external control-plane
 * ABI. The payload is copied before crossing the notifier boundary, and
 * consumers must not depend on the layout or lifetime of bfd_session_t.
 *
 * New in-tree consumers should subscribe through the notifier and filter the
 * snapshot fields they need instead of adding direct calls from BFD.
 */
typedef struct
{
