...
/**
 * @file
 * @brief Implements the vnet-owned BFD notification bridge
 */
...
clib_callback_data_typedef (bfd_vnet_bridge_listener_set_t, bfd_vnet_bridge_listener_t);
 ...
 */
static void
bfd_vnet_bridge_notify_listeners (bfd_listen_event_e event, const vnet_bfd_event_t *payload)
{
  bfd_vnet_bridge_main_t *bbm = &bfd_vnet_bridge_main;
 ...
 */
void
bfd_vnet_bridge_publish (bfd_listen_event_e event, const vnet_bfd_event_t *payload)
{
  ASSERT (payload);
  ...
  if (vlib_get_thread_index () == 0)
	...
	.payload = *payload,
      ...
      vlib_rpc_call_main_thread (bfd_vnet_bridge_publish_rpc_cb, (u8 *) &args, sizeof (args));
    }
}
