...
resolve_event (dns_main_t * dm, f64 now, u8 * reply)
{
  ...
  if (1)
    fformat (stdout, "%U", format_dns_reply, reply, 1 /* verbose */ );
  ...
  switch (rcode)
    ...
    case DNS_RCODE_FORMAT_ERROR:
      /* These are probably permanent errors */
      clib_warning ("name/format error");
      vlib_node_increment_counter (vm, dns46_reply_node.index,
				   DNS46_REPLY_ERROR_FORMAT_ERROR, 1);
      rv = VNET_API_ERROR_NAME_SERVER_FORMAT_ERROR;
      vec_free (reply);
      break;
  ...
  /* $$$ u16 limits cache to 65K entries, maybe not a tragedy */
  ...
  ep->expiration_time = now + 600.0;	/*$$$ get from response.. */
  ...
  for (i = 0; i < vec_len (ep->api_clients_to_notify); i++)
	...
	continue;
      ...
      rmp->retval = rv;
      rmp->reply_length = clib_net_to_host_u32 (vec_len (ep->dns_response));
      if (rv == 0)
