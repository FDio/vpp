...
(vl_api_dns_enable_disable_t * mp)
{
  ...
  rv = dns_enable_disable (dm, mp->is_enable);
  ...
  while (i < vec_len (rv))
    {
      if (rv[i] == '.')
        {
          rv[last_label_index] = i - last_label_index;
                             ...
                             dns_cache_entry_t **retp)
{
  ...
  if (clib_bihash_search_8_8 (&dm->cache_entry_by_name, &kv, &kv))
    {
      pool_index = kv.value;
