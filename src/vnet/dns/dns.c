...
static int
dns_delete_entry_by_index_nolock (dns_main_t * dm, u32 index)
{
...
dns_delete_by_name (dns_main_t * dm, u8 * name)
{
  ...
  rv = dns_delete_entry_by_index_nolock (dm, p[0]);
  ...
  for (i = 0; i < limit; i++)
      ...
      if (!pool_is_free_index (dm->entries, victim_index))
	{
	  ...
	  if ((ep->flags & DNS_CACHE_ENTRY_FLAG_VALID)
	    ...
	    {
	      rv = dns_delete_entry_by_index_nolock (dm, victim_index);
	      dns_cache_unlock (dm);
	      return rv;
	    }
	}
				   ...
				   u8 * reply)
{
  ...
  /* Skip the questions, we ain't interested */
      ...
      else
	{
	  if (ep->flags & DNS_CACHE_ENTRY_FLAG_VALID)
		...
		ss = "    ";
	      ...
	      s = format (s, "%s%U", ss, format_dns_reply, ep->dns_response,
			  verbose);
	      if (verbose > 0)
		{
		  if (!(ep->flags & DNS_CACHE_ENTRY_FLAG_STATIC))
		    s =
		      format (s, "TTL left %.1f", ep->expiration_time - now);
		}
	    }
	  else
	    {
    ...
    if (ep->flags & DNS_CACHE_ENTRY_FLAG_VALID)
      {
        ...
        if (verbose > 0)
          {
            if (!(ep->flags & DNS_CACHE_ENTRY_FLAG_STATIC))
              s = format (s, "TTL left %.1f", ep->expiration_time - now);
          }
      }
    else
      {
