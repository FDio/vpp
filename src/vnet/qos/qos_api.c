...
static qos_source_t
qos_source_decode (vl_api_qos_source_t v)
{
  v = ntohl (v);
  ...
  switch (v)
    ...
    case QOS_API_SOURCE_EXT:
      return (QOS_SOURCE_EXT);
    case QOS_API_SOURCE_VLAN:
      return (QOS_SOURCE_VLAN);
    case QOS_API_SOURCE_MPLS:
      return (QOS_SOURCE_MPLS);
    case QOS_API_SOURCE_IP:
      return (QOS_SOURCE_IP);
    }

  return (QOS_SOURCE_IP + 1);
}
					    ...
					    * mp)
{
  ...
  qs = qos_source_decode (mp->input_source);
  ...
  (vl_api_qos_mark_enable_disable_t * mp)
{
  ...
  qs = qos_source_decode (mp->output_source);
