...
typedef struct qos_egress_map_t_
{
     ...
     */
  qos_bits_t qem_output[QOS_MARK_N_SOURCES][256];
} qos_egress_map_t;
...
extern void qos_egress_map_update (qos_egress_map_id_t tid,
				   qos_mark_source_t input_source,
				   qos_bits_t * values);
 ...
 * Add a qos-egress map to an interface. If sw_if_index = ~0
 * then the configuration is for the 'default' table.
 * If the table is NULL, this is a removal.
 * The output source indocates at which location in the header stack
 * the egress mapping is applied. For example, is output is MPLS then
 * the QoS markings will occur for MPLS pakcets.
 */
extern int qos_egress_map_interface_update (u32 sw_if_index,
 ...
 */
extern index_t *qos_egress_map_configs[QOS_MARK_N_SOURCES];

always_inline qos_egress_map_t *
qos_egress_map_get_for_interface (u32 sw_if_index,
				  qos_mark_source_t output_source)
{
  ASSERT (vec_len (qos_egress_map_configs[output_source]) > sw_if_index);

  return pool_elt_at_index (qem_pool,
			    qos_egress_map_configs[output_source]
			    [sw_if_index]);
}
