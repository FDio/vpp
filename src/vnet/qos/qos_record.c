...
} l2_ip_qos_record_main_t;

static l2_ip_qos_record_main_t l2_ip_qos_record_main;
		   ...
		   vlib_frame_t * frame, int is_ip6, int is_l2)
{
  ...
  while (n_left_from > 0)
	...
	non_ip:
	  if (is_l2)
	    {
	      l2_ip_qos_record_main_t *mp = &l2_ip_qos_record_main;
	      vlib_buffer_advance (b0, -l2_len);
	      next0 = vnet_l2_feature_next (b0, mp->l2_input_next,
...
l2_ip_qos_init (vlib_main_t * vm)
{
  ...
  feat_bitmap_init_next_nodes (vm,
			       ...
			       L2INPUT_N_FEAT,
			       l2input_get_feat_names (), mp->l2_input_next);
  return 0;
}
