				  ...
				  tls_handshake_msg_info_t *info)
{
  ...
  info->extensions_start = p + 2;
  if (PREDICT_FALSE (info->extensions_start + info->extensions_len - b > len))
    return TLS_HS_PARSE_ERR_CIPHER_SUITE_LEN;
			       ...
			       tls_handshake_ext_info_t **exts)
{
  ...
  ASSERT (info->extensions_start != 0);

  if (info->extensions_len < 2)
    return TLS_HS_PARSE_ERR_EXTENSIONS_LEN;

  b = info->extensions_start;
  b_end = info->extensions_start + info->extensions_len;
				 ...
				 tls_handshake_msg_info_t *info)
{
  ...
  info->msg_len = tls_handshake_message_len (msg_hdr);
  if (info->msg_len > len)
    return info->msg_len > TLS_FRAGMENT_MAX_ENC_LEN ?
  ...
  return tls_handshake_msg_parsers[msg_hdr->msg_type](b, info->msg_len, info);
}
