  ...
  if (ctx->tls_type == TRANSPORT_PROTO_DTLS)
    {
      ...
      session_type_t st;
      st = session_type_from_proto_and_ip (TRANSPORT_PROTO_DTLS,
                                           ctx->tcp_is_ip4);
...
int
dtls_session_connected_cb (u32 app_wrk_index, u32 ctx_handle,
...
//  ctx->c_s_index = app_session->session_index;
//
//  st = session_type_from_proto_and_ip (TRANSPORT_PROTO_DTLS, ctx->tcp_is_ip4);
//  app_session->session_type = st;
//  app_session->connection_index = ctx_handle;



  // FIXME should the connection be initialized after handshake?
//  app_worker_t *app_wrk = app_worker_get (app_wrk_index);
//  app_session->app_wrk_index = ctx->parent_app_wrk_index;
//  app_session->session_state = SESSION_STATE_OPENED;
				...
				session_t * tls_session, session_error_t err)
{
  if (session_get_transport_proto (tls_session) == TRANSPORT_PROTO_TCP)
    return tls_session_connected_cb (tls_app_index, ho_ctx_index,
				     tls_session, err);
  else
    return dtls_session_connected_cb (tls_app_index, ho_ctx_index,
				      tls_session, err);
}
...
dtls_migrate_udp (void *arg)
{
  ...
  u32 ctx_handle, thread_index, old_thread;
  session_t *us;

  old_thread = ctx->c_thread_index;
  ...
  clib_warning ("migrated handle %u", ctx_handle);
  ctx->tls_ctx_handle = ctx_handle;
  ...
  /* Clone app session to this thread */
//  session_dgram_connect_notify (&ctx->connection, old_thread,
//                                &new_app_session);
//
//  ctx->app_session_handle = session_handle (new_app_session);
//  clib_warning ("new handle 0x%lx new thread %u", ctx->app_session_handle, vlib_get_thread_index());
}
