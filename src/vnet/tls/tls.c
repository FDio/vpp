...
tls_notify_app_accept (tls_ctx_t * ctx)
{
  ...
  sm = app_worker_get_listen_segment_manager (app, app_listener);
...
{
  vnet_connect_args_t _cargs = {0}, *cargs = &_cargs;
...
tls_start_listen (u32 app_listener_index, transport_endpoint_t * tep)
{
  ...
  tls_app_wrk = application_get_default_worker (tm->app_index);
...
{
  tls_main_t *tm = &tls_main;
  app_worker_t *tls_app;
  tls_ctx_t *lctx;
  lctx = tls_listener_ctx_get (lctx_index);
  tls_app = application_get_default_worker (tm->app_index);
  app_worker_stop_listen (tls_app, lctx->tls_session_handle);
