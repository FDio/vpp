...
{
  return tls_vfts[ctx->tls_ctx_engine].ctx_write (ctx, app_tx_fifo,
                                                  tls_tx_fifo);
}

static inline int
tls_ctx_read (tls_ctx_t * ctx, svm_fifo_t *tls_rx_fifo,
              svm_fifo_t *app_rx_fifo)
{
  return tls_vfts[ctx->tls_ctx_engine].ctx_read (ctx, tls_rx_fifo,
                                                 app_rx_fifo);
}
...
{
  return tls_vfts[ctx->tls_ctx_engine].ctx_handshake_rx (ctx);
}
...
{
  stream_session_t *app_session = 0;
  application_t *app;
  tls_ctx_t *ctx;
  int read;

  ctx = tls_ctx_get (tls_session->opaque);
  if (PREDICT_FALSE (!tls_ctx_handshake_is_over (ctx)))
    {
      tls_handshake_rx (ctx);
      return 0;
    }
  app_session = session_get_from_handle (ctx->app_session_handle);
...
void
tls_session_try_tx (tls_ctx_t *ctx)
{
  stream_session_t *tls_session;
  tls_session = session_get_from_handle (ctx->tls_session_handle);
  tls_ctx_write (ctx, 0 /* no app session */, tls_session->server_tx_fifo);
}

void
tls_session_try_rx (tls_ctx_t *ctx)
{
  stream_session_t *tls_session;
  tls_session = session_get_from_handle (ctx->tls_session_handle);
  tls_ctx_read (ctx, tls_session->server_rx_fifo, 0 /* no app session */);
}
...
tls_disconnect (u32 ctx_handle, u32 thread_index)
{
  ...
  TLS_DBG (1, "Disconnecting %u", ctx_handle);
