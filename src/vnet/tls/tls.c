			   ...
			   session_error_t err)
{
  ...
  ctx = tls_ctx_get_w_thread (ctx_handle, transport_ho_cl_thread ());
...
dtls_connect (transport_endpoint_cfg_t *tep)
{
  ...
  ctx_handle = tls_ctx_alloc_w_thread (engine_type, transport_ho_cl_thread ());
  ctx = tls_ctx_get_w_thread (ctx_handle, transport_ho_cl_thread ());
  ctx->parent_app_wrk_index = sep->app_wrk_index;
  ctx->parent_app_api_context = sep->opaque;
  ctx->tcp_is_ip4 = sep->is_ip4;
...
{
  tls_ctx_t *ho_ctx;
  ho_ctx = tls_ctx_get_w_thread (ho_index, transport_ho_cl_thread ());
...
{
  tls_ctx_t *ctx;
  ctx = tls_ctx_get_w_thread (ho_index, transport_ho_cl_thread ());
  tls_ctx_free (ctx);
}
