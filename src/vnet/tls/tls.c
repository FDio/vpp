...
int
tls_session_connected_cb_dtls (u32 tls_app_index, u32 ho_ctx_index,
			       session_t * tls_session, session_error_t err)
{
  transport_connection_t *tc;
  tls_ctx_t *ctx;
...
//  ctx = tls_ctx_half_open_get (ho_ctx_index);
  ctx = tls_ctx_get_w_thread (ho_ctx_index, 1);
  ...
  ctx->tls_session_handle = session_handle (tls_session);
  tls_session->opaque = ho_ctx_index;
...
//  ctx_index = tls_ctx_half_open_alloc ();
//  ctx = tls_ctx_half_open_get (ctx_index);
  ctx_index = tls_ctx_alloc_w_thread (engine_type, 1 /* because of udp */ );
  ctx = tls_ctx_get_w_thread (ctx_index, 1);
  ctx->parent_app_wrk_index = sep->app_wrk_index;
  ctx->parent_app_api_context = sep->opaque;
  ctx->tcp_is_ip4 = sep->is_ip4;
  ctx->ckpair_index = sep->ckpair_index;
...
//  tls_ctx_half_open_reader_unlock ();
  ...
  TLS_DBG (1, "New DTLS connect request %x engine %d", ctx_index,
	   engine_type);
  return 0;
}
