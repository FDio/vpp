...
//  clib_warning ("about to close app session %u", app_session->session_index);
...
tls_disconnect (u32 ctx_handle, u32 thread_index)
{
  ...
  ctx = tls_ctx_get (ctx_handle);
  clib_warning ("app session for it is %u transport %u",
                session_index_from_handle(ctx->app_session_handle),
