...
format_mac_address_t (u8 *s, va_list *args)
{
  ...
  return format (s, "%02x:%02x:%02x:%02x:%02x:%02x", mac->bytes[0],
...
unformat_mac_address_t (unformat_input_t *input, va_list *args)
{
  ...
  if (unformat (input, "%_%X:%X:%X:%X:%X:%X%_", 1, &mac->bytes[0], 1,
		&mac->bytes[1], 1, &mac->bytes[2], 1, &mac->bytes[3], 1,
		&mac->bytes[4], 1, &mac->bytes[5]))
    return (1);
