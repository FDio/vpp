...
clib_error_t *
vnet_eth_set_rss_key (vnet_main_t *vnm, u32 hw_if_index, u8 *key, u8 key_len)
{
  ...
  if (ei == 0 || ei->cb.set_rss_key == 0)
    return vnet_error (VNET_ERR_UNSUPPORTED, "not supported");

  return ei->cb.set_rss_key (vnm, hi, key, key_len);
}

clib_error_t *
vnet_eth_set_rss_type (vnet_main_t *vnm, u32 hw_if_index, vnet_eth_rss_type_t ip4_type,
		       vnet_eth_rss_type_t ip6_type)
{
  ...
  ei = ethernet_get_interface (em, hw_if_index);
  if (ei == 0 || ei->cb.set_rss_type == 0)
    return vnet_error (VNET_ERR_UNSUPPORTED, "not supported");

  return ei->cb.set_rss_type (vnm, hi, ip4_type, ip6_type);
}
