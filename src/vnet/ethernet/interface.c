			...
			vnet_link_t link_type, const void *dst_address)
{
  ...
  if (sub_sw != sup_sw)
      ...
      if (PREDICT_FALSE (!is_p2p))
	{
	  ...
	  // Wildcard (any) sub-interfaces cannot have L3 addresses because
	  // a deterministic VLAN tag cannot be constructed for TX rewrite.
	  // These sub-interfaces are supported in L2 mode where VTR handles
	  // tag push/pop independently.
