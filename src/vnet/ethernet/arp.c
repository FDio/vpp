				   ...
				   vlib_frame_t * frame)
{
  ...
  while (1)
	...
	default:
      ...
      for (i = 0; i < vec_len (am->ethernet_arp_by_sw_if_index); i++)
	{
	  hash_pair_t *pair;
	  ...
	  vec_validate (am->ethernet_arp_by_sw_if_index, i);
	  arp_int = &am->ethernet_arp_by_sw_if_index[i];
      /* *INDENT-OFF* */
      hash_foreach_pair (pair, arp_int->arp_entries,
      ({
         e = pool_elt_at_index(am->ip4_entry_pool,
	                   pair->value[0]);
         vec_add1 (to_check, e->ip4_address);
      }));
      /* *INDENT-ON* */
	  vec_validate (to_check, hash_elts (arp_int->arp_entries));
	  for (j = 0; j < vec_len (to_check); j++)
	    {
	      e = arp_entry_find (arp_int, &to_check[j]);
	      if (e && (e->flags & ETHERNET_ARP_IP4_ENTRY_FLAG_DYNAMIC))
		{
		  ...
		  adj_nbr_walk_nh4 (e->sw_if_index,
				    &e->ip4_address, arp_mk_adj_counter_walk,
				    e);
