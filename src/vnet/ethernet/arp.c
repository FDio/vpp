...
arp_update_adjacency (vnet_main_t * vnm, u32 sw_if_index, u32 ai)
{
  ...
  switch (adj->lookup_next_index)
    ...
    case IP_LOOKUP_NEXT_MCAST:
    {
      ...
      adj_mcast_update_rewrite (ai, rewrite);
       ...
       * from the address. Mask is in network byte order since its used against
       * addresses in the data-plane
       */
      adj->rewrite_header.dst_mcast_offset = offset;
      adj->rewrite_header.dst_mcast_mask = clib_host_to_net_u32(0x007fffff);
