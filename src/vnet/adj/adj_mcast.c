 ...
 * There is only one mcast per-interface per-protocol, so this is a
 ...
 */
adj_index_t
adj_mcast_add_or_lock (fib_protocol_t proto, vnet_link_t link_type,
		       u32 sw_if_index)
{
  ip_adjacency_t *adj;
  ...
  if (ADJ_INDEX_INVALID == adj_mcasts[proto][sw_if_index])
    {
      ...
      vnet_rewrite_init (
	vnm, sw_if_index, link_type, adj->ia_node_index,
...
adj_mcast_update_rewrite (adj_index_t adj_index, u8 *rewrite, u8 offset)
{
  ...
  adj_nbr_update_rewrite_internal (
    adj, IP_LOOKUP_NEXT_MCAST, adj_get_mcast_node (adj->ia_nh_proto),
...
adj_mcast_midchain_update_rewrite (adj_index_t adj_index,
				   ...
				   const void *fixup_data, adj_flags_t flags,
				   u8 *rewrite, u8 offset, u32 mask)
{
  ip_adjacency_t *adj;
   ...
   */
  adj_nbr_update_rewrite_internal (
    adj, IP_LOOKUP_NEXT_MCAST_MIDCHAIN, adj_get_mcast_node (adj->ia_nh_proto),
    vnet_tx_node_index_for_sw_interface (vnet_get_main (),
					 adj->rewrite_header.sw_if_index),
    rewrite);
 ...
 */
static walk_rc_t
adj_mcast_hw_sw_interface_state_change (vnet_main_t *vnm, u32 sw_if_index,
					void *arg)
{
 ...
 */
static clib_error_t *
adj_mcast_hw_interface_state_change (vnet_main_t *vnm, u32 hw_if_index,
				     u32 flags)
{
  /*
 ...
 */
void
adj_mcast_walk (u32 sw_if_index, fib_protocol_t proto, adj_walk_cb_t cb,
		void *ctx)
{
...
format_adj_mcast (u8 *s, va_list *ap)
{
  ...
  if (adj->rewrite_header.flags & VNET_REWRITE_HAS_FEATURES)
    s = format (s, "[features] ");
  s = format (s, "%U", format_vnet_rewrite, &adj->rewrite_header,
...
format_adj_mcast_midchain (u8 *s, va_list *ap)
{
  ...
  s = format (s, "\n%Ustacked-on:\n%U%U", format_white_space, indent,
	      format_white_space, indent + 2, format_dpo_id,
 ...
 * This is only for testing purposes so an efficient implementation is not
 * needed
 */
u32
adj_mcast_db_size (void)
{
  ...
  for (proto = FIB_PROTOCOL_IP4; proto <= FIB_PROTOCOL_IP6; proto++)
    {
      for (sw_if_index = 0; sw_if_index < vec_len (adj_mcasts[proto]);
	   sw_if_index++)
	{
	  if (ADJ_INDEX_INVALID != adj_mcasts[proto][sw_if_index])
	    {
...
adj_mcast_module_init (void)
{
  ...
  dpo_register (DPO_ADJACENCY_MCAST_MIDCHAIN, &adj_mcast_midchain_dpo_vft,
