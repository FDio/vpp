...
always_inline uword
adj_l2_rewrite_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		       vlib_frame_t *frame, int is_midchain, int do_counters)
{
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_TRUE (len0 <=
		...
		vnet_feature_arc_start_w_cfg_index (
		  em->output_feature_arc_index, tx_sw_if_index0, &next0, p0,
		  adj0->ia_cfg_index);
	      else
...
VLIB_NODE_FN (adj_l2_rewrite_node)
...
VLIB_NODE_FN (adj_l2_midchain_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  if (adj_are_counters_enabled ())
