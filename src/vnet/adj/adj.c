...
int
adj_recursive_loop_detect (adj_index_t ai, fib_node_index_t **entry_indicies)
{
  ip_adjacency_t *adj;
...
adj_last_lock_gone (ip_adjacency_t *adj)
{
  ...
  switch (adj->lookup_next_index)
    ...
    case IP_LOOKUP_NEXT_BCAST:
       ...
       */
      adj_nbr_remove (adj_get_index (adj), adj->ia_nh_proto, adj->ia_link,
...
u32
adj_child_add (adj_index_t adj_index, fib_node_type_t child_type,
...
static void
adj_feature_update (u32 sw_if_index, u8 arc_index, u8 is_enable, void *data)
{
  /*
...
#define ADJ_FROM_NODE(_node)                                                  \
  ((ip_adjacency_t *) ((char *) _node -                                       \
		       STRUCT_OFFSET_OF (ip_adjacency_t, ia_node)))
  ...
  if (summary)
    {
      ...
      vlib_cli_output (vm, "Per-adjacency counters: %s",
		       (adj_are_counters_enabled () ? "enabled" : "disabled"));
    }
  else
    {
 ...
 */
static clib_error_t *
adj_cli_counters_set (vlib_main_t *vm, unformat_input_t *input,
