  ...
  while (n_left > 8)
      ...
      if (interface_count)
	{
	  vlib_increment_combined_counter (
	    ...
	    thread_index, adj0->rewrite_header.sw_if_index, 1,
	    vlib_buffer_length_in_chain (vm, b[0]));
	  vlib_increment_combined_counter (
	    im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX,
	    thread_index, adj1->rewrite_header.sw_if_index, 1,
	    vlib_buffer_length_in_chain (vm, b[1]));
	  vlib_increment_combined_counter (
	    im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX,
	    thread_index, adj2->rewrite_header.sw_if_index, 1,
	    vlib_buffer_length_in_chain (vm, b[2]));
	  vlib_increment_combined_counter (
	    im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX,
	    thread_index, adj3->rewrite_header.sw_if_index, 1,
...
format_adj_midchain_tx_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "adj-midchain:[%d]:%U", tr->ai, format_ip_adjacency, tr->ai,
...
static uword
adj_midchain_tx (vlib_main_t *vm, vlib_node_runtime_t *node,
		 vlib_frame_t *frame)
{
...
static uword
adj_midchain_tx_no_count (vlib_main_t *vm, vlib_node_runtime_t *node,
			  vlib_frame_t *frame)
{
...
adj_midchain_teardown (ip_adjacency_t *adj)
{
  ...
  adj->ia_cfg_index = vnet_feature_modify_end_node (
    ...
    vlib_get_node_by_name (vlib_get_main (), (u8 *) "interface-output")
      ->index);
 ...
 */
void
adj_midchain_setup (adj_index_t adj_index, adj_midchain_fixup_t fixup,
		    const void *data, adj_flags_t flags)
{
  vlib_main_t *vm = vlib_get_main ();
  ip_adjacency_t *adj;
  u32 tx_node;
  ...
  adj->ia_cfg_index = vnet_feature_modify_end_node (
    adj_midchain_get_feature_arc_index_for_link_type (adj),
    adj->rewrite_header.sw_if_index, tx_node);
  vlib_worker_thread_barrier_release (vm);

  /*
   * stack the midchain on the drop so it's ready to forward in the
   * adj-midchain-tx. The graph arc used/created here is from the midchain-tx
   * node to the child's registered node. This is because post adj processing
   * the next node are any output features, then the midchain-tx.  from there
   * we need to get to the stacked child's node.
   */
  dpo_stack_from_node (tx_node, &adj->sub_type.midchain.next_dpo,
				 ...
				 u8 *rewrite)
{
  ...
  adj_nbr_update_rewrite_internal (
    adj, IP_LOOKUP_NEXT_MIDCHAIN, adj_get_midchain_node (adj->ia_link),
    adj_nbr_midchain_get_tx_node (adj), rewrite);
}

void
adj_nbr_midchain_update_next_node (adj_index_t adj_index, u32 next_node)
{
  ip_adjacency_t *adj;
  vlib_main_t *vm;
  ...
  adj->rewrite_header.next_index =
    vlib_node_add_next (vlib_get_main (), adj->ia_node_index, next_node);

  adj->ia_cfg_index = vnet_feature_modify_end_node (
    adj_midchain_get_feature_arc_index_for_link_type (adj),
    adj->rewrite_header.sw_if_index, next_node);
...
adj_nbr_midchain_reset_next_node (adj_index_t adj_index)
{
  ...
  adj->rewrite_header.next_index = vlib_node_add_next (
    vlib_get_main (), adj->ia_node_index, adj_nbr_midchain_get_tx_node (adj));

  adj->ia_cfg_index = vnet_feature_modify_end_node (
    adj_midchain_get_feature_arc_index_for_link_type (adj),
    adj->rewrite_header.sw_if_index, adj_nbr_midchain_get_tx_node (adj));
...
adj_nbr_midchain_unstack (adj_index_t adj_index)
{
   ...
   * stack on the drop
   */
  dpo_stack (DPO_ADJACENCY_MIDCHAIN, vnet_link_to_dpo_proto (adj->ia_link),
...
void
adj_nbr_midchain_stack_on_fib_entry (adj_index_t ai, fib_node_index_t fei,
				     fib_forward_chain_type_t fct)
{
  ...
  if (fib_entry_recursive_loop_detect (adj->sub_type.midchain.fei,
				       &entry_indicies))
    {
      /*
