...
adj_midchain_tx_inline (vlib_main_t * vm,
			...
			vlib_frame_t * frame,
			int interface_count, int collect_detailed_stats)
{
...
format_adj_midchain_tx_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "adj-midchain:[%d]:%U", tr->ai,
	      format_ip_adjacency, tr->ai, FORMAT_IP_ADJACENCY_NONE);
		 ...
		 vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  return adj_midchain_tx_inline
	  (vm, node, frame, 1, /* collect_detailed_stats */ 1);
  }else{
  return adj_midchain_tx_inline
	  (vm, node, frame, 1, /* collect_detailed_stats */ 0);
  }
}

VLIB_REGISTER_NODE (adj_midchain_tx_node, static) =
{
  .function = adj_midchain_tx,.name = "adj-midchain-tx",.vector_size =
    sizeof (u32),.format_trace = format_adj_midchain_tx_trace,.n_next_nodes =
    1,.next_nodes =
  {
  [0] = "error-drop",}
,};
			  ...
			  vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  if(collect_detailed_interface_stats()){
  return (adj_midchain_tx_inline
	  (vm, node, frame, 0, /* collect_detailed_stats */ 1 ));
  }else{
  return (adj_midchain_tx_inline
	  (vm, node, frame, 0, /* collect_detailed_stats */ 0 ));
  }
}

VLIB_REGISTER_NODE (adj_midchain_tx_no_count_node, static) =
{
  .function = adj_midchain_tx_no_count,.name =
    "adj-midchain-tx-no-count",.vector_size = sizeof (u32),.format_trace =
    format_adj_midchain_tx_trace,.n_next_nodes = 1,.next_nodes =
  {
  [0] = "error-drop",}
,};
...
{
.arc_name = "ip4-output",.node_name = "adj-midchain-tx",.runs_before =
    VNET_FEATURES ("interface-output"),.feature_index_ptr =
    &adj_midchain_tx_feature_node[VNET_LINK_IP4],};
VNET_FEATURE_INIT (adj_midchain_tx_no_count_ip4, static) =
{
.arc_name = "ip4-output",.node_name =
    "adj-midchain-tx-no-count",.runs_before =
    VNET_FEATURES ("interface-output"),.feature_index_ptr =
    &adj_midchain_tx_no_count_feature_node[VNET_LINK_IP4],};
VNET_FEATURE_INIT (adj_midchain_tx_ip6, static) =
{
.arc_name = "ip6-output",.node_name = "adj-midchain-tx",.runs_before =
    VNET_FEATURES ("interface-output"),.feature_index_ptr =
    &adj_midchain_tx_feature_node[VNET_LINK_IP6],};
VNET_FEATURE_INIT (adj_midchain_tx_no_count_ip6, static) =
{
.arc_name = "ip6-output",.node_name =
    "adj-midchain-tx-no-count",.runs_before =
    VNET_FEATURES ("interface-output"),.feature_index_ptr =
    &adj_midchain_tx_no_count_feature_node[VNET_LINK_IP6],};
VNET_FEATURE_INIT (adj_midchain_tx_mpls, static) =
{
.arc_name = "mpls-output",.node_name = "adj-midchain-tx",.runs_before =
    VNET_FEATURES ("interface-output"),.feature_index_ptr =
    &adj_midchain_tx_feature_node[VNET_LINK_MPLS],};
VNET_FEATURE_INIT (adj_midchain_tx_no_count_mpls, static) =
{
.arc_name = "mpls-output",.node_name =
    "adj-midchain-tx-no-count",.runs_before =
    VNET_FEATURES ("interface-output"),.feature_index_ptr =
    &adj_midchain_tx_no_count_feature_node[VNET_LINK_MPLS],};
VNET_FEATURE_INIT (adj_midchain_tx_ethernet, static) =
{
.arc_name = "ethernet-output",.node_name = "adj-midchain-tx",.runs_before =
    VNET_FEATURES ("error-drop"),.feature_index_ptr =
    &adj_midchain_tx_feature_node[VNET_LINK_ETHERNET],};
VNET_FEATURE_INIT (adj_midchain_tx_no_count_ethernet, static) =
{
.arc_name = "ethernet-output",.node_name =
    "adj-midchain-tx-no-count",.runs_before =
    VNET_FEATURES ("error-drop"),.feature_index_ptr =
    &adj_midchain_tx_no_count_feature_node[VNET_LINK_ETHERNET],};
VNET_FEATURE_INIT (adj_midchain_tx_nsh, static) =
{
.arc_name = "nsh-output",.node_name = "adj-midchain-tx",.runs_before =
    VNET_FEATURES ("error-drop"),.feature_index_ptr =
    &adj_midchain_tx_feature_node[VNET_LINK_NSH],};
VNET_FEATURE_INIT (adj_midchain_tx_no_count_nsh, static) =
{
.arc_name = "nsh-output",.node_name =
    "adj-midchain-tx-no-count",.runs_before =
    VNET_FEATURES ("error-drop"),.feature_index_ptr =
    &adj_midchain_tx_no_count_feature_node[VNET_LINK_NSH],};
...
adj_midchain_setup (adj_index_t adj_index,
		    adj_midchain_fixup_t fixup, adj_flags_t flags)
{
...
adj_nbr_midchain_stack (adj_index_t adj_index, const dpo_id_t * next)
{
  ...
  dpo_stack_from_node (adj_nbr_midchain_get_tx_node (adj),
		       &adj->sub_type.midchain.next_dpo, next);
}
...
{
  dpo_register (DPO_ADJACENCY_MIDCHAIN, &adj_midchain_dpo_vft,
		midchain_nodes);
}
