 ...
 * is intentional as it encourages the user to carefully consider which
 * adjacency sub-type they are really using, and hence assign it data in the
 * appropriate sub-type space in the union of sub-types. This prevents the adj
 * becoming a disorganised dumping group for 'my features needs a u16
 * somewhere' data. It is important to enforce this approach as space in the
 * adjacency is a premium, as we need it to fit in 1 cache line.
 *
 * the API is also based around an index to an adjacency not a raw pointer.
 * This is so the user doesn't suffer the same limp inducing firearm injuries
 * that the author suffered as the adjacencies can realloc.
 */
...
#define IP4_LOOKUP_NEXT_NODES                                                 \
  {                                                                           \
    [IP_LOOKUP_NEXT_DROP] = "ip4-drop", [IP_LOOKUP_NEXT_PUNT] = "ip4-punt",   \
    [IP_LOOKUP_NEXT_LOCAL] = "ip4-local", [IP_LOOKUP_NEXT_ARP] = "ip4-arp",   \
...
#define IP6_LOOKUP_NEXT_NODES                                                 \
  {                                                                           \
    [IP_LOOKUP_NEXT_DROP] = "ip6-drop", [IP_LOOKUP_NEXT_PUNT] = "ip6-punt",   \
...
#define FOR_EACH_ADJ_ATTR(_attr)                                              \
  for (_attr = ADJ_ATTR_SYNC_WALK_ACTIVE;                                     \
       _attr <= ADJ_ATTR_MIDCHAIN_FIXUP_FLOW_HASH; _attr++)
...
STATIC_ASSERT (
  (STRUCT_OFFSET_OF (ip_adjacency_t, cacheline1) == CLIB_CACHE_LINE_BYTES),
  "IP adjacency cacheline 1 is more than one cacheline size offset");
#if defined __x86_64__
STATIC_ASSERT (
  (STRUCT_OFFSET_OF (ip_adjacency_t, cacheline3) == 3 * CLIB_CACHE_LINE_BYTES),
 ...
 */
extern u32 adj_child_add (adj_index_t adj_index, fib_node_type_t type,
...
/**
 * @brief Return true if the adjacency is 'UP', i.e. can be used for
 * forwarding. 0 is down, !0 is up.
 */
