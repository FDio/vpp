...
format_adj_nsh_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "adj-idx %d : %U", t->adj_index, format_ip_adjacency,
...
always_inline uword
adj_nsh_rewrite_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			vlib_frame_t *frame, int is_midchain)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  vlib_increment_combined_counter (&adjacency_counters, thread_index,
					   adj_index0,
					   /* packet increment */ 0,
	  ...
	  if (PREDICT_TRUE ((vlib_buffer_length_in_chain (vm, p0) <=
		...
		nsh_main_placeholder.output_feature_arc_index, tx_sw_if_index0,
		&next0, p0);
	    }
	  else
	    {
...
VLIB_NODE_FN (adj_nsh_rewrite_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return adj_nsh_rewrite_inline (vm, node, frame, 0);
}

VLIB_NODE_FN (adj_nsh_midchain_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
