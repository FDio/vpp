...
adj_bfd_notify (bfd_listen_event_e event, const bfd_session_t *session)
{
  ...
  fproto = (ip46_address_is_ip4 (&key->peer_addr) ? FIB_PROTOCOL_IP4 :
						    FIB_PROTOCOL_IP6);

  /*
   * find the adj that corresponds to the BFD session.
   */
  ai = adj_nbr_add_or_lock (fproto, fib_proto_to_link (fproto),
  ...
  switch (event)
    ...
    case BFD_LISTEN_EVENT_UPDATE:
       ...
       */
      abd =
	adj_bfd_from_base (adj_delegate_get (adj_get (ai), ADJ_DELEGATE_BFD));
...
adj_delegate_fmt_bfd (const adj_delegate_t *aed, u8 *s)
{
  ...
  s = format (s, "BFD:[state:%s index:%d]",
	      adj_bfd_state_names[abd->abd_state], abd->abd_index);
