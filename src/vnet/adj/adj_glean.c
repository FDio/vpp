...
static adj_index_t
adj_glean_db_lookup (fib_protocol_t proto, u32 sw_if_index,
...
static void
adj_glean_db_insert (fib_protocol_t proto, u32 sw_if_index,
		     const ip46_address_t *nh_addr, adj_index_t ai)
{
  ...
  hash_set_mem_alloc (&adj_gleans[proto][sw_if_index], nh_addr, ai);

  vlib_worker_thread_barrier_release (vm);
}

static void
adj_glean_db_remove (fib_protocol_t proto, u32 sw_if_index,
		     const ip46_address_t *nh_addr)
{
  vlib_main_t *vm = vlib_get_main ();
  ...
  ASSERT (ADJ_INDEX_INVALID !=
	  adj_glean_db_lookup (proto, sw_if_index, nh_addr));
  hash_unset_mem_free (&adj_gleans[proto][sw_if_index], nh_addr);
 ...
 */
adj_index_t
adj_glean_add_or_lock (fib_protocol_t proto, vnet_link_t linkt,
		       u32 sw_if_index, const fib_prefix_t *conn)
{
  ip_adjacency_t *adj;
  adj_index_t ai;
  ...
  if (ADJ_INDEX_INVALID == ai)
    {
      ...
      adj->rewrite_header.max_l3_packet_bytes = vnet_sw_interface_get_mtu (
	vnet_get_main (), sw_if_index, vnet_link_to_mtu (linkt));

      vnet_update_adjacency_for_sw_interface (vnet_get_main (), sw_if_index,
					      ai);
...
adj_glean_update_rewrite (adj_index_t adj_index)
{
  ...
  vnet_rewrite_for_sw_interface (
    vnet_get_main (), adj_fib_proto_2_nd (adj->ia_nh_proto),
    adj->rewrite_header.sw_if_index, adj->ia_node_index,
    VNET_REWRITE_FOR_SW_INTERFACE_ADDRESS_BROADCAST, &adj->rewrite_header,
    sizeof (adj->rewrite_data));
}

static adj_walk_rc_t
adj_glean_update_rewrite_walk (adj_index_t ai, void *data)
{
...
void
adj_glean_walk (u32 sw_if_index, adj_walk_cb_t cb, void *data)
{
...
adj_index_t
adj_glean_get (fib_protocol_t proto, u32 sw_if_index, const ip46_address_t *nh)
{
  if (NULL != nh)
    {
...
adj_glean_get_src (fib_protocol_t proto, u32 sw_if_index,
		   const ip46_address_t *nh)
{
  ...
  hash_foreach_mem (
    conn, ai, adj_gleans[proto][sw_if_index], ({
      adj = adj_get (ai);
...
adj_glean_remove (ip_adjacency_t *adj)
{
  ...
  fib_prefix_normalize (&adj->sub_type.glean.rx_pfx, &norm);
  adj_glean_db_remove (adj->ia_nh_proto, adj->rewrite_header.sw_if_index,
		       &norm.fp_addr);
}

static adj_walk_rc_t
adj_glean_start_backwalk (adj_index_t ai, void *data)
{
 ...
 */
static clib_error_t *
adj_glean_hw_interface_state_change (vnet_main_t *vnm, u32 hw_if_index,
				     u32 flags)
{
  /*
  ...
  vnet_hw_interface_walk_sw (
    vnm, hw_if_index, adj_nbr_hw_sw_interface_state_change, (void *) sw_flags);
...
format_adj_glean (u8 *s, va_list *ap)
{
  ...
  s = format (s, "%U-glean: [src:%U] %U", format_fib_protocol,
	      adj->ia_nh_proto, format_fib_prefix, &adj->sub_type.glean.rx_pfx,
	      format_vnet_rewrite, &adj->rewrite_header,
