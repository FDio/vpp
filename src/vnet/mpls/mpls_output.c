...
format_mpls_output_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "adj-idx %d : %U flow hash: 0x%08x", t->adj_index,
...
static inline uword
mpls_output_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		    vlib_frame_t *from_frame, mpls_output_mode_t mode)
{
...
static uword
mpls_frag (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* IP fragmentation */
	  if (is_ip4)
	    error0 = ip4_frag_do_fragment (
	      vm, pi0, adj0->rewrite_header.max_l3_packet_bytes, encap_size,
	      &frags);
	  else
