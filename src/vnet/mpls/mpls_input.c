...
#define _(a, b)                                                               \
  if (t->next_index == MPLS_INPUT_NEXT_##a)                                   \
    next_name = b;
  foreach_mpls_input_next;
#undef _

  s = format (s, "MPLS: next %s[%d]  label %d ttl %d exp %d", next_name,
	      t->next_index, vnet_mpls_uc_get_label (label),
	      vnet_mpls_uc_get_ttl (label), vnet_mpls_uc_get_exp (label));
...
static inline uword
mpls_input_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_FALSE (h0[3] == 0))
	  ...
	  else
	    {
	      ...
	      vlib_increment_simple_counter (cm, thread_index, sw_if_index0,
					     1);
	    }
	  ...
	  if (PREDICT_FALSE (h1[3] == 0))
	  ...
	  else
	    {
	      ...
	      vlib_increment_simple_counter (cm, thread_index, sw_if_index1,
					     1);
	    }

	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
	    {
	      mpls_input_trace_t *tr =
...
static clib_error_t *
mpls_input_init (vlib_main_t *vm)
{
  mpls_setup_nodes (vm);
