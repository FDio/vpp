...
format_mpls_load_balance_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "MPLS: next [%d], LB index %d hash %d", t->next_index,
	      t->lb_index, t->hash);
  return s;
}
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_FALSE (lb0->lb_n_buckets > 1))
		...
		{
		  hc0 = vnet_buffer (p0)->ip.flow_hash =
		    mpls_compute_flow_hash (mpls0, hc0);
		}
	      dpo0 = load_balance_get_fwd_bucket (
	  ...
	  if (PREDICT_FALSE (lb1->lb_n_buckets > 1))
		...
		{
		  hc1 = vnet_buffer (p1)->ip.flow_hash =
		    mpls_compute_flow_hash (mpls1, hc1);
		}
	      dpo1 = load_balance_get_fwd_bucket (
...
mpls_lookup_init (vlib_main_t *vm)
{
  ...
  mpls_lookup_to_replicate_edge = vlib_node_add_named_next (
    vm, mm->mpls_lookup_node_index, "mpls-replicate");
