...
static u8 *
format_mpls_tunnel_tx_trace (u8 *s, va_list *args)
{
 ...
 */
static u8 *
mpls_tunnel_build_rewrite (vnet_main_t *vnm, u32 sw_if_index,
				...
				fib_node_index_t path_index, void *arg)
{
   ...
   */
  ctx->next_hops =
    fib_path_ext_stack (path_ext, ctx->fct, ctx->fct, ctx->next_hops);

  return (FIB_PATH_LIST_WALK_CONTINUE);
}

static void
mpls_tunnel_mk_lb (mpls_tunnel_t *mt, vnet_link_t linkt,
		   fib_forward_chain_type_t fct, dpo_id_t *dpo_lb)
{
  ...
  if (FIB_NODE_INDEX_INVALID != mt->mt_path_list)
    {
      fib_path_list_walk (mt->mt_path_list, mpls_tunnel_collect_forwarding,
			  &ctx);
    }
  ...
  if (!dpo_id_is_valid (dpo_lb))
	...
	{
	  dpo_set (dpo_lb, DPO_REPLICATE, lb_proto,
      ...
      else
	{
	  ...
	  dpo_set (dpo_lb, DPO_LOAD_BALANCE, lb_proto,
  ...
  if (mt->mt_flags & MPLS_TUNNEL_FLAG_MCAST)
    ...
    {
      load_balance_multipath_update (dpo_lb, ctx.next_hops,
...
mpls_tunnel_stack (adj_index_t ai)
{
   ...
   */
  if (vnet_hw_interface_get_flags (vnet_get_main (), mt->mt_hw_if_index) &
      VNET_HW_INTERFACE_FLAG_LINK_UP)
    {
      ...
      mpls_tunnel_mk_lb (
	mt, adj->ia_link,
	fib_forw_chain_type_from_link_type (adj_get_link_type (ai)), &dpo);
  ...
  mt->mt_sibling_index = fib_path_list_child_add (
    mt->mt_path_list, FIB_NODE_TYPE_MPLS_TUNNEL, mt - mpls_tunnel_pool);
...
mpls_adj_walk_cb (adj_index_t ai, void *ctx)
{
  ...
  if (mt->mt_flags & MPLS_TUNNEL_FLAG_L2)
    {
      ...
      mpls_tunnel_mk_lb (mt, VNET_LINK_MPLS, FIB_FORW_CHAIN_TYPE_ETHERNET,
			 &dpo);
...
static clib_error_t *
mpls_tunnel_admin_up_down (vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
 ...
 */
static void
mpls_tunnel_fixup (vlib_main_t *vm, const ip_adjacency_t *adj,
...
static void
mpls_tunnel_update_adj (vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
  ip_adjacency_t *adj;
  ...
  switch (adj->lookup_next_index)
    ...
    case IP_LOOKUP_NEXT_BCAST:
      adj_nbr_midchain_update_rewrite (ai, mpls_tunnel_fixup, NULL,
				       ADJ_FLAG_NONE,
    ...
    case IP_LOOKUP_NEXT_MCAST:
       ...
       */
      adj_mcast_midchain_update_rewrite (ai, mpls_tunnel_fixup, NULL,
					 ADJ_FLAG_NONE,
					 mpls_tunnel_build_rewrite_i (), 0, 0);
      break;
 ...
 */
void
mpls_tunnel_walk (mpls_tunnel_walk_cb_t cb, void *ctx)
{
  u32 mti;
...
u32
vnet_mpls_tunnel_create (u8 l2_only, u8 is_multicast, u8 *tag)
{
  vnet_hw_interface_t *hi;
  mpls_tunnel_t *mt;
  vnet_main_t *vnm;
   ...
   */
  mt->mt_hw_if_index =
    vnet_register_interface (vnm, mpls_tunnel_class.index, mti,
			     mpls_tunnel_hw_interface_class.index, mti);
...
void
vnet_mpls_tunnel_path_add (u32 sw_if_index, fib_route_path_t *rpaths)
{
  ...
  if (FIB_NODE_INDEX_INVALID == mt->mt_path_list)
    {
      mt->mt_path_list =
	fib_path_list_create (FIB_PATH_LIST_FLAG_SHARED, rpaths);
      mt->mt_sibling_index = fib_path_list_child_add (
  ...
  vec_foreach (rpath, rpaths)
    {
      fib_path_ext_list_insert (&mt->mt_path_exts, mt->mt_path_list,
				FIB_PATH_EXT_MPLS, rpath);
    }
  mpls_tunnel_restack (mt);
}

int
vnet_mpls_tunnel_path_remove (u32 sw_if_index, fib_route_path_t *rpaths)
{
  mpls_tunnel_t *mt;
  u32 mti;
...
static clib_error_t *
vnet_create_mpls_tunnel_command_fn (vlib_main_t *vm, unformat_input_t *input,
				    vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "del %U", unformat_vnet_sw_interface, vnm,
		    &sw_if_index))
	is_del = 1;
      else if (unformat (line_input, "add %U", unformat_vnet_sw_interface, vnm,
			 &sw_if_index))
	is_del = 0;
...
VLIB_CLI_COMMAND (create_mpls_tunnel_command, static) = {
  ...
  .short_help =
    ...
    "[resolve-via-host] [resolve-via-connected] [rx-ip4 <interface>] "
...
format_mpls_tunnel (u8 *s, va_list *args)
{
  ...
  s = format (s, "mpls-tunnel%d: sw_if_index:%d hw_if_index:%d",
	      mt - mpls_tunnel_pool, mt->mt_sw_if_index, mt->mt_hw_if_index);
...
static clib_error_t *
show_mpls_tunnel_command_fn (vlib_main_t *vm, unformat_input_t *input,
			     vlib_cli_command_t *cmd)
{
  mpls_tunnel_t *mt;
  u32 mti = ~0;
 ...
 *     index:26 pl-index:26 ipv4 weight=1 attached-nexthop:
 oper-flags:resolved,
 *      10.0.0.2 loop0
 *         [@0]: ipv4 via 10.0.0.2 loop0: IP4: de:ad:00:00:00:00 ->
 00:00:11:aa:bb:cc
 * @cliexend
 ?*/
...
mpls_tunnel_back_walk (fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
