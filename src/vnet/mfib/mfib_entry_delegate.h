...
#define FOR_EACH_MFIB_DELEGATE(_entry, _fdt, _fed, _body)                     \
  {                                                                           \
    for (_fdt = MFIB_ENTRY_DELEGATE_CHAIN_UNICAST_IP4;                        \
	 _fdt <= MFIB_ENTRY_DELEGATE_ATTACHED_EXPORT; _fdt++)                 \
      {                                                                       \
...
/**
 * A Delagate is a means to implmenet the Delagation design pattern; the
 * extension of an objects functionality through the composition of, and
 * delgation to, other objects. These 'other' objects are delegates. Delagates
 * are thus attached to other MFIB objects to extend their functionality.
 */
...
extern mfib_entry_delegate_t *
mfib_entry_delegate_find_or_add (struct mfib_entry_t_ *mfib_entry,
				 mfib_entry_delegate_type_t fdt);
extern mfib_entry_delegate_t *
mfib_entry_delegate_get (const struct mfib_entry_t_ *mfib_entry,
			 mfib_entry_delegate_type_t type);
