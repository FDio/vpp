		     ...
		     const mfib_prefix_t *prefix)
{
  switch (prefix->fp_proto)
    {
    case FIB_PROTOCOL_IP4:
      return (ip4_mfib_table_lookup (&mfib_table->v4, &prefix->fp_src_addr.ip4,
				     &prefix->fp_grp_addr.ip4,
				     prefix->fp_len));
    case FIB_PROTOCOL_IP6:
      return (ip6_mfib_table_lookup (&mfib_table->v6, &prefix->fp_src_addr.ip6,
...
{
  return (mfib_table_lookup_i (mfib_table_get (fib_index, prefix->fp_proto),
			       prefix));
}
				 ...
				 const mfib_prefix_t *prefix)
{
  switch (prefix->fp_proto)
    {
    case FIB_PROTOCOL_IP4:
      return (ip4_mfib_table_lookup_exact_match (
	&mfib_table->v4, &prefix->fp_grp_addr.ip4, &prefix->fp_src_addr.ip4,
	prefix->fp_len));
    case FIB_PROTOCOL_IP6:
      return (ip6_mfib_table_lookup_exact_match (
	&mfib_table->v6, &prefix->fp_grp_addr.ip6, &prefix->fp_src_addr.ip6,
	prefix->fp_len));
    case FIB_PROTOCOL_MPLS:
      break;
    }
...
{
  return (mfib_table_lookup_exact_match_i (
    mfib_table_get (fib_index, prefix->fp_proto), prefix));
}
				...
				const mfib_prefix_t *prefix)
{
  switch (prefix->fp_proto)
    {
    case FIB_PROTOCOL_IP4:
      return (ip4_mfib_table_get_less_specific (
	&mfib_table->v4, &prefix->fp_src_addr.ip4, &prefix->fp_grp_addr.ip4,
	prefix->fp_len));
    case FIB_PROTOCOL_IP6:
      return (ip6_mfib_table_get_less_specific (
	&mfib_table->v6, &prefix->fp_src_addr.ip6, &prefix->fp_grp_addr.ip6,
	prefix->fp_len));
    case FIB_PROTOCOL_MPLS:
      break;
    }
...
fib_node_index_t
mfib_table_get_less_specific (u32 fib_index, const mfib_prefix_t *prefix)
{
  return (mfib_table_get_less_specific_i (
    mfib_table_get (fib_index, prefix->fp_proto), prefix));
}

static void
mfib_table_entry_remove (mfib_table_t *mfib_table, const mfib_prefix_t *prefix,
			 fib_node_index_t mfib_entry_index)
{
  ...
  switch (prefix->fp_proto)
    {
    case FIB_PROTOCOL_IP4:
      ip4_mfib_table_entry_remove (&mfib_table->v4, &prefix->fp_grp_addr.ip4,
				   &prefix->fp_src_addr.ip4, prefix->fp_len);
      break;
    case FIB_PROTOCOL_IP6:
      ip6_mfib_table_entry_remove (&mfib_table->v6, &prefix->fp_grp_addr.ip6,
				   &prefix->fp_src_addr.ip6, prefix->fp_len);
      break;
    case FIB_PROTOCOL_MPLS:
      ASSERT (0);
      break;
  ...
  mfib_entry_cover_change_notify (mfib_entry_index, FIB_NODE_INDEX_INVALID);
				...
				fib_node_index_t mfib_entry_index)
{
   ...
   */
  mfib_entry_cover_index = mfib_table_get_less_specific_i (mfib_table, prefix);
  /*
...
static void
mfib_table_entry_insert (mfib_table_t *mfib_table, const mfib_prefix_t *prefix,
			 fib_node_index_t mfib_entry_index)
{
  vlib_smp_unsafe_warning ();
  ...
  switch (prefix->fp_proto)
    {
    case FIB_PROTOCOL_IP4:
      ip4_mfib_table_entry_insert (&mfib_table->v4, &prefix->fp_grp_addr.ip4,
				   &prefix->fp_src_addr.ip4, prefix->fp_len,
				   mfib_entry_index);
      break;
    case FIB_PROTOCOL_IP6:
      ip6_mfib_table_entry_insert (&mfib_table->v6, &prefix->fp_grp_addr.ip6,
				   &prefix->fp_src_addr.ip6, prefix->fp_len,
				   mfib_entry_index);
      break;
    case FIB_PROTOCOL_MPLS:
...
fib_node_index_t
mfib_table_entry_update (u32 fib_index, const mfib_prefix_t *prefix,
			 mfib_source_t source, fib_rpf_id_t rpf_id,
			 mfib_entry_flags_t entry_flags)
{
  ...
  if (FIB_NODE_INDEX_INVALID == mfib_entry_index)
      ...
      if (MFIB_ENTRY_FLAG_NONE != entry_flags)
	{
	   ...
	   */
	  mfib_entry_index = mfib_entry_create (
	    fib_index, source, prefix, rpf_id, entry_flags, INDEX_INVALID);
  ...
  else
    {
      ...
      if (mfib_entry_update (mfib_entry_index, source, entry_flags, rpf_id,
			     INDEX_INVALID))
	{
	  /*
				 ...
				 const fib_route_path_t *rpaths)
{
  ...
  if (FIB_NODE_INDEX_INVALID == mfib_entry_index)
	...
	mfib_entry_create (fib_index, source, prefix, MFIB_RPF_ID_NONE,
			   MFIB_ENTRY_FLAG_NONE, INDEX_INVALID);
			      ...
			      const fib_route_path_t *rpath)
{
  ...
  mfib_entry_index =
    mfib_table_entry_paths_update_i (fib_index, prefix, source, rpaths);
...
mfib_table_entry_paths_update (u32 fib_index, const mfib_prefix_t *prefix,
			       ...
			       const fib_route_path_t *rpaths)
{
  return (mfib_table_entry_paths_update_i (fib_index, prefix, source, rpaths));
}

static void
mfib_table_entry_paths_remove_i (u32 fib_index, const mfib_prefix_t *prefix,
				 mfib_source_t source,
				 const fib_route_path_t *rpaths)
{
  fib_node_index_t mfib_entry_index;
  ...
  if (FIB_NODE_INDEX_INVALID == mfib_entry_index)
  ...
  else
    {
      ...
      no_more_sources =
	mfib_entry_path_remove (mfib_entry_index, source, rpaths);
...
void
mfib_table_entry_paths_remove (u32 fib_index, const mfib_prefix_t *prefix,
			       mfib_source_t source,
			       const fib_route_path_t *rpaths)
{
  mfib_table_entry_paths_remove_i (fib_index, prefix, source, rpaths);
}

void
mfib_table_entry_path_remove (u32 fib_index, const mfib_prefix_t *prefix,
			      mfib_source_t source,
			      const fib_route_path_t *rpath)
{
  fib_route_path_t *rpaths = NULL;
...
fib_node_index_t
mfib_table_entry_special_add (u32 fib_index, const mfib_prefix_t *prefix,
			      mfib_source_t source,
			      mfib_entry_flags_t entry_flags, index_t repi)
{
  ...
  if (FIB_NODE_INDEX_INVALID == mfib_entry_index)
    {
      mfib_entry_index = mfib_entry_create (
	fib_index, source, prefix, MFIB_RPF_ID_NONE, entry_flags, repi);
...
static void
mfib_table_entry_delete_i (u32 fib_index, fib_node_index_t mfib_entry_index,
			   const mfib_prefix_t *prefix, mfib_source_t source)
{
  mfib_table_t *mfib_table;
...
void
mfib_table_entry_delete (u32 fib_index, const mfib_prefix_t *prefix,
			 mfib_source_t source)
{
  fib_node_index_t mfib_entry_index;
			       ...
			       mfib_source_t source)
{
  mfib_table_entry_delete_i (mfib_entry_get_fib_index (mfib_entry_index),
			     mfib_entry_index,
			     mfib_entry_get_prefix (mfib_entry_index), source);
}
...
static u32
mfib_table_find_or_create_and_lock_i (fib_protocol_t proto, u32 table_id,
				      mfib_source_t src, const u8 *name)
{
  mfib_table_t *mfib_table;
  fib_node_index_t fi;
  ...
  if (NULL == mfib_table->mft_desc)
	...
	{
	  mfib_table->mft_desc =
	    format (NULL, "%U-VRF:%d", format_fib_protocol, proto, table_id);
	}
    }
...
u32
mfib_table_find_or_create_and_lock (fib_protocol_t proto, u32 table_id,
				    mfib_source_t src)
{
...
u32
mfib_table_find_or_create_and_lock_w_name (fib_protocol_t proto, u32 table_id,
					   mfib_source_t src, const u8 *name)
{
...
static walk_rc_t
mfib_table_mark_cb (fib_node_index_t fib_entry_index, void *arg)
{
  mfib_table_flush_ctx_t *ctx = arg;
...
void
mfib_table_mark (u32 fib_index, fib_protocol_t proto, mfib_source_t source)
{
  mfib_table_flush_ctx_t ctx = {
    .mftf_source = source,
  };
  ...
  mfib_table_walk (fib_index, proto, mfib_table_mark_cb, &ctx);
}

static walk_rc_t
mfib_table_sweep_cb (fib_node_index_t fib_entry_index, void *arg)
{
  mfib_table_flush_ctx_t *ctx = arg;
...
void
mfib_table_sweep (u32 fib_index, fib_protocol_t proto, mfib_source_t source)
{
  mfib_table_flush_ctx_t ctx = {
    .mftf_source = source,
  };
  ...
  mfib_table_walk (fib_index, proto, mfib_table_sweep_cb, &ctx);
...
u32
mfib_table_get_n_routes (fib_node_index_t fib_index, fib_protocol_t proto)
{
  mfib_table_t *mfib_table;
...
void
mfib_table_walk (u32 fib_index, fib_protocol_t proto, mfib_table_walk_fn_t fn,
		 void *ctx)
{
  switch (proto)
    {
