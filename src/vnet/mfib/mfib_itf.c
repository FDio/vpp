...
int
mfib_itf_update (mfib_itf_t *mfib_itf, fib_node_index_t path_index,
		 mfib_itf_flags_t mfi_flags)
{
  ...
  if (MFIB_ITF_FLAG_NONE == mfi_flags)
    ...
    {
      mfib_itf->mfi_hash =
	hash_set (mfib_itf->mfi_hash, path_index, mfi_flags);
    }
...
static void
mfib_itf_prefix4_to_mac (const mfib_prefix_t *pfx, mac_address_t *mac)
{
  mac->bytes[0] = 0x01;
  mac->bytes[1] = 0x0;
  mac->bytes[2] = 0x5e;
...
static void
mfib_itf_prefix6_to_mac (const mfib_prefix_t *pfx, mac_address_t *mac)
{
  mac->bytes[0] = 0x33;
  mac->bytes[1] = 0x33;
...
static void
mfib_itf_prefix_to_mac (const mfib_prefix_t *pfx, mac_address_t *mac)
{
  switch (pfx->fp_proto)
    {
...
static void
mfib_itf_mac_add_del (mfib_itf_t *itf, const mfib_prefix_t *pfx, int add)
{
  ...
  vnet_hw_interface_add_del_mac_address (vnet_get_main (), si->hw_if_index,
					 mac.bytes, add);
}

void
mfib_itf_mac_add (mfib_itf_t *itf, const mfib_prefix_t *pfx)
{
  mfib_itf_mac_add_del (itf, pfx, 1);
}

void
mfib_itf_mac_del (mfib_itf_t *itf, const mfib_prefix_t *pfx)
{
  ...
  if (~0 != mfib_itf->mfi_sw_if_index)
    {
      return (format (s, " %U: %U", format_vnet_sw_interface_name, vnm,
		      vnet_get_sw_interface (vnm, mfib_itf->mfi_sw_if_index),
		      format_mfib_itf_flags, mfib_itf->mfi_flags));
    }
  else
    {
      return (
	format (s, " local: %U", format_mfib_itf_flags, mfib_itf->mfi_flags));
    }
  return (s);
}

static clib_error_t *
show_mfib_itf_command (vlib_main_t *vm, unformat_input_t *input,
...
/*?
 * This commnad displays an MFIB interface, or all interfaces, indexed by their
 unique
