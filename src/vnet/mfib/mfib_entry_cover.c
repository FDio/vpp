...
u32
mfib_entry_cover_track (mfib_entry_t *cover, fib_node_index_t covered)
{
  ...
  if (NULL == mfed)
    {
      mfed =
	mfib_entry_delegate_find_or_add (cover, MFIB_ENTRY_DELEGATE_COVERED);
      mfed->mfd_list = fib_node_list_create ();
    }

  return (fib_node_list_push_front (mfed->mfd_list, 0,
				    FIB_NODE_TYPE_MFIB_ENTRY, covered));
}

void
mfib_entry_cover_untrack (mfib_entry_t *cover, u32 tracked_index)
{
  mfib_entry_delegate_t *mfed;
...
static walk_rc_t
mfib_entry_cover_walk_node_ptr (fib_node_ptr_t *depend, void *args)
{
...
void
mfib_entry_cover_walk (mfib_entry_t *cover, mfib_entry_covered_walk_t walk,
		       void *args)
{
  mfib_entry_delegate_t *mfed;
  ...
  fib_node_list_walk (mfed->mfd_list, mfib_entry_cover_walk_node_ptr, &ctx);
}

static int
mfib_entry_cover_change_one (mfib_entry_t *cover, fib_node_index_t covered,
			     void *args)
{
				...
				fib_node_index_t covered)
{
  ...
  mfib_entry_cover_walk (cover, mfib_entry_cover_change_one,
			 uword_to_pointer (covered, void *));
}

static int
mfib_entry_cover_update_one (mfib_entry_t *cover, fib_node_index_t covered,
			     void *args)
{
...
{
  mfib_entry_cover_walk (mfib_entry, mfib_entry_cover_update_one, NULL);
}
