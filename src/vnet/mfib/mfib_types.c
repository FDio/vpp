...
int
mfib_prefix_is_cover (const mfib_prefix_t *p1, const mfib_prefix_t *p2)
{
  ...
  switch (p1->fp_proto)
    ...
    case FIB_PROTOCOL_IP4:
      return (ip4_destination_matches_route (
	&ip4_main, &p1->fp_grp_addr.ip4, &p2->fp_grp_addr.ip4, p1->fp_len));
    case FIB_PROTOCOL_IP6:
      return (ip6_destination_matches_route (
	&ip6_main, &p1->fp_grp_addr.ip6, &p2->fp_grp_addr.ip6, p1->fp_len));
...
clib_error_t *
mfib_show_route_flags (vlib_main_t *vm, unformat_input_t *main_input,
		       vlib_cli_command_t *cmd)
{
  ...
  FOR_EACH_MFIB_ATTRIBUTE (attr)
  {
    vlib_cli_output (vm, "%s = %s", mfib_flag_names[attr],
...
clib_error_t *
mfib_show_itf_flags (vlib_main_t *vm, unformat_input_t *main_input,
		     vlib_cli_command_t *cmd)
{
  mfib_itf_attribute_t attr;
  ...
  FOR_EACH_MFIB_ITF_ATTRIBUTE (attr)
  {
    vlib_cli_output (vm, "%s = %s", mfib_itf_flag_names[attr],
...
/*?
 * This command displays the set of supported flags applicable to an MFIB
 * interface
 */
