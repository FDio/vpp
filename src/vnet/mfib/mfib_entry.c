...
format_mfib_entry_dpo (u8 *s, va_list *args)
{
  ...
  return (format (s, "%U", format_mfib_entry, fei, MFIB_ENTRY_FORMAT_BRIEF));
}
...
format_mfib_entry_path_ext (u8 *s, va_list *args)
{
  ...
  return (format (s, "path:%d flags:%U", path_ext->mfpe_path,
...
format_mfib_entry (u8 *s, va_list *args)
{
  ...
  s = format (s, "\n  %U-chain\n  %U", format_fib_forw_chain_type,
	      mfib_entry_get_default_chain_type (mfib_entry), format_dpo_id,
	      &mfib_entry->mfe_rep, 2);
  s = format (s, "\n");
...
static int
mfib_entry_src_cmp_for_sort (void *v1, void *v2)
{
...
static void
mfib_entry_src_init (mfib_entry_t *mfib_entry, mfib_source_t source)

{
  ...
  vec_sort_with_function (mfib_entry->mfe_srcs, mfib_entry_src_cmp_for_sort);
}

static mfib_entry_src_t *
mfib_entry_src_find (const mfib_entry_t *mfib_entry, mfib_source_t source,
		     u32 *index)
...
static mfib_entry_src_t *
mfib_entry_src_find_or_create (mfib_entry_t *mfib_entry, mfib_source_t source)
{
  mfib_entry_src_t *msrc;
...
static mfib_entry_src_t *
mfib_entry_src_update (mfib_entry_t *mfib_entry, mfib_source_t source,
...
static mfib_entry_src_t *
mfib_entry_src_update_and_lock (mfib_entry_t *mfib_entry, mfib_source_t source,
				fib_rpf_id_t rpf_id,
...
int
mfib_entry_is_sourced (fib_node_index_t mfib_entry_index, mfib_source_t source)
{
  mfib_entry_t *mfib_entry;
...
int
mfib_entry_is_marked (fib_node_index_t mfib_entry_index, mfib_source_t source)
{
  mfib_entry_t *mfib_entry;
  mfib_entry_src_t *esrc;
...
void
mfib_entry_mark (fib_node_index_t fib_entry_index, mfib_source_t source)
{
  mfib_entry_t *mfib_entry;
  mfib_entry_src_t *esrc;
...
static void
mfib_entry_src_remove (mfib_entry_t *mfib_entry, mfib_source_t source)
...
mfib_entry_child_add (fib_node_index_t mfib_entry_index,
		      fib_node_type_t child_type, fib_node_index_t child_index)
{
  return (fib_node_child_add (FIB_NODE_TYPE_MFIB_ENTRY, mfib_entry_index,
			      child_type, child_index));
};

void
mfib_entry_child_remove (fib_node_index_t mfib_entry_index, u32 sibling_index)
{
  fib_node_child_remove (FIB_NODE_TYPE_MFIB_ENTRY, mfib_entry_index,
			 sibling_index);
}

static mfib_entry_t *
mfib_entry_alloc (u32 fib_index, const mfib_prefix_t *prefix,
		  fib_node_index_t *mfib_entry_index)
{
  ...
  fib_node_init (&mfib_entry->mfe_node, FIB_NODE_TYPE_MFIB_ENTRY);
...
static inline mfib_path_ext_t *
mfib_entry_path_ext_find (mfib_path_ext_t *exts, fib_node_index_t path_index)
{
  uword *p;
...
mfib_path_ext_add (mfib_entry_src_t *msrc, fib_node_index_t path_index,
		   mfib_itf_flags_t mfi_flags)
{
  ...
  hash_set (msrc->mfes_exts, path_index, path_ext - mfib_path_ext_pool);

  return (path_ext);
}

static void
mfib_path_ext_remove (mfib_entry_src_t *msrc, fib_node_index_t path_index)
{
  mfib_path_ext_t *path_ext;
				   ...
				   fib_node_index_t path_index, void *arg)
{
  ...
  path_ext = mfib_entry_path_ext_find (ctx->msrc->mfes_exts, path_index);

  if (NULL != path_ext && !(path_ext->mfpe_flags & MFIB_ITF_FLAG_FORWARD))
    {
      return (FIB_PATH_LIST_WALK_CONTINUE);
    }
...
static void
mfib_entry_stack (mfib_entry_t *mfib_entry, mfib_entry_src_t *msrc)
{
  dpo_proto_t dp;
  ...
  if (FIB_NODE_INDEX_INVALID != mfib_entry->mfe_pl)
    {
      fib_path_list_child_remove (mfib_entry->mfe_pl, mfib_entry->mfe_sibling);
    }
  ...
  if (NULL != msrc)
	...
	.msrc = msrc,
      ...
      if (FIB_NODE_INDEX_INVALID != mfib_entry->mfe_pl)
	{
	  mfib_entry->mfe_sibling = fib_path_list_child_add (
	    mfib_entry->mfe_pl, FIB_NODE_TYPE_MFIB_ENTRY,
	    mfib_entry_get_index (mfib_entry));

	  fib_path_list_walk (mfib_entry->mfe_pl,
			      mfib_entry_src_collect_forwarding, &ctx);
	}
	  ...
	  if (NULL == ctx.next_hops)
	    {
	       ...
	       */
	      dpo_stack (DPO_MFIB_ENTRY, dp, &mfib_entry->mfe_rep,
			 drop_dpo_get (dp));
	    }
	  else
	    {
		  ...
		  dpo_is_drop (&mfib_entry->mfe_rep))
		{
		  ...
		  dpo_set (&tmp_dpo, DPO_REPLICATE, dp,
	      ...
	      replicate_multipath_update (&mfib_entry->mfe_rep, ctx.next_hops);
	    }
	}
      else
	{
    ...
    {
      dpo_stack (DPO_MFIB_ENTRY, dp, &mfib_entry->mfe_rep, drop_dpo_get (dp));
    }
  ...
  if (FIB_NODE_INDEX_INVALID == msrc->mfes_pl)
    {
      ...
      msrc->mfes_pl = fib_path_list_create (FIB_PATH_LIST_FLAG_NO_URPF, NULL);
...
fib_node_index_t
mfib_entry_create (u32 fib_index, mfib_source_t source,
		   const mfib_prefix_t *prefix, fib_rpf_id_t rpf_id,
		   mfib_entry_flags_t entry_flags, index_t repi)
{
  fib_node_index_t mfib_entry_index;
  mfib_entry_t *mfib_entry;
  mfib_entry_src_t *msrc;

  mfib_entry = mfib_entry_alloc (fib_index, prefix, &mfib_entry_index);
  msrc = mfib_entry_src_update (mfib_entry, source, rpf_id, entry_flags);
  ...
  if (INDEX_INVALID != repi)
    {
      ...
      msrc->mfes_pl =
	fib_path_list_create_special (dp, FIB_PATH_LIST_FLAG_EXCLUSIVE, &dpo);
...
static void
mfib_entry_update_i (mfib_entry_t *mfib_entry, mfib_entry_src_t *msrc,
		     mfib_source_t current_best, index_t repi)
{
  if (INDEX_INVALID != repi)
    {
      /*
...
mfib_entry_special_add (fib_node_index_t mfib_entry_index,
			mfib_source_t source, mfib_entry_flags_t entry_flags,
			fib_rpf_id_t rpf_id, index_t repi)
{
  ...
  msrc =
    mfib_entry_src_update_and_lock (mfib_entry, source, rpf_id, entry_flags);
...
int
mfib_entry_update (fib_node_index_t mfib_entry_index, mfib_source_t source,
		   mfib_entry_flags_t entry_flags, fib_rpf_id_t rpf_id,
		   index_t repi)
{
  mfib_source_t current_best;
  mfib_entry_t *mfib_entry;
...
static void
mfib_entry_itf_add (mfib_entry_src_t *msrc, u32 sw_if_index, index_t mi)
{
  hash_set (msrc->mfes_itfs, sw_if_index, mi);
}

static void
mfib_entry_itf_remove (mfib_entry_src_t *msrc, u32 sw_if_index)
{
  mfib_itf_t *mfi;
...
mfib_entry_path_update (fib_node_index_t mfib_entry_index,
			mfib_source_t source, const fib_route_path_t *rpaths)
{
  ...
  vec_foreach_index (ii, path_indices)
      ...
      if (NULL == path_ext)
	{
	  ...
	  path_ext =
	    mfib_path_ext_add (msrc, path_index, rpath->frp_mitf_flags);
	}
      else
	{
	  ...
	  if (old != rpath->frp_mitf_flags)
	    {
	       ...
	       */
	      mfib_itf =
		mfib_entry_itf_find (msrc->mfes_itfs, rpath->frp_sw_if_index);

	      if (NULL == mfib_itf)
		{
		  index_t mfib_itf_i =
		    mfib_itf_create (path_index, rpath->frp_mitf_flags);
		  mfib_entry_itf_add (msrc, rpath->frp_sw_if_index,
				      mfib_itf_i);
	      ...
	      else
		{
		  ...
		  u8 is_accept =
		    !!(rpath->frp_mitf_flags & MFIB_ITF_FLAG_ACCEPT);

		  if (mfib_itf_update (mfib_itf, path_index,
			...
			mfib_source_t source, const fib_route_path_t *rpaths)
{
  ...
  vec_foreach_index (ii, path_indices)
      ...
      if (mfib_entry_path_itf_based (rpath))
	{
	  ...
	  if (mfib_itf_update (mfib_itf, path_index, MFIB_ITF_FLAG_NONE))
	    {
	      if (was_accept)
		{
		  mfib_itf_mac_del (mfib_itf, pfx);
 ...
 */
int
mfib_entry_delete (fib_node_index_t mfib_entry_index, mfib_source_t source)
{
  mfib_source_t current_best;
  mfib_entry_t *mfib_entry;
...
static int
fib_ip4_address_compare (ip4_address_t *a1, ip4_address_t *a2)
{
  /*
...
static int
fib_ip6_address_compare (ip6_address_t *a1, ip6_address_t *a2)
{
  int i;
		...
		fib_node_index_t mfib_entry_index2)
{
  ...
  switch (mfib_entry1->mfe_prefix.fp_proto)
	...
	{
	  cmp =
	    fib_ip4_address_compare (&mfib_entry1->mfe_prefix.fp_src_addr.ip4,
	...
	{
	  cmp =
	    fib_ip6_address_compare (&mfib_entry1->mfe_prefix.fp_src_addr.ip6,
...
mfib_entry_cmp_for_sort (void *i1, void *i2)
{
  ...
  return (mfib_entry_cmp (*mfib_entry_index1, *mfib_entry_index2));
}
...
mfib_entry_back_walk_notify (fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
  mfib_entry_t *mfib_entry;
...
{
  fib_show_memory_usage ("multicast-Entry", pool_elts (mfib_entry_pool),
			 pool_len (mfib_entry_pool), sizeof (mfib_entry_t));
}
...
mfib_entry_encode (fib_node_index_t mfib_entry_index)
{
  ...
  if (FIB_NODE_INDEX_INVALID != bsrc->mfes_pl)
    {
      fib_path_list_walk_w_ext (bsrc->mfes_pl, NULL, fib_path_encode, &ctx);
    }
  ...
  vec_foreach (rpath, ctx.rpaths)
    {
      ...
      mfib_itf = mfib_entry_itf_find (bsrc->mfes_itfs, rpath->frp_sw_if_index);
      if (mfib_itf)
	{
...
mfib_entry_contribute_forwarding (fib_node_index_t mfib_entry_index,
				  ...
				  mfib_entry_fwd_flags_t flags, dpo_id_t *dpo)
{
  /*
...
static clib_error_t *
show_mfib_entry_command (vlib_main_t *vm, unformat_input_t *input,
			 vlib_cli_command_t *cmd)
{
  ...
  if (unformat (input, "%d", &fei))
	...
	{
	  vlib_cli_output (vm, "%d@%U", fei, format_mfib_entry, fei,
...
/*?
 * This commnad displays an entry, or all entries, in the mfib tables indexed
 by their unique
