	...
	if (IP6_MFIB_SPECIAL_TYPE_SOLICITED == _spec->ims_type)               \
	  ...
	  {                                                                   \
	    ip6_set_reserved_multicast_address (                              \
	      &(_pfx)->fp_grp_addr.ip6, _spec->ims_scope, _spec->ims_byte);   \
	  }                                                                   \
...
ip6_create_mfib_with_table_id (u32 table_id, mfib_source_t src)
{
  ...
  FOR_EACH_IP6_SPECIAL (&pfx, ({
    mfib_table_entry_path_update (mfib_table->mft_index, &pfx,
				  MFIB_SOURCE_SPECIAL, &path_for_us);
  }));
...
ip6_mfib_table_destroy (ip6_mfib_t *mfib)
{
   ...
   */
  FOR_EACH_IP6_SPECIAL (&pfx, {
    mfib_table_entry_path_remove (mfib_table->mft_index, &pfx,
				  MFIB_SOURCE_SPECIAL, &path_for_us);
  });
...
ip6_mfib_interface_enable_disable (u32 sw_if_index, int is_enable)
{
  ...
  if (is_enable)
    ...
    {
      FOR_EACH_IP6_SPECIAL (&pfx, {
	mfib_table_entry_path_remove (mfib_index, &pfx, MFIB_SOURCE_SPECIAL,
				      &path);
      });
    }
}
				   ...
				   const ip6_address_t *src, u32 len)
{
  ...
  rv =
    clib_bihash_search_inline_2_40_8 (&ip6_mfib_table.ip6_mhash, &key, &value);
  if (rv == 0)
    return value.value;
 ...
 */
fib_node_index_t
ip6_mfib_table_fwd_lookup (const ip6_mfib_t *mfib, const ip6_address_t *src,
			   const ip6_address_t *grp)
{
 ...
 */
fib_node_index_t
ip6_mfib_table_lookup (const ip6_mfib_t *mfib, const ip6_address_t *src,
		       const ip6_address_t *grp, u32 len)
{
  ip6_mfib_table_instance_t *table;
  ip6_mfib_key_t key, value;
...
void
ip6_mfib_table_entry_insert (ip6_mfib_t *mfib, const ip6_address_t *grp,
			     const ip6_address_t *src, u32 len,
			     fib_node_index_t mfib_entry_index)
{
  ...
  if (0 == table->dst_address_length_refcounts[len]++)
    {
      table->non_empty_dst_address_length_bitmap = clib_bitmap_set (
	table->non_empty_dst_address_length_bitmap, 256 - len, 1);
      compute_prefix_lengths_in_search_order (table);
    }
}

void
ip6_mfib_table_entry_remove (ip6_mfib_t *mfib, const ip6_address_t *grp,
			     const ip6_address_t *src, u32 len)
{
  ip6_mfib_table_instance_t *table;
  ip6_mfib_key_t key;
  ...
  if (--table->dst_address_length_refcounts[len] == 0)
    {
      table->non_empty_dst_address_length_bitmap = clib_bitmap_set (
	table->non_empty_dst_address_length_bitmap, 256 - len, 0);
      compute_prefix_lengths_in_search_order (table);
    }
}
...
format_ip6_mfib_table_memory (u8 *s, va_list *args)
{
  ...
  s = format (s, "%=30s %=6d %=12ld\n", "IPv6 multicast",
	      pool_elts (ip6_main.mfibs), bytes_inuse);

  return (s);
}

static void
ip6_mfib_table_show_one (ip6_mfib_t *mfib, vlib_main_t *vm, ip6_address_t *src,
			 ip6_address_t *grp, u32 mask_len, u32 cover)
{
  if (cover)
    {
      vlib_cli_output (
	vm, "%U", format_mfib_entry,
...
static int
ip6_mfib_walk_cb (clib_bihash_kv_40_8_t *kvp, void *arg)
{
...
void
ip6_mfib_table_walk (ip6_mfib_t *mfib, mfib_table_walk_fn_t fn, void *arg)
{
  ...
  clib_bihash_foreach_key_value_pair_40_8 (&ip6_mfib_table.ip6_mhash,
					   ip6_mfib_walk_cb, &ctx);
}

static clib_error_t *
ip6_show_mfib (vlib_main_t *vm, unformat_input_t *input,
	       vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	verbose = 0;
      ...
      else if (unformat (input, "%U %U", unformat_ip6_address, &src,
  ...
  pool_foreach (mfib_table, im6->mfibs)
	...
	continue;
      ...
      vlib_cli_output (vm, "%U, fib_index %d", format_mfib_table_name,
		       mfib->index, FIB_PROTOCOL_IP6, mfib->index);
 ...
 *   [0] [@1]: ipv6-mcast: test-eth1: IP6: d0:d1:d2:d3:d4:01 ->
 * 01:00:05:00:00:00 [1] [@1]: ipv6-mcast: test-eth2: IP6: d0:d1:d2:d3:d4:02 ->
 * 01:00:05:00:00:00
 *
 * @cliexend
...
{
  clib_bihash_init_40_8 (&ip6_mfib_table.ip6_mhash, "ip6 mFIB table",
