...
#define FOR_EACH_MFIB_ATTRIBUTE(_item)                                        \
  for (_item = MFIB_ENTRY_ATTRIBUTE_FIRST;                                    \
       _item <= MFIB_ENTRY_ATTRIBUTE_LAST; _item++)

#define MFIB_ENTRY_NAMES_SHORT                                                \
  {                                                                           \
    [MFIB_ENTRY_SIGNAL] = "S", [MFIB_ENTRY_CONNECTED] = "C",                  \
    [MFIB_ENTRY_DROP] = "D", [MFIB_ENTRY_ACCEPT_ALL_ITF] = "AA",              \
    [MFIB_ENTRY_INHERIT_ACCEPT] = "IA", [MFIB_ENTRY_EXCLUSIVE] = "E",         \
  }

#define MFIB_ENTRY_NAMES_LONG                                                 \
  {                                                                           \
    [MFIB_ENTRY_SIGNAL] = "Signal", [MFIB_ENTRY_CONNECTED] = "Connected",     \
...
#define FOR_EACH_MFIB_ITF_ATTRIBUTE(_item)                                    \
  for (_item = MFIB_ITF_ATTRIBUTE_FIRST; _item <= MFIB_ITF_ATTRIBUTE_LAST;    \
       _item++)

#define MFIB_ITF_NAMES_SHORT                                                  \
  {                                                                           \
    [MFIB_ITF_NEGATE_SIGNAL] = "NS", [MFIB_ITF_ACCEPT] = "A",                 \
    [MFIB_ITF_FORWARD] = "F", [MFIB_ITF_SIGNAL_PRESENT] = "SP",               \
    [MFIB_ITF_DONT_PRESERVE] = "DP",                                          \
  }

#define MFIB_ITF_NAMES_LONG                                                   \
  {                                                                           \
    [MFIB_ITF_NEGATE_SIGNAL] = "Negate-Signal", [MFIB_ITF_ACCEPT] = "Accept", \
...
#define MFIB_SOURCE_NAMES                                                     \
  {                                                                           \
    [MFIB_SOURCE_SPECIAL] = "Special", [MFIB_SOURCE_6RD] = "6RD",             \
    [MFIB_SOURCE_API] = "API", [MFIB_SOURCE_CLI] = "CLI",                     \
    [MFIB_SOURCE_DHCP] = "DHCP", [MFIB_SOURCE_VXLAN] = "VXLAN",               \
    [MFIB_SOURCE_SRv6] = "SRv6", [MFIB_SOURCE_GTPU] = "GTPU",                 \
    [MFIB_SOURCE_VXLAN_GPE] = "VXLAN-GPE", [MFIB_SOURCE_GENEVE] = "Geneve",   \
    [MFIB_SOURCE_IGMP] = "IGMP", [MFIB_SOURCE_VXLAN_GBP] = "VXLAN-GBP",       \
...
#define FOREACH_MFIB_SOURCE(_ms)                                              \
  for (_ms = MFIB_SOURCE_SPECIAL; _ms <= MFIB_SOURCE_DEFAULT_ROUTE; _ms++)
