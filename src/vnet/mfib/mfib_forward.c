...
format_mfib_forward_lookup_trace (u8 *s, va_list *args)
{
  ...
  mfib_forward_lookup_trace_t *t =
    va_arg (*args, mfib_forward_lookup_trace_t *);
...
static void
mfib_forward_lookup_trace (vlib_main_t *vm, vlib_node_runtime_t *node,
			   vlib_frame_t *frame)
{
  u32 *from, n_left;
...
static uword
mfib_forward_lookup (vlib_main_t *vm, vlib_node_runtime_t *node,
		     vlib_frame_t *frame, int is_v4)
{
  ...
  while (n_left_from > 0)
    {
      vlib_get_next_frame (vm, node, MFIB_FORWARD_LOOKUP_NEXT_RPF, to_next,
			   n_left_to_next);
...
VLIB_NODE_FN (ip6_mfib_forward_lookup_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static int
mfib_forward_connected_check (vlib_buffer_t *b0, u32 sw_if_index, int is_v4)
{
  /*
  ...
  if (is_v4)
	...
	&ip0->src_address);
      ...
      return (FIB_ENTRY_FLAG_ATTACHED & lb0->lb_fib_entry_flags);
    }
  else
    {
      ASSERT (0);
...
static void
mfib_forward_itf_signal (vlib_main_t *vm, const mfib_entry_t *mfe,
			 mfib_itf_t *mfi, vlib_buffer_t *b0)
{
  mfib_itf_flags_t old_flags;

  old_flags =
    clib_atomic_fetch_or (&mfi->mfi_flags, MFIB_ITF_FLAG_SIGNAL_PRESENT);
...
always_inline uword
mfib_forward_rpf (vlib_main_t *vm, vlib_node_runtime_t *node,
		  vlib_frame_t *frame, int is_v4)
{
  u32 n_left_from, n_left_to_next, *from, *to_next;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  mfi0 =
	    mfib_entry_get_itf (mfe0, vnet_buffer (b0)->sw_if_index[VLIB_RX]);
	  ...
	  if (PREDICT_FALSE (eflags0 & MFIB_ENTRY_FLAG_CONNECTED))
	    {
	      ...
	      if (mfib_forward_connected_check (
		    b0, vnet_buffer (b0)->sw_if_index[VLIB_RX], is_v4))
		{
...
VLIB_NODE_FN (ip4_mfib_forward_rpf_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
VLIB_NODE_FN (ip6_mfib_forward_rpf_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
