...
{
    fib_node_index_t mfei, ai_1, ai_2, ai_3, dflt_mfei;
    ...
    if (FIB_PROTOCOL_IP6 == PROTO)
    {
        mfei = ip6_mfib_table_lookup2(ip6_mfib_get(fib_index),
                                      &all_1s.fp_src_addr.ip6,
                                      &all_1s.fp_grp_addr.ip6);
        MFIB_TEST((dflt_mfei == mfei),
                  "%U found via DP LPM: %d",
                  format_mfib_prefix, &all_1s, mfei);
    }
    ...
    if (FIB_PROTOCOL_IP6 == PROTO)
    {
        ...
        MFIB_TEST(mfib_test_entry(mfei,
                                  ...
                                  0),
                  "(*,*) no replcaitions");
        MFIB_TEST(dflt_mfei == mfei, "found (*,*)");
        mfei = ip6_mfib_table_lookup2(ip6_mfib_get(fib_index),
                                      &all_1s.fp_src_addr.ip6,
                                      &all_1s.fp_grp_addr.ip6);
        MFIB_TEST((dflt_mfei == mfei),
    ...
    MFIB_TEST(mfei == mfib_table_lookup(fib_index, pfx_star_g_slash_m),
              "%U found via LPM",
              format_mfib_prefix, pfx_star_g_slash_m);
    if (FIB_PROTOCOL_IP6 == PROTO)
    {
        MFIB_TEST((mfei == ip6_mfib_table_lookup2(
                               ip6_mfib_get(fib_index),
                               &pfx_star_g_slash_m->fp_src_addr.ip6,
