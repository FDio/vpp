...
ip4_create_mfib_with_table_id (u32 table_id, mfib_source_t src)
{
   ...
   */
  mfib_table_entry_update (mfib_table->mft_index, &all_zeros,
			   MFIB_SOURCE_DEFAULT_ROUTE, MFIB_RPF_ID_NONE,
...
ip4_mfib_table_destroy (ip4_mfib_t *mfib)
{
   ...
   */
  mfib_table_entry_delete (mfib_table->mft_index, &all_zeros,
			   MFIB_SOURCE_DEFAULT_ROUTE);

  for (ii = 0; ii < ARRAY_LEN (ip4_specials); ii++)
    {
      mfib_table_entry_delete (mfib_table->mft_index, &ip4_specials[ii],
			       MFIB_SOURCE_SPECIAL);
    }
...
#define IP4_MFIB_MK_KEY(_grp, _src, _len, _key)                               \
  {                                                                           \
    _key =                                                                    \
      ((u64) (_grp->data_u32 & ip4_main.fib_masks[IPV4_MFIB_GRP_LEN (_len)])) \
      << 32;                                                                  \
    _key |= _src->data_u32;                                                   \
  }
 ...
 */
fib_node_index_t
ip4_mfib_table_lookup (const ip4_mfib_t *mfib, const ip4_address_t *src,
...
void
ip4_mfib_table_entry_insert (ip4_mfib_t *mfib, const ip4_address_t *grp,
			     const ip4_address_t *src, u32 len,
...
void
ip4_mfib_table_entry_remove (ip4_mfib_t *mfib, const ip4_address_t *grp,
			     const ip4_address_t *src, u32 len)
{
  uword *hash, *result;
  u64 key;
...
ip4_mfib_table_show_all (ip4_mfib_t *mfib, vlib_main_t *vm)
{
  ...
  for (i = 0; i < ARRAY_LEN (mfib->fib_entry_by_dst_address); i++)
      ...
      if (NULL != hash)
	{
	  ...
	  hash_foreach_pair (
	    p, hash, ({ vec_add1 (mfib_entry_indicies, p->value[0]); }));
	}
    }
  ...
  vec_foreach (mfib_entry_index, mfib_entry_indicies)
    {
      vlib_cli_output (vm, "%U", format_mfib_entry, *mfib_entry_index,
...
static void
ip4_mfib_table_show_one (ip4_mfib_t *mfib, vlib_main_t *vm, ip4_address_t *src,
			 ip4_address_t *grp, u32 mask_len)
{
  vlib_cli_output (vm, "%U", format_mfib_entry,
		   ip4_mfib_table_lookup (mfib, src, grp, mask_len),
		   MFIB_ENTRY_FORMAT_DETAIL);
}

static clib_error_t *
ip4_show_mfib (vlib_main_t *vm, unformat_input_t *input,
	       vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	memory = 1;
      else if (unformat (input, "%U %U", unformat_ip4_address, &src,
  ...
  pool_foreach (mfib_table, im4->mfibs)
      ...
      if (memory)
	{
	  ...
	  if (verbose)
	    vlib_cli_output (vm, "%U hash:%d", format_mfib_table_name,
			     mfib->index, FIB_PROTOCOL_IP4, hash_size);
	  total_hash_memory += hash_size;
	  continue;
	}

      vlib_cli_output (vm, "%U, fib_index:%d flags:%U", format_mfib_table_name,
		       mfib->index, FIB_PROTOCOL_IP4, mfib->index,
 ...
 *   [0] [@1]: ipv4-mcast: test-eth1: IP4: d0:d1:d2:d3:d4:01 ->
 01:00:05:00:00:00
 *   [1] [@1]: ipv4-mcast: test-eth2: IP4: d0:d1:d2:d3:d4:02 ->
 01:00:05:00:00:00
 *
 * @cliexend
...
VLIB_CLI_COMMAND (ip4_show_mfib_command, static) = {
  .path = "show ip mfib",
  .short_help = "show ip mfib [summary] [table <table-id>] [index <fib-id>] "
		"[<grp-addr>[/<mask>]] [<grp-addr>] [<src-addr> <grp-addr>]",
