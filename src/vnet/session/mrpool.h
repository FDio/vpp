...
{
  mrpool_overflow_vec_t *next = 0, *mh = &od->head;
...
#define mrpool_realloc(P)                                                     \
  do                                                                          \
    {                                                                         \
      mrpool_header_t *_p = (mrpool_header_t *) pool_header (P);              \
      u32 len, pending_len, oit_index, ofv_elts, del_elts = 0;                \
      typeof (P) oit;                                                         \
      if (!_p->expand_pending)                                                \
	break;                                                                \
      ...
      while (_ofv)                                                            \
	{                                                                     \
	  clib_memcpy_fast (&(P)[len - del_elts], _ovf->data,                 \
			    pending_len * sizeof (P[0]));                     \
	  ofv_elts = vec_len (_ovf->data);                                    \
