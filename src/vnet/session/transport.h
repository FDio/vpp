...
typedef struct _transport_proto_vft
{
  ...
  session_handle_t (*get_next_transport_session) (u32 conn_idx,
...
transport_get_next_transport_session (transport_proto_t tp, u32 conn_index,
				      u8 thread_index)
{
  if (!tp_vfts[tp].get_next_transport_session)
    return SESSION_INVALID_HANDLE;
  return tp_vfts[tp].get_next_transport_session (conn_index, thread_index);
}
