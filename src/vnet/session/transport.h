...
typedef struct _transport_proto_vft
{
  ...
  void (*handle_icmp) (transport_connection_t *tconn, u8 icmp_type,
		       u8 icmp_code);
...
/* Leaving tconn here as we have to make sure the connection exists before
   calling this in icmp4.c. Additionally, I don't think that we're going to
   call this from anywhere else so maybe it's better to not retrieve the
   connection twice? */
static inline void
