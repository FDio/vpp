				...
				int *n_tx_packets, u8 peek_data)
{
  ...
  n_bufs_per_seg =
    ceil ((double) (MAX_HDRS_LEN + snd_mss0) / n_bytes_per_buf);
  n_bufs_per_evt =
    (ceil ((double) max_len_to_snd0 / (MAX_HDRS_LEN + snd_mss0))) *
    n_bufs_per_seg;
	  ...
	  if (PREDICT_FALSE (n_bufs_per_seg > 1))
	      ...
	      for (j = 1; j < n_bufs_per_seg; j++)
		{
		  ...
		  if (peek_data)
		    ...
		    {
		      n_bytes_read =
			svm_fifo_dequeue_nowait (s0->server_tx_fifo,
						 len_to_deq0, data0);
		    }
