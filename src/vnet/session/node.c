				...
				int *n_tx_packets, u8 peek_data)
{
  ...
  u32 next_index, next0, *to_next, n_left_to_next, bi0, chain_bi0;
  vlib_buffer_t *b0, *chain_b0, *prev_b0;
  u32 rx_offset = 0, max_dequeue0;
  u16 snd_mss0, n_bufs_per_seg;
  u8 *data0;
  int i, j, n_bytes_read;
  ...
  n_bufs_per_seg = ceil ((double) (MAX_HDRS_LEN + snd_mss0) / n_bytes_per_buf);
  n_bufs_per_evt = (ceil ((double) max_len_to_snd0 / (MAX_HDRS_LEN + snd_mss0)))
      * n_bufs_per_seg;
  ...
  for (i = 0; i < n_frames_per_evt; i++)
      ...
      while (left_to_snd0 && n_left_to_next >= n_bufs_per_seg)
	{
	  ...
	  /* Ask transport to push header after current_length is updated */
