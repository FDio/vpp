...
send_session_accept_callback (session_t * s)
{
  ...
  if (session_has_transport (s))
      ...
      if (application_is_proxy (server))
	{
      ...
      vpp_queue = session_get_vpp_event_queue (s->thread_index);
...
mq_send_session_accepted_cb (session_t * s)
{
  ...
  if (session_has_transport (s))
  ...
  else
    {
      ...
      mp->handle = session_handle (s);
      mp->port = ct->c_rmt_port;
      vpp_queue = session_get_vpp_event_queue (main_thread);
      mp->vpp_event_queue_address = pointer_to_uword (vpp_queue);
      mp->client_event_queue_address = ct->client_evt_q;
      mp->server_event_queue_address = ct->server_evt_q;
    }
			      ...
			      session_t * s, u8 is_fail)
{
  ...
  if (session_has_transport (s))
      ...
      if (!tc)
	{
      ...
      vpp_mq = session_get_vpp_event_queue (s->thread_index);
      mp->handle = session_handle (s);
  ...
  else
    {
      ...
      vpp_mq = session_get_vpp_event_queue (main_thread);
      mp->vpp_event_queue_address = pointer_to_uword (vpp_mq);
			  ...
			  session_handle_t handle, int rv)
{
  ...
  vpp_evt_q = session_get_vpp_event_queue (0);
...
vl_api_application_attach_t_handler (vl_api_application_attach_t * mp)
{
  ...
  if ((evt_q_segment = session_get_evt_q_segment ()))
    {
