  ...
  if (!rv)
    {
      ...
      rmp->n_fds = n_fds;
      rmp->fd_flags = fd_flags;
      /* No segment_name and segment_handle since we only support memfds
...
session_api_sock_read_ready (clib_file_t * cf)
{
  ...
  switch (msg.type)
  {
    ...
    case APP_API_MSG_TYPE_ADD_DEL_WORKER:
      session_api_add_worker_handler (app_ns, cs, &msg.attach);
      break;
    default:
...
session_api_sock_error (clib_file_t * cf)
{
  ...
  /* An invalid api client index frees the applications */
  a->api_client_index = APP_INVALID_INDEX;
  vnet_application_detach (a);
  return 0;
}
...
//  cf.private_data = listener_handle;
  handle.l_index = app_namespace_index (app_ns);
  handle.u_index = s - app_ns->app_sockets;
  cf.private_data = handle;
