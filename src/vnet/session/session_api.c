...
static int
send_app_cut_through_registration_add (u32 api_client_index, svm_msg_q_t *mq,
                                       svm_msg_q_t *peer_mq)
{
  ...
  mp->evt_q_address = pointer_to_uword (mq);
  mp->peer_evt_q_address = pointer_to_uword (peer_mq);

  /* segment manager allocates only producer eventfds */
  if ((fd = svm_msg_q_get_consumer_eventfd (mq)) != -1)
    {
      mp->fd_flags |= SESSION_FD_F_MQ_EVENTFD;
      mp->n_fds = 2;
      fds[0] = fd;
      fds[1] = svm_msg_q_get_consumer_eventfd (peer_mq);
    }

  vl_msg_api_send_shmem (reg->vl_input_queue, (u8 *) & mp);

  if (fd != -1)
    session_send_fds (reg, fds, 2);
