...
send_session_accept_callback (stream_session_t * s)
{
  ...
  mp = vl_msg_api_alloc (sizeof (*mp));
  mp->_vl_msg_id = clib_host_to_net_u16 (VL_API_ACCEPT);
				 ...
				 u8 is_fail)
{
  ...
  mp = vl_msg_api_alloc (sizeof (*mp));
  mp->_vl_msg_id = clib_host_to_net_u16 (VL_API_CONNECT_REPLY);
  mp->context = app->api_context;
  mp->retval = is_fail;
  if (!is_fail)
    {
...
send_session_disconnect_callback (stream_session_t * s)
{
  ...
  mp = vl_msg_api_alloc (sizeof (*mp));
  memset (mp, 0, sizeof (*mp));
  mp->_vl_msg_id = clib_host_to_net_u16 (VL_API_DISCONNECT);
...
static void
vl_api_bind_t_handler (vl_api_bind_t * mp)
{
  vl_api_bind_reply_t *rmp;
  vnet_bind_args_t _a, *a = &_a;
  char segment_name[128];
  u32 segment_name_length;
  int rv;
  ...
  REPLY_MACRO2 (VL_API_BIND_REPLY, (
				     {
				     rmp->retval = rv;
				     if (!rv)
				     {
				     ...
				     if (segment_name_length)
				     {
				     memcpy (rmp->segment_name, segment_name,
					     segment_name_length);
				     rmp->segment_name_length =
				     segment_name_length;}
				     rmp->server_event_queue_address =
...
static void
vl_api_unbind_t_handler (vl_api_unbind_t * mp)
{
  vl_api_unbind_reply_t *rmp;
...
static void
vl_api_connect_t_handler (vl_api_connect_t * mp)
{
  vnet_connect_args_t _a, *a = &_a;
...
vl_api_disconnect_t_handler (vl_api_disconnect_t * mp)
{
  ...
  REPLY_MACRO (VL_API_DISCONNECT_REPLY);
}
...
static void
vl_api_reset_reply_t_handler (vl_api_reset_reply_t * mp)
{
  stream_session_t *s;
