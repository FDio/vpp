...
vl_api_app_attach_t_handler (vl_api_app_attach_t * mp)
{
  ...
  if (!session_main_is_enabled () || app_namespace_socket_api_enabled ())
    {
...
static void
session_api_send_fds (app_worker_t * app_wrk, int *fds, int n_fds)
{
  app_api_msg_t smsg = { 0 };
  ...
  cs = app_namespace_get_api_socket (app_ns, app->app_api_socket_index);

  /* There's no payload for the message only the type */
  smsg.type = APP_API_MSG_TYPE_SEND_FDS;
  clib_socket_sendmsg (cs, &smsg, sizeof (smsg), fds, n_fds);
}

static int
mq_send_add_segment_sock_cb (u32 app_wrk_index, u64 segment_handle)
{
  int fds[SESSION_N_FD_TYPE], n_fds = 0;
  svm_msg_q_msg_t _msg, *msg = &_msg;
  session_app_add_segment_msg_t *mp;
   ...
   */
  session_api_send_fds (app_wrk, fds, n_fds);
...
static int
mq_send_del_segment_sock_cb (u32 app_wrk_index, u64 segment_handle)
{
  svm_msg_q_msg_t _msg, *msg = &_msg;
  session_app_del_segment_msg_t *mp;
  app_worker_t *app_wrk;
			    ...
			    app_api_attach_msg_t * mp)
{
  ...
  vnet_app_attach_args_t _a, *a = &_a;
  app_api_attach_reply_msg_t *rmp;
  u8 fd_flags = 0, ctrl_thread;
  ...
  session_mq_cb_vft.add_segment_callback = mq_send_add_segment_sock_cb;
  session_mq_cb_vft.del_segment_callback = mq_send_del_segment_sock_cb;
  ...
  msg.type = APP_API_MSG_TYPE_ATTACH_REPLY;
  ...
  if (!rv)
    {
      ...
      handle->aah_app_index = a->app_index;
    }
...
void
session_api_add_del_worker_handler (app_namespace_t * app_ns,
				    clib_socket_t * cs,
				    app_api_add_del_worker_msg_t * mp)
{
  ...
  api_client_handle = app_socket_api_client_index (app_ns, cs);
  session_api_del_client (api_client_handle);
  ...
  msg.type = APP_API_MSG_TYPE_ADD_DEL_WORKER_REPLY;
  rmp = &msg.add_del_worker_reply;
  rmp->retval = rv;
  rmp->is_add = mp->is_add;
  ...
  if (!rv && mp->is_add)
    {
      ...
      /* Update app index for socket */
      handle = (app_ns_api_handle_t *) & cs->private_data;
      handle->aah_app_index = app->app_index;
    }
...
static void
session_api_app_socket_error (app_namespace_t * app_ns, clib_socket_t * cs)
{
...
session_api_sock_read_ready (clib_file_t * cf)
{
  ...
  if (err)
    {
      clib_error_free (err);
      session_api_app_socket_error (app_ns, cs);
      goto error;
    }
  ...
  switch (msg.type)
    ...
    case APP_API_MSG_TYPE_ADD_DEL_WORKER:
      session_api_add_del_worker_handler (app_ns, cs, &msg.add_del_worker);
      break;
    default:
      clib_warning ("app %u unknown message type: %u", handle->aah_app_index,
		    msg.type);
      break;
    }
...
session_api_add_ns_socket (app_namespace_t * app_ns)
{
  ...
  vec_add (dir, (u8 *) "/app_sockets/", strlen ("/app_sockets/"));
  ...
  s->config = (char *) app_ns->sock_name;
  ...
  handle = (app_ns_api_handle_t *) & s->private_data;
  handle->aah_file_index = clib_file_add (&file_main, &cf);
  handle->aah_app_index = APP_INVALID_INDEX;
