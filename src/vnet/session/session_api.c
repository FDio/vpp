...
_(ADD_APP_CERTIFICATE, add_app_certificate)					\
_(DEL_APP_CERTIFICATE, del_app_certificate)					\
...
static void
vl_api_add_app_certificate_t_handler (vl_api_add_app_certificate_t * mp)
{
  vl_api_add_app_certificate_reply_t *rmp;
  vnet_add_app_certificate_args_t _a, *a = &_a;
  ...
  rv = vnet_add_app_certificate (a);
  vec_free (a->cert);
  vec_free (a->key);

done:
  /* *INDENT-OFF* */
  REPLY_MACRO2 (VL_API_ADD_APP_CERTIFICATE_REPLY, ({
    if (!rv)
      rmp->index = a->index;
  }));
  /* *INDENT-ON* */
...
static void
vl_api_del_app_certificate_t_handler (vl_api_del_app_certificate_t * mp)
{
  vl_api_del_app_certificate_reply_t *rmp;
  int rv = 0;
  if (session_main_is_enabled () == 0)
    {
      rv = VNET_API_ERROR_FEATURE_DISABLED;
  ...
  rv = vnet_del_app_certificate (mp->index);

done:
  REPLY_MACRO (VL_API_ADD_APP_CERTIFICATE_REPLY);
}
