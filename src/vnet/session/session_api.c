...
sapi_send_fds (app_worker_t * app_wrk, int *fds, int n_fds)
{
  ...
  cs = appns_sapi_get_socket (app_ns, app->appns_sapi_sock_index);
...
static void
sapi_free_socket_w_handle (u32 api_handle)
{
			     ...
			     app_api_add_del_worker_msg_t * mp)
{
  ...
  if (!mp->is_add)
    {
      sapi_free_socket_w_handle (api_client_handle);
      goto done;
    }
...
static void
sapi_app_socket_disconnect (app_namespace_t * app_ns, clib_socket_t * cs)
{
...
sapi_sock_read_ready (clib_file_t * cf)
{
  ...
  if (err)
    {
      clib_error_free (err);
      sapi_app_socket_disconnect (app_ns, cs);
      goto error;
    }
  ...
  switch (msg.type)
    ...
    case APP_API_MSG_TYPE_ADD_DEL_WORKER:
      sapi_add_del_worker_handler (app_ns, cs, &msg.add_del_worker);
      break;
    default:
