...
send_session_accept_callback (stream_session_t * s)
{
  ...
  if (session_has_transport (s))
      ...
      if (application_is_proxy (server))
	{
      ...
      mp->port = tc->rmt_port;
      mp->is_ip4 = tc->is_ip4;
      _clib_memcpy (&mp->ip, &tc->rmt_ip, sizeof (tc->rmt_ip));
    }
  else
    {
				 ...
				 stream_session_t * s, u8 is_fail)
{
  ...
  if (session_has_transport (s))
      ...
      if (!tc)
	{
      ...
      mp->handle = session_handle (s);
      mp->vpp_event_queue_address = pointer_to_uword (vpp_queue);
      _clib_memcpy (mp->lcl_ip, &tc->lcl_ip, sizeof (tc->lcl_ip));
      mp->is_ip4 = tc->is_ip4;
  ...
  if (mp->namespace_id_len)
    {
      ...
      _clib_memcpy (a->namespace_id, mp->namespace_id, mp->namespace_id_len);
    }
        ...
        if (app && application_has_global_scope (app))
            {
              ...
              _clib_memcpy (rmp->lcl_ip, &tc->lcl_ip, sizeof(tc->lcl_ip));
...
vl_api_connect_sock_t_handler (vl_api_connect_sock_t * mp)
{
  ...
  if (app)
      ...
      if (mp->hostname_len)
	{
	  ...
	  _clib_memcpy (a->sep_ext.hostname, mp->hostname, mp->hostname_len);
	}
      a->api_context = mp->context;
      a->app_index = app->app_index;
      a->wrk_map_index = mp->wrk_index;
...
vl_api_app_namespace_add_del_t_handler (vl_api_app_namespace_add_del_t * mp)
{
  ...
  _clib_memcpy (ns_id, mp->namespace_id, mp->namespace_id_len);
			    ...
			    vl_api_registration_t * reg, u32 context)
{
  ...
  rmp->is_ip4 = 1;
  _clib_memcpy (rmp->lcl_ip, &match->lcl_ip, sizeof (match->lcl_ip));
  _clib_memcpy (rmp->rmt_ip, &match->rmt_ip, sizeof (match->rmt_ip));
  ...
  if (tag)
    {
      _clib_memcpy (rmp->tag, tag, vec_len (tag));
					   ...
					   mp)
{
  ...
  _clib_memcpy (a->cert, mp->cert, cert_len);
					  ...
					  mp)
{
  ...
  _clib_memcpy (a->key, mp->key, key_len);
