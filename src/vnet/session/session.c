...
session_update_time_fn_vec_update (session_update_time_fn **fns, session_update_time_fn fn,
				   u8 is_add, clib_thread_index_t thread_index)
{
  ...
  if (is_add)
    {
      if (found)
	{
	  clib_warning ("update time fn %p already registered for thread %u", fn, thread_index);
	  return;
	}
      vec_add1 (*fns, fn);
    }
					  ...
					  clib_thread_index_t thread_index)
{
  ...
  session_update_time_fn_vec_update (&wrk->update_time_fns, fn, is_add, thread_index);
}
