...
{
  return session_send_evt_to_thread (f, 0, f->master_thread_index,
	                             evt_type);
}
...
{
  session_manager_main_t *smm = &session_manager_main;
  transport_service_type_t tsp;
  int i, errors = 0, lock;
  stream_session_t *s;
  u32 *indices;

  indices = smm->session_to_enqueue[transport_proto][thread_index];
  tsp = transport_protocol_service_type (transport_proto);
  lock = tsp == TRANSPORT_SERVICE_CL;

  for (i = 0; i < vec_len (indices); i++)
    {
      s = session_get_if_valid (indices[i], thread_index);
      if (s == 0 || session_enqueue_notify (s, lock))
	errors++;
    }
