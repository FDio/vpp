...
u32
session_async_rx_consume (session_t *s, session_async_rx_consume_fn_t fn, void *ctx)
{
  ...
  wrk = session_main_get_worker (s->thread_index);
  if (PREDICT_FALSE (wrk->async_rx_loaned_bytes + data_len > SESSION_ASYNC_RX_MAX_LOANED_BYTES))
    {
      session_flush_async_rx (f);
      return 0;
    }
  ...
  if (PREDICT_FALSE (result != SESSION_ASYNC_RX_CONSUME && result != SESSION_ASYNC_RX_TAKE))
    {
      ASSERT (0);
      session_flush_async_rx (f);
      return 0;
...
void
session_async_rx_release (u32 thread_index, const u32 *buffer_refs, u32 n_buffer_refs, u32 n_bytes)
{
  session_worker_t *wrk;
