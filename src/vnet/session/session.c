...
static void
session_send_evt_to_thred (u64 session_handle, fifo_event_type_t evt_type,
...
session_send_session_evt_to_thread (u64 session_handle,
                                    ...
                                    u32 thread_index)
{
  session_send_evt_to_thred (session_handle, evt_type, thread_index, 0, 0);
}

void
session_send_rpc_evt_to_thread (u32 thread_index, void *fp, void *rpc_args)
{
  session_send_evt_to_thred (0, FIFO_EVENT_RPC, thread_index, fp, rpc_args);
}
...
session_alloc (u32 thread_index)
{
  ...
  if (PREDICT_FALSE (will_expand && smm->session_peekers[thread_index]))
    while (smm->session_peekers[thread_index])
      ;
...
static int
session_alloc_fifos (segment_manager_t *sm,
                     stream_session_t *s)
{
                              ...
                              stream_session_t **new_session)
{
  ...
  /*
   * Find connection and session and cleanup half-open table
   */
//  handle = session_lookup_half_open_handle (tc);
//  if (handle == HALF_OPEN_LOOKUP_INVALID_VALUE)
//    {
...
//  session_lookup_del_half_open (tc);
   ...
   */
  new_s = session_clone_safe (tc->s_index, tc->thread_index);
  ...
  rpc_args->thread_index = tc->thread_index;
...
stream_session_listen (stream_session_t * s, session_endpoint_t * sep)
{
  ...
  if (transport_is_dgram (sep->transport_proto))
    s->session_state = SESSION_STATE_LISTENING_READY;
  return 0;
}
