			    ...
			    session_evt_type_t evt_type)
{
  ...
  mq = session_get_vpp_event_queue (thread_index);
...
session_program_transport_close (session_t * s)
{
  ...
  if (vlib_thread_is_main_w_barrier () || thread_index == s->thread_index)
    {
      wrk = session_get_worker (s->thread_index);
 ...
 */
int
session_flush_enqueue_events (u8 transport_proto, u32 thread_index)
{
  session_worker_t *wrk = session_get_worker (thread_index);
  session_t *s;
  int i, errors = 0;
  u32 *indices;
...
int
session_flush_all_enqueue_events (u8 transport_proto)
{
  vlib_thread_main_t *vtm = vlib_get_thread_main ();
  int i, errors = 0;
  for (i = 0; i < 1 + vtm->n_threads; i++)
    errors += session_flush_enqueue_events (transport_proto, i);
  return errors;
}
...
ssvm_private_t *
session_get_evt_q_segment (void)
{
