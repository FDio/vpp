			       ...
			       transport_cleanup_cb_fn cb_fn)
{
  ...
  if (PREDICT_FALSE (!app_wrk))
      ...
      if (ntf == SESSION_CLEANUP_TRANSPORT)
	{
	  ...
	  //   ((void (*) (transport_connection_t *)) (cb_fn)) (
...
/**
 * Request from transport to delete connection after notifications
 *
 ...
 */
void
session_transport_delete_program (transport_connection_t *tc, void *cleanup_cb)
{
  session_t *s;
  ...
  if (!(s = session_get_if_valid (tc->s_index, tc->thread_index)))
    {
      ...
      //       ((void (*) (transport_connection_t *)) (cleanup_cb)) (tc);
      return;
    }

  //   clib_warning ("this is called state %u", s->session_state);
