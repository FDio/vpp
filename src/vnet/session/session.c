  ...
  if (app_wrk)
    {
      ...
      clib_warning ("detached %p and %p", s->rx_fifo, s->tx_fifo);
			      ...
			      u32 old_thread_index, session_t ** new_session)
{
  ...
  if (app_wrk)
    {
      /* New set of fifos attached to the same memory */
      sm = app_worker_get_connect_segment_manager (app_wrk);
      segment_manager_attach_fifo (sm, &new_s->rx_fifo, new_s);
      segment_manager_attach_fifo (sm, &new_s->tx_fifo, new_s);
      clib_warning ("attached %p and %p", new_s->rx_fifo, new_s->tx_fifo);
    }
