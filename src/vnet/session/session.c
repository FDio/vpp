...
session_enqueue_notify (stream_session_t * s, u8 lock)
{
  ...
  if (PREDICT_FALSE (app == 0))
    {
      clib_warning ("invalid s->app_index = %d", s->app_wrk_index);
      return 0;
    }
...
session_stream_connect_notify (transport_connection_t * tc, u8 is_fail)
{
  ...
  if (!is_fail)
    {
      ...
      alloc_fifos = !application_is_builtin_proxy (app_wrk);
  ...
  if (app->cb_fns.session_connected_callback (app->app_index, opaque, new_s,
	                                      is_fail))
    {
...
stream_session_accept_notify (transport_connection_t * tc)
{
  ...
  s = session_get (tc->s_index, tc->thread_index);
  app_wrk = app_wrk_get_if_valid (s->app_wrk_index);
  if (!app_wrk)
    return;
  app = application_get (app_wrk->app_index);
...
stream_session_disconnect_notify (transport_connection_t * tc)
{
  ...
  app_wrk = application_get_if_valid (s->app_wrk_index);
  if (!app_wrk)
    return;
  app = application_get (app_wrk->app_index);
...
stream_session_reset_notify (transport_connection_t * tc)
{
  ...
  app_wrk = application_get (s->app_wrk_index);
  app = application_get (app_wrk->app_index);
		       ...
		       u8 notify)
{
  ...
  app_wrk = application_get (listener->app_wrk_index);
...
session_open_cl (u32 app_index, session_endpoint_t * rmt, u32 opaque)
{
   ...
   */
  app_wrk = application_get (app_index);
  sm = app_worker_get_connect_segment_manager (app_wrk);
...
session_listen_app (stream_session_t * s, session_endpoint_t * sep)
{
  ...
  esep.app_index = s->app_wrk_index;
