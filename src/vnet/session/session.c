...
always_inline int
session_enqueue_chain_tail (stream_session_t *s, vlib_buffer_t * b, u32 offset,
			    u8 is_in_order)
{
			     ...
			     u32 offset, u8 queue_event, u8 is_in_order)
{
  ...
  if (is_in_order)
	...
	return -1; */
      ...
      /* PREDICT_FALSE ? */
      if (enqueued > 0 && (b->flags & VLIB_BUFFER_NEXT_PRESENT))
	{
  ...
  else
    {
      rv = svm_fifo_enqueue_with_offset (s->server_rx_fifo, offset,
					 b->current_length,
					 vlib_buffer_get_current (b));
      if (!rv && (b->flags & VLIB_BUFFER_NEXT_PRESENT))
