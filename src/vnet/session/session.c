...
session_tx_fifo_dequeue_drop (transport_connection_t * tc, u32 max_bytes)
{
  ...
  tmp = s->opaque;
  s->opaque = SESSION_FIFO_LAST_ACTION_DEQUEUED;
  segment_manager_fifo_tuning (s, s->tx_fifo, rv);
  s->opaque = tmp;
...
session_main_flush_enqueue_events (u8 transport_proto, u32 thread_index)
{
  ...
  for (i = 0; i < vec_len (indices); i++)
      ...
      if (PREDICT_FALSE (!s))
	{
      ...
      tmp = s->opaque;
      s->opaque = SESSION_FIFO_LAST_ACTION_ENQUEUED;
      segment_manager_fifo_tuning (s, s->rx_fifo, 0 /* TODO */ );
      s->opaque = tmp;
