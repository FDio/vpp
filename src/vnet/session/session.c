...
//  pool_alloc (wrk->sessions, clib_max (len, 2 * pending_len));
//
//  /* Copy pending elements and make sure indices are marked as not free */
//  clib_memcpy_fast (&wrk->sessions[len], wrk->sessions_expand,
//		    pending_len * sizeof (session_t));
//
//  pool_header_t *ph = pool_header (wrk->sessions);
//  vec_foreach (s, wrk->sessions_expand)
//    {
//      /* Already freed, mark index as free */
//      if (s->session_index == ~0)
//	{
//	  u32 free_index = s - wrk->sessions_expand;
//	  pending_len -= 1;
//	  ph->free_bitmap =
//	    clib_bitmap_ori_notrim (ph->free_bitmap, free_index);
//	  continue;
//	}
//      ph->free_bitmap =
//	clib_bitmap_andnoti_notrim (ph->free_bitmap, s->session_index);
//    }
//
//  _vec_len (wrk->sessions) += pending_len;
//
...
session_alloc (u32 thread_index)
{
  ...
  //      s->session_index = pool_len (wrk->sessions) + (s -
  //      wrk->sessions_expand);
  //
  ...
  //    }
  mrpool_get (wrk->sessions, s, s->session_index);
...
session_free (session_t * s)
{
  ...
  if (wrk->session_expand_pending)
    {
      if (s->session_index > pool_len (wrk->sessions))
	{
	  s->session_index = ~0;
	  return;
	}
