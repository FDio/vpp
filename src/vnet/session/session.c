  ...
  if (rpc_needed)
    {
      ...
      session_send_rpc_evt_to_thread (args->thread_index, session_switch_pool_rpc, rpc_args);
    }
}
			      ...
			      session_handle_tu_t osh, session_t **new_session)
{
   ...
   * fifo owner
   */
  session_switch_pool_args_t rpc_args = { .new_session_index = new_s->session_index,
					  .new_thread_index = new_s->thread_index,
					  .session_index = tc->s_index,
					  .thread_index = osh.thread_index };
