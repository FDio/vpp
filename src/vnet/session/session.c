...
static void
session_fifo_tuning_apply_update (svm_fifo_t * f, u32 size,
				  session_ft_action_t act)
{
  segment_manager_t *sm = segment_manager_get (f->segment_manager);
  if (act == SESSION_FT_ACTION_ENQUEUED)
    {
      size = clib_min (size, sm->max_fifo_size - svm_fifo_size (f));
      svm_fifo_set_size (f, svm_fifo_size (f) + size);
    }
  else					/* dequeued */
    {
      size = clib_min (size, svm_fifo_size (f) - 4096);
      svm_fifo_set_size (f, svm_fifo_size (f) - size);
    }
}
  ...
  if (s->flags & SESSION_F_CUSTOM_FIFO_TUNING)
    {
      ...
      u32 size = app_worker_session_fifo_tuning (app_wrk, s, f, act, len);
      session_fifo_tuning_apply_update (f, size, act);
    }
  else
    {
      segment_manager_t *sm;
      sm = segment_manager_get (f->segment_manager);
      segment_manager_fifo_tuning (sm, s, s->tx_fifo, act, len);
    }
}
