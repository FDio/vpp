  ...
  for (thread = 0; thread < num_threads; thread++)
    {
      ...
      wrk->ctrl_head = clib_llist_make_head_numa (wrk->event_elts, evt_list,
						  numa);
      wrk->new_head = clib_llist_make_head_numa (wrk->event_elts, evt_list,
						 numa);
      wrk->old_head = clib_llist_make_head_numa (wrk->event_elts, evt_list,
						 numa);
      wrk->last_vlib_time = vlib_time_now (wrk->vm);
      wrk->last_vlib_us_time = wrk->last_vlib_time * CLIB_US_TIME_FREQ;

      if (prealloc_per_wrk && (thread > 0 || num_threads == 1))
	pool_init_fixed_numa (wrk->sessions, prealloc_per_wrk, numa);
