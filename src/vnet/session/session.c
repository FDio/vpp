...
static int
app_wrk_send_io_event (app_worker_t *app_wrk, session_event_t *evt)
{
  ...
  mq_evt->event_type = evt->event_type;
...
static int
app_worker_flush_events_builtin (app_worker_t *app_wrk, u32 thread_index)
{
...
app_wrk_flush_wrk_events (app_worker_t *app_wrk, u32 thread_index)
{
  svm_msg_q_t *mq = app_wrk->event_queue;
  u32 i, n_evts = 128;
  session_event_t *evt, *mq_evt;
  svm_msg_q_msg_t mq_msg;

  if (app_worker_application_is_builtin (app_wrk))
    return app_worker_flush_events_builtin (app_wrk, thread_index);
  ...
  for (i = 0; i < n_evts; i++)
    {
      ...
      if (evt->event_type < SESSION_CTRL_EVT_BOUND)
        rv = app_wrk_send_io_event (app_wrk);
      else
        rv = app_wrk_send_ctrl_event ();
  ...
  if (is_in_order)
    {
      ...
      if (PREDICT_FALSE ((b->flags & VLIB_BUFFER_NEXT_PRESENT)
			 && enqueued >= 0))
	{
	  in_order_off = enqueued > b->current_length ? enqueued :
          0;
