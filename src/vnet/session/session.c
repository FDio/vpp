...
session_cleanup_half_open (session_handle_t ho_handle)
{
  ...
  //   clib_warning ("ho %u cleanup", ho->session_index);
  ...
  for (i = 0; i < f->shr->n_subscribers; i++)
    {
      ...
      if (!app_wrk)
	continue;
      session_program_io_event (app_wrk, s, evt_type,
				s->thread_index == 0 ? 1 : 0);
    }
...
session_dequeue_notify (session_t *s)
{
  ...
  session_program_io_event (app_wrk, s, SESSION_IO_EVT_TX,
			    s->thread_index == 0 ? 1 : 0);
...
/*
 * Enqueue data for delivery to app. If requested, it queues notification
...
session_transport_delete_notify (transport_connection_t * tc)
{
  ...
  switch (s->session_state)
    ...
    case SESSION_STATE_APP_CLOSED:
      ...
      session_cleanup_notify (s, SESSION_CLEANUP_TRANSPORT);
      svm_fifo_dequeue_drop_all (s->tx_fifo);
      // XXX should this be sent from session-input?
