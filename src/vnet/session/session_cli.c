...
format_session_transport (u8 *s, session_t *ss, u32 tp, session_fmt_req_t fmt)
{
  ...
  if (fmt.conn_id)
    {
      if (use_cols)
	s = format (s, "%-" SESSION_CLI_ID_LEN "U", format_transport_connection, tp,
...
/*
 * Format a session according to the normalized session format request.
 * The session layer owns conn-id/state/rx-tx layout for regular sessions,
 * while listener and half-open paths still use the legacy transport formatters.
 */
u8 *
