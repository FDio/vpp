			 ...
			 vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
                         ...
                         &session_index))
	{
      ...
      else if (unformat (line_input, "state %U", unformat_session_state, &state))
	{
	  ...
	  do_filter = 1;
                         ...
                         &transport_proto))
	do_filter = 1;
      else if (unformat (line_input, "thread %u range %u %u", &thread_index, &start,
                         &end))
	do_filter = 1;
      else if (unformat (line_input, "thread %u range %u", &thread_index, &start))
	{
	  end = start + 50;
	  do_filter = 1;
	}
