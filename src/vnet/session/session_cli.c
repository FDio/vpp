...
format_session_fifos (u8 *s, va_list *args)
{
  ...
  if (verbose > 2 && ss->rx_fifo->signals->has_event)
    {
      found = session_node_lookup_fifo_event (ss->rx_fifo, e);
      s =
	format (s, " session node event: %s\n", found ? "found" : "not found");
    }
  s = format (s, " Tx fifo: %U", format_svm_fifo, ss->tx_fifo, verbose);
  if (verbose > 2 && ss->tx_fifo->signals->has_event)
    {
...
static u8 *
format_session_io (u8 *s, session_t *ss)
{
  u32 rxf, txf;
  ...
  return format (s, "%-10u%-10u", rxf, txf);
}

static_always_inline transport_fmt_req_t
session_fmt_req_transport (session_fmt_req_t fmt, transport_fmt_req_flag_t flag)
{
  transport_fmt_req_t t_fmt = transport_fmt_req_decode (fmt.as_u32);

  return transport_fmt_req_with (t_fmt, flag);
}
...
format_session_transport (u8 *s, session_t *ss, u32 tp, session_fmt_req_t fmt)
{
  ...
  if (fmt.transport_detail && !fmt.rx_tx)
    return format (
      s, "%U", format_transport_connection, tp, ss->connection_index, ss->thread_index,
      transport_fmt_req_raw (session_fmt_req_transport (fmt, TRANSPORT_FMT_REQ_F_DETAIL)));
  ...
  if (wants_state)
    {
      if (use_cols)
	s = format (
	  s, "%-" SESSION_CLI_STATE_LEN "U", format_transport_connection, tp, ss->connection_index,
	  ss->thread_index,
	  transport_fmt_req_raw (session_fmt_req_transport (fmt, TRANSPORT_FMT_REQ_F_STATE)));
      else
	s = format (
	  s, "%U", format_transport_connection, tp, ss->connection_index, ss->thread_index,
	  transport_fmt_req_raw (session_fmt_req_transport (fmt, TRANSPORT_FMT_REQ_F_STATE)));
  ...
  if (fmt.transport_detail)
    s = format (s, "%U", format_transport_connection, tp, ss->connection_index, ss->thread_index,
		transport_fmt_req_raw ((transport_fmt_req_t){ .transport_detail = 1 }));
...
{
  session_t *ss = va_arg (*args, session_t *);
  session_fmt_req_t fmt = session_fmt_req_decode (va_arg (*args, int));
...
typedef struct session_tree_node_
{
  u32 session_index;
  u32 *children;
  u32 depth;
...
typedef struct session_tree_
{
  session_tree_node_t *nodes;
  u32 *roots;
  uword *session_index_to_node;
} session_tree_t;

static void
session_tree_insert_node (session_tree_t *st, u32 parent_index,
			  u32 session_index)
{
  session_tree_node_t *node, *parent;
  u32 node_index;

  if (hash_get (st->session_index_to_node, session_index))
    return;

  pool_get_zero (st->nodes, node);
  node->session_index = session_index;
  node_index = node - st->nodes;
  hash_set (st->session_index_to_node, session_index, node_index);

  if (parent_index == SESSION_INVALID_INDEX)
    vec_add1 (st->roots, node_index);
  else
    {
	...
	return;
      ...
      vec_add1 (parent->children, node_index);
    }
}
...
session_tree_insert_session (session_tree_t *st, session_t *s)
{
  ...
  if (parent_sh != SESSION_INVALID_HANDLE && !(hash_get (st->session_index_to_node, parent_index)))
    {
      session_t *parent_s = session_get_from_handle_if_valid (parent_sh);
      if (!parent_s)
	{
	  /* add as root session */
	  session_tree_insert_node (st, SESSION_INVALID_INDEX, s->session_index);
	  return;
	}
      session_tree_insert_session (st, parent_s);
    }

  session_tree_insert_node (st, parent_index, s->session_index);
}

static void
session_tree_reset (session_tree_t *st)
{
  session_tree_node_t *node;

  vec_foreach (node, st->nodes)
    vec_free (node->children);

  pool_free (st->nodes);
  vec_free (st->roots);
  hash_free (st->session_index_to_node);
}

static void
session_cli_show_session_tree (vlib_main_t *vm, u8 force_print)
{
  session_main_t *smm = &session_main;
  const u32 tree_print_limit = 50;
  ...
  for (thread_index = 0; thread_index < vec_len (smm->wrk); thread_index++)
    {
      ...
      vlib_cli_output (vm, "Thread %d: %u sessions", thread_index,
		       pool_elts (wrk->sessions));

      if (pool_elts (wrk->sessions) > tree_print_limit && !force_print)
	{
	  vlib_cli_output (vm, "output suppressed due to number of sessions");
	  continue;
	}

      /*
       * build bottom up session tree/forest, i.e., network towards
       * applications
       */
      pool_foreach (s, wrk->sessions)
	{
      ...
      /* DFT print of tree */
      vec_foreach (node_index, st->roots)
	{
	  vec_add1 (node_stack, *node_index);

	  while (vec_len (node_stack))
	    {
	      session_tree_node_t *node =
...
VLIB_CLI_COMMAND (vlib_cli_show_session_command) = {
  ...
  .short_help =
    ...
    "[lcl|rmt|ep <ip>[:<port>]][tree][force-print]",
...
static clib_error_t *
show_session_fifo_trace_command_fn (vlib_main_t *vm, unformat_input_t *input,
				    vlib_cli_command_t *cmd)
{
  session_t *s = 0;
  u8 is_rx = 0, *str = 0;
  ...
  str = is_rx ? svm_fifo_dump_trace (str, s->rx_fifo) :
				...
				vlib_cli_command_t *cmd)
{
  ...
  str = is_rx ? svm_fifo_replay (str, s->rx_fifo, 0, 1) :
...
session_config_fn (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      if (unformat (input, "wrk-mq-length %d", &nitems))
	{
      ...
      else if (unformat (input, "wrk-mqs-segment-size %U", unformat_memory_size,
      ...
      else if (unformat (input, "nat44-original-dst-enable"))
	{
	  smm->original_dst_lookup =
	    vlib_get_plugin_symbol ("nat_plugin.so", "nat44_original_dst_lookup");
	}
	...
	{
	  clib_warning ("App attachment using binary-api is deprecated in favor "
