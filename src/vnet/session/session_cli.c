			 ...
			 vlib_cli_command_t * cmd)
{
  ...
  stream_session_t *s;
  u8 *str = 0, thread_index, one_session = 0;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	verbose = 1;
      else if (unformat (input, "thread %d %U:%d %U:%d", &thread_index,
  ...
  if (one_session)
    {
      s = stream_session_lookup4 (&lcl_ip4, &rmt_ip4,
				  ...
				  SESSION_TYPE_IP4_TCP, thread_index);
      if (s)
	vlib_cli_output (vm, "%U", format_stream_session, s, 2);
      else
	vlib_cli_output (vm, "session does not exists");
