  ...
  if (!(fmt.as_u32 & SESSION_FMT_REQ_F_MASK))
    {
      fmt.conn_id = 1;
      if (fmt.level >= 1)
	fmt.transport_state = 1;
      if (fmt.level == 1)
	fmt.rx_tx = 1;
      if (fmt.level >= 2)
	{
	  fmt.transport_detail = 1;
	  fmt.session_detail = 1;
	}
    }
...
/*
 * Format a session according to the session format request.
 */
u8 *
format_session (u8 *s, va_list *args)
{
  ...
  if (PREDICT_TRUE (ss->session_state >= SESSION_STATE_ACCEPTING ||
    ...
    {
      /* Transports for migrating sessions might've been cleaned up already */
      ...
      vec_foreach (node_index, st->roots)
	{
	  ...
	  while (vec_len (node_stack))
		...
		pool_elt_at_index (st->nodes, vec_pop (node_stack));
	      ...
	      u32 *child_index;
	      session_fmt_req_t fmt = { .level = 1 };

	      fmt.conn_id_indent = clib_min (node->depth, SESSION_FMT_REQ_CONN_ID_INDENT_MAX);
