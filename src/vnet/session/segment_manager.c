	...
	* (rx_fifo_size + tx_fifo_size);
	...
	(approx_total_size +
	 (ca->private_segment_size - 1)) / (u64) ca->private_segment_size;
      ...
      if (approx_segment_count != ca->private_segment_count)
	{
	  clib_warning
	    ("Note: overriding segment count %u with calculated count %u",
  ...
  if (show_segments)
    {
      ...
      vlib_cli_output (vm, "%d svm fifo segments allocated",
		       pool_elts (segments));
      vlib_cli_output (vm, "%-20s%=12s%=16s%=16s%=16s", "Name", "Size (M)",
		       "ActiveFifos", "FreeFifos", "Address");
	...
	vlib_cli_output (vm, "%-20v%=16u%=16u%=16u%16llx",
                         name, size >> 20, active_fifos, free_fifos,
			 address);
        if (verbose)
