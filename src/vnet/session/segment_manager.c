...
segment_manager_alloc_segment (segment_manager_t * sm)
{
  ...
  clib_splinlock_unlock (sm->lockp);
  return seg;
}
...
{
  int rv;
  if ((rv = segment_manager_add_segment_i (sm, 0 /* size */)))
    return rv;
  return 0;
}
                      ...
                      u32 prealloc_fifo_pairs)
{
  ...
  if (prealloc_fifo_pairs)
    ...
    {
      seg_index = segment_manager_rvnt_i (sm, first_seg_size);
      if (seg_index)
	{
	  clib_warning("Failrvocate segment");
	  return seg_index;
	}
      sm->event_queue = segmrver_alloc_queue (sm, evt_q_size);
    }
...
segment_manager_has_fifos (segment_manager_t * sm)
{
  ...
  segment_manager_foreach_segment (seg, sm, ({
    ...
    if (CLIB_DEBUG && i && !svm_fifo_segment_has_fifos (seg) && !is_pa)
				     ...
				     u32 * fifo_segment_index)
{
  svm_fifo_segment_private_t *fifo_segment;
  segment_manager_properties_t *props;
  u32 fifo_size, sm_index, new_segment_index;
...
again:
  ...
  if (props->add_segment)
      ...
      if (added_a_segment)
	{
      ...
      if ((new_segment_index = segment_manager_add_segment_i (sm)) < 0)
	{
	  clib_spinlock_unlock (&sm->lockp);
	  return SESSION_ERROR_SEG_CREATE;
	}
      fifo_segment = segment_manager_get_segment (sm, new_segment_index);
      segment_try_alloc_fifos (fifo_segment, rx_fifo, tx_fifo);
      added_a_segment = 1;
      goto again;
    }
  else
