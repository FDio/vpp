...
u32 default_first_segment_size = 1 << 20;
...
segment_manager_init (segment_manager_t *sm,
		      segment_manager_properties_t *properties, u32 seg_size)
{
  u32 first_seg_size;
  int rv;
  ...
  first_seg_size = seg_size > 0 ? seg_size : default_first_segment_size;
...
{
  application_t *app = application_get (sm->properties->app_index);
  u32 seg_size = 0;
  u8 *seg_name;
