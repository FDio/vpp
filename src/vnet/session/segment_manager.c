...
segment_manager_dealloc_fifos_ct (svm_fifo_t *rx_fifo, svm_fifo_t *tx_fifo, u32 is_client)
{
  ...
  if (is_client)
    ...
    {
      i32 rx_refcnt_after_free = rx_fifo->refcnt - 1;
      i32 tx_refcnt_after_free = tx_fifo->refcnt - 1;
      u8 expected_peer_ref = (rx_fifo->flags & SVM_FIFO_F_CLIENT_CT_DETACHED) ? 0 : 1;
      i32 remaining_refs = clib_max (rx_refcnt_after_free, tx_refcnt_after_free);

      defer_server_cleanup = remaining_refs > expected_peer_ref;
    }
