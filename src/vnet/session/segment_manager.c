...
static int sm_custom_segment_alloc_fifos (session_t *s, segment_manager_t *sm,
					  clib_thread_index_t thread_index,
					  svm_fifo_t **rx_fifo,
					  svm_fifo_t **tx_fifo);
static void sm_custom_segment_dealloc_fifos (svm_fifo_t *rx_fifo,
					     svm_fifo_t *tx_fifo);
...
int
segment_manager_alloc_session_fifos (session_t *s, segment_manager_t *sm,
				     clib_thread_index_t thread_index,
				     svm_fifo_t **rx_fifo,
				     svm_fifo_t **tx_fifo)
{
  ...
  if (s &&
      (session_type_transport_proto (s->session_type) == TRANSPORT_PROTO_CT) &&
      (s->session_state == SESSION_STATE_CREATED))
    {
      return (sm_custom_segment_alloc_fifos (s, sm, s->thread_index, rx_fifo,
					     tx_fifo));
    }
...
segment_manager_dealloc_fifos (svm_fifo_t *rx_fifo, svm_fifo_t *tx_fifo)
{
  ...
  if (fifo_segment_flags (fs) & FIFO_SEGMENT_F_CUSTOM_USE)
    {
      segment_manager_segment_reader_unlock (sm);
      sm_custom_segment_dealloc_fifos (rx_fifo, tx_fifo);
      return;
    }
...
static void
sm_custom_segment_dealloc_fifos (svm_fifo_t *rx_fifo, svm_fifo_t *tx_fifo)
{
  ...
  if (pool_is_free_index (smm->custom_seg_ctxs, seg_ctx_index))
    return;
  seg_ctx = pool_elt_at_index (smm->custom_seg_ctxs, seg_ctx_index);
  if (pool_is_free_index (seg_ctx->segments, ct_seg_index))
    return;
  ...
  sm_lock_and_del_segment_inline (sm, seg_index, 1);
...
static int
sm_custom_segment_alloc_fifos (session_t *s, segment_manager_t *sm,
			       clib_thread_index_t thread_index,
			       svm_fifo_t **rx_fifo, svm_fifo_t **tx_fifo)
{
  segment_manager_props_t *props;
...
void
sm_custom_segment_dec_session_count (ct_connection_t *ct, svm_fifo_t *rx_fifo,
				     svm_fifo_t *tx_fifo)
{
  u32 cnt;
  u32 seg_ctx_index = rx_fifo->seg_ctx_index;
  ...
  segment_manager_segment_reader_unlock (sm);
  if ((fifo_segment_flags (fs) & FIFO_SEGMENT_F_CUSTOM_USE) == 0)
    return;

  /*
   * Grab exclusive lock and update flags if necessary unless some other
   * threads added more sessions
   */
  clib_rwlock_writer_lock (&smm->custom_segs_lock);
