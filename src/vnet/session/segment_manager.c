...
void
sm_custom_segment_dealloc_fifos (ct_connection_t *ct, svm_fifo_t *rx_fifo,
				 svm_fifo_t *tx_fifo)
{
...
int
sm_custom_segment_alloc_fifos (app_worker_t *server_wrk, ct_connection_t *ct,
			       session_t *ls, session_t *ll)
{
  ...
  if (spp)
      ...
      if (ct_seg)
	{
	  ...
	  ct->ct_seg_index = ct_seg->custom_seg_index;
  ...
  if (fs_index == ~0)
    {
      ...
      ct_seg =
	sm_custom_alloc_segment (server_wrk, table_handle, sm, ct->client_wrk);
      if (!ct_seg)
	{
	  clib_rwlock_writer_unlock (&smm->custom_segs_lock);
	  return -1;
...
void
sm_custom_segment_register (void)
{
  clib_rwlock_init (&smm->custom_segs_lock);
}
