...
typedef struct segment_manager_main_
{
  ...
  u8 default_fifo_inc_thresh;	/**< default fifo increase threshold % */
  u8 default_fifo_dec_thresh;	/**< default fifo decrease threshold % */
...
segment_manager_props_init (segment_manager_props_t * props)
{
  ...
  props->fifo_inc_thresh = sm_main.default_fifo_inc_thresh;
  props->fifo_dec_thresh = sm_main.default_fifo_dec_thresh;
...
segment_manager_init (segment_manager_t * sm)
{
  ...
  sm->fifo_inc_thresh = clib_min (100, props->fifo_inc_thresh);
  sm->fifo_dec_thresh = clib_min (100, props->fifo_dec_thresh);
...
find_max_free_segment (segment_manager_t * sm, u32 thread_index)
{
  ...
  pool_foreach (cur, sm->segments, ({
    fifo_segment_update_free_bytes (cur);
...
always_inline void
segment_manager_fifo_inc (segment_manager_t * sm, svm_fifo_t * f, u32 update)
{
  update = clib_min (update, sm->max_fifo_size - svm_fifo_size (f));
  svm_fifo_set_size (f, svm_fifo_size (f) + update);
  if (svm_fifo_size (f) > sm->max_fifo_size)
    os_panic ();
}

always_inline void
segment_manager_fifo_dec (segment_manager_t * sm, svm_fifo_t * f, u32 update)
{
  update = clib_min (update, svm_fifo_size (f) - 4096);
  svm_fifo_set_size (f, svm_fifo_size (f) - update);
  if (svm_fifo_size (f) < 4096)
    os_panic ();
}

void
segment_manager_fifo_tuning (segment_manager_t * sm, session_t * s,
			     svm_fifo_t * f, session_ft_action_t act,
			     u32 bytes)
{
  fifo_segment_mem_status_t st;
  ...
  if ((act == SESSION_FT_ACTION_ENQUEUED &&
       (fifo_usage <= sm->fifo_inc_thresh ||
	fifo_size == sm->max_fifo_size)) ||
      (act == SESSION_FT_ACTION_DEQUEUED &&
       (fifo_usage >= sm->fifo_dec_thresh || fifo_size == 4096)))
    return;

  ASSERT (fifo_size <= sm->max_fifo_size);

  fs = segment_manager_get_segment (sm, f->segment_index);
  st = fifo_segment_get_mem_status (fs);
  ...
  if (act == SESSION_FT_ACTION_ENQUEUED)
	...
	case MEMORY_PRESSURE_NO_PRESSURE:
	  if (fifo_usage > 40)
	    segment_manager_fifo_inc (sm, f, fifo_in_use);
	  break;
	...
	case MEMORY_PRESSURE_HIGH_PRESSURE:
	case MEMORY_PRESSURE_NO_MEMORY:
	  segment_manager_fifo_dec (sm, f, bytes);
	  break;
	case MEMORY_PRESSURE_LOW_PRESSURE:
	  if (fifo_usage < 60)
	    segment_manager_fifo_dec (sm, f, bytes);
	  break;
	case MEMORY_PRESSURE_NO_PRESSURE:
	  if (fifo_usage > 90 && fifo_size < 2 << 20)
	    segment_manager_fifo_inc (sm, f, fifo_size);
	  break;
	default:
	  break;
	}
...
segment_manager_main_init (segment_manager_main_init_args_t * a)
{
  ...
  sm->default_fifo_inc_thresh = 50;
  sm->default_fifo_dec_thresh = 95;
}
			 ...
			 vlib_cli_command_t * cmd)
{
  ...
  if (verbose && pool_elts (smm->segment_managers))
    {
      vlib_cli_output (vm, "%-6s%=10s%=10s%=13s%=11s%=11s%=15s%=15s%=12s",
		       "Index", "AppIndex", "Segments", "MaxFifoSize",
		       "HighWater", "LowWater", "FifoIncThresh",
		       "FifoDecThresh", "FifoTuning");
