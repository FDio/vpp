...
static int sm_custom_segment_alloc_fifos (session_t *s, segment_manager_t *sm,
					  ...
					  svm_fifo_t **rx_fifo,
					  svm_fifo_t **tx_fifo);
static void sm_custom_segment_dealloc_fifos (session_t *s, svm_fifo_t *rx_fifo,
					     svm_fifo_t *tx_fifo);
...
void
segment_manager_dealloc_fifos (session_t *s, svm_fifo_t *rx_fifo,
			       svm_fifo_t *tx_fifo)
{
  segment_manager_t *sm;
  fifo_segment_t *fs;
  ...
  if (fifo_segment_flags (fs) & FIFO_SEGMENT_F_CUSTOM_USE)
    {
      ASSERT (s);
      if (s == 0)
	log_err ("Bad API call to %s", __FUNCTION__);
      else
	{
	  segment_manager_segment_reader_unlock (sm);
	  sm_custom_segment_dealloc_fifos (s, rx_fifo, tx_fifo);
	}
      return;
    }
...
static void
sm_custom_segment_dealloc_fifos (session_t *s, svm_fifo_t *rx_fifo,
				 svm_fifo_t *tx_fifo)
{
  ...
  if (ct == 0)
    {
      log_err ("Cannot find cut-through connection: connection index %d, "
	       "thread index %d",
	       s->connection_index, s->thread_index);
      return;
    }
  ASSERT (session_get (ct->c_s_index, ct->c_thread_index) == s);
  ...
  seg_ctx = pool_elt_at_index (smm->custom_seg_ctxs, ct->seg_ctx_index);
  ...
  if (!pool_elts (seg_ctx->segments))
    {
      ...
      pool_put_index (smm->custom_seg_ctxs, ct->seg_ctx_index);
    }
