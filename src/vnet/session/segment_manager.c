...
segment_manager_properties_alloc (void)
{
  ...
  pool_get (smm->segment_manager_properties_pool, props);
...
{
  segment_manager_main_t *smm = &segment_manager_main;
  pool_put (smm->segment_manager_properties_pool, props);
...
{
  segment_manager_main_t *smm = &segment_manager_main;
  if (pool_is_free_index (smm->segment_manager_properties_pool, smp_index))
    return 0;
  return pool_elt_at_index (smm->segment_manager_properties_pool, smp_index);
}
...
{
  segment_manager_main_t *smm = &segment_manager_main;
  return p - smm->segment_manager_properties_pool;
}
...
segment_manager_alloc_segment (segment_manager_t * sm)
{
  ...
  u8 will_expand = 0;
  u32 seg_index;
  pool_get_aligned_will_expand (sm->segments, will_expand,
  ...
  if (PREDICT_FALSE (will_expand && vlib_num_workers ()))
    {
      ...
      seg_index = segment_manager_segment_index (sm, seg);
 ...
 */
always_inline void
segment_manager_del_segment_w_lock (segment_manager_t * sm, u32 fs_index)
{
...
/**
 * Adds segment to segment manager's pool with reader lock
 */
always_inline int
segment_manager_add_segment_w_lock (segment_manager_t * sm, u32 segment_size)
{
  segment_manager_main_t *smm = &segment_manager_main;
   ...
   */
  seg_index = segment_manager_alloc_segment (sm);
                      ...
                      u32 prealloc_fifo_pairs)
{
  ...
  u64 approx_total_size, max_seg_size = ((u64) 1 << 32) - clib_mem_get_page_size();
  segment_manager_properties_t *props;
  ...
  if (prealloc_fifo_pairs)
	...
	{
	  seg_index = segment_manager_add_segment_w_lock (sm, max_seg_size);
	  if (seg_index < 0)
	    {
    ...
    {
      seg_index = segment_manager_add_segment_w_lock (sm, first_seg_size);
      if (seg_index)
	{
...
again:
  ...
  if (props->add_segment)
      ...
      if (added_a_segment)
	{
      ...
      if ((new_segment_index = segment_manager_add_segment_w_lock (sm, 0)) < 0)
	{
	  clib_warning ("failed to add new segment");
	  return SESSION_ERROR_SEG_CREATE;
	}
      clib_warning ("new segment");
			       ...
			       svm_fifo_t * tx_fifo)
{
  ...
  if (!svm_fifo_segment_has_fifos (fifo_segment))
	...
	segment_manager_del_segment_w_lock (sm, segment_index);
