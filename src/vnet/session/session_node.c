...
//	  && wrk->last_vlib_time - wrk->pm_switch_time > 1e-4)
	{
	  //	  clib_warning ("switched to interrupt\n");
	  wrk->state = SESSION_WRK_INTERRUPT;
	  vlib_node_set_state (vm, session_queue_node.index,
			       VLIB_NODE_STATE_INTERRUPT);
	  if (PREDICT_TRUE (wrk->vm->thread_index))
	    session_wrk_timerfd_update (wrk, 1e3);
	  else
	    session_wrk_timerfd_update (wrk, vlib_num_workers () ? 5e5 : 1e3);
	}
      //      else
...
//	  wrk->pm_switch_time = wrk->last_vlib_time;
	}
      else if (PREDICT_FALSE (!pool_elts (wrk->sessions)))
	{
	  wrk->state = SESSION_WRK_IDLE;
	  session_wrk_timerfd_update (wrk, 100e3);
	}
    }
  else
    {
