...
//  vlib_buffer_t *pb;
  ...
  vlib_get_buffers (vm, ctx->tx_buffers, ctx->tx_bufs, ctx->n_bufs_needed);
  ...
  while (n_left >= 4)
    {
      ...
      vec_add1 (wrk->pending_tx_buffers, ctx->tx_buffers[tbi0]);
      vec_add1 (wrk->pending_tx_buffers, ctx->tx_buffers[tbi1]);
