...
static void
listen_cb (void *data, u32 data_len)
{
  session_mq_listen_handler (data);
}

static void
unlisten_cb (void *data, u32 data_len)
{
  session_mq_unlisten_handler (data);
}

static void
connect_cb (void *data, u32 data_len)
{
  session_mq_connect_handler (data);
}
...
session_mq_listen_handler (void *data)
{
  ...
  if (session_mq_should_rpc_main ())
    {
      vl_api_rpc_call_main_thread (listen_cb, data,
                                   sizeof (session_listen_msg_t));
      return;
    }
...
session_mq_connect_handler (void *data)
{
  ...
  if (session_mq_should_rpc_main ())
    {
      vl_api_rpc_call_main_thread (connect_cb, data,
...
session_mq_unlisten_handler (void *data)
{
  ...
  if (session_mq_should_rpc_main ())
    {
      vl_api_rpc_call_main_thread (unlisten_cb, data,
                                   sizeof (session_unlisten_msg_t));
      return;
    }
