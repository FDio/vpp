...
int
application_update_session_app_worker (application_t *app,
                                       app_worker_t *app_wrk,
                                       stream_session_t *s)
{
...
session_mq_worker_update_handler (void *data)
{
  ...
  if (mp->wrk_index == s->app_wrk_index)
    {
      clib_warning ("resending to wrk %u", mp->wrk_index);
  ...
  application_update_session_app_worker (app, app_wrk, s);

  clib_warning ("changing session %u:%u worker to %u", s->thread_index, s->session_index, app_wrk->wrk_index);
