...
static uword
session_process (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  f64 now, timeout = 1;
  uword *event_data = 0;
  uword event_type;
  ...
  while (1)
    {
      ...
      switch (event_type)
	{
	case SESSION_PROCESS_FLUSH_FRAMES:
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (session_process_node) =
{
  .function = session_process,
