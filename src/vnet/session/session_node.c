		       ...
		       vlib_frame_t * frame)
{
  ...
  session_evt_elt_t *elt, *next, *new_he, *new_te, *pending_he;
  ...
  while (elt != new_he)
	...
	case SESSION_IO_EVT_TX:
	  ...
	  if (n_tx_packets == VLIB_FRAME_SIZE)
	    {
	      clib_llist_add_tail (wrk->event_elts, evt_list, elt, postponed_he);
	      break;
	    }
	...
	case SESSION_CTRL_EVT_CLOSE:
	  ...
	  if (!e->postponed
	    ...
	    {
	      e->postponed += 1;
	      clib_llist_add_tail (wrk->event_elts, evt_list, elt, pending_he);
	      break;
	    }
