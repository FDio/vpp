...
always_inline void
session_event_dispatch (session_main_t *smm , session_worker_t *wrk, session_evt_elt_t *elt)
{
  session_event_t *e;
  session_t *s;
  int rv;
  ...
  switch (e->event_type)
    ...
    case SESSION_IO_EVT_BUILTIN_TX:
      ...
      wrk->ctx.s = s;
      if (PREDICT_TRUE(s != 0))
	session_tx_fifo_dequeue_internal (vm, node, wrk, elt, &n_tx_packets);
      break;
  ...
  while (!clib_llist_is_empty (wrk->event_elts, evt_list, new_he))
    {
      ...
      e = &elt->evt;
      session_event_dispatch (wrk, elt);
