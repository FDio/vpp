...
//  _vec_len (wrk->postponed_event_vector) = 0;
  ...
  if (n_to_dequeue && svm_msg_q_try_lock (mq) == 0)
      ...
      for (i = 0; i < n_to_dequeue; i++)
	{
	  ...
	  new_he = pool_elt_at_index (wrk->event_elts, wrk->new_head);
	  clib_llist_add_tail (wrk->event_elts, evt_list, new_he, elt);
...
//  for (i = 0; i < n_events; i++)
    {
      ...
      e = &elt->evt;
      elt = clib_llist_next (wrk->event_elts, evt_list, elt);
      switch (e->event_type)
	{
	...
	case SESSION_IO_EVT_TX:
	  ...
	  if (n_tx_packets == VLIB_FRAME_SIZE)
	    {
	      clib_llist_add_tail (wrk->event_elts, evt_list, postponed_he, elt);
...
//	      vec_add1 (wrk->pending_disconnects, *e);
	      clib_llist_add_tail (wrk->event_elts, evt_list, pending_he, elt);
	      break;
	    }
	...
	default:
      ...
      elt = clib_llist_next (wrk->event_elts, evt_list, new_he);
    }
