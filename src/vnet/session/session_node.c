...
session_tx_fifo_read_and_snd_i (session_worker_t *wrk,
                                ...
                                session_evt_elt_t *elt, u8 peek_data)
{
  u32 next_index, next0, next1, *to_next, n_left_to_next, n_left, pbi;
  u32 n_trace, n_bufs_needed = 0;
  ...
  session_tx_set_dequeue_params (wrk->vm, ctx,
                                 VLIB_FRAME_SIZE - wrk->last_tx_packets,
				 peek_data);
  ...
  wrk->last_tx_packets += ctx->n_segs_per_evt;
...
session_tx_fifo_peek_and_snd (session_worker_t * wrk,
			      ...
			      session_evt_elt_t * e)
{
  return session_tx_fifo_read_and_snd_i (wrk, node, e, 1);
}
...
session_tx_fifo_dequeue_and_snd (session_worker_t * wrk,
   			      ...
   			      session_evt_elt_t * e)
{
  return session_tx_fifo_read_and_snd_i (wrk, node, e, 0);
}
                        ...
                        session_evt_elt_t *elt)
{
  ...
  switch (e->event_type)
    ...
    case SESSION_IO_EVT_TX:
      ...
      if (wrk->last_tx_packets == VLIB_FRAME_SIZE)
	{
	  session_evt_add_postponed (wrk, elt);
	  return;
	}

      s = session_event_get_session (e, wrk->thread_index);
      if (PREDICT_FALSE(!s))
	{
	...
	break;
       ...
       * dispatch cycles */
      if (!e->postponed
	  || (e->postponed < 200 && svm_fifo_max_dequeue_cons (s->tx_fifo)))
	{
	  e->postponed += 1;
	  session_evt_add_old (wrk, elt);
	  break;
    ...
    case SESSION_IO_EVT_BUILTIN_TX:
      ...
      wrk->ctx.s = s;
      if (PREDICT_TRUE(s != 0))
	session_tx_fifo_dequeue_internal (wrk, node, elt);
      break;
		       ...
		       vlib_frame_t * frame)
{
  ...
  while (!clib_llist_is_empty (wrk->event_elts, evt_list, new_he))
    {
      ...
      session_event_dispatch (wrk, node, elt);
  ...
  vlib_node_increment_counter (vm, session_queue_node.index,
			       SESSION_QUEUE_ERROR_TX, wrk->last_tx_packets);

  SESSION_EVT (SESSION_EVT_DISPATCH_END, wrk, wrk->last_tx_packets);

  return wrk->last_tx_packets;
}
