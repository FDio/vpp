...
session_error_t
session_rules_table_add_del_ (struct _session_lookup_table *st,
...
typedef session_error_t (*rules_table_add_del) (
  struct _session_lookup_table *st, session_rules_table_t *srt,
  session_rule_table_add_del_args_t *args);
...
session_rules_table_add_del (struct _session_lookup_table *st,
			     session_rules_table_t *srt,
			     session_rule_table_add_del_args_t *args)
{
  if (!session_engine_vft)
    return SESSION_E_NOSUPPORT;
  return session_engine_vft->table_add_del (st, srt, args);
}
