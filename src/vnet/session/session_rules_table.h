			       ...
			       ip46_address_t *rmt_ip, u16 rmt_port, u8 is_ip4)
{
  ...
  if (srtg_handle == SESSION_SRTG_HANDLE_INVALID)
    return;
  session_engine_vft->table_show_rule (vm, srtg_handle, proto, lcl_ip,
			     ...
			     ip6_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
  ...
  if (srtg_handle == SESSION_SRTG_HANDLE_INVALID)
    return SESSION_RULES_TABLE_ACTION_ALLOW;
  return session_engine_vft->table_lookup6 (srtg_handle, proto, lcl_ip, rmt_ip,
					    lcl_port, rmt_port);
}
			      ...
			      u8 fib_proto)
{
  ...
  if (srtg_handle == SESSION_SRTG_HANDLE_INVALID)
    return;
  session_engine_vft->table_cli_dump (vm, srtg_handle, proto, fib_proto);
}
			     ...
			     ip4_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
  ...
  if (srtg_handle == SESSION_SRTG_HANDLE_INVALID)
    return SESSION_RULES_TABLE_ACTION_ALLOW;
  return session_engine_vft->table_lookup4 (srtg_handle, proto, lcl_ip, rmt_ip,
					    lcl_port, rmt_port);
}
			     ...
			     session_rule_table_add_del_args_t *args)
{
  ...
  if (srtg_handle == SESSION_SRTG_HANDLE_INVALID)
    return SESSION_E_NOSUPPORT;
  return session_engine_vft->table_add_del (srtg_handle, proto, args);
}
