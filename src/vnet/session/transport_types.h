...
transport_fmt_req_decode (u32 fmt)
{
    transport_fmt_req_t decoded = { .as_u32 = fmt };

    return decoded;
}

static_always_inline u32
transport_fmt_req_raw (transport_fmt_req_t fmt)
{
    return fmt.as_u32;
}

static_always_inline transport_fmt_req_t
transport_fmt_req_with (transport_fmt_req_t fmt, transport_fmt_req_flag_t flag)
{
    fmt.as_u32 &=
      ~(TRANSPORT_FMT_REQ_F_CONN_ID | TRANSPORT_FMT_REQ_F_STATE | TRANSPORT_FMT_REQ_F_DETAIL);
    if (flag & TRANSPORT_FMT_REQ_F_DETAIL)
    flag |= TRANSPORT_FMT_REQ_F_CONN_ID | TRANSPORT_FMT_REQ_F_STATE;
    fmt.as_u32 |= flag;
    return fmt;
}
