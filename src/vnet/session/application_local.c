	    ...
	    session_endpoint_cfg_t * sep)
{
   ...
   * thread index 0, and offset the index by 1, when we have workers as it is
   * the one dedicated to main thread. Note that n_workers does not include
   * main thread */
  cm->session_counter += 1;
  thread_index = cm->n_workers ? (cm->session_counter % cm->n_workers) + 1: 0;
