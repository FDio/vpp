...
//  return;
  ...
  if (ct->flags & CT_CONN_F_CLIENT)
    __atomic_sub_fetch (&seg_ctx->peer_session_count, 1, __ATOMIC_RELEASE);

  count = seg_ctx->peer_session_count;
...
//      if (ct->flags & CT_CONN_F_CLIENT)
      ...
      for (i = 0; i < vec_len (seg_ctx->segment_indices); i++)
	{
	  if (seg_ctx->segment_indices[i] == seg_index)
	    {
	      vec_del1 (seg_ctx->segment_indices, i);
	      break;
	    }
	}
      if (!vec_len (seg_ctx->segment_indices))
	{
...
ct_session_connect_notify (session_t * ss)
{
  ...
  __atomic_add_fetch (&seg_ctx->peer_session_count, 1, __ATOMIC_RELEASE);
//  clib_atomic_fetch_add_rel (&seg_ctx->peer_session_count, 1);

  if (seg_ctx->peer_session_count == 1)
    {
      if ((err =
...
static u32
ct_lookup_free_segment (segment_manager_t *sm, u32 seg_ctx_index,
			u32 pair_bytes)
{
  ct_main_t *cm = &ct_main;
  u32 max_fifos, *segp, seg_index = ~0;
  uword free_bytes, max_free_bytes;
  fifo_segment_t *fs;

  //  pair_bytes = props->rx_fifo_size + props->tx_fifo_size + (1 << 14);
  max_free_bytes = pair_bytes;
  vec_foreach (segp, cm->app_seg_ctxs[seg_ctx_index].segment_indices)
    {
...
{
  ct_main_t *cm = &ct_main;
  u32 sm_index, min_space, pair_bytes, seg_ctx_index = ~0;
  ...
  if (spp)
    {
      pair_bytes = props->rx_fifo_size + props->tx_fifo_size + (1 << 14);
      seg_index = ct_lookup_free_segment (sm, *spp, pair_bytes);
      seg_ctx_index = *spp;
    }
  ...
  if (seg_index == ~0)
      ...
      if (seg_ctx_index == ~0)
	{
	  ...
	  seg_ctx->sm_index = segment_manager_index (sm);
	}
      else
	seg_ctx = pool_elt_at_index (cm->app_seg_ctxs, seg_ctx_index);

      vec_add1 (seg_ctx->segment_indices, seg_index);
  ...
  seg_handle = segment_manager_segment_handle (sm, seg);
