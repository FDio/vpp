...
cleanup_client:
  ...
  if (cct->client_rx_fifo)
    sm_custom_segment_dealloc_fifos (cct, cct->client_rx_fifo,
				     cct->client_tx_fifo);
...
{
  return sm_custom_segment_alloc_fifos (server_wrk, ct, ls, ll);
}
  ...
  if (app_worker_accept_notify (server_wrk, ss))
    {
      ...
      sm_custom_segment_dealloc_fifos (sct, ss->rx_fifo, ss->tx_fifo);
      ct_connection_free (sct);
      session_free (ss);
    }
}
...
ct_session_cleanup_server_session (session_t *s)
{
  ...
  sm_custom_segment_dealloc_fifos (ct, s->rx_fifo, s->tx_fifo);
  session_free (s);
  ct_connection_free (ct);
}
  ...
  if (ct->flags & CT_CONN_F_CLIENT)
    {
       ...
       * the server */
      sm_custom_segment_dealloc_fifos (ct, ct->client_rx_fifo,
				       ct->client_tx_fifo);
