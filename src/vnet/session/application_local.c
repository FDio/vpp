...
{
  ct_connection_t *tc = va_arg (*args, ct_connection_t *);
  if (!tc)
    return s;
  if (tc->c_is_ip4)
    {
      s = format (s, "[%d:%d][%s] %U:%d->%U:%d", tc->c_thread_index,
		  tc->c_s_index, "CT", format_ip4_address, &tc->c_lcl_ip4,
    ...
    {
      s = format (s, "[%d:%d][%s] %U:%d->%U:%d", tc->c_thread_index,
		  tc->c_s_index, "CT", format_ip6_address, &tc->c_lcl_ip6,
