...
#define foreach_ct_segments_ctx_flags		\
  _(CLIENT_DETACHED, "client detached")		\
  _(SERVER_DETACHED, "server detached")		\


enum ct_segments_ctx_flags_bits_
{
#define _(sym, str) CT_SEGMENT_FLAG_BIT_##sym,
  foreach_ct_segments_ctx_flags
#undef _
};

typedef enum ct_segments_ctx_flags_
{
#define _(sym, str) CT_SEGMENT_FLAG_##sym = 1 << CT_SEGMENT_FLAG_BIT_##sym,
  foreach_ct_segments_ctx_flags
#undef _
} ct_segment_flags_t;
...
{
  u32 segment_index;
  u32 client_session_count;
  u32 server_session_count;
			  ...
			  svm_fifo_t *tx_fifo)
{
  ...
  has_fifos = fifo_segment_has_fifos (fs);
  ...
  if (ct->flags & CT_CONN_F_CLIENT)
    {
      ct_seg->client_session_count -= 1;
      count = ct_seg->client_session_count;
      if (!count)
	ct_seg->flags |= CT_SEGMENT_FLAG_CLIENT_DETACHED;
    }
  else
    {
      ct_seg->server_session_count -= 1;
      count = ct_seg->server_session_count;
      if (!count)
	ct_seg->flags |= CT_SEGMENT_FLAG_SERVER_DETACHED;
    }
  ...
  clib_warning ("ct detached flags %u", flags);

  app_worker_t *app_wrk;
  if (ct->flags & CT_CONN_F_CLIENT)
    {
      // TODO check detached client?
    ...
    {
      app_wrk = app_worker_get_if_valid (ct->server_wrk);
      app_worker_del_segment_notify (app_wrk, ct->segment_handle);
    }

  if (!(flags & CT_SEGMENT_FLAG_CLIENT_DETACHED)
      || !(flags & CT_SEGMENT_FLAG_SERVER_DETACHED))
    return;
  ...
  /*
   * No more segment indices left, remove the context
   */
  ...
  // If server goes down but client is still up, client holds up segment removal
  // it shouldn't because client needs to close the sessiona and then we cleanup
  // only issue: we always remove a segment once we hit 0 client or server sessions on it


  segment_manager_lock_and_del_segment (sm, seg_index);
  /* Cleanup segment manager if needed. If server detaches there's a chance
   * the client's sessions will hold up the c*/
...
ct_session_connect_notify (session_t * ss)
{
  ...
  //  seg = segment_manager_get_segment_w_lock (sm,
  //  ss->rx_fifo->segment_index); segment_handle =
  //  segment_manager_segment_handle (sm, seg);
  ...
  __atomic_add_fetch (&ct_seg->client_session_count, 1, __ATOMIC_RELEASE);

  if (ct_seg->client_session_count == 1)
    {
      if ((err =
	     app_worker_add_segment_notify (client_wrk, cct->segment_handle)))
	{
  ...
  //  if ((err = app_worker_add_segment_notify (client_wrk,
  //  cct->segment_handle)))
  //    {
  //      clib_warning ("failed to notify client %u of new segment",
  //		    sct->client_wrk);
...
//  ct_main_t *cm = &ct_main;
  ...
  pool_foreach (ct_seg, seg_ctx->segments)
    {
      /* Client or server has detached */
      if ((ct_seg->flags & CT_SEGMENT_FLAG_SERVER_DETACHED) ||
	  (ct_seg->flags & CT_SEGMENT_FLAG_CLIENT_DETACHED))
	continue;
...
{
  ct_main_t *cm = &ct_main;
  u32 sm_index, min_space, pair_bytes, seg_ctx_index = ~0, ct_seg_index = ~0;
  ...
  if (spp)
    {
      ...
      pair_bytes = props->rx_fifo_size + props->tx_fifo_size + (1 << 14);
      ct_seg_index = ct_lookup_free_segment (sm, seg_ctx, pair_bytes);
      // optimize return
      if (ct_seg_index != ~0)
	{
  ...
  if (ct_seg_index == ~0)
    {
      ...
      clib_warning ("adding segment");
      fs_index = segment_manager_add_segment (sm, props->segment_size, 0);
      if (fs_index < 0)
	{
  ...
  ls->rx_fifo->segment_index = ct_seg_index;
  ls->tx_fifo->segment_index = ct_seg_index;
