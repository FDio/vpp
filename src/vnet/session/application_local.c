...
static inline u8
ct_lookup_is_self_proxy (application_t *app, session_handle_t lh)
{
  return application_is_proxy (app) && lh == (session_handle_t) app->app_index;
}
...
ct_session_connect (transport_endpoint_cfg_t * tep)
{
  ...
  /*
   * Local proxy wildcard entries store the target app index instead of a
   * listener handle. Break the loop only when that app is the connecting
   * proxy itself.
   */
  if (ct_lookup_is_self_proxy (app, lh))
    goto global_scope;
