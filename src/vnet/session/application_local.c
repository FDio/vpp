...
//  clib_warning ("%u/%u to %u/%u", s->session_index, s->app_wrk_index,
//                peer_s->session_index, peer_s->app_wrk_index);
  if (peer_s->session_state > SESSION_STATE_READY)
    return 0;
//  if (peer_s->rx_fifo->has_event)
//    {
