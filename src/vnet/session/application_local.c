...
ct_session_tx (session_t *s)
{
  ...
  clib_warning ("got on session %u/%u sending on %u/%u has evt %u", s->session_index, s->app_wrk_index,
                peer_s->session_index, peer_s->app_wrk_index, peer_s->rx_fifo->has_event);
