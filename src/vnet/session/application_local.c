			  ...
			  svm_fifo_t *tx_fifo)
{
  ...
  if (ct->flags & CT_CONN_F_CLIENT)
    clib_atomic_fetch_sub_rel (&seg_ctx->peer_session_count, 1);

  need_cleanup = seg_ctx->peer_session_count == 0;
...
ct_session_connect_notify (session_t * ss)
{
  ...
  seg_ctx = pool_elt_at_index (cm->app_seg_ctxs, cct->seg_ctx_index);
  ...
  if (app_worker_connect_notify (client_wrk, cs, err, opaque))
    {
      ...
      ct_session_dealloc_fifos (sct, cs->rx_fifo, cs->tx_fifo);
      session_free (cs);
      return -1;
    }
