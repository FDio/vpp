...
ct_session_connect_notify (session_t *ss, session_error_t err)
{
  ...
  if (app_worker_connect_notify (client_wrk, cs, 0, opaque))
    {
      segment_manager_dealloc_fifos (0, cs->rx_fifo, cs->tx_fifo);
      ct_session_force_disconnect_server (sct);
      goto cleanup_client;
    }
...
cleanup_client:
  ...
  if (cct->client_rx_fifo)
    {
      session_t *s = session_get (cct->c_s_index, cct->c_thread_index);
      ASSERT (ct_connection_get (s->connection_index, s->thread_index) == cct);
      segment_manager_dealloc_fifos (s, cct->client_rx_fifo,
				     cct->client_tx_fifo);
    }
  ct_connection_free (cct);
  if (cs)
    {
      /* in case pool is expanded and cs is moved */
      cs = session_get (cct->c_s_index, cct->c_thread_index);
      session_free (cs);
    }
  return -1;
...
ct_accept_one (clib_thread_index_t thread_index, u32 ho_index)
{
  ...
  if (app_worker_init_accepted (ss))
    {
  ...
  if (app_worker_accept_notify (server_wrk, ss))
    {
      ...
      ASSERT (ct_connection_get (ss->connection_index, ss->thread_index) ==
	      sct);
      segment_manager_dealloc_fifos (ss, ss->rx_fifo, ss->tx_fifo);
      ct_connection_free (sct);
      session_free (ss);
    }
}
...
ct_session_cleanup_server_session (session_t *s)
{
  ...
  ASSERT (ct_connection_get (s->connection_index, s->thread_index) == ct);
  segment_manager_dealloc_fifos (s, s->rx_fifo, s->tx_fifo);
  ct_connection_free (ct);
  session_free (s);
}
  ...
  if (ct->flags & CT_CONN_F_CLIENT)
    {
       ...
       * the server */
      ASSERT (ct_connection_get (s->connection_index, s->thread_index) == ct);
      segment_manager_dealloc_fifos (s, ct->client_rx_fifo,
				     ct->client_tx_fifo);
