...
app_worker_local_session_connect_notify (session_t * ls)
{
  ...
  s = session_alloc_for_connection (&cct->connection);
...
//  u64 segment_handle;
  ...
  cct->c_lcl_port = 0;
  clib_memcopy (&cct->c_rmt_ip, &sep->ip, sizeof (sep->ip));
  ...
  clib_memcopy (&sct->c_lcl_ip, &ll_ct->c_lcl_ip, sizeof (ll_ct->c_lcl_ip));
  ...
  if (session_stream_accept (sct->connection, ll->session_index,
                             0 /* notify */))
    {
  ...
  ls = session_get (sct->c_s_index, 0);
...
format_ct_connection (u8 *s, va_list * args)
{
  ...
  if (verbose)
    {
      s = format (s, "%-15s", format_tcp_state, "ESTABLISHED");
    }
  return s;
}
