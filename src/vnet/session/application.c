                               ...
                               session_handle_t listener_handle)
{
  ...
  listener_id = listener_handle & 0xFFFFFFFF;

  /* *INDENT-OFF* */
  pool_foreach (ls, server->local_sessions, ({
    if (ls->listener_id == listener_id)
      application_local_session_disconnect (server->index, ls);
  }));
  /* *INDENT-ON* */

  pool_put_index(server->local_sessions, listener_id);
  return 0;
}
...
{
  u32 seg_size, seg_index, evt_q_sz, evt_q_elts;
  ...
  seg_size = props->rx_fifo_size + props->tx_fifo_size + evt_q_sz + (8 << 10);
  ...
  ls->listener_id = local_session_listener_id (sep);
