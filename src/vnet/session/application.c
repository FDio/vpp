...
{
  sep->transport_proto = session_type_transport_proto (
				...
				session_handle_t * handle)
{
  ...
  ll->listener_session_type = session_type_from_proto_and_ip (
      sep->transport_proto, sep->is_ip4);

  *handle = application_local_session_handle (ll);
  session_lookup_add_session_endpoint (table_index, sep, local_session_id (ll));
...
application_stop_local_listen (application_t * server, session_handle_t lh)
{
  ...
  if (!session_handle_is_local (lh))
      ...
      if (!sl || listen_session_get_local_session_endpoint (sl, &sep))
	{
      ...
      if (lh == SESSION_INVALID_HANDLE)
	return -1;
      /* TODO local listeners are stored as keys */
      local_session_parse_id (lh, &server_index, &ll_index);
...
application_local_session_disconnect (u32 app_index, local_session_t * ls)
{
  ...
  if (ls->session_state != SESSION_STATE_READY && client)
    {
