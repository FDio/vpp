...
application_free (application_t * app)
{
  ...
  vec_foreach (wrk_map, maps)
    {
      app_wrk = application_get_worker (app, wrk_map);
				...
				session_handle_t * handle)
{
  ...
  table_index = application_local_session_table (app_wrk);
				   ...
				   local_session_t * ll, u32 opaque)
{
  ...
  if (!has_transport)
  ...
  else
    {
      ...
      ls->port = tc->lcl_port;
      sm = application_get_listen_segment_manager (server_wrk, sl);
    }
  ...
  ls->client_index = client_wrk->wrk_index;
...
application_local_session_connect_notify (local_session_t * ls)
{
  ...
  server = application_get (ls->app_wrk_index);
...
application_format_local_connects (app_worker_t * app, int verbose)
{
  ...
  hash_foreach (client_key, value, app->local_connects, ({
    ...
    ls = application_get_local_session (server, session_index);
    vlib_cli_output (vm, "%-40s%-15s%-20s", "TODO", ls->app_wrk_index, ls->client_index);
  }));
  /* *INDENT-ON* */
}
