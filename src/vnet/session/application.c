...
/* crypto engine formatting functions moved to application_crypto.c */
...
format_crypto_context (u8 * s, va_list * args)
{
  ...
  s = format (s, "[engine:%d]", crctx->crypto_engine);
  return s;
}
...
/* certificate command moved to application_crypto.c */
