...
u8 *
format_application_listeners (u8 * s, va_list * args)
{
...
//  u32 segment_size;
  ...
  if (!regp)
    server_name = format (0, "builtin-%d%c", app->index, 0);
  else
  ...
  if (verbose)
    {
      s = format (s, "%-40s%-20s%-20d%-10d%-10d", str, server_name,
		     ...
		     vlib_cli_command_t * cmd)
{
  ...
  if (do_server)
	...
	{
	  vlib_cli_output (vm, "%U", format_application_listeners,
			   0 /* header */ , 0, 0,
			   verbose);
          /* *INDENT-OFF* */
          ...
          ({
            ...
            hash_foreach (handle, index, app->listeners_table,
	    ({
              vlib_cli_output (vm, "%U", format_application_listeners, app,
