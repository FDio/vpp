...
vnet_app_add_tls_cert (vnet_app_add_tls_cert_args_t * a)
{
  ...
  cert = app_certificate_get_default ();
...
{
  app_certificate_t *cert = va_arg (*args, app_certificate_t *);
  int key_len = 0, cert_len = 0;
  cert_len = vec_len (cert->cert);
  key_len = vec_len (cert->key);
  if (cert->cert_index == 0)
    s = format (s, "DEFAULT (cert:%d, key:%d)", cert_len, key_len);
  else
    s =
      format (s, "%d (cert:%d, key:%d)", cert->cert_index, cert_len, key_len);
  return s;
}
...
app_certificate_t *
app_certificate_get_if_valid (u32 index)
{
  if (pool_is_free_index (app_main.certificate_store, index))
    return 0;
  return app_certificate_get (index);
}

app_certificate_t *
app_certificate_get (u32 index)
{
  return pool_elt_at_index (app_main.certificate_store, index);
}

app_certificate_t *
app_certificate_get_default ()
{
  /* To maintain legacy bapi */
  return app_certificate_get (0);
}

int
vnet_add_app_certificate (vnet_add_app_certificate_args_t * a)
{
  app_certificate_t *cert = certificate_alloc ();
  cert->cert = vec_dup (a->cert);
  cert->key = vec_dup (a->key);
...
int
vnet_del_app_certificate (u32 index)
{
  int tp = 0;
  app_certificate_t *cert = app_certificate_get_if_valid (index);
  if (!cert)
    return (VNET_API_ERROR_INVALID_VALUE);
