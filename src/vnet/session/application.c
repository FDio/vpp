...
static inline int
app_send_evt_enqueue (svm_msg_q_t *mq, svm_msg_q_msg_t *msg, u8 lock)
{
...
app_send_io_evt_rx (application_t *app, stream_session_t *s, u8 lock)
{
  ...
  return app_send_evt_enqueue (mq, &msg, lock);
}
...
typedef int (app_send_evt_handler_fn) (application_t *app,
				       stream_session_t *s,
				       u8 lock);
static app_send_evt_handler_fn *app_send_evt_handler_fns[2] = {
    app_send_io_evt_rx,
    app_send_io_evt_tx,
};
/* *INDENT-ON* */
