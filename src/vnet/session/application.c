...
int
application_send_event (application_t *app, stream_session_t *s, u8 evt_type)
{
  ...
  switch (evt_type)
  {
	...
	{
	  msg = svm_msg_q_alloc_msg (mq, sizeof (session_fifo_event_t));
	  if (msg.as_u64 == ~0)
	    {
  ...
  if (PREDICT_TRUE(!svm_msg_q_full (mq)))
    {
      if (svm_msg_q_add (mq, msg, SVM_Q_WAIT))
	{
	  clib_warning("unexpected");
	  return -1;
	}
    }
  else
