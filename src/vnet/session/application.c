...
app_worker_alloc_and_init (application_t * app, app_worker_t **wrk)
{
  ...
  /* We don't use event queue for builtin apps. We use the fact that the
   * queue is not initialized as a flag to indicate that the worker is
   * used by a builtin app */
  if (application_is_builtin (app))
    app_wrk->event_queue = 0;
				   ...
				   local_session_t * ls)
{
  ...
  if (!has_transport)
    ...
    sm = application_get_listen_segment_manager (server_wrk,
                                                 (stream_session_t *) ls);
...
application_local_session_disconnect (u32 app_wrk_index, local_session_t * ls)
{
  ...
  if (app_wrk_index == ls->client_wrk_index)
	...
	{
	  mq_send_local_session_disconnected_cb (client_wrk->app_index, ls);
	}
    }
...
application_format_listeners (application_t *app, int verbose)
{
  ...
  if (!app)
    vlib_cli_output (vm, "%U", format_app_worker_listener,
...
app_worker_format_connects (app_worker_t * app_wrk, int verbose)
{
  ...
  app_name = app_get_name_from_reg_index (app_wrk);
