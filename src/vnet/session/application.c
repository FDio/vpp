...
vnet_connect2_do_connect (vnet_connect_args_t *a)
{
  ...
  if ((rv = vnet_connect (a)))
    {
      app_worker_t *app_wrk =
	application_get_worker (application_get (a->app_index), a->wrk_map_index);
...
{
  u32 thread_index = *(u32 *) args, n_pending;
  u32 n_connects = 0, max_connects;
  app_main_t *am = &app_main;

  /* Ideally connects should happen from first worker, to avoid polling
   * main and grabbing worker barrier */
  ASSERT (session_vlib_thread_is_cl_thread ());
  ...
  if (max_connects < n_pending)
    session_send_rpc_evt_to_thread_force (transport_cl_thread (), vnet_connect2_rpc,
					  (void *) &thread_index);
}
...
vnet_connect2 (vnet_connect_args_t *a)
{
  ...
  if (n_pending == 1)
    session_send_rpc_evt_to_thread_force (connects_thread, vnet_connect2_rpc,
					  (void *) &thread_index);
