...
vnet_listen (vnet_listen_args_t *a)
{
  ...
  if (application_is_transport (app))
    {
      sm = segment_manager_get_if_valid (a->sep_ext.sm_index);
      if (sm == 0)
	return SESSION_E_SEG_CREATE;
    }
  else
    {
      ...
      if (sm == 0)
	return SESSION_E_SEG_CREATE;
      a->sep_ext.sm_index = segment_manager_index (sm);
    }
					...
					u8 transport_proto, u8 is_start)
{
  ...
  if (is_start)
      ...
      if (!s)
	{
	  ...
	  sm = app_worker_alloc_listener_segment_manager (app_wrk);
	  if (sm == 0)
	    return clib_error_return (0, "Failed to allocate segment manager");
	  app_worker_start_listen (app_wrk, al, sm);
