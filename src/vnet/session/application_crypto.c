				       ...
				       vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "app %u", &app_index))
	;
	...
	;
      else if (unformat (input, "min-version %d", &args.min_version))
	;
      else if (unformat (input, "max-version %d", &args.max_version))
	;
      else
...
VLIB_CLI_COMMAND (app_crypto_add_tls_profile_cmd, static) = {
  ...
  .short_help = "app crypto add tls-profile app <app-index> "
		"[cipher-list <list>] [ciphersuites <suites>] "
		"[groups <groups>] [min-version <ver>] [max-version <ver>]",
...
VLIB_CLI_COMMAND (app_crypto_del_tls_profile_cmd, static) = {
  ...
  .short_help = "app crypto del tls-profile app <app-index> <profile-index>",
...
format_tls_profile (u8 *s, va_list *args)
{
  ...
  if (prof->min_version || prof->max_version)
    s = format (s, "versions: %d-%d ", prof->min_version, prof->max_version);
...
VLIB_CLI_COMMAND (show_tls_profile_command, static) = {
  ...
  .short_help = "show app tls-profile [app <app-index>]",
