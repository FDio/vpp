...
/*
 * Period used by transport pacers (1us)
 */
#define TRANSPORT_PACER_PERIOD 1e-6
...
{
  clib_us_time_type_t t = va_arg (*args, clib_us_time_type_t);
  if (t < 1e3)
    s = format (s, "%u us", t);
  else
    s = format (s, "%.3f s", t);
...
static inline u32
spacer_max_burst (spacer_t * pacer, clib_us_time_type_t time_now)
{
...
static inline void
spacer_reset_bucket (spacer_t * pacer, clib_us_time_type_t time_now)
{
  pacer->last_update = time_now;
  pacer->bucket = 0;
}
...
//  pacer->last_update = time_now >> SPACER_CPU_TICKS_PER_PERIOD_SHIFT;
  pacer->last_update = transport_us_time_now (tc->thread_index);
...
{
//  time_now >>= SPACER_CPU_TICKS_PER_PERIOD_SHIFT;
  spacer_reset_bucket (&tc->pacer, transport_us_time_now (tc->thread_index));
}
...
transport_connection_snd_space (transport_connection_t * tc, u16 mss)
{
  ...
  if (snd_space && transport_connection_is_tx_paced (tc))
    {
      clib_us_time_type_t now = transport_us_time_now (tc->thread_index);
//      time_now >>= SPACER_CPU_TICKS_PER_PERIOD_SHIFT;
