...
#define SESSION_EVT_DEQ_NODE_HANDLER(_wrk, _node_evt, _ntx)                   \
  ...
  {									\
    ...
    ed->data[3] =                                                             \
      (vlib_time_now (_wrk->vm) - _wrk->last_vlib_time) * 1000000.0;          \
  }
#else
#define SESSION_EVT_DEQ_NODE_HANDLER(_wrk, _node_evt, _ntx)
...
#define SESSION_EVT_POLL_GAP(_wrk)                                            \
  {                                                                           \
    ...
    DEC_SESSION_ED (_e, 1);                                                   \
    ed->data[0] =                                                             \
      (u32) ((_wrk->last_vlib_time - _wrk->last_event_poll) * 1000000.0);     \
  }
...
#define SESSION_EVT_DSP_CNTRS_MQ_DEQ_HANDLER(_wrk, _diff, _cnt, _dq,          \
					     _args...)                        \
  session_dbg_evts_t *sde = &session_dbg_main.wrk[_wrk->vm->thread_index];    \
