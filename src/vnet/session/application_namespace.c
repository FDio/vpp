...
vnet_app_namespace_add_del (vnet_app_namespace_add_del_args_t *a)
{
  ...
  if (a->is_add)
	...
	return clib_error_create ("sw_if_index %u doesn't exist",
	                          a->sw_if_index);
    ...
    {
      return clib_error_create ("namespace deletion not supported");
    }
  return 0;
}
...
show_app_ns_fn (vlib_main_t * vm, unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  if (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    return clib_error_return(0, "unknown input `%U'", format_unformat_error,
	                     input);
  vlib_cli_output (vm, "%-20s%-20s%-20s", "Namespace", "Secret", "sw_if_index");
