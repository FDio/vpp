...
app_namespace_t *
app_namespace_get_from_id (u8 *ns_id, const u8 *netns)
{
...
{
  hash_unset_mem (app_namespace_lookup_table, app_ns->ns_hash_key);
  vec_free (app_ns->ns_id);
  vec_free (app_ns->netns);
  vec_free (app_ns->ns_hash_key);

  pool_put (app_namespace_pool, app_ns);
}

app_namespace_t *
app_namespace_alloc (u8 *ns_id, u8 *netns)
{
  ...
  app_ns->netns = vec_dup (netns);

  key = vec_dup (ns_id);
  if (NULL != netns)
    key = format (key, "%c%v", 0, netns);

  hash_set_mem (app_namespace_lookup_table, key, app_ns - app_namespace_pool);
  app_ns->ns_hash_key = key;
...
u32
app_namespace_index_from_id (u8 *ns_id, const u8 *netns)
{
  uword *indexp;
  u8 *key;
...
format_app_namespace (u8 * s, va_list * args)
{
  ...
  s =
    format (s, "%-10u%-10lu%-12d%-15v%-20v%-40v", app_namespace_index (app_ns),
