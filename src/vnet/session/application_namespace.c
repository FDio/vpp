...
static u8 app_sock_api_enabled;
...
void
app_namespace_enable_socket_api (void)
{
  app_sock_api_enabled = 1;
}

u8
app_namespace_socket_api_enabled (void)
{
  return app_sock_api_enabled;
}
...
app_namespace_show_api (vlib_main_t * vm, app_namespace_t * app_ns)
{
  ...
  vlib_cli_output (vm, "%12s%5s", "app index", "fd");
  ...
  pool_foreach (cs, app_ns->app_sockets, ({
    ...
    vlib_cli_output (vm, "%12d%5u", handle->aah_app_index, cf->file_descriptor);
  }));
  /* *INDENT-ON* */
}
