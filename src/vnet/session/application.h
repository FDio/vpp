...
{
  ASSERT (ll->session_index < (2 << 16));
  u32 app_or_wrk_index;

  if (ll->session_state == SESSION_STATE_LISTENING)
    {
      ASSERT (ll->app_index < (2 << 16));
      app_or_wrk_index = ll->app_index;
    }
  else
    {
      ASSERT (ll->app_wrk_index < (2 << 16));
      app_or_wrk_index = ll->app_wrk_index;
    }

  return ((u32) app_or_wrk_index << 16 | (u32) ll->session_index);
}
