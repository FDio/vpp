...
void
app_wrk_program_io_msg (app_worker_t *app_wrk, u32 thread_index, session_event_t *msg)
{
  app_wrk_pending_msg_t *pm;

  pm = app_wrk_reserve_msg (app_wrk, thread_index);
  pm->event_type = msg->event_type;
  pm->len = sizeof (session_event_t);
  pm->ring = SESSION_MQ_IO_EVT_RING;
  pm->fd = -1;
  clib_memcpy_fast (pm->data, msg, sizeof (session_event_t));
}
