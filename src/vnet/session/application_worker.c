...
app_worker_init_listener (app_worker_t *app_wrk, session_t *ls, segment_manager_t *sm)
{
  ...
  if (application_is_transport (app))
	...
	clib_warning ("No segment manager passed to %s for transport %s,"
		      "allocating one",
...
session_error_t
app_worker_start_listen (app_worker_t *app_wrk, app_listener_t *app_listener, segment_manager_t *sm)
{
  session_t *ls;
  int rv;
			    ...
			    session_handle_t *rsh)
{
  ...
  /* pass/leak app worker index to vnet_listen */
