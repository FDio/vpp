...
void
session_rules_table_free_ (session_rules_table_t *srt, u8 fib_proto)
{
...
void
session_rules_table_init_ (session_rules_table_t *srt, u8 fib_proto,
			   const u8 *ns_id, u32 scope)
{
  mma_rules_table_16_t *srt4;
  mma_rules_table_40_t *srt6;
  mma_rule_16_t *rule4;
						   ...
						   void *ctx)
{
  ...
  if (st)
    for (i = 0; i < TRANSPORT_N_PROTOS; i++)
      session_rules_table_free (&st->session_rules[i], FIB_PROTOCOL_MAX);
  ...
  if (st)
    for (i = 0; i < TRANSPORT_N_PROTOS; i++)
      session_rules_table_free (&st->session_rules[i], FIB_PROTOCOL_IP4);
  ...
  if (st)
    for (i = 0; i < TRANSPORT_N_PROTOS; i++)
      session_rules_table_free (&st->session_rules[i], FIB_PROTOCOL_IP6);
}
						  ...
						  void *ctx)
{
  ...
  if (st)
    for (i = 0; i < TRANSPORT_N_PROTOS; i++)
      session_rules_table_init (&st->session_rules[i], FIB_PROTOCOL_MAX,
				app_namespace_id (app_ns),
  ...
  if (st)
    for (i = 0; i < TRANSPORT_N_PROTOS; i++)
      session_rules_table_init (&st->session_rules[i], FIB_PROTOCOL_IP4,
				app_namespace_id (app_ns),
  ...
  if (st)
    for (i = 0; i < TRANSPORT_N_PROTOS; i++)
      session_rules_table_init (&st->session_rules[i], FIB_PROTOCOL_IP6,
				app_namespace_id (app_ns),
				SESSION_RULE_SCOPE_GLOBAL);
}
