...
static session_rules_table_block_t *srt_instances;
...
u32
session_rules_table_lookup4_ (u32 srt_handle, u32 proto, ip4_address_t *lcl_ip,
			      ip4_address_t *rmt_ip, u16 lcl_port,
			      u16 rmt_port)
{
  session_rules_table_t *srt = srt_handle_to_srt (srt_handle, proto);
  mma_rules_table_16_t *srt4 = &srt->session_rules_tables_16;
  session_mask_or_match_4_t key = {
    .lcl_ip.as_u32 = lcl_ip->as_u32,
    .rmt_ip.as_u32 = rmt_ip->as_u32,
...
srt_handle_to_srt (u32 srt_handle, u32 proto)
{
  session_rules_table_block_t *srtb =
    pool_elt_at_index (srt_instances, srt_handle);
  session_rules_table_t *srt = &srtb->session_rules[proto];
...
u32
session_rules_table_lookup6_ (u32 srt_handle, u32 proto, ip6_address_t *lcl_ip,
...
{
  session_rules_table_block_t *srtb =
    pool_elt_at_index (srt_instances, st->srt_handle);
  session_rules_table_t *srt;

  vec_foreach (srt, srtb->session_rules)
    {
  ...
  srtb_instance_free (st);
}

void
srtb_instance_free (session_table_t *st)
{
  session_rules_table_block_t *srtb =
    pool_elt_at_index (srt_instances, st->srt_handle);

  vec_free (srtb->session_rules);
  pool_put (srt_instances, srtb);
  st->srt_handle = SESSION_SRT_HANDLE_INVALID;
}

session_rules_table_block_t *
srtb_instance_alloc (session_table_t *st, u32 n_proto)
{
  session_rules_table_block_t *srtb;

  pool_get (srt_instances, srtb);
  vec_validate (srtb->session_rules, n_proto);
  st->srt_handle = srtb - srt_instances;
  return (srtb);
}
...
session_rules_table_init_ (session_table_t *st, u8 fib_proto)
{
  ...
  srtb = srtb_instance_alloc (st, TRANSPORT_N_PROTOS - 1);
...
void
session_rules_table_show_rule_ (vlib_main_t *vm, u32 srt_handle, u32 proto,
...
void
session_rules_table_cli_dump_ (vlib_main_t *vm, u32 srt_handle, u32 proto,
			       u8 fib_proto)
{
  session_rules_table_t *srt = srt_handle_to_srt (srt_handle, proto);
  if (fib_proto == FIB_PROTOCOL_IP4)
