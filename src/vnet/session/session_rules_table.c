			     ...
			     session_rule_table_add_del_args_t * args)
{
  ...
  if (fib_proto == FIB_PROTOCOL_IP4)
      ...
      if (args->is_add)
	{
	  ...
	  session_rules_table_add_rule_16 (srt4, rule);
	}
      else
	{
	  session_rule_16_t rule;
	  ...
	  session_rules_table_del_rule_16 (srt4, &rule, srt4->root_index);
	}
    }
  else if (fib_proto == FIB_PROTOCOL_IP6)
    {
      ...
      if (args->is_add)
	{
	  rule = session_rules_table_alloc_rule_40 (srt6, &args->lcl,
						    ...
						    args->rmt_port);
	  rule->action_index = args->action_index;
	  session_rules_table_add_rule_40 (srt6, rule);
	}
      else
	{
	  session_rule_40_t rule;
	  ...
	  session_rules_table_init_rule_40 (&rule, &args->lcl, args->lcl_port,
					    &args->rmt, args->rmt_port);
	  session_rules_table_del_rule_40 (srt6, &rule, srt6->root_index);
	}
    }
  else
...
{
  session_rules_table_16_t *srt4 =
    &srt->session_rules_tables_16[transport_proto];
  ...
  return session_rules_table_lookup_16 (srt4,
...
{
  session_rules_table_40_t *srt6 =
    &srt->session_rules_tables_40[transport_proto];
  ...
  return session_rules_table_lookup_40 (srt6,
