...
typedef svm_fifo_seg_t app_dgram_seg_t;
...
always_inline int
app_send_dgram_segs_raw_gso (svm_fifo_t *f, app_session_transport_t *at,
			     svm_msg_q_t *vpp_evt_q,
			     app_dgram_seg_t *data_segs, u32 data_nsegs,
			     u32 data_len, u16 gso_size, u8 evt_type,
			     u8 do_evt, u8 noblock)
{
  session_dgram_hdr_t hdr;
  int rv;
  if (svm_fifo_max_enqueue_prod (f) <
      (sizeof (session_dgram_hdr_t) + data_len))
    return 0;
  ...
  svm_fifo_seg_t hdr_seg = { (u8 *) &hdr, sizeof (hdr) };

  rv = svm_fifo_enqueue_segments (f, &hdr_seg, 1, 0 /* allow partial */);
  if (PREDICT_FALSE (rv < 0))
    return 0;
  rv = svm_fifo_enqueue_segments (f, data_segs, data_nsegs,
				  0 /* allow partial */);
  if (PREDICT_FALSE (rv < 0))
    return 0;
...
{
  app_dgram_seg_t seg = { data, len };
  return app_send_dgram_segs_raw_gso (f, at, vpp_evt_q, &seg, 1, len, gso_size,
				      evt_type, do_evt, noblock);
}
...
always_inline int
app_send_dgram_segs (app_session_t *s, app_dgram_seg_t *data_segs,
		     u32 data_nsegs, u32 data_len, u8 noblock)
{
  return app_send_dgram_segs_raw_gso (
    s->tx_fifo, &s->transport, s->vpp_evt_q, data_segs, data_nsegs, data_len,
    0, SESSION_IO_EVT_TX, 1 /* do_evt */, noblock);
}
