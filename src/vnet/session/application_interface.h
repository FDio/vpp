...
app_send_dgram_raw (svm_fifo_t *f, app_session_transport_t *at,
                    svm_queue_t *vpp_evt_q, u8 *data, u32 len, u8 do_block)
{
  ...
  if ((rv = svm_fifo_enqueue_nowait (f, actual_write, data)) > 0)
      ...
      if (svm_fifo_set_event (f))
	{
	  ...
	  svm_queue_add (vpp_evt_q, (u8 *) &evt, do_block);
	}
    }
  return rv;
}
...
always_inline int
app_send_dgram (app_session_t *s, u8 *data, u32 len, u8 block)
{
  app_session_transport_t *at = (app_session_transport_t *)&s->lcl_ip;
  return app_send_dgram_raw (s->tx_fifo, at, s->vpp_evt_q, data, len, block);
}

always_inline int
app_send_stream_raw (svm_fifo_t *f, svm_queue_t *vpp_evt_q, u8 *data, u32 len,
                     u8 do_block)
{
  session_fifo_event_t evt;
  int rv;
...
always_inline int
app_send_stream (app_session_t *s, u8 *data, u32 len, u8 block)
{
  return app_send_stream_raw (s->tx_fifo, s->vpp_evt_q, data, len, block);
}
                    ...
                    app_session_transport_t *at)
{
  ...
  rv = svm_fifo_peek (f, ph.data_offset + SESSION_CONN_HDR_LEN,
                      ph.data_length - ph.data_offset, buf);
  ph.data_offset += rv;
  if (ph.data_offset == ph.data_length)
    svm_fifo_dequeue_drop (f, ph.data_length + SESSION_CONN_HDR_LEN);
  else
    svm_fifo_overwrite_head (f, (u8 *) &ph, sizeof(session_dgram_pre_hdr_t));
  return rv;
}
...
{
  app_session_transport_t *at;
  at = (app_session_transport_t *) &s->lcl_ip;
  return app_recv_dgram_raw (s->rx_fifo, buf, len, at);
}

always_inline int
app_recv_stream_raw (svm_fifo_t *f, u8 *buf, u32 len)
{
  svm_fifo_unset_event (f);
...
{
  return app_recv_stream_raw (s->rx_fifo, buf, len);
}
