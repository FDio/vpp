...
session_logging_main_t session_logging_main;
...
static session_log_buffer_chunk_t *
session_log_buffer_chunk_get (session_log_buffer_t *buf,
			      u32 chunk_index)
{
...
void
session_log_export (application_t *app, session_t *s)
{
...
static clib_error_t *
app_log_collector_command_fn (vlib_main_t *vm, unformat_input_t *input,
			    vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	slm->segment_size = tmp64;
      else if (unformat (line_input, "url %s", &collector_uri))
...
VLIB_CLI_COMMAND (app_log_collector_command, static) = {
  ...
  .function = app_log_collector_command_fn,
};
