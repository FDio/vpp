...
app_log_collector_send (app_log_collector_wrk_t *cwrk)
{
  ...
  if (session_has_transport (cs))
  ...
  else
    {
      ...
      session_t *cts = ct_session_get_peer (cs);
      wrote = svm_fifo_enqueue_segments (
	cts->tx_fifo, cwrk->segs, vec_len (cwrk->segs), 0 /* allow partial*/);
    }
...
app_log_collect (app_log_collector_t *c, session_t *s)
{
  ...
  msg = app_log_buf_chunk_append_uninit (chunk, sizeof (app_log_msg_t));
  msg->msg_type = APP_LOG_MSG_DATA;
  msg->msg_len = sizeof (app_log_msg_t);
  clib_memcpy_fast (msg->data_msg.conn_id, tc->opaque_conn_id,
		    sizeof (tc->opaque_conn_id));
  msg->data_msg.msg_len =
    sizeof (app_log_msg_data_t) + sizeof (tc->opaque_conn_id);
  ...
  switch (tc->proto)
	...
	tcp_transport_stats_t *tcp_stats = app_log_buf_chunk_append_uninit (
	  chunk, sizeof (tcp_transport_stats_t));
	msg->data_msg.msg_len += sizeof (tcp_transport_stats_t);
	msg->data_msg.data_type = APP_LOG_MSG_DATA_TCP;
	msg->msg_len += sizeof (tcp_transport_stats_t);
...
#undef _
	...
	udp_transport_stats_t *udp_stats = app_log_buf_chunk_append_uninit (
	  chunk, sizeof (udp_transport_stats_t));
	msg->data_msg.msg_len += sizeof (udp_transport_stats_t);
	msg->data_msg.data_type = APP_LOG_MSG_DATA_UDP;
	msg->msg_len += sizeof (udp_transport_stats_t);
...
#undef _
      ...
      break;
    default:
      clib_warning ("no transport stats for proto %u", tc->proto);
      break;
    };
...
{
  app_log_collector_t *c = app_log_collector_get (s->opaque >> 16);
  vnet_disconnect_args_t a = { session_handle (s), app_log_main.app_index };
  app_log_collector_wrk_t *cwrk;
  ...
  /* Old session disconnected, try to reallocate connection */
  app_log_collector_program_connect (c->collector_index);
}
...
static int
app_log_collector_add_segment_cb (u32 client_index, u64 segment_handle)
{
  return 0;
}

static int
app_log_collector_del_segment_cb (u32 app_wrk_index, u64 segment_handle)
{
  return 0;
}
...
static session_cb_vft_t app_logger_cb_vft = {
  ...
  .add_segment_callback = app_log_collector_add_segment_cb,
  .del_segment_callback = app_log_collector_del_segment_cb,
};
				     ...
				     vlib_cli_command_t *cmd)
{
  ...
  if (collector_uri)
      ...
      if (parse_uri ((char *) collector_uri, &cfg.sep))
	{
      ...
      if (is_add && app_log_collector_add (&cfg))
	{
	  error = clib_error_return (0, "Failed to add collector");
	  goto done;
	}
      if (!is_add && app_log_collector_del (&cfg))
	{
...
static u8 *
format_app_log_collector (u8 *s, va_list *args)
{
  app_log_collector_t *c = va_arg (*args, app_log_collector_t *);
  u32 i, indent;
...
static clib_error_t *
show_app_log_collector_command_fn (vlib_main_t *vm, unformat_input_t *input,
				   vlib_cli_command_t *cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  app_log_main_t *alm = &app_log_main;
  ...
  pool_foreach (c, alm->collectors)
    vlib_cli_output (vm, " %U", format_app_log_collector, c, 0);

done:
  return error;
}

VLIB_CLI_COMMAND (show_app_log_collector_command, static) = {
  .path = "show app log-collector",
  .short_help = "show app log-collector",
  .function = show_app_log_collector_command_fn,
};
