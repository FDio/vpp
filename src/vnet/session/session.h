...
/** Commit deferred RX data and release its retained buffer chains. */
always_inline void
session_flush_async_rx (svm_fifo_t *f)
{
  ...
  if (n_segs)
    {
      rv = svm_fifo_commit_async_segments (f);
      ASSERT (rv >= 0);
...
/** Enable or disable deferred RX for one session. */
always_inline void
...
#define SESSION_ASYNC_RX_MAX_SEGS 64
 ...
 */
u32 session_async_rx_consume (session_t *s, session_async_rx_consume_fn_t fn, void *ctx);

/** Release VLIB buffer-chain roots previously taken by a built-in app. */
void session_async_rx_release (u32 thread_index, const u32 *buffer_refs, u32 n_buffer_refs,
			       u32 n_bytes);
  ...
  if (PREDICT_FALSE (s->flags & SESSION_F_ASYNC_RX))
    {
      ...
      if (is_in_order && queue_event && svm_fifo_max_dequeue_cons (s->rx_fifo) == 0 &&
					 ...
					 u8 *is_buffer_held)
{
  ...
  ASSERT (s->thread_index == vlib_get_thread_index ());
  if (PREDICT_FALSE (svm_fifo_n_async_segments (s->rx_fifo) >= SESSION_ASYNC_RX_MAX_SEGS))
    session_flush_async_rx (s->rx_fifo);
  cur = b;
