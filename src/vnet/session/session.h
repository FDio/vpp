...
typedef struct session_worker_
{
  ...
  /** Pool of realloc rpc args sent to main */
  pool_realloc_rpc_args_t *pool_realloc_args;
...
pool_program_safe_realloc_rpc (void *args)
{
  ...
  pool_put (session_main.wrk[pra->thread_index].pool_realloc_args, pra);
...
pool_program_safe_realloc (void **p, u32 elt_size, u32 align)
{
  ...
  pool_get_zero (session_main.wrk[thread_index].pool_realloc_args, pra);
  pra->pool = p;
  pra->elt_size = elt_size;
  pra->align = align;
  pra->thread_index = thread_index;
