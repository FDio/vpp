...
stream_session_get_from_handle (u64 handle)
{
  ...
  return pool_elt_at_index(
      smm->sessions[stream_session_thread_from_handle (handle)],
...
stream_session_open (u32 app_index, session_type_t st,
		     transport_endpoint_t *tep, transport_connection_t **tc);
...
{
  return pool_elt_at_index(session_manager_main.listen_sessions[type], index);
}
