...
session_get (u32 si, u32 thread_index)
{
  ...
  return pool_elt_at_index (session_main.wrk[thread_index].sessions,
			    si);
}
...
session_get_if_valid (u64 si, u32 thread_index)
{
  ...
  if (pool_is_free_index (session_main.wrk[thread_index].sessions,
			  si))
    return 0;
...
session_get_worker (u32 thread_index)
{
  return &session_main.wrk[thread_index];
}

always_inline svm_msg_q_t *
session_get_vpp_event_queue (u32 thread_index)
{
...
int session_flush_enqueue_events (u8 proto, u32 thread_index);
int session_flush_all_enqueue_events (u8 transport_proto);
void session_flush_frames_main_thread (vlib_main_t * vm);
ssvm_private_t *session_get_evt_q_segment (void);
