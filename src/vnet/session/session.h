...
always_inline session_t *
session_worker_get_session_if_valid (session_worker_t *wrk, u32 si)
{
  session_t *s;
  ...
  if (PREDICT_FALSE (wrk->session_expand_pending))
	...
	s = wrk->sessions_expand + (si - len);
    }
  else
    {
      if (pool_is_free_index (wrk->sessions, si))
	return 0;
...
session_get_if_valid (u64 si, u32 thread_index)
{
  ...
  return session_worker_get_session_if_valid (wrk, si);
...
session_clone_safe (u32 session_index, u32 thread_index)
{
  ...
  old_s = session_worker_get_session (&session_main.wrk[thread_index],
				      session_index);
