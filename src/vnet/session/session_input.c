  ...
  for (i = 0; i < n_evts; i++)
	...
	case SESSION_CTRL_EVT_TRANSPORT_CLOSED:
	  ...
	  if (PREDICT_FALSE (s->app_wrk_index == APP_INVALID_INDEX))
	    {
	      /* No cleanup notification pending, mark events as flushed. */
	      if (s->session_state < SESSION_STATE_TRANSPORT_DELETED)
		s->al_index = SESSION_INVALID_INDEX;
	      break;
	    }
	...
	case SESSION_CTRL_EVT_CLEANUP:
	  ...
	  if (evt->as_u64[0] >> 32 != SESSION_CLEANUP_SESSION)
	    {
	      /* TODO(fcoras): We should be detaching connection_index but
	       * cut-throughs need it to be valid until session cleanup. Until
	       * fixed, overload al_index */
	      s->al_index = SESSION_INVALID_INDEX;
	      break;
	    }
	  uword_to_pointer (evt->as_u64[1], void (*) (session_t * s)) (s);
