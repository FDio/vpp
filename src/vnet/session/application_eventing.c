...
app_evt_collector_log_session (app_evt_collector_t *c, session_t *s)
{
  ...
  clib_memcpy_fast (data_msg->conn_id, tc->opaque_conn_id,
  ...
  switch (tc->proto)
	...
	data_msg->msg_len =
	  sizeof (app_evt_msg_data_t) + sizeof (tcp_transport_stats_t);
...
#undef _
	...
	data_msg->msg_len =
	  sizeof (app_evt_msg_data_t) + sizeof (udp_transport_stats_t);
