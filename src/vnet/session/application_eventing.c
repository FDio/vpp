...
static app_evt_buffer_chunk_t *
app_evt_buffer_chunk_alloc (app_evt_buffer_t *buf)
{
...
static app_evt_buffer_chunk_t *
app_evt_buffer_chunk_get (app_evt_buffer_t *buf, u32 chunk_index)
{
...
static void
app_evt_buffer_chunk_free (app_evt_buffer_t *buf,
			   app_evt_buffer_chunk_t *chunk)
{
			     ...
			     app_evt_buffer_chunk_t *chunk)
{
  ...
  tail = app_evt_buffer_chunk_get (buf, buf->tail_chunk);
  tail->next_index = chunk->chunk_index;
  buf->tail_chunk = chunk->chunk_index;
}

static void
app_evt_collector_send (app_evt_collector_wrk_t *cwrk)
{
  ...
  c = app_evt_buffer_chunk_get (&cwrk->buf, cwrk->buf.head_chunk);
  while (c)
    {
	...
	break;
      ...
      c = app_evt_buffer_chunk_get (&cwrk->buf, c->next_index);
    }
  ...
  while (wrote > 0)
    {
      c = app_evt_buffer_chunk_get (&cwrk->buf, next_c);
      next_c = c->next_index;
      ASSERT (wrote >= c->len);
      wrote -= c->len;
      app_evt_buffer_chunk_free (&cwrk->buf, c);
    }
  ASSERT (wrote == 0);
...
{
  app_evt_msg_data_conn_stats_t *conn_stats;
  ...
  chunk = app_evt_buffer_chunk_alloc (&cwrk->buf);
  ...
  conn_stats = app_evt_buf_chunk_append_uninit (
    chunk, sizeof (app_evt_msg_data_conn_stats_t));
  conn_stats->transport_proto_type = tc->proto;
  ...
  switch (tc->proto)
	...
	tcp_transport_stats_t *tcp_stats = app_evt_buf_chunk_append_uninit (
	  chunk, sizeof (tcp_transport_stats_t));
	conn_stats->msg_len = sizeof (app_evt_msg_data_conn_stats_t) +
			      sizeof (tcp_transport_stats_t);
...
#undef _
	...
	udp_transport_stats_t *udp_stats = app_evt_buf_chunk_append_uninit (
	  chunk, sizeof (udp_transport_stats_t));
	conn_stats->msg_len = sizeof (app_evt_msg_data_conn_stats_t) +
			      sizeof (udp_transport_stats_t);
...
#undef _
  ...
  data_msg->msg_len = sizeof (app_evt_msg_data_t) + conn_stats->msg_len;
  msg->msg_len = sizeof (app_evt_msg_t) + data_msg->msg_len;

  app_evt_buffer_append_chunk (&cwrk->buf, chunk);
  app_evt_collector_send (cwrk);
}

app_evt_collector_t *
app_evt_collector_get (u32 slc_index)
{
  ...
  if (collector_uri)
    {
      ...
      if (alm->app_index == APP_INVALID_INDEX)
	{
	  error = clib_error_return (0, "app log collector not enabled");
	  goto done;
	}
				   ...
				   vlib_cli_command_t *cmd)
{
  ...
  vlib_cli_output (vm, "app log-collector app-index: %u", alm->app_index);
...
VLIB_CLI_COMMAND (show_app_evt_collector_command, static) = {
  .path = "show app log-collector",
  .short_help = "show app log-collector",
