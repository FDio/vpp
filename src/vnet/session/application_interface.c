...
{
  if (sep->sw_if_index != ENDPOINT_INVALID_INDEX
  ...
  if (application_has_local_scope (app))
    {
      ...
      if (listener)
	{
	  if (listen_session_get_port_and_proto (listener, &sep.port,
	                                         &sep.transport_proto))
	    {
	...
	{
	  if (session_lookup_local_listener_parse_handle (handle, &sep.port,
		                                          &sep.transport_proto))
	    {
...
{
  if (server->cb_fns.redirect_connect_callback (server->api_client_index, mp))
    return clib_error_return_code(0, VNET_API_ERROR_SESSION_REDIRECT, 0,
	                          "redirect failed");
  return 0;
}
                ...
                void *mp)
{
  ...
  if (session_endpoint_is_local (sep) || !application_has_global_scope (app))
    {
      ...
      return clib_error_return_code (0, VNET_API_ERROR_APP_CONNECT_SCOPE, 0,
                                     "connect scope");
    }
  ...
  if (application_open_session (app, sep, api_context))
    return clib_error_return_code (0, VNET_API_ERROR_SESSION_CONNECT, 0,
                                   "session open");
  return 0;
}
