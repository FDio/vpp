...
session_test_rules (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  tc = session_lookup_connection4 (0, &lcl_pref.fp_addr.ip4,
	                           ...
	                           TRANSPORT_PROTO_TCP);
  SESSION_TEST ((tc->c_index == listener->connection_index), "optimized lookup"
      " should return the listener");
  tc = session_lookup_connection_wt4 (0, &lcl_pref.fp_addr.ip4,
	                              &rmt_pref.fp_addr.ip4, lcl_port, rmt_port,
	                              TRANSPORT_PROTO_TCP, 0);
  SESSION_TEST ((tc->c_index == listener->connection_index), "lookup should "
      "return the listener");
  ...
  SESSION_TEST ((tc == 0), "optimized lookup for wrong lcl port + 1 should not"
      " work");

  /*
   * Add rule with wildcarded lcl port
   */
