...
u8
RT (rule_is_exact_match) (RTT (session_rule) *key, RTT (session_rule) *r)
{
  int i;
				   ...
				   RTT (session_rule) *rule)
{
  ...
  for (i = 0; i < vec_len (parent->next_indices); i++)
    {
      child = RT (session_rules_table_get_rule) (srt, parent->next_indices[i]);
				   ...
				   u32 rule_index)
{
  ...
  for (i = 0; i < vec_len(rp->next_indices); i++)
    {
      rv = RT (session_rules_table_del_rule) (srt, rule,
					      rp->next_indices[i]);
      if (rv == 1)
	{
	  RTT (session_rule) *child;
	  u32 *next_indices = 0, *new_elts, left_to_add;
	  child = RT (session_rules_table_get_rule) (srt, rp->next_indices[i]);
...
RTT (session_rule) *
RT (session_rule_free) (RTT(session_rules_table) *srt, RTT(session_rule) *rule)
{
