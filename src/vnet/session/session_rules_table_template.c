...
u8 RT (rule_is_exact_match) (RTT (session_rule) * key, RTT (session_rule) * r)
{
  int i;
...
u8
RT (rule_is_match_for_key) (RTT (session_mask_or_match) * key,
			    RTT (session_rule) * r)
{
...
RTT (session_rule) * RT (session_rule_alloc) (RTT (session_rules_table) * srt)
{
...
RTT (session_rule) *
RT (session_rule_free) (RTT (session_rules_table) * srt,
			RTT (session_rule) * rule)
{
...
static RTT (session_rules_table) *RTT (sort_srt);

int
RT(session_rules_sort_indices) (void *e1, void *e2)
{
  u32 *ri1 = e1, *ri2 = e2;
  RTT (session_rule) *rule1, *rule2;
  rule1 = RT (session_rules_table_get_rule) (RTT (sort_srt), *ri1);
  rule2 = RT (session_rules_table_get_rule) (RTT (sort_srt), *ri2);
  return RTT (sort_srt)->rule_cmp_fn (rule1, rule2);
}

void
RT (session_rules_sort) (RTT (session_rules_table) *srt, u32 *next_indices)
{
  RTT(sort_srt) = srt;
  vec_sort_with_function (next_indices, RT(session_rules_sort_indices));
}
				   ...
				   RTT (session_rule) * rule)
{
  ...
  RTT (session_rule) * parent, *child;

  rule_index = RT (session_rules_table_rule_index) (srt, rule);
  ...
  for (i = 0; i < vec_len (parent->next_indices); i++)
    {
      child =
	RT (session_rules_table_get_rule) (srt, parent->next_indices[i]);
...
int
RT (session_rules_table_del_rule) (RTT (session_rules_table) * srt,
				   RTT (session_rule) * rule, u32 rule_index)
{
  RTT (session_rule) * rp;
  u32 rv;
  int i;
  ...
  for (i = 0; i < vec_len (rp->next_indices); i++)
    {
      rv = RT (session_rules_table_del_rule) (srt, rule, rp->next_indices[i]);
      if (rv == 1)
	{
	  RTT (session_rule) * child;
	  u32 *next_indices = 0, *new_elts, left_to_add;
	  child =
	    RT (session_rules_table_get_rule) (srt, rp->next_indices[i]);
	  ...
	  RT (session_rule_free) (srt, child);
