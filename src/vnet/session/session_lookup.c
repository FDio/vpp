				     ...
				     ip4_address_t * rmt, u16 rmt_port)
{
  ...
  if (st->session_rules == 0)
    return 0;
  srt = &st->session_rules[proto];
  ...
  if (st->session_rules)
    {
       ...
       */
      action_index = session_rules_table_lookup4 (
	&st->session_rules[proto], lcl, rmt, lcl_port, rmt_port);
			       ...
			       u8 * result)
{
  ...
  if (st->session_rules)
    {
      /* Check the session rules table */
      action_index = session_rules_table_lookup6 (
	&st->session_rules[proto], lcl, rmt, lcl_port, rmt_port);
      if (session_lookup_action_index_is_valid (action_index))
	{
	  if (action_index == SESSION_RULES_TABLE_ACTION_DROP)
