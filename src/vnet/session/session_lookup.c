...
u32
session_lookup_get_index_for_nns (u32 nns_index)
{
 ...
 */
int
session_lookup_add_connection (transport_connection_t * tc,
                              u64 value)
{
  session_table_t *slt;
  session_kv4_t kv4;
  ...
  if (sep->is_ip4)
    {
      ...
      kv4.value = value;
      return clib_bihash_add_del_16_8 (&slt->v4_session_hash, &kv4, 1);
    }
  else
    {
      make_v6_listener_kv (&kv6, &sep->ip.ip6, sep->port, sep->transport_proto);
      kv6.value = value;
      return clib_bihash_add_del_48_8 (&slt->v6_session_hash, &kv6, 1);
    }
}
...
session_lookup_del_session_endpoint (u32 table_index, session_endpoint_t *sep)
{
  ...
  if (sep->is_ip4)
    {
      make_v4_listener_kv (&kv4, &sep->ip.ip4, sep->port, sep->transport_proto);
      return clib_bihash_add_del_16_8 (&slt->v4_session_hash, &kv4, 0);
    }
  else
    {
      make_v6_listener_kv (&kv6, &sep->ip.ip6, sep->port, sep->transport_proto);
      return clib_bihash_add_del_48_8 (&slt->v6_session_hash, &kv6, 0);
    }
}
...
session_lookup_listener4_i (session_table_t *slt, ip4_address_t * lcl,
...
static stream_session_t *
session_lookup_listener6_i (session_table_t *slt, ip6_address_t * lcl,
                            u16 lcl_port, u8 proto)
{
  session_kv6_t kv6;
  int rv;
...
session_lookup_local_listener_make_handle (session_endpoint_t *sep)
{
  return ((u64) SESSION_LOCAL_TABLE_PREFIX << 32
      | (u32) sep->port << 8
      | (u32) sep->transport_proto);
}

int
session_lookup_local_listener_parse_handle (u64 handle, u16 *port, u8 *proto)
{
  u32 local_table_handle;
  if (handle >> 32 != SESSION_LOCAL_TABLE_PREFIX)
    return -1;
  local_table_handle = handle & 0xFFFFFFFFULL;
  *port = local_table_handle >> 8;
  *proto = local_table_handle & 0xff;
  return 0;
}
...
session_lookup_init (void)
{
   ...
   */
  session_table_t *slt = session_table_alloc ();
