...
{
  session_table_t *st;
  st = session_table_get_for_fib_index (fib_index);
  if (!st)
    return 0;
...
session_lookup_half_open_handle (transport_connection_t *tc)
{
  ...
  st = session_table_get_for_fib_index (tc->fib_index);
  if (tc->is_ip4)
    {
...
void
session_lookup_show_table_entries (vlib_main_t *vm, session_table_t *table, u8 type,
                                   u8 is_local)
{
