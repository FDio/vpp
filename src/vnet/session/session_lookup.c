				     ...
				     ip4_address_t * rmt, u16 rmt_port)
{
  ...
  action_index = session_rules_table_lookup4 (st->srt_handle, proto, lcl, rmt,
					      lcl_port, rmt_port);
  app_index = session_lookup_action_to_handle (action_index);
  /* Nothing sophisticated for now, action index is app index */
				     ...
				     ip6_address_t * rmt, u16 rmt_port)
{
  ...
  action_index = session_rules_table_lookup6 (st->srt_handle, proto, lcl, rmt,
					      lcl_port, rmt_port);
  app_index = session_lookup_action_to_handle (action_index);
				  ...
				  u8 use_rules)
{
  ...
  if (sep->is_ip4)
      ...
      if (use_rules)
	{
	  ...
	  ai =
	    session_rules_table_lookup4 (st->srt_handle, sep->transport_proto,
      ...
      if (use_rules)
	{
	  ...
	  ai =
	    session_rules_table_lookup6 (st->srt_handle, sep->transport_proto,
  ...
  if (st->srt_handle != SESSION_SRT_HANDLE_INVALID)
    {
       ...
       */
      action_index = session_rules_table_lookup4 (st->srt_handle, proto, lcl,
			       ...
			       u8 * result)
{
  ...
  if (st->srt_handle != SESSION_SRT_HANDLE_INVALID)
    {
      /* Check the session rules table */
      action_index = session_rules_table_lookup6 (st->srt_handle, proto, lcl,
						  rmt, lcl_port, rmt_port);
      if (session_lookup_action_index_is_valid (action_index))
	{
	  if (action_index == SESSION_RULES_TABLE_ACTION_DROP)
  ...
  if ((args->scope & SESSION_RULE_SCOPE_GLOBAL) || args->scope == 0)
    {
      ...
      if ((rv = session_rules_table_add_del (
	     st->srt_handle, args->transport_proto, &args->table_args)))
	return rv;
    }
  if (args->scope & SESSION_RULE_SCOPE_LOCAL)
    {
      ...
      rv = session_rules_table_add_del (st->srt_handle, args->transport_proto,
					&args->table_args);
    }
  return rv;
}
				 ...
				 u8 transport_proto)
{
  ...
  if (st == 0)
    return;
  session_rules_table_cli_dump (vm, st->srt_handle, transport_proto,
				fib_proto);
}
			       ...
			       vlib_cli_command_t * cmd)
{
  ...
  if (show_one)
    {
      if (st)
	session_rules_table_show_rule (vm, st->srt_handle, transport_proto,
