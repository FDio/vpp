...
session_lookup_app_namespace_walk_cb (app_namespace_t *app_ns, void *ctx)
{
  ...
  u32 appns_index = app_namespace_index (app_ns);
  ...
  st = session_table_get (table_index);
  if (st && (ct->in_appns_index != appns_index) &&
      (ct->fib_index == fib_index))
    {
      ct->out_appns_index = appns_index;
...
session_lookup_table_cleanup (u32 fib_proto, u32 fib_index, u32 ns_index)
{
  ...
  if (fib_index_to_lock_count[fib_proto][fib_index] == 0)
      ...
      if (ns_index == st->appns_index)
	{
	  ...
	  ctx.in_appns_index = ns_index;
	  ctx.fib_index = fib_index;
	  app_namespace_walk (session_lookup_app_namespace_walk_cb, &ctx);
	  st->appns_index = ctx.out_appns_index;
	}
    }
}
