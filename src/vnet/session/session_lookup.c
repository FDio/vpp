...
session_table_alloc (void)
{
  ...
  pool_get (lookup_tables, slt);
  memset (slt, 0, sizeof (*slt));
  return slt;
}

u32
session_table_index (session_lookup_table_t *slt)
{
  return (slt - lookup_tables);
}

static session_lookup_table_t *
session_get_table (u32 table_index)
{
  return vec_elt_at_index (lookup_tables, table_index);
}

static session_lookup_table_t *
session_get_or_alloc_table_for_connection (transport_connection_t *tc)
{
  session_lookup_table_t *slt;
  u32 table_index;
...
static session_lookup_table_t *
session_get_table_for_connection (transport_connection_t *tc)
{
  if (vec_len (nns_index_to_table_index) <= tc->fib_index)
    return 0;
  return session_get_table (nns_index_to_table_index[tc->fib_index]);
}

static session_lookup_table_t *
session_get_table_for_fib_index (u32 fib_index)
{
  if (vec_len (nns_index_to_table_index) <= fib_index)
    return 0;
  return session_get_table (nns_index_to_table_index[fib_index]);
}
                              ...
                              u64 value)
{
  ...
  session_kv4_t kv4;
  session_kv6_t kv6;
  slt = session_get_or_alloc_table_for_connection (tc);
  if (tc->is_ip4)
    {
...
void
session_table_add_session (u32 table_index, stream_session_t * s, u64 value)
{
  transport_connection_t *tc;
  tc = tp_vfts[s->session_type].get_connection (s->connection_index,
						s->thread_index);
  session_table_add_connection (table_index, tc, value);
}
...
session_table_del_connection (transport_connection_t * tc)
{
  ...
  slt = session_get_table_for_connection (tc);
  if (!slt)
    return -1;
  if (tc->is_ip4)
    {
...
session_lookup_listener4_i (session_lookup_table_t *slt,
                                  ip4_address_t * lcl, u16 lcl_port, u8 proto)
{
  session_kv4_t kv4;
  int rv;
...
{
  session_lookup_table_t *slt;
  slt = session_get_table_for_fib_index (fib_index);
  if (!slt)
    return 0;
...
{
  session_lookup_table_t *slt;
  slt = session_get_table (table_index);
  if (sep->is_ip4)
...
session_lookup_half_open_handle (transport_connection_t *tc)
{
  ...
  slt = session_get_table_for_fib_index (tc->fib_index);
  if (tc->is_ip4)
    {
