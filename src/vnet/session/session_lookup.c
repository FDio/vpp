				       ...
				       session_endpoint_t * sep)
{
  ...
  session_kv4_t kv4;
  session_kv6_t kv6;
  ip4_address_t lcl4 = {0};
  ip6_address_t lcl6 = {0};
  u32 si;
  int rv;
  ...
  if (sep->is_ip4)
	...
	return (u32) kv4.value;
      ...
      si = session_rules_table_lookup4 (&st->session_rules, sep->transport_proto,
		      ...
		      u16 lcl_port, u16 rmt_port, u8 proto)
{
  ...
  if ((s = session_lookup_listener4_i (st, lcl, lcl_port, proto)))
    return s;
  return session_lookup_rules_table4 (&st->session_rules, proto, lcl, lcl_port,
	                              rmt, rmt_port);
}
		      ...
		      u16 lcl_port, u16 rmt_port, u8 proto)
{
  ...
  if ((s = session_lookup_listener6_i (st, lcl, lcl_port, proto)))
    return s;
  return session_lookup_rules_table6 (&st->session_rules, proto, lcl, lcl_port,
	                              rmt, rmt_port);
}
