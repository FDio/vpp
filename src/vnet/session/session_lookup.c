...
session_table_get_or_alloc (u8 fib_proto, u32 fib_index, u8 *ns_id)
{
  session_table_t *st;
  u32 table_index;
  ...
  if (session_table_is_alloced (fib_proto, fib_index))
  ...
  else
    {
      ...
      session_table_init (st, fib_proto, ns_id, SESSION_RULE_SCOPE_GLOBAL);
  ...
  for (fp = 0; fp < ARRAY_LEN (fib_index_to_table_index); fp++)
    {
      ...
      st = session_table_get_or_alloc (fp, fib_index, app_ns->ns_id);
      if (st)
	{
...
session_lookup_init (void)
{
  ...
  session_table_init (st, FIB_PROTOCOL_IP4, 0, SESSION_RULE_SCOPE_GLOBAL);
  st = session_table_alloc ();
  ...
  session_table_init (st, FIB_PROTOCOL_IP6, 0, SESSION_RULE_SCOPE_GLOBAL);
}
