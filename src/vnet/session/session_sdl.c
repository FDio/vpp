...
session_sdl_table_free (session_table_t *st, u8 fib_proto)
{
  ...
  for (int i = 1; i < vec_len (st->session_rules); i++)
    st->session_rules[i].sdl_block = srt->sdl_block;
}

static session_error_t
session_sdl_add_del (session_table_t *st, session_rules_table_t *srt,
		     session_rule_table_add_del_args_t *args)
{
  ...
  if (args->lcl.fp_proto == FIB_PROTOCOL_IP4)
    {
      if (sdlb->ip_fib_index == ~0)
	session_sdl_table_init (st, args->lcl.fp_proto);
