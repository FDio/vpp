...
static u32
session_sdl_lookup6 (session_rules_table_t *srt, ip6_address_t *lcl_ip,
		     ip6_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
  session_sdl_block_t *sdlb = &srt->sdl_block;

  index_t lbi = ip6_fib_table_fwding_lookup (sdlb->ip6_fib_index, lcl_ip);
  const dpo_id_t *dpo =
...
static u32
session_sdl_lookup4 (session_rules_table_t *srt, ip4_address_t *lcl_ip,
		     ip4_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
  session_sdl_block_t *sdlb = &srt->sdl_block;

  index_t lbi = ip4_fib_forwarding_lookup (sdlb->ip_fib_index, lcl_ip);
  const dpo_id_t *dpo =
    load_balance_get_fwd_bucket (load_balance_get (lbi), 0);
...
static void
session_sdl4_fib_table_show_all_by_fib_index (session_rules_table_t *srt,
					      u32 fib_index, vlib_main_t *vm)
{
  session_sdl_block_t *sdlb = &srt->sdl_block;
  ...
  vec_foreach (fei, ctx.ifsw_indicies)
	  ...
	  fib_entry_is_sourced (*fei, sdl_fib_src))
	{
	  ...
	  index_t lbi =
	    ip4_fib_forwarding_lookup (sdlb->ip_fib_index, &pfx.fp_addr.ip4);
	  const dpo_id_t *dpo =
	    load_balance_get_fwd_bucket (load_balance_get (lbi), 0);

	  vlib_cli_output (vm, "[%d] rule: %U/%d action: %d tag: %U", *fei,
			   format_ip4_address, &pfx.fp_addr.ip4, pfx.fp_len,
			   dpo->dpoi_index, format_session_rule_tag, tag);
	}
    }
...
static void
session_sdl6_fib_table_show_all_by_fib_index (session_rules_table_t *srt,
					      u32 fib_index, vlib_main_t *vm)
{
  session_sdl_block_t *sdlb = &srt->sdl_block;
  ip6_fib_t *fib = ip6_fib_get (fib_index);
  fib_node_index_t *fei;
  ...
  vec_foreach (fei, ctx.entries)
	  ...
	  fib_entry_is_sourced (*fei, sdl_fib_src))
	{
	  ...
	  fib_entry_t *fib_entry = fib_entry_get (*fei);
	  fib_prefix_t pfx = fib_entry->fe_prefix;
	  index_t lbi = ip6_fib_table_fwding_lookup (sdlb->ip6_fib_index,
						     &pfx.fp_addr.ip6);
	  const dpo_id_t *dpo =
	    load_balance_get_fwd_bucket (load_balance_get (lbi), 0);

	  vlib_cli_output (vm, "[%d] rule: %U/%d action: %d tag %U", *fei,
			   format_ip6_address, &pfx.fp_addr.ip6, pfx.fp_len,
			   dpo->dpoi_index, format_session_rule_tag, tag);
	}
    }
...
static void
session_sdl_cli_dump (vlib_main_t *vm, session_rules_table_t *srt,
		      u8 fib_proto)
{
  session_sdl_block_t *sdlb = &srt->sdl_block;
  ...
  if (fib_proto == FIB_PROTOCOL_IP4)
    {
      ...
      session_sdl4_fib_table_show_all_by_fib_index (srt, sdlb->ip_fib_index,
						    vm);
    }
  else if (fib_proto == FIB_PROTOCOL_IP6)
    {
      vlib_cli_output (vm, "IP6 rules, fib index %d", sdlb->ip6_fib_index);
      session_sdl6_fib_table_show_all_by_fib_index (srt, sdlb->ip6_fib_index,
						    vm);
    }
}
				 ...
				 u32 mask_len)
{
  session_sdl_block_t *sdlb = &srt->sdl_block;
  ip4_fib_t *fib = ip4_fib_get (fib_index);
  fib_node_index_t fei = ip4_fib_table_lookup (fib, address, mask_len);
...
{
  session_sdl_block_t *sdlb = &srt->sdl_block;
  fib_node_index_t fei = ip6_fib_table_lookup (fib_index, address, mask_len);
  ...
  if (fei != FIB_NODE_INDEX_INVALID && fib_entry_is_sourced (fei, sdl_fib_src))
    {
      ...
      fib_prefix_t pfx = fib_entry->fe_prefix;
      index_t lbi =
	ip6_fib_table_fwding_lookup (sdlb->ip6_fib_index, &pfx.fp_addr.ip6);
      const dpo_id_t *dpo =
	load_balance_get_fwd_bucket (load_balance_get (lbi), 0);
...
static void
session_sdl_show_rule (vlib_main_t *vm, session_rules_table_t *srt,
...
void
session_sdl_block_init (session_sdl_block_t *sdlb)
{
  sdlb->ip_table_id = ~0;
  sdlb->ip6_table_id = ~0;
  sdlb->ip_fib_index = ~0;
  sdlb->ip6_fib_index = ~0;
}

static void
session_sdl_table_init (session_rules_table_t *srt, u8 fib_proto,
			const u8 *ns_id, u32 scope)
{
  ...
  if (fib_proto == FIB_PROTOCOL_IP4 || all)
    {
      if (sdlb->ip_table_id != ~0 || sdlb->ip_fib_index != ~0)
	return;
      if (ns_id)
	snprintf (name, sizeof (name), "sdl4 %s %s", ns_id,
		  (scope & SESSION_RULE_SCOPE_GLOBAL) ? "global" : "local");
      else
	snprintf (name, sizeof (name), "sdl4 %s",
		  (scope & SESSION_RULE_SCOPE_GLOBAL) ? "global" : "local");
  ...
  if (fib_proto == FIB_PROTOCOL_IP6 || all)
    {
      if (sdlb->ip6_table_id != ~0 || sdlb->ip6_fib_index != ~0)
	return;
      if (ns_id)
	snprintf (name, sizeof (name), "sdl6 %s %s", ns_id,
		  (scope & SESSION_RULE_SCOPE_GLOBAL) ? "global" : "local");
      else
	snprintf (name, sizeof (name), "sdl6 %s",
		  (scope & SESSION_RULE_SCOPE_GLOBAL) ? "global" : "local");
...
static void
session_sdl_table_free (session_rules_table_t *srt, u8 fib_proto)
{
  session_sdl_block_t *sdlb = &srt->sdl_block;
  u8 all = fib_proto > FIB_PROTOCOL_IP6 ? 1 : 0;
...
void
session_sdl_rules_table_init (session_table_t *st, session_rules_table_t *srt,
			      const u8 *ns_id, u32 fib_proto, u32 scope)
{
  session_rules_table_init (srt, fib_proto, ns_id, scope);
  /* propagate the changes to all protos */
  for (int i = 1; i < TRANSPORT_N_PROTOS; i++)
    st->session_rules[i].sdl_block = srt->sdl_block;
}

void
session_sdl_rules_table_free (session_table_t *st, session_rules_table_t *srt,
			      u32 fib_proto)
{
  session_rules_table_free (srt, fib_proto);
  /* propagate the changes to all transport protocols */
  for (int i = 1; i < TRANSPORT_N_PROTOS; i++)
    st->session_rules[i].sdl_block = srt->sdl_block;
}

static session_error_t
session_sdl_add_del (session_rules_table_t *srt,
		     session_rule_table_add_del_args_t *args)
{
  ...
  paths = session_sdl_fib_create_route_paths (sdlb->ip_fib_index, dpo_proto);
  if (args->is_add)
    {
...
static const session_engine_vft_t session_sdl_vft = {
...
static void
session_sdl_app_namespace_walk_disable_cb (app_namespace_t *app_ns, void *ctx)
{
  ...
  if (st)
    {
      ...
      session_sdl_rules_table_free (st, srt, FIB_PROTOCOL_MAX);
    }
  ...
  if (st)
    {
      srt = st->session_rules;
      session_sdl_rules_table_free (st, srt, FIB_PROTOCOL_IP4);
    }
  ...
  if (st)
    {
      srt = st->session_rules;
      session_sdl_rules_table_free (st, srt, FIB_PROTOCOL_IP6);
    }
}

static void
session_sdl_app_namespace_walk_enable_cb (app_namespace_t *app_ns, void *ctx)
{
  session_rules_table_t *srt;
  u32 fib_index, table_index;
  session_table_t *st;
  ...
  if (st)
    {
      srt = st->session_rules;
      session_sdl_rules_table_init (st, srt, app_namespace_id (app_ns),
				    FIB_PROTOCOL_MAX,
...
session_sdl_enable_disable (int enable)
{
  ...
  if (enable)
    {
      error = session_rule_register_engine (&session_sdl_vft);
      if (error)
	{
      ...
      app_namespace_walk (session_sdl_app_namespace_walk_enable_cb, 0);
    }
  else
    {
      app_namespace_walk (session_sdl_app_namespace_walk_disable_cb, 0);

      error = session_rule_deregister_engine (&session_sdl_vft);
      if (error)
			...
			vlib_cli_command_t *cmd)
{
  ...
  session_rule_add_del_args_t args = {
    ...
    .table_args.rmt.fp_addr = { 0 },
    .table_args.rmt.fp_len = 0,
    .table_args.rmt.fp_proto = fib_proto,
    .table_args.lcl_port = 0,
...
VLIB_CLI_COMMAND (session_sdl_command, static) = {
  .path = "session sdl",
  .short_help =
    "session sdl [add|del] <lcl-ip/plen> action <action> [tag <tag>]",
			     ...
			     vlib_cli_command_t *cmd)
{
  ...
  if (show_one)
	...
	{
	  srt = st->session_rules;
	  session_rules_table_show_rule (vm, srt, &lcl_ip, 0, 0, 0,
					 (fib_proto == FIB_PROTOCOL_IP4));
	}
      return 0;
    }
