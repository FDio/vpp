...
static u32
session_sdl_lookup6 (u32 srt_handle, u32 proto, ip6_address_t *lcl_ip,
		     ip6_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
  session_rules_table_t *srt = srt_handle_to_srt (srt_handle, 0);
...
static u32
session_sdl_lookup4 (u32 srt_handle, u32 proto, ip4_address_t *lcl_ip,
...
static void
session_sdl_cli_dump (vlib_main_t *vm, u32 srt_handle, u32 proto, u8 fib_proto)
{
  session_rules_table_t *srt = srt_handle_to_srt (srt_handle, 0);
  session_sdl_block_t *sdlb = &srt->sdl_block;
  ...
  if (fib_proto == FIB_PROTOCOL_IP4)
    {
      ...
      session_sdl_table_walk4 (srt_handle, session_sdl_fib_table_show, vm);
    }
  else if (fib_proto == FIB_PROTOCOL_IP6)
    {
      vlib_cli_output (vm, "IP6 rules, fib index %d", sdlb->ip6_fib_index);
      session_sdl_table_walk6 (srt_handle, session_sdl_fib_table_show, vm);
    }
}
...
static void
session_sdl_show_rule (vlib_main_t *vm, u32 srt_handle, u32 proto,
...
session_sdl_table_init (session_table_t *st, u8 fib_proto)
{
  ...
  srtb = srtb_instance_alloc (st, 0);
  srt = srtb->session_rules;
  sdlb = &srt->sdl_block;
...
{
  session_rules_table_t *srt = srt_handle_to_srt (st->srt_handle, 0);
  session_sdl_block_t *sdlb;
  u8 all = fib_proto > FIB_PROTOCOL_IP6 ? 1 : 0;
...
session_sdl_add_del (u32 srt_handle, u32 proto,
			     ...
			     vlib_cli_command_t *cmd)
{
  ...
  if (show_one)
    {
      ...
      if (st && (st->srt_handle != SESSION_SRT_HANDLE_INVALID))
	session_rules_table_show_rule (vm, st->srt_handle, 0, &lcl_ip, 0, 0, 0,
				       (fib_proto == FIB_PROTOCOL_IP4));
      goto done;
    }
  ...
  if (st && (st->srt_handle != SESSION_SRT_HANDLE_INVALID))
    session_rules_table_cli_dump (vm, st->srt_handle, 0, FIB_PROTOCOL_IP4);

  fib_index = app_namespace_get_fib_index (app_ns, FIB_PROTOCOL_IP6);
  st = session_table_get_for_fib_index (FIB_PROTOCOL_IP6, fib_index);
  if (st && (st->srt_handle != SESSION_SRT_HANDLE_INVALID))
    session_rules_table_cli_dump (vm, st->srt_handle, 0, FIB_PROTOCOL_IP6);
done:
  vec_free (ns_id);
  return 0;
}
...
void
session_sdl_table_walk4 (u32 srt_handle, session_sdl_table_walk_fn_t fn,
			 void *args)
{
  ip4_fib_t *fib;
...
void
session_sdl_table_walk6 (u32 srt_handle, session_sdl_table_walk_fn_t fn,
			 void *args)
{
  ip6_fib_t *fib;
  fib_node_index_t *fei;
