			...
			const u8 *ns_id, u8 is_local)
{
  ...
  if (fib_proto == FIB_PROTOCOL_IP4 || all)
	...
	return;
      snprintf (name, sizeof (name), "sdl4 %s %s", ns_id ? ns_id : (u8 *) "",
  ...
  if (fib_proto == FIB_PROTOCOL_IP6 || all)
	...
	return;
      snprintf (name, sizeof (name), "sdl6 %s %s", ns_id ? ns_id : (u8 *) "",
		is_local ? "local" : "global");
...
session_sdl_rules_table_init (session_table_t *st, session_rules_table_t *srt,
			      const u8 *ns_id, u32 fib_proto, u8 is_local)
{
  session_rules_table_init (srt, fib_proto, ns_id, is_local);
