...
void
session_sdl_block_init (session_table_t *st)
{
  session_sdl_block_t *sdlb = &st->session_rules[0].sdl_block;
...
{
  session_rules_table_t *srt = st->session_rules;
  session_sdl_block_t *sdlb = &srt->sdl_block;
  u8 all = fib_proto > FIB_PROTOCOL_IP6 ? 1 : 0;
  char name[80];
  app_namespace_t *app_ns = app_namespace_get_if_valid (st->appns_index);

  if (fib_proto == FIB_PROTOCOL_IP4 || all)
    if ((sdlb->ip_table_id == ~0) && (sdlb->ip_fib_index == ~0))
      {
	snprintf (name, sizeof (name), "sdl4 %s %s",
		  app_ns ? app_ns->ns_id : (u8 *) "",
		  st->is_local ? "local" : "global");
...
session_sdl_app_namespace_walk_disable_cb (app_namespace_t *app_ns, void *ctx)
{
  ...
  if (st)
    session_sdl_table_free (st, FIB_PROTOCOL_MAX);
  ...
  if (st)
    session_sdl_table_free (st, FIB_PROTOCOL_IP4);
  ...
  st = session_table_get (table_index);
  if (st)
    session_sdl_table_free (st, FIB_PROTOCOL_IP6);
}
...
session_sdl_app_namespace_walk_enable_cb (app_namespace_t *app_ns, void *ctx)
{
  ...
  st = session_table_get (app_ns->local_table_index);
  if (st)
    session_sdl_table_init (st, FIB_PROTOCOL_MAX);

  fib_index = app_namespace_get_fib_index (app_ns, FIB_PROTOCOL_IP4);
  table_index = session_lookup_get_index_for_fib (FIB_PROTOCOL_IP4, fib_index);
  st = session_table_get (table_index);
  if (st)
    session_sdl_table_init (st, FIB_PROTOCOL_IP4);

  fib_index = app_namespace_get_fib_index (app_ns, FIB_PROTOCOL_IP6);
  table_index = session_lookup_get_index_for_fib (FIB_PROTOCOL_IP6, fib_index);
  st = session_table_get (table_index);
  if (st)
    session_sdl_table_init (st, FIB_PROTOCOL_IP6);
}
