  ...
  if ((fib_proto == FIB_PROTOCOL_IP4 || all) && (sdlb->ip_fib_index != ~0))
    {
      ...
      if (sdlm->cleanup_by_fib_index)
	sdlm->cleanup_by_fib_index (FIB_PROTOCOL_IP4, fib_index);
  ...
  if ((fib_proto == FIB_PROTOCOL_IP6 || all) && (sdlb->ip6_fib_index != ~0))
    {
      ...
      if (sdlm->cleanup_by_fib_index)
	sdlm->cleanup_by_fib_index (FIB_PROTOCOL_IP6, fib_index);
...
session_sdl_init (void)
{
  ...
  auto_sdl_plugin_exports_init (&auto_sdl_plugin);
}
...
int
session_sdl_register_callbacks (void)
{
#define _(name) sdlm->name = auto_sdl_plugin.name;
  foreach_auto_sdl_plugin_exported_method_name
#undef _
    if (sdlm->config == 0) return -1;
  return 0;
}

void
session_sdl_deregister_callbacks (void)
{
#define _(name) sdlm->name = NULL;
  foreach_auto_sdl_plugin_exported_method_name
#undef _
}
