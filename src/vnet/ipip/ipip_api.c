...
#define foreach_vpe_api_msg                                                    \
  _(IPIP_ADD_DEL_TUNNEL, ipip_add_del_tunnel)                                  \
  _(IPIP_TUNNEL_DUMP, ipip_tunnel_dump)

static void
vl_api_ipip_add_del_tunnel_t_handler(vl_api_ipip_add_del_tunnel_t *mp) {
  vl_api_ipip_add_del_tunnel_reply_t *rmp;
  int rv = 0;
  ...
  a->is_add = mp->is_add;
  a->transport = mp->is_ipv6 ? IPIP_TRANSPORT_IP6 : IPIP_TRANSPORT_IP4;
  a->instance = ntohl(mp->instance);

  /* ip addresses sent in network byte order */
  if (mp->is_ipv6) {
    ip_set(&a->src, mp->src_address, false);
    ip_set(&a->dst, mp->dst_address, false);
  } else {
    ip_set(&a->src, mp->src_address, true);
    ip_set(&a->dst, mp->dst_address, true);
  }
...
out:
  /* *INDENT-OFF* */
  REPLY_MACRO2(VL_API_IPIP_ADD_DEL_TUNNEL_REPLY,
