...
always_inline uword ipip_input(vlib_main_t *vm, vlib_node_runtime_t *node,
  ...
  while (n_left_from > 0) {
	...
	type0 = IP46_TYPE_IP4;
      ...
      ipip_tunnel_key_t key0 = { .type = type0,
      ...
      p = hash_get_mem(gm->tunnel_by_key, &key0);
      if (!p) {
	...
	goto drop;
      ...
      vlib_increment_combined_counter(
          ...
          tunnel_sw_if_index, 1 /* packets */, len /* bytes */);
...
static uword ipip4_input(vlib_main_t *vm, vlib_node_runtime_t *node,
                         vlib_frame_t *from_frame) {
  return ipip_input(vm, node, from_frame, /* is_ip6 */ false);
}

static uword ipip6_input(vlib_main_t *vm, vlib_node_runtime_t *node,
                         vlib_frame_t *from_frame) {
