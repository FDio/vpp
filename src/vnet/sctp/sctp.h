...
void sctp_make_initack (sctp_connection_t * ts, vlib_buffer_t * b);
...
{
  sctp_init_chunk_t *th;

  th = vlib_buffer_push_uninit (b, sctp_hdr_len);

  th->sctp_hdr.src_port = sp_net;
  th->sctp_hdr.dst_port = dp_net;
  th->sctp_hdr.checksum = 0;
  th->sctp_hdr.verification_tag = 0;
  th->chunk_hdr.type = clib_host_to_net_u32 (INIT);
