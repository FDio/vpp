...
typedef struct _sctp_connection
{
  ...
  u32 smallest_PMTU_idx; /** The index of the sub-connection with the smallest PMTU discovered across all peer's transport addresses. */
  ...
  u8 tmp_subconn_used; /**< Identifies current used subconn for DATA transmission */
...
always_inline void
update_smalles_pmtu_idx (sctp_connection_t * sctp_conn)
{
  u8 i;
...
/* As per RFC4960; section 7.2.1: Slow-Start */
always_inline void
update_cwnd (sctp_connection_t * sctp_conn, u8 idx)
{
  /* Section 7.2.1; point (2) */
  if (sctp_conn->sub_conn[idx].is_retransmitting)
    {
      sctp_conn->sub_conn[idx].cwnd = 1 * sctp_conn->sub_conn[idx].PMTU;
      return;
    }

  /* Section 7.2.2; point (4) */
  if (sctp_conn->sub_conn[idx].last_data_ts >
      sctp_time_now () + SCTP_DATA_IDLE_INTERVAL)
    {
      sctp_conn->sub_conn[idx].cwnd =
	clib_max (sctp_conn->sub_conn[idx].cwnd / 2,
		  4 * sctp_conn->sub_conn[idx].PMTU);
      return;
    }
  ...
  if (sctp_conn->sub_conn[idx].cwnd <= sctp_conn->sub_conn[idx].ssthresh)
    {
      if (!cwnd_fully_utilized (sctp_conn, idx))
	return;

      if (sctp_in_cong_recovery (sctp_conn, idx))
	return;

      sctp_conn->sub_conn[idx].cwnd =
	clib_min (sctp_conn->sub_conn[idx].PMTU, 1);
      sctp_conn->sub_conn[idx].cwnd -=
	sizeof (sctp_payload_data_chunk_t) - sizeof (sctp_full_hdr_t);
    }
}
