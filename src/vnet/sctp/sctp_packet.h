...
vnet_sctp_set_ebit (sctp_payload_data * p, u8 enable)
{
  ...
  p->header.flags = clib_host_to_net_u16 (enable);
}

static inline u8
vnet_sctp_get_ebit (sctp_payload_data * p)
{
  return (clib_net_to_host_u16 (p->header.flags));
}
...
{
  vnet_sctp_get_chunk_type (&p->header, DATA);
  p->header.flags = clib_host_to_net_u16 (enable << 1);
}

static inline u8
vnet_sctp_get_bbit (sctp_payload_data * p)
{
  return (clib_net_to_host_u16 (p->header.flags >> 1));
}
...
{
  vnet_sctp_get_chunk_type (&p->header, DATA);
  p->header.flags = clib_host_to_net_u16 (enable << 2);
}

static inline u8
vnet_sctp_get_ubit (sctp_payload_data * p)
{
  return (clib_net_to_host_u16 (p->header.flags >> 2));
}
...
static inline void
vnet_sctp_set_ipv4_address (sctp_ipv4_address * a, ip4_address_t address)
{
...
static inline void
vnet_sctp_set_ipv6_address (sctp_ipv6_address * a, ip6_address_t address)
{
  ...
  a->address.as_u64[0] = clib_host_to_net_u64 (address.as_u64[1]);
}
...
vnet_sctp_set_tbit (sctp_abort_chunk * a)
{
  ...
  a->header.flags = clib_host_to_net_u16 (1);
}
...
{
  vnet_sctp_set_chunk_type (&a->header, ABORT);
  a->header.flags = clib_host_to_net_u16 (0);
}
