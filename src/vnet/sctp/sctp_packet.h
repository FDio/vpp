...
static inline void
vnet_set_sctp_src_port (sctp_common_header * h, u16 src_port)
{
	h->src_port = clib_host_to_net_u16 (src_port);
}

static inline u16
vnet_get_sctp_src_port (sctp_common_header * h)
{
	return (clib_net_to_host_u16 (h->src_port));
}

static inline void
vnet_set_sctp_dst_port (sctp_common_header * h, u16 dst_port)
{
	h->dst_port = clib_host_to_net_u16 (dst_port);
}

static inline u16
vnet_get_sctp_dst_port (sctp_common_header * h)
{
	return (clib_net_to_host_u16 (h->dst_port));
}

static inline void
vnet_set_sctp_verification_tag (sctp_common_header * h, u32 verification_tag)
{
	h->verification_tag = clib_host_to_net_u32 (verification_tag);
}

static inline u32
vnet_get_sctp_verification_tag (sctp_common_header * h)
{
	return (clib_net_to_host_u32 (h->verification_tag));
}

static inline void
vnet_set_sctp_checksum (sctp_common_header * h, u32 checksum)
{
	h->checksum = clib_host_to_net_u32 (checksum);
}

static inline u32
vnet_get_sctp_checksum (sctp_common_header * h)
{
...
typedef struct
{
	sctp_common_header common_hdr;

	/*
	 * The "chunk" can represent any of the types described for the
	 * "sctp_chunk_fixed_hdr->type" below.
	 */
	u32 chunks[];
...
} sctp_chunk_fixed_hdr;

static inline void
vnet_sctp_set_chunk_type(sctp_chunk_fixed_hdr * h, sctp_chunk_type t)
{
	h->type = clib_host_to_net_u8 (t);
}

static inline u8
vnet_sctp_get_chunk_type(sctp_chunk_fixed_hdr * h, sctp_chunk_type t)
{
	return (clib_net_to_host_u8 (h->type));
}

static inline void
vnet_sctp_set_chunk_length(sctp_chunk_fixed_hdr * h, u16 length)
{
	h->length = clib_host_to_net_u16 (length);
}

static inline u16
vnet_sctp_get_chunk_length(sctp_chunk_fixed_hdr * h)
{
	return (clib_net_to_host_u16 (h->length));
}
...
static inline void
vnet_sctp_set_ebit(sctp_payload_data * p, u8 enable)
{
	vnet_sctp_get_chunk_type(p->header, DATA);
	p->header.flags = clib_host_to_net_u8 (enable);
}

static inline u8
vnet_sctp_get_ebit(sctp_payload_data * p)
{
	return (clib_net_to_host_u8 (p->header.flags));
}

static inline void
vnet_sctp_set_bbit(sctp_payload_data * p, u8 enable)
{
	vnet_sctp_get_chunk_type(p->header, DATA);
	p->header.flags = clib_host_to_net_u8 (enable << 1);
}

static inline u8
vnet_sctp_get_bbit(sctp_payload_data * p)
{
	return (clib_net_to_host_u8 (p->header.flags >> 1));
}

static inline void
vnet_sctp_set_ubit(sctp_payload_data * p, u8 enable)
{
	vnet_sctp_get_chunk_type(p->header, DATA);
	p->header.flags = clib_host_to_net_u8 (enable << 2);
}

static inline u8
vnet_sctp_get_ubit(sctp_payload_data * p)
{
	return (clib_net_to_host_u8 (p->header.flags >> 2));
}

static inline void
vnet_sctp_set_tsn(sctp_payload_data * p, u32 tsn)
{
	vnet_sctp_get_chunk_type(p->header, DATA);
...
static inline u32
vnet_sctp_get_tsn(sctp_payload_data * p)
{
	return (clib_net_to_host_u32(p->tsn));
}

static inline void
vnet_sctp_set_stream_id(sctp_payload_data * p, u16 stream_id)
{
	vnet_sctp_get_chunk_type(p->header, DATA);
...
static inline u16
vnet_sctp_get_stream_id(sctp_payload_data * p)
{
	return (clib_net_to_host_u16(p->stream_id));
}

static inline void
vnet_sctp_set_stream_seq(sctp_payload_data * p, u16 stream_seq)
{
	vnet_sctp_get_chunk_type(p->header, DATA);
...
static inline u16
vnet_sctp_get_stream_seq(sctp_payload_data * p)
{
	return (clib_net_to_host_u16(p->stream_seq));
}

static inline void
vnet_sctp_set_payload_id(sctp_payload_data * p, u32 payload_id)
{
	vnet_sctp_get_chunk_type(p->header, DATA);
...
static inline u32
vnet_sctp_get_payload_id(sctp_payload_data * p)
{
	return (clib_net_to_host_u32(p->payload_id));
}

static inline u16
vnet_sctp_calculate_payload_data_padding(sctp_payload_data * p)
{
	u16 payload_length = vnet_sctp_get_chunk_length(&p->header) -
		sizeof(p->header) -
		sizeof(p->tsn) - 
		sizeof(p->stream_id) -
		sizeof(p->stream_seq) -
...
} sctp_initiation_chunk;

/* OPTIONAL or VARIABLE-LENGTH parameters for INIT */
#define IPV4_ADDRESS_TYPE	5
#define IPV6_ADDRESS_TYPE	6
#define STATE_COOKIE		7
/* #define UNRECOGNIZED_PARAM	8 Defined with the rest of ERROR CAUSES: see line 796 in this file */
#define COOKIE_PRESERVATIVE	9
#define HOSTNAME_ADDRESS_TYPE 	11
#define SUPPORTED_ADDRESS_TYPES	12
...
static inline void
vnet_sctp_set_ipv4_address(sctp_ipv4_address * a, u32 address)
{
	a->type = clib_host_to_net_u16 (IPV4_ADDRESS_TYPE);
	a->length = clib_host_to_net_u16 (8);
	a->address = clib_host_to_net_u32 (address);
}

static inline u32
vnet_sctp_get_ipv4_address(sctp_ipv4_address * a)
{
	return (clib_net_to_host_u32 (a->address));
}
...
static inline void
vnet_sctp_set_ipv6_address(sctp_ipv6_address * a, u128 address)
{
	a->type = clib_host_to_net_u16 (IPV6_ADDRESS_TYPE);
	a->length = clib_host_to_net_u16 (20);
	a->address = clib_host_to_net_u128 (address);
}

static inline u128
vnet_sctp_get_ipv6_address(sctp_ipv6_address * a)
{
	return (clib_net_to_host_u128 (a->address));
}
...
static inline void
vnet_sctp_set_cookie_preservative(sctp_cookie_preservative * c, u32 life_span_inc)
{
	c->type = clib_host_to_net_u16 (COOKIE_PRESERVATIVE);
	c->length = clib_host_to_net_u16 (8);
	c->life_span_inc = clib_host_to_net_u32 (life_span_inc);
}

static inline u32
vnet_sctp_get_cookie_preservative(sctp_cookie_preservative * c)
{
...
static inline void
vnet_sctp_set_hostname_address(sctp_hostname_address * h, char* hostname)
{
...
static inline void
vnet_sctp_set_supported_address_types(sctp_supported_address_types * s)
{
	s->type = clib_host_to_net_u16 (SUPPORTED_ADDRESS_TYPES);
	s->length = 4 /* base = type + length */ + 
		MAX_SUPPORTED_ADDRESS_TYPES * 
		4; /* each address type is 4 bytes */

	s->address_type[0] = clib_host_to_net_u16 (IPV4_ADDRESS_TYPE);
	s->address_type[1] = clib_host_to_net_u16 (IPV6_ADDRESS_TYPE);
	s->address_type[2] = clib_host_to_net_u16 (HOSTNAME_ADDRESS_TYPE);
}
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
typedef sctp_initiation_chunk sctp_initiation_ack_chunk;

/* OPTIONAL or VARIABLE-LENGTH parameters for INIT_ACK */
typedef struct
{
	u16 type; /* = 7 */
	u16 length; /* Variable length */
	u32 value[];

} sctp_state_cookie;

static inline void
vnet_sctp_set_state_cookie(sctp_state_cookie * s)
{
	s->type = clib_host_to_net_u16 (STATE_COOKIE);
...
static inline void
vnet_sctp_set_unrecognized_param(sctp_unrecognized_param * u)
{
	u->type = clib_host_to_net_u16 (UNRECOGNIZED_PARAMETER);
...
} sctp_selective_ack_chunk;

static inline void
vnet_sctp_set_cumulative_tsn_ack(sctp_selective_ack_chunk * s, u32 cumulative_tsn_ack)
{
	vnet_sctp_set_chunk_type(&s->header, SACK);
	s->cumulative_tsn_ack = clib_host_to_net_u32 (cumulative_tsn_ack);
}

static inline u32
vnet_sctp_get_cumulative_tsn_ack(sctp_selective_ack_chunk * s)
{
	return clib_net_to_host_u32 (s->cumulative_tsn_ack);
}

static inline void
vnet_sctp_set_arwnd(sctp_selective_ack_chunk * s, u32 a_rwnd)
{
	vnet_sctp_set_chunk_type(&s->header, SACK);
...
static inline u32
vnet_sctp_get_arwnd(sctp_selective_ack_chunk * s)
{
	return clib_net_to_host_u32 (s->a_rwnd);
}

static inline void 
vnet_sctp_set_gap_ack_blocks_count(sctp_selective_ack_chunk * s, u16 gap_ack_blocks_count)
{
	vnet_sctp_set_chunk_type(&s->header, SACK);
...
static inline u16
vnet_sctp_get_gap_ack_blocks_count(sctp_selective_ack_chunk * s)
{
	return clib_net_to_host_u32 (s->gap_ack_blocks_count);
}

static inline void
vnet_sctp_set_duplicate_tsn_count(sctp_selective_ack_chunk * s, u16 duplicate_tsn_count)
{
	vnet_sctp_set_chunk_type(&s->header, SACK);
...
static inline u16
vnet_sctp_get_duplicate_tsn_count(sctp_selective_ack_chunk * s)
{
...
static inline void
vnet_sctp_set_heartbeat_info(sctp_heartbeat_info * h, u64 hb_info, u16 hb_info_length)
{
	h->hb_info = clib_host_to_net_u16 (1);
	h->hb_info_length = clib_host_to_net_u16 (hb_info_length);
...
typedef struct
{
	...
	sctp_heartbeat_info hb_info;

} sctp_heartbeat_request_chunk;

static inline void
vnet_sctp_set_hb_request_info(sctp_heartbeat_request_chunk * h, sctp_heartbeat_info * hb_info)
{
	vnet_sctp_set_chunk_type(&h->header, HEARTBEAT);
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
typedef sctp_heartbeat_request_chunk sctp_heartbeat_ack_chunk;

static inline void
vnet_sctp_set_hb_ack_info(sctp_heartbeat_ack_chunk * h, sctp_heartbeat_info * hb_info)
{
	vnet_sctp_set_chunk_type(&h->header, HEARTBEAT_ACK);
	memcpy(&h->hb_info, hb_info, sizeof(h->hb_info));
}
...
typedef struct
{
	...
	sctp_error_cause err_causes[];

} sctp_abort_chunk;

static inline void
vnet_sctp_set_tbit(sctp_abort_chunk * a)
{
	vnet_sctp_set_chunk_type(&a->header, ABORT);
	a->header.flags = clib_host_to_net_u8 (1);
}

static inline void
vnet_sctp_unset_tbit(sctp_abort_chunk * a)
{
	vnet_sctp_set_chunk_type(&a->header, ABORT);
	a->header.flags = clib_host_to_net_u8 (0);
}
...
} sctp_shutdown_association_chunk;

static inline void
vnet_sctp_set_tsn_last_received_chunk(sctp_shutdown_association_chunk * s, u32 tsn_last_chunk)
{
	vnet_sctp_set_chunk_type(&s->header, SHUTDOWN);
...
static inline void
vnet_sctp_fill_shutdown_ack(sctp_shutdown_ack * s)
{
	vnet_sctp_set_chunk_type(&s->header, SHUTDOWN_ACK);
	vnet_sctp_set_chunk_length(&s->header, 4);
}
