		      ...
		      u16 sctp_implied_length)
{
  ...
  if (length > sizeof (sctp_init_ack_chunk_t))
      ...
      while (pointer_offset < length)
	{
	  ...
	  SCTP_ADV_DBG
	    ("opt_params_hdr->type = %s, opt_params_hdr->length = %u",
	  ...
	  switch (clib_net_to_host_u16 (opt_params_hdr->type))
		...
		SCTP_ADV_DBG ("state_cookie_param->creation_time = %u, "
...
{
  int written, error = SCTP_ERROR_NONE;

  written =
    session_enqueue_stream_connection (&tc->sub_conn[conn_idx].connection, b,
				       0, 1 /* queue event */ , 1);

  SCTP_ADV_DBG ("written = %d, data_len = %u", written, data_len);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_ip4)
		...
		clib_net_to_host_u16 (ip4_hdr->length) - n_advance_bytes0;
	      ...
	      SCTP_ADV_DBG ("ip4_hdr->dst_address = %u,"
			    "ip4_hdr->src_address = %u,"
			    "sctp_hdr->dst_port = %u,"
			    "sctp_hdr->src_port = %u",
			    ip4_hdr->dst_address,
			    ip4_hdr->src_address,
			    sctp_hdr->dst_port, sctp_hdr->src_port);
		...
		n_advance_bytes0;
	      ...
	      SCTP_ADV_DBG ("ip6_hdr->dst_address = %u,"
			    "ip6_hdr->src_address = %u,"
			    "sctp_hdr->dst_port = %u,"
			    "sctp_hdr->src_port = %u",
			    ip6_hdr->dst_address,
			    ip6_hdr->src_address,
	  ...
	  if (PREDICT_TRUE (0 != sctp_conn))
		...
		(u8 *) sctp_hdr - (u8 *) vlib_buffer_get_current (b0);
	      ...
	      SCTP_DBG ("CONNECTION_INDEX = %u: "
			"CURRENT_CONNECTION_STATE = %s,"
			"CHUNK_TYPE_RECEIVED = %s "
			"NEXT_PHASE = %s",
			sctp_conn->
			sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.c_index,
			sctp_state_to_string (sctp_conn->state),
			sctp_chunk_to_string (type), phase_to_string (next0));
	    }
	  else
	    {
	      if (is_filtered)
	      ...
	      else
		{
	      ...
	      SCTP_DBG ("sctp_conn == NULL, NEXT_PHASE = %s",
