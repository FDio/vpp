			...
			u8 is_ip4)
{
  if (sctp_conn)
    {
      _clib_memcpy (&rx_trace->sctp_connection, sctp_conn,
  ...
  _clib_memcpy (&rx_trace->sctp_header, sctp_hdr,
  ...
  if (length > sizeof (sctp_init_chunk_t))
      ...
      while (pointer_offset < length)
	{
	  ...
	  switch (clib_net_to_host_u16 (opt_params_hdr->type))
		...
		_clib_memcpy (&ip4_addr, &ipv4->address,
		...
		_clib_memcpy (&ip6_addr, &ipv6->address,
		...
		_clib_memcpy (hostname, hostname_addr->hostname,
			      FQDN_MAX_LENGTH);
		break;
	      }
  ...
  if (length > sizeof (sctp_init_ack_chunk_t))
      ...
      while (pointer_offset < length)
	{
	  ...
	  switch (clib_net_to_host_u16 (opt_params_hdr->type))
		...
		_clib_memcpy (&(sctp_conn->cookie_param), state_cookie_param,
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  switch (chunk_type)
	      ...
	      if (error0 == SCTP_ERROR_NONE)
		{
		  ...
		  _clib_memcpy (new_sctp_conn, sctp_conn,
	...
	drop:
	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
		...
		vlib_add_trace (vm, node, b0, sizeof (*sctp_trace));
	      ...
	      if (sctp_hdr != NULL)
		_clib_memcpy (&sctp_trace->sctp_header, sctp_hdr,
			      sizeof (sctp_trace->sctp_header));

	      if (sctp_conn != NULL)
		_clib_memcpy (&sctp_trace->sctp_connection, sctp_conn,
	  ...
	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
	    {
	      sctp_rx_trace_t *t0 =
		vlib_add_trace (vm, node, b0, sizeof (*t0));
	      _clib_memcpy (&t0->sctp_header, sctp_hdr,
			    sizeof (t0->sctp_header));
	      _clib_memcpy (&t0->sctp_connection, sctp_listener,
