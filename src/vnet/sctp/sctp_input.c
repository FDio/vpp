		      ...
		      u16 sctp_implied_length)
{
  ...
  SCTP_DBG
    ("init_tag = %u, init_tsn = %u, A_RWND = %u, Inbound streams = %u, Outbound streams = %u",
     clib_net_to_host_u32 (init_ack_chunk->initiate_tag),
     clib_net_to_host_u32 (init_ack_chunk->initial_tsn),
     clib_net_to_host_u32 (init_ack_chunk->a_rwnd),
     clib_net_to_host_u16 (init_ack_chunk->inboud_streams_count),
     clib_net_to_host_u16 (init_ack_chunk->outbound_streams_count));
  ...
  if (length > sizeof (sctp_init_ack_chunk_t))
      ...
      while (pointer_offset < length)
	{
	  ...
	  SCTP_DBG ("opt_params_hdr->type = %s, opt_params_hdr->length = %u",
		    sctp_optparam_type_to_string (clib_net_to_host_u16
						  (opt_params_hdr->type)),
		    clib_net_to_host_u16 (opt_params_hdr->length));
	  ...
	  switch (clib_net_to_host_u16 (opt_params_hdr->type))
		...
		SCTP_DBG ("state_cookie_param->creation_time = %u, "
			  "state_cookie_param->cookie_lifespan = %u, "
			  "state_cookie_param->mac = %u",
			  clib_net_to_host_u32
			  (state_cookie_param->creation_time),
			  clib_net_to_host_u32
			  (state_cookie_param->cookie_lifespan),
			  clib_net_to_host_u64 (state_cookie_param->mac));
			 ...
			 vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* If we are in SCTP_CLOSED_STATE then the connection
	   * will come from the open connection pool.
	   */
	  if (sctp_conn == NULL)
	    {
	      sctp_conn =
	  ...
	  switch (chunk_type)
	      ...
	      if (error0 == SCTP_ERROR_NONE)
		{
		  ...
		  sctp_timer_reset (sctp_conn,
				    sctp_pick_conn_idx_on_chunk (INIT_ACK),
				    SCTP_TIMER_T1_INIT);
		  ...
		  sctp_half_open_connection_cleanup (sctp_conn);
		}
			      ...
			      vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  sctp_conn =
	    sctp_lookup_connection (sctp_listener->sub_conn[idx].
				    connection.fib_index, b0, my_thread_index,
				    is_ip4);
			      ...
			      vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  sctp_connection_t *sctp_listener;
	  u16 next0 = SCTP_LISTEN_PHASE_NEXT_DROP, error0 =
	    SCTP_ERROR_ENQUEUED;
	  ...
	  if (PREDICT_FALSE (child_conn->state != SCTP_STATE_CLOSED))
	    {
	      SCTP_DBG ("child_conn->state != SCTP_STATE_CLOSED.... STATE=%s",
	  ...
	  if (chunk_type != INIT)
	    {
	      SCTP_DBG ("chunk_type != INIT... chunk_type=%s",
	  ...
	  if (stream_session_accept
	      (&child_conn->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection,
	       sctp_listener->sub_conn[MAIN_SCTP_SUB_CONN_IDX].c_s_index, 0))
	    {
  ...
  sctp_node_inc_counter (vm, is_ip4, sctp4_listen_phase_node.index,
			 sctp6_listen_phase_node.index,
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  switch (chunk_type)
		...
		sctp_handle_cookie_ack (sctp_hdr, sctp_chunk_hdr, sctp_conn,
					b0);
	      // next0 = sctp_next_output (is_ip4);
	      sctp_timer_set (sctp_conn, idx, SCTP_TIMER_T3_RXTX,
			      SCTP_RTO_INIT);
	      break;
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_TRUE (0 != sctp_conn))
		...
		(u8 *) sctp_hdr - (u8 *) vlib_buffer_get_current (b0);
	      ...
	      SCTP_DBG ("NEXT_PHASE = %s, ERROR = %u",
			phase_to_string (next0), error0);
...
do {                                                       	\
   ...
   */
  _(CLOSED, DATA, SCTP_INPUT_NEXT_RCV_PHASE, SCTP_ERROR_NONE);	/* UNEXPECTED DATA chunk which requires special handling */
  ...
  _(CLOSED, COOKIE_ECHO, SCTP_INPUT_NEXT_RCV_PHASE, SCTP_ERROR_NONE);
  ...
  _(ESTABLISHED, DATA, SCTP_INPUT_NEXT_LISTEN_PHASE, SCTP_ERROR_NONE);
  ...
  _(ESTABLISHED, SACK, SCTP_INPUT_NEXT_LISTEN_PHASE, SCTP_ERROR_NONE);
  _(ESTABLISHED, HEARTBEAT, SCTP_INPUT_NEXT_LISTEN_PHASE, SCTP_ERROR_NONE);
  _(ESTABLISHED, HEARTBEAT_ACK, SCTP_INPUT_NEXT_LISTEN_PHASE,
    SCTP_ERROR_NONE);
