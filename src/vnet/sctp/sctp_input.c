  ...
  if (clib_net_to_host_u16 (chunk_hdr->length) > sizeof (sctp_init_chunk_t))
      ...
      while (pointer_offset < clib_net_to_host_u16 (chunk_hdr->length))
	{
	  sctp_optional_parameters_hdr_t *opt_params_hdr =
	    (sctp_optional_parameters_hdr_t *) init_chunk + pointer_offset;
		      ...
		      sctp_connection_t * child0, vlib_buffer_t * b0)
{
  ...
  ip4_address_t *ip4_addr = 0;
  ip6_address_t *ip6_addr = 0;
  sctp_state_cookie_t *state_cookie = 0;
  ...
  if (clib_net_to_host_u16 (chunk_hdr->length) >
	...
	{
	  sctp_optional_parameters_hdr_t *opt_params_hdr =
	    (sctp_optional_parameters_hdr_t *) init_ack_chunk +
	    pointer_offset;
	  ...
	  switch (clib_net_to_host_u16 (opt_params_hdr->type))
	    ...
	    case SCTP_STATE_COOKIE_TYPE:
	      {
		sctp_state_cookie_t *state_cookie_param =
		  (sctp_state_cookie_t *) opt_params_hdr;
		clib_memcpy (state_cookie, state_cookie_param,
			     sizeof (sctp_state_cookie_t));
		break;
	      }
	    case SCTP_HOSTNAME_ADDRESS_TYPE:
	      {
