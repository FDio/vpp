...
phase_to_string (u8 phase)
{
  switch (phase)
    ...
    case SCTP_INPUT_NEXT_INIT_PHASE:
      return "SCTP_INPUT_NEXT_INIT_PHASE";
		      ...
		      u16 sctp_implied_length)
{
  ...
  sctp_timer_reset (sctp_conn, MAIN_SCTP_SUB_CONN_IDX, SCTP_TIMER_T1_INIT);
  ...
  sctp_timer_set (sctp_conn, idx, SCTP_TIMER_T1_COOKIE, SCTP_RTO_INIT);
		  ...
		  u16 * next0)
{
  ...
  *next0 = sctp_next_output (sctp_conn->sub_conn[idx].c_is_ip4);
  sctp_prepare_cookie_ack_chunk (sctp_conn, b);
			...
			sctp_connection_t * sctp_conn, vlib_buffer_t * b0)
{
  ...
  sctp_timer_reset (sctp_conn, MAIN_SCTP_SUB_CONN_IDX, SCTP_TIMER_T1_COOKIE);
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (sctp_conn == NULL)
	    {
	      SCTP_DBG
		("sctp_conn == NULL; getting connection via 'sctp_connection_get()'");
	      sctp_conn =
		sctp_connection_get (vnet_buffer (b0)->sctp.connection_index,
				     my_thread_index);
	    }

	  SCTP_DBG ("src = %d:%d, dst = %d:%d",
		    ip4_hdr->src_address.as_u32,
		    clib_net_to_host_u16 (sctp_hdr->src_port),
		    ip4_hdr->dst_address.as_u32,
		    clib_net_to_host_u16 (sctp_hdr->dst_port));
	  ...
	  switch (chunk_type)
	    ...
	    case INIT:
	      SCTP_DBG ("INIT: my_thread_index = %u", my_thread_index);

	      /* Move connection from half-open pool to current thread pool. */
		...
		new_sctp_conn - tm->connections[my_thread_index];
	      ...
	      error0 =
		sctp_handle_init (sctp_hdr, sctp_chunk_hdr, new_sctp_conn, b0,
				  sctp_implied_length);
	    ...
	    case INIT_ACK:
	      SCTP_DBG ("INIT_ACK: my_thread_index = %u", my_thread_index);
	  ...
	  /*
	     ...
	     (&sctp_conn->sub_conn[idx].connection,
	     sctp_listener->sub_conn[idx].connection.s_index, 0 ))
	     {
			      ...
			      vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (stream_session_accept
	      (&sctp_conn->sub_conn[idx].connection,
	       sctp_listener->sub_conn[idx].connection.s_index,
	       0 /* notify */ ))
	    {
	      clib_warning ("session accept fail");
	      sctp_connection_cleanup (sctp_conn);
				 ...
				 vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  ip6_header_t *ip6_hdr;
	  sctp_connection_t *sctp_connection;
	  u16 next0 = SCTP_ESTABLISHED_PHASE_NEXT_DROP, error0 =
	    SCTP_ERROR_ENQUEUED;
	  ...
	  switch (vnet_sctp_get_chunk_type (chunk_hdr))
	    {
	    case DATA:
	      error0 =
		sctp_handle_data ((sctp_payload_data_chunk_t *) sctp_hdr,
				  sctp_connection, b0, &next0);
	      next0 = sctp_next_output (is_ip4);
	      break;
