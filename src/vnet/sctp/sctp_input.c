			       ...
			       u16 * next0)
{
  ...
  sctp_timer_reset (sctp_conn, idx,
		    SCTP_TIMER_T2_SHUTDOWN);
	  ...
	  if (PREDICT_TRUE (0 != sctp_conn))
	    {
	      ...
	      SCTP_DBG_STATE_MACHINE ("SESSION_INDEX = %u, CURRENT_CONNECTION_STATE = %s,"
				      "CHUNK_TYPE_RECEIVED = %s "
				      "NEXT_PHASE = %s",
					  sctp_conn->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.s_index,
