...
always_inline u16
sctp_handle_data (sctp_payload_data_chunk_t * sctp_data_chunk, sctp_connection_t * sctp_conn,
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  switch (clib_net_to_host_u16 (sctp_chunk_hdr->type))
	    ...
	    case DATA:
		      error0 =
			sctp_handle_data ((sctp_payload_data_chunk_t*)sctp_hdr, sctp_conn, b0, &next0);
		      next0 = sctp_next_output (is_ip4);
		      break;
	      break;
	      ...
	      while (n_left_from > 0 && n_left_to_next > 0)
		{
		  ...
		  switch (clib_net_to_host_u16 (sctp_chunk_hdr->type))
		    ...
		    case SHUTDOWN_COMPLETE:
		      error0 =
			sctp_handle_shutdown_complete (sctp_hdr, sctp_chunk_hdr, sctp_conn,
						 b0, sctp_implied_length);
...
sctp_handle_sack (sctp_selective_ack_chunk_t * sack_chunk,
		  sctp_connection_t * sctp_conn, vlib_buffer_t * b0, u16 * next0)
{
  return SCTP_ERROR_NONE;
}
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  switch (clib_net_to_host_u16 (chunk_hdr->type))
	    ...
	    case DATA:
	      error0 =
		sctp_handle_data ((sctp_payload_data_chunk_t*)sctp_hdr, sctp_connection, b0, &next0);
	      next0 = sctp_next_output (is_ip4);
	      break;

	    case SACK:
	      error0 =
		sctp_handle_sack ((sctp_selective_ack_chunk_t*)sctp_hdr, sctp_connection, b0, &next0);
	      next0 = sctp_next_output (is_ip4);
	      break;

	    case HEARTBEAT:
	      error0 =
		sctp_handle_heartbeat ((sctp_hb_req_chunk_t*)sctp_hdr, sctp_connection,
				       b0, &next0);
	      next0 = sctp_next_output (is_ip4);
	      break;
	    ...
	    case HEARTBEAT_ACK:
	      error0 =
		sctp_handle_heartbeat_ack ((sctp_hb_ack_chunk_t*)sctp_hdr, sctp_connection, b0, &next0);
	      next0 = sctp_next_output (is_ip4);
	      break;
