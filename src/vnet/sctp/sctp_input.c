			   ...
			   u16 data_len, u8 conn_idx)
{
  ...
  SCTP_ADV_DBG ("CONN = %u, DATA_LEN = %d, WRITTEN = %u",
...
sctp_handle_sack (sctp_selective_ack_chunk_t * sack_chunk,
		  ...
		  u16 * next0)
{
    *next0 = sctp_next_output (sctp_conn->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.is_ip4);
	  ...
	  if (is_ip4)
	    {
	      ...
	      n_advance_bytes0 =
		(ip4_header_bytes (ip4_hdr) + sizeof (sctp_payload_data_chunk_t));
	      n_data_bytes0 =
	  ...
	  if (PREDICT_TRUE (0 != sctp_conn))
	    {
	      ...
	      SCTP_ADV_DBG ("CONNECTION_INDEX = %u: "
