		  ...
		  sctp_connection_t * child0, vlib_buffer_t * b0)
{
   ...
   *
   * IMPLEMENT logic as per RFC4960 Section 5.2.2
   */
  if (child0->state != SCTP_STATE_CLOSED)
    {				/* UNEXPECTED scenario */
      switch (child0->state)
	{
  ...
  child0->remote_tag = init_chunk->initiate_tag;
  /*
  ...
  if (clib_net_to_host_u16 (chunk_hdr->length) > sizeof (sctp_init_chunk_t))
    {
      ...
      while (pointer_offset < clib_net_to_host_u16 (chunk_hdr->length))
	{
  ...
  sctp_prepare_initack_chunk (child0, b0, ip4_addr, ip6_addr);

  return SCTP_ERROR_NONE;
}

always_inline u16
sctp_handle_init_ack (sctp_header_t * th0,
		      sctp_chunks_common_hdr_t * chunk_hdr,
		      sctp_connection_t * child0, vlib_buffer_t * b0)
{
  sctp_init_ack_chunk_t *init_ack_chunk = (sctp_init_ack_chunk_t *) (th0);
  ip4_address_t *ip4_addr = 0;
  ip6_address_t *ip6_addr = 0;
  ...
  sctp_prepare_cookie_echo (child0, b0, state_cookie);

  /* Start the T1_COOKIE timer */
  sctp_timer_set (child0, SCTP_TIMER_T1_COOKIE, SCTP_RTO_INIT);

  /* Change state */
  child0->state = SCTP_STATE_COOKIE_ECHOED;

  return SCTP_ERROR_NONE;
}

always_inline u16
sctp_handle_cookie_echo (sctp_header_t * th0,
			 sctp_chunks_common_hdr_t * chunk_hdr,
			 sctp_connection_t * child0, vlib_buffer_t * b0)
{
  ...
  sctp_prepare_cookie_ack (child0, b0);

  /* Change state */
  child0->state = SCTP_STATE_ESTABLISHED;
...
always_inline u16
sctp_handle_cookie_ack (sctp_header_t * th0,
			sctp_chunks_common_hdr_t * chunk_hdr,
			sctp_connection_t * child0, vlib_buffer_t * b0)
{

  /* Stop T1_COOKIE timer */
  sctp_timer_reset (child0, SCTP_TIMER_T1_COOKIE);
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  switch (clib_net_to_host_u16 (chunk_hdr->type))
	    ...
	    case INIT:
	      error0 = sctp_handle_init (th0, chunk_hdr, child0, b0);
	      next0 = sctp_next_output (is_ip4);
	      break;

	    case INIT_ACK:
	      error0 = sctp_handle_init_ack (th0, chunk_hdr, child0, b0);
	      next0 = sctp_next_output (is_ip4);
	      break;

	    case COOKIE_ECHO:
	      error0 = sctp_handle_cookie_echo (th0, chunk_hdr, child0, b0);
	      next0 = sctp_next_output (is_ip4);
	      break;

	    case COOKIE_ACK:
	      error0 = sctp_handle_cookie_ack (th0, chunk_hdr, child0, b0);
	      next0 = sctp_next_output (is_ip4);
	      break;
	  ...
	  if (stream_session_accept
	      (&child0->connection, lc0->c_s_index, 0 /* notify */ ))
	    {
...
do {                                                       	\
  ...
  /* UNEXPECTED INIT chunk */
  _(COOKIE_WAIT, INIT, SCTP_INPUT_NEXT_INIT_PHASE, SCTP_ERROR_NONE);
  _(COOKIE_ECHOED, INIT, SCTP_INPUT_NEXT_INIT_PHASE, SCTP_ERROR_NONE);

  /* UNEXPECTED INIT_ACK chunk */
  _(CLOSED, INIT_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(COOKIE_ECHOED, INIT_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(ESTABLISHED, INIT_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(SHUTDOWN_PENDING, INIT_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(SHUTDOWN_SENT, INIT_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(SHUTDOWN_RECEIVED, INIT_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(SHUTDOWN_ACK_SENT, INIT_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);

  /* UNEXPECTED COOKIE_ACK chunk */
  _(CLOSED, COOKIE_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(COOKIE_WAIT, COOKIE_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(ESTABLISHED, COOKIE_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(SHUTDOWN_PENDING, COOKIE_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(SHUTDOWN_SENT, COOKIE_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(SHUTDOWN_RECEIVED, COOKIE_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
  _(SHUTDOWN_ACK_SENT, COOKIE_ACK, SCTP_INPUT_NEXT_DROP, SCTP_ERROR_ACK_DUP);
