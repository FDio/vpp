...
#define foreach_sctp4_input_next                 \
  _ (DROP, "error-drop")                        \
  _ (INIT_PHASE, "sctp4-closed")                     \
  _ (ESTABLISHED_PHASE, "sctp4-c-process")           \
  _ (SHUTDOWN_PHASE, "sctp4-established")
		      ...
		      vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (child0->verification_tag == 0)
	    {
	      sctp_init_chunk_t *init_chunk =
		(sctp_init_chunk_t *) (th0 + sctp_header_bytes ());
	      child0->verification_tag = init_chunk->initiate_tag;
	    }
...
always_inline uword
sctp46_established_phase_process_inline (vlib_main_t * vm,
...
sctp4_rcv_process (vlib_main_t * vm, vlib_node_runtime_t * node,
		   vlib_frame_t * from_frame)
{
  return sctp46_established_phase_process_inline (vm, node, from_frame,
						  1 /* is_ip4 */ );
}
