...
always_inline uword
sctp46_input_inline (vlib_main_t * vm, vlib_node_runtime_t * node,
		     vlib_frame_t * from_frame, int is_ip4)
{
  u32 n_left_from, next_index, *from, *to_next;
  u32 my_thread_index = vm->thread_index;
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_TRUE (0 != tc0))
		...
		(u8 *) sctp0 - (u8 *) vlib_buffer_get_current (b0);
	      ...
	      /*
	       * TODO: create lookup table for NEXT STATE DISPATCHING
...
sctp4_input (vlib_main_t * vm, vlib_node_runtime_t * node,
	     vlib_frame_t * from_frame)
{
  return sctp46_input_inline (vm, node, from_frame, 1 /* is_ip4 */ );
}
...
sctp6_input (vlib_main_t * vm, vlib_node_runtime_t * node,
	     vlib_frame_t * from_frame)
{
  return sctp46_input_inline (vm, node, from_frame, 0 /* is_ip4 */ );
}
