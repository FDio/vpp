...
sctp_update_rcv_mss (sctp_connection_t * tc)
{
  ...
  tc->a_rwnd = DEFAULT_A_RWND - sizeof(sctp_payload_data_chunk_t);
}
...
sctp_init_mss (sctp_connection_t * tc)
{
	  ...
	  SCTP_ADV_DBG("tc->snd_opts.a_rwnd = %u, tc->a_rwnd = %u",
			  tc->snd_opts.a_rwnd, tc->a_rwnd);

	  if (tc->snd_a_rwnd < sizeof(sctp_full_hdr_t))
	    {
	      clib_warning ("tc->snd_a_rwnd < sizeof(sctp_full_hdr_t)");
...
{
  SCTP_DBG ("Closing connection...");
  if (tc->state != SCTP_STATE_ESTABLISHED)
    {
      SCTP_ADV_DBG ("Invalid close: state is %s",
		    sctp_state_to_string (tc->state));
      return;
    }
   ...
   */
  sctp_check_outstanding_data_chunks (tc);
...
{
  tc->snd_a_rwnd = clib_min (tc->a_rwnd, tc->rcv_opts.a_rwnd);
...
{
	SCTP_DBG("");
  sctp_connection_t *tc = (sctp_connection_t *) trans_conn;
