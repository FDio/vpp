...
static inline sctp_connection_t *
sctp_half_open_connection_new (void)
{
...
sctp_connection_open (transport_endpoint_t * rmt)
{
  ...
  tc = sctp_half_open_connection_new ();
...
sctp_half_open_connection_cleanup (sctp_connection_t * tc)
{
  ...
  if (tc->c_thread_index != vlib_get_thread_index ())
    return 1;
  sctp_timer_reset (tc, SCTP_TIMER_T1_INIT);
  ...
  if (tc->state == SCTP_STATE_COOKIE_WAIT)
    {
      ...
      sctp_half_open_connection_cleanup (tc);
    }
  else
    {
...
sctp_timer_init_handler (u32 conn_index)
{
  ...
  tc = sctp_half_open_connection_get (conn_index);
  if (tc)
    {
      ASSERT (tc->state == SCTP_STATE_COOKIE_WAIT);
