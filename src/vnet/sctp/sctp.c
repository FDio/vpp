...
sctp_connection_bind (u32 session_index, transport_endpoint_t * tep)
{
  ...
  listener->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.c_index =
  ...
  listener->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.s_index =
    session_index;
  ...
  return listener->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.c_index;
}
...
sctp_connection_new (u8 thread_index)
{
  ...
  tc->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.thread_index = thread_index;
  tc->local_tag = 0;
  tc->next_avail_sub_conn = 1;
