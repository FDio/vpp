...
sctp_connection_bind (u32 session_index, transport_endpoint_t * tep)
{
  ...
  ip_copy (&listener->c_lcl_ip, &tep->ip, tep->is_ip4);
  listener->c_is_ip4 = tep->is_ip4;
  listener->c_proto = TRANSPORT_PROTO_SCTP;
  listener->c_s_index = session_index;
  listener->c_fib_index = tep->fib_index;
...
sctp_connection_timers_init (sctp_connection_t * tc)
{
  ...
  /* Set all to invalid */
  for (i = 0; i < SCTP_N_TIMERS; i++)
    {
      tc->timers[i] = SCTP_TIMER_HANDLE_INVALID;
    }
...
sctp_connection_open (transport_endpoint_t * rmt)
{
  ...
  tc = sctp_connection_new (thread_id);
  ...
  tc->c_proto = TRANSPORT_PROTO_SCTP;
...
sctp_connection_cleanup (sctp_connection_t * tc)
{
  ...
  transport_endpoint_cleanup (TRANSPORT_PROTO_SCTP, &tc->c_lcl_ip,
			      tc->c_lcl_port);
...
void
sctp_verify_outstanding_data_tobe_sent (sctp_connection_t * tc)
{
  /* TODO */
}
...
{
  if (tc->state != SCTP_STATE_ESTABLISHED)
    {
      SCTP_DBG ("Invalid close: not in SCTP_STATE_ESTABLISHED state");
      return;
    }
  ...
  /*
   * Check for remaining DATA to be sent by the upper-layer
   * If anything left to be sent then send it now.
   */
  sctp_verify_outstanding_data_tobe_sent (tc);
...
sctp_timer_init_handler (u32 conn_index)
{
  ...
  tc->timers[SCTP_TIMER_T1_INIT] = SCTP_TIMER_HANDLE_INVALID;
...
sctp_main_enable (vlib_main_t * vm)
{
  ...
  transport_register_protocol (TRANSPORT_PROTO_SCTP, 1, &sctp_proto);
  transport_register_protocol (TRANSPORT_PROTO_SCTP, 0, &sctp_proto);
...
clib_error_t *
vnet_sctp_enable_disable (vlib_main_t * vm, u8 is_en)
{
  if (is_en)
    {
