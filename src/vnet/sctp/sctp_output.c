...
sctp_prepare_shutdown_chunk (sctp_connection_t * tc, vlib_buffer_t * b)
{
  ...
  shutdown_chunk->chunk_hdr.type = clib_host_to_net_u32 (SHUTDOWN);
...
sctp_send_shutdown (sctp_connection_t * tc)
{
  ...
  if (sctp_verify_outstanding_data (tc) > 0)
    return;
...
sctp_prepare_shutdown_ack_chunk (sctp_connection_t * tc, vlib_buffer_t * b)
{
  ...
  shutdown_ack_chunk->sctp_hdr.dst_port =
    tc->sub_conn[idx].connection.rmt_port;
  /* The sender of an INIT must set the VERIFICATION_TAG to 0 as per RFC 4960 Section 8.5.1 */
  shutdown_ack_chunk->sctp_hdr.verification_tag = 0x0;
  shutdown_ack_chunk->chunk_hdr.type = clib_host_to_net_u32 (SHUTDOWN_ACK);
				      ...
				      vlib_buffer_t * b)
{
  ...
  shutdown_complete->sctp_hdr.dst_port =
    tc->sub_conn[idx].connection.rmt_port;
  /* The sender of an INIT must set the VERIFICATION_TAG to 0 as per RFC 4960 Section 8.5.1 */
  shutdown_complete->sctp_hdr.verification_tag = 0x0;
  shutdown_complete->chunk_hdr.type =
    clib_host_to_net_u32 (SHUTDOWN_COMPLETE);
...
sctp_send_init (sctp_connection_t * tc)
{
  ...
  sctp_push_ip_hdr (tm, &tc->sub_conn[idx], b);
  sctp_enqueue_to_ip_lookup (vm, b, bi, tc->sub_conn[idx].c_is_ip4);
		      ...
		      vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_ip4)
	    {
	      vlib_buffer_push_ip4 (vm,
				    b0,
				    &tc0->sub_conn[idx].connection.lcl_ip.ip4,
				    &tc0->sub_conn[idx].connection.rmt_ip.ip4,
				    IP_PROTOCOL_SCTP, 1);
