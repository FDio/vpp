			    ...
			    ip6_address_t * ip6_addr)
{
  ...
  if (PREDICT_TRUE (ip4_addr != NULL))
    {
      SCTP_DBG ("alloc_bytes += SCTP_IPV4_ADDRESS_TYPE_LENGTH");
  ...
  if (PREDICT_TRUE (ip6_addr != NULL))
    {
      SCTP_DBG ("alloc_bytes += SCTP_IPV6_ADDRESS_TYPE_LENGTH");
		 ...
		 sctp_state_t next_state)
{
  ...
  u16 alloc_bytes = sizeof (sctp_payload_data_chunk_t);
		      ...
		      vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_ip4)
	    {
	      sctp_hdr = ip4_next_header ((ip4_header_t *) th0);
	    ...
	    {
	      sctp_hdr = ip6_next_header ((ip6_header_t *) th0);
