		   ...
		   ip4_address_t * ip4_addr, ip6_address_t * ip6_addr)
{
  ...
  if (PREDICT_TRUE (ip4_addr != NULL))
    {
      sctp_ipv4_address_t *ipv4_addr =
	(sctp_ipv4_address_t *) init_chunk + sizeof (sctp_init_chunk_t);

      ipv4_addr->param_hdr.type = clib_host_to_net_u16 (IPV4_ADDRESS_TYPE);
      ipv4_addr->param_hdr.length =
	clib_host_to_net_u16 (IPV4_ADDRESS_TYPE_LENGTH);
      ipv4_addr->address.as_u32 = ip4_addr->as_u32;

      pointer_move = IPV4_ADDRESS_TYPE_LENGTH;
    }
  if (PREDICT_TRUE (ip6_addr != NULL))
    {
      sctp_ipv6_address_t *ipv6_addr =
	(sctp_ipv6_address_t *) init_chunk + sizeof (sctp_init_chunk_t) +
	pointer_move;

      ipv6_addr->param_hdr.type = clib_host_to_net_u16 (IPV6_ADDRESS_TYPE);
      ipv6_addr->param_hdr.length =
	clib_host_to_net_u16 (IPV6_ADDRESS_TYPE_LENGTH);
  ...
  init_chunk->sctp_hdr.verification_tag = tc->initiation_tag;
...
sctp_send_init (sctp_connection_t * tc)
{
  ...
  /* Set the connection T1_INIT timer */
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  tc0 = sctp_connection_get (vnet_buffer (b0)->sctp.connection_index,
				     my_thread_index);
	  if (PREDICT_FALSE (tc0 == 0 || tc0->state == SCTP_STATE_CLOSED))
	    {
