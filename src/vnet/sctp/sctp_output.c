  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  u8 is_valid =
		...
		sctp_hdr->dst_port
		|| tc0->sub_conn[MAIN_SCTP_SUB_CONN_IDX].
		connection.rmt_port == sctp_hdr->src_port);
	  ...
	  if (!is_valid)
	    {
	      SCTP_DBG_STATE_MACHINE ("BUFFER IS INCORRECT: conn_index = %u, "
				      ...
				      c_index, packet_length, chunk_type,
				      sctp_chunk_to_string (chunk_type),
				      tc0->
				      sub_conn
				      [MAIN_SCTP_SUB_CONN_IDX].connection.
				      lcl_port, sctp_hdr->src_port,
				      tc0->
				      sub_conn
				      [MAIN_SCTP_SUB_CONN_IDX].connection.
				      rmt_port, sctp_hdr->dst_port);
	  ...
	  SCTP_DBG_STATE_MACHINE
	    ("CONN_INDEX = %u, CURR_CONN_STATE = %u (%s), "
	     ...
	     tc0->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.c_index,
