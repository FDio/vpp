...
sctp_make_init (sctp_connection_t * tc, vlib_buffer_t * b)
{
  ...
  th->sctp_hdr.src_port = tc->c_lcl_port;
  th->sctp_hdr.dst_port = tc->c_rmt_port;
  th->sctp_hdr.checksum = 0;
  th->sctp_hdr.verification_tag = 0;
  th->chunk_hdr.type = clib_host_to_net_u32 (INIT);
  th->initiate_tag = random_u32 (&random_seed);
 ...
 */
void
sctp_make_initack (sctp_connection_t * tc, vlib_buffer_t * b)
{
  vlib_main_t *vm = vlib_get_main ();
  sctp_init_ack_chunk_t *th =
    vlib_buffer_push_uninit (b, sizeof (sctp_init_ack_chunk_t));
  ...
  th->sctp_hdr.dst_port = tc->c_rmt_port;
  th->sctp_hdr.checksum = 0;
  th->sctp_hdr.verification_tag = tc->verification_tag;
  th->chunk_hdr.type = clib_host_to_net_u32 (INIT_ACK);
