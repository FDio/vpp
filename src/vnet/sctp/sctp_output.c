...
sctp_make_initack (sctp_connection_t * tc, vlib_buffer_t * b)
{
  ...
  th = vlib_buffer_push_sctp (b, tc->c_lcl_port, tc->c_rmt_port, tc->iss,
			      tc->rcv_nxt, sizeof (*th));
 ...
 *  The packet is not forwarded through sctpx_output to avoid doing lookups
