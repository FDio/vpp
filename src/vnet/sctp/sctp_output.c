 ...
 */
void
sctp_prepare_init_chunk (sctp_connection_t * tc, vlib_buffer_t * b)
{
  ...
  if (tc->c_is_ip4)
    {
      ...
      ip4_param->address.as_u32 = tc->c_lcl_ip.ip4.as_u32;
  ...
  else
    {
      ...
      ip6_param->address.as_u64[0] = tc->c_lcl_ip.ip6.as_u64[0];
      ip6_param->address.as_u64[1] = tc->c_lcl_ip.ip6.as_u64[1];

      pointer_offset += sizeof (sctp_ipv6_addr_param_t);
    }

  init_chunk->sctp_hdr.src_port = tc->c_lcl_port;	/* No need of host_to_net conversion, already in net-byte order */
  init_chunk->sctp_hdr.dst_port = tc->c_rmt_port;	/* No need of host_to_net conversion, already in net-byte order */
  ...
  init_chunk->a_rwnd = clib_host_to_net_u32 (MIN_A_RWND);
  init_chunk->initiate_tag = random_u32 (&random_seed);
  init_chunk->inboud_streams_count =
    clib_host_to_net_u32 (INBOUND_STREAMS_COUNT);
  init_chunk->outbound_streams_count =
    clib_host_to_net_u32 (OUTBOUND_STREAMS_COUNT);
			    ...
			    ip6_address_t * ip6_addr)
{
  ...
  u16 pointer_offset = sizeof (init_ack_chunk);

  /* Create State Cookie parameter */
  sctp_state_cookie_param_t *state_cookie_param =
    (sctp_state_cookie_param_t *) init_ack_chunk + pointer_offset;
  ...
  init_ack_chunk->sctp_hdr.src_port = tc->c_lcl_port;
  init_ack_chunk->sctp_hdr.dst_port = tc->c_rmt_port;
  ...
  init_ack_chunk->chunk_hdr.type = clib_host_to_net_u32 (INIT_ACK);
  init_ack_chunk->chunk_hdr.length = clib_host_to_net_u32 (alloc_bytes);

  init_ack_chunk->initiate_tag = random_u32 (&random_seed);
  ...
  init_ack_chunk->a_rwnd = clib_host_to_net_u32 (MIN_A_RWND);
...
sctp_prepare_shutdown_chunk (sctp_connection_t * tc, vlib_buffer_t * b)
{
  ...
  shutdown_chunk->sctp_hdr.src_port = tc->c_lcl_port;	/* No need of host_to_net conversion, already in net-byte order */
  shutdown_chunk->sctp_hdr.dst_port = tc->c_rmt_port;	/* No need of host_to_net conversion, already in net-byte order */
  shutdown_chunk->sctp_hdr.checksum = 0;
  /* The sender of an INIT must set the VERIFICATION_TAG to 0 as per RFC 4960 Section 8.5.1 */
  shutdown_chunk->sctp_hdr.verification_tag = 0x0;
  shutdown_chunk->chunk_hdr.type = clib_host_to_net_u32 (INIT);
...
sctp_send_shutdown (sctp_connection_t * tc)
{
  ...
  sctp_timer_set (tc, SCTP_TIMER_T2_SHUTDOWN, SCTP_RTO_INIT);
  tc->state = SCTP_STATE_SHUTDOWN_SENT;
}
...
{
  /* TODO: Not sure what the calling function here would be */
