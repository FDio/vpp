			    ...
			    ip6_address_t * ip6_addr)
{
  ...
  if (PREDICT_TRUE (ip4_addr != NULL))
    {
      SCTP_DBG_OUTPUT ("alloc_bytes += SCTP_IPV4_ADDRESS_TYPE_LENGTH");
  ...
  if (PREDICT_TRUE (ip6_addr != NULL))
    {
      SCTP_DBG_OUTPUT ("alloc_bytes += SCTP_IPV6_ADDRESS_TYPE_LENGTH");
...
sctp_send_shutdown (sctp_connection_t * tc)
{
  ...
  sctp_enqueue_to_output (vm, b, bi, tc->sub_conn[idx].connection.is_ip4);
...
sctp_prepare_sack_chunk (sctp_connection_t * tc, vlib_buffer_t * b)
{
	  ...
	  sctp_selective_ack_chunk_t *sack =
	    vlib_buffer_push_uninit (b, alloc_bytes);
		 ...
		 sctp_state_t next_state)
{
  ...
  u16 data_len = b->current_length + b->total_length_not_including_first_buffer;
   ...
   SCTP_ADV_DBG_OUTPUT("CONN = %u, A_RWND = %u, SND_A_RWND = %u",
		   tc->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.c_index,
		   tc->a_rwnd,
		   tc->snd_a_rwnd);

  u16 alloc_bytes = sizeof (sctp_payload_data_chunk_t) + data_len;
  ...
  vnet_sctp_set_chunk_length (&data_chunk->chunk_hdr, chunk_len);

  i16 offset = b->current_data;
  void *p;
  if (offset < 0)
  {
	  p = ((void*)b->pre_data)+(offset * (-1));
  }
  else
  {
	  p = ((void*)b->data)+(offset);
  }

  clib_memcpy(data_chunk->data, p, data_len);
		      ...
		      vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  SCTP_ADV_DBG_OUTPUT("vnet_buffer (b0)->sctp.connection_index = %u",
			  vnet_buffer (b0)->sctp.connection_index);
	  ...
	  u8 is_valid = (tc0->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.lcl_port == sctp_hdr->src_port ||
			  tc0->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.lcl_port == sctp_hdr->dst_port)
	  		 && (tc0->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.rmt_port == sctp_hdr->dst_port ||
	  				tc0->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.rmt_port == sctp_hdr->src_port);
	  ...
	  if (!is_valid)
	  {
		  SCTP_ADV_DBG_OUTPUT("Buffer is dirty: packet_length = %u, "
				  "chunk_type = %u [%s], "
	    			"connection.lcl_port = %u, sctp_hdr->src_port = %u, "
	    			"connection.rmt_port = %u, sctp_hdr->dst_port = %u",
					packet_length,
					chunk_type, sctp_chunk_to_string(chunk_type),
	    			tc0->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.lcl_port,
					sctp_hdr->src_port,
					tc0->sub_conn[MAIN_SCTP_SUB_CONN_IDX].connection.rmt_port,
					sctp_hdr->dst_port);
	  ...
	  SCTP_ADV_DBG_OUTPUT("PACKET_LENGTH = %u, CURR_CONN_STATE = %u (%s), "
			  "CHUNK_TYPE = %s, "
	  ...
	  switch (tc0->state)
		...
		if (chunk_type != INIT && chunk_type != INIT_ACK)
		  {
	    	SCTP_DBG_OUTPUT("Sending the wrong chunk (%s) based on state-machine status (%s)",
	    			sctp_chunk_to_string(chunk_type),
					sctp_state_to_string(tc0->state));
	  ...
	  SCTP_DBG_OUTPUT ("CONNECTION_INDEX = %u, "
		    "NEW_STATE = %s, "
		    "CHUNK_SENT = %s",
