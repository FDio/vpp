			  ...
			  sctp_state_cookie_t * sc)
{
  ...
  u16 alloc_bytes = sizeof (sctp_cookie_echo_t);

  sctp_cookie_echo_t *cookie_echo_chunk =
			    ...
			    ip6_address_t * ip6_addr)
{
  ...
  /* The minimum size of the message is given by the sctp_init_ack_chunk_t */
  u16 alloc_bytes =
    sizeof (sctp_init_ack_chunk_t) + sizeof (sctp_state_cookie_t);
  ...
  pointer_offset += sizeof (sctp_state_cookie_t);

  if (PREDICT_TRUE (ip4_addr != NULL))
    {
      sctp_ipv4_address_t *ipv4_addr =
	(sctp_ipv4_address_t *) init_chunk + pointer_offset;
