		 ...
		 sctp_state_t next_state)
{
  ...
  u16 data_len = b->current_length + b->total_length_not_including_first_buffer;
   ...
   sctp_payload_data_chunk_t *data_chunk =
     vlib_buffer_push_uninit (b, sizeof (sctp_payload_data_chunk_t));
   ...
   vnet_sctp_set_chunk_length (&data_chunk->chunk_hdr, (b->current_length - sizeof (sctp_header_t)));

   SCTP_ADV_DBG_OUTPUT("b->current_length = %u, "
 		   "b->total_length_not_including_first_buffer = %u, "
 		   "data_len = %u, "
 		   "sizeof(sctp_payload_data_chunk_t) = %u, ",
 		   b->current_length,
 		   b->total_length_not_including_first_buffer,
 		   data_len,
 		   sizeof (sctp_payload_data_chunk_t));

   SCTP_ADV_DBG_OUTPUT("POINTER_WITH_DATA = %p", b->data);
		      ...
		      vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (!is_valid)
	  {
		  SCTP_DBG_STATE_MACHINE("Buffer is dirty: packet_length = %u, "
				  "chunk_type = %u [%s], "
	  ...
	  SCTP_DBG_STATE_MACHINE("CURR_CONN_STATE = %u (%s), "
		  "CHUNK_TYPE = %s, "
	  ...
	  if (chunk_type == DATA)
		  SCTP_ADV_DBG_OUTPUT("PACKET_LENGTH = %u",
			  packet_length);
