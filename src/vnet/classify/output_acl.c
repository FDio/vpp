...
output_acl_main_t output_acl_main;

static int
vnet_outacl_ip_feature_enable (vlib_main_t * vnm,
			      output_acl_main_t * am,
			      u32 sw_if_index,
			      output_acl_table_id_t tid, int feature_enable)
{

  if (tid == OUTPUT_ACL_TABLE_L2)
    {
    ...
    {				/* IP[46] */
      ...
      if (tid == OUTPUT_ACL_TABLE_IP4)
	{
	  vnet_feature_enable_disable ("ip4-unicast", "ip4-outacl",
				       sw_if_index, feature_enable, 0, 0);
	  arc = vnet_get_feature_arc_index ("ip4-unicast");
	}
      else
	{
	  vnet_feature_enable_disable ("ip6-unicast", "ip6-outacl",
				       sw_if_index, feature_enable, 0, 0);
			  ...
			  u32 ip6_table_index, u32 l2_table_index, u32 is_add)
{
  ...
  u32 acl[OUTPUT_ACL_N_TABLES] = { ip4_table_index, ip6_table_index,
    l2_table_index
  };
  u32 ti;
  ...
  for (ti = 0; ti < OUTPUT_ACL_N_TABLES; ti++)
    {
      if (acl[ti] == ~0)
	continue;
	...
	return 0;
      ...
      vnet_outacl_ip_feature_enable (vm, am, sw_if_index, ti, is_add);
...
/*
 * Configure interface to enable/disble output ACL feature:
 * intfc - interface name to be configured as output ACL
 * Ip4-table <index> [del] - enable/disable IP4 output ACL
 * Ip6-table <index> [del] - enable/disable IP6 output ACL
 * l2-table <index> [del] - enable/disable Layer2 output ACL
 *
...
clib_error_t *
output_acl_init (vlib_main_t * vm)
{
  output_acl_main_t *am = &output_acl_main;
  clib_error_t *error = 0;

  if ((error = vlib_call_init_function (vm, ip_outacl_init)))
    return error;
...
VLIB_INIT_FUNCTION (output_acl_init);

uword
unformat_output_acl_type (unformat_input_t * input, va_list * args)
{
  u32 *acl_type = va_arg (*args, u32 *);
  u32 tid = OUTPUT_ACL_N_TABLES;
...
u8 *
format_vnet_outacl_info (u8 * s, va_list * va)
{
  output_acl_main_t *am = va_arg (*va, output_acl_main_t *);
...
{
  output_acl_main_t *am = &output_acl_main;
  u32 type = OUTPUT_ACL_N_TABLES;
  int i;
  u32 *vec_tbl;

  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "type %U", unformat_output_acl_type, &type))
	;
      else
	break;
    }

  if (type == OUTPUT_ACL_N_TABLES)
    return clib_error_return (0, "Invalid output ACL table type.");

  vec_tbl = am->classify_table_index_by_sw_if_index[type];

  if (vec_len (vec_tbl))
    vlib_cli_output (vm, "%U", format_vnet_outacl_info, am, ~0 /* hdr */ , ~0);
  else
    vlib_cli_output (vm, "No output ACL tables configured");
  ...
  for (i = 0; i < vec_len (vec_tbl); i++)
	...
	continue;
      ...
      vlib_cli_output (vm, "%U", format_vnet_outacl_info,
