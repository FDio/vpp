				 ...
				 u32 l2_table_index, u32 is_add)
{
  ...
  for (ti = 0; ti < POLICER_CLASSIFY_N_TABLES; ti++)
	...
	(pcm->classify_table_index_by_sw_if_index[ti], sw_if_index, ~0);
      vec_validate_init_empty (pcm->intf_pc_set_flag[ti], sw_if_index, 0);
	   ...
	   pcm->classify_table_index_by_sw_if_index[ti][sw_if_index]))
	{
      ...
      if (is_add && pcm->intf_pc_set_flag[ti][sw_if_index] != 0)
	return 0;
      ...
      if (is_add)
	{
	  ...
	  pcm->intf_pc_set_flag[ti][sw_if_index] = 1;
	}
      else
	{
	  pcm->intf_pc_set_flag[ti][sw_if_index] = 0;
	}
    }
