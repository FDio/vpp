...
vnet_in_out_acl_ip_feature_enable (vlib_main_t * vnm,
			      ...
			      u32 sw_if_index,
			      in_out_acl_table_id_t tid, int feature_enable, int is_output)
{

  if (tid == IN_OUT_ACL_TABLE_L2)
    {
      l2input_intf_bitmap_enable (sw_if_index, is_output ? L2OUTPUT_FEAT_ACL : L2INPUT_FEAT_ACL,
				  feature_enable);
    }
  else
    {				/* IP[46] */
	...
	{
	  vnet_feature_enable_disable ("ip4-unicast", is_output ? "ip4-outacl" : "ip4-inacl",
				       sw_if_index, feature_enable, 0, 0);
	  arc = vnet_get_feature_arc_index ("ip4-unicast");
	}
      else
	{
	  vnet_feature_enable_disable ("ip6-unicast", is_output ? "ip6-outacl" : "ip6-inacl",
				       sw_if_index, feature_enable, 0, 0);
...
vnet_set_in_out_acl_intfc (vlib_main_t * vm, u32 sw_if_index,
			  u32 ip4_table_index,
			  u32 ip6_table_index, u32 l2_table_index, u32 is_add, u32 is_output)
{
  ...
  for (ti = 0; ti < IN_OUT_ACL_N_TABLES; ti++)
	...
	return 0;
      ...
      vnet_in_out_acl_ip_feature_enable (vm, am, sw_if_index, ti, is_add, is_output);
...
{
  return vnet_set_in_out_acl_intfc(vm, sw_if_index, ip4_table_index, ip6_table_index, l2_table_index, is_add, IN_OUT_ACL_INPUT_TABLE_GROUP);
}
...
vnet_set_output_acl_intfc (vlib_main_t * vm, u32 sw_if_index,
			  ...
			  u32 ip6_table_index, u32 l2_table_index, u32 is_add)
{
  return vnet_set_in_out_acl_intfc(vm, sw_if_index, ip4_table_index, ip6_table_index, l2_table_index, is_add, IN_OUT_ACL_OUTPUT_TABLE_GROUP);
}

static clib_error_t *
set_in_out_acl_command_fn (vlib_main_t * vm,
			  unformat_input_t * input, vlib_cli_command_t * cmd, u32 is_output)
{
  ...
  rv = vnet_set_in_out_acl_intfc (vm, sw_if_index, ip4_table_index,
				 ip6_table_index, l2_table_index, is_add, is_output);
