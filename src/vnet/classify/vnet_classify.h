				 ...
				 u64 hash, f64 now)
{
  ...
  u64x8u result512 __attribute__ ((aligned (sizeof (u64x8))));
  ...
  for (i = 0; i < limit; i++)
	...
	data512 = u64x8_maskz_load_unaligned (kmask.x16[0], (u8 *) h);
      ...
      result512 = u64x8_maskz_ternarylogic (kmask.x16[0], data512, mask512,
					      key512, 0x6A);
      if (kmask.x16[1])
	{
	  data128 = (u32x4 *) h + 4;
	  result512 |= (data128[0] & mask128[0]) ^ key128[0];
	}

      if (u64x8_is_all_zero (result512))
	{
	  if (PREDICT_TRUE (now))
	    {
	      v->hits++;
