				 ...
				 u64 hash, f64 now)
{
...
#if defined(CLIB_HAVE_VEC512)
  u64x8u data512, mask512, key512, result512;
  u32x4 *data128, *mask128, *key128, result128;
  ...
  for (i = 0; i < limit; i++)
	...
	{
	  data128 = (u32x4 *) h + 4;
	  result128 = (data128[0] & mask128[0]) ^ key128[0];
	}

      if (u64x8_is_all_zero (result512) && u32x4_is_all_zero (result128))
	{
	  if (PREDICT_TRUE (now))
	    {
	      v->hits++;
