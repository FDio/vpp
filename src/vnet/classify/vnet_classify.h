...
vnet_classify_hash_packet_inline (vnet_classify_table_t *t, const u8 *h)
{
  ...
  if (PREDICT_FALSE (load_mask > 0xff))
  ...
  xor_sum_x8 ^= u64x8_align_right (xor_sum_x8, xor_sum_x8, 1);
  xor_sum = xor_sum_x8[0];
  ...
  xor_sum_x4 ^= u64x4_align_right (xor_sum_x4, xor_sum_x4, 1);
  xor_sum = xor_sum_x4[0];
