...
unformat_gen_mask (unformat_input_t *input, va_list *args)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	vni = 0x00FFFFFF;
      else if (unformat (input, "optlen %d", &genopt_len))
	;
 ...
 *
 * @cliexcmd{classify filter trace mask gen optlen 56 g3 ip4 src dst
 * match gen optlen 56 g3 ip4 src 192.168.1.10 dst 192.168.2.10}
 * <b><em>trace add dpdk-input 100 filter</em></b>
 *
