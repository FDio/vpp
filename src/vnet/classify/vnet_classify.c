			 ...
			 u32 skip_n_vectors, u32 match_n_vectors)
{
  ...
  _clib_memcpy (t->mask, mask, match_n_vectors * sizeof (u32x4));
  ...
  (vnet_classify_table_t * t, vnet_classify_bucket_t * b)
{
  ...
  _clib_memcpy (working_copy, v, required_length);
  ...
  for (i = 0; i < length_in_entries; i++)
      ...
      if (vnet_classify_entry_is_busy (v))
	{
	  ...
	  for (j = 0; j < t->entries_per_page; j++)
		...
		{
		  _clib_memcpy (new_v, v, sizeof (vnet_classify_entry_t)
		       ...
		       vnet_classify_entry_t * add_v, int is_add)
{
  ...
  if (b->offset == 0)
      ...
      if (is_add == 0)
	{
      ...
      _clib_memcpy (v, add_v, sizeof (vnet_classify_entry_t) +
		    t->match_n_vectors * sizeof (u32x4));
  ...
  for (i = 0; i < limit; i++)
    {
      ...
      if (vnet_classify_entry_is_free (new_v))
	{
	  _clib_memcpy (new_v, add_v, sizeof (vnet_classify_entry_t) +
			t->match_n_vectors * sizeof (u32x4));
	  new_v->flags &= ~(VNET_CLASSIFY_ENTRY_FREE);
...
unformat_ip6_match (unformat_input_t * input, va_list * args)
{
  ...
  if (src)
    _clib_memcpy (&ip->src_address, &src_val, sizeof (ip->src_address));

  if (dst)
    _clib_memcpy (&ip->dst_address, &dst_val, sizeof (ip->dst_address));
...
unformat_l2_match (unformat_input_t * input, va_list * args)
{
  ...
  if (dst)
    _clib_memcpy (match, dst_val, 6);

  if (src)
    _clib_memcpy (match + 6, src_val, 6);
			       ...
			       u8 action, u32 metadata, int is_add)
{
  ...
  _clib_memcpy (&e->key, match + t->skip_n_vectors * sizeof (u32x4),
		t->match_n_vectors * sizeof (u32x4));
