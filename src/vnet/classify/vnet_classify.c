			     ...
			     vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	if (unformat
	    (line_input, "l2-input-hit-next %U", unformat_l2_input_next_index,
	     &hit_next_index))
	;
      else
	if (unformat
	    (line_input, "l2-output-hit-next %U", unformat_l2_output_next_index,
	     &hit_next_index))
	;
      else if (unformat (line_input, "acl-hit-next %U", unformat_acl_next_index,
			 &hit_next_index))
	;
	...
	;
      else if (unformat (line_input, "match %U", unformat_classify_match,
	...
	action = 2;
      else if (unformat (line_input, "action set-sr-policy-index %d", &metadata))
	action = 3;
      else
	{
