...
static clib_error_t *
show_scheduler_engines_command_fn (vlib_main_t * vm,
				   ...
				   vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output (vm, "%-7s%-20s%-8s%s", "Active", "Name", "Prio", "Description");
  /* *INDENT-OFF* */
  vec_foreach (p, sm->engines)
    {
      vlib_cli_output (vm, "%-7s%-20s%-8u%s",
		       (p - sm->engines == sm->def_engine_index ? "*" : ""),
				...
				vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "%u", &engine_index))
	;
      else
  ...
  if (engine_index >= vec_len(sm->engines))
    return (clib_error_return(0, "Invalid engine id"));

  sm->def_engine_index = engine_index;
  sm->enqueue_distribute_handler =
    sm->engines[sm->def_engine_index].enqueue_distribute_handler;
  sm->enqueue_aggregate_handler =
    sm->engines[sm->def_engine_index].enqueue_aggregate_handler;
  sm->dequeue_distribute_handler =
    sm->engines[sm->def_engine_index].dequeue_distribute_handler;
  sm->dequeue_aggregate_handler =
    sm->engines[sm->def_engine_index].dequeue_aggregate_handler;
  sm->set_thread_state_handler =
    sm->engines[sm->def_engine_index].set_thread_state_handler;

  return 0;
}

/*?
 * This command sets default scheduler engine.
 *
 * @cliexpar
...
VLIB_CLI_COMMAND (cmd_vnet_scheduler_select_engine, static) = {
  ...
  .short_help = "scheduler select engine <id>",
...
clib_warning("max: %d, num: %d", max_worker_id, vlib_num_workers());
  if (max_worker_id >= vlib_num_workers()){
    err = clib_error_return (0, "Invalid worker id '%d'", max_worker_id);
    goto error_exit;
  }

  if (vnet_scheduler_change_thread_role (vm,
					 workers[VNET_SCHEDULER_THREAD_ROLE_PRODUCER],
...
/*?
 * This command sets scheduler producer, worker, and consumer list.
 *
 * @cliexpar
 * Example of how to set worker crypto processing off:
 * @cliexstart{set sw_scheduler worker 0 crypto off}
...
VLIB_CLI_COMMAND (cmd_vnet_scheduler_set_role, static) = {
  ...
  .short_help = "scheduler set worker role producer <worker A> <worker B> ... "
      "worker <worker C> <worker D> ... consumer <worker E> <worker F> ...",
				 ...
				 vlib_cli_command_t *cmd)
{
  ...
  for (i = skip_master; i < vlib_num_workers (); i++)
    {
      f64 pps;
      u64 curr, *last;
