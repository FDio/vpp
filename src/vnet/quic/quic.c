...
static inline int
quic_ctx_init_client (quic_ctx_t * ctx)
{
  return quic_vfts[ctx->quic_ctx_engine].ctx_init_client (ctx);
}

static int
quic_session_connected_callback (u32 quic_app_index, u32 ho_ctx_index,
				 stream_session_t * quic_session, u8 is_fail)
{
  ...
  ho_ctx = quic_ctx_half_open_get (ho_ctx_index);
  app = application_get (ho_ctx->quic_parent_app_index);
  cb_fn = app->cb_fns.session_connected_callback;

  if (is_fail)
    {
      quic_ctx_half_open_reader_unlock ();
      quic_ctx_half_open_free (ho_ctx_index);
      return cb_fn (ho_ctx->quic_parent_app_index, ho_ctx->c_s_index, 0,
		    1 /* failed */ );
    }

  ctx_handle = quic_ctx_alloc (ho_ctx->quic_ctx_engine);
  ctx = quic_ctx_get (ctx_handle);
  ...
  QUIC_DBG (1, "UDP connect for %u returned %u. New connection [%u]%x",
  ...
  return quic_ctx_init_client (ctx);
}

static inline u8
quic_ctx_handshake_is_over (quic_ctx_t * ctx)
{
  return quic_vfts[ctx->quic_ctx_engine].ctx_handshake_is_over (ctx);
}

static void
quic_session_disconnect_callback (stream_session_t * quic_session)
{
  stream_session_t *app_session;
  quic_ctx_t *ctx;
  application_t *app;

  ctx = quic_ctx_get (quic_session->opaque);
  if (!quic_ctx_handshake_is_over (ctx))
    {
      stream_session_disconnect (quic_session);
      return;
    }
  ctx->is_passive_close = 1;
  app = application_get (ctx->quic_parent_app_index);
  app_session = session_get_from_handle (ctx->quic_app_session_handle);
  app->cb_fns.session_disconnect_callback (app_session);
}

static inline int
quic_ctx_write (quic_ctx_t * ctx, stream_session_t * app_session)
{
  return quic_vfts[ctx->quic_ctx_engine].ctx_write (ctx, app_session);
}

static inline int
quic_ctx_read (quic_ctx_t * ctx, stream_session_t * quic_session)
{
  return quic_vfts[ctx->quic_ctx_engine].ctx_read (ctx, quic_session);
}
...
static int
quic_app_rx_callback (stream_session_t * quic_session)
{
  quic_ctx_t *ctx;

  ctx = quic_ctx_get (quic_session->opaque);
  quic_ctx_read (ctx, quic_session);
  return 0;
}

static int
quic_app_tx_callback (stream_session_t * app_session)
{
  quic_ctx_t *ctx;
  if (PREDICT_FALSE (app_session->session_state == SESSION_STATE_CLOSED))
    return 0;
  ctx = quic_ctx_get (app_session->connection_index);
  quic_ctx_write (ctx, app_session);
  return 0;
}
