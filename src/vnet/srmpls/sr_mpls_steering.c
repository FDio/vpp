...
sr_mpls_steering_policy (int is_del, mpls_label_t bsid, u32 sr_policy_index,
		    u32 table_id, ip46_address_t * prefix, u32 mask_width, u8 traffic_type)
{
    ...
    if (is_del)
  	  ...
  	  if (steer_pl->classify.traffic_type == SR_STEER_IPV6)
      {
        ...
        fib_table_entry_delete (fib_table_find
  			      (FIB_PROTOCOL_MPLS, steer_pl->classify.fib_table),
  			      &pfx, FIB_SOURCE_SR);
      }
  	  else if (steer_pl->classify.traffic_type == SR_STEER_IPV4)
	    {
	      ...
	      fib_table_entry_delete (fib_table_find
				      (FIB_PROTOCOL_MPLS, steer_pl->classify.fib_table), &pfx,
				      FIB_SOURCE_SR);
	    }
    ...
    else			/* It means user requested to update an existing SR steering policy */
  	{
  	  ...
  	  if (steer_pl->classify.traffic_type == SR_STEER_IPV6)
	    {
	      ...
	      fib_table_entry_delete (fib_table_find
				      (FIB_PROTOCOL_IP6, steer_pl->classify.fib_table),
				      &pfx, FIB_SOURCE_SR);
  	  ...
  	  else if (steer_pl->classify.traffic_type == SR_STEER_IPV4)
	    {
	      ...
	      fib_table_entry_delete (fib_table_find
				      (FIB_PROTOCOL_IP4, steer_pl->classify.fib_table),
				      &pfx, FIB_SOURCE_SR);
...
update_fib: ;
  ...
  if (traffic_type == SR_STEER_IPV6)
  {
    ...
    path.frp_fib_index = sm->fib_table_ip6;

    vec_add1(paths, path);

    fib_table_entry_path_add2(
      fib_table_find(FIB_PROTOCOL_IP6, (table_id != (u32) ~ 0 ? table_id : 0)),
      &pfx,
      FIB_SOURCE_SR, 
  ...
  else if (traffic_type == SR_STEER_IPV4)
  {
    ...
    path.frp_fib_index = sm->fib_table_ip4;

    vec_add1(paths, path);

    fib_table_entry_path_add2(
      fib_table_find(FIB_PROTOCOL_IP4, (table_id != (u32) ~ 0 ? table_id : 0)),
      &pfx,
      FIB_SOURCE_SR, 
      FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT,
      paths);
			    ...
			    vlib_cli_command_t * cmd)
{
  ...
  rv =
    sr_mpls_steering_policy (is_del, bsid,
			sr_policy_index, fib_table, &prefix, dst_mask_width, traffic_type);
