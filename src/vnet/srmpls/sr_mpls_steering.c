...
/**
 * @ brief find the corresponding label for (endpoint, color)
...
find_or_create_internal_label(ip46_address_t endpoint, u32 color)
{
  ...
  if (!sm->sr_policies_c2e2eclabel_hash.hash)
    mhash_init (&sm->sr_policies_c2e2eclabel_hash, sizeof (mhash_t), sizeof (u32));
  ...
  *new_internal_label += 50+(new_internal_label-sm->ec_labels);
  mhash_set ((mhash_t *)color_table, &endpoint, *new_internal_label, NULL);

  return *new_internal_label;
}
...
sr_mpls_steering_policy_add (mpls_label_t bsid, u32 table_id,
           ...
           u8 traffic_type, ip46_address_t * next_hop, u8 nh_type,
           u32 color, char co_bits)
{
  ...
  if (p)
  {
    ...
    ** Add the new color steering - TODO 
    sr_policy = sr_mpls_policy_find_bsid_from_nh_color (next_hop, color, co_bits);
    if (!sr_policy)
    return -2;
  ...
  if (bsid != (u32) ~0)
  {
    ...
    if (traffic_type == SR_STEER_IPV6)
    {
      ...
      vec_add1 (paths, path);
      fib_table_entry_path_add2 (fib_table_find
         (FIB_PROTOCOL_IP6,(table_id != (u32) ~ 0 ? table_id : 0)),&pfx, FIB_SOURCE_SR,
  ...
  else
  {
    ...
    /* Generate the vector of internal MPLS labels */
    ...
    switch(co_bits)
    {
      case SR_TE_CO_BITS_10:
        vec_add1(internal_labels, find_or_create_internal_label(*next_hop, color));
        ip46_address_reset(&zero);
        vec_add1(internal_labels, find_or_create_internal_label(zero, color));
        any.as_u64[0] = any.as_u64[1] = (u64)~0;
        vec_add1(internal_labels, find_or_create_internal_label(any, color));
        break;
    ...
    if (sm->fib_table_EC == (u32) ~0)
    {
      sm->fib_table_EC = fib_table_create_and_lock (FIB_PROTOCOL_MPLS,
          FIB_SOURCE_SR, "SR-MPLS Traffic Engineering (NextHop,Color)");

      fib_table_flush(sm->fib_table_EC, FIB_PROTOCOL_MPLS, FIB_SOURCE_SPECIAL);
    }
    else 
      fib_table_lock(sm->fib_table_EC, FIB_PROTOCOL_MPLS, FIB_SOURCE_SR);
    ...
    vec_foreach_index(label_i, internal_labels)
    {
      ...
      if (traffic_type == SR_STEER_IPV6)
        fib_table_entry_path_add2 (fib_table_find
          (FIB_PROTOCOL_IP6, (table_id != (u32) ~ 0 ? table_id : 0)),
          &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, paths);
      else if (traffic_type == SR_STEER_IPV4)
        fib_table_entry_path_add2 (fib_table_find
          (FIB_PROTOCOL_IP4, (table_id != (u32) ~ 0 ? table_id : 0)),
          &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, paths);
      vec_free (paths);
      paths = NULL;
    }
    ...
    path.frp_local_label = 0;
    clib_memcpy(&path.frp_addr, next_hop, sizeof(* next_hop));
    path.frp_preference = vec_len(internal_labels);
    if(nh_type == SR_STEER_IPV6)
    {
      path.frp_proto = DPO_PROTO_IP6;
      path.frp_fib_index = fib_table_find (FIB_PROTOCOL_IP6, (table_id != (u32) ~ 0 ? table_id : 0));
    }
    else if (nh_type == SR_STEER_IPV4)
    {
      path.frp_proto = DPO_PROTO_IP4;
      path.frp_fib_index = fib_table_find (FIB_PROTOCOL_IP4, (table_id != (u32) ~ 0 ? table_id : 0));
    }
    vec_add1 (paths, path);
    if (traffic_type == SR_STEER_IPV6)
      fib_table_entry_path_add2 (fib_table_find
           ...
           u32 color, char co_bits)
{
  ...
  if (steer_pl->bsid == (u32) ~0)
  {
    ...
    if (vec_len(steer_pl->color))
    {
      ...
      //Trigger code to update FIB paths...for now just remove ALL.
      ...
      { 
        ...
        pfx.fp_len = steer_pl->classify.mask_width;
        pfx.fp_addr.ip6 = steer_pl->classify.prefix.ip6;
        fib_table_entry_delete (fib_table_find(FIB_PROTOCOL_IP6, steer_pl->classify.fib_table),
         &pfx,FIB_SOURCE_SR);
...
VLIB_CLI_COMMAND(sr_mpls_steer_policy_command, static)=
{
  .path = "sr mpls steer",
    ...
    "via [sr policy bsid <mpls_label> || via next-hop <ip46_addr> color <u32> co <0|1|2|3> ](fib-table <fib_table_index>)",
    .long_help =
    ...
    "\t\tsr steer del l3 2001::/64 via next-hop 1.1.1.1 color 1234 co 0\n"
    "\t\tsr steer del l3 2001::/64 via next-hop 2001::1 color 1234 co 2\n",
