 ...
 */
void
compute_sr_te_automated_steering_fib_entry(mpls_sr_steering_policy_t *steer_pl)
{
  mpls_sr_main_t *sm = &sr_mpls_main;
  fib_prefix_t pfx = { 0 };
  ...
  if (steer_pl->classify.traffic_type == SR_STEER_IPV6)
    fib_table_entry_update (fib_table_find
      (FIB_PROTOCOL_IP6, (steer_pl->classify.fib_table != (u32) ~ 0 ? steer_pl->classify.fib_table : 0)),
      &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, paths);
  else if (steer_pl->classify.traffic_type == SR_STEER_IPV4)
    fib_table_entry_update (fib_table_find
      (FIB_PROTOCOL_IP4, (steer_pl->classify.fib_table != (u32) ~ 0 ? steer_pl->classify.fib_table : 0)),
      &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, paths);
...
sr_mpls_steering_policy_add (mpls_label_t bsid, u32 table_id,
           ...
           u32 color, char co_bits, mpls_label_t vpn_label)
{
  mpls_sr_main_t *sm = &sr_mpls_main;
  ...
  if (is_del)
    ...
    rv =
      sr_mpls_steering_policy_add (bsid, fib_table, &prefix, dst_mask_width,
           traffic_type, &nh, nh_type, color, co_bits, vpn_label);
  ...
  switch (rv)
    ...
    case -2:
      return clib_error_return (0,
        "The Next-Hop does not match.");
    case -3:
      return clib_error_return (0,
        "The color already exists.");
    case -4:
      return clib_error_return (0,
        "The co-bits do not match.");
    default:
...
VLIB_CLI_COMMAND(sr_mpls_steer_policy_command, static)=
{
  .path = "sr mpls steer",
    ...
    "via [sr policy bsid <mpls_label> || next-hop <ip46_addr> color <u32> co <0|1|2|3> ](fib-table <fib_table_index>)",
    .long_help =
