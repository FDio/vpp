...
compute_sr_te_automated_steering_fib_entry(mpls_sr_steering_policy_t *steer_pl)
{
  ...
  vec_foreach_index(label_i, internal_labels)
  {
    ...
    {
      vec_free(path.frp_label_stack);
      path.frp_label_stack = 0;
      path.frp_local_label = steer_pl->vpn_label;
      vec_add1 (path.frp_label_stack, internal_labels[label_i]);
    }
    else
           ...
           u32 color, char co_bits, mpls_label_t vpn_label)
{
  ...
  if(!sm->sr_steer_policies_hash.hash)
    mhash_init (&sm->sr_steer_policies_hash, sizeof (uword),sizeof (sr_mpls_steering_key_t));
