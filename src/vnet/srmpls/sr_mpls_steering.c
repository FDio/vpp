			     ...
			     u32 color, char co_bits)
{
  ...
  /* Search for steering policy. If already exists we are adding a new one. */
  ...
  if (p)
	...
	{
	  /* Means lower priority. Add color by now. No changes in FIB */
	  vec_add1 (steer_pl->color, color);
	  vec_sort_with_function (steer_pl->color, sort_color_descent);
	}
      else
	{
	  /* Means higher prority. Push at index 0 AND change FIB */
