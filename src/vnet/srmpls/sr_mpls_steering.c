...
find_or_create_internal_label(ip46_address_t endpoint, u32 color)
{
  ...
  if(!color_table)
  {
    ...
    mhash_init (&color_t, sizeof (mpls_label_t), sizeof (ip46_address_t));
    mhash_set_mem (&sm->sr_policies_c2e2eclabel_hash, &color, (uword *)&color_t, NULL);
    color_table = (uword *)&color_t;
  }
  ...
  *new_internal_label += 50;
  mhash_set_mem ((mhash_t *)color_table, &endpoint, (uword *)new_internal_label, NULL);
           ...
           u32 color, char co_bits)
{
  ...
  else
  {
    ...
    clib_memcpy(&next_hop, &path.frp_addr, sizeof(next_hop));
         ...
         vlib_cli_command_t * cmd)
{
  ...
  sr_policy_set = 1;
         ...
         && unformat (input, "via next-hop %U color %d co %d",
          unformat_ip4_address, &nh, &color, &co_bits))
      {
        sr_policy_set = 1;
        nh_type = SR_STEER_IPV4;
         ...
         && unformat (input, "via next-hop %U color %d co %d",
          unformat_ip6_address, &nh, &color, &co_bits))
      {
        sr_policy_set = 1;
