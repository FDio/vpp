...
/**
 * @brief Dynamically added SR-MPLS FIB Node type
 */
static fib_node_type_t sr_mpls_fib_node_type;

/*************************  SR FIB helper functions **************************/
/**
 * @brief Stack a SR MPLS policy on the forwarding chain of the first-hop
 */
static void
sr_mpls_sl_stack (mpls_sr_sl_t * sl)
{
  dpo_id_t tmp_dpo;
  dpo_reset (&tmp_dpo);

  fib_entry_contribute_forwarding (sl->first_hop_fe_index,
				   FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS,
				   &tmp_dpo);

  dpo_stack (DPO_MPLS_LABEL, DPO_PROTO_MPLS, &sl->first_hop_dpo, &tmp_dpo);

  dpo_reset (&tmp_dpo);
}

static mpls_sr_sl_t *
sr_sl_from_fib_node (fib_node_t * node)
{
#if (CLIB_DEBUG > 0)
  ASSERT (sr_mpls_fib_node_type == node->fn_type);
#endif
  return ((mpls_sr_sl_t *) (((char *) node) -
			    STRUCT_OFFSET_OF (mpls_sr_sl_t, node)));
}
...
sr_sl_back_walk (fib_node_t * node, fib_node_back_walk_ctx_t * ctx)
{
  sr_mpls_sl_stack (sr_sl_from_fib_node (node));
 ...
 */
static fib_node_t *
sr_sl_fib_node_get (fib_node_index_t index)
{
  mpls_sr_main_t *sm = &sr_mpls_main;
  mpls_sr_sl_t *segment_list;

  segment_list = pool_elt_at_index (sm->sid_lists, index);

  return (&segment_list->node);
}
 ...
 */
static void
sr_sl_last_lock_gone (fib_node_t * node)
{
  /*
   * The SR SID list is a root of the graph. As such
...
/*
 * Virtual function table registered by SR SID list
 * for participation in the FIB object graph.
 */
const static fib_node_vft_t sr_fib_vft = {
  .fnv_get = sr_sl_fib_node_get,
  .fnv_last_lock = sr_sl_last_lock_gone,
  .fnv_back_walk = sr_sl_back_walk,
};
...
create_sl (mpls_sr_policy_t * sr_policy, mpls_label_t * sl, u32 weight)
{
 ...
 /**
   ...
   */
  fib_node_init (&segment_list->node, sr_mpls_fib_node_type);
  dpo_reset (&segment_list->first_hop_dpo);
  /* *INDENT-OFF* */
  ...
  /* *INDENT-ON* */
  segment_list->first_hop_fe_index =
    fib_table_entry_special_add (fib_table_find
				 (FIB_PROTOCOL_MPLS, sr_policy->fib_table),
				 &first_hop_pfx, FIB_SOURCE_RR,
				 FIB_ENTRY_FLAG_NONE);

  segment_list->sibling_index =
    fib_entry_child_add (segment_list->first_hop_fe_index,
			 sr_mpls_fib_node_type, segment_list - sm->sid_lists);
  sr_mpls_sl_stack (segment_list);

  /* Create the SR DPOs */
  index_t mldi;
  mldi =
    mpls_label_dpo_create (sl, MPLS_EOS, 255, 0, DPO_PROTO_MPLS,
			   &segment_list->first_hop_dpo);
  dpo_set (&segment_list->sl_eos_dpo, DPO_MPLS_LABEL, DPO_PROTO_MPLS, mldi);

  mldi =
    mpls_label_dpo_create (sl, MPLS_NON_EOS, 255, 0, DPO_PROTO_MPLS,
			   &segment_list->first_hop_dpo);
  dpo_set (&segment_list->sl_neos_dpo, DPO_MPLS_LABEL, DPO_PROTO_MPLS, mldi);

  mldi =
    mpls_label_dpo_create (sl, MPLS_EOS, 255, 0, DPO_PROTO_IP4,
			   &segment_list->first_hop_dpo);
  dpo_set (&segment_list->sl_v4_dpo, DPO_MPLS_LABEL, DPO_PROTO_IP4, mldi);

  mldi =
    mpls_label_dpo_create (sl, MPLS_EOS, 255, 0, DPO_PROTO_IP6,
			   &segment_list->first_hop_dpo);
...
update_lb (mpls_sr_policy_t * sr_policy)
{
  ...
  mpls_sr_main_t *sm = &sr_mpls_main;
  mpls_sr_sl_t *segment_list;
  load_balance_path_t *path_bsid_eos, *path_bsid_neos, *path_bsid_v4,
    *path_bsid_v6;
  ...
  if (!dpo_id_is_valid (&sr_policy->bsid_eos_dpo))
	...
	.fp_payload_proto = DPO_PROTO_MPLS,
      ...
      u32 fib_index =
	fib_table_find (FIB_PROTOCOL_MPLS, sr_policy->fib_table);
		    ...
		    u8 behavior, u32 fib_table, u32 weight)
{
  ...
  u32 fib_index = fib_table_find (FIB_PROTOCOL_MPLS,
  ...
  /* If the lookup DPO doesnt exist creat it */
  if (!dpo_id_is_valid (&sm->mpls_lookup_dpo))
    {
...
sr_mpls_policy_del (mpls_label_t bsid, u32 index)
{
  ...
  vec_foreach (sl_index, sr_policy->segments_lists)
  {
    ...
    dpo_reset (&segment_list->sl_v4_dpo);
    dpo_reset (&segment_list->sl_v6_dpo);
    fib_entry_child_remove (segment_list->first_hop_fe_index,
			    segment_list->sibling_index);
    fib_table_entry_delete_index (segment_list->first_hop_fe_index,
				  FIB_SOURCE_RR);
  ...
  if (is_add)
	...
	return clib_error_return (0, "No Segment List specified");
      ...
      rv = sr_mpls_policy_add (bsid, segments,
				...
				SR_POLICY_TYPE_DEFAULT), fib_table, weight);
    }
  else if (is_del)
    rv =
...
sr_mpls_policy_rewrite_init (vlib_main_t * vm)
{
  ...
  sr_mpls_fib_node_type = fib_node_register_new_type (&sr_fib_vft);
