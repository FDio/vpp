 ...
 */
int
sr_mpls_policy_add (mpls_label_t bsid, mpls_label_t *segments, u8 behavior,
		    u32 weight)
{
  mpls_sr_main_t *sm = &sr_mpls_main;
  ...
  if (1 == pool_elts (sm->sr_policies))
    fib_table_find_or_create_and_lock (
      FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID, FIB_SOURCE_SR);
  sr_policy->bsid = bsid;
 ...
 */
int
sr_mpls_policy_mod (mpls_label_t bsid, u8 operation, mpls_label_t *segments,
			   ...
			   vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	is_mod = 1;
	       ...
	       unformat (input, "bsid %U", unformat_mpls_unicast_label, &bsid))
	policy_set = 1;
      else if (unformat (input, "weight %d", &weight))
	;
      else if (unformat (input, "next %U", unformat_mpls_unicast_label,
			 &next_label))
	{
  ...
  if (is_add)
	...
	return clib_error_return (0, "No Segment List specified");
      ...
      rv = sr_mpls_policy_add (
	bsid, segments,
	(is_spray ? SR_POLICY_TYPE_SPRAY : SR_POLICY_TYPE_DEFAULT), weight);
      vec_free (segments);
    }
  else if (is_del)
				      ...
				      u8 endpoint_type, u32 color)
{
  ...
  if (sr_policy->endpoint_type)
	...
	return -2;
      ...
      {
	pfx.fp_eos = eos;
	fib_table_entry_path_remove (sm->fib_table_EC, &pfx, FIB_SOURCE_SR,
				     DPO_PROTO_MPLS, NULL, ~0, 0, 1,
  ...
  if (endpoint_type)
      ...
      if (sm->fib_table_EC == (u32) ~0)
	{
      ...
      pfx.fp_label = label;
      fib_table_entry_update (sm->fib_table_EC, &pfx, FIB_SOURCE_SR,
				  ...
				  vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (!endpoint_type &&
	  unformat (input, "endpoint %U", unformat_ip6_address, &endpoint.ip6))
  ...
  rv = sr_mpls_policy_assign_endpoint_color (bsid, &endpoint, endpoint_type,
					     color);
