				  ...
				  vlib_cli_command_t * cmd)
{
  ...
  vec_foreach_index (i, vec_policies)
  {
    ...
    if (!ip6_address_is_unspecified (&sr_policy->endpoint.ip6))
      {
	if (ip46_address_is_ip4 (&sr_policy->endpoint))
	  vlib_cli_output (vm, "\tNextHop: %U", format_ip4_address,
			   &sr_policy->endpoint.ip4);
	else
	  vlib_cli_output (vm, "\tNextHop: %U", format_ip6_address,
				  ...
				  vlib_cli_command_t * cmd)
{
  ...
  u32 color = (u32) ~ 0;
  mpls_label_t bsid;
  char clear = 0, endpoint_set = 0, color_set = 0, bsid_set = 0;
  ...
  rv = sr_mpls_policy_assign_endpoint_color (bsid, &endpoint, color);
