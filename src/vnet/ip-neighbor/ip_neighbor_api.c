...
static void
vl_api_ip_neighbor_sleep_interval_t_handler (vl_api_ip_neighbor_sleep_interval_t *mp)
{
  ...
  if (!rv)
    rv = ip_neighbor_sleep_interval (af, clib_net_to_host_f64 (mp->sleep_interval));

  REPLY_MACRO (VL_API_IP_NEIGHBOR_SLEEP_INTERVAL_REPLY);
}

static void
vl_api_ip_neighbor_sleep_interval_get_t_handler (vl_api_ip_neighbor_sleep_interval_get_t *mp)
{
