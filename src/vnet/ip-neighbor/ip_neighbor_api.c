...
static void
  vl_api_ip_neighbor_scan_enable_disable_t_handler
  (vl_api_ip_neighbor_scan_enable_disable_t * mp)
{
  int rv = 0;
  vl_api_ip_neighbor_scan_enable_disable_reply_t *rmp;
  ...
  REPLY_MACRO (VL_API_IP_NEIGHBOR_SCAN_ENABLE_DISABLE_REPLY);
}

static void
vl_api_ip_neighbor_set_limit_t_handler (vl_api_ip_neighbor_set_limit_t * mp)
{
  vl_api_ip_neighbor_set_limit_reply_t *rmp;
  ip_address_family_t af;
  int rv;
  ...
  if (!rv)
    rv = ip_neighbor_set_limit (ip46_type_from_af (af), ntohl (mp->limit));

  REPLY_MACRO (VL_API_IP_NEIGHBOR_SET_LIMIT_REPLY);
}
