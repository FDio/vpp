...
/** Timer wheel for all neighbors */
static tw_timer_wheel_2t_1w_2048sl_t ip_neighbor_timer_wheel;

index_t ip_neighbor_list_head;
...
/** The configured Expiration Time, in seconds, for all neighbors */
static u32 ip_neighbor_max_age[IP46_N_TYPES];
...
ip_neighbor_refresh (ip_neighbor_t * ipn)
{
  ...
  if (ip_neighbor_is_dynamic (ipn))
    {
      head = pool_elt_at_index (ip_neighbor_elt_pool, ip_neighbor_list_head);
...
ip_neighbor_free (ip_neighbor_t * ipn)
{
  ...
  if (ipn->ipn_timer)
    tw_timer_stop_2t_1w_2048sl (&ip_neighbor_timer_wheel, ipn->ipn_timer);
...
ip_neighbor_force_reuse (ip46_type_t type)
{
  ...
  clib_llist_pop_last (ip_neighbor_elt_pool, ipne_anchor, elt, head);
		   ...
		   const mac_address_t * mac, ip_neighbor_flags_t flags)
{
  ...
  /*
   * start the aging timer if the entry is dynamic and if there is aging configured
   */
  if (ip_neighbor_is_dynamic (ipn) && ip_neighbor_max_age[key->ipnk_type])
    ipn->ipn_timer = tw_timer_start_2t_1w_2048sl (&ip_neighbor_timer_wheel,
						  ip_neighbor_get_index (ipn),
						  0,
						  ip_neighbor_max_age
						  [key->ipnk_type]);
...
ip_neighbor_probe (const ip_adjacency_t * adj)
{
  ...
  type = fib_proto_to_ip46 (adj->ia_nh_proto);

  if (ip_nbr_vfts[type].inv_probe)
    return (ip_nbr_vfts[type].inv_probe (adj));
}
...
int
ip_neighbor_send_unsolicited (vlib_main_t * vm,
			      ip46_type_t type,
			      const ip46_address_t * addr, u32 sw_if_index)
{
  if (ip_nbr_vfts[type].inv_send)
    {
      ip_nbr_vfts[type].inv_send (vm, sw_if_index, addr);
      return 0;
    }
				    ...
				    u32 sw_if_index, u32 flags)
{
  ...
  if (flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP)
    {
      FOR_EACH_IP46_TYPE (type) ip_neighbor_populate (type, sw_if_index);
    }
  else
    {
      /* admin down, flush all neighbours */
      FOR_EACH_IP46_TYPE (type) ip_neighbor_flush (type, sw_if_index);
    }
 ...
 */
static clib_error_t *
ip_neighbor_delete_sw_interface (vnet_main_t * vnm, u32 sw_if_index,
				 u32 is_add)
{
...
static void
ip_neighbor_expired_timers_dispatch (u32 * expired_timers)
{
  index_t *ipni;

  vec_foreach (ipni, expired_timers)
    // since we don't set a timer ID, there is a direct mapping
    // between timer-id and neighbor index
    ;
}
...
static uword
ip_neighbor_age_process (vlib_main_t * vm,
  ...
  while (1)
    ...
    restart:
      ...
      ({
        ...
        res = ip_neighbour_age_out(elt->ipne_index, now, &timeout);
        ...
        /* else - PROBE - go poke the next one */
        }));
      /* *INDENT-ON*/
    }
  return 0;
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (ip_neighbor_age_process_node,static) = {
  .function = ip_neighbor_age_process,
  .type = VLIB_NODE_TYPE_PROCESS,
  .name = "ip-neighbor-age-process",
};
/* *INDENT-ON* */
...
ip_neighbor_init (vlib_main_t * vm)
{
  ...
  tw_timer_wheel_init_2t_1w_2048sl (&ip_neighbor_timer_wheel,
				    ip_neighbor_expired_timers_dispatch,
				    1.0, ~0);

  ip_neighbor_list_head = clib_llist_make_head (ip_neighbor_elt_pool,
						ipne_anchor);
