...
extern int ip_neighbor_set_limit (ip46_type_t type, u32 limit);
 ...
 */
extern int ip_neighbor_send_unsolicited (vlib_main_t * vm,
					 ip46_type_t tyoe,
...
/** Probe (send request to resolve) a peer */
typedef void (*ip_neighbor_probe_t) (const ip_adjacency_t * adj);

/** Send an unsolicited gratuitous */
typedef void (*ip_neighbor_send_unsolicited_t) (vlib_main_t * vm,
						u32 sw_if_index,
						const ip46_address_t * addr);
...
{
  ip_neighbor_probe_t inv_probe;
  ip_neighbor_send_unsolicited_t inv_send;
