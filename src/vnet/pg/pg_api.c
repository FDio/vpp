...
vl_api_pg_create_interface_t_handler (vl_api_pg_create_interface_t * mp)
{
  ...
  u8 * interface_name = format (0, "%U%c", format_pg_interface_name, pg_if_id, 0);
  u32 size = ARRAY_LEN(interface_name);
  ...
  if (hw_if_index != ~0)
	{
	  ...
	  a->pcap_file_name = format(0, "%v%c", vl_api_from_api_to_vec(&mp->pcap_file_name), 0);
...
vl_api_pg_capture_timeout_t_handler (vl_api_pg_capture_timeout_t * mp)
{
  ...
  pg_interface_t *pi;
  int rv = 0;
  vnet_hw_interface_t *hi = vnet_get_sup_hw_interface (vnm, ntohl(mp->sw_if_index));
...
static void
send_pg_capture_details(vl_api_registration_t * reg, u32 context, pg_interface_t * pif)
{
  u32 msg_size;
  ...
  u8 * pg_interface_name = format (0, "%U%c", format_pg_interface_name, hi->dev_instance, 0);

  msg_size =
    sizeof (*rmp) + strlen (pif->pcap_main.file_name) + vec_len (pg_interface_name) ;
  ...
  p += vl_api_to_api_string (strlen(pif->pcap_main.file_name), pif->pcap_main.file_name, (vl_api_string_t *) p);
