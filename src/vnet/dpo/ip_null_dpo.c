...
void
ip_null_dpo_add_and_lock (dpo_proto_t proto, ip_null_dpo_action_t action,
			  dpo_id_t *dpo)
{
  int i;
...
format_ip_null_dpo (u8 *s, va_list *ap)
{
  ...
  return (format (s, "%U-null action:%s", format_dpo_proto, proto,
...
always_inline uword
ip_null_dpo_switch (vlib_main_t *vm, vlib_node_runtime_t *node,
  ...
  if (time_now - time_last_seed_change > 1e-1)
    {
      uword i;
      u32 *r =
	clib_random_buffer_get_data (&vm->random_buffer, sizeof (hash_seeds));
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (!drop0))
	      ...
	      if (is_ip4)
		{
		  ...
		  else if (IP_NULL_ACTION_SEND_ICMP_PROHIBIT ==
			   ind0->ind_action)
		    {
		      next0 = IP_NULL_NEXT_ICMP;
		      icmp4_error_set_vnet_buffer (
...
static uword
ip4_null_dpo_switch (vlib_main_t *vm, vlib_node_runtime_t *node,
		     vlib_frame_t *frame)
{
...
static uword
ip6_null_dpo_switch (vlib_main_t *vm, vlib_node_runtime_t *node,
		     vlib_frame_t *frame)
{
