...
void
pw_cw_dpo_create (const dpo_id_t *parent, dpo_id_t *dpo)
{
  pw_cw_dpo_t *pwcw;
...
always_inline uword
pw_cw_pop_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  vnet_buffer (b0)->ip.adj_index[VLIB_TX] =
	    pwcw0->pwcw_parent.dpoi_index;
	  vnet_buffer (b1)->ip.adj_index[VLIB_TX] =
	    pwcw1->pwcw_parent.dpoi_index;
...
{
  fib_show_memory_usage ("PW-CW", pool_elts (pw_cw_dpo_pool),
			 pool_len (pw_cw_dpo_pool), sizeof (pw_cw_dpo_t));
}
