    ...
    while (n_left_from > 0)
    {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2)
        {
            ...
            if (payload_is_ip4)
            {
                 ...
                 * including checksum
                 */
                ip4_input_check_x2 (vm, b0, b1, ip0, ip1,
                                    &error0, &error1,
                                    &next0, &next1, 1);
            ...
            else if (payload_is_ip6)
            {
                 ...
                 */
                ip6_input_check_x2 (vm, b0, b1, ip0, ip1,
                                    &error0, &error1,
                                    &next0, &next1);
                b0->error = error_node->errors[error0];
                b1->error = error_node->errors[error1];
        ...
        while (n_left_from > 0 && n_left_to_next > 0)
        {
            ...
            if (payload_is_ip4)
            {
                 ...
                 * including checksum
                 */
                ip4_input_check_x1 (vm, b0, ip0, &error0, &next0, 1);
                b0->error = error_node->errors[error0];
            }
            else if (payload_is_ip6)
            {
                 ...
                 * IPv6 input checks on the exposed IP header
                 */
                ip6_input_check_x1 (vm, b0, ip0, &error0, &next0);
                b0->error = error_node->errors[error0];
            }
