...
void
mpls_disp_dpo_create (dpo_proto_t payload_proto, fib_rpf_id_t rpf_id,
		      fib_mpls_lsp_mode_t mode, const dpo_id_t *parent,
		      dpo_id_t *dpo)
{
  mpls_disp_dpo_t *mdd;
  dpo_type_t dtype;
  ...
  mdd->mdd_mode = mode;
  dtype = (FIB_MPLS_LSP_MODE_PIPE == mode ? DPO_MPLS_DISPOSITION_PIPE :
...
format_mpls_disp_dpo (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U, %U]", format_dpo_proto, mdd->mdd_payload_proto,
...
mpls_label_disposition_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			       vlib_frame_t *from_frame, u8 payload_is_ip4,
			       u8 payload_is_ip6, fib_mpls_lsp_mode_t mode)
{
  ...
  if (payload_is_ip4)
	...
	error_node = vlib_node_get_runtime (
	  vm, ip4_mpls_label_disposition_pipe_node.index);
      else
	error_node = vlib_node_get_runtime (
	  vm, ip4_mpls_label_disposition_uniform_node.index);
    }
  else
    {
      if (FIB_MPLS_LSP_MODE_PIPE == mode)
	error_node = vlib_node_get_runtime (
	  vm, ip6_mpls_label_disposition_pipe_node.index);
      else
	error_node = vlib_node_get_runtime (
	  vm, ip6_mpls_label_disposition_uniform_node.index);
    }
	  ...
	  if (payload_is_ip4)
	    {
	       ...
	       * including checksum
	       */
	      ip4_input_check_x2 (vm, error_node, b0, b1, ip0, ip1, &next0,
				  &next1, 1);
	  ...
	  else if (payload_is_ip6)
	    {
	       ...
	       */
	      ip6_input_check_x2 (vm, error_node, b0, b1, ip0, ip1, &next0,
				  &next1);
	      ...
	      if (FIB_MPLS_LSP_MODE_UNIFORM == mode)
		{
		  ...
		  ip6_set_traffic_class_network_order (
		    ip1, mpls_exp_to_ip_dscp (vnet_buffer (b1)->mpls.exp));
		}
	    }
...
format_mpls_label_disposition_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "rpf-id:%d %U, %U", t->mddt_rpf_id, format_dpo_proto,
	      t->mddt_payload_proto, format_fib_mpls_lsp_mode, t->mddt_mode);
...
{
  fib_show_memory_usage ("MPLS label", pool_elts (mpls_disp_dpo_pool),
...
const static char *const
  *const mpls_label_disp_uniform_nodes[DPO_PROTO_NUM] = {
