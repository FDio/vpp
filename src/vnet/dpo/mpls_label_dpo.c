...
void
mpls_label_dpo_create (fib_mpls_label_t *label_stack, mpls_eos_bit_t eos,
		       dpo_proto_t payload_proto, mpls_label_dpo_flags_t flags,
  ...
  if (MPLS_LABEL_DPO_MAX_N_LABELS < vec_len (label_stack))
    {
      ...
      dpo_stack (dtype, mld->mld_payload_proto, &mld->mld_dpo,
...
format_mpls_label_dpo (u8 *s, va_list *args)
{
  ...
  s = format (s, "mpls-label[%U@%d]:", format_mpls_label_dpo_flags,
 ...
 */
always_inline mpls_unicast_header_t *
mpls_label_paint_w_ttl_exp (vlib_buffer_t *b0, mpls_label_dpo_t *mld0, u8 ttl0,
			    u8 exp0)
{
  mpls_unicast_header_t *hdr0;
 ...
 */
always_inline mpls_unicast_header_t *
mpls_label_paint_w_ttl_mpls_exp (vlib_buffer_t *b0, mpls_label_dpo_t *mld0,
				 u8 ttl0, u8 exp0)
{
  mpls_unicast_header_t *hdr0;
...
always_inline uword
mpls_label_imposition_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		  ...
		  if (!(MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR & flags))
		    {
		      ...
		      checksum0 =
			ip0->checksum + clib_host_to_net_u16 (0x0100);
		      checksum1 =
			ip1->checksum + clib_host_to_net_u16 (0x0100);
		      checksum2 =
			ip2->checksum + clib_host_to_net_u16 (0x0100);
		      checksum3 =
			ip3->checksum + clib_host_to_net_u16 (0x0100);
		  ...
		  if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE)
		    {
		      ...
		      exp0 = ((u8 *) hdr0)[2] & 0xe;
		      hdr0 =
			mpls_label_paint_w_ttl_mpls_exp (b0, mld0, ttl0, exp0);
		    }
		  else
		    {
		      hdr0 = mpls_label_paint (b0, mld0);
		  ...
		  if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE)
		    {
		      ...
		      exp1 = ((u8 *) hdr1)[2] & 0xe;
		      hdr1 =
			mpls_label_paint_w_ttl_mpls_exp (b1, mld1, ttl1, exp1);
		    }
		  else
		    {
		      hdr1 = mpls_label_paint (b1, mld1);
		  ...
		  if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE)
		    {
		      ...
		      exp2 = ((u8 *) hdr2)[2] & 0xe;
		      hdr2 =
			mpls_label_paint_w_ttl_mpls_exp (b2, mld2, ttl2, exp2);
		    }
		  else
		    {
		      hdr2 = mpls_label_paint (b2, mld2);
		  ...
		  if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE)
		    {
		      ...
		      exp3 = ((u8 *) hdr3)[2] & 0xe;
		      hdr3 =
			mpls_label_paint_w_ttl_mpls_exp (b3, mld3, ttl3, exp3);
		    }
		  else
		    {
		      hdr3 = mpls_label_paint (b3, mld3);
...
format_mpls_label_imposition_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "%Umpls-header:%U", format_white_space, indent,
	      format_mpls_header, hdr);
  return (s);
}

VLIB_NODE_FN (mpls_mpls_label_imposition_pipe_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (vm, node, frame, DPO_PROTO_MPLS,
					MPLS_LABEL_DPO_FLAG_NONE));
}

VLIB_REGISTER_NODE (
  mpls_mpls_label_imposition_pipe_node) = { .name =
...
VLIB_NODE_FN (ip4_mpls_label_imposition_pipe_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
VLIB_REGISTER_NODE (
  ip4_mpls_label_imposition_pipe_node) = { .name =
...
VLIB_NODE_FN (ip6_mpls_label_imposition_pipe_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
VLIB_REGISTER_NODE (
  ip6_mpls_label_imposition_pipe_node) = { .name =
...
VLIB_NODE_FN (ethernet_mpls_label_imposition_pipe_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (vm, node, frame, DPO_PROTO_ETHERNET,
					MPLS_LABEL_DPO_FLAG_NONE));
}

VLIB_REGISTER_NODE (
  ethernet_mpls_label_imposition_pipe_node) = { .name = "ethernet-mpls-label-"
							"imposition-pipe",
						.vector_size = sizeof (u32),
...
VLIB_NODE_FN (mpls_mpls_label_imposition_uniform_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (vm, node, frame, DPO_PROTO_MPLS,
...
VLIB_REGISTER_NODE (
  mpls_mpls_label_imposition_uniform_node) = { .name = "mpls-label-imposition-"
						       "uniform",
					       .vector_size = sizeof (u32),
...
VLIB_NODE_FN (ip4_mpls_label_imposition_uniform_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (vm, node, frame, DPO_PROTO_IP4,
					MPLS_LABEL_DPO_FLAG_UNIFORM_MODE));
...
VLIB_REGISTER_NODE (
  ip4_mpls_label_imposition_uniform_node) = { .name = "ip4-mpls-label-"
						      "imposition-uniform",
					      .vector_size = sizeof (u32),
...
VLIB_NODE_FN (ip6_mpls_label_imposition_uniform_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (vm, node, frame, DPO_PROTO_IP6,
					MPLS_LABEL_DPO_FLAG_UNIFORM_MODE));
...
VLIB_REGISTER_NODE (
  ip6_mpls_label_imposition_uniform_node) = { .name = "ip6-mpls-label-"
						      "imposition-uniform",
					      .vector_size = sizeof (u32),
...
VLIB_NODE_FN (ethernet_mpls_label_imposition_uniform_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (vm, node, frame, DPO_PROTO_ETHERNET,
					MPLS_LABEL_DPO_FLAG_UNIFORM_MODE));
...
VLIB_REGISTER_NODE (
  ethernet_mpls_label_imposition_uniform_node) = { .name =
						     "ethernet-mpls-label-"
						     "imposition-uniform",
						   .vector_size = sizeof (u32),
...
VLIB_NODE_FN (ip4_mpls_label_imposition_pipe_no_ip_ttl_decr_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (vm, node, frame, DPO_PROTO_IP4,
...
VLIB_REGISTER_NODE (
  ip4_mpls_label_imposition_pipe_no_ip_ttl_decr_node) = { .name =
							    "ip4-mpls-label-"
							    "imposition-pipe-"
							    "no-ip-ttl-decr",
							  .vector_size =
...
VLIB_NODE_FN (ip6_mpls_label_imposition_pipe_no_ip_ttl_decr_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (vm, node, frame, DPO_PROTO_IP6,
					MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR));
...
VLIB_REGISTER_NODE (
  ip6_mpls_label_imposition_pipe_no_ip_ttl_decr_node) = { .name =
							    "ip6-mpls-label-"
							    "imposition-pipe-"
							    "no-ip-ttl-decr",
							  .vector_size =
...
VLIB_NODE_FN (ip4_mpls_label_imposition_uniform_no_ip_ttl_decr_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (
    vm, node, frame, DPO_PROTO_IP4,
    (MPLS_LABEL_DPO_FLAG_UNIFORM_MODE | MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR)));
}

VLIB_REGISTER_NODE (
  ip4_mpls_label_imposition_uniform_no_ip_ttl_decr_node) = { .name =
							       "ip4-mpls-"
							       "label-"
							       "imposition-"
							       "uniform-no-ip-"
...
VLIB_NODE_FN (ip6_mpls_label_imposition_uniform_no_ip_ttl_decr_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (mpls_label_imposition_inline (
    vm, node, frame, DPO_PROTO_IP6,
    (MPLS_LABEL_DPO_FLAG_UNIFORM_MODE | MPLS_LABEL_DPO_FLAG_NO_IP_TTL_DECR)));
}

VLIB_REGISTER_NODE (
  ip6_mpls_label_imposition_uniform_no_ip_ttl_decr_node) = { .name =
							       "ip6-mpls-"
							       "label-"
							       "imposition-"
							       "uniform-no-ip-"
...
{
  fib_show_memory_usage ("MPLS label", pool_elts (mpls_label_dpo_pool),
 ...
 */
static void
mpls_label_interpose (const dpo_id_t *original, const dpo_id_t *parent,
		      dpo_id_t *clone)
{
  ...
  clib_memcpy_fast (&mld_clone->mld_hdr, &mld->mld_hdr,
  ...
  dpo_stack (mpls_label_dpo_types[MPLS_LABEL_DPO_FLAG_NONE],
	     mld_clone->mld_payload_proto, &mld_clone->mld_dpo, parent);

  dpo_set (clone, mpls_label_dpo_types[MPLS_LABEL_DPO_FLAG_NONE],
	   mld_clone->mld_payload_proto, mpls_label_dpo_get_index (mld_clone));
}
...
const static char *const
  *const mpls_label_imp_pipe_no_ip_tll_decr_nodes[DPO_PROTO_NUM] = {
...
const static char *const
  *const mpls_label_imp_uniform_no_ip_tll_decr_nodes[DPO_PROTO_NUM] = {
...
mpls_label_dpo_module_init (void)
{
  mpls_label_dpo_types[MPLS_LABEL_DPO_FLAG_NONE] =
    dpo_register_new_type (&mld_vft, mpls_label_imp_pipe_nodes);
