    ...
    while (n_left_from > 0)
    {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4)
        {
	      ...
	      CLIB_PREFETCH (p7->data, sizeof (hdr0[0]), STORE);
            ...
            /* atomic clone a data-plane object */
            dpo_clone(&mld_dpo0, &mld0->mld_dpo);
            dpo_clone(&mld_dpo1, &mld1->mld_dpo);
            dpo_clone(&mld_dpo2, &mld2->mld_dpo);
            dpo_clone(&mld_dpo3, &mld3->mld_dpo);
