        ...
        while (n_left_from >= 8 && n_left_to_next >= 4)
        {
            ...
            else
            {
                ...
                if (PREDICT_TRUE(vnet_buffer(b0)->mpls.first))
                {
                    ...
                    hdr0 = mpls_label_paint_w_ttl_exp(b0, mld0, ttl0,
                                                      vnet_buffer(b0)->mpls.exp);
                }
                else
                {
                    /*
                ...
                if (PREDICT_TRUE(vnet_buffer(b1)->mpls.first))
                {
                    ...
                    hdr1 = mpls_label_paint_w_ttl_exp(b1, mld1, ttl1,
                                                      vnet_buffer(b1)->mpls.exp);
                }
                else
                {
                    if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE)
                ...
                if (PREDICT_TRUE(vnet_buffer(b2)->mpls.first))
                {
                    ...
                    hdr2 = mpls_label_paint_w_ttl_exp(b2, mld2, ttl2,
                                                      vnet_buffer(b2)->mpls.exp);
                }
                else
                {
                    if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE)
                ...
                if (PREDICT_TRUE(vnet_buffer(b3)->mpls.first))
                {
                    ...
                    hdr3 = mpls_label_paint_w_ttl_exp(b3, mld3, ttl3,
                                                      vnet_buffer(b3)->mpls.exp);
                }
                else
                {
                    if (flags & MPLS_LABEL_DPO_FLAG_UNIFORM_MODE)
...
VLIB_REGISTER_NODE (ip4_mpls_label_imposition_pipe__no_ip_ttl_decrnode) = {
