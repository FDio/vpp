 ...
 */
void
receive_dpo_add_or_lock (dpo_proto_t proto, u32 sw_if_index,
			 const ip46_address_t *nh_addr, dpo_id_t *dpo)
{
  receive_dpo_t *rd;
...
{
  fib_show_memory_usage ("Receive", pool_elts (receive_dpo_pool),
			 pool_len (receive_dpo_pool), sizeof (receive_dpo_t));
}
