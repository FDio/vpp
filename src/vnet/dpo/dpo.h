 ...
 * Actions performed on packets in the data-plane can be described and
 * represented by protocol independent objects, i.e. ADJACENCY, but the
 * spceifics actions required during ADJACENCY processing can be protocol
 * dependent. For example, the adjacency rewrite node performs a ip4 checksum
 * calculation,  ip6 and MPLS do not, all 3 perform a TTL decrement. The VLIB
 * graph nodes are thus protocol dependent, and thus each graph edge/arc is
 * too. When programming a DPO's next node arc from child to parent it is thus
 * required to know the parent's data-path protocol so the correct arc index
 * can be used.
 */
...
#define DPO_PROTOS                                                            \
  {                                                                           \
    [DPO_PROTO_IP4] = "ip4", [DPO_PROTO_IP6] = "ip6",                         \
    [DPO_PROTO_ETHERNET] = "ethernet", [DPO_PROTO_MPLS] = "mpls",             \
    [DPO_PROTO_NSH] = "nsh", [DPO_PROTO_BIER] = "bier",                       \
  }

#define FOR_EACH_DPO_PROTO(_proto)                                            \
  for (_proto = DPO_PROTO_IP4; _proto <= DPO_PROTO_NSH; _proto++)
...
#define DPO_TYPES                                                             \
  {                                                                           \
    [DPO_FIRST] = "dpo-invalid", [DPO_DROP] = "dpo-drop",                     \
    [DPO_IP_NULL] = "dpo-ip-null", [DPO_PUNT] = "dpo-punt",                   \
    ...
    [DPO_RECEIVE] = "dpo-receive", [DPO_LOOKUP] = "dpo-lookup",               \
    [DPO_LOAD_BALANCE] = "dpo-load-balance",                                  \
    [DPO_REPLICATE] = "dpo-replicate", [DPO_LISP_CP] = "dpo-lisp-cp",         \
    ...
    [DPO_INTERFACE_TX] = "dpo-interface-tx", [DPO_DVR] = "dpo-dvr",           \
    [DPO_L3_PROXY] = "dpo-l3-proxy", [DPO_BIER_TABLE] = "bier-table",         \
    [DPO_BIER_FMASK] = "bier-fmask", [DPO_BIER_IMP] = "bier-imposition",      \
    [DPO_BIER_DISP_ENTRY] = "bier-disp-entry",                                \
    [DPO_BIER_DISP_TABLE] = "bier-disp-table",                                \
    [DPO_IP6_LL] = "ip6-link-local", [DPO_PW_CW] = "PW-CW",                   \
  }
...
#define DPO_INVALID                                                           \
  {                                                                           \
    .dpoi_type = DPO_FIRST, .dpoi_proto = DPO_PROTO_NONE,                     \
    .dpoi_index = INDEX_INVALID, .dpoi_next_node = 0,                         \
  }
 ...
 */
extern void dpo_mk_interpose (const dpo_id_t *original, const dpo_id_t *parent,
			      dpo_id_t *clone);
 ...
 */
extern void dpo_set (dpo_id_t *dpo, dpo_type_t type, dpo_proto_t proto,
		     index_t index);
 ...
 */
extern void dpo_stack (dpo_type_t child_type, dpo_proto_t child_proto,
		       dpo_id_t *dpo, const dpo_id_t *parent_dpo);
 ...
 *  The parent DPO to stack onto.
 */
extern void dpo_stack_from_node (u32 child_node, dpo_id_t *dpo,
 ...
 */
extern void dpo_register (dpo_type_t type, const dpo_vft_t *vft,
 ...
 */
extern u32 dpo_get_next_node_by_type_and_proto (dpo_type_t child_type,
