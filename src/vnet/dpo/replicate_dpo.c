...
#define REP_DBG(_rep, _fmt, _args...)                                         \
  {                                                                           \
    vlib_log_debug (replicate_logger, "rep:[%U]:" _fmt, format_replicate,     \
		    replicate_get_index (_rep), REPLICATE_FORMAT_NONE,        \
		    ##_args);                                                 \
  }
...
static u8 *
replicate_format (index_t repi, replicate_format_flags_t flags, u32 indent,
		  u8 *s)
{
  vlib_counter_t to;
  replicate_t *rep;
...
static replicate_t *
replicate_create_i (u32 num_buckets, dpo_proto_t rep_proto)
{
  replicate_t *rep;
  ...
  if (!REP_HAS_INLINE_BUCKETS (rep))
    {
      vec_validate_aligned (rep->rep_buckets, rep->rep_n_buckets - 1,
...
index_t
replicate_create (u32 n_buckets, dpo_proto_t rep_proto)
{
  return (replicate_get_index (replicate_create_i (n_buckets, rep_proto)));
}

static inline void
replicate_set_bucket_i (replicate_t *rep, u32 bucket, dpo_id_t *buckets,
			const dpo_id_t *next)
{
...
void
replicate_set_bucket (index_t repi, u32 bucket, const dpo_id_t *next)
{
  replicate_t *rep;
  dpo_id_t *buckets;
...
const dpo_id_t *
replicate_get_bucket (index_t repi, u32 bucket)
{
  replicate_t *rep;
 ...
 */
static void
replicate_fill_buckets (replicate_t *rep, load_balance_path_t *nhs,
...
static inline void
replicate_set_n_buckets (replicate_t *rep, u32 n_buckets)
{
			    ...
			    load_balance_path_t *next_hops)
{
  ...
  if (0 == rep->rep_n_buckets)
	...
	vec_validate_aligned (rep->rep_buckets, rep->rep_n_buckets - 1,
      ...
      replicate_fill_buckets (rep, nhs, replicate_get_buckets (rep),
			      n_buckets);
    }
  else
    {
      ...
      else if (n_buckets > rep->rep_n_buckets)
	{
	  ...
	  if (n_buckets > REP_NUM_INLINE_BUCKETS &&
	      rep->rep_n_buckets <= REP_NUM_INLINE_BUCKETS)
	    {
	      ...
	      vec_validate_aligned (rep->rep_buckets, n_buckets - 1,
				    CLIB_CACHE_LINE_BYTES);
	      ...
	      if (n_buckets <= REP_NUM_INLINE_BUCKETS)
		{
		  ...
		  replicate_fill_buckets (
		    rep, nhs, replicate_get_buckets (rep), n_buckets);
		  CLIB_MEMORY_BARRIER ();
		  replicate_set_n_buckets (rep, n_buckets);
		}
	      else
      ...
      else
	{
	  ...
	  if (n_buckets <= REP_NUM_INLINE_BUCKETS &&
	      rep->rep_n_buckets > REP_NUM_INLINE_BUCKETS)
	    {
	       ...
	       */
	      replicate_fill_buckets (rep, nhs, rep->rep_buckets_inline,
				      n_buckets);
	      CLIB_MEMORY_BARRIER ();
	      replicate_set_n_buckets (rep, n_buckets);
	      CLIB_MEMORY_BARRIER ();
...
index_t
replicate_dup (replicate_flags_t flags, index_t repi)
{
  ...
  if (rep->rep_flags == flags || flags & REPLICATE_FLAGS_HAS_LOCAL)
      ...
      if (rep->rep_n_buckets == 1)
	{
	  ...
	  replicate_set_bucket_i (copy, 0, replicate_get_buckets (copy),
...
{
  fib_show_memory_usage ("replicate", pool_elts (replicate_pool),
			 pool_len (replicate_pool), sizeof (replicate_t));
}
...
static clib_error_t *
replicate_show (vlib_main_t *vm, unformat_input_t *input,
  ...
  if (INDEX_INVALID != repi)
      ...
      pool_foreach (rep, replicate_pool)
	{
	  vlib_cli_output (vm, "%U", format_replicate,
			   replicate_get_index (rep), REPLICATE_FORMAT_NONE);
	}
    }
...
static uword
replicate_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		  vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  vlib_increment_combined_counter (
	    cm, thread_index, repi0, 1, vlib_buffer_length_in_chain (vm, b0));
...
format_replicate_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "replicate: %d via %U", t->rep_index, format_dpo_id, &t->dpo,
	      0);
  return s;
}

static uword
ip4_replicate (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static uword
ip6_replicate (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return (replicate_inline (vm, node, frame));
}
...
static uword
mpls_replicate (vlib_main_t *vm, vlib_node_runtime_t *node,
		vlib_frame_t *frame)
{
  return (replicate_inline (vm, node, frame));
}
