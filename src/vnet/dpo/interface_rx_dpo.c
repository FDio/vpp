 ...
 */
void
interface_rx_dpo_add_or_lock (dpo_proto_t proto, u32 sw_if_index,
			      dpo_id_t *dpo)
{
  interface_rx_dpo_t *ido;

  vec_validate_init_empty (interface_rx_dpo_db[proto], sw_if_index,
			   INDEX_INVALID);
...
static clib_error_t *
interface_rx_dpo_interface_state_change (vnet_main_t *vnm, u32 sw_if_index,
					 u32 flags)
{
  /*
   */
 ...
 */
static clib_error_t *
interface_rx_dpo_hw_interface_state_change (vnet_main_t *vnm, u32 hw_if_index,
					    u32 flags)
{
  return (NULL);
}
...
static clib_error_t *
interface_rx_dpo_interface_delete (vnet_main_t *vnm, u32 sw_if_index,
				   u32 is_add)
{
  return (NULL);
}
...
format_interface_rx_dpo (u8 *s, va_list *ap)
{
  ...
  return (format (s, "%U-rx-dpo: %U", format_vnet_sw_interface_name, vnm,
...
{
  fib_show_memory_usage ("Interface", pool_elts (interface_rx_dpo_pool),
...
{
  dpo_register (DPO_INTERFACE_RX, &interface_rx_dpo_vft,
...
always_inline uword
interface_rx_dpo_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			 vlib_frame_t *from_frame, u8 is_l2)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next > 2)
	{
	  ...
	  vlib_increment_combined_counter (
	    ...
	    thread_index, ido0->ido_sw_if_index, 1,
	    vlib_buffer_length_in_chain (vm, b0));
	  vlib_increment_combined_counter (
	    im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX,
	    thread_index, ido1->ido_sw_if_index, 1,
...
format_interface_rx_dpo_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U sw_if_index:%d", format_white_space, indent,
	      t->sw_if_index);
  return s;
}
