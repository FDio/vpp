...
interface_rx_dpo_module_init (void)
{
  dpo_register (DPO_INTERFACE_RX,
		&interface_rx_dpo_vft, interface_rx_dpo_nodes);
}
			 ...
			 vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* Bump the interface's RX counters */
	  vlib_increment_combined_counter (im->combined_sw_if_counters
					   + VNET_INTERFACE_COUNTER_RX,
					   thread_index,
					   ido0->ido_sw_if_index,
...
{
  .function = interface_rx_dpo_ip4,.name =
    "interface-rx-dpo-ip4",.vector_size = sizeof (u32),.format_trace =
    format_interface_rx_dpo_trace,.n_next_nodes = 2,.next_nodes =
  {
...
{
  .function = interface_rx_dpo_ip6,.name =
    "interface-rx-dpo-ip6",.vector_size = sizeof (u32),.format_trace =
    format_interface_rx_dpo_trace,.n_next_nodes = 2,.next_nodes =
  {
...
{
  .function = interface_rx_dpo_l2,.name = "interface-rx-dpo-l2",.vector_size =
    sizeof (u32),.format_trace = format_interface_rx_dpo_trace,.n_next_nodes =
    2,.next_nodes =
  {
...
VLIB_NODE_FUNCTION_MULTIARCH (interface_rx_dpo_l2_node, interface_rx_dpo_l2)
