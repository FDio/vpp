...
static void
lookup_dpo_add_or_lock_i (fib_node_index_t fib_index, dpo_proto_t proto,
			  lookup_cast_t cast, lookup_input_t input,
...
void
lookup_dpo_add_or_lock_w_table_id (u32 table_id, dpo_proto_t proto,
				   lookup_cast_t cast, lookup_input_t input,
				   lookup_table_t table_config, dpo_id_t *dpo)
{
  ...
  if (LOOKUP_TABLE_FROM_CONFIG == table_config)
      ...
      if (LOOKUP_UNICAST == cast)
	{
	  fib_index = fib_table_find_or_create_and_lock (
	    dpo_proto_to_fib (proto), table_id, FIB_SOURCE_RR);
	}
      else
	{
	  fib_index = mfib_table_find_or_create_and_lock (
	    dpo_proto_to_fib (proto), table_id, MFIB_SOURCE_RR);
	}
    }
  ...
  if (LOOKUP_TABLE_FROM_INPUT_INTERFACE == lkd->lkd_table)
    {
      s = format (s, "%s,%s lookup in interface's %U table",
		  ...
		  lookup_cast_names[lkd->lkd_cast], format_dpo_proto,
		  lkd->lkd_proto);
    }
  else
    {
      if (LOOKUP_UNICAST == lkd->lkd_cast)
	{
	  s = format (s, "%s,%s lookup in %U",
		      lookup_input_names[lkd->lkd_input],
		      lookup_cast_names[lkd->lkd_cast], format_fib_table_name,
		      lkd->lkd_fib_index, dpo_proto_to_fib (lkd->lkd_proto));
	}
      else
	{
	  s = format (s, "%s,%s lookup in %U",
		      lookup_input_names[lkd->lkd_input],
		      lookup_cast_names[lkd->lkd_cast], format_mfib_table_name,
		      lkd->lkd_fib_index, dpo_proto_to_fib (lkd->lkd_proto));
	}
    }
  return (s);
...
always_inline void
ip4_src_fib_lookup_one (u32 src_fib_index0, const ip4_address_t *addr0,
			u32 *src_adj_index0)
{
...
always_inline void
ip4_src_fib_lookup_two (u32 src_fib_index0, u32 src_fib_index1,
			const ip4_address_t *addr0, const ip4_address_t *addr1,
...
always_inline uword
lookup_dpo_ip4_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		       vlib_frame_t *from_frame, int input_src_addr,
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next > 2)
	{
	  ...
	  if (table_from_interface)
	    {
	      fib_index0 = ip4_fib_table_get_index_for_sw_if_index (
		vnet_buffer (b0)->sw_if_index[VLIB_RX]);
	      fib_index1 = ip4_fib_table_get_index_for_sw_if_index (
	  ...
	  /* do lookup */
	  ip4_src_fib_lookup_two (fib_index0, fib_index1, input_addr0,
	  ...
	  dpo1 = load_balance_get_bucket_i (
	  ...
	  vlib_increment_combined_counter (
	    cm, thread_index, lbi1, 1, vlib_buffer_length_in_chain (vm, b1));
	  ...
	  if (PREDICT_FALSE (vnet_buffer2 (b0)->loop_counter >
			     MAX_LUKPS_PER_PACKET))
	    next0 = IP_LOOKUP_NEXT_DROP;
	  if (PREDICT_FALSE (vnet_buffer2 (b1)->loop_counter >
			     MAX_LUKPS_PER_PACKET))
...
format_lookup_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U fib-index:%d addr:%U load-balance:%d", format_white_space,
	      indent, t->fib_index, format_ip46_address, &t->addr,
	      IP46_TYPE_ANY, t->lbi);
  return s;
}
...
always_inline uword
lookup_dpo_ip6_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
		       vlib_frame_t *from_frame, int input_src_addr,
		       int table_from_interface)
{
  vlib_combined_counter_main_t *cm = &load_balance_main.lbm_to_counters;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next > 2)
	{
	  ...
	  if (table_from_interface)
	    {
	      fib_index0 = ip6_fib_table_get_index_for_sw_if_index (
		vnet_buffer (b0)->sw_if_index[VLIB_RX]);
	      fib_index1 = ip6_fib_table_get_index_for_sw_if_index (
		vnet_buffer (b1)->sw_if_index[VLIB_RX]);
	    }
	  else
	    {
...
always_inline uword
lookup_dpo_mpls_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			vlib_frame_t *from_frame, int table_from_interface)
{
  u32 n_left_from, next_index, *from, *to_next;
  u32 thread_index = vlib_get_thread_index ();
  vlib_combined_counter_main_t *cm = &load_balance_main.lbm_to_counters;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (table_from_interface)
	    {
	      fib_index0 = mpls_fib_table_get_index_for_sw_if_index (
		vnet_buffer (b0)->sw_if_index[VLIB_RX]);
	    }
	  else
	    {
	  ...
	  if (MPLS_IS_REPLICATE & lbi0)
	      ...
	      if (PREDICT_FALSE (lb0->lb_n_buckets > 1))
		{
		  ...
		  dpo0 = load_balance_get_fwd_bucket (
		    lb0, (hash0 & (lb0->lb_n_buckets_minus_1)));
		}
	      else
		{
		  dpo0 = load_balance_get_bucket_i (lb0, 0);
...
format_lookup_mpls_trace (u8 *s, va_list *args)
{
  ...
  s = format (s, "%U fib-index:%d hdr:%U load-balance:%d", format_white_space,
	      indent, t->fib_index, format_mpls_header, hdr, t->lbi);
  return s;
}
...
always_inline uword
lookup_dpo_ip_dst_mcast_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
				vlib_frame_t *from_frame, int is_v4)
{
  u32 n_left_from, next_index, *from, *to_next;
...
{
  fib_show_memory_usage ("Lookup", pool_elts (lookup_dpo_pool),
			 pool_len (lookup_dpo_pool), sizeof (lookup_dpo_t));
}
...
const static char *const
  *const lookup_dst_from_interface_nodes[DPO_PROTO_NUM] = {
...
static clib_error_t *
lookup_dpo_show (vlib_main_t *vm, unformat_input_t *input,
		 vlib_cli_command_t *cmd)
{
  ...
  if (INDEX_INVALID != lkdi)
	...
	{
	  vlib_cli_output (vm, "[@%d] %U", lookup_dpo_get_index (lkd),
			   format_lookup_dpo, lookup_dpo_get_index (lkd));
	}
    }
