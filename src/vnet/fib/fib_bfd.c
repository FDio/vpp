...
/**
 * @brief Callback function registered with BFD vnet bridge to receive
 * notifications of the CRUD of BFD sessions
 * would be static but for the fact it's called from the unit-tests
 */
void
fib_bfd_notify (CLIB_UNUSED (bfd_vnet_bridge_listener_t *listener),
...
{
    /*
     * FIB subscribes to public BFD snapshots through the vnet bridge rather
     * than registering directly with the BFD implementation.
     */
    bfd_vnet_bridge_listener_t listener = {
        .fp = fib_bfd_notify,
    };

    bfd_vnet_bridge_register_listener(listener);
...
{
    .runs_after = VLIB_INITS("bfd_vnet_bridge_init", "bfd_main_init"),
};
