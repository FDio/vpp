...
static u32 fib_walk_work_nodes_visited = 2;
...
that_will_do_for_now:
     ...
     */
    bucket = ((n_elts/fib_walk_work_nodes_visited) > N_ELTS_BUCKETS ?
	      N_ELTS_BUCKETS-1 :
	      n_elts/fib_walk_work_nodes_visited);
...
#define USEC 1000000
	...
	if (0 != fib_walk_work_nodes_visited[ii])
	    s = format(s, "%d:%d ",
		       (ii * fib_walk_work_nodes_visited),
				      ...
				      vlib_cli_command_t * cmd)
{
    ...
    {
	fib_walk_work_nodes_visited = new;
    }
    else
    {
