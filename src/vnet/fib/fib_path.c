                                    ...
                                    dpo_id_t *dpo)
{
	 ...
	 */
	ai = adj_nbr_add_or_lock(path->fp_nh_proto, link, &zero_addr,
                                 path->attached_next_hop.fp_interface);
    }
    else
    {
	ai = adj_nbr_add_or_lock(path->fp_nh_proto, link,
                                 &path->attached_next_hop.fp_nh,
                                 path->attached_next_hop.fp_interface);
    }
                           ...
                           dpo_id_t *dpo)
{
                                 ...
                                 path->attached.fp_interface))
    {
        ...
        ai = adj_nbr_add_or_lock(path->fp_nh_proto,
                                 link,
                                 &zero_addr,
                                 path->attached.fp_interface);
        ...
        adj_unlock(ai);
                                       ...
                                       path->attached.fp_interface))
    {
        dpo_copy(dpo, drop_dpo_get(fib_proto_to_dpo(path->fp_nh_proto)));
    }
    else
    {
        adj_index_t ai;

        ai = adj_glean_add_or_lock(path->fp_nh_proto,
                                   link,
                                   path->attached.fp_interface,
                                   NULL);
