...
fib_entry_src_action_copy (fib_entry_t *fib_entry,
                           fib_entry_src_t *orig_src)
{
    fib_entry_src_t *esrc;

    esrc = fib_entry_src_find_or_create(fib_entry, orig_src->fes_src, NULL);
                                               ...
                                               fib_source_t source)
{
        ...
        (esrc->fes_flags & FIB_ENTRY_SRC_FLAG_INHERITED))
    {
         ...
         * Go grab the path-list from the cover, than start a walk from the
         * cover, so we don't recursively update this entry.
         */
