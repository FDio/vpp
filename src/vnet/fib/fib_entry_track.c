...
static u32
fib_entry_track_delegate_child_add (fib_node_index_t fei,
                                    ...
                                    index_t child_index)
{
    ...
    return (fib_node_child_add(FIB_NODE_TYPE_ENTRY_TRACK,
                               fib_entry_delegate_get_index(fed),
                               child_type,
                               child_index));
}
...
static void
fib_entry_track_delegate_child_remove (fib_node_index_t fei,
                                       u32 sibling)
{
    fib_entry_delegate_t *fed;
                 ...
                 u32 *sibling)
{
     ...
     */
    *sibling = fib_entry_track_delegate_child_add(fei,
                                                  child_type,
                                                  child_index);
...
fib_entry_untrack (fib_node_index_t fei,
                   u32 sibling)
{
    fib_entry_track_delegate_child_remove(fei, sibling);
}
...
fib_entry_track_last_lock_gone (fib_node_t *node)
{
     ...
     */
    fib_entry_child_remove(fed->fd_entry_index,
                           fed->fd_track.fedt_sibling);

    fib_entry_delegate_remove(fib_entry_get(fed->fd_entry_index),
                              FIB_ENTRY_DELEGATE_TRACK);

    fib_table_entry_delete_index(fed->fd_entry_index,
                                 FIB_SOURCE_RR);
}
