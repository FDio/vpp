...
#define REPLY_MSG_ID_BASE graph_msg_id_base
			 ...
			 u32 context, vlib_node_t * n, bool want_arcs)
{
  ...
  mp->_vl_msg_id = htons (VL_API_GRAPH_NODE_DETAILS + graph_msg_id_base);
  mp->context = context;
...
graph_api_hookup (vlib_main_t * vm)
{
  ...
  graph_msg_id_base = setup_message_id_table ();

  am->is_autoendian[graph_msg_id_base + VL_API_GRAPH_NODE_DETAILS] = 1;
  am->is_autoendian[graph_msg_id_base + VL_API_GRAPH_NODE_DUMP] = 1;
