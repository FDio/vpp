					    ...
					    vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	{
	  enable = 1;
	  if (unformat (line_input, "txq %u", &number_of_txqs))
	    ;
	}
  ...
  int rv =
    vnet_sw_interface_multi_txq_enable_disable (sw_if_index, number_of_txqs,
						enable);
...
VLIB_CLI_COMMAND (set_interface_feature_multi_txq_command, static) = {
  ...
  .short_help = "set interface feature multi-txq <intfc> [enable [txq <n>]| disable]",
