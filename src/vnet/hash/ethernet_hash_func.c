...
compute_txq_key (vlib_buffer_t *b, txq_ethernet_key_t *key)
{
  ...
  if (PREDICT_TRUE (ethertype == ETHERNET_TYPE_IP4))
    {
      ip4_header_t *ip4 =
	(ip4_header_t *) (vlib_buffer_get_current (b) + l2hdr_sz);
    ...
    {
      ip6_header_t *ip6 =
	(ip6_header_t *) (vlib_buffer_get_current (b) + l2hdr_sz);
      l4_hdr_offset =
...
void
vnet_hash_ethernet_func (vlib_buffer_t **b, u32 *hash, u32 n_packets)
{
  u32 n_left_from = n_packets;

  while (n_left_from >= 8)
    {
      txq_ethernet_key_t key[4] = { { 0 } };
