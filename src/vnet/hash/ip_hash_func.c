...
compute_txq_key (vlib_buffer_t *b, txq_ip_key_t *key)
{
  u16 l4_hdr_offset = 0;
  u8 l4_proto = 0;

  key->sw_if_index[VLIB_RX] = vnet_buffer (b)->sw_if_index[VLIB_RX];
  key->sw_if_index[VLIB_TX] = vnet_buffer (b)->sw_if_index[VLIB_TX];

  if ((((u8 *) vlib_buffer_get_current (b))[0] & 0xf0) == 0x40)
    {
  ...
  else if ((((u8 *) vlib_buffer_get_current (b))[0] & 0xf0) == 0x60)
    {
  ...
  if (l4_proto == IP_PROTOCOL_TCP)
    {
      tcp_header_t *tcp =
	(tcp_header_t *) (vlib_buffer_get_current (b) + l4_hdr_offset);
    ...
    {
      udp_header_t *udp =
	(udp_header_t *) (vlib_buffer_get_current (b) + l4_hdr_offset);
...
void
vnet_hash_ip_func (vlib_buffer_t **b, u32 *hash, u32 n_packets)
{
  u32 n_left_from = n_packets;

  while (n_left_from >= 8)
    {
      txq_ip_key_t key[4] = { { 0 } };
      ...
      compute_txq_key (b[0], &key[0]);
      compute_txq_key (b[1], &key[1]);
      compute_txq_key (b[2], &key[2]);
      compute_txq_key (b[3], &key[3]);
