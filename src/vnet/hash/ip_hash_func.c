...
compute_ip_key (vlib_buffer_t *b, ip_key_t *key)
{
  u16 l4_hdr_offset = 0;
  u8 l4_proto = 0;
  ...
  while (n_left_from >= 8)
    {
      ...
      // CLIB_PREFECH (p[4], LOAD);
      vlib_prefetch_buffer_data (p[4], LOAD);
      vlib_prefetch_buffer_data (p[5], LOAD);
      vlib_prefetch_buffer_data (p[6], LOAD);
      vlib_prefetch_buffer_data (p[7], LOAD);
