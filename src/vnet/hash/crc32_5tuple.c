...
compute_ip6_key (ip6_header_t *ip)
{
   ...
   * below in slow path.
   * It is not good for IP fragmented traffic. One can use hash
   * function based on 3-tuples.
   */
  if (l4hdr == 0)
    {
      if (ip6_ext_hdr (pr))
	{
	  ip6_ext_header_t *eh = ip6_next_header (ip);
	  while (ip6_ext_hdr (eh->next_hdr))
	    eh = ip6_ext_next_header (eh);
	  pr = eh->next_hdr;
	  l4hdr = *(u32 *) (eh + ip6_ext_header_len (eh)) &
