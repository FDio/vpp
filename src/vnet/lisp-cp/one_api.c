...
unformat_one_eid_api (gid_address_t * dst, u32 vni, vl_api_eid_t * eid)
{
  ...
  switch (clib_net_to_host_u32 (eid->type))
    {
...
fid_to_api_eid (fid_address_t * fid, vl_api_eid_t * eid)
{
  ...
  switch (fid_addr_type (fid))
    ...
    case FID_ADDR_IP_PREF:
      ...
      ip_prefix_encode (&fib_prefix, &eid->address.ip);
      break;
    case FID_ADDR_MAC:
  ...
  eid->type = clib_host_to_net_u32 (eid_type);
}
...
vl_api_one_add_del_map_server_t_handler (vl_api_one_add_del_map_server_t * mp)
{
  ...
  ip_address_decode (&mp->ip_address, &ip46);
  ip_address_from_46 (&addr, &ip46);
					   ...
					   * mp)
{
  ...
  ip_address_decode (&mp->ip_address, &ip46);
  ip_address_from_46 (&a->address, &ip46);
...
vl_api_one_enable_disable_t_handler (vl_api_one_enable_disable_t * mp)
{
  ...
  vnet_lisp_enable_disable (mp->is_enabled);
  REPLY_MACRO (VL_API_ONE_ENABLE_DISABLE_REPLY);
}
...
vl_api_show_one_use_petr_t_handler (vl_api_show_one_use_petr_t * mp)
{
  ...
  REPLY_MACRO2 (VL_API_SHOW_ONE_USE_PETR_REPLY,
  {
	...
	ip_address_to_46 (ip, &ip46, &fib_proto);
        ip_address_encode (&ip46, IP46_TYPE_IP4, &rmp->ip_address);
        break;

      case AF_IP6:
        fib_proto = FIB_PROTOCOL_IP6;
        ip_address_to_46 (ip, &ip46, &fib_proto);
        ip_address_encode (&ip46, IP46_TYPE_IP6, &rmp->ip_address);
        break;
			  ...
			  u32 context)
{
  ...
  if (loc->local)
  ...
  else
    {
      ip46_address_t ip46;
      fib_protocol_t fib_proto =
	gid_address_ip_version (&loc->address) ==
	AF_IP4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6;
      ip_address_to_46 (&gid_address_ip (&loc->address), &ip46, &fib_proto);
      ip_address_encode (&ip46, IP46_TYPE_ANY, &rmp->ip_address);
    }
			    ...
			    u8 filter)
{
  ...
  switch (gid_address_type (gid))
    ...
    case GID_ADDR_IP_PREFIX:
      eid_type = EID_TYPE_API_PREFIX;
      rmp->seid.type = clib_host_to_net_u32 (eid_type);
      ip_prefix_to_fib_prefix (&gid_address_ippref (gid), &fib_prefix);
      ip_prefix_encode (&fib_prefix, &rmp->seid.address.ip);
      break;
    case GID_ADDR_MAC:
      eid_type = EID_TYPE_API_MAC;
      rmp->seid.type = clib_host_to_net_u32 (eid_type);
			     ...
			     u32 context)
{
  ...
  fib_proto =
    ip_addr_version (ip) == AF_IP4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6;
  ip_address_to_46 (ip, &ip46, &fib_proto);
  ip_address_encode (&ip46, IP46_TYPE_ANY, &rmp->ip_address);
  ...
  for (i = 0; i < n; i++)
	...
	case GID_ADDR_IP_PREFIX:
	  ...
	  ip_prefix_to_fib_prefix (&gid_address_ippref (&adj->leid),
				   &lfib_prefix);
	  ip_prefix_encode (&rfib_prefix, &a.reid.address.ip);
	  ip_prefix_encode (&lfib_prefix, &a.leid.address.ip);
	  break;
	case GID_ADDR_MAC:
	  eid_type = EID_TYPE_API_PREFIX;
	...
	default:
      ...
      a.reid.type = clib_host_to_net_u32 (eid_type);
      a.leid.type = clib_host_to_net_u32 (eid_type);
      dst[i] = a;
    }
}
  ...
  (vl_api_show_one_stats_enable_disable_t * mp)
{
  ...
  ({
    rmp->is_enabled = vnet_lisp_stats_enable_disable_state ();
  }));
  /* *INDENT-ON* */
}
  ...
  (vl_api_one_stats_enable_disable_t * mp)
{
  ...
  vnet_api_error_t rv = vnet_lisp_stats_enable_disable (mp->is_enabled);
  REPLY_MACRO (VL_API_ONE_ENABLE_DISABLE_REPLY);
}
  ...
  (vl_api_one_enable_disable_xtr_mode_t * mp)
{
  ...
  int rv = vnet_lisp_enable_disable_xtr_mode (mp->is_enabled);
...
vl_api_one_show_xtr_mode_t_handler (vl_api_one_show_xtr_mode_t * mp)
{
  ...
  {
    rmp->is_enabled = vnet_lisp_get_xtr_mode ();
  });
  /* *INDENT-ON* */
}
  ...
  (vl_api_one_enable_disable_pitr_mode_t * mp)
{
  ...
  int rv = vnet_lisp_enable_disable_pitr_mode (mp->is_enabled);
...
vl_api_one_show_pitr_mode_t_handler (vl_api_one_show_pitr_mode_t * mp)
{
  ...
  {
    rmp->is_enabled = vnet_lisp_get_pitr_mode ();
  });
  /* *INDENT-ON* */
}
  ...
  (vl_api_one_enable_disable_petr_mode_t * mp)
{
  ...
  int rv = vnet_lisp_enable_disable_petr_mode (mp->is_enabled);
...
vl_api_one_show_petr_mode_t_handler (vl_api_one_show_petr_mode_t * mp)
{
  ...
  {
    rmp->is_enabled = vnet_lisp_get_petr_mode ();
  });
  /* *INDENT-ON* */
}
