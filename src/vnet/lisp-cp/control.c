...
static void
remove_adjacencies_by_map_index (lisp_cp_main_t * lcm, u8 is_local,
				 u32 mapping_index)
{
  fwd_entry_t *fwd;
  mapping_t *map;
...
static void
remove_fwd_entries_by_locator_set (lisp_cp_main_t * lcm, u8 is_local,
				   u32 ls_index)
{
  u32 i, *map_indexp;
  u32 **eid_indexes;
  ...
  for (i = 0; i < vec_len (eid_indexes[0]); i++)
    {
      map_indexp = vec_elt_at_index (eid_indexes[0], i);
      remove_adjacencies_by_map_index (lcm, is_local, map_indexp[0]);
    }
}
			   ...
			   locator_set_t * ls, u32 * ls_result)
{
  ...
  if (a->is_add)
	...
	vec_foreach (locit, ls->locator_indices)
	{
      ...
      /* remove fwd entries using this locator set from DP */
      if (vec_len (ls->locator_indices) == 0)
	remove_fwd_entries_by_locator_set (lcm, loc->local, ls_index);
    }
