...
static vnet_crypto_key_type_t
lisp_key_type_to_crypto_key_type (lisp_key_type_t key_id)
{
  switch (key_id)
    {
    case HMAC_SHA_1_96:
      return VNET_CRYPTO_KEY_TYPE_HMAC_SHA1;
    case HMAC_SHA_256_128:
      return VNET_CRYPTO_KEY_TYPE_HMAC_SHA256;
    default:
      clib_warning ("unsupported encryption key type: %d!", key_id);
      break;
    }
  return VNET_CRYPTO_KEY_TYPE_NONE;
}
			       ...
			       u16 auth_data_len, u32 msg_len)
{
  ...
  ki = vnet_crypto_key_add (lcm->vlib_main,
			    lisp_key_type_to_crypto_key_type (key_id),
			    key, vec_len (key));
