  ...
  if (LISP_AFI_IP == type || LISP_AFI_IP6 == type)
    {
      ...
      /* if v6 check whether it's neighbor discovery */
      if (LISP_AFI_IP6 == type)
	{
	  if (IP_PROTOCOL_ICMP6 == ip0->protocol)
	    {
	      ...
	      if (ndh->icmp.type == ICMP6_neighbor_solicitation)
		{
		  ...
		  ip_address_set (&gid_address_ndp_ip (dst),
      ...
      get_src_and_dst_ip (ip0, &gid_address_ip (src), &gid_address_ip (dst));
