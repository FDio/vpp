...
static int
update_pending_map_register (pending_map_register_t * r, f64 dt)
{
  lisp_cp_main_t *lcm = vnet_lisp_cp_get_main ();
  lisp_msmr_t *ms;
  ...
  if (r->time_to_expire < 0)
      ...
      else
	{
	  ...
	  /* when an expiration occurs delete all pending map registers */
	  return 0;
	}
    }
  return 1;
...
update_map_register (lisp_cp_main_t * lcm, f64 dt)
{
  ...
  ({
    if (!update_pending_map_register (pmr, dt))
    {
      del_all = 1;
      break;
    }
