...
//udp_connection_t * udp_half_open_get (u32 conn_index);
...
udp_pool_add_peeker (u32 thread_index)
{
  if (thread_index != vlib_get_thread_index ())
    __sync_fetch_and_add (&udp_main.connection_peekers[thread_index], 1);
  ...
  if (thread_index != vlib_get_thread_index ())
    {
      ...
      __sync_fetch_and_sub (&udp_main.connection_peekers[thread_index], 1);
    }
}
...
udp_conenction_clone_safe (u32 connection_index, u32 thread_index)
{
  ...
  old_c = udp_connection_get (connection_index, thread_index);
