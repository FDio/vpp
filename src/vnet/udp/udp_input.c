		    ...
		    vlib_frame_t * frame, u8 is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  udp_connection_t *child0, *new_uc0, *uc0;
	  int written0;
	  ...
	  if (is_ip4)
	    {
	      ...
	      s0 = session_lookup4 (fib_index0, &ip40->dst_address,
	  ...
	  else
	    {
	      ...
	      s0 = session_lookup6 (fib_index0, &ip60->dst_address,
	  ...
	  if (PREDICT_TRUE(s0->session_state == SESSION_STATE_READY))
	  ...
	  else if (s0->session_state == SESSION_STATE_CONNECTING_READY)
	    {
	      ...
	      pool_get(um->connections[my_thread_index], new_uc0);
	      clib_memcpy (new_uc0, uc0, sizeof(*new_uc0));
	      new_uc0->c_c_index = new_uc0 - um->connections[my_thread_index];
	      session_dgram_connect_notify (&new_uc0->connection);
	    }
	  else if (s0->session_state == SESSION_STATE_LISTENING_READY)
	    {
	      tc0 = listen_session_get_transport (s0);

	      pool_get(um->connections[my_thread_index], child0);
	      clib_memcpy (child0, tc0, sizeof(*child0));
	      ...
	      child0->c_c_index = child0 - um->connections[my_thread_index];
	      ...
	      if (stream_session_accept (&child0->connection, tc0->s_index, 1))
		{
	      ...
	      s0 = session_get (tc0->s_index, tc0->thread_index);
