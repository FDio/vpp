                    ...
                    vlib_frame_t * frame, u8 is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_ip4)
	    {
	      ...
	      tc0 = session_lookup_connection_wt4 (fib_index0,
		                                   &ip40->dst_address,
		                                   &ip40->src_address,
		                                   udp0->dst_port,
		                                   udp0->src_port,
	  ...
	  else
	    {
	      ...
	      tc0 = session_lookup_connection_wt6 (fib_index0,
		                                   &ip60->dst_address,
		                                   &ip60->src_address,
		                                   udp0->dst_port,
		                                   udp0->src_port,
	  ...
	  state0 = session_state_for_dgram_connection (tc0);
	  if (PREDICT_TRUE (state0 == SESSION_STATE_READY))
	    {
	      written0 = session_enqueue_dgram_connection (tc0, b0,
		                                           1 /* queue event */);
	  ...
	  else if (state0 == SESSION_STATE_LISTENING)
	    {
	      ...
	      if (stream_session_accept (&child0->connection, tc0->s_index, 1))
		{
	      ...
	      written0 = session_enqueue_dgram_connection (&child0->connection, b0,
		                                           1 /* queue event */);
	      if (PREDICT_FALSE (written0 < 0))
		{
		  error0 = UDP_ERROR_FIFO_FULL;
