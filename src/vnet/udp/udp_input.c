		       ...
		       u32 thread_index, session_t ** s)
{
  ...
  if (hdr->is_ip4)
    {
      ip_set (&uc->c_lcl_ip, &hdr->lcl_ip, 1);
      ip_set (&uc->c_rmt_ip, &hdr->rmt_ip, 1);
    }
  else
    {
      ip_set (&uc->c_lcl_ip, &hdr->lcl_ip, 0);
      ip_set (&uc->c_rmt_ip, &hdr->rmt_ip, 0);
    }
  ...
  if (session_stream_accept (&uc->connection, listener->c_s_index,
			     listener->c_thread_index, 1))
    {
      udp_connection_free (uc);
      return 0;
    }
  s[0] = session_get (uc->c_s_index, uc->c_thread_index);
  s[0]->session_state = SESSION_STATE_READY;
		    ...
		    vlib_frame_t * frame, u8 is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      if (s0->session_state == SESSION_STATE_OPENED)
	{
	  ...
	  if (uc0->flags & UDP_CONN_F_CONNECTED)
	      ...
	      if (s0->thread_index != thread_index)
		{
		  ...
		  /*
		   * Drop the peeker lock on pool resize. We won't try to
		   * drop the lock again lower because s0 is reallocated in
		   * the connect notify on the current thread
		   */
      ...
      else if (s0->session_state == SESSION_STATE_LISTENING)
	{
	  ...
	  if (uc0->flags & UDP_CONN_F_CONNECTED)
	    {
	      uc0 = udp_connection_accept (uc0, &hdr0, thread_index, &s0);
	      if (!uc0)
		{
