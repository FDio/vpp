...
static void
udp_register_connection_port (vlib_main_t * vm, u16 lcl_port, u8 is_ip4)
{
  ...
  if (!pi)
    {
      clib_warning ("adding port %u", lcl_port);
    ...
    {
      clib_warning ("inc connections for %u", lcl_port);
      pi->n_connections += 1;
    }
...
static void
udp_unregister_connection_port (u16 dst_port, u8 is_ip4)
{
  udp_main_t *um = &udp_main;
  udp_dst_port_info_t *pi;
  ...
  if (!pi->n_connections)
    {
      clib_warning ("no connections using port %u", dst_port);
      return;
    }
  ...
  if (!pi->n_connections)
    {
      clib_warning ("removing udp port %u", dst_port);
      udp_unregister_dst_port (0, dst_port, is_ip4);
    }
}

void
udp_port_connections_inc (u16 lcl_port, u8 is_ip4)
{
  udp_main_t *um = &udp_main;
  udp_dst_port_info_t *pi;
...
{
  udp_unregister_connection_port (clib_net_to_host_u16 (uc->c_lcl_port),
				  uc->c_is_ip4);
...
udp_session_bind (u32 session_index, transport_endpoint_t * lcl)
{
  ...
  udp_register_connection_port (vm, clib_net_to_host_u16 (lcl->port),
				lcl->is_ip4);
...
//  vlib_main_t *vm = vlib_get_main ();
  ...
  clib_warning ("unregister %u", clib_net_to_host_u16 (listener->c_lcl_port));
  udp_unregister_connection_port (clib_net_to_host_u16 (listener->c_lcl_port),
				  listener->c_is_ip4);
...
//  udp_main_t *um = vnet_get_udp_main ();
  ...
  clib_warning ("connect with local port %u",
		clib_net_to_host_u16 (rmt->peer.port));
  if (transport_alloc_local_endpoint
      (TRANSPORT_PROTO_UDP, rmt, &lcl_addr, &lcl_port))
    return -1;
  ...
  if (udp_is_valid_dst_port (lcl_port, rmt->is_ip4))
    {
      if (rmt->peer.port)
	{
	  clib_warning ("conflict");
	  return -1;
	}
      while (udp_is_valid_dst_port (lcl_port, rmt->is_ip4))
	{
	  clib_warning ("looking for a different port %u", lcl_port);
  ...
  udp_register_connection_port (vm, lcl_port, rmt->is_ip4);
...
udp_init (vlib_main_t * vm)
{
  ...
  um->local_to_input_edge[UDP_IP4] = vlib_node_add_next (vm,
							 udp4_local_node.index,
							 udp4_input_node.index);
