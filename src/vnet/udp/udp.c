...
udp_connection_alloc (u32 thread_index)
{
  ...
  if (PREDICT_FALSE (will_expand && um->connection_peekers[thread_index]))
    while (um->connections[thread_index])
      ;
...
//    uc = pool_elt_at_index (udp_main.connections[0], conn_index);
  ...
  if (uc)
    {
      ...
      pool_put_index(um->connections[thread_index], connection_index);
    }
...
void
udp_session_cleanup (u32 connection_index, u32 my_thread_index)
{
  udp_main_t *um = vnet_get_udp_main ();
