...
/**
 * @return on success the index, otherwise return -1
 */
static_always_inline i8
gro_flow_table_add_flow (gro_flow_table_t * flow_table)
{
  if (flow_table->flow_table_size < GRO_FLOW_TABLE_MAX_SIZE)
    {
      vec_validate (flow_table->gro_flow, flow_table->flow_table_size);
      flow_table->flow_table_size++;
      return flow_table->flow_table_size;
    }
				 ...
				 gro_flow_key_t * flow_key)
{
  ...
  i8 i = gro_flow_table_add_flow (flow_table);
  if (i >= 0)
    {
      gro_flow = vec_elt_at_index (flow_table->gro_flow, i);
...
static_always_inline void
gro_flow_reset (gro_flow_t * gro_flow)
{
		      ...
		      vlib_buffer_t * b1)
{
  ...
  if (gro_tcp_sequence_check (tcp0, tcp1, payload_len0))
    {
      if (gro_merge_buffers (vm, b0, b1, payload_len1, l234_sz1))
