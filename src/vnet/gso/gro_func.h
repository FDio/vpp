			   ...
			   u32 * to)
{
  if (flow_table->flow_table_size > 0)
	...
	if (gro_flow->next_timeout_ts && gro_flow_is_timeout (gro_flow))
	  ...
	  {
	    // flush the packet
	    vlib_buffer_t *b0 = vlib_get_buffer (vm, gro_flow->buffers);
	    gro_fixup_header (vm, b0, gro_flow->last_ack_number);
	    to[i] = gro_flow->buffers;
						 ...
						 flow_table)
{
  ...
  f->n_vectors = vnet_gro_flow_table_flush (vm, flow_table, to);
			    ...
			    u32 bi0)
{
  ...
  tcp0 = gro_get_packet_data (vm, b0, &gho0, &flow_key0, &pkt_len0);
  if (pkt_len0 == 0)
    return bi0;
  ...
  if (gro_flow->n_buffers == 0)
    {
      ...
      gro_flow_set_timeout (gro_flow, 3e-6);
      return 0;
    }
