			   ...
			   u32 * to)
{
  if (flow_table->flow_table_size > 0)
      ...
      while (i < GRO_FLOW_TABLE_MAX_SIZE)
	{
	  ...
	  if (gro_flow->n_buffers && gro_flow_is_timeout (gro_flow))
	    {
	      // flush the packet
	      vlib_buffer_t *b0 =
		vlib_get_buffer (vm, gro_flow->buffer_index);
	      ...
	      gro_fixup_header (vm, b0, gro_flow->last_ack_number,
				flow_table->is_l2, is_ip4, is_ip6);
			    ...
			    u32 bi0)
{
  ...
  pkt_len0 =
    gro_get_packet_data (vm, b0, &gho0, &flow_key0, is_l2, is_ip4, is_ip6);
  if (pkt_len0 == 0)
    return bi0;
  ...
  if (PREDICT_FALSE (gro_flow->n_buffers == 0))
	...
	(tcp_header_t *) (vlib_buffer_get_current (b0) + gho0.l4_hdr_offset);
      ...
      vnet_generic_header_offset_parser (b_s, &gho_s, is_l2, is_ip4, is_ip6);
      tcp_s =
      ...
      else if (action == GRO_PACKET_ACTION_FLUSH)
	{
	  ...
	  flow_table->n_vectors++;
	  flow_table->total_vectors++;
	  gro_fixup_header (vm, b_s, gro_flow->last_ack_number, is_l2, is_ip4,
			    is_ip6);
	  gro_flow->n_buffers = 0;
	  gro_flow_store_packet (gro_flow, bi0);
	  gro_flow->last_ack_number = tcp0->ack_number;
			...
			int is_l2)
{
  ...
  if (PREDICT_TRUE (((b[0]->flags & VNET_BUFFER_F_GSO) == 0)))
      ...
      while (n_left_from > 1)
	{
      ...
      if (bi >= 2)
	{
	  int is_ip4 = b[0]->flags & VNET_BUFFER_F_IS_IP4;
	  int is_ip6 = b[0]->flags & VNET_BUFFER_F_IS_IP6;
	  gro_fixup_header (vm, b[0], ack_number, is_l2, is_ip4, is_ip6);
	}
    }
  return bi;
}
