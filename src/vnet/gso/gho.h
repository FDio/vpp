...
vnet_get_inner_header_inline (vlib_buffer_t * b0,
			      generic_header_offset_t * gho)
{
  if ((gho->gho_flags & GHO_F_TUNNEL)
      && (gho->gho_flags &
	  (GHO_F_OUTER_IP4 | GHO_F_OUTER_IP6 | GHO_F_OUTER_TCP |
	   GHO_F_OUTER_UDP))
...
static_always_inline void
vnet_get_outer_header_inline (vlib_buffer_t * b0,
			      generic_header_offset_t * gho)
{
  if ((gho->gho_flags & GHO_F_TUNNEL)
      && (gho->gho_flags &
	  (GHO_F_OUTER_IP4 | GHO_F_OUTER_IP6 | GHO_F_OUTER_TCP |
	   GHO_F_OUTER_UDP)) && (b0->current_data == gho->l2_hdr_offset))
    vlib_buffer_advance (b0, -gho->outer_hdr_sz);
}
				       ...
				       generic_header_offset_t * gho)
{
  ...
  gho->gho_flags &= ~(GHO_F_IP4 | GHO_F_IP6 | GHO_F_UDP | GHO_F_TCP);

  vnet_get_inner_header_inline (b0, gho);
