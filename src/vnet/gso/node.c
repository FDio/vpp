				...
				generic_header_offset_t * gho)
{
  ...
  vnet_get_outer_header_inline (sb0, gho);

  while (i < n_tx_bufs)
    {
      vlib_buffer_t *b0 = vlib_get_buffer (vm, ptd->split_buffers[i]);
      vnet_get_outer_header_inline (b0, gho);
...
tso_alloc_tx_bufs (vlib_main_t * vm,
		   ...
		   u16 gso_size, generic_header_offset_t * gho)
{

  /*
   * l2_hdr_offset is an offset within vlib_buffer_t data or pre_data.
  ...
  //if (gso_size < b0->current_length - l234_sz)
  //  n_bufs++;
		    ...
		    u32 do_csums)
{
  ...
  u16 l234_sz = gho->hdr_sz;	// may be - gho->l2_hdr_offset;
	...
	while (from + 8 <= from_end && n_left_to_next >= 4)
	  {
		...
		t0->gho = vnet_generic_header_offset_parser (b[0]);
	      }
		...
		t1->gho = vnet_generic_header_offset_parser (b[1]);
	      }
		...
		t2->gho = vnet_generic_header_offset_parser (b[2]);
	      }
		...
		t3->gho = vnet_generic_header_offset_parser (b[3]);
	      }
	  ...
	  if (do_segmentation0)
	      ...
	      if (PREDICT_FALSE (b[0]->flags & VNET_BUFFER_F_GSO))
		{
		  ...
		  if (PREDICT_FALSE (gho.gho_flags & GHO_F_TUNNEL))
		    {
		      vnet_get_inner_header_inline (b[0], &gho);

		      n_bytes_b0 -= gho.outer_hdr_sz;
		      /*
		       * In case of tunnel encapsulated packet, we will calculate the checksums
		  ...
		  if (PREDICT_FALSE (gho.gho_flags & GHO_F_VXLAN_TUNNEL))
		    {
		      vnet_get_outer_header_inline (b[0], &gho);
		      n_tx_bytes +=
			tso_segment_vxlan_tunnel_fixup (vm, ptd, b[0], &gho);
		    }
		  else if (PREDICT_FALSE (gho.gho_flags & GHO_F_IPIP_TUNNEL))
		    {
		      /* not supported yet */
		      vlib_buffer_free (vm, ptd->split_buffers,
					vec_len (ptd->split_buffers));
		      drop_one_buffer_and_count (vm, vnm, node, from - 1,
						 hi->sw_if_index,
		  ...
		  else if (PREDICT_FALSE (gho.gho_flags & GHO_F_GRE_TUNNEL))
		    {
		      /* not supported yet */
		      vlib_buffer_free (vm, ptd->split_buffers,
					vec_len (ptd->split_buffers));
		    ...
		    if (PREDICT_FALSE (gho.gho_flags & GHO_F_GENEVE_TUNNEL))
		    {
		      /* not supported yet */
		      vlib_buffer_free (vm, ptd->split_buffers,
					vec_len (ptd->split_buffers));
