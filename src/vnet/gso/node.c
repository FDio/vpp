...
tso_segment_buffer (vlib_main_t * vm, vnet_interface_per_thread_data_t * ptd,
		    u32 sbi0, vlib_buffer_t * sb0, u32 n_bytes_b0)
{
  u32 n_tx_bytes = 0;
...
vnet_gso_node_inline (vlib_main_t * vm,
		      ...
		      vlib_frame_t * frame,
		      vnet_main_t * vnm,
		      vnet_hw_interface_t * hi, int do_segmentation)
{
  u32 *to_next;
...
VLIB_NODE_FN (gso_l2_nonip_node) (vlib_main_t * vm,
				  vlib_node_runtime_t * node,
				  vlib_frame_t * frame)
{
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (gso_l2_nonip_node) = {
...
VNET_FEATURE_INIT (gso_l2_nonip_node, static) = {
  .arc_name = "l2-output-nonip",
  .node_name = "gso-l2-nonip",
