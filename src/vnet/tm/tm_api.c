...
vl_api_tm_sys_node_add_t_handler (vl_api_tm_sys_node_add_t *mp)
{
  ...
  rv = tm_sys_node_add (sw->hw_if_index, node_id, parent_node_id, priority, weight, lvl, &n_p,
			flow_name);

  REPLY_MACRO2 (VL_API_TM_SYS_NODE_ADD_REPLY, ({ rmp->node_id = clib_host_to_net_u32 (node_id); }));
}
