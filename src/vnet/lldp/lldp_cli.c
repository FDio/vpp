	       ...
	       vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "if_index %d", &hw_if_index))
	;
      if (unformat
	  (input, "%U", unformat_vnet_sw_interface, vnm, &hw_if_index))
	;
  ...
  return lldp_cfg_err_to_clib_err (lldp_cfg_intf_set (hw_if_index,
						      &port_desc, enable));
}
...
VLIB_CLI_COMMAND(set_interface_lldp_cmd, static) = {
  ...
  .short_help = "set interface lldp <interface> | if_index <idx>"
