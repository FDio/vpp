			  ...
			  u32 if_index, u8 oid_len, u8 * oid)
{
  ...
  _clib_memcpy (&(t->v[2]), addr, addr_len);	/* address */
  ...
  if (oid_len > 0)
    _clib_memcpy ((u8 *) & (t->v[addr_len + 8]), oid, oid_len);
...
lldp_add_chassis_id (const vnet_hw_interface_t * hw, u8 ** t0p)
{
  ...
  _clib_memcpy (&t->id, hw->hw_address, addr_len);
  const size_t len =
...
lldp_add_port_id (const vnet_hw_interface_t * hw, u8 ** t0p)
{
  ...
  _clib_memcpy (&t->id, hw->name, name_len);
  ...
  if (len)
    {
      ...
      _clib_memcpy (t->v, n->port_desc, len);
      *t0p += STRUCT_SIZE_OF (lldp_tlv_t, head) + len;
    }
}
  ...
  if (len)
    {
      ...
      lldp_tlv_set_length (t, len);
      _clib_memcpy (t->v, lm->sys_name, len);
      *t0p += STRUCT_SIZE_OF (lldp_tlv_t, head) + len;
    }
}
...
lldp_send_ethernet (lldp_main_t * lm, lldp_intf_t * n, int shutdown)
{
  ...
  _clib_memcpy (h0->src_address, hw->hw_address, vec_len (hw->hw_address));
