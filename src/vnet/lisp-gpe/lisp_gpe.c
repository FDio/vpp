...
static uword
lisp_gpe_nsh_dummy (vlib_main_t * vm, vlib_node_runtime_t * node,
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (lisp_gpe_nsh_dummy_node) = {
  .function = lisp_gpe_nsh_dummy,
  .name = "lisp-gpe-nsh-dummy",
...
static clib_error_t *
lisp_gpe_test_add_dummy_node (void)
{
  lisp_gpe_main_t *lgm = vnet_lisp_gpe_get_main ();
  vlib_node_add_next (lgm->vlib_main, lisp_gpe_ip4_input_node.index,
		      lisp_gpe_nsh_dummy_node.index);
  return 0;
}
			  ...
			  vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      if (unformat (input, "pcap %v", &file_name))
	{
      ...
      else if (unformat (input, "add-dummy-decap-node"))
	{
	  error = lisp_gpe_test_add_dummy_node ();
	  goto done;
	}
      else
	{
