		     ...
		     vlib_frame_t * frame)
{
  ...
  for (i = 0; i < vec_len (connections_this_batch); i++)
      ...
      if (PREDICT_FALSE (delete_session == 1))
	{
	  ...
	  __atomic_fetch_add (&ecm->tx_total, sp->bytes_sent, __ATOMIC_RELAXED);
	  __atomic_fetch_add (&ecm->rx_total, sp->bytes_received, __ATOMIC_RELAXED);
	  ...
	  if (s)
	    {
	      ...
	      i--;
	      __atomic_fetch_add (&ecm->ready_connections, -1, __ATOMIC_RELEASE);
	    }
	  else
	    {
					 ...
					 stream_session_t * s, u8 is_fail)
{
  ...
  __atomic_fetch_add (&ecm->ready_connections, 1, __ATOMIC_ACQUIRE);
