					 ...
					 session_t * s, u8 is_fail)
{
  ...
  DBG ("QUIC Connection callback %d", api_context);
					 ...
					 session_t * s, u8 is_fail)
{
  ...
  if (api_context == 0)
    return quic_echo_clients_qsession_connected_callback (app_index, api_context, s, is_fail);
					 ...
					 session_t * s, u8 is_fail)
{
  ...
  if (is_fail)
    {
      clib_warning ("connection failed!");
      ecm->run_test = ECHO_CLIENTS_EXITING;
      signal_evt_to_cli (-1);
      return 0;
    }
