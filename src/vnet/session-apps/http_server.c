...
/*
 * TLS server cert/key copied not to depend on mbedtls
 */
...
server_attach ()
{
  ...
  vec_validate (a_cert->cert, sizeof (test_srv_crt_rsa));
  clib_memcpy (a_cert->cert, test_srv_crt_rsa, sizeof (test_srv_crt_rsa));
  vnet_app_add_tls_cert (a_cert);

  memset (a_key, 0, sizeof (*a_key));
  a_key->app_index = a->app_index;
  vec_validate (a_key->key, sizeof (test_srv_key_rsa));
  clib_memcpy (a_key->key, test_srv_key_rsa, sizeof (test_srv_key_rsa));
