...
static http_session_t *
http_server_session_get (u32 hs_index, u32 thread_index)
{
  http_server_main_t *hsm = &http_server_main;
		  ...
		  vlib_frame_t * f)
{
  ...
  /* XXX TODO this will convert into a dangling reference whenever
   * the http session pool is reallocated. To avoid it, we need some form
   * of locking but that's an exercise for another time. For heavy http
   * testing just use static sessions as they don't rely on a process */
  hs = http_server_session_get (args->hs_index, args->thread_index);
  ASSERT (hs);

  request = (u8 *) (void *) (hs->rx_buf);
...
found_start:
  ...
  if (request[request_len - 1] != 0xa || request[request_len - 3] != 0xa
    ...
    {
    postpone:
      if (svm_fifo_set_event (hs->rx_fifo))
