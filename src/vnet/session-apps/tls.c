...
tls_ctx_half_open_alloc (void)
{
  ...
  memset (ctx, 0, sizeof (*ctx));
  return ctx - tm->half_open_ctx_pool;
}

void
tls_ctx_half_open_free (tls_ctx_t *ctx)
{
  pool_put (tls_main.half_open_ctx_pool, ctx);
}
...
{
  return pool_elt_at_index (tls_main.half_open_ctx_pool, ctx_index);
}
...
tls_ctx_init_client (tls_ctx_t *ctx)
{
   ...
   */
  TLS_DBG(1, "Initiating handshake for %u", tls_ctx_index (ctx));
...
tls_ctx_init_server (tls_ctx_t *ctx)
{
  ...
  */
  ...
  /*
   * 4. Start handshake state machine
   */
...
tls_notify_app_connected (tls_ctx_t *ctx)
{
  ...
  app_session->session_type = session_type_from_proto_and_ip (
      TRANSPORT_PROTO_TLS, ctx->tcp_is_ip4);
...
tls_handshake_rx (tls_ctx_t *ctx)
{
  ...
  TLS_DBG(1, "Handshake for %u complete. TLS cipher is %x", tls_ctx_index (ctx),
	  ctx->ssl.session->ciphersuite);
  return 0;
}
                                ...
                                u8 is_fail)
{
  ...
  tls_ctx_half_open_free (ho_ctx);

  TLS_DBG(1, "TCP connect for %u returned %u. New index %u",
          ho_ctx_index, is_fail, (ctx) ? ctx_index : ~0);
...
tls_connect (transport_endpoint_t *tep)
{
  ...
  TLS_DBG(1, "New connect request %u", tls_ctx_half_open_index (ctx));
  return 0;
}
...
format_tls_listener (u8 * s, va_list * args)
{
  ...
  listen_session_parse_handle (ctx->tls_session_handle, &type, &listener_index);
...
format_tls_half_open (u8 * s, va_list * args)
{
  ...
  return format (s, "[TLS] half-open app %u", ctx->parent_app_index);
}
