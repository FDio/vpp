...
void
tls_session_disconnect_callback (stream_session_t * app_session)
{
  application_t *app = application_get (app_session->app_index);
  app->cb_fns.session_disconnect_callback (app_session);
}

int
tls_session_accept_callback (stream_session_t * s)
{
  return 0;
}
...
static int
mbedtls_net_send (void *ctx_indexp, const unsigned char *buf, size_t len)
{
  stream_session_t *tls_session;
  uword ctx_index;
  tls_ctx_t *ctx;
  ...
  tls_session = session_get_from_handle (ctx->tls_session_handle);
  return svm_fifo_enqueue_nowait (tls_session->server_tx_fifo, len, buf);
}

static int
mbedtls_net_recv (void *ctx_indexp, unsigned char *buf, size_t len)
{
  stream_session_t *tls_session;
  uword ctx_index;
  tls_ctx_t *ctx;
  ...
  return svm_fifo_dequeue_nowait (tls_session->server_rx_fifo, len, buf);
}

static void
mbedtls_debug (void *ctx, int level, const char *file, int line, const char *str)
{
  clib_warning("%s:%04d: %s", file, line, str);
}

static int
tls_ctx_init (tls_ctx_t *ctx)
{
  ...
  if ((rv = mbedtls_ssl_config_defaults (&ctx->conf, MBEDTLS_SSL_IS_CLIENT,
    ...
    {
      clib_warning("mbedtls_ssl_config_defaults returned %d\n\n", rv);
      return -1;
    }
  ...
  if ((rv = mbedtls_ssl_setup (&ctx->ssl, &ctx->conf)) != 0)
    {
      clib_warning("mbedtls_ssl_setup returned %d\n", rv);
      return -1;
    }

  if ((rv = mbedtls_ssl_set_hostname (&ctx->ssl, "SERVER NAME")) != 0)
    {
      clib_warning("mbedtls_ssl_set_hostname returned %d\n", rv);
      return -1;
    }

  ctx_ptr = uword_to_pointer (tls_ctx_index (ctx), void *);
  mbedtls_ssl_set_bio (&ctx->ssl, ctx_ptr, mbedtls_net_send, mbedtls_net_recv,
	               NULL);
  return 0;
}

int
tls_session_connected_callback (u32 app_index, u32 ctx_index,
                                stream_session_t * tls_session,
                                u8 is_fail)
{
  int (*cb_fn) (u32, u32, stream_session_t *, u8);
  u32 tls_index, tls_thread_index;
  ...
  ctx = tls_ctx_get (ctx_index);
  parent_app = application_get (ctx->parent_app_index);
  cb_fn = parent_app->cb_fns.session_connected_callback;
  tls_index = tls_session->session_index;
  tls_thread_index = tls_session->thread_index;

  if (is_fail)
    goto failed;

  sm = application_get_connect_segment_manager (parent_app);
  app_session = session_alloc (tls_session->thread_index);
  ...
  app_session->opaque = app_index;
  if (session_alloc_fifos (sm, app_session))
    goto failed;

  /* We allocated a new session in the same pool. Probably we invalidated
   * our session handle */
  tls_session = session_get (tls_index, tls_thread_index);
