...
sfdp_health_snapshot_get (sfdp_health_snapshot_t *snapshot)
{
  ...
  for (u32 shard = 0; shard <= vec_len (sfdp->per_thread_data); shard++)
    {
      health = shard < vec_len (sfdp->per_thread_data) ?
		 &sfdp->per_thread_data[shard].health :
		 &sfdp->unbound_health;
	...
	return -1;
      ...
      for (u32 reason = 0; reason < SFDP_SESSION_CREATE_N_FAILURE_REASONS;
	   reason++)
	{
	  value = __atomic_load_n (&health->create_failures[reason],
				   __ATOMIC_RELAXED);
	  if (value > ~0ULL - snapshot->create_failures[reason])
	    return -1;
	  snapshot->create_failures[reason] += value;
	}
      for (u32 reason = 0; reason < SFDP_SESSION_N_EVICTION_REASONS;
	   reason++)
	{
	  value = __atomic_load_n (&health->evictions[reason],
				   __ATOMIC_RELAXED);
