 ...
 *   - call sfdp_session_timer_stop() on the worker-owned timer wheel. A
 *     stopped timer means the session is already awaiting sfdp-expire, so
 *     repeated kill requests are a no-op.
 */
static int
sfdp_expire_session_now (sfdp_session_t *session, f64 now)
{
  ...
  if (sfdp_timer_is_free_internal (&tptd->wheel, timer->handle))
    return 0;
