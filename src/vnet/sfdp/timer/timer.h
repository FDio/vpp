...
sfdp_timer_tick_interval_sec (void)
{
  return sfdp_main.timer_tick_interval_s;
}
...
sfdp_seconds_to_ticks (f64 seconds)
{
  ...
  f64 interval = sfdp_timer_tick_interval_sec ();
...
{
  return ((f64) ticks) * sfdp_timer_tick_interval_sec ();
}

#define SFDP_TIMER_INTERVAL	       (sfdp_timer_tick_interval_sec ())
				      ...
				      f64 timeout_seconds)
{
  ...
  sfdp_session_timer_update (tw, timer, now, timeout_seconds);
}
