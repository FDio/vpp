...
#define SFDP_DEFAULT_TIMER_INTERVAL_MS (1000)

static_always_inline f64
sfdp_timer_interval_sec (void)
{
  return ((f64) (sfdp_main.timer_interval_ms ? sfdp_main.timer_interval_ms :
					       SFDP_DEFAULT_TIMER_INTERVAL_MS)) /
	 1000.0;
}
...
static_always_inline void
sfdp_session_timer_update (sfdp_tw_t *tw, sfdp_session_timer_t *timer, f64 now, u32 timeout_seconds)
{
  u32 ticks = SFDP_SECONDS_TO_TICKS (timeout_seconds);
