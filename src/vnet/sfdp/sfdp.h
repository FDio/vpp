...
sfdp_health_record_create_failure (sfdp_main_t *sfdp,
				   sfdp_per_thread_data_t *ptd,
...
static_always_inline void
sfdp_health_record_session_created (sfdp_main_t *sfdp,
				    sfdp_per_thread_data_t *ptd,
...
static_always_inline void
sfdp_health_record_session_removed (sfdp_main_t *sfdp,
				    sfdp_session_t *session)
{
  sfdp_health_counters_t *health;
  u64 previous;
  ...
  if (PREDICT_FALSE (!session->health_accounting_valid))
    {
      __atomic_store_n (&sfdp->unbound_health.accounting_valid, 0,
			__ATOMIC_RELEASE);
      return;
    }
  if (session->health_accounting_thread_index == SFDP_UNBOUND_THREAD_INDEX)
    health = &sfdp->unbound_health;
  else if (PREDICT_TRUE (session->health_accounting_thread_index <
			 vec_len (sfdp->per_thread_data)))
    health =
      &sfdp->per_thread_data[session->health_accounting_thread_index].health;
  else
    {
      __atomic_store_n (&sfdp->unbound_health.accounting_valid, 0,
			__ATOMIC_RELEASE);
  ...
  previous =
    __atomic_fetch_sub (&health->active_sessions, 1, __ATOMIC_RELAXED);
...
static_always_inline void
sfdp_health_record_capacity_evictions (sfdp_per_thread_data_t *ptd,
				       u32 count)
{
  __atomic_fetch_add (&ptd->health.evictions[SFDP_SESSION_EVICTION_CAPACITY],
			    ...
			    u64 *lookup_val, u32 scope_index, int is_ipv6)
{
  ...
  if (session_idx == ~0)
    {
      sfdp_health_record_create_failure (
	sfdp, ptd, SFDP_SESSION_CREATE_FAILURE_CAPACITY);
      return 1;
    }
