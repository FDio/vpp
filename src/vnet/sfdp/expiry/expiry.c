...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  /* get list of HW aged flows associated to SFDP sessions */
  sfdp_aged_flow_entry_t *aged_drained = 0;
  clib_spinlock_lock (&ptd->aged_sessions_lock);
  if (vec_len (ptd->aged_flows_pending) > 0)
    {
      aged_drained = ptd->aged_flows_pending;
  ...
  if (PREDICT_FALSE (vec_len (aged_drained) > 0))
      ...
      for (u32 i = 0; i < n_aged; i++)
	{
	  ...
	  vec_add1 (aged_flow_indices, e->flow_index);
