...
static int
sfdp_flow_offload_delete (vlib_main_t *vm, vlib_node_runtime_t *node, u32 session_index,
			  u8 direction)
{
  ...
  if (session->flow_index[direction] == ~0)
    {
      vlib_node_increment_counter (vm, node->node_index, SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_UNSET, 1);
      return -1;
    }

  rv = vnet_flow_disable (vnm, session->flow_index[direction], session->rx_hw_if_index);
  if (rv != 0)
    {
      vlib_node_increment_counter (vm, node->node_index,
				   SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_DISABLE_FAILED, 1);
      return rv;
    }

  rv = vnet_flow_del (vnm, session->flow_index[direction]);
  if (rv != 0)
    {
      vlib_node_increment_counter (vm, node->node_index,
  ...
  session->flow_index[direction] = ~0;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  vec_foreach (session_index, ptd->expired_sessions)
	...
	{
	  sfdp_flow_offload_delete (vm, node, *session_index, SFDP_FLOW_FORWARD);
	}
      if (session->flow_index[SFDP_FLOW_REVERSE] != ~0)
	{
	  sfdp_flow_offload_delete (vm, node, *session_index, SFDP_FLOW_REVERSE);
	}
