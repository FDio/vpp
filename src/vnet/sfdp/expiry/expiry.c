...
#define foreach_sfdp_expire_error                                                                  \
  ...
  _ (FLOW_OFFLOAD_SYNC_DISABLE_OK, "flow-offload-sync-disable-ok", INFO,                           \
     "flow offload sync disable ok")                                                               \
  _ (FLOW_OFFLOAD_SYNC_DISABLE_FAILED, "flow-offload-sync-disable-failed", ERROR,                  \
     "flow offload sync disable failed")                                                           \
  _ (FLOW_OFFLOAD_ASYNC_DISABLE_OK, "flow-offload-async-disable-ok", INFO,                         \
     "flow offload async disable ok")                                                              \
  ...
  _ (AGED_STALE, "aged-stale", INFO, "hw-aged flow ignored after session slot reuse")              \
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  if (PREDICT_FALSE (vec_len (aged_drained) > 0))
      ...
      for (u32 i = 0; i < n_aged; i++)
	{
	  ...
	  /* validate that aging notification is associated with a valid session */
	  sfdp_session_t *s = sfdp_session_at_index_if_valid (e->session_index);

	  if (PREDICT_FALSE (s == 0 || s->session_version != e->session_version))
	    {
	      n_stale++;
	      continue;
	    }
	  ...
	  if (PREDICT_FALSE (s->flow_index_verdict[e->dir] != e->vnet_flow_index))
	    {
	      n_stale++;
	      continue;
	    }
	  ...
	  sfdp_flow_aging_context_invalidate (
	    sfdp, sfdp_flow_aging_context_index (e->session_index, e->dir));
	...
	vlib_node_increment_counter (vm, node->node_index, SFDP_EXPIRE_ERROR_AGED_ALREADY_RETIRED,
				     n_already_retired);
      if (PREDICT_FALSE (n_stale))
	vlib_node_increment_counter (vm, node->node_index, SFDP_EXPIRE_ERROR_AGED_STALE, n_stale);
  ...
  vec_foreach (session_index, ptd->expired_sessions)
      ...
      else
	{
	  ...
	  if (PREDICT_FALSE (same_hw_if_index && session->rx_hw_if_index != hw_if_index_in_use))
		...
		if (PREDICT_FALSE (rv < 0))
		  vlib_node_increment_counter (vm, node->node_index,
					       SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_ASYNC_DISABLE_FAILED,
					       flow_delete);
		else
		  {
		    if (rv)
		      vlib_node_increment_counter (
			vm, node->node_index, SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_ASYNC_DISABLE_OK, rv);
		    if (rv < flow_delete)
		      vlib_node_increment_counter (
			vm, node->node_index, SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_ASYNC_DISABLE_FAILED,
			flow_delete - rv);
		  }
	      }
	    }
	  if (!same_hw_if_index)
	    {
	      if (PREDICT_FALSE (vnet_flow_disable (vnm, session->flow_index[SFDP_FLOW_FORWARD]) !=
				 0))
		vlib_node_increment_counter (vm, node->node_index,
					     SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_SYNC_DISABLE_FAILED, 1);
	      else
		vlib_node_increment_counter (vm, node->node_index,
					     SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_SYNC_DISABLE_OK, 1);
	      if (PREDICT_FALSE (vnet_flow_disable (vnm, session->flow_index[SFDP_FLOW_REVERSE]) !=
				 0))
		vlib_node_increment_counter (vm, node->node_index,
					     SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_SYNC_DISABLE_FAILED, 1);
	      else
	  ...
	  vec_elt (ptd->flow_indices_del, flow_delete + 1) = session->flow_index[SFDP_FLOW_REVERSE];
      ...
      if (session->flow_index_verdict[SFDP_FLOW_FORWARD] != ~0)
	{
	  ...
	  sfdp_flow_aging_context_invalidate (
	    sfdp, sfdp_flow_aging_context_index (*session_index, SFDP_FLOW_FORWARD));
	}
      if (session->flow_index_verdict[SFDP_FLOW_REVERSE] != ~0)
	{
	  ...
	  sfdp_flow_aging_context_invalidate (
	    sfdp, sfdp_flow_aging_context_index (*session_index, SFDP_FLOW_REVERSE));
	}
  ...
  if (same_hw_if_index)
    {
      int rv = vnet_flow_async_range_disable (vnm, ptd->flow_indices_del, hw_if_index_in_use);
      if (PREDICT_FALSE (rv < 0))
	vlib_node_increment_counter (
	  vm, node->node_index, SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_ASYNC_DISABLE_FAILED, flow_delete);
      else
	{
	  if (rv)
	    vlib_node_increment_counter (vm, node->node_index,
					 SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_ASYNC_DISABLE_OK, rv);
	  if (rv < flow_delete)
	    vlib_node_increment_counter (vm, node->node_index,
					 SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_ASYNC_DISABLE_FAILED,
					 flow_delete - rv);
  ...
  vec_foreach (fi, ptd->flow_indices_del)
    {
      if (PREDICT_FALSE (vnet_flow_del (vnm, *fi) != 0))
	vlib_node_increment_counter (vm, node->node_index,
				     SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_DELETE_FAILED, 1);
      else
	vlib_node_increment_counter (vm, node->node_index, SFDP_EXPIRE_ERROR_FLOW_OFFLOAD_DELETE_OK,
				     1);
    }

  vec_set_len (ptd->flow_indices_del, 2 * sfdp_num_sessions ());
