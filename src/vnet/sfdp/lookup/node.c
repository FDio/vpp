			  ...
			  u32 rx_sw_if_index)
{
  ...
  u8 pseudo_dir_reverse = pseudo_dir ^ 0x1;
  int rv = 0;
  ...
  rv = vnet_flow_add (vnm, &flow, &flow_index[pseudo_dir]);
  if (rv != 0)
    {
  ...
  rv = vnet_flow_enable (vnm, flow_index[pseudo_dir], hw_if_index);
  if (rv != 0)
    {
      vnet_flow_del (vnm, flow_index[pseudo_dir]);
  ...
  rv = vnet_flow_add (vnm, &flow, &flow_index[pseudo_dir_reverse]);
  if (rv != 0)
    {
      vnet_flow_disable (vnm, flow_index[pseudo_dir], hw_if_index);
      vnet_flow_del (vnm, flow_index[pseudo_dir]);
      vlib_node_increment_counter (vm, node->node_index, SFDP_LOOKUP_ERROR_FLOW_OFFLOAD_ADD_FAILED,
				   1);
      return rv;
  ...
  /* Enable the flow on the hardware interface */
  rv = vnet_flow_enable (vnm, flow_index[pseudo_dir_reverse], hw_if_index);
  if (rv != 0)
    {
      vnet_flow_disable (vnm, flow_index[pseudo_dir], hw_if_index);
      vnet_flow_del (vnm, flow_index[pseudo_dir]);
      vnet_flow_del (vnm, flow_index[pseudo_dir_reverse]);
      vlib_node_increment_counter (vm, node->node_index,
				   SFDP_LOOKUP_ERROR_FLOW_OFFLOAD_ENABLE_FAILED, 1);
      return rv;
    }
	  ...
	  for (u32 i = 0; i < n_created; i++)
	    {
	      ...
	      sfdp_flow_offload_create (vm, node, session_idx, rx_sw_if_index);
	    }
	}
