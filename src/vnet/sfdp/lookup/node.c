	  ...
	  for (u32 i = 0; i < n_created; i++)
	    {
	      ...
	      u32 rx_hw_if_index = vnet_buffer (bufs[i])->sw_if_index[VLIB_RX];
	      u32 hw_if_index = vnet_get_sw_interface (vnm, rx_sw_if_index)->hw_if_index;
	      sfdp_flow_offload_create (vm, node, session_idx, hw_if_index);
	    }
	}
