...
static int
sfdp_flow_offload_create_v4 (vlib_main_t *vm, vlib_node_runtime_t *node, u32 session_index,
			     u32 rx_sw_if_index)
{
  ...
  u32 hw_if_index;
  vnet_flow_t flow = { 0 };
  u32 flow_index[SFDP_FLOW_F_B_N] = { ~0, ~0 };
  int rv = 0;
  ...
  hw_if_index = vnet_get_sw_interface (vnm, rx_sw_if_index)->hw_if_index;
  ...
  if (rv != 0)
    {
      vlib_node_increment_counter (vm, node->node_index, SFDP_LOOKUP_ERROR_FLOW_OFFLOAD_ADD_FAILED,
				   1);
      goto cleanup_forward_disable_del;
    }
...
cleanup_forward_disable_del:
  vnet_flow_disable (vnm, flow_index[SFDP_FLOW_FORWARD], hw_if_index);
cleanup_forward_del:
...
static int
sfdp_flow_offload_create_v6 (vlib_main_t *vm, vlib_node_runtime_t *node, u32 session_index,
			     u32 rx_sw_if_index)
{
  vnet_main_t *vnm = &vnet_main;
  sfdp_session_t *session = sfdp_session_at_index (session_index);
  ...
  flow.ip6_n_tuple.src_addr.addr.as_u128 =
    session->keys[SFDP_SESSION_KEY_PRIMARY].key6.ip6_key.ip6_addr_hi.as_u128;
  flow.ip6_n_tuple.dst_addr.addr.as_u128 =
    session->keys[SFDP_SESSION_KEY_PRIMARY].key6.ip6_key.ip6_addr_lo.as_u128;
  flow.ip6_n_tuple.src_addr.mask.as_u64[0] = 0xFFFFFFFFFFFFFFFF;
  flow.ip6_n_tuple.src_addr.mask.as_u64[1] = 0xFFFFFFFFFFFFFFFF;
  flow.ip6_n_tuple.dst_addr.mask.as_u64[0] = 0xFFFFFFFFFFFFFFFF;
  flow.ip6_n_tuple.dst_addr.mask.as_u64[1] = 0xFFFFFFFFFFFFFFFF;
  flow.ip6_n_tuple.protocol.prot = (ip_protocol_t) session->proto;
  flow.ip6_n_tuple.protocol.mask = 0xFF;

  flow.ip6_n_tuple.src_port.port =
    clib_net_to_host_u16 (session->keys[SFDP_SESSION_KEY_PRIMARY].key6.ip6_key.port_hi);
  flow.ip6_n_tuple.dst_port.port =
    clib_net_to_host_u16 (session->keys[SFDP_SESSION_KEY_PRIMARY].key6.ip6_key.port_lo);
  flow.ip6_n_tuple.src_port.mask = 0xFFFF;
  flow.ip6_n_tuple.dst_port.mask = 0xFFFF;
...
static int
sfdp_flow_offload_create (vlib_main_t *vm, vlib_node_runtime_t *node, u32 session_index,
			  u32 rx_sw_if_index, u8 is_ipv6)
{
  if (is_ipv6)
    return sfdp_flow_offload_create_v6 (vm, node, session_index, rx_sw_if_index);
  else
    return sfdp_flow_offload_create_v4 (vm, node, session_index, rx_sw_if_index);
}
	  ...
	  if (is_ipv6)
	    {
	      ...
	      int res = ip6_ext_header_walk (b[0], ip, -1, &chain);
	      if (res >= 0)
		{
		  l4_hdr_offset = (i16) chain.eh[res].offset;
		}
	      else
		{
		  u8 *next_header = ip6_next_header (ip);
		  l4_hdr_offset = (i16) (next_header - b[0]->data);
		}
	    }
	  else
	    {
  ...
  if (is_ipv6)
	...
	if (clib_bihash_search_inline_with_hash_24_8 (&sfdp->table4, h[0],
						      &kv.kv4))
	  {
	    ...
	    if (is_offloaded == SFDP_LOOKUP_OFFLOAD_1ST_PACKET)
	      {
		u32 rx_sw_if_index = vnet_buffer (b[0])->sw_if_index[VLIB_RX];
		sfdp_flow_offload_create (vm, node, created_session_indices[n_created],
					  rx_sw_if_index, is_ipv6);
	      }
	    n_created++;
	  }
	else
