...
_(SET_MAC_ADDRESS, set_mac_address)
...
static void
vl_api_set_mac_address_t_handler (vl_api_set_mac_address_t * mp)
{
  vl_api_set_mac_address_reply_t *rmp;
  vnet_main_t *vnm = vnet_get_main ();
  u32 sw_if_index = ntohl (mp->sw_if_index);
  u64 mac;
  clib_error_t *error;
  ...
  BAD_SW_IF_INDEX_LABEL;
out:
  REPLY_MACRO (VL_API_SET_MAC_ADDRESS_REPLY);
}
