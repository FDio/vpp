				     ...
				     u32 queue_id, u8 mode, u32 context)
{
  ...
  mp->_vl_msg_id = ntohs (VL_API_SW_INTERFACE_RX_PLACEMENT_DETAILS);
  mp->sw_if_index = ntohl (sw_if_index);
  mp->queue_id = ntohl (queue_id);
  mp->worker_id = ntohl (worker_id);
  mp->mode = mode;
  mp->context = context;
  mp->is_main = (worker_id == 0) ? 1 : 0;
  ...
  (vl_api_sw_interface_rx_placement_dump_t * mp)
{
  ...
  if (sw_if_index == ~0)
      ...
      if (!vnet_swif_is_api_visible (si))
	{
      ...
      if (si->type != VNET_SW_INTERFACE_TYPE_HARDWARE)
	{
	  clib_warning ("sw_if_index type is not HARDWARE");
	  goto bad_sw_if_index;
	}
