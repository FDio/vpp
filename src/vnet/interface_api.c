				       ...
				       u32 context)
{
  ...
  mp->mq_rx_mode = hwinfo.mq_rx_mode;
  mp->mq_tx_mode = hwinfo.mq_tx_mode;
  ...
  mp->dev_flags = htonl ((u32) (hwinfo.dev_flags));
  ...
  mp->rss_hf = htonl ((u32) (hwinfo.rss_hf));
  mp->rss_hf_enabled = htonl ((u32) (hwinfo.rss_hf_enabled));
  ...
  (vl_api_sw_interface_device_info_dump_t * mp)
{
  ...
  ({
    ...
    /* check sw_if_index if it is set */
    if (sw_if_index && swif->sw_if_index != sw_if_index)
      continue;

    send_sw_interface_device_info_details (am, rp, swif, mp->context);

    /* only single instance was sent if sw_if_index is set */
    if (sw_if_index)
      {
	break;
      }
