 ...
 *
 * A TEIB entry represents the mapping between a peer on an interface in the
 * overlay and a next-hop address in the underlay. i.e. there's a multipoint
 * tunnel providing the overlay (henace a peer on that tunnel) which is
 * reachable via 'tunnel destination' address in the underlay.
 *
 ...
 */
extern int teib_entry_add (u32 sw_if_index, const ip_address_t *peer,
