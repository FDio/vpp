...
#define TEIB_TE_DBG(_te, _fmt, _args...)                                      \
  vlib_log_debug (teib_logger, "[%U]: " _fmt, format_teib_entry,              \
		  _te - teib_pool, ##_args)
#define TEIB_TE_INFO(_te, _fmt, _args...)                                     \
  vlib_log_notice (teib_logger, "[%U]: " _fmt, format_teib_entry,             \
		   _te - teib_pool, ##_args)
...
teib_entry_t *
teib_entry_find_46 (u32 sw_if_index, fib_protocol_t fproto,
...
teib_adj_fib_remove (ip_address_t *ip, u32 sw_if_index, u32 fib_index)
{
  if (AF_IP6 == ip_addr_version (ip) &&
	...
	fib_index, &pfx, FIB_SOURCE_ADJ, fib_proto_to_dpo (pfx.fp_proto),
...
teib_entry_del (u32 sw_if_index, const ip_address_t *peer)
{
  ...
  if (te != NULL)
	...
	ip_address_family_to_fib_proto (ip_addr_version (peer)), sw_if_index);
    ...
    {
      TEIB_INFO ("no such entry: %U, %U, %U", format_vnet_sw_if_index_name,
...
static void
teib_walk_itf_proto (u32 sw_if_index, ip_address_family_t af,
		     teib_walk_cb_t fn, void *ctx)
{
  index_t tei;
...
static void
teib_table_bind_v4 (ip4_main_t *im, uword opaque, u32 sw_if_index,
...
static void
teib_table_bind_v6 (ip6_main_t *im, uword opaque, u32 sw_if_index,
		    u32 new_fib_index, u32 old_fib_index)
{
  teib_table_bind_ctx_t ctx = {
    .old_fib_index = old_fib_index,
