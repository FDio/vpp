...
teib_impl_bind (const teib_impl_vft_t *vft)
{
  ...
  if (NULL == vft || NULL == vft->entry_add || NULL == vft->entry_del ||
      NULL == vft->entry_find || NULL == vft->walk_itf)
...
int
teib_entry_add (u32 sw_if_index, const ip_address_t *peer, u32 nh_table_id,
