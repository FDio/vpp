...
server_accept_callback (stream_session_t * s)
{
  ...
  bpm->server_event_queue[s->thread_index] =
...
server_rx_callback (stream_session_t * s)
{
  ...
  actual_transfer = svm_fifo_peek (rx_fifo, 0 /* relative_offset */ ,
				   max_dequeue, bpm->rx_buf[thread_index]);
  ...
  /*
   * $$$$ FC FIXME: Make this appear in the half-open session table,
   * propagate to the established session
   */
  a->server_handle = ps->vpp_server_handle;
...
static int
active_open_connected_callback (u32 app_index, u32 api_context,
				stream_session_t * s, u8 is_fail)
{
  builtin_proxy_main_t *bpm = &builtin_proxy_main;
  proxy_session_t *ps;
  ...
  if (!bpm->active_open_event_queue[thread_index])
    bpm->active_open_event_queue[thread_index] =
  ...
  key = 0;			/* FC FIXME: server handle propagation */

  p = hash_get (bpm->proxy_session_by_server_handle, key);

  if (p == 0)
    {
      clib_warning ("Cant find proxy session!");
      return 0;
    }
