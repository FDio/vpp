...
/* TCP option lengths + padding (2 bytes) */
...
tcp_options_parse_timestamp (tcp_header_t *th, tcp_options_t *to)
{
  ...
  /* TLEN accounts for 2 NOP alignment bytes: skip them if present */
  if (data[0] == TCP_OPTION_NOOP)
    data += 2;
  ...
  /* Zero out all flags but those set in SYN */
  to->flags &= (TCP_OPTS_FLAG_SACK_PERMITTED | TCP_OPTS_FLAG_WSCALE | TCP_OPTS_FLAG_TSTAMP |
		TCP_OPTS_FLAG_MSS);
