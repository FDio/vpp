...
typedef struct
{
  /* SACK / TFO-SYN-data / TFO-cookie slots never overlap in time:
   *  - sacks is populated only after handshake completion
   *    (post-SYN data flow with SACK_PERMITTED negotiated).
   *  - tfo_syn_data is populated only on the SYN path in tc->snd_opts.
   *  - tfo_cookie is only set on LOCAL tcp_options_t instances built by
   *    tcp_make_syn() / tcp_make_synack() just before tcp_options_write. */
  union
  {
    sack_block_t *sacks;  /**< SACK blocks (post-handshake) */
    u8 *tfo_syn_data;	  /**< TFO early SYN data vec (SYN only, client-side) */
    const u8 *tfo_cookie; /**< TFO cookie ptr for option-write (local snd_opts) */
  };
  ...
  union
  {
    u8 n_sack_blocks;  /**< Number of SACKs blocks (post-handshake) */
...
/* Wire length of a TFO option carrying a cookie of `clen` bytes:
 * Kind(1) + Length(1) + `clen` (cookie length) bytes.
 * Caller pads to a 4-byte TCP option boundary with NOPs. */
#define TCP_OPTION_LEN_FAST_OPEN(clen) (TCP_OPTION_LEN_FAST_OPEN_BASE + (clen))

/**
 * RFC 7413: Locate the TFO cookie bytes in a parsed TCP header.
  ...
  if (tcp_opts_tfo (opts))
    {
      ...
      *data++ = TCP_OPTION_LEN_FAST_OPEN_BASE + opts->tfo_cookie_len;
      if (opts->tfo_cookie_len && opts->tfo_cookie)
	{
	  clib_memcpy_fast (data, opts->tfo_cookie, opts->tfo_cookie_len);
