...
static void
tcp_rack_account_rxt_delivery (tcp_connection_t *tc, tcp_bt_sample_t *bts, u32 delivered_end)
{
  if (!(bts->flags & TCP_BTS_IS_RXT) || (bts->flags & TCP_BTS_IS_LOST))
    return;

  tc->sack_sb.rxt_sacked += delivered_end - bts->min_seq;
}
...
tcp_rack_sample_acked (tcp_connection_t *tc, tcp_bt_sample_t *bts, u32 delivered_end, u32 *fack)
{
  ...
  tcp_rack_account_rxt_delivery (tc, bts, delivered_end);
