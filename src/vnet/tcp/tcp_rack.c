...
static void
tcp_rack_count_rxt (tcp_connection_t *tc, tcp_bt_sample_t *bts, void *opaque)
{
  u32 *bytes = opaque;
  u32 start, end;

  if (!(bts->flags & TCP_BTS_IS_RXT) || (bts->flags & (TCP_BTS_IS_SACKED | TCP_BTS_IS_LOST)))
    return;

  start = seq_gt (bts->min_seq, tc->snd_una) ? bts->min_seq : tc->snd_una;
  end = seq_lt (bts->max_seq, tc->snd_nxt) ? bts->max_seq : tc->snd_nxt;
  if (seq_lt (start, end))
    *bytes += end - start;
}

static u32
tcp_rack_rxt_bytes_in_flight (tcp_connection_t *tc)
{
  u32 bytes = 0;

  tcp_bt_walk_range (tc, tc->snd_una, tc->snd_nxt, tcp_rack_count_rxt, &bytes);
  return bytes;
}

static void
tcp_rack_filter_rxt_sacked (tcp_connection_t *tc, tcp_bt_sample_t *bts, u32 delivered_end)
{
  ...
  end = seq_lt (delivered_end, sb->high_rxt) ? delivered_end : sb->high_rxt;
  if (!seq_lt (bts->min_seq, end))
    return;

  /* RACK already accounted the retransmission as lost. */
  bytes = end - bts->min_seq;
  sb->rxt_sacked -= clib_min (sb->rxt_sacked, bytes);
}
  ...
  if (!tcp_in_cong_recovery (tc))
    {
      ...
      u32 snd_rxt_bytes = tcp_rack_rxt_bytes_in_flight (tc);

      tcp_cc_init_congestion (tc, snd_rxt_bytes);
      scoreboard_init_rxt (&tc->sack_sb, tc->snd_una);
      if (snd_rxt_bytes && seq_gt (high_rxt, tc->sack_sb.high_rxt))
	tc->sack_sb.high_rxt = high_rxt;
...
tcp_rack_sample_acked (tcp_connection_t *tc, tcp_bt_sample_t *bts, u32 delivered_end, u32 *fack)
{
  ...
  tcp_rack_filter_rxt_sacked (tc, bts, delivered_end);
