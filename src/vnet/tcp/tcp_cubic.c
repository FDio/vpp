...
{
  /** time period needed to increase the current window
...
W_cubic (cubic_data_t *cd, f64 t)
{
  ...
  return cubic_c * pow (t - cd->K, 3) + cd->w_max;
}
...
W_est (cubic_data_t *cd, f64 t, f64 rtt)
{
  ...
  return cd->w_max*beta_cubic + (3*(1-beta_cubic)/(1+beta_cubic)) * (t/rtt);
}
...
cubic_rcv_ack (tcp_connection_t *tc)
{
  ...
  /* Estimate window size for aimd and cubic*/
  w_aimd = W_est (cd, t, rtt_sec);
  w_cubic = W_cubic (cd, t + rtt_sec);
  w_inc = clib_max (clib_max (w_aimd, w_cubic) - cd->w_max, 0);
  tcp_cwnd_accumulate (tc, w_inc);

  cd->w_max = clib_max (cd->w_max, w_cubic);
}
