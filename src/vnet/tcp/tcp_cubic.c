...
typedef struct cubic_data_
{
  ...
  /** bytes acked since the start of current congestion avoidance */
  f64 t_start;
...
cubic_rcv_ack (tcp_connection_t * tc)
{
  ...
  delta_w_cubic = delta_W_cubic (cd, t);
  delta_w_aimd = tc->bytes_acked * ((f64)tc->snd_mss/tc->cwnd);

  /* In tcp-friendly region grow by the number of bytes acked.
   * This avoids computing W_est and grows faster for very small
   * rtts */
  tc->cwnd += clib_max (delta_w_cubic, delta_w_aimd);
