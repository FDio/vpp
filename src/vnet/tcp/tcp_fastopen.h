 ...
 *
 * The entry threads itself into a per-cache circular LRU FIFO via the
 * @ref lru_anchor field, using vppinfra's clib_llist (head = oldest,
 * tail = newest). The list is ordered by @ref timestamp ascending, so
 * the periodic sweep can walk from the head and stop at the first
 * non-expired entry, bounding cost to O(expired) instead of O(N).
 */
typedef struct tcp_tfo_cc_entry_
{
  clib_llist_anchor_t lru_anchor; /**< clib_llist FIFO anchor */
  uword ip_key;			  /**< hash key (remote IP) for sweep */
 ...
 *
 * Same clib_llist LRU FIFO trick as @ref tcp_tfo_cc_entry_t.
 */
 ...
 * a freshly computed cookie, attaches the option to @p opts (including
 * staging the cookie pointer in @ref tcp_options_t::tfo_cookie), and
 * advances @p tcp_opts_len by the encoded option length (with
 * realignment). No-op when no TFO option was received in SYN.
 */
