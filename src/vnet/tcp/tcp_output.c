...
tcp_timer_retransmit_handler (u32 tc_index)
{
  ...
  if (tc->state >= TCP_STATE_ESTABLISHED)
      ...
      if (tc->flags & TCP_CONN_FINSNT)
	{
      ...
      if (tc->state == TCP_STATE_ESTABLISHED)
      {
        clib_warning("%u fr %u boff %u flight %u lost %u rxt %u "
		     "rxt-delivered %u cwnd %u space %u diff %u",
...
tcp_fastrecovery_prr_snd_space (tcp_connection_t *tc)
{
  ...
  ASSERT (tc->prev_cwnd <= tc->cwnd);

  /* Available space is  out * rate_reduction - sent where
   * rate_reduction = tc->cwnd / tc->prev_cwnd
   */
  space = ((int)out * tc->cwnd - sent * tc->prev_cwnd) / tc->prev_cwnd;
  return clib_max (space, 0);
}
