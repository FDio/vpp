...
tcp_alloc_tx_buffers (tcp_main_t * tm, u8 thread_index, u16 * n_bufs,
		      u32 wanted)
{
  tcp_worker_ctx_t *ctx = &tm->thread_ctx[thread_index];
  vlib_main_t *vm = vlib_get_main ();
  u32 n_alloc;

  ASSERT (wanted > *n_bufs);
  vec_validate_aligned (ctx->tx_buffers, wanted - 1,
			    ...
			    u8 is_ip4, u32 fib_index, u8 flush)
{
  ...
  f = tm->thread_ctx[thread_index].ip_lookup_tx_frames[!is_ip4];
  if (!f)
    {
      f = vlib_get_frame_to_node (vm, next_index);
      ASSERT (f);
      tm->thread_ctx[thread_index].ip_lookup_tx_frames[!is_ip4] = f;
    }
  ...
  if (flush || f->n_vectors == VLIB_FRAME_SIZE)
    {
      ...
      tm->thread_ctx[thread_index].ip_lookup_tx_frames[!is_ip4] = 0;
    }
}
			 ...
			 u8 is_ip4, u8 flush)
{
  ...
  f = tm->thread_ctx[thread_index].tx_frames[!is_ip4];
  if (!f)
    {
      f = vlib_get_frame_to_node (vm, next_index);
      ASSERT (f);
      tm->thread_ctx[thread_index].tx_frames[!is_ip4] = f;
    }
  to_next = vlib_frame_vector_args (f);
  to_next[f->n_vectors] = bi;
  f->n_vectors += 1;
  if (flush || f->n_vectors == VLIB_FRAME_SIZE)
    {
      vlib_put_frame_to_node (vm, next_index, f);
      tm->thread_ctx[thread_index].tx_frames[!is_ip4] = 0;
    }
}
...
{
  if (tcp_main.thread_ctx[thread_index].tx_frames[!is_ip4])
    {
      u32 next_index;
      next_index = is_ip4 ? tcp4_output_node.index : tcp6_output_node.index;
      vlib_put_frame_to_node (vm, next_index,
                              tcp_main.thread_ctx[thread_index].tx_frames[!is_ip4]);
      tcp_main.thread_ctx[thread_index].tx_frames[!is_ip4] = 0;
    }
}
...
{
  if (tcp_main.thread_ctx[thread_index].ip_lookup_tx_frames[!is_ip4])
    {
      u32 next_index;
      next_index = is_ip4 ? ip4_lookup_node.index : ip6_lookup_node.index;
      vlib_put_frame_to_node (vm, next_index,
			      tcp_main.thread_ctx[thread_index].ip_lookup_tx_frames[!is_ip4]);
      tcp_main.thread_ctx[thread_index].ip_lookup_tx_frames[!is_ip4] = 0;
    }
}
				...
				u32 max_deq_bytes, vlib_buffer_t ** b)
{
  ...
  /* Split mss into multiple buffers */
  else
    {
      ...
      available_bufs = vec_len (tm->thread_ctx[thread_index].tx_buffers);
