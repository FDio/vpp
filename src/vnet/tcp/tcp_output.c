...
tcp_fastrecovery_prr_snd_space (tcp_connection_t * tc)
{
  ...
  if (pipe > tc->ssthresh)
    ...
    {
      /* Slow start reduction bound (RFC 6937): allow one extra segment per
       * the data delivered since the last time we sent, on top of packet
       * conservation, so the pipe re-inflates to ssthresh after heavy loss */
