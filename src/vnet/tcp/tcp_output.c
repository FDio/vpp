...
tcp_timer_retransmit_handler (u32 tc_index)
{
  ...
  if (tc->state >= TCP_STATE_ESTABLISHED)
      ...
      if (tc->flags & TCP_CONN_FINSNT)
	{
      ...
      if (tc->state == TCP_STATE_ESTABLISHED)
      {
        clib_warning("%u fr %u boff %u flight %u lost %u rxt %u rxt-delivered %u cwnd %u space %u",
                     tc->c_c_index, tcp_in_fastrecovery (tc), tc->rto_boff,
                     tcp_flight_size (tc), tc->sack_sb.lost_bytes,
                     tc->snd_rxt_bytes, tc->rxt_delivered, tc->cwnd,
                     tcp_available_cc_snd_space (tc));
        if (tc->sack_sb.lost_bytes && !tcp_in_fastrecovery(tc))
	  ...
	  || (tc->rto_boff > 0 && seq_geq (tc->snd_una, tc->snd_congestion)
	      && !tcp_flight_size (tc)))
	{
	  clib_warning ("%u exit?", tc->c_c_index);
      ...
      if (tc->rto_boff >= TCP_RTO_BOFF_MAX)
	{
      ...
      if (!n_bytes)
	{
	  clib_warning ("%u no buffers?", tc->c_c_index);
	  tcp_timer_update (tc, TCP_TIMER_RETRANSMIT, 1);
	  return;
	}
...
//  tc->cwnd = clib_max (tc->cwnd - n_reduce * tc->snd_mss, tc->ssthresh);
...
//    return snd_space;
  ...
  out = sb->sacked_bytes + sb->lost_bytes + tc->rxt_delivered;
  rate_reduction = (f64) tc->cwnd / tc->prev_cwnd;
  ASSERT (rate_reduction <= 1);
  sent = tc->snd_rxt_bytes + (tc->snd_nxt - tc->snd_congestion);

  return clib_max ((int)(out * rate_reduction - sent), 0);
}
