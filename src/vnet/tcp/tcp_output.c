...
tcp_update_rcv_wnd (tcp_connection_t * tc)
{
   ...
   * avoid advertising a window more than it can enqueue. Minimum non-zero
   * window size calculated here is (1 << rcv_wscale) */
  if (wnd && tc->rcv_wscale)
    wnd = round_down_pow2 (wnd, 1 << tc->rcv_wscale);

  tc->rcv_wnd = clib_min (wnd, TCP_WND_MAX << tc->rcv_wscale);

  /* Avoid a non-zero rcv_wnd less than mss */
  if (tc->rcv_wnd < tc->rcv_opts.mss)
    tc->rcv_wnd = 0;
}
