...
tcp_enqueue_to_ip_lookup_i (vlib_main_t * vm, vlib_buffer_t * b, u32 bi,
			    u8 is_ip4, u32 sw_if_index, u32 fib_index, u8 flush)
{
...
tcp_enqueue_to_ip_lookup_now (vlib_main_t * vm, vlib_buffer_t * b, u32 bi,
			      u8 is_ip4, u32 sw_if_index, u32 fib_index)
{
  tcp_enqueue_to_ip_lookup_i (vm, b, bi, is_ip4, sw_if_index, fib_index, 1);
}
...
tcp_enqueue_to_ip_lookup (vlib_main_t * vm, vlib_buffer_t * b, u32 bi,
			  u8 is_ip4, u32 sw_if_index, u32 fib_index)
{
  tcp_enqueue_to_ip_lookup_i (vm, b, bi, is_ip4, sw_if_index, fib_index, 0);
}
...
tcp_send_reset_w_pkt (tcp_connection_t * tc, vlib_buffer_t * pkt, u8 is_ip4)
{
  ...
  tcp_enqueue_to_ip_lookup_now (vm, b, bi, is_ip4, sw_if_index, fib_index);
...
tcp_send_reset (tcp_connection_t * tc)
{
  ...
  tcp_enqueue_to_ip_lookup_now (vm, b, bi, tc->c_is_ip4, tc->c_sw_if_index, tc->c_fib_index);
  TCP_EVT_DBG (TCP_EVT_RST_SENT, tc);
}
...
tcp_send_syn (tcp_connection_t * tc)
{
  ...
  tcp_enqueue_to_ip_lookup (vm, b, bi, tc->c_is_ip4, tc->c_sw_if_index, tc->c_fib_index);
