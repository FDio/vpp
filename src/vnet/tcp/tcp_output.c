  ...
  if (is_ip4)
  ...
  else
    {
      ...
      ih6 = vlib_buffer_push_ip6_custom (vm, b, &pkt_ih6->dst_address,
				  &pkt_ih6->src_address, IP_PROTOCOL_TCP, tc->ipv6_flow_label);
  ...
  if (tc->c_is_ip4)
  ...
  else
    {
      ...
      ih = vlib_buffer_push_ip6_custom (vm, b, &tc->c_lcl_ip6,
				 &tc->c_rmt_ip6, IP_PROTOCOL_TCP, tc->ipv6_flow_label);
		    ...
		    tcp_connection_t * tc0, u8 is_ip4)
{
  ...
  if (is_ip4)
    ...
    ih0 = vlib_buffer_push_ip6_custom (vm, b0, &tc0->c_lcl_ip6, &tc0->c_rmt_ip6,
