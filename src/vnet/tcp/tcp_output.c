...
tcp_transmit_unsent (tcp_worker_ctx_t * wrk, tcp_connection_t * tc,
		     u32 burst_size)
{
  u32 offset, n_segs = 0, n_written, bi, available_wnd, max_deq, max_bytes;
  vlib_main_t *vm = wrk->vm;
  vlib_buffer_t *b = 0;
  ...
  while (n_segs < burst_size)
    {
      ...
      if (PREDICT_FALSE (max_deq <= offset))
	goto done;

      /* Do not request more bytes than the tx fifo can currently provide. */
      max_bytes = clib_min (tc->snd_mss, max_deq - offset);
      n_written = tcp_prepare_segment (wrk, tc, offset, max_bytes, &b);
      if (!n_written)
	goto done;
