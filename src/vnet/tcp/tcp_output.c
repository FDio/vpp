...
tcp_rxt_timeout_cc (tcp_connection_t * tc)
{
  ...
  if (tcp_in_fastrecovery (tc))
    {
      clib_warning ("%u TIMEOUT snd_cong %u space %u %u rxt %u out %u"
		    "\nscoreboard:\n %U\n",
                    tc->c_c_index, tc->snd_congestion - tc->iss,
                    tcp_available_cc_snd_space (tc),
                    tc->snd_rxt_bytes, tcp_bytes_out (tc),
...
//  while (hole && snd_space > 0 && n_segs++ + segs_rxt <= 8)
  while (snd_space > 0 && n_segs++ < VLIB_FRAME_SIZE/8)
    {
