...
always_inline void
tcp_flush_frame_to_output (vlib_main_t *vm, tcp_main_t *tm, u8 thread_index,
			   u8 is_ip4)
{
  if (tm->tx_frames[!is_ip4][thread_index])
    {
      u32 next_index;
      next_index = is_ip4 ? tcp4_output_node.index : tcp6_output_node.index;
      if (CLIB_DEBUG)
	{
	  u32 *to_next = vlib_frame_vector_args (tm->tx_frames[!is_ip4][thread_index]);
	  ASSERT (to_next[0] != 0xfefefefe);
	}
      vlib_put_frame_to_node (vm, next_index,
			      tm->tx_frames[!is_ip4][thread_index]);
      tm->tx_frames[!is_ip4][thread_index] = 0;
    }
}

/**
 * Flush tx frames populated by retransmits and timer pops
 */
void
tcp_flush_frames_to_output (vlib_main_t *vm, u8 thread_index)
{
  tcp_flush_frame_to_output (vm, &tcp_main, thread_index, 1);
  tcp_flush_frame_to_output (vm, &tcp_main, thread_index, 0);
}
