...
tcp_get_free_buffer_index (tcp_main_t * tm, u32 * bidx)
{
  ...
  /* Injecting allocation failure faults? */
  if (PREDICT_FALSE (tm->buffer_fail_fraction != 0.0))
    {
      if (PREDICT_TRUE(tm->buffer_fail_counters[thread_index] > 0))
        {
          if ((1.0 / (f64) (tm->buffer_fail_counters[thread_index]))
              < tm->buffer_fail_fraction)
            {
              tm->buffer_fail_counters[thread_index] = 0.0000001;
              return -1;
            }
        }
      tm->buffer_fail_counters[thread_index] ++;
    }
