...
tcp_cc_rxt_timeout (tcp_connection_t *tc)
{
  ...
  /* Episode-scoped state, snapshotted once when the congestion episode
   * starts. If we are already in congestion recovery these were taken on
   * entry and must not be overwritten. */
   ...
   * the first timeout of the episode. */
  if (!tc->rto_boff)
    {
      tc->rtt_ts = 0;
      tc->cwnd_acc_bytes = 0;
