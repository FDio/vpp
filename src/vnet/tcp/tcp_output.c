...
tcp_timer_retransmit_handler_i (u32 index, u8 is_syn)
{
  ...
  if (is_syn)
  ...
  else
    {
      ...
      if (PREDICT_FALSE (tc == 0 || tc->state == TCP_STATE_SYN_SENT))
	{
	  clib_warning ("syn-sent happened state %u", tc->state);
	  return;
	}
      if (tc->state == TCP_STATE_CLOSED)
	{
	  clib_warning ("closed happened %U", format_tcp_connection, tc, 2);
  ...
  if (tc->state >= TCP_STATE_ESTABLISHED)
	...
	tc->snd_congestion = tc->snd_una_max;
      ...
      if (!n_bytes)
	{
	  clib_warning ("zero data to send?: %u", session_tx_fifo_max_dequeue (&tc->connection));
	  tcp_retransmit_timer_force_update (tc);
	  return;
	}
