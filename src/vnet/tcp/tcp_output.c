      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (is_ip4)
		  ...
		  ip6_address_is_link_local_unicast (&tc0->c_rmt_ip6)))
		{
		  ...
		  u8 *rewrite;
		  u32 si;
		  si = session_sw_if_index_w_connection (&tc0->connection);
		  vnet_buffer (b0)->sw_if_index[VLIB_TX] = si;
		  next0 = TCP_OUTPUT_NEXT_INTERFACE_OUTPUT;
		  nbr = ip6_neighbors_entries (si);
		  rewrite = ethernet_build_rewrite (vnet_get_main (), si,
			                            VNET_LINK_IP6,
			                            nbr->link_layer_address);
		  b0->current_data -= vec_len (rewrite);
		  b0->current_length += b0->current_data;
		  clib_memcpy (((u8 *)ih0) - b0->current_data, rewrite,
		               b0->current_data);
		}
	    }
