...
tcp_test_ooo (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  TCP_TEST((rv == total_size), "retrieved %u not %u", rv, total_size);
...
tcp_test_fifo1 (vlib_main_t * vm)
{
  ...
  TCP_TEST ((ooo_seg->fifo_position == 8),
	    "first ooo seg position %u", ooo_seg->fifo_position);
  TCP_TEST ((ooo_seg->length == 24),
	    "first ooo seg length %u", ooo_seg->length);

  /* Enqueue the missing u32 */
  rv = svm_fifo_enqueue_nowait (f, 0 /* pid */, sizeof(u32),
				(u8 *) (test_data + 1));
  TCP_TEST((rv == 28), "bytes to be enqueued %u", rv);
  ...
  for (i = 0; i < 7; i++)
	...
	{
	  clib_warning("recovered %d not %d", data_word, test_data[i]);
	  goto out;
	}
    }
...
tcp_test_fifo2 (vlib_main_t * vm)
{
  ...
  TCP_TEST ((svm_fifo_number_ooo_segments (f) == 1),
	    "number of ooo segments %u", svm_fifo_number_ooo_segments (f));
  ooo_seg = svm_fifo_first_ooo_segment (f);
  TCP_TEST ((ooo_seg->fifo_position == 4),
	    "first ooo seg position %u", ooo_seg->fifo_position);
