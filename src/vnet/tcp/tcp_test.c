...
tcp_test_sack ()
{
  ...
  tc->snd_una = 0;
  tc->snd_una_max = snd_una_max;
  tc->snd_nxt = snd_una_max;
  tc->opt.flags |= TCP_OPTS_FLAG_SACK;
  scoreboard_init (&tc->sack_sb);

  for (i = 0; i < snd_una_max/100; i++)
    {
      block.start = i * 100;
      block.end = (i+1) * 100;
  ...
  for (i = 0; i < snd_una_max/200; i++)
    {
      vec_reset_length (&tc->opt);
  ...
  TCP_TEST ((pool_elts (sb->holes) == 5),
	    "scoreboard has 5 elements: %d",
	    pool_elts (sb->holes));

  /* First SACK block should be rejected */
  hole = scoreboard_first_hole (sb);
  TCP_TEST ((hole->start == 0 && hole->end == 200),
	    "first hole start %u end %u",
	    hole->start, hole->end);

  hole = scoreboard_last_hole (sb);
  TCP_TEST ((hole->start == 900 && hole->end == 1000),
	    "last hole start %u end %u",
	    hole->start, hole->end);
  TCP_TEST ((sb->sacked_bytes == 400),
	    "sacked bytes %d", sb->sacked_bytes);
  ...
  TCP_TEST ((pool_elts(sb->holes) == 1),
	    "scoreboard has %d holes",
	    pool_elts (sb->holes));
  TCP_TEST ((hole->start == 0 && hole->end == 100),
	    "first hole start %u end %u",
	    hole->start, hole->end);
  TCP_TEST ((sb->sacked_bytes == 900),
	    "sacked bytes %d", sb->sacked_bytes);
