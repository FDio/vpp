...
static int
tcp_test_fifo2 (vlib_main_t * vm, unformat_input_t * input)
{
  svm_fifo_t *f;
  u32 fifo_size = 1 << 20;
  int i, rv;
  u64 data64;
  test_pattern_t *tp, *vp;
  u8 *validate_pattern = 0;

  /* $$$ parse args */

  /* Validate, and try segments in order... */
  vlib_cli_output (vm, "validate data, present it in order...");

  vec_validate (validate_pattern, sizeof (test_pattern) - 1);

  memcpy (validate_pattern, test_pattern, vec_len (validate_pattern));

  qsort (validate_pattern, ARRAY_LEN (test_pattern), sizeof (test_pattern_t),
	 pattern_cmp);

  tp = (test_pattern_t *) validate_pattern;

  /* Go through the pattern and make 100% sure it's sane */
  for (i = 0; i < ARRAY_LEN (test_pattern) - 1; i++)
    {
      if (tp->offset + tp->len != (tp + 1)->offset)
	vlib_cli_output (vm, "[%d] missing {%d, %d}",
			 i, (tp->offset + tp->len),
			 (tp + 1)->offset - (tp->offset + tp->len));
      tp++;
    }

  vlib_cli_output (vm, "data validate complete");

  vp = (test_pattern_t *) validate_pattern;
  ...
  for (i = 0; i < ARRAY_LEN (test_pattern); i++)
    {
      tp = vp + i;
  ...
  vlib_cli_output (vm, "missing link enqueue returned %d", rv);

  svm_fifo_free (f);

  vec_free (validate_pattern);

  /* Now try it again w/ unsorted data... */

  vlib_cli_output (vm, "-------------");
  vlib_cli_output (vm,
		   "Try again with raw data. Results should be identical.");
...
static int
tcp_test_ooo (vlib_main_t * vm, unformat_input_t * input)
{
  svm_fifo_t *f;
  u32 fifo_size = 4 << 10;
  ...
  if (overlap == 0)
    {
      vlib_cli_output (vm, "validate data...");

      tp = (test_pattern_t *) generate;

      /* Go through the pattern and make 100% sure it's sane */
      for (i = 0; i < vec_len (generate) - 1; i++)
	{
  ...
  if (rv == total_size)
    ...
    vlib_cli_output (vm, "FAIL... Retrieved %u not %u bytes", rv, total_size);
	  ...
	  unformat_input_t * input, vlib_cli_command_t * cmd_arg)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
      ...
      if (unformat (input, "sack"))
	{
      ...
      else if (unformat (input, "ooo"))
	{
	  res = tcp_test_ooo (vm, input);
	}
      else if (unformat (input, "fifo2"))
	{
	  res = tcp_test_fifo2 (vm, input);
	}
