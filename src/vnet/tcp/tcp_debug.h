...
#define TCP_EVT_INIT_HANDLER(_tc, _is_l, ...)                                 \
  {                                                                           \
    ...
    if (_tc->c_is_ip4)                                                        \
      {                                                                       \
	_tc->c_elog_track.name = (char *) format (                            \
	  0, _fmt, _tc->c_thread_index, _tc->c_lcl_ip.ip4.as_u8[3],           \
	  clib_net_to_host_u16 (_tc->c_lcl_port), _tc->c_rmt_ip.ip4.as_u8[3], \
	  clib_net_to_host_u16 (_tc->c_rmt_port), 0);                         \
      }                                                                       \
    else                                                                      \
      _tc->c_elog_track.name = (char *) format (                              \
	0, _fmt, _tc->c_thread_index, _tc->c_lcl_ip.ip6.as_u8[15],            \
...
#define TCP_EVT_ACK_SENT_HANDLER(_tc, ...)                                    \
  {                                                                           \
    ELOG_TYPE_DECLARE (_e) = {                                                \
      .format =                                                               \
	"ack-tx: acked %u rcv_nxt %u rcv_wnd %u snd_nxt %u snd_wnd %u",       \
      .format_args = "i4i4i4i4i4",                                            \
    };                                                                        \
    TCP_DECLARE_ETD (_tc, _e, 5);                                             \
...
#define TCP_EVT_DUPACK_RCVD_HANDLER(_tc, ...)                                 \
  {                                                                           \
    ELOG_TYPE_DECLARE (_e) = {                                                \
      .format =                                                               \
	"dack-rx: snd_una %u cwnd %u snd_wnd %u flight %u rcv_wnd %u",        \
      .format_args = "i4i4i4i4i4",                                            \
    };                                                                        \
    TCP_DECLARE_ETD (_tc, _e, 5);                                             \
    ed->data[0] = _tc->snd_una - _tc->iss;                                    \
