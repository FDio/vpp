...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "out: snd_una %u snd_nxt %u una_max %u",			\
    .format_args = "i4i4i4",						\
  };									\
...
#define TCP_EVT_DBG(_tc, _evt) CC(_evt, _HANDLER)(_tc)

#else
#define TCP_DBG(_tc, _fmt, _args...)
#define TCP_EVT_DBG(_tc, _evt)
#endif
