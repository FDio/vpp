...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "ack prep: acked %u rcv_nxt %u rcv_wnd %u snd_nxt %u",	\
...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "acked rx: %u cwnd %u snd-wnd %u inflight %u snd_una %u",	\
...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "pktize: snd_una %u snd_nxt %u wnd %u flight %u",		\
    .format_args = "i4i4i4i4",						\
  };									\
  DECLARE_ETD(_tc, _e, 4);						\
  ed->data[0] = _tc->snd_una - _tc->iss;				\
...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "in %s: len %u written %u rcv_nxt %u free wnd %u",	\
