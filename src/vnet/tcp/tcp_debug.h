...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "dupack(tx): rcv_nxt %u rcv_wnd %u",			\
    .format_args = "i4i4",						\
  };									\
...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "acked(rx): %u snd_una %u ack %u cwnd %u inflight %u",	\
...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "dupack(rx): snd_una %u cwnd %u snd_wnd %u inflight %u",	\
    .format_args = "i4i4i4i4",						\
  };									\
  DECLARE_ETD(_tc, _e, 4);						\
...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "pktize: snd_una %u snd_nxt %u space %u flight %u",	\
    .format_args = "i4i4i4i4",						\
  };									\
  DECLARE_ETD(_tc, _e, 4);						\
  ed->data[0] = _tc->snd_una - _tc->iss;				\
...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "in(%s): len %u written %d rcv_nxt %u free wnd %u",	\
...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "seg(invalid): seq %u end %u rcv_las %u rcv_nxt %u wnd %u",\
    .format_args = "i4i4i4i4i4",					\
  };									\
  DECLARE_ETD(_tc, _e, 5);						\
...
{									\
  ELOG_TYPE_DECLARE (_e) =						\
  {									\
    .format = "ack(%s): ack %u snd_una %u snd_nxt %u una_max %u",	\
    .format_args = "t4i4i4i4i4",					\
    .n_enum_strings = 3,						\
    .enum_strings = {                                           	\
      "invalid",                                                 	\
