...
#define tcp_evt_grp_dbg_lvl(_evt) tcp_evt_dbg.group_dbg_lvl[tcp_evt_dbg_grp (_evt)]
...
extern tcp_evt_dbg_t tcp_evt_dbg;

#define foreach_tcp_dbg_evt					\
  _(INIT, 		MAIN, 1, "init")			\
  _(DEALLOC, 		MAIN, 1, "dealloc")			\
  _(OPEN, 		MAIN, 1, "open")			\
  _(CLOSE, 		MAIN, 1, "close")			\
  _(BIND, 		MAIN, 1, "bind")			\
  _(UNBIND, 		MAIN, 1, "unbind")			\
  _(DELETE, 		MAIN, 1, "delete")			\
  _(SYN_RCVD, 		MAIN, 1, "SYN rcvd")			\
  _(STATE_CHANGE, 	MAIN, 1, "state change")		\
...
{									\
  vec_add1 (tcp_evt_dbg.free_track_indices, 				\
