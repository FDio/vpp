...
{
  tc->cc_algo = tcp_cc_algo_get (tcp_main.cfg.cc_algo);
  tc->cc_algo->init (tc);
}
  ...
  if (is_ip4)
    {
      ...
      if (tm->last_v4_addr_rotor >= vec_len (tm->cfg.ip4_src_addrs))
	tm->last_v4_addr_rotor = 0;
      lcl_addr->ip4.as_u32 = tm->cfg.ip4_src_addrs[index].as_u32;
    }
  else
    {
      index = tm->last_v6_addr_rotor++;
      if (tm->last_v6_addr_rotor >= vec_len (tm->cfg.ip6_src_addrs))
	tm->last_v6_addr_rotor = 0;
      clib_memcpy_fast (&lcl_addr->ip6, &tm->cfg.ip6_src_addrs[index],
			sizeof (ip6_address_t));
    }
...
tcp_session_open (transport_endpoint_cfg_t * rmt)
{
   ...
   */
  if ((rmt->is_ip4 && vec_len (tm->cfg.ip4_src_addrs))
      || (!rmt->is_ip4 && vec_len (tm->cfg.ip6_src_addrs)))
...
tcp_main_enable (vlib_main_t * vm)
{
  ...
  prealloc_conn_per_wrk = tm->cfg.preallocated_connections / n_workers;
   ...
   */
  if (tm->cfg.preallocated_half_open_connections)
    pool_init_fixed (tm->half_open_connections,
		     tm->cfg.preallocated_half_open_connections);
...
tcp_configuration_init (void)
{
  ...
  tcp_cfg.persist_min_time = 2; /* 0.2s */ ;
}
...
tcp_config_fn (vlib_main_t * vm, unformat_input_t * input)
{
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (input, "persist-min-time %d",
			 &tcp_cfg.persist_min_time))
	;
      else
  ...
  do
    {
      ...
      vec_add1 (tm->cfg.ip4_src_addrs, start[0]);
  ...
  while (1)
    {
      ...
      vec_add1 (tm->cfg.ip6_src_addrs, start[0]);
