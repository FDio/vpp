  ...
  if (is_ip4)
    {
      foreach_ip_interface_address (lm4, ia, sw_if_index, 1 /* unnumbered */ ,
				    (
				      {
				      return
				      ip_interface_address_get_address (lm4,
									ia);
				      }
				    ));
    }
  else
    {
      foreach_ip_interface_address (lm6, ia, sw_if_index, 1 /* unnumbered */ ,
				    (
				      {
				      return
				      ip_interface_address_get_address (lm6,
									ia);
				      }
				    ));
    }
...
tcp_init (vlib_main_t * vm)
{
  ...
  tm->log2_tstamp_clocks_per_tick =
    flt_round_nearest (log
		       (TCP_TSTAMP_RESOLUTION /
			vm->clib_time.seconds_per_clock) / log2);
