...
{
  f64 sample, prev_period = tm->dispatch_period[thread_index], a = 0.5;
...
{
  tcp_main_t *tm = vnet_get_tcp_main ();
  f64 avg_byte_rate,dispatch_period;
  u32 srtt = clib_max (tc->srtt, 1), burst_size;

  if (tc->cwnd == 0)
    os_panic ();
  dispatch_period = tm->dispatch_period[tc->c_thread_index];
  avg_byte_rate = ((f64) tc->cwnd / (f64) srtt) * THZ * 40;
  burst_size = clib_max ((u32)(avg_byte_rate * dispatch_period),
                         tc->snd_mss);
  transport_connection_set_pace_rate (&tc->connection, (u32) avg_byte_rate);
