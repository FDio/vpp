...
void
tcp_program_cleanup1 (tcp_worker_ctx_t * wrk, tcp_connection_t * tc,
		      const char *fnc)
{
  ...
  req->free_time = now + tcp_cfg.cleanup_time / 1000.0;
  clib_warning ("cleanup programmed for %u state %u %s", tc->c_c_index,
		tc->state, fnc);
}
...
tcp_dispatch_pending_timers (tcp_worker_ctx_t * wrk)
{
  ...
  for (i = 0; i < clib_min (n_timers, wrk->max_timers); i++)
    {
...
tcp_expired_timers_dispatch (u32 * expired_timers)
{
  ...
  wrk->max_timers = clib_max (n_left ? wrk->max_timers : 0, max_per_loop);
