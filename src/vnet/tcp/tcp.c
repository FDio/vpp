				 ...
				 dpo_id_t * result)
{
  ...
  if (tc->c_is_ip4)
  ...
  else
    {
      ...
      clib_memset (&hdr, 0, sizeof (hdr));
      hdr.ip.protocol = IP_PROTOCOL_TCP;
      _clib_memcpy (&hdr.ip.src_address, &tc->c_lcl_ip.ip6,
		    sizeof (ip6_address_t));
      _clib_memcpy (&hdr.ip.dst_address, &tc->c_rmt_ip.ip6,
		    sizeof (ip6_address_t));
      hdr.tcp.src_port = tc->c_lcl_port;
      hdr.tcp.dst_port = tc->c_rmt_port;
...
tcp_lookup_rmt_in_fib (tcp_connection_t * tc)
{
  ...
  _clib_memcpy (&prefix.fp_addr, &tc->c_rmt_ip, sizeof (prefix.fp_addr));
				 ...
				 u16 * lcl_port, u8 is_ip4)
{
  ...
  if (is_ip4)
	...
	tm->last_v6_address_rotor = 0;
      _clib_memcpy (&lcl_addr->ip6, &tm->ip6_src_addresses[index],
		    sizeof (ip6_address_t));
    }
