...
tcp_connection_reset (tcp_connection_t * tc)
{
  ...
  clib_warning ("RESET: %U", format_stream_session,
		&session_manager_main.sessions[tc->connection.thread_index][tc->connection.s_index], 2);
  switch (tc->state)
    {
...
{
  tcp_connection_t *tc = (tcp_connection_t *) trans_conn;
  return clib_min (tcp_snd_space (tc), tc->snd_wnd - (tc->snd_nxt - tc->snd_una));
}
