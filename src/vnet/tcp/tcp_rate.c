...
static int
tcp_tx_tracker_is_valid (tcp_tx_tracker_t *tt)
{
  ...
  while (txs)
      ...
      if (tmp)
	{
	  if (tmp->prev != tcp_tx_tracker_sample_index (tt, txs))
	    {
	      clib_warning ("next %u thinks prev is %u should be %u", txs->next, tmp->prev,
			       ...
			       u32 seq, u8 is_end)
{
  ...
  if (!is_end)
    {
      ...
      cur->min_seq = seq;
      rb_tree_add_custom (&tt->sample_lookup, cur->min_seq,
  ...
  if (prev)
    prev->next = TCP_TXS_INVALID_INDEX;
  tt->tail = prev_index;
  tcp_tx_tracker_free_sample (tt, cur);
...
tcp_track_rxt (tcp_connection_t * tc, u32 start, u32 end)
{
  ...
  if (!(txs != 0 && !txs->is_rxt && seq_geq (start, txs->min_seq)))
    os_panic();
  ...
  /* Tail completely overlapped */
  else
    {
      ...
      if (txs->next != TCP_TXS_INVALID_INDEX)
	os_panic ();
      txs->next = TCP_TXS_INVALID_INDEX;
...
//	{
//	  clib_warning ("FAILED");
//	  clib_warning ("cur %u txs %u next %u", cur_index, txs_index,
//	                tcp_tx_tracker_sample_index (tt, next));
//	  os_panic ();
//	}
      tcp_tx_tracker_validate (tt);
    }
...
tcp_tx_tracker_walk_samples (tcp_connection_t * tc, tcp_rate_sample_t * rs)
{
 ...
 if (next && next->min_seq == tc->snd_una)
  ...
  tt->head = tcp_tx_tracker_sample_index (tt, cur);
  tcp_tx_tracker_validate (tt);
}
				 ...
				 tcp_rate_sample_t * rs)
{
  ...
  for (i = 0; i < vec_len (blks); i++)
	...
	{
	  prev = cur;
	  prev_index = tcp_tx_tracker_sample_index (tt, cur);
	  cur = tcp_tx_tracker_next_sample (tt, cur);
	}
      else
	{
	  prev = tcp_tx_tracker_prev_sample (tt, cur);
	  prev_index = cur->prev;
	}
