...
tcp_update_rtt (tcp_connection_t * tc, u32 ack)
{
  ...
  else if (tcp_opts_tstamp (&tc->rcv_opts) && tc->rcv_opts.tsecr && tc->bytes_acked)
    {
		     ...
		     u32 prev_snd_wnd, u32 prev_snd_una, u8 *is_dack)
{
  ...
  *is_dack = tc->sack_sb.last_sacked_bytes
  ...
  return (*is_dack || tcp_in_fastrecovery (tc));
}
  ...
  if (PREDICT_TRUE (!tcp_in_fastrecovery (tc)))
    {
      ...
      /* If a cumulative ack, reset dupacks to 0 */
      tc->rcv_dupacks = 0;
      return;
    }

  /* recovery on */
  clib_warning ("we do get here");
...
tcp_should_fastrecover (tcp_connection_t *tc)
{
  return (tc->rcv_dupacks == TCP_DUPACK_THRESHOLD
      || (TCP_DUPACK_THRESHOLD - 1) * tc->snd_mss > tc->sack_sb.sacked_bytes);
}
...
tcp_cc_handle_event (tcp_connection_t * tc, u32 is_dack)
{
  ...
  if (is_dack)
	...
	{
	  /* RFC6582 heuristic to avoid multiple fast retransmits */
	  if (tc->rcv_opts.tsecr != tc->tsecr_last_ack)
	    {
	      tc->rcv_dupacks = 0;
	      return;
	    }
  ...
  /*
   * Legitimate ACK. See if we can exit recovery or if we can
   * retransmit some more data.
   */
  else
    {
      if (seq_geq(tc->snd_una, tc->snd_congestion))
      ...
      else if (tc->bytes_acked)
	{
	  ...
	  if (tc->sack_sb.last_bytes_delivered
	      && seq_gt(tc->snd_rtx, tc->snd_una))
	    {
	      /* If we have sacks and we haven't gotten an ack beyond snd_rtx,
	       * remove sacked bytes delivered */
	      tc->rtx_bytes -= tc->sack_sb.last_bytes_delivered;
	    }
	  else
	    {
	     ...
	     tcp_header_t * th, u32 * next, u32 * error)
{
  ...
  if (PREDICT_FALSE (seq_lt (vnet_buffer (b)->tcp.ack_number, tc->snd_una)))
      ...
      if (tcp_in_fastrecovery (tc) && tc->rcv_dupacks == TCP_DUPACK_THRESHOLD)
	{
	  ...
	  tcp_cc_handle_event (tc, vnet_buffer (b)->tcp.ack_number);
	}
      /* Don't drop yet */
      return 0;
    }
