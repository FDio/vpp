			      ...
			      u8 has_rxt)
{
  ...
  sb->rxt_sacked_max = seq_max (sb->rxt_sacked_max, end);
  sb->rxt_sacked +=
			  ...
			  u8 have_unsent, u8 * can_rescue, u8 * snd_limited)
{
  ...
  while (hole && seq_leq (hole->end, sb->cur_rxt) && hole->is_lost)
  ...
  if (hole && seq_lt (sb->cur_rxt, hole->start))
    sb->cur_rxt = hole->start;
...
tcp_cc_handle_event (tcp_connection_t * tc, tcp_rate_sample_t * rs,
		     u32 is_dack)
{
  u8 has_sack = tcp_opts_sack_permitted (&tc->rcv_opts), do_rxt = 0;
...
//  tcp_update_delivered (tc, is_dack, has_sack);
  ...
  if (has_sack)
	...
	- sb->last_bytes_delivered;
      ...
      do_rxt = tc->bytes_acked || sb->last_sacked_bytes;

      /* If retransmits are sacked, some were probably lost. Try to retransmit
       * the head */
      if (sb->rxt_sacked && seq_gt (sb->rxt_sacked_max, tc->snd_una))
	{
	  clib_warning ("sack head lost");
	...
	{
	  /* If partial ack, assume that the first un-acked
	   * segment was lost */
	  tcp_fastrecovery_first_on (tc);
	  do_rxt = 1;
	}
  ...
  /*
   * No cumulative ack progress. If something was delivered, program
   * output for more retransmits. Otherwise, we're done
   */
  if (!tc->bytes_acked)
    {
//      ASSERT (is_dack);
  ...
  /*
   * Partial ACK. Notify cc of the event and program output for
   * retransmit because something was delivered
   */
