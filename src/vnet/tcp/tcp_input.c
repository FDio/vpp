...
static void
tcp_input_set_error_next (tcp_main_t *tm, u32 *next0, u32 *error0, u8 is_ip4)
{
	  ...
	  if (PREDICT_TRUE (!tcn0 + !tcn1 == 0))
	    {
	      ...
	      tcp_input_dispatch_buffer (tm, tc0, b0, &next0, &error0);
	      tcp_input_dispatch_buffer (tm, tc1, b1, &next1, &error1);
	    }
	  else
	    {
		...
		tcp_input_set_error_next (tm, &next0, &error0, is_ip4);
		...
		tcp_input_set_error_next (tm, &next1, &error1, is_ip4);
	    }
