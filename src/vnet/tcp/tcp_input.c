...
tcp_estimate_initial_rtt (tcp_connection_t * tc)
{
  ...
  /* Connections are initiated on main thread so use its time */
  if (tc->rtt_ts)
    {
      f64 now = tcp_time_now_us (vlib_num_workers ()? 1 : 0);
      tc->mrtt_us = now - tc->rtt_ts;
      clib_warning ("initial = %.9f", tc->mrtt_us);
      mrtt = clib_max ((u32) (tc->mrtt_us * THZ), 1);
      tc->rtt_ts = 0;
    }
  else
    {
      mrtt = tcp_time_now_w_thread (tc->c_thread_index) - tc->rcv_opts.tsecr;
