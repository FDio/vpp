		       ...
		       tcp_header_t * th0, vlib_buffer_t * b0, u8 is_ip4)
{
  if (tc0)
    {
      _clib_memcpy (&t0->tcp_connection, tc0, sizeof (t0->tcp_connection));
    }
  else
    {
		       ...
		       vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      if (tcp_options_parse (tcp0, &tc0->rcv_opts))
	{
      ...
      _clib_memcpy (new_tc0, tc0, sizeof (*new_tc0));
	...
	{
	  t0 = vlib_add_trace (vm, node, b0, sizeof (*t0));
	  _clib_memcpy (&t0->tcp_header, tcp0, sizeof (t0->tcp_header));
	  _clib_memcpy (&t0->tcp_connection, tc0,
		     ...
		     vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
	...
	{
	  _clib_memcpy (&child0->c_lcl_ip6, &ip60->dst_address,
			sizeof (ip6_address_t));
	  _clib_memcpy (&child0->c_rmt_ip6, &ip60->src_address,
			sizeof (ip6_address_t));
	}
      ...
      if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
	{
	  t0 = vlib_add_trace (vm, node, b0, sizeof (*t0));
	  _clib_memcpy (&t0->tcp_header, th0, sizeof (t0->tcp_header));
	  _clib_memcpy (&t0->tcp_connection, lc0,
			sizeof (t0->tcp_connection));
	}
