...
{
  u32 opts_len = (tcp_doff (th) << 2) - sizeof (tcp_header_t);

  /* Accept at most timestamp option */
  if (opts_len > (TCP_OPTION_LEN_TIMESTAMP + 2))
    return 0;

  if (opts_len && (tcp_options_parse (th, &tc->rcv_opts, 1) ||
...
static void
tcp_input_set_error_next (tcp_main_t *tm, tcp_worker_ctx_t *wrk, u8 is_ip4)
{
		    ...
		    vlib_frame_t * frame, int is_ip4, u8 is_nolookup)
{
  ...
  while (n_left_from >= 4)
      ...
      else
	{
	  if (PREDICT_TRUE (tc0 != 0))
	    ...
	    {
	      tcp_input_set_error_next (tm, wrk, is_ip4);
	    }
  ...
  if (PREDICT_FALSE (node->flags & VLIB_NODE_FLAG_TRACE))
    tcp_input_trace_frame (vm, node, bufs, 0 /*XXX*/, frame->n_vectors,
			   is_ip4);
