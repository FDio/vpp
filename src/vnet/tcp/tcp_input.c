		    ...
		    vlib_frame_t * frame, int is_ip4)
{
  ...
  while (n_left_from > 4)
...

	{
      ...
      vnet_buffer (b[0])->tcp.flags = 0;
      vnet_buffer (b[1])->tcp.flags = 0;
      next[0] = next[1] = TCP_INPUT_NEXT_DROP;

      tcn0 = tcp_input_lookup_connection (b[0], thread_index, &error0,
                                          is_ip4);
      tcn1 = tcp_input_lookup_connection (b[1], thread_index, &error1,
                                          is_ip4);

      if (PREDICT_TRUE (!tcn0 + !tcn1 == 0))
	{
