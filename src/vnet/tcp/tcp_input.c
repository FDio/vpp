...
#define tcp_established_next_drop(is_ip4) (is_ip4 ? TCP_ESTABLISHED_NEXT_DROP4    \
                                                  : TCP_ESTABLISHED_NEXT_DROP6)
#define tcp_rcv_process_next_drop(is_ip4) (is_ip4 ? TCP_RCV_PROCESS_NEXT_DROP4    \
                                                  : TCP_RCV_PROCESS_NEXT_DROP6)
#define tcp_syn_sent_next_drop(is_ip4) (is_ip4 ? TCP_SYN_SENT_NEXT_DROP4          \
                                               : TCP_SYN_SENT_NEXT_DROP6)
#define tcp_listen_next_drop(is_ip4) (is_ip4 ? TCP_LISTEN_NEXT_DROP4              \
                                             : TCP_LISTEN_NEXT_DROP6)
			  ...
			  vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  u32 next0 = tcp_established_next_drop (is_ip4), error0 =
	    TCP_ERROR_ENQUEUED;
		       ...
		       vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  u32 next0 = tcp_syn_sent_next_drop (is_ip4), error0 =
	    TCP_ERROR_ENQUEUED;
			  ...
			  vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  tcp_header_t *tcp0 = 0;
	  tcp_connection_t *tc0;
	  u32 next0 = tcp_rcv_process_next_drop (is_ip4), error0 =
	    TCP_ERROR_ENQUEUED;
	  u8 is_fin0;
		     ...
		     vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  u32 error0 = TCP_ERROR_SYNS_RCVD, next0 =
	    tcp_listen_next_drop (is_ip4);
