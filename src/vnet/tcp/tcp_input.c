			 ...
			 u8 is_ip4, u8 is_nolookup)
{
  ...
  if (is_nolookup)
    tcp_conn =
      tcp_connection_get (vnet_buffer (b)->tcp.connection_index,
			  thread_index);
  else
    tcp_conn = tcp_get_connection_from_transport (tc);
		    ...
		    vlib_frame_t * frame, int is_ip4, u8 is_nolookup)
{
  ...
  while (n_left_from >= 4)
	...
	CLIB_PREFETCH (b[3]->data, 2 * CLIB_CACHE_LINE_BYTES, LOAD);
      ...
      tc0 =
	tcp_input_lookup_buffer (b[0], thread_index, &error0, is_ip4,
				 is_nolookup);
      tc1 =
	tcp_input_lookup_buffer (b[1], thread_index, &error1, is_ip4,
				 is_nolookup);
