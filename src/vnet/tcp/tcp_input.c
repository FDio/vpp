...
static void
tcp_input_set_error_next (tcp_main_t * tm, vlib_buffer_t * b, u16 * next,
			  vlib_node_runtime_t * error_node, u32 error,
			  u8 is_ip4)
{
  ...
  while (n_left_from >= 4)
      ...
      else
	{
	  if (PREDICT_TRUE (tc0 != 0))
	    ...
	    tcp_input_set_error_next (tm, b[0], &next[0], error_node, error0,
				      is_ip4);
	  ...
	  if (PREDICT_TRUE (tc1 != 0))
	    ...
	    tcp_input_set_error_next (tm, b[1], &next[1], error_node, error1,
				      is_ip4);
	}
