...
/* Account ACK feedback while recovery continues. */
always_inline void
...
static void
tcp_cc_transition_recovery (tcp_connection_t *tc, tcp_rate_sample_t *rs, u8 has_sack)
{
		     ...
		     u32 is_dack)
{
  ...
  if (seq_geq (tc->snd_una, tc->snd_congestion))
    {
      tcp_cc_transition_recovery (tc, rs, has_sack);
      return;
    }
