...
static_always_inline void
tcp_gro_set_candidate (vlib_main_t *vm, tcp_connection_t *tc,
		       vlib_buffer_t *b)
{
...
static inline void
tcp_input_dispatch_buffer (vlib_main_t *vm, tcp_main_t *tm,
		    ...
		    vlib_frame_t * frame, int is_ip4, u8 is_nolookup)
{
  ...
  while (n_left_from >= 4)
      ...
      if (PREDICT_TRUE (!tc0 + !tc1 == 0))
	{
	  ...
	  tcp_input_dispatch_buffer (vm, tm, tc0, b[0], bi[0], &next[0],
				     &err_counters);
	  tcp_input_dispatch_buffer (vm, tm, tc1, b[1], bi[1], &next[1],
				     &err_counters);
