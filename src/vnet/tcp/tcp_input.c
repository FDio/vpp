		       ...
		       vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
    {
      u32 bi0, ack0, seq0, error0 = TCP_ERROR_ENQUEUED;
	...
	goto drop;
      ...
      /* SYNs, FINs and data consume sequence numbers */
      vnet_buffer (b0)->tcp.seq_end = seq0 + tcp_is_syn(tcp0) + tcp_is_fin(tcp0)
...
tcp46_rcv_process_inline (vlib_main_t * vm, vlib_node_runtime_t * node,
			  vlib_frame_t * from_frame, int is_ip4)
{
  u32 n_left_from, *from, n_fins = 0, *first_buffer;
  ...
  while (n_left_from > 0)
      ...
      if (CLIB_DEBUG)
	{
	  ...
	  tmp = tcp_lookup_connection (tc0->c_fib_index, b0, my_thread_index,
		                       is_ip4);
