...
tcp_input_set_error_next (tcp_main_t *tm, tcp_worker_ctx_t *wrk,
			  vlib_buffer_t *b, u32 bi, u32 error, u8 is_ip4)
{
  if (b->flags & VLIB_BUFFER_IS_TRACED)
  ...
  while (n_left_from >= 4)
      ...
      else
	{
	  if (PREDICT_TRUE (tc0 != 0))
	    ...
	    {
	      tcp_input_set_error_next (tm, wrk, b[0], bi[0], error0, is_ip4);
	    }
	  ...
	  if (PREDICT_TRUE (tc1 != 0))
	    ...
	    {
	      tcp_input_set_error_next (tm, wrk, b[1], bi[1], error1, is_ip4);
	    }
	}
