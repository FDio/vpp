...
{
  clib_warning ("%u congestion undo", tc->c_c_index);
...
tcp_cc_recover (tcp_connection_t * tc)
{
  ...
  if (tc->sack_sb.sacked_bytes)
    {
      clib_warning ("%u exit congested", tc->c_c_index);
      tc->snd_congestion = tc->snd_nxt;
      tc->snd_rxt_bytes = 0;
      tc->snd_rxt_ts = 0;
  ...
  if (has_sack)
    {
      ...
      if (tc->sack_sb.rxt_sacked > tc->snd_rxt_bytes - tc->rxt_delivered)
	os_panic ();
...
//					 0 /* start bucket */ );
	  tcp_connection_tx_pacer_reset (tc, tc->cwnd,
					 0 /* start bucket */ );
	  tcp_program_retransmit (tc);
	  return;
	}
