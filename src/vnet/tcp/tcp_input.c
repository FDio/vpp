	...
	case TCP_STATE_FIN_WAIT_1:
	  ...
	  /* If FIN is ACKed */
	  else if (tc0->snd_una == tc0->snd_una_max)
	    {
	      ...
	      /* We send our FIN only after the queue was drained so this
	       * acks only our fin, don't try to deq */
	      tc0->flags &= ~TCP_CONN_DEQ_PENDING;
	      vec_del1 (wrk->pending_deq_acked,
			vec_len (wrk->pending_deq_acked) - 1);
	    }
	  break;
