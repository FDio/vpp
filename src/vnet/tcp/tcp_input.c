...
scoreboard_update_bytes (tcp_connection_t * tc, sack_scoreboard_t * sb)
{
  ...
  right = left;
  while ((left = scoreboard_prev_hole (sb, left))
...
void
tcp_scoreboard_recover (tcp_connection_t * tc)
{
  sack_scoreboard_hole_t *hole = scoreboard_first_hole (&tc->sack_sb);
  if (!hole)
    return;

  if (hole->start == tc->snd_una && hole->end == tc->snd_una_max)
