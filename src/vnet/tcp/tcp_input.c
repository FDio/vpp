...
tcp_rcv_sacks (tcp_connection_t * tc, u32 ack)
{
  ...
  u32 bytes_acked = ack - tc->snd_una;
  if (bytes_acked - sb->last_bytes_delivered + sb->last_sacked_bytes < sb->rxt_sacked)
    {
      clib_warning ("acked %u delivered %u sacked %u rxt %u", bytes_acked,
                    sb->last_bytes_delivered, sb->last_sacked_bytes, sb->rxt_sacked);
      os_panic ();
    }
...
{
  clib_warning ("%u congestion undo");
...
tcp_cc_recover (tcp_connection_t * tc)
{
  ...
  if (tc->sack_sb.lost_bytes || tc->sack_sb.sacked_bytes > 2 * tc->snd_mss)
    {
...
static void
tcp_update_delivered (tcp_connection_t * tc, u8 has_sack)
{
  if (has_sack)
    {
...
//	  /* what about last sacked under snd_congestion */
//	  if (tc->snd_rxt_bytes > rxt_delivered)
//	    tc->snd_rxt_bytes -= rxt_delivered;
//	  else
//	    tc->snd_rxt_bytes = 0;
//	  if (tc->snd_rxt_bytes > tc->sack_sb.lost_bytes)
//	    {
//	      clib_warning ("%U", format_tcp_connection, tc, 2);
//	      os_panic ();
//	    }
	  if (tc->sack_sb.rxt_sacked > tc->snd_rxt_bytes)
	    os_panic ();
	  tc->snd_rxt_bytes -= tc->sack_sb.rxt_sacked;
	}
      else
	{
		     ...
		     u32 is_dack)
{
  ...
  if (tcp_in_cong_recovery (tc))
	...
	tc->rcv_dupacks++;
      ...
      if (!tc->bytes_acked)
	{
	  tcp_update_delivered (tc, has_sack);
	  tcp_program_retransmit (tc);
	  if (!has_sack)
