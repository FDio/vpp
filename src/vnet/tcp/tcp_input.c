			  ...
			  u16 data_len)
{
  ...
  /* REMOVE */
//  stream_session_t *s0 = stream_session_get (tc->c_s_index,
//						 tc->c_thread_index);
//
//  /* Get the newest segment from the fifo */
//  svm_fifo_t *f = s0->server_rx_fifo;
//  if (f->tail % f->nitems != tc->rcv_nxt % f->nitems)
//    os_panic ();
//  if (f->ooos_list_head != OOO_SEGMENT_INVALID_INDEX)
//	{
//	  u32 start, end;
//	  ooo_segment_t *head = 0;
//
//	  head = pool_elt_at_index (f->ooo_segments, f->ooos_list_head);
//	  start = ooo_segment_offset (s0->server_rx_fifo, head);
//	  end = ooo_segment_end_offset (s0->server_rx_fifo, head);
//	  if (seq_leq(start, tc->rcv_nxt))
//	    {
//	      fformat (
//		  stdout,
//		  "(1) start %u rcv_nxt %u tail %u data_len %d written %d\n",
//		  start, tc->rcv_nxt, end, data_len, written);
//	      fformat (stdout, "seg: %U", format_ooo_segment, head);
//	      fformat (stdout, "%U", format_svm_fifo, s0->server_rx_fifo, 2);
//	      os_panic ();
//	    }
//	}
  /* END REMOVE */
			 ...
			 u16 data_len)
{
  ...
  /* Enqueue out-of-order data with absolute relative */
...
//	  if (seq_leq(start, tc->rcv_nxt))
//	    {
//	      fformat (stdout, "(2) start %u rcv_nxt %u", start, tc->rcv_nxt);
//	      fformat (stdout, "%U", format_svm_fifo, s0->server_rx_fifo, 2);
//	      os_panic ();
//	    }
	  /* END DELETE */
