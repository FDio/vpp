			  ...
			  vlib_frame_t * frame, int is_ip4)
{
  ...
  b = bufs;
  // clib_warning ("\n\nfree need\n");
  // clib_warning ("vlib buffer index %d ",from[0]);
  // clib_warning ("vlib buffer index %d ",from[1]);
  ...
  u8 need_free = 1;
  // clib_warning ("swrk->trans_tail %d, from 0x%x  %d",swrk->rx_trans_tail,
  // from, frame->n_vectors );
  while (n_left_from > 0)
    {
      ...
      if (vnet_buffer (b[0])->tcp.data_len)
	{
	  ...
	  //   clib_warning ("error %d\n",error);
	  // if((error == TCP_ERROR_ENQUEUED) || (error ==
	  // TCP_ERROR_PARTIALLY_ENQUEUED)|| (error == TCP_ERROR_ZERO_RWND) ||
  ...
  if (PREDICT_FALSE (swrk->rx_dma_enabled))
	...
	{
	  //  clib_warning("establish rx_batch_num %d", swrk->rx_batch_num);
  ...
  if (PREDICT_FALSE (swrk->rx_dma_enabled))
    {
      if (swrk->rx_batch_num)
	{
	  // clib_warning("syn rx_batch_num %d", swrk->rx_batch_num);
	  vlib_dma_batch_set_cookie (vm, swrk->rx_batch, swrk->rx_trans_tail);
	  swrk->rx_batch_num = 0;
	  swrk->rx_trans_tail++;
	  if (swrk->rx_trans_tail == swrk->rx_trans_size)
