...
scoreboard_update_lost (tcp_connection_t * tc, sack_scoreboard_t * sb)
{
  ...
  bytes = seq_lt (sb->high_sacked, hole->end)
      ? hole->end - sb->high_sacked : 0;

  while ((prev = scoreboard_prev_hole (sb, hole))
	  && (bytes < (TCP_DUPACK_THRESHOLD - 1) * tc->snd_mss
	      && holes < TCP_DUPACK_THRESHOLD))
    {
			  ...
			  u8 *snd_limited)
{
  ...
  while (hole && seq_lt (hole->end, sb->high_rxt) && hole->is_lost)
...
tcp_cc_update (tcp_connection_t * tc, vlib_buffer_t * b)
{
  ...
  tc->snd_rxt_bytes -= clib_min(bytes_advanced, tc->snd_rxt_bytes);
  tcp_update_rto (tc);
