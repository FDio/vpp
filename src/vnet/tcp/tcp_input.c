  ...
  while (n_left_from > 0)
      ...
      else
	{
      ...
      child0 = tcp_lookup_connection (lc0->c_fib_index, b0, my_thread_index,
				      is_ip4);
      if (PREDICT_FALSE
	  (child0->state != TCP_STATE_LISTEN)
	  && child0->state != TCP_STATE_TIME_WAIT)
	{
	  error0 = TCP_ERROR_CREATE_EXISTS;
	  goto drop;
	}

      if (PREDICT_FALSE (child0->state == TCP_STATE_TIME_WAIT))
	{
	  /* clean up the old session */
	  tcp_connection_del (child0);
	}
