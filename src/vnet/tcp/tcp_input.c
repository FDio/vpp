  ...
  if (lost)
    {
      ...
      /* Recovery changes reo_wnd; refresh losses and the next timeout. */
      if (!was_in_recovery)
...
tcp_cc_account_recovery_ack (tcp_connection_t *tc, tcp_rate_sample_t *rs, u32 is_dack, u8 has_sack)
{
  if (has_sack)
	...
	tcp_fastrecovery_first_on (tc);
      ...
      tcp_cc_account_rxt_delivery (tc, tc->sack_sb.rxt_sacked);
      ...
      if (is_dack)
	{
      ...
      tcp_cc_account_rxt_delivery (tc, tc->bytes_acked);
      if (is_dack)
