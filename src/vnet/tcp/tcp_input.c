			      ...
			      u8 has_rxt)
{
  ...
  sb->rxt_max_sacked = seq_max (sb->rxt_max_sacked, end);
  sb->rxt_sacked +=
...
//  tcp_update_delivered (tc, is_dack, has_sack);
  ...
  if (has_sack)
	...
	- sb->last_bytes_delivered;
      ...
      /*
       * If retransmits are sacked, some were probably lost. Count those
       * retransmits as lost and program output to try again.
       */
      if (sb->rxt_sacked && sb->rxt_max_sacked != tc->snd_una)
	{
	  sack_scoreboard_hole_t *hole;

	  hole = scoreboard_first_hole (sb);
	  while (hole && seq_leq (hole->end, sb->rxt_max_sacked))
	    {
	      if (hole->is_lost)
		continue;
	      hole = scoreboard_next_hole (sb, hole);
	  ...
	  sb->cur_rxt_hole = sb->head;
	  sb->cur_rxt = tc->snd_una;
	  // lost rxt
	}

      if (tc->bytes_acked || sb->last_sacked_bytes)
	{
	  do_rxt = 1;
	}
    }
