  ...
  if (rv)
    {
      ...
      /* DELETE */
      tcp_send_reset (tc);
      clib_warning ("seq %u seq-end %u", vnet_buffer (b)->tcp.seq_number - tc->irs,
		    vnet_buffer (b)->tcp.seq_end - tc->irs);
      clib_warning ("SEND RESET: %U", format_stream_session,
		    &session_manager_main.sessions[tc->c_thread_index][tc->c_s_index], 2);
      return TCP_ERROR_FIFO_FULL;
    }
