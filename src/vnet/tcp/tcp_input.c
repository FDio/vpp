                       ...
                       vlib_buffer_t **bufs, u32 n_bufs, u8 is_ip4)
{
  ...
  for (i = 0; i < clib_min(n_trace, n_bufs); i++)
    {
      t0 = vlib_add_trace (vm, node, bufs[i], sizeof(*t0));
      tc0 = tcp_connection_get (vnet_buffer (bufs[i])->tcp.connection_index,
	                        vm->thread_index);
      tcp0 = vlib_buffer_get_current (bufs[i]);
      tcp_set_rx_trace_data (t0, tc0, tcp0, bufs[i], is_ip4);
    }
}

static void
tcp_input_set_error_next (tcp_main_t *tm, u16 *next0, u32 *error0, u8 is_ip4)
{
  if (*error0 == TCP_ERROR_FILTERED)
    {
...
tcp_input_lookup_connection (vlib_buffer_t *b, u8 thread_index, u32 *error,
                             u8 is_ip4)
{
...
static inline void
tcp_input_dispatch_buffer (tcp_main_t *tm, tcp_connection_t *tc0,
                           vlib_buffer_t *b0, u16 *next0, u32 *error0)
{
  tcp_header_t *tcp0;
  u8 flags0;
  ...
  while (n_left_from > 4)
    {
      ...
      u32 error1 = TCP_ERROR_NO_LISTENER;

	{
      ...
      tc0 = tcp_input_lookup_connection (b[0], thread_index, &error0, is_ip4);
      tc1 = tcp_input_lookup_connection (b[1], thread_index, &error1, is_ip4);
