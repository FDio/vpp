...
{
  u32 new_snd_wnd;
  u8 should_drop = 0, old_dup_ack = 0;
...
//      return -1;
  ...
  if (old_dup_ack || tcp_ack_is_dupack (tc, b, new_snd_wnd))
    {
		 ...
		 u16 n_data_bytes, u32 * next0)
{
  ...
  if (PREDICT_FALSE (vnet_buffer (b)->tcp.seq_number != tc->rcv_nxt))
    {
      ASSERT (seq_geq(vnet_buffer (b)->tcp.seq_number, tc->rcv_las));

      /* Old sequence numbers allowed through because they were gt rcv_las*/
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (tcp_rcv_ack (tc0, b0, th0, &next0, &error0))
	    {
	      if (error0 == TCP_ERROR_ACK_INVALID)
		invalid_acks++;
	      goto drop;
	    }
