...
tcp_cc_recover (tcp_connection_t * tc)
{
  ...
  if (tc->sack_sb.sacked_bytes)
    {
      clib_warning ("%u recovered congested", tc->c_c_index);
      tc->snd_congestion = tc->snd_nxt;
      tc->snd_rxt_ts = tcp_tstamp (tc);
//      if (tcp_opts_sack_permitted (&tc->rcv_opts))
  ...
  if (has_sack)
    {
      ...
      tc->rxt_delivered += sb->rxt_sacked;
      tc->prr_delivered += tc->bytes_acked + sb->last_sacked_bytes
	- sb->last_bytes_delivered;

      tc->rcv_dupacks += is_dack
			 && !(tc->bytes_acked || sb->last_sacked_bytes);

//      if (0.7 * (tc->bytes_acked + sb->last_sacked_bytes - sb->last_bytes_delivered) > tc->snd_mss
//	  && tcp_flight_size (tc) > tc->ssthresh
//	  && tcp_fastrecovery_prr_snd_space (tc) < tc->snd_mss)
//	os_panic ();
//      tc->rcv_dupacks += is_dack && !tc->bytes_acked;
//      tc->rcv_dupacks += is_dack;

      /* WHAT ABOUT recently lost ?? */

//      if (sb->rxt_sacked && seq_gt (sb->rxt_sacked_max, tc->snd_una))
//      u8 high_sacked = seq_gt (sb->high_sacked, tc->snd_una + 3 * tc->snd_mss);
//      if (tc->bytes_acked && high_sacked)

//      u8 new_sack = seq_gt (sb->high_sacked, tc->snd_congestion + 2 * tc->snd_mss) || tc->rxt_delivered;
//      if (new_sack && (sb->last_sacked_bytes || sb->last_sacked_bytes))

//      if (tc->bytes_acked || (new_sack && sb->last_sacked_bytes))
//      if ((high_sack || tc->rxt_delivered) && (tc->bytes_acked || sb->last_sacked_bytes))

//      if (tc->rcv_dupacks > 30 && timestamp_leq (tc->snd_rxt_ts + 82, tcp_time_now_w_thread (tc->c_c_index)))
//	os_panic ();

//      if (timestamp_leq (tc->snd_rxt_ts + 0.1 * tc->srtt, tc->rcv_opts.tsecr))
//	{
//	  tc->snd_rxt_ts = tcp_tstamp (tc);
//	  scoreboard_init_rxt (&tc->sack_sb, tc->snd_una);
//
//	  clib_warning ("%u prr diff %u new %u diff2 %u rxt_delivered %u sent %u lost %u",
//	                tc->c_c_index,
//	                tc->prr_delivered - tc->snd_rxt_bytes
//	                + (tc->snd_nxt - tc->snd_congestion),
//	                tc->snd_nxt - tc->snd_congestion,
//	                tc->ssthresh - tcp_flight_size (tc),
//	                tc->rxt_delivered, tc->snd_rxt_bytes, sb->lost_bytes);
//
//	  ASSERT (tc->rxt_delivered <= tc->snd_rxt_bytes);
//	  tc->prr_delivered += tc->snd_rxt_bytes - tc->rxt_delivered;
//	  tc->rxt_delivered = tc->snd_rxt_bytes;
//	}

//      u32 delivered = tc->bytes_acked + sb->last_sacked_bytes
//		- sb->last_bytes_delivered;
//      if (seq_gt (sb->high_sacked, tc->snd_congestion + 2 * tc->snd_mss)

//      if (timestamp_leq (tc->snd_rxt_ts, tc->rcv_opts.tsecr)
//	  && sb->rxt_sacked && sb->rxt_sacked < delivered)

      if (timestamp_lt (tc->snd_rxt_ts, tc->rcv_opts.tsecr)
	  && (sb->rxt_sacked && !tc->bytes_acked)
	  && tc->rxt_head != tc->snd_una)
	{
	  tcp_fastrecovery_first_on (tc);
	  clib_warning("%u sack head lost %u dack %u", tc->c_c_index, tc->bytes_acked, tc->rcv_dupacks);
	  tc->rxt_head = tc->snd_una;
	  if (seq_lt (tc->snd_una, sb->high_rxt))
	    {
	      tc->rxt_delivered += tc->snd_mss;
	      ASSERT(tc->rxt_delivered <= tc->snd_rxt_bytes);
	      tc->prr_delivered += tc->snd_mss;
	    }
	}
//      transport_connection_tx_pacer_reset_bucket (&tc->connection,
//                                                  vlib_get_main ()->clib_time.last_cpu_time);

      /* If have rxt and sack something beyond head is lost?*/
//      else if (tc->bytes_acked && sb->last_sacked_bytes)
//	{
//	  if (tc->rxt_head != tc->snd_una && seq_lt (tc->snd_una, sb->high_rxt))
//	    {
//	      tcp_fastrecovery_first_on (tc);
//	      tc->rxt_head = tc->snd_una;
//	      /* Assume the lost rxt segment is out of the network */
////	      if (seq_lt (tc->snd_una, sb->high_rxt))
////		{
//		  clib_warning ("%u sack head lost %u", tc->c_c_index, tc->bytes_acked);
//		  tc->rxt_delivered += tc->snd_mss;
//		  ASSERT (tc->rxt_delivered <= tc->snd_rxt_bytes);
//		  tc->prr_delivered += tc->snd_mss;
////		}
