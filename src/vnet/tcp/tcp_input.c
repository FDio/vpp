	  ...
	  if (PREDICT_FALSE (tc0->flags & TCP_CONN_HALF_OPEN_DONE))
	    {
	      ...
	      ASSERT (tcp_lookup_connection (tc0->c_vrf, b0,
	  ...
	  if (CLIB_DEBUG)
	    {
	      ...
	      tmp = tcp_lookup_connection (tc0->c_vrf, b0, my_thread_index,
	                                   is_ip4);
		     ...
		     vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  child0 = tcp_lookup_connection (lc0->c_vrf, b0, my_thread_index,
	                                  is_ip4);
