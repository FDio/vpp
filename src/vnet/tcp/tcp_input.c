...
{
  return (tc->snd_rtx_ts
	  && tcp_opts_tstamp (&tc->rcv_opts)
	  && timestamp_lt (tc->rcv_opts.tsecr, tc->snd_rtx_ts));
}
...
tcp_cc_update (tcp_connection_t * tc, vlib_buffer_t * b)
{
  ...
  tc->snd_rtx_bytes -= clib_min(bytes_advanced, tc->snd_rtx_bytes);
  tcp_update_rto (tc);
...
tcp_cc_handle_event (tcp_connection_t * tc, u32 is_dack)
{
  ...
  if (is_dack)
      ...
      if (tc->rcv_dupacks > TCP_DUPACK_THRESHOLD)
	{
	  ...
	  /* Pure duplicate ack. If some data got acked, handle it lower */
	  if (!tc->bytes_acked)
	    {
  ...
  if (seq_geq(tc->snd_una, tc->snd_congestion))
    {
      clib_warning ("full ack %d", tc->bytes_acked);
      ...
      if (tcp_cc_is_spurious_retransmit (tc))
	{
      ...
      /* Deflate RTO if we're post RTO timeout */
   ...
   */
  clib_warning ("partial ack %d", tc->bytes_acked);
  ...
  if (tc->sack_sb.last_bytes_delivered && seq_gt(tc->snd_rtx_seq, tc->snd_una))
    {
      /* If we have sacks and we haven't gotten an ack beyond snd_rtx,
       * remove sacked bytes delivered */
      tc->snd_rtx_bytes -= tc->sack_sb.last_bytes_delivered;
    }
  else
    {
