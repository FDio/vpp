...
#define tcp_inc_counter(node_id, err, count)				\
  tcp_node_inc_counter_i (vm, tcp4_##node_id##_node.index,		\
	                   ...
	                   err, count)
#define TCP_N_RCV_COUNTERS 5
#define tcp_maybe_inc_rcv_counter(cnts, err)				\
{									\
  ASSERT (ARRAY_LEN(cnts) < err - TCP_ERROR_ENQUEUED);			\
  cnts[err - TCP_ERROR_ENQUEUED] += (next0 != tcp_next_drop (is_ip4));	\
}
#define tcp_store_rcv_counters(node_id, cnts)				\
{									\
  int i;								\
  for (i = 0; i < ARRAY_LEN (cnts); i++)				\
    if (cnts[i])							\
      tcp_inc_counter(node_id, i + TCP_ERROR_ENQUEUED, cnts[i]);	\
}
			  ...
			  vlib_frame_t * from_frame, int is_ip4)
{
  ...
  u32 rcv_counters[TCP_N_RCV_COUNTERS] = {0};
  u8 is_fin = 0;
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (tcp_segment_validate (vm, tc0, b0, th0, &next0,
	                                           &error0)))
	    {
	      TCP_EVT_DBG (TCP_EVT_SEG_INVALID, tc0,
			   ...
			   vnet_buffer (b0)->tcp.seq_end);
	      tcp_maybe_inc_counter (established, error0, 1);
	      goto done;
	    }
	  ...
	  if (vnet_buffer (b0)->tcp.data_len)
	    {
	      ...
	      tcp_maybe_inc_rcv_counter (rcv_counters, error0);
	    }
	  ...
	  if (PREDICT_FALSE (is_fin))
		...
		tc0->rcv_nxt += 1;
	      ...
	      tcp_inc_counter (established, TCP_ERROR_FIN_RCVD, 1);
	    }
  ...
  errors = session_manager_flush_enqueue_events (TRANSPORT_PROTO_TCP,
						 my_thread_index);
  tcp_inc_counter (established, TCP_ERROR_EVENT_FIFO_FULL, errors);
  tcp_store_rcv_counters (established, rcv_counters);
