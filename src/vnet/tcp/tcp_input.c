...
always_inline int
tcp_try_header_prediction (tcp_worker_ctx_t *wrk, tcp_connection_t *tc, vlib_buffer_t *b,
			   tcp_header_t *th, u32 *error)
{
  ...
  if (opts_len)
    {
      if (opts_len != TCP_OPTION_TIMESTAMP_TLEN || !tcp_opts_tstamp (&tc->rcv_opts) ||
 ...
 */
always_inline int
tcp_should_try_header_prediction (tcp_worker_ctx_t *wrk, tcp_connection_t *tc, vlib_buffer_t *b,
				  tcp_header_t *th)
{
...
always_inline uword
tcp46_coalesce_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
