...
{
  /* This looks like it should work, but it blows up..
     vnet_disconnect_session (~0 / * client_index * /,
...
builtin_server_rx_callback (stream_session_t * s, session_fifo_event_t *e)
{
  ...
  /* 
   * $$$$ the following name is misleading, to say the least. The function
   * pulls bytes off the server_TX_fifo.
   */
  n_written = session_fifo_rx_dequeue (vm, node, smm, e, s, s->thread_index, 
                                       &n_tx_packets);
