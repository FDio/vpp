...
{
  int n_written, max_dequeue, max_enqueue, max_transfer;
  int actual_transfer;
  svm_fifo_t *tx_fifo;
  builtin_server_main_t *bsm = &builtin_server_main;
  session_fifo_event_t evt;
  static int serial_number = 0, byte_index = 0;
  int i;
  ...
  if (PREDICT_FALSE (max_dequeue <= 0))
    {
      clib_warning ("bizarre rx callback: bytes %d", max_dequeue);
      return 0;
    }
  ...
  if (PREDICT_FALSE (max_transfer <= 0))
    {
      clib_warning ("no space in tx fifo, event had %d bytes", max_dequeue);
      return 0;
    }
  ...
  actual_transfer = svm_fifo_dequeue_nowait (s->server_rx_fifo, 0, max_transfer,
					     bsm->rx_buf);
  ASSERT (actual_transfer == max_transfer);

  for (i = 0; i < actual_transfer; i++)
    {
      if (bsm->rx_buf[i] != ((byte_index + i) & 0xff))
	{
	  clib_warning ("expected %d got %d", (byte_index + i) & 0xff, bsm->rx_buf[i]);
	}

      if (byte_index + i == 65535)
	{
	  clib_warning ("byte == %d", bsm->rx_buf[i]);
	}
    }

  byte_index += actual_transfer;

  /*
   * Echo back
   */

  n_written = svm_fifo_enqueue_nowait (tx_fifo, 0, actual_transfer, bsm->rx_buf);
