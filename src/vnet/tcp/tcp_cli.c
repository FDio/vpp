...
format_tcp_vars (u8 * s, va_list * args)
{
  ...
  s = format (s, " rto %u rto_boff %u srtt %u us %.3f rttvar %u rtt_ts %.4f",
	      tc->rto / 1000, tc->rto_boff, tc->srtt / 1000,
	      tc->mrtt_us * 1e3, tc->rttvar, tc->rtt_ts);
...
format_tcp_scoreboard (u8 * s, va_list * args)
{
  ...
  s = format (s, "%Ucur_rxt_hole %u high_rxt %u rescue_rxt %u reord %u",
	      format_white_space, indent, sb->cur_rxt_hole,
	      sb->high_rxt - tc->iss, sb->rescue_rxt - tc->iss, sb->reorder);
