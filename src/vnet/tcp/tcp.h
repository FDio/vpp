...
typedef struct tcp_configuration_
{
  ...
  /** Time to wait for close from app */
  u16 closewait_time;

  /** Time-wait time. Also known as 2MSL */
  u16 timewait_time;

  /** Time to wait in fin-wai1 to send fin and rcv fin-ack */
  u16 finwait1_time;

  /** Time to wait in last ack for ack */
  u16 lastack_time;

  /** Time to wait in fin-wait2 for fin */
  u16 finwait2_time;

  /** Time to wait in closing for fin ack */
  u16 closing_time;

  /** Time to wait before cleaning up the connection */
  u16 cleanup_time;

  /** Min time to wait before "probing" the peer's window */
  u16 persist_min_time;
...
{
  if (tcp_main.cfg.initial_cwnd_multiplier > 0)
    return tcp_main.cfg.initial_cwnd_multiplier * tc->snd_mss;
