...
typedef struct tcp_configuration_
{
  ...
  /** RFC 7413: Enable TCP Fast Open globally for all new listeners */
  u8 enable_fast_open;
} tcp_configuration_t;
...
typedef struct _tcp_main
{
  ...
  clib_rwlock_t tfo_lock;
  /* Current 16-byte secret key for cookie generation */
  u64 tfo_key[2];
  ...
  /* pool of cached cookies (entry 0 = clib_llist sentinel head) */
  struct tcp_tfo_cc_entry_ *tfo_cc_entries;
  /* clib_llist sentinel index in tfo_cc_entries */
  clib_llist_index_t tfo_cc_lru_head_index;

  /** TFO blackhole detection: per-destination failure tracking (tfo_lock) */
  /* hash: ip_key -> pool index */
  uword *tfo_blackhole;
  /* pool of blackhole entries (entry 0 = clib_llist sentinel head) */
  struct tcp_tfo_bh_entry_ *tfo_bh_entries;
  /* clib_llist sentinel index in tfo_bh_entries */
  clib_llist_index_t tfo_bh_lru_head_index;
} tcp_main_t;
