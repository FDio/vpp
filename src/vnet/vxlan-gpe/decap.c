  ...
  while (n_left_from > 0)
    {
	  ...
	  flags0 = b0->flags;
	  good_udp0 = (flags0 & VNET_BUFFER_FLAG_L4_CHECKSUM_CORRECT) != 0;
	  ...
	  if (PREDICT_FALSE (!good_udp0))
	    {
	      if ((flags0 & VNET_BUFFER_FLAG_L4_CHECKSUM_COMPUTED) == 0)
	        {
		  if (is_ip4)
	...
	exit0:
	  ...
	  flags1 = b1->flags;
	  good_udp1 = (flags1 & VNET_BUFFER_FLAG_L4_CHECKSUM_CORRECT) != 0;
	  ...
	  if (PREDICT_FALSE (!good_udp1))
	    {
	      if ((flags1 & VNET_BUFFER_FLAG_L4_CHECKSUM_COMPUTED) == 0)
	        {
		  if (is_ip4)
