  ...
  if (unformat (input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index))
      ...
      if (SW_IF_INDEX_PG0 == sw_if_index)
	{
	  ...
	  if (FIB_PROTOCOL_IP4 == fproto)
	    {
	      punt_register (h1, PUNT_REASON_IP4_ACL_DENY,
			     "punt-test-pg0-ip4");
	      from = vlib_get_node_by_name (vm, (u8 *) "punt-test-pg0-ip4");
	    }
	  else
	    {
	      punt_register (h1, PUNT_REASON_IP6_ACL_DENY,
			     "punt-test-pg0-ip6");
  ...
  else
    {
      ...
      rc |= punt_reason_alloc (h2, "reason1", &r1);
      rc |= punt_reason_alloc (h2, "reason2", &r2);
      rc |= punt_reason_alloc (h2, "reason3", &r3);
      rc |= punt_reason_alloc (h2, "reason4", &r4);
      ...
      punt_unregister (h2, r4, "ip6-rewrite");
    }
  return (NULL);
}
...
{
.path = "test punt",.short_help =
    "punt unit tests - DO NOT RUN ON A LIVE SYSTEM",.function = punt_test,};
