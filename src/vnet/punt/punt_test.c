...
format_punt_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "punt: %U", format_punt_reason, t->pt_reason);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
        {
          ...
          vnet_rewrite_one_header (adj0[0],
                                   ip0,
...
punt_test_pg0_ip4 (vlib_main_t * vm,
                   ...
                   vlib_frame_t * frame)
{
  return (punt_test_fwd(vm, node, frame,
                        FIB_PROTOCOL_IP4, SW_IF_INDEX_PG0));
}
...
{
  return (punt_test_fwd(vm, node, frame,
                        FIB_PROTOCOL_IP4, SW_IF_INDEX_PG1));
}
...
{
  return (punt_test_fwd(vm, node, frame,
                        FIB_PROTOCOL_IP6, SW_IF_INDEX_PG0));
}
...
{
  return (punt_test_fwd(vm, node, frame,
                        FIB_PROTOCOL_IP6, SW_IF_INDEX_PG1));
}
  ...
  if (unformat (input, "%U",
                ...
                &sw_if_index))
    {
      ...
      if (SW_IF_INDEX_PG0 == sw_if_index)
        {
          h1 = punt_client_register("pg0");

          if (FIB_PROTOCOL_IP4 == fproto)
            {
              punt_register(h1, PUNT_REASON_IP4_ACL_DENY, "punt-test-pg0-ip4");
              from = vlib_get_node_by_name(vm, (u8*) "punt-test-pg0-ip4");
            }
          else
            {
              punt_register(h1, PUNT_REASON_IP6_ACL_DENY, "punt-test-pg0-ip6");
              from = vlib_get_node_by_name(vm, (u8*) "punt-test-pg0-ip6");
           }
        }
      else
        {
          h1 = punt_client_register("pg1");

          if (FIB_PROTOCOL_IP4 == fproto)
            {
              punt_register(h1, PUNT_REASON_IP4_ACL_DENY, "punt-test-pg1-ip4");
              from = vlib_get_node_by_name(vm, (u8*) "punt-test-pg1-ip4");
            }
          else
            {
              punt_register(h1, PUNT_REASON_IP6_ACL_DENY, "punt-test-pg1-ip6");
    ...
    {
      h2 = punt_client_register("test2");

      punt_register(h2, PUNT_REASON_IP4_ACL_DENY, "mpls-lookup");
      punt_register(h2, PUNT_REASON_IP4_ACL_DENY, "ip6-lookup");
      punt_register(h2, PUNT_REASON_IP4_ACL_DENY, "ip4-lookup");

      r1 = punt_reason_alloc(h2, "reason1", "error-drop");
      r2 = punt_reason_alloc(h2, "reason2", "error-drop");
      r3 = punt_reason_alloc(h2, "reason3", "error-drop");
      r4 = punt_reason_alloc(h2, "reason4", "error-drop");

      punt_register(h2, r1, "ip4-rewrite");
      punt_register(h2, r2, "ip4-rewrite");
      punt_register(h2, r3, "ip4-rewrite");
      punt_register(h2, r1, "ip4-rewrite");
      punt_register(h2, r4, "ip6-rewrite");
    }
  return (NULL);
}
