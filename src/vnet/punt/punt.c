...
static int
punt_validate_client (punt_hdl_t client)
{
  return (!pool_is_free_index (punt_client_pool, client));
}

static u64
punt_reg_mk_key (punt_reason_t reason, u32 node_index)
{
  return (((u64) node_index) << 32 | reason);
}

static u32
punt_reg_find (punt_reason_t reason, u32 node_index)
{
  uword *p;
 ...
 */
static void
punt_reg_mk_dp (punt_reason_t reason)
{
  u32 pri, *prip, *pris;
  const punt_reg_t *pr;
  u16 *edges, *old;
...
int
punt_register (punt_hdl_t client, punt_reason_t reason, const char *node_name)
{
...
int
punt_unregister (punt_hdl_t client,
		 punt_reason_t reason, const char *node_name)
{
  vlib_node_t *punt_to;
  punt_client_t *pc;
  vlib_main_t *vm;
...
punt_reason_alloc (punt_hdl_t client,
		   const char *reason_name, punt_reason_t * reason)
{
  punt_reason_t new;
...
format_punt_reg (u8 * s, va_list * args)
{
  ...
  s = format (s, "%U -> %U",
	      format_punt_reason, pr->pr_reason,
