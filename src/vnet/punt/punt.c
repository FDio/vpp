 ...
 */
static vnet_punt_reason_t punt_reason_last;
...
u8 *
format_vnet_punt_reason (u8 * s, va_list * args)
{
  vnet_punt_reason_t pr = va_arg (*args, int);

  return (format (s, "%v", punt_reason_data[pr].pd_name));
}

vnet_punt_hdl_t
vnet_punt_client_register (const char *who)
{
  u8 *pc_name;
  uword *p;
  u32 pci;
...
static int
punt_validate_client (vnet_punt_hdl_t client)
{
  return (!pool_is_free_index (punt_client_pool, client));
}

static u64
punt_reg_mk_key (vnet_punt_reason_t reason, u32 node_index)
{
  return (((u64) node_index) << 32 | reason);
}

static u32
punt_reg_find (vnet_punt_reason_t reason, u32 node_index)
{
  uword *p;
 ...
 */
static void
punt_reg_mk_dp (vnet_punt_reason_t reason)
{
  u32 pri, *prip, *pris;
  const punt_reg_t *pr;
  u16 *edges, *old;
  ...
  /*
   * An check for an empty vector is done in the DP, so the a zero
...
int
vnet_punt_register (vnet_punt_hdl_t client, vnet_punt_reason_t reason,
...
int
vnet_punt_unregister (vnet_punt_hdl_t client,
		      vnet_punt_reason_t reason, const char *node_name)
{
  vlib_node_t *punt_to;
  punt_client_t *pc;
  vlib_main_t *vm;
...
int
vnet_punt_reason_alloc (vnet_punt_hdl_t client,
			const char *reason_name, vnet_punt_reason_t * reason)
{
  vnet_punt_reason_t new;
...
format_punt_reg (u8 * s, va_list * args)
{
  ...
  s = format (s, "%U -> %U",
	      format_vnet_punt_reason, pr->pr_reason,
	      ...
	      unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  {
    u8 *s = NULL;
    vlib_cli_output (vm, " %U", format_vnet_punt_reason, ii);
  ...
  for (ii = 0; ii < vlib_combined_counter_n_counters (cm); ii++)
    {
      ...
      vlib_cli_output (vm, "%U packets:%lld bytes:%lld",
		       format_vnet_punt_reason, ii, c.packets, c.bytes);
    }
...
punt_init (vlib_main_t * vm)
{
   ...
   */
  hdl = vnet_punt_client_register ("vnet");

#define _(v, s) {                                       \
    vnet_punt_reason_alloc(hdl,s,&r);                   \
    punt_reg_mk_dp(r);                                  \
  }
