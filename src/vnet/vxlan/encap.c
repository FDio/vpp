	  ...
	  if (is_ip4)
	    {
	      ...
	      ip4_header_t * ip4_0 = &hdr0->ip4;
	      ip4_header_t * ip4_1 = &hdr1->ip4;
	  ...
	  else /* ipv6 */
	    {
          ...
          if (csum_offload)
            {
              b0->flags |= csum_flags;
              vnet_buffer (b0)->l3_hdr_offset = ip_0 - b0->data;
              vnet_buffer (b0)->l4_hdr_offset = (u8 *) udp0 - b0->data;
              b1->flags |= csum_flags;
              vnet_buffer (b1)->l3_hdr_offset = ip_1 - b1->data;
