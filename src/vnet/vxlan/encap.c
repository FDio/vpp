		    ...
		    vlib_frame_t * from_frame, u8 is_ip4, u8 csum_offload)
{
  ...
  u32 const csum_flags = is_ip4 ?
    ...
    VNET_BUFFER_F_IS_IP6 | VNET_BUFFER_F_OFFLOAD_CKSUM |
    VNET_BUFFER_F_L3_HDR_OFFSET_VALID | VNET_BUFFER_F_L4_HDR_OFFSET_VALID;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (csum_offload && (or_flags & VNET_BUFFER_F_OFFLOAD_CKSUM))
	    {
	  ...
	  if (csum_offload)
	    {
	      ...
	      vnet_buffer2 (b0)->oflags = outer_packet_csum_offload_flags;
	      b1->flags |= csum_flags;
	      vnet_buffer (b1)->l3_hdr_offset = l3_1 - b1->data;
	      vnet_buffer (b1)->l4_hdr_offset = (u8 *) udp1 - b1->data;
	      vnet_buffer2 (b1)->oflags = outer_packet_csum_offload_flags;
	    }
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (csum_offload && (b0->flags & VNET_BUFFER_F_OFFLOAD_CKSUM))
	    {
	      /* Only calculate the non-GSO packet csum offload */
	      if ((b0->flags & VNET_BUFFER_F_GSO) == 0)
		{
