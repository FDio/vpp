...
format_vxlan_tunnel (u8 * s, va_list * args)
{
  ...
  if (t->flow_index != 0)
    s = format (s, "flow-index %d ", t->flow_index);
			  ...
			  unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  if (rx_sw_if_index > vec_len (vxm->tunnel_index_by_sw_if_index))
    t_index = ~0;
  else
    t_index = vxm->tunnel_index_by_sw_if_index[rx_sw_if_index];
...
VLIB_CLI_COMMAND (vxlan_offload_command, static) = {
    .path = "flow-offload vxlan",
    .short_help =
    "flow-offload vxlan hw <inerface-name> rx <tunnel-name> [del]",
