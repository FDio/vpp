...
static void
hash_set_key_dup (uword ** h, void * key, uword v) {
...
vtep_addr_ref(ip46_address_t *ip)
{
	...
	ip46_address_is_ip4(ip) ?
	                 ...
	                 hash_set_key_dup (&vxlan_main.vtep6, &ip->ip6, 1);
	return 1;
}
...
mcast_shared_get(ip46_address_t * ip)
{
	...
	mcast_shared_t * ep =
	  (mcast_shared_t *)hash_get_mem (vxlan_main.mcast_shared, ip);
        ASSERT(ep);
	return *ep;
}
...
#undef _
	...
	rv = vxlan6_rewrite (t);
      ...
      /* copy the key */
      if (is_ip6)
        hash_set_key_dup (&vxm->vxlan6_tunnel_by_key, &key6, t - vxm->tunnels);
      else
	  ...
	  if (vtep_addr_ref(&t->dst) == 1)
	    {
		 ...
		 .mcast_adj_index = adj_rewrite_add_and_lock
	       ...
	       hash_set_key_dup (&vxm->mcast_shared, &t->dst, new_ep.as_u64);
	       dpo_reset(&dpo);
	    }
