...
#undef _
      ...
      if (!ip46_address_is_multicast (&t->dst))
	{
	  ...
	  clib_warning ("created flow index:%d", t->flow_index);
	}
      else
	{
	  /* Multicast tunnel -
...
static clib_error_t *
offload_vxlan_command_fn (vlib_main_t * vm,
			  unformat_input_t * input, vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  ...
  u32 t_index =
    (rx_sw_if_index >
     vec_len (vxm->tunnel_index_by_sw_if_index)) ? ~0 :
    vxm->tunnel_index_by_sw_if_index[rx_sw_if_index];
  if (t_index == ~0)
  ...
  if (is_add)
    {
      if (vnet_flow_enable (vnm, t->flow_index, hw_if_index))
	return clib_error_return (0, "error enabling flow");

      t->flows_used++;
    }
  else
    {
      if (vnet_flow_disable (vnm, t->flow_index, hw_if_index))
	return clib_error_return (0, "error disabling flow");
...
/* *INDENT-OFF* */
VLIB_CLI_COMMAND (offload_vxlan_command, static) = {
    .path = "offload vxlan",
    .short_help =
    "offload vxlan hw <inerface name> rx <tunnel name> [del]",
    .function = offload_vxlan_command_fn,
};
/* *INDENT-ON* */
