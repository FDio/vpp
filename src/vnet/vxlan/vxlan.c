...
fib_index_from_fib_id (u32 fid, u32 ipv4_set) {
  uword * index_by_id = ipv4_set ? ip4_main.fib_index_by_table_id :
    ip6_main.fib_index_by_table_id;
  uword * p = hash_get (index_by_id, fid);
  if (!p)
    return ~0;
                                   ...
                                   vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT) {
    if (unformat (line_input, "del"))
      ...
      {
        encap_fib_index = fib_index_from_fib_id (tmp, !ipv6_set);
