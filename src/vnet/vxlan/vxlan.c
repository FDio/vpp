  ...
  (vnet_vxlan_add_del_tunnel_args_t * a, u32 * sw_if_indexp)
{
  ...
  if (!is_ip6)
  ...
  else
    {
      ...
      key6.fib_index = a->encap_fib_index;
      int rv =
	BV (clib_bihash_search_inline) (&vxm->vxlan6_tunnel_by_key, &key6.kv);
...
#undef _
      ...
      if (hash_get (vxm->instance_used, user_instance))
	{
      ...
      if (is_ip6)
	{
	  key6.kv.value = (u64) dev_instance;
	...
	BV (clib_bihash_add_del) (&vxm->vxlan6_tunnel_by_key, &key6.kv,
				  0 /*del */ );
...
vxlan_init (vlib_main_t * vm)
{
  ...
  BV (clib_bihash_init) (&vxm->vxlan6_tunnel_by_key, "vxlan6", 2000, 1 << 20);
