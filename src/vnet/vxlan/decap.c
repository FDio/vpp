...
format_vxlan_rx_trace (u8 * s, va_list * args)
{
  ...
  if (t->tunnel_index != ~0)
    {
      s =
	format (s, "VXLAN decap from vxlan_tunnel%d vni %d next %d error %d",
...
vxlan_input (vlib_main_t * vm,
	     ...
	     vlib_frame_t * from_frame, u32 is_ip4, int collect_detailed_stats)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (is_ip4)
		...
		{
		  uword *p =
		    hash_get (vxm->vxlan4_tunnel_by_key, key4_0.as_u64);
		...
		{
		  uword *p =
		    hash_get_mem (vxm->vxlan6_tunnel_by_key, &key6_0);
		  if (PREDICT_FALSE (p == NULL))
		    {
		      error0 = VXLAN_ERROR_NO_SUCH_TUNNEL;
		      next0 = VXLAN_INPUT_NEXT_DROP;
...
#define inc_stats_if_counter(ctype, rx_tx)                                  \
  if (stats_n_packets[ctype])                                               \
    {                                                                       \
      vlib_increment_combined_counter (                                     \
		  ...
		  if (collect_detailed_stats)
		    ...
		    {
		      inc_stats_if_counter (VNET_INTERFACE_COUNTER_RX,
					    rx);
		      pkts_decapsulated +=
			stats_n_packets[VNET_INTERFACE_COUNTER_RX];
    ...
    trace0:
	  ...
	  if (is_ip4)
		...
		{
		  uword *p =
		    hash_get (vxm->vxlan4_tunnel_by_key, key4_1.as_u64);
		  if (PREDICT_FALSE (p == NULL))
		    {
		...
		{
		  uword *p =
		    hash_get_mem (vxm->vxlan6_tunnel_by_key, &key6_1);
		  if (PREDICT_FALSE (p == NULL))
		    {
		      error1 = VXLAN_ERROR_NO_SUCH_TUNNEL;
		      next1 = VXLAN_INPUT_NEXT_DROP;
  ...
  if (stats_n_packets[VNET_INTERFACE_COUNTER_RX])
	...
	{
	  inc_stats_if_counter (VNET_INTERFACE_COUNTER_RX, rx);
	}
	      ...
	      vlib_node_runtime_t * node, vlib_frame_t * from_frame)
{
  ...
  return vxlan_input (vm, node, from_frame, /* is_ip4 */ 1, /* collect_detailed_stats */ 1);
  }else{
  return vxlan_input (vm, node, from_frame, /* is_ip4 */ 1, /* collect_detailed_stats */ 0);
  }
}
...
{
  if(collect_detailed_interface_stats()){
  return vxlan_input (vm, node, from_frame, /* is_ip4 */ 0, /* collect_detailed_stats */ 1);
  }else{
  return vxlan_input (vm, node, from_frame, /* is_ip4 */ 0, /* collect_detailed_stats */ 0);
  }
}
...
{
  .function = vxlan4_input,.name = "vxlan4-input",
    /* Takes a vector of packets. */
    .vector_size = sizeof (u32),.n_errors = VXLAN_N_ERROR,.error_strings =
    vxlan_error_strings,.n_next_nodes = VXLAN_INPUT_N_NEXT,.next_nodes =
  {
...
{
  .function = vxlan6_input,.name = "vxlan6-input",
    /* Takes a vector of packets. */
    .vector_size = sizeof (u32),.n_errors = VXLAN_N_ERROR,.error_strings =
    vxlan_error_strings,.n_next_nodes = VXLAN_INPUT_N_NEXT,.next_nodes =
  {
...
{
  .function = ip4_vxlan_bypass,.name = "ip4-vxlan-bypass",.vector_size =
    sizeof (u32),.n_next_nodes = IP_VXLAN_BYPASS_N_NEXT,.next_nodes =
  {
...
{
  .function = ip6_vxlan_bypass,.name = "ip6-vxlan-bypass",.vector_size =
    sizeof (u32),.n_next_nodes = IP_VXLAN_BYPASS_N_NEXT,.next_nodes =
  {
  [IP_VXLAN_BYPASS_NEXT_DROP] = "error-drop",
