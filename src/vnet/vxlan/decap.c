...
vxlan_check_ip (vlib_buffer_t *b, u16 payload_len)
{
  ...
  u16 expected = payload_len + sizeof hdr->ip4 + sizeof hdr->udp + sizeof hdr->vxlan;
					      ...
					      vlib_frame_t * f)
{
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from > 3 && n_left_to_next > 3)
    {
      ...
      ASSERT (b1->flags & VNET_BUFFER_F_FLOW_ID_PRESENT);
      ASSERT (b2->flags & VNET_BUFFER_F_FLOW_ID_PRESENT);
      ASSERT (b3->flags & VNET_BUFFER_F_FLOW_ID_PRESENT);
      ...
      b1->flags &= ~VNET_BUFFER_F_FLOW_ID_PRESENT;
      b2->flags &= ~VNET_BUFFER_F_FLOW_ID_PRESENT;
      b3->flags &= ~VNET_BUFFER_F_FLOW_ID_PRESENT;

      u32 flags = b0->flags | b1->flags | b2->flags | b3->flags;

      u32 t_index0 = vnet_buffer (b0)->flow_id - vxm->flow_id_start;
      u32 t_index1 = vnet_buffer (b1)->flow_id - vxm->flow_id_start;
      u32 t_index2 = vnet_buffer (b2)->flow_id - vxm->flow_id_start;
      u32 t_index3 = vnet_buffer (b3)->flow_id - vxm->flow_id_start;
