           ...
           if (PREDICT_FALSE (key4_0.as_u64 != last_key4.as_u64))
              {
                ...
                last_key4 = key4_0;
                last_tunnel_index = p[0];
              }
            tunnel_index0 = last_tunnel_index;
	    stats_t0 = t0 = pool_elt_at_index (vxm->tunnels, tunnel_index0);
	  ...
	  if (PREDICT_FALSE (vxlan1->flags != VXLAN_FLAGS_I))
	    {
          ...
          if (is_ip4) {
	    /* Make sure VXLAN tunnel exist according to packet SIP and VNI */
	    vxlan4_tunnel_key_t key4_1 = {
              .src = ip4_1->src_address.as_u32,
              .vni = vxlan1->vni_reserved
	    };

	    if (PREDICT_FALSE (key4_1.as_u64 != last_key4.as_u64))
              {
                uword * p = hash_get (vxm->vxlan4_tunnel_by_key, key4_1.as_u64);
                if (PREDICT_FALSE (p == NULL))
                  {
 	    ...
 	    stats_t1 = t1 = pool_elt_at_index (vxm->tunnels, tunnel_index1);
		...
		if (PREDICT_TRUE (p != NULL))
		  {
		    stats_t1 = pool_elt_at_index (vxm->tunnels, p[0]);
		    goto next1; /* valid packet */
		  }
	      }
	    error1 = VXLAN_ERROR_NO_SUCH_TUNNEL;
	    ...
	    /* Make sure VXLAN tunnel exist according to packet SIP and VNI */
            vxlan6_tunnel_key_t key6_1 = {
	      .src = ip6_1->src_address,
	      .vni = vxlan1->vni_reserved,
	    };

            if (PREDICT_FALSE (memcmp(&key6_1, &last_key6, sizeof(last_key6)) != 0))
              {
                uword * p = hash_get_mem (vxm->vxlan6_tunnel_by_key, &key6_1);
                if (PREDICT_FALSE (p == NULL))
                  {
                    error1 = VXLAN_ERROR_NO_SUCH_TUNNEL;
                    next1 = VXLAN_INPUT_NEXT_DROP;
...
typedef enum
{
#define _(s,n) VXLAN_OFFLOAD_NEXT_##s,
  foreach_vxlan_offload_input_next
#undef _
  VXLAN_OFFLOAD_N_NEXT,
} dpdk_offload_input_next_t;

#define foreach_vxlan_offload_error					\
  _(NONE, "no error")							\
...
typedef enum
{
#define _(f,s) VXLAN_OFFLOAD_ERROR_##f,
  foreach_vxlan_offload_error
#undef _
    VXLAN_OFFLOAD_N_ERROR,
} vxlan_offload_error_t;

static char *vxlan_offload_error_strings[] = {
#define _(n,s) s,
  foreach_vxlan_offload_error
#undef _
};
...
uword
CLIB_MULTIARCH_FN (vxlan_passthru_input) (vlib_main_t * vm,
					      vlib_node_runtime_t * node,
					      vlib_frame_t * f)
{
  enum {
  ...
  vlib_combined_counter_main_t * rx_counter[VXLAN_OFFLOAD_N_NEXT] = {
    [VXLAN_OFFLOAD_NEXT_DROP] = im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_DROP,
    [VXLAN_OFFLOAD_NEXT_L2_INPUT] = im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX,
  };
  u32 thread_index = vlib_get_thread_index();

  u32 next_index = VXLAN_OFFLOAD_NEXT_L2_INPUT;
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from > 0 && n_left_to_next > 0)
    {
      ...
      u8 error0 = VXLAN_OFFLOAD_ERROR_NONE;

      u8 ip_err0 = dpdk_check_ip (b0);
      u8 udp_err0 = dpdk_check_ip_udp_len (b0);
      u8 csum_err0 = dpdk_check_udp_csum (vm, b0);

      if (csum_err0)
	csum_err0 = !dpdk_validate_udp_csum (vm, b0);
      if (ip_err0 || udp_err0 || csum_err0)
      {
	next0 = VXLAN_OFFLOAD_NEXT_DROP;

	if (ip_err0)
	  error0 = VXLAN_OFFLOAD_ERROR_IP_HEADER_ERROR;
	if (udp_err0)
	  error0 = VXLAN_OFFLOAD_ERROR_UDP_LENGTH_ERROR;
	if (csum_err0)
	  error0 = VXLAN_OFFLOAD_ERROR_UDP_CHECKSUM_ERROR;
...
VLIB_REGISTER_NODE (vxlan4_passthru_input_node) = {
  .name = "vxlan-passthru-input",
  .function = vxlan_passthru_input,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .vector_size = sizeof (u32),

  .format_trace = format_vxlan_rx_trace,

  .n_errors = VXLAN_OFFLOAD_N_ERROR,
  .error_strings = vxlan_offload_error_strings,

  .n_next_nodes = VXLAN_OFFLOAD_N_NEXT,
  .next_nodes = {
#define _(s,n) [VXLAN_OFFLOAD_NEXT_##s] = n,
    foreach_vxlan_offload_input_next
#undef _
  },
};
...
vlib_node_function_t __clib_weak vxlan_passthru_input_avx512;
vlib_node_function_t __clib_weak vxlan_passthru_input_avx2;

#if __x86_64__
static void __clib_constructor
vxlan_passthru_input_multiarch_select (void)
{
  if (vxlan_passthru_input_avx512 && clib_cpu_supports_avx512f ())
    vxlan4_passthru_input_node.function = vxlan_passthru_input_avx512;
  else if (vxlan_passthru_input_avx2 && clib_cpu_supports_avx2 ())
    vxlan4_passthru_input_node.function = vxlan_passthru_input_avx2;
}
#endif
