                    ...
                    vxlan_tunnel_t ** stats_t0)
{
  ...
  .vni = (u64) vxlan0->vni_reserved,
  .fib_index = fib_index,
  };

  if (PREDICT_FALSE (BV (clib_bihash_key_compare) (key6.kv.key, cache->kv.key) == 0))
  {
    int rv = BV (clib_bihash_search_inline) (&vxm->vxlan6_tunnel_by_key, &key6.kv);
    if (PREDICT_FALSE (rv != 0))
      return 0;

    *cache = key6;
  }
  vxlan_tunnel_t * t0 = pool_elt_at_index (vxm->tunnels, cache->kv.value);
  ...
  else
  {
    ...
    /* Make sure mcast VXLAN tunnel exist by packet DIP and VNI */
    key6.src = ip6_0->dst_address;
    int rv = BV (clib_bihash_search_inline) (&vxm->vxlan6_tunnel_by_key, &key6.kv);
    if (PREDICT_FALSE (rv != 0))
      return 0;

    *stats_t0 = pool_elt_at_index (vxm->tunnels, key6.kv.value);
  }
