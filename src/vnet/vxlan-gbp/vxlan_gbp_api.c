...
//_(VXLAN_GBP_OFFLOAD_RX, vxlan_gbp_offload_rx)

/*static void
vl_api_vxlan_gbp_offload_rx_t_handler (vl_api_vxlan_gbp_offload_rx_t * mp)
{
  vl_api_vxlan_gbp_offload_rx_reply_t *rmp;
  int rv = 0;
  ...
  u32 t_index = vnet_vxlan_gbp_get_tunnel_index (sw_if_index);
  if (t_index == ~0)
    {
  ...
  if (vnet_vxlan_gbp_add_del_rx_flow (hw_if_index, t_index, mp->enable))
    {
...
err:
  ...
  REPLY_MACRO (VL_API_VXLAN_GBP_OFFLOAD_RX_REPLY);
}
*/
static void
