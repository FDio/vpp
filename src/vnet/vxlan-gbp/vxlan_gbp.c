...
format_vxlan_gbp_tunnel (u8 * s, va_list * args)
{
  ...
  s = format (s,
	      "[%d] instance %d src %U dst %U vni %d sclass %d fib-idx %d sw-if-idx %d ",
	      t->dev_instance, t->user_instance,
	      format_ip46_address, &t->src, IP46_TYPE_ANY,
	      format_ip46_address, &t->dst, IP46_TYPE_ANY,
	      t->vni, t->sclass, t->encap_fib_index, t->sw_if_index);
...
vxlan_gbp_rewrite (vxlan_gbp_tunnel_t * t, bool is_ip6)
{
  ...
  /* VXLAN header */
  vnet_set_vxlan_gbp_header (vxlan_gbp, t->vni, t->flags, t->sclass);
				     ...
				     vlib_cli_command_t * cmd)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "sclass %d", &sclass))
	;
      else
	{
  ...
  if (sclass == 0)
    return clib_error_return (0, "source class is not specified");
...
VLIB_CLI_COMMAND (create_vxlan_gbp_tunnel_command, static) = {
  ...
  " {dst <remote-vtep-addr>|group <mcast-vtep-addr> <intf-name>} vni <nn> sclass <nn>"
  " [instance <id>]"
...
/*
int
vnet_vxlan_gbp_add_del_rx_flow (u32 hw_if_index, u32 t_index, int is_add)
{
  vxlan_gbp_main_t *vxm = &vxlan_gbp_main;
  vxlan_gbp_tunnel_t *t = pool_elt_at_index (vxm->tunnels, t_index);
  vnet_main_t *vnm = vnet_get_main ();
  if (is_add)
    {
      if (t->flow_index == ~0)
	{
	  ...
	  vnet_flow_t flow = {
	    ...
	    .ip4_vxlan_gbp = {
			  .src_addr = t->dst.ip4,
			  .dst_addr = t->src.ip4,
			  .dst_port = UDP_DST_PORT_vxlan_gbp,
			  .vni = t->vni,
			  }
	    ,
	  };
...
u32
vnet_vxlan_gbp_get_tunnel_index (u32 sw_if_index)
{
  vxlan_gbp_main_t *vxm = &vxlan_gbp_main;
...
static clib_error_t *
vxlan_gbp_offload_command_fn (vlib_main_t * vm,
			  unformat_input_t * input, vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  ...
  u32 t_index = vnet_vxlan_gbp_get_tunnel_index (rx_sw_if_index);;
  if (t_index == ~0)
    return clib_error_return (0, "%U is not a vxlan_gbp tunnel",
  ...
  if (vnet_vxlan_gbp_add_del_rx_flow (hw_if_index, t_index, is_add))
...
*/
/* *INDENT-OFF* */
//VLIB_CLI_COMMAND (vxlan_gbp_offload_command, static) = {
//    .path = "set flow-offload vxlan-gbp",
//    .short_help =
//    "set flow-offload vxlan-gbp hw <inerface-name> rx <tunnel-name> [del]",
//    .function = vxlan_gbp_offload_command_fn,
//};
/* *INDENT-ON* */
...
vxlan_gbp_init (vlib_main_t * vm)
{
  ...
  vnet_flow_get_range (vxm->vnet_main, "vxlan_gbp", 1024 * 1024,
		       &vxm->flow_id_start);
