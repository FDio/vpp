...
format_vxlan_gbp_rx_trace (u8 *s, va_list *args)
{
  ...
  if (t->tunnel_index == ~0)
    return format (
      s, "VXLAN_GBP decap error - tunnel for vni %d does not exist", t->vni);
  return format (s,
		 "VXLAN_GBP decap from vxlan_gbp_tunnel%d vni %d sclass %d"
		 " flags %U next %d error %d",
		 t->tunnel_index, t->vni, t->sclass,
		 format_vxlan_gbp_header_gpflags, t->flags, t->next_index,
		 t->error);
}
			...
			vxlan_gbp_header_t *vxlan_gbp0)
{
  ...
  key4.key[1] =
    (((u64) fib_index << 32) |
     (vxlan_gbp0->vni_reserved & clib_host_to_net_u32 (0xffffff00)));
  key4.key[0] =
...
always_inline uword
vxlan_gbp_input (vlib_main_t *vm, vlib_node_runtime_t *node,
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_FALSE (t0 == NULL || !i_and_g0))
	  ...
	  else
	    {
	      ...
	      /* Set packet input sw_if_index to unicast VXLAN tunnel for
	       * learning */
	  ...
	  vnet_buffer2 (b0)->gbp.flags =
	    (vxlan_gbp_get_gpflags (vxlan_gbp0) | VXLAN_GBP_GPFLAGS_R);
	  ...
	  if (PREDICT_FALSE (t1 == NULL || !i_and_g1))
	  ...
	  else
	    {
	      ...
	      /* Set packet input sw_if_index to unicast VXLAN_GBP tunnel for
	       * learning */
	  ...
	  vnet_buffer2 (b1)->gbp.flags =
	    (vxlan_gbp_get_gpflags (vxlan_gbp1) | VXLAN_GBP_GPFLAGS_R);
...
VLIB_NODE_FN (vxlan4_gbp_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
  return vxlan_gbp_input (vm, node, from_frame, /* is_ip4 */ 1);
}

VLIB_NODE_FN (vxlan6_gbp_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword
ip_vxlan_gbp_bypass_inline (vlib_main_t *vm, vlib_node_runtime_t *node,
			    vlib_frame_t *frame, u32 is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  next0 = error0 ? IP_VXLAN_GBP_BYPASS_NEXT_DROP :
			   IP_VXLAN_GBP_BYPASS_NEXT_VXLAN_GBP;
	...
	exit0:
	  ...
	  next1 = error1 ? IP_VXLAN_GBP_BYPASS_NEXT_DROP :
			   IP_VXLAN_GBP_BYPASS_NEXT_VXLAN_GBP;
...
VLIB_NODE_FN (ip4_vxlan_gbp_bypass_node)
...
VLIB_NODE_FN (ip6_vxlan_gbp_bypass_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
