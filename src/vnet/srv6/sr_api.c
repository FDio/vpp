...
vl_api_sr_localsid_add_del_t_handler (vl_api_sr_localsid_add_del_t *mp)
{
   ...
   *  char end_psp, u8 behavior, u32 sw_if_index, u32 vlan_index, u32
   * fib_table, ip46_address_t *nh_addr, void *ls_plugin_mem)
   */
  if (mp->behavior == SR_BEHAVIOR_X || mp->behavior == SR_BEHAVIOR_DX6 ||
  ...
  rv = sr_cli_localsid (mp->is_del, &localsid, 128, mp->end_psp, mp->behavior,
			ntohl (mp->sw_if_index), ntohl (mp->vlan_index),
...
vl_api_sr_policy_add_t_handler (vl_api_sr_policy_add_t *mp)
{
  ...
  int rv = 0;
  rv =
    sr_policy_add (&bsid_addr, segments, ntohl (mp->sids.weight), mp->is_spray,
...
vl_api_sr_steering_add_del_t_handler (vl_api_sr_steering_add_del_t *mp)
{
  ...
  rv = sr_steering_policy (mp->is_del, &bsid_addr, ntohl (mp->sr_policy_index),
			   ntohl (mp->table_id), &prefix_addr, mp->prefix.len,
...
static void
send_sr_localsid_details (ip6_sr_localsid_t *t, vl_api_registration_t *reg,
			  u32 context)
{
...
static void
send_sr_policies_details (ip6_sr_policy_t *t, vl_api_registration_t *reg,
			  u32 context)
{
...
static void
send_sr_policies_details_with_sl_index (ip6_sr_policy_t *t,
					vl_api_registration_t *reg,
					u32 context)
{
...
static void
send_sr_steering_pol_details (ip6_sr_steering_policy_t *t,
