...
typedef void (sr_sid_encoder_builder_t) (void *mem_p, ip6_address_t *v6src_addr, ip6_address_t *v6dst_addr);
...
typedef struct
{
  u16 sr_sid_encoder_flavor_number;			/**< SR Encap SID Encoder plugin function ID */

  u8 *keyword_str;					/**< Encoder keyword (i.e. End.X) */

  u8 *def_str;						/**< Encoder definition (i.e. Endpoint with cross-connect) */

  u8 *params_str;					/**< Encoder parameters (i.e. <oif> <IP46next_hop>) */

  format_function_t *ls_format;				/**< SID Encoder format function */

  unformat_function_t *ls_unformat;			/**< SID Encoder unformat function */

  sr_sid_encoder_callback_t *removal;			/**< Function within plugin that will be called before encoder removal */
...
extern int sr_policy_add (ip6_address_t *bsid, ip6_address_t *segments, ip6_address_t *encap_src_v6addr,
...
extern int sr_policy_mod (ip6_address_t * bsid, u32 index, u32 fib_table,
			  u8 operation, ip6_address_t * segments, ip6_address_t *encap_src_v6addr,
...
extern int
sr_sid_encoder_policy (int is_del, uint32_t rule_id, void *ls_mem, sr_sid_encoder_flavor_fn_registration_t *flavor);
