			   ...
			   ip6_sr_localsid_t * ls0, u32 * next0)
{
  ...
  switch (next_proto)
      ...
      if (ls0->behavior == SR_BEHAVIOR_DX2)
	{
	  ...
	  /* Set L2 header length for l2-output VTR processing */
	  ethernet_header_t *eh = vlib_buffer_get_current (b0);
	  vnet_buffer (b0)->l2.l2_len = sizeof (ethernet_header_t);
	  u16 ethertype = clib_net_to_host_u16 (eh->type);
	  if (ethernet_frame_is_tagged (ethertype))
	    {
	      vnet_buffer (b0)->l2.l2_len += sizeof (ethernet_vlan_header_t);
	      ethertype = clib_net_to_host_u16 (((ethernet_vlan_header_t *) (eh + 1))->type);
	      if (ethernet_frame_is_tagged (ethertype))
		vnet_buffer (b0)->l2.l2_len += sizeof (ethernet_vlan_header_t);
	    }
