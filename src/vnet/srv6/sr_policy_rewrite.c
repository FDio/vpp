	   ...
	   u8 is_encap)
{
  ...
  segment_list->fib_table = sr_policy->fib_table;
			  ...
			  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 8 && n_left_to_next >= 4)
	{
	  ...
	  vnet_buffer (b0)->sw_if_index[VLIB_TX] = sl0->fib_table;
	  vnet_buffer (b1)->sw_if_index[VLIB_TX] = sl1->fib_table;
	  vnet_buffer (b2)->sw_if_index[VLIB_TX] = sl2->fib_table;
	  vnet_buffer (b3)->sw_if_index[VLIB_TX] = sl3->fib_table;
