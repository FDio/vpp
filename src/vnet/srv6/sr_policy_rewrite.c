 ...
 */
static inline u8 *
compute_rewrite_insert (ip6_address_t * sl, u8 tlv_type1, u8 tlv_type2)
{
  ip6_sr_header_t *srh;
  ip6_address_t *addrp, *this_address;
  u32 header_length = 0;
	       ...
	       u32 weight)
{
  ...
  if (operation == 1)		/* Add SR List to an existing SR policy */
    {
      /* Create the new SL */
      segment_list = create_sl (sr_policy, segments, weight, sr_policy->is_encap, 0, 0);	// FIXME: The last two args are hardcoded for now
		      ...
		      vlib_cli_command_t * cmd)
{
  ...
  char tlv_type1 = 0;		/* FIXME: Convert these to bitmaps */
  char tlv_type2 = 0;		/* FIXME: Convert these to bitmaps */
  ...
  if (!is_tlv_present && (tlv_type1 || tlv_type2))
    return clib_error_return (0, "Incorrect CLI");
  ...
  if (is_add)
	...
	return clib_error_return (0, "No Segment List specified");
      rv = sr_policy_add (&bsid, segments, weight, (is_spray ? SR_POLICY_TYPE_SPRAY : SR_POLICY_TYPE_DEFAULT), fib_table, is_encap, tlv_type1, tlv_type2);	/* FIXME: Convert these to bitmaps */
    }
  else if (is_del)
			  ...
			  vlib_frame_t * from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  limit0 = (ip6_sr_tlv_header_t *) ((u8 *) opt0 + opt0->length);
