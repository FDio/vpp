...
create_sl (ip6_sr_policy_t *sr_policy, ip6_address_t *sl,
	   ip6_address_t *encap_src_v6addr, u32 weight, u8 is_encap)
{
  ...
  if (is_encap)
    {
      if (encap_src_v6addr)
	{
	  clib_memcpy_fast (&encap_srcv6, encap_src_v6addr,
			    sizeof (ip6_address_t));
	}
 ...
 */
int
sr_sid_encoder_flavor_register_function (vlib_main_t *vm, u8 *keyword_str,
					 u8 *def_str, u8 *params_str,
					 ...
					 sr_sid_encoder_builder_t *builder_fn)
{
  ...
  p =
    hash_get_mem (sm->sid_encoder_flavor_plugin_functions_by_key, keyword_str);
  if (p)
    {
      plugin =
	pool_elt_at_index (sm->sid_encoder_flavor_plugin_functions, p[0]);
    }
  /* Else create a new one and set hash key */
  else
    {
      pool_get (sm->sid_encoder_flavor_plugin_functions, plugin);
      hash_set_mem (sm->sid_encoder_flavor_plugin_functions_by_key,
		    keyword_str,
  ...
  plugin->sr_sid_encoder_flavor_number =
    (plugin - sm->sid_encoder_flavor_plugin_functions);
 ...
 */
static clib_error_t *
show_sr_sid_flavors_command_fn (vlib_main_t *vm, unformat_input_t *input,
				vlib_cli_command_t *cmd)
{
  ip6_sr_main_t *sm = &sr_main;
  sr_sid_encoder_flavor_fn_registration_t *plugin;
 ...
 */
static clib_error_t *
show_sr_sid_encoders_command_fn (vlib_main_t *vm, unformat_input_t *input,
				 vlib_cli_command_t *cmd)
{
  ip6_sr_main_t *sm = &sr_main;
  ...
  vec_foreach (instance, sm->encoder_rules)
	...
	pool_elt_at_index (sm->sid_encoder_flavor_plugin_functions,
      ...
      u8 *s = 0;
      s = format (s, "rule: %d, flavor: %s, s/d v6Addr %U -> %U",
		  instance->rule_id, plugin->keyword_str, format_ip6_address,
		  &(instance)->v6src_addr, format_ip6_address,
		  &(instance)->v6dst_addr);
      vlib_cli_output (vm, "%s\n%U", s, plugin->ls_format,
		       (instance)->plugin_mem);
    }
...
static int
sid_encoder_rule_id_is_equal (ip6_sr_sid_encoder_t *a, ip6_sr_sid_encoder_t *b)
{
...
always_inline bool
get_sr_sid_encoder (u32 rule_id, ip6_sr_sid_encoder_t **e)
{
  ip6_sr_main_t *sm = &sr_main;
...
void
sr_sid_encoder_policy (int is_del, u32 rule_id, void *ls_mem,
			   ...
			   vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	{
	  sr_sid_encoder_flavor_fn_registration_t *plugin = 0,
						  **vec_plugins = 0;
  ...
  if (!is_del && flavor < 0)
	...
	0, "Please specify Encoder flavor or check parameter for flavor");
    }
  sr_sid_encoder_policy (is_del, rule_id, ls_mem, p_plugin);
  return 0;
}
...
VLIB_CLI_COMMAND (sr_sid_encoder_command, static) = {
  ...
  .long_help = "Create the contents of the SID when encap\n"
	       "Arguments:\n"
...
sr_policy_add (ip6_address_t *bsid, ip6_address_t *segments,
	       ...
	       u32 fib_table, u8 is_encap, u16 plugin, void *ls_plugin_mem)
{
  ip6_sr_main_t *sm = &sr_main;
...
sr_policy_mod (ip6_address_t *bsid, u32 index, u32 fib_table, u8 operation,
	       ip6_address_t *segments, ip6_address_t *encap_src_v6addr,
	       u32 sl_index, u32 weight)
{
  ...
  if (operation == 1)		/* Add SR List to an existing SR policy */
    {
      /* Create the new SL */
      segment_list = create_sl (sr_policy, segments, encap_src_v6addr, weight,
				sr_policy->is_encap);
		      ...
		      vlib_cli_command_t * cmd)
{
	  ...
	  if (!behavior)
	    {
	      return clib_error_return (0, "Invalid behavior");
	       ...
	       unformat (input, "sid_encoder %d", &sid_encoder_role_id))
	{
	  if (sid_encoder_role_id < 1)
	    {
	      return clib_error_return (
		0, "Invalid sid encoder role id. Please input zero or more.");
	    }
	  ip6_sr_sid_encoder_t *e;
  ...
  if (is_add)
	...
	return clib_error_return (0, "No Segment List specified");
      ...
      rv = sr_policy_add (&bsid, segments, encap_src_v6addr, weight, type,
			  fib_table, is_encap, behavior, ls_plugin_mem);
