					 ...
					 vlib_cli_command_t *cmd)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "iface %U", unformat_vnet_sw_interface, vnm,
		    &iface))
	;
      else
	break;
					  ...
					  vlib_cli_command_t *cmd)
{
  ...
  pool_foreach (ls, sr_pt->sr_pt_probe_inject_iface)
    {
      vec_add1 (sr_pt_probe_inject_iface_list, ls);
    };

  for (i = 0; i < vec_len (sr_pt_probe_inject_iface_list); i++)
    {
      ls = sr_pt_probe_inject_iface_list[i];
      vlib_cli_output (vm, "\tiface   : \t%U", format_vnet_sw_if_index_name,
		       vnm, ls->iface);
      vlib_cli_output (vm, "----------------------------------");
    }
