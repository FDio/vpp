...
static void
send_sr_pt_probe_inject_iface_details (sr_pt_probe_inject_iface_t *t, vl_api_registration_t *reg,
			  u32 context)
{
  ...
  rmp = vl_msg_api_alloc (sizeof (*rmp));
  clib_memset (rmp, 0, sizeof (*rmp));
  rmp->_vl_msg_id = ntohs (REPLY_MSG_ID_BASE + VL_API_SR_PT_PROBE_INJECT_IFACE_DETAILS);
  rmp->sw_if_index = ntohl (t->iface);
  rmp->context = context;
...
static void
vl_api_sr_pt_probe_inject_iface_dump_t_handler (vl_api_sr_pt_probe_inject_iface_dump_t *mp)
{
  vl_api_registration_t *reg;
  sr_pt_main_t *pt = &sr_pt_main;
...
static void
vl_api_sr_pt_probe_inject_iface_add_t_handler (vl_api_sr_pt_probe_inject_iface_add_t *mp)
{
...
static void
vl_api_sr_pt_probe_inject_iface_del_t_handler (vl_api_sr_pt_probe_inject_iface_del_t *mp)
{
