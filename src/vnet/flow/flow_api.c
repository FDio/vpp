...
static int
flow_rule_v2_to_vnet_flow (vl_api_flow_rule_v2_t *f, vnet_flow_t *flow)
{
...
vl_api_flow_add_v2_t_handler (vl_api_flow_add_v2_t *mp)
{
  ...
  rv = flow_rule_v2_to_vnet_flow (&mp->flow, &flow);
  if (rv)
    goto out;
...
static void
vl_api_flow_range_create_t_handler (vl_api_flow_range_create_t *mp)
{
  vl_api_flow_range_create_reply_t *rmp;
  int rv = 0;
  u32 range_index = ~0;

  vnet_main_t *vnm = vnet_get_main ();
  rv = vnet_flow_create_range (vnm, (char *) mp->owner, ntohl (mp->count), &range_index);

  REPLY_MACRO2 (VL_API_FLOW_RANGE_CREATE_REPLY, ({ rmp->range_index = ntohl (range_index); }));
}

static void
vl_api_flow_range_free_t_handler (vl_api_flow_range_free_t *mp)
{
  vl_api_flow_range_free_reply_t *rmp;
  int rv = 0;

  vnet_main_t *vnm = vnet_get_main ();
  rv = vnet_flow_free_range (vnm, ntohl (mp->range_index));

  REPLY_MACRO (VL_API_FLOW_RANGE_FREE_REPLY);
}

static void
vl_api_flow_range_add_flow_t_handler (vl_api_flow_range_add_flow_t *mp)
{
  vl_api_flow_range_add_flow_reply_t *rmp;
  int rv = 0;
  vnet_flow_t flow;
  u32 range_flow_index = ~0;
  ...
  rv = vnet_flow_range_add (vnm, ntohl (mp->range_index), &flow, &range_flow_index);

out:
  REPLY_MACRO2 (VL_API_FLOW_RANGE_ADD_FLOW_REPLY,
		({ rmp->range_flow_index = ntohl (range_flow_index); }));
}

static void
vl_api_flow_range_del_flow_t_handler (vl_api_flow_range_del_flow_t *mp)
{
  vl_api_flow_range_del_flow_reply_t *rmp;
  int rv = 0;

  vnet_main_t *vnm = vnet_get_main ();
  rv = vnet_flow_range_del (vnm, ntohl (mp->range_index), ntohl (mp->range_flow_index));

  REPLY_MACRO (VL_API_FLOW_RANGE_DEL_FLOW_REPLY);
}

static void
vl_api_flow_range_enable_flow_t_handler (vl_api_flow_range_enable_flow_t *mp)
{
  vl_api_flow_range_enable_flow_reply_t *rmp;
  int rv = 0;

  vnet_main_t *vnm = vnet_get_main ();
  rv = vnet_flow_range_enable (vnm, ntohl (mp->range_index), ntohl (mp->range_flow_index),
			       ntohl (mp->hw_if_index));

  REPLY_MACRO (VL_API_FLOW_RANGE_ENABLE_FLOW_REPLY);
}

static void
vl_api_flow_range_disable_flow_t_handler (vl_api_flow_range_disable_flow_t *mp)
{
  vl_api_flow_range_disable_flow_reply_t *rmp;
  int rv = 0;

  vnet_main_t *vnm = vnet_get_main ();
  rv = vnet_flow_range_disable (vnm, ntohl (mp->range_index), ntohl (mp->range_flow_index),
				ntohl (mp->hw_if_index));

  REPLY_MACRO (VL_API_FLOW_RANGE_DISABLE_FLOW_REPLY);
}

static void
vl_api_flow_range_bind_flow_t_handler (vl_api_flow_range_bind_flow_t *mp)
{
  vl_api_flow_range_bind_flow_reply_t *rmp;
  int rv = 0;
  u32 range_flow_index = ~0;

  vnet_main_t *vnm = vnet_get_main ();
  rv = vnet_flow_range_bind (vnm, ntohl (mp->range_index), ntohl (mp->flow_index),
			     &range_flow_index);

  REPLY_MACRO2 (VL_API_FLOW_RANGE_BIND_FLOW_REPLY,
		({ rmp->range_flow_index = ntohl (range_flow_index); }));
}

static void
vl_api_flow_range_unbind_flow_t_handler (vl_api_flow_range_unbind_flow_t *mp)
{
  vl_api_flow_range_unbind_flow_reply_t *rmp;
  int rv = 0;
  u32 flow_index = ~0;

  vnet_main_t *vnm = vnet_get_main ();
  rv = vnet_flow_range_unbind (vnm, ntohl (mp->range_index), ntohl (mp->range_flow_index),
			       &flow_index);

  REPLY_MACRO2 (VL_API_FLOW_RANGE_UNBIND_FLOW_REPLY,
		({ rmp->flow_index = ntohl (flow_index); }));
}
