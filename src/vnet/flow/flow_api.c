...
vl_api_flow_async_range_enable_t_handler (vl_api_flow_async_range_enable_t *mp)
{
  ...
  if (!count)
    {
      rv = VNET_FLOW_ERROR_INVALID_VALUE;
      goto out;
    }

  vec_validate (flow_indices, count - 1);

  vec_foreach_index (i, flow_indices)
    flow_indices[i] = ntohl (mp->flow_indices[i]);
  ...
  if (rv >= 0)
    {
      success = rv;
      rv = rv != count ? VNET_FLOW_ERROR_INTERNAL : 0;
    }

  vec_free (flow_indices);

out:
  REPLY_MACRO2 (VL_API_FLOW_ASYNC_RANGE_ENABLE_REPLY, { rmp->n_success = htonl (success); });
}
...
out:
  REPLY_MACRO2 (VL_API_FLOW_ASYNC_RANGE_DISABLE_REPLY, { rmp->n_success = htonl (success); });
}
