...
static_always_inline int
vnet_flow_enable_disable (vnet_main_t *vnm, u32 flow_index, u32 hw_if_index,
			  uword passed_private_data, bool template, bool enable)
{
  ...
  rv = dev_ops_function (vnm, op, hi->dev_instance, flow_index, &private_data);
...
vnet_flow_del_inline (vnet_main_t *vnm, u32 flow_index, bool template)
{
  ...
  for (int i = vec_len (f->if_data) - 1; i >= 0; i--)
    vnet_flow_enable_disable (vnm, flow_index, f->if_data[i].hw_if_index, 0, template, false);
...
{
  return vnet_flow_enable_disable (vnm, flow_index, hw_if_index, 0, false, false);
}
