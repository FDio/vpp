...
vnet_flow_config (vlib_main_t *vm, unformat_input_t *input)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	return clib_error_return (0, "unknown flow config: `%U`",
...
vnet_flow_cache_refill (vnet_flow_main_t *fm, vnet_flow_per_thread_data_t *ptd,
			u32 n)
{
  vnet_flow_t *f;
...
static_always_inline void
vnet_flow_cache_drain (vnet_flow_main_t *fm, vnet_flow_per_thread_data_t *ptd,
		       u32 n)
{
  clib_spinlock_lock (&fm->flow_pool_lock);
  for (u32 i = 0; i < n; i++)
		      ...
		      bool template)
{
  ...
  if (!template && fm->per_thread_data)
  ...
  else
    {
      ...
      vnet_flow_t **ppool =
	template ? &fm->global_flow_template_pool : &fm->global_flow_pool;
  ...
  if (flow->type == VNET_FLOW_TYPE_GENERIC)
    {
      ...
      clib_memcpy_fast (f->generic_pattern, flow->generic_pattern,
			sizeof (generic_pattern_t));
    }
  else
    {
			  ...
			  bool template, bool enable)
{
  ...
  if (enable)
    ASSERT (f->driver_data.hw_if_index != ~0);
  else
    ASSERT (f->driver_data.hw_if_index == ~0);
...
vnet_flow_del_inline (vnet_main_t *vnm, u32 flow_index, bool template)
{
  vnet_flow_main_t *fm = &flow_main;
  vnet_flow_t *f =
    template ? vnet_get_flow_template (flow_index) : vnet_get_flow (flow_index);
  int rv;
