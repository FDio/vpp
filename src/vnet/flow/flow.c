			  ...
			  bool template, bool enable)
{
  ...
  /* when disabling a flow/template, we fetch the hw_if_index directly from the flow structure, so
   * the argument is ignored and expected to be unset */
  ASSERT (!enable && hw_if_index == ~0);
  ...
  /* we check that the driver callback has properly installed the flow and set hw_if_index.
   * if not, we return an error and consider the flow as not installed.*/
  if ((enable && !IS_FLOW_ENABLED (f)) || (!enable && IS_FLOW_ENABLED (f)))
...
int
vnet_flow_template_disable (vnet_main_t *vnm, u32 flow_index)
{
  return vnet_flow_enable_disable (vnm, flow_index, ~0, 0, true, false);
}
			      ...
			      u32 hw_if_index)
{
  ...
  if (!IS_FLOW_ENABLED (template))
    return VNET_FLOW_ERROR_NOT_SUPPORTED;
  else if (template->driver_data.hw_if_index != hw_if_index)
    return VNET_FLOW_ERROR_INVALID_VALUE;
  ...
  vec_foreach (fi, flow_indices)
      ...
      if (!f)
	{
      ...
      else if (IS_FLOW_ENABLED (f)) /* don't enable flow twice */
	{
	  rv = VNET_FLOW_ERROR_ALREADY_DONE;
	  goto error;
	}
      else if (f->actions != template->actions || f->type != template->type)
	{
	  rv = VNET_FLOW_ERROR_INVALID_VALUE;
	  goto error;
	}
      else if (f->actions & VNET_FLOW_ACTION_REDIRECT_TO_NODE)
	{
	  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
	  f->redirect_device_input_next_index =
	    vlib_node_add_next (vnm->vlib_main, hw->input_node_index, f->redirect_node_index);
  ...
  /* If the callback fails, some operations may already have completed. Successfully processed flows
   * must update driver_data; callers should inspect each requested flow to determine the remaining
   * work. */
...
vnet_flow_async_range_disable (vnet_main_t *vnm, u32 *flow_indices, u32 hw_if_index)
{
  ...
  vec_foreach (fi, flow_indices)
      ...
      if (!f)
	{
      ...
      else if (!IS_FLOW_ENABLED (f)) /* don't disable if not enabled */
	{
	  rv = VNET_FLOW_ERROR_ALREADY_DONE;
	  goto error;
	}
      else if (f->driver_data.hw_if_index != hw_if_index)
	{
	  rv = VNET_FLOW_ERROR_INVALID_VALUE;
	  goto error;
	}
