...
void
vnet_flow_set_private_data (vnet_main_t * vnm, u32 flow_index,
			    u32 hw_if_index, uword data)
{
  vnet_flow_t *f = vnet_get_flow (flow_index);
  vec_validate_init_empty (f->private_data, hw_if_index, ~0);
  f->private_data[hw_if_index] = data;
}

uword
vnet_flow_get_private_data (vnet_main_t * vnm, u32 flow_index,
			    u32 hw_if_index)
{
  vnet_flow_t *f = vnet_get_flow (flow_index);

  if (vec_len (f->private_data) <= hw_if_index)
    return ~0;

  return f->private_data[hw_if_index];
}
