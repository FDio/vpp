...
int
vnet_flow_add (vnet_main_t * vnm, vnet_flow_t * flow)
{
  vnet_flow_main_t *fm = &flow_main;
  vnet_flow_t *f;
  u32 flow_index;
  ...
  if (hash_get (fm->global_flow_pool_index_by_flow_id, flow->id) != 0)
...
int
vnet_flow_del (vnet_main_t * vnm, u32 flow_id)
{
  vnet_flow_main_t *fm = &flow_main;
  ...
  clib_bitmap_free (f->interface_bitmap);
  memset (f, 0, sizeof (*f));
...
int
vnet_flow_enable (vnet_main_t * vnm, u32 flow_id, u32 hw_if_index)
{
  vnet_flow_t *f = vnet_get_flow (flow_id);
  ...
  if (clib_bitmap_get (f->interface_bitmap, hw_if_index))
  ...
  rv = dev_class->flow_ops_function (VNET_FLOW_DEV_OP_ADD_FLOW,
				     hi->dev_instance, f);

  if (rv)
    return rv;

  f->interface_bitmap = clib_bitmap_set (f->interface_bitmap, hw_if_index, 1);
  return 0;
}

int
vnet_flow_disable (vnet_main_t * vnm, u32 flow_id, u32 hw_if_index)
{
  vnet_flow_t *f = vnet_get_flow (flow_id);
  vnet_hw_interface_t *hi;
  vnet_device_class_t *dev_class;
  ...
  if (clib_bitmap_get (f->interface_bitmap, hw_if_index) == 0)
    return VNET_FLOW_ERROR_ALREADY_DONE;

  hi = vnet_get_hw_interface (vnm, hw_if_index);
  dev_class = vnet_get_device_class (vnm, hi->dev_class_index);

  rv = dev_class->flow_ops_function (VNET_FLOW_DEV_OP_DEL_FLOW,
				     hi->dev_instance, f);

  if (rv)
    return rv;

  f->interface_bitmap = clib_bitmap_set (f->interface_bitmap, hw_if_index, 0);
  return 0;
}
