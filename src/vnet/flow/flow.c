...
int
vnet_flow_register_cb (u32 hw_if_index, vnet_flow_interface_cb_t * fn)
{
...
u32
vnet_flow_request_range (u32 n_entries)
{
  vnet_flow_main_t *fm = &flow_main;
  u32 rv;
...
{
  vnet_flow_main_t *fm = &flow_main;
  vnet_flow_t *f = vnet_device_get_flow (flow_id);
  uword hw_if_index;
...
vnet_flow_enable (u32 flow_id, u32 hw_if_index)
{
  ...
  return hwif->callback (VNET_FLOW_INTERFACE_ADD_FLOW, f, hw_if_index);
}
...
vnet_flow_disable (u32 flow_id, u32 hw_if_index)
{
  ...
  return hwif->callback (VNET_FLOW_INTERFACE_DEL_FLOW, f, hw_if_index);
}
