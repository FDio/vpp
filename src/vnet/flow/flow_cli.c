	  ...
	  if (f->actions & VNET_FLOW_ACTION_COUNT)
	    {
	      ...
	      if (rv == 0)
		{
		  vlib_cli_output (vm, "%-10s: %llu", "hits", counter.hits);
		  vlib_cli_output (vm, "%-10s: %llu", "bytes", counter.bytes);
		}
	      else
...
no_args:
  pool_foreach (f, fm->global_flow_pool)
      ...
      if ((f->actions & VNET_FLOW_ACTION_COUNT) && f->driver_data.hw_if_index != ~0)
	{
	  ...
	  int rv = vnet_flow_get_counter (vnm, f->index, &counter);
	  if (rv == 0)
	    vlib_cli_output (vm, "  hits %llu bytes %llu", counter.hits, counter.bytes);
	  else
	    vlib_cli_output (vm, "  counter error: %U", format_flow_error, rv);
	}
    }
