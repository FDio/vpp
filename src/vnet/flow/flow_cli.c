	      ...
	      vlib_cli_command_t * cmd_arg)
{
  ...
  if (id)
	...
	return clib_error_return (0, "no such flow");
      ...
      /* *INDENT-OFF* */
      clib_bitmap_foreach (hw_if_index, f->interface_bitmap,
        ({
		     ...
		     vlib_cli_command_t * cmd_arg)
{
  ...
  vlib_cli_output (vm, "%U", dev_class->format_flow, hi->dev_instance, 0);
  return 0;
}
	   ...
	   vlib_cli_command_t * cmd_arg)
{
  ...
  if (next_test_flow_id == ~0)
    vnet_flow_get_range (vnm, "Test CLI", 1000, &next_test_flow_id);
  ...
  switch (action)
	...
	return clib_error_return (0, "Please specify ip protocol");
      ...
      rv = vnet_flow_add (vnm, &flow);
      break;
    case FLOW_DEL:
      rv = vnet_flow_del (vnm, flow.id);
      break;
    case FLOW_ENABLE:
      rv = vnet_flow_enable (vnm, flow.id, hw_if_index);
      break;
    case FLOW_DISABLE:
      rv = vnet_flow_disable (vnm, flow.id, hw_if_index);
      break;
    default:
...
format_flow (u8 * s, va_list * args)
{
  ...
  s = format (s, "flow-id %u type %s active %u",
	      f->id, flow_type_strings[f->type],
	      clib_bitmap_count_set_bits (f->interface_bitmap));
