	   ...
	   vlib_cli_command_t * cmd_arg)
{
  ...
  ip4_address_and_mask_t ip4_tmp;
  ip6_address_and_mask_t ip6_tmp;
  ...
  clib_memset(&ip4_tmp, 0, sizeof (ip4_address_and_mask_t));
  clib_memset(&ip6_tmp, 0, sizeof (ip6_address_and_mask_t));
  clib_memset(&ip4s, 0, sizeof (ip4_address_and_mask_t));
  clib_memset(&ip4d, 0, sizeof (ip4_address_and_mask_t));
  clib_memset(&ip6s, 0, sizeof (ip6_address_and_mask_t));
  clib_memset(&ip6d, 0, sizeof (ip6_address_and_mask_t));
  clib_memset(&inner_ip4s, 0, sizeof (ip4_address_and_mask_t));
  clib_memset(&inner_ip4d, 0, sizeof (ip4_address_and_mask_t));
  clib_memset(&inner_ip6s, 0, sizeof (ip6_address_and_mask_t));
  clib_memset(&inner_ip6d, 0, sizeof (ip6_address_and_mask_t));
  clib_memset(&sport, 0, sizeof (ip_port_and_mask_t));
  clib_memset(&dport, 0, sizeof (ip_port_and_mask_t));
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	action = FLOW_DISABLE;
			 ...
			 &ip4_tmp)) {
		if (type == VNET_FLOW_TYPE_IP4_GTPU) {
			type = VNET_FLOW_TYPE_IP4_GTPU_IP4;
			clib_memcpy(&inner_ip4s, &ip4_tmp, sizeof(ip4_address_and_mask_t));
		}
			...
			clib_memcpy(&ip4s, &ip4_tmp, sizeof(ip4_address_and_mask_t));
	 }
		...
		if (type == VNET_FLOW_TYPE_IP4_GTPU) {
			type = VNET_FLOW_TYPE_IP4_GTPU_IP4;
			clib_memcpy(&inner_ip4d, &ip4_tmp, sizeof(ip4_address_and_mask_t));
		}
		else if (type == VNET_FLOW_TYPE_IP6_GTPU) {
			type = VNET_FLOW_TYPE_IP6_GTPU_IP4;
			clib_memcpy(&inner_ip4d, &ip4_tmp, sizeof(ip4_address_and_mask_t));
			...
			clib_memcpy(&ip4d, &ip4_tmp, sizeof(ip4_address_and_mask_t));
	 }
		...
		if (type == VNET_FLOW_TYPE_IP4_GTPU) {
			...
			clib_memcpy(&inner_ip6s, &ip6_tmp, sizeof(ip6_address_and_mask_t));
		}
		else if (type == VNET_FLOW_TYPE_IP6_GTPU) {
		...
		else
		{
		    ...
		    clib_memcpy(&ip6s, &ip6_tmp, sizeof(ip6_address_and_mask_t));
		}
	 }
		...
		if (type == VNET_FLOW_TYPE_IP4_GTPU) {
			type = VNET_FLOW_TYPE_IP4_GTPU_IP6;
			clib_memcpy(&inner_ip6d, &ip6_tmp, sizeof(ip6_address_and_mask_t));
		}
		else if (type == VNET_FLOW_TYPE_IP6_GTPU) {
			type = VNET_FLOW_TYPE_IP6_GTPU_IP6;
			clib_memcpy(&inner_ip6d, &ip6_tmp, sizeof(ip6_address_and_mask_t));
		...
		{
			type = VNET_FLOW_TYPE_IP6_N_TUPLE;
			clib_memcpy(&ip6d, &ip6_tmp, sizeof(ip6_address_and_mask_t));
		}
	 }
	...
	return clib_error_return (0, "Please specify at least one action");
	  ...
	  switch (type)
        ...
        case VNET_FLOW_TYPE_IP4_GTPU_IP6:
            clib_memcpy(&flow.ip4_n_tuple.src_addr, &ip4s, sizeof(ip4_address_and_mask_t));
            clib_memcpy(&flow.ip4_n_tuple.dst_addr, &ip4d, sizeof(ip4_address_and_mask_t));
            clib_memcpy(&flow.ip4_n_tuple.src_port, &sport, sizeof(ip_port_and_mask_t));
            clib_memcpy(&flow.ip4_n_tuple.dst_port, &dport, sizeof(ip_port_and_mask_t));
			...
			if (type == VNET_FLOW_TYPE_IP4_GTPC)
				...
				flow.ip4_gtpu.teid = teid;
		  		...
		  		clib_memcpy(&flow.ip4_gtpu_ip4.inner_src_addr, &inner_ip4s, sizeof(ip4_address_and_mask_t));
		  		...
		  		clib_memcpy(&flow.ip4_gtpu_ip6.inner_src_addr, &inner_ip6s, sizeof(ip6_address_and_mask_t));
		  		clib_memcpy(&flow.ip4_gtpu_ip6.inner_dst_addr, &inner_ip6d, sizeof(ip6_address_and_mask_t));
			}
	  	...
	  	case VNET_FLOW_TYPE_IP6_GTPU_IP6:
	  		...
	  		clib_memcpy(&flow.ip6_n_tuple.src_port, &sport, sizeof(ip_port_and_mask_t));
	  		clib_memcpy(&flow.ip6_n_tuple.dst_port, &dport, sizeof(ip_port_and_mask_t));
			...
			if (type == VNET_FLOW_TYPE_IP6_GTPC)
				...
				flow.ip6_gtpu.teid = teid;
		  		...
		  		clib_memcpy(&flow.ip6_gtpu_ip4.inner_src_addr, &inner_ip4s, sizeof(ip4_address_and_mask_t));
		  		...
		  		clib_memcpy(&flow.ip6_gtpu_ip6.inner_src_addr, &inner_ip6s, sizeof(ip6_address_and_mask_t));
		  		clib_memcpy(&flow.ip6_gtpu_ip6.inner_dst_addr, &inner_ip6d, sizeof(ip6_address_and_mask_t));
			}
