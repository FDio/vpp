...
flow_cli_del (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
  ...
  if (is_template)
    rv = vnet_flow_del_async_template (vnm, flow_index);
  else
...
flow_cli_enable (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
  ...
  u32 hw_if_index = ~0;
  u32 flow_index = ~0, template_index = ~0, n_flows = ~0;
  vnet_flow_range_t range = { .start = ~0, .count = ~0 };
  bool is_template = false;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "index %u", &flow_index))
	;
      else if (unformat (line_input, "template-index %u range %u %u", &template_index, &range.start,
			 &range.count))
	;
  ...
  if (is_template && flow_index == ~0)
    return clib_error_return (0, "flow index required");
  else if (!is_template &&
	   ((flow_index == ~0 && range.start == ~0) || (flow_index != ~0 && range.start != ~0)))
    return clib_error_return (0, "either specify index or range");
...
VLIB_CLI_COMMAND (flow_enable_cmd, static) = {
  .path = "flow enable",
  .short_help =
    "flow enable [template <n-flows>] [index <index> | template-index <index> range <start> "
    "<count>] <interface>",
...
flow_cli_disable (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
  ...
  u32 hw_if_index = ~0;
  u32 flow_index = ~0, template_index;
  vnet_flow_range_t range = { .start = ~0, .count = ~0 };
  bool is_template = false;
  ...
  if (is_template)
    ...
    rv = vnet_flow_async_disable (vnm, &range, template_index, hw_if_index);
...
VLIB_CLI_COMMAND (flow_disable_cmd, static) = {
  .path = "flow disable",
  .short_help =
    "flow disable [template] [index <index> | template-index <index> range <start> <end>]",
