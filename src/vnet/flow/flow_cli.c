...
unformat_ip_protocol_and_mask (unformat_input_t * input, va_list * args)
{
  ...
  if (unformat (input, "any"))
    ;
  else if (unformat (input, "%U/%u", unformat_ip_protocol, &prot, &mask))
    ;
  else if (unformat (input, "%u/%u", &prot, &mask))
    ;
