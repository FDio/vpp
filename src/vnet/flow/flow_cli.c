...
static clib_error_t *
parse_flow_params (vlib_main_t *vm, unformat_input_t *line_input, vnet_flow_t *flow,
		   u32 *hw_if_index_out)
{
  vnet_main_t *vnm = vnet_get_main ();
  enum
...
#undef _
      ...
      else if (unformat (line_input, "rss queues"))
	{
      ...
      else if (unformat (line_input, "%U", unformat_vnet_hw_interface, vnm, hw_if_index_out))
	;
      else
	return clib_error_return (0, "parse error: '%U'",
				  format_unformat_error, line_input);
...
test_flow (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
  ...
  switch (action)
    {
    case FLOW_ADD:
      error = parse_flow_params (vm, line_input, &flow, &hw_if_index);
      if (error)
	goto done;

      rv = vnet_flow_add_inline (vnm, &flow, &flow_index);
      if (!rv)
      ...
      if (flow_index == ~0)
	{
      ...
      rv = vnet_flow_del_inline (vnm, flow_index);
      break;
    case FLOW_ENABLE:
      if (flow_index == ~0 || hw_if_index == ~0)
	{
      ...
      rv = vnet_flow_enable_inline (vnm, flow_index, hw_if_index);
      break;
    case FLOW_DISABLE:
      if (flow_index == ~0 || hw_if_index == ~0)
	{
      ...
      rv = vnet_flow_disable_inline (vnm, flow_index, hw_if_index);
      break;
    default:
...
test_flow_range (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
  ...
  enum
  {
    ...
    RANGE_ENABLE,
    RANGE_DISABLE
  } action = RANGE_UNKNOWN_ACTION;

  u32 range_index = ~0, range_flow_index = ~0, hw_if_index = ~0;
  u32 count = 0;
  u8 *owner = 0;
  int rv = 0;
  clib_error_t *error = 0;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "create"))
	{
	  action = RANGE_CREATE;
	  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	    {
	      if (unformat (line_input, "owner %s", &owner))
		;
      ...
      else if (unformat (line_input, "free"))
	{
	  ...
	  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	    {
	      if (unformat (line_input, "range %u", &range_index))
		;
	      else
		{
		  error =
      ...
      else if (unformat (line_input, "add"))
	{
	  ...
	  if (!unformat (line_input, "range %u", &range_index))
	    {
	      error = clib_error_return (0, "expected 'range <index>'");
	      goto done;
	    }
	  break;
	}
	...
	action = RANGE_DISABLE;
      else if (unformat (line_input, "range %u", &range_index))
	;
      else if (unformat (line_input, "range-flow-index %u", &range_flow_index))
	;
      else if (unformat (line_input, "%U", unformat_vnet_hw_interface, vnm, &hw_if_index))
	;
      else
  ...
  switch (action)
	...
	if (!owner || count == 0)
	  {
	    error = clib_error_return (0, "Please specify owner and count > 0");
	    goto done;
	  }
	...
	if (range_index == ~0)
	  {
	    error = clib_error_return (0, "Please specify range index");
	    goto done;
	  }
	rv = vnet_flow_free_range (vnm, range_index);
	break;
      }
    case RANGE_ADD:
      {
	...
	rv = vnet_flow_range_add (vnm, range_index, &flow, &range_flow_index);
	if (!rv)
	  vlib_cli_output (vm, "range-flow-index %u added", range_flow_index);
	break;
      }
    case RANGE_DEL:
      {
	if (range_index == ~0 || range_flow_index == ~0)
	  {
	    error = clib_error_return (0, "Please specify range and range-flow-index");
	    goto done;
	  }
	rv = vnet_flow_range_del (vnm, range_index, range_flow_index);
	break;
      }
    case RANGE_ENABLE:
      {
	if (range_index == ~0 || range_flow_index == ~0 || hw_if_index == ~0)
	  {
	    error = clib_error_return (0, "Please specify range, range-flow-index, and interface");
	    goto done;
	  }
	rv = vnet_flow_range_enable (vnm, range_index, range_flow_index,
				     hw_if_index);
	break;
      }
    case RANGE_DISABLE:
	...
	rv = vnet_flow_range_disable (vnm, range_index, range_flow_index,
				      hw_if_index);
	break;
      }
    default:
      error =
	clib_error_return (0, "please specify action (create, free, add, del, enable, disable)");
      goto done;
    }
...
VLIB_CLI_COMMAND (test_flow_range_command, static) = {
  ...
  .short_help = "test flow range create owner <name> count <n>\n"
		"test flow range free range <range-index>\n"
		"test flow range add range <range-index> [flow params...]\n"
		"test flow range del range <range-index> range-flow-index <idx>\n"
		"test flow range enable range <range-index> range-flow-index <idx> <interface>\n"
		"test flow range disable range <range-index> range-flow-index <idx> <interface>",
