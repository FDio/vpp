	   ...
	   vlib_cli_command_t * cmd_arg)
{
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	action = FLOW_DISABLE;
			 ...
			 unformat_ip4_address_and_mask, &ip4_tmp))
	{
	  if (type == VNET_FLOW_TYPE_IP4_GTPU)
	    {
	      type = VNET_FLOW_TYPE_IP4_GTPU_IP4;
	      clib_memcpy (&inner_ip4s, &ip4_tmp,
			 ...
			 unformat_ip4_address_and_mask, &ip4_tmp))
	{
	  if (type == VNET_FLOW_TYPE_IP4_GTPU)
	    {
	      type = VNET_FLOW_TYPE_IP4_GTPU_IP4;
	      clib_memcpy (&inner_ip4d, &ip4_tmp,
			   sizeof (ip4_address_and_mask_t));
	    }
	  else if (type == VNET_FLOW_TYPE_IP6_GTPU)
	    {
	    ...
	    clib_memcpy (&ip4d, &ip4_tmp, sizeof (ip4_address_and_mask_t));
			 ...
			 unformat_ip6_address_and_mask, &ip6_tmp))
	{
	  if (type == VNET_FLOW_TYPE_IP4_GTPU)
	    {
	      type = VNET_FLOW_TYPE_IP4_GTPU_IP6;
	      clib_memcpy (&inner_ip6s, &ip6_tmp,
			 ...
			 unformat_ip6_address_and_mask, &ip6_tmp))
	{
	  if (type == VNET_FLOW_TYPE_IP4_GTPU)
	    {
	      type = VNET_FLOW_TYPE_IP4_GTPU_IP6;
	      clib_memcpy (&inner_ip6d, &ip6_tmp,
			   sizeof (ip6_address_and_mask_t));
	    }
	  else if (type == VNET_FLOW_TYPE_IP6_GTPU)
	    {
