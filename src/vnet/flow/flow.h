...
typedef struct
{
  ...
  /* queue for VNET_FLOW_ACTION_BUFFER_ADVANCE */
  i32 buffer_advance;
...
#undef _
  ...
  clib_bitmap_t *interface_bitmap;
} vnet_flow_t;

int vnet_flow_get_range (vnet_main_t * vnm, char *owner, u32 count,
			 u32 * start);
int vnet_flow_add (vnet_main_t * vnm, vnet_flow_t * flow);
int vnet_flow_enable (vnet_main_t * vnm, u32 flow_id, u32 hw_if_index);
int vnet_flow_disable (vnet_main_t * vnm, u32 flow_id, u32 hw_if_index);
int vnet_flow_del (vnet_main_t * vnm, u32 flow_id);
