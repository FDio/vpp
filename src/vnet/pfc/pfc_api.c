...
void
vl_api_pfc_sys_get_capabilities_t_handler (vl_api_pfc_sys_get_capabilities_t *mp)
{
  vnet_main_t *vnm = vnet_get_main ();
  vnet_sw_interface_t *sw = vnet_get_sup_sw_interface (vnm, clib_net_to_host_u32 (mp->sw_if_idx));
  ...
  vl_api_pfc_sys_disable_pause_frame_flow_ctrl_t *mp)
{
  ...
  rv = pfc_sys_disable_pause_frame_flow_ctrl (sw->hw_if_index, clib_host_to_net_u32 (mp->disable));
