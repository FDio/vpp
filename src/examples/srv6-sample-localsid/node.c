...
format_srv6_localsid_sample_trace (u8 *s, va_list *args)
{
  ...
  srv6_localsid_sample_trace_t *t =
    va_arg (*args, srv6_localsid_sample_trace_t *);
  s =
    format (s, "SRv6-sample-localsid: localsid_index %d\n", t->localsid_index);
  return s;
}
...
end_srh_processing (vlib_node_runtime_t *node, vlib_buffer_t *b0,
		    ip6_header_t *ip0, ip6_sr_header_t *sr0,
		    ip6_sr_localsid_t *ls0, u32 *next0, u8 psp,
		    ip6_ext_header_t *prev0)
{
  ...
  if (PREDICT_TRUE (sr0->type == ROUTING_HEADER_TYPE_SR))
      ...
      if (sr0->segments_left == 1 && psp)
	{
	   ...
	   * By default in absence of extension headers it is equal to length
	   * of ip6 header With extension headers it number of 8 octet units of
	   * ext headers preceding SR header
	   */
	  copy_len_u64s0 =
 ...
 */
static uword
srv6_localsid_sample_fn (vlib_main_t *vm, vlib_node_runtime_t *node,
			 vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  /* INSERT CODE HERE */
	  /* Example starts here */
	  // In this example we are changing the next VRF table by the one in
	  // CLI
	  ...
	  if (PREDICT_FALSE (b0->flags & VLIB_BUFFER_IS_TRACED))
	    {
	      srv6_localsid_sample_trace_t *tr =
