...
/*
 * Simple dual/single loop version
		...
		vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  src_dst0 = __builtin_shuffle (src_dst0, swapmac);
	  src_dst1 = __builtin_shuffle (src_dst1, swapmac);
		...
		vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from >= 4)
      ...
      if (PREDICT_TRUE (n_left_from >= 8))
	{
      ...
      src_dst0 = __builtin_shuffle (src_dst0, swapmac);
      src_dst1 = __builtin_shuffle (src_dst1, swapmac);
      src_dst2 = __builtin_shuffle (src_dst2, swapmac);
      src_dst3 = __builtin_shuffle (src_dst3, swapmac);
