...
class TestIpIrb(VppTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.pg0.remote_mac_extend = cls.loop0.remote_mac_extend[:100]
        cls.pg0.remote_ip4_extend = cls.loop0.remote_ip4_extend[:100]
        cls.pg1.remote_mac_extend = cls.loop0.remote_mac_extend[100:]
        cls.pg1.remote_ip4_extend = cls.loop0.remote_ip4_extend[100:]
    ...
    def create_stream(self, src_ip_if, dst_ip_if, packet_sizes):
        ...
        for i in range(0, 257):
            ...
            p = (Ether(dst=src_ip_if.local_mac, src=src_ip_if.remote_mac) /
                 IP(src=src_ip_if.remote_ip4, dst=choice(dst_ip_if.remote_ip4_extend)) /
    ...
    def create_stream_l2_to_ip(self, src_l2_if, src_ip_if, dst_ip_if, packet_sizes):
        ...
        for i in range(0, 257):
            ...
            i = randint(0, len(src_l2_if.remote_mac_extend)-1)

            p = (Ether(dst=src_ip_if.local_mac, src=src_l2_if.remote_mac_extend[i]) /
                 IP(src=src_l2_if.remote_ip4_extend[i], dst=dst_ip_if.remote_ip4) /
                 UDP(sport=1234, dport=1234) /
                 Raw(payload))
    ...
    def verify_capture_l2_to_ip(self, dst_ip_if, src_ip_if, capture):
        ...
        for packet in capture:
            ...
            # IP: src, dst
            self.assertIn(ip.src, src_ip_if.remote_ip4_extend)
            self.assertEqual(ip.dst, saved_packet[IP].dst)
            self.assertIn(ip.dst, dst_ip_if.remote_ip4)
    ...
    def test_ip4_irb_2(self):
        ...
        stream1 = self.create_stream_l2_to_ip(self.pg0, self.loop0, self.pg2, self.pg_if_packet_sizes)
        stream2 = self.create_stream_l2_to_ip(self.pg1, self.loop0, self.pg2, self.pg_if_packet_sizes)
