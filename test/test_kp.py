...
class TestKP(VppTestCase):
    ...
    def tearDown(self):
        ...
        if not self.vpp_dead:
            self.logger.info(self.vapi.cli("show kp vip verbose"))
    ...
    def checkCapture(self, nat4, isv4):
        ...
        for podid in self.pods:
            if load[podid] < len(self.packets) / (len(self.pods) * 2):
                self.log(
                    "Pod is not balanced: load[%d] = %d" % (podid, load[podid]))
                raise Exception("Kube-proxy algorithm is biased")

    def test_kp_ip4_nat4(self):
        """ Kube-proxy NAT44 """
        try:
            self.vapi.cli("kp vip 90.0.0.0/8 port 3306 target_port 3307 node_port 30964 nat4")
            for podid in self.pods:
                self.vapi.cli("kp pod 90.0.0.0/8 10.0.0.%u" % (podid))
            ...
            for podid in self.pods:
                self.vapi.cli("kp pod 90.0.0.0/8 10.0.0.%u del" % (podid))
            self.vapi.cli("kp vip 90.0.0.0/8 nat4 del")
    ...
    def test_kp_ip6_nat4(self):
        ...
        try:
            self.vapi.cli("kp vip 2001::/16 encap gre4")
            for podid in self.pods:
                self.vapi.cli("kp pod 2001::/16 10.0.0.%u" % (podid))
            ...
            for podid in self.pods:
                self.vapi.cli("kp pod 2001::/16 10.0.0.%u del" % (podid))
            self.vapi.cli("kp vip 2001::/16 nat4 del")

    @unittest.skipUnless(running_extended_tests(), "part of extended tests")
    def test_kp_ip4_nat6(self):
        """ Kube-proxy NAT46 """
        try:
            self.vapi.cli("kp vip 90.0.0.0/8 encap gre6")
            for podid in self.pods:
                self.vapi.cli("kp pod 90.0.0.0/8 2002::%u" % (podid))
            ...
            for podid in self.pods:
                self.vapi.cli("kp pod 90.0.0.0/8 2002::%u" % (podid))
            self.vapi.cli("kp vip 90.0.0.0/8 nat6 del")

    @unittest.skipUnless(running_extended_tests(), "part of extended tests")
    def test_kp_ip6_nat6(self):
        """ Kube-proxy NAT66 """
        try:
            self.vapi.cli("kp vip 2001::/16 encap gre6")
            for podid in self.pods:
                self.vapi.cli("kp pod 2001::/16 2002::%u" % (podid))
            ...
            for podid in self.pods:
                self.vapi.cli("kp pod 2001::/16 2002::%u del" % (podid))
            self.vapi.cli("kp vip 2001::/16 nat6 del")
