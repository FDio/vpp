...
class SBFDState(NumericConstant):
    """ BFD State """
    admin_down = 0
    down = 1
    up = 3
...
class SBFD(Packet):
    ...
    udp_dport = 7784  #: BFD destination port per RFC 5881
    ...
    fields_desc = [
        ...
        BitEnumField("state", 0, 2, SBFDState.desc_dict),
    ...
    def mysummary(self):
        return self.sprintf("BFD(my_disc=%SBFD.my_discriminator%,"
                            "your_disc=%SBFD.your_discriminator%)")

# glue the BFD packet class to scapy parser
bind_layers(UDP, SBFD, dport=SBFD.udp_dport)
