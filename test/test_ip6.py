...
class TestIPv6(TestIPv6ND):
    ...
    def create_packet(self, src_if, packet_size):
        """Create packet with required packet size for defined interface.

        :param VppInterface src_if: Interface to create packet for.
        :param int packet_size: Required packet size.
        """
        dst_if_idx = packet_size % 2 if packet_size < 1500 else \
            packet_size / 10 % 2
    ...
    def create_stream(self, src_if):
        ...
        for i in xrange(64, 1500):
            pkt = self.create_packet(src_if, i)
            pkts.append(pkt)
        for i in xrange(1500 + hdr_ext, 9020 + hdr_ext, 10):
            pkt = self.create_packet(src_if, i)
            pkts.append(pkt)
