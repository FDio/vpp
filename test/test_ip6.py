...
class TestIPv6(VppTestCase):
    ...
    def validate_ra(self, intf, rx, dst_ip=None, mtu=9000, pi_opt=None):
        ...
        if not pi_opt:
            ...
            ra_opts = ra[ICMPv6NDOptPrefixInfo]
            if type(pi_opt) is list:
                for ii in range(len(pi_opt)):
                    print type(pi_opt[ii])
                    print type(ra_opts[ii][0])
                    print type(ra_opts)
                    self.assertEqual(pi_opt[ii], ra_opts[ii][0])
            else:
                self.assertEqual(pi_opt, ra_opts)
    ...
    def test_rs(self):
        ...
        #
        # Remove the first refix-info - expect the second is still in the advert
        #
        self.pg0.ip6_ra_prefix(self.pg0.local_ip6n,
                               self.pg0.local_ip6_prefix_len,
                               is_no=1)
