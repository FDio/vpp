...
class TestIPFibSource(VppTestCase):
    ...
    def test_fib_source(self):
        ...
        routes = self.vapi.ip_route_dump(0, True)
        ...
        routes = self.vapi.ip_route_dump(0, True, src=adj_source.id)
        self.assertEqual(len(routes), 4)
        routes = self.vapi.ip_route_dump(0, True, src=intf_source.id)
        self.assertEqual(len(routes), 4)
        routes = self.vapi.ip_route_dump(0, True, src=special_source.id)
        self.assertEqual(len(routes), 1)
        routes = self.vapi.ip_route_dump(0, True, src=dr_source.id)
        self.assertEqual(len(routes), 1)
        ...
        # add a route with eh default API source
        r1 = VppIpRoute(self, "2001::1", 128,
                        [VppRoutePath(self.pg0.remote_ip6,
                                      self.pg0.sw_if_index)]).add_vpp_config()

        r2 = VppIpRoute(self, "2001::1", 128,
                        [VppRoutePath(self.pg1.remote_ip6,
                                      self.pg1.sw_if_index)],
