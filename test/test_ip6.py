...
class TestIP6Punt(VppTestCase):
    ...
    def test_ip_punt(self):
        ...
        pkts = p * 1025
        paths = [VppRoutePath(self.pg1.remote_ip6,
                              self.pg1.sw_if_index,
                              proto=DpoProto.DPO_PROTO_IP6).encode(True)]

        #
        # Configure a punt redirect via pg1.
        #
        nh_addr = self.pg1.remote_ip6
        self.vapi.ip_punt_redirect(self.pg0.sw_if_index, paths, is_ip6=1)
        ...
        #
        self.vapi.ip_punt_redirect(0xffffffff, paths, is_ip6=1)
        self.send_and_expect(self.pg0, pkts, self.pg1)

        self.vapi.ip_punt_redirect(0xffffffff, [], is_add=0, is_ip6=1)
    ...
    def test_ip_punt_dump(self):
        ...
        #
        nh_paths = [VppRoutePath(self.pg3.remote_ip6,
                                 self.pg3.sw_if_index,
                                 proto=DpoProto.DPO_PROTO_IP6).encode(True)]
        unnum_paths = [VppRoutePath("0::0",
                                    self.pg3.sw_if_index,
                                    proto=DpoProto.DPO_PROTO_IP6).encode(True)]
        ...
        self.vapi.ip_punt_redirect(self.pg2.sw_if_index, unnum_paths,
                                   is_ip6=1)
        self.logger.info(self.vapi.cli("sh ip6 punt redirect"))
