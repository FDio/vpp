...
def find_abf_attach(test, id, sw_if_index):
    attachs = test.vapi.abf_attach_dump()
    for a in attachs:
        if id == a.policy_id and sw_if_index == a.sw_if_index:
            return True
    return False
...
class VppAbfPolicy(VppObject):
    ...
    def paths_encode(self):
        ...
        for p in self.paths:
            vpaths.append({'next_hop': p.nh_addr,
                           'next_hop_sw_if_index': p.sw_if_index,
                           'next_hop_weight': 1,
                           'next_hop_afi': 0,
    ...
    def add_vpp_config(self):
        self._test.vapi.abf_policy_add_del(
            {'is_add': 1,
             'is_replace': 1,
             'details': {'policy_id': self.policy_id,
    ...
    def query_vpp_config(self):
        return find_abf_policy(self._test, self.id)
...
class VppAbfAttach(VppObject):
    ...
    def add_vpp_config(self):
        self._test.vapi.abf_attach_add_del(
            {'is_add': 1,
             'details': {'policy_id': self.policy_id,
    ...
    def query_vpp_config(self):
        return find_abf_attach(self._test, self.id)
