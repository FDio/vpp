...
class TestEthernet(VppTestCase):
    ...
    def set_promiscuous_mode(self, state: str):
        for i in self.pg_interfaces:
            self.vapi.cli(f"set interface promiscuous {state} {i.name}")

    def test_dmac_filtering(self):
        self.check_mixed_dmacs_filter_result()

    def test_no_dmac_filtering_promisc(self):
        self.addCleanup(self.set_promiscuous_mode, "off")

        self.set_promiscuous_mode("on")
        self.check_mixed_dmacs_filter_result("on")
        self.set_promiscuous_mode("off")
        self.check_mixed_dmacs_filter_result("off")

    def check_mixed_dmacs_filter_result(self, promiscuous_state="off"):
        """
        ...
        Depending on whether the interface is is promiscuous mode,
        ...
        rxs = self.send_and_expect(
            ...
            stats_diff=StatsDiff(
                {"err": {"/err/ethernet-input/l3_mac_mismatch": len(dropped_pkts)}}
            )
        )
