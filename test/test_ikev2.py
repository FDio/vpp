...
class IKEv2SA(object):
    def __init__(self, test, is_initiator=True, spi=b'\x04' * 8,
        ...
        if self.is_initiator:
            ...
            self.r_id = id
            self.r_nonce = None
        self.childs = [IKEv2ChildSA(local_ts, remote_ts)]
    ...
    def generate_dh_data(self):
        ...
        if self.is_initiator:
            if self.ike_dh not in DH:
                raise Exception('%s not in DH group' % self.ike_dh)
...
class TestResponder(VppTestCase):
    ...
    def tearDownClass(cls):
        ...
        split_layers(UDP, ikev2.IKEv2, sport=500)
        split_layers(UDP, ikev2.IKEv2, dport=500)
