...
class TestTLS(VppTestCase):
    ...
    def test_tls_transfer(self):
        ...
        qat = "tls openssl set engine qat alg rsa,pkey_crypto ciphers rsa async"
        error = self.vapi.cli(qat)
        ...
        error = self.vapi.cli("test echo server appns 0 fifo-size 4 tls-engine 1 uri " +
                              uri)
        if error:
            self.logger.critical(error)
            self.assertNotIn("failed", error)
        ...
        error = self.vapi.cli("test echo client mbytes 10 appns 1 "
                              "fifo-size 4 no-output test-bytes tls-engine 1 "
