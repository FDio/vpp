...
class TestSfdpNat(BaseSfdpTest):
    ...
    # Define a set of 30 NAT IPs
    # By default, first 13 IPs will be set in the NAT pool's 'hot' set
    # while all other IPs will be set in the NAT pool's remaining vector set.
    NAT_IPS = [f"198.18.0.{i}" for i in range(1, 30)]
    ...
    def _verify_snat_round_trip(self, protocol):
        ...
        self.assertIn(
            translated[IP].src, self.NAT_IPS
        )  # Check translated IP belongs in NAT IPs set
    ...
    def test_udp_snat_multiple_nat_addresses(self):
        """UDP flows with multiple addresses configured in NAT pool"""
        # Logic to map src_ips to NAT'd IPs currently relies on (source_ip % pool_size).
        # We test iteration over a NAT pool by sending packets with incremented source_ip
        source_ips = [f"193.0.2.{i}" for i in range(1, len(self.NAT_IPS) + 1)]
        forward = [
        ...
        # Send packets, check that IPs are NAT'd
        capture = self.send_and_expect(self.pg0, forward, self.pg1)
        for packet in capture:
            self.assert_packet_checksums_valid(packet)
            self.assertIn(packet[IP].src, self.NAT_IPS)

        # Check that SFDP sessions are created for sent traffic
        sessions = self.sessions()
        self.assertEqual(len(sessions), len(source_ips))
        for session in sessions:
            self.assertEqual(
                len(session.keys), 2
            )  # Primary and Secondary SFDP keys must exist
            self.assertIn(str(session.keys[0].init_addr), source_ips)
            self.assertIn(str(session.keys[1].init_addr), self.NAT_IPS)
