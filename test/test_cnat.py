...
class CnatTestContext(object):
    ...
    # We expect this to be NATed as pg2.remote[0]:<anyport> -> pg1.remote[0]:6661
    ...
    def cnat_translation(self):
        ...
        for nbr, translation in enumerate(self.translations):
            ...
            for src_pgi, sport in product(range(N_REMOTE_HOSTS), [1234, 1233]):
                ...
                #
                ctx.cnat_send(self.pg1, vip.ip, 6666, self.pg0, src_pgi, sport)
                # FIXME ctx.cnat_expect(self.pg1, nbr, 6666, self.pg0,
                # src_pgi, sport)
