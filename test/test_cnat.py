...
class CnatCommonTestCase(VppTestCase):
    ...
    # CNAT flow gauges are populated by a stat-segment collector. Keep its
    # refresh interval below the polling interval used by the assertions.
    extra_vpp_statseg_config = "update-interval 0.05"
    ...
    def assert_cnat_flow_stats(self, **expected):
        ...
        self.statistics.connect()
        actual = {}
        for _ in range(50):
            self.statistics.refresh()
            actual = {
                name: self.statistics.get_counter(paths[name]) for name in expected
            }
            if actual == expected:
                return
            self.sleep(0.1)
        for name, value in expected.items():
            self.assertEqual(actual[name], value, f"CNAT flow gauges: {actual}")
...
class CnatTestContext(object):
    ...
    def cnat_expect(
        self, src_pg, src_id, src_port, dst_pg, dst_id, dst_port, only_udp=False
    ):
        if isinstance(src_id, int):
        ...
        for rx in self.rxs:
            ...
            if self.L4PROTO in [TCP, UDP]:
                if not (only_udp) or self.L4PROTO == UDP:
...
class TestCNatTranslation(CnatCommonTestCase):
    ...
    def setUp(self):
        ...
        self.snat_address_configured = False
        self.snat_policy_interface = None
    ...
    def tearDown(self):
        ...
        if self.snat_policy_interface is not None:
            sw_if_index, table = self.snat_policy_interface
            self.vapi.cnat_snat_policy_add_del_if(
                sw_if_index=sw_if_index, is_add=0, table=table
            )
        if self.snat_address_configured:
    ...
    def make_encap_vxlan(self):
        ...
        VppNeighbor(
            self, rv.sw_if_index, "00:11:22:33:44:55", "2.2.2.2", is_static=True
        ).add_vpp_config()
        VppNeighbor(
            self, rrv.sw_if_index, "00:11:22:33:44:66", "3.3.3.3", is_static=True
        ).add_vpp_config()
        ...
        self.logger.info(self.vapi.cli("sh vxlan tunnel"))
        self.logger.info(self.vapi.cli("sh int addr"))
        self.logger.info(self.vapi.cli("sh ip fib"))

    def make_encap(self):
        rv = VppIpIpTunInterface(
            self,
            self.pg2,
            self.get_ip46(self.pg1.remote_hosts[0]),
            self.get_ip46(self.pg2.remote_hosts[0]),
        ).add_vpp_config()
        rrv = VppIpIpTunInterface(
            self,
            self.pg1,
            self.get_ip46(self.pg2.remote_hosts[0]),
            self.get_ip46(self.pg1.remote_hosts[0]),
    ...
    def cnat_encap_vxlan(self):
        ...
        for nbr, translation in enumerate(self.translations):
            ...
            for src_pgi, sport in product(range(N_REMOTE_HOSTS), [1234, 1233]):
                ...
                ctx.cnat_expect(
                    self.pg1, 0, sport, self.pg2, nbr, dst_port, only_udp=True
                )
                # from vip to client
    ...
    def cnat_encap(self):
        ...
        self.logger.info(self.vapi.cli("sh cnat session verbose 1"))
        self.logger.info(self.vapi.cli("sh cnat timestamp verbose 1"))
        # test no nat encap
        for nbr, translation in enumerate(self.translations):
            vip = translation.vip

            ctx = CnatTestContext(self, translation.iproto, vip.is_v6)
            for sport in [1233, 1234]:
    ...
    def cnat_update(self):
        ...
        for nbr, translation in enumerate(self.mbtranslations):
            vip = translation.vip
            sessions_per_backend[translation.paths[0][DST].port] = 0
            sessions_per_backend[translation.paths[1][DST].port] = 0
            #
            # Test Flows to the VIP
            #
            ctx = CnatTestContext(self, translation.iproto, vip.is_v6)
    ...
    def test_cnat_vip_no_matching_translation(self):
        self._make_translations_v4()
        self.assert_cnat_flow_stats(total=0, nat=0, no_nat=0, pass_through=0)
        ...
        # The VIP address matches, but the destination port does not.
        ctx = CnatTestContext(self, TCP, is_v6=False)
        ctx.cnat_send(
            self.pg0,
            0,
            1234,
        ...
        # The VIP address and port match, but the protocol does not.
        ...
        self.assert_cnat_flow_stats(total=2, nat=0, no_nat=0, pass_through=2)
    ...
    def test_cnat_no_nat(self):
        ...
        self.vapi.cnat_session_purge()
        # cnat_session_purge removes configured translations as well.
        self.translations = []
        self.assert_cnat_flow_stats(total=0, nat=0, no_nat=0, pass_through=0)
        self.cnat_enable_features(0)

    def test_cnat_no_active_backend(self):
        self.create_loopback_interfaces(1)
        backend_if = self.lo_interfaces[0]
        backend_if.admin_up()
        ...
        self.translations = [
            Translation(
                ...
                [
                    (
                        Endpoint(is_v6=False),
                        Endpoint(pg=backend_if, port=4001, is_v6=False),
                    )
                ],
                0x9F,
            ).add_vpp_config(CNAT_TR_FLAG_NO_CLIENT)
        ]
        self.mbtranslations = []
        self.cnat_enable_features()

        error_path = "/err/cnat-input-ip4/no active backend"
        error_before = self.statistics.get_err_counter(error_path)

        translation = self.translations[0]
        vip = translation.vip
        ctx = CnatTestContext(self, translation.iproto, vip.is_v6)
        ctx.cnat_send(self.pg0, 0, 1234, self.pg2, vip.ip, vip.port, no_replies=True)

        self.assertEqual(self.statistics.get_err_counter(error_path) - error_before, 1)
        self.assert_cnat_flow_stats(nat=0, no_nat=0, pass_through=0)

        self.cnat_enable_features(0)

    def test_cnat_snat_address_unavailable(self):
        self.translations = []
        self.mbtranslations = []

        self.create_loopback_interfaces(1)
        snat_if = self.lo_interfaces[0]
        snat_if.admin_up()

        # Select an interface with no IPv4 address.
        self.vapi.cnat_set_snat_addresses(sw_if_index=snat_if.sw_if_index)
        self.snat_address_configured = True
        ...
        self.snat_policy_interface = (
            self.pg0.sw_if_index,
            policy_tables.CNAT_POLICY_INCLUDE_V4,
        )
        self.cnat_enable_features()

        error_path = "/err/cnat-output-ip4/snat address unavailable"
        error_before = self.statistics.get_err_counter(error_path)

        ctx = CnatTestContext(self, TCP, is_v6=False)
        ctx.cnat_send(self.pg0, 0, 1234, self.pg1, 0, 6661, no_replies=True)
    ...
    def test_fib_delete_translation(self):
        ...
        self._make_translations_v4()
        self.cnat_enable_features()  # this is needed for init
        self.cnat_delete()
        self.cnat_enable_features(0)
