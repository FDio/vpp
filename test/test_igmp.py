...
class TestIgmpProxy(VppTestCase):
	...
	# pg0 sends IGMP 'join'
	# as this is the first source in group 224.1.1.1
	...
	# src address is swaped with proxy-interfaces address (pg0)
        self.assertEqual(gr.srcaddrs[0], self.pg0._local_ip4)
        ...
        self.send(self.pg1, p_join)
        self.sleep(1)
	# interest in group 224.1.1.1 was already reported
        ...
        self.sleep(1)
	# there are still some sources interested in 224.1.1.1 group
        self.pg0.assert_nothing_captured()
	...
	# pg0 sends IGMP 'leave'
	# as this was the last source in group 224.1.1.1
        capture = self.pg0.get_capture(1, timeout=1)
