    ...
    def create_bd_and_mac_learn(self, count, start=1):
        ...
        for b in range(start, start + count):
            if b == start:
                ...
                ret = self.vapi.bridge_domain_allocate(bd_id=0xFFFFFFFF, flood=1, uu_flood=1, forward=1, learn=1)
                bd_id = ret.bd_id
    ...
    def delete_bd(self, count, start=1):
        ...
        for b in range(start, start + count):
            ...
            for j in self.bd_if_range(b):
                ...
                self.logger.info(
                    "pg-interface %s removed from bridge domain ID %d" % (pg_if.name, bd_id)
                )
