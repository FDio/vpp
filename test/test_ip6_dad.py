...
# NOTE: Using self.sleep_on_vpp_time() for VPP virtual time
# DAD timers use VPP virtual time, so we must use sleep_on_vpp_time() to advance
# VPP's virtual clock instead of real time sleep(). This keeps tests deterministic.
...
class TestIP6DAD(VppTestCase):
    ...
    def test_dad_conflict_detected(self):
        ...
        # Address should REMAIN configured (NEW BEHAVIOR: IP not removed on duplicate)
        self.assert_address_present(
            self.pg0.sw_if_index, test_address
        )  # NEW: IP remains configured
    ...
    def test_dad_loopback_exemption(self):
        ...
        # VPP skips DAD for loopback addresses (exemption in ip6_dad_start())
        self.vapi.sw_interface_add_del_address(
            sw_if_index=self.pg0.sw_if_index,
    ...
    def test_dad_multicast_exemption(self):
        ...
        # VPP skips DAD for multicast addresses (exemption in ip6_dad_start())
        self.vapi.sw_interface_add_del_address(
            sw_if_index=self.pg0.sw_if_index,
...
class TestIP6DADEvents(VppTestCase):
    ...
    @classmethod
    def tearDownClass(cls):
        super(TestIP6DADEvents, cls).tearDownClass()

    def setUp(self):
        super(TestIP6DADEvents, self).setUp()
    ...
    def tearDown(self):
        # Unregister from DAD events
        self.vapi.want_ip6_dad_events(enable_disable=False)
        ...
        super(TestIP6DADEvents, self).tearDown()

    def verify_dad_event(
        self,
        event,
        expected_state,
        sw_if_index,
        address,
        expected_count=None,
        expected_transmits=None,
    ):
        """Verify a DAD event has correct fields"""
        self.assertEqual(
            event.state,
            expected_state,
            f"Expected state {expected_state}, got {event.state}",
        )
        self.assertEqual(
            event.sw_if_index,
            sw_if_index,
            f"Expected sw_if_index {sw_if_index}, got {event.sw_if_index}",
        )

        # Convert IPv6 address for comparison
        event_addr = str(event.address)
        self.assertEqual(
            event_addr, address, f"Expected address {address}, got {event_addr}"
        )
        ...
        if expected_count is not None:
            self.assertEqual(
                event.dad_count,
                expected_count,
                f"Expected dad_count {expected_count}, got {event.dad_count}",
            )

        if expected_transmits is not None:
            self.assertEqual(
                event.dad_transmits,
                expected_transmits,
                f"Expected dad_transmits {expected_transmits}, got {event.dad_transmits}",
            )

    def test_dad_event_success_single_transmit(self):
        """DAD events: SUCCESS case with 1 transmit

        Expected events:
        1. TENTATIVE (count=0) - initial state
        2. TENTATIVE (count=1) - after first NS sent
        3. PREFERRED (count=1) - success
        """
        # Enable DAD with 1 transmission
        self.vapi.ip6_dad_enable_disable(enable=True, dad_transmits=1)
        ...
        # Event 1: Initial TENTATIVE (count=0)
        e1 = self.vapi.wait_for_event(2, "ip6_dad_event")
        self.verify_dad_event(
            e1,
            self.DS.IP6_DAD_STATE_TENTATIVE,
            self.pg0.sw_if_index,
            test_address,
            expected_count=0,
            expected_transmits=1,
        )

        # Event 2: TENTATIVE after first NS (count=1)
        e2 = self.vapi.wait_for_event(2, "ip6_dad_event")
        self.verify_dad_event(
            e2,
            self.DS.IP6_DAD_STATE_TENTATIVE,
            self.pg0.sw_if_index,
        ...
        # Event 3: PREFERRED (success)
        e3 = self.vapi.wait_for_event(2, "ip6_dad_event")
        self.verify_dad_event(
            e3,
            self.DS.IP6_DAD_STATE_PREFERRED,
            self.pg0.sw_if_index,
            test_address,
            expected_count=1,
            expected_transmits=1,
        ...
        # Verify address is still configured
        self.assert_address_present(self.pg0.sw_if_index, test_address)

    def test_dad_event_success_multiple_transmits(self):
        """DAD events: SUCCESS case with 3 transmits

        Expected events:
        1. TENTATIVE (count=0) - initial
        2. TENTATIVE (count=1) - after 1st NS
        3. TENTATIVE (count=2) - after 2nd NS
        4. TENTATIVE (count=3) - after 3rd NS
        5. PREFERRED (count=3) - success
        """
        # Enable DAD with 3 transmissions
        self.vapi.ip6_dad_enable_disable(
            enable=True, dad_transmits=3, dad_retransmit_delay=1.0
        )
        ...
        self.verify_dad_event(
            ...
            test_address,
            expected_count=0,
            expected_transmits=3,
        )

        # Event 2: TENTATIVE after 1st NS (count=1)
        e2 = self.vapi.wait_for_event(2, "ip6_dad_event")
        self.verify_dad_event(
            e2,
            self.DS.IP6_DAD_STATE_TENTATIVE,
        ...
        # Event 3: TENTATIVE after 2nd NS (count=2)
        e3 = self.vapi.wait_for_event(2, "ip6_dad_event")
        self.verify_dad_event(
            e3,
            self.DS.IP6_DAD_STATE_TENTATIVE,
        ...
        # Event 4: TENTATIVE after 3rd NS (count=3)
        e4 = self.vapi.wait_for_event(2, "ip6_dad_event")
        self.verify_dad_event(
            e4,
            self.DS.IP6_DAD_STATE_TENTATIVE,
            self.pg0.sw_if_index,
        ...
        # Event 5: PREFERRED (success)
        e5 = self.vapi.wait_for_event(2, "ip6_dad_event")
        self.verify_dad_event(
            e5,
            self.DS.IP6_DAD_STATE_PREFERRED,
            self.pg0.sw_if_index,
    ...
    def test_dad_event_duplicate_ip_remains_configured(self):
        """DAD events: DUPLICATE case - IP MUST remain configured

        THIS IS THE KEY CHANGE: IP is NOT removed on duplicate!

        Expected events:
        1. TENTATIVE (count=0) - initial
        2. TENTATIVE (count=1) - after first NS
        3. DUPLICATE (count=1) - conflict detected, IP REMAINS
        """
        # Enable DAD
        self.vapi.ip6_dad_enable_disable(enable=True, dad_transmits=1)
        ...
        # Event 3: DUPLICATE detected
        e3 = self.vapi.wait_for_event(2, "ip6_dad_event")
        self.verify_dad_event(
            e3,
            self.DS.IP6_DAD_STATE_DUPLICATE,
            self.pg0.sw_if_index,
            test_address,
            expected_count=1,
            expected_transmits=1,
        ...
        # CRITICAL TEST: Address MUST still be configured (NEW BEHAVIOR)
        # Old behavior would have removed it, new behavior keeps it
        self.assert_address_present(self.pg0.sw_if_index, test_address)

    def test_dad_event_multiple_addresses(self):
        """DAD events: Multiple addresses get independent event streams"""
        # Enable DAD
        self.vapi.ip6_dad_enable_disable(enable=True, dad_transmits=1)
        ...
        # Should have 6 events total (3 per address):
        # addr1: TENTATIVE(0), TENTATIVE(1), PREFERRED
        # addr2: TENTATIVE(0), TENTATIVE(1), PREFERRED
        self.assertGreaterEqual(
            len(events), 6, f"Expected at least 6 events, got {len(events)}"
        )

        # Count events per address
        addr1_events = [e for e in events if str(e.address) == addr1]
        addr2_events = [e for e in events if str(e.address) == addr2]

        self.assertGreaterEqual(len(addr1_events), 3, f"Expected 3 events for {addr1}")
        self.assertGreaterEqual(len(addr2_events), 3, f"Expected 3 events for {addr2}")

        # Both addresses should end up configured
        self.assert_address_present(self.pg0.sw_if_index, addr1)
        self.assert_address_present(self.pg0.sw_if_index, addr2)

    def test_dad_no_events_when_disabled(self):
        """DAD events: No events sent when DAD is disabled"""
        # Ensure DAD is disabled
        self.vapi.ip6_dad_enable_disable(enable=False)
        ...
        events = self.vapi.collect_events()
        self.assertEqual(
            len(events), 0, f"Expected no events when DAD disabled, got {len(events)}"
        )
