...
class TestIpSessionRedirect(VppTestCase):
    ...
    def setUpClass(cls):
        ...
        itfs = cls.create_pg_interfaces(range(2))
        for itf in itfs:
            itf.admin_up()
            itf.config_ip4()
            itf.resolve_arp()
    ...
    def punt_redirect(self, sport, dport, is_ip6):
        ...
        # create table and add a session redirect rule but not matching the
        # stream: expect to drop
        table_index = self.create_table(is_ip6)
        paths = [VppRoutePath(nh, 0xffffffff).encode()]
        match1 = self.build_match(src, sport + 10, is_ip6)
        self.vapi.ip_session_redirect_add(0, table_index, len(match1), match1,
        ...
        self.vapi.ip_session_redirect_add(0, table_index, len(match2), match2,
                                          is_punt=True, n_paths=1, paths=paths)
