...
class TestIpSessionRedirect(VppTestCase):
    ...
    def punt_redirect(self, sport, dport, is_ip6):
        ...
        r = self.vapi.ip_session_redirect_add(
            0xffffffff,
            table_index,
            len(match1),
            match1,
        ...
        self.vapi.ip_session_redirect_add(
            0xffffffff,
            table_index,
            len(match2),
            match2,
        ...
        # delete by match
        self.vapi.ip_session_redirect_del(table_index=table_index,
                                          match_len=len(match2), match=match2)

        # update the non-matching entry with index so it matches and redirects
        # to pg1
        paths = [VppRoutePath(nh1, 0xffffffff).encode()]
        self.vapi.ip_session_redirect_add(
            r.session_index,
        ...
        # delete by index
        self.vapi.ip_session_redirect_del(session_index=r.session_index)
