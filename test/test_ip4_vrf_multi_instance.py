...
class TestIp4VrfMultiInst(VppTestCase):
    ...
    def create_stream_negative(self, src_if, vrf_id, packet_sizes):
        """
    ...
    def run_verify_test(self):
        ...
        :raise RuntimeError: If no packet captured on pg-ip4 interface assigned
            to VRF or if any packet is captured on pg-ip4 interface not assigned
            to VRF.
        """
        # Test
    ...
    def run_negative_test(self):
        ...
        for vrf_id in self.vrf_list:
            for pg_if in self.pg_if_by_vrf_id[vrf_id]:
                pkts = self.create_stream_negative(pg_if, vrf_id,
