...
class TestSasc(VppTestCase):
    ...
    def _configure_sasc(self, add_packet_stats=False):
        ...
        if add_packet_stats:
            self.sasc_expected_fwd_rvs_service_chain = [
    ...
    def test_sasc(self):
        ...
        self.verify_service_chain(
            service_chains[0],
            0,
            len(self.sasc_expected_miss_service_chain),
            self.sasc_expected_miss_service_chain,
        )
        self.verify_service_chain(
            service_chains[1],
            1,
            len(self.sasc_expected_fwd_rvs_service_chain),
            self.sasc_expected_fwd_rvs_service_chain,
        )
    ...
    def test_sasc_packet_stats_dump(self):
        ...
        sessions = self.vapi.sasc_session_dump()
        self.assertGreaterEqual(
            len(sessions), 1, "Expected at least 1 session to be created"
        )
        ...
        for session in sessions:
            if (
                session.forward_key.sport == test_flow["sport"]
                and session.forward_key.dport == test_flow["dport"]
            ):
        ...
        self.assertEqual(
            len(packet_stats), 1, "Expected one packet stats entry for session"
        )

        stats = packet_stats[0]
        self.assertEqual(
            stats.session_index, session_index, "Session index should match"
        )
        ...
        self.assertGreaterEqual(
            len(all_packet_stats), 1, "Should have at least 1 packet stats entry"
        )
