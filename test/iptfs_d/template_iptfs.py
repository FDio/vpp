...
import socket
import struct
import sys  # for sys.stderr - debugging
import psutil  # number of cores
...
class _TemplateIPTFS4(TemplateIPTFS):
    ...
    # attribute read by CPUInterface.get_vpp_worker_count()
    vpp_worker_count = num_workers
    ...
    @classmethod
    def setUpClass(cls):
        cls.logger.info("%s: setUpClass: calling super (logger)", "_TemplateIPTFS4")
        physical = psutil.cpu_count(logical=False)
        logical = psutil.cpu_count(logical=True)
        print(
            "_TemplateIPTFS4: cores: {}p, {}l".format(physical, logical),
            file=sys.stderr,
        )
        print("_TemplateIPTFS4: setUpClass: calling super (stderr)", file=sys.stderr)
        super(_TemplateIPTFS4, cls).setUpClass()
        cls.logger.info("%s: setUpClass: super completed (logger)", "_TemplateIPTFS4")
        print("_TemplateIPTFS4: setUpClass: super completed (stderr)", file=sys.stderr)
    ...
    def setUpConstants(cls):
        ...
        cls.logger.info("OLD vpp_cmdline args: %s", cls.vpp_cmdline)
        i = cls.vpp_cmdline.index("nodaemon")
        cls.vpp_cmdline.insert(i + 1, "nosyslog")

        #        # 260914 experiment: remove "corelist-workers", "a,b,c"
        #        # and use only "workers"
        #        try:
        #            i = cls.vpp_cmdline.index("corelist-workers")
        #            cls.vpp_cmdline.pop(i)
        #            cls.vpp_cmdline.pop(i)
        #        except:
        #            pass
        #        # 260911 ? superseded by vpp_worker_count attribute?
        ...
        # 260915 experiment: is this killing vpp startup?
