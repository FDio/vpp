...
func QuicCryptoContextTest(s *NoTopoSuite) {
	...
	// add third listener with tls profile which should use new quic crypto context
	o = vpp.Vppctl("app crypto add tls-profile app test_tls_server " +
		"ciphersuites TLS_AES_256_GCM_SHA384")
	Log(o)
	AssertNotContains(o, "error")
	AssertContains(o, "profile 0")
	serverAddress3 := s.Interfaces.Tap.Ip4AddressString() + ":" + s.Ports.NginxServerSsl
	Log(vpp.Vppctl("test tls server alpn-proto1 3 profile-index 0 uri quic://" + serverAddress3))
	o = vpp.Vppctl("show quic crypto context")
	Log(o)
	AssertContains(o, "[0][test_tls_server n_sub: 2, ckpair: 1]")
	AssertContains(o, "[1][test_tls_server n_sub: 1, ckpair: 2][tls_profile: 0]")

	// start anonther app which should use it own quic crypto context
	serverAddress4 := s.Interfaces.Tap.Ip4AddressString() + ":" + s.Ports.NginxHttp3
	Log(vpp.Vppctl("vperf server fifo-size 8k uri quic://" + serverAddress4))
	o = vpp.Vppctl("show quic crypto context")
	Log(o)
	AssertContains(o, "[2][vperf_server n_sub: 1")

	// remove app to test cleanup
	Log(vpp.Vppctl("vperf server stop"))
	o = vpp.Vppctl("show quic crypto context")
	Log(o)
	AssertNotContains(o, "[2][vperf_server n_sub: 1")
	AssertContains(o, "[0][test_tls_server n_sub: 2, ckpair: 1]")
	AssertContains(o, "[1][test_tls_server n_sub: 1, ckpair: 2][tls_profile: 0]")
