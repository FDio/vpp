...
func TcpFastRecoveryTwoHolesPartialAckTest(s *TcpHarnessSuite) {
	...
	RunTcpHarnessScenarioOnState(s, state,
		...
		EnableServerNFQueueScript(TcpHarnessNFQueueScriptConfig{
			DropDataPacketIndices:      dropDataPacketIndices,
			InitialHoleCount:           2,
			InitialInjectCount:         3,
			PartialInjectCount:         2,
			SendPartialAckOnRetransmit: true,
			ReleaseAckGateAfterPartial: true,
		}),
		StartClientSend(controlledBytes, &sendHandle),
		WaitServerNFQueueScriptStage(10*time.Second, TcpHarnessNFQueueScriptStageDone),
...
func TcpSackScoreboardRobustnessTest(s *TcpHarnessSuite) {
	...
	RunTcpHarnessScenarioOnState(s, state,
		...
		EnableServerNFQueueScript(TcpHarnessNFQueueScriptConfig{
			DropDataPacketIndices:      dropDataPacketIndices,
			InitialHoleCount:           len(dropDataPacketIndices),
			InitialInjectCount:         3,
			ReleaseAckGateAfterInitial: true,
		}),
		StartClientSend(controlledBytes, &sendHandle),
		WaitServerNFQueueScriptStage(10*time.Second, TcpHarnessNFQueueScriptStageDone),
		TcpHarnessActionFunc(func(s *TcpHarnessSuite, st *TcpHarnessScenarioState) {
