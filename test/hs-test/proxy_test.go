	...
	go func() {
		...
		if len(response) > 0 {
			if err := conn.SetWriteDeadline(time.Now().Add(timeout)); err != nil {
				done <- fmt.Errorf("set write deadline failed: %w", err)
				return
			}

			if _, err := conn.Write(response); err != nil {
				done <- fmt.Errorf("server write response failed: %w", err)
				return
			}
		}
...
func runFinDrainTcpClient(
	...
	responseLen := strconv.Itoa(len(expectedResponse))
...
reply = bytearray()
if response_len > 0:
    while len(reply) < response_len:
        chunk = s.recv(response_len - len(reply))
        if not chunk:
            break
        reply.extend(chunk)
...
`
	...
	cmd := exec.Command(
		"ip", "netns", "exec", netns, "python3", "-c", python,
		host, port, timeoutSeconds, responseLen,
	)
	...
	if !bytes.Equal(output, expectedResponse) {
		return fmt.Errorf(
			"client response mismatch: got %q, expected %q",
			string(output), string(expectedResponse),
		)
	}
...
func vppConnectProxyClientFinDrainTcp(s *MasqueSuite, isHttp3 bool) {
	...
	clientExpectedResponse := serverResponse

	if isHttp3 {
		// H3 covers client->target FIN drain only; reverse drain is separate.
		serverResponse = nil
		clientExpectedResponse = nil
	}
	...
	serverDone, serverReady := runFinDrainTcpServer(
		targetAddr,
		payload,
		serverResponse,
		timeout,
	...
	AssertNil(runFinDrainTcpClient(
	...
	select {
	case err := <-serverDone:
		AssertNil(err, "server failed during FIN-drain exchange")
	case <-time.After(timeout):
		AssertFail("timed out waiting for TCP server to complete FIN-drain exchange")
	}
