...
func testXEchoVclClient(s *VethsSuite, proto string) {
	...
	serverVpp.Vppctl("test vperf server uri %s://%s/%s fifo-size 64k", proto, s.Interfaces.Server.Ip4AddressString(), s.Ports.Port1)
...
func testXEchoVclServer(s *VethsSuite, proto string) {
	...
	o := clientVpp.Vppctl("test vperf client uri %s://%s/%s fifo-size 64k verbose bytes 2m", proto, serverVethAddress, s.Ports.Port1)
	cancel()
	wg.Wait()
	Log(o)
