...
func init() {
	...
	RegisterHsiMWTests(HsiProxyLiteOffloadMWTest, HsiProxyLiteRepeatedOffloadMWTest,
		...
		HsiProxyLitePeerSetupFailureTest, HsiProxyLiteUdpPeerSetupFailureTest)
}
...
func HsiProxyLitePeerSetupFailureTest(s *HsiSuite) {
	s.CpusPerVppContainer = 3
	s.SetupTest()
	s.SetupNginxServer()
	vpp := s.Containers.Vpp.VppInstance
	Log(vpp.Vppctl("test hsi peer-rpc-fail tcp commit once"))
	s.StartProxyLiteTcp4("hsi-offload")

	hsi := vpp.Vppctl("show hsi")
	baseFailures := HsiCounterValue(hsi, "tcp-peer-rpc-failed")
	baseCleanups := HsiCounterValue(hsi, "tcp-cleanup-completed")
	for i := 0; i < 32; i++ {
		uri := fmt.Sprintf("http://%s:%d/64B?peer-fail=%d", s.ServerAddr(),
			s.Ports.Server, i)
		runCurlIgnoreResult(uri, s.NetNamespaces.Client, 5)

		hsi = vpp.Vppctl("show hsi")
		if HsiCounterValue(hsi, "tcp-peer-rpc-failed") > baseFailures {
			Log(hsi)
			break
		}
	}

	hsi = WaitHsiCounterAtLeast(vpp, "tcp-peer-rpc-failed", baseFailures+1)
	Log(hsi)
	hsi = AssertHsiTcpCleaned(vpp, baseCleanups+2)
	Log(hsi)
}

func HsiProxyLiteUdpPeerSetupFailureTest(s *HsiSuite) {
	s.CpusPerVppContainer = 3
	s.SetupTest()
	remoteServerConn := StartUdpEchoServer(s.ServerAddr(), int(s.Ports.Server))
	defer remoteServerConn.Close()
	...
	vpp := s.Containers.Vpp.VppInstance
	Log(vpp.Vppctl("hsi udp idle-timeout 1"))
	Log(vpp.Vppctl("test hsi peer-rpc-fail udp peer-update once"))
	s.StartProxyLiteUdp4("hsi-offload", "fifo-size 4k")

	for i := 0; i < 16; i++ {
		second := fmt.Sprintf("hsi-udp-peer-fail-second-%d", i)
		finished := StartHsiUdpEchoClient(s.ServerAddr(), s.Ports.Server, s.NetNamespaces.Client,
			100*time.Millisecond, fmt.Sprintf("hsi-udp-peer-fail-warm-%d", i),
			fmt.Sprintf("hsi-udp-peer-fail-first-%d", i), second)
		WaitProxyLiteTrackedCount(vpp, i+1, func() {})
		result := <-finished
		Log(result.Output)
		if result.Err != nil {
			Log("udp peer failure flow exited with expected/allowed error: %v", result.Err)
		} else {
			AssertContains(result.Output, second)
		}

		hsi := vpp.Vppctl("show hsi")
		if HsiCounterValue(hsi, "udp-peer-rpc-failed") > 0 {
			Log(hsi)
			break
		}
	}

	hsi := WaitHsiCounterAtLeast(vpp, "udp-peer-rpc-failed", 1)
	Log(hsi)
	hsi = AssertHsiUdpCleaned(vpp, 2)
	Log(hsi)
}
