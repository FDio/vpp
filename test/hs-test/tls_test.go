...
func TlsClientSessionResumeTest(s *TlsSuite) {
	...
	// Attach the client app (stays alive for multiple connections)
	o := clientVpp.Vppctl("test tls client action attach")
	Log(o)
	AssertNotContains(o, "attach failed")

	// Enable TLS resumption for the client app
	o = clientVpp.Vppctl("app crypto tls-resumption app test_tls_client enabled")
	Log(o)
	AssertContains(o, "TLS resumption enabled")

	// Configure per-app session cache
	o = clientVpp.Vppctl("tls openssl session-cache app test_tls_client max-entries 128")
	Log(o)
	AssertContains(o, "Session cache enabled")
	...
	o = clientVpp.Vppctl("test tls client action connect echo uri " + uri)
	Log(o)
	AssertNotContains(o, "connect failed")
	AssertNotContains(o, "timeout")
	AssertNotContains(o, "not attached")
	...
	// Resize the session cache down
	o = clientVpp.Vppctl("tls openssl session-cache app test_tls_client max-entries 64")
	Log(o)
	AssertContains(o, "max-entries 128 -> 64")

	// Verify max-entries 0 is rejected
	o = clientVpp.Vppctl("tls openssl session-cache app test_tls_client max-entries 0")
	Log(o)
	AssertContains(o, "max-entries must be > 0")

	// Disable TLS resumption (frees all engine caches)
	o = clientVpp.Vppctl("app crypto tls-resumption app test_tls_client disabled")
	Log(o)
	AssertContains(o, "TLS resumption disabled")

	// Verify session-cache rejected when resumption is disabled
	o = clientVpp.Vppctl("tls openssl session-cache app test_tls_client max-entries 32")
	Log(o)
	AssertContains(o, "TLS resumption not enabled")

	// Detach the client app
	o = clientVpp.Vppctl("test tls client action detach")
	Log(o)
	AssertNotContains(o, "detach failed")

	// Verify session-cache rejected when app is detached
	o = clientVpp.Vppctl("tls openssl session-cache app test_tls_client max-entries 32")
	Log(o)
	AssertContains(o, "unknown input")
