...
func QuicTlsProfileNegotiatedParamsTest(s *VethsSuite) {
	...
	if !foundCipher {
		AssertNil(fmt.Errorf("Expected TLS 1.3 cipher suite in QUIC output, got: %s", o))
	}
	...
	if !foundGroup {
		AssertNil(fmt.Errorf("Expected a known key agreement group in QUIC output, got: %s", o))
	}
