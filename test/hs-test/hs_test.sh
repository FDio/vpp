...
reserve_cpus() {
    local id cpus kept pool core fair keep surplus n_live want held_now
    ...
    (
        ...
        # Trim every live holder to the fair share; their surplus joins the pool.
        kept=""
        pool=""
        while IFS="	" read -r id cpus mode; do
            [ -z "$id" ] && continue
            ...
            surplus=$(echo "$cpus" | tr ',' '\n' | tail -n +$((fair + 1)) | paste -sd,)
            kept="$kept$id	$keep	${mode:-plain}
"
            [ -n "$surplus" ] && pool="$pool$surplus,"
        done < "$CPU_RESERVATIONS"
