...
func (s *TcpHarnessSuite) LogTcpTestPeerLog(c *Container) {
	out, err := c.Exec(false, "cat %s", s.TcpTestPeer.LogPath)
	if err != nil {
		Log("failed to read tcp_test_peer log %s: %v", s.TcpTestPeer.LogPath, err)
		return
	}

	out = strings.TrimSpace(out)
	if out == "" {
		Log("tcp_test_peer log is empty: %s", s.TcpTestPeer.LogPath)
		return
	}

	Log("tcp_test_peer log (%s):\n%s", s.TcpTestPeer.LogPath, out)
}
