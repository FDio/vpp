...
type TcpTestPeerCommandResult struct {
	Out string
	Err error
}
...
func (s *TcpHarnessSuite) startTcpTestPeerServerProcess(c *Container, cfg TcpTestPeerServerConfig) {
	if cfg.ControlSock == "" {
...
func (s *TcpHarnessSuite) startTcpTestPeerClientProcess(c *Container, cfg TcpTestPeerClientConfig) {
	if cfg.ControlSock == "" {
...
func (s *TcpHarnessSuite) StartTcpTestPeerServer(cfg TcpTestPeerServerConfig) {
	if cfg.ListenAddr == "" {
	...
	s.startTcpTestPeerServerProcess(s.Containers.ServerApp, cfg)
	s.WaitForTcpTestPeerServerStats(5*time.Second,
		func(stats TcpTestPeerStats) bool {
			return !stats.Accepted && stats.Paused == cfg.PauseRead
		})
}

func (s *TcpHarnessSuite) StartTcpTestPeerClient(cfg TcpTestPeerClientConfig) {
	if cfg.ConnectAddr == "" {
	...
	s.startTcpTestPeerClientProcess(s.Containers.ClientApp, cfg)
	s.WaitForTcpTestPeerClientStats(5*time.Second,
...
func (s *TcpHarnessSuite) TcpTestPeerServerCtl(command string) string {
	return s.TcpTestPeerCtl(s.Containers.ServerApp, s.TcpTestPeer.ControlSock, command)
}

func (s *TcpHarnessSuite) TcpTestPeerServerCtlTry(command string) (string, bool) {
	return s.TcpTestPeerCtlTry(s.Containers.ServerApp, s.TcpTestPeer.ControlSock, command)
}

func (s *TcpHarnessSuite) TcpTestPeerClientCtl(command string) string {
	return s.TcpTestPeerCtl(s.Containers.ClientApp, s.TcpTestPeer.ClientControlSock, command)
}

func (s *TcpHarnessSuite) TcpTestPeerClientCtlTry(command string) (string, bool) {
	return s.TcpTestPeerCtlTry(s.Containers.ClientApp, s.TcpTestPeer.ClientControlSock, command)
}

func (s *TcpHarnessSuite) StartTcpTestPeerClientSend(bytes uint64) <-chan TcpTestPeerCommandResult {
	done := make(chan TcpTestPeerCommandResult, 1)

	go func() {
		o, err := s.Containers.ClientApp.Exec(false, "tcp_test_peer ctl --control %s send %d",
			s.TcpTestPeer.ClientControlSock, bytes)
		done <- TcpTestPeerCommandResult{Out: o, Err: err}
	}()

	return done
}

func (s *TcpHarnessSuite) CloseTcpTestPeerClient() {
	s.TcpTestPeerClientCtl("close")
}
...
func (s *TcpHarnessSuite) LogTcpTestPeerLogs() {
	s.LogTcpTestPeerClientLog(s.Containers.ClientApp)
	s.LogTcpTestPeerLog(s.Containers.ServerApp)
}
...
func (s *TcpHarnessSuite) TcpTestPeerServerStatsGet() TcpTestPeerStats {
	return s.TcpTestPeerStatsGet(s.Containers.ServerApp, s.TcpTestPeer.ControlSock)
}

func (s *TcpHarnessSuite) TcpTestPeerClientStatsGet() TcpTestPeerStats {
	return s.TcpTestPeerStatsGet(s.Containers.ClientApp, s.TcpTestPeer.ClientControlSock)
}
...
func (s *TcpHarnessSuite) WaitForTcpTestPeerServerStats(timeout time.Duration,
	check func(stats TcpTestPeerStats) bool) TcpTestPeerStats {
	return s.WaitForTcpTestPeerStats(s.Containers.ServerApp, s.TcpTestPeer.ControlSock, timeout, check)
}

func (s *TcpHarnessSuite) WaitForTcpTestPeerClientStats(timeout time.Duration,
	check func(stats TcpTestPeerStats) bool) TcpTestPeerStats {
	return s.WaitForTcpTestPeerStats(s.Containers.ClientApp, s.TcpTestPeer.ClientControlSock, timeout, check)
}

func (s *TcpHarnessSuite) StopTcpTestPeerClient() {
	if out, ok := s.TcpTestPeerClientCtlTry("shutdown"); ok {
		if out != "" {
			Log("tcp_test_peer client shutdown: %s", out)
		}
	} else {
		Log("tcp_test_peer client shutdown skipped: control socket is gone")
	}
}

func (s *TcpHarnessSuite) StopTcpTestPeerServer() {
	if out, ok := s.TcpTestPeerServerCtlTry("shutdown"); ok {
		if out != "" {
			Log("tcp_test_peer server shutdown: %s", out)
		}
	} else {
		Log("tcp_test_peer server shutdown skipped: control socket is gone")
	}
}

func (s *TcpHarnessSuite) StopTcpTestPeers() {
	s.StopTcpTestPeerClient()
	s.StopTcpTestPeerServer()
}
