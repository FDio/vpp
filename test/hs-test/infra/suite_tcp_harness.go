...
func EnableServerAckGate() TcpHarnessAction {
	return TcpHarnessActionFunc(func(s *TcpHarnessSuite, st *TcpHarnessScenarioState) {
		s.EnableServerAckGate()
	})
}

func DisableServerAckGate() TcpHarnessAction {
	return TcpHarnessActionFunc(func(s *TcpHarnessSuite, st *TcpHarnessScenarioState) {
		s.DisableServerAckGate()
	})
}

func WaitServerAckGateDrops(timeout time.Duration, count uint32) TcpHarnessAction {
	return TcpHarnessActionFunc(func(s *TcpHarnessSuite, st *TcpHarnessScenarioState) {
		s.WaitForServerAckGateDrops(timeout, count)
	})
}
...
func (h *tcpHarnessNFQueueHelper) handlePacket(a nfqueue.Attribute) int {
	...
	if a.Payload != nil {
		if packet, ok := parsePcapIPv4TCPPacket(pcapLinkTypeRaw, *a.Payload); ok && h.matchPacket(packet) {
			...
			switch {
			case h.cfg.DropAckOnlyPackets && packet.IsAckOnly() && packet.HasTSOpt:
				drop = true
...
func (h *tcpHarnessNFQueueHelper) snapshotSegments() ([]TcpHarnessDataSegment, error) {
	h.mu.Lock()
	defer h.mu.Unlock()

	segments := make([]TcpHarnessDataSegment, 0, len(h.dataSegments))
	for seq, end := range h.dataSegments {
...
func (s *TcpHarnessSuite) EnableServerAckGate() {
	s.DisableServerAckGate()

	cfg := s.defaultServerAckGateConfig(TcpHarnessNFQueueConfig{})
	helper, err := newTcpHarnessNFQueueHelper(cfg)
	AssertNil(err)

	s.NFQueue.ServerAckGate = helper
...
func (s *TcpHarnessSuite) WaitForServerAckGateDrops(timeout time.Duration, count uint32) {
	AssertNotNil(s.NFQueue.ServerAckGate)
	...
	if waitWithTimeout(timeout, 50*time.Millisecond, func() bool {
		drops, seqs, err = s.NFQueue.ServerAckGate.currentState()
		AssertNil(err)
		return drops >= count
	}) {
		return
	...
	Log("last ACK-gate NFQUEUE state: drops=%d dropped_seqs=%v", drops, seqs)
	AssertEqual(true, false, "timed out waiting for ACK-gate NFQUEUE to drop %d packets", count)
}

func (s *TcpHarnessSuite) ServerNFQueueSegments() []TcpHarnessDataSegment {
	AssertNotNil(s.NFQueue.Server)
	segments, err := s.NFQueue.Server.snapshotSegments()
	AssertNil(err)
	return segments
}

func (s *TcpHarnessSuite) ServerAckGateState() TcpHarnessNFQueueAckState {
	AssertNotNil(s.NFQueue.ServerAckGate)
	state, err := s.NFQueue.ServerAckGate.snapshotAckState()
	AssertNil(err)
	return state
}

func (s *TcpHarnessSuite) InjectServerAck(ack uint32, window uint16,
	sackBlocks ...TcpTestEndpointSackBlock) {
	AssertNotNil(s.NFQueue.ServerAckGate)
	AssertNil(s.NFQueue.ServerAckGate.injectAck(ack, window, sackBlocks...))
}
