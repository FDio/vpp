...
func TcpTestEndpointCtlInjectAck(seq, ack uint32, window uint16,
	sackBlocks ...TcpTestEndpointSackBlock) TcpTestEndpointCtl {
	var b strings.Builder

	fmt.Fprintf(&b, "inject-ack %d %d %d", seq, ack, window)
	for _, sack := range sackBlocks {
		fmt.Fprintf(&b, " %d %d", sack.Left, sack.Right)
	}

	return TcpTestEndpointCtl(b.String())
}
...
func (s *TcpHarnessSuite) WaitForClientVppSessions(timeout time.Duration,
	...
	Log("last client show session verbose 2 while waiting for session condition:\n%s", output)
	AssertEqual(true, false, "timed out waiting for client VPP session condition")
	return output
}
...
func (cfg TcpHarnessNFQueueConfig) nftScript() string {
	...
	chain input {
		...
		iifname "%s" ip saddr %s ip daddr %s tcp dport %d queue num %d bypass
	}
}
`, cfg.table(), cfg.InputIf, cfg.SrcIP, cfg.DstIP, cfg.DstPort, cfg.QueueNum)
}
...
func (h *tcpHarnessNFQueueHelper) matchPacket(packet PcapIPv4TCPPacket) bool {
	return packet.SrcIP.String() == h.cfg.SrcIP &&
		packet.DstIP.String() == h.cfg.DstIP &&
		packet.DstPort == h.cfg.DstPort
}
...
func (h *tcpHarnessNFQueueHelper) handlePacket(a nfqueue.Attribute) int {
	...
	if a.Payload != nil {
		if packet, ok := parsePcapIPv4TCPPacket(pcapLinkTypeRaw, *a.Payload); ok &&
			h.matchPacket(packet) &&
			packet.PayloadLen > 0 {
			var drop bool
