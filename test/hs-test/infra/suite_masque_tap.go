...
var masqueTapTests = map[string][]func(s *MasqueTapSuite){}
var masqueTapSoloTests = map[string][]func(s *MasqueTapSuite){}
var masqueTapMWTests = map[string][]func(s *MasqueTapSuite){}

func RegisterMasqueTapTests(tests ...func(s *MasqueTapSuite)) {
	masqueTapTests[GetTestFilename()] = tests
}

func RegisterMasqueTapSoloTests(tests ...func(s *MasqueTapSuite)) {
	masqueTapSoloTests[GetTestFilename()] = tests
}

func RegisterMasqueTapMWTests(tests ...func(s *MasqueTapSuite)) {
	masqueTapMWTests[GetTestFilename()] = tests
}

func (s *MasqueTapSuite) SetupSuite() {
	s.HstSuite.SetupSuite()
	s.ConfigureNetworkTopology("masque_tap")
...
func (s *MasqueTapSuite) SetupTest(serverExtraArgs ...string) {
	s.HstSuite.SetupTest()

	var memoryConfig Stanza
	memoryConfig.NewStanza("memory").Append("main-heap-size 2G")

	cmd := exec.Command("ip", "netns", "add", s.NetNamespaces.Client)
	Log(cmd.String())
	...
	AssertNil(err, fmt.Sprint(err))
	err = linkSetNetns(s.Interfaces.Client.Host.name, s.NetNamespaces.Client)
	AssertNil(err, fmt.Sprint(err))
	err = addAddress(s.Interfaces.Client.Host.name, s.Interfaces.Client.Host.Ip4Address, s.NetNamespaces.Client)
...
func (s *MasqueTapSuite) TeardownTest() {
	...
	cmd = exec.Command("ip", "netns", "del", s.NetNamespaces.Client)
	Log(cmd.String())
	o, err = cmd.CombinedOutput()
	Log(string(o))
	AssertNil(err, fmt.Sprint(err))
...
func (s *MasqueTapSuite) ProxyClientConnect(proto, port string, extraArgs ...string) {
	extras := ""
	if len(extraArgs) > 0 {
...
func (s *MasqueTapSuite) StartNginxServer() {
...
func (s *MasqueTapSuite) NginxAddr() string {
	return s.Interfaces.Server.Host.Ip4AddressString()
}

func (s *MasqueTapSuite) ProxyAddr() string {
	return s.Interfaces.TunnelServer.Ip4AddressString()
}

var _ = Describe("MasqueTapSuite", Ordered, ContinueOnFailure, Label("Masque", "Proxy", "ConnectProxy"), func() {
	var s MasqueTapSuite
	BeforeAll(func() {
		s.SetupSuite()
	})
	BeforeEach(func() {
	...
	for filename, tests := range masqueTapTests {
...
var _ = Describe("MasqueTapSoloSuite", Ordered, ContinueOnFailure, Serial, Label("Masque", "Proxy", "ConnectProxy"), func() {
	var s MasqueTapSuite
	BeforeAll(func() {
		s.SetupSuite()
	})
	...
	for filename, tests := range masqueTapSoloTests {
		for _, test := range tests {
			test := test
			pc := reflect.ValueOf(test).Pointer()
			funcValue := runtime.FuncForPC(pc)
...
var _ = Describe("MasqueTapMWSuite", Ordered, ContinueOnFailure, Serial, Label("Masque", "Proxy", "ConnectProxy", "MW"), func() {
	var s MasqueTapSuite
	BeforeAll(func() {
		s.SetupSuite()
	})
	...
	for filename, tests := range masqueTapMWTests {
		for _, test := range tests {
			test := test
			pc := reflect.ValueOf(test).Pointer()
			funcValue := runtime.FuncForPC(pc)
