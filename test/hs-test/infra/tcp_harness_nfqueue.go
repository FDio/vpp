...
const tcpHarnessNFQueueAckQueueCapacity = 128
...
type tcpHarnessNFQueueController struct {
	...
	mu      sync.Mutex
	script  *tcpHarnessNFQueueScriptState
	lastErr error
}

type tcpHarnessQueuedAck struct {
	raw    []byte
...
type tcpHarnessNFQueueHelper struct {
	...
	queuedAcks  []tcpHarnessQueuedAck
	lastErr     error
}
...
func newTcpHarnessNFQueueController(cfg TcpHarnessNFQueueScriptConfig,
	...
	controller := &tcpHarnessNFQueueController{
		script: newTcpHarnessNFQueueScriptState(cfg),
	}
...
func (c *tcpHarnessNFQueueController) currentStats() (TcpHarnessNFQueueScriptStats, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.script.Stats(), c.lastErr
}

func (c *tcpHarnessNFQueueController) currentTrace() ([]TcpHarnessNFQueueScriptTraceEntry, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *tcpHarnessNFQueueController) recordNaturalAckQueued(packet PcapIPv4TCPPacket) {
	c.mu.Lock()
	c.script.RecordNaturalAckQueued(packet)
	c.mu.Unlock()
}

func (c *tcpHarnessNFQueueController) recordSyntheticAckInjected(spec tcpHarnessNFQueueInjection) {
	c.mu.Lock()
	c.script.RecordSyntheticAckInjected(spec)
	c.mu.Unlock()
}

func (c *tcpHarnessNFQueueController) recordQueuedAckReplayed(packet PcapIPv4TCPPacket) {
	c.mu.Lock()
	c.script.RecordQueuedAckReplayed(packet)
	c.mu.Unlock()
}

func (c *tcpHarnessNFQueueController) recordQueuedAckDiscarded(packet PcapIPv4TCPPacket) {
	c.mu.Lock()
	c.script.RecordQueuedAckDiscarded(packet)
	c.mu.Unlock()
}

func (c *tcpHarnessNFQueueController) stopIngressDrops() {
	if c.ingress != nil {
		c.ingress.stopDrops()
	}
}

func (c *tcpHarnessNFQueueController) applyQueueDisposition(
	disposition TcpHarnessNFQueueAckQueueDisposition) error {
	if c.ackGate == nil || disposition == tcpHarnessNFQueueAckQueueDispositionNone ||
		disposition == TcpHarnessNFQueueAckQueueDispositionKeepQueued {
		return nil
	}
...
func (c *tcpHarnessNFQueueController) onPacket(role tcpHarnessNFQueueRole, packet PcapIPv4TCPPacket,
	originalDrop bool, retransmitSeen bool) tcpHarnessNFQueueScriptAction {
	c.mu.Lock()
	defer c.mu.Unlock()

	return c.script.ApplyEvent(tcpHarnessNFQueueScriptEvent{
		Role:           role,
		Packet:         packet,
...
func (h *tcpHarnessNFQueueHelper) stopDrops() {
	h.mu.Lock()
	h.dropsStopped = true
...
func (h *tcpHarnessNFQueueHelper) queueNaturalAck(raw []byte, packet PcapIPv4TCPPacket) error {
	h.mu.Lock()
	defer h.mu.Unlock()

	if len(h.queuedAcks) >= tcpHarnessNFQueueAckQueueCapacity {
		return fmt.Errorf("queued ACK overflow: %d", len(h.queuedAcks))
	}

	h.queuedAcks = append(h.queuedAcks, tcpHarnessQueuedAck{
...
func (h *tcpHarnessNFQueueHelper) replayQueuedAcks(controller *tcpHarnessNFQueueController) error {
	h.mu.Lock()
	queued := append([]tcpHarnessQueuedAck(nil), h.queuedAcks...)
	h.queuedAcks = nil
	h.mu.Unlock()

	for _, queuedAck := range queued {
		if err := tcpHarnessSendIPv4RawPacket(queuedAck.raw, queuedAck.packet.DstIP); err != nil {
			return err
		}
...
func (h *tcpHarnessNFQueueHelper) discardQueuedAcks(controller *tcpHarnessNFQueueController) {
	h.mu.Lock()
	queued := append([]tcpHarnessQueuedAck(nil), h.queuedAcks...)
	h.queuedAcks = nil
	h.mu.Unlock()
...
func (h *tcpHarnessNFQueueHelper) handlePacket(a nfqueue.Attribute) int {
	...
	var (
		controllerActs   []tcpHarnessNFQueueInjection
