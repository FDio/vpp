...
// needs cores before its first one. Rows of runs that are gone are dropped by
// whichever run notices first, so a killed run does not hold cores for ever.
//
	...
	err := withReservationLock(func() error {
		...
		if *NumaPerProcess {
			...
			for _, other := range others {
				return fmt.Errorf("MW parallelism needs the whole machine, but run %s holds CPUs %s",
		...
		for _, other := range others {
			if other.mode == reservationModeMW {
				return fmt.Errorf("run %s is using the whole machine with MW parallelism; wait for it to finish",
					other.runIdentity)
			}
		}
		...
		if len(reserved) == 0 {
			return fmt.Errorf("no CPUs are free for this run; %d other run(s) hold all %d of them",
...
// releaseCpuReservation drops this run's row. A run that dies without getting here
// is cleaned up by the next run that reads the table.
func (s *HstSuite) releaseCpuReservation() {
	if RunIdentity == "" {
		return
	}
	err := withReservationLock(func() error {
		...
		var remaining []cpuReservation
		for _, r := range all {
			if r.runIdentity != RunIdentity {
				remaining = append(remaining, r)
			}
		}
		return writeReservations(remaining)
	})
	if err != nil {
		Log("could not release CPU reservation: %v", err)
	}
}
