...
func newTcpHarnessNFQueueHelperWithRole(cfg NFQueueConfig,
	role tcpHarnessNFQueueRole, controller *NFQueueController, logf Logf) (*NFQueueHelper, error) {
	h := &NFQueueHelper{
		cfg:                   cfg,
		tableName:             cfg.table(),
...
func NewNFQueueHelper(cfg NFQueueConfig, logf Logf) (*NFQueueHelper, error) {
	return newTcpHarnessNFQueueHelperWithRole(cfg, tcpHarnessNFQueueRoleIngress, nil, logf)
}
...
func NewNFQueueController(cfg NFQueueScriptConfig,
	...
	ingress, err := newTcpHarnessNFQueueHelperWithRole(ingressCfg, tcpHarnessNFQueueRoleIngress, controller, logf)
	if err != nil {
		return nil, err
	}
	controller.ingress = ingress

	ackGate, err := newTcpHarnessNFQueueHelperWithRole(ackGateCfg, tcpHarnessNFQueueRoleEgress, controller, logf)
	if err != nil {
		ingress.Close()
		return nil, err
	}
...
func (c *NFQueueController) injectAck(spec tcpHarnessNFQueueInjection) error {
	if c.ackGate == nil {
...
func (c *NFQueueController) recordSyntheticAckInjected(spec tcpHarnessNFQueueInjection) {
	c.mu.Lock()
...
func (c *NFQueueController) onPacket(role tcpHarnessNFQueueRole, packet PcapIPv4TCPPacket,
...
func (h *NFQueueHelper) dataPacketDropDecisionLocked(packet PcapIPv4TCPPacket) (
	...
	if h.dropsStopped || !h.firstSeenDataSeqLocked(packet) {
		return false, false
	}
