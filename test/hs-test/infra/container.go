...
// runLabel is set on every container hs-test creates, so that cleanup can target
// a single run without matching on container names.
...
func (c *Container) Start() error {
	...
	// the name filter is a regex, so it has to be anchored - an unanchored name
	// can match containers belonging to a concurrently running test run.
