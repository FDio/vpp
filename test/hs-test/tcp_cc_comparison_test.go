...
func init() {
	RegisterSoloVethTests(TcpBbrBufferbloatTest)
	RegisterManualSoloVethTests(TcpBbrRandomLossStartupTest, TcpCcComparisonCubicTest,
		TcpCcComparisonCubicRackTest, TcpCcComparisonBbrTest)
}

const (
	ccCmpRunTime = 20 * time.Second
	ccCmpWarmup  = 2 * time.Second
	ccCmpSample  = 250 * time.Millisecond
	ccCmpBaseRtt = 100.0
...
func runTcpCcComparison(s *VethsSuite, cfg ccCmpConfig) {
...
collect:
	...
	AssertGreaterEqual(len(samples), 20, "comparison needs enough samples for stable percentiles")
	...
	AssertGreaterEqual(len(steady), 16, "comparison needs steady-state samples")
...
func TcpBbrBufferbloatTest(s *VethsSuite) {
...
func TcpBbrRandomLossStartupTest(s *VethsSuite) {
	runTcpCcComparison(s, ccCmpConfig{
...
func TcpCcComparisonCubicTest(s *VethsSuite) {
	runTcpCcComparison(s, ccCmpConfig{name: "cubic", algo: "cubic"})
}

func TcpCcComparisonCubicRackTest(s *VethsSuite) {
	runTcpCcComparison(s, ccCmpConfig{name: "cubic-rack", algo: "cubic", rack: true})
}

func TcpCcComparisonBbrTest(s *VethsSuite) {
