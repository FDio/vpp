...
func ldpWgetVppProxyNginxCutThru(s *VppProxySuite, proxyFifoSize string, vclFifoSize int, wgetArgs, expectedStatus string) {
	...
	cmd := fmt.Sprintf("test proxy server fifo-size %s server-uri tcp://%s:%d client-uri tcp://%s:%d all-scope",
	...
	writeOut, log := RunLdpWgetContainer(s.Containers.IperfC, args)
	AssertNil(<-ctCheck)
	AssertContains(log, expectedStatus)
	AssertContains(log, "saved")
	AssertNotContains(writeOut, "bytes remaining to read")
...
func RunLdpWgetContainer(clientCont *Container, args string) (string, string) {
