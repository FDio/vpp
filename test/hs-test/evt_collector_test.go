...
//  3. Client VPP runs test vperf client to generate traffic.
...
func EvtCollectorSinkTest(s *VethsSuite) {
	...
	o = serverVpp.Vppctl("test vperf server uri tcp://%s/%s", echoAddr, echoPort)
	Log(o)
	...
	o = clientVpp.Vppctl("test vperf client nclients 2 bytes 1024"+
