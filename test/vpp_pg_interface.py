...
class VppPGInterface(VppInterface):
    ...
    def resolve_arp(self, pg_interface=None):
        ...
        try:
            arp_reply = pg_interface.wait_for_packet(1)
        except:
        ...
        try:
            ...
            self.test.logger.error(
                ppp("Unexpected response to ARP request:", copy))
            raise
    ...
    def resolve_ndp(self, pg_interface=None, timeout=1):
        ...
        while now < deadline:
            try:
                p = pg_interface.wait_for_packet(
            ...
            try:
                ...
                self.test.logger.info(
                    ppp("Unexpected response to NDP request:", p))
            now = time.time()
