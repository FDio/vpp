...
class VppPGInterface(VppInterface):
    ...
    def _get_zmq_capture(self, deadline, filter_out_fn=is_ipv6_misc):
        ...
        while time.time() < deadline:
            try:
                ...
                if packet_data:
                    packet = Ether(
                        packet_data
                    )  # Convert raw data to an Ethernet packet
    ...
    def get_capture(
        ...
        if not capture:
            ...
            # TODO: rewrite self.generate_debug_aid("count-mismatch")
            msg = (
                f"Unexpected packets captured, got {len(capture)} packets, "
    ...
    def wait_for_packet(self, timeout, filter_out_fn=is_ipv6_misc):
        ...
        if config.packet_flow == "zmq":
            ...
            while time.time() < deadline:
                packet = self._get_zmq_capture(deadline, filter_out_fn)
                if packet:
                    self.test.logger.debug(
