...
class VppPGInterface(VppInterface):
    ...
    def decode_pcap_files(self, pcap_dir, filename_prefix):
        ...
        pg_decode = f"{pcap_dir}/{filename_prefix}-pcap-decode.txt"
        self.test.logger.debug(f"Generating testcase pg streams decode file {pg_decode}")
        ts_opts = "-r"
        for p in sorted(Path(pcap_dir).glob(f"{filename_prefix}*.pcap")):
            self.test.logger.debug(f"Decoding {p}")
            with open(f"{pg_decode}", "a") as f:
                f.write(f"\ntshark {ts_opts} {p}\n")
            with open(f"{pg_decode}", "a") as f:
