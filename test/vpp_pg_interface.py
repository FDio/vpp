...
class VppPGInterface(VppInterface):
    ...
    def link_pcap_file(self, path, direction, counter):
        ...
        test_name = self.test_name if hasattr(self, "test_name") else f"suite{self.test.__name__}"
        name = f"{self.test.tempdir}/{test_name}.[timestamp:{time.time():.8f}].{self.name}-{direction}-{counter:04}.{filename}"
        if os.path.isfile(name):
            self.test.logger.debug(f"Skipping hard link creation: {name} already exists!")
            return
        try:
            ...
            self.test.logger.debug(f"OSError: Could not create hard link {path}->{name}")
