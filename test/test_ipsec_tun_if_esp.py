...
from vpp_ip_route import VppIpRoute, VppRoutePath, DpoProto, VppMplsLabel, \
    VppMplsTable, VppMplsRoute, FibPathProto, VppIpInterfaceAddress
...
class TestIpsecItf4(TemplateIpsec,
    ...
    def test_tun_44_pin_egress_attached(self):
        ...
        # add a local prefix to the interface that we can swap about
        local = VppIpInterfaceAddress(self, self.pg0,
                                      "100.1.1.1", 24).add_vpp_config()

        r4 = VppIpRoute(self, "100.1.1.2", 32,
                        [VppRoutePath(
                            self.pg0.remote_ip4,
        ...
        self.config_sa_tun(p,
                           ...
                           "100.1.1.2")
        self.config_protect(p)
        self.verify_tun_44(p, count=n_pkts, out_itf=self.pg0)

        # move the source onto a different link
        local.remove_vpp_config()

        # with the source removed, there is no valid egress interface
        self.verify_tun_dropped_44(p, count=n_pkts)
        local = VppIpInterfaceAddress(self, self.pg1,
                                      "100.1.1.1", 24).add_vpp_config()
        self.verify_tun_44(p, count=n_pkts, out_itf=self.pg1)

        # move the source back to the original link
        local.remove_vpp_config()
        local = VppIpInterfaceAddress(self, self.pg0,
                                      "100.1.1.1", 24).add_vpp_config()
        self.verify_tun_44(p, count=n_pkts, out_itf=self.pg0)
