...
class TestFlowPerPkt(VppTestCase):
    ...
    def verify_cflow_data(self, decoder, capture, cflow):
        ...
        if cflow.haslayer(Data):
            #print cflow[Data].data.encode('hex')
            data = decoder.decode_data_set(cflow.getlayer(Set))
            #print data
            for record in data:
                #print record[1]
                #print record[1].encode('hex')
                #print int(record[1].encode('hex'), 16)
                #print struct.pack("i", 11040)
                #print "%16.0x" % 11040
                #print "{0:0{1}x}".format(11040, 16)
                #self.assertEqual(record[1], struct.pack("!H", 11040))
                #, struct.unpack("HH", record[1])
    ...
    def test_0001(self):
        ...
        pkts = self.create_stream(self.pg1, self.pg2)
    ...
    def test_0005(self):
        ...
        self.pg_start()
        self.pg2.get_capture()
        # make sure no one packet arrived
        self.wait_for_cflow_packet(self.pg0, 256, 60, expected=False)
        self.pg0.get_capture(len(pkts))
    ...
    def test_0006(self):
        ...
        # disble IPFIX
        self.vapi.cli("set ipfix exporter collector 0.0.0.0")
        #self.vapi.set_ipfix_exporter(collector_address="0.0.0.0")
