...
class TestFFP_Timers(MethodHolder):
    ...
    def test_0003(self):
        ...
        templates = ipfix.verify_templates(ipfix_decoder, timeout=23)

        # make sure the one packet we expect actually showed up
        cflow = self.wait_for_cflow_packet(self.collector, templates[0], 30)
        self.verify_cflow_data(ipfix_decoder, capture, cflow)
    ...
    def test_0004(self):
        ...
        # make sure the one packet we expect actually showed up
        self.wait_for_cflow_packet(self.collector, templates[0], 15)
