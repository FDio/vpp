...
class TestFlowperpkt(VppTestCase):
    """ Flow-per-packet plugin: test L2, IP4 and IP6 reporting """
    ...
    def verify_l2_ipfix(self, collector_if):
        ...
        # fail to compare.
        l2_data_udp_string = "12831283003c0000"\
                "000a0034XXXXXXXX0000000000000001010000240000000100000002"\
                "%s02020000ff020008XXXXXXXXXXXXXXXX0092" % \
                self.pg1.local_mac.translate(None, ":")

        l2_template_udp_string = "12831283003c0000000a0034XXXXXXXX00000001000"\
            "000010002002401000007000a0004000e0004003800060050000601000002009"\
            "c000801380002"

        self.logger.info("Look for ipfix packets on %s sw_if_index %d "
                         % (collector_if.name, collector_if.sw_if_index))
        # expecting 2 packets on collector interface based on traffic on other
        # interfaces
        ...
        for p in capture:
            ...
            if compare_with_mask(payload, l2_data_udp_string):
                self.logger.info("found l2 data packet")
                found_l2_data_packet = True
            elif compare_with_mask(payload, l2_template_udp_string):
    ...
    def test_l2_fpp(self):
        ...
        # Make sure the 2 pkts we expect actually showed up
        self.verify_l2_ipfix(self.pg2)
        self.vapi.cli("flowperpkt feature add-del pg1 l2 disable")

    def verify_ip4_ipfix(self, collector_if):
        """Check the ipfix capture"""
        ...
        template_udp_string = "12831283003c0000000a0034XXXXXXXX00000001000000"\
        ...
        for p in capture:
            ...
            if compare_with_mask(payload, data_udp_string):
                self.logger.info("found ip4 data packet")
                found_data_packet = True
            elif compare_with_mask(payload, template_udp_string):
    ...
    def test_ip4_fpp(self):
        ...
        # Make sure the 2 pkts we expect actually showed up
        self.verify_ip4_ipfix(self.pg2)
        self.vapi.cli("flowperpkt feature add-del pg1 disable")

    def create_ip6_stream(self, src_if, dst_if, packet_sizes):
        """Create a packet stream to tickle the plugin
    ...
    def verify_ip6_ipfix(self, collector_if):
        """Check the ipfix capture"""
        found_data_packet = False
        found_template_packet = False
        ...
        data_udp_string = "12831283004f0000"\
                          "000a0047XXXXXXXX0000000000000001"\
                          "010000370000000100000002fd010001000000000000000000000002"\
                          "fd01000200000000000000000000000200XXXXXXXXXXXXXXXX0092"

        template_udp_string = "12831283003c0000"\
                              "000a0034XXXXXXXX000000010000000100020024"\
                              "01000007000a0004000e0004001b0010001c001000050001009c000801380002"
        self.logger.info("Look for ipfix packets on %s sw_if_index %d "
                         % (collector_if.name, collector_if.sw_if_index))
        # expecting 2 packets on collector interface based on traffic on other
        # interfaces
        ...
        for p in capture:
            ...
            payload = str(udp)
            if compare_with_mask(payload, data_udp_string):
                self.logger.info("found ip6 data packet")
                found_data_packet = True
            elif compare_with_mask(payload, template_udp_string):
                self.logger.info("found ip6 template packet")
                found_template_packet = True
            else:
                unmasked_payload = "".join(["%02x" % ord(c) for c in payload])
                self.logger.error("unknown pkt '%s'" % unmasked_payload)
    ...
    def test_ip6_fpp(self):
        ...
        # Export flow records for all pkts transmitted on pg1
        self.vapi.cli("flowperpkt feature add-del pg1 ip6")

        # Arrange to minimally trace generated ipfix packets
        self.vapi.cli("trace add flowperpkt-ipv6 10")

        # Create a stream from pg0 -> pg1, which causes
        # an ipfix packet to be transmitted on pg2

        pkts = self.create_ip6_stream(self.pg0, self.pg1,
                                      self.pg_if_packet_sizes)
        self.pg0.add_stream(pkts)
        self.pg_start()
        ...
        # Make sure the 2 pkts we expect actually showed up
        self.verify_ip6_ipfix(self.pg2)

        self.vapi.cli("flowperpkt feature add-del pg1 ip6 disable")
