...
class TestL2Flowperpkt(VppTestCase):
    ...
    def verify_ipfix_l2_ip4(self, data):
        ...
        for record in data:
            # InputInt: 1 (10)
            self.assertEqual(pack("!I", 1), record[10])
            # OutputInt: 2 (14)
            self.assertEqual(pack("!I", 2), record[14])
            # Data Link Frame Size: 146 (312)
            self.assertEqual(pack("!H", 146), record[312])
            ...
            # Ethernet Type: 8 (256)
            self.assertEqual('\x08\x00', record[256])
            # SrcAddr: 172.16.1.2 (8)
            self.assertEqual('\xac\x10\x01\x02', record[8])
            # DstAddr: 172.16.2.2 (12)
            self.assertEqual('\xac\x10\x02\x02', record[12])
            # IP ToS: 0x00 (5)
            self.assertEqual('\x00', record[5])
    ...
    def verify_ipfix_l2_ip6(self, data):
        ...
        for record in data:
            ...
            self.assertEqual('\x02\x02\x00\x00\xff\x02', record[80])
            # Ethernet Type: 8 (256)
            self.assertEqual('\x86\xdd', record[256])
    ...
    def test_l2_ip4_fpp(self):
        ...
        self.vapi.set_ipfix_exporter(
            collector_address=collector_if.remote_ip4n,
            src_address=collector_if.local_ip4n,
            path_mtu=1450,
            template_interval=1)

        self.vapi.flowperpkt_tx_interface_add_del(
            is_add=1, which=2,
            record_l2=True,
            record_l3=True,
