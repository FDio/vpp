    ...
    def create_host_lists(self, count):
        ...
        for i in self.interfaces:
            ...
            for j in range(0, count):
                self.MY_MACS[i.sw_if_index].append(
                    "00:00:00:ff:%02x:%02x" % (i.sw_if_index, j))
                self.MY_IP4S[i.sw_if_index].append(
                    "172.17.1%02x.%u" % (i.sw_if_index, j))

    def create_stream(self, src_if, packet_sizes):
        pkts = []
        for i in range(0, TestL2xc.pkts_per_burst):
            dst_if = self.flows[src_if][0]
            dst_host_id = random.randrange(
                len(self.MY_MACS[dst_if.sw_if_index]))
            src_host_id = random.randrange(
                len(self.MY_MACS[src_if.sw_if_index]))
            pkt_info = self.create_packet_info(
                src_if.sw_if_index, dst_if.sw_if_index)
            payload = self.info_to_payload(pkt_info)
            p = (Ether(dst=self.MY_MACS[dst_if.sw_if_index][dst_host_id],
                       src=self.MY_MACS[src_if.sw_if_index][src_host_id]) /
                 IP(src=self.MY_IP4S[src_if.sw_if_index][src_host_id],
                    dst=self.MY_IP4S[dst_if.sw_if_index][dst_host_id]) /
