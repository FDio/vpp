...
class MethodHolder(VppTestCase):
    ...
    def frag_in_order_in_plus_out(self, proto=IP_PROTOS.tcp):
        ...
        server = self.pg1.remote_hosts[0]

        server_in_addr = server.ip4
        server_out_addr = '11.11.11.11'
        server_in_port = random.randint(1025, 65535)
        server_out_port = random.randint(1025, 65535)

        self.nat44_add_address(server_out_addr)

        # add static mapping for server
        if proto != IP_PROTOS.icmp:
            self.nat44_add_static_mapping(server_in_addr, server_out_addr,
        ...
        for i in range(2):
            ...
            p = self.reass_frags_and_verify(frags,
                                            ...
                                            self.pg0.remote_ip4)
            if proto != IP_PROTOS.icmp:
                self.assertEqual(p[layer].sport, server_out_port)
                self.assertEqual(p[layer].dport, self.port_in)
            # else:
            #     self.assertEqual(p[layer].id, self.port_in)
            self.assertEqual(data, p[Raw].load)
        ...
        self.assertEqual(reass_n_end - reass_n_start, 4)
