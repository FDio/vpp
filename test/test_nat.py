...
class MethodHolder(VppTestCase):
    ...
    def create_stream_out(self, out_if, dst_ip=None, ttl=64, use_inside_ports=False):
        ...
        :param ttl: TTL of generated packets
        :param use_inside_ports: Use inside NAT ports as destination ports instead of outside ports
        """
        if dst_ip is None:
...
class TestNAT44(MethodHolder):
    ...
    @staticmethod
    def ip4_str2binary(ip_str):
        return ''.join(map(chr,map(int,ip_str.split('.'))))

    def test_dynamic_with_acl(self):
        """ NAT44 dynamic translation test with ACL """
        ...
        acl = {
            ...
            'src_ip_addr': self.ip4_str2binary(self.pg0.remote_ip4),
            'dstport_or_icmpcode_first': 0,
            'dstport_or_icmpcode_last': 0xffff,
            'dst_ip_prefix_len': 32,
            'dst_ip_addr': self.ip4_str2binary(self.pg1.remote_ip4)
        }

        acl_index = self.vapi.acl_add_replace(0xffffffff, [acl]).acl_index
        self.vapi.nat44_set_acl(acl_index)

        real_ip = self.ip4_str2binary(self.pg0.remote_ip4)
        alias_ip = self.ip4_str2binary(self.nat_addr)
        self.vapi.nat44_add_del_static_mapping(local_ip=real_ip, external_ip=alias_ip)
        ...
        try:
            ...
            try:
                pkts = self.create_stream_out(self.pg1, dst_ip=self.pg0.remote_ip4, use_inside_ports=True)
                self.pg1.add_stream(pkts)
                self.pg_enable_capture(self.pg_interfaces)
                self.pg_start()
                capture = self.pg0.get_capture(len(pkts))
