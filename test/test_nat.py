    ...
    def verify_capture_out_with_icmp_errors(self, capture, src_ip=None,
        ...
        for packet in capture:
            try:
                ...
                self.assertTrue(bool(packet.haslayer(ICMP)))
                icmp = packet[ICMP]
...
class TestNAT44(MethodHolder):
    ...
    def test_static_unknown_proto(self):
        ...
        try:
            ...
            self.assertEqual(packet[IP].dst, self.pg1.remote_ip4)
            self.assertTrue(bool(packet.haslayer(GRE)))
            self.assert_packet_checksums_valid(packet)
        except:
            self.logger.error(ppp("Unexpected or invalid packet:", packet))
            raise
...
class TestNAT64(MethodHolder):
    ...
    def test_nat64_inside_interface_handles_neighbor_advertisement(self):
        ...
        try:
            ...
            self.assertTrue(bool(packet.haslayer(ICMPv6ND_NS)))
        ...
        try:
            ...
            self.assertTrue(bool(packet.haslayer(ICMPv6EchoReply)))
        except:
            self.logger.error(ppp("Unexpected or invalid packet:", packet))
            raise
