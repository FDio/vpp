...
class TestNAT44(MethodHolder):
    ...
    def clear_nat44(self):
        ...
        for sm in static_mappings:
            self.vapi.nat44_add_static_mapping(sm.local_ip_address,
        ...
        for addr in adresses:
            self.vapi.nat44_add_address_range(addr.ip_address,
                                              addr.ip_address,
                                              is_add=0)
    ...
    def nat44_add_static_mapping(self, local_ip, external_ip='0.0.0.0',
        ...
        self.vapi.nat44_add_static_mapping(
            l_ip,
            e_ip,
    ...
    def nat44_add_address(self, ip, is_add=1, vrf_id=0xFFFFFFFF):
        ...
        self.vapi.nat44_add_address_range(nat_addr, nat_addr, is_add,
                                          vrf_id=vrf_id)
...
class TestDeterministicNAT44(MethodHolder):
    """ Deterministic NAT44 Test Cases """

    @classmethod
    def setUpConstants(cls):
        super(TestDeterministicNAT44, cls).setUpConstants()
        cls.vpp_cmdline.extend(["nat", "{", "deterministic", "}"])

    @classmethod
    def setUpClass(cls):
        super(TestDeterministicNAT44, cls).setUpClass()
    ...
    def test_deterministic_mode(self):
        ...
        self.vapi.nat44_add_det_map(in_addr_n, in_plen, out_addr_n, out_plen)

        rep1 = self.vapi.nat44_det_forward(in_addr_t_n)
        self.assertEqual(rep1.out_addr[:4], out_addr_n)
        rep2 = self.vapi.nat44_det_reverse(out_addr_n, rep1.out_port_hi)
        self.assertEqual(rep2.in_addr[:4], in_addr_t_n)

        deterministic_mappings = self.vapi.nat44_det_map_dump()
    ...
    def test_set_timeouts(self):
        ...
        timeouts_before = self.vapi.nat44_det_get_timeouts()

        self.vapi.nat44_det_set_timeouts(timeouts_before.udp + 10,
                                         timeouts_before.tcp_established + 10,
                                         timeouts_before.tcp_transitory + 10,
                                         timeouts_before.icmp + 10)

        timeouts_after = self.vapi.nat44_det_get_timeouts()
    ...
    def test_det_in(self):
        ...
        self.vapi.nat44_add_det_map(self.pg0.remote_ip4n,
                                    32,
        ...
        # session dump test
        sessions = self.vapi.nat44_det_session_dump(self.pg0.remote_ip4n)
        self.assertEqual(len(sessions), 3)
    ...
    def test_multiple_users(self):
        ...
        self.vapi.nat44_add_det_map(host0.ip4n,
                                    24,
                                    socket.inet_aton(nat_ip),
                                    32)
        self.vapi.nat44_interface_add_del_feature(self.pg0.sw_if_index)
        ...
        dms = self.vapi.nat44_det_map_dump()
        ...
        self.vapi.nat44_det_close_session_out(socket.inet_aton(nat_ip),
                                              port_out1,
        ...
        self.vapi.nat44_det_close_session_in(host0.ip4n,
                                             port_in,
                                             self.pg1.remote_ip4n,
                                             external_port)
        dms = self.vapi.nat44_det_map_dump()
    ...
    def test_session_timeout(self):
        ...
        self.initiate_tcp_session(self.pg0, self.pg1)
        self.vapi.nat44_det_set_timeouts(5, 5, 5, 5)
        pkts = self.create_stream_in(self.pg0, self.pg1)
        self.pg0.add_stream(pkts)
        self.pg_enable_capture(self.pg_interfaces)
        self.pg_start()
    ...
    def clear_nat_det(self):
        ...
        self.vapi.nat44_det_set_timeouts()
        deterministic_mappings = self.vapi.nat44_det_map_dump()
        for dsm in deterministic_mappings:
            self.vapi.nat44_add_det_map(dsm.in_addr,
                                        dsm.in_plen,
                                        dsm.out_addr,
                                        dsm.out_plen,
                                        is_add=0)
    ...
    def tearDown(self):
        super(TestDeterministicNAT44, self).tearDown()
        if not self.vpp_dead:
