...
class TestLwB4(MethodHolder):
    ...
    def assertPSIDRange(self, port):
        """ Asserts that given port matches the PSID parameters of the test """
    ...
    def test_in2out(self):
        ...
        session_str = '{og_ip}:{og_p} out {b4_ip}:{nat_p} protocol udp'.format(
            og_ip=self.pg0.remote_ip4,
            og_p=10000,
            b4_ip=self.b4_ip4,
            nat_p=capture[UDP].sport
        )
