...
class MethodHolder(VppTestCase):
    ...
    def clear_nat44(self):
        ...
        for intf in interfaces:
            self.vapi.nat44_add_del_interface_addr(is_add=0, sw_if_index=intf.sw_if_index, flags=intf.flags)

        self.vapi.nat_ipfix_enable_disable(domain_id=self.ipfix_domain_id, src_port=self.ipfix_src_port, enable=0)
        self.ipfix_src_port = 4739
        self.ipfix_domain_id = 1

        self.vapi.syslog_set_filter(SYSLOG_SEVERITY.EMERG)

        self.vapi.nat_ha_set_listener(ip_address='0.0.0.0', port=0, path_mtu=512)
        self.vapi.nat_ha_set_failover(ip_address='0.0.0.0', port=0, session_refresh_interval=10)
        ...
        for intf in interfaces:
            self.vapi.nat44_interface_add_del_output_feature(is_add=0, flags=intf.flags, sw_if_index=intf.sw_if_index)
        static_mappings = self.vapi.nat44_static_mapping_dump()
        for sm in static_mappings:
            self.vapi.nat44_add_del_static_mapping(is_add=0, local_ip_address=sm.local_ip_address,
                                                   external_ip_address=sm.external_ip_address,
                                                   external_sw_if_index=sm.external_sw_if_index,
                                                   local_port=sm.local_port, external_port=sm.external_port,
                                                   vrf_id=sm.vrf_id, protocol=sm.protocol, flags=sm.flags, tag=sm.tag)

        lb_static_mappings = self.vapi.nat44_lb_static_mapping_dump()
        for lb_sm in lb_static_mappings:
            self.vapi.nat44_add_del_lb_static_mapping(is_add=0, flags=lb_sm.flags, external_addr=lb_sm.external_addr,
                                                      external_port=lb_sm.external_port, protocol=lb_sm.protocol,
                                                      local_num=0, locals=[], tag=lb_sm.tag)

        identity_mappings = self.vapi.nat44_identity_mapping_dump()
        for id_m in identity_mappings:
            self.vapi.nat44_add_del_identity_mapping(ip_address=id_m.ip_address, sw_if_index=id_m.sw_if_index, port=id_m.port,
                                                     flags=id_m.flags, vrf_id=id_m.vrf_id, protocol=id_m.protocol)

        addresses = self.vapi.nat44_address_dump()
        for addr in addresses:
            self.vapi.nat44_add_del_address_range(first_ip_address=addr.ip_address, last_ip_address=addr.ip_address,
                                                  vrf_id=0xFFFFFFFF, flags=addr.flags)

        self.vapi.nat_set_reass(timeout=2, max_reass=1024, max_frag=5, drop_frag=0)
        self.vapi.nat_set_reass(timeout=2, max_reass=1024, max_frag=5, drop_frag=0, is_ip6=1)
        self.verify_no_nat44_user()
        self.vapi.nat_set_timeouts(udp=300, tcp_established=7440, tcp_transitory=240, icmp=60)
    ...
    def nat44_add_static_mapping(self, local_ip, external_ip='0.0.0.0',
        ...
        self.vapi.nat44_add_del_static_mapping(is_add=is_add, local_ip_address=local_ip,
                                               external_ip_address=external_ip,
                                               external_sw_if_index=external_sw_if_index, local_port=local_port,
                                               external_port=external_port, vrf_id=vrf_id, protocol=proto, flags=flags,
                                               tag=tag)
    ...
    def nat44_add_address(self, ip, is_add=1, vrf_id=0xFFFFFFFF, twice_nat=0):
        ...
        self.vapi.nat44_add_del_address_range(first_ip_address=ip, last_ip_address=ip, vrf_id=vrf_id, is_add=is_add,
                                              flags=flags)
...
class TestNAT44(MethodHolder):
    ...
    def test_forwarding(self):
        ...
        flags = self.config_flags.NAT_IS_ADDR_ONLY
        self.vapi.nat44_add_del_static_mapping(is_add=1, local_ip_address=real_ip, external_ip_address=alias_ip,
        ...
        try:
            ...
            self.vapi.nat44_forwarding_enable_disable(enable=0)
            flags = self.config_flags.NAT_IS_ADDR_ONLY
            self.vapi.nat44_add_del_static_mapping(is_add=0, local_ip_address=real_ip, external_ip_address=alias_ip,
                                                   external_sw_if_index=0xFFFFFFFF, flags=flags)
    ...
    def test_identity_nat(self):
        """ Identity NAT """
        flags = self.config_flags.NAT_IS_ADDR_ONLY
        self.vapi.nat44_add_del_identity_mapping(ip_address=self.pg0.remote_ip4n, sw_if_index=0xFFFFFFFF, flags=flags, is_add=1)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg0.sw_if_index,
            flags=flags, is_add=1)
        ...
        self.assertEqual(len(sessions), 0)
        flags = self.config_flags.NAT_IS_ADDR_ONLY
        self.vapi.nat44_add_del_identity_mapping(ip_address=self.pg0.remote_ip4n, sw_if_index=0xFFFFFFFF, flags=flags, vrf_id=1, is_add=1)
        identity_mappings = self.vapi.nat44_identity_mapping_dump()
    ...
    def test_interface_addr(self):
        ...
        self.vapi.nat44_add_del_interface_addr(is_add=1, sw_if_index=self.pg7.sw_if_index)
    ...
    def test_interface_addr_identity_nat(self):
        ...
        port = 53053
        self.vapi.nat44_add_del_interface_addr(is_add=1, sw_if_index=self.pg7.sw_if_index)
        self.vapi.nat44_add_del_identity_mapping(ip_address=b'0', sw_if_index=self.pg7.sw_if_index, port=port, protocol=IP_PROTOS.tcp, is_add=1)
    ...
    def test_ipfix_nat44_sess(self):
        ...
        self.vapi.nat_ipfix_enable_disable(domain_id=self.ipfix_domain_id, src_port=self.ipfix_src_port, enable=1)
    ...
    def test_output_feature(self):
        ...
        self.nat44_add_address(self.nat_addr)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_output_feature(is_add=1, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat44_interface_add_del_output_feature(is_add=1, flags=flags, sw_if_index=self.pg1.sw_if_index)
        self.vapi.nat44_interface_add_del_output_feature(is_add=1, sw_if_index=self.pg3.sw_if_index)
    ...
    def test_output_feature_vrf_aware(self):
        ...
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_output_feature(is_add=1, flags=flags, sw_if_index=self.pg4.sw_if_index)
        self.vapi.nat44_interface_add_del_output_feature(is_add=1, flags=flags, sw_if_index=self.pg6.sw_if_index)
        self.vapi.nat44_interface_add_del_output_feature(is_add=1, sw_if_index=self.pg3.sw_if_index)
    ...
    def test_output_feature_hairpinning(self):
        ...
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_output_feature(is_add=1, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat44_interface_add_del_output_feature(is_add=1, sw_if_index=self.pg1.sw_if_index)
    ...
    def test_del_session(self):
        ...
        self.vapi.nat44_del_session(address=sessions[0].inside_ip_address, port=sessions[0].inside_port,
                                    protocol=sessions[0].protocol, flags=self.config_flags.NAT_IS_INSIDE)
        self.vapi.nat44_del_session(address=sessions[1].outside_ip_address, port=sessions[1].outside_port,
    ...
    def test_set_get_reass(self):
        ...
        self.vapi.nat_set_reass(timeout=reas_cfg1.ip4_timeout + 5, max_reass=reas_cfg1.ip4_max_reass * 2,
                                max_frag=reas_cfg1.ip4_max_frag * 2, drop_frag=0)
        ...
        self.vapi.nat_set_reass(timeout=2, max_reass=1024, max_frag=5, drop_frag=1)
    ...
    def test_frag_forwarding(self):
        ...
        self.vapi.nat44_add_del_interface_addr(is_add=1, sw_if_index=self.pg1.sw_if_index)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg0.sw_if_index,
            flags=flags, is_add=1)
    ...
    def test_ipfix_max_frags(self):
        ...
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg1.sw_if_index,
            is_add=1)
        self.vapi.nat_set_reass(timeout=2, max_reass=1024, max_frag=1, drop_frag=0)
        self.vapi.set_ipfix_exporter(collector_address=self.pg3.remote_ip4n,
                                     src_address=self.pg3.local_ip4n,
                                     path_mtu=512,
                                     template_interval=10)
    ...
    def test_session_timeout(self):
        ...
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg1.sw_if_index,
            is_add=1)
        self.vapi.nat_set_timeouts(udp=5, tcp_established=7440, tcp_transitory=240, icmp=60)
    ...
    def test_ha_send(self):
        ...
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg1.sw_if_index,
            is_add=1)
        self.vapi.nat_ha_set_listener(ip_address=self.pg3.local_ip4, port=12345, path_mtu=512)
        self.vapi.nat_ha_set_failover(ip_address=self.pg3.remote_ip4, port=12346, session_refresh_interval=10)
        ...
        # delete one session
        self.pg_enable_capture(self.pg_interfaces)
        self.vapi.nat44_del_session(address=self.pg0.remote_ip4n, port=self.tcp_port_in, protocol=IP_PROTOS.tcp,
                                    flags=self.config_flags.NAT_IS_INSIDE)
        self.vapi.nat_ha_flush()
...
class TestNAT44EndpointDependent(MethodHolder):
    ...
    def test_frag_in_order_in_plus_out(self):
        ...
        self.vapi.nat_set_reass(timeout=10, max_reass=1024, max_frag=5, drop_frag=0)
    ...
    def test_static_lb(self):
        ...
        self.nat44_add_address(self.nat_addr)
        self.vapi.nat44_add_del_lb_static_mapping(is_add=1, external_addr=external_addr_n, external_port=external_port,
                                                  protocol=IP_PROTOS.tcp, local_num=len(locals), locals=locals)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg0.sw_if_index,
            flags=flags, is_add=1)
    ...
    def test_static_lb_multi_clients(self):
        ...
        self.nat44_add_address(self.nat_addr)
        self.vapi.nat44_add_del_lb_static_mapping(is_add=1, external_addr=external_addr, external_port=external_port,
                                                  protocol=IP_PROTOS.tcp, local_num=len(locals), locals=locals)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg0.sw_if_index,
        ...
        # add new back-end
        self.vapi.nat44_lb_static_mapping_add_del_local(is_add=1, external_addr=external_addr,
                                                        external_port=external_port, local=local,
        ...
        # remove one back-end
        self.vapi.nat44_lb_static_mapping_add_del_local(is_add=0, external_addr=external_addr,
                                                        external_port=external_port, local=local,
                                                        protocol=IP_PROTOS.tcp)
        server1_n = 0
        server2_n = 0
    ...
    def test_static_lb_2(self):
        ...
        self.vapi.nat44_add_del_lb_static_mapping(is_add=1, flags=flags, external_addr=external_addr,
                                                  external_port=external_port, protocol=IP_PROTOS.tcp,
                                                  local_num=len(locals), locals=locals)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg0.sw_if_index,
            flags=flags, is_add=1)
    ...
    def test_lb_affinity(self):
        ...
        self.nat44_add_address(self.nat_addr)
        self.vapi.nat44_add_del_lb_static_mapping(is_add=1, external_addr=external_addr, external_port=external_port,
                                                  protocol=IP_PROTOS.tcp, affinity=10800, local_num=len(locals),
                                                  locals=locals)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg0.sw_if_index,
    ...
    def test_output_feature_and_service(self):
        ...
        self.nat44_add_address(self.nat_addr)
        flags = self.config_flags.NAT_IS_ADDR_ONLY
        self.vapi.nat44_add_del_identity_mapping(ip_address=self.pg1.remote_ip4n, sw_if_index=0xFFFFFFFF, flags=flags, is_add=1)
        flags = self.config_flags.NAT_IS_OUT2IN_ONLY
    ...
    def twice_nat_common(self, self_twice_nat=False, same_pg=False, lb=False,
        ...
        if not lb:
            ...
            self.vapi.nat44_add_del_lb_static_mapping(is_add=1, flags=flags, external_addr=out_addr,
                                                      external_port=port_out, protocol=IP_PROTOS.tcp,
                                                      local_num=len(locals), locals=locals)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_feature(
    ...
    def test_twice_nat_interface_addr(self):
        ...
        flags = self.config_flags.NAT_IS_TWICE_NAT
        self.vapi.nat44_add_del_interface_addr(is_add=1, sw_if_index=self.pg3.sw_if_index, flags=flags)
    ...
    def test_session_timeout(self):
        ...
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg1.sw_if_index,
            is_add=1)
        self.vapi.nat_set_timeouts(udp=300, tcp_established=7440, tcp_transitory=240, icmp=5)
    ...
    def test_session_rst_timeout(self):
        ...
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg1.sw_if_index,
            is_add=1)
        self.vapi.nat_set_timeouts(udp=300, tcp_established=7440, tcp_transitory=5, icmp=60)
...
class TestNAT44Out2InDPO(MethodHolder):
    ...
    def test_464xlat_ce(self):
        ...
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg0.sw_if_index,
            flags=flags, is_add=1)
        self.vapi.nat44_add_del_address_range(first_ip_address=self.nat_addr_n, last_ip_address=self.nat_addr_n,
...
class TestDeterministicNAT(MethodHolder):
    ...
    def test_deterministic_mode(self):
        ...
        self.vapi.nat_det_add_del_map(is_add=1, in_addr=in_addr, in_plen=in_plen, out_addr=out_addr, out_plen=out_plen)
    ...
    def test_set_timeouts(self):
        ...
        self.vapi.nat_set_timeouts(udp=timeouts_before.udp + 10, tcp_established=timeouts_before.tcp_established + 10,
                                   tcp_transitory=timeouts_before.tcp_transitory + 10, icmp=timeouts_before.icmp + 10)
    ...
    def test_det_in(self):
        ...
        self.vapi.nat_det_add_del_map(is_add=1, in_addr=self.pg0.remote_ip4n, in_plen=32,
                                      out_addr=socket.inet_aton(nat_ip), out_plen=32)
    ...
    def test_multiple_users(self):
        ...
        self.vapi.nat_det_add_del_map(is_add=1, in_addr=host0.ip4n, in_plen=24, out_addr=socket.inet_aton(nat_ip),
                                      out_plen=32)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg0.sw_if_index,
    ...
    def test_tcp_session_close_detection_in(self):
        ...
        self.vapi.nat_det_add_del_map(is_add=1, in_addr=self.pg0.remote_ip4n, in_plen=32,
                                      out_addr=socket.inet_aton(self.nat_addr), out_plen=32)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat44_interface_add_del_feature(
            sw_if_index=self.pg0.sw_if_index,
            flags=flags, is_add=1)
    ...
    def test_session_timeout(self):
        ...
        self.initiate_tcp_session(self.pg0, self.pg1)
        self.vapi.nat_set_timeouts(udp=5, tcp_established=5, tcp_transitory=5, icmp=5)
        pkts = self.create_stream_in(self.pg0, self.pg1)
        self.pg0.add_stream(pkts)
        self.pg_enable_capture(self.pg_interfaces)
        self.pg_start()
    ...
    def clear_nat_det(self):
        ...
        for dsm in deterministic_mappings:
            self.vapi.nat_det_add_del_map(is_add=0, in_addr=dsm.in_addr, in_plen=dsm.in_plen, out_addr=dsm.out_addr,
...
class TestNAT64(MethodHolder):
    ...
    def test_nat64_inside_interface_handles_neighbor_advertisement(self):
        ...
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat64_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg5.sw_if_index)
    ...
    def test_pool(self):
        ...
        self.vapi.nat64_add_del_pool_addr_range(start_addr=nat_addr, end_addr=nat_addr, vrf_id=0xFFFFFFFF, is_add=1)

        addresses = self.vapi.nat64_pool_addr_dump()
        self.assertEqual(len(addresses), 1)
        self.assertEqual(str(addresses[0].address), nat_addr)

        self.vapi.nat64_add_del_pool_addr_range(start_addr=nat_addr, end_addr=nat_addr, vrf_id=0xFFFFFFFF, is_add=0)
    ...
    def test_interface(self):
        ...
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat64_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat64_add_del_interface(is_add=1, flags=0, sw_if_index=self.pg1.sw_if_index)
        ...
        self.vapi.nat64_add_del_interface(is_add=0, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat64_add_del_interface(is_add=0, flags=flags, sw_if_index=self.pg1.sw_if_index)
    ...
    def test_static_bib(self):
        ...
        self.vapi.nat64_add_del_static_bib(i_addr=in_addr, o_addr=out_addr, i_port=in_port, o_port=out_port,
    ...
    def test_set_timeouts(self):
        ...
        self.vapi.nat_set_timeouts(udp=200, tcp_established=7450, tcp_transitory=250, icmp=30)
        timeouts = self.vapi.nat_get_timeouts()
    ...
    def test_dynamic(self):
        ...
        self.vapi.nat64_add_del_pool_addr_range(start_addr=self.nat_addr, end_addr=self.nat_addr, vrf_id=0xFFFFFFFF,
                                                is_add=1)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat64_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat64_add_del_interface(is_add=1, flags=0, sw_if_index=self.pg1.sw_if_index)
        ...
        self.vapi.nat64_add_del_pool_addr_range(start_addr=self.vrf1_nat_addr, end_addr=self.vrf1_nat_addr,
                                                vrf_id=self.vrf1_id, is_add=1)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat64_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg2.sw_if_index)
    ...
    def test_static(self):
        ...
        self.vapi.nat64_add_del_static_bib(i_addr=self.pg0.remote_ip6n, o_addr=self.nat_addr, i_port=self.tcp_port_in,
                                           o_port=self.tcp_port_out, proto=IP_PROTOS.tcp, vrf_id=0, is_add=1)
        self.vapi.nat64_add_del_static_bib(i_addr=self.pg0.remote_ip6n, o_addr=self.nat_addr, i_port=self.udp_port_in,
                                           o_port=self.udp_port_out, proto=IP_PROTOS.udp, vrf_id=0, is_add=1)
        self.vapi.nat64_add_del_static_bib(i_addr=self.pg0.remote_ip6n, o_addr=self.nat_addr, i_port=self.icmp_id_in,
                                           o_port=self.icmp_id_out, proto=IP_PROTOS.icmp, vrf_id=0, is_add=1)
    ...
    def test_session_timeout(self):
        ...
        self.vapi.nat64_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat64_add_del_interface(is_add=1, flags=0, sw_if_index=self.pg1.sw_if_index)
        self.vapi.nat_set_timeouts(udp=300, tcp_established=5, tcp_transitory=5, icmp=5)
    ...
    def test_hairpinning(self):
        ...
        self.vapi.nat64_add_del_static_bib(i_addr=server.ip6n, o_addr=self.nat_addr, i_port=server_tcp_in_port,
                                           o_port=server_tcp_out_port, proto=IP_PROTOS.tcp, vrf_id=0, is_add=1)
        self.vapi.nat64_add_del_static_bib(i_addr=server.ip6n, o_addr=self.nat_addr, i_port=server_udp_in_port,
                                           o_port=server_udp_out_port, proto=IP_PROTOS.udp, vrf_id=0, is_add=1)
    ...
    def test_prefix(self):
        ...
        self.vapi.nat64_add_del_prefix(prefix=vrf1_pref64_str, vrf_id=self.vrf1_id, is_add=1)
    ...
    def test_hairpinning_unknown_proto(self):
        ...
        self.vapi.nat64_add_del_pool_addr_range(start_addr=server_nat_ip, end_addr=client_nat_ip, vrf_id=0xFFFFFFFF,
                                                is_add=1)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat64_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat64_add_del_interface(is_add=1, flags=0, sw_if_index=self.pg1.sw_if_index)

        self.vapi.nat64_add_del_static_bib(i_addr=server.ip6n, o_addr=server_nat_ip, i_port=server_tcp_in_port,
                                           o_port=server_tcp_out_port, proto=IP_PROTOS.tcp, vrf_id=0, is_add=1)

        self.vapi.nat64_add_del_static_bib(i_addr=server.ip6n, o_addr=server_nat_ip, i_port=0, o_port=0,
                                           proto=IP_PROTOS.gre, vrf_id=0, is_add=1)

        self.vapi.nat64_add_del_static_bib(i_addr=client.ip6n, o_addr=client_nat_ip, i_port=client_tcp_in_port,
                                           o_port=client_tcp_out_port, proto=IP_PROTOS.tcp, vrf_id=0, is_add=1)
    ...
    def test_one_armed_nat64(self):
        ...
        self.vapi.nat64_add_del_pool_addr_range(start_addr=self.nat_addr, end_addr=self.nat_addr, vrf_id=0xFFFFFFFF,
                                                is_add=1)
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat64_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg3.sw_if_index)
        self.vapi.nat64_add_del_interface(is_add=1, flags=0, sw_if_index=self.pg3.sw_if_index)
    ...
    def test_reass_hairpinning(self):
        ...
        self.vapi.nat64_add_del_static_bib(i_addr=server.ip6n, o_addr=self.nat_addr, i_port=server_in_port,
                                           o_port=server_out_port, proto=IP_PROTOS.tcp, vrf_id=0, is_add=1)
    ...
    def test_interface_addr(self):
        ...
        self.vapi.nat64_add_del_interface_addr(is_add=1, sw_if_index=self.pg4.sw_if_index)
    ...
    def test_ipfix_max_frags(self):
        ...
        self.vapi.nat64_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat64_add_del_interface(is_add=1, flags=0, sw_if_index=self.pg1.sw_if_index)
        self.vapi.nat_set_reass(timeout=2, max_reass=1024, max_frag=1, drop_frag=0, is_ip6=1)
        self.vapi.set_ipfix_exporter(collector_address=self.pg3.remote_ip4n,
                                     src_address=self.pg3.local_ip4n,
                                     path_mtu=512,
                                     template_interval=10)
    ...
    def clear_nat64(self):
        ...
        interfaces = self.vapi.nat64_interface_dump()
        for intf in interfaces:
            self.vapi.nat64_add_del_interface(is_add=0, flags=intf.flags, sw_if_index=intf.sw_if_index)

        bib = self.vapi.nat64_bib_dump(proto=255)
        for bibe in bib:
            if bibe.flags & self.config_flags.NAT_IS_STATIC:
                self.vapi.nat64_add_del_static_bib(i_addr=bibe.i_addr, o_addr=bibe.o_addr, i_port=bibe.i_port,
                                                   o_port=bibe.o_port, proto=bibe.proto, vrf_id=bibe.vrf_id, is_add=0)

        adresses = self.vapi.nat64_pool_addr_dump()
        for addr in adresses:
            self.vapi.nat64_add_del_pool_addr_range(start_addr=addr.address, end_addr=addr.address, vrf_id=addr.vrf_id,
                                                    is_add=0)

        prefixes = self.vapi.nat64_prefix_dump()
        for prefix in prefixes:
            self.vapi.nat64_add_del_prefix(prefix=str(prefix.prefix), vrf_id=prefix.vrf_id, is_add=0)
...
class TestDSlite(MethodHolder):
    ...
    def test_dslite(self):
        ...
        self.vapi.dslite_add_del_pool_addr_range(start_addr=self.nat_addr, end_addr=self.nat_addr, is_add=1)
        aftr_ip4 = '192.0.0.1'
...
class TestNAT66(MethodHolder):
    ...
    def test_static(self):
        """ 1:1 NAT66 test """
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat66_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat66_add_del_interface(is_add=1, sw_if_index=self.pg1.sw_if_index)
        self.vapi.nat66_add_del_static_mapping(local_ip_address=self.pg0.remote_ip6n, external_ip_address=self.nat_addr,
                                               is_add=1)
    ...
    def test_check_no_translate(self):
        ...
        flags = self.config_flags.NAT_IS_INSIDE
        self.vapi.nat66_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg0.sw_if_index)
        self.vapi.nat66_add_del_interface(is_add=1, flags=flags, sw_if_index=self.pg1.sw_if_index)
        self.vapi.nat66_add_del_static_mapping(local_ip_address=self.pg0.remote_ip6n, external_ip_address=self.nat_addr,
                                               is_add=1)
    ...
    def clear_nat66(self):
        ...
        for intf in interfaces:
            self.vapi.nat66_add_del_interface(is_add=0, flags=intf.flags, sw_if_index=intf.sw_if_index)

        static_mappings = self.vapi.nat66_static_mapping_dump()
        for sm in static_mappings:
            self.vapi.nat66_add_del_static_mapping(local_ip_address=sm.local_ip_address,
