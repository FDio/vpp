...
class TestNAT44(MethodHolder):
    ...
    def test_ha_send(self):
        ...
        self.assertEqual(p[IP].src, self.pg3.local_ip4)
        self.assertEqual(p[IP].dst, self.pg3.remote_ip4)
        self.assertEqual(p[UDP].sport, 12345)
        self.assertEqual(p[UDP].dport, 12346)
        self.assertEqual(p[HANATStateSync].version, 1)
        self.assertEqual(p[HANATStateSync].thread_index, 0)
        self.assertEqual(p[HANATStateSync].count, 3)
    ...
    def test_ha_recv(self):
        ...
        capture = self.pg3.get_capture(1)
        p = capture[0]
        self.assertEqual(p[HANATStateSync].sequence_number, 3)
        self.assertEqual(p[HANATStateSync].flags, 'ACK')
        self.assertEqual(p[HANATStateSync].version, 1)
        users = self.vapi.nat44_user_dump()
        self.assertEqual(len(users), 1)
        ...
        capture = self.pg0.get_capture(1)
        p = capture[0]
        self.assertEqual(p[IP].src, self.pg1.remote_ip4)
        self.assertEqual(p[IP].dst, self.pg0.remote_ip4)
        self.assertEqual(p[TCP].sport, self.tcp_external_port)
