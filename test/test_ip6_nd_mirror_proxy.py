...
class TestNDPROXY(VppTestCase):
    ...
    def test_nd_mirror_proxy(self):
        ...
        #
        # Send the unicast ND request with an originating address that
        # is VPP's own address
        #
        ...
        #
        # Send the unicast ND request again with an originating address that
        # is VPP's own address, this time dst address is in local cache
        #
        rx = self.send_and_expect(self.pg1, [unicast_nd_req_from_host],
                                  self.pg1)
        self.assertEqual(rx[0][Ether].src, self.pg1.local_mac)
