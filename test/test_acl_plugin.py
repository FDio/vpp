...
class TestACLplugin(VppTestCase):
        ...
        for dst_if in self.flows[src_if]:
            ...
            for i in range(0, n_int):
                ...
                p = (Ether(dst=dst_host.mac, src=src_host.mac) /
                     ...
                     UDP(sport=10000 + i*(123+traffic_type), dport=500+(i+2*traffic_type)) /
                     Raw(payload))
    ...
    def create_stream_icmp(self, in_if, out_if, type, code):
        """
        ...
        dst_host = dst_hosts[0]
        src_host = src_hosts[0]
        p = (Ether(dst=dst_host.mac, src=src_host.mac) /
             IP(src=src_host.ip4, dst=dst_host.ip4) /
             ICMP(code=code, type=type))
        pkts.append(p)
	...
	self.run_traffic_no_check()
        ...
        self.logger.info("Working with ACL plugin version: %d.%d" % (reply.major, reply.minor))
    ...
    def test_0001_acl_create(self):
        ...
        # Add an ACL
        r = [ 
              {'dstport_or_icmpcode_last': 1234, 'is_permit': 1, 'srcport_or_icmptype_last': 1235, 'dst_ip_addr': '\x00\x00\x00\x00', 'is_ipv6': 0, 
               'dstport_or_icmpcode_first': 1234, 'src_ip_prefix_len': 0, 'src_ip_addr': '\x00\x00\x00\x00', 
               'srcport_or_icmptype_first': 1235, 'proto': 17, 'dst_ip_prefix_len': 0}
            ]
        # Test 1: add a new ACL
        reply = self.api_acl_add_replace(acl_index=4294967295, r=r, count=len(r), tag="permit 1234")
        self.assertEqual(reply.retval, 0)
        self.assertEqual(reply.acl_index, 0) # The very first ACL gets #0
        ...
        for i_rule in range(0, len(r)-1):
          for rule_key in r[i_rule]:
            self.assertEqual(rr[0].r[i_rule][rule_key], r[i_rule][rule_key]) 

        # Add a deny-1234 ACL
        r_deny = [ 
              {'dstport_or_icmpcode_last': 1234, 'is_permit': 0, 'srcport_or_icmptype_last': 1235, 'dst_ip_addr': '\x00\x00\x00\x00', 'is_ipv6': 0, 
               'dstport_or_icmpcode_first': 1234, 'src_ip_prefix_len': 0, 'src_ip_addr': '\x00\x00\x00\x00', 
               'srcport_or_icmptype_first': 1235, 'proto': 17, 'dst_ip_prefix_len': 0},
              {'dstport_or_icmpcode_last': 0, 'is_permit': 1, 'srcport_or_icmptype_last': 0, 'dst_ip_addr': '\x00\x00\x00\x00', 'is_ipv6': 0, 
               'dstport_or_icmpcode_first': 0, 'src_ip_prefix_len': 0, 'src_ip_addr': '\x00\x00\x00\x00', 
               'srcport_or_icmptype_first': 0, 'proto': 0, 'dst_ip_prefix_len': 0}
            ]
        reply = self.api_acl_add_replace(acl_index=4294967295, r=r_deny, count=len(r_deny), tag="deny 1234;permit all")
        self.assertEqual(reply.retval, 0)
        self.assertEqual(reply.acl_index, 1) # The second ACL gets #1

        # Test 2: try to modify a nonexistent ACL
        reply = self.api_acl_add_replace(acl_index=432, r=r, count=len(r), tag="FFFF:FFFF", expected_retval=-1)
        self.assertEqual(reply.retval, -1)
        self.assertEqual(reply.acl_index, 432) # The ACL number should pass through
	  ...
	  for dst_if in self.flows[src_if]:
	    ...
	    for i in range(0, n_int):
	      ...
	      src_host = src_hosts[i % len(src_hosts)]
              ...
              dport = 500 + i
              new_rule = {
                 'dstport_or_icmpcode_last': dport, 'is_permit': 1, 'srcport_or_icmptype_last': sport, 'dst_ip_addr': dst_host.ip4, 'is_ipv6': 0, 
                 'dstport_or_icmpcode_first': dport, 'src_ip_prefix_len': 0, 'src_ip_addr': src_host.ip4,
                 'srcport_or_icmptype_first': sport, 'proto': 17, 'dst_ip_prefix_len': 0}
              r.append(new_rule)

        # deny any any in the end is implicit
        # r.append(
        #      {'dstport_or_icmpcode_last': 0, 'is_permit': 0, 'srcport_or_icmptype_last': 0, 'dst_ip_addr': '\x00\x00\x00\x00', 'is_ipv6': 0, 
        #       'dstport_or_icmpcode_first': 0, 'src_ip_prefix_len': 0, 'src_ip_addr': '\x00\x00\x00\x00', 
        #       'srcport_or_icmptype_first': 0, 'proto': 0, 'dst_ip_prefix_len': 0})
            
        # Test 1: add a new ACL
        reply = self.api_acl_add_replace(acl_index=4294967295, r=r, count=len(r), tag="permit per-flow")
        permit_acl_index = reply.acl_index
        # Apply a permitting ACL on the interface pg0 as inbound
        for i in self.pg_interfaces:
	  self.api_acl_interface_set_acl_list(sw_if_index = i.sw_if_index, count=1, n_input=1, acls = [ permit_acl_index ])
	  ...
	  for dst_if in self.flows[src_if]:
	    ...
	    for i in range(0, n_int):
	      ...
	      src_host = src_hosts[i % len(src_hosts)]
              ...
              dport = 500 + i
              new_rule = {
                 'dstport_or_icmpcode_last': dport, 'is_permit': 0, 'srcport_or_icmptype_last': sport, 'dst_ip_addr': dst_host.ip4, 'is_ipv6': 0, 
                 'dstport_or_icmpcode_first': dport, 'src_ip_prefix_len': 0, 'src_ip_addr': src_host.ip4,
                 'srcport_or_icmptype_first': sport, 'proto': 17, 'dst_ip_prefix_len': 0}
              r.append(new_rule)
        ...
        reply = self.api_acl_add_replace(acl_index=4294967295, r=r, count=len(r), tag="deny per-flow;permit all")
        self.assertEqual(reply.retval, 0)
        deny_acl_index = reply.acl_index

        # Apply a denying ACL all of interfaces as inbound
        for i in self.pg_interfaces:
	  self.api_acl_interface_set_acl_list(sw_if_index = i.sw_if_index, count=1, n_input=1, acls = [ deny_acl_index ])
    ...
    def test_0004_vpp624_permit(self):
        ...
        for src_if in self.pg_interfaces:
            ...
            for dst_if in self.flows[src_if]:
                ...
                for i in range(0, n_int):
                    ...
                    src_host = src_hosts[i % len(src_hosts)]
                    new_rule =  (
                        {'is_permit': 1, 'is_ipv6': 0, 'proto': 1,
                         'srcport_or_icmptype_first': 0, 'srcport_or_icmptype_last': self.icmp_type,
                         'src_ip_prefix_len': 0, 'src_ip_addr': '\x00\x00\x00\x00',
                         'dstport_or_icmpcode_first': 0, 'dstport_or_icmpcode_last': self.icmp_code,
                         'dst_ip_addr': '\x00\x00\x00\x00', 'dst_ip_prefix_len': 0})
                    r.append(new_rule)

        reply = self.api_acl_add_replace(acl_index=4294967295, r=r, count=len(r))
        rr = self.api_acl_dump(reply.acl_index)
        self.logger.info("Dumped ACL: " + str(rr))
        permit_acl_index = reply.acl_index
        # Apply a permitting ACL on the interface pg0 as inbound
        for i in self.pg_interfaces:
            self.api_acl_interface_set_acl_list(sw_if_index=i.sw_if_index, count=1, n_input=1,
                                                acls=[permit_acl_index])

        # Traffic should still pass
        pkts = self.create_stream_icmp(self.pg0, self.pg1, self.icmp_type, self.icmp_code)
        ...
        for packet in capture:
            try:
                self.assertEqual(packet[ICMP].code, self.icmp_type)
                self.assertEqual(packet[ICMP].type, self.icmp_code)
            except:
    ...
    def test_0005_vpp624_deny(self):
        ...
        pkts = self.create_stream_icmp(self.pg0, self.pg1, self.icmp_type + 2, self.icmp_code)
        self.pg0.add_stream(pkts)
        # Traffic should be denied by code
        pkts = self.create_stream_icmp(self.pg2, self.pg3, self.icmp_type, self.icmp_code + 2)
