...
class Punt:
    ...
    @staticmethod
    def type_encode(punt_type):
        if Punt.type_l4 == punt_type:
            return "l4"
        elif Punt.type_ip_proto == punt_type:
            return "ip_proto"
        elif Punt.type_exception == punt_type:
            return "exception"
        else:
    ...
    def dump_reason(cls, reason=None):
        if reason:
            return cls.vapi.punt_reason_dump(reason=reason)
        return cls.vapi.punt_reason_dump()


class BasePunt(object):
    ...
    @property
    def type_en(self):
        return Punt.type_encode(self.type)

    def encode(self):
        return {"type": self._type, "punt": {self.type_en: {}}}
...
class PuntIpProto(BasePunt):
    ...
    def encode(self):
        ...
        ret["punt"][self.type_en] = {"af": self._af, "protocol": self._proto}
        return ret
...
class PuntL4(PuntIpProto):
    ...
    def encode(self):
        ...
        ret["punt"][self.type_en]["port"] = self._port
        return ret
...
class PuntException(BasePunt):
    ...
    def encode(self):
        ...
        ret["punt"][self.type_en] = {"id": self._exception_id}
        return ret
