...
class VppIperf:
    ...
    def __init__(self, server_ns=None, client_ns=None, server_ip=None, logger=None):
        ...
        self.timeout_padding = 20
        self.client_retries = 3
        self.client_retry_delay = 2
        self.client_args = ""
        self.server_args = ""
        self.logger = logger
    ...
    def start_iperf_server(self):
        ...
        try:
            result = subprocess.run(
                args,
                timeout=self.duration + self.timeout_padding,
                capture_output=True,
                encoding="utf-8",
            )
    ...
    def start_iperf_client(self):
        ...
        # Avoid long kernel connect stalls so retry logic can recover races.
        ...
        for attempt in range(1, self.client_retries + 1):
            try:
                ...
                if transient_error and attempt < self.client_retries:
                    self.logger.warning(
                        "iPerf client connect race detected (attempt %s/%s): %s",
                        attempt,
                        self.client_retries,
                        cmd,
                    )
                    VppAsfTestCase.sleep(self.client_retry_delay)
                    continue
                return result
            except subprocess.TimeoutExpired as e:
                if attempt < self.client_retries:
                    self.logger.warning(
                        "iPerf client timeout (attempt %s/%s): %s",
                        attempt,
                        self.client_retries,
                        cmd,
                    )
