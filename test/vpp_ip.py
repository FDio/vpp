    ...
    def __eq__(self, other):
        if (isinstance(other, self.__class__)):
            ...
            if 4 is self.version:
                if self.ip_addr.packed == other.ip4.address:
                    return True
            else:
                if self.ip_addr.packed == other.ip6.address:
                    return True
        return False
...
class VppIpPrefix():
    ...
    def __eq__(self, other):
        ...
        elif (hasattr(other, "address") and
              hasattr(other, "address_length")):
            # vl_api_prefix_t
            return (self.len == other.address_length and
                    self.addr == other.address)
        else:
            print "Comparing VppIpPrefix with unknown type"
        return False
