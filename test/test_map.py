...
class TestMAPTMultiTenant(VppTestCase):
    ...
    Two tenants share a fixed /128 IPv6 destination (DMR) and identify
    themselves via distinct /64 MAP source prefixes (BMRs). Configuration
    ...
    Exercises:
      * overlapping-v4 LPM match keyed on the interface's ingress
        ...
        address;
      * forward-path VRF injection via vnet_buffer(b)->sw_if_index[VLIB_TX]
        set to the tenant's underlay fib_index — the translated v6 packet
        resolves at ip6-lookup in that VRF with the shared DMR as
        destination and the tenant BMR as source;
      * reverse-path domain lookup keyed on v6 destination against
        ip6_src_prefix_tbl — the reply v6 source is the shared DMR
        (same for every tenant), so identification MUST come from the
        v6 dst. ip6_prefix_tbl is deliberately NOT populated for
        dst-keyed domains to avoid the DMR collision across tenants;
    ...
    def test_map_t_multitenant_shared_dmr(self):
        """MAP-T: shared /128 DMR across tenants, distinct per-tenant BMR."""

        # Headend v4 is the same for both tenants — both tenants use
        # the same inside endpoint address.
        headend_v4 = "192.0.2.1/32"

        # Shared /128 DMR — the v6 address the headend listens on.
        # Every tenant's forward traffic lands here; the reverse reply
        # originates here. Identical across all tenants.
        shared_dmr = "2001:db8::cafe/128"

        # Per-tenant /64 source prefix (BMR). Each tenant gets a
        # distinct v6 source prefix; the customer v4 is written at
        # RFC-6052 offset 9 inside that prefix.
        tenant_a_bmr = "2001:db8:a::/64"
        tenant_b_bmr = "2001:db8:b::/64"
    ...
    def test_map_t_multitenant_icmp_echo_reply(self):
        """MAP-T 1:1: ICMP echo-request translates across the DMR.

        Covers ip6_to_ip4_set_icmp_cb / ip6_to_ip4_set_inner_icmp_cb in
        the 1:1 (fixed /128 DMR, ea_bits_len=0) case. The v6 source is
        the opaque /128 DMR with no embedded v4, so the v4 source must
        come from d->ip4_prefix and the DMR-encoded check is skipped;
        without that the callback drops the packet with MAP_ERROR_ICMP.
        """
        ...
        # v6 ICMP echo-request from headend → translated to v4 echo-request.
        p6 = (
            Ether(dst=self.pg1.local_mac, src=self.pg1.remote_mac)
            / IPv6(
                src="2001:db8::cafe",
    ...
    def test_map_t_multitenant_icmp_fragmented(self):
        """MAP-T: oversized ICMP via ip4-frag still honours the per-VRF
        FIB override.

        sw_if_index[VLIB_TX] = d->ip4_fib_index must be set on the
        fragmentation branch of ip6_map_t_icmp as well as the fast
        path. Without it, a reverse-path ICMP packet larger than MTU
        translates correctly but gets fragmented in the default FIB
        and misses the tenant VRF.

        We can't observe the fragmentation routing directly from pg;
        we assert that an oversized echo-request reaches the customer
        pg interface, which requires the frag path's lookup to happen
        in the tenant FIB where the customer route exists.
        """
