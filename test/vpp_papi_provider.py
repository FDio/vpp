...
class VppPapiProvider(object):
    ...
    def connect(self):
        ...
        vpp_papi.connect("unittest", chroot_prefix="unittest")
    ...
    def api(self, api_fn, api_args, expected_retval=0):
        ...
        @return: reply from the API

        """
        self.hook.before_api(api_fn.__name__, api_args)
        reply = api_fn(*api_args)
        if hasattr(reply, 'retval') and reply.retval != expected_retval:
            msg = "API call failed, expected retval == 0, got %s" % repr(reply)
            error(msg)
            raise Exception(msg)
    ...
    def vxlan_add_del_tunnel(self, src_addr, dst_addr, is_add=1, is_ipv6=0,
            encap_vrf_id=0, decap_next_index=0xFFFFFFFF, vni=0):
        """
    ...
    def create_subif(self, sw_if_index, sub_id, outer_vlan, inner_vlan,
        ...
        return self.api(vpp_papi.create_subif,
                (sw_if_index, sub_id, no_tags, one_tag, two_tags, dot1ad,
                    exact_match, default_sub, outer_vlan_id_any,
                    inner_vlan_id_any, outer_vlan_id, inner_vlan_id))
    ...
    def ip_add_del_route(self, dst_address, dst_address_length,
            next_hop_address, next_hop_sw_if_index=0xFFFFFFFF, vrf_id=0,
            lookup_in_vrf=0, resolve_attempts=0,
            classify_table_index=0xFFFFFFFF, create_vrf_if_needed=0,
            resolve_if_needed=0, is_add=1, is_drop=0, is_ipv6=0, is_local=0,
            is_classify=0, is_multipath=0, is_resolve_host=0,
            is_resolve_attached=0, not_last=0, next_hop_weight=1):
        """
        ...
        """
        return self.api(vpp_papi.ip_add_del_route,
                (next_hop_sw_if_index, vrf_id, lookup_in_vrf, resolve_attempts,
                    classify_table_index, create_vrf_if_needed,
                    resolve_if_needed, is_add, is_drop, is_ipv6, is_local,
                    is_classify, is_multipath, is_resolve_host,
                    is_resolve_attached, not_last, next_hop_weight,
                    dst_address_length, dst_address, next_hop_address))
