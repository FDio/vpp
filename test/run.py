...
def vm_test_runner(test_name, kernel_image, test_data_dir,
    ...
    p = Popen([script, test_name, kernel_image,
               test_data_dir, cpu_mask, ram],
...
def set_logging(test_data_dir, test_name):
    ...
    log_file = "vm_{0}_{1}.log".format(test_name,
                                       str(time.time())[-5:])
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Run VPP Unit Tests",
    ...
    parser.add_argument('--kernel-image', dest='kernel_image',
                        ...
                        help='Select a Kernel Image to boot on KVM')
    parser.add_argument('--cpu-list', dest='cpu_list',
                        required=False, action='store', default='5-8',
                        help="Set CPU Affinity\n"
                             "E.g. 5-7,10 will schedule on processors "
                             "#5, #6, #7 and #10. (Default: 5-8)")
    parser.add_argument('--ram', dest='ram',
                        required=False, action='store', default='2',
                        help="Guest RAM in Gibibytes\n"
                             "E.g. 4 (Default: 2)")
    ...
    if vm_tests:
        print("Running VPP unit test(s):{0} inside a QEMU VM".format(
              args.test_name))
        # Check available CPUs/RAM
        cpus = expand_mix_string(args.cpu_list)
        num_cpus, usable_cpus = (len(cpus.split(',')),
                                 len(os.sched_getaffinity(0)))
        if num_cpus > usable_cpus:
            print(f"Error: #CPUs:{num_cpus} > Avail #CPUs:{usable_cpus}")
            sys.exit(1)
        avail_ram = int(os.popen('free -t -g').readlines()[-1].split()[-1])
        if int(args.ram) > avail_ram:
            print(f"Error: RAM size:{args.ram}G > Avail RAM:{avail_ram}G")
            sys.exit(1)
        vm_test_runner(args.test_name,
                       args.kernel_image,
                       test_data_dir,
                       cpus,
                       f"{args.ram}G"
