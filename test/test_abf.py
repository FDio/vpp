...
class TestAbf(VppTestCase):
    ...
    def test_abf4_deny(self):
        ...
        # Rules 1/2
        #
        pg0_subnet = ipaddress.ip_network(
                self.pg0.local_ip4_prefix, strict=False
        )
        pg2_subnet = ipaddress.ip_network(
                self.pg2.local_ip4_prefix, strict=False
        )
        pg3_subnet = ipaddress.ip_network(
                self.pg3.local_ip4_prefix, strict=False
        )
        rule_deny = AclRule(
            is_permit=0,
            proto=17,
    ...
    def test_abf6_deny(self):
        ...
        #
        pg0_subnet = ipaddress.ip_network(
                self.pg0.local_ip6_prefix, strict=False
        )
        pg2_subnet = ipaddress.ip_network(
                self.pg2.local_ip6_prefix, strict=False
        )
        pg3_subnet = ipaddress.ip_network(
                self.pg3.local_ip6_prefix, strict=False
        )
        rule_deny = AclRule(
            is_permit=0,
            proto=17,
