...
class TestIP6DADAutoRemove(VppTestCase):
    ...
    extra_vpp_plugin_config = [
        "plugin ip6_dad_autoremove_plugin.so { enable }",
    ]
    ...
    def setUp(self):
        ...
        # Restore the plugin's default runtime state for each test.
        try:
    ...
    def test_autoremove_enabled_by_default(self):
        """Auto-remove should be enabled by default once the plugin is loaded"""

        status = self.vapi.cli("show ip6 dad autoremove")
        self.assertIn("Enabled: yes", status)
        self.logger.info(f"Status after plugin load: {status}")
    ...
    def test_autoremove_enables_dad(self):
        """Auto-remove should automatically enable DAD when loaded"""

        # Ensure DAD is disabled initially
        self.vapi.ip6_dad_enable_disable(enable=False)

        # Re-enable auto-remove after forcing DAD off
        self.vapi.cli("set ip6 dad autoremove disable")
        self.vapi.cli("set ip6 dad autoremove enable")
