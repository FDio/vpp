...
class QUICEchoExternalTestCase(QUICTestCase):
    ...
    def _kill_worker(self, worker):
    	try:
            self.logger.debug("Killing worker process (pid %d)" % worker.process.pid)
            os.killpg(os.getpgid(worker.process.pid), signal.SIGKILL)
            worker.join(self.timeout)
        except OSError as e:
            self.logger.debug("Couldn't kill worker process")
            return True
        return False
    ...
    def validate_external_test_results(self):
        ...
        if self.worker_server.result is None:
            server_kill_error = self._kill_worker(self.worker_server)
        if self.worker_client.result is None:
            self._kill_worker(self.worker_client)
...
class QUICEchoExternalServerStreamTestCase(QUICEchoExternalTestCase):
    ...
    def test_quic_external_transfer_server_stream(self):
        self.server("mbytes", "1", "quic-setup", "serverstream", "nclients 1/1")
	self.client("mbytes", "1", "quic-setup", "serverstream", "nclients 1/1")
        self.validate_external_test_results()
...
class QUICEchoExternalServerStreamWorkersTestCase(QUICEchoExternalTestCase):
	...
	self.server("nclients", "1/3", "mbytes", "1", "quic-setup", "serverstream")
        self.client("nclients", "1/3", "mbytes", "1", "quic-setup", "serverstream")
        self.validate_external_test_results()
