...
class TestDns(VppTestCase):
    ...
    def create_stream_req(self, src_if):
        ...
        good_request_dynamic = (
                    Ether(dst=src_if.local_mac, src=src_if.remote_mac) /
                    IP(src=src_if.remote_ip4, dst=src_if.local_ip4) /
                    UDP(sport=1234, dport=53) /
                    DNS(rd=1, qd=DNSQR(qname="no.clown.org")))
        pkts = [good_request_static,
                good_request_AAAA_static,
                good_request_dynamic]
        return pkts

    def create_stream_res(self, src_if, dst_if):
        """Response from DNS server to VPP. """
        answer = (
            Ether(dst=src_if.local_mac, src=src_if.remote_mac) /
            IP(src=src_if.remote_ip4, dst=src_if.local_ip4) /
            ...
            DNS(id=1, qr=1, ra=1, rd=1,
                ...
                an=DNSRR(rrname="no.clown.org", ttl=600, rdata="4.3.2.1")))
        pkts = [answer]
        return pkts
    ...
    def test_dns_unittest(self):
        ...
        res_pkts = self.create_stream_res(self.pg1, self.pg0)
        self.pg1.add_stream(res_pkts)
        self.pg_enable_capture(self.pg_interfaces)

        self.pg_start()
        pkts = self.pg0.get_capture(3)
        res_pkts = self.pg1.get_capture(1)
