...
class LDPNonblockConnectPoll(VCLTestCase):
    """LDP nonblock connect() poll does not spawn fresh VPP sessions"""

    def setUp(self):
        super(LDPNonblockConnectPoll, self).setUp()
        self.thru_host_stack_setup()

    def tearDown(self):
        self.thru_host_stack_tear_down()
        super(LDPNonblockConnectPoll, self).tearDown()

    def test_ldp_nonblock_connect_poll_no_session_storm(self):
        """run ldp nonblocking connect() poll test"""
        self.vcl_app_env = {"VCL_APP_SCOPE_GLOBAL": "true"}
        self.update_vcl_app_env("2", "5678", self.sapi_client_sock)
        client = VCLAppWorker(
            "sock_test_nonblock_connect",
            [],
            self.logger,
            self.vcl_app_env,
            "client",
