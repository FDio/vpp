...
class CPUInterface(ABC):
    # Per-suite declarations live on the test classes as plain class
    # attributes (vpp_worker_count = N, helper_count = N). They are
    # deliberately NOT given defaults here: a suite frequently declares
    # them on a shared mixin/base that lands *after* CPUInterface in the
    # MRO, and a default value on CPUInterface would shadow it. Read
    # them through getattr() so an undeclared suite simply means zero.
    ...
    def get_vpp_worker_count(cls):
        return getattr(cls, "vpp_worker_count", 0)
...
class VppAsfTestCase(CPUInterface, unittest.TestCase):
    ...
    @classmethod
    def get_vpp_worker_count(cls):
        # A declaration anywhere in the suite's class hierarchy
        # (vpp_worker_count = N) wins - it is frequently set on a shared
        # mixin/base rather than the leaf test class, so use attribute
        # lookup, which walks the whole MRO. CPUInterface deliberately
        # carries no default, so hasattr is true only for a real
        # declaration. Otherwise take the global config default, unless
        # the test is tagged as incompatible with workers.
        if hasattr(cls, "vpp_worker_count"):
