...
class VppAsfTestCase(CPUInterface, unittest.TestCase):
    ...
    def quit(cls):
        ...
        if thread_join_timeout:
            ...
            cls.logger.info(f"Using thread join timeout: {thread_join_timeout}s from VPP_TEARDOWN_TIMEOUT")
        else:
        ...
        if hasattr(cls, "pump_thread"):
            ...
            if thread_join_timeout and cls.pump_thread.is_alive():
                cls.logger.error(f"[{cls.__name__}] PUMP THREAD TIMEOUT after {thread_join_timeout}s - thread did not stop!")
        if hasattr(cls, "vpp_stderr_reader_thread"):
            ...
            if thread_join_timeout and cls.vpp_stderr_reader_thread.is_alive():
                cls.logger.error(f"[{cls.__name__}] STDERR PUMP THREAD TIMEOUT after {thread_join_timeout}s - thread did not stop!")
    ...
    def tearDownClass(cls):
        ...
        print(f"\n{'='*80}", flush=True)
        print(f"tearDownClass STARTING: {cls.__name__}", flush=True)
        print(f"{'='*80}\n", flush=True)
        ...
        print(f"\n{'='*80}", flush=True)
        print(f"tearDownClass COMPLETED: {cls.__name__}", flush=True)
        print(f"{'='*80}\n", flush=True)
