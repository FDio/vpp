...
class TestHttpStaticVapi(VppAsfTestCase):
    ...
    def setUpClass(cls):
        ...
        # cls.vpp_if_name = f"vpp_out_{''.join(random.choices(string.digits, k=4))}"

        cls.host_if_name = create_host_interface("vppOut1", cls.ns_name, "10.10.1.1/24")

        cls.vapi.cli(f"create host-interface name vppOut1")
        cls.vapi.cli(f"set int state host-vppOut1 up")
        cls.vapi.cli(f"set int ip address host-vppOut1 10.10.1.2/24")
...
class TestHttpStaticCli(VppAsfTestCase):
    ...
    def setUpClass(cls):
        ...
        create_host_interface("vppOut2", cls.ns_name, "10.10.1.1/24")
