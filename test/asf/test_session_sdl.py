...
class TestSessionSDL(VppTestCase):
    ...
    def create_rule(self, lcl, action_index):
        return SessionSdl(lcl=lcl, action_index=action_index)
    ...
    def test_session_sdl_ip4(self):
        ...
        rules = []
        rules.append(self.create_rule(lcl=self.loop0.local_ip4 + "/32", action_index=0))

        # irrelevant rules - add 64k entries in one API call
        for i in range(255):
            for j in range(255):
                prefix = "10.1.{0}.{1}/32".format(i, j)
                rules.append(self.create_rule(lcl=prefix, action_index=0))
        ...
        rules = []
        rules.append(self.create_rule(lcl="172.100.1.0/24", action_index=0))
        self.apply_rules(rules, is_add=1, appns_index=0)
    ...
    def test_session_sdl_ip6(self):
        ...
        rules = []
        rules.append(
            self.create_rule(lcl=self.loop0.local_ip6 + "/128", action_index=0)
        )
        self.apply_rules(rules, is_add=1, appns_index=0)
