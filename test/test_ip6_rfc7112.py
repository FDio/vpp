...
class TestRFC7112(VppTestCase):
    ...
    def test_rfc7112_udp_incomplete_header_dropped(self):
        ...
        # Manually construct packet with incomplete UDP header
        # IPv6 + Fragment header + only 6 bytes (less than 8 needed for UDP)
        pkt = (Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac) /
               IPv6(src=self.pg0.remote_ip6, dst=self.pg1.remote_ip6, nh=44) /
               Raw(b'\x11\x00\x00\x01\x00\x00\x03\xe9' +
                   # Frag: nh=17 UDP, M=1, id=1001
                   b'\x04\xd2\x16\x2e\x00\x6c'))  # 6 bytes (incomplete UDP)

        # Fix IPv6 payload length: 8 (fragment header) + 6 (data) = 14
        pkt_bytes = bytearray(raw(pkt))
        pkt_bytes[18:20] = b'\x00\x0e'
        pkt = Ether(raw(bytes(pkt_bytes)))
        ...
        try:
            rx = self.pg1.get_capture(0, timeout=2)
    ...
    def test_rfc7112_tcp_incomplete_header_dropped(self):
        ...
        # TCP minimum is 20 bytes, only provide 10
        pkt = (Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac) /
               IPv6(src=self.pg0.remote_ip6, dst=self.pg1.remote_ip6, nh=44) /
               Raw(b'\x06\x00\x00\x01\x00\x00\x03\xea' +  # Frag: nh=6 TCP, M=1, id=1002
                   b'\x04\xd2\x00\x50\x00\x00\x03\xe8\x00\x00\x00\x00'))  # 10 bytes

        pkt_bytes = bytearray(raw(pkt))
        pkt_bytes[18:20] = b'\x00\x12'  # payload_length = 18
        pkt = Ether(raw(bytes(pkt_bytes)))
    ...
    def test_rfc7112_icmpv6_complete_header_accepted(self):
        ...
        # ICMPv6 header minimum is 8 bytes: type(1) + code(1) + checksum(2) + data(4)
        # Build manually to ensure exact structure
        pkt = (Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac) /
               IPv6(src=self.pg0.remote_ip6, dst=self.pg1.remote_ip6, nh=44) /
               Raw(b'\x3a\x00\x00\x01\x00\x00\x03\xeb' +  # Frag: nh=58 ICMPv6, M=1
                   b'\x80\x00\x00\x00\x00\x00\x00\x00' +  # ICMPv6 Echo (8 bytes)
                   b'CCCCCCCC'))  # Payload

        pkt_bytes = bytearray(raw(pkt))
        pkt_bytes[18:20] = b'\x00\x18'  # payload_length = 24
        pkt = Ether(raw(bytes(pkt_bytes)))
    ...
    def test_rfc7112_non_first_fragment_accepted(self):
        ...
        pkt = (Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac) /
               IPv6(src=self.pg0.remote_ip6, dst=self.pg1.remote_ip6) /
               IPv6ExtHdrFragment(offset=1, m=0, id=1003, nh=17) /
               # offset=1, not first
               Raw(b"D" * 100))
    ...
    def test_rfc7112_atomic_fragment_complete_accepted(self):
        ...
        pkt = (Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac) /
               IPv6(src=self.pg0.remote_ip6, dst=self.pg1.remote_ip6) /
               IPv6ExtHdrFragment(offset=0, m=0, id=1004, nh=17) /
               # Atomic: offset=0, M=0
    ...
    # NOTE: Error counter test commented out - needs VPP statistics API verification
    # TODO: Add test for IP6_ERROR_REASS_MISSING_UPPER counter
    pass
