...
# Constants matching VPP session_stats.h
SFDP_SESSION_STATS_ALL_TENANTS = 0xFFFFFFFF  # max u32 value for "all tenants"
SFDP_SESSION_STATS_ALL_CONTEXTS = 0xFFFFFFFF  # max u32 value for "all contexts"
SFDP_SESSION_STATS_CUSTOM_API_DATA_INVALID = 0xFFFFFFFFFFFFFFFF  # max u64 value
...
class TestSfdpSessionStats(VppTestCase):
    ...
    def _configure_sfdp_session_stats(
        ...
        Args:
            ...
            enable_custom_data: Enable custom data (default: False)
            custom_api_data: Custom API data when enabled (default: 0)
            custom_api_context_id: Context ID for custom API data (default: None, uses context_id 1)
        ...
        if enable_custom_data:
            ...
            self.assertEqual(reply.retval, 0)
            if custom_api_data:
                # Use provided context_id or default to 1
                context_id = (
                    custom_api_context_id if custom_api_context_id is not None else 1
                )
                reply = self.vapi.sfdp_session_stats_set_custom_api_data(
                    tenant_id=tenant_id, context_id=context_id, value=custom_api_data
                )
    ...
    def test_session_stats_custom_data_config(self):
        ...
        # Set custom API data for tenant_id 1, context_id 1
        test_data = 0x123456789ABCDEF0
        tenant_id = 1
        context_id = 1
        reply = self.vapi.sfdp_session_stats_set_custom_api_data(
            tenant_id=tenant_id, context_id=context_id, value=test_data
        )
        self.assertEqual(reply.retval, 0)

        # Verify API data is set for tenant 1, context 1 using get_tenant_custom_data
        reply = self.vapi.sfdp_session_stats_get_tenant_custom_data(
        ...
        self.assertEqual(reply.context_id, context_id, "Context ID should match")
        ...
        # Clear custom API data for tenant 1, context 1
        reply = self.vapi.sfdp_session_stats_clear_custom_api_data(
            tenant_id=tenant_id, context_id=context_id
        )
        self.assertEqual(reply.retval, 0)
    ...
    def test_session_stats_read_ring_buffer_directly(self):
        ...
        self.assertEqual(schema["version"], 5, "Schema version should be 4")
        ...
        # Validate duration field (schema v4)
        self.assertGreaterEqual(entry["duration"], 0, "Duration should be >= 0")

        # Validate TTL statistics (schema v4)
        ...
        # Validate RTT statistics (schema v4)
        ...
        # Validate TCP-specific fields (schema v4)
        ...
        # Validate custom data flags (should be 0/disabled by default)
        self.assertEqual(
            entry.get("custom_data_flags", 0),
            0,
            "Custom data flags should be 0 by default",
        )
    ...
    def test_session_stats_ring_buffer_with_custom_data(self):
        ...
        tenant_data = self.vapi.sfdp_session_stats_get_tenant_custom_data(
            tenant_id=1, context_id=1
        )
        ...
        # Schema v5 offsets: custom_data_flags at 304, custom_api_data at 312
        entry_bytes = data[0]
        self.assertEqual(len(entry_bytes), 512, "Entry should be 512 bytes")

        # Extract custom data flags
        custom_data_flags = struct.unpack_from("<B", entry_bytes, 304)[0]

        print(f"Custom data flags from ring buffer: {custom_data_flags}")

        # Verify flag bit 0 is set (API data present)
        self.assertTrue(custom_data_flags & 0x01, "Custom data API flag should be set")

        # Extract custom API data (u64 at offset 312)
        custom_api_data = struct.unpack_from("<Q", entry_bytes, 312)[0]
        print(f"Custom API data from ring buffer: 0x{custom_api_data:016X}")

        self.assertEqual(
            custom_api_data, 0xDEADBEEFCAFEBABE, "Custom API data should match"
        )

        # Update custom API data for tenant_id 1, context 1
        reply = self.vapi.sfdp_session_stats_set_custom_api_data(
            tenant_id=1, context_id=1, value=0x1234567890ABCDEF
        )
        self.assertEqual(reply.retval, 0)
        ...
        # Clear custom API data for tenant_id 1, context 1 and verify it's reflected
        reply = self.vapi.sfdp_session_stats_clear_custom_api_data(
            tenant_id=1, context_id=1
        )
        self.assertEqual(reply.retval, 0)
    ...
    def test_session_stats_custom_api_data_values(self):
        """Test that custom API data set via API appears in ring buffer"""
        # Set custom API data for tenant_id 1, context 1 BEFORE enabling custom data
        test_value = 0xFEDCBA0987654321
        tenant_id = 1
        context_id = 1
        reply = self.vapi.sfdp_session_stats_set_custom_api_data(
            tenant_id=tenant_id, context_id=context_id, value=test_value
        )
        self.assertEqual(reply.retval, 0)
        ...
        # Verify custom data flags (bit 0 = API data present)
        custom_data_flags = struct.unpack_from("<B", entry_bytes, 304)[0]
        self.assertTrue(custom_data_flags & 0x01, "API data flag should be set")

        # Extract and verify custom API data (u64 at offset 312)
        custom_api_data = struct.unpack_from("<Q", entry_bytes, 312)[0]
        print(
            f"Custom API data: expected=0x{test_value:016X}, actual=0x{custom_api_data:016X}"
        )
        self.assertEqual(
            custom_api_data,
            test_value,
            f"Custom API data should be 0x{test_value:016X}, got 0x{custom_api_data:016X}",
        )

        # Update the API data value for tenant_id 1, context 1
        new_value = 0x1234567890ABCDEF
        reply = self.vapi.sfdp_session_stats_set_custom_api_data(
            tenant_id=tenant_id, context_id=context_id, value=new_value
        )
        self.assertEqual(reply.retval, 0)
        ...
        custom_api_data = struct.unpack_from("<Q", entry_bytes, 312)[0]
        print(
            f"Updated custom API data: expected=0x{new_value:016X}, actual=0x{custom_api_data:016X}"
        )
        self.assertEqual(
            custom_api_data,
            new_value,
            f"Custom API data should be updated to 0x{new_value:016X}",
        )

        # Cleanup
        self.vapi.sfdp_session_stats_custom_data_enable(enable=False)
        # Use ALL_TENANTS and ALL_CONTEXTS constants to clear all entries
        reply = self.vapi.sfdp_session_stats_clear_custom_api_data(
            tenant_id=SFDP_SESSION_STATS_ALL_TENANTS,
            context_id=SFDP_SESSION_STATS_ALL_CONTEXTS,
        )
        self._cleanup_sfdp_session_stats(config)

    def test_session_stats_custom_api_data_name_before_ring_init(self):
        """Test that custom API data name can be set before ring buffer initialization.

        This test verifies:
        1. Custom API data name can be set before ring buffer is enabled
        2. The name is included in the ring buffer schema when enabled
        3. The name can be retrieved via get API
        """
        custom_name = "TestCustomName"

        # Set custom API data name before ring buffer is enabled
        reply = self.vapi.sfdp_session_stats_set_custom_api_data_name(name=custom_name)
        self.assertEqual(
            reply.retval, 0, "Should be able to set name before ring buffer init"
        )

        # Verify the name was set correctly via get API
        reply = self.vapi.sfdp_session_stats_get_custom_api_data_name()
        self.assertEqual(reply.retval, 0)
        self.assertEqual(
            reply.name,
            custom_name,
            f"Name should be '{custom_name}', got '{reply.name}'",
        )

        # Now enable the ring buffer
        reply = self.vapi.sfdp_session_stats_ring_enable(enable=True, ring_size=256)
        self.assertEqual(reply.retval, 0)

        # Verify the ring buffer is enabled with the schema containing the custom name
        ring_buffer = self.statistics.get_ring_buffer("/sfdp/session/stats")
        schema_json, schema_size, schema_version = ring_buffer.get_schema_string()
        self.assertIsNotNone(schema_json, "Schema should be available")
        self.assertGreater(schema_size, 0, "Schema size should be > 0")

        # Parse schema and verify custom_api_data_name is present
        schema = json.loads(schema_json)
        self.assertIn(
            "custom_api_data_name",
            schema,
            "Schema should contain custom_api_data_name field",
        )
        self.assertEqual(
            schema["custom_api_data_name"],
            custom_name,
            f"Schema custom_api_data_name should be '{custom_name}'",
        )

        # Cleanup: disable ring buffer
        reply = self.vapi.sfdp_session_stats_ring_enable(enable=False)
        self.assertEqual(reply.retval, 0)

        # Clear the custom name for future tests
        reply = self.vapi.sfdp_session_stats_set_custom_api_data_name(name="")
        self.assertEqual(reply.retval, 0)

    def test_session_stats_custom_api_data_name_error_after_ring_init(self):
        """Test that setting custom API data name fails after ring buffer initialization.

        This test verifies:
        1. Ring buffer can be enabled without a custom name
        2. Trying to set the custom name after ring buffer is enabled returns an error
        3. After disabling the ring buffer, the name can be set again
        """
        # First enable the ring buffer without setting a custom name
        reply = self.vapi.sfdp_session_stats_ring_enable(enable=True, ring_size=256)
        self.assertEqual(reply.retval, 0)

        # Verify ring buffer is enabled
        config = self.vapi.sfdp_session_stats_get_config()
        self.assertTrue(config.ring_buffer_enabled, "Ring buffer should be enabled")

        # Now try to set a custom name - this should fail with an error
        with self.vapi.assert_negative_api_retval():
            self.vapi.sfdp_session_stats_set_custom_api_data_name(name="TooLate")

        # Disable the ring buffer
        reply = self.vapi.sfdp_session_stats_ring_enable(enable=False)
        self.assertEqual(reply.retval, 0)

        # Verify ring buffer is disabled
        config = self.vapi.sfdp_session_stats_get_config()
        self.assertFalse(config.ring_buffer_enabled, "Ring buffer should be disabled")

        # Now we should be able to set the custom name
        custom_name = "AfterDisable"
        reply = self.vapi.sfdp_session_stats_set_custom_api_data_name(name=custom_name)
        self.assertEqual(
            reply.retval, 0, "Should be able to set name after ring buffer is disabled"
        )
        ...
        # Re-enable ring buffer and verify schema contains the name
        reply = self.vapi.sfdp_session_stats_ring_enable(enable=True, ring_size=256)
        self.assertEqual(reply.retval, 0)

        ring_buffer = self.statistics.get_ring_buffer("/sfdp/session/stats")
        schema_json, _, _ = ring_buffer.get_schema_string()
        schema = json.loads(schema_json)
        self.assertEqual(
            schema.get("custom_api_data_name"),
            custom_name,
            f"Schema should contain custom_api_data_name='{custom_name}'",
        )
    ...
    def test_session_stats_tcp_ecn_cwr_metrics(self):
        """Test ECN (Explicit Congestion Notification) and CWR metrics tracking.

        This test verifies that the session statistics service correctly tracks:
        - ECN ECT packets: IP packets with ECN ECT(0) or ECT(1) marks
        - ECN CE packets: IP packets with ECN CE (Congestion Experienced) marks
        - TCP ECE packets: TCP packets with ECE (ECN-Echo) flag set
        - TCP CWR packets: TCP packets with CWR (Congestion Window Reduced) flag set

        ECN field in IP header (lower 2 bits of TOS/Traffic Class):
        - 00 (0): Not-ECT - Non ECN-Capable Transport
        - 01 (1): ECT(1) - ECN Capable Transport
        - 10 (2): ECT(0) - ECN Capable Transport
        - 11 (3): CE - Congestion Experienced
        """
        # Use the generic configuration function with ring buffer enabled
        config = self._configure_sfdp_session_stats(
            enable_bidirectional=True,
            enable_ring_buffer=True,
            ring_size=256,
        ...
        # === Phase 1: Normal TCP handshake without ECN ===
        print("\n=== Phase 1: Normal handshake (no ECN) ===")

        # SYN from client (no ECN)
        pkt_syn = (
            Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac)
        ...
        # SYN-ACK from server (no ECN)
        pkt_synack = (
            Ether(src=self.pg1.remote_mac, dst=self.pg1.local_mac)
            / IP(src=self.pg1.remote_ip4, dst=self.pg0.remote_ip4, ttl=64, tos=0)
            / TCP(sport=dport, dport=sport, flags="SA", seq=2000, ack=1001)
            / Raw(b"\xa5" * 10)
        )
        self.pg_send(self.pg1, pkt_synack)

        # ACK from client (no ECN)
        pkt_ack = (
            Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac)
            / IP(src=self.pg0.remote_ip4, dst=self.pg1.remote_ip4, ttl=64, tos=0)
            / TCP(sport=sport, dport=dport, flags="A", seq=1001, ack=2001)
        ...
        # === Phase 2: Send ECN ECT(0) packets ===
        print("\n=== Phase 2: ECN ECT(0) packets ===")

        # ECT(0) is indicated by tos=2 (binary 10 in lower 2 bits)
        for i in range(3):
            pkt = (
                Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac)
                / IP(src=self.pg0.remote_ip4, dst=self.pg1.remote_ip4, ttl=64, tos=2)
                / TCP(sport=sport, dport=dport, flags="A", seq=1002 + i * 100)
                / Raw(b"\xa5" * 100)
            )
            self.pg_send(self.pg0, pkt)
            expected_ecn_ect += 1

        # === Phase 3: Send ECN ECT(1) packets ===
        print("\n=== Phase 3: ECN ECT(1) packets ===")

        # ECT(1) is indicated by tos=1 (binary 01 in lower 2 bits)
        for i in range(2):
            pkt = (
                Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac)
                / IP(src=self.pg0.remote_ip4, dst=self.pg1.remote_ip4, ttl=64, tos=1)
                / TCP(sport=sport, dport=dport, flags="A", seq=1302 + i * 100)
                / Raw(b"\xa5" * 100)
            )
            self.pg_send(self.pg0, pkt)
            expected_ecn_ect += 1

        # === Phase 4: Send ECN CE (Congestion Experienced) packets ===
        print("\n=== Phase 4: ECN CE packets ===")

        # CE is indicated by tos=3 (binary 11 in lower 2 bits)
        for i in range(4):
            pkt = (
                Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac)
                / IP(src=self.pg0.remote_ip4, dst=self.pg1.remote_ip4, ttl=64, tos=3)
                / TCP(sport=sport, dport=dport, flags="A", seq=1502 + i * 100)
                / Raw(b"\xa5" * 100)
            )
            self.pg_send(self.pg0, pkt)
            expected_ecn_ce += 1

        # === Phase 5: Send TCP packets with ECE flag ===
        print("\n=== Phase 5: TCP ECE flag packets ===")

        # TCP ECE flag is 'E' in scapy
        for i in range(3):
            pkt = (
                Ether(src=self.pg1.remote_mac, dst=self.pg1.local_mac)
                / IP(src=self.pg1.remote_ip4, dst=self.pg0.remote_ip4, ttl=64, tos=0)
                / TCP(
                    sport=dport, dport=sport, flags="AE", seq=2001 + i * 100, ack=1902
                )
                / Raw(b"\xa5" * 100)
            )
            self.pg_send(self.pg1, pkt)
        ...
        # === Phase 6: Send TCP packets with CWR flag ===
        print("\n=== Phase 6: TCP CWR flag packets ===")

        # TCP CWR flag is 'C' in scapy
        for i in range(2):
            pkt = (
                Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac)
                / IP(src=self.pg0.remote_ip4, dst=self.pg1.remote_ip4, ttl=64, tos=0)
                / TCP(sport=sport, dport=dport, flags="AC", seq=1902 + i * 100)
                / Raw(b"\xa5" * 100)
            )
            self.pg_send(self.pg0, pkt)
            expected_cwr += 1

        # === Verify via Ring Buffer ===
        print("\n=== Verifying ECN/CWR metrics via ring buffer ===")
        ...
        # Decode the entry
        entry = decode_entry(data[0])
        print(f"Ring buffer entry: {entry}")

        # Verify ECN/CWR metrics
        print(f"\n=== ECN/CWR Verification ===")
        print(f"Expected ECN ECT packets: {expected_ecn_ect}")
        print(f"Actual tcp_ecn_ect_packets: {entry.get('tcp_ecn_ect_packets', 'N/A')}")
        print(f"Expected ECN CE packets: {expected_ecn_ce}")
        print(f"Actual tcp_ecn_ce_packets: {entry.get('tcp_ecn_ce_packets', 'N/A')}")
        print(f"Expected TCP ECE packets: {expected_ece}")
        print(f"Actual tcp_ece_packets: {entry.get('tcp_ece_packets', 'N/A')}")
        print(f"Expected TCP CWR packets: {expected_cwr}")
        print(f"Actual tcp_cwr_packets: {entry.get('tcp_cwr_packets', 'N/A')}")

        # Assert ECN ECT packets (ECT(0) and ECT(1) combined)
        self.assertEqual(
            entry.get("tcp_ecn_ect_packets", 0),
            expected_ecn_ect,
            f"ECN ECT packet count mismatch: expected {expected_ecn_ect}, got {entry.get('tcp_ecn_ect_packets', 0)}",
        )

        # Assert ECN CE packets
        self.assertEqual(
            entry.get("tcp_ecn_ce_packets", 0),
            expected_ecn_ce,
            f"ECN CE packet count mismatch: expected {expected_ecn_ce}, got {entry.get('tcp_ecn_ce_packets', 0)}",
        )

        # Assert TCP ECE packets
        self.assertEqual(
            entry.get("tcp_ece_packets", 0),
            expected_ece,
            f"TCP ECE packet count mismatch: expected {expected_ece}, got {entry.get('tcp_ece_packets', 0)}",
        )

        # Assert TCP CWR packets
        self.assertEqual(
            entry.get("tcp_cwr_packets", 0),
            expected_cwr,
            f"TCP CWR packet count mismatch: expected {expected_cwr}, got {entry.get('tcp_cwr_packets', 0)}",
        )

        print("\n=== All ECN/CWR metrics verified successfully! ===")

        # Cleanup
        self._cleanup_sfdp_session_stats(config)

    def test_session_stats_api_dump_ecn_cwr_metrics(self):
        """Test ECN/CWR metrics are correctly returned via the API dump.

        This test verifies that the sfdp_session_stats_dump API correctly returns
        ECN/CWR metrics without requiring the ring buffer:
        - tcp_ecn_ect_packets: IP packets with ECN ECT(0) or ECT(1) marks
        - tcp_ecn_ce_packets: IP packets with ECN CE (Congestion Experienced) marks
        - tcp_ece_packets: TCP packets with ECE (ECN-Echo) flag set
        - tcp_cwr_packets: TCP packets with CWR (Congestion Window Reduced) flag set
        """
        # Configure SFDP with session stats but WITHOUT ring buffer
        config = self._configure_sfdp_session_stats(
            enable_bidirectional=True,
            enable_ring_buffer=False,  # Not using ring buffer - testing API dump
        )
        ...
        # Verify ECN ECT packets
        self.assertEqual(
            session_stats.tcp_ecn_ect_packets,
            expected_ecn_ect,
            f"ECN ECT packet count mismatch: expected {expected_ecn_ect}, "
            f"got {session_stats.tcp_ecn_ect_packets}",
        )

        # Verify ECN CE packets
        self.assertEqual(
            session_stats.tcp_ecn_ce_packets,
            expected_ecn_ce,
            f"ECN CE packet count mismatch: expected {expected_ecn_ce}, "
            f"got {session_stats.tcp_ecn_ce_packets}",
        )

        # Verify TCP ECE packets
        self.assertEqual(
            session_stats.tcp_ece_packets,
            expected_ece,
            f"TCP ECE packet count mismatch: expected {expected_ece}, "
            f"got {session_stats.tcp_ece_packets}",
        )

        # Verify TCP CWR packets
        self.assertEqual(
            session_stats.tcp_cwr_packets,
            expected_cwr,
            f"TCP CWR packet count mismatch: expected {expected_cwr}, "
            f"got {session_stats.tcp_cwr_packets}",
        )
