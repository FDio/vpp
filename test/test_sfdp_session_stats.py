...
class TestSfdpSessionStats(VppTestCase):
    ...
    def test_session_stats_custom_data_config(self):
        ...
        reply = self.vapi.sfdp_session_stats_clear_tenant_custom_data(tenant_id=tenant_id)
        self.assertEqual(reply.retval, 0)
    ...
    def test_session_stats_ring_buffer_with_custom_data(self):
        ...
        tenant_custom_data = decoded["opaque"]
        self.assertEqual(
            tenant_custom_data, 0x1234567890ABCDEF, "Tenant custom data should be updated"
        )
        ...
        if len(data) > 0:
            ...
            decoded = decode_entry(entry_bytes)
            tenant_custom_data = decoded["opaque"]
            self.assertEqual(tenant_custom_data, 0, "Opaque data should default to zero")
