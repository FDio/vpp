...
class SfdpSessionStatsTestCase(BaseSfdpTest):
    """Base class for SFDP session stats tests"""
        ...
        for direction in [
            ...
            reply = self.vapi.sfdp_set_services(
                ...
                services=[
                    ...
                    {"data": "ip6-lookup" if is_ip6 else "ip4-lookup"},
                ],
            )
            self.assertEqual(reply.retval, 0)
    ...
    def _cleanup_sfdp_session_stats(self, config=None):
        ...
        is_ip6 = config.get("is_ip6", False)
...
class TestSfdpSessionStats(SfdpSessionStatsTestCase):
    """IPv4 SFDP Session Stats Service tests"""
    ...
    def test_session_stats_configuration_api(self):
        ...
        # Negative Test
        # Verify enabling ring buffer with ring_size == 0 fails
        with self.vapi.assert_negative_api_retval():
            reply = self.vapi.sfdp_session_stats_ring_enable(enable=True, ring_size=0)
    ...
    def test_session_stats_ring_buffer_multiple_sessions(self):
        ...
        for entry_bytes in data:
            ...
            parsed_entries.append(
                {
                    "dst_port": dst_port,
                    "packets": packets_forward,
                    "entry": decoded,
                }
            )

        # Verify each session information is set properly
        # (packet count, ips, ports)
        for sess in sessions_info:
            found = False
            for parsed in parsed_entries:
                if parsed["dst_port"] == sess["dport"]:
                    ...
                    entry = parsed["entry"]
                    self.assertTrue(entry["is_ip4"])
                    self.assertEqual(
                        self.format_ip_from_bytes(entry["src_ip"], True),
                        sess["src_ip"],
                    )
                    self.assertEqual(
                        self.format_ip_from_bytes(entry["dst_ip"], True),
                        sess["dst_ip"],
                    )
                    self.assertEqual(entry["src_port"], sess["sport"])
                    self.assertEqual(entry["dst_port"], sess["dport"])
                    self.assertEqual(entry["packets_reverse"], 0)
                    found = True
                    break
            self.assertTrue(
...
class TestSfdpSessionStatsIP6(SfdpSessionStatsTestCase):
    """IPv6 SFDP Session Stats Service tests"""

    is_ip6 = True

    def test_session_stats_ipv6_tcp(self):
        """Test IPv6 TCP session statistics"""
        config = self._configure_sfdp_session_stats()
        packet = self.create_tcp6_packet(
            src_mac=self.pg0.remote_mac,
            dst_mac=self.pg0.local_mac,
        ...
        self.assertEqual(len(sessions), 1, "Should have exactly one SFDP session")
        self.assertEqual(
            sessions[0].session_type,
            VppEnum.vl_api_sfdp_session_type_t.SFDP_API_SESSION_TYPE_IP6,
            "Session should be IPv6",
        )

        stats = self.vapi.sfdp_session_stats_dump()
        self.assertEqual(len(stats), 1)
        self.assertEqual(stats[0].packets_fwd, 1)
        self.assertEqual(stats[0].tcp_syn_packets, 1)
        self.assertEqual(stats[0].tcp_last_seq_fwd, 1100)
        self._cleanup_sfdp_session_stats(config)
