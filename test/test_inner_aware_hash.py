...
These tests verify that the L3 ECMP flow hash
(``ip4_compute_flow_hash`` / ``ip6_compute_flow_hash``) and the new
LAG hash function ``hash-eth-l34-inner`` both dive into the inner
header so that ECMP and LAG distribution use inner-flow entropy
rather than the (constant) outer 5-tuple of transit tunnel traffic.
...
The LAG path is exposed as a new bond load-balance algorithm
``BOND_API_LB_ALGO_L34_INNER`` (``6``), backed by a new registered
hash function ``hash-eth-l34-inner``.  The original
``hash-eth-l34`` is left byte-for-byte unchanged, so existing bonds
keep their current behaviour; only bonds explicitly created with
``BOND_API_LB_ALGO_L34_INNER`` pick up the new inner-aware hash.
...
Fixtures:
  ...
  * TestInnerAwareECMP — pg0 ingress, pg1/pg2/pg3 are ECMP next-hops;
    IP_FLOW_HASH_PEEK_INNER enabled.  Verifies inner-aware ECMP
    distribution for IPinIP / GRE / NVGRE with random inner flows.
  * TestInnerAwareLAG — pg2/pg3 are bond0 members; bond0 created with
    BOND_API_LB_ALGO_L34_INNER.  Verifies inner-aware LAG distribution.
  * TestLagL34LegacyOuterOnly — same topology as TestInnerAwareLAG but
    bond0 is created with the legacy BOND_API_LB_ALGO_L34 enum value;
    tunnel traffic with constant outer 5-tuple MUST collapse to a
    single member, confirming patch 0011 preserves byte-for-byte
    upstream behaviour for the legacy algorithm.
  * TestPeekInnerOff — same as TestInnerAwareECMP but with
    IP_FLOW_HASH_PEEK_INNER NOT set; confirms IP-layer ECMP behaviour
    is unchanged for non-opt-in users.
  * TestSafetyEdges — fragmented outer, inner v6 with HBH / Fragment
    extension header, truncated IPinIP / GRE; confirms the helper
...
def _build_ipinip(outer_l, outer_src, outer_dst, inner_l, isrc, idst, isport, idport):
...
def _build_gre_ip(outer_l, outer_src, outer_dst, inner_l, isrc, idst, isport, idport):
...
def _build_nvgre(outer_l, outer_src, outer_dst, inner_l, isrc, idst, isport, idport):
    return (
        _build_outer(outer_l, outer_src, outer_dst, PROTO_GRE)
...
def _build_tunnel_stream(encap_fn, outer_l, inner_l, *, randomize_inner, seed):
...
class TestInnerAwareECMP(VppTestCase):
    ...
    def test_ecmp_ipinip_collapse_constant_inner(self):
        ...
        self._run_distribution(
            _build_ipinip, IP, IP, randomize_inner=False, expect_paths=1
        )

    def test_ecmp_nvgre_collapse_constant_inner(self):
        """ECMP NVGRE outer-v4 / inner-v4 / fixed inner: collapses to 1 path"""
        self._run_distribution(
            _build_nvgre, IP, IP, randomize_inner=False, expect_paths=1
        )
...
class TestInnerAwareLAG(VppTestCase):
    """Inner-aware LAG / bond (XOR + L34_INNER) flow hash"""

    # Bond load-balance algorithm under test.  Subclasses may override
    # (e.g. TestLagL34LegacyOuterOnly pins this to legacy L34 to assert
    # the upstream-compatible algo did not start peeking the inner hdr).
    lb_algo_name = "BOND_API_LB_ALGO_L34_INNER"

    # MAC used for bond0.  Subclasses override to avoid colliding when
    # the framework reuses VPP state across classes.
    ...
    def setUpClass(cls):
        ...
        cls.vapi.cli(
            "set ip neighbor static BondEthernet0 2001:db8:99::99 abcd.abcd.0001"
        )
    ...
    def test_lag_plain_v4(self):
        ...
        self._run_distribution(None, IP, None, randomize_inner=True, expect_members=2)

    def test_lag_plain_v6(self):
        """LAG plain UDP-over-IPv6: distribution unchanged by helper"""
        self._run_distribution(None, IPv6, None, randomize_inner=True, expect_members=2)


# =========================================================================
#          Regression: legacy BOND_API_LB_ALGO_L34 must NOT peek inner
# =========================================================================
...
class TestLagL34LegacyOuterOnly(TestInnerAwareLAG):
    """Regression: bonds created with the upstream-compatible
    ``BOND_API_LB_ALGO_L34`` enum value (= 1, the historical L34 algo)
    must NOT peek into the inner header.  Tunnel traffic with a constant
    outer 5-tuple must collapse to a single bond member, confirming
    that patch 0011 left the legacy algorithm byte-for-byte upstream.

    We do NOT override ``setUpClass`` here so the framework's CPU
    assignment and class wiring run normally; we just retarget the
    bond's load-balance algorithm via the parent's class attribute.
    Tunnel-distribution tests inherited from the parent assume
    L34_INNER (expect_members=2) and are skipped at runtime in favor
    of the explicit-collapse tests below.
    """

    # Pin bond0 to the legacy, upstream-compatible L34 algo.
    lb_algo_name = "BOND_API_LB_ALGO_L34"
    bond_mac = "02:fe:38:30:59:3d"  # distinct from L34_INNER fixture

    def setUp(self):
        super().setUp()
        # Inherited tunnel-distribution tests assume L34_INNER and would
        # fail under legacy L34.  Skip them; we cover the legacy case
        # with explicit-collapse tests below.
        name = self._testMethodName
        if (
            name.startswith("test_lag_")
            and "plain" not in name
            and "collapse" not in name
            and not name.startswith("test_lag_legacy_")
        ):
            self.skipTest("requires L34_INNER (legacy-collapse variant covered)")

    # --------- legacy: tunnel random-inner MUST collapse to 1 member -----

    def test_lag_legacy_ipinip_v4_collapses(self):
        """Legacy L34: IPinIPv4 random-inner collapses to 1 member"""
        self._run_distribution(
            _build_ipinip, IP, IP, randomize_inner=True, expect_members=1
        )

    def test_lag_legacy_gre_v4_collapses(self):
        """Legacy L34: GRE-IP v4/v4 random-inner collapses to 1 member"""
        self._run_distribution(
            _build_gre_ip, IP, IP, randomize_inner=True, expect_members=1
        )

    def test_lag_legacy_nvgre_v4_collapses(self):
        """Legacy L34: NVGRE v4/v4 random-inner collapses to 1 member"""
        self._run_distribution(
            _build_nvgre, IP, IP, randomize_inner=True, expect_members=1
        )

    def test_lag_legacy_ipinip_v6_collapses(self):
        """Legacy L34: IPinIP v6/v6 random-inner collapses to 1 member"""
        self._run_distribution(
            _build_ipinip, IPv6, IPv6, randomize_inner=True, expect_members=1
        )

    # --------- legacy: plain traffic MUST still distribute ---------------

    def test_lag_legacy_plain_v4_distributes(self):
        """Legacy L34: plain UDP v4 still distributes across members"""
        self._run_distribution(None, IP, None, randomize_inner=True, expect_members=2)

    def test_lag_legacy_plain_v6_distributes(self):
        """Legacy L34: plain UDP v6 still distributes across members"""
        self._run_distribution(None, IPv6, None, randomize_inner=True, expect_members=2)
...
class TestSafetyEdges(TestInnerAwareECMP):
    ...
    def test_outer_v6_fragmented_falls_back(self):
        """Fragmented outer v6 collapses to 1 path (no L4 read).

        Regression test for v6 fragmentation invariant in
        ``ip6_compute_flow_hash``.  After walking a Fragment ext header
        ``cur`` is positioned at the start of the fragment payload --
        which is the real L4 header only for offset==0 fragments and
        is arbitrary payload bytes for later fragments.  Hashing those
        bytes as L4 ports would split fragments of the same flow across
        ECMP paths and break reassembly at the destination.

        The helper MUST ignore the bytes following the Fragment ext
        header.  This test sends N packets that share the same outer
        v6 src/dst (so a correct hash collapses them to one path) but
        carry varying ``UDP(sport, dport)`` right after the Fragment
        header.  A broken helper would distribute on ports; the fixed
        helper collapses to a single path.
        """
        seed = 0xF7A6
        rng = random.Random(seed)
        outer_src, outer_dst = _outer_addrs(IPv6)
        pkts = []
        for _ in range(N_PKTS):
            sport, dport = _rand_port(rng), _rand_port(rng)
            # nh=44 (Fragment) on the IPv6 header, then the Fragment ext
            # header carries nh=17 (UDP) and offset=0, m=1 (i.e., the
            # first fragment of a multi-fragment datagram).  Even when
            # the L4 header IS present (first fragment), the helper
            # must refuse to read its ports -- otherwise the SECOND
            # fragment (which would not carry these ports) would hash
            # to a different path.
            outer = (
                IPv6(src=outer_src, dst=outer_dst, nh=44, hlim=64)
                / IPv6ExtHdrFragment(nh=17, offset=0, m=1, id=0x1234)
                / UDP(sport=sport, dport=dport)
            )
            pkts.append(outer)

        dst_net, prefix = _outer_dst_route(IPv6)
        rip = self._add_ecmp_route(dst_net, prefix, is_ipv6=True)
        try:
            per_if, total = self._send(pkts)
        finally:
            rip.remove_vpp_config()
        ...
        self.assertEqual(
            non_zero,
            1,
            "fragmented outer v6 must NOT hash L4 ports "
            "(would break reassembly); got %s" % per_if,
        )
