...
class TestPPPoEClient(VppTestCase):
    ...
    def test_pppoe_discovery_prefers_combined_ac_and_service_match(self):
        ...
        self.assertEqual(self.normalize_api_string(events[0].service_name), "internet-b")
        self.assertTrue(events[0].raw_tags_len > 0)

        filtered = self.vapi.pppoeclient_control_history_dump(
            pppoeclient_index=1, orphan=False, filter_min_match_score=True, min_match_score=4
        )
        ...
        self.assertTrue(
            all(self.normalize_api_string(e.service_name) == "internet-b" for e in service_filtered)
        )

        top_reason = self.vapi.pppoeclient_control_history_dump(
            pppoeclient_index=1, orphan=False, filter_top_match_reason=True, top_match_reason=8
        )
    ...
    def test_pppoe_request_matches_hostuniqless_pads_by_cookie(self):
        ...
        self.pg0.add_stream(
            [
                self.make_pado(ac_mac, self.AC_NAME, cookie=cookie_one, host_uniq=host_uniq_one),
                self.make_pado(ac_mac, self.AC_NAME, cookie=cookie_two, host_uniq=host_uniq_two),
            ]
        )
        self.pg_start()
        ...
        self.pg0.add_stream(
            [self.make_pads(ac_mac, 0x1002, self.AC_NAME, cookie=cookie_two, host_uniq=None)]
        )
        self.pg_start()
    ...
    def test_pppoe_request_matches_hostuniqless_pads_by_service_name(self):
        ...
        self.pg0.add_stream(
            [
                self.make_pado(
                    ac_mac, self.AC_NAME, host_uniq=host_uniq_one, service_name=b"internet-a"
                ),
                self.make_pado(
                    ac_mac, self.AC_NAME, host_uniq=host_uniq_two, service_name=b"internet-b"
                ),
            ]
        )
        self.pg_start()
    ...
    def test_pppoe_request_rejects_mismatched_service_name_pads(self):
        ...
        first_padr = self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padr)
    ...
    def test_pppoe_request_prefers_cookie_and_service_match(self):
        ...
        top_reason = self.vapi.pppoeclient_control_history_dump(
            pppoeclient_index=1, orphan=False, filter_top_match_reason=True, top_match_reason=11
        )
        self.assertGreaterEqual(len(top_reason), 1)
    ...
    def test_pppoe_debug_cli_reports_recent_control_packets(self):
        ...
        self.assertIn("recent-control-summary max match-score 0 candidates 1 top-score 0 top-count 1", output)
        self.assertIn("recent-control", output)
    ...
    def test_pppoe_summary_cli_can_filter_by_match_reason(self):
        ...
        # PADO + PADS are both direct host-uniq matches; PADT is recorded
        # from the session-state branch which currently drops the prior
        # match-reason before writing the event, so it shows up as match
        # reason "none".
        ...
        self.assertIn("recent-control-summary total 1", none_only)

        session_only = self.vapi.cli(
            "show pppoe client summary 0 match-reason session"
        )
        self.assertIn("recent-control-summary <empty>", session_only)
    ...
    def test_pppoe_control_history_dump_api_reports_recent_packets(self):
        ...
        self.assertTrue(any(self.normalize_api_string(e.ac_name) == "my-ac" for e in events))
        ...
        self.assertTrue(all(self.normalize_api_string(e.ac_name) for e in with_names_and_tags))
        ...
        self.assertTrue(all(e.raw_tags_len == raw_tags_len for e in raw_tags_len_filtered))
        ...
        self.assertTrue(all(self.normalize_api_string(e.ac_name) == "my-ac" for e in ac_name_filtered))
        ...
        recent = self.vapi.pppoeclient_control_history_dump(
            pppoeclient_index=0, orphan=False, filter_max_age_msec=True, max_age_msec=5000
        )
        ...
        scored = self.vapi.pppoeclient_control_history_dump(
            pppoeclient_index=0, orphan=False, filter_min_match_score=True, min_match_score=1
        )
    ...
    def test_pppoe_debug_cli_reports_orphan_control_packets(self):
        ...
        self.pg0.add_stream([self.make_pado("02:00:00:00:00:25", b"orphan-ac", host_uniq=None)])
        self.pg_start()

        output = self.vapi.cli("show pppoe client debug")
        self.assertIn("orphan-control-summary total 1", output)
        self.assertIn("orphan-control-summary fields host-uniq 0 cookie 0 service-name 0 ac-name 1 raw-tags 1 error-tag 0", output)
        self.assertIn("orphan-control-summary max match-score 0 candidates 2 top-score 1 top-count 2", output)
    ...
    def test_pppoe_control_history_dump_api_reports_orphan_packets(self):
        ...
        recent = self.vapi.pppoeclient_control_history_dump(
            pppoeclient_index=0, orphan=True, filter_max_age_msec=True, max_age_msec=5000
        )
        self.assertGreaterEqual(len(recent), 1)
        ...
        self.assertGreaterEqual(len(raw_tags_len_filtered), 1)
        self.assertTrue(
            all(e.raw_tags_len == events[-1].raw_tags_len for e in raw_tags_len_filtered)
        )
        ...
        self.assertGreaterEqual(len(ac_name_filtered), 1)
        self.assertTrue(all(self.normalize_api_string(e.ac_name) == "orphan-ac" for e in ac_name_filtered))
        ...
        top_reason = self.vapi.pppoeclient_control_history_dump(
            pppoeclient_index=0, orphan=True, filter_top_match_reason=True, top_match_reason=4
        )
        self.assertGreaterEqual(len(top_reason), 1)
        ...
        scored = self.vapi.pppoeclient_control_history_dump(
            pppoeclient_index=0, orphan=True, filter_min_match_score=True, min_match_score=1
        )
        self.assertEqual(len(scored), 0)
