...
class TestPPPoEClient(VppTestCase):
    ...
    def test_pppoe_discover(self):
        ...
        pppox_sw_if_index = self.add_client()
        self.vapi.cli(
            "set pppoe client 0 service-name %s"
        ...
        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 1)
        self.assertEqual(clients[0].auth_user.rstrip(b"\x00"), self.AUTH_USERNAME)
        self.assertEqual(clients[0].service_name.rstrip(b"\x00"), self.SERVICE_NAME)
    ...
    def test_pppoe_service_name_any_clears_tag(self):
        ...
        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 1)
        self.assertEqual(clients[0].service_name.rstrip(b"\x00"), b"")
    ...
    def test_pppoe_ac_name_setting_round_trip(self):
        ...
        self.vapi.cli(
            "set pppoe client 0 ac-name %s" % self.AC_NAME.decode("ascii")
        )
        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 1)
        self.assertEqual(clients[0].configured_ac_name.rstrip(b"\x00"), self.AC_NAME)

        self.vapi.cli("set pppoe client 0 ac-name any")
        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 1)
        self.assertEqual(clients[0].configured_ac_name.rstrip(b"\x00"), b"")
