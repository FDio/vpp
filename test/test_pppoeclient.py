...
from scapy.layers.ppp import PPP, PPPoED, PPPoED_Tags, PPPoE
...
class TestPPPoEClient(VppTestCase):
    ...
    AUTH_USERNAME = "pfsense"
    AUTH_PASSWORD = "admin"
    AC_NAME = b"my-ac"
    SERVICE_NAME = b"internet"
    AUTH_USERNAME_MAX = "u" * 63
    AUTH_PASSWORD_MAX = "p" * 63
    ...
    def make_pado(
        ...
        pkt = Ether(dst=self.pg0.local_mac, src=src_mac)
        if vlan is not None:
            pkt /= Dot1Q(vlan=vlan)
        return pkt / PPPoED(code=0x07, sessionid=0) / Raw(b"".join(tags))
    ...
    def make_pads(
        ...
        if vlan is not None:
            pkt /= Dot1Q(vlan=vlan)
        return pkt / PPPoED(code=0x65, sessionid=session_id) / Raw(b"".join(tags))
    ...
    @staticmethod
    def ppp_control_payload(code, ident, options=b""):
        return struct.pack("!BBH", code, ident, 4 + len(options)) + options

    @staticmethod
    def split_ppp_option(options, option_type):
        """Split a PPP configuration-option list into (matching, rest)."""

        matching = b""
        rest = b""
        pos = 0
        while pos + 2 <= len(options):
            opt_len = options[pos + 1]
            if opt_len < 2 or pos + opt_len > len(options):
                rest += options[pos:]
                break
            option = options[pos : pos + opt_len]
            if options[pos] == option_type:
                matching += option
            else:
                rest += option
            pos += opt_len
    ...
    def make_ppp_session_packet(self, src_mac, session_id, proto, payload, vlan=None):
        pkt = Ether(dst=self.pg0.local_mac, src=src_mac)
        if vlan is not None:
            pkt /= Dot1Q(vlan=vlan)
        return pkt / PPPoE(sessionid=session_id) / PPP(proto=proto) / Raw(payload)

    def parse_ppp_control(self, pkt, proto):
        ppp = pkt.getlayer(PPP)
        if ppp is None or ppp.proto != proto:
            return None
        raw = bytes(ppp.payload)
        if len(raw) < 4:
            return None
        code, ident, length = struct.unpack("!BBH", raw[:4])
        if length < 4 or length > len(raw):
            return None
        return code, ident, raw[4:length]

    def wait_for_ppp_control(self, proto, code, ident=None, timeout=10):
        def is_wanted(pkt):
            parsed = self.parse_ppp_control(pkt, proto)
            if parsed is None or parsed[0] != code:
                return False
            return ident is None or parsed[1] == ident

        pkt = self.pg0.wait_for_packet(
            timeout=timeout, filter_out_fn=lambda p: not is_wanted(p)
        )
        return self.parse_ppp_control(pkt, proto)

    def send_ppp_control(
        self, ac_mac, session_id, proto, code, ident, options=b"", vlan=None
    ):
        self.pg0.add_stream(
            [
                self.make_ppp_session_packet(
                    ac_mac,
                    session_id,
                    proto,
                    self.ppp_control_payload(code, ident, options),
                    vlan=vlan,
                )
            ]
        )
    ...
    def negotiate_lcp(self, ac_mac, session_id, our_ident=0x41, vlan=None):
        """Bring the client's LCP to OPENED without an authentication phase.

        The client's Configure-Request asks the peer to authenticate to it
        with PAP (lcp_wantoptions.neg_upap).  Reject that option, ACK the
        remaining ones, then exchange our own empty Configure-Request.
        """

        while True:
            code, ident, options = self.wait_for_ppp_control(
                self.PPP_LCP, self.PPP_CONFREQ
            )
            auth, rest = self.split_ppp_option(options, self.CI_AUTHTYPE)
            if not auth:
                break
            self.send_ppp_control(
                ac_mac,
                session_id,
                self.PPP_LCP,
                self.PPP_CONFREJ,
        ...
        self.wait_for_ppp_control(self.PPP_LCP, self.PPP_CONFACK, ident=our_ident)

    def negotiate_ipv6cp(self, ac_mac, session_id, our_ident=0x51, vlan=None):
        code, ident, options = self.wait_for_ppp_control(
            self.PPP_IPV6CP, self.PPP_CONFREQ
        )
        self.send_ppp_control(
            ac_mac,
            session_id,
        ...
        our_options = struct.pack("!BB", self.CI_IFACEID, 10) + bytes.fromhex(
            "0200000000000001"
        )
        self.send_ppp_control(
            ac_mac,
        ...
        self.wait_for_ppp_control(self.PPP_IPV6CP, self.PPP_CONFACK, ident=our_ident)

    def assert_ipv6cp_opened(self):
        output = self.vapi.cli("show pppoe client debug")
        self.assertIn("ipv6cp OPENED", output)
    ...
    def establish_session(
        ...
        ac_name=None,
        session_id=0x1234,
        pppox_sw_if_index=None,
    ):
        if pppox_sw_if_index is None:
    ...
    def wait_for_condition(self, fn, desc, timeout=3):
        """Poll fn() (returning (ok: bool, detail: object)) until ok or timeout."""
        deadline = time.time() + timeout
        while time.time() < deadline:
            ok, detail = fn()
            if ok:
            ...
            self.sleep(0.1)
        self.fail(
            "Timed out waiting for condition: %s, last detail: %s" % (desc, detail)
        )
    ...
    def test_pppoe_vlan_subif_dispatch(self):
        """A client on a VLAN sub-interface must match tagged frames on the parent."""

        vlan = 100
        subif = self.vapi.create_vlan_subif(sw_if_index=self.pg0.sw_if_index, vlan=vlan)
        sub_sw_if_index = subif.sw_if_index
        self.vapi.sw_interface_set_flags(sw_if_index=sub_sw_if_index, flags=1)

        try:
            response = self.vapi.pppoeclient_add_del(
                is_add=True,
                sw_if_index=sub_sw_if_index,
                host_uniq=self.HOST_UNIQ,
            )
            self.assertEqual(response.retval, 0)
            ...
            clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
            self.assertEqual(len(clients), 1)
            self.assertEqual(clients[0].sw_if_index, sub_sw_if_index)

            # The client's PADI is emitted on the parent port with the
            # sub-interface's VLAN tag.
            padi = self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padi)
            self.assertTrue(padi.haslayer(Dot1Q))
            self.assertEqual(padi[Dot1Q].vlan, vlan)

            # A tagged PADO injected on the parent port must reach the
            # sub-interface client (dispatch VLAN demux).
            ac_mac = "02:00:00:00:00:44"
            self.pg0.enable_capture()
            self.pg0.add_stream([self.make_pado(ac_mac, self.AC_NAME, vlan=vlan)])
            self.pg_start()
            padr = self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padr)
            self.assertEqual(padr[Ether].dst.lower(), ac_mac)
            self.assertTrue(padr.haslayer(Dot1Q))
            self.assertEqual(padr[Dot1Q].vlan, vlan)

            session_id = 0x2345
            self.pg0.add_stream(
                [self.make_pads(ac_mac, session_id, self.AC_NAME, vlan=vlan)]
            )
            self.pg_start()
            self.wait_for_client_session(session_id)

            # Tagged session data must reach the pppd state machine:
            # negotiate LCP over tagged session frames.
            self.negotiate_lcp(ac_mac, session_id, vlan=vlan)
        finally:
            self.delete_all_clients()
            self.vapi.delete_subif(sub_sw_if_index)

    def test_pppoe_wildcard_subif_learns_session_vlan(self):
        """A wildcard sub-interface must learn the session VLAN for the raw PADT."""

        vlan = 321
        subif = self.vapi.create_subif(
            sw_if_index=self.pg0.sw_if_index,
            sub_id=vlan,
            outer_vlan_id=vlan,
            # ONE_TAG without EXACT_MATCH: matches any outer VLAN, so the
            # concrete VLAN is only known from the frames the BAS sends.
            sub_if_flags=VppEnum.vl_api_sub_if_flags_t.SUB_IF_API_FLAG_ONE_TAG,
        )
        sub_sw_if_index = subif.sw_if_index
        self.vapi.sw_interface_set_flags(sw_if_index=sub_sw_if_index, flags=1)
        ...
        try:
            ...
            output = self.vapi.cli("show pppoe client debug")
            self.assertIn(
                "vlan session learned n-tags 1 dot1ad 0 outer %u inner 0" % vlan,
                output,
            )
        finally:
            self.delete_all_clients()
    ...
    def test_pppoe_wildcard_subif_all_clients_learn_session_vlan(self):
        """Every client sharing a wildcard sub-interface must learn its VLAN."""
        ...
        sub_sw_if_index = subif.sw_if_index
        self.vapi.sw_interface_set_flags(sw_if_index=sub_sw_if_index, flags=1)
        host_uniq2 = self.HOST_UNIQ + 1

        try:
            for host_uniq in (self.HOST_UNIQ, host_uniq2):
                response = self.vapi.pppoeclient_add_del(
                    is_add=True, sw_if_index=sub_sw_if_index, host_uniq=host_uniq
                )
                self.assertEqual(response.retval, 0)

            self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padi)

            # Only the second client is driven to SESSION.  The first client
            # is the dispatch match winner and would otherwise be the only
            # one to learn the concrete wildcard VLAN.
            ac_mac = "02:00:00:00:00:48"
            self.pg0.enable_capture()
            self.pg0.add_stream(
                [self.make_pado(ac_mac, self.AC_NAME, host_uniq=host_uniq2, vlan=vlan)]
            )
            self.pg_start()
            self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padr)
            ...
            while time.time() < deadline:
                clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
                if any(
                    c.session_id == session_id and c.client_state == 3 for c in clients
                ):
                    break
                self.sleep(0.1)
            else:
                self.fail("second wildcard client did not reach SESSION")

            output = self.vapi.cli("show pppoe client debug")
            learned = "vlan session learned n-tags 1 dot1ad 0 outer %u inner 0" % vlan
            self.assertEqual(output.count(learned), 2, output)
        finally:
            self.delete_all_clients()
            self.vapi.delete_subif(sub_sw_if_index)

    def test_pppoe_client_cleaned_when_access_subif_deleted(self):
        """Deleting the access sub-interface must delete its client."""
        ...
        response = self.vapi.pppoeclient_add_del(
            is_add=True, sw_if_index=sub_sw_if_index, host_uniq=self.HOST_UNIQ
        )
        self.assertEqual(response.retval, 0)
        ...
        # The interface add/del callback must stop and free the client
        # before the sw_if_index is recycled.
        self.vapi.delete_subif(sub_sw_if_index)

        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 0)

    def test_pppoe_session_tcp_mss_clamp_odd_offset(self):
        """Clamp an MSS option at an odd offset and keep the TCP checksum valid."""

        pppox_sw_if_index, session_id, ac_mac = self.establish_session()
        # Enable IPv4 on the pppox interface so ip4-input forwards the frame;
        # the destination is on pg0's connected subnet, giving us a capture
        # point for the rewritten packet.
        self.vapi.sw_interface_add_del_address(
            sw_if_index=pppox_sw_if_index, prefix="192.0.2.1/32"
        )
        # Pin the MTU: pppox interfaces are recycled and a previous test may
        # have left a jumbo MTU behind, which would disable clamping.
        self.vapi.sw_interface_set_mtu(pppox_sw_if_index, [1500, 0, 0, 0])

        tcp = TCP(
            sport=1234,
            dport=80,
            flags="S",
            options=[("NOP", None), ("MSS", 2000), ("EOL", None)],
        )
        ip = IP(src="10.9.0.1", dst=self.pg0.remote_ip4) / tcp
        self.pg0.enable_capture()
        self.pg0.add_stream(
            [self.make_ppp_session_packet(ac_mac, session_id, self.PPP_IP, bytes(ip))]
        )
        self.pg_start()

        rx = self.pg0.wait_for_packet(
            timeout=10,
            filter_out_fn=lambda pkt: not (pkt.haslayer(IP) and pkt.haslayer(TCP)),
        )
        mss_options = [opt for opt in rx[TCP].options if opt[0] == "MSS"]
        self.assertEqual(len(mss_options), 1)
        self.assertEqual(mss_options[0][1], 1460)
        self.assert_tcp_checksum_valid(rx)

    def test_pppoe_session_ipv6_nonfirst_fragment_not_clamped(self):
        """A non-first IPv6 fragment must not be mistaken for a TCP header."""

        pppox_sw_if_index, session_id, ac_mac = self.establish_session()
        # Adding an address is idempotent on a recycled pppox interface,
        # unlike sw_interface_ip6_enable_disable (VALUE_EXIST if already on).
        self.vapi.sw_interface_add_del_address(
            sw_if_index=pppox_sw_if_index, prefix="fd00:9::2/64"
        )
        self.vapi.sw_interface_set_mtu(pppox_sw_if_index, [1500, 0, 0, 0])
        ...
        fragment = IPv6ExtHdrFragment(offset=1, m=1, nh=6, id=1) / fake_tcp
        ip6 = IPv6(src="fd00:9::1", dst=self.pg0.remote_ip6) / fragment
        # Scapy does not dissect a non-first fragment's payload as TCP, so
        # compare the raw fragment payload instead of parsed options.
        ip6_bytes = bytes(ip6)
        expected_payload = ip6_bytes[40 + 8 :]
        self.pg0.enable_capture()
        self.pg0.add_stream(
            [
                self.make_ppp_session_packet(
                    ac_mac, session_id, self.PPP_IPV6, bytes(ip6)
                )
            ]
        )
        self.pg_start()
        ...
        rx = self.pg0.wait_for_packet(
            timeout=10, filter_out_fn=lambda pkt: not pkt.haslayer(IPv6)
        )
        rx_payload = bytes(rx[IPv6ExtHdrFragment].payload)
        self.assertEqual(rx_payload, expected_payload)

    def test_pppoe_session_output_reports_chained_pppoe_length(self):
        """The PPPoE length field must cover a chained (jumbo) TX packet."""

        pppox_sw_if_index, session_id, ac_mac = self.establish_session()
        self.vapi.sw_interface_set_mtu(pppox_sw_if_index, [9000, 0, 0, 0])

        route = VppIpRoute(
            self,
            "10.10.10.2",
            32,
            [VppRoutePath("0.0.0.0", pppox_sw_if_index)],
        )
        ...
        big = (
            ...
            / IP(src=self.pg0.remote_ip4, dst="10.10.10.2")
            / Raw(b"\xa5" * payload_len)
        )
        self.pg0.enable_capture()
        self.pg0.add_stream([big])
        self.pg_start()

        rx = self.pg0.wait_for_packet(
            timeout=10, filter_out_fn=lambda pkt: not pkt.haslayer(PPPoE)
        )
        self.assertEqual(rx[PPPoE].sessionid, session_id)
        self.assertEqual(rx[PPPoE].len, payload_len + 20 + 2)

    def test_pppoe_discovery_rejects_pado_declared_length_beyond_frame(self):
        """A PADO declaring more payload than the frame carries must be dropped."""
        ...
        # Valid Host-Uniq + END-OF-LIST tags, but the PPPoE length field
        # claims 1280 bytes although only a handful are present.  Before the
        # fix the parser trusted the declared length and accepted the PADO.
        ac_mac = "02:00:00:00:00:43"
        tags = self.pppoe_tag(0x0103, self.HOST_UNIQ.to_bytes(4, "little"))
        tags += self.pppoe_tag(0x0000, b"")
        pppoe = b"\x11\x07\x00\x00" + struct.pack("!H", 0x0500) + tags
        eth = Ether(dst=padi[Ether].src, src=ac_mac, type=0x8863)
        self.pg0.add_stream([Ether(bytes(eth) + pppoe)])
        self.pg_start()
    ...
    def test_pppoe_discovery_rejects_pado_length_spanning_pppoe_header(self):
        """A declared length bigger than the real payload must be dropped even
        when it still fits the header-inclusive byte count."""
        ...
        # Payload is 12 bytes (Host-Uniq TLV + END-OF-LIST), but the length
        # field claims 18: that fits the 6+12 bytes physically present
        # (header inclusive), so a parser that forgets the fixed 6-byte
        # PPPoE header accepts it and copies header slack into raw_tags.
        ac_mac = "02:00:00:00:00:47"
        tags = self.pppoe_tag(0x0103, self.HOST_UNIQ.to_bytes(4, "little"))
        tags += self.pppoe_tag(0x0000, b"")
        pppoe = b"\x11\x07\x00\x00" + struct.pack("!H", 18) + tags
        eth = Ether(dst=padi[Ether].src, src=ac_mac, type=0x8863)
        self.pg0.add_stream([Ether(bytes(eth) + pppoe)])
        self.pg_start()
    ...
    def test_pppoe_session_input_handles_truncated_frame(self):
        """A session frame without a complete PPPoE header must not break state."""

        _, session_id, ac_mac = self.establish_session()

        eth = Ether(dst=self.pg0.local_mac, src=ac_mac, type=0x8864)
        self.pg0.add_stream([Ether(bytes(eth))])
        self.pg_start()
        self.sleep(0.2)

        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 1)
        self.assertEqual(clients[0].session_id, session_id)
        self.assertEqual(clients[0].client_state, 3)

    def test_pppoe_dhcp6_runtime_observability(self):
        """Expose DHCPv6 IA-NA and PD runtime state through the client dump."""
        ...
        self.vapi.dhcp6_client_enable_disable(
            sw_if_index=pppox_sw_if_index, enable=True
        )
        self.vapi.dhcp6_pd_client_enable_disable(
            sw_if_index=pppox_sw_if_index, prefix_group="pppoe-test-pd", enable=True
        )
        try:
            clients = self.vapi.pppoeclient_dump(sw_if_index=self.pg0.sw_if_index)
            self.assertEqual(clients[0].dhcp6_ia_na_enabled, 1)
            self.assertEqual(clients[0].dhcp6_pd_enabled, 1)
            self.assertEqual(
                self.normalize_api_string(clients[0].dhcp6_pd_prefix_group),
                "pppoe-test-pd",
            )
        finally:
            self.vapi.dhcp6_pd_client_enable_disable(
                sw_if_index=pppox_sw_if_index, prefix_group="", enable=False
            )
            self.vapi.dhcp6_client_enable_disable(
                sw_if_index=pppox_sw_if_index, enable=False
            )
    ...
    def test_pppox_set_auth_accepts_full_length_credentials(self):
        """Allow full-length (string[64] max 63-char) username/password via the API."""
    ...
    def test_pppoe_discovery_rejects_mismatched_service_name_pado(self):
        ...
        self.wait_for_client_state(self.HOST_UNIQ, 1, 0)
    ...
    def test_pppoe_duplicate_pado_does_not_clear_selected_cookie(self):
        ...
        duplicate_pado = self.make_pado(
            "02:00:00:00:00:22", self.AC_NAME.decode("ascii")
        )
        self.pg0.enable_capture()
    ...
    def test_pppoe_request_ignores_pado_from_unselected_ac(self):
        ...
        def _check_unselected_ac_counter():
            now = self.statistics.get_err_counter(counter)
            return now >= before + 1, now

        after = self.wait_for_condition(
            _check_unselected_ac_counter,
            "stray PADO bumps the unselected-AC error counter",
        )
    ...
    def test_pppoe_request_ignores_pads_from_unselected_ac(self):
        ...
        good_pado = self.make_pado("02:00:00:00:00:22", self.AC_NAME.decode("ascii"))
        ...
        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 1)
        self.assertEqual(clients[0].client_state, 2)
        self.assertEqual(clients[0].session_id, 0)
    ...
    def test_pppoe_request_matches_hostuniqless_pads_by_selected_ac(self):
        ...
        self.wait_for_client_state(host_uniq_one, 3, 0x1001)
        self.wait_for_client_state(host_uniq_two, 2, 0)
    ...
    def test_pppoe_discovery_matches_hostuniqless_pado_by_ac_name_filter(self):
        ...
        self.wait_for_client_state(host_uniq_one, 2, 0)
    ...
    def test_pppoe_discovery_service_name_match_does_not_override_ac_filter(self):
        ...
        self.wait_for_client_state(host_uniq_filtered, 1, 0)
        self.wait_for_client_state(host_uniq_any, 2, 0)
    ...
    def test_pppoe_discovery_prefers_combined_ac_and_service_match(self):
        ...
        self.wait_for_client_state(host_uniq_ac_only, 1, 0)
        self.wait_for_client_state(host_uniq_combined, 2, 0)
    ...
    def test_pppoe_discovery_hostuniqless_pado_scans_past_ambiguous_clients(self):
        ...
        self.wait_for_client_state(host_uniq_one, 1, 0)
        self.wait_for_client_state(host_uniq_two, 1, 0)
        self.wait_for_client_state(host_uniq_three, 2, 0)
    ...
    def test_pppoe_discovery_hostuniqless_pado_prefers_single_any_client(self):
        ...
        self.wait_for_client_state(host_uniq_any, 2, 0)
        self.wait_for_client_state(host_uniq_filtered_one, 1, 0)
        self.wait_for_client_state(host_uniq_filtered_two, 1, 0)
    ...
    def test_pppoe_discovery_hostuniqless_pado_ambiguous_match_stays_orphan(self):
        ...
        def _orphan_summary_ready():
            try:
                out = self.vapi.cli("show pppoe client summary orphan")
            except Exception:
                return False, ""
            return "orphan-control-summary total 1" in out, out

        output = self.wait_for_condition(
            _orphan_summary_ready, "orphan-control-summary total 1"
        )
    ...
    def test_pppoe_request_ignores_malformed_pads(self):
        ...
        self.pg0.enable_capture()
        self.pg0.add_stream(
            [self.make_pado("02:00:00:00:00:22", self.AC_NAME.decode("ascii"))]
        )
        self.pg_start()
    ...
    def _bring_request_state(self, ac_mac="02:00:00:00:00:22"):
        ...
        self.pg0.enable_capture()
        self.pg0.add_stream([self.make_pado(ac_mac, self.AC_NAME.decode("ascii"))])
        self.pg_start()
        self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padr)
    ...
    def test_pppoe_pads_zero_session_id_bumps_invalid_session_counter(self):
        ...
        def _check_invalid_session_counter():
            now = self.statistics.get_err_counter(counter)
            return now >= before + 1, now

        after = self.wait_for_condition(
            _check_invalid_session_counter,
            "zero session-id PADS bumps the invalid-session counter",
        )
        self.assertEqual(after, before + 1)
    ...
    def test_pppoe_pads_duplicate_session_id_bumps_collision_counter(self):
        ...
        self.pg0.enable_capture()
        self.pg0.add_stream(
            [self.make_pado(ac_mac, self.AC_NAME.decode("ascii"), host_uniq=0x99999999)]
        )
        self.pg_start()
        self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padr)
        ...
        def _check_collision_counter():
            now = self.statistics.get_err_counter(counter)
            return now >= before + 1, now

        after = self.wait_for_condition(
            _check_collision_counter,
            "duplicate session-id PADS bumps the collision counter",
        )
        self.assertEqual(after, before + 1)
    ...
    def test_pppoe_request_matches_hostuniqless_pads_by_cookie(self):
        ...
        self.wait_for_client_state(host_uniq_one, 2, 0)
        self.wait_for_client_state(host_uniq_two, 3, 0x1002)
    ...
    def test_pppoe_request_matches_hostuniqless_pads_by_service_name(self):
        ...
        self.wait_for_client_state(host_uniq_one, 2, 0)
        self.wait_for_client_state(host_uniq_two, 3, 0x2002)
    ...
    def test_pppoe_request_prefers_cookie_and_service_match(self):
        ...
        self.wait_for_client_state(host_uniq_one, 2, 0)
        self.wait_for_client_state(host_uniq_two, 3, 0x3456)
    ...
    def test_pppoe_debug_cli_reports_padt_reason(self):
        ...
        def _check_padt_debug():
            try:
                out = self.vapi.cli("show pppoe client debug")
            except Exception:
                return False, ""
            return ("last-disconnect padt" in out and "reconnects 1" in out), out

        output = self.wait_for_condition(
            _check_padt_debug, "debug CLI reports the PADT disconnect reason"
        )
    ...
    def test_pppoe_debug_cli_reports_recent_control_packets(self):
        ...
        pado = self.make_pado(
            "02:00:00:00:00:22", self.AC_NAME.decode("ascii"), cookie=b"good-cookie"
        )
        self.pg0.enable_capture()
        self.pg0.add_stream([pado])
        self.pg_start()
        ...
        def _check_recent_debug():
            try:
                out = self.vapi.cli("show pppoe client debug")
            except Exception:
                return False, ""
            return (
                "recent-control-summary total 3" in out
                and "recent-control-summary codes pado 1 pads 1 padt 1" in out
            ), out

        output = self.wait_for_condition(
            _check_recent_debug, "debug CLI shows recent-control total 3"
        )
    ...
    def test_pppoe_debug_cli_can_focus_single_client(self):
        ...
        def _check_focused_debug():
            try:
                out = self.vapi.cli("show pppoe client debug 0")
            except Exception:
                return False, ""
            return "recent-control-summary total 3" in out, out

        output = self.wait_for_condition(
            _check_focused_debug, "focused debug CLI shows recent-control total 3"
        )
        self.assertIn("recent-control-summary total 3", output)
        self.assertIn("recent-control", output)
        self.assertIn("PADO", output)
    ...
    def test_pppoe_summary_cli_reports_recent_control_summary(self):
        ...
        def _check_summary_cli():
            try:
                out = self.vapi.cli("show pppoe client summary")
            except Exception:
                return False, ""
            return (
                "recent-control-summary total 3" in out
        ...
        output = self.wait_for_condition(
            _check_summary_cli, "summary CLI shows recent-control total 3"
        )
        self.assertIn("recent-control-summary total 3", output)
        self.assertIn("recent-control-summary codes pado 1 pads 1 padt 1", output)
        self.assertIn(
    ...
    def test_pppoe_history_cli_reports_recent_control_history(self):
        ...
        def _check_history_cli():
            try:
                out = self.vapi.cli("show pppoe client history 0")
            except Exception:
                return False, ""
            return "recent-control-summary total 3" in out, out

        output = self.wait_for_condition(
            _check_history_cli, "history CLI shows recent-control total 3"
        )
        self.assertIn("recent-control-summary total 3", output)
        self.assertIn("recent-control", output)
        self.assertIn("PADO", output)
    ...
    def test_pppoe_summary_cli_can_filter_by_match_reason(self):
        ...
        def _check_host_uniq_filter():
            try:
                ...
                return False, ""
            return "recent-control-summary total 2" in out, out

        host_uniq_only = self.wait_for_condition(
            _check_host_uniq_filter, "summary host-uniq filter shows total 2"
        )
        self.assertIn("recent-control-summary total 2", host_uniq_only)

        def _check_session_filter():
            try:
                out = self.vapi.cli("show pppoe client summary 0 match-reason session")
            except Exception:
                return False, ""
            return "recent-control-summary total 1" in out, out

        session_only = self.wait_for_condition(
            _check_session_filter, "summary session filter shows total 1"
        )
    ...
    def test_pppoe_control_history_dump_api_reports_recent_packets(self):
        ...
        def _check_recent_events():
            ev = self.vapi.pppoeclient_control_history_dump(
                pppoeclient_index=0, orphan=False
            )
            codes = [e.code for e in ev]
            return (
                len(ev) >= 3 and 0x07 in codes and 0x65 in codes and 0xA7 in codes
            ), ev

        events = self.wait_for_condition(
            _check_recent_events,
            "control history dump reports PADO/PADS/PADT events",
        )
        ...
        def _check_latest_event():
            latest = self.vapi.pppoeclient_control_history_dump(
                pppoeclient_index=0, orphan=False, max_events=1
            )
            return len(latest) == 1 and latest[0].code == 0xA7, latest

        latest = self.wait_for_condition(
            _check_latest_event, "latest control history event is the PADT"
        )
        self.assertEqual(len(latest), 1)
        self.assertEqual(latest[0].code, 0xA7)

        def _check_latest_two_events():
            latest_two = self.vapi.pppoeclient_control_history_dump(
                pppoeclient_index=0, orphan=False, max_events=2
            )
            return (
                len(latest_two) == 2 and [e.code for e in latest_two] == [0x65, 0xA7]
            ), latest_two

        latest_two = self.wait_for_condition(
            _check_latest_two_events, "latest two events order as PADS then PADT"
        )
        ...
        def _check_full_summary():
            ...
            return (
                summary.matched_events == 3
                and summary.pado_count == 1
                and summary.pads_count == 1
                and summary.padt_count == 1
            ), summary

        summary = self.wait_for_condition(
            _check_full_summary, "control history summary reports 3 matched events"
        )
    ...
    def test_pppoe_debug_cli_reports_orphan_control_packets(self):
        ...
        output = self.wait_for_condition(
            _check_orphan_debug, "debug CLI shows orphan-control total 1"
        )
        self.assertIn("orphan-control-summary total 1", output)
        self.assertIn(
    ...
    def test_pppoe_debug_cli_can_focus_orphan_history(self):
        ...
        def _check_focused_orphan_debug():
            try:
                out = self.vapi.cli("show pppoe client debug orphan")
            except Exception:
                return False, ""
            return "orphan-control-summary total 1" in out, out

        output = self.wait_for_condition(
            _check_focused_orphan_debug, "focused orphan debug CLI shows total 1"
        )
        self.assertIn("orphan-control-summary total 1", output)
        self.assertIn("orphan-control", output)
        self.assertIn("PADO", output)
    ...
    def test_pppoe_summary_cli_reports_orphan_control_summary(self):
        ...
        output = self.wait_for_condition(
            _check_orphan_summary_cli, "orphan summary CLI shows total 1"
        )
        self.assertIn("orphan-control-summary total 1", output)
        self.assertIn(
            "orphan-control-summary fields host-uniq 0 cookie 0 service-name 0 ac-name 1 raw-tags 1 error-tag 0",
    ...
    def test_pppoe_history_cli_reports_orphan_control_history(self):
        ...
        def _check_orphan_history_cli():
            try:
                out = self.vapi.cli("show pppoe client history orphan")
            except Exception:
                return False, ""
            return "orphan-control-summary total 1" in out, out

        output = self.wait_for_condition(
            _check_orphan_history_cli, "orphan history CLI shows total 1"
        )
        self.assertIn("orphan-control-summary total 1", output)
        self.assertIn("orphan-control", output)
        self.assertIn("PADO", output)
    ...
    def test_pppoe_control_history_dump_api_reports_orphan_packets(self):
        ...
        def _check_orphan_dump():
            ...
            ok = (
                last.orphan == True
                and last.sw_if_index == self.pg0.sw_if_index
                and last.code == 0x07
                and last.client_state == 0
                and last.disposition == 4
                and last.match_reason == 0
                and last.match_score == 0
                and last.candidate_count == 2
                and last.top_match_count == 2
                and last.top_match_reason == 4
                and last.top_match_score == 1
                and self.normalize_api_string(last.ac_name) == "orphan-ac"
                and last.raw_tags_len > 0
            )
            return ok, ev

        events = self.wait_for_condition(
            _check_orphan_dump, "orphan control history dump reports the orphan PADO"
        )
        self.assertGreaterEqual(len(events), 1)
    ...
    def test_pppoe_set_options_api_invalid_value_leaves_state_unchanged(self):
        """A rejected numeric option must not partially apply credentials."""
        ...
        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 1)
        self.assertEqual(self.normalize_api_string(clients[0].auth_user), "keepme")
        self.assertEqual(clients[0].mtu, 1492)
        self.assertEqual(clients[0].mru, 1492)
    ...
    def test_pppoe_set_options_api_applies_mru_to_lcp(self):
        """set_options mru must reach LCP wantoptions before the session opens."""

        pppox_sw_if_index = self.add_client()
        self.vapi.pppoeclient_set_options(pppoeclient_index=0, mru=1400)

        self.establish_session(pppox_sw_if_index=pppox_sw_if_index)
        _, _, options = self.wait_for_ppp_control(self.PPP_LCP, self.PPP_CONFREQ)
        mru_option, _ = self.split_ppp_option(options, self.CI_MRU)
        self.assertTrue(mru_option, "client LCP Configure-Request has no MRU option")
        self.assertEqual(struct.unpack("!H", mru_option[2:4])[0], 1400)
    ...
    def test_pppoe_stop_ignores_late_pado_until_open(self):
        """A stopped client ignores PADO; open clears the stop and redials."""
        ...
        # A PADO that would normally move DISCOVERY -> REQUEST must be ignored.
        self.pg0.enable_capture()
        self.pg0.add_stream([self.make_pado(ac_mac, self.AC_NAME)])
        self.pg_start()
        self.pg0.assert_nothing_captured(timeout=2, filter_out_fn=self.is_not_padr)

        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 1)
        self.assertEqual(clients[0].client_state, 1)

        # OPEN clears the stop and the client must accept a PADO again.
        response = self.vapi.pppoeclient_session_action(pppoeclient_index=0, action=3)
        self.assertEqual(response.retval, 0)
        self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padi)
    ...
    def test_pppoe_ipv6cp_reopens_after_admin_restart(self):
        """IPv6CP must come up again after an admin session restart.

        Regression: ipv6cp_open() short-circuits while the sticky
        ipv6cp_is_open flag is set.  The admin restart path tears the
        session down without lowering PPP, so ipv6cp_init() must clear the
        per-session flags or the second session stays CLOSED.
        """
        ...
        self.pg0.enable_capture()
        self.pg0.add_stream([self.make_pads(ac_mac, session_id, self.AC_NAME)])
        self.pg_start()
        self.wait_for_client_session(session_id)

        self.negotiate_lcp(ac_mac, session_id)
        self.negotiate_ipv6cp(ac_mac, session_id)
        self.assert_ipv6cp_opened()

        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        client_index = clients[0].pppoeclient_index

        # RESTART tears the session down without lowering PPP and reuses the
        # same pppox unit, which is the path that used to leave the sticky
        # ipv6cp_is_open flag behind.
        self.pg0.enable_capture()
        response = self.vapi.pppoeclient_session_action(
            pppoeclient_index=client_index, action=1
        )
        self.assertEqual(response.retval, 0)
        self.vapi.cli("clear pppoe client backoff %u immediate" % client_index)
        self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padi)

        new_session_id = 0x5678
        self.pg0.add_stream([self.make_pado(ac_mac, self.AC_NAME)])
        self.pg_start()
        self.pg0.wait_for_packet(timeout=10, filter_out_fn=self.is_not_padr)

        self.pg0.enable_capture()
        self.pg0.add_stream([self.make_pads(ac_mac, new_session_id, self.AC_NAME)])
        self.pg_start()
        self.wait_for_client_session(new_session_id)

        self.negotiate_lcp(ac_mac, new_session_id)
        self.negotiate_ipv6cp(ac_mac, new_session_id)
        self.assert_ipv6cp_opened()

    def test_pppoe_ipcp_does_not_negotiate_vj_compression(self):
        """IPCP must neither offer nor accept VJ header compression."""

        _, session_id, ac_mac = self.establish_session()
        self.negotiate_lcp(ac_mac, session_id)

        _, _, options = self.wait_for_ppp_control(self.PPP_IPCP, self.PPP_CONFREQ)
        vj_option, _ = self.split_ppp_option(options, self.CI_COMPRESSTYPE)
        self.assertEqual(vj_option, b"", "client offered VJ compression")

        # A peer Configure-Request carrying a VJ option must be rejected:
        # the data path cannot decapsulate PPP protocol 0x002d.
        vj_option = struct.pack(
            "!BBHBB", self.CI_COMPRESSTYPE, 6, self.VJ_PROTOCOL, 15, 1
        )
        self.send_ppp_control(
            ac_mac,
            session_id,
        ...
        _, _, reject_options = self.wait_for_ppp_control(
            self.PPP_IPCP, self.PPP_CONFREJ, ident=0x62
        )
        self.assertIn(vj_option, reject_options)
