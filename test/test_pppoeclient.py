...
class TestPPPoEClient(VppTestCase):
    ...
    def _make_pppoe_session_packet(
        self, ac_mac, session_id, inner_payload, ethertype=0x0021
    ):
        """Build a PPPoE session data packet (BAS -> client).

        Frame structure: Ether(dst=pg0_mac, src=ac_mac) / PPPoE(session) /
        session_id / PPP(proto=ethertype) / inner_payload
        """
        # PPPoE header: ver_type=0x11, code=0x00, session_id, length
        pppoe_hdr = struct.pack(
            "!BBH H",
            0x11,  # ver=1, type=1
            0x00,  # code=SESSION_DATA
            session_id,
            len(inner_payload) + 2,  # payload + PPP proto field
        )
        # PPP protocol field
        ppp_proto = struct.pack("!H", ethertype)
        return Ether(
            dst=self.pg0.local_mac,
            src=ac_mac,
            type=0x8864,  # ETHERNET_TYPE_PPPOE_SESSION
        ) / Raw(pppoe_hdr + ppp_proto + inner_payload)

    def test_session_data_plane_ip4(self):
        """IP4 packet through PPPoE session is decapsulated without error."""

        pppox_sw_if_index, session_id, ac_mac = self.establish_session()

        # Build an inner IPv4 ICMP packet (BAS -> client direction)
        inner_ip = bytes(IP(src="10.0.0.1", dst="10.0.0.2") / ICMP(type="echo-request"))

        packet = self._make_pppoe_session_packet(ac_mac, session_id, inner_ip)
        self.pg0.enable_capture()
        self.pg0.add_stream([packet])
        self.pg_start()

        # Verify no session-input errors were bumped
        errors_before = self.statistics.get_err_counter(
            "/err/pppoeclient-session-input/none"
        )
        # Give VPP time to process
        import time

        time.sleep(0.5)
        errors_after = self.statistics.get_err_counter(
            "/err/pppoeclient-session-input/none"
        )
        # The packet should be processed without error (none counter is OK counter)
        # The key check is that no error counters were bumped
        clients = self.vapi.pppoeclient_dump(sw_if_index=self.ALL_INTERFACES)
        self.assertEqual(len(clients), 1)
        self.assertEqual(clients[0].client_state, 2)  # still in SESSION

    def test_session_data_plane_ip6(self):
        """IP6 packet through PPPoE session is decapsulated without error."""
        ...
        packet = self._make_pppoe_session_packet(
            ac_mac, session_id, inner_ip, ethertype=0x0057
        )
        self.pg0.enable_capture()
        self.pg0.add_stream([packet])
        self.pg_start()
    ...
    def test_dispatch_non_pppoe_passthrough(self):
        """Non-PPPoE packet passes through dispatch node unmodified."""

        self.add_client()

        packet = (
            Ether(dst="ff:ff:ff:ff:ff:ff", src=self.pg0.remote_mac, type=0x0800)
        ...
        # The packet should pass through without PPPoE processing
        # Verify no PPPoE error counters were bumped
        import time

        time.sleep(0.5)

    def test_dispatch_pppoe_discovery_forwarding(self):
        """PPPoE discovery packet (0x8863) is dispatched to discovery node."""

        self.add_client()

        padi = self.make_pado("02:00:00:00:00:33", b"test-ac")
        self.pg0.enable_capture()
        self.pg0.add_stream([padi])
        self.pg_start()

        # The PADO should be processed by the discovery node
        # (it will be recorded in control history)
        import time
    ...
    def test_session_data_plane_tcp_mss_clamp(self):
        """TCP SYN through PPPoE session gets MSS clamped."""

        pppox_sw_if_index, session_id, ac_mac = self.establish_session()

        # Build TCP SYN with MSS=1460 inside PPPoE
        tcp_syn = bytes(
            IP(src="10.0.0.1", dst="10.0.0.2")
            / TCP(
                sport=12345,
        ...
        packet = self._make_pppoe_session_packet(ac_mac, session_id, tcp_syn)
        self.pg0.enable_capture()
        self.pg0.add_stream([packet])
        self.pg_start()
