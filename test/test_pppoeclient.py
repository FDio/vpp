...
class TestPPPoEClient(VppTestCase):
    ...
    def wait_for_condition(self, fn, desc, timeout=3):
        ...
        while time.time() < deadline:
            ...
            if ok:
                return detail
            self.sleep(0.1)
        self.fail("Timed out waiting for condition: %s, last detail: %s" % (desc, detail))
    ...
    def test_pppoe_summary_cli_can_filter_by_match_reason(self):
        ...
        def _check_session_filter():
            try:
                out = self.vapi.cli(
                    "show pppoe client summary 0 match-reason session"
                )
            except Exception:
                return False, ""
    ...
    def test_pppoe_control_history_dump_api_reports_recent_packets(self):
        ...
        def _check_latest_two_events():
            ...
            return (
                len(latest_two) == 2
                and [e.code for e in latest_two] == [0x65, 0xA7]
            ), latest_two
