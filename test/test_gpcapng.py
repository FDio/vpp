...
class MockHTTPCaptureHandler(BaseHTTPRequestHandler):
    ...
    def _read_request_data(self, method):
        ...
        if self.headers.get('Transfer-Encoding', '').lower() == 'chunked':
            ...
            self._log('info', f"[HTTP-SERVER] Handling with Content-Length: {content_length}")
    ...
    def _handle_chunked_stream(self):
        ...
        try:
            while True:
                ...
                try:
                    chunk_size = int(size_line.split(';')[0], 16)  # Handle chunk extensions
                except ValueError:
                    self._log('error', f"[HTTP-SERVER] Invalid chunk size: {size_line}")
                    break
                
                if chunk_count < 5:  # Log first few chunks to avoid spam
                    self._log('info', f"[HTTP-SERVER] Chunk {chunk_count}: size={chunk_size}")
                ...
                if chunk_size == 0:
                    ...
                    self._log('info', f"[HTTP-SERVER] End chunk received, stream complete")
                    break
                
                # Read chunk data
                chunk_data = self.rfile.read(chunk_size)
                if len(chunk_data) != chunk_size:
                    self._log('error', f"[HTTP-SERVER] Chunk data size mismatch: expected {chunk_size}, got {len(chunk_data)}")
                    break
                ...
                if chunk_count % 100 == 0:
                    self._log('info', f"[HTTP-SERVER] Processed {chunk_count} chunks, {len(total_data)} bytes total")
                    
        except Exception as e:
            self._log('error', f"[HTTP-SERVER] Error processing chunked stream: {e}")
        
        self._log('info', f"[HTTP-SERVER] Chunked stream complete: {chunk_count} chunks, {len(total_data)} bytes")
        
        return total_data
    
    def _store_and_respond(self, method, data):
        """Store received data and send response"""
        self._log('info', f"[HTTP-SERVER] {method} request to {self.path}, received {len(data)} bytes, headers: {dict(self.headers)}")
        ...
        if hasattr(MockHTTPCaptureHandler, 'save_dir') and MockHTTPCaptureHandler.save_dir:
            try:
                # Create filename based on path and timestamp
                safe_path = self.path.replace('/', '_').replace(':', '_')
                timestamp = int(time.time() * 1000)  # milliseconds
                filename = f"http_capture_{safe_path}_{timestamp}_{len(data)}bytes.pcapng"
                filepath = os.path.join(MockHTTPCaptureHandler.save_dir, filename)
                
                with open(filepath, 'ab') as f:  # append binary mode
                    f.write(data)
                
                self._log('info', f"[HTTP-SERVER] Saved {len(data)} bytes to {filepath}")
            except Exception as e:
        ...
        self._log('info', f"[HTTP-SERVER] {method} response sent: {len(data)} bytes, total received: {len(MockHTTPCaptureHandler.received_data)} bytes")
...
class HTTPCaptureServer:
    ...
    def start(self):
        ...
        self._log('info', f"[HTTP-SERVER] STARTING: Initializing HTTP server on {self.host}:{self.port}")
        ...
        try:
            ...
            self._log('info', f"[HTTP-SERVER] BIND: Successfully bound to {self.host}:{self.port}")
            ...
            self._log('info', f"[HTTP-SERVER] READY: Server thread started and listening on {self.host}:{self.port}")
    ...
    def stop(self):
        ...
        self._log('info', f"[HTTP-SERVER] STOPPING: Shutting down server on {self.host}:{self.port}")
...
class BaseGPCAPNGTestCase(VppTestCase):
    ...
    def add_filter(self, name, interface=None, vni=None, protocol=None, global_filter=False):
...
class TestGPCAPNG(BaseGPCAPNGTestCase):
    ...
    def create_geneve_packet(self, vni=100, inner_payload=None, outer_src="10.0.0.1", outer_dst="10.0.0.2"):
    ...
    def verify_pcapng_file(self, filepath):
        ...
        with open(filepath, 'rb') as f:
            ...
            if len(block_type) >= 4:
                ...
                self.assertIn(block_type_val, [0x0A0D0D0A, 0x0A0D0D0A], 
    ...
    @unittest.skipIf(not can_create_tap_interfaces("pcap-http"), "Test is not running with root privileges")
    def test_http_server_logging(self):
        ...
        self.http_server = HTTPCaptureServer(host='127.0.0.1', logger=self.logger, save_dir=self.http_server_dir)
    ...
    def test_http_destination(self):
        ...
        self.vapi.cli(f"create tap id 0 host-if-name {tap_name} host-ip4-addr {host_ip}/24")
        ...
        self.http_server = HTTPCaptureServer(host=host_ip, logger=self.logger, save_dir=self.http_server_dir)
        self.http_server.start()
        ...
        self.logger.info(f"HTTP destination test: {request_count} requests, {received_bytes} bytes received")
        ...
        if len(received_data) >= 4:
            ...
            self.assertIn(block_type, [0x0A0D0D0A, 0x0A0D0D0A], 
    ...
    def test_http_vs_file_destinations(self):
        ...
        try:
            ...
            self.logger.info("Output assignment CLI not available - testing with default assignments")
        ...
        self.assertTrue(file_exists or http_received, 
...
class TestGPCAPNGMultiWorker(BaseGPCAPNGTestCase):
    ...
    def verify_worker_files(self, base_path, expected_workers):
        ...
        for worker_id in range(1, expected_workers):  # Workers 1, 2, ...
            worker_file = os.path.join(base_dir, f"{name_without_ext}-{worker_id}.pcapng")
        ...
        for filepath, worker_id in files_found:
            file_size = os.path.getsize(filepath)
            self.assertGreater(file_size, 0, f"Worker {worker_id} capture file {filepath} is empty")
    ...
    def test_multi_worker_capture(self):
        ...
        self.logger.info(f"Multi-worker capture completed: {len(worker_files)} files, {total_size} total bytes")
    ...
    def test_worker_distribution(self):
        ...
        if len(worker_files) > 1:
            ...
            self.logger.info("NOTE: All packets processed by single worker (acceptable depending on traffic pattern)")
    ...
    def test_per_worker_filtering(self):
        ...
        self.logger.info(f"Per-worker filtering test completed: {len(worker_files)} worker files created")
    ...
    def test_multi_worker_http_destination(self):
        ...
        self.assertGreater(received_bytes, 0, "No data received by HTTP server from multi-worker test")
        self.assertGreater(request_count, 0, "No HTTP requests received from multi-worker test")
        
        self.logger.info(f"Multi-worker HTTP test: {request_count} requests, {received_bytes} bytes received")
