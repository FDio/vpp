...
class BaseGPCAPNGTestCase(VppTestCase):
    ...
    def setUp(self):
        ...
        # Clean up any leftover gpcapng state from previous tests
        self.cleanup_gpcapng_state()

        # Clear all VPP state to prevent packet leakage between tests
        try:
            self.vapi.cli("clear interfaces")
            self.vapi.cli("clear buffer traces")
            self.vapi.cli("clear node counters")
        except Exception as e:
            self.logger.info(f"Initial cleanup commands failed (may be normal): {e}")
    ...
    def check_counters_before(self):
        ...
        self.logger.info(
            f"=== GPCAPNG destinations at start of {self._testMethodName} ==="
        )
        try:
    ...
    def check_counters_after(self):
        ...
        self.logger.info(
            f"=== GPCAPNG destinations at end of {self._testMethodName} ==="
        )
        try:
            destinations = self.vapi.cli("show gpcapng destination verbose 2")
            self.logger.info(destinations)
    ...
    def show_commands_at_teardown(self):
        self.logger.debug(self.vapi.ppcli("show gpcapng filters"))
        self.logger.debug(self.vapi.ppcli("show node counters"))
        self.check_counters_after()

    def cleanup_gpcapng_state(self):
        """Clean up all gpcapng filters and destinations"""
        try:
            # Get list of existing destinations and filters to clean up
            destinations_output = self.vapi.cli("show gpcapng destination")
            filters_output = self.vapi.cli("show gpcapng filters")
            self.logger.info("CLEANING UP GPCAPNG STATE")

            # Parse and delete all destinations (look for "name: " entries)
            for line in destinations_output.split("\n"):
                self.logger.info(f"DEST: {line}")
                if "name:" in line:
                    name = line.split("name:")[1].strip().split()[0]
                    if name:
                        try:
                            self.vapi.cli(f"gpcapng destination del name {name}")
                            self.logger.info(f"Cleaned up destination: {name}")
                        except Exception as e:
                            self.logger.info(
                                f"Failed to delete destination {name}: {e}"
                            )

            # Parse and delete all filters (look for "name: " entries)
            filter_type = "global"
            for line in filters_output.split("\n"):
                self.logger.info(f"FILT: {line}")
                if "Interface:" in line:
                    ifname = line.split("Interface: ")[1].split(" ")[0]
                    filter_type = f"interface {ifname}"
                if "Filter Name:" in line:
                    name = line.split("Filter Name:")[1].strip().split()[0]
                    if name:
                        try:
                            self.vapi.cli(
                                f"gpcapng filter del name {name} {filter_type}"
                            )
                            self.logger.info(f"Cleaned up filter: {name}")
                        except Exception as e:
                            self.logger.info(f"Failed to delete filter {name}: {e}")

        except Exception as e:
            self.logger.info(f"GPCAPNG state cleanup failed: {e}")

    def tearDown(self):
        # Disable capture on all interfaces first to stop any ongoing capture
        for i in self.pg_interfaces:
            try:
                self.disable_capture(i)
            except:
        ...
        # Clean up gpcapng-specific state
        self.cleanup_gpcapng_state()

        # Clear buffers and interfaces to prevent packet leakage between tests
        try:
            self.vapi.cli("clear interfaces")
            self.vapi.cli("clear buffer traces")
            self.vapi.cli("clear node counters")
        except Exception as e:
            self.logger.info(f"Teardown cleanup failed: {e}")
    ...
    def verify_pcapng_packets(
        self,
        filepath,
        expected_vnis=None,
        unexpected_vnis=None,
        min_packets=None,
        max_packets=None,
    ):
        ...
        try:
            ...
            self.logger.info(
                f"CAPTURE-VERIFY: Analyzing {packet_count} packets in {filepath}"
            )
            ...
            self.logger.info(
                f"CAPTURE-VERIFY: Found {len(geneve_packets)} GENEVE packets, {len(non_geneve_packets)} non-GENEVE"
            )
            ...
            if expected_vnis is not None:
                for expected_vni in expected_vnis:
                    ...
                    self.assertGreater(
                        vni_counts[expected_vni],
                        0,
                        f"Expected VNI {expected_vni} has 0 packets",
                    )
                    self.logger.info(
                        f"CAPTURE-VERIFY: VNI {expected_vni}: {vni_counts[expected_vni]} packets captured"
                    )
            ...
            if unexpected_vnis is not None:
                for unexpected_vni in unexpected_vnis:
                    ...
                    self.logger.info(
                        f"CAPTURE-VERIFY: VNI {unexpected_vni}: correctly excluded"
                    )
    ...
    def verify_no_packets_captured(self, filepath, allow_headers_only=True):
        ...
        if not os.path.exists(filepath):
            self.logger.info(
                f"CAPTURE-VERIFY: No capture file created at {filepath} - no packets captured"
            )
            return
        ...
        if allow_headers_only and file_size < 1024:  # Small file, likely just headers
            self.logger.info(
                f"CAPTURE-VERIFY: Capture file {filepath} is small ({file_size} bytes) - likely headers only"
            )
            return
        ...
        try:
            ...
            self.logger.info(
                f"CAPTURE-VERIFY: Capture file contains no packets as expected"
            )
        except Exception as e:
            # If we can't read it, assume it's headers-only or empty
            if allow_headers_only:
                self.logger.info(
                    f"CAPTURE-VERIFY: Capture file unreadable as packets - likely headers only: {e}"
                )
            else:
...
class TestGPCAPNG(BaseGPCAPNGTestCase):
    ...
    def tearDown(self):
        ...
        # Disable capture on all interfaces using gpcapng-specific command
        for i in self.pg_interfaces:
            try:
    ...
    def test_vni_filtering(self):
        ...
        # Send all packets
        self.pg0.add_stream(packets_no_match + packets_match + packets_no_match)
        self.pg_enable_capture(self.pg_interfaces)
        self.pg_start()
        ...
        self.verify_pcapng_packets(
            capture_file, expected_vnis=[200], unexpected_vnis=[100], min_packets=100
        )
    ...
    def test_http_destination_queue_management(self):
        ...
        while operation_count < max_operations and (time.time() - start_time) < 5.0:
            ...
            should_add = (
                len(active_destinations) == 0
                or (len(active_destinations) < 4 and random.random() < 0.7)
            ...
            if should_add and len(nonexistent_urls) > 0:
                ...
                self.logger.info(
                    f"Operation {operation_count}: Adding destination {dest_name}: {url}"
                )
                try:
                ...
                self.logger.info(
                    f"Operation {operation_count}: Deleting destination {dest_name}"
                )
                try:
            ...
            if operation_count % 5 == 0:
                self.logger.info(
                    f"Current active destinations: {len(active_destinations)}"
                )
        ...
        self.logger.info(
            f"HTTP destination queue management test completed in {elapsed_time:.2f} seconds"
        )
        self.logger.info(
            f"Total operations: {operation_count}, Cleaned up: {cleanup_count}"
        )
        self.logger.info(
            "Queue structures should have been properly managed during rapid operations"
        )
...
class TestGPCAPNGMultiWorker(BaseGPCAPNGTestCase):
    ...
    def test_multi_worker_http_destination(self):
        ...
        self.set_filter_destination("mw-http-filter", "mw-http-dest")
