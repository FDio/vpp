...
class TestVPPInterfacesQemu(VppTestCase):
    ...
    def create_af_packet(self, version, host_if_name, enable_gso=0):
        ...
        if version == 2:
            try:
                af_packet_interface_flags = (
                    af_packet_interface_flags
                    | af_packet_flags.AF_PACKET_API_FLAG_VERSION_2
                )
            # Use the older API version to create af_packet_v2
    ...
    def test_vpp_interfaces(self):
        ...
        try:
            for test in tests:
                for ip_version in test_config["ip_versions"]:
                    ...
                    for mtu in test_config["mtus"]:
                        ...
                        if result is True:
                            print(
                                f"Running Test: id:{test['id']} "
                                f"client_{test['client_if_type']}"
                                f"_v{test['client_if_version']} "
                                f"gso:{test['client_if_gso']} "
                                f"gro:{test.get('client_if_gro', 0)} "
                                f"--> server_{test['server_if_type']}"
                                f"_v{test['server_if_version']} "
                                f"gso:{test.get('server_if_gso', 0)} "
                                f"gro:{test.get('server_if_gro', 0)} "
            ...
            self.tearDown()
            raise Exception("Exception running test:", e.output)
