...
class TestIPv4FibCrud(VppTestCase):
    ...
    def config_fib_many_to_one(self, start_dest_addr, next_hop_addr, count):
        ...
        for i in range(count):
            r = VppIpRoute(self, start_dest_addr % i, 32,
    ...
    def create_stream(self, src_if, dst_if, routes, count):
        ...
        for _ in range(count):
            dst_addr = random.choice(routes).prefix.addr
            info = self.create_packet_info(src_if, dst_if)
            payload = self.info_to_payload(info)
    ...
    def verify_route_dump(self, routes):
        for r in routes:
            self.assertTrue(find_route(self, r.prefix.addr, r.prefix.len))

    def verify_not_in_route_dump(self, routes):
        for r in routes:
            self.assertFalse(find_route(self, r.prefix.addr, r.prefix.len))
    ...
    def test__routes(self):
        ...
        self.deleted_routes.extend(self.unconfig_fib_many_to_one(
            "10.0.0.%d", self.pg0.remote_ip4, 10))
        ...
        tmp = self.config_fib_many_to_one(
            "10.0.0.%d", self.pg0.remote_ip4, 5)
...
class TestIPDeag(VppTestCase):
    ...
    def test_ip_deag(self):
        ...
        self.logger.error(self.vapi.cli("sh ip fib 1.1.1.1/32"))
