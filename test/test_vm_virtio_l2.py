...
# Create virtio interfaces manually with native plugin (DPDK disabled)
echo "Creating virtio interfaces with native plugin (pre-refacto version)..."
...
# List interfaces to see what was created
echo "Interfaces created:"
{vppctl_bin} -s /tmp/vpp_cli.sock show interface

# Configure L2 xconnect
echo "Configuring L2 xconnect..."
...
echo "=== VPP L2 xconnect configured ==="
{vppctl_bin} -s /tmp/vpp_cli.sock show interface
{vppctl_bin} -s /tmp/vpp_cli.sock show mode
{vppctl_bin} -s /tmp/vpp_cli.sock show l2fib verbose
...
"""
    ...
    def test_virtio_l2_xconnect_ipv4(self):
        ...
        # Log QEMU guest output for debugging
        self.logger.info("=== Reading QEMU guest output ===")
        if hasattr(self, "qemu_log_file") and os.path.exists(self.qemu_log_file):
            with open(self.qemu_log_file, "r") as f:
                qemu_output = f.read()
                self.logger.info(f"QEMU Guest output:\n{qemu_output}")
        else:
            self.logger.warning("No QEMU log file found")

        # Check VPP interface stats on host
        self.logger.info("=== Checking VPP stats on host ===")
        self.logger.info("VPP interfaces on host:")
        host_if_stats = self.vapi.cli("show interface")
        self.logger.info(host_if_stats)

        self.assertTrue(result, "iPerf failed - no traffic through QEMU/VPP")
        self.logger.info("SUCCESS: Traffic passed through virtio L2 xconnect!")
