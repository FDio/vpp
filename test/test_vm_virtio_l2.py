...
"""
    ...
    def launch_qemu(self, tap1, tap2, kernel, initrd):
        ...
        qemu_cmd = [
            ...
            # Kernel command line
            "-append",
            f"rw root=fsRoot rootfstype=9p rootflags=trans=virtio,cache=mmap console=ttyS0 init={init_script}",
        ]
    ...
    def test_virtio_l2_xconnect_ipv4(self):
        ...
        Set these environment variables before running:
          export KERNEL_IMAGE=/boot/vmlinuz-6.8.0-110-generic
          export INITRD_IMAGE=/boot/initrd.img-6.8.0-110-generic

        Or let the test auto-detect the running kernel/initrd.
        ...
        try:
            ...
            self.skipTest("qemu-system-x86_64 not found or not working")

        # Auto-detect or use provided kernel/initrd
        kernel = os.environ.get("KERNEL_IMAGE")
        initrd = os.environ.get("INITRD_IMAGE")

        if not kernel:
            # Try to auto-detect kernel
            ...
            if not os.path.exists(kernel):
                ...
                if not os.path.exists(kernel):
                    self.skipTest(
                        f"Kernel not found. Set KERNEL_IMAGE environment variable."
                    )

        if not initrd:
            # Try to auto-detect initrd
            initrd = f"/boot/initrd.img-{os.uname().release}"
            if not os.path.exists(initrd):
                # Try symlink
                initrd = "/boot/initrd.img"
                if not os.path.exists(initrd):
                    self.skipTest(
                        f"Initrd not found. Set INITRD_IMAGE environment variable."
                    )
