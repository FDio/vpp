...
class TestClassifier(VppTestCase):
    ...
    def _resolve_mask_match(self, mask_match):
        mask_match = binascii.unhexlify(mask_match)
        self.mask_match_len = ((len(mask_match) - 1) // 16 + 1) * 16
        self.mask_match = mask_match + b'\0' * \
            (self.mask_match_len - len(mask_match))
    ...
    def create_classify_table(self, key, mask, data_offset=0):
        ...
        """
        self._resolve_mask_match(mask)
        r = self.vapi.classify_add_del_table(
            is_add=1,
            mask=self.mask_match,
            mask_len=self.mask_match_len,
