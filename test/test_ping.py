...
class TestPing(VppTestCase):
    ...
    def test_ping_src(self):
        ...
        # Add additional address to interface, and do ping
        # requesting specific interface source address
        VppIpInterfaceAddress(self, self.pg1, "10.0.0.11", 24).add_vpp_config()

        self.pg_enable_capture(self.pg_interfaces)
        self.pg_start()
        ping_cmd = (
            "ping %s interval 0.01 repeat 3 source pg1 src-address 10.0.0.11" % nbr_addr
        )
        ret = self.vapi.cli(ping_cmd)
        out = self.pg1.get_capture(3)
        icmp_seq = 1
    ...
    def test_ping_api(self):
        ...
        try:
            ...
            for p in out:
                icmp = self.verify_ping_request(
                    p, "10.0.0.12", self.pg1.remote_ip4, icmp_seq
                )
                icmp_seq = icmp_seq + 1
                if icmp_id is None:
                    icmp_id = icmp.id
