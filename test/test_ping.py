...
class TestPing(VppTestCase):
    ...
    def test_ping_basic(self):
        ...
        try:
            ...
            ret = self.vapi.cli("ping 172.16.2.2 interval 0.01 repeat 10")
            self.logger.info(ret)
            out = self.pg1.get_capture(10)
            icmp_id = None
            icmp_seq = 1
            for p in out:
               # self.logger.info(p.show())
               ip = p[IP]
               self.assertEqual(ip.version, 4)
               self.assertEqual(ip.flags, 0)
               self.assertEqual(ip.src, "172.16.2.1")
               self.assertEqual(ip.dst, "172.16.2.2")
    ...
    def test_ping_burst(self):
        ...
        try:
            ...
            ret = self.vapi.cli("ping 172.16.2.2 interval 0.01 burst 3")
            self.logger.info(ret)
