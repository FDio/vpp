...
class BaseNpolTest(VppTestCase):
    ...
    def vapi_npol_match(self, sw_if_index, pkt, direction):
        ...
        return self.vapi.cli(f"npol match {sw_if_index} {direction} {ipv} {proto} " +
    ...
    def send_test_packets(self, from_if, to_if, passing_packets, dropped_packets):
        for pkt in passing_packets:
            self.assertTrue(self.vapi_npol_match(from_if, pkt, "inbound").strip().endswith("action:ALLOW"))
            self.assertTrue(self.vapi_npol_match(to_if, pkt, "outbound").strip().endswith("action:ALLOW"))
        for pkt in dropped_packets:
            self.assertTrue(
                self.vapi_npol_match(from_if, pkt, "inbound").strip().endswith("action:DENY") or
                self.vapi_npol_match(to_if, pkt, "outbound").strip().endswith("action:DENY")
            )
        ...
        #         from_if, dropped_packets, to_if, timeout=0.1
        #     )
...
class TestNpolMatches(BaseNpolTest):
    ...
    def test_ipport_ipset(self, is_ip6=False):
        ...
        invalid = [  # Change all criteria: address, proto, port
            self.base_ip_packet(is_ip6, second_dst_ip=True)
            / TCP(sport=src_l4, dport=dst_l4)
            / random_payload(),
            self.base_ip_packet(is_ip6)
            / UDP(sport=src_l4, dport=dst_l4)
        ...
        invalid = [  # Change all criteria: address, proto, port
            self.base_ip_packet(is_ip6, second_src_ip=True)
            / TCP(sport=src_l4, dport=dst_l4)
            / random_payload(),
            self.base_ip_packet(is_ip6)
            / UDP(sport=src_l4, dport=dst_l4)
...
class TestNpolPolicies(BaseNpolTest):
    ...
    def test_realistic_policy(self):
        ...
        dropped = [
            ...
            self.base_ip_packet(second_src_ip=True)
