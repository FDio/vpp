...
class VppNpolPolicy(VppObject):
    ...
    def add_vpp_config(self):
        ...
        self._test.logger.info(self._test.vapi.cli(
...
class BaseNpolTest(VppTestCase):
    ...
    def vapi_npol_match(self, sw_if_index, pkt, direction):
        ...
        return self.vapi.cli(f"npol match {sw_if_index} {direction} {ipv} "
                             f"{proto} " +
                             f"{src_addr};{src_port}->{dst_addr};{dst_port}")

    def send_test_packets(self, from_if, to_if, passing_packets, drop_packets):
        for pkt in passing_packets:
            self.assertTrue(self.vapi_npol_match(
                from_if, pkt, "inbound").strip().endswith("action:ALLOW"))
            self.assertTrue(self.vapi_npol_match(
                to_if, pkt, "outbound").strip().endswith("action:ALLOW"))
        for pkt in drop_packets:
            self.assertTrue(
                self.vapi_npol_match(
                    from_if, pkt, "inbound"
                ).strip().endswith("action:DENY") or
                self.vapi_npol_match(
                    to_if, pkt, "outbound"
                ).strip().endswith("action:DENY")
            )
...
class TestNpolMatches(BaseNpolTest):
    ...
    def npol_test_icmp(self, is_v6):
        ...
        filters = [
            VppNpolFilter(FILTER_TYPE_ICMP_TYPE,
                          value=icmp_type, should_match=1)
        ]
        valid = (
        ...
        invalid = (
            self.base_ip_packet(is_v6) / ICMP46(type=11,
                                                code=22) / random_payload()
        )
        self.do_test_one_rule(filters, [], [valid], [invalid])

        # Define filter on ICMP type  / should match = 0
        filters = [VppNpolFilter(
            FILTER_TYPE_ICMP_TYPE, value=11, should_match=0)]
        invalid = (
            self.base_ip_packet(is_v6)
    ...
    def npol_test_icmp_code(self, is_v6):
        ...
        # Define filter on ICMP type
        filters = [
            VppNpolFilter(FILTER_TYPE_ICMP_CODE,
                          value=icmp_code, should_match=1)
        ]
        valid = (
            self.base_ip_packet(is_v6)
            / ICMP46(type=icmp_type, code=icmp_code)
        ...
        filters = [
            VppNpolFilter(FILTER_TYPE_ICMP_CODE,
                          value=icmp_code, should_match=0)
        ]
        valid = (
            self.base_ip_packet(is_v6)
            / ICMP46(type=icmp_type, code=icmp_code + 1)
    ...
    def npol_test_l4proto(self, is_v6, l4proto):
        ...
        filters = [
            VppNpolFilter(FILTER_TYPE_L4_PROTO,
                          value=filter_value, should_match=1)
        ]
        ...
        invalid = self.base_ip_packet(
            is_v6) / ICMP(type=8, code=3) / random_payload()
        self.do_test_one_rule(filters, [], [valid], [invalid])
    ...
    def test_prefixes(self, is_ip6=False):
        def pload(): return TCP(sport=src_l4, dport=dst_l4) / random_payload()
        dst_ip_match = (
            self.pg1.remote_ip6 + "/128" if is_ip6
            else self.pg1.remote_ip4 + "/32"
        )
        match = {
            "is_src": False,
            "is_not": False,
        ...
        src_ip_match = (
            self.pg0.remote_ip6 + "/128" if is_ip6
            else self.pg0.remote_ip4 + "/32"
        )
        match = {
            "is_src": True,
            "is_not": False,
    ...
    def test_port_ranges(self, is_ip6=False):
        ...
        invalid = [
            base / TCP(sport=test_port, dport=test_port + 1) /
            random_payload(),
            base / TCP(sport=test_port, dport=test_port - 1) /
            random_payload(),
        ]
        ...
        valid = base / TCP(sport=test_port, dport=test_port) / random_payload()
        invalid = [
            base / TCP(sport=test_port + 1, dport=test_port) /
            random_payload(),
            base / TCP(sport=test_port - 1, dport=test_port) /
            random_payload(),
        ]
        ...
        valid = [
            ...
            base / TCP(sport=test_port, dport=test_port + 5) /
            random_payload(),
            base / TCP(sport=test_port, dport=test_port + 10) /
            random_payload(),
            base / UDP(sport=test_port, dport=test_port) / random_payload(),
            base / UDP(sport=test_port, dport=test_port + 5) /
            random_payload(),
            base / UDP(sport=test_port, dport=test_port + 10) /
            random_payload(),
        ]
        invalid = [
            base / TCP(sport=test_port, dport=test_port - 1) /
            random_payload(),
            base / TCP(sport=test_port, dport=test_port + 11) /
            random_payload(),
        ]
        self.do_test_one_rule([], [match], valid, invalid)
...
class TestNpolPolicies(BaseNpolTest):
    ...
    def test_inbound_outbound(self):
        ...
        r.add_vpp_config()
        pin = VppNpolPolicy(
            self, [VppNpolPolicyItem(is_inbound=1, rule_id=r.vpp_id())])
        pout = VppNpolPolicy(
        ...
        matching = (
            self.base_ip_packet() / TCP(sport=1, dport=1000) / random_payload()
        )
        not_matching = (
        ...
        self.send_test_packets(self.pg0, self.pg1, [
    ...
    def test_default_verdict(self):
        ...
        passing = [self.base_ip_packet() / TCP(sport=1, dport=1000) /
                   random_payload()]
        dropped = [self.base_ip_packet() / TCP(sport=1, dport=2000) /
                   random_payload()]
    ...
    def test_realistic_policy(self):
        ...
        passing = [
            ...
            self.base_ip_packet() / TCP(sport=1, dport=8080)
            / random_payload(),
        ]
        dropped = [
            ...
            / random_payload(),
            self.base_ip_packet() / UDP(sport=1, dport=8080)
            / random_payload(),
            self.base_ip_packet() / TCP(sport=1, dport=8081)
            / random_payload(),
        ]
        self.send_test_packets(self.pg0, self.pg1, passing, dropped)
        # Cleanup
