...
3. **VPP**:
   ...
   - Configures L2 xconnect between virtio0 and virtio1
   - Traffic flows: iPerf client → vtap0 → virtio0 → VPP L2 xconnect → virtio1 → vtap1 → iPerf server

## Test Files

- `test_vm_virtio_l2.py` - L2 xconnect test (test ID 30)
...
- **Test 30**: virtio L2 xconnect, no GSO/GRO (tested and validated)

Note: Tests 31 and 32 are defined in `vm_test_config.py` but are not yet implemented or validated.
...
- Verify L2 xconnect configuration in VPP: `show mode`
- Check namespace connectivity: ping between namespaces
