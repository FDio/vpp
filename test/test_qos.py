...
from vpp_papi_provider import QOS_MARK_SOURCE
...
class TestQOS(VppTestCase):
    ...
    def test_qos_ip(self):
        ...
        #
        self.vapi.qos_egress_map_interface_bind_unbind(self.pg1.sw_if_index,
                                                       QOS_MARK_SOURCE.IP,
                                                       map_id=1,
                                                       is_bind=1)
        self.vapi.qos_egress_map_interface_bind_unbind(self.pg2.sw_if_index,
                                                       QOS_MARK_SOURCE.IP,
                                                       map_id=2,
                                                       is_bind=1)
        self.vapi.qos_egress_map_interface_bind_unbind(self.pg3.sw_if_index,
                                                       QOS_MARK_SOURCE.IP,
                                                       map_id=3,
                                                       is_bind=1)
        self.vapi.qos_egress_map_interface_bind_unbind(self.pg4.sw_if_index,
                                                       QOS_MARK_SOURCE.IP,
                                                       map_id=4,
                                                       is_bind=1)
        ...
        #
        # Enab;e QoS recrding on IP input for pg0
        #
        self.vapi.ip_qos_record_enable_disable(self.pg0.sw_if_index, 1)
        ...
        #
        self.vapi.ip_qos_record_enable_disable(self.pg0.sw_if_index, 0)
    ...
    def test_qos_vlan(self):
        ...
        sub_if = VppDot1QSubint(self, self.pg2, 92)
        sub_if.admin_up()
        sub_if.config_ip4()
        sub_if.resolve_arp()
        ...
        self.vapi.qos_egress_map_update(1, rows)
        self.vapi.ip_qos_record_enable_disable(self.pg0.sw_if_index, 1)
        self.vapi.qos_egress_map_interface_bind_unbind(sub_if.sw_if_index,
                                                       QOS_MARK_SOURCE.VLAN,
                                                       map_id=1,
                                                       is_bind=1)
        self.logger.info(self.vapi.cli("sh int"))
        self.logger.error(self.vapi.cli("sh int feat pg2.92"))

        p = (Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac) /
             IP(src=self.pg0.remote_ip4, dst=sub_if.remote_ip4, tos=1) /
             UDP(sport=1234, dport=1234) /
             Raw(chr(100) * 65))

        #
        # 3 bits of tos in Dot1Q, so expect value 7.
        #
        rx = self.send_and_expect(self.pg0, p * 65, self.pg2)
        for p in rx:
            p.show()
            self.assertEqual(p[Dot1Q].prio, 7)
            self.assertEqual(p[Dot1Q].id, 0)
            self.assertEqual(p[Dot1Q].vlan, 92)
            self.assertEqual(p[Dot1Q].type, 0x800)
