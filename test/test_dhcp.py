...
class TestDHCP(VppTestCase):
    ...
    def test_dhcp_proxy(self):
        ...
        #
        self.pg3.add_stream(pkts_disc)
        self.pg_enable_capture(self.pg_interfaces)
        self.pg_start()

        rx = self.p10.get_capture(1)
        rx = rx[0]
        self.verify_dhcp_request(rx, self.pg0, src_intf=self.pg3)
    ...
    def test_dhcp6_proxy(self):
        ...
        # VPP expects all routers mcast, not all DHCP servers/relays
        dhcp_disc_dst="ff02::2"
        dhcp_disc_src=mk_ll_addr(self.pg2.remote_mac)

        dmac = in6_getnsmac(inet_pton(socket.AF_INET6, dhcp_disc_dst))
        p_disc = (Ether(dst=dmac, src=self.pg2.remote_mac) /
                  IPv6(src=dhcp_disc_src, dst=dhcp_disc_dst) /
        ...
        #
        server_addr = socket.inet_pton(AF_INET6, self.pg0.remote_ip6)
        src_addr = socket.inet_pton(AF_INET6, self.pg0.local_ip6)
        self.vapi.dhcp_proxy_config(server_addr,
                                    src_addr,
                                    rx_table_id=0,
                                    server_table_id=0,
