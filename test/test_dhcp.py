...
class TestDHCP(VppTestCase):
    ...
    def verify_dhcp6_solicit(self, pkt, intf, peer):
        ether = pkt[Ether]
        self.assertEqual(ether.dst, intf.remote_mac)
        self.assertEqual(ether.src, intf.local_mac)
        ...
        self.assertEqual(in6_ptop(relay.peeraddr), in6_ptop(peer))
    ...
    def verify_dhcp6_advert(self, pkt, intf):
        pkt.show()
        ether = pkt[Ether]
        self.assertEqual(ether.dst, "ff:ff:ff:ff:ff:ff")
        self.assertEqual(ether.src, intf.local_mac)
        ...
        ip = pkt[IPv6]
        self.assertEqual(in6_ptop(ip.dst), "::")
        self.assertEqual(in6_ptop(ip.src), in6_ptop(intf.local_ip6))

        udp = pkt[UDP]
        self.assertEqual(udp.dport, 546)
        self.assertEqual(udp.sport, 547)
    ...
    def test_dhcp_proxy(self):
        ...
        #
        p_disc_vrf0 = (Ether(dst="ff:ff:ff:ff:ff:ff", src=self.pg2.remote_mac) /
                       IP(src="0.0.0.0", dst="255.255.255.255") /
                       UDP(sport=68, dport=67) /
                       BOOTP(op=1) /
                       DHCP(options=[('message-type','discover'), ('end')]))
        pkts_disc_vrf0 = [p_disc_vrf0]
        p_disc_vrf1 = (Ether(dst="ff:ff:ff:ff:ff:ff", src=self.pg3.remote_mac) /
                       IP(src="0.0.0.0", dst="255.255.255.255") /
                       UDP(sport=68, dport=67) /
                       BOOTP(op=1) /
                       DHCP(options=[('message-type','discover'), ('end')]))
        ...
        #
        server_addr = socket.inet_pton(AF_INET, self.pg1.remote_ip4)
        src_addr = socket.inet_pton(AF_INET, self.pg1.local_ip4)
        self.vapi.dhcp_proxy_config(server_addr,
                                    src_addr,
                                    rx_table_id=1,
                                    server_table_id=1,
        ...
        rx = rx[0]
        self.verify_dhcp_request(rx, self.pg1, src_intf=self.pg3)
    ...
    def test_dhcp6_proxy(self):
        ...
        server_addr_vrf0 = socket.inet_pton(AF_INET6, self.pg0.remote_ip6)
        src_addr_vrf0 = socket.inet_pton(AF_INET6, self.pg0.local_ip6)
        server_addr_vrf1 = socket.inet_pton(AF_INET6, self.pg1.remote_ip6)
        src_addr_vrf1 = socket.inet_pton(AF_INET6, self.pg1.local_ip6)
        ...
        # Packets still dropped because the client facing interface has no  IPv6 config
        #
        ...
        rx = rx[0]
        self.verify_dhcp6_advert(rx, self.pg3)
