...
START_TEST (test_invalid_values)
{
  ...
  rv = vapi_send (ctx, sv, vapi_calc_show_version_msg_size (sv));
  ck_assert_int_eq (VAPI_EINVAL, rv);
  rv =
    vapi_connect (ctx, app_name, api_prefix, queue_size, VAPI_MODE_BLOCKING);
  ck_assert_int_eq (VAPI_OK, rv);
  rv = vapi_send (ctx, NULL, 0);
  ck_assert_int_eq (VAPI_EINVAL, rv);
  rv = vapi_send (NULL, NULL, 0);
  ck_assert_int_eq (VAPI_EINVAL, rv);
...
setup_nonblocking (void)
{
  vapi_error_e rv = vapi_ctx_alloc (&ctx);
  ck_assert_int_eq (VAPI_OK, rv);
  rv = vapi_connect (ctx, app_name, api_prefix, queue_size,
...
START_TEST (test_show_version_1)
{
  ...
  vapi_error_e rv = vapi_send (ctx, sv, vapi_calc_show_version_msg_size (sv));
  ck_assert_int_eq (VAPI_OK, rv);
...
START_TEST (test_show_version_2)
{
  ...
  const int attempts = queue_size * 4;
  int i = 0;
