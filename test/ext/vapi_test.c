...
START_TEST (test_loopbacks_1)
{
  ...
  u8 name_filter[sizeof
		 (*((vapi_payload_sw_interface_dump *) (NULL))->name_filter)]
    = { 0 };
  vapi_error_e rv;
  while (VAPI_EAGAIN ==
...
START_TEST (test_loopbacks_2)
{
  ...
  while (VAPI_EAGAIN ==
	 (rv =
	  vapi_sw_interface_dump (ctx, sw_interface_dump_cb, &dctx, 0,
				  name_filter)))
    ;
  for (int i = 0; i < num_ifs; ++i)
    {
...
vapi_error_e
stats_cb (vapi_ctx_t ctx, void *callback_ctx,
	  vapi_error_e rv, bool is_last,
...
{
  printf
    ("--- Receive show version using generic callback - nonblocking API ---\n");
  vapi_error_e rv;
  vapi_msg_show_version *sv = vapi_msg_alloc (ctx, sizeof (*sv));
  ck_assert_ptr_ne (NULL, sv);
  vapi_msg_init_show_version (ctx, sv);
