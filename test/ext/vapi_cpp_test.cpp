...
#include <setjmp.h>
#include <check.h>
#include <vpp-api/vapi/vapi.hpp>
#include <vpe.api.vapi.hpp>
#include <interface.api.vapi.hpp>
...
void verify_show_version_reply (const Vapi_show_version_reply *r)
{
...
struct Show_version_cb
{
  ...
  int called;
  vapi_error_e operator() (Vapi_show_version *sv)
  {
    auto r = sv->get_response ();
    verify_show_version_reply (r);
    ++called;
...
START_TEST (test_show_version_2)
{
  ...
  Show_version_cb cb;
  Vapi_show_version sv (&con, std::ref (cb));
  vapi_error_e rv = sv.execute ();
  ck_assert_int_eq (VAPI_OK, rv);
  con.dispatch (sv);
...
struct Create_loopback_cb
{
  ...
  u32 sw_if_index;
  bool seen;
  vapi_error_e operator() (Vapi_create_loopback *cl)
  {
...
struct Delete_loopback_cb
{
  ...
  u32 sw_if_index;
  bool seen;
  vapi_error_e operator() (Vapi_delete_loopback *dl)
  {
...
template <int num_ifs> struct Sw_interface_dump_cb
{
  ...
  vapi_error_e operator() (Vapi_sw_interface_dump *d)
  {
    for (auto &y : cbs)
      {
        y.seen = false;
...
START_TEST (test_loopbacks_2)
{
  ...
  std::array<std::unique_ptr<Vapi_create_loopback>, num_ifs> clcs;
  for (int i = 0; i < num_ifs; ++i)
    {
      Vapi_create_loopback *cl =
          new Vapi_create_loopback (&con, std::ref (ccbs[i]));
      clcs[i].reset (cl);
      auto p = cl->get_request ()->get_payload ();
      memcpy (p->mac_address, mac_addresses[i], sizeof (p->mac_address));
      auto e = cl->execute ();
  ...
  Vapi_sw_interface_dump d (&con, std::ref (swdcb));
  auto p = d.get_request ()->get_payload ();
  p->name_filter_valid = 0;
  memset (p->name_filter, 0, sizeof (p->name_filter));
  auto rv = d.execute ();
  ...
  std::array<std::unique_ptr<Vapi_delete_loopback>, num_ifs> dlcs;
  for (int i = 0; i < num_ifs; ++i)
    {
      Vapi_delete_loopback *dl =
          new Vapi_delete_loopback (&con, std::ref (dcbs[i]));
      dlcs[i].reset (dl);
...
START_TEST (test_stats_1)
{
  ...
  Vapi_want_stats ws (&con);
  auto payload = ws.get_request ()->get_payload ();
  payload->enable_disable = 1;
  payload->pid = getpid ();
  auto rv = ws.execute ();
  ck_assert_int_eq (VAPI_OK, rv);
  Vapi_event_registration<Vapi_vnet_interface_simple_counters> sc (&con);
...
struct Vnet_interface_simple_counters_cb
{
  ...
  int called;
  vapi_error_e
  operator() (Vapi_event_registration<Vapi_vnet_interface_simple_counters> *e)
  {
    ++called;
...
START_TEST (test_stats_2)
{
  ...
  Vapi_event_registration<Vapi_vnet_interface_simple_counters> sc (
      &con, std::ref (cb));
  rv = con.wait_for_response (sc);
  ck_assert_int_eq (VAPI_OK, rv);
...
START_TEST (test_stats_3)
{
  ...
  ck_assert_int_eq (VAPI_OK, rv);
  Vapi_event_registration<Vapi_vnet_interface_simple_counters> sc (&con);
  Vapi_event_registration<Vapi_vnet_interface_combined_counters> cc (&con);
  rv = con.wait_for_response (sc);
  ck_assert_int_eq (VAPI_OK, rv);
...
START_TEST (test_unsupported)
{
  ...
  try
    {
      Vapi_test_fake_msg fake (&con);
    }
  catch (const Vapi_msg_not_available_exception &)
    {
      thrown = true;
  ...
  try
    {
      Vapi_event_registration<Vapi_test_fake_details> fake (&con);
    }
  catch (const Vapi_msg_not_available_exception &)
    {
      thrown = true;
