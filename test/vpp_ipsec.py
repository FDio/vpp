...
class VppIpsecSpdEntry(VppObject):
    ...
    def __init__(self, test, spd, sa_id,
        ...
        self.spd = spd
        self.sa_id = sa_id
        self.local_start = ip_address(unicode(local_start))
        self.local_stop = ip_address(unicode(local_stop))
        self.remote_start = ip_address(unicode(remote_start))
        self.remote_stop = ip_address(unicode(remote_stop))
        self.proto = proto
...
class VppIpsecSA(VppObject):
    ...
    def __init__(self, test, id, spi,
        ...
        if (tun_src):
            self.tun_src = ip_address(unicode(tun_src))
            self.is_tunnel = 1
            if (self.tun_src.version == 6):
                self.is_tunnel_v6 = 1
        if (tun_dst):
            self.tun_dst = ip_address(unicode(tun_dst))
