...
class FibPathProto:
    ...
    FIB_PATH_NH_PROTO_IP6 = 1
    FIB_PATH_NH_PROTO_MPLS = 2
    FIB_PATH_NH_PROTO_BIER = 3
    FIB_PATH_NH_PROTO_ETHERNET = 4
...
def find_route(test, ip_addr, len, table_id=0):
    ...
    for e in routes:
        if len == e.route.prefix.address_length \
           ...
           and compare_ip_address(e.route.prefix.address.un, ip_addr):
            return True
    return False
...
def find_mroute(test, grp_addr, src_addr, grp_addr_len,
    ...
    for e in routes:
        if table_id == e.route.table_id \
           and grp_addr_len == e.route.prefix.grp_address_length \
           and compare_ip_address(e.route.prefix.grp_address, ip_gaddr) \
           and compare_ip_address(e.route.prefix.src_address, ip_saddr):
            return True
    return False
...
class VppMFibSignal(object):
    ...
    def compare(self, signal):
        ...
        self.test.assertEqual(self.route.prefix.len,
                              signal.prefix.grp_address_length)
        if 4 is self.route.prefix.ip_saddr.version:
            for i in range(4):
                self.test.assertEqual(self.route.prefix.ip_gaddr.packed[i],
                                      signal.prefix.grp_address.ip4.address[i])
                self.test.assertEqual(self.route.prefix.ip_saddr.packed[i],
                                      signal.prefix.src_address.ip4.address[i])
        else:
            for i in range(16):
                self.test.assertEqual(self.route.prefix.ip_gaddr.packed[i],
                                      signal.prefix.grp_address.ip6.address[i])
                self.test.assertEqual(self.route.prefix.ip_saddr.packed[i],
                                      signal.prefix.src_address.ip6.address[i])
