...
class TestInnerAwarePerf(VppTestCase):
    ...
    def tearDownClass(cls):
        try:
            results_path = os.path.join(
                config.tmp_dir, "test_inner_aware_perf.json"
            )
    ...
    def _set_peek_inner(self, on):
        ...
        self.vapi.cli(
            "set ip flow-hash table 0 src dst sport dport proto %s" % kw
        )
        self.vapi.cli(
            "set ip6 flow-hash table 0 src dst sport dport proto %s" % kw
        )
    ...
    def _build_plain_v4(self, rng):
        return (
            IP(src=_rand_v4(rng), dst="203.0.113.5")
    ...
    def _build_plain_v6(self, rng):
        return (
            IPv6(src="2001:db8:dead::1", dst="2001:db8:beef::5")
            / UDP(sport=rng.randint(1024, 65535),
                  dport=rng.randint(1024, 65535))
        )
    ...
    def _build_ipinip(self, rng):
        ...
        outer = IP(src="192.0.2.1", dst="203.0.113.5",
    ...
    def _build_nvgre(self, rng):
        ...
        inner = inner_eth / IP(
            src=_rand_v4(rng), dst=_rand_v4(rng)
        ) / UDP(sport=rng.randint(1024, 65535),
                dport=rng.randint(1024, 65535))
    ...
    def _parse_runtime(runtime, node_names):
        ...
        for line in runtime.splitlines():
            ...
            try:
                ...
                continue
            out[name] = {"calls": calls, "vectors": vectors,
                         "clocks": clocks_per_pkt}
        return out
    ...
    def _measure(self, name, builder, is_ipv6=False, peek_inner=True):
        ...
        for _ in range(N_PKTS):
            ...
            pkts.append(
                ...
                / inner / Raw(PAYLOAD_TAG)
            )
        dst_net, prefix = (
            ("2001:db8:beef::", 64) if is_ipv6 else ("203.0.113.0", 24)
        )
    ...
    def test_perf_plain_v6_off(self):
        self._measure("plain_v6", self._build_plain_v6,
                      is_ipv6=True, peek_inner=False)

    def test_perf_plain_v6_on(self):
        self._measure("plain_v6", self._build_plain_v6,
                      is_ipv6=True, peek_inner=True)
