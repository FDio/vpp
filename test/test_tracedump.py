...
class TestTracedump2(VppTestCase):
    """Tracedump plugin tests"""

    @classmethod
    def setUpClass(cls):
        super(TestTracedump2, cls).setUpClass()
        cls.create_pg_interfaces(range(2))
        for i in cls.pg_interfaces:
            i.admin_up()
            i.config_ip4()
    ...
    def tearDownClass(cls):
        for i in cls.pg_interfaces:
            i.unconfig_ip4()
            i.admin_down()
        super(TestTracedump2, cls).tearDownClass()
    ...
    def test_tracedump_include(self):
        ...
        reply = self.vapi.trace_v2_dump(thread_id=0, position=0, clear_cache=False)
