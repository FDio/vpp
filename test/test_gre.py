...
class TestGRE(VppTestCase):
    ...
    def test_gre(self):
        ...
        #
        with VppGreInterface(self,
                             self.pg0.local_ip4 ,
                             "1.1.1.2") as gre_if:
            gre_if.admin_up()
            gre_if.config_ip4()

            with IpRoute(self, "4.4.4.4", 32,
                         [IpPath("0.0.0.0",
                                 gre_if.sw_if_index)]) as route_via_tun:
                #
                ...
                #
                with IpRoute(self, "1.1.1.2", 32,
                             [IpPath(self.pg0.remote_ip4,
                                     self.pg0.sw_if_index)]) as route_tun_dst:
                    ...
                    #
                    # disable IPv6 on the PG interface now this test is over.
                    #
    ...
    def test_gre_vrf(self):
        ...
        #  - assign an IP Addres
        #
        with VppGreInterface(self, self.pg1.local_ip4 ,
                             "2.2.2.2",
                             outer_fib_id=1) as gre_if:
            gre_if.admin_up()
            gre_if.config_ip4()
            ...
            #
            with IpRoute(self, "9.9.9.9", 32,
                         [IpPath("0.0.0.0", gre_if.sw_if_index)]) as route_via_tun:

                #
                # Add a route that resolves the tunnel's destination - in the
                # underlay table
                #
                with IpRoute(self, "2.2.2.2", 32, table_id=1,
                             paths=[IpPath(self.pg1.remote_ip4,
                                           self.pg1.sw_if_index)]) as route_tun_dst:
    ...
    def test_gre_l2(self):
        ...
        #
        with IpRoute(self, "2.2.2.2", 32,
                     [IpPath(self.pg0.remote_ip4,
                             self.pg0.sw_if_index)]) as route_tun1_dst:
            with IpRoute(self, "2.2.2.3", 32,
                         [IpPath(self.pg0.remote_ip4,
                                 self.pg0.sw_if_index)]) as route_tun2_dst:

                #
                # Create 2 L2 GRE tunnels and x-connect them
                #
                with VppGreInterface(self, self.pg0.local_ip4,
                                     "2.2.2.2",
                                     is_teb=1) as gre_if1:
                    ...
                    with VppGreInterface(self, self.pg0.local_ip4,
                        ...
                        # then x-connect them
                        #
                        with VppDot1QSubint(self, gre_if1, 11) as gre_if_11:
                            with VppDot1QSubint(self, gre_if2, 12) as gre_if_12:
