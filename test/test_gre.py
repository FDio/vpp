...
class TestGRE(VppTestCase):
    ...
    def create_tunnel_stream_4o4_with_key(self, src_if, tunnel_src, tunnel_dst, src_ip, dst_ip, key):
        pkts = []
        for i in range(0, 257):
            info = self.create_packet_info(src_if, src_if)
            payload = self.info_to_payload(info)
    ...
    def verify_tunneled_4o4_with_key(
        ...
        for i in range(len(capture)):
            try:
                ...
                self.assertTrue(hasattr(rx_gre, 'key'), "GRE key field not present in packet")

                # Verify the key value is correct
                self.assertEqual(rx_gre.key, key, f"GRE key value mismatch: expected {key}, got {rx_gre.key}")
    ...
    def test_gre_with_key(self):
        ...
        self.vapi.sw_interface_set_flags(sw_if_index=gre_sw_if_index, flags=1)  # 1 = admin up
        self.vapi.sw_interface_add_del_address(
            sw_if_index=gre_sw_if_index,
            prefix="10.0.0.1/24",
            is_add=1
        )
        ...
        self.vapi.sw_interface_set_flags(sw_if_index=gre_sw_if_index, flags=0)  # 0 = admin down
