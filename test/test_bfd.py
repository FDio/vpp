...
USEC_IN_SEC = 1000000
BFD_STATS_V4_SH_PATH = "/bfd/udp4/sh/sessions"
BFD_STATS_V6_SH_PATH = "/bfd/udp6/sh/sessions"
BFD_STATS_V4_MH_PATH = "/bfd/udp4/mh/sessions"
BFD_STATS_V6_MH_PATH = "/bfd/udp6/mh/sessions"
BFD_IPV4_REMOTE_ADDR = "2.2.2.2"
BFD_IPV6_REMOTE_ADDR = "2::2"
BFD_IPV4_REMOTE_ADDR2 = "3.3.3.3"
BFD_IPV6_REMOTE_ADDR2 = "3::3"

def set_ipv4_pfx_route_info(cls):
    try:
        if (cls.multihop):
            paths = []
            # Get list of all the nextthops
            pg_if = cls.pg0
            for nh_host in pg_if.remote_hosts:
                nh_host_ip = nh_host.ip4
                paths.append(VppRoutePath(nh_host_ip, pg_if.sw_if_index))
            cls.dst_ip_net = BFD_IPV4_REMOTE_ADDR
            #Create a route pointing to list of next hops
            rip = VppIpRoute(cls, cls.dst_ip_net, 32, paths)
            rip.add_vpp_config()
            cls.logger.info("Route via %s on %s created" % (nh_host_ip, pg_if.name))
            cls.src_ip_net = cls.loopback0.local_ip4
        else:
            cls.dst_ip_net = cls.pg0.remote_ip4 
            cls.src_ip_net = cls.pg0.local_ip4
    except BaseException:
        self.vapi.want_bfd_events(enable_disable=0)
        raise

def set_ipv6_pfx_route_info(cls):
    try:
        if (cls.multihop):
            paths = []
            # Get list of all the nextthops
            ...
            for nh_host in pg_if.remote_hosts:
                ...
                paths.append(VppRoutePath(nh_host_ip, pg_if.sw_if_index))
            cls.dst_ip6_net = BFD_IPV6_REMOTE_ADDR
            #Create a route pointing to list of next hops
...
class BFDAPITestCase(VppTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.__doc__ = f"""""Bidirectional Forwarding Detection (BFD) - API, {hoptype_str}"""
    ...
    def test_add_bfd(self):
        ...
        session = VppBFDUDPSession(
            self, self.interface0, self.dst_ip_net,
            local_addr=self.src_ip_net, multihop=self.multihop
        )
    ...
    def test_add_bfd6(self):
        ...
        session = VppBFDUDPSession(
            self, self.interface0, self.dst_ip6_net,
            local_addr=self.src_ip6_net, af=AF_INET6, multihop=self.multihop
        )
        session.add_vpp_config()
        self.logger.debug("Session state is %s", session.state)
        session.remove_vpp_config()
    ...
    def test_add_bfd_sha1(self):
        ...
        session = VppBFDUDPSession(
            self, self.interface0, self.dst_ip_net,
            local_addr=self.src_ip_net, multihop=self.multihop, sha1_key=key
        )
        session.add_vpp_config()
        self.logger.debug("Session state is %s", session.state)
        session.remove_vpp_config()
    ...
    def test_shared_sha1_key(self):
        ...
        sessions = [
            ...
            VppBFDUDPSession(
                self, self.interface0, self.dst_ip6_net,
                local_addr=self.src_ip6_net, af=AF_INET6,
                multihop=self.multihop, sha1_key=key),
            VppBFDUDPSession(
                self, self.interface1, self.dst_ip_net2,
                local_addr=self.src_ip_net2, multihop=self.multihop,
                sha1_key=key),
            VppBFDUDPSession(
                self, self.interface1, self.dst_ip6_net2,
                local_addr=self.src_ip6_net2, af=AF_INET6,
                multihop=self.multihop, sha1_key=key),
        ]
        for s in sessions:
            s.add_vpp_config()
    ...
    def test_change_key(self):
        ...
        session = VppBFDUDPSession(
            self, self.interface0, self.dst_ip_net,
            local_addr=self.src_ip_net, multihop=self.multihop, sha1_key=key1
        )
        session.add_vpp_config()
    ...
    def test_set_del_udp_echo_source(self):
        ...
        if (self.multihop):
            self.skipTest(f"Skipping because echo functionality is not "
                           "supported with multihop")
...
class BFDTestSession(object):
    ...
    def __init__(
        ...
        if multihop:
            self.sw_if_index = 0
            BFD.udp_dport = bfd.BFD_UDP_MH_PORT
        else:
            BFD.udp_dport = bfd.BFD_UDP_SH_PORT
    ...
    def create_packet(self):
        ...
        if self.af == AF_INET6:
            ...
            packet = (
                packet
                / IP(
                    src=self.peer_addr, dst=self.local_addr, ttl=255
                )
                / UDP(sport=self.udp_sport, dport=BFD.udp_dport)
                / bfd
            )
...
class BFD4TestCase(VppTestCase):
    ...
    def setUpClass(cls):
        ...
        hoptype_str = "MultiHop" if cls.multihop else "SingleHop"
        cls.__doc__ = f"""""Bidirectional Forwarding Detection (BFD) - IPv4, {hoptype_str}"""
        cls.vapi.cli("set log class bfd level debug")
        try:
    ...
    def setUp(self):
        ...
        try:
            ...
            self.test_session = BFDTestSession(
                self, self.pg0, AF_INET, local_addr=self.src_ip_net,
                peer_addr=self.dst_ip_net, multihop=self.multihop
            )
        except BaseException:
            self.vapi.want_bfd_events(enable_disable=0)
            raise
    ...
    def test_intf_deleted(self):
        ...
        if (self.multihop):
            ...
            for nh_host in pg_if.remote_hosts:
                nh_host_ip = nh_host.ip4
                paths.append(VppRoutePath(nh_host_ip, pg_if.sw_if_index))
            self.dst_ip_net = BFD_IPV4_REMOTE_ADDR2
            #Create a route pointing to list of next hops
            rip = VppIpRoute(self, self.dst_ip_net, 32, paths)
            rip.add_vpp_config()
            self.logger.info("Route via %s on %s created" % (nh_host_ip, pg_if.name))
        else:
            self.dst_ip_net = intf.remote_ip4 

        vpp_session = VppBFDUDPSession(
            self, intf, self.dst_ip_net,
            local_addr=self.src_ip_net, multihop=self.multihop 
        )
...
class BFD6TestCase(VppTestCase):
    ...
    def setUpClass(cls):
        ...
        hoptype_str = "MultiHop" if cls.multihop else "SingleHop"
        cls.__doc__ = f"""""Bidirectional Forwarding Detection (BFD) - IPv6, {hoptype_str}"""
        cls.vapi.cli("set log class bfd level debug")
        try:
            cls.create_pg_interfaces([0])
            cls.pg0.config_ip6()
    ...
    def setUp(self):
        ...
        try:
            ...
            self.vpp_session.admin_up()
            self.test_session = BFDTestSession(
                self, self.pg0, AF_INET6, local_addr=self.src_ip6_net,
                peer_addr=self.dst_ip6_net, multihop=self.multihop
            )
    ...
    def test_intf_deleted(self):
        ...
        if (self.multihop):
            ...
            for nh_host in pg_if.remote_hosts:
                nh_host_ip = nh_host.ip6
                paths.append(VppRoutePath(nh_host_ip, pg_if.sw_if_index))
            self.dst_ip6_net = BFD_IPV6_REMOTE_ADDR2
            #Create a route pointing to list of next hops
            rip = VppIpRoute(self, self.dst_ip6_net, 128, paths)
            rip.add_vpp_config()
            self.logger.info("Route via %s on %s created" % (nh_host_ip, pg_if.name))
        else:
            self.dst_ip6_net = intf.remote_ip6

        vpp_session = VppBFDUDPSession(
            self, intf, self.dst_ip6_net, local_addr=self.src_ip6_net,
            af=AF_INET6, multihop=self.multihop
        )
...
class BFDFIBTestCase(VppTestCase):
    ...
    def test_session_with_fib(self):
        ...
        self.vpp_session = VppBFDUDPSession(
            self, self.pg0, self.dst_ip6_net, local_addr=self.src_ip6_net,
            af=AF_INET6, multihop=self.multihop
        )
        self.vpp_session.add_vpp_config()
        self.vpp_session.admin_up()
...
class BFDSHA1TestCase(VppTestCase):
    ...
    def test_session_up(self):
        ...
        key.add_vpp_config()
        self.vpp_session = VppBFDUDPSession(
            self, self.pg0, self.dst_ip_net, local_addr=self.src_ip_net,
            multihop=self.multihop, sha1_key=key
        )
        self.vpp_session.add_vpp_config()
        self.vpp_session.admin_up()
...
class BFDAuthOnOffTestCase(VppTestCase):
    ...
    def test_auth_change_key_immediate(self):
        ...
        self.vpp_session = VppBFDUDPSession(
            self, self.pg0, self.dst_ip_net, local_addr=self.src_ip_net,
            multihop=self.multihop, sha1_key=key1
        )
        self.vpp_session.add_vpp_config()
        self.test_session = BFDTestSession(
            self,
...
class BFDCLITestCase(VppTestCase):
    ...
    def setUpClass(cls):
        ...
        hoptype_str = "MultiHop" if cls.multihop else "SingleHop"
        cls.__doc__ = f"""""Bidirectional Forwarding Detection (BFD) - CLI - {hoptype_str}"""
        cls.vapi.cli("set log class bfd level debug")
        try:
            cls.create_pg_interfaces([0])
            cls.create_loopback_interfaces(1)
    ...
    def test_show(self):
        ...
        s2 = VppBFDUDPSession(
            self, self.pg0, self.dst_ip6_net, local_addr=self.src_ip6_net,
            af=AF_INET6, multihop=self.multihop, sha1_key=k2
        )
        s2.add_vpp_config()
    ...
    def test_set_del_sha1_key(self):
        ...
        self.vpp_session = VppBFDUDPSession(
            self, self.pg0, self.dst_ip_net, local_addr=self.src_ip_net,
            multihop=self.multihop, sha1_key=k
        )
        self.vpp_session.add_vpp_config()
        self.test_session = BFDTestSession(
            self,
    ...
    def test_set_del_meticulous_sha1_key(self):
        ...
        self.vpp_session = VppBFDUDPSession(
            self, self.pg0, self.dst_ip6_net, local_addr=self.src_ip6_net,
            af=AF_INET6, multihop=self.multihop, sha1_key=k
        )
        self.vpp_session.add_vpp_config()
        self.vpp_session.admin_up()
        self.test_session = BFDTestSession(
    ...
    def test_add_mod_del_bfd_udp(self):
        ...
        cli_del_cmd = (
            "bfd udp session del %s local-addr %s "
        ...
        if self.multihop:
            ...
            cli_del_cmd = (
                "bfd udp session del local-addr 1.1.1.1 peer-addr 2.2.2.2 "
            )
            self.cli_verify_response(
                cli_del_cmd,
                "bfd udp session del: Incompatible parameter combination, "
    ...
    def test_add_mod_del_bfd_udp6(self):
        ...
        cli_del_cmd = (
            "bfd udp session del %s local-addr %s "
            "peer-addr %s" % (self.cli_str, self.src_ip6_net, self.dst_ip6_net)
        )
        self.cli_verify_no_response(cli_del_cmd)
        # 2nd del is expected to fail
        self.cli_verify_response(
    ...
    def test_add_mod_del_bfd_udp6_auth(self):
        ...
        vpp_session = VppBFDUDPSession(
            self, self.pg0, self.dst_ip6_net, local_addr=self.src_ip6_net,
            af=AF_INET6, multihop=self.multihop,  sha1_key=key
        )
        self.registry.register(vpp_session, self.logger)
        cli_add_cmd = (
            "bfd udp session add %s local-addr %s "
