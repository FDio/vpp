...
class BFDAPITestCase(VppTestCase):
    ...
    def test_session_set_tos(self):
        """test BFD session set_tos functionality"""
        # Create a BFD session
        session = VppBFDUDPSession(
            self,
            self.interface0,
        ...
        session.add_vpp_config()
        # Test setting TOS with explicit value
        test_tos_value = 112
        session.set_tos(test_tos_value)
        current_tos = self.vapi.bfd_udp_get_tos()
        self.assertEqual(
            current_tos.tos, test_tos_value, f"TOS should be {test_tos_value}"
        )
        # Test setting TOS with default parameter (should use BFD_UDP_DEFAULT_TOS)
        session.set_tos()
        current_tos = self.vapi.bfd_udp_get_tos()
        self.assertEqual(
            current_tos.tos,
            BFD_UDP_DEFAULT_TOS,
            f"TOS should be default value {BFD_UDP_DEFAULT_TOS}",
        )
        # Test boundary values
        session.set_tos(0)  # Minimum value
        current_tos = self.vapi.bfd_udp_get_tos()
        self.assertEqual(current_tos.tos, 0, "TOS should be 0")
        session.set_tos(255)  # Maximum value
        current_tos = self.vapi.bfd_udp_get_tos()
        self.assertEqual(current_tos.tos, 255, "TOS should be 255")
    ...
    def test_bfd_traffic_with_tos_configured(self):
        ...
        for tos_value in test_tos_values:
            with self.subTest(tos=tos_value):
                ...
                # Set the TOS value using the session method
                self.vpp_session.set_tos(tos_value)
