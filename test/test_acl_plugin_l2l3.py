...
from scapy.layers.inet6 import IPv6, ICMPv6Unknown, ICMPv6EchoRequest, ICMPv6EchoReply, IPv6ExtHdrRouting
...
class TestIpIrb(VppTestCase):
    ...
    def api_acl_add_replace(self, acl_index, r, count, tag = "", expected_retval = 0):
        """Add/replace an ACL
        ### self.api_acl_interface_set_acl_list(sw_if_index = self.pg_interfaces[0].sw_if_index, count=1, n_input=1, acls = [ permit_acl_index ])
    ...
    def api_acl_interface_set_acl_list(self, sw_if_index, count, n_input, acls, expected_retval = 0):
    ...
    def create_stream(self, src_ip_if, dst_ip_if, reverse, packet_sizes, is_ip6, expect_blocked, expect_established, add_extension_header):
        pkts = []
        rules = []
        permit_rules = []
        ...
        for i in range(0, total_packet_count):
            ...
            is_permit = i % 2
            remote_dst_host = dst_ip_if.remote_hosts[i % len(dst_ip_if.remote_hosts)]
            if is_permit == 1:
              info = self.create_packet_info(src_ip_if, dst_ip_if)
              payload = self.info_to_payload(info)
            else:
              if (expect_blocked and not expect_established) or not can_reflect_this_packet:
            ...
            if can_reflect_this_packet:
              if is_ip6:
                ...
                if add_extension_header:
                  ...
                  ulp = IPv6ExtHdrRouting() / IPv6ExtHdrRouting() / IPv6ExtHdrRouting() / IPv6ExtHdrRouting() / IPv6ExtHdrRouting() / ulp_l4
                  # uncomment below to test invalid ones
                  # ulp = IPv6ExtHdrRouting(len = 200) / IPv6ExtHdrRouting() / IPv6ExtHdrRouting() / IPv6ExtHdrRouting() / IPv6ExtHdrRouting() / ulp_l4
                else:
                  ulp = ulp_l4
            ...
            rule_l4_sport = p[UDP].sport if p.haslayer(UDP) else p[ICMP].type if p.haslayer(ICMP) else p[ICMPv6Unknown].type
            rule_l4_dport = p[UDP].dport if p.haslayer(UDP) else p[ICMP].code if p.haslayer(ICMP) else p[ICMPv6Unknown].code
            rule_l4_proto = ulp_l4.overload_fields[IPv6]['nh'] if p.haslayer(IPv6) else p[IP].proto
            new_rule = {
                 'is_permit': is_permit,
            ...
            # if new_rule_permit_and_reflect['is_permit'] == 0:
            if can_reflect_this_packet:
    ...
    def verify_capture(self, dst_ip_if, src_ip_if, capture, reverse):
        ...
        for packet in capture:
            ...
            ip = packet[l3]
            l4 = UDP if packet.haslayer(UDP) else ICMP if packet.haslayer(ICMP) else ICMPv6Unknown
            # Scapy IPv6 stuff is too smart for its own good. So we do this and coerce the ICMP into unknown type
            data = str(packet[UDP][Raw]) if packet.haslayer(UDP) else str(ICMP(str(packet[l3].payload))[Raw]) if l3 == IP else str(ICMPv6Unknown(str(packet[l3].payload)).msgbody)
    ...
    def create_acls_for_a_stream(self, stream_dict, test_l2_action, is_reflect):
        ...
        reply = self.api_acl_add_replace(acl_index=4294967295, r=r_action, count=len(r_action), tag="action acl  for traffic")
        action_acl_index = reply.acl_index
        reply = self.api_acl_add_replace(acl_index=4294967295, r=r_permit, count=len(r_permit), tag="permit traffic")
    ...
    def apply_acl_ip46_x_to_y(self, bridged_to_routed, test_l2_deny, is_ip6, is_reflect, add_eh):
        """ Apply the ACLs
        """
        self.reset_packet_infos()
        stream_dict = self.create_stream(
            self.pg2, self.loop0, bridged_to_routed,  self.pg_if_packet_sizes, is_ip6, not is_reflect, False, add_eh)
        stream = stream_dict['stream']
        acl_idx = self.create_acls_for_a_stream(stream_dict, test_l2_deny, is_reflect)

        self.api_acl_interface_set_acl_list(sw_if_index = self.pg2.sw_if_index, count=1, n_input=0 if bridged_to_routed else 1, acls = [ acl_idx['L3'] ])
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg0.sw_if_index, count=1, n_input=1 if bridged_to_routed else 0, acls = [ acl_idx['L2'] ])
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg1.sw_if_index, count=1, n_input=1 if bridged_to_routed else 0, acls = [ acl_idx['L2'] ])

    def apply_acl_ip46_both_directions_reflect(self, primary_is_bridged_to_routed, reflect_on_l2, is_ip6, add_eh):
        primary_is_routed_to_bridged = not primary_is_bridged_to_routed
        self.reset_packet_infos()
        stream_dict_fwd = self.create_stream(
            self.pg2, self.loop0, primary_is_bridged_to_routed,  self.pg_if_packet_sizes, is_ip6, False, False, add_eh)
        acl_idx_fwd = self.create_acls_for_a_stream(stream_dict_fwd, reflect_on_l2, True)

        stream_dict_rev = self.create_stream(
            self.pg2, self.loop0, not primary_is_bridged_to_routed,  self.pg_if_packet_sizes, is_ip6, True, True, add_eh) # True if we want that test to fail (for Traff gen)
        acl_idx_rev = self.create_acls_for_a_stream(stream_dict_rev, reflect_on_l2, False) # We want the primary action to be "deny" rather than reflect

        inbound_l2_acl   = acl_idx_fwd['L2'] if primary_is_bridged_to_routed else acl_idx_rev['L2']
        outbound_l2_acl  = acl_idx_fwd['L2'] if primary_is_routed_to_bridged else acl_idx_rev['L2']

        inbound_l3_acl   = acl_idx_fwd['L3'] if primary_is_routed_to_bridged else acl_idx_rev['L3']
        outbound_l3_acl  = acl_idx_fwd['L3'] if primary_is_bridged_to_routed else acl_idx_rev['L3']

        self.api_acl_interface_set_acl_list(sw_if_index = self.pg2.sw_if_index, count=2, n_input=1, acls = [ inbound_l3_acl, outbound_l3_acl ])
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg0.sw_if_index, count=2, n_input=1, acls = [ inbound_l2_acl, outbound_l2_acl ])
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg1.sw_if_index, count=2, n_input=1, acls = [ inbound_l2_acl, outbound_l2_acl ])

    def apply_acl_ip46_routed_to_bridged(self, test_l2_deny, is_ip6, is_reflect, add_eh):
        self.apply_acl_ip46_x_to_y(False, test_l2_deny, is_ip6, is_reflect, add_eh)

    def apply_acl_ip46_bridged_to_routed(self, test_l2_deny, is_ip6, is_reflect, add_eh):
        self.apply_acl_ip46_x_to_y(True, test_l2_deny, is_ip6, is_reflect, add_eh)


    def run_traffic_ip46_x_to_y(self, bridged_to_routed, test_l2_deny, is_ip6, is_reflect, is_established, add_eh):
        self.reset_packet_infos()
        stream_dict = self.create_stream(
            self.pg2, self.loop0, bridged_to_routed,  self.pg_if_packet_sizes, is_ip6, not is_reflect, is_established, add_eh)
        stream = stream_dict['stream']
    ...
    def run_traffic_ip46_routed_to_bridged(self, test_l2_deny, is_ip6, is_reflect, is_established, add_eh):
        self.run_traffic_ip46_x_to_y(False, test_l2_deny, is_ip6, is_reflect, is_established, add_eh)
    def run_traffic_ip46_bridged_to_routed(self, test_l2_deny, is_ip6, is_reflect, is_established, add_eh):
        self.run_traffic_ip46_x_to_y(True, test_l2_deny, is_ip6, is_reflect, is_established, add_eh)


    def run_test_ip46_routed_to_bridged(self, test_l2_deny, is_ip6, is_reflect, add_eh):
	self.apply_acl_ip46_routed_to_bridged(test_l2_deny, is_ip6, is_reflect, add_eh)
	self.run_traffic_ip46_routed_to_bridged(test_l2_deny, is_ip6, is_reflect, False, add_eh)

    def run_test_ip46_bridged_to_routed(self, test_l2_deny, is_ip6, is_reflect, add_eh):
	self.apply_acl_ip46_bridged_to_routed(test_l2_deny, is_ip6, is_reflect, add_eh)
	self.run_traffic_ip46_bridged_to_routed(test_l2_deny, is_ip6, is_reflect, False, add_eh)

    def run_test_ip46_routed_to_bridged_and_back(self, test_l2_action, is_ip6, add_eh):
	self.apply_acl_ip46_both_directions_reflect(False, test_l2_action, is_ip6, add_eh)
	self.run_traffic_ip46_routed_to_bridged(test_l2_action, is_ip6, True, False, add_eh)
	self.run_traffic_ip46_bridged_to_routed(test_l2_action, is_ip6, False, True, add_eh)

    def run_test_ip46_bridged_to_routed_and_back(self, test_l2_action, is_ip6, add_eh):
	self.apply_acl_ip46_both_directions_reflect(True, test_l2_action, is_ip6, add_eh)
	self.run_traffic_ip46_bridged_to_routed(test_l2_action, is_ip6, True, False, add_eh)
	self.run_traffic_ip46_routed_to_bridged(test_l2_action, is_ip6, False, True, add_eh)


    def test_0000_ip6_irb_1(self):
       """ IRB ACL plugin prepare"""
       if not self.vpp_dead:
            self.logger.info(self.vapi.ppcli("set acl-plugin session timeout udp idle 2000"))
            # uncomment to not skip past the routing header and watch the EH tests fail
            # self.logger.info(self.vapi.ppcli("set acl-plugin skip-ipv6-extension-header 43 0"))
            # uncomment to test the session limiter (stateful tests will fail)
            # self.logger.info(self.vapi.ppcli("set acl-plugin session table max-entries 1"))
            # new datapath is the default, but the command is there, just in case
            # self.logger.info(self.vapi.ppcli("set acl-plugin l2-datapath new"))
            # If you want to see some tests fail, uncomment the next line
            # self.logger.info(self.vapi.ppcli("set acl-plugin l2-datapath old"))
    def test_0001_ip6_irb_1(self):
       """ IRB ACL plugin IPv6 test routed -> bridged, L2 ACL deny"""
       self.run_test_ip46_routed_to_bridged(True, True, False, self.WITHOUT_EH)

    def test_0002_ip6_irb_1(self):
       """ IRB ACL plugin IPv6 test routed -> bridged, L3 ACL deny"""
       self.run_test_ip46_routed_to_bridged(False, True, False, self.WITHOUT_EH)

    def test_0003_ip4_irb_1(self):
       """ IRB ACL plugin IPv4 test routed -> bridged, L2 ACL deny"""
       self.run_test_ip46_routed_to_bridged(True, False, False, self.WITHOUT_EH)

    def test_0004_ip4_irb_1(self):
       """ IRB ACL plugin IPv4 test routed -> bridged, L3 ACL deny"""
       self.run_test_ip46_routed_to_bridged(False, False, False, self.WITHOUT_EH)


    def test_0005_ip6_irb_1(self):
       """ IRB ACL plugin IPv6 test bridged -> routed, L2 ACL deny """
       self.run_test_ip46_bridged_to_routed(True, True, False, self.WITHOUT_EH)

    def test_0006_ip6_irb_1(self):
       """ IRB ACL plugin IPv6 test bridged -> routed, L3 ACL deny """
       self.run_test_ip46_bridged_to_routed(False, True, False, self.WITHOUT_EH)

    def test_0007_ip6_irb_1(self):
       """ IRB ACL plugin IPv4 test bridged -> routed, L2 ACL deny """
       self.run_test_ip46_bridged_to_routed(True, False, False, self.WITHOUT_EH)

    def test_0008_ip6_irb_1(self):
       """ IRB ACL plugin IPv4 test bridged -> routed, L3 ACL deny """
       self.run_test_ip46_bridged_to_routed(False, False, False, self.WITHOUT_EH)


    def test_0101_ip6_irb_1(self):
       """ IRB ACL plugin IPv6 test routed -> bridged, L2 ACL permit+reflect"""
       self.run_test_ip46_routed_to_bridged_and_back(True, True, self.WITHOUT_EH)

    def test_0102_ip6_irb_1(self):
       """ IRB ACL plugin IPv6 test bridged -> routed, L2 ACL permit+reflect"""
       self.run_test_ip46_bridged_to_routed_and_back(True, True, self.WITHOUT_EH)

    def test_0103_ip6_irb_1(self):
       """ IRB ACL plugin IPv4 test routed -> bridged, L2 ACL permit+reflect"""
       self.run_test_ip46_routed_to_bridged_and_back(True, False, self.WITHOUT_EH)

    def test_0104_ip6_irb_1(self):
       """ IRB ACL plugin IPv4 test bridged -> routed, L2 ACL permit+reflect"""
       self.run_test_ip46_bridged_to_routed_and_back(True, False, self.WITHOUT_EH)

    def test_0111_ip6_irb_1(self):
       """ IRB ACL plugin IPv6 test routed -> bridged, L3 ACL permit+reflect"""
       self.run_test_ip46_routed_to_bridged_and_back(False, True, self.WITHOUT_EH)

    def test_0112_ip6_irb_1(self):
       """ IRB ACL plugin IPv6 test bridged -> routed, L3 ACL permit+reflect"""
       self.run_test_ip46_bridged_to_routed_and_back(False, True, self.WITHOUT_EH)

    def test_0113_ip6_irb_1(self):
       """ IRB ACL plugin IPv4 test routed -> bridged, L3 ACL permit+reflect"""
       self.run_test_ip46_routed_to_bridged_and_back(False, False, self.WITHOUT_EH)

    def test_0114_ip6_irb_1(self):
       """ IRB ACL plugin IPv4 test bridged -> routed, L3 ACL permit+reflect"""
