...
class TestIpIrb(VppTestCase):
    ...
    def apply_acl_ip46_x_to_y(self, bridged_to_routed, test_l2_deny, is_ip6, is_reflect, add_eh):
        ...
        stream_dict = self.create_stream(
                                         self.pg2, self.loop0, bridged_to_routed, 
        ...
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg2.sw_if_index, count=1,
                                            n_input=0 if bridged_to_routed else 1, acls = [ acl_idx['L3'] ])
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg0.sw_if_index, count=1,
                                            n_input=1 if bridged_to_routed else 0, acls = [ acl_idx['L2'] ])
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg1.sw_if_index, count=1,
                                            n_input=1 if bridged_to_routed else 0, acls = [ acl_idx['L2'] ])

    def apply_acl_ip46_both_directions_reflect(self, primary_is_bridged_to_routed,
                                               reflect_on_l2, is_ip6, add_eh):
        primary_is_routed_to_bridged = not primary_is_bridged_to_routed
        self.reset_packet_infos()
        stream_dict_fwd = self.create_stream(self.pg2, self.loop0, primary_is_bridged_to_routed,
                                             self.pg_if_packet_sizes, is_ip6,
                                             False, False, add_eh)
        acl_idx_fwd = self.create_acls_for_a_stream(stream_dict_fwd, reflect_on_l2, True)

        stream_dict_rev = self.create_stream(self.pg2, self.loop0, not primary_is_bridged_to_routed,
                                             self.pg_if_packet_sizes, is_ip6, True, True, add_eh)
        # We want the primary action to be "deny" rather than reflect
        acl_idx_rev = self.create_acls_for_a_stream(stream_dict_rev, reflect_on_l2, False)
        ...
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg2.sw_if_index, count=2,
                                            n_input=1, acls = [ inbound_l3_acl, outbound_l3_acl ])
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg0.sw_if_index, count=2,
                                            n_input=1, acls = [ inbound_l2_acl, outbound_l2_acl ])
        self.api_acl_interface_set_acl_list(sw_if_index = self.pg1.sw_if_index, count=2,
                                            n_input=1, acls = [ inbound_l2_acl, outbound_l2_acl ])
    ...
    def run_traffic_ip46_x_to_y(self, bridged_to_routed, test_l2_deny, is_ip6,
                                is_reflect, is_established, add_eh):
        self.reset_packet_infos()
        stream_dict = self.create_stream(self.pg2, self.loop0, bridged_to_routed,
                                         self.pg_if_packet_sizes, is_ip6,
                                         not is_reflect, is_established, add_eh)
        stream = stream_dict['stream']
