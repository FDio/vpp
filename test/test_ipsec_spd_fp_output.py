...
class SpdFastPathOutbound(SpdFlowCacheTemplate):
...
class IPSec4SpdTestCaseOverwriteRule(SpdFastPathOutbound):
    """ IPSec/IPv4 outbound: Policy mode test case with fast path \
        (overwrite stale entries)"""
    def test_ipsec_spd_outbound_overwrite(self):
        # In this test, 3 active rules are created and matched.
        # A single entry is removed.
        # We then readd the rule.
        self.create_interfaces(3)
        pkt_count = 2
        # bind SPD to all interfaces
        self.spd_create_and_intf_add(1, self.pg_interfaces)
        ...
        # adding an inbound policy should not invalidate output
        self.spd_add_rem_policy(  # inbound
            1, self.pg0, self.pg1, socket.IPPROTO_UDP,
            is_out=0, priority=10, policy_type="bypass")

        # remove a bypass policy
...
class IPSec4SpdTestCaseCollision(SpdFastPathOutbound):
    """ IPSec/IPv4 outbound: Policy mode test case with fast path \
        (hash collision)"""
    ...
    def setUpConstants(cls):
        super(SpdFastPathOutbound, cls).setUpConstants()
        cls.vpp_cmdline.extend(["ipsec", "{",
                                "ipv4-outbound-spd-flow-cache off",
                                "}"])
        cls.logger.info("VPP modified cmdline is %s" % " "
                        .join(cls.vpp_cmdline))

    def test_ipsec_spd_outbound_collision(self):
        # The fast path operation is setup to add an entry to
        # a vector stored in bihash value
