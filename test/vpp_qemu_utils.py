...
def create_host_interface(
    vpp_interface_name, host_namespace, *host_ip_prefixes, host_interface_name = None
):
    ...
    with lock:
        ...
        for attempt in range(retries):
            if_name = host_interface_name or f"vpp_host_{''.join(random.choices(string.digits, k=4))}"
            ...
            if result.returncode == 0:
                host_interface_name = if_name
                break
            if attempt < retries - 1:
                time.sleep(1)
            else:
                raise Exception(
                    f"Failed to create host interface {if_name} after {retries} attempts"
                )
