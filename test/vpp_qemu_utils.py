...
def delete_host_interfaces_from_file(file_path):
    """Delete all host interfaces whose names have been added to the file."""
    if os.path.exists(file_path):
        with open(file_path, "r") as if_file:
            for line in if_file:
                if_name = line.strip()
                if if_name:
                    delete_host_interfaces(if_name)
            os.remove(file_path)
...
def create_host_interface(
    host_namespace, *host_ip_prefixes, vpp_if_name=None, host_if_name=None
):
    ...
    with lock:
        ...
        for attempt in range(retries):
            if_name = (
                host_if_name or f"host_if_{''.join(random.choices(string.digits, k=4))}"
            )
            new_vpp_if_name = (
                vpp_if_name or f"vppout{''.join(random.choices(string.digits, k=4))}"
            )
            ...
            if attempt >= retries - 1:
                raise Exception(
                    f"Failed to create host interface {if_name} and vpp {new_vpp_if_name} after {retries} attempts"
                )
...
def delete_namespaces_from_file(file_path):
    """Delete all namespaces whose names have been added to the file."""
    if os.path.exists(file_path):
