...
def generate_unique_namespace_name(base_prefix="vpp_ns_", max_retries=100):
    """
    ...
    Arguments:
    base_prefix -- the prefix to use for the namespace name (default is "vpp_ns_")
    max_retries -- maximum number of suffix increments to try for uniqueness (default is 100)

    Returns:
    A unique namespace name as a string.

    Raises:
    Exception if a unique namespace name could not be generated after max_retries attempts.
    """
    with lock:
        for suffix in range(max_retries):
        ...
        raise Exception(
            f"Failed to generate a unique namespace name after {max_retries} attempts."
        )


def generate_unique_host_interface_name(base_prefix="vpp_host_", max_retries=100):
    """
    ...
    Raises:
    Exception if a unique interface name could not be generated after max_retries attempts.
    """
    with lock:
        for suffix in range(max_retries):
        ...
        raise Exception(
            f"Failed to generate a unique host interface name after {max_retries} attempts."
        )
