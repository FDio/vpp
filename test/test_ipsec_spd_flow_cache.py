...
class IPSec4SpdCommonMethods(VppTestCase):
    ...
    def setUpConstants(cls):
        ...
        cls.vpp_cmdline.extend(["ipsec", "{", "ipv4-out-spd-flow-cache on", "}"])
    ...
    def spd_add_low_discard_high_bypass(self, spd_id, src_if, dst_if, proto, num_rules, is_out):
        spd = VppIpsecSpd(self, spd_id)
    ...
    def spd_remove_high_bypass(self, spd_id, src_if, dst_if, proto, num_rules, is_out):
        spd = VppIpsecSpd(self, spd_id)
        e = VppEnum.vl_api_ipsec_spd_action_t
    ...
    def get_spd_bihash_values(self):
        ...
        regex_match = re.search('Hash table \'IPSec IPv4 OUTBOUND SPD Flow cache\'(.*)scrap', bihash_reply, re.DOTALL)
        if regex_match is None:
            raise Exception("Unable to find spd flow cache bihash table in \'show bihash\' CLI output")
        else:
            ...
            key = ["active elements", "active buckets", "free lists", "linear search buckets", "heap chunk(s) allocated"]
            for i in range(len(bihash_values)):
                self.logger.info("bihash contents: " + key[i] +": " + str(bihash_values[i]))
        return bihash_values

    def check_spd_bihash_active_elements(self, expected_elements):
        bihash_values = self.get_spd_bihash_values()
        self.assertEqual(bihash_values[0], expected_elements) # bihash_values[0] = active elements

class IPSec4SpdTestCaseAdd(IPSec4SpdCommonMethods):
    """ IPSec/IPv4 outbound: Policy mode test case with flow cache (add rule)"""
    def test_ipsec_spd_outbound_add(self):
        ...
        self.spd_add_low_discard_high_bypass(1, self.pg0, self.pg1, socket.IPPROTO_UDP, num_discard_rules, 1)
        ...
        # check policy in SPD has been cached after traffic matched BYPASS rule in SPD
        self.check_spd_bihash_active_elements(1)

class IPSec4SpdTestCaseRemove(IPSec4SpdCommonMethods):
    """ IPSec/IPv4 outbound: Policy mode test case with flow cache (remove rule)"""
    def test_ipsec_spd_outbound_remove(self):
        ...
        self.spd_create_and_intf_add(1, [self.pg1])
        self.spd_add_low_discard_high_bypass(1, self.pg0, self.pg1, socket.IPPROTO_UDP, num_discard_rules, 1)
        self.spd_remove_high_bypass(1, self.pg0, self.pg1, socket.IPPROTO_UDP, num_discard_rules, 1)
...
class IPSec4SpdTestCaseReadd(IPSec4SpdCommonMethods):
    """ IPSec/IPv4 outbound: Policy mode test case with flow cache (add, remove, re-add)"""
    def test_ipsec_spd_outbound_readd(self):
        ...
        # by removing the BYPASS rule, we invalidated the corresponding bihash entry
        # when the same IP tuple hash is searched again, bihash lookup fails and we overwrite
        # the entry with the updated policy (DISCARD) matched in the linear search
        # we should therefore still expect a single (1) active element with the hash of the IP tuple
        self.check_spd_bihash_active_elements(1)

        # Readd high priority bypass SPD rule, but don't add any more discard rules
        self.spd_add_low_discard_high_bypass(1, self.pg0, self.pg1, socket.IPPROTO_UDP, 0, 1)
        # create the packet stream
        packets = self.create_stream(self.pg0, self.pg1, count)
        # add the stream to the source interface
        self.pg0.add_stream(packets)
        ...
        self.pg0.assert_nothing_captured()
        # verify capture
        self.logger.debug("SPD Readd/After readd: Num packets: %s", len(capture.res))

        # as above, we overwrote the previous bihash entry, so should still expect a single active element in the table matching the IP tuple
        self.check_spd_bihash_active_elements(1)

class IPSec4SpdTestCaseMultiple(IPSec4SpdCommonMethods):
    """ IPSec/IPv4 outbound: Policy mode test case with flow cache (multiple interfaces, multiple rules)"""
    def test_ipsec_spd_outbound_multiple(self):
        # In this test case, packets in IPv4 FWD path are configured to go through IPSec outbound SPD policy lookup.
        ...
        self.spd_add_low_discard_high_bypass(1, self.pg0, self.pg1, socket.IPPROTO_UDP, num_discard_rules, 1)
        self.spd_add_low_discard_high_bypass(1, self.pg1, self.pg2, socket.IPPROTO_UDP, num_discard_rules, 1)
        self.spd_add_low_discard_high_bypass(1, self.pg2, self.pg0, socket.IPPROTO_UDP, num_discard_rules, 1)
        # remove bypass rule on traffic to pg0 - leaving discard rule
        self.spd_remove_high_bypass(1, self.pg2, self.pg0, socket.IPPROTO_UDP, num_discard_rules, 1)
