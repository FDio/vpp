...
def get_truthy_env_var(envvar, default='n'):
    return os.getenv(envvar, default).lower() in \
           ("y", "yes", "1")


debug_framework = get_truthy_env_var('TEST_DEBUG')
if debug_framework:
...
def _is_skip_aarch64_set():
    return get_truthy_env_var('SKIP_AARCH64')
...
def _running_extended_tests():
    return get_truthy_env_var("EXTENDED_TESTS")
...
class VppTestCase(unittest.TestCase):
    ...
    """
    get_truthy_envar = staticmethod(get_truthy_env_var)
    ...
    def setUpConstants(cls):
        ...
        cls.step = get_truthy_env_var('STEP')
