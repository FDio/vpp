...
class TestLB(VppTestCase):
    ...
    def test_lb_as_dump_per_port(self):
        """lb_as_dump: per-port VIP uses ntohs(mp->port) for matching"""
        try:
            self.vapi.lb_add_del_vip(
                pfx="90.0.0.0/8", encap=0, protocol=6, port=80
            )
            self.vapi.lb_add_del_as(
                pfx="90.0.0.0/8", as_address="10.0.0.1", protocol=6, port=80
            )

            ass = self.vapi.lb_as_dump(pfx="90.0.0.0/8", protocol=6, port=80)
            self.assertEqual(len(ass), 1, "expected 1 AS for per-port VIP")
            self.assertEqual(str(ass[0].app_srv), "10.0.0.1")
        finally:
            self.vapi.lb_add_del_as(
                pfx="90.0.0.0/8",
                as_address="10.0.0.1",
            ...
            self.vapi.lb_add_del_vip(
                pfx="90.0.0.0/8", encap=0, protocol=6, port=80, is_del=True
            )
            self.vapi.cli("test lb flowtable flush")
