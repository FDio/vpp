...
class TestLB(VppTestCase):
    ...
    def test_lb_ip4_punt(self):
        ...
        try:
            ...
            self.vapi.lb_add_del_vip_v3(
                pfx="%s/32" % VIP4, encap=0, protocol=6, port=80, is_punt=True
            )
            self.vapi.lb_add_del_as(
                pfx="%s/32" % VIP4, protocol=6, port=80, as_address=AS4
            )

            # Verify is_punt is returned by the v2 dump
            vips = self.vapi.lb_vip_v2_dump()
            self.assertEqual(len(vips), 1)
            self.assertTrue(vips[0].is_punt)
            ...
            self.vapi.lb_add_del_as(
                pfx="%s/32" % VIP4, protocol=6, port=80, as_address=AS4, is_del=True
            )
            self.vapi.lb_add_del_vip_v3(
                pfx="%s/32" % VIP4,
                encap=0,
    ...
    def test_lb_ip6_punt(self):
        ...
        try:
            ...
            self.vapi.lb_add_del_vip_v3(
                pfx="%s/128" % VIP6, encap=1, protocol=6, port=80, is_punt=True
            )
            self.vapi.lb_add_del_as(
                pfx="%s/128" % VIP6, protocol=6, port=80, as_address=AS6
            )
            ...
            self.vapi.lb_add_del_as(
                pfx="%s/128" % VIP6, protocol=6, port=80, as_address=AS6, is_del=True
            )
            self.vapi.lb_add_del_vip_v3(
                pfx="%s/128" % VIP6,
                encap=1,
