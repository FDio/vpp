...
class TemplateTraceFilter(VppTestCase):
    ...
    def add_trace_filter(self, mask, match, inner=False, interface_name=None):
        if inner and interface_name is not None:
            self.cli(f"classify filtering feature {interface_name}")
        self.cli("classify filter trace mask %s match %s" % (mask, match))
        self.clear()
        self.cli("trace add pg-input 1000 filter")

    def del_trace_filters(self, inner=False, interface_name=None):
        if inner and interface_name is not None:
            self.cli(f"classify filtering feature {interface_name} disable")
