...
class TestSfdp(VppTestCase):
    ...
    # with only minor touches (sleep timining, code comments, test case name) by humans.
    def test_sfdp_tcp_retransmit_of_last_fin(self):
        """Test SFDP TCP late retransmit of receiver FIN+ACK."""
        # Remove try/finally when VPP code makes this test pass.
        try:
            ...
            # Responder suffered RX loss, nothing is sent for the duration of RTO.
            self.virtual_sleep(1.0)

            # 3. The retransmission of FIN+ACK from Responder.
            late_fin_ack = (
                Ether(src=mac1, dst=lmac1)
                / IP(src=ip1, dst=ip0)
                / TCP(sport=dport, dport=sport, flags="FA", seq=201, ack=102)
            )
            # This test does not check whether the retransmitted packet is forwarded.
            self.pg_send(self.pg1, [late_fin_ack])

            # 4. In the meantime, initiator TCP stack got rebooted,
            # so the 5-tuple is reused before 2MSL, with incompatible sequence number.
            new_syn = (
                Ether(src=mac0, dst=lmac0)
                / IP(src=ip0, dst=ip1)
            ...
            # Confirm the new SYN packet passes.
            # Regardless of the way initiator and responder will resolve their state conflict,
            # VPP should honor Conntrack TIME-WAIT Timeout and keep forwarding at this point.
            self.send_and_expect(self.pg0, new_syn, self.pg1)

        finally:
            # Explicit deconfiguration, needed even if one of asserts above fail.
            self._cleanup_sfdp(bidir=True)
            # TODO: Move cleanups to tearDown() so try/finally is not needed.
