...
class TestSfdp(VppTestCase):
    ...
    def test_sfdp_tcp_retransmit_of_last_fin(self):
        ...
        syn = Ether(src=mac0, dst=lmac0) / IP(src=ip0, dst=ip1) / TCP(sport=sport, dport=dport, flags="S", seq=100)
        self.send_and_expect(self.pg0, syn, self.pg1)

        syn_ack = Ether(src=mac1, dst=lmac1) / IP(src=ip1, dst=ip0) / TCP(sport=dport, dport=sport, flags="SA", seq=200, ack=101)
        self.send_and_expect(self.pg1, syn_ack, self.pg0)

        ack = Ether(src=mac0, dst=lmac0) / IP(src=ip0, dst=ip1) / TCP(sport=sport, dport=dport, flags="A", seq=101, ack=201)
        self.send_and_expect(self.pg0, ack, self.pg1)

        # 2. Teardown (Clean close to trigger remove_session = 1)
        fin = Ether(src=mac0, dst=lmac0) / IP(src=ip0, dst=ip1) / TCP(sport=sport, dport=dport, flags="F", seq=101, ack=201)
        self.send_and_expect(self.pg0, fin, self.pg1)

        # Responder ACKs the FIN and sends its own FIN
        fin_ack = Ether(src=mac1, dst=lmac1) / IP(src=ip1, dst=ip0) / TCP(sport=dport, dport=sport, flags="FA", seq=201, ack=102)
        self.send_and_expect(self.pg1, fin_ack, self.pg0)

        # Initiator sends the final ACK. VPP marks the session for eventual removal.
        last_ack = Ether(src=mac0, dst=lmac0) / IP(src=ip0, dst=ip1) / TCP(sport=sport, dport=dport, flags="A", seq=102, ack=202)
        ...
        late_fin_ack = Ether(src=mac1, dst=lmac1) / IP(src=ip1, dst=ip0) / TCP(sport=dport, dport=sport, flags="FA", seq=201, ack=102)
        ...
        new_syn = Ether(src=mac0, dst=lmac0) / IP(src=ip0, dst=ip1) / TCP(sport=sport, dport=dport, flags="S", seq=300)
