...
class TestSfdp(VppTestCase):
    ...
    def test_sfdp_api_configuration(self):
        ...
        reply = self.vapi.sfdp_set_timeout(
            tenant_id=100,
            timeout_id=VppEnum.vl_api_sfdp_timeout_t.SFDP_API_TIMEOUT_EMBRYONIC,
            timeout_value=31,
        )
        self.assertEqual(reply.retval, 0)
        ...
        reply = self.vapi.sfdp_set_timeout(
            tenant_id=100,
            timeout_id=VppEnum.vl_api_sfdp_timeout_t.SFDP_API_TIMEOUT_ESTABLISHED,
            timeout_value=3601,
        )
        self.assertEqual(reply.retval, 0)
        ...
        tenant = tenants[0]
        self.assertEqual(
            tenant.timeout[VppEnum.vl_api_sfdp_timeout_t.SFDP_API_TIMEOUT_EMBRYONIC],
            31,
            "embryonic timeout should be 31 seconds",
        )
        self.assertEqual(
            tenant.timeout[VppEnum.vl_api_sfdp_timeout_t.SFDP_API_TIMEOUT_ESTABLISHED],
            3601,
