...
class TestLldpDump(VppTestCase):
    ...
    def test_lldp_dump(self):
        ...
        #
        self.vapi.lldp_config(system_name=self.SYSTEM_NAME,
        ...
        for d in details:
                ...
                port_id = "%02x:%02x:%02x:%02x:%02x:%02x" %                \
                          (d.port_id[0], d.port_id[1], d.port_id[2],       \
                          d.port_id[3], d.port_id[4], d.port_id[5])

                chassis_id = "%02x:%02x:%02x:%02x:%02x:%02x" %                \
                          (d.chassis_id[0], d.chassis_id[1], d.chassis_id[2], \
                          d.chassis_id[3], d.chassis_id[4], d.chassis_id[5])
