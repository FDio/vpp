...
class BFDTestSession(object):
    ...
    def create_packet(self):
        if self.sha1_key:
            ...
            bfd.auth_len = SBFD.sha1_auth_len
            bfd.auth_key_id = self.bfd_key_id
            bfd.auth_seq_num = self.our_seq_number
            bfd.length = SBFD.sha1_auth_len + BFD.bfd_pkt_len
        else:
            bfd = SBFD()
        if self.af == AF_INET6:
            packet = (Ether(src=self.interface.remote_mac,
                      ...
                      IPv6(src=self.interface.remote_ip6,
                           ...
                           hlim=255) /
                      UDP(sport=self.udp_sport, dport=SBFD.udp_dport) /
                      bfd)
        else:
            packet = (Ether(src=self.interface.remote_mac,
                            dst=self.interface.local_mac) /
                      ...
                      UDP(sport=self.udp_sport, dport=SBFD.udp_dport) /
                      bfd)
        self.test.logger.debug("SBFD: Creating packet")
        for name, value in self.bfd_values.iteritems():
            self.test.logger.debug("SBFD: setting packet.%s=%s", name, value)
            packet[SBFD].setfieldval(name, value)
        if self.sha1_key:
            hash_material = str(packet[SBFD])[:32] + self.sha1_key.key + \
                "\0" * (20 - len(self.sha1_key.key))
            self.test.logger.debug("SBFD: Calculated SHA1 hash: %s" %
                                   hashlib.sha1(hash_material).hexdigest())
            packet[SBFD].auth_key_hash = hashlib.sha1(hash_material).digest()
        return packet
    ...
    def verify_sha1_auth(self, packet):
        """ Verify correctness of authentication in SBFD layer. """
        bfd = packet[SBFD]
            ...
            if self.vpp_seq_number < 0xffffffff:
                ...
                if self.sha1_key.auth_type == \
                    ...
                    self.test.assertIn(recvd_seq_num, (self.vpp_seq_number, 0),
                                       "SBFD sequence number not one of "
    ...
    def verify_bfd(self, packet):
        """ Verify correctness of SBFD layer. """
        bfd = packet[SBFD]
        self.test.assert_equal(bfd.version, 1, "SBFD version")
        self.test.assert_equal(bfd.your_discriminator,
                               self.bfd_values['my_discriminator'],
                               "SBFD - your discriminator")
        if self.sha1_key:
...
class BFDCommonCode:
    ...
    def bfd_session_up(self):
        """ Bring SBFD session up """
        self.pg_enable_capture([self.pg0])
        self.logger.info(self.vapi.cli("enable sbfd"))
        self.logger.info("SBFD: Waiting for slow hello")
        p, timeout = self.wait_for_bfd_packet(1)
        self.logger.info("SBFD: Sending UP")
        self.test_session.update(my_discriminator=randint(0, 40000000),
                                 your_discriminator=p[SBFD].my_discriminator,
                                 state=SBFDState.up,
                                 required_min_rx_interval=100000)
        self.test_session.send_packet()
        self.logger.info("SBFD: Waiting for event")
        e = self.vapi.wait_for_event(1, "bfd_udp_session_details")
        self.verify_event(e, expected_state=SBFDState.up)
        self.logger.info("SBFD: Session is Up")
        self.test_session.update(state=SBFDState.up)
        self.assert_equal(self.vpp_session.state, SBFDState.up, SBFDState)

    def bfd_session_down(self):
        """ Bring SBFD session down """
        self.assert_equal(self.vpp_session.state, SBFDState.up, SBFDState)
        self.test_session.update(state=SBFDState.down)
        self.test_session.send_packet()
        self.logger.info("SBFD: Waiting for event")
        e = self.vapi.wait_for_event(1, "bfd_udp_session_details")
        self.verify_event(e, expected_state=SBFDState.down)
        self.logger.info("SBFD: Session is Down")
        self.assert_equal(self.vpp_session.state, SBFDState.down, SBFDState)
    ...
    def verify_udp(self, packet):
        ...
        udp = packet[UDP]
        self.assert_equal(udp.dport, SBFD.udp_dport, "UDP destination port")
        self.assert_in_range(udp.sport, SBFD.udp_sport_min, SBFD.udp_sport_max,
                             "UDP source port")

    def verify_event(self, event, expected_state):
        """ Verify correctness of event values. """
        e = event
        self.logger.debug("SBFD: Event: %s" % repr(e))
        self.assert_equal(e.sw_if_index,
                          self.vpp_session.interface.sw_if_index,
                          "SBFD interface index")
        is_ipv6 = 0
        if self.vpp_session.af == AF_INET6:
            is_ipv6 = 1
        ...
        if self.vpp_session.af == AF_INET:
            ...
            self.assert_equal(e.peer_addr, self.vpp_session.peer_addr_n,
                              "Peer IPv6 address")
        self.assert_equal(e.state, expected_state, SBFDState)

    def wait_for_bfd_packet(self, timeout=1):
        """ wait for SBFD packet

        :param timeout: how long to wait max

        :returns: tuple (packet, time spent waiting for packet)
        """
        self.logger.info("SBFD: Waiting for SBFD packet")
        before = time.time()
        p = self.pg0.wait_for_packet(timeout=timeout)
        after = time.time()
        self.logger.debug(ppp("SBFD: Got packet:", p))
        bfd = p[SBFD]
        if bfd is None:
            raise Exception(ppp("Unexpected or invalid SBFD packet:", p))
        if bfd.payload:
            raise Exception(ppp("Unexpected payload in SBFD packet:", bfd))
        self.verify_ip(p)
        self.verify_udp(p)
...
class BFD4TestCase(VppTestCase, BFDCommonCode):
    """Bidirectional Forwarding Detection (SBFD)"""
