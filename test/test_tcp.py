...
class TestTCP(VppTestCase):
    ...
    def test_tcp_transfer(self):
        ...
        error = self.vapi.cli("test tcp server appns 0 uri tcp://"
                              + self.loop0.local_ip4 + "/1234")
        if error:
            self.logger.critical(error)
        error = self.vapi.cli("test tcp client mbytes 1 appns 1 uri tcp://"
                              + self.loop0.local_ip4 + "/1234 no-output")
        if error:
            self.logger.critical(error)
