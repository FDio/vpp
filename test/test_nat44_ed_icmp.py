...
class TestNATICMPError(VppTestCase):
    ...
    def test_nat_local_icmp_time_exceeded(self):
        ...
        p_ip4_reply = p_ip4
        p_ip4_reply.ttl = 0
        p4_reply = (IP(src=self.pg0.local_ip4, dst=self.pg0.remote_ip4, ttl=254, id=0) /
    ...
    def test_nat_out2in_icmp_error_checksum(self):
        ...
        p_icmp4 = ICMP(type='time-exceeded', code='ttl-zero-during-transit', chksum=0x1234)
        icmp4_reply = (IP(src='88.88.88.88',
                          dst=rx[0][IP].src,
                          ttl=254, id=0) /
                       p_icmp4 / rx[0][IP])
    ...
    def test_nat_out2in_icmp_error_short(self):
        ...
        # Bit of a hack, just setting the outer IP length and shipping packet with a bit of padding
        icmp4_reply.len = 28
        p_o2i = p_o2i_ether / icmp4_reply
        print('ICMP REPLY TO SEND:')
        icmp4_reply.show2()

        # Verify that we do not receive an ICMP back. Should be silently dropped
        rx = self.send_and_expect(self.pg1, p_o2i*1, self.pg0)
        self.assertEqual(len(rx), 0)
