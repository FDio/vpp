...
class TestGsoExperimentTestCase(VppTestCase):
    ...
    def kill_iperf_server(self):
        ...
        pid = int(open("/tmp/tap1-tcpdump.pid").read().rstrip('\x00'))
        os.kill(pid, signal.SIGKILL)
        pid = int(open("/tmp/tap2-tcpdump.pid").read().rstrip('\x00'))
        os.kill(pid, signal.SIGKILL)

    def tap_test(self, gso_cli):
        # tweak this according to needs to tune the starting sw_if_index
        # self.create_loopback_interfaces(1)
        cli = "create tap id 1 hw-addr 00:fe:00:00:00:01 rx-ring-size 1024 tx-ring-size 1024 host-ns vpp1 host-ip4-addr 172.16.1.2/24 host-ip4-gw 172.16.1.1 host-if-name vpp"
        self.logger.info(self.vapi.ppcli(cli))
        cli = "create tap id 2 hw-addr 00:fe:00:00:00:02 rx-ring-size 1024 tx-ring-size 1024 host-ns vpp2 host-ip4-addr 172.16.2.2/24 host-ip4-gw 172.16.2.1 host-if-name vpp"
        self.logger.info(self.vapi.ppcli(cli))
        ...
        cli = "ip netns exec vpp1 ethtool -k vpp"
        self.logger.info(check_output(cli.split()))
        ...
        self.logger.info(check_output(cli.split()))
        cli = "daemonize -p /tmp/tap1-tcpdump.pid -o /tmp/tap1-out.txt /sbin/ip netns exec vpp1 tcpdump -ln -i vpp -v -e"
        self.logger.info(check_output(cli.split()))
        cli = "daemonize -p /tmp/tap2-tcpdump.pid -o /tmp/tap2-out.txt /sbin/ip netns exec vpp2 tcpdump -ln -i vpp -v -e"
        self.logger.info(check_output(cli.split()))

        cli = "ip netns exec vpp2 iperf3 -c 172.16.1.2 -w 2M -t 5"
        result = ""
        try:
