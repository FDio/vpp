...
class TestPvti(VppTestCase):
    ...
    def test_0054_pvti_send_simple_500pkt_big(self):
        """v4o4 TX: Simple big packets: 500 -> 200"""
        self.prepare_for_test("v4o4_500big_pkt")

        send_pkts = self.create_packets(self.pg1, count=500, size=1050)
        self.logger.info("count: %d " % len(send_pkts))
        # self.logger.info(len(pkts))
        rx = self.send_and_expect(self.pg1, send_pkts, self.pg0, n_rx=2)
        for p in rx:
