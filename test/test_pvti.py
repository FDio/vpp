...
from scapy.fields import FlagsField, XByteField, ThreeBytesField, \
    ...
    LEIntField, ByteField, StrLenField, PacketListField, LEShortField, \
    IntField, ShortField
...
class PVTI(Packet):
    ...
    fields_desc = [
        ...
        StrLenField("pad", "X" * PVTI_ALIGN_BYTES, length_from=lambda pkt: pkt.pad_bytes),
    ...
    def post_build(self, p, pay):
        if self.chunk_count is None:
            ...
            p =  p[:offset_of_chunk_count] + struct.pack("b", l) + p[offset_of_chunk_count+1:]
        return p+pay
...
class VppPvtiInterface(VppInterface):
    ...
    def __init__(self, test, local_ip, local_port, remote_ip, remote_port, underlay_mtu=1500):
...
class TestPvti(VppTestCase):
    ...
    def create_packets(self, src_ip_if, count=1, size=150):
        pkts = []
    ...
    def create_rx_packets(self, dst_ip_if, rx_intf=None, count=1, size=150):
        ...
        for i in range(0, total_packet_count):
            ...
            p = Ether(dst=dst_mac, src=src_mac) / IP(src="192.0.2.1", dst=rx_intf.local_ip4) / UDP(sport=12312,dport=12312) / pvti
            # p.show()
            # Ether(raw(p)).show()
    ...
    def test_0002_pvti_send_simple_2pkt(self):
        ...
        pkts = self.create_packets(self.pg1, count=2)
    ...
    def prepare_for_test(self, test_name, underlay_mtu=1500):
        ...
        self.pvti0 = VppPvtiInterface(self, self.pg1.local_ip4, local_port, peer_addr, peer_port, underlay_mtu).add_vpp_config()
    ...
    def test_0003_pvti_send_simple_1pkt_big(self):
        """Simple big packet: 1 -> 2"""
        self.prepare_for_test("1big_pkt")

        pkts = self.create_packets(self.pg1, count=1, size=1900)
    ...
    def test_0005_pvti_send_mix_3pkt_medium_mtu(self):
        """ small+big+small packets over medium mtu: 3 -> 3 """
    ...
    def test_0006_pvti_send_mix_4pkt_medium_mtu(self):
        """ small+big+small packets over 600 mtu: 4 -> 3 """
    ...
    def test_0008_pvti_chained_1_3_pkt(self):
        """Chained packet: 2700 byte 1 -> 3, mtu 1000 """
        ...
        payload = rx[0][PVTI].chunks[0].data + rx[1][PVTI].chunks[0].data + rx[2][PVTI].chunks[0].data + rx[2][PVTI].chunks[1].data
        pkt1 = IP(payload)
...
@unittest.skipIf(
    True or "pvti" in config.excluded_plugins, "Exclude PVTI plugin tests"
)
