...
class PVTIChunk(Packet):
     name = "PVTIChunk"
     fields_desc = [
         LEShortField("total_chunk_length",0),
         LEShortField("_pad0",0),
         LEIntField("_pad1", 0),
...
class PVTI(Packet):
    ...
    fields_desc = [
        ...
        ByteField("pad_bytes", 3),
        StrLenField("pad", "XXX", length_from=lambda pkt: pkt.pad_bytes),
        PacketListField("chunks", [], PVTIChunk, length_from=lambda p: p.total_len - 8)
    ]

    def mysummary(self):
        return self.sprintf("PVTI (len=%PVTI.total_len%)")

bind_layers(UDP, PVTI, dport=6192)
