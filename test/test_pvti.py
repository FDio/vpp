...
class TestPvti(VppTestCase):
    ...
    def create_rx_packets(self, dst_ip_if, rx_intf=None, count=1, size=150):
        ...
        for i in range(0, total_packet_count):
            ...
            p = Ether(dst=dst_mac, src=src_mac) / IP(src="1.1.1.1", dst=rx_intf.local_ip4) / UDP(sport=12312,dport=12312) / pvti
            p.show()
            Ether(raw(p)).show()
    ...
    def prepare_for_test(self, test_name):
        local_port = 12312
        peer_addr = "192.0.2.1"
        peer_port = 31234
        peer_port = 12312
        ...
        self.pvti0 = VppPvtiInterface(self, self.pg1.local_ip4, local_port, peer_addr, peer_port).add_vpp_config()
