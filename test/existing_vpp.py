...
class ExistingVPP(object):
    ...
    def pump_output(self):
        ...
        pump the output from vpp's stdout/stderr to a deque. This
        method sends journals written by the existing vpp
        instance to stdout.
        """
...
# Detect a running VPP if told to do so
exists_vpp: Optional[object] = None
RUNNING_VPP: bool = False
if os.getenv('USE_RUNNING', '0') == '1':
    exists_vpp = ExistingVPP()
    RUNNING_VPP = exists_vpp.running
    if not RUNNING_VPP:
        print('Could not detect a running VPP when USE_RUNNING=1')
    else:
        print("@@@@ Tests will run against the below VPP instance @@@@")
        print(exists_vpp.vpp_instance)
