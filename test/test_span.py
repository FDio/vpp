...
class TestSpan(VppTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dst_sub_if = VppDot1QSubint(cls, cls.pg2, 100)
	cls.dst_sub_if.set_vtr(L2_VTR_OP.L2_PUSH_1, tag=300)
    ...
    def test_span_l2_rx(self):
        ...
        self.vapi.sw_interface_set_flags(self.sub_if.sw_if_index,
                                         admin_up_down=1)
        # Create bi-directional cross-connects between pg0 and pg1
    ...
    def test_span_l2_rx_dst_vtr(self):
        ...
        self.vapi.sw_interface_set_flags(self.sub_if.sw_if_index,
                                         admin_up_down=1)
        self.vapi.sw_interface_set_flags(self.dst_sub_if.sw_if_index,
                                         admin_up_down=1)
        ...
        pg2_expected = self.get_packet_count_for_if_idx(self.pg1.sw_if_index)
        pg1_pkts = self.pg1.get_capture()
        pg2_pkts = [self.remove_tags(p, [Tag(dot1=DOT1Q, vlan=300),
                                         Tag(dot1=DOT1Q, vlan=100)]) for p in self.pg2.get_capture(pg2_expected)]
        self.verify_capture(
            self.pg1,
            pg1_pkts,
            pg2_pkts)
        ...
        # Disable SPAN on pg0 sub if (mirrored to vxlan)
        self.vapi.sw_interface_span_enable_disable(
            self.sub_if.sw_if_index, self.dst_sub_if.sw_if_index, state=0, is_l2=1)
        self.xconnect(self.sub_if.sw_if_index, self.pg1.sw_if_index, is_add=0)
