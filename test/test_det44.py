...
class TestDET44(VppTestCase):
    ...
    def create_stream_in(self, in_if, out_if, ttl=64, session_offset=0):
        ...
        p = (
            ...
            / TCP(
                sport=self.tcp_port_in + session_offset,
        ...
        p = (
            ...
            / IP(src=in_if.remote_ip4, dst=out_if.remote_ip4, ttl=ttl)
            / UDP(
                sport=self.udp_port_in + session_offset,
    ...
    def test_session_limits(self):
        ...
        self.vapi.det44_add_del_map_sessions(
            is_add=1,
            in_addr=self.pg0.remote_ip4,
            in_plen=in_plen,
            out_addr=socket.inet_aton(self.nat_addr),
            out_plen=out_plen,
            ses_per_user=ses_per_user,
            tcp_per_user=tcp_per_user,
            udp_per_user=udp_per_user,
        ...
        for pkts in groups:
            ...
            self.logger.info(
                "Number of sessions initiated / allowed: {0} / {1}".format(
                    len(pkts), ses_per_group
                )
            )
        ...
        for pkts in groups:
            ...
            self.logger.info(
                "Number of sessions initiated / allowed: {0} / {1}".format(
                    len(pkts), ses_per_group + n_sessions_to_fail
                )
            )
    ...
    def test_session_limits_single(self):
        ...
        for pkts in groups:
            ...
            self.vapi.det44_add_del_map_sessions(
                ...
                out_addr=socket.inet_aton(self.nat_addr),
                out_plen=out_plen,
                ses_per_user=ses_per_group,
                tcp_per_user=session_limits[0],
                udp_per_user=session_limits[1],
    ...
    def test_close_sessions_in(self):
        ...
        for p, proto, port_in, port_out in zip(groups, protos, ports_in, ports_out):
            ...
            self.vapi.det44_close_session_in_proto(
                self.pg0.remote_ip4, port_in, self.pg1.remote_ip4, port_out, proto
            )
    ...
    def test_close_sessions_out(self):
        ...
        tcp_pkt = (
            Ether(src=self.pg0.remote_mac, dst=self.pg1.local_mac)
        ...
        for p, proto, port_in in zip(groups, protos, ports_in):
            ...
            self.vapi.det44_close_session_out_proto(
                self.nat_addr, sessions[0].out_port, self.pg1.remote_ip4, port_in, proto
            )
