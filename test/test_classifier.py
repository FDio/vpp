...
class TestClassifierPuntIPv6(TestClassifier):
    ...
    def test_punt_udp_ip6_fib_index(self):
        ...
        self.assertIn("ip6-punt-acl", trace)

        # Extract fib_index from the ip6-punt-acl trace entry.
        # With the fix  : ip6_main table used → pg0 is in default IPv6 VRF →
        #                 fib_index == 0.
        # Without the fix: ip4_main table used → pg0 is in IPv4 VRF ip4_table_id →
        #                  fib_index > 0.
        m = re.search(r"ip6-punt-acl.*?fib_index (\d+)", trace, re.DOTALL)
        self.assertIsNotNone(m, "fib_index not found in ip6-punt-acl trace")
