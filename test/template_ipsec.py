...
class IpsecTun4(object):
    ...
    def verify_padding(self, sa, esp_payload, decrypt_pkt):
    ...
    def verify_encrypted(self, p, sa, rxs):
        ...
        for rx in rxs:
            ...
            try:
                rx_ip = rx[IP]
                assert rx_ip.proto == socket.IPPROTO_ESP
                esp = ESP(rx_ip.payload)
                decrypt_pkt = p.vpp_tun_sa.decrypt(rx_ip)
                if not decrypt_pkt.haslayer(IP):
                    decrypt_pkt = IP(decrypt_pkt[Raw].load)
                self.verify_padding(sa, esp.data, decrypt_pkt)
