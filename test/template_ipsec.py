...
class TemplateIpsec(VppTestCase):
    ...
    def gen_encrypt_pkts(self, sa, sw_intf, src, dst, count, payload):
        return [Ether(src=sw_intf.remote_mac, dst=sw_intf.local_mac) /
                sa.encrypt(IP(src=src, dst=dst) / ICMP() / payload)
                for i in range(count)]

    def gen_encrypt_pkts6(self, sa, sw_intf, src, dst, count, payload):
        return [Ether(src=sw_intf.remote_mac, dst=sw_intf.local_mac) /
                sa.encrypt(IPv6(src=src, dst=dst) /
                           ICMPv6EchoRequest(id=0, seq=1, data=payload))
                for i in range(count)]

    def gen_pkts(self, sw_intf, src, dst, count=1, payload="X"*80):
        return [Ether(src=sw_intf.remote_mac, dst=sw_intf.local_mac) /
                IP(src=src, dst=dst) / ICMP() / payload
                for i in range(count)]

    def gen_pkts6(self, sw_intf, src, dst, count=1, payload="X"*80):
        return [Ether(src=sw_intf.remote_mac, dst=sw_intf.local_mac) /
...
class IpsecTraTests(object):
    ...
    def test_tra_basic(self, count=1, payload='X'*80):
    ...
    def test_tra_basic6(self, count=1, payload='X'*80):
...
class IpsecTun4Tests(object):
    def test_tun_basic44(self, count=1, payload='X'*80):
...
class IpsecTun6Tests(object):
    def test_tun_basic66(self, count=1, payload='X'*80):
