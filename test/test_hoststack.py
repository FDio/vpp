...
class HOSTSTACKTestCase(VppTestCase):
    """ VPP Host Stack / VCL Test """
    ...
    @classmethod
    def setUpClass(cls):
        super(HOSTSTACKTestCase, cls).setUpClass()

        cls.vapi.session_enable_disable(is_enabled=1)

    def setUp(self):
        super(HOSTSTACKTestCase, self).setUp()

    def test_hoststack_vcl_cutthru(self):
        """ run hoststack vcl cut-thru test """
        timeout = 5
        var = "BR"
        build_root = os.getenv(var, None)
        self.assertIsNotNone(build_root,
                             "Environment variable `%s' not set" % var)
        vcl_dir = "%s/build-vpp-native/vpp/.libs" % build_root
        executable = "%s/vcl_test_server" % vcl_dir
...
#        worker_server.env["VCL_DEBUG"] = "2"
        ...
        self.logger.info(self.vapi.cli("show graph"))
        worker_server.start()
        executable = "%s/vcl_test_client" % vcl_dir
