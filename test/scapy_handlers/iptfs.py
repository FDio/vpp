...
def gen_encrypt_pktstream(  # pylint: disable=W0612  # pylint: disable=R0913,R0914
    ...
    if not payload_spread:
        pstream = [
            IP(src=src, dst=dst) / ICMP(seq=i) / Raw(load="d" * payload_size)
    ...
    logger.debug(
        "gen_encrypt_pktstream: ipsec_payload_size {}".format(ipsec_payload_size)
    )

    logger.debug(
        " XXXPKT: len: {} pkt: {}".format(len(pstream[0]), pstream[0].show(dump=True))
    )
    ...
    for pkt in tunpkts:
        logger.debug(" XXX: len: {} pkt: {}".format(len(pkt), pkt.show(dump=True)))
...
class SecurityAssociation(ipsec.SecurityAssociation):
    ...
    def _get_ipsec_overhead(self):
        ...
        if self.crypt_algo.icv_size:
            logger.debug(
                "_get_ipsec_overhead: icv_size {}, iv_size {}".format(
                    self.crypt_algo.icv_size, self.crypt_algo.iv_size
                )
            )
    ...
    def encrypt_esp_raw(self, payload, seq_num=None, iv=None, esn_en=None, esn=None):
        ...
        logger.debug(
            "encrypt_esp_raw: 1: len(ip_header) {}, _ {}, len(payload) {}".format(
                len(ip_header), _, len(payload)
            )
        )
        ...
        if self.crypt_algo.name == "AES-GCM":
            ...
            logger.debug(
                "crypt_algo {}: assuming padding OK".format(self.crypt_algo.name)
            )
