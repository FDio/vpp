...
class SimpleDriver(Driver):
    ...
    def configure_tc(self, tc):
        self.test.logger.error('DBG: TC: {}'.format(tc))
    ...
    def run(self, dest):
        ...
        for tc in enumerate(self.test_cases):
            ...
            self.test.pg_start()
            capture = self.test.pg1.get_capture(filter_out_fn=None)
        ...
        test_cases = [
            {
                ...
                'remote-mappings': [
                    VppRemoteMapping(self, self.deid_ip4,
                        [{
                            "is_ip4": 1,
                            "priority": 1,
                            "weight": 1,
                ...
                'adjacencies': [
                    VppLispAdjacency(self, self.seid_ip4, self.deid_ip4)
                ]
            }
        ]
        self.test_driver = SimpleDriver(self, test_cases)
        self.test_driver.run(self.faf.get_ip4())
