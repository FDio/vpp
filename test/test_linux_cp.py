...
class TestLinuxCPOSPFMcast(TestLinuxCPNetNSBase):
    ...
    def setUpClass(cls):
        ...
        cls.phy.admin_up()
        cls.vapi.cli(
            "lcp create %s host-if hospf netns %s" % (cls.phy.name, cls.ns_name)
        )
    ...
    def test_ospf_mcast_host_to_phy(self):
        ...
        subprocess.run(
            [
                ...
                "sendp(Ether(sys.stdin.buffer.read()), iface='hospf')",
            ],
            input=bytes(pkt),
            check=True,
        )
    ...
    def test_ospf_mcast_phy_to_host(self):
        ...
        tap_pcap = "%s/tap_capture.pcap" % self.tempdir
        sniffer = subprocess.Popen(
            [
                ...
                "-c",
                "import sys; from scapy.all import *; "
                "wrpcap(sys.argv[1], sniff(iface='hospf', "
                "filter='ip proto %d', count=1, timeout=3))" % self.ospf_ip_proto,
                tap_pcap,
            ],
        ...
        try:
            ...
            sniffer.kill()
            self.fail("sniff timed out - no packet arrived on TAP")

        try:
            captured = rdpcap(tap_pcap)
        except Exception as e:
            self.fail(f"Could not read capture file: {e}")
        self.assertEqual(len(captured), 1)
        rx = captured[0]
        self.assertEqual(rx[Ether].dst, self.ospf_mcast_mac)
        self.assertEqual(rx[IP].dst, self.ospf_mcast_ip4)
