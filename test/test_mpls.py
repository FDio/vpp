...
class TestMPLS(VppTestCase):
    ...
    def verify_capture_labelled_ip4(self, src_if, capture, sent,
        ...
        try:
            ...
            for i in range(len(capture)):
                ...
                self.verify_mpls_stack(rx, mpls_labels, rx_ip.ttl, len(mpls_labels)-1)
    ...
    def verify_capture_tunneled_ip4(self, src_if, capture, sent, mpls_labels):
        try:
            ...
            for i in range(len(capture)):
                ...
                self.verify_mpls_stack(rx, mpls_labels, 255, len(mpls_labels)-1)
    ...
    def test_swap(self):
        ...
        self.vapi.mpls_route_add_del(32, # label
                                     1,  # EOS
                                     1,  # IPv4 next-hop
                                     nh_addr,
        ...
        self.vapi.mpls_route_add_del(33, # label
                                     1,  # EOS
                                     1,  # IPv4 next-hop
                                     nh_addr,
                                     self.pg0.sw_if_index,
        ...
        #
        self.vapi.mpls_route_add_del(34, # label
                                     1,  # EOS
                                     1,  # IPv4 next-hop
                                     nh_addr,
                                     0xffffffff, # sw if index
                                     0, # table-id
                                     0, # next-hop-table-id
                                     1, # next-hop-weight
                                     2, # num-out-labels,
        ...
        self.vapi.ip_add_del_route(dest_addr,
                                   ...
                                   "", # IP next-hop
                                   0xffffffff, # if index
                                   next_hop_via_label = 34,
                                   next_hop_n_out_labels = 1,
                                   next_hop_out_label_stack = [55])
    ...
    def test_bind(self):
        ...
        self.vapi.mpls_ip_bind_unbind(44,
                                      dest_addr,
                                      dest_addr_len);
    ...
    def test_imposition(self):
        ...
        #
        dest_addr = socket.inet_pton(socket.AF_INET, "10.0.0.2")
        nh_addr   = socket.inet_pton(socket.AF_INET, self.pg0.remote_ip4)
        dest_addr_len = 32
        ...
        #
        dest_addr = socket.inet_pton(socket.AF_INET, "11.0.0.1")
        nh_addr   = socket.inet_pton(socket.AF_INET, "10.0.0.1")
        dest_addr_len = 32
        ...
        #
        dest_addr = socket.inet_pton(socket.AF_INET, "11.0.0.2")
        nh_addr   = socket.inet_pton(socket.AF_INET, "10.0.0.2")
        dest_addr_len = 32

        self.vapi.ip_add_del_route(dest_addr,
                                   dest_addr_len,
                                   nh_addr,
                                   0xffffffff, # sw_if_index
                                   0, # table-id
                                   0, # next-hop-table-id
                                   1, # next-hop-weight
                                   2, # num-out-labels,
        ...
        rx = self.pg0.get_capture()
        self.verify_capture_labelled_ip4(self.pg0, rx, tx, [32, 33, 34, 44, 45])
