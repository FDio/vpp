    ...
    def test_api_flowprobe_tx_interface_add_del(self):
        ...
        with self.assertRaises(UnexpectedApiReturnValueError) as ctx:
            rv = self.vapi.flowprobe_tx_interface_add_del(
                ...
                sw_if_index=sw_if_index)
            print(vars(ctx))
        self.assertEqual(ctx.exception.retval, -100)
        ...
        # what happens when we try to add and there is an existing record?.
        rv = self.vapi.flowprobe_params(record_l3=1)
        with self.assertRaises(UnexpectedApiReturnValueError) as ctx:
            rv = self.vapi.flowprobe_tx_interface_add_del(
                is_add=1,
                which=0,
                sw_if_index=sw_if_index)
        self.assertEqual(ctx.exception.retval, 1)
