...
class TestLoopbackInterfaceCRUD(VppTestCase):
    ...
    def test_crud(self):
        ...
        for i in loopbacks:
            self.assertTrue(i.querry_vpp_lo_config_from_dump(if_dump))
            self.assertTrue(i.querry_vpp_ip4_config_from_dump(fib4_dump))
        ...
        fib4_dump = self.vapi.ip_fib_dump()
        for i in loopbacks:
            self.assertFalse(i.querry_vpp_lo_config_from_dump(if_dump))
            self.assertFalse(i.querry_vpp_ip4_config_from_dump(fib4_dump))
    ...
    @unittest.skip('Not finished. Questions sent to vpp-dev mailing list.')
    def test_down(self):
        # create
        loopbacks = self.create_loopback_interfaces(range(20))
        for i in loopbacks:
