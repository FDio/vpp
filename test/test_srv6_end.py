...
class TestSRv6EndMGTP4E(VppTestCase):
    ...
    def create_packets(self, inner):
        ...
        ip4_dst = IPv4Address(unicode(self.ip4_dst))
        # 32bit prefix + 32bit IPv4 DA + 8bit + 32bit TEID + 24bit
        dst = '\xaa' * 4 + ip4_dst.packed + '\x11' + '\xbb' * 4 + '\x11' * 3
        ip6_dst = IPv6Address(dst)

        ip4_src = IPv4Address(unicode(self.ip4_src))
        # 64bit prefix + 32bit IPv4 SA + 16 bit port + 16bit
        src = '\xcc' * 8 + ip4_src.packed + '\xdd' * 2 + '\x11' * 2
    ...
    def test_srv6_end(self):
        """ test_srv6_end """
        pkts = self.create_packets([
            ("A::1", "B::1"),
            ("C::1", "D::1"),
            ])

        self.vapi.cli("sr localsid address {} behavior end.m.gtp4.e".format(
            pkts[0]['IPv6'].dst))
