...
def find_gbp_endpoint(test, sw_if_index, ip=None, mac=None):
    ...
    for ep in eps:
        ...
        if ep.endpoint.sw_if_index != sw_if_index:
            continue
        for eip in ep.endpoint.ips[0]:
            if compare_ip_address(eip, vip):
                return True
    return False
...
class VppGbpEndpoint(VppObject):
    ...
    def __init__(self, test, itf, epg, recirc, ips):
        self._test = test
        self.itf = itf
        self.epg = epg
        self.recirc = recirc
        ...
        for ip in ips:
            self.ips.append([VppIpAddress(ip[0]),
                             VppIpAddress(ip[1])])
    ...
    def query_vpp_config(self):
        ...
        for s in ss:
            if s.subnet.table_id == self.table_id and \
               compare_ip_prefix(s.subnet.prefix, self.prefix):
                return True
        return False
...
class TestGBP(VppTestCase):
    ...
    def test_gbp(self):
        ...
        pkt_bcast = (Ether(dst="ff:ff:ff:ff:ff:ff",
                           src=self.pg0.remote_mac) /
                     IP(src=eps[0].ips[0][0].address, dst="232.1.1.1") /
        ...
        pkt_intra_epg = (Ether(src=self.pg0.remote_mac,
                         ...
                         IP(src=eps[0].ip.address, dst=eps[1].ip.address) /
                         UDP(sport=1234, dport=1234) /
                         Raw('\xa5' * 100))
        ...
        pkt_inter_epg_220_to_221 = (Ether(src=self.pg0.remote_mac,
                                    ...
                                    IP(src=eps[0].ip.address,
                                       dst=eps[2].ip.address) /
                                    UDP(sport=1234, dport=1234) /
                                    Raw('\xa5' * 100))
        pkt_inter_epg_221_to_220 = (Ether(src=self.pg2.remote_mac,
                                          dst=self.pg0.remote_mac) /
                                    IP(src=eps[2].ip.address,
                                       dst=eps[0].ip.address) /
                                    UDP(sport=1234, dport=1234) /
                                    Raw('\xa5' * 100))
        pkt_inter_epg_220_to_222 = (Ether(src=self.pg0.remote_mac,
                                          dst=self.router_mac) /
                                    IP(src=eps[0].ip.address,
                                       dst=eps[3].ip.address) /
                                    UDP(sport=1234, dport=1234) /
                                    Raw('\xa5' * 100))
        ...
        for ep in eps:
            ...
            self.vapi.nat44_add_del_static_mapping(ep.ip.bytes,
                                                   ep.fip.bytes,
                                                   vrf_id=0,
                                                   addr_only=1,
                                                   is_add=0)
