...
class VppGbpExtItf(VppObject):
    ...
    def __init__(self, test, itf, bd, rd, anon=False):
        ...
        self.rd = rd
        self.anon = anon
        self.config_fn = self._test.vapi.gbp_ext_itf_add_del
        if anon:
            self.config_fn = self.anon_add_del__

    def anon_add_del__(self, is_add, sw_if_index, bd_id, rd_id):
        self._test.vapi.gbp_ext_itf_anon_add_del(
            is_add=is_add,
            ext_itf={
    ...
    def add_vpp_config(self):
        self.config_fn(1, self.itf.sw_if_index, self.bd.bd_id, self.rd.rd_id)
        self._test.registry.register(self, self._test.logger)

    def remove_vpp_config(self):
        self.config_fn(0, self.itf.sw_if_index, self.bd.bd_id, self.rd.rd_id)

    def object_id(self):
        return "gbp-ext-itf:[%d]%s" % (self.itf.sw_if_index,
                                       " [anon]" if self.anon else "")
