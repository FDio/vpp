...
class VppGbpEndpoint(VppObject):
    ...
    def __init__(self, test, itf, epg, recirc, ip, fip=None, is_ip6=False):
        self._test = test
        self.itf = itf
        self.epg = epg
        self.recirc = recirc
        ...
        self.is_ip6 = is_ip6
        if is_ip6:
            self.ip_n = inet_pton(AF_INET6, ip)
        else:
            self.ip_n = inet_pton(AF_INET, ip)
        if fip:
            if is_ip6:
                self.floating_ip_n = inet_pton(AF_INET6, fip)
            else:
                self.floating_ip_n = inet_pton(AF_INET, fip)
...
class VppGbpEndpointGroup(VppObject):
    ...
    def __init__(self, test, epg, rd, bd, uplink,
        ...
        self.bvi_ip = bvi_ip
        if is_ip6:
            self.bvi_ip_n = inet_pton(AF_INET6, bvi_ip)
        else:
            self.bvi_ip_n = inet_pton(AF_INET, bvi_ip)
        self.epg = epg
        self.bd = bd
        self.rd = rd
...
class TestGBP(VppTestCase):
    ...
    def test_gbp(self):
        ...
        # Bridge Domains
        #
        self.vapi.bridge_domain_add_del(1, flood=1, uu_flood=0, forward=1,
                                        learn=0, arp_term=1, is_add=1)
        self.vapi.bridge_domain_add_del(2, flood=1, uu_flood=0, forward=1,
                                        learn=0, arp_term=1, is_add=1)
        self.vapi.bridge_domain_add_del(20, flood=1, uu_flood=0, forward=1,
                                        learn=0, arp_term=1, is_add=1)
        ...
        #
        pkt_intra_epg_220_l3 = (Ether(src=self.pg0.remote_mac,
                                      dst=self.router_mac) /
                                IP(src=eps[0].ip, dst="10.0.0.99") /
                                UDP(sport=1234, dport=1234) /
                                Raw('\xa5' * 100))
        pkt_inter_epg_222_l3 = (Ether(src=self.pg0.remote_mac,
                                      dst=self.router_mac) /
                                IP(src=eps[0].ip, dst="10.0.1.99") /
                                UDP(sport=1234, dport=1234) /
                                Raw('\xa5' * 100))

        self.send_and_assert_no_replies(self.pg0, pkt_intra_epg_220_l3 * 65)

        #
        # Add the subnet routes
        #
        self.vapi.gbp_subnet_add_del(1, 0, 1,
                                     inet_pton(AF_INET, "10.0.0.0"), 24)
        self.vapi.gbp_subnet_add_del(1, 0, 1,
                                     inet_pton(AF_INET, "10.0.1.0"), 24)
        self.vapi.gbp_subnet_add_del(1, 0, 1,
                                     inet_pton(AF_INET, "10.0.2.0"), 24)

        self.send_and_expect_bridged(self.pg0,
                                     pkt_intra_epg_220_l3 * 65,
                                     self.pg4)
        self.send_and_expect_bridged(self.pg3,
                                     pkt_inter_epg_222_l3 * 65,
                                     self.pg6)

        self.logger.info(self.vapi.cli("sh ip fib 10.0.2.1"))
        ...
        self.vapi.gbp_subnet_add_del(1, 0, 0,
                                     inet_pton(AF_INET, "0.0.0.0"), 0,
                                     recirc_nat.recirc.sw_if_index,
                                     epg_nat.epg)
        self.vapi.gbp_subnet_add_del(1, 00, 0,
                                     inet_pton(AF_INET, "11.0.0.0"), 8,
                                     recirc_nat.recirc.sw_if_index,
                                     epg_nat.epg)
        # in the NAT RD an external subnet via the NAT EPG's uplink
        self.vapi.gbp_subnet_add_del(1, 20, 0,
                                     inet_pton(AF_INET, "0.0.0.0"), 0,
                                     epg_nat.uplink.sw_if_index,
                                     epg_nat.epg)
        self.vapi.gbp_subnet_add_del(1, 20, 0,
                                     inet_pton(AF_INET, "11.0.0.0"), 8,
                                     epg_nat.uplink.sw_if_index,
        ...
        self.vapi.gbp_subnet_add_del(0, 0, 1,
                                     inet_pton(AF_INET, "10.0.0.0"), 24)
        self.vapi.gbp_subnet_add_del(0, 0, 1,
                                     inet_pton(AF_INET, "10.0.1.0"), 24)
        self.vapi.gbp_subnet_add_del(0, 0, 1,
                                     inet_pton(AF_INET, "10.0.2.0"), 24)
        self.vapi.gbp_subnet_add_del(0, 0, 0,
                                     inet_pton(AF_INET, "0.0.0.0"), 0,
                                     recirc_nat.recirc.sw_if_index)
        self.vapi.gbp_subnet_add_del(0, 0, 0,
                                     inet_pton(AF_INET, "11.0.0.0"), 8,
                                     recirc_nat.recirc.sw_if_index)
        self.vapi.gbp_subnet_add_del(0, 20, 0,
                                     inet_pton(AF_INET, "0.0.0.0"), 0,
                                     epg_nat.uplink.sw_if_index)
        self.vapi.gbp_subnet_add_del(0, 20, 0,
                                     inet_pton(AF_INET, "11.0.0.0"), 8,
