...
from cdp import CDPVersion, CDP, TLVType, \
    TLV, CustomTLV 
...
class TestCDP(VppTestCase):
    ...
    nen_ptr = compile("not enabled")
    cdp_ptr = compile("^([-\.\w]+)\s+([-\.\w]+)\s+([-\.\w]+)\s+([-\.\w]+)$")
    err_ptr = compile("^([\d]+)\s+([-\w]+)\s+([ -\.\w)(]+)$")
    ...
    def test_send_cdp_packet(self):
        ...
        self.assert_equal(system, self.device_id, "CDP received invalid device id")

    def test_send_cdp_bad_packet(self):
        self.logger.info(self.vapi.cli("cdp enable"))
        self.send_packet(self.create_bad_packet(3, "."))
    ...
    def create_packet(self):
        packet = (self.create_base_packet() /
                  TLV(type=TLVType.device_id, value=self.device_id) /
                  TLV(type=TLVType.version, value=self.version) /
                  TLV(type=TLVType.port_id, value=self.port_id) /
                  TLV(type=TLVType.platform, value=self.platform))
        return packet

    def create_bad_packet(self, tl=4, tv=""):
        packet = (self.create_base_packet() /
                  CustomTLV(type=TLVType.device_id,
                            length=tl,
                            value=tv))
        return packet
