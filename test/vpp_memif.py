...
class VppMemif(VppObject):
    ...
    def admin_up(self):
        if self.sw_if_index:
            return self._test.vapi.sw_interface_set_flags(sw_if_index=self.sw_if_index, flags=1)

    def admin_down(self):
        if self.sw_if_index:
            return self._test.vapi.sw_interface_set_flags(sw_if_index=self.sw_if_index, flags=0)
