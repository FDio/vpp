...
if __name__ == '__main__':
    ...
    parallel = os.getenv("PARALLEL", "1").lower()
    if parallel == 'auto':
        concurrent_tests = cpu_count()
        if concurrent_tests > 10:
            # TODO figure out the right maximum
            # (cpus/shared memory/regular memory)
            concurrent_tests = 10
    elif parallel.isdigit():
        concurrent_tests = int(parallel)
    else:
        concurrent_tests = 1
    ...
    if run_interactive and concurrent_tests > 1:
        raise NotImplemented(
            ...
            'set) in parallel (PARALLEL is more than 1) is not supported')
