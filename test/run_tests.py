...
if __name__ == '__main__':
    ...
    test_processes = os.getenv("TEST_PROCESSES", "1").lower()
    if test_processes == 'auto':
        # TODO should we use free instead of total?
        shm_size = psutil.disk_usage('/dev/shm').total
        shm_max_processes = 1
        if shm_size < min_req_shm:
            raise Exception('/dev/shm is too small. Please enlarge it to at'
                            ' least %sM.' % (min_req_shm >> 20))
        else:
            extra_shm = shm_size - min_req_shm
            shm_max_processes += extra_shm / shm_per_process
        concurrent_tests = max(cpu_count(), shm_max_processes)
    elif test_processes.isdigit():
        concurrent_tests = int(test_processes)
    else:
        concurrent_tests = 1
    ...
    if run_interactive and concurrent_tests > 1:
        raise NotImplemented(
            ...
            'set) in parallel (TEST_PROCESSES is more than 1) is not '
            'supported')
