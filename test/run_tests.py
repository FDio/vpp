...
def _handle_failed_suite(logger, last_test_temp_dir, vpp_pid, vpp_binary):
    if last_test_temp_dir:
        ...
        if uses_coredumpctl() and not is_core_present(last_test_temp_dir):
            ...
            extract_coredumpctl_core(logger, last_test_temp_dir, vpp_pid, wait=False)
    ...
    try:
        while wrapped_testcase_suites or testcase_suites:
            ...
            for finished_testcase in finished_testcase_suites:
                ...
                # it finished - so we terminate it and join it.
                ...
                if join_end - join_start >= test_finished_join_timeout:
                    ...
                    finished_testcase.child.terminate()
                    time.sleep(0.1)
                    if not finished_testcase.child.is_alive():
                        finished_testcase.child.join(1.0)
