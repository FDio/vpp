...
from framework import VppTestRunner, running_extended_tests, VppTestCase, get_testcase_doc_name
from debug import spawn_gdb
from log import get_parallel_logger, double_line_delim, RED, YELLOW, GREEN, colorize
...
def stdouterr_reader_wrapper(unread_testcases, finished_unread_testcases, read_testcases):
    read_testcase = None
...
def parse_results(results):
    ...
    class NonPassedResults(object):
        ...
        def _add_result(self, test, result):
            ...
            if isinstance(test, VppTestCase):
                ...
                if not identifier in self.results_per_suite:
                    # failed, errored, skipped, expectedly failed, unexpectedly passed
                    self.results_per_suite[identifier] = {'failures': [],
                                                          'errors': [],
                                                          'skipped': [],
                                                          'expectedFailures': [],
                                                          'unexpectedSuccesses': []}
    ...
    print('          Passed tests: {}'.format(colorize(str(all_testcases - results_per_suite.all_nonpassed()), GREEN)))
    if results_per_suite.all_failed > 0:
        print('          Failed tests: {}'.format(colorize(str(results_per_suite.all_failed), RED)))
    if results_per_suite.all_errored > 0:
        print('         Errored tests: {}'.format(colorize(str(results_per_suite.all_errored), RED)))
    if results_per_suite.all_skipped > 0:
        print('         Skipped tests: {}'.format(colorize(str(results_per_suite.all_skipped), YELLOW)))
    if results_per_suite.all_expected_failures > 0:
        print('     Expected failures: {}'.format(colorize(str(results_per_suite.all_expected_failures), GREEN)))
    if results_per_suite.all_unexpected_successes > 0:
        print('  Unexpected successes: {}'.format(colorize(str(results_per_suite.all_unexpected_successes), YELLOW)))

    if results_per_suite.all_failed > 0:
        print('FAILED TESTS:')
        for doc_name, failed_tests in results_per_suite.results_per_suite:
            print('  Testcase doc name: {}'.format(colorize(doc_name, RED)))
            for failed_test in failed_tests['failures']:
                print('     FAILED: {}'.format(colorize(failed_test.id().split('.')[2], RED)))
            for failed_test in failed_tests['errors']:
                print('    ERRORED: {}'.format(colorize(failed_test.id().split('.')[2], RED)))
...
if __name__ == '__main__':
    ...
    force_foreground = os.getenv("FORCE_FOREGROUND", "n").lower() in ("y", "yes", "1")
    ...
    if run_interactive and parallel:
        raise NotImplemented('Ruunning tests interactively (DEBUG, STEP or FORCE_FOREGROUND is set)'
                             'in parallel (PARALLEL is set) is not supported')
    ...
    if run_interactive:
        ...
        sys.exit(not VppTestRunner(
            verbosity=verbose, failfast=failfast).run(suites[0]).wasSuccessful())
    else:
        exit_code = 1
