...
def parse_results(results):
        ...
        def _add_result(self, test, result):
            if isinstance(test, VppTestCase):
                ...
                if len(parts) == 3:
                    ...
                    self.results_per_suite[tc_class][result].append(test)
                    return tc_class
            return None

        def add_failure(self, test):
            tc_class = self._add_result(test, self.failures_id)
            if tc_class:
                self.all_failed += 1
            return tc_class

        def add_error(self, test):
            tc_class = self._add_result(test, self.errors_id)
            if tc_class:
                self.all_failed += 1
            return tc_class

        def add_skip(self, test):
            tc_class = self._add_result(test, self.skipped_id)
            if tc_class:
                self.all_failed += 1
            return tc_class

        def add_expected_failure(self, test):
            tc_class = self._add_result(test, self.expectedFailures_id)
            if tc_class:
                self.all_failed += 1
            return tc_class

        def add_unexpected_success(self, test):
            tc_class = self._add_result(test, self.unexpectedSuccesses_id)
            if tc_class:
                self.all_failed += 1
            return tc_class
    ...
    for result in results:
        ...
        new_suite = suite_from_failed(testcase_suite, failed_testcase_classes)
        if new_suite.countTestCases():
            rerun.append(new_suite)
    ...
    if results_per_suite.all_nonpassed > 0:
        ...
        for testcase_class, suite_results in \
            ...
            failed_testcases = suite_results[results_per_suite.failures_id]
            errored_testcases = suite_results[results_per_suite.errors_id]
