...
from cpu_config import num_cpus, max_cpus, available_cpus
...
def test_runner_wrapper(suite, keep_alive_pipe, stdouterr_queue,
    ...
    runner = VppTestRunner(keep_alive_pipe=keep_alive_pipe,
...
def run_forked(testcase_suites):
    ...
    while free_cpus and testcase_suites:
        ...
        if a_suite.is_tagged_run_solo:
            ...
            continue
        if a_suite.cpus_used < len(free_cpus) and\
                tests_running < max_concurrent_tests:
            a_suite = testcase_suites.pop(0)
            run_suite(a_suite)
        else:
            break
                ...
                if stop_run:
                    ...
                    while a_suite and a_suite.is_tagged_run_solo:
                        ...
                        if testcase_suites:
                            ...
                            a_suite = None
                    if a_suite and tests_running < max_concurrent_tests and\
                            a_suite.cpus_used < len(free_cpus):
                        run_suite(a_suite)
...
class TestSuiteWrapper(unittest.TestSuite):
    ...
    def addTest(self, test):
        if test.get_cpus_used() <= max_cpus:
            self.cpus_used = max(self.cpus_used, test.get_cpus_used())
        super().addTest(test)
...
if __name__ == '__main__':
    ...
    if test_jobs == 'auto':
        if run_interactive:
            ...
            print('Running at most %s tests concurrently.' %
        ...
        print("Running at most %s tests concurrently as set by 'TEST_JOBS'." %
              max_concurrent_tests)

    print("Running on at most %s cpus." % max_cpus)
    ...
    if run_interactive and suites:
        ...
        print('Running each VPPTestCase in a separate background process'
              ' with {} parallel process(es)'.format(max_concurrent_tests))
        exit_code = 0
