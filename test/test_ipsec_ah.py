...
* ipsec ah v4 transport basic test  - IPv4 Transport mode scenario using
  HMAC-SHA1-96 intergrity algo
* ipsec ah v4 transport burst test  - Above test for 256 pkts
* ipsec ah 4o4 tunnel basic test    - IPv4 Tunnel mode scenario using
  HMAC-SHA1-96 intergrity algo
    ...
    def test_ipsec_ah_tun_basic(self, count=1):
        ...
        try:
            ...
            for recv_pkt in recv_pkts:
                assert(
                    recv_pkt[IP].src == self.remote_pg0_lb_addr and
                    recv_pkt[IP].dst == self.remote_pg1_lb_addr)
            ...
            for recv_pkt in recv_pkts:
                ...
                assert(
                    decrypt_pkt.src == self.remote_pg1_lb_addr and
                    decrypt_pkt.dst == self.remote_pg0_lb_addr)
        finally:
            self.logger.info(self.vapi.ppcli("show error"))
            self.logger.info(self.vapi.ppcli("show ipsec"))
