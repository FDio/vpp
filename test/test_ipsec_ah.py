...
class TestIpsecAh(VppTestCase):
    ...
    def configScapySA(self, is_tun=False):
        if is_tun:
            ...
            self.remote_tra_sa = SecurityAssociation(
                AH, spi=0x000007d0, auth_algo='HMAC-SHA1-96', auth_key='C91KUR9GYMm5GfkEvNjX')
            self.local_tra_sa = SecurityAssociation(
                AH, spi=0x000007d1, auth_algo='HMAC-SHA1-96', auth_key='C91KUR9GYMm5GfkEvNjX')
    ...
    def gen_encrypt_pkts(self, sa, sw_intf, src, dst, count=1):
        gen_pkts = list()
        icmp_pkt = IP(src=src, dst=dst) / ICMP() / \
            "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
        for _ in range(count):
            encrypt_pkt = sa.encrypt(icmp_pkt)
            gen_pkts.append(
                Ether(
                    src=sw_intf.remote_mac,
                    dst=sw_intf.local_mac) /
                encrypt_pkt)
        return gen_pkts
    ...
    def test_ipsec_ah_tun_basic(self, count=1):
        ...
        try:
            ...
            for recv_pkt in recv_pkts:
                assert(
                    recv_pkt[IP].src == self.remote_pg0_lb_addr and recv_pkt[IP].dst == self.remote_pg1_lb_addr)
            ...
            for recv_pkt in recv_pkts:
                ...
                assert(
                    decrypt_pkt.src == self.remote_pg1_lb_addr and decrypt_pkt.dst == self.remote_pg0_lb_addr)
        finally:
            self.logger.info(self.vapi.ppcli("show error"))
            self.logger.info(self.vapi.ppcli("show ipsec"))
