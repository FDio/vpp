...
class TestSRv6(VppTestCase):
    ...
    def setUp(self):
        ...
        **Config:**
        ...
        # some tests require to enable IPv4: End.DX4, T.encaps(IPv4)
        if self._testMethodName in ['test_SRv6_End_DX4',
                                    'test_SRv6_T_encaps_IPv4']:
            for i in self.pg_interfaces:
                i.config_ip4()
                i.resolve_arp()
...
#            i.admin_down()
    ...
    def test_SRv6_T_encaps_IPv4(self):
        ...
        # steer IPv4 traffic to 7.0.0.0/24 into SRv6 Policy
        # use the bsid of the above self.sr_policy
        pol_steering = VppSRv6Steering(
                        self,
                        bsid=inet_ntop(AF_INET6, self.sr_policy.bsid),
                        prefix="7.0.0.0", mask_width=24,
        ...
        # create IPv4 packets
        packet_header = self.create_packet_header_IPv4(dst_in)
        # create traffic stream pg0->pg1
        pkts.extend(self.create_stream(self.pg0, self.pg1, packet_header,
                                       self.pg_packet_sizes, count))
    ...
    def compare_rx_tx_packet_T_Encaps_IPv4(self, tx_pkt, rx_pkt):
        ...
        """
        compare input and output packet after going through
        the T.Encaps behavior (IPv4 variant)
        """
        # get first (outer) IPv6 header of rx'ed packet
        rx_ip = rx_pkt.getlayer(IPv6)
        rx_srh = None
    ...
    def create_packet_header_IPv4(self, dst):
        """Create IPv4 packet: IPv4 header with UDP header

        :param dst: IPv4 destination address

        IPv4 source address is 123.0.0.1
        UDP source port and destination port are 1234
        """

        p = (IP(src='123.0.0.1', dst=dst) /
             UDP(sport=1234, dport=1234))
        return p
