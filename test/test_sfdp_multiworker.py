...
#   In timer.h, sfdp_session_timer_update used vlib_get_main() to send the
#   interrupt. vlib_get_main() always returns the main thread (thread 0).
#   When sessions are owned by worker threads (thread 1, 2, ...), the interrupt
#   never reaches the owning worker's expire_node, so the kill is not
#   immediate.
#
# NOTE on API vs CLI kill:
#   The VAPI sfdp_kill_session handler (vl_api_sfdp_kill_session_t_handler)
#   does NOT call vlib_worker_thread_barrier_sync, so it races with active
#   workers modifying their timer wheels.  CLI commands run under barrier_sync
#   (workers are stopped), which makes them safe.  All kill operations in this
#   test therefore use the CLI path ("kill sfdp session ...").
#
import unittest
...
class TestSfdpKillMultiWorker(VppTestCase):
    ...
    # 20ms per tick, same as TestSfdpTimer; sessions-log2 5 = 32 sessions (small footprint)
    ...
    def tearDown(self):
        # Use CLI (barrier sync) to avoid racing with active worker threads
        self.vapi.cli("kill sfdp session all")
    ...
    def test_kill_all_sessions_on_worker(self):
        ...
        With vlib_get_main() (broken): the interrupt targets thread 0 (main)
        even for sessions owned by a worker thread.  The worker's expire_node
        never receives the interrupt, so sessions are not killed within one tick.

        With vlib_get_main_by_index(thread_index) (fixed): the interrupt goes
        ...
        # Kill all sessions using CLI (runs under barrier_sync, workers stopped)
        self.vapi.cli("kill sfdp session all")
    ...
    def test_kill_individual_session_on_worker(self):
        ...
        Ensures that the interrupt targets the correct worker, not just
        the kill_all path.
        """
        ...
        # Kill by index using CLI (barrier_sync stops the owning worker)
        self.vapi.cli(f"kill sfdp session {sess.session_idx}")
    ...
    def test_kill_rearmed_session_on_worker(self):
        """Killing a rearmed session on a worker thread works within one tick
        ...
        be immediate regardless of which pass the timer is on.
        """
