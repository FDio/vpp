...
class TestVxlan(BridgeDomain, VppTestCase):
    ...
    def check_encapsulation(self, pkt, vni, local_only=False):
        ...
        if not local_only: self.assertEqual(pkt[Ether].dst, self.pg0.remote_mac)
        # Verify VXLAN tunnel source IP is VPP_IP and destination IP is MY_IP.
        self.assertEqual(pkt[IP].src, self.pg0.local_ip4)
        if not local_only: self.assertEqual(pkt[IP].dst, self.pg0.remote_ip4)
    ...
    def add_del_mcast_load(cls, is_add):
        for dest_addr in cls.ip4_range(cls.mcast_ip4n, 10, 210):
    ...
    def setUpClass(cls):
	    ...
	    cls.mcast_mac4 = "01:00:5e:%02x:%02x:%02x" % (
            ...
            cls.add_del_mcast_load(1)
            cls.add_del_mcast_load(0)
