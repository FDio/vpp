...
class TestSnortVapi(VppTestCase):
    ...
    def test_snort_03_create(self):
        reply = self.vapi.snort_instance_create(queue_size = 8, drop_on_disconnect = 0, name = "snortTest0")
        self.assertEqual(reply.instance_index, 0)
        reply = self.vapi.snort_instance_create(queue_size = 32, drop_on_disconnect = 1, name = "snortTest1")
    ...
    def test_snort_04_attach_if(self):
        reply = self.vapi.snort_interface_attach(instance_index = 0,
                                        sw_if_index = 0,
                                        snort_dir = 1)
        try:
            reply = self.vapi.snort_interface_attach(instance_index = 1,
                                        sw_if_index = 0,
                                        snort_dir = 1)
        except:
            pass
