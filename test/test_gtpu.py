...
class TestGtpuUDP(VppTestCase):
    ...
    def _check_udp_port_ip4(self, enabled=True):
        ...
        err = self.statistics.get_counter('/err/ip4-udp-lookup/no listener for dst port')
    ...
    def _check_udp_port_ip6(self, enabled=True):
        ...
        err = self.statistics.get_counter('/err/ip6-udp-lookup/no listener for dst port')
    ...
    def test_udp_port(self):
        ...
        r = self.vapi.gtpu_add_del_tunnel(is_ipv6 = 1, src_addr=self.pg0.local_ip6n,
                                          dst_addr=self.pg0.remote_ip6n)

        # UDP port 2152 enabled for ip6
        self._check_udp_port_ip6()

        r = self.vapi.gtpu_add_del_tunnel(is_add = 0, src_addr=self.pg0.local_ip4n,
                                          dst_addr=self.pg0.remote_ip4n)
