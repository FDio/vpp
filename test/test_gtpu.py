...
class TestGtpu(BridgeDomain, VppTestCase):
    ...
    def encapsulate(self, pkt, teid):
        ...
        return (Ether(src=self.pg0.remote_mac, dst=self.pg0.local_mac) /
                ...
                GTP_U_Header(TEID=teid, gtp_type=self.gtp_type) /
                pkt)

    def encap_mcast(self, pkt, src_ip, src_mac, teid):
        """
        Encapsulate the original payload frame by adding GTPU header with its
        UDP, IP and Ethernet fields
        """
    ...
    def decapsulate(self, pkt):
        ...
        """
        # check if is GTP_U Header
        self.assertEqual(pkt[GTP_U_Header].gtp_type, int('0xff', 16))
        return pkt[GTP_U_Header].payload

    # Method for checking GTPU encapsulation.
    #
    def check_encapsulation(self, pkt, teid, local_only=False, mcast_pkt=False):
        ...
        # Verify TEID
        self.assertEqual(pkt[GTP_U_Header].TEID, teid)
