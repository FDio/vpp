...
class TestL2bd(VppTestCase):
    ...
    def setUp(self):
        ...
        self.MY_MACS = dict()
        self.MY_IP4S = dict()
        self.create_mac_entries(TestL2bd.mac_entries)
    ...
    def create_mac_entries(self, count):
        """
        ...
        for pg_if in self.pg_interfaces:
            ...
            self.MY_MACS[pg_if.sw_if_index] = []
            self.MY_IP4S[pg_if.sw_if_index] = []
            packets = []
            for j in range(start_nr, end_nr):
                self.MY_MACS[pg_if.sw_if_index].append(
                    "00:00:00:ff:%02x:%02x" % (pg_if.sw_if_index, j))
                self.MY_IP4S[pg_if.sw_if_index].append(
                    "172.17.1%02x.%u" % (pg_if.sw_if_index, j))
                host_id = len(self.MY_MACS[pg_if.sw_if_index])-1
                packet = (Ether(dst="ff:ff:ff:ff:ff:ff",
                                src=self.MY_MACS[pg_if.sw_if_index][host_id]))
