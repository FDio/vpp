...
test_pinning_conf () {
    ...
    if [ -n "$RELATIVE" ];
	then
		VPP_CPU_EXTRA_OPTIONS="${VPP_CPU_EXTRA_OPTIONS} relative"
	fi
	...
	nodaemon cli-listen $VPP_SOCK_PATH/cli.sock} cpu { \n main-core ${MAIN_CORE} \n  ${VPP_CPU_EXTRA_OPTIONS} } plugins \
	{ plugin dpdk_plugin.so {disable } }"

	(docker run -d $VPP_CONTAINER_CPUSET --name="$DOCKER_CONTAINER_NAME" --privileged \
	-e LD_LIBRARY_PATH="/vpp/build-root/build-vpp_debug-native/vpp/lib/x86_64-linux-gnu/" -v $VPP_SOCK_PATH:$VPP_SOCK_PATH \
	-v $WS_ROOT:/vpp -v /lib/x86_64-linux-gnu:/lib/x86_64-linux-gnu  ubuntu:22.04 /usr/bin/bash -c "echo '$VPP_CONF' > /tmp/vpp-test.conf && \
...
test_invalid_conf () {
	...
	nodaemon cli-listen $VPP_SOCK_PATH/cli.sock} cpu {\n main-core ${MAIN_CORE} \n  ${VPP_CPU_EXTRA_OPTIONS} } plugins \
	{ plugin dpdk_plugin.so {disable } }"
	(docker run -d $VPP_CONTAINER_CPUSET --privileged --name="$DOCKER_CONTAINER_NAME" \
	-e LD_LIBRARY_PATH="/vpp/build-root/build-vpp_debug-native/vpp/lib/x86_64-linux-gnu/" -v $VPP_SOCK_PATH:$VPP_SOCK_PATH \
	-v $WS_ROOT:/vpp -v /lib/x86_64-linux-gnu:/lib/x86_64-linux-gnu  ubuntu:22.04 /usr/bin/bash -c "echo '$VPP_CONF' > /tmp/vpp-test.conf && \
	/vpp/build-root/build-vpp_debug-native/vpp/bin/vpp -c /tmp/vpp-test.conf" > /dev/null )
	sleep 3 # wait for vpp to initialize socket
