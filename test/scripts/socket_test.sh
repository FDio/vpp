...
tmp_gdb_cmdfile_client="${tmp_gdb_cmdfile_prefix}_vppcom_client.$$"
tmp_gdb_cmdfile_server="${tmp_gdb_cmdfile_prefix}_vppcom_server.$$"
...
OPTIONS:
  ...
  -g a[ll]            Run all in gdb.
     ...
     v[pp]            Run vpp in gdb.
  -p a[ll]            Run "perf top" on all.
     c[lient]         Run "perf top" on client.
     s[erver]         Run "perf top" on server.
     v[pp]            Run "perf top" vpp.
...
while getopts ":hitlbcde:g:p:E:N:P:R:S:T:UBVX" opt; do
    case $opt in
        h) usage ;;
        l) leave_tmp_files=1
           ;;
        ...
        P) sock_srvr_port="$OPTARG"
           ;;
  E|N|R|T) sock_clnt_options="$sock_clnt_options -$opt \"$OPTARG\""
           ;;
