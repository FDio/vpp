...
mount -t 9p /dev/libmod9p /lib/modules
depmod -a
modprobe -a vhost_net vhost tap
lsmod
...
--venv-dir=${VENV_PATH} --vpp-ws-dir=${WS_ROOT}"
poweroff -f
_EOF_
    ...
    sudo taskset -c ${CPU_MASK} ${QEMU} \
                 ...
                 -virtfs local,path=/tmp/vpp-vm-tests/lib/modules,mount_tag=/dev/libmod9p,security_model=none,id=libmod9p,multidevs=remap \
