...
# Make custom VPP startup.conf for running unit tests (Optional)
...
run_in_vm
# Set the ownership back to the original user after the QEMU run
USER=$(id -u -n)
sudo chown -R ${USER}:${USER} ${WS_ROOT} \
                              /tmp/vpp-failed-unittests \
                              /tmp/vpp-unittest-* \
                              /tmp/api_post_mortem.* \
                              ${TEST_DATA_DIR} &>/dev/null
