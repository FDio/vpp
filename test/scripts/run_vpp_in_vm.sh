...
if [[ "$#" -ne 3 ]]; then
    echo "ERROR: Incorrect number of arguments..3 args required -
    (TEST_SELECTION, KERNEL_BIN, TEST_DATA_DIR)"
    exit 1
fi

if [[ -z "${1:-}" ]]; then
    echo "ERROR: A non-empty test selection is required to run 
    tests in the QEMU VM"
    exit 1
fi
TEST=${1:-}
...
# Download a KVM Kernel Image for Debian/Ubuntu Pkgs.
if [[ -z "${2:-}" ]] || [[ ! -f "${2:-}" ]]; then
...
if [[ -z "${WS_ROOT:-}" ]]; then
    echo "ERROR: WS_ROOT envvar is not set"
    exit 1
fi
if [[ -z "${RND_SEED:-}" ]]; then
    echo "ERROR: RND_SEED envvar is not set"
    exit 1
fi
if [[ -z "${BR:-}" ]]; then
    echo "ERROR: Build Root (BR) envvar is not set"
    exit 1
fi
if [[ -z "${VENV_PATH:-}" ]]; then
    echo "ERROR: VENV_PATH envvar is not set"
    exit 1
fi
if [[ -z "${TEST_JOBS:-}" ]]; then
    echo "ERROR: TEST_JOBS envvar is not set"
    exit 1
fi
if [[ -z "${VPP_BUILD_DIR:-}" ]]; then
    echo "ERROR: VPP_BUILD_DIR envvar is not set"
    exit 1
fi
if [[ -z "${VPP_BIN:-}" ]]; then
    echo "ERROR: VPP_BIN envvar is not set"
    exit 1
fi
if [[ -z "${VPP_PLUGIN_PATH:-}" ]]; then
    echo "ERROR: VPP_PLUGIN_PATH envvar is not set"
    exit 1
fi
if [[ -z "${VPP_TEST_PLUGIN_PATH:-}" ]]; then
    echo "ERROR: VPP_TEST_PLUGIN_PATH envvar is not set"
    exit 1
fi
if [[ -z "${VPP_INSTALL_PATH:-}" ]]; then
    echo "ERROR: VPP_INSTALL_PATH envvar is not set"
    exit 1
fi
if [[ -z "${LD_LIBRARY_PATH:-}" ]]; then
    echo "ERROR: LD_LIBRARY_PATH envvar is not set"
    exit 1
fi
