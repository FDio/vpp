...
LOG2_SUBPOOL_SIZES="${LOG2_SUBPOOL_SIZES:-12}"
REBUILD="${REBUILD:-1}"
...
run_worker_count() {
  ...
  local mode_list="$5"
  local batch_list="$6"
  local log2_subpool_size batch mode cmd
...
run_worker_count "$OUTPUT_CSV" 0 "" "$LOG2_SUBPOOL_SIZES" "local refill" "$BATCH_SIZES"
run_worker_count "$OUTPUT_CSV" 2 "2,3" "$LOG2_SUBPOOL_SIZES" "$MULTIWORKER_MODE" "$BATCH_SIZES"
run_worker_count "$OUTPUT_CSV" 4 "2,3,4,5" "$LOG2_SUBPOOL_SIZES" "$MULTIWORKER_MODE" "$BATCH_SIZES"
run_worker_count "$OUTPUT_CSV" 8 "2,3,4,5,6,7,8,9" "$LOG2_SUBPOOL_SIZES" "$MULTIWORKER_MODE" "$BATCH_SIZES"
