...
func LdpIperfUdpMWTest(s *LdpSuite) {
	...
	s.SetupTest()
	s.AssertIperfMinTransfer(ldPreloadIperf(s, "-u -P 5", false), 50)
}

func LdpIperfUdpVppInterruptModeTest(s *LdpSuite) {
	s.AssertIperfMinTransfer(ldPreloadIperf(s, "-u", false), 100)
}
...
func ldpIperfTcpReorder(s *LdpSuite, netInterface *NetInterface, extraIperfArgs string) {
	...
	s.AssertIperfMinTransfer(ldPreloadIperf(s, extraIperfArgs, true), 20)
}
...
func LdpIperfTlsTcpTest(s *LdpSuite) {
	...
	s.AssertIperfMinTransfer(ldPreloadIperf(s, "", false), 100)
}
...
func ldPreloadIperf(s *LdpSuite, extraClientArgs string, isReorder bool) IPerfResult {
	...
	cmd = fmt.Sprintf("iperf3 -c %s -B %s -l 1460 -b 10g -J -p %s %s", serverAddress, clientBindAddress, s.Ports.Port1, extraClientArgs)
