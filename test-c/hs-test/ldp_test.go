...
func LdpIperfUdpMWTest(s *LdpSuite) {
	...
	s.SetupTest()
	s.AssertGreaterEqual(ldPreloadIperf(s, "-u -b 1g -P 5", false), 50)
}

func LdpIperfUdpVppInterruptModeTest(s *LdpSuite) {
	s.AssertGreaterEqual(ldPreloadIperf(s, "-u -b 1g", false), 100)
}
...
func ldpIperfTcpReorder(s *LdpSuite, netInterface *NetInterface, extraIperfArgs string) {
	...
	s.AssertGreaterEqual(ldPreloadIperf(s, extraIperfArgs, true), 20)
}
...
func LdpIperfTlsTcpTest(s *LdpSuite) {
	...
	s.AssertGreaterEqual(ldPreloadIperf(s, "", false), 100)
}
