...
setup_master() {
  ...
  if ! build_calicovpp; then
    echo -e "\e[31m*** Build failed. Reverting repo. Try running 'make -C ../.. wipe' and 'make -C ../.. wipe-release' ***\e[0m"
    restore_repo
    exit 1
  fi
...
rebuild_master() {
  ...
  envsubst < kubernetes/kind-calicovpp-config-template.yaml > kubernetes/kind-calicovpp-config.yaml
  if ! build_calicovpp; then
    red "*** Build failed. Reverting repo. Try running 'make -C ../.. wipe' and 'make -C ../.. wipe-release' ***"
    restore_repo
    exit 1
  fi
  push_tag_to_registry
