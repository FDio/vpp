...
func FelixPolicyChurnTest(s *FelixSuite) {
	...
	out, err := s.ExecVppctlInKubeNode("kind-worker", "show", "npol", "rules")
	s.AssertNil(err, "Failed to run 'show npol rules' on kind-worker")
	...
	out, err = s.ExecVppctlInKubeNode("kind-worker2", "show", "npol", "rules")
	s.AssertNil(err, "Failed to run 'show npol rules' on kind-worker2")
