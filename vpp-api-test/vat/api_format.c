...
static int api_vxlan_add_del_tunnel (vat_main_t * vam)
{
    ...
    if (ipv6_set) {
        *(u64 *)&(mp->src_address[0]) = src6.as_u64[0];
        *(u64 *)&(mp->src_address[8]) = src6.as_u64[1];
        *(u64 *)&(mp->dst_address[0]) = dst6.as_u64[0];
        *(u64 *)&(mp->dst_address[8]) = dst6.as_u64[1];
    } else { 
        *(u32 *)&(mp->src_address[0]) = src4.as_u32;
        *(u32 *)&(mp->dst_address[0]) = dst4.as_u32;
    }
...
(vl_api_vxlan_tunnel_details_t * mp)
{
    ...
    fformat(vam->ofp, "%11d%24U%24U%14d%18d%13d\n",
            ntohl(mp->sw_if_index),
            format_ip46_address, &(mp->src_address[0]), mp->is_ipv6,
            format_ip46_address, &(mp->dst_address[0]), mp->is_ipv6,
