...
vl_api_mpls_fib_details_t_handler (vl_api_mpls_fib_details_t * mp)
{
  ...
  for (i = 0; i < count; i++)
    {
      fformat (vam->ofp, "  weight %d, sw_if_index %d, is_local %d, is_drop %d, is_unreach %d, "
	       "is_prohitbit %d, afi %d, next_hop %U\n",
	       ntohl(fp->weight), ntohl(fp->sw_if_index), fp->is_local, fp->is_drop, fp->is_unreach,
	       fp->is_prohibit, fp->afi, format_ip46_address, fp->next_hop, fp->afi);
      fp++;
    }
}
