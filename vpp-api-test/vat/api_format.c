...
api_ip_mroute_add_del (vat_main_t * vam)
{
  ...
  if (is_ipv6)
    {
      clib_memcpy (mp->grp_address, &v6_grp_address,
                   sizeof (v6_grp_address));
      clib_memcpy (mp->src_address, &v6_src_address,
                   sizeof (v6_src_address));
    }
  else
    {
      clib_memcpy (mp->grp_address, &v4_grp_address,
                   sizeof (v4_grp_address));
      clib_memcpy (mp->src_address, &v4_src_address,
                   sizeof (v4_src_address));
