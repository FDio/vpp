...
static int api_sr_policy_add_del (vat_main_t * vam)
{
  ...
  int is_del = 0;
  u8 * name = 0;
  u8 * this_tunnel_name = 0;
  u8 * tunnel_name = 0;
  u8 ** tunnel_names = 0;
  u8 * tunnel_name_lengths = 0;
  int name_set = 0 ;
  int tunnel_set = 0;
  int j = 0;
  int tunnel_name_len = 0;
  ...
  while (unformat_check_input (i) != UNFORMAT_END_OF_INPUT)
	...
	name_set = 1;
      else if (unformat (i, "tunnel %s", &tunnel_name))
        {
	  memcpy (this_tunnel_name, tunnel_name, vec_len(tunnel_name));
	  vec_add2 (tunnel_names, this_tunnel_name, vec_len(tunnel_name));

	  vec_add1 (tunnel_name_lengths, vec_len(tunnel_name));
	  /*	  if (tunnel_name)
	    {
  ...
  for (j = 0; j < vec_len(tunnel_name_lengths); j++)
    {
      tunnel_name_len += tunnel_name_lengths[j];
      errmsg ("%d tunnel names length %d",j, tunnel_name_len);
    }



  M2(SR_POLICY_ADD_DEL, sr_policy_add_del, tunnel_name_len);
  ...
  memcpy (mp->name, name, vec_len(name));
  memcpy (mp->tunnel_names, tunnel_names, tunnel_name_len);
...
_(sr_tunnel_add_del,                                                    \
  ...
  "[policy <policy_name>]")						\
_(sr_policy_add_del,                                                    \
  "[name <name>] [tunnel <tunnel-name>]* [del]")			\
