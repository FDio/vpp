...
#define _(n,v) else if (unformat(i, n)) {vtr_op = v;}
      foreach_pbb_vtr_op
#undef _
	else if (unformat (i, "translate_qinq"))
	{
	  if (unformat (i, "%d %d", &tmp1, &tmp2))
	    {
	    ...
	    {
	      errmsg
		("translate_qinq operation requires inner and outer tag definition\n");
	      return -99;
	    }
	}
	...
	;
      else if (unformat (i, "vlanid %d", &tmp1))
	vlanid = tmp1;
      else
	{
	  clib_warning ("parse error '%U'", format_unformat_error, i);
...
_(l2_interface_pbb_tag_rewrite,                                         \
  "<intfc> | sw_if_index <nn> \n"                                       \
  "[disable | push | pop | translate_qinq <inner_tag> <outter_tag>] \n" \
