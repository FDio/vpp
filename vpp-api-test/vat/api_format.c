...
_(SPAN_ADD_DEL_REPLY, span_add_del_reply)                               \
...
static int
api_span_add_del (vat_main_t * vam)
{
  unformat_input_t *i = vam->input;
  vl_api_span_add_del_t *mp;
  f64 timeout;
  u32 src_sw_if_index;
  u32 dst_sw_if_index;
  u8 is_add = 1;
...
_(span_add_del, "src <src interface name> [dst <dst interface name>|del]") \
_(span_dump, "")                                                        \
