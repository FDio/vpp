...
cpu_config (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	{
	  if (memcmp
	      (policy_name, sched_policies[0],
	       sizeof (*sched_policies[0])) == 0)
	    tm->sched_policy = SCHED_OTHER;
	  else
	    if (memcmp
		(policy_name, sched_policies[1],
		 sizeof (*sched_policies[1])) == 0)
	    tm->sched_policy = SCHED_BATCH;
	  else
	    if (memcmp
		(policy_name, sched_policies[2],
		 sizeof (*sched_policies[2])) == 0)
	    tm->sched_policy = SCHED_IDLE;
	  else
	    if (memcmp
		(policy_name, sched_policies[3],
		 sizeof (*sched_policies[3])) == 0)
	    tm->sched_policy = SCHED_FIFO;
	  else
	    if (memcmp
		(policy_name, sched_policies[4],
		 sizeof (*sched_policies[4])) == 0)
