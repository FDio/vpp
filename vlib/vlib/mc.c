  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ELOG_TYPE_DECLARE (e) =
      {
      .format = "tx-msg: stream %d local seq %d attempt %d",.format_args =
	  "i4i4i4",};
      struct
      {
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ELOG_TYPE_DECLARE (e) =
      {
      .format = "delete peer %s from all_peer_bitmap",.format_args = "T4",};
      struct
      {
	u32 peer;
      } *ed = 0;
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ELOG_TYPE_DECLARE (e) =
      {
	.format = "get_or_create %s peer %s stream %d seq %d",.format_args =
	  "t4T4i4i4",.n_enum_strings = 2,.enum_strings =
	{
	"old", "new",}
      ,};
      struct
...
mc_resend_retired (mc_main_t * mcm, mc_stream_t * s, u32 local_sequence)
{
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ELOG_TYPE_DECLARE (e) =
      {
      .format = "resend-retired: search for local seq %d",.format_args =
	  "i4",};
      struct
      {
	u32 local_sequence;
  ...
  clib_fifo_foreach (retry, s->retired_fifo, (
					       {
					       if (retry->local_sequence ==
						   local_sequence)
					       {
					       elog_tx_msg (mcm, s->index,
							    retry->
							    local_sequence,
							    -13);
					       mcm->transport.tx_buffer (mcm->
									 transport.
									 opaque,
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ...
      {
      .format =
	  "resend-retired: FAILED search for local seq %d",.format_args =
	  "i4",};
      struct
      {
	u32 local_sequence;
	  ...
	  if (MC_EVENT_LOGGING > 0)
	    {
	      ...
	      ELOG_TYPE_DECLARE (t) =
	      {
	      .format = "resend local seq %d attempt %d",.format_args =
		  "i4i4",};
...
unserialize_mc_register_stream_name (serialize_main_t * m, va_list * va)
{
  ...
  if ((p = hash_get_mem (mcm->stream_index_by_name, name)))
      ...
      if (MC_EVENT_LOGGING > 0)
	{
	  ELOG_TYPE_DECLARE (e) =
	  {
	  .format = "stream index %d already named %s",.format_args =
	      "i4s16",};
	  struct
	  {
	    u32 stream_index;
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ELOG_TYPE_DECLARE (e) =
      {
      .format = "stream index %d named %s",.format_args = "i4s16",};
      struct
      {
	u32 stream_index;
	char name[16];
...
{
.name = "mc_register_stream_name",.serialize =
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ELOG_TYPE_DECLARE (e) =
      {
      .format = "stream index %d join request %s",.format_args = "i4s16",};
      struct
      {
	u32 stream_index;
	char name[16];
			     ...
			     u32 buffer_index)
{
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ...
      {
      .format =
	  "rx-msg: peer %s stream %d rx seq %d seq_cmp %d",.format_args =
	  "T4i4i4i4",};
      struct
      {
  ...
  if (1)
      ...
      if (MC_EVENT_LOGGING > 0)
	{
	  ELOG_TYPE_DECLARE (e) =
	  {
	  .format = "tx-ack: stream %d local seq %d",.format_args = "i4i4",};
	  struct
	  {
	    u32 stream_index;
	    u32 local_sequence;
			 ...
			 u32 buffer_index)
{
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ...
      {
      .format =
	  "rx-ack: local seq %d peer %s seq_cmp_result %d",.format_args =
	  "i4T4i4",};
      struct
      {
	u32 local_sequence;
  ...
  if (p == 0)
      ...
      if (MC_EVENT_LOGGING > 0)
	{
	  ...
	  {
	  .format =
	      "ack: for seq %d from peer %s no fifo elt",.format_args =
	      "i4T4",};
	  struct
	  {
	    u32 seq;
  ...
  if (!peer_created &&
      ...
      if (MC_EVENT_LOGGING > 0)
	{
	  ELOG_TYPE_DECLARE (t) =
	  {
	  .format = "dup-ack: for seq %d from peer %s",.format_args =
	      "i4T4",};
	  struct
	  {
	    u32 seq;
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ELOG_TYPE_DECLARE (t) =
      {
      .format = "ack: for seq %d from peer %s",.format_args = "i4T4",};
      struct
      {
	u32 seq;
	u32 peer;
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ...
      {
      .format =
	  "ack: retire fifo elt loc seq %d after %d acks",.format_args =
	  "i4i4",};
      struct
      {
	u32 seq;
  ...
  if (MC_EVENT_LOGGING > 0)
    {
      ELOG_TYPE_DECLARE (t) =
      {
      .format = "catchup-request: from %s stream %d",.format_args = "T4i4",};
      struct
      {
	u32 peer, stream;
      } *ed;
	  ...
	  if (MC_EVENT_LOGGING)
	    {
	      ELOG_TYPE_DECLARE (t) =
	      {
	      .format = "catchup replay local sequence 0x%x",.format_args =
		  "i4",};
	      struct
	      {
		u32 local_sequence;
	  ...
	  if (MC_EVENT_LOGGING)
	    {
	      ELOG_TYPE_DECLARE (t) =
	      {
	      .format = "catchup discard local sequence 0x%x",.format_args =
		  "i4",};
	      struct
	      {
		u32 local_sequence;
	  ...
	  if (MC_EVENT_LOGGING > 1)
	    {
	      ELOG_TYPE_DECLARE (e) =
	      {
	      .format = "tx-massert: peer %s global seq %u",.format_args =
		  "T4i4",};
	      struct
	      {
  ...
  if (MC_EVENT_LOGGING > 1)
    {
      ...
      {
      .format =
	  "rx-massert: peer %s global seq %u upd %d slave %d",.format_args =
	  "T4i4i1i1",};
      struct
      {
	u32 peer;
  ...
  if (MSG_ID_DEBUG && MC_EVENT_LOGGING > 0)
    {
      ELOG_TYPE_DECLARE (e) =
      {
      .format = "serialize-msg: %s index %d",.format_args = "T4i4",};
      struct
      {
	u32 c[2];
      } *ed;
			...
			mc_stream_t * s, serialize_main_t * m)
{
  ...
  if (!(si == ~0 || MSG_ID_DEBUG))
      ...
      if (MSG_ID_DEBUG && MC_EVENT_LOGGING > 0)
	{
	  ELOG_TYPE_DECLARE (e) =
	  {
	  .format = "unserialize-msg: %s rx index %d",.format_args = "T4i4",};
	  struct
	  {
	    u32 c[2];
	  } *ed;
	  ...
	  if (MC_EVENT_LOGGING > 0)
	    {
	      ELOG_TYPE_DECLARE (e) =
	      {
	      .format = "msg-bind: stream %d %s to index %d",.format_args =
		  "i4T4i4",};
	      struct
	      {
		u32 c[3];
	  ...
	  if (gi != sm->global_index && MC_EVENT_LOGGING > 0)
	    {
	      ...
	      {
	      .format =
		  "msg-id-ERROR: %s index %d expected %d",.format_args =
		  "T4i4i4",};
	      struct
	      {
		u32 c[3];
